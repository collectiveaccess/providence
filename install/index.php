		...
		if ( is_array( $_FILES['files']['tmp_name'] ) ) {
			foreach ( $_FILES['files']['tmp_name'] as $vn_i => $vs_tmp_name ) {
				...
				if ( ! ( @$vo_profile->schemaValidate( $vs_profile_dir . "/profile.xsd" ) ) ) {
					$va_response['skippedMessage'][] = _t( '%1 is not a valid profile',
						$_FILES['files']['name'][ $vn_i ] );
					continue;
				}
				...
				if ( @copy( $vs_tmp_name, $vs_profile_dir . '/' . $_FILES['files']['name'][ $vn_i ] ) ) {
					...
					$va_response['uploadMessage'][] = $vb_exists ? _t( 'Updated %1', $_FILES['files']['name'][ $vn_i ] )
						: _t( 'Added %1', $_FILES['files']['name'][ $vn_i ] );
				}
...
$pb_overwrite = ( isset( $_REQUEST['overwrite'] )
                  && defined( '__CA_ALLOW_INSTALLER_TO_OVERWRITE_EXISTING_INSTALLS__' ) )
	? (bool) $_REQUEST['overwrite'] : false;
$pb_debug     = ( isset( $_REQUEST['debug'] ) && defined( '__CA_ALLOW_INSTALLER_TO_OVERWRITE_EXISTING_INSTALLS__' ) )
	? (bool) $_REQUEST['debug'] : false;
