...
<div id='box'>
	...
	<div id="content">
		...
		if ( $vo_installer->numErrors() ) {
			...
			if ( $vo_installer->numErrors() ) {
				caSetMessage( "There were errors loading the database schema: " . join( "; ",
				...
				try {
					...
					if ( $vo_installer->numErrors() ) {
						...
						caSetMessage( "<div class='contentFailure'><div class='contentFailureHead'>There were errors during installation:</div><ul>"
						              . join( "", $va_errors ) . "</ul><br/>{$vs_time}</div>" );
					} else {
						$vs_message
							= "<div class='contentSuccess'><span style='font-size:18px;'><b>Installation was successful!</b></span><br/>You can now <a href='../index.php?action=login'>login</a> with ";
					...
					caSetMessage( "<div class='contentFailure'><div class='contentFailureHead'>There were errors during installation:</div>"
					              . $e->getMessage() . "</div>" );
				}
			}
		}
