...
<div id='box'>
    ...
    <div id="content">
        ...
        if ($vo_installer->numErrors()) {
            ...
            if ($vo_installer->numErrors()) {
                caSetMessage(
                    "There were errors loading the database schema: " . join("; ", $vo_installer->getErrors())
                );
            } else {
                try {
                    $vn_progress += 7;
                    ...
                    if ($vo_installer->numErrors()) {
                        ...
                        caSetMessage(
                            "<div class='contentFailure'><div class='contentFailureHead'>There were errors during installation:</div><ul>" . join(
                                "",
                                $va_errors
                            ) . "</ul><br/>{$vs_time}</div>"
                        );
                    ...
                    caSetMessage(
                        "<div class='contentFailure'><div class='contentFailureHead'>There were errors during installation:</div>" . $e->getMessage(
                        ) . "</div>"
                    );
                }
            }
