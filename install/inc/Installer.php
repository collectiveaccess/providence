...
class Installer {
	...
	public function __construct(
		$ps_profile_dir, $ps_profile_name, $ps_admin_email = null, $pb_overwrite = false, $pb_debug = false,
		$pb_skip_load = false, $pb_log_output = false
	) {
	...
	public static function getFromString(
		$ps_profile_xml, $ps_admin_email = '', $pb_overwrite = false, $pb_log_output = false
	) {
		$o_installer = new Installer( __CA_BASE_DIR__ . '/install/profiles/xml', 'export', $ps_admin_email,
			$pb_overwrite, false, true, $pb_log_output );
		...
		if ( $this->opo_base ) {
			...
			if ( $this->opb_debug ) {
				...
				$vb_return               = $vo_profile->schemaValidate( $this->ops_profile_dir . "/profile.xsd" )
				                           && $vo_base->schemaValidate( $this->ops_profile_dir . "/profile.xsd" );
	...
	protected static function addLabelsFromXMLElement( $t_instance, $po_labels, $pa_locales, $pb_force_preferred = false
		...
		if ( ! $po_labels || ! $po_labels->children() ) {
			$t_instance->addLabel( array( $t_instance->getLabelDisplayField() => "???" ), array_shift( $pa_locales ),
				false, true );
		...
		foreach ( $po_labels->children() as $vo_label ) {
			...
			$va_existing_labels = $vb_preferred ? $t_instance->getPreferredLabels( array( $vn_locale_id ) )
				: $t_instance->getNonPreferredLabels( array( $vn_locale_id ) );
			if (
				is_array( $va_existing_labels ) && ( sizeof( $va_existing_labels ) > 0 )
				&& ( $vn_label_id
					= $va_existing_labels[ (int) $t_instance->getPrimaryKey() ][ (int) $vn_locale_id ][0]['label_id'] )
			) {
				$vn_label_id = $t_instance->editLabel( $vn_label_id, $va_label_values, $vn_locale_id, null,
					$vb_preferred );
			} else {
		...
		if ( sizeof( $this->opa_metadata_element_deferred_settings_processing ) ) {
			foreach ( $this->opa_metadata_element_deferred_settings_processing as $vs_element_code => $va_settings ) {
				...
				foreach ( $va_settings as $vs_setting_name => $va_setting_values ) {
					if ( ! isset( $va_available_settings[ $vs_setting_name ] ) ) {
						continue;
					}

					if ( isset( $va_available_settings[ $vs_setting_name ]['multiple'] )
					     && $va_available_settings[ $vs_setting_name ]['multiple']
					) {
	 ...
	 *
	 * @param callable $f_callback Function to be called for each SQL statement in the schema. Function is passed four
	 *                             parameters: the SQL code of the statement, the table name, the number of the table
	 *                             being loaded and the total number of tables.
	 *
	...
	public function loadSchema( $f_callback = null ) {
		...
		while ( $qr_tables->nextRow() ) {
			...
			if ( in_array( $vs_table, $va_ca_tables ) ) {
				$this->addError( "Table " . $vs_table
				                 . " already exists; have you already installed CollectiveAccess?" );
		...
		foreach ( $va_schema_statements as $vs_statement ) {
			...
			if ( is_callable( $f_callback )
			     && preg_match( '!create[ ]+table[ ]+([A-Za-z0-9_]+)!i', $vs_statement, $va_matches )
			) {
				$vn_i ++;
			...
			if ( $this->opo_db->numErrors() ) {
				$this->addError( "Error while loading the database schema: " . join( "; ",
						$this->opo_db->getErrors() ) );
	...
	public function processLocales() {
		...
		if ( $this->ops_base_name ) {
			...
			foreach ( $this->opo_profile->locales->children() as $vo_locale ) {
				$key = self::getAttribute( $vo_locale, "lang" ) . '_' . self::getAttribute( $vo_locale, "country" )
				       . '_' . self::getAttribute( $vo_locale, "dialect" );
				if ( isset( $va_locales[ $key ] ) ) {
					continue;
				}
		...
		foreach ( $va_locales as $vo_locale ) {
			...
			$vs_locale_code              = $vs_dialect ? $vs_language . "_" . $vs_country . '_' . $vs_dialect
				: $vs_language . "_" . $vs_country;

			if ( isset( $this->opa_locales[ $vs_locale_code ] )
			     && ( $vn_locale_id
					= $this->opa_locales[ $vs_locale_code ] )
			...
			if ( $t_locale->numErrors() ) {
				$this->addError( "There was an error while inserting locale {$vs_locale_code}: " . join( " ",
						$t_locale->getErrors() ) );
			}
		...
		if ( ! $vn_locale_id ) {
			throw new Exception( "The locale default is set to a non-existing locale. Try adding '" . $va_locales[0]
			                     . "' to your profile." );
		}
	...
	public function processLists( $f_callback = null ) {
		...
		foreach ( $va_lists as $vo_list ) {
			...
			if ( ! ( $t_list = ca_lists::find( array( 'list_code' => $vs_list_code ),
			...
			if ( $t_list->numErrors() ) {
				$this->addError( "There was an error while inserting list {$vs_list_code}: " . join( " ",
						$t_list->getErrors() ) );
			} else {
				...
				if ( $t_list->numErrors() ) {
					$this->addError( "There was an error while inserting list label for {$vs_list_code}: " . join( " ",
							$t_list->getErrors() ) );
				}
				if ( $vo_list->items ) {
	...
	protected function processListItems( $t_list, $po_items, $pn_parent_id ) {
		foreach ( $po_items->children() as $vo_item ) {
			...
			if ( $vn_item_id = caGetListItemID( $t_list->get( 'list_code' ), $vs_item_idno,
				array( 'dontCache' => true ) )
			) {
				...
				$t_item = $t_list->editItem( $vn_item_id, $vs_item_value, $vn_enabled, $vn_default, $pn_parent_id,
					$vs_item_idno, '', (int) $vs_status, (int) $vs_access, (int) $vs_rank, $vs_color );
			} else {
				$this->logStatus( _t( 'List item with idno %1 is a new item', $vs_item_idno ) );
				if ( $vb_deleted ) {
					continue;
				} else {
					$t_item = $t_list->addItem( $vs_item_value, $vn_enabled, $vn_default, $pn_parent_id, $vn_type_id,
						$vs_item_idno, '', (int) $vs_status, (int) $vs_access, (int) $vs_rank, $vs_color );
				}
			}

			if ( ( $t_list->numErrors() > 0 ) || ! is_object( $t_item ) ) {
				$this->addError( "There was an error while inserting list item {$vs_item_idno}: " . join( " ",
						$t_list->getErrors() ) );
				...
				if ( $vo_item->settings ) {
					...
					$t_item->update();
					if ( $t_item->numErrors() ) {
						$this->addError( "There was an error while adding a setting for list item with idno {$vs_item_idno}: "
						                 . join( " ", $t_item->getErrors() ) );
					}
				}
				self::addLabelsFromXMLElement( $t_item, $vo_item->labels, $this->opa_locales );
				if ( $t_item->numErrors() ) {
					$this->addError( "There was an error while inserting list item label for {$vs_item_idno}: "
					                 . join( " ", $t_item->getErrors() ) );
				}
			}
		...
		foreach ( $va_elements as $vs_element_code => $vo_element ) {
			if ( $vn_element_id = $this->processMetadataElement( $vo_element, null ) ) {
				...
				if ( sizeof( $vo_element->typeRestrictions->children() ) ) {
					$this->opo_db->query( 'DELETE FROM ca_metadata_type_restrictions WHERE element_id=?',
						$vn_element_id );
				}
				...
				foreach ( $vo_element->typeRestrictions->children() as $vo_restriction ) {
					...
					if ( $t_restriction->numErrors() ) {
						$this->addError( "There was an error while inserting type restriction {$vs_restriction_code} for metadata element {$vs_element_code}: "
						                 . join( "; ", $t_restriction->getErrors() ) );
					}

					$this->logStatus( _t( 'Successfully added type restriction %1 for element %2', $vs_restriction_code,
						$vs_element_code ) );
				}
			}
		}
	...
	private function processMetadataElement( $po_element, $pn_parent_id ) {
		...
		if ( ( $vn_datatype = ca_metadata_elements::getAttributeTypeCode( self::getAttribute( $po_element,
				"datatype" ) ) ) === false
		) {
		...
		$this->_processSettings( $t_md_element, $po_element->settings,
			[ 'settingsInfo' => $t_md_element->getAvailableSettings() ] );
		...
		if ( $t_md_element->numErrors() ) {
			$this->addError( "There was an error while inserting metadata element {$vs_element_code}: " . join( " ",
					$t_md_element->getErrors() ) );
	...
	public function processMetadataDictionary() {
		...
		foreach ( $this->opo_profile->metadataDictionary->children() as $vo_entry ) {
			...
			if ( $t_entry->numErrors() > 0 || ! ( $t_entry->getPrimaryKey() > 0 ) ) {
				$this->addError( "There were errors while adding dictionary entry: " . join( ';',
						$t_entry->getErrors() ) );
			...
			if ( $vo_entry->rules ) {
				foreach ( $vo_entry->rules->children() as $vo_rule ) {
					...
					$t_rule->insert();
					if ( $t_rule->numErrors() ) {
						$this->addError( "There were errors while adding dictionary rule: " . join( ';',
								$t_rule->getErrors() ) );
						continue;
					}
				}
	...
	public function processUserInterfaces() {
		...
		foreach ( $va_uis as $vs_ui_code => $vo_ui ) {
			...
			// create ui row
			if ( ! ( $t_ui = ca_editor_uis::find( array( 'editor_code' => $vs_ui_code, 'editor_type' => $vn_type ),
				array( 'returnAs' => 'firstModelInstance' ) ) )
			) {
			...
			if ( $vs_color = self::getAttribute( $vo_ui, "color" ) ) {
				$t_ui->set( 'color', $vs_color );
			}
			...
			if ( $vo_ui->typeRestrictions ) {
				...
				$this->logStatus( _t( 'Successfully nuked all type restrictions for user interface with code %1',
					$vs_ui_code ) );
				...
				foreach ( $vo_ui->typeRestrictions->children() as $vo_restriction ) {
					...
					if ( strlen( $vs_restriction_type ) > 0 ) {
						// interstitial with type restriction -> code is relationship type code
						if ( $t_instance instanceof BaseRelationshipModel ) {
							$vn_type_id = $t_rel_types->getRelationshipTypeID( $t_instance->tableName(),
								$vs_restriction_type );
						...
						$t_ui->addTypeRestriction( $vn_type_id,
							[ 'includeSubtypes' => self::getAttribute( $vo_restriction, "includeSubtypes" ) ] );

						$this->logStatus( _t( 'Successfully added type restriction %1 for user interface with code %2',
							$vs_restriction_type, $vs_ui_code ) );
					}
				}
			}
			...
			if ( $vs_type_restrictions = self::getAttribute( $vo_ui, "typeRestrictions" ) ) {
				...
				foreach ( $va_ids as $vn_i => $vn_type_id ) {
					$t_ui->addTypeRestriction( $vn_type_id,
						[ 'includeSubtypes' => self::getAttribute( $vo_ui, "includeSubtypes" ) ] );
					$this->logStatus( _t( 'Successfully added type restriction %1 for user interface with code %2',
						$va_codes[ $vn_i ], $vs_ui_code ) );
				}
			}
			...
			foreach ( $vo_ui->screens->children() as $vo_screen ) {
				...
				$this->logStatus( _t( 'Processing screen with code %1 for user interface with code %2', $vs_screen_idno,
					$vs_ui_code ) );
				...
				if ( $t_ui_screens->getPrimaryKey() ) {
					$this->logStatus( _t( 'Screen with code %1 for user interface with code %2 already exists',
						$vs_screen_idno, $vs_ui_code ) );
				} else {
					$this->logStatus( _t( 'Screen with code %1 for user interface with code %2 is new', $vs_screen_idno,
						$vs_ui_code ) );
				}

				if ( self::getAttribute( $vo_screen, 'deleted' ) && $t_ui_screens->getPrimaryKey() ) {
					$this->logStatus( _t( 'Deleting screen with code %1 for user interface with code %2',
						$vs_screen_idno, $vs_ui_code ) );
				...
				if ( $vs_color = self::getAttribute( $vo_screen, "color" ) ) {
					$t_ui_screens->set( 'color', $vs_color );
				}
				...
				if ( $t_ui_screens->numErrors() ) {
					$this->addError( "Errors inserting UI screen {$vs_screen_idno} for UI {$vs_ui_code}: " . join( "; ",
							$t_ui_screens->getErrors() ) );

					return false;
				}

				$this->logStatus( _t( 'Successfully updated/inserted screen with code %1 for user interface with code %2',
					$vs_screen_idno, $vs_ui_code ) );
				...
				if ( sizeof( $vo_screen->bundlePlacements->children() ) ) {
					$this->opo_db->query( 'DELETE FROM ca_editor_ui_bundle_placements WHERE screen_id=?',
						$t_ui_screens->getPrimaryKey() );
				}

				$this->logStatus( _t( 'Successfully nuked all bundle placements for screen with code %1 for user interface with code %2',
					$vs_screen_idno, $vs_ui_code ) );
				...
				if ( $vo_screen->userAccess ) {
					...
					foreach ( $vo_screen->userAccess->children() as $vo_permission ) {
						...
						$vn_access = $this->_convertUserGroupAccessStringToInt( self::getAttribute( $vo_permission,
							'access' ) );

						if ( ! $t_user->load( array( 'user_name' => $vs_user ) ) ) {
							continue;
						}
						if ( $vn_access ) {
				...
				if ( $vo_screen->roleAccess ) {
					...
					if ( sizeof( $va_ui_screen_roles ) > 0 ) {
						...
						foreach ( $all_roles as $role_id => $role_info ) {
							if ( ! isset( $va_ui_screen_roles[ $role_id ] ) ) {
								$va_ui_screen_roles[ $role_id ] = 0;
							}
						}
				...
				foreach ( $vo_screen->bundlePlacements->children() as $vo_placement ) {
					...
					if ( $vs_bundle_type_restrictions ) {
						...
						// as settings on the placement record.
						if ( $t_instance instanceof BaseRelationshipModel ) {
							$va_ids = caMakeRelationshipTypeIDList( $t_instance->tableNum(),
								preg_split( "![ ,;\|]!", $vs_bundle_type_restrictions ) );
						} elseif ( $t_instance instanceof ca_representation_annotations ) {
							$va_ids = [];
							...
							$va_ids = caMakeTypeIDList( $t_instance->tableNum(),
								preg_split( "![ ,;\|]!", $vs_bundle_type_restrictions ),
								[ 'dontIncludeSubtypesInTypeRestriction' => true ] );
						}

						if ( ! $vo_placement->settings ) {
							$vo_placement->addChild( "settings" );
						}
					...
					$va_settings = $this->_processSettings( null, $vo_placement->settings, [
						'settingsInfo' => array_merge( $t_placement->getAvailableSettings(),
							is_array( $va_available_bundles[ $vs_bundle ]['settings'] )
								? $va_available_bundles[ $vs_bundle ]['settings'] : [] )
					] );
					$this->logStatus( _t( 'Adding bundle %1 with code %2 for screen with code %3 and user interface with code %4',
						$vs_bundle, $vs_placement_code, $vs_screen_idno, $vs_ui_code ) );

					if ( ! $t_ui_screens->addPlacement( $vs_bundle, $vs_placement_code, $va_settings, null,
						array( 'additional_settings' => $va_available_bundles[ $vs_bundle ]['settings'] ) )
					) {
				...
				if ( $vo_screen->typeRestrictions ) {
					...
					if ( sizeof( $vo_screen->typeRestrictions->children() ) ) {
						$this->opo_db->query( 'DELETE FROM ca_editor_ui_screen_type_restrictions WHERE screen_id=?',
							$t_ui_screens->getPrimaryKey() );
					}

					$this->logStatus( _t( 'Successfully nuked all type restrictions for screen with code %1 for user interface with code %2',
						$vs_screen_idno, $vs_ui_code ) );
					...
					foreach ( $vo_screen->typeRestrictions->children() as $vo_restriction ) {
						...
						if ( strlen( $vs_restriction_type ) > 0 ) {
							...
							if ( $t_instance instanceof BaseRelationshipModel ) {
								...
								$vs_type_list_name = $t_instance->getFieldListCode( $t_instance->getTypeFieldName() );
								$vn_type_id        = $t_list->getItemIDFromList( $vs_type_list_name,
									$vs_restriction_type );
							}
							...
							$this->logStatus( _t( 'Successfully added type restriction %1 for screen with code %2 for user interface with code %3',
								$vs_restriction_type, $vs_screen_idno, $vs_ui_code ) );
						}
					}
				}
				...
				if ( $vs_type_restrictions = self::getAttribute( $vo_screen, "typeRestrictions" ) ) {
					...
					foreach ( $va_ids as $vn_i => $vn_type_id ) {
						$t_ui_screens->addTypeRestriction( $vn_type_id,
							[ 'includeSubtypes' => self::getAttribute( $vo_screen, "includeSubtypes" ) ] );
						$this->logStatus( _t( 'Successfully added type restriction %1 for screen with code %2 for user interface with code %3',
							$va_codes[ $vn_i ], $vs_screen_idno, $vs_ui_code ) );
					}
				}
			}
			...
			if ( $vo_ui->roleAccess ) {
				...
				if ( sizeof( $va_ui_roles ) > 0 ) {
					$all_roles = $t_role->getRoleList();
					foreach ( $all_roles as $role_id => $role_info ) {
						if ( ! isset( $va_ui_roles[ $role_id ] ) ) {
							$va_ui_roles[ $role_id ] = 0;
						}
					}
	...
	public function processRelationshipTypes() {
		...
		// get list items
		$qr_list_item_result
			= $this->opo_db->query( "SELECT * FROM ca_list_items cli INNER JOIN ca_list_item_labels AS clil ON clil.item_id = cli.item_id" );
		while ( $qr_list_item_result->nextRow() ) {
			$vs_type_code = $va_list_names[ $qr_list_item_result->get( 'list_id' ) ];
			$va_list_item_ids[ $vs_type_code ][ $qr_list_item_result->get( 'item_value' ) ]
			              = $qr_list_item_result->get( 'item_id' );
		}
		...
		foreach ( $va_rel_tables as $vs_table => $vo_rel_table ) {
			...
			if ( $t_rel_type->numErrors() ) {
				$this->addError( "Errors inserting relationship root for {$vs_table}: " . join( "; ",
						$t_rel_type->getErrors() ) );
			...
			$this->processRelationshipTypesForTable( $vo_rel_table->types, $vn_table_num, $vs_left_table,
				$vs_right_table, $vn_parent_id, $va_list_item_ids );
		}
	...
	# --------------------------------------------------
	private function processRelationshipTypesForTable(
		$po_relationship_types, $pn_table_num, $ps_left_table, $ps_right_table, $pn_parent_id, $pa_list_item_ids
	) {
		...
		foreach ( $po_relationship_types->children() as $vo_type ) {
			...
			if (
				...
				$this->logStatus( _t( 'Adding left type restriction %1 for relationship type with code %2',
					$vs_left_subtype_code, $vs_type_code ) );
			...
			if (
				...
				$this->logStatus( _t( 'Adding right type restriction %1 for relationship type with code %2',
					$vs_right_subtype_code, $vs_type_code ) );

				if ( isset( $pa_list_item_ids[ $vs_list_code ][ $vs_right_subtype_code ] ) ) {
					$t_rel_type->set( 'sub_type_right_id',
						$pa_list_item_ids[ $vs_list_code ][ $vs_right_subtype_code ] );
			...
			if ( $t_rel_type->numErrors() ) {
				$this->addError( "Errors inserting relationship {$vs_type_code}: " . join( "; ",
						$t_rel_type->getErrors() ) );
			...
			if ( $vo_type->types ) {
				$this->processRelationshipTypesForTable( $vo_type->types, $pn_table_num, $ps_left_table,
					$ps_right_table, $t_rel_type->getPrimaryKey(), $pa_list_item_ids );
			}
		}
	}
	...
	public function processRoles() {
		...
		foreach ( $va_roles as $vs_role_code => $vo_role ) {
			...
			if ( ! ( $t_role = ca_user_roles::find( array( 'code' => (string) $vs_role_code ),
				array( 'returnAs' => 'firstModelInstance' ) ) )
			) {
			...
			if ( $vo_role->actions ) {
				...
				$this->logStatus( _t( 'Role actions for user role with code %1 are: %2', $vs_role_code,
					join( ',', $va_actions ) ) );
			}
			...
			if ( $t_role->numErrors() ) {
				$this->addError( "Errors inserting access role {$vs_role_code}: " . join( "; ",
						$t_role->getErrors() ) );
			...
			if ( $vo_role->bundleLevelAccessControl ) {
				...
				if ( sizeof( $vo_role->bundleLevelAccessControl->children() ) > 0 ) {
					...
					$this->logStatus( _t( 'Successfully nuked all bundle level access control items for user role with code %1',
						$vs_role_code ) );
				}
				...
				foreach ( $vo_role->bundleLevelAccessControl->children() as $vo_permission ) {
					...
					$vn_permission_access = $this->_convertACLStringToConstant( self::getAttribute( $vo_permission,
						'access' ) );

					if ( ! $t_role->setAccessSettingForBundle( $vs_permission_table, $vs_permission_bundle,
						$vn_permission_access )
					) {
					...
					$this->logStatus( _t( 'Added bundle level access control item for user role with code %1: table %2, bundle %3, access %4',
						$vs_role_code, $vs_permission_table, $vs_permission_bundle, $vn_permission_access ) );
				}
			}
			...
			if ( $vo_role->typeLevelAccessControl ) {
				...
				if ( sizeof( $vo_role->typeLevelAccessControl->children() ) > 0 ) {
					...
					$this->logStatus( _t( 'Successfully nuked all type level access control items for user role with code %1',
						$vs_role_code ) );
				}
				...
				foreach ( $vo_role->typeLevelAccessControl->children() as $vo_permission ) {
					...
					if ( ! $t_role->setAccessSettingForType( $vs_permission_table, $vs_permission_type,
						$vn_permission_access )
					) {
					...
					$this->logStatus( _t( 'Added type level access control item for user role with code %1: table %2, type %3, access %4',
						$vs_role_code, $vs_permission_table, $vs_permission_type, $vn_permission_access ) );
				}
			}
			...
			if ( $vo_role->sourceLevelAccessControl ) {
				...
				if ( sizeof( $vo_role->sourceLevelAccessControl->children() ) > 0 ) {
					...
					$this->logStatus( _t( 'Successfully nuked all source level access control items for user role with code %1',
						$vs_role_code ) );
				}
				...
				foreach ( $vo_role->sourceLevelAccessControl->children() as $vo_permission ) {
					...
					if ( ! $t_role->setAccessSettingForSource( $vs_permission_table, $vs_permission_source,
						$vn_permission_access, (bool) $vs_permission_default )
					) {
					...
					$this->logStatus( _t( 'Added source level access control item for user role with code %1: table %2, source %3, access %4',
						$vs_role_code, $vs_permission_table, $vs_permission_source, $vn_permission_access ) );
				}
			}
		}
	...
	public function processDisplays() {
		...
		foreach ( $va_displays as $vo_display ) {
			...
			if ( ! ( $t_display = ca_bundle_displays::find( array( 'display_code' => $vs_display_code ),
				array( 'returnAs' => 'firstModelInstance' ) ) )
			) {
			...
			if ( $t_display->numErrors() ) {
				$this->addError( "There was an error while inserting display {$vs_display_code}: " . join( " ",
						$t_display->getErrors() ) );
			} else {
				...
				if ( $t_display->numErrors() ) {
					$this->addError( "There was an error while inserting display label for {$vs_display_code}: "
					                 . join( " ", $t_display->getErrors() ) );
				}
			...
			if ( $vo_display->typeRestrictions ) {
				...
				if ( sizeof( $vo_display->typeRestrictions->children() ) ) {
					$this->opo_db->query( 'DELETE FROM ca_bundle_display_type_restrictions WHERE display_id=?',
						$t_display->getPrimaryKey() );
					$this->logStatus( _t( 'Successfully nuked all type restrictions for display with code %1',
						$vs_display_code ) );
				}
				...
				foreach ( $vo_display->typeRestrictions->children() as $vo_restriction ) {
					...
					$t_display->addTypeRestriction( array_pop( caMakeTypeIDList( $vn_table_num, [ $vs_type ],
						[ 'dontIncludeSubtypesInTypeRestriction' => true ] ) ),
						[ 'includeSubtypes' => (bool) $vo_restriction->includeSubtypes ? 1 : 0 ] );

					if ( $t_display->numErrors() ) {
						$this->addError( "There was an error while inserting type restriction {$vs_restriction_code} in display {$vs_display_code}: "
						                 . join( "; ", $t_display->getErrors() ) );
					}

					$this->logStatus( _t( 'Added type restriction with code %1 and type %2 for display with code %3',
						$vs_restriction_code, $vs_type, $vs_display_code ) );
				}
			}
			if ( $vs_type_restrictions = self::getAttribute( $vo_display, "typeRestrictions" ) ) {
				...
				foreach ( $va_ids as $vn_i => $vn_type_id ) {
					$t_display->addTypeRestriction( $vn_type_id,
						[ 'includeSubtypes' => self::getAttribute( $vo_display, "includeSubtypes" ) ] );
					$this->logStatus( _t( 'Added type restriction with type %1 for display with code %2',
						$va_codes[ $vn_i ], $vs_display_code ) );
				}
			}
			...
			if ( $vo_display->roleAccess ) {
				...
				if ( sizeof( $va_display_roles ) > 0 ) {
					$all_roles = $t_role->getRoleList();
					foreach ( $all_roles as $role_id => $role_info ) {
						if ( ! isset( $va_display_roles[ $role_id ] ) ) {
							$va_display_roles[ $role_id ] = 0;
						}
					}
	...
	private function processDisplayPlacements( $t_display, $po_placements ) {
		...
		if ( sizeof( $po_placements->children() ) ) {
			$this->logStatus( _t( 'Successfully nuked all placements for display with code %1',
				$t_display->get( 'display_code' ) ) );
			$this->opo_db->query( 'DELETE FROM ca_bundle_display_placements WHERE display_id=?',
				$t_display->getPrimaryKey() );
		}
		...
		foreach ( $po_placements->children() as $vo_placement ) {
			...
			$va_settings = $this->_processSettings( null, $vo_placement->settings );
			$t_display->addPlacement( $vs_bundle, $va_settings, $vn_i,
				array( 'additional_settings' => $va_available_bundles[ $vs_bundle ]['settings'] ) );
			if ( $t_display->numErrors() ) {
				$this->addError( "There was an error while inserting display placement {$vs_code}: " . join( " ",
						$t_display->getErrors() ) );

				return false;
			}

			$this->logStatus( _t( 'Added bundle placement %1 with code %2 for display with code %3', $vs_bundle,
				$vs_code, $t_display->get( 'display_code' ) ) );
			$vn_i ++;
		}
	...
	public function processSearchForms() {
		...
		foreach ( $va_forms as $vo_form ) {
			...
			if ( ! ( $t_form = ca_search_forms::find( array(
				'form_code' => (string) $vs_form_code,
			...
			if ( $t_form->numErrors() ) {
				$this->addError( "There was an error while inserting search form {$vs_form_code}: " . join( " ",
						$t_form->getErrors() ) );
			} else {
				...
				if ( $t_form->numErrors() ) {
					$this->addError( "There was an error while inserting search form label for {$vs_form_code}: "
					                 . join( " ", $t_form->getErrors() ) );
				}
			...
			if ( $vo_form->typeRestrictions ) {
				...
				if ( sizeof( $vo_form->typeRestrictions->children() ) ) {
					$this->opo_db->query( 'DELETE FROM ca_search_form_type_restrictions WHERE form_id=?',
						$t_form->getPrimaryKey() );
					$this->logStatus( _t( 'Successfully nuked all type restrictions for form with code %1',
						$vs_form_code ) );
				}
				...
				foreach ( $vo_form->typeRestrictions->children() as $vo_restriction ) {
					...
					$t_form->addTypeRestriction( array_pop( caMakeTypeIDList( $vn_table_num, [ $vs_type ],
						[ 'dontIncludeSubtypesInTypeRestriction' => true ] ) ),
						[ 'includeSubtypes' => (bool) $vo_restriction->includeSubtypes ? 1 : 0 ] );

					if ( $t_form->numErrors() ) {
						$this->addError( "There was an error while inserting type restriction {$vs_restriction_code} in form {$vs_form_code}: "
						                 . join( "; ", $t_form->getErrors() ) );
					}

					$this->logStatus( _t( 'Added type restriction with code %1 and type %2 for form with code %3',
						$vs_restriction_code, $vs_type, $vs_form_code ) );
				}
			}
			if ( $vs_type_restrictions = self::getAttribute( $vo_form, "typeRestrictions" ) ) {
				...
				foreach ( $va_ids as $vn_i => $vn_type_id ) {
					$t_form->addTypeRestriction( $vn_type_id,
						[ 'includeSubtypes' => self::getAttribute( $vo_form, "includeSubtypes" ) ] );
					$this->logStatus( _t( 'Added type restriction with type %1 for form with code %2',
						$va_codes[ $vn_i ], $vs_form_code ) );
				}
			}
			...
			if ( $vo_form->roleAccess ) {
				...
				if ( sizeof( $va_form_roles ) > 0 ) {
					$all_roles = $t_role->getRoleList();
					foreach ( $all_roles as $role_id => $role_info ) {
						if ( ! isset( $va_form_roles[ $role_id ] ) ) {
							$va_form_roles[ $role_id ] = 0;
						}
					}
	...
	private function processSearchFormPlacements( $t_form, $po_placements ) {
		...
		// if we're updating, we expect the list of restrictions to include all restrictions!
		if ( sizeof( $po_placements->children() ) ) {
			$this->logStatus( _t( 'Successfully nuked all placements for form with code %1',
				$t_form->get( 'form_code' ) ) );
		...
		foreach ( $po_placements->children() as $vo_placement ) {
			...
			$va_settings = $this->_processSettings( null, $vo_placement->settings );
			$t_form->addPlacement( $vs_bundle, $va_settings, $vn_i,
				array( 'additional_settings' => $va_available_bundles[ $vs_bundle ]['settings'] ) );
			if ( $t_form->numErrors() ) {
				$this->addError( "There was an error while inserting search form placement {$vs_code}: " . join( " ",
						$t_form->getErrors() ) );

				return false;
			}

			$this->logStatus( _t( 'Added bundle placement %1 with code %2 for form with code %3', $vs_bundle, $vs_code,
				$t_form->get( 'form_code' ) ) );
			$vn_i ++;
		}
	...
	public function processGroups() {
		...
		// Create root group
		$t_user_group = ca_user_groups::find( array( 'code' => 'Root', 'parent_id' => null ),
			array( 'returnAs' => 'firstModelInstance' ) );
		...
		if ( is_array( $va_groups ) ) {
			foreach ( $va_groups as $vs_group_code => $vo_group ) {
				if ( ! ( $t_group = ca_user_groups::find( array( 'code' => (string) $vs_group_code ),
					array( 'returnAs' => 'firstModelInstance' ) ) )
				) {
					$t_group = new ca_user_groups();
				}
				...
				if ( $t_group->numErrors() ) {
					$this->addError( "Errors inserting user group {$vs_group_code}: " . join( "; ",
							$t_group->getErrors() ) );
	...
	public function processMetadataAlerts() {
		...
		foreach ( $va_md_alerts as $vo_md_alert ) {
			...
			if ( ! ( $t_md_alert = ca_metadata_alert_rules::find( array(
				'code'      => (string) $vs_alert_code,
				'table_num' => $vn_table_num
			), array( 'returnAs' => 'firstModelInstance' ) ) )
			) {
			...
			if ( $t_md_alert->numErrors() ) {
				$this->addError( "There was an error while inserting metadata alert {$vs_alert_code}: " . join( " ",
						$t_md_alert->getErrors() ) );
			} else {
				...
				if ( $t_md_alert->numErrors() ) {
					$this->addError( "There was an error while inserting metadata alert label for {$vs_alert_code}: "
					                 . join( " ", $t_md_alert->getErrors() ) );
				}
			...
			if ( $vo_md_alert->typeRestrictions ) {
				...
				if ( sizeof( $vo_md_alert->typeRestrictions->children() ) ) {
					$this->opo_db->query( 'DELETE FROM ca_metadata_alert_rule_type_restrictions WHERE rule_id = ?',
						[ $t_md_alert->getPrimaryKey() ] );
					$this->logStatus( _t( 'Successfully nuked all type restrictions for metadata alert with code %1',
						$vs_alert_code ) );
				}
				...
				foreach ( $vo_md_alert->typeRestrictions->children() as $vo_restriction ) {
					...
					$t_md_alert->addTypeRestriction( array_pop( caMakeTypeIDList( $vn_table_num, [ $vs_type ],
						[ 'dontIncludeSubtypesInTypeRestriction' => true ] ) ),
						[ 'includeSubtypes' => (bool) $vo_restriction->includeSubtypes ? 1 : 0 ] );

					if ( $t_md_alert->numErrors() ) {
						$this->addError( "There was an error while inserting type restriction {$vs_restriction_code} in metadata alert {$vs_alert_code}: "
						                 . join( "; ", $t_md_alert->getErrors() ) );
					}

					$this->logStatus( _t( 'Added type restriction with code %1 and type %2 for metadata alert with code %3',
						$vs_restriction_code, $vs_type, $vs_alert_code ) );
				}
			}
			if ( $vs_type_restrictions = self::getAttribute( $vo_md_alert, "typeRestrictions" ) ) {
				...
				foreach ( $va_ids as $vn_i => $vn_type_id ) {
					$t_md_alert->addTypeRestriction( $vn_type_id,
						[ 'includeSubtypes' => self::getAttribute( $vo_md_alert, "includeSubtypes" ) ] );
					$this->logStatus( _t( 'Added type restriction with type %1 for metadata alert with code %2',
						$va_codes[ $vn_i ], $vs_alert_code ) );
				}
			}
	...
	private function processMetadataAlertTriggers( $t_md_alert, $po_triggers ) {
		...
		if ( sizeof( $po_triggers->children() ) ) {
			$this->logStatus( _t( 'Successfully nuked all triggers for metadata alert with code %1',
				$t_md_alert->get( 'code' ) ) );
			$this->opo_db->query( 'DELETE FROM ca_metadata_alert_triggers WHERE rule_id = ?',
				[ $t_md_alert->getPrimaryKey() ] );
		}
		...
		foreach ( $po_triggers->children() as $vo_trigger ) {
			...
			if ( ! ( $vn_element_id = ca_metadata_elements::getElementID( $vs_metadata_element ) ) ) {
				$this->logStatus( _t( 'Skipped trigger %1 for metadata alert %2 because element code %3 is invalid',
					$vs_code, $t_md_alert->get( 'code' ), $vs_metadata_element ) );
				continue;
			}
			...
			if ( is_array( $va_metadata_element_filters_raw = explode( ';', $vs_metadata_element_filter ) )
			     && sizeof( $va_metadata_element_filters_raw )
			) {
				foreach ( $va_metadata_element_filters_raw as $vs_metadata_element_filters_raw ) {
					if ( is_array( $va_tmp = explode( ":", $vs_metadata_element_filters_raw ) )
					     && ( sizeof( $va_tmp ) > 1 )
					) {
						if ( $t_element = ca_metadata_elements::getInstance( $va_tmp[0] ) ) {
							if ( is_array( $va_item_ids = ca_lists::IDNOsToItemIDs( explode( "|", $va_tmp[1] ) ) )
							     && sizeof( $va_item_ids )
							) {
			...
			foreach ( $va_settings as $vs_setting => $vs_setting_value ) {
				...
				if ( ! $t_trigger->setSetting( $vs_setting, $v ) ) {
					$this->logStatus( _t( 'Skipped setting %1 for trigger %2 on metadata alert %3 because value %4 is invalid',
						$vs_setting, $vs_code, $t_md_alert->get( 'code' ), $vs_setting_value ) );
				}
			}
			...
			if ( $t_trigger->numErrors() ) {
				$this->addError( "There was an error while inserting metadata alert trigger {$vs_code}: " . join( " ",
						$t_trigger->getErrors() ) );
	...
	public function processMiscHierarchicalSetup() {
		...
		if ( $t_storage_location->numErrors() ) {
			$this->addError( "Errors inserting the storage location root: " . join( "; ",
					$t_storage_location->getErrors() ) );
	...
	public function createAdminAccount() {
		...
		if ( $t_user->numErrors() ) {
			$this->addError( "Errors while adding the default administrator account: " . join( "; ",
					$t_user->getErrors() ) );
	...
	private function _processSettings( $pt_instance, $po_settings_node, $pa_options = null ) {
		...
		if ( $po_settings_node ) {
			foreach ( $po_settings_node->children() as $vo_setting ) {
				...
				if ( isset( $pa_settings_info[ $vs_setting_name ] )
				     && isset( $pa_settings_info[ $vs_setting_name ]['deferred'] )
				     && $pa_settings_info[ $vs_setting_name ]['deferred']
				) {
					$this->opa_metadata_element_deferred_settings_processing[ $pt_instance->get( 'element_code' ) ][ $vs_setting_name ][]
						= $vs_value;
					continue;
				}
				...
				if ( ( strlen( $vs_setting_name ) > 0 )
				     && ( strlen( $vs_value ) > 0 )
				) { // settings need at least name and value
					$vs_datatype = (int) $pt_instance ? $pt_instance->get( 'datatype' ) : null;
					if ( $vs_setting_name === 'restrictToTypes'
					     && $t_authority_instance = AuthorityAttributeValue::elementTypeToInstance( $vs_datatype )
					) {
					...
					if ( $vs_locale ) { // settings with locale (those can't repeat)
						...
						if ( isset( $va_settings[ $vs_setting_name ] )
						     && ( ! isset( $pa_settings_info[ $vs_setting_name ] )
						          || ( $pa_settings_info[ $vs_setting_name ]['multiple'] ) )
						) {
