...
class Installer
{
     ...
     * @param bool $pb_overwrite overwrite existing install? optional, defaults to false
     * @param bool $pb_debug enable or disable debugging mode
     * @param bool $pb_skip_load dont actually load profile (useful if you want to fill in some gaps by hand)
     * @param bool $pb_log_output log output using Klogger
     */
        ...
        if (!$pb_skip_load) {
            if ($this->loadProfile($ps_profile_dir, $ps_profile_name)) {
                ...
                if (!$this->validateProfile()) {
                    $this->addError(
                        "Profile validation failed. Your profile doesn't conform to the required XML schema."
                    );
                }
            } else {
     ...
     * @return array
     */
    public static function getProfileInfo($ps_profile_dir, $ps_profile_name)
    {
    ...
    private function validateProfile()
    {
        ...
        if ($this->opo_base) {
            ...
            if ($this->opb_debug) {
                ob_start();
                $vb_return = $vo_profile->schemaValidate(
                        $this->ops_profile_dir . "/profile.xsd"
    ...
    protected static function addLabelsFromXMLElement($t_instance, $po_labels, $pa_locales, $pb_force_preferred = false)
    {
        ...
        if (!$po_labels || !$po_labels->children()) {
            $t_instance->addLabel(
                array($t_instance->getLabelDisplayField() => "???"),
                array_shift($pa_locales),
                false,
                true
            );
            return true;
        ...
        foreach ($po_labels->children() as $vo_label) {
            ...
            $va_existing_labels = $vb_preferred ? $t_instance->getPreferredLabels(
                array($vn_locale_id)
            ) : $t_instance->getNonPreferredLabels(array($vn_locale_id));
            if (
                is_array($va_existing_labels) &&
                (sizeof($va_existing_labels) > 0) &&
                ($vn_label_id = $va_existing_labels[(int)$t_instance->getPrimaryKey(
                )][(int)$vn_locale_id][0]['label_id'])
            ) {
                $vn_label_id = $t_instance->editLabel(
                    $vn_label_id,
                    $va_label_values,
                    $vn_locale_id,
                    null,
     ...
     * @return bool Returns true on success, false if an error occurred
     */
    public function loadSchema($f_callback = null)
    {
        ...
        if (defined(
            ...
            ) && __CA_ALLOW_INSTALLER_TO_OVERWRITE_EXISTING_INSTALLS__ && ($this->opb_overwrite)) {
        ...
        foreach ($va_schema_statements as $vs_statement) {
            ...
            if (is_callable($f_callback) && preg_match(
    ...
    public function processLocales()
    {
        ...
        if ($this->ops_base_name) {
            ...
            foreach ($this->opo_profile->locales->children() as $vo_locale) {
                $key = self::getAttribute($vo_locale, "lang") . '_' . self::getAttribute(
                        $vo_locale,
                        "country"
        ...
        foreach ($va_locales as $vo_locale) {
            ...
            if ($t_locale->numErrors()) {
                $this->addError(
                    "There was an error while inserting locale {$vs_locale_code}: " . join(" ", $t_locale->getErrors())
                );
            }
        ...
        if (!$vn_locale_id) {
            throw new Exception(
                "The locale default is set to a non-existing locale. Try adding '" . $va_locales[0] . "' to your profile."
            );
        }
        // Ensure the default locale comes first.
        uksort(
            $this->opa_locales,
            function ($a) use ($vn_locale_id) {
    ...
    public function processLists($f_callback = null)
    {
        ...
        foreach ($va_lists as $vo_list) {
            ...
            if (!($t_list = ca_lists::find(
                array('list_code' => $vs_list_code),
            ...
            if ($t_list->numErrors()) {
                $this->addError(
                    "There was an error while inserting list {$vs_list_code}: " . join(" ", $t_list->getErrors())
                );
            } else {
                ...
                if ($t_list->numErrors()) {
                    $this->addError(
                        "There was an error while inserting list label for {$vs_list_code}: " . join(
                            " ",
                            $t_list->getErrors()
                        )
                    );
    ...
    protected function processListItems($t_list, $po_items, $pn_parent_id)
    {
        foreach ($po_items->children() as $vo_item) {
            ...
            if (($t_list->numErrors() > 0) || !is_object($t_item)) {
                $this->addError(
                    "There was an error while inserting list item {$vs_item_idno}: " . join(" ", $t_list->getErrors())
                );
                return false;
            } else {
                ...
                if ($vo_item->settings) {
                    ...
                    if ($t_item->numErrors()) {
                        $this->addError(
                            "There was an error while adding a setting for list item with idno {$vs_item_idno}: " . join(
                                " ",
                                $t_item->getErrors()
                            )
                        );
                ...
                if ($t_item->numErrors()) {
                    $this->addError(
                        "There was an error while inserting list item label for {$vs_item_idno}: " . join(
                            " ",
                            $t_item->getErrors()
                        )
                    );
        ...
        foreach ($va_elements as $vs_element_code => $vo_element) {
            if ($vn_element_id = $this->processMetadataElement($vo_element, null)) {
                ...
                if (sizeof($vo_element->typeRestrictions->children())) {
                    $this->opo_db->query(
                        'DELETE FROM ca_metadata_type_restrictions WHERE element_id=?',
                        $vn_element_id
                    );
                }
                ...
                foreach ($vo_element->typeRestrictions->children() as $vo_restriction) {
                    ...
                    if (!($vn_table_num = Datamodel::getTableNum((string)$vo_restriction->table))) {
                        $this->addError(
                            "Invalid table specified for restriction $vs_restriction_code in element $vs_element_code"
                        );
                        return false;
                    }
                    ...
                    if ($t_restriction->numErrors()) {
                        $this->addError(
                            "There was an error while inserting type restriction {$vs_restriction_code} for metadata element {$vs_element_code}: " . join(
                                "; ",
                                $t_restriction->getErrors()
                            )
                        );
                    }
                    ...
                    $this->logStatus(
                        _t(
                            'Successfully added type restriction %1 for element %2',
                            $vs_restriction_code,
                            $vs_element_code
                        )
                    );
    ...
    private function processMetadataElement($po_element, $pn_parent_id)
    {
        ...
        if (($vn_datatype = ca_metadata_elements::getAttributeTypeCode(
                self::getAttribute($po_element, "datatype")
            )) === false) {
        ...
        if ($t_md_element->numErrors()) {
            $this->addError(
                "There was an error while inserting metadata element {$vs_element_code}: " . join(
                    " ",
        ...
        foreach ($this->opo_profile->metadataDictionary->children() as $vo_entry) {
            ...
            if ($vo_entry->rules) {
                foreach ($vo_entry->rules->children() as $vo_rule) {
                    ...
                    if ($t_rule->numErrors()) {
                        $this->addError(
                            "There were errors while adding dictionary rule: " . join(';', $t_rule->getErrors())
                        );
                        continue;
                    }
                }
    ...
    public function processUserInterfaces()
    {
        ...
        foreach ($va_uis as $vs_ui_code => $vo_ui) {
            ...
            // create ui row
            if (!($t_ui = ca_editor_uis::find(
                array('editor_code' => $vs_ui_code, 'editor_type' => $vn_type),
                array('returnAs' => 'firstModelInstance')
            ))) {
            ...
            if ($vo_ui->typeRestrictions) {
                ...
                $this->logStatus(
                    _t('Successfully nuked all type restrictions for user interface with code %1', $vs_ui_code)
                );
                ...
                foreach ($vo_ui->typeRestrictions->children() as $vo_restriction) {
                    ...
                    if (strlen($vs_restriction_type) > 0) {
                        // interstitial with type restriction -> code is relationship type code
                        if ($t_instance instanceof BaseRelationshipModel) {
                            $vn_type_id = $t_rel_types->getRelationshipTypeID(
                        ...
                        $t_ui->addTypeRestriction(
                            $vn_type_id,
                            [
                                'includeSubtypes' => self::getAttribute(
                                    $vo_restriction,
                                    "includeSubtypes"
                                )
                            ]
                        ...
                        $this->logStatus(
                            _t(
                                'Successfully added type restriction %1 for user interface with code %2',
                                $vs_restriction_type,
                                $vs_ui_code
                            )
                        );
            ...
            foreach ($vo_ui->screens->children() as $vo_screen) {
                ...
                $this->logStatus(
                    _t('Processing screen with code %1 for user interface with code %2', $vs_screen_idno, $vs_ui_code)
                );

                $t_ui_screens = ca_editor_ui_screens::find(
                    array(
                        'idno' => $vs_screen_idno,
                        'ui_id' => $vn_ui_id
                    ),
                ...
                if ($t_ui_screens->getPrimaryKey()) {
                    $this->logStatus(
                        _t(
                            'Screen with code %1 for user interface with code %2 already exists',
                            $vs_screen_idno,
                            $vs_ui_code
                        )
                    );
                    ...
                    $this->logStatus(
                        _t('Screen with code %1 for user interface with code %2 is new', $vs_screen_idno, $vs_ui_code)
                    );
                }

                if (self::getAttribute($vo_screen, 'deleted') && $t_ui_screens->getPrimaryKey()) {
                    $this->logStatus(
                        _t('Deleting screen with code %1 for user interface with code %2', $vs_screen_idno, $vs_ui_code)
                    );
                ...
                if ($t_ui_screens->numErrors()) {
                    $this->addError(
                        "Errors inserting UI screen {$vs_screen_idno} for UI {$vs_ui_code}: " . join(
                            "; ",
                ...
                $this->logStatus(
                    _t(
                        'Successfully updated/inserted screen with code %1 for user interface with code %2',
                        $vs_screen_idno,
                        $vs_ui_code
                    )
                );
                ...
                if (sizeof($vo_screen->bundlePlacements->children())) {
                    $this->opo_db->query(
                        'DELETE FROM ca_editor_ui_bundle_placements WHERE screen_id=?',
                ...
                $this->logStatus(
                    _t(
                        'Successfully nuked all bundle placements for screen with code %1 for user interface with code %2',
                        $vs_screen_idno,
                        $vs_ui_code
                    )
                );
                ...
                if ($vo_screen->userAccess) {
                    ...
                    foreach ($vo_screen->userAccess->children() as $vo_permission) {
                        ...
                        $vn_access = $this->_convertUserGroupAccessStringToInt(
                            self::getAttribute($vo_permission, 'access')
                        );
                        ...
                        if ($vn_access) {
                            ...
                            $this->addError(
                                "User name or access value invalid for UI screen {$vs_screen_idno} (permission item with user name '{$vs_user}')"
                            );
                        }
                    }
                ...
                if ($vo_screen->groupAccess) {
                    ...
                    foreach ($vo_screen->groupAccess->children() as $vo_permission) {
                        ...
                        if ($vn_access) {
                            ...
                            $this->addError(
                                "Group code or access value invalid for UI screen {$vs_screen_idno} (permission item with group code '{$vs_group}')"
                            );
                        }
                    }
                ...
                if ($vo_screen->roleAccess) {
                    ...
                    foreach ($vo_screen->roleAccess->children() as $vo_permission) {
                        ...
                        if (!is_null($vn_access)) {
                            ...
                            $this->addError(
                                "Role code or access value invalid for UI screen {$vs_screen_idno} (permission item with role code '{$vs_role}')"
                            );
                        }
                    }
                ...
                foreach ($vo_screen->bundlePlacements->children() as $vo_placement) {
                    ...
                    if ($vs_bundle_type_restrictions) {
                        ...
                        // as settings on the placement record.
                        if ($t_instance instanceof BaseRelationshipModel) {
                            $va_ids = caMakeRelationshipTypeIDList(
                                $t_instance->tableNum(),
                    ...
                    $va_settings = $this->_processSettings(
                        null,
                        $vo_placement->settings,
                        [
                            'settingsInfo' => array_merge(
                                $t_placement->getAvailableSettings(),
                                is_array(
                    ...
                    $this->logStatus(
                        _t(
                            'Adding bundle %1 with code %2 for screen with code %3 and user interface with code %4',
                            $vs_bundle,
                            $vs_placement_code,
                            $vs_screen_idno,
                            $vs_ui_code
                ...
                if ($vo_screen->typeRestrictions) {
                    ...
                    if (sizeof($vo_screen->typeRestrictions->children())) {
                        $this->opo_db->query(
                            'DELETE FROM ca_editor_ui_screen_type_restrictions WHERE screen_id=?',
                            $t_ui_screens->getPrimaryKey()
                        );
                    }
                    ...
                    $this->logStatus(
                        _t(
                            'Successfully nuked all type restrictions for screen with code %1 for user interface with code %2',
                            $vs_screen_idno,
                            $vs_ui_code
                        )
                    );
                    ...
                    foreach ($vo_screen->typeRestrictions->children() as $vo_restriction) {
                        ...
                        if (strlen($vs_restriction_type) > 0) {
                            ...
                            if ($vn_type_id) {
                                $t_ui_screens->addTypeRestriction(
                                    $vn_type_id,
                                    [
                                        'includeSubtypes' => self::getAttribute(
                                            $vo_restriction,
                            ...
                            $this->logStatus(
                                _t(
                                    'Successfully added type restriction %1 for screen with code %2 for user interface with code %3',
                                    $vs_restriction_type,
                                    $vs_screen_idno,
                                    $vs_ui_code
                                )
            ...
            if ($vo_ui->userAccess) {
                ...
                foreach ($vo_ui->userAccess->children() as $vo_permission) {
                    ...
                    if ($vn_access) {
                        ...
                        $this->addError(
                            "User name or access value invalid for UI {$vs_ui_code} (permission item with user name '{$vs_user}')"
                        );
                    }
                }
            ...
            if ($vo_ui->groupAccess) {
                ...
                foreach ($vo_ui->groupAccess->children() as $vo_permission) {
                    ...
                    if ($vn_access) {
                        ...
                        $this->addError(
                            "Group code or access value invalid for UI {$vs_ui_code} (permission item with group code '{$vs_group}')"
                        );
                    }
                }
            ...
            if ($vo_ui->roleAccess) {
                ...
                foreach ($vo_ui->roleAccess->children() as $vo_permission) {
                    ...
                    if (!is_null($vn_access)) {
                        ...
                        $this->addError(
                            "Role code or access value invalid for UI {$vs_ui_code} (permission item with role code '{$vs_role}')"
                        );
                    }
                }
    ...
    public function processRelationshipTypes()
    {
        ...
        // get list items
        $qr_list_item_result = $this->opo_db->query(
        ...
        while ($qr_list_item_result->nextRow()) {
            ...
            $va_list_item_ids[$vs_type_code][$qr_list_item_result->get('item_value')] = $qr_list_item_result->get(
                'item_id'
            );
        }
        ...
        foreach ($va_rel_tables as $vs_table => $vo_rel_table) {
            ...
            if ($t_rel_type->numErrors()) {
                $this->addError(
                    "Errors inserting relationship root for {$vs_table}: " . join("; ", $t_rel_type->getErrors())
                );
                return false;
            }
            ...
            $this->processRelationshipTypesForTable(
                $vo_rel_table->types,
                $vn_table_num,
                $vs_left_table,
                $vs_right_table,
    ...
    private function processRelationshipTypesForTable(
        ...
        foreach ($po_relationship_types->children() as $vo_type) {
            ...
            if (
                ...
                $this->logStatus(
                    _t(
                        'Adding left type restriction %1 for relationship type with code %2',
                        $vs_left_subtype_code,
                        $vs_type_code
                    )
                );
            ...
            if (
                ...
                $this->logStatus(
                    _t(
                        'Adding right type restriction %1 for relationship type with code %2',
                        $vs_right_subtype_code,
                        $vs_type_code
                    )
                );
            ...
            if ($t_rel_type->numErrors()) {
                $this->addError(
                    "Errors inserting relationship {$vs_type_code}: " . join("; ", $t_rel_type->getErrors())
                );
                return false;
            }
    ...
    public function processRoles()
    {
        ...
        foreach ($va_roles as $vs_role_code => $vo_role) {
            ...
            if (!($t_role = ca_user_roles::find(
                array('code' => (string)$vs_role_code),
                array('returnAs' => 'firstModelInstance')
            ))) {
            ...
            if ($vo_role->actions) {
                ...
                $this->logStatus(
                    _t('Role actions for user role with code %1 are: %2', $vs_role_code, join(',', $va_actions))
                );
            }
            ...
            if ($vo_role->bundleLevelAccessControl) {
                ...
                if (sizeof($vo_role->bundleLevelAccessControl->children()) > 0) {
                    ...
                    $this->logStatus(
                        _t(
                            'Successfully nuked all bundle level access control items for user role with code %1',
                            $vs_role_code
                        )
                    );
                }
                ...
                foreach ($vo_role->bundleLevelAccessControl->children() as $vo_permission) {
                    ...
                    $vn_permission_access = $this->_convertACLStringToConstant(
                        self::getAttribute($vo_permission, 'access')
                    );

                    if (!$t_role->setAccessSettingForBundle(
                        $vs_permission_table,
                        $vs_permission_bundle,
                        $vn_permission_access
                    )) {
                        $this->addError(
                            "Could not add bundle level access control for table '{$vs_permission_table}' and bundle '{$vs_permission_bundle}'. Check the table and bundle names."
                        );
                        //return false;
                    }
                    ...
                    $this->logStatus(
                        _t(
                            'Added bundle level access control item for user role with code %1: table %2, bundle %3, access %4',
                            $vs_role_code,
                            $vs_permission_table,
                            $vs_permission_bundle,
                            $vn_permission_access
            ...
            if ($vo_role->typeLevelAccessControl) {
                ...
                if (sizeof($vo_role->typeLevelAccessControl->children()) > 0) {
                    ...
                    $this->logStatus(
                        _t(
                            'Successfully nuked all type level access control items for user role with code %1',
                            $vs_role_code
                        )
                    );
                }
                ...
                foreach ($vo_role->typeLevelAccessControl->children() as $vo_permission) {
                    ...
                    if (!$t_role->setAccessSettingForType(
                        $vs_permission_table,
                        $vs_permission_type,
                        $vn_permission_access
                    )) {
                        $this->addError(
                            "Could not add type level access control for table '{$vs_permission_table}' and type '{$vs_permission_type}'. Check the table name and the type code."
                        );
                        //return false;
                    }
                    ...
                    $this->logStatus(
                        _t(
                            'Added type level access control item for user role with code %1: table %2, type %3, access %4',
                            $vs_role_code,
                            $vs_permission_table,
                            $vs_permission_type,
                            $vn_permission_access
            ...
            if ($vo_role->sourceLevelAccessControl) {
                ...
                if (sizeof($vo_role->sourceLevelAccessControl->children()) > 0) {
                    ...
                    $this->logStatus(
                        _t(
                            'Successfully nuked all source level access control items for user role with code %1',
                            $vs_role_code
                        )
                    );
                }
                ...
                foreach ($vo_role->sourceLevelAccessControl->children() as $vo_permission) {
                    ...
                    if (!$t_role->setAccessSettingForSource(
                        $vs_permission_table,
                        $vs_permission_source,
                        $vn_permission_access,
                        (bool)$vs_permission_default
                    )) {
                        $this->addError(
                            "Could not add source level access control for table '{$vs_permission_table}' and source '{$vs_permission_source}'. Check the table name and the source code."
                        );
                        //return false;
                    }
                    ...
                    $this->logStatus(
                        _t(
                            'Added source level access control item for user role with code %1: table %2, source %3, access %4',
                            $vs_role_code,
                            $vs_permission_table,
                            $vs_permission_source,
                            $vn_permission_access
    ...
    public function processDisplays()
    {
        ...
        foreach ($va_displays as $vo_display) {
            ...
            if (!($t_display = ca_bundle_displays::find(
                array('display_code' => $vs_display_code),
                array('returnAs' => 'firstModelInstance')
            ))) {
            ...
            if ($t_display->numErrors()) {
                $this->addError(
                    "There was an error while inserting display {$vs_display_code}: " . join(
                        " ",
                        $t_display->getErrors()
                    )
                );
                ...
                if ($t_display->numErrors()) {
                    $this->addError(
                        "There was an error while inserting display label for {$vs_display_code}: " . join(
                            " ",
                            $t_display->getErrors()
                        )
                    );
            ...
            if ($vo_display->typeRestrictions) {
                ...
                if (sizeof($vo_display->typeRestrictions->children())) {
                    $this->opo_db->query(
                        'DELETE FROM ca_bundle_display_type_restrictions WHERE display_id=?',
                        $t_display->getPrimaryKey()
                    );
                    $this->logStatus(
                        _t('Successfully nuked all type restrictions for display with code %1', $vs_display_code)
                    );
                }
                ...
                foreach ($vo_display->typeRestrictions->children() as $vo_restriction) {
                    ...
                    if ($t_display->numErrors()) {
                        $this->addError(
                            "There was an error while inserting type restriction {$vs_restriction_code} in display {$vs_display_code}: " . join(
                                "; ",
                                $t_display->getErrors()
                            )
                        );
                    ...
                    $this->logStatus(
                        _t(
                            'Added type restriction with code %1 and type %2 for display with code %3',
                            $vs_restriction_code,
                            $vs_type,
                            $vs_display_code
                        )
            ...
            if ($vs_type_restrictions = self::getAttribute($vo_display, "typeRestrictions")) {
                ...
                foreach ($va_ids as $vn_i => $vn_type_id) {
                    ...
                    $this->logStatus(
                        _t(
                            'Added type restriction with type %1 for display with code %2',
                            $va_codes[$vn_i],
                            $vs_display_code
                        )
                    );
            ...
            if ($vo_display->userAccess) {
                ...
                foreach ($vo_display->userAccess->children() as $vo_permission) {
                    ...
                    if ($vn_access) {
                        ...
                        $this->addError(
                            "User name or access value invalid for display {$vs_display_code} (permission item with user name '{$vs_user}')"
                        );
                    }
                }
            ...
            if ($vo_display->groupAccess) {
                ...
                foreach ($vo_display->groupAccess->children() as $vo_permission) {
                    ...
                    if ($vn_access) {
                        ...
                        $this->addError(
                            "Group code or access value invalid for display {$vs_display_code} (permission item with group code '{$vs_group}')"
                        );
                    }
                }
            ...
            if ($vo_display->roleAccess) {
                ...
                foreach ($vo_display->roleAccess->children() as $vo_permission) {
                    ...
                    if (!is_null($vn_access)) {
                        ...
                        $this->addError(
                            "Role code or access value invalid for display {$vs_display_code} (permission item with role code '{$vs_role}')"
                        );
                    }
                }
    ...
    private function processDisplayPlacements($t_display, $po_placements)
    {
        ...
        if (sizeof($po_placements->children())) {
            $this->logStatus(
                _t('Successfully nuked all placements for display with code %1', $t_display->get('display_code'))
            );
            $this->opo_db->query(
                'DELETE FROM ca_bundle_display_placements WHERE display_id=?',
                $t_display->getPrimaryKey()
            );
        }
        ...
        foreach ($po_placements->children() as $vo_placement) {
            ...
            if ($t_display->numErrors()) {
                $this->addError(
                    "There was an error while inserting display placement {$vs_code}: " . join(
                        " ",
                        $t_display->getErrors()
                    )
                );
            ...
            $this->logStatus(
                _t(
                    'Added bundle placement %1 with code %2 for display with code %3',
                    $vs_bundle,
                    $vs_code,
    ...
    public function processSearchForms()
    {
        ...
        foreach ($va_forms as $vo_form) {
            ...
            if (!($t_form = ca_search_forms::find(
                array('form_code' => (string)$vs_form_code, 'table_num' => $vn_table_num),
                array('returnAs' => 'firstModelInstance')
            ))) {
            ...
            if ($t_form->numErrors()) {
                $this->addError(
                    "There was an error while inserting search form {$vs_form_code}: " . join(" ", $t_form->getErrors())
                );
            } else {
                ...
                if ($t_form->numErrors()) {
                    $this->addError(
                        "There was an error while inserting search form label for {$vs_form_code}: " . join(
                            " ",
                            $t_form->getErrors()
                        )
                    );
            ...
            if ($vo_form->typeRestrictions) {
                ...
                if (sizeof($vo_form->typeRestrictions->children())) {
                    $this->opo_db->query(
                        'DELETE FROM ca_search_form_type_restrictions WHERE form_id=?',
                        $t_form->getPrimaryKey()
                    );
                    $this->logStatus(
                        _t('Successfully nuked all type restrictions for form with code %1', $vs_form_code)
                    );
                }
                ...
                foreach ($vo_form->typeRestrictions->children() as $vo_restriction) {
                    ...
                    if ($t_form->numErrors()) {
                        $this->addError(
                            "There was an error while inserting type restriction {$vs_restriction_code} in form {$vs_form_code}: " . join(
                                "; ",
                                $t_form->getErrors()
                            )
                        );
                    ...
                    $this->logStatus(
                        _t(
                            'Added type restriction with code %1 and type %2 for form with code %3',
                            $vs_restriction_code,
                            $vs_type,
                            $vs_form_code
                        )
            ...
            if ($vs_type_restrictions = self::getAttribute($vo_form, "typeRestrictions")) {
                ...
                foreach ($va_ids as $vn_i => $vn_type_id) {
                    ...
                    $this->logStatus(
                        _t('Added type restriction with type %1 for form with code %2', $va_codes[$vn_i], $vs_form_code)
                    );
                }
            }
            ...
            if ($vo_form->userAccess) {
                ...
                foreach ($vo_form->userAccess->children() as $vo_permission) {
                    ...
                    if ($vn_access) {
                        ...
                        $this->addError(
                            "User name or access value invalid for search form {$vs_form_code} (permission item with user name '{$vs_user}')"
                        );
                    }
                }
            ...
            if ($vo_form->groupAccess) {
                ...
                foreach ($vo_form->groupAccess->children() as $vo_permission) {
                    ...
                    if ($vn_access) {
                        ...
                        $this->addError(
                            "Group code or access value invalid for search form {$vs_form_code} (permission item with group code '{$vs_group}')"
                        );
                    }
                }
            ...
            if ($vo_form->roleAccess) {
                ...
                foreach ($vo_form->roleAccess->children() as $vo_permission) {
                    ...
                    if (!is_null($vn_access)) {
                        ...
                        $this->addError(
                            "Role code or access value invalid for form {$vs_form_code} (permission item with role code '{$vs_role}')"
                        );
                    }
                }
    ...
    private function processSearchFormPlacements($t_form, $po_placements)
    {
        ...
        foreach ($po_placements->children() as $vo_placement) {
            ...
            if ($t_form->numErrors()) {
                $this->addError(
                    "There was an error while inserting search form placement {$vs_code}: " . join(
                        " ",
                        $t_form->getErrors()
                    )
                );
            ...
            $this->logStatus(
                _t(
                    'Added bundle placement %1 with code %2 for form with code %3',
                    $vs_bundle,
                    $vs_code,
    ...
    {
        // Create root group
        $t_user_group = ca_user_groups::find(
            array('code' => 'Root', 'parent_id' => null),
            array('returnAs' => 'firstModelInstance')
        );
        ...
        if (is_array($va_groups)) {
            foreach ($va_groups as $vs_group_code => $vo_group) {
                if (!($t_group = ca_user_groups::find(
                    array('code' => (string)$vs_group_code),
                    array('returnAs' => 'firstModelInstance')
                ))) {
                    $t_group = new ca_user_groups();
                }
                ...
                if ($t_group->numErrors()) {
                    $this->addError(
                        "Errors inserting user group {$vs_group_code}: " . join("; ", $t_group->getErrors())
                    );
                    return false;
                }
            }
    ...
    public function processMetadataAlerts()
    {
        ...
        foreach ($va_md_alerts as $vo_md_alert) {
            ...
            if (!($t_md_alert = ca_metadata_alert_rules::find(
                array('code' => (string)$vs_alert_code, 'table_num' => $vn_table_num),
                array('returnAs' => 'firstModelInstance')
            ))) {
            ...
            if ($t_md_alert->numErrors()) {
                $this->addError(
                    "There was an error while inserting metadata alert {$vs_alert_code}: " . join(
                        " ",
                ...
                if ($t_md_alert->numErrors()) {
                    $this->addError(
                        "There was an error while inserting metadata alert label for {$vs_alert_code}: " . join(
                            " ",
                            $t_md_alert->getErrors()
                        )
                    );
            ...
            if ($vo_md_alert->typeRestrictions) {
                ...
                if (sizeof($vo_md_alert->typeRestrictions->children())) {
                    $this->opo_db->query(
                        'DELETE FROM ca_metadata_alert_rule_type_restrictions WHERE rule_id = ?',
                        [$t_md_alert->getPrimaryKey()]
                    );
                    $this->logStatus(
                        _t('Successfully nuked all type restrictions for metadata alert with code %1', $vs_alert_code)
                    );
                }
                ...
                foreach ($vo_md_alert->typeRestrictions->children() as $vo_restriction) {
                    ...
                    if ($t_md_alert->numErrors()) {
                        $this->addError(
                            "There was an error while inserting type restriction {$vs_restriction_code} in metadata alert {$vs_alert_code}: " . join(
                                "; ",
                                $t_md_alert->getErrors()
                            )
                        );
                    ...
                    $this->logStatus(
                        _t(
                            'Added type restriction with code %1 and type %2 for metadata alert with code %3',
                            $vs_restriction_code,
                            $vs_type,
                            $vs_alert_code
                        )
            ...
            if ($vs_type_restrictions = self::getAttribute($vo_md_alert, "typeRestrictions")) {
                ...
                foreach ($va_ids as $vn_i => $vn_type_id) {
                    ...
                    $this->logStatus(
                        _t(
                            'Added type restriction with type %1 for metadata alert with code %2',
                            $va_codes[$vn_i],
                            $vs_alert_code
                        )
                    );
            ...
            if ($vo_md_alert->userAccess) {
                ...
                foreach ($vo_md_alert->userAccess->children() as $vo_permission) {
                    ...
                    if ($vn_access && $t_user->load(array('user_name' => $vs_user))) {
                        ...
                        $this->addError(
                            "User name or access value invalid for metadata alert {$vs_alert_code} (permission item with user name '{$vs_user}')"
                        );
                    }
                }
            ...
            if ($vo_md_alert->groupAccess) {
                ...
                foreach ($vo_md_alert->groupAccess->children() as $vo_permission) {
                    ...
                    if ($vn_access && $t_group->load(array('code' => $vs_group))) {
                        ...
                        $this->addError(
                            "Group code or access value invalid for metadata alert {$vs_alert_code} (permission item with group code '{$vs_group}')"
                        );
                    }
                }
            ...
            if ($vo_md_alert->roleAccess) {
                ...
                foreach ($vo_md_alert->roleAccess->children() as $vo_permission) {
                    ...
                    if (!is_null($vn_access)) {
                        ...
                        $this->addError(
                            "Role code or access value invalid for metadata alert {$vs_alert_code} (permission item with role code '{$vs_role}')"
                        );
                    }
                }
    ...
    private function processMetadataAlertTriggers($t_md_alert, $po_triggers)
    {
        ...
        if (sizeof($po_triggers->children())) {
            $this->logStatus(
                _t('Successfully nuked all triggers for metadata alert with code %1', $t_md_alert->get('code'))
            );
            $this->opo_db->query(
                'DELETE FROM ca_metadata_alert_triggers WHERE rule_id = ?',
                [$t_md_alert->getPrimaryKey()]
            );
        }
        ...
        foreach ($po_triggers->children() as $vo_trigger) {
            ...
            if (!($vn_element_id = ca_metadata_elements::getElementID($vs_metadata_element))) {
                $this->logStatus(
                    _t(
                        'Skipped trigger %1 for metadata alert %2 because element code %3 is invalid',
                        $vs_code,
                        $t_md_alert->get('code'),
                        $vs_metadata_element
                    )
            ...
            if (is_array($va_metadata_element_filters_raw = explode(';', $vs_metadata_element_filter)) && sizeof(
                    $va_metadata_element_filters_raw
                )) {
                foreach ($va_metadata_element_filters_raw as $vs_metadata_element_filters_raw) {
                    if (is_array($va_tmp = explode(":", $vs_metadata_element_filters_raw)) && (sizeof($va_tmp) > 1)) {
                        if ($t_element = ca_metadata_elements::getInstance($va_tmp[0])) {
                            if (is_array($va_item_ids = ca_lists::IDNOsToItemIDs(explode("|", $va_tmp[1]))) && sizeof(
                                    $va_item_ids
                                )) {
            ...
            foreach ($va_settings as $vs_setting => $vs_setting_value) {
                ...
                if (!$t_trigger->setSetting($vs_setting, $v)) {
                    $this->logStatus(
                        _t(
                            'Skipped setting %1 for trigger %2 on metadata alert %3 because value %4 is invalid',
                            $vs_setting,
                            $vs_code,
                            $t_md_alert->get('code'),
                            $vs_setting_value
            ...
            if ($t_trigger->numErrors()) {
                $this->addError(
                    "There was an error while inserting metadata alert trigger {$vs_code}: " . join(
                        " ",
                        $t_trigger->getErrors()
                    )
                );
    ...
    public function processMiscHierarchicalSetup()
    {
        ...
        if ($t_storage_location->numErrors()) {
            $this->addError(
                "Errors inserting the storage location root: " . join("; ", $t_storage_location->getErrors())
            );
            return;
        }
    }
    ...
    public function createAdminAccount()
    {
        ...
        if ($t_user->numErrors()) {
            $this->addError(
                "Errors while adding the default administrator account: " . join("; ", $t_user->getErrors())
            );
            return false;
        }
        ...
        if ($po_settings_node) {
            foreach ($po_settings_node->children() as $vo_setting) {
                ...
                if (isset($pa_settings_info[$vs_setting_name]) && isset($pa_settings_info[$vs_setting_name]['deferred']) && $pa_settings_info[$vs_setting_name]['deferred']) {
                    $this->opa_metadata_element_deferred_settings_processing[$pt_instance->get(
                        'element_code'
                    )][$vs_setting_name][] = $vs_value;
                    continue;
                }

                if ((strlen($vs_setting_name) > 0) && (strlen(
                            $vs_value
                        ) > 0)) { // settings need at least name and value
                    $vs_datatype = (int)$pt_instance ? $pt_instance->get('datatype') : null;
                    if ($vs_setting_name === 'restrictToTypes' && $t_authority_instance = AuthorityAttributeValue::elementTypeToInstance(
                            $vs_datatype
                        )) {
