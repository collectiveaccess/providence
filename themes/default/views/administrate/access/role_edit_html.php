    ...
    <div id="role_tabs" class="tabContainer">
        ...
        <div id="role_actions">
            <table>
                ...
                foreach ($va_actions as $vs_group => $va_group_info) {
                    ...
                    $vs_check_all_link = '<a href="#" onclick="jQuery(\'.role_action_group_' . $vs_group . '\').attr(\'checked\', true); return false;" class="roleCheckAllNoneButton">' . _t(
                            'All'
                        ) . '</a>';
                    $vs_check_none_link = '<a href="#" onclick="jQuery(\'.role_action_group_' . $vs_group . '\').attr(\'checked\', false); return false;" class="roleCheckAllNoneButton">' . _t(
                            'None'
                        ) . '</a>';
                    ...
                    foreach ($va_group_info['actions'] as $vs_action => $va_action_info) {
                        ...
                        print "<td>" . caHTMLCheckboxInput(
                                $vs_action,
                                $va_attributes
                            ) . "</td><td width='120'><span id='role_label_" . $vs_action . "'>" . $va_action_info['label'] . "</span></td>";
                        TooltipManager::add(
                            '#role_label_' . $vs_action,
        ...
        <div id="role_bundles">
            ...
            foreach ($va_bundle_list as $vs_table => $va_bundles_by_table) {
                ...
                print "<tr align='center' valign='middle'><th width='180' align='left'>" . _t(
                        'Element'
                    ) . "</th><th width='180'><a href='#' onclick='jQuery(\".{$vs_table}_bundle_access_none\").prop(\"checked\", 1); return false;'>" . _t(
                        'No access'
                    ) . "</a></th><th width='180'><a href='#' onclick='jQuery(\".{$vs_table}_bundle_access_readonly\").prop(\"checked\", 1); return false;'>" . _t(
                        'Read-only access'
                    ) . "</a></th><th><a href='#' onclick='jQuery(\".{$vs_table}_bundle_access_edit\").prop(\"checked\", 1); return false;'>" . _t(
                        'Read/edit access'
                    ) . "</a></th></tr>\n";
                ...
                foreach ($va_bundles_by_table as $vs_bundle_name => $va_info) {
                    ...
                    if (in_array(
                        ...
                        print "<td>" . caHTMLRadioButtonInput(
                                $vs_table . '_' . $vs_bundle_name,
                                array(
                                    'value' => __CA_BUNDLE_ACCESS_NONE__,
                                    'class' => "{$vs_table}_bundle_access_none"
                                ),
                                array('checked' => ($vs_access == __CA_BUNDLE_ACCESS_NONE__))
                            ) . "</td>\n";
                    }
                    print "<td>" . caHTMLRadioButtonInput(
                            $vs_table . '_' . $vs_bundle_name,
                            ...
                            array('checked' => ($vs_access == __CA_BUNDLE_ACCESS_READONLY__))
                        ) . "</td>\n";
                    print "<td>" . caHTMLRadioButtonInput(
                            $vs_table . '_' . $vs_bundle_name,
                            array(
                                'value' => __CA_BUNDLE_ACCESS_EDIT__,
                                'class' => "{$vs_table}_bundle_access_edit"
                            ),
                            array('checked' => ($vs_access == __CA_BUNDLE_ACCESS_EDIT__))
                        ) . "</td>\n";
                }
                print "</tr>\n";
                print "</table>\n";
        ...
        if ($t_role->getAppConfig()->get('perform_type_access_checking')) {
            ...
            <div id="type_list">
                ...
                foreach ($va_type_list as $vs_table => $va_types_by_table) {
                    ...
                    print "<tr align='center' valign='middle'><th width='180' align='left'>" . _t(
                            'Type'
                        ) . "</th><th width='180'>" . _t('No access') . "</th><th width='180'>" . _t(
                            'Read-only access'
                        ) . "</th><th>" . _t('Read/edit access') . "</th></tr>\n";
                    ...
                    foreach ($va_types_by_table as $vn_id => $va_type) {
                        ...
                        print "<td align='left'>" . str_repeat(
                                "&nbsp;",
                                $vn_indent
                        ...
                        print "<td>" . caHTMLRadioButtonInput(
                                $vs_table . '_type_' . $va_type['type_info']['item_id'],
        ...
        if ($t_role->getAppConfig()->get('perform_source_access_checking')) {
            ...
            <div id="source_list">
                ...
                foreach ($va_source_list as $vs_table => $va_sources_by_table) {
                    ...
                    print "<table width='100%'>\n";
                    print "<tr><td colspan='4'><h1>" . $va_table_names[$vs_table] . "</h1></td></tr>\n";
                    print "<tr align='center' valign='middle'><th>" . _t(
                            'Default'
                        ) . "</th><th width='180' align='left'>" . _t('Source') . "</th><th width='180'>" . _t(
                            'No access'
                        ) . "</th><th width='180'>" . _t('Read-only access') . "</th><th>" . _t(
                            'Read/edit access'
                        ) . "</th></tr>\n";
                    ...
                    foreach ($va_sources_by_table as $vn_id => $va_source) {
                        ...
                        print "<td>" . caHTMLRadioButtonInput(
                                $vs_table . '_source_' . $va_source['source_info']['item_id'],
                                array('value' => __CA_BUNDLE_ACCESS_NONE__),
                                array('checked' => ($vs_access == __CA_BUNDLE_ACCESS_NONE__))
                            ) . "</td>\n";
                        print "<td>" . caHTMLRadioButtonInput(
        ...
        <div id="pawtucket">
            <?php
            print "<p>" . _t(
                    'Access values are used to control if and to whom a record should be displayed in a public-facing Pawtucket-based interface. Users logged into the Pawtucket interface will be given the levels of access specified here. If you wish to use the default Pawtucket access policy for an access value set the level to "Use Pawtucket default."'
                ) . "</p>\n";
            ?>
            <table width='100%'>
                ...
                foreach ($va_access_status_list as $vn_id => $va_access_status) {
                    ...
                    print "<td>" . caHTMLRadioButtonInput(
                            'access_status_' . $va_access_status['access_status_info']['item_id'],
                            array('value' => 0),
                            array('checked' => ($vn_access === 0))
                        ) . "</td>\n";
                    print "<td>" . caHTMLRadioButtonInput(
                            'access_status_' . $va_access_status['access_status_info']['item_id'],
                            array('value' => 1),
                            array('checked' => ($vn_access === 1))
                        ) . "</td>\n";
                    print "<td>" . caHTMLRadioButtonInput(
                            'access_status_' . $va_access_status['access_status_info']['item_id'],
                            array('value' => null),
                            array('checked' => (is_null($vn_access)))
                        ) . "</td>\n";
                }
                ?>
                </tr>
