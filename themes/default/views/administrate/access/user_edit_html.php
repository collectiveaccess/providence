...
<div class="sectionBox">
	...
	print $vs_control_box = caFormControlBox(
		...
		caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', 'administrate/access', 'Users',
			'ListUsers', array( 'user_id' => 0 ) ),
		'',
		( $vn_user_id > 0 ) ? caFormNavButton( $this->request, __CA_NAV_ICON_DELETE__, _t( "Delete" ), '',
			'administrate/access', 'Users', 'Delete', array( 'user_id' => $vn_user_id ) ) : ''
	);
	?>
	<?php
		...
		switch ( $vs_f ) {
			case 'password':
				if ( AuthenticationManager::supports( __CA_AUTH_ADAPTER_FEATURE_UPDATE_PASSWORDS__ ) ) {
					...
					print $t_user->htmlFormElement( $vs_f, null, array(
						'value'        => '',
						'placeholder'  => _t( 'Change password' ),
						'field_errors' => $this->request->getActionErrors( 'field_' . $vs_f )
					) );
					print $t_user->htmlFormElement( $vs_f, str_replace( '^LABEL', _t( "Confirm password" ),
						$this->appconfig->get( 'form_element_display_format' ) ), array(
						'value'       => '',
				...
				break;
			case 'entity_id':
				print "<div class='formLabel'><span id='_ca_user_entity_id_'>" . ( $vs_entity_label
						= $t_user->getFieldInfo( 'entity_id', 'LABEL' ) ) . "</span><br/>";
				$vs_template = join( $this->request->config->get( 'ca_entities_lookup_delimiter' ),
					$this->request->config->get( 'ca_entities_lookup_settings' ) );
				print caHTMLTextInput( 'entity_id_lookup', array(
					'class' => 'lookupBg',
					'size'  => 70,
					'id'    => 'ca_users_entity_id_lookup',
					'value' => caProcessTemplateForIDs( $vs_template, 'ca_entities',
						array( $vn_entity_id = $t_user->get( 'entity_id' ) ) )
				) );
				if ( $vn_entity_id ) {
					print "<a href='#' onclick='caClearUserEntityID(); return false;'>" . _t( 'Clear' )
					      . " &rsaquo;</a>\n";
				}
				print caHTMLHiddenInput( 'entity_id',
				...
				break;
			default:
				print $t_user->htmlFormElement( $vs_f, null,
				...
				if ( is_array( $va_user_profile_settings ) && sizeof( $va_user_profile_settings ) ) {
					foreach ( $va_user_profile_settings as $vs_field => $va_info ) {
						if ( $va_errors[ $vs_field ] ) {
							print "<div class='formErrors' style='text-align: left;'>" . $va_errors[ $vs_field ]
							      . "</div>";
						}
