...
?>
	...
	<div class="searchReindexRowProgressGroup">
		...
		<div class="searchReindexElapsedTime" id="searchReindexElapsedTime"></div>
		<br style="clear: both;"/>
	</div>
...
<?php
function caIncrementSortValueReloadProgress(
	$pn_rows_complete, $pn_total_rows, $ps_row_message, $ps_table_message, $pn_elapsed_time, $pn_memory_used,
	$pa_table_list, $pn_table_num, $ps_display_name, $pn_tables_processed
) {
	...
	if ( is_null( $ps_table_message ) ) {
		$ps_table_message = _t( 'Reloading sort values for %1 (%2/%3)', $ps_display_name, $pn_tables_processed,
			sizeof( $pa_table_list ) );
	}
	...
	print "jQuery('#searchReindexElapsedTime').html('" . caFormatInterval( $pn_elapsed_time ) . "/"
