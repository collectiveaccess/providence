...
if (true) { //ca_search_indexing_queue::lockExists()) {
    if (true) {//ca_search_indexing_queue::lockCanBeRemoved()) {
        print caFormTag(
            $this->request,
            'RemoveLock',
            'caClearSearchIndexingLockFileForm',
            null,
            'post',
            'multipart/form-data',
            '_top',
        ...
        print "<div style='text-align: center'>" . caFormSubmitButton(
                $this->request,
                __CA_NAV_ICON_GO__,
                _t("Clear search indexing queue lock file"),
                'caClearSearchIndexingLockFileForm',
                array()
            ) . "</div>";
        print "</form>";
        ...
        print _t(
            "<p>You cannot clear the lock file as <em>it is not writeable by the web server user.</em> Check file permissions and try again.</p>"
        );
    }
} else {
