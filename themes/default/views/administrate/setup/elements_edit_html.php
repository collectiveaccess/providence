...
<div class="sectionBox">
    ...
    foreach ($t_element->getFormFields() as $vs_f => $va_user_info) {
        ...
        switch ($vs_f) {
            case 'element_code':
                if ($t_element->getPrimaryKey()) {
                    if ((bool)$t_element->getAppConfig()->get(
                        'ca_metadata_elements_dont_allow_editing_of_codes_when_in_use'
                    )) {
                        $vb_element_editable = false;
                        $vs_warning = '<span class="formLabelWarning"><i class="caIcon fa fa-info-circle fa-1x"></i> ' . _t(
                                'Value cannot be edited because it is in use'
                            ) . '</span>';
                    } else {
                        $vs_warning = '<span class="formLabelWarning"><i class="caIcon fa fa-exclamation-triangle fa-1x"></i> ' . _t(
                                'Changing this value may break parts of the system configuration'
                            ) . '</span>';
                    }
                }
                break;
            case 'datatype':
                if ($t_element->getPrimaryKey()) {
                    if ((bool)$t_element->getAppConfig()->get(
                            'ca_metadata_elements_dont_allow_editing_of_data_types_when_in_use'
                        ) && ca_metadata_elements::elementIsInUse($vn_element_id)) {
                        $vb_element_editable = false;
                        $vs_warning = '<span class="formLabelWarning"><i class="caIcon fa fa-info-circle fa-1x"></i> ' . _t(
                                'Element type cannot be changed because element is in use'
                            ) . '</span>';
                    } else {
                        $vs_warning = '<span class="formLabelWarning"><i class="caIcon fa fa-exclamation-triangle fa-1x"></i> ' . _t(
                                'Changing this value may delete existing data in this element'
                            ) . '</span>';
                    }
                }
                break;
        ...
        print $t_element->htmlFormElement(
            $vs_f,
            "<div class='formLabel'>^EXTRA^LABEL<br/>^ELEMENT<br/>{$vs_warning}</div>",
            array_merge(
                $va_options,
                array(
    ...
    <div id='elementSettingsForm' class='formSettings'>
        <div class='formLabel'><span id="_ca_metadata_element_labels_"><?php print _t(
                    "Datatype-specific options"
                ); ?></span><br/></div>
        <div style="margin-left: 20px;">
            <?php
            foreach ($va_available_settings as $vs_code => $va_properties) {
                print $t_element->settingHTMLFormElement(
                    $vs_code,
		...
		<textarea class='caItemTemplate' style='display: none;'>
			<div id="Item_{n}" class="labelInfo">
				...
				<table class="objectRepresentationListItem">
					<tr valign="top">
						<td><?php
                            print $t_restriction->htmlFormElement(
                                'table_num',
                                null,
                                array(
                                    'classname' => '',
...
?>
						...
						<td><?php
                            foreach ($t_restriction->getAvailableSettings() as $vs_setting => $va_setting_info) {
                                print $t_restriction->settingHTMLFormElement(
                                    $vs_setting,
                                    array(
        ...
        <div class="bundleContainer">
            ...
            <div class='button labelInfo caAddItemButton'><a href='#'><?php print caNavIcon(
                        __CA_NAV_ICON_ADD__,
                        1
                    ); ?> <?php print _t("Add type restriction"); ?> &rsaquo;</a></div>
        </div>
    </div>
    <?php
    ...
    <script type="text/javascript">
        ...
        caUI.initBundle('#type_restrictions', {
            ...
            templateValues: <?php print json_encode(
                array_merge(
                    array_keys($t_restriction->getAvailableSettings()),
        ...
        <div class="bundleContainer">
            <div class="caLabelList">
                ...
                foreach ($va_sub_elements as $va_sub_element):
                    ?>
                    <div class="labelInfo">
                        <a href="<?php print caNavUrl(
                            $this->request,
                            'administrate/setup',
                            'Elements',
                            'MoveElementUp',
                            array(
                                'parent_id' => $vn_element_id,
                                'element_id' => $va_sub_element['element_id']
                            )
				...
				<?php print $va_sub_element['name'] . ' (' . $va_sub_element['element_code'] . ') [' . ca_metadata_elements::getAttributeNameForTypeCode(
                        $va_sub_element['datatype']
                    ) . ']'; ?>
			</span>
                    </div>
                <?php
...
<script type="text/javascript">
    function caSetElementsSettingsForm(opts) {
        ...
        jQuery("#elementSettingsForm").load('<?php print caNavUrl(
            $this->request,
            'administrate/setup',
            'Elements',
            'getElementSettingsForm'
        ); ?>', opts, function () {
