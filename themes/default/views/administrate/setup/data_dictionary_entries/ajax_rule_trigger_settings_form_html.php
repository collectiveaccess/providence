	...
	<div class="formLabel"><?php print _t( 'Attach to metadata element' ); ?><br/>
		...
		<?php print ca_metadata_elements::getElementListAsHTMLSelect( "{$vs_id_prefix}_element_id",
			[ "id" => "{$vs_id_prefix}_element_id" ], [
				...
				'addEmptyOption'      => true,
				'emptyOption'         => '-',
				'value'               => ( $vn_element_id = $t_trigger->get( 'element_id' ) ) ? $vn_element_id
					: $va_triggers['_non_element_filter'],
	...
	<script type="text/javascript">
		jQuery(document).ready(function () {
			jQuery("#<?php print $vs_id_prefix . '_element_id'; ?>").off().on('change', function () {
				jQuery("#<?php print $vs_id_prefix; ?>_filter").load('<?php print caNavUrl( $this->request,
					'manage/metadata_alert_rules', 'MetadataAlertRuleEditor', 'getTriggerTypeFilterForm' ); ?>', {
