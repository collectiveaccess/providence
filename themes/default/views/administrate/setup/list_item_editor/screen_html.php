...
$vs_context_id
	= $this->getVar( '_context_id' );    // used to restrict idno uniqueness checking to within the current list
...
if ( $vb_can_edit ) {
	print $vs_control_box = caFormControlBox(
		...
		caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '',
			'administrate/setup/list_item_editor', 'ListItemEditor', 'Edit/' . $this->request->getActionExtra(),
			array( 'item_id' => $vn_item_id ) ),
		'',
		( ( intval( $vn_item_id ) > 0 ) && $vb_can_delete ) ? caNavButton( $this->request, __CA_NAV_ICON_DELETE__,
			_t( "Delete" ), '', 'administrate/setup/list_item_editor', 'ListItemEditor',
			'Delete/' . $this->request->getActionExtra(), array( 'item_id' => $vn_item_id ) ) : ''
	);
}
?>
<div class="sectionBox">
	...
	if ( ! $vb_can_edit ) {
		...
		<div class='notification-warning-box'>
			<ul class='notification-warning-box'>
				<li class='notification-info-box'><?php print ( ( intval( $vn_item_id ) == 0 )
						? _t( "You are not allowed to add items to this list" )
						: _t( "You are not allowed to edit items in this list" ) ); ?></li>
			</ul>
		</div>
		<?php
	...
	if ( ( intval( $vn_item_id ) > 0 ) || $vb_can_edit ) {
		...
		print caFormTag( $this->request, 'Save/' . $this->request->getActionExtra() . '/item_id/' . $vn_item_id,
			'ListItemEditorForm', null, 'POST', 'multipart/form-data' );
