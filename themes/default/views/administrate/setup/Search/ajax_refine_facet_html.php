...
if ( ( ! isset( $va_facet_info['groupings'][ $vs_grouping_field ] )
       || ! ( $va_facet_info['groupings'][ $vs_grouping_field ] ) )
...
if ( $vs_grouping_attribute_element_code = ( preg_match( '!^ca_attribute_([\w]+)!', $vs_grouping_field, $va_matches ) )
	? $va_matches[1] : null
) {
	...
	switch ( $va_facet_info['group_mode'] ) {
			...
			<div class="browseSelectPanelList">
				<table class='browseSelectPanelListTable'>
					...
					foreach ( $va_facet as $vn_i => $va_item ) {
						?>
						<?php
						$va_row[] = "<td class='browseSelectPanelListCell'>" . caNavLink( $this->request,
								$va_item['label'], 'browseSelectPanelLink', $this->request->getModulePath(),
		...
		default:
			...
			$va_label_order_by_fields = isset( $va_facet_info['order_by_label_fields'] )
				? $va_facet_info['order_by_label_fields'] : array( 'label' );
			foreach ( $va_facet as $vn_i => $va_item ) {
				$va_groups = array();
							...
							switch ( $vn_element_datatype ) {
								case 2: //date
									...
									if ( isset( $va_item[ 'ca_attribute_' . $vn_grouping_attribute_id ] )
										...
										foreach (
											$va_item[ 'ca_attribute_' . $vn_grouping_attribute_id ] as $vn_i => $va_v
										) {
