...
if (!$this->request->isAjax()) {
    if (!$this->getVar('uses_hierarchy_browser')) {
        ...
        print caFormControlBox(
            '<div class="simple-search-box">' . _t(
                'Search'
            ) . ': <input type="text" id="browseSearch" name="search" value="' . htmlspecialchars(
                $this->getVar('search'),
                ENT_QUOTES,
                'UTF-8'
            ) . '" size="40"/></div>' .
                    ...
                    <form action='#'>
                        ...
                        if ($vs_table == 'ca_list_items') {
                            ...
                            <div style="float: right;">
                                <?php print caNavLink(
                                    $this->request,
                                    caNavIcon(__CA_NAV_ICON_ADD__, "30px") . ' ' . _t('Add new list'),
                                    'list-link',
                    ...
                    <form action='#'>
                        ...
                        if ($vs_table == 'ca_list_items') {
                            ...
                            <div style="float: right;">
                                <?php print caNavLink(
                                    $this->request,
                                    caNavIcon(__CA_NAV_ICON_ADD__, 1) . ' ' . _t('Add new list'),
                                    'list-link',
                                    'administrate/setup/list_editor',
                                    'ListEditor',
                                    'Edit',
                        ...
                        if ($this->getVar('num_types') > 0) {
                            ...
                            print _t(
                                'Add under %2 new %1',
                                _t('item') . ' <a href="#" onclick="_navigateToNewForm(0)">' . caNavIcon(
                                    __CA_NAV_ICON_ADD__,
                                    1
                                ) . "</a>",
                                "<span id='browseCurrentSelection'>?</span>"
            ...
            jQuery(document).ready(function () {
                ...
                oHierBrowser = caUI.initHierBrowser('hierarchyBrowser', {
                    ...
                    allowDragAndDropSorting: <?php print is_array(
                        $vm_ret = caGetDragAndDropSortingAvailabilityMap(
                            $this->request,
                            $t_subject->tableName(),
                            $t_subject->getPrimaryKey()
                        )
                    ) ? json_encode($vm_ret) : ($vm_ret ? "true" : "false"); ?>,
