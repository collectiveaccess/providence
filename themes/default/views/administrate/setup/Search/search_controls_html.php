...
if ( ! $this->request->isAjax() ) {
	if ( ! $this->getVar( 'uses_hierarchy_browser' ) ) {
		...
		print caFormControlBox(
			'<div class="simple-search-box">' . _t( 'Search' )
			. ': <input type="text" id="browseSearch" name="search" value="'
			...
			caFormJSButton( $this->request, __CA_NAV_ICON_SEARCH__, _t( "Search" ), 'submitSearch',
				array(
					'href'    => '#',
					'onclick' => 'caCloseBrowser(); jQuery("#resultBox").load("' . caNavUrl( $this->request,
							$this->request->getModulePath(), $this->request->getController(), 'Index',
							array( 'search' => '' ) )
					             . '" + escape(jQuery("#browseSearch").attr("value"))); return false;'
				) ),
			'',
				...
				if ( $this->getVar( 'num_types' ) > 0 ) {
					...
					<form action='#'>
						...
						if ( $vs_table == 'ca_list_items' ) {
							...
							<div style="float: right;">
								<?php print caNavLink( $this->request,
									caNavIcon( __CA_NAV_ICON_ADD__, "30px" ) . ' ' . _t( 'Add new list' ), 'list-link',
									'administrate/setup/list_editor', 'ListEditor', 'Edit',
									array( 'list_id' => 0 ) ); ?>
							</div>
							<?php
						}
					...
					<form action='#'>
						...
						if ( $vs_table == 'ca_list_items' ) {
							...
							<div style="float: right;">
								<?php print caNavLink( $this->request,
									caNavIcon( __CA_NAV_ICON_ADD__, 1 ) . ' ' . _t( 'Add new list' ), 'list-link',
									'administrate/setup/list_editor', 'ListEditor', 'Edit',
									array( 'list_id' => 0 ) ); ?>
							</div>
							<?php
						...
						if ( $this->getVar( 'num_types' ) > 0 ) {
							...
							print _t( 'Add under %2 new %1',
								_t( 'item' ) . ' <a href="#" onclick="_navigateToNewForm(0)">'
								. caNavIcon( __CA_NAV_ICON_ADD__, 1 ) . "</a>",
								"<span id='browseCurrentSelection'>?</span>" );
						}
						print "</div>";
		...
		<script type="text/javascript">
			...
			jQuery(document).ready(function () {
				...
				oHierBrowser = caUI.initHierBrowser('hierarchyBrowser', {
					...
					allowDragAndDropSorting: <?php print is_array(
						$vm_ret = caGetDragAndDropSortingAvailabilityMap( $this->request, $t_subject->tableName(),
							$t_subject->getPrimaryKey() ) ) ? json_encode( $vm_ret )
						: ( $vm_ret ? "true" : "false" ); ?>,
					...
					dontAllowEditForFirstLevel: <?php print ( $vs_table == 'ca_relationship_types' ) ? 'true'
						: 'false'; ?>
				});
			...
			function _navigateToNewForm(type_id) {
				document.location = '<?php print caEditorUrl( $this->request, $vs_table,
					0 ); ?>/type_id/' + type_id + '/parent_id/' + oHierBrowser.getSelectedItemID();
			}
		</script>
