...
$lookup_urls = caJSONLookupServiceUrl($this->request, $table, array('noInline' => 1));
...
<?php 
		print caFormControlBox(
			'<div class="simple-search-box">'._t('Search').': <input type="text" id="BasicSearchInput" name="search" value="'.htmlspecialchars($this->getVar('search'), ENT_QUOTES, 'UTF-8').'" size="40"/>'.$type_id_form_element.'</div>',
			'',
		...
		print caFormTag($this->request, 'Index', 'BasicSearchForm', null, 'post', 'multipart/form-data', '_top', array('noCSRFToken' => true, 'disableUnsavedChangesWarning' => true));
			print caFormControlBox(
				'<div class="simple-search-box">'._t('Search').': <input type="text" id="browseSearch" name="search" value="'.htmlspecialchars($this->getVar('search'), ENT_QUOTES, 'UTF-8').'" size="40"/></div>'.
					caFormJSButton($this->request, __CA_NAV_ICON_SEARCH__, _t("Search"), 'submitSearch', ['href' => '#', 'onclick' => 'caCloseBrowser(); jQuery("#resultBox").load("'.caNavUrl($this->request, $this->request->getModulePath(), $this->request->getController(), 'Index', ['search' => ''], ['allowEmptyParameters' => true]).'" + escape(jQuery("#browseSearch").val())); return false;']),
				'',
		...
		<script type="text/javascript">
			...
			jQuery(document).ready(function() {
				...
				oHierBrowser = caUI.initHierBrowser('hierarchyBrowser', {
					...
					editUrl: '<?= caEditorUrl($this->request, $table); ?>',
					...
					dontAllowEditForFirstLevel: <?= ($table == 'ca_relationship_types') ? 'true' : 'false'; ?>
				});
				
				jQuery('#browseSearch').autocomplete(
					{
						minLength: 3, delay: 800, html: true,
						source: '<?= $lookup_urls['search']; ?>',
						select: function(event, ui) {
							if (parseInt(ui.item.id) > 0) {
								oHierBrowser.setUpHierarchy(ui.item.id);	// jump browser to selected item
								if (stateCookieJar.get('<?= $table; ?>BrowserIsClosed') == 1) {
				...
				jQuery("#browseToggle").click(function() {
					jQuery("#browse").slideToggle(350, function() { 
						stateCookieJar.set('<?= $table; ?>BrowserIsClosed', (this.style.display == 'block') ? 0 : 1); 
				...
				if (<?= ($this->getVar('force_hierarchy_browser_open') ? 'true' : "!stateCookieJar.get('".$table."BrowserIsClosed')"); ?>) {
			...
			function caCloseBrowser() {
				if (!stateCookieJar.get('<?= $table; ?>BrowserIsClosed')) {
					jQuery("#browseToggle").click();
				}
			}
			function _navigateToNewForm(type_id) {
				document.location = '<?= caEditorUrl($this->request, $table, 0); ?>/type_id/' + type_id + '/parent_id/' + oHierBrowser.getSelectedItemID();
			}
		</script>
