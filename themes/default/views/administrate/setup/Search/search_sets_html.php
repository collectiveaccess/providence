...
?>
<div class='setTools'>
	<a href="#" id='searchSetToolsShow'
	   onclick="$('.setTools').hide(); return caShowSearchSetTools();"><?php print caNavIcon( __CA_NAV_ICON_SETS__, 1 )
	                                                                               . " " . _t( "Set Tools" ); ?></a>
</div><!-- end setTools -->
...
<div id="searchSetTools">
	...
	if ( is_array( $va_sets = $this->getVar( 'available_sets' ) ) && sizeof( $va_sets ) ) {
		...
		<div class="col">
			...
			<form id="caAddToSet">
				...
				print caHTMLSelect( 'set_id', $va_options,
					array( 'id' => 'caAddToSetID', 'class' => 'searchSetsSelect' ),
				...
				<div class="searchSetsToggle"><a href="#" onclick="return caToggleAddToSet();"
				                                 class="searchSetsToggle"><?php print _t( "Toggle checked" ); ?></a>
				</div>
			</form>
		</div>
		<br class="clear"/>
	...
	<div class="col">
		...
		<form id="caCreateSetFromResults">
			...
			print caHTMLTextInput( 'set_name', array(
				'id'    => 'caCreateSetFromResultsInput',
				'class' => 'searchSetsTextInput',
				'value' => $o_result_context->getSearchExpression()
			), array( 'width' => '150px' ) );
			print " ";
			...
			?>
			<a href='#' onclick="caCreateSetFromResults(); return false;" class="button"><?php print _t( 'Create' ); ?>
				&rsaquo;</a>
			<?php
			if ( $this->request->user->canDoAction( 'can_batch_edit_' . $t_subject->tableName() ) ) {
				print '<div class="searchSetsBatchEdit">' . caHTMLCheckboxInput( 'batch_edit',
						array( 'id' => 'caCreateSetBatchEdit', 'value' => 1 ) ) . " "
				      . _t( 'Open set for batch editing' ) . "</div>\n";
			}
			?>
		</form>
	</div>
	...
	<a href='#' id='hideSets'
	   onclick='caHideSearchSetTools(); $(".setTools").slideDown(250); '><?php print caNavIcon( __CA_NAV_ICON_COLLAPSE__,
			1 ); ?></a>
	<div class="clear">&nbsp;</div>
...
<script type="text/javascript">
	...
	function caAddItemsToSet() {
		...
		jQuery.post(
			'<?php print caNavUrl( $this->request, $this->request->getModulePath(), $this->request->getController(),
				'addToSet' ); ?>',
			{
			...
			function (res) {
				...
				if (res['status'] === 'ok') {
					...
					if (jQuery('#caCreateSetBatchEdit').prop('checked')) {
						window.location = '<?php print caNavUrl( $this->request, 'batch', 'Editor', 'Edit',
							array() ); ?>/set_id/' + res['set_id'];
					} else {
