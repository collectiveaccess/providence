...
<div class='setTools'>
    <a href="#" id='searchSetToolsShow'
       onclick="$('.setTools').hide(); return caShowSearchSetTools();"><?php print caNavIcon(
                __CA_NAV_ICON_SETS__,
                1
            ) . " " . _t("Set Tools"); ?></a>
</div><!-- end setTools -->
...
<div id="searchSetTools">
        ...
        <div class="col">
            ...
            <form id="caAddToSet">
                ...
                <div class="searchSetsToggle"><a href="#" onclick="return caToggleAddToSet();"
                                                 class="searchSetsToggle"><?php print _t("Toggle checked"); ?></a></div>
            </form>
        </div>
        <br class="clear"/>
        <?php
    ...
    <div class="col">
        ...
        <form id="caCreateSetFromResults">
            ...
            if ($this->request->user->canDoAction('can_batch_edit_' . $t_subject->tableName())) {
                print '<div class="searchSetsBatchEdit">' . caHTMLCheckboxInput(
                        'batch_edit',
                        array(
                            'id' => 'caCreateSetBatchEdit',
                            'value' => 1
                        )
    ...
    <a href='#' id='hideSets' onclick='caHideSearchSetTools(); $(".setTools").slideDown(250); '><?php print caNavIcon(
        ...
        jQuery.post(
            ...
            function (res) {
                ...
                if (res['status'] === 'ok') {
                    ...
                    var msg = '<?php print addslashes(
                        _t('Created set <i>^set_name</i> with ^num_items ^item_type_name')
                    );?>';
                    msg = msg.replace('^num_items', res['num_items_added']);
                    msg = msg.replace('^item_type_name', item_type_name);
                    msg = msg.replace('^set_name', res['set_name']);
