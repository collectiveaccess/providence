			...
			<table class="listtable">
					...
					foreach ( $va_display_list as $va_display_item ) {
						if ( $va_display_item['is_sortable'] ) {
							if ( $vs_current_sort == $va_display_item['bundle_name'] ) {
								print "<th class='list-header-sorted-asc'><span id='listHeader" . $vn_id_count
								      . "'><nobr>" . $va_display_item['display'] . "</nobr></span></th>";
								TooltipManager::add( '#listHeader' . $vn_id_count,
									'Currently sorting by ' . $va_display_item['display'] );
							} else {
								print "<th class='list-header-unsorted'><span id='listHeader1" . $vn_id_count
								      . "'><nobr>" . caNavLink( $this->request, $va_display_item['display'], '',
										$this->request->getModulePath(), $this->request->getController(), 'Index',
										array( 'sort' => $va_display_item['bundle_name'] ) ) . "</nobr></span></th>";
								TooltipManager::add( '#listHeader1' . $vn_id_count,
									'Click to sort by ' . $va_display_item['display'] );
							}
						} else {
							print "<th class='list-header-nosort'><span id='listHeader2" . $vn_id_count . "'><nobr>"
							      . $va_display_item['display'] . "</nobr></span></th>";
				...
				while ( ( $vn_item_count < $vn_items_per_page ) && $vo_result->nextHit() ) {
					...
					<tr <?php print ( $i == 1 ) ? "class='odd'" : ""; ?>>
						...
						print "<td class='listtableEditDelete'>" . caEditorLink( $this->request,
								caNavIcon( __CA_NAV_ICON_EDIT__, 2 ), 'editIcon', 'ca_relationship_types', $vn_type_id,
								array() );
						print " <a href='#' onclick='caOpenBrowserWith({$vn_type_id}); return false;' class='hierarchyIcon'>"
						      . caNavIcon( __CA_NAV_ICON_HIER__, 2,
								array( 'title' => _t( 'View in hierarchy' ), 'style' => 'margin-top:5px;' ),
								array( 'rotate' => 270 ) ) . "</a>";
						print "</td>";
						?>
					</tr>
