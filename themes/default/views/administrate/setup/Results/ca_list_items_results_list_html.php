            ...
            <table class="listtable">
                    ...
                    foreach ($va_display_list as $va_display_item) {
                        if ($va_display_item['is_sortable']) {
                            if ($vs_current_sort == $va_display_item['bundle_sort']) {
                                print "<th class='list-header-sorted-asc'><span id='listHeader" . $vn_id_count . "'><nobr>" . ((mb_strlen(
                                            $va_display_item['display']
                                        ) > 17) ? strip_tags(
                                            mb_substr($va_display_item['display'], 0, 15)
                                ...
                                print "<th class='list-header-unsorted'><span id='listHeader1" . $vn_id_count . "'><nobr>" . caNavLink(
                                        $this->request,
                                        ((mb_strlen($va_display_item['display']) > 17) ? strip_tags(
                                                mb_substr($va_display_item['display'], 0, 15)
                                            ) . "..." : $va_display_item['display']),
                                        '',
                            ...
                            print "<th class='list-header-nosort'><span id='listHeader2" . $vn_id_count . "'><nobr>" . ((mb_strlen(
                                        $va_display_item['display']
                                    ) > 17) ? strip_tags(
                                        mb_substr($va_display_item['display'], 0, 15)
                                    ) . "..." : $va_display_item['display']) . "</nobr></span></th>";
                ...
                while (($vn_item_count < $vn_items_per_page) && $vo_result->nextHit()) {
                    ...
                    <tr <?php print ($i == 1) ? "class='odd'" : ""; ?>>
                        ...
                        print " <a href='#' onclick='caOpenBrowserWith({$vn_item_id}); return false;'>" . caNavIcon(
                                __CA_NAV_ICON_GO__,
                                2,
                                array('title' => _t('View in hierarchy'))
                            ) . "</a>";
                        print "</td>";
                        ?>
                    </tr>
