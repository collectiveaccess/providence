...
while($result->nextHit()) {
	...
	$contentCell->addText(
		html_entity_decode(strip_tags(br2nl($result->get('preferred_labels'))), ENT_QUOTES | ENT_HTML5),
		$styleHeaderFont
	);

	foreach($list as $placement_id => $display_item) {

		if (
			(strpos($display_item['bundle_name'], 'ca_object_representations.media') !== false)
			&&
			($display_item['settings']['display_mode'] == 'media') // make sure that for the 'url' mode we don't insert the image here
		) {
			// Inserting bundle name on one line
			$contentCell->addText($display_item['display'].' :', $styleBundleNameFont);

			// Fetching version asked & corresponding file
			$version = str_replace("ca_object_representations.media.", "", $display_item['bundle_name']);
		...
		} elseif ($display_text = $t_display->getDisplayValue($result, $placement_id, array('request' => $this->request))) {
			$textrun = $contentCell->addTextRun();
			$textrun->addText($display_item['display'].' :', $styleBundleNameFont);
			$textrun->addText(
				html_entity_decode(strip_tags(br2nl($display_text)), ENT_QUOTES | ENT_HTML5),
				$styleContentFont
			);
