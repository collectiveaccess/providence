...
<div class="caLibraryFindAutocompleteContainer">
	<form>
		...
		<?php print caHTMLTextInput( 'user', array( 'id' => 'user_autocomplete' ),
...
<script type="text/javascript">
	jQuery(document).ready(function () {
		...
		jQuery('#user_autocomplete').autocomplete(
			{
				source: '<?php print caNavUrl( $this->request, 'lookup', 'User', 'Get',
					array( 'max' => 100, 'inlineCreate' => 0, 'quickadd' => 0 ) ); ?>',
				minLength: 3, delay: 800, html: true,
				select: function (event, ui) {
					var user_id = ui.item.id;
					if (parseInt(user_id) && (user_id > 0)) {
						jQuery('#nextButton').fadeIn(500).attr('href', '<?php print caNavUrl( $this->request, '*', '*',
