...
<script type="text/javascript">
    ...
    var caDashboardWidgetPanel = caUI.initWidgetPanel({
        widgetUrl: '<?php print caNavUrl(
            $this->request,
            '',
            'Dashboard',
            'getAvailableWidgetList'
        ); ?>'
    });
</script>
<div id="dashboardControls">
    <div id="clearDashboardControl" style="float: left;">
        <?php print caNavLink(
            $this->request,
            _t('Clear dashboard') . ' &rsaquo;',
            'dashboardControl',
            '',
            'Dashboard',
            'clear',
            array(),
            array('id' => 'dashboardClearButton')
        );
        print caNavLink(
            $this->request,
            _t('Default dashboard') . ' &rsaquo;',
            'dashboardControl',
            '',
            'Dashboard',
            'default',
            array(),
            array('id' => 'dashboardDefaultButton')
        ); ?>
    </div>
...
<div class="dashboard">
    <div class="dashboardWelcomeMessage" id="dashboard_welcome_message"><?php print _t(
            'This is your CollectiveAccess dashboard.  Click the "Edit Dashboard" button above to add widgets to your dashboard that will allow you to monitor system activity.  You\'ll see your dashboard whenever you login or  click the CollectiveAccess logo above.'
        ); ?></div>
    <div class="dashboardWelcomeMessage" id="dashboard_edit_message"><?php print _t(
...
function caGetDashboardWidgetHTML($po_request, $pn_column)
{
    ...
    foreach ($va_widget_list as $vn_i => $va_widget_info) {
        if (!($widget_content = $o_dashboard_manager->renderWidget(
            $va_widget_info['widget'],
            $va_widget_info['widget_id'],
            $va_widget_info['settings']
        ))) {
            continue;
        }
        ...
        print caNavLink(
            $po_request,
            caNavIcon(__CA_NAV_ICON_DELETE__, '16px'),
        ...
        if ($o_dashboard_manager->widgetHasSettings($va_widget_info['widget'])) {
            print "<a href='#' class='dashboardWidgetSettingsButton' onclick='jQuery(\"#content_" . $va_widget_info['widget_id'] . "\").load(\"" . caNavUrl(
                    $po_request,
                    '',
                    'Dashboard',
                    'getSettingsForm'
                ) . "\", { widget_id: \"" . $va_widget_info['widget_id'] . "\" }); return false;'>" . caNavIcon(
