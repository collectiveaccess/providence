...
<script type="text/javascript">
	...
	var caDashboardWidgetPanel = caUI.initWidgetPanel({
		widgetUrl: '<?php print caNavUrl( $this->request, '', 'Dashboard', 'getAvailableWidgetList' ); ?>'
	});
</script>
<div id="dashboardControls">
	<div id="clearDashboardControl" style="float: left;">
		<?php print caNavLink( $this->request, _t( 'Clear dashboard' ) . ' &rsaquo;', 'dashboardControl', '',
			'Dashboard', 'clear', array(), array( 'id' => 'dashboardClearButton' ) );
		print caNavLink( $this->request, _t( 'Default dashboard' ) . ' &rsaquo;', 'dashboardControl', '', 'Dashboard',
			'default', array(), array( 'id' => 'dashboardDefaultButton' ) ); ?>
	</div>

	<a href="#" onclick='caDashboard.editDashboard(1);' class='dashboardControl'
	   id='dashboardEditButton'><?php print _t( 'Edit dashboard' ); ?> &rsaquo;</a>
	<a href="#" onclick='caDashboardWidgetPanel.showWidgetPanel();' class='dashboardControl'
	   id='dashboardAddButton'><?php print _t( 'Add widget' ); ?> &rsaquo;</a>
	<a href="#" onclick='caDashboard.editDashboard(0);' class='dashboardControl'
	   id='dashboardDoneEditingButton'><?php print _t( 'Done' ); ?> &rsaquo;</a>
</div>
<div class="dashboard">
	<div class="dashboardWelcomeMessage"
	     id="dashboard_welcome_message"><?php print _t( 'This is your CollectiveAccess dashboard.  Click the "Edit Dashboard" button above to add widgets to your dashboard that will allow you to monitor system activity.  You\'ll see your dashboard whenever you login or  click the CollectiveAccess logo above.' ); ?></div>
	<div class="dashboardWelcomeMessage"
	     id="dashboard_edit_message"><?php print _t( 'Use the button above to add a widget to your dashboard.  You can drag and drop the widgets in the left or right columns in the order you would like them to appear.  To customize the information in each widget, click the <i>"i"</i> button in the upper right corner of the widget.  To remove the widget from your dashboard click the "X" button in the upper right corner of the widget.  Click the "Clear dashboard" button above to remove all widgets from your dashboard.  When you are finished editing your dashboard, click the "Done" button above.' ); ?></div>
...
function caGetDashboardWidgetHTML( $po_request, $pn_column ) {
	...
	foreach ( $va_widget_list as $vn_i => $va_widget_info ) {
		if ( ! ( $widget_content = $o_dashboard_manager->renderWidget( $va_widget_info['widget'],
			$va_widget_info['widget_id'], $va_widget_info['settings'] ) )
		) {
			continue;
		}
		print "<div class='portlet' id='dashboardWidget_{$pn_column}_{$vn_i}'>";
		print caNavLink( $po_request, caNavIcon( __CA_NAV_ICON_DELETE__, '16px' ), 'dashboardRemoveWidget', '',
			'Dashboard', 'removeWidget',
			array( 'widget' => $va_widget_info['widget'], 'widget_id' => $va_widget_info['widget_id'] ) );
		if ( $o_dashboard_manager->widgetHasSettings( $va_widget_info['widget'] ) ) {
			print "<a href='#' class='dashboardWidgetSettingsButton' onclick='jQuery(\"#content_"
			      . $va_widget_info['widget_id'] . "\").load(\"" . caNavUrl( $po_request, '', 'Dashboard',
					'getSettingsForm' ) . "\", { widget_id: \"" . $va_widget_info['widget_id']
			      . "\" }); return false;'>" . caNavIcon( __CA_NAV_ICON_INFO__, '16px' ) . "</a>";
		}
		print '<div class="portlet-header">' . WidgetManager::getWidgetTitle( $va_widget_info['widget'] ) . '</div>';
		print '<div class="portlet-content" id="content_' . $va_widget_info['widget_id'] . '">' . $widget_content
		      . '</div>';
		print '</div>';
	}
}
