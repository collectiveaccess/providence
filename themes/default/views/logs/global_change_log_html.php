...
<div class="sectionBox">
    ...
    print caFormControlBox(
        ...
        caHTMLSelect(
            'filter_change_type',
            [_t('--Any--') => '', _t('Added') => 'I', _t('Edited') => 'U', _t('Deleted') => 'D'],
            null,
            ['value' => $vs_filter_change_type]
        ) .
        caHTMLSelect(
            'filter_table',
            array_merge([_t('--Any--') => ''], caGetPrimaryTablesForHTMLSelect()),
            null,
            ['value' => $vn_filter_table]
        ),
        _t('Show from') . ': ' . caHTMLTextInput(
            'change_log_search',
            array('size' => 15, 'value' => $this->getVar('change_log_search'))
        ...
        if (sizeof($va_change_log_list)) {
            foreach ($va_change_log_list as $vs_log_key => $va_log_entry) {
                ...
                <tr>
                    ...
                    <td>
                        <?php print Datamodel::getInstance($va_log_entry[0]['subject_table_num'], true)->getProperty(
                            'NAME_PLURAL'
                        ); ?>
                    </td>
                    <td>
                        <?php
                        print "<span style='font-size:12px; font-weight:bold;'><a href='" . caEditorUrl(
                                $this->request,
                                $va_log_entry[0]['subject_table_num'],
                                $va_log_entry[0]['subject_id']
            ...
            <tr>
                <td colspan='5'>
                    <div align="center">
                        <?php print (trim($this->getVar('change_log_search'))) ? _t('No log entries found') : _t(
                            'Enter a date to display change log from above'
                        ); ?>
                    </div>
                </td>
            </tr>
