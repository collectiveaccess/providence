...
$vs_site_page_template_menu = $vs_page_select ? '<div class="sf-small-menu form-header-button rounded" style="padding: 6px;">' .
    ...
    '<a href="#" onclick="_navigateToNewForm(jQuery(\'#templateList\').val(), jQuery(\'#tableList\').val());">' . caNavIcon(
        __CA_NAV_ICON_ADD__,
        2
    ) . '</a>' .
    '</div>' .
    ...
    <table id="caItemList" class="listtable">
        ...
        if (sizeof($va_page_list)) {
            ...
            <tr>
                <td colspan='8'>
                    <div align="center">
                        <?php print $vs_page_select ? _t('No pages have been created') : _t(
                            'No page templates are been defined'
                        ); ?>
                    </div>
                </td>
            </tr>
