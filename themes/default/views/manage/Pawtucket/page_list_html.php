...
$vs_site_page_template_menu = $vs_page_select
	? '<div class="sf-small-menu form-header-button rounded" style="padding: 6px;">' .
	  '<div class="caNavHeaderIcon">' .
	  '<a href="#" onclick="_navigateToNewForm(jQuery(\'#templateList\').val(), jQuery(\'#tableList\').val());">'
...
<script type="text/javascript">
	...
	function _navigateToNewForm(template_id) {
		document.location = '<?php print caNavUrl( $this->request, 'manage/site_pages', 'SitePageEditor', 'Edit',
			array( 'page_id' => 0, 'template_id' => '' ) ); ?>' + template_id;
	}
</script>
		...
		if ( sizeof( $va_page_list ) ) {
			foreach ( $va_page_list as $va_page ) {
				...
				<tr>
					...
					<td>
						<div><?php print ( $va_page['path'] === 'PROVIDENCE_HELP_MENU' )
								? _t( '<em>Providence help menu</em>' ) : $va_page['path']; ?></div>
					</td>
					<td>
					...
					<td class="listtableEditDelete">
						<?php print caNavButton( $this->request, __CA_NAV_ICON_EDIT__, _t( "Edit" ), '',
							'manage/site_pages', 'SitePageEditor', 'Edit', array( 'page_id' => $va_page['page_id'] ),
							array(), array(
						...
						<?php print caNavButton( $this->request, __CA_NAV_ICON_DELETE__, _t( "Delete" ), '',
							'manage/site_pages', 'SitePageEditor', 'Delete', array( 'page_id' => $va_page['page_id'] ),
							array(), array(
								'icon_position'     => __CA_NAV_ICON_ICON_POS_LEFT__,
								'use_class'         => 'list-button',
								'no_background'     => true,
			...
			<tr>
				<td colspan='8'>
					<div align="center">
						<?php print $vs_page_select ? _t( 'No pages have been created' )
							: _t( 'No page templates are been defined' ); ?>
					</div>
				</td>
			</tr>
			<?php
