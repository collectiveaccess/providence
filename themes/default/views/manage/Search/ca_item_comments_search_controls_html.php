...
if ( ! $this->request->isAjax() ) {
	...
	print caFormControlBox(
		...
		. _t( 'Show from %1 by %2 with text %3 and status %4',
			caHTMLTextInput( 'filter_daterange',
				array( 'size' => 10, 'value' => ( $filter_daterange ) ? $filter_daterange : '', 'class' => 'dateBg' ) ),
			caHTMLSelect( 'filter_user', array_merge( [ _t( 'any user' ) => '' ], $user_list ), [],
				[ 'value' => $filter_user_id, 'width' => '100px' ] ),
			caHTMLTextInput( 'search', [ 'value' => $filter_search, 'size' => '20', 'id' => 'BasicSearchInput' ] ),
			caHTMLSelect( 'filter_moderation',
				[ _t( 'any' ) => - 1, _t( 'approved' ) => 1, _t( 'needs moderation' ) => 0 ], [],
				[ 'value' => $filter_moderation, 'width' => '100px' ] )
		) . '</div>',
