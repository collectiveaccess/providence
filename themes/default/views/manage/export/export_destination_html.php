...
<table>
    <tr>
        ...
        <td><?php print caHTMLTextInput(
                'file_name',
                array('id' => 'file_name', 'value' => $vs_filename, 'size' => 40)
            ); ?></td>
    </tr>
    <tr>
        <td style="vertical-align: top;"><span class='formLabelPlain'><?php print _t("Destination(s)"); ?>&colon;</td>
        <td>
            <div><?php print caJSButton(
                    $this->request,
                    ...
                    array(
                        'id' => 'file_download',
                        'onclick' => 'caProcessDestination("file_download");'
                    )
                ); ?></div>
            <?php
...
<script type="text/javascript">
    ...
    function caProcessDestination(dest_code) {
        ...
        if (dest_code == 'file_download') { // for file download, really redirect to action
            window.location.href = "<?php print caNavUrl(
                $this->request,
                'manage',
                'MetadataExport',
                'ProcessDestination'
            ); ?>?file_name=" + encodeURIComponent(file_name) + "&destination=file_download";
            ...
            jQuery("#caExporterDestinationFeedback").load('<?php print caNavUrl(
                $this->request,
                'manage',
                'MetadataExport',
                'ProcessDestination'
            ); ?>', {file_name: file_name, destination: dest_code});
        }
    }
</script>
