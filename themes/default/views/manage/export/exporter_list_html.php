...
if ( ! $this->request->isAjax() ) {
	...
	<div class="sectionBox">
		...
		print caFormControlBox(
			...
			. ': <input type="text" name="filter" value="" onkeyup="jQuery(\'#caExporterList\').caFilterTable(this.value); return false;" size="20"/></div>',
			'',
			caFormJSButton( $this->request, __CA_NAV_ICON_ADD__, _t( "Add exporters" ), 'caAddExportersButton',
				array( 'onclick' => 'jQuery("#exporterUploadArea").slideToggle(150); return false;' ) )
		);
		?>
		...
		<div id="exporterUploadArea">
			<span
				class="exporterUploadText"><?php print _t( "Drag exporter worksheets here to add or update" ); ?></span>
		</div>
		<?php
		}
		?>
			...
			<table id="caExporterList" class="listtable">
				...
				foreach ( $va_exporter_list as $va_exporter ) {
					...
					<tr>
						...
						<td>
							<?php print caGetLocalizedDate( $va_exporter['last_modified_on'],
								array( 'dateFormat' => 'delimited' ) ); ?>
						</td>
						<td>
							<?php print caNavButton( $this->request, __CA_NAV_ICON_DELETE__, _t( "Delete" ), '',
								'manage', 'MetadataExport', 'Delete',
								array( 'exporter_id' => $va_exporter['exporter_id'] ), array(), array(
		...
		jQuery(document).ready(function () {
			...
			jQuery('#exporterUploadArea').fileupload({
				...
				singleFileUploads: false,
					...
					jQuery("#caExporterListContainer").load("<?php print caNavUrl( $this->request, 'manage',
						'MetadataExport', 'Index' ); ?>");
				},
