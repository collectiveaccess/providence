...
$vs_new_menu = '<div class="sf-small-menu form-header-button rounded">' .
               ...
               '<a href="#" onclick="_navigateToNewForm(jQuery(\'#tableList\').val());">'
               . caNavIcon( __CA_NAV_ICON_ADD__, 2 ) . '</a>' .
               '</div>' .
               '<form action="#">' . _t( 'New metadata alert rule for ' ) . ' ' . caHTMLSelect( 'table_num',
...
<script language="JavaScript" type="text/javascript">
	...
	function _navigateToNewForm(table_num) {
		document.location = '<?php print caNavUrl( $this->request, 'manage/metadata_alert_rules',
			'MetadataAlertRuleEditor', 'Edit', array( 'rule_id' => 0 ) ); ?>' + '/table_num/' + table_num;
	}
...
<div class="sectionBox">
	...
	print caFormControlBox(
		...
		. ': <input type="text" name="filter" value="" onkeyup="$(\'#caMetadataAlertList\').caFilterTable(this.value); return false;" size="20"/></div>',
		'',
		$vs_new_menu
	);
	?>
	...
	<table id="caMetadataAlertList" class="listtable">
		...
		if ( sizeof( $va_list ) ) {
			foreach ( $va_list as $va_rule ) {
				...
				<tr>
					<td>
						<div class="caMetadataAlertListName"><?php print $va_rule['name'] . ( $va_rule['code']
									? "<br/>(" . $va_rule['code'] . ")" : "" ); ?></div>
					</td>
					<td>
					...
					<td>
						<div class="caMetadataAlertListOwner"><?php print $va_rule['fname'] . ' ' . $va_rule['lname']
						                                                  . ( $va_rule['email']
									? "<br/>(<a href='mailto:" . $va_rule['email'] . "'>" . $va_rule['email'] . "</a>)"
									: "" ); ?></div>
					</td>
					<td class="listtableEditDelete">
						<?php print caNavButton( $this->request, __CA_NAV_ICON_EDIT__, _t( "Edit" ), '',
							'manage/metadata_alert_rules', 'MetadataAlertRuleEditor', 'Edit',
							array( 'rule_id' => $va_rule['rule_id'] ), array(), array(
						...
						<?php print caNavButton( $this->request, __CA_NAV_ICON_DELETE__, _t( "Delete" ), '',
							'manage/metadata_alert_rules', 'MetadataAlertRuleEditor', 'Delete',
							array( 'rule_id' => $va_rule['rule_id'] ), array(), array(
								'icon_position'     => __CA_NAV_ICON_ICON_POS_LEFT__,
								'use_class'         => 'list-button',
								'no_background'     => true,
