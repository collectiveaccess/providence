...
$vs_new_menu = '<div class="sf-small-menu form-header-button rounded">' .
    ...
    '<a href="#" onclick="_navigateToNewForm(jQuery(\'#tableList\').val());">' . caNavIcon(
        __CA_NAV_ICON_ADD__,
        2
    ) . '</a>' .
    '</div>' .
    '<form action="#">' . _t('New metadata alert rule for ') . ' ' . caHTMLSelect(
        'table_num',
        caGetPrimaryTablesForHTMLSelect(true),
        array('id' => 'tableList')
    ) . '</form>' .
    '</div>';
?>
        ...
        if (sizeof($va_list)) {
            foreach ($va_list as $va_rule) {
                ...
                <tr>
                    <td>
                        <div
                            class="caMetadataAlertListName"><?php print $va_rule['name'] . ($va_rule['code'] ? "<br/>(" . $va_rule['code'] . ")" : ""); ?></div>
                    </td>
                    <td>
                    ...
                    <td>
                        <div
                            class="caMetadataAlertListOwner"><?php print $va_rule['fname'] . ' ' . $va_rule['lname'] . ($va_rule['email'] ? "<br/>(<a href='mailto:" . $va_rule['email'] . "'>" . $va_rule['email'] . "</a>)" : ""); ?></div>
                    </td>
