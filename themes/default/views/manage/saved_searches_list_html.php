...
if ( sizeof( $va_saved_searches ) > 0 ) {
	...
	<div class="sectionBox">
		...
		print caFormControlBox(
			...
			'',
			"<a href='#' onclick='jQuery(\"#SavedSearchesListForm\").attr(\"action\", \"" . caNavUrl( $this->request,
				'manage', 'SavedSearches', 'Delete' ) . "\").submit();' class='form-button'><span class='delete'>"
		...
		<form id="SavedSearchesListForm">
			...
			<table id="caItemList" class="listtable">
				...
				<tr>
					...
					<th class="{sorter: false} list-header-nosort listtableEdit"><input type='checkbox' name='record'
					                                                                    value=''
					                                                                    id='savedSearchesSelectAllControl'
					                                                                    class=''
					                                                                    onchange="jQuery('.savedSearchesControl').attr('checked', jQuery('#savedSearchesSelectAllControl').attr('checked'));"/>
					</th>
				</tr>
				</thead>
				<tbody>
						...
						if ( sizeof( $va_search_info ) > 0 ) {
							foreach ( array_reverse( $va_search_info ) as $vs_key => $va_search ) {
								...
								<tr>
									...
									<td>
										<?php print caNavLink( $this->request, $vs_search, "", "find",
											$vs_controller . ( ( $vs_search_type == "advanced_search" ) ? "Advanced"
												: "" ), 'doSavedSearch', array( "saved_search_key" => $vs_key ) ); ?>
									</td>
									<td class="listtableEdit">
										<input type="checkbox" class="savedSearchesControl" name="saved_search_id[]"
										       value="<?php print $vs_table . "-" . $vs_search_type . "-"
										                          . $vs_key; ?>">
									</td>
								</tr>
								<?php
