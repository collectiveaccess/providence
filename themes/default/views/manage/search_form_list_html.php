...
$vs_set_type_menu = '<div class="sf-small-menu form-header-button rounded">' .
                    ...
                    '</div>' .
                    '<form action="#">' . _t( 'New search form for ' ) . ' ' . caHTMLSelect( 'table_num',
		$this->getVar( 'table_list' ), array( 'id' => 'tableList' ) ) . '</form>' .
                    '</div>';
...
<script language="JavaScript" type="text/javascript">
	...
	function _navigateToNewForm(table_num) {
		document.location = '<?php print caNavUrl( $this->request, 'manage/search_forms', 'SearchFormEditor', 'Edit',
			array( 'form_id' => 0 ) ); ?>' + '/table_num/' + table_num;
	}
...
<div class="sectionBox">
	...
	print caFormControlBox(
		...
		. ': <input type="text" name="filter" value="" onkeyup="$(\'#caFormList\').caFilterTable(this.value); return false;" size="20"/></div>',
		'',
		$vs_set_type_menu
	);
	?>
				...
				<tr>
					...
					<td class="listtableEditDelete">
						<?php print caNavButton( $this->request, __CA_NAV_ICON_EDIT__, _t( "Edit" ), '',
							'manage/search_forms', 'SearchFormEditor', 'Edit',
							array( 'form_id' => $va_form['form_id'] ), array(), array(
						...
						<?php print caNavButton( $this->request, __CA_NAV_ICON_DELETE__, _t( "Delete" ), '',
							'manage/search_forms', 'SearchFormEditor', 'Delete',
							array( 'form_id' => $va_form['form_id'] ), array(), array(
								'icon_position'     => __CA_NAV_ICON_ICON_POS_LEFT__,
								'use_class'         => 'list-button',
								'no_background'     => true,
