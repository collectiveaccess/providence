...
<div class="sectionBox">
	...
	if ( $vb_can_edit ) {
		print $vs_control_box = caFormControlBox(
			...
			caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', 'manage/site_pages',
				'SitePageEditor', 'Edit/' . $this->request->getActionExtra(), array( 'page_id' => $vn_page_id ) ),
			'',
			( ( intval( $vn_page_id ) > 0 ) && ( $vb_can_delete ) ) ? caFormNavButton( $this->request,
				__CA_NAV_ICON_DELETE__, _t( "Delete" ), 'deleteButton form-button', 'manage/site_pages',
				'SitePageEditor', 'Delete/' . $this->request->getActionExtra(), array( 'page_id' => $vn_page_id ) ) : ''
		);
	}

	print caFormTag( $this->request, 'Save/' . $this->request->getActionExtra() . '/page_id/' . $vn_page_id,
		'SitePageEditorForm', null, 'POST', 'multipart/form-data' );
