...
if ( $this->getVar( 'num_pages' ) > 1 ) {
	...
	if ( $this->getVar( 'page' ) > 1 ) {
		$vs_searchNav .= "<a href='#' onclick='jQuery(\"#resultBox\").load(\"" . caNavUrl( $this->request, 'manage',
				'Comments', 'Index', $va_previous_link_params ) . "\"); return false;' class='button'>&lsaquo; "
		                 . _t( "Previous" ) . "</a>";
	}
	...
	if ( $this->getVar( 'page' ) < $this->getVar( 'num_pages' ) ) {
		$vs_searchNav .= "<a href='#' onclick='jQuery(\"#resultBox\").load(\"" . caNavUrl( $this->request, 'manage',
				'Comments', 'Index', $va_next_link_params ) . "\"); return false;' class='button'>" . _t( "Next" )
		                 . " &rsaquo;</a>";
	}
	$vs_searchNav .= "</div>";
	$vs_searchNav .= '<form action="#">' . _t( "Jump to page" )
	                 ...
	                 . caNavUrl( $this->request, 'manage', 'Comments', 'Index', $va_jump_to_params )
	                 . '/page/" + jQuery("#jumpToPageNum").val());\' class="button">' . _t( "GO" ) . '</a></form>';
}
$vs_searchNav .= _t( 'Your %1 found', _t( $this->getVar( 'mode' ) ) ) . ' ' . $vo_result->numHits() . ' '
