...
<div class="sectionBox">
	...
	print $vs_control_box = caFormControlBox(
		...
		caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', 'manage/bundle_displays',
			'BundleDisplayEditor', 'Edit/' . $this->request->getActionExtra(),
			array( 'display_id' => $vn_display_id ) ),
		'',
		( intval( $vn_display_id ) > 0 ) ? "<span class='deleteButton'>" . caFormNavButton( $this->request,
				__CA_NAV_ICON_DELETE__, _t( "Delete" ), '', 'manage/bundle_displays', 'BundleDisplayEditor',
				'Delete/' . $this->request->getActionExtra(), array( 'display_id' => $vn_display_id ) ) . '<span>' : ''
	);

	print caFormTag( $this->request, 'Save/' . $this->request->getActionExtra() . '/display_id/' . $vn_display_id,
		'BundleDisplayEditorForm', null, 'POST', 'multipart/form-data' );
