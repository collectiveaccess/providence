...
<?php
print caFormTag( $this->request, 'Run', $vs_form_id, null, 'POST', 'multipart/form-data', '_top',
	array( 'noCSRFToken' => true, 'disableUnsavedChangesWarning' => true, 'noTimestamp' => true ) );

print $vs_control_box = caFormControlBox(
	caFormJSButton( $this->request, __CA_NAV_ICON_SAVE__, _t( "Run" ), "caRunTool{$vs_tool_identifier}",
		array( 'onclick' => 'caShowConfirmToolExecutionPanel(); return false;' ) ) . ' ' .
	caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', 'manage', 'Tools', 'Settings',
		array( 'tool' => $vs_tool_identifier ) ),
	'',
	''
);
	...
	<div class='bundleLabel'>
		...
		<div class="bundleContainer">
			<div class="caLabelList">
				<p>
					<?php
					print $o_tool->settingHTMLFormElement( $vs_setting, array(
						'id'             => "{$vs_form_id}_{$vs_setting}",
						'name'           => "{$vs_form_id}_{$vs_setting}",
...
<script type="text/javascript">
	function caShowConfirmToolExecutionPanel() {
		var msg = '<?php print addslashes( _t( "You are about to run <em>%2</em> in <em>%1</em>",
			$o_tool->getToolName() ) ); ?>';
