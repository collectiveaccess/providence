...
<div id="caConfirmBatchExecutionPanel" class="caConfirmBatchExecutionPanel">
	...
	<div id="caConfirmBatchExecutionPanelContentArea">
				...
				<tr style="vertical-align: top;">
					...
					if ( $vb_queue_enabled ) {
						...
						<td class="caConfirmBatchExecutionPanelAlertControls">
							...
							if ( isset( $va_last_settings['runInBackground'] )
							     && $va_last_settings['runInBackground']
							) {
								$va_opts['checked'] = 1;
							}
					...
					if ( ( $vs_sms = trim( $this->request->user->get( 'sms_number' ) ) )
					     && (bool) $this->request->config->get( 'enable_sms_notifications' )
					) {
						?>
						<td class="caConfirmBatchExecutionPanelAlertControl">
							<?php
		...
		<div id="caConfirmBatchExecutionPanelControlButtons">
			<table>
				<tr>
					<td align="right"><?php print caJSButton( $this->request, __CA_NAV_ICON_SAVE__, _t( 'Run tool' ),
							'caConfirmBatchExecutionFormExecuteButton',
							array( 'onclick' => 'caRunTool(); return false;' ), array() ); ?></td>
					<td align="left"><?php print caJSButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( 'Cancel' ),
							'caConfirmBatchExecutionFormCancelButton',
							array( 'onclick' => 'caConfirmBatchExecutionPanel.hidePanel(); return false;' ),
							array() ); ?></td>
				</tr>
			</table>
		</div>
