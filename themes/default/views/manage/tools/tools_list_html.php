...
if ( ! $this->request->isAjax() ) {
	...
	<div class="sectionBox">
		...
		print caFormControlBox(
			...
			. ': <input type="text" name="filter" value="" onkeyup="jQuery(\'#caToolList\').caFilterTable(this.value); return false;" size="20"/></div>',
			'',
			''
		);
		?>
		...
		<div id="caToolListContainer">
			<table id="caToolList" class="listtable">
				...
				if ( sizeof( $va_tool_list ) > 0 ) {
						...
						<tr>
							...
							<td>
								...
								foreach ( $va_commands as $vs_command ) {
									print "<u>{$vs_command}</u> – <em>" . $o_tool->getShortHelpText( $vs_command )
									      . "</em>";
								}
								?>
							</td>
							<td>
								<?php print caNavButton( $this->request, __CA_NAV_ICON_GO__, _t( "Run" ), '', 'manage',
									'Tools', 'Settings', array( 'tool' => $o_tool->getToolIdentifier() ), array(),
									array(
					...
					?>
					<tr>
						<td colspan="4" align="center"><?php print _t( 'No tools are available' ); ?></td>
					</tr>
					<?php
				}
				?>
		...
		jQuery(document).ready(function () {
			...
			jQuery('#exporterUploadArea').fileupload({
				...
				singleFileUploads: false,
					...
					jQuery("#caToolListContainer").load("<?php print caNavUrl( $this->request, 'manage',
