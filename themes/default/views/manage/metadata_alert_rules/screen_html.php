...
<div class="sectionBox">
	...
	print $vs_control_box = caFormControlBox(
		...
		caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', 'manage/metadata_alert_rules',
			'MetadataAlertRuleEditor', 'Edit/' . $this->request->getActionExtra(), array( 'rule_id' => $vn_rule_id ) ),
		'',
		( intval( $vn_rule_id ) > 0 ) ? caFormNavButton( $this->request, __CA_NAV_ICON_DELETE__, _t( "Delete" ), '',
			'manage/metadata_alert_rules', 'MetadataAlertRuleEditor', 'Delete/' . $this->request->getActionExtra(),
			array( 'rule_id' => $vn_rule_id ) ) : ''
	);

	print caFormTag( $this->request, 'Save/' . $this->request->getActionExtra() . '/rule_id/' . $vn_rule_id,
		'MetadataAlertRuleEditorForm', null, 'POST', 'multipart/form-data' );
