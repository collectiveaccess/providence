...
$vs_set_type_menu = '<div class="sf-small-menu form-header-button rounded" style="padding: 6px;">' .
    ...
    '<a href="#" onclick="_navigateToNewForm(jQuery(\'#typeList\').val(), jQuery(\'#tableList\').val());">' . caNavIcon(
        __CA_NAV_ICON_ADD__,
        2
    ) . '</a>' .
    '</div>' .
...
if (!$vn_type_id) {
    ...
    $vs_set_type_menu .= " <b>" . mb_strtolower(
            $vs_type_name_singular
        ) . "</b><input type='hidden' id='typeList' name='set_type' value='" . $vn_type_id . "'> ";
}
$vs_set_type_menu .= _t('containing') . ' ' . caHTMLSelect(
        'table_num',
        $this->getVar('table_list'),
        array('id' => 'tableList')
    ) . '</form>' . '</div>';
    ...
    <script language="JavaScript" type="text/javascript">
            ...
            jQuery('#setSearch').autocomplete(
                {
                    ...
                    select: function (event, ui) {
                        if (parseInt(ui.item.id) > 0) {
                            ...
                            document.location = '<?php print caNavUrl(
                                $this->request,
                                'manage/sets',
                                'SetEditor',
                                'Edit'
        ...
        function _navigateToNewForm(type_id, table_num) {
            document.location = '<?php print caNavUrl(
                ...
                'Edit',
                array('set_id' => 0)
            ); ?>/type_id/' + type_id + '/table_num/' + table_num;
        }
    </script>
<div class="sectionBox">
    ...
    print caFormControlBox(
        '<div class="simple-search-box">' . _t(
            'Search'
        ) . ': <input type="text" id="setSearch" name="search" value="' . htmlspecialchars(
            $this->getVar('search'),
            ENT_QUOTES,
            'UTF-8'
        ) . '" size="20"/></div>' . $vs_type_id_form_element,
        '',
        $vs_set_type_menu
    );
    ?>
    ...
    <div id="resultBox">
        ...
        <div id="algebraSetControls">
            <?php print _t(
                "Create new set %1 from the %2 of %3 selected sets",
                "<input type='text' size='10' name='algebra_set_name' id='algebraSetName'/>",
                caHTMLSelect(
                    "algebra_set_operation",
                    [
            ...
            <tr>
                ...
                <th class="<?php print (($vs_current_sort == "name") ? "list-header-sorted-" . $vs_current_sort_direction : ""); ?> list-header-nolink">
                    <?php print caNavLink(
                        ...
                        array(
                            'sort' => 'name',
                            'direction' => ((($vs_current_sort == "name") && ($vs_current_sort_direction != "desc")) ? "desc" : "asc")
                        )
                    ); ?>
                </th>
                <th class="<?php print (($vs_current_sort == "set_content_type") ? "list-header-sorted-" . $vs_current_sort_direction : ""); ?> list-header-nolink">
                    <?php print caNavLink(
                        ...
                        array(
                            ...
                            'direction' => ((($vs_current_sort == "set_content_type") && ($vs_current_sort_direction != "desc")) ? "desc" : "asc")
                        )
                    ); ?>
                </th>
                <?php
                if (!$vn_type_id) {
                    ...
                    <th class="<?php print (($vs_current_sort == "set_type") ? "list-header-sorted-" . $vs_current_sort_direction : ""); ?> list-header-nolink">
                        <?php print caNavLink(
                            ...
                            array(
                                'sort' => 'set_type',
                                'direction' => ((($vs_current_sort == "set_type") && ($vs_current_sort_direction != "desc")) ? "desc" : "asc")
                            )
                        ); ?>
                    </th>
                    <?php
                ...
                <th class="<?php print (($vs_current_sort == "item_count") ? "list-header-sorted-" . $vs_current_sort_direction : ""); ?> list-header-nolink">
                    <?php print caNavLink(
                        ...
                        array(
                            'sort' => 'item_count',
                            'direction' => ((($vs_current_sort == "item_count") && ($vs_current_sort_direction != "desc")) ? "desc" : "asc")
                        )
                    ); ?>
                </th>
                <th class="<?php print (($vs_current_sort == "lname") ? "list-header-sorted-" . $vs_current_sort_direction : ""); ?> list-header-nolink">
                    <?php print caNavLink(
                        ...
                        array(
                            'sort' => 'lname',
                            'direction' => ((($vs_current_sort == "lname") && ($vs_current_sort_direction != "desc")) ? "desc" : "asc")
                        )
                    ); ?>
                </th>
                <th class="<?php print (($vs_current_sort == "access") ? "list-header-sorted-" . $vs_current_sort_direction : ""); ?> list-header-nolink">
                    <?php print caNavLink(
                        ...
                        array(
                            'sort' => 'access',
                            'direction' => ((($vs_current_sort == "access") && ($vs_current_sort_direction != "desc")) ? "desc" : "asc")
                        )
                    ); ?>
                </th>
                <th class="<?php print (($vs_current_sort == "status") ? "list-header-sorted-" . $vs_current_sort_direction : ""); ?> list-header-nolink">
                    <?php print caNavLink(
                        ...
                        array(
                            'sort' => 'status',
                            'direction' => ((($vs_current_sort == "status") && ($vs_current_sort_direction != "desc")) ? "desc" : "asc")
                        )
                    ); ?>
                </th>
            ...
            if (sizeof($va_set_list)) {
                foreach ($va_set_list as $va_set) {
                    ...
                    <tr>
                        ...
                        <td>
                            <div
                                class="caItemListName"><?php print $va_set['name'] . ($va_set['set_code'] ? "<br/>(" . $va_set['set_code'] . ")" : ""); ?></div>
                        </td>
                        <td>
                        ...
                        <td align="center">
                            <div>
                                <?php
                                if (($va_set['item_count'] > 0) && ($this->request->user->canDoAction(
                                        'can_batch_edit_' . Datamodel::getTableName($va_set['table_num'])
                                    ))) {
                                    print caNavButton(
                                        $this->request,
                                        __CA_NAV_ICON_BATCH_EDIT__,
                                        _t('Batch edit'),
                                        'batchIcon',
                                        'batch',
                                        'Editor',
                                        'Edit',
                                        array('set_id' => $va_set['set_id']),
                                        array(),
                                        array(
                        ...
                        <td>
                            <div
                                class="caItemListOwner"><?php print $va_set['fname'] . ' ' . $va_set['lname'] . ($va_set['email'] ? "<br/>(<a href='mailto:" . $va_set['email'] . "'>" . $va_set['email'] . "</a>)" : ""); ?></div>
                        </td>
                        <td>
                        ...
                        <td class="listtableEditDelete">
                            ...
                            <?php ($va_set['can_delete'] == true) ? print caNavButton(
                                $this->request,
