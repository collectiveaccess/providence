...
<div class="sectionBox">
	...
	if ( ! $this->getVar( 'confirmed' ) ) {
		...
		print caFormTag( $this->request, 'Delete', 'caDeleteForm', null, 'post', 'multipart/form-data', '_top',
			array( 'disableUnsavedChangesWarning' => true ) );
		print "<div class='delete-control-box'>" . caFormControlBox(
				"<div class='delete_warning_box'>" . _t( 'Really delete ALL records in "%1"?',
					$t_set->getLabelForDisplay() ) . "</div>",
				"",
				caFormSubmitButton( $this->request, __CA_NAV_ICON_DELETE__, _t( "Delete" ), 'caDeleteForm', array() ) .
				caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', "batch", "Editor", "Edit",
		...
		<div id="batchProcessingMore">
			<?php print caNavLink( $this->request, _t( 'Leave batch editor and delete this set' ), '', 'manage/sets',
				'SetEditor', 'Delete', array( 'set_id' => $this->getVar( 'set_id' ) ) ); ?>
		</div>
		...
		<?php
		function caIncrementBatchEditorProgress(
			$po_request, $pn_rows_complete, $pn_total_rows, $ps_message, $pn_elapsed_time, $pn_memory_used,
			...
			print "jQuery('#batchProcessingCounts').html('" . addslashes( _t( "%1 processed; %2 errors",
					$pn_num_processed, $pn_num_errors ) ) . "');";
		...
		function caCreateBatchEditorResultsReport( $po_request, $pa_general, $pa_notices, $pa_errors ) {
			...
			if ( is_array( $pa_errors ) && sizeof( $pa_errors ) ) {
				...
				foreach ( $pa_errors as $vn_id => $va_error ) {
					...
					$vs_buf .= "<li><em>" . caEditorLink( $po_request, $va_error['label'], '', $pa_general['table'],
							$vn_id ) . "</em> (" . $va_error['idno'] . "): " . join( "; ", $va_error_list ) . "</li>";
				}
				$vs_buf .= "</ul>";

				$vs_buf .= '<div class="batchProcessingReportSectionWarning">'
				           . _t( 'Note: The above record(s) were not deleted due to the errors.' ) . "</div>";
			}
			if ( is_array( $pa_notices ) && sizeof( $pa_notices ) ) {
				$vs_buf .= '<div class="batchProcessingReportSectionHead">' . _t( 'Deleted successfully' )
				           . ':</div><ol>';
				foreach ( $pa_notices as $vn_id => $va_notice ) {
					$vs_buf .= "<li><em>" . preg_replace( "![\r\n\t]+!", " ", $va_notice['label'] ) . "</em> ("
					           . $va_notice['idno'] . "): " . $va_notice['status'] . "</li>";
				}
				$vs_buf .= "</ol>";
			}
