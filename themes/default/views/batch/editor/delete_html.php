...
<div class="sectionBox">
    ...
    if (!$this->getVar('confirmed')) {
        ...
        print "<div class='delete-control-box'>" . caFormControlBox(
                "<div class='delete_warning_box'>" . _t(
                    'Really delete ALL records in "%1"?',
        ...
        <div id="batchProcessingMore">
            <?php print caNavLink(
                $this->request,
                _t('Leave batch editor and delete this set'),
                '',
                'manage/sets',
                'SetEditor',
                'Delete',
            ...
            if (is_array($pa_errors) && sizeof($pa_errors)) {
                ...
                foreach ($pa_errors as $vn_id => $va_error) {
                    ...
                    $vs_buf .= "<li><em>" . caEditorLink(
                            ...
                            $pa_general['table'],
                            $vn_id
                        ) . "</em> (" . $va_error['idno'] . "): " . join("; ", $va_error_list) . "</li>";
                }
                $vs_buf .= "</ul>";

                $vs_buf .= '<div class="batchProcessingReportSectionWarning">' . _t(
                        'Note: The above record(s) were not deleted due to the errors.'
                    ) . "</div>";
            }
            if (is_array($pa_notices) && sizeof($pa_notices)) {
                $vs_buf .= '<div class="batchProcessingReportSectionHead">' . _t(
                ...
                foreach ($pa_notices as $vn_id => $va_notice) {
                    $vs_buf .= "<li><em>" . preg_replace(
                            ...
                            " ",
                            $va_notice['label']
                        ) . "</em> (" . $va_notice['idno'] . "): " . $va_notice['status'] . "</li>";
                }
                $vs_buf .= "</ol>";
            }
