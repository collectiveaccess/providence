...
?>
	...
	<div id="batchProcessingMore">
		<?php print caNavLink( $this->request, _t( 'Perform another batch edit' ), '', 'batch', 'Editor',
			'Edit/' . $this->request->getActionExtra(), array( 'set_id' => $this->getVar( 'set_id' ) ) ); ?>
	</div>
...
function caCreateBatchEditorResultsReport( $po_request, $pa_general, $pa_notices, $pa_errors ) {
	...
	if ( is_array( $pa_errors ) && sizeof( $pa_errors ) ) {
		...
		foreach ( $pa_errors as $vn_id => $va_error ) {
			...
			$vs_buf .= "<li><em>" . caEditorLink( $po_request, $va_error['label'], '', $pa_general['table'], $vn_id )
			           . "</em> (" . $va_error['idno'] . "): " . join( "; ", $va_error_list ) . "</li>";
		}
		$vs_buf .= "</ul>";

		$vs_buf .= '<div class="batchProcessingReportSectionWarning">' . ( ( sizeof( $pa_errors ) == 1 )
				? _t( 'Note: <strong>NO</strong> batch changes were saved due to the error.' )
				: _t( 'Note: <strong>NO</strong> batch changes were saved due to %1 errors.', sizeof( $pa_errors ) ) )
		           . "</div>";
	}
	if ( is_array( $pa_notices ) && sizeof( $pa_notices ) ) {
		...
		foreach ( $pa_notices as $vn_id => $va_notice ) {
			$vs_buf .= "<li><em>" . caEditorLink( $po_request, preg_replace( "![\r\n\t]+!", " ", $va_notice['label'] ),
					'', $pa_general['table'], $vn_id ) . "</em> (" . $va_notice['idno'] . "): " . $va_notice['status']
			           . "</li>";
		}
		$vs_buf .= "</ol>";
	}
