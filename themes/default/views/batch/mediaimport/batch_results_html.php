...
?>
	...
	<div id="batchProcessingMediaPreview"
	     style="width: 320px; height: 320px; overflow: hidden; margin: 0 auto 0 auto;"></div>
	<div id="batchProcessingReport"></div>
	<div class="editorBottomPadding"><!-- empty --></div>

	<div id="batchProcessingMore">
		<?php print caNavLink( $this->request, _t( 'Perform another media import' ), '', 'batch', 'MediaImport',
...
<?php
function caIncrementBatchMediaImportProgress(
	$po_request, $pn_rows_complete, $pn_total_rows, $ps_message, $t_new_rep, $pn_elapsed_time, $pn_memory_used,
	$pn_num_processed, $pn_num_errors
) {
	...
	print "jQuery('#batchProcessingElapsedTime').html('" . caFormatInterval( $pn_elapsed_time ) . "/"
	      . sprintf( "%4.2f mb", ( $pn_memory_used / 1048576 ) ) . "');";
	print "jQuery('#batchProcessingCounts').html('" . addslashes( _t( "%1 processed; %2 errors", $pn_num_processed,
			$pn_num_errors ) ) . "');";

	if ( is_object( $t_new_rep ) ) {
		print "jQuery('#batchProcessingMediaPreview').html('" . addslashes( $t_new_rep->getMediaTag( 'media',
				'small' ) ) . "');";
	} else {
...
function caCreateBatchMediaImportResultsReport( $po_request, $pa_general, $pa_notices, $pa_errors ) {
	...
	if ( is_array( $pa_notices ) && sizeof( $pa_notices ) ) {
		...
		foreach ( $pa_notices as $vn_id => $va_notice ) {
			switch ( $va_notice['status'] ) {
				case 'SUCCESS':
					$vs_buf .= "<li><em>" . caEditorLink( $po_request, $va_notice['label'], '', $pa_general['table'],
							$vn_id ) . "</em> (" . $va_notice['idno'] . "): " . _t( $va_notice['status'] ) . "</li>";
					break;
				case 'SKIPPED':
				case 'EXISTS':
				case 'NO_MATCH':
				...
				case 'RELATED':
					...
					break;
				default:
					$vs_buf .= "<li><em>" . $va_notice['label'] . "</em> (" . $va_notice['idno'] . "): "
					           . _t( $va_notice['status'] ) . "</li>";
					break;
			}
		}
		$vs_buf .= "</ol>";
	...
	if ( $pa_general['set_id'] ) {
		$vs_buf .= caNavButton( $po_request, __CA_NAV_ICON_BATCH_EDIT__, _t( 'Batch edit' ), '', 'batch', 'Editor',
				'Edit', array( 'set_id' => $pa_general['set_id'] ), array(), array(
					'icon_position' => __CA_NAV_ICON_ICON_POS_LEFT__,
					'use_class' => 'list-button',
					'no_background' => true,
					'dont_show_content' => true
				) ) . ' ' .
		           _t( 'Batch edit set <em>%1</em> containing imported media',
			           caNavLink( $po_request, $pa_general['setName'], '', 'batch', 'Editor', 'Edit',
				           array( 'set_id' => $pa_general['set_id'] ) ) );
	}

	if ( $pa_general['skiplog'] ) {
		$vs_buf .= caNavLink( $po_request, _t( "Download skipped files log" ), '', '*', '*', 'DownloadLog',
				[ 'file' => pathinfo( $pa_general['skiplog'], PATHINFO_FILENAME ) ] ) . "<br/>";
	}
	if ( $pa_general['errorlog'] ) {
		$vs_buf .= caNavLink( $po_request, _t( "Download error log" ), '', '*', '*', 'DownloadLog',
				[ 'file' => pathinfo( $pa_general['errorlog'], PATHINFO_FILENAME ) ] ) . "<br/>";
	}
	print "<script type='text/javascript'>";
