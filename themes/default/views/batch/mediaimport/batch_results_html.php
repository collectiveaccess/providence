...
<div id="batchProcessingMore">
    <?php print caNavLink(
        $this->request,
        _t('Perform another media import'),
        '',
        'batch',
        'MediaImport',
...
function caIncrementBatchMediaImportProgress(
    ...
    print "jQuery('#batchProcessingElapsedTime').html('" . caFormatInterval($pn_elapsed_time) . "/" . sprintf(
            "%4.2f mb",
            ($pn_memory_used / 1048576)
        ) . "');";
    print "jQuery('#batchProcessingCounts').html('" . addslashes(
            _t("%1 processed; %2 errors", $pn_num_processed, $pn_num_errors)
        ) . "');";

    if (is_object($t_new_rep)) {
        print "jQuery('#batchProcessingMediaPreview').html('" . addslashes(
                $t_new_rep->getMediaTag('media', 'small')
            ) . "');";
    } else {
...
function caCreateBatchMediaImportResultsReport($po_request, $pa_general, $pa_notices, $pa_errors)
{
    ...
    if (is_array($pa_notices) && sizeof($pa_notices)) {
        ...
        foreach ($pa_notices as $vn_id => $va_notice) {
            switch ($va_notice['status']) {
                case 'SUCCESS':
                    $vs_buf .= "<li><em>" . caEditorLink(
                            $po_request,
                            $va_notice['label'],
                            '',
                            $pa_general['table'],
                            $vn_id
                        ) . "</em> (" . $va_notice['idno'] . "): " . _t($va_notice['status']) . "</li>";
                    break;
                case 'SKIPPED':
                case 'MATCHED':
                case 'RELATED':
                    ...
                    break;
                default:
                    $vs_buf .= "<li><em>" . $va_notice['label'] . "</em> (" . $va_notice['idno'] . "): " . _t(
                            $va_notice['status']
                        ) . "</li>";
                    break;
            }
    ...
    if ($pa_general['set_id']) {
        $vs_buf .=
            ...
            _t(
                'Batch edit set <em>%1</em> containing imported media',
                caNavLink(
                    $po_request,
                    $pa_general['setName'],
                    '',
