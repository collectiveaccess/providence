...
print $vs_control_box = caFormControlBox(
	caFormJSButton( $this->request, __CA_NAV_ICON_SAVE__, _t( "Execute media import" ), 'caBatchMediaImportFormButton',
		array( 'onclick' => 'caShowConfirmBatchExecutionPanel(); return false;' ) ) . ' ' .
	caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', 'batch', 'MediaImport',
		'Index/' . $this->request->getActionExtra(), array() ),
	'',
	''
);
...
<div class="sectionBox">
	<?php
	print caFormTag( $this->request, 'Save/' . $this->request->getActionExtra(), 'caBatchMediaImportForm', null, 'POST',
	...
	<div class='bundleLabel'>
		...
		<div class="bundleContainer">
				...
				<script type="text/javascript">
					...
					jQuery(document).ready(function () {
						oDirBrowser = caUI.initDirectoryBrowser('directoryBrowser', {
							levelDataUrl: '<?php print caNavUrl( $this->request, 'batch', 'MediaImport',
								'GetDirectoryLevel' ); ?>',
							initDataUrl: '<?php print caNavUrl( $this->request, 'batch', 'MediaImport',
								'GetDirectoryAncestorList' ); ?>',

							openDirectoryIcon: "<?php print caNavIcon( __CA_NAV_ICON_RIGHT_ARROW__, 1 ); ?>",
							disabledDirectoryIcon: "<?php print caNavIcon( __CA_NAV_ICON_DOT__, 1,
							...
							allowDragAndDropUpload: <?php print is_writable( $this->request->config->get( 'batch_media_import_root_directory' ) )
								? "true" : "false"; ?>,
							dragAndDropUploadUrl: "<?php print caNavUrl( $this->request, 'batch', 'MediaImport',
								'UploadFiles' ); ?>",
			...
			<div style="margin: 8px 0px 0px 0px; padding-bottom:5px;">
				...
				if ( isset( $va_last_settings['includeSubDirectories'] )
				     && $va_last_settings['includeSubDirectories']
				) {
					$va_opts['checked'] = 1;
				}
				print caHTMLCheckboxInput( 'include_subdirectories', $va_opts ) . ' '
				      . _t( 'Include all sub-directories' );
				$va_opts['style'] = 'margin-left: 10px';
				print caHTMLCheckboxInput( 'delete_media_on_import', $va_opts ) . ' '
				      . _t( 'Delete media after import' );
				?>
			</div>
		</div>
	</div>
				...
				<div style='padding:10px 0px 10px 10px;'>
					<table style="width: 100%;">
						<tr>
							<td class='formLabel'>
								<?php
								print _t( 'Type used for newly created %1',
										caGetTableDisplayName( $t_instance->tableName(), false ) ) . "<br/>\n"
							...
							<?php if ( $vs_reltypes = $this->getVar( $t_instance->tableName()
							                                         . '_representation_relationship_type' )
							) { ?>
								<td class='formLabel'>
									<?php
		...
		<div class="bundleContainer">
			<div class="caLabelList" id="caMediaImportSetControls">
				<div style='padding:10px 0px 10px 10px;'>
					<table>
						<?php
						if ( is_array( $this->getVar( 'available_sets' ) )
						     && sizeof( $this->getVar( 'available_sets' ) )
						) {
							?>
							<tr>
								<td><?php
									$va_attrs = array( 'value' => 'add', 'checked' => 1, 'id' => 'caAddToSet' );
									if ( isset( $va_last_settings['setMode'] )
									     && ( $va_last_settings['setMode'] == 'add' )
									) {
										$va_attrs['checked'] = 1;
									}
									print caHTMLRadioButtonInput( 'set_mode', $va_attrs );
									?></td>
								<td class='formLabel'><?php print _t( 'Add imported media to set %1',
										caHTMLSelect( 'set_id', $this->getVar( 'available_sets' ), array(
											'id'    => 'caAddToSetID',
											'class' => 'searchSetsSelect',
											'width' => '300px'
										), array( 'value' => null, 'width' => '170px' ) ) ); ?></td>
							</tr>
							<?php
						}
						?>
						<tr>
							<td><?php
								...
								if ( isset( $va_last_settings['setMode'] )
								     && ( $va_last_settings['setMode'] == 'create' )
								) {
									$va_attrs['checked'] = 1;
								}
								print caHTMLRadioButtonInput( 'set_mode', $va_attrs );
								?></td>
							<td class='formLabel'><?php print _t( 'Create set %1 with imported media',
									caHTMLTextInput( 'set_create_name', array(
										'value' => '',
										'width' => '200px',
										'id'    => 'caSetCreateName'
									) ) ); ?></td>
						...
						<tr>
							<td><?php
								...
								if ( ! ( ( isset( $va_last_settings['setMode'] )
								           && ( in_array( $va_last_settings['setMode'], array( 'add', 'create' ) ) ) ) )
								) {
									$va_attrs['checked'] = 1;
								}
								print caHTMLRadioButtonInput( 'set_mode', $va_attrs );
	...
	<div class='bundleLabel'>
		<span class="formLabelText"><?php print _t( '%1 identifier',
				ucfirst( caGetTableDisplayName( $t_instance->tableName(), false ) ) ); ?></span>
		<div class="bundleContainer">
					...
					<table>
						<tr>
							<td><?php
								...
								if ( isset( $va_last_settings['idnoMode'] )
								     && ( $va_last_settings['idnoMode'] == 'form' )
								) {
									$va_attrs['checked'] = 1;
								}
								print caHTMLRadioButtonInput( 'idno_mode', $va_attrs );
								?></td>
							<td class='formLabel' id='caIdnoFormModeForm'><?php print _t( 'Set %1 identifier to %2',
									caGetTableDisplayName( $t_instance->tableName(), false ),
									$t_instance->htmlFormElement( 'idno', '^ELEMENT',
										array( 'request' => $this->request ) ) ); ?></td>
						</tr>
						<tr>
							<td><?php
								$va_attrs = array( 'value' => 'filename', 'id' => 'caIdnoFilenameMode' );
								if ( isset( $va_last_settings['idnoMode'] )
								     && ( $va_last_settings['idnoMode'] == 'filename' )
								) {
									$va_attrs['checked'] = 1;
								}
								print caHTMLRadioButtonInput( 'idno_mode', $va_attrs );
								?></td>
							<td class='formLabel'><?php print _t( 'Set %1 identifier to file name',
									caGetTableDisplayName( $t_instance->tableName(), false ) ); ?></td>
						</tr>
						<tr>
							<td><?php
								$va_attrs = array( 'value' => 'filename_no_ext', 'id' => 'caIdnoFilenameNoExtMode' );
								if ( isset( $va_last_settings['idnoMode'] )
								     && ( $va_last_settings['idnoMode'] == 'filename_no_ext' )
								) {
									$va_attrs['checked'] = 1;
								}
								print caHTMLRadioButtonInput( 'idno_mode', $va_attrs );
								?></td>
							<td class='formLabel'><?php print _t( 'Set %1 identifier to file name without extension',
									caGetTableDisplayName( $t_instance->tableName(), false ) ); ?></td>
						</tr>
						<tr>
							<td><?php
								$va_attrs = array(
									'value' => 'directory_and_filename',
									'id'    => 'caIdnoDirectoryAndFilenameMode'
								);
								if ( isset( $va_last_settings['idnoMode'] )
								     && ( $va_last_settings['idnoMode'] == 'directory_and_filename' )
								) {
									$va_attrs['checked'] = 1;
								}
								print caHTMLRadioButtonInput( 'idno_mode', $va_attrs );
								?></td>
							<td class='formLabel'><?php print _t( 'Set %1 identifier to directory and file name',
									caGetTableDisplayName( $t_instance->tableName(), false ) ); ?></td>
						</tr>
					</table>
					<script type="text/javascript">
	...
	<div class='bundleLabel'>
		<span class="formLabelText"><?php print ( ( $this->getVar( 'ca_object_representations_mapping_list_count' )
		                                            > 0 )
		                                          || ( $this->getVar( $t_instance->tableName() . '_mapping_list_count' )
		                                               > 0 ) ) ? _t( 'Status, access &amp; metadata extraction' )
				: _t( 'Status &amp; access' ); ?></span>
		<div class="bundleContainer">
			<div class="caLabelList">
				<div style='padding:10px 0px 10px 10px;'>
					<table style="width: 100%;">
						<tr style="vertical-align: top;">
							<td class='formLabel'>
								<?php
								print _t( 'Set %1 status to<br/>%2',
									caGetTableDisplayName( $t_instance->tableName(), false ),
									$t_instance->htmlFormElement( 'status', '',
										array( 'name' => $t_instance->tableName() . '_status' ) ) );
								print "<br/>";
								print _t( 'Set %1 access to<br/>%2',
									caGetTableDisplayName( $t_instance->tableName(), false ),
									$t_instance->htmlFormElement( 'access', '',
										array( 'name' => $t_instance->tableName() . '_access' ) ) );

								if ( $this->getVar( $t_instance->tableName() . '_mapping_list_count' ) > 0 ) {
									print "<br/>";
									print _t( 'Extract embedded metadata into %1 using mapping<br/>%2',
										caGetTableDisplayName( $t_instance->tableName(), false ),
										$this->getVar( $t_instance->tableName() . '_mapping_list' ) );
								}
								?>
							</td>
							<td class='formLabel'>
								<?php
								print _t( 'Set representation status to<br/>%1', $t_rep->htmlFormElement( 'status', '',
									array( 'name' => 'ca_object_representations_status' ) ) );
								print "<br/>";
								print _t( 'Set representation access to<br/>%1', $t_rep->htmlFormElement( 'access', '',
									array( 'name' => 'ca_object_representations_access' ) ) );

								if ( $this->getVar( 'ca_object_representations_mapping_list_count' ) > 0 ) {
									print "<br/>";
									print _t( 'Extract embedded metadata into representation using mapping<br/>%1',
										$this->getVar( 'ca_object_representations_mapping_list' ) );
								}
								?>
							</td>
						</tr>
	...
	<div class='bundleLabel'>
		<span id="caBatchMediaAdvancedHeader" class="formLabelText"><a href="#"
		                                                               id="caBatchMediaAdvancedHeaderText"><?php print _t( "Show advanced options" ); ?> &gt;</a></span>
	</div>
	<script type="text/javascript">
		jQuery(document).ready(function () {
	...
	<div id="caBatchMediaAdvancedContent" style="display: none">
				...
				<div class="caLabelList">
					<table style="width: 100%;">
						<tr>
							...
							<td class='formLabel'>
								<?php
								print _t( "Limit to types" ) . "<br/>\n" . $this->getVar( $t_instance->tableName()
								                                                          . '_limit_to_types_list' );
								?>
							</td>
						</tr>
					</table>
		...
		<div class='bundleLabel'>
			<span class="formLabelText"><?php print _t( '%1 identifier',
					ucfirst( caGetTableDisplayName( 'ca_object_representations', false ) ) ); ?></span>
			<div class="bundleContainer">
						...
						<table>
							<tr>
								<td><?php
									$va_attrs = array(
										'value'   => 'form',
										'checked' => 1,
										'id'      => 'caRepresentationIdnoFormMode'
									);
									if ( isset( $va_last_settings['representationIdnoMode'] )
									     && ( $va_last_settings['representationIdnoMode'] == 'form' )
									) {
										$va_attrs['checked'] = 1;
									}
									...
									?></td>
								<td class='formLabel'
								    id='caRepresentationIdnoFormModeForm'><?php print _t( 'Set %1 identifier to %2',
										caGetTableDisplayName( 'ca_object_representations', false ),
										$t_rep->htmlFormElement( 'idno', '^ELEMENT',
											array( 'request' => $this->request ) ) ); ?></td>
							</tr>
							<tr>
								<td><?php
									$va_attrs = array(
										'value' => 'filename',
										'id'    => 'caRepresentationIdnoFilenameMode'
									);
									if ( isset( $va_last_settings['representationIdnoMode'] )
									     && ( $va_last_settings['representationIdnoMode'] == 'filename' )
									) {
										$va_attrs['checked'] = 1;
									}
									print caHTMLRadioButtonInput( 'representation_idno_mode', $va_attrs );
									?></td>
								<td class='formLabel'><?php print _t( 'Set %1 identifier to file name',
										caGetTableDisplayName( 'ca_object_representations', false ) ); ?></td>
							</tr>
							<tr>
								<td><?php
									$va_attrs = array(
										'value' => 'filename_no_ext',
										'id'    => 'caRepresentationIdnoFilenameNoExtMode'
									);
									if ( isset( $va_last_settings['representationIdnoMode'] )
									     && ( $va_last_settings['representationIdnoMode'] == 'filename_no_ext' )
									) {
										$va_attrs['checked'] = 1;
									}
									print caHTMLRadioButtonInput( 'representation_idno_mode', $va_attrs );
							...
							<tr>
								<td><?php
									$va_attrs = array(
										'value' => 'directory_and_filename',
										'id'    => 'caRepresentationIdnoDirectoryAndFilenameMode'
									);
									if ( isset( $va_last_settings['representationIdnoMode'] )
									     && ( $va_last_settings['representationIdnoMode'] == 'directory_and_filename' )
									) {
										$va_attrs['checked'] = 1;
									}
									print caHTMLRadioButtonInput( 'representation_idno_mode', $va_attrs );
				...
				<div class="caLabelList">
					...
					<div style='padding:10px 0px 10px 10px;'>
						<table>
							<?php
							foreach (
								array( 'ca_entities', 'ca_places', 'ca_occurrences', 'ca_collections' ) as $vs_rel_table
							) {
								...
								$t_rel = ca_relationship_types::getRelationshipTypeInstance( $t_instance->tableName(),
									$vs_rel_table );
								if ( ! $t_rel ) {
									continue;
								}
								...
								<tr>
									<td class='formLabel'>
										<?php
										$checked             = ( is_array( $va_last_settings['create_relationship_for'] )
										                         && in_array( $vs_rel_table,
												$va_last_settings['create_relationship_for'] ) ) ? true : false;
										$default_rel_type_id = isset( $va_last_settings[ 'relationship_type_id_for_'
										                                                 . $vs_rel_table ] )
											? $va_last_settings[ 'relationship_type_id_for_' . $vs_rel_table ] : null;
										print caHTMLCheckboxInput( 'create_relationship_for[]', array(
											'value'   => $vs_rel_table,
											'id'      => "caCreateRelationshipForMedia{$vs_rel_table}",
											'checked' => $checked,
											'onclick' => "jQuery('#caRelationshipTypeIdFor{$vs_rel_table}').prop('disabled', !jQuery('#caCreateRelationshipForMedia{$vs_rel_table}').prop('checked'))"
										), [ 'dontConvertAttributeQuotesToEntities' => true ] );
										print ' ' . _t( "to %1 with relationship type",
												$t_rel_table->getProperty( 'NAME_SINGULAR' ) );
										?>
									</td>
									<td class='formLabel'>
										<?php
										$opts = [
											'name' => "relationship_type_id_for_{$vs_rel_table}",
											'id'   => "caRelationshipTypeIdFor{$vs_rel_table}"
										];
										if ( ! $checked ) {
											$opts['disabled'] = true;
										}
										print $t_rel->getRelationshipTypesAsHTMLSelect( 'ltor', null, null, $opts,
											[ 'value' => $default_rel_type_id ] );
										?>
									</td>
								</tr>
								<?php
		...
		<div class='bundleLabel'>
			...
			<div class="bundleContainer">
				<div class="caLabelList">
					...
					<p>
						<?php
						print caHTMLTextInput( 'skip_file_list',
							array( 'value' => $va_last_settings['skipFileList'], 'id' => "caSkipFilesList" ),
							array( 'width' => '700px', 'height' => '100px' ) );
						?>
					</p>
				</div>
			</div>
		...
		<div class='bundleLabel'>
			...
			<div class="bundleContainer">
				<div class="caLabelList">
					<p class='formLabel'>
						<?php
						print caHTMLCheckboxInput( 'allow_duplicate_media', array(
							'value'   => 1,
							'id'      => 'caAllowDuplicateMedia',
							'checked' => $va_last_settings['allowDuplicateMedia']
						), array() );
					...
					<p class='formLabel'>
						...
						print caHTMLSelect( 'log_level', caGetLogLevels(), array( 'id' => 'caLogLevel' ),
							array( 'value' => $va_last_settings['logLevel'] ) );
						?>
					</p>
				</div>
			</div>
