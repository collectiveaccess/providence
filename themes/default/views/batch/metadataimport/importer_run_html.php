...
print $vs_control_box = caFormControlBox(
	caFormJSButton( $this->request, __CA_NAV_ICON_SAVE__, _t( "Execute data import" ),
		'caBatchMetadataImportFormButton', array( 'onclick' => 'caShowConfirmBatchExecutionPanel(); return false;' ) )
	. ' ' .
	caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', 'batch', 'MetadataImport', 'Index',
		array() ),
	'',
	''
);
...
<div class="sectionBox">
	<?php
	print caFormTag( $this->request, 'ImportData/' . $this->request->getActionExtra(), 'caBatchMetadataImportForm',
		null, 'POST', 'multipart/form-data', '_top',
	...
	<div class='bundleLabel'>
		...
		<div class="bundleContainer">
			<div class="caLabelList">
				<p>
					<?php
					print ca_data_importers::getImporterListAsHTMLFormElement( 'importer_id', null,
						array( 'id' => 'caImporterList', 'onchange' => 'caSetBatchMetadataImportFormState(true);' ),
						array( 'value' => $t_importer->getPrimaryKey() ) );
					?>
				</p>
			</div>
		</div>
	...
	<div class='bundleLabel'>
		...
		<div class="bundleContainer">
			<div class="caLabelList">
				<p>
					<?php
					print ca_data_importers::getInputFormatListAsHTMLFormElement( 'inputFormat', array(
						'id'       => 'caInputFormatList',
						'onchange' => 'caSetBatchMetadataImportFormState(true);'
					) );

					print "<span id='caImportAllDatasetsContainer' class='formLabelPlain'>"
					      . caHTMLCheckboxInput( 'importAllDatasets',
							array( 'id' => 'caImportAllDatasets', 'value' => 1 ), array() ) . ' '
					      . _t( 'Import all data sets' ) . "</span>\n";
					?>
				</p>
			</div>
		</div>
					...
					<table class="caFileSourceControls">
						<tr class="caFileSourceControls">
							<td class="caSourceFileControlRadio">
								...
								$va_attr = array(
									'value'   => 'file',
									'onclick' => 'caSetBatchMetadataImportFormState();',
									'id'      => 'caFileInputRadio'
								);
								if ( caGetOption( 'fileInput', $va_last_settings, 'file' ) === 'file' ) {
									$va_attr['checked'] = 'checked';
								}
								print caHTMLRadioButtonInput( "fileInput", $va_attr )
								      . "</td><td class='formLabel caFileSourceControls'>" . _t( 'From a file' )
								      . " <span id='caFileInputContainer'><input type='file' name='sourceFile' id='caSourceFile'/></span>";
						...
						<tr class="caFileSourceControls">
							<td class="caSourceFileControlRadio">
								...
								$va_attr = array(
									'value'   => 'import',
									'onclick' => 'caSetBatchMetadataImportFormState();',
									'id'      => 'caFileBrowserRadio'
								);
								if ( caGetOption( 'fileInput', $va_last_settings, 'file' ) === 'import' ) {
									$va_attr['checked'] = 'checked';
								}
								print caHTMLRadioButtonInput( "fileInput", $va_attr )
								      . "</td><td class='formLabel caFileSourceControls'>"
								      . _t( 'From the import directory' ) . " <div id='caFileBrowserContainer'>"
								      . $this->getVar( 'file_browser' ) . "</div>";
								?>
							</td>
						</tr>
						<tr class="caFileSourceControls" id='caFileGoogleDriveContainer'>
							<td class="caSourceFileControlRadio">
								...
								$va_attr = array(
									'value'   => 'googledrive',
									'onclick' => 'caSetBatchMetadataImportFormState();',
									'id'      => 'caFileGoogleDriveRadio'
								);
								if ( caGetOption( 'fileInput', $va_last_settings, 'file' ) === 'googledrive' ) {
									$va_attr['checked'] = 'checked';
								}
								print caHTMLRadioButtonInput( "fileInput", $va_attr )
								      . "</td><td class='formLabel caFileSourceControls'>" . _t( 'From GoogleDrive' )
								      . " <span id='caFileGoogleDriveInputContainer'>"
								      . caHTMLTextInput( 'google_drive_url', [
										'value' => caGetOption( 'googleDriveUrl', $va_last_settings, '' ),
										'class' => 'urlBg',
										'id'    => 'caFileGoogleDriveInput'
									], [ 'width' => '500px' ] ) . "</span>";
								?>
							</td>
						</tr>
	...
	<div class='bundleLabel' id="caSourceUrlContainer">
		...
		<div class="bundleContainer">
			<div class="caLabelList">
				<p>
					<?php
					print caHTMLTextInput( 'sourceUrl', array( 'id' => 'caSourceUrl', 'class' => 'urlBg' ),
						array( 'width' => '300px' ) );
					?>
				</p>
			</div>
	...
	<div class='bundleLabel' id="caSourceTextContainer">
		...
		<div class="bundleContainer">
			<div class="caLabelList">
				<p>
					<?php
					print caHTMLTextInput( 'sourceText', array( 'id' => 'caSourceText' ),
						array( 'width' => '600px', 'height' => 3 ) );
					?>
				</p>
			</div>
		</div>
	...
	<div class='bundleLabel'>
		...
		<div class="bundleContainer">
			<div class="caLabelList">
				<p>
					<?php
					print caHTMLSelect( 'logLevel', caGetLogLevels(), array( 'id' => 'caLogLevel' ),
		...
		<div class="bundleContainer">
			<div class="caLabelList">
				<table style="width: 600px; margin-left: 10px;">
					...
					$c                     = 0;
					$acc                   = [];
					$limit_log_to_selected = caGetOption( 'limitLogTo', $va_last_settings, [],
						[ 'castTo' => 'array' ] );
					foreach (
						[
							'GENERAL'                => _t( 'General information' ),
							'EXISTING_RECORD_POLICY' => _t( 'Existing record policy messages' ),
							'SKIP'                   => _t( 'Skip message' ),
							'RELATIONSHIPS'          => _t( 'Relationship creation messages' )
						] as $level => $name
					) {
						$attr = [ 'value' => $level ];
						...
						$acc[] = "<td class='formLabelPlain' style='padding: 5px'>"
						         . caHTMLCheckboxInput( 'limitLogTo[]', $attr, [] ) . " {$name}</td>";
						$c ++;
						if ( ( $c % 2 ) == 0 ) {
...
<script type="text/javascript">
	...
	var caImporterInfo = <?php print json_encode( ca_data_importers::getImporters( null,
		[ 'dontIncludeWorksheet' => true ] ) ); ?>;
	...
	function caSetBatchMetadataImportFormState(dontAnimate) {
		...
		if (!currentFormat) {
			currentFormat = relevantFormats[0];
			jQuery("#caInputFormatList").val(currentFormat);
		}
