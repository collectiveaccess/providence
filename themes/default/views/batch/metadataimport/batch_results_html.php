...
?>
	...
	<h2><?php print _t( 'Importing <em>%1</em> using <em>%2</em>', $t_subject->getProperty( 'NAME_PLURAL' ),
			$t_importer->get( 'ca_data_importers.preferred_labels.name' ) . ' (' . $t_importer->get( 'importer_code' )
			. ')' ); ?></h2>
	...
	<div id="batchProcessingMore">
		<?php print caNavLink( $this->request, _t( 'Perform another metadata import' ), '', 'batch', 'MetadataImport',
...
<?php
function caIncrementBatchMetadataImportProgress(
	$po_request, $pn_file_number, $pn_number_of_files, $ps_file_path, $pn_rows_complete, $pn_total_rows, $ps_message,
	$pn_elapsed_time, $pn_memory_used, $pn_num_processed, $pn_num_errors
) {
	if ( $pn_total_rows == 0 ) {
		return;
	}
	...
	$ps_file_message = _t( 'File %1/%2 [%3]', $pn_file_number, $pn_number_of_files,
		pathinfo( $ps_file_path, PATHINFO_FILENAME ) );
			...
			switch ( $va_notice['status'] ) {
				case 'SUCCESS':
					$vs_buf .= "<li><em>" . caEditorLink( $po_request, $va_notice['label'], '', $pa_general['table'],
							$vn_id ) . "</em> (" . $va_notice['idno'] . "): " . $va_notice['status'] . "</li>";
					break;
				case 'SKIPPED':
				case 'MATCHED':
