...
<div class="sectionBox">
	...
	if ( $vb_can_edit ) {
		print $vs_control_box = caFormControlBox(
			...
			caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '',
				...
				'Access/' . $this->request->getActionExtra(),
				array( $t_instance->primaryKey() => $t_instance->getPrimaryKey() ) ),
			'',
			''
		);
	}
	...
	<div class='globalAccess'>
		...
		<div id='editGlobalAccessLink' class='editLink'><a href='#'
		                                                   onclick='jQuery("#editGlobalAccess").show(250); jQuery("#editGlobalAccessLink").hide()'><?php print caNavIcon( __CA_NAV_ICON_EDIT__,
					2 ); ?><?php print _t( 'Edit Global Access' ); ?></a></div>
	...
	<div class='globalAccess'>
		...
		if ( ( $t_instance->getACLUserGroups() ) || ( $t_instance->getACLUsers() ) ) {
			print "<p>" . _t( 'The following groups and users have special access or restrictions for this record' )
			      . ".</p>";
		} else {
		...
		foreach ( $va_group_access as $va_group_access_item ) {
			print "<div><span class='accessName'>" . ucwords( $va_group_access_item['name'] )
			      . "</span> <b>(group)</b>: " . $va_group_access_item['access_display'] . "</div>";
		}
		...
		foreach ( $va_user_access as $va_user_access_item ) {
			print "<div><span class='accessName'>" . $va_user_access_item['lname'] . ", "
			      . $va_user_access_item['fname'] . "</span>: " . $va_user_access_item['access_display'] . "</div>";
		}
		?>
		...
		if ( ( $t_instance->getACLUserGroups() ) || ( $t_instance->getACLUsers() ) ) {
			?>
			<div id='editUserAccessLink' class='editLink'><a href='#'
			                                                 onclick='jQuery("#editUserAccess").show(250); jQuery("#editUserAccessLink").hide()'><?php print caNavIcon( __CA_NAV_ICON_EDIT__,
						2 ); ?><?php print _t( 'Edit Exceptions' ); ?></a></div>
			<?php
		} else {
			?>
			<div id='editUserAccessLink' class='editLink'><a href='#'
			                                                 onclick='jQuery("#editUserAccess").show(250); jQuery("#editUserAccessLink").hide()'><?php print caNavIcon( __CA_NAV_ICON_EDIT__,
						2 ); ?><?php print _t( 'Create an Exception' ); ?></a></div>
			<?php
		}
		?>
		<div style='width:100%; clear:both; height: 1px;'></div>
	...
	if ( $t_instance->hasField( 'acl_inherit_from_ca_collections' ) ) {
		print $t_instance->getBundleFormHTML( 'acl_inherit_from_ca_collections', '',
			array( 'forACLAccessScreen' => true ), array( 'request' => $this->request ) );
	}
	if ( $t_instance->hasField( 'acl_inherit_from_parent' ) ) {
		print $t_instance->getBundleFormHTML( 'acl_inherit_from_parent', '', array( 'forACLAccessScreen' => true ),
