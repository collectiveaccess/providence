...
<?php	
			$vs_buf = "";
				...
				if ($vs_type_list = $this->getVar('type_list')) {
					...
					$vs_buf .= '<div class="saveButton">'.caFormSubmitLink($this->request, caNavIcon(__CA_NAV_ICON_SAVE__, 1), '', 'NewChildForm')." "._t('Save')."</div>";					
					$vs_buf .= caJSButton($this->request, __CA_NAV_ICON_CANCEL__, _t('Cancel'), 'caChangeTypeFormCancelButton', array('onclick' => 'caCreateChildPanel.hidePanel(); return false;'), array());
					$vs_buf .= "</div>";
					$vs_buf .= "</form></div>\n";
				}
				...
				if (($t_item->tableName() == 'ca_collections') && $this->request->config->get('ca_objects_x_collections_hierarchy_enabled')) {
					...
					$vs_buf .= '<div id="caTypeChangePanelControlButtons">';
					$vs_buf .= '<div class="saveButton">'.caFormSubmitLink($this->request, caNavIcon(__CA_NAV_ICON_SAVE__, 1), '', 'NewChildObjectForm')." "._t('Save')."</div>";					
					$vs_buf .= caJSButton($this->request, __CA_NAV_ICON_CANCEL__, _t('Cancel'), 'caChangeTypeFormCancelButton', array('onclick' => 'caCreateChildPanel.hidePanel(); return false;'), array());
					$vs_buf .= "</div>";
					$vs_buf .= "</form></div>\n";
