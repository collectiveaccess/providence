...
$vb_read_only     = ( ( isset( $va_settings['readonly'] ) && $va_settings['readonly'] )
                      || ( $this->request->user->getBundleAccessLevel( $t_instance->tableName(), 'ca_list_items' )
...
<?php
print caGetRelationDisplayString( $this->request, 'ca_list_items',
...
<?php
if ( $vs_checklist = ca_lists::getListAsHTMLFormElement( null, $vs_id_prefix . "_id{n}", null, array(
	'render'                                               => 'checklist',
	'limitToItemsRelatedToCollections'                     => $t_instance->get( 'ca_collections.collection_id',
		array( 'returnAsArray' => true ) ),
	'limitToItemsRelatedToCollectionWithRelationshipTypes' => $va_settings['restrictToTermsOnCollectionWithRelationshipType'],
	'limitToListIDs'                                       => $va_settings['restrict_to_lists'],
	'maxColumns'                                           => 3
) )
) {
	print $vs_checklist;
...
if ( isset( $va_settings['restrictToTermsOnCollectionUseRelationshipType'] )
     && is_array( $va_settings['restrictToTermsOnCollectionUseRelationshipType'] )
) {
	?>
	<input type="hidden" name="<?php print $vs_id_prefix; ?>_type_id{n}" id="<?php print $vs_id_prefix; ?>_type_id{n}"
	       value="<?php print array_pop( $va_settings['restrictToTermsOnCollectionUseRelationshipType'] ); ?>"/>
	<?php
}
?>
						</td>
...
if ( ! (bool) $va_settings['useHierarchicalBrowser'] ) {
	...
	<table class="caListItem">
					<tr>
						<td><input type="text" size="60" name="<?php print $vs_id_prefix; ?>_autocomplete{n}"
						           value="{{label}}" id="<?php print $vs_id_prefix; ?>_autocomplete{n}"
						           class="lookupBg"/></td>
						<td>
...
?>
	...
	<div style='width: 700px; height: <?php print $va_settings['hierarchicalBrowserHeight']; ?>;'>
					...
					<div id='<?php print $vs_id_prefix; ?>_hierarchyBrowser{n}' style='width: 100%; height: 140px;'
	...
	<script type='text/javascript'>
					jQuery(document).ready(function () {
						...
						= caUI.initHierBrowser('<?php print $vs_id_prefix; ?>_hierarchyBrowser{n}', {
							uiStyle: 'horizontal',
							levelDataUrl: '<?php print caNavUrl( $this->request, 'lookup', 'ListItem',
								'GetHierarchyLevel', array(
									'noSymbols' => 1,
									'voc'       => 1,
									'lists'     => is_array( $va_settings['restrict_to_lists'] ) ? join( ';',
										$va_settings['restrict_to_lists'] ) : ""
								) ); ?>',
							initDataUrl: '<?php print caNavUrl( $this->request, 'lookup', 'ListItem',
						...
						jQuery('#<?php print $vs_id_prefix; ?>_hierarchyBrowserSearch{n}').autocomplete(
							{
								source: '<?php print caNavUrl( $this->request, 'lookup', 'ListItem', 'Get', array(
									'noInline' => 1,
									'noSymbols' => 1,
									'lists' => is_array( $va_settings['restrict_to_lists'] ) ? join( ';',
										$va_settings['restrict_to_lists'] ) : ""
...
<div id="caRelationEditorPanel<?php print $vs_id_prefix; ?>" class="caRelationQuickAddPanel">
	...
	<textarea class='caBundleDisplayTemplate' style='display: none;'>
		<?php print caGetRelationDisplayString( $this->request, 'ca_list_items', array(),
...
<script type="text/javascript">
	...
	jQuery(document).ready(function () {
		...
		caRelationBundle<?php print $vs_id_prefix; ?> = caUI.initRelationBundle('#<?php print $vs_id_prefix; ?>', {
			...
			autocompleteUrl: '<?php print caNavUrl( $this->request, 'lookup', 'Vocabulary', 'Get',
				$va_lookup_params ); ?>',
