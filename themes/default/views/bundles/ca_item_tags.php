...
if ( $vb_batch ) {
	...
	print caEditorBundleShowHideControl( $this->request, $vs_id_prefix, $va_settings,
		caInitialValuesArrayHasValue( $vs_id_prefix . $t_item->tableNum() . '_rel',
...
<?php
print caGetRelationDisplayString( $this->request, 'ca_item_tags',
	array( 'class' => 'caEditItemButton', 'id' => "{$vs_id_prefix}_edit_related_{n}" ),
	array( 'display' => 'tag', 'makeLink' => false, 'relationshipTypeDisplayPosition' => 'none' ) );
?>
				{{moderation_message}}
			</span>
			<input type="hidden" name="<?php print $vs_id_prefix; ?>_id{n}"
			       id="<?php print $vs_id_prefix; ?>_relation_id{n}" value="{relation_id}"/>
		</div>
		<?php
		break;
	case 'bubbles':
				...
				<tr>
					<td>
						<input type="text" size="60" name="<?php print $vs_id_prefix; ?>_autocomplete{n}"
						       value="{{tag}}" id="<?php print $vs_id_prefix; ?>_autocomplete{n}" class="lookupBg"/>
					</td>
					<td>
						<input type="hidden" name="<?php print $vs_id_prefix; ?>_id{n}"
					...
					<td>
						<a href="#" class="caDeleteItemButton"><?php print caNavIcon( __CA_NAV_ICON_DEL_BUNDLE__,
								1 ); ?></a>
						<a href="<?php print urldecode( caEditorUrl( $this->request, 'ca_item_tags', '{id}' ) ); ?>"
	...
	<div class="bundleContainer">
		...
		if ( ! $vb_read_only ) {
			...
			<div class='button labelInfo caAddItemButton'><a href='#'><?php print caNavIcon( __CA_NAV_ICON_ADD__,
						'15px' ); ?><?php print $vs_add_label ? $vs_add_label : _t( "Add tag" ); ?></a></div>
			<?php
		}
		?>
	</div>
	...
	jQuery(document).ready(function () {
		...
		caRelationBundle<?php print $vs_id_prefix; ?> = caUI.initRelationBundle('#<?php print $vs_id_prefix; ?>', {
			...
			sortUrl: '<?php print caNavUrl( $this->request, $this->request->getModulePath(),
				$this->request->getController(), 'Sort', array( 'table' => 'ca_items_x_tags' ) ); ?>',
