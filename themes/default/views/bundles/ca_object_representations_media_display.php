...
<div id="<?= $vs_id_prefix; ?>">
	<div class="bundleContainer">
		<table  style="width: 100%;">
			<tr>
				<td>
					<div id="<?= "{$vs_id_prefix}"; ?>_media_upload_control">
						<?= $t_subject->htmlFormElement('media', null, array('displayMediaVersion' => null, 'name' => "{$vs_id_prefix}_media", 'id' => "{$vs_id_prefix}_media", "value" => "", 'no_tooltips' => false, 'tooltip_namespace' => 'bundle_ca_object_representations_media_display')); ?>
<?php
	if ($vb_allow_fetching_from_urls) {
?>		
						<a href='#' onclick='jQuery("#<?= "{$vs_id_prefix}"; ?>_media_url_control").slideDown(200); jQuery("#<?= "{$vs_id_prefix}"; ?>_media_upload_control").slideUp(200); return false'><?= _t('or fetch from a URL'); ?></a>
<?php
	}
?>
					</div>
<?php
	if ($vb_allow_fetching_from_urls) {
?>
					<div id="<?= "{$vs_id_prefix}"; ?>_media_url_control" style="display: none;">
						<?= _t('Fetch from URL').':<br/>'.caHTMLTextInput("{$vs_id_prefix}_url", array('id' => "{$vs_id_prefix}_url"), array('width' => 40, 'height' => '3')); ?>
						<br/>
						<a href='#' onclick='jQuery("#<?= "{$vs_id_prefix}"; ?>_media_url_control").slideUp(200); jQuery("#<?= "{$vs_id_prefix}"; ?>_media_upload_control").slideDown(200); return false'><?= _t('or upload a file'); ?></a>
					</div>
<?php
	}
?>
...
?>						
						<div style="float:right; margin-left: 5px;">
							<?= urldecode(caNavButton($this->request, __CA_NAV_ICON_DOWNLOAD__, _t('Download'), '', '*', '*', 'DownloadMedia', array('version' => 'original', 'representation_id' => $t_subject->getPrimaryKey(), 'download' => 1), array('id' => "{$vs_id_prefix}download"), array('no_background' => true, 'dont_show_content' => true))); ?>
						</div>
<?php
	}
?>
...
<?php
	if ($vb_media_is_set) {
?>
		<div class="objectRepresentationMediaDisplayDerivativeOptionsContainer" id="<?= "{$vs_id_prefix}_derivative_options_container"; ?>">
...
?>
		<div class="objectRepresentationMediaDisplayDerivativeOptions rounded" id="<?= "{$vs_id_prefix}_derivative_options"; ?>">
...
?>
			...
			<div class="objectRepresentationMediaDisplayDerivativeHelpText" id="<?= "{$vs_id_prefix}_derivative_options_help_text"; ?>">
