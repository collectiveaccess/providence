...
$t_location            = ca_storage_locations::find( $t_item->get( 'home_location_id' ),
	[ 'returnAs' => 'firstModelInstance' ] );
$home_location_idno    = $t_location ? $t_location->getWithTemplate( $this->request->config->get( [
	'inspector_home_location_display_template',
	'ca_storage_locations_hierarchy_browser_display_settings'
] ) ) : null;
	...
	function _initSetHomeLocationHierarchyBrowser() {
		...
		if (!oSetHomeLocationHierarchyBrowser) {
			oSetHomeLocationHierarchyBrowser = caUI.initHierBrowser('SetHomeLocationHierarchyBrowser', {
				...
				incrementalLoadIndicator: "<?php print caNavIcon( __CA_NAV_ICON_SPINNER__, 1 ) . ' '
				                                       . _t( 'Loading' ); ?>",
			...
			jQuery("#SetHomeLocationHierarchyBrowserSelectionMessage").html('<?php print addslashes( $home_location_idno
				? str_replace( '%', $home_location_idno, $home_location_message )
				: _t( 'Home location is not set' ) ); ?>');
		}
	}
...
<div id="caSetHomeLocationPanel" class="caSetHomeLocationPanel">
	...
	<div id="caSetHomeLocationPanelContentArea">
		<?php print caFormTag( $this->request, '#', 'caSetHomeLocationForm', null, 'post', 'multipart/form-data',
			'_top', [ 'noCSRFToken' => true, 'disableUnsavedChangesWarning' => true ] ); ?>
		<div>
			<div class="hierarchyBrowserFind" style="float: right;">
				<?php print _t( 'Find' ); ?>: <input type="text" id="SetHomeLocationHierarchyBrowserSearch"
				                                     name="search" value="" size="25"/>
			</div>
			<div class="hierarchyBrowserMessageContainer">
				<div id='SetHomeLocationHierarchyBrowserSelectionMessage' class='hierarchyBrowserNewLocationMessage'>
					<!-- Message specifying move destination is dynamically inserted here by ca.hierbrowser --></div>
			</div>
		...
		<div id="caSetHomeLocationPanelControlButtons">
			<table>
				<tr>
					<td align="right"><?php print caFormJSButton( $this->request, __CA_NAV_ICON_SAVE__, _t( 'Save' ),
							'caSetHomeLocationForm', [ 'onclick' => 'setHomeLocation(); return false;' ] ); ?></td>
					<td align="left"><?php print caJSButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( 'Cancel' ),
							'caSetHomeLocationFormCancelButton',
							array( 'onclick' => 'caSetHomeLocationPanel.hidePanel(); return false;' ),
							array( 'size' => '30px' ) ); ?></td>
				</tr>
			</table>
		</div>
