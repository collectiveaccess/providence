...
$vb_read_only     = ( ( isset( $va_settings['readonly'] ) && $va_settings['readonly'] )
                      || ( $this->request->user->getBundleAccessLevel( $t_instance->tableName(), 'ca_objects' )
                           == __CA_BUNDLE_ACCESS_READONLY__ ) );
$vb_dont_show_del = ( ( isset( $va_settings['dontShowDeleteButton'] ) && $va_settings['dontShowDeleteButton'] ) ) ? true
	: false;
...
<div id="<?php print $vs_id_prefix; ?>" <?php print $vb_batch ? "class='editorBatchBundleContent'" : ''; ?>>
	...
	if ( is_array( $this->getVar( 'initialValues' ) ) && sizeof( $this->getVar( 'initialValues' ) ) ) {
		...
		if ( caGetOption( 'showReturnToHomeLocations', $va_settings, false ) ) {
			print caReturnToHomeLocationControlForRelatedObjectBundle( $vs_id_prefix, $this->request, $t_instance,
				$t_item, $t_item_rel, $this->getVar( 'initialValues' ), $this->getVar( 'history_tracking_policy' ) );
		}
	}
	if ( is_array( $this->getVar( 'initialValues' ) ) && sizeof( $this->getVar( 'initialValues' ) ) && ! $vb_read_only
		...
		print caEditorBundleSortControls( $this->request, $vs_id_prefix, $t_item->tableName(),
			array_merge( $va_settings, [ 'sort' => $loaded_sort, 'sortDirection' => $loaded_sort_direction ] ) );
	}
...
<?php
print caGetRelationDisplayString( $this->request, 'ca_objects',
	array( 'class' => 'caEditItemButton', 'id' => "{$vs_id_prefix}_edit_related_{n}" ), array(
		'display'                         => '_display',
		'makeLink'                        => true,
		'prefix'                          => $vs_id_prefix,
		'relationshipTypeDisplayPosition' => ( ( $t_subject->tableName() == 'ca_object_lots' ) ? 'none' : null )
	) );
?>
			</span>
			<input type="hidden" name="<?php print $vs_id_prefix; ?>_type_id{n}"
			       id="<?php print $vs_id_prefix; ?>_type_id{n}" value="{type_id}"/>
		...
		<div id="<?php print $vs_id_prefix; ?>Item_{n}" class="labelInfo caRelatedItem">
			<table class="caListItem">
				<tr>
					...
					<td>
<?php
if ( is_array( $this->getVar( 'relationship_types_by_sub_type' ) )
     && sizeof( $this->getVar( 'relationship_types_by_sub_type' ) )
) {
	?>
...
?>
					...
					<td>
						...
						<a href="<?php print urldecode( caEditorUrl( $this->request, 'ca_objects',
							'{object_id}' ) ); ?>" class="caEditItemButton"
...
<div id="caRelationEditorPanel<?php print $vs_id_prefix; ?>" class="caRelationQuickAddPanel">
	...
	<textarea class='caBundleDisplayTemplate' style='display: none;'>
		<?php print caGetRelationDisplayString( $this->request, 'ca_objects', array(),
			array( 'display' => '_display', 'makeLink' => false ) ); ?>
	</textarea>
</div>
	...
	jQuery(document).ready(function () {
		...
		caRelationBundle<?php print $vs_id_prefix; ?> = caUI.initRelationBundle('#<?php print $vs_id_prefix; ?>', {
			...
			quickaddUrl: '<?php print caNavUrl( $this->request, 'editor/objects', 'ObjectQuickAdd', 'Form', array(
				'object_id'                                 => 0,
				'source'                                    => $t_instance->tableName(),
				'source_id'                                 => $t_instance->getPrimaryKey(),
