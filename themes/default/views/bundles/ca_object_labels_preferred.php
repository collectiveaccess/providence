...
?>
	<textarea class='caLabelTemplate' style='display: none;'>
		<div id="{fieldNamePrefix}Label_{n}" class="labelInfo">
			...
			<?= $t_label->htmlFormElement('name', "^ELEMENT", array_merge($settings, array('request' => $this->request, 'name' => "{fieldNamePrefix}name_{n}", 'id' => "{fieldNamePrefix}name_{n}", "value" => "{{name}}", 'no_tooltips' => true, 'textAreaTagName' => 'textentry', 'readonly' => $read_only))); ?>
			<div class="formLabel">
...
<script type="text/javascript">
	caUI.initLabelBundle('#<?= $id_prefix; ?>Labels', {
		...
		templateValues: ['name', 'locale_id', 'type_id', 'effective_date', 'access'],
