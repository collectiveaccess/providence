...
$va_occ_lookup_params['types'] = join(
    ",",
    array_map(
        function ($v) {
            return $v['item_id'];
    ...
    <div id="<?php print $vs_id_prefix; ?>">
        <div class="bundleContainer">
            <div class="caUseHistoryButtonBar labelInfo">
                <?php
                if (!caGetOption('hide_include_child_history_controls', $va_settings, false) && ($this->getVar(
                            'child_count'
                        ) > 0)) {
                    ?>
                    <div style='float: left;' class='button caSetChildViewButton'><a href="#"
                                                                                     id="<?php print $vs_id_prefix; ?>SetChildView"><?php print caNavIcon(
                                __CA_NAV_ICON_CHILD__,
                                '15px'
                            ); ?><?php print Session::getVar('ca_objects_history_showChildHistory') ? _t(
                                'Hide child history'
                            ) : _t('Include child history'); ?></a></div>
                    <?php
                }
                if (!$vb_read_only && !caGetOption('hide_add_to_loan_controls', $va_settings, false)) {
                    ...
                    <div style='float: left;' class='button caAddLoanButton'><a href="#"
                                                                                id="<?php print $vs_id_prefix; ?>AddLoan"><?php print caNavIcon(
                                __CA_NAV_ICON_ADD__,
                                '15px'
                            ); ?><?php print _t('Add to loan'); ?></a></div>
                    <?php
                }
                if (!$vb_read_only && !caGetOption('hide_update_location_controls', $va_settings, false)) {
                    ?>
                    <div style='float: left;' class='button caChangeLocationButton'><a href="#"
                                                                                       id="<?php print $vs_id_prefix; ?>ChangeLocation"><?php print caNavIcon(
                                __CA_NAV_ICON_ADD__,
                                '15px'
                            ); ?><?php print _t('Update location'); ?></a></div>
                    <?php
                }
                ...
                if (!$vb_read_only && !caGetOption('hide_add_to_occurrence_controls', $va_settings, false)) {
                    foreach ($va_occ_types as $vn_type_id => $va_type_info) {
                        ...
                        <div style='float: left;'
                             class='button caAddOccurrenceButton caAddOccurrenceButton<?php print $vn_type_id; ?>'><a
                                href="#"
                                id="<?php print $vs_id_prefix; ?>AddOcc<?php print $vn_type_id; ?>"><?php print caNavIcon(
                                    __CA_NAV_ICON_ADD__,
                                    '15px'
                                ); ?><?php print _t('Add to %1', $va_type_info['name_singular']); ?></a></div>
                        <?php
                    }
                }
...
if (!(bool)$va_settings['useHierarchicalBrowser']) {
    ...
    <table class="caListItem">
				<tr>
					<td>
						<input type="text" size="60" name="<?php print $vs_id_prefix; ?>_location_autocomplete{n}"
                               value="{{label}}" id="<?php print $vs_id_prefix; ?>_location_autocomplete{n}"
                               class="lookupBg"/>
						<input type="hidden" name="<?php print $vs_id_prefix; ?>_location_id{n}"
                               id="<?php print $vs_id_prefix; ?>_location_id{n}" value="{id}"/>
					</td>
					<td>
						<a href="#" class="caDeleteLocationButton"><?php print caNavIcon(
...
?>
    ...
    <div style="float: right;"><a href="#" class="caDeleteLocationButton"><?php print caNavIcon(
                __CA_NAV_ICON_DEL_BUNDLE__,
                1
            ); ?></a></div>
        ...
        if (is_array($va_storage_location_elements) && sizeof($va_storage_location_elements)) {
            ...
            foreach ($va_storage_location_elements as $vs_element) {
                ...
                if ($t_rel->hasField($vs_element)) {
                    ...
                    print "<td><div class='attributeListItem'>" . $t_rel->getDisplayLabel(
                            $t_rel->tableName() . "." . $vs_element
                        ) . "</td><td>" . $t_rel->htmlFormElement(
                            $vs_element,
                            '',
                            [
                                'name' => $vs_id_prefix . '_location_' . $vs_element . '{n}',
                                'id' => $vs_id_prefix . '_location_' . $vs_element . '{n}',
                                'value' => _t('now'),
                                'classname' => $vs_field_class
                            ]
                        ) . "</td>";
                } else {
                    print "<td>" . $t_rel->getDisplayLabel(
                            $t_rel->tableName() . "." . $vs_element
                        ) . "</td><td>" . $t_rel->getAttributeHTMLFormBundle(
                            $this->request,
                            null,
                            $vs_element,
    ...
    <script type='text/javascript'>
				jQuery(document).ready(function () {
                    ...
                    = caUI.initHierBrowser('<?php print $vs_id_prefix; ?>_hierarchyBrowser{n}', {
                        uiStyle: 'horizontal',
                        levelDataUrl: '<?php print caNavUrl(
                            $this->request,
                            'lookup',
                            'StorageLocation',
                            'GetHierarchyLevel',
                        ...
                        initDataUrl: '<?php print caNavUrl(
                            $this->request,
                            'lookup',
                            'StorageLocation',
...
?>
		...
		<div id="<?php print $vs_id_prefix; ?>Loan_{n}" class="labelInfo caRelatedLoan">
			<table class="caListItem">
				<tr>
					...
					<td>
						<input type="text" size="60" name="<?php print $vs_id_prefix; ?>_loan_autocomplete{n}"
                               value="{{label}}" id="<?php print $vs_id_prefix; ?>_loan_autocomplete{n}"
                               class="lookupBg"/>
					</td>
					<td>
						<select name="<?php print $vs_id_prefix; ?>_loan_type_id{n}"
                                id="<?php print $vs_id_prefix; ?>_loan_type_id{n}" style="display: none;"></select>
						<input type="hidden" name="<?php print $vs_id_prefix; ?>_loan_id{n}"
                               id="<?php print $vs_id_prefix; ?>_loan_id{n}" value="{id}"/>
					</td>
					<td>
						<a href="#" class="caDeleteLoanButton"><?php print caNavIcon(
                                __CA_NAV_ICON_DEL_BUNDLE__,
                                1
                            ); ?></a>
					</td>
		...
		<div class="clear"><!-- empty --></div>
		<div id="<?php print $vs_id_prefix; ?>Occurrence_<?php print $vn_type_id; ?>_{n}"
             class="labelInfo caRelatedOccurrence">
			<table class="caListItem">
				<tr>
					<td><h2><?php print _t('Add to %1', $va_type_info['name_singular']); ?></h2></td>
					<td>
						<input type="text" size="60"
                               name="<?php print $vs_id_prefix; ?>_occurrence_<?php print $vn_type_id; ?>_autocomplete{n}"
                               value="{{label}}"
                               id="<?php print $vs_id_prefix; ?>_occurrence_<?php print $vn_type_id; ?>_autocomplete{n}"
                               class="lookupBg"/>
					</td>
					<td>
						<select name="<?php print $vs_id_prefix; ?>_occurrence_<?php print $vn_type_id; ?>_type_id{n}"
                                id="<?php print $vs_id_prefix; ?>_occurrence_<?php print $vn_type_id; ?>_type_id{n}"
                                style="display: none;"></select>
						<input type="hidden"
                               name="<?php print $vs_id_prefix; ?>_occurrence_<?php print $vn_type_id; ?>_id{n}"
                               id="<?php print $vs_id_prefix; ?>_occurrence_<?php print $vn_type_id; ?>_id{n}"
                               value="{id}"/>
					</td>
					<td>
						<a href="#" class="caDeleteOccurrenceButton<?php print $vn_type_id; ?>"><?php print caNavIcon(
                                $this->request,
                                __CA_NAV_ICON_DEL_BUNDLE__,
                                1
                            ); ?></a>
    ...
    <script type="text/javascript">
        ...
        jQuery(document).ready(function () {
            ...
            caRelationBundle<?php print $vs_id_prefix; ?> = caUI.initRelationBundle('#<?php print $vs_id_prefix; ?>', {
                ...
                relationshipTypes: <?php print json_encode(
                    $this->getVar('location_relationship_types_by_sub_type')
                ); ?>,
                ...
                quickaddUrl: '<?php print caNavUrl(
                    $this->request,
                    'editor/storage_locations',
                    'StorageLocationQuickAdd',
                    'Form',
                    array(
                        'location_id' => 0,
            ...
            caRelationBundle<?php print $vs_id_prefix; ?>_ca_occurrences_<?php print $vn_type_id; ?> = caUI.initRelationBundle('#<?php print $vs_id_prefix; ?>', {
                ...
                showEmptyFormsOnLoad: 0,
                relationshipTypes: <?php print json_encode(
                    $this->getVar('occurrence_relationship_types_by_sub_type')
                ); ?>,
            ...
            jQuery('#<?php print $vs_id_prefix; ?>SetChildView').on('click', function (e) {
                if (caBundleUpdateManager) {
                    caBundleUpdateManager.reloadBundle('ca_objects_history', {
                        'showChildHistory': <?php print Session::getVar(
