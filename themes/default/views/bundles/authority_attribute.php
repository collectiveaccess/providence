...
<script type='text/javascript'>
    jQuery(document).ready(function () {
        if (caUI.initPanel) {
            var caRelationQuickAddPanel<?php print $vs_field_name_prefix; ?>_
            {
                n
            }
            ;
            caRelationQuickAddPanel<?php print $vs_field_name_prefix; ?>_
            {
                n
            }
            = caUI.initPanel({
        ...
        jQuery('#<?php print $vs_field_name_prefix; ?>_autocomplete{n}').autocomplete({
            ...
            source: function (request, response) {
                $.ajax({
                    ...
                    data: {
                        term: request.term,
                        quickadd: <?php print $this->getVar('allowQuickadd') ? 1 : 0; ?>,
            ...
            select: function (event, ui) {
                var quickaddPanel = caRelationQuickAddPanel<?php print $vs_field_name_prefix; ?>_
                {
                    n
                }
                ;
                ...
                if (!parseInt(ui.item.id) || (ui.item.id == 0)) {
                    ...
                    quickaddPanel.showPanel(panelUrl, null, null, {
                        q: ui.item._query,
                        field_name_prefix: '<?php print $vs_field_name_prefix; ?>'
                    });
