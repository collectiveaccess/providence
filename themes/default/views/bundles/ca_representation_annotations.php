...
print caEditorBundleMetadataDictionary(
    $this->request,
    $vs_id_prefix . $t_item->tableNum() . '_annotations',
    $va_settings
);
?>
<!-- BEGIN Media Player -->
...
$va_media_player_config = caGetMediaDisplayInfo(
    'annotation_editor',
    $t_subject->getMediaInfo(
        'media',
        $o_properties->getDisplayMediaVersion(),
        'MIMETYPE'
    )
);
?>
<div class="caAnnotationMediaPlayerContainer">
    <?php print $t_subject->getMediaTag(
        'media',
        $o_properties->getDisplayMediaVersion(),
        array(
...
foreach ($va_properties as $vs_property) {
    print '<td>' . $o_properties->htmlFormElement(
            $vs_property,
            array(
...
?>
                            ...
                            class="button" id="{fieldNamePrefix}gotoButton_{n}"><?php print _t(
                                'Play Clip'
                            ); ?> &rsaquo;</a></td>
<?php
}
...
?>
					<td><?php print $t_item_label->htmlFormElement(
                            'name',
                            null,
                            array(
                                'classname' => 'labelLocale',
			...
			<div style="display:none;" id="{fieldNamePrefix}moreOptions_{n}">
				<table class="representationAnnotationListItem">
					<tr>
						...
						<td><?php print $t_item->htmlFormElement(
                                'status',
                                null,
                                array(
                                    'classname' => 'labelLocale',
						...
						<td><?php print urldecode(
                                caNavLink(
                                    ...
                                    array('id' => "{fieldNamePrefix}edit_{n}")
                                )
                            ); ?></td>
					</tr>
				</table>
    ...
    <div class="bundleContainer">
        <div class='button labelInfo caAddItemButton'><a href='#'><?php print caNavIcon(
                    __CA_NAV_ICON_ADD__,
                    '15px'
                ); ?> <?php print _t("Add annotation"); ?> &rsaquo;</a></div>
