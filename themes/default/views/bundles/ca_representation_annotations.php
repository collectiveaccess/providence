...
$vn_timecode_offset = isset( $va_media_props['PROPERTIES']['timecode_offset'] )
	? (float) $va_media_props['PROPERTIES']['timecode_offset'] : 0;
...
if ( sizeof( $va_inital_values ) ) {
	foreach ( $va_inital_values as $vn_annotation_id => $va_info ) {
		if ( is_array(
			$va_action_errors = $this->request->getActionErrors( 'ca_representation_annotations', $vn_annotation_id ) )
		) {
			foreach ( $va_action_errors as $o_error ) {
				$va_errors[ $vn_annotation_id ][] = array(
...
foreach ( $this->request->getActionErrorSubSources( 'ca_representation_annotations' ) as $vs_error_subsource ) {
	if ( substr( $vs_error_subsource, 0, 4 ) === 'new_' ) {
		...
		foreach ( $va_action_errors as $o_error ) {
			$va_failed_inserts[] = array_merge( $va_init_props, array(
				'_errors' => array(
					array(
						'errorDescription' => $o_error->getErrorDescription(),
						'errorCode'        => $o_error->getErrorNumber()
...
print caEditorBundleMetadataDictionary( $this->request, $vs_id_prefix . $t_item->tableNum() . '_annotations',
	$va_settings );
?>
<!-- BEGIN Media Player -->
...
$va_media_player_config = caGetMediaDisplayInfo( 'annotation_editor',
	$t_subject->getMediaInfo( 'media', $o_properties->getDisplayMediaVersion(), 'MIMETYPE' ) );
?>
<div class="caAnnotationMediaPlayerContainer">
	<?php print $t_subject->getMediaTag( 'media', $o_properties->getDisplayMediaVersion(), array(
		'class'            => 'caAnnotationMediaPlayer',
		'viewer_width'     => $va_media_player_config['viewer_width'],
		'viewer_height'    => $va_media_player_config['viewer_height'],
		'id'               => 'annotation_media_player',
		'poster_frame_url' => $t_subject->getMediaUrl( 'media', 'medium' )
	) ); ?>
</div>
</div>
<!-- END Media Player -->
...
foreach ( $va_properties as $vs_property ) {
	print '<td>' . $o_properties->htmlFormElement( $vs_property, array(
			'classname' => 'labelLocale',
			'id'        => "{fieldNamePrefix}" . $vs_property . "_{n}",
			'name'      => "{fieldNamePrefix}" . $vs_property . "_{n}",
			"value"     => "{{" . $vs_property . "}}"
		) ) . "</td>\n";
	$vn_col_count ++;
}
if ( $vs_goto_property ) {
?>
					</tr><tr><td <?php print ( $vn_col_count > 1 ) ? "colspan='" . $vn_col_count . "'" : ""; ?>><a
							href="#"
							onclick="if (!jQuery('#annotation_media_player').data('hasBeenPlayed')) { jQuery('#annotation_media_player')[0].player.play(); jQuery('#annotation_media_player').data('hasBeenPlayed', true); } jQuery('#annotation_media_player')[0].player.setCurrentTime((parseFloat({{startTimecode_raw}}) >= 0 ? parseFloat({{startTimecode_raw}}) : 0) + <?php print $vn_timecode_offset; ?>); return false;"
							class="button" id="{fieldNamePrefix}gotoButton_{n}"><?php print _t( 'Play Clip' ); ?> &rsaquo;</a></td>
<?php
}
print "</tr></table></td>";
}
?>
					<td><?php print $t_item_label->htmlFormElement( 'name', null, array(
							'classname'       => 'labelLocale',
							'id'              => "{fieldNamePrefix}label_{n}",
							'name'            => "{fieldNamePrefix}label_{n}",
							"value"           => "{{label}}",
							'no_tooltips'     => false,
							'width'           => 35,
					...
					<td><a href="#"
					       onclick="jQuery('#{fieldNamePrefix}moreOptions_{n}').slideToggle(250); return false;"
					       class="button"><?php print _t( 'More' ); ?> &rsaquo;</a></td>
			...
			<div style="display:none;" id="{fieldNamePrefix}moreOptions_{n}">
				<table class="representationAnnotationListItem">
					<tr>
						<td><?php print $t_item_label->htmlFormElement( 'locale_id', null, array(
								'classname'   => 'labelLocale',
								'id'          => "{fieldNamePrefix}locale_id_{n}",
								'name'        => "{fieldNamePrefix}locale_id_{n}",
								"value"       => "",
								'no_tooltips' => false,
								'WHERE'       => array( '(dont_use_for_cataloguing = 0)' )
							) ); ?></td>
						<td><?php print $t_item->htmlFormElement( 'status', null, array(
								'classname'   => 'labelLocale',
						...
						<td><?php print $t_item->htmlFormElement( 'access', null, array(
								'classname'   => 'labelLocale',
						...
						<td><?php print urldecode( caNavLink( $this->request, caNavIcon( __CA_NAV_ICON_EDIT__, 1 ), '',
								'editor/representation_annotations', 'RepresentationAnnotationEditor', 'Edit',
								array( 'annotation_id' => "{n}" ),
								array( 'id' => "{fieldNamePrefix}edit_{n}" ) ) ); ?></td>
					</tr>
				</table>
			</div>
		</div>
	...
	<div class="bundleContainer">
		<div class='button labelInfo caAddItemButton'><a href='#'><?php print caNavIcon( __CA_NAV_ICON_ADD__,
					'15px' ); ?> <?php print _t( "Add annotation" ); ?> &rsaquo;</a></div>
