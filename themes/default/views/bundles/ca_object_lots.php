...
$vb_read_only = ( ( isset( $va_settings['readonly'] ) && $va_settings['readonly'] )
                  || ( $this->request->user->getBundleAccessLevel( $t_instance->tableName(), 'ca_object_lots' )
...
?>
			<a href="<?php print urldecode( caEditorUrl( $this->request, 'ca_object_lots', '{lot_id}' ) ); ?>"
...
<?php
print caGetRelationDisplayString( $this->request, 'ca_object_lots',
...
?>
	...
	default:
		...
		<div id="<?php print $vs_id_prefix; ?>Item_{n}" class="labelInfo roundedRel">
			<a href="<?php print urldecode( caEditorUrl( $this->request, 'ca_object_lots', '{lot_id}' ) ); ?>"
			   class="caEditItemButton" id="<?php print $vs_id_prefix; ?>_edit_related_{n}">{{label}}</a>
			...
			<table class="caListItem">
				<tr>
					...
					<td>
						...
						<a href="<?php print urldecode( caEditorUrl( $this->request, 'ca_object_lots',
							'{lot_id}' ) ); ?>" class="caEditItemButton"
	...
	<div class="bundleContainer">
		...
		if ( ! $vb_read_only ) {
			...
			<div class='button labelInfo caAddItemButton'><a href='#'><?php print caNavIcon( __CA_NAV_ICON_ADD__,
						'15px' ); ?><?php print $vs_add_label ? $vs_add_label : _t( "Add lot" ); ?></a></div>
			<?php
		}
		?>
	</div>
...
<div id="caRelationEditorPanel<?php print $vs_id_prefix; ?>" class="caRelationQuickAddPanel">
	...
	<textarea class='caBundleDisplayTemplate' style='display: none;'>
		<?php print caGetRelationDisplayString( $this->request, 'ca_object_lots', array(),
...
<script type="text/javascript">
	...
	jQuery(document).ready(function () {
		...
		caRelationBundle<?php print $vs_id_prefix; ?> = caUI.initRelationBundle('#<?php print $vs_id_prefix; ?>', {
			...
			autocompleteUrl: '<?php print caNavUrl( $this->request, 'lookup', 'ObjectLot', 'Get',
				$va_lookup_params ); ?>',
			...
			sortUrl: '<?php print caNavUrl( $this->request, $this->request->getModulePath(),
				$this->request->getController(), 'Sort', array( 'table' => $t_item->tableName() ) ); ?>',
			...
			isSortable: <?php print ( $t_subject->tableName() != 'ca_objects' ) ? ( $vb_read_only ? "false" : "true" )
				: "false"; ?>,
			...
			quickaddUrl: '<?php print caNavUrl( $this->request, 'editor/object_lots', 'ObjectLotQuickAdd', 'Form',
				array(
					'lot_id'                                    => 0,
