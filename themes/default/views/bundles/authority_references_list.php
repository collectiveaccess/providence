				...
				<div id="<?php print $vs_id_prefix; ?>AuthorityReferenceTabs" class="authorityReferenceListContainer">
					<ul>
						...
						foreach ( $va_references as $vn_table_num => $va_rows ) {
							if ( $t_ref_instance = Datamodel::getInstance( $vn_table_num, true ) ) {
								...
								?>
								<li>
									<a href="#<?php print $vs_id_prefix; ?>AuthorityReferenceTabs-<?php print $vs_ref_table_name; ?>"><span><?php print _t( '%1 (%2)',
												caUcFirstUTF8Safe( $t_ref_instance->getProperty( 'NAME_PLURAL' ) ),
												sizeof( $va_rows ) ); ?></span></a></li>
								<?php
							}
						}
						?>
					...
					foreach ( $va_references as $vn_table_num => $va_rows ) {
						if ( $t_ref_instance = Datamodel::getInstance( $vn_table_num, true ) ) {
							...
							?>
							<div
								id="<?php print $vs_id_prefix; ?>AuthorityReferenceTabs-<?php print $vs_ref_table_name; ?>"
								class="authorityReferenceListTab">
								<?php
								print "<ul class='authorityReferenceList'>\n";

								if ( ! ( $vs_template = caGetOption( "{$vs_ref_table_name}_displayTemplate",
									$va_settings, null ) )
								) {
									if ( is_array( $vs_template = $t_instance->getAppConfig()
									                                         ->getList( "{$vs_ref_table_name}_lookup_settings" ) )
									) {
										$vs_template = join( $t_instance->getAppConfig()
										                                ->get( "{$vs_ref_table_name}_lookup_delimiter" ),
											$vs_template );
									} elseif ( ! ( $vs_template = $t_instance->getAppConfig()
									                                         ->get( "{$vs_ref_table_name}_lookup_settings" ) )
									) {
								...
								$va_items = caProcessTemplateForIDs( $vs_template, $t_ref_instance->tableName(),
									array_keys( $va_rows ), array( 'returnAsArray' => true ) );
								...
								if ( ( sizeof( $va_items ) >= $vn_max_items )
								     && is_array( $va_search_strings[ $vs_ref_table_name ] )
								) {
									print "<li>" . caSearchLink( $this->request,
											_t( '... and %1 more', sizeof( $va_items ) - $vn_max_items ), '',
											$vs_ref_table_name,
											join( " OR ", $va_search_strings[ $vs_ref_table_name ] ) ) . "</li>\n";
								}
