...
$vs_bundle_preview = '(' . $vn_items_in_hier . ') ' . caProcessTemplateForIDs(
        "^preferred_labels",
        $t_subject->tableName(),
        array($t_subject->getPrimaryKey())
    );
...
$vb_objects_x_collections_hierarchy_enabled = (bool)$t_subject->getAppConfig()->get(
    'ca_objects_x_collections_hierarchy_enabled'
);
$vs_disabled_items_mode = $t_subject->getAppConfig()->get(
    $t_subject->tableName() . '_hierarchy_browser_disabled_items_mode'
);
...
if ($t_subject->getProperty('HIERARCHY_ID_FLD') && ($vn_hier_id = (int)$t_subject->get(
        $t_subject->getProperty('HIERARCHY_ID_FLD')
    ))) {
    $va_search_lookup_extra_params['currentHierarchyOnly'] = $vn_hier_id;
}
if (in_array(
        $t_subject->tableName(),
        array('ca_objects', 'ca_collections')
    ) && $vb_objects_x_collections_hierarchy_enabled) {
    $va_lookup_urls_for_move = $va_lookup_urls = array(
        'search' => caNavUrl(
            $this->request,
            'lookup',
            'ObjectCollectionHierarchy',
            'Get',
    ...
    $va_lookup_urls_for_move['search'] = caNavUrl(
        $this->request,
        'lookup',
        'ObjectCollectionHierarchy',
        'Get',
    ...
    $va_lookup_urls_for_move = caJSONLookupServiceUrl(
        $this->request,
        $t_subject->tableName(),
        array_merge(
            $va_search_lookup_extra_params,
...
$vb_strict_type_hierarchy = (bool)$this->request->config->get(
    $t_subject->tableName() . '_enforce_strict_type_hierarchy'
);
$vs_type_selector = trim(
    $t_subject->getTypeListAsHTMLFormElement(
        "{$vs_id_prefix}type_id",
        array('id' => "{$vs_id_prefix}typeList"),
        array(
...
$vb_read_only = ((isset($pa_bundle_settings['readonly']) && $pa_bundle_settings['readonly']) || ($this->request->user->getBundleAccessLevel(
            $t_subject->tableName(),
            'hierarchy_location'
        ) == __CA_BUNDLE_ACCESS_READONLY__));

$hier_browser_width = ($pdim = caParseElementDimension(
    caGetOption('width', $pa_bundle_settings, null)
)) ? $pdim['expression'] : null;
$hier_browser_height = ($pdim = caParseElementDimension(
    caGetOption('height', $pa_bundle_settings, null)
)) ? $pdim['expression'] : null;
    ...
    foreach ($pa_ancestors as $vn_id => $va_item) {
        ...
        if ($vn_id === '') {
            ...
            if (($va_item['table'] != $t_subject->tableName(
                    )) || ($va_item['item_id'] && ($va_item['item_id'] != $pn_id))) {
                $va_path[] = '<a href="' . caEditorUrl(
                        $this->request,
                        $va_item['table'],
                        $va_item['item_id']
                    ) . '">' . $vs_label . '</a>';
    ...
    <div class="bundleContainer">
        ...
        if (!$vb_batch) {
            ...
            <div class="hierNav">
                <?php
                if (sizeof($va_errors)) {
                    print "<div class='formLabel'><span class='formLabelError'>" . join(
                            '; ',
                            $va_errors
                        ) . "</span></div>\n";
                }
                ...
                if (!$vb_batch && ($pn_id > 0)) {
                    ...
                    <div class="hierarchyCountDisplay"><?php if ($vn_items_in_hier > 0) {
                            print _t(
                                "Number of %1 in hierarchy: %2",
                                caGetTableDisplayName($t_subject->tableName(), true),
                                $vn_items_in_hier
                            );
                        } ?></div>
        ...
        if ($pn_id > 0) {
                ...
                <div id="<?php print $vs_id_prefix; ?>HierarchyBrowserTabs">
                    <ul>
                        ...
                        if (!$vb_batch) {
                            ...
                            <li><a href="#<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-explore"
                                   onclick='_init<?php print $vs_id_prefix; ?>ExploreHierarchyBrowser();'><span><?php print _t(
                                            'Explore'
                                        ); ?></span></a></li>
                            <?php
                        }
                        if ((!$vb_strict_type_hierarchy || $vb_batch) && !$vb_read_only) {
                            ...
                            <li><a href="#<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-move"
                                   onclick='_init<?php print $vs_id_prefix; ?>MoveHierarchyBrowser();'><span><?php print _t(
                                            'Move'
                                        ); ?></span></a></li>
                            <?php
                        }
                        if ((!$vb_read_only && $vb_has_privs && !$vb_batch) && (!$vb_strict_type_hierarchy || ($vb_strict_type_hierarchy && $vs_type_selector))) {
                            ...
                            <li><a href="#<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-add"
                                   onclick='_init<?php print $vs_id_prefix; ?>AddHierarchyBrowser();'><span><?php print ($vb_objects_x_collections_hierarchy_enabled && ($t_subject->tableName(
                                                ) == 'ca_collections')) ? _t('Add level') : _t('Add'); ?></span></a>
                            </li>
                            <?php
                        }
                        if ((!$vb_read_only && $vb_has_privs && !$vb_batch) && $vb_objects_x_collections_hierarchy_enabled && ($t_subject->tableName(
                                ) == 'ca_collections')) {
                            ?>
                            <li><a href="#<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-addObject"
                                   onclick='_init<?php print $vs_id_prefix; ?>AddObjectHierarchyBrowser();'><span><?php print _t(
                                            'Add object'
                                        ); ?></span></a></li>
                            <?php
                        }
                    ...
                    if ((!$vb_strict_type_hierarchy || $vb_batch) && !$vb_read_only) {
                        ...
                        <div id="<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-move" class="hierarchyBrowseTab">
                            ...
                            <div class="hierarchyBrowserMessageContainer">
                                <?php print _t(
                                    'Click on an arrow to choose the location to move this record under.',
                                    $t_subject->getProperty('NAME_SINGULAR')
                                ); ?>
                            ...
                            <?php
                            if (($t_subject->tableName() == 'ca_storage_locations') && (bool)$t_subject->getAppConfig(
                                )->get('record_movement_information_when_moving_storage_location')) {
                                if ($t_ui = ca_editor_uis::loadDefaultUI(
                                    'ca_movements',
                                    $this->request,
                                    null,
                                    ...
                                    <div id="<?php print $vs_id_prefix; ?>StorageLocationMovementForm"
                                        ...
                                        print caHTMLHiddenInput(
                                            $vs_id_prefix . '_movement_screen',
                                            array('value' => $va_nav['defaultScreen'])
                                        );
                                        print caHTMLHiddenInput(
                                            $vs_id_prefix . '_movement_form_name',
                    ...
                    if ((!$vb_read_only && $vb_has_privs && !$vb_batch) && (!$vb_strict_type_hierarchy || ($vb_strict_type_hierarchy && $vs_type_selector))) {
                        ...
                        <div id="<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-add" class="hierarchyBrowseTab">
                            <div class="hierarchyBrowserMessageContainer">
                                <?php print _t(
                                    'Use the controls below to create new %1 relative to this record in the hierarchy.',
                            ...
                            <div id='<?php print $vs_id_prefix; ?>HierarchyBrowseTypeMenu' style="margin-top: 15px;">
                                <div style="float: left; width: 700px">
                                    ...
                                    if ($vs_type_selector) {
                                        ...
                                        print "<div id='{$vs_id_prefix}HierarchyBrowseAdd'>" . _t(
                                                "Add a new %1 %2 <em>%3</em>",
                                                $vs_type_selector,
                                                caHTMLSelect(
                                                    'add_type',
                                                    $va_add_types,
                                        ...
                                        print " <a href='#' onclick='_navigateToNewForm(jQuery(\"#{$vs_id_prefix}typeList\").val(), jQuery(\"#{$vs_id_prefix}addType\").val(), (jQuery(\"#{$vs_id_prefix}addType\").val() == \"next_to\") ? " . intval(
                                                $pn_parent_id
                                            ) . " : " . intval($pn_id) . "," . intval($pn_id) . ")'>" . caNavIcon(
                                                __CA_NAV_ICON_ADD__,
                                                '15px'
                                            ) . "</a></div>";
                                    } else {
                                        ...
                                        print " <a href='#' onclick='_navigateToNewForm(0, jQuery(\"#{$vs_id_prefix}addType\").val(), (jQuery(\"#{$vs_id_prefix}addType\").val() == \"next_to\") ? " . intval(
                                                $pn_parent_id
                                            ) . " : " . intval($pn_id) . ")'>" . caNavIcon(
                                                __CA_NAV_ICON_ADD__,
                                                '15px'
                                            ) . "</a></div>";
                                    }
                    ...
                    if ((!$vb_read_only && $vb_has_privs && !$vb_batch) && $vb_objects_x_collections_hierarchy_enabled && ($t_subject->tableName(
                        ...
                        <div id="<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-addObject"
                            ...
                            <div class="hierarchyBrowserMessageContainer">
                                <?php print _t(
                                    'Use the controls below to create new %1 relative to this %2 in the hierarchy.',
                                ...
                                <div style="float: left; width: 700px">
                                    ...
                                    $vs_type_selector = trim(
                                        $t_object->getTypeListAsHTMLFormElement(
                                            "{$vs_id_prefix}object_type_id",
                                            array('id' => "{$vs_id_prefix}objectTypeList"),
                                            array(
                                                'childrenOfCurrentTypeOnly' => $vb_strict_type_hierarchy,
                                                'includeSelf' => !$vb_strict_type_hierarchy,
                                                'directChildrenOnly' => $vb_strict_type_hierarchy
                                    ...
                                    print "<div id='{$vs_id_prefix}HierarchyBrowseAdd'>" . _t(
                                            "Add a new %1 under <em>%2</em>",
                                            $vs_type_selector,
                                            $vs_subject_label
                                        );

                                    // Note the jQuery(\"#{$vs_id_prefix}childTypeList\").val() which grabs the value of the type
                                    print " <a href='#' onclick='_navigateToNewObjectForm(jQuery(\"#{$vs_id_prefix}objectTypeList\").val(), " . intval(
                                            $pn_id
            ...
            <script type="text/javascript">
                jQuery(document).ready(function () {
                    ...
                    jQuery("#<?php print $vs_id_prefix; ?>browseToggle").click(function (e, opts) {
                        ...
                        jQuery("#<?php print $vs_id_prefix; ?>HierarchyBrowserContainer").slideToggle(delay, function () {
                            jQuery("#<?php print $vs_id_prefix; ?>browseToggle").html((this.style.display == 'block') ? '<?php print '<span class="form-button">' . _t(
                                    'Close browser'
                        ...
                        switch (action) {
                            ...
                            case 'next_to':
                                document.location = '<?php print caEditorUrl(
                                    ...
                                    $t_subject->tableName(),
                                    0
                                ); ?>/type_id/' + type_id + '/parent_id/' + id + '/after_id/' + after_id;
                                break;
                            default:
                ...
                function _init<?php print $vs_id_prefix; ?>ExploreHierarchyBrowser() {
                    if (!o<?php print $vs_id_prefix; ?>ExploreHierarchyBrowser) {
                        o<?php print $vs_id_prefix; ?>ExploreHierarchyBrowser = caUI.initHierBrowser('<?php print $vs_id_prefix; ?>ExploreHierarchyBrowser', {
                            ...
                            dontAllowEditForFirstLevel: <?php print (in_array(
                                $t_subject->tableName(),
                                array(
                                    'ca_places',
                            ...
                            autoShrink: <?php print (caGetOption(
                                'auto_shrink',
                                $pa_bundle_settings,
                                false
                            ) ? 'true' : 'false'); ?>,
                ...
                function _init<?php print $vs_id_prefix; ?>MoveHierarchyBrowser() {
                    if (!o<?php print $vs_id_prefix; ?>MoveHierarchyBrowser) {
                        o<?php print $vs_id_prefix; ?>MoveHierarchyBrowser = caUI.initHierBrowser('<?php print $vs_id_prefix; ?>MoveHierarchyBrowser', {
                            ...
                            currentSelectionDisplayFormat: '<?php print addslashes(
                                _t('Will be moved under <em>%1</em> after next save.')
                            ); ?>',

                            allowExtractionFromHierarchy: <?php print ($t_subject->getProperty(
                                    'HIERARCHY_TYPE'
                                ) == __CA_HIER_TYPE_ADHOC_MONO__) ? 'true' : 'false'; ?>,
                            extractFromHierarchyButtonIcon: "<?php print caNavIcon(__CA_NAV_ICON_EXTRACT__, 1); ?>",
                            extractFromHierarchyMessage: '<?php print addslashes(
                                _t('Will be placed at the top of its own hierarchy after next save.')
                            ); ?>',
                            ...
                            onSelection: function (id, parent_id, name, formattedDisplay) {
                                ...
                                if (id.substr(0, 10) == 'ca_objects') {
                                    formattedDisplay = '<?php print addslashes(
                                        _t("Cannot move collection under object")
                                    ); ?>';
                ...
                if ((!$vb_batch && !$vb_read_only && $vb_has_privs) && $vb_objects_x_collections_hierarchy_enabled && ($t_subject->tableName(
                    ) == 'ca_collections')) {
                ?>
