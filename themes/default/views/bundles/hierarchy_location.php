...
$vs_bundle_preview = '(' . $vn_items_in_hier . ') ' . caProcessTemplateForIDs( "^preferred_labels",
		$t_subject->tableName(), array( $t_subject->getPrimaryKey() ) );
...
$vb_objects_x_collections_hierarchy_enabled = (bool) $t_subject->getAppConfig()
                                                               ->get( 'ca_objects_x_collections_hierarchy_enabled' );
$vs_disabled_items_mode                     = $t_subject->getAppConfig()->get( $t_subject->tableName()
...
if ( $t_subject->getProperty( 'HIERARCHY_ID_FLD' )
     && ( $vn_hier_id
		= (int) $t_subject->get( $t_subject->getProperty( 'HIERARCHY_ID_FLD' ) ) )
) {
	$va_search_lookup_extra_params['currentHierarchyOnly'] = $vn_hier_id;
}
if ( in_array( $t_subject->tableName(), array( 'ca_objects', 'ca_collections' ) )
     && $vb_objects_x_collections_hierarchy_enabled
) {
	$va_lookup_urls_for_move           = $va_lookup_urls = array(
		'search'       => caNavUrl( $this->request, 'lookup', 'ObjectCollectionHierarchy', 'Get',
			$va_search_lookup_extra_params ),
	...
	$va_lookup_urls_for_move['search'] = caNavUrl( $this->request, 'lookup', 'ObjectCollectionHierarchy', 'Get',
		array_merge( $va_search_lookup_extra_params, [ 'currentHierarchyOnly' => null ] ) );
	...
	$va_lookup_urls          = caJSONLookupServiceUrl( $this->request, $t_subject->tableName(),
		$va_search_lookup_extra_params );
	$va_lookup_urls_for_move = caJSONLookupServiceUrl( $this->request, $t_subject->tableName(),
		array_merge( $va_search_lookup_extra_params, [ 'currentHierarchyOnly' => null ] ) );
...
$vs_type_selector      = trim( $t_subject->getTypeListAsHTMLFormElement( "{$vs_id_prefix}type_id",
	array( 'id' => "{$vs_id_prefix}typeList" ), array(
		'childrenOfCurrentTypeOnly' => (bool) $strict_type_hierarchy,
		'includeSelf'               => ! (bool) $strict_type_hierarchy,
		'directChildrenOnly'        => ( (bool) $strict_type_hierarchy && ( $strict_type_hierarchy !== '~' ) )
	) ) );

$pa_bundle_settings = $this->getVar( 'settings' );
$vb_read_only       = ( ( isset( $pa_bundle_settings['readonly'] ) && $pa_bundle_settings['readonly'] )
                        || ( $this->request->user->getBundleAccessLevel( $t_subject->tableName(), 'hierarchy_location' )
                             == __CA_BUNDLE_ACCESS_READONLY__ ) );

$show_add  = ( ! $vb_read_only && $vb_has_privs && ! $vb_batch )
             && ( ! $strict_type_hierarchy
                  || ( $strict_type_hierarchy
                       && $vs_type_selector ) );
$show_move = ( ( ! $strict_type_hierarchy || ( $strict_type_hierarchy === '~' ) || $vb_batch ) && ! $vb_read_only );

$show_add_object = ( ! $vb_read_only && $vb_has_privs && ! $vb_batch ) && $vb_objects_x_collections_hierarchy_enabled
                   && ( $t_subject->tableName() == 'ca_collections' )
                   && ( ( ( ! $strict_type_hierarchy || ( $strict_type_hierarchy === '~' ) ) )
                        || ( ( $strict_type_hierarchy && ( $strict_type_hierarchy !== '~' ) )
                             && ( ! sizeof( $t_subject->getTypeInstance()->get( 'ca_list_items.children.item_id',
				[ 'returnAsArray' => true ] ) ) ) ) );

$hier_browser_width  = ( $pdim = caParseElementDimension( caGetOption( 'width', $pa_bundle_settings, null ) ) )
	? $pdim['expression'] : null;
$hier_browser_height = ( $pdim = caParseElementDimension( caGetOption( 'height', $pa_bundle_settings, null ) ) )
	? $pdim['expression'] : null;
...
if ( is_array( $pa_ancestors ) && sizeof( $pa_ancestors ) > 0 ) {
	foreach ( $pa_ancestors as $vn_id => $va_item ) {
		$vs_item_id = $vb_do_objects_x_collections_hierarchy ? ( $va_item['table'] . '-' . $va_item['item_id'] )
			: $va_item['item_id'];
		if ( $vn_id === '' ) {
			$va_path[] = "<a href='#'>" . _t( 'New %1', $t_subject->getTypeName() ) . "</a>";
		} else {
			$vs_label = $va_item['label'];
			if ( ( $va_item['table'] != $t_subject->tableName() )
			     || ( $va_item['item_id']
			          && ( $va_item['item_id'] != $pn_id ) )
			) {
				$va_path[] = '<a href="' . caEditorUrl( $this->request, $va_item['table'], $va_item['item_id'] ) . '">'
				             . $vs_label . '</a>';
			} else {
				$vn_item_id = array_pop( explode( "-", $vs_item_id ) );
				$va_path[]
				            = "<a href='#' onclick='jQuery(\"#{$vs_id_prefix}HierarchyBrowserContainer\").slideDown(250); o{$vs_id_prefix}ExploreHierarchyBrowser.setUpHierarchy(\"{$vn_item_id}\"); return false;'>{$vs_label}</a>";
			}
		}
	}
		...
		if (jQuery('#<?php print $vs_id_prefix; ?>HierarchyHeaderContent').width() > jQuery('#<?php print $vs_id_prefix; ?>HierarchyPanelHeader').width()) {
			...
			jQuery('#<?php print $vs_id_prefix; ?>NextControl').click(function () {
				if ((parseInt(jQuery('#<?php print $vs_id_prefix; ?>HierarchyHeaderContent').css('left'))) > ((hierarchyHeaderContentWidth - hierarchyPanelHeaderWidth) * -1)) {
					...
					if (dl < ((hierarchyHeaderContentWidth - hierarchyPanelHeaderWidth) * -1)) {
						dl = ((hierarchyHeaderContentWidth - hierarchyPanelHeaderWidth) * -1);
					}
					jQuery('#<?php print $vs_id_prefix; ?>HierarchyHeaderContent').stop().animate({'left': dl + "px"}, {
						duration: 300, easing: 'swing', queue: false, complete: function () {
							if ((parseInt(jQuery('#<?php print $vs_id_prefix; ?>HierarchyHeaderContent').css('left'))) <= ((hierarchyHeaderContentWidth - hierarchyPanelHeaderWidth) * -1)) {
...
if ( $vb_batch ) {
	...
	print caEditorBundleShowHideControl( $this->request, $vs_id_prefix, $pa_bundle_settings, false,
		$vs_bundle_preview );
}
	...
	<div class="bundleContainer">
		...
		if ( ! $vb_batch ) {
			...
			<div class="hierNav">
				<?php
				if ( sizeof( $va_errors ) ) {
					print "<div class='formLabel'><span class='formLabelError'>" . join( '; ', $va_errors )
					      . "</span></div>\n";
				}
				...
				if ( ! $vb_batch && ( $pn_id > 0 ) ) {
					...
					<div class="hierarchyCountDisplay"><?php if ( $vn_items_in_hier > 0 ) {
							print _t( "Number of %1 in hierarchy: %2",
								caGetTableDisplayName( $t_subject->tableName(), true ), $vn_items_in_hier );
						} ?></div>
					<div class="buttonPosition">
						<a href="#" id="<?php print $vs_id_prefix; ?>browseToggle" class="form-button"><span
								class="form-button"><?php print _t( 'Show Hierarchy' ); ?></span></a>
					</div>
					<?php
				}

				print '<div id="' . $vs_id_prefix . 'HierarchyPanelHeader" class="hierarchyPanelHeader"><div id="'
				      . $vs_id_prefix . 'HierarchyHeaderContent" class="hierarchyHeaderContent">';
				...
				<div id="<?php print $vs_id_prefix; ?>HierarchyHeaderScrollButtons"
				     class="hierarchyHeaderScrollButtons">
					<div id="<?php print $vs_id_prefix; ?>NextPrevControls" class="nextPrevControls"><a href="#"
					                                                                                    id="<?php print $vs_id_prefix; ?>PrevControl"
					                                                                                    class="prevControl">&larr;</a>
						<a href="#" id="<?php print $vs_id_prefix; ?>NextControl" class="nextControl">&rarr;</a></div>
				</div>
			</div><!-- end hiernav -->
			<?php
		}
		if ( $pn_id > 0 ) {
			...
			<div id="<?php print $vs_id_prefix; ?>HierarchyBrowserContainer" class="editorHierarchyBrowserContainer">
				<div id="<?php print $vs_id_prefix; ?>HierarchyBrowserTabs">
					<ul>
						...
						if ( ! $vb_batch ) {
							?>
							<li><a href="#<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-explore"
							       onclick='_init<?php print $vs_id_prefix; ?>ExploreHierarchyBrowser();'><span><?php print _t( 'Explore' ); ?></span></a>
							</li>
							<?php
						}
						if ( $show_move ) {
							?>
							<li><a href="#<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-move"
							       onclick='_init<?php print $vs_id_prefix; ?>MoveHierarchyBrowser();'><span><?php print _t( 'Move' ); ?></span></a>
							</li>
							<?php
						}
						if ( $show_add ) {
							?>
							<li><a href="#<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-add"
							       onclick='_init<?php print $vs_id_prefix; ?>AddHierarchyBrowser();'><span><?php print ( $vb_objects_x_collections_hierarchy_enabled
							                                                                                              && ( $t_subject->tableName()
							                                                                                                   == 'ca_collections' ) )
											? _t( 'Add level' ) : _t( 'Add' ); ?></span></a></li>
							<?php
						}
						if ( $show_add_object ) {
							?>
							<li><a href="#<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-addObject"
							       onclick='_init<?php print $vs_id_prefix; ?>AddObjectHierarchyBrowser();'><span><?php print _t( 'Add object' ); ?></span></a>
							</li>
							<?php
						}
						?>
					...
					if ( ! $vb_batch ) {
						...
						<div id="<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-explore" class="hierarchyBrowseTab">
							<div class="hierarchyBrowserFind">
								<?php print _t( 'Find' ); ?>: <input type="text"
								                                     id="<?php print $vs_id_prefix; ?>ExploreHierarchyBrowserSearch"
								                                     name="search" value="" size="25"/>
							</div>
							<div class="hierarchyBrowserMessageContainer">
								<?php print _t( 'Click %1 names to explore. Click on an arrow icon to open a %1 for editing.',
									$t_subject->getProperty( 'NAME_SINGULAR' ) ); ?>
							</div>
							<div class="clear"><!-- empty --></div>
							<div id="<?php print $vs_id_prefix; ?>ExploreHierarchyBrowser"
							     class="hierarchyBrowserSmall" <?php print $hier_browser_dim_style; ?>>
					...
					if ( $show_move ) {
						...
						<div id="<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-move" class="hierarchyBrowseTab">
							<div class="hierarchyBrowserFind">
								<?php print _t( 'Find' ); ?>: <input type="text"
								                                     id="<?php print $vs_id_prefix; ?>MoveHierarchyBrowserSearch"
								                                     name="search" value="" size="25"/>
							</div>
							<div class="hierarchyBrowserMessageContainer">
								<?php print _t( 'Click on an arrow to choose the location to move this record under.',
									$t_subject->getProperty( 'NAME_SINGULAR' ) ); ?>
								<div id='<?php print $vs_id_prefix; ?>HierarchyBrowserSelectionMessage'
								     class='hierarchyBrowserNewLocationMessage'>
									<!-- Message specifying move destination is dynamically inserted here by ca.hierbrowser --></div>
							</div>
							<div class="clear"><!-- empty --></div>
							<div id="<?php print $vs_id_prefix; ?>MoveHierarchyBrowser"
							     class="hierarchyBrowserSmall" <?php print $hier_browser_dim_style; ?>>
								<!-- Content for hierarchy browser is dynamically inserted here by ca.hierbrowser -->
							</div><!-- end hierbrowser -->
							<?php
							if ( ( $t_subject->tableName() == 'ca_storage_locations' )
							     && (bool) $t_subject->getAppConfig()
							                         ->get( 'record_movement_information_when_moving_storage_location' )
							) {
								if ( $t_ui = ca_editor_uis::loadDefaultUI( 'ca_movements', $this->request, null,
									...
									//
									?>
									<div id="<?php print $vs_id_prefix; ?>StorageLocationMovementForm"
									     style="width: 98%; margin: 5px 0px 2px 6px;">
										<h3><?php print _t( 'Movement details' ); ?></h3>
										<?php
										$va_nav = $t_ui->getScreensAsNavConfigFragment( $this->request, null,
										...
										$va_form_elements
										            = $t_movement->getBundleFormHTMLForScreen( $va_nav['defaultScreen'],
											array(
												'request'  => $this->request,
												'formName' => $vs_id_prefix . 'StorageLocationMovementForm',
												'omit'     => array( 'ca_storage_locations' )
											) );
										print caHTMLHiddenInput( $vs_id_prefix . '_movement_screen',
											array( 'value' => $va_nav['defaultScreen'] ) );
										print caHTMLHiddenInput( $vs_id_prefix . '_movement_form_name',
											array( 'value' => $vs_id_prefix . 'StorageLocationMovementForm' ) );
					...
					if ( $show_add ) {
						...
						<div id="<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-add" class="hierarchyBrowseTab">
							<div class="hierarchyBrowserMessageContainer">
								<?php print _t( 'Use the controls below to create new %1 relative to this record in the hierarchy.',
									$t_subject->getProperty( 'NAME_PLURAL' ), $vs_subject_label ); ?>
							</div>
							...
							<div id='<?php print $vs_id_prefix; ?>HierarchyBrowseTypeMenu' style="margin-top: 15px;">
								<div style="float: left; width: 700px">
									...
									if ( $vs_type_selector ) {
										...
										print "<div id='{$vs_id_prefix}HierarchyBrowseAdd'>"
										      . _t( "Add a new %1 %2 <em>%3</em>", $vs_type_selector,
												caHTMLSelect( 'add_type', $va_add_types,
													array( 'id' => "{$vs_id_prefix}addType" ),
													[ 'value' => Session::getVar( 'default_hierarchy_add_mode' ) ] ),
												$vs_subject_label );

										// Note the jQuery(\"#{$vs_id_prefix}childTypeList\").val() which grabs the value of the type
										print " <a href='#' onclick='_navigateToNewForm(jQuery(\"#{$vs_id_prefix}typeList\").val(), jQuery(\"#{$vs_id_prefix}addType\").val(), (jQuery(\"#{$vs_id_prefix}addType\").val() == \"next_to\") ? "
										      . intval( $pn_parent_id ) . " : " . intval( $pn_id ) . ","
										      . intval( $pn_id ) . ")'>" . caNavIcon( __CA_NAV_ICON_ADD__, '15px' )
										      . "</a></div>";
									} else {
										// for items without types
										print "<div id='{$vs_id_prefix}HierarchyBrowseAdd'>"
										      . _t( "Add a new %1 %2 <em>%3</em>",
												$t_subject->getProperty( 'NAME_SINGULAR' ),
												caHTMLSelect( 'add_type', $va_add_types,
													array( 'id' => "{$vs_id_prefix}addType" ),
													[ 'value' => Session::getVar( 'default_hierarchy_add_mode' ) ] ),
												$vs_subject_label );
										print " <a href='#' onclick='_navigateToNewForm(0, jQuery(\"#{$vs_id_prefix}addType\").val(), (jQuery(\"#{$vs_id_prefix}addType\").val() == \"next_to\") ? "
										      . intval( $pn_parent_id ) . " : " . intval( $pn_id ) . ")'>"
										      . caNavIcon( __CA_NAV_ICON_ADD__, '15px' ) . "</a></div>";
									}
									?>
								</div>
							</div>
							<div class="clear"><!-- empty --></div>
							<div id="<?php print $vs_id_prefix; ?>AddHierarchyBrowser"
							     class="hierarchyBrowserSmall" <?php print $hier_browser_dim_style; ?>>
								<!-- Content for hierarchy browser is dynamically inserted here by ca.hierbrowser -->
							</div><!-- end hierbrowser -->
						</div>
					...
					if ( ( ! $vb_read_only && $vb_has_privs && ! $vb_batch )
					     && $vb_objects_x_collections_hierarchy_enabled
					     && ( $t_subject->tableName() == 'ca_collections' )
					) {
						?>
						<div id="<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-addObject"
						     class="hierarchyBrowseTab">
							<div class="hierarchyBrowserMessageContainer">
								<?php print _t( 'Use the controls below to create new %1 relative to this %2 in the hierarchy.',
									$t_object->getProperty( 'NAME_PLURAL' ),
									mb_strtolower( $t_subject->getTypeName() ) ); ?>
							</div>

							<div id='<?php print $vs_id_prefix; ?>AddObjectHierarchyBrowseTypeMenu'
							     style="margin-top: 15px;">
								<div style="float: left; width: 700px">
									<?php
									print "<div id='{$vs_id_prefix}HierarchyBrowseAdd'>"
									      . _t( "Add a new %1 under <em>%2</em>", $this->getVar( 'objectTypeList' ),
											$vs_subject_label );

									// Note the jQuery(\"#{$vs_id_prefix}childTypeList\").val() which grabs the value of the type
									print " <a href='#' onclick='_navigateToNewObjectForm(jQuery(\"#{$vs_id_prefix}objectTypeList\").val(), "
									      . intval( $pn_id ) . ")'>" . caNavIcon( __CA_NAV_ICON_ADD__, '15px' )
									      . "</a></div>";
									?>
								</div>
							...
							<div class="clear"><!-- empty --></div>
							<div id="<?php print $vs_id_prefix; ?>AddObjectHierarchyBrowser"
							     class="hierarchyBrowserSmall" <?php print $hier_browser_dim_style; ?>>
								<!-- Content for hierarchy browser is dynamically inserted here by ca.hierbrowser -->
							</div><!-- end hierbrowser -->
						</div>
			...
			<input type='hidden' name='<?php print $vs_id_prefix; ?>_new_parent_id'
			       id='<?php print $vs_id_prefix; ?>_new_parent_id' value='<?php print $pn_parent_id; ?>'/>
			...
			<script type="text/javascript">
				jQuery(document).ready(function () {
					...
					jQuery('#<?php print $vs_id_prefix; ?>MoveHierarchyBrowserSearch').autocomplete(
						{
							source: '<?php print $va_lookup_urls_for_move['search']; ?>',
							minLength: 3,
							delay: 800,
							html: true,
					...
					jQuery("#<?php print $vs_id_prefix; ?>browseToggle").click(function (e, opts) {
						...
						jQuery("#<?php print $vs_id_prefix; ?>HierarchyBrowserContainer").slideToggle(delay, function () {
							jQuery("#<?php print $vs_id_prefix; ?>browseToggle").html((this.style.display == 'block') ? '<?php print '<span class="form-button">'
							                                                                                                         . _t( 'Close browser' )
				...
				if (typeof _navigateToNewForm != 'function') {
					function _navigateToNewForm(type_id, action, id, after_id) {
						switch (action) {
							case 'above':
								document.location = '<?php print caEditorUrl( $this->request, $t_subject->tableName(),
									0 ); ?>/type_id/' + type_id + '/above_id/' + id;
								break;
							case 'under':
								document.location = '<?php print caEditorUrl( $this->request, $t_subject->tableName(),
									0 ); ?>/type_id/' + type_id + '/parent_id/' + id;
								break;
							case 'next_to':
								document.location = '<?php print caEditorUrl( $this->request, $t_subject->tableName(),
									0 ); ?>/type_id/' + type_id + '/parent_id/' + id + '/after_id/' + after_id;
								break;
							default:
				...
				if (typeof _navigateToNewObjectForm != 'function') {
					function _navigateToNewObjectForm(type_id, parent_collection_id) {
						document.location = '<?php print caEditorUrl( $this->request, "ca_objects",
							0 ); ?>/type_id/' + type_id + '/collection_id/' + parent_collection_id;
					}
				}
				...
				function _init<?php print $vs_id_prefix; ?>ExploreHierarchyBrowser() {
					if (!o<?php print $vs_id_prefix; ?>ExploreHierarchyBrowser) {
						o<?php print $vs_id_prefix; ?>ExploreHierarchyBrowser = caUI.initHierBrowser('<?php print $vs_id_prefix; ?>ExploreHierarchyBrowser', {
							...
							dontAllowEditForFirstLevel: <?php print ( in_array( $t_subject->tableName(),
								array( 'ca_places', 'ca_storage_locations', 'ca_list_items', 'ca_relationship_types' ) )
								? 'true' : 'false' ); ?>,
							...
							allowDragAndDropSorting: <?php print caDragAndDropSortingForHierarchyEnabled( $this->request,
								$t_subject->tableName(), $t_subject->getPrimaryKey() ) ? "true" : "false"; ?>,
							...
							autoShrink: <?php print ( caGetOption( 'auto_shrink', $pa_bundle_settings, false ) ? 'true'
								: 'false' ); ?>,
				...
				function _init<?php print $vs_id_prefix; ?>MoveHierarchyBrowser() {
					if (!o<?php print $vs_id_prefix; ?>MoveHierarchyBrowser) {
						o<?php print $vs_id_prefix; ?>MoveHierarchyBrowser = caUI.initHierBrowser('<?php print $vs_id_prefix; ?>MoveHierarchyBrowser', {
							...
							allowExtractionFromHierarchy: <?php print ( $t_subject->getProperty( 'HIERARCHY_TYPE' )
							                                            == __CA_HIER_TYPE_ADHOC_MONO__ ) ? 'true'
								: 'false'; ?>,
				...
				if (! $vb_batch && ( ! $vb_read_only && $vb_has_privs )
				    && ( ! $strict_type_hierarchy
				         || ( $strict_type_hierarchy
				              && $vs_type_selector ) )) {
				?>
				...
				if (( ! $vb_batch && ! $vb_read_only && $vb_has_privs ) && $vb_objects_x_collections_hierarchy_enabled
				    && ( $t_subject->tableName() == 'ca_collections' )) {
				?>
