		...
		<div class="bundleContainer <?php print $vb_batch ? "editorBatchBundleContent" : ''; ?>"
			...
			<div class="caItemList">
				<div class="labelInfo">
					...
					//
					if ( ( $vs_bundle_name == 'access' )
					     && (bool) $t_instance->getAppConfig()->get( $t_instance->tableName()
					                                                 . '_allow_access_inheritance' )
					     && $t_instance->hasField( 'access_inherit_from_parent' )
					     && ( $t_instance->get( 'parent_id' ) > 0 )
					) {
						print "<div class='inheritFromParent'>" . caHTMLCheckboxInput( $vs_id_prefix
						                                                               . 'access_inherit_from_parent',
								array( 'value' => 1, 'id' => $vs_id_prefix . 'access_inherit_from_parent' ), array() )
						...
						<script type="text/javascript">
							jQuery(document).ready(function () {
								...
								}).prop('checked', <?php print (bool) $t_instance->get( 'access_inherit_from_parent' )
									? 'true' : 'false'; ?>);
