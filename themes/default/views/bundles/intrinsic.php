...
if ($vb_batch) {
    ...
    if (!caGetOption('forACLAccessScreen', $va_settings, false)) {
        print caEditorBundleShowHideControl(
            ...
            $vs_id_prefix,
            $va_settings,
            caInitialValuesArrayHasValue($vs_id_prefix, $vs_bundle_preview)
        );

        ?>
        <script type="text/javascript">
            jQuery(document).ready(function () {
                jQuery('#' + '<?php print $vs_id_prefix; ?>' + '_BundleContentPreview').text(<?php print caEscapeForBundlePreview(
                    $vs_bundle_preview
                ); ?>);
            });
        </script>
...
<div>
    ...
    <div class="globalAccess">
        ...
        <p>
            ...
            ?>
        <div class="bundleContainer <?php print $vb_batch ? "editorBatchBundleContent" : ''; ?>"
             id="<?php print $vs_id_prefix; ?>">
            <div class="caItemList">
                <div class="labelInfo">
                    ...
                    //
                    if (($vs_bundle_name == 'access') && (bool)$t_instance->getAppConfig()->get(
                            $t_instance->tableName() . '_allow_access_inheritance'
                        ) && $t_instance->hasField('access_inherit_from_parent') && ($t_instance->get(
                                'parent_id'
                            ) > 0)) {
                        print "<div class='inheritFromParent'>" . caHTMLCheckboxInput(
                                $vs_id_prefix . 'access_inherit_from_parent',
                                array('value' => 1, 'id' => $vs_id_prefix . 'access_inherit_from_parent'),
                                array()
                            ) . ' ' . _t('Inherit from parent?') . "</div>";
                        ?>
                        <script type="text/javascript">
                            jQuery(document).ready(function () {
                                ...
                                }).prop('checked', <?php print (bool)$t_instance->get(
