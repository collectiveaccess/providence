...
if (($t_element->get(
            'datatype'
        ) == __CA_ATTRIBUTE_VALUE_CONTAINER__) && isset($va_element_settings['readonlyTemplate']) && (strlen(
            $va_element_settings['readonlyTemplate']
        ) > 0)) {
    $vb_is_read_only_for_existing_vals = true;

    $va_display_vals = array_shift(
        $t_instance->getAttributeDisplayValues($va_root_element['element_id'], $t_instance->getPrimaryKey())
    );
...
if ($t_instance->getAppConfig()->get('always_show_bundle_preview_for_attributes') || $vs_display_template) {
    $vs_bundle_preview = $vs_display_template ? $t_instance->getWithTemplate(
        $vs_display_template
    ) : $t_instance->getAttributesForDisplay($va_root_element['element_id'], null, array('showHierarchy' => true));
}
...
if (is_array($va_attribute_list) && sizeof($va_attribute_list)) {
    ...
    foreach ($va_attribute_list as $o_attr) {
        ...
        foreach ($o_attr->getValues() as $o_value) {
            ...
            if ($va_failed_updates[$vn_attr_id] && !in_array(
                    $o_value->getDatatype(),
                    array(
                ...
                $vs_display_val = $o_value->getDisplayValue(
                    array('request' => $this->request, 'includeID' => true, 'showMediaInfo' => true)
                );
            }
...
if (caGetOption('canMakePDF', $va_element_info[$t_element->getPrimaryKey()]['settings'], false)) {
    $va_template_list = caGetAvailablePrintTemplates(
        'bundles',
        array(
    ...
    if (sizeof($va_template_list) > 0) {
        ...
        <div class='iconButton'>
            <?php
            print (sizeof($va_template_list) > 1) ? caHTMLSelect(
                'template',
                $va_template_list,
                array(
                    'class' => 'dontTriggerUnsavedChangeWarning',
                    'id' => "{$vs_id_prefix}PrintTemplate"
                )
            ) : caHTMLHiddenInput(
                'template',
                array('value' => array_pop($va_template_list), 'id' => "{$vs_id_prefix}PrintTemplate")
            );
            print "<a href='#' onclick='{$vs_id_prefix}Print(); return false;'>" . caNavIcon(
                    __CA_NAV_ICON_PDF__,
                    1
                ) . "</a>";
            ?>
...
print (sizeof($va_template_list) > 1) ? caHTMLSelect(
    ...
    array(
        'class' => 'dontTriggerUnsavedChangeWarning',
        'id' => "{$vs_id_prefix}PrintTemplate{n}"
    )
) : caHTMLHiddenInput(
    'template',
...
if (isset($va_elements['_locale_id'])) {
    print ($va_elements['_locale_id']['hidden']) ? $va_elements['_locale_id']['element'] : '<div class="formLabel">' . _t(
            'Locale'
        ) . ' ' . $va_elements['_locale_id']['element'] . '</div>';
}
?>
		</div>
	</textarea>
...
<script type="text/javascript">
    ...
    function <?php print $vs_id_prefix; ?>Print(attribute_id) {
        ...
        window.location = '<?php print caNavUrl(
            $this->request,
            '*',
            '*',
            'PrintBundle',
