...
$vb_read_only        = ( ( isset( $va_settings['readonly'] ) && $va_settings['readonly'] )
                         || ( $this->request->user->getBundleAccessLevel( $this->getVar( 't_instance' )->tableName(),
			$this->getVar( 'element_code' ) ) == __CA_BUNDLE_ACCESS_READONLY__ ) );
...
if ( ( $t_element->get( 'datatype' ) == __CA_ATTRIBUTE_VALUE_CONTAINER__ )
     && isset( $va_element_settings['readonlyTemplate'] )
     && ( strlen( $va_element_settings['readonlyTemplate'] ) > 0 )
) {
	$vb_is_read_only_for_existing_vals = true;

	$va_display_vals      = array_shift( $t_instance->getAttributeDisplayValues( $va_root_element['element_id'],
	...
	if ( is_array( $va_display_vals ) ) {
		...
		foreach ( $va_display_vals as $vn_attr_id => $va_display_val ) {
			$vs_template
				                                 = "<unit relativeTo='{$t_instance->tableName()}.{$t_element->get('element_code')}' start='{$vn_i}' length='1'>{$va_element_settings['readonlyTemplate']}</unit>";
			$va_readonly_previews[ $vn_attr_id ] = caProcessTemplateForIDs( $vs_template, $t_instance->tableName(),
				array( $t_instance->getPrimaryKey() ) );
			$vn_i ++;
		}
	}
}
...
if ( $t_instance->getAppConfig()->get( 'always_show_bundle_preview_for_attributes' ) || $vs_display_template ) {
	$vs_bundle_preview = $vs_display_template ? $t_instance->getWithTemplate( $vs_display_template )
		: $t_instance->getAttributesForDisplay( $va_root_element['element_id'], null,
			array( 'showHierarchy' => true ) );
}
	...
	foreach ( $va_attribute_list as $o_attr ) {
		...
		foreach ( $o_attr->getValues() as $o_value ) {
			...
			if ( $va_failed_updates[ $vn_attr_id ]
			     && ! in_array( $o_value->getDatatype(), array(
				...
				$vs_display_val = $o_value->getDisplayValue( array(
			...
			if ( $attr_table && isset( $va_element_info[ $vn_element_id ] )
			     && ( ( isset( $va_element_info[ $vn_element_id ]['settings']['render'] )
			            && ( $va_element_info[ $vn_element_id ]['settings']['render'] == 'lookup' ) )
			          || $minimize_existing_values )
		...
		if ( is_array(
			...
			foreach ( $va_action_errors as $o_error ) {
				$va_errors[ $o_attr->getAttributeID() ][]
					= array(
	...
	if ( is_array( $va_item_ids ) && sizeof( $va_item_ids ) ) {
		foreach ( $va_item_ids as $attr_table => $item_ids_for_table ) {
			if ( $element_template = $t_element->getSetting( 'displayTemplate' ) ) {
				$va_labels = caProcessTemplateForIDs( $element_template, $attr_table, $item_ids_for_table,
			...
			foreach ( $va_initial_values as $vn_attr_id => $va_values ) {
				foreach ( $va_values as $vn_element_id => $vs_value ) {
					$va_initial_values[ $vn_attr_id ][ $vn_element_id . '_label' ]
						= $va_labels[ $va_initial_values[ $vn_attr_id ][ $vn_element_id ] ];
				}
			}
		}
	}
...
if ( $vb_batch ) {
	...
	print caEditorBundleShowHideControl( $this->request, $vs_id_prefix, $va_settings,
		caInitialValuesArrayHasValue( $vs_id_prefix, $va_initial_values ) );
}
print caEditorBundleMetadataDictionary( $this->request, $vs_id_prefix, $va_settings );


if ( caGetOption( 'canMakePDF', $va_element_info[ $root_element_id ]['settings'], false ) ) {
	$va_template_list = caGetAvailablePrintTemplates( 'bundles', array(
		'table'           => $t_instance->tableName(),
		'restrictToTypes' => $t_instance->getTypeID(),
		'elementCode'     => $t_element->get( 'element_code' ),
		'forHTMLSelect'   => true
	) );
	if ( sizeof( $va_template_list ) > 0 ) {
		...
		<div class='iconButton'>
			<?php
			print ( sizeof( $va_template_list ) > 1 ) ? caHTMLSelect( 'template', $va_template_list,
				array( 'class' => 'dontTriggerUnsavedChangeWarning', 'id' => "{$vs_id_prefix}PrintTemplate" ) )
				: caHTMLHiddenInput( 'template',
				array( 'value' => array_pop( $va_template_list ), 'id' => "{$vs_id_prefix}PrintTemplate" ) );
			print "<a href='#' onclick='{$vs_id_prefix}Print(); return false;'>" . caNavIcon( __CA_NAV_ICON_PDF__, 1 )
			      . "</a>";
			?>
		</div>
		<?php
...
if ( ( $vs_render_mode !== 'checklist' )
     && ! $vb_read_only
) {        // static (non-repeating) checkbox list for list attributes
	?>
...
<?php
print ( sizeof( $va_template_list ) > 1 ) ? caHTMLSelect( 'template', $va_template_list,
	array( 'class' => 'dontTriggerUnsavedChangeWarning', 'id' => "{$vs_id_prefix}PrintTemplate{n}" ) )
	: caHTMLHiddenInput( 'template',
	array( 'value' => array_pop( $va_template_list ), 'id' => "{$vs_id_prefix}PrintTemplate{n}" ) );
...
?>
	...
	if ( $minimize_existing_values ) {
		...
		<div id="<?php print $vs_id_prefix; ?>Item_{n}" class="labelInfo roundedRel caRelatedItem">
			...
			<span
				id='<?php print $vs_id_prefix; ?>_BundleTemplateDisplay{n}'>{<?php print "{$root_element_id}_label"; ?>}</span>
			<?php print caHTMLHiddenInput( $vs_id_prefix . '_' . $root_element_id . '_{n}',
				[ 'value' => '{' . $root_element_id . '}' ] ); ?>
			<?php
	...
	<div class="bundleContainer">
			...
			if ( $vb_is_read_only_for_existing_vals ) {
				...
				<div style="visibility: hidden; height: 0px;">
					...
					foreach ( $va_readonly_previews as $vn_attr_id => $vs_readonly_preview ) {
						?>
						<div class="caReadonlyContainer"
						     id="caReadonlyContainer<?php print $vs_id_prefix . '_' . $vn_attr_id; ?>">
							<a class="caReadonlyContainerEditLink"
							   id="caContainerEditLink<?php print $vs_id_prefix . '_' . $vn_attr_id; ?>"
							   href="#"><?php print _t( 'Edit' ); ?></a>
							<div class="caReadonlyContainerDisplay"><?php print $vs_readonly_preview; ?></div>
						</div>
						<script type="text/javascript">
							jQuery(document).ready(function () {
								jQuery("#caContainerEditLink<?php print $vs_id_prefix . '_'
								                                        . $vn_attr_id; ?>").click(function () {
									jQuery('#<?php print $vs_id_prefix; ?>Item_<?php print $vn_attr_id; ?>').show();
									jQuery('#caReadonlyContainer<?php print $vs_id_prefix . '_'
									                                        . $vn_attr_id; ?>').hide();
									jQuery('input[name="<?php print $vs_id_prefix . '_dont_save_'
									                                . $vn_attr_id; ?>"]').val('0');
								});
							});
						</script>
		...
		if ( ( $vs_render_mode !== 'checklist' ) && ! $vb_read_only ) {
			...
			<div class='button labelInfo caAddItemButton'><a href='#'><?php print caNavIcon( __CA_NAV_ICON_ADD__,
						"15px" ); ?><?php print $vs_add_label; ?></a></div>
			<?php
		}
		?>
	</div>
...
<script type="text/javascript">
	...
	caUI.initBundle('#<?php print $vs_id_prefix; ?>', {
		...
		itemID: '<?php print $vs_id_prefix; ?>Item_',
		templateClassName: 'caItemTemplate',
		initialValueTemplateClassName: '<?php print $minimize_existing_values ? 'caExistingItemTemplate'
			: 'caItemTemplate'; ?>',
		itemListClassName: 'caItemList',
	...
	function <?php print $vs_id_prefix; ?>Print(attribute_id) {
		...
		window.location = '<?php print caNavUrl( $this->request, '*', '*', 'PrintBundle', array(
			'element_code'            => $t_element->get( 'element_code' ),
			$t_instance->primaryKey() => $t_instance->getPrimaryKey()
		) ); ?>/template/' + template + '/attribute_id/' + attribute_id;
	}
</script>
