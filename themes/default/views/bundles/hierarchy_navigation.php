	...
	<div class="bundleContainer">
		<div class="hierNav">
			...
			if ( $pn_id > 0 ) {
				...
				<div class="buttonPosition" <?php print ( isset( $pa_bundle_settings['no_close_button'] )
				                                          && $pa_bundle_settings['no_close_button'] )
					? "style='display: none;'" : ""; ?>><a href="#" id="<?php print $vs_id_prefix; ?>browseToggle"
				                                           class="form-button"><span
							class="form-button"><?php print _t( 'Show in browser' ); ?></span></a></div>
				<?php
			}
			...
			if ( $vb_objects_x_collections_hierarchy_enabled
			...
			if ( is_array( $pa_ancestors ) && sizeof( $pa_ancestors ) > 0 ) {
				...
				foreach ( $pa_ancestors as $vn_id => $va_item ) {
					$vs_item_id = $vb_do_objects_x_collections_hierarchy ? ( $va_item['table'] . '-'
					                                                         . $va_item['item_id'] )
						: $va_item['item_id'];
					if ( $vn_id === '' ) {
						...
						if ( $pn_id && $va_item[ $t_subject->primaryKey() ] && ( $vs_item_id != $pn_id ) ) {
							$va_path[] = '<a href="' . caEditorUrl( $this->request, $t_subject->tableName(), $vn_id )
							             . '">' . $vs_label . '</a>';
						} else {
							$va_tmp     = explode( "-", $vs_item_id );
							$vn_item_id = array_pop( $va_tmp );
							$va_path[]
							            = "<a href='#' onclick='jQuery(\"#{$vs_id_prefix}HierarchyBrowserContainer\").slideDown(250); o{$vs_id_prefix}HierarchyBrowser.setUpHierarchy(\"{$vn_item_id}\"); return false;'>{$vs_label}</a>";
						}
					}
				}
				print join( ' ➔ ', $va_path );
	...
	if ( $pn_id > 0 ) {
		...
		<div id="<?php print $vs_id_prefix; ?>HierarchyBrowserContainer" class="editorHierarchyBrowserContainer">
			<div id="<?php print $vs_id_prefix; ?>HierarchyBrowserTabs">
				...
				<div id="<?php print $vs_id_prefix; ?>HierarchyBrowserTabs-explore"
				     class="<?php print ( isset( $pa_bundle_settings['hierarchy_browse_tab_class'] )
				                          && $pa_bundle_settings['hierarchy_browse_tab_class'] )
					     ? $pa_bundle_settings['hierarchy_browse_tab_class'] : "hierarchyBrowseTab"; ?>">
					...
					<div id="<?php print $vs_id_prefix; ?>HierarchyBrowser"
		...
		jQuery(document).ready(function () {
			o<?php print $vs_id_prefix; ?>HierarchyBrowser = caUI.initHierBrowser('<?php print $vs_id_prefix; ?>HierarchyBrowser', {
				...
				dontAllowEditForFirstLevel: <?php print ( in_array( $t_subject->tableName(),
					array( 'ca_places', 'ca_storage_locations', 'ca_list_items', 'ca_relationship_types' ) ) ? 'true'
					: 'false' ); ?>,
