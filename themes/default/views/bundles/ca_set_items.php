...
<div id="<?php print $vs_id_prefix; ?>" class='setItemEditor'>
	...
	if ( ! $vn_table_num ) {
		...
		if ( is_array( $va_items ) && sizeof( $va_items ) ) {
			print caGetPrintFormatsListAsHTMLForSetItemBundles( $vs_id_prefix, $this->request, $t_set,
				$t_set->getItemRowIDs() );
		}
		?>
		...
		<div id="<?php print $vs_id_prefix; ?>setItems" class="setItems">
			<div class="setEditorAddItemForm" id="<?php print $vs_id_prefix; ?>addItemForm">
				...
				<input type="text" size="70" name="setItemAutocompleter"
				       id="<?php print $vs_id_prefix; ?>setItemAutocompleter" class="lookupBg"/>
			</div>
			...
			<br style="clear: both;"/>
			<input type="hidden" id="<?php print $vs_id_prefix; ?>setRowIDList"
			       name="<?php print $vs_id_prefix; ?>setRowIDList" value=""/>
			...
			<script type="text/javascript">
				...
				jQuery(document).ready(function () {
					setEditorOps = caUI.seteditor({
						...
						displayTemplate: <?php print ( isset( $va_settings['displayTemplate'] )
							? json_encode( $va_settings['displayTemplate'] ) : 'null' ); ?>,

						editSetItemButton: '<?php print addslashes( caNavIcon( __CA_NAV_ICON_EDIT__, "20px" ) ); ?>',
						deleteSetItemButton: '<?php print addslashes( caNavIcon( __CA_NAV_ICON_DEL_BUNDLE__,
							"20px" ) ); ?>',

						lookupURL: '<?php print $va_lookup_urls['search']; ?>',
						itemInfoURL: '<?php print caNavUrl( $this->request, 'manage/sets', 'SetEditor',
							'GetItemInfo' ); ?>',
						editSetItemsURL: '<?php print caNavUrl( $this->request, 'manage/set_items', 'SetItemEditor',
							'Edit', array( 'set_id' => $vn_set_id ) ); ?>',
