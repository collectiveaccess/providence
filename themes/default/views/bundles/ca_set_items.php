...
<div id="<?php print $vs_id_prefix; ?>" class='setItemEditor'>
    ...
    if (!$vn_table_num) {
        ...
        if (is_array($va_items) && sizeof($va_items)) {
            print caGetPrintFormatsListAsHTMLForSetItemBundles(
                $vs_id_prefix,
                $this->request,
                $t_set,
                $t_set->getItemRowIDs()
            ...
            <script type="text/javascript">
                ...
                jQuery(document).ready(function () {
                    setEditorOps = caUI.seteditor({
                        ...
                        displayTemplate: <?php print (isset($va_settings['displayTemplate']) ? json_encode(
                            $va_settings['displayTemplate']
                        ) : 'null'); ?>,

                        editSetItemButton: '<?php print addslashes(caNavIcon(__CA_NAV_ICON_EDIT__, "20px")); ?>',
                        deleteSetItemButton: '<?php print addslashes(
                            caNavIcon(__CA_NAV_ICON_DEL_BUNDLE__, "20px")
                        ); ?>',

                        lookupURL: '<?php print $va_lookup_urls['search']; ?>',
                        itemInfoURL: '<?php print caNavUrl(
                            $this->request,
                            'manage/sets',
                            'SetEditor',
                            'GetItemInfo'
                        ); ?>',
                        editSetItemsURL: '<?php print caNavUrl(
                            $this->request,
                            'manage/set_items',
                            'SetItemEditor',
                            'Edit',
