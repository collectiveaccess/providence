...
$read_only		=	((isset($settings['readonly']) && $settings['readonly'])  || ($this->request->user->getBundleAccessLevel('ca_set_items', 'nonpreferred_labels') == __CA_BUNDLE_ACCESS_READONLY__));
...
?>
	<textarea class='caLabelTemplate' style='display: none;'>
		<div id="{fieldNamePrefix}Label_{n}" class="labelInfo">
			...
			<?= $t_label->htmlFormElement('name', "^ELEMENT", array_merge($settings, array('caption' => "{fieldNamePrefix}caption_{n}", 'id' => "{fieldNamePrefix}caption_{n}", "value" => "{{caption}}", 'no_tooltips' => true, 'textAreaTagName' => 'textentry', 'readonly' => $read_only))); ?>
			<br/>
...
<script type="text/javascript">
	caUI.initLabelBundle('#<?= $id_prefix; ?>NPLabels', {
		...
		bundlePreview: <?php $cur = current($initial_values); print caEscapeForBundlePreview($cur['caption']); ?>,
