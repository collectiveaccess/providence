        ...
        <div class="caItemList">
            <div id="<?php print $vs_id_prefix; ?>Container" class="editorHierarchyBrowserContainer">
                <div id="<?php print $vs_id_prefix; ?>Tabs">
                    <ul>
                        <li><a href="#<?php print $vs_id_prefix; ?>Tabs-status"><span><?php print _t(
                                        'Current status'
                                    ); ?></span></a></li>
                        <li><a href="#<?php print $vs_id_prefix; ?>Tabs-history"><span><?php print _t(
                                        'History'
                                    ); ?></span></a></li>
                    </ul>
                    <div id="<?php print $vs_id_prefix; ?>Tabs-status" class="hierarchyBrowseTab">
                        <?php
                        if ($t_subject->canBeCheckedOut() && ($va_checkout_status = $t_subject->getCheckoutStatus(
                            ...
                            switch ($vn_status = $va_checkout_status['status']) {
                                ...
                                case __CA_OBJECTS_CHECKOUT_STATUS_RESERVED__:
                                    if (in_array(
                                        ...
                                        print _t(
                                                '<strong>Borrowed by:</strong> %1 on %2',
                                                $va_checkout_status['user_name'],
                                                $va_checkout_status['checkout_date']
                                            ) . "<br/>\n";
                                        if ($va_checkout_status['due_date']) {
                                            print _t(
                                                    '<strong>Due on:</strong> %1',
                                                    $va_checkout_status['due_date']
                                                ) . "<br/>\n";
                                        }
                                        if ($va_checkout_status['checkout_notes']) {
                                            print _t(
                                                    '<strong>Notes:</strong> %1',
                                                    $va_checkout_status['checkout_notes']
                                                ) . "<br/>\n";
                                        }
                                    }
                                    if (in_array(
                    ...
                    <div id="<?php print $vs_id_prefix; ?>Tabs-history" class="hierarchyBrowseTab caLocationHistoryTab">
                        <h2>
                            <?php print ($vn_checkout_count != 1) ? _t('Checked out %1 times', $vn_checkout_count) : _t(
                                'Checked out %1 time',
                                $vn_checkout_count
                            ); ?>
                            <?php print ($vn_client_count != 1) ? _t('by %1 clients', $vn_client_count) : _t(
                                'by %1 client',
                                $vn_client_count
                            ); ?>
                        </h2>
