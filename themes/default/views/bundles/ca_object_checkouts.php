		...
		<div class="caItemList">
			<div id="<?php print $vs_id_prefix; ?>Container" class="editorHierarchyBrowserContainer">
				<div id="<?php print $vs_id_prefix; ?>Tabs">
					<ul>
						<li>
							<a href="#<?php print $vs_id_prefix; ?>Tabs-status"><span><?php print _t( 'Current status' ); ?></span></a>
						</li>
						<li>
					...
					<div id="<?php print $vs_id_prefix; ?>Tabs-status" class="hierarchyBrowseTab">
						<?php
						if ( $t_subject->canBeCheckedOut()
						     && ( $va_checkout_status
								= $t_subject->getCheckoutStatus( array( 'returnAsArray' => true ) ) )
						) {

							print _t( '<strong>Status:</strong> %1', $va_checkout_status['status_display'] )
							      . "<br/>\n";
							...
							switch ( $vn_status = $va_checkout_status['status'] ) {
								...
								case __CA_OBJECTS_CHECKOUT_STATUS_RESERVED__:
									if ( in_array( $vn_status, array(
										__CA_OBJECTS_CHECKOUT_STATUS_OUT__,
										__CA_OBJECTS_CHECKOUT_STATUS_OUT_WITH_RESERVATIONS__
									) )
									) {
										print _t( '<strong>Borrowed by:</strong> %1 on %2',
												$va_checkout_status['user_name'], $va_checkout_status['checkout_date'] )
										      . "<br/>\n";
										if ( $va_checkout_status['due_date'] ) {
											print _t( '<strong>Due on:</strong> %1', $va_checkout_status['due_date'] )
											      . "<br/>\n";
										}
										if ( $va_checkout_status['checkout_notes'] ) {
											print _t( '<strong>Notes:</strong> %1',
													$va_checkout_status['checkout_notes'] ) . "<br/>\n";
										}
									}
									if ( in_array( $vn_status, array(
										__CA_OBJECTS_CHECKOUT_STATUS_RESERVED__,
										__CA_OBJECTS_CHECKOUT_STATUS_OUT_WITH_RESERVATIONS__
									) )
									) {
										$va_reservations = $t_subject->getCheckoutReservations();
										print _t( "<strong>Reservations:</strong> %1",
												$vn_num_reservations = sizeof( $va_reservations ) ) . "<br/>\n";
										if ( $vn_num_reservations > 0 ) {
											...
											print _t( "<strong>Reserved for:</strong> %1",
												join( ", ", $va_reservation_users ) );
										}
									}
									break;
					...
					<div id="<?php print $vs_id_prefix; ?>Tabs-history" class="hierarchyBrowseTab caLocationHistoryTab">
						<h2>
							<?php print ( $vn_checkout_count != 1 ) ? _t( 'Checked out %1 times', $vn_checkout_count )
								: _t( 'Checked out %1 time', $vn_checkout_count ); ?>
							<?php print ( $vn_client_count != 1 ) ? _t( 'by %1 clients', $vn_client_count )
								: _t( 'by %1 client', $vn_client_count ); ?>
						</h2>
						<table class='caLibraryHistory'>
							...
							foreach ( $va_history as $va_event ) {
								print "<tr class='caLibraryHistory'><td class='caLibraryHistory'>"
								      . $va_event['user_name'] . "</td><td class='caLibraryHistory'>"
								      . $va_event['checkout_date'] . "</td><td class='caLibraryHistory'>"
								      . caHTMLTextInput( "{$vs_id_prefix}checkout_notes_" . $va_event['checkout_id'],
										[ 'value' => $va_event['checkout_notes'] ],
										[ 'width' => '175px', 'height' => '50px' ] )
								      . "</td><td class='caLibraryHistory'>" . $va_event['due_date'] . "</td><td>"
								      . $va_event['return_date'] . "</td><td class='caLibraryHistory'>"
								      . caHTMLTextInput( "{$vs_id_prefix}return_notes_" . $va_event['checkout_id'],
										[ 'value' => $va_event['return_notes'] ],
										[ 'width' => '175px', 'height' => '50px' ] ) . "</td></td></tr>\n";
							}
							?>
							</tbody>
						</table>
