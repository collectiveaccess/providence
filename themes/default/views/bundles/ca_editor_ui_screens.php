				...
				<tr>
					<td width="200">
						<div class="formLabel" id="{fieldNamePrefix}edit_name_{n}" style="display: none;">
							<?php print _t( "Name" ) . "<br/>" . caHTMLTextInput( '{fieldNamePrefix}name_{n}',
									array( 'id' => '{fieldNamePrefix}name_{n}', 'value' => '{name}' ),
									array( 'width' => 40 ) ); ?>
						</div>
					...
					<td class="listtableEditDelete"><div style="float:right;">
							<span id="{fieldNamePrefix}edit_{n}"><?php print urldecode( caNavLink( $this->request,
									caNavIcon( __CA_NAV_ICON_EDIT__, "18px" ), '',
									'administrate/setup/interface_screen_editor', 'InterfaceScreenEditor', 'Edit',
									array( 'screen_id' => '{screen_id}' ) ) ); ?></span>
							<a href="#" class="caDeleteScreenButton"><?php print caNavIcon( __CA_NAV_ICON_DEL_BUNDLE__,
									"18px" ); ?></a>
					</div></td>
				</tr>
			</table>
...
?>
	...
	<div class="bundleContainer">
		...
		<div class='button labelInfo caAddItemButton'><a href='#'><?php print caNavIcon( __CA_NAV_ICON_ADD__,
					'15px' ); ?> <?php print _t( "Add screen" ); ?> &rsaquo;</a></div>
	</div>
</div>

<input type="hidden" id="<?php print $vs_id_prefix; ?>_ScreenBundleList"
       name="<?php print $vs_id_prefix; ?>_ScreenBundleList" value=""/>
<?php
// order element
?>
...
<script type="text/javascript">
	caUI.initBundle('#<?php print $vs_id_prefix; ?>', {
		...
		initialValueOrder: <?php print json_encode( is_array( $va_initial_values ) ? array_keys( $va_initial_values )
			: null ); ?>,
