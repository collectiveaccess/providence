...
$read_only		= (caGetOption('readonly', $settings, false)  || ($this->request->user->getBundleAccessLevel($t_subject->tableName(), 'ca_collections') == __CA_BUNDLE_ACCESS_READONLY__));
...
<?php
	...
	if(is_array($this->getVar('initialValues')) && sizeof($this->getVar('initialValues'))) {
		...
		if(caGetOption('showReturnToHomeLocations', $settings, false) && caHomeLocationsEnabled('ca_collections', null, ['enableIfAnyTypeSet' => true])) {
			print caReturnToHomeLocationControlForRelatedBundle($this->request, $id_prefix, $t_subject, $this->getVar('history_tracking_policy'), $this->getVar('initialValues'));
		}
...
?>			<a href="#" class="caInterstitialEditButton listRelEditButton" aria-label="<?= htmlspecialchars(_t('Edit relationship data')); ?>"><?= caNavIcon(__CA_NAV_ICON_INTERSTITIAL_EDIT_BUNDLE__, "16px"); ?></a><?php
	}
...
<?php
			print caGetRelationDisplayString($this->request, 'ca_collections', array('class' => 'caEditItemButton', 'id' => "{$id_prefix}_edit_related_{n}", "aria-label" => _t('Edit')), array('display' => '_display', 'makeLink' => $make_link, 'prefix' => $id_prefix, 'relationshipTypeDisplayPosition' => $dont_show_relationship_type));
?>
			</span>
				...
				<script type='text/javascript'>
					jQuery(document).ready(function() { 
						var <?= $id_prefix; ?>oHierBrowser{n} = caUI.initHierBrowser('<?= $id_prefix; ?>_hierarchyBrowser{n}', {
							uiStyle: 'horizontal',
							levelDataUrl: '<?= caNavUrl($this->request, 'lookup', 'Collection', 'GetHierarchyLevel', []); ?>',
							initDataUrl: '<?= caNavUrl($this->request, 'lookup', 'Collection', 'GetHierarchyAncestorList'); ?>',
							
							bundle: '<?= $id_prefix; ?>',
							
							selectOnLoad : true,
							browserWidth: <?= json_encode($settings['hierarchicalBrowserWidth']); ?>,
						...
						jQuery('#<?= $id_prefix; ?>_hierarchyBrowserSearch{n}').autocomplete(
							{
								source: '<?= caNavUrl($this->request, 'lookup', 'Collection', 'Get', ['noInline' => 1]); ?>', 
								minLength: <?= (int)$t_subject->getAppConfig()->get(["ca_collections_autocomplete_minimum_search_length", "autocomplete_minimum_search_length"]); ?>, delay: 800, html: true,
...
<?php if($quick_add_enabled) { ?>
			...
			quickaddUrl: '<?= caNavUrl($this->request, 'editor/collections', 'CollectionQuickAdd', 'Form', array('collection_id' => 0, 'placement_id' => $placement_id, 'source' => $t_subject->tableName(), 'source_id' => $t_subject->getPrimaryKey(), 'dont_include_subtypes_in_type_restriction' => (int)($settings['dont_include_subtypes_in_type_restriction'] ?? 0))); ?>',
...
<?php } ?>
			...
			partialLoadUrl: '<?= caNavUrl($this->request, '*', '*', 'loadBundleValues', array($t_subject->primaryKey() => $t_subject->getPrimaryKey(), 'placement_id' => $placement_id, 'bundle' => 'ca_collections')); ?>',
