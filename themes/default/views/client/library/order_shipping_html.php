 ...
 */
	...
	print caFormTag($this->request, 'SaveShipping', 'caClientLoanShippingForm', null, 'post', 'multipart/form-data', '_top', array());
?>
...
?>
		<h2><?php print _t('No shipping is required for this loan'); ?></h2>
<?php
	}
?>
</form>
...
<script type="text/javascript">
	...
	function caUpdateFormAvailability() {
		var available = true;
		if (jQuery('#caClientLoanShippingForm #shipping_method').val() == 'NONE') {
			available = false;
		}
		jQuery('#caClientLoanShippingForm input').attr('readonly', !available);
		jQuery('#caClientLoanShippingForm textarea').attr('readonly', !available);
