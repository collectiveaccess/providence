...
if ( sizeof( $group_links ) > 1 ) {
	...
	<div class="statisticsDashboardGroupList">
		<?php
		print ( $all_link = $this->getVar( 'all_link' ) ) ? _t( "View statistics for group: %1 or %2",
			join( ", ", $group_links ), $all_link ) : _t( "View statistics for group: %1", join( ", ", $group_links ) );
		?>
	</div>
	<?php
}
...
if ( sizeof( $data ) ) {
	foreach ( $panels as $panel => $panel_options ) {
		print "<div class='statisticsDashboardPanel'>" . StatisticsDashboard::renderPanel( $this->request, $panel,
				$data, $panel_options ) . "</div>";
	}
}
