	...
	<div id="footer">
		<div style="position: relative;">
			...
			if ( $this->request->isLoggedIn() ) {
				print _p( "User" ) . ': ' . $this->request->user->getName() . ' &gt; ' . caNavLink( $this->request,
						_t( 'Preferences' ), '', 'system', 'Preferences', 'EditUIPrefs' ) . ' &gt; '
				      . caNavLink( $this->request, _t( 'Logout' ), '', 'system', 'auth', 'logout' );
			} else {
				print caNavLink( $this->request, _t( 'Login' ), '', 'system', 'auth', 'login' );
			}
			?>
			&nbsp;&nbsp;|&nbsp;&nbsp; &copy; 2020 Whirl-i-Gig, <a href="http://www.collectiveaccess.org"
			                                                      target="_blank">CollectiveAccess</a> <?php _p( "is a trademark of" ); ?>
			<a href="http://www.whirl-i-gig.com" target="_blank">Whirl-i-Gig</a>
			[<?php print Session::elapsedTime( 4 ) . 's'; ?>/<?php print caGetMemoryUsage(); ?>]
			<?php if ( Db::$monitor ) {
				print " [<a href='#' onclick='jQuery(\"#caApplicationMonitor\").slideToggle(100); return false;'>$</a>]";
			} ?>
		</div>
	</div><!-- end footer -->
...
<?php
if ( Db::$monitor ) {
	print $this->render( 'system/monitor_html.php' );
}
?>
</body>
</html>
