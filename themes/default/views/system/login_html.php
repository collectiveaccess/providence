	...
	<div id="loginBox">
		...
		<div id="loginForm">
			...
			<input name="redirect" type="hidden"
			       value="<?php echo htmlspecialchars( $this->getVar( 'redirect' ), ENT_QUOTES ); ?>"/>
			...
			<div class="loginSubmitButton"><?php print caFormSubmitButton( $this->request, __CA_NAV_ICON_LOGIN__,
					_t( "Login" ), "login", array( 'icon_position' => __CA_NAV_ICON_ICON_POS_RIGHT__ ) ); ?></div>
			</form>
			<?php if ( AuthenticationManager::supports( __CA_AUTH_ADAPTER_FEATURE_RESET_PASSWORDS__ ) ) { ?>
				<div id="forgotLink"><?php print caNavLink( $this->request, _t( "Forgot your password?" ), 'forgotLink',
						'system/auth', 'forgot', '' ); ?></div>
			<?php } else if ( $vs_adapter_account_link = AuthenticationManager::getAccountManagementLink() ) { ?>
				<div id="forgotLink"><a href="<?php print $vs_adapter_account_link; ?>"
				                        target="_blank"><?php print _t( "Manage your account" ); ?></a></div>
			<?php } ?>
		</div><!-- end loginForm -->
	</div><!-- end loginBox -->
</div><!-- end center -->
