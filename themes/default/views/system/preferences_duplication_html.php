...
<div class="sectionBox">
	...
	print $vs_control_box = caFormControlBox(
		...
		caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Reset" ), '', 'system', 'Preferences',
			$this->request->getAction() . "/" . $this->request->getActionExtra(), array() ),
		'',
		''
	);
	...
	if ( caTableIsActive( $vs_current_table )
	     && $this->request->user->canDoAction( 'can_duplicate_' . $vs_current_table )
	) {
		...
		if ( in_array( "duplicate_relationships", $va_prefs ) ) {
			print $t_user->preferenceHtmlFormElement( "{$vs_current_table}_duplicate_relationships", null,
				array( 'useTable' => true, 'numTableColumns' => 3 ) );
		}
		if ( in_array( "duplicate_children", $va_prefs ) ) {
			print "<br/>" . $t_user->preferenceHtmlFormElement( "{$vs_current_table}_duplicate_children", null,
					array( 'useTable' => true, 'numTableColumns' => 3 ) );
		}
		...
		if ( $t_user->isValidPreference( $vs_current_table . '_duplicate_element_settings' ) ) {
			...
			print "<thead><tr><th>" . _t( 'Metadata element' )
			      . "</th><th><a href='#' class='preferenceColumnSelector' id='duplicationOn'>" . _t( 'Duplicate' )
			      . "</a></th><th><a href='#' class='preferenceColumnSelector' id='duplicationOff'>"
			      . _t( 'Do not duplicate' ) . "</a></th></tr></thead>\n";
			...
			foreach ( $va_bundle_list as $vs_bundle_name => $va_info ) {
				...
				print "<td>" . caHTMLRadioButtonInput( "duplicate_element_settings[{$vs_bundle_name}]",
						array( 'value' => 1, 'class' => "duplication_setting_on" ),
						array( 'checked' => ( $vn_duplication_setting == 1 ), "disabled" => ! $vb_duplicate_metadata ) )
				      . "</td>\n";
				print "<td>" . caHTMLRadioButtonInput( "duplicate_element_settings[{$vs_bundle_name}]",
						array( 'value' => 0, 'class' => "duplication_setting_off" ),
						array( 'checked' => ( $vn_duplication_setting == 0 ), "disabled" => ! $vb_duplicate_metadata ) )
				      . "</td>\n";
				print "</tr>\n";
			}
			print "<tbody>";
...
<script type="text/javascript">
	jQuery(document).ready(function () {
		jQuery("#duplicationOn").on('click', function (e) {
			jQuery(".duplication_setting_on").prop("checked", 1);
			return false;
		});
		jQuery("#duplicationOff").on('click', function (e) {
			jQuery(".duplication_setting_off").prop("checked", 1);
			return false;
		});
