...
<div id="browse">
    <script type="text/javascript">
        var caUIBrowsePanel = caUI.initBrowsePanel({
            useStaticDiv: true,
            facetUrl: '<?php print caNavUrl(
                $this->request,
                $this->request->getModulePath(),
                $this->request->getController(),
                'getFacet'
            ); ?>',
            addCriteriaUrl: '<?php print caNavUrl(
                $this->request,
                $this->request->getModulePath(),
                $this->request->getController(),
                'addCriteria'
        ...
        if (sizeof($va_criteria)) {
            ...
            if (sizeof($va_facets)) {
                ...
                <div id="refineBrowse"><span class='refineHeading'><?php print _t('Refine results by'); ?>:</span>
                    ...
                    foreach ($va_available_facets as $vs_facet_code => $va_facet_info) {
                        ...
                        print "<a href='#' class='facetLink facet{$vs_facet_code}' onclick='caUIBrowsePanel.showBrowsePanel(\"{$vs_facet_code}\"); $(\".facetLink\").removeClass(\"active\"); $(\".facet{$vs_facet_code}\").addClass(\"active\");' " . $vs_style . ">" . caNavIcon(
                                __CA_NAV_ICON_ADD__,
                                1
            ...
            foreach ($va_criteria as $vs_facet_name => $va_row_ids) {
                ...
                foreach ($va_row_ids as $vn_row_id => $vs_label) {
                    print "<div class='criteriaLink'>{$vs_facet_label}: {$vs_label}" . caNavLink(
                            $this->request,
                            'x',
                            'close',
                            $this->request->getModulePath(),
            ...
            if (sizeof($va_facets)) {
                ...
                foreach ($va_available_facets as $vs_facet_code => $va_facet_info) {
                    ...
                    print "<div class='facetHeadingLink facet{$vs_facet_code}' " . $vs_style . "><a href='#' onclick='caUIBrowsePanel.showBrowsePanel(\"{$vs_facet_code}\"); $(\".facetHeadingLink\").removeClass(\"active\"); $(\".facet{$vs_facet_code}\").addClass(\"active\");'>" . caNavIcon(
                            __CA_NAV_ICON_ADD__,
                            1
                        ) . " " . $va_facet_info['label_plural'] . "</a></div>\n";
