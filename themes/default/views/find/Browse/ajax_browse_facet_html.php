...
$va_service_urls = caJSONLookupServiceUrl( $this->request, $va_facet_info['table'],
	array( 'noInline' => 1, 'noSymbols' => 1 ) );
...
<script type="text/javascript">
	function caUpdateFacetDisplay(grouping) {
		caUIBrowsePanel.showBrowsePanel('<?php print $vs_facet_name; ?>', <?php print ( ( intval( $vm_modify_id ) > 0 )
			? 'true' : 'false' ); ?>, <?php print ( ( intval( $vm_modify_id ) > 0 ) ? $vm_modify_id
			: 'null' ); ?>, grouping);
	}
</script>

<div class="browseSelectPanelContentArea <?php print ( $vb_multiple_selection_facet )
	? "browseSelectMultiplePanelContentArea" : "" ?>" id="browseSelectPanelContentArea">
	<?php
	if ( $vb_multiple_selection_facet ) {
		?>
		<div class='applyFacetContainer'><a href="#" id="facet_apply" data-facet="<?php print $vs_facet_name; ?>"
		                                    class="facetApply">Apply</a></div>
		<?php
	}
			...
			<div id="<?php print $vs_facet_name; ?>_facet_container">
				...
				if ( $t_item && $t_subject ) {
					...
					<div class="hierarchyBrowserHelpText">
						<?php print _t( "Click on a %1 to find %2 related to it. Click on the arrow next to a %3 to see more specific %4 within that %5, or use the search field.",
							$t_item->getProperty( 'NAME_SINGULAR' ), $t_subject->getProperty( 'NAME_PLURAL' ),
		...
		<script type="text/javascript">
			...
			jQuery(document).ready(function () {
				...
				oHierBrowser = caUI.initHierBrowser('hierarchyBrowser', {
					levelDataUrl: '<?php print caNavUrl( $this->request, $this->request->getModulePath(),
						$this->request->getController(), 'getFacetHierarchyLevel',
						array( 'facet' => $vs_facet_name ) ); ?>',
					initDataUrl: '<?php print caNavUrl( $this->request, $this->request->getModulePath(),
						$this->request->getController(), 'getFacetHierarchyAncestorList',
						array( 'facet' => $vs_facet_name ) ); ?>',

					editUrl: '<?php print caNavUrl( $this->request, $this->request->getModulePath(),
						$this->request->getController(), 'addCriteria',
						array( 'facet' => $vs_facet_name, 'id' => '' ) ); ?>',
		...
		<div class="browseSelectPanelList">
			<table class='browseSelectPanelListTable' id='<?php print $vs_facet_name; ?>_facet_container'>
				...
				foreach ( $va_facet as $vn_i => $va_item ) {
					...
					$vs_content_count = ( isset( $va_item['content_count'] ) && ( $va_item['content_count'] > 0 ) )
						? " (" . $va_item['content_count'] . ")" : "";
					$va_row[]
					                  = "<td class='browseSelectPanelListCell facetItem' width='{$va_td_width}%;' data-facet_item_id='{$va_item['id']}'>"
					                    . caNavLink( $this->request, html_entity_decode( $vs_label ),
							'browseSelectPanelLink', 'find', $this->request->getController(),
							( ( strlen( $vm_modify_id ) ) ? 'modifyCriteria' : 'addCriteria' ), array(
								'facet'  => $vs_facet_name,
								'id'     => urlencode( $va_item['id'] ),
								'mod_id' => $vm_modify_id
							) ) . "{$vs_content_count}</td>";
	...
	<div class="browseSelectPanelHeader">
		...
		foreach ( $va_groups as $vs_group ) {
			if ( $vb_individual_group_display ) {
				print " <a href='#' onclick='loadFacetGroup(\"" . ( ( $vs_group === '~' ) ? '~' : $vs_group )
				      . "\"); return false;' " . ( ( $vs_g == $vs_group )
						? "class='browseSelectPanelFacetGroupSelected'" : "class='browseSelectPanelFacetGroup'" )
				      . ">{$vs_group}</a> ";
			} else {
	...
	<div style="float: right; clear:right;" id='browseFacetGroupingControls'>
		...
		if ( isset( $va_facet_info['groupings'] ) && is_array( $va_facet_info['groupings'] )
			...
			foreach ( $va_facet_info['groupings'] as $vs_grouping => $vs_grouping_label ) {
				print "<a href='#' onclick='caUpdateFacetDisplay(\"{$vs_grouping}\");' style='" . ( ( $vs_grouping
				                                                                                      == $vs_grouping_field )
						? 'color:#333; text-decoration:underline;' : '' ) . "'>{$vs_grouping_label}</a> ";
			}
		}
		?>
	</div>
					...
					foreach ( $va_items as $va_item ) {
						...
						$va_row[]
						                  = "<td class='browseSelectPanelListCell facetItem' width='{$va_td_width}%;' data-facet_item_id='{$va_item['id']}'>"
						                    . caNavLink( $this->request, html_entity_decode( $vs_label ),
								'browseSelectPanelLink', 'find', $this->request->getController(),
								( ( strlen( $vm_modify_id ) > 0 ) ? 'modifyCriteria' : 'addCriteria' ), array(
									'facet'  => $vs_facet_name,
									'id'     => urlencode( $va_item['id'] ),
									'mod_id' => $vm_modify_id
								) ) . " {$vs_content_count}</td>";
...
<script type="text/javascript">
	function loadFacetGroup(g) {
		jQuery('#browseSelectPanelContentArea').parent().load("<?php print caNavUrl( $this->request,
			$this->request->getModulePath(), $this->request->getController(), 'getFacet', array(
				'facet'      => $vs_facet_name,
				'grouping'   => $this->getVar( 'grouping' ),
				'show_group' => ''
			) ); ?>" + escape(g));
	}
	...
	jQuery(document).ready(function () {
		...
		jQuery(".facetApply").on('click', function (e) {
			...
			if (ids.length) {
				window.location = '<?php print caNavUrl( $this->request, 'find', $this->request->getController(),
					( ( strlen( $vm_modify_id ) ) ? 'modifyCriteria' : 'addCriteria' ),
					array( 'mod_id' => $vm_modify_id ) ); ?>/facet/' + facet + '/id/' + ids.join('|');
			}
			e.preventDefault();
		});
	});
