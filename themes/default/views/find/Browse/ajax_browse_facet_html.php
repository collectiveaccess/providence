...
$va_service_urls = caJSONLookupServiceUrl(
    $this->request,
    $va_facet_info['table'],
...
<script type="text/javascript">
    function caUpdateFacetDisplay(grouping) {
        caUIBrowsePanel.showBrowsePanel('<?php print $vs_facet_name; ?>', <?php print ((intval(
                $vm_modify_id
            ) > 0) ? 'true' : 'false'); ?>, <?php print ((intval(
                $vm_modify_id
            ) > 0) ? $vm_modify_id : 'null'); ?>, grouping);
    }
</script>

<div
    class="browseSelectPanelContentArea <?php print ($vb_multiple_selection_facet) ? "browseSelectMultiplePanelContentArea" : "" ?>"
    id="browseSelectPanelContentArea">
    <?php
            ...
            <div id="<?php print $vs_facet_name; ?>_facet_container">
                ...
                if ($t_item && $t_subject) {
                    ...
                    <div class="hierarchyBrowserHelpText">
                        <?php print _t(
                            "Click on a %1 to find %2 related to it. Click on the arrow next to a %3 to see more specific %4 within that %5, or use the search field.",
        ...
        <div class="browseSelectPanelList">
            <table class='browseSelectPanelListTable' id='<?php print $vs_facet_name; ?>_facet_container'>
                ...
                foreach ($va_facet as $vn_i => $va_item) {
                    ...
                    $va_row[] = "<td class='browseSelectPanelListCell facetItem' width='{$va_td_width}%;' data-facet_item_id='{$va_item['id']}'>" . caNavLink(
                            $this->request,
                            html_entity_decode($vs_label),
                            'browseSelectPanelLink',
                            'find',
                            $this->request->getController(),
...
<script type="text/javascript">
    function loadFacetGroup(g) {
        jQuery('#browseSelectPanelContentArea').parent().load("<?php print caNavUrl(
            $this->request,
            $this->request->getModulePath(),
            $this->request->getController(),
            'getFacet',
        ...
        jQuery(".facetApply").on('click', function (e) {
            ...
            if (ids.length) {
                window.location = '<?php print caNavUrl(
                    ...
                    $this->request->getController(),
                    ((strlen($vm_modify_id)) ? 'modifyCriteria' : 'addCriteria'),
                    array('mod_id' => $vm_modify_id)
                ); ?>/facet/' + facet + '/id/' + ids.join('|');
            }
            e.preventDefault();
        });
    });
