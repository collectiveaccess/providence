...
<div id="scrollingResults">
	<form id="caFindResultsForm<?php print $vs_interstitial_prefix; ?>">
		<table id="<?php print $vs_interstitial_prefix; ?>RelatedList" class="listtable" width="100%" border="0"
		       cellpadding="0" cellspacing="1">
			<thead>
			<tr>
				<th style="width:10px; text-align:center;" class='list-header-nosort'>
					<!-- column for interstitial and delete buttons --></th>
				...
				<tr <?php print ( $i == 1 ) ? "class='odd'"
					...
					foreach ( $va_display_list as $vn_placement_id => $va_info ) {
						...
						if ( $vs_template = $va_info['settings']['format'] ) {
							...
							print caProcessTemplateForIDs( $vs_template, $vs_related_rel_table,
								array( $vn_relation_id ), $va_opts );
						} else {
							print $t_display->getDisplayValue( $vo_result, $vn_placement_id,
			...
			update: function () {
				...
				jQuery.post(
					'<?php print caNavUrl( $this->request, $this->request->getModulePath(),
						$this->request->getController(), 'SaveUserSort' ); ?>',
