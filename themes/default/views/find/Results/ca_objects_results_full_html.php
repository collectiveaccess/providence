...
<form id="caFindResultsForm">
	...
	while ( ( $item_count < $items_per_page ) && ( $result->nextHit() ) ) {
		...
		$object_id = (int) $result->get( 'ca_objects.object_id' );
		$checkbox_html
		           = "<input type='checkbox' name='add_to_set_ids' value='{$object_id}' class='addItemToSetControl addItemToSetControlInThumbnails' />";
		...
		if ( is_array( $tmp ) && sizeof( $tmp ) ) {
			...
			print "<div class='objectFullImageContainer' style='padding: " . $padding_top_bottom . "px 0px "
			      . $padding_top_bottom . "px 0px;'>";
			print $checkbox_html;
			?>

			<?php
			print caEditorLink( $this->request, array_shift( $tmp ), '', 'ca_objects', $object_id, array(), array(
				'onmouseover' => 'jQuery(".qlButtonContainerFull").css("display", "none"); jQuery("#ql_' . $object_id
				                 . '").css("display", "block");',
				'onmouseout'  => 'jQuery(".qlButtonContainerFull").css("display", "none");'
			) );
			print "<div class='qlButtonContainerFull' id='ql_{$object_id}' onmouseover='jQuery(\"#ql_{$object_id}\").css(\"display\", \"block\");'><a class='qlButton' onclick='caMediaPanel.showPanel(\""
			      . caNavUrl( $this->request, 'find', 'SearchObjects', 'QuickLook', array( 'object_id' => $object_id ) )
			      . "\"); return false;' >" . _t( "Quick Look" ) . "</a></div>";

			print "</div>";
		} else {

			print "<div class='objectFullImageContainer noMedia'>";
			print $checkbox_html;
			print "</div>";
		}
		...
		print "<div class='objectFullTextTitle'>" . caEditorLink( $this->request, implode( "<br/>", $labels ), '',
				'ca_objects', $object_id, array() ) . "</div>";

		// Output configured fields in display
		foreach ( $display_list as $placement_id => $info ) {
			if ( in_array( $info['bundle_name'], [ 'ca_objects.preferred_labels', 'ca_object_labels.name' ] ) ) {
				continue;
			}
			if ( $display_text = $t_display->getDisplayValue( $result,
				...
				print "<div class='objectFullTextTextBlock'><span class='formLabel'>" . $info['display'] . "</span>: "
				      . $display_text . "</div>\n";
			}
		}
