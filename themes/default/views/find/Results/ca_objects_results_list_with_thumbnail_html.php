		...
		<table class="listtable">
			...
			while(($vn_item_count < $vn_items_per_page) && $vo_result->nextHit()) {
				...
				<tr <?= ($i ==1) ? "class='odd'" : ""; ?>>
					...
					$va_tmp = $vo_result->getMediaTags(
						'ca_object_representations.media',
						'thumbnail'
					);

					if (sizeof($va_tmp) == 0) {
						$va_tmp[] = "<span style='opacity: 0.3;'>".caNavIcon(__CA_NAV_ICON_OVERVIEW__, "64px")."</span>";
					}
			...
			if (is_array($va_bottom_line = $this->getVar('bottom_line'))) {
				?>
				<tr>
					<td colspan="3" class="listtableTotals"><?= _t('Totals'); ?></td>
					<?php
			...
			if ($vs_bottom_line_totals = $this->getVar('bottom_line_totals')) {
				?>
				<tr>
					<td colspan="<?= sizeof($va_display_list) + 3; ?>" class="listtableAggregateTotals"><?= $vs_bottom_line_totals; ?></td>
				</tr>
				<?php
			}
			?>
