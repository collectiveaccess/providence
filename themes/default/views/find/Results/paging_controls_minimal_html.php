...
if(($this->getVar('num_pages') > 1) && !$this->getVar('dontShowPages')){
	...
	if ($this->getVar('page') > 1) {
		$vs_searchNav .= "<a href='#' onclick='jQuery(\"#resultBox\").load(\"".caNavUrl($this->request, 'find', $this->request->getController(), $this->request->getAction(), $va_previous_link_params)."\"); return false;' class='button' aria-label='".htmlspecialchars(_t('Previous page'))."'>&lsaquo; "._t("Previous")."</a>";
	}
	$vs_searchNav .= '&nbsp;&nbsp;&nbsp;'._t("Page").' '.$this->getVar('page').'/'.$this->getVar('num_pages').'&nbsp;&nbsp;&nbsp;';
	if ($this->getVar('page') < $this->getVar('num_pages')) {
		$vs_searchNav .= "<a href='#' onclick='jQuery(\"#resultBox\").load(\"".caNavUrl($this->request, 'find', $this->request->getController(), $this->request->getAction(), $va_next_link_params)."\"); return false;' class='button' aria-label='".htmlspecialchars(_t('Next page'))."'>"._t("Next")." &rsaquo;</a>";
	}
	$vs_searchNav .= "</div>";
}
$vs_searchNav .= "</div>";
