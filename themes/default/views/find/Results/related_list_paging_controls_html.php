...
if (($this->getVar('num_pages') > 1) && !$this->getVar('dontShowPages')) {
    ...
    if ($this->getVar('page') > 1) {
        $vs_searchNav .= "<a href='#' onclick='jQuery(\"#resultBox\").load(\"" . caNavUrl(
            ...
            ) . "\", function() { if(typeof caHackSearchResultForm == \"function\") { caHackSearchResultForm(false); }}); return false;' class='button'>&lsaquo; Previous</a>";
    }
    $vs_searchNav .= '&nbsp;&nbsp;&nbsp;Page ' . $this->getVar('page') . '/' . $this->getVar(
            'num_pages'
        ) . '&nbsp;&nbsp;&nbsp;';
    if ($this->getVar('page') < $this->getVar('num_pages')) {
        $vs_searchNav .= "<a href='#' onclick='jQuery(\"#resultBox\").load(\"" . caNavUrl(
                ...
                $this->request->getAction(),
                $va_next_link_params
            ) . "\", function() { if(typeof caHackSearchResultForm == \"function\") { caHackSearchResultForm(false); }}); return false;' class='button'>Next &rsaquo;</a>";
    }
    $vs_searchNav .= "</div>";
}
$vs_searchNav .= "</div>";
