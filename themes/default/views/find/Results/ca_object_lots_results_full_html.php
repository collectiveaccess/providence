...
<form id="caFindResultsForm">
	...
	while ( ( $vn_item_count < $vn_items_per_page ) && ( $vo_result->nextHit() ) ) {
		...
		print caEditorLink( $this->request, array_shift( $va_tmp ), '', 'ca_object_lots', $vn_lot_id, array(), array(
			'onmouseover' => 'jQuery(".qlButtonContainerFull").css("display", "none"); jQuery("#ql_' . $vn_lot_id
			                 . '").css("display", "block");',
			'onmouseout'  => 'jQuery(".qlButtonContainerFull").css("display", "none");'
		) );
		print "<div class='qlButtonContainerFull' id='ql_" . $vn_lot_id . "' onmouseover='jQuery(\"#ql_" . $vn_lot_id
		      . "\").css(\"display\", \"block\");'><a class='qlButton' onclick='caMediaPanel.showPanel(\""
		      . caNavUrl( $this->request, 'find', 'SearchObjectLots', 'QuickLook', array( 'lot_id' => $vn_lot_id ) )
		...
		print "<div class='objectFullTextTitle'>" . caEditorLink( $this->request, implode( "<br/>", $va_labels ), '',
				'ca_object_lots', $vn_lot_id, array() ) . "</div>";
