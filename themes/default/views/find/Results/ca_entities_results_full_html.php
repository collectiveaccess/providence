...
<form id="caFindResultsForm">
    ...
    while (($vn_item_count < $vn_items_per_page) && ($vo_result->nextHit())) {
        ...
        print caEditorLink(
            ...
            array(),
            array(
                'onmouseover' => 'jQuery(".qlButtonContainerFull").css("display", "none"); jQuery("#ql_' . $vn_entity_id . '").css("display", "block");',
                'onmouseout' => 'jQuery(".qlButtonContainerFull").css("display", "none");'
            )
        );
        print "<div class='qlButtonContainerFull' id='ql_" . $vn_entity_id . "' onmouseover='jQuery(\"#ql_" . $vn_entity_id . "\").css(\"display\", \"block\");'><a class='qlButton' onclick='caMediaPanel.showPanel(\"" . caNavUrl(
                $this->request,
                'find',
                'SearchEntities',
                'QuickLook',
        ...
        foreach ($va_display_list as $vn_placement_id => $va_info) {
            if (in_array($va_info['bundle_name'], array('ca_entities.preferred_labels', 'ca_entity_labels.name'))) {
                continue;
