...
$headerimage = $this->request->getThemeDirectoryPath() . "/graphics/logos/"
               . $this->request->config->get( 'report_img' );
...
while ( $vo_result->nextHit() ) {
	...
	$contentCell->addText(
		caEscapeForXML( html_entity_decode( strip_tags( br2nl( $vo_result->get( 'preferred_labels' ) ) ),
	...
	foreach ( $list as $vn_placement_id => $va_info ) {
		...
		if (
			...
			&& ( $va_info['settings']['display_mode']
			     == 'media' ) // make sure that for the 'url' mode we don't insert the image here
		) {
		...
		} elseif ( $vs_display_text = $t_display->getDisplayValue( $vo_result, $vn_placement_id,
			array_merge( array( 'request' => $this->request, 'purify' => true ),
			...
			$textrun->addText(
				preg_replace( "![\n\r]!", "<w:br/>",
					caEscapeForXML( html_entity_decode( strip_tags( br2nl( $vs_display_text ) ),
						ENT_QUOTES | ENT_HTML5 ) ) ),
				$styleContentFont
			);
