...
if ( $vo_result->numHits() > 0 ) {
	?>
	<a href='#' id='showResultsEditor'
	   onclick='caResultsEditorPanel.showPanel("<?php print caNavUrl( $this->request, '*', '*',
		   'resultsEditor' ); ?>"); return false;'><?php print caNavIcon( __CA_NAV_ICON_SPREADSHEET__, "24px" ); ?></a>
	<?php
}
?>
...
if ( $vo_result->numHits() > 0 ) {
	if ( ( $this->getVar( 'mode' ) === 'search' ) && ( $this->request->user->canDoAction( 'can_browse_' . $vs_table ) )
	     && ( ! $this->getVar( 'noRefine' ) && ! $this->getVar( 'noRefineControls' ) )
	) {
		?>
...
<div id="searchOptionsBox">
	<div class="bg">
		...
		print _t( "Layout" )
		      . ": <select id='view_select' name='view' style='width: 100px;' onchange='caUpdateDisplayList();'>\n";
		...
		if ( is_array( $va_display_lists ) && sizeof( $va_display_lists ) > 0 ) {
			...
			print _t( "Display" ) . ": " . caHTMLSelect( 'display_id', $va_opts, [ "id" => "display_select" ],
					[ "width" => "100px", "value" => (int) $this->getVar( "current_display_list" ) ] );
		}
		print "</select>\n";
		print "</div>";
		...
		if ( $this->getVar( 'show_children_display_mode_control' ) ) {
			print "<div class='col'>";
			print _t( 'Child records' ) . ': ' . caHTMLSelect( 'children',
					[ _t( 'show' ) => 'show', _t( 'hide' ) => 'hide' ], [],
					[ 'value' => $this->getVar( 'children_display_mode' ) ] );
			print "</div>";
		}
		?>
		<div class="clear"></div>
		...
		<a href='#' id='hideOptions'
		   onclick='return caHandleResultsUIBoxes("display", "hide"); return false;'><?php print caNavIcon( __CA_NAV_ICON_COLLAPSE__,
				"18px" ); ?></a>
		<a href='#' id='saveOptions'
		   onclick='jQuery("#caSearchOptionsForm").submit(); return false;'><?php print caNavIcon( __CA_NAV_ICON_GO__,
				"18px" ); ?></a>
		</form>
