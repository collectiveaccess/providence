...
if ($vo_result->numHits() > 0) {
    ?>
    <a href='#' id='showResultsEditor' onclick='caResultsEditorPanel.showPanel("<?php print caNavUrl(
        $this->request,
        '*',
        '*',
        'resultsEditor'
    ); ?>"); return false;'><?php print caNavIcon(__CA_NAV_ICON_SPREADSHEET__, "24px"); ?></a>
    <?php
}
?>
...
<div id="searchOptionsBox">
    <div class="bg">
        ...
        if (is_array($va_display_lists) && sizeof($va_display_lists) > 0) {
            ...
            print _t("Display") . ": " . caHTMLSelect(
                    'display_id',
                    $va_opts,
                    ["id" => "display_select"],
                    [
                        "width" => "100px",
                        "value" => (int)$this->getVar("current_display_list")
                    ]
                );
        }
        print "</select>\n";
        ...
        if ($this->getVar('show_children_display_mode_control')) {
            print "<div class='col'>";
            print _t('Child records') . ': ' . caHTMLSelect(
                    'children',
        ...
        <a href='#' id='hideOptions'
           onclick='return caHandleResultsUIBoxes("display", "hide"); return false;'><?php print caNavIcon(
                __CA_NAV_ICON_COLLAPSE__,
                "18px"
            ); ?></a>
        <a href='#' id='saveOptions'
           onclick='jQuery("#caSearchOptionsForm").submit(); return false;'><?php print caNavIcon(
                __CA_NAV_ICON_GO__,
                "18px"
            ); ?></a>
        </form>
