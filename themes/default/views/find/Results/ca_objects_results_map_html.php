    ...
    if ($va_map_stats['points'] > 0) {
        if ($va_map_stats['items'] < $vn_num_hits) {
            ...
            <script type="text/javascript">
                jQuery('div.searchNav').html('<?php print _t(
                        "%1 of %2 results have been mapped.  To see all results chose a different display.",
                        $va_map_stats['items'],
                        $vn_num_hits
                    ) . "</div>"; ?>');
            </script>
        ...
        print "<div id='map2' style='width: 740px; height: 450px;'> </div><div>" . $o_map->render(
                'HTML',
                array('delimiter' => "<br/>")
            ) . "</div>";
    } else {
        ...
        <div>
            <?php print _t(
                'It is not possible to show a map of the results because none of the items found have map coordinates.'
            ); ?>
        </div>
        <?php
    }
