            ...
            <table class="listtable">
                ...
                <tr>
                    ...
                    foreach ($va_display_list as $va_display_item) {
                        $vs_item_display_str =
                            ((mb_strlen($va_display_item['display']) > 30) ? strip_tags(
                                    mb_substr($va_display_item['display'], 0, 27)
                        ...
                        if ($va_display_item['is_sortable']) {
                            if ($vs_current_sort == $va_display_item['bundle_sort']) {
                                ...
                                print "<th class='{$vs_th_class}'><span id='listHeader" . $vn_id_count . "'><nobr>" .
                                    ...
                                    . "</nobr></span></th>";
                                TooltipManager::add(
                                    '#listHeader' . $vn_id_count,
                                    _t("Currently sorting by ") . $va_display_item['display']
                                );
                            } else {
                                print "<th class='list-header-unsorted'><span id='listHeader1" . $vn_id_count . "'><nobr>" . caNavLink(
                                        $this->request,
                                        $vs_item_display_str,
                                        '',
                                        $this->request->getModulePath(),
                                        $this->request->getController(),
                                        'Index',
                                        array('sort' => $va_display_item['bundle_sort'])
                                    ) . "</nobr></span></th>";
                                TooltipManager::add(
                                    '#listHeader1' . $vn_id_count,
                ...
                while (($vn_item_count < $vn_items_per_page) && $vo_result->nextHit()) {
                    ...
                    <tr <?php print ($i == 1) ? "class='odd'" : ""; ?>>
                        ...
                        if ($vs_mode == 'search') {
                            print " <a href='#' onclick='caOpenBrowserWith({$vn_place_id});'>" . caNavIcon(
                                    __CA_NAV_ICON_HIER__,
                                    2
                                ) . "</a>";
                        }
                        print "</td>";
                        foreach ($va_display_list as $vn_placement_id => $va_info) {
                            print "<td>" . $t_display->getDisplayValue(
                                    $vo_result,
                                    $vn_placement_id,
                                    array_merge(
