...
// Add default reps
if (
	( $version = $config->get( $vo_result->tableName()
	                           . "_always_include_primary_representation_media_in_xlsx_output" ) )
	&&
...
while ( $vo_result->nextHit() ) {
	...
	foreach ( $va_display_list as $va_info ) {
		...
		if ( is_array( $va_info['settings'] ) && isset( $va_info['settings']['format'] )
		     && ( $tags
				= array_filter( caGetTemplateTags( $va_info['settings']['format'] ), function ( $v ) {
...
if ( $this->request->config->get( 'excel_report_header_enabled' )
     || $this->request->config->get( 'excel_report_footer_enabled' )
) {
	...
	if ( $this->request->config->get( 'excel_report_header_enabled' ) ) {
		if ( file_exists( $this->request->getThemeDirectoryPath() . "/graphics/logos/"
			...
			$vs_logo_path = $this->request->getThemeDirectoryPath() . '/graphics/logos/'
			                . $this->request->config->get( 'report_img' );
		}
		...
		$o_sheet->getHeaderFooter()
		        ->addImage( $objDrawing, \PhpOffice\PhpSpreadsheet\Worksheet\HeaderFooter::IMAGE_HEADER_LEFT );
		$vs_criteria_summary = str_replace( "&", "+", strip_tags( html_entity_decode( $vs_criteria_summary ) ) );
		$vs_criteria_summary = ( strlen( $vs_criteria_summary ) > 90 ) ? mb_substr( $vs_criteria_summary, 0, 90 )
		                                                                 . "..." : $vs_criteria_summary;
		$vs_criteria_summary = wordwrap( $vs_criteria_summary, 50, "\n", true );
		$o_sheet->getHeaderFooter()->setOddHeader( '&L&G& '
		                                           . ( ( $this->request->config->get( 'excel_report_show_search_term' ) )
				? '&R&B&12 ' . $vs_criteria_summary : '' ) );
	...
	if ( $this->request->config->get( 'excel_report_footer_enabled' ) ) {
		...
		$o_sheet->getHeaderFooter()->setOddFooter( '&L&10' . ucfirst( $t_instance->getProperty( 'NAME_SINGULAR' )
		                                                              . ' report' ) . ' &C&10Page &P of &N &R&10 '
		                                           . date( "m/t/y" ) );
	}
}
