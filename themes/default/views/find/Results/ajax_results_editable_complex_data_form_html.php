...
print caFormTag( $this->request, '#', 'caEditableResultsComplexDataForm', null, 'POST', 'multipart/form-data', null,
	array( 'noCSRFToken' => true, 'disableUnsavedChangesWarning' => true, 'disableSubmit' => true ) );
?>
...
<div id="caResultsComplexDataEditorPanelControlButtons">
	<?php
	if ( $vb_can_edit ) {
		print caFormControlBox( caFormJSButton( $this->request, __CA_NAV_ICON_SAVE__, _t( "Save" ),
				"caEditableResultsComplexDataFormSaveButton",
				array( "onclick" => "caEditableResultsComplexDataFormHandler.save(event);" ) )
		                        . ' ' . caFormJSButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ),
				"caEditableResultsComplexDataFormCancelButton",
				array( "onclick" => "jQuery(\"#caEditableResultsComplexDataForm\").parent().parent().data(\"panel\").hidePanel();" ) ),
			'',
			''
		);
	}
	...
	var caEditableResultsComplexDataFormHandler = caUI.initQuickAddFormHandler({
		...
		onSave: function (resp) {
			if (resp.status == 0) {
				...
				if (jQuery("#caEditableResultsComplexDataForm") && jQuery("#caEditableResultsComplexDataForm").parent() && jQuery("#caEditableResultsComplexDataForm").parent().parent() && jQuery("#caEditableResultsComplexDataForm").parent().parent().data("panel")) {
					jQuery("#caEditableResultsComplexDataForm").parent().parent().data("panel").hidePanel();
				}
				jQuery(".caResultsEditorStatus").html("Saved changes").show();
				setTimeout(function () {
					jQuery('.caResultsEditorStatus').fadeOut(500);
				}, 5000);
			} else {
