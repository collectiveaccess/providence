	...
	foreach ( $va_type_groups as $vs_type => $va_matches ) {
		...
		foreach ( $va_matches as $vn_id => $va_match ) {
			...
			switch ( $vs_table ) {
				# --------------------------------------------------------
				case 'ca_objects':
					$vs_url = caNavUrl( $this->request, 'editor/objects', 'ObjectEditor', 'Edit',
				...
				# --------------------------------------------------------
				case 'ca_entities':
					$vs_url                   = caNavUrl( $this->request, 'editor/entities', 'EntityEditor', 'Edit',
				...
				# --------------------------------------------------------
				case 'ca_places':
					$vs_url                   = caNavUrl( $this->request, 'editor/places', 'PlaceEditor', 'Edit',
				...
				# --------------------------------------------------------
				case 'ca_occurrences':
					$vs_url                   = caNavUrl( $this->request, 'editor/occurrences', 'OccurrenceEditor',
						'Edit', array( 'occurrence_id' => $vn_id ) );
					$va_cur_data['results'][] = array(
						$vs_url,
						$vs_match,
						''
				...
				# --------------------------------------------------------
				case 'ca_collections':
					$vs_url                   = caNavUrl( $this->request, 'editor/collections', 'CollectionEditor',
						'Edit', array( 'collection_id' => $vn_id ) );
					$va_cur_data['results'][] = array(
						$vs_url,
						$vs_match,
						''
