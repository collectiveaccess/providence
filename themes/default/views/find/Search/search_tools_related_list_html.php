...
?>
		<div class="col">
			...
			print caFormTag($this->request, 'ExportData'.$vs_url_string, 'caExportWithMappingForm', 'manage/MetadataExport', 'post', 'multipart/form-data', '_top', array('disableUnsavedChangesWarning' => true));
...
<?php
			...
			print caFormTag($this->request, 'printLabels' . $vs_url_string, 'caPrintLabelsForm', $this->request->getModulePath().'/'.$this->request->getController(), 'post', 'multipart/form-data', '_top', array('disableUnsavedChangesWarning' => true));
...
<?php
		...
		print caFormTag($this->request, 'export'.$vs_url_string, 'caExportForm', $this->request->getModulePath().'/'.$this->request->getController(), 'post', 'multipart/form-data', '_top', array('disableUnsavedChangesWarning' => true));
...
<script type="text/javascript">
	function caDownloadRepresentations(mode) {
		...
		//if(tmp[0] == 'all') {	// download all search results
			jQuery(window).attr('location', '<?php print caNavUrl($this->request, $this->request->getModulePath(), $this->request->getController(), 'DownloadRepresentations'); ?>' + '/<?php print $t_subject->tableName(); ?>/all/version/' + tmp[1] + '/download/1<?php print $vs_url_string; ?>');
		//} else {
