    ...
    <div class="bg">
        ...
        if (is_array($va_export_mappings = $this->getVar('exporter_list')) && sizeof($va_export_mappings) > 0) {
            ...
            <div class="col">
                ...
                print caFormTag(
                    $this->request,
                    'ExportData' . $vs_url_string,
                    'caExportWithMappingForm_' . $this->getVar('interstitialPrefix'),
                    'manage/MetadataExport',
                    'post',
                    'multipart/form-data',
                    '_top',
                    array('noCSRFToken' => true, 'disableUnsavedChangesWarning' => true)
                );
                print ca_data_exporters::getExporterListAsHTMLFormElement(
                    'exporter_id',
                    $t_subject->tableNum(),
                    array('id' => 'caExporterList'),
                    array('width' => '150px')
                );
                ...
                print caFormSubmitLink(
                    $this->request,
                    caNavIcon(__CA_NAV_ICON_GO__, "18px"),
                    'button',
                    'caExportWithMappingForm_' . $this->getVar('interstitialPrefix')
                );
                ?>
        ...
        if (is_array($va_forms = $this->getVar('label_formats')) && sizeof($va_forms)) {
            ...
            <div class="col">
                ...
                print caFormTag(
                    $this->request,
                    'printLabels' . $vs_url_string,
                    'caPrintLabelsForm_' . $this->getVar('interstitialPrefix'),
                    $this->request->getModulePath() . '/' . $this->request->getController(),
                    'post',
                    'multipart/form-data',
                    '_top',
                    array('noCSRFToken' => true, 'disableUnsavedChangesWarning' => true)
        ...
        if (in_array(
                $t_subject->tableName(),
                array('ca_objects', 'ca_object_representations')
            ) && ($this->request->user->canDoAction('can_download_ca_object_representations')) && is_array(
            ...
            <div class="col">
                ...
                <form id="caDownloadMediaFromSearchResult_<?php print $this->getVar('interstitialPrefix'); ?>">
                    ...
                    ?>
                    <a href='#' onclick="caDownloadRepresentations_<?php print $this->getVar(
                        'interstitialPrefix'
                    ); ?>(jQuery('#caDownloadRepresentationMode').val());" class="button"><?php print _t('Download'); ?>
                        &rsaquo;</a>
                </form>
            </div>
            <?php
        ...
        <a href='#' class='hideTools' id='hideTools_<?php print $this->getVar('interstitialPrefix'); ?>'
           onclick='return caHandleResultsUIBoxes_<?php print $this->getVar(
               'interstitialPrefix'
           ); ?>("tools", "hide");'><?php print caNavIcon(__CA_NAV_ICON_COLLAPSE__, '18px'); ?></a>
...
<script type="text/javascript">
    function caDownloadRepresentations_<?php print $this->getVar('interstitialPrefix'); ?>(mode) {
        ...
        jQuery(window).attr('location', '<?php print caNavUrl(
            $this->request,
            $this->request->getModulePath(),
            $this->request->getController(),
            'DownloadMedia'
        ); ?>' + '/<?php print $t_subject->tableName(
        ); ?>/all/version/' + tmp[1] + '/download/1<?php print $vs_url_string; ?>');
        //} else {
        //	jQuery(window).attr('location', '<?php print caNavUrl(
        $this->request,
        $this->request->getModulePath(),
    ...
    ); ?>/' + caGetSelectedItemIDsToAddToSet().join(';') + '/version/' + tmp[1] + '/download/1');
        //}
    }
</script>
