...
<div id="searchToolsBox">
    <div class="bg">
        ...
        <a href='#' id='hideTools' onclick='return caHandleResultsUIBoxes("tools", "hide");'><?php print caNavIcon(
                __CA_NAV_ICON_COLLAPSE__,
                '18px'
            ); ?></a>
    ...
    function caDownloadRepresentations(mode) {
        ...
        if (tmp[0] == 'all') {	// download all search results
            jQuery(window).attr('location', '<?php print caNavUrl(
                ...
                'DownloadMedia'
            ); ?>' + '/<?php print $t_subject->tableName(); ?>/all/version/' + tmp[1] + '/download/1');
        } else {
            jQuery(window).attr('location', '<?php print caNavUrl(
                $this->request,
                $this->request->getModulePath(),
