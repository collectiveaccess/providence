		...
		foreach ( $va_criteria as $vs_facet_name => $va_row_ids ) {
			foreach ( $va_row_ids as $vn_row_id => $vs_label ) {
				if ( $vs_facet_name != '_search' ) {
					...
					print caNavLink( $this->request, 'x', 'close', $this->request->getModulePath(),
							$this->request->getController(), 'removeCriteria',
							array( 'facet' => $vs_facet_name, 'id' => $vn_row_id ) ) . "</div>";
				}
			}
		}
		print caNavLink( $this->request, _t( 'clear all' ), 'startOver', $this->request->getModulePath(),
...
?>
	<div id="searchRefineBox">
		<div class="bg">
			...
			<a href='#' id="hideRefine"
			   onclick='return caHandleResultsUIBoxes("refine", "hide");'><?php print caNavIcon( __CA_NAV_ICON_COLLAPSE__,
					'18px' ); ?></a>
	...
	<script type="text/javascript">
		jQuery(document).ready(function () {
			// load facets
			jQuery("#searchRefineContent").load("<?php print caNavUrl( $this->request, $this->request->getModulePath(),
				$this->request->getController(), 'Facets' );?>");
		});
	</script>
<?php
	...
	<script type="text/javascript">
		var caUIBrowsePanel = caUI.initBrowsePanel({
			facetUrl: '<?php print caNavUrl( $this->request, $this->request->getModulePath(),
				$this->request->getController(), 'getFacet' ); ?>', useExpose: false
		});
