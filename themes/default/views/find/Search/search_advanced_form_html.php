...
if ( ! $vn_form_id ) {
	...
	<div class="notification-warning-box">
		<ul class='notification-warning-box'>
			<li class='notification-warning-box'><?php print _t( "You must define a search form before you can use the advanced search." )
			                                                 . ' ' . caNavLink( $this->request,
						_t( 'Click here to create a new form.' ), '', 'manage', 'SearchForm', 'ListForms' ); ?></li>
		</ul>
	</div>
	<?php
} else {
	if ( ! $t_form->haveAccessToForm( $this->request->getUserID(), __CA_SEARCH_FORM_READ_ACCESS__, $vn_form_id ) ) {
		...
		?>
		<?php print caFormTag( $this->request, 'Index', 'AdvancedSearchForm', null, 'post', 'multipart/form-data',
		...
		<div class="control-box rounded">
			...
			<div style="float:right; ">
				<?php print caJSButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Reset" ), 'AdvancedSearchForm',
					array( 'onclick' => 'caAdvancedSearchFormReset()' ), array() ); ?>
				<?php print caFormSearchButton( $this->request, __CA_NAV_ICON_SEARCH__, _t( "Search" ),
					'AdvancedSearchForm' ); ?>
			</div>
			<div class="saveAs" style="float: right; margin-right:20px;">
				<?php print _t( "Save search as" ); ?>:
				<?php print caHTMLTextInput( '_label',
					array( 'size' => 10, 'id' => 'caAdvancedSearchSaveLabelInput' ) ); ?>
				<a href="#"
				   onclick="caSaveSearch('AdvancedSearchForm', jQuery('#caAdvancedSearchSaveLabelInput').val(), [<?php print join( ',',
					   $va_flds ); ?>]); return false;" class="button"><?php print caNavIcon( __CA_NAV_ICON_GO__,
						"18px" ); ?></a>
			</div>
		</div>
		</form>
