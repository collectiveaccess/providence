...
if (!$this->request->isAjax()) {
    $vs_form_list_select = $t_form->getFormsAsHTMLSelect(
        'form_id',
        array(
    ...
    if ($vs_form_list_select) {
        ?>
        <a href='#' class='button' id='advancedSearchFormContainerToggle'><?php print _t('Hide search form'); ?>
            &rsaquo;</a>
        <?php
    }
    ?>
    <div class='searchFormSelector' style='float:right;'>
        <form action='#' id='advancedSearchFormContainerFormSelector'><?php print ($vs_form_list_select) ? _t(
                    'Form'
                ) . ': ' . $vs_form_list_select : ''; ?></form>
    </div>
    ...
    <script type='text/javascript'>
        function caLoadAdvancedSearchForm(form_id) {
            jQuery('#advancedSearchFormContainer').load('<?php print caNavUrl(
                $this->request,
        ...
        if (caCookieJar.get('<?php print $this->getVar('table_name'); ?>_hide_adv_search_form') == 1) {
            ...
            jQuery("#advancedSearchFormContainerToggle").html('<?php print addslashes(
                _t('Show search form')
            );?> &rsaquo;');
        }
        ...
        jQuery("#advancedSearchFormContainerToggle").click(function () {
            jQuery("#advancedSearchFormContainer").slideToggle(350, function () {
                caCookieJar.set('<?php print $this->getVar(
                    'table_name'
                ); ?>_hide_adv_search_form', (this.style.display == 'block') ? 0 : 1);
                jQuery("#advancedSearchFormContainerToggle").html((this.style.display == 'block') ? '<?php print addslashes(
                    _t('Hide search form')
                );?> &rsaquo;' : '<?php print addslashes(_t('Show search form'));?> &rsaquo;');
