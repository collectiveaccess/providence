    ...
    <h3 class="tools"><?php print _t("History"); ?>:
        <div>
            ...
            print caFormTag(
                ...
                'Index',
                'caSearchHistoryForm',
                'find/SearchObjectRepresentations',
                'post',
                'multipart/form-data',
                '_top',
            ...
            foreach (array_reverse($va_search_history, true) as $vs_search => $va_search_info) {
                ...
                print "<option value='" . htmlspecialchars(
                        ...
                        ENT_QUOTES,
                        'UTF-8'
                    ) . "' {$SELECTED}>" . $vs_display . " (" . $va_search_info['hits'] . ")</option>\n";
            }
            print "</select>\n ";
...
if ($vo_result) {
    ...
    if ($vs_viz_list = Visualizer::getAvailableVisualizationsAsHTMLFormElement(
        $vo_result->tableName(),
        'viz',
        ...
        array(
            ...
            'restrictToTypes' => array(
                $vo_result_context->getTypeRestriction(
                    $vb_type_restriction_has_changed
                )
            )
        )
    )) {
        ...
        <div class='visualize'>
            <div id='vizLink'>
                <?php print "<a href='#'  onclick='jQuery(\"#caSearchVizOptsContainer\").slideToggle(250); jQuery(\"#vizLink\").hide();return false;'>" . caNavIcon(
                        __CA_NAV_ICON_VISUALIZE__,
                        1
                    ) . " " . _t("Visualize") . "</a>"; ?>
            ...
            <div id='caSearchVizOptsContainer' style="display:none;">
                ...
                <?php print "<a href='#'  onclick='caMediaPanel.showPanel(\"" . caNavUrl(
                        $this->request,
                        $this->request->getModulePath(),
                        $this->request->getController(),
                        'Viz',
                        array()
                    ) . "/viz/\" + jQuery(\"#caSearchVizOpts\").val()); return false;'>" . caNavIcon(
                        __CA_NAV_ICON_GO__,
                        "18px"
                    ) . "</a>"; ?>

                <a href='#' id='hideViz'
                   onclick='$("#caSearchVizOptsContainer").slideUp(250); $("#vizLink").slideDown(250); '><?php print caNavIcon(
