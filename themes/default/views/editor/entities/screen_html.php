...
if ($vb_can_edit) {
	$vs_control_box = caFormControlBox(
		...
		($this->getVar('show_save_and_return') ? caFormSubmitButton($this->request, __CA_NAV_BUTTON_SAVE__, _t("Save and return"), 'EntityEditorForm', array('isSaveAndReturn' => true)) : '').' '.
		caNavButton($this->request, __CA_NAV_BUTTON_CANCEL__, _t("Cancel"), '', 'editor/entities', 'EntityEditor', 'Edit/'.$this->request->getActionExtra(), ($vn_entity_id ? array('entity_id' => $vn_entity_id) : array('type_id' => $t_entity->getTypeID()))),
		'',
...
$va_form_elements = $t_entity->getBundleFormHTMLForScreen(
...
<div class="sectionBox">
	<?php print caFormTag($this->request, 'Save/'.$this->request->getActionExtra().'/entity_id/'.$vn_entity_id, 'EntityEditorForm', null, 'POST', 'multipart/form-data'); ?>
		<div class="grid">
			<?php print join("\n", $va_form_elements); ?>
