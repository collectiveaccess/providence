...
if ($vb_can_edit) {
    $va_cancel_parameters = ($vn_entity_id ? array('entity_id' => $vn_entity_id) : array(
        'type_id' => $t_entity->getTypeID()
    ));
    print $vs_control_box = caFormControlBox(
        ...
        '',
        ((intval($vn_entity_id) > 0) && $vb_can_delete) ? caFormNavButton(
            $this->request,
...
<div class="sectionBox">
    ...
    print caFormTag(
        $this->request,
        'Save/' . $this->request->getActionExtra() . '/entity_id/' . $vn_entity_id,
        'EntityEditorForm',
        null,
        'POST',
        'multipart/form-data'
