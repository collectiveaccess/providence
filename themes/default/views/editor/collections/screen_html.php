...
if ($vb_can_edit) {
    $va_cancel_parameters = ($vn_collection_id ? array('collection_id' => $vn_collection_id) : array(
        'type_id' => $t_collection->getTypeID()
    ));
    print $vs_control_box = caFormControlBox(
        ...
        '',
        ((intval($vn_collection_id) > 0) && $vb_can_delete) ? caFormNavButton(
            $this->request,
...
<div class="sectionBox">
    ...
    print caFormTag(
        $this->request,
        'Save/' . $this->request->getActionExtra() . '/collection_id/' . $vn_collection_id,
        'CollectionEditorForm',
        null,
        'POST',
        'multipart/form-data'
