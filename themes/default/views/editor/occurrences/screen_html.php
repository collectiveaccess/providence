...
if ($vb_can_edit) {
    $va_cancel_parameters = ($vn_occurrence_id ? array('occurrence_id' => $vn_occurrence_id) : array(
        'type_id' => $t_occurrence->getTypeID()
    ));
    print $vs_control_box = caFormControlBox(
        ...
        '',
        ((intval($vn_occurrence_id) > 0) && $vb_can_delete) ? caFormNavButton(
            $this->request,
...
<div class="sectionBox">
    ...
    print caFormTag(
        $this->request,
        'Save/' . $this->request->getActionExtra() . '/occurrence_id/' . $vn_occurrence_id,
        'OccurrenceEditorForm',
        null,
        'POST',
        'multipart/form-data'
