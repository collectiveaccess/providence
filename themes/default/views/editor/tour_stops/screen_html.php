...
if ( $vb_can_edit ) {
	$va_cancel_parameters = ( $vn_stop_id ? array( 'stop_id' => $vn_stop_id )
		: array( 'type_id' => $t_stop->getTypeID() ) );
	print $vs_control_box = caFormControlBox(
		...
		( $this->getVar( 'show_save_and_return' ) ? caFormSubmitButton( $this->request, __CA_NAV_ICON_SAVE__,
			_t( "Save and return" ), 'TourStopEditorForm', array( 'isSaveAndReturn' => true ) ) : '' ) . ' ' .
		caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', 'editor/tour_stops',
			'TourStopEditor', 'Edit/' . $this->request->getActionExtra(), $va_cancel_parameters ),
		...
		( ( intval( $vn_stop_id ) > 0 ) && ( $vb_can_delete ) ) ? caFormNavButton( $this->request,
			__CA_NAV_ICON_DELETE__, _t( "Delete" ), '', 'editor/tour_stops', 'TourStopEditor',
			'Delete/' . $this->request->getActionExtra(), array( 'stop_id' => $vn_stop_id ) ) : ''
	);
}
?>
	<div class="sectionBox">
		<?php
		print caFormTag( $this->request, 'Save/' . $this->request->getActionExtra() . '/stop_id/' . $vn_stop_id,
			'TourStopEditorForm', null, 'POST', 'multipart/form-data' );
