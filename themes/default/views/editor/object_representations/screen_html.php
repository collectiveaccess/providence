...
if ( $vb_can_edit ) {
	$va_cancel_parameters = ( $vn_representation_id ? array( 'representation_id' => $vn_representation_id )
		: array( 'type_id' => $t_object_representation->getTypeID() ) );
	print $vs_control_box = caFormControlBox(
		...
		( $this->getVar( 'show_save_and_return' ) ? caFormSubmitButton( $this->request, __CA_NAV_ICON_SAVE__,
			_t( "Save and return" ), 'ObjectRepresentationEditorForm', array( 'isSaveAndReturn' => true ) ) : '' ) . ' '
		.
		caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', 'editor/object_representations',
			'ObjectRepresentationEditor', 'Edit/' . $this->request->getActionExtra(), $va_cancel_parameters ),
		...
		( ( intval( $vn_representation_id ) > 0 ) && $vb_can_delete ) ? caFormNavButton( $this->request,
			__CA_NAV_ICON_DELETE__, _t( "Delete" ), '', 'editor/object_representations', 'ObjectRepresentationEditor',
			'Delete/' . $this->request->getActionExtra(), array( 'representation_id' => $vn_representation_id ) ) : ''
	);
}
?>
	<div class="sectionBox">
		...
		print caFormTag( $this->request,
			'Save/' . $this->request->getActionExtra() . '/representation_id/' . $vn_representation_id,
			'ObjectRepresentationEditorForm', null, 'POST', 'multipart/form-data' );

		$va_form_elements = $t_object_representation->getBundleFormHTMLForScreen( $this->request->getActionExtra(),
			array(
