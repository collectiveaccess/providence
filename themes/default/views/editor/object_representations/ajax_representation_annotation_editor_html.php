...
<div class="caMediaOverlayControls">
	...
	<div class='close'><a href="#" onclick="caMediaPanel.hidePanel(); return false;"
					...
					[ 'color' => 'white' ] ) . ' ' . _t( 'Close annotations' ); ?></a></div>
</div>
...
<div class="caAnnoEditorPlayerAndFormContainer">
	<div class="caAnnoMediaPlayerContainer">
		...
		<div class="caAnnoMediaPlayerControlsLeft">
			<?php print "<span id='caAnnoEditorInOutButtonLabel'>" . _t( 'Set' ) . ': </span>'
			            . caJSButton( $this->request, __CA_NAV_ICON_ADD__, _t( "start" ), "caAnnoEditorAddAtButton",
					array(
						"id"      => "caAnnoEditorInButton",
						"onclick" => "caAnnoEditorSetInTime(caAnnoEditorGetPlayerTime(), \"PLAY\")"
					) ); ?>
			<?php print caJSButton( $this->request, __CA_NAV_ICON_ADD__, _t( "end" ), "caAnnoEditorAddAtButton", array(
				"id"      => "caAnnoEditorOutPauseButton",
				"onclick" => "caAnnoEditorSetOutTime(caAnnoEditorGetPlayerTime(), \"PAUSE\")"
			) ); ?>
		</div>
		<div class="caAnnoMediaPlayerControlsRight">
			<?php print caJSButton( $this->request, __CA_NAV_ICON_ADD__, _t( "Set end &amp; Save" ),
				"caAnnoEditorAddAtButton", array(
					"id"      => "caAnnoEditorOutAndSavePauseButton",
					"onclick" => "caAnnoEditorSetOutTime(caAnnoEditorGetPlayerTime(), null, true)"
				) ); ?>
		</div>
	</div>
...
<script type="text/javascript">
	jQuery(document).ready(function () {
		...
		jQuery('#caAnnoEditorTlSyncSlider').slider({
			min: 0, max: c, animate: 'fast',
		...
		jQuery('#caAnnoEditorTlSyncButton').click(
			function (e) {
				...
				jQuery('#caAnnoEditorTlSyncSlider').slider("value", (<?php print (int) $vn_annotation_count
				                                                                 + 1; ?> -visibleItems));
				return false;
			}
		);
	...
	function caAnnoEditorTlLoad(theCarousel, start, count) {
		if (!count) count = 0;
		jQuery.getJSON('<?php print caNavUrl( $this->request, '*', '*', 'getAnnotationList' ); ?>', {
			representation_id: <?php print (int) $vn_representation_id; ?>,
			s: start,
			n: count
		}, function (data) {
			...
			jQuery.each(data['list'], function (k, v) {
				...
				var item = "<div id='caAnnoEditorTlAnnotationContainer" + annotation_id + "' class='caAnnoEditorTlAnnotationContainer'>" +
					...
					"<div class='editAnnoButton'><a href='#' onclick='caAnnoEditorEdit(" + annotation_id + "); event.preventDefault(); return false;'><?php print caNavIcon( __CA_NAV_ICON_EDIT__,
						1 ); ?></a></div>" +
					"<div class='deleteAnnoButton'><a href='#' onclick='caAnnoEditorDelete(" + annotation_id + "); event.preventDefault(); return false;'><?php print caNavIcon( __CA_NAV_ICON_DEL_BUNDLE__,
						1 ); ?></a></div>";
	...
	function caAnnoEditorEdit(annotation_id, inTime, outTime, state) {
		...
		jQuery("#caAnnoEditorEditorScreen").load("<?php print caNavUrl( $this->request,
			'editor/representation_annotations', 'RepresentationAnnotationQuickAdd', 'Form', array(
				'representation_id' => $vn_representation_id,
				'annotation_id'     => ''
			) ); ?>" + annotation_id, {startTimecode: inTime, endTimecode: outTime}).show();
	...
	function caAnnoEditorDelete(annotation_id) {
		jQuery.getJSON('<?php print caNavUrl( $this->request, 'editor/representation_annotations',
			'RepresentationAnnotationQuickAdd',
			'deleteAnnotation' ); ?>', {annotation_id: annotation_id}, function (resp) {
			if (resp.code == 0) {
	...
	function caAnnoEditorDisableAnnotationForm() {
		caAnnoSetEditFormSize();
		jQuery("#caAnnoEditorEditorScreen").load('<?php print caNavUrl( $this->request,
			'editor/representation_annotations', 'RepresentationAnnotationQuickAdd', 'Form',
			array( 'representation_id' => $vn_representation_id, 'annotation_id' => '0' ) ); ?>', {}, function () {
