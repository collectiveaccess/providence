...
<div class="caMediaOverlayControls">
    <div class="objectInfo"><?php print "{$vs_media_type}; " . caGetRepresentationDimensionsForDisplay(
                $t_rep,
                'original'
            ); ?></div>
    <div class='close'><a href="#" onclick="caMediaPanel.hidePanel(); return false;"
                          title="close"><?php print caNavIcon(
                __CA_NAV_ICON_CLOSE__,
                "18px",
                [],
                ['color' => 'white']
            ); ?></a></div>
...
<div class="caAnnoEditorPlayerAndFormContainer">
    <div class="caAnnoMediaPlayerContainer">
        ...
        <div class="caAnnoMediaPlayerControlsLeft">
            <?php print "<span id='caAnnoEditorInOutButtonLabel'>" . _t('Set') . ': </span>' . caJSButton(
                    $this->request,
                    __CA_NAV_ICON_ADD__,
                    _t("start"),
                    "caAnnoEditorAddAtButton",
                    array(
...
<script type="text/javascript">
    ...
    function caAnnoEditorTlLoad(theCarousel, start, count) {
        if (!count) count = 0;
        jQuery.getJSON('<?php print caNavUrl(
            $this->request,
            '*',
            '*',
            'getAnnotationList'
        ); ?>', {representation_id: <?php print (int)$vn_representation_id; ?>, s: start, n: count}, function (data) {
            ...
            jQuery.each(data['list'], function (k, v) {
                ...
                var item = "<div id='caAnnoEditorTlAnnotationContainer" + annotation_id + "' class='caAnnoEditorTlAnnotationContainer'>" +
                    ...
                    "<div class='editAnnoButton'><a href='#' onclick='caAnnoEditorEdit(" + annotation_id + "); event.preventDefault(); return false;'><?php print caNavIcon(
                        __CA_NAV_ICON_EDIT__,
                        1
                    ); ?></a></div>" +
                    "<div class='deleteAnnoButton'><a href='#' onclick='caAnnoEditorDelete(" + annotation_id + "); event.preventDefault(); return false;'><?php print caNavIcon(
    ...
    function caAnnoEditorTlSetCount(total) {
        ...
        var msg = (total == 1) ? '<?php print addslashes(_t("%1 clip")); ?>' : '<?php print addslashes(
            _t("%1 clips")
        ); ?>';
    ...
    function caAnnoEditorEdit(annotation_id, inTime, outTime, state) {
        ...
        jQuery("#caAnnoEditorEditorScreen").load("<?php print caNavUrl(
            $this->request,
    ...
    function caAnnoEditorDelete(annotation_id) {
        jQuery.getJSON('<?php print caNavUrl(
            ...
            'RepresentationAnnotationQuickAdd',
            'deleteAnnotation'
        ); ?>', {annotation_id: annotation_id}, function (resp) {
            if (resp.code == 0) {
