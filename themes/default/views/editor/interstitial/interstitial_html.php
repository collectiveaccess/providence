...
<form action="#" name="<?php print $vs_form_name; ?>" method="POST" enctype="multipart/form-data"
	...
	<div class='dialogHeader quickAddDialogHeader'><?php
		...
		if ( $vb_can_edit ) {
			print "<div style='float: right;'>" . caJSButton( $this->request, __CA_NAV_ICON_SAVE__, _t( "Save" ),
			...
			function caSave<?php print $vs_form_name . $vs_field_name_prefix . $vs_n; ?>(e) {
				...
				$.ajax({
					...
					success: function (resp, textStatus) {
						if (resp.status == 0) {
							jQuery.jGrowl('<?php print addslashes( _t( 'Saved changes to' ) ); ?> <em>' + resp.display + '</em>', {
								header: '<?php print addslashes( _t( 'Edit %1',
									$t_subject->getProperty( 'NAME_SINGULAR' ) ) ); ?>'
							});
							jQuery("#<?php print $vs_form_name . $vs_field_name_prefix
							                     . $vs_n; ?>").parent().data('panel').hidePanel();
							var displayContent = jQuery('#caRelationEditorPanel<?php print substr( $vs_field_name_prefix,
								0, strlen( $vs_field_name_prefix )
								   - 1 ); ?> .caBundleDisplayTemplate').template(resp.bundleDisplay);
