...
<div class="sectionBox">
    <?php
    if ($vb_can_edit) {
        $va_cancel_parameters = ($vn_location_id ? array('location_id' => $vn_location_id) : array(
            'type_id' => $t_location->getTypeID()
        ));
        print $vs_control_box = caFormControlBox(
            ...
            '',
            ((intval($vn_location_id) > 0) && $vb_can_delete) ? caFormNavButton(
                $this->request,
    ...
    print caFormTag(
        $this->request,
        'Save/' . $this->request->getActionExtra() . '/location_id/' . $vn_location_id,
        'StorageLocationEditorForm',
        null,
        'POST',
        'multipart/form-data'
    ...
    $va_form_elements = $t_location->getBundleFormHTMLForScreen(
