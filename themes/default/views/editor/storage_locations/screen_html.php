...
?>
	<div class="sectionBox">
		<?php
		if ( $vb_can_edit ) {
			$va_cancel_parameters = ( $vn_location_id ? array( 'location_id' => $vn_location_id )
				: array( 'type_id' => $t_location->getTypeID() ) );
			print $vs_control_box = caFormControlBox(
				...
				( $this->getVar( 'show_save_and_return' ) ? caFormSubmitButton( $this->request, __CA_NAV_ICON_SAVE__,
					_t( "Save and return" ), 'StorageLocationEditorForm', array( 'isSaveAndReturn' => true ) ) : '' )
				. ' ' .
				caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', 'editor/storage_locations',
					'StorageLocationEditor', 'Edit/' . $this->request->getActionExtra(), $va_cancel_parameters ),
				...
				( ( intval( $vn_location_id ) > 0 ) && $vb_can_delete ) ? caFormNavButton( $this->request,
					__CA_NAV_ICON_DELETE__, _t( "Delete" ), '', 'editor/storage_locations', 'StorageLocationEditor',
					'Delete/' . $this->request->getActionExtra(), array( 'location_id' => $vn_location_id ) ) : ''
			);
		}
		?>
		<?php
		print caFormTag( $this->request, 'Save/' . $this->request->getActionExtra() . '/location_id/' . $vn_location_id,
			'StorageLocationEditorForm', null, 'POST', 'multipart/form-data' );
