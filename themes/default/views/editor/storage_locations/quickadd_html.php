...
<script type="text/javascript">
	var caQuickAddFormHandler = caUI.initQuickAddFormHandler({
		...
		formUrl: '<?php print caNavUrl($this->request, 'editor/storage_locations', 'StorageLocationQuickAdd', 'Form'); ?>',
		fileUploadUrl: '<?php print caNavUrl($this->request, "editor/storage_locations", "StorageLocationEditor", "UploadFiles"); ?>',
		saveUrl: '<?php print caNavUrl($this->request, "editor/storage_locations", "StorageLocationQuickAdd", "Save"); ?>',
...
<form action="#" name="<?php print $vs_form_name; ?>" method="POST" enctype="multipart/form-data" id="<?php print $vs_form_name.$vs_field_name_prefix.$vs_n; ?>">
	...
	if ($vb_can_edit) {
		print "<div style='float: right;'>".caJSButton($this->request, __CA_NAV_BUTTON_ADD_LARGE__, _t("Add %1", $t_subject->getProperty('NAME_SINGULAR')), "{$vs_form_name}{$vs_field_name_prefix}{$vs_n}", array("onclick" => "caQuickAddFormHandler.save(event);"))
