...
<script type="text/javascript">
	var caQuickAddFormHandler = caUI.initQuickAddFormHandler({
		...
		formUrl: '<?php print caNavUrl( $this->request, 'editor/storage_locations', 'StorageLocationQuickAdd',
			'Form' ); ?>',
		fileUploadUrl: '<?php print caNavUrl( $this->request, "editor/storage_locations", "StorageLocationEditor",
			"UploadFiles" ); ?>',
		saveUrl: '<?php print caNavUrl( $this->request, "editor/storage_locations", "StorageLocationQuickAdd",
			"Save" ); ?>',
...
<form action="#" class="quickAddSectionForm" name="<?php print $vs_form_name; ?>" method="POST"
      enctype="multipart/form-data" id="<?php print $vs_form_name . $vs_field_name_prefix . $vs_n; ?>">
	<div class='quickAddDialogHeader'><?php
		print "<div class='quickAddTypeList'>" . _t( 'Quick Add %1',
				$t_subject->getTypeListAsHTMLFormElement( 'change_type_id', array(
					'id'       => "{$vs_form_name}TypeID{$vs_field_name_prefix}{$vs_n}",
					'onchange' => "caQuickAddFormHandler.switchForm();"
				), array( 'value' => $t_subject->get( 'type_id' ), 'restrictToTypes' => $va_restrict_to_types ) ) )
		      . "</div>";
		if ( $vb_can_edit ) {
			print "<div class='quickAddControls'>" . caJSButton( $this->request, __CA_NAV_ICON_ADD__,
					_t( "Add %1", $t_subject->getTypeName() ), "{$vs_form_name}{$vs_field_name_prefix}{$vs_n}",
					array( "onclick" => "caQuickAddFormHandler.save(event);" ) )
			      . ' ' . caJSButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ),
					"{$vs_form_name}{$vs_field_name_prefix}{$vs_n}", array(
						"onclick" => "jQuery(\"#{$vs_form_name}" . $vs_field_name_prefix . $vs_n
						             . "\").parent().data(\"panel\").hidePanel();"
					) ) . "</div>\n";
		}
	...
	<div class="quickAddErrorContainer"
	     id="<?php print $vs_form_name; ?>Errors<?php print $vs_field_name_prefix . $vs_n; ?>"></div>
	<div class="quickAddSectionBox" id="{$vs_form_name}Container<?php print $vs_field_name_prefix . $vs_n; ?>">
			...
			<div class="bundleContainer">
				<div class="caItemList">
					<div class="hierarchyBrowserContainer">
						...
						<div>
							<?php print _t( 'Search' ); ?>: <input type="text"
							                                       id="caQuickAdd<?php print $vs_form_name; ?>HierarchyBrowserSearch"
							                                       name="search"
							                                       value="<?php print htmlspecialchars( $this->getVar( 'search' ),
								                                       ENT_QUOTES, 'UTF-8' ); ?>" size="100"/>
						</div>
					</div>
					...
					<script type="text/javascript">
						...
						if (!o<?php print $vs_form_name . $vs_field_name_prefix; ?>HierarchyBrowser) {
							o<?php print $vs_form_name
							             . $vs_field_name_prefix; ?>HierarchyBrowser = caUI.initHierBrowser('caQuickAdd<?php print $vs_form_name
							                                                                                                       . $vs_field_name_prefix; ?>HierarchyBrowser', {
						...
						jQuery('#caQuickAdd<?php print $vs_form_name
						                               . $vs_field_name_prefix; ?>HierarchyBrowserSearch').autocomplete(
							{
								...
								select: function (event, ui) {
									if (parseInt(ui.item.id) > 0) {
										o<?php print $vs_form_name
										             . $vs_field_name_prefix; ?>HierarchyBrowser.setUpHierarchy(ui.item.id);	// jump browser to selected item
									}
									jQuery('#caQuickAdd<?php print $vs_form_name
									                               . $vs_field_name_prefix; ?>HierarchyBrowserSearch').val('');
								}
							}
						);
					</script>
					<input type="hidden" name="parent_id"
					       value="<?php print (int) $this->getVar( "default_parent_id" ); ?>"
					       id="<?php print $vs_form_name; ?>_parent_id"/>
				</div>
			</div>
		</div>
		<?php
		$va_form_elements = $t_subject->getBundleFormHTMLForScreen( $this->getVar( 'screen' ), array(
			...
			// force query text to be default in label fields
		...
		<input type='hidden' name='types'
		       value='<?php print htmlspecialchars( is_array( $va_restrict_to_types ) ? join( ',',
