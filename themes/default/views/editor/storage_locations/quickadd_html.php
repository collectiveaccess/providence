...
<script type="text/javascript">
    var caQuickAddFormHandler = caUI.initQuickAddFormHandler({
        ...
        formUrl: '<?php print caNavUrl(
            $this->request,
            'editor/storage_locations',
            'StorageLocationQuickAdd',
            'Form'
        ); ?>',
        fileUploadUrl: '<?php print caNavUrl(
            $this->request,
            "editor/storage_locations",
            "StorageLocationEditor",
            "UploadFiles"
        ); ?>',
        saveUrl: '<?php print caNavUrl(
            $this->request,
            "editor/storage_locations",
            "StorageLocationQuickAdd",
            "Save"
...
<form action="#" class="quickAddSectionForm" name="<?php print $vs_form_name; ?>" method="POST"
    ...
    <div class='quickAddDialogHeader'><?php
        print "<div class='quickAddTypeList'>" . _t(
                'Quick Add %1',
                $t_subject->getTypeListAsHTMLFormElement(
                    'change_type_id',
                    array(
        ...
        if ($vb_can_edit) {
            print "<div class='quickAddControls'>" . caJSButton(
                    $this->request,
                    __CA_NAV_ICON_ADD__,
                    _t("Add %1", $t_subject->getTypeName()),
                    "{$vs_form_name}{$vs_field_name_prefix}{$vs_n}",
                ...
                . ' ' . caJSButton(
                    ...
                    _t("Cancel"),
                    "{$vs_form_name}{$vs_field_name_prefix}{$vs_n}",
                    array("onclick" => "jQuery(\"#{$vs_form_name}" . $vs_field_name_prefix . $vs_n . "\").parent().data(\"panel\").hidePanel();")
                ) . "</div>\n";
        }
    ...
    <div class="quickAddSectionBox" id="{$vs_form_name}Container<?php print $vs_field_name_prefix . $vs_n; ?>">
                    ...
                    <div class="hierarchyBrowserContainer">
                        ...
                        <div>
                            <?php print _t('Search'); ?>: <input type="text"
                                                                 ...
                                                                 name="search" value="<?php print htmlspecialchars(
                                $this->getVar('search'),
                                ENT_QUOTES,
                                'UTF-8'
                            ); ?>" size="100"/>
        ...
        <input type='hidden' name='types' value='<?php print htmlspecialchars(
            is_array($va_restrict_to_types) ? join(',', $va_restrict_to_types) : ''
        ); ?>'/>
    </div>
</form>
