...
if ( $vb_can_edit ) {
	$va_cancel_parameters = ( $vn_place_id ? array( 'place_id' => $vn_place_id )
		: array( 'type_id' => $t_place->getTypeID() ) );
	print $vs_control_box = caFormControlBox(
		...
		( $this->getVar( 'show_save_and_return' ) ? caFormSubmitButton( $this->request, __CA_NAV_ICON_SAVE__,
			_t( "Save and return" ), 'PlaceEditorForm', array( 'isSaveAndReturn' => true ) ) : '' ) . ' ' .
		caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', 'editor/places', 'PlaceEditor',
		...
		( ( intval( $vn_place_id ) > 0 ) && $vb_can_delete ) ? caFormNavButton( $this->request, __CA_NAV_ICON_DELETE__,
			_t( "Delete" ), '', 'editor/places', 'PlaceEditor', 'Delete/' . $this->request->getActionExtra(),
...
?>
	<div class="sectionBox">
		<?php
		print caFormTag( $this->request, 'Save/' . $this->request->getActionExtra() . '/place_id/' . $vn_place_id,
			'PlaceEditorForm', null, 'POST', 'multipart/form-data' );
