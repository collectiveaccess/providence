...
?>
		<div id="printButton">
			<a href="<?php print caNavUrl($this->request, $this->request->getModulePath(), $this->request->getController(), "PrintSummary", array("object_id" => $t_item->getPrimaryKey()))?>">
				<?php print caNavIcon(__CA_NAV_BUTTON_PDF__, 2); ?>
			</a>
		</div>
<?php
		...
		foreach($va_reps as $va_rep) {
			if(sizeof($va_reps) > 1){
				...
				if ($this->request->user->canDoAction('can_download_ca_object_representations')) {
					print "<div class='download".$va_rep['representation_id']." downloadMediaContainer'>".caNavLink($this->request, caNavIcon(__CA_NAV_BUTTON_DOWNLOAD__, 1), 'downloadMedia', 'editor/objects', 'ObjectEditor', 'DownloadMedia', array('object_id' => $vn_item_id, 'representation_id' => $va_rep['representation_id'], 'version' => 'original'))."</div>\n";
				}
