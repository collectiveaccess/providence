...
if ( $vb_can_edit ) {
	$va_cancel_parameters = ( $vn_lot_id ? array( 'lot_id' => $vn_lot_id )
		: array( 'type_id' => $t_lot->getTypeID() ) );
	print $vs_control_box = caFormControlBox(
		...
		( $this->getVar( 'show_save_and_return' ) ? caFormSubmitButton( $this->request, __CA_NAV_ICON_SAVE__,
			_t( "Save and return" ), 'ObjectLotEditorForm', array( 'isSaveAndReturn' => true ) ) : '' ) . ' ' .
		caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', 'editor/object_lots',
			'ObjectLotEditor', 'Edit/' . $this->request->getActionExtra(), $va_cancel_parameters ),
		...
		( ( intval( $vn_lot_id ) > 0 ) && $vb_can_delete ) ? caFormNavButton( $this->request, __CA_NAV_ICON_DELETE__,
			_t( "Delete" ), '', 'editor/object_lots', 'ObjectLotEditor', 'Delete/' . $this->request->getActionExtra(),
...
?>
	<div class="sectionBox">
		<?php
		print caFormTag( $this->request, 'Save/' . $this->request->getActionExtra() . '/lot_id/' . $vn_lot_id,
			'ObjectLotEditorForm', null, 'POST', 'multipart/form-data' );
