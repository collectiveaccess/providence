...
if ( $vb_can_edit ) {
	$va_cancel_parameters = ( $vn_movement_id ? array( 'movement_id' => $vn_movement_id )
		: array( 'type_id' => $t_movement->getTypeID() ) );
	print $vs_control_box = caFormControlBox(
		...
		( $this->getVar( 'show_save_and_return' ) ? caFormSubmitButton( $this->request, __CA_NAV_ICON_SAVE__,
			_t( "Save and return" ), 'MovementEditorForm', array( 'isSaveAndReturn' => true ) ) : '' ) . ' ' .
		caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', 'editor/movements',
			'MovementEditor', 'Edit/' . $this->request->getActionExtra(), $va_cancel_parameters ),
		...
		( ( intval( $vn_movement_id ) > 0 ) && $vb_can_delete ) ? caFormNavButton( $this->request,
			__CA_NAV_ICON_DELETE__, _t( "Delete" ), '', 'editor/movements', 'MovementEditor',
			'Delete/' . $this->request->getActionExtra(), array( 'movement_id' => $vn_movement_id ) ) : ''
	);
}
?>
	<div class="sectionBox">
		<?php
		print caFormTag( $this->request, 'Save/' . $this->request->getActionExtra() . '/movement_id/' . $vn_movement_id,
			'MovementEditorForm', null, 'POST', 'multipart/form-data' );
