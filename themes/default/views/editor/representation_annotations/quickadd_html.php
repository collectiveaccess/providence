...
<form action="#" class="quickAddSectionForm" name="<?php print $vs_form_name; ?>" method="POST"
	...
	<div class='quickAddDialogHeader'><?php
		if ( $vb_can_edit ) {
			if ( ( $vn_subject_id > 0 ) && ( preg_match( "!timebased!i", $t_subject->getAnnotationType() ) ) ) {
				print "<div style='float: right;'>" . caJSButton( $this->request, __CA_NAV_ICON_DELETE__,
						_t( "Delete annotation" ), "{$vs_form_name}{$vs_field_name_prefix}{$vs_n}",
						array( "onclick" => "caConfirmDeleteAnnotation(true);" ) ) . "</div>\n";
			}
			print "<div style='float: left;'>" . caJSButton( $this->request, __CA_NAV_ICON_SAVE__,
					_t( "Save annotation" ), "caAnnoEditorScreenSaveButton",
					array( "onclick" => "caSaveAnnotation{$vs_form_name}{$vs_field_name_prefix}{$vs_n}(event);" ) )
			      . ' ' . caJSButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ),
					"{$vs_form_name}{$vs_field_name_prefix}{$vs_n}",
					array( "onclick" => "return caAnnoEditorDisableAnnotationForm();" ) )
	...
	<div class="caAnnoEditorEditorErrorContainer"
	     id="<?php print $vs_form_name; ?>Errors<?php print $vs_field_name_prefix . $vs_n; ?>"></div>
	<div class="quickAddSectionBox"
	     id="<?php print $vs_form_name; ?>Container<?php print $vs_field_name_prefix . $vs_n; ?>">
		<?php
		...
		<script type="text/javascript">
			...
			function caSaveAnnotation<?php print $vs_form_name . $vs_field_name_prefix . $vs_n; ?>(e) {
				...
				jQuery.post('<?php print caNavUrl( $this->request, "editor/representation_annotations",
					"RepresentationAnnotationQuickAdd", "Save" ); ?>', jQuery("#<?php print $vs_form_name
				                                                                            . $vs_field_name_prefix
				                                                                            . $vs_n; ?>").serialize(), function (resp, textStatus) {
					if (resp.status == 0) {

						var inputID = jQuery("#<?php print $vs_form_name . $vs_field_name_prefix
						                                   . $vs_n; ?>").parent().data('autocompleteInputID');
						var itemIDID = jQuery("#<?php print $vs_form_name . $vs_field_name_prefix
						                                    . $vs_n; ?>").parent().data('autocompleteItemIDID');
						...
						jQuery("#<?php print $vs_form_name; ?>Errors<?php print $vs_field_name_prefix
						                                                        . $vs_n; ?>").html(content).slideDown(200);
				...
				if (show) {
					var content = '<div class="notification-info-box rounded"><ul class="notification-info-box">' +
						'<li class="notification-info-box"><?php print addslashes( _t( "Really delete annotation? %1 %2",
							caJSButton( $this->request, __CA_NAV_ICON_DELETE__, _t( "Yes" ),
								"{$vs_form_name}{$vs_field_name_prefix}{$vs_n}",
								array( "onclick" => "caDeleteAnnotation(true);" ) ),
							caJSButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "No" ),
								"{$vs_form_name}{$vs_field_name_prefix}{$vs_n}",
								array( "onclick" => "caConfirmDeleteAnnotation(false); return false;" ) )
						) ); ?></li>' +
						'</ul></div>';
					jQuery('#<?php print $vs_form_name; ?>Errors<?php print $vs_field_name_prefix
					                                                        . $vs_n; ?>').html(content).slideDown(200);
			...
			function caDeleteAnnotation() {
				jQuery.getJSON('<?php print caNavUrl( $this->request, '*', '*',
					'deleteAnnotation' ); ?>', {annotation_id: <?php print (int) $vn_subject_id; ?>}, function (resp) {
					if (resp.code == 0) {
			...
			jQuery(document).ready(function () {
				...
				foreach ( $va_notifications as $va_notification ) {
					switch ( $va_notification['type'] ) {
						case __NOTIFICATION_TYPE_ERROR__:
							$vs_content .= "<li class='notification-error-box'>" . $va_notification['message']
							               . "</li>";
							break;
						case __NOTIFICATION_TYPE_WARNING__:
							$vs_content .= "<li class='notification-warning-box'>" . $va_notification['message']
							               . "</li>";
							break;
				...
				content += '</ul></div>';
				jQuery("#<?php print $vs_form_name; ?>Errors<?php print $vs_field_name_prefix
				                                                        . $vs_n; ?>").hide().html(content).slideDown(200);
