...
if ($vb_can_edit) {
	$vs_control_box = caFormControlBox(
		...
		($this->getVar('show_save_and_return') ? caFormSubmitButton($this->request, __CA_NAV_BUTTON_SAVE__, _t("Save and return"), 'RepresentationAnnotationEditorForm', array('isSaveAndReturn' => true)) : '').' '.
		caNavButton($this->request, __CA_NAV_BUTTON_CANCEL__, _t("Cancel"), '', 'editor/representation_annotations', 'RepresentationAnnotationEditor', 'Edit/'.$this->request->getActionExtra(), ($vn_annotation_id ? array('annotation_id' => $vn_annotation_id) : array('type_id' => $t_representation_annotation->getTypeID()))),
		'',
...
$vs_form_elements = $t_representation_annotation->getBundleFormHTMLForScreen(
