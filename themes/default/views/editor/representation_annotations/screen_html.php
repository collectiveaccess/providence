...
if ( $vb_can_edit ) {
	$va_cancel_parameters = ( $vn_annotation_id ? array( 'annotation_id' => $vn_annotation_id )
		: array( 'type_id' => $t_representation_annotation->getTypeID() ) );
	print $vs_control_box = caFormControlBox(
		...
		( $this->getVar( 'show_save_and_return' ) ? caFormSubmitButton( $this->request, __CA_NAV_ICON_SAVE__,
			_t( "Save and return" ), 'RepresentationAnnotationEditorForm', array( 'isSaveAndReturn' => true ) ) : '' )
		. ' ' .
		caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '',
		...
		( ( intval( $vn_annotation_id ) > 0 ) && $vb_can_delete ) ? caFormNavButton( $this->request,
			__CA_NAV_ICON_DELETE__, _t( "Delete" ), '', 'editor/representation_annotations',
			'RepresentationAnnotationEditor', 'Delete/' . $this->request->getActionExtra(),
...
?>
	<div class="sectionBox">
		...
		print caFormTag( $this->request,
			'Save/' . $this->request->getActionExtra() . '/annotation_id/' . $vn_annotation_id,
			'RepresentationAnnotationEditorForm', null, 'POST', 'multipart/form-data' );

		$va_form_elements = $t_representation_annotation->getBundleFormHTMLForScreen( $this->request->getActionExtra(),
			array(
