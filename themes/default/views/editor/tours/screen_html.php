...
if ( $vb_can_edit ) {
	$va_cancel_parameters = ( $vn_tour_id ? array( 'tour_id' => $vn_tour_id )
		: array( 'type_id' => $t_tour->getTypeID() ) );
	print $vs_control_box = caFormControlBox(
		...
		( $this->getVar( 'show_save_and_return' ) ? caFormSubmitButton( $this->request, __CA_NAV_ICON_SAVE__,
			_t( "Save and return" ), 'TourEditorForm', array( 'isSaveAndReturn' => true ) ) : '' ) . ' ' .
		caFormNavButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', 'editor/tours', 'TourEditor',
		...
		( ( intval( $vn_tour_id ) > 0 ) && $vb_can_delete ) ? caFormNavButton( $this->request, __CA_NAV_ICON_DELETE__,
			_t( "Delete" ), '', 'editor/tours', 'TourEditor', 'Delete/' . $this->request->getActionExtra(),
...
<div class="sectionBox">
	<?php
	print caFormTag( $this->request, 'Save/' . $this->request->getActionExtra() . '/tour_id/' . $vn_tour_id,
		'TourEditorForm', null, 'POST', 'multipart/form-data' );
