 ...
 */
	...
	if ($vb_can_edit) {
		...
		print $vs_control_box = caFormControlBox(
			...
			caNavButton($this->request, __CA_NAV_ICON_CANCEL__, _t("Cancel"), '', 'editor/loans', 'LoanEditor', 'Edit/'.$this->request->getActionExtra(), $va_cancel_parameters),
			'', 
			((intval($vn_loan_id) > 0) && $vb_can_delete) ? caNavButton($this->request, __CA_NAV_ICON_DELETE__, _t("Delete"), '', 'editor/loans', 'LoanEditor', 'Delete/'.$this->request->getActionExtra(), array('loan_id' => $vn_loan_id)) : ''
		);
	}
?>
