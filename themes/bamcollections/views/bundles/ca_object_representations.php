...
<?php
			$t_list = new ca_lists();
			$vn_record_type = $t_list->getItemIDFromList("object_types", "docs"); 
			
			if (($t_subject->tableName() != 'ca_collections') && ($t_subject->tableName() != 'ca_occurrences') && ($t_subject->get('type_id') != $vn_record_type)) {
				if ($t_item_rel->hasField('type_id')) {
				 	print "<h2>"._t('Add representation with relationship type %1', $t_item_rel->getRelationshipTypesAsHTMLSelect($vs_rel_dir, $vn_left_sub_type_id, $vn_right_sub_type_id, array('name' => '{fieldNamePrefix}type_id_{n}'), $va_settings))."</h2>";
				 } else {
				 	print "<h2>"._t('Add representation')."</h2>";
				 }
			} elseif ($t_subject->get('type_id') == $vn_record_type){
				print "<h2>"._t('Add Documentation')."</h2>";	 	
			} else {
				print "<h2>".($t_item_rel->hasField('type_id')) ? _t('Document Type %1', $t_item_rel->getRelationshipTypesAsHTMLSelect($vs_rel_dir, $vn_left_sub_type_id, $vn_right_sub_type_id, array('name' => '{fieldNamePrefix}type_id_{n}'), $va_settings)) : _t('Add representation')."</h2>"; 
			}
?>
