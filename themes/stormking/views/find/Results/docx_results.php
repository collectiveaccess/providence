 ...
 */
	...
	$vs_header_label 		= $this->getVar('criteria_summary_truncated');
	
	if (substr($vs_header_label, 0, 4) == 'set:'){
		$vs_set_string = explode(':',$vs_header_label);
		$vs_set_code = $vs_set_string[1];
		$vs_set_code = str_replace('"', "", $vs_set_code);
		$t_set = new ca_sets();
		$t_set->load(array('set_code' => $vs_set_code));
		$vs_search_string = $t_set->get('ca_sets.preferred_labels');
	}
    ...
    $styleHeaderFont = array('bold'=>true, 'size'=>13, 'name'=>'Arial');
    $styleBundleNameFont = array('bold'=>false, 'underline'=>'single', 'color'=>'666666', 'size'=>10, 'name'=>'Arial');
	$styleContentFont = array('bold'=>false, 'size'=>10, 'name'=>'Arial');
	$styleNoImage = array('bold'=>false, 'size'=>12, 'name'=>'Arial', 'color'=>'cccccc', 'align'=>'center', 'valign'=>'center');
	
// Define table style arrays
$styleTable = array('borderSize'=>1, 'borderColor'=>'000000', 'cellMargin'=>200);
$styleFirstRow = array('borderSize'=>0, 'borderBottomColor'=>'0000FF');

// Define cell style arrays
$styleCell = array('valign'=>'center');
$styleCellBTLR = array('valign'=>'center');
$styleCellNoImage = array('valign'=>'center', 'align'=>'center');
...
$cellColSpan = array('gridSpan' => 2, 'align'=>'center');
	...
	while($vo_result->nextHit()) {
		...
		if ($vn_row_count == 0){
			$table->addRow();
			$headerCell = $table->addCell(12 * $cmToTwips, $cellColSpan);   
			$headerCell->addText($vs_search_string, $fontStyle, $textCenter);
		}
		$table->addRow();
    	$list = $va_display_list;

		// First column : media
		$mediaCell = $table->addCell( 5 * $cmToTwips, $styleCellNoImage);
		...
		if($va_info['MIMETYPE'] == 'image/jpeg') { // don't try to insert anything non-jpeg into an Excel file
			...
			if (is_file($vs_path)) {
				...
				$mediaCell->addText('no image', $styleNoImage);
		}
