...
if ($this->request->config->get('summary_header_enabled')) {
	...
	/* @var $t_display ca_bundle_displays */
	$t_display = $this->getVar('t_display');
	$vs_display_name = $t_display->getLabelForDisplay();
	$vs_footer .= "<td class='footerText'>{$vs_display_name}</td>";
	...
	switch ($this->getVar('PDFRenderer')) {
		...
		case 'wkhtmltopdf':
			...
			<head>
				<link type="text/css"
				      href="<?php print $this->getVar('base_path'); ?>/pdf.css"
				      rel="stylesheet"/>
			</head>
			<body>
			<?php
