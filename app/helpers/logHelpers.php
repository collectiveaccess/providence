 ...
 *                       configured log directory is not writable. [Default is false]
 *
 * @param string $opt_name Name of app.conf configuration entry to use for log directory. [Default is null - use
...
function caGetLogger( $options = null, $opt_name = null ) {
	...
	return new KLogger( $log_dir, caLogLevelStringToNumber( caGetOption( 'logLevel', $options, 'INFO' ) ),
		caGetOption( 'logName', $options, null ) );
}
...
function caGetLogPath( $options = null, $opt_name = null ) {
	...
	if ( ! is_writeable( $log_dir ) ) {
		if ( ! caGetOption( 'logToTempDirectoryIfLogDirectoryIsNotWritable', $options, false ) ) {
			throw new ApplicationException( _t( "Cannot write log to %1. Please check the directory's permissions and retry.",
				$log_dir ) );
		} elseif ( is_writable( $tmp_dir = caGetTempDirPath() ) ) {
			$log_dir = $tmp_dir;
		} else {
			throw new ApplicationException( _t( "Cannot write log to %1 or temporary directory %2. Please check directory permissions and retry.",
				$log_dir, $tmp_dir ) );
		}
	}
