...
function caGetLogger($options = null, $opt_name = null)
{
    ...
    return new KLogger(
        $log_dir,
        caLogLevelStringToNumber(caGetOption('logLevel', $options, 'INFO')),
    ...
    if (!is_writeable($log_dir)) {
        if (!caGetOption('logToTempDirectoryIfLogDirectoryIsNotWritable', $options, false)) {
            throw new ApplicationException(
                _t("Cannot write log to %1. Please check the directory's permissions and retry.", $log_dir)
            );
            ...
            throw new ApplicationException(
                _t(
                    "Cannot write log to %1 or temporary directory %2. Please check directory permissions and retry.",
                    $log_dir,
                    $tmp_dir
                )
            );
