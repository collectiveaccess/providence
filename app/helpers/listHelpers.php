...
function caGetListItemIdno($pn_item_id, $pa_options = null)
{
    ...
    if (is_array($pa_check_access = caGetOption('checkAccess', $pa_options, null)) && (sizeof(
                $pa_check_access
            ) > 0) && !in_array($t_item->get('ca_list_items.access'), $pa_check_access)) {
        return null;
    }
...
function caGetListItemForDisplay($ps_list_code, $ps_idno, $pa_options = null)
{
    ...
    return $g_list_item_label_cache[$vs_cache_key] = $t_list->getItemFromListForDisplay(
        $ps_list_code,
        $ps_idno,
        $pa_options
    );
...
function caGetListItemIDForValue($ps_list_code, $ps_value, $pa_options = null)
{
    ...
    if (!caGetOption(['noCache', 'dontCache'], $pa_options, false)) {
        if (isset($g_list_item_ids_for_values[$vs_cache_key])) {
            return $g_list_item_ids_for_values[$vs_cache_key];
        }
    }
...
function caGetListItemIDForLabel($ps_list_code, $ps_label, $pa_options = null)
{
    ...
    return $g_list_item_id_for_label_cache[$vs_cache_key] = $t_list->getItemIDFromListByLabel(
        $ps_list_code,
        $ps_label,
        $pa_options
    );
...
function caGetDefaultItemID($ps_list_code, $pa_options = null)
{
    ...
    return $g_default_list_item_id_cache[$vs_cache_key] = $t_list->getDefaultItemID(
        $ps_list_code,
        array_merge(
            $pa_options,
            ['useFirstElementAsDefaultDefault' => true]
        )
    );
}
...
function caGetAncestorsForItemID($pm_item_id, $pa_options = null)
{
    ...
    foreach ($pm_item_id as $pn_item_id) {
        if (is_array(
            $va_ancestors = $t_item->getHierarchyAncestors(
                $pn_item_id,
...
function caGetLabelTypeList($pm_table_name_or_num, $pb_preferred = true)
{
    ...
    return $o_conf->get(
        $pb_preferred ? "{$vs_table_name}_preferred_label_type_list" : "{$vs_table_name}_nonpreferred_label_type_list"
    );
}
