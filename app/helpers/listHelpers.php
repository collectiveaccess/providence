 ...
 *                             filtering is performed. [Default is null]
 *
...
function caGetListName( $pm_list_code_or_id, $pa_options = null ) {
	...
	if ( is_numeric( $pm_list_code_or_id ) ) {
		...
		if ( ! ( $t_list = ca_lists::find( [ 'list_code' => $pm_list_code_or_id ],
 ...
 *                          noCache = Don't use cache. [Default is false]
 *                          dontCache = Synonym for noCache
 *                          checkAccess = Array of access values to filter returned values on. If omitted no filtering
...
function caGetListItemIdno( $pn_item_id, $pa_options = null ) {
	...
	if ( is_array( $pa_check_access = caGetOption( 'checkAccess', $pa_options, null ) )
	     && ( sizeof( $pa_check_access ) > 0 )
	     && ! in_array( $t_item->get( 'ca_list_items.access' ), $pa_check_access )
	) {
		return null;
	}
 ...
 * @param bool   $pb_return_plural If true, return plural version of label. Default is to return singular version of
 *                                 label.
 * @param array  $pa_options       Options include:
 *                                 transaction = transaction to execute queries within. [Default=null]
 *                                 noCache = Don't use cache. [Default is false]
...
function caGetListItemForDisplay( $ps_list_code, $ps_idno, $pa_options = null ) {
	...
	return $g_list_item_label_cache[ $vs_cache_key ] = $t_list->getItemFromListForDisplay( $ps_list_code, $ps_idno,
		$pa_options );
}
...
function caGetListItemIDForLabel( $ps_list_code, $ps_label, $pa_options = null ) {
	...
	return $g_list_item_id_for_label_cache[ $vs_cache_key ] = $t_list->getItemIDFromListByLabel( $ps_list_code,
		$ps_label, $pa_options );
}
...
function caGetListItems( $ps_list_code, $pa_options = null ) {
	...
	foreach ( $list_items as $id => $item ) {
		...
		$list[ isset( $item[ $key ] ) ? $item[ $key ] : $id ] = isset( $item[ $value ] ) ? $item[ $value ]
			: $item['name_plural'];
	}
	ksort( $list );
...
function caGetDefaultItemID( $ps_list_code, $pa_options = null ) {
	...
	return $g_default_list_item_id_cache[ $vs_cache_key ] = $t_list->getDefaultItemID( $ps_list_code,
		array_merge( $pa_options, [ 'useFirstElementAsDefaultDefault' => true ] ) );
}
 ...
 *                          dontCache = Synonym for noCache
 *
 * @return array An array of item_ids for items that are ancestors of the specified item. The specified item_id is only
 *               included in the returned list if the includeSelf option is set.
 */
...
function caGetAncestorsForItemID( $pm_item_id, $pa_options = null ) {
	...
	foreach ( $pm_item_id as $pn_item_id ) {
		if ( is_array( $va_ancestors = $t_item->getHierarchyAncestors( $pn_item_id,
			[ 'idsOnly' => true, 'includeSelf' => caGetOption( 'includeSelf', $pa_options, false ) ] ) )
		) {
...
function caGetTypeList( $pm_table_name_or_num, $pa_options = null ) {
	if ( ( $t_instance = Datamodel::getInstance( $pm_table_name_or_num ) )
	     && ( method_exists( $t_instance, "getTypeList" ) )
	) {
		if ( $o_trans = caGetOption( 'transaction', $pa_options, null ) ) {
...
function caGetLabelTypeList( $pm_table_name_or_num, $pb_preferred = true ) {
	...
	return $o_conf->get( $pb_preferred ? "{$vs_table_name}_preferred_label_type_list"
		: "{$vs_table_name}_nonpreferred_label_type_list" );
}
