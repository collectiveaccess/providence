...
function caPuppySearch($po_request, $ps_search_expression, $pa_blocks, $pa_options = null)
{
    ...
    //
    $vb_ajax_mode = false;
    if ($po_request->isAjax() && ($ps_block = $po_request->getParameter(
            'block',
            pString
    ...
    foreach ($pa_blocks as $vs_block => $va_block_info) {
        ...
        if (!($ps_sort = $po_request->getParameter("{$vs_block}Sort", pString))) {
            if (isset($va_contexts[$vs_block])) {
                if ((!($ps_sort = $va_contexts[$vs_block]->getCurrentSort()) && ($va_sorts) && sizeof(
                            $va_sorts
        ...
        $qr_res = $o_search->search(
            trim($ps_search_expression) . (($vb_match_on_stem && !preg_match(
                    '![\*\"\']$!',
        ...
        $o_view->setVar(
            'sortBySelect',
            $vs_sort_by_select = (is_array($va_sort_dd) ? caHTMLSelect(
                "{$vs_block}_sort",
                $va_sort_dd,
                array('id' => "{$vs_block}_sort", "class" => "form-control input-sm"),
                array("value" => $ps_sort)
            ) : '')
        );
        $o_view->setVar(
            'sortByControl',
            ($va_block_info["sortControlType"] && ($va_block_info["sortControlType"] == "list")) ? $vs_sort_list : $vs_sort_by_select
        ...
        $o_view->setVar(
            ...
            '<a href="#" id="' . $vs_block . '_sort_direction"><span class="glyphicon glyphicon-sort-by-alphabet' . (($ps_sort_direction == 'desc') ? '-alt' : '') . '"></span></a>'
        );
...
function caGetQueryStringForHTMLFormInput($po_result_context, $pa_options = null)
{
        ...
        switch ($vs_element) {
            case '_fieldlist':
                foreach ($pa_form_values[$vs_dotless_element . '_field'] as $vn_j => $vs_fieldlist_field) {
                    ...
                    $va_default_values['_fieldlist_value'][] = trim(
                        $pa_form_values[$vs_dotless_element . '_value'][$vn_j]
                    );
    ...
    $po_result_context->setParameter(
        "pawtucketAdvancedSearchFormContent_{$pa_form_values['_advancedFormName']}",
        $va_default_values
    );
    $po_result_context->setParameter(
        "pawtucketAdvancedSearchFormBooleans_{$pa_form_values['_advancedFormName']}",
        $va_booleans
    );
    ...
    if (is_array($va_values) && sizeof($va_values)) {
        foreach ($va_values as $vs_element => $va_value_list) {
            foreach ($va_value_list as $vn_i => $vs_value) {
                ...
                $vs_query_element .= ($vb_match_on_stem && !preg_match('!\*$!', $vs_query_element) && preg_match(
                        '!^[\w]+$!',
                        $vs_query_element
...
function caGetDisplayStringForHTMLFormInput($po_result_context, $pa_options = null)
{
    ...
    foreach ($va_form_contents as $vn_i => $vs_element) {
        ...
        if (
            ...
            ||
            (is_array($pa_form_values[$vs_dotless_element]) && !sizeof(
                    array_filter($pa_form_values[$vs_dotless_element])
                ))
        ) {
            continue;
        }
            ...
            switch ($va_fld[1]) {
                ...
                case $t_table->primaryKey():
                    $va_display_string[] = join(
                        ", ",
                        $t_table->getPreferredDisplayLabelsForIDs(
                            $pa_form_values[$vs_dotless_element]
                        )
                    );
                    break(2);
                default:
    ...
    $po_result_context->setParameter(
        "pawtucketAdvancedSearchFormDisplayString_{$pa_form_values['_advancedFormName']}",
        $va_display_string
    );
    $po_result_context->saveContext();
    ...
    foreach ($va_items as $id => $subquery) {
        ...
        switch (get_class($subquery)) {
            case 'Zend_Search_Lucene_Search_Query_Phrase':
                ...
                $va_query[] = ($vs_field_disp && !$pb_omit_field_names ? "{$vs_field_disp}: \"" : "") . caGetDisplayValueForBundle(
                        $vs_field,
                        join(" ", $va_terms)
                    ) . "\"";
                break;
            ...
            case 'Zend_Search_Lucene_Search_Query_Range':
                ...
                $va_query[] = ($vs_field && !$pb_omit_field_names ? "{$vs_field}: " : "") . _t(
                        "%1 to %2",
                        $subquery->getLowerTerm()->text,
                        $subquery->getUpperTerm()->text
                    );
                break;
...
function caGetNamedSearch($ps_name, $pa_values, $pa_options = null)
{
    ...
    if ($va_named_search_config = caGetNamedSearchConfig($ps_name)) {
        if (caGetOption('display', $pa_options, false)) {
            if (isset($va_named_search_config['displayCallback']) && function_exists(
                    $va_named_search_config['displayCallback']
                )) {
...
function caGetBundlesAvailableForSearch($pm_table_name_or_num, $pa_options = null)
{
    ...
    $va_access_points = (isset($va_search_settings['_access_points']) && is_array(
        ...
        )) ? $va_search_settings['_access_points'] : array();
    ...
    if ($pb_for_select) {
        foreach ($va_sorted_bundles as $vs_label => $vs_key) {
            ...
            if (preg_match('!_labels$!', $va_tmp[0])) {
                if (($t_label_instance = Datamodel::getInstanceByTableName($va_tmp[0], true)) && (is_a(
                        $t_label_instance,
                        'BaseLabel'
                    ))) {
                    $va_sorted_bundles[$vs_label] = $t_label_instance->getSubjectTableName(
                        ) . '.preferred_labels' . ($va_tmp[1] ? '.' . $va_tmp[1] : '');
                }
            }
        }
    }
...
function caGetAvailableSortFields($ps_table, $pn_type_id = null, $pa_options = null)
{
    ...
    if (CompositeCache::contains("available_sorts") && is_array(
            $va_cached_data = CompositeCache::fetch("available_sorts")
    ...
    if ($ps_related_table = caGetOption('includeInterstitialSortsFor', $pa_options, null)) {
        if (is_array($va_path = array_keys(Datamodel::getPath($ps_table, $ps_related_table))) && (sizeof(
                    $va_path
                ) == 3)) {
    ...
    if (isset($pa_options['request']) && ($t_ui = ca_editor_uis::loadDefaultUI(
            $ps_table,
            $pa_options['request'],
            $pn_type_id
        ))) {
            ...
            if (is_array($va_placements = $t_ui->getScreenBundlePlacements($va_screen['screen_id']))) {
                foreach ($va_placements as $va_placement) {
                    ...
                    if (isset($va_placement['settings']['label'])) {
                        if ($vs_label_tmp = isset($va_placement['settings']['label'][$g_ui_locale_id]) ? $va_placement['settings']['label'][$g_ui_locale_id] : array_shift(
                            $va_placement['settings']['label']
                        )) {
    ...
    switch ($ps_table) {
        ...
        case 'ca_entities':
            $va_base_fields = array(
                ...
                'ca_entities.type_id;ca_entities.preferred_labels.surname;ca_entities.preferred_labels.forename' => _t(
                    'type'
                ),
    ...
    if ($ps_table) {
        ...
        if (caGetOption('includeUserSorts', $pa_options, true)) {
            ...
            if (!($po_request = caGetOption('request', $pa_options)) || ($po_request->getUser()->canDoAction(
                    'can_use_user_sorts'
                ))) {
        ...
        if ($t_rel) {
            $va_sortable_elements = ca_metadata_elements::getSortableElements(
                $vs_relation_table = $t_rel->tableName(),
                null,
                ['indexByElementCode' => true]
            );

            $pb_distinguish_interstitials = caGetOption('distinguishInterstitials', $pa_options, true);
            foreach ($va_sortable_elements as $vn_element_id => $va_sortable_element) {
                $va_base_fields[$vs_relation_table . '.' . $va_sortable_element['element_code']] = $va_sortable_element['display_label'] . ($pb_distinguish_interstitials ? " (" . _t(
                            'Interstitial'
                        ) . ")" : "");
            }
    ...
    if ($pn_display_id > 0) {
        ...
        $va_display_bundles = array_map(
            function ($v) {
                return $v['bundle_name'];
            },
            $t_display->getPlacements()
        );
        $va_base_fields = array_filter(
            $va_base_fields,
...
function caGetSortForDisplay($ps_table, $ps_sort_fields, $pa_options = null)
{
    ...
    $va_available_sorts = caGetAvailableSortFields(
        $ps_table,
        null,
        ...
        switch (get_class($subquery)) {
            case 'Zend_Search_Lucene_Search_Query_Phrase':
                foreach ($subquery->getQueryTerms() as $o_term) {
                    ...
                    } elseif ((in_array($vs_field, ['ca_sets.set_code', 'set'])) && ($vn_set_id = ca_sets::find(
                            ['set_code' => $vs_value],
                            ['returnAs' => 'firstId']
                        ))) {
