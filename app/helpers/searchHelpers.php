...
function caSearchLink(
	$po_request, $ps_content, $ps_classname, $ps_table, $ps_search, $pa_other_params = null, $pa_attributes = null,
	$pa_options = null
) {
 ...
 * @return array|string
 */
function caSearchUrl(
	$po_request, $ps_table, $ps_search = null, $pb_return_url_as_pieces = false, $pa_additional_parameters = null,
	$pa_options = null
) {
	...
	switch ( $ps_table ) {
		...
		case 56:
			$vs_module     = 'find';
			$vs_controller = ( $vb_return_advanced ) ? 'SearchObjectRepresentationsAdvanced'
				: 'SearchObjectRepresentations';
			$vs_action     = 'Index';
			break;
		case 'ca_representation_annotations':
		case 82:
			$vs_module     = 'find';
			$vs_controller = ( $vb_return_advanced ) ? 'SearchRepresentationAnnotationsAdvanced'
				: 'SearchRepresentationAnnotations';
			$vs_action     = 'Index';
			break;
		case 'ca_user_representation_annotations':
		case 219:
			$vs_module     = 'find';
			$vs_controller = ( $vb_return_advanced ) ? 'SearchUserRepresentationAnnotationsAdvanced'
				: 'SearchUserRepresentationAnnotations';
			$vs_action     = 'Index';
			break;
		case 'ca_relationship_types':
		case 79:
...
function caPuppySearch( $po_request, $ps_search_expression, $pa_blocks, $pa_options = null ) {
	...
	//
	$vb_ajax_mode = false;
	if ( $po_request->isAjax() && ( $ps_block = $po_request->getParameter( 'block', pString ) )
	     && isset( $pa_blocks[ $ps_block ] )
	) {
	...
	foreach ( $pa_blocks as $vs_block => $va_block_info ) {
		...
		if ( ! ( $ps_sort = $po_request->getParameter( "{$vs_block}Sort", pString ) ) ) {
			if ( isset( $va_contexts[ $vs_block ] ) ) {
				if ( ( ! ( $ps_sort = $va_contexts[ $vs_block ]->getCurrentSort() ) && ( $va_sorts )
				       && sizeof( $va_sorts ) )
				     || ( ! in_array( $ps_sort, array_keys( $va_sorts ) ) )
				) {
		...
		$qr_res = $o_search->search( trim( $ps_search_expression ) . ( ( $vb_match_on_stem
		                                                                 && ! preg_match( '![\*\"\']$!',
					$ps_search_expression ) ) ? '*' : '' ), $va_options );
		...
		if ( is_array( $va_sort_by ) ) {
			...
			foreach ( $va_sort_by as $vs_sort_label => $vs_sort ) {
				$va_sort_list[]               = "<li" . ( ( $vs_sort_label == $ps_sort ) ? " class='selectedSort'"
						: '' ) . "><a href='#' rel='{$vs_sort_label}'>{$vs_sort_label}</a></li>";
		...
		$o_view->setVar( 'sortBySelect',
			$vs_sort_by_select = ( is_array( $va_sort_dd ) ? caHTMLSelect( "{$vs_block}_sort", $va_sort_dd,
				array( 'id' => "{$vs_block}_sort", "class" => "form-control input-sm" ), array( "value" => $ps_sort ) )
				: '' ) );
		$o_view->setVar( 'sortByControl',
			( $va_block_info["sortControlType"] && ( $va_block_info["sortControlType"] == "list" ) ) ? $vs_sort_list
				: $vs_sort_by_select ); // synonym for sortBySelect
		$o_view->setVar( 'sortByList', $vs_sort_list );
		$o_view->setVar( 'sort', $ps_sort );
		$o_view->setVar( 'accessValues', $va_access_values );

		$o_view->setVar( 'sortDirectionControl',
			'<a href="#" id="' . $vs_block . '_sort_direction"><span class="glyphicon glyphicon-sort-by-alphabet'
			. ( ( $ps_sort_direction == 'desc' ) ? '-alt' : '' ) . '"></span></a>' );
...
function caGetQueryStringForHTMLFormInput( $po_result_context, $pa_options = null ) {
		...
		switch ( $vs_element ) {
			case '_fieldlist':
				foreach ( $pa_form_values[ $vs_dotless_element . '_field' ] as $vn_j => $vs_fieldlist_field ) {
					...
					$va_values[ $vs_fieldlist_field ][]      = trim( $pa_form_values[ $vs_dotless_element
					                                                                  . '_value' ][ $vn_j ] );
					$va_default_values['_fieldlist_field'][] = $vs_fieldlist_field;
					$va_default_values['_fieldlist_value'][] = trim( $pa_form_values[ $vs_dotless_element
					                                                                  . '_value' ][ $vn_j ] );
					$va_booleans["_fieldlist:boolean"][]
					                                         = $va_booleans["{$vs_fieldlist_field}:boolean"][]
						= isset( $pa_form_values["_fieldlist:boolean"][ $vn_j ] )
						? $pa_form_values["_fieldlist:boolean"][ $vn_j ] : null;
			...
			default:
				...
				if ( is_array( $pa_form_values[ $vs_dotless_element ] ) ) {
					...
					if ( is_array( $pa_form_values[ $vs_dotless_element . ':relationshipTypes' ] ) ) {
						$vs_element_rel_type = "/" . join( ";",
								$pa_form_values[ $vs_dotless_element . ':relationshipTypes' ] );
					}
					...
					if ( isset( $pa_form_values["{$vs_dotless_element}{$vs_element_rel_type}_autocomplete"] ) ) {
						...
						foreach (
							$pa_form_values["{$vs_dotless_element}{$vs_element_rel_type}"] as $vn_j => $vs_element_value
						) {
							if ( $t_table ) {
							...
							$va_booleans["{$vs_search_element}{$vs_element_rel_type}:boolean"][]
							                                                          = isset( $pa_form_values["{$vs_dotless_element}{$vs_element_rel_type}:boolean"][ $vn_j ] )
								? $pa_form_values["{$vs_dotless_element}{$vs_element_rel_type}:boolean"][ $vn_j ]
								: null;
						}
						continue( 2 );
					}
					foreach (
						...
						$va_booleans["{$vs_element}{$vs_element_rel_type}:boolean"][]
						                                                           = isset( $pa_form_values["{$vs_dotless_element}{$vs_element_rel_type}:boolean"][ $vn_j ] )
							? $pa_form_values["{$vs_dotless_element}{$vs_element_rel_type}:boolean"][ $vn_j ] : null;
					}
				}
				break;
		}
	...
	$po_result_context->setParameter( "pawtucketAdvancedSearchFormContent_{$pa_form_values['_advancedFormName']}",
		$va_default_values );
	$po_result_context->setParameter( "pawtucketAdvancedSearchFormBooleans_{$pa_form_values['_advancedFormName']}",
		$va_booleans );
	...
	if ( is_array( $va_values ) && sizeof( $va_values ) ) {
		foreach ( $va_values as $vs_element => $va_value_list ) {
			foreach ( $va_value_list as $vn_i => $vs_value ) {
				...
				$vs_query_element .= ( $vb_match_on_stem && ! preg_match( '!\*$!', $vs_query_element )
				                       && preg_match( '!^[\w]+$!', $vs_query_element )
				                       && ! preg_match( '!^[0-9]+$!', $vs_query_element ) ) ? '*' : '';

				$va_query_booleans[ $vs_element ][] = ( isset( $va_booleans["{$vs_element}:boolean"][ $vn_i ] )
				                                        && $va_booleans["{$vs_element}:boolean"][ $vn_i ] )
					? $va_booleans["{$vs_element}:boolean"][ $vn_i ] : 'AND';
				switch ( $vs_element ) {
					...
					case '_fieldlist_field':
						...
						$va_query_elements[ $vs_element ][] = "(" . $va_values['_fieldlist_field'][ $vn_i ] . ":"
						                                      . $pa_form_values['_fieldlist_value'][ $vn_i ] . ")";
						break;
					default:
	...
	foreach ( $va_query_elements as $vs_element => $va_query_elements_by_element ) {
		$vs_query_string .= ( $vs_query_string ? ( ( $vs_b = $va_query_booleans[ $vs_element ][0] ) ? " {$vs_b} "
				: ' AND ' ) : '' ) . '(';
...
function caGetDisplayStringForHTMLFormInput( $po_result_context, $pa_options = null ) {
	...
	foreach ( $va_form_contents as $vn_i => $vs_element ) {
		...
		if (
			( ! is_array( $pa_form_values[ $vs_dotless_element ] )
			  && ! strlen( $pa_form_values[ $vs_dotless_element ] ) )
			|| ( is_array( $pa_form_values[ $vs_dotless_element ] )
			     && ! sizeof( array_filter( $pa_form_values[ $vs_dotless_element ] ) ) )
		) {
			continue;
		}
		...
		if ( isset( $pa_form_values["{$vs_dotless_element}_autocomplete"] ) ) {
			if ( ! is_array( $pa_form_values["{$vs_dotless_element}_autocomplete"] ) ) {
				$pa_form_values["{$vs_dotless_element}_autocomplete"]
					= [ $pa_form_values["{$vs_dotless_element}_autocomplete"] ];
			}

			foreach ( $pa_form_values[ $vs_dotless_element ] as $vn_j => $vs_element_value ) {
				$pa_form_values[ $vs_dotless_element ][ $vn_j ]
					= $pa_form_values["{$vs_dotless_element}_autocomplete"][ $vn_j ];
			}
		}
		$va_fld = explode( ".", $vs_element );
		...
		if ( $t_table && ( $t_table->hasField( $va_fld[1] ) ) ) {
			switch ( $va_fld[1] ) {
				...
				case $t_table->primaryKey():
					$va_display_string[] = join( ", ",
						$t_table->getPreferredDisplayLabelsForIDs( $pa_form_values[ $vs_dotless_element ] ) );
					break( 2 );
				default:
	...
	$po_result_context->setParameter( "pawtucketAdvancedSearchFormDisplayString_{$pa_form_values['_advancedFormName']}",
		$va_display_string );
	$po_result_context->saveContext();
 ...
 *                           omitFieldNames = Omit field names (aka. qualifiers) in display string. Only search terms
 *                           will be included. [Default is false] omitQualifiers = Synonym for omitFieldNames
 *
 * @return string
 */
	...
	foreach ( $va_items as $id => $subquery ) {
		...
		switch ( get_class( $subquery ) ) {
			case 'Zend_Search_Lucene_Search_Query_Phrase':
				...
				$va_query[]    = ( $vs_field_disp && ! $pb_omit_field_names ? "{$vs_field_disp}: \"" : "" )
				                 . caGetDisplayValueForBundle( $vs_field, join( " ", $va_terms ) ) . "\"";
				break;
			...
			case 'Zend_Search_Lucene_Search_Query_Term':
				...
				$va_query[] = ( $vs_field && ! $pb_omit_field_names ? "{$vs_field}: " : "" )
				              . $subquery->getTerm()->text;
				break;
			case 'Zend_Search_Lucene_Search_Query_Range':
				$vs_field   = caGetLabelForBundle( $subquery->getLowerTerm()->field );
				$va_query[] = ( $vs_field && ! $pb_omit_field_names ? "{$vs_field}: " : "" ) . _t( "%1 to %2",
						$subquery->getLowerTerm()->text, $subquery->getUpperTerm()->text );
				break;
		...
		case 'Zend_Search_Lucene_Search_Query_Boolean':
			...
			foreach ( $po_parsed_query->getSubqueries() as $o_q ) {
				...
				$va_items      = array_merge( $va_items,
					is_array( $va_subqueries['items'] ) ? $va_subqueries['items'] : [] );
				$va_signs      = array_merge( $va_signs,
					is_array( $va_subqueries['signs'] ) ? $va_subqueries['signs'] : [] );
			}
			break;
...
function caGetNamedSearch( $ps_name, $pa_values, $pa_options = null ) {
	...
	if ( $va_named_search_config = caGetNamedSearchConfig( $ps_name ) ) {
		if ( caGetOption( 'display', $pa_options, false ) ) {
			if ( isset( $va_named_search_config['displayCallback'] )
			     && function_exists( $va_named_search_config['displayCallback'] )
			) {
...
# ---------------------------------------
/**
 * Returns all available search form placements - those data bundles that can be searches for the given content type,
 * in other words. The returned value is a list of arrays; each array contains a 'bundle' specifier than can be passed
 * got Model::get() or SearchResult::get() and a display name
 *
...
function caGetBundlesAvailableForSearch( $pm_table_name_or_num, $pa_options = null ) {
	...
	foreach ( $va_search_settings as $vs_table => $va_fields ) {
		...
		if ( $vs_table == $vs_primary_table ) {
			$va_element_codes = ( method_exists( $t_instance, 'getApplicableElementCodes' )
				? $t_instance->getApplicableElementCodes( null, false, false ) : array() );
			...
			foreach ( $va_field_list as $vs_field => $va_field_indexing_info ) {
				...
				if ( is_array( $pa_filter ) && sizeof( $pa_filter )
				     && ! in_array( $vs_table . '.' . $vs_field, $pa_filter )
				) {
					continue;
				}
	...
	$va_access_points = ( isset( $va_search_settings['_access_points'] )
	                      && is_array( $va_search_settings['_access_points'] ) ) ? $va_search_settings['_access_points']
		: array();
	...
	foreach ( $va_access_points as $vs_access_point => $va_access_point_info ) {
		...
		$vs_label                                              = ( ( isset( $va_access_point_info['name'] )
		                                                             && $va_access_point_info['name'] )
			? $va_access_point_info['name'] : $vs_access_point );
	...
	if ( $pb_for_select ) {
		foreach ( $va_sorted_bundles as $vs_label => $vs_key ) {
			...
			if ( preg_match( '!_labels$!', $va_tmp[0] ) ) {
				if ( ( $t_label_instance = Datamodel::getInstanceByTableName( $va_tmp[0], true ) )
				     && ( is_a( $t_label_instance, 'BaseLabel' ) )
				) {
					$va_sorted_bundles[ $vs_label ] = $t_label_instance->getSubjectTableName() . '.preferred_labels'
					                                  . ( $va_tmp[1] ? '.' . $va_tmp[1] : '' );
				}
			}
		}
...
function caRewriteDateForElasticSearch( $ps_date, $pb_is_start = true ) {
	...
	if ( preg_match( "/^([\-]{0,1}[\d]+)\-(\d+)\-(\d+)T(\d+)\:(\d+)\:(\d+)Z/", $ps_date, $va_date_parts ) ) {
		...
		// fix messed up days
		$vn_days_in_month = cal_days_in_month( CAL_GREGORIAN, intval( $va_date_parts[2] ),
			intval( abs( $va_date_parts[1] ) ) );
...
function caGetAvailableSortFields( $ps_table, $pn_type_id = null, $pa_options = null ) {
	...
	if ( CompositeCache::contains( "available_sorts" )
	     && is_array( $va_cached_data = CompositeCache::fetch( "available_sorts" ) )
	...
	if ( $ps_related_table = caGetOption( 'includeInterstitialSortsFor', $pa_options, null ) ) {
		if ( is_array( $va_path = array_keys( Datamodel::getPath( $ps_table, $ps_related_table ) ) )
		     && ( sizeof( $va_path ) == 3 )
		) {
	...
	if ( isset( $pa_options['request'] )
	     && ( $t_ui = ca_editor_uis::loadDefaultUI( $ps_table, $pa_options['request'], $pn_type_id ) )
	) {
			...
			if ( is_array( $va_placements = $t_ui->getScreenBundlePlacements( $va_screen['screen_id'] ) ) ) {
				foreach ( $va_placements as $va_placement ) {
					...
					if ( isset( $va_placement['settings']['label'] ) ) {
						if ( $vs_label_tmp = isset( $va_placement['settings']['label'][ $g_ui_locale_id ] )
							? $va_placement['settings']['label'][ $g_ui_locale_id ]
							: array_shift( $va_placement['settings']['label'] )
						) {
	...
	if ( $ps_table ) {
		...
		if ( caGetOption( 'includeUserSorts', $pa_options, true ) ) {
			...
			if ( ! ( $po_request = caGetOption( 'request', $pa_options ) )
			     || ( $po_request->getUser()->canDoAction( 'can_use_user_sorts' ) )
			) {
		...
		foreach ( $va_sortable_elements as $vn_element_id => $va_sortable_element ) {
			$va_base_fields[ $ps_table . '.' . $va_sortable_element['element_code'] ]
				= $va_sortable_element['display_label'];
		}

		// Add interstitial sorts
		if ( $t_rel ) {
			$va_sortable_elements = ca_metadata_elements::getSortableElements( $vs_relation_table = $t_rel->tableName(),
				null, [ 'indexByElementCode' => true ] );

			$pb_distinguish_interstitials = caGetOption( 'distinguishInterstitials', $pa_options, true );
			foreach ( $va_sortable_elements as $vn_element_id => $va_sortable_element ) {
				$va_base_fields[ $vs_relation_table . '.' . $va_sortable_element['element_code'] ]
					= $va_sortable_element['display_label'] . ( $pb_distinguish_interstitials ? " ("
					                                                                            . _t( 'Interstitial' )
					                                                                            . ")" : "" );
			}
		...
		if ( is_array( $config_sorts = $o_config->get( "{$ps_table}_include_result_sorts" ) )
		     && sizeof( $config_sorts )
		) {
		...
		if ( caGetOption( 'distinguishNonUniqueNames', $pa_options, true ) ) {
			foreach (
				array_count_values( array_filter( $va_base_fields, function ( $v ) {
					return ! is_null( $v );
				} ) ) as $vn_v => $vn_c
			) {
				if ( $vn_c > 1 ) {
					foreach ( array_keys( $va_base_fields, $vn_v ) as $vs_k ) {
						...
						if ( is_array( $va_sortable_elements[ $vs_code ]['typeRestrictions'] ) ) {
							$va_restrictions = [];
							foreach (
								$va_sortable_elements[ $vs_code ]['typeRestrictions'] as $vs_table => $va_type_list
							) {
	...
	if ( $pn_display_id > 0 ) {
		...
		$va_base_fields     = array_filter( $va_base_fields,
			function ( $v, $k ) use ( $va_display_bundles, $config_sorts ) {
...
function caGetSortForDisplay( $ps_table, $ps_sort_fields, $pa_options = null ) {
	...
	$va_available_sorts = caGetAvailableSortFields( $ps_table, null,
		array_merge( $pa_options, [ 'includeUserSorts' => false ] ) );
		...
		switch ( get_class( $subquery ) ) {
			case 'Zend_Search_Lucene_Search_Query_Phrase':
				foreach ( $subquery->getQueryTerms() as $o_term ) {
					...
					} elseif ( ( in_array( $vs_field, [ 'ca_sets.set_code', 'set' ] ) )
					           && ( $vn_set_id
							= ca_sets::find( [ 'set_code' => $vs_value ], [ 'returnAs' => 'firstId' ] ) )
					) {
