...
{
    $va_available_cli_opts = array_merge(
        array(
    ...
    if ($vs_log = $o_opts->getOption('log')) {
        ...
        try {
            ...
            $o_writer = null;
            print CLIUtils::textWithColor(
                    "Couldn't open log file. Now logging via system log.",
                    "bold_red"
                ) . PHP_EOL . PHP_EOL;
        }
    }
...
function caFormatCmdOptionsForDisplay($vs_opt_format, $vs_opt_desc): string
{
    ...
    $vn_padding = 25;
    return "\t" . CLIUtils::textWithColor(
            str_pad("--" . $va_tmp[0] . " " . ($vs_alias ? "(-{$vs_alias})" : ""), $vn_padding),
            "red"
        ) .
            wordwrap(
            $vs_opt_desc,
