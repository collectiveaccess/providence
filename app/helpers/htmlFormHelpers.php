...
function caHTMLSelect($ps_name, $pa_content, $pa_attributes = null, $pa_options = null)
{
    ...
    if (isset($pa_options['contentArrayUsesKeysForValues']) && $pa_options['contentArrayUsesKeysForValues']) {
        foreach ($pa_content as $vs_val => $vs_opt) {
            ...
            if (is_null($vs_selected_val) || !($SELECTED = (((string)$vs_selected_val === (string)$vs_val) && strlen(
                        $vs_selected_val
                    )) ? ' selected="1"' : '')) {
                $SELECTED = (is_array($va_selected_vals) && in_array(
                        $vs_val,
                        $va_selected_vals
                    )) ? ' selected="1"' : '';
            }
            $DISABLED = (isset($va_disabled_options[$vs_val]) && $va_disabled_options[$vs_val]) ? ' disabled="1"' : '';
            $vs_element .= "<option value='" . htmlspecialchars(
                    $vs_val,
                    ENT_QUOTES,
                    'UTF-8'
...
function caHTMLTextInput($ps_name, $pa_attributes = null, $pa_options = null)
{
    ...
    if (is_array(
        $va_dim = caParseFormElementDimension(
    ...
    if ($vb_use_wysiwyg_editor) {
        ...
        if (!is_array(
            $va_toolbar_config = $o_config->getAssoc(caGetOption('cktoolbar', $pa_options, 'wysiwyg_editor_toolbar'))
        )) {
            $va_toolbar_config = [];
        }
...
function caHTMLRadioButtonsInput($ps_name, $pa_content, $pa_attributes = null, $pa_options = null)
{
    ...
    if ($vb_content_is_list) {
        foreach ($pa_content as $vs_val) {
            ...
            $DISABLED = (isset($va_disabled_options[$vs_val]) && $va_disabled_options[$vs_val]) ? ' disabled="1"' : '';
            $vs_element .= "<input type='radio' name='{$ps_name}' {$vs_id_attr} value='" . htmlspecialchars(
                    $vs_val,
                    ENT_QUOTES,
                    'UTF-8'
                ) . "'{$SELECTED}{$DISABLED}> " . $vs_val . "\n";
...
function caHTMLImage($ps_url, $pa_options = null)
{
    ...
    if ($vn_scale_css_width_to || $vn_scale_css_height_to) {
        ...
        $va_scaled_dimensions = caFitImageDimensions(
            $va_attributes['width'],
            $va_attributes['height'],
            $vn_scale_css_width_to,
            $vn_scale_css_height_to
        );
