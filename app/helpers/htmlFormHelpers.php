 ...
 * @param array  $pa_content    Associative array with keys as display options and values as option values. If the
 *                              'contentArrayUsesKeysForValues' is set then keys use interpreted as option values and
 *                              values as display options.
 * @param array  $pa_attributes Optional associative array of <select> tag options; keys are attribute names and values
 ...
 *                              values                = an array of values for the element, when the <select> allows
 *                              multiple selections disabledOptions        = an associative array indicating whether
 *                              options are disabled or not; keys are option *values*, values are boolean
 *                              (true=disabled; false=enabled) contentArrayUsesKeysForValues = normally the keys of the
 *                              $pa_content array are used as display options and the values as option values. Setting
 *                              'contentArrayUsesKeysForValues' to true will reverse the interpretation, using keys as
 *                              option values. useOptionsForValues = set values to be the same as option text. [Default
 *                              is false] width                = width of select in characters or pixels. If specifying
 *                              pixels use "px" as the units (eg. 100px) colors                =
 *
...
function caHTMLSelect( $ps_name, $pa_content, $pa_attributes = null, $pa_options = null ) {
	...
	if ( is_array(
		$va_dim = caParseFormElementDimension( isset( $pa_options['width'] ) ? $pa_options['width'] : null ) )
	) {
	...
	if ( is_array(
		$va_dim = caParseFormElementDimension( isset( $pa_options['height'] ) ? $pa_options['height'] : null ) )
	) {
		if ( $va_dim['type'] == 'pixels' ) {
	...
	if ( isset( $pa_options['contentArrayUsesKeysForValues'] ) && $pa_options['contentArrayUsesKeysForValues'] ) {
		foreach ( $pa_content as $vs_val => $vs_opt ) {
			if ( $vb_use_options_for_values ) {
				$vs_val = preg_replace( "!^[\s]+!", "",
					preg_replace( "![\s]+$!", "", str_replace( "&nbsp;", "", $vs_opt ) ) );
			}
			if ( $COLOR = ( $vs_color = $va_colors[ $vs_val ] ) ? " data-color='#{$vs_color}'" : '' ) {
				$vb_uses_color = true;
			}
			if ( is_null( $vs_selected_val )
			     || ! ( $SELECTED = ( ( (string) $vs_selected_val === (string) $vs_val )
			                          && strlen( $vs_selected_val ) ) ? ' selected="1"' : '' )
			) {
				$SELECTED = ( is_array( $va_selected_vals ) && in_array( $vs_val, $va_selected_vals ) )
					? ' selected="1"' : '';
			}
			$DISABLED   = ( isset( $va_disabled_options[ $vs_val ] ) && $va_disabled_options[ $vs_val ] )
				? ' disabled="1"' : '';
			$vs_element .= "<option value='" . htmlspecialchars( $vs_val, ENT_QUOTES, 'UTF-8' )
			               . "'{$SELECTED}{$DISABLED}{$COLOR}>" . $vs_opt . "</option>\n";
		}
	} else {
		if ( $vb_content_is_list ) {
			foreach ( $pa_content as $vs_val ) {
				...
				if ( is_null( $vs_selected_val )
				     || ! ( $SELECTED = ( (string) $vs_selected_val === (string) $vs_val ) ? ' selected="1"' : '' )
				) {
					$SELECTED = ( is_array( $va_selected_vals ) && in_array( $vs_val, $va_selected_vals ) )
						? ' selected="1"' : '';
				}
				$DISABLED   = ( isset( $va_disabled_options[ $vs_val ] ) && $va_disabled_options[ $vs_val ] )
					? ' disabled="1"' : '';
				$vs_element .= "<option value='" . htmlspecialchars( $vs_val, ENT_QUOTES, 'UTF-8' )
				               . "'{$SELECTED}{$DISABLED}{$COLOR}>" . $vs_val . "</option>\n";
			}
		} else {
	...
	if ( $vb_uses_color && isset( $pa_attributes['id'] ) && $pa_attributes['id'] ) {
		$vs_element .= "<script type='text/javascript'>jQuery(document).ready(function() { var f; jQuery('#"
		               . $pa_attributes['id'] . "').on('change', f=function() { var c = jQuery('#"
		               . $pa_attributes['id'] . "').find('option:selected').data('color'); jQuery('#"
		               . $pa_attributes['id']
		               . "').css('background-color', c ? c : '#fff'); return false;}); f(); });</script>";
	}
 ...
 * @param array  $pa_attributes An array of attributes to include in the rendered HTML form element. If you need to set
 *                              class, id, alt or other attributes, set them here.
 * @param array Options include:
 *                              width = Width of element in pixels (number with "px" suffix) or characters (number with
 *                              no suffix) [Default is null] height = Height of element in pixels (number with "px"
 *                              suffix) or characters (number with no suffix) [Default is null] usewysiwygeditor = Use
 *                              rich text editor (CKEditor) for text element. Only available when the height of the
 *                              text element is multi-line. [Default is false] cktoolbar = app.conf directive name to
 *                              pull CKEditor toolbar spec from. [Default is wysiwyg_editor_toolbar] contentUrl = URL
 *                              to use to load content when CKEditor is use with CA-specific plugins. [Default is null]
 *                              textAreaTagName =
 *
 * @return string
 */
function caHTMLTextInput( $ps_name, $pa_attributes = null, $pa_options = null ) {
	...
	if ( is_array( $va_dim = caParseFormElementDimension(
		( isset( $pa_options['width'] )
			? $pa_options['width']
			:
			( isset( $pa_attributes['size'] )
				? $pa_attributes['size']
				:
	...
	if ( is_array( $va_dim = caParseFormElementDimension(
		...
		if ( ( $pa_attributes['rows'] = ( isset( $pa_attributes['height'] ) && $pa_attributes['height'] )
				? $pa_attributes['height'] : 1 ) > 1
		) {
			$vb_is_textarea = true;
		}
	}
	...
	if ( $vb_is_textarea ) {
		...
		$vs_attr_string = _caHTMLMakeAttributeString( $pa_attributes, $pa_options );
		$vs_element     = "<{$tag_name} name='{$ps_name}' wrap='soft' {$vs_attr_string}>" . $vs_value
		                  . "</{$tag_name}>\n";
	} else {
	...
	if ( $vb_use_wysiwyg_editor ) {
		...
		if ( ! is_array( $va_toolbar_config = $o_config->getAssoc( caGetOption( 'cktoolbar', $pa_options,
 ...
 * $pa_attributes - optional associative array of <input> tag options applied to each radio button; keys are attribute
 * names and values are attribute values
 * $pa_options - optional associative array of options. Valid options are:
 *        value                = the default value of the element
 *        disabledOptions        = an associative array indicating whether options are disabled or not; keys are option
 *        *values*, values are boolean (true=disabled; false=enabled)
 */
function caHTMLRadioButtonsInput( $ps_name, $pa_content, $pa_attributes = null, $pa_options = null ) {
	...
	if ( $vb_content_is_list ) {
		foreach ( $pa_content as $vs_val ) {
			...
			$DISABLED   = ( isset( $va_disabled_options[ $vs_val ] ) && $va_disabled_options[ $vs_val ] )
				? ' disabled="1"' : '';
			$vs_element .= "<input type='radio' name='{$ps_name}' {$vs_id_attr} value='" . htmlspecialchars( $vs_val,
 ...
 *
 * $ps_name - name of the element
 * $pa_attributes - optional associative array of <input> tag options applied to the radio button; keys are attribute
 * names and values are attribute values
 * $pa_options - optional associative array of options. Valid options are:
 ...
 *
 * $ps_name - name of the element
 * $pa_attributes - optional associative array of <input> tag options applied to the checkbox; keys are attribute names
 * and values are attribute values
 * $pa_options - optional associative array of options. Valid options are:
 *        value                = the default value of the element
 *        disabled            = boolean indicating if checkbox is enabled or not (true=disabled; false=enabled)
 *        returnValueIfUnchecked = boolean indicating if checkbox should return value in request if unchecked; default
 *        is false
 */
 ...
 *                    scaleCSSWidthTo = width in pixels to *style* via CSS the returned image to. Does not actually
 *                    alter the image. Aspect ratio of the image is preserved, with the combination of scaleCSSWidthTo
 *                    and scaleCSSHeightTo being taken as a bounding box for the image. Only applicable to standard
 *                    <img> tags. Tilepic display size cannot be styled using CSS; use the "width" and "height" options
 *                    instead. scaleCSSHeightTo = height in pixels to *style* via CSS the returned image to.
 *
 * @return string
 */
function caHTMLImage( $ps_url, $pa_options = null ) {
	...
	if ( $vn_scale_css_width_to || $vn_scale_css_height_to ) {
		...
		$va_scaled_dimensions    = caFitImageDimensions( $va_attributes['width'], $va_attributes['height'],
			$vn_scale_css_width_to, $vn_scale_css_height_to );
 ...
 *                          dontConvertAttributeQuotesToEntities = if true, attribute values are not passed through
 *                          htmlspecialchars(); if you set this be sure to only use single quotes in your attribute
 *                          values or escape all double quotes since double quotes are used to enclose tem
 */
function _caHTMLMakeAttributeString( $pa_attributes, $pa_options = null ) {
	...
	if ( is_array( $pa_attributes ) ) {
		foreach ( $pa_attributes as $vs_attr => $vs_attr_val ) {
			...
			if ( isset( $pa_options['dontConvertAttributeQuotesToEntities'] )
 ...
 */
function caHTMLMakeLabeledFormElement(
	$ps_field, $ps_label, $ps_id, $ps_description = '', $ps_format = '', $pb_emit_tooltip = true
) {
	$o_config = Configuration::load();
	if ( ! $ps_format ) {
