...
function caPhantomJSInstalled($ps_phantomjs_path = null)
{
    ...
    if (!$ps_phantomjs_path) {
        $ps_phantomjs_path = caGetExternalApplicationPath('phantomjs');
    }

    if (!trim($ps_phantomjs_path) || (preg_match("/[^\/A-Za-z0-9\.:]+/", $ps_phantomjs_path)) || !@is_readable(
            $ps_phantomjs_path
        )) {
