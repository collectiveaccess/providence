...
function caGetExternalApplicationPath($ps_application_name, $options = null)
{
    ...
    $app_paths = $o_ext_app_config->get(
        ["{$ps_application_name}_app", "{$ps_application_name}_path", $ps_application_name]
    );
    if (!$app_paths) {
        return null;
    }
 ...
 *
 * @param bool $pb_dont_worry_about_jpegs If set will return true if GD is installed without JPEG support; default is to consider JPEG-less GD worthless.
 * @return bool - true if available, false if not
 */
...
function caWkhtmltopdfInstalled($ps_wkhtmltopdf_path = null)
{
    ...
    if (!trim($ps_wkhtmltopdf_path) || (preg_match("/[^\/A-Za-z0-9\.:]+/", $ps_wkhtmltopdf_path)) || !@is_readable(
            $ps_wkhtmltopdf_path
        )) {
...
function caYouTubeDlInstalled($youtube_dl_path = null)
{
    ...
    if (!trim($youtube_dl_path) || (preg_match("/[^\/A-Za-z0-9\.:\-]+/", $youtube_dl_path)) || !@is_readable(
            $youtube_dl_path
        )) {
...
function caExifToolInstalled($ps_exiftool_path = null)
{
    ...
    if (!trim($ps_exiftool_path) || (preg_match("/[^\/A-Za-z0-9\.:]+/", $ps_exiftool_path)) || !@is_readable(
            $ps_exiftool_path
        )) {
...
function caExtractMetadataWithExifTool($ps_filepath, $pb_skip_unknown = false)
{
    if ($vs_path_to_exif_tool = caExifToolInstalled()) {
        ...
        caExec(
            "{$vs_path_to_exif_tool} -json -a {$vs_unknown_param} -g1 " . caEscapeShellArg(
                $ps_filepath
            ) . " 2> /dev/null",
            $va_output,
            $vn_return
...
function caExtractRemoveOrientationTagWithExifTool($filepath)
{
    ...
    if ($path_to_exif_tool = caExifToolInstalled()) {
        caExec(
            "{$path_to_exif_tool} -overwrite_original_in_place -P -fast -Orientation= " . caEscapeShellArg(
                $filepath
            ) . " 2> /dev/null",
            $output,
            $return
...
function caExtractEmbeddedMetadata($po_instance, $pa_metadata, $pn_locale_id)
{
    ...
    if (!isset($pa_metadata['system']['filename']) && ($vs_original_filename = $po_instance->get(
            'original_filename'
        ))) {
    ...
    if (isset($pa_metadata['EXIF']) && is_array($pa_metadata['EXIF']) && ((is_array($va_georef_elements) && sizeof(
                    $va_georef_elements
                )) || (is_array($va_georef_containers) && sizeof($va_georef_containers)) || (is_array(
                    $va_date_elements
                ) && sizeof($va_date_elements)) || (is_array($va_date_containers) && sizeof($va_date_containers)))) {
                        ...
                        foreach ($va_info['map'] as $vs_sub_element => $vs_value) {
                            ...
                            if ($t_element = ca_metadata_elements::getInstance($vs_sub_element)) {
                                switch ($t_element->get('datatype')) {
                                    case 3:    // List
                                        $t_list = new ca_lists();
                                        $va_data[$vs_sub_element] = $t_list->getItemIDFromList(
            ...
            if (($vs_raw_date = $va_exif_data['IFD0']['DateTimeOriginal']) || ($vs_raw_date = $va_exif_data['EXIF']['DateTimeOriginal']) || ($vs_raw_date = $va_exif_data['ExifIFD']['DateTimeOriginal'])) {
                ...
                foreach ($va_date_elements as $vs_element) {
                    ...
                    if (strlen($po_instance->get($vs_element)) > 0) {
                        $po_instance->addAttribute(
                            array($va_tmp[1] => $vs_date, 'locale_id' => $pn_locale_id),
                            $va_tmp[1]
                        );
                    } else {
...
function caEmbedMediaMetadataIntoFile($ps_file, $ps_table, $pn_pk, $ps_type_code, $pn_rep_pk, $ps_rep_type_code)
{
    ...
    if ($vs_subject_table_export = caExportMediaMetadataForRecord($ps_table, $ps_type_code, $pn_pk)) {
        ...
        caExec(
            "{$vs_path_to_exif_tool} -tagsfromfile {$vs_export_filename} -all:all " . caEscapeShellArg(
                $vs_tmp_filepath
            ),
            $va_output,
            $vn_return
    ...
    if ($vs_representation_export = caExportMediaMetadataForRecord(
...
function caGetDefaultMediaIconTag($ps_type, $pn_width, $pn_height, $pa_options = null)
{
    if (is_array($va_selected_size = caGetMediaIconForSize($ps_type, $pn_width, $pn_height, $pa_options))) {
        ...
        return caHTMLImage(
            $o_icon_config->get('icon_folder_url') . '/' . $va_icons[$va_selected_size['size']],
...
function caParseMediaIdentifier($ps_identifier, $pa_options = null)
{
    ...
    switch ($vs_type = strtolower($va_tmp[0])) {
        ...
        case 'attribute':
            ...
            break;
        default:
            if (($vs_table = caMediaIdentifierTypeToTable($vs_type)) && Datamodel::getInstance(
                    $vs_table,
                    true
                ) && ($t_instance = $vs_table::find(
                    (int)$va_tmp[1],
                    $pa_options
...
function caGetPDFInfo($ps_filepath)
{
    ...
    if ((!$o_config->get('dont_use_graphicsmagick_to_identify_pdfs')) && caMediaPluginGraphicsMagickInstalled()) {
        ...
        caExec(
            $vs_graphicsmagick_path . ' identify -format "%m;%w;%h;%p\n" ' . caEscapeShellArg(
                $ps_filepath
    ...
    if ((!$o_config->get('dont_use_imagemagick_to_identify_pdfs')) && caMediaPluginImageMagickInstalled()) {
        ...
        caExec(
            $vs_imagemagick_path . '/identify -format "%m;%w;%h;%p\n" ' . caEscapeShellArg($ps_filepath) . (caIsPOSIX(
            ) ? " 2> /dev/null" : ""),
            $va_output,
            $vn_return
        );
    ...
    if (caMediaPluginPdftotextInstalled()) {
        ...
        caExec(
            "{$vs_path_to_pdf_to_text} " . caEscapeShellArg($ps_filepath) . (caIsPOSIX() ? " 2> /dev/null" : ""),
            $va_output,
            $vn_return
        );
