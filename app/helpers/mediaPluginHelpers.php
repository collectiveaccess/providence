...
# ------------------------------------------------------------------------------------------------
/**
 * Get path in external_applications.conf for specified application. A path is only returned if it exists on the
 * system.
 * If none of the configured paths for an application exist on the system, null is returned.
 *
 * @param string $ps_application_name The name of the application. This is the same as the relevant entry in
 *                                    external_applications.conf without the trailing "_app" (Ex. pdfminer, dcraw,
 *                                    ffmpeg)
 * @param array  $options             Options inclide:
 *                                    executableName = Name of executable to test for when app path is a directory (Ex.
 *                                    for ImageMagick) returnAsArray = Return full list of configured paths. Paths are
...
function caGetExternalApplicationPath( $ps_application_name, $options = null ) {
	...
	$app_paths = $o_ext_app_config->get( [
		"{$ps_application_name}_app",
		"{$ps_application_name}_path",
		$ps_application_name
	] );
	if ( ! $app_paths ) {
		return null;
...
# ------------------------------------------------------------------------------------------------
/**
 * Detects if GD PHP extension is available. Return false if GD is installed but lacks JPEG support unless "don't worry
 * about JPEGs" parameter is set to true.
 *
 * @param boolean $pb_dont_worry_about_jpegs If set will return true if GD is installed without JPEG support; default
 *                                           is to consider JPEG-less GD worthless.
 *
 * @return boolean - true if available, false if not
 */
...
# ------------------------------------------------------------------------------------------------
/**
 * Detects if Meshlab (http://meshlab.sourceforge.net), and specifically the meshlabserver command line tool, is
...
function caWkhtmltopdfInstalled( $ps_wkhtmltopdf_path = null ) {
	...
	if ( ! trim( $ps_wkhtmltopdf_path ) || ( preg_match( "/[^\/A-Za-z0-9\.:]+/", $ps_wkhtmltopdf_path ) )
	     || ! @is_readable( $ps_wkhtmltopdf_path )
	) {
...
function caYouTubeDlInstalled( $youtube_dl_path = null ) {
	...
	if ( ! trim( $youtube_dl_path ) || ( preg_match( "/[^\/A-Za-z0-9\.:\-]+/", $youtube_dl_path ) )
	     || ! @is_readable( $youtube_dl_path )
	) {
...
function caExifToolInstalled( $ps_exiftool_path = null ) {
	...
	if ( ! trim( $ps_exiftool_path ) || ( preg_match( "/[^\/A-Za-z0-9\.:]+/", $ps_exiftool_path ) )
	     || ! @is_readable( $ps_exiftool_path )
	) {
 ...
 *                                Use this if metadata extraction fails for unknown reasons. Sometimes tools like
 *                                Photoshop write weird binary data into the files that causes json_decode to barf.
 *
...
function caExtractMetadataWithExifTool( $ps_filepath, $pb_skip_unknown = false ) {
	if ( $vs_path_to_exif_tool = caExifToolInstalled() ) {
		...
		caExec( "{$vs_path_to_exif_tool} -json -a {$vs_unknown_param} -g1 " . caEscapeShellArg( $ps_filepath )
		        . " 2> /dev/null", $va_output, $vn_return );
...
function caExtractRemoveOrientationTagWithExifTool( $filepath ) {
	...
	if ( $path_to_exif_tool = caExifToolInstalled() ) {
		caExec( "{$path_to_exif_tool} -overwrite_original_in_place -P -fast -Orientation= "
		        . caEscapeShellArg( $filepath ) . " 2> /dev/null", $output, $return );
...
function caExtractEmbeddedMetadata( $po_instance, $pa_metadata, $pn_locale_id ) {
	...
	if ( ! isset( $pa_metadata['system']['filename'] )
	     && ( $vs_original_filename
			= $po_instance->get( 'original_filename' ) )
	) {
	...
	if ( isset( $pa_metadata['EXIF'] ) && is_array( $pa_metadata['EXIF'] )
	     && ( ( is_array( $va_georef_elements )
	            && sizeof( $va_georef_elements ) )
	          || ( is_array( $va_georef_containers )
	               && sizeof( $va_georef_containers ) )
	          || ( is_array( $va_date_elements )
	               && sizeof( $va_date_elements ) )
	          || ( is_array( $va_date_containers )
	               && sizeof( $va_date_containers ) ) )
	) {
			...
			if ( is_array( $va_coords = caParseEXIFLatLong( $va_exif_data ) ) ) {
				foreach ( $va_georef_elements as $vs_element ) {
					...
					$po_instance->addAttribute( array(
						$va_tmp[1]  => "[" . $va_coords['latitude'] . ", " . $va_coords['longitude'] . "]",
		...
		if ( is_array( $va_georef_containers ) ) {
			if ( is_array( $va_coords = caParseEXIFLatLong( $va_exif_data ) ) ) {
				foreach ( $va_georef_containers as $vs_container => $va_info ) {
					...
					$va_data = array(
						$vs_value_element => "[" . $va_coords['latitude'] . ", " . $va_coords['longitude'] . "]",
						'locale_id'       => $pn_locale_id
					);
						...
						foreach ( $va_info['map'] as $vs_sub_element => $vs_value ) {
							...
							if ( $t_element = ca_metadata_elements::getInstance( $vs_sub_element ) ) {
								switch ( $t_element->get( 'datatype' ) ) {
									case 3:    // List
										$t_list = new ca_lists();
										$va_data[ $vs_sub_element ]
										        = $t_list->getItemIDFromList( $t_element->get( 'list_id' ), $vs_value );
										break;
									default:
		...
		if ( is_array( $va_date_elements ) ) {
			if ( ( $vs_raw_date = $va_exif_data['IFD0']['DateTimeOriginal'] )
			     || ( $vs_raw_date
					= $va_exif_data['EXIF']['DateTimeOriginal'] )
			     || ( $vs_raw_date = $va_exif_data['ExifIFD']['DateTimeOriginal'] )
			) {
				$va_date_tmp = preg_split( '![: ]+!', $vs_raw_date );
				$vs_date     = $va_date_tmp[0] . '-' . $va_date_tmp[1] . '-' . $va_date_tmp[2] . 'T' . $va_date_tmp[3]
				               . ':' . $va_date_tmp[4] . ':' . $va_date_tmp[5];
				foreach ( $va_date_elements as $vs_element ) {
					$va_tmp = explode( '.', $vs_element );
					if ( strlen( $po_instance->get( $vs_element ) ) > 0 ) {
						$po_instance->addAttribute( array( $va_tmp[1] => $vs_date, 'locale_id' => $pn_locale_id ),
							$va_tmp[1] );
					} else {
						$po_instance->replaceAttribute( array( $va_tmp[1] => $vs_date, 'locale_id' => $pn_locale_id ),
							$va_tmp[1] );
					}
				}
				$vb_did_mapping = true;
 ...
 * @param string $ps_table         Table name of the subject record. This is used to figure out the appropriate mapping
 *                                 to use from media_metadata.conf
 * @param int    $pn_pk            Primary key of the subject record. This is used to run the export for the right
 *                                 record.
 ...
 *                                 If there are export mapping for object representations, we run them after the
 *                                 mapping for the subject table. Fields that get exported here should overwrite fields
 *                                 from the subject table export.
...
function caEmbedMediaMetadataIntoFile( $ps_file, $ps_table, $pn_pk, $ps_type_code, $pn_rep_pk, $ps_rep_type_code ) {
	...
	if ( $vs_subject_table_export = caExportMediaMetadataForRecord( $ps_table, $ps_type_code, $pn_pk ) ) {
		...
		caExec( "{$vs_path_to_exif_tool} -tagsfromfile {$vs_export_filename} -all:all "
		        . caEscapeShellArg( $vs_tmp_filepath ), $va_output, $vn_return );
	...
	if ( $vs_representation_export = caExportMediaMetadataForRecord( 'ca_object_representations', $ps_rep_type_code,
		$pn_rep_pk )
	) {
...
function caGetDefaultMediaIconTag( $ps_type, $pn_width, $pn_height, $pa_options = null ) {
	if ( is_array( $va_selected_size = caGetMediaIconForSize( $ps_type, $pn_width, $pn_height, $pa_options ) ) ) {
		...
		return caHTMLImage( $o_icon_config->get( 'icon_folder_url' ) . '/' . $va_icons[ $va_selected_size['size'] ],
			array( 'width' => $va_selected_size['width'], 'height' => $va_selected_size['height'] ) );
	}
	...
	if ( is_array( $va_icons = $o_icon_config->getAssoc( $ps_type ) ) ) {
		...
		foreach ( $va_icons as $vs_size => $vs_filename ) {
			...
			if (
				...
				&& ( ( ( $vn_diff_y = ( (int) $pn_height - (int) $va_tmp[1] ) ) >= 0 )
				     && ( $vn_diff_y <= $vn_min_diff_y ) )
			) {
...
# ------------------------------------------------------------------------------------------------
/**
 * Resolve media identifiers in the form <type>:<id> or <type>:<id>:<page> into references suitable for using with the
 * IIIF service. Identifier types include: "representation", "attribute", "object", "entity", "place", "occurrence",
 ...
 *
 * type = the "type" portion of resolved IIIF identifier. This will always be either "representation" or "attribute",
 * no matter what the type in the original identifer is. id = the "id" portion of the resolved identifier. This will be
 * a representation_id or attribute value_id page = the "page" portion of the identifer, if specified. This will be
 * unchanged from the original identifier. subject = the table name of the subject of the original media identifier.
 * For representations this will be the representation itself. For attributes this will be the table name of the record
 * it is bonud to. For table-related types this will be the table name of the type (Eg. for "objects", this will be
 * "ca_objects") subject_id = the primary key of the subject row. For representations this will be the
 * representation_id. For attributes this will be the id of the row to which the attribute is bound. For table-related
 * types this will be the id of the referenced row. instance = An instance of the resolved media
 * (ca_object_representations or ca_attribute_values) when the "includeInstance" option is set. subject_instance = An
 * instance of the resolved subject (ca_object_representations, ca_objects, or other primary table) when the "includeInstance" option is set.
 *
 * @param string $ps_identifier The identifier
 * @param array  $pa_options    Options include:
 *                              includeInstance = Include resolved media and subject instances in return value.
 *                              [Default is false] checkAccess = check resolved media and subject against provided
 *                              access values and return null if access is not allowed. [Default is null]
 *
 * @return array
 */
	...
	switch ( $vs_type = strtolower( $va_tmp[0] ) ) {
		case 'representation':
			...
			$va_ret = [
				'type'       => $vs_type,
				'id'         => (int) $va_tmp[1],
				'page'       => isset( $va_tmp[2] ) ? (int) $va_tmp[2] : null,
				'subject'    => 'ca_object_representations',
				'subject_id' => (int) $va_tmp[1]
			];
		...
		case 'attribute':
			...
			break;
		default:
			if ( ( $vs_table = caMediaIdentifierTypeToTable( $vs_type ) ) && Datamodel::getInstance( $vs_table, true )
			     && ( $t_instance = $vs_table::find( (int) $va_tmp[1], $pa_options ) )
			     && ( $vn_rep_id = $t_instance->getPrimaryRepresentationID( $pa_options ) )
			) {
				...
				$va_ret = [
					'type'       => 'representation',
					'id'         => (int) $va_tmp[0],
					'page'       => isset( $va_tmp[1] ) ? (int) $va_tmp[1] : null,
					'subject'    => 'ca_object_representations',
					'subject_id' => (int) $va_tmp[0]
				];
			}
...
function caMediaIdentifierTypeToTable( $ps_type, $pa_options = null ) {
	$va_map = [
		...
		'locations'   => 'ca_storage_locations'
	];
...
function caGetPDFInfo( $ps_filepath ) {
	...
	if ( caMediaPluginPdftotextInstalled() ) {
		...
		caExec( "{$vs_path_to_pdf_to_text} " . caEscapeShellArg( $ps_filepath ) . ( caIsPOSIX() ? " 2> /dev/null"
				: "" ), $va_output, $vn_return );
