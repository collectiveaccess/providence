...
function caGetAvailablePrintTemplates($ps_type, $pa_options = null)
{
    ...
    $va_cached_list = (ExternalCache::contains($vs_cache_key, 'PrintTemplates')) ? ExternalCache::fetch(
        $vs_cache_key,
        'PrintTemplates'
    ) : [];
            ...
            if (is_resource($r_dir = opendir($vs_path))) {
                while (($vs_template = readdir($r_dir)) !== false) {
                    ...
                    if (is_array($va_template_info = caGetPrintTemplateDetails($ps_type, $vs_template_tag))) {
                        ...
                        $va_template_show_only_in = array_filter(
                            array_map(
                                function ($v) {
                                    return trim($v);
                                },
                                explode(",", caGetOption('showOnlyIn', $va_template_info, null))
                            ),
                            function ($v) {
                                return (bool)strlen($v);
                            }
                        );
                        if (is_array($va_show_only_in) && (sizeof($va_show_only_in) > 0) && is_array(
                                $va_template_show_only_in
                            ) && (sizeof($va_template_show_only_in) > 0) && !sizeof(
                                array_intersect($va_template_show_only_in, $va_show_only_in)
                            )) {
                            continue;
                        }
                        ...
                        if ($vs_tablename && (!in_array($vs_tablename, $va_template_info['tables'])) && (!in_array(
                                '*',
                                $va_template_info['tables']
                            ))) {
                            continue;
                        }

                        if (!is_dir($vs_path . '/' . $vs_template) && preg_match(
                                "/^[A-Za-z_]+[A-Za-z0-9_]*$/",
                                $vs_template_tag
                            )) {
...
function caGetPrintTemplateDetails($ps_type, $ps_template, $pa_options = null)
{
    ...
    foreach ($va_template_paths as $vs_template_path) {
        ...
        if (ExternalCache::contains($vs_cache_key, 'PrintTemplateDetails')) {
            ...
            $vn_template_rev = file_exists($vs_template_path) ? array_shift(
                array_map("filemtime", glob("{$vs_template_path}/*.{php,css}", GLOB_BRACE))
            ) : 0;
        ...
        }    // pdf is inferred for templates without a specific file format
...
function caGenerateBarcode($ps_value, $pa_options = null)
{
    ...
    switch ($ps_barcode_type) {
        ...
        case 'upca':
            ...
            if (!($va_dimensions = $o_barcode->draw(
                $ps_value,
                "{$vs_tmp}.png",
                $ps_barcode_type,
                'png',
...
function caParseBarcodeViewTag($ps_tag, $po_view, $po_result, $pa_options = null)
{
    ...
    if (substr($ps_tag, 0, 7) == 'barcode') {
        ...
        $vs_tmp = caGenerateBarcode(
            $po_result->getWithTemplate($vs_template, $pa_options),
...
{
    return caDoTemplateTagSubstitution(
        $po_view,
        $po_result,
        $ps_template_path,
...
function caGetPrintFormatsListAsHTMLForRelatedBundles(
    ...
    $va_formats = caGetAvailablePrintTemplates(
        'results',
        [
            'table' => $pt_related->tableName(),
            'type' => null,
            'showOnlyIn' => 'editor_relationship_bundle'
        ]
    );
    ...
    foreach (
        caExtractValuesByUserLocale(
            $t_display->getBundleDisplays(['table' => $pt_related->tableName()])
        ) as $va_display_info
    ) {
        if (is_array($va_display_info['settings']['show_only_in']) && sizeof(
    ...
    $vs_buf .= caJSButton(
        ...
        __CA_NAV_ICON_GO__,
        '',
        "{$ps_id_prefix}_report",
        ['onclick' => "caGetExport{$ps_id_prefix}(); return false;"],
        ['size' => '15px']
    );
    ...
    $vs_buf .= "
			<script type='text/javascript'>
				function caGetExport{$ps_id_prefix}() {
					...
					var f = jQuery('<form id=\"caTempExportForm\" action=\"{$vs_url}/export_format/' + s + '\" method=\"post\" style=\"display:none;\"><textarea name=\"ids\">" . json_encode(
            $va_ids
        ) . "</textarea></form>');
...
function caGetPrintFormatsListAsHTMLForSetItemBundles($ps_id_prefix, $po_request, $pt_set, $pa_row_ids)
{
    ...
    if (is_array(
        $va_displays = caExtractValuesByUserLocale(
            $t_display->getBundleDisplays(['user_id' => $po_request->getUserID(), 'table' => $vs_set_table])
        )
    )) {
...
function caEditorPrintSummaryControls($po_view)
{
    ...
    if ($vs_display_select_html = $t_display->getBundleDisplaysAsHTMLSelect(
        'display_id',
        array(
            'onchange' => 'jQuery("#caSummaryDisplaySelectorForm").submit();',
        ...
        $vs_buf .= "<div id='printButton'>
                <a href='#' onclick='return caShowSummaryDownloadOptionsPanel();'>" . caNavIcon(
                __CA_NAV_ICON_PDF__,
                2
            ) . "</a>
                    <script type='text/javascript'>
