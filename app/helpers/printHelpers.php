...
function caGetAvailablePrintTemplates( $ps_type, $pa_options = null ) {
	...
	if ( $vs_tablename = caGetOption( 'table', $pa_options, null ) ) {
		if ( ( $restrict_to_types = caGetOption( 'restrictToTypes', $pa_options, false ) )
		     && ! is_array( $restrict_to_types )
		) {
	...
	$va_cached_list = ( ExternalCache::contains( $vs_cache_key, 'PrintTemplates' ) )
		? ExternalCache::fetch( $vs_cache_key, 'PrintTemplates' ) : [];
			...
			if ( is_resource( $r_dir = opendir( $vs_path ) ) ) {
				while ( ( $vs_template = readdir( $r_dir ) ) !== false ) {
					...
					if ( is_array( $va_template_info = caGetPrintTemplateDetails( $ps_type, $vs_template_tag ) ) ) {
						...
						if ( ! is_array(
							$template_restrict_to_types = caGetOption( 'restrictToTypes', $va_template_info, null ) )
						) {
						...
						if ( is_array( $va_show_only_in ) && ( sizeof( $va_show_only_in ) > 0 )
						     && is_array( $va_template_show_only_in )
						     && ( sizeof( $va_template_show_only_in ) > 0 )
						     && ! sizeof( array_intersect( $va_template_show_only_in, $va_show_only_in ) )
						) {
							continue;
						}

						if ( $vs_element_code
						     && ( caGetOption( 'elementCode', $va_template_info, null ) !== $vs_element_code )
						) {
							continue;
						}

						if ( $vs_tablename && ( ! in_array( $vs_tablename, $va_template_info['tables'] ) )
						     && ( ! in_array( '*', $va_template_info['tables'] ) )
						) {
							continue;
						}
						if ( $vs_tablename && ( ! in_array( $vs_tablename, $va_template_info['tables'] ) )
						...
						if ( ! is_dir( $vs_path . '/' . $vs_template )
						     && preg_match( "/^[A-Za-z_]+[A-Za-z0-9_]*$/", $vs_template_tag )
						) {
...
function caGetPrintTemplateDetails( $ps_type, $ps_template, $pa_options = null ) {
	...
	foreach ( $va_template_paths as $vs_template_path ) {
		...
		if ( ExternalCache::contains( $vs_cache_key, 'PrintTemplateDetails' ) ) {
			...
			if ( ExternalCache::fetch( "{$vs_cache_key}_mtime", 'PrintTemplateDetails' )
			     >= filemtime( $vs_template_path )
			) {
				return $va_list;
			}
		}
 ...
 *
 * @param $ps_value   string The value to convert. Valid units are in, cm, mm, px and p. If units are invalid or
 *                    omitted points are assumed.
 * @param $pa_options array Options include:
 *                    dpi = dots-per-inch factor to use when converting physical units (in, cm, etc.) to points
 *                    [Default is 72dpi] ppi = synonym for dpi option
 *
...
/**
 * Converts string quantity with units ($ps_value parameter) to a numeric quantity in
 * the units specified by the $ps_units parameter. Units are limited to inches, centimeters, millimeters, pixels and
 * points as this function is primarily used to switch between units used when generating PDFs.
 *
 * @param $ps_value string The value to convert. Valid units are in, cm, mm, px and p. If units are invalid or omitted
 *                  points are assumed.
 * @param $ps_units string A valid measurement unit: in, cm, mm, px, p (inches, centimeters, millimeters, pixels,
 *                  points) respectively.
 *
...
function caGenerateBarcode( $ps_value, $pa_options = null ) {
	...
	switch ( $ps_barcode_type ) {
		...
		case 'upca':
			...
			if ( ! ( $va_dimensions = $o_barcode->draw( $ps_value, "{$vs_tmp}.png", $ps_barcode_type, 'png',
				$pn_barcode_height ) )
			) {
				return null;
			}
...
function caParseBarcodeViewTag( $ps_tag, $po_view, $po_result, $pa_options = null ) {
	...
	if ( substr( $ps_tag, 0, 7 ) == 'barcode' ) {
		...
		$vs_tmp = caGenerateBarcode( $po_result->getWithTemplate( $vs_template, $pa_options ),
			array( 'type' => $vs_type, 'height' => $vn_size ) );
...
function caDoPrintViewTagSubstitution( $po_view, $po_result, $ps_template_path, $pa_options = null ) {
	return caDoTemplateTagSubstitution( $po_view, $po_result, $ps_template_path,
		[ 'render' => false, 'barcodes' => true, 'clearVars' => true ] );
}
...
function caPrintLabels( $po_view, $po_result, $ps_title ) {
	try {
		...
		$va_defined_vars
			         = array_keys( $po_view->getAllVars() );        // get list defined vars (we don't want to copy over them)
		...
		while ( $po_result->nextHit() ) {
			$va_barcode_files_to_delete = array_merge( $va_barcode_files_to_delete,
				caDoPrintViewTagSubstitution( $po_view, $po_result, $va_template_info['path'],
 ...
 *
 */
function caGetPrintFormatsListAsHTMLForRelatedBundles(
	$ps_id_prefix, $po_request, $pt_primary, $pt_related, $pt_relation, $placement_id
) {
	$va_formats = caGetAvailablePrintTemplates( 'results',
		[ 'table' => $pt_related->tableName(), 'type' => null, 'showOnlyIn' => 'editor_relationship_bundle' ] );
	...
	foreach (
		caExtractValuesByUserLocale( $t_display->getBundleDisplays( [ 'table' => $pt_related->tableName() ] ) ) as
		$va_display_info
	) {
		if ( is_array( $va_display_info['settings']['show_only_in'] )
		     && sizeof( $va_display_info['settings']['show_only_in'] )
		     && ! in_array( 'editor_relationship_bundle', $va_display_info['settings']['show_only_in'] )
		) {
			continue;
		}
	...
	$vs_buf .= caHTMLSelect( 'export_format', $va_options,
			array( 'id' => "{$ps_id_prefix}_reportList", 'class' => 'dontTriggerUnsavedChangeWarning' ),
			array( 'value' => null, 'width' => '150px' ) ) . "\n";

	$vs_buf .= caJSButton( $po_request, __CA_NAV_ICON_GO__, '', "{$ps_id_prefix}_report",
		[ 'onclick' => "caGetExport{$ps_id_prefix}(); return false;" ], [ 'size' => '15px' ] );

	$vs_url = caNavUrl( $po_request, 'find', 'RelatedList', 'Export', [
		'relatedRelTable' => $pt_relation->tableName(),
		'primaryTable'    => $pt_primary->tableName(),
		'primaryID'       => $pt_primary->getPrimaryKey(),
		'download'        => 1,
		'relatedTable'    => $pt_related->tableName()
	] );
	$vs_buf .= "</div>";
	$vs_buf .= "
...
function caGetPrintFormatsListAsHTMLForSetItemBundles( $ps_id_prefix, $po_request, $pt_set, $pa_row_ids ) {
	...
	$va_formats   = caGetAvailablePrintTemplates( 'sets',
		[ 'showOnlyIn' => 'set_item_bundle', 'table' => $vs_set_table, 'type' => null ] );
	...
	$t_display = new ca_bundle_displays();
	if ( is_array( $va_displays
		= caExtractValuesByUserLocale( $t_display->getBundleDisplays( [
	...
	$vs_url = caNavUrl( $po_request, 'manage', 'sets', 'setEditor/exportSetItems',
		[ 'set_id' => $pt_set->get( "set_id" ), 'download' => 1 ] );
	$vs_buf .= "</div>";
	$vs_buf .= "
			<script type='text/javascript'>
				function caGetExport{$ps_id_prefix}() {
...
function caEditorPrintSummaryControls( $po_view ) {
	...
	$vs_buf = $po_view->render( $request->getViewsDirectoryPath( true )
	                            . '/bundles/summary_download_options_html.php' );

	if ( $vs_display_select_html = $t_display->getBundleDisplaysAsHTMLSelect( 'display_id',
		array( 'onchange' => 'jQuery("#caSummaryDisplaySelectorForm").submit();', 'class' => 'searchFormSelector' ),
		array(
		...
		$vs_buf .= "<div id='printButton'>
                <a href='#' onclick='return caShowSummaryDownloadOptionsPanel();'>" . caNavIcon( __CA_NAV_ICON_PDF__,
				2 ) . "</a>
                    <script type='text/javascript'>
		...
		$vs_buf .= caFormTag( $request, 'Summary', 'caSummaryDisplaySelectorForm', null, 'post', 'multipart/form-data',
				'_top', [ 'noCSRFToken' => true, 'disableUnsavedChangesWarning' => true ] ) .
