    ...
    if (is_array($pa_other_params) && sizeof($pa_other_params)) {
        ...
        if (caIsAssociativeArray($pa_other_params)) {
            ...
            foreach ($pa_other_params as $vs_name => $vs_value) {
                ...
                if (is_array(
                    $vs_value
                )) { // is the value is array we need to serialize is... just treat it as a list of values which *should* be what it is.
                ...
                if ($use_query_string) {
                    ...
                    $vs_url .= '/' . $vs_name . "/" . (caGetOption(
                            'dontURLEncodeParameters',
                            $pa_options,
                            false
...
function caFormTag(
    ...
    $ps_action,
    $ps_id,
    $ps_module_and_controller_path = null,
    $ps_method = 'post',
    $ps_enctype = 'multipart/form-data',
    $ps_target = '_top',
    $pa_options = null
) {
    if ($ps_target) {
    ...
    if ($ps_module_and_controller_path) {
        $vs_action = (caUseCleanUrls()) ?
            ...
            :
            $po_request->getBaseUrlPath() . '/' . $po_request->getScriptName(
            ) . '/' . $ps_module_and_controller_path . '/' . $ps_action;
    } else {
        $vs_action = (caUseCleanUrls()) ?
...
function caNavFormParameters($pa_other_params = null)
{
    ...
    if (is_array($pa_other_params) && sizeof($pa_other_params)) {
        foreach ($pa_other_params as $vs_name => $vs_val) {
            $vs_buf .= '<input type="hidden" name="' . $vs_name . '" value="' . htmlspecialchars(
                    $vs_val,
                    ENT_QUOTES,
                    'UTF-8'
                ) . '"/>' . "\n";
...
function caEditorUrl(
    ...
    switch ($vs_table) {
        case 'ca_relationship_types':
            $vs_action = isset($pa_options['action']) ? $pa_options['action'] : (($po_request->isLoggedIn(
        ...
        default:
            if (isset($pa_options['action'])) {
                ...
                !((bool)$po_request->config->get(
                        $vs_table . '_editor_defaults_to_summary_view'
...
function caDetailUrl(
    ...
    if (((int)$pn_id > 0) && ($vs_use_alt_identifier_in_urls = caUseAltIdentifierInUrls($ps_table))
        ...
        $pn_id = (strlen($vn_id_for_idno) && (strpos(
                    $vn_id_for_idno,
                    '/'
                ) === false)) ? $vn_id_for_idno : "id:{$pn_id}";
    }
