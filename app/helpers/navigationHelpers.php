 ...
 *                                     dontURLEncodeParameters = Don't apply url encoding to parameters in URL [Default
 *                                     is false] absolute = return absolute URL. [Default is to return relative URL]
 *                                     useQueryString = encode other parameters as query string rather than in url path
 *                                     [Default is false]
 *
 * @return string
 */
		...
		if ( caIsAssociativeArray( $pa_other_params ) ) {
			...
			foreach ( $pa_other_params as $vs_name => $vs_value ) {
				...
				if ( $use_query_string ) {
					...
					$vs_url .= '/' . $vs_name . "/" . ( caGetOption( 'dontURLEncodeParameters', $pa_options, false )
 ...
 */
function caNavLink(
	$po_request, $ps_content, $ps_classname, $ps_module_path, $ps_controller, $ps_action, $pa_other_params = null,
	$pa_attributes = null, $pa_options = null
) {
 ...
 *
 */
function caFormNavButton(
	$po_request, $pn_type, $ps_content, $ps_classname, $ps_module_path, $ps_controller, $ps_action,
	$pa_other_params = null, $pa_attributes = null, $pa_options = null
) {
	if ( ! is_array( $pa_options ) ) {
		$pa_options = array();
	}

	return caNavButton( $po_request, $pn_type, $ps_content, $ps_classname, $ps_module_path, $ps_controller, $ps_action,
		$pa_other_params, $pa_attributes, array_merge( $pa_options, array( 'size' => '30px' ) ) );
}
...
function caNavButton(
	...
	$pb_dont_show_content = ( isset( $pa_options['dont_show_content'] ) && $pa_options['dont_show_content'] ) ? true
		: false;
 ...
 *                          iconMargin =
 */
function caNavHeaderButton(
	$po_request, $pn_type, $ps_content, $ps_module_path, $ps_controller, $ps_action, $pa_other_params = null,
	$pa_attributes = null, $pa_options = null
) {
	if ( ! ( $vs_url = caNavUrl( $po_request, $ps_module_path, $ps_controller, $ps_action, $pa_other_params ) ) ) {
		return ''; //<strong>Error: no url for navigation</strong>";
	...
	$ps_icon_pos          = isset( $pa_options['icon_position'] ) ? $pa_options['icon_position']
		: __CA_NAV_ICON_ICON_POS_LEFT__;
 ...
 *
 * Options:
 *    disableUnsavedChangesWarning = if true, unsaved change warnings (when user tries to navigate away from the form
 *    before saving) are disabled. [Default is false] noTimestamp = if true no form timestamp (used to determine if
 *    other users have made changes while the form is being displayed) is included. [Default is false] noCSRFToken = if
 *    true CSRF token is omitted. [Default is false] disableSubmit = don't allow form to be submitted. [Default is
 *    false] submitOnReturn = submit form if user hits return in any form element. [Default is false]
 */
function caFormTag(
	$po_request, $ps_action, $ps_id, $ps_module_and_controller_path = null, $ps_method = 'post',
	$ps_enctype = 'multipart/form-data', $ps_target = '_top', $pa_options = null
) {
	if ( $ps_target ) {
	...
	if ( $ps_module_and_controller_path ) {
		$vs_action = ( caUseCleanUrls() )
			...
			:
			$po_request->getBaseUrlPath() . '/' . $po_request->getScriptName() . '/' . $ps_module_and_controller_path
			. '/' . $ps_action;
	} else {
		$vs_action = ( caUseCleanUrls() )
			?
	...
	$vs_buf = "<form action='" . $vs_action . "' method='" . $ps_method . "' id='" . $ps_id . "' $vs_target enctype='"
...
function caFormSubmitLink( $po_request, $ps_content, $ps_classname, $ps_form_id, $ps_id = null ) {
	return "<a href='#' onclick='document.getElementById(\"{$ps_form_id}\").submit();' class='{$ps_classname}' "
	       . ( $ps_id ? "id='{$ps_id}'" : '' ) . ">" . $ps_content . "</a>";
}
...
function caFormSubmitButton( $po_request, $pn_type, $ps_content, $ps_id, $pa_options = null ) {
	$ps_use_classname             = isset( $pa_options['class'] ) ? $pa_options['class'] : '';
	$pb_no_background             = ( isset( $pa_options['no_background'] ) && $pa_options['no_background'] ) ? true
		: false;
	$pb_dont_show_content         = ( isset( $pa_options['dont_show_content'] ) && $pa_options['dont_show_content'] )
		? true : false;
	$pb_prevent_duplicate_submits = ( isset( $pa_options['preventDuplicateSubmits'] )
	                                  && $pa_options['preventDuplicateSubmits'] ) ? true : false;
	...
	if ( $pb_prevent_duplicate_submits ) {
		$vs_button
			= "<a href='#' onclick='$vs_extra jQuery(\".caSubmit{$ps_id}\").fadeTo(\"fast\", 0.5).attr(\"onclick\", null); jQuery(\"#{$ps_id}\").submit();' class='{$vs_classname} caSubmit{$ps_id} {$vs_id}' "
			  . ( $css_id ? "id='{$css_id}'" : "" ) . ">";
	} else {
...
function caFormSearchButton( $po_request, $pn_type, $ps_content, $ps_id, $pa_options = null ) {
	...
	return caFormSubmitButton( $po_request, $pn_type, $ps_content, $ps_id,
		array_merge( $pa_options, array( 'size' => 2 ) ) );
}
...
function caFormJSButton( $po_request, $pn_type, $ps_content, $ps_id, $pa_attributes = null, $pa_options = null ) {
	...
	return caJSButton( $po_request, $pn_type, $ps_content, $ps_id, $pa_attributes,
		array_merge( $pa_options, array( 'size' => '30px' ) ) );
}
...
function caNavFormParameters( $pa_other_params = null ) {
	...
	if ( is_array( $pa_other_params ) && sizeof( $pa_other_params ) ) {
		foreach ( $pa_other_params as $vs_name => $vs_val ) {
			$vs_buf .= '<input type="hidden" name="' . $vs_name . '" value="' . htmlspecialchars( $vs_val, ENT_QUOTES,
					'UTF-8' ) . '"/>' . "\n";
		}
	}
 ...
 * @param mixed $pn_size       Size of icon expressed as FontAwesome magnification level (Ex. 2) or pixel height (Ex.
 *                             24px). Text values will be applied as CSS classes to the icon. [Default is 2]
...
function _caNavIconTypeToName( $pn_type ) {
	...
	return array(
		'class'    => trim( "{$vs_fa_class} {$vs_ca_class}" ),
		'fa-class' => $vs_fa_class,
		'ca-class' => $vs_ca_class
	);
}

# ------------------------------------------------------------------------------------------------
/**
 * Returns an HTML to edit an item in the appropriate bundle-based editor. If no specified item is specified (eg. no id
 * value is set) the a link to create a new item of the specfied type is returned.
 *
 ...
 * @param int         $pn_id                    Optional row_id for edited item. If omitted a link will be returned to
 *                                              create a new item record. Note that unless the verifyLink option is
 *                                              set, the link will be returned with the specified id whether or not it
 *                                              actually exists.
 * @param array       $pa_additional_parameters Optional array of parameters to return on the editor url
 * @param array       $pa_attributes            Optional array of attributes to set on the link's <a> tag. You can use
 *                                              this to set the id of the link, as well as any other <a> parameter.
 * @param array       $pa_options               Optional array of options. Supported options are:
 *                                              verifyLink - if true and $pn_id is set, then existence of record with
 *                                              specified id is verified before link is returned. If the id does not
 *                                              exist then null is returned. Default is false - no verification
 *                                              performed.
 *
 * @return string The <a> tag as string
 */
function caEditorLink(
	$po_request, $ps_content, $ps_classname, $ps_table, $pn_id, $pa_additional_parameters = null, $pa_attributes = null,
	$pa_options = null
) {
	if ( ! ( $vs_url = caEditorUrl( $po_request, $ps_table, $pn_id, false, $pa_additional_parameters,
		$pa_options ) )
	) {
		return null;
	}
 ...
 *                                              specified id is verified before link is returned. If the id does not
 *                                              exist then null is returned. Default is false - no verification
 *                                              performed. preferredDetail =
 */
function caDetailLink(
	$po_request, $ps_content, $ps_classname, $ps_table, $pn_id, $pa_additional_parameters = null, $pa_attributes = null,
	$pa_options = null
) {
	if ( ! ( $vs_url = caDetailUrl( $po_request, $ps_table, $pn_id, false, $pa_additional_parameters,
		$pa_options ) )
	) {
		return null;
	}
...
# ------------------------------------------------------------------------------------------------
/**
 * Returns url to edit an item in the appropriate bundle-based editor. If no specified item is specified (eg. no id
 * value is set) the a link to create a new item of the specfied type is returned.
 *
 * @param HTTPRequest $po_request               The current request object
 * @param string      $ps_table                 The name or table_num of the edited items table
 ...
 *                                              set, the link will be returned with the specified id whether or not it
 *                                              actually exists.
 * @param boolean     $pb_return_url_as_pieces  If true an array is returned with the various components of the editor
 *                                              URL as separate keys. The keys will be 'module', 'controller', 'action'
 *                                              and '_pk' (the name of the primary key for the item); the primary key
 *                                              value itself is returned as both 'id' and whatever the primary key name
 *                                              is (eg. named whatever the value of _pk is). Default is false - return
 *                                              as a string rather than array.
 * @param array       $pa_additional_parameters Optional array of parameters to return on the editor url
 * @param array       $pa_options               Optional array of options. Supported options are:
 *                                              verifyLink - if true and $pn_id is set, then existence of record with
 *                                              specified id is verified before link is returned. If the id does not
 *                                              exist then null is returned. Default is false - no verification
 *                                              performed. action - if set, action of returned link will be set to the
 *                                              supplied value quick_add - if set to true, returned link will point to
 *                                              the QuickAdd controller instead
 *
 * @return array|string
 */
function caEditorUrl(
	$po_request, $ps_table, $pn_id = null, $pb_return_url_as_pieces = false, $pa_additional_parameters = null,
	$pa_options = null
) {
	...
	switch ( $vs_table ) {
		case 'ca_relationship_types':
			$vs_action = isset( $pa_options['action'] )
				? $pa_options['action']
				: ( ( $po_request->isLoggedIn()
				      && $po_request->user->canDoAction( 'can_configure_relationship_types' ) ) ? 'Edit' : 'Summary' );
			break;
		default:
			if ( isset( $pa_options['action'] ) ) {
				...
				&&
				! ( (bool) $po_request->config->get( $vs_table . '_editor_defaults_to_summary_view' )
				    && $pn_id ) // when the id is null/0, we go to the Edit action, even when *_editor_defaults_to_summary_view is set
			) {
				$vs_action = 'Edit';
			} else {
				$vs_action = 'Summary';
 ...
 *                                              exist then null is returned. Default is false - no verification
 *                                              performed. action = if set, action of returned link will be set to the
 *                                              supplied value preferredDetail = type_id = type_id of item to get
 *                                              detail for
 */
function caDetailUrl(
	$po_request, $ps_table, $pn_id = null, $pb_return_url_as_pieces = false, $pa_additional_parameters = null,
	$pa_options = null
) {
	if ( is_numeric( $ps_table ) ) {
	...
	if ( isset( $pa_options['action'] ) ) {
		...
		$vs_action = caGetDetailForType( $ps_table, $vn_type_id, array(
			'request'         => $po_request,
			'preferredDetail' => caGetOption( 'preferredDetail', $pa_options, null )
		) );
	}
	...
	if ( ( (int) $pn_id > 0 ) && ( $vs_use_alt_identifier_in_urls = caUseAltIdentifierInUrls( $ps_table ) )
		...
		$va_ids = $t_table->getFieldValuesForIDs( array( $pn_id ),
			array( $t_table->getProperty( 'ID_NUMBERING_ID_FIELD' ) ) );
		if ( is_array( $va_ids ) && ( $vn_id_for_idno = array_shift( $va_ids ) ) ) {
			$vb_id_exists = true;
		}

		$pn_id = ( strlen( $vn_id_for_idno ) && ( strpos( $vn_id_for_idno, '/' ) === false ) ) ? $vn_id_for_idno
			: "id:{$pn_id}";
	}
...
function caUseCleanUrls() {
	...
	return $g_use_clean_urls = ( defined( '__CA_USE_CLEAN_URLS__' ) && ( __CA_USE_CLEAN_URLS__ )
	                             && caModRewriteIsAvailable() );
}
