...
function caGetUserAccessValues($po_request, $pa_options = null)
{
    ...
    $vb_dont_enforce_access_settings = isset($pa_options['dont_enforce_access_settings']) ? (bool)$pa_options['dont_enforce_access_settings'] : $po_request->config->get(
        'dont_enforce_access_settings'
    );
    $va_privileged_access_settings = isset($pa_options['privileged_access_settings']) && is_array(
        $pa_options['privileged_access_settings']
    ) ? (bool)$pa_options['privileged_access_settings'] : (array)$po_request->config->getList(
        'privileged_access_settings'
    );
    $va_public_access_settings = isset($pa_options['public_access_settings']) && is_array(
        $pa_options['public_access_settings']
...
{
    $va_privileged_networks = isset($pa_options['privileged_networks']) && is_array(
        $pa_options['privileged_networks']
    ) ? $pa_options['privileged_networks'] : (array)$po_request->config->getList('privileged_networks');
...
function caGetSourceRestrictionsForUser($pm_table_name_or_num, $pa_options = null)
{
    ...
    $va_source_ids = null;
    if ((bool)$t_instance->getAppConfig()->get(
            'perform_source_access_checking'
        ) && $g_request && $g_request->isLoggedIn()) {
        if (is_array(
            $va_source_ids = $g_request->user->getSourcesWithAccess(
                $t_instance->tableName(),
                $vn_min_access,
                $pa_options
            )
        )) {
            $va_source_ids = caMakeSourceIDList(
                $pm_table_name_or_num,
                $va_source_ids,
                array_merge(
                    $pa_options,
                    array('dont_include_subsources_in_source_restriction' => true)
                )
            );
        }
    }
...
function caMakeTypeIDList($pm_table_name_or_num, $pa_types, $pa_options = null)
{
    ...
    $vs_cache_key = caMakeCacheKeyFromOptions(
        array_merge($pa_options, $pa_types),
...
{
    if (!is_array($pa_item_idnos) && !strlen($pa_item_idnos)) {
        return [];
    }
    if (!is_array($pa_item_idnos)) {
        $pa_item_idnos = [$pa_item_idnos];
    }
    if (!is_array($pa_options)) {
        $pa_options = [];
    }
    $vs_cache_key = caMakeCacheKeyFromOptions(
        array_merge($pa_options, $pa_item_idnos),
...
function caGetRelationshipTableName($pm_table_name_or_num_left, $pm_table_name_or_num_right, $pa_options = null)
{
    ...
    foreach (array_keys($va_path) as $vs_table) {
        if ($t_instance = Datamodel::getInstanceByTableName($vs_table, true)) {
            if (method_exists($t_instance, "isRelationship") && $t_instance->isRelationship() && $t_instance->hasField(
                    'type_id'
                )) {
                return $vs_table;
            }
...
function caMergeTypeRestrictionLists($t_instance, $pa_options)
{
    ...
    $o_config = Configuration::load();
    if ((bool)$o_config->get('perform_type_access_checking') && method_exists(
            $t_instance,
            'getTypeFieldName'
        ) && ($vs_type_field_name = $t_instance->getTypeFieldName())) {
        $va_types = caGetTypeRestrictionsForUser($t_instance->tableName());
    }
    if (is_array($va_types) && sizeof($va_types) && is_array($va_restrict_to_type_ids) && sizeof(
            $va_restrict_to_type_ids
        )) {
...
function caMergeSourceRestrictionLists($t_instance, $pa_options)
{
    ...
    if (is_array($pa_options['restrictToSources']) && sizeof($pa_options['restrictToSources'])) {
        $va_restrict_to_source_ids = caMakeSourceIDList(
            $t_instance->tableName(),
            $pa_options['restrictToSources'],
    ...
    $o_config = Configuration::load();
    if ((bool)$o_config->get('perform_source_access_checking') && method_exists(
            $t_instance,
            'getSourceFieldName'
        ) && ($vs_source_field_name = $t_instance->getSourceFieldName())) {
        $va_sources = caGetSourceRestrictionsForUser($t_instance->tableName());
    }
    if (is_array($va_sources) && sizeof($va_sources) && is_array($va_restrict_to_source_ids) && sizeof(
            $va_restrict_to_source_ids
        )) {
...
function caGetBundleAccessLevel($ps_table_name, $ps_bundle_name)
{
    ...
    if ($g_request) {
        return $g_bundle_access_level_cache[$ps_table_name][$ps_bundle_name] = $g_request->user->getBundleAccessLevel(
            $ps_table_name,
            $ps_bundle_name
        );
    }
    ...
    $o_config = Configuration::load();
    return $g_bundle_access_level_cache[$ps_table_name][$ps_bundle_name] = (int)$o_config->get(
        'default_bundle_access_level'
    );
}
...
function caGetTypeAccessLevel($ps_table_name, $pm_type_code_or_id)
{
    ...
    if ($g_request) {
        return $g_type_access_level_cache[$ps_table_name][$pm_type_code_or_id] = $g_request->user->getTypeAccessLevel(
            $ps_table_name,
            $pm_type_code_or_id
        );
    }
    ...
    $o_config = Configuration::load();
    return $g_type_access_level_cache[$ps_table_name][$pm_type_code_or_id] = (int)$o_config->get(
        'default_type_access_level'
    );
}
...
function caGetSourceAccessLevel($ps_table_name, $pm_source_code_or_id)
{
    ...
    if ($g_request) {
        return $g_source_access_level_cache[$ps_table_name][$pm_source_code_or_id] = $g_request->user->getSourceAccessLevel(
            $ps_table_name,
            $pm_source_code_or_id
        );
    }

    $o_config = Configuration::load();
    return $g_source_access_level_cache[$ps_table_name][$pm_source_code_or_id] = (int)$o_config->get(
        'default_source_access_level'
    );
}
