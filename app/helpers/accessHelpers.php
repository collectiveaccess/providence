 ...
 * @param array       $pa_options Optional options. If omitted settings are taken application configuration file is
 *                                used. Any array passed to this function should include the following keys:
 *                                "dont_enforce_access_settings", "public_access_settings",
 *                                "privileged_access_settings", "privileged_networks"
 *
 * @return array An array of integer values that, if present in a record, indicate that the record should be displayed
 *               to the current user
 */
function caGetUserAccessValues( $po_request, $pa_options = null ) {
	...
	$vb_dont_enforce_access_settings = isset( $pa_options['dont_enforce_access_settings'] )
		? (bool) $pa_options['dont_enforce_access_settings']
		: $po_request->config->get( 'dont_enforce_access_settings' );
	$va_privileged_access_settings   = isset( $pa_options['privileged_access_settings'] )
	                                   && is_array( $pa_options['privileged_access_settings'] )
		? (bool) $pa_options['privileged_access_settings']
		: (array) $po_request->config->getList( 'privileged_access_settings' );
	$va_public_access_settings       = isset( $pa_options['public_access_settings'] )
	                                   && is_array( $pa_options['public_access_settings'] )
		? $pa_options['public_access_settings'] : (array) $po_request->config->getList( 'public_access_settings' );
 ...
 * @param RequestHTTP $po_request The current request
 * @param array       $pa_options Optional options. If omitted settings are taken application configuration file is
 *                                used. Any array passed to this function should include "privileged_networks" as a key
 *                                with a value listing all privileged networks
 *
 * @return boolean True if user is privileged, false if not
 */
function caUserIsPrivileged( $po_request, $pa_options = null ) {
	$va_privileged_networks = isset( $pa_options['privileged_networks'] )
	                          && is_array( $pa_options['privileged_networks'] ) ? $pa_options['privileged_networks']
		: (array) $po_request->config->getList( 'privileged_networks' );
 ...
 *                                    access = minimum access level user must have to a type for it to be returned.
 *                                    Values are:
 *                                    __CA_BUNDLE_ACCESS_NONE__ (0)
 *                                    __CA_BUNDLE_ACCESS_READONLY__ (1)
 *                                    __CA_BUNDLE_ACCESS_EDIT__ (2)
 *                                    If not specified types are returned for which the user has at least
...
function caGetTypeRestrictionsForUser( $pm_table_name_or_num, $pa_options = null ) {
	...
	if ( isset( $g_access_helpers_type_restriction_cache[ $vs_cache_key ] ) ) {
		return $g_access_helpers_type_restriction_cache[ $vs_cache_key ];
	}
	...
	$va_type_ids = null;
	if ( (bool) $t_instance->getAppConfig()->get( 'perform_type_access_checking' ) && $g_request
	     && $g_request->isLoggedIn()
	) {
		if ( is_array(
			$va_type_ids = $g_request->user->getTypesWithAccess( $t_instance->tableName(), $vn_min_access ) )
		) {
			$va_type_ids = caMakeTypeIDList( $pm_table_name_or_num, $va_type_ids,
				array_merge( $pa_options, array( 'dont_include_subtypes_in_type_restriction' => true ) ) );
		}
	}
 ...
 * @param array $pa_options           Array of options:
 *                                    access = minimum access level user must have to a source for it to be returned.
 *                                    Values are:
 *                                    __CA_BUNDLE_ACCESS_NONE__ (0)
 *                                    __CA_BUNDLE_ACCESS_READONLY__ (1)
 *                                    __CA_BUNDLE_ACCESS_EDIT__ (2)
 *                                    If not specified sources are returned for which the user has at least
 *                                    __CA_BUNDLE_ACCESS_READONLY__
 *
 *        exactAccess = if set only sources with access equal to the "access" value are returned. Default is false.
 *
...
function caGetSourceRestrictionsForUser( $pm_table_name_or_num, $pa_options = null ) {
	...
	$va_source_ids = null;
	if ( (bool) $t_instance->getAppConfig()->get( 'perform_source_access_checking' ) && $g_request
	     && $g_request->isLoggedIn()
	) {
		if ( is_array(
			$va_source_ids = $g_request->user->getSourcesWithAccess( $t_instance->tableName(), $vn_min_access,
				$pa_options ) )
		) {
			$va_source_ids = caMakeSourceIDList( $pm_table_name_or_num, $va_source_ids,
				array_merge( $pa_options, array( 'dont_include_subsources_in_source_restriction' => true ) ) );
		}
	}
...
# ---------------------------------------------------------------------------------------------
/**
 * Converts the given list of type codes or type_ids into an expanded list of numeric type_ids suitable for enforcing
 * type restrictions. Processing includes expansion of types to include subtypes and conversion of any type codes to
 * type_ids.
 *
 ...
 * @param array $pa_options           Array of options:
 *                                    dont_include_subtypes_in_type_restriction = if set, returned list is not expanded
 *                                    to include subtypes dontIncludeSubtypesInTypeRestriction = synonym for
...
function caMakeTypeIDList( $pm_table_name_or_num, $pa_types, $pa_options = null ) {
	...
	$vs_cache_key = caMakeCacheKeyFromOptions( array_merge( $pa_options, $pa_types ),
		"caMakeTypeIDList:{$pm_table_name_or_num}" );
 ...
 * type restrictions, which is why the options for expanding the list to include sub-items use "subtypes" in their
 * names
 ...
 * @param array $pa_options         Array of options:
 *                                  dont_include_subtypes_in_type_restriction = if set, returned list is not expanded
 *                                  to include sub-items dontIncludeSubtypesInTypeRestriction = synonym for
 *                                  dont_include_subtypes_in_type_restriction
 *
...
function caMakeItemIDList( $pm_list_code_or_id, $pa_item_idnos, $pa_options = null ) {
	...
	$vs_cache_key = caMakeCacheKeyFromOptions( array_merge( $pa_options, $pa_item_idnos ),
		"caMakeItemIDList:{$pm_list_code_or_id}" );
	if ( ExternalCache::contains( $vs_cache_key, 'listItems' ) ) {
		return ExternalCache::fetch( $vs_cache_key, 'listItems' );
	}
...
function caMakeTypeList( $pm_table_name_or_num, $pa_type_ids, $pa_options = null ) {
	...
	foreach ( $pa_type_ids as $vm_type ) {
		...
		if ( $vs_type_code && ( ! isset( $pa_options['noChildren'] ) || ! $pa_options['noChildren'] ) ) {
			if ( $qr_children = $t_item->getHierarchy( caGetListItemID( $vs_type_list_code, $vs_type_code ),
				array() )
			) {
				while ( $qr_children->nextRow() ) {
...
# ---------------------------------------------------------------------------------------------
/**
 * Converts the given list of source names or source_ids into an expanded list of numeric source_ids suitable for
 * enforcing source restrictions. Processing includes expansion of sources to include subsources and conversion of any
 * source codes to source_ids.
 *
 ...
 * @param array $pa_options           Array of options:
 *                                    dont_include_subsources_in_source_restriction = if set, returned list is not
 *                                    expanded to include subsources dontIncludeSubsourcesInSourceRestriction = synonym
 *                                    for dont_include_subsources_in_source_restriction
 *
 * @return array List of numeric source_ids
 */
function caMakeSourceIDList( $pm_table_name_or_num, $pa_sources, $pa_options = null ) {
	if ( isset( $pa_options['dontIncludeSubsourcesInSourceRestriction'] )
	     && ( ! isset( $pa_options['dont_include_subsources_in_source_restriction'] )
	          || ! $pa_options['dont_include_subsources_in_source_restriction'] )
	) {
		$pa_options['dont_include_subsources_in_source_restriction']
			= $pa_options['dontIncludeSubsourcesInSourceRestriction'];
	}
	...
	if ( isset( $pa_options['dont_include_subsources_in_source_restriction'] )
	     && $pa_options['dont_include_subsources_in_source_restriction']
	) {
		$pa_options['noChildren'] = true;
	}
...
# ------------------------------------------------------
/**
 * Converts the given list of relationship type names or relationship type_ids into an expanded list of numeric
 * type_ids suitable for enforcing relationship type restrictions. Processing includes expansion of types to include
 * subtypes and conversion of any type codes to type_ids.
 *
 * @param mixed $pm_table_name_or_num Table name or number to which types apply
 * @param array $pa_types             List of type codes and/or type_ids that are the basis of the list
 * @param array $pa_options           Array of options:
...
function caMakeRelationshipTypeIDList( $pm_table_name_or_num, $pa_types, $pa_options = null ) {
	...
	$pa_options['includeChildren'] = ( isset( $pa_options['dont_include_subtypes_in_type_restriction'] )
	                                   && $pa_options['dont_include_subtypes_in_type_restriction'] ) ? false : true;
...
# ---------------------------------------------------------------------------------------------
/**
 * Merges types specified with any specified "restrict_to_types"/"restrictToTypes" option, user access settings and
 * types configured in app.conf into a single list of type_ids suitable for enforcing type restrictions.
 *
 * @param BaseModel $t_instance A model instance for the table to which the types apply
 * @param array     $pa_options An array of options containing, if specified, a list of types for either the
 *                              "restrict_to_types" or "restrictToTypes" keys. Other options include:
 *                              dontIncludeSubtypesInTypeRestriction = Don't expand types to include child types.
 *                              [Default is true]
 *
...
function caMergeTypeRestrictionLists( $t_instance, $pa_options ) {
	...
	if ( is_array( $pa_options['restrictToTypes'] ) && sizeof( $pa_options['restrictToTypes'] ) ) {
		$va_restrict_to_type_ids = caMakeTypeIDList( $t_instance->tableName(), $pa_options['restrictToTypes'], array(
			'noChildren' => caGetOption( [
				'dontIncludeSubtypesInTypeRestriction',
				'dont_include_subtypes_in_type_restriction'
			], $pa_options, true )
		) );
	}
	...
	$o_config = Configuration::load();
	if ( (bool) $o_config->get( 'perform_type_access_checking' ) && method_exists( $t_instance, 'getTypeFieldName' )
	     && ( $vs_type_field_name = $t_instance->getTypeFieldName() )
	) {
		$va_types = caGetTypeRestrictionsForUser( $t_instance->tableName() );
	}
	if ( is_array( $va_types ) && sizeof( $va_types ) && is_array( $va_restrict_to_type_ids )
	     && sizeof( $va_restrict_to_type_ids )
	) {
...
# ---------------------------------------------------------------------------------------------
/**
 * Merges sources specified with any specified "restrict_to_sources"/"restrictToSources" option, user access settings
 * and sources configured in app.conf into a single list of source_ids suitable for enforcing source restrictions.
 *
 * @param BaseModel $t_instance A model instance for the table to which the sources apply
 * @param array     $pa_options An array of options containing, if specified, a list of sources for either the
 *                              "restrict_to_sources" or "restrictToSources" keys
 *
...
function caMergeSourceRestrictionLists( $t_instance, $pa_options ) {
	...
	if ( is_array( $pa_options['restrictToSources'] ) && sizeof( $pa_options['restrictToSources'] ) ) {
		$va_restrict_to_source_ids = caMakeSourceIDList( $t_instance->tableName(), $pa_options['restrictToSources'],
			array( 'noChildren' => true ) );
	}

	$va_sources = null;

	$o_config = Configuration::load();
	if ( (bool) $o_config->get( 'perform_source_access_checking' ) && method_exists( $t_instance, 'getSourceFieldName' )
	     && ( $vs_source_field_name = $t_instance->getSourceFieldName() )
	) {
		$va_sources = caGetSourceRestrictionsForUser( $t_instance->tableName() );
	}
	if ( is_array( $va_sources ) && sizeof( $va_sources ) && is_array( $va_restrict_to_source_ids )
	     && sizeof( $va_restrict_to_source_ids )
	) {
...
function caGetBundleAccessLevel( $ps_table_name, $ps_bundle_name ) {
	...
	if ( $g_request ) {
		return $g_bundle_access_level_cache[ $ps_table_name ][ $ps_bundle_name ]
			= $g_request->user->getBundleAccessLevel( $ps_table_name, $ps_bundle_name );
	}

	$o_config = Configuration::load();

	return $g_bundle_access_level_cache[ $ps_table_name ][ $ps_bundle_name ]
		= (int) $o_config->get( 'default_bundle_access_level' );
}
...
function caGetTypeAccessLevel( $ps_table_name, $pm_type_code_or_id ) {
	...
	if ( $g_request ) {
		return $g_type_access_level_cache[ $ps_table_name ][ $pm_type_code_or_id ]
			= $g_request->user->getTypeAccessLevel( $ps_table_name, $pm_type_code_or_id );
	}

	$o_config = Configuration::load();

	return $g_type_access_level_cache[ $ps_table_name ][ $pm_type_code_or_id ]
		= (int) $o_config->get( 'default_type_access_level' );
}
...
function caGetSourceAccessLevel( $ps_table_name, $pm_source_code_or_id ) {
	...
	if ( $g_request ) {
		return $g_source_access_level_cache[ $ps_table_name ][ $pm_source_code_or_id ]
			= $g_request->user->getSourceAccessLevel( $ps_table_name, $pm_source_code_or_id );
	}

	$o_config = Configuration::load();

	return $g_source_access_level_cache[ $ps_table_name ][ $pm_source_code_or_id ]
		= (int) $o_config->get( 'default_source_access_level' );
}
 ...
 * @param mixed  $pm_id          A primary key value of the row, or an array of values to check. If a single integer
 *                               value is provided then a boolean result will be returned; if an array of values is
 *                               provided then an array will be returned with all ids that are readable
 * @param string $ps_bundle_name An optional bundle to check access for
 *
 * @return array If $pm_id is an integer return true if user has read access, otherwise false if the user does not have
 *               access; if $pm_id is an array of ids, returns an array with all ids the are readable; returns null if
...
# ---------------------------------------------------------------------------------------------
/**
 * Transforms various alternative bundle expressions supported by get() into canonical bundle names suitable for
 * determining users' access rights.
 *
...
function caTranslateBundlesForAccessChecking( $ps_table_name, $ps_bundle_name ) {
	...
	// Related tables
	if ( $t_rel && ( is_array( $va_path = Datamodel::getPath( $va_tmp[0], $ps_table_name ) ) )
	     && ( sizeof( $va_path ) == 3 )
	) {
