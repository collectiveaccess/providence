 ...
 *
 * @param array $pa_theme_device_mappings Array of mappings; keys are Perl-compatible regexes to be applied to the user
 *                                        agent; values are the names of themes to do upon a regex match; the theme
 *                                        assigned to the special _default_ key is used if there are no matches
 *
 * @return string Name of theme to use. If there are no matches and there is no _default_ value set in the mappings,
 *                then the string "default" will be returned. (It is assumed there is always a theme named "default"
 *                available.)
 */
...
# --------------------------------------------------------------------------------------------
/**
 * Return true if system is configured to alternate identifers (a metadata element) rather than internal numeric CA
 * primary keys in urls when referring to a specific record
 *
 * @return bool
 */
function caUseAltIdentifierInUrls( $ps_table ) {
	...
	return (bool) $o_config->get( 'use_alternate_identifiers_in_urls_for_' . $ps_table )
		? $o_config->get( 'use_alternate_identifiers_in_urls_for_' . $ps_table ) : false;
}
...
function caCheckVendorLibraries() {
	if ( ! file_exists( __CA_BASE_DIR__ . "/vendor/autoload.php" ) ) {
		if ( ( (int) $_REQUEST['updateVendor'] === 1 )
		     && ( defined( '__CA_ALLOW_AUTOMATIC_UPDATE_OF_VENDOR_DIR__' )
		          && __CA_ALLOW_AUTOMATIC_UPDATE_OF_VENDOR_DIR__ )
		) {
			if ( sizeof( $errors = caInstallVendorLibraries() ) ) {
				$opa_error_messages = [
					"Automatic installation of the required vendor libraries failed: " . join( "; ", $errors )
				];
			...
			if ( defined( '__CA_ALLOW_AUTOMATIC_UPDATE_OF_VENDOR_DIR__' )
				...
				$opa_error_messages
					= [ "Your installation is missing required vendor libraries. This is normal if you have just installed from a Git branch. See the <a href=\"http://docs.collectiveaccess.org/wiki/Installing_Vendor_Libraries\">wiki</a> for instructions on installing the required libraries manually." ];
			}
			require_once( __CA_THEME_DIR__ . "/views/system/configuration_error_html.php" );
			exit;
		}
	...
	if ( ! file_exists( __CA_BASE_DIR__ . "/vendor/autoload.php" ) ) {
		if ( ! file_exists( __CA_BASE_DIR__ . "/vendor" ) ) {
			if ( ! ( @mkdir( __CA_BASE_DIR__ . "/vendor" ) ) || ! file_exists( __CA_BASE_DIR__ . "/vendor" ) ) {
				return [
					"Could not create the vendor directory. Make sure " . __CA_BASE_DIR__
					. " is writable or create the directory manually and make sure it is writeable by the user which runs the webserver."
				];
			}
		}
		if ( ! is_writable( __CA_BASE_DIR__ . "/vendor" ) ) {
			return [
				"The vendor directory is not writable. Please change the permissions of " . __CA_BASE_DIR__
				. "/vendor and enable the user which runs the webserver to write to this directory."
			];
		}
		if ( ! copy( __CA_BASE_DIR__ . '/support/scripts/install_composer.sh.txt',
			__CA_APP_DIR__ . '/tmp/install_composer.sh' )
		) {
