...
{
    $o_config = Configuration::load();
    return (bool)$o_config->get('use_alternate_identifiers_in_urls_for_' . $ps_table) ? $o_config->get(
        'use_alternate_identifiers_in_urls_for_' . $ps_table
    ) : false;
}
...
function caCheckVendorLibraries()
{
    if (!file_exists(__CA_BASE_DIR__ . "/vendor/autoload.php")) {
        if (((int)$_REQUEST['updateVendor'] === 1) && (defined(
                    '__CA_ALLOW_AUTOMATIC_UPDATE_OF_VENDOR_DIR__'
                ) && __CA_ALLOW_AUTOMATIC_UPDATE_OF_VENDOR_DIR__)) {
            if (sizeof($errors = caInstallVendorLibraries())) {
                $opa_error_messages = [
                    "Automatic installation of the required vendor libraries failed: " . join(
                        "; ",
                        $errors
                    )
                ];
...
function caInstallVendorLibraries()
{
    ...
    if (!file_exists(__CA_BASE_DIR__ . "/vendor/autoload.php")) {
        ...
        if (!copy(
            __CA_BASE_DIR__ . '/support/scripts/install_composer.sh.txt',
