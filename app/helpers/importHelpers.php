...
function caProcessRefineryParents(
    ...
    foreach ($pa_parents as $vn_i => $va_parent) {
        ...
        $va_attributes = (isset($va_parent['attributes']) && is_array(
                $va_parent['attributes']
            )) ? $va_parent['attributes'] : array();
        ...
        foreach ($va_attributes as $vs_element_code => $va_attrs) {
            if (is_array($va_attrs)) {
                ...
                foreach ($va_attrs as $vs_k => $vs_v) {
                    ...
                    $va_attr_vals = BaseRefinery::parsePlaceholder(
                        $vs_v,
                        $pa_source_data,
                        $pa_item,
                        null,
                        array(
                            ...
                            'delimiter' => $vs_attr_delimiter
                        )
                    );

                    if (!is_array($va_attr_vals)) {
                        $va_attr_vals = [$va_attr_vals];
                    }
                    ...
                    foreach ($va_rels as $va_rel) {
                        if (!$t_subject->addRelationship($vs_table, $va_rel['id'], $va_rel['_relationship_type'])) {
                            if ($o_log) {
                                $o_log->logDebug(
                                    _t(
                                        '[%6] Could not create relationship between parent %1 and %2 for ids %3 and %4 with type %5',
                                        $ps_table,
                                        $vs_table,
                                        $vn_id,
                                        $va_rel['id'],
    ...
    foreach ($pa_related_option_list as $vn_i => $pa_related_options) {
        ...
        if (!is_array(
            $va_name = BaseRefinery::parsePlaceholder(
                caGetOption(['name', 'preferredLabel', 'preferredLabels'], $pa_related_options, null),
                $pa_source_data,
                $pa_item,
                $pn_c,
        ...
        foreach ($va_name as $i => $vs_name) {
            ...
            if (is_array($va_non_preferred_labels = $pa_related_options["nonPreferredLabels"])) {
                ...
                if ($ps_refinery_name == 'entitySplitter') {
                    if ($vs_npl = trim(
                        DataMigrationUtils::splitEntityName(
                        ...
                        $pa_options['nonPreferredLabels'][] = $vs_npl;
                    };
                } else {
                    if ($vs_npl = trim(
                        BaseRefinery::parsePlaceholder(
                ...
                if ($va_hierarchy_setting = $pa_item['settings']["{$ps_refinery_name}_hierarchy"]) {
                    ...
                    if (($ps_refinery_name == 'storageLocationSplitter') && ($va_hier_delimiter = $pa_item['settings']['storageLocationSplitter_hierarchicalDelimiter'])) {
                        ...
                        if (sizeof($va_location_hier) > 1) {
                            ...
                            if (!is_array(
                                $va_types = $pa_item['settings']['storageLocationSplitter_hierarchicalStorageLocationTypes']
                            )) {
                                $va_types = array();
                            }
                            ...
                            foreach ($va_location_hier as $vn_ix => $vs_parent) {
                                ...
                                $vn_location_id = DataMigrationUtils::getStorageLocationID(
                                    $vs_parent,
                                    $vn_location_id,
                                    $vs_type,
                                    $g_ui_locale_id,
