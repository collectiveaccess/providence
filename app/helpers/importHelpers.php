...
function caProcessRefineryParents(
    ...
    foreach ($pa_parents as $vn_i => $va_parent) {
        ...
        if (!is_array($va_parent)) {
            $o_log->logWarn(
                _t(
                    '[%2] Parents options invalid. Did you forget to pass a list? Parents list passed was: %1',
                    print_r($pa_parents, true),
                    $ps_refinery_name
                )
            );
            break;
        }
        $vs_name = BaseRefinery::parsePlaceholder(
            $va_parent['name'],
            $pa_source_data,
            $pa_item,
            $pn_c,
        ...
        if (($vs_idno = BaseRefinery::parsePlaceholder(
                $va_parent['idno'],
                $pa_source_data,
                $pa_item,
                $pn_c,
            ...
            $vs_idno = BaseRefinery::parsePlaceholder(
                $va_parent['idnoPrefix'] . $va_parent['idno'],
                $pa_source_data,
                $pa_item,
                $pn_c,
                array(
        ...
        $vs_type = BaseRefinery::parsePlaceholder(
            $va_parent['type'],
            $pa_source_data,
            $pa_item,
            $pn_c,
        ...
        if ((get_class($id_numberer) === 'MultipartIDNumber') && caGetOption('normalizeIdno', $va_parent, false)) {
            ...
            $id_number_elements = method_exists($id_numberer, 'getElements') ? $id_numberer->getElements(
                $vs_type
            ) : null;
            ...
            if (is_array($id_number_elements) && (sizeof(
                        $id_number_elements
                    ) > 1)) {        // don't bother normalizing single-element formats
        ...
        $va_attributes_spec = (isset($va_parent['attributes']) && is_array(
                $va_parent['attributes']
            )) ? $va_parent['attributes'] : [];

        // Process "attributes" block
        $va_attributes = array_merge(
            caProcessRefineryAttributes(
                $va_attributes_spec,
                $pa_source_data,
                $pa_item,
                $pn_c,
        ...
        if (isset($va_parent['rules']) && is_array($va_parent['rules'])) {
            foreach ($va_parent['rules'] as $va_rule) {
                try {
                                ...
                                case 'set':
                                    switch ($va_action['target']) {
                                        ...
                                        case 'type':
                                            ...
                                            break;
                                        default:
                                            $va_attributes[$va_action['target']] = BaseRefinery::parsePlaceholder(
                                                $va_action['value'],
                                                $pa_source_data,
                                                $pa_item,
                                                $pn_c,
                                ...
                                default:
                                    if ($o_log) {
                                        if ($vs_action_code != 'skip') {
                                            $o_log->logInfo(
                                                _t(
                                                    '[%3] Parent was skipped using rule "%1" with default action because an invalid action ("%2") was specified',
                                                    $va_rule['trigger'],
                                                    $vs_action_code,
                                                    $ps_refinery_name
                                                )
                                            ...
                                            $o_log->logDebug(
                                                _t(
                                                    '[%3] Parent was skipped using rule "%1" with action "%2"',
                                                    $va_rule['trigger'],
                                                    $vs_action_code,
                                                    $ps_refinery_name
                                                )
                    ...
                    $o_log->logError(
                        _t(
                            '[%3] Error processing rule "%1" as an error occurred. Error number was "%2"',
                            $va_rule['trigger'],
                            $o_error->getMessage(),
                            $ps_refinery_name
                        )
        ...
        if (!($va_match_on = caGetOption("{$ps_refinery_name}_matchOn", $pa_item['settings'], false))) {
            $va_match_on = caGetOption(
                "{$ps_refinery_name}_dontMatchOnLabel",
                $pa_item['settings'],
                false
        ...
        $pa_options = array_merge(
            array('matchOn' => $va_match_on, 'ignoreParent' => $vb_ignore_parent, 'ignoreType' => $vb_ignore_type),
            $pa_options
        );
        ...
        switch ($ps_table) {
            case 'ca_objects':
                $vn_id = DataMigrationUtils::getObjectID(
                    $vs_name,
                    $vn_id,
                    $vs_type,
                    $g_ui_locale_id,
            ...
            case 'ca_entities':
                $vn_id = DataMigrationUtils::getEntityID(
                    $va_entity_label = DataMigrationUtils::splitEntityName(
                        $vs_name,
                        array_merge(
                            $pa_options,
            ...
            case 'ca_places':
                ...
                $vn_id = DataMigrationUtils::getPlaceID(
                    $vs_name,
                    $vn_id,
                    $vs_type,
                    $g_ui_locale_id,
                ...
                break;
            case 'ca_occurrences':
                $vn_id = DataMigrationUtils::getOccurrenceID(
                    $vs_name,
                    $vn_id,
                    $vs_type,
                    $g_ui_locale_id,
                ...
                break;
            case 'ca_collections':
                $vn_id = DataMigrationUtils::getCollectionID(
                    $vs_name,
                    $vs_type,
                    $g_ui_locale_id,
                    $va_attributes,
                ...
                break;
            case 'ca_loans':
                $vn_id = DataMigrationUtils::getLoanID(
                    $vs_name,
                    $vs_type,
                    $g_ui_locale_id,
                    $va_attributes,
                ...
                break;
            case 'ca_movements':
                $vn_id = DataMigrationUtils::getMovementID(
                    $vs_name,
                    $vs_type,
                    $g_ui_locale_id,
                    $va_attributes,
            ...
            case 'ca_list_items':
                ...
                $vn_id = DataMigrationUtils::getListItemID(
                    $vn_list_id,
                    $vs_idno,
                    $vs_type,
                    $g_ui_locale_id,
            ...
            case 'ca_storage_locations':
                ...
                $vn_id = DataMigrationUtils::getStorageLocationID(
                    $vs_name,
                    $vn_id,
                    $vs_type,
                    $g_ui_locale_id,
        ...
        if ($o_log) {
            $o_log->logDebug(
                _t(
                    '[%6] Got parent %1 (%2) with id %3 and type %4 for %5',
                    $vs_name,
                    $vs_idno,
                    $vn_id,
                    $vs_type,
        ...
        $vn_i_flip = sizeof(
                $pa_parents
            ) - $vn_i - 1;        // parent list is reversed but parent settings are not... to calculate "inverse" index here
        ...
        caProcessRefineryRelatedMultiple(
            $o_refinery_instance,
            $va_item,
            $pa_source_data,
            null,
            ...
            if (is_array($va_val['_related_related'])) {
                foreach ($va_val['_related_related'] as $vs_table => $va_rels) {
                    foreach ($va_rels as $va_rel) {
                        if (!$t_subject->addRelationship(
                            $vs_table,
                            $va_rel['id'],
                            $va_rel['_relationship_type'],
                            null,
                            null,
                            null,
                            null,
                            ['setErrorOnDuplicate' => true]
                        )) {
                            if ($o_log) {
                                $o_log->logDebug(
                                    _t(
                                        '[%6] Could not create relationship between parent %1 and %2 for ids %3 and %4 with type %5: %7',
                                        $ps_table,
                                        $vs_table,
                                        $vn_id,
                                        $va_rel['id'],
...
function caProcessRefineryAttributes($pa_attributes, $pa_source_data, $pa_item, $pn_c, $pa_options = null)
{
    ...
    if (is_array($pa_attributes)) {
        ...
        foreach ($pa_attributes as $vs_element_code => $va_attrs) {
            ...
            if (in_array(
                    ca_metadata_elements::getElementDatatype($vs_element_code),
                    [__CA_ATTRIBUTE_VALUE_FILE__, __CA_ATTRIBUTE_VALUE_MEDIA__]
                ) && $vs_batch_media_directory && isset($pa_item['settings']["{$ps_refinery_name}_mediaPrefix"]) && $pa_item['settings']["{$ps_refinery_name}_mediaPrefix"]) {
            ...
            if (caIsIndexedArray($va_attrs)) {
                ...
                foreach ($va_attrs as $va_attrs_i) {
                    ...
                    foreach ($va_attrs_i as $vs_k => $vs_v) {
                        ...
                        $va_vals = BaseRefinery::parsePlaceholder(
                            $vs_v,
                            $pa_source_data,
                            $pa_item,
                            $pn_c,
                        ...
                        if ($vb_is_repeating) {
                            ...
                            foreach ($va_vals as $vn_x => $va_v) {
                                ...
                                if ($vs_prefix && is_array($va_v)) {
                                    ...
                                    foreach ($va_v as $vn_y => $vm_val_to_import) {
                                        ...
                                        if (!file_exists(
                                                $vs_path = $vs_prefix . $vm_val_to_import
                            ...
                            if ($vm_val_to_import = trim(
                                (is_array(
                                    $vm_v = BaseRefinery::parsePlaceholder(
                                        $vs_v,
                                        $pa_source_data,
                                        $pa_item,
                                        $pn_c,
                ...
                if ($vm_val_to_import = trim(
                    ...
                    if (!file_exists($vs_path = $vs_prefix . $vm_val_to_import) && ($va_candidates = array_filter(
                            $va_prefix_file_list,
                            function ($v) use ($vs_path) {
                                return preg_match("!^{$vs_path}!", $v);
                            }
...
function caProcessInterstitialAttributes(
    $ps_refinery_name,
    $pm_import_tablename_or_num,
    $pm_target_tablename_or_num,
    $pa_source_data,
    $pa_item,
    $pn_c,
    $pa_options = null
            ...
            if ($vs_linking_table) {
                foreach ($pa_item['settings']["{$ps_refinery_name}_interstitial"] as $vs_element_code => $va_attrs) {
                    if (!is_array($va_attrs)) {
                        $va_attr_vals['_interstitial'][$vs_element_code] = BaseRefinery::parsePlaceholder(
                            $va_attrs,
                            $pa_source_data,
                            $pa_item,
                            $pn_c,
                        ...
                        foreach ($va_attrs as $vs_k => $vs_v) {
                            $va_attr_vals['_interstitial'][$vs_element_code][$vs_k] = BaseRefinery::parsePlaceholder(
                                $vs_v,
                                $pa_source_data,
                                $pa_item,
                                $pn_c,
...
function caProcessRefineryRelated(
    ...
    foreach ($pa_related_option_list as $vn_i => $pa_related_options) {
        ...
        $vs_name_spec = caGetOption(
            caGetPreferredLabelNameKeyList(),
            $pa_related_options,
            caGetOption(
                caGetPreferredLabelNameKeyList(),
                $pa_related_options['attributes'],
                null
            )
        );
        $vs_idno_spec = caGetOption(
            caGetIdnoNameKeyList(),
            $pa_related_options,
            caGetOption(caGetIdnoNameKeyList(), $pa_related_options['attributes'], null)
        );
        $vs_type_spec = caGetOption(
            ['type', 'type_id'],
            $pa_related_options,
            caGetOption(['type', 'type_id'], $pa_related_options['attributes'], null)
        );
        $vs_parent_id_spec = caGetOption(
            'parent_id',
            $pa_related_options,
        ...
        if (!is_array(
            $va_name = BaseRefinery::parsePlaceholder(
                $vs_name_spec,
                $pa_source_data,
                $pa_item,
                $pn_c,
        ...
        if (!is_array(
            $va_idno = BaseRefinery::parsePlaceholder(
                $vs_idno_spec,
                $pa_source_data,
                $pa_item,
                $pn_c,
        ...
        if (!is_array(
            $va_type = BaseRefinery::parsePlaceholder(
                $vs_type_spec,
                $pa_source_data,
                $pa_item,
                $pn_c,
        ...
        if (!is_array(
            $va_parent_id = BaseRefinery::parsePlaceholder(
                $vs_parent_id_spec,
                $pa_source_data,
                $pa_item,
                $pn_c,
        ...
        foreach ($va_name as $i => $vs_name) {
            ...
            if ($ps_related_table == 'ca_entities') {
                ...
                if (!$vs_name) {
                    ...
                    foreach ($t_entity->getLabelUIFields() as $vs_label_fld) {
                        ...
                        $va_name[$vs_label_fld] = BaseRefinery::parsePlaceholder(
                            $pa_related_options[$vs_label_fld],
                            $pa_source_data,
                            $pa_item,
                            $pn_c,
                    ...
                    $va_name = DataMigrationUtils::splitEntityName(
                        $vs_name,
                        array_merge(
                            $pa_options,
                ...
                if (!is_array($va_name) || !$va_name) {
                    if ($o_log) {
                        $o_log->logDebug(
                            _t(
                                '[importHelpers:caProcessRefineryRelated] No name specified for table %1',
                                $ps_related_table
                            )
                        );
                    }
            ...
            if (!$vs_name) {
                if ($o_log) {
                    $o_log->logDebug(
                        _t('[importHelpers:caProcessRefineryRelated] No name specified for table %1', $ps_related_table)
                    );
                }
                return null;
            }

            $va_attributes = (isset($pa_related_options['attributes']) && is_array(
                    $pa_related_options['attributes']
                )) ? $pa_related_options['attributes'] : array();
            ...
            foreach ($va_attributes as $vs_element_code => $va_attrs) {
                if (is_array($va_attrs)) {
                    foreach ($va_attrs as $vs_k => $vs_v) {
                        ...
                        $va_attributes[$vs_element_code][$vs_k] = BaseRefinery::parsePlaceholder(
                            $vs_v,
                            $pa_source_data,
                            $pa_item,
                            $pn_c,
                    ...
                    $va_attributes[$vs_element_code] = array(
                        $vs_element_code => BaseRefinery::parsePlaceholder(
                            $va_attrs,
                            $pa_source_data,
                            $pa_item,
                            $pn_c,
            ...
            if ($ps_related_table != 'ca_object_lots') {
                ...
                $vs_idno_stub = BaseRefinery::parsePlaceholder(
                    $pa_related_options['idno_stub'],
                    $pa_source_data,
                    $pa_item,
                    $pn_c,
                    array(
            ...
            if (is_array($va_non_preferred_labels = $pa_related_options["nonPreferredLabels"])) {
                ...
                foreach ($va_non_preferred_labels as $va_label) {
                    foreach ($va_label as $vs_k => $vs_v) {
                        ...
                        $va_label[$vs_k] = BaseRefinery::parsePlaceholder(
                            $vs_v,
                            $pa_source_data,
                            $pa_item,
                            $i,
                ...
                if ($ps_refinery_name == 'entitySplitter') {
                    if (is_array(
                        $va_npl = DataMigrationUtils::splitEntityName(
                            BaseRefinery::parsePlaceholder(
                                $vs_non_preferred_label,
                                $pa_source_data,
                                $pa_item,
                                $pn_c,
                                array(
            ...
            $pa_options = array_merge(
                array('transaction' => $o_trans, 'matchOn' => array('idno', 'label')),
                $pa_options
            );
            ...
            switch ($ps_related_table) {
                case 'ca_objects':
                    ...
                    break;
                case 'ca_object_lots':
                    $vn_id = DataMigrationUtils::getObjectLotID(
                        $vs_idno_stub,
                        $vs_name,
                        $vs_type,
                        $g_ui_locale_id,
                ...
                default:
                    if ($o_log) {
                        $o_log->logDebug(
                            _t('[importHelpers:caProcessRefineryRelated] Invalid table %1', $ps_related_table)
                        );
                    }
                    return null;
                    break;
...
function caGenericImportSplitter(
    ...
    $va_group_dest = (isset($pa_item['destination']) && $pa_item['destination']) ? explode(
        ".",
        $pa_item['destination']
    ) : explode(".", $pa_group['destination']);

    $vs_terminal = array_pop($va_group_dest);
    $vs_dest_table = $va_group_dest[0];
    $va_group_dest[] = $vs_terminal;

    $pm_value = (!isset($pa_source_data[$pa_item['source']]) && $o_reader) ? caProcessImportItemSettingsForValue(
        $o_reader->get($pa_item['source'], array('returnAsArray' => true)),
        $pa_item['settings']
    ) : $pa_source_data[$pa_item['source']];
    ...
    } elseif (is_array($va_match_on = $pa_item['settings']["{$ps_refinery_name}_matchOn"]) || is_array(
            $va_match_on = $pa_item['settings']["matchOn"]
        )) {
    ...
    $pa_options['dontCreate'] = $pb_dont_create = caGetOption(
        'dontCreate',
        $pa_options,
    ...
    if (
        ...
        foreach ($va_delimited_items as $vn_x => $vs_delimited_item) {
            $va_items = sizeof($va_delimiter) ? preg_split(
                "!(" . join("|", $va_delimiter) . ")!",
                $vs_delimited_item
            ) : array($vs_delimited_item);
            ...
            foreach ($va_items as $vn_i => $vs_item) {
                ...
                if (!($vs_item = trim($vs_item))) {
                    if (is_array($va_parents) && (sizeof($va_parents) > 0)) {
                        ...
                        while (sizeof($va_parents) > 0) {
                            ...
                            if ($vs_laddered_val = BaseRefinery::parsePlaceholder(
                                $va_p[$vs_display_field],
                                $pa_source_data,
                                $pa_item,
                                $pn_value_index,
                                array(
                                ...
                                if ($o_log) {
                                    $o_log->logDebug(
                                        _t(
                                            "[{$ps_refinery_name}] Used parent value %1 because the mapped value was blank",
                                            $vs_item
                                        )
                                    );
                                }
                                $va_val['_type'] = BaseRefinery::parsePlaceholder(
                                    $va_p['type'],
                                    $pa_source_data,
                                    $pa_item,
                                    $pn_value_index,
                                ...
                                if ($vs_idno_fld = $t_instance->getProperty('ID_NUMBERING_ID_FIELD')) {
                                    $va_val[$vs_idno_fld] = BaseRefinery::parsePlaceholder(
                                        $va_p[$vs_idno_fld],
                                        $pa_source_data,
                                        $pa_item,
                                        $pn_value_index,
                ...
                if (is_array($va_skip_values = $pa_item['settings']["{$ps_refinery_name}_skipIfValue"]) && in_array(
                        $vs_item,
                        $va_skip_values
                    )) {
                    if ($o_log) {
                        $o_log->logDebug(
                            _t('[%1] Skipped %2 because it was in the skipIfValue list', $ps_refinery_name, $vs_item)
                        );
                    }
                    continue;
                }
                ...
                if ($va_hierarchy_setting = $pa_item['settings']["{$ps_refinery_name}_hierarchy"]) {
                    $va_val = array_merge(
                        $va_val,
                        caProcessRefineryParents(
                            $ps_refinery_name,
                            $ps_table,
                            $va_hierarchy_setting,
                            $pa_source_data,
                            $pa_item,
                            $pn_value_index,
                            array_merge(
                    ...
                    if ((!isset($va_val['_type']) || !$va_val['_type']) && ($vs_type_opt = $pa_item['settings']["{$ps_refinery_name}_{$ps_item_prefix}TypeDefault"])) {
                        if (!($va_val['_type'] = BaseRefinery::parsePlaceholder(
                            $vs_type_opt,
                            $pa_source_data,
                            $pa_item,
                            $pn_value_index,
                    ...
                    ($vs_type_opt = $pa_item['settings']["{$ps_refinery_name}_{$ps_item_prefix}Status"])
                        ...
                        $va_val['_status'] = BaseRefinery::parsePlaceholder(
                            $vs_type_opt,
                            $pa_source_data,
                            $pa_item,
                            $pn_value_index,
                    ...
                    if ((!isset($va_val['_type']) || !$va_val['_type']) && $o_log) {
                        $o_log->logWarn(
                            _t("[{$ps_refinery_name}] No %2 type is set for %2 \"%1\"", $vs_item, $ps_item_prefix)
                        );
                    }
                    ...
                    if (
                        ...
                        if (sizeof($va_item_hier) > 1) {
                            ...
                            if (!is_array(
                                $va_types = $pa_item['settings']["{$ps_refinery_name}_hierarchical" . ucfirst(
                                    $key_prefix
                                ) . "Types"]
                            )) {
                                $va_types = array();
                            ...
                            foreach ($va_item_hier as $vn_ix => $vs_parent) {
                                ...
                                switch ($ps_refinery_name) {
                                    case 'storageLocationSplitter':
                                        $vn_hier_item_id = DataMigrationUtils::getStorageLocationID(
                                            $vs_parent,
                                            $vn_hier_item_id,
                                            $vs_type,
                                            $g_ui_locale_id,
                                    ...
                                    case 'listItemSplitter':
                                        ...
                                        $vn_hier_item_id = DataMigrationUtils::getListItemID(
                                            $pa_item['settings']['listItemSplitter_list'],
                                            $vs_parent,
                                            $vs_type,
                                            $g_ui_locale_id,
                                            $vals,
                        ...
                        // Set parents
                        if ($va_parents) {
                            $va_val['parent_id'] = $va_val['_parent_id'] = caProcessRefineryParents(
                                $ps_refinery_name,
                                $ps_table,
                                $va_parents,
                                $pa_source_data,
                        ...
                        $pa_source_data['PARENT_IDNO'] = $va_val['parent_id'] ? $ps_table::getIdnoForId(
                            $va_val['parent_id'],
                            ['transaction' => $o_trans]
                        ) : 'o_trans';
                    }

                    if (isset($pa_options['hierarchyID']) && $pa_options['hierarchyID'] && ($vs_hier_id_fld = $t_instance->getProperty(
                            'HIERARCHY_ID_FLD'
                        ))) {
                    ...
                    if (is_array(
                        $va_attr_vals = caProcessRefineryAttributes(
                            $pa_item['settings']["{$ps_refinery_name}_attributes"],
                            $pa_source_data,
                            $pa_item,
                            $pn_value_index,
                            array(
                    ...
                    if (isset($pa_options['mapping']) && is_array(
                            $va_interstitial_attr_vals = caProcessInterstitialAttributes(
                                $ps_refinery_name,
                                $pa_options['mapping']->get('table_num'),
                                $ps_table,
                                $pa_source_data,
                                $pa_item,
                                $pn_value_index,
                                array('log' => $o_log, 'reader' => $o_reader)
                            )
                        )) {
                    ...
                    } elseif ($vs_non_preferred_label = trim(
                        $pa_item['settings']["{$ps_refinery_name}_nonPreferredLabels"]
                    )) {
                        if ($ps_refinery_name == 'entitySplitter') {
                            $pa_options['nonPreferredLabels'][] = DataMigrationUtils::splitEntityName(
                                BaseRefinery::parsePlaceholder(
                                    $vs_non_preferred_label,
                                    $pa_source_data,
                                    $pa_item,
                            ...
                            $pa_options['nonPreferredLabels'][] = [
                                $vs_label_fld => BaseRefinery::parsePlaceholder(
                                    $vs_non_preferred_label,
                                    $pa_source_data,
                                    $pa_item,
                                    $pn_value_index,
                ...
                if (($o_reader instanceof CollectiveAccessDataReader) && ($vs_table_idno_fld = Datamodel::getTableProperty(
                        $ps_table,
                        'ID_NUMBERING_ID_FIELD'
                    )) && in_array($vs_table_idno_fld, $pa_options['matchOn']) && !isset($va_val[$vs_table_idno_fld])) {
                ...
                (($vs_dest_table != $ps_table) && (sizeof($va_group_dest) > 1))
                    ...
                    $vs_item = BaseRefinery::parsePlaceholder(
                        $vs_item,
                        $pa_source_data,
                        $pa_item,
                        $pn_value_index,
                    ...
                    $va_attr_vals_with_parent = array_merge(
                        $va_val,
                    ...
                    switch ($ps_table) {
                        case 'ca_objects':
                            $vn_item_id = DataMigrationUtils::getObjectID(
                                $va_val['preferred_labels'] ? $va_val['preferred_labels'] : $vs_item,
                                $va_val['parent_id'],
                                $va_val['_type'],
                                $g_ui_locale_id,
                                $va_attr_vals_with_parent,
                                $pa_options
                            );
                            break;
                        case 'ca_object_lots':
                            ...
                            $vn_item_id = DataMigrationUtils::getObjectLotID(
                                $va_val['idno_stub'] ? $va_val['idno_stub'] : $vs_item,
                                $va_val['preferred_labels'] ? $va_val['preferred_labels'] : $vs_item,
                                $va_val['_type'],
                                $g_ui_locale_id,
                                $va_attr_vals,
                        ...
                        case 'ca_entities':
                            ...
                            $va_val['preferred_labels'] = is_array($n) ? $n : DataMigrationUtils::splitEntityName(
                                $n,
                                array_merge(
                                    $pa_options,
                                    ['doNotParse' => $pa_item['settings']["{$ps_refinery_name}_doNotParse"]]
                            ...
                            $vn_item_id = DataMigrationUtils::getEntityID(
                                $va_val['preferred_labels'],
                                $va_val['_type'],
                                $g_ui_locale_id,
                                $va_attr_vals_with_parent,
                                $pa_options
                            ...
                            break;
                        case 'ca_places':
                            $vn_item_id = DataMigrationUtils::getPlaceID(
                                $va_val['preferred_labels'] ? $va_val['preferred_labels'] : $vs_item,
                                $va_val['parent_id'],
                                $va_val['_type'],
                                $g_ui_locale_id,
                            ...
                            break;
                        case 'ca_occurrences':
                            $vn_item_id = DataMigrationUtils::getOccurrenceID(
                                $va_val['preferred_labels'] ? $va_val['preferred_labels'] : $vs_item,
                                $va_val['parent_id'],
                                $va_val['_type'],
                                $g_ui_locale_id,
                            ...
                            break;
                        case 'ca_collections':
                            $vn_item_id = DataMigrationUtils::getCollectionID(
                                $va_val['preferred_labels'] ? $va_val['preferred_labels'] : $vs_item,
                                $va_val['_type'],
                                $g_ui_locale_id,
                                $va_attr_vals_with_parent,
                            ...
                            break;
                        case 'ca_loans':
                            $vn_item_id = DataMigrationUtils::getLoanID(
                                $va_val['preferred_labels'] ? $va_val['preferred_labels'] : $vs_item,
                                $va_val['_type'],
                                $g_ui_locale_id,
                                $va_attr_vals_with_parent,
                            ...
                            break;
                        case 'ca_movements':
                            $vn_item_id = DataMigrationUtils::getMovementID(
                                $va_val['preferred_labels'] ? $va_val['preferred_labels'] : $vs_item,
                                $va_val['_type'],
                                $g_ui_locale_id,
                                $va_attr_vals_with_parent,
                        ...
                        case 'ca_list_items':
                            if (!$pa_options['list_id']) {
                                if ($o_log) {
                                    $o_log->logDebug(
                                        _t('[importHelpers:caGenericImportSplitter] List was not specified')
                                    );
                                }
                                continue(2);
                            }
                            ...
                            $vn_item_id = DataMigrationUtils::getListItemID(
                                $pa_options['list_id'],
                                $vs_idno,
                                $va_val['_type'],
                                $g_ui_locale_id,
                            ...
                            break;
                        case 'ca_storage_locations':
                            $vn_item_id = DataMigrationUtils::getStorageLocationID(
                                $va_val['preferred_labels'] ? $va_val['preferred_labels'] : $vs_item,
                                $va_val['parent_id'],
                                $va_val['_type'],
                                $g_ui_locale_id,
                        ...
                        case 'ca_object_representations':
                            if ($o_log) {
                                $o_log->logDebug(
                                    _t(
                                        '[importHelpers:caGenericImportSplitter] Only media paths can be mapped for object representations.'
                                    )
                                );
                            }
                            continue(2);
                        default:
                            if ($o_log) {
                                $o_log->logDebug(
                                    _t('[importHelpers:caGenericImportSplitter] Invalid table %1', $ps_table)
                                );
                            }
                            continue(2);
                            break;
                    ...
                    if ($vn_item_id) {
                        ...
                        if ($o_log && !$pb_dont_create) {
                            $o_log->logError(
                                _t("[{$ps_refinery_name}Refinery] Could not add %2 %1", $vs_item, $ps_item_prefix)
                            );
                        }
                    }
                    ...
                    if (
                        ...
                        $va_val['_relationship_type'] = BaseRefinery::parsePlaceholder(
                            $vs_rel_type_opt,
                            $pa_source_data,
                            $pa_item,
                            $pn_value_index,
                    ...
                    if (
                        ...
                        if (!($va_val['_relationship_type'] = BaseRefinery::parsePlaceholder(
                            $vs_rel_type_opt,
                            $pa_source_data,
                            $pa_item,
                            $pn_value_index,
                    ...
                    if ((!isset($va_val['_relationship_type']) || !$va_val['_relationship_type']) && $o_log && ($ps_refinery_name !== 'objectRepresentationSplitter')) {
                        $o_log->logWarn(
                            _t(
                                "[{$ps_refinery_name}Refinery] No relationship type is set for %2 \"%1\"",
                                $vs_item,
                                $ps_item_prefix
                            )
                        );
                    ...
                    switch ($ps_table) {
                        case 'ca_entities':
                            $va_val['preferred_labels'] = DataMigrationUtils::splitEntityName(
                                $vs_item,
                                array_merge(
                                    $pa_options,
                                    ['doNotParse' => $pa_item['settings']["{$ps_refinery_name}_doNotParse"]]
                        ...
                        case 'ca_object_lots':
                            ...
                            break;
                        case 'ca_object_representations':
                            if (!($vs_batch_media_directory = $t_instance->getAppConfig()->get(
                                'batch_media_import_root_directory'
                            ))) {
                                break;
                            }

                            if (isset($va_val['name']) && is_array(
                                    $va_val['name']
                                ) && isset($va_val['name']['name']) && $va_val['name']['name']) {
                            ...
                            if (!isset($va_val['preferred_labels']) || !strlen($va_val['preferred_labels'])) {
                                $va_val['preferred_labels'] = $vs_name ? $vs_name : '[' . caGetBlankLabelText(
                            ...
                            if ($va_val['media']['media'] || $vs_item) {
                                ...
                                $va_files = caBatchFindMatchingMedia(
                                    $vs_batch_media_directory . $vs_media_dir_prefix,
                                    $vs_item,
                                    [
                                ...
                                foreach ($va_files as $vs_file) {
                                    ...
                                    if (isset($pa_options['nonPreferredLabels']) && is_array(
                                            $pa_options['nonPreferredLabels']
                                        )) {
                    ...
                    switch ($ps_table) {
                        case 'ca_entities':
                            $va_val = DataMigrationUtils::splitEntityName(
                                $vs_item,
                                array_merge(
                                    $pa_options,
                                    ['doNotParse' => $pa_item['settings']["{$ps_refinery_name}_doNotParse"]]
                        ...
                        case 'ca_object_representations':
                            if ($o_log) {
                                $o_log->logDebug(
                                    _t(
                                        '[importHelpers:caGenericImportSplitter] Cannot map preferred labels to object representations. Only media paths can be mapped.'
                                    )
                                );
                            }
                            continue(2);
                    ...
                    if ($o_log) {
                        $o_log->logError(
                            _t(
                                "[{$ps_refinery_name}Refinery] Could not add %2 %1: cannot map %3 using %1",
                                $vs_item,
                                $ps_item_prefix,
                                join(".", $va_group_dest)
                            )
        ...
        if ($o_log) {
            $o_log->logError(
                _t("[{$ps_refinery_name}Refinery] Cannot map %1 using this refinery", $pa_group['destination'])
            );
        }
        return array();
    }
...
function caProcessRefineryRelatedMultiple(
    ...
    if (is_array(
        ...
        $pa_item['settings'][] = is_array(
            $pa_item['settings'][$vs_relationship_settings_key]
    ...
    if (is_array($va_relationships = $pa_item['settings'][$vs_relationship_settings_key])) {
        foreach ($va_relationships as $va_relationship_settings) {
            if ($vs_table_name = caGetOption('relatedTable', $va_relationship_settings)) {
                if (is_array(
                    $va_rels = caProcessRefineryRelated(
                        $po_refinery_instance,
                        $vs_table_name,
                        array($va_relationship_settings),
                        $pa_source_data,
                        $pa_item,
                        $pn_value_index,
                        array_merge(
                    ...
                    if (is_array($va_rel_rels)) {
                        ...
                        foreach ($va_rel_rels as $vs_rel_table => $va_rel_info) {
                            ...
                            $va_val['_related_related'][$vs_rel_table] = array_merge(
                                $va_val['_related_related'][$vs_rel_table],
                                $va_rel_info
                            );
                        }
...
function caPhpExcelLoadFile($ps_xlsx)
{
    if (MemoryCache::contains($ps_xlsx, 'CAPHPExcel')) {
        ...
        /**  Identify the type  **/
        $vs_input_filetype = IOFactory::identify($ps_xlsx);
        /**  Create a new Reader of that very type  **/
        $o_reader = IOFactory::createReader($vs_input_filetype);
...
function caPhpExcelGetCellContentAsString($po_sheet, $pn_row_num, $pm_col)
{
    if (!is_numeric($pm_col)) {
        $pm_col = Coordinate::columnIndexFromString($pm_col);
    }
 ...
 * @param Worksheet $po_sheet The work sheet
 * @param int $pn_row_num row number (zero indexed)
 * @param string|int $pm_col either column number (zero indexed) or column letter ('A', 'BC')
...
function caPhpExcelGetDateCellContent($po_sheet, $pn_row_num, $pm_col, $pn_offset = 1)
{
    ...
    if (strlen($vs_val) > 0) {
        $vn_timestamp = Date::excelToTimestamp(
                trim((string)$o_val->getValue())
            ) + $pn_offset;
 ...
 * @param int $pn_row_num row number (zero indexed)
 * @param string|int $pm_col either column number (zero indexed) or column letter ('A', 'BC')
 * @return Cell|null the cell, if a value exists
 */
...
function caMatchAAT($pa_hierarchy_path, $pn_threshold = 180, $pa_options = array(), $o_service = null)
{
    ...
    foreach ($va_hits as $vn_i => $va_hit) {
        if (stripos(
                $va_hit['TermPrefLabel']['value'],
                $vs_lookup
            ) !== false) { // only consider terms that match what we searched
            ...
            similar_text(
                $va_hit['ParentsFull']['value'],
                join(' ', array_reverse($pa_hierarchy_path)),
                $vn_parent_percent
            );
