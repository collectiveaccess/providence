 ...
 *                    successMessage = Message to use for logging on successful send of email. Use %1 as a placeholder
 *                    for a list of recipient email addresses. [Default is 'Email was sent to %1'] failureMessage =
 *                    Message to use for logging on failure of send. Use %1 as a placeholder for a list of recipient
 *                    email addresses; %2 for the error message. [Default is 'Could not send email to %1: %2']
 *
 ...
 */
function caSendmail(
	$pa_to, $pa_from, $ps_subject, $ps_body_text, $ps_body_html = '', $pa_cc = null, $pa_bcc = null,
	$pa_attachments = null, $pa_options = null
) {
	...
	try {
		...
		if ( caGetOption( 'log', $pa_options, true ) && caGetOption( 'logSuccess', $pa_options, true ) ) {
			$o_log->log( array(
				'CODE'    => 'SYS',
				'SOURCE'  => caGetOption( 'source', $pa_options, 'Registration' ),
				'MESSAGE' => _t( caGetOption( 'successMessage', $pa_options, 'Email was sent to %1' ),
					join( ';', array_keys( $pa_to ) ) )
			) );
		}
		...
		if ( caGetOption( 'log', $pa_options, true ) && caGetOption( 'logDFailure', $pa_options, true ) ) {
			$o_log->log( array(
				'CODE'    => 'ERR',
				'SOURCE'  => caGetOption( 'source', $pa_options, 'Registration' ),
				'MESSAGE' => _t( caGetOption( 'failureMessage', $pa_options, 'Could not send email to %1: %2' ),
					join( ';', array_keys( $pa_to ) ), $e->getMessage() )
			) );
		}
 ...
 */
function caSendMessageUsingView(
	$po_request, $pa_to, $pa_from, $ps_subject, $ps_view, $pa_values, $pa_cc = null, $pa_bcc = null, $pa_options = null
) {
	$vs_view_path = ( is_object( $po_request ) ) ? $po_request->getViewsDirectoryPath()
		: __CA_BASE_DIR__ . '/themes/default/views';
	if ( ! is_object( $po_request ) ) {
		$po_request = null;
	}

	$o_view = new View( $po_request, $vs_view_path . "/mailTemplates", 'UTF8',
		array( 'includeDefaultThemePath' => false ) );
	...
	return caSendmail( $pa_to, $pa_from, $ps_subject, null, $o_view->render( $ps_view ), $pa_cc, $pa_bcc,
		caGetOption( [ 'attachment', 'attachments' ], $pa_options, null ), $pa_options );
}
# ------------------------------------------------------------------------------------------------
