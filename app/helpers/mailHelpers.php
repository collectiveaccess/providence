        ...
        if (caGetOption('log', $pa_options, true) && caGetOption('logSuccess', $pa_options, true)) {
            $o_log->log(
                array(
                    ...
                    'MESSAGE' => _t(
                        caGetOption('successMessage', $pa_options, 'Email was sent to %1'),
        ...
        if (caGetOption('log', $pa_options, true) && caGetOption('logDFailure', $pa_options, true)) {
            $o_log->log(
                array(
                    ...
                    'SOURCE' => caGetOption('source', $pa_options, 'Registration'),
                    'MESSAGE' => _t(
                        caGetOption('failureMessage', $pa_options, 'Could not send email to %1: %2'),
                        join(';', array_keys($pa_to)),
                        $e->getMessage()
                    )
                )
...
function caSendMessageUsingView(
    ...
    $o_view = new View(
        $po_request, $vs_view_path . "/mailTemplates", 'UTF8', array('includeDefaultThemePath' => false)
    );
    ...
    return caSendmail(
        ...
        $ps_subject,
        null,
        $o_view->render($ps_view),
        $pa_cc,
        $pa_bcc,
        caGetOption(['attachment', 'attachments'], $pa_options, null),
        $pa_options
    );
}
# ------------------------------------------------------------------------------------------------
