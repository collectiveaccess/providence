...
if ( ! defined( "__CA_BASE_DIR__" ) ) {
	define( "__CA_BASE_DIR__",
		( $_SERVER['SCRIPT_FILENAME'] && ( php_sapi_name() !== 'cli' ) ) ? preg_replace( "!/install$!", "",
...
if ( ! defined( "__CA_SITE_PROTOCOL__" ) ) {
	define( "__CA_SITE_PROTOCOL__", isset( $_SERVER['HTTPS'] )
		? 'https'
...
if ( ! ( file_exists( $_CA_CONFIG_PATH ) ) ) {
	$opa_error_messages = array(
		"Configuration files are missing for hostname '" . ( isset( $_SERVER['HTTP_HOST'] ) ? $_SERVER['HTTP_HOST']
			: '[unknown]' )
...
if ( ! defined( "__CA_THEME__" ) ) {
	define( "__CA_THEME__",
		$g_configuration_cache_suffix = caGetPreferredThemeForCurrentDevice( $_CA_THEMES_BY_DEVICE ) );
}
