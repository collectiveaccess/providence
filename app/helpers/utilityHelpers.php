...
function _t($ps_key)
{
    ...
    if (!isset($g_translation_cache[$ps_key])) {
        ...
        if (is_array($g_translation_replacements)) {
            $vs_str = str_replace(
                array_keys($g_translation_replacements),
                array_values($g_translation_replacements),
                $vs_str
            );
        }
...
function &caGetDirectoryContentsAsList(
    $dir,
    $pb_recursive = true,
    $pb_include_hidden_files = false,
    $pb_sort = false,
    $pb_include_directories = false,
    $pa_options = null
) {
    $va_file_list = array();
    if (substr($dir, -1, 1) == "/") {
        ...
        foreach ($va_paths as $item) {
            if ($item != "." && $item != ".." && ($pb_include_hidden_files || (!$pb_include_hidden_files && $item{0} !== '.'))) {
                ...
                if ($pb_recursive && $vb_is_dir) {
                    $va_file_list = array_merge(
                        $va_file_list,
                        array_flip(
                            caGetDirectoryContentsAsList(
                                "{$dir}/{$item}",
                                true,
                                $pb_include_hidden_files,
                                false,
                                $pb_include_directories
                            )
                        )
                    );
                } else {
    ...
    if ($handle = @opendir($dir)) {
        while (false !== ($item = readdir($handle))) {
            if ($item != "." && $item != ".." && ($pb_include_hidden_files || (!$pb_include_hidden_files && $item{0} !== '.'))) {
                ...
                if ($pb_recursive && $vb_is_dir) {
                    $va_recursive_counts = caGetDirectoryContentsCount(
                        "{$dir}/{$item}",
                        true,
                        $pb_include_hidden_files
                    );
...
function caIsValidFilePath($ps_path)
{
    ...
    if (!ini_get('open_basedir') && !@is_readable(
            $ps_path
...
function caEscapeSearchForURL($ps_search)
{
    ...
    ); // encode slashes as html entities to avoid Apache considering it a directory separator
}
...
function caEscapeForDelimitedRegexp($ps_text, $ps_delimiter = '!')
{
    ...
    return preg_replace(
        $vs_regex_delimiter . '(?<!\\\\)' . $ps_delimiter . $vs_regex_delimiter,
        '\\' . $ps_delimiter,
        $ps_text
    );
}
...
function caMakeDelimitedRegexp($ps_text, $ps_delimiter = '!')
{
    ...
    return $ps_delimiter . preg_replace(
            $vs_regex_delimiter . '(?<!\\\\)' . $ps_delimiter . $vs_regex_delimiter,
            '\\' . $ps_delimiter,
            $ps_text
        ) . $ps_delimiter;
...
function caCleanUserMediaDirectory(int $user_id)
{
    ...
    $files_to_delete = caGetDirectoryContentsAsList(
        $user_dir,
        true,
        false,
        false,
...
function caMakeGetFilePath($ps_prefix = null, $ps_extension = null, $options = null)
{
    ...
    do {
        $vs_file_path = $vs_path . DIRECTORY_SEPARATOR . $ps_prefix . mt_rand() . getmypid(
 ...
 *
 * @return array|bool Return array with protocol and url keys if valid URL, false if invalid.
 */
function isURL($ps_url, $pa_options = null)
{
    ...
    caGetOption('strict', $pa_options, false)
        ?
        preg_match(
            "!^(http|ftp|https|rtmp|rtsp|mysql):\/\/[\w\-_]+(\.[\w\-_]+)*([\w\-\.,@?^=%&;:/~\+#]*[\w\-\@?^=%&/~\+#])?$!",
            $ps_url,
            $va_matches
        )
        :
...
function caDecodeAllEntities($ps_string, $pn_quotes = ENT_COMPAT, $ps_charset = 'UTF-8')
{
    return html_entity_decode(
        preg_replace_callback('/&([a-zA-Z][a-zA-Z0-9]+);/', 'caConvertEntity', html_entity_decode($ps_string)),
        $pn_quotes,
        $ps_charset
    );
}
 ...
 *
 * @return bool True if request wasrun from command line, false if not
 */
 ...
 *
 * @return bool true if request addressed service.php, false if not
 */
...
function caGetOption($pm_option, $pa_options, $pm_default = null, $pa_parse_options = null)
{
    ...
    if (is_array($pm_option)) {
        ...
        $vm_val = (isset($pa_options[$pm_option]) && !is_null(
                $pa_options[$pm_option]
            )) ? $pa_options[$pm_option] : $pm_default;
    }
    if (isset($pa_parse_options['defaultOnEmptyString']) && $pa_parse_options['defaultOnEmptyString'] && is_string(
            $vm_val
        ) && (strlen($vm_val) === 0)) {
        $vm_val = $pm_default;
    }
    ...
    if (
        ((is_string(
                    $vm_val
                ) && !isset($pa_parse_options['castTo'])) || (isset($pa_parse_options['castTo']) && ($pa_parse_options['castTo'] == 'string')))
        &&
    ...
    $vs_cast_to = (isset($pa_parse_options['castTo']) && ($pa_parse_options['castTo'])) ? strtolower(
        $pa_parse_options['castTo']
    ) : '';
    switch ($vs_cast_to) {
        case 'int':
        case 'integer':
...
function caSanitizeArray($pa_array, $pa_options = null)
{
    ...
    foreach ($pa_array as $vn_k => $vm_v) {
        if (is_array($vm_v)) {
            ...
            if (((!empty($vm_v) && strlen($vm_v) < 8192) && (!preg_match("!^\X+$!", $vm_v))) || (!mb_detect_encoding(
                    $vm_v
                ))) {
                unset($pa_array[$vn_k]);
                continue;
 ...
 * @return bool True if string is a roman number, false otherwise
 */
 ...
 * @param bool $print_r Flag to switch between print_r() and var_export() for data conversion to string.
    ...
    if (defined('__CA_ENABLE_DEBUG_OUTPUT__') && __CA_ENABLE_DEBUG_OUTPUT__) {
        if (caIsRunFromCLI()) {
            ...
            if (caIsServiceRequest()) {
                ...
                $vs_string = htmlspecialchars(
                    ($print_r ? print_r($vm_data, true) : var_export($vm_data, true)),
                    ENT_QUOTES,
                    'UTF-8'
                );
...
{
    if ($t_instance = Datamodel::getInstanceByTableName(
        'ca_objects',
        true
...
function caUploadFileToGitHub(
    ...
    if (!$ps_user || !$ps_token || !$ps_owner || !$ps_repo || !$ps_git_path || !$ps_local_filepath) {
        caLogEvent(
            'DEBG',
            "Invalid parameters for GitHub file upload. Check your configuration!",
    ...
    try {
        $o_client->repositories()->contents()->create(
            $ps_owner,
            $ps_repo,
            $ps_git_path,
            $vs_content,
        ...
        switch ($e->getCode()) {
            case 401:
                caLogEvent(
                    'DEBG',
                    "Could not authenticate with GitHub. Error message was: " . $e->getMessage(
                    ) . " - Code was: " . $e->getCode(),
                    'caUploadFileToGitHub'
                );
                break;
                ...
                if ($pb_update_on_conflict) {
                    try {
                        ...
                        if (isset($va_content['sha'])) {
                            $o_client->repositories()->contents()->update(
                                $ps_owner,
                                $ps_repo,
                                $ps_git_path,
                                $vs_content,
...
function caExportDataToResourceSpace($ps_user, $ps_key, $ps_base_url, $ps_local_filepath)
{
    ...
    if (!$ps_user || !$ps_key || !$ps_base_url || !$ps_local_filepath) {
        caLogEvent(
            'DEBG',
            "Invalid parameters for ResourceSpace export. Check your configuration!",
    ...
    $o_client = new Client(['base_uri' => $ps_base_url]);
    foreach ($va_records as $va_record) {
        ...
        try {
            $vs_query = 'user=' . $ps_user . '&function=create_resource&param1=' . $vs_resource_type . '&param2=0&param3=' . rawurlencode(
                    $vs_media_url
                ) . '&param4=&param5=&param6=&param7=' . rawurlencode(json_encode($va_record));
            ...
            if (!$vn_rs_id) {
                caLogEvent(
                    'ERR',
                    "Could not create Resource. Check your ResourceSpace configuration",
                    'caExportDataToResourceSpace'
                );
                continue;
            }
            if ($vs_collection_name) {
                ...
                if (!$vs_collection_id) {
                    $vs_query = 'user=' . $ps_user . '&function=create_collection&param1=' . rawurlencode(
                            $vs_collection_name
                        );
                    $vs_hash = hash('sha256', $ps_key . $vs_query);
                    ...
                    if ($vb_success) {
                        ...
                        caLogEvent(
                            'DEBG',
                            "Could not create Collection. Check your ResourceSpace configuration",
                            'caExportDataToResourceSpace'
                        );
                    }
                }
            ...
            caLogEvent(
                'ERR',
                "Could not export data to ResourceSpace with error: " . $e->getMessage(
                ) . " - Code was: " . $e->getCode(),
                'caExportDataToResourceSpace'
            );
            return false;
...
function caQueryExternalWebservice($ps_url)
{
    ...
    if (curl_getinfo($vo_curl, CURLINFO_HTTP_CODE) !== 200) {
        throw new WebServiceError(
            _t('An error occurred while querying an external webservice') . _t(" at %1", $ps_url) . " " . print_r(
                curl_getinfo($vo_curl),
                true
            )
        );
    }
...
function caParseLengthExpression($ps_expression, $pa_options = null)
{
    ...
    foreach ($va_measurements as $vn_i => $vs_measurement) {
        ...
        try {
            if (!($vo_parsed_measurement = caParseLengthDimension($vs_measurement))) {
                ...
                $va_unit_map[] = $vs_extracted_units = caGetLengthUnitType(
                    $vo_parsed_measurement->getType(),
                    ['short' => true]
                );
                if (!strlen($vn_parse_precision = $pn_precision) && !strlen(
                        $vn_parse_precision = $o_dimensions_config->get(
                            caGetLengthUnitType($vs_extracted_units, ['code' => true]) . "_decimal_precision"
                        )
                    )) {
    ...
    foreach ($va_extracted_measurements as $vn_i => $va_measurement) {
        if (!$va_unit_map[$vn_i]) {
            ...
            try {
                if ($vo_parsed_measurement = caParseLengthDimension(
                    $va_measurement['string'] . " {$vs_inferred_units}"
                )) {
                    if (!strlen($vn_parse_precision = $pn_precision) && !strlen(
                            $vn_parse_precision = $o_dimensions_config->get(
                                caGetLengthUnitType($vs_inferred_units, ['code' => true]) . "_decimal_precision"
                            )
                        )) {
                        $vn_parse_precision = 4;
                    }
...
function caGenerateCSRFToken($po_request = null)
{
    ...
    if (function_exists("random_bytes")) {           // PHP 7
        ...
        $vs_token = md5(
            uniqid(rand(), true)
        );   // this is not very good, and is only used if one of the more secure options above is available (one of them should be in almost all cases)
    }
    if ($po_request) {
...
function caLengthToFractions($pn_inches_as_float, $pn_denom, $pb_reduce = true, $pa_options = null)
{
    ...
    $pb_use_unicode_fraction_glyphs = (is_array($pa_use_unicode_fraction_glyphs_for) && sizeof(
            $pa_use_unicode_fraction_glyphs_for
        ));
    if (is_null($pa_use_unicode_fraction_glyphs_for)) {
    ...
    $pn_inches_as_float = (float)preg_replace(
        "![^\d\.]+!",
        "",
        $pn_inches_as_float
    );    // remove commas and such; also remove "-" as dimensions can't be negative
    ...
    if ($pb_use_unicode_fraction_glyphs) {
        ...
        if ($unicode_frac && is_array($pa_use_unicode_fraction_glyphs_for) && sizeof(
                $pa_use_unicode_fraction_glyphs_for
...
function caSortableValue($ps_text, $pa_options = null)
{
    ...
    foreach ($va_articles as $vs_article) {
        if (preg_match('!^(' . $vs_article . ')[ ]+!i', $vs_display_value, $va_matches)) {
            $vs_display_value = trim(
                str_replace($va_matches[1], '', $vs_display_value) . ($pb_omit_article ? '' : ', ' . $va_matches[1])
            );
            break;
        }
    }
...
function caNormalizeValueArray($pa_values, $pa_options = null)
{
    ...
    foreach ($pa_values as $vs_key => $vm_val) {
        if (is_array($vm_val)) {
            if (isset($vm_val[0]) && !is_array($vm_val[0]) && caIsValidSqlOperator(
                    $vm_val[0],
                    [
                        'nullable' => true,
                        'isList' => true
            ...
            foreach ($vm_val as $vs_key2 => $vm_list_vals) {
                if (is_array($vm_list_vals) && !is_array($vm_list_vals[0]) && caIsValidSqlOperator(
                        $vm_list_vals[0],
                        [
                            'nullable' => true,
                            'isList' => true
                            ...
                            if (!is_numeric($vs_key2)) {
                                $va_values_proc[$vs_key][$vs_key2][] = [
                                    ...
                                    $o_purifier && !is_null($vm_list_val[1]) ? str_replace(
                                        "&amp;",
                                        "&",
                                        $o_purifier->purify($vm_list_val[1])
                                    ) : $vm_list_val[1]
                                ];
                            } else {
            ...
            $va_values_proc[$vs_key][] = [
                is_null($vm_val) ? 'IS' : '=',
                $o_purifier && !is_null($vm_val) ? str_replace("&amp;", "&", $o_purifier->purify($vm_val)) : $vm_val
            ];
        }
    }
...
function caExtractTagsFromTemplate($ps_template, $pa_options = null)
{
    ...
    for ($i = 0; $i < mb_strlen($ps_template); $i++) {
        switch ($vs_char = mb_substr($ps_template, $i, 1)) {
            ...
            case ',':
            case '<':
                if (!$vb_in_single_quote && !$vb_in_double_quote && (!$vb_have_seen_param_delimiter || (!in_array(
                            $vs_char,
                            [',']
                        )))) {
            ...
            default:
                if ($vb_in_tag) {
                    ...
                    if (
                        ($vb_is_ca_get_ref && !$vb_have_seen_param_delimiter && (!preg_match(
                                "![A-Za-z0-9_\-\.~:]!",
                                $vs_char
                            )))
                        ||
                        (($vs_char === ':') && !preg_match(
                                "!^[A-Za-z0-9]+!",
                                mb_substr($ps_template, $i + 1)
                            ))    // colon not followed by letters of numbers is not part of tag
                    ) {
                        if ($vs_tag = trim($vs_tag)) {
                            $va_tags[] = $vs_tag;
                        }
