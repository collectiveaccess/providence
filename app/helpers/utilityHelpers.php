...
function _t( $ps_key ) {
	...
	if ( ! isset( $g_translation_cache[ $ps_key ] ) ) {
		...
		if ( is_array( $g_translation_replacements ) ) {
			$vs_str = str_replace( array_keys( $g_translation_replacements ),
				array_values( $g_translation_replacements ), $vs_str );
		}
...
function caMakeProperUTF8ForXML( $ps_text ) {
	...
	for ( $i = 0; $i < $vn_length; $i ++ ) {
		...
		if ( ( $vn_current == 0x9 ) || ( $vn_current == 0xA ) || ( $vn_current == 0xD )
		     || ( ( $vn_current >= 0x20 )
		          && ( $vn_current <= 0xD7FF ) )
...
# ----------------------------------------
/**
 * Removes directory $dir and recursively all content within. This means all files and subdirectories within the
 * specified directory will be removed without any question!
 *
 * @param string $dir           The path to the directory you wish to remove
 * @param bool   $pb_delete_dir By default caRemoveDirectory() will remove the specified directory after delete
 *                              everything within it. Setting this to false will retain the directory after removing
 *                              everything inside of it, effectively "cleaning" the directory.
 *
...
# ----------------------------------------
/**
 * Returns a list of files for the directory $dir and all sub-directories. Optionally can be restricted to return only
 * files that are in $dir (no sub-directories).
 *
 * @param string $dir                     The path to the directory you wish to get the contents list for
 * @param bool   $pb_recursive            Optional. By default caGetDirectoryContentsAsList() will recurse through all
 *                                        sub-directories of $dir; set this to false to only consider files that are in
 *                                        $dir itself.
 * @param bool   $pb_include_hidden_files Optional. By default caGetDirectoryContentsAsList() does not consider hidden
 *                                        files (files starting with a '.') when calculating file counts. Set this to
 *                                        true to include hidden files in counts. Note that the special UNIX '.' and
 *                                        '..' directory entries are *never* counted as files.
 * @param bool   $pb_sort                 Optional. If set paths are returns sorted alphabetically. Default is false.
 * @param bool   $pb_include_directories  . If set paths to directories are included. Default is false (only files are
 *                                        returned).
 * @param array  $pa_options              Additional options, including:
 *                                        modifiedSince = Only return files and directories modified after a Unix
 *                                        timestamp [Default=null] notModifiedSince = Only return files and directories
 *                                        not modified after a Unix timestamp [Default=null]
 *
 * @return array An array of file paths.
 */
function &caGetDirectoryContentsAsList(
	$dir, $pb_recursive = true, $pb_include_hidden_files = false, $pb_sort = false, $pb_include_directories = false,
	$pa_options = null
) {
	$va_file_list = array();
	if ( substr( $dir, - 1, 1 ) == "/" ) {
		...
		foreach ( $va_paths as $item ) {
			if ( $item != "." && $item != ".."
				...
				if ( $pb_recursive && $vb_is_dir ) {
					$va_file_list = array_merge( $va_file_list,
						array_flip( caGetDirectoryContentsAsList( "{$dir}/{$item}", true, $pb_include_hidden_files,
							false, $pb_include_directories ) ) );
				} else {
					if ( ! $vb_is_dir ) {
						$va_file_list["{$dir}/{$item}"] = true;
					}
	...
	if ( $handle = @opendir( $dir ) ) {
		while ( false !== ( $item = readdir( $handle ) ) ) {
			if ( $item != "." && $item != ".."
				...
				if ( $pb_recursive && $vb_is_dir ) {
					$va_recursive_counts      = caGetDirectoryContentsCount( "{$dir}/{$item}", true,
						$pb_include_hidden_files );
...
# ----------------------------------------
/**
 * Returns a list of directories from all directories under $dir as an array of directory paths with associated file
 * counts.
 *
 * @param string $dir                     The path to the directory you wish to get the contents list for
 * @param bool   $pb_include_root         Optional. By default caGetSubDirectoryList() omits the root directory ($dir)
 *                                        and any files in it. Set this to true to include the root directory if it
 *                                        contains files.
 * @param bool   $pb_include_hidden_files Optional. By default caGetSubDirectoryList() does not consider hidden files
 *                                        (files starting with a '.') when calculating file counts. Set this to true to
 *                                        include hidden files in counts. Note that the special UNIX '.' and '..'
 *                                        directory entries are *never* counted as files.
 *
	...
	if ( $handle = @opendir( $dir ) ) {
		while ( false !== ( $item = readdir( $handle ) ) ) {
			if ( $item != "." && $item != ".."
				...
				if ( is_dir( "{$dir}/{$item}" ) ) {
					$va_dir_list = array_merge( $va_dir_list,
						caGetSubDirectoryList( "{$dir}/{$item}", true, $pb_include_hidden_files ) );
				} else {
					$vn_file_count ++;
				}
			}
...
function caIsValidFilePath( $ps_path ) {
	...
	if ( ! ini_get( 'open_basedir' )
	     && ! @is_readable( $ps_path )
	) { // open_basedir and is_readable() have some weird interactions
		return false;
	}
...
function caEscapeSearchForURL( $ps_search ) {
	return rawurlencode( str_replace( '/', '&#47;',
		$ps_search ) ); // encode slashes as html entities to avoid Apache considering it a directory separator
}
...
function caEscapeForDelimitedRegexp( $ps_text, $ps_delimiter = '!' ) {
	...
	return preg_replace( $vs_regex_delimiter . '(?<!\\\\)' . $ps_delimiter . $vs_regex_delimiter, '\\' . $ps_delimiter,
		$ps_text );
}
...
function caMakeDelimitedRegexp( $ps_text, $ps_delimiter = '!' ) {
	...
	return $ps_delimiter . preg_replace( $vs_regex_delimiter . '(?<!\\\\)' . $ps_delimiter . $vs_regex_delimiter,
			'\\' . $ps_delimiter, $ps_text ) . $ps_delimiter;
}
 ...
 *                       useAppTmpDir = Return application temporary directory rather than system tmp directory in all
...
function caCleanUserMediaDirectory( int $user_id ) {
	...
	$files_to_delete = caGetDirectoryContentsAsList( $user_dir, true, false, false, true,
		[ 'notModifiedSince' => time() - $timeout ] );
	$count           = 0;
...
function caMakeGetFilePath( $ps_prefix = null, $ps_extension = null, $options = null ) {
	...
	do {
		$vs_file_path = $vs_path . DIRECTORY_SEPARATOR . $ps_prefix . mt_rand() . getmypid() . ( $ps_extension
				? ".{$ps_extension}" : "" );
 ...
 *
 * If $pb_dont_include_base_usage is set to false then this function returns the same value as the PHP
 * memory_get_usage() built-in with the "real memory" parameter set.
 *
 * If the $pb_format_for_display is set (default = true), then the memory usage is returned as megabytes in a string
 * (ex. 9.75M) If it is not set then an integer representing the number of bytes used is returned (ex. 10223616)
 */
...
function isURL( $ps_url, $pa_options = null ) {
	...
	caGetOption( 'strict', $pa_options, false )
		?
		preg_match( "!^(http|ftp|https|rtmp|rtsp|mysql):\/\/[\w\-_]+(\.[\w\-_]+)*([\w\-\.,@?^=%&;:/~\+#]*[\w\-\@?^=%&/~\+#])?$!",
			$ps_url, $va_matches )
		:
		preg_match( "!(http|ftp|https|rtmp|rtsp|mysql):\/\/[\w\-_]+(\.[\w\-_]+)*([\w\-\.,@?^=%&;:/~\+#]*[\w\-\@?^=%&/~\+#])?!",
			$ps_url, $va_matches )
 ...
 *                          html = if true, then HTML formatted output will be returned; otherwise plain-text output is
 *                          returned. [Default is false] print = if true output is printed to standard output. [Default
 *                          is false] skip = number of calls to skip from the top of the stack. [Default is 0]
 *
...
function caPrintStacktrace( $pa_options = null ) {
	...
	foreach ( $va_trace as $va_line ) {
		if ( isset( $pa_options['html'] ) && $pa_options['html'] ) {
			...
			$va_buf[] = $va_line['file'] . ':' . ( $va_line['class'] ? $va_line['class'] . ':' : '' )
			            . $va_line['function'] . '@' . $va_line['line'] . "<br/>\n";
		}
	}
 ...
 * to be expressed as pixels. If an integer suffixed with 'px' is passed. Percentages are parsed as relative
 * dimensions.
 * then the dimension is considered to be expressed in pixels. If non-integers are passed they will
 * be cast to integers.
 *
 ...
 * @param string $ps_dimension
 * @param array  $pa_options Options include:
 *                           returnAsString = return normalized dimension expression as string rather than an array of
 *                           values. [Default is false] default = dimension expression to use if $ps_dimension is
 ...
 * @param array $pa_options   Options include:
 *                            dontRemoveKeyPrefixes = By default keys that are period-delimited will have the prefix
 *                            before the first period removed (this is to ease sorting by field names). Set to true to
 *                            disable this behavior. [Default is false] caseInsenstive = Sort case insensitively.
 *                            [Default is true] naturalSort = Sort case insensitively and only considers letters and
 *                            numbers in sort, stripping punctuation and other characters. [Default is false]
 *
...
function caSortArrayByKeyInValue( $pa_values, $pa_sort_keys, $ps_sort_direction = "ASC", $pa_options = null ) {
	...
	foreach ( $pa_values as $vn_id => $va_data ) {
		...
		foreach ( $va_sort_keys as $vs_sort_key ) {
			$k = isset( $va_data[ $vs_sort_key . '_sort_' ] ) ? $va_data[ $vs_sort_key . '_sort_' ]
				: $va_data[ $vs_sort_key ];  // an alternative sort-specific value for a key may be present with the suffix "_sort_"; when present we use this in preference to the key value
...
function caUcFirstUTF8Safe( $ps_string, $pb_capitalize_all_words = false ) {
	...
	foreach ( $va_words as $vs_string ) {
		...
		$va_proc_words[] = mb_strtoupper( $vs_first_char, 'UTF-8' ) . mb_substr( $vs_string, 1, $vn_strlen - 1,
				'UTF-8' );
	}
...
function caGetCacheObject( $ps_prefix, $pn_lifetime = 86400, $ps_cache_dir = null, $pn_cleaning_factor = 100 ) {
	...
	$va_frontend_options = array(
		...
		'logging'                   => false,
		/* do not use Zend_Log to log what happens */
		'write_control'             => false,
		/* immediate read after write is enabled (we don't write often) */
		'automatic_cleaning_factor' => $pn_cleaning_factor,
		/* automatic cache cleaning */
		'automatic_serialization'   => true
		/* we store arrays, so we have to enable that */
	);
...
# ------------------------------------------------------------------------------------------------
/**
 * Returns the media class to which a MIME type belongs, or null if the MIME type does not belong to a class. Possible
 * classes are 'image', 'video', 'audio' and 'document'
 *
 * @param string $ps_mimetype A media MIME type
 *
 * @return string The media class that includes the specified MIME type, or null if the MIME type does not belong to a
 *                class. Returned classes are 'image', 'video', 'audio' and 'document'
 */
...
# ------------------------------------------------------------------------------------------------
/**
 * Returns all MIME types contained in the specified media class. Note that inclusive type groups will be returned as a
 * general group type. Eg. for class 'images' the inclusive MIME type 'image/*' is returned rather than a list of
 * specific image types such as 'image/jpeg', 'image/tiff', etc.
 *
 * @param string $ps_class   The media class to return MIME types for. Valid classes are 'image', 'video', 'audio' and
 *                           'document'
 * @param array  $pa_options Options include
 *                           returnAsRegex = if set a string suitable for use as a regular expression for matching of
 *                           MIME types is returned instead of an array
 *
 ...
 *
 * @return string The JSON-encoded data formatted for ease of reading. Other than spacing and indentation, the returned
 *                data is unchanged from the input.
 */
 ...
 *
 * @param string $ps_date_expression A valid date/time expression as described in
 *                                   http://docs.collectiveaccess.org/wiki/Date_and_Time_Formats
 *
 * @return array The start and end timestamps for the parsed date/time range. Array contains values key'ed under 0 and
 *               1 and 'start' and 'end'; null is returned if expression cannot be parsed.
 */
 ...
 *
 * @param string $ps_date_expression A date/time expression as described in
 *                                   http://docs.collectiveaccess.org/wiki/Date_and_Time_Formats
 *
...
function caDecodeAllEntities( $ps_string, $pn_quotes = ENT_COMPAT, $ps_charset = 'UTF-8' ) {
	return html_entity_decode( preg_replace_callback( '/&([a-zA-Z][a-zA-Z0-9]+);/', 'caConvertEntity',
		html_entity_decode( $ps_string ) ), $pn_quotes, $ps_charset );
}
 ...
 * @param string $ps_orientation Side of string to based truncation from. "start" will truncate $pn_max_length
 *                               characters from the beginning; "end" $pn_max_length characters from the end.
 *                               [Default="start"]
 *
 ...
 * caGetOption() provides a simple interface to grab values, force default values for non-existent settings and enforce
 * simple validation rules.
 *
 * @param mixed $pm_option        The option to extract. If an array is provided then each option is tried, in order,
 *                                until a non-false value is found.
 * @param array $pa_options       The options array to extract values from. An instance of Zend_Console_Getopt may also
 *                                be passed, allowing processing of command line options.
 ...
 *                                validValues = array of values that are possible for this option. If the option value
 *                                is not in the list then the default is returned. If no default is set then the first
 *                                value in the validValues list is returned. Note that by default all comparisons are
 *                                case-insensitive. caseSensitive = do case sensitive comparisons when checking the
 *                                option value against the validValues list [default=false] castTo =
 *                                array|int|string|float|bool delimiter = A delimiter, or array of delimiters, to break
 *                                a string option value on. When this option is set an array will always be returned.
 *                                [Default is null] defaultOnEmptyString = Force use of default value when option is
 *                                set to an empty string). [Default is false]
 *
 * @return mixed
 */
function caGetOption( $pm_option, $pa_options, $pm_default = null, $pa_parse_options = null ) {
	...
	if ( is_array( $pm_option ) ) {
		...
		$vm_val = ( isset( $pa_options[ $pm_option ] ) && ! is_null( $pa_options[ $pm_option ] ) )
			? $pa_options[ $pm_option ] : $pm_default;
	}
	if ( isset( $pa_parse_options['defaultOnEmptyString'] ) && $pa_parse_options['defaultOnEmptyString']
	     && is_string( $vm_val )
	     && ( strlen( $vm_val ) === 0 )
	) {
		$vm_val = $pm_default;
	...
	if (
		( ( is_string( $vm_val ) && ! isset( $pa_parse_options['castTo'] ) )
		  || ( isset( $pa_parse_options['castTo'] )
		       && ( $pa_parse_options['castTo'] == 'string' ) ) )
	...
	$vs_cast_to = ( isset( $pa_parse_options['castTo'] ) && ( $pa_parse_options['castTo'] ) )
		? strtolower( $pa_parse_options['castTo'] ) : '';
	switch ( $vs_cast_to ) {
		case 'int':
		case 'integer':
			$vm_val = (int) $vm_val;
	...
	if ( is_string( $vm_val )
	     && ( isset( $pa_parse_options['delimiter'] )
	          && ( $va_delimiter
				= $pa_parse_options['delimiter'] ) )
	) {
 ...
 *
 * Note that function is of limited use outside of the case it was designed for: to remove binary entries from
 * extracted EXIF metadata arrays.
 *
 * @param array $pa_array The array to sanitize
 * @param array $pa_options
 *                        allowStdClass = stdClass object array values are allowed. This is useful for arrays that are
 *                        about to be passed to json_encode [Default=false] removeNonCharacterData = remove
 *                        non-character data from all array values. This option leaves all character data in-place
 *                        [Default=false]
 *
...
function caSanitizeArray( $pa_array, $pa_options = null ) {
	...
	foreach ( $pa_array as $vn_k => $vm_v ) {
		if ( is_array( $vm_v ) ) {
			...
			if ( ( ( ! empty( $vm_v ) && strlen( $vm_v ) < 8192 ) && ( ! preg_match( "!^\X+$!", $vm_v ) ) )
			     || ( ! mb_detect_encoding( $vm_v ) )
			) {
				unset( $pa_array[ $vn_k ] );
				continue;
			}
...
# ---------------------------------------
/**
 * Process all string values in an array with HTMLPurifier. Arrays may be of any depth. If a string is passed it will
 * be purified and returned.
 *
 * @param array $pm_array   The array or string to purify
 * @param array $pa_options Array of options:
 *                          purifier = HTMLPurifier instance to use for processing. If null a new instance will be
 ...
 *                          Set $print_r to TRUE when dealing with a recursive data structure as var_export() will
 *                          generate an error.
 */
function caDebug( $vm_data, $vs_label = null, $print_r = false ) {
	if ( defined( '__CA_ENABLE_DEBUG_OUTPUT__' ) && __CA_ENABLE_DEBUG_OUTPUT__ ) {
		if ( caIsRunFromCLI() ) {
			...
			$vs_string = htmlspecialchars( ( $print_r ? print_r( $vm_data, true ) : var_export( $vm_data, true ) ),
				ENT_QUOTES, 'UTF-8' );
 ...
 * @param null|array $pa_options Options include and option from
 *                               BundlableLabelableBaseModelWithAttributes::makeSearchResult as well as : transaction =
 *                               transaction to execute queries within. [Default is null]
 *
...
function caMakeSearchResult( $ps_table, $pa_ids, $pa_options = null ) {
	if ( $t_instance = Datamodel::getInstanceByTableName( 'ca_objects',
		true )
	) {    // get an instance of a model inherits from BundlableLabelableBaseModelWithAttributes; doesn't matter which one
 ...
 *                    numericValue = return floating point numeric value only, without currency specifier. Default is
 *                    false.
 *
 * @return string Converted value with currency specifier, unless numericValue option is set. Returns null if value
 ...
 * @param $element_code_or_id Metadata element code or element_id. If provided currency symbol settings from the
 *                            element will be used. If omitted only global (app.conf) settingd will be used. [Default
 *                            is null]
 *
 ...
 */
function caFitImageDimensions(
	$pn_original_width, $pn_original_height, $pn_target_width, $pn_target_height, $pa_options = null
) {
 ...
 * @param string $ps_token              access token. Global account password can be used here but it's recommended to
 *                                      create a personal access token instead.
 * @param string $ps_owner              The repository owner
 * @param string $ps_repo               repository name
 * @param string $ps_git_path           path for the file destination inside the repository, e.g.
 *                                      "/exports/from_collectiveaccess/export.xml."
 * @param string $ps_local_filepath     file to upload as absolute local path. Note that the file must be loaded in
 *                                      memory to be committed to GitHub.
 ...
 *                                      true means the file is updated in place for. false means we abort. default is
 *                                      true
 * @param string $ps_commit_msg         commit message
 *
 * @return bool success state
 */
function caUploadFileToGitHub(
	$ps_user, $ps_token, $ps_owner, $ps_repo, $ps_git_path, $ps_local_filepath, $ps_branch = 'master',
	$pb_update_on_conflict = true, $ps_commit_msg = null
) {
	// check mandatory params
	if ( ! $ps_user || ! $ps_token || ! $ps_owner || ! $ps_repo || ! $ps_git_path || ! $ps_local_filepath ) {
		caLogEvent( 'DEBG', "Invalid parameters for GitHub file upload. Check your configuration!",
	...
	try {
		$o_client->repositories()->contents()
		         ->create( $ps_owner, $ps_repo, $ps_git_path, $vs_content, $ps_commit_msg, $ps_branch );
	} catch ( Github\Exception\RuntimeException $e ) {
		switch ( $e->getCode() ) {
			case 401:
				caLogEvent( 'DEBG',
					"Could not authenticate with GitHub. Error message was: " . $e->getMessage() . " - Code was: "
					. $e->getCode(), 'caUploadFileToGitHub' );
				break;
			case 422:
				if ( $pb_update_on_conflict ) {
					try {
						...
						if ( isset( $va_content['sha'] ) ) {
							$o_client->repositories()->contents()
							         ->update( $ps_owner, $ps_repo, $ps_git_path, $vs_content, $ps_commit_msg,
								         $va_content['sha'], $ps_branch );
						}
						...
						caLogEvent( 'DEBG',
							"Could not update exiting file in GitHub. Error message was: " . $ex->getMessage()
							. " - Code was: " . $ex->getCode(), 'caUploadFileToGitHub' );
						break;
					}
				} else {
					caLogEvent( 'DEBG',
						"Could not upload file to GitHub. It looks like a file already exists at {$ps_git_path}.",
						'caUploadFileToGitHub' );
				}
				break;
			default:
				caLogEvent( 'DEBG',
					"Could not upload file to GitHub. A generic error occurred. Error message was: " . $e->getMessage()
					. " - Code was: " . $e->getCode(), 'caUploadFileToGitHub' );
				break;
		}
		...
		caLogEvent( 'DEBG',
			"Could not upload file to GitHub. The parameter validation failed. Error message was: " . $e->getMessage()
			. " - Code was: " . $e->getCode(), 'caUploadFileToGitHub' );
...
function caExportDataToResourceSpace( $ps_user, $ps_key, $ps_base_url, $ps_local_filepath ) {
	...
	if ( ! $ps_user || ! $ps_key || ! $ps_base_url || ! $ps_local_filepath ) {
		caLogEvent( 'DEBG', "Invalid parameters for ResourceSpace export. Check your configuration!",
			'caExportDataToResourceSpace' );
	...
	foreach ( $va_records as $va_record ) {
		...
		try {
			$vs_query    = 'user=' . $ps_user . '&function=create_resource&param1=' . $vs_resource_type
			               . '&param2=0&param3=' . rawurlencode( $vs_media_url ) . '&param4=&param5=&param6=&param7='
			               . rawurlencode( json_encode( $va_record ) );
			...
			if ( ! $vn_rs_id ) {
				caLogEvent( 'ERR', "Could not create Resource. Check your ResourceSpace configuration",
					'caExportDataToResourceSpace' );
				continue;
			}
			if ( $vs_collection_name ) {
				...
				if ( ! $vs_collection_id ) {
					$vs_query = 'user=' . $ps_user . '&function=create_collection&param1='
					            . rawurlencode( $vs_collection_name );
					$vs_hash  = hash( 'sha256', $ps_key . $vs_query );
					...
					if ( $vb_success ) {
						...
						caLogEvent( 'DEBG', "Could not create Collection. Check your ResourceSpace configuration",
							'caExportDataToResourceSpace' );
					}
				}
				$vs_query = 'user=' . $ps_user . '&function=add_resource_to_collection&param1=' . $vn_rs_id . '&param2='
				            . $vs_collection_id;
				$vs_hash  = hash( 'sha256', $ps_key . $vs_query );
			...
			caLogEvent( 'ERR',
				"Could not export data to ResourceSpace with error: " . $e->getMessage() . " - Code was: "
				. $e->getCode(), 'caExportDataToResourceSpace' );
...
function caQueryExternalWebservice( $ps_url ) {
	...
	if ( curl_getinfo( $vo_curl, CURLINFO_HTTP_CODE ) !== 200 ) {
		throw new WebServiceError( _t( 'An error occurred while querying an external webservice' ) . _t( " at %1",
				$ps_url ) . " " . print_r( curl_getinfo( $vo_curl ), true ) );
	}
	curl_close( $vo_curl );
...
# ----------------------------------------
/**
 * Parses and normalizes length exprssions in the form <dimension1> <delimiter> <dimension2> <delimiter> <dimension3>
 * ... (Ex. 4" x 5") into an array of normalized dimension string. When no units are specified default units are
 * specified (Ex. 4x6 is returned as ["4 in", "6 in"]). When units are specified for some, but not all, quantities then
 * the first specified unit in the expression in applied to all unit-less quantities
 * (Ex. 4x6cm is returned as ["4 cm", "6 cm"] no matter what default units are set to). When units are specified that
 * are always used for the quantity they apply to (Ex. 4 x 6cm x 8" is returned as ["4 cm", "6 cm", "8 in"])
 *
 * @param string     $ps_expression Expression to parse
 * @param null|array $pa_options    Options include:
 *                                  delimiter = Delimiter string between dimensions. Delimiter will be processed
 *                                  case-insensitively. [Default is 'x'] units = Units to use as default for quantities
 *                                  that lack a specification. [Default is inches] returnExtractedMeasurements = return
 *                                  an array of arrays, each of which includes the numeric quantity, units and display
 *                                  string as separate values. [Default is false] precision = number of significant
 *                                  digits to the right of the decimal point to return, overriding any unit specific
 *                                  settings in dimensions.conf. [Default is null]
 *
 * @return array An array of parsed and normalized length dimensions, parseable by caParseLengthDimension() or
 *               Zend_Measure
 */
function caParseLengthExpression( $ps_expression, $pa_options = null ) {
	...
	foreach ( $va_measurements as $vn_i => $vs_measurement ) {
		...
		try {
			if ( ! ( $vo_parsed_measurement = caParseLengthDimension( $vs_measurement ) ) ) {
				...
				if ( ! strlen( $vn_parse_precision = $pn_precision )
				     && ! strlen(
						$vn_parse_precision = $o_dimensions_config->get( caGetLengthUnitType( $vs_extracted_units,
								[ 'code' => true ] ) . "_decimal_precision" ) )
				) {
		...
		$va_extracted_measurements[] = [
			'quantity' => trim( preg_replace( "![^\d\. \/]+!", "", $vs_measurement ) ),
			'string'   => $vs_measurement,
			'units'    => $vs_extracted_units,
			'source'   => $vs_measurement_original
		];
	}
	...
	foreach ( $va_extracted_measurements as $vn_i => $va_measurement ) {
		...
		if ( ! $va_unit_map[ $vn_i ] ) {
			...
			try {
				if ( $vo_parsed_measurement = caParseLengthDimension( $va_measurement['string']
				                                                      . " {$vs_inferred_units}" )
				) {
					if ( ! strlen( $vn_parse_precision = $pn_precision )
					     && ! strlen(
							$vn_parse_precision = $o_dimensions_config->get( caGetLengthUnitType( $vs_inferred_units,
									[ 'code' => true ] ) . "_decimal_precision" ) )
					) {
						$vn_parse_precision = 4;
					}
...
function caGenerateCSRFToken( $po_request = null ) {
	...
	if ( function_exists( "random_bytes" ) ) {           // PHP 7
		...
		$vs_token = md5( uniqid( rand(),
			true ) );   // this is not very good, and is only used if one of the more secure options above is available (one of them should be in almost all cases)
	}
	if ( $po_request ) {
		...
		if ( sizeof( $va_tokens ) > 600 ) {
			$va_tokens = array_slice( $va_tokens, 0, 200,
				true ); // delete last third of token buffer if it gets too long
		}
 ...
 *
 * @param RequestHTTP $po_request Current request
 * @param string      $ps_token   CSRF token to validate. If omitted token in the "crsfToken" parameter is extracted
 ...
 *
 * @param string       $ps_id_prefix the id prefix for the field in question, used to figure out what format the array
 *                                   has
...
function caLengthToFractions( $pn_inches_as_float, $pn_denom, $pb_reduce = true, $pa_options = null ) {
	...
	$pb_use_unicode_fraction_glyphs = ( is_array( $pa_use_unicode_fraction_glyphs_for )
	                                    && sizeof( $pa_use_unicode_fraction_glyphs_for ) );
	if ( is_null( $pa_use_unicode_fraction_glyphs_for ) ) {
		$pb_use_unicode_fraction_glyphs = $o_config->get( 'use_unicode_fractions_for_measurements' );
	}

	$pn_inches_as_float = (float) preg_replace( "![^\d\.]+!", "",
		$pn_inches_as_float );    // remove commas and such; also remove "-" as dimensions can't be negative
	...
	if ( $pb_use_unicode_fraction_glyphs ) {
		...
		if ( $unicode_frac && is_array( $pa_use_unicode_fraction_glyphs_for )
		     && sizeof( $pa_use_unicode_fraction_glyphs_for )
		     && in_array( $unicode_frac, $pa_use_unicode_fraction_glyphs_for )
		) {
			$frac = $unicode_frac;
		} else {
			$frac = "{$num}/{$pn_denom}";
 ...
 * @param array  $pa_options Options include:
 *                           locale = Locale settings to use. If omitted current default locale is used. [Default is
 *                           current locale] omitArticle = Omit leading definite and indefinited articles, rather than
 *                           moving them to the end of the text [Default is true]
 *
...
function caSortableValue( $ps_text, $pa_options = null ) {
	...
	foreach ( $va_articles as $vs_article ) {
		if ( preg_match( '!^(' . $vs_article . ')[ ]+!i', $vs_display_value, $va_matches ) ) {
			$vs_display_value = trim( str_replace( $va_matches[1], '', $vs_display_value ) . ( $pb_omit_article ? ''
					: ', ' . $va_matches[1] ) );
			break;
		}
	}
...
function caNormalizeValueArray( $pa_values, $pa_options = null ) {
	...
	foreach ( $pa_values as $vs_key => $vm_val ) {
		if ( is_array( $vm_val ) ) {
			if ( isset( $vm_val[0] ) && ! is_array( $vm_val[0] )
			     && caIsValidSqlOperator( $vm_val[0], [ 'nullable' => true, 'isList' => true ] )
			) {
				$vm_val = [ $vm_val ];
			}
			foreach ( $vm_val as $vs_key2 => $vm_list_vals ) {
				if ( is_array( $vm_list_vals ) && ! is_array( $vm_list_vals[0] )
				     && caIsValidSqlOperator( $vm_list_vals[0], [ 'nullable' => true, 'isList' => true ] )
				) {
					...
					if ( caIsValidSqlOperator( $vm_list_val[0], [ 'nullable' => true, 'isList' => true ] ) ) {
						if ( is_array( $vm_list_val[1] ) && $o_purifier ) {
							...
							foreach ( $vm_list_val[1] as $vm_sublist_val ) {
								$va_vals_proc[] = ! is_null( $vm_sublist_val ) ? str_replace( "&amp;", "&",
									$o_purifier->purify( $vm_sublist_val ) ) : $vm_sublist_val;
							}
							...
							if ( ! is_numeric( $vs_key2 ) ) {
								$va_values_proc[ $vs_key ][ $vs_key2 ][] = [
									$vm_list_val[0],
									$o_purifier && ! is_null( $vm_list_val[1] ) ? str_replace( "&amp;", "&",
										$o_purifier->purify( $vm_list_val[1] ) ) : $vm_list_val[1]
								];
							} else {
								$va_values_proc[ $vs_key ][] = [
									$vm_list_val[0],
									$o_purifier && ! is_null( $vm_list_val[1] ) ? str_replace( "&amp;", "&",
										$o_purifier->purify( $vm_list_val[1] ) ) : $vm_list_val[1]
								];
			...
			$va_values_proc[ $vs_key ][] = [
				is_null( $vm_val ) ? 'IS' : '=',
				$o_purifier && ! is_null( $vm_val ) ? str_replace( "&amp;", "&", $o_purifier->purify( $vm_val ) )
					: $vm_val
			];
		}
	}
...
function caExtractTagsFromTemplate( $ps_template, $pa_options = null ) {
		...
		switch ( $vs_char = mb_substr( $ps_template, $i, 1 ) ) {
			...
			default:
				if ( $vb_in_tag ) {
					...
					if (
						( $vb_is_ca_get_ref && ! $vb_have_seen_param_delimiter
						  && ( ! preg_match( "![A-Za-z0-9_\-\.~:]!", $vs_char ) ) )
						|| ( ( $vs_char === ':' )
						     && ! preg_match( "!^[A-Za-z0-9]+!", mb_substr( $ps_template,
								$i + 1 ) ) )    // colon not followed by letters of numbers is not part of tag
					) {
						if ( $vs_tag = trim( $vs_tag ) ) {
							$va_tags[] = $vs_tag;
						}
	...
	foreach ( $va_tags as $vn_i => $vs_tag ) {
		...
		$va_tags[ $vn_i ] = rtrim( $vs_tag,
			")/.,%" );    // remove trailing slashes, periods and percent signs as they're potentially valid tag characters that are never meant to be at the end
	}
 ...
 * Ex. if attribute string is: idno="2010.001.5" type="object" and the values to be extracted are "idno" and "type" an
 * array in the form ["idno" => "2010.001.5", "type" => "object"] is returned.
 *
 ...
 *                          conjunction = Delimiter used between second-to-last and last list item. [Default is
 *                          localized test for 'and']
 *
 * @return string
 */
 ...
 * @param array  $options Options include:
 *                        mimetypes = MIME type, or list of MIME types expected in response to URL. If content type of
 *                        response does ot match the MIME type(s) specified here a UrlFetchException is thrown.
 *
...
function caFetchFileFromUrl( $url, $dest = null, array $options = null ) {
	...
	if ( $status_code !== 200 ) {
		@unlink( $tmp_file );
		throw new UrlFetchException( _t( "Media fetched from URL [%1] failed with status code %2", $url,
			$status_code ) );
	}
