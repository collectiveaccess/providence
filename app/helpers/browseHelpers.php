 ...
 *                          termClass = CSS class to <span> browse term with. If not set, defaults to
 *                          'hierarchyBrowserItemTerm' pathClass = CSS class to <span> browse path elements with. If
 *                          not set, defaults to 'hierarchyBrowserItemPath'
 *
 * @return string
 */
function caGetLabelForDisplay( &$pa_facet, $pa_item, $pa_facet_info, $pa_options = null ) {
	$vs_term_class = ( isset( $pa_options['termClass'] ) && $pa_options['termClass'] ) ? $pa_options['termClass']
		: 'hierarchyBrowserItemTerm';
	$vs_label      = "<span class='{$vs_term_class}'>" . htmlentities( isset( $pa_item['display_label'] )
			? $pa_item['display_label'] : $pa_item['label'], ENT_COMPAT, 'UTF-8' ) . "</span>";
	if ( $pa_facet_info['show_hierarchy'] && $pa_item['parent_id'] ) {
		...
		if ( isset( $pa_facet_info['hierarchy_limit'] ) && ( $pa_facet_info['hierarchy_limit'] > 0 )
		     && ( sizeof( $va_hierarchy ) > $pa_facet_info['hierarchy_limit'] )
		) {
 ...
 * @param array $pa_facet_info
 * @param array $pa_options List of display options. Supported options are:
 *                          pathClass = CSS class to <span> browse path elements with. If not set, defaults to
...
function caGetHierarchicalLabelsForDisplay( &$pa_facet, $pn_id, $pa_options = null ) {
	...
	$vs_path_class = ( isset( $pa_options['pathClass'] ) && $pa_options['pathClass'] ) ? $pa_options['pathClass']
		: 'hierarchyBrowserItemPath';
	$va_values     = array(
		"<span class='{$vs_path_class}'>" . htmlentities( $pa_facet[ $pn_id ]['label'], ENT_COMPAT, 'UTF-8' )
		. "</span>"
	);
...
function caGetBrowseInstance( $pm_table_name_or_num, $pa_options = null ) {
	...
	$vs_table = ( is_numeric( $pm_table_name_or_num ) ) ? Datamodel::getTableName( (int) $pm_table_name_or_num )
...
function caGetBrowseTypes( $pa_options = null ) {
	...
	if ( caGetOption( 'forMenuBar', $pa_options, false ) ) {
		foreach ( $va_browse_types as $vs_k => $va_browse_info ) {
			if ( isset( $va_browse_info['dontIncludeInBrowseMenu'] )
			     && (bool) $va_browse_info['dontIncludeInBrowseMenu']
			) {
...
function caGetFacetForMenuBar( $po_request, $vs_browse_type, $pa_options = null ) {
	...
	if ( ( isset( $va_browse_info['facetGroup'] )
	       && ( ( $vs_menu_bar_facet_group
				= $o_browse_config->get( 'menubarFacetGroup' ) )
	            || ( $vs_menu_bar_facet_group
				= $va_browse_info['facetGroup'] ) ) )
	) {
	...
	$o_browse->execute( array(
		'checkAccess' => caGetUserAccessValues( $po_request ),
		'showAllForNoCriteriaBrowse' => true
	) );
	...
	foreach ( $va_facets as $vs_facet_name => $va_facet_info ) {
		...
		$vs_buf .= "<li " . ( ( ( $vs_default_facet == $vs_facet_name ) && $vb_select_default_facet ) ? "class='active'"
				: "" ) . "><a href='#' onclick='jQuery(\".browseMenuFacet\").load(\"" . caNavUrl( $po_request, '*',
				'Browse', $vs_browse_type,
				array( 'facet' => $vs_facet_name, 'getFacet' => 1, 'key' => $vs_key, 'isNav' => 1 ) )
		           . "\", function() { jQuery(this).parent().scrollTop(0); }); jQuery(this).parent().siblings().removeClass(\"active\"); jQuery(this).parent().addClass(\"active\"); return false;'>"
		           . caUcFirstUTF8Safe( $va_facet_info['label_plural'] ) . "</a></li>\n";
	}

	if ( $vs_default_facet && $vb_select_default_facet ) {
		$vs_buf .= "<script type='text/javascript'>jQuery(document).ready(function() { jQuery(\".browseMenuFacet\").load(\""
		           . caNavUrl( $po_request, '*', 'Browse', $vs_browse_type,
				array( 'facet' => $vs_default_facet, 'getFacet' => 1, 'key' => $vs_key, 'isNav' => 1 ) )
		           . "\"); });</script>\n";
	}
