...
function caLoadAAT( $ps_path_to_aat_data = null, $pa_options = null ) {
	...
	$vn_list_item_relation_type_id_related = $t_rel_types->getRelationshipTypeID( 'ca_list_items_x_list_items',
		'related' );
		...
		switch ( $o_xml->name ) {
			...
			case 'Subject':
				if ( $o_xml->nodeType == XMLReader::END_ELEMENT ) {
					...
					$vs_message = "\tIMPORTING #" . ( $vn_term_count + 1 ) . " [" . $va_subject['subject_id'] . "] "
					              . $vs_preferred_term;
					...
					if ( $t_item = $t_list->addItem( $va_subject['subject_id'], $pb_is_enabled, false, null,
						$vn_type_id, $va_subject['subject_id'], '', 4, 1 )
					) {
						$va_aat_id_to_item_id[ $va_subject['subject_id'] ] = $t_item->getPrimaryKey();

						if ( $va_subject['preferred_parent_subject_id'] != 300000000 ) {
							$va_parent_child_links[ $va_subject['subject_id'] ]
								= $va_subject['preferred_parent_subject_id'];
						}

						// add preferred labels
						if ( ! ( $t_item->addLabel(
							array(
								'name_singular' => trim( htmlentities( $vs_preferred_term, ENT_NOQUOTES ) ),
								'name_plural'   => trim( htmlentities( $vs_preferred_term, ENT_NOQUOTES ) ),
								'description'   => $va_subject['description']
							),
							$pn_en_locale_id, null, true
						) )
						) {
							print "ERROR: Could not add preferred label to AAT term [" . $va_subject['subject_id']
							      . "] " . $vs_preferred_term . ": " . join( "; ", $t_item->getErrors() ) . "\n";
						}
						...
						if ( is_array( $va_subject['non_preferred_terms'] ) ) {
							for ( $vn_i = 0; $vn_i < sizeof( $va_subject['non_preferred_terms'] ); $vn_i ++ ) {
								...
								if ( ! ( $t_item->addLabel(
									array(
										'name_singular' => trim( htmlentities( $vs_np_label, ENT_NOQUOTES ) ),
										'name_plural'   => trim( htmlentities( $vs_np_label, ENT_NOQUOTES ) ),
										'description'   => ''
									),
									$pn_en_locale_id, $vn_np_term_type_id, false
								) )
								) {
									print "ERROR: Could not add non-preferred label to AAT term ["
									      . $va_subject['subject_id'] . "] " . $vs_np_label
						...
						print "ERROR: Could not import AAT term [" . $va_subject['subject_id'] . "] "
						      . $vs_preferred_term . ": " . join( "; ", $t_list->getErrors() ) . "\n";
					}
				} else {
	...
	foreach ( $va_parent_child_links as $vs_child_id => $vs_parent_id ) {
		...
		if ( $t_item->numErrors() ) {
			print "ERROR: could not set parent_id for {$vs_child_id} (was translated to item_id={$vn_child_item_id}): "
			      . join( '; ', $t_item->getErrors() ) . "\n";
		}
	}
	...
	if ( $vn_list_item_relation_type_id_related > 0 ) {
		...
		foreach ( $va_item_item_links as $vs_left_id => $vs_right_id ) {
			...
			if ( $t_link->numErrors() ) {
				print "ERROR: could not set link between {$vs_left_id} (was translated to item_id={$vn_left_item_id}) and {$vs_right_id} (was translated to item_id={$vn_right_item_id}): "
				      . join( '; ', $t_link->getErrors() ) . "\n";
			}
		}
	} else {
...
function caLoadULAN( $ps_path_to_ulan_data = null, $ps_path_to_ulan_config = null, $pa_options = null ) {
					...
					if ( $o_xml->nodeType == XMLReader::END_ELEMENT ) {
						...
						if ( $vs_ulan_import_mode == 'ca_entities' ) {
							...
							if ( is_array( $va_subject['non_preferred_terms'] ) ) {
								for ( $vn_i = 0; $vn_i < sizeof( $va_subject['non_preferred_terms'] ); $vn_i ++ ) {
									$va_np_labels[]
										= DataMigrationUtils::splitEntityName( trim( htmlentities( $va_subject['non_preferred_terms'][ $vn_i ] ) ) );
								}
							}

							$t_item
								= DataMigrationUtils::getEntityID( DataMigrationUtils::splitEntityName( trim( htmlentities( $vs_preferred_term,
								ENT_NOQUOTES ) ) ), $vn_type_id, $pn_en_locale_id,
								array( 'idno' => $va_subject['subject_id'] ),
								array( 'nonPreferredLabels' => $va_np_labels, 'returnInstance' => true ) );
							if ( ! $t_item ) {
							...
							if ( $t_item = $t_list->addItem( $va_subject['subject_id'], $pb_is_enabled, false, null,
								...
								if ( ! ( $t_item->addLabel(
									...
									$o_log->logError( "Could not add preferred label to ULAN term ["
									                  . $va_subject['subject_id'] . "] " . $vs_preferred_term . ": "
									                  . join( "; ", $t_item->getErrors() ) );
								}
								...
								if ( is_array( $va_subject['non_preferred_terms'] ) ) {
									for ( $vn_i = 0; $vn_i < sizeof( $va_subject['non_preferred_terms'] ); $vn_i ++ ) {
										...
										if ( ! ( $t_item->addLabel(
											...
											$o_log->logError( "Could not add non-preferred label to ULAN term ["
											                  . $va_subject['subject_id'] . "] " . $vs_np_label );
										}
									}
								}
							} else {
								$o_log->logError( "Could not import ULAN term [" . $va_subject['subject_id'] . "] "
								                  . $vs_preferred_term . ": " . join( "; ", $t_list->getErrors() ) );
								break;
							}
						}
						...
						foreach ( $va_mapping as $vs_dest => $vs_source ) {
							...
							if ( sizeof( $va_values ) ) {
								...
								$t_item->update( array(
									'dontCheckCircularReferences' => true,
									'dontSetHierarchicalIndexing' => true
								) );
								if ( $t_item->numErrors() ) {
									$o_log->logError( "Could not update ULAN list item with content values: "
									                  . join( "; ", $t_item->getErrors() ) );
								}
							}
						}
										...
										case 'Nationality_Code':
											switch ( $o_xml->nodeType ) {
												case XMLReader::ELEMENT:
													...
													$va_nationality['name'] = array_pop( explode( '/',
														$o_xml->value ) );
													break;
											}
											break;
	...
	foreach ( $va_parent_child_links as $vs_child_id => $vs_parent_id ) {
		...
		if ( in_array( $vs_parent_id, array( '500000000', '500000001' ) ) ) {
			...
			if ( $t_item->numErrors() ) {
				$o_log->logError( "Could not set parent_id for {$vs_child_id} to root): " . join( '; ',
						$t_item->getErrors() ) );
				continue;
			}
		...
		if ( $t_item->numErrors() ) {
			$o_log->logError( "Could not set parent_id for {$vs_child_id} (was translated to item_id={$vn_child_item_id}): "
			                  . join( '; ', $t_item->getErrors() ) );
		}
	}
	...
	$vn_list_item_relation_type_id_related = $t_rel_type->getRelationshipTypeID( 'ca_list_items_x_list_items',
		'related' );
	if ( $vn_list_item_relation_type_id_related > 0 ) {
		...
		foreach ( $va_item_item_links as $vs_left_id => $vs_right_id ) {
			...
			if ( $t_link->numErrors() ) {
				$o_log->logError( "Could not set link between {$vs_left_id} (was translated to item_id={$vn_left_item_id}) and {$vs_right_id} (was translated to item_id={$vn_right_item_id}): "
				                  . join( '; ', $t_link->getErrors() ) );
			}
		}
	} else {
