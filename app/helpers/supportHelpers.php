...
function caLoadAAT($ps_path_to_aat_data = null, $pa_options = null)
{
    ...
    $vn_list_item_relation_type_id_related = $t_rel_types->getRelationshipTypeID(
                ...
                if ($o_xml->nodeType == XMLReader::END_ELEMENT) {
                    ...
                    if ($t_item = $t_list->addItem(
                        ...
                        if (!($t_item->addLabel(
                            ...
                            print "ERROR: Could not add preferred label to AAT term [" . $va_subject['subject_id'] . "] " . $vs_preferred_term . ": " . join(
                                    "; ",
                                    $t_item->getErrors()
                                ) . "\n";
                        }
                        ...
                        print "ERROR: Could not import AAT term [" . $va_subject['subject_id'] . "] " . $vs_preferred_term . ": " . join(
                                "; ",
                                $t_list->getErrors()
                            ) . "\n";
                    }
    ...
    foreach ($va_parent_child_links as $vs_child_id => $vs_parent_id) {
        ...
        if ($t_item->numErrors()) {
            print "ERROR: could not set parent_id for {$vs_child_id} (was translated to item_id={$vn_child_item_id}): " . join(
                    '; ',
                    $t_item->getErrors()
                ) . "\n";
        }
    ...
    if ($vn_list_item_relation_type_id_related > 0) {
        ...
        foreach ($va_item_item_links as $vs_left_id => $vs_right_id) {
            ...
            if ($t_link->numErrors()) {
                print "ERROR: could not set link between {$vs_left_id} (was translated to item_id={$vn_left_item_id}) and {$vs_right_id} (was translated to item_id={$vn_right_item_id}): " . join(
                        '; ',
                        $t_link->getErrors()
                    ) . "\n";
            }
...
function caLoadULAN($ps_path_to_ulan_data = null, $ps_path_to_ulan_config = null, $pa_options = null)
{
                ...
                case 'Subject':
                    if ($o_xml->nodeType == XMLReader::END_ELEMENT) {
                        ...
                        if ($vs_ulan_import_mode == 'ca_entities') {
                            ...
                            if (is_array($va_subject['non_preferred_terms'])) {
                                for ($vn_i = 0; $vn_i < sizeof($va_subject['non_preferred_terms']); $vn_i++) {
                                    $va_np_labels[] = DataMigrationUtils::splitEntityName(
                                        trim(htmlentities($va_subject['non_preferred_terms'][$vn_i]))
                                    );
                                }
                            }

                            $t_item = DataMigrationUtils::getEntityID(
                                DataMigrationUtils::splitEntityName(
                                    trim(htmlentities($vs_preferred_term, ENT_NOQUOTES))
                                ),
                                $vn_type_id,
                                $pn_en_locale_id,
                            ...
                            if ($t_item = $t_list->addItem(
                                ...
                                if (!($t_item->addLabel(
                                    ...
                                    $o_log->logError(
                                        "Could not add preferred label to ULAN term [" . $va_subject['subject_id'] . "] " . $vs_preferred_term . ": " . join(
                                            "; ",
                                            $t_item->getErrors()
                                        )
                                    );
                                ...
                                if (is_array($va_subject['non_preferred_terms'])) {
                                    for ($vn_i = 0; $vn_i < sizeof($va_subject['non_preferred_terms']); $vn_i++) {
                                        ...
                                        if (!($t_item->addLabel(
                                            ...
                                            $o_log->logError(
                                                "Could not add non-preferred label to ULAN term [" . $va_subject['subject_id'] . "] " . $vs_np_label
                                            );
                                        }
                                    }
                                }
                                ...
                                $o_log->logError(
                                    "Could not import ULAN term [" . $va_subject['subject_id'] . "] " . $vs_preferred_term . ": " . join(
                                        "; ",
                                        $t_list->getErrors()
                                    )
                                );
                        ...
                        foreach ($va_mapping as $vs_dest => $vs_source) {
                            ...
                            if (sizeof($va_values)) {
                                ...
                                $t_item->update(
                                    array('dontCheckCircularReferences' => true, 'dontSetHierarchicalIndexing' => true)
                                );
                                if ($t_item->numErrors()) {
                                    $o_log->logError(
                                        "Could not update ULAN list item with content values: " . join(
                                            "; ",
                                            $t_item->getErrors()
                                        )
                                    );
    ...
    foreach ($va_parent_child_links as $vs_child_id => $vs_parent_id) {
        ...
        if (in_array($vs_parent_id, array('500000000', '500000001'))) {
            if (!$t_item->load($vn_child_item_id)) {
                $o_log->logError(
                    "Could not load item for {$vs_child_id} (was translated to item_id={$vn_child_item_id})"
                );
                continue;
            }
            ...
            if ($t_item->numErrors()) {
                $o_log->logError(
                    "Could not set parent_id for {$vs_child_id} to root): " . join('; ', $t_item->getErrors())
                );
                continue;
            }
        ...
        if ($t_item->numErrors()) {
            $o_log->logError(
                "Could not set parent_id for {$vs_child_id} (was translated to item_id={$vn_child_item_id}): " . join(
                    '; ',
                    $t_item->getErrors()
                )
            );
    ...
    if ($vn_list_item_relation_type_id_related > 0) {
        ...
        foreach ($va_item_item_links as $vs_left_id => $vs_right_id) {
            ...
            if ($t_link->numErrors()) {
                $o_log->logError(
                    "Could not set link between {$vs_left_id} (was translated to item_id={$vn_left_item_id}) and {$vs_right_id} (was translated to item_id={$vn_right_item_id}): " . join(
                        '; ',
                        $t_link->getErrors()
                    )
                );
        ...
        $o_log->logWarn(
            "Skipped import of term-term relationships because the ca_list_items_x_list_items 'related' relationship type is not defined for your installation"
        );
    }
