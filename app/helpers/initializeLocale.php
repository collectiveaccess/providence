...
function validateLocale( $ps_locale ) {
	$va_locale_paths = [];
	if ( file_exists( $vs_locale_path = __CA_THEME_DIR__ . '/locale/' . $ps_locale . '/messages.mo' ) ) {
		$va_locale_paths[] = $vs_locale_path;
	}
	if ( file_exists( $vs_locale_path = __CA_THEMES_DIR__ . '/default/locale/' . $ps_locale . '/messages.mo' ) ) {
		$va_locale_paths[] = $vs_locale_path;
	}
	if ( file_exists( $vs_locale_path = __CA_APP_DIR__ . '/locale/user/' . $ps_locale . '/messages.mo' ) ) {
		$va_locale_paths[] = $vs_locale_path;
	}
	if ( file_exists( $vs_locale_path = __CA_APP_DIR__ . '/locale/' . $ps_locale . '/messages.mo' ) ) {
		$va_locale_paths[] = $vs_locale_path;
	}
...
function initializeLocale( $ps_locale ) {
	...
	if ( ! headers_sent() ) {
		setcookie( 'CA_' . __CA_APP_NAME__ . '_ui_locale', $ps_locale, time() + 36000, $cookiepath );
	}
 ...
 *                           return = Set to "definite" to return an array of definite articles for the locale or
 *                           language; set to "indefinite" for a list of indefinite articles. [Default is null – return
