...
function caExtractMetadataWithGetID3($ps_filepath)
{
    ...
    //
    if ((isset($va_getid3_info["error"])) && (is_array($va_getid3_info["error"])) && (sizeof(
                $va_getid3_info["error"]
            ) == 1)) {
...
function caExtractVideoFileDurationWithMediaInfo($ps_filepath)
{
    ...
    $va_output = array();
    caExec(
        $ps_mediainfo_path . ' --Inform="Video;%Duration/String3%" ' . caEscapeShellArg($ps_filepath),
        $va_output,
        $vn_return
    );
