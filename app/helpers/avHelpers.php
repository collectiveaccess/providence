...
function caExtractMetadataWithGetID3( $ps_filepath ) {
	...
	//
	if ( ( isset( $va_getid3_info["error"] ) ) && ( is_array( $va_getid3_info["error"] ) )
	     && ( sizeof( $va_getid3_info["error"] ) == 1 )
	) {
 ...
 * @param string $ps_mediainfo_path optional path to MediaInfo binary. If omitted the path configured in
 *                                  external_applications.conf is used.
 *
...
function caExtractVideoFileDurationWithMediaInfo( $ps_filepath ) {
	...
	$va_output = array();
	caExec( $ps_mediainfo_path . ' --Inform="Video;%Duration/String3%" ' . caEscapeShellArg( $ps_filepath ), $va_output,
		$vn_return );
