...
function caExportItemAsPDF($po_request, $pt_subject, $ps_template, $ps_output_filename, $pa_options = null)
{
    ...
    if (substr($ps_template, 0, 5) === '_pdf_') {
        ...
        if ($vn_display_id && ($t_display->isLoaded()) && ($t_display->haveAccessToDisplay(
...
function caExportResult($po_request, $po_result, $ps_template, $ps_output_filename, $pa_options = null)
{
    ...
    if (!(bool)$o_config->get('disable_pdf_output') && substr($ps_template, 0, 5) === '_pdf_') {
        $va_template_info = caGetPrintTemplateDetails(
            caGetOption('printTemplateType', $pa_options, 'results'),
        ...
        if ($vn_display_id && ($t_display->haveAccessToDisplay(
                $po_request->getUserID(),
                __CA_BUNDLE_DISPLAY_READ_ACCESS__
            ))) {
            $o_view->setVar('display', $t_display);
        ...
        if (is_array($va_export_config) && is_array($va_export_config[$vs_table]) && is_array(
                $va_export_config[$vs_table][$ps_template]
            )) {
            ...
            while ($po_result->nextHit()) {
                ...
                foreach ($va_export_config[$vs_table][$ps_template]['columns'] as $vs_title => $va_settings) {
                    if (
                        ...
                        &&
                        preg_match(
                            "!ca_object_representations\.media\.([A-Za-z0-9_\-]+)!",
                            $va_settings['template'],
                            $va_matches
                        )
                    ) {
                        ...
                        if ($va_info['MIMETYPE'] == 'image/jpeg') { // don't try to insert anything non-jpeg into an Excel file
                            ...
                            $vn_width = floor(
                                intval($va_info['PROPERTIES']['width']) * $vn_ratio_pixels_to_excel_width
                            );
                            $vn_height = floor(
                                intval($va_info['PROPERTIES']['height']) * $vn_ratio_pixels_to_excel_height
                            );
                        ...
                        $o_sheet->setCellValue(
                            $vs_supercol . $vs_column . $vn_line,
                            html_entity_decode(
                                strip_tags(br2nl($vs_display_text)),
                        ...
                        if ($o_sheet->getColumnDimension($vs_supercol . $vs_column)->getWidth(
                            ) == -1) {  // don't overwrite existing settings
        ...
        case 'pptx':
            ...
            while ($po_result->nextHit()) {
                ...
                foreach ($va_export_config[$vs_table][$ps_template]['columns'] as $vs_title => $va_settings) {
                    if (
                                ...
                                $shape->setName($va_info['ORIGINAL_FILENAME'])
                                    ...
                                    ->setWidth(
                                        caConvertMeasurementToPoints(
                                            caGetOption('width', $va_settings, '100px'),
                                            array('dpi' => 96)
                                        )
                                    )
                                    ->setHeight(
                                        caConvertMeasurementToPoints(
                                            caGetOption('height', $va_settings, '100px'),
                                            array('dpi' => 96)
                                        )
                                    )
                                    ->setOffsetX(
                                        caConvertMeasurementToPoints(
                                            caGetOption('x', $va_settings, '100px'),
                                            array('dpi' => 96)
                                        )
                                    )
                                    ->setOffsetY(
                                        caConvertMeasurementToPoints(
                                            caGetOption('y', $va_settings, '100px'),
                                            array('dpi' => 96)
                                        )
                                    );
                    ...
                    } elseif ($vs_display_text = html_entity_decode(
                        strip_tags(br2nl($po_result->getWithTemplate($va_settings['template'])))
                    )) {
                        switch ($vs_align = caGetOption('align', $va_settings, 'center')) {
                            case 'center':
                                $vs_align = Alignment::HORIZONTAL_CENTER;
                                break;
                            case 'left':
                                $vs_align = Alignment::HORIZONTAL_LEFT;
                                break;
                            case 'right':
                            default:
                                $vs_align = Alignment::HORIZONTAL_RIGHT;
                                break;
                        }
                        ...
                        $textRun->getFont()->setBold((bool)caGetOption('bold', $va_settings, false))
                            ->setSize(
                                caConvertMeasurementToPoints(
                                    caGetOption('size', $va_settings, '36px'),
                                    array('dpi' => 96)
                                )
                            )
                            ->setColor(
                                new Color(caGetOption('color', $va_settings, 'cccccc'))
                            );
                    }
                }
            ...
            $o_writer = IOFactory::createWriter($ppt, 'PowerPoint2007');
            $o_writer->save('php://output');
            exit;
            break;
        case 'pdf':
...
function caExportViewAsPDF($po_view, $ps_template_identifier, $ps_output_filename, $pa_options = null)
{
    ...
    try {
        ...
        $va_page_size = PDFRenderer::getPageSize(
            caGetOption('pageSize', $pa_template_info, 'letter'),
            'mm',
...
function caExportContentAsPDF($ps_content, $pa_template_info, $ps_output_filename, $pa_options = null)
{
    ...
    try {
        ...
        $ps_output_filename = (($ps_output_filename) ? preg_replace(
                '![^A-Za-z0-9_\-\.]+!',
                '_',
                $ps_output_filename
            ) : 'export') . ".pdf";
...
function caGenerateDownloadFileName($ps_template, $pa_options = null)
{
    ...
    } elseif ((strpos($ps_template, "^") !== false) && is_array(
            $va_values = caGetOption('values', $pa_options, null)
        )) {
