...
function caExportItemAsPDF( $po_request, $pt_subject, $ps_template, $ps_output_filename, $pa_options = null ) {
	...
	if ( substr( $ps_template, 0, 5 ) === '_pdf_' ) {
		...
		if ( $vn_display_id && ( $t_display->isLoaded() )
		     && ( $t_display->haveAccessToDisplay( $po_request->getUserID(), __CA_BUNDLE_DISPLAY_READ_ACCESS__ ) )
		) {
	...
	caDoTemplateTagSubstitution( $o_view, $pt_subject, $va_template_info['path'],
		[ 'checkAccess' => $pa_access_values, 'render' => false ] );
...
function caExportResult( $po_request, $po_result, $ps_template, $ps_output_filename, $pa_options = null ) {
	...
	if ( ! (bool) $o_config->get( 'disable_pdf_output' ) && substr( $ps_template, 0, 5 ) === '_pdf_' ) {
		$va_template_info = caGetPrintTemplateDetails( caGetOption( 'printTemplateType', $pa_options, 'results' ),
			substr( $ps_template, 5 ) );
		$vs_type          = 'pdf';
		...
		if ( is_array( $va_export_config ) && is_array( $va_export_config[ $vs_table ] )
		     && is_array( $va_export_config[ $vs_table ][ $ps_template ] )
		) {
	...
	if ( ! ( $filename_stub = caGetOption( 'filename', $pa_options, null ) ) ) {
		if ( is_array( $va_template_info ) ) {
			...
			$filename_stub = caGetOption( 'filename', $va_export_config[ $vs_table ][ $ps_template ], 'export_results' )
			                 . '_' . date( "Y-m-d" );
		} else {
			...
			while ( $po_result->nextHit() ) {
				...
				foreach ( $va_export_config[ $vs_table ][ $ps_template ]['columns'] as $vs_title => $va_settings ) {
					...
					if (
						...
						&& preg_match( "!ca_object_representations\.media\.([A-Za-z0-9_\-]+)!",
							$va_settings['template'], $va_matches )
					) {
						...
						if ( $va_info['MIMETYPE']
							...
							if ( is_file(
								$vs_path = $po_result->getMediaPath( 'ca_object_representations.media', $vs_version ) )
							) {
							...
							$vn_width  = floor( intval( $va_info['PROPERTIES']['width'] )
							                    * $vn_ratio_pixels_to_excel_width );
							$vn_height = floor( intval( $va_info['PROPERTIES']['height'] )
							                    * $vn_ratio_pixels_to_excel_height );
						...
						$o_sheet->setCellValue( $vs_supercol . $vs_column . $vn_line,
						...
						if ( $o_sheet->getColumnDimension( $vs_supercol . $vs_column )->getWidth()
						     == - 1
			...
			while ( $po_result->nextHit() ) {
				...
				foreach ( $va_export_config[ $vs_table ][ $ps_template ]['columns'] as $vs_title => $va_settings ) {
					...
					if (
						...
						if ( $va_info['MIMETYPE']
							...
							if ( is_file(
								...
								$shape->setName( $va_info['ORIGINAL_FILENAME'] )
								      ...
								      ->setPath( $vs_path )
								      ->setWidth( caConvertMeasurementToPoints( caGetOption( 'width', $va_settings,
									      '100px' ), array( 'dpi' => 96 ) ) )
								      ->setHeight( caConvertMeasurementToPoints( caGetOption( 'height', $va_settings,
									      '100px' ), array( 'dpi' => 96 ) ) )
								      ->setOffsetX( caConvertMeasurementToPoints( caGetOption( 'x', $va_settings,
									      '100px' ), array( 'dpi' => 96 ) ) )
								      ->setOffsetY( caConvertMeasurementToPoints( caGetOption( 'y', $va_settings,
									      '100px' ), array( 'dpi' => 96 ) ) );
						...
						= html_entity_decode( strip_tags( br2nl( $po_result->getWithTemplate( $va_settings['template'] ) ) ) )
					) {
						...
						$textRun->getFont()->setBold( (bool) caGetOption( 'bold', $va_settings, false ) )
						        ->setSize( caConvertMeasurementToPoints( caGetOption( 'size', $va_settings, '36px' ),
							        array( 'dpi' => 96 ) ) )
						        ->setColor( new \PhpOffice\PhpPresentation\Style\Color( caGetOption( 'color',
							        $va_settings, 'cccccc' ) ) );
					}
 ...
 *                           returnFile = return file content instead of streaming to browser -> passed through to
 *                           caExportContentAsPDF
 *
 * @return bool
 *
 * @throws ApplicationException
...
function caExportViewAsPDF( $po_view, $ps_template_identifier, $ps_output_filename, $pa_options = null ) {
	...
	try {
		...
		$va_page_size   = PDFRenderer::getPageSize( caGetOption( 'pageSize', $pa_template_info, 'letter' ), 'mm',
			caGetOption( 'pageOrientation', $pa_template_info, 'portrait' ) );
...
function caExportContentAsPDF( $ps_content, $pa_template_info, $ps_output_filename, $pa_options = null ) {
	...
	try {
		...
		$o_pdf->setPage( caGetOption( 'pageSize', $pa_template_info, 'letter' ),
			caGetOption( 'pageOrientation', $pa_template_info, 'portrait' ),
			caGetOption( 'marginTop', $pa_template_info, '0mm' ),
			caGetOption( 'marginRight', $pa_template_info, '0mm' ),
			caGetOption( 'marginBottom', $pa_template_info, '0mm' ),
			caGetOption( 'marginLeft', $pa_template_info, '0mm' ) );

		$ps_output_filename = ( ( $ps_output_filename ) ? preg_replace( '![^A-Za-z0-9_\-\.]+!', '_',
				$ps_output_filename ) : 'export' ) . ".pdf";
...
# ----------------------------------------
/**
 * Generate name for downloadable file. Can take a display template evaluated relative to a provided model instance, a
 * template evaluated with an array of tag values or static text. (Note: for compatibility reasons if the static text
 * "label" is passed and a model instance is passed in the 't_subject' option then the preferred label of the instance
 * will be returned).
 *
 * The returned value will have all non-alphanumeric characters replaced with underscores, ready for use as a download
 * file name.
 *
 * @param string $ps_template A display template or static text used to generate the file name.
 * @param array  $pa_options  Options include:
 *                            t_subject = a model instance to evaluate the filename template relative to. [Default is
 *                            null] values = an array of values, where keys are tag names in the filename template.
 *                            [Default is null]
 *
...
function caGenerateDownloadFileName( $ps_template, $pa_options = null ) {
	...
	if ( ( strpos( $ps_template, "^" ) !== false ) && ( $pt_subject ) ) {
		return caProcessTemplateForIDs( $ps_template, $pt_subject->tableName(), [ $pt_subject->getPrimaryKey() ],
			$pa_options );
	} elseif ( ( strpos( $ps_template, "^" ) !== false )
	           && is_array( $va_values = caGetOption( 'values', $pa_options, null ) )
	) {
