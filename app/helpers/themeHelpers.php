    ...
    if ($o_config->get('allowThemeInheritance')) {
        ...
        while ($vs_inherit_from_theme = trim(trim($o_config->get(['inheritFrom', 'inherit_from'])), "/")) {
            ...
            if (file_exists(__CA_THEMES_DIR__ . "/{$vs_inherit_from_theme}/{$vs_file_path}")) {
                return caHTMLImage(
                    __CA_THEMES_URL__ . "/{$vs_inherit_from_theme}/{$vs_file_path}",
            ...
            $o_config = Configuration::load(
                __CA_THEMES_DIR__ . "/{$vs_inherit_from_theme}/conf/app.conf",
                false,
                false,
                true
            );
        ...
        foreach ($va_versions as $vs_version) {
            ...
            if ($vb_return_tags) {
                if (sizeof($va_versions) == 1) {
                    ...
                    $va_media_tags['tags'][$vs_version] = $qr_res->getMediaTag(
            ...
            if ($vb_return_info) {
                if (sizeof($va_versions) == 1) {
                    ...
                    $va_media_tags['info'][$vs_version] = $qr_res->getMediaInfo(
                        'ca_object_representations.media',
                        $vs_version
                    );
                }
            ...
            if ($vb_return_urls) {
                if (sizeof($va_versions) == 1) {
                    ...
                    $va_media_tags['urls'][$vs_version] = $qr_res->getMediaUrl(
                        'ca_object_representations.media',
                        $vs_version
                    );
                }
...
function caObjectRepresentationThumbnails($po_request, $pn_representation_id, $pt_object, $pa_options)
{
    ...
    $va_reps = $pt_object->getRepresentations(
        array($ps_version),
        null,
        array(
            "checkAccess" => caGetUserAccessValues($po_request),
            'primaryOnly' => $pb_primary_only
        )
    );
    if (sizeof($va_reps) < 2) {
        return null;
    ...
    foreach ($va_reps as $vn_rep_id => $va_rep) {
        ...
        switch ($ps_link_to) {
            # -------------------------------
            case "viewer":
                $va_links[$vn_rep_id] = "<a href='#' onclick='caMediaPanel.showPanel(\"" . caNavUrl(
                        $po_request,
                        '',
                        'Detail',
                        'GetMediaOverlay',
                    ...
                    ) . "\"); return false;' " . (($vs_class) ? "class='" . $vs_class . "'" : "") . ">" . $vs_thumb . "</a>\n";
                break;
            # -------------------------------
            case "carousel":
            ...
            default:
            case "detail":
                $va_links[$vn_rep_id] = caDetailLink(
                    $po_request,
                    $vs_thumb,
                    $vs_class,
                    $pt_object->tableName(),
                    $pt_object->getPrimaryKey(),
                    array("representation_id" => $vn_rep_id)
                );
                break;
            # -------------------------------
        }
...
function caDetailItemComments($po_request, $pn_item_id, $t_item, $va_comments, $va_tags)
{
    ...
    if ($po_request->isLoggedIn()) {
        $vs_tmp .= "<button type='button' class='btn btn-default' onclick='caMediaPanel.showPanel(\"" . caNavUrl(
                $po_request,
                '',
                'Detail',
                'CommentForm',
        ...
        $vs_tmp .= "<button type='button' class='btn btn-default' onclick='caMediaPanel.showPanel(\"" . caNavUrl(
                ...
                'LoginForm',
                array()
            ) . "\"); return false;' >" . _t("Login/register to comment on this object") . "</button>";
    }
    return $vs_tmp;
}
...
function caLightboxSetListItem($po_request, $t_set, $va_check_access = array(), $pa_options = array())
{
    ...
    $va_set_items = caExtractValuesByUserLocale(
        $t_set->getItems(
            array(
    ...
    $vs_set_display .= "<H2>" . caNavLink(
            $po_request,
            $t_set->getLabelForDisplay(),
            "",
            "",
            "Lightbox",
            "setDetail",
            array("set_id" => $vn_set_id),
            array('id' => "lbSetName{$vn_set_id}")
        ) . "</H2>";
    ...
    if (sizeof($va_set_items)) {
        ...
        foreach ($va_set_items as $va_set_item) {
            ...
            if ($vn_i == 1) {
                ...
                if ($va_set_item["representation_tag_" . $vs_large_icon]) {
                    $vs_primary_image_block .= "<div class='col-sm-6'><div class='lbSetImg'>" . caNavLink(
                            $po_request,
                            $va_set_item["representation_tag_" . $vs_large_icon],
                            "",
                            "",
                            "Lightbox",
                            "setDetail",
                            array("set_id" => $vn_set_id)
                        ) . "</div><!-- end lbSetImg --></div>\n";
                } else {
                    $vs_primary_image_block .= "<div class='col-sm-6'><div class='lbSetImg'>" . caNavLink(
                            $po_request,
                            "<div class='lbSetImgPlaceholder'>" . $vs_placeholder . "</div><!-- end lbSetImgPlaceholder -->",
                            "",
                            "",
                            "Lightbox",
                            "setDetail",
                ...
                if ($va_set_item["representation_tag_icon"]) {
                    $vs_secondary_image_block .= "<div class='col-xs-3 col-sm-6 lbSetThumbCols'><div class='lbSetThumb'>" . caNavLink(
                            $po_request,
                            $va_set_item["representation_tag_icon"],
                            "",
                            "",
                            "Lightbox",
                            "setDetail",
                    ...
                    $vs_secondary_image_block .= "<div class='col-xs-3 col-sm-6 lbSetThumbCols'>" . caNavLink(
                            $po_request,
                            "<div class='lbSetThumbPlaceholder'>" . caGetThemeGraphic(
                                $po_request,
                                'spacer.png',
                                array("alt" => "spacer")
                            ) . $vs_placeholder . "</div><!-- end lbSetThumbPlaceholder -->",
                            "",
                            "",
                            "Lightbox",
                            "setDetail",
        ...
        $vs_primary_image_block .= "<div class='col-sm-6'><div class='lbSetImg'><div class='lbSetImgPlaceholder'>" . _t(
                "this %1 contains no items",
                $vs_lightbox_displayname
            ) . "</div><!-- end lbSetImgPlaceholder --></div><!-- end lbSetImg --></div>\n";
        $i = 1;
        while ($vn_i < 4) {
            $vs_secondary_image_block .= "<div class='col-xs-3 col-sm-6 lbSetThumbCols'><div class='lbSetThumbPlaceholder'>" . caGetThemeGraphic(
                    $po_request,
                    'spacer.png'
    ...
    $vs_set_display .= "<div class='lbSetExpandedInfo' id='lbExpandedInfo{$vn_set_id}'>\n<hr><div>created by: " . trim(
            $t_set->get("ca_users.fname") . " " . $t_set->get("ca_users.lname")
        ) . "</div>\n";
    $vs_set_display .= "<div>" . _t(
            "Items: %1",
            $t_set->getItemCount(
                array(
                    "user_id" => $po_request->user->get("user_id"),
    ...
    if ($vb_write_access) {
        $vs_set_display .= "<div class='pull-right'><a href='#' data-set_id=\"" . (int)$t_set->get(
                'set_id'
            ) . "\" data-set_name=\"" . addslashes(
    ...
    $vs_set_display .= "<div><a href='#' onclick='jQuery(\"#comment{$vn_set_id}\").load(\"" . caNavUrl(
            $po_request,
            '',
            'Lightbox',
            'AjaxListComments',
        ...
        ) . "\", function(){jQuery(\"#lbSetThumbRow{$vn_set_id}\").hide(); jQuery(\"#comment{$vn_set_id}\").show();}); return false;' title='" . _t(
            "Comments"
        ) . "'><span class='glyphicon glyphicon-comment'></span> <small>" . $t_set->getNumComments() . "</small></a>";
    if ($vb_write_access) {
        $vs_set_display .= "&nbsp;&nbsp;&nbsp;<a href='#' onclick='caMediaPanel.showPanel(\"" . caNavUrl(
                $po_request,
                '',
                'Lightbox',
                'setForm',
                array("set_id" => $vn_set_id)
            ) . "\"); return false;' title='" . _t(
...
function caClassroomSetListItem($po_request, $t_set, $va_check_access = array(), $pa_options = array())
{
    ...
    $vs_set_display .= "<H5 id='crSetName" . $t_set->get("set_id") . "'>" . caNavLink(
            $po_request,
            $t_set->getLabelForDisplay(),
            "",
            "",
    ...
    if (!$t_set->get("parent_id")) {
        ...
        if (sizeof($va_set_items)) {
            ...
            foreach ($va_set_items as $va_set_item) {
                ...
                if ($va_set_item["representation_tag_" . $vs_large_icon]) {
                    $vs_image_block .= "<div class='col-xs-4 col-sm-2 crSetImg'>" . caNavLink(
                            $po_request,
                            $va_set_item["representation_tag_" . $vs_large_icon],
                            "",
                            "",
                    ...
                    $vs_image_block .= "<div class='col-xs-4 col-sm-2 crSetImg'>" . caNavLink(
                            $po_request,
                            "<div class='crSetImgPlaceholder'>" . $vs_placeholder . "</div><!-- end lbSetImgPlaceholder -->",
                            "",
                            "",
                            "Classroom",
                            "setDetail",
    ...
    if ($vb_write_access) {
        ...
        $vs_set_display .= "<a href='#' title='" . _t("Delete") . "' data-set_id=\"" . (int)$t_set->get(
                'set_id'
            ) . "\" data-set_name=\"" . addslashes(
                $t_set->get('ca_sets.preferred_labels.name')
            ) . "\" data-toggle='modal' data-target='#confirm-delete'><span class='glyphicon glyphicon-trash'></span></a></div>\n";
...
function caClassroomSetResponseItem($po_request, $t_set, $va_check_access = array(), $pa_options = array())
{
    ...
    $vs_set_display .= "<small>\n";
    $vs_set_display .= _t("Created by:") . trim(
            $t_set->get("ca_users.fname") . " " . $t_set->get("ca_users.lname")
        ) . "<br/>\n";
    $vs_set_display .= _t("Items:") . $t_set->getItemCount(
            array("user_id" => $po_request->user->get("user_id"), "checkAccess" => $va_check_access)
        ) . "&nbsp;&nbsp;&nbsp;\n";
...
function caGetDetailForType($pm_table, $pm_type = null, $pa_options = null)
{
    ...
    foreach ($va_detail_types as $vs_code => $va_info) {
        if ($va_info['table'] == $vs_table) {
            ...
            if (is_null($pm_type) || !is_array($va_info['restrictToTypes']) || (sizeof(
                        $va_info['restrictToTypes']
                    ) == 0) || in_array($vs_type, $va_info['restrictToTypes'])) {
...
function caGetDisplayImagesForAuthorityItems($pm_table, $pa_ids, $pa_options = null)
{
    ...
    if ((!caGetOption("useRelatedObjectRepresentations", $pa_options, array())) && method_exists(
            $t_instance,
            "getPrimaryMediaForIDs"
        )) {
        // Use directly related media if defined
        $va_media = $t_instance->getPrimaryMediaForIDs(
            $pa_ids,
            array($vs_version = caGetOption('version', $pa_options, 'icon')),
            $pa_options
        );
    ...
    if ($pa_options['checkAccess']) {
        $vs_access_wheres = " AND ca_objects.access IN (" . join(
                ",",
                $pa_access_values
    ...
    while ($qr_res->nextRow()) {
        switch ($ps_return) {
            case 'urls':
                $va_res[$qr_res->get($vs_pk)] = $qr_res->getMediaUrl(
                    "media",
                    caGetOption('version', $pa_options, 'icon')
                );
                break;
            case 'paths':
                $va_res[$qr_res->get($vs_pk)] = $qr_res->getMediaPath(
                    "media",
                    caGetOption('version', $pa_options, 'icon')
                );
                break;
            ...
            default:
                ...
                $va_res[$qr_res->get($vs_pk)] = $qr_res->getMediaTag(
                    "media",
                    caGetOption('version', $pa_options, 'icon'),
                    ['alt' => $alt_text]
                );
...
function caGetGallerySetsAsList($po_request, $vs_class, $pa_options = null)
{
    ...
    if ($vn_gallery_set_type_id) {
        ...
        $va_sets = caExtractValuesByUserLocale(
            $t_set->getSets(
                array('table' => 'ca_objects', 'checkAccess' => $va_access_values, 'setType' => $vn_gallery_set_type_id)
            )
        );
...
{
    if ($po_request->isLoggedIn() && !$po_request->config->get("disable_lightbox") && ($po_request->config->get(
                "disable_classroom"
            ) || !in_array(
                $po_request->user->getPreference('user_profile_classroom_role'),
...
{
    if ($po_request->isLoggedIn() && !$po_request->config->get("disable_classroom") && in_array(
            $po_request->user->getPreference('user_profile_classroom_role'),
            array('STUDENT', 'EDUCATOR')
        )) {
        return true;
...
function caSetAdvancedSearchFormInView($po_view, $ps_function, $ps_view, $pa_options = null)
{
    ...
    foreach ($pa_tags as $vs_tag) {
        ...
        if (($vs_default_value = caGetOption('default', $va_opts, null)) || ($vs_default_value = caGetOption(
                $vs_tag_proc,
                $va_default_form_values,
                null
            ))) {
        ...
        switch (strtolower($vs_tag_proc)) {
            case 'submit':
                $po_view->setVar(
                    $vs_tag,
                    "<a href='#' class='caAdvancedSearchFormSubmit'>" . ((isset($va_opts['label']) && $va_opts['label']) ? $va_opts['label'] : _t(
                        'Submit'
                    )) . "</a>"
                );
            ...
            case 'reset':
                $po_view->setVar(
                    $vs_tag,
                    "<a href='#' class='caAdvancedSearchFormReset'>" . ((isset($va_opts['label']) && $va_opts['label']) ? $va_opts['label'] : _t(
                        'Reset'
                    )) . "</a>"
                );
                $vb_submit_or_reset_set = true;
            ...
            default:
                if (preg_match("!^(.*):label$!", $vs_tag_proc, $va_matches)) {
                    ...
                    if ($t_rel = $pt_subject->getRelationshipInstance($va_tmp[0])) {
                        $vs_select = $t_rel->getRelationshipTypesAsHTMLSelect(
                            $va_tmp[0],
                            null,
                            null,
                            array_merge(
                                array('class' => 'caAdvancedSearchRelationshipTypes'),
                                $va_opts,
                                array('name' => $vs_tag_proc . '[]')
                            ),
                            $va_opts
                        );
                    }
                    ...
                    if (isset($va_opts['restrictToTypes']) && $va_opts['restrictToTypes'] && !is_array(
                            $va_opts['restrictToTypes']
                        )) {
                        $va_opts['restrictToTypes'] = preg_split("![,;]+!", $va_opts['restrictToTypes']);
                    }

                    // Relationship type restrictions
                    if (isset($va_opts['restrictToRelationshipTypes']) && $va_opts['restrictToRelationshipTypes'] && !is_array(
                    ...
                    if ($vs_tag_val = $pt_subject->htmlFormElementForSearch($po_request, $vs_tag_proc, $va_opts)) {
                        ...
                        $vs_tag_val .= caHTMLHiddenInput(
                            "{$vs_tag_proc}{$vs_rel_types}_label",
                            array(
                                'value' => isset($va_opts['label']) ? str_replace(
                                    "_",
                                    " ",
                                    urldecode($va_opts['label'])
                                ) : $pt_subject->getDisplayLabel($vs_tag_proc)
                            )
                    ...
                    if ((($t_element = ca_metadata_elements::getInstance($va_tmp[1])) && ($t_element->get(
                                'datatype'
                            ) == 0))) {
    ...
    if ($vb_submit_or_reset_set) {
        $vs_script = "],textarea').val('');
			...
			jQuery(document).ready(function() {
				var f, defaultValues =; " . json_encode($va_default_form_values) . ", defaultBooleans =; " . json_encode(
                $va_default_form_booleans
            ) . ";
    ...
    $po_view->setVar(
        "/form",
        $vs_script . caHTMLHiddenInput(
            "_advancedFormName",
            array("value" => $ps_function)
        ) . caHTMLHiddenInput(
            "_formElements",
            array("value" => join('|', $va_form_elements))
        ) . caHTMLHiddenInput("_advanced", array("value" => 1)) . "</form>"
    );
...
function caGetAdvancedSearchFormAutocompleteJS($po_request, $ps_field, $pt_instance, $pa_options = null)
{
    ...
    $vs_buf = caHTMLTextInput(
        "{$vs_field_proc}_autocomplete{$index}",
        array(
...
{
    if (!($ps_view = caGetOption('view', $pa_options, null)) && ($o_request = caGetOption(
            'request',
            $pa_options,
            null
        ))) {
...
function caGetComparisonList($po_request, $ps_table, $pa_options = null)
{
    ...
    $va_display_list = caProcessTemplateForIDs(
        caGetOption('title_template', $va_compare_config, "^{$ps_table}.preferred_labels"),
        $ps_table,
        $va_comparison_list,
        ['returnAsArray' => true]
    );
...
function caGetCollectionLevelSummary($po_request, $va_collection_ids, $vn_level)
{
    ...
    if ($o_collections_config->get("export_max_levels") && ($vn_level > $o_collections_config->get(
                "export_max_levels"
            ))) {
        return null;
    }
    ...
    if ($va_exclude_collection_type_idnos = $o_collections_config->get("export_exclude_collection_types")) {
        # --- convert to type_ids
        $va_exclude_collection_type_ids = $t_list->getItemIDsFromList(
            "collection_types",
            $va_exclude_collection_type_idnos,
            array("dontIncludeSubItems" => true)
        );
    }
    $vs_output = "";
    ...
    if ($qr_collections->numHits()) {
        while ($qr_collections->nextHit()) {
            if ($va_exclude_collection_type_ids && is_array($va_exclude_collection_type_ids) && (in_array(
                    $qr_collections->get("ca_collections.type_id"),
                    $va_exclude_collection_type_ids
                ))) {
                continue;
            }

            $vs_icon = "";
            if (is_array($va_collection_type_icons) && $va_collection_type_icons[$qr_collections->get(
                    "ca_collections.type_id"
                )]) {
                $vs_icon = $va_collection_type_icons[$qr_collections->get("ca_collections.type_id")];
            }
            # --- related objects?
            $va_object_ids = $qr_collections->get(
                "ca_objects.object_id",
                array("returnAsArray" => true, 'checkAccess' => $va_access_values)
            );
            $vn_rel_object_count = sizeof($va_object_ids);
            $va_child_ids = $qr_collections->get(
                "ca_collections.children.collection_id",
                array(
                    "returnAsArray" => true,
            ...
            if (!$vb_dont_show_top_level_description) {
                $vs_desc = "";
                if ($vs_sub_collection_desc_template && ($vs_desc = $qr_collections->getWithTemplate(
                        $vs_sub_collection_desc_template
                    ))) {
