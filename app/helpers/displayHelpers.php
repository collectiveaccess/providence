...
function caEditorInspector($po_view, $pa_options = null)
{
    ...
    if (($po_view->request->getAction() === 'Delete') && ($po_view->request->getParameter('confirm', pInteger))) {
        ...
        if ($vn_item_id) {
            ...
            if ($t_item->hasField('is_deaccessioned') && $t_item->get('is_deaccessioned') && ($t_item->get(
                ...
                if ($po_view->request->user->canDoAction('can_see_current_location_in_inspector_ca_objects')) {
                    if (($t_ui && method_exists($t_item, "getObjectHistory")) && (is_array(
                                $va_placements = $t_ui->getPlacementsForBundle('ca_objects_history')
                            ) && (sizeof($va_placements) > 0))) {
                        //
                        ...
                        $va_bundle_settings = caConvertCurrentLocationCriteriaToBundleSettings(
                        ); //$va_placement['settings'];
                        if (is_array(
                                $va_history = $t_item->getObjectHistory(
                                    $va_bundle_settings,
                                    array('limit' => 1, 'currentOnly' => true)
                                )
                            ) && (sizeof($va_history) > 0)) {
                            $va_current_location = array_shift(array_shift($va_history));

                            if (!($vs_inspector_current_location_label = $po_view->request->config->get(
                                "ca_objects_inspector_current_location_label"
                            ))) {
                        ...
                        if ($vs_current_location = $t_item->getLastLocationForDisplay(
                            "<ifdef code='ca_storage_locations.parent.preferred_labels'>^ca_storage_locations.parent.preferred_labels ➜ </ifdef>^ca_storage_locations.preferred_labels.name"
                        )) {
                            $vs_buf .= "<br/><div class='inspectorCurrentLocation'>" . _t(
                                    'Location: %1',
                                    $vs_current_location
                                ) . "</div>\n";
                            $vs_full_location_hierarchy = $t_item->getLastLocationForDisplay(
		...
		";
            ...
            if ($vn_item_id) {
                ...
                if (in_array(
                    ...
                    $vs_watch = "<div class='watchThis'><div><a href='#' title='" . _t(
		...
		function caToggleItemWatch() {
			...
			jQuery.getJSON(url, {}, function(data, status) {
                ...
                if ($t_item->getPrimaryKey() && $po_view->request->config->get(
                    ...
                    if ($vb_show_add_child_control) {
                        ...
                        if ($vs_type_list) {
                            $vs_buf .= "<div id='inspectorCreateChild'><div id='inspectorCreateChildButton'><a href='#' onclick='caCreateChildPanel.showPanel(); return false;'>" . caNavIcon(
            ...
            if ($vn_num_objects > 0) {
                $vs_buf .= "<div id='inspectorLotMediaDownloadButton'>" . caNavLink(
                        $po_view->request,
                        caNavIcon(
            ...
            if (($vs_table_name == 'ca_sets') && (sizeof($t_item->getItemRowIDs()) > 0)) {
                $vs_buf .= "<div id='inspectorSetMediaDownloadButton'>" . caNavLink(
                        $po_view->request,
                        caNavIcon(
                            __CA_NAV_ICON_DOWNLOAD__,
                            '20px'
            ...
            if ($va_creation['timestamp'] || $va_last_change['timestamp']) {
                ...
                if (method_exists($t_item, 'getMetadataDictionaryRuleViolations') && is_array(
                    ...
                    ) && (($vn_num_violations = (sizeof($va_violations))) > 0)) {
                    ...
                    $vs_more_info .= "<div id='caInspectorViolationsList'>" . ($vs_num_violations_display = "<img src='" . $po_view->request->getThemeUrlPath(
                            ) . "/graphics/icons/warning_small.gif' border='0'/> " . (($vn_num_violations > 1) ? _t(
                                '%1 problems require attention',
                                $vn_num_violations
                            ) : _t('%1 problem requires attention', $vn_num_violations))) . "</div>\n";
                    TooltipManager::add(
            ...
            if ($vs_more_info) {
                $vs_buf .= "<div class='button info'><div><a href='#' id='inspectorMoreInfo'>" . caNavIcon(
	...
	jQuery(document).ready(function() {
        ...
        if ($po_view->request->user->canDoAction('can_export_' . $vs_table_name) && $t_item->getPrimaryKey() && (sizeof(
                    ca_data_exporters::getExporters($t_item->tableNum())
                ) > 0)) {
            $vs_buf .= '<div style="border-top: 1px solid #aaaaaa; margin-top: 5px; font-size: 10px; text-align: right;" id="caExportItemButton">';
                ...
                try {
                    ...
                    switch ($vs_units = strtolower($va_tmp[1])) {
                        ...
                        case 'metric':
                            ...
                            } elseif ($vn_value_in_cm <= $o_dimensions_config->get(
        ...
        foreach ($pa_options['relatedItems'] as $vn_relation_id => $va_relation) {
            ...
            if ($vs_template) {
                $va_items[$va_relation[$vs_rel_pk]]['_display'] = caProcessTemplateForIDs(
                    $vs_template,
                    $pt_rel->tableName(),
                    array($va_relation['relation_id'] ? $va_relation['relation_id'] : $va_relation[$vs_pk]),
                    array(
                        'returnAsArray' => false,
                        'returnAsLink' => false,
                        'delimiter' => caGetOption(
...
function caEditorBundleMetadataDictionary($po_request, $ps_id_prefix, $pa_settings)
{
    ...
    if (!($vs_definition = trim(caGetOption($g_ui_locale, $pa_settings['definition'], null)))) {
        return '';
    }
...
function caEditorBundleSortControls($po_request, $ps_id_prefix, $ps_table, $pa_options = null)
{
    ...
    if (!is_array(
            $va_sort_fields = caGetAvailableSortFields(
                $ps_table,
                null,
                array_merge(['request' => $po_request], $pa_options)
            )
...
function caGetBlankLabelText()
{
    ...
    if ($g_blank_label_text) {
        return $g_blank_label_text;
    }
