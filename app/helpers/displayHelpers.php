...
function caGetUserLocaleRules($ps_item_locale = null, $pa_preferred_locales = null)
{
    ...
    if (is_array($va_default_locales)) {
        foreach ($va_default_locales as $vs_fallback_locale) {
            $va_similar_locales = array_merge(
                $va_similar_locales,
    ...
    $va_fallback_locales = array_filter(
        $va_fallback_locales,
...
function caExtractValuesByUserLocale(
    $pa_values,
    $ps_item_locale = null,
    $pa_preferred_locales = null,
    $pa_options = null
) {
    return caExtractValuesByLocale(
        caGetUserLocaleRules($ps_item_locale, $pa_preferred_locales),
        $pa_values,
        $pa_options
    );
}
...
function caExtractValuesByUserLocaleFromHierarchyAncestorList(
    $pa_list,
    $ps_primary_key_name,
    $ps_label_display_field,
    $ps_use_if_no_label_field,
    $ps_default_text = '???'
) {
    if (!is_array($pa_list)) {
        return array();
...
function caDeleteRemapper($po_request, $t_instance)
{
    ...
    switch ($vs_instance_table) {
        case 'ca_relationship_types':
            ...
            if ($vn_rel_count == 1) {
                $va_reference_to_buf[] = _t(
                        "Type is used by %1 %2",
                        $vn_rel_count,
                        $t_rel_instance->getProperty('NAME_PLURAL')
                    ) . "<br>\n";
            } else {
                $va_reference_to_buf[] = _t(
                        "Type is used by %1 %2",
        ...
        default:
            ...
            foreach ($va_tables as $vs_table) {
                ...
                if ($vn_c == 1) {
                    $va_reference_to_buf[] = _t(
                            "Has %1 relationship to %2",
                            $vn_c,
                            caGetTableDisplayName($vs_table, true)
                        ) . "<br>\n";
                } else {
                    $va_reference_to_buf[] = _t(
                            "Has %1 relationships to %2",
                            $vn_c,
                            caGetTableDisplayName($vs_table, true)
                        ) . "<br>\n";
                }
            ...
            if (is_array($va_references_from = $t_instance->getAuthorityElementList()) && sizeof($va_references_from)) {
                foreach ($va_references_from as $va_ref) {
                    ...
                    $va_reference_from_buf[] = _t(
                        ($va_ref['count'] == 1) ? "%1 reference in %2" : "%1 references in %2",
                        $va_ref['count'],
                        $t_element->getLabelForDisplay()
                    );
            ...
            if ($t_instance->isHierarchical() && is_array(
                    $va_children = call_user_func(
                        $t_instance->tableName() . "::getHierarchyChildrenForIDs",
    ...
    if (!in_array(
        ...
        if (!in_array(
            ...
            if (!in_array(
                $vs_default = strtolower($o_config->get('delete_reference_handling_default')),
                ['transfer', 'remap', 'remove', 'delete']
            )) {
                $vs_default = null;
            }
    ...
    if (sizeof($va_reference_to_buf)) {
        ...
        if ($vn_reference_to_count == 1) {
            $vs_output .= "<h3 id='caReferenceHandlingToCount'>" . _t(
                    'This %1 is referenced %2 time',
                    $vs_typename,
                    $vn_reference_to_count
                ) . ". " . _t('When deleting this %1:', $vs_typename) . "</h3>\n";
        } else {
            $vs_output .= "<h3 id='caReferenceHandlingToCount'>" . _t(
                    'This %1 is referenced %2 times',
                    $vs_typename,
                    $vn_reference_to_count
                ) . ". " . _t('When deleting this %1:', $vs_typename) . "</h3>\n";
        }
        ...
        $vs_output .= caHTMLRadioButtonInput('caReferenceHandlingTo', $va_delete_opts) . ' ' . _t(
                'remove all references'
            ) . "<br/>\n";

        if ($vs_instance_table === 'ca_storage_locations') {
            AssetLoadManager::register('hierBrowser');
            $vs_output .= caHTMLRadioButtonInput('caReferenceHandlingTo', $va_remap_opts) . ' ' . _t(
                    'transfer references to'
                ) . ' ';
            ...
            $vs_output .= caHTMLRadioButtonInput('caReferenceHandlingTo', $va_remap_opts) . ' ' . _t(
                    'transfer references to'
                ) . ' ' . caHTMLTextInput('caReferenceHandlingToRemapTo', $va_remap_lookup_opts);
        }
        $vs_output .= "<a href='#' class='button' onclick='jQuery(\"#caReferenceHandlingToRemapToID\").val(\"\"); jQuery(\"#caReferenceHandlingToRemapTo\").val(\"\"); jQuery(\"#caReferenceHandlingToClear\").css(\"display\", \"none\"); return false;' style='display: none;' id='caReferenceHandlingToClear'>" . _t(
                'Clear'
            ) . '</a>';
        $vs_output .= caHTMLHiddenInput(
            'caReferenceHandlingToRemapToID',
            array('value' => '', 'id' => 'caReferenceHandlingToRemapToID')
        );

        if ($vn_child_count > 0) {
            $vs_output .= '<p class="formLabelWarning" id="caChildDeletionWarning"><i class="caIcon fa fa-info-circle fa-1x"></i> ' . _t(
                    'Child records will be deleted'
                ) . "</p>\n";
        }

        $vs_output .= "<script type='text/javascript'>";

        $va_service_info = caJSONLookupServiceUrl(
            $po_request,
    ...
    if (sizeof($va_reference_from_buf)) {
        ...
        if ($vn_reference_from_count == 1) {
            $vs_output .= "<h3 id='caReferenceHandlingFromCount'>" . _t(
                    'This %1 references %2 other item in metadata',
                    $vs_typename,
                    $vn_reference_from_count
                ) . ". " . _t('When deleting this %1:', $vs_typename) . "</h3>\n";
        } else {
            $vs_output .= "<h3 id='caReferenceHandlingFromCount'>" . _t(
                    'This %1 references %2 other items in metadata',
                    $vs_typename,
                    $vn_reference_from_count
                ) . ". " . _t('When deleting this %1:', $vs_typename) . "</h3>\n";
        }
        ...
        $vs_output .= caHTMLRadioButtonInput('caReferenceHandlingFrom', $va_delete_opts) . ' ' . _t(
                'remove these references'
            ) . "<br/>\n";
        $vs_output .= caHTMLRadioButtonInput('caReferenceHandlingFrom', $va_remap_opts) . ' ' . _t(
                'transfer these references and accompanying metadata to'
            ) . ' ' . caHTMLTextInput('caReferenceHandlingToRemapFrom', $va_remap_lookup_opts);
        $vs_output .= "<a href='#' class='button' onclick='jQuery(\"#caReferenceHandlingToRemapFromID\").val(\"\"); jQuery(\"#caReferenceHandlingToRemapFrom\").val(\"\"); jQuery(\"#caReferenceHandlingClear\").css(\"display\", \"none\"); return false;' style='display: none;' id='caReferenceHandlingClear'>" . _t(
                'Clear'
            ) . '</a>';
        $vs_output .= caHTMLHiddenInput(
        ...
        TooltipManager::add(
            '#caReferenceHandlingFromCount',
            "<h2>" . _t(
                'References by this %1',
                $t_instance->getProperty('NAME_SINGULAR')
...
function caGetMediaInfoForDisplay($pm_media, $ps_version)
{
    ...
    if (isset($va_media_info[$ps_version]['PROPERTIES']['resolution']) && is_array(
            $va_resolution = $va_media_info[$ps_version]['PROPERTIES']['resolution']
        )) {
...
function caFormatMediaMetadata($pa_metadata)
{
    ...
    if (is_array($pa_metadata) && sizeof($pa_metadata)) {
        foreach ($pa_metadata as $vs_metadata_type => $va_metadata_data) {
            if (isset($va_metadata_data) && is_array($va_metadata_data)) {
                $vs_buf .= "<tr><th>" . preg_replace(
                        '!^METADATA_!',
                        '',
                        $vs_metadata_type
                    ) . "</th><th colspan='2'><!-- empty --></th></tr>\n";
                foreach ($va_metadata_data as $vs_key => $vm_value) {
                    $vs_buf .= "<tr valign='top'><td><!-- empty --></td><td>{$vs_key}</td><td>" . _caFormatMediaMetadataArray(
                            $vm_value,
                            0,
                            $vs_key
                        ) . "</td></tr>\n";
    ...
    if (!$vn_metadata_rows) {
        $vs_buf .= "<tr valign='top'><td colspan='3'>" . _t(
                'No embedded metadata was extracted from the media'
            ) . "</td></tr>\n";
    }
    $vs_buf .= "</table>\n";
...
function _caFormatMediaMetadataArray($pa_array, $pn_level = 0, $ps_key = null)
{
    ...
    foreach ($pa_array as $vs_key => $vs_val) {
        ...
        $vs_buf .= "<tr><td width='130'>{$vs_key}</td><td>" . _caFormatMediaMetadataArray(
                $vs_val,
                $pn_level + 1,
                $vs_key
            ) . "</td></tr>";
...
function caEditorFindResultNavigation($po_request, $po_instance, $po_result_context, $pa_options = null)
{
    ...
    if (is_array($va_found_ids) && sizeof($va_found_ids)) {
        if ($vn_prev_id > 0) {
            if (
                $po_request->user->canAccess(
                    $po_request->getModulePath(),
                    $po_request->getController(),
                    "Edit",
                ...
                $vs_buf .= caNavLink(
                        $po_request,
                        caNavIcon(__CA_NAV_ICON_SCROLL_LT__, 2),
                        'prev record',
                        $po_request->getModulePath(),
                        $po_request->getController(),
                        'Edit' . '/' . $po_request->getActionExtra(),
                        array($vs_pk => $vn_prev_id)
                    ) . '&nbsp;';
            } else {
                $vs_buf .= caNavLink(
        ...
        $vs_buf .= "<span class='resultCount'>" . ResultContext::getResultsLinkForLastFind(
                $po_request,
                $vs_table_name,
                $vs_back_text,
                ''
            ) . " (" . ($vn_current_pos) . "/" . sizeof($va_found_ids) . ")</span>";
        ...
        if ($vn_next_id > 0) {
            if (
                ...
                $vs_buf .= '&nbsp;' . caNavLink(
                        $po_request,
                        caNavIcon(__CA_NAV_ICON_SCROLL_RT__, 2),
                        'next record',
                        $po_request->getModulePath(),
                        $po_request->getController(),
                        'Edit' . '/' . $po_request->getActionExtra(),
...
function caSetupEditorScreenOverlays($po_request, $pt_subject, $pa_bundle_list, $pa_options = null)
{
    ...
    if ($pt_subject && $pt_subject->isHierarchical()) {
        $vs_buf .= caEditorHierarchyOverview(
            $po_request,
            $pt_subject->tableName(),
...
function caEditorFieldList($po_request, $pt_subject, $pa_bundle_list, $pa_options = null)
{
    $vs_buf = "<script type=\"text/javascript\">
		jQuery(document).on('ready', function() {
			...
			if (typeof caBundleUpdateManager !== 'undefined') { caBundleUpdateManager = caUI.initBundleUpdateManager({url:'" . caNavUrl(
            $po_request,
            '*',
            '*',
            'reload'
        ) . "', screen:'" . $po_request->getActionExtra() . "', key:'" . $pt_subject->primaryKey(
        ) . "', id: " . (int)$pt_subject->getPrimaryKey() . "}) }
...
function caEditorHierarchyOverview($po_request, $ps_table, $pn_id, $pa_options = null)
{
    ...
    $vs_buf = "<script type=\"text/javascript\">
		jQuery(document).on('ready', function() {
            ...
            $po_request->getModulePath(),
            $po_request->getController(),
            'getHierarchyForDisplay',
            array($t_subject->primaryKey() => $pn_id)
        ) . "';
				}
...
function caEditorInspector($po_view, $pa_options = null)
{
    ...
    if (($vn_item_id) || ($po_view->request->getAction() === 'Delete')) {
        $vs_buf = '<h3 class="nextPrevious" ' . $vs_style . '>' . caEditorFindResultNavigation(
                $po_view->request,
                $t_item,
                $o_result_context,
                $pa_options
    ...
    if (($po_view->request->getAction() === 'Delete') && ($po_view->request->getParameter('confirm', pInteger))) {
        ...
        if ($vn_item_id) {
            if (!$po_view->request->config->get("{$vs_priv_table_name}_inspector_disable_headline")) {
                if ($po_view->request->user->canDoAction("can_edit_" . $vs_priv_table_name) && (sizeof(
                            $t_item->getTypeList()
                        ) > 1)) {
            ...
            if ($t_item->hasField('is_deaccessioned') && $t_item->get('is_deaccessioned') && ($t_item->get(
                        'deaccession_date',
                        array('getDirectDate' => true)
                    ) <= caDateToHistoricTimestamp(_t('now')))) {
                // If currently deaccessioned then display deaccession message
                $vs_buf .= "<br/><div class='inspectorDeaccessioned'>" . _t(
                        'Deaccessioned %1',
                        $t_item->get('deaccession_date')
                    ) . "</div>\n";
                ...
                if ($po_view->request->user->canDoAction('can_see_current_location_in_inspector_' . $vs_table_name)) {
                    ...
                    if (method_exists(
                            $t_item,
                            "getHistory"
                        ) && ($inspector_current_value_label = $t_item->getInspectorHistoryTrackingDisplayPolicy(
                            'label'
                        ))) {
                        if ($inspector_current_value = $t_item->getCurrentValueForDisplay()) {
                            $vs_buf .= "<div class='inspectorCurrentLocation'><strong>{$inspector_current_value_label}:</strong><br/>{$inspector_current_value}" . (($is_home) ? ' ' . _t(
                                        '[HOME]'
                                    ) : '') . "</div>";
                        }
                    }
                    ...
                    if ((!$is_home || !$inspector_current_value) && caHomeLocationsEnabled(
                        ...
                        $vs_buf .= "<div class='inspectorCurrentLocation'><strong>" . _t(
                                'Home location'
            ...
            if (is_array(
                ...
                if (!($vs_display_flag_delim = $po_view->request->config->get(
                    "{$vs_table_name}_inspector_display_flags_delimiter"
                ))) {
            ...
            if (!(($vs_table_name === 'ca_objects') && $vb_dont_use_labels_for_ca_objects)) {
                if ($vs_get_spec = $po_view->request->config->get("{$vs_table_name}_inspector_display_title")) {
                    ...
                    $va_object_collection_collection_ancestors = $po_view->getVar(
                    ...
                    if (
                        ...
                        is_array($va_object_collection_collection_ancestors) && sizeof(
                        ...
                        foreach ($va_object_collection_collection_ancestors as $va_collection_ancestor) {
                            $va_collection_links[] = caEditorLink(
                                $po_view->request,
                                $va_collection_ancestor['label'],
                                '',
                                'ca_collections',
                                $va_collection_ancestor['collection_id']
                            );
                        }
                    ...
                    if (method_exists($t_item, 'getLabelForDisplay') && ($t_item->getLabelTableInstance())) {
                        ...
                        if ($vn_parent_id && (($vs_table_name != 'ca_places') || ($vn_parent_index > 0))) {
                            ...
                            if ($va_parent['NODE'][$vs_disp_fld] && ($vs_editor_link = caEditorLink(
                                    $po_view->request,
                                    $va_parent['NODE'][$vs_disp_fld],
                                    '',
                                    $vs_table_name,
                                    $va_parent['NODE'][$t_item->primaryKey()]
                                ))) {
                            ...
                            if (($vs_table_name === 'ca_editor_uis') && (in_array(
                                    $po_view->request->getAction(),
                                    array(
                                        'EditScreen',
                                        'DeleteScreen',
                                        'SaveScreen'
                                ...
                                $t_screen = new ca_editor_ui_screens(
                                    $po_view->request->getParameter('screen_id', pInteger)
                                );
                        ...
                        $vs_label .= $t_item->hasField('name') ? $t_item->get('name') : $t_item->get(
                            array_shift($t_item->getProperty('LIST_FIELDS'))
                        );
                    }
                }
            }

            $vb_show_idno = (!$po_view->request->config->get(
                    "{$vs_table_name}_inspector_dont_display_idno"
            ...
            if (($vs_table_name === 'ca_object_lots') && $t_item->getPrimaryKey()) {
                $vs_buf .= "<div id='inspectorLotMediaDownload'><strong>" . ((($vn_num_objects = $t_item->numObjects(
                            null,
                            [
                                'excludeChildObjects' => $po_view->request->config->get(
                                    "exclude_child_objects_in_inspector_log_count"
                                )
                            ]
                        )) == 1) ? _t('Lot contains %1 object', $vn_num_objects) : _t(
                        'Lot contains %1 objects',
                        $vn_num_objects
                    )) . "</strong>\n";
            }
            ...
            $vs_buf .= "<div class='creatingNew'>" . _t("Creating new %1", $vs_type_name) . " " . ($vs_parent_name ? _t(
                    "%1 &gt; New %2",
                    $vs_parent_name,
                    $vs_type_name
                ) : '') . "</div>\n";
            ...
            if (is_array($va_reps) && (sizeof($va_reps) > 0)) {
                ...
                if (sizeof($va_reps) > 1) {
					...
					<div class='leftScroll'>
						<a href='#' onclick='inspectorInfoRepScroller.scrollToPreviousImage(); return false;'>" . caNavIcon(
                            __CA_NAV_ICON_SCROLL_LT__,
                            '16px'
                        ) . "</a>
					</div>
		";
		...
		";
					...
					<div class='rightScroll'>
						<a href='#' onclick='inspectorInfoRepScroller.scrollToNextImage(); return false;'>" . caNavIcon(
                                __CA_NAV_ICON_SCROLL_RT__,
                                '16px'
                            ) . "</a>
					</div>
		";
            ...
            if ($vs_additional_info = $po_view->request->config->get("{$vs_table_name}_inspector_additional_info")) {
                if (is_array($vs_additional_info)) {
                    ...
                    foreach ($vs_additional_info as $vs_info) {
                        $vs_buf .= caProcessTemplateForIDs(
                                $vs_info,
                                $vs_table_name,
                    ...
                    $vs_buf .= "<br/>" . caProcessTemplateForIDs(
                            $vs_additional_info,
                            $vs_table_name,
                            array($t_item->getPrimaryKey()),
                            array('requireLinkTags' => true)
            ...
            if ($vn_item_id) {
                ...
                if (in_array(
                    ...
                    $vs_watch = "<div class='watchThis inspectorActionButton'><div><a href='#' title='" . _t(
                            'Add/remove item to/from watch list.'
                        ) . "' onclick='caToggleItemWatch(); return false;' id='caWatchItemButton'>" . caNavIcon(
                            $t_watch_list->isItemWatched(
                                $vn_item_id,
                                $t_item->tableNum(),
                                $po_view->request->user->get("user_id")
                            ) ? __CA_NAV_ICON_UNWATCH__ : __CA_NAV_ICON_WATCH__,
                            '20px'
                        ) . "</a></div></div>";
		...
		function caToggleItemWatch() {
			var url = '" . caNavUrl(
                            $po_view->request,
                            $po_view->request->getModulePath(),
                            $po_view->request->getController(),
                            'toggleWatch',
			...
			jQuery.getJSON(url, {}, function(data, status) {
				if (data['status'] == 'ok') {
					jQuery('#caWatchItemButton').html((data['state'] == 'watched') ? '" . addslashes(
                            caNavIcon(__CA_NAV_ICON_UNWATCH__, '20px')
                ...
                if ($po_view->request->user->canDoAction("can_change_type_{$vs_table_name}") && (sizeof(
                            $t_item->getTypeList()
                        ) > 1)) {
                    $vs_buf .= "<div id='inspectorChangeType' class='inspectorActionButton'><div id='inspectorChangeTypeButton'><a href='#' onclick='caTypeChangePanel.showPanel(); return false;'>" . caNavIcon(
                            __CA_NAV_ICON_CHANGE__,
                            '20px',
                            array('title' => _t('Change type'))
                        ) . "</a></div></div>\n";

                    $vo_change_type_view = new View(
                        $po_view->request,
                        $po_view->request->getViewsDirectoryPath() . "/bundles/"
                    );
                ...
                if ($t_item->getPrimaryKey() && $po_view->request->config->get(
                        $vs_table_name . '_show_add_child_control_in_inspector'
                    )) {
                    $vb_show_add_child_control = true;
                    if (is_array(
                            $va_restrict_add_child_control_to_types = $po_view->request->config->getList(
                                $vs_table_name . '_restrict_child_control_in_inspector_to_types'
                            )
                        ) && sizeof($va_restrict_add_child_control_to_types)) {
                        $t_type_instance = $t_item->getTypeInstance();
                        if (!in_array(
                                $t_type_instance->get('idno'),
                                $va_restrict_add_child_control_to_types
                            ) && !in_array(
                                $t_type_instance->getPrimaryKey(),
                                $va_restrict_add_child_control_to_types
                            )) {
                    ...
                    if ($vb_show_add_child_control) {
                        if ((bool)$po_view->request->config->get($vs_table_name . '_enforce_strict_type_hierarchy')) {
                            // strict menu
                            $vs_type_list = $t_item->getTypeListAsHTMLFormElement(
                                'type_id',
                                array('style' => 'width: 90px; font-size: 9px;'),
                                array(
                                    'childrenOfCurrentTypeOnly' => true,
                                    'directChildrenOnly' => ($po_view->request->config->get(
                        ...
                        if ($vs_type_list) {
                            $vs_buf .= "<div id='inspectorCreateChild' class='inspectorActionButton'><div id='inspectorCreateChildButton'><a href='#' onclick='caCreateChildPanel.showPanel(); return false;'>" . caNavIcon(
                                    __CA_NAV_ICON_CHILD__,
                                    '20px',
                                    array('title' => _t('Create Child Record'))
                                ) . "</a></div></div>\n";

                            $vo_create_child_view = new View(
                                $po_view->request,
                                $po_view->request->getViewsDirectoryPath() . "/bundles/"
                            );
            ...
            if ($po_view->request->user->canDoAction('can_duplicate_' . $vs_table_name) && $t_item->getPrimaryKey()) {
                ...
                $vs_buf .= caFormTag(
                    ...
                    'Edit',
                    'DuplicateItemForm',
                    $po_view->request->getModulePath() . '/' . $po_view->request->getController(),
                    'post',
                    'multipart/form-data',
                    '_top',
                    array(
                ...
                $vs_buf .= "<div>" . caFormSubmitLink(
                        $po_view->request,
                        caNavIcon(__CA_NAV_ICON_DUPLICATE__, '20px'),
                        '',
                        'DuplicateItemForm'
                    ) . "</div>";
            ...
            if (caHomeLocationsEnabled(
                ...
                $vs_buf .= "<div id='inspectorSetHomeLocation' class='inspectorActionButton'><div id='inspectorSetHomeLocationButton'><a href='#' onclick='_initSetHomeLocationHierarchyBrowser(); return false;'>" . caNavIcon(
                        ...
                        '20px',
                        array('title' => _t('Set home location'))
                    ) . "</a></div></div>\n";
            ...
            if ($vn_num_objects > 0) {
                $vs_buf .= "<div id='inspectorLotMediaDownloadButton' class='inspectorActionButton'>" . caNavLink(
                        $po_view->request,
                        caNavIcon(__CA_NAV_ICON_DOWNLOAD__, '20px'),
                        "button",
                        $po_view->request->getModulePath(),
                        $po_view->request->getController(),
                        'getLotMedia',
                        array('lot_id' => $t_item->getPrimaryKey(), 'download' => 1),
                        array()
                    ) . "</div>\n";
                TooltipManager::add(
            ...
            if (($vs_table_name == 'ca_sets') && (sizeof($t_item->getItemRowIDs()) > 0)) {
                $vs_buf .= "<div id='inspectorSetMediaDownloadButton' class='inspectorActionButton'>" . caNavLink(
                        $po_view->request,
                        caNavIcon(__CA_NAV_ICON_DOWNLOAD__, '20px'),
                        "button",
                        $po_view->request->getModulePath(),
                        $po_view->request->getController(),
                        'getSetMedia',
                        array('set_id' => $t_item->getPrimaryKey(), 'download' => 1),
                        array()
                    ) . "</div>\n";

                TooltipManager::add(
                    '#inspectorSetMediaDownloadButton',
            ...
            if (is_array(
                    $va_sets = caExtractValuesByUserLocale(
                        $t_set->getSetsForItem(
                            ...
                            array(
                                'user_id' => $po_view->request->getUserID(),
                ...
                foreach ($va_sets as $vn_set_id => $va_set) {
                    ...
                    $va_links[] = "<a {$class} href='" . caEditorUrl(
                            $po_view->request,
                            'ca_sets',
                            $vn_set_id
                        ) . "'>" . $va_set['name'] . "</a>";
                }
                $vs_more_info .= "<div><strong>" . ((sizeof($va_links) == 1) ? _t("In set") : _t(
                        "In sets"
            ...
            if ($vn_item_id && $vs_select = $po_view->getVar('available_mappings_as_html_select')) {
                $vs_more_info .= "<div class='inspectorExportControls'>" . caFormTag(
                        $po_view->request,
                        'exportItem',
                        'caExportForm',
                        null,
            ...
            if ($va_creation['timestamp'] || $va_last_change['timestamp']) {
                ...
                if ($va_creation['timestamp']) {
                    ...
                    $vs_interval = (($vn_t = (time() - $va_creation['timestamp'])) == 0) ? _t('Just now') : _t(
                        '%1 ago',
                        caFormatInterval(
                            $vn_t,
                            2
                    ...
                    $vs_more_info .= "<div class='inspectorChangeDateListLine'  id='caInspectorCreationDate'>" .
                        ($vs_name ? _t('<strong>Created</strong><br/>%1 by %2', $vs_interval, $vs_name) : _t(
                            '<strong>Created</strong><br/>%1',
                            $vs_interval
                        )) .
                        "</div>";

                    TooltipManager::add(
                        "#caInspectorCreationDate",
                        "<h2>" . _t('Created on') . "</h2>" . _t(
                            'Created on %1',
                            caGetLocalizedDate(
                                $va_creation['timestamp'],
                ...
                if ($va_last_change['timestamp'] && ($va_creation['timestamp'] != $va_last_change['timestamp'])) {
                    ...
                    $vs_interval = (($vn_t = (time() - $va_last_change['timestamp'])) == 0) ? _t('Just now') : _t(
                        '%1 ago',
                        caFormatInterval($vn_t, 2)
                    );

                    $vs_more_info .= "<div class='inspectorChangeDateListLine' id='caInspectorChangeDate'>" .
                        ($vs_name ? _t('<strong>Last changed</strong><br/>%1 by %2', $vs_interval, $vs_name) : _t(
                            '<strong>Last changed</strong><br/>%1',
                            $vs_interval
                        )) .
                        "</div>";

                    TooltipManager::add(
                        "#caInspectorChangeDate",
                        "<h2>" . _t('Last changed on') . "</h2>" . _t(
                            'Last changed on %1',
                            caGetLocalizedDate(
                                $va_last_change['timestamp'],
                                array('dateFormat' => 'delimited')
                            )
                        )
                    );
                ...
                if (method_exists($t_item, 'getMetadataDictionaryRuleViolations') && is_array(
                        $va_violations = $t_item->getMetadataDictionaryRuleViolations()
                    ) && (($total_num_violations = (sizeof($va_violations))) > 0)) {
                    $screen_id = $po_view->request->getActionExtra();
                    if (!$screen_id || !is_array(
                            $violations_for_current_screen = $t_item->getMetadataDictionaryRuleViolations(
                                null,
                                ['screen_id' => $screen_id]
                            )
                        )) {
                    ...
                    foreach ($va_violations as $vn_violation_id => $va_violation) {
                        ...
                        if (isset($violations_for_current_screen[$vn_violation_id])) {
                            ...
                            $placements = $t_ui->getPlacementsForBundle(
                                $va_violation['bundle_name'],
                                $po_view->request,
                                []
                            );
                            if (!is_array($placements) || !sizeof($placements)) {
                                $placements = $t_ui->getPlacementsForBundle(
                                    "ca_attribute_{$bundle}",
                                    $po_view->request,
                                    []
                                );
                                if (!is_array($placements) || !sizeof($placements)) {
                        ...
                        $va_violation_messages[] = "<li " . ($in_this_tab ? "class='caMetadataDictionaryViolationInCurrentTab'" : "class='caMetadataDictionaryViolationNotInCurrentTab'") . "><em><u>{$vs_label}</u></em> " . caExtractSettingsValueByUserLocale(
                                'violationMessage',
                                $va_violation
                            ) . " {$screen}</li>";
                    }
                    ...
                    if ($total_num_violations_for_current_screen > 0) {
                        if ($total_num_violations_for_current_screen != $total_num_violations) {
                            $vs_more_info .= "<div id='caInspectorViolationsList'>" . caNavIcon(
                                    __CA_NAV_ICON_ALERT__,
                                    "14px"
                                ) . " " . ($vs_num_violations_display = (($total_num_violations_for_current_screen > 1) ? _t(
                                    '%1 problems on this tab require attention (of %2 total)',
                                    $total_num_violations_for_current_screen,
                                    $total_num_violations
                                ) : _t(
                                    '%1 problem on this tab requires attention (of %2 total)',
                                    $total_num_violations_for_current_screen,
                                    $total_num_violations
                                ))) . "</div>\n";
                        } else {
                            $vs_more_info .= "<div id='caInspectorViolationsList'>" . caNavIcon(
                                    ...
                                    '%1 problems on this tab require attention',
                                    $total_num_violations
                                ) : _t(
                                    '%1 problem on this tab requires attention',
                                    $total_num_violations
                                ))) . "</div>\n";
                        }
                    } else {
                        $vs_more_info .= "<div id='caInspectorViolationsList'>" . caNavIcon(
                                __CA_NAV_ICON_ALERT__,
                                "14px"
                            ) . " " . ($vs_num_violations_display = (($total_num_violations > 1) ? _t(
                                '%1 problems require attention',
                                $total_num_violations
                            ) : _t('%1 problem requires attention', $total_num_violations))) . "</div>\n";
                    }
                    if ($vs_num_violations_display) {
                        TooltipManager::add(
                            "#caInspectorViolationsList",
	...
	<div id='inspectorInfo' >{$vs_more_info}</div>\n";
            ...
            if ($vs_rel_table && Datamodel::tableExists($vs_rel_table) && $vn_rel_type_id && $vn_rel_id) {
                ...
                if ($t_rel && $t_rel->load($vn_rel_id)) {
                    $vs_buf .= '<strong>' . _t(
                            ...
                            $t_rel->getTypeName()
                        ) . '</strong>: ' . $t_rel->getLabelForDisplay();
                }
            }
        }
        ...
        if (($vs_table_name === 'ca_objects') && ($vn_lot_id)) {
            ...
            if (($t_lot->get('deleted') == 0) && (in_array($t_lot->get('type_id'), $va_lot_lots))) {
                ...
                if ($vs_lot_displayname) {
                    if (!($vs_part_of_lot_msg = $po_view->request->config->get(
                        "ca_objects_inspector_part_of_lot_msg"
                    ))) {
                        $vs_part_of_lot_msg = _t('Part of lot');
                    }
                    if (!($vs_will_be_part_of_lot_msg = $po_view->request->config->get(
                        "ca_objects_inspector_will_be_part_of_lot_msg"
                    ))) {
                        $vs_will_be_part_of_lot_msg = _t('Will be part of lot');
                    }
                    $vs_buf .= "<br/><strong>" . ($vb_is_currently_part_of_lot ? $vs_part_of_lot_msg : $vs_will_be_part_of_lot_msg) . "</strong>: " . caNavLink(
                            $po_view->request,
                            $vs_lot_displayname,
                            '',
                            'editor/object_lots',
        ...
        $vb_can_add_component = (($vs_table_name === 'ca_objects') && $t_item->getPrimaryKey(
            ) && ($po_view->request->user->canDoAction('can_create_ca_objects')) && $t_item->canTakeComponents());

        if (method_exists($t_item, 'getComponentCount')) {
            if ($vn_component_count = $t_item->getComponentCount()) {
                if ($t_ui && ($vs_component_list_screen = $t_ui->getScreenWithBundle(
                        "ca_objects_components_list",
                        $po_view->request
                    )) && ($vs_component_list_screen !== $po_view->request->getActionExtra())) {
                    $vs_component_count_link = caNavLink(
                        $po_view->request,
                        (($vn_component_count == 1) ? _t('%1 component', $vn_component_count) : _t(
                            '%1 components',
                            $vn_component_count
                        )),
                        '',
                    ...
                    $vs_component_count_link = (($vn_component_count == 1) ? _t(
                        '%1 component',
                        $vn_component_count
        ...
        if ($vb_can_add_component) {
            $vs_buf .= ' <a href="#" onclick=\'caObjectComponentPanel.showPanel("' . caNavUrl(
                    $po_view->request,
                    '*',
                    'ObjectComponent',
                    'Form',
                    array(
                        'parent_id' => $t_item->getPrimaryKey()
                    )
                ) . '"); return false;\')>' . caNavIcon(__CA_NAV_ICON_ADD__, '18px') . '</a>';

            $vo_change_type_view = new View(
                $po_view->request, $po_view->request->getViewsDirectoryPath() . "/bundles/"
            );
            $vo_change_type_view->setVar('t_item', $t_item);
        ...
        if (($vs_table_name === 'ca_object_lots') && $t_item->getPrimaryKey()) {
            ...
            if (is_array($va_component_types) && sizeof($va_component_types)) {
                $vs_buf .= "<br/><strong>" . ((($vn_num_objects = $t_item->numObjects(
                            null,
                            array(
                                'return' => 'objects',
                                'excludeChildObjects' => $po_view->request->config->get(
                ...
                $vs_buf .= "<br/><strong>" . ((($vn_num_components = $t_item->numObjects(
                            null,
                            array('return' => 'components')
                        )) == 1) ? _t('Lot contains %1 component', $vn_num_components) : _t(
                        'Lot contains %1 components',
                        $vn_num_components
                    )) . "</strong>\n";
            } else {
                $vs_buf .= "<br/><strong>" . ((($vn_num_objects = $t_item->numObjects(
            ...
            if (((bool)$po_view->request->config->get(
                    'allow_automated_renumbering_of_objects_in_a_lot'
                )) && ($va_nonconforming_objects = $t_item->getObjectsWithNonConformingIdnos())) {
                $vs_buf .= '<br/><br/><em>' . ((($vn_c = sizeof($va_nonconforming_objects)) == 1) ? _t(
                        'There is %1 object with non-conforming numbering',
                        $vn_c
                    ) : _t('There are %1 objects with non-conforming numbering', $vn_c)) . "</em>\n";

                $vs_buf .= "<a href='#' onclick='jQuery(\"#inspectorNonConformingNumberList\").toggle(250); return false;'>" . caNavIcon(
                        __CA_NAV_ICON_ADD__,
                        '18px'
                    );
            ...
            if (!(bool)$po_view->request->config->get('disable_add_object_to_lot_inspector_controls')) {
                    ...
                    $vs_buf .= _t(
                        ...
                        $t_object->getTypeListAsHTMLFormElement(
                            ...
                            array(
                                'childrenOfCurrentTypeOnly' => true,
                                'directChildrenOnly' => ($po_view->request->config->get(
                                        'ca_objects_enforce_strict_type_hierarchy'
                                    ) == '~') ? false : true,
                                'returnHierarchyLevels' => true,
                                'access' => __CA_BUNDLE_ACCESS_EDIT__
                            )
                ...
                $vs_buf .= "&nbsp;<a href='#' onclick='caAddObjectToLotForm()'>" . caNavIcon(
                        __CA_NAV_ICON_ADD__,
                        '16px'
                    ) . '</a>';
                $vs_buf .= "</form></div>\n";
	...
	function caAddObjectToLotForm() { 
		window.location='" . caEditorUrl(
                        $po_view->request,
                        'ca_objects',
                        0,
                        false,
                        array('lot_id' => $t_item->getPrimaryKey(), 'rel' => 1, 'type_id' => '')
                    ) . "' + jQuery('#caAddObjectToLotForm_type_id').val();
	}
	jQuery(document).ready(function() {
        ...
        //
        if (is_array(
                $va_show_counts_for = $po_view->request->config->getList(
                    $t_item->tableName() . '_show_related_counts_in_inspector_for'
                )
                    ...
                    $vs_buf .= caSearchLink(
                            ...
                            _t(
                                ...
                                Datamodel::getTableProperty(
                                    $vs_rel_table,
                                    ($vn_count === 1) ? 'NAME_SINGULAR' : 'NAME_PLURAL'
                                )
                            ),
                            '',
                            $vs_rel_table,
                            $t_item->primaryKey(true) . ":" . $t_item->getPrimaryKey()
                        ) . "<br/>\n";
                }
            }
        }
        ...
        if ($vs_table_name === 'ca_object_representations') {
            foreach (
                ...
                if (sizeof($va_objects = $t_item->getRelatedItems($vs_rel_table))) {
                    $vs_buf .= "<div><strong>" . _t(
                            "Related %1",
                            Datamodel::getTableProperty($vs_rel_table, 'NAME_PLURAL')
                        ) . "</strong>: <br/>\n";
                    ...
                    foreach ($va_objects as $vn_rel_id => $va_rel_info) {
                        if ($vs_label = array_shift($va_rel_info['labels'])) {
                            $vs_buf .= caEditorLink(
                                    $po_view->request,
                                    '&larr; ' . $vs_label . ' (' . $va_rel_info['idno'] . ')',
                                    '',
                                    $vs_rel_table,
                                    $va_rel_info[Datamodel::primaryKey($vs_rel_table)],
                                    array(),
                                    array(),
                                    array('action' => 'Edit' . ($vs_screen ? "/{$vs_screen}" : ""))
                                ) . "<br/>\n";
                        }
                    }
                    $vs_buf .= "</div>\n";
        ...
        if ($vs_table_name === 'ca_sets') {
            ...
            if (($vn_set_item_count > 0) && ($po_view->request->user->canDoAction(
                    'can_batch_edit_' . Datamodel::getTableName($t_item->get('table_num'))
                ))) {
                $vs_buf .= caNavButton(
                    $po_view->request,
                    ...
                    'Editor',
                    'Edit',
                    array('set_id' => $t_item->getPrimaryKey()),
                    array(),
                    array(
            ...
            if ($t_item->getPrimaryKey()) {
                $vs_buf .= "<strong>" . _t("Type of content") . "</strong>: " . caGetTableDisplayName(
                        $vn_set_table_num
                    ) . "<br/>\n";
                ...
                if (!(bool)$po_view->request->config->get(
                    ...
                    ) && $po_view->request->user->canDoAction(
                ...
                if ($vn_set_table_num = $po_view->request->getParameter('table_num', pInteger)) {
                    $vs_buf .= "<div><strong>" . _t("Type of content") . "</strong>: " . caGetTableDisplayName(
                            $vn_set_table_num
                        ) . "<br/>\n";
            ...
            $t_user = new ca_users(
                ($vn_user_id = $t_item->get('user_id')) ? $vn_user_id : $po_view->request->getUserID()
            );
            if ($t_user->getPrimaryKey()) {
                $vs_buf .= "<div><strong>" . _t('Owner') . "</strong>: " . $t_user->get('fname') . ' ' . $t_user->get(
                        'lname'
                    ) . "</div>\n";
            }
            ...
            if ($po_view->request->user->canDoAction('can_export_' . $vs_set_table_name) && $t_item->getPrimaryKey(
                ...
                $vs_buf .= "<a class='button' onclick='jQuery(\"#exporterFormList\").show();' style='text-align:right;' href='#'>" . caNavIcon(
                        __CA_NAV_ICON_EXPORT_SMALL__,
                        '16px'
                    ) . "</a>";
                ...
                $vs_buf .= ca_data_exporters::getExporterListAsHTMLFormElement(
                    'exporter_id',
                    $vn_set_table_num,
        ...
        if ($vs_table_name === 'ca_set_items') {
            ...
            if ($t_set->load($vn_set_id = $t_item->get('set_id'))) {
                $vs_buf .= "<div><strong>" . _t("Part of set") . "</strong>: " . caEditorLink(
                        $po_view->request,
        ...
        if (($vs_table_name == 'ca_metadata_alert_rules')) {
            if ($t_item->getPrimaryKey()) {
                $vs_buf .= "<div><strong>" . _t("Rule content type") . "</strong>: " . caGetTableDisplayName(
                        $t_item->get('table_num')
                    ) . "<br/></div>\n";
            } else {
                if ($vn_set_table_num = $po_view->request->getParameter('table_num', pInteger)) {
                    $vs_buf .= "<div><strong>" . _t("Rule content type") . "</strong>: " . caGetTableDisplayName(
        ...
        if (($vs_table_name === 'ca_lists') && $t_item->getPrimaryKey()) {
            ...
            $vs_type_list = $t_list_item->getTypeListAsHTMLFormElement(
                'type_id',
                array('style' => 'width: 90px; font-size: 9px;'),
                array('access' => __CA_BUNDLE_ACCESS_EDIT__)
            );
            ...
            if ($vs_type_list) {
                ...
                $vs_buf .= caFormTag(
                    ...
                    'Edit',
                    'NewChildForm',
                    'administrate/setup/list_item_editor/ListItemEditor',
                    'post',
                    'multipart/form-data',
                    '_top',
                    array('noCSRFToken' => true, 'disableUnsavedChangesWarning' => true)
                );
                $vs_buf .= _t('Add a %1 to this list', $vs_type_list) . caHTMLHiddenInput(
                        $t_list_item->primaryKey(),
                        array('value' => '0')
                    ) . caHTMLHiddenInput('parent_id', array('value' => $t_list_item->getPrimaryKey()));
                $vs_buf .= caFormSubmitLink(
                    $po_view->request,
        ...
        if ($vs_table_name === 'ca_list_items') {
            if ($t_list = $po_view->getVar('t_list')) {
                ...
                $vs_buf .= "<div><strong>" . _t("Part of") . "</strong>: " . caEditorLink(
                        $po_view->request,
                        $t_list->getLabelForDisplay(),
                        '',
                        'ca_lists',
                        $vn_list_id
                    ) . "</div>\n";
        ...
        if ($vs_table_name === 'ca_relationship_types') {
            ...
            if ($t_rel_instance) {
                $vs_buf .= "<div><strong>" . _t("Is a") . "</strong>: " . $t_rel_instance->getProperty(
                        'NAME_SINGULAR'
                    ) . "<br/></div>\n";
            }
        }
        ...
        if (($vs_table_name === 'ca_metadata_elements') && $t_item->getPrimaryKey()) {
            $vs_buf .= "<div><strong>" . _t("Element code") . "</strong>: " . $t_item->get(
                    'element_code'
                ) . "<br/></div>\n";
            ...
            if (sizeof($va_uis = $t_item->getUIs()) > 0) {
                ...
                foreach ($va_uis as $vn_ui_id => $va_ui_info) {
                    ...
                    $vs_buf .= " (" . Datamodel::getTableProperty(
                            $va_ui_info['editor_type'],
                            'NAME_PLURAL'
                        ) . ")<br/>\n";
                }
                $vs_buf .= "</div>\n";
        ...
        if ($vs_table_name === 'ca_editor_uis') {
            ...
            if ($t_item->getPrimaryKey()) {
                $vs_buf .= "<div><strong>" . _t("Edits") . "</strong>: " . caGetTableDisplayName(
                        $t_item->get('editor_type')
                    ) . "<br/>\n";
            } else {
                $vs_buf .= "<div><strong>" . _t("Edits") . "</strong>: " . caGetTableDisplayName(
                        $po_view->request->getParameter('editor_type', pInteger)
                    ) . "<br/>\n";
            }
            $vs_buf .= "</div>\n";
        }
        ...
        if ($vs_table_name === 'ca_editor_ui_screens') {
            ...
            $vs_buf .= "<div><strong>" . _t("Part of") . "</strong>: " . caNavLink(
                    $po_view->request,
                    $t_ui->getLabelForDisplay(),
                    '',
        ...
        if ($vs_table_name === 'ca_bundle_displays') {
            $vs_buf .= "<div><strong>" . _t("Number of placements") . "</strong>: " . $t_item->getPlacementCount(
                    array('user_id' => $po_view->request->getUserID())
                ) . "<br/>\n";

            if ($t_item->getPrimaryKey()) {
                $vn_content_table_num = $t_item->get('table_num');
                $vs_buf .= "<strong>" . _t("Type of content") . "</strong>: " . caGetTableDisplayName(
                        $vn_content_table_num
                    ) . "\n";
        ...
        if ($vs_table_name === 'ca_search_forms') {
            ...
            if ($t_item->getPrimaryKey()) {
                $vn_content_table_num = $t_item->get('table_num');
                $vs_buf .= "<strong>" . _t("Searches for") . "</strong>: " . caGetTableDisplayName(
                        $vn_content_table_num
                    ) . "\n";
                $vs_buf .= "</div>\n";
            } else {
        ...
        if ($vs_table_name === 'ca_tour_stops') {
            ...
            $vs_buf .= "<strong>" . _t("Part of") . "</strong>: " . caEditorLink(
                    $po_view->request,
                    $t_tour->getLabelForDisplay(),
                    '',
                    'ca_tours',
                    $vn_tour_id
                ) . "<br/>\n";
        ...
        if ($vs_table_name === 'ca_bundle_mappings') {
            if ($t_item->getPrimaryKey()) {
                $vn_content_table_num = $t_item->get('table_num');
                $vs_buf .= "<br/><strong>" . _t("Type of content") . "</strong>: " . caGetTableDisplayName(
                        $vn_content_table_num
                    ) . "<br/>\n";
                $vs_buf .= "<strong>" . _t("Type") . "</strong>: " . $t_item->getChoiceListValue(
                        'direction',
                        $t_item->get(
                            'direction'
                        )
                ...
                if ($vn_content_table_num = $po_view->request->getParameter('table_num', pInteger)) {
                    ...
                    $vs_buf .= "<strong>" . _t("Type") . "</strong>: " . $t_item->getChoiceListValue(
                            'direction',
                            $po_view->request->getParameter(
                                'direction',
                                pString
                            )
                        ) . "<br/>\n";
                    $vs_buf .= "<strong>" . _t("Target format") . "</strong>: " . $po_view->request->getParameter(
                            'target',
                            pString
                        ) . "<br/>\n";
        ...
        if ($t_item->getPrimaryKey() && $po_view->request->config->get(
            ...
            if (is_array(
                ...
                $t_type_instance = $t_item->getTypeInstance();
                if (!in_array($t_type_instance->get('idno'), $va_restrict_add_child_control_to_types) && !in_array(
                        $t_type_instance->getPrimaryKey(),
                        $va_restrict_add_child_control_to_types
                    )) {
                    $vb_show_add_child_control = false;
        ...
        if ($po_view->request->user->canDoAction('can_export_' . $vs_table_name) && $t_item->getPrimaryKey()) {
            ...
            if (ExternalExportManager::getTargets(
                    ['countOnly' => true, 'table' => $t_item->tableNum(), 'restrictToTypes' => [$t_item->getTypeCode()]]
                ) > 0) {
                ...
                $vs_buf .= "<a class='button' onclick='jQuery(\"#externalExporterFormList\").show();' style='text-align:right;' href='#'>" . caNavIcon(
                        __CA_NAV_ICON_EXPORT_SMALL__,
                        '16px'
                    ) . "</a>";
                ...
                $vs_buf .= ExternalExportManager::getTargetListAsHTMLFormElement(
                    'target',
                    $t_item->tableNum(),
        ...
        if ($t_item->getPrimaryKey()) {
			...
			jQuery('#inspectorMoreInfo').click(function() {
				jQuery('#inspectorInfo').slideToggle(350, function() { 
					...
					jQuery('#inspectorMoreInfoButton').html((this.style.display == 'block') ? '" . addslashes(
                        caNavIcon(__CA_NAV_ICON_COLLAPSE__, '20px')
...
function caBatchEditorInspector($po_view, $pa_options = null)
{
    ...
    $vs_buf = '<h3 class="nextPrevious"><span class="resultCount" style="padding-top:10px;">' . caNavLink(
            $po_view->request,
            'Back to Sets',
            '',
            'manage',
    ...
    if ($po_view->request->user->canDoAction("can_edit_" . $vs_priv_table_name) && (sizeof(
        ...
        if ($po_view->request->user->canDoAction("can_change_type_{$vs_table_name}")) {
            $vs_buf .= "<div id='inspectorChangeType'><div id='inspectorChangeTypeButton'><a href='#' onclick='caTypeChangePanel.showPanel(); return false;'>" . caNavIcon(
                    __CA_NAV_ICON_CHANGE__,
                    '18px',
                    array('title' => _t('Change type'))
                ) . "</a></div></div>\n";
    ...
    if (($vn_item_count > 0) && ($po_view->request->user->canDoAction(
            'can_batch_delete_' . Datamodel::getTableName($t_set->get('table_num'))
        ))) {
        $vs_buf .= "<div class='button' style='text-align:right;'><a href='#' id='inspectorMoreInfo'>" . _t(
                "More options"
            ) . "</a> &rsaquo;</div>
        ...
        if ($po_view->request->user->canDoAction("can_change_type_{$vs_table_name}")) {
            $vs_buf .= "<a href='#' onclick='caTypeChangePanel.showPanel(); return false;'>" . caNavIcon(
                    __CA_NAV_ICON_CHANGE__,
                    '20px',
        ...
        $vs_buf .= "<script type='text/javascript'>
				jQuery('#inspectorMoreInfo').click(function() {
					jQuery('#inspectorInfo').slideToggle(350, function() { 
						jQuery('#inspectorMoreInfo').html((this.style.display == 'block') ? '" . addslashes(
                _t('Close options')
            ) . "' : '" . addslashes(_t('More options')) . "');
					}); 
					return false;
				});
			</script>";
...
function caBatchMetadataExportInspector($po_view)
{
    ...
    if ($vn_id = $po_view->request->getParameter('item_id', pInteger)) {
        $vs_buf .= "<p>" . caEditorLink(
                $po_view->request,
                _t("Back to record"),
                'caResultsEditorEditLink',
                $t_item->getTargetTableName(),
                $vn_id
            ) . "</p>";
    }
...
{
    $t_instance = is_numeric($pm_table) ? Datamodel::getInstanceByTableNum(
        $pm_table,
        true
        ...
        switch (strtoupper($va_tmp[0])) {
            case 'UNITS':
                try {
                    ...
                    if ($force_english_units && in_array(
                            $vs_units,
                            [
                                'in',
                                'in.',
                    ...
                    switch ($vs_units) {
                        ...
                        case 'fractionalenglish':
                            $vn_maximum_denominator = array_reduce(
                                $o_dimensions_config->get('display_fractions_for'),
                                function ($acc, $v) {
                                    $t = explode("/", $v);
                            ...
                            if (!$force_english_units && ($vn_value_in_inches > $o_dimensions_config->get(
                                        'use_feet_for_display_up_to'
                                    ))) {
                                if ($vn_in_miles = (int)($vn_value_in_inches / (12 * 5280))) {
                                    $va_measure_conv[] = "{$vn_in_miles} miles" . ((!$pb_omit_units && in_array(
                                                'MILE',
                                                $va_add_periods_list
                                            )) ? '.' : '');
                                }
                            ...
                            if (($force_english_units == 'ft') || (!$force_english_units && ($vn_value_in_inches > $o_dimensions_config->get(
                                            'use_inches_for_display_up_to'
                                        )))) {
                                if ($vn_in_feet = (int)($vn_value_in_inches / 12)) {
                                    $va_measure_conv[] = "{$vn_in_feet} ft" . ((!$pb_omit_units && in_array(
                                                'FEET',
                                                $va_add_periods_list
                                            )) ? '.' : '');
                                }
                            ...
                            if ($vn_value_in_inches > 0) {
                                ...
                                $va_measure_conv[] = (in_array(
                                        $vs_units,
                                        ['fractionalenglish', 'infrac']
                                    ) ? caLengthToFractions(
                                        $vn_value_in_inches,
                                    ...
                                    )) . ((!$pb_omit_units && in_array('INCH', $va_add_periods_list)) ? '.' : '');
                            }
                        ...
                        case 'metric':
                            ...
                            if (($force_metric_units == 'mm') || (!$force_metric_units && ($vn_value_in_cm <= $o_dimensions_config->get(
                                            'use_millimeters_for_display_up_to'
                                        )))) {
                                $vs_measure_conv = $vo_measurement->convertTo(
                                        Zend_Measure_Length::MILLIMETER,
                                        (int)$o_dimensions_config->get(
                                            'millimeter_decimal_precision'
                                        )
                                    ) . ((!$pb_omit_units && in_array('MILLIMETER', $va_add_periods_list)) ? '.' : '');
                            } elseif (($force_metric_units == 'cm') || (!$force_metric_units && ($vn_value_in_cm <= $o_dimensions_config->get(
                                ...
                                $vs_measure_conv = $vo_measurement->convertTo(
                                    ...
                                    ) . ((!$pb_omit_units && in_array('CENTIMETER', $va_add_periods_list)) ? '.' : '');
                            } elseif (($force_metric_units == 'm') || (!$force_metric_units && ($vn_value_in_cm <= $o_dimensions_config->get(
                                ...
                                $vs_measure_conv = $vo_measurement->convertTo(
                                        ...
                                        $o_dimensions_config->get(
                                            'kilometer_decimal_precision'
                                        )
                                    ) . ((!$pb_omit_units && in_array('KILOMETER', $va_add_periods_list)) ? '.' : '');
                            }
                            break;
                        case 'm':
                            $vs_measure_conv = $vo_measurement->convertTo(
                        ...
                        case "'":
                            $vs_measure_conv = $vo_measurement->convertTo(
                                    Zend_Measure_Length::FEET,
                                    $o_dimensions_config->get(
                                        'feet_decimal_precision'
                                    )
                                ) . ((!$pb_omit_units && in_array('FEET', $va_add_periods_list)) ? '.' : '');
                            break;
                        case 'in':
                        case 'inch':
                        case 'inches':
...
function caGetRelationDisplayString($po_request, $ps_table, $pa_attributes = null, $pa_options = null)
{
    ...
    if (!($vs_relationship_type_display_position = caGetOption('relationshipTypeDisplayPosition', $pa_options, null))) {
        $vs_relationship_type_display_position = strtolower(
            $o_config->get($ps_table . '_lookup_relationship_type_position')
        );
    }
    ...
    if (caGetOption('makeLink', $pa_options, false)) {
        $vs_display = "<a href='" . urldecode(
                caEditorUrl(
                    $po_request,
                    $ps_table,
                    '{' . Datamodel::primaryKey($ps_table) . '}',
                    false,
                    array('rel' => true)
                )
    ...
    $vs_reltype_disp = caGetOption(
        'editableRelationshipType',
        $pa_options,
        (bool)$o_config->get("{$ps_table}_lookup_relationship_type_editable")
    ) ? "<select name='{$ps_prefix}_type_id{n}' id='{$ps_prefix}_type_id{n}' class='listRelRelationshipTypeEdit'></select>" : "({{relationship_typename}}) <input type='hidden' name='{$ps_prefix}_type_id{n}' id='{$ps_prefix}_type_id{n}' value='{type_id}'/>";
...
function caProcessRelationshipLookupLabel($qr_rel_items, $pt_rel, $pa_options = null)
{
    ...
    $va_primary_ids = (method_exists($pt_rel, "isSelfRelationship") && ($vb_is_self_rel = $pt_rel->isSelfRelationship(
        ))) ? caGetOption("primaryIDs", $pa_options, null) : null;
    ...
    if (is_object($qr_rel_items)) {
        if (!$qr_rel_items->numHits()) {
            ...
            $va_primary_ids = (method_exists(
                    $pt_rel,
                    "isSelfRelationship"
                ) && ($vb_is_self_rel = $pt_rel->isSelfRelationship())) ? caGetOption(
                "primaryIDs",
                $pa_options,
                null
            ) : null;
    ...
    if (isset($pa_options['relatedItems']) && is_array($pa_options['relatedItems']) && sizeof(
            $pa_options['relatedItems']
        )) {
        $va_tmp = array();
        foreach ($pa_options['relatedItems'] as $vn_relation_id => $va_relation) {
            ...
            if ($vs_template) {
                ...
                $va_items[$va_relation[$vs_rel_pk]]['_display'] = caProcessTemplateForIDs(
                    $vs_template,
                    $pt_rel->tableName(),
                    array($va_relation['relation_id'] ? $va_relation['relation_id'] : $va_relation[$pk]),
                    array(
                        'returnAsArray' => false,
                        'returnAsLink' => false,
...
function caObjectsDisplayDownloadLink($po_request, $pn_object_id = null, $pt_representation = null)
{
    ...
    $va_types = $o_config->get(
        ['restrictObjectRepresentationDownloadToObjectTypes', 'allow_ca_objects_representation_download_types']
    );
    ...
    if ((!$po_request->config->get("dont_enforce_access_settings")) && $pt_representation && is_array(
            $va_download_access_settings
...
function caCreateLinksFromText(
    ...
    foreach ($pa_text as $vn_i => $vs_text) {
        ...
        if (sizeof($va_l_tags)) {
            ...
            $vs_content = preg_replace_callback(
                "/(&#[0-9]+;)/",
                function ($m) {
            ...
            foreach ($va_l_tags as $va_l) {
                if ($vb_can_handle_target) {
                    ...
                    switch (__CA_APP_TYPE__) {
                        case 'PROVIDENCE':
                            $vs_link_text = caEditorLink(
                                $g_request,
                                $va_l['content'],
                                $ps_class,
                                $ps_table_name,
                                $pa_row_ids[$vn_i],
                                ($pb_add_rel ? array('rel' => true) : array()),
                                null,
                                $va_link_opts
                            );
                            break;
                        case 'PAWTUCKET':
                            $vs_link_text = caDetailLink(
                                $g_request,
                                $va_l['content'],
                                $ps_class,
                                $ps_table_name,
            ...
            if ($vb_can_handle_target) {
                ...
                switch (__CA_APP_TYPE__) {
                    case 'PROVIDENCE':
                        $va_links[$vn_i] = ($vs_link = caEditorLink(
                            $g_request,
                            $vs_text,
                            $ps_class,
                            $ps_table_name,
                        ...
                        break;
                    case 'PAWTUCKET':
                        $va_links[$vn_i] = ($vs_link = caDetailLink(
                            $g_request,
                            $vs_text,
                            $ps_class,
                            $ps_table_name,
    ...
    if (!$vs_template) {
        if (!trim(
            ...
            if (is_array(
                ...
                if (!($vs_lookup_delimiter = $pt_subject->getAppConfig()->get(
                    "{$ps_related_table}_lookup_delimiter"
                ))) {
                    $vs_lookup_delimiter = '';
                }
...
function caEditorBundleShowHideControl(
    ...
    $vs_buf .= "<a href='#' onclick='caBundleVisibilityManager.toggle(\"{$ps_id_prefix}\");  return false;'>" . caNavIcon(
            __CA_NAV_ICON_VISIBILITY_TOGGLE__,
            '18px',
            array('id' => "{$ps_id_prefix}VisToggleButton")
        ) . "</a>";
    $vs_buf .= "</span>\n";
...
function caEditorBundleMetadataDictionary($po_request, $ps_id_prefix, $pa_settings)
{
    ...
    $vs_buf = '';
    $vs_buf .= "<span class='iconButton'>";
    $vs_buf .= "<a href='#' class='caMetadataDictionaryDefinitionToggle' onclick='caBundleVisibilityManager.toggleDictionaryEntry(\"{$ps_id_prefix}\");  return false;'>" . caNavIcon(
            __CA_NAV_ICON_INFO__,
            1,
            array('id' => "{$ps_id_prefix}MetadataDictionaryToggleButton")
        ) . "</a>";
...
function caEditorBundleSortControls($po_request, $ps_id_prefix, $ps_table, $pa_options = null)
{
    ...
    if (!is_array(
            $va_sort_fields = caGetAvailableSortFields(
                $ps_table,
                null,
...
function caReturnToHomeLocationControlForRelatedBundle(
    ...
    $vs_buf .= "<div id='{$ps_id_prefix}_editor_bundle_return_to_home_controls' class='editorBundleReturnToHomeControls'>" .
        ca_storage_locations::getHistoryTrackingChronologyInterstitialElementAddHTMLForm(
            $po_request,
            $ps_id_prefix,
            $pt_related->tableName(),
            $settings,
            [
                'placement_code' => $ps_id_prefix,
                'noTemplate' => true
            ]
        ) .
        caJSButton(
            ...
            ['onclick' => "caReturnToHomeLocation{$ps_id_prefix}(); return false;"],
            ['size' => '15px']
        ) .
        "</div>\n";
    $vs_buf .= "
    ...
    foreach ($va_tags as $va_tag) {
        ...
        switch (strtolower($va_tag_bits[0])) {
            case 'sum':
                ...
                $vs_val = caProcessBottomLineTemplateForPlacement(
                    $po_request,
                    $va_placements,
                    $pr_res,
                    [
...
function caProcessBottomLineTemplateForPlacement($po_request, $pa_placement, $pr_res, $pa_options = null)
{
    ...
    foreach ($pa_placements as $pa_placement) {
        ...
        if ($vn_datatype == 0) {    // container
            foreach ($va_tags as $vs_raw_tag) {
                ...
                $va_subelements_to_process["{$vs_bundle_name}.{$vs_subelement}"] = ca_metadata_elements::getElementDatatype(
                    $vs_subelement
                );
            }
        } else {
        ...
        while ($pr_res->nextHit()) {
            foreach ($va_subelements_to_process as $vs_subelement => $vn_subelement_datatype) {
                ...
                switch ($vn_subelement_datatype) {
                    case 6:        // currency
                        $va_values = $pr_res->get(
                            $vs_subelement,
                            array(
                                'returnAsDecimalWithCurrencySpecifier' => true,
                                'returnAsArray' => true
                            )
                        );
                        ...
                        if (is_array($va_values)) {
                            foreach ($va_values as $vs_value) {
                                $vn_value = (float)caConvertCurrencyValue(
                                    $vs_value,
                                    $vs_user_currency,
    ...
    foreach ($va_subelements_to_process as $vs_subelement => $vn_subelement_datatype) {
        ...
        switch ($vn_subelement_datatype) {
            case 6:        // currency
                $va_tag_values[$vs_value_name]['PAGEAVG'] = ($vn_page_len > 0) ? sprintf(
                    "%1.2f",
                    $va_tag_values[$vs_value_name]['PAGESUM'] / $vn_page_len
                ) : 0;
                $va_tag_values[$vs_value_name]['AVG'] = ($vn_c > 0) ? sprintf(
                    "%1.2f",
                    $va_tag_values[$vs_value_name]['SUM'] / $vn_c
                ) : "0.00";
            ...
            case 8:        // length
                ...
                foreach ($va_tag_values[$vs_value_name] as $vs_tag => $vn_val) {
                    ...
                    $va_tag_values[$vs_value_name][$vs_tag] = $vo_measurement->convertTo(
                        ($g_ui_units_pref == 'metric') ? Zend_Measure_Length::METER : Zend_Measure_Length::FEET,
                        4
                    );
                }
            ...
            case 9:        // weight
                ...
                foreach ($va_tag_values[$vs_value_name] as $vs_tag => $vn_val) {
                    ...
                    $va_tag_values[$vs_value_name][$vs_tag] = $vo_measurement->convertTo(
                        ($g_ui_units_pref == 'metric') ? Zend_Measure_Weight::KILOGRAM : Zend_Measure_Weight::POUND,
                        4
                    );
                }
...
function caRepresentationViewer($po_request, $po_data, $pt_subject, $pa_options = null)
{
    ...
    $ps_annotation_display_template = caGetOption(
        'displayAnnotationTemplate',
        $pa_options,
        caGetOption(
            'displayAnnotationTemplate',
            $va_detail_config['options'],
    ...
    $o_view->setVar(
        'context',
        ($vs_context = $po_request->getParameter(
            'context',
            pString
    ...
    while ($vo_data->nextHit()) {
        if (!($vn_representation_id = $vo_data->get(
        ...
        if (sizeof($va_rep_ids) > 0) {
            ...
            $va_rep_tags = $qr_reps->getRepresentationViewerHTMLBundles(
                $po_request,
                $pt_subject,
                array_merge(
                    $pa_options,
            ...
            while ($qr_reps->nextHit()) {
                ...
                if ($vn_rep_id == $vn_primary_id) {
                    $vn_index = 0;
                } elseif (!($vn_index = (int)$qr_reps->get(
                    RepresentableBaseModel::getRepresentationRelationshipTableName($qr_reps->tableName()) . '.rank'
                ))) {
            ...
            if (!$po_request->config->get("disable_lightbox")) {
                ...
                if ($po_request->isLoggedIn()) {
                    $vs_tool_bar .= " <a href='#' onclick='caMediaPanel.showPanel(\"" . caNavUrl(
                            $po_request,
                            '',
                            'Lightbox',
                            'addItemForm',
                            array(
                                $pt_subject->primaryKey() => $pt_subject->getPrimaryKey()
                            )
            ...
            $vs_placeholder = "<div class='detailMediaPlaceholder'>" . caGetPlaceholder(
                    $pt_subject->getTypeCode(),
                    "placeholder_large_media_icon"
                ) . "</div>" . $vs_tool_bar;
        }
    }
...
function caRepToolbar($po_request, $pt_representation, $pt_subject, $pa_options = null)
{
    ...
    $va_rep_display_info = caGetMediaDisplayInfo(
        $ps_display_type,
        $pt_representation->getMediaInfo('media', 'INPUT', 'MIMETYPE')
    );
    $va_rep_display_info['poster_frame_url'] = $pt_representation->getMediaUrl(
        'media',
    ...
    if (!caGetOption(['no_overlay'], $va_rep_display_info, false)) {
        $vs_tool_bar .= "<a href='#' class='zoomButton' onclick='caMediaPanel.showPanel(\"" . caNavUrl(
                $po_request,
                '',
                'Detail',
                'GetMediaOverlay',
    ...
    if (($ps_table == "ca_objects") && is_array($va_add_to_set_link_info) && sizeof($va_add_to_set_link_info)) {
        $vs_tool_bar .= " <a href='#' class='setsButton' onclick='caMediaPanel.showPanel(\"" . caNavUrl(
                $po_request,
                '',
                $va_add_to_set_link_info['controller'],
                'addItemForm',
                array(
                    'context' => $ps_context,
                    (is_object(
                            $pt_subject
                        ) && $pt_subject->primaryKey()) ? $pt_subject->primaryKey() : "object_id" => $pn_subject_id
                )
            ) . "\"); return false;' title='" . $va_add_to_set_link_info['link_text'] . "'>" . $va_add_to_set_link_info['icon'] . "</a>\n";
    }
...
function caGetMediaAnnotationList($po_data, $pa_options = null)
{
    ...
    if (
        is_array(
            $va_annotations = $po_data->get(
                'ca_representation_annotations.annotation_id',
        ...
        while ($qr_annotations->nextHit()) {
            ...
            $va_annotation_list[] = "<a href='#' onclick='caUI.mediaPlayerManager.seek(\"caMediaOverlayTimebased_{$vn_rep_id}_{$ps_display_type}\", " . ((float)$qr_annotations->getPropertyValue(
                        'startTimecode',
                        true
                    ) - (float)$va_props['timecode_offset']) . ") return false;'>" . $qr_annotations->getWithTemplate(
                    $ps_annotation_display_template
                ) . "</a>";
        }
    }
...
function caGetMediaViewerHTML($po_request, $ps_identifier, $pt_subject, $pa_options = null)
{
    ...
    switch ($va_identifier['type']) {
        case 'representation':
            ...
            if ($pb_inline) {
                $vs_caption = ($vs_template = caGetOption(
                    'captionTemplate',
                    $pa_options,
            ...
            if (!$pn_subject_id) {
                if (is_array(
                        $va_subject_ids = $t_instance->get(
                            $pt_subject->tableName() . '.' . $pt_subject->primaryKey(),
                            array('returnAsArray' => true)
                        )
            ...
            if (($ps_display_annotations) && (is_array(
                    $va_annotation_list = caGetMediaAnnotationList($t_instance, $pa_options)
                ))) {
                $vs_viewer .= join("<br/>\n", $va_annotation_list);
            }
            return $vs_viewer;
...
function caGetMediaViewerData($po_request, $ps_identifier, $pt_subject, $pa_options = null)
{
    ...
    switch ($va_identifier['type']) {
        case 'representation':
            ...
            return $vs_viewer_name::getViewerData(
                $po_request,
                $ps_identifier,
                [
                    't_subject' => $pt_subject,
...
function caSearchMediaData($po_request, $ps_identifier, $pt_subject, $pa_options = null)
{
    ...
    switch ($va_identifier['type']) {
        case 'representation':
            ...
            return $vs_viewer_name::searchViewerData(
                $po_request,
                $ps_identifier,
                [
                    't_subject' => $pt_subject,
...
function caMediaDataAutocomplete($po_request, $ps_identifier, $pt_subject, $pa_options = null)
{
    ...
    switch ($va_identifier['type']) {
        case 'representation':
            ...
            return $vs_viewer_name::autocomplete(
                $po_request,
                $ps_identifier,
                [
                    't_subject' => $pt_subject,
...
function caRepresentationViewerHTMLBundles($po_request, $po_data, $pt_subject, $pa_options = null)
{
    ...
    while ($po_data->nextHit()) {
        $va_representation_ids = $po_data->get(
            'ca_object_representations.representation_id',
            ['returnAsArray' => true]
        );
        ...
        foreach ($va_representation_ids as $vn_representation_id) {
            ...
            $va_display_info = caGetMediaDisplayInfo($ps_display_type, $vs_mimetype);
            if ($pt_subject && ($vn_use_universal_viewer_for_image_list_length = caGetOption(
                    'use_universal_viewer_for_image_list_length_at_least',
                    $va_display_info,
                    null
                ))) {
            ...
            $vs_tool_bar = caGetOption('noToolBar', $pa_options, false) ? "" : caRepToolbar(
                $po_request,
                $t_instance,
                $pt_subject,
                array(
            ...
            $vs_caption = (isset($pa_options["captionTemplate"]) && $pa_options["captionTemplate"]) ? $po_data->getWithTemplate(
                $pa_options["captionTemplate"]
            ) : "";

            $va_reps[$vn_rep_id = $po_data->get(
                'ca_object_representations.representation_id'
            )] = "<div data-representation_id='{$vn_rep_id}' class='repViewerContCont'><div id='cont{$vn_rep_id}' class='repViewerCont'>" . $vs_viewer_name::getViewerHTML(
                    $po_request,
...
function caDragAndDropSortingForHierarchyEnabled($pt_request, $ps_table, $pn_id = null)
{
    ...
    if (!$pt_request->isLoggedIn() || (!$pt_request->user->canDoAction(
                "can_edit_{$ps_table}"
            ) && (($vs_hier_table = $t_instance->getProperty(
                'HIERARCHY_DEFINITION_TABLE'
    ...
    if ($vs_def_table_name && ($t_def = Datamodel::getInstanceByTableName($vs_def_table_name, true)) && ($t_def->load(
            $t_instance->get($vs_def_id_fld)
        )) && ($t_def->hasField('default_sort')) && ((int)$t_def->get('default_sort') === __CA_LISTS_SORT_BY_RANK__)) {
        return true;
    } else {
    ...
    if ($ps_table == 'ca_list_items') {
        ...
        foreach ($va_list_of_lists as $vn_list_id => $va_lists_by_locale) {
            foreach ($va_lists_by_locale as $vn_locale_id => $va_item) {
                $va_map[$va_item['root_id']] = caDragAndDropSortingForHierarchyEnabled(
                    $pt_request,
                    'ca_list_items',
                    $va_item['root_id']
                );
        ...
        if (preg_match_all("!\[{$vs_ref_tag} ([^\]]+)\]([^\[]+)\[/{$vs_ref_tag}\]!", $ps_text, $va_matches)) {
            foreach ($va_matches[1] as $i => $vs_attr_string) {
                if (sizeof($va_vals = caParseAttributes($vs_attr_string, ['id', 'idno', 'class', 'version'])) > 0) {
                    ...
                    $va_idnos[$vs_ref_type][$va_matches[0][$i]] = array_filter(
                        $va_vals,
                        function ($v) {
                            return !is_null($v);
                        }
                            ...
                            $ps_text = str_replace(
                                ...
                                $t_val->getMediaTag(
                                    ...
                                    caGetOption(
                                        'version',
                                        $va_tag,
                                        array_shift($t_val->getMediaVersions('value_blob'))
                                    )
                                ),
                                $ps_text
                            );
                ...
                case 'ca_site_page_media':
                    foreach ($va_idnos[$vs_ref_type] as $vs_tag => $va_l) {
                        ...
                        while ($qr_m->nextHit()) {
                            ...
                            $alt_text = caGetOption(
                                ['caption', 'title', 'idno'],
                                ['caption' => $caption, 'title' => $title, 'idno' => $idno],
                                null
                            );
                                ...
                                $vs_template = str_replace(
                                    ...
                                    $qr_m->getMediaTag(
                                        ...
                                        caGetOption(
                                            'version',
                                            $va_l,
                                            array_shift($qr_m->getMediaVersions('media'))
                                        ),
                                        ['alt' => $alt_text]
                                    ),
                                    $vs_template
                    ...
                    break;
                default:
                    $va_map = call_user_func(
                        $vs_ref_type . '::getIDsForIdnos',
                        array_map(
                            function ($v) {
                                return $v['idno'];
                            },
                    ...
                    $va_idnos[$vs_ref_type] = array_map(
                        function ($v) use ($va_map) {
                            $v['id'] = $va_map[$v['idno']];
                            return $v;
                        },
                        $va_tags
            ...
            foreach ($va_types as $vs_type => $va_config) {
                switch ($vs_table) {
                    ...
                    case 'ca_objects_x_storage_locations':
                        $va_bundle_settings["{$vs_table}_showRelationshipTypes"][] = $t_rel_type->getRelationshipTypeID(
                    ...
                    default:
                        ...
                        $va_bundle_settings["{$vs_table}_showTypes"][] = array_shift(
                            caMakeTypeIDList($vs_table, array($vs_type))
                        );
...
function caGetRepresentationDownloadFileName(string $table, array $data, ?array $options = null): string
{
    ...
    switch ($mode = caGetOption(
        ...
        case 'idno_and_version':
            $filename = $data['idno'] . '_' . $data['version'] . '_' . (strlen(
                    $data['index']
                ) ? '_' . $data['index'] : '') . '.' . $data['extension'];
            break;
