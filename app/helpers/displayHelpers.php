 ...
 *
 * @return Array - returns an associative array defining which locales should be used when displaying values; suitable
 *               for use with caExtractValuesByLocale()
 */
...
function caGetUserLocaleRules( $ps_item_locale = null, $pa_preferred_locales = null ) {
	...
	if ( is_array( $va_default_locales ) ) {
		foreach ( $va_default_locales as $vs_fallback_locale ) {
			$va_similar_locales = array_merge( $va_similar_locales,
				ca_locales::localesForLanguage( $vs_fallback_locale, [ 'codesOnly' => true ] ) );
			if ( ! isset( $va_preferred_locales[ $vs_fallback_locale ] )
			     || ! $va_preferred_locales[ $vs_fallback_locale ]
			) {
	...
	$va_fallback_locales = array_filter( $va_fallback_locales,
		function ( $v, $k ) use ( $ps_item_locale, $va_fallback_locales, $va_preferred_locales ) {
			return ! isset( $va_preferred_locales[ $k ] ) && ( $k !== $ps_item_locale );
		}, ARRAY_FILTER_USE_BOTH );

	$va_rules = array(
		'preferred' => $va_preferred_locales,
		/* all of these locales will display if available */
		'fallback'  => $va_fallback_locales
		/* the first of these that is available will display, but only if none of the preferred locales are available */
	);
...
/**
 *
 * @param $pa_locale_rules            - Associative array defining which locales to extract, and how to fall back to
 *                                    alternative locales should your preferred locales not exist in $pa_values
 * @param $pa_values                  - Associative array keyed by unique item_id and then locale code (eg. en_US) or
 *                                    locale_id; the values can be anything - string, numbers, objects, arrays, etc.
 * @param $pa_options                 [optional] - Associative array of options; available options are:
 *                                    'returnList' = return an indexed array of found values rather than an associative
 *                                    array keys on unique item_id [default is false]
 *                                    'debug' = print debugging information [default is false]
 *
 * @return Array - an array of found values keyed by unique item_id; or an indexed list of found values if option
 *               'returnList' is passed in $pa_options
 */
...
# ------------------------------------------------------------------------------------------------
function caExtractValuesByUserLocale(
	$pa_values, $ps_item_locale = null, $pa_preferred_locales = null, $pa_options = null
) {
	return caExtractValuesByLocale( caGetUserLocaleRules( $ps_item_locale, $pa_preferred_locales ), $pa_values,
		$pa_options );
}

# ------------------------------------------------------------------------------------------------
/**
 * Takes the output of BaseModel->getHierarchyAncestors() and tries to extract the appropriate values for the current
 * user's locale. This is designed for the common case where you want to get a list of ancestors with their labels in
 * the appropriate language, so you call getHierarchyAncestors() with the 'getHierarchyAncestors' option set to the
 * label table. What you get back is a simple list where each item is a node with the table fields + the label fields;
 * if a node has labels in several languages then you'll get back dupes - one for each language.
 *
 * This function takes that list with dupes and returns an array key'ed upon the primary key containing a single entry
 * for each node and the label set to the appropriate language - no dupes!
 *
 ...
 */
function caExtractValuesByUserLocaleFromHierarchyAncestorList(
	$pa_list, $ps_primary_key_name, $ps_label_display_field, $ps_use_if_no_label_field, $ps_default_text = '???'
) {
	if ( ! is_array( $pa_list ) ) {
		return array();
	}
	...
	foreach ( $pa_list as $vn_i => $va_item ) {
		if ( ! isset( $va_item[ $ps_label_display_field ] ) || ! $va_item[ $ps_label_display_field ] ) {
			if ( ! isset( $va_item[ $ps_use_if_no_label_field ] )
			     || ! ( $va_item[ $ps_label_display_field ]
					= $va_item[ $ps_use_if_no_label_field ] )
			) {
...
# ------------------------------------------------------------------------------------------------
function caExtractValuesByUserLocaleFromHierarchyChildList(
	$pa_list, $ps_primary_key_name, $ps_label_display_field, $ps_use_if_no_label_field, $ps_default_text = '???'
) {
	if ( ! is_array( $pa_list ) ) {
		return array();
 ...
 *
 */
function caDeleteWarningBox(
	$po_request, $t_instance, $ps_item_name, $ps_module_path, $ps_controller, $ps_cancel_action, $pa_parameters
) {
	...
	$vs_output             = caFormTag( $po_request, 'Delete', 'caDeleteForm', null, 'post', 'multipart/form-data',
		'_top', array( 'noCSRFToken' => false, 'disableUnsavedChangesWarning' => true ) );
	$vs_output             .= "<div class='delete-control-box'>" . caFormControlBox(
			...
			caFormNavButton( $po_request, __CA_NAV_ICON_CANCEL__, _t( "Cancel" ), '', $ps_module_path, $ps_controller,
				$ps_cancel_action, $pa_parameters )
		) . "</div>\n";
 ...
 *
 */
function caDeleteMultipleWarningBox(
	$po_request, $t_instance, $row_ids, $message, $ps_module_path, $ps_controller, $ps_cancel_action, $pa_parameters
) {
	if ( $vs_warning = isset( $pa_parameters['warning'] ) ? $pa_parameters['warning'] : null ) {
		$vs_warning = '<br/>' . $vs_warning;
	}

	$vs_output = caFormTag( $po_request, 'Delete', 'caDeleteForm', null, 'post', 'multipart/form-data', '_top',
		array( 'noCSRFToken' => false, 'disableUnsavedChangesWarning' => true ) );
	$vs_output .= "<div class='delete-control-box'>" . caFormControlBox(
...
function caDeleteRemapper( $po_request, $t_instance ) {
	...
	switch ( $vs_instance_table ) {
		case 'ca_relationship_types':
			...
			if ( $vn_rel_count == 1 ) {
				$va_reference_to_buf[] = _t( "Type is used by %1 %2", $vn_rel_count,
						$t_rel_instance->getProperty( 'NAME_PLURAL' ) ) . "<br>\n";
			} else {
				$va_reference_to_buf[] = _t( "Type is used by %1 %2", $vn_rel_count,
						$t_rel_instance->getProperty( 'NAME_PLURAL' ) ) . "<br>\n";
			}
		...
		default:
			...
			foreach ( $va_tables as $vs_table ) {
				...
				if ( $vn_c == 1 ) {
					$va_reference_to_buf[] = _t( "Has %1 relationship to %2", $vn_c,
							caGetTableDisplayName( $vs_table, true ) ) . "<br>\n";
				} else {
					$va_reference_to_buf[] = _t( "Has %1 relationships to %2", $vn_c,
							caGetTableDisplayName( $vs_table, true ) ) . "<br>\n";
				}
			...
			if ( is_array( $va_references_from = $t_instance->getAuthorityElementList() )
			     && sizeof( $va_references_from )
			) {
				foreach ( $va_references_from as $va_ref ) {
					if ( ! ( $t_element = ca_metadata_elements::getInstance( $va_ref['hier_element_id'] ) ) ) {
						continue;
					}
					$va_reference_from_buf[] = _t( ( $va_ref['count'] == 1 ) ? "%1 reference in %2"
						: "%1 references in %2", $va_ref['count'], $t_element->getLabelForDisplay() );
			...
			if ( $t_instance->isHierarchical()
			     && is_array( $va_children = call_user_func( $t_instance->tableName() . "::getHierarchyChildrenForIDs",
	...
	if ( ! in_array(
		$vs_default = strtolower( $po_request->user->getPreference( 'cataloguing_delete_reference_handling_default' ) ),
		[ 'transfer', 'remap', 'remove', 'delete' ] )
	) {
		if ( ! in_array(
			$vs_default = strtolower( $o_config->get( "{$vs_instance_table}_delete_reference_handling_default" ) ),
			[ 'transfer', 'remap', 'remove', 'delete' ] )
		) {
			if ( ! in_array( $vs_default = strtolower( $o_config->get( 'delete_reference_handling_default' ) ),
				[ 'transfer', 'remap', 'remove', 'delete' ] )
			) {
				$vs_default = null;
			}
	...
	if ( sizeof( $va_reference_to_buf ) ) {
		...
		if ( $vn_reference_to_count == 1 ) {
			$vs_output .= "<h3 id='caReferenceHandlingToCount'>" . _t( 'This %1 is referenced %2 time', $vs_typename,
					$vn_reference_to_count ) . ". " . _t( 'When deleting this %1:', $vs_typename ) . "</h3>\n";
		} else {
			$vs_output .= "<h3 id='caReferenceHandlingToCount'>" . _t( 'This %1 is referenced %2 times', $vs_typename,
					$vn_reference_to_count ) . ". " . _t( 'When deleting this %1:', $vs_typename ) . "</h3>\n";
		}

		$va_delete_opts       = [ 'value' => 'delete', 'id' => 'caReferenceHandlingToDelete' ];
		$va_remap_opts        = [ 'value' => 'remap', 'id' => 'caReferenceToHandlingRemap' ];
		$va_remap_lookup_opts = [
			'value' => '',
			'size'  => 40,
			'id'    => 'caReferenceHandlingToRemapTo',
			'class' => 'lookupBg'
		];
		...
		$vs_output .= caHTMLRadioButtonInput( 'caReferenceHandlingTo', $va_delete_opts ) . ' '
		              . _t( 'remove all references' ) . "<br/>\n";

		if ( $vs_instance_table === 'ca_storage_locations' ) {
			AssetLoadManager::register( 'hierBrowser' );
			$vs_output .= caHTMLRadioButtonInput( 'caReferenceHandlingTo', $va_remap_opts ) . ' '
			              . _t( 'transfer references to' ) . ' ';
			...
			$vs_output .= caHTMLRadioButtonInput( 'caReferenceHandlingTo', $va_remap_opts ) . ' '
			              . _t( 'transfer references to' ) . ' ' . caHTMLTextInput( 'caReferenceHandlingToRemapTo',
					$va_remap_lookup_opts );
		}
		$vs_output .= "<a href='#' class='button' onclick='jQuery(\"#caReferenceHandlingToRemapToID\").val(\"\"); jQuery(\"#caReferenceHandlingToRemapTo\").val(\"\"); jQuery(\"#caReferenceHandlingToClear\").css(\"display\", \"none\"); return false;' style='display: none;' id='caReferenceHandlingToClear'>"
		              . _t( 'Clear' ) . '</a>';
		$vs_output .= caHTMLHiddenInput( 'caReferenceHandlingToRemapToID',
			array( 'value' => '', 'id' => 'caReferenceHandlingToRemapToID' ) );

		if ( $vn_child_count > 0 ) {
			$vs_output .= '<p class="formLabelWarning" id="caChildDeletionWarning"><i class="caIcon fa fa-info-circle fa-1x"></i> '
			              . _t( 'Child records will be deleted' ) . "</p>\n";
		}

		$vs_output .= "<script type='text/javascript'>";

		$va_service_info = caJSONLookupServiceUrl( $po_request, $t_instance->tableName(), array(
			'noSymbols' => 1,
			'noInline'  => 1,
			'exclude'   => (int) $t_instance->getPrimaryKey(),
			'table_num' => (int) $t_instance->get( 'table_num' )
		) );
	...
	jQuery(document).ready(function() {
		...
		TooltipManager::add( '#caReferenceHandlingToCount',
			"<h2>" . _t( 'References to this %1', $t_instance->getProperty( 'NAME_SINGULAR' ) ) . "</h2>\n"
			. join( "\n", $va_reference_to_buf ) );
	}
	...
	if ( sizeof( $va_reference_from_buf ) ) {
		...
		if ( $vn_reference_from_count == 1 ) {
			$vs_output .= "<h3 id='caReferenceHandlingFromCount'>" . _t( 'This %1 references %2 other item in metadata',
					$vs_typename, $vn_reference_from_count ) . ". " . _t( 'When deleting this %1:', $vs_typename )
			              . "</h3>\n";
		} else {
			$vs_output .= "<h3 id='caReferenceHandlingFromCount'>"
			              . _t( 'This %1 references %2 other items in metadata', $vs_typename,
					$vn_reference_from_count ) . ". " . _t( 'When deleting this %1:', $vs_typename ) . "</h3>\n";
		}
		...
		$va_remap_lookup_opts = [
			'value' => '',
			'size'  => 40,
			'id'    => 'caReferenceHandlingToRemapFrom',
			'class' => 'lookupBg'
		];
		if ( $vs_default === 'delete' ) {
			$va_delete_opts['checked']        = 1;
		...
		$vs_output .= caHTMLRadioButtonInput( 'caReferenceHandlingFrom', $va_delete_opts ) . ' '
		              . _t( 'remove these references' ) . "<br/>\n";
		$vs_output .= caHTMLRadioButtonInput( 'caReferenceHandlingFrom', $va_remap_opts ) . ' '
		              . _t( 'transfer these references and accompanying metadata to' ) . ' '
		              . caHTMLTextInput( 'caReferenceHandlingToRemapFrom', $va_remap_lookup_opts );
		$vs_output .= "<a href='#' class='button' onclick='jQuery(\"#caReferenceHandlingToRemapFromID\").val(\"\"); jQuery(\"#caReferenceHandlingToRemapFrom\").val(\"\"); jQuery(\"#caReferenceHandlingClear\").css(\"display\", \"none\"); return false;' style='display: none;' id='caReferenceHandlingClear'>"
		              . _t( 'Clear' ) . '</a>';
		$vs_output .= caHTMLHiddenInput( 'caReferenceHandlingToRemapFromID',
			array( 'value' => '', 'id' => 'caReferenceHandlingToRemapFromID' ) );
		$vs_output .= "<script type='text/javascript'>";
		...
		TooltipManager::add( '#caReferenceHandlingFromCount',
			"<h2>" . _t( 'References by this %1', $t_instance->getProperty( 'NAME_SINGULAR' ) ) . "</h2>\n"
...
function caGetMediaInfoForDisplay( $pm_media, $ps_version ) {
	...
	if ( isset( $va_media_info[ $ps_version ]['WIDTH'] ) && isset( $va_media_info[ $ps_version ]['HEIGHT'] ) ) {
		if ( ( $vn_w = $va_media_info[ $ps_version ]['WIDTH'] )
		     && ( $vn_h
				= $va_media_info[ $ps_version ]['WIDTH'] )
		) {
			$va_dimensions[] = $va_media_info[ $ps_version ]['WIDTH'] . 'p x ' . $va_media_info[ $ps_version ]['HEIGHT']
			                   . 'p';
		}
	}
	if ( isset( $va_media_info[ $ps_version ]['PROPERTIES']['bitdepth'] )
	     && ( $vn_depth
			= $va_media_info[ $ps_version ]['PROPERTIES']['bitdepth'] )
	) {
		$va_dimensions[] = $va_ret['bitdepth'] = intval( $vn_depth ) . ' bpp';
	}
	if ( isset( $va_media_info[ $ps_version ]['PROPERTIES']['colorspace'] )
	     && ( $vs_colorspace
			= $va_media_info[ $ps_version ]['PROPERTIES']['colorspace'] )
	) {
		$va_dimensions[] = $va_ret['colorspace'] = $vs_colorspace;
	}
	if ( isset( $va_media_info[ $ps_version ]['PROPERTIES']['resolution'] )
	     && is_array( $va_resolution = $va_media_info[ $ps_version ]['PROPERTIES']['resolution'] )
	) {
	...
	if ( isset( $va_media_info[ $ps_version ]['PROPERTIES']['duration'] )
	     && ( $vn_duration
			= $va_media_info[ $ps_version ]['PROPERTIES']['duration'] )
	) {
		$va_dimensions[] = $va_ret['duration'] = sprintf( "%4.1f", $vn_duration ) . 's';
	}
	if ( isset( $va_media_info[ $ps_version ]['PROPERTIES']['pages'] )
	     && ( $vn_pages
			= $va_media_info[ $ps_version ]['PROPERTIES']['pages'] )
	) {
		$va_dimensions[] = $va_ret['pages'] = $vn_pages . ' ' . ( ( $vn_pages == 1 ) ? _t( 'page' ) : _t( 'pages' ) );
	}
	if ( ! isset( $va_media_info[ $ps_version ]['PROPERTIES']['filesize'] )
	     || ! ( $vn_filesize
			= $va_media_info[ $ps_version ]['PROPERTIES']['filesize'] )
	) {
 ...
 *
 * @param $pa_metadata array - array key'ed by metadata system (eg. EXIF, DPX, IPTC) where values are arrays containing
...
function caFormatMediaMetadata( $pa_metadata ) {
	...
	if ( is_array( $pa_metadata ) && sizeof( $pa_metadata ) ) {
		foreach ( $pa_metadata as $vs_metadata_type => $va_metadata_data ) {
			if ( isset( $va_metadata_data ) && is_array( $va_metadata_data ) ) {
				$vs_buf .= "<tr><th>" . preg_replace( '!^METADATA_!', '', $vs_metadata_type )
				           . "</th><th colspan='2'><!-- empty --></th></tr>\n";
				foreach ( $va_metadata_data as $vs_key => $vm_value ) {
					$vs_buf .= "<tr valign='top'><td><!-- empty --></td><td>{$vs_key}</td><td>"
					           . _caFormatMediaMetadataArray( $vm_value, 0, $vs_key ) . "</td></tr>\n";
					$vn_metadata_rows ++;
				}
			}
		}
	...
	if ( ! $vn_metadata_rows ) {
		$vs_buf .= "<tr valign='top'><td colspan='3'>" . _t( 'No embedded metadata was extracted from the media' )
		           . "</td></tr>\n";
	}
	$vs_buf .= "</table>\n";
...
function _caFormatMediaMetadataArray( $pa_array, $pn_level = 0, $ps_key = null ) {
	...
	foreach ( $pa_array as $vs_key => $vs_val ) {
		...
		$vs_buf .= "<tr><td width='130'>{$vs_key}</td><td>" . _caFormatMediaMetadataArray( $vs_val, $pn_level + 1,
				$vs_key ) . "</td></tr>";
	}
	$vs_buf .= "</table></div>\n";
...
function caEditorFindResultNavigation( $po_request, $po_instance, $po_result_context, $pa_options = null ) {
	...
	if ( is_array( $va_found_ids ) && sizeof( $va_found_ids ) ) {
		if ( $vn_prev_id > 0 ) {
			if (
				$po_request->user->canAccess( $po_request->getModulePath(), $po_request->getController(), "Edit",
					array( $vs_pk => $vn_prev_id ) )
				&&
				! ( $po_request->getAppConfig()->get( $vs_table_name . '_editor_defaults_to_summary_view' ) )
			) {
				$vs_buf .= caNavLink( $po_request, caNavIcon( __CA_NAV_ICON_SCROLL_LT__, 2 ), 'prev record',
						$po_request->getModulePath(), $po_request->getController(),
						'Edit' . '/' . $po_request->getActionExtra(), array( $vs_pk => $vn_prev_id ) ) . '&nbsp;';
			} else {
				$vs_buf .= caNavLink( $po_request, caNavIcon( __CA_NAV_ICON_SCROLL_LT__, 2 ), 'prev record',
						$po_request->getModulePath(), $po_request->getController(), 'Summary',
						array( $vs_pk => $vn_prev_id ) ) . '&nbsp;';
			}
		...
		$vs_buf .= "<span class='resultCount'>" . ResultContext::getResultsLinkForLastFind( $po_request, $vs_table_name,
				$vs_back_text, '' ) . " (" . ( $vn_current_pos ) . "/" . sizeof( $va_found_ids ) . ")</span>";
		...
		if ( $vn_next_id > 0 ) {
			if (
				$po_request->user->canAccess( $po_request->getModulePath(), $po_request->getController(), "Edit",
					array( $vs_pk => $vn_next_id ) )
				&&
				! ( $po_request->getAppConfig()->get( $vs_table_name . '_editor_defaults_to_summary_view' ) )
			) {
				$vs_buf .= '&nbsp;' . caNavLink( $po_request, caNavIcon( __CA_NAV_ICON_SCROLL_RT__, 2 ), 'next record',
						$po_request->getModulePath(), $po_request->getController(),
						'Edit' . '/' . $po_request->getActionExtra(), array( $vs_pk => $vn_next_id ) );
			} else {
				$vs_buf .= '&nbsp;' . caNavLink( $po_request, caNavIcon( __CA_NAV_ICON_SCROLL_RT__, 2 ), 'next record',
						$po_request->getModulePath(), $po_request->getController(), 'Summary',
						array( $vs_pk => $vn_next_id ) );
...
function caSetupEditorScreenOverlays( $po_request, $pt_subject, $pa_bundle_list, $pa_options = null ) {
	...
	if ( $pt_subject && $pt_subject->isHierarchical() ) {
		$vs_buf .= caEditorHierarchyOverview( $po_request, $pt_subject->tableName(), $pt_subject->getPrimaryKey(),
			$pa_options );
	}
...
function caEditorFieldList( $po_request, $pt_subject, $pa_bundle_list, $pa_options = null ) {
	$vs_buf = "<script type=\"text/javascript\">
		jQuery(document).on('ready', function() {
			...
			if (typeof caBundleUpdateManager !== 'undefined') { caBundleUpdateManager = caUI.initBundleUpdateManager({url:'"
	          . caNavUrl( $po_request, '*', '*', 'reload' ) . "', screen:'" . $po_request->getActionExtra() . "', key:'"
	          . $pt_subject->primaryKey() . "', id: " . (int) $pt_subject->getPrimaryKey() . "}); }
...
<div id=\"editorFieldListHTML\">";
	if ( is_array( $pa_bundle_list ) ) {
		foreach ( $pa_bundle_list as $vs_anchor => $va_info ) {
			$vs_buf .= "<a href=\"#\" onclick=\"jQuery.scrollTo('a[name={$vs_anchor}]', {duration: 350, offset: -80 , onAfter : function(selector, data){jQuery(selector).parent('.bundleLabel').find('a:link').first().focus();}}); return false;\" class=\"editorFieldListLink\">"
			           . $va_info['name'] . "</a><br/>";
		}
	}
	$vs_buf .= "</div>\n";
		...
		jQuery(document).on('ready', function() {
			jQuery(document).on('keydown.ctrl_h', function() {
				...
				if (jQuery('#caHierarchyOverviewContentArea').html().length == 0) {
					url = '" . caNavUrl( $po_request, $po_request->getModulePath(), $po_request->getController(),
			'getHierarchyForDisplay', array( $t_subject->primaryKey() => $pn_id ) ) . "';
				}
...
function caEditorInspector( $po_view, $pa_options = null ) {
	...
	if ( ( $vn_item_id ) || ( $po_view->request->getAction() === 'Delete' ) ) {
		$vs_buf = '<h3 class="nextPrevious" ' . $vs_style . '>' . caEditorFindResultNavigation( $po_view->request,
				$t_item, $o_result_context, $pa_options ) . "</h3>\n";
	}
	...
	if ( ( $po_view->request->getAction() === 'Delete' )
	     && ( $po_view->request->getParameter( 'confirm', pInteger ) )
	) {
		...
		if ( $vn_item_id ) {
			...
			if ( $t_item->hasField( 'is_deaccessioned' ) && $t_item->get( 'is_deaccessioned' )
			     && ( $t_item->get( 'deaccession_date', array( 'getDirectDate' => true ) )
			          <= caDateToHistoricTimestamp( _t( 'now' ) ) )
			) {
				// If currently deaccessioned then display deaccession message
				$vs_buf .= "<br/><div class='inspectorDeaccessioned'>" . _t( 'Deaccessioned %1',
						$t_item->get( 'deaccession_date' ) ) . "</div>\n";
				if ( $vs_deaccession_notes = $t_item->get( 'deaccession_notes' ) ) {
					TooltipManager::add( ".inspectorDeaccessioned", $vs_deaccession_notes );
				}
			} else {
				if ( $po_view->request->user->canDoAction( 'can_see_current_location_in_inspector_'
				                                           . $vs_table_name )
				) {
					$is_home                 = $t_item->isInHomeLocation();
					$inspector_current_value = null;
					if ( method_exists( $t_item, "getHistory" )
					     && ( $inspector_current_value_label
							= $t_item->getInspectorHistoryTrackingDisplayPolicy( 'label' ) )
					) {
						if ( $inspector_current_value = $t_item->getCurrentValueForDisplay() ) {
							$vs_buf .= "<div class='inspectorCurrentLocation'><strong>{$inspector_current_value_label}:</strong><br/>{$inspector_current_value}"
							           . ( ( $is_home ) ? ' ' . _t( '[HOME]' ) : '' ) . "</div>";
						}
					}

					if ( ( ! $is_home || ! $inspector_current_value ) && $t_item->hasField( 'home_location_id' )
					     && ( $home_location_id = $t_item->get( 'home_location_id' ) )
					     && ( $t_home_loc = ca_storage_locations::find( $home_location_id,
							[ 'returnAs' => 'firstModelInstance' ] ) )
					) {
						if ( ! ( $template
							= $po_view->request->config->get( 'inspector_home_location_display_template' ) )
						) {
							$template = '^ca_storage_locations.hierarchy.preferred_labels.name%delimiter=_➜_';
						}
						$vs_buf .= "<div class='inspectorCurrentLocation'><strong>" . _t( 'Home location' )
						           . ":</strong><br/>" . $t_home_loc->getWithTemplate( $template ) . "</div>";
					}
				}
			}
			...
			//
			if ( is_array(
				$va_display_flags = $po_view->request->config->getAssoc( "{$vs_table_name}_inspector_display_flags" ) )
			) {
				...
				if ( ! ( $vs_display_flag_delim
					= $po_view->request->config->get( "{$vs_table_name}_inspector_display_flags_delimiter" ) )
				) {
			...
			if ( ! ( ( $vs_table_name === 'ca_objects' ) && $vb_dont_use_labels_for_ca_objects ) ) {
				if ( $vs_get_spec = $po_view->request->config->get( "{$vs_table_name}_inspector_display_title" ) ) {
					$vs_label = caProcessTemplateForIDs( $vs_get_spec, $vs_table_name,
						array( $t_item->getPrimaryKey() ) );
				} else {
					$va_object_collection_collection_ancestors
						= $po_view->getVar( 'object_collection_collection_ancestors' );
					if (
						...
						&& sizeof( $va_object_collection_collection_ancestors )
					) {
						$va_collection_links = array();
						foreach ( $va_object_collection_collection_ancestors as $va_collection_ancestor ) {
							$va_collection_links[] = caEditorLink( $po_view->request, $va_collection_ancestor['label'],
								'', 'ca_collections', $va_collection_ancestor['collection_id'] );
						}
					...
					if ( method_exists( $t_item, 'getLabelForDisplay' ) && ( $t_item->getLabelTableInstance() ) ) {
						...
						if ( $vn_parent_id && ( ( $vs_table_name != 'ca_places' ) || ( $vn_parent_index > 0 ) ) ) {
							...
							if ( $va_parent['NODE'][ $vs_disp_fld ]
							     && ( $vs_editor_link
									= caEditorLink( $po_view->request, $va_parent['NODE'][ $vs_disp_fld ], '',
									$vs_table_name, $va_parent['NODE'][ $t_item->primaryKey() ] ) )
							) {
								$vs_label .= $vs_editor_link . ' &gt; ' . $t_item->getLabelForDisplay();
							} else {
								$vs_label .= ( $va_parent['NODE'][ $vs_disp_fld ] ? $va_parent['NODE'][ $vs_disp_fld ]
								                                                    . ' &gt; ' : '' )
							...
							if ( ( $vs_table_name === 'ca_editor_uis' )
							     && ( in_array( $po_view->request->getAction(),
									array( 'EditScreen', 'DeleteScreen', 'SaveScreen' ) ) )
							) {
								$t_screen = new ca_editor_ui_screens( $po_view->request->getParameter( 'screen_id',
									pInteger ) );
						...
						$vs_label .= $t_item->hasField( 'name' ) ? $t_item->get( 'name' )
							: $t_item->get( array_shift( $t_item->getProperty( 'LIST_FIELDS' ) ) );
					}
				}
			}

			$vb_show_idno = ( ! $po_view->request->config->get( "{$vs_table_name}_inspector_dont_display_idno" ) )
			                && (bool) ( $vs_idno = $t_item->get( $t_item->getProperty( 'ID_NUMBERING_ID_FIELD' ) ) );
			...
			$vs_buf .= "<div class='recordTitle {$vs_table_name}' style='width:190px; overflow:hidden;'>{$vs_label}"
			           . ( ( $vb_show_idno ) ? "<a title='$vs_idno'>" . ( $vs_idno ? " ({$vs_idno})" : '' ) : "" )
			           . "</a></div>";
			if ( ( $vs_table_name === 'ca_object_lots' ) && $t_item->getPrimaryKey() ) {
				$vs_buf .= "<div id='inspectorLotMediaDownload'><strong>" . ( ( ( $vn_num_objects
							= $t_item->numObjects( null,
							[ 'excludeChildObjects' => $po_view->request->config->get( "exclude_child_objects_in_inspector_log_count" ) ] ) )
				                                                                == 1 ) ? _t( 'Lot contains %1 object',
			...
			if ( $po_view->request->config->get( "include_custom_inspector" ) ) {
				if ( file_exists( $po_view->request->getViewsDirectoryPath() . "/bundles/inspector_info.php" ) ) {
					$vo_inspector_view = new View( $po_view->request,
						$po_view->request->getViewsDirectoryPath() . "/bundles/" );
			...
			$vs_buf .= "<div class='creatingNew'>" . _t( "Creating new %1", $vs_type_name ) . " " . ( $vs_parent_name
					? _t( "%1 &gt; New %2", $vs_parent_name, $vs_type_name ) : '' ) . "</div>\n";
			$vs_buf .= "<br/>\n";
		}
		...
		if ( $t_item->getPrimaryKey() ) {
			if ( is_array( $va_reps ) && ( sizeof( $va_reps ) > 0 ) ) {
				...
				foreach ( $va_reps as $va_rep ) {
					...
					$va_imgs[] = "{url:'" . $va_rep['urls']['preview170'] . "', width: "
					             . $va_rep['info']['preview170']['WIDTH'] . ", height: " .
					             $va_rep['info']['preview170']['HEIGHT']
					             . ", link: '#', onclick:  'caMediaPanel.showPanel(\'" .
					             caNavUrl( $po_view->request, '*', '*', 'GetMediaOverlay', array(
						             $t_item->primaryKey() => $vn_item_id,
				...
				if ( sizeof( $va_reps ) > 1 ) {
					...
					<div class='leftScroll'>
						<a href='#' onclick='inspectorInfoRepScroller.scrollToPreviousImage(); return false;'>"
					           . caNavIcon( __CA_NAV_ICON_SCROLL_LT__, '16px' ) . "</a>
					</div>
		";
				}
		...
		";
					...
					<div class='rightScroll'>
						<a href='#' onclick='inspectorInfoRepScroller.scrollToNextImage(); return false;'>"
						           . caNavIcon( __CA_NAV_ICON_SCROLL_RT__, '16px' ) . "</a>
					</div>
		";
					}
				...
				if ( $vs_get_spec
					= $po_view->request->config->get( "{$vs_table_name}_inspector_display_below_media" )
				) {
					$vs_buf .= caProcessTemplateForIDs( $vs_get_spec, $vs_table_name,
						array( $t_item->getPrimaryKey() ) );
				}
			}
			...
			if ( $vs_additional_info
				= $po_view->request->config->get( "{$vs_table_name}_inspector_additional_info" )
			) {
				if ( is_array( $vs_additional_info ) ) {
					$vs_buf .= "<br/>";
					foreach ( $vs_additional_info as $vs_info ) {
						$vs_buf .= caProcessTemplateForIDs( $vs_info, $vs_table_name, array( $t_item->getPrimaryKey() ),
								array( 'requireLinkTags' => true ) ) . "<br/>\n";
					}
				} else {
					$vs_buf .= "<br/>" . caProcessTemplateForIDs( $vs_additional_info, $vs_table_name,
							array( $t_item->getPrimaryKey() ), array( 'requireLinkTags' => true ) ) . "<br/>\n";
				}
			}
			...
			if ( $vn_item_id ) {
				# --- watch this link
				$vs_watch = "";
				if ( in_array( $vs_table_name, array(
					'ca_objects',
					'ca_object_lots',
					'ca_entities',
					'ca_places',
					...
					$vs_watch     = "<div class='watchThis inspectorActionButton'><div><a href='#' title='"
					                . _t( 'Add/remove item to/from watch list.' )
					                . "' onclick='caToggleItemWatch(); return false;' id='caWatchItemButton'>"
					                . caNavIcon( $t_watch_list->isItemWatched( $vn_item_id, $t_item->tableNum(),
							$po_view->request->user->get( "user_id" ) ) ? __CA_NAV_ICON_UNWATCH__
							: __CA_NAV_ICON_WATCH__, '20px' ) . "</a></div></div>";

					$vs_buf .= "\n<script type='text/javascript'>
		function caToggleItemWatch() {
			var url = '" . caNavUrl( $po_view->request, $po_view->request->getModulePath(),
							$po_view->request->getController(), 'toggleWatch',
							array( $t_item->primaryKey() => $vn_item_id ) ) . "';
			
			jQuery.getJSON(url, {}, function(data, status) {
				if (data['status'] == 'ok') {
					jQuery('#caWatchItemButton').html((data['state'] == 'watched') ? '"
					           . addslashes( caNavIcon( __CA_NAV_ICON_UNWATCH__, '20px' ) ) . "' : '"
					           . addslashes( caNavIcon( __CA_NAV_ICON_WATCH__, '20px' ) ) . "');
				} else {
				...
				if ( $po_view->request->user->canDoAction( "can_change_type_{$vs_table_name}" )
					...
					$vs_buf .= "<div id='inspectorChangeType' class='inspectorActionButton'><div id='inspectorChangeTypeButton'><a href='#' onclick='caTypeChangePanel.showPanel(); return false;'>"
					           . caNavIcon( __CA_NAV_ICON_CHANGE__, '20px', array( 'title' => _t( 'Change type' ) ) )
					           . "</a></div></div>\n";

					$vo_change_type_view = new View( $po_view->request,
						$po_view->request->getViewsDirectoryPath() . "/bundles/" );
				...
				if ( $t_item->getPrimaryKey()
				     && $po_view->request->config->get( $vs_table_name . '_show_add_child_control_in_inspector' )
				) {
					$vb_show_add_child_control = true;
					if ( is_array( $va_restrict_add_child_control_to_types
							= $po_view->request->config->getList( $vs_table_name
							                                      . '_restrict_child_control_in_inspector_to_types' ) )
					     && sizeof( $va_restrict_add_child_control_to_types )
					) {
					...
					if ( $vb_show_add_child_control ) {
						if ( (bool) $po_view->request->config->get( $vs_table_name
						                                            . '_enforce_strict_type_hierarchy' )
						) {
							// strict menu
							$vs_type_list = $t_item->getTypeListAsHTMLFormElement( 'type_id',
								array( 'style' => 'width: 90px; font-size: 9px;' ), array(
									'childrenOfCurrentTypeOnly' => true,
									'directChildrenOnly'        => ( $po_view->request->config->get( $vs_table_name
									                                                                 . '_enforce_strict_type_hierarchy' )
									                                 == '~' ) ? false : true,
						...
						if ( $vs_type_list ) {
							$vs_buf .= "<div id='inspectorCreateChild' class='inspectorActionButton'><div id='inspectorCreateChildButton'><a href='#' onclick='caCreateChildPanel.showPanel(); return false;'>"
							           . caNavIcon( __CA_NAV_ICON_CHILD__, '20px',
									array( 'title' => _t( 'Create Child Record' ) ) ) . "</a></div></div>\n";

							$vo_create_child_view = new View( $po_view->request,
								$po_view->request->getViewsDirectoryPath() . "/bundles/" );
							...
							TooltipManager::add( "#inspectorCreateChildButton",
								_t( 'Create a child record under this one' ) );
						}
					}
				}
			}

			if ( $po_view->request->user->canDoAction( 'can_duplicate_' . $vs_table_name )
			     && $t_item->getPrimaryKey()
			) {
				$vs_buf .= "<div id='caDuplicateItemButton' class='inspectorActionButton'>";

				$vs_buf .= caFormTag( $po_view->request, 'Edit', 'DuplicateItemForm',
					$po_view->request->getModulePath() . '/' . $po_view->request->getController(), 'post',
					'multipart/form-data', '_top',
					array( 'noCSRFToken' => true, 'disableUnsavedChangesWarning' => true, 'noTimestamp' => true ) );
				$vs_buf .= "<div>" . caFormSubmitLink( $po_view->request,
						caNavIcon( __CA_NAV_ICON_DUPLICATE__, '20px' ), '', 'DuplicateItemForm' ) . "</div>";
			...
			if ( ( $vs_table_name == 'ca_objects' )
			     && $po_view->request->user->canDoAction( "can_set_home_location" )
			) {
				$vs_buf .= "<div id='inspectorSetHomeLocation' class='inspectorActionButton'><div id='inspectorSetHomeLocationButton'><a href='#' onclick='_initSetHomeLocationHierarchyBrowser(); return false;'>"
				           . caNavIcon( __CA_NAV_ICON_HOME__, '20px', array( 'title' => _t( 'Set home location' ) ) )
				           . "</a></div></div>\n";
			...
			if ( $vn_num_objects > 0 ) {
				$vs_buf .= "<div id='inspectorLotMediaDownloadButton' class='inspectorActionButton'>"
				           . caNavLink( $po_view->request, caNavIcon( __CA_NAV_ICON_DOWNLOAD__, '20px' ), "button",
						$po_view->request->getModulePath(), $po_view->request->getController(), 'getLotMedia',
						array( 'lot_id' => $t_item->getPrimaryKey(), 'download' => 1 ), array() ) . "</div>\n";
				TooltipManager::add( '#inspectorLotMediaDownloadButton',
					_t( "Download all media associated with objects in this lot" ) );
			}

			//
			// Download media in set
			if ( ( $vs_table_name == 'ca_sets' ) && ( sizeof( $t_item->getItemRowIDs() ) > 0 ) ) {
				$vs_buf .= "<div id='inspectorSetMediaDownloadButton' class='inspectorActionButton'>"
				           . caNavLink( $po_view->request, caNavIcon( __CA_NAV_ICON_DOWNLOAD__, '20px' ), "button",
						$po_view->request->getModulePath(), $po_view->request->getController(), 'getSetMedia',
						array( 'set_id' => $t_item->getPrimaryKey(), 'download' => 1 ), array() ) . "</div>\n";

				TooltipManager::add( '#inspectorSetMediaDownloadButton',
					_t( "Download all media associated with records in this set" ) );
			}
			...
			$t_set = new ca_sets();
			if ( is_array( $va_sets = caExtractValuesByUserLocale( $t_set->getSetsForItem( $t_item->tableNum(),
					$t_item->getPrimaryKey(),
					array( 'user_id' => $po_view->request->getUserID(), 'access' => __CA_SET_READ_ACCESS__ ) ) ) )
			     && sizeof( $va_sets )
			) {
				$va_links = array();
				...
				foreach ( $va_sets as $vn_set_id => $va_set ) {
					...
					$va_links[] = "<a {$class} href='" . caEditorUrl( $po_view->request, 'ca_sets', $vn_set_id ) . "'>"
					              . $va_set['name'] . "</a>";
				}
				$vs_more_info .= "<div><strong>" . ( ( sizeof( $va_links ) == 1 ) ? _t( "In set" ) : _t( "In sets" ) )
				                 . "</strong> " . join( ", ", $va_links ) . "</div>\n";
			}


			// export options
			if ( $vn_item_id && $vs_select = $po_view->getVar( 'available_mappings_as_html_select' ) ) {
				$vs_more_info .= "<div class='inspectorExportControls'>" . caFormTag( $po_view->request, 'exportItem',
						'caExportForm', null, 'post', 'multipart/form-data', '_top',
						array( 'noCSRFToken' => true, 'disableUnsavedChangesWarning' => true ) );
				$vs_more_info .= $vs_select;
				$vs_more_info .= caHTMLHiddenInput( $t_item->primaryKey(),
					array( 'value' => $t_item->getPrimaryKey() ) );
				$vs_more_info .= caHTMLHiddenInput( 'download', array( 'value' => 1 ) );
				$vs_more_info .= caFormSubmitLink( $po_view->request, _t( 'Export &rsaquo;' ), 'button',
					'caExportForm' );
			...
			if ( $va_creation['timestamp'] || $va_last_change['timestamp'] ) {
				...
				if ( $va_creation['timestamp'] ) {
					...
					$vs_interval = ( ( $vn_t = ( time() - $va_creation['timestamp'] ) ) == 0 ) ? _t( 'Just now' )
						: _t( '%1 ago', caFormatInterval( $vn_t, 2 ) );

					$vs_more_info .= "<div class='inspectorChangeDateListLine'  id='caInspectorCreationDate'>" .
					                 ( $vs_name ? _t( '<strong>Created</strong><br/>%1 by %2', $vs_interval, $vs_name )
						                 : _t( '<strong>Created</strong><br/>%1', $vs_interval ) ) .
					                 "</div>";

					TooltipManager::add( "#caInspectorCreationDate",
						"<h2>" . _t( 'Created on' ) . "</h2>" . _t( 'Created on %1',
							caGetLocalizedDate( $va_creation['timestamp'], array( 'dateFormat' => 'delimited' ) ) ) );
				}
				...
				if ( $va_last_change['timestamp'] && ( $va_creation['timestamp'] != $va_last_change['timestamp'] ) ) {
					...
					$vs_interval = ( ( $vn_t = ( time() - $va_last_change['timestamp'] ) ) == 0 ) ? _t( 'Just now' )
						: _t( '%1 ago', caFormatInterval( $vn_t, 2 ) );

					$vs_more_info .= "<div class='inspectorChangeDateListLine' id='caInspectorChangeDate'>" .
					                 ( $vs_name ? _t( '<strong>Last changed</strong><br/>%1 by %2', $vs_interval,
						                 $vs_name ) : _t( '<strong>Last changed</strong><br/>%1', $vs_interval ) ) .
					                 "</div>";

					TooltipManager::add( "#caInspectorChangeDate",
						"<h2>" . _t( 'Last changed on' ) . "</h2>" . _t( 'Last changed on %1',
							caGetLocalizedDate( $va_last_change['timestamp'],
								array( 'dateFormat' => 'delimited' ) ) ) );
				}

				if ( method_exists( $t_item, 'getMetadataDictionaryRuleViolations' )
				     && is_array( $va_violations = $t_item->getMetadataDictionaryRuleViolations() )
				     && ( ( $total_num_violations = ( sizeof( $va_violations ) ) ) > 0 )
				) {
					$screen_id = $po_view->request->getActionExtra();
					if ( ! $screen_id
					     || ! is_array(
							$violations_for_current_screen = $t_item->getMetadataDictionaryRuleViolations( null,
								[ 'screen_id' => $screen_id ] ) )
					) {
					...
					foreach ( $va_violations as $vn_violation_id => $va_violation ) {
						...
						if ( isset( $violations_for_current_screen[ $vn_violation_id ] ) ) {
							...
							$placements = $t_ui->getPlacementsForBundle( $va_violation['bundle_name'],
								$po_view->request, [] );
							if ( ! is_array( $placements ) || ! sizeof( $placements ) ) {
								$placements = $t_ui->getPlacementsForBundle( "ca_attribute_{$bundle}",
									$po_view->request, [] );
								if ( ! is_array( $placements ) || ! sizeof( $placements ) ) {
						...
						$va_violation_messages[] = "<li " . ( $in_this_tab
								? "class='caMetadataDictionaryViolationInCurrentTab'"
								: "class='caMetadataDictionaryViolationNotInCurrentTab'" )
						                           . "><em><u>{$vs_label}</u></em> "
						                           . caExtractSettingsValueByUserLocale( 'violationMessage',
								$va_violation ) . " {$screen}</li>";
					}
					...
					if ( $total_num_violations_for_current_screen > 0 ) {
						if ( $total_num_violations_for_current_screen != $total_num_violations ) {
							$vs_more_info .= "<div id='caInspectorViolationsList'>" . caNavIcon( __CA_NAV_ICON_ALERT__,
									"14px" ) . " " . ( $vs_num_violations_display
									= ( ( $total_num_violations_for_current_screen > 1 )
									? _t( '%1 problems on this tab require attention (of %2 total)',
										$total_num_violations_for_current_screen, $total_num_violations )
									: _t( '%1 problem on this tab requires attention (of %2 total)',
										$total_num_violations_for_current_screen, $total_num_violations ) ) )
							                 . "</div>\n";
						} else {
							$vs_more_info .= "<div id='caInspectorViolationsList'>" . caNavIcon( __CA_NAV_ICON_ALERT__,
									"14px" ) . " " . ( $vs_num_violations_display
									= ( ( $total_num_violations_for_current_screen > 1 )
									? _t( '%1 problems on this tab require attention', $total_num_violations )
									: _t( '%1 problem on this tab requires attention', $total_num_violations ) ) )
							                 . "</div>\n";
						}
					} else {
						$vs_more_info .= "<div id='caInspectorViolationsList'>" . caNavIcon( __CA_NAV_ICON_ALERT__,
								"14px" ) . " " . ( $vs_num_violations_display = ( ( $total_num_violations > 1 )
								? _t( '%1 problems require attention', $total_num_violations )
								: _t( '%1 problem requires attention', $total_num_violations ) ) ) . "</div>\n";
					}
					if ( $vs_num_violations_display ) {
						TooltipManager::add( "#caInspectorViolationsList",
							"<h2>{$vs_num_violations_display}</h2><ol>" . join( "\n", $va_violation_messages )
							. "</ol>\n" );
					}
				}
			...
			if ( $vs_get_spec = $po_view->request->config->get( "{$vs_table_name}_inspector_display_more_info" ) ) {
				$vs_more_info .= caProcessTemplateForIDs( $vs_get_spec, $vs_table_name,
					array( $t_item->getPrimaryKey() ) );
			}
			if ( $vs_more_info ) {
				$vs_buf .= "
	...
	<div id='inspectorInfo' >{$vs_more_info}</div>\n";
		...
		if ( ! $t_item->getPrimaryKey() ) { // only applies to new records
			...
			if ( $vs_rel_table && Datamodel::tableExists( $vs_rel_table ) && $vn_rel_type_id && $vn_rel_id ) {
				...
				if ( $t_rel && $t_rel->load( $vn_rel_id ) ) {
					$vs_buf .= '<strong>' . _t( "Will be related to %1", $t_rel->getTypeName() ) . '</strong>: '
		...
		if ( ( $vs_table_name === 'ca_objects' ) && ( $vn_lot_id ) ) {
			...
			if ( ( $t_lot->get( 'deleted' ) == 0 ) && ( in_array( $t_lot->get( 'type_id' ), $va_lot_lots ) ) ) {
				...
				if ( $vs_lot_displayname ) {
					if ( ! ( $vs_part_of_lot_msg
						= $po_view->request->config->get( "ca_objects_inspector_part_of_lot_msg" ) )
					) {
						$vs_part_of_lot_msg = _t( 'Part of lot' );
					}
					if ( ! ( $vs_will_be_part_of_lot_msg
						= $po_view->request->config->get( "ca_objects_inspector_will_be_part_of_lot_msg" ) )
					) {
						$vs_will_be_part_of_lot_msg = _t( 'Will be part of lot' );
					}
					$vs_buf .= "<br/><strong>" . ( $vb_is_currently_part_of_lot ? $vs_part_of_lot_msg
							: $vs_will_be_part_of_lot_msg ) . "</strong>: " . caNavLink( $po_view->request,
							$vs_lot_displayname, '', 'editor/object_lots', 'ObjectLotEditor', 'Edit',
		...
		$vb_can_add_component      = ( ( $vs_table_name === 'ca_objects' ) && $t_item->getPrimaryKey()
		                               && ( $po_view->request->user->canDoAction( 'can_create_ca_objects' ) )
		                               && $t_item->canTakeComponents() );

		if ( method_exists( $t_item, 'getComponentCount' ) ) {
			if ( $vn_component_count = $t_item->getComponentCount() ) {
				if ( $t_ui
				     && ( $vs_component_list_screen = $t_ui->getScreenWithBundle( "ca_objects_components_list",
						$po_view->request ) )
				     && ( $vs_component_list_screen !== $po_view->request->getActionExtra() )
				) {
					$vs_component_count_link = caNavLink( $po_view->request,
						( ( $vn_component_count == 1 ) ? _t( '%1 component', $vn_component_count )
							: _t( '%1 components', $vn_component_count ) ), '', '*', '*',
						$po_view->request->getAction() . '/' . $vs_component_list_screen,
						array( $t_item->primaryKey() => $t_item->getPrimaryKey() ) );
				} else {
					$vs_component_count_link = ( ( $vn_component_count == 1 ) ? _t( '%1 component',
						$vn_component_count ) : _t( '%1 components', $vn_component_count ) );
				}
		...
		if ( $vb_can_add_component ) {
			$vs_buf .= ' <a href="#" onclick=\'caObjectComponentPanel.showPanel("' . caNavUrl( $po_view->request, '*',
					'ObjectComponent', 'Form', array( 'parent_id' => $t_item->getPrimaryKey() ) )
			           . '"); return false;\')>' . caNavIcon( __CA_NAV_ICON_ADD__, '18px' ) . '</a>';
		...
		if ( ( $vs_table_name === 'ca_object_lots' ) && $t_item->getPrimaryKey() ) {
			...
			if ( is_array( $va_component_types ) && sizeof( $va_component_types ) ) {
				$vs_buf .= "<br/><strong>" . ( ( ( $vn_num_objects = $t_item->numObjects( null, array(
							'return'              => 'objects',
							'excludeChildObjects' => $po_view->request->config->get( "exclude_child_objects_in_inspector_log_count" )
						) ) ) == 1 ) ? _t( 'Lot contains %1 object', $vn_num_objects )
						: _t( 'Lot contains %1 objects', $vn_num_objects ) ) . "</strong>\n";
				$vs_buf .= "<br/><strong>" . ( ( ( $vn_num_components = $t_item->numObjects( null,
							array( 'return' => 'components' ) ) ) == 1 ) ? _t( 'Lot contains %1 component',
						$vn_num_components ) : _t( 'Lot contains %1 components', $vn_num_components ) ) . "</strong>\n";
			} else {
				$vs_buf .= "<br/><strong>" . ( ( ( $vn_num_objects = $t_item->numObjects( null,
							[ 'excludeChildObjects' => $po_view->request->config->get( "exclude_child_objects_in_inspector_log_count" ) ] ) )
				                                 == 1 ) ? _t( 'Lot contains %1 object', $vn_num_objects )
						: _t( 'Lot contains %1 objects', $vn_num_objects ) ) . "</strong>\n";
			}

			if ( ( (bool) $po_view->request->config->get( 'allow_automated_renumbering_of_objects_in_a_lot' ) )
			     && ( $va_nonconforming_objects = $t_item->getObjectsWithNonConformingIdnos() )
			) {

				$vs_buf .= '<br/><br/><em>' . ( ( ( $vn_c = sizeof( $va_nonconforming_objects ) ) == 1 )
						? _t( 'There is %1 object with non-conforming numbering', $vn_c )
						: _t( 'There are %1 objects with non-conforming numbering', $vn_c ) ) . "</em>\n";

				$vs_buf .= "<a href='#' onclick='jQuery(\"#inspectorNonConformingNumberList\").toggle(250); return false;'>"
				           . caNavIcon( __CA_NAV_ICON_ADD__, '18px' );
				...
				foreach ( $va_nonconforming_objects as $vn_object_id => $va_object_info ) {
					$vs_buf .= '<li>' . caEditorLink( $po_view->request, $va_object_info['idno'], '', 'ca_objects',
							$vn_object_id ) . "</li>\n";
				}
				$vs_buf .= "</ol></div>";
				$vs_buf .= caNavLink( $po_view->request, _t( 'Re-number objects' ) . ' &rsaquo;', 'button',
					$po_view->request->getModulePath(), $po_view->request->getController(), 'renumberObjects',
					array( 'lot_id' => $t_item->getPrimaryKey() ) );
				$vs_buf .= "</div>\n";
			}
			...
			if ( ! (bool) $po_view->request->config->get( 'disable_add_object_to_lot_inspector_controls' ) ) {
				...
				if ( (bool) $po_view->request->config->get( 'ca_objects_enforce_strict_type_hierarchy' ) ) {
					// strict menu
					$vs_buf .= _t( 'Add new %1 to lot', $t_object->getTypeListAsHTMLFormElement( 'type_id',
						array( 'id' => 'caAddObjectToLotForm_type_id' ), array(
							'childrenOfCurrentTypeOnly' => true,
							'directChildrenOnly'        => ( $po_view->request->config->get( 'ca_objects_enforce_strict_type_hierarchy' )
							                                 == '~' ) ? false : true,
							'returnHierarchyLevels'     => true,
							'access'                    => __CA_BUNDLE_ACCESS_EDIT__
						) ) );
					...
					// all types
					$vs_buf .= _t( 'Add new %1 to lot', $t_object->getTypeListAsHTMLFormElement( 'type_id',
						array( 'id' => 'caAddObjectToLotForm_type_id' ),
						array( 'access' => __CA_BUNDLE_ACCESS_EDIT__ ) ) );
				}

				$vs_buf .= "&nbsp;<a href='#' onclick='caAddObjectToLotForm()'>" . caNavIcon( __CA_NAV_ICON_ADD__,
						'16px' ) . '</a>';
				$vs_buf .= "</form></div>\n";

				$vs_buf .= "<script type='text/javascript'>
	function caAddObjectToLotForm() { 
		window.location='" . caEditorUrl( $po_view->request, 'ca_objects', 0, false,
						array( 'lot_id' => $t_item->getPrimaryKey(), 'rel' => 1, 'type_id' => '' ) ) . "' + jQuery('#caAddObjectToLotForm_type_id').val();
	}
	jQuery(document).ready(function() {
		...
		//
		if ( is_array( $va_show_counts_for = $po_view->request->config->getList( $t_item->tableName()
		                                                                         . '_show_related_counts_in_inspector_for' ) )
		     && sizeof( $va_show_counts_for )
		) {
			foreach ( $va_show_counts_for as $vs_rel_table ) {
				if ( ( $vn_count = (int) $t_item->getRelatedItems( $vs_rel_table, [ 'returnAs' => 'count' ] ) ) > 0 ) {
					$vs_buf .= caSearchLink( $po_view->request, _t( '%1 related %2', $vn_count,
							Datamodel::getTableProperty( $vs_rel_table,
								( $vn_count === 1 ) ? 'NAME_SINGULAR' : 'NAME_PLURAL' ) ), '', $vs_rel_table,
							$t_item->primaryKey( true ) . ":" . $t_item->getPrimaryKey() ) . "<br/>\n";
				}
			}
		}
		...
		if ( $vs_table_name === 'ca_object_representations' ) {
			foreach (
				...
				if ( sizeof( $va_objects = $t_item->getRelatedItems( $vs_rel_table ) ) ) {
					$vs_buf .= "<div><strong>" . _t( "Related %1",
							Datamodel::getTableProperty( $vs_rel_table, 'NAME_PLURAL' ) ) . "</strong>: <br/>\n";
					...
					foreach ( $va_objects as $vn_rel_id => $va_rel_info ) {
						if ( $vs_label = array_shift( $va_rel_info['labels'] ) ) {
							$vs_buf .= caEditorLink( $po_view->request,
									'&larr; ' . $vs_label . ' (' . $va_rel_info['idno'] . ')', '', $vs_rel_table,
									$va_rel_info[ Datamodel::primaryKey( $vs_rel_table ) ], array(), array(),
									array( 'action' => 'Edit' . ( $vs_screen ? "/{$vs_screen}" : "" ) ) ) . "<br/>\n";
						}
					}
					$vs_buf .= "</div>\n";
				}
		...
		if ( $vs_table_name === 'ca_representation_annotations' ) {
			if ( $vn_representation_id = $t_item->get( 'representation_id' ) ) {
				...
				$vs_buf .= caNavLink( $po_view->request, '&larr; ' . $t_rep->getLabelForDisplay(), '',
						'editor/object_representations', 'ObjectRepresentationEditor',
						'Edit/' . $po_view->getVar( 'representation_editor_screen' ),
						array( 'representation_id' => $vn_representation_id ) ) . '<br/>';
		...
		if ( $vs_table_name === 'ca_sets' ) {
			...
			if ( ( $vn_set_item_count > 0 )
			     && ( $po_view->request->user->canDoAction( 'can_batch_edit_'
			                                                . Datamodel::getTableName( $t_item->get( 'table_num' ) ) ) )
			) {
				$vs_buf .= caNavButton( $po_view->request, __CA_NAV_ICON_BATCH_EDIT__, _t( 'Batch edit' ),
					'editorBatchSetEditorLink', 'batch', 'Editor', 'Edit',
					array( 'set_id' => $t_item->getPrimaryKey() ), array(), array(
			...
			if ( $t_item->getPrimaryKey() ) {
				...
				$vs_buf .= "<strong>" . _t( "Type of content" ) . "</strong>: "
				           . caGetTableDisplayName( $vn_set_table_num ) . "<br/>\n";

				$vs_buf .= "</div>\n";

				if ( ! (bool) $po_view->request->config->get( 'ca_sets_disable_duplication_of_items' )
				     && $po_view->request->user->canDoAction( 'can_duplicate_items_in_sets' )
				     && $po_view->request->user->canDoAction( 'can_duplicate_' . $vs_set_table_name )
				) {
					$vs_buf .= '<div style="border-top: 1px solid #aaaaaa; margin-top: 5px; font-size: 10px; text-align: right;" ></div>';
					$vs_buf .= caFormTag( $po_view->request, 'DuplicateItems', 'caDupeSetItemsForm',
						'manage/sets/SetEditor', 'post', 'multipart/form-data', '_top',
						array( 'noCSRFToken' => true, 'disableUnsavedChangesWarning' => true ) );
					...
					$vs_buf .= caFormSubmitLink( $po_view->request, caNavIcon( __CA_NAV_ICON_GO__, "18px" ), "button",
						"caDupeSetItemsForm" );
					$vs_buf .= "</form>";
					$vs_buf .= '<div style="border-top: 1px solid #aaaaaa; margin-top: 5px; font-size: 10px; text-align: right;" ></div>';
				}
				...
				if ( $vn_set_table_num = $po_view->request->getParameter( 'table_num', pInteger ) ) {
					$vs_buf .= "<div><strong>" . _t( "Type of content" ) . "</strong>: "
					           . caGetTableDisplayName( $vn_set_table_num ) . "<br/>\n";

					$vs_buf .= "</div>\n";
				}
			}
			$t_user = new ca_users( ( $vn_user_id = $t_item->get( 'user_id' ) ) ? $vn_user_id
				: $po_view->request->getUserID() );
			if ( $t_user->getPrimaryKey() ) {
				$vs_buf .= "<div><strong>" . _t( 'Owner' ) . "</strong>: " . $t_user->get( 'fname' ) . ' '
				           . $t_user->get( 'lname' ) . "</div>\n";
			}

			if ( $po_view->request->user->canDoAction( 'can_export_' . $vs_set_table_name ) && $t_item->getPrimaryKey()
			     && ( sizeof( ca_data_exporters::getExporters( $vn_set_table_num ) ) > 0 )
			) {
				...
				$vs_buf .= "<a class='button' onclick='jQuery(\"#exporterFormList\").show();' style='text-align:right;' href='#'>"
				           . caNavIcon( __CA_NAV_ICON_EXPORT_SMALL__, '16px' ) . "</a>";

				$vs_buf .= caFormTag( $po_view->request, 'ExportData', 'caExportForm', 'manage/MetadataExport', 'post',
					'multipart/form-data', '_top',
					array( 'noCSRFToken' => true, 'disableUnsavedChangesWarning' => true ) );
				$vs_buf .= "<div id='exporterFormList'>";
				$vs_buf .= ca_data_exporters::getExporterListAsHTMLFormElement( 'exporter_id', $vn_set_table_num,
					array( 'id' => 'caExporterList' ), array( 'width' => '135px' ) );
				$vs_buf .= caHTMLHiddenInput( 'set_id', array( 'value' => $t_item->getPrimaryKey() ) );
				$vs_buf .= caFormSubmitLink( $po_view->request, _t( 'Export' ) . " &rsaquo;", "button",
					"caExportForm" );
				$vs_buf .= "</div>\n";
				$vs_buf .= "</form>";
		...
		if ( $vs_table_name === 'ca_set_items' ) {
			...
			if ( $t_set->load( $vn_set_id = $t_item->get( 'set_id' ) ) ) {
				$vs_buf .= "<div><strong>" . _t( "Part of set" ) . "</strong>: " . caEditorLink( $po_view->request,
						$t_set->getLabelForDisplay(), '', 'ca_sets', $vn_set_id ) . "<br/>\n";

				$t_content_instance = Datamodel::getInstanceByTableNum(
					$vn_item_table_num = $t_item->get( 'table_num' ) );
				if ( $t_content_instance->load( $vn_row_id = $t_item->get( 'row_id' ) ) ) {
					...
					$vs_buf .= "<strong>" . _t( "Is %1",
							caGetTableDisplayName( $vn_item_table_num, false ) . "</strong>: "
							. caEditorLink( $po_view->request, $vs_label, '', $vn_item_table_num, $vn_row_id ) )
					           . "<br/>\n";
				}
		...
		if ( ( $vs_table_name == 'ca_metadata_alert_rules' ) ) {
			if ( $t_item->getPrimaryKey() ) {
				$vs_buf .= "<div><strong>" . _t( "Rule content type" ) . "</strong>: "
				           . caGetTableDisplayName( $t_item->get( 'table_num' ) ) . "<br/></div>\n";
			} else {
				if ( $vn_set_table_num = $po_view->request->getParameter( 'table_num', pInteger ) ) {
					$vs_buf .= "<div><strong>" . _t( "Rule content type" ) . "</strong>: "
					           . caGetTableDisplayName( $vn_set_table_num ) . "<br/>\n";
		...
		if ( ( $vs_table_name === 'ca_lists' ) && $t_item->getPrimaryKey() ) {
			$vs_buf      .= "<strong>" . _t( "Number of items" ) . "</strong>: " . $t_item->numItemsInList()
			                . "<br/>\n";
			$t_list_item = new ca_list_items();
			$t_list_item->load( array( 'list_id' => $t_item->getPrimaryKey(), 'parent_id' => null ) );
			$vs_type_list = $t_list_item->getTypeListAsHTMLFormElement( 'type_id',
				array( 'style' => 'width: 90px; font-size: 9px;' ), array( 'access' => __CA_BUNDLE_ACCESS_EDIT__ ) );

			if ( $vs_type_list ) {
				$vs_buf .= '<div style="border-top: 1px solid #aaaaaa; margin-top: 5px; font-size: 10px;">';
				$vs_buf .= caFormTag( $po_view->request, 'Edit', 'NewChildForm',
					'administrate/setup/list_item_editor/ListItemEditor', 'post', 'multipart/form-data', '_top',
					array( 'noCSRFToken' => true, 'disableUnsavedChangesWarning' => true ) );
				$vs_buf .= _t( 'Add a %1 to this list', $vs_type_list ) . caHTMLHiddenInput( $t_list_item->primaryKey(),
						array( 'value' => '0' ) ) . caHTMLHiddenInput( 'parent_id',
						array( 'value' => $t_list_item->getPrimaryKey() ) );
				$vs_buf .= caFormSubmitLink( $po_view->request, caNavIcon( __CA_NAV_ICON_ADD__, '18px' ), '',
					'NewChildForm' );
				$vs_buf .= "</form></div>\n";
			}
		}
		...
		if ( $vs_table_name === 'ca_list_items' ) {
			if ( $t_list = $po_view->getVar( 't_list' ) ) {
				...
				$vs_buf     .= "<div><strong>" . _t( "Part of" ) . "</strong>: " . caEditorLink( $po_view->request,
						$t_list->getLabelForDisplay(), '', 'ca_lists', $vn_list_id ) . "</div>\n";
		...
		if ( $vs_table_name === 'ca_relationship_types' ) {
			...
			if ( $t_rel_instance ) {
				$vs_buf .= "<div><strong>" . _t( "Is a" ) . "</strong>: "
				           . $t_rel_instance->getProperty( 'NAME_SINGULAR' ) . "<br/></div>\n";
			}
		}
		...
		if ( ( $vs_table_name === 'ca_metadata_elements' ) && $t_item->getPrimaryKey() ) {
			$vs_buf .= "<div><strong>" . _t( "Element code" ) . "</strong>: " . $t_item->get( 'element_code' )
			           . "<br/></div>\n";
			...
			if ( sizeof( $va_uis = $t_item->getUIs() ) > 0 ) {
				...
				foreach ( $va_uis as $vn_ui_id => $va_ui_info ) {
					$vs_buf .= caNavLink( $po_view->request, $va_ui_info['name'], '',
						'administrate/setup/interface_screen_editor', 'InterfaceScreenEditor', 'Edit',
						array( 'ui_id' => $vn_ui_id, 'screen_id' => $va_ui_info['screen_id'] ) );
					$vs_buf .= " (" . Datamodel::getTableProperty( $va_ui_info['editor_type'], 'NAME_PLURAL' )
					           . ")<br/>\n";
				}
				$vs_buf .= "</div>\n";
			}
		...
		if ( $vs_table_name === 'ca_editor_uis' ) {
			...
			if ( $t_item->getPrimaryKey() ) {
				$vs_buf .= "<div><strong>" . _t( "Edits" ) . "</strong>: "
				           . caGetTableDisplayName( $t_item->get( 'editor_type' ) ) . "<br/>\n";
			} else {
				$vs_buf .= "<div><strong>" . _t( "Edits" ) . "</strong>: "
				           . caGetTableDisplayName( $po_view->request->getParameter( 'editor_type', pInteger ) )
				           . "<br/>\n";
			}
			$vs_buf .= "</div>\n";
		}
		...
		if ( $vs_table_name === 'ca_editor_ui_screens' ) {
			...
			$vs_buf .= "<div><strong>" . _t( "Part of" ) . "</strong>: " . caNavLink( $po_view->request,
					$t_ui->getLabelForDisplay(), '', 'administrate/setup/interface_editor', 'InterfaceEditor', 'Edit',
		...
		if ( $vs_table_name === 'ca_bundle_displays' ) {
			$vs_buf .= "<div><strong>" . _t( "Number of placements" ) . "</strong>: "
			           . $t_item->getPlacementCount( array( 'user_id' => $po_view->request->getUserID() ) ) . "<br/>\n";

			if ( $t_item->getPrimaryKey() ) {

				$vn_content_table_num = $t_item->get( 'table_num' );
				$vs_buf               .= "<strong>" . _t( "Type of content" ) . "</strong>: "
				                         . caGetTableDisplayName( $vn_content_table_num ) . "\n";
		...
		if ( $vs_table_name === 'ca_search_forms' ) {
			...
			if ( $t_item->getPrimaryKey() ) {
				...
				$vn_content_table_num = $t_item->get( 'table_num' );
				$vs_buf               .= "<strong>" . _t( "Searches for" ) . "</strong>: "
				                         . caGetTableDisplayName( $vn_content_table_num ) . "\n";
				$vs_buf               .= "</div>\n";
			} else {
				if ( $vn_content_table_num = $po_view->request->getParameter( 'table_num', pInteger ) ) {
		...
		if ( $vs_table_name === 'ca_tour_stops' ) {
			...
			$vs_buf .= "<strong>" . _t( "Part of" ) . "</strong>: " . caEditorLink( $po_view->request,
					$t_tour->getLabelForDisplay(), '', 'ca_tours', $vn_tour_id ) . "<br/>\n";
		}
		...
		if ( $vs_table_name === 'ca_bundle_mappings' ) {
			if ( $t_item->getPrimaryKey() ) {
				$vn_content_table_num = $t_item->get( 'table_num' );
				$vs_buf               .= "<br/><strong>" . _t( "Type of content" ) . "</strong>: "
				                         . caGetTableDisplayName( $vn_content_table_num ) . "<br/>\n";
				$vs_buf               .= "<strong>" . _t( "Type" ) . "</strong>: "
				                         . $t_item->getChoiceListValue( 'direction', $t_item->get( 'direction' ) )
				                         . "<br/>\n";
				$vs_buf               .= "<strong>" . _t( "Target format" ) . "</strong>: " . $t_item->get( 'target' )
				                         . "<br/>\n";

				$va_stats = $t_item->getMappingStatistics();
				$vs_buf   .= "<div><strong>" . _t( "Number of groups" ) . "</strong>: " . $va_stats['groupCount']
				             . "<br/>\n";
				...
				if ( $vn_content_table_num = $po_view->request->getParameter( 'table_num', pInteger ) ) {
					$vs_buf .= "<div><strong>" . _t( "Type of content" ) . "</strong>: "
					           . caGetTableDisplayName( $vn_content_table_num ) . "<br/>\n";
					$vs_buf .= "<strong>" . _t( "Type" ) . "</strong>: " . $t_item->getChoiceListValue( 'direction',
							$po_view->request->getParameter( 'direction', pString ) ) . "<br/>\n";
					$vs_buf .= "<strong>" . _t( "Target format" ) . "</strong>: "
					           . $po_view->request->getParameter( 'target', pString ) . "<br/>\n";
		...
		if ( $t_item->getPrimaryKey()
			...
			$vb_show_add_child_control = true;
			if ( is_array( $va_restrict_add_child_control_to_types = $po_view->request->config->getList( $vs_table_name
			                                                                                             . '_restrict_child_control_in_inspector_to_types' ) )
			     && sizeof( $va_restrict_add_child_control_to_types )
			) {
				$t_type_instance = $t_item->getTypeInstance();
		...
		if ( $po_view->request->user->canDoAction( 'can_export_' . $vs_table_name ) && $t_item->getPrimaryKey() ) {
			if ( ca_data_exporters::getExporters( $t_item->tableNum(), [ 'countOnly' => true ] ) > 0 ) {
				...
				$vs_buf .= caFormTag( $po_view->request, 'ExportSingleData', 'caExportForm', 'manage/MetadataExport',
					'post', 'multipart/form-data', '_top', array( 'disableUnsavedChangesWarning' => true ) );
				$vs_buf .= "<div id='exporterFormList'>";
				$vs_buf .= ca_data_exporters::getExporterListAsHTMLFormElement( 'exporter_id', $t_item->tableNum(),
					array( 'id' => 'caExporterList' ),
					array( 'width' => '120px', 'recordType' => $t_item->getTypeCode() ) );
			...
			if ( ExternalExportManager::getTargets( [
					'countOnly'       => true,
					'table'           => $t_item->tableNum(),
					'restrictToTypes' => [ $t_item->getTypeCode() ]
				] ) > 0
			) {
				...
				$vs_buf .= "<a class='button' onclick='jQuery(\"#externalExporterFormList\").show();' style='text-align:right;' href='#'>"
				           . caNavIcon( __CA_NAV_ICON_EXPORT_SMALL__, '16px' ) . "</a>";

				$vs_buf .= caFormTag( $po_view->request, 'ExternalExportSingle', 'caExternalExportForm',
					'manage/MetadataExport', 'post', 'multipart/form-data', '_top',
					array( 'disableUnsavedChangesWarning' => true ) );
				$vs_buf .= "<div id='externalExporterFormList'>";
				$vs_buf .= ExternalExportManager::getTargetListAsHTMLFormElement( 'target', $t_item->tableNum(),
					array( 'id' => 'caExternalExporterList' ),
					array( 'width' => '120px', 'restrictToTypes' => [ $t_item->getTypeCode() ] ) );
				$vs_buf .= caHTMLHiddenInput( 'item_id', array( 'value' => $t_item->getPrimaryKey() ) );
				$vs_buf .= caFormSubmitLink( $po_view->request, _t( 'Export' ) . " &rsaquo;", "button",
					"caExternalExportForm" );
				$vs_buf .= "</div>\n";
				$vs_buf .= "</form>";
				$vs_buf .= "</div>";
		...
		if ( $t_item->getPrimaryKey() ) {
			...
			jQuery('#inspectorMoreInfo').click(function() {
				jQuery('#inspectorInfo').slideToggle(350, function() { 
					...
					jQuery('#inspectorMoreInfoButton').html((this.style.display == 'block') ? '"
				           . addslashes( caNavIcon( __CA_NAV_ICON_COLLAPSE__, '20px' ) ) . "' : '"
				           . addslashes( caNavIcon( __CA_NAV_ICON_INFO__, '20px' ) ) . "');
					caResizeSideNav();
				}); 
				return false;
			});
...
function caEditorACLEditor( $po_view, $pt_instance, $pa_options = null ) {
	$vs_view_path = ( isset( $pa_options['viewPath'] ) && $pa_options['viewPath'] ) ? $pa_options['viewPath']
		: $po_view->request->getViewsDirectoryPath();
...
function caBatchEditorInspector( $po_view, $pa_options = null ) {
	...
	$vs_buf = '<h3 class="nextPrevious"><span class="resultCount" style="padding-top:10px;">'
	          . caNavLink( $po_view->request, 'Back to Sets', '', 'manage', 'Set', 'ListSets' ) . "</span></h3>\n";
	...
	if ( $po_view->request->user->canDoAction( "can_edit_" . $vs_priv_table_name )
		...
		if ( $po_view->request->user->canDoAction( "can_change_type_{$vs_table_name}" ) ) {
			...
			$vs_buf .= "<div id='inspectorChangeType'><div id='inspectorChangeTypeButton'><a href='#' onclick='caTypeChangePanel.showPanel(); return false;'>"
			           . caNavIcon( __CA_NAV_ICON_CHANGE__, '18px', array( 'title' => _t( 'Change type' ) ) )
			           . "</a></div></div>\n";
			TooltipManager::add( "#inspectorChangeType", _t( 'Change Record Type' ) );
	...
	$vs_item_name  = ( $vn_item_count == 1 ) ? $t_item->getProperty( "NAME_SINGULAR" )
	...
	if ( ( $vn_item_count > 0 )
	     && ( $po_view->request->user->canDoAction( 'can_batch_delete_'
	                                                . Datamodel::getTableName( $t_set->get( 'table_num' ) ) ) )
	) {

		$vs_buf .= "<div class='button' style='text-align:right;'><a href='#' id='inspectorMoreInfo'>"
		           . _t( "More options" ) . "</a> &rsaquo;</div>
				<div id='inspectorInfo' class='setDelete'>";
		$vs_buf .= caNavLink( $po_view->request,
			caNavIcon( __CA_NAV_ICON_NUKE__, '24px',
				array( 'style' => 'margin-top:7px; vertical-align: text-bottom;' ) ) . " "
			. _t( "Delete <strong><em>all</em></strong> records in set" )
			, null, 'batch', 'Editor', 'Delete', array( 'set_id' => $t_set->getPrimaryKey() )
		);
		if ( $po_view->request->user->canDoAction( "can_change_type_{$vs_table_name}" ) ) {

			$vs_buf .= "<a href='#' onclick='caTypeChangePanel.showPanel(); return false;'>"
			           . caNavIcon( __CA_NAV_ICON_CHANGE__, '20px',
					array( 'style' => 'margin: 7px 4px 0 0; vertical-align: text-bottom;' ) ) . " "
			           . _t( "Set type for records in set" ) . "</a>\n";
		...
		$vs_buf .= "<script type='text/javascript'>
				jQuery('#inspectorMoreInfo').click(function() {
					jQuery('#inspectorInfo').slideToggle(350, function() { 
						jQuery('#inspectorMoreInfo').html((this.style.display == 'block') ? '"
		           . addslashes( _t( 'Close options' ) ) . "' : '" . addslashes( _t( 'More options' ) ) . "');
					}); 
					return false;
				});
			</script>";
...
function caBatchMediaImportInspector( $po_view, $pa_options = null ) {
	...
	$vs_buf                               .= "<p>" . _t( '<strong>Server directory:</strong> %1',
			$vs_batch_media_import_root_directory ) . "</p>\n";
...
function caBatchMetadataExportInspector( $po_view ) {
	...
	if ( $vn_id = $po_view->request->getParameter( 'item_id', pInteger ) ) {
		$vs_buf .= "<p>" . caEditorLink( $po_view->request, _t( "Back to record" ), 'caResultsEditorEditLink',
				$t_item->getTargetTableName(), $vn_id ) . "</p>";
	}
...
function caTableIsActive( $pm_table ) {
	$t_instance = is_numeric( $pm_table ) ? Datamodel::getInstanceByTableNum( $pm_table, true )
		: Datamodel::getInstanceByTableName( $pm_table, true );
	if ( ! $t_instance ) {
		return null;
	}
 ...
 *        parseOptions = Parse tag options are return an array where each value is an array of options; the tag itself
 *        is put in the 'originalTag' key. [Default is false] firstPartOnly = Return a list of first elements in the
 *        tags. For the template "^ca_entities.entity_id ^ca_objects_x_entities.source_text" the list  ["ca_entites",
 *        "ca_objects_x_entities"] would be returned. [Default is false]
 *
 ...
 *                            prefix = string to add to beginning of tags extracted from template before doing lookup
 *                            into value array removePrefix = string to remove from tags extracted from template before
 *                            doing lookup into value array getFrom = a model instance to draw data from. If set,
 *                            $pa_values is ignored. quote = quote replacement values (Eg. ^ca_objects.idno becomes
 *                            "2015.001" rather than 2015.001). Value containing quotes will be escaped with a
 *                            backslash. [Default is false]
 *
 ...
 *        LP = left pad value; parameters are in the format <padding>/<length>. Ex. LP:0/10 will left pad a string with
 *        zeros to a length of 10. RP = right pad value; parameters are in the format <padding>/<length> PREFIX = add
 *        prefix to value if not empty; parameter is the prefix text SUFFIX = add suffix to value if not empty;
 *        parameter is the suffix text
 *
...
function caProcessTemplateTagDirectives( $ps_value, $pa_directives, $pa_options = null ) {
	...
	$force_metric_units  = caGetOption( 'forceMetricUnits', $pa_options, null,
		[ 'validValues' => [ 'm', 'cm', 'mm' ] ] );
			...
			case 'UNITS':
				try {
					...
					if ( $force_english_units
					     && in_array( $vs_units,
							[ 'in', 'in.', 'inches', 'inch', '"', 'ft', 'ft.', 'foot', 'feet', "'" ] )
					) {
					...
					switch ( $vs_units ) {
						...
						case 'fractionalenglish':
							$vn_maximum_denominator
								= array_reduce( $o_dimensions_config->get( 'display_fractions_for' ),
								function ( $acc, $v ) {
									$t = explode( "/", $v );
							...
							if ( ! $force_english_units
							     && ( $vn_value_in_inches > $o_dimensions_config->get( 'use_feet_for_display_up_to' ) )
							) {
								if ( $vn_in_miles = (int) ( $vn_value_in_inches / ( 12 * 5280 ) ) ) {
									$va_measure_conv[] = "{$vn_in_miles} miles" . ( ( ! $pb_omit_units
									                                                  && in_array( 'MILE',
												$va_add_periods_list ) ) ? '.' : '' );
								}
								$vn_value_in_inches -= ( $vn_in_miles * ( 12 * 5280 ) );
							}
							if ( ( $force_english_units == 'ft' )
							     || ( ! $force_english_units
							          && ( $vn_value_in_inches
							               > $o_dimensions_config->get( 'use_inches_for_display_up_to' ) ) )
							) {
								if ( $vn_in_feet = (int) ( $vn_value_in_inches / 12 ) ) {
									$va_measure_conv[] = "{$vn_in_feet} ft" . ( ( ! $pb_omit_units
									                                              && in_array( 'FEET',
												$va_add_periods_list ) ) ? '.' : '' );
								}
							...
							if ( $vn_value_in_inches > 0 ) {
								...
								$va_measure_conv[] = ( in_array( $vs_units, [ 'fractionalenglish', 'infrac' ] )
										? caLengthToFractions( $vn_value_in_inches, $vn_maximum_denominator, true )
										: $vo_inches->convertTo( Zend_Measure_Length::INCH,
											$o_dimensions_config->get( 'inch_decimal_precision' ) ) )
								                     . ( ( ! $pb_omit_units
								                           && in_array( 'INCH', $va_add_periods_list ) ) ? '.' : '' );
							}
						...
						case 'metric':
							...
							if ( ( $force_metric_units == 'mm' )
							     || ( ! $force_metric_units
							          && ( $vn_value_in_cm
							               <= $o_dimensions_config->get( 'use_millimeters_for_display_up_to' ) ) )
							) {
								$vs_measure_conv = $vo_measurement->convertTo( Zend_Measure_Length::MILLIMETER,
										(int) $o_dimensions_config->get( 'millimeter_decimal_precision' ) )
								                   . ( ( ! $pb_omit_units
								                         && in_array( 'MILLIMETER', $va_add_periods_list ) ) ? '.'
										: '' );
							} elseif ( ( $force_metric_units == 'cm' )
							           || ( ! $force_metric_units
							                && ( $vn_value_in_cm
							                     <= $o_dimensions_config->get( 'use_centimeters_for_display_up_to' ) ) )
							) {
								$vs_measure_conv = $vo_measurement->convertTo( Zend_Measure_Length::CENTIMETER,
										(int) $o_dimensions_config->get( 'centimeter_decimal_precision' ) )
								                   . ( ( ! $pb_omit_units
								                         && in_array( 'CENTIMETER', $va_add_periods_list ) ) ? '.'
										: '' );
							} elseif ( ( $force_metric_units == 'm' )
							           || ( ! $force_metric_units
							                && ( $vn_value_in_cm
							                     <= $o_dimensions_config->get( 'use_meters_for_display_up_to' ) ) )
							) {
								$vs_measure_conv = $vo_measurement->convertTo( Zend_Measure_Length::METER,
										(int) $o_dimensions_config->get( 'meter_decimal_precision' ) )
								                   . ( ( ! $pb_omit_units && in_array( 'METER', $va_add_periods_list ) )
										? '.' : '' );
							} else {
								$vs_measure_conv = $vo_measurement->convertTo( Zend_Measure_Length::KILOMETER,
										$o_dimensions_config->get( 'kilometer_decimal_precision' ) )
								                   . ( ( ! $pb_omit_units
								                         && in_array( 'KILOMETER', $va_add_periods_list ) ) ? '.'
										: '' );
							}
							break;
						case 'm':
							$vs_measure_conv = $vo_measurement->convertTo( Zend_Measure_Length::METER,
									$o_dimensions_config->get( 'meter_decimal_precision' ) ) . ( ( ! $pb_omit_units
							                                                                       && in_array( 'METER',
										$va_add_periods_list ) ) ? '.' : '' );
							break;
						case 'cm':
							$vs_measure_conv = $vo_measurement->convertTo( Zend_Measure_Length::CENTIMETER,
									$o_dimensions_config->get( 'centimeter_decimal_precision' ) ) . ( ( ! $pb_omit_units
							                                                                            && in_array( 'CENTIMETER',
										$va_add_periods_list ) ) ? '.' : '' );
							break;
						case 'ft':
						...
						case 'feet':
						case "'":
							$vs_measure_conv = $vo_measurement->convertTo( Zend_Measure_Length::FEET,
									$o_dimensions_config->get( 'feet_decimal_precision' ) ) . ( ( ! $pb_omit_units
							                                                                      && in_array( 'FEET',
										$va_add_periods_list ) ) ? '.' : '' );
							break;
						case 'in':
						...
						case 'in.':
						case '"':
							$vs_measure_conv = $vo_measurement->convertTo( Zend_Measure_Length::INCH,
									$o_dimensions_config->get( 'inch_decimal_precision' ) ) . ( ( ! $pb_omit_units
							                                                                      && in_array( 'INCH',
										$va_add_periods_list ) ) ? '.' : '' );
							break;
					}
 ...
 * @param array  $pa_row_ids          An array of primary key values in the specified table to be pulled into the
 *                                    template
 * @param array  $pa_options          Supported options are:
 ...
 *                                display = Name of bundle field to use as display text for related item. [Default is
 *                                "_display"] relationshipTypeDisplayPosition = Position to render relationship type in
 *                                relative to display text. Valid values are "left", "right" and "none". [Default is
 *                                "right"] editableRelationshipType = Render relationship type as drop-down to support
 *                                in-place editing. [Default is to use table-specific settings in app.conf]
 *
 * @return string
 */
function caGetRelationDisplayString( $po_request, $ps_table, $pa_attributes = null, $pa_options = null ) {
	...
	if ( ! ( $vs_relationship_type_display_position = caGetOption( 'relationshipTypeDisplayPosition', $pa_options,
		null ) )
	) {
		$vs_relationship_type_display_position = strtolower( $o_config->get( $ps_table
		                                                                     . '_lookup_relationship_type_position' ) );
	}
	...
	if ( caGetOption( 'makeLink', $pa_options, false ) ) {
		$vs_display = "<a href='" . urldecode( caEditorUrl( $po_request, $ps_table,
				'{' . Datamodel::primaryKey( $ps_table ) . '}', false, array( 'rel' => true ) ) )
		              . "' {$vs_attr_str}>{$vs_display}</a>";
	}

	$vs_reltype_disp = caGetOption( 'editableRelationshipType', $pa_options,
		(bool) $o_config->get( "{$ps_table}_lookup_relationship_type_editable" ) )
		? "<select name='{$ps_prefix}_type_id{n}' id='{$ps_prefix}_type_id{n}' class='listRelRelationshipTypeEdit'></select>"
...
# ------------------------------------------------------------------------------------------------
/**
 * Returns date/time as a localized string for display, subject to the settings in the app/conf/datetime.conf
 * configuration
 *
 * @param null|int   $pn_timestamp Unix timestamp for date/time to localize; if omitted defaults to current date and
 *                                 time.
 ...
 * configuration
 *
 * @param int   $pn_timestamp Historic timestamp for date/time to localize; if omitted defaults to current date and
 *                            time.
...
# ------------------------------------------------------------------------------------------------
/**
 * Returns date range as a localized string for display, subject to the settings in the app/conf/datetime.conf
 * configuration
 *
 ...
 * @param array  $pa_options
 *                                 delimiter = A string to join multiple values with when returning normalized date
 *                                 range as a string. Default is semicolon followed by space ("; ") returnAsArray = If
 *                                 set an array of normalized values will be returned rather than a string. Default is
 *                                 false.
 *
 * @return array|bool The normalized expression. If the expression normalizes to multiple values (eg. a range of years
 *                    being normalized to months) then the values will be joined with a delimiter and returned as a
 *                    string unless the "returnAsArray" option is set.
 */
 ...
 *
 * @param DbResult or ca_object_representations instance $po_rep An object containing representation data. Can be
 *                    either a DbResult object (ie. a query result) or ca_object_representations instance (an instance
 *                    representing a row in the ca_object_representation class)
 ...
 *
 * @return mixed Text ready for display describing dimensions of the representation's media. Can be array if
 *               'returnAsArray' option is set.
 */
function caGetRepresentationDimensionsForDisplay( $po_rep, $ps_version, $pa_options = null ) {
	...
	if ( isset( $va_tmp['PROPERTIES']['resolution'] )
	     && is_array( $va_resolution = $va_tmp['PROPERTIES']['resolution'] )
	) {
		if ( isset( $va_resolution['x'] ) && isset( $va_resolution['y'] ) && $va_resolution['x']
		     && $va_resolution['y']
		) {
 ...
 *                                 relatedItems =
 *                                 sortOrder =
 *                                 self_id = ID of record being related to when relationship is to self (Eg.
 *                                 object-object). This id will be omitted from returned items. primaryIDs = row_ids
 *                                 for primary rows in related table, keyed by table name; when resolving ambiguous
 *                                 relationships the row_ids will be excluded from consideration. This option is rarely
 *                                 used and exists primarily to take care of a single edge case: you are processing a
 *                                 template relative to a self-relationship such as ca_entities_x_entities that
 *                                 includes references to the subject table (ca_entities, in the case of
 *                                 ca_entities_x_entities). There are two possible paths to take in this situations;
 *                                 primaryIDs lets you specify which ones you *don't* want to take by row_id. For
 *                                 interstitial editors, the ids will be set to a single id: that of the subject (Eg.
 *                                 ca_entities) row from which the interstitial was launched.
 *
 * @return mixed
 */
function caProcessRelationshipLookupLabel( $qr_rel_items, $pt_rel, $pa_options = null ) {
	...
	$o_config = ( ! isset( $pa_options['config'] ) || ! is_object( $pa_options['config'] ) ) ? Configuration::load()
		: $pa_options['config'];
	...
	$va_primary_ids = ( method_exists( $pt_rel, "isSelfRelationship" )
	                    && ( $vb_is_self_rel
			= $pt_rel->isSelfRelationship() ) ) ? caGetOption( "primaryIDs", $pa_options, null ) : null;
	...
	if ( isset( $pa_options['relatedItems'] ) && is_array( $pa_options['relatedItems'] )
	     && sizeof( $pa_options['relatedItems'] )
	) {
		$va_tmp = array();
		foreach ( $pa_options['relatedItems'] as $vn_relation_id => $va_relation ) {
			...
			$va_items[ $va_relation[ $vs_rel_pk ] ]['relationship_type_id']
			                                                                 =
			$va_items[ $va_relation[ $vs_rel_pk ] ]['type_id'] = ( $va_relation['direction'] )
				? $va_relation['direction'] . '_' . $va_relation['relationship_type_id']
				: $va_relation['relationship_type_id'];
			...
			if ( ! isset( $va_items[ $va_relation[ $vs_rel_pk ] ][ $vs_rel_pk ] )
			     || ! $va_items[ $va_relation[ $vs_rel_pk ] ][ $vs_rel_pk ]
			) {
				$va_items[ $va_relation[ $vs_rel_pk ] ][ $vs_rel_pk ]
					= $va_items[ $va_relation[ $vs_rel_pk ] ]['id'] = $va_relation[ $vs_rel_pk ];
			}

			if ( $vs_template ) {
				$pk                                                 = is_object( $qr_rel_items )
					? $qr_rel_items->primaryKey() : null;
				$va_items[ $va_relation[ $vs_rel_pk ] ]['_display'] = caProcessTemplateForIDs( $vs_template,
					$pt_rel->tableName(),
					array( $va_relation['relation_id'] ? $va_relation['relation_id'] : $va_relation[ $pk ] ), array(
						'returnAsArray'     => false,
						'returnAsLink'      => false,
						'delimiter'         => caGetOption( 'delimiter', $pa_options, $vs_display_delimiter ),
						'resolveLinksUsing' => $vs_rel_table,
	...
	foreach ( $va_items as $va_item ) {
		...
		if ( isset( $pa_options['stripTags'] ) && $pa_options['stripTags'] ) {
			if ( preg_match( '!(<[A-Za-z0-9]+[ ]+[A-Za-z0-9 ,;\&\-_]*>)!', $vs_display,
				$va_matches )
			...
			if ( preg_match( '!(<[A-Za-z0-9]+[ ]+[A-Za-z0-9 ,;\&\-_]*>)!', $vs_label,
				$va_matches )
			) {    // convert text in <> to non-tags if the text has only letters, numbers and spaces in it
				array_shift( $va_matches );
				foreach ( $va_matches as $vs_match ) {
		...
		if ( ( $vs_display_lc == $ps_inline_create_query_lc )
		     || ( isset( $va_item['label'] )
		          && ( $va_item['label'] == $ps_inline_create_query_lc ) )
		) {
			$vb_include_inline_add_message = false;
		}
		...
		if ( $po_request
		     && ca_editor_uis::loadDefaultUI( $pt_rel->tableName(), $po_request, $va_item['rel_type_id'] )
		) {
		...
		$va_initial_values[ $va_item['relation_id'] ? (int) $va_item['relation_id'] : $va_item[ $vs_rel_pk ] ]
			= array_merge(
			$va_item,
			array(
				'label' => $vs_display
...
function caObjectsDisplayDownloadLink( $po_request, $pn_object_id = null, $pt_representation = null ) {
	...
	if ( $vs_allow = $o_config->get( [
		'allowObjectRepresentationDownload',
	...
	$va_types = $o_config->get( [
		'restrictObjectRepresentationDownloadToObjectTypes',
	...
	if ( ( ! $po_request->config->get( "dont_enforce_access_settings" ) ) && $pt_representation
	     && is_array( $va_download_access_settings )
	     && sizeof( $va_download_access_settings )
	) {
 ...
 * Used in SearchResult::get() and BundlableLabelableBaseModelWithAttributes::get() to automatically generate links
 * when fetching information from related tables.
 *
 ...
 * @param array  $pa_options    Supported options are:
 *                              requireLinkTags = if set then links are only added when explicitly defined with <l>
 *                              tags. Default is to make the entire text a link in the absence of <l> tags.
 *                              addRelParameter = absolute = Return absolute urls [Default is false]
 *
 * @return array A list of HTML links
 */
function caCreateLinksFromText(
	$pa_text, $ps_table_name, $pa_row_ids, $ps_class = null, $ps_target = null, $pa_options = null
) {
	...
	foreach ( $pa_text as $vn_i => $vs_text ) {
		...
		foreach ( $o_links as $o_link ) {
			...
			$va_l_tags[] = array(
				'directive' => html_entity_decode( $vs_html ),
				'content'   => $vs_content
			);    //html_entity_decode
		}
		...
		if ( sizeof( $va_l_tags ) ) {
			...
			foreach ( $va_l_tags as $va_l ) {
				if ( $vb_can_handle_target ) {
					$va_params    = array(
						'request'              => $g_request,
						'content'              => $va_l['content'],
						'table'                => $ps_table_name,
						'id'                   => $pa_row_ids[ $vn_i ],
						'classname'            => $ps_class,
						'target'               => $ps_target,
						'additionalParameters' => null,
						'options'              => $va_link_opts
					);
					...
					switch ( __CA_APP_TYPE__ ) {
						case 'PROVIDENCE':
							$vs_link_text = caEditorLink( $g_request, $va_l['content'], $ps_class, $ps_table_name,
								$pa_row_ids[ $vn_i ], ( $pb_add_rel ? array( 'rel' => true ) : array() ), null,
								$va_link_opts );
							break;
						case 'PAWTUCKET':
							$vs_link_text = caDetailLink( $g_request, $va_l['content'], $ps_class, $ps_table_name,
								$pa_row_ids[ $vn_i ], null, null, $va_link_opts );
							break;
					}
				}
			...
			if ( $vb_can_handle_target ) {
				...
				switch ( __CA_APP_TYPE__ ) {
					case 'PROVIDENCE':
						$va_links[ $vn_i ] = ( $vs_link = caEditorLink( $g_request, $vs_text, $ps_class, $ps_table_name,
							$pa_row_ids[ $vn_i ], ( $pb_add_rel ? array( 'rel' => true ) : array() ), null,
							$va_link_opts ) ) ? $vs_link : $vs_text;
						break;
					case 'PAWTUCKET':
						$va_links[ $vn_i ] = ( $vs_link = caDetailLink( $g_request, $vs_text, $ps_class, $ps_table_name,
							$pa_row_ids[ $vn_i ], null, null, $va_link_opts ) ) ? $vs_link : $vs_text;
						break;
					default:
						$va_links[ $vn_i ] = $vs_text;
						break;
...
function caGetBundleDisplayTemplate( $pt_subject, $ps_related_table, $pa_bundle_settings, $pa_options = null ) {
	...
	if ( ! $vs_template ) {
		if ( ! trim(
			$vs_template = $pt_subject->getAppConfig()->get( "{$ps_related_table}_default_editor_display_template" ) )
		) {    // use explicit setting
			if ( is_array(
				$va_lookup_settings = $pt_subject->getAppConfig()->getList( "{$ps_related_table}_lookup_settings" ) )
			) {    // fall back to derive from lookup setting
				if ( ! ( $vs_lookup_delimiter = $pt_subject->getAppConfig()
				                                           ->get( "{$ps_related_table}_lookup_delimiter" ) )
				) {
					$vs_lookup_delimiter = '';
				}
 ...
 */
function caEditorBundleShowHideControl(
	$po_request, $ps_id_prefix, $pa_settings = null, $pb_has_value = false, $ps_preview_init = "&nbsp;"
) {
	...
	$vs_buf
		    = "<span class='bundleContentPreview' id='{$ps_preview_id_prefix}_BundleContentPreview'>{$ps_preview_init}</span>";
	$vs_buf .= "<span class='iconButton'>";
	$vs_buf .= "<a href='#' onclick='caBundleVisibilityManager.toggle(\"{$ps_id_prefix}\");  return false;'>"
	           . caNavIcon( __CA_NAV_ICON_VISIBILITY_TOGGLE__, '18px',
			array( 'id' => "{$ps_id_prefix}VisToggleButton" ) ) . "</a>";
	$vs_buf .= "</span>\n";
...
function caEditorBundleMetadataDictionary( $po_request, $ps_id_prefix, $pa_settings ) {
	...
	$vs_buf = '';
	$vs_buf .= "<span class='iconButton'>";
	$vs_buf .= "<a href='#' class='caMetadataDictionaryDefinitionToggle' onclick='caBundleVisibilityManager.toggleDictionaryEntry(\"{$ps_id_prefix}\");  return false;'>"
	           . caNavIcon( __CA_NAV_ICON_INFO__, 1, array( 'id' => "{$ps_id_prefix}MetadataDictionaryToggleButton" ) )
	           . "</a>";
...
function caEditorBundleSortControls( $po_request, $ps_id_prefix, $ps_table, $pa_options = null ) {
	...
	if ( ! is_array( $va_sort_fields = caGetAvailableSortFields( $ps_table, null,
			array_merge( [ 'request' => $po_request ], $pa_options, [ 'naturalSortLabel' => _t( 'default' ) ] ) ) )
	     || ! sizeof( $va_sort_fields )
	) {
		return '';
	}
	...
	return "<div class='editorBundleSortControl'>" . _t( 'Sort using %1 %2',
			caHTMLSelect( "{$ps_id_prefix}_RelationBundleSortControl",
...
function caReturnToHomeLocationControlForRelatedObjectBundle(
	$ps_id_prefix, $po_request, $pt_primary, $pt_related, $pt_relation, $pa_initial_values, $ps_policy = null
) {
	$policies = array_filter( ca_objects::getHistoryTrackingCurrentValuePoliciesForTable( 'ca_objects' ),
		function ( $v ) {
	...
	$vs_buf = "<div id='{$ps_id_prefix}_editor_bundle_return_to_home_button' class='editorBundleReturnToHomeControl'>" .
	          caJSButton( $po_request, __CA_NAV_ICON_HOME__, _t( "Return to home locations" ),
		          "{$ps_id_prefix}_return_to_home_locations",
		          [ 'onclick' => "caReturnToHomeLocationToggleForm{$ps_id_prefix}(); return false;" ],
		          [ 'size' => '15px' ] ) .
	          "</div>";
	...
	$vs_buf .= "<div id='{$ps_id_prefix}_editor_bundle_return_to_home_controls' class='editorBundleReturnToHomeControls'>"
	           .
	           ca_storage_locations::getHistoryTrackingChronologyInterstitialElementAddHTMLForm( $po_request,
		           $ps_id_prefix, $pt_related->tableName(), $settings,
		           [ 'placement_code' => $ps_id_prefix, 'noTemplate' => true ] ) .
	           caJSButton( $po_request, __CA_NAV_ICON_GO__, _t( "Apply" ),
		           "{$ps_id_prefix}_return_to_home_locations_execute",
		           [ 'onclick' => "caReturnToHomeLocation{$ps_id_prefix}(); return false;" ], [ 'size' => '15px' ] ) .
	           "</div>\n";
	$vs_buf .= "
			<script type='text/javascript'>
	...
	foreach ( $va_tags as $va_tag ) {
		...
		switch ( strtolower( $va_tag_bits[0] ) ) {
			case 'sum':
				...
				$vs_val = caProcessBottomLineTemplateForPlacement( $po_request, $va_placements, $pr_res, [
					'template' => '^SUM' . ( isset( $va_tag_bits[1] ) ? ":{$va_tag_bits[1]}" : "" ),
					'multiple' => true
				] );
...
function caProcessBottomLineTemplateForPlacement( $po_request, $pa_placement, $pr_res, $pa_options = null ) {
	...
	foreach ( $pa_placements as $pa_placement ) {
		...
		if ( $vn_datatype == 0 ) {    // container
			foreach ( $va_tags as $vs_raw_tag ) {
				...
				$va_subelements_to_process["{$vs_bundle_name}.{$vs_subelement}"]
				                                   = ca_metadata_elements::getElementDatatype( $vs_subelement );
			}
		} else {
			$va_tmp = explode( ".", $vs_bundle_name );
			if ( sizeof( $va_tmp ) == 2 ) {
		...
		while ( $pr_res->nextHit() ) {
			foreach ( $va_subelements_to_process as $vs_subelement => $vn_subelement_datatype ) {
				...
				switch ( $vn_subelement_datatype ) {
					case 6:        // currency
						$va_values = $pr_res->get( $vs_subelement,
							array( 'returnAsDecimalWithCurrencySpecifier' => true, 'returnAsArray' => true ) );

						if ( is_array( $va_values ) ) {
							foreach ( $va_values as $vs_value ) {
								$vn_value = (float) caConvertCurrencyValue( $vs_value, $vs_user_currency,
									array( 'numericValue' => true ) );

								$va_tag_values[ $vs_value_name ]['SUM'] += $vn_value;
								if ( is_null( $va_tag_values[ $vs_value_name ]['MIN'] )
								     || ( $vn_value < $va_tag_values[ $vs_value_name ]['MIN'] )
								) {
									$va_tag_values[ $vs_value_name ]['MIN'] = $vn_value;
								}
								if ( is_null( $va_tag_values[ $vs_value_name ]['MAX'] )
								     || ( $vn_value > $va_tag_values[ $vs_value_name ]['MAX'] )
								) {
								...
								if ( ( $vn_c >= $pn_page_start ) && ( $vn_c <= $pn_page_end ) ) {
									...
									if ( is_null( $va_tag_values[ $vs_value_name ]['PAGEMIN'] )
									     || ( $vn_value < $va_tag_values[ $vs_value_name ]['PAGEMIN'] )
									) {
										$va_tag_values[ $vs_value_name ]['PAGEMIN'] = $vn_value;
									}
									if ( is_null( $va_tag_values[ $vs_value_name ]['PAGEMAX'] )
									     || ( $vn_value > $va_tag_values[ $vs_value_name ]['PAGEMAX'] )
									) {
					...
					case 9:        // weight
						$va_values = $pr_res->get( $vs_subelement,
							array( 'returnAsDecimalMetric' => true, 'returnAsArray' => true ) );
					...
					case 10:    // timecode
						$va_values = $pr_res->get( $vs_subelement,
							array( 'returnAsDecimal' => true, 'returnAsArray' => true ) );
	...
	if ( $vb_is_multiple ) {
		...
		foreach ( array_keys( $va_tag_values ) as $vs_value_name ) {
			$vn_datatype                                 = preg_match( "!_([\d]+)$!", $vs_value_name, $va_matches )
				? (int) $va_matches[1] : 0;
	...
	foreach ( $va_subelements_to_process as $vs_subelement => $vn_subelement_datatype ) {
		...
		switch ( $vn_subelement_datatype ) {
			case 6:        // currency
				$va_tag_values[ $vs_value_name ]['PAGEAVG'] = ( $vn_page_len > 0 ) ? sprintf( "%1.2f",
					$va_tag_values[ $vs_value_name ]['PAGESUM'] / $vn_page_len ) : 0;
				$va_tag_values[ $vs_value_name ]['AVG']     = ( $vn_c > 0 ) ? sprintf( "%1.2f",
					$va_tag_values[ $vs_value_name ]['SUM'] / $vn_c ) : "0.00";

				foreach ( $va_tag_values[ $vs_value_name ] as $vs_tag => $vn_val ) {
					$va_tag_values[ $vs_value_name ][ $vs_tag ] = "{$vs_user_currency} "
			...
			case 8:        // length
				...
				foreach ( $va_tag_values[ $vs_value_name ] as $vs_tag => $vn_val ) {
					$vo_measurement                             = new Zend_Measure_Length( (float) $vn_val, 'METER',
						$g_ui_locale );
					$va_tag_values[ $vs_value_name ][ $vs_tag ] = $vo_measurement->convertTo( ( $g_ui_units_pref
					                                                                            == 'metric' )
						? Zend_Measure_Length::METER : Zend_Measure_Length::FEET, 4 );
				}
			...
			case 9:        // weight
				...
				foreach ( $va_tag_values[ $vs_value_name ] as $vs_tag => $vn_val ) {
					$vo_measurement                             = new Zend_Measure_Length( (float) $vn_val, 'KILOGRAM',
						$g_ui_locale );
					$va_tag_values[ $vs_value_name ][ $vs_tag ] = $vo_measurement->convertTo( ( $g_ui_units_pref
					                                                                            == 'metric' )
						? Zend_Measure_Weight::KILOGRAM : Zend_Measure_Weight::POUND, 4 );
				}
			...
			case 10:    // timecode
				$va_tag_values[ $vs_value_name ]['PAGEAVG'] = ( $vn_page_len > 0 ) ? sprintf( "%1.2f",
					$va_tag_values[ $vs_value_name ]['PAGESUM'] / $vn_page_len ) : 0;
				$va_tag_values[ $vs_value_name ]['AVG']     = ( $vn_c > 0 ) ? sprintf( "%1.2f",
					$va_tag_values[ $vs_value_name ]['SUM'] / $vn_c ) : 0;
 ...
 * @param BaseModel|SearchResult $po_data    A model instance (ca_object_representations or a model inheriting from
 *                                           RepresentableBaseModel) or a search result (for a RepresentableBaseModel
 *                                           table) for which to render the viewer.
 * @param RepresentableBaseModel $pt_subject = A model instance loaded with the subject (the record the media is shown
 *                                           in the context of. Eg. if a representation is shown for an object this is
 *                                           an instance for that object record)
 * @param array                  $pa_options Options include:
 *                                           primaryOnly = return only primary representations. [Default is false]
 *                                           currentRepClass = CSS class to apply to thumbnail of currently visible
 *                                           representation. [Default is "active"] dontShowPlaceholder = Don't use
 *                                           placeholder when no representation is available. [Default is false]
 *                                           display = media_display.conf display version to use. [Default is 'detail']
 *                                           displayAnnotations = Mode of display for annotations on representation.
 *                                           Valid values are: viewer (in viewer), div (in external div with class
 *                                           #detailAnnotations), none (no display) [Default is none]
 *                                           displayAnnotationTemplate = Template to use when formatting list of
 *                                           annotations [Default is the annotation title
 *                                           (^ca_representation_annotations.preferred_labels.name)]
 *
...
function caRepresentationViewer( $po_request, $po_data, $pt_subject, $pa_options = null ) {
	...
	$ps_annotation_display_template = caGetOption( 'displayAnnotationTemplate', $pa_options,
		caGetOption( 'displayAnnotationTemplate', $va_detail_config['options'],
			'^ca_representation_annotations.preferred_labels.name' ) );
	...
	$o_view->setVar( 'context', ( $vs_context = $po_request->getParameter( 'context', pString ) )
		? $vs_context : $vs_context = $po_request->getAction() );

	$va_rep_ids = array();
	while ( $vo_data->nextHit() ) {
		if ( ! ( $vn_representation_id = $vo_data->get( 'ca_object_representations.representation_id',
			[ 'checkAccess' => $va_access_values, 'limit' => 1 ] ) )
		) {
			continue;
		}
		...
		if ( $pb_primary_only ) {
			...
			$va_rep_ids = $t_instance->getRepresentationIDs( [ "checkAccess" => $va_access_values ] ) )
		) {
		...
		if ( sizeof( $va_rep_ids ) > 0 ) {
			...
			$va_rep_tags = $qr_reps->getRepresentationViewerHTMLBundles( $po_request, $pt_subject,
				array_merge( $pa_options, [ 'context' => $vs_context ] ) );
			...
			while ( $qr_reps->nextHit() ) {
				...
				if ( $vn_rep_id == $vn_primary_id ) {
					...
					= (int) $qr_reps->get( RepresentableBaseModel::getRepresentationRelationshipTableName( $qr_reps->tableName() )
					                       . '.rank' ) )
				) {
			...
			foreach ( $va_rep_info as $vn_order => $va_rep ) {
				...
				$vs_slides .= ( $vn_count == 0 ) ? "<div id='slideContent{$va_rep['rep_id']}'>" . $va_rep["tag"]
				                                   . "</div>"
					: "<div id='slideContent{$va_rep['rep_id']}'></div>";    // initially only load first one
			...
			if ( ! $po_request->config->get( "disable_lightbox" ) ) {
				...
				if ( $po_request->isLoggedIn() ) {
					$vs_tool_bar .= " <a href='#' onclick='caMediaPanel.showPanel(\"" . caNavUrl( $po_request, '',
							'Lightbox', 'addItemForm',
							array( $pt_subject->primaryKey() => $pt_subject->getPrimaryKey() ) )
					                . "\"); return false;' title='" . _t( "Add item to %1", $vs_lightbox_displayname )
					                . "'>" . $vs_lightbox_icon . "</a>\n";
				} else {
					$vs_tool_bar .= " <a href='#' onclick='caMediaPanel.showPanel(\"" . caNavUrl( $po_request, '',
							'LoginReg', 'LoginForm' ) . "\"); return false;' title='" . _t( "Login to add item to %1",
							$vs_lightbox_displayname ) . "'>" . $vs_lightbox_icon . "</a>\n";
				}
				$vs_tool_bar .= "</div><!-- end detailMediaToolbar -->\n";
			}

			$vs_placeholder = "<div class='detailMediaPlaceholder'>" . caGetPlaceholder( $pt_subject->getTypeCode(),
					"placeholder_large_media_icon" ) . "</div>" . $vs_tool_bar;
		}
	}
...
function caRepToolbar( $po_request, $pt_representation, $pt_subject, $pa_options = null ) {
	...
	$va_rep_display_info                     = caGetMediaDisplayInfo( $ps_display_type,
		$pt_representation->getMediaInfo( 'media', 'INPUT', 'MIMETYPE' ) );
	$va_rep_display_info['poster_frame_url'] = $pt_representation->getMediaUrl( 'media',
		$va_rep_display_info['poster_frame_version'] );
	...
	if ( ! caGetOption( [ 'no_overlay' ], $va_rep_display_info, false ) ) {
		$vs_tool_bar .= "<a href='#' class='zoomButton' onclick='caMediaPanel.showPanel(\"" . caNavUrl( $po_request, '',
				'Detail', 'GetMediaOverlay', array(
					'context'           => $ps_context,
					'id'                => $pn_subject_id,
					'representation_id' => $vn_rep_id,
					'overlay'           => 1
				) ) . "\"); return false;' title='" . _t( "Zoom" )
	...
	if ( ( $ps_table == "ca_objects" ) && is_array( $va_add_to_set_link_info ) && sizeof( $va_add_to_set_link_info ) ) {
		$vs_tool_bar .= " <a href='#' class='setsButton' onclick='caMediaPanel.showPanel(\"" . caNavUrl( $po_request,
				'', $va_add_to_set_link_info['controller'], 'addItemForm', array(
					'context'         => $ps_context,
					( is_object( $pt_subject ) && $pt_subject->primaryKey() ) ? $pt_subject->primaryKey()
						: "object_id" => $pn_subject_id
				) ) . "\"); return false;' title='" . $va_add_to_set_link_info['link_text'] . "'>"
		                . $va_add_to_set_link_info['icon'] . "</a>\n";
	}
	if ( caObjectsDisplayDownloadLink( $po_request, $pn_subject_id, $pt_representation ) ) {
		...
		$va_download_display_info = caGetMediaDisplayInfo( 'download',
			$pt_representation->getMediaInfo( 'media', 'INPUT', 'MIMETYPE' ) );
		$vs_download_version      = caGetOption( [ 'download_version', 'display_version' ], $va_download_display_info );
		if ( $vs_download_version ) {
			$vs_tool_bar .= caNavLink( $po_request, " <span class='glyphicon glyphicon-download-alt'></span>",
				'dlButton', 'Detail', 'DownloadRepresentation', '', array(
					'context'           => $ps_context,
					'representation_id' => $pt_representation->getPrimaryKey(),
					"id"                => $pn_subject_id,
					"download"          => 1,
					"version"           => $vs_download_version
				), array( "title" => _t( "Download" ) ) );
		}
	}
	$vs_tool_bar .= "</div><!-- end detailMediaToolbar -->\n";
...
# ---------------------------------------
/**
 * Extract and IIIF service-style media identifier from the current request. First checks for a "identifier" parameter,
 * which is assumed to be an IIIF service-style media identifier (Ex. representation:114; attribute:29341). If that is
 * not defined the numeric representation_id or value_id parameters are converted into representation and attribute
 * IIIF identifiers respectively.
 *
 * @param RequestHTTP $po_request The current request
 *
...
function caGetMediaAnnotationList( $po_data, $pa_options = null ) {
	...
	if (
		is_array( $va_annotations = $po_data->get( 'ca_representation_annotations.annotation_id',
		...
		while ( $qr_annotations->nextHit() ) {
			...
			$va_annotation_list[]
			           = "<a href='#' onclick='caUI.mediaPlayerManager.seek(\"caMediaOverlayTimebased_{$vn_rep_id}_{$ps_display_type}\", "
			             . ( (float) $qr_annotations->getPropertyValue( 'startTimecode', true )
			                 - (float) $va_props['timecode_offset'] ) . "); return false;'>"
			             . $qr_annotations->getWithTemplate( $ps_annotation_display_template ) . "</a>";
		}
	}
...
function caGetMediaViewerHTML( $po_request, $ps_identifier, $pt_subject, $pa_options = null ) {
	...
	$ps_context                     = caGetOption( 'context', $pa_options,
		$po_request->getParameter( 'context', pString ) );
	$ps_display_annotations         = caGetOption( 'displayAnnotations', $pa_options, false );
	$ps_annotation_display_template = caGetOption( 'displayAnnotationTemplate', $pa_options,
		caGetOption( 'displayAnnotationTemplate', $va_detail_config['options'],
			'^ca_representation_annotations.preferred_labels.name' ) );
	...
	switch ( $va_identifier['type'] ) {
		case 'representation':
			...
			if ( $pb_inline ) {
				$vs_caption = ( $vs_template = caGetOption( 'captionTemplate', $pa_options,
					caGetOption( 'captionTemplate', $va_display_info, null ) ) )
					? $t_instance->getWithTemplate( $vs_template ) : '';
			}
			...
			if ( ! $pn_subject_id ) {
				if ( is_array(
					     $va_subject_ids = $t_instance->get( $pt_subject->tableName() . '.' . $pt_subject->primaryKey(),
						     array( 'returnAsArray' => true ) ) )
			...
			$vs_viewer = $vs_viewer_name::getViewerHTML(
				...
				[
					...
					't_subject'    => $pt_subject,
					'display'      => $va_display_info,
					'display_type' => $ps_display_type
				],
				[
					'viewerWrapper'       => caGetOption( 'inline', $pa_options, false ) ? 'viewerInline' : null,
					'context'             => $ps_context,
					'hideOverlayControls' => $pb_hide_overlay_controls,
					'noOverlay'           => $pb_no_overlay,
					'checkAccess'         => $pa_check_acccess
				]
			);

			if ( $pb_inline ) {
				$vs_tool_bar = caRepToolbar( $po_request, $t_instance, $pt_subject, array(
					'display'     => $ps_display_type,
					'context'     => $ps_context,
					'checkAccess' => $pa_check_acccess
				) );
				$vs_viewer
				             = "<div class='repViewerContCont'><div id='cont{$pn_representation_id}' class='repViewerCont'>{$vs_viewer}{$vs_tool_bar}{$vs_caption}</div></div>";
			}

			if ( ( $ps_display_annotations )
			     && ( is_array( $va_annotation_list = caGetMediaAnnotationList( $t_instance, $pa_options ) ) )
			) {
				$vs_viewer .= join( "<br/>\n", $va_annotation_list );
			}
		...
		case 'attribute':
			...
			if ( ! ( $vs_viewer_name = MediaViewerManager::getViewerForMimetype( $ps_display_type,
				$vs_mimetype = $t_instance->getMediaInfo( 'value_blob', 'original', 'MIMETYPE' ) ) )
			) {
			...
			$vs_viewer = $vs_viewer_name::getViewerHTML(
				...
				[
					't_instance'   => $t_instance,
					't_subject'    => $pt_subject,
					'display'      => caGetMediaDisplayInfo( $ps_display_type, $vs_mimetype ),
					'display_type' => $ps_display_type
				],
				[
					'viewerWrapper'       => caGetOption( 'inline', $pa_options, false ) ? 'viewerInline' : null,
			...
			if ( $pb_inline ) {
				...
				$vs_viewer
				             = "<div data-value_id='{$pn_value_id}' class='repViewerContCont'><div id='cont{$pn_value_id}' class='repViewerCont'>{$vs_viewer}{$vs_tool_bar}{$vs_caption}{$vs_tool_bar}</div></div>";
			}
...
function caGetMediaViewerData( $po_request, $ps_identifier, $pt_subject, $pa_options = null ) {
	...
	switch ( $va_identifier['type'] ) {
		case 'representation':
			...
			if ( ! ( $vs_viewer_name = MediaViewerManager::getViewerForMimetype( $ps_display_type,
				$vs_mimetype = $t_instance->getMediaInfo( 'media', 'original', 'MIMETYPE' ) ) )
			) {
			...
			return $vs_viewer_name::getViewerData( $po_request, $ps_identifier, [
				't_subject'    => $pt_subject,
				't_instance'   => $t_instance,
				'display'      => $va_display_info,
				'display_type' => $ps_display_type,
...
function caSearchMediaData( $po_request, $ps_identifier, $pt_subject, $pa_options = null ) {
	...
	switch ( $va_identifier['type'] ) {
		case 'representation':
			...
			return $vs_viewer_name::searchViewerData( $po_request, $ps_identifier, [
				't_subject'    => $pt_subject,
				't_instance'   => $t_instance,
				'display'      => $va_display_info,
				'display_type' => $ps_display_type,
...
function caMediaDataAutocomplete( $po_request, $ps_identifier, $pt_subject, $pa_options = null ) {
	...
	switch ( $va_identifier['type'] ) {
		case 'representation':
			...
			return $vs_viewer_name::autocomplete( $po_request, $ps_identifier, [
				't_subject'    => $pt_subject,
				't_instance'   => $t_instance,
				'display'      => $va_display_info,
				'display_type' => $ps_display_type,
 ...
 *
 * @param RequestHTTP            $po_request The current request
 * @param SearchResult           $po_data    A ca_object_represention or RepresentableBaseModel search result to render
 *                                           slides for. A slide will be rendered for each representation.
 * @param RepresentableBaseModel $pt_subject A model instance loaded with the subject (the record the media is shown in
 *                                           the context of. Eg. if a representation is shown for an object this is an
 *                                           instance for that object record)
 * @param array                  $pa_options Options include:
 *                                           display = media_display.conf display version to use. [Default is 'detail']
 *
...
function caRepresentationViewerHTMLBundles( $po_request, $po_data, $pt_subject, $pa_options = null ) {
	...
	while ( $po_data->nextHit() ) {
		$va_representation_ids = $po_data->get( 'ca_object_representations.representation_id',
			[ 'returnAsArray' => true ] );
		...
		foreach ( $va_representation_ids as $vn_representation_id ) {
			...
			} elseif ( ! ( $vs_viewer_name = MediaViewerManager::getViewerForMimetype( $ps_display_type,
				$vs_mimetype ) )
			) {
				throw new ApplicationException( _t( 'Invalid viewer %1/%2', $ps_display_type, $vs_mimetype ) );
			}
			...
			$vs_tool_bar = caGetOption( 'noToolBar', $pa_options, false ) ? ""
				: caRepToolbar( $po_request, $t_instance, $pt_subject,
					array( 'display' => $ps_display_type, 'context' => caGetOption( 'context', $pa_options, null ) ) );

			$vs_caption = ( isset( $pa_options["captionTemplate"] ) && $pa_options["captionTemplate"] )
				? $po_data->getWithTemplate( $pa_options["captionTemplate"] ) : "";

			$va_reps[ $vn_rep_id = $po_data->get( 'ca_object_representations.representation_id' ) ]
				= "<div data-representation_id='{$vn_rep_id}' class='repViewerContCont'><div id='cont{$vn_rep_id}' class='repViewerCont'>"
...
function caGetTooltipJS( $pa_tooltips, $ps_class = 'tooltipFormat' ) {
	...
	foreach ( $pa_tooltips as $vs_element_selector => $vs_tooltip_text ) {
		$vs_buf .= "jQuery('{$vs_element_selector}').attr('title', '" . preg_replace( '![\n\r]{1}!', ' ',
				addslashes( $vs_tooltip_text ) )
		           . "').tooltip({ tooltipClass: '{$ps_class}', show: 150, hide: 150});\n";
	}
 ...
 * the detail controller. More usefully, you can pull data from the item being detailed by using a valid "get"
 * expression as a tag (Eg. {{{ca_objects.idno}}}. Even more usefully for some, you can also use a valid bundle display
 * template
...
function caDoTemplateTagSubstitution( $po_view, $pm_subject, $ps_template_path, $pa_options = null ) {
	...
	foreach ( $va_tag_list as $vs_tag ) {
		...
		if ( $pb_barcodes
		     && ( $vs_barcode_file = caParseBarcodeViewTag( $vs_tag, $po_view, $pm_subject, $pa_options ) )
		) {
			$va_barcode_files_to_delete[] = $vs_barcode_file;
		} elseif ( ( strpos( $vs_tag, "^" ) !== false ) || ( strpos( $vs_tag, "<" ) !== false ) ) {
			$po_view->setVar( $vs_tag,
				$pm_subject->getWithTemplate( $vs_tag, array( 'checkAccess' => $pa_access_values ) ) );
...
# ------------------------------------------------------------------
/**
 * Check if hierarchy browser drag-and-drop sorting is enabled for the current user in the current table for a given
 * item.
 *
 * @param RequestHTTP $pt_request The current request
 * @param string      $ps_table   The table being browsed
 * @param int         $pn_id      The primary key for the parent of the hierarchy level being browsed. Some tables
 *                                (notably ca_list_items) can have different enabled statuses for different items. If
 *                                null then status is determined at the table level. [Default is null]
 *
 * @return bool
 */
function caDragAndDropSortingForHierarchyEnabled( $pt_request, $ps_table, $pn_id = null ) {
	...
	if ( ! $pt_request->isLoggedIn()
	     || ( ! $pt_request->user->canDoAction( "can_edit_{$ps_table}" )
	          && ( ( $vs_hier_table = $t_instance->getProperty( 'HIERARCHY_DEFINITION_TABLE' ) )
				? ! $pt_request->user->canDoAction( "can_edit_{$vs_hier_table}" ) : false ) )
	) {
		return false;
	}
	if ( ! $t_instance->isHierarchical() ) {
	...
	if ( $vs_def_table_name && ( $t_def = Datamodel::getInstanceByTableName( $vs_def_table_name, true ) )
	     && ( $t_def->load( $t_instance->get( $vs_def_id_fld ) ) )
	     && ( $t_def->hasField( 'default_sort' ) )
	     && ( (int) $t_def->get( 'default_sort' ) === __CA_LISTS_SORT_BY_RANK__ )
	) {
		return true;
	} else {
...
# ------------------------------------------------------------------
/**
 * Return an array of statuses for drag-and-drop reordering by row_id for a given table. If the table does not support
 * separate drag-and-drop statuses per row then a boolean value is returned that applies to the entire table.
 *
 ...
 * @param string      $ps_table   The table being browsed
 * @param int         $pn_id      The primary key for the parent of the hierarchy level being browsed. Some tables
 *                                (notably ca_list_items) can have different enabled statuses for different items.
 *
 * @return mixed An array if the table supports per-row drag-and-drop reordering statuses, boolean values as would be
 *               returned by caDragAndDropSortingForHierarchyEnabled() otherwise.
 *
	...
	if ( $ps_table == 'ca_list_items' ) {
		...
		foreach ( $va_list_of_lists as $vn_list_id => $va_lists_by_locale ) {
			foreach ( $va_lists_by_locale as $vn_locale_id => $va_item ) {
				$va_map[ $va_item['root_id'] ] = caDragAndDropSortingForHierarchyEnabled( $pt_request, 'ca_list_items',
					$va_item['root_id'] );
			}
		}
...
function caExtractSettingValueByLocale( $pa_settings, $ps_key, $ps_locale ) {
	if ( isset( $pa_settings[ $ps_key ] ) ) {
		...
		if ( isset( $pa_settings[ $ps_key ][ $ps_locale ] ) && ( $pa_settings[ $ps_key ][ $ps_locale ] ) ) {
			...
			$va_locales_for_language = ca_locales::localesForLanguage( $ps_locale, [ 'codesOnly' => true ] ) )
		) {
...
function caProcessReferenceTags( $po_request, $ps_text, $pa_options = null ) {
	...
	foreach (
		[
			...
			'loan'       => 'ca_loans',
			'movement'   => 'ca_movements',
			'location'   => 'ca_storage_locations',
			'media'      => 'ca_site_page_media',
			'mediaRef'   => 'ca_attributes'
		] as $vs_ref_tag => $vs_ref_type
	) {
		if ( preg_match_all( "!\[{$vs_ref_tag} ([^\]]+)\]([^\[]+)\[/{$vs_ref_tag}\]!", $ps_text, $va_matches ) ) {
			foreach ( $va_matches[1] as $i => $vs_attr_string ) {
				if ( sizeof( $va_vals = caParseAttributes( $vs_attr_string, [ 'id', 'idno', 'class', 'version' ] ) )
				     > 0
				) {
			...
			switch ( $vs_ref_type ) {
				case 'ca_attributes':
					foreach ( $va_tags as $vs_tag => $va_tag ) {
						...
						if ( $vs_template = $va_tag['content'] ) {
							...
							$ps_text = str_replace( $vs_tag, caProcessTemplate( $vs_template,
								$t_attr->getAttributeValues( [
									'returnAs' => 'array',
									'version'  => caGetOption( 'version', $va_tag,
										array_shift( $t_attr->getMediaVersions( 'value_blob' ) ) )
								] ), [] ), $ps_text );
						} else {
							$t_val   = new ca_attribute_values( $vn_value_id );
							$ps_text = str_replace( $vs_tag, $t_val->getMediaTag( 'value_blob',
								caGetOption( 'version', $va_tag,
									array_shift( $t_val->getMediaVersions( 'value_blob' ) ) ) ), $ps_text );
						}
					}
					break;
				case 'ca_site_page_media':
					foreach ( $va_idnos[ $vs_ref_type ] as $vs_tag => $va_l ) {
						...
						while ( $qr_m->nextHit() ) {
							if ( is_array( $va_access_values )
							     && ! in_array( $qr_m->get( 'access' ), $va_access_values )
							) {
							...
							$alt_text = caGetOption( [ 'caption', 'title', 'idno' ],
								[ 'caption' => $caption, 'title' => $title, 'idno' => $idno ], null );
							...
							if ( $vs_template = $va_l['content'] ) {
								...
								$vs_template = str_replace( "^file", $qr_m->getMediaTag( 'media',
									caGetOption( 'version', $va_l, array_shift( $qr_m->getMediaVersions( 'media' ) ) ),
									[ 'alt' => $alt_text ] ), $vs_template );
								$ps_text     = str_replace( $vs_tag, $vs_template, $ps_text );
							} else {
								$ps_text = str_replace( $vs_tag, $qr_m->getMediaTag( 'media',
									caGetOption( 'version', $va_l, array_shift( $qr_m->getMediaVersions( 'media' ) ) ),
									[ 'alt' => $alt_text ] ), $ps_text );
							}
				...
				default:
					$va_map = call_user_func( $vs_ref_type . '::getIDsForIdnos', array_map( function ( $v ) {
						return $v['idno'];
					}, $va_idnos[ $vs_ref_type ] ),
						[ 'forceToLowercase' => true, 'checkAccess' => $va_access_values ] );
					...
					foreach ( $va_idnos[ $vs_ref_type ] as $vs_tag => $va_l ) {
						...
						if ( isset( $va_l['id'] ) && $va_l['id'] ) {
							switch ( __CA_APP_TYPE__ ) {
								case 'PROVIDENCE':
									$vs_link_text = caEditorLink( $po_request, $va_l['content'], $va_l['class'],
										$vs_ref_type, $va_l['id'] );
									break;
								case 'PAWTUCKET':
									$vs_link_text = caDetailLink( $po_request, $va_l['content'], $va_l['class'],
										$vs_ref_type, $va_l['id'] );
									break;
							}
						}
...
function caGetLookupUrlsForTables( $po_request = null, $pa_tables = null ) {
	...
	foreach ( $pa_tables as $vs_table ) {
		...
		$va_lookup_urls[ $vs_table ] = [
			'singular' => Datamodel::getTableProperty( $vs_table, 'NAME_SINGULAR' ),
			'plural'   => Datamodel::getTableProperty( $vs_table, 'NAME_PLURAL' ),
			'code'     => strtolower( Datamodel::getTableProperty( $vs_table, 'NAME_SINGULAR' ) ),
			'url'      => $va_urls['search']
		];
	}
			...
			foreach ( $va_types as $vs_type => $va_config ) {
				switch ( $vs_table ) {
					...
					case 'ca_objects_x_storage_locations':
						$va_bundle_settings["{$vs_table}_showRelationshipTypes"][]
							= $t_rel_type->getRelationshipTypeID( 'ca_objects_x_storage_locations', $vs_type );
						break;
					default:
						if ( ! is_array( $va_config ) ) {
							break;
						}
						$va_bundle_settings["{$vs_table}_showTypes"][]            = array_shift( caMakeTypeIDList( $vs_table,
							array( $vs_type ) ) );
 ...
 *
 * @param string $table   Table name of primary record (Eg. ca_objects when downloaded representations related to an
 *                        object).
 ...
 *                        mode = Naming mode. Can be idno, idno_and_version, idno_and_rep_id_and_version,
 *                        original_name. If not set defaults to value in <table>_downloaded_file_naming or
 *                        _downloaded_file_naming app.conf directive.
 *
 * @return string File name
 */
function caGetRepresentationDownloadFileName( string $table, array $data, ?array $options = null ): string {
	$config = Configuration::load();
	switch ( $mode = caGetOption( 'mode', $options,
		$config->get( [ "{$table}_downloaded_file_naming", 'downloaded_file_naming' ] ) ) ) {
		case 'idno':
			$filename = $data['idno'] . ( strlen( $data['index'] ) ? '_' . $data['index'] : '' ) . '.'
			            . $data['extension'];
			break;
		case 'idno_and_version':
			$filename = $data['idno'] . '_' . $data['version'] . '_' . ( strlen( $data['index'] ) ? '_' . $data['index']
					: '' ) . '.' . $data['extension'];
			break;
		...
		default:
			if ( strpos( $mode, "^" ) !== false ) { // template
				$filename = preg_replace( '!\.[A-Za-z]{1}[A-Za-z0-9]{1,3}$!', '',
					caProcessTemplateForIDs( $mode, 'ca_object_representations', [ $data['representation_id'] ] ) );
...
function caGetMediaDownloadArchiveName( $table, $id, $options = null ) {
	$config = Configuration::load();
	switch ( $mode = $config->get( [
		"{$table}_downloaded_media_archive_file_naming",
		'downloaded_media_archive_file_naming',
		"{$table}_downloaded_file_naming",
