 ...
 *
 * @param string $ps_value The UTM expression in the format <zone><hemisphere> <easting> <northing>. Example:  17N
 *                         630084 4833438
 *
 * @return array Array with 'latitude' and 'longitude' keys and decimal values. Return null if value cannot be
 *               converted.
 */
	...
	while ( sizeof( $va_tokens ) ) {
		...
		switch ( $vn_state ) {
			...
			case 3:    // distance
				...
				$vn_length     = ( (float) array_shift( explode( ' ',
						preg_replace( '![^\d\.]+!', '', $va_length_val['value_decimal1'] ) ) ) )
				                 / 1000;        // kilometers
 ...
 *
 * @param string $ps_country The country code. If set to a false value the entire state list, with states listed by
 *                           country code, is returned.
 *
 * @return array A list of state/province/zones for the specified country. An empty array will be returned if there are
 *               none defined for the specified country. Null will be returned if the country code is invalid.
 */
