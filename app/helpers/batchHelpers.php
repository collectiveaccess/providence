...
function caBatchEditorRelationshipModeControl($t_item, $ps_id_prefix)
{
    switch ($t_item->tableName()) {
        case 'ca_object_representations':
            $vs_select = caHTMLSelect(
                $ps_id_prefix . "_batch_mode",
                array(
...
function caBatchEditorSetsModeControl($pn_table_num, $ps_id_prefix)
{
    $vs_buf = "	<div class='editorBatchModeControl'>" . _t("In batch") . " " .
        caHTMLSelect(
            ...
            array('id' => $ps_id_prefix . $pn_table_num . '_sets_batch_mode_select')
        ) . "</div>\n
...
function caBatchEditorPreferredLabelsModeControl($t_item, $ps_id_prefix)
{
    $vs_buf = "	<div class='editorBatchModeControl'>" . _t("In batch") . " " .
        caHTMLSelect(
            ...
            array('id' => $ps_id_prefix . 'Labels_batch_mode_select')
        ) . "</div>\n
...
function caBatchEditorIntrinsicModeControl($t_item, $ps_id_prefix)
{
    $vs_buf = "<div class='editorBatchModeControl'>" . _t("In batch") . " " .
        caHTMLSelect(
            ...
            array("id" => "{$ps_id_prefix}_batch_mode_select")
        ) . "</div>\n
	<script type=\"text/javascript\">
		jQuery(document).ready(function() {
    ...
    foreach ($va_replacements_list as $vs_replacement_code => $va_replacement) {
        if (!isset($va_replacement['search']) || !is_array($va_replacement['search'])) {
            if ($o_log) {
                $o_log->logError(
                    _t(
                        "List of search expressions for replacement %1 is invalid. Check your configuration.",
                        $vs_replacement_code
                    )
                );
            }
        ...
        if (!isset($va_replacement['replace']) || !is_array($va_replacement['replace'])) {
            if ($o_log) {
                $o_log->logError(
                    _t(
                        "List of replacement patterns for replacement %1 is invalid. Check your configuration.",
                        $vs_replacement_code
                    )
                );
            }
        ...
        if (sizeof($va_replacement['search']) != sizeof($va_replacement['replace'])) {
            if ($o_log) {
                $o_log->logError(
                    _t(
                        "The search and replacement pattern lists for replacement %1 have different lengths. Check your configuration.",
                        $vs_replacement_code
                    )
                );
            }
        ...
        foreach ($va_replacement['search'] as $vs_search) {
            if (@preg_match('!' . $vs_search . '!', "Just a test") === false) {
                if ($o_log) {
                    $o_log->logError(
                        _t(
                            "One of the search patterns for replacement %1 is not a valid PCRE. Check your configuration.",
                            $vs_replacement_code
                        )
                    );
                }
...
function caBatchFindMatchingMedia($ps_directory, $ps_value, $pa_options = null)
{
    ...
    foreach ($va_files as $vs_file) {
        if (preg_match('!@SynoResource!', $vs_file)) {
            continue;
        }
                ...
                foreach ($va_names_to_match as $vs_match_name) {
                    if (preg_match('!' . $vs_regex . '!', $vs_match_name, $va_matches)) {
                        ...
                        switch (strtoupper($ps_match_type)) {
                            ...
                            default:
                                $vb_match = ((strtolower($va_matches[1]) === strtolower($ps_value)) || (strtolower(
                                            $va_matches[1]
                                        ) === strtolower(pathinfo($ps_value, PATHINFO_FILENAME))));
                                break;
                        }
                        if ($vb_match) {
