        ...
        foreach ($v['regexes'] as $match) {
            if (@preg_match('!' . $match . '!', "Just a test") === false) {
                if ($o_log) {
                    $o_log->logError(
                        _t(
                            "Filename-to-idno matching replacement pattern %1 in %2 is not a valid PCRE. Check your configuration.",
                            $match,
                            $k
                        )
                    );
        ...
        foreach ($v['regexes'] as $match => $replace) {
            if (@preg_match('!' . $match . '!', "Just a test") === false) {
                if ($o_log) {
                    $o_log->logError(
                        _t(
                            "Idno replacement pattern %1 in %2 is not a valid PCRE. Check your configuration.",
                            $match,
                            $k
                        )
                    );
        ...
        foreach ($v['regexes'] as $match => $replace) {
            if (@preg_match('!' . $match . '!', "Just a test") === false) {
                if ($o_log) {
                    $o_log->logError(
                        _t(
                            "Filename replacement pattern %1 in %2 is not a valid PCRE. Check your configuration.",
                            $match,
                            $k
                        )
                    );
            ...
            foreach ($va_regex_info['regexes'] as $vs_regex) {
                ...
                foreach ($va_names_to_match as $vs_match_name) {
                    if (preg_match('!' . $vs_regex . '!', $vs_match_name, $va_matches)) {
                        ...
                        if ($o_log) {
                            $o_log->logDebug(
                                _t("Extracted value from name %1 using regex %2", $vs_match_name, $vs_regex)
                            );
                        }
                        ...
                        switch (strtoupper($ps_match_type)) {
                            ...
                            default:
                                ...
                                if ($o_log) {
                                    $o_log->logDebug(
                                        _t("Case-insensitive match on %1 to value %2", $va_matches[1], $ps_value)
                                    );
                                }
                                break;
                        }
