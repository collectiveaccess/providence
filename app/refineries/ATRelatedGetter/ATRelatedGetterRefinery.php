...
{
    # -------------------------------------------------------
    static public $s_sql_field_cache;
    ...
    public function refine(&$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null)
    {
        ...
        if (ATRelatedGetterRefinery::$s_sql_field_cache[$vs_rel_table]) {
            ...
            ATRelatedGetterRefinery::$s_sql_field_cache[$vs_rel_table] = array(
                'fields' => $va_sql_field_list,
                'bitfields' => $va_bit_fields
            );
        }
                ...
                if (!is_array($vs_at_key)) {
                    ...
                    if (isset($va_relationship_map[$va_tmp[0]]) && ($va_keys = $va_relationship_map[$va_tmp[0]])) {
                        ...
                        if (ATRelatedGetterRefinery::$s_sql_field_cache[$va_tmp[0]]) {
                            ...
                            ATRelatedGetterRefinery::$s_sql_field_cache[$va_tmp[0]] = array(
                                'fields' => $va_sql_field_list,
                                'bitfields' => $va_sub_bit_fields
                            );
                        }
                        ...
                        if ($qr_relrel->nextRow()) {
                            $va_val[$vs_ca_key] = BaseRefinery::parsePlaceholder(
                                $qr_relrel->get($va_tmp[1]),
                                $pa_source_data,
                                $pa_item
                            );
                        } else {
...
BaseRefinery::$s_refinery_settings['ATRelatedGetter'] = array(
    ...
    'ATRelatedGetter_relationships' => array(
        ...
        'default' => '',
        'label' => _t(
            'Map of tables and foreign keys related to the related table we are getting from. Keys are [table].[field] specification in tables RELATED to the table we are getting from. Values are foreign keys IN the related table we are getting from.'
        ),
        'description' => _t('Map of related table field values to CollectiveAccess element')
    ),
