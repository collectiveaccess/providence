		...
		while ( $qr_res->nextRow() ) {
			...
			foreach ( $va_map as $vs_ca_key => $vs_at_key ) {
				...
				if ( ! is_array( $vs_at_key ) ) {
					...
					if ( isset( $va_relationship_map[ $va_tmp[0] ] )
					     && ( $va_keys
							= $va_relationship_map[ $va_tmp[0] ] )
					) {
						...
						if ( ATRelatedGetterRefinery::$s_sql_field_cache[ $va_tmp[0] ] ) {
							...
							ATRelatedGetterRefinery::$s_sql_field_cache[ $va_tmp[0] ]
								= array( 'fields' => $va_sql_field_list, 'bitfields' => $va_sub_bit_fields );
						}
						...
						if ( $qr_relrel->nextRow() ) {
							$va_val[ $vs_ca_key ] = BaseRefinery::parsePlaceholder( $qr_relrel->get( $va_tmp[1] ),
								$pa_source_data, $pa_item );
						} else {
							$va_val[ $vs_ca_key ] = '';
						}
						continue;
