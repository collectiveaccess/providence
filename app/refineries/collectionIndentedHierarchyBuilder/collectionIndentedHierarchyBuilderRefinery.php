...
class collectionIndentedHierarchyBuilderRefinery extends BaseRefinery
{
    ...
    static public $opa_level_values = null;
    static public $opa_level_value_ids = null;
    ...
    public function __construct()
    {
        ...
        $this->ops_description = _t(
            'Imports spreadsheets with hierarchies expressed indented values spread across several columns as hierarchical collections.'
        );
    ...
    public function refine(&$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null)
    {
        ...
        if ($t_mapping) {
            if ($t_mapping->get('table_num') != Datamodel::getTableNum('ca_collections')) {
                if ($o_log) {
                    $o_log->logError(
                        _t("collectionIndentedHierarchyBuilder refinery may only be used in imports to ca_collections")
                    );
                }
                return null;
            }
        ...
        if (!is_array($va_levels = $pa_item['settings']['collectionIndentedHierarchyBuilder_levels'])) {
            if ($o_log) {
                $o_log->logError(
                    _t(
                        "collectionIndentedHierarchyBuilder requires levels option be set to a list of data source placeholders"
                    )
                );
            }
            return null;
        ...
        foreach ($va_levels as $vn_i => $vs_level_placeholder) {
            ...
            if (strlen($vs_level_placeholder)) {
                if ($vs_level_value = BaseRefinery::parsePlaceholder(
                    $vs_level_placeholder,
                    $pa_source_data,
                    $pa_item,
                    0,
                    array(
                    ...
                    $vs_idno_val = BaseRefinery::parsePlaceholder(
                        $vs_idno,
                        $pa_source_data,
                        $pa_item,
                        0,
                    ...
                    $values = [
                        ...
                        'idno' => $vs_idno_val ? $vs_idno_val : preg_replace("![^A-Za-z0-9_]+!", "_", $vs_level_value)
                    ];
                    ...
                    if ($vn_collection_id = DataMigrationUtils::getCollectionID(
                        $vs_level_value,
                        $vs_type,
                        $pn_locale_id,
                        $values,
                        ...
                        if (is_array($rels) && is_array($rels['_related_related']) && sizeof(
                                $rels['_related_related']
                            )) {
                                    ...
                                    foreach ($rels_by_table as $rel) {
                                        if (!$t_coll->addRelationship($table, $rel['id'], $rel['_relationship_type'])) {
                                            if ($o_log) {
                                                $o_log->logError(
                                                    _t(
                                                        "Could not create relationship between collection and %1: %2",
                                                        $table,
                                                        join("; ", $t_coll->getErrors())
                                                    )
                                                );
                                            }
                                        }
        ...
        collectionIndentedHierarchyBuilderRefinery::$opa_level_values = array_slice(
            $va_level_values,
            0,
            $vn_max_level + 1
        );
        collectionIndentedHierarchyBuilderRefinery::$opa_level_value_ids = array_slice(
            $va_level_value_ids,
            0,
            $vn_max_level + 1
        );
...
BaseRefinery::$s_refinery_settings['collectionIndentedHierarchyBuilder'] = array(
    ...
    'collectionIndentedHierarchyBuilder_mode' => array(
        ...
        'description' => _t(
            'Set to "returnData" to return the id of lowest item in the hierarchy to the importer; set to "processOnly" to create the list items in the hierarchy but not return values to the importer. Default is to process only.'
        )
    ),
