...
class collectionIndentedHierarchyBuilderRefinery extends BaseRefinery {
	...
	public function __construct() {
		...
		$this->ops_description
		                 = _t( 'Imports spreadsheets with hierarchies expressed indented values spread across several columns as hierarchical collections.' );
	...
	public function refine( &$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null ) {
		...
		if ( ! is_array(
			$va_relationships = $pa_item['settings']['collectionIndentedHierarchyBuilder_relationships'] )
		) {
		...
		foreach ( $va_levels as $vn_i => $vs_level_placeholder ) {
			...
			if ( strlen( $vs_level_placeholder ) ) {
				if ( $vs_level_value = BaseRefinery::parsePlaceholder( $vs_level_placeholder, $pa_source_data, $pa_item,
					0, array( 'reader' => $o_reader, 'returnAsString' => true ) )
				) {
					if ( ! $vn_parent_id
					     && isset( collectionIndentedHierarchyBuilderRefinery::$opa_level_value_ids[ $vn_i - 1 ] )
					) {
					...
					$vs_idno_val = BaseRefinery::parsePlaceholder( $vs_idno, $pa_source_data, $pa_item, 0,
					...
					$values = [
						...
						'idno'      => $vs_idno_val
							? $vs_idno_val : preg_replace( "![^A-Za-z0-9_]+!", "_", $vs_level_value )
					];
					...
					if ( is_array( $va_relationships ) ) {
						$rels = [];
						caProcessRefineryRelatedMultiple( $this, $pa_item, $pa_source_data, null, $o_log, $o_reader,
							$rels, $attr_values, [] );

					}
					if ( $vn_collection_id = DataMigrationUtils::getCollectionID( $vs_level_value, $vs_type,
						$pn_locale_id, $values, array(
							'matchOnIdno'       => false,
							'log'               => $o_log,
							'transaction'       => $o_trans,
							'importEvent'       => caGetOption( 'event', $pa_options, null ),
							'importEventSource' => 'collectionIndentedHierarchyBuilder'
						) )
					) {
						...
						if ( is_array( $rels ) && is_array( $rels['_related_related'] )
						     && sizeof( $rels['_related_related'] )
						) {
							...
							if ( $t_coll->load( $vn_collection_id ) ) {
								foreach ( $rels['_related_related'] as $table => $rels_by_table ) {
									foreach ( $rels_by_table as $rel ) {
										if ( ! $t_coll->addRelationship( $table, $rel['id'],
											$rel['_relationship_type'] )
										) {
											if ( $o_log ) {
												$o_log->logError( _t( "Could not create relationship between collection and %1: %2",
													$table, join( "; ", $t_coll->getErrors() ) ) );
											}
										}
									}
								}
		...
		collectionIndentedHierarchyBuilderRefinery::$opa_level_values    = array_slice( $va_level_values, 0,
			$vn_max_level + 1 );
		collectionIndentedHierarchyBuilderRefinery::$opa_level_value_ids = array_slice( $va_level_value_ids, 0,
			$vn_max_level + 1 );
