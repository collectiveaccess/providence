...
class entityJoinerRefinery extends BaseRefinery {
	...
	public function refine( &$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null ) {
		...
		foreach ( $va_entities as $pm_value ) {
			...
			if ( is_array( $va_skip_values = $pa_item['settings']['entityJoiner_skipIfValue'] )
			...
			foreach ( $t_entity->getLabelUIFields() as $vs_fld ) {
				$va_name[ $vs_fld ] = BaseRefinery::parsePlaceholder( $pa_item['settings'][ 'entityJoiner_' . $vs_fld ],
			...
			if ( ( ! isset( $va_val['_relationship_type'] ) || ! $va_val['_relationship_type'] )
			     && ( $vs_rel_type_opt
					= $pa_item['settings']['entityJoiner_relationshipTypeDefault'] )
			) {
				$va_val['_relationship_type'] = BaseRefinery::parsePlaceholder( $vs_rel_type_opt, $pa_source_data,
					$pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) );
			...
			if ( ( ! isset( $va_val['_type'] ) || ! $va_val['_type'] )
			     && ( $vs_type_opt
					= $pa_item['settings']['entityJoiner_entityTypeDefault'] )
			) {
				$va_val['_type'] = BaseRefinery::parsePlaceholder( $vs_type_opt, $pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) );
			}
			...
			// Set attributes
			if ( is_array( $va_attr_vals = caProcessRefineryAttributes( $pa_item['settings']['entityJoiner_attributes'],
				$pa_source_data, $pa_item, $vn_c,
			...
			if ( isset( $pa_options['mapping'] )
			     && is_array( $va_attr_vals = caProcessInterstitialAttributes( 'entityJoiner',
					$pa_options['mapping']->get( 'table_num' ), 'ca_entities', $pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'log' => $o_log ) ) )
			) {
				$va_val = array_merge( $va_val, $va_attr_vals );
			}

			caProcessRefineryRelatedMultiple( $this, $pa_item, $pa_source_data, $vn_c, $o_log,
				caGetOption( 'reader', $pa_options, null ), $va_val, $va_vals, $pa_options );

			if ( isset( $pa_item['settings']['entityJoiner_nonpreferred_labels'] )
			     && ! isset( $pa_item['settings']['entityJoiner_nonPreferredLabels'] )
			) {
				$pa_item['settings']['entityJoiner_nonPreferredLabels']
					= $pa_item['settings']['entityJoiner_nonpreferred_labels'];
			}
			...
			if ( is_array( $pa_item['settings']['entityJoiner_nonPreferredLabels'] ) ) {
					...
					if ( is_array( $va_elements ) ) {
						...
						foreach ( $va_elements as $vs_k => $vs_v ) {
							...
							if ( $vs_k == 'split' ) {
								...
								if ( $vs_name = BaseRefinery::parsePlaceholder( $vs_v, $pa_source_data, $pa_item, $vn_c,
									array(
									...
									$va_non_preferred_labels[ $vn_index ]
										                       = array_merge( $va_non_preferred_labels[ $vn_index ],
										DataMigrationUtils::splitEntityName( $vs_name ) );
									$vb_non_pref_label_was_set = true;
								}
							} else {
								if ( $va_non_preferred_labels[ $vn_index ][ $vs_k ]
									= trim( BaseRefinery::parsePlaceholder( $vs_v, $pa_source_data, $pa_item, $vn_c,
									array(
										'reader'         => caGetOption( 'reader', $pa_options, null ),
										'returnAsString' => true
									) ) )
						...
						if ( $va_non_preferred_labels[ $vn_index ]['displayname']
							= trim( BaseRefinery::parsePlaceholder( $va_elements, $pa_source_data, $pa_item, $vn_c,
							array(
								'reader'         => caGetOption( 'reader', $pa_options, null ),
								'returnAsString' => true
							) ) )
				...
				if ( $va_non_preferred_labels[0]
					= DataMigrationUtils::splitEntityName( trim( BaseRefinery::parsePlaceholder( $pa_item['settings']['entityJoiner_nonPreferredLabels'],
					$pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) ) ) )
				) {
					$vb_non_pref_label_was_set = true;
