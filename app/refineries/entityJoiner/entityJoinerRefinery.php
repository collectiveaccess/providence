...
class entityJoinerRefinery extends BaseRefinery
{
    ...
    public function refine(&$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null)
    {
        ...
        foreach ($va_entities as $pm_value) {
            ...
            if (is_array($va_skip_values = $pa_item['settings']['entityJoiner_skipIfValue']) && in_array(
                    $vs_entity,
                    $va_skip_values
                )) {
                return array();
            ...
            foreach ($t_entity->getLabelUIFields() as $vs_fld) {
                $va_name[$vs_fld] = BaseRefinery::parsePlaceholder(
                    $pa_item['settings']['entityJoiner_' . $vs_fld],
                    $pa_source_data,
                    $pa_item,
                    $vn_c,
                    array(
            ...
            if (is_array(
                $va_attr_vals = caProcessRefineryAttributes(
                    $pa_item['settings']['entityJoiner_attributes'],
                    $pa_source_data,
                    $pa_item,
                    $vn_c,
                    array('reader' => caGetOption('reader', $pa_options, null), 'log' => $o_log)
                )
            )) {
            ...
            if (isset($pa_options['mapping']) && is_array(
                    $va_attr_vals = caProcessInterstitialAttributes(
                        'entityJoiner',
            ...
            caProcessRefineryRelatedMultiple(
                ...
                $vn_c,
                $o_log,
                caGetOption('reader', $pa_options, null),
                $va_val,
                $va_vals,
                $pa_options
            );
            ...
            if (is_array($pa_item['settings']['entityJoiner_nonPreferredLabels'])) {
                    ...
                    if (is_array($va_elements)) {
                        ...
                        foreach ($va_elements as $vs_k => $vs_v) {
                            ...
                            if ($vs_k == 'split') {
                                ...
                                if ($vs_name = BaseRefinery::parsePlaceholder(
                                    $vs_v,
                                    $pa_source_data,
                                    $pa_item,
                                    $vn_c,
                                    ...
                                    $va_non_preferred_labels[$vn_index] = array_merge(
                                        $va_non_preferred_labels[$vn_index],
                                        DataMigrationUtils::splitEntityName($vs_name)
                                    );
                                    $vb_non_pref_label_was_set = true;
                                }
                            } else {
                                if ($va_non_preferred_labels[$vn_index][$vs_k] = trim(
                        ...
                        if ($va_non_preferred_labels[$vn_index]['displayname'] = trim(
                            BaseRefinery::parsePlaceholder(
                                $va_elements,
                                $pa_source_data,
                                $pa_item,
                ...
                if ($va_non_preferred_labels[0] = DataMigrationUtils::splitEntityName(
                    trim(
                        BaseRefinery::parsePlaceholder(
                            $pa_item['settings']['entityJoiner_nonPreferredLabels'],
                            $pa_source_data,
                            $pa_item,
                            $vn_c,
                            array(
...
BaseRefinery::$s_refinery_settings['entityJoiner'] = array(
    ...
    'entityJoiner_middlename' => array(
        ...
        'description' => _t(
            'Accepts a constant value for the middle name or a reference to the location in the data source where the middle name can be found.'
        )
    ),
    ...
    'entityJoiner_displayname' => array(
        ...
        'description' => _t(
            'Accepts a constant value for the display name or a reference to the location in the data source where the display name can be found.'
        )
    ),
    'entityJoiner_prefix' => array(
        ...
        'label' => _t('Prefix'),
        'description' => _t(
            'Accepts a constant value for the entity prefix or a reference to the location in the data source where the prefix can be found.'
        )
    ),
    'entityJoiner_suffix' => array(
        ...
        'label' => _t('Suffix'),
        'description' => _t(
            'Accepts a constant value for the entity suffix or a reference to the location in the data source where the suffix can be found.'
        )
    ),
    ...
    'entityJoiner_nonPreferredLabels' => array(
        ...
        'description' => _t(
            'List of non-preferred label values or references to locations in the data source where nonpreferred label values can be found. Use the <em>split</em> value for a label to indicate a value that should be split into entity label components before import.'
        )
    ),
    'entityJoiner_interstitial' => array(
        ...
        'description' => _t(
            'Sets or maps metadata for the interstitial entity <em>relationship</em> record by referencing the metadataElement code and the location in the data source where the data values can be found.'
        )
    )
);
?>