...
class measurementsSplitterRefinery extends BaseRefinery
{
    ...
    public function refine(&$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null)
    {
        ...
        foreach ($va_measurements as $vs_measurement) {
            ...
            $va_parsed_measurements = caParseLengthExpression(
                $vs_measurement,
                [
                    'returnExtractedMeasurements' => true,
                    'delimiter' => $pa_item['settings']['measurementsSplitter_delimiter'],
                    'units' => $pa_item['settings']['measurementsSplitter_units']
                ]
            );
            ...
            if (is_array($va_elements = $pa_item['settings']['measurementsSplitter_elements'])) {
                ...
                foreach ($va_elements as $vn_i => $va_element) {
                    ...
                    if (((strpos($va_measurement['source'], '/') !== false) || (preg_match(
                            "![\d\.]+[ ]*[\"'a-zA-Z\.]+[ ]+[\d\.]+[ ]*[a-zA-Z\.\"\']+!",
                            $va_measurement['source']
                        )))) {
                    ...
                    if ($vs_measurement) {
                        ...
                        if (isset($va_element['typeElement']) && $va_element['typeElement']) {
                            $va_val[$va_element['typeElement']] = BaseRefinery::parsePlaceholder(
                                $va_element["type"],
                                $pa_source_data,
                                $pa_item,
                                $vn_c,
                ...
                if (is_array($pa_item['settings']['measurementsSplitter_attributes'])) {
                    ...
                    foreach ($pa_item['settings']['measurementsSplitter_attributes'] as $vs_element_code => $vs_v) {
                        ...
                        $va_attr_vals[$vs_element_code] = BaseRefinery::parsePlaceholder(
                            $vs_v,
                            $pa_source_data,
                            $pa_item,
                            $vn_c,
