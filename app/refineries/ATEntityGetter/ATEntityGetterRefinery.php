...
class ATEntityGetterRefinery extends BaseRefinery
{
    ...
    public function refine(&$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null)
    {
        ...
        while ($qr_rel_names->nextRow()) {
            ...
            if (is_array($va_skip_values = $pa_item['settings']['ATEntityGetter_skipIfValue']) && in_array(
                    $vs_entity,
                    $va_skip_values
                )) {
                continue;
            ...
            if (!preg_match(
                    ...
                    $va_row['role'],
                    $va_matches
                ) || !($va_val['_relationship_type'] = trim($va_matches[1]))) {
                ...
                foreach ($pa_item['settings']['ATEntityGetter_attributes'] as $vs_element_code => $va_attrs) {
                    if (is_array($va_attrs)) {
                        foreach ($va_attrs as $vs_k => $vs_v) {
                            $va_attr_vals[$vs_element_code][$vs_k] = BaseRefinery::parsePlaceholder(
                                $vs_v,
                                $pa_source_data,
                                $pa_item,
                                $vs_delimiter,
...
BaseRefinery::$s_refinery_settings['ATEntityGetter'] = array(
    'ATEntityGetter_key' => array(
        ...
        'label' => _t('Key'),
        'description' => _t(
            'Foreign key in ArchDescriptionNames to select on (eg. ResourceID, resourceComponentId, accessionId, digitalObjectId.'
        )
    ),
    ...
    'ATEntityGetter_entityTypes' => array(
        ...
        'description' => _t(
            'Map equating three AT name types (personal, family and corporate) with CA entity_types. Keys are AT types, values are CA types.'
        )
    ),
