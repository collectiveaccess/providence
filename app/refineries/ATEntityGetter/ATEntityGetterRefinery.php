...
class ATEntityGetterRefinery extends BaseRefinery {
	...
	public function refine( &$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null ) {
		...
		$o_trans = ( isset( $pa_options['transaction'] ) && ( $pa_options['transaction'] instanceof Transaction ) )
			? $pa_options['transaction'] : null;
		...
		while ( $qr_rel_names->nextRow() ) {
			...
			if ( is_array( $va_skip_values = $pa_item['settings']['ATEntityGetter_skipIfValue'] )
			     && in_array( $vs_entity, $va_skip_values )
			) {
				continue;
			}
			...
			if ( ! preg_match( "!^([A-Za-z0-9 ]+)!", $va_row['role'], $va_matches )
			     || ! ( $va_val['_relationship_type']
					= trim( $va_matches[1] ) )
			) {
				if ( $vs_rel_type_opt = $pa_item['settings']['ATEntityGetter_relationshipTypeDefault'] ) {
					$va_val['_relationship_type'] = BaseRefinery::parsePlaceholder( $vs_rel_type_opt, $pa_source_data,
						$pa_item, $vs_delimiter, $vn_c );
				}
			}
			$va_val['_relationship_type'] = str_replace( " ", "_", $va_val['_relationship_type'] );
			$va_val['_relationship_type'] = BaseRefinery::parsePlaceholder( $va_val['_relationship_type'],
				$pa_source_data, $pa_item, $vs_delimiter, $vn_c );
				...
				foreach ( $pa_item['settings']['ATEntityGetter_attributes'] as $vs_element_code => $va_attrs ) {
					if ( is_array( $va_attrs ) ) {
						foreach ( $va_attrs as $vs_k => $vs_v ) {
							$va_attr_vals[ $vs_element_code ][ $vs_k ] = BaseRefinery::parsePlaceholder( $vs_v,
								$pa_source_data, $pa_item, $vs_delimiter, $vn_c );
						}
					}
				}
