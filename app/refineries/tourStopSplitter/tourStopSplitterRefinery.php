...
class tourStopSplitterRefinery extends BaseRefinery {
	...
	public function refine( &$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null ) {
		$this->opb_returns_multiple_values = true;
		$o_log                             = ( isset( $pa_options['log'] ) && is_object( $pa_options['log'] ) )
			? $pa_options['log'] : null;
		...
		foreach ( $va_tour_stops as $vn_i => $vs_tour_stop ) {
			...
			if ( ( ! isset( $va_val['_relationship_type'] ) || ! $va_val['_relationship_type'] )
			     && ( $vs_rel_type_opt
					= $pa_item['settings']['tourStopSplitter_relationshipTypeDefault'] )
			) {
				$va_val['_relationship_type'] = BaseRefinery::parsePlaceholder( $vs_rel_type_opt, $pa_source_data,
					$pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) );
			...
			if ( ( ! isset( $va_val['_relationship_type'] ) || ! $va_val['_relationship_type'] ) && $o_log ) {
				$o_log->logWarn( _t( '[tourStopSplitterRefinery] No relationship type is set for tour stop %1',
					$vs_tour_stop ) );
			}
			...
			if ( ( ! isset( $va_val['_type'] ) || ! $va_val['_type'] )
			     && ( $vs_type_opt
					= $pa_item['settings']['tourStopSplitter_tourStopTypeDefault'] )
			) {
				$va_val['_type'] = BaseRefinery::parsePlaceholder( $vs_type_opt, $pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) );
			}
			...
			if ( ( ! isset( $va_val['_type'] ) || ! $va_val['_type'] ) && $o_log ) {
				$o_log->logWarn( _t( '[tourStopSplitterRefinery] No collection type is set for tour stop %1',
					$vs_tour_stop ) );
			}
			...
			if ( ! $vn_tour_id ) {
				// No tour = bail!
				if ( $o_log ) {
					$o_log->logWarn( _t( '[tourStopSplitterRefinery] Could not find tour %1 to relate stop stops to.',
						$vs_tour_stop ) );
				}
			...
			if ( $va_parents = $pa_item['settings']['tourStopSplitter_parents'] ) {
				$va_val['parent_id']
					= $va_val['_parent_id'] = caProcessRefineryParents( 'tourStopSplitterRefinery', 'ca_tour_stops',
					$va_parents, $pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'log' => $o_log ) );
			}

			// Set attributes
			if ( is_array(
				$va_attr_vals = caProcessRefineryAttributes( $pa_item['settings']['tourStopSplitter_attributes'],
