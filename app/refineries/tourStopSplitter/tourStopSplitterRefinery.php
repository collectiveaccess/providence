        ...
        foreach ($va_tour_stops as $vn_i => $vs_tour_stop) {
            ...
            $va_val = array(
                'preferred_labels' => array(
                    'name_singular' => $vs_tour_stop,
                    'name_plural' => $vs_tour_stop
                )
            );
            ...
            if ((!isset($va_val['_relationship_type']) || !$va_val['_relationship_type']) && $o_log) {
                $o_log->logWarn(
                    _t('[tourStopSplitterRefinery] No relationship type is set for tour stop %1', $vs_tour_stop)
                );
            }
            ...
            if ((!isset($va_val['_type']) || !$va_val['_type']) && $o_log) {
                $o_log->logWarn(
                    _t('[tourStopSplitterRefinery] No collection type is set for tour stop %1', $vs_tour_stop)
                );
            }
            ...
            if (!$vn_tour_id) {
                ...
                if ($o_log) {
                    $o_log->logWarn(
                        _t('[tourStopSplitterRefinery] Could not find tour %1 to relate stop stops to.', $vs_tour_stop)
                    );
                }
                return array();
            }
            ...
            if (is_array(
                $va_attr_vals = caProcessRefineryAttributes(
                    $pa_item['settings']['tourStopSplitter_attributes'],
                    $pa_source_data,
                    $pa_item,
                    $vn_c,
...
BaseRefinery::$s_refinery_settings['tourStopSplitter'] = array(
    ...
    'tourStopSplitter_tourStopType' => array(
        ...
        'description' => _t(
            'Accepts a constant list item idno from the list tour_stop_types or a reference to the location in the data source where the type can be found.'
        )
    ),
    'tourStopSplitter_attributes' => array(
        ...
        'label' => _t('Attributes'),
        'description' => _t(
            'Sets or maps metadata for the tour stop record by referencing the metadataElement code and the location in the data source where the data values can be found.'
        )
    ),
    ...
    'tourStopSplitter_tourStopTypeDefault' => array(
        ...
        'description' => _t(
            'Sets the default tour stop type that will be used if none are defined or if the data source values do not match any values in the CollectiveAccess list tour_stop_types'
        )
    ),
    'tourStopSplitter_interstitial' => array(
        ...
        'description' => _t(
            'Sets or maps metadata for the interstitial tour stop <em>relationship</em> record by referencing the metadataElement code and the location in the data source where the data values can be found.'
        )
    ),
