        ...
        switch ($vs_mode = $pa_item['settings']['dateJoiner_mode']) {
            ...
            case 'range':
                ...
                if ($vs_date_start = BaseRefinery::parsePlaceholder(
                    ...
                    if ($pa_item['settings']['dateJoiner_ignorePresentDayStartDates'] && (caDateToUnixTimestamp(
                                $vs_date_start
                            ) === caDateToUnixTimestamp(_t('today')))) {
                        $vs_date_start = '';
                    }
                    if (
                ...
                if ($vs_date_end = BaseRefinery::parsePlaceholder(
                    ...
                    if ($pa_item['settings']['dateJoiner_ignorePresentDayEndDates'] && (caDateToUnixTimestamp(
                                $vs_date_end
                            ) === caDateToUnixTimestamp(_t('today')))) {
            ...
            case 'multiColumnDate':
                ...
                if (sizeof($va_date)) {                            // TODO: this is assuming US-style dates for now
                    ...
                    if ($o_log) {
                        $o_log->logWarn(
                            _t(
                                '[dateJoinerRefinery] Could not find multiColumnDate values. Is your configuration correct?'
                            )
                        );
                    }
                }
            ...
            case 'multiColumnRange':
                ...
                if (sizeof($va_date)) {
                    ...
                    if ($o_log) {
                        $o_log->logWarn(
                            _t(
                                '[dateJoinerRefinery] Could not find multiColumnRange value for start date. Is your configuration correct?'
                            )
                        );
                    }
                }
                ...
                if (sizeof($va_date)) {
                    ...
                    if ($o_log) {
                        $o_log->logWarn(
                            _t(
                                '[dateJoinerRefinery] Could not find multiColumnRange value for end date. Is your configuration correct?'
                            )
                        );
                    }
                }
                ...
                if (sizeof($va_dates) > 0) {
                    ...
                    if ($o_log) {
                        $o_log->logWarn(
                            _t(
                                '[dateJoinerRefinery] Could not find multiColumnRange values. Is your configuration correct?'
                            )
                        );
                    }
                }
...
BaseRefinery::$s_refinery_settings['dateJoiner'] = array(
    ...
    'dateJoiner_ignorePresentDayStartDates' => array(
        ...
        'description' => _t(
            'Excel will sometimes return the present date for blank dates. Set this option to treat them as blanks. (For Two-column range).'
        )
    ),
