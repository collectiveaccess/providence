...
class dateJoinerRefinery extends BaseRefinery {
	...
	public function __construct() {
		...
		$this->ops_description
		                 = _t( 'Joins data with partial date values into a single valid date expression for import.' );
	...
	public function refine( &$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null ) {
		...
		switch ( $vs_mode = $pa_item['settings']['dateJoiner_mode'] ) {
			...
			case 'range':
				...
				if ( $vs_date_expression
				     && ( $vs_exp = BaseRefinery::parsePlaceholder( $vs_date_expression, $pa_source_data, $pa_item,
						$vn_c,
					...
					if ( $o_tep->parse( $vs_exp ) ) {
						...
						if ( $o_log ) {
							$o_log->logWarn( _t( '[dateJoinerRefinery] Could not parse date expression %1 assembled from range',
								$vs_exp ) );
						}
					}
				}
				...
				$va_date = array();
				if ( $vs_date_start = BaseRefinery::parsePlaceholder( $vs_date_start, $pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) )
				) {
					if ( $pa_item['settings']['dateJoiner_ignorePresentDayStartDates']
					     && ( caDateToUnixTimestamp( $vs_date_start ) === caDateToUnixTimestamp( _t( 'today' ) ) )
					) {
						$vs_date_start = '';
					}
					if (
						( ! ( $vs_skip_start_exp = $pa_item['settings']['dateJoiner_skipStartIfExpression'] )
						  ||
						  ! ExpressionParser::evaluate( $vs_skip_start_exp, array_merge( $pa_source_data, array(
							  'start'      => $vs_date_start,
							  'end'        => $vs_date_end,
							  'expression' => $ps_expression
						  ) ) ) )
						&&
						! ( $pa_item['settings']['dateJoiner_skipStartIfEmpty']
						    && ( strlen( trim( $vs_date_start ) ) === 0 ) )
					) {
						$va_date[] = $vs_date_start;
					} elseif ( $vs_skip_start_replacement
						= $pa_item['settings']['dateJoiner_skipStartIfExpressionReplacementValue']
					) {
						$va_date[] = $vs_skip_start_replacement;
					}
				}
				if ( $vs_date_end = BaseRefinery::parsePlaceholder( $vs_date_end, $pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) )
				) {
					if ( $pa_item['settings']['dateJoiner_ignorePresentDayEndDates']
					     && ( caDateToUnixTimestamp( $vs_date_end ) === caDateToUnixTimestamp( _t( 'today' ) ) )
					) {
					...
					if (
						( ! ( $vs_skip_end_exp = $pa_item['settings']['dateJoiner_skipEndIfExpression'] )
						  ||
						  ! ExpressionParser::evaluate( $vs_skip_end_exp, array_merge( $pa_source_data, array(
							  'start'      => $vs_date_start,
							  'end'        => $vs_date_end,
							  'expression' => $ps_expression
						  ) ) ) )
						&&
						! ( $pa_item['settings']['dateJoiner_skipEndIfEmpty']
						    && ( strlen( trim( $vs_date_end ) ) === 0 ) )
					) {
						$va_date[] = $vs_date_end;
					} elseif ( $vs_skip_end_replacement
			...
			case 'multiColumnDate':
				...
				$va_date = array();
				if ( $vs_date_month = trim( BaseRefinery::parsePlaceholder( $pa_item['settings']['dateJoiner_month'],
					$pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) ) )
				) {
				...
				if ( $vs_date_day = trim( BaseRefinery::parsePlaceholder( $pa_item['settings']['dateJoiner_day'],
					$pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) ) )
				) {
					$va_date[] = $vs_date_day;
				}
				if ( $vs_date_year = trim( BaseRefinery::parsePlaceholder( $pa_item['settings']['dateJoiner_year'],
					$pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) ) )
				) {
				...
				if ( sizeof( $va_date ) ) {                            // TODO: this is assuming US-style dates for now
					if ( $o_tep->parse( join( "/", $va_date ) ) ) {
						...
						if ( $o_log ) {
							$o_log->logWarn( _t( '[dateJoinerRefinery] Could not parse date expression %1 assembled from multiColumnDate',
								join( "/", $va_date ) ) );
						}
					}
				} else {
			...
			case 'multiColumnRange':
				...
				$va_date = array();
				if ( $vs_date_month
					= trim( BaseRefinery::parsePlaceholder( $pa_item['settings']['dateJoiner_startMonth'],
					$pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) ) )
				) {
					if ( ( $vn_m = array_search( $vs_date_month, $va_month_list ) ) !== false ) {
				...
				if ( $vs_date_day = trim( BaseRefinery::parsePlaceholder( $pa_item['settings']['dateJoiner_startDay'],
					$pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) ) )
				) {
					$va_date[] = $vs_date_day;
				}
				if ( $vs_date_year = trim( BaseRefinery::parsePlaceholder( $pa_item['settings']['dateJoiner_startYear'],
					$pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) ) )
				) {
					$va_date[] = $vs_date_year;
				...
				if ( sizeof( $va_date ) ) {
					if ( $o_tep->parse( join( "/", $va_date ) ) ) {    // TODO: this is assuming US-style dates for now
						...
						if ( $o_log ) {
							$o_log->logWarn( _t( '[dateJoinerRefinery] Could not parse date expression %1 assembled from multiColumnRange',
								join( "/", $va_date ) ) );
						}
					}
				} else {
				...
				// Process end date
				$va_date = array();
				if ( $vs_date_month = trim( BaseRefinery::parsePlaceholder( $pa_item['settings']['dateJoiner_endMonth'],
					$pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) ) )
				) {
					if ( ( $vn_m = array_search( $vs_date_month, $va_month_list ) ) !== false ) {
				...
				if ( $vs_date_day = trim( BaseRefinery::parsePlaceholder( $pa_item['settings']['dateJoiner_endDay'],
					$pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) ) )
				) {
					$va_date[] = $vs_date_day;
				}
				if ( $vs_date_year = trim( BaseRefinery::parsePlaceholder( $pa_item['settings']['dateJoiner_endYear'],
					$pa_source_data, $pa_item, $vn_c,
					array( 'reader' => caGetOption( 'reader', $pa_options, null ), 'returnAsString' => true ) ) )
				) {
					$va_date[] = $vs_date_year;
