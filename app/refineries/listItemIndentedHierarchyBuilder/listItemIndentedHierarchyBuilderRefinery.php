...
class listItemIndentedHierarchyBuilderRefinery extends BaseRefinery
{
    ...
    public function __construct()
    {
        ...
        $this->ops_description = _t(
            'Imports spreadsheets with hierarchies expressed indented values spread across several columns as hierarchical lists.'
        );
    ...
    public function refine(&$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null)
    {
        ...
        if ($t_mapping) {
            if ($t_mapping->get('table_num') != Datamodel::getTableNum('ca_list_items')) {
                if ($o_log) {
                    $o_log->logError(
                        _t("listItemIndentedHierarchyBuilder refinery may only be used in imports to ca_list_items")
                    );
                }
                return null;
            }
        ...
        if (!is_array($va_levels = $pa_item['settings']['listItemIndentedHierarchyBuilder_levels'])) {
            if ($o_log) {
                $o_log->logError(
                    _t(
                        "listItemIndentedHierarchyBuilder requires levels option be set to a list of data source placeholders"
                    )
                );
            }
            return null;
        ...
        if (!$t_list->load(array('list_code' => $vs_list_code))) {
            ...
            if ($t_list->numErrors()) {
                if ($o_log) {
                    $o_log->logError(
                        _t(
                            "listItemIndentedHierarchyBuilder could not create list %1: %2",
                            $vs_list_code,
            ...
            if ($t_list->numErrors()) {
                if ($o_log) {
                    $o_log->logError(
                        _t(
                            "listItemIndentedHierarchyBuilder could not create list label %1: %2",
                            $vs_list_code,
                            join("; ", $t_list->getErrors())
                        )
                    );
                    ...
                    if ($vn_item_id = DataMigrationUtils::getListItemID(
                        ...
                        array(
                            ...
                            'preferred_labels' => array(
                                'name_singular' => $vs_level_value,
                                'name_plural' => $vs_level_value
                            )
                        ),
                        array(
                            'matchOnIdno' => true,
                            'log' => $o_log,
                            'transaction' => caGetOption(
                                'transaction',
                                $pa_options,
                                null
                            ),
        ...
        listItemIndentedHierarchyBuilderRefinery::$opa_level_values = array_slice(
            $va_level_values,
            0,
            $vn_max_level + 1
        );
        listItemIndentedHierarchyBuilderRefinery::$opa_level_value_ids = array_slice(
            $va_level_value_ids,
            0,
            $vn_max_level + 1
        );
