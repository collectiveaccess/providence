    ...
    public function refine(&$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null)
    {
        ...
        if ($t_mapping) {
            if ($t_mapping->get('table_num') != Datamodel::getTableNum('ca_collections')) {
                if ($o_log) {
                    $o_log->logError(
                        _t("collectionHierarchyBuilder refinery may only be used in imports to ca_collections")
                    );
                }
                return null;
            }
        ...
        if ($va_parents = $pa_item['settings']['collectionHierarchyBuilder_parents']) {
            ...
            $vn_parent_id = caProcessRefineryParents(
...
BaseRefinery::$s_refinery_settings['collectionHierarchyBuilder'] = array(
    'collectionHierarchyBuilder_matchOn' => array(
        ...
        'label' => _t('Match on'),
        'description' => _t(
            'List indicating sequence of checks for an existing record; values of array can be "preferred_labels" (or "label"), "nonpreferred_labels", "idno" or a metadata element code. Ex. array("idno", "label") will first try to match on idno and then label if the first match fails'
        )
    ),
