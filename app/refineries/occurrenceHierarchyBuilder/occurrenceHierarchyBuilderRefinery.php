 ...
 */
	...
	class occurrenceHierarchyBuilderRefinery extends BaseRefinery {
		...
		public function refine(&$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options=null) {
			...
			if ($t_mapping) {
				$o_dm = Datamodel::load();
				if ($t_mapping->get('table_num') != $o_dm->getTableNum('ca_occurrences')) { 
					if ($o_log) {
			...
			if ($va_parents = $pa_item['settings']['occurrenceHierarchyBuilder_parents']) {
				$vn_parent_id = caProcessRefineryParents('occurrenceHierarchyBuilderRefinery', 'ca_occurrences', $va_parents, $pa_source_data, $pa_item, null, null, array('reader' => caGetOption('reader', $pa_options, null), 'log' => $o_log));
			}
