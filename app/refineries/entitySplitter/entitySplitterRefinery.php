...
class entitySplitterRefinery extends BaseRefinery
{
    ...
    public function __construct()
    {
        ...
        $this->ops_description = _t(
            'Provides several entity-related import functions: splitting of entity names into component names (forename, surname, Etc.), splitting of many names in a string into separate names, and merging entity data with entity names (life dates, nationality, Etc.).'
        );
...
BaseRefinery::$s_refinery_settings['entitySplitter'] = array(
    ...
    'entitySplitter_matchOn' => array(
        ...
        'label' => _t('Match on'),
        'description' => _t(
            'List indicating sequence of checks for an existing record; values of array can be "label", "idno" or "displayname". Ex. array("idno", "label", "displayname") will first try to match on idno and label [forename, surname] if the first match fails, then finally the displayname field'
        )
    ),
    ...
    'entitySplitter_displaynameFormat' => array(
        ...
        'description' => _t(
            'Format to force entity display name to after parsing. You can pass a template using name element prefixed with carets (^) or one of the following: surnameCommaForename, forenameCommaSurname, forenameSurname, original.'
        )
    ),
    ...
    'entitySplitter_doNotParse' => array(
        ...
        'description' => _t(
            'Take the entity name as is from the data source and insert it without intervention in the surname and display name fields. This is often useful for organization names, especially when using the entity class "org" setting.'
        )
    ),
