...
class EADCollectionHierarchyBuilderRefinery extends BaseRefinery
{
     ...
     *
     */
    static public $mapping_options = ['skipGroupIfEmpty'];
    ...
    public function refine(&$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null)
    {
        ...
        if ($t_mapping) {
            if ($t_mapping->get('table_num') != Datamodel::getTableNum('ca_collections')) {
                if ($logger) {
                    $logger->logError(
                        _t("EADCollectionHierarchyBuilder refinery may only be used in imports to ca_collections")
                    );
                }
                return null;
            }
        ...
        if ($pa_group['destination'] !== 'ca_collections._children') {
            if ($logger) {
                $logger->logError(
                    _t(
                        "Target CollectiveAccess element EADCollectionHierarchyBuilder must be ca_collections._collection"
                    )
                );
            }
            return null;
        ...
        if ($reader && ($subcollection_xml = $reader->get(
                '/archdesc/dsc'
            )) && isset($pa_item['settings']['EADCollectionHierarchyBuilder_levels']) && is_array(
                $level_mappings = $pa_item['settings']['EADCollectionHierarchyBuilder_levels']
            )) {
            $subcollections = new SimpleXMLElement(
                "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><subcollections>{$subcollection_xml}</subcollections>"
            );

            $l = 1;
            foreach ($subcollections as $tag => $data) {
                $children[] = $this->_mapLevels(
                    $data,
                    $l,
                    $level_mappings,
                    [
    ...
    {
        if (isset($level_mappings[(string)$data["level"]]) && is_array(
                $mapping = $level_mappings[(string)$data["level"]]
            )) {
            $tag = 'c' . sprintf("%02d", $l);
            ...
            foreach (
                array_merge(
                    $mapping['attributes'],
                    ['preferred_labels' => ['name' => $mapping['preferredLabel']]]
                ) as $f => $m
            ) {
                ...
                foreach ($m as $index => $t) {
                    ...
                    if (sizeof(
                            array_intersect(
                                array_merge(
                                    ['source', 'delimiter', 'useAsSingleValue', 'skipGroupIfEmpty'],
                        ...
                        foreach ($t as $sf => $st) {
                            ...
                            if (is_array($xpaths[$sf]) && (sizeof($xpaths[$sf]) > 0)) {
                                ...
                                for ($z = 0; $z < sizeof($xpaths); $z++) {
                                    if (!isset($mapped_values[$f][$mapping_index + $z]) || !is_array(
                                            $mapped_values[$f][$mapping_index + $z]
                        ...
                        if (is_array($mapped_values[$f]) && caIsIndexedArray($mapped_values[$f]) && (sizeof(
                                    $mapped_values[$f]
                                ) > 1) && caGetOption('useAsSingleValue', $t, false)) {
            ...
            if (isset($mapping['relationships']) && is_array($mapping['relationships'])) {
                ...
                foreach ($mapping['relationships'] as $rel) {
                    ...
                    $labels = BaseRefinery::parsePlaceholder(
                        $rel['preferredLabel'],
                        [],
                        [],
                        null,
                    ...
                    $idnos = BaseRefinery::parsePlaceholder(
                        $rel['attributes']['idno'],
                        [],
                        [],
                        null,
                    ...
                    $rel_data = caProcessRefineryRelated(
                        $this,
                        $rel['relatedTable'],
                        $items,
                        $refinery_data['source_data'],
                        $refinery_data['item'],
                        0,
