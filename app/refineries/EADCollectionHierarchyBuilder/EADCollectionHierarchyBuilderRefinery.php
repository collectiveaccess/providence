...
class EADCollectionHierarchyBuilderRefinery extends BaseRefinery {
	...
	public function refine( &$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null ) {
		...
		if ( $reader && ( $subcollection_xml = $reader->get( '/archdesc/dsc' ) )
		     && isset( $pa_item['settings']['EADCollectionHierarchyBuilder_levels'] )
		     && is_array( $level_mappings = $pa_item['settings']['EADCollectionHierarchyBuilder_levels'] )
		) {
			$subcollections
				= new SimpleXMLElement( "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?><subcollections>{$subcollection_xml}</subcollections>" );

			$l = 1;
			foreach ( $subcollections as $tag => $data ) {
				$children[] = $this->_mapLevels( $data, $l, $level_mappings, [
					'subcollection_xml' => $subcollection_xml,
					'group'             => $pa_group,
					'item'              => $pa_item,
					'source_data'       => $pa_source_data,
					'options'           => $pa_options
				] );
			}
		}
	 ...
	 * @param array            $level_mappings Mappings for each collection level. Keys are level attribute values;
	 *                                         values are dictionaries with per-field mappings.
	 *
	 * @return array Mapped values in a format ready for import
	 */
	private function _mapLevels( $data, &$l, $level_mappings, $refinery_data ) {
		if ( isset( $level_mappings[ (string) $data["level"] ] )
		     && is_array( $mapping = $level_mappings[ (string) $data["level"] ] )
		) {
			$tag = 'c' . sprintf( "%02d", $l );
			...
			foreach (
				array_merge( $mapping['attributes'],
					[ 'preferred_labels' => [ 'name' => $mapping['preferredLabel'] ] ] ) as $f => $m
			) {
				...
				foreach ( $m as $index => $t ) {
					...
					if ( sizeof( array_intersect( array_merge( [
							'source',
							'delimiter',
							'useAsSingleValue',
							'skipGroupIfEmpty'
						], self::$mapping_options ), $mapping_keys ) ) !== sizeof( $mapping_keys )
					) {
						// it's a container
						$mapped_values[ $f ][] = [];
							...
							if ( is_array( $xpaths[ $sf ] ) && ( sizeof( $xpaths[ $sf ] ) > 0 ) ) {
								foreach ( $xpaths[ $sf ] as $i => $p ) {    // replace tags with values
									foreach ( $values[ $i ] as $vindex => $v ) {
										...
										if ( ! trim( $tv ) && isset( $st['skipGroupIfEmpty'] )
										     && $st['skipGroupIfEmpty']
										) {
								...
								for ( $z = 0; $z < sizeof( $xpaths ); $z ++ ) {
									if ( ! isset( $mapped_values[ $f ][ $mapping_index + $z ] )
									     || ! is_array( $mapped_values[ $f ][ $mapping_index + $z ] )
									     || ! sizeof( $mapped_values[ $f ][ $mapping_index + $z ] )
									) {
										continue;
									}
						...
						if ( is_array( $mapped_values[ $f ] ) && caIsIndexedArray( $mapped_values[ $f ] )
						     && ( sizeof( $mapped_values[ $f ] ) > 1 )
						     && caGetOption( 'useAsSingleValue', $t, false )
						) {
			...
			if ( isset( $mapping['relationships'] ) && is_array( $mapping['relationships'] ) ) {
				...
				foreach ( $mapping['relationships'] as $rel ) {
					...
					$dxml   = preg_replace( "!</" . $data->getName() . "[^>]*>!", "</ead>",
						preg_replace( "!<" . $data->getName() . "[^>]*>!",
							"<ead audience=\"internal\" xmlns=\"http://ead3.archivists.org/schema/\">",
							$data->asXML() ) );
					$reader->read( null,
						[ 'fromString' => "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>" . $dxml ] );
					$reader->nextRow();

					$labels = BaseRefinery::parsePlaceholder( $rel['preferredLabel'], [], [], null,
						array( 'reader' => $reader, 'returnAsString' => false ) );
					$idnos  = BaseRefinery::parsePlaceholder( $rel['attributes']['idno'], [], [], null,
						array( 'reader' => $reader, 'returnAsString' => false ) );
					...
					$rel_data = caProcessRefineryRelated( $this, $rel['relatedTable'], $items,
						$refinery_data['source_data'], $refinery_data['item'], 0,
						array_merge( $refinery_data['options'], [ 'reader' => $reader ] ) );
