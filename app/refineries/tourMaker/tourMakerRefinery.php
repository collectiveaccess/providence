...
class tourMakerRefinery extends BaseRefinery
{
    ...
    public function refine(&$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null)
    {
        ...
        foreach ($va_tours as $pm_value) {
            ...
            if ((is_null(
                        $vs_type
                    ) || !$vs_type) && ($vs_type_opt = $pa_item['settings']['tourMaker_tourTypeDefault'])) {
                $vs_type = BaseRefinery::parsePlaceholder(
                    $vs_type_opt,
                    $pa_source_data,
                    $pa_item,
            ...
            if (!$t_tour->insert()) {
                if ($o_log) {
                    $o_log->logError(
                        _t(
                            '[tourMakerRefinery] Could not create tour %1: %2',
                            $pm_value,
                            join("; ", $t_tour->getErrors())
                        )
                    );
                }
                return null;
            ...
            if ($t_tour->numErrors() > 0) {
                if ($o_log) {
                    $o_log->logError(
                        _t(
                            '[tourMakerRefinery] Could not add label for tour %1: %2',
                            $pm_value,
                            join("; ", $t_tour->getErrors())
                        )
                    );
            ...
            if (is_array($pa_item['settings']['tourMaker_attributes'])) {
                foreach ($pa_item['settings']['tourMaker_attributes'] as $vs_element => $va_attr) {
                    if (!is_array($va_attr)) {
                        $va_attr = array(
                            $vs_element => BaseRefinery::parsePlaceholder(
                                $va_attr,
                                $pa_source_data,
                                $pa_item,
                                $vn_c,
                        ...
                        foreach ($va_attrs as $vs_k => $vs_v) {
                            $va_attr[$vs_k] = BaseRefinery::parsePlaceholder(
                                $vs_v,
                                $pa_source_data,
                                $pa_item,
                                $vn_c,
                ...
                if (!$t_tour->update()) {
                    if ($o_log) {
                        $o_log->logError(
                            _t(
                                '[tourMakerRefinery] Could not save data for tour %1: %2',
                                $pm_value,
                                join("; ", $t_tour->getErrors())
                            )
                        );
...
BaseRefinery::$s_refinery_settings['tourMaker'] = array(
    'tourMaker_tourType' => array(
        ...
        'description' => _t(
            'Accepts a constant list item idno from the list tour_types or a reference to the location in the data source where the type can be found.'
        )
    ),
    'tourMaker_attributes' => array(
        ...
        'label' => _t('Attributes'),
        'description' => _t(
            'Sets or maps metadata for the tour record by referencing the metadataElement code or the location in the data source where the data values can be found.'
        )
    ),
    'tourMaker_tourTypeDefault' => array(
        ...
        'description' => _t(
            'Sets the default tour type that will be used if none are defined or if the data source values do not match any values in the CollectiveAccess list tour_types.'
        )
    )
);
?>