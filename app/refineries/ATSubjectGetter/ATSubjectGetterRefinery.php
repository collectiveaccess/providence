...
class ATSubjectGetterRefinery extends BaseRefinery {
	...
	public function refine( &$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null ) {
		...
		while ( $qr_rel_names->nextRow() ) {
			...
			if ( is_array( $va_skip_values = $pa_item['settings']['ATSubjectGetter_skipIfValue'] )
			...
			// Set item_type
			if ( $va_item_type ) {
				if ( ! ( $va_val['_type'] = BaseRefinery::parsePlaceholder( $va_item_type, $pa_source_data, $pa_item,
					$vn_c ) )
				) {
			...
			if ( isset( $va_target['list'] ) ) {
				...
				if ( ! preg_match( "!^([A-Za-z0-9 ]+)!", $va_target['relationshipType'], $va_matches )
