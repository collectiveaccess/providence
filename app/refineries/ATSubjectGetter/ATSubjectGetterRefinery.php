...
class ATSubjectGetterRefinery extends BaseRefinery
{
    ...
    public function refine(&$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null)
    {
        ...
        while ($qr_rel_names->nextRow()) {
            ...
            if (is_array($va_skip_values = $pa_item['settings']['ATSubjectGetter_skipIfValue']) && in_array(
                    $vs_item,
                    $va_skip_values
                )) {
                continue;
            ...
            if (isset($va_target['list'])) {
                ...
                if (!preg_match(
                        "!^([A-Za-z0-9 ]+)!",
                        $va_target['relationshipType'],
                        $va_matches
                    ) || !($va_val['_relationship_type'] = $va_matches[1])) {
                ...
                $pa_destination_data[$vs_subject_table][] = array(
                    $va_target['element'] => array(
                        'locale_id' => $pa_options['locale_id'],
                        $va_target['element'] => $vs_subject
                    )
                );
            }
...
BaseRefinery::$s_refinery_settings['ATSubjectGetter'] = array(
    'ATSubjectGetter_key' => array(
        ...
        'label' => _t('Key'),
        'description' => _t(
            'Foreign key in ArchDescriptionSubjects to select on (eg. ResourceID, resourceComponentId, accessionId, digitalObjectId.'
        )
    ),
    ...
    'ATSubjectGetter_targets' => array(
        ...
        'label' => _t('Targets'),
        'description' => _t(
            'List of CA metadata elements or vocabulary lists to target for imported subjects. The target is determined by the AT subjectSource.'
        )
    ),
    'ATSubjectGetter_itemTypes' => array(
        ...
        'label' => _t('Item types'),
        'description' => _t(
            'Map equating three AT name types (personal, family and corporate) with CA list_item_types. Keys are AT types, values are CA types.'
        )
    ),
    'ATSubjectGetter_attributes' => array(
        ...
        'label' => _t('Attributes'),
        'description' => _t(
            'Sets or maps metadata for the item record by referencing the metadataElement code and the location in the data source where the data values can be found.'
        )
    ),
    ...
    'ATSubjectGetter_itemTypeDefault' => array(
        ...
        'description' => _t(
            'Sets the default item type that will be used if none are defined or if the data source values do not match any values in the CollectiveAccess list list_item_types.'
        )
    ),
