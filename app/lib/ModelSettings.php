			...
			if ( isset( $va_properties['takesLocale'] ) && $va_properties['takesLocale'] ) {
				...
				if (
					...
					|| ( isset( $va_properties['showVocabularies'] ) && $va_properties['showVocabularies']
					     && ( $va_properties['height'] > 1 ) )
					|| ( isset( $va_properties['multiple'] ) && $va_properties['multiple']
					     && ( $va_properties['height'] > 1 ) )
				) {
					...
					switch ( $va_properties['formatType'] ) {
						case FT_BIT:
							...
							$vs_val = $po_request->getParameter( "{$vs_placement_code}{$vs_id_prefix}{$vs_setting}",
								pString );
							if ( $vs_val == '' ) {
								continue( 2 );
							}
						...
						default:
							$va_values[ $vs_setting ]
								= $po_request->getParameter( "{$vs_placement_code}{$vs_id_prefix}{$vs_setting}",
								pString );
							break;
					}
