...
class HTTPMultiClientResult {
	 ...
	 *
	 * @return array A list of valid request names for responses received thus far. Request names can be used with
	 *               HTTPMultiClientResult::getResponse() to fetch an individual response.
	 */
	 ...
	 *
	 * @return bool True if requests are still running after the update, false if all requests are complete (and
	 *              therefore, calling updateStatus() again would be of little use.)
	 */
		...
		while ( ( $va_message = curl_multi_info_read( $this->opo_multi_handle, $vb_running ) ) ) {
			if ( $va_message['msg'] == CURLMSG_DONE ) {
				if ( $va_message['result'] == CURLE_OK ) {
					$this->opa_responses[ $this->opa_requests_by_handle[ (string) $va_message['handle'] ]['name'] ]
						= curl_multi_getcontent( $va_message['handle'] );
				} else {
					$this->opa_responses[ $this->opa_requests_by_handle[ (string) $va_message['handle'] ]['name'] ]
						= "ERROR";
				}
