...
class BaseFindEngine extends BaseObject
{
    ...
    public function loadListIntoTemporaryResultTable($pa_hits, $ps_key, $pa_options = null)
    {
        ...
        if ((($g_mysql_has_file_priv === true) && (sizeof($pa_hits) > 500))) {
            ...
            if (is_array($va_sortable_values)) {
                ...
                $this->opo_db->query(
                    "LOAD DATA INFILE '{$this->ops_tmp_file_path}' INTO TABLE {$this->ops_tmp_table_name} (row_id)"
                );
            }
        } else {
    ...
    public function sortHits(&$hits, $table, $sort_list, $sort_directions = 'asc', $options = null)
    {
        ...
        $embedded_sort_directions = array_map(
            function ($v) {
                $t = explode(':', $v);
        ...
        foreach ($sort_fields as $i => $sort_field) {
            ...
            $sort_tmp = preg_split(
                '![/\|]+!',
                $sort_field
            );        // isolate relationship type (and/or item type) if present
            ...
            $set_id = null;
            if (!($set_id = preg_match(
                "!^ca_sets.set_id:([\d]+)$!",
                $sort_field,
                $va_matches
            ) ? $va_matches[1] : null)) {    // sort on set_id (Eg. ca_sets.set_id:23)
            ...
            if ($set_id) {    // sort by set ranks
                ...
                if (!$t_table->hasField($sort_field)) {
                    if ($sort_keys = $this->_getSortKeysForElement(
                        $sort_subfield ? $sort_subfield : $sort_field,
                        $table_num,
                        $hits
                    )) {
                ...
                if (sizeof($path) > 1) {
                    ...
                    foreach (array_keys($path) as $join_table) {
                        ...
                        if ($t_join->isRelationship() && $sort_rel_type) {
                            if (is_array(
                                    $va_rel_types = caMakeRelationshipTypeIDList(
                                        $join_table,
                                ...
                                $t_join,
                                "getTypeFieldName"
                            ) && ($vs_type_fld_name = $t_join->getTypeFieldName())) {
                            if (($join_table !== $table) && is_array(
                    ...
                    if ($t_sort_field_table->hasField($sort_field)) { // sorting on intrinsic in related table
                        ...
                        foreach ($acc as $id => $rel_ids) {
                            ...
                            foreach (array_keys($rel_ids) as $rel_id) {
                                $sort_keys[$id] = array_filter(
                                    array_merge($sort_keys[$id], array_keys($keys[$rel_id])),
                                    "strlen"
                                );
                            }
                        }
    ...
    private function _doSort(&$hits, $sortable_values, $sort_directions, $options = null)
    {
        ...
        if (!is_array($sort_directions)) {
            $sort_directions = array_pad(
                $sort_directions,
                sizeof($sortable_values),
        ...
        if (sizeof($hits) < $max_hits_for_in_memory_sort) {
            ...
            //
            $sort_tmp_table = $this->loadListIntoTemporaryResultTable(
                $hits,
                caGenerateGUID(),
        ...
        switch ($vn_datatype = (int)$t_element->get('datatype')) {
            ...
            case __CA_ATTRIBUTE_VALUE_OBJECTLOTS__:
                ...
                $vs_sql = "
							...
							INNER JOIN " . $t_auth_instance->getLabelTableName(
                    ) . " AS lil ON lil." . $t_auth_instance->primaryKey() . " = attr_vals.item_id
							WHERE
