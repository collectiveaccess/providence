...
class BaseFindEngine extends BaseObject
{
    ...
    public function sortHits(&$pa_hits, $ps_table, $pm_field, $ps_direction = 'asc', $pa_options = null)
    {
        ...
        $vn_set_id = (is_string($pm_field) && preg_match(
                "!^ca_sets.set_id:([\d]+)$!",
                $pm_field,
                $va_matches
            )) ? $va_matches[1] : null;
        ...
        foreach ($va_bundles as $vs_bundle) {
            ...
            if (!($t_instance = Datamodel::getInstanceByTableName($vs_field_table, true))) {
                break;
            }
            ...
            if ($vn_set_id) {
                ...
                if (!$t_table->hasField($vs_field)) {
                    if ($va_sort_keys = $this->_getSortKeysForElement(
                        $vs_subfield ? $vs_subfield : $vs_field,
                        $vn_table_num,
                        $pa_hits
                    )) {
                ...
                if (sizeof($va_path) > 2) {
                    ...
                    foreach ($va_path as $vs_table => $va_info) {
                        ...
                        if ($t_instance->isRelationship() && $vs_rel_type) {
                            if (is_array(
                                    $va_rel_types = caMakeRelationshipTypeIDList($vs_table, explode(",", $vs_rel_type))
                                ) && sizeof($va_rel_types)) {
                            ...
                            if (($ps_table !== $vs_table) && is_array(
                                    $va_item_types = caMakeTypeIDList($vs_table, explode(",", $vs_item_type))
                                ) && sizeof($va_item_types)) {
                                $vs_item_type_sql = " AND {$vs_table}.{$vs_type_fld_name} IN (" . join(
                                        ",",
                                        $va_item_types
                                    ) . ")";
                            }
                    ...
                    if ($t_rel->hasField($vs_field)) { // intrinsic in related table
                        ...
                        $va_maps = $this->_mapRowIDsForPathLength2(
                            $vn_table_num,
                            $t_rel->tableNum(),
                            $pa_hits,
                            $pa_options
    ...
    private function _doSort(&$pa_hits, $pa_sortable_values, $ps_direction = 'asc', $pa_options = null)
    {
        ...
        if (sizeof($pa_hits) < 1000000) {
            ...
            foreach ($pa_hits as $vn_idx => $vn_hit) {
                ...
                $va_sort_buffer[$vs_key . str_pad(
                    $vn_c,
                    8,
                    '0',
                    STR_PAD_LEFT
                )] = $vb_return_index ? $vn_idx . '/' . $vn_hit : $vn_hit;
                $vn_c++;
            }
            ...
            if (!$o_conf->get('dont_use_natural_sort')) {
                $vn_sort_mode |= SORT_NATURAL;
            } else {
                $vn_sort_mode |= SORT_STRING;
            }

            ksort($va_sort_buffer, $vn_sort_mode);
            if ($ps_direction == 'desc') {
                $va_sort_buffer = array_reverse($va_sort_buffer);
            }
            ...
            //
            $vs_sort_tmp_table = $this->loadListIntoTemporaryResultTable(
                $pa_hits,
                caGenerateGUID(),
                array('sortableValues' => $pa_sortable_values)
            );
            $vs_sql = "
					SELECT row_id, idx
    ...
    {
        if (!($t_original_table = Datamodel::getInstance($pn_original_table_num, true))) {
            return false;
        }
        if (!($t_target_table = Datamodel::getInstance($pn_target_table, true))) {
            return false;
        }
