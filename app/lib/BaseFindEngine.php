...
class BaseFindEngine extends BaseObject {
	...
	/**
	 * @param Db $po_db A database client object to use rather than creating a new connection. [Default is to create a
	 ...
	 * Only one temporary table can exist at a time for a given instance. If you call
	 * loadListIntoTemporaryResultTable()
	 * on an instance for which a temporary table has already been loaded the previously created table will be
	 * discarded
	 * before the new one is created.
	 *
	 * @param array  $pa_hits Array of row_id values
	 * @param string $ps_key  Unique alphanumeric identifier for the temporary table. Should only contain letters,
	...
	public function loadListIntoTemporaryResultTable( $pa_hits, $ps_key, $pa_options = null ) {
		...
		if ( ( ( $g_mysql_has_file_priv === true ) && ( sizeof( $pa_hits ) > 500 ) ) ) {
			...
			if ( is_array( $va_sortable_values ) ) {
				$vs_sql
					= "LOAD DATA INFILE '{$this->ops_tmp_file_path}' INTO TABLE {$this->ops_tmp_table_name} FIELDS TERMINATED BY ',' (row_id, sort_key1, sort_key2, sort_key3)";
			...
			if ( is_array( $va_sortable_values ) ) {
				$vs_sql
					= "INSERT IGNORE INTO {$this->ops_tmp_table_name} (row_id, idx, sort_key1, sort_key2, sort_key3) VALUES ";
				foreach ( $pa_hits as $vn_index => $vn_hit ) {
					...
					$vs_sql .= "(" . (int) $vn_hit . "," . (int) $vn_index . ",'" . $vs_sort_key_1 . "','"
					           . $vs_sort_key_2 . "','" . $vs_sort_key_3 . "'),";
				}
			} else {
	...
	public function filterHitsByACL( $pa_hits, $pn_table_num, $pn_user_id, $pn_access = __CA_ACL_READONLY_ACCESS__ ) {
		...
		$qr_items = $this->opo_db->query( "
			...
			", (int) $pn_table_num, (int) $pn_access, $pa_hits, (int) $pn_table_num, (int) $pn_access,
			(int) $pn_user_id );
	 ...
	 * @param string $field_list An array or semicolon-delimited string of fully qualified bundle names (Eg.
	 *                           ca_objects.idno;ca_objects.due_date)
	 * @param string $directions An array or semicolon-delimited string of sort directions corresponding to sort
	 *                           criteria specified in $field_list. Values for direction may be either 'asc' (ascending
	 *                           order) or 'desc' (descending order). If not specified 'asc' is assumed.
	...
	public function sortHits( &$hits, $table, $sort_list, $sort_directions = 'asc', $options = null ) {
		...
		foreach ( $sort_fields as $i => $sort_field ) {
			...
			$sort_tmp       = preg_split( '![/\|]+!',
				$sort_field );        // isolate relationship type (and/or item type) if present
			...
			$set_id = null;
			if ( ! ( $set_id = preg_match( "!^ca_sets.set_id:([\d]+)$!", $sort_field, $va_matches ) ? $va_matches[1]
				: null )
			) {    // sort on set_id (Eg. ca_sets.set_id:23)
			...
			if ( $set_id ) {    // sort by set ranks
				...
				if ( ! $t_table->hasField( $sort_field ) ) {
					if ( $sort_keys = $this->_getSortKeysForElement( $sort_subfield ? $sort_subfield : $sort_field,
						$table_num, $hits )
					) {
				...
				if ( sizeof( $path ) > 1 ) {
					...
					foreach ( array_keys( $path ) as $join_table ) {
						...
						if ( $t_join->isRelationship() && $sort_rel_type ) {
							if ( is_array( $va_rel_types = caMakeRelationshipTypeIDList( $join_table,
									explode( ",", $sort_rel_type ) ) )
							     && sizeof( $va_rel_types )
							) {
								$sort_rel_type_sql = " AND {$join_table}.type_id IN (" . join( ",", $va_rel_types )
								                     . ")";
							}
						} elseif ( method_exists( $t_join, "getTypeFieldName" )
						           && ( $vs_type_fld_name
								= $t_join->getTypeFieldName() )
						) {
							if ( ( $join_table !== $table )
							     && is_array(
								     $va_item_types = caMakeTypeIDList( $join_table, explode( ",", $sort_item_type ) ) )
							     && sizeof( $va_item_types )
							) {
								$sort_item_type_sql = " AND {$join_table}.{$vs_type_fld_name} IN (" . join( ",",
										$va_item_types ) . ")";
							}
						}
						...
						if ( $last_table ) {
							...
							if ( $join_table == $rels['one_table'] ) {
								$joins[ $join_table ] = "INNER JOIN " . $rels['one_table'] . " ON " . $rels['one_table']
								                        . "." . $rels['one_table_field'] . " = " . $rels['many_table']
								                        . "." . $rels['many_table_field'] . $sort_rel_type_sql
								                        . $sort_item_type_sql . $deleted_sql;
							} else {
								$joins[ $join_table ] = "INNER JOIN " . $rels['many_table'] . " ON "
								                        . $rels['many_table'] . "." . $rels['many_table_field'] . " = "
								                        . $rels['one_table'] . "." . $rels['one_table_field']
								                        . $sort_rel_type_sql . $sort_item_type_sql . $deleted_sql;
							}
						}
					...
					if ( $t_sort_field_table->hasField( $sort_field ) ) { // sorting on intrinsic in related table
						$joins[ $sort_table ] = 'INNER JOIN ' . $sort_table . ' ON ' . $last_table . '.'
						                        . $rels[ $last_table ][ $sort_table ][0][0] . ' = ' . $sort_table . '.'
						                        . $rels[ $last_table ][ $sort_table ][0][1] . "\n";
						...
						while ( $qr_sort->nextRow() ) {
							...
							$acc[ $qr_sort->get( "{$table}.{$table_pk}" ) ][ $rel_id ]
							        = true;    // list of sortables per hit
							$rel_ids[]
							        = $rel_id;    // list of sortables
						}
						if ( ! sizeof( $rel_ids ) ) {
							return $hits;
						}
						...
						while ( $qr_keys->nextHit() ) {    // Loop on sortables
							...
							$sort_values = $qr_keys->get( "{$sort_table}.{$sort_field}",
								[ 'sortable' => true, 'returnAsArray' => true ] );
						...
						foreach ( $acc as $id => $rel_ids ) {
							...
							foreach ( array_keys( $rel_ids ) as $rel_id ) {
								$sort_keys[ $id ] = array_filter( array_merge( $sort_keys[ $id ],
									array_keys( $keys[ $rel_id ] ) ), "strlen" );
							}
						}
	...
	private function _doSort( &$hits, $sortable_values, $sort_directions, $options = null ) {
		...
		if ( ! is_array( $sort_directions ) ) {
			$sort_directions = array_pad( $sort_directions, sizeof( $sortable_values ),
				( strtolower( $sort_directions ) == 'desc' ) ? 'desc' : 'asc' );
		}
		...
		if ( sizeof( $hits ) < $max_hits_for_in_memory_sort ) {
			...
			//
			$sort_tmp_table = $this->loadListIntoTemporaryResultTable( $hits, caGenerateGUID(),
				array( 'sortableValues' => $sortable_values ) );
			$vs_sql         = "
					SELECT row_id, idx
					FROM {$sort_tmp_table}
		...
		if ( $datatype == 0 ) {
			...
			foreach ( $elements as $e ) {
				...
				foreach ( $keys as $id => $val ) {
					...
					$concatenated_keys[ $id ] = str_pad( $sv, 30, " ",
						is_numeric( $sv ) ? STR_PAD_LEFT : STR_PAD_RIGHT );
				}
			}
		...
		switch ( $vn_datatype = (int) $t_element->get( 'datatype' ) ) {
			...
			case __CA_ATTRIBUTE_VALUE_OBJECTLOTS__:
				...
				$vs_sql                = "
							...
							INNER JOIN " . $t_auth_instance->getLabelTableName() . " AS lil ON lil."
				                         . $t_auth_instance->primaryKey() . " = attr_vals.item_id
							WHERE
	...
	private function _mapRowIDsForPathLength2( $pn_original_table_num, $pn_target_table, $pa_hits, $pa_options = null
		...
		if ( is_array( $va_primary_ids ) && ( sizeof( $va_primary_ids ) > 0 ) ) {
			...
			if ( isset( $va_tmp[ $vs_resolve_links_using ][ $vs_target_table ][0][1] ) ) {
				$vs_primary_id_sql
					             = "AND {$vs_target_table}.{$va_tmp[$vs_resolve_links_using][$vs_target_table][0][1]} IN (?)";
