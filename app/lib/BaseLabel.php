...
class BaseLabel extends BaseModel
{
    ...
    public function update($pa_options = null)
    {
        ...
        unset(
            LabelableBaseModelWithAttributes::$s_label_cache[$this->getSubjectTableName()][$this->get(
                $this->getSubjectKey()
            )]
        );
    ...
    public function getSubjectTableInstance($pa_options = null)
    {
        if ($vs_subject_table_name = $this->getSubjectTableName()) {
            ...
            if (!caGetOption("dontLoadInstance", $pa_options, false) && ($vn_id = $this->get(
    ...
    {
        if (($ps_field == $this->getDisplayField()) && (is_array(
                $va_use_list = caGetOption('use_list', $pa_options, false)
            ...
            if ($vn_list_id > 0) {
                ...
                if ($ret_format == 'select') {
                    ...
                    $va_urls = caJSONLookupServiceUrl(
                        $po_request,
                        'ca_list_items',
