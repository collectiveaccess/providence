...
class BaseLabel extends BaseModel {
	...
	/**
	 * Returns list of secondary display fields. If not defined for the label (most don't have these) an empty array is
	 * returned.
	 */
	 ...
	 *                          dontLoadInstance = If set returned instance is not preloaded with subject. Default is
	 *                          false - load subject data
	 *
	...
	public function getSubjectTableInstance( $pa_options = null ) {
		if ( $vs_subject_table_name = $this->getSubjectTableName() ) {
			...
			if ( ! caGetOption( "dontLoadInstance", $pa_options, false )
			     && ( $vn_id
					= $this->get( $t_subject->primaryKey() ) )
			) {
	...
	public function htmlFormElement( $ps_field, $ps_format = null, $pa_options = null ) {
		if ( ( $ps_field == $this->getDisplayField() )
		     && ( is_array( $va_use_list = caGetOption( 'use_list', $pa_options, false ) ) )
			...
			if ( $vn_list_id > 0 ) {
				...
				if ( $ret_format == 'select' ) {
					return ca_lists::getListAsHTMLFormElement( $vn_list_id, $pa_options['name'],
						[ 'id' => $pa_options['name'] ],
						array_merge( $pa_options, [ 'useOptionsForValues' => true ] ) );
				} else {
					$va_urls = caJSONLookupServiceUrl( $po_request, 'ca_list_items',
						[ 'list' => caGetListCode( $vn_list_id ) ] );
