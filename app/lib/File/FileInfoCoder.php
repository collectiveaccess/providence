...
class FileInfoCoder {
	...
	public function getFileUrl( $ps_data ) {
		...
		$vs_fpath    = join( "/",
			array( $vs_url_path, $va_file_info["HASH"], $va_file_info["MAGIC"] . "_" . $va_file_info["FILENAME"] ) );
	...
	public function getFileConversions( $ps_data ) {
		...
		if ( ! is_array( $va_file_info ) || ! isset( $va_file_info["CONVERSIONS"] )
		     || ! is_array( $va_file_info["CONVERSIONS"] )
		) {
			return array();
		}
	...
	public function getFileConversionPath( $ps_data, $ps_mimetype ) {
		...
		if ( $va_conversions[ $ps_mimetype ] ) {
			return join( "/", array(
				$va_volume_info["absolutePath"],
				$va_file_info["HASH"],
				$va_file_info["MAGIC"] . "_" . $va_conversions[ $ps_mimetype ]["FILENAME"]
			) );
		} else {
			return false;
		}
	...
	public function getFileConversionUrl( $ps_data, $ps_mimetype ) {
		...
		if ( $va_conversions[ $ps_mimetype ] ) {
			return $va_volume_info["protocol"] . "://" . join( "/", array(
					$va_volume_info["hostname"],
