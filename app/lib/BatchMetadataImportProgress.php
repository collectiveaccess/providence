...
class BatchMetadataImportProgress extends AppControllerPlugin {
	...
	public function dispatchLoopShutdown() {
		...
		if ( $req->isLoggedIn() ) {
			...
			if ( ( $vs_file_input === 'import' )
			     && ( is_dir( $vs_base_import_dir . '/' . $vs_file_import_directory ) )
			) {
			...
			$va_errors = BatchProcessor::importMetadata(
				...
				array_merge( $this->opa_options, array(
					'originalFilename' => $vs_input_filename,
					'progressCallback' => 'caIncrementBatchMetadataImportProgress',
					'reportCallback'   => 'caUpdateBatchMetadataImportResultsReport'
				) )
			);
		}
	}
