...
class BaseEditorController extends ActionController
{
    ...
    public function Summary($pa_options = null)
    {
        ...
        $formats = [];
        if (is_array(
            $available_templates = caGetAvailablePrintTemplates('summary', ['table' => $t_subject->tableName()])
        )) {
            ...
            switch ($va_template_info['fileFormat']) {
                case 'pdf':
                    ...
                    $o_pdf->render(
                        ...
                        array(
                            'stream' => true,
                            'filename' => ($vs_filename = $this->view->getVar(
                                'filename'
                            )) ? $vs_filename : caGetOption(
                                'filename',
                                $va_template_info,
                                'print_summary.pdf'
                            )
    ...
    public function _genTypeNav($pa_params)
    {
        ...
        if (is_array($va_hier)) {
            ...
            $va_limit_to_types = $this->getRequest()->config->get(
                $this->ops_table_name . '_navigation_new_menu_limit_types_to'
            );

            foreach ($va_hier as $vn_item_id => $va_item) {
                if (is_array($va_limit_to_types) && sizeof($va_limit_to_types) && !in_array(
                        $va_item['idno'],
                        $va_limit_to_types
                    )) {
                    continue;
                ...
                $va_subtypes = array();
                if (
                    !(bool)$this->getRequest()->config->get(
    ...
    private function _getSubTypes($pa_subtypes, $pa_types_by_parent_id, $pn_sort_type, $pa_restrict_to_types = null)
    {
        ...
        foreach ($pa_subtypes as $vn_i => $va_type) {
            ...
            if (isset($pa_types_by_parent_id[$va_type['item_id']]) && is_array(
        ...
        foreach ($va_subtypes as $vs_sort_key => $va_type) {
            foreach ($va_type as $vn_item_id => $va_item) {
                ...
                if (is_array($va_limit_to_type_ids) && sizeof($va_limit_to_type_ids) && !in_array(
                        $va_item['parameters']['type_id'],
                        $va_limit_to_type_ids
                    )) {
                    continue;
                }
            ...
            default:
                ...
                $this->response->addContent(
                    $t_subject->getBundleFormHTML(
                        ...
                        array('request' => $this->request, 'contentOnly' => true),
                        $vs_label
                    )
                );
                break;
            ...
            $this->response->addContent(
                $vs_viewer_name::getViewerHTML(
                    ...
                    [
                        ...
                        't_subject' => $t_subject,
                        'display' => caGetMediaDisplayInfo('media_overlay', $vs_mimetype)
                    ]
                )
            );
        ...
        if (sizeof($va_file_paths) > 1) {
            ...
            $o_view->setVar(
                ...
                preg_replace(
                    '![^A-Za-z0-9\.\-]+!',
                    '_',
                    trim($t_subject->get($t_subject->getProperty('ID_NUMBERING_ID_FIELD')), "/")
                ) . '.zip'
            );
        } else {
            foreach ($va_file_paths as $vs_path => $vs_name) {
    ...
    public function UploadFiles($pa_options = null)
    {
        ...
        $va_files_to_delete = caGetDirectoryContentsAsList(
            $vs_user_dir,
            true,
            false,
            false,
            true,
            array('modifiedSince' => time() - $vn_timeout)
        );
        ...
        foreach ($_FILES as $vn_i => $va_file) {
            ...
            copy(
                $va_file['tmp_name'],
                $vs_dest_path = $vs_tmp_directory . "/userMedia{$vn_user_id}/{$vs_dest_filename}"
            );

            // write file metadata
            file_put_contents(
                "{$vs_dest_path}_metadata",
                json_encode(
                    array(
