...
class BaseEditorController extends ActionController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		$this->opo_result_context     = new ResultContext( $po_request, $this->ops_table_name,
			ResultContext::getLastFind( $po_request, $this->ops_table_name ) );
	}
	# -------------------------------------------------------

	/**
	 * Generates a form for editing new or existing records. The form is rendered into the current view, inherited from
	 * ActionController
	 *
	 * @param array $pa_values  An optional array of values to preset in the format, overriding any existing values in
	 *                          the model of the record being editing.
	...
	public function Edit( $pa_values = null, $pa_options = null ) {
		...
		list( $vn_subject_id, $t_subject, $t_ui, $vn_parent_id, $vn_above_id, $vn_after_id )
		...
		//
		if ( ( $vs_mode == 'dupe' )
		     && $this->request->user->canDoAction( 'can_duplicate_' . $t_subject->tableName() )
		) {
			if ( ! ( $vs_type_name = $t_subject->getTypeName() ) ) {
				$vs_type_name = $t_subject->getProperty( 'NAME_SINGULAR' );
			}
			// Trigger "before duplicate" hook
			$this->opo_app_plugin_manager->hookBeforeDuplicateItem( array(
				'id'         => $vn_subject_id,
			...
			if ( $t_dupe = $t_subject->duplicate( array(
				...
				'duplicate_nonpreferred_labels'        => $this->request->user->getPreference( $t_subject->tableName()
				                                                                               . '_duplicate_nonpreferred_labels' ),
				'duplicate_attributes'                 => $this->request->user->getPreference( $t_subject->tableName()
				                                                                               . '_duplicate_attributes' ),
				'duplicate_relationships'              => $this->request->user->getPreference( $t_subject->tableName()
				                                                                               . '_duplicate_relationships' ),
				'duplicate_current_relationships_only' => $this->request->user->getPreference( $t_subject->tableName()
				                                                                               . '_duplicate_current_relationships_only' ),
				'duplicate_relationship_attributes'    => $this->request->user->getPreference( $t_subject->tableName()
				                                                                               . '_duplicate_relationship_attributes' ),
				'duplicate_media'                      => $this->request->user->getPreference( $t_subject->tableName()
				                                                                               . '_duplicate_media' ),
				'duplicate_subitems'                   => $this->request->user->getPreference( $t_subject->tableName()
				                                                                               . '_duplicate_subitems' ),
				'duplicate_element_settings'           => $this->request->user->getPreference( $t_subject->tableName()
				                                                                               . '_duplicate_element_settings' ),
				'duplicate_children'                   => $this->request->user->getPreference( $t_subject->tableName()
				                                                                               . '_duplicate_children' )
			) )
			) {
				$this->notification->addNotification( _t( 'Duplicated %1 "%2" (%3)', $vs_type_name,
					$t_subject->getLabelForDisplay(),
					$t_subject->get( $t_subject->getProperty( 'ID_NUMBERING_ID_FIELD' ) ) ),
					__NOTIFICATION_TYPE_INFO__ );

				// Trigger duplicate hook
				$this->opo_app_plugin_manager->hookDuplicateItem( array(
					'id'         => $vn_subject_id,
					'table_num'  => $t_subject->tableNum(),
					'table_name' => $t_subject->tableName(),
					'instance'   => $t_subject,
				...
				$this->response->setRedirect( caNavUrl( $this->request, $this->request->getModulePath(),
					$this->request->getController(), $this->request->getAction(),
					array( $t_subject->primaryKey() => $t_dupe->getPrimaryKey() ) ) );

				return;
			} else {
				$this->notification->addNotification( _t( 'Could not duplicate %1: %2', $vs_type_name,
					join( '; ', $t_subject->getErrors() ) ), __NOTIFICATION_TYPE_ERROR__ );
			}
		}
		...
		if ( $vn_above_id ) {
			...
			if ( ( $t_instance = Datamodel::getInstanceByTableName( $this->ops_table_name ) )
			     && $t_instance->load( $vn_above_id )
			) {
				$vn_parent_id = $t_instance->get(
					$vs_parent_id_fld = $t_instance->getProperty( 'HIERARCHY_PARENT_ID_FLD' ) );
		...
		if ( ( ! $t_subject->getPrimaryKey() ) && ( $vn_subject_id ) ) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/2500?r='
		...
		if ( ! $t_ui || ! $t_ui->getPrimaryKey() ) {
			$this->notification->addNotification( _t( 'There is no configuration available for this editor. Check your system configuration and ensure there is at least one valid configuration for this type of editor.' ),
				__NOTIFICATION_TYPE_ERROR__ );

			$this->postError( 1260,
				_t( 'There is no configuration available for this editor. Check your system configuration and ensure there is at least one valid configuration for this type of editor.' ),
				"BaseEditorController->Edit()" );

			return;
		}

		$va_nav = $t_ui->getScreensAsNavConfigFragment( $this->request, $vn_type_id, $this->request->getModulePath(),
			$this->request->getController(), $this->request->getAction(),
			[],
			[]
		);
		if ( ! $this->request->getActionExtra()
		     || ! isset( $va_nav['fragment'][ str_replace( "Screen", "screen_", $this->request->getActionExtra() ) ] )
		) {
			if ( ( $vs_bundle = $this->request->getParameter( 'bundle', pString ) )
			     && ( $vs_bundle_screen
					= $t_ui->getScreenWithBundle( $vs_bundle ) )
			) {
		...
		$this->opo_app_plugin_manager->hookEditItem( array(
			'id'         => $vn_subject_id,
			'table_num'  => $t_subject->tableNum(),
			'table_name' => $t_subject->tableName(),
			'instance'   => $t_subject
		...
		if ( $vn_subject_id ) { // don't save "empty" / new record editor location. pk has to be set
			...
			Session::setVar( 'save_and_return_locations',
				caPushToStack( $va_save, $va_save_and_return, __CA_SAVE_AND_RETURN_STACK_SIZE__ ) );
		}
		...
		$violations_to_prompt = $t_subject->getMetadataDictionaryRuleViolations( null,
			[ 'limitToShowAsPrompt' => true, 'screen_id' => $this->request->getActionExtra() ] );
		$this->getView()->setVar( 'show_show_notifications',
			( is_array( $violations_to_prompt ) && ( sizeof( $violations_to_prompt ) > 0 ) ) );
	 ...
	 * into the current view, inherited from ActionController
	 *
	...
	public function Save( $pa_options = null ) {
		...
		list( $vn_subject_id, $t_subject, $t_ui, $vn_parent_id, $vn_above_id, $vn_after_id, $vs_rel_table,
			$vn_rel_type_id, $vn_rel_id )
			= $this->_initView( $pa_options );
		...
		if ( ! sizeof( $_POST ) ) {
			$this->notification->addNotification( _t( "Cannot save using empty request. Are you using a bookmark?" ),
				__NOTIFICATION_TYPE_ERROR__ );
			$this->render( 'screen_html.php' );
		...
		if ( $vn_subject_id && ! $t_subject->getPrimaryKey() ) {
			$this->notification->addNotification( _t( "%1 does not exist", $vs_type_name ),
				__NOTIFICATION_TYPE_ERROR__ );
		...
		if ( $this->_beforeSave( $t_subject, $vb_is_insert ) ) {
			if ( $vb_save_rc = $t_subject->saveBundlesForScreen( $this->request->getActionExtra(), $this->request,
				$va_opts )
			) {
		...
		if ( ! $vn_subject_id ) { // this was an insert
			...
			if ( ! $vb_save_rc ) { // failed insert
				...
				if ( $t_subject->getAppConfig()->get( 'perform_item_level_access_checking' )
				     && ! $t_subject->getAppConfig()->get( "{$this->ops_table_name}_dont_do_item_level_access_control" )
				) {
				...
				if ( $vn_above_id && ( $t_instance = Datamodel::getInstanceByTableName( $this->ops_table_name, true ) )
				     && $t_instance->load( $vn_above_id )
				) {
					...
					if ( $t_instance->numErrors() ) {
						$this->notification->addNotification( join( "; ", $t_instance->getErrors() ),
							__NOTIFICATION_TYPE_ERROR__ );
					}
				}
				...
				if ( $vn_after_id ) {
					...
					if ( $t_subject->numErrors() ) {
						$this->notification->addNotification( join( "; ", $t_subject->getErrors() ),
							__NOTIFICATION_TYPE_ERROR__ );
					}
				}
			}
		...
		$va_general_errors
		           = $this->request->getActionErrors( 'general' );        // just "general" errors - ones that are not attached to a specific part of the form
		...
		$vb_has_errors = ( is_array( $va_errors )
		                   && ( sizeof( $va_errors ) > 0 ) ); // don't redirect back when there were form errors
		if ( ( (bool) $this->getRequest()->getParameter( 'is_save_and_return', pInteger ) ) && ! $vb_has_errors ) {
			...
			if ( is_array( $va_save_and_return ) ) {
				...
				do {
					...
					( sizeof( $va_save_and_return ) > 0 )
					&& // only keep going if there are more saved locations
					(
						! $va_pop['key']
						|| // keep going if key is empty (i.e. it was a "create new record" screen)
						( ( $va_pop['table'] == $t_subject->tableName() )
						  && ( $va_pop['key'] == $vn_subject_id ) ) // keep going if the record is the current one
					)
				);
				...
				if ( ( $va_pop['table'] != $t_subject->tableName() ) || ( $va_pop['key'] != $vn_subject_id ) ) {
					if ( isset( $va_pop['url_path'] ) && ( strlen( $va_pop['url_path'] ) > 0 ) ) {
						...
						$this->getResponse()->setRedirect( caEditorUrl( $this->getRequest(), $va_pop['table'],
							$va_pop['key'] ) );
					}
				}
			}
	...
	/**
	 * Performs two-step delete of an existing record. The first step is a confirmation dialog, followed by actual
	...
	public function Delete( $pa_options = null ) {
		...
		if ( $t_subject->getAppConfig()->get( 'perform_item_level_access_checking' ) ) {
			if ( $t_subject->checkACLAccessForUser( $this->request->user ) < __CA_ACL_EDIT_DELETE_ACCESS__ ) {
				$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/2580?r='
				                              . urlencode( $this->request->getFullUrlPath() ) );
		...
		if ( ! $vn_parent_id
		     && ( in_array( $t_subject->getProperty( 'HIERARCHY_TYPE' ),
				array( __CA_HIER_TYPE_SIMPLE_MONO__, __CA_HIER_TYPE_MULTI_MONO__ ) ) )
		) {
			$this->notification->addNotification( _t( "Cannot delete root of hierarchy" ),
				__NOTIFICATION_TYPE_ERROR__ );
		...
		if ( $vb_confirm = ( $this->request->getParameter( 'confirm', pInteger ) == 1 ) ? true : false ) {
			...
			if ( ( $vn_remap_id = $this->request->getParameter( 'caReferenceHandlingToRemapToID', pInteger ) )
			     && ( $this->request->getParameter( 'caReferenceHandlingTo', pString ) == 'remap' )
			) {
				switch ( $subject_table ) {
					case 'ca_relationship_types':
						if ( $vn_c = $t_subject->moveRelationshipsToType( $vn_remap_id ) ) {
							...
							$this->notification->addNotification( ( $vn_c == 1 )
								? _t( "Transferred %1 relationship to type <em>%2</em>", $vn_c,
									$t_target->getLabelForDisplay() )
								: _t( "Transferred %1 relationships to type <em>%2</em>", $vn_c,
									$t_target->getLabelForDisplay() ), __NOTIFICATION_TYPE_INFO__ );
						}
						break;
					default:
						...
						if ( $vn_c > 0 ) {
							...
							$this->notification->addNotification( ( $vn_c == 1 )
								? _t( "Transferred %1 relationship to <em>%2</em> (%3)", $vn_c,
									$t_target->getLabelForDisplay(),
									$t_target->get( $t_target->getProperty( 'ID_NUMBERING_ID_FIELD' ) ) )
								: _t( "Transferred %1 relationships to <em>%2</em> (%3)", $vn_c,
									$t_target->getLabelForDisplay(),
									$t_target->get( $t_target->getProperty( 'ID_NUMBERING_ID_FIELD' ) ) ),
								__NOTIFICATION_TYPE_INFO__ );
						}
						...
						if ( $t_subject->isHierarchical()
						     && is_array( $va_children = call_user_func( "{$subject_table}::getHierarchyChildrenForIDs",
							...
							$this->notification->addNotification( ( $vn_child_count == 1 )
								? _t( "Transferred %1 children to <em>%2</em> (%3)", $vn_child_count,
									$t_target->getLabelForDisplay(),
									$t_target->get( $t_target->getProperty( 'ID_NUMBERING_ID_FIELD' ) ) )
								: _t( "Transferred %1 children to <em>%2</em> (%3)", $vn_child_count,
									$t_target->getLabelForDisplay(),
				...
				if ( $t_subject->isHierarchical()
					...
					$this->notification->addNotification( ( $vn_child_count == 1 ) ? _t( "Deleted %1 child",
						$vn_child_count ) : _t( "Deleted %1 children", $vn_child_count ), __NOTIFICATION_TYPE_INFO__ );
				}
			}

			// Do we need to move references contained in attributes bound to this item?
			if ( ( $vn_remap_id = $this->request->getParameter( 'caReferenceHandlingToRemapFromID', pInteger ) )
			     && ( $this->request->getParameter( 'caReferenceHandlingFrom', pString ) == 'remap' )
			) {
				try {
					$t_subject->moveAttributes( $vn_remap_id,
						$t_subject->getAuthorityElementList( [ 'idsOnly' => true ] ) );
				} catch ( ApplicationException $o_e ) {
					$this->notification->addNotification( _t( "Could not move references to other items in metadata before delete: %1",
						$o_e->getErrorDescription() ), __NOTIFICATION_TYPE_ERROR__ );
				}
			}
		...
		if ( $t_subject->numErrors() ) {
			...
			if ( $vb_confirm ) {
				$this->notification->addNotification( _t( "%1 was deleted", caUcFirstUTF8Safe( $vs_type_name ) ),
					__NOTIFICATION_TYPE_INFO__ );
				...
				$this->opo_app_plugin_manager->hookDeleteItem( array(
					'id'         => $vn_subject_id,
					'table_num'  => $t_subject->tableNum(),
	...
	protected function redirectAfterDelete( $t_subject ) {
		...
		$redirect_url = $this->opo_result_context->getResultsUrlForLastFind( $this->getRequest(),
			$t_subject->tableName() );
	...
	public function Summary( $pa_options = null ) {
		...
		if ( ( defined( '__CA_ENABLE_DEBUG_OUTPUT__' ) && __CA_ENABLE_DEBUG_OUTPUT__ )
		     || (bool) $this->request->config->get( 'display_template_debugger' )
		) {
		...
		$va_displays
		           = caExtractValuesByUserLocale( $t_display->getBundleDisplays( array(
			'table'           => $t_subject->tableNum(),
			'user_id'         => $this->request->getUserID(),
			'access'          => __CA_BUNDLE_DISPLAY_READ_ACCESS__,
			'restrictToTypes' => array( $t_subject->getTypeID() )
		) ) );

		if ( ( ! ( $vn_display_id = $this->request->getParameter( 'display_id', pInteger ) ) )
		     || ! isset( $va_displays[ $vn_display_id ] )
		) {
			$vn_display_id = $this->request->user->getVar( $t_subject->tableName() . '_summary_display_id' );
		}
		if ( ! isset( $va_displays[ $vn_display_id ] )
		     || ( is_array( $va_displays[ $vn_display_id ]['settings']['show_only_in'] )
		          && sizeof( $va_displays[ $vn_display_id ]['settings']['show_only_in'] )
		          && ! in_array( 'editor_summary', $va_displays[ $vn_display_id ]['settings']['show_only_in'] ) )
		) {
			$va_tmp        = array_filter( $va_displays, function ( $v ) {
				return ! isset( $v['settings']['show_only_in'] ) || ! is_array( $v['settings']['show_only_in'] )
				       || in_array( 'editor_summary', $v['settings']['show_only_in'] );
			} );
		...
		if ( $t_display->load( $vn_display_id )
		     && ( $t_display->haveAccessToDisplay( $this->request->getUserID(), __CA_BUNDLE_DISPLAY_READ_ACCESS__ ) )
		) {
			$this->view->setVar( 'display_id', $vn_display_id );

			$va_placements = $t_display->getPlacements( array(
				'returnAllAvailableIfEmpty' => true,
				'table'                     => $t_subject->tableNum(),
				'user_id'                   => $this->request->getUserID(),
				'access'                    => __CA_BUNDLE_DISPLAY_READ_ACCESS__,
				'no_tooltips'               => true,
			...
			$va_display_list = $t_display->getDisplayListForResultsEditor( $t_subject->tableName(),
				[ 'user_id' => $this->request->getUserID() ] );
		...
		$formats = [];
		if ( is_array( $available_templates = caGetAvailablePrintTemplates( 'summary',
			[ 'table' => $t_subject->tableName(), 'restrictToTypes' => $t_subject->getTypeID() ] ) )
		) {
		...
		$this->view->setVar( $t_subject->tableName() . '_summary_last_settings',
			Session::getVar( $t_subject->tableName() . '_summary_last_settings' ) );

		$this->opo_app_plugin_manager->hookSummarizeItem( array(
			'id'         => $vn_subject_id,
			'table_num'  => $t_subject->tableNum(),
			'table_name' => $t_subject->tableName(),
			'instance'   => $t_subject
	...
	public function PrintSummary( $pa_options = null ) {
		...
		if ( $t_display->load( $vn_display_id )
			...
			foreach ( $va_placements as $vn_placement_id => $va_display_item ) {
				...
				if ( $t_subject->hasElement( $e )
				     && ( ca_metadata_elements::getElementDatatype( $e ) === __CA_ATTRIBUTE_VALUE_MEDIA__ )
				     && isset( $va_settings['appendMultiPagePDFToPDFOutput'] )
				     && (bool) $va_settings['appendMultiPagePDFToPDFOutput']
				) {
					$media = $t_subject->get( $va_display_item['bundle_name'] . '.path',
						[ 'returnAsArray' => true, 'version' => 'original' ] );;
					$mimetypes = $t_subject->get( $va_display_item['bundle_name'] . '.original.mimetype',
						[ 'returnAsArray' => true ] );
		...
		// PDF output
		//
		if ( ( $ps_template = $this->request->getParameter( 'template', pString ) )
		     && ( preg_match( "!^_([A-Za-z0-9]+)_(.*)$!", $ps_template, $m ) )
		     && ( in_array( $m[1], [ 'pdf', 'docx' ] ) )
		     && is_array( $va_template_info = caGetPrintTemplateDetails( 'summary', $m[2] ) )
		) {
			...
			if ( ! $vn_display_id || ! $t_display
			     || ! is_array( $va_template_info = caGetPrintTemplateDetails( 'summary',
					"{$this->ops_table_name}_" . $t_display->get( 'display_code' ) . "_summary" ) )
			) {
				if ( ! is_array(
					$va_template_info = caGetPrintTemplateDetails( 'summary', "{$this->ops_table_name}_summary" ) )
				) {
					if ( ! is_array( $va_template_info = caGetPrintTemplateDetails( 'summary', "summary" ) ) ) {
						$this->postError( 3110, _t( "Could not find view for PDF" ),
							"BaseEditorController->PrintSummary()" );
		...
		try {
			...
			$va_barcode_files_to_delete += caDoPrintViewTagSubstitution( $this->view, $t_subject,
				$va_template_info['path'], array( 'checkAccess' => $this->opa_access_values ) );
			...
			switch ( $va_template_info['fileFormat'] ) {
				case 'pdf':
					...
					$va_page_size   = PDFRenderer::getPageSize( caGetOption( 'pageSize', $va_template_info, 'letter' ),
						'mm', caGetOption( 'pageOrientation', $va_template_info, 'portrait' ) );
					...
					$o_pdf->setPage( caGetOption( 'pageSize', $va_template_info, 'letter' ),
						caGetOption( 'pageOrientation', $va_template_info, 'portrait' ),
						caGetOption( 'marginTop', $va_template_info, '0mm' ),
						caGetOption( 'marginRight', $va_template_info, '0mm' ),
						caGetOption( 'marginBottom', $va_template_info, '0mm' ),
						caGetOption( 'marginLeft', $va_template_info, '0mm' ) );

					if ( ! $filename_template = $this->request->config->get( $t_subject->tableName()
					                                                         . '_summary_file_naming' )
					) {
						$filename_template = $this->view->getVar( 'filename' ) ? $filename_template
							: caGetOption( 'filename', $va_template_info, 'print_summary' );
					}
					if ( ! ( $filename = caProcessTemplateForIDs( $filename_template, $t_subject->tableName(),
						[ $vn_subject_id ] ) )
					) {
						$filename = 'print_summary';
					}
					$o_pdf->render( $vs_content,
						[ 'stream' => true, 'append' => $media_to_append, 'filename' => "{$filename}.pdf" ] );
	...
	public function PrintBundle( $pa_options = null ) {
		...
		if ( $this->request->user->getBundleAccessLevel( $t_subject->tableName(), $vs_element )
		...
		if ( $vn_attribute_id > 0 ) {
			...
			if ( ( (int) $o_attr->getRowID() !== (int) $vn_subject_id )
			     || ( (int) $o_attr->getTableNum() !== (int) $t_subject->tableNum() )
			) {
				$this->postError( 2320, _t( "Element is not part of current item" ),
					"BaseEditorController->PrintBundle()" );

				return;
			}
			$this->view->setVar( 'valuesAsAttributeInstances', $va_values = array( $o_attr ) );
		} else {
			$this->view->setVar( 'valuesAsAttributeInstances',
				$va_values = $t_subject->getAttributesByElement( $vs_element ) );
		}
		...
		try {
			...
			$o_pdf->setPage( caGetOption( 'pageSize', $va_template_info, 'letter' ),
				...
				caGetOption( 'marginBottom', $va_template_info, '0mm' ),
				caGetOption( 'marginLeft', $va_template_info, '0mm' ) );
			$o_pdf->render( $vs_content, array(
				'stream'   => true,
				'filename' => caGetOption( 'filename', $va_template_info, 'print_bundles.pdf' )
			) );
	...
	public function Log( $pa_options = null ) {
		...
		if ( ca_user_roles::isValidAction( 'can_view_change_log_' . $t_subject->tableName() )
		     && ( ! $this->request->user->canDoAction( 'can_view_change_log_' . $t_subject->tableName() ) )
		) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/2575?r='
			                              . urlencode( $this->request->getFullUrlPath() ) );
	...
	public function Access( $pa_options = null ) {
		...
		if ( ( ! $this->request->user->canDoAction( 'can_change_acl_' . $t_subject->tableName() ) ) ) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/2570?r='
			                              . urlencode( $this->request->getFullUrlPath() ) );
	...
	public function SetAccess( $pa_options = null ) {
		...
		if ( ( ! $t_subject->isSaveable( $this->request ) )
		     || ( ! $this->request->user->canDoAction( 'can_change_acl_' . $t_subject->tableName() ) )
		) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/2570?r='
			                              . urlencode( $this->request->getFullUrlPath() ) );
		...
		foreach ( $_REQUEST as $vs_key => $vs_val ) {
			if ( preg_match( "!^{$vs_form_prefix}_user_id(.*)$!", $vs_key, $va_matches ) ) {
				$vn_user_id = (int) $this->request->getParameter( $vs_form_prefix . '_user_id' . $va_matches[1],
					pInteger );
				$vn_access  = $this->request->getParameter( $vs_form_prefix . '_user_access_' . $va_matches[1],
					pInteger );
				if ( $vn_access >= 0 ) {
		...
		foreach ( $_REQUEST as $vs_key => $vs_val ) {
			if ( preg_match( "!^{$vs_form_prefix}_group_id(.*)$!", $vs_key, $va_matches ) ) {
				$vn_group_id = (int) $this->request->getParameter( $vs_form_prefix . '_group_id' . $va_matches[1],
					pInteger );
				$vn_access   = $this->request->getParameter( $vs_form_prefix . '_group_access_' . $va_matches[1],
					pInteger );
				if ( $vn_access >= 0 ) {
		...
		if ( $t_subject->hasField( 'acl_inherit_from_ca_collections' )
		     || $t_subject->hasField( 'acl_inherit_from_parent' )
		) {
			$t_subject->setMode( ACCESS_WRITE );
			if ( $t_subject->hasField( 'acl_inherit_from_ca_collections' ) ) {
				$t_subject->set( 'acl_inherit_from_ca_collections',
					$this->request->getParameter( 'acl_inherit_from_ca_collections', pInteger ) );
			}
			if ( $t_subject->hasField( 'acl_inherit_from_parent' ) ) {
				$t_subject->set( 'acl_inherit_from_parent',
					$this->request->getParameter( 'acl_inherit_from_parent', pInteger ) );
			}
			$t_subject->update();

			if ( $t_subject->numErrors() ) {
				$this->postError( 1250,
					_t( 'Could not set ACL inheritance settings: %1', join( "; ", $t_subject->getErrors() ) ),
					"BaseEditorController->SetAccess()" );
			}
		}
	...
	public function ChangeType( $pa_options = null ) {
		...
		if ( $this->request->user->canDoAction( "can_change_type_" . $t_subject->tableName() ) ) {
			if ( method_exists( $t_subject, "changeType" ) ) {
				...
				if ( ! $t_subject->changeType(
					$vn_new_type_id = $this->request->getParameter( 'type_id', pInteger ) )
				) {
					// post error
					$this->notification->addNotification( _t( 'Could not set type to <em>%1</em>: %2',
						caGetListItemForDisplay( $t_subject->getTypeListCode(), $vn_new_type_id ),
						join( "; ", $t_subject->getErrors() ) ), __NOTIFICATION_TYPE_ERROR__ );
				} else {
					$this->notification->addNotification( _t( 'Set type to <em>%1</em>', $t_subject->getTypeName() ),
						__NOTIFICATION_TYPE_INFO__ );
				}
				$this->opo_app_plugin_manager->hookSaveItem( array(
					'id'         => $vn_subject_id,
	...
	protected function _initView( $pa_options = null ) {
		...
		if ( $t_subject->getAppConfig()->get( $t_subject->tableName() . '_dont_use_labels' ) ) {
			MetaTagManager::setWindowTitle( _t( "Editing %1 : %2",
				( $vs_type = $t_subject->getTypeName() ) ? $vs_type : $t_subject->getProperty( 'NAME_SINGULAR' ),
				( $vn_subject_id ) ? $t_subject->get( $t_subject->getProperty( 'ID_NUMBERING_ID_FIELD' ) )
					: _t( 'new %1', $t_subject->getTypeName() ) ) );
		} else {
			MetaTagManager::setWindowTitle( _t( "Editing %1 : %2",
				( $vs_type = $t_subject->getTypeName() ) ? $vs_type : $t_subject->getProperty( 'NAME_SINGULAR' ),
				( $vn_subject_id )
					? $t_subject->getLabelForDisplay( true ) : _t( 'new %1', $t_subject->getTypeName() ) ) );
		}
		...
		if ( $vs_parent_id_fld = $t_subject->getProperty( 'HIERARCHY_PARENT_ID_FLD' ) ) {
			$this->view->setVar( 'parent_id',
				$vn_parent_id = $this->request->getParameter( $vs_parent_id_fld, pInteger ) );
			...
			if ( ! $vn_subject_id ) {
				...
				if (
					...
					&& ( $t_parent->get( 'parent_id' )
					     || ( $t_parent->getHierarchyType() == __CA_HIER_TYPE_ADHOC_MONO__ ) )
					&& ( ( ! method_exists( $t_parent, "getIDNoPlugInInstance" )
					       || ! ( $o_numbering_plugin
								= $t_parent->getIDNoPlugInInstance() ) )
					     || ( $o_numbering_plugin->formatHas( 'FREE', 0 ) ) )
				) {
	...
	public function _genDynamicNav( $pa_params, $pa_options = null ) {
		...
		if ( ! ( $vn_type_id = $t_subject->getTypeID() )
		     && ! ( $vn_type_id
				= $this->request->getParameter( $t_subject->getTypeFieldName(), pInteger ) )
		) {
			$vn_type_id = $t_subject->getDefaultTypeID();
		}
		$va_nav = $t_ui->getScreensAsNavConfigFragment( $this->request, $vn_type_id, $pa_params['default']['module'],
			$pa_params['default']['controller'], $pa_params['default']['action'],
	...
	# -------------------------------------------------------
	/**
	 * Returns navigation fragment for types and subtypes of a given primary item type (Eg. ca_objects). Used to
	 * generate dynamic type menus from database by AppNavigation class.
	 *
	...
	public function _genTypeNav( $pa_params ) {
		...
		if ( $t_subject->getAppConfig()->get( 'perform_type_access_checking' ) ) {
			$va_restrict_to_types = caGetTypeRestrictionsForUser( $this->ops_table_name,
		...
		if ( is_array( $va_hier ) ) {
			...
			foreach ( $va_hier as $vn_item_id => $va_item ) {
				if ( $va_item['settings'] ) {
					...
					if ( is_array( $va_settings ) && isset( $va_settings['render_in_new_menu'] )
					     && ! ( (bool) $va_settings['render_in_new_menu'] )
					) {
			...
			$limit_to_types = $this->getRequest()->config->get( $this->ops_table_name
			                                                    . '_navigation_new_menu_limit_types_to' );
			$exclude_types  = $this->getRequest()->config->get( $this->ops_table_name
			                                                    . '_navigation_new_menu_exclude_types' );

			$show_top_level_types_only     = (bool) $this->getRequest()->config->get( $this->ops_table_name
			                                                                          . '_navigation_new_menu_shows_top_level_types_only' );
			$enforce_strict_type_hierarchy = $this->getRequest()->config->get( $this->ops_table_name
			                                                                   . '_enforce_strict_type_hierarchy' );

			foreach ( $va_hier as $vn_item_id => $va_item ) {
				if ( is_array( $limit_to_types ) && sizeof( $limit_to_types )
				     && ! in_array( $va_item['idno'], $limit_to_types )
				) {
					continue;
				}
				if ( is_array( $exclude_types ) && sizeof( $exclude_types )
				     && in_array( $va_item['idno'], $exclude_types )
				) {
					continue;
				}
	...
	/**
	 * Returns navigation fragment for subtypes of a given primary item type (Eg. ca_objects). Used to generate dynamic
	 * type menus from database by AppNavigation class. Called via _genTypeNav(), which is in turn called by
	 * AppNavigation.
	 *
	 ...
	 *                                     firstEnabled = Stop returning subtypes once an enabled item is found.
	 *                                     [Default is false]
	 *
	 * @return array List of subtypes ready for inclusion in a menu spec
	 */
	private function _getSubTypes(
		$pa_subtypes, $pa_types_by_parent_id, $pn_sort_type, $pa_restrict_to_types = null, $options = null
	) {
		$va_subtypes   = array();
		...
		foreach ( $pa_subtypes as $vn_i => $va_type ) {
			...
			if ( $first_enabled && $va_type['is_enabled'] ) {
				$va_subsubtypes
					= [];    // in "first enabled" mode we don't pull subtypes when we encounter an enabled item
			} elseif ( isset( $pa_types_by_parent_id[ $va_type['item_id'] ] )
			           && is_array( $pa_types_by_parent_id[ $va_type['item_id'] ] )
			) {
				$va_subsubtypes = $this->_getSubTypes( $pa_types_by_parent_id[ $va_type['item_id'] ],
					$pa_types_by_parent_id, $pn_sort_type, $pa_restrict_to_types, $options );
			} else {
				$va_subsubtypes = [];
			}
		...
		$va_limit_to_type_ids = ( is_array( $va_limit_to_types ) && sizeof( $va_limit_to_types ) )
			? caMakeTypeIDList( $this->ops_table_name, $va_limit_to_types,
				[ 'dontIncludeSubtypesInTypeRestriction' => true ] ) : null;
		foreach ( $va_subtypes as $vs_sort_key => $va_type ) {
			foreach ( $va_type as $vn_item_id => $va_item ) {
				...
				if ( is_array( $limit_to_types ) && sizeof( $limit_to_types )
				     && ! in_array( $va_item['parameters']['type_id'], $limit_to_types )
				) {
					continue;
				}
				if ( is_array( $exclude_types ) && sizeof( $exclude_types )
	...
	# ------------------------------------------------------------------
	/**
	 * Add item to user's watch list. Intended to be called via ajax, and JSON response is returned in the current view
	...
	public function getHierarchyForDisplay( $pa_options = null ) {
		...
		$vs_hierarchy_display = $t_subject->getHierarchyNavigationHTMLFormBundle( $this->request,
			'caHierarchyOverviewPanelBrowser', array(),
			array( 'open_hierarchy' => true, 'no_close_button' => true, 'hierarchy_browse_tab_class' => 'foo' ) );
	...
	public function reload() {
		...
		switch ( $ps_bundle ) {
			case '__inspector__':
				$this->response->addContent( $this->info( array(
					$t_subject->primaryKey() => $vn_subject_id,
			...
			default:
				...
				$this->response->addContent( $t_subject->getBundleFormHTML( $ps_bundle, "P{$pn_placement_id}",
					array_merge( $t_placement->get( 'settings' ), [ 'placement_id' => $pn_placement_id ] ), [
	...
	public function loadBundleValues() {
		...
		if ( $this->request->user->getBundleAccessLevel( $t_subject->tableName(), $ps_bundle_name )
		...
		$d = $t_subject->getBundleFormValues( $ps_bundle_name, "{$pn_placement_id}", $t_placement->get( 'settings' ),
			array(
				'start'         => $pn_start,
				'limit'         => $pn_limit,
				'sort'          => $sort,
	...
	public function processTemplate() {
		...
		$this->view->setVar( 'processed_template',
			json_encode( caProcessTemplateForIDs( $ps_template, $t_subject->tableNum(), array( $vn_subject_id ) ) ) );
	...
	/**
	 * Returns formatted list of media in a media attribute or container attribute that includes at least one media
	 * attribute. Used by CKEditor media reference dialog.
	 */
	public function getMediaAttributeList() {
		...
		$va_media_list       = array_shift( $t_subject->get( $t_subject->tableName() . ".{$va_bundle_name_bits[0]}",
			[ 'returnAsArray' => true, 'returnWithStructure' => true ] ) );
				...
				switch ( $o_val->getType() ) {
					case __CA_ATTRIBUTE_VALUE_MEDIA__:
						...
						foreach ( $o_val->getVersions() as $vs_version ) {
							$va_media_list[ $vn_attribute_id ][ $vs_element_code ]['urls'][ $vs_version ]
								= $o_val->getDisplayValue( [
								'return'  => 'url',
								'version' => $vs_version
							] );
							$va_media_list[ $vn_attribute_id ][ $vs_element_code ]['tags'][ $vs_version ]
								= $o_val->getDisplayValue( [
								'return'  => 'tag',
								'version' => $vs_version
							] );
							$va_media_list[ $vn_attribute_id ][ $vs_element_code ]['value_id']
								= $o_val->getValueID();
						}
						break;
	...
	public function info( $pa_parameters ) {
		...
		if ( $t_item->getPrimaryKey() ) {
			if ( method_exists( $t_item, "getRepresentations" ) ) {
				$this->view->setVar( 'representations',
					$t_item->getRepresentations( array( 'preview170', 'preview' ) ) );
			} else {
			...
			if ( $t_item->isHierarchical() ) {
				...
				$va_ancestors = array_reverse( caExtractValuesByUserLocaleFromHierarchyAncestorList(
					$t_item->getHierarchyAncestors( null, array(
							...
							'additionalTableWheres'       => ( $t_label->hasField( 'is_preferred' ) )
								? array( "({$vs_label_table}.is_preferred = 1 OR {$vs_label_table}.is_preferred IS NULL)" )
								: array(),
							'includeSelf'                 => false
						)
					), $vs_pk, $vs_display_field, 'idno' ) );

				$this->view->setVar( 'object_collection_collection_ancestors',
					array() ); // collections to display as object parents when ca_objects_x_collections_hierarchy_enabled is enabled
				if ( ( $t_item->tableName() == 'ca_objects' )
					...
					if ( is_array( $va_collections = $t_item->getRelatedItems( 'ca_collections', array(
						'restrictToRelationshipTypes' => array(
							$t_item->getAppConfig()->get( 'ca_objects_x_collections_hierarchy_relationship_type' )
						)
					) ) )
					) {
	...
	protected function _checkAccess( $pt_subject, $pa_options = null ) {
		...
		if ( $pt_subject->hasField( 'deleted' ) && $pt_subject->get( 'deleted' ) ) {
			if ( ! caGetOption( 'dontRedirectOnDelete', $pa_options, false ) ) {
				$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/2550?r='
				                              . urlencode( $this->request->getFullUrlPath() ) );
			}
		...
		if ( $pt_subject->getAppConfig()->get( 'perform_type_access_checking' ) ) {
			$va_restrict_to_types = caGetTypeRestrictionsForUser( $this->ops_table_name,
				array( 'access' => __CA_BUNDLE_ACCESS_READONLY__ ) );
		}
		if (
			is_array( $va_restrict_to_types ) && sizeof( $va_restrict_to_types )
			&& (
				( $pt_subject->get( 'type_id' ) )
				&& ( $pt_subject->getPrimaryKey()
				     && ! in_array( $pt_subject->get( 'type_id' ), $va_restrict_to_types ) )
				//||
				//(!$pt_subject->getPrimaryKey() && !in_array($this->request->getParameter('type_id', pInteger), $va_restrict_to_types))
			)
		) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/2560?r='
			                              . urlencode( $this->request->getFullUrlPath() ) );
		...
		if ( $pt_subject->getAppConfig()->get( 'perform_source_access_checking' )
		     && $pt_subject->hasField( 'source_id' )
		) {
			if ( is_array( $va_restrict_to_sources = caGetSourceRestrictionsForUser( $this->ops_table_name,
				array( 'access' => __CA_BUNDLE_ACCESS_READONLY__ ) ) )
			) {
				if ( is_array( $va_restrict_to_sources ) && $pt_subject->get( 'source_id' )
				     && ! in_array( $pt_subject->get( 'source_id' ), $va_restrict_to_sources )
				) {
					$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/2562?r='
					                              . urlencode( $this->request->getFullUrlPath() ) );
				...
				if (
					...
					|| ( $pt_subject->get( 'source_id' )
					     && ! in_array( $pt_subject->get( 'source_id' ), $va_restrict_to_sources ) )
					|| ( ( strlen( $vn_source_id = $this->request->getParameter( 'source_id', pInteger ) ) )
					     && ! in_array( $vn_source_id, $va_restrict_to_sources ) )
				) {
					$pt_subject->set( 'source_id',
						$pt_subject->getDefaultSourceID( array( 'request' => $this->request ) ) );
				}
			}
		}
	 ...
	 *        representation_id = the id of the ca_object_representations record to display; the representation must
	 *        belong to the specified object value_id =
	 *
	 *    Optional request parameters:
	 *        version = The version of the representation to display. If omitted the display version configured in
	 *        media_display.conf is used
	 *
	 */
	public function GetMediaOverlay() {
		...
		if ( $pn_value_id = $this->request->getParameter( 'value_id', pInteger ) ) {
			...
			if ( ! ( $vs_viewer_name = MediaViewerManager::getViewerForMimetype( "media_overlay",
				$vs_mimetype = $t_instance->getMediaInfo( 'value_blob', 'INPUT', 'MIMETYPE' ) ) )
			) {
				throw new ApplicationException( _t( 'Invalid viewer' ) );
			}

			$va_display_info = caGetMediaDisplayInfo( 'media_overlay', $vs_mimetype );
			if ( ( $t_instance->numFiles() > 1 ) && ( caGetMediaClass( $vs_mimetype ) === 'image' )
			     && ( $multipage_viewer = caGetOption( 'viewer_for_multipage_images', $va_display_info, null ) )
			) {
			...
			$this->response->addContent( $vs_viewer_name::getViewerHTML(
				...
				[
					'context'    => 'media_overlay',
					't_instance' => $t_instance,
					't_subject'  => $t_subject,
					'display'    => $va_display_info
				] )
			);
			...
			if ( ( ( $vn_use_universal_viewer_for_image_list_length
					= caGetOption( 'use_universal_viewer_for_image_list_length_at_least', $va_display_info, null ) )
			       || ( $vn_use_mirador_for_image_list_length
					= caGetOption( 'use_mirador_for_image_list_length_at_least', $va_display_info, null ) ) )
			) {
				...
				if ( $vn_image_count == $vn_rep_count ) {
					if ( ! is_null( $vn_use_universal_viewer_for_image_list_length )
					     && ( $vn_image_count >= $vn_use_universal_viewer_for_image_list_length )
					) {
						$va_display_info['viewer'] = $vs_viewer_name = 'UniversalViewer';
					} elseif ( ! is_null( $vn_use_mirador_for_image_list_length )
					           && ( $vn_image_count >= $vn_use_mirador_for_image_list_length )
					) {
			...
			if ( ! $vn_subject_id ) {
				if ( is_array(
					     $va_subject_ids = $t_instance->get( $t_subject->tableName() . '.' . $t_subject->primaryKey(),
						     array( 'returnAsArray' => true ) ) )
				     && sizeof( $va_subject_ids )
				) {
					$vn_subject_id = array_shift( $va_subject_ids );
				} else {
					$this->postError( 1100, _t( 'Invalid object/representation' ),
						'ObjectEditorController->GetRepresentationInfo' );
			...
			if ( ! $vn_subject_id ) {
				if ( is_array(
					...
					$this->postError( 1100, _t( 'Invalid object/media' ),
						'ObjectEditorController->GetRepresentationInfo' );
	...
	public function GetMediaData() {
		...
		switch ( $va_identifier['type'] ) {
			case 'representation':
				...
				$va_display_info = caGetMediaDisplayInfo( 'media_overlay', $vs_mimetype );
				if ( $t_subject
				     && ( ( $vn_use_universal_viewer_for_image_list_length
							= caGetOption( 'use_universal_viewer_for_image_list_length_at_least', $va_display_info,
							null ) )
				          || ( $vn_use_mirador_for_image_list_length
							= caGetOption( 'use_mirador_for_image_list_length_at_least', $va_display_info, null ) ) )
				) {
				...
				$this->response->addContent( $vs_viewer_name::getViewerData( $this->request, $ps_identifier, [
					'request'    => $this->request,
					't_subject'  => $t_subject,
					't_instance' => $t_instance,
					'display'    => $va_display_info
	...
	public function SearchMediaData() {
		...
		switch ( $va_identifier['type'] ) {
			case 'representation':
				...
				$this->response->addContent( $vs_viewer_name::searchViewerData( $this->request, $ps_identifier, [
					'request'    => $this->request,
					't_subject'  => $t_subject,
					't_instance' => $t_instance,
					'display'    => $va_display_info
	...
	public function MediaDataAutocomplete() {
		...
		switch ( $va_identifier['type'] ) {
			case 'representation':
				...
				$this->response->addContent( $vs_viewer_name::autocomplete( $this->request, $ps_identifier, [
					'request'    => $this->request,
					't_subject'  => $t_subject,
					't_instance' => $t_instance,
					'display'    => $va_display_info
	 ...
	 *
	 * Expects the following request parameters:
	 *        representation_id = the id of the ca_object_representations record to save annotations to; the
	 *        representation must belong to the specified object
	 *
	 */
	public function SaveAnnotations() {
		...
		if ( is_array( $pa_annotations ) ) {
			foreach ( $pa_annotations as $vn_i => $va_annotation ) {
				$vs_label = ( isset( $va_annotation['label'] ) && ( $va_annotation['label'] ) )
					? $va_annotation['label'] : '';
				if ( isset( $va_annotation['annotation_id'] )
				     && ( $vn_annotation_id
						= $va_annotation['annotation_id'] )
				) {
					...
					// new annotation
					$va_annotation_ids[ $va_annotation['index'] ] = $t_rep->addAnnotation( $vs_label, $g_ui_locale_id,
						$this->request->getUserID(), $va_annotation, 0, 0 );
				}
			}
		}
		$va_annotations = array(
		...
		if (
			...
			$t_rep->setMediaScale( 'media', $vs_measurement,
				sqrt( pow( $vn_width * $vn_image_width, 2 ) + pow( $vn_height * $vn_image_height, 2 ) )
				/ $vn_image_width );
		...
		if ( ! $t_rep->getPrimaryKey() ) {
			...
			if ( $t_rep->applyMediaTransformation( 'media', $ps_op, array( 'angle' => $pn_angle ),
				...
				$va_response = array(
					'action'  => 'process',
					'status'  => 10,
					'message' => _t( 'Transformation failed' )
				);
			}
		}
	...
	public function DownloadMedia( $pa_options = null ) {
		...
		$va_ancestor_ids = ( $t_subject->isHierarchical() ) ? $t_subject->getHierarchyAncestors( null,
			array( 'idsOnly' => true, 'includeSelf' => true ) ) : array( $vn_subject_id );
		if ( $vn_parent_id = array_pop( $va_ancestor_ids ) ) {
			$t_subject->load( $vn_parent_id );
			array_unshift( $va_ancestor_ids, $vn_parent_id );
		}

		$va_child_ids = ( $t_subject->isHierarchical() ) ? $t_subject->getHierarchyChildren( null,
			array( 'idsOnly' => true ) ) : array( $vn_subject_id );
		...
		foreach ( $va_child_ids as $vn_child_id ) {
			...
			foreach ( $va_reps as $vn_representation_id => $va_rep ) {
				...
				$vs_filename                   = caGetRepresentationDownloadFileName( $t_subject->tableName(), [
					'idno'              => $vs_idno,
					'index'             => $vn_c,
					'version'           => $ps_version,
					'extension'         => $va_rep_info['EXTENSION'],
					'original_filename' => $va_rep['info']['original_filename'],
				...
				if ( isset( $va_rep['representation_id'] ) && ( $va_rep['representation_id'] > 0 ) ) {
					...
					if ( ! ( $vs_path = caEmbedMediaMetadataIntoFile( $t_rep->getMediaPath( 'media', $ps_version ),
						$t_subject->tableName(), $t_subject->getPrimaryKey(), $t_subject->getTypeCode(),
						// subject table info
		...
		if ( sizeof( $va_file_paths ) > 1 ) {
			...
			$o_view->setVar( 'archive_name', caGetMediaDownloadArchiveName( $t_subject->tableName(), $vn_subject_id,
				[ 'extension' => 'zip' ] ) ); //preg_replace('![^A-Za-z0-9\.\-]+!', '_', trim($t_subject->get($t_subject->getProperty('ID_NUMBERING_ID_FIELD')), "/")).'.zip');
		} else {
			foreach ( $va_file_paths as $vs_path => $vs_name ) {
	...
	public function DownloadAttributeFile( $pa_options = null ) {
		...
		//
		if ( ( $this->request->user->getBundleAccessLevel( $this->ops_table_name, $t_element->get( 'element_code' ) ) )
		     < __CA_BUNDLE_ACCESS_READONLY__
		) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/2580?r='
			                              . urlencode( $this->request->getFullUrlPath() ) );
	...
	public function UploadFiles( $pa_options = null ) {
		...
		if ( is_array( $_FILES['files'] ) ) {
			foreach ( $_FILES['files']['tmp_name'] as $i => $f ) {
				...
				$stored_files[ $md5 ]
					= "userMedia{$user_id}/{$dest_filename}"; // only return the user directory and file name, not the entire path
			}
		}

		$this->response->addContent( json_encode( [
			'files' => array_values( $stored_files ),
			'msg'   => _t( 'Uploaded %1 files', sizeof( $stored_files ) )
		] ) );
	}
	# -------------------------------------------------------
	...
	public function SetHomeLocation( $options = null ) {
		...
		if ( $t_subject->numErrors() > 0 ) {
			...
			$resp = [
				'ok'        => 1,
				'label'     => $t_location->getWithTemplate( $this->request->config->get( 'ca_storage_locations_hierarchy_browser_display_settings' ) ),
				'timestamp' => time()
			];
		}
	...
	public function ReturnToHomeLocations( $options = null ) {
		if ( ! $this->request->user->canDoAction( "can_edit_ca_objects" ) ) {
			...
			$policies = ca_objects::getHistoryTrackingCurrentValuePoliciesForTable( 'ca_objects' ) )
		) {
			$resp = [
				'ok'        => 0,
				'message'   => _t( 'No policies configured' ),
				'updated'   => [],
				'errors'    => [],
				'timestamp' => time()
			];
			...
			if ( is_array( $policies ) && sizeof( $policies ) ) {
				...
				if ( is_array( $object_ids ) && sizeof( $object_ids ) ) {
					...
					while ( $qr_objects->nextHit() ) {
						...
						if ( ! ( $location_id = $t_object->get( 'home_location_id' ) ) ) {
							continue;
						}
						if ( ! $t_object->isSaveable( $this->request ) ) {
							continue;
						}
						...
						foreach ( $policies as $n => $p ) {
							...
							$d  = isset( $pe[ $t_object->getTypeCode() ] ) ? $pe[ $t_object->getTypeCode() ]
								: $pe['__default__'];
							if ( ! is_array( $d ) || ! isset( $d['trackingRelationshipType'] ) ) {
								$errors[] = $object_id;
								continue;
							}
							...
							$interstitial_values = [];
							if ( is_array(
								$interstitial_elements = caGetOption( "setInterstitialElementsOnAdd", $d, null ) )
							) {
							...
							if ( is_array( $cv = $t_object->getCurrentValue( $n ) )
							     && ( ( $cv['type'] == 'ca_storage_locations' ) && ( $cv['id'] == $location_id ) )
							) {
								$already_home[] = $object_id;
								continue;
							}

							$t_item_rel = $t_object->addRelationship( 'ca_storage_locations', $location_id,
								$d['trackingRelationshipType'], $effective_date );
							ca_objects::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm( $this->request,
								null, null, $t_item_rel, null, $interstitial_elements, [ 'noTemplate' => true ] );
					...
					if ( $h > 0 ) {
						$msg = ( $n == 1 ) ? _t( '%1 %2 returned to home location; %3 already home', $n,
							Datamodel::getTableProperty( 'ca_objects', 'NAME_SINGULAR' ), $h )
							: _t( '%1 %2 returned to home locations; %3 already home', $n,
								Datamodel::getTableProperty( 'ca_objects', 'NAME_PLURAL' ), $h );
					} else {
						$msg = ( $n == 1 ) ? _t( '%1 %2 returned to home location', $n,
							Datamodel::getTableProperty( 'ca_objects', 'NAME_SINGULAR' ) )
							: _t( '%1 %2 returned to home locations', $n,
								Datamodel::getTableProperty( 'ca_objects', 'NAME_PLURAL' ) );
					}
					if ( sizeof( $errors ) ) {
				...
				$resp = [
					'ok'        => 0,
					'message'   => _t( 'No policies available' ),
					'updated'   => [],
					'errors'    => [],
					'timestamp' => time()
				];
	 ...
	 * Gets passed a table name, a list of ids and a key to sort on. Will return a JSON list of the same IDs, just
	 * sorted.
	 *
	 * TODO: remove this
	 */
	public function Sort() {
		if ( ! $this->getRequest()->isLoggedIn() || ( (int) $this->getRequest()->user->get( 'userclass' ) !== 0 ) ) {
			$this->getResponse()->setRedirect( $this->getRequest()->config->get( 'error_display_url' ) . '/n/2320?r='
			                                   . urlencode( $this->getRequest()->getFullUrlPath() ) );
		...
		if ( ! ( $vs_sort_direction = strtolower( $this->getRequest()->getParameter( 'sortDirection', pString ) ) )
		     || ! in_array( $vs_sort_direction, array( 'asc', 'desc' ) )
		) {
		...
		$o_res         = caMakeSearchResult( $t_instance->tableName(), $va_ids,
			array( 'sort' => $va_sort_keys, 'sortDirection' => $vs_sort_direction ) );
