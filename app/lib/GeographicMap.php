...
class GeographicMap {
	 ...
	 *
	 * @param $po_data_object             BaseModel|SearchResult A model instance or search result object from which to
	 *                                    extract data for the map
	 * @param $ps_georeference_field_name string The name of the georeference or geonames attribute to plot map items
	 *                                    with; should be in <table>.<element_code> format (eg.
	 *                                    "ca_objects.map_coords")
	 * @param $pa_options                 array Optional array of options; supported options include:
	 *                                    label - attribute (or field) to use for a short label for the map point, in
	 *                                    <table>.<element_code> format (eg. "ca_objects.idno" or
	 *                                    "ca_objects.preferred_labels.name") content - attribute (or field) to use for
	 *                                    info balloon content shown when map item is clicked, in
	 *                                    <table>.<element_code> format (eg. "ca_objects.description"). The content of
	 *                                    the field is used as-is, so you must apply any styling to the data before it
	 *                                    is stored in the database. If you want to style content "on-the-fly" use
	 *                                    contentView or contentTemplate contentTemplate - text template to use for
	 *                                    info balloon content shown when map item is clicked; attributes in
	 *                                    <table>.<element_code> format will be substituted when prefixed with a caret
	 *                                    ("^"). Eg. "The title of this is ^ca_objects.preferred_labels.name and the date is ^ca_objects.creation_date" contentView - view to use to render info balloon content shown when map item is clicked; specify the view filename with path relative to the main view directory (eg. "Splash/splash_html.php"); the view will be passed a single variable, "data", containing the data object ajaxContentUrl - URL to use to load via AJAX in info balloon. The primary key of the item will be added to the URL as the "id" parameter. The AJAX handler referenced by the URL must return ready-to-display HTML suitable for display in an info balloon. Unlike all other balloon content options, ajaxContentUrl defers rendering of balloon content until viewed, and does not embed content in the initial HTML load. For these reasons it is usually the best performing and most scaleable option. checkAccess - array of access field values to filter data (item and representation level); omit or pass empty array to do no filtering viewPath - path to views; will use standard system view path if not defined request = current request; required for generation of editor links color = hex color to use for item marker; can include bundle display template tags for inclusion of colors stored in metadata elements
	 *
	 * @return array Returns an array with two keys: 'points' = number of unique markers added to map; 'items' = number
	 *               of result hits than were plotted at least once on the map
	 */
	public function mapFrom( $po_data_object, $ps_georeference_field_name, $pa_options = null ) {
		...
		if ( $is_related = ( $t_georef_instance
		                     && ( $t_georef_instance->tableName() !== $po_data_object->tableName() ) )
		) {
		...
		if ( is_subclass_of( $po_data_object, 'SearchResult' ) ) {
			...
			if ( $is_related
			     && is_array(
				     $path = Datamodel::getPath( $t_georef_instance->tableName(), $po_data_object->tableName() ) )
			     && ( sizeof( $path ) === 3 )
			) {
				...
				while ( $po_data_object->nextHit() ) {
					if ( is_array( $rel_ids_for_row = $po_data_object->get( $path[1] . ".relation_id",
						[ 'returnAsArray' => true ] ) )
					) {
			...
			while ( $po_data_object->nextHit() ) {
				if ( is_array( $va_access_values )
				     && ! in_array( $po_data_object->get( $chk_related_for_access ? $chk_related_for_access
						: "{$vs_table}.access" ), $va_access_values )
				) {
					continue;
				}
				if ( $va_coordinates = $po_data_object->get( $ps_georeference_field_name,
					array( 'coordinates' => true, 'returnWithStructure' => true, 'returnAllLocales' => false ) )
				) {
					...
					foreach ( $va_coordinates as $vn_element_id => $va_coord_list ) {
						foreach ( $va_coord_list as $vn_attribute_id => $va_geoname ) {
							...
							if ( ! is_null( $pa_options['labelTemplate'] ) ) {
								$vs_label = caProcessTemplateForIDs( $pa_options['labelTemplate'], $vs_table,
									array( $vn_id ), array() );
							} else {
								if ( ! is_null( $pa_options['label'] ) ) {
									$vs_label = $po_data_object->get( $pa_options['label'], array(
										'returnAsLink' => $vb_render_label_as_link
										                  || ( strpos( $pa_options['contentTemplate'], "<l>" )
										                       !== false )
									) );
							...
							if ( ! is_null( $vs_color ) && $vs_color && ( strpos( $vs_color, '^' ) !== false ) ) {
								$vs_color = caProcessTemplateForIDs( $pa_options['color'], $vs_table, [ $vn_id ],
									[ 'returnAsLink' => false ] );
							}
								...
								if ( ! is_null( $pa_options['contentView'] ) && $pa_options['request'] ) {
									$o_view = new View( $pa_options['request'],
										( isset( $pa_options['viewPath'] ) && $pa_options['viewPath'] )
											? $pa_options['viewPath']
											: $pa_options['request']->getViewsDirectoryPath() );
									...
									if ( ! is_null( $pa_options['contentTemplate'] ) ) {
										$vs_content = caProcessTemplateForIDs( $pa_options['contentTemplate'],
											$vs_table, [ $vn_id ], [] );
									} else {
							...
							foreach ( $va_path_items as $vs_path_item ) {
								...
								if ( sizeof( $va_path ) > 1 ) {
									...
									$this->addMapItem( new GeographicMapItem( [
										'coordinates'    => $va_coordinate_pairs,
										'label'          => $vs_label,
										'content'        => $vs_content,
										'ajaxContentUrl' => $vs_ajax_content,
										'ajaxContentID'  => $vn_id,
										'color'          => $vs_color
									] ) );
								} else {
	 ...
	 *
	 * @param $ps_format  - the format in which to render the map. Use 'HTML' for html output (no other formats are
	 *                    currently supported)
	 * @param $pa_options - optional array of options, passed through to the render() method of the underlying mapping
	 *                    plugin. Options support will depend upon the plugin.
	 *
