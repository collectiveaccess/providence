        ...
        if (is_subclass_of($po_data_object, 'SearchResult')) {
            ...
            if ($is_related && is_array(
                ...
                while ($po_data_object->nextHit()) {
                    if (is_array(
                        $rel_ids_for_row = $po_data_object->get($path[1] . ".relation_id", ['returnAsArray' => true])
                    )) {
            ...
            while ($po_data_object->nextHit()) {
                if (is_array($va_access_values) && !in_array(
                        $po_data_object->get($chk_related_for_access ? $chk_related_for_access : "{$vs_table}.access"),
                        $va_access_values
                    )) {
                    continue;
                }
