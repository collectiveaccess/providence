...
class GeographicMap
{
    ...
    public function mapFrom($po_data_object, $ps_georeference_field_name, $pa_options = null)
    {
        ...
        if (is_subclass_of($po_data_object, 'BaseModel')) {
            if ($po_data_object->hasField('access') && isset($pa_options['checkAccess']) && is_array(
            ...
            if (is_array(
                $va_coordinates = $po_data_object->get(
                    $ps_georeference_field_name,
                ...
                foreach ($va_coordinates as $vn_i => $va_coord_list) {
                    foreach ($va_coord_list as $vn_attribute_id => $va_geoname) {
                        ...
                        if (!$va_coordinate['latitude'] || !$va_coordinate['longitude']) {
                            continue;
                        }
                        ...
                        if (!is_null($pa_options['labelTemplate'])) {
                            $vs_label = caProcessTemplateForIDs(
                                $pa_options['labelTemplate'],
                                $po_data_object->tableName(),
                                array($vn_id),
                                array(
                                    'returnAsLink' => $vb_render_label_as_link || (strpos(
                                                $pa_options['contentTemplate'],
                                                "<l>"
                                            ) !== false)
                                )
                            );
                        ...
                        if (!is_null($vs_color) && $vs_color && (strpos($vs_color, '^') !== false)) {
                            $vs_color = caProcessTemplateForIDs(
                                $vs_color,
                                $po_data_object->tableName(),
                                array($vn_id),
                                array('returnAsLink' => false)
                            );
                        }
                        ...
                        if (isset($pa_options['ajaxContentUrl']) && $pa_options['ajaxContentUrl']) {
                            ...
                            if (!is_null($pa_options['contentView']) && $pa_options['request']) {
                                ...
                                if (!is_null($pa_options['contentTemplate'])) {
                                    $vs_content = caProcessTemplateForIDs(
                                        $pa_options['contentTemplate'],
                                        $po_data_object->tableName(),
                                        $po_data_object->get(
                                            $po_data_object->tableName() . "." . $po_data_object->primaryKey(),
                        ...
                        foreach ($va_path_items as $vs_path_item) {
                            ...
                            if (sizeof($va_path) > 1) {
                                ...
                                $this->addMapItem(
                                    new GeographicMapItem(
                                        array(
                                            'latitude' => $va_coordinate['latitude'],
                                            'longitude' => $va_coordinate['longitude'],
                                            'label' => $vs_label,
                                            'content' => $vs_content,
                                            'ajaxContentUrl' => $vs_ajax_content,
                                            'ajaxContentID' => $vn_id,
        ...
        if (is_subclass_of($po_data_object, 'SearchResult')) {
            ...
            while ($po_data_object->nextHit()) {
                ...
                if ($va_coordinates = $po_data_object->get(
                    $ps_georeference_field_name,
                    array(
                        'coordinates' => true,
