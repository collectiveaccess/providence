...
class RequestDispatcher extends BaseObject {
	...
	private function parseRequest() {
		...
		$this->request->setControllerUrl( preg_replace( "![/]+!", "/", join( '/',
			array_merge( array( $this->request->getBaseUrlPath(), $this->request->getScriptName() ),
				array( $this->request->getModulePath() ), array( $this->ops_controller ) ) ) ) );
		...
		if ( $this->isDispatchable() ) {
			do {
				...
				if ( ! defined( '__CA_THEME_DIR__' )
				     || ! file_exists( __CA_THEME_DIR__ . '/controllers/' . join( '/', $this->opa_module_path ) . '/'
				                       . $vs_classname . '.php' )
				     || ! include_once( __CA_THEME_DIR__ . '/controllers/' . join( '/', $this->opa_module_path ) . '/'
				                        . $vs_classname . '.php' )
				) {
					// then check controllers directory...
					if ( ! file_exists( $this->ops_controller_path . '/' . join( '/', $this->opa_module_path ) . '/'
					                    . $vs_classname . '.php' )
					     || ! include_once( $this->ops_controller_path . '/' . join( '/', $this->opa_module_path ) . '/'
					                        . $vs_classname . '.php' )
					) {
						// ... next check theme plugins
						if ( ! file_exists( $this->ops_theme_plugins_path . '/' . join( '/', $this->opa_module_path )
						                    . '/' . $vs_classname . '.php' )
						     || ! include_once( $this->ops_theme_plugins_path . '/' . join( '/',
									$this->opa_module_path ) . '/' . $vs_classname . '.php' )
						) {
							// ... next check application plugins
							if ( ! file_exists( $this->ops_application_plugins_path . '/' . join( '/',
										$this->opa_module_path ) . '/' . $vs_classname . '.php' )
							     || ! include_once( $this->ops_application_plugins_path . '/' . join( '/',
										$this->opa_module_path ) . '/' . $vs_classname . '.php' )
							) {

								// ... next check the generic "_root_" plugin directory
								// plugins in here act as if they are in the app/controllers directory
								if ( ! file_exists( $this->ops_application_plugins_path . '/_root_/controllers/'
								                    . join( '/', $this->opa_module_path ) . '/' . $vs_classname
								                    . '.php' )
								     || ! include_once( $this->ops_application_plugins_path . '/_root_/controllers/'
								                        . join( '/', $this->opa_module_path ) . '/' . $vs_classname
								                        . '.php' )
								) {
									...
									foreach ( $va_app_plugin_names as $vs_app_plugin_name ) {
										...
										if ( file_exists( $this->ops_application_plugins_path . '/'
										                  . $vs_app_plugin_name . '/controllers/_root_/' . $vs_classname
										                  . '.php' )
										     && include_once( $this->ops_application_plugins_path . '/'
										                      . $vs_app_plugin_name . '/controllers/_root_/'
										                      . $vs_classname . '.php' )
										) {
											$vb_is_error = false;
										}
									...
									if ( $vb_is_error ) {
										...
										if ( file_exists( $this->ops_controller_path . '/DefaultController.php' )
										     && @include_once( $this->ops_controller_path . '/DefaultController.php' )
										) {
											...
											$this->postError( 2300, _t( "Invalid controller path" ),
				...
				if ( ! $this->request->user->canAccess( $this->opa_module_path, $this->ops_controller,
					$this->ops_action )
				) {
					switch ( $this->request->getScriptName() ) {
						case "service.php":
							...
							if ( ! $this->request->isServiceAuthRequest() ) {
								...
								$this->response->addHeader( 'WWW-Authenticate',
				...
				$o_action_controller = new $vs_classname( $this->request, $this->response,
					$this->request->getViewsDirectoryPath() . '/' . join( '/', $this->opa_module_path ) );
				...
				if ( ! $this->response->isRedirect() ) {
					...
					if ( ! $vb_plugin_cancelled_dispatch ) {
						if ( ! $this->ops_action
						     || ! ( method_exists( $o_action_controller, $this->ops_action )
						            || method_exists( $o_action_controller, '__call' ) )
						) {
