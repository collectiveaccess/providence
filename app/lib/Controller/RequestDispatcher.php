        ...
        $this->request->setControllerUrl(
            preg_replace(
                ...
                join(
                    '/',
                    array_merge(
                        array($this->request->getBaseUrlPath(), $this->request->getScriptName()),
                        array($this->request->getModulePath()),
    ...
    public function dispatch($pa_plugins)
    {
        ...
        if ($this->isDispatchable()) {
            do {
                ...
                if (!$this->request->user->canAccess(
                    $this->opa_module_path,
                    $this->ops_controller,
                    $this->ops_action
                )) {
                ...
                $o_action_controller = new $vs_classname(
                    $this->request,
                    $this->response,
                    $this->request->getViewsDirectoryPath() . '/' . join(
                        '/',
                        $this->opa_module_path
                    )
                );
