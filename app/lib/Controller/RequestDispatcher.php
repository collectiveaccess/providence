...
class RequestDispatcher extends BaseObject
{
    ...
    private function parseRequest()
    {
        ...
        $this->opo_request->setControllerUrl(
            preg_replace(
                "![/]+!",
                "/",
                join(
                    '/',
                    array_merge(
                        array($this->opo_request->getBaseUrlPath(), $this->opo_request->getScriptName()),
                        array($this->opo_request->getModulePath()),
                        array($this->ops_controller)
                    )
                )
            )
        );
    ...
    public function dispatch($pa_plugins)
    {
        ...
        if ($this->isDispatchable()) {
            do {
                ...
                if (!defined('__CA_THEME_DIR__') || !file_exists(
                        __CA_THEME_DIR__ . '/controllers/' . join(
                            '/',
                            $this->opa_module_path
                        ) . '/' . $vs_classname . '.php'
                    ) || !include_once(__CA_THEME_DIR__ . '/controllers/' . join(
                            '/',
                            $this->opa_module_path
                        ) . '/' . $vs_classname . '.php')) {
                    // then check controllers directory...
                    if (!file_exists(
                            $this->ops_controller_path . '/' . join(
                                '/',
                                $this->opa_module_path
                            ) . '/' . $vs_classname . '.php'
                        ) || !include_once($this->ops_controller_path . '/' . join(
                                '/',
                                $this->opa_module_path
                            ) . '/' . $vs_classname . '.php')) {
                        // ... next check theme plugins
                        if (!file_exists(
                                $this->ops_theme_plugins_path . '/' . join(
                                    '/',
                                    $this->opa_module_path
                                ) . '/' . $vs_classname . '.php'
                            ...
                            if (!file_exists(
                                    $this->ops_application_plugins_path . '/' . join(
                                        '/',
                                        $this->opa_module_path
                                    ) . '/' . $vs_classname . '.php'
                                ...
                                if (!file_exists(
                                        $this->ops_application_plugins_path . '/_root_/controllers/' . join(
                                            '/',
                                            $this->opa_module_path
                                        ) . '/' . $vs_classname . '.php'
                                    ) || !include_once($this->ops_application_plugins_path . '/_root_/controllers/' . join(
                                            '/',
                                            $this->opa_module_path
                                        ) . '/' . $vs_classname . '.php')) {
                                    ...
                                    foreach ($va_app_plugin_names as $vs_app_plugin_name) {
                                        ...
                                        if (file_exists(
                                                $this->ops_application_plugins_path . '/' . $vs_app_plugin_name . '/controllers/_root_/' . $vs_classname . '.php'
                                            ) && include_once($this->ops_application_plugins_path . '/' . $vs_app_plugin_name . '/controllers/_root_/' . $vs_classname . '.php')) {
                                            $vb_is_error = false;
                                        }
                                    }
                                    ...
                                    if ($vb_is_error) {
                                        ...
                                        if (file_exists(
                                                $this->ops_controller_path . '/DefaultController.php'
                ...
                if (!$this->opo_request->user->canAccess(
                    $this->opa_module_path,
                    $this->ops_controller,
                    $this->ops_action
                )) {
                ...
                $o_action_controller = new $vs_classname(
                    $this->opo_request,
                    $this->opo_response,
                    $this->opo_request->getViewsDirectoryPath() . '/' . join(
                        '/',
                        $this->opa_module_path
                    )
                );
                ...
                if (!$this->opo_response->isRedirect()) {
                    ...
                    if (!$vb_plugin_cancelled_dispatch) {
                        if (!$this->ops_action || !(method_exists(
                                    $o_action_controller,
                                    $this->ops_action
