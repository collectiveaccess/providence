...
class ActionController extends BaseObject {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		$this->opa_view_paths           = is_array( $pa_view_paths ) ? $pa_view_paths
			: ( $pa_view_paths ? array( $pa_view_paths ) : array() );
	...
	public function __call( $ps_methodname, $pa_params ) {
		...
		if ( file_exists( __CA_APP_DIR__ . "/controllers/DefaultController.php" ) ) {
			...
			$o_default_controller = new DefaultController( $this->opo_request, $this->opo_response,
				$this->opa_view_paths );
		...
		$this->postError( 2310, _t( "Action '%1' in class '%2' is invalid", $ps_methodname, get_class( $this ) ),
