...
/**
 *
 * Returns number of seconds since session object was instantiated. This is very, very close to the total time the
 * request has been executing. You can use this number as a measure of performance.
 *
 * @access public
...
class Request {
	...
	/**
	 *
	 * Creates a new Auth object. The Auth object marshals the various application session and user authentication
	 * objects into once easy-to-handle package.
	 *
	 * By default, a new Auth object looks for login credentials (existing or incoming). If there aren't any, it checks
	 * the requests client IP address against the ip-authentication database (the Ip object and underlying weblib_ips
	 * table). If there is no match, the Auth object then directs the user to the application login page, as configured
	 * in the main application configuration file. If there is a match, the user is redirected to the application
	 * "welcome" page, as configured.
	 *
	 * You may override the default behavior by passing parameters to the constructor. You may do so in two ways:
	 * (1) Pass individual function parameters or (2) Pass an associative array as the first (any only) parameter. The
	 * array may define any of the following keys:
	 *
	 *    - user_name                    [string]
	 ...
	 *
	 *  - no_headers                [boolean]
	 *        (if set, no HTTP headers will be emitted under any circumstances. This is equivalent to setting
	 *        "dont_redirect_to_login",
	 *         "dont_create_new_session" and "dont_redirect_to_welcome")
	 *
	 *  - no_authentication            [boolean]
	 *        (if set, authentication is not performed by the constructor and must be triggered manually by calling
	 *        doAuthentication())
	 *
	 *  - dont_add_default_handlers    [boolean]
	 *        (if set, the default user auth handlers are added to the new Auth object; the default user auth handler
	 *        uses the weblib_user table and PHPWeblib User class)
	 *
	 *    - options                    [indexed array of strings and/or numbers]
	 *        (optional associative array of authentication option information to be passed to auth handlers. Options
	 *        can be used by the handler in its authentication decision. Note that the default handlers do not support
	 *        any options.)
	 *
	 * The keys are analogous to their like-named counterpart function parameters. The array syntax is preferable for
	 * readability and is required for those keys without a corresponding parameter.
	 *
	 * Because under normal conditions Auth may emit redirect headers and/or set a cookie, you must instantiate your
	 * Auth object *before* outputting anything to the browser. If anything at all is output to the browser, redirects
	 * will fail and error messages will be generated. If for some reason you need to instantiate Auth after output has
	 * started, you must pass "no_headers" as true to disable all redirects and cookie manipulation. Remember, if you
	 * use "no_headers", users will not be redirected to the login screen (if they are not authenticated) or the
	 * welcome screen (if they are authenticated), nor will new sessions be created for users who lack one. In other
	 * words, only use "no_headers" if you know what you're doing.
	 *
	 * When a user is currently logged in, Auth defines a global variable, $AUTH_CURRENT_USER_ID, that contains - you
	 * guessed it - the user_id of the currently logged in user. You can use this variable to get at the user_id in
	 * those odd situations where there is no other clean and easy way to access the Auth object.
	 *
	 * @access public
	 *
	 ...
	 * @param bool If true, never redirect user to login screen, even if no user is logged in, or login attempt fails.
	 *                Default is false.
	 ...
	 *
	 *  - pInteger    =    Integer parameter. If parameter input is numeric, getParameter() will force it to a legal
	 *  integer value using the PHP intval() function. Non-numerics will cause a fatal error.
	 *    - pFloat    =    Floating point parameter. If parameter input is numeric, it is passed unchanged.
	 *    Non-numerics will cause a fatal error.
	...
	public function getActionErrors( $ps_source = null, $ps_subsource = null ) {
		if ( is_null( $ps_source ) ) {
			...
			if ( $ps_subsource ) {
				return isset( $this->opa_action_errors[ $ps_source ][ $ps_subsource ] )
					? $this->opa_action_errors[ $ps_source ][ $ps_subsource ] : null;
			}

			return $this->_unrollActionErrors( isset( $this->opa_action_errors[ $ps_source ] )
				? $this->opa_action_errors[ $ps_source ] : null );
		}
	}
	...
	public function getActionErrorSubSources( $ps_source ) {
		if ( ! isset( $this->opa_action_errors[ $ps_source ] )
		     || ! is_array( $this->opa_action_errors[ $ps_source ] )
		) {
			return array();
		}
	...
	public function numActionErrors( $ps_source = null, $ps_subsource = null ) {
		if ( is_null( $ps_source ) ) {
			...
			if ( $ps_subsource ) {
				return isset( $this->opa_action_errors[ $ps_source ][ $ps_subsource ] )
					? sizeof( $this->opa_action_errors[ $ps_source ][ $ps_subsource ] ) : 0;
			}

			return isset( $this->opa_action_errors[ $ps_source ] ) ? sizeof( $this->opa_action_errors[ $ps_source ] )
				: 0;
		}
	}
	# ----------------------------------------
