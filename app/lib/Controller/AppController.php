...
class AppController {
	...
	public function __construct( &$po_request = null, &$po_response = null ) {
		if ( ! isset( $GLOBALS ) || ! $GLOBALS['AppController'] ) {
			...
			if ( $po_request ) {
				...
				$this->opo_request = new RequestHTTP( $this->opo_response,
					array( 'dont_redirect' => true, 'no_authentication' => true ) );
			}
	...
	public function dispatch( $pb_dont_send_response = false ) {
		...
		if ( ! $this->opo_dispatcher->dispatch( $this->getPlugins() ) ) {
			...
			$this->opo_response->setRedirect( $this->opo_request->config->get( 'error_display_url' ) . '/n/'
			                                  . join( ';', $va_errors ) . '?r='
			                                  . urlencode( $this->opo_request->getFullUrlPath() ) );
		}
		foreach ( $this->getPlugins() as $vo_plugin ) {
