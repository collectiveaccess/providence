...
class Session
{
    ...
    public static function init($ps_app_name = null, $pb_dont_create_new_session = false)
    {
        ...
        if (!$pb_dont_create_new_session) {
            ...
            if (!($session_id = self::getSessionID())) {
                ...
                if (!caIsRunFromCLI()) {
                    setcookie(
                        ...
                        Session::$lifetime ? time() + Session::$lifetime : null,
                        $vs_cookiepath
                    );
                }
            }
            ...
            if ($session_id && ExternalCache::contains($session_id, 'SessionVars')) {
                if (!is_array(
                    Session::$s_session_vars = unserialize(
                        gzuncompress(ExternalCache::fetch($session_id, 'SessionVars'))
                    )
                )) {
            ...
            if (
                ...
                ||
                (is_numeric(Session::$s_session_vars['session_end_timestamp']) && (time(
                        ) > Session::$s_session_vars['session_end_timestamp']))
            ) {
    ...
    public static function save()
    {
        ...
        ExternalCache::save(
            $session_id,
            @gzcompress(@serialize($va_current_values)),
            'SessionVars',
            $vn_session_lifetime
        );
    }
