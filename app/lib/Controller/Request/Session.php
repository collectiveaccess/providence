...
class Session {
	...
	# ----------------------------------------
	/**
	 * @param string $ps_app_name                An app name to use if no app name is configured in the application
	 *                                           configuration file.
	...
	public static function init( $ps_app_name = null, $pb_dont_create_new_session = false ) {
		...
		if ( ! $pb_dont_create_new_session ) {
			...
			if ( ! ( $session_id = self::getSessionID() ) ) {
				...
				if ( ! caIsRunFromCLI() ) {
					setcookie( Session::$name, $_COOKIE[ Session::$name ] = $session_id = caGenerateGUID(),
						Session::$lifetime ? time() + Session::$lifetime : null, $vs_cookiepath, null, $secure, true );
				}
			}
			...
			if ( $session_id && ExternalCache::contains( $session_id, 'SessionVars' ) ) {
				if ( ! is_array(
					Session::$s_session_vars = unserialize( gzuncompress( ExternalCache::fetch( $session_id,
						'SessionVars' ) ) ) )
				) {
			...
			if (
				...
				|| ( is_numeric( Session::$s_session_vars['session_end_timestamp'] )
				     && ( time() > Session::$s_session_vars['session_end_timestamp'] ) )
			) {
	...
	public static function save() {
		...
		// Get old vars
		if ( ! is_array(
			$va_current_values = @unserialize( @gzuncompress( ExternalCache::fetch( $session_id, 'SessionVars' ) ) ) )
		) {
			$va_current_values = [];
		}
		...
		ExternalCache::save( $session_id, @gzcompress( @serialize( $va_current_values ) ), 'SessionVars',
			$vn_session_lifetime );
	}
	# ----------------------------------------
	# --- Page performance
