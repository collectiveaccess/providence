...
class RequestHTTP extends Request {
	...
	public function __construct( $po_response, $pa_options = null ) {
		...
		$va_tmp                = ( isset( $_SERVER['SCRIPT_NAME'] ) && $_SERVER['SCRIPT_NAME'] ) ? explode( '/',
			$_SERVER['SCRIPT_NAME'] ) : array();
		...
		if ( ( $this->ops_script_name == "service.php" ) && isset( $_GET['authToken'] )
		     && ( strlen( $_GET['authToken'] ) > 0 )
		) {
		...
		// plain old session
		if ( ! $this->session_id ) {
			$this->session_id = Session::init( $vs_app_name,
				isset( $pa_options["dont_create_new_session"] ) ? $pa_options["dont_create_new_session"] : false );
		}
		...
		if ( ! caUseCleanUrls() && ! preg_match( "!/index.php!", $this->ops_full_path )
		     && ! preg_match( "!/service.php!", $this->ops_full_path )
		) {
			$this->ops_full_path = rtrim( $this->ops_full_path, "/" ) . "/index.php";
		}
		$this->ops_full_path = preg_replace( "![/]+!", "/", $this->ops_full_path );
		$vs_path_info        = str_replace( $_SERVER['SCRIPT_NAME'], "",
			str_replace( "?" . $_SERVER['QUERY_STRING'], "", $this->ops_full_path ) );

		$this->ops_path_info = $vs_path_info ? $vs_path_info
	...
	/**
	 * Returns url path to "default" theme. This is not (necessarily) the same as the configured theme for the
	 * installation that is returned when calling getThemeUrlPath() with the $pb_use_default parameter set. The path
	 * returned by this method is the url path to the base theme named "default"
	 */
	 ...
	 *
	 * @param bool $pb_use_default Return path in default theme no matter what theme user has selected. [Default is
	 *                             false]
	 *
	 ...
	 *
	 * @param string $ps_relative_file_path Path to theme file relative to the theme root directory. To find the file
	 *                                      path of the base.css file "css/base.css" would be passed.
	 *
	 ...
	 * by the default theme, ceasing when the file is found.
	 *
	 * @param string $ps_relative_file_path Path to theme file relative to the theme root directory. To find the URL
	 *                                      path of the base.css file "css/base.css" would be passed.
	 *
	 * @return URL to file
	 */
	...
	static public function getPurifier() {
		if ( ! RequestHTTP::$html_purifier ) {
			RequestHTTP::$html_purifier = new HTMLPurifier();
		}
		...
		foreach ( $pa_name as $ps_name ) {
			if ( in_array( $ps_http_method, array( 'GET', 'POST', 'COOKIE', 'PATH', 'REQUEST' ) ) ) {
				...
				foreach ( array( 'GET', 'POST', 'PATH', 'COOKIE', 'REQUEST' ) as $http_method ) {
					$vm_val = ( array_key_exists( $http_method, $this->opa_params )
					            && array_key_exists( $ps_name, $this->opa_params[ $http_method ] ) )
						? $this->opa_params[ $http_method ][ $ps_name ] : null;
					if ( isset( $vm_val ) ) {
						break;
					}
				}
	...
	public function getParameter( $pa_name, $pn_type, $ps_http_method = null, $pa_options = array() ) {
		...
		if ( ( caGetOption( 'purify', $pa_options, true ) && $this->config->get( 'purify_all_text_input' ) )
		     || caGetOption( 'forcePurify', $pa_options, false )
		) {
			if ( is_array( $vm_val ) ) {
				$vm_val = array_map( function ( $v ) {
					return is_array( $v ) ? $v
						: str_replace( "&amp;", "&", RequestHTTP::getPurifier()->purify( rawurldecode( $v ) ) );
				}, $vm_val );
			} else {
			...
			case pString:
				if ( is_string( $vm_val ) ) {
					if ( caGetOption( 'retainBackslashes', $pa_options, true ) ) {
						$vm_val = str_replace( "\\", "\\\\",
							$vm_val );    // retain backslashes for some strange people desire them as valid input
					}
	...
	function close() {
		...
		if ( defined( '__CA_SITE_HOSTNAME__' ) && strlen( __CA_SITE_HOSTNAME__ ) > 0 ) {
			...
			if (
				! ( $indexing_hostname = trim( $this->getAppConfig()
				                                    ->get( 'out_of_process_search_indexing_hostname' ) ) )
				&&
			...
			if ( ( __CA_APP_TYPE__ === 'PROVIDENCE' )
			     && ! $this->getAppConfig()->get( 'disable_out_of_process_search_indexing' )
			) {
				require_once( __CA_MODELS_DIR__ . "/ca_search_indexing_queue.php" );
				if ( ! ca_search_indexing_queue::lockExists() ) {
					$dont_verify_ssl_cert = (bool) $this->getAppConfig()
					                                    ->get( 'out_of_process_search_indexing_dont_verify_ssl_cert' );
					...
					$r_socket = stream_socket_client( $proto . '://' . $indexing_hostname . ':' . $port, $errno,
						$errstr, ini_get( "default_socket_timeout" ), STREAM_CLIENT_CONNECT, $context );
	 ...
	 * the time if you need custom authentication you can just create a custom user auth handler class
	...
	public function doAuthentication( $pa_options ) {
		...
		foreach (
			array(
				'no_headers',
				'dont_redirect_to_login',
				'dont_create_new_session',
				'dont_redirect_to_welcome',
				'user_name',
				'password',
				'options',
				'noPublicUsers',
		...
		if ( ! $pa_options["user_name"] ) {        // no incoming login
			...
			//
			if ( $vn_user_id = Session::getVar( $vs_app_name
			                                    . "_user_id" )
				...
				if ( ( ! $this->user->isActive() ) || ( $this->user->numErrors() )
				     || ( $pa_options['noPublicUsers']
				          && $this->user->isPublicUser() )
				...
				if ( $vb_login_successful ) {                                                                    // Login was successful
					Session::setVar( $vs_app_name . "_lastping",
						time() );                    // set last time we heard from client in session
			...
			if ( ! $vb_login_successful ) {
				...
				if ( ! AuthenticationManager::supports( __CA_AUTH_ADAPTER_FEATURE_USE_ADAPTER_LOGIN_FORM__ )
				     || $pa_options['allow_external_auth']
				) {
					$vs_tmp1 = $vs_tmp2 = null;
					if ( ( $vn_auth_type = $this->user->authenticate( $vs_tmp1, $vs_tmp2,
						$pa_options["options"] ) )
					) {    # error means user_id in session is invalid
						if ( ( $pa_options['noPublicUsers'] && $this->user->isPublicUser() )
						     || ! $this->user->isActive()
						) {
							$o_event_log->log( array(
								"CODE"    => "LOGF",
								"SOURCE"  => "Auth",
								"MESSAGE" => "Failed login for user id '" . $vn_user_id . "' ("
								             . $_SERVER['REQUEST_URI'] . "); IP=" . RequestHTTP::ip() . "; user agent='"
					...
					if ( ! $vb_login_successful ) {                                                                    // throw user to login screen
						if ( ! $pa_options["dont_redirect_to_login"] ) {
							$o_event_log->log( array(
								"CODE"    => "LOGF",
								"SOURCE"  => "Auth",
								"MESSAGE" => "Failed login with redirect for user id '" . $vn_user_id . "' ("
								             . $_SERVER['REQUEST_URI'] . "); IP=" . RequestHTTP::ip() . "; user agent='"
								             . $_SERVER["HTTP_USER_AGENT"] . "'"
							) );
							...
							$this->opo_response->addHeader( "Location",
								$this->getBaseUrlPath() . '/' . $this->getScriptName() . '/'
								. $this->config->get( "auth_login_path" ) . $vs_redirect );
						}
					...
					try {
						...
						$o_event_log->log( array(
							"CODE"    => "LOGF",
							"SOURCE"  => "Auth",
							"MESSAGE" => "Failed login with exception '" . $e->getMessage() . " ("
							             . $_SERVER['REQUEST_URI'] . "); IP=" . $_SERVER["REMOTE_ADDR"]
		...
		if ( $pa_options["user_name"] ) {
			...
			if ( ( $vn_auth_type = $this->user->authenticate( $pa_options["user_name"], $pa_options["password"],
				$pa_options["options"] ) )
			) {    # error means user_id in session is invalid
		...
		if ( ! $vb_login_successful ) {
			...
			if ( $pa_options["user_name"] ) {
				$o_event_log->log( array(
					"CODE"    => "LOGF",
					"SOURCE"  => "Auth",
					"MESSAGE" => "Failed login for '" . $pa_options["user_name"] . "' (" . $_SERVER['REQUEST_URI']
					             . "); IP=" . RequestHTTP::ip() . "; user agent='" . $_SERVER["HTTP_USER_AGENT"] . "'"
				) );
			}
			if ( ! $pa_options["dont_redirect_to_login"] ) {
				$vs_auth_login_url = $this->getBaseUrlPath() . '/' . $this->getScriptName() . '/'
				                     . $this->config->get( "auth_login_path" );
				$this->opo_response->addHeader( "Location", $vs_auth_login_url );
			}
			...
			$o_event_log->log( array(
				"CODE"    => "LOGN",
				"SOURCE"  => "Auth",
				"MESSAGE" => "Successful login for '" . $pa_options["user_name"] . "'; IP=" . $_SERVER["REMOTE_ADDR"]
				             . "; user agent=" . RequestHTTP::ip()
			) );

			$this->session_id = Session::init( $vs_app_name,
				isset( $pa_options["dont_create_new_session"] ) ? $pa_options["dont_create_new_session"] : false );

			Session::setVar( $vs_app_name . "_user_auth_type",
				$vn_auth_type );                // type of auth used: 1=username/password; 2=ip-base auth
			Session::setVar( $vs_app_name . "_user_id",
				$vn_user_id );                        // auth succeeded; set user_id in session
			Session::setVar( $vs_app_name . "_logintime",
				time() );                            // also set login time (unix timestamp) in session
			...
			if ( ( ! $pa_options["dont_redirect_to_welcome"] )
			     && ! AuthenticationManager::supports( __CA_AUTH_ADAPTER_FEATURE_USE_ADAPTER_LOGIN_FORM__ )
			) {
				// redirect to "welcome" page
				$this->opo_response->setRedirect( $this->getBaseUrlPath() . '/' . $this->getScriptName() . '/'
				                                  . $this->config->get( "auth_login_welcome_path" ) );
				$this->opo_response->sendResponse();
				exit;
			}
	...
	public function isServiceAuthRequest() {
		...
		$va_action = explode( "#",
			$_SERVER["HTTP_SOAPACTION"] ); // I hope this is set no matter what Soap client you use :-)
