...
class RequestHTTP extends Request
{
    ...
    public function __construct($po_response, $pa_options = null)
    {
        ...
        if (($this->ops_script_name == "service.php") && isset($_GET['authToken']) && (strlen(
                    $_GET['authToken']
                ) > 0)) {
        ...
        if (!caUseCleanUrls() && !preg_match("!/index.php!", $this->ops_full_path) && !preg_match(
                "!/service.php!",
                $this->ops_full_path
            )) {
        ...
        foreach ($pa_name as $ps_name) {
            if (in_array($ps_http_method, array('GET', 'POST', 'COOKIE', 'PATH', 'REQUEST'))) {
                ...
                foreach (array('GET', 'POST', 'PATH', 'COOKIE', 'REQUEST') as $http_method) {
                    $vm_val = (array_key_exists($http_method, $this->opa_params) && array_key_exists(
                            $ps_name,
                            $this->opa_params[$http_method]
    ...
    public function getParameter($pa_name, $pn_type, $ps_http_method = null, $pa_options = array())
    {
        ...
        if ((caGetOption('purify', $pa_options, true) && $this->config->get('purify_all_text_input')) || caGetOption(
                'forcePurify',
                $pa_options,
                false
            )) {
            if (is_array($vm_val)) {
                $vm_val = array_map(
                    function ($v) {
                        return is_array($v) ? $v : str_replace(
                            "&amp;",
                            "&",
                            RequestHTTP::getPurifier()->purify(rawurldecode($v))
                        );
                    },
                    $vm_val
                );
                ...
                if (is_string($vm_val)) {
                    if (caGetOption('retainBackslashes', $pa_options, true)) {
                        $vm_val = str_replace(
                            ...
                            "\\\\",
                            $vm_val
                        );    // retain backslashes for some strange people desire them as valid input
                    }
    ...
    public function close()
    {
        ...
        if (defined('__CA_SITE_HOSTNAME__') && strlen(__CA_SITE_HOSTNAME__) > 0) {
            ...
            if ((__CA_APP_TYPE__ === 'PROVIDENCE') && !$this->getAppConfig()->get(
                    'disable_out_of_process_search_indexing'
                )) {
                require_once(__CA_MODELS_DIR__ . "/ca_search_indexing_queue.php");
                if (!ca_search_indexing_queue::lockExists()) {
                    $dont_verify_ssl_cert = (bool)$this->getAppConfig()->get(
                        'out_of_process_search_indexing_dont_verify_ssl_cert'
                    );
                    ...
                    $r_socket = stream_socket_client(
                        $proto . '://' . $indexing_hostname . ':' . $port,
                        $errno,
                        $errstr,
     ...
     *
     * @access public
     * @return int User_id of currently logged in user or null if user is not logged in.
     */
    ...
    public function doAuthentication($pa_options)
    {
        ...
        if (!$pa_options["user_name"]) {        // no incoming login
            ...
            //
            if ($vn_user_id = Session::getVar(
                $vs_app_name . "_user_id"
                ...
                if ((!$this->user->isActive()) || ($this->user->numErrors(
                    )) || ($pa_options['noPublicUsers'] && $this->user->isPublicUser(
            ...
            if (!$vb_login_successful) {
                ...
                if (!AuthenticationManager::supports(
                        __CA_AUTH_ADAPTER_FEATURE_USE_ADAPTER_LOGIN_FORM__
                    ) || $pa_options['allow_external_auth']) {
                    $vs_tmp1 = $vs_tmp2 = null;
                    if (($vn_auth_type = $this->user->authenticate(
                        $vs_tmp1,
                        $vs_tmp2,
                        $pa_options["options"]
                    ))) {    # error means user_id in session is invalid
                        if (($pa_options['noPublicUsers'] && $this->user->isPublicUser()) || !$this->user->isActive()) {
                            $o_event_log->log(
                                array(
                                    "CODE" => "LOGF",
                                    "SOURCE" => "Auth",
                                    "MESSAGE" => "Failed login for user id '" . $vn_user_id . "' (" . $_SERVER['REQUEST_URI'] . "); IP=" . RequestHTTP::ip(
                        ...
                        if (!$pa_options["dont_redirect_to_login"]) {
                            $o_event_log->log(
                                array(
                                    "CODE" => "LOGF",
                                    "SOURCE" => "Auth",
                                    "MESSAGE" => "Failed login with redirect for user id '" . $vn_user_id . "' (" . $_SERVER['REQUEST_URI'] . "); IP=" . RequestHTTP::ip(
                                        ) . "; user agent='" . $_SERVER["HTTP_USER_AGENT"] . "'"
                                )
                            );
                            ...
                            $this->opo_response->addHeader(
                                "Location",
                                $this->getBaseUrlPath() . '/' . $this->getScriptName() . '/' . $this->config->get(
                                    "auth_login_path"
                                ) . $vs_redirect
                            );
                        }
                        ...
                        $o_event_log->log(
                            array(
                                ...
                                "MESSAGE" => "Failed login with exception '" . $e->getMessage(
                                    ) . " (" . $_SERVER['REQUEST_URI'] . "); IP=" . $_SERVER["REMOTE_ADDR"] . "; user agent='" . $_SERVER["HTTP_USER_AGENT"] . "'"
                            )
                        );
        ...
        if (!$vb_login_successful) {
            ...
            if ($pa_options["user_name"]) {
                $o_event_log->log(
                    array(
                        "CODE" => "LOGF",
                        "SOURCE" => "Auth",
                        "MESSAGE" => "Failed login for '" . $pa_options["user_name"] . "' (" . $_SERVER['REQUEST_URI'] . "); IP=" . RequestHTTP::ip(
                            ) . "; user agent='" . $_SERVER["HTTP_USER_AGENT"] . "'"
                    )
                );
            }
            if (!$pa_options["dont_redirect_to_login"]) {
                $vs_auth_login_url = $this->getBaseUrlPath() . '/' . $this->getScriptName() . '/' . $this->config->get(
                        "auth_login_path"
                    );
                $this->opo_response->addHeader("Location", $vs_auth_login_url);
            }
            ...
            $o_event_log->log(
                array(
                    "CODE" => "LOGN",
                    "SOURCE" => "Auth",
                    "MESSAGE" => "Successful login for '" . $pa_options["user_name"] . "'; IP=" . $_SERVER["REMOTE_ADDR"] . "; user agent=" . RequestHTTP::ip(
                        )
                )
            );
            ...
            if ((!$pa_options["dont_redirect_to_welcome"]) && !AuthenticationManager::supports(
                    __CA_AUTH_ADAPTER_FEATURE_USE_ADAPTER_LOGIN_FORM__
                )) {
                // redirect to "welcome" page
                $this->opo_response->setRedirect(
                    $this->getBaseUrlPath() . '/' . $this->getScriptName() . '/' . $this->config->get(
                        "auth_login_welcome_path"
                    )
                );
                $this->opo_response->sendResponse();
                exit;
    ...
    public function isServiceAuthRequest()
    {
        ...
        ); // I hope this is set no matter what Soap client you use :-)
