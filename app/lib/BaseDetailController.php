...
class BaseDetailController extends ActionController
{
    ...
    public function Show()
    {
        ...
        //
        if ($this->request->config->get('allow_browse_within_detail_for_' . $this->ops_tablename) && is_object(
                $this->opo_browse
            )) {
            ...
            if (Session::getVar(
                    $this->ops_tablename . '_' . $this->ops_appname . '_detail_current_item_id'
                ) != $vn_item_id) {
        ...
        $opo_result_context = new ResultContext(
            $this->request,
            $this->ops_tablename,
        ...
        if (method_exists($t_item, 'getPrimaryRepresentationInstance')) {
            if ($t_primary_rep = $t_item->getPrimaryRepresentationInstance()) {
                if (!sizeof($va_access_values) || in_array(
                        $t_primary_rep->get('access'),
                        $va_access_values
                    )) {        // check rep access
                    ...
                    $va_rep_display_info['poster_frame_url'] = $t_primary_rep->getMediaUrl(
                        'media',
        ...
        #
        $this->view->setVar(
            't_relationship_types',
            new ca_relationship_types()
...
//  			}
        ...
        if ($ps_comment || $pn_rank || $ps_tags) {
            ...
            if ($ps_comment || $ps_tags) {
                $this->notification->addNotification(
                    _t(
                        "Thank you for contributing.  Your comments will be posted on this page after review by site staff."
                    ),
                    "message"
                );
            } else {
    ...
    public function getFacet()
    {
        ...
        if ($this->request->getParameter('clear', pInteger)) {
            ...
            $this->opo_browse->execute(array('checkAccess' => $va_access_values));
            Session::setVar(
                $this->ops_tablename . '_' . $this->ops_context . '_current_browse_id',
                $this->opo_browse->getBrowseID()
            );
        } else {
        ...
        $this->view->setVar(
            'other_parameters',
            array(
                $t_table->primaryKey() => $this->request->getParameter(
                    $t_table->primaryKey(),
                    pInteger
                )
            )
