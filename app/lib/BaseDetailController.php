...
class BaseDetailController extends ActionController {
	 ...
	 *
	 * So for example, the detail for objects of type 'artwork' (where 'artwork' is the type code for the artwork
	 * object type) the view would be named "ca_objects_artwork_detail_html.php
	 *
	 * If the type specific view does not exist, then Show() will attemp to use a generic table-wide view name like
	 * this:
	...
	public function Show() {
		...
		//
		if ( $this->request->config->get( 'allow_browse_within_detail_for_' . $this->ops_tablename )
		     && is_object( $this->opo_browse )
		) {
			...
			if ( Session::getVar( $this->ops_tablename . '_' . $this->ops_appname . '_detail_current_item_id' )
			     != $vn_item_id
			) {
			...
			if ( $vs_limit_facet_name ) {
				...
				Session::setVar( $this->ops_tablename . '_' . $this->ops_appname . '_detail_current_browse_id',
					$this->opo_browse->getBrowseID() );
				...
				$this->view->setVar( 'sorts',
					$this->opa_sorts );                // supported sorts for the object browse
		...
		$opo_result_context = new ResultContext( $this->request, $this->ops_tablename,
			ResultContext::getLastFind( $this->request, $this->ops_tablename ) );
		...
		if ( method_exists( $t_item, 'getPrimaryRepresentationInstance' ) ) {
			if ( $t_primary_rep = $t_item->getPrimaryRepresentationInstance() ) {
				if ( ! sizeof( $va_access_values )
				     || in_array( $t_primary_rep->get( 'access' ), $va_access_values )
				) {        // check rep access
					$va_info = $t_primary_rep->getMediaInfo( 'media', 'original' );
					$this->view->setVar( 't_primary_rep', $t_primary_rep );

					$va_rep_display_info = caGetMediaDisplayInfo( 'detail',
						$t_primary_rep->getMediaInfo( 'media', 'original', 'MIMETYPE' ) );

					$this->view->setVar( 'primary_rep_display_version', $va_rep_display_info['display_version'] );
					unset( $va_display_info['display_version'] );
					$va_rep_display_info['poster_frame_url'] = $t_primary_rep->getMediaUrl( 'media',
		...
		$this->view->setVar( 'ranking',
			$t_item->getAverageRating( null ) );    // null makes it ignore moderation status
		# -- get number of user rankings
		$this->view->setVar( 'numRankings',
			$t_item->getNumRatings( null ) );    // null makes it ignore moderation status
		...
		#
		$this->view->setVar( 't_relationship_types',
			new ca_relationship_types() );                    // relationship types object - used for displaying relationship type of related authority information
...
//  			}
		...
		// Render view
		//
		if ( $this->getView()->viewExists( $this->ops_tablename . '_' . $t_item->getTypeCode()
		                                   . '_detail_html.php' )
		) {
	...
	public function saveCommentRanking() {
		...
		if ( $ps_comment || $pn_rank || $ps_tags ) {
			...
			if ( $ps_comment || $ps_tags ) {
				$this->notification->addNotification( _t( "Thank you for contributing.  Your comments will be posted on this page after review by site staff." ),
					"message" );
			} else {
	...
	public function getFacet() {
		...
		if ( $this->request->getParameter( 'clear', pInteger ) ) {
			$this->opo_browse->removeAllCriteria();
			$this->opo_browse->execute( array( 'checkAccess' => $va_access_values ) );
			Session::setVar( $this->ops_tablename . '_' . $this->ops_context . '_current_browse_id',
				$this->opo_browse->getBrowseID() );
		} else {
		...
		$t_table = Datamodel::getInstance( $this->ops_tablename );
		$this->view->setVar( 'other_parameters',
			array( $t_table->primaryKey() => $this->request->getParameter( $t_table->primaryKey(), pInteger ) ) );
