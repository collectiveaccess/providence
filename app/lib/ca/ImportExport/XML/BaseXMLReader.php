<?php
/** ---------------------------------------------------------------------
 * app/lib/ca/ImportExport/XML/BaseXMLReader.php :
 * ----------------------------------------------------------------------
 * CollectiveAccess
  ...
  */
	...
	class BaseXMLReader {
		...
		public function read($ps_base_url, $po_processor, $pa_mappings_by_group, $po_instance, $pa_options) {
			...
			$this->opo_data_import_event->set('type_code', 'XML?');
			$this->opo_data_import_event->set('source', $ps_base_url);
			
			$this->opo_data_import_event->insert();
		
			$o_xml_data = simplexml_load_file($this->ops_base_url);
			
			// call superclass record harvest method
			$va_records = $this->parseRecords($o_xml_data);
		
			foreach ($va_records as $va_record) {
				// If returned value is null then we skip the record
				if(is_null($va_record_data = $this->opo_app_plugin_manager->hookXMLPreprocessRecord(array('url' => $this->ops_base_url, 'record' => $va_record, 'metadata_prefix' => $this::METADATA_PREFIX, 'mapping_code' => $pa_options['mapping_code'])))) {
					continue;
				}
				$va_data = $va_record_data['record'];
				$va_errors =  DataImporter::importRecord($va_data, $this->opo_data_import_event, $pa_mappings_by_group, $po_instance, array_merge($pa_options, array('debug' => $this->opb_debug, 'skipElements' => $this->opa_skip_elements)));
				...
				$va_data = $this->opo_app_plugin_manager->hookXMLAfterRecordImport(array('url' => $this->ops_base_url, 'record' => $va_data, 'metadata_prefix' => $this::METADATA_PREFIX, 'mapping_code' => $pa_options['mapping_code']));
			}
		}
		# -------------------------------------------------------
		/** 
