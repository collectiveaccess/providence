	...
	class BatchProcessor {
		...
		public static function importMediaFromDirectory($po_request, $pa_options=null) {
			global $g_ui_locale_id;
 			...
 			$va_files_to_process = caGetDirectoryContentsAsList($pa_options['importFromDirectory'], true);
...
// 										'message' => _t('Matched media %1 to object using %2', $f, $vs_regex_name),
// 										'status' => 'MATCHED'
// 									);
									break(2);
								}
...
// 							'idno' => $t_object->get($t_object->getProperty('ID_NUMBERING_ID_FIELD')),
// 							'label' => $t_object->getLabelForDisplay(),
// 							'message' => _t('Matched media %1 to object using filename', $f),
// 							'status' => 'MATCHED'
// 						);
					}
				}
				...
				if ($t_object->getPrimaryKey()) {
					...
					if ($t_object->numErrors()) {	
						...
						$va_errors[$f] = array(
							...
							'message' => _t("Error importing %1 from %2: %3", $f, $vs_directory, join('; ', $t_object->getErrors())),
							'status' => 'ERROR',
						);
						$o_trans->rollback();
						continue;
					...
					if (in_array($vs_import_mode, array('TRY_TO_MATCH', 'DONT_MATCH'))) {
						...
						if ($t_object->numErrors()) {	
							...
							$va_errors[$f] = array(
								...
								'label' => $t_object->getLabelForDisplay(),
								'errors' => $t_object->errors(),
								'message' => _t("Error creating new object while importing %1 from %2: %3", $f, $vs_directory, join('; ', $t_object->getErrors())),
								'status' => 'ERROR',
							);
							$o_trans->rollback();
							continue;
						...
						if ($t_object->numErrors()) {	
							...
							$va_errors[$f] = array(
								...
								'label' => $t_object->getLabelForDisplay(),
								'errors' => $t_object->errors(),
								'message' => _t("Error creating object label while importing %1 from %2: %3", $f, $vs_directory, join('; ', $t_object->getErrors())),
								'status' => 'ERROR',
							);
							$o_trans->rollback();
							continue;
				...
				if ($t_object->getPrimaryKey()) {
					...
					$va_notices[$vs_f] = array(
						'idno' => '',
						'label' => $f,
						'message' => (($vs_import_mode == 'ALWAYS_MATCH') ? _t('Skipped %1 because it could not be matched', $f) : _t('Skipped %1', $f)),
						'status' => 'SKIPPED'
					);
				}
 				
 				if (isset($pa_options['progressCallback']) && ($ps_callback = $pa_options['progressCallback'])) {
					$ps_callback($po_request, $vn_c, $vn_num_items, _t("[%3/%4] Processing %1 (%2)", caTruncateStringWithEllipsis($f, 50), $t_object->get($t_object->getProperty('ID_NUMBERING_ID_FIELD')), $vn_c, $vn_num_items), $t_new_rep, time() - $vn_start_time, memory_get_usage(true), sizeof($va_notices), sizeof($va_errors));
				}
