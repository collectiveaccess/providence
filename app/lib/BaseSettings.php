...
class BaseSettings {
	...
	public function getSetting( $ps_setting ) {
		...
		$vs_default = isset( $this->opa_settings_defs[ $ps_setting ]['default'] )
			? $this->opa_settings_defs[ $ps_setting ]['default'] : null;
	 ...
	 */
	public function getHTMLSettingFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pn_table_num, $pa_options = null
	) {
	 ...
	 *  'label_id' => sets the id of the label for the setting form element (used to link tools tips to the label); if
	 *  not set then the default is to set it to  'setting_<name_of_setting>_label'
	 */
	public function settingHTMLFormElement( $ps_setting, $pa_options = null ) {
		...
		$vs_return .= '<span id="' . $vs_label_id . '"  class="' . $vs_label_id . '">' . $va_properties['label']
		              . '</span>';


		if ( $vs_help_text = $pa_options['helpText'] ) {
			$vs_return .= "<a href='#' onclick='jQuery(\"#" . str_replace( ".", "_", $vs_label_id )
			              . "_help_text\").slideToggle(250); return false;' class='settingsKeyButton'>" . _t( 'Key' )
			              . "</a>";
		}
		...
		if ( $vs_help_text ) {
			$vs_return .= "\n<div id='" . str_replace( ".", "_", $vs_label_id )
			              . "_help_text' class='settingsKey'>{$vs_help_text}</div>\n";
		}
		...
		switch ( $va_properties['displayType'] ) {
			...
			case DT_FIELD:
				...
				foreach ( $va_locales as $vs_locale => $va_locale_info ) {
					...
					if ( ( $vs_locale != '_generic' )
					     && ( is_array( $vs_value ) )
					) {        // _generic means this setting doesn't take a locale
						if ( ! ( $vs_text_value = $vs_value[ $va_locale_info['locale_id'] ] ) ) {
							$vs_text_value = ( is_array( $vs_value ) && isset( $vs_value[ $va_locale_info['code'] ] ) )
								? $vs_value[ $va_locale_info['code'] ] : '';
						}
					} else {
						$vs_text_value = $vs_value;
					}
					$vs_return .= ( $vs_locale_label ? "{$vs_locale_label}<br/>" : "" )
					              . caHTMLTextInput( $vs_input_name . $vs_input_name_suffix, array(
							'size'   => $va_properties["width"],
							'height' => $va_properties["height"],
							'value'  => $vs_text_value,
							'id'     => $vs_input_id . $vs_input_name_suffix
						) ) . "<br/>\n";
			...
			case DT_CHECKBOXES:
				...
				if ( isset( $va_properties['hideOnSelect'] ) ) {
					...
					$va_attributes['onchange'] = 'jQuery(this).prop("checked") ? jQuery("' . join( ",", $va_ids )
					                             . '").slideUp(250).find("input, textarea").val("") : jQuery("'
					                             . join( ",", $va_ids ) . '").slideDown(250);';
				...
				if ( isset( $va_properties['showOnSelect'] ) ) {
					...
					$va_attributes['onchange'] = 'jQuery(this).prop("checked") ? jQuery("' . join( ",", $va_ids )
					                             . '").slideDown(250).find("input, textarea").val("") : jQuery("'
					                             . join( ",", $va_ids ) . '").slideUp(250);';
		...
		jQuery('" . join( ",", $va_ids ) . "').hide();
			...
			case DT_SELECT:
				...
				if ( $va_properties['showTypesForTable'] ) {
					$vs_select_element = '';
					if ( ! ( $t_show_types_for_table
						= Datamodel::getInstanceByTableName( $va_properties['showTypesForTable'], true ) )
					) {
						break;
					}

					$va_type_opts = [];
					if ( $t_show_types_for_table instanceof
					     BaseRelationshipModel
					) { // interstitial type restriction incoming
						...
						foreach ( $va_rel_type_list as $vn_type_id => $va_type_info ) {
							...
							$va_type_opts[ $va_type_info['typename'] . '/' . $va_type_info['typename_reverse'] ]
								= $va_type_info['type_id'];
						}
					...
					|| ( $vb_locale_list
						= (bool) $va_properties['useLocaleList'] )
					...
					if ( $vs_rel_table ) {
						...
						foreach ( $va_rels as $vn_type_id => $va_rel_type_info ) {
							...
							$va_rel_opts[ $va_rel_type_info['typename'] . '/' . $va_rel_type_info['typename_reverse'] ]
								= $va_rel_type_info['type_id'];
						}
					} else {
						if ( $vb_locale_list ) {
							include_once( __CA_MODELS_DIR__ . '/ca_locales.php' );
							$va_rel_opts
								= array_flip( ca_locales::getLocaleList( array( 'return_display_values' => true ) ) );
						} elseif ( $vb_policy_list ) {
							include_once( __CA_MODELS_DIR__ . '/ca_objects.php' );
							$va_rel_opts = array_flip( array_map( function ( $v ) {
								return $v['name'];
							}, ca_objects::getHistoryTrackingCurrentValuePolicies( caGetOption( 'table', $pa_options,
								null ) ) ) );
						} elseif ( $vb_referring_policy_list ) {
							include_once( __CA_MODELS_DIR__ . '/ca_objects.php' );
							$va_rel_opts = array_flip( array_map( function ( $v ) {
								return $v['name'];
							}, ca_objects::getDependentHistoryTrackingCurrentValuePolicies( caGetOption( 'table',
								$pa_options, null ) ) ) );
						} else {
							if ( $vb_show_lists ) {
								...
								foreach ( $va_lists as $vn_list_id => $va_list_info ) {
									...
									$va_rel_opts[ $va_list_info['name'] . ' (' . $va_list_info['list_code'] . ')' ]
										= $vn_list_id;
								}
							}
						}
					...
					if ( $vs_list_code ) {
						...
						$vs_select_element = $t_list->getListAsHTMLFormElement( $vs_list_code, $vs_input_name, $va_attr,
							$va_opts );
					} else {
						if ( ! isset( $va_opts['value'] ) ) {
							$va_opts['value'] = - 1;
					...
					if ( strlen( $va_properties['showSortableBundlesFor'] ) > 0 ) {
						...
						if ( ! ( $t_rel = Datamodel::getInstanceByTableName( $va_properties['showSortableBundlesFor'],
							true ) )
						) {
							break;
						}
						$va_elements
							= ca_metadata_elements::getSortableElements( $va_properties['showSortableBundlesFor'] );

						$va_select_opts = array(
							_t( 'User defined sort order' ) => '',
							_t( 'Order created' )           => 'relation_id',
							// forces sorting by relationship primary key  - aka order relationships were created
							_t( 'Preferred label' )         => $va_properties['showSortableBundlesFor']
							                                   . ".preferred_labels." . $t_rel->getLabelDisplayField()
						);
						if ( $t_rel->tableName() == 'ca_sets' ) {
							unset( $va_select_opts[ _t( 'User defined sort order' ) ] );
						}
						if ( ( $vs_idno_fld = $t_rel->getProperty( 'ID_NUMBERING_SORT_FIELD' ) )
						     || ( $vs_idno_fld
								= $t_rel->getProperty( 'ID_NUMBERING_ID_FIELD' ) )
						) {
						...
						foreach ( $va_elements as $vn_element_id => $va_element ) {
							...
							$va_select_opts[ _t( 'Element: %1', $va_element['display_label'] ) ]
								= $va_properties['showSortableBundlesFor'] . "." . $va_element['element_code'];
						}
						...
						$va_opts           = array(
							...
							'width'  => $vn_width,
							'height' => $vn_height,
							'value'  => is_array( $vs_value ) ? $vs_value[0] : $vs_value,
							'values' => is_array( $vs_value ) ? $vs_value : array( $vs_value )
						);
						...
						foreach ( $va_properties['table'] as $vs_table ) {
							$va_rep_elements = ca_metadata_elements::getElementsAsList( true, $vs_table, null, true,
								false, true, is_numeric( $va_properties['showMetadataElementsWithDataType'] )
									? array( $va_properties['showMetadataElementsWithDataType'] ) : null );
							...
							if ( $va_properties['includeIntrinsics'] ) {
								...
								foreach ( $t_rep->getFormFields() as $vs_f => $va_field_info ) {
									if ( is_array( $va_properties['includeIntrinsics'] )
									     && ! in_array( $vs_f, $va_properties['includeIntrinsics'] )
									) {
										continue;
									}
							...
							if ( $va_properties['includePreferredLabels'] ) {
								...
								foreach ( $t_rep->getLabelUIFields() as $vs_f ) {
									$va_select_opts[ $t_rep->getDisplayLabel( "{$vs_table}.preferred_labels.{$vs_f}" ) ]
										= "{$vs_table}.preferred_labels.{$vs_f}";
								}
							}
						}
						...
						if ( sizeof( $va_select_opts ) ) {
							...
							$vs_select_element = caHTMLSelect( $vs_input_name, $va_select_opts, $va_select_attr,
								$va_opts );
						}
					} else {
						...
						$va_opts = array(
							'width'  => $vn_width,
							'height' => $vn_height,
							'values' => is_array( $vs_value ) ? $vs_value : [ $vs_value ]
						);
						if ( ! isset( $va_opts['value'] ) ) {
							$va_opts['value'] = - 1;
						}        // make sure default list item is never selected
						$va_attr['id']     = $vs_input_id;
						$vs_select_element = caHTMLSelect( $vs_input_name, $va_properties['options'], $va_attr,
							$va_opts );
					}
				}
			...
			case DT_INTERVAL:
				...
				$vs_return .= caHTMLTextInput( "{$vs_input_name}Quantity", [
						'size'   => $va_properties["width"],
						'height' => $va_properties["height"],
						'value'  => $va_val_tmp[0],
						'id'     => "{$vs_input_id}Quantity"
					] ) . ' ';
				$vs_return .= caHTMLSelect( "{$vs_input_name}Units",
						[ _t( 'hours' ) => 'HOURS', _t( 'days' ) => 'DAYS', _t( 'weeks' ) => 'WEEKS' ],
						[ 'id' => "{$vs_input_id}Units" ], [ 'value' => $va_val_tmp[1] ] ) . ' ';
				$vs_return .= caHTMLSelect( "{$vs_input_name}Direction",
					[ _t( 'before' ) => 'BEFORE', _t( 'after' ) => 'AFTER' ], [ 'id' => "{$vs_input_id}Direction" ],
					[ 'value' => $va_val_tmp[2] ] );
	...
	public function setSettingsFromHTMLForm( $po_request ) {
		...
		foreach ( array_keys( $va_available_settings ) as $vs_setting ) {
			...
			if ( isset( $va_properties['takesLocale'] ) && $va_properties['takesLocale'] ) {
				foreach ( $va_locales as $vs_locale => $va_locale_info ) {
					$va_values[ $vs_setting ][ $va_locale_info['locale_id'] ] = $po_request->getParameter( 'setting_'
					                                                                                       . $vs_setting
					                                                                                       . '_'
					                                                                                       . $vs_locale,
						pString );
				...
				if (
					( isset( $va_properties['showTypesForTable'] ) && $va_properties['showTypesForTable']
					...
					|| ( isset( $va_properties['useList'] ) && $va_properties['useList']
					     && ( $va_properties['height'] > 1 ) )
					|| ( isset( $va_properties['showMetadataElementsWithDataType'] )
					     && $va_properties['showMetadataElementsWithDataType']
					     && ( $va_properties['height'] > 1 ) )
