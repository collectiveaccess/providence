...
class BaseSettings
{
    ...
    public function settingHTMLFormElement($ps_setting, $pa_options = null)
    {
        ...
        if ($vs_help_text = $pa_options['helpText']) {
            $vs_return .= "<a href='#' onclick='jQuery(\"#" . str_replace(
                    ".",
                    "_",
                    $vs_label_id
                ) . "_help_text\").slideToggle(250); return false;' class='settingsKeyButton'>" . _t('Key') . "</a>";
        }
        ...
        switch ($va_properties['displayType']) {
            ...
            case DT_FIELD:
                ...
                foreach ($va_locales as $vs_locale => $va_locale_info) {
                    ...
                    if (($vs_locale != '_generic') && (is_array(
                            $vs_value
                        ))) {        // _generic means this setting doesn't take a locale
                        if (!($vs_text_value = $vs_value[$va_locale_info['locale_id']])) {
                            $vs_text_value = (is_array(
                                    $vs_value
                                ) && isset($vs_value[$va_locale_info['code']])) ? $vs_value[$va_locale_info['code']] : '';
                        }
                    } else {
                        $vs_text_value = $vs_value;
                    }
                    $vs_return .= ($vs_locale_label ? "{$vs_locale_label}<br/>" : "") . caHTMLTextInput(
            ...
            case DT_CHECKBOXES:
                ...
                if (isset($va_properties['hideOnSelect'])) {
                    ...
                    $va_attributes['onchange'] = 'jQuery(this).prop("checked") ? jQuery("' . join(
                            ",",
                            $va_ids
                        ) . '").slideUp(250).find("input, textarea").val("") : jQuery("' . join(
                            ",",
                            $va_ids
                        ) . '").slideDown(250);';
                }
                if (isset($va_properties['showOnSelect'])) {
                    ...
                    $va_attributes['onchange'] = 'jQuery(this).prop("checked") ? jQuery("' . join(
                            ",",
                            $va_ids
                        ) . '").slideDown(250).find("input, textarea").val("") : jQuery("' . join(
                            ",",
                            $va_ids
                        ) . '").slideUp(250);';
		...
		jQuery('" . join(",", $va_ids) . "').hide();
            ...
            case DT_SELECT:
                ...
                if ($va_properties['showTypesForTable']) {
                    $vs_select_element = '';
                    if (!($t_show_types_for_table = Datamodel::getInstanceByTableName(
                        $va_properties['showTypesForTable'],
                        true
                    ))) {
                        break;
                    }
                    ...
                    if ($vs_rel_table) {
                        ...
                        if ($vb_locale_list) {
                            ...
                            $va_rel_opts = array_flip(
                                ca_locales::getLocaleList(array('return_display_values' => true))
                            );
                            ...
                            $va_rel_opts = array_flip(
                                array_map(
                                    ...
                                    ca_objects::getHistoryTrackingCurrentValuePolicies(
                                        caGetOption('table', $pa_options, null)
                                    )
                                )
                            );
                            ...
                            $va_rel_opts = array_flip(
                                array_map(
                                    ...
                                    ca_objects::getDependentHistoryTrackingCurrentValuePolicies(
                                        caGetOption('table', $pa_options, null)
                                    )
                                )
                            );
                    ...
                    if ($vs_list_code) {
                        ...
                        $vs_select_element = $t_list->getListAsHTMLFormElement(
                            $vs_list_code,
                            $vs_input_name,
                            $va_attr,
                            $va_opts
                    ...
                    if (strlen($va_properties['showSortableBundlesFor']) > 0) {
                        ...
                        if (!($t_rel = Datamodel::getInstanceByTableName(
                            $va_properties['showSortableBundlesFor'],
                            true
                        ))) {
                            break;
                        }
                        $va_elements = ca_metadata_elements::getSortableElements(
                            $va_properties['showSortableBundlesFor']
                        );
                        ...
                        $va_select_opts = array(
                            ...
                            _t(
                                'Preferred label'
                            ) => $va_properties['showSortableBundlesFor'] . ".preferred_labels." . $t_rel->getLabelDisplayField(
                                )
                        );
                        if ($t_rel->tableName() == 'ca_sets') {
                            unset($va_select_opts[_t('User defined sort order')]);
                        }
                        if (($vs_idno_fld = $t_rel->getProperty(
                        ...
                        foreach ($va_properties['table'] as $vs_table) {
                            $va_rep_elements = ca_metadata_elements::getElementsAsList(
                                true,
                                $vs_table,
                                null,
                                true,
                            ...
                            if ($va_properties['includeIntrinsics']) {
                                ...
                                foreach ($t_rep->getFormFields() as $vs_f => $va_field_info) {
                                    if (is_array($va_properties['includeIntrinsics']) && !in_array(
                                            $vs_f,
                                            $va_properties['includeIntrinsics']
                                        )) {
                                        continue;
                                    }
                            ...
                            if ($va_properties['includePreferredLabels']) {
                                ...
                                foreach ($t_rep->getLabelUIFields() as $vs_f) {
                                    $va_select_opts[$t_rep->getDisplayLabel(
                        ...
                        if (sizeof($va_select_opts)) {
                            ...
                            $vs_select_element = caHTMLSelect(
                                $vs_input_name,
                                $va_select_opts,
                                $va_select_attr,
                                $va_opts
            ...
            case DT_INTERVAL:
                ...
                $vs_return .= caHTMLTextInput(
                        "{$vs_input_name}Quantity",
                        [
                            'size' => $va_properties["width"],
                            'height' => $va_properties["height"],
                            'value' => $va_val_tmp[0],
                ...
                $vs_return .= caHTMLSelect(
                        "{$vs_input_name}Units",
                        [_t('hours') => 'HOURS', _t('days') => 'DAYS', _t('weeks') => 'WEEKS'],
                        ['id' => "{$vs_input_id}Units"],
                        ['value' => $va_val_tmp[1]]
                    ) . ' ';
                $vs_return .= caHTMLSelect(
                    "{$vs_input_name}Direction",
                    [_t('before') => 'BEFORE', _t('after') => 'AFTER'],
                    ['id' => "{$vs_input_id}Direction"],
                    ['value' => $va_val_tmp[2]]
                );
            ...
            if (isset($va_properties['takesLocale']) && $va_properties['takesLocale']) {
                foreach ($va_locales as $vs_locale => $va_locale_info) {
                    $va_values[$vs_setting][$va_locale_info['locale_id']] = $po_request->getParameter(
                        'setting_' . $vs_setting . '_' . $vs_locale,
                        pString
                    );
                }
            } else {
