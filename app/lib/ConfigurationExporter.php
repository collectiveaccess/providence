...
final class ConfigurationExporter {
	 ...
	 * @param int|null $pn_modified_after If set, only configuration elements modified *after* this unix timestamp are
	 *                                    exported
	 ...
	 */
	public static function exportConfigurationAsXML(
		$ps_name = "", $ps_description = "", $ps_base = "", $ps_info_url = "", $pn_modified_after = null,
		$pb_print_status = false
	) {
	...
	public function getLocalesAsDOM() {
		...
		while ( $qr_locales->nextRow() ) {
			if ( $this->opn_modified_after ) {
				if ( $t_locale->getLastChangeTimestampAsInt( $qr_locales->get( 'locale_id' ) )
				     < $this->opn_modified_after
				) {
					continue;
				}

				$this->printStatus( _t( "Exporting changes for locale %1",
					caEscapeForXML( $qr_locales->get( "name" ) ) ) );
			}
	...
	public function getListsAsDOM() {
		...
		if ( $this->opn_modified_after
		     && is_array( $va_deleted = $this->getDeletedItemsFromChangeLogByIdno( $t_list->tableNum(), 'list_code' ) )
		) {
		...
		while ( $qr_lists->nextRow() ) {
			...
			if ( ( $vn_exclude_lists_larger_than > 0 )
			     && ( $t_list->numItemsInList( $qr_lists->get( 'list_id' ) ) > $vn_exclude_lists_larger_than )
			) {
				continue;
			}
			...
			$qr_list_labels = $this->opo_db->query( "SELECT * FROM ca_list_labels WHERE list_id=?",
				$qr_lists->get( "list_id" ) );
			...
			if ( $qr_list_labels->numRows() == 0 ) {
				...
				while ( $qr_list_labels->nextRow() ) {
					...
					$vo_label->setAttribute( "locale",
						$this->opt_locale->localeIDToCode( $qr_list_labels->get( "locale_id" ) ) );
					$vo_label->appendChild( $this->opo_dom->createElement( "name",
						caEscapeForXML( $qr_list_labels->get( "name" ) ) ) );
			...
			if ( $this->opn_modified_after ) {
				...
				if ( ! $vo_items->childNodes->length ) {
					if ( $t_list->getLastChangeTimestampAsInt( $qr_lists->get( 'list_id' ) )
					     < $this->opn_modified_after
					) {
						continue;
					}
	...
	private function getListItemsAsDOM( $pn_parent_id, $pn_list_id ) {
		$qr_items    = $this->opo_db->query( "SELECT * FROM ca_list_items WHERE parent_id=? AND deleted=0",
			$pn_parent_id );
		...
		if ( $this->opn_modified_after
		     && is_array( $va_deleted = $this->getDeletedItemsFromChangeLogByIdno( $t_list_item->tableNum(), 'idno',
				array( 'list_id' => $pn_list_id ) ) )
		) {
			foreach ( $va_deleted as $vs_deleted_idno ) {
		...
		while ( $qr_items->nextRow() ) {
			...
			$vo_labels           = $this->opo_dom->createElement( "labels" );
			$qr_list_item_labels = $this->opo_db->query( "SELECT * FROM ca_list_item_labels WHERE item_id=?",
				$qr_items->get( "item_id" ) );
			if ( $qr_list_item_labels->numRows() > 0 ) {
				while ( $qr_list_item_labels->nextRow() ) {
					$vo_label = $this->opo_dom->createElement( "label" );

					$vo_label->setAttribute( "locale",
						$this->opt_locale->localeIDToCode( $qr_list_item_labels->get( "locale_id" ) ) );
					...
					$vo_label->appendChild( $this->opo_dom->createElement( "name_singular",
						caEscapeForXML( $qr_list_item_labels->get( "name_singular" ) ) ) );
					$vo_label->appendChild( $this->opo_dom->createElement( "name_plural",
						caEscapeForXML( $qr_list_item_labels->get( "name_plural" ) ) ) );
					$vo_label->appendChild( $this->opo_dom->createElement( "description",
						caEscapeForXML( $qr_list_item_labels->get( "description" ) ) ) );
			...
			if ( $this->opn_modified_after && ! $vo_sub_items->childNodes->length ) {
				if ( $t_list_item->getLastChangeTimestampAsInt( $qr_items->get( 'item_id' ) )
				     < $this->opn_modified_after
				) {
					continue;
				}
	...
	public function getElementsAsDOM() {
		...
		$qr_elements
			= $this->opo_db->query( "SELECT * FROM ca_metadata_elements WHERE parent_id IS NULL ORDER BY element_id" );

		$t_element = new ca_metadata_elements();

		if ( $this->opn_modified_after
		     && is_array(
			     $va_deleted = $this->getDeletedItemsFromChangeLogByIdno( $t_element->tableNum(), 'element_code' ) )
		) {
			foreach ( $va_deleted as $vs_deleted_idno ) {
		...
		while ( $qr_elements->nextRow() ) {
			...
			$vo_element->setAttribute( "datatype",
				ca_metadata_elements::getAttributeNameForTypeCode( $qr_elements->get( "datatype" ) ) );
			...
			$vo_labels         = $this->opo_dom->createElement( "labels" );
			$qr_element_labels = $this->opo_db->query( "SELECT * FROM ca_metadata_element_labels WHERE element_id=?",
				$qr_elements->get( "element_id" ) );
			...
			while ( $qr_element_labels->nextRow() ) {
				...
				$vo_label->setAttribute( "locale",
					$this->opt_locale->localeIDToCode( $qr_element_labels->get( "locale_id" ) ) );
				$vo_label->appendChild( $this->opo_dom->createElement( "name",
					caEscapeForXML( $qr_element_labels->get( "name" ) ) ) );
				if ( strlen( trim( $qr_element_labels->get( "description" ) ) ) > 0 ) {
					$vo_label->appendChild( $this->opo_dom->createElement( "description",
						caEscapeForXML( $qr_element_labels->get( "description" ) ) ) );
				}
			...
			if ( ! $vn_num_labels ) {
				$vo_label = $this->opo_dom->createElement( "label" );
				$vo_label->setAttribute( "locale",
					$this->opt_locale->localeIDToCode( ca_locales::getDefaultCataloguingLocaleID() ) );
				...
				foreach ( $t_element->getSettings() as $vs_setting => $va_values ) {
					...
					foreach ( $va_values as $vs_value ) {
						if ( $t_element->isValidSetting( $vs_setting ) ) {
							...
							if ( $this->opn_modified_after
							     || ( $vs_value != $va_available_settings[ $vs_setting ]["default"] )
							) {
			...
			if ( $this->opn_modified_after && ! $vo_sub_elements->childNodes->length ) {
				if ( $t_element->getLastChangeTimestampAsInt( $qr_elements->get( 'element_id' ) )
				     < $this->opn_modified_after
				) {
					continue;
				}
	...
	private function getElementAsDOM( $pn_parent_id, $pb_ignore_timestamp = false ) {
		...
		$qr_elements
			= $this->opo_db->query( "SELECT * FROM ca_metadata_elements WHERE parent_id = ? ORDER BY element_id",
			$pn_parent_id );
	...
	public function getMetadataDictionaryAsDOM() {
		...
		while ( $qr_entries->nextRow() ) {
			...
			if ( $this->opn_modified_after ) {
				if ( $t_entry->getLastChangeTimestampAsInt( $qr_entries->get( 'entry_id' ) )
				     < $this->opn_modified_after
				) {
					continue;
				}
	...
	public function getUIsAsDOM() {
		...
		if ( $this->opn_modified_after
		     && is_array( $va_deleted = $this->getDeletedItemsFromChangeLogByIdno( $t_ui->tableNum(), 'editor_code' ) )
		) {
			foreach ( $va_deleted as $vs_deleted_idno ) {
		...
		while ( $qr_uis->nextRow() ) {
			...
			// labels
			$vo_labels    = $this->opo_dom->createElement( "labels" );
			$qr_ui_labels = $this->opo_db->query( "SELECT * FROM ca_editor_ui_labels WHERE ui_id=?",
				$qr_uis->get( "ui_id" ) );
			if ( $qr_ui_labels->numRows() > 0 ) {
				while ( $qr_ui_labels->nextRow() ) {
					if ( $vs_locale = $this->opt_locale->localeIDToCode( $qr_ui_labels->get( "locale_id" ) ) ) {
						...
						$vo_label->appendChild( $this->opo_dom->createElement( "name",
							caEscapeForXML( $qr_ui_labels->get( "name" ) ) ) );
						$vo_labels->appendChild( $vo_label );
					}
				}
			} else {
			...
			if ( is_array( $va_users ) && ( sizeof( $va_users ) > 0 ) ) {
				...
				foreach ( $va_users as $va_user_info ) {
					if ( ( $acc = $this->_convertUserGroupAccessToString( intval( $va_user_info['access'] ) ) )
					     === false
					) {
						continue;
					}
			...
			if ( is_array( $va_groups ) && ( sizeof( $va_groups ) > 0 ) ) {
				...
				foreach ( $va_groups as $va_group_info ) {
					if ( ( $acc = $this->_convertUserGroupAccessToString( intval( $va_group_info['access'] ) ) )
					     === false
					) {
						continue;
					}
			...
			if ( is_array( $va_roles ) && ( sizeof( $va_roles ) > 0 ) ) {
				...
				foreach ( $va_roles as $va_role_info ) {
					if ( ( $acc = $this->_convertUserGroupAccessToString( intval( $va_role_info['access'] ) ) )
					     === false
					) {
						continue;
					}
			...
			$qr_screens
			            = $this->opo_db->query( "SELECT * FROM ca_editor_ui_screens WHERE parent_id IS NOT NULL AND ui_id=? ORDER BY `rank`,screen_id",
				$qr_uis->get( "ui_id" ) );

			$t_screen = new ca_editor_ui_screens();

			if ( $this->opn_modified_after
			     && is_array( $va_deleted = $this->getDeletedItemsFromChangeLogByIdno( $t_screen->tableNum(), 'idno',
					array( 'ui_id' => $qr_uis->get( "ui_id" ) ) ) )
			) {
				foreach ( $va_deleted as $vs_deleted_idno ) {
			...
			while ( $qr_screens->nextRow() ) {
				...
				if ( $this->opn_modified_after ) {
					if ( $t_screen->getLastChangeTimestampAsInt( $qr_screens->get( 'screen_id' ) )
					     < $this->opn_modified_after
					) {
						continue;
					}
				...
				$vo_labels        = $this->opo_dom->createElement( "labels" );
				$qr_screen_labels = $this->opo_db->query( "SELECT * FROM ca_editor_ui_screen_labels WHERE screen_id=?",
					$qr_screens->get( "screen_id" ) );
				if ( $qr_ui_labels->numRows() > 0 ) {
					while ( $qr_screen_labels->nextRow() ) {
						if ( $vs_locale = $this->opt_locale->localeIDToCode( $qr_screen_labels->get( "locale_id" ) ) ) {
							...
							$vo_label->setAttribute( "locale", $vs_locale );
							$vo_label->appendChild( $this->opo_dom->createElement( "name",
								caEscapeForXML( $qr_screen_labels->get( "name" ) ) ) );
							if ( strlen( trim( $qr_screen_labels->get( "description" ) ) ) > 0 ) {
								$vo_label->appendChild( $this->opo_dom->createElement( "description",
									caEscapeForXML( $qr_screen_labels->get( "description" ) ) ) );
							}
							$vo_labels->appendChild( $vo_label );
						}
					}
				...
				if ( is_array( $t_screen->getTypeRestrictions() ) && sizeof( $t_screen->getTypeRestrictions() ) > 0 ) {
					...
					foreach ( $t_screen->getTypeRestrictions() as $va_restriction ) {
						...
						if ( $t_instance instanceof BaseRelationshipModel ) {
							...
							$vs_type_code = $t_instance->getTypeListCode();
							$va_item      = $t_list->getItemFromListByItemID( $vs_type_code,
								$va_restriction["type_id"] );
				...
				if ( is_array( $va_placements ) ) {
					...
					foreach ( $va_placements as $va_placement ) {
						...
						$vo_placement->setAttribute( "code",
							$vs_code = $this->makeIDNO( $va_placement["placement_code"], 30, $va_used_codes ) );

						if ( isset( $va_placement['settings']['bundleTypeRestrictions'] )
						     && ( is_array( $va_type_restrictions
									= $va_placement['settings']['bundleTypeRestrictions'] )
						          || strlen( $va_type_restrictions ) )
						) {
							...
							if ( is_array( $va_type_restrictions ) && ( sizeof( $va_type_restrictions ) > 0 ) ) {
								$vo_placement->setAttribute( "typeRestrictions",
									join( ",", array_unique( caMakeTypeList( $vs_type, $va_type_restrictions ) ) ) );
							}
						}
						if ( isset( $va_placement['settings']['bundleTypeRestrictionsIncludeSubtypes'] )
						     && (bool) $va_placement['settings']['bundleTypeRestrictionsIncludeSubtypes']
						) {
						...
						$vo_placement->appendChild( $this->opo_dom->createElement( "bundle",
							caEscapeForXML( $va_placement["bundle"] ) ) );
	...
	private function getRelationshipTypesForParentAsDOM( $pn_parent_id, $pn_table_num ) {
		...
		if ( $this->opn_modified_after
		     && is_array(
			     $va_deleted = $this->getDeletedItemsFromChangeLogByIdno( $t_rel_types->tableNum(), 'type_code',
				     array( 'table_num' => $pn_table_num ) ) )
		) {
			foreach ( $va_deleted as $vs_deleted_idno ) {
		...
		$qr_types
			= $this->opo_db->query( "SELECT * FROM ca_relationship_types WHERE parent_id= ? ORDER BY `rank`, type_id",
			[ $pn_parent_id ] );
		...
		while ( $qr_types->nextRow() ) {
			...
			if ( $this->opn_modified_after ) {
				if ( $t_rel_types->getLastChangeTimestampAsInt( $qr_types->get( 'type_id' ) )
				     < $this->opn_modified_after
				) {
					continue;
				}
			...
			if ( preg_match( "/root\_for\_[0-9]{1,3}/",
				$qr_types->get( "type_code" ) )
			...
			$vo_labels      = $this->opo_dom->createElement( "labels" );
			$qr_type_labels = $this->opo_db->query( "SELECT * FROM ca_relationship_type_labels WHERE type_id = ?",
				[ $qr_types->get( "type_id" ) ] );
			while ( $qr_type_labels->nextRow() ) {
				$vo_label = $this->opo_dom->createElement( "label" );

				$vo_label->setAttribute( "locale",
					$this->opt_locale->localeIDToCode( $qr_type_labels->get( "locale_id" ) ) );
				$vo_label->appendChild( $this->opo_dom->createElement( "typename",
					caEscapeForXML( $qr_type_labels->get( "typename" ) ) ) );
				$vo_label->appendChild( $this->opo_dom->createElement( "typename_reverse",
					caEscapeForXML( $qr_type_labels->get( "typename_reverse" ) ) ) );
			...
			if ( $qr_types->get( "sub_type_left_id" ) ) {
				...
				$vo_type->setAttribute( "includeSubtypesLeft",
					(bool) $qr_types->get( "include_subtypes_left" ) ? 1 : 0 );
			}
			...
			if ( $qr_types->get( "sub_type_right_id" ) ) {
				...
				$va_item      = $t_list->getItemFromListByItemID( $vs_type_code,
					$qr_types->get( "sub_type_right_id" ) );

				$vo_type->setAttribute( "typeRestrictionRight", $va_item["idno"] );
				$vo_type->setAttribute( "includeSubtypesRight",
					(bool) $qr_types->get( "include_subtypes_right" ) ? 1 : 0 );
			}
	...
	public function getRolesAsDOM() {
		...
		if ( $this->opn_modified_after
		     && is_array( $va_deleted = $this->getDeletedItemsFromChangeLogByIdno( $t_role->tableNum(), 'code' ) )
		) {
			foreach ( $va_deleted as $vs_deleted_idno ) {
		...
		while ( $qr_roles->nextRow() ) {
			...
			if ( is_array( $va_vars['bundle_access_settings'] ) ) {
				...
				foreach ( $va_vars['bundle_access_settings'] as $vs_bundle => $vn_val ) {
					...
					if ( $t_ui_screens->isAvailableBundle( $vs_table_name,
						$vs_bundle_name )
			...
			if ( is_array( $va_vars['source_access_settings'] ) ) {
				...
				foreach ( $va_vars['source_access_settings'] as $vs_id => $vn_val ) {
					...
					$vo_permission->setAttribute( 'access', $vs_access );
					$vb_default = (int) ( isset( $va_vars['source_access_settings'][ $vs_table_name . '_default_id' ] )
					                      && $vn_type_id == $va_vars['source_access_settings'][ $vs_table_name
					                                                                            . '_default_id' ] );
	...
	public function getGroupsAsDOM() {
		...
		if ( $this->opn_modified_after
		     && is_array( $va_deleted = $this->getDeletedItemsFromChangeLogByIdno( $t_group->tableNum(), 'code' ) )
		) {
			foreach ( $va_deleted as $vs_deleted_idno ) {
		...
		while ( $qr_groups->nextRow() ) {
			if ( $this->opn_modified_after ) {
				if ( $t_group->getLastChangeTimestampAsInt( $qr_groups->get( 'group_id' ) )
				     < $this->opn_modified_after
				) {
					continue;
				}
			...
			$vo_group->appendChild( $this->opo_dom->createElement( "name",
				caEscapeForXML( $t_group->get( "name" ) ) ) );
			$vo_group->appendChild( $this->opo_dom->createElement( "description",
				caEscapeForXML( $t_group->get( "description" ) ) ) );

			if ( is_array( $va_roles = $t_group->getGroupRoles() ) ) {
				$vo_roles = $this->opo_dom->createElement( "roles" );
				foreach ( $va_roles as $va_role ) {
					$vo_roles->appendChild( $this->opo_dom->createElement( "role",
						$this->makeIDNO( $va_role["code"] ) ) );
				}
	...
	public function getSearchFormsAsDOM() {
		...
		if ( $this->opn_modified_after
		     && is_array( $va_deleted = $this->getDeletedItemsFromChangeLogByIdno( $t_form->tableNum(), 'form_code' ) )
		) {
			foreach ( $va_deleted as $vs_deleted_idno ) {
		...
		while ( $qr_forms->nextRow() ) {
			...
			if ( is_array( $va_restrictions = $t_form->getTypeRestrictions() ) && sizeof( $va_restrictions ) ) {
				...
				if ( is_array( $va_type_ids ) && ( sizeof( $va_type_ids ) > 0 ) ) {
					$vo_form->setAttribute( "typeRestrictions",
						join( ",", caMakeTypeList( $t_form->get( 'table_num' ), $va_type_ids ) ) );
					$vo_form->setAttribute( "includeSubtypes", $vb_include_subtypes ? 1 : 0 );
				}
			}

			$vo_labels      = $this->opo_dom->createElement( "labels" );
			$qr_form_labels = $this->opo_db->query( "SELECT * FROM ca_search_form_labels WHERE form_id=?",
				$qr_forms->get( "form_id" ) );
			while ( $qr_form_labels->nextRow() ) {
				$vo_label = $this->opo_dom->createElement( "label" );

				$vo_label->setAttribute( "locale",
					$this->opt_locale->localeIDToCode( $qr_form_labels->get( "locale_id" ) ) );
				$vo_label->appendChild( $this->opo_dom->createElement( "name",
					caEscapeForXML( $qr_form_labels->get( "name" ) ) ) );
			...
			$vo_placements = $this->opo_dom->createElement( "bundlePlacements" );
			$qr_placements
			               = $this->opo_db->query( "SELECT * FROM ca_search_form_placements WHERE form_id=? ORDER BY placement_id",
				$qr_forms->get( "form_id" ) );
			while ( $qr_placements->nextRow() ) {
				...
				$vo_placements->appendChild( $vo_placement );
				$vo_placement->appendChild( $this->opo_dom->createElement( "bundle",
					caEscapeForXML( $qr_placements->get( "bundle_name" ) ) ) );
	...
	public function getDisplaysAsXML() {
		...
		if ( $this->opn_modified_after
		     && is_array(
			     $va_deleted = $this->getDeletedItemsFromChangeLogByIdno( $t_display->tableNum(), 'display_code' ) )
		) {
			foreach ( $va_deleted as $vs_deleted_idno ) {
		...
		foreach ( $va_displays as $vn_i => $va_display_by_locale ) {
			...
			if ( is_array( $va_restrictions = $t_display->getTypeRestrictions() ) && sizeof( $va_restrictions ) ) {
				...
				if ( is_array( $va_type_ids ) && sizeof( $va_type_ids ) ) {
					$vs_type_restriction_attr = "typeRestrictions=\"" . join( ",",
							caMakeTypeList( $t_display->get( 'table_num' ), $va_type_ids ) ) . "\" includeSubtypes=\""
					                            . ( $vb_include_subtypes ? 1 : 0 ) . "\"";
				}
			}

			$vs_buf .= "\t<display code='" . ( $va_info['display_code']
			                                   && preg_match( '!^[A-Za-z0-9_]+$!', $va_info['display_code'] )
					? $va_info['display_code'] : 'display_' . $va_info['display_id'] ) . "' type='"
			           . Datamodel::getTableName( $va_info['table_num'] ) . "' system='"
			           . $t_display->get( 'is_system' ) . "' {$vs_type_restriction_attr}>\n";
			$vs_buf .= "\t\t<labels>\n";
			foreach ( $va_display_by_locale as $vn_locale_id => $va_display_info ) {
				if ( strlen( $this->opt_locale->localeIDToCode( $vn_locale_id ) ) > 0 ) {
					$vs_buf .= "\t\t\t<label locale='" . $this->opt_locale->localeIDToCode( $vn_locale_id ) . "'><name>"
					           . caEscapeForXML( $va_display_info['name'] ) . "</name></label>\n";
				}
			}
			$vs_buf .= "\t\t</labels>\n";
			...
			foreach ( $va_placements as $vn_placement_id => $va_placement_info ) {
				$vs_buf      .= "\t\t<placement code='" . preg_replace( "![^A-Za-z0-9_]+!", "_",
						$va_placement_info['bundle_name'] ) . "'><bundle>" . $va_placement_info['bundle_name']
				                . "</bundle>\n";
							...
							case 'label':
								if ( is_array( $vm_value ) ) {
									foreach ( $vm_value as $vn_locale_id => $vm_locale_specific_value ) {
										if ( preg_match( "/^[a-z]{2,3}\_[A-Z]{2,3}$/",
											$vn_locale_id )
										) { // locale code
											$vs_locale_code = $vn_locale_id;
										} else {
											if ( ! ( $vs_locale_code
												= $this->opt_locale->localeIDToCode( $vn_locale_id ) )
											) {
												$vs_locale_code = 'en_US';
											}
										}
										$vs_buf .= "<setting name='label' locale='" . $vs_locale_code . "'>"
										           . caEscapeForXML( $vm_locale_specific_value ) . "</setting>\n";
									}
								}
								break;
							case 'restrict_to_relationship_types':
								if ( is_array( $vm_value ) ) {
									foreach ( $vm_value as $vn_val ) {
										...
										if ( $t_rel_type->getPrimaryKey() ) {
											$vs_value = $t_rel_type->get( 'type_code' );
											$vs_buf   .= "\t\t\t\t<setting name='{$vs_setting}'><![CDATA[" . $vs_value
											             . "]]></setting>\n";
										}
									}
								}
							...
							default:
								if ( is_array( $vm_value ) ) {
									foreach ( $vm_value as $vn_i => $vn_val ) {
										$vs_buf .= "\t\t\t\t<setting name='{$vs_setting}'><![CDATA[" . $vn_val
										           . "]]></setting>\n";
									}
								} else {
									$vs_buf .= "\t\t\t\t<setting name='{$vs_setting}'><![CDATA[" . $vm_value
									           . "]]></setting>\n";
								}
								break;
						}
	 ...
	 */
	private function getDeletedItemsFromChangeLogByIdno(
		$pn_table_num, $ps_idno_field = 'code', $pa_additional_filters = array()
	) {
		if ( ! $this->opn_modified_after ) {
			return array();
		}
