...
final class ConfigurationExporter
{
    ...
    public function getListsAsDOM()
    {
        ...
        if ($this->opn_modified_after && is_array(
                $va_deleted = $this->getDeletedItemsFromChangeLogByIdno($t_list->tableNum(), 'list_code')
            )) {
        ...
        while ($qr_lists->nextRow()) {
            ...
            if (($vn_exclude_lists_larger_than > 0) && ($t_list->numItemsInList(
                        $qr_lists->get('list_id')
                    ) > $vn_exclude_lists_larger_than)) {
                continue;
            }
            ...
            $qr_list_labels = $this->opo_db->query(
                "SELECT * FROM ca_list_labels WHERE list_id=?",
                $qr_lists->get("list_id")
            );
            ...
            if ($qr_list_labels->numRows() == 0) {
                ...
                while ($qr_list_labels->nextRow()) {
                    ...
                    $vo_label->setAttribute(
                        "locale",
                        $this->opt_locale->localeIDToCode($qr_list_labels->get("locale_id"))
                    );
                    $vo_label->appendChild(
                        $this->opo_dom->createElement("name", caEscapeForXML($qr_list_labels->get("name")))
                    );
            ...
            $vo_items = $this->getListItemsAsDOM(
                $t_list->getRootItemIDForList($qr_lists->get("list_code")),
                $qr_lists->get('list_id')
            );
    ...
    private function getListItemsAsDOM($pn_parent_id, $pn_list_id)
    {
        ...
        if ($this->opn_modified_after &&
            is_array(
                $va_deleted = $this->getDeletedItemsFromChangeLogByIdno(
                    $t_list_item->tableNum(),
                    'idno',
        ...
        while ($qr_items->nextRow()) {
            ...
            $vo_labels = $this->opo_dom->createElement("labels");
            $qr_list_item_labels = $this->opo_db->query(
                "SELECT * FROM ca_list_item_labels WHERE item_id=?",
    ...
    public function getElementsAsDOM()
    {
        ...
        while ($qr_elements->nextRow()) {
            ...
            $vo_labels = $this->opo_dom->createElement("labels");
            $qr_element_labels = $this->opo_db->query(
                "SELECT * FROM ca_metadata_element_labels WHERE element_id=?",
            ...
            while ($qr_element_labels->nextRow()) {
                ...
                $vo_label->appendChild(
                    $this->opo_dom->createElement("name", caEscapeForXML($qr_element_labels->get("name")))
                );
            ...
            if ($this->opn_modified_after && !$vo_sub_elements->childNodes->length) {
                if ($t_element->getLastChangeTimestampAsInt(
                        $qr_elements->get('element_id')
        ...
        while ($qr_entries->nextRow()) {
            ...
            if (is_array($va_rules) && sizeof($va_rules) > 0) {
                ...
                foreach ($va_rules as $va_rule) {
                    ...
                    // expression
                    if (isset($va_rule['expression']) && is_array($va_rule['expression']) && sizeof(
                            $va_rule['expression']
                        ) > 0) {
    ...
    public function getUIsAsDOM()
    {
        ...
        if ($this->opn_modified_after && is_array(
                $va_deleted = $this->getDeletedItemsFromChangeLogByIdno($t_ui->tableNum(), 'editor_code')
            )) {
            foreach ($va_deleted as $vs_deleted_idno) {
        ...
        while ($qr_uis->nextRow()) {
            ...
            // labels
            $vo_labels = $this->opo_dom->createElement("labels");
            $qr_ui_labels = $this->opo_db->query(
                "SELECT * FROM ca_editor_ui_labels WHERE ui_id=?",
                $qr_uis->get("ui_id")
            );
            if ($qr_ui_labels->numRows() > 0) {
                while ($qr_ui_labels->nextRow()) {
                    if ($vs_locale = $this->opt_locale->localeIDToCode($qr_ui_labels->get("locale_id"))) {
                        ...
                        $vo_label->appendChild(
                            $this->opo_dom->createElement("name", caEscapeForXML($qr_ui_labels->get("name")))
                        );
                        $vo_labels->appendChild($vo_label);
                    }
                }
            ...
            if (is_array($va_users) && (sizeof($va_users) > 0)) {
                ...
                foreach ($va_users as $va_user_info) {
                    ...
                    $vo_permission->setAttribute(
                        "access",
                        $this->_convertUserGroupAccessToString(intval($va_user_info['access']))
                    );
                }
            }
            ...
            if (is_array($va_groups) && (sizeof($va_groups) > 0)) {
                ...
                foreach ($va_groups as $va_group_info) {
                    ...
                    $vo_permission->setAttribute(
                        "access",
                        $this->_convertUserGroupAccessToString(
                            intval($va_group_info['access'])
                        )
                    );
                }
            }
            ...
            $qr_screens = $this->opo_db->query(
                "SELECT * FROM ca_editor_ui_screens WHERE parent_id IS NOT NULL AND ui_id=? ORDER BY `rank`,screen_id",
                $qr_uis->get("ui_id")
            );
            ...
            while ($qr_screens->nextRow()) {
                ...
                if ($this->opn_modified_after) {
                    if ($t_screen->getLastChangeTimestampAsInt(
                ...
                $vo_labels = $this->opo_dom->createElement("labels");
                $qr_screen_labels = $this->opo_db->query(
                    "SELECT * FROM ca_editor_ui_screen_labels WHERE screen_id=?",
                    $qr_screens->get("screen_id")
                );
                if ($qr_ui_labels->numRows() > 0) {
                    while ($qr_screen_labels->nextRow()) {
                        if ($vs_locale = $this->opt_locale->localeIDToCode($qr_screen_labels->get("locale_id"))) {
                            ...
                            $vo_label->setAttribute("locale", $vs_locale);
                            $vo_label->appendChild(
                                $this->opo_dom->createElement("name", caEscapeForXML($qr_screen_labels->get("name")))
                            );
                ...
                if (is_array($va_placements)) {
                    ...
                    foreach ($va_placements as $va_placement) {
                        ...
                        $vo_placement->setAttribute(
                            "code",
                            $vs_code = $this->makeIDNO(
                        ...
                        if (isset($va_placement['settings']['bundleTypeRestrictions']) && (is_array(
                                    $va_type_restrictions = $va_placement['settings']['bundleTypeRestrictions']
                            ...
                            if (is_array($va_type_restrictions) && (sizeof($va_type_restrictions) > 0)) {
                                $vo_placement->setAttribute(
                                    "typeRestrictions",
                                    join(",", caMakeTypeList($vs_type, $va_type_restrictions))
                                );
                            }
                        }
                        ...
                        $vo_placement->appendChild(
                            $this->opo_dom->createElement("bundle", caEscapeForXML($va_placement["bundle"]))
                        );
    ...
    public function getRelationshipTypesAsDOM()
    {
        ...
        while ($qr_tables->nextRow()) {
            ...
            if ($qr_root->nextRow()) {
                ...
                if ($vo_types = $this->getRelationshipTypesForParentAsDOM(
                    $vn_parent,
    ...
    private function getRelationshipTypesForParentAsDOM($pn_parent_id, $pn_table_num)
    {
        ...
        while ($qr_types->nextRow()) {
            ...
            $vo_labels = $this->opo_dom->createElement("labels");
            $qr_type_labels = $this->opo_db->query(
                "SELECT * FROM ca_relationship_type_labels WHERE type_id = ?",
            ...
            while ($qr_type_labels->nextRow()) {
                ...
                $vo_label->appendChild(
                    $this->opo_dom->createElement("typename", caEscapeForXML($qr_type_labels->get("typename")))
                );
                $vo_label->appendChild(
                    $this->opo_dom->createElement(
                        "typename_reverse",
    ...
    public function getRolesAsDOM()
    {
        ...
        if ($this->opn_modified_after && is_array(
                $va_deleted = $this->getDeletedItemsFromChangeLogByIdno($t_role->tableNum(), 'code')
            )) {
            foreach ($va_deleted as $vs_deleted_idno) {
        ...
        while ($qr_roles->nextRow()) {
            ...
            if (is_array($va_vars['bundle_access_settings'])) {
                ...
                foreach ($va_vars['bundle_access_settings'] as $vs_bundle => $vn_val) {
                    ...
                    if ($t_ui_screens->isAvailableBundle(
                        $vs_table_name,
                        $vs_bundle_name
    ...
    public function getGroupsAsDOM()
    {
        ...
        if ($this->opn_modified_after && is_array(
                $va_deleted = $this->getDeletedItemsFromChangeLogByIdno($t_group->tableNum(), 'code')
            )) {
            foreach ($va_deleted as $vs_deleted_idno) {
        ...
        while ($qr_groups->nextRow()) {
            ...
            $vo_group->appendChild(
                $this->opo_dom->createElement("description", caEscapeForXML($t_group->get("description")))
            );
    ...
    public function getSearchFormsAsDOM()
    {
        ...
        if ($this->opn_modified_after && is_array(
                $va_deleted = $this->getDeletedItemsFromChangeLogByIdno($t_form->tableNum(), 'form_code')
            )) {
            foreach ($va_deleted as $vs_deleted_idno) {
        ...
        while ($qr_forms->nextRow()) {
            ...
            $vo_labels = $this->opo_dom->createElement("labels");
            $qr_form_labels = $this->opo_db->query(
                "SELECT * FROM ca_search_form_labels WHERE form_id=?",
            ...
            while ($qr_form_labels->nextRow()) {
                ...
                $vo_label->appendChild(
                    $this->opo_dom->createElement("name", caEscapeForXML($qr_form_labels->get("name")))
                );
            ...
            $vo_placements = $this->opo_dom->createElement("bundlePlacements");
            $qr_placements = $this->opo_db->query(
                "SELECT * FROM ca_search_form_placements WHERE form_id=? ORDER BY placement_id",
                $qr_forms->get("form_id")
            );
            while ($qr_placements->nextRow()) {
                ...
                $vo_placements->appendChild($vo_placement);
                $vo_placement->appendChild(
                    $this->opo_dom->createElement("bundle", caEscapeForXML($qr_placements->get("bundle_name")))
                );
    ...
    public function getDisplaysAsXML()
    {
        ...
        foreach ($va_displays as $vn_i => $va_display_by_locale) {
            ...
            if (is_array($va_restrictions = $t_display->getTypeRestrictions()) && sizeof($va_restrictions)) {
                ...
                if (is_array($va_type_ids) && sizeof($va_type_ids)) {
                    $vs_type_restriction_attr = "typeRestrictions=\"" . join(
                            ",",
                            caMakeTypeList(
                                $t_display->get('table_num'),
                                $va_type_ids
                            )
                        ) . "\" includeSubtypes=\"" . ($vb_include_subtypes ? 1 : 0) . "\"";
                }
            }

            $vs_buf .= "\t<display code='" . ($va_info['display_code'] && preg_match(
                    '!^[A-Za-z0-9_]+$!',
                    $va_info['display_code']
                ) ? $va_info['display_code'] : 'display_' . $va_info['display_id']) . "' type='" . Datamodel::getTableName(
                    $va_info['table_num']
                ) . "' system='" . $t_display->get('is_system') . "' {$vs_type_restriction_attr}>\n";
            $vs_buf .= "\t\t<labels>\n";
            foreach ($va_display_by_locale as $vn_locale_id => $va_display_info) {
                if (strlen($this->opt_locale->localeIDToCode($vn_locale_id)) > 0) {
                    $vs_buf .= "\t\t\t<label locale='" . $this->opt_locale->localeIDToCode(
                            $vn_locale_id
                        ) . "'><name>" . caEscapeForXML($va_display_info['name']) . "</name></label>\n";
                }
            }
            $vs_buf .= "\t\t</labels>\n";
            ...
            if (is_array($va_users) && (sizeof($va_users) > 0)) {
                $vs_buf .= "\t\t<userAccess>\n";
                foreach ($va_users as $va_user_info) {
                    $vs_buf .= "\t\t\t<permission user='" . $va_user_info["user_name"] . "' access='" . $this->_convertUserGroupAccessToString(
                            intval($va_user_info['access'])
                        ) . "'/>\n";
                }
            ...
            if (is_array($va_groups) && (sizeof($va_groups) > 0)) {
                $vs_buf .= "\t\t<groupAccess>\n";
                foreach ($va_groups as $va_group_info) {
                    $vs_buf .= "\t\t\t<permission group='" . $va_group_info["code"] . "' access='" . $this->_convertUserGroupAccessToString(
                            intval($va_group_info['access'])
                        ) . "'/>\n";
                }
            ...
            foreach ($va_placements as $vn_placement_id => $va_placement_info) {
                $vs_buf .= "\t\t<placement code='" . preg_replace(
                        "![^A-Za-z0-9_]+!",
                        "_",
                        $va_placement_info['bundle_name']
                    ) . "'><bundle>" . $va_placement_info['bundle_name'] . "</bundle>\n";
                            ...
                            case 'label':
                                if (is_array($vm_value)) {
                                    foreach ($vm_value as $vn_locale_id => $vm_locale_specific_value) {
                                        ...
                                        $vs_buf .= "<setting name='label' locale='" . $vs_locale_code . "'>" . caEscapeForXML(
                                                $vm_locale_specific_value
                                            ) . "</setting>\n";
                                    }
                                }
                                break;
