...
class TaskQueue extends BaseObject {
	...
	function __construct() {
		...
		$va_tmp
			                           = $this->opo_app_plugin_manager->hookRegisterTaskQueuePluginDirectories( array(
			'handler_plugin_directories' => $this->opa_handler_plugin_dirs,
			'instance'                   => $this
		) );
	...
	function addTask( $ps_handler, $pa_parameters, $pa_options ) {
		...
		$o_db->query( "
			...
			VALUES
			(?, ?, ?, ?, NULL, NULL, ?, ?, ?, ?)
		", $vn_user_id, md5( $pa_options["entity_key"] ), md5( $pa_options["row_key"] ), time(), $vn_priority,
			$ps_handler, base64_encode( serialize( $pa_parameters ) ),
			$pa_options["notes"] ? $pa_options["notes"] : '' );
		if ( $o_db->numErrors() ) {
		...
		if ( $tmpdir = $this->opo_config->get( "taskqueue_tmp_directory" ) ) {
			if ( ! file_exists( $tmpdir . '/' . $ps_handler ) ) {
				if ( ! mkdir( $tmpdir . '/' . $ps_handler ) ) {
					$this->postError( 100, _t( "Could not create tmp directory for handler '%1'", $ps_handler ),
						"TaskQueue->copyFileToQueueTmp()" );
	...
	function resetUnfinishedTasks() {
		...
		while ( $qr_unfinished->nextRow() ) {
			...
			$o_db->query( "UPDATE ca_task_queue SET started_on = NULL WHERE task_id = ?",
				$qr_unfinished->get( 'task_id' ) );
		}
	}
	# ---------------------------------------------------------------------------
	...
	function processQueue( $ps_handler = "" ) {
		...
		while ( ( $vn_num_rows > 0 ) && ( $vn_processed_count <= $vn_max_process_count ) ) {
			...
			if ( ( $vn_num_rows = $qr_tasks->numRows() ) > 0 ) {
				...
				$this->updateRegisteredProcess( $vn_proc_id, $qr_tasks->get( 'row_key' ),
					$qr_tasks->get( 'entity_key' ) );
				...
				if ( ! $vb_found_handler ) {
					$this->opo_eventlog->log( array(
						"CODE"    => "ERR",
						"SOURCE"  => "TaskQueue->processQueue()",
						"MESSAGE" => "Queue processing failed because of invalid task queue handler '$proc_handler'; queue was halted"
					) );
					$this->postError( 500, _t( "Invalid task queue handler '%1'", $proc_handler ),
				...
				if ( $o_db->numErrors() ) {
					$this->opo_eventlog->log( array(
							"CODE"    => "ERR",
							"SOURCE"  => "TaskQueue->processQueue()",
							"MESSAGE" => "Queue processing failed while closing task record using {$proc_handler}: "
							             . join( '; ', $o_db->getErrors() ) . "; queue was halted"
						)
					);
					$this->postError( 515,
						_t( "Error while closing task record: %1", join( '; ', $o_db->getErrors() ) ),
						"TaskQueue->processQueue()" );
	...
	function cancelPendingTasksForEntity( $ps_entity_key, $ps_handler = "" ) {
		...
		if ( ! sizeof( $this->opa_handler_plugin_dirs ) ) {
			$this->opo_eventlog->log( array(
				...
				"MESSAGE" => "Cancelling of task failed because no handler directories are configured"
			) );
		...
		while ( $qr_tasks->nextRow() ) {
			...
			if ( ! $vb_found_handler ) {
				$this->opo_eventlog->log( array(
					"CODE"    => "ERR",
					"SOURCE"  => "TaskQueue->cancelPendingTasks()",
					"MESSAGE" => "Cancelling of task failed because of invalid task queue handler '{$proc_handler}'; plugin directories were "
					             . join( '; ', $this->opa_handler_plugin_dirs )
				) );
				$this->postError( 500, _t( "Invalid task queue handler '%1'", $proc_handler ),
					"TaskQueue->cancelPendingTasks()" );
