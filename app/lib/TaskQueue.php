...
class TaskQueue extends BaseObject
{
    ...
    function __construct()
    {
        ...
        $va_tmp = $this->opo_app_plugin_manager->hookRegisterTaskQueuePluginDirectories(
            array('handler_plugin_directories' => $this->opa_handler_plugin_dirs, 'instance' => $this)
        );
    ...
    function addTask($ps_handler, $pa_parameters, $pa_options)
    {
		...
		",
            $vn_user_id,
            md5($pa_options["entity_key"]),
            md5($pa_options["row_key"]),
            time(),
            $vn_priority,
            $ps_handler,
            base64_encode(serialize($pa_parameters)),
        ...
        if ($tmpdir = $this->opo_config->get("taskqueue_tmp_directory")) {
            if (!file_exists($tmpdir . '/' . $ps_handler)) {
                if (!mkdir($tmpdir . '/' . $ps_handler)) {
                    $this->postError(
                        100,
                        _t("Could not create tmp directory for handler '%1'", $ps_handler),
        ...
        while (($vn_num_rows > 0) && ($vn_processed_count <= $vn_max_process_count)) {
            ...
            if (($vn_num_rows = $qr_tasks->numRows()) > 0) {
                ...
                if (!$vb_found_handler) {
                    ...
                    $this->postError(
                        500,
                        _t("Invalid task queue handler '%1'", $proc_handler),
                ...
                if ($va_report = $h->process($proc_parameters)) {
                    ...
                    $o_db->query(
                        ...
                        ,
                        time(),
                        caSerializeForDatabase($va_report),
                        (int)$qr_tasks->get("task_id")
                    );
                } else {
                    $this->opo_eventlog->log(
                        array(
                            "CODE" => "ERR",
                            "SOURCE" => "TaskQueue->processQueue()",
                            "MESSAGE" => "Queue processing failed using handler $proc_handler: " . ($h->error ? $h->error->getErrorDescription(
                                ) : '') . " [" . $h->error->getErrorNumber() . "]; queue was <b>NOT</b> halted"
                        )
                    );
                    $this->errors[] = $h->error;
                ...
                if ($o_db->numErrors()) {
                    $this->opo_eventlog->log(
                        array(
                            "CODE" => "ERR",
                            "SOURCE" => "TaskQueue->processQueue()",
                            "MESSAGE" => "Queue processing failed while closing task record using {$proc_handler}: " . join(
                                    '; ',
                                    $o_db->getErrors()
                                ) . "; queue was halted"
                        )
            ...
            if (!$vb_found_handler) {
                $this->opo_eventlog->log(
                    array(
                        "CODE" => "ERR",
                        "SOURCE" => "TaskQueue->cancelPendingTasks()",
                        "MESSAGE" => "Cancelling of task failed because of invalid task queue handler '{$proc_handler}'; plugin directories were " . join(
                                '; ',
                                $this->opa_handler_plugin_dirs
                            )
                    )
                );
                $this->postError(
