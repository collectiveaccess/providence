...
class BaseModelWithAttributes extends BaseModel implements ITakesAttributes
{
    ...
    public function editAttribute(
        ...
        if (sizeof($va_attr_values) != (sizeof(
                    $pa_values
                ) - 1)) {        // -1 to remove locale_id which is not in attribute values array
    ...
    public function getAttributeHTMLFormBundle(
        ...
        foreach ($va_element_set as $va_element) {
            ...
            $va_elements_by_container[$va_element['parent_id']][] = ($va_element['datatype'] == 0) ? '' : $vs_br . ca_attributes::attributeHtmlFormElement(
                    $va_element,
                    array_merge(
                        $pa_bundle_settings,
                        array_merge(
        ...
        if ($vb_should_output_locale_id) {    // output locale_id, if necessary, in its' own special '_locale_id' container
            $va_elements_by_container['_locale_id'] = array(
                'hidden' => false,
    ...
    public function htmlFormElementForAttributeSearch($po_request, $pm_element_code_or_id, $pa_options = null)
    {
        ...
        foreach ($va_element_set as $va_element) {
            ...
            $vs_subelement_code = $this->tableName() . '.' . ($vb_is_sub_element ? $t_parent->get(
                        'element_code'
