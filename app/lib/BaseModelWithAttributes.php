...
{
    # ------------------------------------------------------------------
    static public $s_applicable_element_code_cache = array();
    static public $s_element_label_cache = array();
    # ------------------------------------------------------------------
    ...
    public function addAttribute($pa_values, $pm_element_code_or_id, $ps_error_source = null, $pa_options = null)
    {
            ...
            if (($vn_max > 0) && $vn_count >= $vn_max) {
                if (caGetOption('showRepeatCountErrors', $pa_options, false)) {
                    $this->postError(
                        1965,
                        ($vn_max == 1) ? _t(
                            'Cannot add another value; only %1 value is allowed',
                            $vn_max
                        ) : _t('Cannot add another value; only %1 values are allowed', $vn_max),
                        'BaseModelWithAttributes->addAttribute()',
                        $ps_error_source
                    );
                }
                return null;
        ...
        if (caGetOption('skipExistingValues', $pa_options, false) || !caGetOption(
    ...
    public function _addAttribute($pa_values, $pm_element_code_or_id, $po_trans = null, $pa_info = null)
    {
        ...
        $vn_attribute_id = $t_attr->addAttribute(
            $this->tableNum(),
            $this->getPrimaryKey(),
        ...
        if ($t_attr->numErrors()) {
            foreach ($t_attr->errors as $o_error) {
                $this->postError(
                    $o_error->getErrorNumber(),
                    $o_error->getErrorDescription(),
                    $o_error->getErrorContext(),
    ...
    public function editAttribute(
                    ...
                    foreach ($o_attr->getValues() as $o_value) {
                        ...
                        if (
                            (
                                ...
                                &&
                                ((strlen($pa_values[$vn_element_id] && ($pa_values[$vn_element_id] != $pv))) || !strlen(
                                        $pa_values[$vn_element_id]
                                    ))
                            )
                            ||
                            (
                                ...
                                &&
                                ((strlen(
                                        $pa_values[$vs_element_code] && ($pa_values[$vs_element_code] != $pv)
        ...
        if (
            ...
            is_array(
                $elements = array_filter(
                    ca_metadata_elements::getElementsForSet($vn_attr_element_id, ['omitContainers' => true]),
                    function ($v) {
            ...
            foreach ($va_attr_values as $o_attr_value) {
                ...
                if (
                    ...
                    (
                        in_array($vn_element_datatype, [__CA_ATTRIBUTE_VALUE_MEDIA__, __CA_ATTRIBUTE_VALUE_FILE__])
                        &&
                        array_key_exists($vs_element_code, $pa_values) && is_array(
                    ...
                    (
                        in_array($vn_element_datatype, [__CA_ATTRIBUTE_VALUE_MEDIA__, __CA_ATTRIBUTE_VALUE_FILE__])
                        &&
                        array_key_exists($vn_element_id, $pa_values) && is_array($pa_values[$vn_element_id]) && sizeof(
                            $pa_values[$vn_element_id]
                        )
        ...
        if (!$ps_error_source) {
            $ps_error_source = $this->tableName() . '.' . ca_metadata_elements::getElementCodeForId(
                    $pm_element_code_or_id
                );
        }
    ...
    public function _editAttribute($pn_attribute_id, $pa_values, $po_trans = null, $pa_info = null)
    {
        ...
        if ((!$t_attr->getPrimaryKey()) || ($t_attr->get('table_num') != $this->tableNum()) || ($this->getPrimaryKey(
                ) != $t_attr->get('row_id'))) {
            $this->postError(
                1969,
                _t('Can\'t edit invalid attribute'),
                'BaseModelWithAttributes->editAttribute()',
                $pa_info['error_source']
            );
            return false;
        }
    ...
    public function removeAttribute(
        ...
        if (($vb_require_value = (bool)$t_element->getSetting(
                'requireValue'
        ...
        if (!caGetOption('dontCheckMinMax', $pa_options, false)) {
            ...
            $vn_count = $this->getAttributeCountByElement(
                    $t_element->getPrimaryKey(),
                    ['includeBlanks' => true]
            ...
            if ($vn_count <= $vn_min) {
                if (caGetOption('showRepeatCountErrors', $pa_options, false)) {
                    $this->postError(
                        1967,
                        ($vn_min == 1) ? _t(
                            'Cannot remove value; at least %1 value is required',
                            $vn_min
                        ) : _t('Cannot remove value; at least %1 values are required', $vn_min),
                        'BaseModelWithAttributes->removeAttribute()',
                        $ps_error_source
                    );
                }
                return null;
    ...
    private function _commitAttributes($po_trans = null)
    {
        ...
        foreach ($this->opa_attributes_to_add as $va_info) {
            if ((!($vn_attribute_id = $this->_addAttribute(
                    $va_info['values'],
                    $va_info['element'],
                    $po_trans,
                    $va_info
    ...
    private function addAttributesToFieldValuesArray()
    {
        ...
        foreach ($va_attributes as $o_attr) {
            foreach ($o_attr->getValues() as $o_attr_value) {
                $va_field_content['_ca_attribute_' . $o_attr->getElementID()][$o_attr->getAttributeID(
                )][$o_attr_value->getValueID()] = $o_attr_value->getDisplayValue();
            }
        }
    ...
    public function insert($pa_options = null)
    {
        ...
        if ($vn_id = parent::insert($pa_options)) {
            ...
            $this->doSearchIndexing(
                array_merge($this->getFieldValuesArray(true), $va_fields_changed_array),
                false,
                $va_index_options
            );
    ...
    public function delete($pb_delete_related = false, $pa_options = null, $pa_fields = null, $pa_table_list = null)
    {
        ...
        if (($vn_rc = parent::delete($pb_delete_related, $pa_options, $pa_fields, $pa_table_list)) && (!$this->hasField(
                    'deleted'
                ) || caGetOption('hard', $pa_options, false))) {
                            ...
                            if (is_array($va_values)) {
                                foreach ($va_values as $vs_sub_code => $vs_value) {
                                    ...
                                    switch ((int)$t_element->get('datatype')) {
                                        case 3:        // list
                                            $va_list_item = $t_list->getItemFromListByItemID(
    ...
    public function set($pa_fields, $pm_value = "", $pa_options = null)
    {
        if ($this->ATTRIBUTE_TYPE_LIST_CODE) {
            if (is_array($pa_fields)) {
                if (isset($pa_fields[$this->ATTRIBUTE_TYPE_ID_FLD]) && !is_numeric(
                        $pa_fields[$this->ATTRIBUTE_TYPE_ID_FLD]
                    )) {
                    if ($vn_id = ca_lists::getItemID(
                        $this->ATTRIBUTE_TYPE_LIST_CODE,
                        $pa_fields[$this->ATTRIBUTE_TYPE_ID_FLD]
                    )) {
        ...
        switch (sizeof($va_tmp)) {
            ...
            case 1:        // simple name
                if (!$t_instance->hasField($va_tmp[0])) {    // is it intrinsic?
                    ...
                    if (!$vb_return_as_array) {
                        ...
                        $va_values = $t_instance->getAttributeDisplayValues(
                            $va_tmp[0],
            ...
            case 2:        // table_name.field_name
                if ($va_tmp[0] === $t_instance->tableName()) {
                    if (!$t_instance->hasField(
                            $va_tmp[1]
                        ) && ($va_tmp[1] != 'created') && ($va_tmp[1] != 'lastModified')) {
                        ...
                        if (!$vb_return_as_array) {
                            ...
                            $va_values = $t_instance->getAttributeDisplayValues(
                                $va_tmp[1],
                                $vn_row_id = $t_instance->getPrimaryKey(),
                                $pa_options
                            );
                            if (!$vb_return_all_locales) {
                                $va_values = array_shift($va_values);
                                ...
                                if ($vs_template) {
                                    ...
                                    foreach ($va_values as $vn_i => $va_value_list) {
                                        $va_values_tmp[] = caProcessTemplateForIDs(
                                            $vs_template,
                                            $va_tmp[0],
                                            array($vn_row_id),
                                            array_merge(
                        ...
                        if (!$this->hasField($va_tmp[1])) {
                            if (($va_tmp[2] === 'hierarchy') || ($va_tmp[3] === 'hierarchy')) {
                                ...
                                if (in_array(
                                    ...
                                    array(__CA_ATTRIBUTE_VALUE_LIST__)
                                )) {
                                    $va_items = $this->get(
                                        join(
                                            '.',
                                            $vb_is_in_container ? array(
                                                $va_tmp[0],
                                                $va_tmp[1],
                                                $va_tmp[2]
                                            ) : array($va_tmp[0], $va_tmp[1])
                                        ),
                                        array('returnAsArray' => true)
                                    );
                                    $va_item_ids = $va_item_ids = caExtractValuesFromArrayList(
                                        $va_items,
                                        $vb_is_in_container ? $va_tmp[2] : $va_tmp[1],
                                ...
                                if ($vb_return_all_locales) {
                                    ...
                                    foreach ($va_values as $vn_id => $va_attribute_values) {
                                        foreach ($va_attribute_values as $vn_attribute_id => $va_data) {
                                            if (isset($va_data[$va_tmp[2]])) {
                                                if ($vs_template) {
                                                    $va_subvalues[$vn_attribute_id] = caProcessTemplateForIDs(
                                                        $vs_template,
                                                        $va_tmp[0],
                                                        array($vn_row_id),
                                                        array_merge(
    ...
    public function getSourceListAsHTMLFormElement($ps_name, $pa_attributes = null, $pa_options = null)
    {
        ...
        if (isset($pa_options['restrictToTypes']) && is_array($pa_options['restrictToTypes'])) {
            if (!$pa_options['limitToItemsWithID'] || !is_array($pa_options['limitToItemsWithID'])) {
                ...
                $pa_options['limitToItemsWithID'] = array_intersect(
                    $pa_options['limitToItemsWithID'],
    ...
    public function getTypeID($pn_id = null)
    {
        ...
        if ($pn_id) {
            $qr_res = $this->getDb()->query(
                "SELECT {$this->ATTRIBUTE_TYPE_ID_FLD} FROM " . $this->tableName() . " WHERE " . $this->primaryKey(
                ) . " = ?",
                array((int)$pn_id)
            );
            if ($qr_res->nextRow()) {
    ...
    {
        $t_list = new ca_lists();
        return $t_list->getDefaultItemID(
    ...
    public function getTypeListAsHTMLFormElement($ps_name, $pa_attributes = null, $pa_options = null)
    {
        ...
        if (caGetOption('inUse', $pa_options, false)) {
            $vs_access_sql = '';
            $va_sql_params = array();
            if (($va_check_access = caGetOption('checkAccess', $pa_options, null)) && is_array(
                    $va_check_access
                ) && sizeof($va_check_access) && $this->hasField('access')) {
                array_walk(
                    $va_check_access,
                    function (&$pm_item, $ps_key) {
                        $pm_item = (int)$pm_item;
                    }
                );
                ...
                $qr_types_in_use = $this->getDb()->query(
                    "SELECT DISTINCT type_id FROM " . $this->tableName() . ($this->hasField(
                        'deleted'
                    ) ? " WHERE deleted = 0 {$vs_access_sql}" : ""),
                    $va_sql_params
                );
            }
        ...
        return $t_list->getListAsHTMLFormElement(
            $this->getTypeListCode(),
            $ps_name,
            $pa_attributes,
            array_merge(
                ...
                [
                    'value' => caGetOption(
                        'value',
                        $pa_options,
                        $this->get($this->getTypeFieldName())
                    )
                ]
            )
        );
    ...
    public function getDisplayLabel($ps_field)
    {
        ...
        if ($va_tmp[0] == $this->tableName()) {
            if (!$this->hasField($va_tmp[1]) && !in_array(
                    $va_tmp[1],
                    array('created', 'modified', 'lastModified')
                ) && !in_array($va_tmp[0], array('created', 'modified', 'lastModified'))) {
                $va_tmp[1] = preg_replace(
                    '!^ca_attribute_!',
                    '',
                    $va_tmp[1]
                );    // if field space is a bundle placement-style bundlename (eg. ca_attribute_<element_code>) then strip it before trying to pull label
    ...
    public function htmlFormElementForSearch($po_request, $ps_field, $pa_options = null)
    {
        ...
        if ($va_tmp[1] == $this->getTypeFieldName()) {
            return $this->getTypeListAsHTMLFormElement(
                ...
                if (!caGetOption('nojs', $pa_options, false)) {
                    ...
                    return $vs_buf . caHTMLTextInput(
                            caGetOption(
                                ...
                                $pa_options,
                                $ps_field
                            ) . $vs_rel_types . $vs_autocomplete . ($vb_as_array_element ? "[]" : ""),
                            array(
        ...
        if (!in_array(
            ...
            if (!$this->hasField($vs_fld)) {
                ...
                return $this->htmlFormElementForAttributeSearch(
                    $po_request,
                    $vs_fld,
                    array_merge(
                        $pa_options,
                        array(
                            'values' => (isset($pa_options['values']) && is_array(
                                    $pa_options['values']
    ...
    public function htmlFormElementForSimpleForm($po_request, $ps_field, $pa_options = null)
    {
                ...
                array_merge(
                    ...
                    array(
                        ...
                        'width' => array_key_exists(
                            'width',
                            $pa_options
                        ) ? (($pa_options['width'] > 0) ? $pa_options['width'] : null) : 20,
                        'height' => array_key_exists(
                            'height',
                            $pa_options
                        ) ? (($pa_options['height'] > 0) ? $pa_options['height'] : null) : 1,
                        'class' => (isset($pa_options['class']) && $pa_options['class']) ? $pa_options['class'] : '',
        ...
        return parent::htmlFormElementForSimpleForm(
            $po_request,
            $ps_field,
            array_merge(
                $pa_options,
    ...
    public function getAttributeHTMLFormBundle(
        ...
        if ($t_element->get('parent_id')) {
            $this->postError(
                1930,
                _t('Element is not the root of the element set'),
        ...
        foreach ($va_element_set as $va_element) {
            ...
            if (isset($pa_bundle_settings['usewysiwygeditor']) && strlen(
                    $pa_bundle_settings['usewysiwygeditor']
                ) == 0) {
            ...
            $va_elements_by_container[$va_element['parent_id']][] = ($va_element['datatype'] == 0) ? '' :
                $vs_br . ca_attributes::attributeHtmlFormElement(
                    $va_element,
                    array_merge(
                        $pa_bundle_settings,
                        array_merge(
                            ...
                            [
                                ...
                                // Set format to single line when displaying yes_no checkboxes
                                    (($va_element['datatype'] == 3) && ($va_element['settings']['render'] === 'yes_no_checkboxes')) ? $this->getAppConfig(
            ...
            if (strlen($vs_setting)) {
                ...
                $va_element_value_defaults[$va_element['element_id']] = caProcessTemplate(
                    $tmp_element->getSetting($vs_setting),
                    $user_values
                );
            }
        }
        ...
        if ($vb_should_output_locale_id) {    // output locale_id, if necessary, in its' own special '_locale_id' container
            $va_elements_by_container['_locale_id'] = [
                'hidden' => false,
                'element' => $t_attr->htmlFormElement(
                    'locale_id',
                    '^ELEMENT',
                    [
        ...
        $va_attribute_list = $this->getAttributesByElement(
            $t_element->get('element_id'),
    ...
    public function htmlFormElementForAttributeSearch($po_request, $pm_element_code_or_id, $pa_options = null)
    {
        ...
        foreach ($va_element_set as $va_element) {
            ...
            $vs_subelement_code = $this->tableName(
                ) . '.' . ($policy ? "current_value.{$policy}." : '') . ($vb_is_sub_element ? $t_parent->get(
                        'element_code'
                    ) . '.' : '') . (($vs_element_code !== $va_element['element_code']) ? "{$vs_element_code}." : "") . $va_element['element_code'];
            ...
            if (is_null($vm_values) && $use_current_row_value) {
                ...
                $vm_values = $this->get(
                    $vs_subelement_code,
                    [
                        'returnAsArray' => $is = ((ca_metadata_elements::getDataTypeForElementCode(
            ...
            if ($vs_force_value = caGetOption('force', $pa_options, false)) {
                ...
                $vs_form_element = str_replace(
                    '{fieldNamePrefix}' . $va_element['element_id'] . '_{n}',
                    str_replace('.', '_', $f),
                    $vs_form_element
                );
                $vs_form_element = str_replace(
                    '{fieldNamePrefix}' . $va_element['element_id'] . '_autocomplete{n}',
                    str_replace('.', '_', $f) . '_autocomplete',
                    $vs_form_element
                );
    ...
    public function getReferencedAttributeValues($pm_element_code_or_id, $pa_reference_limit_ids)
    {
        ...
        return ca_attributes::getReferencedAttributes(
            $this->getDb(),
            $this->tableNum(),
            $pa_reference_limit_ids,
            ...
            switch ($ps_field) {
                case $vs_source_id_fld_name:
                    if ((bool)$this->getAppConfig()->get('perform_source_access_checking')) {
                        ...
                        $pa_options['disableItemsWithID'] = caGetSourceRestrictionsForUser(
                            $this->tableName(),
                            array(
                                'access' => __CA_BUNDLE_ACCESS_READONLY__,
                                'exactAccess' => true
                            )
                        );
                ...
                case $vs_type_id_fld_name:
                    $pa_options['value'] = $this->get($ps_field);
                    $vs_element = $this->getTypeListAsHTMLFormElement(
                        $pa_options['name'],
                        array(
                            'id' => caGetOption(
                                'id',
                ...
                return str_replace(
                    ...
                    str_replace(
                        ...
                        str_replace(
                            "^EXTRA",
                            '',
                            str_replace("^ELEMENT", $vs_element, $ps_format)
                        )
                    )
                );
            }
    ...
    public function getAttributes($pa_options = null, $pa_element_ids = null)
    {
        ...
        if (is_array($pa_element_ids) && sizeof($pa_element_ids)) {
            ...
            $va_element_ids = $this->getApplicableElementCodes(
                method_exists($this, "getTypeID") ? $this->getTypeID() : null,
                false,
                false
            );
        }
    ...
    public function getAttributesByElement($pm_element_code_or_id, $pa_options = null)
    {
        ...
        $va_attribute_list = is_array(
            $va_attributes[$vn_hier_id = ca_metadata_elements::getElementHierarchyID($vn_element_id)]
        ) ? $va_attributes[$vn_hier_id] : array();

        $vs_sort_dir = (isset($pa_options['sort']) && (in_array(
                strtolower($pa_options['sortDirection']),
                array('asc', 'desc')
    ...
    public function getAttributeCountByElement($pm_element_code_or_id, $pa_options = null)
    {
        ...
        $vn_element_id = ca_metadata_elements::getElementID($pm_element_code_or_id);
        return ca_attributes::getAttributeCount(
            $this->getDb(),
            $this->tableNum(),
            $vn_row_id,
            $vn_element_id,
     ...
     *
     * @param $pm_element_code_or_id string|int -
     * @param $pn_row_id int -
        ...
        foreach ($va_attribute_list as $o_attribute) {
            ...
            foreach ($va_values as $o_value) {
                ...
                if (get_class($o_value) == 'ListAttributeValue') {
                    ...
                    $vn_list_id = (!isset($pa_options['convertCodesToDisplayText']) || !(bool)$pa_options['convertCodesToDisplayText']) ? null : $t_element->get(
                        'list_id'
                    );
                } else {
                    $vn_list_id = null;
                ...
                if ($ps_filter_expression) {
                    $va_raw_values[($vs_container_element_code == $vs_element_code) ? "{$vs_table_name}.{$vs_element_code}" : "{$vs_table_name}.{$vs_container_element_code}.{$vs_element_code}"] = $va_raw_values[$vs_element_code] = $o_value->getDisplayValue(
                        array('list_id' => $vn_list_id, 'returnIdno' => true)
                    );
                }
                ...
                if (isset($pa_options['convertLineBreaks']) && $pa_options['convertLineBreaks']) {
                    $vs_converted_value = preg_replace(
                        "!(\n|\r\n){2}!",
                        "<p/>",
                        $o_value->getDisplayValue(
                            array_merge($pa_options, array('list_id' => $vn_list_id))
                        )
                    );
                    $vs_display_value = preg_replace("![\n]{1}!", "<br/>", $vs_converted_value);
                } else {
                    $vs_display_value = $o_value->getDisplayValue(
                        array_merge($pa_options, array('list_id' => $vn_list_id))
                    );
                }
                ...
                if ($vs_sort_fld && ($vs_sort_fld == $vs_element_code)) {
                    $vs_sort_key = $o_value->getDisplayValue(
                        array_merge($pa_options, array('getDirectDate' => true, 'list_id' => $vn_list_id))
                    );
                }
            }
            if (!$vs_sort_key) {
    ...
    public function getRawValue(
        ...
        $vn_sub_element_id = $pm_sub_element_code_or_id ? ca_metadata_elements::getElementID(
            $pm_sub_element_code_or_id
        ) : null;
        ...
        foreach ($va_attributes as $o_attr) {
            if ($o_attr->getElementID() == $vn_element_id) {
                ...
                $va_ret_values[][(int)$o_attr->getLocaleID(
                )] = $va_values[$vn_sub_element_id ? $vn_sub_element_id : $vn_element_id];
            }
        }
    ...
    public function getAttributeForIDs($pm_element_code_or_id, $pa_ids, $pa_options = null)
    {
        ...
        return caExtractValuesByUserLocale(
            ca_attributes::getAttributeValueForIDs(
                $this->getDb(),
                $this->tableNum(),
                $pa_ids,
                $vn_element_id,
    ...
    public function getAttributesForDisplay($pm_element_code_or_id, $ps_template = null, $pa_options = null)
    {
        ...
        $va_tmp = $this->getAttributeDisplayValues(
            $vn_hier_id = ca_metadata_elements::getElementHierarchyID($pm_element_code_or_id),
            $vn_row_id,
            array_merge($pa_options, array('returnAllLocales' => false))
        );

        if (!$ps_template && ($vs_template_tmp = $t_element->getSetting('displayTemplate', true)) && !caGetOption(
                'dontUseElementTemplate',
                $pa_options,
                false
    ...
    public function getRawAttributeValuesForIDs($pm_element_code_or_id, $pa_ids, $pa_options = null)
    {
        ...
        return ca_attributes::getRawAttributeValuesForIDs(
            $this->getDb(),
            $this->tableNum(),
            $pa_ids,
            $vn_element_id,
    ...
    public function addMetadataElementToType($pm_element_code_or_id, $pn_type_id)
    {
        ...
        if ($t_restriction->numErrors()) {
            $this->postError(
                1980,
                _t(
                    "Couldn't add element to restriction list: %1",
        ...
        if ($t_restriction->load(
            ...
            if ($t_restriction->numErrors()) {
                $this->postError(
                    1981,
                    _t(
                        "Couldn't remove element from restriction list: %1",
                        join('; ', $t_restriction->getErrors())
                    ),
                    'BaseModelWithAttributes->addMetadataElementToType()'
                );
    ...
    public function getAuthorityElementReferences($pa_options = null)
    {
        ...
        foreach ($va_references as $vn_table_num => $va_rows) {
            ...
            if ((sizeof($va_row_ids) > 0) && $t_instance = Datamodel::getInstanceByTableNum($vn_table_num, true)) {
                ...
                $qr_del = $o_db->query(
                    "SELECT {$vs_pk} FROM " . $t_instance->tableName(
                    ) . " WHERE {$vs_pk} IN (?)" . ($t_instance->hasField('deleted') ? "AND deleted = 1" : ''),
                    array($va_row_ids)
                );
    ...
    public function moveAttributes($pn_to_id, $pa_element_codes_or_ids, $pa_options = null)
    {
        ...
        if (is_array($pa_element_codes_or_ids) && sizeof($pa_element_codes_or_ids)) {
            ...
            foreach ($pa_element_codes_or_ids as $vm_element) {
                ...
                if (!$t_instance->hasElement(
                    ca_metadata_elements::getElementCodeForId(
    ...
    public static function valueExistsForElement($pm_element_code_or_id, $ps_value, $pa_options = null)
    {
        ...
        $qr_values = $o_db->query(
            "
				...
				WHERE 
					cav.element_id = ? AND cav.value_longtext1 = ? {$vs_value_sql}",
            $va_sql_params
        );
    ...
    public function getApplicableElementCodes(
        ...
        if (!$pb_dont_cache && is_array(
                $va_tmp = BaseModelWithAttributes::$s_applicable_element_code_cache[$this->tableNum(
                ) . '/' . $pn_type_id . '/' . ($pb_include_sub_element_codes ? 1 : 0)]
            )) {
            return $va_tmp;
        }
        ...
        if (
            ...
            if ($t_type_instance = $this->getTypeInstance()) {
                $va_ancestors = $t_type_instance->getHierarchyAncestors(
                    null,
            ...
            if (is_array($va_ancestors) && (sizeof($va_ancestors) > 1)) {
                $vs_type_sql = '((camtr.type_id = ' . intval(
                        $pn_type_id
                    ) . ') OR (camtr.type_id IS NULL) OR ((camtr.include_subtypes = 1) AND camtr.type_id IN (' . join(
                        ',',
                        $va_ancestors
                    ) . '))) AND ';
            } else {
        ...
        BaseModelWithAttributes::$s_applicable_element_code_cache[$this->tableNum(
        ) . '/' . $pn_type_id . '/' . ($pb_include_sub_element_codes ? 1 : 0)] = $va_codes;
        return $va_codes;
    }
    # ------------------------------------------------------------------
    ...
    {
        $t_restriction = new ca_metadata_type_restrictions();
        if (is_subclass_of($this, 'BaseRelationshipModel') && ($t_restriction->load(
                array(
            ...
            array('element_id' => (int)$pn_element_id, 'table_num' => (int)$this->tableNum(), 'type_id' => null)
        )) {
            return $t_restriction;
        }
    ...
    {
        $this->opa_failed_attribute_inserts[ca_metadata_elements::getElementID(
            $pm_element_code_or_id
        )] = $pa_attribute_values;
    }
    ...
    {
        $this->opa_failed_attribute_updates[ca_metadata_elements::getElementID(
            $pm_element_code_or_id
        )] = $pa_attribute_values;
    }
