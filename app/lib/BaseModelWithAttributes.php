...
class BaseModelWithAttributes extends BaseModel implements ITakesAttributes {
	 ...
	 *                           skipExistingValues = Don't add values that already exist on this record. [Default is
	 *                           false] showRepeatCountErrors = Emit visible errors when minimum/maximum repeat counts
	 *                           are exceeded. [Default is false] dontCheckMinMax = Don't do minimum/maximum repeat
	 *                           count checks. [Default is false]
	 *
	...
	public function addAttribute( $pa_values, $pm_element_code_or_id, $ps_error_source = null, $pa_options = null ) {
		...
		if ( ! caGetOption( 'dontCheckMinMax', $pa_options, false ) ) {
			...
			if ( ( $vn_max > 0 ) && $vn_count >= $vn_max ) {
				if ( caGetOption( 'showRepeatCountErrors', $pa_options, false ) ) {
					$this->postError( 1965,
						( $vn_max == 1 ) ? _t( 'Cannot add another value; only %1 value is allowed', $vn_max )
							: _t( 'Cannot add another value; only %1 values are allowed', $vn_max ),
						'BaseModelWithAttributes->addAttribute()', $ps_error_source );
				}
		...
		if ( caGetOption( 'skipExistingValues', $pa_options, false )
		     || ! caGetOption( 'allowDuplicateValues', $element_info, false )
		) {
		...
		$this->opa_attributes_to_add[]                                   = array(
			...
			'options'      => array_merge( $pa_options, [ 'skipExistingValues' => false ] )
			// don't invoke low-level value skipping
		);
	...
	public function _addAttribute( $pa_values, $pm_element_code_or_id, $po_trans = null, $pa_info = null ) {
		...
		$vn_attribute_id = $t_attr->addAttribute( $this->tableNum(), $this->getPrimaryKey(),
			$t_element->getPrimaryKey(), $pa_values, $pa_info['options'] );
		if ( $t_attr->numErrors() ) {
			foreach ( $t_attr->errors as $o_error ) {
				$this->postError( $o_error->getErrorNumber(), $o_error->getErrorDescription(),
					$o_error->getErrorContext(), $pa_info['error_source'] );
			}
	...
	public function editAttribute(
		$pn_attribute_id, $pm_element_code_or_id, $pa_values, $ps_error_source = null, $pa_options = null
	) {
					...
					foreach ( $o_attr->getValues() as $o_value ) {
						...
						if (
							(
								...
								&& ( ( strlen( $pa_values[ $vn_element_id ]
								               && ( $pa_values[ $vn_element_id ] != $pv ) ) )
								     || ! strlen( $pa_values[ $vn_element_id ] ) )
							)
							|| (
								array_key_exists( $vs_element_code, $pa_values )
								&& ( ( strlen( $pa_values[ $vs_element_code ]
								               && ( $pa_values[ $vs_element_code ] != $pv ) ) )
								     || ! strlen( $pa_values[ $vs_element_code ] ) )
							)
						) {
							continue( 2 );
		...
		if (
			...
			is_array( $elements = array_filter( ca_metadata_elements::getElementsForSet( $vn_attr_element_id,
				[ 'omitContainers' => true ] ), function ( $v ) {
			...
			if ( ! $elements ) {
				$elements = array_filter( ca_metadata_elements::getElementsForSet( $vn_attr_element_id,
					[ 'omitContainers' => true ] ), function ( $v ) {
					return (int) $v['datatype'] !== 0;
				} );
			}
			...
			foreach ( $va_attr_values as $o_attr_value ) {
				...
				if (
					(
						array_key_exists( $vn_element_id, $pa_values )
						&& ( $pa_values[ $vn_element_id ] !== $o_attr_value->getDisplayValue() )
						&&
						! ( ( $pa_values[ $vn_element_id ] == "" ) && ( is_null( $o_attr_value->getDisplayValue() ) ) )
						&&
						! in_array( $vn_element_datatype,
							[ __CA_ATTRIBUTE_VALUE_MEDIA__, __CA_ATTRIBUTE_VALUE_FILE__ ] )
					)
					|| (
						array_key_exists( $vs_element_code, $pa_values )
						&& ( $pa_values[ $vs_element_code ] !== $o_attr_value->getDisplayValue() )
						&&
						! ( ( $pa_values[ $vs_element_code ] == "" )
						    && ( is_null( $o_attr_value->getDisplayValue() ) ) )
						&&
						! in_array( $vn_element_datatype,
							[ __CA_ATTRIBUTE_VALUE_MEDIA__, __CA_ATTRIBUTE_VALUE_FILE__ ] )
					)
					|| (
						...
						&& array_key_exists( $vs_element_code, $pa_values )
						&& is_array( $pa_values[ $vs_element_code ] )
						&& sizeof( $pa_values[ $vs_element_code ] )

					)
					|| (
						in_array( $vn_element_datatype, [ __CA_ATTRIBUTE_VALUE_MEDIA__, __CA_ATTRIBUTE_VALUE_FILE__ ] )
						&& array_key_exists( $vn_element_id, $pa_values )
						&& is_array( $pa_values[ $vn_element_id ] )
						&& sizeof( $pa_values[ $vn_element_id ] )
		...
		if ( ! $ps_error_source ) {
			$ps_error_source = $this->tableName() . '.'
			                   . ca_metadata_elements::getElementCodeForId( $pm_element_code_or_id );
		}

		if ( isset( $this->_FIELD_VALUE_CHANGED[ '_ca_attribute_' . $vn_attr_element_id ] )
		     && $this->_FIELD_VALUE_CHANGED[ '_ca_attribute_' . $vn_attr_element_id ]
		) {
			$this->opa_attributes_to_edit[] = array(
				'values'       => $pa_values,
	...
	public function _editAttribute( $pn_attribute_id, $pa_values, $po_trans = null, $pa_info = null ) {
		...
		if ( ( ! $t_attr->getPrimaryKey() ) || ( $t_attr->get( 'table_num' ) != $this->tableNum() )
		     || ( $this->getPrimaryKey() != $t_attr->get( 'row_id' ) )
		) {
			$this->postError( 1969, _t( 'Can\'t edit invalid attribute' ), 'BaseModelWithAttributes->editAttribute()',
				$pa_info['error_source'] );
	...
	public function removeAttribute(
		$pn_attribute_id, $ps_error_source = null, $pa_extra_info = null, $pa_options = null
	) {
		$t_attr = new ca_attributes( $pn_attribute_id );
		$t_attr->purify( $this->purify() );
		if ( ! $t_attr->getPrimaryKey() ) {
		...
		if ( ( $vb_require_value = (bool) $t_element->getSetting( 'requireValue' ) )
		     && ( ! isset( $pa_options['dontCheckMinMax'] ) )
		) {
		...
		if ( ! caGetOption( 'dontCheckMinMax', $pa_options, false ) ) {
			...
			$vn_count = $this->getAttributeCountByElement( $t_element->getPrimaryKey(), [ 'includeBlanks' => true ] )
			            + $vn_add_cnt - $vn_del_cnt;
			if ( $vn_count <= $vn_min ) {
				if ( caGetOption( 'showRepeatCountErrors', $pa_options, false ) ) {
					$this->postError( 1967,
						( $vn_min == 1 ) ? _t( 'Cannot remove value; at least %1 value is required', $vn_min )
							: _t( 'Cannot remove value; at least %1 values are required', $vn_min ),
						'BaseModelWithAttributes->removeAttribute()', $ps_error_source );
				}
	...
	public function _removeAttribute( $pn_attribute_id, $po_trans = null, $pa_options = null ) {
		...
		if ( ! $t_attr->removeAttribute() ) {
			foreach ( $t_attr->errors as $o_error ) {
				$this->postError( $o_error->getErrorNumber(), $o_error->getErrorDescription(),
					$o_error->getErrorContext(), $pa_options['error_source'] );
			}
	 ...
	 *
	 * Note that this method respects the minAttributesPerRow type restriction setting and will only delete attributes
	 * until the minimum permissible number of reached. If you wish to remove *all* attributes, ignoring any
	 * minAttributesPerRow constraints, pass the 'force' option set to true.
	 *
	 * @param mixed $pm_element_code_or_id
	 * @param array $pa_options Options are:
	 *                          force = if set to true all attributes are removed, even if there is a non-zero
	 *                          minAttributesPerRow constraint set
	 *
	...
	public function removeAttributes( $pm_element_code_or_id = null, $pa_options = null ) {
		...
		if ( $pm_element_code_or_id ) {
			...
			foreach ( $va_attributes as $o_attribute ) {
				$this->removeAttribute( $o_attribute->getAttributeID(), null, null,
					array( 'dontCheckMinMax' => isset( $pa_options['force'] ) && $pa_options['force'] ) );
			}
		} else {
			if ( is_array(
				$va_element_codes = $this->getApplicableElementCodes( $this->getTypeID(), false, false ) )
			) {
	...
	private function _commitAttributes( $po_trans = null ) {
		...
		foreach ( $this->opa_attributes_to_add as $va_info ) {
			if ( ( ! ( $vn_attribute_id = $this->_addAttribute( $va_info['values'], $va_info['element'], $po_trans,
					$va_info ) ) )
			     && ! is_null( $vn_attribute_id )
			) {
		...
		foreach ( $this->opa_attributes_to_edit as $va_info ) {
			if ( ! $this->_editAttribute( $va_info['attribute_id'], $va_info['values'], $po_trans, $va_info ) ) {
				$va_updated_attributes_that_errored[ $va_info['element'] ][ $va_info['attribute_id'] ]
					= $va_info['values'];
			}
		}
	...
	private function addAttributesToFieldValuesArray() {
		...
		foreach ( $va_attributes as $o_attr ) {
			foreach ( $o_attr->getValues() as $o_attr_value ) {
				$va_field_content[ '_ca_attribute_'
				                   . $o_attr->getElementID() ][ $o_attr->getAttributeID() ][ $o_attr_value->getValueID() ]
					= $o_attr_value->getDisplayValue();
			}
		}
	...
	public function insert( $pa_options = null ) {
		...
		$va_field_values
			= $this->getFieldValuesArray();        // get pre-insert field values (including attribute values)
		...
		if ( $vn_id = parent::insert( $pa_options ) ) {
			...
			$va_field_values_with_updated_attributes
				= $this->addAttributesToFieldValuesArray();    // copy committed attribute values to field values array
			...
			$this->doSearchIndexing( array_merge( $this->getFieldValuesArray( true ), $va_fields_changed_array ), false,
				$va_index_options );
	...
	public function update( $pa_options = null ) {
		...
		$va_field_values
			= $this->getFieldValuesArray();        // get pre-update field values (including attribute values)
	...
	public function delete( $pb_delete_related = false, $pa_options = null, $pa_fields = null, $pa_table_list = null ) {
		...
		if ( ( $vn_rc = parent::delete( $pb_delete_related, $pa_options, $pa_fields, $pa_table_list ) )
		     && ( ! $this->hasField( 'deleted' ) || caGetOption( 'hard', $pa_options, false ) )
		) {
	...
	public function getValuesForExport( $pa_options = null ) {
		...
		foreach ( $va_codes as $vs_code ) {
			if ( $va_v = $this->get( $this->tableName() . '.' . $vs_code, array(
							...
							if ( is_array( $va_values ) ) {
								foreach ( $va_values as $vs_sub_code => $vs_value ) {
									...
									switch ( (int) $t_element->get( 'datatype' ) ) {
										case 3:        // list
											$va_list_item
												      = $t_list->getItemFromListByItemID( $t_element->get( 'list_id' ),
												$vs_value );
											$vs_value = $vs_value . ":" . $va_list_item['idno'];
											break;
									}
									$va_data[ 'ca_attribute_' . $vs_code ][ $vs_locale ][ 'value_'
									                                                      . $vn_i ][ $vs_sub_code ]
										= $vs_value;
								}
								$vn_i ++;
								...
								$va_data[ 'ca_attribute_' . $vs_code ][ $vs_locale ][ 'value_' . $vn_i ][ $vs_code ]
									= $va_values;
							}
						}
					}
	...
	public function set( $pa_fields, $pm_value = "", $pa_options = null ) {
		if ( $this->ATTRIBUTE_TYPE_LIST_CODE ) {
			if ( is_array( $pa_fields ) ) {
				if ( isset( $pa_fields[ $this->ATTRIBUTE_TYPE_ID_FLD ] )
				     && ! is_numeric( $pa_fields[ $this->ATTRIBUTE_TYPE_ID_FLD ] )
				) {
					if ( $vn_id = ca_lists::getItemID( $this->ATTRIBUTE_TYPE_LIST_CODE,
						$pa_fields[ $this->ATTRIBUTE_TYPE_ID_FLD ] )
					) {
	...
	/**
	 * Get value(s) for specified attribute. $ps_field specifies the value to fetch in <table_name>.<element_code> or
	 * <table_name>.<element_code>.<subelement_code> Will return a string containing the retrieved value or values
	 * (since attributes can repeat). The values will be formatted using the 'template' option with values separated by
	 * a delimiter as set in the 'delimiter' option (default is a space).
	 *
	 * If the 'returnAsArray' option is set the an array containing all values will be returned.
	 * The array will be keyed on the current row primary key, and then attribute_id, with each attribute_id value
	 * containing an array keyed on element code and having values set to attribute values (this is a bit more
	 * complicated than one might hope since not only can values repeat, but they can be composed of many sub-values...
	 * the final array key'ed on element_code may have several values if the attribute is complex).
	 *
	 * If the 'returnAllLocales' option is set *and* 'returnAsArray' is set then the returned array will include an
	 * extra dimension (or key if that's what you prefer to call it) that separates values by numeric locale_id. Thus
	 * the returned array will have several layers of keys: current row primary key, then locale_id, then attribute_id
	 * and then finally, element codes. This format is, incidentally, compatible with the caExtractValuesByUserLocale()
	 * helper function, which would strip all values not needed for display in the current locale.
	 *
	 * @param $pa_options array - array of options for get; in addition to the standard get() options, will also pass
	 *                    through options to attribute value handlers Supported options include: locale = returnAsArray
	 *                    = if true, return an array, otherwise return a string (default is false) returnAllLocales =
	 *                    template = delimiter = convertCodesToDisplayText = returnAsLink = if true and $ps_field is a
	 *                    URL attribute and returnAllLocales is not set, then returned values will be links. Default is
	 *                    false. returnAsLinkText = *For URL attributes only* Text to use a content of HTML link. If
	 *                    omitted the url itself is used as the link content. returnAsLinkAttributes = array of
	 *                    attributes to include in link <a> tag. Use this to set class, alt and any other link
	 *                    attributes.
	 *
	 * @return mixed -
	 *
	...
	public function get( $ps_field, $pa_options = null ) {
		...
		$vb_return_as_array    = ( isset( $pa_options['returnAsArray'] ) ) ? (bool) $pa_options['returnAsArray']
			: false;
		$vb_return_all_locales = ( isset( $pa_options['returnAllLocales'] ) ) ? (bool) $pa_options['returnAllLocales']
			: false;

		$vb_return_as_link            = ( isset( $pa_options['returnAsLink'] ) ) ? (bool) $pa_options['returnAsLink']
			: false;
		$vs_return_as_link_text       = ( isset( $pa_options['returnAsLinkText'] ) )
			? (string) $pa_options['returnAsLinkText'] : '';
		$vs_return_as_link_attributes = ( isset( $pa_options['returnAsLinkAttributes'] ) )
			? (string) $pa_options['returnAsLinkAttributes'] : array();
		...
		$pa_options = array_merge( $pa_options,
			array( 'indexByRowID' => true ) );        // force arrays to be indexed by current row_id
		...
		switch ( sizeof( $va_tmp ) ) {
			...
			case 1:        // simple name
				if ( ! $t_instance->hasField( $va_tmp[0] ) ) {    // is it intrinsic?
					...
					if ( ! $vb_return_as_array ) {
						...
						$va_values = $t_instance->getAttributeDisplayValues( $va_tmp[0], $t_instance->getPrimaryKey(),
							$pa_options );
			...
			case 2:        // table_name.field_name
				if ( $va_tmp[0] === $t_instance->tableName() ) {
					if ( ! $t_instance->hasField( $va_tmp[1] ) && ( $va_tmp[1] != 'created' )
					     && ( $va_tmp[1] != 'lastModified' )
					) {
						// try it as an attribute
						if ( ! $vb_return_as_array ) {
							return $t_instance->getAttributesForDisplay( $va_tmp[1], $vs_template, $pa_options );
						} else {
							$va_values = $t_instance->getAttributeDisplayValues( $va_tmp[1],
								$vn_row_id = $t_instance->getPrimaryKey(), $pa_options );
							if ( ! $vb_return_all_locales ) {
								$va_values = array_shift( $va_values );
								...
								if ( $vs_template ) {
									...
									foreach ( $va_values as $vn_i => $va_value_list ) {
										$va_values_tmp[] = caProcessTemplateForIDs( $vs_template, $va_tmp[0],
											array( $vn_row_id ), array_merge( $pa_options, array(
												'returnAsArray'     => false,
												'placeholderPrefix' => array_slice( $va_tmp, 0, 2 )
											) ) );
									}
					...
					if ( $va_tmp[0] === $t_instance->tableName() ) {
						...
						if ( ! $this->hasField( $va_tmp[1] ) ) {
							if ( ( $va_tmp[2] === 'hierarchy' ) || ( $va_tmp[3] === 'hierarchy' ) ) {
								...
								if ( in_array( ca_metadata_elements::getElementDatatype( $vb_is_in_container
									? $va_tmp[2] : $va_tmp[1] ), array( __CA_ATTRIBUTE_VALUE_LIST__ ) )
								) {

									$va_items = $this->get( join( '.',
										$vb_is_in_container ? array( $va_tmp[0], $va_tmp[1], $va_tmp[2] )
											: array( $va_tmp[0], $va_tmp[1] ) ), array( 'returnAsArray' => true ) );
									$va_item_ids
									          = $va_item_ids = caExtractValuesFromArrayList( $va_items,
										$vb_is_in_container ? $va_tmp[2] : $va_tmp[1],
								...
								if ( $vb_return_all_locales ) {
										...
										foreach ( $va_attributes_by_locale as $vn_locale_id => $va_attribute_values ) {
											foreach ( $va_attribute_values as $vn_attribute_id => $va_data ) {
												if ( isset( $va_data[ $va_tmp[2] ] ) ) {
													$va_subvalues[ $vn_attribute_id ][ (int) $vn_locale_id ][ $vn_attribute_id ]
														= $va_data[ $va_tmp[2] ];
												}
											}
										}
									...
									foreach ( $va_values as $vn_id => $va_attribute_values ) {
										foreach ( $va_attribute_values as $vn_attribute_id => $va_data ) {
											if ( isset( $va_data[ $va_tmp[2] ] ) ) {
												if ( $vs_template ) {
													$va_subvalues[ $vn_attribute_id ]
														= caProcessTemplateForIDs( $vs_template, $va_tmp[0],
														array( $vn_row_id ), array_merge( $pa_options, array(
															'requireLinkTags'   => true,
															'returnAsArray'     => false,
															'placeholderPrefix' => array_slice( $va_tmp, 0, 2 )
														) ) );
												} else {
	 ...
	 *
	 * @return string - idno (aka "item code") for current row's source or null if no row is loaded or model does not
	 *                support sources
	 */
	 ...
	 *
	 * @param array $pa_options Array of options, passed as-is to ca_lists::getItemsForList() [the underlying
	 *                          implemenetation]
	 *
	 ...
	 * @param array  $pa_options    An array of options. Supported options are anything supported by
	 *                              ca_lists::getListAsHTMLFormElement as well as: childrenOfCurrentSourceOnly =
	 *                              Returns only sourcs below the current source restrictToSources = Array of
	 *                              source_ids to restrict source list to
	 *
	...
	public function getSourceListAsHTMLFormElement( $ps_name, $pa_attributes = null, $pa_options = null ) {
		...
		if ( isset( $pa_options['restrictToTypes'] ) && is_array( $pa_options['restrictToTypes'] ) ) {
			if ( ! $pa_options['limitToItemsWithID'] || ! is_array( $pa_options['limitToItemsWithID'] ) ) {
				...
				$pa_options['limitToItemsWithID'] = array_intersect( $pa_options['limitToItemsWithID'],
	 ...
	 *
	 * @param $pn_id Option primary key to fetch type_id for. If omitted the type_id for currently loaded row is
	...
	public function getTypeID( $pn_id = null ) {
		...
		if ( $pn_id ) {
			$qr_res = $this->getDb()->query( "SELECT {$this->ATTRIBUTE_TYPE_ID_FLD} FROM " . $this->tableName()
			                                 . " WHERE " . $this->primaryKey() . " = ?", array( (int) $pn_id ) );
			if ( $qr_res->nextRow() ) {
	...
	/**
	 * Field in this table that defines the type of the row; the type determines which attributes are applicable to the
	 * row
	 */
	 ...
	 *
	 * @return bool Returns true if type is optional (may be null in the database), false if it is mandatory or null if
	 *              the model does not support types.
	 */
	 ...
	 *
	 * @return string - idno (aka "item code") for current row's type or null if no row is loaded or model does not
	 *                support types
	 */
	...
	public function getDefaultTypeID() {
		...
		return $t_list->getDefaultItemID( $this->getTypeListCode(),
			[ 'omitRoot' => true, 'useFirstElementAsDefaultDefault' => true ] );
	}
	# ------------------------------------------------------------------
	 ...
	 * @param array  $pa_attributes An optional array of HTML attributes to place into the returned <select> tag
	 * @param array  $pa_options    An array of options. Supported options are anything supported by
	 *                              ca_lists::getListAsHTMLFormElement as well as: childrenOfCurrentTypeOnly = Returns
	 *                              only types below the current type restrictToTypes = Array of type_ids to restrict
	 *                              type list to inUse = Return only types that are used by at least one record.
	 *                              [Default is false] checkAccess = Array of access values to filter returned values
	 *                              on. Available for any related table with an "access" field (ca_objects,
	 *                              ca_entities, etc.). If omitted no filtering is performed. [Default is null]
	 *
	 * @return string HTML for list element
	 */
	public function getTypeListAsHTMLFormElement( $ps_name, $pa_attributes = null, $pa_options = null ) {
		...
		if ( caGetOption( 'inUse', $pa_options, false ) ) {
			$vs_access_sql = '';
			$va_sql_params = array();
			if ( ( $va_check_access = caGetOption( 'checkAccess', $pa_options, null ) ) && is_array( $va_check_access )
			     && sizeof( $va_check_access )
				...
				$qr_types_in_use = $this->getDb()->query( "SELECT DISTINCT type_id FROM " . $this->tableName()
				                                          . ( $this->hasField( 'deleted' )
						? " WHERE deleted = 0 {$vs_access_sql}" : "" ), $va_sql_params );
			}
		...
		return $t_list->getListAsHTMLFormElement( $this->getTypeListCode(), $ps_name, $pa_attributes,
			array_merge( $pa_options,
				[ 'value' => caGetOption( 'value', $pa_options, $this->get( $this->getTypeFieldName() ) ) ] ) );
	}
	# ------------------------------------------------------------------
	// --- Forms
	# ------------------------------------------------------------------
	/**
	 * Returns field label text for element specified by standard "get" bundle code (eg. <table_name>.<element_code>
	 * format)
	 */
	public function getDisplayLabel( $ps_field ) {
		...
		if ( $va_tmp[0] == $this->tableName() ) {
			if ( ! $this->hasField( $va_tmp[1] )
			     && ! in_array( $va_tmp[1], array( 'created', 'modified', 'lastModified' ) )
			     && ! in_array( $va_tmp[0], array( 'created', 'modified', 'lastModified' ) )
			) {
				$va_tmp[1] = preg_replace( '!^ca_attribute_!', '',
					$va_tmp[1] );    // if field space is a bundle placement-style bundlename (eg. ca_attribute_<element_code>) then strip it before trying to pull label
	...
	public function getDisplayDescription( $ps_field ) {
		...
		if ( ! $this->hasField( $va_tmp[1] ) && ! in_array( $va_tmp[1], array( 'created', 'modified', 'lastModified' ) )
		     && ! in_array( $va_tmp[0], array( 'created', 'modified', 'lastModified' ) )
		) {
			$va_tmp[1] = preg_replace( '!^ca_attribute_!', '',
				$va_tmp[1] );    // if field space is a bundle placement-style bundlename (eg. ca_attribute_<element_code>) then strip it before trying to pull label
	 ...
	 * <table_name>.<bundle_name> format) This method handles generation of search form widgets for all metadata
	 * elements bound to the primary table. If this method can't handle the bundle (because it is not a metadata
	 * element bound to the primary table...) it will pass the request to the superclass implementation of
	...
	public function htmlFormElementForSearch( $po_request, $ps_field, $pa_options = null ) {
		...
		if ( $va_tmp[1] == $this->getTypeFieldName() ) {
			return $this->getTypeListAsHTMLFormElement( $ps_field . $vs_rel_types,
				array( 'class' => caGetOption( 'class', $pa_options, null ) ),
				array_merge( $pa_options, array( 'nullOption' => '-' ) ) );
		}
		...
		if ( in_array( $va_tmp[1], array( 'preferred_labels', 'nonpreferred_labels' ) ) ) {
			...
			if ( $va_tmp[0] == $this->tableName() && $vs_autocomplete ) {
				...
				if ( ! caGetOption( 'nojs', $pa_options, false ) ) {
					...
					return $vs_buf . caHTMLTextInput( caGetOption( 'name', $pa_options, $ps_field ) . $vs_rel_types
				...
				return caHTMLTextInput( caGetOption( 'name', $pa_options, $ps_field ) . $vs_rel_types . $vs_autocomplete
				                        . ( $vb_as_array_element ? "[]" : "" ), array(
					'value' => $pa_options['values'][ $ps_field ],
					'class' => $pa_options['class'],
					'id'    => caGetOption( 'id', $pa_options,
						str_replace( '.', '_', caGetOption( 'name', $pa_options, $ps_field ) ) )
		...
		if ( ! in_array( $va_tmp[0],
			array( 'created', 'modified' ) )
		) {        // let change log searches filter down to BaseModel
			if ( $va_tmp[0] != $this->tableName() ) {
				return null;
			}
			$vs_fld = array_pop( $va_tmp );
			if ( ! $this->hasField( $vs_fld ) ) {
				$vs_fld = preg_replace( '!^ca_attribute_!', '',
					$vs_fld );    // if field space is a bundle placement-style bundlename (eg. ca_attribute_<element_code>) then strip it before trying to pull label

				return $this->htmlFormElementForAttributeSearch( $po_request, $vs_fld, array_merge( $pa_options, array(
					'values'           => ( isset( $pa_options['values'] ) && is_array( $pa_options['values'] ) )
	 ...
	 *
	 * This method handles generation of form widgets for all metadata elements bound to the primary table. If this
	 * method can't handle the bundle (because it is not a metadata element bound to the primary table...) it will pass
	 * the request to the superclass implementation of htmlFormElementForSimpleForm()
	 *
	 * @param $po_request HTTPRequest
	 * @param $ps_field   string
	 * @param $pa_options array Options include:
	...
	public function htmlFormElementForSimpleForm( $po_request, $ps_field, $pa_options = null ) {
		...
		if ( $va_tmp[1] == $this->getTypeFieldName() ) {
			return $this->getTypeListAsHTMLFormElement( $ps_field,
				( $use_current_row_value ? [ 'value' => $this->get( $this->getTypeFieldName() ) ] : null ),
				$pa_options );
		}
		...
		if ( ! $this->hasField( $va_tmp[1] ) ) {
			...
			return $this->htmlFormElementForAttributeSearch( $po_request, $vs_fld, array_merge( $pa_options, array(
				'values'           => ( isset( $pa_options['values'] ) && is_array( $pa_options['values'] ) )
					? $pa_options['values'] : array(),
				'width'            => array_key_exists( 'width', $pa_options ) ? ( ( $pa_options['width'] > 0 )
					? $pa_options['width'] : null ) : 20,
				'height'           => array_key_exists( 'height', $pa_options ) ? ( ( $pa_options['height'] > 0 )
					? $pa_options['height'] : null ) : 1,
				'class'            => ( isset( $pa_options['class'] ) && $pa_options['class'] ) ? $pa_options['class']
					: '',
				'name'             => ( isset( $pa_options['name'] ) && $pa_options['name'] ) ? $pa_options['name']
					: null,
		...
		return parent::htmlFormElementForSimpleForm( $po_request, $ps_field, array_merge( $pa_options,
			array( 'view' => caGetOption( 'view', $pa_options, 'ca_simple_form_attributes.php' ) ) ) );
	}
	# ------------------------------------------------------------------
	 ...
	 *
	 * @param $pa_options array Options include:
	 *                    elementsOnly = Render only form elements and structural formattings, but not headers,
	 *                    enclosing divs or controls. [Default is false] batch = render for batch editor. [Default is
	 *                    false]
	 *
	 */
	public function getAttributeHTMLFormBundle(
		...
		if ( $t_element->get( 'parent_id' ) ) {
			$this->postError( 1930, _t( 'Element is not the root of the element set' ),
				'BaseModelWithAttributes->getAttributeHTMLFormBundle()' );
		...
		foreach ( $va_element_set as $va_element ) {
			if ( $va_element['datatype'] == 0 ) {        // containers are not active form elements
				if ( isset( $va_element['settings'] )
				     && isset( $va_element["settings"]["lineBreakAfterNumberOfElements"] )
				) {
					$va_elements_break_by_container[ $va_element['element_id'] ]
						= (int) $va_element["settings"]["lineBreakAfterNumberOfElements"];
				}
			}
		}
		...
		foreach ( $va_element_set as $va_element ) {
			...
			if ( isset( $va_elements_break_by_container[ $va_element['parent_id'] ] ) ) {
				if ( $va_elements_without_break_by_container[ $va_element['parent_id'] ]
				     == $va_elements_break_by_container[ $va_element['parent_id'] ] + 1
				) {
					$va_elements_without_break_by_container[ $va_element['parent_id'] ] = 1;
					$vs_br
					                                                                    = "</td></tr></table><table class=\"attributeListItem\"><tr><td class=\"attributeListItem\">";
				}
			}

			if ( isset( $pa_bundle_settings['usewysiwygeditor'] )
			     && strlen( $pa_bundle_settings['usewysiwygeditor'] ) == 0
			) {
			...
			$va_elements_by_container[ $va_element['parent_id'] ][] = ( $va_element['datatype'] == 0 )
				? ''
				:
				$vs_br . ca_attributes::attributeHtmlFormElement( $va_element,
					array_merge( $pa_bundle_settings, array_merge( $pa_options, [
						...
						// Set format to single line when displaying yes_no checkboxes
							( ( $va_element['datatype'] == 3 )
							  && ( $va_element['settings']['render'] === 'yes_no_checkboxes' ) ) ? $this->getAppConfig()
							                                                                            ->get( 'form_element_display_format_single_line' )
								: null
			...
			if ( strlen( $vs_setting ) ) {
				...
				$va_element_value_defaults[ $va_element['element_id'] ]
				             = caProcessTemplate( $tmp_element->getSetting( $vs_setting ), $user_values );
			}
		}
		...
		$o_view->setVar( 'render_mode',
			$t_element->getSetting( 'render' ) );    // only set for list attributes (as of 26 Sept 2010 at least)

		if ( $t_restriction = $this->getTypeRestrictionInstance( $t_element->get( 'element_id' ) ) ) {
			// If batch mode force minimums to zero
			$o_view->setVar( 'max_num_repeats',
				$vb_batch ? 9999 : $t_restriction->getSetting( 'maxAttributesPerRow' ) );

			$vn_min_repeats = $t_restriction->getSetting( 'minAttributesPerRow' );
			if ( ( $vn_min_repeats < 1 ) && ( isset( $va_element['settings']['requireValue'] ) )
			     && ( (bool) $va_element['settings']['requireValue'] )
			) {
				$vn_min_repeats = 1;
			}

			$o_view->setVar( 'min_num_repeats', $vb_batch ? 0 : $vn_min_repeats );
			$o_view->setVar( 'min_num_to_display',
				$vb_batch ? 1 : $t_restriction->getSetting( 'minimumAttributeBundlesToDisplay' ) );
		}
		...
		$va_attribute_list = $this->getAttributesByElement( $t_element->get( 'element_id' ),
			array( 'sort' => $vs_sort, 'sortDirection' => $vs_sort_dir ) );
	...
	public function htmlFormElementForAttributeSearch( $po_request, $pm_element_code_or_id, $pa_options = null ) {
		...
		$t_parent          = $vb_is_sub_element ? ca_metadata_elements::getInstance( $t_element->get( 'parent_id' ) )
			: null;
		...
		foreach ( $va_element_set as $va_element ) {
			...
			$vs_subelement_code = $this->tableName() . '.' . ( $policy ? "current_value.{$policy}." : '' )
			                      . ( $vb_is_sub_element ? $t_parent->get( 'element_code' ) . '.' : '' )
			                      . ( ( $vs_element_code !== $va_element['element_code'] ) ? "{$vs_element_code}."
					: "" ) . $va_element['element_code'];

			if ( is_null(
				$vm_values = ( isset( $pa_options['values'] ) && isset( $pa_options['values'][ $vs_subelement_code ] ) )
					? $pa_options['values'][ $vs_subelement_code ] : null )
			) {
			...
			if ( is_null( $vm_values ) && $use_current_row_value ) {
				...
				$vm_values = $this->get( $vs_subelement_code, [
					'returnAsArray' => $is
						= ( ( ca_metadata_elements::getDataTypeForElementCode( $va_element['element_code'] ) == 3 )
						    || $index_set )
				] );
				if ( $is && $index_set ) {
			...
			$va_element_opts = array_merge( array(
				...
				//(isset($va_element['settings']['render']) && ($va_element['settings']['render'] == 'lookup')) ? $va_element['settings']['render'] : isset($pa_options['render']) ? $pa_options['render'] : 'select'
			...
			if ( $vs_force_value = caGetOption( 'force', $pa_options, false ) ) {
				...
				$vs_form_element = str_replace( '{fieldNamePrefix}' . $va_element['element_id'] . '_{n}',
					str_replace( '.', '_', $f ), $vs_form_element );
				$vs_form_element = str_replace( '{fieldNamePrefix}' . $va_element['element_id'] . '_autocomplete{n}',
					str_replace( '.', '_', $f ) . '_autocomplete', $vs_form_element );
			...
			$va_elements_by_container[ $va_element['parent_id'] ? $va_element['parent_id']
	...
	public function getReferencedAttributeValues( $pm_element_code_or_id, $pa_reference_limit_ids ) {
		...
		return ca_attributes::getReferencedAttributes( $this->getDb(), $this->tableNum(), $pa_reference_limit_ids,
	...
	public function htmlFormElement( $ps_field, $ps_format = null, $pa_options = null ) {
		...
		if ( $vs_source_id_fld_name || $vs_type_id_fld_name ) {
			...
			switch ( $ps_field ) {
				case $vs_source_id_fld_name:
					if ( (bool) $this->getAppConfig()->get( 'perform_source_access_checking' ) ) {
						...
						$pa_options['disableItemsWithID'] = caGetSourceRestrictionsForUser( $this->tableName(),
							array( 'access' => __CA_BUNDLE_ACCESS_READONLY__, 'exactAccess' => true ) );
						$vs_element                       = $this->getSourceListAsHTMLFormElement( $pa_options['name'],
							array(), $pa_options );
					}
					break;
				case $vs_type_id_fld_name:
					$pa_options['value'] = $this->get( $ps_field );
					$vs_element          = $this->getTypeListAsHTMLFormElement( $pa_options['name'],
						array( 'id' => caGetOption( 'id', $pa_options, null ) ), $pa_options );
					break;
			}
			...
			if ( $vs_element ) {
				...
				return str_replace( "^DESCRIPTION", $vs_field_desc, str_replace( "^LABEL", $vs_field_label,
					str_replace( "^EXTRA", '', str_replace( "^ELEMENT", $vs_element, $ps_format ) ) ) );
			}
		}
	 ...
	 *
	 * @param array $pa_options Options include:
	 *                          row_id = Row to fetch attributes for. If omitted the currently loaded row is used.
	 *                          [Default is null]
	 *
	...
	public function getAttributes( $pa_options = null, $pa_element_ids = null ) {
		...
		if ( is_array( $pa_element_ids ) && sizeof( $pa_element_ids ) ) {
			...
			$va_element_ids = $this->getApplicableElementCodes( method_exists( $this, "getTypeID" ) ? $this->getTypeID()
				: null, false, false );
		}
		...
		$va_attributes                     = ca_attributes::getAttributes( $this->getDb(), $this->tableNum(),
			$vn_row_id, array_keys( $va_element_ids ), $pa_options );
	...
	public function getAttributesByElement( $pm_element_code_or_id, $pa_options = null ) {
		...
		$va_attributes = ca_attributes::getAttributes( $this->getDb(), $this->tableNum(), $vn_row_id,
			array( $vn_element_id ), $pa_options );

		$va_attribute_list = is_array( $va_attributes[ $vn_hier_id
			= ca_metadata_elements::getElementHierarchyID( $vn_element_id ) ] ) ? $va_attributes[ $vn_hier_id ]
			: array();

		$vs_sort_dir = ( isset( $pa_options['sort'] )
		                 && ( in_array( strtolower( $pa_options['sortDirection'] ), array( 'asc', 'desc' ) ) ) )
			? strtolower( $pa_options['sortDirection'] ) : 'asc';
	 ...
	 *                               convertLineBreaks = if set to true, will attempt to convert line break characters
	 *                               to HTML <p> and <br> tags; default is false. locale = if set to a valid locale_id
	 *                               or locale code, values will be returned in locale *if available*, otherwise will
	 *                               fallback to values in languages that are available using the standard fallback
	 *                               mechanism. Default is to use user's current locale. returnAllLocales = if set to
	 *                               true, values for all locales are returned, locale option is ignored and the
	 *                               returned array is indexed first by attribute_id and then by locale_id. Default is
	 *                               false. indexByRowID = if true first index of returned array is $pn_row_id,
	 *                               otherwise it is the element_id of the retrieved metadata element
	 *                               indexValuesByValueID = index value array by value_id [default=false]
	 *                               indexValuesByElementCode = index value array by element code [default=true] indexValuesByElementCodeAndValueID = index value array by element codes, each of which has a sub-array indexed by value_id. Implies indexValuesByElementCode. [default=false] convertCodesToDisplayText = filter = ignoreLocale = if set all values are returned regardless of locale, but in the flattened structure returned when returnAllLocales is false sort = the full bundle specification for the element value or subvalue (if the element is a container) to sort returned values on. sortDirection = determines the direction of the sort. Valid values are ASC (ascending) and DESC (descending). [Default=ASC]
	 *
	 * @return array
	 */
	public function getAttributeDisplayValues( $pm_element_code_or_id, $pn_row_id, $pa_options = null ) {
		...
		$ps_sort_direction    = caGetOption( 'sortDirection', $pa_options, 'asc',
			array( 'forceLowercase' => true, 'validValues' => array( 'asc', 'desc' ) ) );
		...
		$pb_index_by_element_code_and_value_id = caGetOption( 'indexValuesByElementCodeAndValueID', $pa_options,
			false );
		...
		foreach ( $va_attribute_list as $o_attribute ) {
			...
			foreach ( $va_values as $o_value ) {
				...
				if ( get_class( $o_value ) == 'ListAttributeValue' ) {
					...
					$vn_list_id = ( ! isset( $pa_options['convertCodesToDisplayText'] )
					                || ! (bool) $pa_options['convertCodesToDisplayText'] ) ? null
				...
				if ( $ps_filter_expression ) {
					$va_raw_values[ ( $vs_container_element_code == $vs_element_code )
						? "{$vs_table_name}.{$vs_element_code}"
						: "{$vs_table_name}.{$vs_container_element_code}.{$vs_element_code}" ]
						= $va_raw_values[ $vs_element_code ] = $o_value->getDisplayValue( array(
						'list_id'    => $vn_list_id,
						'returnIdno' => true
					) );
				}
				...
				if ( isset( $pa_options['convertLineBreaks'] ) && $pa_options['convertLineBreaks'] ) {
					$vs_converted_value = preg_replace( "!(\n|\r\n){2}!", "<p/>",
						$o_value->getDisplayValue( array_merge( $pa_options, array( 'list_id' => $vn_list_id ) ) ) );
					$vs_display_value   = preg_replace( "![\n]{1}!", "<br/>", $vs_converted_value );
				} else {
					$vs_display_value = $o_value->getDisplayValue( array_merge( $pa_options,
				...
				if ( $vs_sort_fld && ( $vs_sort_fld == $vs_element_code ) ) {
					$vs_sort_key = $o_value->getDisplayValue( array_merge( $pa_options,
						array( 'getDirectDate' => true, 'list_id' => $vn_list_id ) ) );
				}
			}
			if ( ! $vs_sort_key ) {
				$vs_sort_key = '0';
			...
			if ( $pb_ignore_locale ) {
				...
				$va_attributes[ $vs_sort_key ][ $vs_index ][ (int) $o_attribute->getLocaleID() ][ $o_attribute->getAttributeID() ]
					= $va_display_values;
			}
		}
	...
	/**
	 * Return raw value of attribute for a given row. The "raw" value is the display value, or values, joined with the
	 * specified delimiter and filtered on the current user locale.
	 *
	 * @param int    $pn_row_id                 row_id attribute is attached to in the table the instance represents
	 * @param mixed  $pm_element_code_or_id     Element code or element_id of the metadata element to load the
	 *                                          attribute for
	 * @param mixed  $pm_sub_element_code_or_id Optional sub-element code or element_id to fetch value for. This is
	 *                                          used to select a sub-element in complex attributes; if you want the
	 *                                          top-level (aka root) element leave this set to null
	 * @param string $ps_delimiter              Optional delimiter to use when multiple values are defined. Default is
	 *                                          a comma (",").
	 *
	 * @return string The "raw" value
	 */
	public function getRawValue(
		$pn_row_id, $pm_element_code_or_id, $pm_sub_element_code_or_id = null, $ps_delimiter = ',', $pa_options = null
	) {
		$vn_element_id = ca_metadata_elements::getElementID( $pm_element_code_or_id );
		$va_attributes = ca_attributes::getAttributes( $this->getDb(), $this->tableNum(), $pn_row_id,
			array( $vn_element_id ), array() );

		if ( ! is_array( $va_attributes = $va_attributes[ $vn_element_id ] ) ) {
			return null;
		}

		$vn_sub_element_id = $pm_sub_element_code_or_id
			? ca_metadata_elements::getElementID( $pm_sub_element_code_or_id ) : null;

		$va_ret_values = array();
		foreach ( $va_attributes as $o_attr ) {
			if ( $o_attr->getElementID() == $vn_element_id ) {
				$va_values                                       = $o_attr->getDisplayValues( true, $pa_options );
				$va_ret_values[][ (int) $o_attr->getLocaleID() ] = $va_values[ $vn_sub_element_id ? $vn_sub_element_id
					: $vn_element_id ];
			}
		}
	...
	public function getAttributeForIDs( $pm_element_code_or_id, $pa_ids, $pa_options = null ) {
		...
		return caExtractValuesByUserLocale( ca_attributes::getAttributeValueForIDs( $this->getDb(), $this->tableNum(),
			$pa_ids, $vn_element_id, $pa_options ) );
	}
	# ------------------------------------------------------------------
	 ...
	 *    convertLineBreaks = if true will convert line breaks to HTML <br/> tags for display in a web browser; default
	 *    is false dontUseElementTemplate = By default any display template set in the metadata element is used if the
	 *    $ps_template parameter is blank or null. Set this option to prevent the metadata element template from being
	 *    used in any event. [Default is false]
	 */
	public function getAttributesForDisplay( $pm_element_code_or_id, $ps_template = null, $pa_options = null ) {
		...
		if ( ! $pm_element_code_or_id
		     || ! ( $t_element
				= ca_metadata_elements::getInstance( $pm_element_code_or_id ) )
		) {
			return null;
		}
		if ( ! is_array( $pa_options ) ) {
		...
		$va_tmp = $this->getAttributeDisplayValues(
			$vn_hier_id = ca_metadata_elements::getElementHierarchyID( $pm_element_code_or_id ), $vn_row_id,
			array_merge( $pa_options, array( 'returnAllLocales' => false ) ) );

		if ( ! $ps_template && ( $vs_template_tmp = $t_element->getSetting( 'displayTemplate', true ) )
		     && ! caGetOption( 'dontUseElementTemplate', $pa_options, false )
		...
		if ( $ps_template ) {
			...
			return caProcessTemplateForIDs( $ps_template, $this->tableNum(), array( $vn_row_id ),
				array_merge( $pa_options, array(
					'requireLinkTags'   => true,
					'placeholderPrefix' => $this->tableName() . '.' . $t_element->get( 'element_code' )
				) ) );
		} else {
			// no template
			if ( sizeof( $va_tmp ) > 0 ) {
	...
	public function getRawAttributeValuesForIDs( $pm_element_code_or_id, $pa_ids, $pa_options = null ) {
		...
		return ca_attributes::getRawAttributeValuesForIDs( $this->getDb(), $this->tableNum(), $pa_ids, $vn_element_id,
			$pa_options );
	}
	# ------------------------------------------------------------------
	// --- Utilities
	...
	public function copyAttributesTo( $pn_row_id, $pa_options = null ) {
		...
		foreach ( $va_elements as $vn_element_id => $vs_element_code ) {
			...
			$va_vals = $this->get( "{$vs_table}.{$vs_element_code}", array(
				"returnAsArray"       => true,
				"returnWithStructure" => true,
				"returnAllLocales"    => true,
				'forDuplication'      => true
	...
	public function addMetadataElementToType( $pm_element_code_or_id, $pn_type_id ) {
		...
		if ( $t_restriction->numErrors() ) {
			$this->postError( 1980,
				_t( "Couldn't add element to restriction list: %1", join( '; ', $t_restriction->getErrors() ) ),
				'BaseModelWithAttributes->addMetadataElementToType()' );
	...
	public function removeMetadataElementFromType( $pm_element_code_or_id, $pn_type_id ) {
		...
		if ( $t_restriction->load( array(
			...
			if ( $t_restriction->numErrors() ) {
				$this->postError( 1981, _t( "Couldn't remove element from restriction list: %1",
	 ...
	 *
	 */
	public function getAuthorityReferenceListHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pa_bundle_settings, $pa_options
	) {
		$va_errors = array();
	...
	public static function getAuthorityElementDatatypeList() {
		...
		return array(
			...
			'ca_object_lots'            => __CA_ATTRIBUTE_VALUE_OBJECTLOTS__,
			'ca_entities'               => __CA_ATTRIBUTE_VALUE_ENTITIES__,
			'ca_places'                 => __CA_ATTRIBUTE_VALUE_PLACES__,
			'ca_occurrences'            => __CA_ATTRIBUTE_VALUE_OCCURRENCES__,
			'ca_collections'            => __CA_ATTRIBUTE_VALUE_COLLECTIONS__,
			'ca_storage_locations'      => __CA_ATTRIBUTE_VALUE_STORAGELOCATIONS__,
			'ca_list_items'             => __CA_ATTRIBUTE_VALUE_LIST__,
			'ca_loans'                  => __CA_ATTRIBUTE_VALUE_LOANS__,
			'ca_movements'              => __CA_ATTRIBUTE_VALUE_MOVEMENTS__,
			'ca_object_representations' => __CA_ATTRIBUTE_VALUE_OBJECTREPRESENTATIONS__
		);
	}
	# ------------------------------------------------------------------
	 ...
	 *                          row_id = Return references for specified row. [Default is to return references for
	...
	/**
	 * Returns a list of row_ids indexed by table that reference the currently loaded row using authority metadata
	 * elements.
	 *
	 * @param array $pa_options Option include:
	 ...
	 *                          currently loaded row]
	 *
	 * @return mixed A list of references, key'ed on table number and then primary key value; values are arrays of
	 *               element_ids. If the 'countOnly' option is set then an integer count of the references is returned.
	 */
	public function getAuthorityElementReferences( $pa_options = null ) {
		...
		foreach ( $va_references as $vn_table_num => $va_rows ) {
			...
			if ( ( sizeof( $va_row_ids ) > 0 )
				...
				$qr_del = $o_db->query( "SELECT {$vs_pk} FROM " . $t_instance->tableName() . " WHERE {$vs_pk} IN (?)"
				                        . ( $t_instance->hasField( 'deleted' ) ? "AND deleted = 1" : '' ),
					array( $va_row_ids ) );
	 ...
	 *
	 * @param array $pa_options Options include:
	 *                          row_id = Return used elements for specified row. [Default is to return references for
	 *                          currently loaded row] idsOnly = Return element_id values only. [Default is false]
	 *                          rootIdsOnly = Return element_id values only. [Default is false] omitLists = Don't
	 *                          include list elements. [Default is true]
	 */
	 ...
	 * attributes will be referenced by the target only. Internally, the same attribute entries are reused for the
	 * target with attribute row_id values rewritten.
	 *
	 * Note that if a sub-element code or element_id is specified the entire container for the sub-element will be
	 * moved.
	 *
	 * @param int   $pn_to_id                Primary key of row to move element data to
	 * @param array $pa_element_codes_or_ids A list of element codes or element_ids of attributes to move. If
	 *                                       sub-elements are specified the enclosing containers will be moved.
	 * @param array $pa_options              Options include:
	 *                                       row_id = Return used elements for specified row. [Default is to return
	 *                                       references for currently loaded row]
	 *
	 * @return bool True on success, false on error, null if this model is not used for references or no row is loaded.
	...
	public function moveAttributes( $pn_to_id, $pa_element_codes_or_ids, $pa_options = null ) {
		...
		if ( is_array( $pa_element_codes_or_ids ) && sizeof( $pa_element_codes_or_ids ) ) {
			...
			foreach ( $pa_element_codes_or_ids as $vm_element ) {
				...
				if ( ! $t_instance->hasElement( ca_metadata_elements::getElementCodeForId(
					$vn_hier_element_id = ca_metadata_elements::getElementHierarchyID( $vn_element_id ) ) )
				) {
					continue;
				}
	...
	public function hasBundle( $ps_bundle, $pn_type_id = null ) {
		...
		if ( $vn_num_bits == 1 ) {
			...
			if ( $va_bundle_bits[0] == $this->tableName() ) {
				return ( $this->hasElement( $va_bundle_bits[1] ) ) ? true
					: parent::hasBundle( $ps_bundle, $pn_type_id );
	...
	public function hasElement(
		$ps_element_code, $pn_type_id = null, $pb_include_sub_element_codes = false, $pa_options = null
	) {
		...
		$va_codes = $this->getApplicableElementCodes( $pn_type_id, $pb_include_sub_element_codes,
			caGetOption( 'dontCache', $pa_options, false ) );
	 ...
	 * @param array  $pa_options            Options include:
	 *                                      transaction = A database transaction to execute the search within. [Default
	 *                                      is null] value_id = An optional value id to exclude when looking for
	 *                                      existing values. [Default is null]
	 *
	 ...
	 *
	 * Normally only top-level attribute codes are returned. This is good: in general you should only be dealing with
	 * attributes via the top-level element. However, there are a few cases where you might need an inventory of *all*
	 * element codes that can be attached to a model, even those that are part of an element hierarchy. Setting the
	 * $pb_include_sub_element_codes to true will include sub-elements in the returned list.
	 */
	public function getApplicableElementCodes(
		...
		if ( ! $pb_dont_cache
		     && is_array( $va_tmp = BaseModelWithAttributes::$s_applicable_element_code_cache[ $this->tableNum() . '/'
		                                                                                       . $pn_type_id . '/'
		                                                                                       . ( $pb_include_sub_element_codes
				? 1 : 0 ) ] )
		) {
			return $va_tmp;
		}
		...
		if (
			...
			if ( $t_type_instance = $this->getTypeInstance() ) {
				$va_ancestors = $t_type_instance->getHierarchyAncestors( null,
			...
			if ( is_array( $va_ancestors ) && ( sizeof( $va_ancestors ) > 1 ) ) {
				$vs_type_sql = '((camtr.type_id = ' . intval( $pn_type_id )
				               . ') OR (camtr.type_id IS NULL) OR ((camtr.include_subtypes = 1) AND camtr.type_id IN ('
				               . join( ',', $va_ancestors ) . '))) AND ';
			} else {
		...
		while ( $qr_res->nextRow() ) {
			...
			$va_element_labels_by_locale[ $vn_element_id ][ $vn_locale_id ]
				= $va_element_labels_by_locale[ $vs_element_code ][ $vn_locale_id ] = $qr_res->getRow();
		...
		BaseModelWithAttributes::$s_applicable_element_code_cache[ $this->tableNum() . '/' . $pn_type_id . '/'
		                                                           . ( $pb_include_sub_element_codes ? 1 : 0 ) ]
			= $va_codes;
	 ...
	 *
	 */
	public function getApplicableElementCodesForTypes(
		$pa_type_ids, $pb_include_sub_element_codes = false, $pb_dont_cache = true
	) {
		$va_codes = array();
	...
	public function getTypeRestrictionInstance( $pn_element_id ) {
		...
		if ( is_subclass_of( $this, 'BaseRelationshipModel' )
		     && ( $t_restriction->load( array(
				'element_id' => (int) $pn_element_id,
				'table_num'  => (int) $this->tableNum(),
				'type_id'    => $this->get( 'type_id' )
			) ) )
		) {
			return $t_restriction;
		} elseif ( $t_restriction->load( array(
			'element_id' => (int) $pn_element_id,
			'table_num'  => (int) $this->tableNum(),
			'type_id'    => $this->get( $this->ATTRIBUTE_TYPE_ID_FLD )
		) )
		) {
			return $t_restriction;
		} elseif ( $t_restriction->load( array(
	...
	public function setFailedAttributeInserts( $pm_element_code_or_id, $pa_attribute_values ) {
		$this->opa_failed_attribute_inserts[ ca_metadata_elements::getElementID( $pm_element_code_or_id ) ]
			= $pa_attribute_values;
	}
	...
	public function getFailedAttributeInserts( $pm_element_code_or_id ) {
		...
		return isset( $this->opa_failed_attribute_inserts[ $vs_k ] ) ? $this->opa_failed_attribute_inserts[ $vs_k ]
			: null;
	}

	# ------------------------------------------------------------------
	public function setFailedAttributeUpdates( $pm_element_code_or_id, $pa_attribute_values ) {
		$this->opa_failed_attribute_updates[ ca_metadata_elements::getElementID( $pm_element_code_or_id ) ]
			= $pa_attribute_values;
	}
	...
	public function getFailedAttributeUpdates( $pm_element_code_or_id ) {
		...
		return isset( $this->opa_failed_attribute_updates[ $vs_k ] ) ? $this->opa_failed_attribute_updates[ $vs_k ]
			: null;
	}
	# ------------------------------------------------------------------
	...
	public function attributeHasChanged( $pm_element_code_or_id ) {
		...
		return isset( $this->_FIELD_VALUE_CHANGED[ '_ca_attribute_' . $vn_id ] )
			? $this->_FIELD_VALUE_CHANGED[ '_ca_attribute_' . $vn_id ] : false;
	}
	# ------------------------------------------------------------------
	...
	public function attributeDidChange( $pm_element_code_or_id ) {
		...
		return isset( $this->_FIELD_VALUE_DID_CHANGE[ '_ca_attribute_' . $vn_id ] )
			? $this->_FIELD_VALUE_DID_CHANGE[ '_ca_attribute_' . $vn_id ] : false;
	}
	# ------------------------------------------------------------------
