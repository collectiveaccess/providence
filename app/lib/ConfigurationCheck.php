...
final class ConfigurationCheck
{
    ...
    public static function caUrlRootQuickCheck()
    {
        ...
        if (caGetOSFamily() === OS_WIN32) {    // Windows paths are case insensitive
            if (!in_array(
                ...
                self::addError(
                    _t(
                        "It looks like the __CA_URL_ROOT__ variable in your setup.php is not set correctly. Please try to set it to &quot;%1&quot;.",
                        $possible_url_roots[0]
                    )
                );
            }
        } else {
            if (!in_array(__CA_URL_ROOT__, $possible_url_roots)) {
                self::addError(
                    _t(
                        "It looks like the __CA_URL_ROOT__ variable in your setup.php is not set correctly. Please try to set it to &quot;%1&quot;. Note that paths are case sensitive.",
                        $possible_url_roots[0]
                    )
                );
            }
    ...
    public static function caBaseDirQuickCheck()
    {
        ...
        if (caGetOSFamily() === OS_WIN32) {    // Windows paths are case insensitive
            if (!in_array(
                ...
                self::addError(
                    _t(
                        "It looks like the __CA_BASE_DIR__ variable in your setup.php is not set correctly. Please try to set it to &quot;%1&quot;.",
                        $possible_bases[0]
                    )
                );
            }
            ...
            if (!in_array(__CA_BASE_DIR__, $possible_bases)) {
                self::addError(
                    _t(
                        "It looks like the __CA_BASE_DIR__ variable in your setup.php is not set correctly. Please try to set it to &quot;%1&quot;. Note that paths are case sensitive.",
                        $possible_bases[0]
                    )
                );
            }
    ...
    private static function _baseGuesses()
    {
        return [
            ...
            $_SERVER['SCRIPT_FILENAME'] ? pathinfo($_SERVER['SCRIPT_FILENAME'], PATHINFO_DIRNAME) : join(
                DIRECTORY_SEPARATOR,
                array_slice(explode(DIRECTORY_SEPARATOR, __FILE__), 0, -3)
            )
        ];
    }
