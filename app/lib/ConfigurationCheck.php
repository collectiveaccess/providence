...
final class ConfigurationCheck
{
    ...
    public static function performQuick($options = null)
    {
        ...
        foreach ($va_methods as $vo_method) {
            if (strpos($vo_method->name, "QuickCheck") !== false) {
                if (caGetOption('skipPathChecks', $options, false) && in_array(
                        $vo_method->name,
                        [
                            'caUrlRootQuickCheck',
                            'caBaseDirQuickCheck'
    ...
    public static function permissionInstallCheck()
    {
        ...
        if (!is_writeable(__CA_APP_DIR__ . '/tmp')) {
            self::addError(
                _t(
                    'The CollectiveAccess <i>app/tmp</i> directory is NOT writeable by the installer. This may result in installation errors. It is recommended that you change permissions on this directory (<i>%1</i>) to allow write access prior to installation. You can reload the installer to verify that the changed permissions are correct.',
                    __CA_APP_DIR__ . '/tmp'
                )
            );
        }
        ...
        if (!is_writeable(__CA_APP_DIR__ . '/log')) {
            self::addError(
                _t(
                    'The CollectiveAccess <i>app/log</i> directory is NOT writeable by the installer. This may result in installation errors. It is recommended that you change permissions on this directory (<i>%1</i>) to allow write access prior to installation. You can reload the installer to verify that the changed permissions are correct.',
                    __CA_APP_DIR__ . '/log'
                )
            );
        }
    ...
    public static function DBInnoDBQuickCheck()
    {
        ...
        if (!$vb_innodb_available) {
            self::addError(
                _t(
                    "Your MySQL installation doesn't support the InnoDB storage engine which is required by CollectiveAccess. For more information also see %1.",
                    "<a href='http://dev.mysql.com/doc/refman/5.1/en/innodb.html' target='_blank'>http://dev.mysql.com/doc/refman/5.1/en/innodb.html</a>"
                )
            );
        }
    ...
    public static function DBLoginQuickCheck()
    {
        ...
        if (!self::$opo_db->connected()) {
            self::addError(
                _t("Could not connect to database. Did you enter your database login information into setup.php?")
            );
        }
    ...
    public static function DBTableQuickCheck()
    {
        ...
        if (!is_array($va_tmp) || !in_array('ca_users', $va_tmp)) {
            self::addError(
                _t(
                    "It looks like you have not installed your database yet. Check your configuration or run the <a href=\"%1/install/\">installer</a>.",
                    __CA_URL_ROOT__
                )
            );
            return false;
    ...
    public static function PHPVersionQuickCheck()
    {
        ...
        if ($va_php_version["versionInt"] < 50600) {
            self::addError(
                _t(
                    "CollectiveAccess requires PHP version 5.6 or higher to function properly. You're running %1. Please upgrade.",
                    $va_php_version["version"]
                )
            );
        }
        return true;
    ...
    public static function SiteProtocolSetQuickCheck()
    {
        if (!defined("__CA_SITE_PROTOCOL__")) {
            self::addError(
                _t(
                    "Your setup.php file does not include a setting for __CA_SITE_PROTOCOL__. Please update it using the current setup.php-dist as a template."
                )
            );
        }
    ...
    public static function tmpDirQuickCheck()
    {
        if (!file_exists(__CA_APP_DIR__ . "/tmp") || !is_writable(__CA_APP_DIR__ . "/tmp")) {
            self::addError(
                _t(
                    "It looks like the directory for temporary files is not writable by the webserver. Please change the permissions of %1 and enable the user which runs the webserver to write to this directory.",
                    __CA_APP_DIR__ . "/tmp"
                )
            );
        }
        ...
        if (__CA_CACHE_BACKEND__ == 'file') {
            if (
                file_exists(
                    __CA_CACHE_FILEPATH__ . DIRECTORY_SEPARATOR . __CA_APP_NAME__ . 'Cache'
                ) && // if it doesn't exist, it can be probably be created or the above check would fail
                !is_writable(__CA_CACHE_FILEPATH__ . DIRECTORY_SEPARATOR . __CA_APP_NAME__ . 'Cache')
            ) {
                self::addError(
                    _t(
                        "It looks like the cache directory is not writable by the webserver. Please change the permissions of %1 and enable the user which runs the webserver to write to this directory.",
                        __CA_CACHE_FILEPATH__ . DIRECTORY_SEPARATOR . __CA_APP_NAME__ . 'Cache'
                    )
                );
            }
    ...
    public static function logDirQuickCheck()
    {
        ...
        if (!file_exists($vs_log_path) || !is_writable($vs_log_path)) {
            self::addError(
                _t(
                    "It looks like the log directory is not writable by the webserver. Please change the permissions of %1 (or create it if it doesn't exist already) and enable the user which runs the webserver to write to this directory.",
                    $vs_log_path
                )
            );
        }
    ...
    public static function mediaDirQuickCheck()
    {
        ...
        if (!file_exists($vs_media_root) || !is_writable($vs_media_root)) {
            self::addError(
                _t(
                    "It looks like the media directory is not writable by the webserver. Please change the permissions of %1 (or create it if it doesn't exist already) and enable the user which runs the webserver to write to this directory.",
                    $vs_media_root
                )
            );
        }
    ...
    public static function htmlPurifierDirQuickCheck()
    {
        ...
        if (!file_exists($vs_purifier_path) || !is_writable($vs_purifier_path)) {
            self::addError(
                _t(
                    "It looks like the directory for HTML filtering caches is not writable by the webserver. Please change the permissions of %1 and enable the user which runs the webserver to write to this directory.",
                    $vs_purifier_path
                )
            );
        }
    ...
    public static function caUrlRootQuickCheck()
    {
        ...
        if (caGetOSFamily() === OS_WIN32) {    // Windows paths are case insensitive
            if (strcasecmp($vs_probably_correct_urlroot, __CA_URL_ROOT__) != 0) {
                self::addError(
                    _t(
                        "It looks like the __CA_URL_ROOT__ variable in your setup.php is not set correctly. Please try to set it to &quot;%1&quot;. We came up with this suggestion because you accessed this script via &quot;&lt;your_hostname&gt;%2&quot;.",
                        $vs_probably_correct_urlroot,
                        $vs_script_name
                    )
                );
            }
            ...
            if (!($vs_probably_correct_urlroot == __CA_URL_ROOT__)) {
                self::addError(
                    _t(
                        "It looks like the __CA_URL_ROOT__ variable in your setup.php is not set correctly. Please try to set it to &quot;%1&quot;. We came up with this suggestion because you accessed this script via &quot;&lt;your_hostname&gt;%2&quot;. Note that paths are case sensitive.",
                        $vs_probably_correct_urlroot,
                        $vs_script_name
                    )
                );
    ...
    public static function caBaseDirQuickCheck()
    {
        ...
        if (caGetOSFamily() === OS_WIN32) {    // Windows paths are case insensitive
            if (strcasecmp($vs_probably_correct_base, __CA_BASE_DIR__) != 0) {
                self::addError(
                    _t(
                        "It looks like the __CA_BASE_DIR__ variable in your setup.php is not set correctly. Please try to set it to &quot;%1&quot;. We came up with this suggestion because the location of this script is &quot;%2&quot;.",
                        $vs_probably_correct_base,
                        $vs_script_filename
                    )
                );
            }
            ...
            if (!($vs_probably_correct_base == __CA_BASE_DIR__)) {
                self::addError(
                    _t(
                        "It looks like the __CA_BASE_DIR__ variable in your setup.php is not set correctly. Please try to set it to &quot;%1&quot;. We came up with this suggestion because the location of this script is &quot;%2&quot;. Note that paths are case sensitive.",
                        $vs_probably_correct_base,
                        $vs_script_filename
                    )
                );
    ...
    public static function PHPModuleRequirementQuickCheck()
    {
        ...
        if (!function_exists("mysql_query") && !function_exists("mysqli_query")) {
            self::addError(
                _t("PHP mysql or mysqli module is required for CollectiveAccess to run. Please install it.")
            );
        }
        ...
        if (!class_exists("DOMDocument")) {
            self::addError(
                _t("PHP Document Object Model (DOM) module is required for CollectiveAccess to run. Please install it.")
            );
        }
        if (!function_exists('mcrypt_create_iv') && !function_exists('openssl_random_pseudo_bytes')) {
            self::addError(
                _t(
                    "Either the mcrypt or openssl module are required for CollectiveAccess to run. Please install one of them (or both)."
                )
            );
        }
        if (!function_exists('hash_hmac')) {
            self::addError(
                _t("The PHP Message Digest (hash) engine is required for CollectiveAccess to run. Please install it.")
            );
        }
        if (!in_array('sha256', hash_algos())) {
            self::addError(
                _t(
                    "Your PHP installation doesn't seem to have support for the sha256 hashing algorithm. Please install a newer version of either PHP or the hash module."
                )
            );
        }
        ...
        if (@preg_match('/\p{L}/u', 'a') != 1) {
            self::addError(
                _t(
                    "Your version of the PHP PCRE module lacks unicode features. Please install a module version with UTF-8 support."
                )
            );
        }
        return true;
    ...
    public static function appNameValidityQuickCheck()
    {
        if (!preg_match("/^[[:alnum:]|_]+$/", __CA_APP_NAME__)) {
            self::addError(
                _t(
                    'It looks like the __CA_APP_NAME__ setting in your setup.php is invalid. It may only consist of alphanumeric ASCII characters and underscores (&quot;_&quot;)'
                )
            );
        }
        return true;
    ...
    public static function mediaDirRecursiveExpensiveCheck()
    {
        ...
        foreach ($va_dirs as $vs_dir) {
            if (!file_exists($vs_dir) || !is_writable($vs_dir)) {
                ...
                self::addError(
                    _t(
                        "It looks like a subdirectory in the media directory is not writable by the webserver. Please change the permissions for %1 and enable the user which runs the webserver to write to this directory.",
                        $vs_dir
                    )
                );
            }
    ...
    public static function suhoshinExpensiveCheck()
    {
        ...
        if ($vs_post_max_name_length && intval($vs_post_max_name_length) < 256) {
            self::addError(
                _t(
                    'It looks like you have the PHP Suhoshin extension installed which introduces some default constraints than can prevent CA from saving information entered into forms.'
                ) .
                _t(
                    'In particular, you have to set the configuration value "suhosin.post.max_name_length" to 256 or higher to ensure that CA works correctly. Your value is %1',
                    $vs_post_max_name_length
                )
            );
        }
        ...
        if ($vs_request_max_varname_length && intval($vs_request_max_varname_length) < 256) {
            self::addError(
                ...
                _t(
                    'In particular, you have to set the configuration value "suhosin.request.max_varname_length" to 256 or higher to ensure that CA works correctly. Your value is %1',
                    $vs_request_max_varname_length
                )
            );
        }
        ...
        if ($vs_post_max_totalname_length && intval($vs_post_max_totalname_length) < 5012) {
            self::addError(
                ...
                _t(
                    'In particular, you have to set the configuration value "suhosin.post.max_totalname_length" to 5012 or higher to ensure that CA works correctly. Your value is %1',
                    $vs_post_max_totalname_length
                )
            );
        }
        ...
        if ($vs_request_max_totalname_length && intval($vs_request_max_totalname_length) < 5012) {
            self::addError(
                ...
                _t(
                    'In particular, you have to set the configuration value "suhosin.request.max_totalname_length" to 5012 or higher to ensure that CA works correctly. Your value is %1',
                    $vs_request_max_totalname_length
                )
            );
        }
        return true;
        ...
        if (($vn_schema_revision = self::getSchemaVersion()) < __CollectiveAccess_Schema_Rev__) {
            for ($vn_i = ($vn_schema_revision + 1); $vn_i <= __CollectiveAccess_Schema_Rev__; $vn_i++) {
                ...
                if (is_array($va_new_messages = $o_updater->applyDatabaseUpdate())) {
                    ...
                    $va_messages["error_{$vn_i}_sql_fail"] = _t(
                        'Could not apply database update for migration %1',
                        $vn_i
                    );
                }
                // post-update tasks
                ...
                if ($vs_message = $o_updater->getPostupdateMessage()) {
                    $va_messages[(sizeof(
                        $va_methods_that_errored
                    ) ? "error" : "info") . "_{$vn_i}_{$vs_postupdate_method}_postupdate_message"] = _t(
                            "For migration %1",
                            $vn_i
                        ) . ": {$vs_message}";
                } else {
                    if (sizeof($va_methods_that_errored)) {
                        $va_messages["error_{$vn_i}_{$vs_postupdate_method}_postupdate_message"] = _t(
                                "For migration %1",
                                $vn_i
                            ) . ": " . _t(
                                "The following tasks did not complete: %1",
                                join(', ', $va_methods_that_errored)
                            );
                    } else {
