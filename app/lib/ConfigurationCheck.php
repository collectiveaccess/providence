...
final class ConfigurationCheck {
	...
	public static function performQuick( $options = null ) {
		...
		foreach ( $va_methods as $vo_method ) {
			if ( strpos( $vo_method->name, "QuickCheck" ) !== false ) {
				if ( caGetOption( 'skipPathChecks', $options, false )
				     && in_array( $vo_method->name, [ 'caUrlRootQuickCheck', 'caBaseDirQuickCheck' ] )
				) {
					continue;
				}
	...
	public static function updateDatabaseSchema() {
		require_once( self::$opo_config->get( "views_directory" )
		              . "/system/configuration_error_schema_update_html.php" );
	}
	...
	public static function permissionInstallCheck() {
		...
		if ( ! $vb_perm_media_error && ! $vb_at_least_one_part_of_the_media_path_exists
		     && ! is_writeable( $vs_base_dir )
		) {
			$vb_perm_media_error = true;
	...
	public static function DBInnoDBQuickCheck() {
		...
		if ( ! $vb_innodb_available ) {
			self::addError( _t( "Your MySQL installation doesn't support the InnoDB storage engine which is required by CollectiveAccess. For more information also see %1.",
	...
	public static function DBTableQuickCheck() {
		...
		if ( ! is_array( $va_tmp ) || ! in_array( 'ca_users', $va_tmp ) ) {
			self::addError( _t( "It looks like you have not installed your database yet. Check your configuration or run the <a href=\"%1/install/\">installer</a>.",
				__CA_URL_ROOT__ ) );
	...
	public static function PHPVersionQuickCheck() {
		...
		if ( $va_php_version["versionInt"] < 50600 ) {
			self::addError( _t( "CollectiveAccess requires PHP version 5.6 or higher to function properly. You're running %1. Please upgrade.",
	...
	public static function tmpDirQuickCheck() {
		if ( ! file_exists( __CA_APP_DIR__ . "/tmp" ) || ! is_writable( __CA_APP_DIR__ . "/tmp" ) ) {
			self::addError( _t( "It looks like the directory for temporary files is not writable by the webserver. Please change the permissions of %1 and enable the user which runs the webserver to write to this directory.",
				__CA_APP_DIR__ . "/tmp" ) );
		}
		...
		if ( __CA_CACHE_BACKEND__ == 'file' ) {
			if (
				file_exists( __CA_CACHE_FILEPATH__ . DIRECTORY_SEPARATOR . __CA_APP_NAME__ . 'Cache' )
				...
				self::addError( _t( "It looks like the cache directory is not writable by the webserver. Please change the permissions of %1 and enable the user which runs the webserver to write to this directory.",
	...
	public static function logDirQuickCheck() {
		...
		if ( ! file_exists( $vs_log_path ) || ! is_writable( $vs_log_path ) ) {
			self::addError( _t( "It looks like the log directory is not writable by the webserver. Please change the permissions of %1 (or create it if it doesn't exist already) and enable the user which runs the webserver to write to this directory.",
				$vs_log_path ) );
		}
	...
	public static function mediaDirQuickCheck() {
		...
		if ( ! file_exists( $vs_media_root ) || ! is_writable( $vs_media_root ) ) {
			self::addError( _t( "It looks like the media directory is not writable by the webserver. Please change the permissions of %1 (or create it if it doesn't exist already) and enable the user which runs the webserver to write to this directory.",
				$vs_media_root ) );
		}
	...
	public static function htmlPurifierDirQuickCheck() {
		$vs_purifier_path = __CA_BASE_DIR__
		...
		if ( ! file_exists( $vs_purifier_path ) || ! is_writable( $vs_purifier_path ) ) {
			self::addError( _t( "It looks like the directory for HTML filtering caches is not writable by the webserver. Please change the permissions of %1 and enable the user which runs the webserver to write to this directory.",
				$vs_purifier_path ) );
		}
	...
	public static function caUrlRootQuickCheck() {
		...
		if ( caGetOSFamily() === OS_WIN32 ) {    // Windows paths are case insensitive
			if ( ! in_array( strtolower( __CA_URL_ROOT__ ), array_map( function ( $v ) {
				...
				self::addError( _t( "It looks like the __CA_URL_ROOT__ variable in your setup.php is not set correctly. Please try to set it to &quot;%1&quot;.",
					$possible_url_roots[0] ) );
			}
		} else {
			if ( ! in_array( __CA_URL_ROOT__, $possible_url_roots ) ) {
				self::addError( _t( "It looks like the __CA_URL_ROOT__ variable in your setup.php is not set correctly. Please try to set it to &quot;%1&quot;. Note that paths are case sensitive.",
					$possible_url_roots[0] ) );
			}
		}
	...
	public static function caBaseDirQuickCheck() {
		...
		if ( caGetOSFamily() === OS_WIN32 ) {    // Windows paths are case insensitive
			if ( ! in_array( strtolower( __CA_BASE_DIR__ ), array_map( function ( $v ) {
				...
				self::addError( _t( "It looks like the __CA_BASE_DIR__ variable in your setup.php is not set correctly. Please try to set it to &quot;%1&quot;.",
					$possible_bases[0] ) );
			}
		} else {
			if ( ! in_array( __CA_BASE_DIR__, $possible_bases ) ) {
				self::addError( _t( "It looks like the __CA_BASE_DIR__ variable in your setup.php is not set correctly. Please try to set it to &quot;%1&quot;. Note that paths are case sensitive.",
					$possible_bases[0] ) );
			}
		}
	...
	private static function _baseGuesses() {
		return [
			...
			$_SERVER['SCRIPT_FILENAME'] ? pathinfo( $_SERVER['SCRIPT_FILENAME'], PATHINFO_DIRNAME )
	...
	public static function mediaDirRecursiveExpensiveCheck() {
		...
		foreach ( $va_dirs as $vs_dir ) {
			if ( ! file_exists( $vs_dir ) || ! is_writable( $vs_dir ) ) {
				if ( $i ++
				     == 10
				) { // we don't want to spam houndreds of directories. I guess the admin will get the pattern after a few.
					return;
				}
				self::addError( _t( "It looks like a subdirectory in the media directory is not writable by the webserver. Please change the permissions for %1 and enable the user which runs the webserver to write to this directory.",
					$vs_dir ) );
			}
		}
	...
	public static function uploadLimitExpensiveCheck() {
		...
		if ( ( $vn_post_max_size > 0 && $vn_post_max_size < 5242880 )
		     || ( $vn_upload_max_filesize > 0
		          && $vn_upload_max_filesize < 5242880 )
		) {
			self::addError( _t(
	...
	public static function suhoshinExpensiveCheck() {
		...
		if ( $vs_post_max_name_length && intval( $vs_post_max_name_length ) < 256 ) {
			self::addError(
				...
				.
				_t( 'In particular, you have to set the configuration value "suhosin.post.max_name_length" to 256 or higher to ensure that CA works correctly. Your value is %1',
		...
		if ( $vs_request_max_varname_length && intval( $vs_request_max_varname_length ) < 256 ) {
			self::addError(
				_t( 'It looks like you have the PHP Suhoshin extension installed which introduces some default constraints than can prevent CA from saving information entered into forms.' )
				.
				_t( 'In particular, you have to set the configuration value "suhosin.request.max_varname_length" to 256 or higher to ensure that CA works correctly. Your value is %1',
		...
		if ( $vs_post_max_totalname_length && intval( $vs_post_max_totalname_length ) < 5012 ) {
			self::addError(
				_t( 'It looks like you have the PHP Suhoshin extension installed which introduces some default constraints than can prevent CA from saving information entered into forms.' )
				.
				_t( 'In particular, you have to set the configuration value "suhosin.post.max_totalname_length" to 5012 or higher to ensure that CA works correctly. Your value is %1',
		...
		if ( $vs_request_max_totalname_length && intval( $vs_request_max_totalname_length ) < 5012 ) {
			self::addError(
				_t( 'It looks like you have the PHP Suhoshin extension installed which introduces some default constraints than can prevent CA from saving information entered into forms.' )
				.
				_t( 'In particular, you have to set the configuration value "suhosin.request.max_totalname_length" to 5012 or higher to ensure that CA works correctly. Your value is %1',
			...
			while ( false !== ( $vs_file = readdir( $vr_handle ) ) ) {
				if ( $vs_file != "." && $vs_file != ".." ) {
					if ( is_dir( $vs_dir . "/" . $vs_file ) ) {
						$va_items   = array_merge( $va_items,
							self::getSubdirectoriesAsArray( $vs_dir . "/" . $vs_file ) );
		...
		if ( ( $vn_schema_revision = self::getSchemaVersion() ) < __CollectiveAccess_Schema_Rev__ ) {
			...
			for ( $vn_i = ( $vn_schema_revision + 1 ); $vn_i <= __CollectiveAccess_Schema_Rev__; $vn_i ++ ) {
				...
				if ( is_array( $va_new_messages = $o_updater->applyDatabaseUpdate() ) ) {
					...
					$va_messages["error_{$vn_i}_sql_fail"] = _t( 'Could not apply database update for migration %1',
						$vn_i );
				}
				// post-update tasks
				...
				if ( $vs_message = $o_updater->getPostupdateMessage() ) {
					$va_messages[ ( sizeof( $va_methods_that_errored ) ? "error" : "info" )
					              . "_{$vn_i}_{$vs_postupdate_method}_postupdate_message" ]
						= _t( "For migration %1", $vn_i ) . ": {$vs_message}";
				} else {
					if ( sizeof( $va_methods_that_errored ) ) {
						$va_messages["error_{$vn_i}_{$vs_postupdate_method}_postupdate_message"]
							= _t( "For migration %1", $vn_i ) . ": " . _t( "The following tasks did not complete: %1",
