...
class BaseAdvancedSearchController extends BaseRefineableSearchController
{
    ...
    public function Index($pa_options = null)
    {
        ...
        if (!(
            (($vn_form_id = (isset($pa_options['form_id'])) ? $pa_options['form_id'] : null) || ($vn_form_id = $this->getRequest(
                )->getParameter('form_id', pInteger)) || ($vn_form_id = $this->opo_result_context->getParameter(
                    'form_id'
                )))
            &&
        ...
        if (isset($pa_options['saved_search']) && $pa_options['saved_search']) {
            ...
            if (!($vs_search = $t_form->getLuceneQueryStringForHTMLFormInput(
                $_REQUEST
                ...
                $va_form_data = $t_form->extractFormValuesFromArray(
                    $_REQUEST
    ...
    public function getAdvancedSearchForm($pb_render_view = null)
    {
        ...
        if (!($vn_form_id = $this->request->getParameter('form_id', pInteger))) {
            if ((!($vn_form_id = $this->opo_result_context->getParameter('form_id'))) || (!$t_form->haveAccessToForm(
                    $this->request->getUserID(),
                    __CA_SEARCH_FORM_READ_ACCESS__,
                    $vn_form_id
                ))) {
                if (sizeof(
                    $va_forms = $t_form->getForms(
