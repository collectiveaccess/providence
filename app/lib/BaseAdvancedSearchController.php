...
class BaseAdvancedSearchController extends BaseRefineableSearchController {
	...
	public function Index( $pa_options = null ) {
		...
		$va_forms = $t_form->getForms( array(
		...
		if ( ! (
			( ( $vn_form_id = ( isset( $pa_options['form_id'] ) ) ? $pa_options['form_id'] : null )
			  || ( $vn_form_id
					= $this->getRequest()->getParameter( 'form_id', pInteger ) )
			  || ( $vn_form_id = $this->opo_result_context->getParameter( 'form_id' ) ) )
		...
		if ( isset( $pa_options['saved_search'] ) && $pa_options['saved_search'] ) {
			...
			if ( ! ( $vs_search
				= $t_form->getLuceneQueryStringForHTMLFormInput( $_REQUEST ) )
			) { // try to get search off of request
				$vs_search
					              = $this->opo_result_context->getSearchExpression();                // get the search out of the result context
				$va_form_data     = $this->opo_result_context->getParameter( 'form_data' );
				$vb_is_new_search = ! $this->opo_result_context->cacheIsValid()
				                    || $this->opb_type_restriction_has_changed;
			} else {
				$va_form_data
					              = $t_form->extractFormValuesFromArray( $_REQUEST );                // ah ok, its an incoming request, so get the form values out for interpretation/processing/whatever
				$vb_is_new_search = true;
			}
		}
		...
		if ( $vs_search && ( $vs_search != "" ) ) { /* any request? */
			...
			if ( $vb_is_new_search || isset( $pa_options['saved_search'] )
			     || ( is_subclass_of( $po_search, "BrowseEngine" ) && ! $po_search->numCriteria() )
			) {
		...
		switch ( $pa_options['output_format'] ) {
			# ------------------------------------
			case 'LABELS':
				$this->_genLabels( $vo_result, $this->request->getParameter( "label_form", pString ), $vs_search,
					$vs_search );
				break;
			# ------------------------------------
			case 'PDF':
			...
			# ------------------------------------
			case 'EXPORT':
				$this->_genExport( $vo_result, $this->request->getParameter( "export_format", pString ), $vs_search,
					$vs_search );
				break;
			# ------------------------------------
			case 'HTML':
	...
	public function getAdvancedSearchForm( $pb_render_view = null ) {
		...
		if ( ! ( $vn_form_id = $this->request->getParameter( 'form_id', pInteger ) ) ) {
			if ( ( ! ( $vn_form_id = $this->opo_result_context->getParameter( 'form_id' ) ) )
			     || ( ! $t_form->haveAccessToForm( $this->request->getUserID(), __CA_SEARCH_FORM_READ_ACCESS__,
					$vn_form_id ) )
			) {
				if ( sizeof( $va_forms = $t_form->getForms( array(
					'table'           => $this->ops_tablename,
					'user_id'         => $this->request->getUserID(),
					'access'          => __CA_SEARCH_FORM_READ_ACCESS__,
					'restrictToTypes' => [ $this->opn_type_restriction_id ]
