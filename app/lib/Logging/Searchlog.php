	...
	public function log( $pa_entry ) {
		if ( is_array( $pa_entry ) ) {
			$this->o_db->query( "
				...
				VALUES
				(unix_timestamp(), ?, ?, ?, ?, ?, ?, ?, ?, ?)
			", $pa_entry['user_id'], $pa_entry['table_num'], substr( $pa_entry['search_expression'], 0, 255 ),
				$pa_entry['num_hits'], $pa_entry['form_id'], $pa_entry['ip_addr'], $pa_entry['details'],
				$pa_entry['execution_time'], $pa_entry['search_source'] );
