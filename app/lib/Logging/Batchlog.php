...
class Batchlog extends BaseLogger
{
    ...
    public function log($pa_entry)
    {
        ...
        if (is_array($pa_entry)) {
            ...
            if (!in_array(
                $pa_entry['batch_type'],
                array("SR", "BE", "BD", "MI", "TC")
            )) {    // batch types are "SR" (search/replace), "BE" (batch editor), "BD" (batch delete), "MI" (media import), "TC" (type change)
                return false;
            }
            $this->o_db->query(
                "
    ...
    public function addItem($pn_row_id, $pa_errors = null)
    {
        ...
        $this->o_db->query(
			...
			",
            (int)$pn_row_id,
            (int)$this->opn_last_batch_id,
            caSerializeForDatabase($va_errors)
        );
