...
class RepresentableBaseModel extends BundlableLabelableBaseModelWithAttributes
{
    ...
    public function getRepresentations($pa_versions = null, $pa_version_sizes = null, $pa_options = null)
    {
        ...
        $qr_reps = $o_db->query(
            $vs_sql = "
				SELECT 	caor.representation_id, caor.media, caoor.is_primary, caor.access, caor.status, caor.is_transcribable,
						...
						caor.md5, caor.mimetype, caor.original_filename, caoor.`rank`, caoor.relation_id" . ($t_link->hasField(
                    'type_id'
                ) ? ', caoor.type_id rel_type_id' : '') . "
        ...
        if ($AUTH_CURRENT_USER_ID) {
            $va_can_read = caCanRead(
                $AUTH_CURRENT_USER_ID,
                'ca_object_representations',
                $qr_reps->getAllFieldValues('representation_id'),
                null,
        ...
        while ($qr_reps->nextRow()) {
            ...
            foreach ($pa_versions as $vs_version) {
                ...
                $va_tmp['dimensions'][$vs_version] = caGetRepresentationDimensionsForDisplay(
                    $qr_reps,
                    'original',
                    array()
                );
    ...
    {
        $va_restrict_to_types = caGetOption(
            'restrictToTypes',
            $pa_options,
            caGetOption('restrict_to_types', $pa_options, null)
        );
        $va_restrict_to_relationship_types = caGetOption(
        ...
        if ($va_restrict_to_relationship_types || $va_restrict_to_types) {
            if ($va_restrict_to_relationship_types && ($t_rel = Datamodel::getInstanceByTableName(
                    $ps_linking_table,
                    true
                )) && ($t_rel->hasField('type_id'))) {
                $va_restrict_to_relationship_types = caMakeRelationshipTypeIDList(
                    $ps_linking_table,
    ...
    {
        $va_mimetypes = array();
        if (isset($pa_options['mimetypes']) && (is_array($pa_options['mimetypes'])) && (sizeof(
                $pa_options['mimetypes']
            ))) {
            $va_mimetypes = array_flip($pa_options['mimetypes']);
        }

        $vs_sortby = null;
        if (isset($pa_options['sortby']) && $pa_options['sortby'] && in_array(
                $pa_options['sortby'],
                array('filesize', 'duration')
            )) {
        ...
        foreach ($va_reps as $vn_i => $va_rep) {
            if (is_array($va_mimetypes) && (sizeof(
                    $va_mimetypes
    ...
    public function getRepresentationIDs($pa_options = null)
    {
        ...
        if (!is_array($pa_options['return_with_access']) && is_array($pa_options['checkAccess']) && sizeof(
    ...
    public function getRepresentationCount($pa_options = null)
    {
        ...
        if (isset($pa_options['return_with_type']) && $pa_options['return_with_type']) {
            if (!is_numeric($pa_options['return_with_type'])) {
                $t_list = new ca_lists();
                $pa_options['return_with_type'] = $t_list->getItemIDFromList(
     ...
     *        checkAccess - synonym for return_with_access
     *
     * @return int A representation_id
     */
    ...
    public function addRepresentation(
            ...
            foreach ($va_match_on as $vs_match_on) {
                switch ($vs_match_on) {
                    case 'idno':
                        ...
                        $va_ids = ca_object_representations::find(
                            array('idno' => trim($pa_values['idno'])),
                            array(
                                'returnAs' => 'ids',
                    ...
                    case 'label':
                        ...
                        $va_ids = ca_object_representations::find(
                            array('preferred_labels' => array('name' => trim($pa_values['preferred_labels']['name']))),
        ...
        if (strlen($vn_center_x) && (strlen(
                $vn_center_y
            )) && ($vn_center_x >= 0) && ($vn_center_y >= 0) && ($vn_center_x <= 1) && ($vn_center_y <= 1)) {
    ...
    public function editRepresentation(
        ...
        if (!$t_rep->load(array('representation_id' => $pn_representation_id))) {
            $this->postError(
                750,
                _t("Representation id=%1 does not exist", $pn_representation_id),
            ...
            if (!$t_oxor->load(array($vs_pk => $vn_id, 'representation_id' => $pn_representation_id))) {
                $this->postError(
                    750,
                    _t(
                        "Representation id=%1 is not related to %3 id=%2",
                        $pn_representation_id,
                        $vn_id,
                        $this->getProperty('NAME_SINGULAR')
                    ),
                    "RepresentableBaseModel->editRepresentation()"
                );
                return false;
                ...
                if (isset($pa_options['rel_type_id']) && ($vn_rel_type_id = (int)$pa_options['rel_type_id']) && $t_oxor->hasField(
                        'type_id'
                    )) {
    ...
    public function removeRepresentation($pn_representation_id, $pa_options = null)
    {
        ...
        if (is_array($va_path) && sizeof($va_path) == 3) {
            ...
            if ($t_rel = Datamodel::getInstanceByTableName($vs_rel_table)) {
                ...
                if ($t_rel->load(
                    array($this->primaryKey() => $this->getPrimaryKey(), 'representation_id' => $pn_representation_id)
                )) {
                    ...
                    if (($vb_update_is_primary) && (is_array(
                            $va_rels = call_user_func(
                                "{$vs_rel_table}::find",
        ...
        if (!$t_rep->load($pn_representation_id)) {
            ...
            if (is_array(
                    $va_rels
                ) && isset($va_rels['ca_object_representation_labels'])) { // labels don't count as relationships in this case
                unset($va_rels['ca_object_representation_labels']);
            }
            if (is_array(
                    $va_rels
                ) && isset($va_rels['ca_objects_x_object_representations']) && ($va_rels['ca_objects_x_object_representations'] < 1)) {
    ...
    public function linkRepresentation($pn_representation_id, $pb_is_primary, $pa_options = null)
    {
        ...
        if (!ca_object_representations::find(
            array('representation_id' => $pn_representation_id),
        ...
        if ($t_oxor->hasField('type_id') && ($pm_type_id = caGetOption('type_id', $pa_options, null))) {
            ...
            if ($pm_type_id && !is_numeric($pm_type_id)) {
                ...
                if ($vs_linking_table = $t_rel_type->getRelationshipTypeTable(
                    ...
                    $this->postError(
                        2510,
                        _t('Type id "%1" is not valid', $pm_type_id),
    ...
    public function getBundleFormValues($ps_bundle_name, $ps_placement_code, $pa_bundle_settings, $pa_options = null)
    {
        ...
        $vs_bundle_template = caGetOption(
            'display_template',
            $pa_bundle_settings,
            Configuration::load()->get(
                'ca_object_representations_default_editor_display_template'
            ),
        ...
        if (is_array($va_reps) && sizeof($va_reps)) {
            ...
            if ($vs_bundle_template && ($vs_linking_table = RepresentableBaseModel::getRepresentationRelationshipTableName(
                    $this->tableName()
                ))) {
            ...
            foreach ($va_relation_ids as $relation_id) {
                ...
                (array_filter(
                    $va_reps,
                    function ($v) use ($relation_id) {
                        return ($v['relation_id'] == $relation_id);
                    }
                )) as $va_rep
                ) {
                    $vn_num_multifiles = $va_rep['num_multifiles'];
                    if ($vs_extracted_metadata = caFormatMediaMetadata(
                        caSanitizeArray(
                            caUnserializeForDatabase($va_rep['media_metadata']),
                            array('removeNonCharacterData' => true)
                        )
                    )) {
                        $vs_extracted_metadata = "<h3>" . _t(
                                'Extracted metadata'
                            ) . ":</h3>\n{$vs_extracted_metadata}\n";
                    }
                    $vs_md5 = isset($va_rep['info']['original']['MD5']) ? "<h3>" . _t(
                            'MD5 signature'
                        ) . ':</h3>' . $va_rep['info']['original']['MD5'] : '';
                    ...
                    $va_initial_values[$va_rep['relation_id']] = array(
                        ...
                        'filename' => $va_rep['info']['original_filename'] ? $va_rep['info']['original_filename'] : _t(
                            'Unknown'
                        ),
                        'num_multifiles' => ($vn_num_multifiles ? (($vn_num_multifiles == 1) ? _t(
                            '+ 1 additional preview'
                        ) : _t('+ %1 additional previews', $vn_num_multifiles)) : ''),
                        ...
                        'fetched' => $va_rep['fetched_from'] ? _t(
                            "<h3>Fetched from:</h3> URL %1 on %2 using %3 URL handler",
                            '<a href="' . $va_rep['fetched_from'] . '" target="_ext" title="' . $va_rep['fetched_from'] . '">' . $va_rep['fetched_from'] . '</a>',
                            date('c', $va_rep['fetched_on']),
                            caGetOption('fetched_by', $va_rep, 'default')
                        ) : ""
                    );

                    if (is_array($bundle_data[$va_rep['representation_id']])) {
                        $va_initial_values[$va_rep['relation_id']] = array_merge(
    ...
    public static function getTranscribable(array $options = null)
    {
        ...
        if ($restrict_to_types = caGetOption('restrictToTypes', $options, null)) {
            ...
            $types = caMakeTypeIDList(
                $return_representations ? 'ca_object_representations' : $table,
                $restrict_to_types
            );
        ...
        if (is_array($ids = $qr->getAllFieldValues($return_representations ? 'representation_id' : $pk)) && sizeof(
                $ids
            )) {
            ...
            switch ($return_as) {
                ...
                case 'firstModelInstance':
                    $id = array_shift($ids);
                    $t_instance = DataModel::getInstance(
                        $return_representations ? 'ca_object_representations' : $table,
                        false
                    );
    ...
    public static function getTranscriptionStatusForIDs(array $ids, array $options = null)
    {
        ...
        while ($qr->nextRow()) {
            ...
            if ($items[$item_id]['is_completed']) {
                ...
                if ($items[$item_id]['has_transcription']) {
