...
class RepresentableBaseModel extends BundlableLabelableBaseModelWithAttributes {
	# ------------------------------------------------------
	/**
	 * Returns information about representations linked to the currently loaded item. Use this if you want to get the
	 * urls, tags and other information for all representations associated with a given item.
	 *
	 * @param array $pa_versions      An array of media versions to include information for. If you omit this then a
	 *                                single version, 'preview170', is assumed by default.
	 * @param array $pa_version_sizes Optional array of sizes to force specific versions to. The array keys are version
	 *                                names; the values are arrays with two keys: 'width' and 'height'; if present
	 *                                these values will be used in lieu of the actual values in the database
	 * @param array $pa_options       An optional array of options to use when getting representation information.
	 *                                Supported options are: return_primary_only - If true then only the primary
	 *                                representation will be returned [Default is false] primaryOnly = Synonym for
	 *                                return_primary_only return_with_access - Set to an array of access values to
	 *                                filter representation through; only representations with an access value in the
	 *                                list will be returned checkAccess - synonym for return_with_access start = limit
	 *                                = restrict_to_types = An array of type_ids or type codes to restrict count to
	 *                                specified types of representations to restrict_to_relationship_types = An array
	 *                                of relationship type_ids or relationship codes to restrict count to
	...
	public function getRepresentations( $pa_versions = null, $pa_version_sizes = null, $pa_options = null ) {
		...
		$qr_reps = $o_db->query( $vs_sql = "
				SELECT 	caor.representation_id, caor.media, caoor.is_primary, caor.access, caor.status, caor.is_transcribable,
						...
						caor.md5, caor.mimetype, caor.original_filename, caoor.`rank`, caoor.relation_id"
		                                   . ( $t_link->hasField( 'type_id' ) ? ', caoor.type_id rel_type_id' : '' ) . "
		...
		if ( $AUTH_CURRENT_USER_ID ) {
			$va_can_read = caCanRead( $AUTH_CURRENT_USER_ID, 'ca_object_representations',
				$qr_reps->getAllFieldValues( 'representation_id' ), null, array( 'returnAsArray' => true ) );
		} else {
		...
		while ( $qr_reps->nextRow() ) {
			...
			foreach ( $pa_versions as $vs_version ) {
				...
				if ( $vn_width && $vn_height ) {
					$va_tmp['tags'][ $vs_version ] = $qr_reps->getMediaTag( 'media', $vs_version,
						array_merge( $pa_options,
							array( 'alt' => $alt_text, 'viewer_width' => $vn_width, 'viewer_height' => $vn_height ) ) );
				} else {
					$va_tmp['tags'][ $vs_version ] = $qr_reps->getMediaTag( 'media', $vs_version,
				...
				$va_tmp['dimensions'][ $vs_version ] = caGetRepresentationDimensionsForDisplay( $qr_reps, 'original',
					array() );
			}
	...
	/**
	 * General SQL query WHERE clauses and parameters to restrict queries to specific representation and/or
	 * relationship types
	 */
	private function _getRestrictionSQL( $ps_linking_table, $pn_id, $pa_options ) {
		$va_restrict_to_types              = caGetOption( 'restrictToTypes', $pa_options,
			caGetOption( 'restrict_to_types', $pa_options, null ) );
		$va_restrict_to_relationship_types = caGetOption( 'restrictToRelationshipTypes', $pa_options,
			caGetOption( 'restrict_to_relationship_types', $pa_options, null ) );

		$vs_filter_sql = '';
		$pa_params     = array( $pn_id );
		if ( $va_restrict_to_relationship_types || $va_restrict_to_types ) {
			if ( $va_restrict_to_relationship_types
			     && ( $t_rel = Datamodel::getInstanceByTableName( $ps_linking_table, true ) )
			     && ( $t_rel->hasField( 'type_id' ) )
			) {
				$va_restrict_to_relationship_types = caMakeRelationshipTypeIDList( $ps_linking_table,
	...
	/**
	 * Finds and returns information about representations meeting the specified criteria. Returns information in the
	 * same format as getRepresentations()
	 *
	 ...
	 *                          sortby = if set, representations are return sorted using the criteria in ascending
	 *                          order. Valid values are 'filesize' (sort by file size), 'duration' (sort by length of
	 *                          time-based media)
	 *
	 * @return array List of representations. Each entry in the list is an associative array of the same format as
	 *               returned by getRepresentations() and includes properties, tags and urls for the representation.
	 */
	public function findRepresentations( $pa_options ) {
		$va_mimetypes = array();
		if ( isset( $pa_options['mimetypes'] ) && ( is_array( $pa_options['mimetypes'] ) )
		     && ( sizeof( $pa_options['mimetypes'] ) )
		) {
			$va_mimetypes = array_flip( $pa_options['mimetypes'] );
		}

		$vs_sortby = null;
		if ( isset( $pa_options['sortby'] ) && $pa_options['sortby']
		     && in_array( $pa_options['sortby'], array( 'filesize', 'duration' ) )
		) {
		...
		foreach ( $va_reps as $vn_i => $va_rep ) {
			if ( is_array( $va_mimetypes ) && ( sizeof( $va_mimetypes ) )
			     && isset( $va_mimetypes[ $va_rep['info']['original']['MIMETYPE'] ] )
			) {
				switch ( $vs_sortby ) {
					case 'filesize':
	 ...
	 *                          return_with_access - Set to an array of access values to filter representation through;
	 *                          only representations with an access value in the list will be returned checkAccess -
	 *                          synonym for return_with_access restrict_to_types = An array of type_ids or type codes
	 *                          to restrict count to specified types of representations to
	 *                          restrict_to_relationship_types = An array of relationship type_ids or relationship
	 *                          codes to restrict count to
	 *
	...
	public function getRepresentationIDs( $pa_options = null ) {
		...
		if ( ! is_array( $pa_options['return_with_access'] ) && is_array( $pa_options['checkAccess'] )
		     && sizeof( $pa_options['checkAccess'] ) > 0
		) {
			$pa_options['return_with_access'] = $pa_options['checkAccess'];
		}
	 ...
	 *                          restrict_to_types = An array of type_ids or type codes to restrict count to specified
	 *                          types of representations to restrict_to_relationship_types = An array of relationship
	 *                          type_ids or relationship codes to restrict count to return_with_type - A type to
	 *                          restrict the count to. Can be either an integer type_id or item_code string
	 *                          return_with_access - An array of access values to restrict counts to checkAccess -
	 *                          synonym for return_with_access
	 *
	...
	public function getRepresentationCount( $pa_options = null ) {
		...
		if ( isset( $pa_options['return_with_type'] ) && $pa_options['return_with_type'] ) {
			if ( ! is_numeric( $pa_options['return_with_type'] ) ) {
				$t_list                         = new ca_lists();
				$pa_options['return_with_type'] = $t_list->getItemIDFromList( 'object_representation_types',
					$pa_options['return_with_type'] );
			}
	 ...
	 *                                these values will be used in lieu of the actual values in the database
	 * @param array $pa_options       An optional array of options to use when getting representation information.
	 *                                Supported options are: return_with_access - Set to an array of access values to
	 *                                filter representation through; only representations with an access value in the
	 *                                list will be returned checkAccess - synonym for return_with_access
	 *                                .. and options supported by getMediaTag() .. [they are passed through]
	 *
	 * @return array An array of information about the linked representations
	 */
	public function getPrimaryRepresentation( $pa_versions = null, $pa_version_sizes = null, $pa_options = null ) {
		...
		if ( is_array( $va_reps = $this->getRepresentations( $pa_versions, $pa_version_sizes,
			array_merge( $pa_options, array( 'return_primary_only' => 1 ) ) ) )
		) {
	 ...
	 *
	 * @param array $pa_options An optional array of options to use when getting representation information. Supported
	 *                          options are: return_with_access - Set to an array of access values to filter
	 *                          representation through; only representations with an access value in the list will be
	 *                          returned checkAccess - synonym for return_with_access
	 *
	 ...
	 * Use this if you want to efficiently access information, including attributes, labels and intrinsics, for all
	 * representations associated with a given item.
	 *
	 * @param array $pa_options An optional array of options to use when getting representation information. Supported
	 *                          options are: return_primary_only - If true then only the primary representation will be
	 *                          returned return_with_access - Set to an array of access values to filter representation
	 *                          through; only representations with an access value in the list will be returned
	 ...
	 * @param $pn_type_id    - the item_id of the representation type, in the ca_list with list_code
	 *                       'object_represention_types'
	 * @param $pn_locale_id  - the locale_id of the locale of the representation
	 * @param $pn_status     - the status code for the representation (as defined in item_value fields of items in the
	 *                       'workflow_statuses' ca_list)
	 * @param $pn_access     - the access code for the representation (as defined in item_value fields of items in the
	 *                       'access_statuses' ca_list)
	 * @param $pb_is_primary - if set to true, representation is designated "primary." Primary representation are used
	 *                       in cases where only one representation is required (such as search results). If a primary
	 *                       representation is already attached to this item, then it will be changed to non-primary as
	 *                       only one representation can be primary at any given time. If no primary representations
	 *                       exist, then the new representation will always be marked primary no matter what the
	 *                       setting of this parameter (there must always be a primary representation, if
	 *                       representations are defined).
	 * @param $pa_values     - array of attributes to attach to new representation
	 * @param $pa_options    - an array of options passed through to BaseModel::set() when creating the new
	 *                       representation. Currently supported options: original_filename - the name of the file
	 *                       being uploaded; will be recorded in the database and used as the filename when the file is
	 *                       subsequently downloaded rank - a numeric rank used to order the representations when
	 *                       listed returnRepresentation = if set the newly created ca_object_representations instance
	 *                       is returned rather than the link_id of the newly created relationship record matchOn =
	 *                       centerX = Horizontal position of image center used when cropping as a percentage expressed
	 *                       as a decimal between 0 and 1. If omitted existing value is maintained. Note that both
	 *                       centerX and centerY must be specified for the center to be changed. centerY = Vertical
	 *                       position of image center used when cropping as a percentage expressed as a decimal between
	 *                       0 and 1. If omitted existing value is maintained. Note that both centerX and centerY must be specified for the center to be changed.
	 *
	 * @return mixed Returns primary key (link_id) of the relatipnship row linking the newly created representation to
	 *               the item; if the 'returnRepresentation' is set then an instance for the newly created
	 *               ca_object_representations is returned instead; boolean false is returned on error
	 */
	public function addRepresentation(
		$ps_media_path, $pn_type_id, $pn_locale_id, $pn_status, $pn_access, $pb_is_primary, $pa_values = null,
		$pa_options = null
	) {
		if ( ! ( $vn_id = $this->getPrimaryKey() ) ) {
			return null;
			...
			foreach ( $va_match_on as $vs_match_on ) {
				switch ( $vs_match_on ) {
					case 'idno':
						...
						$va_ids = ca_object_representations::find( array( 'idno' => trim( $pa_values['idno'] ) ),
					...
					case 'label':
						...
						$va_ids
							= ca_object_representations::find( array( 'preferred_labels' => array( 'name' => trim( $pa_values['preferred_labels']['name'] ) ) ),
							array( 'returnAs' => 'ids', 'transaction' => $this->getTransaction() ) );
						break;
				}
		...
		if ( ! $vn_rep_id ) {
			...
			if ( $t_rep->getPreferredLabelCount() == 0 ) {
				$vs_label = ( isset( $pa_values['name'] ) && $pa_values['name'] ) ? $pa_values['name']
		...
		if ( strlen( $vn_center_x ) && ( strlen( $vn_center_y ) ) && ( $vn_center_x >= 0 ) && ( $vn_center_y >= 0 )
		     && ( $vn_center_x <= 1 )
		     && ( $vn_center_y <= 1 )
		) {
	...
	/**
	 * Convenience method to edit a representation instance. Allows to you edit a linked representation from an
	 * instance.
	 *
	 ...
	 * @param int    $pn_status
	 * @param int    $pn_access
	 * @param bool   $pb_is_primary Sets 'primaryness' of representation. If you wish to leave the primary setting to
	 *                              its current value set this null or omit the parameter.
	 * @param array  $pa_values
	 * @param array  $pa_options
	 *                              centerX = Horizontal position of image center used when cropping as a percentage
	 *                              expressed as a decimal between 0 and 1. If omitted existing value is maintained.
	 *                              Note that both centerX and centerY must be specified for the center to be changed.
	 *                              centerY = Vertical position of image center used when cropping as a percentage
	 *                              expressed as a decimal between 0 and 1. If omitted existing value is maintained.
	 *                              Note that both centerX and centerY must be specified for the center to be changed.
	 *                              label = Preferred label in specified locale for representation. [Default is null]
	 *                              type_id = Type to force representation to. [Default is null] rel_type_id =
	 *                              Relationship type force representation to use. [Default is null]
	 *
	 * @return bool ca_object_representations model instance on success, false on failure, null if no row has been
	 *              loaded into the object model
	 */
	public function editRepresentation(
		$pn_representation_id, $ps_media_path, $pn_locale_id, $pn_status, $pn_access, $pb_is_primary = null,
		$pa_values = null, $pa_options = null
	) {
		if ( ! ( $vn_id = $this->getPrimaryKey() ) ) {
			return null;
		}
		...
		if ( ! $t_rep->load( array( 'representation_id' => $pn_representation_id ) ) ) {
			$this->postError( 750, _t( "Representation id=%1 does not exist", $pn_representation_id ),
				"RepresentableBaseModel->editRepresentation()" );
			...
			if ( $pn_locale_id && ( $ps_label = caGetOption( 'label', $pa_options, null ) ) ) {
				$t_rep->replaceLabel( array( 'name' => $ps_label ), $pn_locale_id, null, true,
			...
			if ( ! $t_oxor->load( array( $vs_pk => $vn_id, 'representation_id' => $pn_representation_id ) ) ) {
				$this->postError( 750,
					_t( "Representation id=%1 is not related to %3 id=%2", $pn_representation_id, $vn_id,
						$this->getProperty( 'NAME_SINGULAR' ) ), "RepresentableBaseModel->editRepresentation()" );
				...
				if ( isset( $pa_options['rel_type_id'] ) && ( $vn_rel_type_id = (int) $pa_options['rel_type_id'] )
				     && $t_oxor->hasField( 'type_id' )
				) {
	...
	/**
	 * Remove a single representation from the currently loaded object. Note that the representation will be removed
	 * from the database completely, so if it is also linked to other items it will be removed from them as well.
	 *
	 ...
	 *
	 * @return bool True if delete succeeded, false if there was an error. You can get the error messages by calling
	 *              getErrors() on the instance.
	 */
	public function removeRepresentation( $pn_representation_id, $pa_options = null ) {
		...
		if ( is_array( $va_path ) && sizeof( $va_path ) == 3 ) {
			...
			if ( $t_rel = Datamodel::getInstanceByTableName( $vs_rel_table ) ) {
				...
				if ( $t_rel->load( array(
					...
					if ( ( $vb_update_is_primary )
					     && ( is_array( $va_rels = call_user_func( "{$vs_rel_table}::find",
							[ $this->primaryKey() => $this->getPrimaryKey() ], [ 'returnAs' => 'arrays' ] ) ) )
					     && sizeof( $va_rels )
					) {
		...
		if ( ! $t_rep->load( $pn_representation_id ) ) {
			$this->postError( 750, _t( "Representation id=%1 does not exist", $pn_representation_id ),
				"RepresentableBaseModel->removeRepresentation()" );
			...
			if ( is_array( $va_rels )
			     && isset( $va_rels['ca_object_representation_labels'] )
			) { // labels don't count as relationships in this case
				unset( $va_rels['ca_object_representation_labels'] );
			}
			if ( is_array( $va_rels ) && isset( $va_rels['ca_objects_x_object_representations'] )
			     && ( $va_rels['ca_objects_x_object_representations'] < 1 )
			) {
	 ...
	 * @param $pb_is_primary        - if set to true, representation is designated "primary." Primary representation
	 *                              are used in cases where only one representation is required (such as search
	 *                              results). If a primary representation is already attached to this item, then it
	 *                              will be changed to non-primary as only one representation can be primary at any
	 *                              given time. If no primary representations exist, then the new representation will
	 *                              always be marked primary no matter what the setting of this parameter (there must
	 *                              always be a primary representation, if representations are defined).
	 * @param $pa_options           - an array of options passed through to BaseModel::set() when creating the new
	 *                              representation. Currently supported options: rank - a numeric rank used to order
	 *                              the representations when listed
	 *
	 * @return mixed Returns primary key (link_id) of the relatipnship row linking the  representation to the item;
	...
	public function linkRepresentation( $pn_representation_id, $pb_is_primary, $pa_options = null ) {
		...
		if ( ! ca_object_representations::find( array( 'representation_id' => $pn_representation_id ),
			...
			if ( $pm_type_id && ! is_numeric( $pm_type_id ) ) {
				...
				if ( $vs_linking_table = $t_rel_type->getRelationshipTypeTable( $this->tableName(),
					...
					$this->postError( 2510, _t( 'Type id "%1" is not valid', $pm_type_id ),
						'RepresentableBaseModel->linkRepresentation()' );
	 ...
	 *                           "video" and "document"
	 * @param array  $pa_options Options for selection of representations to count; same as options for
	 ...
	 * @param array  $pa_options Options for selection of representations to return; same as options for
	 *                           self::getRepresentations()
	 *
	...
	public function representationsWithMimeType( $pa_mimetypes, $pa_options = null ) {
		...
		$va_rep_list = array();
		if ( is_array(
			$va_reps = $this->getRepresentations( caGetOption( 'versions', $pa_options, null ), null, $pa_options ) )
		) {
			foreach ( $va_reps as $vn_rep_id => $va_rep ) {
	 ...
	 *                           self::getRepresentations()
	 *
	 * @return array An array with information about the matching representation, in the same format as that returned
	 *               by self::getRepresentations(), or null if there is no match
	 */
	 ...
	 */
	public function getObjectRepresentationAccessStatusHTMLFormBundle(
	...
	public function getRelatedBundleFormValues(
		...
		if ( $ps_related_table === 'ca_object_representations' ) {
			return $this->getBundleFormValues( $ps_related_table, $ps_placement_code, $pa_bundle_settings,
				$pa_options );
		}

		return parent::getRelatedBundleFormValues( $po_request, $ps_form_name, $ps_related_table, $ps_placement_code,
			$pa_bundle_settings, $pa_options );
	}
	# ------------------------------------------------------
	...
	public function getBundleFormValues( $ps_bundle_name, $ps_placement_code, $pa_bundle_settings, $pa_options = null
		...
		$vs_bundle_template = caGetOption( 'display_template', $pa_bundle_settings,
			Configuration::load()->get( 'ca_object_representations_default_editor_display_template' ),
		...
		if ( is_array( $va_reps ) && sizeof( $va_reps ) ) {
			$o_type_config               = Configuration::load( $t_item->getAppConfig()
			                                                           ->get( 'annotation_type_config' ) );
			...
			if ( $vs_bundle_template
			     && ( $vs_linking_table
					= RepresentableBaseModel::getRepresentationRelationshipTableName( $this->tableName() ) )
			) {
				$va_display_template_values = caProcessTemplateForIDs( $vs_bundle_template, $vs_linking_table,
					$va_relation_ids, array_merge( $pa_options, array(
						'start'                     => null,
						'limit'                     => null,
						'returnAsArray'             => true,
			...
			foreach ( $va_relation_ids as $relation_id ) {
				foreach (
				( array_filter( $va_reps, function ( $v ) use ( $relation_id ) {
					return ( $v['relation_id'] == $relation_id );
				} ) ) as $va_rep
				) {

					$vn_num_multifiles = $va_rep['num_multifiles'];
					if ( $vs_extracted_metadata
						= caFormatMediaMetadata( caSanitizeArray( caUnserializeForDatabase( $va_rep['media_metadata'] ),
						array( 'removeNonCharacterData' => true ) ) )
					) {
						$vs_extracted_metadata = "<h3>" . _t( 'Extracted metadata' )
						                         . ":</h3>\n{$vs_extracted_metadata}\n";
					}
					$vs_md5 = isset( $va_rep['info']['original']['MD5'] ) ? "<h3>" . _t( 'MD5 signature' ) . ':</h3>'
					                                                        . $va_rep['info']['original']['MD5'] : '';
					...
					$va_initial_values[ $va_rep['relation_id'] ] = array(
						...
						'idno'                     => $va_rep['idno'],
						'_display'                 => ( $vs_bundle_template
						                                && isset( $va_display_template_values[ $relation_id ] ) )
							? $va_display_template_values[ $relation_id ] : '',
						...
						'annotation_type'          => isset( $va_annotation_type_mappings[ $va_rep['info']['original']['PROPERTIES']['mimetype'] ] )
							? $va_annotation_type_mappings[ $va_rep['info']['original']['PROPERTIES']['mimetype'] ]
							: null,
						'type'                     => $va_rep['info']['original']['PROPERTIES']['typename'],
						'dimensions'               => $va_rep['dimensions']['original'],
						'filename'                 => $va_rep['info']['original_filename']
							? $va_rep['info']['original_filename'] : _t( 'Unknown' ),
						'num_multifiles'           => ( $vn_num_multifiles ? ( ( $vn_num_multifiles == 1 )
							? _t( '+ 1 additional preview' ) : _t( '+ %1 additional previews', $vn_num_multifiles ) )
							: '' ),
						...
						'fetched'                  => $va_rep['fetched_from']
							? _t( "<h3>Fetched from:</h3> URL %1 on %2 using %3 URL handler",
								'<a href="' . $va_rep['fetched_from'] . '" target="_ext" title="'
								. $va_rep['fetched_from'] . '">' . $va_rep['fetched_from'] . '</a>',
								date( 'c', $va_rep['fetched_on'] ), caGetOption( 'fetched_by', $va_rep, 'default' ) )
							: ""
					);

					if ( is_array( $bundle_data[ $va_rep['representation_id'] ] ) ) {
						$va_initial_values[ $va_rep['relation_id'] ]
							= array_merge( $va_initial_values[ $va_rep['relation_id'] ],
							$bundle_data[ $va_rep['representation_id'] ] );
					}
	 ...
	 *                       when the calling subclass is searchable (ie. <classname>Search and <classname>SearchResult
	 *                       classes are defined) ids                        = an array of ids (aka. primary keys)
	 *                       modelInstances            = an array of instances, one for each match. Each instance is
	 *                       the same class as the caller, a subclass of BaseModel firstId                    = the id
	 *                       (primary key) of the first match. This is the same as the first item in the array returned
	 *                       by 'ids' firstModelInstance        = the instance of the first match. This is the same as
	 *                       the first instance in the array returned by 'modelInstances' count                    =
	 *                       the number of matches
	 *
	 *        returnRepresentations = Set to return representations. Otherwise a list of primary records is returned.
	 *        [Default is false]
	 *
	 * @return mixed
	 */
	public static function getTranscribable( array $options = null ) {
		...
		if ( $restrict_to_types = caGetOption( 'restrictToTypes', $options, null ) ) {
			...
			$types = caMakeTypeIDList( $return_representations ? 'ca_object_representations' : $table,
				$restrict_to_types );
		...
		if ( is_array( $ids = $qr->getAllFieldValues( $return_representations ? 'representation_id' : $pk ) )
		     && sizeof( $ids )
		) {
			...
			switch ( $return_as ) {
				...
				case 'firstModelInstance':
					$id         = array_shift( $ids );
					$t_instance = DataModel::getInstance( $return_representations ? 'ca_object_representations'
						: $table, false );
	 ...
	 * following keys:
	 *        status = One of the following constants: __CA_TRANSCRIPTION_STATUS_NOT_STARTED__,
	 *        __CA_TRANSCRIPTION_STATUS_IN_PROGRESS__, __CA_TRANSCRIPTION_STATUS_COMPLETED__ has_transcription = True
	 *        if at least one transcription exists is_completed = Set to the Unix timestamp of the data/time of
	 *        completion, or null if not complete
	 *
	 * @param array $ids
