...
trait HistoryTrackingCurrentValueTrait
{
     ...
     *
     */
    static public $s_history_tracking_current_value_type_configuration_cache = [];

    /**
     *
     */
    static public $s_history_tracking_newly_added_current_values = [];

    /**
     *
     */
    static public $s_history_tracking_deleted_current_values = [];
    ...
    public static function policy2bundleconfig($options = null)
    {
        ...
        if ($policy && is_array($history_tracking_policies) && is_array(
                $history_tracking_policies['policies']
        ...
        foreach ($map as $table => $types) {
            ...
            if (is_array($types)) {
                ...
                foreach ($types as $type_list => $config) {
                    if (!is_array(
                        $config
                    )) {    // some old configs omit the types, so we hoist the values into a config for all types here
                        $config = $types;
                        $type_list = '*';
                        $exit = true;
                    }
                    ...
                    foreach ($types as $type) {
                        ...
                        if ((sizeof($path) === 3) && ($rel_types = caGetOption(
                                ['restrictToRelationshipTypes', 'showRelationshipTypes'],
                                $config,
                                null
                            )) && $path[1]) {
                            $bundle_settings["{$table}_showRelationshipTypes"] = [];
                            foreach ($rel_types as $rel_type) {
                                if (($rel_type_id = $t_rel_type->getRelationshipTypeID(
                                        $path[1],
                                        $rel_type
    ...
    {
        if (is_array($policy = self::getHistoryTrackingCurrentValuePolicy($policy)) && is_array(
                $map = $policy['elements']
            ) && is_array($map[$table])) {
    ...
    public static function getHistoryTrackingCurrentValuePolicy($policy, $key = null)
    {
        if ($policy && is_array(
                $history_tracking_policies = self::getHistoryTrackingCurrentValuePolicyConfig()
            ) && is_array($history_tracking_policies['policies']) && is_array(
                $history_tracking_policies['policies'][$policy]
            )) {
            if ($key) {
    ...
    {
        if (is_array($history_tracking_policies = self::getHistoryTrackingCurrentValuePolicyConfig()) && is_array(
                $history_tracking_policies['defaults']
    ...
    public static function getConfigurationForHistoryTrackingCurrentValue(
        ...
        $policy_config = self::getHistoryTrackingCurrentValuePolicy(
            $policy
        ); //$o_config->getAssoc('current_location_criteria');
        ...
        if (isset($map[$table])) {
            if ((!$type_id) && (isset($map[$table]['*']) || isset($map[$table]['__default__']))) {
                return self::$s_history_tracking_current_value_type_configuration_cache[caMakeCacheKeyFromOptions(
                    $options,
                    "{$table}/{$type_id}"
                )] = self::$s_history_tracking_current_value_type_configuration_cache[$cache_key] = $map[$table]['*'];
            ...
            if ($type_id) {
                ...
                if ($type && isset($map[$table][$type])) {
                    ...
                    return self::$s_history_tracking_current_value_type_configuration_cache[caMakeCacheKeyFromOptions(
                        $options,
                        "{$table}/{$type_id}"
                    )] = self::$s_history_tracking_current_value_type_configuration_cache[$cache_key] = $map[$table][$type];
                    ...
                    if (is_array(
                            $facet_display_config
                        ) && (isset($facet_display_config[$table]['*']) || isset($facet_display_config[$table]['__default__']))) {
    ...
    public function setHistoryTrackingCurrentValue($policy, $values = null, $options = null)
    {
        ...
        if (is_null($values) && !$is_future) {
            if ($l = ca_history_tracking_current_values::find(
                ['policy' => $policy, 'table_num' => $subject_table_num, 'row_id' => $row_id],
                ['returnAs' => 'firstModelInstance', 'transaction' => $this->getTransaction()]
            )) {
                $l->setDb($this->getDb());
                self::$s_history_tracking_deleted_current_values[$l->get('tracked_table_num')][$l->get(
                    'tracked_row_id'
                )][$policy] =
                self::$s_history_tracking_deleted_current_values[$l->get('current_table_num')][$l->get(
                    'current_row_id'
                )][$policy] =
        ...
        if (($values['current_table_num'] && !Datamodel::getTableName(
                    $values['current_table_num']
    ...
    public function deriveHistoryTrackingCurrentValue($options = null)
    {
        ...
        if (is_array($policies = self::getHistoryTrackingCurrentValuePolicies($this->tableName()))) {
            foreach ($policies as $policy => $policy_info) {
                ...
                if ($current_entry) {
                    if (!($this->setHistoryTrackingCurrentValue(
                        $policy,
                        $current_entry,
                    ...
                    $this->setHistoryTrackingCurrentValue(
                        $policy,
                        null,
                        ['row_id' => $row_id, 'isFuture' => $is_future]
    ...
    public function updateDependentHistoryTrackingCurrentValues($options = null)
    {
        ...
        if (is_array(
            $policies = self::getDependentHistoryTrackingCurrentValuePolicies(
                $this->tableName(),
    ...
    public function isInHomeLocation($policy = null, $options = null)
    {
        ...
        if (is_array(
                $current_value
            ) && ($current_value['type'] === 'ca_storage_locations') && ($current_value['id'] == $home_location_id)) {
            return true;
        }
        return false;
    }
    ...
    public function getHistory($options = null)
    {
        ...
        if ((($pb_date_mode = caGetOption(
                    'dateMode',
                    $options,
                    false
                )) === 'dateless') || ($pb_date_mode = (caGetOption(
                    'dateMode',
                    $pa_bundle_settings,
                    false
                )) === 'dateless')) {
        ...
        // Lots
        $linking_table = null;
        if (is_array($path = Datamodel::getPath($table, 'ca_object_lots')) && (((sizeof(
                            $path
                        ) == 2) && ($vn_lot_id = $qr->get("{$table}.lot_id"))) || ((sizeof(
                            $path
                        ) == 3) && ($path = array_keys($path)) && ($linking_table = $path[1])))) {
            if ($linking_table) {
                    ...
                    if (!sizeof($va_dates)) {
                        $va_dates[] = [
                            'sortable' => $vn_date = caUnixTimestampToHistoricTimestamps(
                                $qr_lots->getCreationTimestamp(null, array('timestampOnly' => true))
                            ),
        ...
        // Loans
        if (is_array($path = Datamodel::getPath($table, 'ca_loans')) && (sizeof($path) == 3) && ($path = array_keys(
                $path
            ...
            if (caGetOption('ca_loans_includeFromChildren', $pa_bundle_settings, false)) {
                $va_child_loans = array_reduce(
                    $qr->getWithTemplate(
                        "<unit relativeTo='{$table}.children' delimiter=';'>^{$linking_table}.relation_id</unit>",
                        ['returnAsArray' => true]
                    ),
                    function ($c, $i) {
                    ...
                    foreach ($va_dates as $va_date) {
                        ...
                        $va_history[$va_date['sortable']][] = array(
                            ...
                            'display' => $qr_loans->getWithTemplate(
                                ($vn_rel_row_id != $row_id) ? $vs_child_display_template : $vs_display_template
                            ),
                            'color' => $vs_color,
                            'icon_url' => $vs_icon_url = $o_media_coder->getMediaTag('icon'),
        ...
        // Movements
        if (is_array($path = Datamodel::getPath($table, 'ca_movements')) && (sizeof($path) == 3) && ($path = array_keys(
                $path
            )) && ($linking_table = $path[1])) {
            ...
            if (is_array(
                    $va_movement_types = caGetOption('ca_movements_showTypes', $pa_bundle_settings, null)
                ) && is_array($va_movements)) {
                $qr_movements = caMakeSearchResult(
                    $linking_table,
                    $va_movements,
                    [
                        'transaction' => $this->getTransaction(),
        ...
        // Occurrences
        if (is_array($path = Datamodel::getPath($table, 'ca_occurrences')) && (sizeof(
                    $path
                ) == 3) && ($path = array_keys($path)) && ($linking_table = $path[1])) {
            ...
            if (is_array(
                ...
                $qr_occurrences = caMakeSearchResult(
                    $linking_table,
                    $va_occurrences,
                    [
                        'transaction' => $this->getTransaction(),
                ...
                while ($qr_occurrences->nextHit()) {
                    ...
                    $vs_display_template = $pb_display_label_only ? $vs_default_display_template : caGetOption(
                        ["ca_occurrences_{$vs_type_idno}_displayTemplate", "ca_occurrences_displayTemplate"],
                        $pa_bundle_settings,
                        $vs_default_display_template
                    );
        ...
        // entities
        if (is_array($path = Datamodel::getPath($table, 'ca_entities')) && (sizeof($path) == 3) && ($path = array_keys(
                $path
            )) && ($linking_table = $path[1])) {
            $va_entities = $qr->get("{$linking_table}.relation_id", array('returnAsArray' => true));
            $va_child_entities = [];
            if (is_array(
                    $va_entity_types = caGetOption('ca_entities_showTypes', $pa_bundle_settings, null)
                ...
                foreach ($va_entity_types as $vn_type_id) {
                    if (caGetOption(
                        "ca_entities_{$va_entity_type_info[$vn_type_id]['idno']}_includeFromChildren",
                        $pa_bundle_settings,
                        false
                    )) {
                        $va_child_entities = array_reduce(
                            $qr->getWithTemplate(
                                "<unit relativeTo='{$table}.children' delimiter=';'>^{$linking_table}.relation_id</unit>",
                                ['returnAsArray' => true]
                            ),
                ...
                $qr_entities = caMakeSearchResult(
                    $linking_table,
                    $va_entities,
                    [
                        'transaction' => $this->getTransaction(),
                ...
                foreach ($va_entity_types as $vn_type_id) {
                    if (!is_array(
                            $va_date_elements = caGetOption(
                                "ca_entities_{$va_entity_type_info[$vn_type_id]['idno']}_dateElement",
                                $pa_bundle_settings,
                                null
                            )
                        ) && $va_date_elements) {
                ...
                while ($qr_entities->nextHit()) {
                    ...
                    $vs_display_template = $pb_display_label_only ? $vs_default_display_template : caGetOption(
                        ["ca_entities_{$vs_type_idno}_displayTemplate", "ca_entities_displayTemplate"],
                        $pa_bundle_settings,
                        $vs_default_display_template
                    );
                    $vs_child_display_template = $pb_display_label_only ? $vs_default_child_display_template : caGetOption(
                    ...
                    if ($pb_date_mode) {
                        ...
                        if (is_array($va_date_elements_by_type[$vn_type_id]) && sizeof(
                            ...
                            foreach ($va_date_elements_by_type[$vn_type_id] as $vs_date_element) {
                                ...
                                $vs_date_spec = (Datamodel::tableExists(
                                    $va_date_bits[0]
                                )) ? $vs_date_element : "ca_entities.{$vs_date_element}";
                                $va_dates[] = array(
                        ...
                        if (!sizeof($va_dates)) {
                            $va_dates[] = array(
                                'sortable' => $vn_date = caUnixTimestampToHistoricTimestamps(
                                    $qr_entities->get('lastModified.direct')
                                ),
                                'bounds' => array(0, $vn_date),
                                'display' => caGetLocalizedDate($vn_date)
                            );
                    ...
                    foreach ($va_dates as $va_date) {
                        ...
                        if (!$vs_color || ($vs_color == '000000')) {
                            $vs_color = caGetOption(
                                "ca_entities_{$va_entity_type_info[$vn_type_id]['idno']}_color",
                                $pa_bundle_settings,
                                'ffffff'
                            );
                        }
                        ...
                        $va_history[$va_date['sortable']][] = array(
                            'type' => 'ca_entities',
                            'id' => $vn_entity_id,
                            'display' => $qr_entities->getWithTemplate(
                                ($vn_rel_row_id != $row_id) ? $vs_child_display_template : $vs_display_template
                            ),
                            'color' => $vs_color,
                            'icon_url' => $vs_icon_url = $o_media_coder->getMediaTag('icon'),
        ...
        // Collections
        if (is_array($path = Datamodel::getPath($table, 'ca_collections')) && (sizeof(
                    $path
                ) == 3) && ($path = array_keys($path)) && ($linking_table = $path[1])) {
            ...
            if (is_array(
                ...
                $qr_collections = caMakeSearchResult(
                    $linking_table,
                    $va_collections,
                    [
                        'transaction' => $this->getTransaction(),
        ...
        // objects
        if (is_array($path = Datamodel::getPath($table, 'ca_objects')) && (sizeof($path) == 3) && ($path = array_keys(
                $path
            )) && ($linking_table = $path[1])) {
            ...
            if (is_array($va_object_types = caGetOption('ca_objects_showTypes', $pa_bundle_settings, null)) && is_array(
                    $va_objects
                )) {
                $qr_objects = caMakeSearchResult(
                    $linking_table,
                    $va_objects,
                    [
                        'transaction' => $this->getTransaction(),
                ...
                foreach ($va_object_types as $vn_type_id) {
                    if (!is_array(
                            $va_date_elements = caGetOption(
                                "ca_objects_{$va_object_type_info[$vn_type_id]['idno']}_dateElement",
                                $pa_bundle_settings,
                                null
                            )
                        ) && $va_date_elements) {
                ...
                while ($qr_objects->nextHit()) {
                    ...
                    $vs_child_display_template = $pb_display_label_only ? $vs_default_child_display_template : caGetOption(
                        ['ca_objects_childDisplayTemplate', 'ca_objects_childTemplate'],
                        $pa_bundle_settings,
                        $vs_display_template
                    );
                    ...
                    if ($pb_date_mode) {
                        ...
                        if (is_array($va_date_elements_by_type[$vn_type_id]) && sizeof(
                            ...
                            foreach ($va_date_elements_by_type[$vn_type_id] as $vs_date_element) {
                                ...
                                $vs_date_spec = (Datamodel::tableExists(
                                    $va_date_bits[0]
                                )) ? $vs_date_element : "ca_objects.{$vs_date_element}";
                                $va_dates[] = array(
                        ...
                        if (!sizeof($va_dates)) {
                            $va_dates[] = array(
                                'sortable' => $vn_date = caUnixTimestampToHistoricTimestamps(
                                    $qr_objects->get('lastModified.direct')
                                ),
                                'bounds' => array(0, $vn_date),
                                'display' => caGetLocalizedDate($vn_date)
                            );
                    ...
                    foreach ($va_dates as $va_date) {
                        ...
                        if (!$vs_color || ($vs_color == '000000')) {
                            $vs_color = caGetOption(
                                "ca_objects_{$va_object_type_info[$vn_type_id]['idno']}_color",
                                $pa_bundle_settings,
                                'ffffff'
                            );
                        }
                        ...
                        $va_history[$va_date['sortable']][] = array(
                            'type' => 'ca_objects',
                            'id' => $vn_object_id,
                            'display' => $qr_objects->getWithTemplate(
                                ($vn_rel_row_id != $row_id) ? $vs_child_display_template : $vs_display_template
                            ),
                            'color' => $vs_color,
                            'icon_url' => $vs_icon_url = $o_media_coder->getMediaTag('icon'),
        ...
        // Storage locations
        if (is_array($path = Datamodel::getPath($table, 'ca_storage_locations')) && (sizeof(
                    $path
                ) == 3) && ($path = array_keys($path)) && ($linking_table = $path[1])) {
            ...
            if (is_array(
                ...
                $qr_locations = caMakeSearchResult(
                    $linking_table,
                    $va_locations,
                    [
                        'transaction' => $this->getTransaction(),
                ...
                while ($qr_locations->nextHit()) {
                    ...
                    $vs_display_template = $pb_display_label_only ? "" : caGetOption(
                        [
                            ...
                            "ca_storage_locations_" . $qr_locations->get(
                    ...
                    if (sizeof($va_location_types) && sizeof($va_location_types) && !in_array(
                            $vn_rel_type_id = $qr_locations->get("{$linking_table}.type_id"),
                            $va_location_types
                        )) {
                        continue;
        ...
        if (($table === 'ca_objects') && ((caGetOption(
                    'showDeaccessionInformation',
                    $pa_bundle_settings,
                    false
            ...
            if (caGetOption(['deaccession_includeFromChildren'], $pa_bundle_settings, false)) {
                if (is_array(
                        $va_child_object_ids = $qr->get("ca_objects.children.object_id", ['returnAsArray' => true])
    ...
    public function getContents($policy, $options = null)
    {
        ...
        $values = ca_history_tracking_current_values::find(
            ['policy' => $policy, 'current_table_num' => $this->tableNum(), 'current_row_id' => $row_id],
            ['returnAs' => 'arrays', 'transaction' => $this->getTransaction()]
        );
    ...
    public function getHistoryTrackingChronologyHTMLFormBundle(
        ...
        if (!($policy = caGetOption(
            'policy',
            $pa_options,
            caGetOption(
                'policy',
                $pa_bundle_settings,
                $this->getDefaultHistoryTrackingCurrentValuePolicy()
            )
        ))) {
            return null;
        }
        ...
        if (is_array($path = Datamodel::getPath($this->tableName(), 'ca_occurrences')) && ($path = array_keys(
                $path
            )) && (sizeof($path) === 3)) {
            $linking_table = $path[1];
            if (($t_occ_rel = Datamodel::getInstance($linking_table, true)) && ($t_occ = Datamodel::getInstance(
                    'ca_occurrences',
                    true
                ))) {
                $va_occ_types_to_show = array_filter(
                    caGetOption('add_to_occurrence_types', $pa_bundle_settings, array(), ['castTo' => 'array']),
                    function ($v) {
                        return (bool)$v;
                    }
                );
                if (sizeof($va_occ_types_to_show) > 0) {
                    ...
                    foreach ($va_occ_types as $vn_type_id => $va_type_info) {
                        if (!in_array($vn_type_id, $va_occ_types_to_show) && !in_array(
                                $va_type_info['idno'],
                                $va_occ_types_to_show
                            )) {
                    ...
                    $o_view->setVar(
                        'occurrence_relationship_types',
                        $t_occ_rel->getRelationshipTypes(
                            null,
                            null,
                    ...
                    $o_view->setVar(
                        'occurrence_relationship_types_by_sub_type',
                        $t_occ_rel->getRelationshipTypesBySubtype(
                            $this->tableName(),
                            $this->get('type_id'),
                            array_merge($pa_options, $pa_bundle_settings)
                        )
        ...
        if (is_array($path = Datamodel::getPath($this->tableName(), 'ca_collections')) && ($path = array_keys(
                $path
            )) && (sizeof($path) === 3)) {
            $linking_table = $path[1];
            if (($t_coll_rel = Datamodel::getInstance($linking_table, true)) && ($t_coll = Datamodel::getInstance(
                    'ca_collections',
                    true
                ))) {
                $va_coll_types = $t_coll->getTypeList();
                $va_coll_types_to_show = caGetOption(
                    'add_to_collection_types',
                    $pa_bundle_settings,
                    array(),
                    ['castTo' => 'array']
                ...
                foreach ($va_coll_types as $vn_type_id => $va_type_info) {
                    if (!in_array($vn_type_id, $va_coll_types_to_show) && !in_array(
                            $va_type_info['idno'],
                            $va_coll_types_to_show
                        )) {
                ...
                $o_view->setVar(
                    'collection_relationship_types',
                    $t_coll_rel->getRelationshipTypes(
                        null,
                        null,
                        array_merge($pa_options, $pa_bundle_settings)
                    )
                ...
                $o_view->setVar(
                    'collection_relationship_types_by_sub_type',
                    $t_coll_rel->getRelationshipTypesBySubtype(
                        $this->tableName(),
                        $this->get('type_id'),
                        array_merge($pa_options, $pa_bundle_settings)
                    )
        ...
        if (is_array($path = Datamodel::getPath($this->tableName(), 'ca_entities')) && ($path = array_keys(
                $path
            )) && (sizeof($path) === 3)) {
            $linking_table = $path[1];
            if (($t_entity_rel = Datamodel::getInstance($linking_table, true)) && ($t_entity = Datamodel::getInstance(
                    'ca_entities',
                    true
                ))) {
                $va_entity_types = $t_entity->getTypeList();
                $va_entity_types_to_show = caGetOption(
                    'add_to_entity_types',
                    $pa_bundle_settings,
                    array(),
                    ['castTo' => 'array']
                ...
                foreach ($va_entity_types as $vn_type_id => $va_type_info) {
                    if (!in_array($vn_type_id, $va_entity_types_to_show) && !in_array(
                            $va_type_info['idno'],
                            $va_entity_types_to_show
                        )) {
                ...
                $o_view->setVar(
                    ...
                    $t_entity_rel->getRelationshipTypes(
                        null,
                        null,
                        array_merge($pa_options, $pa_bundle_settings)
                    )
                ...
                $o_view->setVar(
                    'entity_relationship_types_by_sub_type',
                    $t_entity_rel->getRelationshipTypesBySubtype(
                        $this->tableName(),
                        $this->get('type_id'),
                        array_merge($pa_options, $pa_bundle_settings)
                    )
        ...
        if (is_array($path = Datamodel::getPath($this->tableName(), 'ca_loans')) && ($path = array_keys(
                $path
            )) && (sizeof($path) === 3)) {
            $linking_table = $path[1];
            if ($t_loan_rel = Datamodel::getInstance($linking_table, true)) {
                $o_view->setVar(
                    'loan_relationship_types',
                    $t_loan_rel->getRelationshipTypes(
                        null,
                        null,
                        array_merge($pa_options, $pa_bundle_settings)
                    )
                ...
                $o_view->setVar(
                    'loan_relationship_types_by_sub_type',
                    $t_loan_rel->getRelationshipTypesBySubtype(
                        $this->tableName(),
                        $this->get('type_id'),
                        array_merge($pa_options, $pa_bundle_settings)
                    )
        ...
        if (is_array($path = Datamodel::getPath($this->tableName(), 'ca_movements')) && ($path = array_keys(
                $path
            )) && (sizeof($path) === 3)) {
            $linking_table = $path[1];
            if ($t_movement_rel = Datamodel::getInstance($linking_table, true)) {
                $o_view->setVar(
                    'movement_relationship_types',
                    $t_movement_rel->getRelationshipTypes(
                        null,
                        null,
                        array_merge($pa_options, $pa_bundle_settings)
                    )
                ...
                $o_view->setVar(
                    'movement_relationship_types_by_sub_type',
                    $t_movement_rel->getRelationshipTypesBySubtype(
                        $this->tableName(),
                        $this->get('type_id'),
                        array_merge($pa_options, $pa_bundle_settings)
                    )
        ...
        if (is_array($path = Datamodel::getPath($this->tableName(), 'ca_objects')) && ($path = array_keys(
                $path
            )) && (sizeof($path) === 3)) {
            $linking_table = $path[1];
            if ($t_object_rel = Datamodel::getInstance($linking_table, true)) {
                $o_view->setVar(
                    ...
                    $t_object_rel->getRelationshipTypes(
                        null,
                        null,
                        array_merge($pa_options, $pa_bundle_settings)
                    )
                ...
                $o_view->setVar(
                    'object_relationship_types_by_sub_type',
                    $t_object_rel->getRelationshipTypesBySubtype(
                        $this->tableName(),
                        $this->get('type_id'),
                        array_merge($pa_options, $pa_bundle_settings)
                    )
        ...
        if (is_array($path = Datamodel::getPath($this->tableName(), 'ca_storage_locations')) && ($path = array_keys(
                $path
            )) && (sizeof($path) === 3)) {
            $linking_table = $path[1];
            if ($t_location_rel = Datamodel::getInstance($linking_table, true)) {
                $o_view->setVar(
                    ...
                    $t_location_rel->getRelationshipTypes(
                        null,
                        null,
                        array_merge($pa_options, $pa_bundle_settings)
                    )
                ...
                $o_view->setVar(
                    'location_relationship_types_by_sub_type',
                    $t_location_rel->getRelationshipTypesBySubtype(
                        $this->tableName(),
                        $this->get('type_id'),
                        array_merge($pa_options, $pa_bundle_settings)
                    )
    ...
    public static function getHistoryTrackingChronologyInterstitialElementAddHTMLForm(
        ...
        if ((is_array(
                    $interstitial_elements = $settings["{$rel_table}_" . ($type_idno ? "{$type_idno}_" : "") . "setInterstitialElementsOnAdd"]
                ) || is_array($interstitial_elements = $settings["setInterstitialElementsOnAdd"])) && sizeof(
            ...
            $t_ui = ca_editor_uis::find(
                ['editor_type' => Datamodel::getTableNum($linking_table)],
                [
            ...
            foreach ($interstitial_elements as $element_code) {
                ...
                if ($t_rel->hasField($element_code)) {
                    ...
                    $buf .= "<td><div class='formLabel'>{$label}<br/>" . $t_rel->htmlFormElement(
                            $element_code,
                            '',
                            [
                                'name' => "{$id_prefix}_{$rel_table}_{$type_idno}_{$element_code}" . ($no_template ? '' : '{n}'),
                                'id' => "{$id_prefix}_{$rel_table}_{$type_idno}_{$element_code}" . ($no_template ? '' : '{n}'),
                                'value' => _t(
                                    'today'
                                ),
                    ...
                    $buf .= "<td class='formLabel'>{$label}<br/>" . $t_rel->getAttributeHTMLFormBundle(
                            $request,
                            null,
                            $element_code,
                            $placement_code,
    ...
    public static function setHistoryTrackingChronologyInterstitialElementsFromHTMLForm(
        ...
        if ($t_item_rel) {
            ...
            if (is_array(
                $interstitial_elements = $no_template ? $settings : caGetOption(
                    ["{$rel_table}_{$type}_setInterstitialElementsOnAdd", "{$rel_table}_setInterstitialElementsOnAdd"],
                    $settings,
                    array()
                )
            )) {
                foreach ($interstitial_elements as $element_code) {
                    if ($t_item_rel->hasField($element_code)) {
                        ...
                        foreach ($sub_element_ids as $sub_element_id) {
                            $vals[ca_metadata_elements::getElementCodeForID(
                                $sub_element_id
        ...
        switch ($ps_bundle_name) {
            ...
            case 'history_tracking_current_date':
                if (method_exists($this, "getHistory")) {
                    ...
                    if (is_array(
                            $h = $this->getHistory(
                                array('limit' => 1, 'currentOnly' => true, 'policy' => $policy, 'row_id' => $pn_row_id)
                            )
                        ) && (sizeof($h) > 0)) {
                        ...
                        if (is_array($va_path_components) && $va_path_components['subfield_name']) {
                            if (($t_loc = Datamodel::getInstanceByTableName(
                                    $va_current_location['type'],
                                    true
                                )) && $t_loc->load($va_current_location['id'])) {
                                return $t_loc->get(
                                    $va_current_location['type'] . '.' . $va_path_components['subfield_name']
                                );
                            }
                        }
                        return (in_array(
    ...
    public static function getHistoryTrackingEditorBundleSettingsData($table, $options = null)
    {
        ...
        foreach ($types as $vn_type_id => $type) {
            ...
            $additional_settings["ca_object_lots_{$type['idno']}_displayTemplate"] = array(
                ...
                'description' => _t(
                    'Layout for lot when displayed in history list (can include HTML). The template is evaluated relative to the lot. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_object_lots.idno_stub</i>.'
                )
            );
            $additional_settings["ca_object_lots_{$type['idno']}_includeFromChildren"] = array(
                ...
                'description' => _t(
                    'If checked history from lots that are related to sub-objects (children) is included.'
                )
            );
            $additional_settings["ca_object_lots_{$type['idno']}_childDisplayTemplate"] = array(
                ...
                'description' => _t(
                    'Layout for lot related to child objects, when displayed in history list (can include HTML). The template is evaluated relative to the lot. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_object_lots.idno_stub</i>.'
                )
            );
        ...
        foreach ($types as $vn_type_id => $type) {
            ...
            $additional_settings["ca_occurrences_{$type['idno']}_displayTemplate"] = array(
                ...
                'description' => _t(
                    'Layout for %1 when displayed in history list (can include HTML). The template is evaluated relative to the %1. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_occurrences.idno</i>.',
                    $type['name_singular']
                )
            );
            $additional_settings["ca_occurrences_{$type['idno']}_includeFromChildren"] = array(
                ...
                'description' => _t(
                    'If checked history from %1 that are related to sub-objects (children) is included.',
                    $type['name_plural']
                )
            );
            $additional_settings["ca_occurrences_{$type['idno']}_childDisplayTemplate"] = array(
                ...
                'description' => _t(
                    'Layout for %1 related to child objects, when displayed in history list (can include HTML). The template is evaluated relative to the lot. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_object_lots.idno_stub</i>.',
                    $type['name_plural']
                )
            );
        ...
        foreach ($types as $vn_type_id => $type) {
            ...
            $additional_settings["ca_collections_{$type['idno']}_displayTemplate"] = array(
                ...
                'label' => _t('%1 display template', $type['name_singular']),
                'description' => _t(
                    'Layout for %1 when displayed in history list (can include HTML). The template is evaluated relative to the %1. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_collections.idno</i>.',
                    $type['name_singular']
                )
            );
        ...
        foreach ($types as $vn_type_id => $type) {
            ...
            $additional_settings["ca_entities_{$type['idno']}_displayTemplate"] = array(
                ...
                'label' => _t('%1 display template', $type['name_singular']),
                'description' => _t(
                    'Layout for %1 when displayed in history list (can include HTML). The template is evaluated relative to the %1. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_entities.idno</i>.',
                    $type['name_singular']
                )
            );
        ...
        foreach ($types as $vn_type_id => $type) {
            ...
            $additional_settings["ca_movements_{$type['idno']}_displayTemplate"] = array(
                ...
                'label' => _t('%1 display template', $type['name_singular']),
                'description' => _t(
                    'Layout for %1 when displayed in history list (can include HTML). The template is evaluated relative to the %1. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_movements.idno</i>.',
                    $type['name_singular']
                )
            );
        ...
        foreach ($types as $vn_type_id => $type) {
            ...
            $additional_settings["ca_loans_{$type['idno']}_displayTemplate"] = array(
                ...
                'label' => _t('%1 display template', $type['name_singular']),
                'description' => _t(
                    'Layout for %1 when displayed in history list (can include HTML). The template is evaluated relative to the %1. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_loans.idno</i>.',
                    $type['name_singular']
                )
            );
        ...
        foreach ($types as $vn_type_id => $type) {
            ...
            $additional_settings["ca_objects_{$type['idno']}_displayTemplate"] = array(
                ...
                'label' => _t('%1 display template', $type['name_singular']),
                'description' => _t(
                    'Layout for %1 when displayed in history list (can include HTML). The template is evaluated relative to the %1. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_objects.idno</i>.',
                    $type['name_singular']
                )
            );
        ...
        $additional_settings += array(
            ...
            'ca_storage_locations_displayTemplate' => array(
                ...
                'description' => _t(
                    'Layout for storage location when displayed in history list (can include HTML). The template is evaluated relative to the object-storage location relationship. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_object_lots.idno_stub</i>.'
                )
            ),
            'ca_storage_locations_includeFromChildren' => array(
                ...
                'description' => _t(
                    'If checked history from storage locations that are related to sub-objects (children) is included.'
                )
            ),
            ...
            'deaccession_displayTemplate' => array(
                ...
                'description' => _t(
                    'Layout for deaccession information when displayed in history list (can include HTML). The template is evaluated relative to the object. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_objects.deaccession_notes</i>.'
                )
            ),
            'deaccession_includeFromChildren' => array(
                ...
                'description' => _t(
                    'If checked history from deaccession information that are related to sub-objects (children) is included.'
                )
            ),
            'deaccession_childDisplayTemplate' => array(
                ...
                'description' => _t(
                    'Layout for deaccession information related to child objects when displayed in history list (can include HTML). The template is evaluated relative to the object. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_objects.deaccession_notes</i>.'
                )
            )
        );
    ...
    public static function getDependentCurrentValues($table_num, $row_id, $options = null)
    {
        ...
        $current = ca_history_tracking_current_values::find(
            ['current_table_num' => $table_num, 'current_row_id' => $row_id],
            $opts
        );
        $tracked = ca_history_tracking_current_values::find(
            ['tracked_table_num' => $table_num, 'tracked_row_id' => $row_id],
            $opts
        );

        $rows = array_reduce(
            array_merge($current, $tracked),
            function ($c, $i) {
                if (!$i['is_future']) {
