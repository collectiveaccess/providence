...
class getid3_ac3 extends getid3_handler
{
    ...
    public function Analyze()
    {
            ...
            if (!$this->isDependencyFor('matroska')) {
                ...
                return $this->error(
                    'Expecting "' . getid3_lib::PrintHexBytes(
                        ...
                        substr($this->AC3header['syncinfo'], 0, 2)
                    ) . '"'
                );
            }
            $offset = 0;
        ...
        $thisfile_ac3_raw['synchinfo']['crc1'] = getid3_lib::LittleEndian2Int(
            substr($this->AC3header['syncinfo'], $offset, 2)
        );
        $ac3_synchinfo_fscod_frmsizecod = getid3_lib::LittleEndian2Int(
            substr($this->AC3header['syncinfo'], ($offset + 2), 1)
        );
        ...
        if ($thisfile_ac3_raw_bsi['bsid'] > 8) {
            ...
            $this->error(
                'Bit stream identification is version ' . $thisfile_ac3_raw_bsi['bsid'] . ', but getID3() only understands up to version 8'
            );
            unset($info['ac3']);
            return false;
        }
