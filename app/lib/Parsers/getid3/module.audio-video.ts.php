...
class getid3_ts extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if (substr($TSheader, 0, 1) != $magic) {
            $info['error'][] = 'Expecting "' . getid3_lib::PrintHexBytes(
                    $magic
        ...
        $info['ts']['packet']['scrambling_control'] = $this->TSscramblingControlLookup(
            $info['ts']['packet']['raw']['scrambling_control']
        );
        ...
        if ($info['ts']['packet']['flags']['adaption_field_exists']) {
            ...
            if ($info['ts']['packet']['adaption']['flags']['pcr']) {
                $info['ts']['packet']['adaption']['raw']['pcr'] = getid3_lib::BigEndian2Int(
                    substr($TSheader, $offset, 6)
                );
                $offset += 6;
            }
            if ($info['ts']['packet']['adaption']['flags']['opcr']) {
                $info['ts']['packet']['adaption']['raw']['opcr'] = getid3_lib::BigEndian2Int(
                    substr($TSheader, $offset, 6)
                );
                $offset += 6;
            }
