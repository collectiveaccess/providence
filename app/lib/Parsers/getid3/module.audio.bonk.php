...
class getid3_bonk extends getid3_handler
{
    public function Analyze()
    {
        ...
        if (!getid3_lib::intValueSupported($thisfile_bonk['dataend'])) {
            $info['warning'][] = 'Unable to parse BONK file from end (v0.6+ preferred method) because PHP filesystem functions only support up to ' . round(
            ...
            while ($this->BonkIsValidTagName(substr($PossibleBonkTag, 4, 4), true)) {
                ...
                if (($TagHeaderTest{0} != "\x00") || (substr($PossibleBonkTag, 4, 4) != strtolower(
                    ...
                    $info['error'][] = 'Expecting "' . getid3_lib::PrintHexBytes(
                        ...
                        ) . '" at offset ' . $BonkTagOffset . ', found "' . getid3_lib::PrintHexBytes(
                            $TagHeaderTest
                        ) . '"';
                    return false;
                }
