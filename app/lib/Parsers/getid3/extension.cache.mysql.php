...
class getID3_cached_mysql extends getID3
{
    ...
    public function getID3_cached_mysql($host, $database, $username, $password, $table = 'getid3_cache')
    {
        ...
        $version = '';
        if ($this->cursor = mysql_query(
            "SELECT `value` FROM `" . mysql_real_escape_string(
                $this->table
            ) . "` WHERE (`filename` = '" . mysql_real_escape_string(
                getID3::VERSION
            ) . "') AND (`filesize` = '-1') AND (`filetime` = '-1') AND (`analyzetime` = '-1')",
            $this->connection
        )) {
        ...
        if (file_exists($filename)) {
            ...
            $this->cursor = mysql_query(
                "SELECT `value` FROM `" . mysql_real_escape_string(
                    ...
                    $filename
                ) . "') AND (`filesize` = '" . mysql_real_escape_string(
                    $filesize
        ...
        if (file_exists($filename)) {
            $this->cursor = mysql_query(
                "INSERT INTO `" . mysql_real_escape_string(
                    ...
                    $filename
                ) . "', '" . mysql_real_escape_string($filesize) . "', '" . mysql_real_escape_string(
                    $filetime
                ) . "', '" . mysql_real_escape_string(time()) . "', '" . mysql_real_escape_string(
                    base64_encode(serialize($analysis))
                ) . "')",
                $this->connection
            );
        }
