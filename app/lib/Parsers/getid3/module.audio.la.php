    ...
    public function Analyze()
    {
        ...
        switch (substr($rawdata, $offset, 4)) {
            ...
            case 'LA04':
                ...
                if ($WAVEchunk !== 'WAVE') {
                    $info['error'][] = 'Expected "WAVE" (' . getid3_lib::PrintHexBytes(
                            'WAVE'
                ...
                if ($fmt_chunk !== 'fmt ') {
                    $info['error'][] = 'Expected "fmt " (' . getid3_lib::PrintHexBytes(
                            'fmt '
                ...
                if ($info['la']['flags']['seekable']) {
                    $info['la']['seekpoint_count'] = floor(
                        $info['la']['samples'] / ($info['la']['blocksize'] * $info['la']['seekevery'])
                    );
                ...
                if ($info['la']['footerstart'] < $info['avdataend']) {
                    if ($RIFFtempfilename = tempnam(GETID3_TEMP_DIR, 'id3')) {
                        if ($RIFF_fp = fopen($RIFFtempfilename, 'w+b')) {
                            ...
                            $RIFFdata = 'RIFF' . getid3_lib::LittleEndian2String(
                                    strlen($RIFFdata),
                                    4,
                                    false
                                ) . $RIFFdata;
                            ...
                            if (empty($getid3_temp->info['error'])) {
                                ...
                                $info['warning'][] = 'Error parsing RIFF portion of La file: ' . implode(
            ...
            default:
                if (substr($rawdata, $offset, 2) == 'LA') {
                    $info['error'][] = 'This version of getID3() [' . $this->getid3->version(
                        ) . '] does not support LA version ' . substr($rawdata, $offset + 2, 1) . '.' . substr(
                            $rawdata,
                            $offset + 3,
                            1
