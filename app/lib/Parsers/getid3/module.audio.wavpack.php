        ...
        while (true) {
            ...
            if ($blockheader_magic != $magic) {
                $info['error'][] = 'Expecting "' . getid3_lib::PrintHexBytes(
                        $magic
                    ) . '" at offset ' . $blockheader_offset . ', found "' . getid3_lib::PrintHexBytes(
                        $blockheader_magic
                    ) . '"';
            ...
            if (empty($info['wavpack']['blockheader']['block_samples']) ||
                ...
                $info['wavpack']['blockheader']['total_samples'] = getid3_lib::LittleEndian2Int(
                    substr($wavpackheader, 12, 4)
                );
                $info['wavpack']['blockheader']['block_index'] = getid3_lib::LittleEndian2Int(
                    substr($wavpackheader, 16, 4)
                );
                $info['wavpack']['blockheader']['block_samples'] = getid3_lib::LittleEndian2Int(
                    substr($wavpackheader, 20, 4)
                );
                $info['wavpack']['blockheader']['flags_raw'] = getid3_lib::LittleEndian2Int(
                    substr($wavpackheader, 24, 4)
                );
            ...
            while (!feof($this->getid3->fp) && (ftell(
                        $this->getid3->fp
                    ) < ($blockheader_offset + $blockheader_size + 8))) {
                ...
                $metablock['size'] = getid3_lib::LittleEndian2Int(
                        substr($metablockheader, 1)
                    ) * 2; // size is stored in words
                $metablock['data'] = null;
                ...
                if ($metablock['size'] > 0) {
                    switch ($metablock['function_id']) {
                        ...
                        default:
                            $info['warning'][] = 'Unexpected metablock type "0x' . str_pad(
                    ...
                    switch ($metablock['function_id']) {
                        ...
                        case 0x22: // ID_RIFF_TRAILER
                            ...
                            $getid3_riff = new getid3_riff($getid3_temp);
                            $metablock['riff'] = $getid3_riff->ParseRIFF(
                                $startoffset,
                        ...
                        case 0x26: // ID_MD5_CHECKSUM
                            if (strlen($metablock['data']) == 16) {
                                $info['md5_data_source'] = strtolower(
                                    getid3_lib::PrintHexBytes($metablock['data'], true, false, false)
                                );
                            } else {
                                $info['warning'][] = 'Expecting 16 bytes of WavPack "MD5 Checksum" in metablock at offset ' . $metablock['offset'] . ', but found ' . strlen(
                                        $metablock['data']
                                    ) . ' bytes';
                            }
                            break;
