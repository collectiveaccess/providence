        ...
        for ($i = 0; $i < $thisfile_midi_raw['tracks']; $i++) {
            while ((strlen($MIDIdata) - $offset) < 8) {
                if ($buffer = fread($this->getid3->fp, $this->getid3->fread_buffer_size())) {
                    ...
                    $info['error'][] = 'Unabled to read more file data at ' . ftell(
                            $this->getid3->fp
            ...
            foreach ($trackdataarray as $tracknumber => $trackdata) {
                ...
                while ($eventsoffset < strlen($trackdata)) {
                    ...
                    if ($MIDIevents[$tracknumber][$eventid]['eventid'] == 0x08) { // Note off (key is released)
                        ...
                        if ($tracknumber == 10) {
                            $thisfile_midi_raw['track'][$tracknumber]['instrument'] = $this->GeneralMIDIpercussionLookup(
                                $newprogramnum
                            );
                        } else {
                            $thisfile_midi_raw['track'][$tracknumber]['instrument'] = $this->GeneralMIDIinstrumentLookup(
                                $newprogramnum
                            );
                        }
                        ...
                        switch ($METAeventCommand) {
                            case 0x00: // Set track sequence number
                                $track_sequence_number = getid3_lib::BigEndian2Int(
                                    substr($METAeventData, 0, $METAeventLength)
                                );
                            ...
                            case 0x51: // Tempo: microseconds / quarter note
                                $CurrentMicroSecondsPerBeat = getid3_lib::BigEndian2Int(
                                    substr($METAeventData, 0, $METAeventLength)
                                );
                            ...
                            case 0x58: // Time signature
                                ...
                                $timesig_denominator = pow(
                                    2,
                                    getid3_lib::BigEndian2Int($METAeventData{1})
                                ); // $02 -> x/4, $03 -> x/8, etc
                            ...
                            case 0x59: // Keysignature
                                ...
                                $keysig_majorminor = getid3_lib::BigEndian2Int(
                                    $METAeventData{1}
                                ); // 0 -> major, 1 -> minor
                                $keysigs = array(
                                    -7 => 'Cb',
