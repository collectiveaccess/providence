...
class getid3_mpeg extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if ($foundVideo) {
            ...
            $info['mpeg']['video']['pixel_aspect_ratio'] = $this->MPEGvideoAspectRatioLookup(
                $info['mpeg']['video']['raw']['pixel_aspect_ratio']
            );
            $info['mpeg']['video']['pixel_aspect_ratio_text'] = $this->MPEGvideoAspectRatioTextLookup(
                $info['mpeg']['video']['raw']['pixel_aspect_ratio']
            );
            $info['mpeg']['video']['frame_rate'] = $this->MPEGvideoFramerateLookup(
                $info['mpeg']['video']['raw']['frame_rate']
            );

            $info['mpeg']['video']['raw']['bitrate'] = getid3_lib::Bin2Dec(substr($assortedinformation, 0, 18));
            $info['mpeg']['video']['raw']['marker_bit'] = (bool)getid3_lib::Bin2Dec(
                substr($assortedinformation, 18, 1)
            );
            $info['mpeg']['video']['raw']['vbv_buffer_size'] = getid3_lib::Bin2Dec(
                substr($assortedinformation, 19, 10)
            );
            $info['mpeg']['video']['raw']['constrained_param_flag'] = (bool)getid3_lib::Bin2Dec(
                substr($assortedinformation, 29, 1)
            );
            $info['mpeg']['video']['raw']['intra_quant_flag'] = (bool)getid3_lib::Bin2Dec(
                substr($assortedinformation, 30, 1)
            );
            if ($info['mpeg']['video']['raw']['intra_quant_flag']) {
                // read 512 bits
                $info['mpeg']['video']['raw']['intra_quant'] = getid3_lib::BigEndian2Bin(
                    substr($MPEGstreamData, $VideoChunkOffset, 64)
                );
                $VideoChunkOffset += 64;

                $info['mpeg']['video']['raw']['non_intra_quant_flag'] = (bool)getid3_lib::Bin2Dec(
                    substr($info['mpeg']['video']['raw']['intra_quant'], 511, 1)
                );
                $info['mpeg']['video']['raw']['intra_quant'] = getid3_lib::Bin2Dec(
                        substr($assortedinformation, 31, 1)
                    ) . substr(getid3_lib::BigEndian2Bin(substr($MPEGstreamData, $VideoChunkOffset, 64)), 0, 511);
            ...
            if ($info['mpeg']['video']['raw']['bitrate'] == 0x3FFFF) { // 18 set bits
                ...
                $info['warning'][] = 'This version of getID3() [' . $this->getid3->version(
        ...
        if (!empty($info['video']['bitrate']) && !empty($info['audio']['bitrate'])) {
            ...
            $info['playtime_seconds'] *= $this->MPEGsystemNonOverheadPercentage(
                $info['video']['bitrate'],
                $info['audio']['bitrate']
            );
    ...
    {
        $MPEGvideoAspectRatioLookup = array(
            0,
            1,
            0.6735,
            0.7031,
    ...
    {
        $MPEGvideoAspectRatioTextLookup = array(
            'forbidden',
            'square pixels',
            '0.6735',
            '16:9, 625 line, PAL',
