...
class getid3_mpc extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if (preg_match('#^MPCK#', $info['mpc']['header']['preamble'])) {
            ...
            $info['error'][] = 'Expecting "MP+" or "MPCK" at offset ' . $info['avdataoffset'] . ', found "' . getid3_lib::PrintHexBytes(
        ...
        while ($offset < $info['avdataend']) {
            ...
            switch ($thisPacket['key']) {
                case 'SH': // Stream Header
                    ...
                    $packet_offset += 4;
                    $thisPacket['stream_version'] = getid3_lib::BigEndian2Int(
                        substr($MPCheaderData, $packet_offset, 1)
                    );
                    $packet_offset += 1;
                ...
                case 'RG': // Replay Gain
                    ...
                    $thisPacket['replaygain_version'] = getid3_lib::BigEndian2Int(
                        substr($MPCheaderData, $packet_offset, 1)
                    );
                    $packet_offset += 1;
                    $thisPacket['replaygain_title_gain'] = getid3_lib::BigEndian2Int(
                        substr($MPCheaderData, $packet_offset, 2)
                    );
                    $packet_offset += 2;
                    $thisPacket['replaygain_title_peak'] = getid3_lib::BigEndian2Int(
                        substr($MPCheaderData, $packet_offset, 2)
                    );
                    $packet_offset += 2;
                    $thisPacket['replaygain_album_gain'] = getid3_lib::BigEndian2Int(
                        substr($MPCheaderData, $packet_offset, 2)
                    );
                    $packet_offset += 2;
                    $thisPacket['replaygain_album_peak'] = getid3_lib::BigEndian2Int(
                        substr($MPCheaderData, $packet_offset, 2)
                    );
                    $packet_offset += 2;
    ...
    public function ParseMPCsv7()
    {
        ...
        $MPCheaderData .= fread(
            $this->getid3->fp,
            $thisfile_mpc_header['size'] - strlen($info['mpc']['header']['preamble'])
        );
        $offset = strlen('MP+');
        ...
        $offset += 2;
        $thisfile_mpc_header['raw']['title_gain'] = getid3_lib::LittleEndian2Int(
            substr($MPCheaderData, $offset, 2),
            true
        );
        $offset += 2;
        ...
        $offset += 2;
        $thisfile_mpc_header['raw']['album_gain'] = getid3_lib::LittleEndian2Int(
            substr($MPCheaderData, $offset, 2),
            true
        );
        $offset += 2;
        ...
        $offset += 3;
        $thisfile_mpc_header['raw']['encoder_version'] = getid3_lib::LittleEndian2Int(
            substr($MPCheaderData, $offset, 1)
        );
        $offset += 1;
        ...
        $thisfile_mpc_header['encoder_version'] = $this->MPCencoderVersionLookup(
