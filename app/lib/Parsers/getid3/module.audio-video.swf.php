...
class getid3_swf extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        switch ($info['swf']['header']['signature']) {
            ...
            default:
                $info['error'][] = 'Expecting "FWS" or "CWS" at offset ' . $info['avdataoffset'] . ', found "' . getid3_lib::PrintHexBytes(
        ...
        if ($info['swf']['header']['compressed']) {
            ...
            if ($decompressed = @gzuncompress($SWFfileData)) {
                ...
                $info['error'][] = 'Error decompressing compressed SWF data (' . strlen(
                        $SWFfileData
        ...
        $info['swf']['header']['frame_rate'] = getid3_lib::LittleEndian2Int(
            substr($SWFfileData, 9 + $FrameSizeDataLength, 1)
        );
        $info['swf']['header']['frame_count'] = getid3_lib::LittleEndian2Int(
            substr($SWFfileData, 10 + $FrameSizeDataLength, 2)
        );
...
//echo __LINE__.'='.number_format(microtime(true) - $start_time, 3).'<br>';
            ...
            switch ($TagID) {
                ...
                case 9: // Set background color
                    ...
                    $info['swf']['bgcolor'] = strtoupper(
                        str_pad(dechex(getid3_lib::BigEndian2Int($TagData['data'])), 6, '0', STR_PAD_LEFT)
                    );
                    break;
