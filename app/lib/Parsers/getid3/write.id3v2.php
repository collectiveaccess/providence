...
class getid3_write_id3v2
{
    ...
    public function WriteID3v2()
    {
        ...
        if (!empty($this->filename) && (is_writeable($this->filename) || (!file_exists($this->filename) && is_writeable(
            ...
            if (!getid3_lib::intValueSupported($OldThisFileInfo['filesize'])) {
                $this->errors[] = 'Unable to write ID3v2 because file is larger than ' . round(
            ...
            if ($NewID3v2Tag = $this->GenerateID3v2Tag()) {
                if (file_exists($this->filename) && is_writeable(
                        $this->filename
                    ...
                    if (file_exists($this->filename)) {
                        ...
                        if (is_writable($this->filename) && is_file($this->filename) && ($fp = fopen(
                                $this->filename,
                                'wb'
                            ))) {
                            rewind($fp);
                    ...
                    if ($tempfilename = tempnam(GETID3_TEMP_DIR, 'getID3')) {
                        if (is_readable($this->filename) && is_file($this->filename) && ($fp_source = fopen(
                                $this->filename,
                                'rb'
                            ))) {
                            if (is_writable($tempfilename) && is_file($tempfilename) && ($fp_temp = fopen(
                                    $tempfilename,
                                    'wb'
                                ))) {
    ...
    public function RemoveID3v2()
    {
        ...
        if (is_writeable(dirname($this->filename))) {
            ...
            if (is_readable($this->filename) && is_file($this->filename) && ($fp_source = fopen(
                ...
                $OldThisFileInfo = $getID3->analyze($this->filename);
                if (!getid3_lib::intValueSupported($OldThisFileInfo['filesize'])) {
                    $this->errors[] = 'Unable to remove ID3v2 because file is larger than ' . round(
                            PHP_INT_MAX / 1073741824
                        ) . 'GB';
                    fclose($fp_source);
                    return false;
                ...
                if (is_writable($this->filename) && is_file($this->filename) && ($fp_temp = fopen(
                        $this->filename . 'getid3tmp',
                        'w+b'
                    ))) {
                    while ($buffer = fread($fp_source, $this->fread_buffer_size)) {
                        fwrite($fp_temp, $buffer, strlen($buffer));
            ...
            if (is_readable($this->filename) && is_file($this->filename) && ($fp_source = fopen(
                ...
                if ($fp_temp = tmpfile()) {
                    ...
                    fclose($fp_source);
                    if (is_writable($this->filename) && is_file($this->filename) && ($fp_source = fopen(
                            $this->filename,
                            'wb'
                        ))) {
                        rewind($fp_temp);
            ...
            switch ($frame_name) {
                case 'UFID':
                    ...
                    if (strlen($source_data_array['data']) > 64) {
                        $this->errors[] = 'Identifier not allowed to be longer than 64 bytes in ' . $frame_name . ' (supplied data was ' . strlen(
    ...
    {
        if ((strlen($mimestring) >= 3) && (strpos($mimestring, '/') > 0) && (strpos($mimestring, '/') < (strlen(
                        $mimestring
                    ) - 1))) {
            return true;
        }
        ...
        if ($parts = $this->safe_parse_url($url)) {
            if (($parts['scheme'] != 'http') && ($parts['scheme'] != 'https') && ($parts['scheme'] != 'ftp') && ($parts['scheme'] != 'gopher')) {
                return false;
                    ...
                    '#^[[:alnum:]]([-.]?[0-9a-z])*\\.[a-z]{2,3}$#i',
                    $parts['host'],
                    $regs
                ) && !preg_match('#^[0-9]{1,3}(\\.[0-9]{1,3}){3}$#', $parts['host'])) {
                return false;
                ...
                return false;
            } elseif (!empty($parts['query']) && !preg_match(
                    '#^[[:alnum:]?&=+:;_()%\\#/,\\.-]*$#i',
                    $parts['query'],
                    $regs
                )) {
