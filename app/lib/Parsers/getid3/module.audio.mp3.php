...
class getid3_mp3 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if (!empty($CurrentDataLAMEversionString) && (substr(
            ...
            if (($PossiblyLongerLAMEversion_String = strstr(
    ...
    public function GuessEncoderOptions()
    {
        ...
        } elseif (!empty($thisfile_mpeg_audio_lame['preset_used']) && (!in_array(
    ...
    public function decodeMPEGaudioHeader(
        ...
        if ($MPEGaudioHeaderValidCache[$head4]) {
            ...
            $info['error'][] = 'Invalid MPEG audio header (' . getid3_lib::PrintHexBytes(
                    $head4
                ) . ') at offset ' . $offset;
            return false;
        }
        ...
        if (substr($headerstring, 4 + 32, 4) == 'VBRI') {
            ...
            $thisfile_mpeg_audio['VBR_encoder_version'] = getid3_lib::BigEndian2Int(
                substr($headerstring, $FraunhoferVBROffset + 4, 2)
            ); // VbriVersion
            $thisfile_mpeg_audio['VBR_encoder_delay'] = getid3_lib::BigEndian2Int(
            ...
            $thisfile_mpeg_audio['VBR_seek_offsets'] = getid3_lib::BigEndian2Int(
            ...
            $thisfile_mpeg_audio['VBR_entry_frames'] = getid3_lib::BigEndian2Int(
...
//				}
                ...
                $thisfile_mpeg_audio['xing_flags_raw'] = getid3_lib::BigEndian2Int(
                ...
                if ($thisfile_mpeg_audio['xing_flags']['vbr_scale']) {
                    $thisfile_mpeg_audio['VBR_scale'] = getid3_lib::BigEndian2Int(
                ...
                if (substr($headerstring, $VBRidOffset + 120, 4) == 'LAME') {
                    ...
                    if ($thisfile_mpeg_audio_lame['short_version'] >= 'LAME3.90') {
                        ...
                        $thisfile_mpeg_audio_lame['vbr_quality'] = getid3_lib::BigEndian2Int(
                            substr($headerstring, $LAMEtagOffsetContant + 0x9B, 1)
                        );
                        ...
                        $LAMEtagRevisionVBRmethod = getid3_lib::BigEndian2Int(
                            substr($headerstring, $LAMEtagOffsetContant + 0xA5, 1)
                        );
                        ...
                        $thisfile_mpeg_audio_lame['vbr_method'] = self::LAMEvbrMethodLookup(
                        ...
                        $thisfile_mpeg_audio_lame['lowpass_frequency'] = getid3_lib::BigEndian2Int(
                                substr($headerstring, $LAMEtagOffsetContant + 0xA6, 1)
                            ) * 100;
                        ...
                        if ($thisfile_mpeg_audio_lame['short_version'] >= 'LAME3.94b') {
                            ...
                            $thisfile_mpeg_audio_lame_RGAD['peak_amplitude'] = getid3_lib::LittleEndian2Float(
                                substr($headerstring, $LAMEtagOffsetContant + 0xA7, 4)
                            );
                        }
                        if ($thisfile_mpeg_audio_lame_RGAD['peak_amplitude'] == 0) {
                            ...
                            $thisfile_mpeg_audio_lame_RGAD['peak_db'] = getid3_lib::RGADamplitude2dB(
                                $thisfile_mpeg_audio_lame_RGAD['peak_amplitude']
                            );
                        }

                        $thisfile_mpeg_audio_lame_raw['RGAD_track'] = getid3_lib::BigEndian2Int(
                            substr($headerstring, $LAMEtagOffsetContant + 0xAB, 2)
                        );
                        $thisfile_mpeg_audio_lame_raw['RGAD_album'] = getid3_lib::BigEndian2Int(
                            substr($headerstring, $LAMEtagOffsetContant + 0xAD, 2)
                        );
                        ...
                        if ($thisfile_mpeg_audio_lame_raw['RGAD_track'] != 0) {
                            ...
                            $thisfile_mpeg_audio_lame_RGAD_track['name'] = getid3_lib::RGADnameLookup(
                        ...
                        if ($thisfile_mpeg_audio_lame_raw['RGAD_album'] != 0) {
                            ...
                            $thisfile_mpeg_audio_lame_RGAD_album['name'] = getid3_lib::RGADnameLookup(
                        ...
                        $EncodingFlagsATHtype = getid3_lib::BigEndian2Int(
                            substr($headerstring, $LAMEtagOffsetContant + 0xAF, 1)
                        );
                        ...
                        $thisfile_mpeg_audio_lame['raw']['abrbitrate_minbitrate'] = getid3_lib::BigEndian2Int(
                            substr($headerstring, $LAMEtagOffsetContant + 0xB0, 1)
                        );
                        ...
                        $EncoderDelays = getid3_lib::BigEndian2Int(
                            substr($headerstring, $LAMEtagOffsetContant + 0xB1, 3)
                        );
                        ...
                        $thisfile_mpeg_audio_lame['stereo_mode'] = self::LAMEmiscStereoModeLookup(
                        ...
                        $thisfile_mpeg_audio_lame['mp3_gain_db'] = (getid3_lib::RGADamplitude2dB(
                                    2
                                ) / 4) * $thisfile_mpeg_audio_lame_raw['mp3_gain'];
                        ...
                        $PresetSurroundBytes = getid3_lib::BigEndian2Int(
                            substr($headerstring, $LAMEtagOffsetContant + 0xB6, 2)
                        );
                        // Reserved                                                    = ($PresetSurroundBytes & 0xC000);
                        $thisfile_mpeg_audio_lame_raw['surround_info'] = ($PresetSurroundBytes & 0x3800);
                        $thisfile_mpeg_audio_lame['surround_info'] = self::LAMEsurroundInfoLookup(
                            $thisfile_mpeg_audio_lame_raw['surround_info']
                        );
                        $thisfile_mpeg_audio_lame['preset_used_id'] = ($PresetSurroundBytes & 0x07FF);
                        $thisfile_mpeg_audio_lame['preset_used'] = self::LAMEpresetUsedLookup(
                        ...
                        $thisfile_mpeg_audio_lame['audio_bytes'] = getid3_lib::BigEndian2Int(
                            substr($headerstring, $LAMEtagOffsetContant + 0xB8, 4)
                        );
                        ...
                        $thisfile_mpeg_audio_lame['music_crc'] = getid3_lib::BigEndian2Int(
                            substr($headerstring, $LAMEtagOffsetContant + 0xBC, 2)
                        );

                        // bytes $BE-$BF  CRC-16 of Info Tag
                        $thisfile_mpeg_audio_lame['lame_tag_crc'] = getid3_lib::BigEndian2Int(
                            substr($headerstring, $LAMEtagOffsetContant + 0xBE, 2)
                        );
                        ...
                        if ($thisfile_mpeg_audio_lame_raw['vbr_method'] == 1) {
                            $thisfile_mpeg_audio['bitrate_mode'] = 'cbr';
                            $thisfile_mpeg_audio['bitrate'] = self::ClosestStandardMP3Bitrate(
                                $thisfile_mpeg_audio['bitrate']
                            );
                            $info['audio']['bitrate'] = $thisfile_mpeg_audio['bitrate'];
    ...
    public function FreeFormatFrameLength($offset, $deepscan = false)
    {
        ...
        if (!$framelength) {
            ...
            if (!$framelength) {
                $info['error'][] = 'Cannot find next free-format synch pattern (' . getid3_lib::PrintHexBytes(
                        $SyncPattern1
                ...
                if ($MPEGaudioHeaderLengthCache[$head4] > 4) {
                    ...
                    if ($next4{0} == "\xFF") {
                        ...
                        if (!isset($MPEGaudioHeaderValidCache[$next4])) {
                            $MPEGaudioHeaderValidCache[$next4] = self::MPEGaudioHeaderValid(
                        ...
                        if ($MPEGaudioHeaderValidCache[$next4]) {
                            ...
                            if ($max_frames_scan && (++$frames_scanned >= $max_frames_scan)) {
                                $pct_data_scanned = (ftell(
                                            $this->getid3->fp
                                        ) - $info['avdataoffset']) / ($info['avdataend'] - $info['avdataoffset']);
                                $info['warning'][] = 'too many MPEG audio frames to scan, only scanned first ' . $max_frames_scan . ' frames (' . number_format(
        ...
        while ($SynchSeekOffset < $sync_seek_buffer_size) {
            if ((($avdataoffset + $SynchSeekOffset) < $info['avdataend']) && !feof($this->getid3->fp)) {
                if ($SynchSeekOffset > $sync_seek_buffer_size) {
                    ...
                    $info['error'][] = 'Could not find valid MPEG audio synch within the first ' . round(
                ...
                if ($this->decodeMPEGaudioHeader($avdataoffset + $SynchSeekOffset, $dummy, true)) {
                    ...
                    if ($BitrateHistogram) {
                        ...
                        if ($pct_data_scanned > 0) {
                            $info['warning'][] = 'too many MPEG audio frames to scan, only scanned ' . $frames_scanned . ' frames in ' . $max_scan_segments . ' segments (' . number_format(
                                    $pct_data_scanned * 100,
                                    1
                                ) . '% of file) and extrapolated distribution, playtime and bitrate may be incorrect.';
