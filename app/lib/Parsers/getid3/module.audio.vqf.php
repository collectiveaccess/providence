    ...
    public function Analyze()
    {
        ...
        while (ftell($this->getid3->fp) < $info['avdataend']) {
            ...
            switch ($ChunkName) {
                case 'COMM':
                    ...
                    $chunkoffset += 4;
                    $thisfile_vqf_COMM['security_level'] = getid3_lib::BigEndian2Int(
                        substr($ChunkData, $chunkoffset, 4)
                    );
                    $chunkoffset += 4;
                ...
                case 'COMT':
                case 'ALBM':
                    $thisfile_vqf['comments'][$this->VQFcommentNiceNameLookup($ChunkName)][] = trim(
                        substr($ChunkData, 8)
                    );
                    break;
        ...
        if (isset($thisfile_vqf['DSIZ']) && (($thisfile_vqf['DSIZ'] != ($info['avdataend'] - $info['avdataoffset'] - strlen(
            ...
            switch ($thisfile_vqf['DSIZ']) {
                ...
                default:
                    $info['warning'][] = 'Probable corrupted file - should be ' . $thisfile_vqf['DSIZ'] . ' bytes, actually ' . ($info['avdataend'] - $info['avdataoffset'] - strlen(
                                'DATA'
                            ));
                    break;
            }
