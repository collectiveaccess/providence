    ...
    public function Analyze()
    {
        ...
        switch (substr($TIFFheader, 0, 2)) {
            ...
            case 'MM':
                ...
                break;
            default:
                $info['error'][] = 'Invalid TIFF byte order identifier (' . substr(
                        $TIFFheader,
                        0,
                        2
        ...
        while ($nextIFDoffset > 0) {
            ...
            for ($i = 0; $i < $CurrentIFD['fieldcount']; $i++) {
                $CurrentIFD['fields'][$i]['raw']['tag'] = $this->TIFFendian2Int(
                    fread($this->getid3->fp, 2),
                    $info['tiff']['byte_order']
                );
                $CurrentIFD['fields'][$i]['raw']['type'] = $this->TIFFendian2Int(
                    fread($this->getid3->fp, 2),
                    $info['tiff']['byte_order']
                );
                $CurrentIFD['fields'][$i]['raw']['length'] = $this->TIFFendian2Int(
                ...
                switch ($CurrentIFD['fields'][$i]['raw']['type']) {
                    case 1: // BYTE  An 8-bit unsigned integer.
                        if ($CurrentIFD['fields'][$i]['raw']['length'] <= 4) {
                            $CurrentIFD['fields'][$i]['value'] = $this->TIFFendian2Int(
                                substr($CurrentIFD['fields'][$i]['raw']['offset'], 0, 1),
                                $info['tiff']['byte_order']
                            );
                        } else {
                            $CurrentIFD['fields'][$i]['offset'] = $this->TIFFendian2Int(
                                $CurrentIFD['fields'][$i]['raw']['offset'],
                                $info['tiff']['byte_order']
                            );
                        }
                        break;
                    ...
                    case 3: // SHORT A 16-bit (2-byte) unsigned integer.
                        if ($CurrentIFD['fields'][$i]['raw']['length'] <= 2) {
                            $CurrentIFD['fields'][$i]['value'] = $this->TIFFendian2Int(
                                substr($CurrentIFD['fields'][$i]['raw']['offset'], 0, 2),
                                $info['tiff']['byte_order']
                            );
                        } else {
                            $CurrentIFD['fields'][$i]['offset'] = $this->TIFFendian2Int(
                ...
                switch ($fieldarray['raw']['tag']) {
                    ...
                    case 259: // Compression
                        if (!isset($fieldarray['value'])) {
                            ...
                            $info['tiff']['ifd'][$IFDid]['fields'][$key]['raw']['data'] = fread(
                                $this->getid3->fp,
