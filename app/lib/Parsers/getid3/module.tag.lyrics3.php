...
class getid3_lyrics3 extends getid3_handler
{
    ...
    public function Analyze()
    {
        ...
        if (!getid3_lib::intValueSupported($info['filesize'])) {
            $info['warning'][] = 'Unable to check for Lyrics3 because file is larger than ' . round(
        ...
        if ($lyrics3end == 'LYRICSEND') {
            ...
            $lyrics3size = strrev(substr(strrev($lyrics3_id3v1), 9, 6)) + 6 + strlen(
                    'LYRICS200'
                ); // LSZ = lyrics + 'LYRICSBEGIN'; add 6-byte size field; add 'LYRICS200'
            $lyrics3offset = $info['filesize'] - $lyrics3size;
            $lyrics3version = 2;
    ...
    public function getLyrics3Data($endoffset, $version, $length)
    {
        ...
        if (substr($rawdata, 0, 11) != 'LYRICSBEGIN') {
            if (strpos($rawdata, 'LYRICSBEGIN') !== false) {
                $info['warning'][] = '"LYRICSBEGIN" expected at ' . $endoffset . ' but actually found at ' . ($endoffset + strpos(
                            $rawdata,
                            'LYRICSBEGIN'
                ...
                $info['error'][] = '"LYRICSBEGIN" expected at ' . $endoffset . ' but found "' . substr(
                        $rawdata,
                        0,
                        11
                    ) . '" instead';
        ...
        switch ($version) {
            case 1:
                if (substr($rawdata, strlen($rawdata) - 9, 9) == 'LYRICSEND') {
                    ...
                    $info['error'][] = '"LYRICSEND" expected at ' . (ftell(
                                $this->getid3->fp
                            ) - 11 + $length - 9) . ' but found "' . substr(
                            $rawdata,
                            strlen($rawdata) - 9,
                            9
                        ) . '" instead';
            ...
            case 2:
                if (substr($rawdata, strlen($rawdata) - 9, 9) == 'LYRICS200') {
                    ...
                    if (isset($ParsedLyrics3['raw']['IND'])) {
                        ...
                        foreach ($flagnames as $flagname) {
                            if (strlen($ParsedLyrics3['raw']['IND']) > $i++) {
                                $ParsedLyrics3['flags'][$flagname] = $this->IntString2Bool(
                                    substr($ParsedLyrics3['raw']['IND'], $i, 1 - 1)
                                );
                            }
                        }
                    }
                    ...
                    if (isset($ParsedLyrics3['raw']['IMG'])) {
                        ...
                        foreach ($imagestrings as $key => $imagestring) {
                            if (strpos($imagestring, '||') !== false) {
                                ...
                                $ParsedLyrics3['images'][$key]['timestamp'] = $this->Lyrics3Timestamp2Seconds(
                    ...
                    $info['error'][] = '"LYRICS200" expected at ' . (ftell(
                                $this->getid3->fp
                            ) - 11 + $length - 9) . ' but found "' . substr(
                            $rawdata,
                            strlen($rawdata) - 9,
