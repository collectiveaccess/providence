...
class DisplayTemplateParser
{
     ...
     *
     */
    static public $template_cache = null;

    /**
     *
     */
    static public $value_cache = null;

    /**
     *
     */
    static public $value_count_cache = null;

    /**
     *
     */
    static public $join_tag_vals = [];
            ...
            switch ($vs_tag = $o_node->tag) {
                case 'unit':
                    if ($vs_relative_to = $o_node->relativeTo) {
                        ...
                        $va_get_options['restrictToTypes'] = DisplayTemplateParser::_getCodesFromAttribute(
                            $o_node,
                            ['attribute' => 'restrictToTypes']
                        );
                        $va_get_options['excludeTypes'] = DisplayTemplateParser::_getCodesFromAttribute(
                            $o_node,
                            ['attribute' => 'excludeTypes']
                        );
                        $va_get_options['restrictToRelationshipTypes'] = DisplayTemplateParser::_getCodesFromAttribute(
                            $o_node,
                            ['attribute' => 'restrictToRelationshipTypes']
                        );
                        $va_get_options['excludeRelationshipTypes'] = DisplayTemplateParser::_getCodesFromAttribute(
                            $o_node,
                        ...
                        try {
                            $va_row_ids = DisplayTemplateParser::_getRelativeIDsForRowIDs(
                                $ps_tablename,
                                $vs_relative_to,
                                $pa_row_ids,
                                'related',
                            ...
                            if ((($o_node->filterNonPrimaryRepresentations == '0') || (strtolower(
                                            $o_node->filterNonPrimaryRepresentations
                                        ) == 'no')) && ($qr_res instanceof ObjectSearchResult)) {
                                if (method_exists($qr_res, "filterNonPrimaryRepresentations")) {
                                    $qr_res->filterNonPrimaryRepresentations(false);
                                }
                            }

                            $va_cache_opts = $qr_res->get(
                                $vs_relative_to . "." . $qr_res->primaryKey(),
                                array_merge($va_get_options, ['returnCacheOptions' => true])
                            );

                            $qr_res->prefetchRelated(
                                $vs_relative_to,
                                0,
                                $qr_res->getOption('prefetch'),
                                $va_cache_opts
                            );

                            if ($o_node->children) {
                                DisplayTemplateParser::prefetchAllRelatedIDs(
                                    $o_node->children,
                                    $vs_relative_to,
                                    $va_row_ids,
                                    $pa_options
    ...
    public static function process($ps_template, $pm_tablename_or_num, array $pa_row_ids, array $pa_options = null)
    {
        ...
        $ps_tablename = is_numeric($pm_tablename_or_num) ? Datamodel::getTableName(
            $pm_tablename_or_num
        ) : $pm_tablename_or_num;
        ...
        if ($t_instance instanceof BaseLabel) {
            return self::_processLabelTemplate(
                $t_instance,
                $ps_template,
                $pa_row_ids,
        ...
        if ((!$vm_filter_non_primary_reps || ($vm_filter_non_primary_reps == '0') || (strtolower(
                        $vm_filter_non_primary_reps
                    ) == 'no')) && ($qr_res instanceof ObjectSearchResult)) {
            if (method_exists($qr_res, "filterNonPrimaryRepresentations")) {
                $qr_res->filterNonPrimaryRepresentations(false);
            }
        ...
        while ($qr_res->nextHit()) {
            ...
            if (strlen($ps_skip_if_expression) > 0) {
                ...
                foreach ($va_skip_if_expression_tags as $vs_expression_tag) {
                    if (!isset($va_expression_vars[$vs_expression_tag])) {
                        $va_expression_vars[$vs_expression_tag] = $qr_res->get(
                            $vs_expression_tag,
                            [
                                'assumeDisplayField' => true,
                                'returnIdno' => true,
                                'delimiter' => $ps_delimiter
                            ]
                        );
                    }
                }
            ...
            if ($pa_options['relativeToContainer']) {
                $va_vals = DisplayTemplateParser::_getValues(
                    $qr_res,
                    array_merge(
                        $va_template['tags'],
                        array_flip(caGetTemplateTags($ps_skip_when))
                    ),
                    $pa_options
                );
                if (isset($pa_options['sort']) && is_array($pa_options['sort'])) {
                    $va_vals = caSortArrayByKeyInValue(
                        $va_vals,
                        array('__sort__'),
                        $pa_options['sortDirection'],
                ...
                foreach ($va_vals as $vn_index => $va_val_list) {
                    ...
                    $v = is_array($va_val_list) ? DisplayTemplateParser::_processChildren(
                        $qr_res,
                        $va_template['tree']->children,
                        $va_val_list,
                        array_merge(
                            ...
                            [
                                'index' => $vn_index,
                                'returnAsArray' => $pa_options['aggregateUnique']
                            ]
                        )
                    ) : '';
                ...
                $va_val_list = DisplayTemplateParser::_getValues(
                    $qr_res,
                    array_merge(
                        $va_template['tags'],
                        array_flip(caGetTemplateTags($ps_skip_when))
                ...
                $v = DisplayTemplateParser::_processChildren(
                    $qr_res,
                    $va_template['tree']->children,
                    $va_val_list,
                    array_merge(
        ...
        if ($ps_skip_when && (($vn_start = caGetOption('unitStart', $pa_options, 0)) || ($vn_length = caGetOption(
                    'unitLength',
                    $pa_options,
                    null
                )))) {
    ...
    public static function parse($ps_template, $pa_options = null)
    {
        ...
        $ps_template = str_replace(
            "<~root~>",
            "",
            str_replace("</~root~>", "", $o_doc->html())
    ...
    private static function _processChildren(SearchResult $pr_res, $po_nodes, array $pa_vals, array $pa_options = null)
    {
        ...
        foreach ($po_nodes as $vn_index => $o_node) {
            switch ($vs_tag = strtolower($o_node->tag)) {
                case 'case':
                    if (!$pb_is_case) {
                        $vs_acc .= DisplayTemplateParser::_processChildren(
                            $pr_res,
                            $o_node->children,
                            $pa_vals,
                ...
                case 'if':
                    if (strlen($vs_rule = $o_node->rule) && ExpressionParser::evaluate($vs_rule, $pa_vals)) {
                        $vs_acc .= $content = DisplayTemplateParser::_processChildren(
                            $pr_res,
                            $o_node->children,
                            DisplayTemplateParser::_getValues(
                                $pr_res,
                                DisplayTemplateParser::_getTags(
                                    $o_node->children,
                                    $pa_options
                                ),
                                $pa_options
                ...
                case 'ifnotdef':
                    ...
                    if ((($vs_tag == 'ifdef') && $vb_defined) || (($vs_tag == 'ifnotdef') && $vb_defined)) {
                        ...
                        $vs_acc .= $content = DisplayTemplateParser::_processChildren(
                            $pr_res,
                            $o_node->children,
                            $v = DisplayTemplateParser::_getValues(
                                $pr_res,
                                DisplayTemplateParser::_getTags(
                                    $o_node->children,
                                    $pa_options
                ...
                case 'ifcount':
                    ...
                    if (!is_array($va_codes = DisplayTemplateParser::_getCodesFromAttribute($o_node)) || !sizeof(
                            $va_codes
                        )) {
                        break;
                    }

                    $pa_check_access = ($t_instance->hasField('access')) ? caGetOption(
                        'checkAccess',
                        $pa_options,
                        null
                    ) : null;
                    ...
                    $va_restrict_to_types = DisplayTemplateParser::_getCodesFromAttribute(
                        $o_node,
                        ['attribute' => 'restrictToTypes']
                    );
                    $va_exclude_types = DisplayTemplateParser::_getCodesFromAttribute(
                        $o_node,
                        ['attribute' => 'excludeTypes']
                    );
                    $va_restrict_to_relationship_types = DisplayTemplateParser::_getCodesFromAttribute(
                        $o_node,
                        ['attribute' => 'restrictToRelationshipTypes']
                    );
                    $va_exclude_to_relationship_types = DisplayTemplateParser::_getCodesFromAttribute(
                        $o_node,
                        ['attribute' => 'excludeRelationshipTypes']
                    );
                    $vb_omit_blanks = !is_null($o_node->omitBlanks) ? (bool)$o_node->omitBlanks : null;
                    ...
                    foreach ($va_codes as $vs_code) {
                        if ($vs_filter_regex) {
                            $vals = $pr_res->get(
                                ...
                                array_merge(
                                    $va_get_options,
                                    ['returnAsCount' => false, 'returnAsArray' => true]
                                )
                            );
                            $vals = array_filter(
                                $vals,
                                function ($v) use ($vs_filter_regex) {
                ...
                case 'more':
                    ...
                    for ($vn_i = $vn_index + 1; $vn_i < sizeof($po_nodes); $vn_i++) {
                        if ($po_nodes[$vn_i] && ($po_nodes[$vn_i]->tag == '~text~') && is_array(
                                $va_following_tags = caGetTemplateTags($po_nodes[$vn_i]->text)
                            )) {
                ...
                case 'between':
                    ...
                    for ($vn_i = 0; $vn_i < $vn_index; $vn_i++) {
                        if ($po_nodes[$vn_i] && ($po_nodes[$vn_i]->tag == '~text~') && is_array(
                                $va_preceding_tags = caGetTemplateTags($po_nodes[$vn_i]->text)
                            )) {
                ...
                case 'expression':
                    if ($vs_exp = trim($o_node->getInnerText())) {
                        $vs_acc .= $content = ExpressionParser::evaluate(
                            DisplayTemplateParser::_processChildren(
                                $pr_res,
                                $o_node->children,
                                DisplayTemplateParser::_getValues(
                                    $pr_res,
                                ...
                                array_merge($pa_options, ['quote' => true])
                            ),
                            $pa_vals
                        );
                ...
                case 'unit':
                    ...
                    if ($va_relative_to_tmp[0] && !($t_rel_instance = Datamodel::getInstanceByTableName(
                    ...
                    if (!$vb_filter_non_primary_reps || (($o_node->filterNonPrimaryRepresentations == '0') || (strtolower(
                                    $o_node->filterNonPrimaryRepresentations
                                ) == 'no'))) {
                    ...
                    $va_get_options['filterTypes'] = DisplayTemplateParser::_getCodesFromAttribute(
                        $o_node,
                    ...
                    if (
                        ...
                        ((sizeof(
                                    $va_relative_to_tmp
                                ) >= 1) && ($va_relative_to_tmp[0] == $ps_tablename) && ($va_relative_to_tmp[1] != 'related'))
                    ) {
                        $vs_relative_to_container = null;
                        switch (strtolower($va_relative_to_tmp[1])) {
                            case 'hierarchy':
                                $va_relative_ids = $pr_res->get(
                                    $t_rel_instance->tableName() . ".hierarchy." . $t_rel_instance->primaryKey(),
                                    $va_get_options
                                );
                            ...
                            case 'parent':
                                $va_relative_ids = $pr_res->get(
                                    $t_rel_instance->tableName() . ".parent." . $t_rel_instance->primaryKey(),
                                    $va_get_options
                                );
                                $va_relative_ids = array_values($va_relative_ids);
                                break;
                            case 'children':
                                $va_relative_ids = $pr_res->get(
                                    $t_rel_instance->tableName() . ".children." . $t_rel_instance->primaryKey(),
                                    $va_get_options
                                );
                                $va_relative_ids = array_values($va_relative_ids);
                                break;
                            case 'siblings':
                                $va_relative_ids = $pr_res->get(
                                    $t_rel_instance->tableName() . ".siblings." . $t_rel_instance->primaryKey(),
                                    $va_get_options
                                );
                                $va_relative_ids = array_values($va_relative_ids);
                                break;
                            ...
                            case 'nonpreferred_labels':
                                ...
                                $va_relative_ids = $pr_res->get(
                                    $t_rel_instance->tableName() . '.' . $va_relative_to_tmp[1] . '.label_id',
                                    ['restrictToTypes' => $va_get_options['restrictToTypes'], 'returnAsArray' => true]
                                );
                                break;
                            default:
                                // If relativeTo is not set to a valid attribute try to guess from template, looking for container
                                if ($t_rel_instance->isValidMetadataElement(
                                    join(".", array_slice($va_relative_to_tmp, 1, 1)),
                                    true
                                )) {
                                    ...
                                    foreach (array_keys($va_tags) as $vs_tag) {
                                        ...
                                        if (sizeof($va_tag) >= 2) {
                                            if ($t_rel_instance->isValidMetadataElement(
                                                    $va_tag[1],
                                                    true
                                                ) && (ca_metadata_elements::getElementDatatype(
                                                        $va_tag[1]
                        ...
                        if (sizeof($va_relative_ids) && is_array($va_get_options['filterTypes'])) {
                            ...
                            if (is_array(
                                    $va_filter_types = caMakeTypeIDList(
                                        $vs_rel_table_name,
                                ...
                                if ($qr_types = caMakeSearchResult($vs_rel_table_name, $va_relative_ids)) {
                                    ...
                                    while ($qr_types->nextHit()) {
                                        if (in_array(
                                            $qr_types->get("{$vs_rel_table_name}.type_id"),
                                            $va_filter_types
                                        )) {
                                            $va_filtered_ids[] = (int)$qr_types->get(
                                                "{$vs_rel_table_name}.{$vs_rel_pk}"
                                            );
                                        }
                                    }
                        ...
                        if ((($vn_start > 0) || ($vn_length > 0)) && sizeof($va_relative_ids)) {
                            ...
                            // are accurate.
                            $va_relative_ids = array_slice(
                                $va_relative_ids,
                                $vn_start,
                                ($vn_length > 0) ? $vn_length : null
                            ); // trim to start/length
                        }
                        ...
                        if ($vs_unit_filter_regex) {
                            $va_tmpl_val = array_filter(
                                $va_tmpl_val,
                                function ($v) use ($vs_unit_filter_regex) {
                        ...
                        if ($t_instance->isRelationship()) {
                            ...
                            //
                            if (!is_array($va_get_options['restrictToTypes']) || !sizeof(
                                    $va_get_options['restrictToTypes']
                                )) {
                                $va_get_options['restrictToTypes'] = $pa_options['restrictToTypes'];
                            }
                            if (!is_array($va_get_options['excludeTypes']) || !sizeof(
                        ...
                        switch (strtolower($va_relative_to_tmp[1])) {
                            ...
                            default:
                                if (method_exists($t_instance, 'isSelfRelationship') && $t_instance->isSelfRelationship(
                                    ) && is_array($pa_primary_ids) && isset(
                                        $pa_primary_ids[$t_rel_instance->tableName()]
                                    )) {
                                    if (!is_array(
                                        $va_relative_ids = array_values(
                                            $t_instance->getRelatedIDsForSelfRelationship(
                                                $pa_primary_ids[$t_rel_instance->tableName()],
                                    ...
                                    $va_relation_ids = array_keys(
                                        $t_instance->getRelatedItems(
                                            $t_rel_instance->tableName(),
                                            array_merge(
                                                $va_get_options,
                                                array(
                                                    'returnAs' => 'data',
                                    ...
                                    if (is_array($va_relation_ids) && sizeof($va_relation_ids)) {
                                        $qr_rels = caMakeSearchResult(
                                            $t_rel_instance->getSelfRelationTableName(),
                                            $va_relation_ids
                                        );
                                        $va_relationship_type_ids = $qr_rels->getAllFieldValues(
                                            $t_rel_instance->getSelfRelationTableName() . '.type_id'
                                        );
                                    }
                                } else {
                                    if (method_exists(
                                    ...
                                    if (method_exists(
                                        ...
                                        ) && ($t_opposite = $t_rel_instance->getInstanceOpposite(
                                        ...
                                        if (isset($va_get_options['restrictToTypes']) && is_array(
                                                $va_get_options['restrictToTypes']
                                            ) && sizeof($va_get_options['restrictToTypes'])) {
                                            $va_related_items = $t_instance->getRelatedItems(
                                                $t_opposite->tableName(),
                                                array_merge(
                                                    $va_get_options,
                                                    array(
                                                        'returnAs' => 'data',
                                        ...
                                        $va_relative_ids = $va_relation_ids = array_map(
                                            function ($v) {
                                        ...
                                        if (!is_array(
                                            $va_relative_ids = $pr_res->get(
                                                $t_rel_instance->tableName(
                                                ) . ".related." . $t_rel_instance->primaryKey(),
                                                $va_get_options
                                            )
                                        )) {
                                            $va_relative_ids = [];
                                        ...
                                        $rels = $t_instance->getRelatedItems(
                                            $t_rel_instance->tableName(),
                                            array_merge(
                                                $va_get_options,
                                                array(
                                    ...
                                    if (is_array($va_relation_ids) && sizeof($va_relation_ids)) {
                                        ...
                                        $va_relationship_type_ids = $pr_res->get(
                                            $t_rel_instance->tableName() . ".type_id",
                                            ['returnAsArray' => true]
                                        );
                                    } elseif (($vs_rel_tablename = $t_rel_instance->getRelationshipTableName(
                                            $ps_tablename
                                        )) && Datamodel::isRelationship($vs_rel_tablename)) {
                                        // grab type from adjacent relationship table
                                        $va_relationship_type_ids = $pr_res->get(
                                            "{$vs_rel_tablename}.type_id",
                                            ['returnAsArray' => true]
                                        );
                                    }
                                }
                ...
                case 'whenunitomits':
                    if ($vn_last_unit_omit_count > 0) {
                        $vs_proc_template = caProcessTemplate(
                            $o_node->getInnerText(),
                            array_merge(
                                $pa_vals,
                                ['omitcount' => (int)$vn_last_unit_omit_count]
                            ),
                            ['quote' => $pb_quote]
                        );
                ...
                default:
                    if ($o_node->children && (sizeof($o_node->children) > 0)) {
                        $vs_proc_template = DisplayTemplateParser::_processChildren(
                            $pr_res,
                            $o_node->children,
                            $pa_vals,
                            $pa_options
                    ...
                    if ($vs_tag === 'l') {
                        ...
                        if ($t_instance->isRelationship() && (is_array(
                                    $va_tmp = caGetTemplateTags($o_node->html(), ['firstPartOnly' => true])
                                ) && sizeof($va_tmp))) {
                            ...
                            if (in_array(
                                ...
                                $va_linking_ids = $pr_res->get(
                                    "{$vs_linking_context}." . Datamodel::primaryKey($vs_linking_context),
                        ...
                        if ($o_node->children && (sizeof($o_node->children) > 0)) {
                            ...
                            if ($o_node->attributes) {
                                foreach ($o_node->attributes as $attribute => $value) {
                                    $vs_attr .= " {$attribute}=\"" . htmlspecialchars(
                                            caProcessTemplate($value, $pa_vals, ['quote' => $pb_quote])
                                        ) . "\"";
                                }
                            }
    ...
    private static function _getValues(SearchResult $pr_res, array $pa_tags, array $pa_options = null)
    {
        ...
        $vs_cache_key = md5(
            $vs_table . "/" . $pr_res->getPrimaryKey() . "/" . print_r($pa_tags, true) . "/" . print_r(
                $pa_options,
                true
            )
        );
        ...
        foreach (array_keys($pa_tags) as $vs_tag) {
            ...
            if (!in_array(
                ...
                $va_tag = explode(".", $vs_tag);
                $vs_get_spec = $vs_tag;
                if ($ps_prefix && (!Datamodel::tableExists($va_tag[0])) && (!preg_match(
                        "!^" . preg_quote($ps_prefix, "!") . "\.!",
                        $vs_tag
                    )) && (sizeof($va_tag) > 0)) {
            ...
            if (is_array($va_parsed_tag_opts = DisplayTemplateParser::_parseTagOpts($vs_get_spec))) {
                ...
                if (isset($va_parsed_tag_opts['options']['restrictToRelationshipTypes']) && is_array(
                        $va_parsed_tag_opts['options']['restrictToRelationshipTypes']
                ...
                if (isset($va_parsed_tag_opts['options']['restrictToTypes']) && is_array(
                        $va_parsed_tag_opts['options']['restrictToTypes']
            ...
            if (DisplayTemplateParser::$value_cache[$vs_cache_key]) {
                ...
                foreach (array_keys($pa_tags) as $vs_tag) {
                    ...
                    if (substr($vs_tag, 0, 5) === '^join') {
                        ...
                        $vs_get_spec = array_shift(
                            explode("~", array_shift(explode(";", $va_parsed_tag_opts['options']['elements'])))
                        );
                    } else {
                ...
                if (!caGetOption('skipWhen', $pa_options, false)) {
                    if ((($vn_start > 0) || ($vn_length > 0)) && ($vn_start < sizeof(
                                $va_tag_vals
            ...
            foreach (array_keys($pa_tags) as $vs_tag) {
                ...
                if ((substr($vs_tag, 0, 5) === '^join')) {
                    ...
                    if ($vs_relative_to_container) {
                        $va_rel_vs = $pr_res->get(
                            $vs_relative_to_container,
                            array_merge(
                                $pa_options,
                                $va_parsed_tag_opts['options'],
                                [
                        ...
                        foreach ($va_rel_vs as $va_rel_v) {
                            ...
                            $va_val_list[] = DisplayTemplateParser::processJoinTag(
                                $va_rel_v[$vn_c],
                                $va_parsed_tag_opts
                            );
                        }
                        ...
                        $vn_max_values_to_show_labels = caGetOption(
                            'maxValuesToShowLabels',
                            $va_parsed_tag_opts['options'],
                            null
                        );
                    ...
                    switch (strtolower($vs_get_spec)) {
                        case 'relationship_typename':
                            ...
                            if (is_array($va_relationship_type_ids) && is_array(
                                ...
                                if ($qr_rels->nextHit()) {
                                    $va_val_list = $qr_rels->get(
                                        'ca_relationship_types.preferred_labels.' . ((caGetOption(
                                                'orientation',
                                                $pa_options,
                                                'LTOR'
                        ...
                        case 'relationship_type_code':
                            ...
                            if (is_array($va_relationship_type_ids) && is_array(
                                ...
                                if ($qr_rels->nextHit()) {
                                    $va_val_list = $qr_rels->get(
                                        'ca_relationship_types.type_code',
                                        $va_opts = array_merge(
                                            $pa_options,
                                            $va_parsed_tag_opts['options'],
                                            [
                        ...
                        default:
                            ...
                            if (is_array($va_parsed_tag_opts['modifiers']) && (sizeof(
                                        $va_parsed_tag_opts['modifiers']
                                    ) > 0)) {
                                $va_val_list = array_map(
                                    function ($v) use ($va_parsed_tag_opts) {
    ...
    private static function _evaluateCodeAttribute(SearchResult $pr_res, $po_node, array $pa_options = null)
    {
        ...
        foreach ($va_codes as $vs_code => $vs_bool) {
            ...
            if (!is_array($va_val_list)) {  // no value
                ...
                if ($filter_regex) {
                    $va_val_list = array_filter(
                        $va_val_list,
                        function ($v) use ($filter_regex) {
                            return preg_match($filter_regex, $v);
                        }
                    );
                }
                if (!is_null($pn_index)) {
                    if (!isset($va_val_list[$pn_index]) || ((is_numeric(
                                    $va_val_list[$pn_index]
                                ) && (float)$va_val_list[$pn_index] == 0) || !strlen(trim($va_val_list[$pn_index])))) {
     ...
     *
     */
    public static function _getRelativeIDsForRowIDs(
        $ps_tablename,
        $ps_relative_to,
        $pa_row_ids,
        $ps_mode,
        ...
        switch ($ps_mode) {
            case 'related':
                ...
                if ($ps_tablename !== $ps_relative_to) {
                    ...
                    if (!is_array($va_path = array_keys(Datamodel::getPath($ps_tablename, $ps_relative_to))) || !sizeof(
                            $va_path
                        )) {
                    ...
                    if ($va_relationship_types = caGetOption('restrictToRelationshipTypes', $pa_options, null)) {
                        ...
                        $va_relationship_type_ids = $t_rel_type->relationshipTypeListToIDs(
                            $vs_link,
    ...
    public static function _processLabelTemplate($t_instance, $ps_template, array $pa_row_ids, array $pa_options)
    {
        ...
        foreach ($pa_row_ids as $vn_row_id) {
            ...
            $t_instance->setLabelTypeList(
                Configuration::load()->get(
                    $pb_is_preferred ? $t_instance->getSubjectTableName(
                        ) . "_preferred_label_type_list" : $t_instance->getSubjectTableName(
                        ) . "_nonpreferred_label_type_list"
                )
            );
            ...
            if (is_array($sort) && (sizeof($sort) > 0)) {
                ...
                foreach ($sort as $s) {
                    ...
                    if (is_array(
                        $sort_key_list = $t_instance->get(
                            $sf,
                            ['convertCodesToDisplayText' => true, 'returnAsArray' => true]
                        )
                    )) {
                        $sort_key_list = array_map(
                            function ($x) {
                                return is_numeric($x) ? str_pad(substr($x, 0, 50), 50, '0', STR_PAD_LEFT) : str_pad(
                                    substr($x, 0, 50),
                                    50,
                                    ' ',
                                    STR_PAD_RIGHT
    ...
    public static function processTemplate($ps_template, $pa_values, $pa_options = null)
    {
        ...
        if ($o_dim_config->get('omit_repeating_units_for_measurements_in_templates')) {
            $pa_options['dimensionsUnitMap'] = self::createDimensionsUnitMap(
                $ps_template
            );    // list of dimensional units used by tags; needed to support convoluted function to omit repeating units on quantities
        }
    ...
    private static function _processTemplateSubTemplates($po_nodes, array $pa_values, array &$pa_options = null)
    {
        ...
        foreach ($po_nodes as $vn_index => $o_node) {
            switch ($vs_tag = strtolower($o_node->tag)) {
                case 'case':
                    if (!$pb_is_case) {
                        $vs_acc .= DisplayTemplateParser::_processTemplateSubTemplates(
                            $o_node->children,
                            $pa_values,
                            array_merge(
                                $pa_options,
                ...
                case 'if':
                    if (strlen($vs_rule = $o_node->rule) && ExpressionParser::evaluate($vs_rule, $pa_values)) {
                        $vs_acc .= $content = DisplayTemplateParser::_processTemplateSubTemplates(
                            $o_node->children,
                            $pa_values,
                            $pa_options
                        );
                ...
                case 'ifdef':
                case 'ifnotdef':
                    $vb_defined = DisplayTemplateParser::_processTemplateEvaluateCodeAttribute(
                        $o_node,
                        $pa_values,
                        ['mode' => ($vs_tag == 'ifdef') ? 'present' : 'not_present']
                    );
    ...
    public static function processSimpleTemplate($ps_template, $pa_values, &$pa_options = null)
    {
        ...
        foreach ($va_tags as $vs_tag) {
            ...
            if ((substr($vs_tag, 0, 5) === '^join') && !isset($pa_values[$va_tmp[0]])) {
                $pa_values[$vs_tag] = DisplayTemplateParser::processJoinTag(
                    $pa_values,
                    DisplayTemplateParser::_parseTagOpts(
                        $vs_tag
                    )
                );
            }
            if ($t_instance && !isset($pa_values[$va_tmp[0]])) {
                $vs_gotten_val = caProcessTemplateTagDirectives(
                    $t_instance->get($va_tmp[0], $pa_options),
                    array_slice($va_tmp, 1)
                );

                $ps_template = preg_replace(
                    "/\^" . preg_quote($vs_tag, '/') . "(?![A-Za-z0-9]+)/",
                    $vs_gotten_val,
                    $ps_template
                );
            } else {
                ...
                $vs_val = caProcessTemplateTagDirectives(
                    $vs_val,
                    $va_tmp,
                ...
                $ps_template = preg_replace(
                    "/[\{]{0,1}\^(?={$vs_tag_proc}[^A-Za-z0-9_]+|{$vs_tag_proc}$){$vs_tag_proc}[\}]{0,1}/",
                    str_replace("$", "\\$", $vs_val),
                    $ps_template
    ...
    public static function processJoinTag($pa_vals, $pa_parsed_tag_opts)
    {
        ...
        $o_dim_config = Configuration::load(__CA_APP_DIR__ . "/conf/dimensions.conf");
        $vb_omit_repeating_units_for_measurements_in_templates = (bool)$o_dim_config->get(
            'omit_repeating_units_for_measurements_in_templates'
        );
        ...
        foreach (array_reverse($va_elements) as $vs_element) {
            ...
            if (!$force_english_units) {
                if ((($threshold = $o_dim_config->get(
                            'force_feet_for_all_when_dimension_exceeds'
                        )) > 0) && ($in_inches > $threshold)) {
                    $force_english_units = 'ft';
                } elseif ((($threshold = $o_dim_config->get(
                            'force_inches_for_all_when_dimension_exceeds'
                        )) > 0) && ($in_inches > $threshold)) {
            ...
            if (!$force_metric_units) {
                if ((($threshold = $o_dim_config->get(
                            'force_meters_for_all_when_dimension_exceeds'
                        )) > 0) && ($in_cm > $threshold)) {
                    $force_metric_units = 'm';
                } elseif ((($threshold = $o_dim_config->get(
                            'force_centimeters_for_all_when_dimension_exceeds'
                        )) > 0) && ($in_cm > $threshold)) {
                    $force_metric_units = 'cm';
                } elseif ((($threshold = $o_dim_config->get(
                            'force_millimeters_for_all_when_dimension_exceeds'
                        )) > 0) && ($in_cm > $threshold)) {
        ...
        foreach (array_reverse($va_elements) as $vs_element) {
            ...
            $vs_val = caProcessTemplateTagDirectives(
                ...
                $va_directives,
                [
                    'forceEnglishUnits' => $force_english_units,
                    'forceMetricUnits' => $force_metric_units
                ]
            );
        ...
        if (is_array($va_labels) && (sizeof($va_labels) > 0)) {
            if (is_null($vn_max_values_to_show_labels) || ($vn_max_values_to_show_labels >= sizeof(
                        array_filter($va_acc, "strlen")
                    ))) {
                $va_acc = array_map(
                    function ($v, $l) {
