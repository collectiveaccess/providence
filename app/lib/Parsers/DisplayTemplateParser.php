    ...
    static private function _processChildren(SearchResult $pr_res, $po_nodes, array $pa_vals, array $pa_options = null)
    {
        ...
        foreach ($po_nodes as $vn_index => $o_node) {
            switch ($vs_tag = strtolower($o_node->tag)) {
                ...
                case 'ifdef':
                case 'ifnotdef':
                    $vb_defined = DisplayTemplateParser::_evaluateCodeAttribute(
                        $pr_res,
                        $o_node,
                        [
                            'filters' => caGetOption(
                ...
                case 'expression':
                    if ($vs_exp = trim($o_node->getInnerText())) {
                        $vs_acc .= ExpressionParser::evaluate(
                            ...
                            default:
                                if (method_exists($t_instance, 'isSelfRelationship') && $t_instance->isSelfRelationship(
                                    ...
                                    if (is_array($va_relation_ids) && sizeof($va_relation_ids)) {
                                        ...
                                        $va_relationship_type_ids = $qr_rels->getAllFieldValues(
                                            $t_rel_instance->getRelationshipTableName($ps_tablename) . '.type_id'
                                        );
