...
class DisplayTemplateParser {
			...
			switch ( $vs_tag = $o_node->tag ) {
				case 'unit':
					if ( $vs_relative_to = $o_node->relativeTo ) {
						...
						$va_get_options['restrictToTypes'] = DisplayTemplateParser::_getCodesFromAttribute( $o_node,
							[ 'attribute' => 'restrictToTypes' ] );
						$va_get_options['excludeTypes']
						                                   = DisplayTemplateParser::_getCodesFromAttribute( $o_node,
							[ 'attribute' => 'excludeTypes' ] );
						$va_get_options['restrictToRelationshipTypes']
						                                   = DisplayTemplateParser::_getCodesFromAttribute( $o_node,
							[ 'attribute' => 'restrictToRelationshipTypes' ] );
						$va_get_options['excludeRelationshipTypes']
						                                   = DisplayTemplateParser::_getCodesFromAttribute( $o_node,
							[ 'attribute' => 'excludeRelationshipTypes' ] );
						...
						try {
							$va_row_ids = DisplayTemplateParser::_getRelativeIDsForRowIDs( $ps_tablename,
								$vs_relative_to, $pa_row_ids, 'related', $va_get_options );
							...
							if ( ( ( $o_node->filterNonPrimaryRepresentations == '0' )
							       || ( strtolower( $o_node->filterNonPrimaryRepresentations ) == 'no' ) )
							     && ( $qr_res instanceof ObjectSearchResult )
							) {
								if ( method_exists( $qr_res, "filterNonPrimaryRepresentations" ) ) {
									$qr_res->filterNonPrimaryRepresentations( false );
								}
							}

							$va_cache_opts = $qr_res->get( $vs_relative_to . "." . $qr_res->primaryKey(),
								array_merge( $va_get_options, [ 'returnCacheOptions' => true ] ) );

							$qr_res->prefetchRelated( $vs_relative_to, 0, $qr_res->getOption( 'prefetch' ),
								$va_cache_opts );

							if ( $o_node->children ) {
								DisplayTemplateParser::prefetchAllRelatedIDs( $o_node->children, $vs_relative_to,
									$va_row_ids, $pa_options );
							}
						} catch ( Exception $e ) {
							// prefetch failed
						}
	 ...
	 *
	 * @param string $ps_template         String with embedded tags. Tags are just alphanumeric strings prefixed with a
	 *                                    caret ("^")
	 * @param string $pm_tablename_or_num Table name or number of table from which values are being formatted
	 * @param string $pa_row_ids          An array of primary key values in the specified table to be pulled into the
	 *                                    template
	 * @param array  $pa_options          Supported options are:
	 *                                    returnAsArray = if true an array of processed template values is returned,
	 *                                    otherwise the template values are returned as a string joined together with a
	 *                                    delimiter. Default is false. delimiter = value to string together template
	 *                                    values with when returnAsArray is false. Default is ';' (semicolon)
	 *                                    placeholderPrefix = attribute container to implicitly place primary record
	 *                                    fields into. Ex. if the table is "ca_entities" and the placeholder is
	 *                                    "address" then tags like ^city will resolve to ca_entities.address.city
	 *                                    requireLinkTags = if set then links are only added when explicitly defined
	 *                                    with <l> tags. [Default is true] primaryIDs = row_ids for primary rows in
	 *                                    related table, keyed by table name; when resolving ambiguous relationships
	 *                                    the row_ids will be excluded from consideration. This option is rarely used and exists primarily to take care of a single edge case: you are processing a template relative to a self-relationship such as ca_entities_x_entities that includes references to the subject table (ca_entities, in the case of ca_entities_x_entities). There are two possible paths to take in this situations; primaryIDs lets you specify which ones you *don't* want to take by row_id. For interstitial editors, the ids will be set to a single id: that of the subject (Eg. ca_entities) row from which the interstitial was launched. sort = optional list of tag values to sort repeating values within a row template on. The tag must appear in the template. You can specify more than one tag by separating the tags with semicolons. sortDirection = The direction of the sort of repeating values within a row template. May be either ASC (ascending) or DESC (descending). [Default is ASC] linkTarget = Optional target to use when generating <l> tag-based links. By default links point to standard detail pages, but plugins may define linkTargets that point elsewhere. relativeToContainer = evaluate template in a repeating container context. [Default is false] skipIfExpression = skip the elements in $pa_row_ids for which the given expression does not evaluate true. This pulls values for the row as a whole and skips the *entire* row if the expression evaluates true. [Default is false] skipWhen = tests each templating iteration (there may be more than more if relativeToContainer is set) against an expression and skips the iteration if the expression evaluates true. If set all iterations are generated and tested, even when unitStart and unitLength are set. [Default is null] includeBlankValuesInArray = include blank template values in primary template and all <unit>s in returned array when returnAsArray is set. If you need the returned array of values to line up with the row_ids in $pa_row_ids this should be set. [Default is false] includeBlankValuesInTopLevelForPrefetch = include blank template values in *primary template* (not <unit>s) in returned array when returnAsArray is set. Used by template prefetcher to ensure returned values align with id indices. [Default is false] forceValues = Optional array of values indexed by placeholder without caret (eg. ca_objects.idno) and row_id. When present these values will be used in place of the placeholders, rather than whatever value normal processing would result in. [Default is null] aggregateUnique = Remove duplicate values. If set then array of evaluated templates may not correspond one-to-one with the original list of row_ids set in $pa_row_ids. [Default is false] unitStart = Offset to start evaluating templating iterations at (there may be more than one iteration when relativeToContainer is set). [Default is 0] unitLength = Maximum number of templating iteration to evaluate. If null, no limit is enforced (there may be more than one iteration when relativeToContainer is set). [Default is null] indexWithIDs = Return array with indexes set to row_ids. [Default is false; use numeric indices starting with zero]
	 *
	...
	static public function process( $ps_template, $pm_tablename_or_num, array $pa_row_ids, array $pa_options = null ) {
		...
		$ps_tablename = is_numeric( $pm_tablename_or_num ) ? Datamodel::getTableName( $pm_tablename_or_num )
			: $pm_tablename_or_num;
		...
		if ( ! $pa_options['isUnit'] && ! caGetOption( 'dontPrefetchRelated', $pa_options, false ) ) {
			DisplayTemplateParser::prefetchAllRelatedIDs( $va_template['tree']->children, $ps_tablename, $pa_row_ids,
				$pa_options );
		}
		...
		if ( $t_instance instanceof BaseLabel ) {
			return self::_processLabelTemplate( $t_instance, $ps_template, $pa_row_ids,
				array_merge( $pa_options, [ 'returnAsArray' => true ] ) );
		}

		$qr_res = caMakeSearchResult( $ps_tablename, $pa_row_ids, [
			'sort'          => caGetOption( 'sort', $pa_options, null ),
			'sortDirection' => caGetOption( 'sortDirection', $pa_options, null )
		] );
		...
		if ( ( ! $vm_filter_non_primary_reps || ( $vm_filter_non_primary_reps == '0' )
		       || ( strtolower( $vm_filter_non_primary_reps ) == 'no' ) )
		     && ( $qr_res instanceof ObjectSearchResult )
		) {
			if ( method_exists( $qr_res, "filterNonPrimaryRepresentations" ) ) {
				$qr_res->filterNonPrimaryRepresentations( false );
		...
		$pa_check_access = ( $t_instance->hasField( 'access' ) ) ? caGetOption( 'checkAccess', $pa_options, null )
			: null;
		...
		while ( $qr_res->nextHit() ) {
			...
			if ( strlen( $ps_skip_if_expression ) > 0 ) {
				...
				foreach ( $va_skip_if_expression_tags as $vs_expression_tag ) {
					if ( ! isset( $va_expression_vars[ $vs_expression_tag ] ) ) {
						$va_expression_vars[ $vs_expression_tag ] = $qr_res->get( $vs_expression_tag,
							[ 'assumeDisplayField' => true, 'returnIdno' => true, 'delimiter' => $ps_delimiter ] );
					}
				}
			...
			if ( $pa_options['relativeToContainer'] ) {
				$va_vals = DisplayTemplateParser::_getValues( $qr_res,
					array_merge( $va_template['tags'], array_flip( caGetTemplateTags( $ps_skip_when ) ) ),
					$pa_options );
				if ( isset( $pa_options['sort'] ) && is_array( $pa_options['sort'] ) ) {
					$va_vals = caSortArrayByKeyInValue( $va_vals, array( '__sort__' ), $pa_options['sortDirection'],
						array( 'dontRemoveKeyPrefixes' => true ) );
				}
				foreach ( $va_vals as $vn_index => $va_val_list ) {
					...
					$v = is_array( $va_val_list ) ? DisplayTemplateParser::_processChildren( $qr_res,
						$va_template['tree']->children, $va_val_list, array_merge( $pa_options,
							[ 'index' => $vn_index, 'returnAsArray' => $pa_options['aggregateUnique'] ] ) ) : '';
				...
				$va_val_list = DisplayTemplateParser::_getValues( $qr_res,
					array_merge( $va_template['tags'], array_flip( caGetTemplateTags( $ps_skip_when ) ) ),
					$pa_options );
				...
				$v = DisplayTemplateParser::_processChildren( $qr_res, $va_template['tree']->children, $va_val_list,
					array_merge( $pa_options, [ 'returnAsArray' => $pa_options['aggregateUnique'] ] ) );
				if ( $pb_index_with_ids ) {
					$va_proc_templates[ $qr_res->get( $vs_pk ) ] = $v;
				} else {
					$va_proc_templates[] = $v;
		...
		if ( $ps_skip_when
		     && ( ( $vn_start = caGetOption( 'unitStart', $pa_options, 0 ) )
		          || ( $vn_length
					= caGetOption( 'unitLength', $pa_options, null ) ) )
		) {
	...
	static public function parse( $ps_template, $pa_options = null ) {
		...
		$ps_template = str_replace( "<~root~>", "", str_replace( "</~root~>", "",
			$o_doc->html() ) );    // replace template with parsed version; this allows us to do text find/replace later
		...
		return DisplayTemplateParser::$template_cache[ $vs_cache_key ] = [
			'original_template' => $ps_template_original,
			// template as passed by caller
			'template'          => $ps_template,
			// full template with compatibility transformations performed and units replaced with placeholders
			'tags'              => $va_tags,
			// all placeholder tags used in template, both replaceable (eg. ^ca_objects.idno) and directive codes (eg. <ifdef code="ca_objects.idno">...</ifdef>
			'tree'              => $o_doc,
			// ganon instance containing parsed template HTML
			'units'             => $va_units
			// map of nested units in template
		];
	}
	# -------------------------------------------------------------------
	...
	static private function _processChildren( SearchResult $pr_res, $po_nodes, array $pa_vals, array $pa_options = null
		...
		foreach ( $po_nodes as $vn_index => $o_node ) {
			switch ( $vs_tag = strtolower( $o_node->tag ) ) {
				case 'case':
					if ( ! $pb_is_case ) {
						$vs_acc .= DisplayTemplateParser::_processChildren( $pr_res, $o_node->children, $pa_vals,
							array_merge( $pa_options, [ 'isCase' => true ] ) );
					}
					break;
				case 'if':
					if ( strlen( $vs_rule = $o_node->rule ) && ExpressionParser::evaluate( $vs_rule, $pa_vals ) ) {
						$vs_acc .= $content = DisplayTemplateParser::_processChildren( $pr_res, $o_node->children,
							DisplayTemplateParser::_getValues( $pr_res,
								DisplayTemplateParser::_getTags( $o_node->children, $pa_options ), $pa_options ),
							$pa_options );
				...
				case 'ifnotdef':
					...
					$opts           = [
						...
						'mode'        => ( $vs_tag == 'ifdef' ) ? 'present' : 'not_present',
						'filter'      => $vs_filter,
						'checkAccess' => caGetOption( 'checkAccess', $pa_options, null )
					];
					...
					if ( ( ( $vs_tag == 'ifdef' ) && $vb_defined ) || ( ( $vs_tag == 'ifnotdef' ) && $vb_defined ) ) {
						...
						$vs_acc .= $content = DisplayTemplateParser::_processChildren( $pr_res, $o_node->children,
							$v = DisplayTemplateParser::_getValues( $pr_res,
								DisplayTemplateParser::_getTags( $o_node->children, $pa_options ), $pa_options ),
							$pa_options );
						if ( $pb_is_case && $content ) {
							break( 2 );
				...
				case 'ifcount':
					...
					if ( ! is_array( $va_codes = DisplayTemplateParser::_getCodesFromAttribute( $o_node ) )
					     || ! sizeof( $va_codes )
					) {
						break;
					}

					$pa_check_access = ( $t_instance->hasField( 'access' ) ) ? caGetOption( 'checkAccess', $pa_options,
						null ) : null;
					if ( ! is_array( $pa_check_access ) || ! sizeof( $pa_check_access ) ) {
						$pa_check_access = null;
					}
					...
					$va_restrict_to_types              = DisplayTemplateParser::_getCodesFromAttribute( $o_node,
						[ 'attribute' => 'restrictToTypes' ] );
					$va_exclude_types                  = DisplayTemplateParser::_getCodesFromAttribute( $o_node,
						[ 'attribute' => 'excludeTypes' ] );
					$va_restrict_to_relationship_types = DisplayTemplateParser::_getCodesFromAttribute( $o_node,
						[ 'attribute' => 'restrictToRelationshipTypes' ] );
					$va_exclude_to_relationship_types  = DisplayTemplateParser::_getCodesFromAttribute( $o_node,
						[ 'attribute' => 'excludeRelationshipTypes' ] );
					$vb_omit_blanks                    = ! is_null( $o_node->omitBlanks ) ? (bool) $o_node->omitBlanks
						: null;
					$vs_filter                         = ! is_null( $o_node->filter ) ? (string) $o_node->filter : null;
					...
					foreach ( $va_codes as $vs_code ) {
						if ( $vs_filter_regex ) {
							$vals     = $pr_res->get( $vs_code,
								array_merge( $va_get_options, [ 'returnAsCount' => false, 'returnAsArray' => true ] ) );
				...
				case 'more':
					...
					for ( $vn_i = $vn_index + 1; $vn_i < sizeof( $po_nodes ); $vn_i ++ ) {
						if ( $po_nodes[ $vn_i ] && ( $po_nodes[ $vn_i ]->tag == '~text~' )
						     && is_array( $va_following_tags = caGetTemplateTags( $po_nodes[ $vn_i ]->text ) )
						) {

							foreach ( $va_following_tags as $vs_following_tag ) {
								if ( isset( $pa_vals[ $vs_following_tag ] )
								     && strlen( $pa_vals[ $vs_following_tag ] )
								) {
									$vs_acc .= $content = DisplayTemplateParser::_processChildren( $pr_res,
										$o_node->children, $pa_vals, $pa_options );
									if ( $pb_is_case && $content ) {
										break( 2 );
									}
								}
				...
				case 'between':
					...
					for ( $vn_i = 0; $vn_i < $vn_index; $vn_i ++ ) {
						if ( $po_nodes[ $vn_i ] && ( $po_nodes[ $vn_i ]->tag == '~text~' )
						     && is_array( $va_preceding_tags = caGetTemplateTags( $po_nodes[ $vn_i ]->text ) )
						) {

							foreach ( $va_preceding_tags as $vs_preceding_tag ) {
								if ( isset( $pa_vals[ $vs_preceding_tag ] )
								     && strlen( $pa_vals[ $vs_preceding_tag ] )
								) {
				...
				case 'expression':
					if ( $vs_exp = trim( $o_node->getInnerText() ) ) {
						$vs_acc .= $content
							= ExpressionParser::evaluate( DisplayTemplateParser::_processChildren( $pr_res,
							$o_node->children, DisplayTemplateParser::_getValues( $pr_res,
								DisplayTemplateParser::_getTags( $o_node->children, $pa_options ), $pa_options ),
							array_merge( $pa_options, [ 'quote' => true ] ) ), $pa_vals );
				...
				case 'unit':
					...
					if ( $va_relative_to_tmp[0]
					     && ! ( $t_rel_instance
							= Datamodel::getInstanceByTableName( $va_relative_to_tmp[0], true ) )
					) {
						continue( 2 );
					}
					...
					if ( ! $vb_filter_non_primary_reps
					     || ( ( $o_node->filterNonPrimaryRepresentations == '0' )
					          || ( strtolower( $o_node->filterNonPrimaryRepresentations ) == 'no' ) )
					) {
						$vb_filter_non_primary_reps = false;
						if ( method_exists( $pr_res, "filterNonPrimaryRepresentations" ) ) {
					...
					if (
						...
						|| ( ( sizeof( $va_relative_to_tmp ) >= 1 ) && ( $va_relative_to_tmp[0] == $ps_tablename )
						     && ( $va_relative_to_tmp[1] != 'related' ) )
					) {

						$vs_relative_to_container = null;
						switch ( strtolower( $va_relative_to_tmp[1] ) ) {
							case 'hierarchy':
								$va_relative_ids = $pr_res->get( $t_rel_instance->tableName() . ".hierarchy."
								                                 . $t_rel_instance->primaryKey(), $va_get_options );
								$va_relative_ids = array_values( $va_relative_ids );
								break;
							case 'parent':
								$va_relative_ids = $pr_res->get( $t_rel_instance->tableName() . ".parent."
								                                 . $t_rel_instance->primaryKey(), $va_get_options );
								$va_relative_ids = array_values( $va_relative_ids );
								break;
							case 'children':
								$va_relative_ids = $pr_res->get( $t_rel_instance->tableName() . ".children."
								                                 . $t_rel_instance->primaryKey(), $va_get_options );
								$va_relative_ids = array_values( $va_relative_ids );
								break;
							case 'siblings':
								$va_relative_ids = $pr_res->get( $t_rel_instance->tableName() . ".siblings."
								                                 . $t_rel_instance->primaryKey(), $va_get_options );
								$va_relative_ids = array_values( $va_relative_ids );
								break;
							...
							case 'nonpreferred_labels':
								...
								$va_relative_ids = $pr_res->get( $t_rel_instance->tableName() . '.'
								                                 . $va_relative_to_tmp[1] . '.label_id', [
							...
							default:
								...
								if ( $t_rel_instance->isValidMetadataElement( join( ".",
									array_slice( $va_relative_to_tmp, 1, 1 ) ), true )
								) {
									...
									foreach ( array_keys( $va_tags ) as $vs_tag ) {
										...
										if ( sizeof( $va_tag ) >= 2 ) {
											if ( $t_rel_instance->isValidMetadataElement( $va_tag[1], true )
											     && ( ca_metadata_elements::getElementDatatype( $va_tag[1] )
											          === __CA_ATTRIBUTE_VALUE_CONTAINER__ )
											) {
						...
						if ( sizeof( $va_relative_ids ) && is_array( $va_get_options['filterTypes'] ) ) {
							...
							if ( is_array( $va_filter_types = caMakeTypeIDList( $vs_rel_table_name,
									$va_get_options['filterTypes'],
									[ 'dontIncludeSubtypesInTypeRestriction' => true ] ) )
							     && sizeof( $va_filter_types )
							) {
								if ( $qr_types = caMakeSearchResult( $vs_rel_table_name, $va_relative_ids ) ) {
									$va_filtered_ids = [];
									while ( $qr_types->nextHit() ) {
										if ( in_array( $qr_types->get( "{$vs_rel_table_name}.type_id" ),
											$va_filter_types )
										) {
											$va_filtered_ids[]
												= (int) $qr_types->get( "{$vs_rel_table_name}.{$vs_rel_pk}" );
										}
									}
						...
						if ( ( ( $vn_start > 0 ) || ( $vn_length > 0 ) ) && sizeof( $va_relative_ids ) ) {
							...
							// are accurate.
							$va_relative_ids = array_slice( $va_relative_ids, $vn_start,
								( $vn_length > 0 ) ? $vn_length : null ); // trim to start/length
						}
						...
						if ( $t_instance->isRelationship() ) {
							...
							//
							if ( ! is_array( $va_get_options['restrictToTypes'] )
							     || ! sizeof( $va_get_options['restrictToTypes'] )
							) {
								$va_get_options['restrictToTypes'] = $pa_options['restrictToTypes'];
							}
							if ( ! is_array( $va_get_options['excludeTypes'] )
							     || ! sizeof( $va_get_options['excludeTypes'] )
							) {
						...
						switch ( strtolower( $va_relative_to_tmp[1] ) ) {
							...
							case 'related':
							default:
								if ( method_exists( $t_instance, 'isSelfRelationship' )
								     && $t_instance->isSelfRelationship()
								     && is_array( $pa_primary_ids )
								     && isset( $pa_primary_ids[ $t_rel_instance->tableName() ] )
								) {
									if ( ! is_array( $va_relative_ids
										= array_values( $t_instance->getRelatedIDsForSelfRelationship( $pa_primary_ids[ $t_rel_instance->tableName() ],
										array( $pr_res->getPrimaryKey() ) ) ) )
									) {
										$va_relative_ids = [];
									}

									$va_relation_ids
										                      = array_keys( $t_instance->getRelatedItems( $t_rel_instance->tableName(),
										array_merge( $va_get_options, array(
											'returnAs' => 'data',
											'row_ids'  => [ $pr_res->getPrimaryKey() ]
										) ) ) );
									$va_relationship_type_ids = array();
									if ( is_array( $va_relation_ids ) && sizeof( $va_relation_ids ) ) {
										$qr_rels = caMakeSearchResult( $t_rel_instance->getSelfRelationTableName(),
											$va_relation_ids );
										$va_relationship_type_ids
										         = $qr_rels->getAllFieldValues( $t_rel_instance->getSelfRelationTableName()
										                                        . '.type_id' );
									}
								} else {
									if ( method_exists( $t_rel_instance, 'isSelfRelationship' )
									     && $t_rel_instance->isSelfRelationship()
									) {
										$va_get_options['primaryIDs'][ $t_instance->tableName() ]
											= [ $pr_res->getPrimaryKey() ];
									}

									if ( method_exists( $t_rel_instance, 'isRelationship' )
									     && $t_rel_instance->isRelationship()
									     && ( $t_opposite
											= $t_rel_instance->getInstanceOpposite( $t_instance->tableName() ) )
									) {
										...
										if ( isset( $va_get_options['restrictToTypes'] )
										     && is_array( $va_get_options['restrictToTypes'] )
										     && sizeof( $va_get_options['restrictToTypes'] )
										) {
											$va_related_items = $t_instance->getRelatedItems( $t_opposite->tableName(),
												array_merge( $va_get_options, array(
													'returnAs' => 'data',
													'row_ids'  => [ $pr_res->getPrimaryKey() ]
												) ) );
											...
											$va_related_items
												= $t_instance->getRelatedItems( $t_rel_instance->tableName(),
												array_merge( $va_get_options, array(
													'returnAs' => 'data',
													'row_ids'  => [ $pr_res->getPrimaryKey() ]
												) ) );
										...
										if ( ! is_array(
											$va_relative_ids = $pr_res->get( $t_rel_instance->tableName() . ".related."
											                                 . $t_rel_instance->primaryKey(),
												$va_get_options ) )
										) {
											$va_relative_ids = [];
										...
										$rels            = $t_instance->getRelatedItems( $t_rel_instance->tableName(),
											array_merge( $va_get_options, array(
												'returnAs' => 'data',
												'row_ids'  => [ $pr_res->getPrimaryKey() ]
											) ) );
									...
									$va_relationship_type_ids = array();
									if ( is_array( $va_relation_ids ) && sizeof( $va_relation_ids ) ) {
										$qr_rels                  = caMakeSearchResult(
											$t = $t_rel_instance->isRelationship() ? $t_rel_instance->tableName()
												: $t_rel_instance->getRelationshipTableName( $ps_tablename ),
											$va_relation_ids );
										...
										$va_relationship_type_ids = $pr_res->get( $t_rel_instance->tableName()
										                                          . ".type_id",
											[ 'returnAsArray' => true ] );
									} elseif ( ( $vs_rel_tablename
											= $t_rel_instance->getRelationshipTableName( $ps_tablename ) )
									           && Datamodel::isRelationship( $vs_rel_tablename )
									) {
				...
				case 'whenunitomits':
					if ( $vn_last_unit_omit_count > 0 ) {
						$vs_proc_template = caProcessTemplate( $o_node->getInnerText(),
							array_merge( $pa_vals, [ 'omitcount' => (int) $vn_last_unit_omit_count ] ),
							[ 'quote' => $pb_quote ] );
				...
				default:
					if ( $o_node->children && ( sizeof( $o_node->children ) > 0 ) ) {
						$vs_proc_template = DisplayTemplateParser::_processChildren( $pr_res, $o_node->children,
							$pa_vals, $pa_options );
					} else {
					...
					if ( $vs_tag === 'l' ) {
						...
						if ( $t_instance->isRelationship()
						     && ( is_array( $va_tmp = caGetTemplateTags( $o_node->html(),
									[ 'firstPartOnly' => true ] ) )
						          && sizeof( $va_tmp ) )
						) {
							$vs_linking_context = array_shift( $va_tmp );
							if ( in_array( $vs_linking_context,
								[ $t_instance->getLeftTableName(), $t_instance->getRightTableName() ] )
							) {
								$va_linking_ids = $pr_res->get( "{$vs_linking_context}."
								                                . Datamodel::primaryKey( $vs_linking_context ),
									[ 'returnAsArray' => true, 'primaryIDs' => $pa_options['primaryIDs'] ] );
							}
						}
						...
						if ( $o_node->children && ( sizeof( $o_node->children ) > 0 ) ) {
							...
							if ( $o_node->attributes ) {
								foreach ( $o_node->attributes as $attribute => $value ) {
									$vs_attr .= " {$attribute}=\"" . htmlspecialchars( caProcessTemplate( $value,
											$pa_vals, [ 'quote' => $pb_quote ] ) ) . "\"";
								}
							}
							...
							$vs_attr = '';
							foreach ( $o_node->attributes as $attribute => $value ) {
								$vs_attr .= " {$attribute}=\"" . htmlspecialchars( caProcessTemplate( $value, $pa_vals,
										[ 'quote' => $pb_quote ] ) ) . "\"";
							}
	...
	static private function _getValues( SearchResult $pr_res, array $pa_tags, array $pa_options = null ) {
		...
		$vs_cache_key = md5( $vs_table . "/" . $pr_res->getPrimaryKey() . "/" . print_r( $pa_tags, true ) . "/"
		                     . print_r( $pa_options, true ) );
		...
		foreach ( array_keys( $pa_tags ) as $vs_tag ) {
			...
			if ( ! in_array( strtolower( $vs_tag ), [
				...
				$va_tag      = explode( ".", $vs_tag );
				$vs_get_spec = $vs_tag;
				if ( $ps_prefix && ( ! Datamodel::tableExists( $va_tag[0] ) )
				     && ( ! preg_match( "!^" . preg_quote( $ps_prefix, "!" ) . "\.!", $vs_tag ) )
				     && ( sizeof( $va_tag ) > 0 )
				) {
			...
			if ( is_array( $va_parsed_tag_opts = DisplayTemplateParser::_parseTagOpts( $vs_get_spec ) ) ) {
				...
				if ( isset( $va_parsed_tag_opts['options']['restrictToRelationshipTypes'] )
				     && is_array( $va_parsed_tag_opts['options']['restrictToRelationshipTypes'] )
				     && sizeof( $va_parsed_tag_opts['options']['restrictToRelationshipTypes'] )
				) {
					unset( $va_remove_opts_for_related['restrictToRelationshipTypes'] );
				}
				if ( isset( $va_parsed_tag_opts['options']['restrictToTypes'] )
				     && is_array( $va_parsed_tag_opts['options']['restrictToTypes'] )
				     && sizeof( $va_parsed_tag_opts['options']['restrictToTypes'] )
				) {
			...
			if ( DisplayTemplateParser::$value_cache[ $vs_cache_key ] ) {
				...
				foreach ( array_keys( $pa_tags ) as $vs_tag ) {
					...
					if ( substr( $vs_tag, 0, 5 ) === '^join' ) {
						...
						$vs_get_spec = array_shift( explode( "~",
							array_shift( explode( ";", $va_parsed_tag_opts['options']['elements'] ) ) ) );
					} else {
					...
					$va_vals = $pr_res->get( $vs_get_spec, array_merge( $pa_options, $va_parsed_tag_opts['options'], [
						'sort'              => '',
						'filters'           => $va_parsed_tag_opts['filters'],
						'returnAsArray'     => true,
						'returnBlankValues' => true
					], $va_get_specs[ $vs_tag ]['isRelated'] ? $va_remove_opts_for_related : [] ) );
				...
				if ( isset( $pa_options['sort'] ) && is_array( $pa_options['sort'] )
					...
					foreach ( $pa_options['sort'] as $vs_sort_spec ) {
						$va_sortables[] = $pr_res->get( $vs_sort_spec,
							array_merge( $pa_options, $va_parsed_tag_opts['options'], [
								'sort'              => '',
								'filters'           => $va_parsed_tag_opts['filters'],
								'sortable'          => true,
								'returnAsArray'     => true,
				...
				if ( ! caGetOption( 'skipWhen', $pa_options, false ) ) {
					if ( ( ( $vn_start > 0 ) || ( $vn_length > 0 ) ) && ( $vn_start < sizeof( $va_tag_vals ) )
					     && ( ! $vn_length || ( $vn_start + $vn_length <= sizeof( $va_tag_vals ) ) )
					) {
			...
			foreach ( array_keys( $pa_tags ) as $vs_tag ) {
				...
				if ( ( substr( $vs_tag, 0, 5 ) === '^join' ) ) {
					...
					if ( $vs_relative_to_container ) {
						$va_rel_vs = $pr_res->get( $vs_relative_to_container,
							array_merge( $pa_options, $va_parsed_tag_opts['options'], [
								'filters'             => $va_parsed_tag_opts['filters'],
								'returnAsArray'       => true,
						...
						foreach ( $va_rel_vs as $va_rel_v ) {
							...
							$va_val_list[] = DisplayTemplateParser::processJoinTag( $va_rel_v[ $vn_c ],
								$va_parsed_tag_opts );
						}
						$vb_val_is_set = true;
					} else {
						$va_elements                  = explode( ";",
							caGetOption( 'elements', $va_parsed_tag_opts['options'], '' ) );
						$va_labels                    = explode( ";",
							caGetOption( 'labels', $va_parsed_tag_opts['options'], '' ) );
						$vn_max_values_to_show_labels = caGetOption( 'maxValuesToShowLabels',
							$va_parsed_tag_opts['options'], null );

						foreach ( $va_elements as $vs_element ) {
							$va_directives = explode( '~', $vs_element );
							$vs_spec       = array_shift( $va_directives );
							if ( strlen(
								$vs_v = caProcessTemplateTagDirectives( $pr_res->get( $vs_spec ), $va_directives ) )
							) {
								$va_acc[] = $vs_v;
							}
						}
						$va_val_list[] = join( caGetOption( 'delimiter', $va_parsed_tag_opts['options'], '; ' ),
							$va_acc );
						$vb_val_is_set = true;
					}
				} else {
					switch ( strtolower( $vs_get_spec ) ) {
						case 'relationship_typename':
							...
							if ( is_array( $va_relationship_type_ids )
							     && is_array(
								     $va_relationship_type_ids = array_slice( $va_relationship_type_ids, $vn_start ) )
							     && ( $vn_type_id = $va_relationship_type_ids[ $pr_res->currentIndex() ] )
							) {
								$qr_rels = caMakeSearchResult( 'ca_relationship_types', array( $vn_type_id ) );
								if ( $qr_rels->nextHit() ) {
									$va_val_list = $qr_rels->get( 'ca_relationship_types.preferred_labels.'
									                              . ( ( caGetOption( 'orientation', $pa_options,
												'LTOR' ) == 'LTOR' ) ? 'typename' : 'typename_reverse' ),
										$va_opts = array_merge( $pa_options, $va_parsed_tag_opts['options'],
											[ 'returnAsArray' => true, 'returnWithStructure' => false ] ) );
								}
							} else {
								$va_val_list = $pr_res->get( 'ca_relationship_types.preferred_labels.'
								                             . ( ( caGetOption( 'orientation', $pa_options, 'LTOR' )
								                                   == 'LTOR' ) ? 'typename' : 'typename_reverse' ),
									$va_opts = array_merge( $pa_options, $va_parsed_tag_opts['options'],
										[ 'returnAsArray' => true, 'returnWithStructure' => false ] ) );
							}
						...
						case 'relationship_type_id':
							if ( is_array( $va_relationship_type_ids )
								...
								$va_val_list = $pr_res->get( 'ca_relationship_types.type_id',
									$va_opts = array_merge( $pa_options, $va_parsed_tag_opts['options'],
										[ 'returnAsArray' => true, 'returnWithStructure' => false ] ) );
							}
							$vb_rel_type_is_set = true;
						...
						case 'relationship_type_code':
							...
							if ( is_array( $va_relationship_type_ids )
								...
								$qr_rels = caMakeSearchResult( 'ca_relationship_types', array( $vn_type_id ) );
								if ( $qr_rels->nextHit() ) {
									$va_val_list = $qr_rels->get( 'ca_relationship_types.type_code',
										$va_opts = array_merge( $pa_options, $va_parsed_tag_opts['options'],
											[ 'returnAsArray' => true, 'returnWithStructure' => false ] ) );
								}
							} else {
								$va_val_list = $pr_res->get( 'ca_relationship_types.type_code',
									$va_opts = array_merge( $pa_options, $va_parsed_tag_opts['options'],
										[ 'returnAsArray' => true, 'returnWithStructure' => false ] ) );
							}
							$vb_rel_type_is_set = true;
						...
						default:
							if ( isset( $pa_options['forceValues'][ $vs_get_spec ][ $pr_res->getPrimaryKey() ] ) ) {
								...
								if ( ! caGetOption( 'skipWhen', $pa_options, false ) ) {
									if ( ( ( $vn_start > 0 ) || ( $vn_length > 0 ) )
									     && ( $vn_start < sizeof( $va_val_list ) )
									     && ( ! $vn_length || ( $vn_start + $vn_length <= sizeof( $va_val_list ) ) )
									) {
										$va_val_list = array_slice( $va_val_list, $vn_start,
											( $vn_length > 0 ) ? $vn_length : null );
									}
								}
							}

							if ( is_array( $va_parsed_tag_opts['modifiers'] )
							     && ( sizeof( $va_parsed_tag_opts['modifiers'] ) > 0 )
							) {
							...
							$vb_val_is_referenced
								= true;                                                        // Flag that a data value is in the template
	...
	static private function _getValuesForCodeAttribute( SearchResult $pr_res, $po_node, array $pa_options = null ) {
		...
		foreach ( $va_codes as $vs_code ) {
			...
			if ( ! is_array( $va_val_list = $pr_res->get( $va_parsed_tag_opts['tag'],
				array_merge( $va_parsed_tag_opts['options'],
					[ 'filters' => $va_parsed_tag_opts['filters'], 'returnAsArray' => true ] ) ) )
			) {
				continue;
			}
	...
	static private function _evaluateCodeAttribute( SearchResult $pr_res, $po_node, array $pa_options = null ) {
		...
		foreach ( $va_codes as $vs_code => $vs_bool ) {
			...
			$va_parsed_tag_opts = DisplayTemplateParser::_parseTagOpts( $vs_code );
			$va_val_list        = $pr_res->get( $va_parsed_tag_opts['tag'], array_merge( $va_parsed_tag_opts['options'],
				[
					'filters'                   => $va_parsed_tag_opts['filters'],
					'returnAsArray'             => true,
					'returnBlankValues'         => true,
					'convertCodesToDisplayText' => true,
					'returnAsDecimal'           => true,
					'getDirectDate'             => true,
					'checkAccess'               => caGetOption( 'checkAccess', $pa_options, null )
				] ) );
			if ( ! is_array( $va_val_list ) ) {  // no value
				...
				if ( ! is_null( $pn_index ) ) {
					if ( ! isset( $va_val_list[ $pn_index ] )
					     || ( ( is_numeric( $va_val_list[ $pn_index ] )
					            && (float) $va_val_list[ $pn_index ] == 0 )
					          || ! strlen( trim( $va_val_list[ $pn_index ] ) ) )
					) {
	...
	static private function _parseTagOpts( $ps_get_spec ) {
		...
		return [
			'tag'       => $ps_get_spec,
			'options'   => $va_tag_opts,
			'filters'   => $va_tag_filters,
			'modifiers' => $va_modifiers
		];
	}
	# -------------------------------------------------------------------
	 ...
	 *
	 */
	static public function _getRelativeIDsForRowIDs(
		$ps_tablename, $ps_relative_to, $pa_row_ids, $ps_mode, $pa_options = null
	) {
		$t_instance = Datamodel::getInstanceByTableName( $ps_tablename, true );
		if ( ! $t_instance ) {
			return null;
		...
		switch ( $ps_mode ) {
			case 'related':
				...
				if ( $ps_tablename !== $ps_relative_to ) {
					...
					if ( ! is_array( $va_path = array_keys( Datamodel::getPath( $ps_tablename, $ps_relative_to ) ) )
					     || ! sizeof( $va_path )
					) {
					...
					if ( $va_relationship_types = caGetOption( 'restrictToRelationshipTypes', $pa_options, null ) ) {
						...
						$va_relationship_type_ids = $t_rel_type->relationshipTypeListToIDs( $vs_link,
	 ...
	 *                              returnAsArray = if true an array of processed template values is returned,
	 *                              otherwise the template values are returned as a string joined together with a
	 *                              delimiter. Default is false. delimiter = value to string together template values
	 *                              with when returnAsArray is false. Default is ';' (semicolon) sort = optional list
	 *                              of tag values to sort repeating values within a label template on. The tag must
	 *                              reference a label field. You can specify more than one tag by separating the tags
	 *                              with semicolons. sortDirection = the direction of the sort of repeating values
	 *                              within a label template. May be either ASC (ascending) or DESC (descending).
	 *                              [Default is ASC]
	 *
	 * @return array
	 */
	public static function _processLabelTemplate( $t_instance, $ps_template, array $pa_row_ids, array $pa_options ) {
		...
		foreach ( $pa_row_ids as $vn_row_id ) {
			...
			$pb_is_preferred = (bool) ( $t_instance->hasField( 'is_preferred' ) ? $t_instance->get( 'is_preferred' )
				: false );

			$t_instance->setLabelTypeList( Configuration::load()->get( $pb_is_preferred
				? $t_instance->getSubjectTableName() . "_preferred_label_type_list"
				: $t_instance->getSubjectTableName() . "_nonpreferred_label_type_list" ) );
			...
			if ( is_array( $sort ) && ( sizeof( $sort ) > 0 ) ) {
				...
				foreach ( $sort as $s ) {
					...
					if ( is_array( $sort_key_list = $t_instance->get( $sf,
						[ 'convertCodesToDisplayText' => true, 'returnAsArray' => true ] ) )
					) {
						$sort_key_list = array_map( function ( $x ) {
							return is_numeric( $x ) ? str_pad( substr( $x, 0, 50 ), 50, '0', STR_PAD_LEFT )
								: str_pad( substr( $x, 0, 50 ), 50, ' ', STR_PAD_RIGHT );
						}, $sort_key_list );
						sort( $sort_key_list );
	 ...
	 * @param array  $pa_options  Supported options are:
	 *                            prefix = string to add to beginning of tags extracted from template before doing
	 *                            lookup into value array removePrefix = string to remove from tags extracted from
	 *                            template before doing lookup into value array getFrom = a model instance to draw data
	 *                            from. If set, $pa_values is ignored. quote = quote replacement values (Eg.
	 *                            ^ca_objects.idno becomes "2015.001" rather than 2015.001). Value containing quotes
	 *                            will be escaped with a backslash. [Default is false]
	 *
	...
	public static function processTemplate( $ps_template, $pa_values, $pa_options = null ) {
		...
		if ( $o_dim_config->get( 'omit_repeating_units_for_measurements_in_templates' ) ) {
			$pa_options['dimensionsUnitMap']
				= self::createDimensionsUnitMap( $ps_template );    // list of dimensional units used by tags; needed to support convoluted function to omit repeating units on quantities
		}
	...
	static private function createDimensionsUnitMap( $ps_template, $pa_options = null ) {
		...
		$units = array_map( function ( $v ) {
			return preg_replace( "!^units:!i", "",
				array_shift( array_filter( array_slice( explode( "~", $v ), 1 ), function ( $x ) {
					$t = explode( ":", $x );
	...
	static private function _processTemplateSubTemplates( $po_nodes, array $pa_values, array &$pa_options = null ) {
		...
		foreach ( $po_nodes as $vn_index => $o_node ) {
			switch ( $vs_tag = strtolower( $o_node->tag ) ) {
				case 'case':
					if ( ! $pb_is_case ) {
						$vs_acc .= DisplayTemplateParser::_processTemplateSubTemplates( $o_node->children, $pa_values,
							array_merge( $pa_options, [ 'isCase' => true ] ) );
					}
					break;
				case 'if':
					if ( strlen( $vs_rule = $o_node->rule ) && ExpressionParser::evaluate( $vs_rule, $pa_values ) ) {
						$vs_acc .= $content = DisplayTemplateParser::_processTemplateSubTemplates( $o_node->children,
				...
				case 'ifdef':
				case 'ifnotdef':
					$vb_defined = DisplayTemplateParser::_processTemplateEvaluateCodeAttribute( $o_node, $pa_values,
						[ 'mode' => ( $vs_tag == 'ifdef' ) ? 'present' : 'not_present' ] );
	 ...
	 *                            from. If set, $pa_values is ignored. quote = quote replacement values (Eg.
	 *                            ^ca_objects.idno becomes "2015.001" rather than 2015.001). Value containing quotes
	 *                            will be escaped with a backslash. [Default is false] skipTagsWithoutValues = Don't
	 *                            process tags without a corresponding value in $pa_values. [Default is false]
	 *
	 * @return string Output of processed template
	 */
	static public function processSimpleTemplate( $ps_template, $pa_values, &$pa_options = null ) {
		...
		foreach ( $va_tags as $vs_tag ) {
			...
			if ( ( substr( $vs_tag, 0, 5 ) === '^join' ) && ! isset( $pa_values[ $va_tmp[0] ] ) ) {
				$pa_values[ $vs_tag ] = DisplayTemplateParser::processJoinTag( $pa_values,
					DisplayTemplateParser::_parseTagOpts( $vs_tag ) );
			}
			if ( $t_instance && ! isset( $pa_values[ $va_tmp[0] ] ) ) {
				$vs_gotten_val = caProcessTemplateTagDirectives( $t_instance->get( $va_tmp[0], $pa_options ),
					array_slice( $va_tmp, 1 ) );

				$ps_template = preg_replace( "/\^" . preg_quote( $vs_tag, '/' ) . "(?![A-Za-z0-9]+)/", $vs_gotten_val,
					$ps_template );
			} else {
				if (
				...
				$vs_val = caProcessTemplateTagDirectives( $vs_val, $va_tmp,
					[ 'omitUnits' => ( isset( $pa_options['dimensionsUnitMap'] ) && ( $cur_unit == $next_unit ) ) ] );
				...
				$ps_template
				             = preg_replace( "/[\{]{0,1}\^(?={$vs_tag_proc}[^A-Za-z0-9_]+|{$vs_tag_proc}$){$vs_tag_proc}[\}]{0,1}/",
					str_replace( "$", "\\$", $vs_val ),
					$ps_template );    // escape "$" to prevent interpretation as backreferences
			}
			$pa_options['tagIndex'] ++;
		}
	...
	static public function processJoinTag( $pa_vals, $pa_parsed_tag_opts ) {
		...
		$o_dim_config = Configuration::load( __CA_APP_DIR__ . "/conf/dimensions.conf" );
		$vb_omit_repeating_units_for_measurements_in_templates
		              = (bool) $o_dim_config->get( 'omit_repeating_units_for_measurements_in_templates' );
		...
		foreach ( array_reverse( $va_elements ) as $vs_element ) {
			...
			if ( ! $force_english_units ) {
				if ( ( ( $threshold = $o_dim_config->get( 'force_feet_for_all_when_dimension_exceeds' ) ) > 0 )
				     && ( $in_inches > $threshold )
				) {
					$force_english_units = 'ft';
				} elseif ( ( ( $threshold = $o_dim_config->get( 'force_inches_for_all_when_dimension_exceeds' ) ) > 0 )
				           && ( $in_inches > $threshold )
				) {
			...
			if ( ! $force_metric_units ) {
				if ( ( ( $threshold = $o_dim_config->get( 'force_meters_for_all_when_dimension_exceeds' ) ) > 0 )
				     && ( $in_cm > $threshold )
				) {
					$force_metric_units = 'm';
				} elseif ( ( ( $threshold = $o_dim_config->get( 'force_centimeters_for_all_when_dimension_exceeds' ) )
				             > 0 )
				           && ( $in_cm > $threshold )
				) {
					$force_metric_units = 'cm';
				} elseif ( ( ( $threshold = $o_dim_config->get( 'force_millimeters_for_all_when_dimension_exceeds' ) )
				             > 0 )
				           && ( $in_cm > $threshold )
				) {
		...
		foreach ( array_reverse( $va_elements ) as $vs_element ) {
			...
			$vs_val        = caProcessTemplateTagDirectives( $pa_vals[ $vs_spec ], $va_directives,
				[ 'forceEnglishUnits' => $force_english_units, 'forceMetricUnits' => $force_metric_units ] );
		...
		if ( is_array( $va_labels ) && ( sizeof( $va_labels ) > 0 ) ) {
			if ( is_null( $vn_max_values_to_show_labels )
			     || ( $vn_max_values_to_show_labels >= sizeof( array_filter( $va_acc, "strlen" ) ) )
			) {
