...
class TimecodeParser {
	...
	public function parse( $ps_timecode ) {
		if ( preg_match( "/^([\d]+[\.]{0,1}[\d]*)[s]{0,1}$/", $ps_timecode, $va_matches ) ) {
			...
			if ( preg_match( "/[\d]+[hmsf]{1}/", $ps_timecode ) ) {
				...
				return $this->opn_parsed_value_in_seconds = ( $vn_hours * 3600 ) + ( $vn_minutes * 60 )
				                                            + ( $vn_seconds ) + ( $vn_frames / $this->opn_timebase );
			} else {
	 ...
	 *                           COLON_DELIMITED = hh:mm:ss (Ex. 3:30:05); "delimited" and "colon" may also be used to
	 *                           select this format. HOURS_MINUTES_SECONDS = h m s (Ex. 3h 30m 5s); "hms" and "time"
	 *                           may also be used to select this format. HOURS_MINUTES = h m (Ex. 3h 30m); "hm" may
	 *                           also be used to select this format.
	 * @param array  $pa_options Options include:
	 *                           blankOnZero = Return blank if timecode value is zero seconds. [Default is false]
	 *                           noFractionalSeconds = Return seconds value as whole number, truncating decimals.
	 *                           [Default is false] omitSeconds = Omit seconds from returned value when format is set
	 *                           to COLON_DELIMITED or HOURS_MINUTES_SECONDS. [Default is false]
	 *
	 * @return string
	 */
	public function getText( $ps_format = "RAW", $pa_options = null ) {
		...
		$pb_no_fractional_seconds = caGetOption( [ 'NO_FRACTIONAL_SECONDS', 'noFractionalSeconds' ], $pa_options,
			false );
				...
				if ( ! $vn_time_in_seconds && $pb_blank_on_zero ) {
					...
					switch ( $ps_format ) {
						case 'COLON_DELIMITED':
							...
							return $vn_hours . ":" . sprintf( "%02d", $vn_minutes ) . ( $pb_omit_seconds ? ''
									: ":" . $vs_seconds );
							break;
						case 'HOURS_MINUTES':
							return ( ( $vn_hours > 0 ) ? "{$vn_hours}h " : '' ) . ( ( $vn_minutes > 0 )
									? "{$vn_minutes}m " : '' );
							break;
						case 'HOURS_MINUTES_SECONDS':
						default:
							return trim( ( ( $vn_hours > 0 ) ? "{$vn_hours}h " : '' ) . ( ( $vn_minutes > 0 )
									? "{$vn_minutes}m " : '' ) . ( $pb_omit_seconds ? ''
									: sprintf( "%02.1f", $vn_seconds ) . "s" ) );
							break;
					}
				}
				break;
