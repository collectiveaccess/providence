...
// $cachedir is optional cache directory
	function Ogg( $OggFile, $options = 0, $cachedir = "/tmp" ) {
		...
		if ( $this->cache ) {
			...
			$cache                      = $this->cachedir . "/" . strtr( $this->Streams['oggfile'], "/:?&#", "-----" )
			                              . ".cache";
			$this->Streams['cachefile'] = $cache;
			if ( ( ( $options & UPDATECACHE ) == 0 )
			     && is_resource( $cachefile = @fopen( $this->realpath( $this->Streams['cachefile'] ), "r" ) )
			) {
				if ( $s = fread( $cachefile, filesize( $this->realpath( $this->Streams['cachefile'] ) ) ) ) {
					$this->Streams = unserialize( $s );
					$this->Streams['picturable']
					               = $this->Picturable(); // Can the server extract a picture ? better check again.
					...
					if ( ( strpos( $OggFile, "://" ) === false )
					     && ( $this->Streams['size'] != filesize( $this->realpath( $this->Streams['oggfile'] ) ) )
					) {
		...
		if ( ( strpos( $OggFile, "://" ) === false )
		     && ( $tmp = glob( $this->realpath( $this->Streams['oggfile'] . "*.tmp" ) ) )
		) {
			if ( count( $tmp ) == 1 ) {
			...
			if ( isset( $this->Streams['tmpfile'] ) ) {
				$this->Streams['tmpfile'] = substr( $this->Streams['tmpfile'],
					strpos( $this->Streams['tmpfile'], $this->Streams['oggfile'] ) );
				list( $this->refresh, $this->Streams['tmpfileptr'] ) = sscanf( $this->Streams['tmpfile'],
					$this->Streams['oggfile'] . ".r%d.p%d.tmp" );
			}
		}
	}
...
// Public methods
	...
	) //return $path of picture or false; $frame=frame number; $where=path to file to create, if false will be in cache;
	{
		...
		if ( ! $frame ) {
			...
			return ( $this->creturn( "Unable to extract the frame #" . $framepos . " from "
			                         . $this->Streams['oggfile'] ) );
		}
		$image = $frame->toGDImage();
		if ( ! $image ) {
			error_reporting( $errorreport );

			return ( $this->creturn( "Unable to extract an image in frame #" . $framepos . " from "
			                         . $this->Streams['oggfile'] ) );
		}
		if ( ! imagejpeg( $image, $this->realpath( $file ) ) ) {
			error_reporting( $errorreport );

			return ( $this->creturn( "Unable to save a jpeg image from frame #" . $framepos . " from "
			                         . $this->Streams['oggfile'] ) );
		}
		if ( ! isset( $this->Streams['theora']['pictures'] ) ) {
			$this->Streams['theora']['pictures'] = array();
			...
			if ( $this->Streams['vorbis']['commentpos'] < $this->Streams['theora']['commentpos'] ) {
				...
				if ( $this->Streams['vorbis']['commentnext'] < $this->Streams['theora']['commentpos'] ) {
					fwrite( $this->OutputFile, substr( $this->Data, $this->Streams['vorbis']['commentnext'],
						$this->Streams['theora']['commentpos'] - $this->Streams['vorbis']['commentnext'] ) );
				}
				...
				if ( $this->Streams['theora']['commentnext'] < $this->Streams['vorbis']['commentpos'] ) {
					fwrite( $this->OutputFile, substr( $this->Data, $this->Streams['theora']['commentnext'],
						$this->Streams['vorbis']['commentpos'] - $this->Streams['theora']['commentnext'] ) );
				}
				fwrite( $this->OutputFile, $vorbispacket );
	...
	function ContinueWrite() {
		...
		if ( $interupted ) {
			@rename( $this->realpath( $this->Streams['tmpfile'] ),
				$this->realpath( $this->Streams['oggfile'] . ".r" . $this->refresh . ".p" . $interupted . ".tmp" ) );
...
// Private methods
	...
	function Read32LE( &$buffer, $pos ) {
		return ( ord( $buffer[ $pos + 0 ] ) + ( ord( $buffer[ $pos + 1 ] ) << 8 ) + ( ord( $buffer[ $pos + 2 ] ) << 16 )
		         + ( ord( $buffer[ $pos + 3 ] ) << 24 ) );
	...
	function readBits( $nb, &$buffer, &$bitptr ) //read and decodes an integer up to 32 bits from the buffer
	{
		...
		for ( $bit = $nb, $r = 0; $bit > 0; $bit --, $bitptr ++ ) {
			$r += ( ord( substr( $buffer, $bitptr >> 3, 1 ) ) & pow( 2, 7 - $bitptr % 8 ) ) > 0 ? pow( 2, $bit - 1 )
				: 0;
		}
	...
	function PackComments( $stream ) {
		...
		$data = ( isset( $stream['channels'] ) ? chr( 0x03 ) . "vorbis" : chr( 0x81 ) . "theora" ) . pack( "V",
				strlen( $this->EncodeUTF8( $stream['vendor'] ) ) ) . $this->EncodeUTF8( $stream['vendor'] ) . pack( "V",
				count( $stream['comments'] ) );
		...
		$packet   = "OggS" . pack( "CCVVVVVC", 0, 0, 0, 0, $stream['serial'], 1, 0,
				$segments + strlen( $stream['commentleftSegments'] ) );
		...
		if ( ( $stream['commentpos'] + $stream['commentlen'] ) < $stream['commentnext'] ) {
			$packet .= substr( $this->Data,
				$stream['commentpos'] + ord( $this->Data[ $stream['commentpos'] + 26 ] ) + 27 + $stream['commentlen'],
				$stream['commentnext'] - $stream['commentpos'] - $stream['commentlen'] - 27
				- ord( $this->Data[ $stream['commentpos'] + 26 ] ) );
		}
		$this->crcOgg( $packet );
	...
	function analyze() // Parse headers to retrieve identification and comments infos
	{
		...
		$this->Streams['summary'] = basename( $this->Streams['oggfile'] ) . " (" . floor( $this->Streams['size']
		                                                                                  / 1024 ) . " kB)\n\n";
			...
			// parse OGG pages to retrieve interresting data
		{
			...
			if ( isset( $this->Streams['cmml'] )
			     && $this->Read32LE( $this->Data, $pos + 14 ) == $this->Streams['cmml']['serial']
			) // read CMML data
			{
			...
			if ( $this->Read32LE( $this->Data, $pos + 18 )
			     == 0
			) // Page Count = 0 : we can find the stream type and read identification headers
			{
				if ( ( substr( $this->Data, $packet + 1, 6 ) == "vorbis" )
				     && ! isset( $this->Streams['vorbis'] )
				) // decode vorbis identification header
				{
					...
					if ( ord( $this->Data[ $packet ] ) != 0x01 ) {
						return ( $this->creturn( sprintf( "Incorrect Vorbis Identification Header(0x%x)",
							ord( $this->Data[ $packet ] ) ) ) );
					}
					if ( $this->Read32LE( $this->Data, $packet + 7 ) != 0 ) {
						return ( $this->creturn( "Incorrect Vorbis stream version = " . $this->Read32LE( $this->Data,
								$packet + 7 ) ) );
					}
					if ( ( $vorbis['channels'] = ord( $this->Data[ $packet + 11 ] ) ) == 0 ) {
						return ( $this->creturn( "Incorrect Vorbis channels number = " . ord( $this->Data[ $packet
						                                                                                   + 11 ] ) ) );
					}
					if ( ( $vorbis['samplerate'] = $this->Read32LE( $this->Data, $packet + 12 ) ) == 0 ) {
						return ( $this->creturn( "Incorrect Vorbis sample rate = " . $this->Read32LE( $this->Data,
								$packet + 12 ) ) );
					}
					...
					$vorbis['bitrate']    = ( $vorbis['nombitrate'] != 0 ) ? $vorbis['nombitrate']
						: ( $vorbis['minbitrate'] + $vorbis['maxbitrate'] ) / 2;
				} elseif ( ( substr( $this->Data, $packet + 1, 6 ) == "theora" )
				           && ! isset( $this->Streams['theora'] )
				) // decode theora identification header
				{
					...
					if ( ord( $this->Data[ $packet ] ) != 0x80 ) {
						return ( $this->creturn( sprintf( "Incorrect Theora Identification Header(0x%x)",
							ord( $this->Data[ $packet ] ) ) ) );
					}
					if ( ( ( $theora['vmaj'] = ord( $this->Data[ $packet + 7 ] ) ) != 3 )
					     || ( ( $theora['vmin']
								= ord( $this->Data[ $packet + 8 ] ) ) != 2 )
					) {
				...
				} elseif ( ( substr( $this->Data, $packet, 8 ) == "fishead\0" )
				           && ! isset( $this->Streams['skeleton'] )
				) //decode ogg skeleton primary header
				{
					$this->Streams['skeleton']            = array();
					$this->Streams['skeleton']['version'] = ( ord( $this->Data[ $packet + 8 ] )
					                                          + ( ord( $this->Data[ $packet + 9 ] ) << 8 ) ) . "."
					                                        . ( ord( $this->Data[ $packet + 10 ] )
					                                            + ( ord( $this->Data[ $packet + 11 ] ) << 8 ) );
					if ( ord( $this->Data[ $packet + 44 ] ) > 0 ) {
						$this->Streams['skeleton']['utc'] = substr( $this->Data, $packet + 44, 20 );
					}
				} elseif ( ( substr( $this->Data, $packet, 8 ) == "CMML\0\0\0\0" )
				           && ! isset( $this->Streams['cmml'] )
				) //decode ogg CMML primary header
				{
					$this->Streams['cmml']            = array();
					$this->Streams['cmml']['serial']  = $this->Read32LE( $this->Data, $pos + 14 );
					$this->Streams['cmml']['version'] = ( ord( $this->Data[ $packet + 8 ] )
					                                      + ( ord( $this->Data[ $packet + 9 ] ) << 8 ) ) . "."
					                                    . ( ord( $this->Data[ $packet + 10 ] )
					                                        + ( ord( $this->Data[ $packet + 11 ] ) << 8 ) );
				}
			} elseif ( $this->Read32LE( $this->Data, $pos + 18 ) == 1 ) // Page Count = 1 : we can read comments headers
			{
				...
				if ( $type == "vorbis" ) {
					if ( ord( $this->Data[ $packet ] ) != 0x03 ) {
						return ( $this->creturn( sprintf( "Incorrect Vorbis Comment Header(0x%x)",
							ord( $this->Data[ $packet ] ) ) ) );
					}
					$table =& $this->Streams['vorbis'];
				} elseif ( $type == "theora" ) {
					if ( ord( $this->Data[ $packet ] ) != 0x81 ) {
						return ( $this->creturn( sprintf( "Incorrect Theora Comment Header(0x%x)",
							ord( $this->Data[ $packet ] ) ) ) );
					}
				...
				$table['commentleftSegments'] = substr( $this->Data, $pos + 27 + ( $table['commentlen'] >> 8 ) + 1,
					$lseg );
			}
			if ( isset( $this->Streams['vorbis']['vendor'] ) && isset( $this->Streams['theora']['vendor'] )
			     && ! isset( $this->Streams['cmml'] )
			) {
				break;
		...
		{
			if ( isset( $this->Streams['vorbis'] ) && ( $this->Read32LE( $endbuffer, $pos + 6 ) != - 1 )
			     && ( $this->Read32LE( $endbuffer, $pos + 14 ) == $this->Streams['vorbis']['serial'] )
			     && ( ord( $endbuffer[ $pos + 5 ] ) & 0x4 )
			) {
				$this->Streams['vorbis']['duration'] = round( $this->Read32LE( $endbuffer, $pos + 6 )
				                                              / $this->Streams['vorbis']['samplerate'] );
			} elseif ( isset( $this->Streams['theora'] ) && ( $this->Read32LE( $endbuffer, $pos + 6 ) != - 1 )
			           && ( $this->Read32LE( $endbuffer, $pos + 14 ) == $this->Streams['theora']['serial'] )
			           && ( ord( $endbuffer[ $pos + 5 ] ) & 0x4 )
			) {
				$this->Streams['theora']['framecount'] = ( $this->Read32LE( $endbuffer, $pos + 6 )
				                                           >> $this->Streams['theora']['kfgshift'] )
				                                         + ( $this->Read32LE( $endbuffer, $pos + 6 ) & ( pow( 2,
								$this->Streams['theora']['kfgshift'] ) - 1 ) );
				$this->Streams['theora']['duration']   = round( $this->Streams['theora']['framecount']
				                                                * $this->Streams['theora']['frd']
				                                                / $this->Streams['theora']['frn'] );
			}
		...
		if ( isset( $this->Streams['vorbis']['duration'] ) && isset( $this->Streams['theora']['duration'] ) ) {
			$this->Streams['duration'] = $this->Streams['vorbis']['duration'] > $this->Streams['theora']['duration']
				? $this->Streams['vorbis']['duration'] : $this->Streams['theora']['duration'];
