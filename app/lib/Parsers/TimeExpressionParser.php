...
class TimeExpressionParser
{
     ...
     */
    static public $s_language_settings_list_cache = array();
     ...
     */
    static public $early_mid_late_range_intervals = ['century' => 100, 'decade' => 10];
    static public $early_mid_late_range_lengths = ['century' => 20, 'decade' => 4];
                        ...
                        switch ($va_token['type']) {
                            ...
                            case TEP_TOKEN_ALPHA:
                                ...
                                if (sizeof($va_decade_dates) > 0) { // found decade
                                    ...
                                    if (is_array(
                                            $va_next_token
                                        ) && ($va_next_token['type'] == TEP_TOKEN_RANGE_CONJUNCTION)) { // decade is part of range
                                        $va_decade_dates['start']['year'] = self::applyPartOfRangeQualifier(
                                            $part_of_range_qualifier,
                                            'start',
                                            'decade',
                                        ...
                                        $va_decade_dates['end']['year'] = self::applyPartOfRangeQualifier(
                                            $part_of_range_qualifier,
                                            'end',
                                            'decade',
    ...
    private function preprocess($ps_expression)
    {
        ...
        $va_hook_result = $o_app_plugin_manager->hookTimeExpressionParserPreprocessBefore(
            array("expression" => $ps_expression)
        );
        ...
        $va_era_list = array_merge(
            array_keys($this->opo_language_settings->getAssoc("ADBCTable")),
            array(
        ...
        $va_meridian_list = array_merge(
            array_keys($this->opo_language_settings->getAssoc("meridianTable")),
            array(
        ...
        $ps_expression = preg_replace(
            "/([\d]+)T([\d]+)/i",
            "$1 " . $va_datetime_conjunctions[0] . " $2",
            $ps_expression
        );
        ...
        // Trigger TimeExpressionParser preprocess hook
        $va_hook_result = $o_app_plugin_manager->hookTimeExpressionParserPreprocessAfter(
            array("expression" => $ps_expression)
        );
        if ($va_hook_result["expression"] != $ps_expression) {
            $ps_expression = $va_hook_result["expression"];
                ...
                case TEP_STATE_DATE_ELEMENT_GET_MONTH_NEXT:
                    switch ($va_token['type']) {
                        ...
                        case TEP_TOKEN_ALPHA_MONTH:
                            ...
                            if ($va_peek = $this->peekToken()) {
                                if (($va_peek['type'] == TEP_TOKEN_INTEGER) && (intval(
                                            $va_peek['value']
                                        ) >= 1) && (intval($va_peek['value']) <= 9999)) {
                            ...
                            if (
                                ...
                                if ($va_peek['type'] == TEP_TOKEN_MERIDIAN) {
                                    ...
                                    array_unshift(
                                        $this->opa_tokens,
                                        $va_token['value']
                                    ); # restore first token to token queue
                                    return false;
                                }
                            } else {
                                # is not a time expression; return false but do not set a parse error
    ...
    private function _parseDecade($va_token, $vb_circa_is_set = false)
    {
        ...
        if (sizeof($va_decade_indicators)) {
            if (
                (preg_match(
                    "/^([\d]{2,4})[\']{0,1}(" . join("|", $va_decade_indicators) . "){1}$/i",
                    $va_token['value'],
                    $va_matches
                ))
                ||
    ...
    private function _parseCentury($va_token, $part_of_range_qualifier = null)
    {
        ...
        if (
            ($vn_use_romans && in_array(
                    $vs_next_token_lc,
                    $this->opo_language_settings->getList("centuryIndicator")
                ) && preg_match("/^([MDCLXVI]+)(.*)$/", $va_token['value'], $va_roman_matches))
            ||
            ((in_array($vs_next_token_lc, $this->opo_language_settings->getList("centuryIndicator"))) && (preg_match(
                    "/^([\d]+)(.*)$/",
                    $va_token['value'],
                    $va_matches
                )))
            ...
            if (($vn_century > -100) && ($vn_century < 100)) {
                ...
                if (!isset($va_dates['start']) || !$va_dates['start']) {
                    $vn_start_year = self::applyPartOfRangeQualifier(
                        $part_of_range_qualifier,
                        'start',
                        'century',
                        $vn_start_year
                ...
                if (!$vb_is_range) {
                    $vn_end_year = self::applyPartOfRangeQualifier(
                        $part_of_range_qualifier,
                        'end',
                        'century',
                        $vn_end_year
    ...
    private function &getToken()
    {
        ...
        $va_ordinal_word_alts = array_map(
            function ($v) use ($ordinal_suffix_default) {
        ...
        if (preg_match("/^([\d]{4})#([\d]{2})#([\d]{2})$/", $vs_token, $va_matches)) {
            // year-month-day
            if ((($va_matches[1] >= 1000) && ($va_matches[1] <= 2999)) && (($va_matches[2] >= 1) && ($va_matches[2] <= 12)) && (($va_matches[3] >= 1) && ($va_matches[3] <= $this->daysInMonth(
                            $va_matches[2],
                            $va_matches[1]
                        )))) {
                return (array(
        ...
        if (preg_match("/^([\d]{1,2})#([A-Za-z]{3,15})#([\d]{2,4})$/", $vs_token, $va_matches)) {
            ...
            $vs_m = isset($va_month_table[strtolower($va_matches[2])]) ? $va_month_table[strtolower(
                $va_matches[2]
            )] : strtolower($va_matches[2]);
            $vn_month = array_search($vs_m, $this->getLanguageSettingsWordList('monthList')) + 1;
            if ((($va_matches[3] >= 0) && ($va_matches[3] <= 2999)) && ($vs_m) && (($va_matches[1] >= 1) && ($va_matches[1] <= $this->daysInMonth(
                            $vn_month,
                            $va_matches[3]
                        )))) {
                return (array(
            ...
            case 2:
                ...
                if (($va_tmp[0] >= 1) && ($va_tmp[0] <= 12)) {
                    ...
                    if (is_numeric($va_tmp[1]) && ($va_tmp[1] > 0) && ($va_tmp[1] == intval($va_tmp[1]))) {
                        if (($va_tmp[1] >= 1) && ($va_tmp[1] <= $this->daysInMonth(
                                    $vn_month,
                                    2004
        ...
        switch (sizeof($va_tmp)) {
            case 2:
                if (is_numeric($va_tmp[0]) && ($va_tmp[0] == intval(
                            $va_tmp[0]
                        )) && ($va_tmp[0] >= 0) && ($va_tmp[0] <= 23)) {
                    $vn_hours = intval($va_tmp[0]);
                    $vn_minutes = intval($va_tmp[1]);
                    if (is_numeric($va_tmp[1]) && ($va_tmp[1] == intval(
                                $va_tmp[1]
                            )) && ($va_tmp[1] >= 0) && ($va_tmp[1] <= 59)) {
                        return (array(
                            'value' => $vs_token,
                            'hours' => $vn_hours,
                            'minutes' => $vn_minutes,
            ...
            case 4:
                ...
                $vn_seconds = (int)$va_tmp[2] + (is_numeric($va_tmp[3]) ? (intval($va_tmp[3]) / pow(
                            10,
                            strlen(
                                (intval(
                                    $va_tmp[3]
                ...
                if (is_numeric($vn_hours) && ($vn_hours == intval(
                            $vn_hours
                        )) && ($vn_hours >= 0) && ($vn_hours <= 23)) {
                    if (is_numeric($vn_minutes) && ($vn_minutes == intval(
                                $vn_minutes
                            )) && ($vn_minutes >= 0) && ($vn_minutes <= 59)) {
    ...
    private function _processParseResults($pa_dates, $pa_options)
    {
        ...
        if (!$pa_dates['start']['day'] && !$pa_dates['start']['month'] && !$pa_dates['start']['year']) {
            ...
            $vn_start = (intval($pa_dates['start']['hours']) * 3600) + (intval(
                        $pa_dates['start']['minutes']
                    ) * 60) + intval($pa_dates['start']['seconds']);
            $vn_end = (intval($pa_dates['end']['hours']) * 3600) + (intval($pa_dates['end']['minutes']) * 60) + intval(
            ...
            if (($pa_dates['start']['day'] === null) && ($pa_dates['end']['day'] === null) && ($pa_dates['start']['year'] != TEP_START_OF_UNIVERSE) && $pa_dates['end']['year'] != TEP_END_OF_UNIVERSE) {
                $pa_dates['start']['day'] = 1;
                $pa_dates['end']['day'] = $this->daysInMonth(
                    $pa_dates['end']['month'],
                    $pa_dates['end']['year'] ? $pa_dates['end']['year'] : 2004
                ); // use leap year if no year is defined
            ...
            $vn_start_historic = $pa_dates['start']['year'] . "." . sprintf(
                    '%02d',
                    $pa_dates['start']['month']
                ) . sprintf('%02d', $pa_dates['start']['day']) . sprintf('%02d', $pa_dates['start']['hours']) . sprintf(
                    '%02d',
                    $pa_dates['start']['minutes']
                ) . sprintf('%02d', $pa_dates['start']['seconds']) . sprintf('%01d', $vn_start_attributes) . strlen(
                    $vn_start_uncertainty
                ) . $vn_start_uncertainty;
            ...
            $vn_end_historic = $pa_dates['end']['year'] . "." . sprintf('%02d', $pa_dates['end']['month']) . sprintf(
                    '%02d',
                    $pa_dates['end']['day']
                ) . sprintf('%02d', $pa_dates['end']['hours']) . sprintf('%02d', $pa_dates['end']['minutes']) . sprintf(
                    '%02d',
                    $pa_dates['end']['seconds']
                ) . sprintf('%01d', $vn_end_attributes) . strlen($vn_end_uncertainty) . $vn_end_uncertainty;
    ...
    public function getText($pa_options = null)
    {
        ...
        if (isset($pa_options['rangePreConjunction']) && is_array($va_range_preconjunctions) && in_array(
                $pa_options['rangePreConjunction'],
                $va_range_preconjunctions
            )) {
        ...
        if (isset($pa_options['rangeConjunction']) && is_array($va_range_conjunctions) && in_array(
                $pa_options['rangeConjunction'],
                $va_range_conjunctions
            )) {
            $vs_range_conjunction = $pa_options['rangeConjunction'];
        } else {
            $vs_range_conjunction = $va_range_conjunctions[0];
        }
        if (isset($pa_options['dateTimeConjunction']) && is_array($va_datetime_conjunctions) && in_array(
                $pa_options['dateTimeConjunction'],
                $va_datetime_conjunctions
            )) {
        ...
        if (isset($va_dates['start']) && ($va_dates['start'] != null)) {
            ...
            if (caGetOption('dateFormat', $pa_options) == 'onlyDatesWithHyphens') {
                ...
                $vs_date = $va_start_pieces['year'] . '-' . sprintf('%02d', $va_start_pieces['month']) . '-' . sprintf(
                        '%02d',
                        $va_start_pieces['day']
                    );
                ...
                if (!(
                    ...
                    $vs_date .= '/' . $va_end_pieces['year'] . '-' . sprintf(
                            '%02d',
                            $va_end_pieces['month']
                ...
                for ($vn_i = 0; $vn_i < strlen($pa_options['format']); $vn_i++) {
                    ...
                    switch ($vs_c = $pa_options['format'][$vn_i]) {
                        ...
                        case 'y':
                            ...
                            break;
                        case 'd':
                            $vs_output .= (!$va_seen[$vs_c]) ? sprintf("%02d", $va_start_pieces['day']) : sprintf(
                                "%02d",
                                $va_end_pieces['day']
                            );
                            break;
                        case 'j':
                            ...
                            break;
                        case 'F':
                            $vs_output .= (!$va_seen[$vs_c]) ? $this->getMonthName(
                                $va_start_pieces['month']
                            ) : $this->getMonthName($va_end_pieces['month']);
                            break;
                        case 'm':
                            $vs_output .= (!$va_seen[$vs_c]) ? sprintf("%02d", $va_start_pieces['month']) : sprintf(
                                "%02d",
                                $va_end_pieces['month']
                            );
                            break;
                        case 'n':
                            ...
                            break;
                        case 't':
                            $vs_output .= (!$va_seen[$vs_c]) ? $this->daysInMonth(
                                $va_start_pieces['month'],
                                $va_start_pieces['year']
                        ...
                        case 'h':
                            ...
                            break;
                        case 'H':
                            $vs_output .= (!$va_seen[$vs_c]) ? sprintf("%02d", $va_start_pieces['hours']) : sprintf(
                                "%02d",
                                $va_end_pieces['hours']
                            );
                            break;
                        case 'i':
                            $vs_output .= (!$va_seen[$vs_c]) ? sprintf("%02d", $va_start_pieces['minutes']) : sprintf(
                                "%02d",
                                $va_end_pieces['minutes']
                            );
                            break;
                        case 's':
                            $vs_output .= (!$va_seen[$vs_c]) ? sprintf("%02d", $va_start_pieces['seconds']) : sprintf(
                                "%02d",
            ...
            if ($va_dates['start'] <= TEP_START_OF_UNIVERSE) {
                ...
                if ($pa_options['isLifespan'] && (sizeof($va_died_qualifiers) > 0)) {
                    ...
                    if ($pa_options['beforeQualifier'] && in_array(
                            $pa_options['beforeQualifier'],
                            $va_before_qualifiers
                        )) {
                ...
                if ($va_end_pieces['hours'] == 23 && $va_end_pieces['minutes'] == 59 && $va_end_pieces['seconds'] == 59) {
                    if ($va_end_pieces['day'] == 31 && $va_end_pieces['month'] == 12) {
                        return $vs_before_qualifier . ' ' . $this->_dateToText(
                                array(
            ...
            if ($va_dates['end'] >= TEP_END_OF_UNIVERSE) {
                ...
                if (caGetOption('useConjunctionForAfterDates', $pa_options, false)) {
                    ...
                    if ($pa_options['isLifespan'] && (sizeof($va_born_qualifiers) > 0)) {
                        ...
                        if ($pa_options['afterQualifier'] && in_array(
                                $pa_options['afterQualifier'],
                                $va_after_qualifiers
                            )) {
            ...
            $vs_circa_indicator = ($pa_options['circaIndicator'] && in_array(
                    $pa_options['circaIndicator'],
                    $va_circa_indicators
            ...
            $vs_probably_indicator = ($pa_options['probablyIndicator'] && in_array(
                    $pa_options['probablyIndicator'],
                    $va_probably_indicators
            ...
            if ($va_start_pieces['year'] == $va_end_pieces['year']) {
                if ($va_start_pieces['month'] == $va_end_pieces['month']) {
                    if ($va_start_pieces['day'] == $va_end_pieces['day']) {    // dates on same day
                        ...
                        if (!$vb_full_day_time_range) {
                            ...
                            return $vs_start_circa . $vs_day . ' ' . $vs_datetime_conjunction . ' ' . $this->_timerangeToText(
                                    $vn_start_time,
                                    $vn_end_time,
                                    $pa_options
                                );
                        ...
                        if ($vb_differing_uncertainties) {
                            ...
                            if (
                                ...
                                $va_end_pieces['day'] == $this->daysInMonth(
                                    $va_end_pieces['month'],
                                    $va_end_pieces['year'] ? $va_end_pieces['year'] : 2004
                                )
                            ) {
                                // month and year only
                                return $vs_start_circa . $this->_dateToText(
                                        array(
                                            'month' => $va_start_pieces['month'],
                                            'year' => $va_start_pieces['year'],
                                            'era' => $va_start_pieces['era'],
                                ...
                                if ($vb_full_day_time_range) {
                                    // days, but no times
                                    if (is_null(
                                        $vb_month_comes_first = $this->opo_datetime_settings->get(
                                            'monthComesFirstInDelimitedDate'
                                        )
                                    )) {
                                        $vb_month_comes_first = $this->opo_language_settings->get(
                                            'monthComesFirstInDelimitedDate'
                                        );
                                    }
                                    if ($vb_month_comes_first) {
                                        ...
                                        $vs_start_date = $this->_dateToText(
                                            array(
                                                'month' => $va_start_pieces['month'],
                                                'day' => $va_start_pieces['day']
                                            ),
                                        ...
                                        $vs_end_date = $this->_dateToText(
                                            array('day' => $va_end_pieces['day']),
                                            $pa_options
                                        );
                                    } else {
                                        $vs_start_date = $this->_dateToText(
                                            array('day' => $va_start_pieces['day']),
                                            $pa_options
                                        );
                                        $vs_end_date = $this->_dateToText(
                                            array('month' => $va_start_pieces['month'], 'day' => $va_end_pieces['day']),
                                            $pa_options
                                        );
                                    }

                                    $vs_year = $this->_dateToText(
                                        array(
                                            'year' => $va_start_pieces['year'],
                                            'era' => $va_start_pieces['era'],
                                            'uncertainty' => $va_start_pieces['uncertainty'],
                                    ...
                                    // days with times
                                    $vs_start_date = $this->_datetimeToText(
                                        array(
                                            'month' => $va_start_pieces['month'],
                                            'day' => $va_start_pieces['day'],
                                    ...
                                    $vs_end_date = $this->_datetimeToText(
                                        array(
                    ...
                    if ($vb_differing_uncertainties) {
                        ...
                        if (
                            ...
                            if ($vb_full_day_time_range) {
                                ...
                                list($va_start_pieces, $va_end_pieces) = $this->_nullDaysAndMonthsForDisplay(
                                    $va_start_pieces,
                                    $va_end_pieces
                                );
                                ...
                                if (caGetOption('dateFormat', $pa_options, null) == 'text') {
                                    $vs_start_date = $this->_dateToText(
                                        array('month' => $va_start_pieces['month'], 'day' => $va_start_pieces['day']),
                                        $pa_options
                                    );
                                    if ((bool)$pa_options['showCommaAfterDayForTextDates'] || (bool)$this->opo_datetime_settings->get(
                                            'forceCommaAfterDay'
                                        )) {
                                        $pa_options['forceCommaAfterDay'] = true;
                                    }
                                    $vs_end_date = $this->_dateToText(
                                        array('month' => $va_end_pieces['month'], 'day' => $va_end_pieces['day']),
                                        $pa_options
                                    );
                                    $vs_year = $this->_dateToText(
                                        array(
                ...
                if (!$this->opo_datetime_settings->get('dontInferQualifiedRanges') && is_array(
                        $qualified_range_info = self::inferRangeQualifier(
                            ['start' => $va_start_pieces, 'end' => $va_end_pieces],
                            $pa_options
                        )
                    )) {
                ...
                if (
                    ...
                    // catch decade dates
                    $vs_start_year = $this->_dateToText(
                        array(
                            'year' => $va_start_pieces['year'],
                            'era' => $va_start_pieces['era'],
                            'uncertainty' => $va_start_pieces['uncertainty'],
    ...
    {
        if (((int)$pa_start_pieces['day'] === 1) && ((int)$pa_end_pieces['day'] === (int)$this->daysInMonth(
                    $pa_end_pieces['month'],
                    $pa_end_pieces['year']
                ))) {
        ...
        if (
            (((int)$pa_end_pieces['day'] === (int)$this->daysInMonth(
                        12,
                        $pa_end_pieces['year']
        ...
        if (($pa_start_pieces['day'] === null) && ((int)$pa_end_pieces['day'] === (int)$this->daysInMonth(
                    $pa_end_pieces['month'],
                    $pa_end_pieces['year']
                ))) {
    ...
    { // pn_start and pn_end are both number of seconds since midnight
        ...
        if ($pn_start != $pn_end) {
            ...
            if (isset($pa_options['timeRangeConjunction']) && (in_array(
                    $pa_options['timeRangeConjunction'],
                    $va_tmp
                ))) {
    ...
    private function _dateToText($pa_date_pieces, $pa_options = null)
    {
        ...
        if ($pa_options['uncertaintyIndicator'] && in_array(
                $pa_options['uncertaintyIndicator'],
                $va_uncertainty_indicators
            )) {
        ...
        if (($vs_day > 0) && ($pa_options['dateFormat'] != 'delimited') && ($vs_day_suffix = $this->opo_language_settings->get(
                'daySuffix'
            ))) {
            // add day suffix
    ...
    public function setLanguage($ps_iso_code)
    {
        if (file_exists(__CA_LIB_DIR__ . '/Parsers/TimeExpressionParser/' . $ps_iso_code . '.lang')) {
            ...
            $this->opo_language_settings = Configuration::load(
                __CA_LIB_DIR__ . '/Parsers/TimeExpressionParser/' . $ps_iso_code . '.lang'
            );
    ...
    public function unixToHistoricTimestamp($pn_unix_timestamp)
    {
        ...
        return $va_date_info['year'] . "." . sprintf('%02d', $va_date_info['mon']) . sprintf(
                '%02d',
                $va_date_info['mday']
            ) . sprintf('%02d', $va_date_info['hours']) . sprintf('%02d', $va_date_info['minutes']) . sprintf(
                '%02d',
                $va_date_info['seconds']
            ) . '00';
    }
    ...
    public function getISODateTime($pa_date, $ps_mode = 'START', $pa_options = null)
    {
        ...
        if ($ps_mode == 'FULL') {
            $vs_date = $pa_date['year'] . '-' . sprintf("%02d", $pa_date['month']) . '-' . sprintf(
                    "%02d",
                    $pa_date['day']
                );

            if (!isset($pa_options['timeOmit']) || !$pa_options['timeOmit']) {
                $vs_date .= 'T' . sprintf("%02d", $pa_date['hours']) . ':' . sprintf(
                        "%02d",
                        $pa_date['minutes']
        ...
        if (!isset($pa_options['timeOmit']) || !$pa_options['timeOmit']) {
            if (
                ...
                $vs_time .= 'T' . sprintf("%02d", $pa_date['hours']) . ':' . sprintf(
                        "%02d",
                        $pa_date['minutes']
                    ) . ':' . sprintf("%02d", $pa_date['seconds']) . 'Z';
            }
            ...
            case 'centuries':
                ...
                if ($vn_s <= $vn_e) {
                    ...
                    for ($vn_y = $vn_s; $vn_y <= $vn_e; $vn_y += 100) {
                        ...
                        $va_values[(int)$vn_y] = ($vn_century_num) . $vs_ordinal_to_display . ' ' . $va_century_indicators[0] . ((floor(
                                    $vn_y / 100
                                ) < 0) ? ' ' . $vs_bc_indicator : '');
                    }
                }
    ...
    public static function inferRangeQualifier($dates, $options = null)
    {
        ...
        // Mid century
        if (((($start_pieces['year'] - floor(self::$early_mid_late_range_intervals['century'] / 2) + floor(
                            self::$early_mid_late_range_lengths['century'] / 2
                        )) % 100) == 0) && ($end_pieces['year'] == ($start_pieces['year'] + self::$early_mid_late_range_lengths['century']))) {
            return [
        ...
        // Mid decade
        if (!$options['isSpan'] && ((($start_pieces['year'] - floor(
                            self::$early_mid_late_range_intervals['decade'] / 2
                        ) + floor(
                            self::$early_mid_late_range_lengths['decade'] / 2
                        )) % 10) == 0) && ($end_pieces['year'] == ($start_pieces['year'] + self::$early_mid_late_range_lengths['decade']))) {
            return [
                'qualifier' => TEP_TOKEN_MID,
                'range_type' => 'decade',
        ...
        if ((($end_pieces['year'] - ($end_pieces['year'] % 100)) - ($start_pieces['year'] - ($start_pieces['year'] % 100))) >= 100) {
            ...
            if (($start_pieces['year'] % 100) == 0) { // early
                ...
                $first_century_info = self::inferRangeQualifier(
                    ['start' => $start_pieces, 'end' => $mod_end],
                    $options
                );
            ...
            } elseif ((($start_pieces['year'] + floor(self::$early_mid_late_range_lengths['century'] / 2) - floor(
                            self::$early_mid_late_range_intervals['century'] / 2
                        )) % 100) == 0) { // mid
                $mod_end['year'] = $start_pieces['year'] + self::$early_mid_late_range_lengths['century'];
                $first_century_info = self::inferRangeQualifier(
                    ['start' => $start_pieces, 'end' => $mod_end],
            ...
            if (($end_pieces['year'] % 100) == self::$early_mid_late_range_lengths['century']) { // early
                ...
                $second_century_info = self::inferRangeQualifier(
                    ['start' => $mod_start, 'end' => $end_pieces],
                    $options
                );
            ...
            } elseif ((($end_pieces['year'] - floor(self::$early_mid_late_range_lengths['century'] / 2) - floor(
                            self::$early_mid_late_range_intervals['century'] / 2
                        )) % 100) == 0) { // mid
                $mod_start['year'] = $end_pieces['year'] - ($end_pieces['year'] % 100) - floor(
                        self::$early_mid_late_range_lengths['century'] / 2
            ...
            if (is_array($first_century_info) && is_array(
                    $second_century_info
                ) && !(($first_century_info['qualifier'] == TEP_TOKEN_EARLY) && ($second_century_info['qualifier'] == TEP_TOKEN_LATE))) {
                return [
                    'qualifier' => null,
                    'range_type' => 'century',
        ...
        if ((($end_pieces['year'] - ($end_pieces['year'] % 10)) - ($start_pieces['year'] - ($start_pieces['year'] % 10))) >= 10) {
            ...
            if (($start_pieces['year'] % 10) == 0) { // early
                ...
                $first_decade_info = self::inferRangeQualifier(['start' => $start_pieces, 'end' => $mod_end], $options);
            } elseif ((($start_pieces['year'] + floor(self::$early_mid_late_range_lengths['decade'] / 2) - floor(
                            self::$early_mid_late_range_intervals['decade'] / 2
                        )) % 10) == 0) { // mid
                $mod_end['year'] = $start_pieces['year'] + self::$early_mid_late_range_lengths['decade'];
                $first_decade_info = self::inferRangeQualifier(['start' => $start_pieces, 'end' => $mod_end], $options);
            ...
            if (($end_pieces['year'] % 10) == self::$early_mid_late_range_lengths['decade']) { // early
                ...
                $second_decade_info = self::inferRangeQualifier(
                    ['start' => $mod_start, 'end' => $end_pieces],
                    $options
                );
            ...
            } elseif ((($end_pieces['year'] - floor(self::$early_mid_late_range_lengths['decade'] / 2) - floor(
                            self::$early_mid_late_range_intervals['decade'] / 2
                        )) % 10) == 0) { // mid
                $mod_start['year'] = $end_pieces['year'] - ($end_pieces['year'] % 10) - floor(
                        self::$early_mid_late_range_lengths['decade'] / 2
            ...
            if (is_array($first_decade_info) && is_array(
                    $second_decade_info
                ) && !(($first_decade_info['qualifier'] == TEP_TOKEN_EARLY) && ($second_decade_info['qualifier'] == TEP_TOKEN_LATE))) {
                return [
                    'qualifier' => null,
                    'range_type' => 'decade',
