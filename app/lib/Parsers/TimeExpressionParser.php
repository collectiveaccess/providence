...
class TimeExpressionParser
{
                            ...
                            case TEP_TOKEN_ALPHA:
                                ...
                                if (
                                    ($vn_use_romans && in_array(
                                            $vs_next_token_lc,
                                            $this->opo_language_settings->getList(
                                                "centuryIndicator"
                                            )
        ...
        foreach (['dateCircaIndicator', 'beforeQualifier', 'afterQualifier'] as $l) {
            if ($keywords = $this->getLanguageSettingsWordList($l)) {
                ...
                foreach ($keywords as $c) {
                    if (!preg_match('!' . preg_quote($c, '!') . '([\-\d]+)!i', $ps_expression, $m)) {
                        continue;
                    }
    ...
    private function &getToken()
    {
        ...
        switch (sizeof($va_tmp)) {
            ...
            case 4:
                $vn_hours = $va_tmp[0];
                $vn_minutes = $va_tmp[1];
                $vn_seconds = $va_tmp[2] + (intval($va_tmp[3]) / pow(10, strlen((intval($va_tmp[3])))));
    ...
    public static function inferRangeQualifier($dates, $options = null)
    {
        ...
        // Mid decade
        if (((($start_pieces['year'] - floor(self::$early_mid_late_range_intervals['decade'] / 2) + floor(
