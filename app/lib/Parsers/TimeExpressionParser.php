...
class TimeExpressionParser {
	...
	public function __construct( $ps_expression = null, $ps_iso_code = null, $pb_debug = false ) {
		...
		$this->opa_error_messages = array(
			_t( "No error" ),
			_t( "Start must be before date in range" ),
			_t( "Invalid date" ),
			_t( "Invalid time" ),
			_t( "Invalid uncertainty" ),
			_t( "Uncertainty must not exceed 9 digits" ),
			_t( "Invalid expression" ),
			_t( "Trailing characters in otherwise valid expression" ),
			_t( "Parser error" )
		);
						...
						case TEP_TOKEN_INTEGER:
							...
							if ( ( (int) $va_token['value'] > 0 ) && ( (int) $va_token['value'] <= 21 ) ) {
								...
								if ( $va_peek['type'] == TEP_TOKEN_ALPHA ) {
									if ( preg_match( '!^Q([\d]{1})$!i', $va_peek['value'], $va_matches ) ) {
										...
										if ( ( $vn_q >= 1 ) && ( $vn_q <= 4 ) ) {
											$vn_start_year     = ( ( (int) $va_token['value'] - 1 ) * 100 ) + ( ( $vn_q
											                                                                      - 1 )
											                                                                    * 25 );
											$vn_end_year       = ( ( (int) $va_token['value'] - 1 ) * 100 )
											                     + ( ( $vn_q ) * 25 );
											$va_dates['start'] = array(
												'month'             => 1,
												'day'               => 1,
							...
							if ( $va_peek['type'] == TEP_TOKEN_MYA ) {
								$va_dates['start'] = array(
									'month'             => 1,
									'day'               => 1,
									'year'              => intval( $va_token['value'] ) * - 1000000,
									'hours'             => null,
									'minutes'           => null,
									'seconds'           => null,
									'uncertainty'       => false,
								...
								$va_dates['start'] = array(
									'month'             => 1,
									'day'               => 1,
									'year'              => 1950 - intval( $va_token['value'] ),
									'hours'             => null,
									'minutes'           => null,
									'seconds'           => null,
									'uncertainty'       => false,
						...
						switch ( $va_token['type'] ) {
							...
							case TEP_TOKEN_SEASON_AUTUMN:
								...
								$va_dates['end']   = array(
									'month'             => $vn_end_month,
									'day'               => 20,
									'year'              => $vn_start_year + $vn_year_offset,
									'uncertainty'       => false,
									'uncertainty_units' => '',
									'is_circa'          => false,
									'is_probably'       => false
							...
							case TEP_TOKEN_ALPHA:
								...
								if ( sizeof( $va_decade_dates ) > 0 ) { // found decade
									...
									if ( is_array( $va_next_token )
									     && ( $va_next_token['type'] == TEP_TOKEN_RANGE_CONJUNCTION )
									) { // decade is part of range
										$va_decade_dates['start']['year']
											               = self::applyPartOfRangeQualifier( $part_of_range_qualifier,
											'start', 'decade', $va_decade_dates['start']['year'] );
										...
										$va_decade_dates['start']['year']
											= self::applyPartOfRangeQualifier( $part_of_range_qualifier, 'start',
											'decade', $va_decade_dates['start']['year'] );
										$va_decade_dates['end']['year']
											= self::applyPartOfRangeQualifier( $part_of_range_qualifier, 'end',
											'decade', $va_decade_dates['end']['year'] );
							...
							case TEP_TOKEN_CIRCA:
								...
								if ( $va_date_element = $this->_parseDateElement() ) {
									...
									$va_dates['start'] = array(
										'month'             => $va_date_element['month'],
										'day'               => $va_date_element['day'],
	...
	private function preprocess( $ps_expression ) {
		...
		$va_hook_result
		                      = $o_app_plugin_manager->hookTimeExpressionParserPreprocessBefore( array( "expression" => $ps_expression ) );
		...
		foreach ( [ 'dateCircaIndicator', 'beforeQualifier', 'afterQualifier' ] as $l ) {
			if ( $keywords = $this->getLanguageSettingsWordList( $l ) ) {
				...
				foreach ( $keywords as $c ) {
					$ps_expression = preg_replace( '!(?<=^|[\b]{1})(' . preg_quote( $c, '!' ) . ')[-–]+!i', "$1 ",
						$ps_expression );
					if ( ! preg_match( '!(^|[^A-Za-z]+)' . preg_quote( $c, '!' ) . '([\-\d]+)!i', $ps_expression,
						$m )
					) {
						continue;
					}
					$ps_expression = preg_replace( '!(^|[^A-Za-z]+)' . preg_quote( $c, '!' ) . '([\-]*)!i',
						"$1 {$c} $2", $ps_expression );
					break;
				}
			}
		...
		if ( preg_match( "/([\d]{4})-([\d]{2})(\/|$)/", $ps_expression, $va_matches ) ) {
			if ( intval( $va_matches[2] ) > 12 ) {
				$ps_expression = preg_replace( "/([\d]{4})-([\d]{2})(\/|$)/",
					"$1-" . substr( $va_matches[1], 0, 2 ) . "$2$3", $ps_expression );
			} else {
		...
		$va_era_list = array_merge( array_keys( $this->opo_language_settings->getAssoc( "ADBCTable" ) ), array(
			$this->opo_language_settings->get( "dateADIndicator" ),
			$this->opo_language_settings->get( "dateBCIndicator" )
		) );
		foreach ( $va_era_list as $vs_era ) {
			$ps_expression = preg_replace( "/([\d]+)" . $vs_era . "[ ]*/i", "$1 $vs_era ",
				$ps_expression ); #str_replace($vs_era, " ".$vs_era, $ps_expression);
		}

		$va_meridian_list = array_merge( array_keys( $this->opo_language_settings->getAssoc( "meridianTable" ) ), array(
			$this->opo_language_settings->get( "timeAMMeridian" ),
			$this->opo_language_settings->get( "timePMMeridian" )
		) );
		foreach ( $va_meridian_list as $vs_meridian ) {
			$ps_expression = preg_replace( "/([\d]+)" . $vs_meridian . "[ ]*/i", "$1 $vs_meridian ",
				$ps_expression ); #str_replace($vs_meridian, " ".$vs_meridian, $ps_expression);
		}
		...
		$ps_expression            = preg_replace( "/([\d]+)T([\d]+)/i", "$1 " . $va_datetime_conjunctions[0] . " $2",
			$ps_expression );
		...
		$ps_expression = preg_replace( "/([\d]{4}#[\d]{2}#[\d]{2})\/([\d]{4}#[\d]{2}#[\d]{2})/", "$1 - $2",
			$ps_expression );

		// Trigger TimeExpressionParser preprocess hook
		$va_hook_result
			= $o_app_plugin_manager->hookTimeExpressionParserPreprocessAfter( array( "expression" => $ps_expression ) );
		if ( $va_hook_result["expression"] != $ps_expression ) {
			$ps_expression = $va_hook_result["expression"];
		}
	...
	private function &_parseDateElement( $pa_options = null ) {
		...
		if ( is_null(
			$vb_month_comes_first = $this->opo_datetime_settings->get( 'monthComesFirstInDelimitedDate' ) )
		) {
				...
				case TEP_STATE_DATE_ELEMENT_GET_MONTH_NEXT:
					switch ( $va_token['type'] ) {
						...
						case TEP_TOKEN_ALPHA_MONTH:
							...
							if ( $va_peek = $this->peekToken() ) {
								if ( ( $va_peek['type'] == TEP_TOKEN_INTEGER ) && ( intval( $va_peek['value'] ) >= 1 )
								     && ( intval( $va_peek['value'] ) <= 9999 )
								) {
						...
						# ----------------------
						default:
							if ( isset( $pa_options['start'] ) && isset( $pa_options['start']['month'] )
							     && $pa_options['start']['month']
							) {
		...
		if ( $vn_day && isset( $pa_options['start'] ) && isset( $pa_options['start']['month'] )
		     && $pa_options['start']['month']
		) {
			$vn_month = $pa_options['start']['month'];
				...
				case TEP_STATE_BEGIN_DATE_EXPRESSION:
					if ( $va_time = $this->_parseTimeExpression() ) {
						$va_date = array(
							...
							'year'              => null,
							'hours'             => $va_time['hours'],
							'minutes'           => $va_time['minutes'],
							'seconds'           => $va_time['seconds'],
							'uncertainty'       => false,
							'uncertainty_units' => '',
							'is_circa'          => false,
							'is_probably'       => false
						...
						switch ( $va_token['type'] ) {
							...
							case TEP_TOKEN_NOW:
								...
								$va_date = array(
									...
									'year'              => $va_now['year'],
									'hours'             => $va_now['hours'],
									'minutes'           => $va_now['minutes'],
									'seconds'           => $va_now['seconds'],
									'uncertainty'       => false,
									'uncertainty_units' => '',
									'is_circa'          => false,
									'is_probably'       => false
							...
							case TEP_TOKEN_YESTERDAY:
								...
								$va_date      = array(
									'month'             => $va_yesterday['mon'],
									'day'               => $va_yesterday['mday'],
									'year'              => $va_yesterday['year'],
									'hours'             => 0,
									'minutes'           => 0,
									'seconds'           => 0,
									'uncertainty'       => false,
							...
							case TEP_TOKEN_TOMORROW:
								...
								$va_date     = array(
									'month'             => $va_tomorrow['mon'],
									'day'               => $va_tomorrow['mday'],
									'year'              => $va_tomorrow['year'],
									'hours'             => 0,
									'minutes'           => 0,
									'seconds'           => 0,
									'uncertainty'       => false,
							...
							default:
								if ( $va_date_element = $this->_parseDateElement( $pa_options ) ) {
									$va_date = array(
										...
										'uncertainty'       => false,
										'uncertainty_units' => '',
										'is_circa'          => $va_date_element['is_circa'],
										'is_probably'       => $va_date_element['is_probably']
									);
						...
						case TEP_TOKEN_INTEGER:
							if (
								...
								if ( $va_peek['type'] == TEP_TOKEN_MERIDIAN ) {
									...
									array_unshift( $this->opa_tokens,
										$va_token['value'] ); # restore first token to token queue
	...
	private function _parseDecade( $va_token, $vb_circa_is_set = false ) {
		...
		if ( sizeof( $va_decade_indicators ) ) {
			if (
				( preg_match( "/^([\d]{2,4})[\']{0,1}(" . join( "|", $va_decade_indicators ) . "){1}$/i",
					$va_token['value'], $va_matches ) )
				...
				$va_dates['start'] = array(
					...
					'uncertainty'       => false,
					'uncertainty_units' => '',
					'is_circa'          => $vn_is_circa,
					'is_probably'       => false
				);
				$va_dates['end']   = array(
					'month'             => 12,
					'day'               => 31,
					'year'              => $vb_is_bc ? ( $vn_start_year - 9 ) : ( $vn_start_year + 9 ),
					'uncertainty'       => false,
					'uncertainty_units' => '',
					'is_circa'          => $vn_is_circa,
					'is_probably'       => false
	...
	private function _parseCentury( $va_token, $part_of_range_qualifier = null ) {
		...
		if (
			( $vn_use_romans
			  && in_array( $vs_next_token_lc, $this->opo_language_settings->getList( "centuryIndicator" ) )
			  && preg_match( "/^([MDCLXVI]+)(.*)$/", $va_token['value'], $va_roman_matches ) )
			|| ( ( in_array( $vs_next_token_lc, $this->opo_language_settings->getList( "centuryIndicator" ) ) )
			     && ( preg_match( "/^([\d]+)(.*)$/", $va_token['value'], $va_matches ) ) )
			...
			if ( $va_modfier_token = ( is_array( $va_next_token ) ? $va_next_token : $this->peekToken() ) ) {
				...
				switch ( $va_modfier_token['type'] ) {
					case TEP_TOKEN_ERA:
						...
						if ( is_array( $va_next_next_token )
						     && ( $va_next_next_token['type'] == TEP_TOKEN_RANGE_CONJUNCTION )
						) {
							$vb_is_range = true;
							$this->skipToken();
						}
			...
			if ( ( $vn_century > - 100 ) && ( $vn_century < 100 ) ) {
				...
				if ( ! isset( $va_dates['start'] ) || ! $va_dates['start'] ) {
					$vn_start_year     = self::applyPartOfRangeQualifier( $part_of_range_qualifier, 'start', 'century',
						$vn_start_year );
					$va_dates['start'] = array(
						'month'             => 1,
						'day'               => 1,
				...
				if ( ! $vb_is_range ) {
					$vn_end_year     = self::applyPartOfRangeQualifier( $part_of_range_qualifier, 'end', 'century',
						$vn_end_year );
					$va_dates['end'] = array(
						'month'             => 12,
						'day'               => 31,
	...
	private function &getToken() {
		...
		foreach ( $this->getLanguageSettingsWordList( "rangeConjunctions" ) as $vs_conjunction ) {
			if ( preg_match( "!^" . preg_quote( $vs_token_lc, '!' ) . "!", $vs_conjunction ) ) {
				...
				if ( $vb_is_match ) {
					...
					return array(
						'value' => join( ' ', array_merge( [ $vs_token_lc ], $va_pieces ) ),
						'type'  => TEP_TOKEN_RANGE_CONJUNCTION
					);
				}
			}
		}
		...
		foreach ( $va_ordinals as $vs_ordinal ) {
			...
			if ( substr( $vs_token_lc, 0 - strlen( $vs_ordinal ) ) == $vs_ordinal ) {
				...
				if ( preg_match( "/^\d+$/", $vn_cent ) ) { // could use is_numeric here but this seems safer
					...
					if ( $va_next_tok['type']
					     == TEP_TOKEN_ALPHA
					) { // must not be TEP_TOKEN_ALPHA_MONTH, as in 28. Januar 1985
		...
		if ( preg_match( "/^([\d]{2}):([\d]{2}):([\d]{2}[\.]{0,1}[\d]*)$/", $vs_token, $va_matches ) ) {
			// year-month
			if ( ( ( $va_matches[1] >= 0 ) && ( $va_matches[1] <= 23 ) )
			     && ( ( $va_matches[2] >= 0 )
			          && ( $va_matches[2] <= 59 ) )
			     && ( ( $va_matches[3] >= 0 ) && ( $va_matches[3] < 60 ) )
			) {
				return ( array(
					'value'   => $vs_token,
					'minutes' => $va_matches[2],
					'seconds' => floor( $va_matches[3] ),
					'hours'   => $va_matches[1],
					'type'    => TEP_TOKEN_TIME
				) );
			}
		...
		if ( preg_match( "/^([\d]{4})#([\d]{2})$/", $vs_token, $va_matches ) ) {
			// year-month
			if ( ( ( $va_matches[1] >= 1000 ) && ( $va_matches[1] <= 2999 ) )
			     && ( ( $va_matches[2] >= 1 )
			          && ( $va_matches[2] <= 12 ) )
			) {
				return ( array(
		...
		if ( preg_match( "/^([\d]{4})#([\d]{2})#([\d]{2})$/", $vs_token, $va_matches ) ) {
			...
			if ( ( ( $va_matches[1] >= 1000 ) && ( $va_matches[1] <= 2999 ) )
			     && ( ( $va_matches[2] >= 1 )
			          && ( $va_matches[2] <= 12 ) )
			     && ( ( $va_matches[3] >= 1 )
			          && ( $va_matches[3] <= $this->daysInMonth( $va_matches[2], $va_matches[1] ) ) )
			) {
				return ( array(
					'value' => $vs_token,
					'month' => $va_matches[2],
		...
		if ( preg_match( "/^([\d]{1,2})#([A-Za-z]{3,15})#([\d]{2,4})$/", $vs_token, $va_matches ) ) {
			...
			$vs_m     = isset( $va_month_table[ strtolower( $va_matches[2] ) ] )
				? $va_month_table[ strtolower( $va_matches[2] ) ] : strtolower( $va_matches[2] );
			$vn_month = array_search( $vs_m, $this->getLanguageSettingsWordList( 'monthList' ) ) + 1;
			if ( ( ( $va_matches[3] >= 0 ) && ( $va_matches[3] <= 2999 ) ) && ( $vs_m )
			     && ( ( $va_matches[1] >= 1 )
			          && ( $va_matches[1] <= $this->daysInMonth( $vn_month, $va_matches[3] ) ) )
			) {
				return ( array(
					'value' => $vs_token,
					'month' => $vn_month,
		...
		switch ( sizeof( $va_tmp ) ) {
			case 2:
				...
				if ( ( $va_tmp[0] >= 1 ) && ( $va_tmp[0] <= 12 ) ) {
					...
					if ( is_numeric( $va_tmp[1] ) && ( $va_tmp[1] > 0 ) && ( $va_tmp[1] == intval( $va_tmp[1] ) ) ) {
						if ( ( $va_tmp[1] >= 1 )
						     && ( $va_tmp[1] <= $this->daysInMonth( $vn_month, 2004 ) )
						) {        // since year is unspecified we use a leap year
							// got day
							$vn_day = $va_tmp[1];
						} else {
							$vn_year = $va_tmp[1];
			...
			case 3:
				...
				if ( ( $vn_month >= 1 ) && ( $vn_month <= 12 ) ) {
					if ( ( $vn_day >= 1 )
					     && ( $vn_day <= $this->daysInMonth( $vn_month, $vn_year ? $vn_year : 2004 ) )
					) {
						if ( $vn_year > 0 ) {
							return ( array(
								'value' => $vs_token,
		...
		switch ( sizeof( $va_tmp ) ) {
			case 2:
				if ( is_numeric( $va_tmp[0] ) && ( $va_tmp[0] == intval( $va_tmp[0] ) ) && ( $va_tmp[0] >= 0 )
				     && ( $va_tmp[0] <= 23 )
				) {
					$vn_hours   = intval( $va_tmp[0] );
					$vn_minutes = intval( $va_tmp[1] );
					if ( is_numeric( $va_tmp[1] ) && ( $va_tmp[1] == intval( $va_tmp[1] ) ) && ( $va_tmp[1] >= 0 )
					     && ( $va_tmp[1] <= 59 )
					) {
						return ( array(
							'value'      => $vs_token,
			...
			case 4:
				...
				$vn_seconds = (int) $va_tmp[2] + ( is_numeric( $va_tmp[3] ) ? ( intval( $va_tmp[3] ) / pow( 10,
							strlen( ( intval( $va_tmp[3] ) ) ) ) ) : 0 );
				if ( is_numeric( $vn_hours ) && ( $vn_hours == intval( $vn_hours ) ) && ( $vn_hours >= 0 )
				     && ( $vn_hours <= 23 )
				) {
					if ( is_numeric( $vn_minutes ) && ( $vn_minutes == intval( $vn_minutes ) ) && ( $vn_minutes >= 0 )
					     && ( $vn_minutes <= 59 )
					) {
	...
	private function _processParseResults( $pa_dates, $pa_options ) {
		...
		if ( ! is_array( $pa_dates['end'] ) ) {
			if ( ( $pa_dates['start']['hours'] == 0 ) && ( $pa_dates['start']['minutes'] == 0 )
		...
		if ( ! $pa_dates['start']['day'] && ! $pa_dates['start']['month'] && ! $pa_dates['start']['year'] ) {
			...
			$vn_start = ( intval( $pa_dates['start']['hours'] ) * 3600 ) + ( intval( $pa_dates['start']['minutes'] )
			                                                                 * 60 )
			            + intval( $pa_dates['start']['seconds'] );
			$vn_end   = ( intval( $pa_dates['end']['hours'] ) * 3600 ) + ( intval( $pa_dates['end']['minutes'] ) * 60 )
			            + intval( $pa_dates['end']['seconds'] );
			...
			if ( ( $pa_options['mode'] == 'date' )
			     && ( ( isset( $pa_dates['start']['hours'] ) )
			          || ( isset( $pa_dates['end']['hours'] ) ) )
			) {    // don't parse date expressions with time components
			...
			if (
				...
				&& ( ! $pa_dates['start']['era'] && ( $pa_dates['start']['year'] > 0 )
				     && ( $pa_dates['start']['year'] <= 99 ) )
			) {
				$va_tmp             = $this->gmgetdate();
				$vn_current_year    = intval( substr( $va_tmp['year'], 2,
					2 ) );        // get last two digits of current year
			...
			if (
				...
				&& ( ! isset( $pa_dates['end']['era'] ) && ( $pa_dates['end']['year'] > 0 )
				     && ( $pa_dates['end']['year'] <= 99 ) )
			) {
				$va_tmp             = $this->gmgetdate();
				$vn_current_year    = intval( substr( $va_tmp['year'], 2,
					2 ) );        // get last two digits of current year
			...
			if ( ( $pa_dates['start']['year'] === TEP_START_OF_UNIVERSE )
			     && ( $pa_dates['end']['year'] !== TEP_END_OF_UNIVERSE )
			) {
				if ( $pa_dates['end']['month'] === null ) {
			...
			if ( ( $pa_dates['start']['month'] === null ) && ( $pa_dates['end']['month'] === null )
			     && ( $pa_dates['start']['year'] != TEP_START_OF_UNIVERSE )
			     && ( $pa_dates['end']['year'] != TEP_END_OF_UNIVERSE )
			) {
				$pa_dates['start']['month'] = 1;
			...
			if ( ( $pa_dates['start']['day'] === null ) && ( $pa_dates['end']['day'] === null )
			     && ( $pa_dates['start']['year'] != TEP_START_OF_UNIVERSE )
			     && $pa_dates['end']['year'] != TEP_END_OF_UNIVERSE
			) {
				$pa_dates['start']['day'] = 1;
				$pa_dates['end']['day']   = $this->daysInMonth( $pa_dates['end']['month'],
					$pa_dates['end']['year'] ? $pa_dates['end']['year'] : 2004 ); // use leap year if no year is defined
			} elseif ( ( $pa_dates['end']['day'] === null ) && ( $pa_dates['end']['year'] != TEP_END_OF_UNIVERSE )
			           && ( $pa_dates['start']['year'] != TEP_START_OF_UNIVERSE )
			) {
			...
			if (
				...
				&& ( $pa_dates['start']['year'] <= 2037 )
				&& ( $pa_dates['end']['year'] <= 2037 )
			) {
			...
			$vn_start_historic = $pa_dates['start']['year'] . "." . sprintf( '%02d', $pa_dates['start']['month'] )
			                     . sprintf( '%02d', $pa_dates['start']['day'] ) . sprintf( '%02d',
					$pa_dates['start']['hours'] ) . sprintf( '%02d', $pa_dates['start']['minutes'] ) . sprintf( '%02d',
					$pa_dates['start']['seconds'] ) . sprintf( '%01d', $vn_start_attributes )
			                     . strlen( $vn_start_uncertainty ) . $vn_start_uncertainty;
			...
			$vn_end_historic = $pa_dates['end']['year'] . "." . sprintf( '%02d', $pa_dates['end']['month'] )
			                   . sprintf( '%02d', $pa_dates['end']['day'] ) . sprintf( '%02d',
					$pa_dates['end']['hours'] ) . sprintf( '%02d', $pa_dates['end']['minutes'] ) . sprintf( '%02d',
					$pa_dates['end']['seconds'] ) . sprintf( '%01d', $vn_end_attributes )
			                   . strlen( $vn_end_uncertainty ) . $vn_end_uncertainty;
			...
			if ( ( $vn_start_historic < 0 ) && ( $vn_end_historic < 0 ) ) {
				...
				if ( ( (int) $vn_start_historic === (int) $vn_end_historic )
				     && ( $vn_end_historic > $vn_start_historic )
				) {
					$this->setParseError( null, TEP_ERROR_RANGE_ERROR );
	...
	public function getHistoricTimestamps() {
		...
		return array(
			0       => $this->opn_start_historic,
			1       => $this->opn_end_historic,
			'start' => $this->opn_start_historic,
			'end'   => $this->opn_end_historic
		);
	}
	...
	public function getUnixTimestamps() {
		...
		return array(
			0       => $this->opn_start_unixtime,
			1       => $this->opn_end_unixtime,
			'start' => $this->opn_start_unixtime,
			'end'   => $this->opn_end_unixtime
		);
	}
	...
	public function getTimes() {
		...
		return array(
			0       => $this->opn_start_time,
			1       => $this->opn_end_time,
			'start' => $this->opn_start_time,
			'end'   => $this->opn_end_time
		);
	}
	# -------------------------------------------------------------------

	/**
	 * Return parsed date/time range as text expression for display. The returned expression is always in a parseable
	 * format subject to settings in datetime.conf and the options described below.
	 *
	 * @param array $pa_options Options include:
	 *                          timeFormat = force use of specific time format (12 or 24 hour). Set to 12 or 24.
	 *                          [Default is 24] timeFormat        (12|24) [default is 24]        =    time format; 12
	 *                          hour or 24 hour format timeDelimiter    (string) [default is first delimiter in
	 *                          language config file]    =    Must be a valid time delimiter for the current language
	 *                          or default will be used timeRangeConjunction (string)    [default is first in lang.
	 *                          config] timeOmitSeconds (true|false) [default is false] timeOmit        (true|false)
	 *                          [default is false] if true, no times are displayed
	 *
	 ...
	 *        showUndated        (true|false) [default is false; if true empty dates are output with the first undated
	 *        specified for the current language] isLifespan        (true|false) [default is false; if true, date is
	 *        output with 'born' and 'died' syntax if appropriate] useQuarterCenturySyntaxForDisplay (true|false)
	 *        [default is false; if true dates ranging over uniform quarter centuries (eg. 1900 - 1925, 1925 - 1950,
	 *        1950 - 1975, 1975-2000) will be output in the format "20 Q1" (eg. 1st quarter of 20th century... 1900 -
	 *        1925) useRomanNumeralsForCenturies (true|false) [default is false; if true century only dates (eg 18th
	 *        century) will be output in roman numerals like "XVIIIth century" start_as_iso8601 (true|false) [if true
	 *        only the start date of the range is returned, in ISO8601 format] end_as_iso8601 (true|false) [if true
	 *        only the end date of the range is returned, in ISO8601 format] dontReturnValueIfOnSameDayAsStart
	 *        (true|false) [Only valid in conjunction with end_as_iso8601] startHistoricTimestamp endHistoricTimestamp
	 *        format            (string) Format date/time output using PHP date()-style format string. The following subset of PHP date() formtting characters are supported: Y y d j F m n t g G h H i s [Default is null; don't use format string for output] normalize = normalize parsed date range to "days"/"years"/"decades"/"centuries" and return as display text. Setting normalize to "years", for example will return a range that only includes the start and end year, regardless of the specificity of the parsed date range. [Default is null, no normalization]
	 *
	 * @return string
	 */
	public function getText( $pa_options = null ) {
		...
		foreach (
			array(
				...
				'afterQualifier',
				'presentDate',
				'useQuarterCenturySyntaxForDisplay',
				'timeOmit',
				'useRomanNumeralsForCenturies',
				'rangePreConjunction',
				'rangeConjunction',
		...
		if ( isset( $pa_options['rangePreConjunction'] ) && is_array( $va_range_preconjunctions )
		     && in_array( $pa_options['rangePreConjunction'], $va_range_preconjunctions )
		) {
		...
		if ( isset( $pa_options['rangeConjunction'] ) && is_array( $va_range_conjunctions )
		     && in_array( $pa_options['rangeConjunction'], $va_range_conjunctions )
		) {
			$vs_range_conjunction = $pa_options['rangeConjunction'];
		} else {
			$vs_range_conjunction = $va_range_conjunctions[0];
		}
		if ( isset( $pa_options['dateTimeConjunction'] ) && is_array( $va_datetime_conjunctions )
		     && in_array( $pa_options['dateTimeConjunction'], $va_datetime_conjunctions )
		) {
		...
		if ( isset( $pa_options['timeOnly'] ) && $pa_options['timeOnly'] ) {
			...
			$vn_start        = ( $va_start_pieces['hours'] * 60 * 60 ) + ( $va_start_pieces['minutes'] * 60 )
			                   + $va_start_pieces['seconds'];
			$vn_end          = ( $va_end_pieces['hours'] * 60 * 60 ) + ( $va_end_pieces['minutes'] * 60 )
			                   + $va_end_pieces['seconds'];
		...
		if ( isset( $va_dates['start'] ) && ( $va_dates['start'] != null ) ) {
			...
			if ( $va_dates['start'] == $va_dates['end'] ) {
				...
				return ( $va_start_pieces['year'] != $va_end_pieces['year'] ) ? $va_start_pieces['year'] . " "
				                                                                . $va_range_conjunctions[0] . " "
				                                                                . $va_end_pieces['year']
			...
			if ( $va_start_pieces['year'] == 0 ) {        // year is not known
				$va_start_pieces['year'] = '????';
				$pa_options['dateFormat']
				                         = 'delimited';        // always output dates with unknown years as delimited as that is the only format that supports them
			}
			...
			if ( ( $va_start_pieces['era'] != $va_end_pieces['era'] )
			     && ( $va_dates['start'] > TEP_START_OF_UNIVERSE )
			) {
			...
			if ( caGetOption( 'dateFormat', $pa_options ) == 'onlyDatesWithHyphens' ) {
				...
				if (
					$va_start_pieces['year'] == $va_end_pieces['year'] && $va_start_pieces['day'] == 1
					&& $va_start_pieces['month'] == 1
					&& $va_start_pieces['hours'] == 0
					&& $va_start_pieces['minutes'] == 0
					&& $va_start_pieces['seconds'] == 0
					&& $va_end_pieces['day'] == 31
					&& $va_end_pieces['month'] == 12
					&& $va_end_pieces['hours'] == 23
					&& $va_end_pieces['minutes'] == 59
					&& $va_end_pieces['seconds'] == 59
				) {
					return '' . $va_start_pieces['year'];
				}

				$vs_date = $va_start_pieces['year'] . '-' . sprintf( '%02d', $va_start_pieces['month'] ) . '-'
				           . sprintf( '%02d', $va_start_pieces['day'] );
				...
				if ( ! (
					...
					$vs_date .= '/' . $va_end_pieces['year'] . '-' . sprintf( '%02d', $va_end_pieces['month'] ) . '-'
					            . sprintf( '%02d', $va_end_pieces['day'] );
				}
			...
			if ( $pa_options['start_as_na_date'] ) {
				...
				if ( ! ( $va_start_pieces['hours'] == 0 && $va_start_pieces['minutes'] == 0
				         && $va_start_pieces['seconds'] == 0 )
				) {
					$vs_date .= ' ' . $va_start_pieces['hours'] . ':' . $va_start_pieces['minutes'] . ':'
					            . $va_start_pieces['seconds'];
				}
			...
			if ( $pa_options['end_as_na_date'] ) {
				...
				if ( ! ( $va_end_pieces['hours'] == 23 && $va_end_pieces['minutes'] == 59
				         && $va_end_pieces['seconds'] == 59 )
				) {
					$vs_date .= ' ' . $va_end_pieces['hours'] . ':' . $va_end_pieces['minutes'] . ':'
					            . $va_end_pieces['seconds'];
				}
				...
				for ( $vn_i = 0; $vn_i < strlen( $pa_options['format'] ); $vn_i ++ ) {
					...
					switch ( $vs_c = $pa_options['format'][ $vn_i ] ) {
						...
						case 'y':
							...
							break;
						case 'd':
							$vs_output .= ( ! $va_seen[ $vs_c ] ) ? sprintf( "%02d", $va_start_pieces['day'] )
								: sprintf( "%02d", $va_end_pieces['day'] );
							break;
						case 'j':
							$vs_output .= ( ! $va_seen[ $vs_c ] ) ? (int) $va_start_pieces['day']
								: (int) $va_end_pieces['day'];
							break;
						case 'F':
							$vs_output .= ( ! $va_seen[ $vs_c ] ) ? $this->getMonthName( $va_start_pieces['month'] )
								: $this->getMonthName( $va_end_pieces['month'] );
							break;
						case 'm':
							$vs_output .= ( ! $va_seen[ $vs_c ] ) ? sprintf( "%02d", $va_start_pieces['month'] )
								: sprintf( "%02d", $va_end_pieces['month'] );
							break;
						case 'n':
							$vs_output .= ( ! $va_seen[ $vs_c ] ) ? (int) $va_start_pieces['month']
								: (int) $va_end_pieces['month'];
							break;
						case 't':
							$vs_output .= ( ! $va_seen[ $vs_c ] ) ? $this->daysInMonth( $va_start_pieces['month'],
								$va_start_pieces['year'] )
								: $this->daysInMonth( $va_end_pieces['month'], $va_end_pieces['year'] );
							break;
						case 'g':
							$vn_24h_time = ( ! $va_seen[ $vs_c ] ) ? (int) $va_start_pieces['hours']
								: (int) $va_end_pieces['hours'];
							if ( $vn_24h_time > 12 ) {
								$vn_24h_time -= 12;
							}
							...
							break;
						case 'G':
							$vs_output .= ( ! $va_seen[ $vs_c ] ) ? (int) $va_start_pieces['hours']
								: (int) $va_end_pieces['hours'];
							break;
						case 'h':
							$vn_24h_time = ( ! $va_seen[ $vs_c ] ) ? (int) $va_start_pieces['hours']
							...
							break;
						case 'H':
							$vs_output .= ( ! $va_seen[ $vs_c ] ) ? sprintf( "%02d", $va_start_pieces['hours'] )
								: sprintf( "%02d", $va_end_pieces['hours'] );
							break;
						case 'i':
							$vs_output .= ( ! $va_seen[ $vs_c ] ) ? sprintf( "%02d", $va_start_pieces['minutes'] )
								: sprintf( "%02d", $va_end_pieces['minutes'] );
							break;
						case 's':
							$vs_output .= ( ! $va_seen[ $vs_c ] ) ? sprintf( "%02d", $va_start_pieces['seconds'] )
								: sprintf( "%02d", $va_end_pieces['seconds'] );
							break;
						default:
							$vb_not_handled = true;
							$vs_output      .= $vs_c;
			...
			// catch 'present' date
			if ( ( $va_start_pieces['year'] == TEP_START_OF_UNIVERSE )
			     && ( $va_end_pieces['year'] == TEP_END_OF_UNIVERSE )
			) {
			...
			if ( $va_dates['start'] <= TEP_START_OF_UNIVERSE ) {
				...
				if ( $pa_options['isLifespan'] && ( sizeof( $va_died_qualifiers ) > 0 ) ) {
					...
					if ( $pa_options['beforeQualifier']
					     && in_array( $pa_options['beforeQualifier'], $va_before_qualifiers )
					) {
			...
			if ( $va_dates['end'] >= TEP_END_OF_UNIVERSE ) {
				...
				if ( caGetOption( 'useConjunctionForAfterDates', $pa_options, false ) ) {
					...
					if ( $pa_options['isLifespan'] && ( sizeof( $va_born_qualifiers ) > 0 ) ) {
						...
						if ( $pa_options['afterQualifier']
						     && in_array( $pa_options['afterQualifier'], $va_after_qualifiers )
						) {
			...
			$vs_circa_indicator  = ( $pa_options['circaIndicator']
			                         && in_array( $pa_options['circaIndicator'], $va_circa_indicators ) )
				? $pa_options['circaIndicator'] : $va_circa_indicators[0];

			$va_probably_indicators = $this->opo_language_settings->getList( 'dateProbablyIndicator' );
			$vs_probably_indicator  = ( $pa_options['probablyIndicator']
			                            && in_array( $pa_options['probablyIndicator'], $va_probably_indicators ) )
				? $pa_options['probablyIndicator'] : $va_probably_indicators[0];
			...
			if ( $va_start_pieces['year'] == $va_end_pieces['year'] ) {
				if ( $va_start_pieces['month'] == $va_end_pieces['month'] ) {
					if ( $va_start_pieces['day'] == $va_end_pieces['day'] ) {    // dates on same day
						// print date
						$vs_day = $this->_dateToText( array(
							'year'              => $va_start_pieces['year'],
							'month'             => $va_start_pieces['month'],
							'day'               => $va_start_pieces['day'],
							'era'               => $va_end_pieces['era'],
							'uncertainty'       => $va_end_pieces['uncertainty'],
							'uncertainty_units' => $va_end_pieces['uncertainty_units']
						), $pa_options );
						...
						if ( ! $vb_full_day_time_range ) {
							$vn_start_time = ( $va_start_pieces['hours'] * 3600 ) + ( $va_start_pieces['minutes'] * 60 )
							                 + $va_start_pieces['seconds'];
							$vn_end_time   = ( $va_end_pieces['hours'] * 3600 ) + ( $va_end_pieces['minutes'] * 60 )
							                 + $va_end_pieces['seconds'];

							return $vs_start_circa . $vs_day . ' ' . $vs_datetime_conjunction . ' '
							       . $this->_timerangeToText( $vn_start_time, $vn_end_time, $pa_options );
						}
						...
						if ( $vb_differing_uncertainties ) {
							...
							return ( $vs_range_preconjunction ? $vs_range_preconjunction . ' ' : '' ) . $vs_start_date
							       . ' ' . $vs_range_conjunction . ' ' . $vs_end_date;
						} else {
							if (
								$vb_full_day_time_range && $va_start_pieces['day'] == 1
								&& $va_end_pieces['day'] == $this->daysInMonth( $va_end_pieces['month'],
									$va_end_pieces['year'] ? $va_end_pieces['year'] : 2004 )
							) {
								// month and year only
								return $vs_start_circa . $this->_dateToText( array(
										'month'             => $va_start_pieces['month'],
										'year'              => $va_start_pieces['year'],
										'era'               => $va_start_pieces['era'],
										'uncertainty'       => $va_start_pieces['uncertainty'],
								...
								if ( $vb_full_day_time_range ) {
									// days, but no times
									if ( is_null( $vb_month_comes_first
										= $this->opo_datetime_settings->get( 'monthComesFirstInDelimitedDate' ) )
									) {
										$vb_month_comes_first
											= $this->opo_language_settings->get( 'monthComesFirstInDelimitedDate' );
									}
									if ( $vb_month_comes_first ) {
										...
										$vs_start_date
											         = $this->_dateToText( array(
											'month' => $va_start_pieces['month'],
											'day'   => $va_start_pieces['day']
										), $pa_options );
										$vs_end_date = $this->_dateToText( array( 'day' => $va_end_pieces['day'] ),
											$pa_options );
									} else {
										$vs_start_date = $this->_dateToText( array( 'day' => $va_start_pieces['day'] ),
											$pa_options );
										$vs_end_date
										               = $this->_dateToText( array(
											'month' => $va_start_pieces['month'],
											'day'   => $va_end_pieces['day']
										), $pa_options );
									}

									$vs_year = $this->_dateToText( array(
										'year'              => $va_start_pieces['year'],
										'era'               => $va_start_pieces['era'],
										'uncertainty'       => $va_start_pieces['uncertainty'],
										'uncertainty_units' => $va_start_pieces['uncertainty_units']
									...
									return ( $vs_range_preconjunction ? $vs_range_preconjunction . ' ' : '' )
									       . $vs_start_date . ' ' . $vs_range_conjunction . ' ' . $vs_end_date
									       . ( ( ( (bool) $pa_options['showCommaAfterDayForTextDates']
									               || $pa_options['forceCommaAfterDay'] )
									             && ( $pa_options['dateFormat'] == 'text' ) ) ? ', ' : ' ' ) . $vs_year;
								} else {
									// days with times
									$vs_start_date
										         = $this->_datetimeToText( array(
										'month'   => $va_start_pieces['month'],
										'day'     => $va_start_pieces['day'],
										'hours'   => $va_start_pieces['hours'],
										'minutes' => $va_start_pieces['minutes'],
										'seconds' => $va_start_pieces['seconds']
									), $pa_options );
									$vs_end_date = $this->_datetimeToText( array(
										'month'             => $va_end_pieces['month'],
										'day'               => $va_end_pieces['day'],
										'year'              => $va_end_pieces['year'],
										'era'               => $va_end_pieces['era'],
										'uncertainty'       => $va_end_pieces['uncertainty'],
										'uncertainty_units' => $va_end_pieces['uncertainty_units'],
										'hours'             => $va_end_pieces['hours'],
										'minutes'           => $va_end_pieces['minutes'],
										'seconds'           => $va_end_pieces['seconds']
									), $pa_options );
					...
					if ( $vb_differing_uncertainties ) {
						...
						if (
							$vb_full_day_time_range && $va_start_pieces['month'] == 1 && $va_start_pieces['day'] == 1
							&& $va_end_pieces['month'] == 12
							&& $va_end_pieces['day'] == 31
						) {
							// year only
							return $vs_start_circa . $this->_dateToText( array(
									...
									'uncertainty'       => $va_start_pieces['uncertainty'],
									'uncertainty_units' => $va_start_pieces['uncertainty_units'],
									'is_bp'             => $va_start_pieces['is_bp']
								), $pa_options );
						} else {
							if ( $vb_full_day_time_range ) {
								// date range within single year without time

								list( $va_start_pieces, $va_end_pieces )
									= $this->_nullDaysAndMonthsForDisplay( $va_start_pieces, $va_end_pieces );

								if ( caGetOption( 'dateFormat', $pa_options, null ) == 'text' ) {
									$vs_start_date = $this->_dateToText( array(
										'month' => $va_start_pieces['month'],
										'day'   => $va_start_pieces['day']
									), $pa_options );
									if ( (bool) $pa_options['showCommaAfterDayForTextDates']
									     || (bool) $this->opo_datetime_settings->get( 'forceCommaAfterDay' )
									) {
										$pa_options['forceCommaAfterDay'] = true;
									}
									$vs_end_date = $this->_dateToText( array(
										'month' => $va_end_pieces['month'],
										'day'   => $va_end_pieces['day']
									), $pa_options );
									$vs_year     = $this->_dateToText( array(
									...
									return ( $vs_range_preconjunction ? $vs_range_preconjunction . ' '
											: $vs_start_circa ) . $vs_start_date . ' ' . $vs_range_conjunction . ' '
									       . $vs_end_circa . $vs_end_date . ( $pa_options['forceCommaAfterDay'] ? ', '
											: ' ' ) . $vs_year;
								} else {
									...
									return ( $vs_range_preconjunction ? $vs_range_preconjunction . ' '
											: $vs_start_circa ) . $vs_start_date . ' ' . $vs_range_conjunction . ' '
									       . $vs_end_circa . $vs_end_date;
								}
							} else {
								// date range within single year with time
								$vs_start_date = $this->_datetimeToText( array(
									'month'   => $va_start_pieces['month'],
									'day'     => $va_start_pieces['day'],
									'hours'   => $va_start_pieces['hours'],
									'minutes' => $va_start_pieces['minutes'],
								...
								return ( $vs_range_preconjunction ? $vs_range_preconjunction . ' ' : $vs_start_circa )
								       . $vs_start_date . ' ' . $vs_range_conjunction . ' ' . $vs_end_circa
								       . $vs_end_date;
							}
						}
					}
				...
				if ( ! $this->opo_datetime_settings->get( 'dontInferQualifiedRanges' )
				     && is_array( $qualified_range_info = self::inferRangeQualifier( [
						'start' => $va_start_pieces,
						'end'   => $va_end_pieces
					], $pa_options ) )
				) {
				...
				if (
					...
					// catch decade dates
					$vs_start_year = $this->_dateToText( array(
						'year'              => $va_start_pieces['year'],
						'era'               => $va_start_pieces['era'],
						'uncertainty'       => $va_start_pieces['uncertainty'],
						'uncertainty_units' => $va_start_pieces['uncertainty_units']
					), $pa_options );
					$vs_end_year   = $this->_dateToText( array(
						'year'              => $va_end_pieces['year'],
						'era'               => $va_end_pieces['era'],
						'uncertainty'       => $va_end_pieces['uncertainty'],
						'uncertainty_units' => $va_end_pieces['uncertainty_units']
					), $pa_options );
					if ( ( ( (int) $vs_start_year % 10 ) == 0 )
					     && ( (int) $vs_end_year == ( (int) $vs_start_year + 9 ) )
					) {
						return $this->makeDecadeString( [ 'start' => $va_start_pieces, 'end' => $va_end_pieces ],
							$pa_options );
					} else {
						// catch century dates
						if (
							...
							&& (
							( ( (int) $va_start_pieces['year'] >= 0 )
							  && ( (int) $va_end_pieces['year'] == ( (int) $va_start_pieces['year'] + 99 ) ) )
							)
						) {
							return $this->makeCenturyString( [ 'start' => $va_start_pieces, 'end' => $va_end_pieces ],
								$pa_options );
						}
						if (
							...
							&& (
							( ( (int) $va_start_pieces['year'] <= 0 )
							  && ( (int) $va_start_pieces['year'] == ( (int) $va_end_pieces['year'] - 99 ) ) )
							)
						) {
							return $this->makeCenturyString( [ 'start' => $va_start_pieces, 'end' => $va_end_pieces ],
								$pa_options );
						...
						return ( $vs_range_preconjunction ? $vs_range_preconjunction . ' ' : $vs_start_circa )
						       . $vs_start_year . ' ' . $vs_range_conjunction . ' ' . $vs_end_circa . $vs_end_year;
					}
					...
					if ( $vb_full_day_time_range ) {
						...
						return ( $vs_range_preconjunction ? $vs_range_preconjunction . ' ' : $vs_start_circa )
						       . $vs_start_date . ' ' . $vs_range_conjunction . ' ' . $vs_end_circa . $vs_end_date;
					} else {
	...
	private function _nullDaysAndMonthsForDisplay( $pa_start_pieces, $pa_end_pieces ) {
		if ( ( (int) $pa_start_pieces['day'] === 1 )
		     && ( (int) $pa_end_pieces['day'] === (int) $this->daysInMonth( $pa_end_pieces['month'],
					$pa_end_pieces['year'] ) )
		) {
			$pa_start_pieces['day'] = null;
		}
		if (
			( ( (int) $pa_end_pieces['day'] === (int) $this->daysInMonth( 12, $pa_end_pieces['year'] ) )
			  && ( (int) $pa_end_pieces['month'] === 12 ) )
	...
	private function _timeToText( $pn_seconds, $pa_options = null
		...
		if ( $pa_options['timeFormat'] == 12 ) {
			...
			if ( $pa_options['timeOmitSeconds'] || ( $vn_seconds == 0 ) ) {
				...
				$vs_text = join( $vs_time_delim,
						array( $vn_hours, sprintf( '%02d', $vn_minutes ), sprintf( '%02d', $vn_seconds ) ) ) . ' '
				           . $vs_meridian;
			}
		} else {
			if ( $pa_options['timeOmitSeconds'] || ( $vn_seconds == 0 ) ) {
	...
	private function _timerangeToText( $pn_start, $pn_end, $pa_options = null
		...
		if ( $pn_start != $pn_end ) {
			...
			if ( isset( $pa_options['timeRangeConjunction'] )
			     && ( in_array( $pa_options['timeRangeConjunction'], $va_tmp ) )
			) {
	...
	private function _dateToText( $pa_date_pieces, $pa_options = null ) {
		foreach (
			array( 'dateFormat', 'dateDelimiter', 'uncertaintyIndicator', 'showADEra', 'forceCommaAfterDay' ) as $vs_opt
		) {
			if ( ! isset( $pa_options[ $vs_opt ] ) && ( $vs_opt_val = $this->opo_datetime_settings->get( $vs_opt ) ) ) {
				$pa_options[ $vs_opt ] = $vs_opt_val;
			}
		...
		if ( $pa_options['uncertaintyIndicator']
		     && in_array( $pa_options['uncertaintyIndicator'], $va_uncertainty_indicators )
		) {
		...
		if ( $pa_date_pieces['year'] ) {
			...
			if ( $pa_date_pieces['uncertainty_units'] && $pa_date_pieces['uncertainty'] ) {
				$vs_year .= ' ' . $vs_uncertainty_indicator . ' ' . $pa_date_pieces['uncertainty']
				            . $pa_date_pieces['uncertainty_units'];
			}
		}
		...
		if ( ( $vs_day > 0 ) && ( $pa_options['dateFormat'] != 'delimited' )
		     && ( $vs_day_suffix
				= $this->opo_language_settings->get( 'daySuffix' ) )
		) {
			// add day suffix
		...
		if ( $pa_options['dateFormat'] === 'ymd' ) {
			...
			if ( $vs_month ) {
				$va_date[] = ( ( $pa_options['dateFormat'] == 'delimited' ) ? sprintf( "%02d", $vs_month )
					: $vs_month );
			}
			if ( $vs_day ) {
				if (
					( ( (bool) $pa_options['showCommaAfterDayForTextDates'] ) && ( $pa_options['dateFormat'] == 'text' )
					  && $vs_year )
	...
	private function _dateTimeToText( $pa_date_pieces, $pa_options = null ) {
		...
		if ( ! $pa_options['timeOmit'] ) {
			$vn_seconds = ( $pa_date_pieces['hours'] * 3600 ) + ( $pa_date_pieces['minutes'] * 60 )
			              + $pa_date_pieces['seconds'];
	...
	public function setLanguage( $ps_iso_code ) {
		if ( file_exists( __CA_LIB_DIR__ . '/Parsers/TimeExpressionParser/' . $ps_iso_code . '.lang' ) ) {
			...
			$this->opo_language_settings = Configuration::load( __CA_LIB_DIR__ . '/Parsers/TimeExpressionParser/'
			                                                    . $ps_iso_code . '.lang' );
	...
	public function unixToHistoricTimestamp( $pn_unix_timestamp ) {
		...
		return $va_date_info['year'] . "." . sprintf( '%02d', $va_date_info['mon'] ) . sprintf( '%02d',
				$va_date_info['mday'] ) . sprintf( '%02d', $va_date_info['hours'] ) . sprintf( '%02d',
				$va_date_info['minutes'] ) . sprintf( '%02d', $va_date_info['seconds'] ) . '00';
	}
	...
	/**
	 * Convert date value array (array with keys "month", "day", "year", "hours", "minutes", "seconds" used internally
	 * by parser) to ISO 8601 date/time expression.
	 *
	 * @param array  $pa_date
	 * @param string $ps_mode    Part of date range to return. Valid values are "START" (beginning of range) "END" (end
	 *                           of range) and "FULL" (full range). [Default is "START"]
	 * @param array  $pa_options Options include:
	 *                           timeOmit = Omit time from returned ISO 8601 date. [Default is false]
	 *                           returnUnbounded = Return extreme value for unbounded dates. For "before" dates the
	 *                           start date would be equal to -9999; for "after" dates the end date would equal "9999".
	 *                           [Default is false] dateFormat = If set to "yearOnly" will return bare year. [Default
	 *                           is null]
	 *
	 * @return string
	 */
	public function getISODateTime( $pa_date, $ps_mode = 'START', $pa_options = null ) {
		...
		if ( $ps_mode == 'FULL' ) {
			$vs_date = $pa_date['year'] . '-' . sprintf( "%02d", $pa_date['month'] ) . '-' . sprintf( "%02d",
					$pa_date['day'] );

			if ( ! isset( $pa_options['timeOmit'] ) || ! $pa_options['timeOmit'] ) {
				$vs_date .= 'T' . sprintf( "%02d", $pa_date['hours'] ) . ':' . sprintf( "%02d", $pa_date['minutes'] )
				            . ':' . sprintf( "%02d", $pa_date['seconds'] ) . 'Z';
			}
		...
		if ( ! isset( $pa_options['timeOmit'] ) || ! $pa_options['timeOmit'] ) {
			if (
				( ! ( $pa_date['hours'] == 0 && $pa_date['minutes'] == 0 && $pa_date['seconds'] == 0
				      && ( $ps_mode == 'START' ) ) )
				&& ( ! ( $pa_date['hours'] == 23 && $pa_date['minutes'] == 59 && $pa_date['seconds'] == 59
				         && ( $ps_mode == 'END' ) ) )
			) {
				$vs_time .= 'T' . sprintf( "%02d", $pa_date['hours'] ) . ':' . sprintf( "%02d", $pa_date['minutes'] )
				            . ':' . sprintf( "%02d", $pa_date['seconds'] ) . 'Z';
			}
		}
	...
	public function normalizeDateRange( $pn_historic_start, $pn_historic_end, $ps_normalization ) {
		...
		switch ( $ps_normalization ) {
			case 'decades':
				...
				if ( ( $vn_s <= TEP_START_OF_UNIVERSE ) || ( $vn_e >= TEP_END_OF_UNIVERSE ) || ( $vn_s == 0 )
				     || ( $vn_e == 0 )
				) {
					break;
				}
				...
				if ( $vn_s <= $vn_e ) {
					for ( $vn_y = $vn_s; $vn_y <= $vn_e; $vn_y += 10 ) {
						...
						if ( $vn_y < 0 ) {
							$va_values[ (int) $vn_y ] = abs( $vn_y ) . $va_decade_indicators[0] . ' '
							                            . $vs_bc_indicator;
						} else {
			...
			case 'centuries':
				...
				if ( $vn_s <= $vn_e ) {
					...
					for ( $vn_y = $vn_s; $vn_y <= $vn_e; $vn_y += 100 ) {
						...
						$vn_century_num = ( $vn_y >= 0 ) ? abs( floor( $vn_y / 100 ) ) + 1
							: abs( floor( $vn_y / 100 ) );
						if ( $vn_century_num == 0 ) {
							continue;
						}
						...
						if ( is_array( $va_ordinal_exceptions )
						     && isset( $va_ordinal_exceptions[ $vn_century_num ] )
						) {
							$vs_ordinal_to_display = $va_ordinal_exceptions[ $vn_century_num ];
						} else {
							$vs_ordinal_to_display = isset( $va_ordinals[ $vn_x ] ) ? $va_ordinals[ $vn_x ]
								: $vs_ordinal_default;
						}

						$va_values[ (int) $vn_y ] = ( $vn_century_num ) . $vs_ordinal_to_display . ' '
						                            . $va_century_indicators[0] . ( ( floor( $vn_y / 100 ) < 0 ) ? ' '
						                                                                                           . $vs_bc_indicator
								: '' );
					}
				}
					...
					for ( $vn_y = $vn_s_year; $vn_y <= $vn_e_year; $vn_y ++ ) {
						...
						for ( $vn_m = $vn_start_month; $vn_m <= $vn_end_month; $vn_m ++ ) {
							...
							for ( $vn_d = 1; $vn_d < $vn_num_days; $vn_d ++ ) {
								...
								$va_values["{$vn_y}.{$vn_m}.{$vn_d}"] = $va_month_indicators[ ( $vn_m - 1 ) ] . ' '
								                                        . $vn_d . ' ' . $vn_y;
							}
						}
					}
	 ...
	 *
	 * @param array $pa_options Options include:
	 *                          returnAs = time units of return value. Valid values are: days, hours, minutes, seconds.
	 *                          [Default is seconds]
	 *
	...
	public function interval( $pa_options = null ) {
		...
		switch ( strtolower( caGetOption( 'returnAs', $pa_options, 'seconds' ) ) ) {
			case 'days':
				...
				break;
			case 'hours':
				return ( $vo_interval->days * 24 * 60 * 60 ) + ( $vo_interval->h * 60 * 60 ) + ( ( $vo_interval->m
				                                                                                   >= 30 ) ? 1 : 0 );
				break;
			case 'minutes':
				return ( $vo_interval->days * 24 * 60 * 60 ) + ( $vo_interval->h * 60 * 60 ) + ( $vo_interval->m * 60 )
				       + ( ( $vo_interval->s >= 30 ) ? 1 : 0 );
				break;
			case 'seconds':
			default:
				return ( $vo_interval->days * 24 * 60 * 60 ) + ( $vo_interval->h * 60 * 60 ) + ( $vo_interval->m * 60 )
	 ...
	 *
	 * @param string $qualifier    A TEP_TOKEN_* constant for the qualifier to apply. Possible values are
	 *                             TEP_TOKEN_EARLY, TEP_TOKEN_MID or TEP_TOKEN_LATE.
	...
	public static function inferRangeQualifier( $dates, $options = null ) {
		...
		// Early century
		if ( ( ( $start_pieces['year'] % 100 ) == 0 )
		     && ( $end_pieces['year'] == ( $start_pieces['year'] + self::$early_mid_late_range_lengths['century'] ) )
		) {
			return [
				'qualifier'  => TEP_TOKEN_EARLY,
				'range_type' => 'century',
				'value'      => $early_qualifiers[0] . ' ' . $o_tep->makeCenturyString( $dates, $options )
			];
		}

		// Early decade
		if ( ( ( $start_pieces['year'] % 10 ) == 0 )
		     && ( $end_pieces['year'] == ( $start_pieces['year'] + self::$early_mid_late_range_lengths['decade'] ) )
		) {
			return [
				'qualifier'  => TEP_TOKEN_EARLY,
				'range_type' => 'decade',
				'value'      => $early_qualifiers[0] . ' ' . $o_tep->makeDecadeString( $dates, $options )
			];
		}

		// Late century
		if ( ( ( ( $end_pieces['year'] - 99 ) % 100 ) == 0 )
		     && ( $start_pieces['year'] == ( $end_pieces['year'] - ( self::$early_mid_late_range_lengths['century']
		                                                             - 1 ) ) )
		) {
			return [
				'qualifier'  => TEP_TOKEN_LATE,
				'range_type' => 'century',
				'value'      => $late_qualifiers[0] . ' ' . $o_tep->makeCenturyString( $dates, $options )
			];
		}

		// Late decade
		if ( ( ( ( $end_pieces['year'] - 9 ) % 10 ) == 0 )
		     && ( $start_pieces['year'] == ( $end_pieces['year'] - ( self::$early_mid_late_range_lengths['decade']
		                                                             - 1 ) ) )
		) {
			return [
				'qualifier'  => TEP_TOKEN_LATE,
				'range_type' => 'century',
				'value'      => $late_qualifiers[0] . ' ' . $o_tep->makeDecadeString( $dates, $options )
			];
		}

		// Mid century
		if ( ( ( ( $start_pieces['year'] - floor( self::$early_mid_late_range_intervals['century'] / 2 )
		           + floor( self::$early_mid_late_range_lengths['century'] / 2 ) ) % 100 ) == 0 )
		     && ( $end_pieces['year'] == ( $start_pieces['year'] + self::$early_mid_late_range_lengths['century'] ) )
		) {
			return [
				'qualifier'  => TEP_TOKEN_MID,
				'range_type' => 'century',
				'value'      => $mid_qualifiers[0] . ' ' . $o_tep->makeCenturyString( $dates, $options )
			];
		}

		// Mid decade
		if ( ! $options['isSpan']
		     && ( ( ( $start_pieces['year'] - floor( self::$early_mid_late_range_intervals['decade'] / 2 )
		              + floor( self::$early_mid_late_range_lengths['decade'] / 2 ) ) % 10 ) == 0 )
		     && ( $end_pieces['year'] == ( $start_pieces['year'] + self::$early_mid_late_range_lengths['decade'] ) )
		) {
			return [
				'qualifier'  => TEP_TOKEN_MID,
				'range_type' => 'decade',
				'value'      => $mid_qualifiers[0] . ' ' . $o_tep->makeDecadeString( $dates, $options )
			];
		}
		...
		if ( ( ( $end_pieces['year'] - ( $end_pieces['year'] % 100 ) ) - ( $start_pieces['year']
		                                                                   - ( $start_pieces['year'] % 100 ) ) ) >= 100
		) {
			$options['isSpan'] = true;
			...
			if ( ( $start_pieces['year'] % 100 ) == 0 ) { // early
				...
				$first_century_info = self::inferRangeQualifier( [ 'start' => $start_pieces, 'end' => $mod_end ],
					$options );
			} elseif ( ( $start_pieces['year'] % 100 ) == ( self::$early_mid_late_range_intervals['century']
			                                                - self::$early_mid_late_range_lengths['century'] )
			) { // late
				$mod_end['year']    = $start_pieces['year'] + self::$early_mid_late_range_lengths['century'] - 1;
				$first_century_info = self::inferRangeQualifier( [ 'start' => $start_pieces, 'end' => $mod_end ],
					$options );
			} elseif ( ( ( $start_pieces['year'] + floor( self::$early_mid_late_range_lengths['century'] / 2 )
			               - floor( self::$early_mid_late_range_intervals['century'] / 2 ) ) % 100 ) == 0
			) { // mid
				$mod_end['year']    = $start_pieces['year'] + self::$early_mid_late_range_lengths['century'];
				$first_century_info = self::inferRangeQualifier( [ 'start' => $start_pieces, 'end' => $mod_end ],
					$options );
			...
			if ( ( $end_pieces['year'] % 100 ) == self::$early_mid_late_range_lengths['century'] ) { // early
				...
				$second_century_info = self::inferRangeQualifier( [ 'start' => $mod_start, 'end' => $end_pieces ],
					$options );
			} elseif ( ( ( $end_pieces['year'] + 1 ) % 100 ) == 0 ) { // late
				$mod_start['year']   = $end_pieces['year'] - ( $end_pieces['year'] % 100 )
				                       + self::$early_mid_late_range_intervals['century']
				                       - self::$early_mid_late_range_lengths['century'];
				$second_century_info = self::inferRangeQualifier( [ 'start' => $mod_start, 'end' => $end_pieces ],
					$options );
			} elseif ( ( ( $end_pieces['year'] - floor( self::$early_mid_late_range_lengths['century'] / 2 )
			               - floor( self::$early_mid_late_range_intervals['century'] / 2 ) ) % 100 ) == 0
			) { // mid
				$mod_start['year']   = $end_pieces['year'] - ( $end_pieces['year'] % 100 )
				                       - floor( self::$early_mid_late_range_lengths['century'] / 2 )
				                       + floor( self::$early_mid_late_range_intervals['century'] / 2 );
				$second_century_info = self::inferRangeQualifier( [ 'start' => $mod_start, 'end' => $end_pieces ],
					$options );
			}

			if ( is_array( $first_century_info ) && is_array( $second_century_info )
			     && ! ( ( $first_century_info['qualifier'] == TEP_TOKEN_EARLY )
			            && ( $second_century_info['qualifier'] == TEP_TOKEN_LATE ) )
			) {
				return [
					'qualifier'  => null,
					'range_type' => 'century',
					'value'      => $first_century_info['value'] . " - " . $second_century_info['value']
				];
			}
		}
		...
		if ( ( ( $end_pieces['year'] - ( $end_pieces['year'] % 10 ) ) - ( $start_pieces['year']
		                                                                  - ( $start_pieces['year'] % 10 ) ) ) >= 10
		) {
			$options['isSpan'] = true;
			...
			if ( ( $start_pieces['year'] % 10 ) == 0 ) { // early
				...
				$first_decade_info = self::inferRangeQualifier( [ 'start' => $start_pieces, 'end' => $mod_end ],
					$options );
			} elseif ( ( $start_pieces['year'] % 10 ) == ( self::$early_mid_late_range_intervals['decade']
			                                               - self::$early_mid_late_range_lengths['decade'] )
			) { // late
				$mod_end['year']   = $start_pieces['year'] + self::$early_mid_late_range_lengths['decade'] - 1;
				$first_decade_info = self::inferRangeQualifier( [ 'start' => $start_pieces, 'end' => $mod_end ],
					$options );
			} elseif ( ( ( $start_pieces['year'] + floor( self::$early_mid_late_range_lengths['decade'] / 2 )
			               - floor( self::$early_mid_late_range_intervals['decade'] / 2 ) ) % 10 ) == 0
			) { // mid
				$mod_end['year']   = $start_pieces['year'] + self::$early_mid_late_range_lengths['decade'];
				$first_decade_info = self::inferRangeQualifier( [ 'start' => $start_pieces, 'end' => $mod_end ],
					$options );
			...
			if ( ( $end_pieces['year'] % 10 ) == self::$early_mid_late_range_lengths['decade'] ) { // early
				...
				$second_decade_info = self::inferRangeQualifier( [ 'start' => $mod_start, 'end' => $end_pieces ],
					$options );
			} elseif ( ( ( $end_pieces['year'] + 1 ) % 10 ) == 0 ) { // late
				$mod_start['year']  = $end_pieces['year'] - ( $end_pieces['year'] % 10 )
				                      + self::$early_mid_late_range_intervals['decade']
				                      - self::$early_mid_late_range_lengths['decade'];
				$second_decade_info = self::inferRangeQualifier( [ 'start' => $mod_start, 'end' => $end_pieces ],
					$options );
			} elseif ( ( ( $end_pieces['year'] - floor( self::$early_mid_late_range_lengths['decade'] / 2 )
			               - floor( self::$early_mid_late_range_intervals['decade'] / 2 ) ) % 10 ) == 0
			) { // mid
				$mod_start['year']  = $end_pieces['year'] - ( $end_pieces['year'] % 10 )
				                      - floor( self::$early_mid_late_range_lengths['decade'] / 2 )
				                      + floor( self::$early_mid_late_range_intervals['decade'] / 2 );
				$second_decade_info = self::inferRangeQualifier( [ 'start' => $mod_start, 'end' => $end_pieces ],
					$options );
			}

			if ( is_array( $first_decade_info ) && is_array( $second_decade_info )
			     && ! ( ( $first_decade_info['qualifier'] == TEP_TOKEN_EARLY )
			            && ( $second_decade_info['qualifier'] == TEP_TOKEN_LATE ) )
			) {
				return [
					'qualifier'  => null,
					'range_type' => 'decade',
					'value'      => $first_decade_info['value'] . " - " . $second_decade_info['value']
				];
			}
		}
