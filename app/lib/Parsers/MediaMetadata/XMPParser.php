...
class XMPParser extends BaseMediaMetadataParser
{
     ...
     */
    static public $s_jpeg_segment_names = array(
        0xC0 => "SOF0",
        0xC1 => "SOF1",
        0xC2 => "SOF2",
        0xC3 => "SOF4",
    ...
    public function initMetadata()
    {
		...
		<rdf:Description rdf:about="" xmlns:xmp="http://ns.adobe.com/xap/1.0/" xmlns:xmpMM="http://ns.adobe.com/xap/1.0/mm/" xmlns:stEvt="http://ns.adobe.com/xap/1.0/sType/ResourceEvent#" xmlns:photoshop="http://ns.adobe.com/photoshop/1.0/" xmlns:crs="http://ns.adobe.com/camera-raw-settings/1.0/" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:xmpRights="http://ns.adobe.com/xap/1.0/rights/" xmlns:Iptc4xmpCore="http://iptc.org/std/Iptc4xmpCore/1.0/xmlns/" xmp:CreatorTool="CollectiveAccess" xmp:ModifyDate="' . date(
                "c"
            ) . '" xmp:CreateDate="' . date("c") . '" xmp:MetadataDate="' . date(
                "c"
    ...
    public function write($ps_filepath = null)
    {
        ...
        $this->opa_header_data = $this->removeJPEGHeaderSegment(
            'APP1',
            $this->opa_header_data
        );    // get rid of existing XMP
        $this->opa_header_data = $this->removeJPEGHeaderSegment('APP13', $this->opa_header_data);    // get rid of IPTC

        $vs_packet = '<?xpacket begin="ï»¿" id="W5M0MpCehiHzreSzNTczkc9d"?>' . str_replace(
                '<?xml version="1.0"?>',
                '',
...
<?xpacket end="w"?>';
        ...
        return $this->putJPEGHeaderData(
            $this->ops_filepath,
            ($ps_filepath ? $ps_filepath : $this->ops_filepath),
            $this->opa_header_data
        );
    }
    # -------------------------------------------------------
    ...
    public function set($ps_field, $ps_value)
    {
        ...
        switch ($ps_field) {
            case 'Format':
                ...
                break;
            case 'DateCreated':
                $this->o_rdf_desc[0]->addAttribute(
            ...
            case 'CreatorWebsite':
                if (!$this->o_creator) {
                    if (!($this->o_creator = $this->opo_metadata->xpath('//Iptc4xmpCore:CreatorContactInfo'))) {
                        $this->o_creator = $this->o_rdf_desc[0]->addChild(
