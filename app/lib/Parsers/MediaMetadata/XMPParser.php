 ...
 *            by Evan Hunter. It is Copyright Evan Hunter 2004, and licensed under the same GPL license as
 *            CollectiveAccess
 *
 */
class XMPParser extends BaseMediaMetadataParser {
	...
	public function parse( $ps_filepath ) {
		...
		if ( is_object( $this->opo_old_metadata ) ) {
			...
			$this->opo_metadata->registerXPathNamespace( "Iptc4xmpCore",
	...
	public function initMetadata() {
		...
		<rdf:Description rdf:about="" xmlns:xmp="http://ns.adobe.com/xap/1.0/" xmlns:xmpMM="http://ns.adobe.com/xap/1.0/mm/" xmlns:stEvt="http://ns.adobe.com/xap/1.0/sType/ResourceEvent#" xmlns:photoshop="http://ns.adobe.com/photoshop/1.0/" xmlns:crs="http://ns.adobe.com/camera-raw-settings/1.0/" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:xmpRights="http://ns.adobe.com/xap/1.0/rights/" xmlns:Iptc4xmpCore="http://iptc.org/std/Iptc4xmpCore/1.0/xmlns/" xmp:CreatorTool="CollectiveAccess" xmp:ModifyDate="'
		               . date( "c" ) . '" xmp:CreateDate="' . date( "c" ) . '" xmp:MetadataDate="' . date( "c" )
	 ...
	 *
	 * @param string $ps_filepath Optional path to write output to. If not specified the location of the input file is
	 *                            used.
	 *
	...
	public function write( $ps_filepath = null ) {
		...
		$this->opa_header_data = $this->removeJPEGHeaderSegment( 'APP1',
			$this->opa_header_data );    // get rid of existing XMP
		$this->opa_header_data = $this->removeJPEGHeaderSegment( 'APP13',
			$this->opa_header_data );    // get rid of IPTC

		$vs_packet = '<?xpacket begin="ï»¿" id="W5M0MpCehiHzreSzNTczkc9d"?>' . str_replace( '<?xml version="1.0"?>', '',
				$this->opo_metadata->asXML() ) . '

<?xpacket end="w"?>';

		$this->opa_header_data = $this->putXMPData( $this->opa_header_data, $vs_packet );

		return $this->putJPEGHeaderData( $this->ops_filepath, ( $ps_filepath ? $ps_filepath : $this->ops_filepath ),
			$this->opa_header_data );
	}
	# -------------------------------------------------------
	...
	public function set( $ps_field, $ps_value ) {
		...
		switch ( $ps_field ) {
			case 'Format':
				...
				break;
			case 'DateCreated':
				$this->o_rdf_desc[0]->addAttribute( 'photoshop:DateCreated', $ps_value,
					"http://ns.adobe.com/photoshop/1.0/" );
				break;
			case 'DescriptionWriter':
				$this->o_rdf_desc[0]->addAttribute( 'photoshop:CaptionWriter', $ps_value,
					"http://ns.adobe.com/photoshop/1.0/" );
				break;
			case 'RightsURL':
				$this->o_rdf_desc[0]->addAttribute( 'xmpRights:WebStatement', $ps_value,
					"http://ns.adobe.com/xap/1.0/rights/" );
				break;
			case 'CopyrightStatus':
				if ( ! is_null( $ps_value ) ) {
					$vs_value = (bool) $ps_value ? "True" : "False";
					$this->o_rdf_desc[0]->addAttribute( 'xmpRights:Marked', $vs_value,
						"http://ns.adobe.com/xap/1.0/rights/" );
				}
				break;
			case 'Title':
			...
			case 'CreatorWebsite':
				if ( ! $this->o_creator ) {
					if ( ! ( $this->o_creator = $this->opo_metadata->xpath( '//Iptc4xmpCore:CreatorContactInfo' ) ) ) {
						$this->o_creator = $this->o_rdf_desc[0]->addChild( "Iptc4xmpCore:CreatorContactInfo", '',
							"http://iptc.org/std/Iptc4xmpCore/1.0/xmlns/" );
					}
				}
				...
				switch ( $ps_field ) {
					case 'CreatorAddress':
						$this->o_creator[0]->addAttribute( 'Iptc4xmpCore:CiAdrExtadr', $ps_value,
							"http://iptc.org/std/Iptc4xmpCore/1.0/xmlns/" );
						break;
					case 'CreatorCity':
						$this->o_creator[0]->addAttribute( 'Iptc4xmpCore:CiAdrCity', $ps_value,
							"http://iptc.org/std/Iptc4xmpCore/1.0/xmlns/" );
						break;
					case 'CreatorStateRegion':
						$this->o_creator[0]->addAttribute( 'Iptc4xmpCore:CiAdrRegion', $ps_value,
							"http://iptc.org/std/Iptc4xmpCore/1.0/xmlns/" );
						break;
					case 'CreatorPostalCode':
						$this->o_creator[0]->addAttribute( 'Iptc4xmpCore:CiAdrPcode', $ps_value,
							"http://iptc.org/std/Iptc4xmpCore/1.0/xmlns/" );
						break;
					case 'CreatorCountry':
						$this->o_creator[0]->addAttribute( 'Iptc4xmpCore:CiAdrCtry', $ps_value,
							"http://iptc.org/std/Iptc4xmpCore/1.0/xmlns/" );
						break;
					case 'CreatorPhone':
						$this->o_creator[0]->addAttribute( 'Iptc4xmpCore:CiTelWork', $ps_value,
							"http://iptc.org/std/Iptc4xmpCore/1.0/xmlns/" );
						break;
					case 'CreatorEmail':
						$this->o_creator[0]->addAttribute( 'Iptc4xmpCore:CiEmailWork', $ps_value,
							"http://iptc.org/std/Iptc4xmpCore/1.0/xmlns/" );
						break;
					case 'CreatorWebsite':
						$this->o_creator[0]->addAttribute( 'Iptc4xmpCore:CiUrlWork', $ps_value,
							"http://iptc.org/std/Iptc4xmpCore/1.0/xmlns/" );
						break;
				}
				break;
	 ...
	 *                           returnAsArray = if true an array of values is returned. Default is false, in which
	 *                           case a string of values concatenated with a delimiter is returned. delimiter = text to
	 *                           insert between returned values when those values are returned as a string. Default is
	 *                           ";" (a single semicolon)
	 *
	 * @return mixed Value or null if no value is set or field is invalid. Value is a string unless returnAsArray
	 *               setting is passed, in which case an array is returned.
	 */
	 ...
	 * @param string $ps_new_filename     Path where modified file should be written. Can be the same of
	 *                                    $ps_od_filename if you wish to modify the file in-place.
	...
	private function putXMPData( $pa_jpeg_header_data, $ps_new_xmp_data ) {
		...
		while ( ( $pa_jpeg_header_data[ $i ]['SegName'] == "APP0" )
		        || ( $pa_jpeg_header_data[ $i ]['SegName'] == "APP1" ) ) {
			$i ++;
		}
		...
		array_splice( $pa_jpeg_header_data, $i, 0, array(
			array(
				"SegType" => 0xE1,
				"SegName" => "APP1",
