...
class TilepicParser
{
    ...
    # ------------------------------------------------------------------------------------
    public function TilepicParser($filename = "")
    {
    ...
    private function _imageMagickRead($ps_filepath)
    {
        if ($this->ops_imagemagick_path) {
            caExec(
                $this->ops_imagemagick_path . '/identify -format "%m;%w;%h\n" ' . caEscapeShellArg(
                    $ps_filepath
    ...
    private function _graphicsMagickRead($ps_filepath)
    {
        if ($this->ops_graphicsmagick_path) {
            caExec(
                $this->ops_graphicsmagick_path . ' identify -format "%m;%w;%h\n" ' . caEscapeShellArg(
                    $ps_filepath
                ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                $va_output,
                $vn_return
    ...
    private function _imageMagickProcess($ps_source_filepath, $ps_dest_filepath, $pa_ops, $pn_quality = null)
    {
        ...
        caExec(
            $this->ops_imagemagick_path . '/convert ' . caEscapeShellArg($ps_source_filepath . '[0]') . ' ' . join(
                ' ',
                $va_ops
            ) . ' "' . $ps_dest_filepath . '"'
        );
    ...
    private function _graphicsMagickProcess($ps_source_filepath, $ps_dest_filepath, $pa_ops, $pn_quality = null)
    {
        ...
        caExec(
            $this->ops_graphicsmagick_path . ' convert ' . caEscapeShellArg($ps_source_filepath . '[0]') . ' ' . join(
                ' ',
                $va_ops
            ) . ' "' . $ps_dest_filepath . '"'
        );
    ...
    {
        caExec(
            $this->ops_imagemagick_path . '/montage ' . join(
                ' ',
                $pa_tiles
            ) . ' -mode Concatenate -tile ' . $pn_tile_width . 'x' . $pn_tile_height . ' "' . $ps_dest_filepath . '"'
        );
    ...
    {
        caExec(
            $this->ops_graphicsmagick_path . ' montage ' . join(
                ' ',
                $pa_tiles
            ) . ' -mode Concatenate -tile ' . $pn_tile_width . 'x' . $pn_tile_height . ' "' . $ps_dest_filepath . '"'
        );
        ...
        if ($fh = fopen($ps_output_path . ".tpc", "w")) {
            ...
            if (!fwrite(
                ...
                pack(
                    ...
                    $base_width,
                    $base_height,
                    $pa_options["tile_width"],
                    $pa_options["tile_height"],
                    $tiles,
                    $pa_options["layers"],
                    $pa_options["layer_ratio"],
                    strlen($attribute_list),
                    $attributes
                )
            )) {
    ...
    public function encode_gmagick($ps_filepath, $ps_output_path, $pa_options)
    {
        ...
        if (function_exists('exif_read_data') && !($this->opo_config->get('dont_use_exif_read_data'))) {
            ...
            if (abs(
                    $rotation
                ) === 90) {    // flip dimensions due to EXIF rotation (Gmagick doesn't update dimensions properly)
                $d = $image_dimensions;
        ...
        for ($l = $pa_options["layers"]; $l >= 1; $l--) {
            ...
            while ($y < $image_height) {
                ...
                if (abs(
                        $rotation
                    ) === 90) {    // flip rotation due to EXIF rotation (Gmagick doesn't set EXIF rotation properly)
    ...
    public function getLayer_gd($layer_number, $output_mimetype)
    {
        ...
        $r_new_image = imagecreatetruecolor(
            $layer_tiles[$layer_number]["width"],
