...
class TilepicParser {
	...
	function load( $ps_filepath ) {
		...
		if ( $this->fh = $fh = @fopen( $ps_filepath, 'r' ) ) {
			...
			if ( preg_match( "/TPC\n/", $sig ) ) {
				...
				$header_values
				        = unpack( "Nwidth/Nheight/Ntile_width/Ntile_height/Ntiles/nlayers/nratio/Nattributes_size/Nattributes",
					$header );

				# --- Check header values
				if ( ( $header_values["width"] < 1 ) || ( $header_values["height"] < 1 )
				     || ( $header_values["tile_width"] < 1 )
				     || ( $header_values["tile_height"] < 1 )
				     || ( $header_values["tiles"] < 1 )
				     || ( $header_values["layers"] < 1 )
				) {
	...
	function encode( $ps_filepath, $ps_output_path, $pa_options ) {
		...
		#
		if ( ( $pa_options["layer_ratio"] = ( isset( $pa_options["layer_ratio"] ) ) ? $pa_options["layer_ratio"] : 2 )
		     <= 0
		) {
			$pa_options["layer_ratio"] = 2;
		}

		if ( ( $pa_options["scale_factor"] = ( isset( $pa_options["scale_factor"] ) ) ? $pa_options["scale_factor"]
				: 1 ) <= 0
		) {
		...
		if ( ( $pa_options["antialiasing"] = ( isset( $pa_options["antialiasing"] ) ) ? $pa_options["antialiasing"]
				: 1 ) < 0
		) {
	...
	private function _imageMagickProcess( $ps_source_filepath, $ps_dest_filepath, $pa_ops, $pn_quality = null ) {
		...
		foreach ( $pa_ops as $va_op ) {
			switch ( $va_op['op'] ) {
				...
				case 'crop':
					...
					$va_ops[] = '-crop ' . $va_op['width'] . 'x' . $va_op['height'] . '+' . $va_op['x'] . '+'
					            . $va_op['y'];
					break;
				case 'rotate':
		...
		caExec( $this->ops_imagemagick_path . '/convert ' . caEscapeShellArg( $ps_source_filepath . '[0]' ) . ' '
		        . join( ' ', $va_ops ) . ' "' . $ps_dest_filepath . '"' );
	...
	private function _graphicsMagickProcess( $ps_source_filepath, $ps_dest_filepath, $pa_ops, $pn_quality = null ) {
		...
		caExec( $this->ops_graphicsmagick_path . ' convert ' . caEscapeShellArg( $ps_source_filepath . '[0]' ) . ' '
		        . join( ' ', $va_ops ) . ' "' . $ps_dest_filepath . '"' );
	...
	private function _imageMagickImageFromTiles( $ps_dest_filepath, $pa_tiles, $pn_tile_width, $pn_tile_height ) {
		...
		caExec( $this->ops_imagemagick_path . '/montage ' . join( ' ', $pa_tiles ) . ' -mode Concatenate -tile '
		        . $pn_tile_width . 'x' . $pn_tile_height . ' "' . $ps_dest_filepath . '"' );
	...
	private function _graphicsMagickImageFromTiles( $ps_dest_filepath, $pa_tiles, $pn_tile_width, $pn_tile_height ) {
		caExec( $this->ops_graphicsmagick_path . ' montage ' . join( ' ', $pa_tiles ) . ' -mode Concatenate -tile '
		        . $pn_tile_width . 'x' . $pn_tile_height . ' "' . $ps_dest_filepath . '"' );
	...
	function encode_imagemagick( $ps_filepath, $ps_output_path, $pa_options ) {
		...
		if ( $fh = fopen( $ps_output_path . ".tpc", "w" ) ) {
			...
			if ( ! fwrite( $fh, pack( "NNNNNNnnNN", 40, $base_width, $base_height, $pa_options["tile_width"],
				$pa_options["tile_height"], $tiles, $pa_options["layers"], $pa_options["layer_ratio"],
				strlen( $attribute_list ), $attributes ) )
			) {
	...
	public function setResourceLimits_imagick( $po_handle ) {
		$po_handle->setResourceLimit( imagick::RESOURCETYPE_MEMORY, 1024 * 1024
		                                                            * 1024 );        // Set maximum amount of memory in bytes to allocate for the pixel cache from the heap.
		$po_handle->setResourceLimit( imagick::RESOURCETYPE_MAP,
			1024 * 1024 * 1024 );        // Set maximum amount of memory map in bytes to allocate for the pixel cache.
		$po_handle->setResourceLimit( imagick::RESOURCETYPE_AREA, 6144
		                                                          * 6144 );            // Set the maximum width * height of an image that can reside in the pixel cache memory.
		$po_handle->setResourceLimit( imagick::RESOURCETYPE_FILE,
			1024 );                    // Set maximum number of open pixel cache files.
		$po_handle->setResourceLimit( imagick::RESOURCETYPE_DISK, 64 * 1024 * 1024
		                                                          * 1024 );                    // Set maximum amount of disk space in bytes permitted for use by the pixel cache.
	...
	function encode_imagick( $ps_filepath, $ps_output_path, $pa_options ) {
		...
		if ( $pa_options["scale_factor"] != 1 ) {
			...
			if ( ! $h->resizeImage( $image_width, $image_height, imagick::FILTER_CUBIC,
				$pa_options["antialiasing"] )
			) {
				$this->error = "Couldn't scale image";
	...
	function encode_gmagick( $ps_filepath, $ps_output_path, $pa_options ) {
		...
		if ( function_exists( 'exif_read_data' ) && ! ( $this->opo_config->get( 'dont_use_exif_read_data' ) ) ) {
			...
			if ( abs( $rotation )
			     === 90
			) {    // flip dimensions due to EXIF rotation (Gmagick doesn't update dimensions properly)
				$d                          = $image_dimensions;
		...
		if ( $pa_options["scale_factor"] != 1 ) {
			...
			if ( ! $h->resizeimage( $image_width, $image_height, Gmagick::FILTER_CUBIC,
				$pa_options["antialiasing"] )
			) {
				$this->error = "Couldn't scale image";
		...
		for ( $l = $pa_options["layers"]; $l >= 1; $l -- ) {
			...
			while ( $y < $image_height ) {
				...
				if ( $rotation == 180 ) {
					$slice->rotateimage( "#FFFFFF", $rotation );
				}
				...
				if ( abs( $rotation )
				     === 90
				) {    // flip rotation due to EXIF rotation (Gmagick doesn't set EXIF rotation properly)
	...
	function encode_gd( $ps_filepath, $ps_output_path, $pa_options ) {
			...
			switch ( $va_info[2] ) {
				...
				case IMAGETYPE_JPEG:
					...
					if ( function_exists( 'exif_read_data' )
					     && ! ( $this->opo_config->get( 'dont_use_exif_read_data' ) )
					) {
						if ( is_array( $va_exif = @exif_read_data( $ps_filepath, 'EXIF', true, false ) ) ) {
							if ( isset( $va_exif['IFD0']['Orientation'] ) ) {
								$vn_orientation = $va_exif['IFD0']['Orientation'];
		...
		if ( $pa_options["scale_factor"] != 1 ) {
			...
			if ( imagecopyresampled( $r_new_image, $r_image, 0, 0, 0, 0, $image_width, $image_height, $va_info[0],
				$va_info[1] )
			) {
		...
		for ( $l = $pa_options["layers"]; $l >= 1; $l -- ) {
			...
			if ( $l < $pa_options["layers"] ) {
				...
				$r_color     = ImageColorAllocate( $r_new_image, 255, 255, 255 );
				imagefilledrectangle( $r_new_image, 0, 0, $image_width - 1, $image_height - 1, $r_color );
				if ( ! imagecopyresampled( $r_new_image, $r_image, 0, 0, 0, 0, $image_width, $image_height,
	...
	function getLayer_imagick( $layer_number, $magick ) {
		...
		$h = new Imagick();
		$this->setResourceLimits_imagick( $h );
		if ( $h->newImage( $layer_tiles[ $layer_number ]["width"], $layer_tiles[ $layer_number ]["height"],
			"#ffffff" )
		) {
	...
	function getLayer_gd( $layer_number, $output_mimetype ) {
		...
		$r_new_image = imagecreatetruecolor( $layer_tiles[ $layer_number ]["width"],
			$layer_tiles[ $layer_number ]["height"] );
		if ( ! $r_new_image ) {
	...
	function getFileGeometry() {
		...
		for ( $l = 1; $l <= $this->properties['layers']; $l ++ ) {
			...
			$layer_tiles[ $l ] = array(
				...
				"effective_tile_height" => $effective_tile_height,
				"scale_factor"          => $scale_factor,
				"horizontal_tiles"      => ceil( $width_tiles ),
				"vertical_tiles"        => ceil( $height_tiles ),
				"width"                 => intval( $width_tiles * $this->properties['tile_width'] ),
				"height"                => intval( $height_tiles * $this->properties['tile_height'] ),
			...
			if ( preg_match( "/TPC\n/", $sig ) ) {
				...
				if ( ! fseek( $fh, ( $x['header_size'] ) + ( ( $pn_tile_number - 1 ) * 4 ) ) ) {
					...
					if ( ! fseek( $fh, $x["offset"] ) ) {
						...
						if ( $pb_print_errors ) {
							print "File seek error while getting tile; tried to seek to " . $x["offset"]
							      . " and read $vn_len bytes";
						}
						fclose( $fh );
