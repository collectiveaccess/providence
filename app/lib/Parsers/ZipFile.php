...
class ZipFile {
	 ...
	 *                            compression = a number between 0 and 9 where zero is no compression (fastest) and 9
	 *                            is most compression (slowest). Default is to use PHP default compression level.
	 *
	 * @access public
	 */
	public function addFile( $data, $name, $time = 0, $pa_options = null ) {
		...
		if ( ( strlen( $data ) <= 255 )
		     && ( ( preg_match( "/^\/.*[A-Za-z0-9_]{1}$/", $data ) )
		          || ( preg_match( "/^[A-Za-z]{1}:.*[A-Za-z0-9_]{1}$/", $data ) ) )
		) {
			$path = $data;
			if ( $fp = fopen( $path, "r" ) ) {
				$l    = filesize( $path );
		...
		$zdata   = gzcompress( $data,
			( isset( $pa_options['compression'] ) && ( (int) $pa_options['compression'] >= 0 ) )
				? (int) $pa_options['compression'] : - 1 );

		$data = "";

		$c_len = strlen( $zdata )
		         - 6; // fix crc bug by triming first two chars and last four (actual trimming is done below)
		$fr    .= pack( 'V', $crc );             // crc32
	 ...
	 *                    ZIPFILE_RETURN_STRING = return output as string; this will consume as much memory as the
	 *                    total output size and should only be used for small archives. ZIPFILE_PASSTHRU = pass through
	 *                    compressed data to standard output ZIPFILE_FILEPATH = return a path to the temporary file
	 *                    containing the ZIP archive output The default is to return a file handle.
	 *
