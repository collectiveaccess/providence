...
class ZipFile
{
    ...
    public function addFile($data, $name, $time = 0, $pa_options = null)
    {
        ...
        if ((strlen($data) <= 255) && ((preg_match("/^\/.*[A-Za-z0-9_]{1}$/", $data)) || (preg_match(
                    "/^[A-Za-z]{1}:.*[A-Za-z0-9_]{1}$/",
                    $data
                )))) {
            $path = $data;
            if ($fp = fopen($path, "r")) {
        ...
        $zdata = gzcompress(
            $data,
            (isset($pa_options['compression']) && ((int)$pa_options['compression'] >= 0)) ? (int)$pa_options['compression'] : -1
        );
