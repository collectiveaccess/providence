...
class ZipStream {
	 ...
	 *                           crc = Precomputed CRC to use for file. This can save time as the costliest operation
	 *                           for very large files in calculation of the CRC. CRC should be numeric. [Default is
	 *                           null]
	 *
	 * @access public
	 */
	...
	private function _stream( $pa_options = null ) {
		...
		foreach ( $this->opa_file_list as $vs_name => $va_file ) {
			...
			if ( $r_in !== false ) {
				$vs_hex = $vs_dtime[6] . $vs_dtime[7] . $vs_dtime[4] . $vs_dtime[5] . $vs_dtime[2] . $vs_dtime[3]
				          . $vs_dtime[0] . $vs_dtime[1];
				...
				if ( $vb_need_zip64 ) {
					...
					$vs_extended_info .= pack( "P", - 1 ) . pack( "P", - 1 ) . pack( "P", $vn_old_offset ) . pack( "V",
							0 );
				}
				...
				$r_fltr = stream_filter_append( $r_in, "zlib.deflate", STREAM_FILTER_READ,
					array( 'level' => isset( $pa_options['compression'] ) ? (int) $pa_options['compression'] : 0 ) );
				...
				if ( $vb_need_zip64 ) {
					...
					$vs_extended_info .= pack( "P", $va_file['size'] ) . pack( "P", $vn_compressed_filesize )
					                     . pack( "P", $vn_old_offset ) . pack( "V", 0 );
				}
				...
				$vs_central_directory_entry .= $vb_need_zip64 ? "\xFF\xFF\xFF\xFF"
					: pack( 'V', $vn_compressed_filesize );       // compressed filesize
				$vs_central_directory_entry .= $vb_need_zip64 ? "\xFF\xFF\xFF\xFF"
					: pack( 'V', $va_file['size'] );             // uncompressed filesize
				...
				$vs_central_directory_entry .= pack( 'V',
					32 );                             // external file attributes - 'archive' bit set

				$vs_central_directory_entry .= $vb_need_zip64 ? "\xFF\xFF\xFF\xFF"
					: pack( 'V', $vn_old_offset );                 // relative offset of local header
		...
		if ( $vb_need_zip64 ) {
			fwrite( $r_out, $vs_zip64_ctrl_dir = "\x50\x4b\x06\x06" .
			                                     ...
			                                     pack( 'v', $vs_extract_version )
			                                     .              // version needed to extract (2)

			                                     pack( 'V', 0 ) .              // number of this disk (4)
			                                     pack( 'V', 0 )
			                                     .              // number of disk with start of central directory (4)
			                                     pack( 'P', sizeof( $va_ctrl_dir ) )
		...
		if ( $vb_need_zip64 ) {
			fwrite( $r_out, $vs_end_of_zip64_ctrl_dir = "\x50\x4b\x06\x07\x00\x00\x00\x00" .
			                                            pack( 'P', $vn_zip64_ctrl_dir_offset )
	 ...
	 *
	 * @param array $pa_options An array of options. Supported options are:
	 *                          compression = level of ZLIB compression to apply. Set to zero for no compression.
	 *                          [Default is 0] timeLimit = number of seconds to set PHP execution time limit too. Set
	 *                          to zero to suspend time limit. [Default is 0] useZIP64 = force use or disuse of ZIP64
	 *                          format. [Default is to use ZIP64 only if archive is over 3.9gigs in size] dontWrite =
	 *                          send output to /dev/null. Used for debugging purposes [Default is false]
	 */
