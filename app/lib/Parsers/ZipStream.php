...
class ZipStream
{
    ...
    private function _stream($pa_options = null)
    {
        ...
        foreach ($this->opa_file_list as $vs_name => $va_file) {
            ...
            if ($r_in !== false) {
                ...
                if ($vb_need_zip64) {
                    ...
                    $vs_extended_info .= pack("P", $va_file['size']) . pack("P", $vn_compressed_filesize) . pack(
                            "P",
                            $vn_old_offset
                        ) . pack("V", 0);
                }
                ...
                $vs_central_directory_entry .= $vb_need_zip64 ? "\xFF\xFF\xFF\xFF" : pack(
                    'V',
                    $vn_compressed_filesize
                );       // compressed filesize
                $vs_central_directory_entry .= $vb_need_zip64 ? "\xFF\xFF\xFF\xFF" : pack(
                    'V',
                    $va_file['size']
                ...
                $vs_central_directory_entry .= pack(
                    'V',
                    32
        ...
        if ($vb_need_zip64) {
            fwrite(
                $r_out,
                $vs_zip64_ctrl_dir = "\x50\x4b\x06\x06" .
        ...
        if ($vb_need_zip64) {
            fwrite(
                $r_out,
                $vs_end_of_zip64_ctrl_dir = "\x50\x4b\x06\x07\x00\x00\x00\x00" .
        ...
        fwrite(
            $r_out,
            $vs_end_of_ctrl_dir = "\x50\x4b\x05\x06\x00\x00\x00\x00" .
