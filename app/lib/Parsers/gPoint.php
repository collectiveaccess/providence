...
class gPoint
{
    ...
    */
    ...
    public $Xp, $Yp;                                // X,Y pixel location
    public $lat, $long;                            // Latitude & Longitude of the point
    public $utmNorthing, $utmEasting, $utmZone;    // UTM Coordinates of the point
    public $lccNorthing, $lccEasting;                // Lambert coordinates of the point
    public $falseNorthing, $falseEasting;            // Origin coordinates for Lambert Projection
...
//
    public function convertLLtoTM($LongOrigin = null)
    {
        ...
        $M = $this->a * ((1 - $this->e2 / 4 - 3 * $this->e2 * $this->e2 / 64 - 5 * $this->e2 * $this->e2 * $this->e2 / 256) * $LatRad
                - (3 * $this->e2 / 8 + 3 * $this->e2 * $this->e2 / 32 + 45 * $this->e2 * $this->e2 * $this->e2 / 1024) * sin(
                    2 * $LatRad
                )
...
//
    public function UTMLetterDesignator()
    {
        if ((84 >= $this->lat) && ($this->lat >= 72)) {
            ...
            if ((72 > $this->lat) && ($this->lat >= 64)) {
                $LetterDesignator = 'W';
            } else {
                if ((64 > $this->lat) && ($this->lat >= 56)) {
                    $LetterDesignator = 'V';
                } else {
                    if ((56 > $this->lat) && ($this->lat >= 48)) {
                        $LetterDesignator = 'U';
                    } else {
                        if ((48 > $this->lat) && ($this->lat >= 40)) {
                            $LetterDesignator = 'T';
                        } else {
                            if ((40 > $this->lat) && ($this->lat >= 32)) {
                                $LetterDesignator = 'S';
                            } else {
                                if ((32 > $this->lat) && ($this->lat >= 24)) {
                                    $LetterDesignator = 'R';
                                } else {
                                    if ((24 > $this->lat) && ($this->lat >= 16)) {
                                        $LetterDesignator = 'Q';
                                    } else {
                                        if ((16 > $this->lat) && ($this->lat >= 8)) {
                                            $LetterDesignator = 'P';
                                        } else {
                                            if ((8 > $this->lat) && ($this->lat >= 0)) {
                                                $LetterDesignator = 'N';
                                            } else {
