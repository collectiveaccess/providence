...
class DelimitedDataParser implements IDataParser {
	...
	/**
	 * @mixed Parsed file input. For text files, a PHP file resource; for Excel files a
	 *        \PhpOffice\PhpSpreadsheet\Spreadsheet row iterator instance.
	 */
	private $opr_file;
	 ...
	 *                            textMarker = Marker used to enclose text blocks in text files. [Default is double
	 *                            quote (")] worksheet = Worksheet number to read from when parsing Excel files.
	 *                            [Default is 0]
	 *
	 * @return DelimitedDataParser
	 */
	static public function load( $ps_filepath, $pa_options = null ) {
		return new DelimitedDataParser( caGetOption( 'delimiter', $pa_options, "\t" ),
			caGetOption( 'textMarker', $pa_options, '"' ), $ps_filepath, $pa_options );
	}
	# ----------------------------------------
			...
			if ( $o_row = $this->opr_file->current() ) {
				...
				foreach ( $o_cells as $o_cell ) {
					if ( \PhpOffice\PhpSpreadsheet\Shared\Date::isDateTime( $o_cell ) ) {
						if ( ! ( $vs_val
							= caGetLocalizedDate( \PhpOffice\PhpSpreadsheet\Shared\Date::excelToTimestamp( trim( (string) $o_cell->getValue() ) ) ) )
						) {
							if ( ! ( $vs_val
								= trim( \PhpOffice\PhpSpreadsheet\Style\NumberFormat::toFormattedString( (string) $o_cell->getValue(),
								'YYYY-MM-DD' ) ) )
							) {
