...
class DelimitedDataParser implements IDataParser
{
    ...
    public static function load($ps_filepath, $pa_options = null)
    {
        return new DelimitedDataParser(
            caGetOption('delimiter', $pa_options, "\t"),
            caGetOption('textMarker', $pa_options, '"'),
            $ps_filepath,
            $pa_options
        );
    }
    ...
    public function parse($ps_filepath, $pa_options = null)
    {
        ...
        try {
            ...
            foreach ($va_excel_types as $vs_type) {
                $o_reader = IOFactory::createReader($vs_type);
            ...
            $this->opo_excel = IOFactory::load($ps_filepath);
        ...
        if ($this->ops_type == 'xlsx') {
            ...
            if ($o_row = $this->opr_file->current()) {
                ...
                foreach ($o_cells as $o_cell) {
                    if (Date::isDateTime($o_cell)) {
                        if (!($vs_val = caGetLocalizedDate(
                            Date::excelToTimestamp(trim((string)$o_cell->getValue()))
                        ))) {
                            if (!($vs_val = trim(
                                NumberFormat::toFormattedString(
                                    (string)$o_cell->getValue(),
                                    'YYYY-MM-DD'
                                )
                            ))) {
