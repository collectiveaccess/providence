...
class Tokenizer_Base {
	...
	function addError( $error ) {
		$this->errors[] = htmlentities( $error . ' at ' . ( $this->line_pos[0] + 1 ) . ', ' . ( $this->pos
		                                                                                        - $this->line_pos[1]
		                                                                                        + 1 ) . '!' );
	}
	...
	protected function expect( $token, $do_next = true, $try_next = false, $next_on_match = 1 ) {
		...
		if ( is_int( $token ) ) {
			...
			if ( ( $this->doc[ $this->pos ] !== $token )
			     && ( ( ! $try_next )
			          || ( ( ( ( $try_next === 1 )
			                   && ( $this->next() !== self::TOK_NULL ) )
			                 || ( ( $try_next === true )
			                      && ( $this->next_no_whitespace() !== self::TOK_NULL ) ) )
			               && ( $this->doc[ $this->pos ] !== $token ) ) )
			) {
...
class HTML_Parser_Base extends Tokenizer_Base {
	...
	function parse_doctype() {
		...
		if ( $this->next_search( '[>', false ) === self::TOK_UNKNOWN ) {
			if ( $this->doc[ $this->pos ] === '[' ) {
				if ( ( $this->next_pos( ']', false ) !== self::TOK_UNKNOWN )
				     || ( $this->next_pos( '>', false ) !== self::TOK_UNKNOWN )
				) {
	...
	function parse_style() {
		if ( $this->parse_attributes() && ( $this->token === self::TOK_TAG_CLOSE ) && ( $start = $this->pos )
		     && ( $this->next_pos( '</style>', false ) === self::TOK_UNKNOWN )
		) {
			$len                      = $this->pos - 1 - $start;
			$this->status['text']     = ( ( $len > 0 ) ? substr( $this->doc, $start + 1, $len ) : '' );
			$this->pos                += 7;
	...
	function parse_script() {
		if ( $this->parse_attributes() && ( $this->token === self::TOK_TAG_CLOSE ) && ( $start = $this->pos )
		     && ( $this->next_pos( '</script>', false ) === self::TOK_UNKNOWN )
		) {
			$len                      = $this->pos - 1 - $start;
			$this->status['text']     = ( ( $len > 0 ) ? substr( $this->doc, $start + 1, $len ) : '' );
			$this->pos                += 8;
	...
	function parse_tag_default() {
		...
		if ( $this->token !== self::TOK_TAG_CLOSE ) {
			if ( $this->token === self::TOK_SLASH_FORWARD ) {
				...
				$this->pos ++;
				if ( isset( $this->char_map[ $this->doc[ $this->pos ] ] )
				     && ( ! is_string( $this->char_map[ $this->doc[ $this->pos ] ] ) )
				) {
	...
	function parse_tag() {
		...
		if ( $next === '!' ) {
			...
			if ( substr( $this->doc, $this->pos + 2, 2 ) === '--' ) {
				...
				if ( ( $this->doc[ $this->pos + 4 ] === '[' )
				     && ( strcasecmp( substr( $this->doc, $this->pos + 5, 2 ), 'if' ) === 0 )
				) {
...
class HTML_Parser extends HTML_Parser_Base {
	...
	protected function parse_hierarchy( $self_close = null ) {
		if ( $self_close === null ) {
			$this->status['self_close'] = ( $self_close
				= isset( $this->tags_selfclose[ strtolower( $this->status['tag_name'] ) ] ) );
		}
		if ( $self_close ) {
			if ( $this->status['closing_tag'] ) {
				...
				$index = null;
				$this->hierarchy[ count( $this->hierarchy ) - 1 ]->addXML( $this->status['tag_name'], '',
					$this->status['attributes'], $index );
			} elseif ( $this->status['tag_name'][0] === '%' ) {
				$index = null;
				$this->hierarchy[ count( $this->hierarchy ) - 1 ]->addASP( $this->status['tag_name'], '',
					$this->status['attributes'], $index );
			} else {
				$index = null;
			...
			for ( $count = count( $this->hierarchy ), $i = $count - 1; $i >= 0; $i -- ) {
				if ( strcasecmp( $this->hierarchy[ $i ]->tag, $this->status['tag_name'] ) === 0 ) {
					for ( $ii = ( $count - $i - 1 ); $ii >= 0; $ii -- ) {
						...
						if ( $ii > 0 ) {
							$this->addError( 'Closing tag "' . $this->status['tag_name'] . '" while "' . $e->tag
							                 . '" is not closed yet' );
						}
					}
					$found = true;
	...
	function parse_conditional() {
		...
		if ( $this->status['comment'] ) {
			$index = null;
			$e     = $this->hierarchy[ count( $this->hierarchy ) - 1 ]->addConditional( $this->status['tag_condition'],
				true, $index );
			if ( $this->status['text'] !== '' ) {
				$index = null;
			...
			if ( $this->status['closing_tag'] ) {
				...
				$index             = null;
				$this->hierarchy[] = $this->hierarchy[ count( $this->hierarchy )
				                                       - 1 ]->addConditional( $this->status['tag_condition'], false,
					$index );
			}
		}
...
class HTML_Parser_HTML5 extends HTML_Parser {
	...
	protected function parse_hierarchy( $self_close = null ) {
		...
		if ( ! ( $self_close || $this->status['closing_tag'] ) ) {
			...
			if ( isset( $this->tags_optional_close[ $tag_curr ] )
			     && isset( $this->tags_optional_close[ $tag_curr ][ $tag_prev ] )
			) {
...
class HTML_Node {
	...
	function dumpLocation() {
		return ( ( $this->parent ) ? ( ( $p = $this->parent->dumpLocation() ) ? $p . ' > ' : '' ) . $this->tag . '('
		                             . $this->typeIndex() . ')' : '' );
	}
	...
	protected function toString_attributes() {
		...
		foreach ( $this->attributes as $a => $v ) {
			$s .= ' ' . $a . ( ( ( ! $this->attribute_shorttag ) || ( $this->attributes[ $a ] !== $a ) ) ? '="'
			                                                                                               . htmlspecialchars( $this->attributes[ $a ],
						ENT_QUOTES, '', false ) . '"' : '' );
		}
	...
	function getChildrenByMatch( $conditions, $recursive = true, $check_self = false, $custom_filters = array() ) {
		...
		if ( $count > 0 ) {
			...
			for ( $i = 0; $i < $count; $i ++ ) {
				...
				if ( $recursive ) {
					$res = array_merge( $res,
						$this->children[ $i ]->getChildrenByMatch( $conditions, $recursive, false, $custom_filters ) );
				}
			}
		}
					...
					foreach ( $possibles as $a ) {
						...
						switch ( $match['operator_value'] ) {
							...
							case 'contains_prefix':
								$res = ( ( preg_match( '`\b' . preg_quote( $match['value'] ) . '[\-\s]?`s', $val ) > 0 )
								         === $match['match'] );
								if ( $res ) {
									break 1;
								} else {
							...
							case '~=':
							case 'contains_word':
								$res = ( ( preg_match( '`\b' . preg_quote( $match['value'] ) . '\b`s', $val ) > 0 )
								         === $match['match'] );
								if ( $res ) {
									break 1;
								} else {
							...
							case '$=':
							case 'ends_with':
								$res = ( ( substr( $val, - strlen( $match['value'] ) ) === $match['value'] )
								         === $match['match'] );
								if ( $res ) {
									break 1;
								} else {
							...
							case '^=':
							case 'starts_with':
								$res = ( ( substr( $val, 0, strlen( $match['value'] ) ) === $match['value'] )
								         === $match['match'] );
								if ( $res ) {
									break 1;
								} else {
							...
							default:
								trigger_error( 'Unknown operator "' . $match['operator_value']
								               . '" to match attributes!' );
	...
	protected function parse_conditions() {
		...
		while ( $tok !== Tokenizer_CSSQuery::TOK_NULL ) {
			...
			if ( $tok === Tokenizer_CSSQuery::TOK_ALL ) {
				$tok = $p->next();
				if ( ( $tok === Tokenizer_CSSQuery::TOK_PIPE ) && ( $tok = $p->next() )
				     && ( $tok !== Tokenizer_CSSQuery::TOK_ALL )
				) {
				...
				while ( true ) {
					...
					switch ( $tok ) {
						...
						case Tokenizer_CSSQuery::TOK_COMPARE_SMALLER_THAN:
							$operator_value = $p->getTokenString( ( $tok === Tokenizer_CSSQuery::TOK_COMPARE_EQUALS )
								? 0 : - 1 );
			...
			while ( $tok === Tokenizer_CSSQuery::TOK_COLON ) {
				...
				if ( ( $tok = $p->next() ) === Tokenizer_CSSQuery::TOK_BRACE_OPEN ) {
					$start = $p->pos;
					$count = 1;
					while ( ( ( $tok = $p->next() ) !== Tokenizer_CSSQuery::TOK_NULL )
					        && ! ( ( $tok === Tokenizer_CSSQuery::TOK_BRACE_CLOSE )
					               && ( -- $count === 0 ) ) ) {
...
class HTML_Formatter {
	...
	static function minify_html( &$root, $strip_comments = true, $recursive = true ) {
		...
		foreach (
			$root->select( '(!pre + !xmp + !style + !script + !"?php" + !"~text~" + !"~comment~"):not-empty > "~text~"',
				false, $recursive, true ) as $c
		) {
	...
	function format_html( &$root, $recursive = null ) {
		...
		for ( $i = 0; $i < $child_count; $i ++ ) {
			...
			if ( ! $n->isText() ) {
				...
				$new_line      = isset( $this->block_elements[ $n_tag ] )
				                 && $this->block_elements[ $n_tag ]['new_line'];
				$as_block      = isset( $this->block_elements[ $n_tag ] )
				                 && $this->block_elements[ $n_tag ]['as_block'];
				$format_inside = ( ( ! isset( $this->block_elements[ $n_tag ] ) )
				                   || $this->block_elements[ $n_tag ]['format_inside'] );
				if ( $prev && ( $prev->isText() ) && $prev->text && ( $char = $prev->text[ strlen( $prev->text ) - 1 ] )
				     && isset( $this->whitespace[ $char ] )
				) {
					if ( $this->whitespace[ $char ] ) {
						$prev->text .= str_repeat( $this->indent_string, $indent );
					} else {
						$prev->text = substr_replace( $prev->text,
							$this->linebreak_string . str_repeat( $this->indent_string, $indent ), - 1, 1 );
					}
				...
				if ( $format_inside && count( $n->children ) ) {
					...
					$last_asblock = ( $last_tag && isset( $this->block_elements[ $last_tag ] )
					                  && $this->block_elements[ $last_tag ]['as_block'] );
					if ( ( $n->childCount( true ) > 0 ) || ( trim( $n->getPlainText() ) ) ) {
						if ( $last && ( $last->isText() ) && $last->text
						     && ( $char = $last->text[ strlen( $last->text ) - 1 ] )
						     && isset( $this->whitespace[ $char ] )
						) {
							if ( $as_block || ( $last->index() > 0 ) || isset( $this->whitespace[ $last->text[0] ] ) ) {
								if ( $this->whitespace[ $char ] ) {
									...
									$last->text = substr_replace( $last->text,
										$this->linebreak_string . str_repeat( $this->indent_string, $indent ), - 1, 1 );
								}
							}
				...
				if ( isset( $this->whitespace[ $char ] ) ) {
					if ( $this->whitespace[ $char ] ) {
						...
						$n->text = substr_replace( $n->text,
							$this->linebreak_string . str_repeat( $this->indent_string, $indent ), 0, 1 );
					}
				} else {
