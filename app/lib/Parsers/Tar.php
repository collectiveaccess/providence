...
class Archive_Tar extends PEAR
{
    ...
    function _readHeader($v_binary_data, &$v_header)
    {
        ...
        if ($v_header['checksum'] != $v_checksum) {
            ...
            $this->_error(
                'Invalid checksum for file "' . $v_data['filename'] . '" : ' . $v_checksum . ' calculated, ' . $v_header['checksum'] . ' expected'
            );
            return false;
        }
    ...
    function _extractList($p_path, &$p_list_detail, $p_mode, $p_file_list, $p_remove_path)
    {
        ...
        if ($p_path == '' || (substr($p_path, 0, 1) != '/' && substr($p_path, 0, 3) != "../" && !strpos(
                    $p_path,
                    ':'
                ))) {
            $p_path = "./" . $p_path;
            ...
            if ((!$v_extract_all) && (is_array($p_file_list))) {
                ...
                for ($i = 0; $i < sizeof($p_file_list); $i++) {
                    ...
                    if (substr($p_file_list[$i], -1) == '/') {
                        ...
                        if ((strlen($v_header['filename']) > strlen($p_file_list[$i])) && (substr(
                                    $v_header['filename'],
                                    0,
                                    strlen(
                                        $p_file_list[$i]
                ...
                if ($v_extract_file) {
                    if ($v_header['typeflag'] == "5") {
                        ...
                        if (filesize($v_header['filename']) != $v_header['size']) {
                            $this->_error(
                                'Extracted file ' . $v_header['filename'] . ' does not have the correct file size \'' . filesize(
                                    $v_filename
    ...
    function _openAppend()
    {
        ...
        if ($this->_compress) {
            ...
            if (!@rename($this->_tarname, $this->_tarname . ".tmp")) {
                $this->_error(
                    'Error while renaming \'' . $this->_tarname . '\' to temporary file \'' . $this->_tarname . '.tmp\''
                );
                return false;
            }
