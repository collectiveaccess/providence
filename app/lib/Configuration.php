 ...
 * - List: a list of strings or numbers separated by commas and enclosed in square brackets ("[" and "]"). A string
 * must be enclosed in double quotes if it contains a comma. You may not place the double quote character in a list
 * item. Lists are retrievable as indexed PHP arrays. Lists may not be nested.
 * - Associative array: a list of key-value pairs. Both keys and values must be enclosed in double quotes if they
 * contain commas. Neither may contain double quotes. Associative arrays are enclosed with curly brackets ("{" and
 * "}"). Separate keys from values with "=" Separate key-value pairs from each other using commas. Values may be
 * strings, numbers or nested associative arrays. Associative arrays may be nested to any depth.
 *
 * Keys are always separated from values by "=" You may place as many spaces as you like on either side of the "="
 * character.
 *
...
class Configuration {
	 ...
	 * @param bool   $pb_dont_cache_instance         Don't attempt to cache config file Configuration instance.
	 *                                               [Default is false]
	 * @param bool   $pb_dont_load_from_default_path Don't attempt to load additional configuration files from default
	 *                                               paths (defined by __CA_LOCAL_CONFIG_DIRECTORY__ and
	 *                                               __CA_LOCAL_CONFIG_DIRECTORY__). [Default is false]
	 *
	 * @return Configuration
	 */
	static function load(
		$ps_file_path = __CA_APP_CONFIG__, $pb_dont_cache = false, $pb_dont_cache_instance = false,
		$pb_dont_load_from_default_path = false
	) {
		if ( ! $ps_file_path ) {
			$ps_file_path = __CA_APP_CONFIG__;
		}

		if ( ! MemoryCache::contains( $ps_file_path, 'ConfigurationInstances' ) || $pb_dont_cache
		     || $pb_dont_cache_instance
		) {
			MemoryCache::save( $ps_file_path,
				new Configuration( $ps_file_path, true, $pb_dont_cache, $pb_dont_load_from_default_path ),
	...
	/* ---------------------------------------- */
	/**
	 * Load a configuration file. In addition to the parameters described below two global variables can also affect
	 * loading:
	 *
	 *        $g_ui_locale - if it contains the current locale code, this code will be used when computing the MD5
	 *        signature of the current configuration for caching purposes. By setting this to the current locale
	 *        simultaneous caching of configurations for various locales (eg. config files with gettext-translated
	 *        strings in them) is enabled.
	 *        $g_configuration_cache_suffix - any text it contains is used along with the configuration path and
	 *        $g_ui_locale to compute the MD5 signature of the current configuration for caching purposes. By setting
	 *        this to some value you can support simultaneous caching of configurations for several different modes.
	 *        This is mainly used to support caching of theme-specific configurations. Since the theme can change based
	 *        upon user agent, we need to potentially keep several computed configurations cached at the same time, one
	 *        for each theme used.
	 *
	 * @param string $ps_file_path                   Absolute path to configuration file to parse
	 * @param bool   $pb_die_on_error                If true, request processing will halt with call to die() on error
	 *                                               in parsing config file. [Default is false]
	 * @param bool   $pb_dont_cache                  If true, file will be parsed even if it's already cached. [Default
	 *                                               is false]
	 * @param bool   $pb_dont_load_from_default_path Don't attempt to load additional configuration files from default
	 *                                               paths (defined by __CA_LOCAL_CONFIG_DIRECTORY__ and
	 *                                               __CA_LOCAL_CONFIG_DIRECTORY__). [Default is false]
	...
	public function __construct(
		$ps_file_path = __CA_APP_CONFIG__, $pb_die_on_error = false, $pb_dont_cache = false,
		$pb_dont_load_from_default_path = false
	) {
		...
		$vs_path_as_md5 = md5( $_SERVER['HTTP_HOST'] . $this->ops_config_file_path . '/' . $g_ui_locale
		...
		if ( ! $pb_dont_load_from_default_path ) {
			if ( defined( '__CA_LOCAL_CONFIG_DIRECTORY__' )
			     && file_exists( __CA_LOCAL_CONFIG_DIRECTORY__ . '/' . $vs_config_filename )
			) {
				$va_config_file_list[]
					= $vs_top_level_config_path = __CA_LOCAL_CONFIG_DIRECTORY__ . '/' . $vs_config_filename;
			}

			// Theme config overrides local config
			if ( defined( '__CA_DEFAULT_THEME_CONFIG_DIRECTORY__' )
			     && file_exists( __CA_DEFAULT_THEME_CONFIG_DIRECTORY__ . '/' . $vs_config_filename )
			) {
				$va_config_file_list[]
					= $vs_top_level_config_path = __CA_DEFAULT_THEME_CONFIG_DIRECTORY__ . '/' . $vs_config_filename;
			}

			// Appname-specific config overrides local config
			$appname_specific_path = __CA_LOCAL_CONFIG_DIRECTORY__ . '/' . pathinfo( $vs_config_filename,
					PATHINFO_FILENAME ) . '_' . __CA_APP_NAME__ . '.' . pathinfo( $vs_config_filename,
					PATHINFO_EXTENSION );
		...
		$o_config = ( $vs_top_level_config_path === $this->ops_config_file_path ) ? $this
			: Configuration::load( $vs_top_level_config_path, false, false, true );
		...
		if ( ( $vb_inherit_config = $o_config->get( 'allowThemeInheritance' ) ) && ! $pb_dont_load_from_default_path ) {
			...
			while ( $vs_inherit_from_theme = trim( trim( $o_config->get( [ 'inheritFrom', 'inherit_from' ] ) ),
				...
				if ( file_exists( $vs_inherited_config_path )
				     && ! in_array( $vs_inherited_config_path, $va_config_file_list )
		...
		if ( ( ! defined( '__CA_DISABLE_CONFIG_CACHING__' ) || ! __CA_DISABLE_CONFIG_CACHING__ ) && ! $pb_dont_cache ) {
			...
			if ( ! $vb_setup_has_changed && isset( self::$s_config_cache[ $vs_path_as_md5 ] ) ) {
				...
				foreach ( $va_config_file_list as $vs_config_file_path ) {
					...
					if ( $vs_config_mtime != self::$s_config_cache[ $k = 'mtime_' . $vs_path_as_md5
					                                                     . md5( $vs_config_file_path ) ]
	 ...
	 * @param $pn_num_lines_to_read - if set to a positive integer, will abort parsing after the first
	 *                              $pn_num_lines_to_read lines of the config file are read. This is useful for reading
	 *                              in headers in config files without having to parse the entire file.
	 *
	...
	public function loadFile( $ps_filepath, $pb_die_on_error = false, $pn_num_lines_to_read = null ) {
		...
		while ( ! feof( $r_file ) ) {
			...
			while ( sizeof( $va_tokens ) ) {
				...
				switch ( $vn_state ) {
					...
					case 10:
						switch ( $vs_token ) {
							case '[':
								if ( ! is_array( $this->ops_config_settings["lists"][ $vs_key ] )
								     || ! $vb_merge_mode
								) {
								...
								break;
							case '{':
								if ( ! is_array( $this->ops_config_settings["assoc"][ $vs_key ] )
								     || ! $vb_merge_mode
								) {
									...
									if ( ! $vn_in_quote ) {
										if ( sizeof( $va_tokens ) == 0 ) {
											$this->ops_config_settings["scalars"][ $vs_key ]
												      = $this->_trimScalar( $vs_scalar_value );
											$vn_state = - 1;
										}
									}
								}
						...
						switch ( $vs_token ) {
							...
							case ',':
								if ( $vn_in_quote || $vb_escape_set ) {
									...
									if ( strlen( $vs_item
											= trim( $this->_interpolateScalar( $this->_trimScalar( $vs_scalar_value ) ) ) )
									     > 0
									) {
					...
					# get associative key
					case 40:
						if ( $vb_quoted_item_is_closed
						     && ( ! in_array( trim( $vs_token ), [ ',', '=', '}', ')' ] ) )
						) {
							break;
						}
						switch ( $vs_token ) {
							...
							case ',':
								if ( $vn_in_quote || $vb_escape_set ) {
									...
									if ( $vs_assoc_key ) {
										$va_assoc_pointer_stack[ sizeof( $va_assoc_pointer_stack ) - 1 ][]
											= trim( $vs_assoc_key );
									}
									$vs_assoc_key             = "";
									$vs_scalar_value          = "";
						...
						switch ( $vs_token ) {
							...
							case ',':
								if ( $vn_in_quote || $vb_escape_set ) {
									...
									if ( $vs_assoc_key ) {
										$va_assoc_pointer_stack[ sizeof( $va_assoc_pointer_stack )
										                         - 1 ][ $vs_assoc_key ]
											= $this->_trimScalar( $this->_interpolateScalar( $vs_scalar_value ) );
									}
									$vs_assoc_key             = "";
									$vs_scalar_value          = "";
									$vn_state                 = 40;
							...
							case '{':
								if ( ! $vn_in_quote && ! $vb_escape_set ) {
									...
									if ( ! is_array( $va_assoc_pointer_stack[ $i ][ $vs_assoc_key ] )
									     || ! $vb_merge_mode
									) {
							...
							case '[':
								if ( $vn_in_quote || $vb_escape_set ) {
									...
									$i = sizeof( $va_assoc_pointer_stack ) - 1;
									if ( ! is_array( $va_assoc_pointer_stack[ sizeof( $va_assoc_pointer_stack )
									                                          - 1 ][ $vs_assoc_key ] )
									     || ! $vb_merge_mode
									) {
										$va_assoc_pointer_stack[ $i ][ $vs_assoc_key ] = array();
			...
			if ( $vn_in_quote && ! in_array( $vn_state, [ 10, 20 ] ) ) {
				switch ( $vn_state ) {
					...
					default:
						$this->ops_error
							= "Missing trailing quote in '$vs_key' [Last token was '{$vs_token}'; state was $vn_state]<br/><strong>Last "
							  . sizeof( $va_token_history ) . " tokens were: </strong>"
							  . $this->_formatTokenHistory( $va_token_history, array( 'outputAsHTML' => true ) );
						break;
				}
				fclose( $r_file );
		...
		if ( $vn_state > 0 ) {
			$this->ops_error = "Syntax error in configuration file: missing { or } [state=$vn_state]<br/><strong>Last "
			                   . sizeof( $va_token_history ) . " tokens were: </strong>"
			                   . $this->_formatTokenHistory( $va_token_history, array( 'outputAsHTML' => true ) );
			fclose( $r_file );
	...
	public function get( $pm_key ) {
		...
		foreach ( $pm_key as $ps_key ) {
			if ( isset( Configuration::$s_get_cache[ $this->ops_md5_path ][ $ps_key ] )
			     && Configuration::$s_get_cache[ $this->ops_md5_path ][ $ps_key ]
			) {
	 ...
	 *
	 * @return Opis\JsonSchema\ValidationResult Returns null if schema could not be loaded, either because it is
	...
	public function validate() {
		...
		$loader = new \Opis\JsonSchema\Loaders\File( "https://collectiveaccess.org",
			[ __CA_LIB_DIR__ . "/Configuration/" . ucfirst( strtolower( __CA_APP_TYPE__ ) ) . "/schemas" ] );
