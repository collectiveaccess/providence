...
class Configuration
{
    ...
    static public $s_get_cache;
    static public $s_config_cache = null;
    static public $s_have_to_write_config_cache = false;
    ...
    public static function load(
        ...
        if (!MemoryCache::contains(
                $ps_file_path,
                'ConfigurationInstances'
            ) || $pb_dont_cache || $pb_dont_cache_instance) {
            MemoryCache::save(
                $ps_file_path,
    ...
    public function __construct(
        ...
        $vs_path_as_md5 = md5(
            $_SERVER['HTTP_HOST'] . $this->ops_config_file_path . '/' . $g_ui_locale . (isset($g_configuration_cache_suffix) ? '/' . $g_configuration_cache_suffix : '')
        );
        ...
        if (!$pb_dont_load_from_default_path) {
            if (defined('__CA_LOCAL_CONFIG_DIRECTORY__') && file_exists(
                    __CA_LOCAL_CONFIG_DIRECTORY__ . '/' . $vs_config_filename
                )) {
            ...
            if (defined('__CA_DEFAULT_THEME_CONFIG_DIRECTORY__') && file_exists(
                    __CA_DEFAULT_THEME_CONFIG_DIRECTORY__ . '/' . $vs_config_filename
                )) {
            ...
            $appname_specific_path = __CA_LOCAL_CONFIG_DIRECTORY__ . '/' . pathinfo(
                    $vs_config_filename,
                    PATHINFO_FILENAME
                ) . '_' . __CA_APP_NAME__ . '.' . pathinfo($vs_config_filename, PATHINFO_EXTENSION);
        ...
        $o_config = ($vs_top_level_config_path === $this->ops_config_file_path) ? $this : Configuration::load(
            $vs_top_level_config_path,
            false,
            false,
            true
        );
        ...
        if (($vb_inherit_config = $o_config->get('allowThemeInheritance')) && !$pb_dont_load_from_default_path) {
            ...
            while ($vs_inherit_from_theme = trim(trim($o_config->get(['inheritFrom', 'inherit_from'])), "/")) {
                ...
                if (file_exists($vs_inherited_config_path) && !in_array(
                        $vs_inherited_config_path,
                        $va_config_file_list
        ...
        if ((!defined('__CA_DISABLE_CONFIG_CACHING__') || !__CA_DISABLE_CONFIG_CACHING__) && !$pb_dont_cache) {
            ...
            if (!$vb_setup_has_changed && isset(self::$s_config_cache[$vs_path_as_md5])) {
                ...
                foreach ($va_config_file_list as $vs_config_file_path) {
                    ...
                    if ($vs_config_mtime != self::$s_config_cache[$k = 'mtime_' . $vs_path_as_md5 . md5(
                                $vs_config_file_path
     ...
     * @return bool - returns true if parse succeeded, false if parse failed
     */
    public function loadFile($ps_filepath, $pb_die_on_error = false, $pn_num_lines_to_read = null)
    {
                                ...
                                if (!preg_match("/^[ \t]*$/", $vs_token)) {
                                    ...
                                    if (!$vn_in_quote) {
                                        if (sizeof($va_tokens) == 0) {
                                            $this->ops_config_settings["scalars"][$vs_key] = $this->_trimScalar(
                                                $vs_scalar_value
                                            );
                                            $vn_state = -1;
                                        }
                        ...
                        switch ($vs_token) {
                            ...
                            case ',':
                                if ($vn_in_quote || $vb_escape_set) {
                                    ...
                                    if (strlen(
                                            $vs_item = trim(
                                                $this->_interpolateScalar($this->_trimScalar($vs_scalar_value))
                                            )
                                        ) > 0) {
                        ...
                        switch ($vs_token) {
                            ...
                            case ',':
                                if ($vn_in_quote || $vb_escape_set) {
                                    ...
                                    if ($vs_assoc_key) {
                                        $va_assoc_pointer_stack[sizeof($va_assoc_pointer_stack) - 1][] = trim(
                                            $vs_assoc_key
                                        );
                                    }
                                    $vs_assoc_key = "";
                        ...
                        switch ($vs_token) {
                            ...
                            case ',':
                                if ($vn_in_quote || $vb_escape_set) {
                                    ...
                                    if ($vs_assoc_key) {
                                        $va_assoc_pointer_stack[sizeof(
                                            $va_assoc_pointer_stack
                                        ) - 1][$vs_assoc_key] = $this->_trimScalar(
                                            $this->_interpolateScalar($vs_scalar_value)
                                        );
                                    }
                                    $vs_assoc_key = "";
                                    $vs_scalar_value = "";
                            ...
                            case '[':
                                if ($vn_in_quote || $vb_escape_set) {
                                    ...
                                    $i = sizeof($va_assoc_pointer_stack) - 1;
                                    if (!is_array(
                                            $va_assoc_pointer_stack[sizeof($va_assoc_pointer_stack) - 1][$vs_assoc_key]
                                        ) || !$vb_merge_mode) {
                                        $va_assoc_pointer_stack[$i][$vs_assoc_key] = array();
                                    }
                                    $va_assoc_pointer_stack[] =& $va_assoc_pointer_stack[$i][$vs_assoc_key];
            ...
            if ($vn_in_quote && !in_array($vn_state, [10, 20])) {
                switch ($vn_state) {
                    ...
                    case 50:
                        ...
                        continue(2);    // allow multiline quoted entries
                    default:
                        $this->ops_error = "Missing trailing quote in '$vs_key' [Last token was '{$vs_token}'; state was $vn_state]<br/><strong>Last " . sizeof(
                                $va_token_history
                            ) . " tokens were: </strong>" . $this->_formatTokenHistory(
                                $va_token_history,
                                array('outputAsHTML' => true)
                            );
                        break;
                }
                fclose($r_file);
        ...
        if ($vn_state > 0) {
            $this->ops_error = "Syntax error in configuration file: missing { or } [state=$vn_state]<br/><strong>Last " . sizeof(
                    $va_token_history
                ) . " tokens were: </strong>" . $this->_formatTokenHistory(
                    $va_token_history,
                    array('outputAsHTML' => true)
    ...
    public function validate()
    {
        ...
        $loader = new \Opis\JsonSchema\Loaders\File(
            ...
            [
                __CA_LIB_DIR__ . "/Configuration/" . ucfirst(
                    strtolower(__CA_APP_TYPE__)
                ) . "/schemas"
            ]
        );
