...
class WidgetManager
{
     ...
     */
    static public $s_widget_manager_did_do_widget_init = false;

    /**
     *
     */
    static public $s_widget_instances = array();
    /**
     *
     */
    static public $s_widget_hooks = array();
    ...
    public static function getWidgetNames()
    {
        ...
        if (is_resource($r_dir = opendir($vs_base_widget_dir))) {
            while (($vs_widget_dir = readdir($r_dir)) !== false) {
                if (is_dir($vs_base_widget_dir . '/' . $vs_widget_dir) && preg_match(
                        "/^[A-Za-z_]+[A-Za-z0-9_]*$/",
                        $vs_widget_dir
                    )) {
    ...
    public static function checkWidgetStatus($ps_widget_name)
    {
        ...
        if (isset(WidgetManager::$s_widget_instances[$ps_widget_name]) && is_object(
                WidgetManager::$s_widget_instances[$ps_widget_name]
            )) {
    ...
    {
        WidgetManager::initWidgets();
        if (WidgetManager::$s_widget_instances[$ps_widget_name] && is_object(
                WidgetManager::$s_widget_instances[$ps_widget_name]
            )) {
            ...
            foreach ($va_available_settings as $vs_setting_name => $va_setting_info) {
                ...
                if (isset($va_setting_info['scope']) && ($va_setting_info['scope'] == 'application')) {
                    ...
                    $pa_settings[$vs_setting_name] = $o_appvar->getVar(
                        'widget_settings_' . $ps_widget_name . '_' . $vs_setting_name
                    );
                }
                $vs_buf .= WidgetManager::$s_widget_instances[$ps_widget_name]->settingHTMLFormElement(
                    $ps_widget_id,
                    $vs_setting_name,
