...
class WidgetManager {
	...
	public static function getWidgetNames() {
		...
		if ( is_resource( $r_dir = opendir( $vs_base_widget_dir ) ) ) {
			while ( ( $vs_widget_dir = readdir( $r_dir ) ) !== false ) {
				if ( is_dir( $vs_base_widget_dir . '/' . $vs_widget_dir )
				     && preg_match( "/^[A-Za-z_]+[A-Za-z0-9_]*$/", $vs_widget_dir )
				) {
	...
	public static function checkWidgetStatus( $ps_widget_name ) {
		...
		if ( isset( WidgetManager::$s_widget_instances[ $ps_widget_name ] )
		     && is_object( WidgetManager::$s_widget_instances[ $ps_widget_name ] )
		) {
	...
	public function renderWidget( $ps_widget_name, $ps_widget_id, $pa_settings = null ) {
		WidgetManager::initWidgets();
		if ( WidgetManager::$s_widget_instances[ $ps_widget_name ]
		     && is_object( WidgetManager::$s_widget_instances[ $ps_widget_name ] )
		) {
		...
		if ( WidgetManager::$s_widget_instances[ $ps_widget_name ]
			...
			foreach ( $va_available_settings as $vs_setting_name => $va_setting_info ) {
				...
				if ( isset( $va_setting_info['scope'] ) && ( $va_setting_info['scope'] == 'application' ) ) {
					...
					$pa_settings[ $vs_setting_name ] = $o_appvar->getVar( 'widget_settings_' . $ps_widget_name . '_'
					                                                      . $vs_setting_name );
				}
				$vs_buf .= WidgetManager::$s_widget_instances[ $ps_widget_name ]->settingHTMLFormElement( $ps_widget_id,
	...
	static public function getWidgetRoleActions() {
		...
		foreach ( $va_widget_dirs as $vs_widget_dir ) {
			...
			$va_actions = array_merge( $va_actions,
				call_user_func( array( $vs_widget_classname, 'getRoleActionList' ) ) );
		}
	...
	public function __call( $ps_hook_name, $pa_params ) {
		...
		foreach ( WidgetManager::$s_widget_hooks as $vs_plugin_name => $va_hooks ) {
			if ( isset( WidgetManager::$s_widget_hooks[ $vs_plugin_name ][ $ps_hook_name ] )
			     && WidgetManager::$s_widget_hooks[ $vs_plugin_name ][ $ps_hook_name ]
			) {
