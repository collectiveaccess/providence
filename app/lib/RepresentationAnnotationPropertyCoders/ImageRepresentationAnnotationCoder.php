...
class ImageRepresentationAnnotationCoder extends BaseRepresentationAnnotationCoder
	implements IRepresentationAnnotationPropertyCoder {
	# ------------------------------------------------------------------
	/**
	 *
	 *
					...
					switch ( $va_property_info['displayType'] ) {
						case 'DT_SELECT':
							$vs_element = caHTMLMakeLabeledFormElement(
								caHTMLSelect( $pa_attributes['name'] ? $pa_attributes['name'] : $ps_property,
									$va_property_info['options'], $pa_attributes, array( 'height' => 1 ) ),
								$vs_label, $pa_attributes['name'] ? $pa_attributes['name'] : $ps_property,
								$va_property_info['description'], $vs_format, false
							);
							break;
						default:
							$vs_element = caHTMLMakeLabeledFormElement(
								caHTMLTextInput( $pa_attributes['name'] ? $pa_attributes['name'] : $ps_property,
									$pa_attributes ),
								$vs_label, $pa_attributes['name'] ? $pa_attributes['name'] : $ps_property,
								$va_property_info['description'], $vs_format, false
							);
	...
	public function setProperty( $ps_property, $pm_value ) {
		...
		switch ( $va_info['fieldType'] ) {
			case 'FT_NUMBER':
				...
				$this->postError( 1500, _t( "Invalid numeric value '%1' for %2", $pm_value, $va_info['label'] ),
					'ImageRepresentationAnnotationCoder->setProperty()' );
			...
			case 'FT_VARS':
				switch ( $ps_property ) {
					case 'points':
						if ( is_string( $pm_value )
						     && ( trim( $pm_value ) == '*' )
						) {        // Generate 5% wide square polygon path at random location
							$pm_value = array();
							$vn_x     = rand( 5, 95 );
							$vn_y     = rand( 5, 95 );
