...
class TimeBasedRepresentationAnnotationCoder extends BaseRepresentationAnnotationCoder
	...
	public function setProperty( $ps_property, $pm_value ) {
		...
		switch ( $va_info['fieldType'] ) {
			case 'FT_TIMECODE':
				...
				$this->postError( 1500, _t( "Invalid timecode '%1' for %2", $pm_value, $va_info['label'] ),
					'TimeBasedRepresentationAnnotationCoder->setProperty()' );

				return false;
				break;
			default:
				// unsupported property?
				$this->postError( 1500, _t( "Invalid property '%1'", $ps_property ),
					'TimeBasedRepresentationAnnotationCoder->setProperty()' );
	...
	public function validate() {
		if ( $this->opa_property_values['startTimecode'] > $this->opa_property_values['endTimecode'] ) {
			$this->postError( 1500, _t( "Start must be less than end" ),
				"TimeBasedRepresentationAnnotationCoder->validate()" );
