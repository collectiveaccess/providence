...
class TimeBasedRepresentationAnnotationCoder extends BaseRepresentationAnnotationCoder implements
    IRepresentationAnnotationPropertyCoder
{
    ...
    public function setProperty($ps_property, $pm_value)
    {
        ...
        switch ($va_info['fieldType']) {
            case 'FT_TIMECODE':
                ...
                $this->postError(
                    1500,
                    _t("Invalid timecode '%1' for %2", $pm_value, $va_info['label']),
            ...
            default:
                ...
                $this->postError(
                    1500,
                    _t("Invalid property '%1'", $ps_property),
                    'TimeBasedRepresentationAnnotationCoder->setProperty()'
                );
                return null;
                break;
    ...
    public function validate()
    {
        if ($this->opa_property_values['startTimecode'] > $this->opa_property_values['endTimecode']) {
            $this->postError(
                1500,
                _t("Start must be less than end"),
