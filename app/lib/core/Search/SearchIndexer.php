												...
												foreach($va_labels_by_locale as $vn_locale_id => $va_label_list) {
													foreach($va_label_list as $va_label) {
														...
														foreach($va_label_info['related']['fields'] as $vs_label_field => $va_config) {
															$this->opo_engine->indexField($vn_label_table_num, 'I'.($vn_fn = Datamodel::getFieldNum($vn_label_table_num, $vs_label_field)), $vn_row_id, [$va_label[$vs_label_field]], array_merge($va_config, array('relationship_type_id' => $vn_rel_type_id, 'PRIVATE' => $vn_private)));
...
// update indexing for each relationship
				foreach($va_rel_tables_to_index_list as $vs_rel_table) {
					...
					$vn_rel_table_num = Datamodel::getTableNum($vs_rel_table);
