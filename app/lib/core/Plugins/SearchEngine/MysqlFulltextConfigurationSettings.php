<?php
/** ---------------------------------------------------------------------
 * app/lib/core/Plugins/SearchEngine/MysqlFulltextConfigurationSettings.php :
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
define('__CA_MYSQLFULLTEXT_RUNNING_MYSQL__',4001);
define('__CA_MYSQLFULLTEXT_TABLES_EXIST__',4002);
# ------------------------------------------------
...
# ------------------------------------------------
class MysqlFulltextConfigurationSettings extends ASearchConfigurationSettings {
	# ------------------------------------------------
	...
	public function setSettings(){
		$this->opa_possible_errors = array(
			__CA_MYSQLFULLTEXT_RUNNING_MYSQL__, __CA_MYSQLFULLTEXT_TABLES_EXIST__
		);
	}
	# ------------------------------------------------
	public function checkSetting($pn_setting_num){
		switch($pn_setting_num){
			case __CA_MYSQLFULLTEXT_RUNNING_MYSQL__:
				return $this->_checkMysqlIsRunning();
			case __CA_MYSQLFULLTEXT_TABLES_EXIST__:
				return $this->_checkMysqlFulltextTables();
			default:
				return false;
		}
	}
	...
	public function getSettingName($pn_setting_num){
		switch($pn_setting_num){
			...
			case __CA_MYSQLFULLTEXT_TABLES_EXIST__:
				return _t("MysqlFulltext database tables exist");
			default:
				return null;
		}
	}
	...
	public function getSettingDescription($pn_setting_num){
		...
		switch($pn_setting_num){
			case __CA_MYSQLFULLTEXT_RUNNING_MYSQL__:
				return _t("The MysqlFulltext search engine requires that MySQL be the back-end database for your CollectiveAccess installation.");
			case __CA_MYSQLFULLTEXT_TABLES_EXIST__:
				return _t("The MysqlFulltext search engine requires that certain tables be present in your database. They are installed by default and should be present, but if they are not MysqlFulltext will not be able to operate.");
			default:
				return null;
		}
	}
	...
	# ------------------------------------------------
	private function _checkMysqlFulltextTables(){
		$o_db = new Db();
