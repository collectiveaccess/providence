...
class MultipartIDNumber extends IDNumber
{
    ...
    public function getElementOrderForSort()
    {
        if (($vs_format = $this->getFormat()) && ($vs_type = $this->getType(
            ...
            return (is_array($this->opa_formats[$vs_format][$vs_type]['sort_order']) && sizeof(
                    $this->opa_formats[$vs_format][$vs_type]['sort_order']
        ...
        if ($vs_separator && $this->formatHas('PARENT', 0)) {
            ...
            foreach ($this->getElements() as $va_element_info) {
                switch ($va_element_info['type']) {
                    case 'LIST':
                        ...
                        foreach ($va_element_info['values'] as $vs_value) {
                            if (substr($ps_value, $vn_strpos, mb_strlen($vs_value)) === $vs_value && (is_null(
                                        $vs_matching_value
                    ...
                    case 'ALPHANUMERIC':
                        ...
                        $vn_width = mb_strlen(
                            preg_replace('/^([A-Za-z0-9]+).*$/', '$1', substr($ps_value, $vn_strpos))
                        );
                        break;
                    case 'FREE':
                    case 'PARENT':
                ...
                if (isset($va_element_info['width'])) {
                    ...
                    $vn_width = is_null($vn_width) ? intval($va_element_info['width']) : min(
                        $vn_width,
                        intval(
                            $va_element_info['width']
                        )
                    );
                }
            ...
            switch ($va_element_info['type']) {
                ...
                case 'SERIAL':
                    if ($vs_value) {
                        if (!preg_match("/^[A-Za-z0-9]+$/", $vs_value)) {
                            $va_element_errors[$vs_element_name] = _t(
                                "'%1' is not valid for %2; only letters and numbers are allowed",
                                $vs_value,
                                $va_element_info['description']
                            );
                        }
                ...
                case 'CONSTANT':
                    if ($vs_value && ($vs_value != $va_element_info['value'])) {
                        $va_element_errors[$vs_element_name] = _t(
                            "%1 must be set to %2; was %3",
                            $va_element_info['description'],
                            $va_element_info['value'],
                            $vs_value
                        );
                    }
                    break;
                case 'FREE':
                    if (isset($va_element_info['minimum_length']) && ($vn_value_len < $va_element_info['minimum_length'])) {
                        if ($va_element_info['minimum_length'] == 1) {
                            $va_element_errors[$vs_element_name] = _t(
                                "%1 must not be shorter than %2 character",
                                $va_element_info['description'],
                                $va_element_info['minimum_length']
                            );
                        } else {
                            $va_element_errors[$vs_element_name] = _t(
                                "%1 must not be shorter than %2 characters",
                                $va_element_info['description'],
                                $va_element_info['minimum_length']
                            );
                        }
                    ...
                    if (isset($va_element_info['maximum_length']) && ($vn_value_len > $va_element_info['maximum_length'])) {
                        if ($va_element_info['minimum_length'] == 1) {
                            $va_element_errors[$vs_element_name] = _t(
                                "%1 must not be longer than %2 character",
                                $va_element_info['description'],
                                $va_element_info['maximum_length']
                            );
                        } else {
                            $va_element_errors[$vs_element_name] = _t(
                                "%1 must not be longer than %2 characters",
                                $va_element_info['description'],
                                $va_element_info['maximum_length']
                            );
                        }
                ...
                case 'NUMERIC':
                    ...
                    if (isset($va_element_info['minimum_value']) && ($vs_value < $va_element_info['minimum_value'])) {
                        $va_element_errors[$vs_element_name] = _t(
                            "%1 must not be less than %2",
                            $va_element_info['description'],
                            $va_element_info['minimum_value']
                        );
                    }
                    if (isset($va_element_info['maximum_value']) && ($vs_value > $va_element_info['maximum_value'])) {
                        $va_element_errors[$vs_element_name] = _t(
                            "%1 must not be more than %2",
                            $va_element_info['description'],
                            $va_element_info['maximum_value']
                        );
                    }
                    if (isset($va_element_info['minimum_length']) && ($vn_value_len < $va_element_info['minimum_length'])) {
                        if ($va_element_info['minimum_length'] == 1) {
                ...
                case 'ALPHANUMERIC':
                    if ($vs_value != '' && !preg_match("/^[A-Za-z0-9]+$/", $vs_value)) {
                        $va_element_errors[$vs_element_name] = _t(
                            "%1 must consist only letters and numbers",
                            $va_element_info['description']
                        );
                    }
                    if (isset($va_element_info['minimum_length']) && ($vn_value_len < $va_element_info['minimum_length'])) {
                    ...
                    if ($vs_value != '') {
                        if ($va_element_info['width'] == 2) {
                            if (($vs_value < 0) || ($vs_value > 99)) {
                                $va_element_errors[$vs_element_name] = _t(
                                    "%1 must be a valid two-digit year",
                                    $va_element_info['description']
                                );
                            }
                        } elseif ((($vs_value < 1000) || ($vs_value > ($va_tmp['year'] + 10))) || ($vs_value != intval(
                                    $vs_value
                                ))) {
                            $va_element_errors[$vs_element_name] = _t(
                                "%1 must be a valid year",
                                $va_element_info['description']
                            );
                        }
                    }
                ...
                case 'MONTH':
                    if ($vs_value != '') {
                        if ((($vs_value < 1) || ($vs_value > 12)) || ($vs_value != intval($vs_value))) {
                            $va_element_errors[$vs_element_name] = _t(
                                "%1 must be a valid numeric month (between 1 and 12)",
                                $va_element_info['description']
                            );
                        }
                    }
                ...
                case 'DAY':
                    if ($vs_value != '') {
                        if ((($vs_value < 1) || ($vs_value > 31)) || ($vs_value != intval($vs_value))) {
                            $va_element_errors[$vs_element_name] = _t(
                                "%1 must be a valid numeric day (between 1 and 31)",
                                $va_element_info['description']
                            );
                        }
                    }
        ...
        foreach ($va_elements as $vs_element) {
            ...
            switch ($va_element_info['type']) {
                ...
                case 'NUMERIC':
                    ...
                    $va_output[] = str_repeat(' ', $vn_padding - strlen(intval($va_element_vals[$vn_i]))) . intval(
                            $va_element_vals[$vn_i]
                        );
                    break;
                case 'YEAR':
    ...
    public function getIndexValues($ps_value = null, $pa_options = null)
    {
        ...
        if ($va_tokens = preg_split(
            "![" . $this->opo_search_config->get('indexing_tokenizer_regex') . "]+!",
            $ps_value
        )) {
            $va_output_values = array_merge($va_output_values, $va_tokens);
        }

        if (isset($pa_options['INDEX_IDNO_PARTS']) || (is_array($pa_options) && (in_array(
                    'INDEX_IDNO_PARTS',
                    $pa_options
                )))) {
            if (is_array(
                    $va_delimiters = caGetOption(
                        'IDNO_DELIMITERS',
                        $pa_options,
                        $this->getSeparator() ? [$this->getSeparator()] : null
                    )
                ) && sizeof($va_delimiters)) {
                $va_output_values = array_merge(
                    $va_output_values,
                    preg_split(
                        "![" . preg_quote(join('', $va_delimiters), "!") . "]!",
                        $ps_value
                    )
                );
            }
    ...
    public function htmlFormElement($ps_name, &$pa_errors = null, $pa_options = null)
    {
        ...
        if ((sizeof($va_elements) < sizeof($va_element_vals)) && (bool)$this->getFormatProperty(
            ...
            foreach ($va_extra_vals as $vn_i => $vs_extra_val) {
                $va_element_controls[] = "<input type='text' name='{$ps_name}_extra_{$vn_i}' id='{$ps_name}_extra_{$vn_i}' value='" . htmlspecialchars(
                        $vs_extra_val,
                        ENT_QUOTES,
                        'UTF-8'
                    ) . "' size='{$vn_extra_size}'" . ($pa_options['readonly'] ? ' disabled="1" ' : '') . ">";
        ...
        if ($o_config->exists($this->getFormat() . '_dont_allow_editing_of_codes_when_in_use')) {
            if (isset($pa_options['row_id']) && ($pa_options['row_id'] > 0)) {
                if ($vb_dont_allow_editing) {
                    $va_element_controls[] = '<span class="formLabelWarning"><i class="caIcon fa fa-info-circle fa-1x"></i> ' . _t(
                            'Value cannot be edited because it is in use'
                        ) . '</span>';
                } else {
                    $va_element_controls[] = '<span class="formLabelWarning"><i class="caIcon fa fa-exclamation-triangle fa-1x"></i> ' . _t(
    ...
    public function htmlFormValuesAsArray(
        ...
        if ($ps_value) {
            ...
            if ((sizeof($va_tmp) > 0) && (bool)$this->getFormatProperty(
                    'allow_extra_elements',
                    array('default' => 1)
                )) {
                $vn_i = 0;
        ...
        foreach ($va_elements as $vs_element_name => $va_element_info) {
            ...
            if ($vn_zeropad_to_length = caGetOption('zeropad_to_length', $va_element_info, null)) {
                $va_tmp[$vs_element_name] = str_pad(
                    $va_tmp[$vs_element_name],
                    $vn_zeropad_to_length,
                    "0",
                    STR_PAD_LEFT
                );
            }
        ...
        switch ($va_element_info['type']) {
                    ...
                    if ($vs_element_value == '') {
                        ...
                        if ($va_element_info['editable']) {
                            $vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix . $vs_element_form_name . '" value="' . htmlspecialchars(
                                    ...
                                    ENT_QUOTES,
                                    'UTF-8'
                                ) . '" size="' . $vn_width . '" maxlength="' . $vn_width . '"' . ($pa_options['readonly'] ? ' disabled="1" ' : '') . '/>';
                        } else {
                            $vs_element .= '<input type="hidden" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix . $vs_element_form_name . '" value="' . htmlspecialchars(
                                    $vs_element_value,
                                    ENT_QUOTES,
            ...
            case 'CONSTANT':
                ...
                if ($va_element_info['editable'] || $pb_generate_for_search_form) {
                    $vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix . $vs_element_form_name . '" value="' . htmlspecialchars(
                            ...
                            ENT_QUOTES,
                            'UTF-8'
                        ) . '" size="' . $vn_width . '"' . ($pa_options['readonly'] ? ' disabled="1" ' : '') . '/>';
                } else {
                    $vs_element .= '<input type="hidden" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix . $vs_element_form_name . '" value="' . htmlspecialchars(
                            $vs_element_value,
                            ENT_QUOTES,
            ...
            case 'DAY':
                ...
                if ($vs_element_value == '') {
                    ...
                    if (!$pb_generate_for_search_form) {
                        if ($va_element_info['type'] == 'YEAR') {
                            $vn_value = ($va_element_info['width'] == 2) ? substr(
                                $va_date['year'],
                                2,
                                2
                            ) : $va_date['year'];
                ...
                if ($pb_generate_for_search_form) {
                    ...
                    if ($vs_element_value == '') {
                        ...
                        $vs_element .= '&lt;' . _t(
                                '%1',
                                $vs_next_num
                            ) . '&gt;' . '<input type="hidden" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix . $vs_element_form_name . '" value="' . htmlspecialchars(
                                $vs_next_num,
                                ENT_QUOTES,
                                'UTF-8'
                            ) . '"/>';
