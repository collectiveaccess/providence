...
class MultipartIDNumber extends IDNumber {
	 ...
	 * @param Db     $po_db     A database connection to use for all queries. If omitted a new connection (may be
	 *                          pooled) is allocated. [Default is null]
	 */
	...
	/**
	 * Return list of elements for current format and type using order specified in optional "sort_order" setting.
	 * Returns null if option is not set
	 *
	...
	public function getElementOrderForSort() {
		if ( ( $vs_format = $this->getFormat() ) && ( $vs_type = $this->getType() )
		     && isset( $this->opa_formats[ $vs_format ][ $vs_type ]['sort_order'] )
		) {
			return ( is_array( $this->opa_formats[ $vs_format ][ $vs_type ]['sort_order'] )
			         && sizeof( $this->opa_formats[ $vs_format ][ $vs_type ]['sort_order'] ) )
				? $this->opa_formats[ $vs_format ][ $vs_type ]['sort_order'] : null;
		}
	...
	/**
	 * Determine if the specified format and type contains an element of a given type. A specific element position may
	 * be specified. If omitted all elements will be examined.
	 *
	 * @param string $ps_element_type The type of element to look for (Eg. SERIAL, YEAR, LIST)
	 * @param int    $pn_index        The zero-based position in the element list to examine. If omitted all elements
	 *                                are examined. [Default is null]
	 ...
	 * @param array  $pa_options      Options include:
	 *                                checkLastElementOnly = check only the last element in the element list. This is
	 *                                the same as setting $pn_index to the last element, but saves you having to
	 *                                calculate what that index is. [Default is null]
	 *
	 * @return bool
	 */
	public function formatHas(
		$ps_element_type, $pn_index = null, $ps_format = null, $ps_type = null, $pa_options = null
	) {
		if ( $ps_format ) {
	...
	/**
	 * Determine if the specified format and type contain a SERIAL element its last element; that is, that the format
	 * and type is designed as an auto incrementing sequence with 0 or more prefix elements.
	 *
	 * @param string $ps_format A format to test. If omitted the current format is used. [Default is null]
	 * @param string $ps_type   A type to test. If omitted the current type is used. [Default is null]
	 *
	 ...
	 *
	 * @return array An array of element information arrays, of the same format as returned by getElementInfo(), or
	 *               null if the format and type are not set
	 */
	...
	/**
	 * Return array of configuration from multipart_id_numbering.conf for the specified element in the current format
	 * and type
	 *
	 * @param string $ps_element_name The element to return information for
	 *
	 * @return array An array of information with the same keys as in multipart_id_numbering.conf, or null if the
	...
	/**
	 * Breaks apart value using configuration of current format and type. When a format type specifies a separator this
	 * is generally equivalent to explode()'ing the value on the separator, except when PARENT elements (which may
	 * container the separator) are configured. explodeValue() can also split values when no separator is configured,
	 * using configured element widths to determine boundaries.
	 *
		...
		if ( $vs_separator && $this->formatHas( 'PARENT', 0 ) ) {
			...
			foreach ( $this->getElements() as $va_element_info ) {
				switch ( $va_element_info['type'] ) {
					case 'LIST':
						...
						foreach ( $va_element_info['values'] as $vs_value ) {
							if ( substr( $ps_value, $vn_strpos, mb_strlen( $vs_value ) ) === $vs_value
							     && ( is_null( $vs_matching_value )
							          || mb_strlen( $vs_matching_value ) < mb_strlen( $vs_value ) )
							) {
					...
					case 'ALPHANUMERIC':
						...
						$vn_width = mb_strlen( preg_replace( '/^([A-Za-z0-9]+).*$/', '$1',
							substr( $ps_value, $vn_strpos ) ) );
						break;
					case 'FREE':
					case 'PARENT':
					default:
				...
				if ( isset( $va_element_info['width'] ) ) {
					...
					$vn_width = is_null( $vn_width ) ? intval( $va_element_info['width'] )
						: min( $vn_width, intval( $va_element_info['width'] ) );
				}
			...
			switch ( $va_element_info['type'] ) {
				case 'LIST':
					if ( ! in_array( $vs_value, $va_element_info['values'] ) ) {
						$va_element_errors[ $vs_element_name ] = _t( "'%1' is not valid for %2", $vs_value,
							$va_element_info['description'] );
					}
					break;
				case 'SERIAL':
					if ( $vs_value ) {
						if ( ! preg_match( "/^[A-Za-z0-9]+$/", $vs_value ) ) {
							$va_element_errors[ $vs_element_name ]
								= _t( "'%1' is not valid for %2; only letters and numbers are allowed", $vs_value,
								$va_element_info['description'] );
						}
					}
					break;
				case 'CONSTANT':
					if ( $vs_value && ( $vs_value != $va_element_info['value'] ) ) {
						$va_element_errors[ $vs_element_name ] = _t( "%1 must be set to %2; was %3",
							$va_element_info['description'], $va_element_info['value'], $vs_value );
					}
					break;
				case 'FREE':
					if ( isset( $va_element_info['minimum_length'] )
					     && ( $vn_value_len < $va_element_info['minimum_length'] )
					) {
						if ( $va_element_info['minimum_length'] == 1 ) {
							$va_element_errors[ $vs_element_name ] = _t( "%1 must not be shorter than %2 character",
								$va_element_info['description'], $va_element_info['minimum_length'] );
						} else {
							$va_element_errors[ $vs_element_name ] = _t( "%1 must not be shorter than %2 characters",
								$va_element_info['description'], $va_element_info['minimum_length'] );
						}
					}
					if ( isset( $va_element_info['maximum_length'] )
					     && ( $vn_value_len > $va_element_info['maximum_length'] )
					) {
						if ( $va_element_info['minimum_length'] == 1 ) {
							$va_element_errors[ $vs_element_name ] = _t( "%1 must not be longer than %2 character",
								$va_element_info['description'], $va_element_info['maximum_length'] );
						} else {
							$va_element_errors[ $vs_element_name ] = _t( "%1 must not be longer than %2 characters",
								$va_element_info['description'], $va_element_info['maximum_length'] );
						}
					}
					break;
				case 'NUMERIC':
					if ( ! preg_match( "/^[\d]+[a-zA-Z]{0,1}$/", $vs_value ) ) {
						$va_element_errors[ $vs_element_name ] = _t( "%1 must be a number",
							$va_element_info['description'] );
					}
					if ( isset( $va_element_info['minimum_value'] )
					     && ( $vs_value < $va_element_info['minimum_value'] )
					) {
						$va_element_errors[ $vs_element_name ] = _t( "%1 must not be less than %2",
							$va_element_info['description'], $va_element_info['minimum_value'] );
					}
					if ( isset( $va_element_info['maximum_value'] )
					     && ( $vs_value > $va_element_info['maximum_value'] )
					) {
						$va_element_errors[ $vs_element_name ] = _t( "%1 must not be more than %2",
							$va_element_info['description'], $va_element_info['maximum_value'] );
					}
					if ( isset( $va_element_info['minimum_length'] )
					     && ( $vn_value_len < $va_element_info['minimum_length'] )
					) {
				...
				case 'ALPHANUMERIC':
					if ( $vs_value != '' && ! preg_match( "/^[A-Za-z0-9]+$/", $vs_value ) ) {
						$va_element_errors[ $vs_element_name ] = _t( "%1 must consist only letters and numbers",
							$va_element_info['description'] );
					}
					if ( isset( $va_element_info['minimum_length'] )
					     && ( $vn_value_len < $va_element_info['minimum_length'] )
					...
					if ( $vs_value != '' ) {
						if ( $va_element_info['width'] == 2 ) {
							if ( ( $vs_value < 0 ) || ( $vs_value > 99 ) ) {
								$va_element_errors[ $vs_element_name ] = _t( "%1 must be a valid two-digit year",
									$va_element_info['description'] );
							}
						} elseif ( ( ( $vs_value < 1000 ) || ( $vs_value > ( $va_tmp['year'] + 10 ) ) )
						           || ( $vs_value != intval( $vs_value ) )
						) {
							$va_element_errors[ $vs_element_name ] = _t( "%1 must be a valid year",
								$va_element_info['description'] );
						}
					}
					break;
				case 'MONTH':
					if ( $vs_value != '' ) {
						if ( ( ( $vs_value < 1 ) || ( $vs_value > 12 ) ) || ( $vs_value != intval( $vs_value ) ) ) {
							$va_element_errors[ $vs_element_name ]
								= _t( "%1 must be a valid numeric month (between 1 and 12)",
								$va_element_info['description'] );
						}
					}
					break;
				case 'DAY':
					if ( $vs_value != '' ) {
						if ( ( ( $vs_value < 1 ) || ( $vs_value > 31 ) ) || ( $vs_value != intval( $vs_value ) ) ) {
							$va_element_errors[ $vs_element_name ]
								= _t( "%1 must be a valid numeric day (between 1 and 31)",
								$va_element_info['description'] );
						}
					}
					break;
	 ...
	 *
	 * @param string $ps_value Value from which to derive the sortable value. If omitted the current value is used.
	 *                         [Default is null]
	 *
		...
		foreach ( $va_elements as $vs_element ) {
			...
			switch ( $va_element_info['type'] ) {
				...
				case 'CONSTANT':
					...
					$va_output[]   = ( ( $vn_repeat_len > 0 ) ? str_repeat( ' ',
							$vn_padding - mb_strlen( $va_element_vals[ $vn_i ] ) ) : '' ) . $va_element_vals[ $vn_i ];
					break;
				case 'FREE':
				case 'ALPHANUMERIC':
				...
				case 'NUMERIC':
					...
					$va_output[] = str_repeat( ' ', $vn_padding - strlen( intval( $va_element_vals[ $vn_i ] ) ) )
					               . intval( $va_element_vals[ $vn_i ] );
					break;
				case 'YEAR':
				...
				case 'DAY':
					...
					break;
				case 'PARENT':
					$va_output[] = $va_element_vals[ $vn_i ] . str_repeat( ' ',
							$vn_padding - mb_strlen( $va_element_vals[ $vn_i ] ) );
					break;
				default:
					$va_output[] = str_repeat( ' ', $vn_padding - mb_strlen( $va_element_vals[ $vn_i ] ) )
					               . $va_element_vals[ $vn_i ];
					break;
	 ...
	 *
	 * @param string $ps_value Value from which to derive the index values. If omitted the current value is used.
	 *                         [Default is null]
	 *
	...
	public function getIndexValues( $ps_value = null, $pa_options = null ) {
		...
		if ( $va_tokens = preg_split( "![" . $this->opo_search_config->get( 'indexing_tokenizer_regex' ) . "]+!",
			$ps_value )
		) {
			$va_output_values = array_merge( $va_output_values, $va_tokens );
		}

		if ( isset( $pa_options['INDEX_IDNO_PARTS'] )
		     || ( is_array( $pa_options )
		          && ( in_array( 'INDEX_IDNO_PARTS', $pa_options ) ) )
		) {
			if ( is_array( $va_delimiters = caGetOption( 'IDNO_DELIMITERS', $pa_options,
					$this->getSeparator() ? [ $this->getSeparator() ] : null ) )
			     && sizeof( $va_delimiters )
			) {
				$va_output_values = array_merge( $va_output_values,
					preg_split( "![" . preg_quote( join( '', $va_delimiters ), "!" ) . "]!", $ps_value ) );
			}
		}
	 ...
	 *
	 * @param string $ps_name    Name of form element. Is used as a prefix for each form element. The number element
	 *                           name will be used as a suffix for each.
	 * @param array  $pa_errors  Passed-by-reference array. Will contain any validation errors for the value, indexed
	 *                           by element.
	 * @param array  $pa_options Options include:
	 ...
	 *                           error_icon = Icon to display next to error messages; should be ready-to-display HTML.
	 *                           [Default is null] readonly = Make all form elements read-only. [Default is false]
	 ...
	 *                           progress_indicator = URL for spinner graphic to use while running duplicate number
	 *                           check. [Default is null] table = Table to perform duplicate number check in. [Default
	 *                           is null] search_url = Search service URL to use when performing duplicate number
	 *                           check. [Default is null] row_id = ID of row to exclude from duplicate number check
	 *                           (typically the current record id). [Default is null] context_id = context ID of row to
	 *                           exclude from duplicate number check (typically the current record context). [Default
	 *                           is null]
	 *
	...
	public function htmlFormElement( $ps_name, &$pa_errors = null, $pa_options = null ) {
		...
		foreach ( $va_elements as $vs_element_name => $va_element_info ) {
			...
			$vs_tmp                     = $this->genNumberElement( $vs_element_name, $ps_name,
				$va_element_vals[ $vn_i ], $vs_id_prefix, $vb_generate_for_search_form, $pa_options );
		...
		if ( ( sizeof( $va_elements ) < sizeof( $va_element_vals ) )
		     && (bool) $this->getFormatProperty( 'allow_extra_elements', array( 'default' => 1 ) )
		) {
			$va_extra_vals = array_slice( $va_element_vals, sizeof( $va_elements ) );

			if ( ( $vn_extra_size = (int) $this->getFormatProperty( 'extra_element_width', array( 'default' => 10 ) ) )
			     < 1
			) {
				$vn_extra_size = 10;
			}
			foreach ( $va_extra_vals as $vn_i => $vs_extra_val ) {
				$va_element_controls[]
					                        = "<input type='text' name='{$ps_name}_extra_{$vn_i}' id='{$ps_name}_extra_{$vn_i}' value='"
					                          . htmlspecialchars( $vs_extra_val, ENT_QUOTES, 'UTF-8' )
					                          . "' size='{$vn_extra_size}'" . ( $pa_options['readonly']
						? ' disabled="1" ' : '' ) . ">";
	...
	/**
	 * When displayed in a form for editing a multipart identifier will be composed of as many form elements as there
	 * are elements defined for the identifier format. Each form element will have a name beginning with the identifier
	 * field name and suffixed with the name of the identifier element. htmlFormValue() will pull these values from
	 * either an incoming request or, if specified, from the value specified in the $ps_value parameter and return it
	 * as a string. This method is identical to htmlFormValuesAsArray() save that it returns a string rather than an
	 * array.
	 *
	 * @param string $ps_name                           Name of the identifier field (eg. idno)
	 * @param string $ps_value                          An optional value to extract form values from. If null, values
	 *                                                  are pulled from the current request. [Default is null]
	 * @param bool   $pb_dont_mark_serial_value_as_used Don't record incoming value of the new maximum for SERIAL
	 *                                                  element sequences. [Default is false]
	 * @param bool   $pb_generate_for_search_form       Return array of empty values suitable for use in a search (not
	 *                                                  editing) form. [Default is false]
	 * @param bool   $pb_always_generate_serial_values  Always generate new values for SERIAL elements, even if they
	 *                                                  are not set with placeholders. [Default is false]
	 *
	 ...
	 * When displayed in a form for editing a multipart identifier will be composed of as many form elements as there
	 * are elements defined for the identifier format. Each form element will have a name beginning with the identifier
	 * field name and suffixed with the name of the identifier element. htmlFormValuesAsArray() will pull these values
	 * from either an incoming request or, if specified, from the value specified in the $ps_value parameter and return
	 * them as an array indexed with keys that identifier name + "_" + element name.
	 *
	 * @param string $ps_name                           Name of the identifier field (eg. idno)
	 * @param string $ps_value                          An optional value to extract form values from. If null, values
	 *                                                  are pulled from the current request. [Default is null]
	...
	public function htmlFormValuesAsArray(
		...
		foreach ( $va_elements as $vs_element_name => $va_element_info ) {
			if ( $va_element_info['type'] == 'SERIAL' ) {
				...
				if ( ( $va_element_values[ $ps_name . '_' . $vs_element_name ] == '' )
					...
					$va_tmp[ $vs_element_name ] = $this->getNextValue( $vs_element_name, $va_tmp,
					...
					if ( ! $pb_dont_mark_serial_value_as_used
					     && ( intval( $va_element_values[ $ps_name . '_' . $vs_element_name ] )
					          > $this->getSequenceMaxValue( $ps_name, $vs_element_name, '' ) )
					) {
			...
			} elseif ( ( $va_element_info['type'] == 'YEAR' )
			           && ! $va_element_values[ $ps_name . '_' . $vs_element_name ]
			) {  // set constant
				$va_date                                                = getdate();
			...
			} elseif ( ( $va_element_info['type'] == 'LIST' )
			           && ( ! isset( $va_element_values[ $ps_name . '_' . $vs_element_name ] )
			                || ! $va_element_values[ $ps_name . '_' . $vs_element_name ] )
			) {
				if ( ! is_array( $va_element_info['values'] ) ) {
			...
			if ( $vn_zeropad_to_length = caGetOption( 'zeropad_to_length', $va_element_info, null ) ) {
				$va_tmp[ $vs_element_name ] = str_pad( $va_tmp[ $vs_element_name ], $vn_zeropad_to_length, "0",
					STR_PAD_LEFT );
			}
		...
		if ( (bool) $this->getFormatProperty( 'allow_extra_elements', array( 'default' => 1 ) ) ) {
			$vn_i = 0;
			while ( true ) {
				if ( isset( $va_element_values[ $ps_name . '_extra_' . $vn_i ] )
				     && ( $vs_tmp
						= $va_element_values[ $ps_name . '_extra_' . $vn_i ] )
				) {
	 ...
	 * @param int   $pn_default      Default width, in characters, to use when width is not set in element info
	 *                               [Default is 3]
	 *
	...
	/**
	 * Generate an individual HTML form element for a specific number element. Used by htmlFormElement() to create a
	 * set of form element for the current format type.
	 *
	 * @param string $ps_element_name             Number element to generate form element for.
	 * @param string $ps_name                     Name of the identifier field (eg. idno)
	 * @param string $ps_value                    An optional value to extract form values from. If null, values are
	 *                                            pulled from the current request. [Default is null]
		...
		switch ( $va_element_info['type'] ) {
			...
			case 'LIST':
				...
				if ( ! $vs_element_value || $va_element_info['editable'] || $pb_generate_for_search_form ) {
					...
					$vs_element = '<select name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
					              . $vs_element_form_name . '">';
					if ( $pb_generate_for_search_form ) {
					...
					foreach ( $va_element_info['values'] as $ps_value ) {
						...
						$vs_element .= '<option value="' . $ps_value . '"' . $vs_selected . '>' . $ps_value
						               . '</option>';
					}

					if ( ! $pb_generate_for_search_form ) {
						if ( ! in_array( $vs_element_value, $va_element_info['values'] )
						     && strlen( $vs_element_value ) > 0
						) {
							$vs_element .= '<option value="' . $vs_element_value . '" selected="selected">'
							               . $vs_element_value . '</option>';
						}
					}

					$vs_element .= '</select>';
				} else {
					$vs_element_val_proc = ( in_array( $vs_element_value, $va_element_info['values'] )
						? $vs_element_value : $va_element_info['values'][0] );
					$vs_element          .= '<input type="hidden" name="' . $vs_element_form_name . '" id="'
					                        . $ps_id_prefix . $vs_element_form_name . '" value="'
					                        . htmlspecialchars( $vs_element_val_proc, ENT_QUOTES, 'UTF-8' ) . '"/>'
					                        . $vs_element_val_proc;
				}
			...
			case 'SERIAL':
				...
				if ( $pb_generate_for_search_form ) {
					$vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
					               . $vs_element_form_name . '" value="" maxlength="' . $vn_width . '" size="'
					               . $vn_width . '"' . ( $pa_options['readonly'] ? ' disabled="1" ' : '' ) . '/>';
				} else {
					if ( $vs_element_value == '' ) {
						...
						if ( $va_element_info['editable'] ) {
							$vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="'
							               . $ps_id_prefix . $vs_element_form_name . '" value="'
							               . htmlspecialchars( $vs_element_value, ENT_QUOTES, 'UTF-8' ) . '" size="'
							               . $vn_width . '" maxlength="' . $vn_width . '"' . ( $pa_options['readonly']
									? ' disabled="1" ' : '' ) . '/>';
						} else {
							$vs_element .= '<input type="hidden" name="' . $vs_element_form_name . '" id="'
							               . $ps_id_prefix . $vs_element_form_name . '" value="'
							               . htmlspecialchars( $vs_element_value, ENT_QUOTES, 'UTF-8' ) . '"/>'
							               . $vs_element_value;
						}
					}
				}
			...
			case 'CONSTANT':
				...
				if ( $va_element_info['editable'] || $pb_generate_for_search_form ) {
					$vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
					               . $vs_element_form_name . '" value="' . htmlspecialchars( $vs_element_value,
							ENT_QUOTES, 'UTF-8' ) . '" size="' . $vn_width . '"' . ( $pa_options['readonly']
							? ' disabled="1" ' : '' ) . '/>';
				} else {
					$vs_element .= '<input type="hidden" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
					               . $vs_element_form_name . '" value="' . htmlspecialchars( $vs_element_value,
							ENT_QUOTES, 'UTF-8' ) . '"/>' . $vs_element_value;
				}
				break;
			# ----------------------------------------------------
			case 'FREE':
			...
			case 'ALPHANUMERIC':
				...
				if ( ! $vs_element_value || $va_element_info['editable'] || $pb_generate_for_search_form ) {
					$vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
					               . $vs_element_form_name . '" value="' . htmlspecialchars( $vs_element_value,
							ENT_QUOTES, 'UTF-8' ) . '" size="' . $vn_width . '" maxlength="' . $vn_width . '"'
					               . ( $pa_options['readonly'] ? ' disabled="1" ' : '' ) . '/>';
				} else {
					$vs_element .= '<input type="hidden" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
					               . $vs_element_form_name . '" value="' . htmlspecialchars( $vs_element_value,
							ENT_QUOTES, 'UTF-8' ) . '"/>' . $vs_element_value;
			...
			case 'DAY':
				...
				if ( $vs_element_value == '' ) {
					...
					if ( ! $pb_generate_for_search_form ) {
						if ( $va_element_info['type'] == 'YEAR' ) {
							$vn_value = ( $va_element_info['width'] == 2 ) ? substr( $va_date['year'], 2, 2 )
								: $va_date['year'];
						}
						if ( $va_element_info['type'] == 'MONTH' ) {
					...
					if ( $va_element_info['editable'] || $pb_generate_for_search_form ) {
						$vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
						               . $vs_element_form_name . '" value="' . htmlspecialchars( $vn_value, ENT_QUOTES,
								'UTF-8' ) . '" size="' . $vn_width . '"' . ( $pa_options['readonly'] ? ' disabled="1" '
								: '' ) . '/>';
					} else {
						$vs_element .= '<input type="hidden" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
						               . $vs_element_form_name . '" value="' . htmlspecialchars( $vn_value, ENT_QUOTES,
			...
			case 'PARENT':
				...
				if ( $pb_generate_for_search_form ) {
					...
					if ( $vs_element_value == '' ) {
						...
						$vs_element  .= '&lt;' . _t( '%1', $vs_next_num ) . '&gt;' . '<input type="hidden" name="'
						                . $vs_element_form_name . '" id="' . $ps_id_prefix . $vs_element_form_name
						                . '" value="' . htmlspecialchars( $vs_next_num, ENT_QUOTES, 'UTF-8' ) . '"/>';
					} else {
						if ( $va_element_info['editable'] ) {
							$vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="'
							               . $ps_id_prefix . $vs_element_form_name . '" value="'
	...
	public function getSequenceMaxValue( $ps_format, $ps_element, $ps_idno_stub ) {
		...
		$vn_minimum_value = caGetOption( 'minimum_value', $this->getElementInfo( $ps_element ), 0,
			[ 'castTo' => 'int' ] );
		if ( ! ( $qr_res = $this->opo_db->query( "
			SELECT seq
