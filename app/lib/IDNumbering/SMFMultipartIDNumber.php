...
class SMFMultipartIDNumber extends IDNumber implements IIDNumbering {
	...
	public function getSeparator() {
		if ( ( $vs_format = $this->getFormat() ) && ( $vs_type = $this->getType() )
		     && isset( $this->opa_formats[ $vs_format ][ $vs_type ]['separator'] )
		) {
			return $this->opa_formats[ $vs_format ][ $vs_type ]['separator']
				? $this->opa_formats[ $vs_format ][ $vs_type ]['separator'] : '';
		}
			...
			switch ( $va_element_info['type'] ) {
				...
				case 'CONSTANT':
					if ( $vs_value && ( $vs_value != $va_element_info['value'] ) ) {
						$va_element_errors[ $vs_element_name ] = _t( "%1 must be set to %2",
							$va_element_info['description'], $va_element_info['value'] );
					}
					break;
				case 'FREE':
					# noop
				...
				case 'YEAR':
					$va_tmp = getdate();
					if ( $vs_value != '' ) {
						if ( ( ( $vs_value < 1800 ) || ( $vs_value > ( $va_tmp['year'] + 10 ) ) )
			...
			switch ( $va_element_info['type'] ) {
				...
				case 'NUMERIC':
					...
					if ( preg_match( "/^([0-9]+)([A-Za-z]{1})$/", $va_element_vals[ $vn_i ], $va_matches ) ) {
						$va_output[] = str_repeat( ' ', $vn_padding - mb_strlen( intval( $va_matches[1] ) ) )
						               . intval( $va_matches[1] ) . $va_matches[2];
					} else {
						$va_output[] = str_repeat( ' ', $vn_padding - mb_strlen( intval( $va_element_vals[ $vn_i ] ) ) )
	...
	public function htmlFormElement( $ps_name, &$pa_errors = null, $pa_options = null ) {
		...
		if ( sizeof( $va_elements ) < sizeof( $va_element_vals ) ) {
			$vs_extra_vals              = join( $vs_separator,
				array_slice( $va_element_vals, sizeof( $va_elements ) ) );
			$va_element_controls[]      = "<input type='text' name='" . $ps_name . "_extra' value='"
			                              . htmlspecialchars( $vs_extra_vals, ENT_QUOTES, 'UTF-8' ) . "' size='10'"
			...
			case 'SERIAL':
				...
				if ( $pb_generate_for_search_form ) {
					$vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
					               ...
					               . $vn_width . '"' . ( $pa_options['readonly'] ? ' readonly="readonly" ' : '' )
					               . '/>';
				} else {
					if ( $vs_element_value == '' ) {
				...
				if ( $vs_element_value == '' ) {
					...
					if ( $va_element_info['editable'] || $pb_generate_for_search_form ) {
						$vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
								...
								'UTF-8' ) . '" size="' . $vn_width . '"' . ( $pa_options['readonly']
								? ' readonly="readonly" ' : '' ) . '/>';
					} else {
						$vs_element .= '<input type="hidden" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
						               . $vs_element_form_name . '" value="' . htmlspecialchars( $vn_value, ENT_QUOTES,
