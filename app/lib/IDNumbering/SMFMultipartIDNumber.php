...
class SMFMultipartIDNumber extends IDNumber implements IIDNumbering
{
        ...
        foreach ($va_elements as $vs_element_name => $va_element_info) {
            ...
            switch ($va_element_info['type']) {
                ...
                case 'YEAR':
                    $va_tmp = getdate();
                    if ($vs_value != '') {
                        if ((($vs_value < 1800) || ($vs_value > ($va_tmp['year'] + 10))) || ($vs_value != intval(
                                    $vs_value
                                ))) {
                            $va_element_errors[$vs_element_name] = _t(
            ...
            switch ($va_element_info['type']) {
                ...
                case 'NUMERIC':
                    ...
                    if (preg_match("/^([0-9]+)([A-Za-z]{1})$/", $va_element_vals[$vn_i], $va_matches)) {
                        $va_output[] = str_repeat(' ', $vn_padding - mb_strlen(intval($va_matches[1]))) . intval(
                                $va_matches[1]
                            ) . $va_matches[2];
                    } else {
                        $va_output[] = str_repeat(
                                ' ',
                                $vn_padding - mb_strlen(intval($va_element_vals[$vn_i]))
    ...
    public function htmlFormElement($ps_name, &$pa_errors = null, $pa_options = null)
    {
        ...
        if (sizeof($va_elements) < sizeof($va_element_vals)) {
            ...
            $va_element_controls[] = "<input type='text' name='" . $ps_name . "_extra' value='" . htmlspecialchars(
                    $vs_extra_vals,
                    ENT_QUOTES,
                    'UTF-8'
                ) . "' size='10'" . ($pa_options['readonly'] ? ' readonly="readonly" ' : '') . ">";
            ...
            case 'CONSTANT':
                ...
                if ($va_element_info['editable'] || $pb_generate_for_search_form) {
                    $vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix . $vs_element_form_name . '" value="' . htmlspecialchars(
                            ...
                            ENT_QUOTES,
                            'UTF-8'
                        ) . '" size="' . $vn_width . '"' . ($pa_options['readonly'] ? ' readonly="readonly" ' : '') . '/>';
                } else {
                    $vs_element .= '<input type="hidden" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix . $vs_element_form_name . '" value="' . htmlspecialchars(
                            $vs_element_value,
                            ENT_QUOTES,
