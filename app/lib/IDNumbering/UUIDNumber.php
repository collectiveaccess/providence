...
class UUIDNumber extends IDNumber implements IIDNumbering
{
        ...
        if (($vs_format = $this->getFormat()) && ($vs_type = $this->getType())) {
            if (is_array($this->opa_formats[$vs_format][$vs_type]['elements'])) {
                ...
                foreach ($this->opa_formats[$vs_format][$vs_type]['elements'] as $vs_k => $va_element_info) {
                    if (!$vb_is_child && isset($va_element_info['child_only']) && (bool)$va_element_info['child_only']) {
                        continue;
                    }
            ...
            switch ($va_element_info['type']) {
                ...
                case 'VER5':
                    ...
                    if ($va_element_info['namespaceID'] == '') {
                        $va_element_errors[$vs_element_name] = _t(
                            "A NamespaceID (root UUID) is required for UUID %1",
                            $va_element_info['type']
                        );
                    }
                    break;
                case 'VER4':
    ...
    public function htmlFormElement($ps_name, &$pa_errors = null, $pa_options = null)
    {
        ...
        $vb_dont_allow_editing = isset($pa_options['row_id']) && ($pa_options['row_id'] > 0) && $o_config->exists(
                $this->getFormat() . '_dont_allow_editing_of_codes_when_in_use'
            ) && (bool)$o_config->get($this->getFormat() . '_dont_allow_editing_of_codes_when_in_use');
        ...
        foreach ($va_elements as $vs_element_name => $va_element_info) {
            $vs_tmp = $this->genNumberElement(
                $vs_element_name,
                $ps_name,
                $va_element_val,
                $vb_generate_for_search_form,
                $pa_options
            );
            ...
            if (($pa_options['show_errors']) && (isset($pa_errors[$vs_element_name]))) {
                ...
                TooltipManager::add(
                    "#caIdno_{$vs_id_prefix}_{$ps_name}",
    ...
    public function htmlFormValue(
        $ps_name,
        $ps_value = null,
        $pb_dont_mark_serial_value_as_used = false,
        $pb_generate_for_search_form = false,
        $pb_always_generate_serial_values = false
    ) {
        $va_tmp = $this->htmlFormValuesAsArray(
            $ps_name,
            $ps_value,
            $pb_dont_mark_serial_value_as_used,
            $pb_generate_for_search_form,
        ...
        foreach ($va_elements as $vs_element_name => $va_element_info) {
            if ($va_element_info['type'] == 'SERIAL') {
                ...
                if (($va_element_values[$ps_name . '_' . $vs_element_name] == '') || ($va_element_values[$ps_name . '_' . $vs_element_name] == '%') || $pb_always_generate_serial_values) {
                    ...
                    $va_tmp[$vs_element_name] = $this->getNextValue(
                        $vs_element_name,
                        join($vs_separator, $va_tmp),
                        $pb_dont_mark_serial_value_as_used
                    );
                    $vb_isset = $vb_is_not_empty = true;
                    continue;
                } else {
                    if (!$pb_dont_mark_serial_value_as_used && (intval(
                                $va_element_values[$ps_name . '_' . $vs_element_name]
                            ) > $this->getSequenceMaxValue($ps_name, $vs_element_name, ps_element_name))) {
                        $this->setSequenceMaxValue(
                            $this->getFormat(),
                            $vs_element_name,
                            join($vs_separator, $va_tmp),
                            $va_element_values[$ps_name . '_' . $vs_element_name]
    ...
    # -------------------------------------------------------
    public function genUUID($ps_type)
    {
        switch ($ps_type) {
            # ----------------------------------------------------
            case 'VER3':
            ...
            case 'VER4':
                ...
                if ($va_element_info['editable'] || $pb_generate_for_search_form) {
                    $vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix . $vs_element_form_name . '" value="' . htmlspecialchars(
                            $vs_uuid4,
                            ENT_QUOTES,
                            'UTF-8'
                        ) . '" size="' . $vn_width . '" maxlength="' . $vn_width . '"' . ($pa_options['readonly'] ? ' readonly="readonly" ' : '') . '/>';
                } else {
                    $vs_element .= '<input type="hidden" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix . $vs_element_form_name . '" value="' . htmlspecialchars(
                            $vs_uuid4,
                            ENT_QUOTES,
                            'UTF-8'
                        ) . '"/>' . $vs_uuid4;
