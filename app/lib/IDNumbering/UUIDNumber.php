...
class UUIDNumber extends IDNumber implements IIDNumbering {
	...
	public function __construct( $ps_format = null, $pm_type = null, $ps_value = null, $po_db = null ) {
		...
		if ( $ps_format ) {
			$this->setFormat( $ps_format );
		}
		if ( $pm_type ) {
			$this->setType( $pm_type );
		}
		if ( $ps_value ) {
		...
		if ( ( $vs_format = $this->getFormat() ) && ( $vs_type = $this->getType() ) ) {
			if ( is_array( $this->opa_formats[ $vs_format ][ $vs_type ]['elements'] ) ) {
				...
				foreach ( $this->opa_formats[ $vs_format ][ $vs_type ]['elements'] as $vs_k => $va_element_info ) {
					if ( ! $vb_is_child && isset( $va_element_info['child_only'] )
					     && (bool) $va_element_info['child_only']
					) {
						continue;
					}
			...
			switch ( $va_element_info['type'] ) {
				...
				case 'VER5':
					...
					if ( $va_element_info['namespaceID'] == '' ) {
						$va_element_errors[ $vs_element_name ]
							= _t( "A NamespaceID (root UUID) is required for UUID %1", $va_element_info['type'] );
					}
					break;
				case 'VER4':
					# noop
	...
	/**
	 * Return a list of modified identifier values suitable for search indexing according to the format of the
	 * specified format and type Modifications include removal of leading zeros, stemming and more.
	 *
	 * @param string $ps_value
	 *
	 * @return array
	...
	public function htmlFormElement( $ps_name, &$pa_errors = null, $pa_options = null ) {
		...
		$vb_dont_allow_editing = isset( $pa_options['row_id'] ) && ( $pa_options['row_id'] > 0 )
		                         && $o_config->exists( $this->getFormat() . '_dont_allow_editing_of_codes_when_in_use' )
		                         && (bool) $o_config->get( $this->getFormat()
		                                                   . '_dont_allow_editing_of_codes_when_in_use' );
		if ( $vb_dont_allow_editing ) {
			$pa_options['readonly'] = true;
		}
		...
		foreach ( $va_elements as $vs_element_name => $va_element_info ) {
			$vs_tmp                     = $this->genNumberElement( $vs_element_name, $ps_name, $va_element_val,
				$vb_generate_for_search_form, $pa_options );
			...
			if ( ( $pa_options['show_errors'] ) && ( isset( $pa_errors[ $vs_element_name ] ) ) ) {
				...
				if ( $pa_options['error_icon'] ) {
					$vs_tmp .= "<a href='#' id='caIdno_{$vs_id_prefix}_{$ps_name}'>" . $pa_options['error_icon']
					           . "</a>";
				} else {
					$vs_tmp .= "<a href='#' id='caIdno_{$vs_id_prefix}_{$ps_name}'>[" . _t( 'Error' ) . "]</a>";
				}
				TooltipManager::add( "#caIdno_{$vs_id_prefix}_{$ps_name}",
					"<h2>" . _t( 'Error' ) . "</h2>{$vs_error_message}" );
			}
	...
	# -------------------------------------------------------
	public function htmlFormValue(
		$ps_name, $ps_value = null, $pb_dont_mark_serial_value_as_used = false, $pb_generate_for_search_form = false,
		$pb_always_generate_serial_values = false
	) {
		$va_tmp = $this->htmlFormValuesAsArray( $ps_name, $ps_value, $pb_dont_mark_serial_value_as_used,
			$pb_generate_for_search_form, $pb_always_generate_serial_values );
		if ( ! ( $vs_separator = $this->getSeparator() ) ) {
			$vs_separator = '';
		}
	 ...
	 * with "%" characters, resulting is a template suitable for use with
	 * BundlableLabelableBaseModelWithAttributes::setIdnoTWithTemplate If the $pb_no_placeholders parameter is set to
	 * true then SERIAL values are omitted altogether from the returned template.
	 *
	 * Note that when the number of element replacements is limited, the elements are counted right-to-left. This means
	 * that if you limit the template to two replacements, the *rightmost* two SERIAL elements will be replaced with
	 * placeholders.
	 *
	 * @param string $ps_value                The id number to use as the basis of the template
	 * @param int    $pn_max_num_replacements The maximum number of elements to replace with placeholders. Set to 0 (or
	 *                                        omit) to replace all SERIAL elements.
	 * @param bool   $pb_no_placeholders      If set SERIAL elements are omitted altogether rather than being replaced
		...
		foreach ( $va_elements as $vs_element_name => $va_element_info ) {
			...
			if ( $va_element_info['type'] == 'SERIAL' ) {
				...
				if ( $pn_max_num_replacements <= 0 ) {    // replace all
					if ( $pb_no_placeholders ) {
						unset( $va_values[ $vn_i ] );
						$vn_i ++;
						continue;
					}
					$va_values[ $vn_i ] = '%';
	...
	# -------------------------------------------------------
	public function htmlFormValuesAsArray(
		$ps_name, $ps_value = null, $pb_dont_mark_serial_value_as_used = false, $pb_generate_for_search_form = false,
		$pb_always_generate_serial_values = false
	) {
		if ( is_null( $ps_value ) ) {
			if ( isset( $_REQUEST[ $ps_name ] ) && $_REQUEST[ $ps_name ] ) {
				return $_REQUEST[ $ps_name ];
			}
		}
		if ( ! is_array( $va_element_list = $this->getElements() ) ) {
			return null;
		}
		...
		if ( $ps_value ) {
			...
			foreach ( $va_element_names as $vs_element_name ) {
				if ( isset( $_REQUEST[ $ps_name . '_' . $vs_element_name ] ) ) {
					$va_element_values[ $ps_name . '_' . $vs_element_name ] = $_REQUEST[ $ps_name . '_'
					                                                                     . $vs_element_name ];
				}
			}
		}
		...
		foreach ( $va_elements as $vs_element_name => $va_element_info ) {
			if ( $va_element_info['type'] == 'SERIAL' ) {
				...
				if ( ( $va_element_values[ $ps_name . '_' . $vs_element_name ] == '' )
				     || ( $va_element_values[ $ps_name . '_' . $vs_element_name ] == '%' )
				     || $pb_always_generate_serial_values
				) {
					if ( $va_element_values[ $ps_name . '_' . $vs_element_name ] == '%' ) {
						$va_element_values[ $ps_name . '_' . $vs_element_name ] = '';
					}
					$va_tmp[ $vs_element_name ] = $this->getNextValue( $vs_element_name, join( $vs_separator, $va_tmp ),
						$pb_dont_mark_serial_value_as_used );
					$vb_isset                   = $vb_is_not_empty = true;
					continue;
				} else {
					if ( ! $pb_dont_mark_serial_value_as_used
					     && ( intval( $va_element_values[ $ps_name . '_' . $vs_element_name ] )
					          > $this->getSequenceMaxValue( $ps_name, $vs_element_name, ps_element_name ) )
					) {
						$this->setSequenceMaxValue( $this->getFormat(), $vs_element_name,
							join( $vs_separator, $va_tmp ), $va_element_values[ $ps_name . '_' . $vs_element_name ] );
					}
				}
			}
		...
		if ( isset( $va_element_values[ $ps_name . '_extra' ] )
		     && ( $vs_tmp = $va_element_values[ $ps_name . '_extra' ] )
		) {
		...
		switch ( $ps_type ) {
			...
			case 'VER4':
				...
				if ( $va_element_info['editable'] || $pb_generate_for_search_form ) {
					...
					$vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
					               . $vs_element_form_name . '" value="' . htmlspecialchars( $vs_uuid4, ENT_QUOTES,
							'UTF-8' ) . '" size="' . $vn_width . '" maxlength="' . $vn_width . '"'
					               . ( $pa_options['readonly'] ? ' readonly="readonly" ' : '' ) . '/>';
				} else {
					$vs_element .= '<input type="hidden" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
					               . $vs_element_form_name . '" value="' . htmlspecialchars( $vs_uuid4, ENT_QUOTES,
							'UTF-8' ) . '"/>' . $vs_uuid4;
				}
				break;
	...
	# -------------------------------------------------------
	private function genNumberElement(
		$ps_element_name, $ps_name, $ps_value, $ps_id_prefix = null, $pb_generate_for_search_form = false,
		$pa_options = null
	) {
		...
		if ( preg_match( "/^[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}$/", $ps_value ) == true ) {
			if ( $va_element_info['editable'] || $pb_generate_for_search_form ) {
				$vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
				               . $vs_element_form_name . '" value="' . htmlspecialchars( $ps_value, ENT_QUOTES,
						'UTF-8' ) . '" size="' . $vn_width . '" maxlength="' . $vn_width . '"'
				               . ( $pa_options['readonly'] ? ' readonly="readonly" ' : '' ) . '/>';
			} else {
				$vs_element .= '<input type="hidden" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
		...
		if ( $va_element_info['editable'] || $pb_generate_for_search_form ) {
			$vs_element .= '<input type="text" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
			               . $vs_element_form_name . '" value="' . htmlspecialchars( $vs_uuid, ENT_QUOTES, 'UTF-8' )
			               . '" size="' . $vn_width . '" maxlength="' . $vn_width . '"' . ( $pa_options['readonly']
					? ' readonly="readonly" ' : '' ) . '/>';
		} else {
			$vs_element .= '<input type="hidden" name="' . $vs_element_form_name . '" id="' . $ps_id_prefix
			               . $vs_element_form_name . '" value="' . htmlspecialchars( $vs_uuid, ENT_QUOTES, 'UTF-8' )
	...
	public function formatIsExtensionOf( $ps_format, $ps_type = '__default__' ) {
		...
		for ( $vn_i = 0; $vn_i < $vn_num_elements; $vn_i ++ ) {
			...
			if ( $va_base_element['type'] != $va_ext_element['type'] ) {
				return false;
			}
			if ( $va_base_element['width'] > $va_ext_element['width'] ) {
				return false;
			}

			switch ( $va_base_element['type'] ) {
				case 'LIST':
					if ( ! is_array( $va_base_element['values'] ) || ! is_array( $va_ext_element['values'] ) ) {
						return false;
					}
					if ( sizeof( $va_base_element['values'] ) != sizeof( $va_ext_element['values'] ) ) {
						return false;
					}
					for ( $vn_j = 0; $vn_j < sizeof( $va_base_element['values'] ); $vn_j ++ ) {
						if ( $va_base_element['values'][ $vn_j ] != $va_ext_element['values'][ $vn_j ] ) {
							return false;
						}
					}
					break;
				case 'CONSTANT';
					if ( $va_base_element['value'] != $va_ext_element['value'] ) {
						return false;
					}
					break;
				case 'NUMERIC':
					if ( $va_base_element['minimum_length'] < $va_ext_element['minimum_length'] ) {
						return false;
					}
					if ( $va_base_element['maximum_length'] > $va_ext_element['maximum_length'] ) {
						return false;
					}
					if ( $va_base_element['minimum_value'] < $va_ext_element['minimum_value'] ) {
						return false;
					}
					if ( $va_base_element['maximum_value'] > $va_ext_element['maximum_value'] ) {
						return false;
					}
					break;
				case 'ALPHANUMERIC':
	...
	public function getFormatProperty( $ps_property, $pa_options = null ) {
		if ( ( $vs_format = $this->getFormat() ) && ( $vs_type = $this->getType() )
		     && isset( $this->opa_formats[ $vs_format ][ $vs_type ][ $ps_property ] )
		) {
			return $this->opa_formats[ $vs_format ][ $vs_type ][ $ps_property ]
				? $this->opa_formats[ $vs_format ][ $vs_type ][ $ps_property ] : '';
		}
	 ...
	 * @param array  $pa_options     Options include:
	 *                               singleElementsOnly = Only consider formats with a single editable element to be
	...
	public function isFormatEditable( $ps_format_name, $pa_options = null ) {
		...
		foreach ( $va_elements as $vs_element => $va_element_info ) {
			if ( isset( $va_element_info['editable'] ) && (bool) $va_element_info['editable'] ) {
				return true;
			}
			if ( $vb_single_elements_only ) {
				return false;
			}
		}
