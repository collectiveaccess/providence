...
class BaseObject implements IErrorSetter
{
    ...
    public function postError($pn_num, $ps_message, $ps_context, $ps_source = '')
    {
        ...
        $dummy = null;
        if (($app = AppController::getInstance($dummy, $dummy, true)) && ($o_request = $app->getRequest()) && defined(
            ...
            $o_notification->addNotification(
                "[{$pn_num}] {$ps_message} ({$ps_context}" . ($ps_source ? "; {$ps_source}" : '') . $vs_stacktrace
            );
        }
        return true;
    }
