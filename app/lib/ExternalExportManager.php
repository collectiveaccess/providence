...
class ExternalExportManager {
	...
	public function __construct( $options = null ) {
		...
		$this->log    = caGetLogger( [ 'logLevel' => caGetOption( 'logLevel', $options, null ) ],
	 ...
	 *                       skipTransport = Don't perform  transport of exported data to configured destination.
	 *                       [Default is false] logLevel = KLogger constant for minimum log level to record. Default is
	 *                       KLogger::INFO. Constants are, in descending order of shrillness: ALERT = Alert messages
	 *                       (action must be taken immediately) CRIT = Critical conditions ERR = Error conditions WARN
	 *                       = Warnings NOTICE = Notices (normal but significant conditions) INFO = Informational
	 *                       messages DEBUG = Debugging messages mediaIndex = Zero-based index of media file to
	 *                       restrict output to. If null all media is processed. This option supports limitations of
	 *                       the CTDA digital preservation system and is unlikely to be useful for much else. [Default
	 *                       is null]
	 *
	...
	public function process( $table, $id, $options = null ) {
		...
		if ( $skip_transport = caGetOption( 'skipTransport', $options, false ) ) {
			$this->log->logDebug( _t( 'Skipping transport for %1:%2 because "skipTransport" runtime option was set',
				$table, $id ) );
		}
		...
		foreach ( $targets as $target => $target_info ) {
			...
			if ( ! ( $t_instance = $table::find( $id, [ 'returnAs' => 'firstModelInstance' ] ) ) ) {
				$this->log->logError( _t( 'Skipping  %1:%2 for target %3 because the row does not exist', $table, $id,
					$target ) );
				continue;
			}
			...
			if ( is_array( $access ) && sizeof( $access )
				...
				if ( ! in_array( (string) $t_instance->get( 'access' ), $access, true ) ) {
					$this->log->logDebug( _t( 'Skipping %1:%2 for target %3 because access is denied', $table, $id,
						$target ) );
					continue;
				}
			}
			...
			if ( is_array( $restrict_to_types ) && sizeof( $restrict_to_types )
			     && ( $types = caMakeTypeList( $table, [ $t_instance->getTypeCode() ] ) )
			) {    // Check access on record if specified in export policy
				if ( ! sizeof( array_intersect( $types, $restrict_to_types ) ) ) {
					$this->log->logDebug( _t( 'Skipping %1:%2 for target %3 because of type restrictions', $table, $id,
						$target ) );
					continue;
				}
			}
			$files[]
				= $f = $plugin->process( $t_instance, array_merge( $target_info,
				[ 'target' => $target, 'logLevel' => caGetOption( 'logLevel', $options, null ) ] ),
	...
	public function processPending( $options = null ) {
		...
		foreach ( $targets as $target => $target_info ) {
			$latest_log_id_seen
				              = $last_log_id
				= null;        // we reset the log counter for each target, then record the max value after we're finished.
			$target_table     = caGetOption( 'table', $target_info, null );
			...
			if ( is_array( $triggers ) ) {
				...
				foreach ( $triggers as $k => $trigger ) {
					$this->log->logDebug( _t( '[ExternalExportManager] Processing trigger %1 for target %2', $k,
						$target ) );
					$ids          = [];
					$ids_from_log = $ids_from_query = null;

					if ( ( $get_from = is_null( $last_log_id ) ? caGetOption( [ 'from_log_timestamp', 'from_log_id' ],
							$trigger, null ) : $last_log_id ) > 0
					) {
						if ( is_array( $this->log_entries = ca_change_log::getLog( $get_from ) ) ) {
							$last_log_id_in_list = array_pop( array_keys( $this->log_entries ) );
							if ( ( is_null( $latest_log_id_seen ) && $latest_log_id_seen )
							     || ( $last_log_id_in_list > $latest_log_id_seen )
							) {
							...
							$ids_from_log = array_map( function ( $v ) {
								...
								return ( ( (int) $v['logged_table_num'] === $target_table_num )
								         && ( $v['changetype'] !== 'D' ) );
							} ) );

							$ids_from_log_subjects = [];
							foreach ( $this->log_entries as $l ) {
								$ids_from_log_subjects = array_merge( $ids_from_log_subjects,
						...
						$this->log->logDebug( _t( '[ExternalExportManager] Found %1 new log entries with %4 unique ids for target %2 using "get from" value %3',
							sizeof( $this->log_entries ), $target, $get_from, sizeof( $ids_from_log ) ) );
					}

					if ( ( $query = caGetOption( 'query', $trigger, null ) )
					     && $search = caGetSearchInstance( $target_table )
					) {
						$qr             = $search->search( $query, [
							'restrictToTypes' => caGetOption( 'restrictToTypes', $target_info, null ),
							'checkAccess'     => caGetOption( 'checkAccess', $target_info, null )
						] );
						$ids_from_query = array_unique( $qr->getAllFieldValues( Datamodel::primaryKey( $target_table,
							true ) ) );

						$this->log->logDebug( _t( '[ExternalExportManager] Found %1 unique ids for target %2 using "query" value %3',
							sizeof( $ids_from_query ), $target, $query ) );
					}
					if ( ! is_null( $access = caGetOption( 'access', $trigger, null ) )
					     && $search = caGetSearchInstance( $target_table )
					) {
						$ids_from_query = $target_table::find( [ 'access' => $access ], [
							'returnAs'        => 'ids',
							'restrictToTypes' => caGetOption( 'restrictToTypes', $target_info, null ),
							'checkAccess'     => caGetOption( 'checkAccess', $target_info, null )
						] );

						$this->log->logDebug( _t( '[ExternalExportManager] Found %1 unique ids for target %2 using "access" value %3',
							sizeof( $ids_from_query ), $target, $access ) );
					}
					...
					$ids = $target_table::find( [ Datamodel::primaryKey( $target_table ) => [ 'IN', $ids ] ],
						[ 'returnAs' => 'ids' ] );    // Filter out deleted records
					$this->log->logInfo( _t( '[ExternalExportManager] Processing %1 unique ids for target %2',
						sizeof( $ids ), $target ) );
					...
					foreach ( $ids as $id ) {
						...
						if ( ( caGetOption( 'requireMedia', $target_info, false, [ 'castTo' => 'bool' ] ) ) ) {
							if ( $t = $target_table::find( $id, [ 'returnAs' => 'firstModelInstance' ] ) ) {
								if (
									! is_array( $rep_ids = $t->get( 'ca_object_representations.representation_id',
										[ 'filterNonPrimaryRepresentations' => false, 'returnAsArray' => true ] ) )
									|| ( sizeof( $rep_ids ) === 0 )
								) {
									$this->log->logDebug( _t( '[ExternalExportManager] Skipped id %1 for target %2 because media is required and no media was found',
										$id, $target ) );
									continue;
								}
							}
						...
						$this->log->logDebug( _t( '[ExternalExportManager] Processing %1 for target %2',
							"{$target_table}:{$id}", $target ) );
						if (
							...
							foreach ( $rep_ids as $media_index => $rep_id ) {
								$files = array_merge( $files, $f = $this->process( $target_table, $id,
									array_merge( $options, [
										'mediaIndex'    => $media_index,
										'target'        => $target,
										'skipTransport' => true
							...
							$files = $this->process( $target_table, $id,
								array_merge( $options, [ 'target' => $target, 'skipTransport' => true ] ) );
						}
						$seen["{$target_table}/{$id}"] = true;

						if ( is_array( $files ) ) {
							$this->log->logDebug( _t( '[ExternalExportManager] Generated %1 export packages for %2 for target %3',
								sizeof( $files ), "{$target_table}:{$id}", $target ) );
						} else {
							$this->log->logError( _t( '[ExternalExportManager] Could not generate export packages for %1 for target %2',
								"{$target_table}:{$id}", $target ) );
						}
	...
	public function transport( $target, $files, $options = null ) {
		...
		if ( is_array( $destination ) ) {
			...
			try {
				...
				$this->log->logDebug( _t( 'Sent %1 files for for target %2 via %3', sizeof( $files ), $target,
					$transport ) );
			} catch ( Exception $e ) {
				$this->log->logError( _t( 'Could not send %1 files for target %2 via %3: %4', sizeof( $files ), $target,
	...
	public static function getTargets( $options = null ) {
		...
		$targets = array_filter( $config->get( 'targets' ), function ( $v ) use ( $table, $restrict_to_types ) {
			...
			if ( is_array( $restrict_to_types ) && sizeof( $restrict_to_types ) && isset( $v['restrictToTypes'] )
			     && is_array( $v['restrictToTypes'] )
			     && sizeof( $v['restrictToTypes'] )
			     &&
