...
class RefineryManager {
	 ...
	 */
	static $s_refinery_manager_did_do_refinery_init = false;
	...
	public static function initRefineries() {
		...
		foreach ( $va_refinery_dirs as $vs_refinery_dir ) {
			if ( ! file_exists( $vs_base_refinery_dir . '/' . $vs_refinery_dir . '/' . $vs_refinery_dir
			                    . 'Refinery.php' )
			) {
				continue;
			}
	...
	public static function getRefineryInstance( $ps_refinery_name ) {
		RefineryManager::initRefineries();
		if ( isset( RefineryManager::$s_refinery_instances[ $ps_refinery_name ] )
		     && RefineryManager::$s_refinery_instances[ $ps_refinery_name ]
		) {
	...
	public static function getRefineryNames() {
		...
		if ( is_resource( $r_dir = opendir( $vs_base_refinery_dir ) ) ) {
			while ( ( $vs_refinery = readdir( $r_dir ) ) !== false ) {
				if ( is_dir( $vs_base_refinery_dir . '/' . $vs_refinery )
				     && preg_match( "/^[A-Za-z_]+[A-Za-z0-9_]*$/", $vs_refinery )
				) {
	...
	public static function checkRefineryStatus( $ps_refinery_name ) {
		...
		if ( isset( RefineryManager::$s_refinery_instances[ $ps_refinery_name ] )
		     && is_object( RefineryManager::$s_refinery_instances[ $ps_refinery_name ] )
		) {
	...
	public function getRefinerySettingsForm( $ps_refinery_name, $ps_refinery_id, $pa_settings ) {
		...
		$o_appvar = null;
		RefineryManager::initRefineries();
		if ( RefineryManager::$s_refinery_instances[ $ps_refinery_name ]
		     && is_object( RefineryManager::$s_refinery_instances[ $ps_refinery_name ] )
		) {
			$va_available_settings
				= RefineryManager::$s_refinery_instances[ $ps_refinery_name ]->getAvailableSettings();
			...
			foreach ( $va_available_settings as $vs_setting_name => $va_setting_info ) {
				...
				if ( isset( $va_setting_info['scope'] ) && ( $va_setting_info['scope'] == 'application' ) ) {
					...
					$pa_settings[ $vs_setting_name ] = $o_appvar->getVar( 'refinery_settings_' . $ps_refinery_name . '_'
					                                                      . $vs_setting_name );
				}
				$vs_buf .= RefineryManager::$s_refinery_instances[ $ps_refinery_name ]->settingHTMLFormElement( $ps_refinery_id,
					$vs_setting_name, array( 'value' => $pa_settings[ $vs_setting_name ] ) );
			}
		}
