...
abstract class BaseRefinery {
	...
	public function getRefinerySettings() {
		...
		if ( $this->supportsRelationships() ) {
			$va_base_settings[ $this->getName() . '_relationships' ] = array(
				...
				'description' => _t( 'A list (array) of relationships related to the %refinery.',
					array( 'refinery' => $this->getTitle() ) )
			);
		}
	 ...
	 *                               expression with embedded placeholders (Eg. "This is ^1 and this is ^2). The
	 *                               placeholders are valid specifiers for the data reader being used prefixed with a
	 *                               caret ("^"). For flat formats like Excel, they will look like ^1, ^2, etc. For XML
	 *                               formats they will be Xpath. Eg. ^/teiHeader/encodingDesc/projectDesc
	 * @param array  $pa_source_data An array of data to use in substitutions. Array is indexed by placeholder name
	 *                               *without* the leading caret.
	 * @param array  $pa_item        The mapping item information array containing settings for the current mapping.
	 * @param int    $pn_index       The index of the value to return. For non-repeating values this should be omitted
	 *                               or set to zero. For repeating values, this is a zero-based index indicating which
	 *                               value is returned. If a value for the specified index does not exist null will be
	 *                               returned. If the index is set to null then an array with all values is returned.
	 * @param array  $pa_options     An array of options. Options include:
	 *                               reader = An instance of BaseDataReader. Will be used to pull values for
	 *                               placeholders that are not defined in $pa_source_data. This is useful for formats
	 *                               like XML where placeholders may be arbitrary XPath expressions that must be
	 *                               executed rather than parsed. [Default is null] returnAsString = Return array of
	 *                               repeating values as string using delimiter. Has effect only is $pn_index parameter
	 *                               is set to null. [Default is false] delimiter = Delimiter to join array values with
	 *                               when returnAsString option is set; or the delimiter to use when breaking apart a
	 *                               value for return via the returnDelimitedValueAt option. [Default is ";"]
	 *                               returnDelimitedValueAt = Return a specific part of a value delimited by the
	 *                               "delimiter" option. Only has effect when returning a specific index of a repeating
	 *                               value (Eg. $pn_index is not null). The option value is a zero-based index. [Default is null – return entire value] applyImportItemSettings = Apply mapping options such as applyRegularExpressions to value. [Default is true]
	 *
	 * @return mixed An array or string
	 */
	public static function parsePlaceholder(
		$ps_placeholder, $pa_source_data, $pa_item, $pn_index = null, $pa_options = null
	) {
		...
		if ( $o_reader && ! $o_reader->valuesCanRepeat() && $vs_delimiter ) {
			...
			foreach ( $pa_source_data as $vs_k => $vm_v ) {
				if ( ! is_array( $pa_source_data[ $vs_k ] ) ) {
					$pa_source_data[ $vs_k ] = array_filter( explode( $vs_delimiter, $pa_source_data[ $vs_k ] ),
						"strlen" );
				}
			}
		}
		if ( $vs_delimiter && ! is_null( $pn_index ) && ! is_null( $pn_get_at_index ) ) {
			...
			foreach ( $pa_source_data as $vs_k => $vm_v ) {
				...
				if ( isset( $pa_source_data[ $vs_k ][ $pn_index ] ) ) {
					$va_tmp = preg_split( "!(" . join( "|", $va_delimiter ) . ")!",
						$pa_source_data[ $vs_k ][ $pn_index ] );

					// If only one delimited value in value string then return that regardless of the index (it's not a delimited string)
					$pa_source_data[ $vs_k ][ $pn_index ] = ( sizeof( $va_tmp ) == 1 ) ? $va_tmp[0]
						: ( isset( $va_tmp[ $pn_get_at_index ] ) ? $va_tmp[ $pn_get_at_index ] : null );
		...
		if ( ( $ps_placeholder[0] == '^' ) && ( strpos( $ps_placeholder, '^', 1 ) === false )
		     && ( sizeof( $t
					= caExtractTagsFromTemplate( $ps_placeholder ) ) == 1 )
		     && ( array_shift( $t ) === $ps_placeholder )
		) {
		...
		if ( is_array( $vm_val ) ) {
			foreach ( $vm_val as $vn_i => $vs_val ) {
				if ( is_array( $pa_item['settings']['original_values'] )
				     && ( ( $vn_ix
							= array_search( mb_strtolower( $vs_val ), $pa_item['settings']['original_values'] ) )
				          !== false )
				) {
		...
		foreach ( $vm_val as $i => $v ) {
			if ( is_array( $pa_item['settings']['original_values'] )
			     && ( ( $vn_i = array_search( mb_strtolower( $v ), $pa_item['settings']['original_values'] ) )
			          !== false )
			) {
	 ...
	 *
	 * @param array $pa_destination_data Array of data that to be imported. Will contain the product of all mappings
	 *                                   performed on the current source row *to date*. The refinery can make any
	 *                                   required additions and modifications to this data; since it's passed by
	 *                                   reference those changes will be returned.
	 * @param array $pa_group            Specification and settings for the mapping group being processed
	 * @param mixed $pa_item             Specification and settings for the mapping item being processed. Settings are
	 *                                   in an array under the "settings" key
	 * @param array $pa_source_data      The entire source row. You can extract the current value being processed by
	 *                                   plugging the item "source" specification into $pa_source_data
	...
	/**
	 * Identifies return value of refinery. If the refinery is designed to return a set of values overwriting all group
	 * values this method will return true. If the refinery returns a single transformed value intended to be inserted
	 * in place of the source value (Eg. a simple single-item mapping) this method will return false.
	 *
