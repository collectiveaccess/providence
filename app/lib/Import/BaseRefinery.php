...
abstract class BaseRefinery
{
     ...
     *
     */
    static public $s_refinery_settings = array();
    ...
    public function getRefinerySettings()
    {
        ...
        if ($this->supportsRelationships()) {
            $va_base_settings[$this->getName() . '_relationships'] = array(
                ...
                'description' => _t(
                    'A list (array) of relationships related to the %refinery.',
    ...
    public static function parsePlaceholder(
        ...
        if ($vs_delimiter && !is_null($pn_index) && !is_null($pn_get_at_index)) {
            ...
            foreach ($pa_source_data as $vs_k => $vm_v) {
                ...
                if (isset($pa_source_data[$vs_k][$pn_index])) {
                    ...
                    $pa_source_data[$vs_k][$pn_index] = (sizeof(
                            $va_tmp
                        ) == 1) ? $va_tmp[0] : (isset($va_tmp[$pn_get_at_index]) ? $va_tmp[$pn_get_at_index] : null);
                }
            }
        }
        if (($ps_placeholder[0] == '^') && (strpos($ps_placeholder, '^', 1) === false) && (sizeof(
                    $t = caExtractTagsFromTemplate($ps_placeholder)
                ) == 1) && (array_shift($t) === $ps_placeholder)) {
        ...
        if (is_array($vm_val)) {
            foreach ($vm_val as $vn_i => $vs_val) {
                if (is_array($pa_item['settings']['original_values']) && (($vn_ix = array_search(
                            mb_strtolower($vs_val),
        ...
        foreach ($vm_val as $i => $v) {
            if (is_array($pa_item['settings']['original_values']) && (($vn_i = array_search(
                        mb_strtolower($v),
                        $pa_item['settings']['original_values']
                    )) !== false)) {
     ...
     */
    public abstract function refine(&$pa_destination_data, $pa_group, $pa_item, $pa_source_data, $pa_options = null);
    # -------------------------------------------------------
     ...
     */
    public abstract function returnsMultipleValues();
    # -------------------------------------------------------
