...
class CollectiveAccessDataReader extends BaseDataReader {
	...
	public function __construct( $ps_source = null, $pa_options = null ) {
		...
		$this->opa_formats
			= array( 'collectiveaccess' );    // must be all lowercase to allow for case-insensitive matching
	...
	public function read( $ps_source, $pa_options = null ) {
		...
		try {
			...
			$this->ops_url_root
			                  =
			$vs_url_root = $va_url['scheme'] . "://" . $va_url['user'] . ":" . $va_url['pass'] . "@" . $va_url['host']
			               . ( ( $va_url['port'] != 80 ) ? ":{$va_url['port']}" : '' );

			$response = $this->opo_client->request( "GET",
				$vs_url_root . ( $vs_path ? "{$vs_path}" : "" ) . "/service.php/find/{$vs_table}?" . $va_url['query'] );
	...
	public function nextRow() {
		...
		if ( isset( $this->opa_row_ids[ $this->opn_current_row ] )
			...
			try {
				$response                                    = $this->opo_client->request( "GET",
					$this->ops_url_root . ( $this->ops_path ? $this->ops_path : "" )
					. "/service.php/item/{$this->ops_table}/id/{$vn_id}?pretty=1&format=import" );
				$data                                        = json_decode( $response->getBody(), true );
	...
	public function get( $ps_col, $pa_options = null ) {
		...
		$pa_filter_to_relationship_types = isset( $pa_options['filterToRelationshipTypes'] )
			? $pa_options['filterToRelationshipTypes'] : false;
		...
		$pb_return_all_locales            = isset( $pa_options['returnAllLocales'] )
			? (bool) $pa_options['returnAllLocales'] : false;
		$pb_convert_codes_to_display_text = isset( $pa_options['convertCodesToDisplayText'] )
			? (bool) $pa_options['convertCodesToDisplayText'] : true;
		...
		if ( $va_col[0] == $this->ops_table ) {    // field in this table
			...
			switch ( sizeof( $va_col ) ) {
				...
				case 2:
					...
					if ( isset( $va_data['intrinsic'][ $va_col[1] ] ) ) {
						...
						if ( $pb_convert_codes_to_display_text
						     && isset( $va_data['intrinsic'][ $va_col[1] . '_display' ] )
						) {
					...
					if ( isset( $va_data['attributes'][ $va_col[1] ] )
					     && is_array( $va_data['attributes'][ $va_col[1] ] )
					     && sizeof( $va_data['attributes'][ $va_col[1] ] )
					) {
						$va_vals = [];
			...
			//
			if ( ( $va_col[0] == 'ca_object_representations' ) && ( $va_col[1] == 'media' )
			     && ( $this->ops_table == 'ca_objects' )
			) {
				$va_urls = [];
			...
			if ( ! ( in_array( $va_col[0], [ 'tags', 'ca_item_tags' ] ) )
			     && ! isset( $va_data['related'][ $va_col[0] ] )
			) {
				return null;
			}
			...
			switch ( sizeof( $va_col ) ) {
				...
				case 2:
					if ( $va_col[1] == 'preferred_labels' ) {
						...
						if ( $t_instance = Datamodel::getInstanceByTableName( $va_col[0], true ) ) {
							...
							foreach ( $va_rel_data as $vn_i => $va_rel ) {
								...
								if ( is_array( $pa_filter_to_types )
								     && ! in_array( $va_rel['type_id_code'], $pa_filter_to_types )
								) {
									continue;
								}
								if ( is_array( $pa_filter_to_relationship_types )
								     && ! in_array( $va_rel['relationship_typename'], $pa_filter_to_relationship_types )
								     && ! in_array( $va_rel['relationship_type_code'],
										$pa_filter_to_relationship_types )
								) {
									continue;
								}
					...
					foreach ( $va_rel_data as $vn_i => $va_rel ) {
						...
						if ( is_array( $pa_filter_to_relationship_types )
						     && ! in_array( $va_rel['relationship_typename'], $pa_filter_to_relationship_types )
						     && ! in_array( $va_rel['relationship_type_code'], $pa_filter_to_relationship_types )
						) {
							continue;
						}
						...
						if ( isset( $va_rel['intrinsic'][ $va_col[1] ] ) ) {
							if ( $pb_convert_codes_to_display_text
							     && isset( $va_rel['intrinsic'][ $va_col[1] . '_display' ] )
							) {
								$va_rels[] = $va_rel['intrinsic'][ $va_col[1] . '_display' ];
							} else {
								$va_rels[] = isset( $va_rel['intrinsic'][ $va_col[1] . '_code' ] )
									? $va_rel['intrinsic'][ $va_col[1] . '_code' ] : $va_rel['intrinsic'][ $va_col[1] ];
							}
							...
							if ( $pb_convert_codes_to_display_text && isset( $va_rel[ $va_col[1] . '_display' ] ) ) {
								...
								$va_rels[] = isset( $va_rel[ $va_col[1] . '_code' ] ) ? $va_rel[ $va_col[1] . '_code' ]
									: $va_rel[ $va_col[1] ];
							}
						} else {
							$va_rels[] = '';
					...
					if ( ( $va_col[1] == 'hierarchy' ) && ( $va_col[2] == 'preferred_labels' ) ) {
						...
						if ( $t_instance = Datamodel::getInstanceByTableName( $va_col[0], true ) ) {
							...
							foreach ( $va_rel_data as $vn_i => $va_rel ) {
								...
								$va_rels[] = join( caGetOption( 'hierarchicalDelimiter', $pa_options, ';' ),
									$va_labels );
							}
	...
	public function getRow( $pa_options = null ) {
		if ( isset( $this->opa_row_buf[ $this->opn_current_row ] )
		     && ( $va_data
				= $this->opa_row_buf[ $this->opn_current_row ] )
		) {
			$va_row                                           = [];
				...
				foreach ( $va_data['related'] as $vs_table => $va_rels ) {
					foreach ( $va_rels as $vn_i => $va_rel ) {
						...
						foreach ( $va_rel['preferred_labels'] as $vs_f => $vs_v ) {
							$va_row[ $vs_table . ".preferred_labels.{$vs_f}" ] = $this->get( $vs_table
							                                                                 . ".preferred_labels.{$vs_f}",
								[ 'returnAsArray' => true ] );
						}
