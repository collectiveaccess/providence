...
class BaseXMLDataReader extends BaseDataReader {
	...
	public function read( $ps_source, $pa_options = null ) {
		...
		if ( ( $ps_base_path = caGetOption( 'basePath', $pa_options, null ) )
		     && ( ! preg_match( "!#XML tree#!", $ps_base_path ) )
		) {
	...
	public function get( $ps_spec, $pa_options = null ) {
		...
		foreach ( $o_node_list as $o_node ) {
			$va_values[] = html_entity_decode( ( $vs_xml = $this->getInnerXML( $o_node ) ) ? $vs_xml
				: $o_node->nodeValue, null, 'UTF-8' );
		}
	...
	public function _convertXPathExpression( $ps_spec, $pa_options = null ) {
		...
		if ( $vb_add_root_tag && ! $vb_double_slash && ( $va_tmp[0] != $this->ops_root_tag ) ) {
			...
			if ( $this->ops_xml_namespace_prefix
			     && ! preg_match( "!^" . $this->ops_xml_namespace_prefix . ":!", $vs_tag_tmp )
			) {
		...
		foreach ( $va_tmp as $vn_i => $vs_spec_element ) {
			...
			if (
				...
				&& ( ! preg_match( "!^[\.]+$!", $vs_spec_element ) )
				// . and .. should not get the default namespace applied
				&& ( ! preg_match( "!^\([A-Za-z0-9\-_]+!",
					$vs_spec_element ) )        // groups in parens should not get the default namespace applied
			) {
				if ( $this->ops_xml_namespace_prefix ) {

					if ( preg_match( "!^(parent|ancestor|ancestor-or-self|attribute|child|descendant|descendant-or-self|following|following-sibling|namespace|preceding|preceding-sibling|self)::(.*)$!",
						$vs_spec_element, $m )
					) {    // handle axes
