...
class ResourceSpaceDataReader extends BaseDataReader {
	...
	public function __construct( $ps_source = null, $pa_options = null ) {
		...
		$this->opa_formats
			= array( 'resourcespace' );    // must be all lowercase to allow for case-insensitive matching
		...
		foreach ( $va_api_credentials as $vs_instance => $va_instance_api ) {
			$rs_api = array(
				'rsInstance' => $vs_instance,
				'apiURL'     => $va_instance_api['resourcespace_base_api_url'],
				'apiKey'     => $va_instance_api['resourcespace_api_key'],
				'user'       => $va_instance_api['resourcespace_user']
			);
	...
	public function nextRow() {
		...
		if ( isset( $this->opa_row_ids[ $this->opn_current_row ] )
		     && ( $vs_resourcespace_string
				= $this->opa_row_ids[ $this->opn_current_row ] )
		) {
			...
			foreach ( $this->opa_api_credentials as $va_api ) {
				...
				$o_temp   = array();
				try {
					$vs_query = 'user=' . $va_api['user'] . '&function=get_resource_field_data&param1='
					            . $vn_resourcespace_id;
					...
					$vs_res_query      = 'user=' . $va_api['user'] . '&function=get_resource_data&param1='
					                     . $vn_resourcespace_id;
					...
					$vs_path_query    = 'user=' . $va_api['user'] . '&function=get_resource_path&param1='
					                    . $vn_resourcespace_id . '&param2=&param3=&param4=1&param5='
					                    . $vs_file_extension . '&param6=&param7=&param8=';
					...
					if ( $vn_rs_collection_id > 0 ) {
						$vs_coll_query    = 'user=' . $va_api['user'] . '&function=search_public_collections&param1='
						                    . $vn_rs_collection_id . '&param2=name&param3=ASC&param4=0&param5=0';
	...
	public function get( $ps_field, $pa_options = null ) {
		...
		if ( is_array( $this->opa_row_buf ) && ( strlen( $ps_field ) )
		     && ( isset( $this->opa_row_buf[ $ps_field ] ) )
		) {
