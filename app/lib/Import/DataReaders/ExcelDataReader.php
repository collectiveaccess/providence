...
class ExcelDataReader extends BaseDataReader
{
    ...
    public function read($ps_source, $pa_options = null)
    {
        ...
        try {
            $this->opo_handle = IOFactory::load($ps_source);
            ...
            if ($o_row = $this->opo_rows->current()) {
                ...
                foreach ($o_cells as $o_cell) {
                    if (Date::isDateTime($o_cell)) {
                        ...
                        // Strip nulls
                        $this->opa_row_buf[] = $vs_val = str_replace(
                            "\\0",
                            '/0',
                            trim((string)self::getCellAsHTML($o_cell))
                        );
                    }
    ...
    public function get($pn_col, $pa_options = null)
    {
        ...
        if (!is_numeric($pn_col)) {
            try {
                $pn_col = Coordinate::columnIndexFromString($pn_col);
            } catch (Exception $e) {
    ...
    public static function getCellAsHTML($po_cell)
    {
        ...
        if ($o_value instanceof RichText) {
            ...
            foreach ($va_elements as $o_element) {
                ...
                } elseif ($o_element instanceof TextElement) {
