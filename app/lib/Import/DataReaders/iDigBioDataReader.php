...
class iDigBioDataReader extends BaseDataReader {
	...
	public function __construct( $source = null, $options = null ) {
		...
		$this->ops_description
		                        = _t( 'Reads data from the iDigBio data service (http://idigbio.org) using the version 2 API' );
	...
	private function getData() {
		try {
			...
			$url          = "https://search.idigbio.org/v2/search/records?limit=" . (int) $this->limit . "&offset="
			                . (int) $this->start . "&rq=" . urlencode( $this->source );

			$response = $this->client->request( "GET", $url );

			$data = json_decode( $response->getBody(), true );

			if ( is_array( $data ) && isset( $data['itemCount'] ) && ( (int) $data['itemCount'] > 0 )
			     && is_array( $data['items'] )
			) {
				...
				foreach ( $data as $di => $r ) {
					if ( is_array( $r['indexTerms']['mediarecords'] ) && sizeof( $r['indexTerms']['mediarecords'] ) ) {
						...
						if ( is_array( $media_info = $this->getMedia( array_unique( $media_ids ) ) )
						     && is_array( $media_info['items'] )
						     && sizeof( $media_info['items'] )
						) {
							$data[ $di ]['data']['media'] = $media_info['items'];
						}
					} elseif ( isset( $r['data']['dwc:associatedMedia'] )
					           && strlen( $r['data']['dwc:associatedMedia'] )
					) {
						// add media
						$data[ $di ]['data']['media'][] = [
	...
	public function getRow( $options = null ) {
		if ( isset( $this->items[ $this->current_offset ] )
		     && is_array( $row = $this->items[ $this->current_offset ] )
		) {
