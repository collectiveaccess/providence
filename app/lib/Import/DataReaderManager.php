...
class DataReaderManager
{
     ...
     */
    static public $s_data_reader_manager_did_do_reader_init = false;

    /**
     *
     */
    static public $s_data_reader_instances = array();
    ...
    public static function getDataReaderNames()
    {
        ...
        if (is_resource($r_dir = opendir($vs_base_reader_dir))) {
            while (($vs_reader = readdir($r_dir)) !== false) {
                if (file_exists($vs_base_reader_dir . '/' . $vs_reader) && preg_match(
    ...
    public static function checkDataReaderStatus($ps_reader_name)
    {
        ...
        if (isset(DataReaderManager::$s_data_reader_instances[$ps_reader_name]) && is_object(
                DataReaderManager::$s_data_reader_instances[$ps_reader_name]
            )) {
