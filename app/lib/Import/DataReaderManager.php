...
class DataReaderManager {
	...
	public static function getDataReaderInstance( $ps_reader, $pa_options = null ) {
		DataReaderManager::initDataReaders();
		if ( isset( DataReaderManager::$s_data_reader_instances[ $ps_reader ] )
		     && DataReaderManager::$s_data_reader_instances[ $ps_reader ]
		) {
	...
	public static function getDataReaderNames() {
		...
		if ( is_resource( $r_dir = opendir( $vs_base_reader_dir ) ) ) {
			while ( ( $vs_reader = readdir( $r_dir ) ) !== false ) {
				if ( file_exists( $vs_base_reader_dir . '/' . $vs_reader )
				     && preg_match( "/^([A-Za-z_]+[A-Za-z0-9_]*)\.php$/", $vs_reader, $va_matches )
				) {
	...
	public static function checkDataReaderStatus( $ps_reader_name ) {
		...
		if ( isset( DataReaderManager::$s_data_reader_instances[ $ps_reader_name ] )
		     && is_object( DataReaderManager::$s_data_reader_instances[ $ps_reader_name ] )
		) {
