...
/**
 * Input type constants. Returned by inputType() to indicate what sort of input this reader takes (eg. File, URL, text,
 * etc.)
 */
...
abstract class BaseDataReader {
	...
	public function get( $ps_field, $pa_options = null ) {
		...
		switch ( $ps_field ) {
			...
			case '__filepath__':
				...
				break;
			case '__filename__':
				$vs_val = ( $vs_original_filename = caGetOption( 'originalFilename', $pa_options, null ) )
					? $vs_original_filename : pathinfo( $this->ops_source, PATHINFO_BASENAME );
				break;
			default:
				return null;
				break;
	...
	public function canReadFormat( $ps_format ) {
		return in_array( strtolower( $ps_format ),
			$this->opa_formats );    // lowercase $ps_format for case-insensitive matching (format list is already all lower-case)
	}
	# -------------------------------------------------------
