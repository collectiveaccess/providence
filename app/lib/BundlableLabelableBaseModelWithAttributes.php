...
/**
 * Returned by BundlableLabelableBaseModelWithAttributes::saveBundlesForScreenWillChangeParent() when parent will not
 * be changed
 */
define( '__CA_PARENT_UNCHANGED__', 0 );

/**
 * Returned by BundlableLabelableBaseModelWithAttributes::saveBundlesForScreenWillChangeParent() when parent will be
 * changed
 */
...
/**
 * Returned by BundlableLabelableBaseModelWithAttributes::saveBundlesForScreenWillChangeParent() when parent will be
 * changed to a related collection in a object-collection hierarchy
 */
...
class BundlableLabelableBaseModelWithAttributes extends LabelableBaseModelWithAttributes implements IBundleProvider {
	...
	public function load( $pm_id = null, $pb_use_cache = true ) {
		...
		if ( $this->isHierarchical() && $this->opo_idno_plugin_instance ) {
			$this->opo_idno_plugin_instance->isChild( ( ( $vs_parent_id_fld
					= $this->getProperty( 'HIERARCHY_PARENT_ID_FLD' ) )
			                                            && $this->get( $vs_parent_id_fld ) > 0 ) ? true : false );
		}
	...
	/**
	 * Override insert() to check type_id (or whatever the type key is called in the table as returned by
	 * getTypeFieldName()) against the ca_lists list for the table (as defined by getTypeListCode())
	 */
	public function insert( $pa_options = null ) {
		if ( ! is_a( $this, "BaseRelationshipModel" ) ) {
			...
			$this->opo_app_plugin_manager->hookBeforeBundleInsert( array(
				'id'         => null,
			...
			if ( $this->getTypeFieldName() && ! ( ! $vn_type_id && $va_field_info['IS_NULL'] ) ) {
				if ( ! ( $vn_ret = $t_list->itemIsEnabled( $this->getTypeListCode(), $vn_type_id ) ) ) {
					$va_type_list = $this->getTypeList( array(
						'directChildrenOnly'    => false,
						'returnHierarchyLevels' => true,
						'item_id'               => null
					) );
					if ( ! isset( $va_type_list[ $vn_type_id ] ) ) {
						$this->postError( 2510, _t( "Type must be specified" ),
							"BundlableLabelableBaseModelWithAttributes->insert()",
							$this->tableName() . '.' . $this->getTypeFieldName() );
					} else {
						if ( is_null( $vn_ret ) ) {
							$this->postError( 2510,
								_t( "<em>%1</em> is invalid", $va_type_list[ $vn_type_id ]['name_singular'] ),
								"BundlableLabelableBaseModelWithAttributes->insert()",
								$this->tableName() . '.' . $this->getTypeFieldName() );
						} else {
							$this->postError( 2510,
								_t( "<em>%1</em> is not enabled", $va_type_list[ $vn_type_id ]['name_singular'] ),
								"BundlableLabelableBaseModelWithAttributes->insert()",
								$this->tableName() . '.' . $this->getTypeFieldName() );
						}
					}
				...
				if ( $this->HIERARCHY_PARENT_ID_FLD
				     && (bool) $this->getAppConfig()->get( $this->tableName() . '_enforce_strict_type_hierarchy' )
				) {
					...
					if ( $vn_parent_id = $this->get( $this->HIERARCHY_PARENT_ID_FLD ) ) {
						...
						if ( $t_parent->load( $vn_parent_id ) ) {
							...
							$va_type_list
							                   = $t_parent->getTypeList( array(
								'directChildrenOnly'        => ( $this->getAppConfig()->get( $this->tableName()
								                                                             . '_enforce_strict_type_hierarchy' )
								                                 === '~' ) ? false : true,
								'childrenOfCurrentTypeOnly' => true,
								'returnHierarchyLevels'     => true
							) );
							...
							if ( ! isset( $va_type_list[ $this->getTypeID() ] ) ) {
								...
								$this->postError( 2510,
									_t( "<em>%1</em> is not a valid type for a child record of type <em>%2</em>",
										$va_type_list[ $this->getTypeID() ]['name_singular'],
										$va_type_list[ $vn_parent_type_id ]['name_singular'] ),
									"BundlableLabelableBaseModelWithAttributes->insert()",
									$this->tableName() . '.' . $this->getTypeFieldName() );
								$vb_error = true;
							}
							...
							// error - no parent?
							$this->postError( 2510, _t( "No parent was found when verifying type of new child" ),
								"BundlableLabelableBaseModelWithAttributes->insert()",
								$this->tableName() . '.' . $this->getTypeFieldName() );
							$vb_error = true;
						}
						...
						if ( ! isset( $va_type_list[ $this->getTypeID() ] ) && ( $t = $this->getTypeInstance() ) ) {
							...
							if ( sizeof( array_filter( $ancestors, function ( $v ) {
								...
								$this->postError( 2510, _t( "<em>%1</em> is not a valid type for a top-level record",
									$va_type_list[ $this->getTypeID() ]['name_singular'] ),
									"BundlableLabelableBaseModelWithAttributes->insert()",
									$this->tableName() . '.' . $this->getTypeFieldName() );
								$vb_error = true;
			...
			if ( $vb_error ) {
				...
				$this->_FIELD_VALUES[ $this->primaryKey() ]
					= null;        // clear primary key set by BaseModel::insert()
			...
			if ( (bool) $this->getAppConfig()->get( $this->tableName() . '_allow_access_inheritance' )
			     && $this->hasField( 'access_inherit_from_parent' )
			) {
				// Child record with inheritance set
				if ( (bool) $this->get( 'access_inherit_from_parent' )
				     && ( ( $vn_parent_id
		...
		$this->opo_app_plugin_manager->hookAfterBundleInsert( array(
	...
	public function update( $pa_options = null ) {
		...
		if ( $this->getAppConfig()->get( 'perform_item_level_access_checking' ) ) {
			if ( $this->checkACLAccessForUser( new ca_users( $AUTH_CURRENT_USER_ID ) ) < __CA_ACL_EDIT_ACCESS__ ) {
				$this->postError( 2580, _t( "You do not have edit access for this item: %1/%2", $this->tableName(),
					$this->getPrimaryKey() ), "BundlableLabelableBaseModelWithAttributes->update()" );
		...
		$this->opo_app_plugin_manager->hookBeforeBundleUpdate( array(
			'id'         => $this->getPrimaryKey(),
			'table_num'  => $this->tableNum(),
			'table_name' => $this->tableName(),
			'instance'   => $this
		...
		if ( (bool) $this->getAppConfig()->get( $this->tableName() . '_allow_access_inheritance' )
			...
			// Child record with inheritance set
			if ( (bool) $this->get( 'access_inherit_from_parent' )
			     && ( ( $vn_parent_id = $this->get( 'parent_id' ) ) > 0 )
			) {
				$t_parent = Datamodel::getInstanceByTableNum( $this->tableNum(), false );
				if ( $t_parent->load( $vn_parent_id ) ) {
					$this->set( 'access', $t_parent->set( 'access' ) );
		...
		$this->opo_app_plugin_manager->hookAfterBundleUpdate( array(
			'id'         => $this->getPrimaryKey(),
			'table_num'  => $this->tableNum(),
			'table_name' => $this->tableName(),
			'instance'   => $this
	...
	public function delete( $pb_delete_related = false, $pa_options = null, $pa_fields = null, $pa_table_list = null ) {
		global $AUTH_CURRENT_USER_ID;
		if ( $this->getAppConfig()->get( 'perform_item_level_access_checking' ) ) {
			if ( $this->checkACLAccessForUser( new ca_users( $AUTH_CURRENT_USER_ID ) )
			     < __CA_ACL_EDIT_DELETE_ACCESS__
			) {
				$this->postError( 2580, _t( "You do not have delete access for this item" ),
					"BundlableLabelableBaseModelWithAttributes->delete()" );
	 ...
	 * specific to a model - should be duplicated by the model by overriding
	 * BundlableLabelablleBaseModelWithAttributes::duplicate() and doing any required work after
	 * BundlableLabelablleBaseModelWithAttributes::duplicate() has finished
	 *
	 ...
	 *        duplicate_relationships = if set to an array of table names, all relationships to be specified tables
	 *        will be duplicated. [Default is null - no relationships duplicated] duplicate_current_relationships_only
	 *        = duplicate only current relationships, as determined by configured history tracking policies. [Default
	 *        is false] duplicate_relationship_attributes = duplicate metadata attributes attached to duplicated
	 *        relationships. [Default is false] duplicate_element_settings = per-metdata element duplication settings;
	 *        keys are element names, values are 1 (duplicate) or 0 (don't duplicate); if element is not set then it
	 *        will be duplicated. [Default is null] duplicate_children = duplicate child records. [Default is false]
	 *        user_id = User ID of the user to make owner of the newly duplicated record (for records that support
	 *        ownership by a user like ca_bundle_displays) [Default is null]
	 *
	...
	public function duplicate( $pa_options = null ) {
		...
		$vb_duplicate_nonpreferred_labels        = isset( $pa_options['duplicate_nonpreferred_labels'] )
		                                           && $pa_options['duplicate_nonpreferred_labels'];
		$vb_duplicate_attributes                 = isset( $pa_options['duplicate_attributes'] )
		                                           && $pa_options['duplicate_attributes'];
		$vb_duplicate_relationship_attributes    = isset( $pa_options['duplicate_relationship_attributes'] )
		                                           && $pa_options['duplicate_relationship_attributes'];
		$va_duplicate_relationships              = ( isset( $pa_options['duplicate_relationships'] )
		                                             && is_array( $pa_options['duplicate_relationships'] )
		                                             && sizeof( $pa_options['duplicate_relationships'] ) )
			? $pa_options['duplicate_relationships'] : array();
		$vb_duplicate_current_relationships_only = isset( $pa_options['duplicate_current_relationships_only'] )
		                                           && $pa_options['duplicate_current_relationships_only'];
		$va_duplicate_element_settings           = ( isset( $pa_options['duplicate_element_settings'] )
		                                             && is_array( $pa_options['duplicate_element_settings'] )
		                                             && sizeof( $pa_options['duplicate_element_settings'] ) )
			? $pa_options['duplicate_element_settings'] : array();
		$vb_duplicate_relationship_attributes    = isset( $pa_options['duplicate_relationship_attributes'] )
		                                           && $pa_options['duplicate_relationship_attributes'];
		$vb_duplicate_children                   = isset( $pa_options['duplicate_children'] )
		                                           && $pa_options['duplicate_children'];
		...
		if ( $this->isHierarchical() ) {
			if ( ! $this->get( $vs_parent_id_fld ) ) {
				if ( $this->getHierarchyType()
				     == __CA_HIER_TYPE_ADHOC_MONO__
				) {    // If we're duping the root of an adhoc hierarchy then we need to set the HIERARCHY_ID_FLD to null
					...
					$this->postError( 2055, _t( "Cannot duplicate root of hierarchy" ),
						"BundlableLabelableBaseModelWithAttributes->duplicate()" );
		...
		foreach ( $va_field_list as $vn_i => $vs_field ) {
			...
			switch ( $va_fld_info['FIELD_TYPE'] ) {
				...
				case FT_VARS:
					...
					break;
				default:
					if ( ! isset( $va_duplicate_element_settings[ $vs_field ] )
					     || ( isset( $va_duplicate_element_settings[ $vs_field ] )
					          && ( $va_duplicate_element_settings[ $vs_field ] == 1 ) )
					) {
		...
		if ( $vs_idno_fld
		     && ( ! isset( $va_duplicate_element_settings[ $vs_idno_fld ] )
		          || ( isset( $va_duplicate_element_settings[ $vs_idno_fld ] )
		               && ( $va_duplicate_element_settings[ $vs_idno_fld ] == 1 ) ) )
		) {
			$vb_needs_suffix_generated = false;
			if ( method_exists( $this, "getIDNoPlugInInstance" )
			     && ( $o_numbering_plugin
					= $this->getIDNoPlugInInstance() )
			) {
				if ( ! ( $vs_sep = $o_numbering_plugin->getSeparator() ) ) {
					$vs_sep = '-';
				}    // Must have a separator or you can get inexplicable numbers as numeric increments are appended string-style

				$vs_idno_template = $o_numbering_plugin->makeTemplateFromValue( $this->get( $vs_idno_fld ),
					1 );    // make template out of original idno by replacing last SERIAL element with "%"
				if ( ! preg_match( "!%$!", $vs_idno_template ) ) {
					$vb_needs_suffix_generated = true;
				}

				if ( ! is_array(
					$va_idno_values = $o_numbering_plugin->htmlFormValuesAsArray( $vs_idno_fld, $vs_idno_template,
						false, false, false ) )
				) {
			...
			if ( $vs_idno_stub ) {
				if ( $vb_needs_suffix_generated ) {
					...
					do {
						...
						$vs_idno   = trim( $vs_idno_stub ) . $vs_sep
						             . trim( $vs_suffix );    // force separator if none defined otherwise you end up with agglutinative numbers
		...
		foreach ( $va_labels as $vn_label_id => $va_labels_by_locale ) {
			foreach ( $va_labels_by_locale as $vn_locale_id => $va_label_list ) {
				foreach ( $va_label_list as $vn_i => $va_label_info ) {
					...
					if ( ! $vb_duplicate_nonpreferred_labels && key_exists( 'is_preferred', $va_label_info )
					     && ! $va_label_info['is_preferred']
					) {
						continue;
					}
					...
					$t_dupe->addLabel(
						$va_label_info, $va_label_info['locale_id'], $va_label_info['type_id'],
		...
		$history_tracking_policies = method_exists( $table, "getHistoryTrackingCurrentValuePoliciesForTable" )
			? array_keys( $table::getHistoryTrackingCurrentValuePoliciesForTable( $table ) ) : [];
		foreach (
			array(
				'ca_objects',
				'ca_object_lots',
			...
			if ( $vb_duplicate_current_relationships_only ) {
				...
				if ( is_array( $history_tracking_policies ) && sizeof( $history_tracking_policies ) ) {
					...
					foreach ( $history_tracking_policies as $policy ) {
						...
						if ( ! is_array(
								$h = $this->getHistory( [ 'currentOnly' => true, 'limit' => 1, 'policy' => $policy ] ) )
						     || ! sizeof( $h )
						) {
							continue;
						}
			...
			if ( $this->copyRelationships( $vs_rel_table, $t_dupe->getPrimaryKey(), array(
					'copyAttributes' => $vb_duplicate_relationship_attributes,
					'relationIds'    => $limit_to_relation_ids
				) ) === false
			) {
				$this->errors = $t_dupe->errors;
				if ( $vb_we_set_transaction ) {
		...
		if ( $t_dupe->getProperty( 'RANK' ) && $this->isHierarchical()
		     && ( $vn_parent_id
				= $this->get( $vs_parent_id_fld ) > 0 )
		) {
			$t_dupe->setRankAfter( $this->getPrimaryKey() );
		}

		if ( $vb_duplicate_children && $this->isHierarchical()
		     && ( $child_ids = $this->getHierarchyChildren( null, [ 'idsOnly' => true ] ) )
		) {
	 ...
	 *                          allowSettingOfTypeID = if true then type_id may be set for existing rows; default is to
	 *                          not allow type_id to be set for existing rows.
	 */
	public function set( $pa_fields, $pm_value = "", $pa_options = null ) {
		...
		if ( ( $vs_type_list_code = $this->getTypeListCode() )
		     && ( $vs_type_field_name
				= $this->getTypeFieldName() )
		) {
		...
		if ( $this->getPrimaryKey() && ! $this->isRelationship() && isset( $pa_fields[ $this->getTypeFieldName() ] )
		     && ! ( isset( $pa_options['allowSettingOfTypeID'] ) && $pa_options['allowSettingOfTypeID'] )
		) {
			$this->postError( 2520, _t( "Type id cannot be set after insert" ),
				"BundlableLabelableBaseModelWithAttributes->set()",
				$this->tableName() . '.' . $this->getTypeFieldName() );
		...
		if ( $this->opo_idno_plugin_instance ) {
			...
			$this->opo_idno_plugin_instance->isChild( ( ( ( $vs_parent_id_fld
						= $this->getProperty( 'HIERARCHY_PARENT_ID_FLD' ) )
			                                              && isset( $pa_fields[ $vs_parent_id_fld ] )
			                                              && ( $pa_fields[ $vs_parent_id_fld ] > 0 ) )
			                                            || ( $this->get( $vs_parent_id_fld ) ) ) ? true : false );

			if ( in_array( $this->getProperty( 'ID_NUMBERING_ID_FIELD' ), $pa_fields ) ) {
				if ( ! $this->_validateIncomingAdminIDNo( true, true ) ) {
					if ( ! $this->get( $vs_parent_id_fld ) && isset( $pa_fields[ $vs_parent_id_fld ] )
					     && ( $pa_fields[ $vs_parent_id_fld ] > 0 )
					) {
		...
		if ( $vn_rc = parent::set( $pa_fields, "", $pa_options ) ) {
			...
			if ( in_array( $vs_type_field_name = $this->getTypeFieldName(), array_keys( $pa_fields ) )
	 ...
	 *        retrictToRelationshipTypes - array of ca_relationship_types.type_id values to filter related items on.
	 *        *MUST BE INTEGER TYPE_IDs, NOT type_code's* This limitation is for performance reasons. You have to
	 *        convert codes to integer type_id's before invoking get sort = optional array of bundles to sort returned
	 *        values on. Currently only supported when getting related values via simple related <table_name> and
	 *        <table_name>.related invokations. Eg. from a ca_objects results you can use the 'sort' option got
	 *        get('ca_entities'), get('ca_entities.related') or get('ca_objects.related'). The bundle specifiers are
	 *        fields with or without tablename. Only those fields returned for the related tables (intrinsics and label
	 *        fields) are sortable. You cannot sort on attributes. returnAsLink = if true and $ps_field is set to a
	 *        specific field in a related table, or $ps_field is set to a related table (eg. ca_entities or
	 *        ca_entities.related) AND the template option is set and returnAllLocales is not set, then returned values
	 *        will be links. The destination of the link will be the appropriate editor when executed within Providence or the appropriate detail page when executed within Pawtucket or another front-end. Default is false. returnAsLinkText = text to use a content of HTML link. If omitted the url itself is used as the link content. returnAsLinkAttributes = array of attributes to include in link <a> tag. Use this to set class, alt and any other link attributes. returnAsLinkTarget = Optional link target. If any plugin implementing hookGetAsLink() responds to the specified target then the plugin will be used to generate the links rather than CA's default link generator. filterNonPrimaryRepresentations = Set filtering of non-primary representations in those models that support representations [Default is true]
	 */
	public function get( $ps_field, $pa_options = null ) {
		$vn_s = sizeof( $va_tmp = explode( '.', $ps_field ) );
		if ( ( ( $vn_s == 1 ) && ( $vs_field = $ps_field ) )
		     || ( ( $vn_s == 2 ) && ( $va_tmp[0] == $this->TABLE )
		          && ( $vs_field = $va_tmp[1] ) )
		) {
			if ( $this->hasField( $vs_field )
			     || ( in_array( strtolower( $vs_field ), [ 'created', 'lastmodified' ] ) )
			) {
		...
		if ( $this->_rowAsSearchResult ) {
			if ( method_exists( $this->_rowAsSearchResult, "filterNonPrimaryRepresentations" ) ) {
				$this->_rowAsSearchResult->filterNonPrimaryRepresentations( caGetOption( 'filterNonPrimaryRepresentations',
					$pa_options, true ) );
			}
	 ...
	 *
	 */
	private function _validateIncomingAdminIDNo(
		$pb_post_errors = true, $pb_dont_validate_idnos_for_parents_in_mono_hierarchies = false
	) {

		// we should not bother to validate
		$vn_hier_type = $this->getHierarchyType();
		if ( $pb_dont_validate_idnos_for_parents_in_mono_hierarchies
		     && in_array( $vn_hier_type, array( __CA_HIER_TYPE_SIMPLE_MONO__, __CA_HIER_TYPE_MULTI_MONO__ ) )
		     && ( $this->get( 'parent_id' ) == null )
		) {
			return true;
		}
	...
	public function getValuesForExport( $pa_options = null ) {
		...
		foreach (
			...
			$va_related_items = $this->getRelatedItems( $vs_table,
				array( 'returnAsArray' => true, 'returnAllLocales' => true ) );
		...
		if ( $vs_idno_field = $this->getProperty( 'ID_NUMBERING_ID_FIELD' ) ) {
			...
			if ( $vs_idno_context_field = $this->getProperty( 'ID_NUMBERING_CONTEXT_FIELD' ) ) {
				if ( $vn_context_id = $this->get( $vs_idno_context_field ) ) {
					$vs_idno_context_sql = ' AND (' . $vs_idno_context_field . ' = '
					                       . $this->quote( $vs_idno_context_field, $vn_context_id ) . ')';
				} else {
	...
	private function _generateSortableIdentifierValue() {
		if ( ( $vs_idno_field = $this->getProperty( 'ID_NUMBERING_ID_FIELD' ) )
		     && ( $vs_idno_sort_field
				= $this->getProperty( 'ID_NUMBERING_SORT_FIELD' ) )
		) {

			if ( ( $o_idno = $this->getIDNoPlugInInstance() )
			     && ( method_exists( $o_idno, 'getSortableValue' ) )
	...
	protected function initLabelDefinitions( $pa_options = null ) {
		...
		$this->BUNDLES = ( is_subclass_of( $this, "BaseRelationshipModel" ) )
			? array()
			: array(
				'preferred_labels'    => array(
					'type'      => 'preferred_label',
					'repeating' => true,
					'label'     => _t( "Preferred labels" )
				),
				'nonpreferred_labels' => array(
					'type'      => 'nonpreferred_label',
					'repeating' => true,
		...
		foreach ( $this->getFormFields() as $vs_f => $va_info ) {
			...
			$this->BUNDLES[ $vs_f ] = array(
				'type'      => 'intrinsic',
				'repeating' => false,
				'label'     => $this->getFieldInfo( $vs_f, 'LABEL' )
			);
		}
		...
		if ( $vs_field = $this->getProperty( 'ID_NUMBERING_ID_FIELD' ) ) {
			...
			$va_types = array();
			$o_db
			          = $this->getDb();        // have to do direct query here... if we use ca_list_items model we'll just endlessly recurse
	 ...
	 * @param string              $ps_bundle_name Optional bundle name to test readability on. If omitted readability
	 *                                            is considered for the item as a whole.
	 *
	...
	function isReadable( $po_request, $ps_bundle_name = null ) {
		...
		if ( $ps_bundle_name ) {
			if ( $t_user->getBundleAccessLevel( $this->tableName(), $ps_bundle_name )
			     < __CA_BUNDLE_ACCESS_READONLY__
			) {
				return false;
			}
		}
	 ...
	 *
	 * @param RequestHTTP|ca_user $po_request
	 * @param string              $ps_bundle_name Optional bundle name to test write-ability on. If omitted
	 *                                            write-ability is considered for the item as a whole.
	 *
	...
	public function getBundleFormValues( $ps_bundle_name, $ps_placement_code, $pa_bundle_settings, $pa_options = null
		...
		if ( $pa_options['request']->user->getBundleAccessLevel( $this->tableName(), $ps_bundle_name )
		     == __CA_BUNDLE_ACCESS_NONE__
		) {
			return;
		}
		...
		if ( (bool) $this->getAppConfig()->get( 'perform_type_access_checking' ) ) {
			$vn_type_access = $pa_options['request']->user->getTypeAccessLevel( $this->tableName(),
				$this->getTypeID() );
		...
		if ( (bool) $this->getAppConfig()->get( 'perform_source_access_checking' ) ) {
			$vn_source_access = $pa_options['request']->user->getSourceAccessLevel( $this->tableName(),
				$this->getSourceID() );
			...
			case 'related_table':
				switch ( $ps_bundle_name ) {
					...
					case 'ca_object_representations':
						return parent::getRelatedBundleFormValues( $pa_options['request'], $pa_options['formName'],
							$ps_bundle_name, $ps_placement_code, $pa_bundle_settings, $pa_options );
						break;
					case 'ca_entities':
					case 'ca_places':
					case 'ca_occurrences':
					...
					case 'ca_object_lots_related_list':
						return $this->getRelatedBundleFormValues( $pa_options['request'], $pa_options['formName'],
							$ps_bundle_name, $ps_placement_code, $pa_bundle_settings, $pa_options );
						break;
					# -------------------------------
					case 'ca_object_lots':
			...
			case 'special':
				switch ( $ps_bundle_name ) {
					case 'ca_site_page_media':
						return parent::getBundleFormValues( $ps_bundle_name, $ps_placement_code, $pa_bundle_settings,
							$pa_options );
						break;
				}
				break;
	 ...
	 *                           request
	 */
	public function getBundleFormHTML(
		$ps_bundle_name, $ps_placement_code, $pa_bundle_settings, $pa_options = null, &$ps_bundle_label = null
	) {
		...
		$vb_read_only_because_deaccessioned = ( $this->hasField( 'is_deaccessioned' )
		                                        && (bool) $this->getAppConfig()->get( 'deaccession_dont_allow_editing' )
		                                        && (bool) $this->get( 'is_deaccessioned' ) );
		...
		if (
			( $va_dictionary_entry = ca_metadata_dictionary_entries::getEntry( $dict_bundle_spec = $ps_bundle_name_proc,
				$this, $pa_bundle_settings ) )
			|| ( $va_dictionary_entry = ca_metadata_dictionary_entries::getEntry(
				$dict_bundle_spec = $this->tableName() . '.' . $ps_bundle_name_proc, $this, $pa_bundle_settings ) )
		) {
			...
			if ( ! ( $pa_bundle_settings['definition'][ $g_ui_locale ]
				= caExtractSettingsValueByUserLocale( 'definition', $va_dictionary_entry['settings'] ) )
			) {
			...
			if ( caGetOption( 'mandatory', $va_dictionary_entry['settings'], false ) ) {
				$def                                              = caExtractSettingsValueByUserLocale( 'definition',
					$pa_bundle_settings );
				$pa_bundle_settings['definition'][ $g_ui_locale ] = $this->getAppConfig()
				                                                         ->get( 'required_field_marker' )
				                                                    . ( is_array( $def ) ? $def[ $g_ui_locale ]
						: $def );
			}

			$va_violations = $this->getMetadataDictionaryRuleViolations( $dict_bundle_spec,
				[ 'placement_id' => (int) str_replace( "P", "", $ps_placement_code ) ] );
			if ( is_array( $va_violations ) && sizeof( $va_violations ) ) {
				$va_violation_text = array();
				foreach ( $va_violations as $va_violation ) {
					$va_violation_text[]
						= "<li class='caMetadataDictionaryViolation'><span class='caMetadataDictionaryViolation"
						  . ( ucfirst( strtolower( $va_violation['level'] ) ) ) . "'>" . $va_violation['levelDisplay']
						  . '</span>: ' . caExtractSettingsValueByUserLocale( 'violationMessage', $va_violation )
						  . "</li>";
				}
				$pa_bundle_settings['definition'][ $g_ui_locale ]
					= "<div class='caMetadataDictionaryViolationsList'><div class='caMetadataDictionaryViolationsListHeading'>"
					  . _t( 'These problems require attention:' ) . "</div><ol>" . join( "\n", $va_violation_text )
					  . "</ol></div>\n" . $pa_bundle_settings['definition'][ $g_ui_locale ]
					  . "<br style='clear: both;'/>";
			}
		}
		...
		$vs_documentation_url = trim( ( isset( $pa_bundle_settings['documentation_url'] )
		                                && $pa_bundle_settings['documentation_url'] )
			? $pa_bundle_settings['documentation_url'] : '' );
		...
		switch ( $vs_type ) {
			...
			case 'nonpreferred_label':
				...
				$vs_element              = ( $vs_type === 'preferred_label' )
					? $this->getPreferredLabelHTMLFormBundle( $pa_options['request'], $pa_options['formName'],
						$ps_placement_code, $pa_bundle_settings, $pa_options )
					: $this->getNonPreferredLabelHTMLFormBundle( $pa_options['request'], $pa_options['formName'],
						$ps_placement_code, $pa_bundle_settings, $pa_options );
				...
				if ( ( $vs_type == 'preferred_label' ) && $o_config->get( 'show_required_field_marker' )
				     && $o_config->get( 'require_preferred_label_for_' . $this->tableName() )
				) {
			...
			# -------------------------------------------------
			case 'intrinsic':
				if ( ! ( $pa_options['label'] = caExtractSettingValueByLocale( $pa_bundle_settings, 'label',
					$g_ui_locale ) )
				) {
					$pa_options['label'] = $this->getFieldInfo( $ps_bundle_name, 'LABEL' );
				}

				$vs_view_path = ( isset( $pa_options['viewPath'] ) && $pa_options['viewPath'] )
					? $pa_options['viewPath'] : $pa_options['request']->getViewsDirectoryPath();
				...
				if ( $this->getFieldInfo( $ps_bundle_name, 'IDENTITY' ) ) {
					$o_view->setVar( 'form_element', ( $vn_id = (int) $this->get( $ps_bundle_name ) )
						? $vn_id : "&lt;" . _t( 'Not yet issued' ) . "&gt;" );
				} else {
					$vb_read_only = ( $pa_bundle_settings['readonly']
					                  || ( $pa_options['request']->user->getBundleAccessLevel( $this->tableName(),
								$ps_bundle_name ) == __CA_BUNDLE_ACCESS_READONLY__ ) ) ? true : false;
					...
					$o_view->setVar( 'form_element', $this->htmlFormElement( $ps_bundle_name,
						( $this->getProperty( 'ID_NUMBERING_ID_FIELD' ) == $ps_bundle_name )
							? $o_config->get( 'idno_element_display_format_without_label' )
							: $o_config->get( 'bundle_element_display_format_without_label' ),
						array_merge(
							array(
						...
						$o_view->setVar( 'form_element_raw', $this->htmlFormElement( $ps_bundle_name, '^ELEMENT',
							array_merge(
								array(
									...
									'name'             => $ps_placement_code . $pa_options['formName']
									                      . $ps_bundle_name,
									'usewysiwygeditor' => $pa_bundle_settings['usewysiwygeditor']
								),
								$pa_options,
				...
				if ( method_exists( $this, "getDefaultMediaPreviewVersion" ) ) {
					$o_view->setVar( 'display_media', $this->getMediaTag( $ps_bundle_name,
						$this->getDefaultMediaPreviewVersion( $ps_bundle_name ) ) );
				}

				$vs_field_id = 'ca_intrinsic_' . $pa_options['formName'] . '_' . $ps_placement_code;
				$vs_label    = '<span class="formLabelText" id="' . $vs_field_id . '">' . $pa_options['label']
				               . '</span>';

				if ( $o_config->get( 'show_required_field_marker' ) ) {
					if ( ( $this->getFieldInfo( $ps_bundle_name, 'FIELD_TYPE' ) == FT_TEXT )
					     && is_array( $va_bounds = $this->getFieldInfo( $ps_bundle_name, 'BOUNDS_LENGTH' ) )
					     && ( $va_bounds[0] > 0 )
					) {
						$vs_label .= ' ' . $vs_required_marker;
					} else {
						if ( ( in_array( $this->getFieldInfo( $ps_bundle_name, 'FIELD_TYPE' ),
								array( FT_NUMBER, FT_HISTORIC_DATERANGE, FT_DATERANGE ) )
				...
				if ( (bool) $this->getAppConfig()->get( $this->tableName() . '_allow_access_inheritance' )
					...
					$this->set( 'access_inherit_from_parent',
						(bool) $this->getAppConfig()->get( $this->tableName() . '_access_inheritance_default' ) ? 1
							: 0 );
				}
				...
				$vs_element = $custom_view_exists ? $o_view->render( $s, true )
					: $o_view->render( 'intrinsic.php', true );


				if ( ! ( $vs_description = caExtractSettingValueByLocale( $pa_bundle_settings, 'description',
					$g_ui_locale ) )
				) {
			...
			case 'attribute':
				...
				$vs_element = $this->getAttributeHTMLFormBundle( $pa_options['request'], $pa_options['formName'],
					$vs_attr_element_code, $ps_placement_code, $pa_bundle_settings, $pa_options );
				...
				if ( $vb_batch ) {
					...
					if ( sizeof( $va_type_restrictions ) ) {
						...
						$vs_label            = '<span class="formLabelText" id="' . $vs_field_id . '">' . $vs_label_text
						                       . '</span> <span class="formLabelSubtext" id="subtext_' . $vs_field_id
						                       . '">(' . caTruncateStringWithEllipsis( $vs_restriction_list, 75 )
						                       . ')</span>';
						TooltipManager::add( "#subtext_{$vs_field_id}",
							"<h3>" . _t( "Restricted to types" ) . "</h3>" . join( "<br/>", $va_type_restrictions ) );
					} else {
						$vs_label = '<span class="formLabelText" id="' . $vs_field_id . '">' . $vs_label_text
						            . '</span>';
					}
				...
				$vs_documentation_url = trim( ( isset( $pa_bundle_settings['documentation_url'] )
				                                && $pa_bundle_settings['documentation_url'] )
					? $pa_bundle_settings['documentation_url']
					: $vs_documentation_url = $this->getAttributeDocumentationUrl( $vs_attr_element_code ) );

				if ( $t_element = ca_metadata_elements::getInstance( $vs_attr_element_code ) ) {
					if ( $o_config->get( 'show_required_field_marker' )
					     && ( ( $t_element->getSetting( 'minChars' ) > 0 )
					          || ( (bool) $t_element->getSetting( 'mustNotBeBlank' ) )
					          || ( (bool) $t_element->getSetting( 'requireValue' ) ) )
					) {
						$vs_label .= ' ' . $vs_required_marker;
					}
				}
			...
			case 'related_table':
				...
				switch ( $ps_bundle_name ) {
					...
					case 'ca_sets':
						if ( ( $this->_CONFIG->get( $ps_bundle_name . '_disable' ) )
						     && ( $ps_bundle_name !== 'ca_object_representations' )
						) {
							return '';
						}        // don't display if master "disable" switch is set
						$pa_options['start'] = 0;
						$pa_options['limit'] = caGetOption( 'numPerPage', $pa_bundle_settings, 10 );
						$vs_element          = $this->getRelatedHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_bundle_name, $ps_placement_code, $pa_bundle_settings,
							$pa_options );
						break;
					# -------------------------------
					case 'ca_object_lots':
						...
						if ( ( $this->tableName() != 'ca_object_lots' )
						     && ( $vn_lot_id
								= $pa_options['request']->getParameter( 'lot_id', pInteger ) )
						) {
							$pa_lot_options['force'][] = $vn_lot_id;
						}
						$vs_element = $this->getRelatedHTMLFormBundle( $pa_options['request'], $pa_options['formName'],
							$ps_bundle_name, $ps_placement_code, $pa_bundle_settings, $pa_lot_options );
						break;
					# -------------------------------
					case 'ca_representation_annotations':
						...
						$pa_options['fields'] = array(
							'ca_representation_annotations.status',
							'ca_representation_annotations.access',
							'ca_representation_annotations.props',
							'ca_representation_annotations.representation_id'
						);

						$vs_element = $this->getRepresentationAnnotationHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_bundle_settings, $pa_options );
					...
					case 'ca_object_lots_related_list':
						...
						$pa_options['start'] = 0;
						$vs_element          = $this->getRelatedListHTMLFormBundle( $pa_options['request'],
							$ps_bundle_name, $pa_options['formName'], $ps_placement_code, $pa_bundle_settings,
							$pa_options );
						break;
					# -------------------------------
					default:
				...
				$vs_label = '<span class="formLabelText" id="' . $pa_options['formName'] . '_' . $ps_placement_code
				            . '">' . $vs_label_text . '</span>';

				$vs_description = caExtractSettingValueByLocale( $pa_bundle_settings, 'description', $g_ui_locale );

				if ( ( $vs_label_text ) && ( $vs_description ) ) {
					TooltipManager::add( '#' . $pa_options['formName'] . '_' . $ps_placement_code,
			...
			case 'special':
				...
				$vb_read_only = ( $pa_options['request']->user->getBundleAccessLevel( $this->tableName(),
						$ps_bundle_name ) == __CA_BUNDLE_ACCESS_READONLY__ ) ? true : false;
				if ( ! $pa_bundle_settings['readonly'] ) {
					$pa_bundle_settings['readonly'] = ( ! isset( $pa_bundle_settings['readonly'] )
					                                    || ! $pa_bundle_settings['readonly'] ) ? $vb_read_only : true;
				}
				...
				switch ( $ps_bundle_name ) {
					...
					case 'ca_representation_annotation_properties':
						...
						foreach ( $this->getPropertyList() as $vs_property ) {
							$vs_element .= $this->getPropertyHTMLFormBundle( $pa_options['request'],
								$pa_options['formName'], $ps_placement_code, $vs_property, $pa_options );
						}
						break;
					# -------------------------------
					...
					case 'ca_set_items':
						...
						$vs_element .= $this->getSetItemHTMLFormBundle( $pa_options['request'], $pa_options['formName'],
							$ps_placement_code, $pa_options, $pa_bundle_settings );
						break;
					# -------------------------------
					// This bundle is only available for types which support set membership
					case 'ca_sets_checklist':
						require_once( __CA_MODELS_DIR__
						              . "/ca_sets.php" );    // need to include here to avoid dependency errors on parse/compile
						$t_set      = new ca_sets();
						$vs_element .= $t_set->getItemSetMembershipHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $this->tableNum(), $this->getPrimaryKey(),
							$pa_options['request']->getUserID(), $pa_bundle_settings, $pa_options );
						break;
					# -------------------------------
					...
					case 'ca_editor_ui_screens':
						...
						$vs_element .= $this->getScreenHTMLFormBundle( $pa_options['request'], $pa_options['formName'],
							$ps_placement_code, $pa_options );
						break;
					# -------------------------------
					...
					case 'ca_editor_ui_bundle_placements':
						...
						$vs_element .= $this->getPlacementsHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_options );
						break;
					# -------------------------------
					...
					case 'ca_tour_stops_list':
						...
						$vs_element .= $this->getTourStopHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_options );
						break;
					# -------------------------------
					...
					case 'hierarchy_navigation':
						...
						if ( $this->isHierarchical() ) {
							$vs_element .= $this->getHierarchyNavigationHTMLFormBundle( $pa_options['request'],
								$pa_options['formName'], $ps_placement_code, $pa_options, $pa_bundle_settings );
						}
						break;
					# -------------------------------
					...
					case 'hierarchy_location':
						if ( $this->isHierarchical() ) {
							$vs_element .= $this->getHierarchyLocationHTMLFormBundle( $pa_options['request'],
								$pa_options['formName'], $ps_placement_code, $pa_options, $pa_bundle_settings );
						}
						break;
					# -------------------------------
					...
					case 'ca_search_form_placements':
						...
						$vs_element .= $this->getSearchFormHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_options );
						break;
					# -------------------------------
					...
					case 'ca_bundle_display_placements':
						...
						//if (!$this->getPrimaryKey()) { return ''; }
						$vs_element .= $this->getBundleDisplayHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_options );
						break;
					# -------------------------------
					// This bundle is only available when editing objects of type ca_bundle_displays
					...
					case 'ca_editor_ui_type_restrictions':
						$vs_element .= $this->getTypeRestrictionsHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_options );
						break;
					# -------------------------------
					// This bundle is only available when editing objects of type ca_bundle_displays
					...
					//
					case 'ca_users':
						if ( ! $pa_options['request']->user->canDoAction( 'is_administrator' )
						     && ( $pa_options['request']->getUserID() != $this->get( 'user_id' ) )
						) {
							return '';
						}    // don't allow setting of per-user access if user is not owner
						$vs_element .= $this->getUserHTMLFormBundle( $pa_options['request'], $pa_options['formName'],
							$ps_placement_code, $this->tableNum(), $this->getPrimaryKey(),
							$pa_options['request']->getUserID(), $pa_options );
						break;
					# -------------------------------
					//
					case 'ca_user_groups':
						...
						$vs_element .= $this->getUserGroupHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $this->tableNum(), $this->getPrimaryKey(),
							$pa_options['request']->getUserID(), $pa_options );
						break;
					# -------------------------------
					...
					case 'ca_user_roles':
						...
						$vs_element .= $this->getUserRoleHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $this->tableNum(), $this->getPrimaryKey(),
							$pa_options['request']->getUserID(), $pa_options );
						break;
					# -------------------------------
					case 'settings':
						...
						$vs_element .= $this->getHTMLSettingFormBundle( $pa_options['request'], $pa_options['formName'],
							$ps_placement_code, $this->tableNum(), $pa_options );
						break;
					# -------------------------------
					...
					case 'ca_object_representations_media_display':
						...
						$vs_element .= $this->getMediaDisplayHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_bundle_settings, $pa_options );
						break;
					# -------------------------------
					// This bundle is only available when editing objects of type ca_object_representations
					case 'ca_object_representation_captions':
						...
						$vs_element .= $this->getCaptionHTMLFormBundle( $pa_options['request'], $pa_options['formName'],
							$ps_placement_code, $pa_bundle_settings, $pa_options );
						break;
					# -------------------------------
					...
					case 'ca_objects_components_list':
						...
						$vs_element .= $this->getComponentListHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_bundle_settings, $pa_options );
					...
					case 'history_tracking_chronology':
						...
						if ( strlen( $pb_show_child_history = $pa_options['request']->getParameter( "showChildHistory",
							pInteger ) )
						) {
							Session::setVar( "{$ps_bundle_name}_showChildHistory", (bool) $pb_show_child_history );
						}
						$vs_element .= $this->getHistoryTrackingChronologyHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_bundle_settings, array_merge( $pa_options,
								[
									'bundleName'       => $ps_bundle_name,
									'showChildHistory' => Session::getVar( "{$ps_bundle_name}_showChildHistory" )
								] ) );
					...
					case 'ca_objects_deaccession':        // object deaccession information
						...
						$vs_element .= $this->getObjectDeaccessionHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_bundle_settings, $pa_options );
					...
					case 'ca_object_checkouts':        // object checkout information
						...
						$vs_element .= $this->getObjectCheckoutsHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_bundle_settings, $pa_options );
					...
					case 'ca_object_representation_chooser':
						...
						$vs_element .= $this->getRepresentationChooserHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_bundle_settings, $pa_options );
						break;
					# -------------------------------
					...
					case 'history_tracking_current_contents':
						...
						$vs_element .= $this->getHistoryTrackingCurrentContentsHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_bundle_settings, $pa_options );
					...
					case 'ca_object_circulation_status':        // circulation status for objects (part of the library module)
						...
						$vs_element .= $this->getObjectCirculationStatusHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_bundle_settings, $pa_options );
					...
					case 'authority_references_list':
						$vs_element .= $this->getAuthorityReferenceListHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_bundle_settings, $pa_options );
						break;
					# -------------------------------
					...
					case 'ca_object_representations_access_status':
						if ( ( $vb_batch ) && ( is_a( $this, 'RepresentableBaseModel' ) ) ) {
							$vs_element .= $this->getObjectRepresentationAccessStatusHTMLFormBundle( $pa_options['request'],
								$pa_options['formName'], $ps_placement_code, $pa_bundle_settings, $pa_options );
						} else {
							return null;
						}
					...
					case 'ca_metadata_alert_triggers':
						...
						$vs_element .= $this->getTriggerHTMLFormBundle( $pa_options['request'], $pa_options['formName'],
							$ps_placement_code, $pa_bundle_settings, $pa_options );
						break;
					# -------------------------------
					...
					case 'ca_metadata_dictionary_rules':
						...
						$vs_element .= $this->getRulesHTMLFormBundle( $pa_options['request'], $pa_options['formName'],
							$ps_placement_code, $pa_bundle_settings, $pa_options );
						break;
					# -------------------------------
					// This bundle is only available items for ca_site_pages
					case 'ca_site_pages_content':
						$vs_element .= $this->getPageContentHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_bundle_settings, $pa_options );
						break;
					# -------------------------------
					// This bundle is only available items for ca_site_pages
					case 'ca_site_page_media':
						$vs_element .= $this->getPageMediaHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_bundle_settings, $pa_options );
						break;
					# -------------------------------
					//
					case 'ca_item_tags':
						$vs_element .= $this->getItemTagHTMLFormBundle( $pa_options['request'], $pa_options['formName'],
							$ps_placement_code, $pa_options, $pa_bundle_settings );
						break;
					# -------------------------------
					//
					case 'ca_item_comments':
						$vs_element .= $this->getItemCommentHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_options, $pa_bundle_settings );
						break;
					# -------------------------------
					// This bundle is only available items for ca_object_representations
					case 'ca_representation_transcriptions':
						$vs_element = $this->getTranscriptionHTMLFormBundle( $pa_options['request'],
							$pa_options['formName'], $ps_placement_code, $pa_options, $pa_bundle_settings );
						break;
					# -------------------------------
					default:
		...
		if ( $vs_documentation_url ) {
			...
			if ( preg_match( "!^[a-z]!", $vs_documentation_url )
			     && ! preg_match( "!^http[s]?://!", $vs_documentation_url )
			) {
				$vs_documentation_url = "http://" . $vs_documentation_url;
			}
			$vs_documentation_link = "<a class='bundleDocumentationLink' href='$vs_documentation_url' target='_blank' >"
			                         . caNavIcon( __CA_NAV_ICON_INFO__, '15px' ) . "</a>";
		}


		if ( is_array( $va_violations ) && sizeof( $va_violations ) ) {
			$vs_label .= caNavIcon( __CA_NAV_ICON_ALERT__, "12px", [
				'style'   => 'margin: 0 0 5px 5px;',
				'onclick' => 'jQuery(this).parent().find(\'.caMetadataDictionaryDefinitionToggle\').click();  return false;'
			] );
		}
		...
		if ( is_array( $va_violations ) ) {
			foreach ( $va_violations as $v ) {
				if ( is_array( $v ) && isset( $v['showasprompt'] ) && (bool) $v['showasprompt']
				     && ( $v['bundle_name'] == $dict_bundle_spec )
				) {
		...
		if ( is_array( $violations_to_prompt ) && sizeof( $violations_to_prompt ) ) {
			...
			$prompt          = "<script type='text/javascript'>caPromptManager.addPrompt('{$prompt_id}', '"
			                   . addslashes( preg_replace( "![\n\r\t ]+!", " ", join( "<br/>", $violations_text ) ) )
			                   . "');</script>";
		}
	...
	public function isValidBundle( $ps_bundle_name ) {
		return ( isset( $this->BUNDLES[ $ps_bundle_name ] ) && is_array( $this->BUNDLES[ $ps_bundle_name ] ) ) ? true
			: false;
	}
	# ------------------------------------------------------
	...
	public function getDisplayLabel( $ps_field ) {
		...
		if ( ( sizeof( $va_tmp ) == 2 ) && ( $va_tmp[0] == $this->getLabelTableName() )
		     && ( $va_tmp[1] == $this->getLabelDisplayField() )
		) {
			...
			case 3:        // table_name.field_name.sub_element
				...
				switch ( $va_tmp[1] ) {
					...
					case 'preferred_labels':
						if ( method_exists( $t_instance, 'getLabelTableInstance' )
						     && ( $t_label_instance
								= $t_instance->getLabelTableInstance() )
						) {
							if ( ! isset( $va_tmp[2] ) ) {
								return caUcFirstUTF8Safe( $t_label_instance->getProperty( 'NAME_PLURAL' ) )
								       . $vs_suffix;
							} else {
								return caUcFirstUTF8Safe( $t_label_instance->getDisplayLabel( $t_label_instance->tableName()
								                                                              . '.' . $va_tmp[2] ) )
								       . $vs_suffix;
							}
						}
					...
					case 'nonpreferred_labels':
						if ( method_exists( $t_instance, 'getLabelTableInstance' )
							...
							if ( $va_tmp[0] !== $this->tableName() ) {
								$vs_suffix = ' (' . _t( 'alternates from related %1',
		...
		if ( ( $va_tmp[0] === $this->tableName() ) && isset( $this->BUNDLES[ $va_tmp[1] ] )
		     && $this->BUNDLES[ $va_tmp[1] ]['label']
		) {
				...
				switch ( $va_tmp[1] ) {
					...
					case 'preferred_labels':
						if ( method_exists( $t_instance, 'getLabelTableInstance' )
							...
							if ( ! isset( $va_tmp[2] ) ) {
								return _t( 'A list of %1 %2', $t_label_instance->getProperty( 'NAME_PLURAL' ),
									$vs_suffix );
							} else {
								return _t( 'A list of %1 %2',
									$t_label_instance->getDisplayLabel( $t_label_instance->tableName() . '.'
									                                    . $va_tmp[2] ), $vs_suffix );
							}
						}
						break;
					...
					case 'nonpreferred_labels':
						if ( method_exists( $t_instance, 'getLabelTableInstance' )
							...
							if ( ! isset( $va_tmp[2] ) ) {
								return _t( 'A list of alternate %1 %2', $t_label_instance->getProperty( 'NAME_PLURAL' ),
									$vs_suffix );
							} else {
								return _t( 'A list of alternate %1 %2',
									$t_label_instance->getDisplayLabel( $t_label_instance->tableName() . '.'
									                                    . $va_tmp[2] ), $vs_suffix );
							}
						}
	 ...
	 * <table_name>.<bundle_name> format) This method handles generation of search form widgets for (1) related tables
	 * (eg. ca_places),  preferred and non-preferred labels for both the primary and related tables, and all other
	 * types of elements for related tables. If this method can't handle the bundle it will pass the request to the
	 ...
	 *
	 * @return string HTML text of form element. Will return null (from superclass) if it is not possible to generate
	 *                an HTML form widget for the bundle.
	 *
	 */
	public function htmlFormElementForSearch( $po_request, $ps_field, $pa_options = null ) {
		...
		switch ( $va_tmp[0] ) {
			...
			case '_fieldlist':
				...
				if ( is_array( $va_fields = preg_split( "![;,]+!", caGetOption( 'fields', $pa_options, null ) ) )
				     && sizeof( $va_fields )
				) {
				...
				$va_options = caGetBundlesAvailableForSearch( $this->tableName(),
					array( 'forSelect' => true, 'filter' => $va_filter ) );
				...
				foreach (
					$va_relationship_restricted_searches as $vs_without_rel_restriction => $va_with_rel_restrictions
				) {
					foreach ( $va_with_rel_restrictions as $vs_with_rel_restriction ) {
						$vs_label                = ( isset( $va_alt_names[ $vs_with_rel_restriction ] ) )
							? $va_alt_names[ $vs_with_rel_restriction ] : $vs_with_rel_restriction;
			...
			switch ( sizeof( $va_tmp ) ) {
				...
				case 1:        // table_name
					if ( $va_tmp[0] != $this->tableName() ) {
						...
						return caHTMLTextInput( $ps_field . $vs_rel_types . ( $vb_as_array_element ? "[]" : "" ), array(
							'value' => $pa_options['values'][ $ps_field ],
							'size'  => $pa_options['width'],
							'class' => $pa_options['class'],
				...
				case 3:        // table_name.field_name.sub_element
					...
					if ( ( $va_tmp[0] != $this->tableName() ) || ( $va_tmp[1] == 'related' ) ) {
						switch ( sizeof( $va_tmp ) ) {
							case 1:
								return caHTMLTextInput( $ps_field . ( $vb_as_array_element ? "[]" : "" ), array(
									'value' => $pa_options['values'][ $ps_field ],
									'size'  => $pa_options['width'],
									'class' => $pa_options['class'],
									'id'    => $pa_options['id']
							...
							case 3:
								if ( $ps_render = caGetOption( 'render', $pa_options, null ) ) {
									switch ( $ps_render ) {
										case 'is_set':
											return caHTMLCheckboxInput( $ps_field . $vs_rel_types,
												array( 'value' => '[' . _t( 'SET' ) . ']' ) );
											break;
										case 'is':
											return caHTMLCheckboxInput( $ps_field . $vs_rel_types,
												array( 'value' => caGetOption( 'value', $pa_options, null ) ) );
											break;
									}
								}
								...
								if ( caGetOption( 'autocomplete', $pa_options, false ) ) {
									...
									return caGetAdvancedSearchFormAutocompleteJS( $po_request, $ps_field, $t_instance,
										$pa_options );
									...
									$vs_sort = caGetOption( 'sort', $pa_options,
										$t_instance->tableName() . '.preferred_labels.' . $vs_label_display_field );
									$qr_res  = call_user_func_array( $va_tmp[0] . '::find', array(
										$va_find_params,
										array(
											'restrictToTypes' => caGetOption( 'restrictToTypes', $pa_options, null ),
											'sort'            => $vs_sort,
									...
									if ( caGetOption( 'inUse', $pa_options, false ) ) {
										if ( is_array(
											$va_path = Datamodel::getPath( $this->tableName(), $va_tmp[0] ) )
										) {
											...
											if ( sizeof( $va_path ) == 3 ) {
												if ( $t_rel = Datamodel::getInstanceByTableName( $va_tmp[0], true ) ) {
													...
													if ( $this->hasField( 'access' ) && is_array( $va_access )
													     && sizeof( $va_access )
													) {
									...
									if ( $qr_res ) {
										$vs_field = ( $va_tmp[1] == 'related' ) ? $va_tmp[0] . '.' . $va_tmp[2]
											: $ps_field;
										while ( $qr_res->nextHit() ) {
											if ( ( $va_tmp[0] == 'ca_list_items' )
											     && ( ! $qr_res->get( 'parent_id' ) )
											) {
												continue;
											}
											if ( is_array( $va_access ) && sizeof( $va_access )
											     && ! in_array( $qr_res->get( $va_tmp[0] . '.access' ), $va_access )
											) {
												continue;
											}
											if ( is_array( $va_in_use_list )
											     && ! in_array( $vn_item_id = $qr_res->get( $vs_rel_pk ),
													$va_in_use_list )
											) {
												continue;
											}
											$va_opts[ $qr_res->get( $va_tmp[0]
											                        . ".preferred_labels.{$vs_label_display_field}" ) ]
												= $qr_res->get( $vs_field );
										}
									}
									...
									return caHTMLSelect( $ps_field . $vs_rel_types . ( $vb_as_array_element ? "[]"
											: "" ), $va_opts, array(
										'value' => $pa_options['values'][ $ps_field ],
										'class' => $pa_options['class'],
										'id'    => $pa_options['id']
	 ...
	 *
	 * This method handles generation of search form widgets for (1) related tables (eg. ca_places),  preferred and
	 * non-preferred labels for both the primary and related tables, and all other types of elements for related
	 * tables. If this method can't handle the bundle it will pass the request to the superclass implementation of
	...
	public function htmlFormElementForSimpleForm( $po_request, $ps_field, $pa_options = null ) {
		...
		if ( $vs_rel_type = caGetOption( 'relationshipType', $pa_options, null ) ) {
			$vs_buf .= caHTMLHiddenInput( $ps_field . '_relationship_type' . ( $vb_as_array_element ? "[]" : "" ),
				array( 'value' => $vs_rel_type ) );
		}
		if ( $vs_type = caGetOption( 'type', $pa_options, null ) ) {
			$vs_buf .= caHTMLHiddenInput( $ps_field . '_type' . ( $vb_as_array_element ? "[]" : "" ),
				array( 'value' => $vs_type ) );
		}
				...
				if ( $va_tmp[0] != $this->tableName() ) {
					...
					if ( caGetOptions( 'autocomplete', $pa_options, false ) ) {
						...
						if ( ! is_null( $index = caGetOption( 'index', $pa_options, null ) ) ) {
							...
							$values      = $this->get( $ps_field,
								[ 'returnAsArray' => true, 'restrictToRelationshipTypes' => [ $vs_rel_type ] ] );
							$related_ids = $this->get( $t_instance->primaryKey( true ),
								[ 'returnAsArray' => true, 'restrictToRelationshipTypes' => [ $vs_rel_type ] ] );
							$value       = $values[ $value_index ];
							$related_id  = $related_ids[ $value_index ];
						}

						return $vs_buf . caGetAdvancedSearchFormAutocompleteJS( $po_request, $ps_field, $t_instance,
								array(
									'index'                       => $index,
									'value'                       => $value,
									'id_value'                    => $related_id,
									...
									'class'                       => $pa_options['class'],
									'id'                          => str_replace( '.', '_', $ps_field ),
									'asArrayElement'              => $vb_as_array_element,
									'restrictToRelationshipTypes' => [ $vs_rel_type ]
								) );
					}

					return $vs_buf . caHTMLTextInput( $ps_field . ( $vb_as_array_element ? "[]" : "" ), array(
							'value' => $value,
							'size'  => $pa_options['width'],
							'class' => $pa_options['class'],
							'id'    => str_replace( '.', '_', $ps_field )
				...
				switch ( $va_tmp[1] ) {
					...
					default:
						if ( ( $va_tmp[0] != $this->tableName() ) || ( $va_tmp[1] == 'related' ) ) {
							switch ( sizeof( $va_tmp ) ) {
								case 1:
									return $vs_buf . caHTMLTextInput( $ps_field . ( $vb_as_array_element ? "[]" : "" ),
											array(
												'value' => $value,
												'size'  => $pa_options['width'],
												'class' => $pa_options['class'],
								...
								case 3:
									if ( caGetOption( 'autocomplete', $pa_options, false ) ) {
										...
										if ( ! is_null( $index = caGetOption( 'index', $pa_options, null ) ) ) {
											...
											$related_ids = $this->get( $ps_field . "." . $t_instance->primaryKey(),
												[ 'returnAsArray' => true ] );
											$value       = $values[ $index ];
											$related_id  = $related_ids[ $index ];
										}

										return $vs_buf . caGetAdvancedSearchFormAutocompleteJS( $po_request, $ps_field,
												$t_instance, array(
													'index'          => $index,
													'value'          => $value,
													'id_value'       => $related_id,
									...
									return $vs_buf . $t_instance->htmlFormElementForSearch( $po_request, $ps_field,
											$pa_options );
									break;
							}
						}
	 ...
	 *                          force = list of bundles to force onto form if they are not included in the UI; forced
	 *                          bundles will be included at the bottom of the form forceHidden = list of *intrinsic*
	 *                          fields to force onto form as hidden <input> elements if they are not included in the
	 *                          UI; NOTE: only intrinsic fields may be specified omit = list of bundles to omit from
	 *                          form in the event they are included in the UI restrictToTypes = bundles =
	 *                          dontAllowBundleShowHide = Do not provide per-bundle show/hide control. [Default is
	 *                          false]
	 *
	...
	public function getBundleFormHTMLForScreen( $pm_screen, $pa_options, &$pa_placements = null ) {
		$va_omit_bundles                = ( isset( $pa_options['omit'] ) && is_array( $pa_options['omit'] ) )
			? $pa_options['omit'] : array();
		...
		if ( ( $vn_ui_access = ca_editor_uis::getAccessForUI( $pa_options['request'], $t_ui->getPrimaryKey() ) )
		     === __CA_BUNDLE_ACCESS_NONE__
		) {
			// no access to UI
			$this->postError( 2320, _t( 'Access denied to UI %1', $t_ui->get( 'editor_code' ) ),
				"BundlableLabelableBaseModelWithAttributes->getBundleFormHTMLForScreen()" );
		...
		if ( isset( $pa_options['bundles'] ) && is_array( $pa_options['bundles'] ) ) {
			...
			if ( ( $vn_screen_access = ca_editor_uis::getAccessForScreen( $pa_options['request'], $pm_screen ) )
			     === __CA_BUNDLE_ACCESS_NONE__
			) {
				// no access to screen
				$this->postError( 2320, _t( 'Access denied to screen %1', $pm_screen ),
					"BundlableLabelableBaseModelWithAttributes->getBundleFormHTMLForScreen()" );
		...
		if ( is_array( $va_bundles ) ) {
			...
			foreach ( $va_bundles as $va_bundle ) {
				...
				$va_definition_bundle_names[ ( ( ! Datamodel::tableExists( $va_bundle['bundle_name'] )
				                                 && ! preg_match( "!^{$vs_table_name}\.!", $va_bundle['bundle_name'] ) )
					? "{$vs_table_name}." : "" ) . str_replace( "ca_attribute_", "", $va_bundle['bundle_name'] ) ]
					= 1;
			}
			...
			if ( is_subclass_of( $this, 'BaseRelationshipModel' ) ) {
				...
				if ( isset( $pa_options['restrictToTypes'] ) && is_array( $pa_options['restrictToTypes'] ) ) {
					$va_valid_element_codes
						= $this->getApplicableElementCodesForTypes( caMakeRelationshipTypeIDList( $vs_table_name,
						$pa_options['restrictToTypes'] ) );
				} else {
				...
				$vs_hier_parent_id_fld = isset( $this->HIERARCHY_PARENT_ID_FLD ) ? $this->HIERARCHY_PARENT_ID_FLD
					: null;

				if ( isset( $pa_options['restrictToTypes'] ) && is_array( $pa_options['restrictToTypes'] ) ) {
					$va_valid_element_codes
						= $this->getApplicableElementCodesForTypes( caMakeTypeIDList( $vs_table_name,
						$pa_options['restrictToTypes'] ) );
				} else {
					$va_valid_element_codes = null;
				}
			...
			foreach ( $va_bundles as $va_bundle ) {
				...
				if ( $vs_element_set_code = preg_replace( "/^(ca_attribute_|" . $this->tableName() . "\.)/", "",
					$va_bundle['bundle_name'] )
				) {
					if ( ( $o_element = ca_metadata_elements::getInstance( $vs_element_set_code ) )
					     && ( $this->hasElement( $vs_element_set_code ) )
					) {
				...
				if ( $vn_screen_access === __CA_BUNDLE_ACCESS_READONLY__ ) {
					$va_bundle['settings']['readonly']
						= true;    // force all bundles to read-only when user has "Read" access to screen
				...
				if ( $vs_bundle_form_html = $this->getBundleFormHTML( $va_bundle['bundle_name'],
					'P' . $va_bundle['placement_id'], $va_bundle['settings'],
					array_merge( $pa_options, [ 'bundle_id' => "{$pm_screen}_{$va_bundle['placement_id']}" ] ),
					$vs_bundle_display_name )
				) {
					$va_bundle_html[ $va_bundle['placement_code'] ]
						                                             = "<a name=\"{$pm_screen}_{$va_bundle['placement_id']}\"></a><span id=\"{$pm_screen}_{$va_bundle['placement_id']}_bundle\">{$vs_bundle_form_html}</span>";
					$va_bundles_present[ $va_bundle['bundle_name'] ] = true;

					$pa_placements["{$pm_screen}_{$va_bundle['placement_id']}"] = array(
						'name'         => $vs_bundle_display_name ? $vs_bundle_display_name
							: $this->getDisplayLabel( $vs_table_name . "." . $va_bundle['bundle_name'] ),
		...
		if ( ! $vn_pk_id ) {
			...
			foreach ( $va_mandatory_fields as $vs_field ) {
				if ( ! isset( $va_bundles_present[ $vs_field ] ) || ! $va_bundles_present[ $vs_field ] ) {
					$va_bundle_html[ $vs_field ] = $this->getBundleFormHTML( $vs_field, 'mandatory_' . $vs_field,
						array(), $pa_options );
				}
			}
			...
			if ( isset( $this->ATTRIBUTE_TYPE_ID_FLD ) && $this->ATTRIBUTE_TYPE_ID_FLD
			     && ! in_array( 'type_id', $va_omit_bundles )
			) {
				$va_bundle_html[ $this->ATTRIBUTE_TYPE_ID_FLD ] = caHTMLHiddenInput( $this->ATTRIBUTE_TYPE_ID_FLD,
					array( 'value' => $pa_options['request']->getParameter( $this->ATTRIBUTE_TYPE_ID_FLD, pString ) ) );
			}

			// add parent_id
			if ( isset( $this->HIERARCHY_PARENT_ID_FLD ) && $this->HIERARCHY_PARENT_ID_FLD
			     && ! in_array( 'parent_id', $va_omit_bundles )
			) {
				$va_bundle_html[ $this->HIERARCHY_PARENT_ID_FLD ] = caHTMLHiddenInput( $this->HIERARCHY_PARENT_ID_FLD,
					array(
						'value' => $pa_options['request']->getParameter( $this->HIERARCHY_PARENT_ID_FLD, pInteger )
					) );
			}
			...
			if ( isset( $pa_options['force'] ) && $pa_options['force'] ) {
				...
				foreach ( $pa_options['force'] as $vn_x => $vs_bundle ) {
					if ( ! isset( $va_bundles_present[ $vs_bundle ] ) || ! $va_bundles_present[ $vs_bundle ] ) {
						$va_bundle_html[ '_force_' . $vs_bundle ] = $this->getBundleFormHTML( $vs_bundle,
							'force_' . $vs_field, array(), $pa_options );
					}
				}
			}
			...
			if ( isset( $pa_options['forceHidden'] ) && $pa_options['forceHidden'] ) {
				...
				foreach ( $pa_options['forceHidden'] as $vn_x => $vs_field ) {
					if ( ! isset( $va_bundles_present[ $vs_field ] ) || ! $va_bundles_present[ $vs_field ] ) {
						$va_bundle_html[ '_force_hidden_' . $vs_field ] = caHTMLHiddenInput( $vs_field,
							array( 'value' => $pa_options['request']->getParameter( $vs_field, pString ) ) );
					}
				}
			}
		}
	 ...
	 *
	 */
	public function getHierarchyNavigationHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pa_options = null, $pa_bundle_settings = null
	) {

		$o_view = $this->_getHierarchyLocationHTMLFormBundleInfo( $po_request, $ps_form_name, $ps_placement_code,
			$pa_options, $pa_bundle_settings );
	 ...
	 *
	 */
	public function getHierarchyLocationHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pa_options = null, $pa_bundle_settings = null
	) {
	 ...
	 *
	 */
	private function _getHierarchyLocationHTMLFormBundleInfo(
		$po_request, $ps_form_name, $ps_placement_code, $pa_options = null, $pa_bundle_settings = null
	) {
		$vs_view_path = ( isset( $pa_options['viewPath'] ) && $pa_options['viewPath'] ) ? $pa_options['viewPath']
			: $po_request->getViewsDirectoryPath();
		...
		if ( $this->supportsPreferredLabelFlag() ) {
			if ( ! ( $va_ancestor_list = $this->getHierarchyAncestors( $vn_id, array(
				...
				'additionalTableWheres'       => array(
					'(' . $vs_label_table_name . '.is_preferred = 1 OR ' . $vs_label_table_name
					. '.is_preferred IS NULL)'
				),
				'includeSelf'                 => true
			) ) )
			) {
		...
		foreach ( $va_ancestor_list as $vn_ancestor_id => $va_info ) {
			...
			$vn_locale_id                                           = isset( $va_info['NODE']['locale_id'] )
				? $va_info['NODE']['locale_id'] : null;
			$va_ancestor                                            = array(
				'item_id'   => $vn_item_id = $va_info['NODE'][ $vs_pk ],
				'parent_id' => $va_info['NODE']['parent_id'],
				'label'     => $va_info['NODE'][ $vs_display_fld ] ? $va_info['NODE'][ $vs_display_fld ]
					: $va_info['NODE'][ $vs_idno_field ],
				'idno'      => $va_info['NODE'][ $vs_idno_field ],
		...
		$vs_object_collection_rel_type = $this->getAppConfig()
		                                      ->get( 'ca_objects_x_collections_hierarchy_relationship_type' );
		$vb_strict_type_hierarchy      = $po_request->config->get( $this->tableName()
		                                                           . '_enforce_strict_type_hierarchy' );

		$t_object = new ca_objects();
		$t_rel
		          = ca_relationship_types::findAsInstance( [
			'table_num' => Datamodel::getTableNum( 'ca_objects_x_collections' ),
			'type_code' => $vs_object_collection_rel_type
		] );

		$o_view->setVar( 'objectTypeList',
			trim( $t_object->getTypeListAsHTMLFormElement( "{$ps_placement_code}{$ps_form_name}object_type_id",
				...
				[
					...
					'directChildrenOnly'                   => $vb_strict_type_hierarchy
					                                          && ( $vb_strict_type_hierarchy !== '~' ),
					'restrictToTypes'                      => $t_rel
						? [ $t_rel->get( 'ca_relationship_types.sub_type_left_id' ) ] : null,
					'dontIncludeSubtypesInTypeRestriction' => $t_rel
						? ! $t_rel->get( 'ca_relationship_types.include_subtypes_left' ) : null
				] ) ) );


		$o_view->setVar( 'object_collection_collection_ancestors',
		...
		if ( ( $this->tableName() == 'ca_objects' )
			...
			if ( is_array( $va_collections = $t_object->getRelatedItems( 'ca_collections',
				array( 'restrictToRelationshipTypes' => array( $vs_object_collection_rel_type ) ) ) )
			) {
				...
				foreach ( $va_collections as $vs_key => $va_collection ) {
					...
					if ( ! ( $va_collection_ancestor_list
						= $t_collection->getHierarchyAncestors( $va_collection['collection_id'], array(
				...
				$o_view->setVar( 'object_collection_collection_ancestors',
					array_reverse( $va_related_collections_by_level, true ) );
			}
		}
	 ...
	 * @return array|mixed
	 */
	public function getRelatedBundleFormValues(
		$po_request, $ps_form_name, $ps_related_table, $ps_placement_code = null, $pa_bundle_settings = null,
		$pa_options = null
	) {
		if ( ! is_array( $pa_bundle_settings ) ) {
			$pa_bundle_settings = array();
		...
		$sort_direction = caGetOption( 'sortDirection', $pa_options,
			caGetOption( 'sortDirection', $pa_bundle_settings, null ) );
		...
		if ( isset( $pa_bundle_settings['restrictToTermsRelatedToCollection'] )
		     && $pa_bundle_settings['restrictToTermsRelatedToCollection']
		) {
			$va_get_related_opts['restrict_to_relationship_types']
		...
		if ( $s = caGetOption( 'sort', $pa_options, caGetOption( 'sort', $pa_bundle_settings, null ) ) ) {
			...
			$va_get_related_opts['sortDirection'] = caGetOption( 'sortDirection', $pa_options,
				caGetOption( 'sortDirection', $pa_bundle_settings, null ) );
		}
		...
		$policy = caGetOption( 'showCurrentOnly', $pa_bundle_settings, false ) ? caGetOption( 'policy',
			$pa_bundle_settings, null ) : null;
		...
		if ( $ps_related_table == 'ca_sets' ) {
			...
			if ( $ps_sort = caGetOption( 'sort', $va_get_related_opts, null ) ) {
				$va_items = caSortArrayByKeyInValue( $va_items, array( $ps_sort ),
					caGetOption( 'sortDirection', $va_get_related_opts, 'ASC' ) );
			}
			...
			if ( ( $policy )
			     && is_array( $h = $this->getContents( $policy, [ 'returnHistoryTrackingData' => true ] ) )
			) {
				...
				$va_items = array_map( function ( $v ) use ( $rel_pk ) {
					return [
						'relation_id' => $v['tracked_row_id'],
						$rel_pk       => $v['row_id']
					];    // Convert history list to item list format required below
				}, array_filter( $h, function ( $v ) use ( $rel_table_num ) {
					return ( (int) $v['table_num']
					         === $rel_table_num );        // Filter out any current values thar aren't for the the related table
				} ) );
			} else {
				$va_items = $this->getRelatedItems( $ps_related_table, array_merge( $va_get_related_opts, [
					'returnAs'      => 'data',
					'sort'          => $sort,
	 ...
	 *
	 */
	public function getRelatedHTMLFormBundle(
		$po_request, $ps_form_name, $ps_related_table, $ps_placement_code = null, $pa_bundle_settings = null,
		$pa_options = null
	) {
		global $g_ui_locale;
		...
		$vb_read_only = ( $po_request->user->getBundleAccessLevel( $this->tableName(), $ps_related_table )
		...
		if ( method_exists( $t_item, 'policy2bundleconfig' )
		     && ( $default_policy
				= $t_item->getDefaultHistoryTrackingCurrentValuePolicy() )
		) {    // TODO: make policy configurable?
			$o_view->setVar( 'history_tracking_policy', $default_policy );
			$o_view->setVar( 'history_tracking_policy_info',
				$t_item::policy2bundleconfig( [ 'policy' => $default_policy ] ) );
		}
		...
		if ( method_exists( $t_item_rel, 'getRelationshipTypes' ) ) {
			$o_view->setVar( 'relationship_types',
				$t_item_rel->getRelationshipTypes( null, null, array_merge( $pa_options, $pa_bundle_settings ) ) );
			$o_view->setVar( 'relationship_types_by_sub_type',
				$t_item_rel->getRelationshipTypesBySubtype( $this->tableName(), $this->get( 'type_id' ),
					array_merge( $pa_options, $pa_bundle_settings ) ) );
		}
		$o_view->setVar( 't_subject', $this );
		$va_initial_values = $this->getRelatedBundleFormValues( $po_request, $ps_form_name, $ps_related_table,
			$ps_placement_code, $pa_bundle_settings, $pa_options );

		$c = ( caGetOption( 'limit', $pa_options, caGetOption( 'numPerPage', $pa_bundle_settings, null ) ) > 0 )
			? $this->getRelatedItems( $ps_related_table, array_merge( $pa_bundle_settings, [ 'returnAs' => 'count' ] ) )
			: sizeof( $va_initial_values );
				...
				if ( $t_item->load( $vn_id ) ) {
					...
					$va_force_new_values[ $vn_id ] = array_merge(
						...
						array(
							...
							'idno_stub'            => ( $vn_idno_stub = $t_item->get( 'idno_stub' ) ) ? $vn_idno_stub
								: null,
		...
		$o_view->setVar( 'defaultRepresentationUploadType',
			$po_request->user->getVar( 'defaultRepresentationUploadType' ) );
	 ...
	 */
	public function getRelatedListHTMLFormBundle(
		$po_request, $ps_bundle_name, $ps_form_name, $ps_placement_code = null, $pa_bundle_settings = null,
		$pa_options = null
	) {
		global $g_ui_locale;
		...
		$vb_read_only = ( $po_request->user->getBundleAccessLevel( $this->tableName(), $ps_bundle_name )
		                  == __CA_BUNDLE_ACCESS_READONLY__ ) ? true : false;
		if ( ! $pa_bundle_settings['readonly'] ) {
			$pa_bundle_settings['readonly'] = ( ! isset( $pa_bundle_settings['readonly'] )
			                                    || ! $pa_bundle_settings['readonly'] ) ? $vb_read_only : true;
		...
		$o_view->setVar( 'relationship_types',
			method_exists( $t_item_rel, 'getRelationshipTypes' ) ? $t_item_rel->getRelationshipTypes( null, null,
				array_merge( $pa_options, $pa_bundle_settings ) ) : [] );
		$o_view->setVar( 'relationship_types_by_sub_type', method_exists( $t_item_rel, 'getRelationshipTypesBySubtype' )
			? $t_item_rel->getRelationshipTypesBySubtype( $this->tableName(), $this->get( 'type_id' ),
				array_merge( $pa_options, $pa_bundle_settings ) ) : [] );

		$va_initial_values = $this->getRelatedBundleFormValues( $po_request, $ps_form_name, $vs_table_name,
			$ps_placement_code, $pa_bundle_settings, $pa_options );
	...
	protected function getBundleListsForScreen( $pm_screen, $po_request, $t_ui, $pa_options = null ) {
		...
		if ( is_array( $va_bundles ) ) {
			foreach ( $va_bundles as $vn_i => $va_tmp ) {
				...
				if ( ( $po_request->user->getBundleAccessLevel( $this->tableName(), $va_tmp['bundle_name'] ) )
				     < __CA_BUNDLE_ACCESS_EDIT__
				) {    // don't save bundles use doesn't have edit access for
					continue;
				}
		...
		if ( ! is_array( $va_fields_by_type['intrinsic'] ) ) {
			$va_fields_by_type['intrinsic'] = array();
		}
		if ( ! $this->getPrimaryKey() ) {
		...
		if ( ( $this->tableName() == 'ca_objects' ) && ( ! in_array( 'lot_id', $va_fields_by_type['intrinsic'] ) )
		     && ( $po_request->getParameter( 'lot_id', pInteger ) )
		) {
	 ...
	 *                                dontReturnSerialIdno = if set idno's are always returned as set in the request
	 *                                rather than being returns as SERIAL values if so configured.
	 *
	 * @return array
	 */
	public function extractValuesFromRequest( $pm_screen, $po_request, $pa_options ) {
				...
				switch ( $vs_f ) {
					case $vs_idno_field:
						if ( ( $this->opo_idno_plugin_instance ) && ! $pa_options['dontReturnSerialIdno'] ) {
							...
							$va_values['intrinsic'][ $vs_f ]
								= $this->opo_idno_plugin_instance->htmlFormValue( $vs_idno_field, null, true );
						} else {
		...
		if ( is_array( $va_bundle_lists['fields_by_type']['attribute'] ) ) {
			foreach ( $va_bundle_lists['fields_by_type']['attribute'] as $vs_placement_code => $vs_f ) {
				...
				foreach ( $_REQUEST as $vs_key => $vs_val ) {
					...
					if (
						preg_match( '/' . $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_id
						            . '_([\w\-_]+)_new_([\d]+)/', $vs_key, $va_matches )
						|| preg_match( '/' . $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_id
						               . '_([\w\-_]+)_([\d]+)/', $vs_key, $va_matches )
					) {
						$vn_c = intval( $va_matches[2] );
						// yep - grab the locale and value
						$vn_locale_id = isset( $_REQUEST[ $vs_placement_code . $vs_form_prefix . '_attribute_'
						                                  . $vn_element_id . '_locale_id_new_' . $vn_c ] )
							? $_REQUEST[ $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_id
							             . '_locale_id_new_' . $vn_c ] : null;

						$va_attributes_by_element[ $vn_element_id ][ $vn_c ]['locale_id']
							= $va_attributes[ $vn_c ]['locale_id'] = $vn_locale_id;
						$va_attributes_by_element[ $vn_element_id ][ $vn_c ][ $va_matches[1] ]
							= $va_attributes[ $vn_c ][ $va_matches[1] ] = $vs_val;
					}
				}
			}
		}
		...
		if ( is_array( $va_bundle_lists['fields_by_type']['preferred_label'] ) ) {
			foreach ( $va_bundle_lists['fields_by_type']['preferred_label'] as $vs_placement_code => $vs_f ) {
				foreach ( $_REQUEST as $vs_key => $vs_value ) {
					if (
						! preg_match( '/' . $vs_placement_code . $vs_form_prefix . '_Pref' . 'locale_id_(new_[\d]+)/',
							$vs_key, $va_matches )
						&&
						! preg_match( '/' . $vs_placement_code . $vs_form_prefix . '_Pref' . 'locale_id_([\d]+)/',
							$vs_key, $va_matches )
					) {
						continue;
					}
					...
					( $vn_label_locale_id = $vs_value )
						...
						if ( is_array( $va_label_values = $this->getLabelUIValuesFromRequest( $po_request,
							$vs_placement_code . $vs_form_prefix, $vn_c, true ) )
						) {
		...
		if ( method_exists( $this, "getPropertyList" ) ) {
			foreach ( $this->getPropertyList() as $vs_property ) {
				$va_values['annotation_properties'][ $vs_property ] = $po_request->getParameter( $vs_property,
					pString );
			}
		}
		...
		foreach ( $_REQUEST as $vs_k => $vs_v ) {
			...
			if ( $this->getPrimaryKey() && $this->HIERARCHY_PARENT_ID_FLD && ( $vn_parent_id > 0 ) ) {
				...
				if ( $vs_parent_table == $this->tableName() ) {
					...
					if ( (bool) $this->getAppConfig()->get( 'ca_objects_x_collections_hierarchy_enabled' )
					     && ( $vs_parent_table == 'ca_collections' )
					     && ( $this->tableName() == 'ca_objects' )
					     && ( $vs_coll_rel_type = $this->getAppConfig()
					                                   ->get( 'ca_objects_x_collections_hierarchy_relationship_type' ) )
					) {
				...
				if ( $this->getPrimaryKey() && $this->HIERARCHY_PARENT_ID_FLD
	 ...
	 *        array $pa_options Optional array of parameters; expected to be the same as that passed to
	 *        saveBundlesForScreen()
	 *
	 * The processBundlesBeforeBaseModelSave() is useful for those odd cases where you need to do some processing
	 * before the basic database record defined by the model (eg. intrinsic fields and hierarchy coding) is inserted or
	 * updated. You usually don't need to use it.
	 *
	 * @param mixed       $pm_screen
	 * @param RequestHTTP $po_request
	 * @param array       $pa_options Options are:
	 *                                formName =
	 *                                dryRun = Go through the motions of saving but don't actually write information to
	 *                                the database batch = Process save in "batch" mode. Specifically this means
	 *                                honoring batch mode settings (add, replace, remove), skipping bundles that are
	 *                                not supported in batch mode and ignoring updates existingRepresentationMap = an
	 *                                array of representation_ids key'ed on file path. If set saveBundlesForScreen()
	 *                                use link the specified representation to the row it is saving rather than
	 *                                processing the uploaded file. saveBundlesForScreen() will build the map as it
	 *                                goes, adding newly uploaded files. If you want it to process a file in a batch
	 *                                situation where it should be processed the first time and linked subsequently
	 *                                then pass an empty array here. saveBundlesForScreen() will use the empty array to
	 *                                build the map.
	 *
	 * @return mixed
	 */
	public function saveBundlesForScreen( $pm_screen, $po_request, &$pa_options ) {
		...
		$vb_we_set_transaction = false;
		$vs_form_prefix        = caGetOption( 'formName', $pa_options,
			$po_request->getParameter( '_formName', pString ) );
		...
		if ( ! $this->inTransaction() ) {
			...
			if ( $vb_dryrun ) {
				$this->postError( 799,
					_t( 'Cannot do dry run save when in transaction. Try again without setting a transaction.' ),
					"BundlableLabelableBaseModelWithAttributes->saveBundlesForScreen()" );
		...
		// get items on screen
		$t_ui = caGetOption( 'ui_instance', $pa_options,
			ca_editor_uis::loadDefaultUI( $table, $po_request, $this->getTypeID() ) );
		...
		if ( is_array( $va_fields_by_type['intrinsic'] ) ) {
			...
			foreach ( $va_fields_by_type['intrinsic'] as $vs_placement_code => $vs_f ) {
				...
				if ( $vb_batch ) {
					$vs_batch_mode = $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_batch_mode",
						pString );
					if ( $vs_batch_mode == '_disabled_' ) {
						continue;
					}
				}
				if ( isset( $_FILES["{$vs_placement_code}{$vs_form_prefix}{$vs_f}"] )
				     && $_FILES["{$vs_placement_code}{$vs_form_prefix}{$vs_f}"]
				) {
					// media field
					$this->set( $vs_f, $_FILES["{$vs_placement_code}{$vs_form_prefix}{$vs_f}"]['tmp_name'],
						array( 'original_filename' => $_FILES["{$vs_placement_code}{$vs_form_prefix}{$vs_f}"]['name'] ) );
				} else {
					switch ( $vs_f ) {
						case 'access':
							if ( (bool) $this->getAppConfig()->get( $this->tableName() . '_allow_access_inheritance' )
								...
								$this->set( 'access_inherit_from_parent',
									$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}access_inherit_from_parent",
										pInteger ) );
							}
							if ( ! (bool) $this->getAppConfig()->get( $this->tableName() . '_allow_access_inheritance' )
							     || ! $this->hasField( 'access_inherit_from_parent' )
							     || ! (bool) $this->get( 'access_inherit_from_parent' )
							) {
								$this->set( 'access',
									$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}access",
										pString ) );
							}
							break;
						case $vs_idno_field:
							if ( ! (bool) $this->getAppConfig()->get( $this->tableName()
							                                          . '_dont_allow_editing_of_codes_when_in_use' )
							     || ! $this->getPrimaryKey()
							) {
								if ( $this->opo_idno_plugin_instance ) {
									$this->opo_idno_plugin_instance->setDb( $this->getDb() );
									...
									$this->set( $vs_f,
										$vs_tmp = $this->opo_idno_plugin_instance->htmlFormValue( $vs_idno_field ) );
								} else {
									$this->set( $vs_f,
										$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}{$vs_f}",
											pString ) );
								}
							}
							break;
						default:
							...
							if ( ! strlen(
								$vs_v = $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}{$vs_f}",
									pString ) )
							) {
		...
		if ( isset( $va_fields_by_type['attribute'] ) && is_array( $va_fields_by_type['attribute'] ) ) {
			...
			foreach ( $va_fields_by_type['attribute'] as $vs_placement_code => $vs_f ) {
				...
				$vs_batch_mode = $_REQUEST[ $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_id
				                            . '_batch_mode' ];
				...
				foreach ( $_REQUEST as $vs_key => $vs_val ) {
					...
					if ( preg_match( '/' . $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_id
					                 . '_([\w\d\-_]+)_new_([\d]+)/', $vs_key, $va_matches )
					) {
						if ( $vb_batch ) {
						...
						$va_inserted_attributes_by_element[ $vn_element_id ][ $vn_c ]['locale_id']
							= $va_attributes_to_insert[ $vn_c ]['locale_id'] = $vn_locale_id;
						$va_inserted_attributes_by_element[ $vn_element_id ][ $vn_c ][ $va_matches[1] ]
							= $va_attributes_to_insert[ $vn_c ][ $va_matches[1] ] = $vs_val;
					} else {
						// is it a delete key?
						if ( preg_match( '/' . $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_id
						                 . '_([\d]+)_delete/', $vs_key, $va_matches )
						) {
				...
				foreach ( $_FILES as $vs_key => $va_val ) {
					if ( preg_match( '/' . $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_id
					                 . '_locale_id_new_([\d]+)/', $vs_key, $va_locale_matches )
					) {
				...
				if ( ! $vb_batch ) {
					...
					foreach ( $va_attributes_to_delete as $vn_attribute_id => $vb_tmp ) {
						$this->removeAttribute( $vn_attribute_id, $vs_f,
							array( 'pending_adds' => $va_attributes_to_insert ) );
					}
				}
				// do inserts
				...
				if ( ! $vb_batch ) {
					...
					if ( is_array( $va_attrs = $this->getAttributesByElement( $vn_element_id ) ) ) {
						...
						foreach ( $va_attrs as $o_attr ) {
							...
							if ( isset( $va_inserted_attributes[ $vn_attribute_id ] )
							     && $va_inserted_attributes[ $vn_attribute_id ]
							) {
								continue;
							}
							if ( isset( $va_attributes_to_delete[ $vn_attribute_id ] )
							     && $va_attributes_to_delete[ $vn_attribute_id ]
							) {
								continue;
							}
							...
							// skip element
							if ( $po_request->getParameter( $vs_placement_code . $vs_form_prefix . '_attribute_'
							                                . $vn_element_set_id . '_dont_save_' . $vn_attribute_id,
								pInteger )
							) {
								continue;
							}
							...
							$va_attr_update = array(
								'locale_id' => $po_request->getParameter( $vs_placement_code . $vs_form_prefix
								                                          . '_attribute_' . $vn_element_set_id
								                                          . '_locale_id_' . $vn_attribute_id, pString )
							);
							...
							foreach ( $va_sub_elements as $vn_i => $va_element_info ) {
								...
								$vs_k = $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_set_id . '_'
								        . $vn_element_id . '_' . $vn_attribute_id;
		...
		if ( is_array( $va_fields_by_type['special'] ) ) {
			foreach ( $va_fields_by_type['special'] as $vs_placement_code => $vs_bundle ) {
				...
				if ( $vb_batch
				     && ( $po_request->getParameter( $vs_placement_code . $vs_form_prefix . '_batch_mode', pString )
				          !== '_replace_' )
				) {
					continue;
				}

				$va_parent_tmp = explode( "-",
					$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_new_parent_id", pString ) );
					...
					if ( $vs_parent_table == $this->tableName() ) {
						...
						if ( (bool) $this->getAppConfig()->get( 'ca_objects_x_collections_hierarchy_enabled' )
							...
							if ( ! ( $this->addRelationship( 'ca_collections', $vn_parent_id, $vs_coll_rel_type ) ) ) {
								$this->postError( 2510, _t( 'Could not move object under collection: %1',
									join( "; ", $this->getErrors() ) ),
									"BundlableLabelableBaseModelWithAttributes->saveBundlesForScreen()" );
							}
						}
		...
		if ( $this->getProperty( 'LABEL_TABLE_NAME' ) ) {
			$vb_check_for_dupe_labels      = $this->_CONFIG->get( 'allow_duplicate_labels_for_' . $this->tableName() )
				? false : true;
			$vb_error_inserting_pref_label = false;
			if ( is_array( $va_fields_by_type['preferred_label'] ) ) {
				$vs_label_must_be_in_list = $this->_CONFIG->get( 'preferred_label_for_' . $this->tableName()
				                                                 . '_must_be_in_list' );
				...
				foreach ( $va_fields_by_type['preferred_label'] as $vs_placement_code => $vs_f ) {
						...
						foreach ( $va_preferred_labels as $vn_item_id => $va_labels_by_locale ) {
							foreach ( $va_labels_by_locale as $vn_locale_id => $va_label_list ) {
								foreach ( $va_label_list as $va_label ) {
									if ( $vn_label_locale_id = $po_request->getParameter( $vs_placement_code
									                                                      . $vs_form_prefix . '_Pref'
									                                                      . 'locale_id_'
									                                                      . $va_label['label_id'],
										pString )
									) {
										$vn_label_type_id = $po_request->getParameter( $vs_placement_code
										                                               . $vs_form_prefix . '_Pref'
										                                               . 'type_id_'
										                                               . $va_label['label_id'],
											pInteger );
										if ( is_array(
											$va_label_values = $this->getLabelUIValuesFromRequest( $po_request,
												$vs_placement_code . $vs_form_prefix, $va_label['label_id'], true ) )
										) {
											if ( $vb_check_for_dupe_labels
											     && $this->checkForDupeLabel( $vn_label_locale_id, $va_label_values )
											) {
												$this->postError( 1125,
													_t( 'Value <em>%1</em> is already used and duplicates are not allowed',
													...
													"BundlableLabelableBaseModelWithAttributes->saveBundlesForScreen()",
													$this->tableName() . '.preferred_labels' );
												$po_request->addActionErrors( $this->errors(), 'preferred_labels' );
												continue;
											}
											if ( $vs_label_must_be_in_list
											     && ! caGetListItemIDForLabel( $vs_label_must_be_in_list,
													$vs_label_val = $va_label_values[ $this->getLabelDisplayField() ] )
											) {
												$this->postError( 1100,
													_t( 'Value <em>%1</em> is not in <em>%2</em>', $vs_label_val,
														caGetListName( $vs_label_must_be_in_list ) ),
													"BundlableLabelableBaseModelWithAttributes->saveBundlesForScreen()",
													$this->tableName() . '.preferred_labels' );
												$po_request->addActionErrors( $this->errors(), 'preferred_labels' );
												continue;
											}
											$this->editLabel( $va_label['label_id'], $va_label_values,
												$vn_label_locale_id, $vn_label_type_id, true,
											...
											$this->editLabel( $va_label['label_id'],
												array(
													$this->getLabelDisplayField() => '[' . caGetBlankLabelText( $table )
													                                 . ']'
												),
												$vn_label_locale_id,
												$vn_label_type_id,
										...
										if ( $po_request->getParameter( $vs_placement_code . $vs_form_prefix
										                                . '_PrefLabel_' . $va_label['label_id']
										                                . '_delete', pString )
										) {
											// delete
					...
					foreach ( $_REQUEST as $vs_key => $vs_value ) {
						if ( ! preg_match( '/' . $vs_placement_code . $vs_form_prefix . '_Pref'
						                   . 'locale_id_new_([\d]+)/', $vs_key, $va_matches )
						) {
							continue;
						}

						if ( $vb_batch ) {
							$vs_batch_mode = $po_request->getParameter( $vs_placement_code . $vs_form_prefix
							                                            . '_Pref_batch_mode', pString );
							switch ( $vs_batch_mode ) {
								case '_disabled_':        // skip
									continue( 2 );
									break;
						...
						if ( $vn_new_label_locale_id = $po_request->getParameter( $vs_placement_code . $vs_form_prefix
						                                                          . '_Pref' . 'locale_id_new_' . $vn_c,
							pString )
						) {

							if ( is_array( $va_label_values = $this->getLabelUIValuesFromRequest( $po_request,
								$vs_placement_code . $vs_form_prefix, 'new_' . $vn_c, true ) )
							) {
								...
								if ( $vb_batch && ( $vs_batch_mode == '_add_' ) ) {
									...
									$va_labels_for_this_locale
										= $this->getPreferredLabels( array( $vn_new_label_locale_id ) );
										...
										foreach ( $va_labels_for_this_locale as $vn_id => $va_labels_by_locale ) {
											foreach ( $va_labels_by_locale as $vn_locale_id => $va_labels ) {
												foreach ( $va_labels as $vn_i => $va_label ) {
													if ( isset( $va_label[ $this->getLabelDisplayField() ] )
													     && ( $va_label[ $this->getLabelDisplayField() ] == '['
													                                                        . caGetBlankLabelText( $table )
													                                                        . ']' )
													) {
														$this->removeLabel( $va_label['label_id'],
															array( 'queueIndexing' => true ) );
									...
									$va_labels_for_this_locale
										= $this->getPreferredLabels( array( $vn_new_label_locale_id ), true,
										array( 'forDisplay' => true ) );
									if ( is_array( $va_labels_for_this_locale )
									     && ( sizeof( $va_labels_for_this_locale ) > 0 )
									) {
										$this->postError( 1125,
											_t( 'A preferred label for this locale already exists. Only one preferred label per locale is allowed.' ),
											"BundlableLabelableBaseModelWithAttributes->saveBundlesForScreen()",
											$this->tableName() . '.preferred_labels' );
								...
								if ( $vb_check_for_dupe_labels
								     && $this->checkForDupeLabel( $vn_new_label_locale_id, $va_label_values )
								) {
									$this->postError( 1125,
										_t( 'Value <em>%1</em> is already used and duplicates are not allowed',
											join( '', array_map( function ( $v ) {
								...
								$vn_label_type_id = $po_request->getParameter( $vs_placement_code . $vs_form_prefix
								                                               . '_Pref' . 'type_id_new_' . $vn_c,
									pInteger );
		...
		if ( $this->getProperty( 'LABEL_TABLE_NAME' ) ) {
			if ( $vb_error_inserting_pref_label || ! $this->addDefaultLabel( $vn_new_label_locale_id ) ) {
				...
				if ( $vb_is_insert ) {
					$this->_FIELD_VALUES[ $this->primaryKey() ]
						= null;                                            // clear primary key, which doesn't actually exist since we rolled back the transaction
		...
		if ( $this->getProperty( 'LABEL_TABLE_NAME' ) && isset( $va_fields_by_type['nonpreferred_label'] )
		     && is_array( $va_fields_by_type['nonpreferred_label'] )
		) {
			if ( ! $vb_batch ) {
				foreach ( $va_fields_by_type['nonpreferred_label'] as $vs_placement_code => $vs_f ) {
					...
					foreach ( $va_nonpreferred_labels as $vn_item_id => $va_labels_by_locale ) {
						foreach ( $va_labels_by_locale as $vn_locale_id => $va_label_list ) {
							foreach ( $va_label_list as $va_label ) {
								if ( $vn_label_locale_id = $po_request->getParameter( $vs_placement_code
								                                                      . $vs_form_prefix . '_NPref'
								                                                      . 'locale_id_'
								                                                      . $va_label['label_id'], pString )
								) {
									$vn_label_type_id = $po_request->getParameter( $vs_placement_code . $vs_form_prefix
									                                               . '_NPref' . 'type_id_'
									                                               . $va_label['label_id'], pInteger );
									if ( is_array( $va_label_values = $this->getLabelUIValuesFromRequest( $po_request,
										$vs_placement_code . $vs_form_prefix, $va_label['label_id'], false ) )
									) {
										$this->editLabel( $va_label['label_id'], $va_label_values, $vn_label_locale_id,
											$vn_label_type_id, false, array( 'queueIndexing' => true ) );
										if ( $this->numErrors() ) {
											foreach ( $this->errors() as $o_e ) {
												switch ( $o_e->getErrorNumber() ) {
													case 795:
									...
									if ( $po_request->getParameter( $vs_placement_code . $vs_form_prefix
									                                . '_NPrefLabel_' . $va_label['label_id']
									                                . '_delete', pString )
									) {
										// delete
			...
			foreach ( $va_fields_by_type['nonpreferred_label'] as $vs_placement_code => $vs_f ) {
				...
				foreach ( $_REQUEST as $vs_key => $vs_value ) {
					if ( ! preg_match( '/^' . $vs_placement_code . $vs_form_prefix . '_NPref'
					                   . 'locale_id_new_([\d]+)/', $vs_key, $va_matches )
					) {
						continue;
					}
					$vn_c = intval( $va_matches[1] );
					if ( $vn_new_label_locale_id = $po_request->getParameter( $vs_placement_code . $vs_form_prefix
					                                                          . '_NPref' . 'locale_id_new_' . $vn_c,
						pString )
					) {
						if ( is_array( $va_label_values = $this->getLabelUIValuesFromRequest( $po_request,
							$vs_placement_code . $vs_form_prefix, 'new_' . $vn_c, false ) )
						) {
							$vn_new_label_type_id = $po_request->getParameter( $vs_placement_code . $vs_form_prefix
							                                                   . '_NPref' . 'type_id_new_' . $vn_c,
								pInteger );
			...
			foreach ( $va_fields_by_type['related_table'] as $vs_placement_code => $vs_f ) {
				...
				switch ( $vs_f ) {
					...
					case 'ca_object_representations':
						...
						$vb_allow_existing_rep
						                = (bool) $this->_CONFIG->get( 'ca_objects_allow_relationships_to_existing_representations' );
						$va_rep_ids_sorted
						                = $va_rep_sort_order = explode( ';',
							$po_request->getParameter( $vs_prefix_stub . 'ObjectRepresentationBundleList', pString ) );
						...
						if ( ! $vb_batch && is_array( $va_reps ) ) {
							if ( $va_bundle_settings['uiStyle'] === 'CLASSIC' ) {
								...
								$bundles_to_save = caGetOption( 'showBundlesForEditing', $va_bundle_settings, [],
									[ 'castTo' => 'array' ] );
							}
							...
							foreach ( $va_reps as $vn_i => $va_rep ) {
								...
								if ( is_array( $bundles_on_screen_proc ) && sizeof( $bundles_on_screen_proc ) ) {
									if ( $vb_allow_fetching_of_urls
									     && ( $vs_path = $_REQUEST[ $vs_prefix_stub . 'media_url_'
									                                . $va_rep['relation_id'] ] )
									) {
										$va_tmp           = explode( '/', $vs_path );
										$vs_original_name = array_pop( $va_tmp );
									} else {
										$vs_path          = $_FILES[ $vs_prefix_stub . 'media_'
										                             . $va_rep['relation_id'] ]['tmp_name'];
										$vs_original_name = $_FILES[ $vs_prefix_stub . 'media_'
										                             . $va_rep['relation_id'] ]['name'];
									}

									$vals = [
										'is_primary'  => $va_rep['is_primary'],
										'rep_type_id' => $va_rep['type_id']
									];
									if ( is_array( $bundles_on_screen_proc ) ) {
										foreach ( $bundles_on_screen_proc as $b ) {
											$f = array_pop( explode( '.', $b ) );
											if ( ! is_null( $po_request->parameterExists( $vs_prefix_stub . $f . '_'
											                                              . $va_rep['relation_id'] ) )
											) {
												$vals[ $f ] = $po_request->getParameter( $vs_prefix_stub . $f . '_'
												                                         . $va_rep['relation_id'],
													pString );
											}
										}
									...
									$vn_center_x = $po_request->getParameter( $vs_prefix_stub . 'center_x_'
									                                          . $va_rep['relation_id'], pString );
									$vn_center_y = $po_request->getParameter( $vs_prefix_stub . 'center_y_'
									                                          . $va_rep['relation_id'], pString );

									$vs_rep_label = $po_request->getParameter( $vs_prefix_stub . 'rep_label_'
									                                           . $va_rep['relation_id'], pString );

									$vn_rank = null;
									if ( ( $vn_rank_index = array_search( $va_rep['relation_id'], $va_rep_sort_order ) )
									     !== false
									) {
										$vn_rank = $va_rep_ids_sorted[ $vn_rank_index ];
									}

									$vn_rel_type_id = $po_request->getParameter( $vs_prefix_stub . 'rel_type_id_'
									                                             . $va_rep['relation_id'], pString );
									$t_rep          = $this->editRepresentation( $va_rep['representation_id'], $vs_path,
										$vals['locale_id'], $vals['status'], $vals['access'], $vals['is_primary'],
										$vals, array(
											'original_filename' => $vs_original_name,
											'rank'              => $vn_rank,
											'centerX'           => $vn_center_x,
											'centerY'           => $vn_center_y,
									...
									if ( $this->numErrors() ) {
										...
										$vn_object_representation_mapping_id
											= $po_request->getParameter( $vs_prefix_stub . 'importer_id_'
											                             . $va_rep['relation_id'], pInteger );

										$log = caGetImportLogger( [
											'logLevel' => $po_request->getAppConfig()
											                         ->get( 'embedded_metadata_extraction_mapping_log_level' )
										] );
										if ( ! $vn_object_representation_mapping_id
										     && is_array(
											     $media_metadata_extraction_defaults = $po_request->getAppConfig()
											                                                      ->getAssoc( 'embedded_metadata_extraction_mapping_defaults' ) )
										) {
											...
											foreach ( $media_metadata_extraction_defaults as $m => $importer_code ) {
												if ( caCompareMimetypes( $media_mimetype, $m ) ) {
													if ( ! ( $vn_object_representation_mapping_id
														...
														if ( $log ) {
															$log->logInfo( _t( 'Could not find embedded metadata importer with code %1',
																$importer_code ) );
														}
													}
													break;
										...
										if ( $vn_object_representation_mapping_id
											...
											if ( $log ) {
												$log->logDebug( _t( 'Using embedded media mapping %1 (format %2)',
													$t_mapping->get( 'importer_code' ), $format ) );
											}

											$t_importer = new ca_data_importers();
											$t_importer->importDataFromSource( $t_rep->getMediaPath( 'media',
												'original' ), $vn_object_representation_mapping_id, [
												'logLevel'                  => $po_request->getAppConfig()
												                                          ->get( 'embedded_metadata_extraction_mapping_log_level' ),
												'format'                    => $format,
												'forceImportForPrimaryKeys' => [
													$t_rep->getPrimaryKey(),
									...
									if ( $vs_rep_label ) {
										...
										if ( $t_rep->load( $va_rep['representation_id'] ) ) {
											$t_rep->replaceLabel( array( 'name' => $vs_rep_label ), $g_ui_locale_id,
												null, true, array( 'queueIndexing' => true ) );
											if ( $t_rep->numErrors() ) {
												$po_request->addActionErrors( $t_rep->errors(), $vs_f,
													$va_rep['relation_id'] );
											}
										}
									}
								...
								// is it a delete key?
								$this->clearErrors();
								if ( ( $po_request->getParameter( $vs_prefix_stub . $va_rep['relation_id'] . '_delete',
										pInteger ) ) > 0
								) {
									// delete!
						...
						if ( $vb_batch ) {
							...
							if ( ( $vs_batch_mode == '_replace_' ) && sizeof( $va_reps ) ) {
								foreach ( $va_reps as $va_rep ) {
									$pn_access = $po_request->getParameter( "{$vs_prefix_stub}access_new_0_enabled",
										pInteger ) ? $po_request->getParameter( "{$vs_prefix_stub}access_new_0",
										pInteger ) : null;
									$pn_status
									           = $po_request->getParameter( "{$vs_prefix_stub}status_new_0_enabled",
										pInteger ) ? $po_request->getParameter( "{$vs_prefix_stub}status_new_0",
										pInteger ) : null;
									$pn_type_id
									           = $po_request->getParameter( "{$vs_prefix_stub}rep_type_id_new_0_enabled",
										pInteger ) ? $po_request->getParameter( "{$vs_prefix_stub}rep_type_id_new_0",
										pInteger ) : null;
									$ps_rep_label
									           = $po_request->getParameter( "{$vs_prefix_stub}rep_label_new_0_enabled",
										pInteger )
										? trim( $po_request->getParameter( "{$vs_prefix_stub}rep_label_new_0",
											pString ) ) : null;
									$pn_is_transcribable
									           = $po_request->getParameter( "{$vs_prefix_stub}is_transcribable_new_0_enabled",
										pInteger );
									$this->editRepresentation( $va_rep['representation_id'], null, $g_ui_locale_id,
										$pn_status, $pn_access, null, [ 'is_transcribable' => $pn_is_transcribable ],
										[ 'label' => $ps_rep_label, 'type_id' => $pn_type_id ] );
								}
							}
							...
							$va_file_list = $_FILES;
							foreach ( $_REQUEST as $vs_key => $vs_value ) {
								if ( preg_match( '/^' . $vs_prefix_stub . 'media_url_new_([\d]+)$/', $vs_key,
									$va_matches )
								) {
									$va_file_list[ $vs_key ] = array(
										'url' => $vs_value
									);
								} elseif ( preg_match( '/^' . $vs_prefix_stub . 'media_new_([\d]+)$/', $vs_key,
									$va_matches )
								) {
									$va_file_list[ $vs_key ] = array(
										'tmp_name' => $vs_value,
								...
								} elseif ( preg_match( '/^' . $vs_prefix_stub . 'autocompletenew_([\d]+)$/', $vs_key,
									$va_matches )
								) {
									$va_file_list[ $vs_key ] = array(
										'tmp_name' => $vs_value,
								...
								} elseif ( preg_match( '/^' . $vs_prefix_stub . 'mediarefsnew_([\d]+)$/', $vs_key,
									$va_matches )
								) {
									$files = explode( ";", $vs_value );
									foreach ( $files as $f ) {
										$va_file_list["{$vs_key}_{$file_index}"]
											= [        // Add numeric suffix to allow for multiple uploads in a single request
											           'tmp_name' => $f,
											           'name'     => $f
										];
										$file_index ++;
							...
							foreach ( $va_file_list as $vs_key => $va_values ) {
								...
								if (
									...
									&&
									! preg_match( '/^' . $vs_prefix_stub . 'mediarefsnew_([\d]+)_([\d]+)$/', $vs_key,
										$va_matches )
									&& ( ( $vb_allow_fetching_of_urls
									       && ! preg_match( '/^' . $vs_prefix_stub . 'media_url_new_([\d]+)$/', $vs_key,
												$va_matches ) )
									     || ! $vb_allow_fetching_of_urls )
									&& ( ( $vb_allow_existing_rep
									       && ! preg_match( '/^' . $vs_prefix_stub . 'autocompletenew_([\d]+)$/',
												$vs_key, $va_matches ) )
									     || ! $vb_allow_existing_rep )
								) {
									continue;
								}
								if ( $vs_upload_type = $po_request->getParameter( $vs_prefix_stub . 'upload_typenew_'
								...
								if ( $vs_upload_type === "search" ) {
									$vn_type_id = $po_request->getParameter( $vs_prefix_stub . 'rep_type_id_new_'
									                                         . $va_matches[1], pInteger );
								} else {
									$vn_type_id = $po_request->getParameter( $vs_prefix_stub . 'rel_type_id_new_'
									                                         . $va_matches[1], pInteger );
								}

								if ( $vn_existing_rep_id = $po_request->getParameter( $vs_prefix_stub . 'idnew_'
								                                                      . $va_matches[1], pInteger )
								) {
									$this->addRelationship( 'ca_object_representations', $vn_existing_rep_id,
										$vn_type_id );
								} else {
									if ( $vb_allow_fetching_of_urls && ( $vs_path = $va_values['url'] ) ) {
										$va_tmp           = explode( '/', $vs_path );
										$vs_original_name = array_pop( $va_tmp );
									} elseif ( preg_match( "!^userMedia" . $po_request->getUserID() . "!",
										$va_values['tmp_name'] )
									) {
										if ( ! is_writeable( $vs_tmp_directory
											= $po_request->config->get( 'ajax_media_upload_tmp_directory' ) )
										) {
											$vs_tmp_directory = caGetTempDirPath();
										}
										$vs_path          = $vs_tmp_directory . '/' . $va_values['tmp_name'];
										$md               = json_decode( @file_get_contents( "{$vs_path}_metadata" ),
											true );
									...
									$vn_rep_type_id = $po_request->getParameter( [
										$vs_prefix_stub . 'rep_type_id_new_' . $va_matches[1],
										$vs_prefix_stub . 'type_id_new_' . $va_matches[1]
									], pInteger );
									if ( ! $vn_rep_type_id
									     && ! ( $vn_rep_type_id
											= caGetDefaultItemID( 'object_representation_types' ) )
									) {
										require_once( __CA_MODELS_DIR__ . '/ca_lists.php' );
										$t_list = new ca_lists();
										if ( is_array(
											$va_rep_type_ids = $t_list->getItemsForList( 'object_representation_types',
												array( 'idsOnly' => true, 'enabledOnly' => true ) ) )
										) {
											$vn_rep_type_id = array_shift( $va_rep_type_ids );
										}
									}

									if ( is_array( $pa_options['existingRepresentationMap'] )
									     && isset( $pa_options['existingRepresentationMap'][ $vs_path ] )
									     && $pa_options['existingRepresentationMap'][ $vs_path ]
									) {
										$this->addRelationship( 'ca_object_representations',
											$pa_options['existingRepresentationMap'][ $vs_path ], $vn_type_id );
										break;
									}
									...
									if ( is_array( $bundles_on_screen_proc ) ) {
										foreach ( $bundles_on_screen_proc as $b ) {
											$f = array_pop( explode( '.', $b ) );
											if ( $v = $po_request->getParameter( $vs_prefix_stub . $f . '_new_'
											                                     . $va_matches[1], pString )
											) {
												$vals[ $f ] = $v;
											}
									...
									// Get user-specified center point (images only)
									$vn_center_x = $po_request->getParameter( $vs_prefix_stub . 'center_x_new_'
									                                          . $va_matches[1], pString );
									$vn_center_y = $po_request->getParameter( $vs_prefix_stub . 'center_y_new_'
									                                          . $va_matches[1], pString );

									$t_rep = $this->addRepresentation( $vs_path, $vn_rep_type_id, $vals['locale_id'],
										$vals['status'], $vals['access'], $vn_is_primary,
										array_merge( $vals, [ 'name' => $vals['rep_label'] ] ), array(
											'original_filename'    => $vs_original_name,
									...
									if ( $this->numErrors() ) {
										...
										// import embedded metadata?
										$vn_object_representation_mapping_id
											= $po_request->getParameter( $vs_prefix_stub . 'importer_id_new_'
											                             . $va_matches[1], pInteger );
					...
					case 'ca_movements':
					case 'ca_tour_stops':
						$this->_processRelated( $po_request, $vs_f, $vs_form_prefix, $vs_placement_code,
							array( 'batch' => $vb_batch, 'settings' => $va_bundle_settings ) );
						break;
					# -------------------------------------
					case 'ca_sets':
			...
			foreach ( $va_fields_by_type['special'] as $vs_placement_code => $vs_f ) {
				...
				switch ( $vs_f ) {
					...
					case 'ca_representation_annotation_properties':
						...
						foreach ( $this->getPropertyList() as $vs_property ) {
							$this->setPropertyValue( $vs_property,
								$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}{$vs_property}",
									pString ) );
						}
						if ( ! $this->validatePropertyValues() ) {
							$po_request->addActionErrors( $this->errors(), 'ca_representation_annotation_properties',
								'general' );
						}
						$this->update();
						break;
					...
					case 'ca_sets_checklist':
						...
						$t_set = new ca_sets();
						if ( ! $vb_batch ) {
							$va_sets = caExtractValuesByUserLocale( $t_set->getSetsForItem( $this->tableNum(),
								$this->getPrimaryKey(), array( 'user_id' => $po_request->getUserID() ) ) );

							foreach ( $va_sets as $vn_set_id => $va_set_info ) {
								$vn_item_id = $va_set_info['item_id'];

								if ( $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_set_id_{$vn_item_id}_delete",
									pString )
								) {
									// delete
									$t_set->load( $va_set_info['set_id'] );
									$t_set->removeItem( $this->getPrimaryKey(),
										$po_request->getUserID() );    // remove *all* instances of the item in the set, not just the specified id
									if ( $t_set->numErrors() ) {
						...
						foreach ( $_REQUEST as $vs_key => $vs_value ) {
							if ( ! preg_match( "/{$vs_placement_code}{$vs_form_prefix}_set_id_new_([\d]+)/", $vs_key,
								$va_matches )
							) {
								continue;
							}
							$vn_c = intval( $va_matches[1] );
							if ( $vn_new_set_id
								= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_set_id_new_{$vn_c}",
								pString )
							) {
					...
					case 'ca_set_items':
						...
						$va_rids = explode( ';',
							$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}setRowIDList", pString ) );

						$this->reorderItems( $va_rids, array(
							'user_id'             => $po_request->getUserID(),
							'treatRowIDsAsRIDs'   => true,
							'deleteExcludedItems' => true
						) );
						break;
					# -------------------------------------
					...
					case 'ca_editor_ui_screens':
						...
						$va_screen_ids = explode( ';',
							$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ScreenBundleList",
								pString ) );
						...
						foreach ( $_REQUEST as $vs_key => $vs_val ) {
							...
							if ( preg_match( "!^{$vs_placement_code}{$vs_form_prefix}_name_new_([\d]+)$!", $vs_key,
								$va_matches )
							) {
								if ( ! ( $t_screen = $this->addScreen( $vs_val, $g_ui_locale_id,
									'screen_' . $this->getPrimaryKey() . '_' . $va_matches[1] ) )
								) {
									break;
								}
							...
							if ( preg_match( "!^{$vs_placement_code}{$vs_form_prefix}_([\d]+)_delete$!", $vs_key,
								$va_matches )
							) {
					...
					case 'ca_tour_stops_list':
						...
						$va_stop_ids = explode( ';',
							$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_StopBundleList",
								pString ) );
						...
						foreach ( $_REQUEST as $vs_key => $vs_val ) {
							...
							if ( preg_match( "!^{$vs_placement_code}{$vs_form_prefix}_name_new_([\d]+)$!", $vs_key,
								...
								$vn_type_id = $_REQUEST[ "{$vs_placement_code}{$vs_form_prefix}_type_id_new_"
								                         . $va_matches[1] ];
								if ( ! ( $t_stop = $this->addStop( $vs_val, $vn_type_id, $g_ui_locale_id,
									mb_substr( preg_replace( '![^A-Za-z0-9_]+!', '_', $vs_val ), 0, 255 ) ) )
								) {
									break;
								}
					...
					case 'ca_user_groups':
						...
						if ( ! $po_request->user->canDoAction( 'is_administrator' )
						     && ( $po_request->getUserID() != $this->get( 'user_id' ) )
						) {
							break;
						...
						foreach ( $_REQUEST as $vs_key => $vs_val ) {
							if ( preg_match( "!^{$vs_placement_code}{$vs_form_prefix}_id(.*)$!", $vs_key,
								$va_matches )
							) {
								$vs_effective_date
									= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_effective_date_"
									                             . $va_matches[1], pString );
								$vn_group_id
									= (int) $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_id"
									                                   . $va_matches[1], pInteger );
								$vn_access
									= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_access_"
									                             . $va_matches[1], pInteger );
								if ( $vn_access >= 0 ) {
						...
						$this->setUserGroups( $va_groups_to_set, $va_group_effective_dates,
							array( 'user_id' => $po_request->getUserID() ) );
						...
						foreach ( $_REQUEST as $vs_key => $vs_val ) {
							if ( preg_match( "!^{$vs_placement_code}{$vs_form_prefix}_id(.*)$!", $vs_key,
								...
								$vn_user_id
									       = (int) $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_id"
									                                          . $va_matches[1], pInteger );
								$vn_access = $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_access_"
								                                        . $va_matches[1], pInteger );
								if ( $vn_access > 0 ) {
					...
					case 'ca_user_roles':
						...
						foreach ( $_REQUEST as $vs_key => $vs_val ) {
							if ( preg_match( "!^{$vs_placement_code}{$vs_form_prefix}_access_([\d]+)$!", $vs_key,
								$va_matches )
							) {
					...
					case 'settings':
						...
						$this->setSettingsFromHTMLForm( $po_request,
							array( 'id' => $vs_form_prefix . '_', 'placement_code' => $vs_placement_code ) );
						break;
					# -------------------------------
					// This bundle is only available when editing objects of type ca_object_representations
					case 'ca_object_representations_media_display':
						...
						if ( $vb_use_options
							= (bool) $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_derivative_options_selector",
							pInteger )
						) {
							// update only specified versions
							$va_versions_to_process
								= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_set_versions",
								pArray );
						}
						...
						if ( $vb_use_options
						     && ( $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_derivative_options_mode",
									pString ) == 'timecode' )
						) {
							// timecode
							if ( ! (string) ( $vs_timecode
								= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_derivative_options_mode_timecode_value",
								pString ) )
							) {
								$vs_timecode = "1s";
							}
							...
							if ( $o_media->read( $this->getMediaPath( 'media', 'original' ) ) ) {
								$va_files = $o_media->writePreviews( array(
									'force'             => true,
									'outputDirectory'   => $this->_CONFIG->get( "taskqueue_tmp_directory" ),
									'minNumberOfFrames' => 1,
									'maxNumberOfFrames' => 1,
									'startAtTime'       => $vs_timecode,
									'endAtTime'         => $vs_timecode,
									'width'             => 720,
									'height'            => 540
								) );
							...
							if ( $vb_use_options
								...
								if ( ! (int) ( $vn_page
									= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_derivative_options_mode_page_value",
									pInteger ) )
								) {
									$vn_page = 1;
								}
								...
								if ( ( $vs_media_url = trim( $po_request->getParameter( $vs_key, pString ) ) )
								     && isURL( $vs_media_url )
								) {
									...
									if ( isset( $_FILES[ $vs_key ] ) ) {
										$this->set( 'media', $_FILES[ $vs_key ]['tmp_name'],
											array( 'original_filename' => $_FILES[ $vs_key ]['name'] ) );
									}
								}
							}
						}
						...
						if ( $this->changed( 'media' ) ) {
							$this->update( ( $vs_version != '_all' ) ? array(
								'updateOnlyMediaVersions' => $va_versions_to_process,
								'startAtTime'             => $vs_timecode,
								'startAtPage'             => $vn_page
							) : array() );
							if ( $this->numErrors() ) {
								$po_request->addActionErrors( $this->errors(),
									'ca_object_representations_media_display', 'general' );
							}
						}
					...
					case 'ca_object_representation_captions':
						...
						foreach ( $_REQUEST as $vs_key => $vs_val ) {
							...
							if ( preg_match( "!^{$vs_placement_code}{$vs_form_prefix}_locale_id(.*)$!", $vs_key,
								$va_matches )
							) {
								$vn_locale_id
									= (int) $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_locale_id"
									                                   . $va_matches[1], pInteger );
								$this->addCaptionFile( $_FILES[ "{$vs_placement_code}{$vs_form_prefix}_caption_file"
								                                . $va_matches[1] ]['tmp_name'], $vn_locale_id, array(
									'originalFilename' => $_FILES[ "{$vs_placement_code}{$vs_form_prefix}_caption_file"
									                               . $va_matches[1] ]['name']
								) );
							}
						}
					...
					case 'ca_object_representation_chooser':
						...
						if ( ! is_array(
							$va_rep_ids = $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}", pArray ) )
						) {
							$va_rep_ids = array();
						}

						if ( $vs_element_code = caGetOption( array( 'elementCode', 'element_code' ),
							$va_bundle_settings, null )
						) {
							if ( ! is_array(
								$va_current_rep_ids = $this->get( $this->tableName() . "." . $vs_element_code,
									array( 'returnAsArray' => true, 'idsOnly' => true ) ) )
							) {
								$va_current_rep_ids = $va_current_rep_id_with_structure = array();
							} else {
								$va_current_rep_id_with_structure = $this->get( $this->tableName() . "."
								                                                . $vs_element_code,
									array( 'returnWithStructure' => true, 'idsOnly' => true ) );
							}
					...
					case 'history_tracking_chronology':
						...
						if ( strlen(
							$pb_show_child_history = $po_request->getParameter( "showChildHistory", pInteger ) )
						) {
						...
						if ( ! caGetOption( 'hide_value_delete', $va_bundle_settings, false ) ) {
							// handle deletes
							$refs
								= $this->getHistoryReferences( [ 'policy' => $policy ] );    // get all references present in this history
							foreach ( $refs as $t => $r ) {
								if ( is_array( $rp
									= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_delete_{$t}",
									pArray ) )
								) {
									if ( is_array(
										$rows_to_delete = array_intersect( $r, $rp ) )
									) {    // only attempt to delete values passed that are actually in the history
										...
										foreach ( $rows_to_delete as $row_id ) {
											if ( $t_instance->isRelationship() && $t_instance->load( $row_id ) ) {
												if ( ! $t_instance->delete() ) {    // TODO: check error handling
													$po_request->addActionErrors( $t_instance->errors(), $vs_f,
														'general' );
												}
											} elseif ( ( $t === 'ca_object_lots' )
											           && ( $this->tableName() === 'ca_objects' )
											) {
						...
						if ( ! caGetOption( 'hide_update_location_controls', $va_bundle_settings, false )
						     || ! caGetOption( 'hide_return_to_home_location_controls', $va_bundle_settings, false )
						) {
							// set storage location
							if ( $this->hasField( 'home_location_id' )
							     && $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_storage_locations_return_homenew_0",
									pInteger ) == 1
							) {
								...
								$vn_location_id
									= $vn_location_id
									= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_storage_locations_idnew_0",
									pInteger );
							}
							...
							if ( $vn_location_id ) {
								...
								if ( $t_loc->load( $vn_location_id ) ) {
									if ( $policy ) {
										$policy_info             = $table::getHistoryTrackingCurrentValuePolicyElement( $policy,
											'ca_storage_locations', $t_loc->getTypeCode() );
										$vn_relationship_type_id = caGetOption( 'trackingRelationshipType',
											$policy_info, null );
									}
									if ( ! $vn_relationship_type_id ) {
										$vn_relationship_type_id = $this->getAppConfig()
										                                ->get( 'object_storage_location_tracking_relationship_type' );
									}

									if ( $vn_relationship_type_id ) {
										// is effective date set?
										$vs_effective_date
											= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_storage_locations__effective_datenew_0",
											pString );

										$t_item_rel = $this->addRelationship( 'ca_storage_locations', $vn_location_id,
											$vn_relationship_type_id, $vs_effective_date );
										if ( $this->numErrors() ) {
											$po_request->addActionErrors( $this->errors(), $vs_f, 'general' );
										} else {
											ca_storage_locations::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm( $po_request,
												$vs_placement_code, $vs_form_prefix . '_ca_storage_locations',
												$t_item_rel, $vn_location_id, $processed_bundle_settings );
										}

										$change_has_been_made = true;
										SearchResult::clearResultCacheForRow( 'ca_storage_locations', $vn_location_id );
										if ( $t_item_rel ) {
											SearchResult::clearResultCacheForRow( $t_item_rel->tableName(),
												$t_item_rel->getPrimaryKey() );
										}
									}
								}
							}
						...
						if ( ! caGetOption( 'hide_add_to_loan_controls', $va_bundle_settings, false ) ) {
							// set loan
							if ( $vn_loan_id
								= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_loans_idnew_0",
								pInteger )
							) {
								if ( $vn_loan_type_id
									= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_loans_type_idnew_0",
									pInteger )
								) {
									...
									if ( $this->numErrors() ) {
										...
										ca_loans::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm( $po_request,
											$vs_placement_code, $vs_form_prefix . '_ca_loans', $t_item_rel, $vn_loan_id,
											$processed_bundle_settings );
									}
						...
						if ( ! caGetOption( 'hide_add_to_movement_controls', $va_bundle_settings, false ) ) {
							// set movement
							if ( $vn_movement_id
								= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_movements_idnew_0",
								pInteger )
							) {

								if ( $vn_movement_type_id
									= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_movements_type_idnew_0",
									pInteger )
								) {

									$t_item_rel = $this->addRelationship( 'ca_movements', $vn_movement_id,
										$vn_movement_type_id );
									if ( $this->numErrors() ) {
										$po_request->addActionErrors( $this->errors(), $vs_f, 'general' );
									} else {
										ca_movements::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm( $po_request,
											$vs_placement_code, $vs_form_prefix . '_ca_movements', $t_item_rel,
											$vn_movement_id, $processed_bundle_settings );
									}
						...
						if ( ! caGetOption( 'hide_add_to_occurrence_controls', $va_bundle_settings, false ) ) {
							...
							foreach ( $va_occ_types as $vn_type_id => $vn_type_info ) {
								if ( $vn_occurrence_id
									= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_occurrences_{$vn_type_id}_idnew_0",
									pInteger )
								) {
									if ( $vn_occ_type_id
										= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_occurrences_{$vn_type_id}_type_idnew_0",
										pInteger )
									) {
										$t_item_rel = $this->addRelationship( 'ca_occurrences', $vn_occurrence_id,
											$vn_occ_type_id );
										if ( $this->numErrors() ) {
											$po_request->addActionErrors( $this->errors(), $vs_f, 'general' );
										} else {
											ca_occurrences::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm( $po_request,
												$vs_placement_code, $vs_form_prefix . '_ca_occurrences', $t_item_rel,
												$vn_occurrence_id, $processed_bundle_settings );
										}
										$change_has_been_made = true;
						...
						if ( ! caGetOption( 'hide_add_to_collection_controls', $va_bundle_settings, false ) ) {
							...
							foreach ( $va_coll_types as $vn_type_id => $vn_type_info ) {
								if ( $vn_collection_id
									= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_collections_{$vn_type_id}_idnew_0",
									pInteger )
								) {
									if ( $vn_coll_type_id
										= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_collections_{$vn_type_id}_type_idnew_0",
										pInteger )
									) {
										$t_item_rel = $this->addRelationship( 'ca_collections', $vn_collection_id,
											$vn_coll_type_id );
										if ( $this->numErrors() ) {
											$po_request->addActionErrors( $this->errors(), $vs_f, 'general' );
										} else {
											ca_collections::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm( $po_request,
												$vs_placement_code, $vs_form_prefix . '_ca_collections', $t_item_rel,
												$vn_collection_id, $processed_bundle_settings );
										}
										$change_has_been_made = true;
						...
						if ( ! caGetOption( 'hide_add_to_entity_controls', $va_bundle_settings, false ) ) {
							...
							foreach ( $va_entity_types as $vn_type_id => $vn_type_info ) {
								if ( $vn_entity_id
									= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_entities_{$vn_type_id}_idnew_0",
									pInteger )
								) {
									if ( $vn_entity_type_id
										= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_entities_{$vn_type_id}_type_idnew_0",
										pInteger )
									) {
										$t_item_rel = $this->addRelationship( 'ca_entities', $vn_entity_id,
											$vn_entity_type_id );
										if ( $this->numErrors() ) {
											$po_request->addActionErrors( $this->errors(), $vs_f, 'general' );
										} else {
											ca_entities::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm( $po_request,
												$vs_placement_code, $vs_form_prefix . '_ca_entities', $t_item_rel,
												$vn_entity_id, $processed_bundle_settings );
										}
										$change_has_been_made = true;
						...
						if ( ! caGetOption( 'hide_add_to_object_controls', $va_bundle_settings, false ) ) {
							// set object
							if ( $vn_object_id
								= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_objects_idnew_0",
								pInteger )
							) {
								if ( $vn_object_type_id
									= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_ca_objects_type_idnew_0",
									pInteger )
								) {
									$t_item_rel = $this->addRelationship( 'ca_objects', $vn_object_id,
										$vn_object_type_id );
									if ( $this->numErrors() ) {
										$po_request->addActionErrors( $this->errors(), $vs_f, 'general' );
									} else {
										ca_objects::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm( $po_request,
											$vs_placement_code, $vs_form_prefix . '_ca_objects', $t_item_rel,
											$vn_object_id, $processed_bundle_settings );
									}
					...
					case 'ca_objects_deaccession':        // object deaccession information
						...
						$this->set( 'is_deaccessioned', $vb_is_deaccessioned
							= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}is_deaccessioned",
							pInteger ) );
						$this->set( 'deaccession_notes',
							$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}deaccession_notes",
								pString ) );
						$this->set( 'deaccession_authorized_by',
							$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}deaccession_authorized_by",
								pString ) );
						$this->set( 'deaccession_type_id',
							$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}deaccession_type_id",
								pString ) );

						$this->set( 'deaccession_date',
							$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}deaccession_date",
								pString ) );
						if ( $po_request->config->get( 'deaccession_use_disposal_date' ) ) {
							$this->set( 'deaccession_disposal_date',
								$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}deaccession_disposal_date",
									pString ) );
						}
						if ( $vb_is_deaccessioned
						     && (bool) $this->getAppConfig()->get( 'deaccession_force_access_private' )
						) {
							$this->get( 'access', 0 );
					...
					case 'ca_object_checkouts':        // object checkout information
						...
						foreach ( $_REQUEST as $k => $v ) {
							if ( preg_match( "!^{$vs_placement_code}{$vs_form_prefix}(checkout|return)_notes_([\d]+)$!",
								$k, $m )
							) {
								$edits[ $m[2] ][ $m[1] ] = $v;
							}
						...
						foreach ( $edits as $checkout_id => $data ) {
							if ( $tc = ca_object_checkouts::find( [ 'checkout_id' => $checkout_id ],
								[ 'returnAs' => 'firstModelInstance' ] )
							) {
					...
					case 'ca_object_circulation_status':
						...
						$this->set( 'circulation_status_id',
							$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}ca_object_circulation_status",
								pInteger ) );
						break;
					# -------------------------------
					// This bundle is only available items for batch editing on representable models
					case 'ca_object_representations_access_status':
						if ( ( $vb_batch ) && ( is_a( $this, 'RepresentableBaseModel' ) )
						     && ( $vs_batch_mode
								= $_REQUEST[ $vs_prefix_stub . 'batch_mode' ] )
						     && ( $vs_batch_mode === '_replace_' )
						) {
							$vn_access = $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_access",
								pString );
							$vn_status = $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_status",
								pString );
							...
							if ( is_array( $va_rep_ids = $this->getRepresentationIDs() ) ) {
								foreach ( array_keys( $va_rep_ids ) as $vn_rep_id ) {
									if ( $t_rep->load( $vn_rep_id ) ) {
										...
										if ( $t_rep->numErrors() ) {
											$po_request->addActionErrors( $t_rep->errors(),
												'ca_object_representations_access_status', 'general' );
										}
									}
								}
							}
					...
					case 'ca_site_pages_content':
						if ( is_array( $va_field_list = $this->getHTMLFormElements() ) ) {
							...
							foreach ( $va_field_list as $vs_field => $va_element_info ) {
								$vs_value
									= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_{$va_element_info['code']}",
									pString );
					...
					case 'ca_site_page_media':
						...
						$vb_allow_fetching_of_urls
						                = (bool) $this->_CONFIG->get( 'allow_fetching_of_media_from_remote_urls' );
						$va_media_ids_sorted
						                = $va_mediasort_order = explode( ';',
							$po_request->getParameter( $vs_prefix_stub . 'MediaBundleList', pString ) );
						...
						if ( is_array( $va_media_ids_sorted ) ) {
							foreach ( $va_media_list as $vn_i => $va_media ) {
								$this->clearErrors();
								if ( strlen( $po_request->getParameter( $vs_prefix_stub . 'access_'
								                                        . $va_media['media_id'], pInteger ) ) > 0
								) {
									if ( $vb_allow_fetching_of_urls
									     && ( $vs_path = $_REQUEST[ $vs_prefix_stub . 'media_url_'
									                                . $va_media['media_id'] ] )
										...
										$vs_path          = $_FILES[ $vs_prefix_stub . 'media_'
										                             . $va_media['media_id'] ]['tmp_name'];
										$vs_original_name = $_FILES[ $vs_prefix_stub . 'media_'
										                             . $va_media['media_id'] ]['name'];
									}

									$vs_idno    = $po_request->getParameter( $vs_prefix_stub . 'idno_'
									                                         . $va_media['media_id'], pString );
									$vn_access  = $po_request->getParameter( $vs_prefix_stub . 'access_'
									                                         . $va_media['media_id'], pInteger );
									$vs_title   = trim( $po_request->getParameter( $vs_prefix_stub . 'title_'
									                                               . $va_media['media_id'], pString ) );
									$vs_caption = trim( $po_request->getParameter( $vs_prefix_stub . 'caption_'
									                                               . $va_media['media_id'], pString ) );

									$vn_rank = null;
									if ( ( $vn_rank_index = array_search( $va_media['media_id'], $va_mediasort_order ) )
									     !== false
									) {
										$vn_rank = $va_media_ids_sorted[ $vn_rank_index ];
									}

									$this->editMedia( $va_media['media_id'], $vs_path, $vs_title, $vs_caption, $vs_idno,
										$vn_access, [ 'original_filename' => $vs_original_name, 'rank' => $vn_rank ] );
									if ( $this->numErrors() ) {
										foreach ( $this->errors() as $o_e ) {
											switch ( $o_e->getErrorNumber() ) {
												case 795:
									...
									if ( $vs_title ) {
										...
										if ( $t_rep->load( $va_media['media_id'] ) ) {
											$t_rep->replaceLabel( array( 'name' => $vs_title ), $g_ui_locale_id, null,
												true, array( 'queueIndexing' => true ) );
											if ( $t_rep->numErrors() ) {
												$po_request->addActionErrors( $t_rep->errors(), $vs_f,
													$va_media['media_id'] );
									...
									// is it a delete key?
									$this->clearErrors();
									if ( ( $po_request->getParameter( $vs_prefix_stub . $va_media['media_id']
									                                  . '_delete', pInteger ) ) > 0
									) {
						...
						foreach ( $va_file_list as $vs_key => $va_values ) {
							...
							if ( ! preg_match( '/^' . $vs_prefix_stub . 'media_new_([\d]+)$/', $vs_key, $va_matches )
							     && ( ( $vb_allow_fetching_of_urls
							            && ! preg_match( '/^' . $vs_prefix_stub . 'media_url_new_([\d]+)$/', $vs_key,
											$va_matches ) )
							          || ! $vb_allow_fetching_of_urls )
							...
							$vs_title   = trim( $po_request->getParameter( $vs_prefix_stub . 'title_new_'
							                                               . $va_matches[1], pString ) );
							$vs_caption = $po_request->getParameter( $vs_prefix_stub . 'caption_new_' . $va_matches[1],
								pString );
							$vs_idno    = $po_request->getParameter( $vs_prefix_stub . 'idno_new_' . $va_matches[1],
								pString );
							$vn_access  = $po_request->getParameter( $vs_prefix_stub . 'access_new_' . $va_matches[1],
								pInteger );
					...
					case 'ca_item_tags':
						if ( $vb_batch ) {
							$vs_batch_mode
								= $po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}_batch_mode",
								pString );
						...
						if ( ! $vb_batch || ( $vb_batch && in_array( $vs_batch_mode, [ '_add_', '_replace_' ] ) ) ) {
							foreach ( $_REQUEST as $vs_key => $vs_val ) {
								...
								if ( preg_match( "!^{$vs_placement_code}{$vs_form_prefix}_autocompletenew_([\d]+)$!",
									$vs_key, $va_matches )
								) {
									...
									foreach ( preg_split( "![,;]+!", $vs_val ) as $v ) {
										...
										$this->addTag( $v, $po_request->getUserID(), null, 1,
											$po_request->getUserID() );
										continue;
									}
								}
						...
						if ( ! $vb_batch
						     && is_array( $ids_sorted
								= $va_rel_sort_order = explode( ';',
								$po_request->getParameter( "{$vs_placement_code}{$vs_form_prefix}BundleList",
									pString ) ) )
						) {
							$tags              = $this->getTags();
		...
		foreach ( $va_bundles as $va_bundle ) {
			...
			if ( ! Datamodel::getInstanceByTableName( $vs_bundle_name, true )
			     && ! preg_match( "!^" . $this->tableName() . "\.!", $vs_bundle_name )
			) {
		...
		try {
			...
			if ( sizeof( $va_violations ) ) {
				if ( $vb_we_set_transaction && isset( $va_violations['ERR'] ) && is_array( $va_violations['ERR'] )
				     && ( sizeof( $va_violations['ERR'] ) > 0 )
				) {
					BaseModel::unsetChangeLogUnitID();
					$this->removeTransaction( false );
					if ( $vb_is_insert ) {
					...
					foreach ( $va_violations['ERR'] as $vs_bundle => $va_errs_by_bundle ) {
						foreach ( $va_errs_by_bundle as $vn_i => $va_rule ) {
							...
							$po_request->addActionErrors( array(
								new ApplicationError( 1100,
									caExtractSettingsValueByUserLocale( 'violationMessage', $va_rule['rule_settings'] ),
									"BundlableLabelableBaseModelWithAttributes->saveBundlesForScreen()",
									'MetadataDictionary', false, false )
							), $vs_bundle, 'general' );
						}
					}
			...
			$po_request->addActionErrors( array(
				new ApplicationError( 1100, _t( 'Invalid rule expression: %1', $e->getMessage() ),
					"BundlableLabelableBaseModelWithAttributes->saveBundlesForScreen()", 'MetadataDictionary', false,
					false )
			), $vs_bundle, 'general' );
		}
		if ( $vb_dryrun ) {
	 ...
	 *
	 */
	private function _processRelated(
		$po_request, $ps_bundle_name, $ps_form_prefix, $ps_placement_code, $pa_options = null
	) {
		...
		$va_rel_ids_sorted
			= $va_rel_sort_order = explode( ';',
			$po_request->getParameter( "{$ps_placement_code}{$ps_form_prefix}BundleList", pString ) );
		...
		if ( ! $vb_batch ) {
			foreach ( $va_rel_items as $va_rel_item ) {
				...
				if ( ( ( $vn_rank_index = array_search( $va_rel_item['relation_id'], $va_rel_sort_order ) ) !== false )
				     && ( ! isset( $pa_settings['disableSorts'] ) || ! $pa_settings['disableSorts'] )
				) {
					$vn_rank = $va_rel_ids_sorted[ $vn_rank_index ];
				}

				$this->clearErrors();
				$vn_id = $po_request->getParameter( "{$ps_placement_code}{$ps_form_prefix}_id"
				                                    . $va_rel_item[ $vs_key ], pString );
				if ( $vn_id ) {
					$vn_type_id   = $po_request->getParameter( "{$ps_placement_code}{$ps_form_prefix}_type_id"
					                                           . $va_rel_item[ $vs_key ], pString );
					$vs_direction = null;
					...
					$this->editRelationship( $ps_bundle_name, $va_rel_item[ $vs_key ], $vn_id, $vn_type_id, null, null,
						$vs_direction, $vn_rank );
					...
					// is it a delete key?
					$this->clearErrors();
					if ( ( $po_request->getParameter( "{$ps_placement_code}{$ps_form_prefix}_" . $va_rel_item[ $vs_key ]
					                                  . '_delete', pInteger ) ) > 0
					) {
						$va_rels_to_delete[] = array(
							'bundle'      => $ps_bundle_name,
							'relation_id' => $va_rel_item[ $vs_key ]
						);
					}
				}
			}
		...
		if ( $vb_batch ) {
			...
			if ( $vs_batch_mode == '_delete_' ) {                // remove all relationships and return
				$this->removeRelationships( $ps_bundle_name,
					caGetOption( 'restrict_to_relationship_types', $pa_settings, null ),
					[ 'restrictToTypes' => caGetOption( 'restrict_to_types', $pa_settings, null ) ] );
		...
		foreach ( $_REQUEST as $vs_key => $vs_value ) {
			if ( preg_match( "/^{$ps_placement_code}{$ps_form_prefix}_idnew_([\d]+)/", $vs_key, $va_matches ) ) {
				$vn_c = intval( $va_matches[1] );
				if ( $vn_new_id = $po_request->getParameter( "{$ps_placement_code}{$ps_form_prefix}_idnew_{$vn_c}",
					pString )
				) {
					$vn_new_type_id
						= $po_request->getParameter( "{$ps_placement_code}{$ps_form_prefix}_type_idnew_{$vn_c}",
						pString );
			...
			if ( $ps_bundle_name == 'ca_list_items' ) {
				if ( preg_match( "/^{$ps_placement_code}{$ps_form_prefix}_item_id_new_([\d]+)/", $vs_key,
					$va_matches )
				) {
					if ( $vn_rel_type_id
						= $po_request->getParameter( "{$ps_placement_code}{$ps_form_prefix}_type_idchecklist",
						pInteger )
					) {
				...
				if ( preg_match( "/^{$ps_placement_code}{$ps_form_prefix}_item_id_([\d]+)_delete/", $vs_key,
					$va_matches )
				) {
		...
		if ( $vn_min_relationships && ( $vn_total_rel_count < $vn_min_relationships ) ) {
			$po_request->addActionErrors( array(
				new ApplicationError( 2590,
					( $vn_min_relationships == 1 ) ? _t( 'There must be at least %1 relationship for %2',
						$vn_min_relationships, Datamodel::getTableProperty( $ps_bundle_name, 'NAME_PLURAL' ) )
						: _t( 'There must be at least %1 relationships for %2', $vn_min_relationships,
						Datamodel::getTableProperty( $ps_bundle_name, 'NAME_PLURAL' ) ),
					'BundleableLabelableBaseModelWithAttributes::_processRelated()', null, null, false, false )
			), $ps_bundle_name );
		...
		if ( $vn_max_relationships && ( $vn_total_rel_count > $vn_max_relationships ) ) {
			$po_request->addActionErrors( array(
				new ApplicationError( 2590,
					( $vn_max_relationships == 1 ) ? _t( 'There must be no more than %1 relationship for %2',
						$vn_max_relationships, Datamodel::getTableProperty( $ps_bundle_name, 'NAME_PLURAL' ) )
						: _t( 'There must be no more than %1 relationships for %2', $vn_max_relationships,
						Datamodel::getTableProperty( $ps_bundle_name, 'NAME_PLURAL' ) ),
					'BundleableLabelableBaseModelWithAttributes::_processRelated()', null, null, false, false )
			), $ps_bundle_name );
		...
		foreach ( $va_rels_to_add as $va_rel_to_add ) {
			$this->addRelationship( $va_rel_to_add['bundle'], $va_rel_to_add['row_id'], $va_rel_to_add['type_id'],
				_t( caGetOption( 'effectiveDateDefault', $pa_settings, null ) ), null, $va_rel_to_add['direction'] );
			if ( $this->numErrors() ) {
				$po_request->addActionErrors( $this->errors(), $ps_bundle_name );
			}
		}
	...
	public function getRelatedItemsAsSearchResult( $pm_rel_table_name_or_num, $pa_options = null ) {
		if ( is_array( $va_related_ids = $this->getRelatedItems( $pm_rel_table_name_or_num,
			array_merge( $pa_options, array( 'idsOnly' => true, 'sort' => null ) ) ) )
		) {
	 ...
	 *
	 * @param mixed $pm_rel_table_name_or_num The table name or table number of the item type you want to get a list of
	 *                                        (eg. if you are calling this on an ca_objects instance passing
	 *                                        'ca_entities' here will get you a list of entities related to the object)
	 ...
	 *            [Default is null - nothing is excluded]. restrictToBundleValues = Restrict returned items to those
	 *            with specified bundle values. Specify an associative array with keys set to bundle names and key
	 *            values set to arrays of values to filter on (eg. [bundle_name1 => [value1, value2, ...]]). [Default is null] where = Restrict returned items to specified field values. The fields must be intrinsic and in the related table. This option can be useful when you want to efficiently fetch specific rows from a related table. Note that multiple fields/values are logically AND'ed together – all must match for a row to be returned - and that only equivalence is supported. [Default is null] criteria = Restrict returned items using SQL criteria appended directly onto the query. Criteria is used as-is and must be compatible with the generated SQL query. [Default is null] showCurrentOnly = Returns the relationship with the latest effective date for the row_id that is not greater than the current date. This option is only supported for standard many-many self and non-self relations and is ignored for all other kinds of relationships. [Default is false] showCurrentUsingDate = Bundle (intrinsic or attribute) to use to select the "current" relationship. [Default is effective_date] currentOnly = Synonym for showCurrentOnly
	 *
	 *        [Options controlling scope of data in return value]
	 *            restrictToTypes = Restrict returned items to those of the specified types. An array or
	 *            comma/semicolon delimited string of list item idnos and/or item_ids may be specified. [Default is
	 *            null] restrictToRelationshipTypes =  Restrict returned items to those related using the specified
	 *            relationship types. An array or comma/semicolon delimited string of relationship type idnos and/or
	 *            type_ids may be specified. [Default is null] excludeTypes = Restrict returned items to those *not* of
	 *            the specified types. An array or comma/semicolon delimited string of list item idnos and/or item_ids
	 *            may be specified. [Default is null] excludeRelationshipTypes = Restrict returned items to those *not*
	 *            related using the specified relationship types. An or comma/semicolon delimited string array of
	 *            relationship type idnos and/or type_ids may be specified. [Default is null] restrictToType = Synonym
	 *            for restrictToTypes. [Default is null] restrictToRelationshipType = Synonym for
	 *            restrictToRelationshipTypes. [Default is null] excludeType = Synonym for excludeTypes. [Default is null] excludeRelationshipType = Synonym for excludeRelationshipTypes. [Default is null] restrictToLists = Restrict returned items to those that are in one or more specified lists. This option is only relevant when fetching related ca_list_items. An array or comma/semicolon delimited string of list list_codes or list_ids may be specified. [Default is null] fields = array of fields (in table.fieldname format) to include in returned data. [Default is null] returnNonPreferredLabels = Return non-preferred labels in returned data. [Default is false] returnLabelsAsArray = Return all labels associated with row in an array, rather than as a text value in the current locale. [Default is false] dontReturnLabels = Don't include labels in returned data. [Default is false] idsOnly = Return one-dimensional array of related primary key values only. [Default is false]
	 *
	 *        [Options controlling format of data in return value]
	 *            useLocaleCodes = Return locale values as codes (Ex. en_US) rather than numeric database-specific
	 *            locale_ids. [Default is false] sort = Array list of bundles to sort returned values on. The sortable
	 *            bundle specifiers are fields with or without tablename. Only those fields returned for the related
	 *            table (intrinsics, attributes and label fields) are sortable. [Default is null] sortDirection =
	 *            Direction of sort. Use "asc" (ascending) or "desc" (descending). [Default is asc] groupFields =
	 *            Groups together fields in an arrangement that is easier for import to another system. Used by the
	 *            ItemInfo web service when in "import" mode. [Default is false]
	 *
	 ...
	 *                ids                        = an array of ids (aka. primary keys); same as setting the 'idsOnly'
	 *                option modelInstances            = an array of instances, one for each match. Each instance is
	 *                the  class of the related item, a subclass of BaseModel firstId                    = the id
	 *                (primary key) of the first match. This is the same as the first item in the array returned by
	 *                'ids' firstModelInstance        = the instance of the first match. This is the same as the first
	 *                instance in the array returned by 'modelInstances' count                    = the number of
	 *                related items
	 *
	 *                    Default is "data" - returns a list of arrays with data about each related item
	 *
	 * @param int   $pn_count                 Variable to return number of related items. The count reflects the
	 *                                        absolute number of related items, independent of how the start and limit
	 *                                        options are set, and may differ from the number of items actually
	 *                                        returned.
	 *
	...
	public function getRelatedItems( $pm_rel_table_name_or_num, $pa_options = null, &$pn_count = null ) {
		...
		$ps_return_as = caGetOption( 'returnAs', $pa_options, 'data', array(
			'forceLowercase' => true,
			'validValues'    => array(
				'data',
				'searchResult',
		...
		// convert options
		if ( ( $pa_options['restrictToTypes'] = caGetOption( array(
				'restrictToTypes',
				'restrict_to_types',
				'restrictToType',
				'restrict_to_type'
			), $pa_options, null ) )
		     && ! is_array( $pa_options['restrictToTypes'] )
		) {
			$pa_options['restrictToTypes'] = preg_split( "![;,]{1}!", $pa_options['restrictToTypes'] );
		}
		if ( ( $pa_options['restrictToRelationshipTypes'] = caGetOption( array(
				'restrictToRelationshipTypes',
				'restrict_to_relationship_types',
				'restrictToRelationshipType',
				'restrict_to_relationship_type'
			), $pa_options, null ) )
		     && ! is_array( $pa_options['restrictToRelationshipTypes'] )
		) {
			$pa_options['restrictToRelationshipTypes'] = preg_split( "![;,]{1}!",
				$pa_options['restrictToRelationshipTypes'] );
		}
		if ( ( $pa_options['excludeTypes'] = caGetOption( array(
				'excludeTypes',
				'exclude_types',
				'excludeType',
				'exclude_type'
			), $pa_options, null ) )
		     && ! is_array( $pa_options['excludeTypes'] )
		) {
			$pa_options['excludeTypes'] = preg_split( "![;,]{1}!", $pa_options['excludeTypes'] );
		}
		if ( ( $pa_options['excludeRelationshipTypes'] = caGetOption( array(
				'excludeRelationshipTypes',
				'exclude_relationship_types',
				'excludeRelationshipType',
				'exclude_relationship_type'
			), $pa_options, null ) )
		     && ! is_array( $pa_options['excludeRelationshipTypes'] )
		) {
			$pa_options['excludeRelationshipTypes'] = preg_split( "![;,]{1}!",
				$pa_options['excludeRelationshipTypes'] );
		}

		if ( ! isset( $pa_options['dontIncludeSubtypesInTypeRestriction'] )
		     && ( isset( $pa_options['dont_include_subtypes_in_type_restriction'] )
		          && $pa_options['dont_include_subtypes_in_type_restriction'] )
		) {
			$pa_options['dontIncludeSubtypesInTypeRestriction']
				= $pa_options['dont_include_subtypes_in_type_restriction'];
		}
		if ( ! isset( $pa_options['returnNonPreferredLabels'] )
		     && ( isset( $pa_options['restrict_to_type'] )
		          && $pa_options['restrict_to_type'] )
		) {
			$pa_options['returnNonPreferredLabels'] = $pa_options['restrict_to_type'];
		}
		if ( ! isset( $pa_options['returnLabelsAsArray'] )
		     && ( isset( $pa_options['return_labels_as_array'] )
		          && $pa_options['return_labels_as_array'] )
		) {
			$pa_options['returnLabelsAsArray'] = $pa_options['return_labels_as_array'];
		}
		if ( ! isset( $pa_options['restrictToLists'] )
		     && ( isset( $pa_options['restrict_to_lists'] )
		          && $pa_options['restrict_to_lists'] )
		) {
			$pa_options['restrictToLists'] = $pa_options['restrict_to_lists'];
		}

		if ( ( $pa_options['restrictToLists'] = caGetOption( array( 'restrictToLists', 'restrict_to_lists' ),
				$pa_options, null ) )
		     && ! is_array( $pa_options['restrictToLists'] )
		) {
		...
		$pa_primary_ids         = ( isset( $pa_options['primaryIDs'] ) && is_array( $pa_options['primaryIDs'] ) )
			? $pa_options['primaryIDs'] : null;
		$pb_show_current_only   = caGetOption( 'showCurrentOnly', $pa_options,
			caGetOption( 'currentOnly', $pa_options, false ) );
		$ps_current_date_bundle = caGetOption( 'showCurrentUsingDate', $pa_options, 'effective_date' );

		if ( ! isset( $pa_options['useLocaleCodes'] )
		     && ( isset( $pa_options['returnLocaleCodes'] )
		          && $pa_options['returnLocaleCodes'] )
		) {
		...
		$pa_get_where = ( isset( $pa_options['where'] ) && is_array( $pa_options['where'] )
		                  && sizeof( $pa_options['where'] ) ) ? $pa_options['where'] : null;

		$pa_row_ids = ( isset( $pa_options['row_ids'] ) && is_array( $pa_options['row_ids'] ) ) ? $pa_options['row_ids']
			: null;
		$pn_row_id  = ( isset( $pa_options['row_id'] ) && $pa_options['row_id'] ) ? $pa_options['row_id']
			: $this->getPrimaryKey();
		...
		$pa_sort_fields    = ( isset( $pa_options['sort'] ) && is_array( $pa_options['sort'] ) )
			? array_filter( $pa_options['sort'], "strlen" ) : null;
		$ps_sort_direction = ( isset( $pa_options['sortDirection'] ) && $pa_options['sortDirection'] )
		...
		$pb_return_labels_as_array = ( isset( $pa_options['returnLabelsAsArray'] )
		                               && $pa_options['returnLabelsAsArray'] ) ? true : false;
		$pn_limit                  = ( isset( $pa_options['limit'] ) && ( (int) $pa_options['limit'] > 0 ) )
			? (int) $pa_options['limit'] : 4000;
		$pn_start                  = ( isset( $pa_options['start'] ) && ( (int) $pa_options['start'] > 0 ) )
			? (int) $pa_options['start'] : 0;
		...
		switch ( sizeof(
			$va_path = array_keys( Datamodel::getPath( $vs_subject_table_name, $vs_related_table_name ) ) ) ) {
			case 3:
				$t_item_rel             = Datamodel::getInstance( $va_path[1] );
			...
			case 2:
				...
				if ( ! ( $t_rel_item = Datamodel::getInstance( $va_path[1] ) ) ) {
					return null;
				}
				$vs_rel_item_table_name = $t_rel_item->tableName();
			...
			default:
				...
				if (
					...
					&& $t_rel_item->hasField( 'table_num' )
					&& $t_rel_item->hasField( 'row_id' )
				) {
					$vs_key                 = $t_rel_item->primaryKey();
					$vs_rel_item_table_name = $t_rel_item->tableName();
		...
		if ( ( $t_tmp = $t_item_rel ) || ( $t_rel_item->isRelationship() && ( $t_tmp = $t_rel_item ) ) ) {
			...
			if ( $vb_uses_relationship_types && isset( $pa_options['restrictToRelationshipTypes'] )
			     && $pa_options['restrictToRelationshipTypes']
			) {
				...
				if ( sizeof( $pa_options['restrictToRelationshipTypes'] ) ) {
					...
					foreach ( $pa_options['restrictToRelationshipTypes'] as $vm_type ) {
						...
						if ( $vn_type_id > 0 ) {
							...
							if ( is_array(
								$va_children = $t_rel->getHierarchyChildren( $vn_type_id, array( 'idsOnly' => true ) ) )
							) {
			...
			if ( $vb_uses_relationship_types && isset( $pa_options['excludeRelationshipTypes'] )
			     && $pa_options['excludeRelationshipTypes']
			) {
		...
		if ( is_array( $va_type_ids ) && ( sizeof( $va_type_ids ) > 0 ) ) {
			$va_wheres[] = "({$vs_related_table}.type_id IN (" . join( ',', $va_type_ids ) . ')'
			               . ( $t_rel_item->getFieldInfo( 'type_id', 'IS_NULL' )
					? " OR ({$vs_related_table}.type_id IS NULL)" : '' ) . ')';
		}

		$va_source_ids = caMergeSourceRestrictionLists( $t_rel_item, $pa_options );
		if ( method_exists( $t_rel_item, "getSourceFieldName" )
		     && ( $vs_source_id_fld
				= $t_rel_item->getSourceFieldName() )
		     && is_array( $va_source_ids )
		     && ( sizeof( $va_source_ids ) > 0 )
		) {
		...
		if ( $this->getAppConfig()->get( 'perform_item_level_access_checking' ) ) {
			...
			$va_joins_post_add[] = 'LEFT JOIN ca_acl ON ' . $vs_related_table_name . '.' . $t_rel_item->primaryKey()
			                       . ' = ca_acl.row_id AND ca_acl.table_num = ' . $t_rel_item->tableNum() . "\n";
			$va_wheres[]         = "(
				((
		...
		if ( method_exists( $t_rel_item, "getLabelTableName" )
		     && ( ! isset( $pa_options['dontReturnLabels'] )
		          || ! $pa_options['dontReturnLabels'] )
		) {
			if ( $vs_label_table_name
				= $t_rel_item->getLabelTableName()
				...
				if ( $t_rel_item_label->hasField( 'is_preferred' )
				     && ( ! isset( $pa_options['returnNonPreferredLabels'] )
				          || ! $pa_options['returnNonPreferredLabels'] )
				) {
		...
		if ( $vb_self_relationship ) {
			...
			foreach ( $va_rel_info[ $va_path[0] ][ $va_path[1] ] as $va_possible_keys ) {
				$va_joins   = array();
				$va_joins[] = "INNER JOIN " . $va_path[1] . " ON " . $va_path[1] . '.' . $va_possible_keys[1] . ' = '
				              . $va_path[0] . '.' . $va_possible_keys[0] . "\n";

				if ( $vs_label_table_name ) {
					$va_joins[] = "INNER JOIN " . $vs_label_table_name . " ON " . $vs_label_table_name . '.'
					              . $va_label_rel_info[ $va_path[0] ][ $vs_label_table_name ][0][1] . ' = '
					              . $va_path[0] . '.' . $va_label_rel_info[ $va_path[0] ][ $vs_label_table_name ][0][0]
					              . "\n";
				}

				$vs_other_field = ( $vn_i == 0 ) ? $va_rel_info[ $va_path[0] ][ $va_path[1] ][1][1]
					: $va_rel_info[ $va_path[0] ][ $va_path[1] ][0][1];
				...
				$vs_sql = "
					...
					WHERE
						" . join( ' AND ', array_merge( $va_wheres, array(
						'(' . $va_path[1] . '.' . $vs_other_field . ' IN (' . join( ',', $pa_row_ids ) . '))'
					) ) ) . "
					{$vs_order_by}";
...
// 					}
					...
					if ( $vb_uses_effective_dates ) {    // return effective dates as display/parse-able text
						if ( $va_rels[ $vs_sort_key ][ $vn_id ]['sdatetime']
						     || $va_rels[ $vs_sort_key ][ $vn_id ]['edatetime']
						) {
							$o_tep->setHistoricTimestamps( $va_rels[ $vs_sort_key ][ $vn_id ]['sdatetime'],
								$va_rels[ $vs_sort_key ][ $vn_id ]['edatetime'] );
					...
					if ( $pb_use_locale_codes ) {
						$va_rels[ $vs_sort_key ][ $vn_id ]['locale_id']
							= $vn_locale_id = $t_locale->localeIDToCode( $vn_locale_id );
					}

					$va_rels[ $vs_sort_key ][ $vn_id ]['labels'][ $vn_locale_id ] = ( $pb_return_labels_as_array )
						? $va_row : $vs_display_label;
					$va_rels[ $vs_sort_key ][ $vn_id ]['_key']                    = $vs_key;
					$va_rels[ $vs_sort_key ][ $vn_id ]['direction']               = $vs_direction;

					$vn_c ++;
					if ( $vb_uses_relationship_types ) {
						$va_rels[ $vs_sort_key ][ $vn_id ]['relationship_typename'] = ( $vs_direction == 'ltor' )
							? $va_rel_types[ $va_row['relationship_type_id'] ]['typename']
							: $va_rel_types[ $va_row['relationship_type_id'] ]['typename_reverse'];
						$va_rels[ $vs_sort_key ][ $vn_id ]['relationship_type_code']
						                                                            = $va_rel_types[ $va_row['relationship_type_id'] ]['type_code'];
					}
					...
					if ( $pb_group_fields ) {
						...
						if ( $t_rel_item_label ) {
							foreach ( $t_rel_item_label->getFormFields() as $vs_field => $va_field_info ) {
								if ( ! isset( $va_rels[ $vs_sort_key ][ $vn_id ][ $vs_field ] )
								     || ( $vs_field == $vs_rel_pk )
								) {
									continue;
								}
								$va_rels[ $vs_sort_key ][ $vn_id ]['preferred_labels'][ $vs_field ]
									= $va_rels[ $vs_sort_key ][ $vn_id ][ $vs_field ];
						...
						foreach ( $t_rel_item->getFormFields() as $vs_field => $va_field_info ) {
							...
							$va_rels[ $vs_sort_key ][ $vn_id ]['intrinsic'][ $vs_field ]
								= $va_rels[ $vs_sort_key ][ $vn_id ][ $vs_field ];
					...
					if ( $pb_show_current_only && $t_item_rel ) {
						...
						while ( $qr_rels->nextHit() ) {
							foreach (
								$qr_rels->get( $ps_current_date_bundle,
									[ 'returnAsArray' => true, 'sortable' => true ] ) as $vs_date
							) {
								$va_tmp = explode( "/", $vs_date );
								if ( $va_tmp[0] > $vn_current_date ) {
									continue;
								}    // skip future dates
								$va_rels_by_date[ $vs_date . '/' . sprintf( "%09d",
									$qr_rels->get( $t_item_rel->tableName()
									               . ".relation_id" ) ) ][ $vs_sort_key ][ $vn_id ]
									= $va_rels[ $vs_sort_key ][ $vn_id ];
							}
						}
					}
				}
			...
			if ( $ps_return_as !== 'data' ) {
				$va_rels = caExtractArrayValuesFromArrayOfArrays( $va_rels,
					( $ps_return_as === 'relationids' ) ? 'relation_id' : $t_rel_item->primaryKey() );
			}
			...
			foreach ( $va_rel_info[ $vs_cur_table ][ $vs_rel_table ] as $vn_i => $va_rel ) {
				$va_joins = array(
					'INNER JOIN ' . $vs_rel_table . ' ON ' . $vs_cur_table . '.' . $va_rel[0] . ' = ' . $vs_rel_table
					. '.' . $va_rel[1] . "\n"
				);
				...
				foreach ( $va_path as $vs_join_table ) {
					...
					$va_joins[]        = 'INNER JOIN ' . $vs_join_table . ' ON ' . $vs_base_table . '.'
					                     . $va_label_rel_info[ $vs_base_table ][ $vs_join_table ][0][0] . ' = '
					                     . $vs_join_table . '.'
					                     . $va_label_rel_info[ $vs_base_table ][ $vs_join_table ][0][1] . "\n";
				...
				while ( $qr_res->nextRow() ) {
					...
					if ( $vb_uses_effective_dates ) {    // return effective dates as display/parse-able text
						if ( $va_rels[ $vs_v ]['sdatetime'] || $va_rels[ $vs_v ]['edatetime'] ) {
							$o_tep->setHistoricTimestamps( $va_rels[ $vs_v ]['sdatetime'],
								$va_rels[ $vs_v ]['edatetime'] );
					...
					$va_rels[ $vs_v ]['labels'][ $vn_locale_id ] = ( $pb_return_labels_as_array ) ? $va_row
						: $vs_display_label;
					...
					$vn_c ++;
					if ( $vb_uses_relationship_types ) {
						$va_rels[ $vs_v ]['relationship_typename'] = ( $vs_direction == 'ltor' )
							? $va_rel_types[ $va_row['relationship_type_id'] ]['typename']
							: $va_rel_types[ $va_row['relationship_type_id'] ]['typename_reverse'];
						$va_rels[ $vs_v ]['relationship_type_code']
						                                           = $va_rel_types[ $va_row['relationship_type_id'] ]['type_code'];
					}
			...
			if ( $vb_is_combo_key_relation ) {
				if ( $vs_item_rel_table_name ) {
					$va_joins   = [
						"INNER JOIN {$vs_item_rel_table_name} ON {$vs_item_rel_table_name}.row_id = "
						. $this->primaryKey( true ) . " AND {$vs_item_rel_table_name}.table_num = " . $this->tableNum()
					];
					$va_joins[] = "INNER JOIN {$vs_related_table_name} ON {$vs_related_table_name}."
					              . $t_rel_item->primaryKey() . " = {$vs_item_rel_table_name}."
					              . $t_rel_item->primaryKey();
				} else {
					$va_joins = [
						"INNER JOIN {$vs_related_table_name} ON {$vs_related_table_name}.row_id = "
						. $this->primaryKey( true ) . " AND {$vs_related_table_name}.table_num = " . $this->tableNum()
					];
				}
				if ( method_exists( $t_rel_item, "getLabelTableInstance" )
				     && ( $t_rel_label
						= $t_rel_item->getLabelTableInstance() )
				) {
					$vs_related_label_table_name = $t_rel_label->tableName();
					$vs_rel_pk                   = $t_rel_item->primaryKey();
					$va_joins[]
					                             = "INNER JOIN {$vs_related_label_table_name} ON {$vs_related_label_table_name}.{$vs_rel_pk} = {$vs_related_table_name}.{$vs_rel_pk}";
				}
			} else {
				foreach ( $va_path as $vs_join_table ) {
					...
					foreach ( $va_rel_info[ $vs_cur_table ][ $vs_join_table ] as $vn_i => $va_rel ) {
						$va_tmp[] = $vs_cur_table . "." . $va_rel_info[ $vs_cur_table ][ $vs_join_table ][ $vn_i ][0]
						            . ' = ' . $vs_join_table . '.'
						            . $va_rel_info[ $vs_cur_table ][ $vs_join_table ][ $vn_i ][1] . "\n";
					}
					...
					if ( is_array( $pa_options['restrictToTypes'] ) && sizeof( $pa_options['restrictToTypes'] ) ) {
						...
						foreach ( $va_rels as $vs_rel_pk => $va_rel_info ) {
							if ( $va_rel_info['one_table'] != $this->tableName() ) {
								$va_type_ids = caMakeTypeIDList( $va_rel_info['one_table'],
									$pa_options['restrictToTypes'] );

								if ( is_array( $va_type_ids ) && sizeof( $va_type_ids ) ) {
									$va_joins[]
										         = "INNER JOIN {$va_rel_info['one_table']} AS r ON r.{$va_rel_info['one_table_field']} = "
										           . $t_rel_item->tableName() . ".{$vs_rel_pk}";
			...
			while ( $qr_res->nextRow() ) {
				...
				$va_row = $qr_res->getRow();
				$vs_v   = ( sizeof( $va_path ) <= 2 ) ? $va_row['row_id'] . '/' . $va_row[ $vs_key ]
					: $va_row[ $vs_key ];
				...
				if ( $vb_uses_effective_dates ) {    // return effective dates as display/parse-able text
					if ( $va_rels_for_id[ $vs_v ]['sdatetime'] || $va_rels_for_id[ $vs_v ]['edatetime'] ) {
						$o_tep->setHistoricTimestamps( $va_rels_for_id[ $vs_v ]['sdatetime'],
							$va_rels_for_id[ $vs_v ]['edatetime'] );
				...
				$va_rels_for_id[ $vs_v ]['labels'][ $vn_locale_id ] = ( $pb_return_labels_as_array ) ? $va_row
					: $vs_display_label;
				...
				$vn_c ++;
				if ( $vb_uses_relationship_types ) {
					$va_rels_for_id[ $vs_v ]['relationship_typename'] = ( $vs_direction == 'ltor' )
						? $va_rel_types[ $va_row['relationship_type_id'] ]['typename']
						: $va_rel_types[ $va_row['relationship_type_id'] ]['typename_reverse'];
					$va_rels_for_id[ $vs_v ]['relationship_type_code']
					                                                  = $va_rel_types[ $va_row['relationship_type_id'] ]['type_code'];
				}
				...
				if ( $pb_group_fields ) {
					...
					if ( $t_rel_item_label ) {
						foreach ( $t_rel_item_label->getFormFields() as $vs_field => $va_field_info ) {
							...
							$va_rels_for_id[ $vs_v ]['preferred_labels'][ $vs_field ]
								= $va_rels_for_id[ $vs_v ][ $vs_field ];
				...
				if ( $pb_show_current_only && $t_item_rel ) {
					if ( $this->isRelationship() ) {
						$k  = $this->tableName() . "." . ( ( $this->getLeftTableFieldName() == $vs_key )
								? $this->getRightTableFieldName() : $this->getLeftTableFieldName() );
					...
					if ( $cd == 'effective_date' ) {
						$cd = $t_item_rel->tableName() . ".{$cd}";
					}

					$qr_rels = caMakeSearchResult( $t, [ $id ] );
					while ( $qr_rels->nextHit() ) {
						foreach (
							$d = $qr_rels->get( $cd, [ 'returnAsArray' => true, 'sortable' => true ] ) as $vs_date
						) {
							$va_tmp = explode( "/", $vs_date );
							if ( $va_tmp[0] > $vn_current_date ) {
								continue;
							}    // skip future dates
							$va_rels_for_id_by_date[ $qr_rels->get( $k ) ][ $vs_date . '/' . sprintf( "%09d",
								$qr_rels->get( $t_item_rel->tableName() . ".relation_id" ) ) ][ $vs_v ]
								= $va_rels_for_id[ $vs_v ];
						}
					}
				}
			...
			foreach ( $va_rels as $vn_pk => $va_related_item ) {
				foreach ( $va_filters as $vs_filter => $va_filter_vals ) {
					...
					foreach ( $va_filter_vals as $vn_index => $vs_filter_val ) {
						...
						if ( ! is_numeric( $vs_filter_val )
						     && ( ( $t_element
									= ca_metadata_elements::getInstance( $vs_element ) )
						          && ( $t_element->get( 'datatype' ) == 3 ) )
						) {
							$va_filter_vals[ $vn_index ] = caGetListItemID( $t_element->get( 'list_id' ),
								$vs_filter_val );
						}
					}

					$t_rel_item->load( $va_related_item[ $t_rel_item->primaryKey() ] );
					$va_filter_values = $t_rel_item->get( $vs_filter,
						array( 'returnAsArray' => true, 'alwaysReturnItemID' => true ) );
		...
		if ( is_array( $pa_sort_fields ) && sizeof( $pa_sort_fields ) && sizeof( $va_rels ) ) {
			...
			if ( sizeof( $va_ids ) > 0 ) {
				$qr_sort = caMakeSearchResult( $vs_related_table_name, array_values( $va_ids ),
					array( 'sort' => $pa_sort_fields, 'sortDirection' => $ps_sort_direction ) );
	...
	public function getTypeMenu() {
		...
		if ( is_array( $va_hierarchy ) ) {
			...
			foreach ( $va_hierarchy as $vn_item_id => $va_item ) {
				...
				if ( isset( $va_types_by_parent_id[ $va_item['item_id'] ] )
				     && is_array( $va_types_by_parent_id[ $va_item['item_id'] ] )
				) {
					$va_subtypes = $this->_getSubTypes( $va_types_by_parent_id[ $va_item['item_id'] ],
	 ...
	 * plugin to generate HTML for idno. If no plugin is specified then the call is passed on to
	 * BaseModel::htmlFormElement() Calls for fields other than idno are passed to BaseModel::htmlFormElement()
	 */
	public function htmlFormElement( $ps_field, $ps_format = null, $pa_options = null ) {
		...
		if (
			...
			if ( ( $this->tableName() == 'ca_objects' )
			     && $this->getAppConfig()->get( 'ca_objects_x_collections_hierarchy_enabled' )
			     && ! $this->getAppConfig()
			               ->get( 'ca_objects_x_collections_hierarchy_disable_object_collection_idno_inheritance' )
			     && $pa_options['request']
			     && ( $vn_collection_id = $pa_options['request']->getParameter( 'collection_id', pInteger ) )
			) {
				require_once( __CA_MODELS_DIR__ . "/ca_collections.php" );
				...
				if ( $t_coll->getPrimaryKey() ) {
					...
					if ( ! $this->opo_idno_plugin_instance->formatHas( 'PARENT' )
					     && ! $this->opo_idno_plugin_instance->getFormatProperty( 'dont_inherit_from_parent_collection' )
					) {
						$this->set( $vs_idno_fld, $t_coll->get( 'idno' ) );
					}
				}
			} elseif ( $vn_parent_id = $this->get(
				$vs_parent_id_fld = $this->getProperty( 'HIERARCHY_PARENT_ID_FLD' ) )
			) {
				// Parent will be set
				...
				if ( ! $this->opo_idno_plugin_instance->getFormatProperty( 'dont_inherit_from_parent' ) ) {
					if ( $t_parent->load( $vn_parent_id ) ) {
						$this->opo_idno_plugin_instance->isChild( true,
							$t_parent->get( $this->tableName() . ".{$vs_idno_fld}" ) );
						if ( ! $this->getPrimaryKey() && ! $this->opo_idno_plugin_instance->formatHas( 'PARENT' ) ) {
							$this->set( $vs_idno_fld,
								( $t_parent->get( $vs_idno_fld ) )
									?
									$this->opo_idno_plugin_instance->makeTemplateFromValue( $t_parent->get( $vs_idno_fld ),
										1, true )
									:
									''
							);
...
// TODO: should be in config file
				...
				if (
					...
					if ( preg_match( "/\^DESCRIPTION/", $ps_formatted_element ) ) {
						$ps_formatted_element = str_replace( "^LABEL",
							isset( $pa_options['label'] ) ? $pa_options['label'] : $va_attr["LABEL"],
							$ps_formatted_element );
						$ps_formatted_element = str_replace( "^DESCRIPTION", $va_attr["DESCRIPTION"],
							$ps_formatted_element );
					} else {
						...
						$ps_formatted_element = str_replace( "^LABEL",
							'<span id="' . $vs_field_id . '">' . ( isset( $pa_options['label'] ) ? $pa_options['label']
								: $va_attr["LABEL"] ) . '</span>', $ps_formatted_element );

						if ( ! $pa_options['no_tooltips'] ) {
							TooltipManager::add( '#' . $vs_field_id,
								"<h3>" . ( isset( $pa_options['label'] ) ? $pa_options['label'] : $va_attr["LABEL"] )
								. "</h3>" . $va_attr["DESCRIPTION"] );
						}
					}
				} else {
					$ps_formatted_element = str_replace( "^LABEL",
	...
	public function loadIDNoPlugInInstance( $pa_options = null ) {
		...
		return $this->opo_idno_plugin_instance = IDNumbering::newIDNumberer( $this->tableName(), $this->getTypeCode(),
			null, $this->getDb(), caGetOption( 'IDNumberingConfig', $pa_options, null ) );
	}
	# ----------------------------------------
	...
	public function validateAdminIDNo( $ps_admin_idno ) {
		$va_errors = array();
		if ( $this->_CONFIG->get( 'require_valid_id_number_for_' . $this->tableName() )
		     && sizeof( $va_admin_idno_errors = $this->opo_idno_plugin_instance->isValidValue( $ps_admin_idno ) )
		) {
			$va_errors[] = join( '; ', $va_admin_idno_errors );
		} else {
			if ( ! $this->_CONFIG->get( 'allow_duplicate_id_number_for_' . $this->tableName() )
			     && sizeof( $this->checkForDupeAdminIdnos( $ps_admin_idno ) )
			) {
	...
	private function _getSubTypes( $pa_subtypes, $pa_types_by_parent_id ) {
		...
		foreach ( $pa_subtypes as $vn_i => $va_type ) {
			if ( is_array( $pa_types_by_parent_id[ $va_type['item_id'] ] ) ) {
				$va_subsubtypes = $this->_getSubTypes( $pa_types_by_parent_id[ $va_type['item_id'] ],
					$pa_types_by_parent_id );
			} else {
	...
	/**
	 * Creates a search result instance for the specified table containing the specified keys as if they had been
	 * returned by a search or browse. This method is useful when you need to efficiently retrieve data from an
	 * arbitrary set of records since you get all of the lazy loading functionality of a standard search result without
	 * having to actually perform a search.
	 *
	 * @param mixed $pm_rel_table_name_or_num The name or table number of the table for which to create the result set.
	 *                                        Must be a searchable/browsable table
	 * @param array $pa_ids                   List of primary key values to create result set for. Result set will
	 *                                        contain specified keys in the order in which that are passed in the
	 *                                        array.
	 * @param array $pa_options               Optional array of options to pass through to SearchResult::init(). If you
	 *                                        need the search result to connect to the database with a specific
	 *                                        database transaction you should pass the Db instance used by the
	 *                                        transaction here in the 'db' option. Other options include: sort = field
	 *                                        or attribute to sort on in <table name>.<field or attribute name> format
	 *                                        (eg. ca_objects.idno); default is to sort on relevance (aka.
	 *                                        sort='_natural') sortDirection = direction to sort results by, either
	 *                                        'asc' for ascending order or 'desc' for descending order; default is
	 *                                        'asc' instance = An instance of the model for $pm_rel_table_name_or_num;
	 *                                        if passed makeSearchResult can use it to directly extract model
	 *                                        information potentially saving time [Default is null]
	 *
	...
	public function makeSearchResult( $pm_rel_table_name_or_num, $pa_ids, $pa_options = null ) {
		...
		if ( $pa_sort = caGetOption( 'sort', $pa_options, null ) ) {
			...
			$va_ids  = $vo_sort->sortHits( $va_ids, $t_instance->tableName(), join( ';', $pa_sort ),
				caGetOption( 'sortDirection', $pa_options, 'asc' ), $pa_options );
		}
		if ( ! ( $vs_search_result_class = $t_instance->getProperty( 'SEARCH_RESULT_CLASSNAME' ) ) ) {
			return null;
		}
		if ( ! class_exists( $vs_search_result_class ) ) {
		...
		$o_res
			= new $vs_search_result_class( $t_instance->tableName() );    // we pass the table name here so generic multi-table search classes such as InterstitialSearch know what table they're operating over
	 ...
	 *
	 * @param array $pa_ids List of primary key values to create result set for. Result set will contain specified keys
	 *                      in the order in which that are passed in the array.
	 *
	 * @return SearchResult A search result of for the specified table
	 */
	 ...
	 *                          returnAsInitialValuesForBundle = if set array is returned suitable for use with the
	 *                          ACLUsers bundle; the array is key'ed by acl_id and includes the _display entry required
	 *                          by the bundle
	 *
	...
	public function getACLUsers( $pa_options = null ) {
		...
		$vb_return_for_bundle = ( isset( $pa_options['returnAsInitialValuesForBundle'] )
		                          && $pa_options['returnAsInitialValuesForBundle'] ) ? true : false;
		...
		if ( $qr_users = $this->makeSearchResult( 'ca_users', $va_user_ids ) ) {
			$va_initial_values = caProcessRelationshipLookupLabel( $qr_users, new ca_users(),
				array( 'stripTags' => true ) );
		} else {
	...
	public function getACLGroupHTMLFormBundle( $po_request, $ps_form_name, $pa_options = null ) {
		...
		$o_view->setVar( 'initialValues',
			$this->getACLUserGroups( array( 'returnAsInitialValuesForBundle' => true ) ) );
	 ...
	 * is key'ed on user group group_id; each value is an  array containing information about the group. Array keys
	 * are:
	...
	public function getACLUserGroups( $pa_options = null ) {
		...
		if ( ( $qr_groups = $this->makeSearchResult( 'ca_user_groups', $va_group_ids ) ) ) {
			$va_initial_values = caProcessRelationshipLookupLabel( $qr_groups, new ca_user_groups(),
				array( 'stripTags' => true ) );
		} else {
			$va_initial_values = array();
		}
	...
	public function getACLWorldAccess( $pa_options = null ) {
		...
		if ( ! strlen( $va_row['access_display'] ) ) {    // show default
			$va_row['access_display'] = $t_acl->getChoiceListValue( 'access',
				$this->getAppConfig()->get( 'default_item_access_level' ) );
		}
	...
	/**
	 * Checks access control list for currently loaded row for the specified user and returns an access value. Values
	 * are:
	 *
	 ...
	 *
	 * @param string $ps_template_value Template to use. If omitted or left blank the template defaults to a single "%"
	 *                                  This is useful for the common case of assigning simple auto-incrementing
	 *                                  integers as idno values.
	 * @param array  $pa_options        Options are:
	 *                                  dontSetValue = The template will be processed and the idno value generated but
	 *                                  not actually set for the current row. Default is false. serialOnly = Only set
	 *                                  idno if the idno is configured to be SERIAL (auto-incrementing numbers).
	 *                                  [Default is true]
	 *
	 ...
	 *                                         puts the current row on the left
	 * @param null   $pn_rank
	 *                                         side. For many self-relations the direction determines the nature and
	...
	public function addRelationship(
		...
		if ( is_null( $ps_effective_date ) && is_array( $pa_options )
		     && is_array( $pa_options['interstitialValues'] )
		) {
		...
		if ( $t_rel = parent::addRelationship( $pm_rel_table_name_or_num, $pn_rel_id, $pm_type_id, $ps_effective_date,
			$ps_source_info, $ps_direction, $pn_rank, $pa_options )
		) {
			if ( $t_rel->numErrors() ) {
	 ...
	 *
	 * @param mixed    $pm_rel_table_name_or_num Table name (eg. "ca_entities") or number as defined in datamodel.conf
	 *                                           of table containing row to create relationships to.
	 * @param int      $pn_relation_id           primary key value of the relation to edit.
	 * @param int      $pn_rel_id                primary key value of row to creation relationship to.
	 * @param mixed    $pm_type_id               Relationship type type_code or type_id, as defined in the
	 *                                           ca_relationship_types table. This is required for all relationships
	 *                                           that use relationship types. This includes all of the most common
	 *                                           types of relationships.
	 * @param string   $ps_effective_date        Optional date expression to qualify relation with. Any expression that
	 *                                           the TimeExpressionParser can handle is supported here.
	 * @param mixed    $pa_source_info           Array or text for storing information about source of relationship.
	 *                                           Not currently used.
	 * @param string   $ps_direction             Optional direction specification for self-relationships (relationships
	 *                                           linking two rows in the same table). Valid values are 'ltor'
	 *                                           (left-to-right) and  'rtol' (right-to-left); the direction determines
	 *                                           which "side" of the relationship the currently loaded row is on:
	 *                                           'ltor' puts the current row on the left side. For many self-relations
	 *                                           the direction determines the nature and display text for the
	 *                                           relationship.
	 * @param null|int $pn_rank
	 * @param array    $pa_options               Array of additional options:
	 *                                           allowDuplicates = if set to true, attempts to edit a relationship to
	 *                                           match one that already exists will succeed. Default is false –
	 *                                           duplicate relationships will not be created. setErrorOnDuplicate = if
	 *                                           set to true, an error will be set if an attempt is made to create a
	 *                                           duplicate relationship. Default is false – don't set error.
	 *                                           editRelationship() will always return false when editing of a
	 *                                           relationship fails, no matter how the setErrorOnDuplicate option is
	 *                                           set.
	 *
	...
	public function editRelationship(
		...
		if ( $t_rel = parent::editRelationship( $pm_rel_table_name_or_num, $pn_relation_id, $pn_rel_id, $pm_type_id,
			$ps_effective_date, $pa_source_info, $ps_direction, $pn_rank, $pa_options )
		) {
			if ( $t_rel->numErrors() ) {
				$this->errors = $t_rel->errors;
	 ...
	 *
	 * For an ad-hoc hierarchy like that of an collection, there is only ever one hierarchy to display - that of the
	 * current collection. So for adhoc hierarchies we just return a single entry corresponding to the root of the
	...
	public function getHierarchyList( $pb_dummy = false ) {
		...
		if ( ! $vn_id ) {
			...
			if ( is_array( $va_type_ids ) && sizeof( $va_type_ids ) ) {
				...
				$va_wheres[] = "(o.{$vs_type_fld} IN (?)" . ( $this->getFieldInfo( $vs_type_fld, 'IS_NULL' )
						? " OR o.{$vs_type_fld} IS NULL" : "" ) . ")";
			}
			...
			foreach ( $pa_options['interstitialValues'] as $vs_element => $va_value ) {
				...
				foreach ( $va_value as $va_value_instance ) {
					...
					if ( ! isset( $va_value_instance['locale_id'] ) ) {
						$va_value_instance['locale_id'] = $g_ui_locale_id ? $g_ui_locale_id
	...
	public function getMetadataDictionaryRuleViolations( $ps_bundle_name = null, $options = null ) {
		...
		if ( ( $screen_id = caGetOption( 'screen_id', $options, null ) )
		     || ( $placement_id
				= caGetOption( 'placement_id', $options, null ) )
		) {
			$t_screen = Datamodel::getInstance( 'ca_editor_ui_screens', true );
			if ( is_array(
				$screen_placements = $t_screen->getPlacements( $placement_id ? [ 'placement_id' => $placement_id ]
					: [ 'screen_id' => $screen_id ] ) )
			) {
		...
		if ( $ps_bundle_name = str_replace( "ca_attribute_", "", $ps_bundle_name ) ) {
			if ( ! Datamodel::tableExists( $ps_bundle_name )
			     && ( ! preg_match( '!^' . $this->tableName() . '.!', $ps_bundle_name ) )
			) {
		...
		while ( $qr_res->nextRow() ) {
			...
			$t_rule = ( isset( $va_rule_instances[ $vn_rule_id ] ) ) ? $va_rule_instances[ $vn_rule_id ]
				: ( $va_rule_instances[ $vn_rule_id ] = new ca_metadata_dictionary_rules( $vn_rule_id ) );
			...
			if ( is_array( $bundles_on_screen ) ) {
				if ( ! in_array( join( ".", $bundle_elements ), $bundles_on_screen ) ) {
					...
					if ( is_array( $bundles_on_screen )
					     && ! in_array( join( ".", $bundle_elements ), $bundles_on_screen )
					) {
						continue;
					}
				}
				...
				$rule_settings_restrict_to_types = array_filter( caGetOption( [
					'restrict_to_types',
					'restrictToTypes'
				], $rule_settings, [] ), function ( $v ) {
					return strlen( $v );
				} );
				$rule_settings_restrict_to_relationship_types
				                                 = array_filter( caGetOption( [
					'restrict_to_relationship_types',
					'restrictToRelationshipTypes'
				], $rule_settings, [] ), function ( $v ) {
					return strlen( $v );
				...
				foreach ( $bundles_on_screen as $placement_id => $placement_bundle_name ) {
					if ( $placement_bundle_name == $p ) {
						...
						$placement_restrict_to_types = array_filter( caGetOption( [
							'restrict_to_types',
							'restrictToTypes'
						], $screen_placements[ $placement_id ]['settings'], [] ), function ( $v ) {
							return strlen( $v );
						} );
						$placement_restrict_to_relationship_types
						                             = array_filter( caGetOption( [
							'restrict_to_relationship_types',
							'restrictToRelationshipTypes'
						], $screen_placements[ $placement_id ]['settings'], [] ), function ( $v ) {
						...
						if ( sizeof( $placement_restrict_to_types )
						     && ! sizeof( array_intersect( $placement_restrict_to_types,
								$rule_settings_restrict_to_types ) )
						) {
							continue;
						}

						if ( sizeof( $placement_restrict_to_relationship_types )
						     && ! sizeof( array_intersect( $placement_restrict_to_relationship_types,
								$rule_settings_restrict_to_relationship_types ) )
						) {
							continue;
						}
	...
	public function validateUsingMetadataDictionaryRules( $pa_options = null ) {
		...
		$va_rules           = ca_metadata_dictionary_rules::getRules( array(
			'db'      => $o_db,
			'table'   => $this->tableNum(),
			'bundles' => caGetOption( 'bundles', $pa_options, null )
		) );
		...
		foreach ( $va_rules as $va_rule ) {
			...
			if ( $t_found = ca_metadata_dictionary_rule_violations::find( array(
				'rule_id'   => $va_rule['rule_id'],
				'row_id'    => $this->getPrimaryKey(),
				'table_num' => $this->tableNum()
			), array( 'returnAs' => 'firstModelInstance', 'transaction' => $this->getTransaction() ) )
			) {
				$t_violation = $t_found;
			}
	 ...
	 *
	 * This method implementation is just a stub and always returns null. Models implementing custom rendering will
	 ...
	 */
	public function getItemTagHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pa_options = null, $pa_bundle_settings = null
	) {
		...
		$o_view->setVar( 'lookup_urls',
			caJSONLookupServiceUrl( $po_request, Datamodel::getTableName( $this->get( 'table_num' ) ) ) );
	 ...
	 * @return string Rendered HTML bundle for display
	 */
	public function getItemCommentHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pa_options = null, $pa_bundle_settings = null
	) {
		$o_view = new View( $po_request, $po_request->getViewsDirectoryPath() . '/bundles/' );
		...
		$initial_values = [];
		foreach (
		( $this->getPrimaryKey() ? $this->getComments( null, null,
			[ 'request' => $po_request, 'sortDirection' => $pa_bundle_settings['sortDirection'] ] ) : [] ) as $v
		) {
