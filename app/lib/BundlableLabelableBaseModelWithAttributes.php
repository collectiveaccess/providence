...
class BundlableLabelableBaseModelWithAttributes extends LabelableBaseModelWithAttributes implements IBundleProvider
{
     ...
     * ca_locales model
     */
    static public $s_locales = null;

    /**
     * TimeExpressionParser
     */
    static public $s_tep = null;
    ...
    public function load($pm_id = null, $pb_use_cache = true)
    {
        ...
        if ($this->isHierarchical() && $this->opo_idno_plugin_instance) {
            $this->opo_idno_plugin_instance->isChild(
                (($vs_parent_id_fld = $this->getProperty('HIERARCHY_PARENT_ID_FLD')) && $this->get(
                        $vs_parent_id_fld
                    ) > 0) ? true : false
            );
        }
    ...
    public function insert($pa_options = null)
    {
        if (!is_a($this, "BaseRelationshipModel")) {
            ...
            $this->opo_app_plugin_manager->hookBeforeBundleInsert(
                array(
                    'id' => null,
            ...
            if ($this->getTypeFieldName() && !(!$vn_type_id && $va_field_info['IS_NULL'])) {
                if (!($vn_ret = $t_list->itemIsEnabled($this->getTypeListCode(), $vn_type_id))) {
                    $va_type_list = $this->getTypeList(
                        array('directChildrenOnly' => false, 'returnHierarchyLevels' => true, 'item_id' => null)
                    );
                    if (!isset($va_type_list[$vn_type_id])) {
                        $this->postError(
                            2510,
                            _t("Type must be specified"),
                        ...
                        if (is_null($vn_ret)) {
                            ...
                            $this->postError(
                                2510,
                                _t(
                                    "<em>%1</em> is not enabled",
                                    $va_type_list[$vn_type_id]['name_singular']
                                ),
                                "BundlableLabelableBaseModelWithAttributes->insert()",
                                $this->tableName() . '.' . $this->getTypeFieldName()
                            );
                ...
                if ($this->HIERARCHY_PARENT_ID_FLD && (bool)$this->getAppConfig()->get(
                        $this->tableName() . '_enforce_strict_type_hierarchy'
                    )) {
                    ...
                    if ($vn_parent_id = $this->get($this->HIERARCHY_PARENT_ID_FLD)) {
                        ...
                        if ($t_parent->load($vn_parent_id)) {
                            ...
                            $va_type_list = $t_parent->getTypeList(
                                array(
                                    'directChildrenOnly' => ($this->getAppConfig()->get(
                                            $this->tableName() . '_enforce_strict_type_hierarchy'
                                        ) === '~') ? false : true,
                            ...
                            if (!isset($va_type_list[$this->getTypeID()])) {
                                ...
                                $this->postError(
                                    2510,
                                    _t(
                                        "<em>%1</em> is not a valid type for a child record of type <em>%2</em>",
                            ...
                            $this->postError(
                                2510,
                                _t("No parent was found when verifying type of new child"),
                                "BundlableLabelableBaseModelWithAttributes->insert()",
                                $this->tableName() . '.' . $this->getTypeFieldName()
                            );
                            $vb_error = true;
                        ...
                        if (!isset($va_type_list[$this->getTypeID()]) && ($t = $this->getTypeInstance())) {
                            ...
                            if (sizeof(
                                ...
                                $this->postError(
                                    2510,
                                    _t(
                                        "<em>%1</em> is not a valid type for a top-level record",
                                        $va_type_list[$this->getTypeID()]['name_singular']
                                    ),
                                    "BundlableLabelableBaseModelWithAttributes->insert()",
                                    $this->tableName() . '.' . $this->getTypeFieldName()
            ...
            if ((bool)$this->getAppConfig()->get($this->tableName() . '_allow_access_inheritance') && $this->hasField(
        ...
        $this->opo_app_plugin_manager->hookAfterBundleInsert(
            array(
    ...
    public function update($pa_options = null)
    {
        ...
        if ($this->getAppConfig()->get('perform_item_level_access_checking')) {
            if ($this->checkACLAccessForUser(new ca_users($AUTH_CURRENT_USER_ID)) < __CA_ACL_EDIT_ACCESS__) {
                $this->postError(
                    2580,
                    _t(
                        "You do not have edit access for this item: %1/%2",
                        $this->tableName(),
                        $this->getPrimaryKey()
                    ),
                    "BundlableLabelableBaseModelWithAttributes->update()"
                );
                return false;
            }
        }
        ...
        $this->opo_app_plugin_manager->hookBeforeBundleUpdate(
            array(
                'id' => $this->getPrimaryKey(),
                'table_num' => $this->tableNum(),
                'table_name' => $this->tableName(),
        ...
        $this->opo_app_plugin_manager->hookAfterBundleUpdate(
            array(
                'id' => $this->getPrimaryKey(),
                'table_num' => $this->tableNum(),
                'table_name' => $this->tableName(),
    ...
    public function delete($pb_delete_related = false, $pa_options = null, $pa_fields = null, $pa_table_list = null)
    {
        ...
        if ($this->getAppConfig()->get('perform_item_level_access_checking')) {
            if ($this->checkACLAccessForUser(new ca_users($AUTH_CURRENT_USER_ID)) < __CA_ACL_EDIT_DELETE_ACCESS__) {
                $this->postError(
                    2580,
                    _t("You do not have delete access for this item"),
    ...
    public function duplicate($pa_options = null)
    {
        ...
        $va_duplicate_relationships = (isset($pa_options['duplicate_relationships']) && is_array(
                $pa_options['duplicate_relationships']
            ) && sizeof($pa_options['duplicate_relationships'])) ? $pa_options['duplicate_relationships'] : array();
        ...
        $va_duplicate_element_settings = (isset($pa_options['duplicate_element_settings']) && is_array(
                $pa_options['duplicate_element_settings']
            ) && sizeof(
                $pa_options['duplicate_element_settings']
        ...
        if ($this->isHierarchical()) {
            if (!$this->get($vs_parent_id_fld)) {
                if ($this->getHierarchyType(
                    ) == __CA_HIER_TYPE_ADHOC_MONO__) {    // If we're duping the root of an adhoc hierarchy then we need to set the HIERARCHY_ID_FLD to null
                    ...
                    $this->postError(
                        2055,
                        _t("Cannot duplicate root of hierarchy"),
        ...
        if ($vs_idno_fld && (!isset($va_duplicate_element_settings[$vs_idno_fld]) || (isset($va_duplicate_element_settings[$vs_idno_fld]) && ($va_duplicate_element_settings[$vs_idno_fld] == 1)))) {
            ...
            if (method_exists($this, "getIDNoPlugInInstance") && ($o_numbering_plugin = $this->getIDNoPlugInInstance(
                ...
                $vs_idno_template = $o_numbering_plugin->makeTemplateFromValue(
                    $this->get($vs_idno_fld),
                    1
                );    // make template out of original idno by replacing last SERIAL element with "%"
                if (!preg_match("!%$!", $vs_idno_template)) {
                    $vb_needs_suffix_generated = true;
                }

                if (!is_array(
                    $va_idno_values = $o_numbering_plugin->htmlFormValuesAsArray(
                        $vs_idno_fld,
                        $vs_idno_template,
                        false,
                        false,
            ...
            if ($vs_idno_stub) {
                if ($vb_needs_suffix_generated) {
                    ...
                    do {
                        ...
                        $vs_idno = trim($vs_idno_stub) . $vs_sep . trim(
                                $vs_suffix
                            );    // force separator if none defined otherwise you end up with agglutinative numbers
        ...
        foreach ($va_labels as $vn_label_id => $va_labels_by_locale) {
            foreach ($va_labels_by_locale as $vn_locale_id => $va_label_list) {
                foreach ($va_label_list as $vn_i => $va_label_info) {
                    ...
                    if (!$vb_duplicate_nonpreferred_labels && key_exists(
                            'is_preferred',
                            $va_label_info
        ...
        $history_tracking_policies = method_exists(
            $table,
            "getHistoryTrackingCurrentValuePoliciesForTable"
        ...
        foreach (
            ...
            if ($this->copyRelationships(
                    $vs_rel_table,
                    $t_dupe->getPrimaryKey(),
                    array(
        ...
        if ($t_dupe->getProperty('RANK') && $this->isHierarchical() && ($vn_parent_id = $this->get(
                    $vs_parent_id_fld
                ) > 0)) {
            $t_dupe->setRankAfter($this->getPrimaryKey());
        }

        if ($vb_duplicate_children && $this->isHierarchical() && ($child_ids = $this->getHierarchyChildren(
                null,
                ['idsOnly' => true]
            ))) {
    ...
    public function set($pa_fields, $pm_value = "", $pa_options = null)
    {
        ...
        if ($this->getPrimaryKey() && !$this->isRelationship() && isset(
                $pa_fields[$this->getTypeFieldName()]
            ) && !(isset($pa_options['allowSettingOfTypeID']) && $pa_options['allowSettingOfTypeID'])) {
            $this->postError(
                2520,
                _t("Type id cannot be set after insert"),
        ...
        if ($this->opo_idno_plugin_instance) {
            ...
            $this->opo_idno_plugin_instance->isChild(
                ((($vs_parent_id_fld = $this->getProperty(
                            'HIERARCHY_PARENT_ID_FLD'
                        )) && isset($pa_fields[$vs_parent_id_fld]) && ($pa_fields[$vs_parent_id_fld] > 0)) || ($this->get(
                        $vs_parent_id_fld
                    ))) ? true : false
            );
            ...
            if (in_array($this->getProperty('ID_NUMBERING_ID_FIELD'), $pa_fields)) {
                if (!$this->_validateIncomingAdminIDNo(true, true)) {
                    if (!$this->get(
                            $vs_parent_id_fld
                        ) && isset($pa_fields[$vs_parent_id_fld]) && ($pa_fields[$vs_parent_id_fld] > 0)) {
    ...
    public function get($ps_field, $pa_options = null)
    {
        ...
        if ($this->_rowAsSearchResult) {
            if (method_exists($this->_rowAsSearchResult, "filterNonPrimaryRepresentations")) {
                $this->_rowAsSearchResult->filterNonPrimaryRepresentations(
                    caGetOption('filterNonPrimaryRepresentations', $pa_options, true)
                );
            }
    ...
    private function _validateIncomingAdminIDNo(
        $pb_post_errors = true,
        $pb_dont_validate_idnos_for_parents_in_mono_hierarchies = false
    ) {
        // we should not bother to validate
        $vn_hier_type = $this->getHierarchyType();
        if ($pb_dont_validate_idnos_for_parents_in_mono_hierarchies && in_array(
                $vn_hier_type,
                array(
                    __CA_HIER_TYPE_SIMPLE_MONO__,
                    __CA_HIER_TYPE_MULTI_MONO__
                )
    ...
    public function hasBundle($ps_bundle, $pn_type_id = null)
    {
        ...
        if (($va_bundle_bits[0] != $this->tableName()) && ($t_rel = Datamodel::getInstanceByTableName(
                $va_bundle_bits[0],
                true
            ))) {
    ...
    public function getValuesForExport($pa_options = null)
    {
        ...
        foreach (
            ...
            $va_related_items = $this->getRelatedItems(
                $vs_table,
        ...
        if ($vs_idno_field = $this->getProperty('ID_NUMBERING_ID_FIELD')) {
            ...
            if ($vs_idno_context_field = $this->getProperty('ID_NUMBERING_CONTEXT_FIELD')) {
                if ($vn_context_id = $this->get($vs_idno_context_field)) {
                    $vs_idno_context_sql = ' AND (' . $vs_idno_context_field . ' = ' . $this->quote(
                            $vs_idno_context_field,
                            $vn_context_id
                        ) . ')';
                } else {
    ...
    {
        if (($vs_idno_field = $this->getProperty('ID_NUMBERING_ID_FIELD')) && ($vs_idno_sort_field = $this->getProperty(
                'ID_NUMBERING_SORT_FIELD'
            ))) {
            if (($o_idno = $this->getIDNoPlugInInstance()) && (method_exists(
                    $o_idno,
                    'getSortableValue'
    ...
    public function getBundleFormValues($ps_bundle_name, $ps_placement_code, $pa_bundle_settings, $pa_options = null)
    {
        ...
        if ($pa_options['request']->user->getBundleAccessLevel(
                $this->tableName(),
                $ps_bundle_name
        ...
        if ((bool)$this->getAppConfig()->get('perform_source_access_checking')) {
            $vn_source_access = $pa_options['request']->user->getSourceAccessLevel(
                $this->tableName(),
                $this->getSourceID()
            );
            ...
            case 'related_table':
                switch ($ps_bundle_name) {
                    ...
                    case 'ca_object_representations':
                        return parent::getRelatedBundleFormValues(
                            $pa_options['request'],
                            $pa_options['formName'],
                            $ps_bundle_name,
                            $ps_placement_code,
                    ...
                    case 'ca_object_lots_related_list':
                        return $this->getRelatedBundleFormValues(
                            $pa_options['request'],
                            $pa_options['formName'],
                            $ps_bundle_name,
                            $ps_placement_code,
            ...
            case 'special':
                switch ($ps_bundle_name) {
                    case 'ca_site_page_media':
                        return parent::getBundleFormValues(
                            $ps_bundle_name,
                            $ps_placement_code,
                            $pa_bundle_settings,
                            $pa_options
    ...
    public function getBundleFormHTML(
        ...
        $vb_read_only_because_deaccessioned = ($this->hasField('is_deaccessioned') && (bool)$this->getAppConfig()->get(
                'deaccession_dont_allow_editing'
        ...
        if (
            ($va_dictionary_entry = ca_metadata_dictionary_entries::getEntry(
                $dict_bundle_spec = $ps_bundle_name_proc,
                $this,
                $pa_bundle_settings
            ))
            ||
            ($va_dictionary_entry = ca_metadata_dictionary_entries::getEntry(
                $dict_bundle_spec = $this->tableName() . '.' . $ps_bundle_name_proc,
                $this,
                $pa_bundle_settings
            ))
        ) {
            ...
            if (!($pa_bundle_settings['definition'][$g_ui_locale] = caExtractSettingsValueByUserLocale(
                'definition',
                $va_dictionary_entry['settings']
            ))) {
            ...
            if (caGetOption('mandatory', $va_dictionary_entry['settings'], false)) {
                ...
                $pa_bundle_settings['definition'][$g_ui_locale] = $this->getAppConfig()->get(
                        'required_field_marker'
                    ) . (is_array($def) ? $def[$g_ui_locale] : $def);
            }

            $va_violations = $this->getMetadataDictionaryRuleViolations(
                $dict_bundle_spec,
                [
                    'placement_id' => (int)str_replace(
                        "P",
                        "",
                        $ps_placement_code
                    )
            ...
            if (is_array($va_violations) && sizeof($va_violations)) {
                ...
                foreach ($va_violations as $va_violation) {
                    $va_violation_text[] = "<li class='caMetadataDictionaryViolation'><span class='caMetadataDictionaryViolation" . (ucfirst(
                            strtolower($va_violation['level'])
                        )) . "'>" . $va_violation['levelDisplay'] . '</span>: ' . caExtractSettingsValueByUserLocale(
                            'violationMessage',
                            $va_violation
                        ) . "</li>";
                }
                $pa_bundle_settings['definition'][$g_ui_locale] = "<div class='caMetadataDictionaryViolationsList'><div class='caMetadataDictionaryViolationsListHeading'>" . _t(
        ...
        $vs_documentation_url = trim(
            (isset($pa_bundle_settings['documentation_url']) && $pa_bundle_settings['documentation_url']) ? $pa_bundle_settings['documentation_url'] : ''
        );
        ...
        switch ($vs_type) {
            ...
            case 'preferred_label':
            case 'nonpreferred_label':
                if (is_array($va_error_objects = $pa_options['request']->getActionErrors($ps_bundle_name)) && sizeof(
                        $va_error_objects
                    )) {
                ...
                $vs_element = ($vs_type === 'preferred_label') ? $this->getPreferredLabelHTMLFormBundle(
                    $pa_options['request'],
                    $pa_options['formName'],
                    $ps_placement_code,
                    $pa_bundle_settings,
                    $pa_options
                ) : $this->getNonPreferredLabelHTMLFormBundle(
                    $pa_options['request'],
                    $pa_options['formName'],
                    $ps_placement_code,
                    $pa_bundle_settings,
                ...
                if (($vs_type == 'preferred_label') && $o_config->get('show_required_field_marker') && $o_config->get(
                        'require_preferred_label_for_' . $this->tableName()
                    )) {
            ...
            # -------------------------------------------------
            case 'intrinsic':
                if (!($pa_options['label'] = caExtractSettingValueByLocale(
                    $pa_bundle_settings,
                    'label',
                    $g_ui_locale
                ))) {
                ...
                if ($this->getFieldInfo($ps_bundle_name, 'IDENTITY')) {
                    $o_view->setVar(
                        'form_element',
                        ($vn_id = (int)$this->get($ps_bundle_name)) ? $vn_id : "&lt;" . _t(
                                'Not yet issued'
                            ) . "&gt;"
                    );
                } else {
                    $vb_read_only = ($pa_bundle_settings['readonly'] || ($pa_options['request']->user->getBundleAccessLevel(
                                $this->tableName(),
                                $ps_bundle_name
                    ...
                    $o_view->setVar(
                        ...
                        $this->htmlFormElement(
                            $ps_bundle_name,
                            ($this->getProperty('ID_NUMBERING_ID_FIELD') == $ps_bundle_name) ? $o_config->get(
                ...
                if ($o_config->get('show_required_field_marker')) {
                    if (($this->getFieldInfo($ps_bundle_name, 'FIELD_TYPE') == FT_TEXT) && is_array(
                            $va_bounds = $this->getFieldInfo($ps_bundle_name, 'BOUNDS_LENGTH')
                        ) && ($va_bounds[0] > 0)) {
                        $vs_label .= ' ' . $vs_required_marker;
                    } else {
                        if ((in_array(
                                $this->getFieldInfo($ps_bundle_name, 'FIELD_TYPE'),
                ...
                if ((bool)$this->getAppConfig()->get(
                        $this->tableName() . '_allow_access_inheritance'
                    ) && $this->hasField('access_inherit_from_parent') && !$this->getPrimaryKey()) {
                    $this->set(
                        'access_inherit_from_parent',
                        (bool)$this->getAppConfig()->get(
                            $this->tableName() . '_access_inheritance_default'
                        ) ? 1 : 0
                    );
                }
                ...
                if (!($vs_description = caExtractSettingValueByLocale(
                    $pa_bundle_settings,
                    'description',
                    $g_ui_locale
                ))) {
            ...
            case 'attribute':
                ...
                $vs_element = $this->getAttributeHTMLFormBundle(
                    $pa_options['request'],
                    $pa_options['formName'],
                    $vs_attr_element_code,
                    $ps_placement_code,
                    $pa_bundle_settings,
                    $pa_options
                );
                ...
                if ($vb_batch) {
                    ...
                    if (sizeof($va_type_restrictions)) {
                        ...
                        $vs_label = '<span class="formLabelText" id="' . $vs_field_id . '">' . $vs_label_text . '</span> <span class="formLabelSubtext" id="subtext_' . $vs_field_id . '">(' . caTruncateStringWithEllipsis(
                                $vs_restriction_list,
                                75
                            ) . ')</span>';
                        TooltipManager::add(
                            "#subtext_{$vs_field_id}",
                            "<h3>" . _t("Restricted to types") . "</h3>" . join(
                                "<br/>",
                                $va_type_restrictions
                            )
                        );
                    } else {
                        $vs_label = '<span class="formLabelText" id="' . $vs_field_id . '">' . $vs_label_text . '</span>';
                ...
                $vs_documentation_url = trim(
                    (isset($pa_bundle_settings['documentation_url']) && $pa_bundle_settings['documentation_url']) ? $pa_bundle_settings['documentation_url'] : $vs_documentation_url = $this->getAttributeDocumentationUrl(
                        $vs_attr_element_code
                    )
                );

                if ($t_element = ca_metadata_elements::getInstance($vs_attr_element_code)) {
                    if ($o_config->get('show_required_field_marker') && (($t_element->getSetting(
                                    'minChars'
                                ) > 0) || ((bool)$t_element->getSetting(
                                'mustNotBeBlank'
            ...
            case 'related_table':
                ...
                switch ($ps_bundle_name) {
                    ...
                    case 'ca_sets':
                        ...
                        $vs_element = $this->getRelatedHTMLFormBundle(
                            $pa_options['request'],
                            $pa_options['formName'],
                            $ps_bundle_name,
                            $ps_placement_code,
                    ...
                    case 'ca_object_lots':
                        ...
                        if (($this->tableName(
                                ) != 'ca_object_lots') && ($vn_lot_id = $pa_options['request']->getParameter(
                                'lot_id',
                                pInteger
                            ))) {
                    ...
                    case 'ca_representation_annotations':
                        ...
                        $vs_element = $this->getRepresentationAnnotationHTMLFormBundle(
                            $pa_options['request'],
                            $pa_options['formName'],
                            $ps_placement_code,
                            $pa_bundle_settings,
            ...
            case 'special':
                ...
                switch ($ps_bundle_name) {
                    ...
                    case 'ca_metadata_dictionary_rules':
                        ...
                        $vs_element .= $this->getRulesHTMLFormBundle(
                            $pa_options['request'],
                            $pa_options['formName'],
                            $ps_placement_code,
                            $pa_bundle_settings,
                    ...
                    case 'ca_site_pages_content':
                        $vs_element .= $this->getPageContentHTMLFormBundle(
                            $pa_options['request'],
                            $pa_options['formName'],
                            $ps_placement_code,
                            $pa_bundle_settings,
                    ...
                    // This bundle is only available items for ca_site_pages
                    case 'ca_site_page_media':
                        $vs_element .= $this->getPageMediaHTMLFormBundle(
                            $pa_options['request'],
                            $pa_options['formName'],
                            $ps_placement_code,
                            $pa_bundle_settings,
                    ...
                    //
                    case 'ca_item_tags':
                        $vs_element .= $this->getItemTagHTMLFormBundle(
                            $pa_options['request'],
                            $pa_options['formName'],
                            $ps_placement_code,
                            $pa_options,
                    ...
                    case 'ca_item_comments':
                        $vs_element .= $this->getItemCommentHTMLFormBundle(
                            $pa_options['request'],
                            $pa_options['formName'],
                            $ps_placement_code,
                            $pa_options,
                    ...
                    case 'ca_representation_transcriptions':
                        $vs_element = $this->getTranscriptionHTMLFormBundle(
                            $pa_options['request'],
                            $pa_options['formName'],
                            $ps_placement_code,
                            $pa_options,
        ...
        if ($vs_documentation_url) {
            ...
            $vs_documentation_link = "<a class='bundleDocumentationLink' href='$vs_documentation_url' target='_blank' >" . caNavIcon(
                    __CA_NAV_ICON_INFO__,
                    '15px'
                ) . "</a>";
        }
        ...
        if (is_array($va_violations)) {
            foreach ($va_violations as $v) {
                if (is_array(
                        $v
                    ) && isset($v['showasprompt']) && (bool)$v['showasprompt'] && ($v['bundle_name'] == $dict_bundle_spec)) {
        ...
        if (is_array($violations_to_prompt) && sizeof($violations_to_prompt)) {
            ...
            $violations_text = array_map(
                function ($v) {
                    return caExtractSettingsValueByUserLocale('violationMessage', $v);
                },
                $violations_to_prompt
            );
            $prompt = "<script type='text/javascript'>caPromptManager.addPrompt('{$prompt_id}', '" . addslashes(
    ...
    public function getDisplayLabel($ps_field)
    {
                    ...
                    case 'preferred_labels':
                        if (method_exists(
                            ...
                            if (!isset($va_tmp[2])) {
                                ...
                                return caUcFirstUTF8Safe(
                                        $t_label_instance->getDisplayLabel(
                                            $t_label_instance->tableName() . '.' . $va_tmp[2]
                                        )
                                    ) . $vs_suffix;
                            }
                        }
        ...
        if (($va_tmp[0] === $this->tableName(
                )) && isset($this->BUNDLES[$va_tmp[1]]) && $this->BUNDLES[$va_tmp[1]]['label']) {
                ...
                switch ($va_tmp[1]) {
                    ...
                    case 'nonpreferred_labels':
                        if (method_exists(
                            ...
                            if (!isset($va_tmp[2])) {
                                return _t(
                                    'A list of alternate %1 %2',
                                    $t_label_instance->getProperty('NAME_PLURAL'),
                                    $vs_suffix
                                );
                            } else {
                                return _t(
    ...
    public function htmlFormElementForSearch($po_request, $ps_field, $pa_options = null)
    {
        ...
        switch ($va_tmp[0]) {
            ...
            case '_fieldlist':
                ...
                if (is_array($va_fields = preg_split("![;,]+!", caGetOption('fields', $pa_options, null))) && sizeof(
                        $va_fields
                    )) {
                ...
                $va_options = caGetBundlesAvailableForSearch(
                    $this->tableName(),
                ...
                return caHTMLSelect(
                        "_fieldlist_field" . ($vb_as_array_element ? "[]" : ""),
                        $va_options,
                        array(
                    ...
                    caHTMLTextInput(
                        "_fieldlist_value" . ($vb_as_array_element ? "[]" : ""),
                        array(
            ...
            switch (sizeof($va_tmp)) {
                ...
                case 1:        // table_name
                    if ($va_tmp[0] != $this->tableName()) {
                        ...
                        return caHTMLTextInput(
                            $ps_field . $vs_rel_types . ($vb_as_array_element ? "[]" : ""),
                            array(
                        ...
                        switch (sizeof($va_tmp)) {
                            ...
                            case 3:
                                if ($ps_render = caGetOption('render', $pa_options, null)) {
                                    switch ($ps_render) {
                                        case 'is_set':
                                            return caHTMLCheckboxInput(
                                                $ps_field . $vs_rel_types,
                                                array('value' => '[' . _t('SET') . ']')
                                            );
                                            break;
                                        case 'is':
                                ...
                                if (caGetOption('autocomplete', $pa_options, false)) {
                                    ...
                                    return caGetAdvancedSearchFormAutocompleteJS(
                                        $po_request,
                                        $ps_field,
                                        $t_instance,
                                        $pa_options
                                        ...
                                        if (is_array($va_path = Datamodel::getPath($this->tableName(), $va_tmp[0]))) {
                                            ...
                                            if (sizeof($va_path) == 3) {
                                                if ($t_rel = Datamodel::getInstanceByTableName($va_tmp[0], true)) {
                                                    ...
                                                    if ($this->hasField('access') && is_array($va_access) && sizeof(
                                                            $va_access
                                                        )) {
                                                    ...
                                                    $qr_in_use = $this->getDb()->query(
														...
														" . ((sizeof($va_sql_wheres) > 0) ? "WHERE " . join(
                                                                " AND ",
                                                                $va_sql_wheres
                                                            ) : "") . "		
													",
                                    ...
                                    if ($qr_res) {
                                        ...
                                        while ($qr_res->nextHit()) {
                                            ...
                                            if (is_array($va_access) && sizeof($va_access) && !in_array(
                                                    $qr_res->get($va_tmp[0] . '.access'),
                                                    $va_access
                                                )) {
                                                continue;
                                            }
                                            if (is_array($va_in_use_list) && !in_array(
                                                    $vn_item_id = $qr_res->get($vs_rel_pk),
                                                    $va_in_use_list
                                                )) {
                                                continue;
                                            }
                                            $va_opts[$qr_res->get(
                                                $va_tmp[0] . ".preferred_labels.{$vs_label_display_field}"
                                            )] = $qr_res->get($vs_field);
                                        }
                                    }
    ...
    public function htmlFormElementForSimpleForm($po_request, $ps_field, $pa_options = null)
    {
        ...
        if ($vs_rel_type = caGetOption('relationshipType', $pa_options, null)) {
            $vs_buf .= caHTMLHiddenInput(
                $ps_field . '_relationship_type' . ($vb_as_array_element ? "[]" : ""),
                array('value' => $vs_rel_type)
            );
        }
        if ($vs_type = caGetOption('type', $pa_options, null)) {
            $vs_buf .= caHTMLHiddenInput(
                $ps_field . '_type' . ($vb_as_array_element ? "[]" : ""),
                array('value' => $vs_type)
            );
        }
                ...
                if ($va_tmp[0] != $this->tableName()) {
                    ...
                    if (caGetOptions('autocomplete', $pa_options, false)) {
                        ...
                        if (!is_null($index = caGetOption('index', $pa_options, null))) {
                            ...
                            $related_ids = $this->get(
                                $t_instance->primaryKey(true),
                                [
                                    'returnAsArray' => true,
                                    'restrictToRelationshipTypes' => [$vs_rel_type]
                                ]
                                ...
                                case 3:
                                    if (caGetOption('autocomplete', $pa_options, false)) {
                                        ...
                                        if (!is_null($index = caGetOption('index', $pa_options, null))) {
                                            ...
                                            $related_ids = $this->get(
                                                $ps_field . "." . $t_instance->primaryKey(),
                                                ['returnAsArray' => true]
                                            );
                                            $value = $values[$index];
                                    ...
                                    return $vs_buf . $t_instance->htmlFormElementForSearch(
                                            $po_request,
                                            $ps_field,
                                            $pa_options
                                        );
    ...
    {
        $va_omit_bundles = (isset($pa_options['omit']) && is_array(
                $pa_options['omit']
        ...
        if (($vn_ui_access = ca_editor_uis::getAccessForUI(
                $pa_options['request'],
                $t_ui->getPrimaryKey()
            )) === __CA_BUNDLE_ACCESS_NONE__) {
            // no access to UI
            $this->postError(
                2320,
                _t('Access denied to UI %1', $t_ui->get('editor_code')),
        ...
        if (isset($pa_options['bundles']) && is_array($pa_options['bundles'])) {
            ...
            if (($vn_screen_access = ca_editor_uis::getAccessForScreen(
                    $pa_options['request'],
                    $pm_screen
                )) === __CA_BUNDLE_ACCESS_NONE__) {
                // no access to screen
                $this->postError(
                    2320,
                    _t('Access denied to screen %1', $pm_screen),
                    "BundlableLabelableBaseModelWithAttributes->getBundleFormHTMLForScreen()"
                );
                return false;
            }
        ...
        if (is_array($va_bundles)) {
            ...
            foreach ($va_bundles as $va_bundle) {
                ...
                $va_definition_bundle_names[((!Datamodel::tableExists($va_bundle['bundle_name']) && !preg_match(
                        "!^{$vs_table_name}\.!",
                        $va_bundle['bundle_name']
                    )) ? "{$vs_table_name}." : "") . str_replace("ca_attribute_", "", $va_bundle['bundle_name'])] = 1;
            }
            ...
            if (is_subclass_of($this, 'BaseRelationshipModel')) {
                ...
                if (isset($pa_options['restrictToTypes']) && is_array($pa_options['restrictToTypes'])) {
                    $va_valid_element_codes = $this->getApplicableElementCodesForTypes(
                        caMakeRelationshipTypeIDList($vs_table_name, $pa_options['restrictToTypes'])
                    );
                } else {
                ...
                if (isset($pa_options['restrictToTypes']) && is_array($pa_options['restrictToTypes'])) {
                    $va_valid_element_codes = $this->getApplicableElementCodesForTypes(
                        caMakeTypeIDList($vs_table_name, $pa_options['restrictToTypes'])
                    );
                } else {
                    $va_valid_element_codes = null;
                }
            ...
            foreach ($va_bundles as $va_bundle) {
                ...
                if ($vs_element_set_code = preg_replace(
                    "/^(ca_attribute_|" . $this->tableName() . "\.)/",
                    "",
                    $va_bundle['bundle_name']
                )) {
                    if (($o_element = ca_metadata_elements::getInstance($vs_element_set_code)) && ($this->hasElement(
                            $vs_element_set_code
                        ))) {
                ...
                if ($vs_bundle_form_html = $this->getBundleFormHTML(
                    $va_bundle['bundle_name'],
                    'P' . $va_bundle['placement_id'],
                    $va_bundle['settings'],
                    array_merge(
                        $pa_options,
                        ['bundle_id' => "{$pm_screen}_{$va_bundle['placement_id']}"]
                    ),
                    ...
                    $pa_placements["{$pm_screen}_{$va_bundle['placement_id']}"] = array(
                        'name' => $vs_bundle_display_name ? $vs_bundle_display_name : $this->getDisplayLabel(
                            $vs_table_name . "." . $va_bundle['bundle_name']
                        ),
        ...
        if (!$vn_pk_id) {
            ...
            foreach ($va_mandatory_fields as $vs_field) {
                if (!isset($va_bundles_present[$vs_field]) || !$va_bundles_present[$vs_field]) {
                    $va_bundle_html[$vs_field] = $this->getBundleFormHTML(
                        $vs_field,
                        'mandatory_' . $vs_field,
                        array(),
                        $pa_options
            ...
            // add type_id
            if (isset($this->ATTRIBUTE_TYPE_ID_FLD) && $this->ATTRIBUTE_TYPE_ID_FLD && !in_array(
                    'type_id',
                    $va_omit_bundles
                )) {
                $va_bundle_html[$this->ATTRIBUTE_TYPE_ID_FLD] = caHTMLHiddenInput(
                    $this->ATTRIBUTE_TYPE_ID_FLD,
                    array(
                        'value' => $pa_options['request']->getParameter(
                            $this->ATTRIBUTE_TYPE_ID_FLD,
                            pString
                        )
                    )
            ...
            // add parent_id
            if (isset($this->HIERARCHY_PARENT_ID_FLD) && $this->HIERARCHY_PARENT_ID_FLD && !in_array(
                    'parent_id',
                    $va_omit_bundles
                )) {
                $va_bundle_html[$this->HIERARCHY_PARENT_ID_FLD] = caHTMLHiddenInput(
            ...
            if (isset($pa_options['force']) && $pa_options['force']) {
                ...
                foreach ($pa_options['force'] as $vn_x => $vs_bundle) {
                    if (!isset($va_bundles_present[$vs_bundle]) || !$va_bundles_present[$vs_bundle]) {
                        $va_bundle_html['_force_' . $vs_bundle] = $this->getBundleFormHTML(
                            $vs_bundle,
                            'force_' . $vs_field,
                            array(),
                            $pa_options
            ...
            if (isset($pa_options['forceHidden']) && $pa_options['forceHidden']) {
                ...
                foreach ($pa_options['forceHidden'] as $vn_x => $vs_field) {
                    if (!isset($va_bundles_present[$vs_field]) || !$va_bundles_present[$vs_field]) {
                        $va_bundle_html['_force_hidden_' . $vs_field] = caHTMLHiddenInput(
                            $vs_field,
                            array(
                                'value' => $pa_options['request']->getParameter(
                                    $vs_field,
    ...
    public function getHierarchyNavigationHTMLFormBundle(
        ...
        $o_view = $this->_getHierarchyLocationHTMLFormBundleInfo(
            $po_request,
            $ps_form_name,
            $ps_placement_code,
            $pa_options,
    ...
    private function _getHierarchyLocationHTMLFormBundleInfo(
        ...
        if ($this->supportsPreferredLabelFlag()) {
            if (!($va_ancestor_list = $this->getHierarchyAncestors(
                $vn_id,
                array(
        ...
        $vs_object_collection_rel_type = $this->getAppConfig()->get(
        ...
        $t_rel = ca_relationship_types::findAsInstance(
            [
        ...
        $o_view->setVar(
            'objectTypeList',
            trim(
                $t_object->getTypeListAsHTMLFormElement(
                    ...
                    [
                        ...
                        'dontIncludeSubtypesInTypeRestriction' => $t_rel ? !$t_rel->get(
                            'ca_relationship_types.include_subtypes_left'
                        ) : null
                    ]
                )
            )
        ...
        $o_view->setVar(
            'object_collection_collection_ancestors',
            []
        ); // collections to display as object parents when ca_objects_x_collections_hierarchy_enabled is enabled
        if (($this->tableName() == 'ca_objects') && $this->getAppConfig()->get(
                'ca_objects_x_collections_hierarchy_enabled'
            )) {
            ...
            if (is_array(
                $va_collections = $t_object->getRelatedItems(
                    'ca_collections',
                ...
                foreach ($va_collections as $vs_key => $va_collection) {
                    ...
                    if (!($va_collection_ancestor_list = $t_collection->getHierarchyAncestors(
                        $va_collection['collection_id'],
                        array(
    ...
    public function getRelatedBundleFormValues(
        ...
        if ($s = caGetOption('sort', $pa_options, caGetOption('sort', $pa_bundle_settings, null))) {
            ...
            $va_get_related_opts['sortDirection'] = caGetOption(
                'sortDirection',
                $pa_options,
                caGetOption('sortDirection', $pa_bundle_settings, null)
            );
        ...
        $policy = caGetOption('showCurrentOnly', $pa_bundle_settings, false) ? caGetOption(
            'policy',
            $pa_bundle_settings,
            null
        ) : null;
        ...
        if ($ps_related_table == 'ca_sets') {
            ...
            $t_set = new ca_sets();
            $va_items = caExtractValuesByUserLocale(
                $t_set->getSetsForItem($this->tableNum(), $this->getPrimaryKey(), $va_get_related_opts)
            );

            // sort
            if ($ps_sort = caGetOption('sort', $va_get_related_opts, null)) {
                $va_items = caSortArrayByKeyInValue(
                    $va_items,
                    array($ps_sort),
            ...
            if ($vb_is_many_many) {
                ...
                $qr_rel_items = $t_item->makeSearchResult(
                    $t_item_rel->tableNum(),
                    $va_ids,
    ...
    public function getRelatedHTMLFormBundle(
        ...
        $vb_read_only = ($po_request->user->getBundleAccessLevel(
                $this->tableName(),
                $ps_related_table
            ) == __CA_BUNDLE_ACCESS_READONLY__) ? true : false;
        if (!$pa_bundle_settings['readonly']) {
        ...
        if (method_exists(
                $t_item,
                'policy2bundleconfig'
            ) && ($default_policy = $t_item->getDefaultHistoryTrackingCurrentValuePolicy(
            ))) {    // TODO: make policy configurable?
            $o_view->setVar('history_tracking_policy', $default_policy);
            $o_view->setVar(
                'history_tracking_policy_info',
        ...
        if (method_exists($t_item_rel, 'getRelationshipTypes')) {
            ...
            $o_view->setVar(
                'relationship_types_by_sub_type',
        ...
        $o_view->setVar('t_subject', $this);
        $va_initial_values = $this->getRelatedBundleFormValues(
            $po_request,
            $ps_form_name,
            $ps_related_table,
            $ps_placement_code,
        ...
        $c = (caGetOption(
                'limit',
                $pa_options,
                caGetOption('numPerPage', $pa_bundle_settings, null)
            ) > 0) ? $this->getRelatedItems(
            $ps_related_table,
            array_merge($pa_bundle_settings, ['returnAs' => 'count'])
        ...
        $o_view->setVar(
            'defaultRepresentationUploadType',
    ...
    public function getRelatedListHTMLFormBundle(
        ...
        $o_view->setVar(
            'relationship_types',
            method_exists($t_item_rel, 'getRelationshipTypes') ? $t_item_rel->getRelationshipTypes(
                null,
                null,
                array_merge($pa_options, $pa_bundle_settings)
            ) : []
    ...
    protected function getBundleListsForScreen($pm_screen, $po_request, $t_ui, $pa_options = null)
    {
        ...
        if (is_array($va_bundles)) {
            foreach ($va_bundles as $vn_i => $va_tmp) {
                ...
                if (($po_request->user->getBundleAccessLevel(
                        $this->tableName(),
                        $va_tmp['bundle_name']
                    )) < __CA_BUNDLE_ACCESS_EDIT__) {    // don't save bundles use doesn't have edit access for
                    continue;
                }
        ...
        if (($this->tableName() == 'ca_objects') && (!in_array(
                'lot_id',
                ...
                switch ($vs_f) {
                    case $vs_idno_field:
                        if (($this->opo_idno_plugin_instance) && !$pa_options['dontReturnSerialIdno']) {
                            ...
                            $va_values['intrinsic'][$vs_f] = $this->opo_idno_plugin_instance->htmlFormValue(
                                $vs_idno_field,
                                null,
                                true
                            );
        ...
        if (is_array($va_bundle_lists['fields_by_type']['attribute'])) {
            foreach ($va_bundle_lists['fields_by_type']['attribute'] as $vs_placement_code => $vs_f) {
                ...
                foreach ($_REQUEST as $vs_key => $vs_val) {
                    ...
                    if (
                        preg_match(
                            '/' . $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_id . '_([\w\-_]+)_new_([\d]+)/',
                            $vs_key,
                            $va_matches
                        )
                        ||
                        preg_match(
                            '/' . $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_id . '_([\w\-_]+)_([\d]+)/',
                            $vs_key,
                            $va_matches
                        )
                    ) {
        ...
        if (is_array($va_bundle_lists['fields_by_type']['preferred_label'])) {
            foreach ($va_bundle_lists['fields_by_type']['preferred_label'] as $vs_placement_code => $vs_f) {
                foreach ($_REQUEST as $vs_key => $vs_value) {
                    if (
                        !preg_match(
                            '/' . $vs_placement_code . $vs_form_prefix . '_Pref' . 'locale_id_(new_[\d]+)/',
                            $vs_key,
                            $va_matches
                        )
                        &&
                        !preg_match(
                            '/' . $vs_placement_code . $vs_form_prefix . '_Pref' . 'locale_id_([\d]+)/',
                            $vs_key,
                            $va_matches
                        )
                    ) {
                    ...
                    ($vn_label_locale_id = $vs_value)
                        ...
                        if (is_array(
                            $va_label_values = $this->getLabelUIValuesFromRequest(
                                $po_request,
                                $vs_placement_code . $vs_form_prefix,
                                $vn_c,
                                true
                            )
                        )) {
            ...
            if ($this->getPrimaryKey() && $this->HIERARCHY_PARENT_ID_FLD && ($vn_parent_id > 0)) {
                if ($vs_parent_table == $this->tableName()) {
                    ...
                    if ((bool)$this->getAppConfig()->get(
                            ...
                            ) == 'ca_objects') && ($vs_coll_rel_type = $this->getAppConfig()->get(
                            'ca_objects_x_collections_hierarchy_relationship_type'
                        ))) {
                ...
                if ($this->getPrimaryKey(
                    ) && $this->HIERARCHY_PARENT_ID_FLD && ($this->HIERARCHY_TYPE == __CA_HIER_TYPE_ADHOC_MONO__) && isset($_REQUEST[$vs_k]) && ($vn_parent_id <= 0)) {
                    return __CA_PARENT_COLLECTION_CHANGED__;
                }
            }
            break;
    ...
    public function saveBundlesForScreen($pm_screen, $po_request, &$pa_options)
    {
        ...
        if (!$this->inTransaction()) {
            ...
            if ($vb_dryrun) {
                $this->postError(
                    799,
                    _t(
                        'Cannot do dry run save when in transaction. Try again without setting a transaction.'
                    ),
        ...
        if (is_array($va_fields_by_type['intrinsic'])) {
            ...
            foreach ($va_fields_by_type['intrinsic'] as $vs_placement_code => $vs_f) {
                ...
                if ($vb_batch) {
                    $vs_batch_mode = $po_request->getParameter(
                        "{$vs_placement_code}{$vs_form_prefix}_batch_mode",
                        pString
                    );
                ...
                if (isset($_FILES["{$vs_placement_code}{$vs_form_prefix}{$vs_f}"]) && $_FILES["{$vs_placement_code}{$vs_form_prefix}{$vs_f}"]) {
                    ...
                    $this->set(
                        $vs_f,
                        $_FILES["{$vs_placement_code}{$vs_form_prefix}{$vs_f}"]['tmp_name'],
                    ...
                    switch ($vs_f) {
                        case 'access':
                            if ((bool)$this->getAppConfig()->get(
                                ...
                                $this->set(
                                    'access_inherit_from_parent',
                                    $po_request->getParameter(
                                        "{$vs_placement_code}{$vs_form_prefix}access_inherit_from_parent",
                                        pInteger
                                    )
                                );
                            }
                            if (!(bool)$this->getAppConfig()->get(
                                    $this->tableName() . '_allow_access_inheritance'
                                ) || !$this->hasField('access_inherit_from_parent') || !(bool)$this->get(
                                    'access_inherit_from_parent'
                                )) {
                                $this->set(
                                    'access',
                                    $po_request->getParameter(
                                        "{$vs_placement_code}{$vs_form_prefix}access",
                                        pString
                                    )
                                );
                            }
                        ...
                        case $vs_idno_field:
                            if (!(bool)$this->getAppConfig()->get(
                                    $this->tableName() . '_dont_allow_editing_of_codes_when_in_use'
                                ) || !$this->getPrimaryKey()) {
                                if ($this->opo_idno_plugin_instance) {
                                    $this->opo_idno_plugin_instance->setDb($this->getDb());
                                    ...
                                    $this->set(
                                        $vs_f,
                                        $po_request->getParameter(
                                            "{$vs_placement_code}{$vs_form_prefix}{$vs_f}",
                                            pString
                                        )
                                    );
                                }
        ...
        if (isset($va_fields_by_type['attribute']) && is_array($va_fields_by_type['attribute'])) {
            ...
            foreach ($va_fields_by_type['attribute'] as $vs_placement_code => $vs_f) {
                ...
                foreach ($_REQUEST as $vs_key => $vs_val) {
                    ...
                    if (preg_match(
                        '/' . $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_id . '_([\w\d\-_]+)_new_([\d]+)/',
                        $vs_key,
                        $va_matches
                    )) {
                        if ($vb_batch) {
                        ...
                        // is it a delete key?
                        if (preg_match(
                            '/' . $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_id . '_([\d]+)_delete/',
                            $vs_key,
                            $va_matches
                        )) {
                ...
                foreach ($_FILES as $vs_key => $va_val) {
                    if (preg_match(
                        '/' . $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_id . '_locale_id_new_([\d]+)/',
                        $vs_key,
                        $va_locale_matches
                    )) {
                ...
                if (!$vb_batch) {
                    ...
                    if (is_array($va_attrs = $this->getAttributesByElement($vn_element_id))) {
                        ...
                        foreach ($va_attrs as $o_attr) {
                            ...
                            // skip element
                            if ($po_request->getParameter(
                                $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_set_id . '_dont_save_' . $vn_attribute_id,
                                pInteger
                            )) {
                                continue;
                            }
                            ...
                            $va_attr_update = array(
                                'locale_id' => $po_request->getParameter(
                                    $vs_placement_code . $vs_form_prefix . '_attribute_' . $vn_element_set_id . '_locale_id_' . $vn_attribute_id,
                                    pString
                                )
                            );
        ...
        if (is_array($va_fields_by_type['special'])) {
            foreach ($va_fields_by_type['special'] as $vs_placement_code => $vs_bundle) {
                ...
                if ($vb_batch && ($po_request->getParameter(
                            $vs_placement_code . $vs_form_prefix . '_batch_mode',
                            pString
                        ) !== '_replace_')) {
                    continue;
                ...
                $va_parent_tmp = explode(
                    "-",
                    $po_request->getParameter(
                        "{$vs_placement_code}{$vs_form_prefix}_new_parent_id",
                        pString
                    )
                );
                ...
                if ($this->getPrimaryKey() && $this->HIERARCHY_PARENT_ID_FLD && ($vn_parent_id > 0)) {
                        ...
                        if ((bool)$this->getAppConfig()->get(
                            ...
                            if (!($this->addRelationship('ca_collections', $vn_parent_id, $vs_coll_rel_type))) {
                                $this->postError(
                                    2510,
                                    _t(
                                        'Could not move object under collection: %1',
                                        join("; ", $this->getErrors())
                                    ),
                                    "BundlableLabelableBaseModelWithAttributes->saveBundlesForScreen()"
                                );
                    ...
                    if ($this->getPrimaryKey(
                        ) && $this->HIERARCHY_PARENT_ID_FLD && ($this->HIERARCHY_TYPE == __CA_HIER_TYPE_ADHOC_MONO__) && isset($_REQUEST["{$vs_placement_code}{$vs_form_prefix}_new_parent_id"]) && ($vn_parent_id <= 0)) {
                        $this->set($this->HIERARCHY_PARENT_ID_FLD, null);
                        $this->set($this->HIERARCHY_ID_FLD, $this->getPrimaryKey());
                        ...
                        if ((bool)$this->getAppConfig()->get(
                                'ca_objects_x_collections_hierarchy_enabled'
                            ) && ($this->tableName() == 'ca_objects') && ($vs_coll_rel_type = $this->getAppConfig(
                            )->get(
                                'ca_objects_x_collections_hierarchy_relationship_type'
                            )) && ($vn_parent_id == -1)) {    // -1 = extract from hierarchy
                            $this->removeRelationships('ca_collections', $vs_coll_rel_type);
                        }
                    }
                }
        ...
        if ($this->getProperty('LABEL_TABLE_NAME')) {
            $vb_check_for_dupe_labels = $this->_CONFIG->get(
                'allow_duplicate_labels_for_' . $this->tableName()
            ) ? false : true;
            $vb_error_inserting_pref_label = false;
            if (is_array($va_fields_by_type['preferred_label'])) {
                $vs_label_must_be_in_list = $this->_CONFIG->get(
                    'preferred_label_for_' . $this->tableName() . '_must_be_in_list'
                );
                ...
                foreach ($va_fields_by_type['preferred_label'] as $vs_placement_code => $vs_f) {
                        ...
                        foreach ($va_preferred_labels as $vn_item_id => $va_labels_by_locale) {
                            foreach ($va_labels_by_locale as $vn_locale_id => $va_label_list) {
                                foreach ($va_label_list as $va_label) {
                                    if ($vn_label_locale_id = $po_request->getParameter(
                                        $vs_placement_code . $vs_form_prefix . '_Pref' . 'locale_id_' . $va_label['label_id'],
                                        pString
                                    )) {
                                        $vn_label_type_id = $po_request->getParameter(
                                            $vs_placement_code . $vs_form_prefix . '_Pref' . 'type_id_' . $va_label['label_id'],
                                            pInteger
                                        );
                                        if (is_array(
                                            $va_label_values = $this->getLabelUIValuesFromRequest(
                                            ...
                                            if ($vb_check_for_dupe_labels && $this->checkForDupeLabel(
                                                    $vn_label_locale_id,
                                                    $va_label_values
                                                )) {
                                                $this->postError(
                                                    1125,
                                                    _t(
                                                        'Value <em>%1</em> is already used and duplicates are not allowed',
                                                        join(
                                                            '',
                                                            array_map(
                                                                function ($v) {
                                            ...
                                            if ($vs_label_must_be_in_list && !caGetListItemIDForLabel(
                                                    $vs_label_must_be_in_list,
                                                    $vs_label_val = $va_label_values[$this->getLabelDisplayField()]
                                                )) {
                                                $this->postError(
                                                    1100,
                                                    _t(
                                                        'Value <em>%1</em> is not in <em>%2</em>',
                                                        $vs_label_val,
                                            ...
                                            $this->editLabel(
                                                $va_label['label_id'],
                                                array(
                                                    $this->getLabelDisplayField() => '[' . caGetBlankLabelText(
                                                            $table
                                                        ) . ']'
                                                ),
                                                $vn_label_locale_id,
                                        ...
                                        if ($po_request->getParameter(
                                            $vs_placement_code . $vs_form_prefix . '_PrefLabel_' . $va_label['label_id'] . '_delete',
                                            pString
                                        )) {
                                            // delete
                    ...
                    foreach ($_REQUEST as $vs_key => $vs_value) {
                        if (!preg_match(
                            '/' . $vs_placement_code . $vs_form_prefix . '_Pref' . 'locale_id_new_([\d]+)/',
                            $vs_key,
                            $va_matches
                        )) {
                            continue;
                        ...
                        if ($vb_batch) {
                            $vs_batch_mode = $po_request->getParameter(
                                $vs_placement_code . $vs_form_prefix . '_Pref_batch_mode',
                                pString
                            );
                            switch ($vs_batch_mode) {
                                case '_disabled_':        // skip
                        ...
                        if ($vn_new_label_locale_id = $po_request->getParameter(
                            $vs_placement_code . $vs_form_prefix . '_Pref' . 'locale_id_new_' . $vn_c,
                            pString
                        )) {
                            if (is_array(
                                $va_label_values = $this->getLabelUIValuesFromRequest(
                                ...
                                if ($vb_batch && ($vs_batch_mode == '_add_')) {
                                    ...
                                    $va_labels_for_this_locale = $this->getPreferredLabels(
                                        array($vn_new_label_locale_id)
                                    );
                                    ...
                                    if (is_array($va_labels_for_this_locale) && (sizeof(
                                                $va_labels_for_this_locale
                                            ) > 0)) {
                                        $this->postError(
                                            1125,
                                            _t(
                                                'A preferred label for this locale already exists. Only one preferred label per locale is allowed.'
                                            ),
                                            "BundlableLabelableBaseModelWithAttributes->saveBundlesForScreen()",
                                            $this->tableName() . '.preferred_labels'
                                        );
                                ...
                                $vn_label_type_id = $po_request->getParameter(
                                    $vs_placement_code . $vs_form_prefix . '_Pref' . 'type_id_new_' . $vn_c,
                                    pInteger
                                );
                                $this->addLabel(
                                    $va_label_values,
        ...
        if ($this->getProperty('LABEL_TABLE_NAME') && isset($va_fields_by_type['nonpreferred_label']) && is_array(
                $va_fields_by_type['nonpreferred_label']
            )) {
            if (!$vb_batch) {
                foreach ($va_fields_by_type['nonpreferred_label'] as $vs_placement_code => $vs_f) {
                    ...
                    foreach ($va_nonpreferred_labels as $vn_item_id => $va_labels_by_locale) {
                        foreach ($va_labels_by_locale as $vn_locale_id => $va_label_list) {
                            foreach ($va_label_list as $va_label) {
                                if ($vn_label_locale_id = $po_request->getParameter(
                                    $vs_placement_code . $vs_form_prefix . '_NPref' . 'locale_id_' . $va_label['label_id'],
                                    pString
                                )) {
                                    $vn_label_type_id = $po_request->getParameter(
                                        $vs_placement_code . $vs_form_prefix . '_NPref' . 'type_id_' . $va_label['label_id'],
                                        pInteger
                                    );
                                    if (is_array(
                                        $va_label_values = $this->getLabelUIValuesFromRequest(
                                    ...
                                    if ($po_request->getParameter(
                                        $vs_placement_code . $vs_form_prefix . '_NPrefLabel_' . $va_label['label_id'] . '_delete',
                                        pString
                                    )) {
                                        // delete
                                        $this->removeLabel($va_label['label_id'], array('queueIndexing' => true));
            ...
            foreach ($va_fields_by_type['nonpreferred_label'] as $vs_placement_code => $vs_f) {
                if ($vb_batch) {
                    $vs_batch_mode = $po_request->getParameter(
                        $vs_placement_code . $vs_form_prefix . '_NPref_batch_mode',
                        pString
                    );
                ...
                foreach ($_REQUEST as $vs_key => $vs_value) {
                    if (!preg_match(
                        '/^' . $vs_placement_code . $vs_form_prefix . '_NPref' . 'locale_id_new_([\d]+)/',
                        $vs_key,
                        $va_matches
                    )) {
                        continue;
                    ...
                    $vn_c = intval($va_matches[1]);
                    if ($vn_new_label_locale_id = $po_request->getParameter(
                        $vs_placement_code . $vs_form_prefix . '_NPref' . 'locale_id_new_' . $vn_c,
                        pString
                    )) {
                        if (is_array(
                            $va_label_values = $this->getLabelUIValuesFromRequest(
                            ...
                            $vn_new_label_type_id = $po_request->getParameter(
                                $vs_placement_code . $vs_form_prefix . '_NPref' . 'type_id_new_' . $vn_c,
                                pInteger
                            );
                            $this->addLabel(
                                $va_label_values,
                                $vn_new_label_locale_id,
                                $vn_new_label_type_id,
                                false,
                                array('queueIndexing' => true)
                            );
            ...
            foreach ($va_fields_by_type['related_table'] as $vs_placement_code => $vs_f) {
                ...
                switch ($vs_f) {
                    ...
                    case 'ca_object_representations':
                        ...
                        $vb_allow_fetching_of_urls = (bool)$this->_CONFIG->get(
                            'allow_fetching_of_media_from_remote_urls'
                        );
                        $vb_allow_existing_rep = (bool)$this->_CONFIG->get(
                            'ca_objects_allow_relationships_to_existing_representations'
                        );
                        $va_rep_ids_sorted = $va_rep_sort_order = explode(
                            ';',
                            $po_request->getParameter(
                                $vs_prefix_stub . 'ObjectRepresentationBundleList',
                                pString
                            )
                        );
                        ...
                        if (!$vb_batch && is_array($va_reps)) {
                            ...
                            $bundles_on_screen_proc = array_map(
                                function ($v) {
                            ...
                            foreach ($va_reps as $vn_i => $va_rep) {
                                ...
                                if (is_array($bundles_on_screen_proc) && sizeof($bundles_on_screen_proc)) {
                                    ...
                                    if (is_array($bundles_on_screen_proc)) {
                                        foreach ($bundles_on_screen_proc as $b) {
                                            ...
                                            if (!is_null(
                                                $po_request->parameterExists(
                                                    $vs_prefix_stub . $f . '_' . $va_rep['relation_id']
                                                )
                                            )) {
                                                $vals[$f] = $po_request->getParameter(
                                                    $vs_prefix_stub . $f . '_' . $va_rep['relation_id'],
                                                    pString
                                                );
                                            }
                                    ...
                                    $vn_center_x = $po_request->getParameter(
                                        $vs_prefix_stub . 'center_x_' . $va_rep['relation_id'],
                                        pString
                                    );
                                    $vn_center_y = $po_request->getParameter(
                                        $vs_prefix_stub . 'center_y_' . $va_rep['relation_id'],
                                        pString
                                    );

                                    $vs_rep_label = $po_request->getParameter(
                                        $vs_prefix_stub . 'rep_label_' . $va_rep['relation_id'],
                                        pString
                                    );

                                    $vn_rank = null;
                                    if (($vn_rank_index = array_search(
                                            $va_rep['relation_id'],
                                            $va_rep_sort_order
                                        )) !== false) {
                                    ...
                                    $vn_rel_type_id = $po_request->getParameter(
                                        $vs_prefix_stub . 'rel_type_id_' . $va_rep['relation_id'],
                                        pString
                                    );
                                    ...
                                    if ($this->numErrors()) {
                                        ...
                                        $vn_object_representation_mapping_id = $po_request->getParameter(
                                            $vs_prefix_stub . 'importer_id_' . $va_rep['relation_id'],
                                            pInteger
                                        );

                                        $log = caGetImportLogger(
                                            [
                                                'logLevel' => $po_request->getAppConfig()->get(
                                                    'embedded_metadata_extraction_mapping_log_level'
                                                )
                                            ]
                                        );
                                        if (!$vn_object_representation_mapping_id && is_array(
                                            ...
                                            foreach ($media_metadata_extraction_defaults as $m => $importer_code) {
                                                if (caCompareMimetypes($media_mimetype, $m)) {
                                                    if (!($vn_object_representation_mapping_id = ca_data_importers::find(
                                                        ...
                                                        if ($log) {
                                                            $log->logInfo(
                                                                _t(
                                                                    'Could not find embedded metadata importer with code %1',
                                                                    $importer_code
                                                                )
                                                            );
                                                        }
                                        ...
                                        if ($vn_object_representation_mapping_id && ($t_mapping = ca_data_importers::find(
                                                ['importer_id' => $vn_object_representation_mapping_id],
                                            ...
                                            if ($log) {
                                                $log->logDebug(
                                                    _t(
                                                        'Using embedded media mapping %1 (format %2)',
                                                        $t_mapping->get('importer_code'),
                                                        $format
                                                    )
                                                );
                                            }
                                            ...
                                            $t_importer->importDataFromSource(
                                                $t_rep->getMediaPath('media', 'original'),
                                ...
                                $this->clearErrors();
                                if (($po_request->getParameter(
                                        $vs_prefix_stub . $va_rep['relation_id'] . '_delete',
                                        pInteger
                                    )) > 0) {
                                    // delete!
                        ...
                        if ($vb_batch) {
                            ...
                            if (($vs_batch_mode == '_replace_') && sizeof($va_reps)) {
                                foreach ($va_reps as $va_rep) {
                                    $pn_access = $po_request->getParameter(
                                        "{$vs_prefix_stub}access_new_0_enabled",
                                        pInteger
                                    ) ? $po_request->getParameter("{$vs_prefix_stub}access_new_0", pInteger) : null;
                                    $pn_status = $po_request->getParameter(
                                        "{$vs_prefix_stub}status_new_0_enabled",
                                        pInteger
                                    ) ? $po_request->getParameter("{$vs_prefix_stub}status_new_0", pInteger) : null;
                                    $pn_type_id = $po_request->getParameter(
                                        "{$vs_prefix_stub}rep_type_id_new_0_enabled",
                                        pInteger
                                    ) ? $po_request->getParameter(
                                        "{$vs_prefix_stub}rep_type_id_new_0",
                                        pInteger
                                    ) : null;
                                    $ps_rep_label = $po_request->getParameter(
                                        "{$vs_prefix_stub}rep_label_new_0_enabled",
                                        pInteger
                                    ) ? trim(
                                        $po_request->getParameter("{$vs_prefix_stub}rep_label_new_0", pString)
                                    ) : null;
                                    $pn_is_transcribable = $po_request->getParameter(
                                        "{$vs_prefix_stub}is_transcribable_new_0_enabled",
                                        pInteger
                                    );
                                    $this->editRepresentation(
                                        $va_rep['representation_id'],
                                        null,
                                        $g_ui_locale_id,
                                        $pn_status,
                                        $pn_access,
                                        null,
                                        ['is_transcribable' => $pn_is_transcribable],
                            ...
                            foreach ($_REQUEST as $vs_key => $vs_value) {
                                if (preg_match(
                                    '/^' . $vs_prefix_stub . 'media_url_new_([\d]+)$/',
                                    $vs_key,
                                    $va_matches
                                )) {
                                    ...
                                    '/^' . $vs_prefix_stub . 'autocompletenew_([\d]+)$/',
                                    $vs_key,
                                    $va_matches
                                )) {
                                    $va_file_list[$vs_key] = array(
                                    ...
                                    '/^' . $vs_prefix_stub . 'mediarefsnew_([\d]+)$/',
                                    $vs_key,
                                    $va_matches
                                )) {
                            ...
                            foreach ($va_file_list as $vs_key => $va_values) {
                                ...
                                if (
                                    ...
                                    !preg_match(
                                        '/^' . $vs_prefix_stub . 'mediarefsnew_([\d]+)_([\d]+)$/',
                                        $vs_key,
                                        $va_matches
                                    ) &&
                                    (($vb_allow_fetching_of_urls && !preg_match(
                                                '/^' . $vs_prefix_stub . 'media_url_new_([\d]+)$/',
                                                $vs_key,
                                                $va_matches
                                            )) || !$vb_allow_fetching_of_urls) &&
                                    (($vb_allow_existing_rep && !preg_match(
                                                '/^' . $vs_prefix_stub . 'autocompletenew_([\d]+)$/',
                                                $vs_key,
                                                $va_matches
                                ...
                                if ($vs_upload_type = $po_request->getParameter(
                                    $vs_prefix_stub . 'upload_typenew_' . $va_matches[1],
                                    pString
                                )) {
                                ...
                                if ($vs_upload_type === "search") {
                                    $vn_type_id = $po_request->getParameter(
                                        $vs_prefix_stub . 'rep_type_id_new_' . $va_matches[1],
                                        pInteger
                                    );
                                } else {
                                    $vn_type_id = $po_request->getParameter(
                                        $vs_prefix_stub . 'rel_type_id_new_' . $va_matches[1],
                                        pInteger
                                    );
                                }

                                if ($vn_existing_rep_id = $po_request->getParameter(
                                    $vs_prefix_stub . 'idnew_' . $va_matches[1],
                                    pInteger
                                )) {
                                    ...
                                    if ($vb_allow_fetching_of_urls && ($vs_path = $va_values['url'])) {
                                        ...
                                        "!^userMedia" . $po_request->getUserID() . "!",
                                        $va_values['tmp_name']
                                    )) {
                                        if (!is_writeable(
                                            $vs_tmp_directory = $po_request->config->get(
                                                'ajax_media_upload_tmp_directory'
                                            )
                                        )) {
                                    ...
                                    if (!$vn_rep_type_id && !($vn_rep_type_id = caGetDefaultItemID(
                                            'object_representation_types'
                                        ))) {
                                        require_once(__CA_MODELS_DIR__ . '/ca_lists.php');
                                        $t_list = new ca_lists();
                                        if (is_array(
                                            $va_rep_type_ids = $t_list->getItemsForList(
                                                'object_representation_types',
                                    ...
                                    if (is_array(
                                        ...
                                        ) && isset($pa_options['existingRepresentationMap'][$vs_path]) && $pa_options['existingRepresentationMap'][$vs_path]) {
                                        $this->addRelationship(
                                            'ca_object_representations',
                                    ...
                                    if (is_array($bundles_on_screen_proc)) {
                                        foreach ($bundles_on_screen_proc as $b) {
                                            $f = array_pop(explode('.', $b));
                                            if ($v = $po_request->getParameter(
                                                $vs_prefix_stub . $f . '_new_' . $va_matches[1],
                                                pString
                                            )) {
                                                $vals[$f] = $v;
                                            }
                                    ...
                                    // Get user-specified center point (images only)
                                    $vn_center_x = $po_request->getParameter(
                                        $vs_prefix_stub . 'center_x_new_' . $va_matches[1],
                                        pString
                                    );
                                    $vn_center_y = $po_request->getParameter(
                                        $vs_prefix_stub . 'center_y_new_' . $va_matches[1],
                                        pString
                                    );
                                    ...
                                    if ($this->numErrors()) {
                                        ...
                                        // import embedded metadata?
                                        $vn_object_representation_mapping_id = $po_request->getParameter(
                                            $vs_prefix_stub . 'importer_id_new_' . $va_matches[1],
                                            pInteger
                                        );
            ...
            foreach ($va_fields_by_type['special'] as $vs_placement_code => $vs_f) {
                ...
                switch ($vs_f) {
                    ...
                    case 'ca_representation_annotation_properties':
                        ...
                        foreach ($this->getPropertyList() as $vs_property) {
                            $this->setPropertyValue(
                                $vs_property,
                                $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}{$vs_property}",
                                    pString
                                )
                            );
                        }
                    ...
                    case 'ca_sets_checklist':
                        ...
                        $t_set = new ca_sets();
                        if (!$vb_batch) {
                            $va_sets = caExtractValuesByUserLocale(
                                $t_set->getSetsForItem(
                                    $this->tableNum(),
                                    $this->getPrimaryKey(),
                            ...
                            foreach ($va_sets as $vn_set_id => $va_set_info) {
                                ...
                                if ($po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_set_id_{$vn_item_id}_delete",
                                    pString
                                )) {
                                    // delete
                        ...
                        foreach ($_REQUEST as $vs_key => $vs_value) {
                            if (!preg_match(
                                "/{$vs_placement_code}{$vs_form_prefix}_set_id_new_([\d]+)/",
                                $vs_key,
                                $va_matches
                            )) {
                                continue;
                            ...
                            $vn_c = intval($va_matches[1]);
                            if ($vn_new_set_id = $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}_set_id_new_{$vn_c}",
                                pString
                            )) {
                    ...
                    case 'ca_set_items':
                        ...
                        $va_rids = explode(
                            ';',
                            $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}setRowIDList",
                                pString
                            )
                        );
                    ...
                    case 'ca_editor_ui_screens':
                        ...
                        $va_screen_ids = explode(
                            ';',
                            $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}_ScreenBundleList",
                                pString
                            )
                        );
                        ...
                        foreach ($_REQUEST as $vs_key => $vs_val) {
                            ...
                            if (preg_match(
                                "!^{$vs_placement_code}{$vs_form_prefix}_name_new_([\d]+)$!",
                                $vs_key,
                                $va_matches
                            )) {
                                if (!($t_screen = $this->addScreen(
                                    $vs_val,
                                    $g_ui_locale_id,
                            ...
                            if (preg_match(
                                "!^{$vs_placement_code}{$vs_form_prefix}_([\d]+)_delete$!",
                                $vs_key,
                                $va_matches
                            )) {
                    ...
                    case 'ca_tour_stops_list':
                        ...
                        $va_stop_ids = explode(
                            ';',
                            $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}_StopBundleList",
                                pString
                            )
                        );
                        ...
                        foreach ($_REQUEST as $vs_key => $vs_val) {
                            ...
                            if (preg_match(
                                ...
                                if (!($t_stop = $this->addStop(
                                    $vs_val,
                                    $vn_type_id,
                                    $g_ui_locale_id,
                                    mb_substr(
                                        preg_replace('![^A-Za-z0-9_]+!', '_', $vs_val),
                                        0,
                                        255
                                    )
                                ))) {
                    ...
                    case 'ca_user_groups':
                        ...
                        if (!$po_request->user->canDoAction('is_administrator') && ($po_request->getUserID(
                                ) != $this->get('user_id'))) {
                            break;
                        ...
                        foreach ($_REQUEST as $vs_key => $vs_val) {
                            if (preg_match("!^{$vs_placement_code}{$vs_form_prefix}_id(.*)$!", $vs_key, $va_matches)) {
                                $vs_effective_date = $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_effective_date_" . $va_matches[1],
                                    pString
                                );
                                $vn_group_id = (int)$po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_id" . $va_matches[1],
                                    pInteger
                                );
                                $vn_access = $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_access_" . $va_matches[1],
                                    pInteger
                                );
                                if ($vn_access >= 0) {
                    ...
                    case 'ca_users':
                        ...
                        foreach ($_REQUEST as $vs_key => $vs_val) {
                            if (preg_match("!^{$vs_placement_code}{$vs_form_prefix}_id(.*)$!", $vs_key, $va_matches)) {
                                ...
                                $vn_user_id = (int)$po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_id" . $va_matches[1],
                                    pInteger
                                );
                                $vn_access = $po_request->getParameter(
                    ...
                    case 'ca_user_roles':
                        ...
                        foreach ($_REQUEST as $vs_key => $vs_val) {
                            if (preg_match(
                                "!^{$vs_placement_code}{$vs_form_prefix}_access_([\d]+)$!",
                                $vs_key,
                                $va_matches
                            )) {
                    ...
                    case 'ca_object_representations_media_display':
                        ...
                        if ($vb_use_options = (bool)$po_request->getParameter(
                            "{$vs_placement_code}{$vs_form_prefix}_derivative_options_selector",
                            pInteger
                        )) {
                            // update only specified versions
                            $va_versions_to_process = $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}_set_versions",
                                pArray
                            );
                        }
                        ...
                        if ($vb_use_options && ($po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_derivative_options_mode",
                                    pString
                                ) == 'timecode')) {
                            // timecode
                            if (!(string)($vs_timecode = $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}_derivative_options_mode_timecode_value",
                                pString
                            ))) {
                                $vs_timecode = "1s";
                            }
                            ...
                            if ($o_media->read($this->getMediaPath('media', 'original'))) {
                                $va_files = $o_media->writePreviews(
                                    array(
                                        'force' => true,
                            ...
                            if ($vb_use_options && ($po_request->getParameter(
                                ...
                                if (!(int)($vn_page = $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_derivative_options_mode_page_value",
                                    pInteger
                                ))) {
                                    $vn_page = 1;
                                }
                                ...
                                if (($vs_media_url = trim($po_request->getParameter($vs_key, pString))) && isURL(
                                        $vs_media_url
                                    )) {
                        ...
                        if ($this->changed('media')) {
                            ...
                            if ($this->numErrors()) {
                                $po_request->addActionErrors(
                                    $this->errors(),
                                    'ca_object_representations_media_display',
                                    'general'
                                );
                            }
                        }
                    ...
                    case 'ca_object_representation_captions':
                        ...
                        foreach ($_REQUEST as $vs_key => $vs_val) {
                            ...
                            if (preg_match(
                                "!^{$vs_placement_code}{$vs_form_prefix}_locale_id(.*)$!",
                                $vs_key,
                                $va_matches
                            )) {
                                $vn_locale_id = (int)$po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_locale_id" . $va_matches[1],
                                    pInteger
                                );
                                $this->addCaptionFile(
                                    $_FILES["{$vs_placement_code}{$vs_form_prefix}_caption_file" . $va_matches[1]]['tmp_name'],
                                    $vn_locale_id,
                                    array('originalFilename' => $_FILES["{$vs_placement_code}{$vs_form_prefix}_caption_file" . $va_matches[1]]['name'])
                                );
                            }
                        }
                    ...
                    case 'ca_object_representation_chooser':
                        ...
                        if ($vs_element_code = caGetOption(
                            array('elementCode', 'element_code'),
                            $va_bundle_settings,
                            null
                        )) {
                            if (!is_array(
                                $va_current_rep_ids = $this->get(
                                    $this->tableName() . "." . $vs_element_code,
                                    array('returnAsArray' => true, 'idsOnly' => true)
                                )
                            )) {
                                $va_current_rep_ids = $va_current_rep_id_with_structure = array();
                            } else {
                                $va_current_rep_id_with_structure = $this->get(
                                    $this->tableName() . "." . $vs_element_code,
                    ...
                    case 'history_tracking_chronology':
                        ...
                        if (!caGetOption('hide_value_delete', $va_bundle_settings, false)) {
                            // handle deletes
                            $refs = $this->getHistoryReferences(
                                ['policy' => $policy]
                            );    // get all references present in this history
                            foreach ($refs as $t => $r) {
                                if (is_array(
                                    $rp = $po_request->getParameter(
                                        "{$vs_placement_code}{$vs_form_prefix}_delete_{$t}",
                                        pArray
                                    )
                                )) {
                                    if (is_array(
                                        ...
                                        foreach ($rows_to_delete as $row_id) {
                                            ...
                                            } elseif (($t === 'ca_object_lots') && ($this->tableName(
                                                    ) === 'ca_objects')) {
                                            ...
                                            } elseif (($t === 'ca_objects') && ($this->tableName(
                                                    ) === 'ca_objects') && ($this->get('is_deaccessioned'))) {
                        ...
                        if (!caGetOption('hide_update_location_controls', $va_bundle_settings, false) || !caGetOption(
                                'hide_return_to_home_location_controls',
                                $va_bundle_settings,
                                false
                            )) {
                            // set storage location
                            if ($this->hasField('home_location_id') && $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_ca_storage_locations_return_homenew_0",
                                    pInteger
                                ) == 1) {
                                $vn_location_id = $this->get('home_location_id');
                            } else {
                                $vn_location_id = $vn_location_id = $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_ca_storage_locations_idnew_0",
                                    pInteger
                                );
                            }
                            ...
                            if ($vn_location_id) {
                                ...
                                if ($t_loc->load($vn_location_id)) {
                                    if ($policy) {
                                        $policy_info = $table::getHistoryTrackingCurrentValuePolicyElement(
                                            $policy,
                                            'ca_storage_locations',
                                            $t_loc->getTypeCode()
                                        );
                                        $vn_relationship_type_id = caGetOption(
                                    ...
                                    if (!$vn_relationship_type_id) {
                                        $vn_relationship_type_id = $this->getAppConfig()->get(
                                    ...
                                    if ($vn_relationship_type_id) {
                                        ...
                                        $vs_effective_date = $po_request->getParameter(
                                            "{$vs_placement_code}{$vs_form_prefix}_ca_storage_locations__effective_datenew_0",
                                            pString
                                        );

                                        $t_item_rel = $this->addRelationship(
                                            'ca_storage_locations',
                                            $vn_location_id,
                                            $vn_relationship_type_id,
                                            $vs_effective_date
                                        );
                                        if ($this->numErrors()) {
                                            $po_request->addActionErrors($this->errors(), $vs_f, 'general');
                                        } else {
                                            ca_storage_locations::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm(
                                                $po_request,
                                                $vs_placement_code,
                                                $vs_form_prefix . '_ca_storage_locations',
                                                $t_item_rel,
                                                $vn_location_id,
                                        ...
                                        if ($t_item_rel) {
                                            SearchResult::clearResultCacheForRow(
                        ...
                        if (!caGetOption('hide_add_to_loan_controls', $va_bundle_settings, false)) {
                            // set loan
                            if ($vn_loan_id = $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}_ca_loans_idnew_0",
                                pInteger
                            )) {
                                if ($vn_loan_type_id = $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_ca_loans_type_idnew_0",
                                    pInteger
                                )) {
                                    ...
                                    if ($this->numErrors()) {
                                        ...
                                        ca_loans::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm(
                                            $po_request,
                                            $vs_placement_code,
                                            $vs_form_prefix . '_ca_loans',
                                            $t_item_rel,
                                            $vn_loan_id,
                                            $processed_bundle_settings
                                        );
                        ...
                        if (!caGetOption('hide_add_to_movement_controls', $va_bundle_settings, false)) {
                            // set movement
                            if ($vn_movement_id = $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}_ca_movements_idnew_0",
                                pInteger
                            )) {
                                if ($vn_movement_type_id = $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_ca_movements_type_idnew_0",
                                    pInteger
                                )) {
                                    $t_item_rel = $this->addRelationship(
                                        'ca_movements',
                                    ...
                                    if ($this->numErrors()) {
                                        ...
                                        ca_movements::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm(
                                            $po_request,
                                            $vs_placement_code,
                                            $vs_form_prefix . '_ca_movements',
                                            $t_item_rel,
                                            $vn_movement_id,
                                            $processed_bundle_settings
                                        );
                        ...
                        if (!caGetOption('hide_add_to_occurrence_controls', $va_bundle_settings, false)) {
                            ...
                            foreach ($va_occ_types as $vn_type_id => $vn_type_info) {
                                if ($vn_occurrence_id = $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_ca_occurrences_{$vn_type_id}_idnew_0",
                                    pInteger
                                )) {
                                    if ($vn_occ_type_id = $po_request->getParameter(
                                        "{$vs_placement_code}{$vs_form_prefix}_ca_occurrences_{$vn_type_id}_type_idnew_0",
                                        pInteger
                                    )) {
                                        $t_item_rel = $this->addRelationship(
                                            'ca_occurrences',
                                        ...
                                        if ($this->numErrors()) {
                                            ...
                                            ca_occurrences::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm(
                                                $po_request,
                                                $vs_placement_code,
                                                $vs_form_prefix . '_ca_occurrences',
                                                $t_item_rel,
                                                $vn_occurrence_id,
                                                $processed_bundle_settings
                                            );
                        ...
                        if (!caGetOption('hide_add_to_collection_controls', $va_bundle_settings, false)) {
                            ...
                            foreach ($va_coll_types as $vn_type_id => $vn_type_info) {
                                if ($vn_collection_id = $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_ca_collections_{$vn_type_id}_idnew_0",
                                    pInteger
                                )) {
                                    if ($vn_coll_type_id = $po_request->getParameter(
                                        "{$vs_placement_code}{$vs_form_prefix}_ca_collections_{$vn_type_id}_type_idnew_0",
                                        pInteger
                                    )) {
                                        $t_item_rel = $this->addRelationship(
                                            'ca_collections',
                                        ...
                                        if ($this->numErrors()) {
                                            ...
                                            ca_collections::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm(
                                                $po_request,
                                                $vs_placement_code,
                                                $vs_form_prefix . '_ca_collections',
                                                $t_item_rel,
                                                $vn_collection_id,
                                                $processed_bundle_settings
                                            );
                        ...
                        if (!caGetOption('hide_add_to_entity_controls', $va_bundle_settings, false)) {
                            ...
                            foreach ($va_entity_types as $vn_type_id => $vn_type_info) {
                                if ($vn_entity_id = $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_ca_entities_{$vn_type_id}_idnew_0",
                                    pInteger
                                )) {
                                    if ($vn_entity_type_id = $po_request->getParameter(
                                        "{$vs_placement_code}{$vs_form_prefix}_ca_entities_{$vn_type_id}_type_idnew_0",
                                        pInteger
                                    )) {
                                        $t_item_rel = $this->addRelationship(
                                            'ca_entities',
                                        ...
                                        if ($this->numErrors()) {
                                            ...
                                            ca_entities::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm(
                                                $po_request,
                                                $vs_placement_code,
                                                $vs_form_prefix . '_ca_entities',
                                                $t_item_rel,
                                                $vn_entity_id,
                                                $processed_bundle_settings
                                            );
                        ...
                        if (!caGetOption('hide_add_to_object_controls', $va_bundle_settings, false)) {
                            // set object
                            if ($vn_object_id = $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}_ca_objects_idnew_0",
                                pInteger
                            )) {
                                if ($vn_object_type_id = $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_ca_objects_type_idnew_0",
                                    pInteger
                                )) {
                                    $t_item_rel = $this->addRelationship(
                                        'ca_objects',
                                    ...
                                    if ($this->numErrors()) {
                                        ...
                                        ca_objects::setHistoryTrackingChronologyInterstitialElementsFromHTMLForm(
                                            $po_request,
                                            $vs_placement_code,
                                            $vs_form_prefix . '_ca_objects',
                                            $t_item_rel,
                                            $vn_object_id,
                                            $processed_bundle_settings
                                        );
                    ...
                    case 'ca_objects_deaccession':        // object deaccession information
                        ...
                        $this->set(
                            'is_deaccessioned',
                            $vb_is_deaccessioned = $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}is_deaccessioned",
                                pInteger
                            )
                        );
                        $this->set(
                            'deaccession_notes',
                            $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}deaccession_notes",
                                pString
                            )
                        );
                        $this->set(
                            'deaccession_authorized_by',
                            $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}deaccession_authorized_by",
                                pString
                            )
                        );
                        $this->set(
                            'deaccession_type_id',
                            $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}deaccession_type_id",
                                pString
                            )
                        );
                        ...
                        $this->set(
                            'deaccession_date',
                            $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}deaccession_date",
                                pString
                            )
                        );
                        if ($po_request->config->get('deaccession_use_disposal_date')) {
                            $this->set(
                                ...
                                $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}deaccession_disposal_date",
                                    pString
                                )
                            );
                        }
                        if ($vb_is_deaccessioned && (bool)$this->getAppConfig()->get(
                                'deaccession_force_access_private'
                            )) {
                            $this->get('access', 0);
                    ...
                    case 'ca_object_checkouts':        // object checkout information
                        ...
                        foreach ($_REQUEST as $k => $v) {
                            if (preg_match(
                                "!^{$vs_placement_code}{$vs_form_prefix}(checkout|return)_notes_([\d]+)$!",
                                $k,
                                $m
                            )) {
                                $edits[$m[2]][$m[1]] = $v;
                        ...
                        foreach ($edits as $checkout_id => $data) {
                            if ($tc = ca_object_checkouts::find(
                    ...
                    case 'ca_object_circulation_status':
                        ...
                        $this->set(
                            ...
                            $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}ca_object_circulation_status",
                                pInteger
                            )
                        );
                        break;
                    ...
                    case 'ca_object_representations_access_status':
                        if (($vb_batch) && (is_a(
                                $this,
                                'RepresentableBaseModel'
                            )) && ($vs_batch_mode = $_REQUEST[$vs_prefix_stub . 'batch_mode']) && ($vs_batch_mode === '_replace_')) {
                            $vn_access = $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}_access",
                                pString
                            );
                            $vn_status = $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}_status",
                                pString
                            );
                    ...
                    case 'ca_site_pages_content':
                        if (is_array($va_field_list = $this->getHTMLFormElements())) {
                            ...
                            foreach ($va_field_list as $vs_field => $va_element_info) {
                                $vs_value = $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_{$va_element_info['code']}",
                                    pString
                                );
                    ...
                    case 'ca_site_page_media':
                        ...
                        $va_media_ids_sorted = $va_mediasort_order = explode(
                            ';',
                            $po_request->getParameter(
                                $vs_prefix_stub . 'MediaBundleList',
                                pString
                            )
                        );
                        ...
                        if (is_array($va_media_ids_sorted)) {
                            foreach ($va_media_list as $vn_i => $va_media) {
                                ...
                                if (strlen(
                                        $po_request->getParameter(
                                            $vs_prefix_stub . 'access_' . $va_media['media_id'],
                                            pInteger
                                        )
                                    ) > 0) {
                                    ...
                                    $vs_idno = $po_request->getParameter(
                                        $vs_prefix_stub . 'idno_' . $va_media['media_id'],
                                        pString
                                    );
                                    $vn_access = $po_request->getParameter(
                                        $vs_prefix_stub . 'access_' . $va_media['media_id'],
                                    ...
                                    $vs_title = trim(
                                        $po_request->getParameter(
                                            $vs_prefix_stub . 'title_' . $va_media['media_id'],
                                            pString
                                        )
                                    );
                                    $vs_caption = trim(
                                        $po_request->getParameter(
                                            $vs_prefix_stub . 'caption_' . $va_media['media_id'],
                                            pString
                                        )
                                    );
                                    ...
                                    $this->editMedia(
                                        ...
                                        $vs_idno,
                                        $vn_access,
                                        ['original_filename' => $vs_original_name, 'rank' => $vn_rank]
                                    );
                                    if ($this->numErrors()) {
                                        foreach ($this->errors() as $o_e) {
                                            switch ($o_e->getErrorNumber()) {
                                    ...
                                    $this->clearErrors();
                                    if (($po_request->getParameter(
                                            $vs_prefix_stub . $va_media['media_id'] . '_delete',
                                            pInteger
                                        )) > 0) {
                        ...
                        foreach ($va_file_list as $vs_key => $va_values) {
                            ...
                            $vs_title = trim(
                                $po_request->getParameter($vs_prefix_stub . 'title_new_' . $va_matches[1], pString)
                            );
                            $vs_caption = $po_request->getParameter(
                                $vs_prefix_stub . 'caption_new_' . $va_matches[1],
                                pString
                            );
                            $vs_idno = $po_request->getParameter(
                                $vs_prefix_stub . 'idno_new_' . $va_matches[1],
                                pString
                            );
                            $vn_access = $po_request->getParameter(
                                $vs_prefix_stub . 'access_new_' . $va_matches[1],
                                pInteger
                            );
                            $this->addMedia(
                                $vs_path,
                    ...
                    case 'ca_item_tags':
                        ...
                        if (!$vb_batch || ($vb_batch && in_array($vs_batch_mode, ['_add_', '_replace_']))) {
                            foreach ($_REQUEST as $vs_key => $vs_val) {
                                ...
                                if (preg_match(
                                    "!^{$vs_placement_code}{$vs_form_prefix}_autocompletenew_([\d]+)$!",
                                    $vs_key,
                                    $va_matches
                                )) {
                        ...
                        if (!$vb_batch && is_array(
                                $ids_sorted = $va_rel_sort_order = explode(
                                    ';',
                                    $po_request->getParameter(
                                        "{$vs_placement_code}{$vs_form_prefix}BundleList",
                                        pString
                                    )
                                )
                            )) {
        ...
        foreach ($va_bundles as $va_bundle) {
            ...
            if (!Datamodel::getInstanceByTableName($vs_bundle_name, true) && !preg_match(
                    "!^" . $this->tableName() . "\.!",
                    $vs_bundle_name
                )) {
        ...
        try {
            ...
            if (sizeof($va_violations)) {
                if ($vb_we_set_transaction && isset($va_violations['ERR']) && is_array(
                        $va_violations['ERR']
                            ...
                            $po_request->addActionErrors(
                                array(
                                    new ApplicationError(
                                        1100,
                                        caExtractSettingsValueByUserLocale(
                                            'violationMessage',
                                            $va_rule['rule_settings']
                                        ),
                                        "BundlableLabelableBaseModelWithAttributes->saveBundlesForScreen()",
                                        'MetadataDictionary',
                                        false,
            ...
            $po_request->addActionErrors(
                array(
                    new ApplicationError(
                        1100,
                        _t('Invalid rule expression: %1', $e->getMessage()),
                        "BundlableLabelableBaseModelWithAttributes->saveBundlesForScreen()",
                        'MetadataDictionary',
                        false,
                        false
    ...
    private function _processRelated(
        ...
        $va_rel_ids_sorted = $va_rel_sort_order = explode(
            ';',
            $po_request->getParameter(
                "{$ps_placement_code}{$ps_form_prefix}BundleList",
                pString
            )
        );
        ...
        if (!$vb_batch) {
            foreach ($va_rel_items as $va_rel_item) {
                ...
                $vn_rank = null;
                if ((($vn_rank_index = array_search(
                            $va_rel_item['relation_id'],
                            $va_rel_sort_order
                        )) !== false) && (!isset($pa_settings['disableSorts']) || !$pa_settings['disableSorts'])) {
                    $vn_rank = $va_rel_ids_sorted[$vn_rank_index];
                }

                $this->clearErrors();
                $vn_id = $po_request->getParameter(
                    "{$ps_placement_code}{$ps_form_prefix}_id" . $va_rel_item[$vs_key],
                    pString
                );
                if ($vn_id) {
                    $vn_type_id = $po_request->getParameter(
                        "{$ps_placement_code}{$ps_form_prefix}_type_id" . $va_rel_item[$vs_key],
                        pString
                    );
                    $vs_direction = null;
                    ...
                    $this->clearErrors();
                    if (($po_request->getParameter(
                            "{$ps_placement_code}{$ps_form_prefix}_" . $va_rel_item[$vs_key] . '_delete',
                            pInteger
                        )) > 0) {
        ...
        foreach ($_REQUEST as $vs_key => $vs_value) {
            if (preg_match("/^{$ps_placement_code}{$ps_form_prefix}_idnew_([\d]+)/", $vs_key, $va_matches)) {
                $vn_c = intval($va_matches[1]);
                if ($vn_new_id = $po_request->getParameter(
                    "{$ps_placement_code}{$ps_form_prefix}_idnew_{$vn_c}",
                    pString
                )) {
                    $vn_new_type_id = $po_request->getParameter(
                        "{$ps_placement_code}{$ps_form_prefix}_type_idnew_{$vn_c}",
                        pString
                    );
            ...
            if ($ps_bundle_name == 'ca_list_items') {
                if (preg_match("/^{$ps_placement_code}{$ps_form_prefix}_item_id_new_([\d]+)/", $vs_key, $va_matches)) {
                    if ($vn_rel_type_id = $po_request->getParameter(
                        "{$ps_placement_code}{$ps_form_prefix}_type_idchecklist",
                        pInteger
                    )) {
                ...
                if (preg_match(
                    "/^{$ps_placement_code}{$ps_form_prefix}_item_id_([\d]+)_delete/",
                    $vs_key,
                    $va_matches
                )) {
        ...
        if ($vn_min_relationships && ($vn_total_rel_count < $vn_min_relationships)) {
            $po_request->addActionErrors(
                array(
                    new ApplicationError(
                        2590,
                        ($vn_min_relationships == 1) ? _t(
                            'There must be at least %1 relationship for %2',
                            $vn_min_relationships,
                            Datamodel::getTableProperty($ps_bundle_name, 'NAME_PLURAL')
                        ) : _t(
                            'There must be at least %1 relationships for %2',
                            $vn_min_relationships,
                            Datamodel::getTableProperty($ps_bundle_name, 'NAME_PLURAL')
                        ),
                        'BundleableLabelableBaseModelWithAttributes::_processRelated()',
                        null,
                        null,
                        false,
                        false
        ...
        if ($vn_max_relationships && ($vn_total_rel_count > $vn_max_relationships)) {
            $po_request->addActionErrors(
                array(
                    new ApplicationError(
                        2590,
                        ($vn_max_relationships == 1) ? _t(
                            'There must be no more than %1 relationship for %2',
                            $vn_max_relationships,
                            Datamodel::getTableProperty($ps_bundle_name, 'NAME_PLURAL')
                        ) : _t(
                            'There must be no more than %1 relationships for %2',
                            $vn_max_relationships,
                            Datamodel::getTableProperty($ps_bundle_name, 'NAME_PLURAL')
                        ),
                        'BundleableLabelableBaseModelWithAttributes::_processRelated()',
        ...
        foreach ($va_rels_to_add as $va_rel_to_add) {
            $this->addRelationship(
                $va_rel_to_add['bundle'],
                $va_rel_to_add['row_id'],
                $va_rel_to_add['type_id'],
                _t(caGetOption('effectiveDateDefault', $pa_settings, null)),
                null,
                $va_rel_to_add['direction']
            );
            if ($this->numErrors()) {
                $po_request->addActionErrors($this->errors(), $ps_bundle_name);
            }
    ...
    {
        if (is_array(
            $va_related_ids = $this->getRelatedItems(
                $pm_rel_table_name_or_num,
    ...
    public function getRelatedItems($pm_rel_table_name_or_num, $pa_options = null, &$pn_count = null)
    {
        ...
        // convert options
        if (($pa_options['restrictToTypes'] = caGetOption(
                array('restrictToTypes', 'restrict_to_types', 'restrictToType', 'restrict_to_type'),
                $pa_options,
                null
            )) && !is_array($pa_options['restrictToTypes'])) {
            $pa_options['restrictToTypes'] = preg_split("![;,]{1}!", $pa_options['restrictToTypes']);
        }
        if (($pa_options['restrictToRelationshipTypes'] = caGetOption(
                array(
            ...
            $pa_options['restrictToRelationshipTypes'] = preg_split(
                "![;,]{1}!",
                $pa_options['restrictToRelationshipTypes']
            );
        }
        if (($pa_options['excludeTypes'] = caGetOption(
                array('excludeTypes', 'exclude_types', 'excludeType', 'exclude_type'),
                $pa_options,
                null
            )) && !is_array($pa_options['excludeTypes'])) {
            $pa_options['excludeTypes'] = preg_split("![;,]{1}!", $pa_options['excludeTypes']);
        }
        if (($pa_options['excludeRelationshipTypes'] = caGetOption(
                array(
        ...
        if (($pa_options['restrictToLists'] = caGetOption(
                array('restrictToLists', 'restrict_to_lists'),
                $pa_options,
                null
        ...
        $pa_primary_ids = (isset($pa_options['primaryIDs']) && is_array(
                $pa_options['primaryIDs']
            )) ? $pa_options['primaryIDs'] : null;
        $pb_show_current_only = caGetOption(
            'showCurrentOnly',
            $pa_options,
        ...
        $pa_get_where = (isset($pa_options['where']) && is_array($pa_options['where']) && sizeof(
                $pa_options['where']
            )) ? $pa_options['where'] : null;

        $pa_row_ids = (isset($pa_options['row_ids']) && is_array(
                $pa_options['row_ids']
            )) ? $pa_options['row_ids'] : null;
        ...
        $pa_sort_fields = (isset($pa_options['sort']) && is_array($pa_options['sort'])) ? array_filter(
            $pa_options['sort'],
            "strlen"
        ) : null;
        ...
        if (is_array($va_type_ids) && (sizeof($va_type_ids) > 0)) {
            $va_wheres[] = "({$vs_related_table}.type_id IN (" . join(
                    ',',
                    $va_type_ids
                ) . ')' . ($t_rel_item->getFieldInfo(
                    'type_id',
                    'IS_NULL'
        ...
        if (method_exists($t_rel_item, "getSourceFieldName") && ($vs_source_id_fld = $t_rel_item->getSourceFieldName(
            )) && is_array($va_source_ids) && (sizeof($va_source_ids) > 0)) {
        ...
        if (is_array($pa_get_where)) {
            foreach ($pa_get_where as $vs_fld => $vm_val) {
                if ($t_rel_item->hasField($vs_fld)) {
                    $va_wheres[] = "({$vs_related_table_name}.{$vs_fld} = " . (!is_numeric(
                            $vm_val
                        ) ? "'" . $this->getDb()->escape($vm_val) . "'" : $vm_val) . ")";
                }
            }
        }
        ...
        if (method_exists(
                $t_rel_item,
                "getLabelTableName"
            ) && (!isset($pa_options['dontReturnLabels']) || !$pa_options['dontReturnLabels'])) {
            if ($vs_label_table_name = $t_rel_item->getLabelTableName(
                ...
                if ($t_rel_item_label->hasField(
                        'is_preferred'
                    ) && (!isset($pa_options['returnNonPreferredLabels']) || !$pa_options['returnNonPreferredLabels'])) {
        ...
        if (isset($pa_options['checkAccess']) && is_array($pa_options['checkAccess']) && sizeof(
        ...
        if (($va_criteria = (isset($pa_options['criteria']) ? $pa_options['criteria'] : null)) && (is_array(
                $va_criteria
            )) && (sizeof($va_criteria))) {
                ...
                $vs_sql = "
					...
					WHERE
                        ...
                        array_merge(
                            $va_wheres,
                            array(
                                '(' . $va_path[1] . '.' . $vs_other_field . ' IN (' . join(
                                    ',',
                                    $pa_row_ids
                                ) . '))'
                            )
...
// 					}
                    ...
                    if ($vb_uses_effective_dates) {    // return effective dates as display/parse-able text
                        if ($va_rels[$vs_sort_key][$vn_id]['sdatetime'] || $va_rels[$vs_sort_key][$vn_id]['edatetime']) {
                            $o_tep->setHistoricTimestamps(
                                $va_rels[$vs_sort_key][$vn_id]['sdatetime'],
                    ...
                    if ($pb_use_locale_codes) {
                        $va_rels[$vs_sort_key][$vn_id]['locale_id'] = $vn_locale_id = $t_locale->localeIDToCode(
                            $vn_locale_id
                        );
                    }
                    ...
                    if ($pb_show_current_only && $t_item_rel) {
                        ...
                        while ($qr_rels->nextHit()) {
                            foreach (
                                $qr_rels->get(
                                    $ps_current_date_bundle,
                                    ['returnAsArray' => true, 'sortable' => true]
                                ) as $vs_date
                            ) {
                                ...
                                $va_rels_by_date[$vs_date . '/' . sprintf(
                                    "%09d",
                                    $qr_rels->get(
                                        $t_item_rel->tableName() . ".relation_id"
                                    )
                                )][$vs_sort_key][$vn_id] = $va_rels[$vs_sort_key][$vn_id];
                            }
                        }
                    }
                }
            ...
            if ($ps_return_as !== 'data') {
                $va_rels = caExtractArrayValuesFromArrayOfArrays(
                    $va_rels,
            ...
            if (method_exists($this, 'isSelfRelationship') && $this->isSelfRelationship()) {
                ...
                $va_wheres[] = "({$vs_subject_table_name}." . $this->primaryKey() . " IN (" . join(
                        ",",
                        $pa_row_ids
                    ) . "))";
                ...
                if ($vb_is_combo_key_relation) {
                    if ($vs_item_rel_table_name) {
                        $va_joins = [
                            "INNER JOIN {$vs_item_rel_table_name} ON {$vs_item_rel_table_name}.row_id = " . $this->primaryKey(
                                true
                            ) . " AND {$vs_item_rel_table_name}.table_num = " . $this->tableNum()
                        ];
                        $va_joins[] = "INNER JOIN {$vs_related_table_name} ON {$vs_related_table_name}." . $t_rel_item->primaryKey(
                            ) . " = {$vs_item_rel_table_name}." . $t_rel_item->primaryKey();
                    } else {
                        $va_joins = [
                            "INNER JOIN {$vs_related_table_name} ON {$vs_related_table_name}.row_id = " . $this->primaryKey(
                                true
                            ) . " AND {$vs_related_table_name}.table_num = " . $this->tableNum()
                        ];
                    }
                    if (method_exists(
                            $t_rel_item,
                            "getLabelTableInstance"
                        ) && ($t_rel_label = $t_rel_item->getLabelTableInstance())) {
                        ...
                        if (is_array($pa_options['restrictToTypes']) && sizeof($pa_options['restrictToTypes'])) {
                            ...
                            foreach ($va_rels as $vs_rel_pk => $va_rel_info) {
                                if ($va_rel_info['one_table'] != $this->tableName()) {
                                    $va_type_ids = caMakeTypeIDList(
                                        $va_rel_info['one_table'],
                                        $pa_options['restrictToTypes']
                                    );

                                    if (is_array($va_type_ids) && sizeof($va_type_ids)) {
                                        $va_joins[] = "INNER JOIN {$va_rel_info['one_table']} AS r ON r.{$va_rel_info['one_table_field']} = " . $t_rel_item->tableName(
                                            ) . ".{$vs_rel_pk}";
                ...
                while ($qr_res->nextRow()) {
                    ...
                    if ($pb_show_current_only && $t_item_rel) {
                        if ($this->isRelationship()) {
                            $k = $this->tableName() . "." . (($this->getLeftTableFieldName(
                                    ) == $vs_key) ? $this->getRightTableFieldName() : $this->getLeftTableFieldName());
                        ...
                        while ($qr_rels->nextHit()) {
                            foreach (
                                ...
                                $va_rels_for_id_by_date[$qr_rels->get($k)][$vs_date . '/' . sprintf(
                                    "%09d",
                                    $qr_rels->get(
                                        $t_item_rel->tableName() . ".relation_id"
                                    )
                                )][$vs_v] = $va_rels_for_id[$vs_v];
                            }
                        }
                    }
            ...
            foreach ($va_rels as $vn_pk => $va_related_item) {
                foreach ($va_filters as $vs_filter => $va_filter_vals) {
                    ...
                    foreach ($va_filter_vals as $vn_index => $vs_filter_val) {
                        ...
                        if (!is_numeric($vs_filter_val) && (($t_element = ca_metadata_elements::getInstance(
                                    $vs_element
                    ...
                    $va_filter_values = $t_rel_item->get(
                        $vs_filter,
    ...
    public function getTypeMenu()
    {
        ...
        if (is_array($va_hierarchy)) {
            ...
            foreach ($va_hierarchy as $vn_item_id => $va_item) {
                ...
                if (isset($va_types_by_parent_id[$va_item['item_id']]) && is_array(
                        $va_types_by_parent_id[$va_item['item_id']]
                    )) {
                    $va_subtypes = $this->_getSubTypes(
                        $va_types_by_parent_id[$va_item['item_id']],
    ...
    public function htmlFormElement($ps_field, $ps_format = null, $pa_options = null)
    {
        ...
        if (
            ...
            if (($this->tableName() == 'ca_objects') && $this->getAppConfig()->get(
                ...
                ) && $pa_options['request'] && ($vn_collection_id = $pa_options['request']->getParameter(
                    'collection_id',
                    pInteger
                ))) {
                require_once(__CA_MODELS_DIR__ . "/ca_collections.php");
                ...
                if ($t_coll->getPrimaryKey()) {
                    ...
                    if (!$this->opo_idno_plugin_instance->formatHas(
                            'PARENT'
                        ) && !$this->opo_idno_plugin_instance->getFormatProperty(
                        ...
                        if (!$this->getPrimaryKey() && !$this->opo_idno_plugin_instance->formatHas('PARENT')) {
                            $this->set(
                                ...
                                ($t_parent->get($vs_idno_fld)) ?
                                    $this->opo_idno_plugin_instance->makeTemplateFromValue(
                                        $t_parent->get($vs_idno_fld),
                                        1,
                                        true
                                    )
            ...
            if (is_null($ps_format)) {
                if (isset($pa_options['field_errors']) && is_array($pa_options['field_errors']) && sizeof(
                        $pa_options['field_errors']
                    )) {
...
// TODO: should be in config file
                ...
                if (
                    ...
                    if (preg_match("/\^DESCRIPTION/", $ps_formatted_element)) {
                        $ps_formatted_element = str_replace(
                            "^LABEL",
                        ...
                        $vs_field_id = '_' . $this->tableName() . '_' . $this->getPrimaryKey(
                            ) . '_' . $pa_options["name"] . '_' . $pa_options['form_name'];
                        $ps_formatted_element = str_replace(
                            "^LABEL",
                            '<span id="' . $vs_field_id . '">' . (isset($pa_options['label']) ? $pa_options['label'] : $va_attr["LABEL"]) . '</span>',
                            $ps_formatted_element
                        );
                        ...
                        if (!$pa_options['no_tooltips']) {
                            TooltipManager::add(
                                '#' . $vs_field_id,
                                "<h3>" . (isset($pa_options['label']) ? $pa_options['label'] : $va_attr["LABEL"]) . "</h3>" . $va_attr["DESCRIPTION"]
                            );
                        }
                    }
                } else {
    ...
    public function getIDNoPlugInInstance()
    {
        ...
        $this->opo_idno_plugin_instance->setDb(
            $this->getDb()
        );    // Make sure returned instance is using current transaction database handle
    ...
    public function loadIDNoPlugInInstance($pa_options = null)
    {
        ...
        return $this->opo_idno_plugin_instance = IDNumbering::newIDNumberer(
            $this->tableName(),
            $this->getTypeCode(),
            null,
            $this->getDb(),
    ...
    {
        $va_errors = array();
        if ($this->_CONFIG->get('require_valid_id_number_for_' . $this->tableName()) && sizeof(
                $va_admin_idno_errors = $this->opo_idno_plugin_instance->isValidValue($ps_admin_idno)
            )) {
            $va_errors[] = join('; ', $va_admin_idno_errors);
        } else {
            if (!$this->_CONFIG->get('allow_duplicate_id_number_for_' . $this->tableName()) && sizeof(
                    $this->checkForDupeAdminIdnos($ps_admin_idno)
                )) {
    ...
    private function _getSubTypes($pa_subtypes, $pa_types_by_parent_id)
    {
        ...
        foreach ($pa_subtypes as $vn_i => $va_type) {
            if (is_array($pa_types_by_parent_id[$va_type['item_id']])) {
                $va_subsubtypes = $this->_getSubTypes(
                    $pa_types_by_parent_id[$va_type['item_id']],
    ...
    public function makeSearchResult($pm_rel_table_name_or_num, $pa_ids, $pa_options = null)
    {
        ...
        if ($pa_sort = caGetOption('sort', $pa_options, null)) {
            ...
            $va_ids = $vo_sort->sortHits(
                $va_ids,
                $t_instance->tableName(),
                join(';', $pa_sort),
                caGetOption('sortDirection', $pa_options, 'asc'),
                $pa_options
            );
        }
        ...
        $o_res = new $vs_search_result_class(
            $t_instance->tableName()
        );    // we pass the table name here so generic multi-table search classes such as InterstitialSearch know what table they're operating over
    ...
    public function getACLUsers($pa_options = null)
    {
        ...
        if ($qr_users = $this->makeSearchResult('ca_users', $va_user_ids)) {
            $va_initial_values = caProcessRelationshipLookupLabel(
                $qr_users,
                new ca_users(),
    ...
    public function addACLUsers($pa_user_ids)
    {
        ...
        foreach ($pa_user_ids as $vn_user_id => $vn_access) {
            $t_acl->clear();
            $t_acl->load(
                array('user_id' => $vn_user_id, 'table_num' => $vn_table_num, 'row_id' => $vn_id)
    ...
    public function addACLUserGroups($pa_group_ids, $pa_options = null)
    {
        ...
        foreach ($pa_group_ids as $vn_group_id => $vn_access) {
            ...
            $t_acl->clear();
            $t_acl->load(
                array('group_id' => $vn_group_id, 'table_num' => $vn_table_num, 'row_id' => $vn_id)
            );        // try to load existing record
    ...
    public function getACLWorldAccess($pa_options = null)
    {
        ...
        if (!strlen($va_row['access_display'])) {    // show default
            $va_row['access_display'] = $t_acl->getChoiceListValue(
                'access',
                $this->getAppConfig()->get(
    ...
    public function setACLWorldAccess($pn_world_access)
    {
        ...
        $t_acl = new ca_acl();
        $t_acl->load(
            array('group_id' => null, 'user_id' => null, 'table_num' => $vn_table_num, 'row_id' => $vn_id)
        );        // try to load existing record
    ...
    public function addRelationship(
        ...
        $pn_rel_id,
        $pm_type_id = null,
        $ps_effective_date = null,
        $ps_source_info = null,
        $ps_direction = null,
        $pn_rank = null,
        $pa_options = null
    ) {
        $this->opo_app_plugin_manager->hookAddRelationship(
            array(
                'table_name' => $this->tableName(),
                'instance' => &$this,
        ...
        if ($t_rel = parent::addRelationship(
            $pm_rel_table_name_or_num,
            $pn_rel_id,
            $pm_type_id,
            $ps_effective_date,
    ...
    public function editRelationship(
        ...
        if ($t_rel = parent::editRelationship(
            $pm_rel_table_name_or_num,
            $pn_relation_id,
            $pn_rel_id,
            $pm_type_id,
    ...
    public function moveRelationships($pm_rel_table_name_or_num, $pn_to_id, $pa_options = null)
    {
        ...
        $this->opo_app_plugin_manager->hookBeforeMoveRelationships(
            array(
                'table_name' => $this->tableName(),
                'instance' => &$this,
                'related_table' => &$pm_rel_table_name_or_num,
        ...
        $this->opo_app_plugin_manager->hookAfterMoveRelationships(
            array(
                'table_name' => $this->tableName(),
                'instance' => &$this,
                'related_table' => &$pm_rel_table_name_or_num,
    ...
    public function getHierarchyList($pb_dummy = false)
    {
        ...
        if (!$vn_id) {
            ...
            if (is_array($va_type_ids) && sizeof($va_type_ids)) {
                ...
                $va_wheres[] = "(o.{$vs_type_fld} IN (?)" . ($this->getFieldInfo(
                        $vs_type_fld,
                        'IS_NULL'
    ...
    public function getMetadataDictionaryRuleViolations($ps_bundle_name = null, $options = null)
    {
        ...
        if (($screen_id = caGetOption('screen_id', $options, null)) || ($placement_id = caGetOption(
                'placement_id',
                $options,
                null
            ))) {
            ...
            if (is_array(
                $screen_placements = $t_screen->getPlacements(
                    $placement_id ? ['placement_id' => $placement_id] : ['screen_id' => $screen_id]
                )
            )) {
                $bundles_on_screen = array_map(
                    function ($v) {
        ...
        if ($ps_bundle_name = str_replace("ca_attribute_", "", $ps_bundle_name)) {
            if (!Datamodel::tableExists($ps_bundle_name) && (!preg_match(
                    '!^' . $this->tableName() . '.!',
                    $ps_bundle_name
                ))) {
        ...
        while ($qr_res->nextRow()) {
            ...
            $t_rule = (isset($va_rule_instances[$vn_rule_id])) ? $va_rule_instances[$vn_rule_id] : ($va_rule_instances[$vn_rule_id] = new ca_metadata_dictionary_rules(
                $vn_rule_id
            ));
            ...
            if (is_array($bundles_on_screen)) {
                ...
                $rule_settings_restrict_to_types = array_filter(
                    caGetOption(['restrict_to_types', 'restrictToTypes'], $rule_settings, []),
                    function ($v) {
                        return strlen($v);
                    }
                );
                $rule_settings_restrict_to_relationship_types = array_filter(
                    caGetOption(['restrict_to_relationship_types', 'restrictToRelationshipTypes'], $rule_settings, []),
                    function ($v) {
                        return strlen($v);
                    }
                );
                ...
                foreach ($bundles_on_screen as $placement_id => $placement_bundle_name) {
                    if ($placement_bundle_name == $p) {
                        ...
                        $placement_restrict_to_types = array_filter(
                            caGetOption(
                                ['restrict_to_types', 'restrictToTypes'],
                                $screen_placements[$placement_id]['settings'],
                                []
                            ),
                            function ($v) {
                                return strlen($v);
                        ...
                        $placement_restrict_to_relationship_types = array_filter(
                            caGetOption(
                        ...
                        if (sizeof($placement_restrict_to_types) && !sizeof(
                                array_intersect($placement_restrict_to_types, $rule_settings_restrict_to_types)
                            )) {
                            continue;
                        }

                        if (sizeof($placement_restrict_to_relationship_types) && !sizeof(
                                array_intersect(
    ...
    public function validateUsingMetadataDictionaryRules($pa_options = null)
    {
        ...
        $va_rules = ca_metadata_dictionary_rules::getRules(
            array('db' => $o_db, 'table' => $this->tableNum(), 'bundles' => caGetOption('bundles', $pa_options, null))
        );
        ...
        foreach ($va_rules as $va_rule) {
            ...
            if ($t_found = ca_metadata_dictionary_rule_violations::find(
                array(
    ...
    public function getItemCommentHTMLFormBundle(
        ...
        $initial_values = [];
        foreach (
        ($this->getPrimaryKey() ? $this->getComments(
            null,
            null,
            [
                'request' => $po_request,
                'sortDirection' => $pa_bundle_settings['sortDirection']
            ]
        ) : []) as $v
        ) {
