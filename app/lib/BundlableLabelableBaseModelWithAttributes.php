...
class BundlableLabelableBaseModelWithAttributes extends LabelableBaseModelWithAttributes implements IBundleProvider
{
    ...
    public function duplicate($pa_options = null)
    {
        ...
        foreach (
            ...
            if ($this->copyRelationships(
                    ...
                    array('copyAttributes' => $vb_duplicate_relationship_attributes)
                ) === false) {
                $this->errors = $t_dupe->errors;
                if ($vb_we_set_transaction) {
                    $o_t->rollback();
    ...
    public function getBundleFormHTML(
        ...
        if (
            ...
            ||
            ($va_dictionary_entry = ca_metadata_dictionary_entries::getEntry(
                $this->tableName() . '.' . $ps_bundle_name_proc,
                $this,
                $pa_bundle_settings
            ))
        ) {
            ...
            if (is_array($va_violations) && sizeof($va_violations)) {
                ...
                foreach ($va_violations as $va_violation) {
                    $va_violation_text[] = "<li class='caMetadataDictionaryViolation'><span class='caMetadataDictionaryViolation" . (ucfirst(
                        ...
                        )) . "'>" . $va_violation['levelDisplay'] . '</span>: ' . $va_violation['violationMessage'] . "</li>";
                }
            ...
            case 'special':
                ...
                switch ($ps_bundle_name) {
                    ...
                    case 'ca_objects_location':        // storage location via ca_objects_x_storage_locations or ca_movements_x_objects
                        ...
                        $vs_element .= $this->getObjectLocationHTMLFormBundle(
                            $pa_options['request'],
                            $pa_options['formName'],
                            $ps_placement_code,
                            $pa_bundle_settings,
                    ...
                    case 'ca_objects_history':        // summary of object accession, movement, exhibition and deaccession
                        ...
                        $vs_element .= $this->getObjectHistoryHTMLFormBundle(
                            $pa_options['request'],
                            $pa_options['formName'],
                            $ps_placement_code,
                            $pa_bundle_settings,
                            array_merge(
                                ...
                                [
                                    'showChildHistory' => Session::getVar(
                                        "ca_objects_history_showChildHistory"
                                    )
                                ]
                            )
                        );
                    ...
                    case 'ca_storage_locations_contents':        // objects in storage location via ca_objects_x_storage_locations or ca_movements_x_objects
                        ...
                        $vs_element .= $this->getLocationContentsHTMLFormBundle(
                            $pa_options['request'],
                            $pa_options['formName'],
                            $ps_placement_code,
                            $pa_bundle_settings,
    ...
    public function getBundleFormHTMLForScreen($pm_screen, $pa_options, &$pa_placements = null)
    {
        ...
        if (is_array($va_bundles)) {
            ...
            foreach ($va_bundles as $va_bundle) {
                ...
                $va_definition_bundle_names[(!Datamodel::tableExists(
        ...
        if ($ps_related_table == 'ca_sets') {
            ...
            if ($ps_sort = caGetOption('sort', $va_get_related_opts, null)) {
                $va_items = caSortArrayByKeyInValue(
                    $va_items,
                    array($ps_sort),
                    caGetOption('sortDirectio ', $va_get_related_opts, 'ASC')
                );
            }
            ...
            return $va_vals;
        } elseif (($ps_related_table == 'ca_objects') && ($this->tableName() == 'ca_storage_locations') && (strlen(
                    $vs_mode = $pa_bundle_settings['locationTrackingMode']
                ) > 0)) {
            // Limit list to objects _currently_ in this location
            if (!($qr_results = $this->getLocationContents($vs_mode))) {
                return [];
            }

            if (sizeof($va_ids = $qr_results->getAllFieldValues('ca_objects.object_id')) == 0) {
                return [];
            }
    ...
    public function saveBundlesForScreen($pm_screen, $po_request, &$pa_options)
    {
        ...
        $t_ui = caGetOption(
            'ui_instance',
            $pa_options,
            ca_editor_uis::loadDefaultUI($this->tableName(), $po_request, $this->getTypeID())
        );
                    ...
                    case 'ca_object_representations':
                        ...
                        if (!$vb_batch && is_array($va_reps)) {
                            foreach ($va_reps as $vn_i => $va_rep) {
                                ...
                                if (strlen(
                                        $po_request->getParameter(
                                            $vs_prefix_stub . 'access_' . $va_rep['representation_id'],
                                            pInteger
                                        )
                                    ) > 0) {
                                    ...
                                    $vn_is_primary = ($po_request->getParameter(
                                            $vs_prefix_stub . 'is_primary_' . $va_rep['representation_id'],
                                            pString
                                        ) != '') ? $po_request->getParameter(
                                        $vs_prefix_stub . 'is_primary_' . $va_rep['representation_id'],
                                        pInteger
                                    ) : null;
                                    $vn_locale_id = $po_request->getParameter(
                                        $vs_prefix_stub . 'locale_id_' . $va_rep['representation_id'],
                                        pInteger
                                    );
                                    $vs_idno = $po_request->getParameter(
                                        $vs_prefix_stub . 'idno_' . $va_rep['representation_id'],
                                        pString
                                    );
                                    ...
                                    $vn_status = $po_request->getParameter(
                                        $vs_prefix_stub . 'status_' . $va_rep['representation_id'],
                                        pInteger
                                    );
                                    $vs_rep_label = trim(
                                        $po_request->getParameter(
                                            $vs_prefix_stub . 'rep_label_' . $va_rep['representation_id'],
                                            pString
                                        )
                                    );
                                    ...
                                    $vn_center_x = $po_request->getParameter(
                                        $vs_prefix_stub . 'center_x_' . $va_rep['representation_id'],
                                        pString
                                    );
                                    $vn_center_y = $po_request->getParameter(
                                        $vs_prefix_stub . 'center_y_' . $va_rep['representation_id'],
                                        pString
                                    );
                                    ...
                                    $this->clearErrors();
                                    if (($po_request->getParameter(
                                            $vs_prefix_stub . $va_rep['representation_id'] . '_delete',
                                            pInteger
                                        )) > 0) {
                                        // delete!
                            ...
                            if (($vs_batch_mode == '_replace_') && sizeof($va_reps)) {
                                foreach ($va_reps as $va_rep) {
                                    ...
                                    $pn_status = $po_request->getParameter(
                                        "{$vs_prefix_stub}access_status_new_0_enabled",
                                        pInteger
                            ...
                            foreach ($va_file_list as $vs_key => $va_values) {
                                ...
                                if (!preg_match(
                                        ...
                                        $va_matches
                                                ...
                                                $vs_key,
                                                $va_matches
                                            )) || !$vb_allow_fetching_of_urls) && (($vb_allow_existing_rep && !preg_match(
                                                '/^' . $vs_prefix_stub . 'autocompletenew_([\d]+)$/',
                                                $vs_key,
                                                $va_matches
                                ...
                                if ($vn_existing_rep_id = $po_request->getParameter(
                                    ...
                                    $vs_rep_label = trim(
                                        $po_request->getParameter(
                                            $vs_prefix_stub . 'rep_label_new_' . $va_matches[1],
                                            pString
                                        )
                                    );
                                    $vn_locale_id = $po_request->getParameter(
                                        $vs_prefix_stub . 'locale_id_new_' . $va_matches[1],
                                        pInteger
                                    );
                                    $vs_idno = $po_request->getParameter(
                                    ...
                                    $vn_status = $po_request->getParameter(
                                        $vs_prefix_stub . 'status_new_' . $va_matches[1],
                                        pInteger
                                    );
                                    $vn_access = $po_request->getParameter(
                                    ...
                                    $vn_is_primary = $po_request->getParameter(
                                        $vs_prefix_stub . 'is_primary_new_' . $va_matches[1],
                                        pInteger
                                    );
                ...
                switch ($vs_f) {
                    ...
                    case 'ca_object_representations_media_display':
                        ...
                        if ($this->changed('media')) {
                            $this->update(
                                ($vs_version != '_all') ? array('updateOnlyMediaVersions' => $va_versions_to_process) : array()
                            );
                            if ($this->numErrors()) {
                                $po_request->addActionErrors(
                                    $this->errors(),
                        ...
                        foreach ($_REQUEST as $vs_key => $vs_val) {
                            if (preg_match(
                                ...
                                $this->addCaptionFile(
                                    ...
                                    $vn_locale_id,
                                    array('originalFilename' => $_FILES["{$vs_placement_code}{$vs_form_prefix}_captions_caption_file" . $va_matches[1]]['name'])
                                );
                            } else {
                                // any to delete?
                                if (preg_match(
                    ...
                    case 'ca_objects_location':
                        ...
                        if ($vn_location_id = $po_request->getParameter(
                            "{$vs_placement_code}{$vs_form_prefix}_location_idnew_0",
                            pInteger
                        )) {
                            if (
                                ($vs_relationship_type = $this->getAppConfig()->get(
                                    'object_storage_location_tracking_relationship_type'
                                ))
                                &&
                                (is_array(
                                    $va_relationship_type_ids = caMakeRelationshipTypeIDList(
                                ...
                                $this->addRelationship(
                                    'ca_storage_locations',
                                    $vn_location_id,
                                    array_shift($va_relationship_type_ids),
                                    null,
                                    null,
                                    null,
                                    null,
                                    array('allowDuplicates' => true)
                                );
                                if ($this->numErrors()) {
                                ...
                                $o_error = new ApplicationError(
                                    2593,
                                    _t('No relationship type configured'),
                                    'BundleableLabelableBaseModelWithAttributes->saveBundlesForScreen',
                                    'general',
                                    false,
                                    false
                                );
                    ...
                    case 'ca_objects_history':
                        ...
                        if ($vn_location_id = $po_request->getParameter(
                            ...
                            if (
                                ...
                                (
                                    (is_array(
                                        $va_relationship_types = caGetOption(
                                            'ca_storage_locations_showRelationshipTypes',
                                            $va_bundle_settings,
                                            null
                                        )
                                    ))
                                ...
                                $vs_effective_date = $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_location_effective_datenew_0",
                                    pString
                                );
                                ...
                                if ($this->numErrors()) {
                                    ...
                                    if (is_array(
                                        $va_storage_location_elements = caGetOption(
                                            'ca_storage_locations_elements',
                                            $va_bundle_settings,
                                            array()
                                        )
                                    )) {
                                        foreach ($va_storage_location_elements as $vs_element) {
                                            if ($vs_element == 'effective_date') {
                                                continue;
                                            }
                                            if ($this->hasField($vs_element)) {
                                                $t_item_rel->set(
                                                    $vs_element,
                                                    $vs_val = $po_request->getParameter(
                                                        "{$vs_placement_code}{$vs_form_prefix}_location_{$vs_element}new_0",
                                                        pString
                                                    )
                                                );
                                            } elseif ($vn_element_id = ca_metadata_elements::getElementID(
                                                $vs_element
                                            )) {
                                                $va_sub_element_ids = ca_metadata_elements::getElementsForSet(
                                                    $vn_element_id,
                                                    ['idsOnly' => true]
                                                );
                                                ...
                                                foreach ($va_sub_element_ids as $vn_sub_element_id) {
                                                    $va_vals[ca_metadata_elements::getElementCodeForID(
                                                        $vn_sub_element_id
                                                    )] = $po_request->getParameter(
                                                        "{$vs_placement_code}{$vs_form_prefix}_location_{$vn_sub_element_id}_new_0",
                                                        pString
                                                    );
                                                }
                        ...
                        if ($vn_loan_id = $po_request->getParameter(
                            "{$vs_placement_code}{$vs_form_prefix}_loan_idnew_0",
                            pInteger
                        )) {
                            if ($vn_loan_type_id = $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}_loan_type_idnew_0",
                                pInteger
                            )) {
                        ...
                        foreach ($va_occ_types as $vn_type_id => $vn_type_info) {
                            if ($vn_occurrence_id = $po_request->getParameter(
                                "{$vs_placement_code}{$vs_form_prefix}_occurrence_{$vn_type_id}_idnew_0",
                                pInteger
                            )) {
                                if ($vn_occ_type_id = $po_request->getParameter(
                                    "{$vs_placement_code}{$vs_form_prefix}_occurrence_{$vn_type_id}_type_idnew_0",
                                    pInteger
                                )) {
        ...
        if (sizeof($va_violations)) {
            if ($vb_we_set_transaction && isset($va_violations['ERR']) && is_array($va_violations['ERR']) && (sizeof(
                        $va_violations['ERR']
                    ) > 0)) {
                BaseModel::unsetChangeLogUnitID();
                $this->removeTransaction(false);
                        ...
                        $po_request->addActionErrors(
                            array(
                                new ApplicationError(
                                    1100,
                                    $va_rule['rule_settings']['violationMessage'],
                                    "BundlableLabelableBaseModelWithAttributes->saveBundlesForScreen()",
                                    'MetadataDictionary',
                                    false,
                                    false
        ...
        if (!$vb_batch) {
            foreach ($va_rel_items as $va_rel_item) {
                ...
                if ($vn_id) {
                    ...
                    $vs_effective_daterange = $po_request->getParameter(
                        "{$ps_placement_code}{$ps_form_prefix}_effective_date" . $va_rel_item[$vs_key],
                        pString
                    );
    ...
    public function getRelatedItemsAsSearchResult($pm_rel_table_name_or_num, $pa_options = null)
    {
        if (is_array(
            $va_related_ids = $this->getRelatedItems(
                ...
                array_merge($pa_options, array('idsOnly' => true))
            )
        )) {
            $va_ids = array_map(
                function ($pn_v) {
...
// TODO: should be in config file
    ...
    public function validateUsingMetadataDictionaryRules($pa_options = null)
    {
        ...
        $va_rules = ca_metadata_dictionary_rules::getRules(
            array('db' => $o_db, 'bundles' => caGetOption('bundles', $pa_options, null))
        );
