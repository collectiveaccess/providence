...
class ActiveDirectoryAuthAdapter extends AbstractLDAPAuthAdapter {
	...
	protected function isUserInAnyGroup( $pr_ldap, $ps_username, $pa_group_cn_list, $pa_config ) {
		...
		$vs_user_search_dn     = $this->getProcessedConfigValue( $pa_config, "ldap_user_search_dn_format", $ps_username,
			$vs_user_ou, $vs_base_dn );
		$vs_user_search_filter = $this->getProcessedConfigValue( $pa_config, "ldap_user_search_filter_format",
		...
		return sizeof( array_intersect( array_map( 'strtolower', $pa_group_cn_list ),
				array_map( 'strtolower', $va_attrs[ $vs_member_of_attr ] ) ) ) > 0;
	}
