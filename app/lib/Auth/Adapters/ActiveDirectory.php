...
class ActiveDirectoryAuthAdapter extends AbstractLDAPAuthAdapter
{
    ...
    protected function isUserInAnyGroup($pr_ldap, $ps_username, $pa_group_cn_list, $pa_config)
    {
        ...
        $vs_user_search_dn = $this->getProcessedConfigValue(
            $pa_config,
        ...
        $vs_user_search_filter = $this->getProcessedConfigValue(
            $pa_config,
        ...
        return sizeof(
                array_intersect(
                    array_map('strtolower', $pa_group_cn_list),
                    array_map('strtolower', $va_attrs[$vs_member_of_attr])
                )
            ) > 0;
    }
