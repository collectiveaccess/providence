...
class ExternalDBAuthAdapter extends BaseAuthAdapter implements IAuthAdapter
{
        ...
        if (!$o_ext_db->connected()) {
            $o_log->log(
                array(
                    ...
                    'MESSAGE' => _t(
                        'Could not login user %1 using external database because login to external database failed [%2]',
                        $ps_username,
                        $_SERVER['REMOTE_ADDR']
                    )
                )
    ...
    public function getUserInfo($ps_username, $ps_password, $pa_options = null)
    {
        ...
        if (!$qr_auth || !$qr_auth->nextRow()) {
            throw new ExternalDBException(
                _t(
                    'Could not login user %1 using external database because external authentication failed [%2]',
                    $ps_username,
                    $_SERVER['REMOTE_ADDR']
                )
            );
        ...
        if (($vs_extdb_active_field = $o_auth_config->get('extdb_active_field')) && is_array(
                $va_extdb_active_field_map
            )) {
        ...
        if (($vs_extdb_access_field = $o_auth_config->get('extdb_access_field')) && is_array(
                $va_extdb_access_field_map
            )) {
        ...
        switch ($vs_extdb_access_value) {
            ...
            default:
                ...
                throw new ExternalDBException(
                    _t(
                        'Could not login user %1 after authentication from external database because user class was not set.',
                        $ps_username
                    )
                );
        }
        ...
        if (($vs_extdb_groups_field = $o_auth_config->get('extdb_groups_field')) && is_array(
                $va_extdb_groups_field_map
            )) {
