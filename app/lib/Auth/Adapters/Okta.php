        ...
        if (!$this->isAuthenticated()) {
            $query = http_build_query(
                [
                    ...
                    'redirect_uri' => $g_request ? caNavUrl(
                        $g_request,
                        'system',
                        'auth',
                        'callback',
    ...
    public function verifyJwt($jwt)
    {
        $headers = [
            'Authorization: Basic ' . base64_encode(
                $this->auth_config->get('okta_client_id') . ":" . $this->auth_config->get('okta_secret')
            )
        ];
        $resp = self::oktaRequest(
            $this->auth_config->get('okta_issuer') . '/oauth2/v1/introspect',
            $headers,
            [
                'jsonDecode' => true,
                'post' => true,
                'postParams' => 'token_type_hint=access_token&token=' . $jwt
            ]
        );
        // try {
// 			$jwtVerifier = (new \Okta\JwtVerifier\JwtVerifierBuilder())
