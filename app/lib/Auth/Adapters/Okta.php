	...
	public function authenticate( $username, $password = '', $options = null ) {
		...
		if ( ! $this->isAuthenticated() ) {
			$query = http_build_query( [
				...
				'redirect_uri'  => $g_request ? caNavUrl( $g_request, 'system', 'auth', 'callback', null,
					[ 'absolute' => true ] ) : '',
				'state'         => $this->state,
...
// 		return $jwt->claims;
	...
	public function verifyJwt( $jwt ) {
		$headers = [
			'Authorization: Basic ' . base64_encode( $this->auth_config->get( 'okta_client_id' ) . ":"
			                                         . $this->auth_config->get( 'okta_secret' ) )
		];
		$resp    = self::oktaRequest( $this->auth_config->get( 'okta_issuer' ) . '/oauth2/v1/introspect', $headers,
			[ 'jsonDecode' => true, 'post' => true, 'postParams' => 'token_type_hint=access_token&token=' . $jwt ] );
		// try {
// 			$jwtVerifier = (new \Okta\JwtVerifier\JwtVerifierBuilder())
// 				->setIssuer($this->auth_config->get('okta_issuer').'/oauth2')
// 				->setAudience('api://default')
...
// 		}
	...
	public function exchangeCode( $code ) {
		...
		$authHeaderSecret = base64_encode( $this->auth_config->get( 'okta_client_id' ) . ':'
		                                   . $this->auth_config->get( 'okta_secret' ) );
	 ...
	 *                        jsonDecode = Decode response as JSON and return PHP array. [Default is false; raw
