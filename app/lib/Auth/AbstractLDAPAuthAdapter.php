...
abstract class AbstractLDAPAuthAdapter extends BaseAuthAdapter {
	...
	public function authenticate( $ps_username, $ps_password = '', $pa_options = null ) {
		...
		foreach ( $this->getLinkIdentifiers() as $vs_key => $r_ldap ) {
			$vo_bind = $this->bindToDirectory( $r_ldap, $ps_username, $ps_password,
			...
			if ( ! $vo_bind ) {
				if ( ldap_get_option( $r_ldap, 0x0032, $extended_error ) ) {
					...
					caLogEvent( "ERR", "LDAP ERROR (" . ldap_errno( $r_ldap ) . ") {$extended_error} [{$vs_bind_rdn}]",
						"OpenLDAP::Authenticate" );
				}
				continue; // try next one
			}

			// check group membership
			if ( ! $this->hasRequiredGroupMembership( $r_ldap, $ps_username,
				$this->opa_auth_config_fragments[ $vs_key ] )
			) {
				continue; // try next one
			}
	...
	private function bindToDirectory( $po_ldap, $ps_username, $ps_password, $pa_config ) {
		...
		$vs_bind_rdn        = $this->getProcessedConfigValue( $pa_config, "ldap_bind_rdn_format", $ps_username,
			$vs_user_ou, $vs_base_dn );
		$vs_bind_rdn_filter = $this->getProcessedConfigValue( $pa_config, "ldap_bind_rdn_filter", $ps_username,
			$vs_user_ou, $vs_base_dn );
	...
	private function syncWithDirectory( $pr_ldap, $ps_username, $pa_config ) {
		...
		if ( $pa_config['ldap_sync_user_roles'] ) {
			$va_expected_roles = array_merge( $va_default_roles,
				$this->getRolesToAddFromDirectory( $pr_ldap, $ps_username, $pa_config ) );
		...
		if ( $pa_config['ldap_sync_user_groups'] ) {
			$va_expected_groups = array_merge( $va_default_groups,
				$this->getGroupsToAddFromDirectory( $pr_ldap, $ps_username, $pa_config ) );
