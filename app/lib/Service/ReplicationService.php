...
class ReplicationService {
	...
	public static function getLog( $po_request ) {
		...
		$pa_options = array();
		if ( $ps_skip_if_expression = $po_request->getParameter( 'skipIfExpression', pString, null,
			array( 'retainBackslashes' => false ) )
		) {
		...
		if ( $ps_ignore_tables = $po_request->getParameter( 'ignoreTables', pString, null,
			array( 'retainBackslashes' => false ) )
		) {
		...
		if ( $ps_only_tables = $po_request->getParameter( 'onlyTables', pString, null,
			array( 'retainBackslashes' => false ) )
		) {
		...
		if ( $ps_include_metadata = $po_request->getParameter( 'includeMetadata', pString, null,
			array( 'retainBackslashes' => false ) )
		) {
		...
		if ( $ps_exclude_metadata = $po_request->getParameter( 'excludeMetadata', pString, null,
			array( 'retainBackslashes' => false ) )
		) {
		...
		if (
			( $ps_push_media_to = $po_request->getParameter( 'pushMediaTo', pString, null,
				[ 'retainBackslashes' => false ] ) )
			...
			$va_log = ca_change_log::getLog( $pn_from, $pn_limit,
				array_merge( $pa_options, [ 'forceValuesForAllAttributeSLots' => true ] ), $va_media );
			...
			if ( sizeof( $va_media ) > 0 ) {
				...
				foreach ( $va_media as $vs_md5 => $vs_url ) {
					...
					ReplicationService::$s_logger->log( "Push media {$vs_url}::{$vs_md5} [" . caHumanFilesize(
							$vn_filesize = @filesize( $vs_local_path ) ) . "]" );
					...
					while ( $vn_retries > 0 ) {
						...
						curl_setopt( $o_curl, CURLOPT_USERPWD,
							$va_target_conf['service_user'] . ':' . $va_target_conf['service_key'] );
						...
						if ( $vn_code != 200 ) {
							if ( $vn_retries == 0 ) {
								throw new Exception( _t( "Could not upload file [%1] to target [%2]. HTTP response code was %3.",
									$vs_local_path, $ps_push_media_to, $vn_code ) );
							}
							ReplicationService::$s_logger->log( _t( "Could not upload file [%1] to target [%2]. HTTP response code was %3. Retrying (%4 remaining)",
								$vs_local_path, $ps_push_media_to, $vn_code, $vn_retries ) );
							sleep( 2 );
							continue;
						}
			...
			$va_log = ca_change_log::getLog( $pn_from, $pn_limit,
				array_merge( $pa_options, [ 'forceValuesForAllAttributeSLots' => true ] ) );
		}
	...
	public static function setLastLogID( $po_request ) {
		...
		if ( $log_id > 0 ) {
			...
			if ( $t_replication_log->numErrors() ) {
				throw new Exception( _t( 'Could not set last log_id: %1',
					join( "; ", $t_replication_log->getErrors() ) ) );
			}
		} else {
	...
	public static function applyLog( $po_request ) {
		...
		if ( $ps_set_intrinsics = $po_request->getParameter( 'setIntrinsics', pString, null,
			array( 'retainBackslashes' => false ) )
		) {
		...
		if ( ! is_array( $va_log ) ) {
			throw new \Exception( 'log must be array' );
		}
		$o_db = new Db();
		...
		foreach ( $va_log as $vn_log_id => $va_log_entry ) {
			...
			try {
				...
				$o_tx->rollback();
				$vn_last_applied_log_id = $vn_log_id;
				ReplicationService::$s_logger->log( "[InvalidLogEntryException] Sanity check error: "
				                                    . $e->getMessage() );
				continue;
			} catch ( \Exception $e ) {
				// append log entry to message for easier debugging
				$va_sanity_check_errors[] = $e->getMessage() . ' ' . _t( "Log entry was: %1",
						print_r( $va_log_entry, true ) );
			...
			try {
				...
				$vs_error = get_class( $e ) . ': ' . $e->getMessage() . $e->getTraceAsString() . ' '
				            . _t( "Log entry was: %1", print_r( $va_log_entry, true ) );
				$o_tx->rollback();
	...
	public static function dedup( $po_request ) {
		$pa_tables = [];
		if ( $ps_tables = $po_request->getParameter( 'tables', pString, null,
			array( 'retainBackslashes' => false ) )
		) {
		...
		foreach ( $pa_tables as $vs_table ) {
			...
			if ( class_exists( $vs_table ) && method_exists( $vs_table, 'listPotentialDupes' )
			     && method_exists( $vs_table, 'mergeRecords' )
			) {
				...
				if ( sizeof( $va_dupes ) ) {
					...
					foreach ( $va_dupes as $vs_sha2 => $va_keys ) {
						...
						if ( ! $vn_entity_id ) {
							throw new Exception( _t( "It seems like there was an error while deduplicating records. Keys were: %1",
								print_r( $va_keys, true ) ) );
						}
	...
	public static function hasGUID( $po_request ) {
		...
		if ( ( ! is_array( $va_guids_to_check ) || ! sizeof( $va_guids_to_check ) )
		     && ( $vs_guid
				= $po_request->getParameter( 'guid', pString ) )
		) {
	...
	public static function hasAccess( $po_request ) {
		...
		if ( is_array( $va_guids_to_check ) ) {
			foreach ( $va_guids_to_check as $vs_guid ) {
				...
				$va_results[ $vs_guid ] = is_null( $vn_access_for_guid ) ? '?'
					: ( in_array( $vn_access_for_guid, $va_access ) ? 1 : 0 );
			}
		}
