...
class ItemService extends BaseJSONService {
	...
	public function dispatch() {
		switch ( $this->getRequestMethod() ) {
			case "GET":
			case "POST":
				if ( strlen( $this->opn_id )
				     > 0
				) {    // we allow that this might be a string here for idno-based fetching
			...
			case "PUT":
				...
				if ( strlen( $this->opn_id )
				     > 0
				) {   // we allow that this might be a string here for idno-based updating
	...
	protected function getSpecificItemInfo() {
		...
		foreach ( $va_post["bundles"] as $vs_bundle => $va_options ) {
			...
			if ( caGetOption( 'coordinates', $va_options,
				true )
			) { // Geocode attribute "coordinates" option returns an array which we want to serialize in the response, so we'll need to get it as a structured return value
			...
			if ( caGetOption( 'returnWithStructure', $va_options,
				true )
			) {    // unroll structured response into flat list
	...
	protected function getAllItemInfo() {
		...
		$va_get_options = array_merge( array( "returnWithStructure" => true, "returnAllLocales" => true ),
			$va_get_options );
			...
			if ( $va_vals = $t_instance->get(
				...
				foreach ( $va_vals_by_locale as $vn_locale_id => $va_locale_vals ) {
					...
					foreach ( $va_locale_vals as $vs_val_id => $va_actual_data ) {
						$vs_locale_code                                       = isset( $va_locales[ $vn_locale_id ]["code"] )
							? $va_locales[ $vn_locale_id ]["code"] : "none";
				...
				if ( $t_rel_instance instanceof RepresentableBaseModel ) {
					foreach ( $va_related_items as &$va_rel_item ) {
						if ( $t_rel_instance->load( $va_rel_item[ $t_rel_instance->primaryKey() ] ) ) {
							$va_rel_item['representations']
								= caSanitizeArray( $t_rel_instance->getRepresentations( $va_versions ),
								[ 'removeNonCharacterData' => true ] );
						}
					}
				}
	...
	private function getItemInfoForEdit() {
		...
		// preferred labels
		$va_labels = $t_instance->get( $this->ops_table . ".preferred_labels",
			array( "returnWithStructure" => true, "returnAllLocales" => true, "assumeDisplayField" => false ) );
		$va_labels = end( $va_labels );
		if ( is_array( $va_labels ) ) {
			foreach ( $va_labels as $vn_locale_id => $va_labels_by_locale ) {
		...
		// nonpreferred labels
		$va_labels = $t_instance->get( $this->ops_table . ".nonpreferred_labels",
			array( "returnWithStructure" => true, "returnAllLocales" => true, "assumeDisplayField" => false ) );
		$va_labels = end( $va_labels );
		if ( is_array( $va_labels ) ) {
			foreach ( $va_labels as $vn_locale_id => $va_labels_by_locale ) {
		...
		foreach ( $va_codes as $vs_code ) {
			if ( $va_vals = $t_instance->get( $this->ops_table . "." . $vs_code,
				...
				$va_vals_by_locale
					= end( $va_vals ); // I seriously have no idea what that additional level of nesting in the return format is for
				foreach ( $va_vals_by_locale as $vn_locale_id => $va_locale_vals ) {
					foreach ( $va_locale_vals as $vs_val_id => $va_actual_data ) {
						...
						$va_return['attributes'][ $vs_code ][] = array_merge( array( 'locale' => $vs_locale_code ),
							$va_actual_data );
					}
	...
	private function getItemInfoForImport() {
		...
		// preferred labels
		$va_labels = $t_instance->get( $this->ops_table . ".preferred_labels",
			array( "returnWithStructure" => true, "assumeDisplayField" => false, "returnAllLocales" => true ) );
		$va_labels = end( $va_labels );
		...
		if ( is_array( $va_labels ) ) {
			...
			if ( isset( $va_flatten['locales'] ) ) {
				$va_return["preferred_labels"]
					= array_pop( caExtractValuesByUserLocale( array( $va_return["preferred_labels"] ) ) );
			}
		} else {
			$va_return["preferred_labels"] = [];
		}
		...
		$va_labels = $t_instance->get( $this->ops_table . ".hierarchy.preferred_labels",
			array( "returnWithStructure" => true, "assumeDisplayField" => true, "returnAllLocales" => true ) );
		$va_labels = end( $va_labels );
		...
		if ( is_array( $va_labels ) ) {
			...
			if ( isset( $va_flatten['locales'] ) ) {
				$va_return["preferred_labels_hierarchy"]
					= array_pop( caExtractValuesByUserLocale( array( $va_return["preferred_labels_hierarchy"] ) ) );
			}
		} else {
			$va_return["preferred_labels_hierarchy"] = [];
		}
		...
		if ( is_array( $va_labels ) ) {
			...
			if ( isset( $va_flatten['locales'] ) ) {
				$va_return["nonpreferred_labels"]
					= array_pop( caExtractValuesByUserLocale( array( $va_return["nonpreferred_labels"] ) ) );
			}
		} else {
			$va_return["nonpreferred_labels"] = [];
		}
				...
				foreach ( $va_vals_by_locale as $vn_locale_id => $va_locale_vals ) {
					foreach ( $va_locale_vals as $vs_val_id => $va_actual_data ) {
						...
						foreach ( $va_actual_data as $vs_f => $vs_v ) {
							if ( isset( $va_vals_as_text[ $vn_locale_id ][ $vs_val_id ][ $vs_f ] )
							     && ( $vs_v != $va_vals_as_text[ $vn_locale_id ][ $vs_val_id ][ $vs_f ] )
							) {
								$va_actual_data[ $vs_f . '_display' ]
									= $va_vals_as_text[ $vn_locale_id ][ $vs_val_id ][ $vs_f ];
						...
						$va_return['attributes'][ $vs_code ][ $vs_locale_code ][]
							= array_merge( array( 'locale' => $vs_locale_code ), $va_actual_data );
					}
				}
			}
		}
		...
		foreach ( $this->opa_valid_tables as $vs_rel_table ) {
			...
			// set-related hacks
			if ( ( $this->ops_table == "ca_sets" )
			     && ( $vs_rel_table == "ca_tours" )
			) { // throw SQL error in getRelatedItems
				continue;
			}

			$va_related_items = $t_instance->getRelatedItems( $vs_rel_table, array(
				'returnWithStructure' => true,
				'returnAsArray'       => true,
				'useLocaleCodes'      => true,
				'groupFields'         => true
			...
			if ( ( $this->ops_table == "ca_objects" ) && ( $vs_rel_table == "ca_object_representations" ) ) {
				...
				if ( isset( $va_flatten['all'] ) ) {
					...
					$va_return['representations'] = caSanitizeArray( $t_instance->getRepresentations( [ 'original' ] ),
						[ 'removeNonCharacterData' => true ] );
				}
				...
				foreach ( $va_related_items as $va_rel_item ) {
					...
					foreach ( $va_rel_item as $vs_fld => $vs_val ) {
						...
						if ( $vs_fld == 'preferred_labels' ) {
							$q = caMakeSearchResult( $vs_rel_table,
								[ $va_rel_item[ Datamodel::primaryKey( $vs_rel_table ) ] ] );
							if ( $q->nextHit() ) {
								$va_item_add['preferred_labels_hierarchy']
									= $q->get( "{$vs_rel_table}.hierarchy.preferred_labels",
									[ 'returnAsArray' => true ] );
							}
	...
	public function addItem( $ps_table = null, $pa_data = null ) {
		...
		if ( is_array( $pa_data["related"] ) && sizeof( $pa_data["related"] ) > 0 ) {
			foreach ( $pa_data["related"] as $vs_table => $va_relationships ) {
				if ( $vs_table == 'ca_sets' ) {
					...
					foreach ( $va_relationships as $va_relationship ) {
						$vs_source_info    = isset( $va_relationship["source_info"] ) ? $va_relationship["source_info"]
							: null;
						$vs_effective_date = isset( $va_relationship["effective_date"] )
							? $va_relationship["effective_date"] : null;
						$vs_direction      = isset( $va_relationship["direction"] ) ? $va_relationship["direction"]
							: null;

						$t_rel_instance = $this->_getTableInstance( $vs_table );

						$vs_pk      = isset( $va_relationship[ $t_rel_instance->primaryKey() ] )
							? $va_relationship[ $t_rel_instance->primaryKey() ] : null;
						$vs_type_id = isset( $va_relationship["type_id"] ) ? $va_relationship["type_id"] : null;

						$t_rel = $t_instance->addRelationship( $vs_table, $vs_pk, $vs_type_id, $vs_effective_date,
							$vs_source_info, $vs_direction );

						// deal with interstitial attributes
						if ( $t_rel instanceof BaseRelationshipModel ) {

							$vb_have_to_update = false;
							if ( is_array( $va_relationship["attributes"] )
							     && sizeof( $va_relationship["attributes"] )
							) {
			...
			if ( ( $t_instance instanceof RepresentableBaseModel ) && isset( $pa_data['representations'] )
				...
				foreach ( $pa_data['representations'] as $va_rep ) {
					if ( ! isset( $va_rep['media'] )
					     || ( ! file_exists( $va_rep['media'] )
					          && ! isURL( $va_rep['media'] ) )
					) {
						continue;
					}
		...
		if ( ( $ps_table == 'ca_sets' ) && is_array( $pa_data["set_content"] )
		     && sizeof( $pa_data["set_content"] ) > 0
		) {
			...
			if ( $t_set_table = Datamodel::getInstance( $vn_table_num ) ) {
				...
				foreach ( $pa_data["set_content"] as $vs_idno ) {
					if ( $vn_set_item_id = $vs_set_table::find( [ 'idno' => $vs_idno ],
						[ 'returnAs' => 'firstId' ] )
					) {
		...
		if ( $t_instance->getAppConfig()->get( 'perform_item_level_access_checking' )
		     && ! $t_instance->getAppConfig()->get( "{$ps_table}_dont_do_item_level_access_control" )
		) {
	...
	private function editItem( $ps_table = null ) {
		...
		if ( ( $ps_table == 'ca_sets' ) && is_array( $va_post["set_content"] )
		     && sizeof( $va_post["set_content"] ) > 0
		) {
			$vn_table_num = $t_instance->get( 'table_num' );
			if ( $t_set_table = Datamodel::getInstance( $vn_table_num ) ) {
				$vs_set_table = $t_set_table->tableName();
