...
class ItemService extends BaseJSONService
{
    ...
    {
        if (!($t_instance = $this->_getTableInstance(
            $this->ops_table,
            $this->opn_id
    ...
    protected function getAllItemInfo()
    {
        ...
        if ($vs_template) {
            $va_return['display'] = caProcessTemplateForIDs(
                $vs_template,
                $this->ops_table,
                array($this->opn_id),
                $va_get_options
                ...
                if ($t_rel_instance instanceof RepresentableBaseModel) {
                    foreach ($va_related_items as &$va_rel_item) {
                        if ($t_rel_instance->load($va_rel_item[$t_rel_instance->primaryKey()])) {
                            $va_rel_item['representations'] = caSanitizeArray(
                                $t_rel_instance->getRepresentations($va_versions),
    ...
    private function getItemInfoForEdit()
    {
        ...
        // preferred labels
        $va_labels = $t_instance->get(
            $this->ops_table . ".preferred_labels",
            array(
        ...
        // nonpreferred labels
        $va_labels = $t_instance->get(
            $this->ops_table . ".nonpreferred_labels",
            array(
                "returnWithStructure" => true,
                "returnAllLocales" => true,
                "assumeDisplayField" => false
            ...
            if ($va_vals = $t_instance->get(
                ...
                foreach ($va_vals_by_locale as $vn_locale_id => $va_locale_vals) {
                    foreach ($va_locale_vals as $vs_val_id => $va_actual_data) {
                        ...
                        $va_return['attributes'][$vs_code][] = array_merge(
                            array('locale' => $vs_locale_code),
                            $va_actual_data
                        );
                    }
                }
    ...
    private function getItemInfoForImport()
    {
        ...
        if (is_array($va_labels)) {
            ...
            if (isset($va_flatten['locales'])) {
                $va_return["preferred_labels"] = array_pop(
                    caExtractValuesByUserLocale(array($va_return["preferred_labels"]))
                );
            }
        } else {
        ...
        $va_labels = $t_instance->get(
            $this->ops_table . ".hierarchy.preferred_labels",
            array(
                "returnWithStructure" => true,
        ...
        if (is_array($va_labels)) {
            ...
            if (isset($va_flatten['locales'])) {
                $va_return["preferred_labels_hierarchy"] = array_pop(
                    caExtractValuesByUserLocale(array($va_return["preferred_labels_hierarchy"]))
                );
            }
        } else {
        ...
        if (is_array($va_labels)) {
            ...
            if (isset($va_flatten['locales'])) {
                $va_return["nonpreferred_labels"] = array_pop(
                    caExtractValuesByUserLocale(array($va_return["nonpreferred_labels"]))
                );
            }
        } else {
            ...
            if ($va_vals = $t_instance->get(
                ...
                foreach ($va_vals_by_locale as $vn_locale_id => $va_locale_vals) {
                    foreach ($va_locale_vals as $vs_val_id => $va_actual_data) {
                        ...
                        $va_return['attributes'][$vs_code][$vs_locale_code][] = array_merge(
                            array('locale' => $vs_locale_code),
                            $va_actual_data
                        );
                    }
        ...
        foreach ($this->opa_valid_tables as $vs_rel_table) {
            ...
            if (($this->ops_table == "ca_objects") && ($vs_rel_table == "ca_object_representations")) {
                ...
                if (isset($va_flatten['all'])) {
                    ...
                    $va_return['representations'] = caSanitizeArray(
                        $t_instance->getRepresentations(['original']),
                        ['removeNonCharacterData' => true]
                    );
                }
                ...
                foreach ($va_related_items as $va_rel_item) {
                    ...
                    foreach ($va_rel_item as $vs_fld => $vs_val) {
                        ...
                        if ($vs_fld == 'preferred_labels') {
                            $q = caMakeSearchResult(
                                $vs_rel_table,
                                [$va_rel_item[Datamodel::primaryKey($vs_rel_table)]]
                            );
                            if ($q->nextHit()) {
                                $va_item_add['preferred_labels_hierarchy'] = $q->get(
                                    "{$vs_rel_table}.hierarchy.preferred_labels",
                                    ['returnAsArray' => true]
                                );
                            }
                        }
    ...
    public function addItem($ps_table = null, $pa_data = null)
    {
        ...
        if (is_array($pa_data["related"]) && sizeof($pa_data["related"]) > 0) {
            ...
            if (($t_instance instanceof RepresentableBaseModel) && isset($pa_data['representations']) && is_array(
                    $pa_data['representations']
                )) {
        ...
        if ($t_instance->getAppConfig()->get('perform_item_level_access_checking') && !$t_instance->getAppConfig()->get(
                "{$ps_table}_dont_do_item_level_access_control"
            )) {
