...
class OAIPMHService extends BaseService {
	...
	public function __construct( $po_request, $ps_provider ) {
		...
		$o_root_node->setAttributeNS( self::XML_SCHEMA_NAMESPACE_URI, 'xsi:schemaLocation',
		...
		$this->_baseUrl   = $this->config->get( 'site_host' ) . $this->config->get( 'ca_url_root' )
		                    . '/service.php/OAI/' . $this->ops_provider;
	...
	private function getRecord( $oaiData ) {
		...
		$va_access_values                = caGetUserAccessValues( $this->opo_request,
			array_merge( $this->opa_provider_info, array( 'ignoreProvidence' => true ) ) );
	...
	private function getIdForIdno( $facet_name, $idno ) {
		...
		switch ( $facet_info['type'] ) {
			case 'authority':
				...
				if ( $id = $t::find( [ Datamodel::getTableProperty( $t, 'ID_NUMBERING_ID_FIELD' ) => $idno ],
					[ 'returnAs' => 'firstId' ] )
				) {
					return $id;
				} else {
	...
	private function listSets( $oaiData ) {
		$va_access_values                = caGetUserAccessValues( $this->opo_request,
			array_merge( $this->opa_provider_info, [ 'ignoreProvidence' => true ] ) );
		$vb_show_deleted                 = (bool) $this->opa_provider_info['show_deleted'];
		$vb_dont_enforce_access_settings = (bool) $this->opa_provider_info['dont_enforce_access_settings'];
		...
		if ( $vs_facet_name = $this->opa_provider_info['setFacet'] ) {
			...
			$o_browse->execute( array(
				'no_cache'    => $vb_dont_cache,
				'showDeleted' => $vb_show_deleted,
				'checkAccess' => $vb_dont_enforce_access_settings ? null : $va_access_values
			) );
			$va_facet = $o_browse->getFacet( $vs_facet_name,
				array( 'checkAccess' => ( $vb_dont_enforce_access_settings ? null : $va_access_values ) ) );
	...
	private function listResponse( $oaiData, $verb, $metadataPrefix, $cursor, $set, $from, $until ) {
		...
		if ( ( $set && $this->opa_provider_info['setFacet'] ) || $vs_range ) {
			if ( $vs_query === '*' ) {
				...
				$qr_res = $o_search->search( $vs_range ? 'modified:"' . $vs_range . '"' : '*', array(
					'showDeleted'     => $vb_show_deleted,
					'no_cache'        => $vb_dont_cache,
					'checkAccess'     => $vb_dont_enforce_access_settings ? null : $va_access_values,
					'dontFilterByACL' => $this->opa_provider_info['dontFilterByACL']
				) );
			} else {
				$o_browse = caGetBrowseInstance( $this->table );
			...
			$qr_res = $o_search->search( strlen( $this->opa_provider_info['query'] ) ? $this->opa_provider_info['query']
				: "*", array(
				'no_cache'          => $vb_dont_cache,
				'limitToModifiedOn' => $vs_range,
				'showDeleted'       => $vb_show_deleted,
		...
		if ( $qr_res->numHits() == 0 ) {
			...
			if ( $vb_show_deleted ) {
				...
				$qr_res->seek( 0 );
				$va_deleted_timestamps = $t_change_log->getDeleteOnTimestampsForIDs( $vs_table,
					array_keys( $va_get_deleted_timestamps_for ) );
			}

			// Export data using metadata mapping
			$va_items
				= ca_data_exporters::exportRecordsFromSearchResultToArray( $this->getMappingCode( $metadataPrefix ),
				$qr_res, array(
					'start'           => $cursor,
					'limit'           => $listLimit,
					'dontFilterByACL' => $this->opa_provider_info['dontFilterByACL']
				...
				foreach ( $va_items as $vn_id => $vs_item_xml ) {
					...
					if ( $vb_show_deleted && $va_deleted_items[ $vn_id ] ) {
						$headerData = array(
							...
							'datestamp'  => self::unixToUtc( $va_deleted_timestamps[ $vn_id ]['timestamp']
								? $va_deleted_timestamps[ $vn_id ]['timestamp']
								: $va_timestamps[ $vn_id ]['timestamp'] )
						);
						...
						if ( $verb == 'ListIdentifiers' ) {
							...
							$header        = $this->createElementWithChildren( $oaiData, $recordElement, 'header',
								$headerData );
							$header->setAttribute( "status", "deleted" );
						}
					} else {
						...
						if ( $verb == 'ListIdentifiers' ) {
							...
							if ( $o_doc_src ) { // just in case the xml fails to load through DomDocument for some reason (e.g. a bad mapping or very weird characters)
								$metadataElement->appendChild( $oaiData->importNode( $o_doc_src->documentElement,
									true ) );
							}
						}
					}
	...
	private function checkParameters( $pa_required_parameters, $pa_optional_parameters ) {
		...
		if ( $_SERVER['REQUEST_METHOD'] == 'GET'
		     && ( urldecode( $_SERVER['QUERY_STRING'] ) != urldecode( http_build_query( $_REQUEST ) ) )
		) {
		...
		if ( ( ! ( $vs_metadata_prefix = $this->opo_request->getParameter( 'metadataPrefix', pString ) ) )
		     && ( ( in_array( 'metadataPrefix', $pa_required_parameters ) )
		          || ( in_array( 'metadataPrefix', $pa_optional_parameters ) ) )
		     && isset( $this->opa_provider_info['default_format'] )
		) {
		...
		if ( $vs_mapping = $this->getMappingCode() ) {
			if ( $this->exporter = ca_data_exporters::loadExporterByCode( $vs_mapping ) ) {
				if ( $this->exporter->getSetting( 'exporter_format' ) != "XML" ) {
					$this->throwError( self::OAI_ERR_BAD_ARGUMENT,
						_t( "Selected mapping %1 is invalid", $vs_mapping ) );
				}

				$this->table = Datamodel::getTableName( $this->exporter->get( 'table_num' ) );
			} else {
				$this->throwError( self::OAI_ERR_CANNOT_DISSEMINATE_FORMAT,
					_t( "Exporter with code %1 does not exist", $vs_mapping ) );
			}
		} else {
			$this->throwError( self::OAI_ERR_CANNOT_DISSEMINATE_FORMAT,
				_t( "metadataPrefix or default_format is invalid", $vs_mapping ) );
		}
