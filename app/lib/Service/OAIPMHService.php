...
class OAIPMHService extends BaseService
{
    ...
    public function __construct($po_request, $ps_provider)
    {
        ...
        $this->_baseUrl = $this->config->get('site_host') . $this->config->get(
                'ca_url_root'
    ...
    private function getRecord($oaiData)
    {
        ...
        $va_access_values = caGetUserAccessValues(
            $this->opo_request,
            array_merge(
                $this->opa_provider_info,
                array('ignoreProvidence' => true)
            )
        );
        ...
        switch ($facet_info['type']) {
            case 'authority':
                ...
                if ((bool)$this->opa_provider_info['useIdentifiersForSets']) {
                    return $t_instance->getFieldValuesForIDs(
                        array_map(
                            function ($v) {
                                return $v['id'];
                            },
    ...
    private function getIdForIdno($facet_name, $idno)
    {
        ...
        switch ($facet_info['type']) {
            case 'authority':
                ...
                if ($id = $t::find(
                    [Datamodel::getTableProperty($t, 'ID_NUMBERING_ID_FIELD') => $idno],
                    ['returnAs' => 'firstId']
                )) {
                    return $id;
                } else {
    ...
    private function listResponse($oaiData, $verb, $metadataPrefix, $cursor, $set, $from, $until)
    {
        ...
        if (($set && $this->opa_provider_info['setFacet']) || $vs_range) {
            ...
            $qr_res = $o_search->search(
                strlen($this->opa_provider_info['query']) ? $this->opa_provider_info['query'] : "*",
                array(
                    'no_cache' => $vb_dont_cache,
                    'limitToModifiedOn' => $vs_range,
                    'showDeleted' => $vb_show_deleted,
        ...
        if ($qr_res->numHits() == 0) {
            ...
            if ($vb_show_deleted) {
                ...
                $qr_res->seek(0);
                $va_deleted_timestamps = $t_change_log->getDeleteOnTimestampsForIDs(
                    $vs_table,
                    array_keys(
                        $va_get_deleted_timestamps_for
                    )
                );
            }

            // Export data using metadata mapping
            $va_items = ca_data_exporters::exportRecordsFromSearchResultToArray(
                $this->getMappingCode($metadataPrefix),
                $qr_res,
                array(
                    'start' => $cursor,
                    'limit' => $listLimit,
                ...
                foreach ($va_items as $vn_id => $vs_item_xml) {
                    ...
                    if ($vb_show_deleted && $va_deleted_items[$vn_id]) {
                        $headerData = array(
                            ...
                            'datestamp' => self::unixToUtc(
                                $va_deleted_timestamps[$vn_id]['timestamp'] ? $va_deleted_timestamps[$vn_id]['timestamp'] : $va_timestamps[$vn_id]['timestamp']
                            )
                        );
     ...
     *
     * @return bool True if parameters are valid, fase if not.
     */
    private function checkParameters($pa_required_parameters, $pa_optional_parameters)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'GET' && (urldecode($_SERVER['QUERY_STRING']) != urldecode(
                    http_build_query($_REQUEST)
                ))) {
            $this->throwError(self::OAI_ERR_BAD_ARGUMENT, _t("Duplicate parameters in request"));
        }

        if ((!($vs_metadata_prefix = $this->opo_request->getParameter('metadataPrefix', pString))) && ((in_array(
                    'metadataPrefix',
                    $pa_required_parameters
                )) || (in_array(
                    'metadataPrefix',
                    $pa_optional_parameters
        ...
        if ($vs_metadata_prefix) {
            if (!in_array($vs_metadata_prefix, array_keys($this->opa_provider_info['formats']))) {
                $this->throwError(
                    self::OAI_ERR_CANNOT_DISSEMINATE_FORMAT,
    ...
    public static function getGranularity($ps_datetime)
    {
        if (preg_match(self::OAI_DATE_PCRE, $ps_datetime)) {
            ...
            if (preg_match(self::OAI_DATETIME_PCRE, $ps_datetime)) {
