...
class UserContentService extends BaseService
{
    ...
    public function addTag($type, $item_id, $tag_info_array)
    {
        ...
        if ($t_subject_instance->numErrors() == 0) {
            ...
            throw new SoapFault(
                "Server",
                "There were errors while adding the comment: " . join(
                    ";",
    ...
    public function updateSet($set_id, $set_info_array)
    {
        ...
        if ($t_set->numErrors() == 0) {
            ...
            throw new SoapFault(
                "Server", "There were errors while updating the set: " . join(";", $t_set->getErrors())
            );
        }
    }
    # -------------------------------------------------------
    ...
    public function removeSet($set_id)
    {
        ...
        if ($t_set->numErrors() == 0) {
            ...
            throw new SoapFault(
                "Server", "There were errors while deleting the set: " . join(";", $t_set->getErrors())
            );
        }
    }
    # -------------------------------------------------------
