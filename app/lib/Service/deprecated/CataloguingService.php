...
class CataloguingService extends BaseService
{
    ...
    public function add($type, $fieldInfo)
    {
        ...
        if (is_array($fieldInfo)) {
            ...
            if ($t_subject_instance->numErrors() == 0) {
                ...
                throw new SoapFault(
                    "Server",
                    "There were errors while inserting the new record: " . join(
                        ";",
    ...
    public function update($type, $item_id, $fieldInfo)
    {
        ...
        if (is_array($fieldInfo)) {
            ...
            if ($vb_success && $t_subject_instance->numErrors() == 0) {
                ...
                throw new SoapFault(
                    "Server",
                    "There were errors while updating the record: " . join(
                        ";",
                        $t_subject_instance->getErrors()
                    )
                );
    ...
    public function remove($type, $item_id)
    {
        ...
        if ($vb_success && $t_subject_instance->numErrors() == 0) {
            ...
            throw new SoapFault(
                "Server",
                "There were errors while deleting the record: " . join(
                    ";",
                    $t_subject_instance->getErrors()
                )
            );
    ...
    public function addAttribute($type, $item_id, $attribute_code_or_id, $attribute_data_array)
    {
        ...
        if ($t_subject_instance instanceof BaseModelWithAttributes) {
            ...
            if ($t_subject_instance->numErrors() == 0) {
                ...
                throw new SoapFault(
                    "Server",
                    "There were errors while adding the attribute: " . join(
                        ";",
                        $t_subject_instance->getErrors()
                    )
                );
    ...
    public function addAttributes($type, $item_id, $attribute_list_array)
    {
        ...
        if ($t_subject_instance instanceof BaseModelWithAttributes) {
            ...
            if ($t_subject_instance->numErrors() == 0) {
                ...
                throw new SoapFault(
                    "Server",
                    "There were errors while adding an attribute: " . join(
                        ";",
                        $t_subject_instance->getErrors()
                    )
                );
    ...
    public function editAttribute($type, $item_id, $attribute_id, $attribute_code_or_id, $attribute_data_array)
    {
        ...
        if ($t_subject_instance instanceof BaseModelWithAttributes) {
            ...
            if ($t_subject_instance->numErrors() == 0) {
                ...
                throw new SoapFault(
                    "Server",
                    "There were errors while editing the attribute: " . join(
                        ";",
                        $t_subject_instance->getErrors()
                    )
                );
    ...
    public function replaceAttribute($type, $item_id, $attribute_code_or_id, $attribute_data_array)
    {
        ...
        if ($t_subject_instance instanceof BaseModelWithAttributes) {
            ...
            if ($t_subject_instance->numErrors() == 0) {
                ...
                throw new SoapFault(
                    "Server",
                    "There were errors while removing an attribute: " . join(
                        ";",
                        $t_subject_instance->getErrors()
                    )
                );
        ...
        if ($t_subject_instance instanceof BaseModelWithAttributes) {
            if (is_array($attribute_list_array)) {
                ...
                if ($t_subject_instance->numErrors() == 0) {
                    ...
                    throw new SoapFault(
                        "Server",
                        "There were errors while modifying attribute: " . join(
                            ";",
                            $t_subject_instance->getErrors()
                        )
                    );
    ...
    public function removeAttribute($type, $item_id, $attribute_id)
    {
        ...
        if ($t_subject_instance instanceof BaseModelWithAttributes) {
            ...
            if ($t_subject_instance->numErrors() == 0) {
                ...
                throw new SoapFault(
                    "Server",
                    "There were errors while deleting attribute: " . join(
                        ";",
                        $t_subject_instance->getErrors()
                    )
                );
    ...
    public function removeAttributes($type, $item_id, $attribute_code_or_id)
    {
        ...
        if ($t_subject_instance instanceof BaseModelWithAttributes) {
            ...
            if ($t_subject_instance->numErrors() == 0) {
                ...
                throw new SoapFault(
                    "Server",
                    "There were errors while deleting attributes: " . join(
                        ";",
                        $t_subject_instance->getErrors()
                    )
                );
    ...
    public function addLabel($type, $item_id, $label_data_array, $localeID, $is_preferred)
    {
        ...
        if ($t_subject_instance instanceof LabelableBaseModelWithAttributes) {
            ...
            if ($t_subject_instance->numErrors() == 0 && intval($vn_id) > 0) {
                ...
                throw new SoapFault(
                    "Server",
                    "There were errors while adding the label: " . join(
                        ";",
                        $t_subject_instance->getErrors()
                    )
                );
    ...
    public function removeLabel($type, $item_id, $label_id)
    {
        ...
        if ($t_subject_instance instanceof LabelableBaseModelWithAttributes) {
            ...
            if ($t_subject_instance->numErrors() == 0) {
                ...
                throw new SoapFault(
                    "Server",
                    "There were errors while removing the label: " . join(
                        ";",
                        $t_subject_instance->getErrors()
                    )
                );
    ...
    public function removeAllLabels($type, $item_id)
    {
        ...
        if ($t_subject_instance instanceof LabelableBaseModelWithAttributes) {
            ...
            if ($t_subject_instance->numErrors() == 0) {
                ...
                throw new SoapFault(
                    "Server",
                    "There were errors while removing a label: " . join(
                        ";",
                        $t_subject_instance->getErrors()
                    )
                );
    ...
    public function addObjectRepresentation($object_id, $media_url, $type_id, $locale_id, $status, $access, $is_primary)
    {
        ...
        $vn_id = $t_subject_instance->addRepresentation(
            $vs_tmpfile,
            $type_id,
            $locale_id,
            $status,
        ...
        if ($t_subject_instance->numErrors() == 0) {
            ...
            throw new SoapFault(
                "Server",
                "There were errors while adding the representation: " . join(
                    ";",
                    $t_subject_instance->getErrors()
                )
            );
    ...
    public function addRelationship(
        ...
        $vn_return = $t_subject_instance->addRelationship(
            $related_type,
            $related_item_id,
            $relationship_type_id,
            null,
        ...
        if ($vn_return && $t_subject_instance->numErrors() == 0) {
            ...
            throw new SoapFault(
                "Server",
                "There were errors while adding the relationship: " . join(
                    ";",
                    $t_subject_instance->getErrors()
                )
            );
    ...
    public function updateRelationship($type, $related_type, $relation_id, $relationship_type_id, $source_info)
    {
        ...
        if ($t_rel_instance->numErrors() == 0) {
            ...
            throw new SoapFault(
                "Server",
                "There were errors while modifying the relationship: " . join(
                    ";",
                    $t_rel_instance->getErrors()
                )
            );
        }
    }
    ...
    public function removeRelationship($type, $related_type, $relation_id)
    {
        ...
        if ($t_instance->numErrors() == 0) {
            ...
            throw new SoapFault(
                "Server",
                "There were errors while deleting the relationship: " . join(
                    ";",
                    $t_instance->getErrors()
                )
            );
        }
    }
