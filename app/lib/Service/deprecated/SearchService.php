...
class SearchService extends BaseService {
	 ...
	 *                                   "ca_storage_locations", "ca_movements", "ca_loans", "ca_tours",
	 *                                   "ca_tour_stops"
	 * @param string $query              the search query
	 * @param array  $additional_bundles associated array which defines the additional data to get. It must be an array
	 *                                   of
	 ...
	 *    )
	 *
	 *    For a list of available options @return DOMDocument
	 *
	 * @throws SoapFault
	 * @see ItemInfo::get().
	 */
	public function queryRest( $type, $query, $additional_bundles ) {
		...
		while ( $vo_result->nextHit() ) {
			...
			if ( is_array( $va_display_labels = $vo_result->getDisplayLabels() ) ) {
				$vo_display_label = $vo_dom->createElement( "displayLabel",
					caEscapeForXML( array_pop( $va_display_labels ) ) );
			...
			if ( is_array( $additional_bundles ) ) {
				foreach ( $additional_bundles as $vs_bundle => $va_options ) {
					...
					$vo_bundle = $vo_dom->createElement( $vs_bundle,
						htmlspecialchars( $vo_result->get( $vs_bundle, $va_options ) ) );
