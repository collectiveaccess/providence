...
class ItemInfoService extends BaseService {
	 ...
	 * @param array  $options  associative array of option arrays to pass through options to BaseModel::get() for each
	 *                         bundle. For example (corresponding to the example above): array(
	 ...
	 *                         (string) values: COLON_DELIMITED, HOURS_MINUTES_SECONDS, RAW; data will be passed
	 *                         through floatval() by default
	 ...
	 *                         -DONT_STRIP_SLASHES: if set to true, return value will not be passed through
	 *                         stripslashes()
	 *                         -template: formatting string to use for returned value; ^<fieldname> placeholder is used
	 *                         to represent field value in template
	 *                         -returnAsArray: if true, fields that can return multiple values [currently only
	 *                         <table_name>.children.<field>] will return values in an indexed array; default is false
	 *                         -returnAllLocales:
	 *                         -delimiter: if set, value is used as delimiter when fields that can return multiple
	 *                         fields are returned as strings; default is a single space
	 *                         -convertCodesToDisplayText: if set, id values refering to foreign keys are returned as
	 *                         preferred label text in the current locale
	 *
			...
			while ( $qr_result->nextHit() ) {
				foreach ( $bundles as $vs_bundle ) {
					...
					if ( isset( $options[ $vs_bundle ] ) ) {
						$va_return[ $qr_result->get( $t_subject_instance->primaryKey() ) ][ $vs_bundle ]
							= $qr_result->get( $vs_bundle, $options[ $vs_bundle ] );
					} else {
						$va_return[ $qr_result->get( $t_subject_instance->primaryKey() ) ][ $vs_bundle ]
							= $qr_result->get( $vs_bundle );
					}
				}
			}
		}
	 ...
	 *
	 * @param string $type    can be one of: ["ca_objects", "ca_object_lots", "ca_entities", "ca_places",
	 *                        "ca_occurrences", "ca_collections", "ca_list_items", "ca_object_representations",
	 *                        "ca_storage_locations", "ca_movements", "ca_loans", "ca_tours", "ca_tour_stops"]
	 ...
	 *
	 * @param string  $type                      can be one of: "ca_objects", "ca_entities", "ca_places",
	 *                                           "ca_occurrences", "ca_collections", "ca_list_items",
	 *                                           "ca_object_representations", "ca_storage_locations", "ca_movements",
	...
	/**
	 *
	 * @param string $type    can be one of: ["ca_objects", "ca_entities", "ca_places", "ca_occurrences",
	 *                        "ca_collections", "ca_list_items", "ca_object_representations", "ca_storage_locations",
	 *                        "ca_movements", "ca_loans", "ca_tours", "ca_tour_stops"]
	 * @param int    $item_id primary key
	 *
	 * @return array
	 * @throws SoapFault
	...
	/**
	 * Returns text of attributes in the user's currently selected locale, or else falls back to whatever locale is
	 * available
	 *
	 * @param string $type    can be one of: ["ca_objects", "ca_entities", "ca_places", "ca_occurrences",
	 *                        "ca_collections", "ca_list_items", "ca_object_representations", "ca_storage_locations",
	 ...
	 *                        convertLinkBreaks = if true will convert line breaks to HTML <br/> tags for display in a
	...
	public function getAttributesForDisplay( $type, $item_id, $attribute_code_or_id, $template = null, $options = null
		...
		return $t_subject_instance->getAttributesForDisplay( $attribute_code_or_id, $ps_template = null,
			$pa_options = null );
	}
	# -------------------------------------------------------
	 ...
	 *
	 * @param string $type         can be one of: ["ca_objects", "ca_entities", "ca_places", "ca_occurrences",
	 *                             "ca_collections", "ca_list_items", "ca_object_representations",
	 *                             "ca_storage_locations", "ca_movements", "ca_loans", "ca_tours", "ca_tour_stops"]
	 * @param int    $item_id      primary key
	 * @param string $related_type can be one of: ["ca_objects", "ca_entities", "ca_places", "ca_occurrences",
	 *                             "ca_collections", "ca_list_items", "ca_object_representations",
	 *                             "ca_storage_locations", "ca_movements", "ca_loans", "ca_tours", "ca_tour_stops"]
	 * @param array options Supported options:
	 *                             restrict_to_type = restricts returned items to those of the specified type; only
	 *                             supports a single type which can be specified as a list item_code or item_id
	 *                             restrictToType = synonym for restrict_to_type restrict_to_types = restricts returned
	 *                             items to those of the specified types; pass an array of list item_codes or item_ids
	 *                             restrictToTypes = synonym for restrict_to_types
	 *                             dont_include_subtypes_in_type_restriction = if set subtypes are not included when
	 *                             enforcing restrict_to_types. Note that restrict_to_relationship_types always
	 *                             includes subtypes in its restriction. dontIncludeSubtypesInTypeRestriction = synonym
	 *                             for dont_include_subtypes_in_type_restriction restrict_to_relationship_types =
	 *                             restricts returned items to those related to the current row by the specified
	 *                             relationship type(s). You can pass either an array of types or a single type. The types can be relationship type_code's or type_id's. restrictToRelationshipTypes = synonym for restrict_to_relationship_types
	 *
	 *        exclude_relationship_types - omits any items related to the current row with any of the specified types
	 *        from the returned set of its. You can pass either an array of types or a single type. The types can be
	 *        relationship type_code's or type_id's. excludeRelationshipTypes = synonym for exclude_relationship_types
	 *        exclude_type = excludes returned items of the specified type; only supports a single type which can be
	 *        specified as a list item_code or item_id excludeType = synonym for exclude_type exclude_types = omits any
	 *        items related to the current row that are of any of the specified types from the returned set of ids. You
	 *        can pass either an array of types or a single type. The types can be type_code's or type_id's.
	 ...
	 *        checkAccess - array of access values to filter results by; if defined only items with the specified
	 *        access code(s) are returned returnLabelsAsArray - if set to true then all labels associated with row are
	 *        returned in an array, otherwise only a text value in the current locale is returned; default is false -
	 *        return single label in current locale row_ids - array of primary key values to use when fetching related
	 *        items; if omitted or set to a null value the 'row_id' option (single value) will be used; if row_id is
	 *        also not set then the currently loaded primary key value will be used row_id - primary key value to use
	 *        when fetching related items; if omitted or set to a false value (eg. null, false, 0) then the currently
	 *        loaded primary key value is used [default] limit - number of items to limit return set to; default is
	 *        1000 sort = optional array of bundles to sort returned values on. Currently only supported when getting
	 *        related values via simple related <table_name> and <table_name>.related invokations. Eg. from a
	 *        ca_objects results you can use the 'sort' option got get('ca_entities'), get('ca_entities.related') or get('ca_objects.related'). The bundle specifiers are fields with or without tablename. Only those fields returned for the related tables (intrinsics, label fields and attributes) are sortable. bundles = associative array of bundle => bundle_options pairs in @return array
	 *
	 * @throws SoapFault
	 * @see BaseModel::get() notation to retrieve for each related item
	 */
			...
			if ( is_array( $options["bundles"] ) ) {
				...
				while ( $qr_result->nextHit() ) {
					foreach ( $options["bundles"] as $vs_bundle => $va_bundle_options ) {
						...
						$va_items[ $va_item_id_to_index[ $qr_result->get( $t_related_instance->primaryKey() ) ] ][ $vs_bundle ]
							= $qr_result->get( $vs_bundle, $va_bundle_options );
					}
				}
			}
	 ...
	 *                          thumbnailVersions = A list of of a media versions to return with each item. Only used
	 *                          if the set content type is ca_objects. thumbnailVersion = Same as 'thumbnailVersions'
	 *                          except it is a single value. (Maintained for compatibility with older code.) limit =
	 *                          Limits the total number of records to be returned checkAccess = An array of row-level
	 *                          access values to check set members for, often produced by the caGetUserAccessValues()
	 *                          helper. Set members with access values not in the list will be omitted. If this option
	 *                          is not set or left null no access checking is done. returnRowIdsOnly = If true a simple
	 *                          array of row_ids (keys of the set members) for members of the set is returned rather
	 *                          than full item-level info for each set member. returnItemIdsOnly = If true a simple
	 *                          array of item_ids (keys for the ca_set_items rows themselves) is returned rather than
	 *                          full item-level info for each set member. returnItemAttributes = A list of attribute element codes for the ca_set_item record to return values for.
	 *
	 * @return array
	 * @throws SoapFault
	 */
	...
	public function getElementTypesForCodes( $element_codes ) {
		...
		foreach ( $element_codes as $vs_element_code ) {
			$qr_elements = $vo_db->query( "SELECT datatype FROM ca_metadata_elements WHERE element_code=?",
				$vs_element_code );
			while ( $qr_elements->nextRow() ) {
				$va_return[ $vs_element_code ]
					= ca_metadata_elements::getAttributeNameForTypeCode( intval( $qr_elements->get( "datatype" ) ) );
			}
		}
	 ...
	 *
	 * @param string $type      can be one of: [ca_objects, ca_entities, ca_places, ca_occurrences, ca_collections,
	 *                          ca_list_items]
	...
	public function getLastChangedItems( $type, $timestamp ) {
		...
		while ( $qr_subject_results->nextRow() ) {
			$va_last_changes[ intval( $qr_subject_results->get( "subject_row_id" ) ) ]
				= intval( $qr_subject_results->get( "timestamp" ) );
		}
		...
		while ( $qr_logged_results->nextRow() ) {
			if ( $va_last_changes[ intval( $qr_logged_results->get( "logged_row_id" ) ) ]
			     < intval( $qr_logged_results->get( "timestamp" ) )
			) {
				$va_last_changes[ intval( $qr_logged_results->get( "logged_row_id" ) ) ]
					= intval( $qr_logged_results->get( "timestamp" ) );
			}
		}
