...
class ItemInfoService extends BaseService
{
            ...
            while ($qr_result->nextHit()) {
                foreach ($bundles as $vs_bundle) {
                    ...
                    if (isset($options[$vs_bundle])) {
                        $va_return[$qr_result->get($t_subject_instance->primaryKey())][$vs_bundle] = $qr_result->get(
                            $vs_bundle,
                            $options[$vs_bundle]
                        );
                    } else {
     ...
     * @return bool true if bundle should not be returned to user
     */
     ...
     * @param bool $include_sub_element_codes determines whether to return only top-level element codes or not
        ...
        foreach ($va_attrs as $vo_attr) {
            ...
            foreach ($vo_attr->getValues() as $vo_value) {
                ...
                $va_attr[] = array(
                    ...
                    "attribute_info" => $t_subject_instance->getAttributeLabelAndDescription(
                        $vo_value->getElementCode()
                    ),
    ...
    public function getAttributesForDisplay($type, $item_id, $attribute_code_or_id, $template = null, $options = null)
    {
        ...
        return $t_subject_instance->getAttributesForDisplay(
            $attribute_code_or_id,
            $ps_template = null,
            $pa_options = null
        );
    }
     ...
     *        bundles = associative array of bundle => bundle_options pairs in @return array
     * @throws SoapFault
            ...
            if (is_array($options["bundles"])) {
                ...
                while ($qr_result->nextHit()) {
                    foreach ($options["bundles"] as $vs_bundle => $va_bundle_options) {
                        ...
                        $va_items[$va_item_id_to_index[$qr_result->get(
                            $t_related_instance->primaryKey()
                        )]][$vs_bundle] = $qr_result->get($vs_bundle, $va_bundle_options);
                    }
                }
            }
    ...
    public function getRelationshipTypes($type, $sub_type_id, $related_type, $related_sub_type_id)
    {
        ...
        if ($t_rel_table->getLeftTableName() == $type) {
            ...
            if ($t_rel_table->getRightTableName() == $type) {
    ...
    public function getElementTypesForCodes($element_codes)
    {
        ...
        foreach ($element_codes as $vs_element_code) {
            $qr_elements = $vo_db->query(
                "SELECT datatype FROM ca_metadata_elements WHERE element_code=?",
                $vs_element_code
            );
            while ($qr_elements->nextRow()) {
                $va_return[$vs_element_code] = ca_metadata_elements::getAttributeNameForTypeCode(
                    intval($qr_elements->get("datatype"))
                );
            }
        }
        return $va_return;
    ...
    public function getLastChangedItems($type, $timestamp)
    {
        ...
        while ($qr_subject_results->nextRow()) {
            $va_last_changes[intval($qr_subject_results->get("subject_row_id"))] = intval(
                $qr_subject_results->get("timestamp")
            );
        }
        ...
        while ($qr_logged_results->nextRow()) {
            if ($va_last_changes[intval($qr_logged_results->get("logged_row_id"))] < intval(
                    $qr_logged_results->get("timestamp")
                )) {
                $va_last_changes[intval($qr_logged_results->get("logged_row_id"))] = intval(
                    $qr_logged_results->get("timestamp")
                );
            }
        }
