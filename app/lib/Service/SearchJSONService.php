...
class SearchJSONService extends BaseJSONService {
	...
	protected function search( $pa_bundles = null ) {
		...
		$vs_template = $this->opo_request->getParameter( 'template',
			pString );        // allow user-defined template to be passed; allows flexible formatting of returned label
		...
		while ( $vo_result->nextHit() ) {
			...
			if ( $vs_template ) {
				$va_item["display_label"] = caProcessTemplateForIDs( $vs_template, $vs_table_name, array( $vn_id ),
				...
				foreach ( $pa_bundles as $vs_bundle => $va_options ) {
					...
					if ( trim( $vs_bundle ) == 'ca_object_representations.media' ) {
						if ( $t_instance instanceof RepresentableBaseModel ) {
							if ( $vs_rep_version
								= $o_service_config->get( 'search_service_return_media_version_as_url' )
							) {
								...
								if ( sizeof( $va_object_ids ) ) {
									if ( $qr_objects = caMakeSearchResult( 'ca_objects', $va_object_ids ) ) {
										while ( $qr_objects->nextHit() ) {
											$vs_reps = $qr_objects->get( $vs_bundle . '.' . $vs_rep_version . '.url',
												[ 'returnAsArray' => false ] );
											if ( $vs_reps ) {
