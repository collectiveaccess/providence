...
class IIIFService {
		...
		if ( $pb_is_info_request ) {
			...
			if (
				...
				&& (
					...
					|| ( ( ( $va_dimensions['width'] <= $vn_tile_width )
					       || ( $va_dimensions['height']
					            <= $vn_tile_height ) ) ) // && ($va_dimensions['mode'] == 'incomplete'))
				)
			) {
				$vn_scale_factor = ceil( $va_region['width']
				                         / $va_dimensions['width'] );                        // magnification = width of region requested/width of returned tile
				$vn_level        = floor( $va_tilepic_info['PROPERTIES']['layers'] - log( $vn_scale_factor,
						2 ) );        // tilepic layer # = total # layers  - num of layer with relevant magnification (layers are stored from smallest to largest)

				$x = floor( ( $va_region['x'] ) / ( $vn_scale_factor
				                                    * $vn_tile_width ) );                            // scaled x-origin of tile
				$y = floor( ( $va_region['y'] ) / ( $vn_scale_factor
				                                    * $vn_tile_height ) );                            // scaled y-origin of tile

				$vn_num_tiles_per_row = ceil( ( $vn_width / $vn_scale_factor )
				                              / $vn_tile_width );                    // number of tiles per row for this layer/magnification
				...
				if ( ! CompositeCache::contains( $ps_identifier, 'IIIFTileCounts' ) ) {
					...
					for ( $vn_l = $va_tilepic_info['PROPERTIES']['layers']; $vn_l > 0; $vn_l -- ) {
						$va_tile_counts[ $vn_l ] = ceil( $vn_layer_width / $vn_tile_width ) * ceil( $vn_layer_height
						                                                                            / $vn_tile_height );
	...
	private static function calculateSize( $pn_image_width, $pn_image_height, $ps_size ) {
		if ( preg_match( "!^([\d]+),$!", $ps_size, $va_matches ) ) {                // w,
			...
			$vn_width          = (int) ( $pn_image_width * ( ( $vn_scale_factor_w < $vn_scale_factor_h )
					? $vn_scale_factor_w : $vn_scale_factor_h ) );
			$vn_height         = (int) ( $pn_image_height * ( ( $vn_scale_factor_w < $vn_scale_factor_h )
					? $vn_scale_factor_w : $vn_scale_factor_h ) );
			$vs_mode           = 'fit';
		} else {                                                                // full
	...
	private static function calculateRegion( $pn_image_width, $pn_image_height, $ps_region ) {
		if ( preg_match( "!^([\d]+),([\d]+),([\d]+),([\d]+)$!", $ps_region, $va_matches ) ) {                // x,y,w,h
			...
			$vn_w = $va_matches[3];
			$vn_h = $va_matches[4];
		} elseif ( preg_match( "!^pct:([\d]+),([\d]+),([\d]+),([\d]+)$!", $ps_region,
			$va_matches )
		) {        // pct:x,y,w,h
	...
	private static function imageInfo( $pt_media, $ps_fldname, $po_request ) {
		...
		$va_tiles = [
			'width'        => $va_tilepic_info['PROPERTIES']['tile_width'],
			'height'       => $va_tilepic_info['PROPERTIES']['tile_height'],
			'scaleFactors' => $va_scales
		];
		...
		if ( ! $o_media->read( $pt_media->getMediaPath( $ps_fldname, 'original' ) )
		     && ! $o_media->read( $pt_media->getMediaPath( $ps_fldname, 'large' ) )
		) {
			throw new Exception( "Cannot open file" );
		}
	...
	private static function getAvailableSizes( $pt_media, $ps_fldname, $pa_options = null ) {
		...
		foreach ( $pt_media->getMediaVersions( $ps_fldname ) as $vs_version ) {
			...
			$va_sizes[ $vs_version ] = [
				'width'  => (int) $pt_media->getMediaInfo( $ps_fldname, $vs_version, 'WIDTH' ),
				'height' => (int) $pt_media->getMediaInfo( $ps_fldname, $vs_version, 'HEIGHT' )
			];
		}
