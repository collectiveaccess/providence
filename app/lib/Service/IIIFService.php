...
class IIIFService
{
    ...
    public static function dispatch($ps_identifier, $po_request, $po_response)
    {
        ...
        if ($pb_is_info_request) {
            ...
            if (
                ...
                $vn_scale_factor = ceil(
                    $va_region['width'] / $va_dimensions['width']
                );                        // magnification = width of region requested/width of returned tile
                $vn_level = floor(
                    $va_tilepic_info['PROPERTIES']['layers'] - log($vn_scale_factor, 2)
                );        // tilepic layer # = total # layers  - num of layer with relevant magnification (layers are stored from smallest to largest)

                $x = floor(
                    ($va_region['x']) / ($vn_scale_factor * $vn_tile_width)
                );                            // scaled x-origin of tile
                $y = floor(
                    ($va_region['y']) / ($vn_scale_factor * $vn_tile_height)
                );                            // scaled y-origin of tile

                $vn_num_tiles_per_row = ceil(
                    ($vn_width / $vn_scale_factor) / $vn_tile_width
                );                    // number of tiles per row for this layer/magnification
                ...
                if (!CompositeCache::contains($ps_identifier, 'IIIFTileCounts')) {
                    ...
                    for ($vn_l = $va_tilepic_info['PROPERTIES']['layers']; $vn_l > 0; $vn_l--) {
                        $va_tile_counts[$vn_l] = ceil($vn_layer_width / $vn_tile_width) * ceil(
                                $vn_layer_height / $vn_tile_height
                            );
    ...
    private static function imageInfo($pt_media, $ps_fldname, $po_request)
    {
        ...
        $o_media = new Media();
        if (!$o_media->read($pt_media->getMediaPath($ps_fldname, 'original')) && !$o_media->read(
                $pt_media->getMediaPath($ps_fldname, 'large')
            )) {
            throw new Exception("Cannot open file");
        }
