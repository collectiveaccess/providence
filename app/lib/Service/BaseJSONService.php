...
class BaseJSONService {
	...
	public function __construct( $po_request, $ps_table = "" ) {
		...
		$this->opn_id = $this->opo_request->getParameter( "id",
			pString );    // we allow for a string to support fetching by idno; typically it's a numeric id
	 ...
	 * @param mixed  $pn_id    integer primary key value of the record to load, or string idno value for the record to
	 *                         load
	 *
	...
	protected function _getTableInstance( $ps_table, $pn_id = null
		...
		if ( $pn_id && ! is_numeric( $pn_id ) && ( $vs_idno_fld = $t_instance->getProperty( 'ID_NUMBERING_ID_FIELD' ) )
		     && preg_match( "!^[A-Za-z0-9_\-\.,\[\]]+$!", $pn_id )
		) {
