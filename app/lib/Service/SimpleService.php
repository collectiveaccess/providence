...
class SimpleService {
	...
	public static function dispatch( $ps_endpoint, $po_request ) {
		...
		if ( ! $po_request->getParameter( 'noCache', pInteger )
		     && ( $po_request->getParameter( 'sort', pString ) !== '_random_' )
		) {
	...
	private static function runDetailEndpoint( $pa_config, $po_request ) {
		...
		if ( $pm_id = $po_request->getParameter( 'id', pString ) ) {
			...
			if ( is_array( $info = ca_guids::getInfoForGUID( $guid ) )
		...
		// restrictToTypes
		if ( $pa_config['restrictToTypes'] && is_array( $pa_config['restrictToTypes'] )
		     && ( sizeof( $pa_config['restrictToTypes'] ) > 0 )
		) {
		...
		foreach ( $pa_config['content'] as $vs_key => $vm_template ) {
			$va_return[ self::sanitizeKey( $vs_key ) ] = SimpleService::processContentKey( $t_instance, $vs_key,
				$vm_template, $va_get_options );
		}
	...
	private static function runSearchEndpoint( $pa_config, $po_request ) {
		...
		if ( $from_log_id ) {
			...
			$log = ca_change_log::getLog( $from_log_id, null,
		...
		if ( ( trim( $ps_q ) === '*' ) && is_array( $id_list ) && sizeof( $id_list ) ) {
			...
			$o_res = $o_search->search( $ps_q, array(
				'sort'          => $sort = ( ( $po_request->getParameter( 'sort', pString ) )
					? $po_request->getParameter( 'sort', pString ) : $pa_config['sort'] ),
		...
		if ( isset( $pa_config['includeCount'] ) && $pa_config['includeCount'] ) {
			$va_return['resultCount'] = ( is_array( $id_list ) && sizeof( $id_list ) ) ? sizeof( $id_list )
				: $o_res->numHits();
		}
		$va_get_options = [];
		if ( isset( $pa_config['checkAccess'] ) && is_array( $pa_config['checkAccess'] ) ) {
		...
		while ( $o_res->nextHit() ) {
			...
			foreach ( $pa_config['content'] as $vs_key => $vm_template ) {
				$va_hit[ self::sanitizeKey( $vs_key ) ] = SimpleService::processContentKey( $o_res, $vs_key,
					$vm_template, $va_get_options );
			}
	...
	private static function runRefineableSearchEndpoint( $pa_config, $po_request ) {
		...
		$va_search_opts = array(
			'sort'                            => ( $po_request->getParameter( 'sort', pString ) )
				? $po_request->getParameter( 'sort', pString ) : $pa_config['sort'],
			'sort_direction'                  => ( $po_request->getParameter( 'sortDirection', pString ) )
				? $po_request->getParameter( 'sortDirection', pString ) : $pa_config['sortDirection'],
	...
	private static function getEndpointConfig( $ps_endpoint ) {
		...
		if ( ! is_array( $va_endpoints ) || ! isset( $va_endpoints[ $ps_endpoint ] )
		     || ! is_array( $va_endpoints[ $ps_endpoint ] )
		) {
			throw new Exception( 'Invalid service endpoint' );
		}

		if ( ! isset( $va_endpoints[ $ps_endpoint ]['type'] )
		     || ! in_array( $va_endpoints[ $ps_endpoint ]['type'],
				array( 'search', 'detail', 'refineablesearch', 'stats', 'site_page' ) )
		) {
			throw new Exception( 'Service endpoint config is invalid: type must be search or detail' );
		}

		if ( ! isset( $va_endpoints[ $ps_endpoint ]['content'] )
		     || ! is_array( $va_endpoints[ $ps_endpoint ]['content'] )
		) {
	...
	private static function isSimpleTemplate( $ps_template ) {
		...
		return SimpleService::$s_simple_template_cache[ $ps_template ] = preg_match( "!^\^ca_[A-Za-z0-9_\-\.]+$!",
			$ps_template );
	}
	# -------------------------------------------------------
	...
	private static function processContentKey( $pt_instance, $ps_key, $pm_template, $pa_options = null ) {
		if ( is_array( $pm_template ) ) {
			...
			if ( SimpleService::isSimpleTemplate( $vs_template ) ) {
				...
				$vs_v = $pt_instance->getWithTemplate( $vs_template,
