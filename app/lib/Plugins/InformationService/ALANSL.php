...
$g_information_service_settings_ala_nsl = array(
    ...
    'indexFields' => array(
        ...
        'description' => _t(
            "Specify additional fields to include in the search index.
...
class WLPlugInformationServiceALANSL extends BaseInformationServicePlugin implements IWLPlugInformationService
{
    ...
    public function getClient()
    {
        if (!isset ($this->o_client)) {
            $this->o_client = new \Guzzle\Http\Client(
                self::NSL_SERVICES_URL, array(
                ...
                new CachePlugin(
                    array(
                        'storage' => new DefaultCacheStorage(
                            new DoctrineCacheAdapter(new FilesystemCache(caGetTempDirPath()))
                        ),
                        'can_cache' => function () {
            ...
            if ($vs_proxy = $o_conf->get('web_services_proxy_url')) { /* proxy server is configured */
                ...
                if (($vs_proxy_user = $o_conf->get('web_services_proxy_auth_user')) && ($vs_proxy_pass = $o_conf->get(
                        'web_services_proxy_auth_pw'
                    ))) {
    ...
    public function getDataForSearchIndexing($pa_settings, $ps_url)
    {
        ...
        if (!is_array($vm_search_fields)) {
            $vm_search_fields = array_intersect(
                $this->pa_available_search_fields,
                explode("\n", preg_replace('/\s+/g', "\n", $vm_search_fields))
            );
        }
    ...
    public function getExtendedInformation($pa_settings, $ps_url)
    {
        ...
        $vs_request_url = str_replace(
                '/boa/name/',
                '/nsl/services/name/',
                $ps_url
            ) . '/api/' . $vs_format . '?embed=true';
        $vs_display = $this->relativeToAbsoluteUrls(
            $this->getClient()->get($vs_request_url)->send()->getBody(true),
            self::NSL_SERVICES_BASE
        );
    ...
    {
        $va_extra_info = $this->getClient()->get($ps_url . '/api/simple-name')->addHeader(
            'Accept',
            'application/json'
        )->send()->json();
