...
$g_information_service_settings_EOL = array(
    'keyCode' => array(
        ...
        'label' => _t('API Key'),
        'description' => _t(
            'EOLkey code. See http://www.ubio.org/index.php?pagename=xml_services for details. Default is the ubio_keycode setting in app.conf'
        )
    )
);
...
class WLPlugInformationServiceEOL extends BaseInformationServicePlugin Implements IWLPlugInformationService
{
    ...
    public function lookup($pa_settings, $ps_search, $pa_options = null)
    {
        ...
        while ($count <= $maxcount) {
            $vs_data = caQueryExternalWebservice(
                "http://eol.org/api/search/1.0.json?q={$ps_search}&page={$p}" . ($vs_eol_keycode ? "&key={$vs_eol_keycode}" : "")
            );
            ...
            if ($va_data = json_decode($vs_data, true)) {
                if (is_array($va_data['results']) && (sizeof($va_data['results']) > 0)) {
                    foreach ($va_data['results'] as $va_entry) {
                        $va_items[(string)$va_entry['title']] = array(
                            'label' => (string)$va_entry['title'],
                            'idno' => (string)$va_entry['id'],
                            'url' => $va_entry['link']
                        );
                        $count++;
                    }
    ...
    public function getExtendedInformation($pa_settings, $ps_url)
    {
        ...
        $n = (int)$matches[1];
        $vs_result = caQueryExternalWebservice(
            "http://eol.org/api/pages/1.0.json?id={$n}&batch=false" . ($vs_eol_keycode ? "&key={$vs_eol_keycode}" : "")
        );
        if (!$vs_result) {
            return array();
        }
        ...
        if (is_array($va_data['taxonConcepts'])) {
            foreach ($va_data['taxonConcepts'] as $tconcept) {
                $tconcepts[] = _t(
                    "%1 [%3] (source: %2)",
                    htmlentities($tconcept['scientificName']),
                    htmlentities($tconcept['nameAccordingTo']),
                    htmlentities($tconcept['taxonRank'])
                );
            }
        }
