...
class WLPlugInformationServiceEOL extends BaseInformationServicePlugin Implements IWLPlugInformationService {
	...
	public function lookup( $pa_settings, $ps_search, $pa_options = null ) {
		...
		while ( $count <= $maxcount ) {
			$vs_data = caQueryExternalWebservice( "http://eol.org/api/search/1.0.json?q={$ps_search}&page={$p}"
			                                      . ( $vs_eol_keycode ? "&key={$vs_eol_keycode}" : "" ) );
	...
	public function getExtendedInformation( $pa_settings, $ps_url ) {
		...
		$n         = (int) $matches[1];
		$vs_result = caQueryExternalWebservice( "http://eol.org/api/pages/1.0.json?id={$n}&batch=false"
		                                        . ( $vs_eol_keycode ? "&key={$vs_eol_keycode}" : "" ) );
		if ( ! $vs_result ) {
			return array();
		}
		...
		if ( is_array( $va_data['taxonConcepts'] ) ) {
			foreach ( $va_data['taxonConcepts'] as $tconcept ) {
				$tconcepts[] = _t( "%1 [%3] (source: %2)", htmlentities( $tconcept['scientificName'] ),
					htmlentities( $tconcept['nameAccordingTo'] ), htmlentities( $tconcept['taxonRank'] ) );
			}
		}
