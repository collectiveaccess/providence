...
$g_information_service_settings_WorldCat = array(
    'user' => array(
        ...
        'description' => _t(
            'WorldCat Z39.50 login user name. Used to connect to WorldCat if Z39.50 support is available on your server.'
        )
    ),
    'password' => array(
        'formatType' => FT_TEXT,
        ...
        'description' => _t(
            'WorldCat Z39.50 login password. Used to connect to WorldCat if Z39.50 support is available on your server.'
        )
    ),
    'APIKey' => array(
        'formatType' => FT_TEXT,
        ...
        'label' => _t('API Key'),
        'description' => _t(
            'WorldCat API Key. Used to connect to WorldCat if Z39.50 login is not specified or Z39.50 support is not available on your server.'
        )
    ),
    'labelFormat' => array(
        'formatType' => FT_TEXT,
    ...
    'detailStyle' => array(
        ...
        'description' => _t(
            'Sets the style of the detail view. Use <em>Labeled MARC fields</em> to display MARC fields with english labels, <em>MARC codes</em> to display with numeric MARC codes, and <em>template</em> to use the XSL template specified below.'
        )
    ),
    'detailXSLTemplate' => array(
        ...
        'description' => _t(
            'A valid XSL template for transforming MARCXML provided by WorldCat into HTML for display. Only used when valid XSL and <em>Detail style</em> is set to <em>template</em>.'
        )
    )
);
...
class WLPlugInformationServiceWorldCat Extends BaseInformationServicePlugin Implements IWLPlugInformationService
{
     ...
     */
    static public $s_worldcat_search_url = "http://www.worldcat.org/webservices/catalog/search/worldcat/opensearch";

    /**
     * WorldCat web API catalog detail url
     */
    static public $s_worldcat_detail_url = "http://www.worldcat.org/webservices/catalog/content/";

    /**
     * WorldCat Z39.50 host
     */
    static public $s_worldcat_z3950_host = "zcat.oclc.org:210/OLUCWorldCat";
    ...
    public function lookup($pa_settings, $ps_search, $pa_options = null)
    {
        ...
        if ($va_config['user'] && $va_config['z39IsAvailable']) {
            $r_conn = yaz_connect(
                WLPlugInformationServiceWorldCat::$s_worldcat_z3950_host,
                array('user' => $va_config['user'], 'password' => $va_config['password'])
            );
            ...
            for ($vn_index = $vn_start + 1; $vn_index <= $vn_start + $vn_count; $vn_index++) {
                ...
                if ($vs_isbn_metadata_element_code) {
                    ...
                    if ($va_ids = ca_objects::find(
                        [$vs_isbn_metadata_element_code => $vs_isbn],
                        ['returnAs' => 'ids']
                    )) {
                ...
                $va_data['results'][] = array(
                    ...
                    'existingObject' => $vn_isbn_exists_object_id ? caProcessTemplateForIDs(
                        $vs_isbn_exists_template,
                        'ca_objects',
                        [$vn_isbn_exists_object_id]
                    ) : '',
                    'url' => $vs_oclc_num,
            ...
            try {
                if (!$va_config['curlIsAvailable']) {
                    throw new Exception(
                        _t('CURL is required for WorldCat web API usage but not available on this server')
                    );
                }
                if (!$va_config['APIKey']) {
                    if (!$va_config['z39IsAvailable']) {
                        throw new Exception(
                            _t('Neither Z39.50 client is installed nor is WorldCat web API key configured')
                        );
                    } else {
                        throw new Exception(_t('WorldCat web API key is not configured'));
                    }
                }
                $o_feed = Zend_Feed::import(
                    WLPlugInformationServiceWorldCat::$s_worldcat_search_url . "?start={$vn_start}&count={$vn_count}&q=" . urlencode(
                        $ps_search
                    ) . "&wskey=" . $va_config['APIKey']
                );
            } catch (Exception $e) {
                $va_data['results'][] = array(
    ...
    public function getExtendedInformation($pa_settings, $ps_url, $pa_options = null)
    {
        ...
        if ($va_config['user'] && $va_config['z39IsAvailable']) {
            ...
            $o_client = new Client(
                ['base_uri' => WLPlugInformationServiceWorldCat::$s_worldcat_detail_url]
            );
            try {
                if (!$va_config['curlIsAvailable']) {
                    throw new Exception(
        ...
        switch ($pa_settings['detailStyle']) {
            case 'labels':
            default:
                $vs_template = file_get_contents(
                    __CA_LIB_DIR__ . "/Plugins/InformationService/WorldCat/MARC21slim2English.xml"
                );
                break;
            case 'codes':
                $vs_template = file_get_contents(
                    __CA_LIB_DIR__ . "/Plugins/InformationService/WorldCat/MARC21slim2HTML.xml"
                );
                break;
            case 'template':
        ...
        try {
            ...
            return array(
                'display' => _t(
                    'WorldCat detail display template could not be created: %1',
                    $e->getMessage()
                )
            );
        }
    ...
    private function _getConfiguration($pa_settings, $pa_options)
    {
        ...
        if (!($vs_api_key = caGetOption('APIKey', $pa_settings, null)) && !($vs_api_key = caGetOption(
                'APIKey',
                $pa_options,
                null
            ))) {
        ...
        if (!($vs_z3950_user = caGetOption('user', $pa_settings, null)) && !($vs_z3950_user = caGetOption(
                'user',
                $pa_options,
                null
            ))) {
        ...
        if (!($vs_z3950_password = caGetOption('password', $pa_settings, null)) && !($vs_z3950_password = caGetOption(
                'password',
                $pa_options,
                null
            ))) {
        ...
        return array(
            'APIKey' => $vs_api_key,
            'user' => $vs_z3950_user,
            'password' => $vs_z3950_password,
            'z39IsAvailable' => $vb_z3950_available,
            'curlIsAvailable' => caCurlIsAvailable(),
            'config' => $o_config
        );
    }
    # ------------------------------------------------
