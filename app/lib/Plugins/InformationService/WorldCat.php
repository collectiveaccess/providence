...
class WLPlugInformationServiceWorldCat Extends BaseInformationServicePlugin Implements IWLPlugInformationService {
	...
	# ------------------------------------------------
	/**
	 * Perform lookup on WorldCat data service. Z39.50 is used if the PHP YAZ Z39.50 client is installed on the server
	 * and a login is configured in either the plugin settings passed or in the $pa_settings parameter or in app.conf.
	 * The WorldCat search API will be used a web service API key is configured and either PHP YAZ is unavailable or a
	 * Z39.50 login is not configured.
	 *
	 * @param array  $pa_settings Plugin settings values
	 * @param string $ps_search   The expression with which to query the remote data service
	 * @param array  $pa_options  Options are:
	 *                            APIKey = WorldCat API key to use. [Default is the key configured in worldcat_api_key
	 *                            in app.conf] user = Worldcat Z39.50 login name. [Default is the username configured
	 *                            in worldcat_z39.50_user in app.conf] password = WorldCAt Z39.50 password. [Default is
	 *                            the password configured in worldcat_z39.50_password in app.conf] start = Zero-based
	 *                            record number to begin returned result set at [Default is 0] count = Maximum number
	 *                            of records to return [Default is 25]
	 *
	 * @return array
	 */
	public function lookup( $pa_settings, $ps_search, $pa_options = null ) {
		...
		if ( $va_config['user'] && $va_config['z39IsAvailable'] ) {
			$r_conn = yaz_connect( WLPlugInformationServiceWorldCat::$s_worldcat_z3950_host,
			...
			for ( $vn_index = $vn_start + 1; $vn_index <= $vn_start + $vn_count; $vn_index ++ ) {
				...
				if ( $vs_isbn_metadata_element_code ) {
					...
					if ( $va_ids = ca_objects::find( [ $vs_isbn_metadata_element_code => $vs_isbn ],
						[ 'returnAs' => 'ids' ] )
					) {
				...
				$va_data['results'][] = array(
					...
					'existingObject' => $vn_isbn_exists_object_id ? caProcessTemplateForIDs( $vs_isbn_exists_template,
						'ca_objects', [ $vn_isbn_exists_object_id ] ) : '',
					'url'            => $vs_oclc_num,
			...
			try {
				...
				$o_feed = Zend_Feed::import( WLPlugInformationServiceWorldCat::$s_worldcat_search_url
				                             . "?start={$vn_start}&count={$vn_count}&q=" . urlencode( $ps_search )
	 ...
	 *                            in app.conf] user = Worldcat Z39.50 login name. [Default is the username configured
	 *                            in worldcat_z39.50_user in app.conf] password = WorldCAt Z39.50 password. [Default is
	 *                            the password configured in worldcat_z39.50_password in app.conf]
	 *
	...
	public function getExtendedInformation( $pa_settings, $ps_url, $pa_options = null ) {
		...
		if ( $va_config['user'] && $va_config['z39IsAvailable'] ) {
			...
			$o_client
				= new \GuzzleHttp\Client( [ 'base_uri' => WLPlugInformationServiceWorldCat::$s_worldcat_detail_url ] );
			try {
				if ( ! $va_config['curlIsAvailable'] ) {
					throw new Exception( _t( 'CURL is required for WorldCat web API usage but not available on this server' ) );
				}
		...
		switch ( $pa_settings['detailStyle'] ) {
			case 'labels':
			default:
				$vs_template = file_get_contents( __CA_LIB_DIR__
				                                  . "/Plugins/InformationService/WorldCat/MARC21slim2English.xml" );
				break;
			case 'codes':
				$vs_template = file_get_contents( __CA_LIB_DIR__
				                                  . "/Plugins/InformationService/WorldCat/MARC21slim2HTML.xml" );
				break;
			case 'template':
		...
		try {
			...
			return array(
				'display' => _t( 'WorldCat detail display template could not be parsed: %1', $e->getMessage() )
			);
		}
		...
		try {
			...
			return array(
				'display' => _t( 'WorldCat detail display template could not be created: %1', $e->getMessage() )
			);
		}
	...
	private function _getConfiguration( $pa_settings, $pa_options ) {
		...
		if ( ! ( $vs_api_key = caGetOption( 'APIKey', $pa_settings, null ) )
		     && ! ( $vs_api_key = caGetOption( 'APIKey', $pa_options, null ) )
		) {
			$vs_api_key = $o_config->get( 'worldcat_api_key' );
		}

		if ( ! ( $vs_z3950_user = caGetOption( 'user', $pa_settings, null ) )
		     && ! ( $vs_z3950_user
				= caGetOption( 'user', $pa_options, null ) )
		) {
			$vs_z3950_user = $o_config->get( 'worldcat_z39.50_user' );
		}

		if ( ! ( $vs_z3950_password = caGetOption( 'password', $pa_settings, null ) )
		     && ! ( $vs_z3950_password
				= caGetOption( 'password', $pa_options, null ) )
		) {
