...
class WLPlugInformationServicePtolemaic extends BaseInformationServicePlugin Implements IWLPlugInformationService
{
    ...
    public function __construct()
    {
        ...
        $this->description = _t(
            'Provides access to Ptolemaic Coins Online numismatic data service (http://numismatics.org/pco/)'
        );
    }
    # ------------------------------------------------
    ...
    public function lookup($pa_settings, $ps_search, $pa_options = null)
    {
        ...
        while ($count <= $maxcount) {
            $vs_data = caQueryExternalWebservice(
                'http://numismatics.org/pco/apis/search?format=rss&q=' . urlencode($ps_search)
            );
                    ...
                    if ($o_entries && sizeof($o_entries)) {
                        foreach ($o_entries as $o_entry) {
                            ...
                            $va_items[(string)$o_entry->{'title'}] = array(
                                ...
                                'idno' => str_replace("http://numismatics.org/pco/id/", "", (string)$o_entry->{'link'}),
                        ...
                        default:
                            ...
                            foreach ($v as $vi) {
                                if (is_array($vi)) {
                                    ...
                                    $va_display[] = "<div style='margin-left: 10px;'><strong>{$k}</strong>: " . join(
                                            "; ",
                                            $d
                                        ) . "</div>";
                                } else {
