...
class WLPlugInformationServiceResourceSpace Extends BaseInformationServicePlugin Implements IWLPlugInformationService {
	...
	# ------------------------------------------------
	/**
	 * Perform lookup on a ResourceSpace instance for individual items. The target instances are provided in the config
	 * files.
	 *
	 * @param array  $pa_settings Plugin settings values
	 * @param string $ps_search   The expression with which to query the remote data service
	 * @param array  $pa_options  Options are:
	 *                            rsInstance = ResourceSpace instance to limit this search to. [Default is all of the
	 *                            systems configured in app.conf] user = ResourceSpace login name. [Default is the
	 *                            username configured in resourcespace_user in app.conf] start = Zero-based record
	 *                            number to begin returned result set at [Default is 0] count = Maximum number of
	 *                            records to return [Default is 25]
	 *
	 * @return array
	 */
		...
		foreach ( $va_config as $va_api ) {
			...
			if ( $va_api['user'] && $va_api['apiKey'] ) {
				...
				if ( $vs_resource_search ) {
					$va_raw_data = $this->_resourceSearch( $vs_resource_search, $va_api['apiKey'], $va_api['user'],
						$vs_resourcespace_base_api_url, $vn_count );
				}
				// Get collection results if provided
				if ( $vs_collection_search ) {
					$vs_rs_collection_query = 'user=' . $va_api['user'] . '&function=search_public_collections&param1='
					                          . $vs_collection_search . '&param2=name&param3=ASC&param4=0&param5=0';
					$vs_rs_collection_hash  = hash( 'sha256', $va_api['apiKey'] . $vs_rs_collection_query );
					$va_coll_request        = curl_init();
					curl_setopt( $va_coll_request, CURLOPT_URL,
						$vs_resourcespace_base_api_url . $vs_rs_collection_query . '&sign=' . $vs_rs_collection_hash );
					...
					if ( $vn_rs_http_code == 200 ) {
						...
						foreach ( $va_coll_data as $va_coll ) {
							...
							$va_coll_items    = $this->_resourceSearch( '%21collection' . $vs_coll_ref,
								$va_api['apiKey'], $va_api['user'], $vs_resourcespace_base_api_url, $vn_count );
							$va_coll_contents = array();
							foreach ( $va_coll_items as $vs_coll_item ) {
								array_push( $va_coll_contents, array(
									'ref'     => $vs_coll_item['ref'],
									'title'   => $vs_coll_item['field8'],
									'url_pre' => $vs_coll_item['url_pre']
								) );
							}
							array_push( $va_coll_results, array(
								$va_coll['name'] => $va_coll_contents,
								'ref'            => $vs_coll_ref,
								'count'          => count( $va_coll_items )
							) );
						}
					}

				}
				array_push( $va_data, array(
					$va_api['rsInstance'] => array(
						'count'       => count( $va_raw_data ),
						'results'     => $va_raw_data,
						'collResults' => $va_coll_results,
						'label'       => $va_api['label']
					)
				) );
				#return $va_data;
	 ...
	 *                            APIKey = ResourceSpace API key to use. [Default is the key configured in
	 *                            resourcespace_api_key in app.conf] user = ResourceSpace login name. [Default is the
	 *                            username configured in resourcespace_user in app.conf]
	 *
	...
	public function getExtendedInformation( $pa_settings, $ps_rs_id, $pa_options = null ) {
		...
		foreach ( $va_config as $va_api ) {
			...
			$va_data = array();
			if ( $va_api['user'] && $va_api['apiKey'] ) {
				$vs_resourcespace_query = $vs_resourcespace_base_api_url . '?user=' . $va_api['user']
				                          . '&function=get_resource_field_data&param1=' . $ps_rs_id;
	...
	private function _getImagePreview( $pa_settings, $ps_rs_id, $pa_options = null ) {
		...
		if ( $va_config['user'] && $va_config['apiKey'] ) {
			...
			$va_request             = curl_init();
			curl_setopt( $va_request, CURLOPT_URL,
				$vs_resourcespace_base_api_url . $vs_resourcespace_query . '&sign=' . $vs_resourcespace_hash );
			curl_setopt( $va_request, CURLOPT_HEADER, 0 );
			curl_setopt( $va_request, CURLOPT_RETURNTRANSFER, 1 );
			$va_resourcespace_return = curl_exec( $va_request );
			$va_data                 = json_decode( $va_resourcespace_return, true );
			...
			return array(
				'data' => $va_data,
				'url'  => $vs_resourcespace_base_api_url . $vs_resourcespace_query . '&sign=' . $vs_resourcespace_hash
			);
		}
		...
		if ( $va_api_credentials = $o_config->get( 'resourcespace_apis' ) ) {
			foreach ( $va_api_credentials as $vs_instance => $va_instance_api ) {
				$rs_api = array(
					...
					'label'      => $va_instance_api['resourcespace_label']
				);
	...
	private function _resourceSearch( $ps_search, $ps_api_key, $ps_api_user, $ps_api_url, $pn_count ) {
		$vs_resourcespace_query = 'user=' . $ps_api_user . '&function=search_get_previews&param1=' . $ps_search
		                          . '&param2=&param3=&param4=0&param5=-1&param6=&param7=&param8=pre&param9=';
		$vs_resourcespace_hash  = hash( 'sha256', $ps_api_key . $vs_resourcespace_query );
		$va_request             = curl_init();
		curl_setopt( $va_request, CURLOPT_URL,
			$ps_api_url . $vs_resourcespace_query . '&sign=' . $vs_resourcespace_hash );
		curl_setopt( $va_request, CURLOPT_HEADER, 0 );
		curl_setopt( $va_request, CURLOPT_RETURNTRANSFER, 1 );
