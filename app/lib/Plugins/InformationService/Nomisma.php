...
class WLPlugInformationServiceNomisma extends BaseNomismaLODServicePlugin implements IWLPlugInformationService
{
    ...
    public function lookup($pa_settings, $ps_search, $pa_options = null)
    {
        ...
        $ontology_filter = (is_array($pa_settings['ontologies']) && sizeof(
                $pa_settings['ontologies']
            )) ? "FILTER (?t IN (" . join(',', $pa_settings['ontologies']) . "))" : "";
        $query_filter = ($is_url) ? 'FILTER (regex(?label, "^' . trim(
                $vs_search
            ) . '$", "i"))' : 'FILTER (regex(?label, "\\\\b' . trim($vs_search) . '\\\\b", "i"))';
        $vs_query = urlencode(
...
LIMIT ' . (int)$pn_limit
        ...
        foreach ($va_results as $va_values) {
            ...
            $vs_label = (caGetOption(
                    'format',
                    $pa_options,
                    null,
                    ['forceToLowercase' => true]
                ) === 'short') ? $va_values['label']['value'] : ($parent ? $parent . " ➜ " : "") . $va_values['label']['value'] . " [" . $vs_id . "]";
