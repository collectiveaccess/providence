...
class WLPlugInformationServiceNomisma extends BaseNomismaLODServicePlugin implements IWLPlugInformationService {
	...
	public function lookup( $pa_settings, $ps_search, $pa_options = null ) {
		...
		$ontology_filter = ( is_array( $pa_settings['ontologies'] ) && sizeof( $pa_settings['ontologies'] ) )
			? "FILTER (?t IN (" . join( ',', $pa_settings['ontologies'] ) . "))" : "";

		if ( $is_url ) {
			$query_filter = 'FILTER (?data IN (<' . preg_replace( "![^A-Za-z0-9_\-\%\/\&\:\.]+!", "",
		...
		foreach ( $va_results as $va_values ) {
			...
			$vs_label = ( caGetOption( 'format', $pa_options, null, [ 'forceToLowercase' => true ] ) === 'short' )
				? $va_values['label']['value']
				: ( $parent ? $parent . " ➜ " : "" ) . $va_values['label']['value'] . " [" . $vs_id . "]";
