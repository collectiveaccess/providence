...
class WLPlugInformationServiceWikipedia Extends BaseInformationServicePlugin Implements IWLPlugInformationService {
	...
	public function lookup( $pa_settings, $ps_search, $pa_options = null ) {
		...
		if ( ! is_array( $va_content ) || ! isset( $va_content['query']['pages'] )
		     || ! is_array( $va_content['query']['pages'] )
		     || ! sizeof( $va_content['query']['pages'] )
		) {
			return array();
		}
	...
	public function getExtendedInformation( $pa_settings, $ps_url ) {
		...
		$vs_display .= "<div style='float:right; margin: 10px 0px 10px 10px;'><img src='" . $va_info['image_thumbnail']
		               . "' /></div>";
