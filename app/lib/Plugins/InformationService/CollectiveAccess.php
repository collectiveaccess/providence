...
class WLPlugInformationServiceCollectiveAccess Extends BaseInformationServicePlugin
	...
	public function lookup( $pa_settings, $ps_search, $pa_options = null ) {
		...
		$o_response = $o_client->request( "GET",
			$vs_url = $u['path'] . '/service.php/find/' . $pa_settings['table'] . '?q=' . urlencode( $ps_search )
		...
		if ( isset( $va_data['results'] ) && is_array( $va_data['results'] ) ) {
			foreach ( $va_data['results'] as $vs_k => $va_result ) {
				...
				$va_data['results'][ $vs_k ]['url'] = $pa_settings['baseURL'] . '/service.php/item/'
				                                      . $pa_settings['table'] . '/id/' . $va_result[ $vs_pk ];
			}
		}
	...
	public function getExtendedInformation( $pa_settings, $ps_url ) {
		...
		if ( ! ( $vs_template
			= $pa_settings['detailFormat'] )
		) {        // if no detailFormat options is set default to just outputting preferred labels
		...
		$u          = parse_url( $pa_settings['baseURL'] );
		$o_response = $o_client->request( "GET",
			$vs_url = $u['path'] . '/service.php/item/' . $pa_settings['table'] . '/id/' . urlencode( $ps_id )
