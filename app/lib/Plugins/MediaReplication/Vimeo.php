...
class WLPlugMediaReplicationVimeo Extends BaseMediaReplicationPlugin
{
    ...
    public function initiateReplication($ps_filepath, $pa_data, $pa_options = null)
    {
        ...
        try {
            if (!$o_client) {
                throw new VimeoAPIException(
                    _t(
                        "Initial connection to Vimeo failed. Did you authorize CollectiveAccess to use your Vimeo account? Enable the 'vimeo' application plugin and navigate to Manage > Vimeo integration to do so."
                    )
                );
            }
            ...
            if ($vs_video_id = $o_client->upload($ps_filepath)) {
                ...
                $o_client->call(
                    ...
                    array('title' => $pa_data['title'], 'video_id' => $vs_video_id)
                );
                $o_client->call(
                    'vimeo.videos.setDescription',
                    array('description' => $pa_data['description'], 'video_id' => $vs_video_id)
                );
                if (isset($pa_data['tags'])) {
                    $o_client->call(
                        'vimeo.videos.addTags',
                        array('tags' => $pa_data['tags'], 'video_id' => $vs_video_id)
                    );
                }
                ...
                $o_client->call(
                    ...
                    array('privacy' => $vs_privacy_setting, 'video_id' => $vs_video_id)
                );
                $o_client->call(
                    'vimeo.videos.setDownloadPrivacy',
                    array('download' => $vn_dl_privacy, 'video_id' => $vs_video_id)
                );
                ...
                if ($vs_preset = caGetOption('embed_preset', $pa_target_options, false)) {
                    $o_client->call(
                        ...
                        array('preset_id' => $vs_preset, 'video_id' => $vs_video_id)
                    );
                }
                ...
                if (isset($pa_target_options['channel'])) {
                    $o_client->call(
                        ...
                        array('channel_id' => $pa_target_options['channel'], 'video_id' => $vs_video_id)
                    );
                }
            } else {
    ...
    public function getReplicationStatus($ps_request_token, $pa_options = null)
    {
        ...
        $vs_video_id = preg_replace(
            "!^" . $this->info['NAME'] . "://!",
            "",
            $ps_request_token
        ); // remove plugin identifier to obtain raw video ID
