...
class ElasticSearchConfigurationSettings extends ASearchConfigurationSettings
{
    ...
    public function __construct()
    {
        ...
        $this->opo_search_indexing_config = Configuration::load(
            $this->opo_search_config->get("search_indexing_config")
        );
    ...
    private function _initMessages()
    {
        ...
        $this->opa_setting_descriptions[__CA_ELASTICSEARCH_SETTING_INDEX_EXISTS__] =
            _t(
                "CollectiveAccess uses only a single index in an ElasticSearch setup. The name of that index can be set in the CollectiveAccess configuration."
            );
        # ------------------------------------------------
        $this->opa_setting_hints[__CA_ELASTICSEARCH_SETTING_RUNNING__] =
            _t(
                "Install and start the ElasticSearch service. If it is already running, check your CollectiveAccess configuration (the ElasticSearch URL and index name in particular).",
                $this->ops_webserver_user
            );
        $this->opa_setting_hints[__CA_ELASTICSEARCH_SETTING_INDEX_EXISTS__] =
            _t(
                "If the service is running and can be accessed by CollectiveAccess but the index is missing, let CollectiveAccess generate a fresh index and create the related indexing mappings. There is a tool in support/utils."
            );
        # ------------------------------------------------
    }
