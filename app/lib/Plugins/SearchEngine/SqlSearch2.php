...
require_once(__CA_LIB_DIR__.'/Plugins/SearchEngine/SqlSearch2Result.php');
...
require_once(__CA_LIB_DIR__.'/Plugins/SearchEngine/SqlSearch2/SearchQuery.php');
...
class WLPlugSearchEngineSqlSearch2 extends BaseSearchPlugin implements IWLPlugSearchEngine {
	 ...
	 */
	public function search(int $subject_tablenum, string $search_expression, array $filters=[], $rewritten_query): WLPlugSearchEngineSqlSearch2Result {
...
//		arsort($hits, SORT_NUMERIC);	// sort by boost
		...
		return new WLPlugSearchEngineSqlSearch2Result($hits, $subject_tablenum);
	}
	# -------------------------------------------------------
	/**
	 ...
	 * @throws ApplicationException
*/
	private function _processQuery(int $subject_tablenum, Zend_Search_Lucene_Search_Query $query):?SearchQuery {
		$qclass = get_class($query);

		switch($qclass) {
			case 'Zend_Search_Lucene_Search_Query_Boolean':
				$query = $this->_processQueryBoolean($subject_tablenum, $query);
				break;
			case 'Zend_Search_Lucene_Search_Query_MultiTerm':
				$query = $this->_processQueryMultiterm($subject_tablenum, $query);
				break;
			case 'Zend_Search_Lucene_Index_Term':
			case 'Zend_Search_Lucene_Search_Query_Term':
				$query = $this->_processQueryTerm($subject_tablenum, $query);
				break;
			case 'Zend_Search_Lucene_Search_Query_Phrase':
				$query = $this->_processQueryPhrase($subject_tablenum, $query);
				break;
			case 'Zend_Search_Lucene_Search_Query_Range':
				$query = $this->_processQueryRange($subject_tablenum, $query);
				break;
			default:
	 ...
	 * @throws ApplicationException
	 */
	private function _processQueryBoolean(int $subject_tablenum, $query):SearchQuery {
		$signs = $query->getSigns();
		$subqueries = $query->getSubqueries();
		$current_query = $this->createSearchQuery();
		foreach($subqueries as $i =>$subquery) {
			$hits = $this->_processQuery($subject_tablenum, $subquery);
			if (is_null($hits)) {
				continue;
			} // skip stop words
			$current_query = $this->_processBooleanOperator( $signs, $i, $hits, $current_query, $subject_tablenum );
	 ...
	 * @throws ApplicationException
	 */
	private function _processQueryTerm(int $subject_tablenum, $query):?SearchQuery {
		$qclass = get_class($query);
		$term = ($qclass === 'Zend_Search_Lucene_Search_Query_Term') ? $query->getTerm() : $query;

		$field = $term->field;
		if ( $field && $this->isChangeLog( $field ) ) {
		...
		foreach($words as $i => $text) {
			...
			if ($is_bare_wildcard) {
				$query->setQuery( $this->getStarQuery( $subject_tablenum ) );
			} elseif($use_boost) {
				$query->setQuery("
		...
		$query = array_shift($results);
		// And combine that query with all subsequent queries.
		/** @var SearchQuery $r */
	...
	private function _processQueryPhrase(int $subject_tablenum, $query) {
		...
		$search_query = $this->createSearchQuery();
		if ($this->getOption('strictPhraseSearching')) {
			...
			foreach($words as $w => $word) {
				...
				$search_query->createTemporaryTable($temp_table);
	 ...
	 * @throws SearchException
	 */
	private function _processQueryChangeLog(int $subject_tablenum, Object $term):SearchQuery {
		...
		if ($this->isChangeLog($field)) {
			...
			if (sizeof($field_elements) > 1) {
				$user_element = $field_elements[1];
				if (!is_int( $user_element )) {
					$t_user = new ca_users();
					if (
						$t_user->load(["user_name" => $user_element ])
						||
						( ( strpos( $user_element, "_") !== false) && $t_user->load([ "user_name" => str_replace("_", " ",
								$user_element )]))
					) {
						$user_id = (int)$t_user->getPrimaryKey();
					}
				} else {
					$user_id = (int) $user_element;
				}
				$user_sql = ($user_id)  ? " AND (ccl.user_id = {$user_id})" : "";
			}
			$query = $this->createSearchQuery();
			switch(reset($field_elements)) {
				case _t('created'):
					$query->setQuery("
							...
							WHERE
								...
								(ccl.changetype = 'I')
								{$user_sql}
						")->setArguments([(int)$range['start'], (int)$range['end'], $subject_tablenum]);
					break;
				case _t('modified'):
					// TODO: confirm that this optimised query is still returning the correct records
					$query->setQuery("
							SELECT IF(ccl.logged_table_num = ? , ccl.logged_row_id, ccls.subject_row_id) row_id, 1 boost
							FROM ca_change_log ccl
							    LEFT JOIN ca_change_log_subjects AS ccls ON ccls.log_id = ccl.log_id
							WHERE
								(ccl.log_datetime BETWEEN ? AND ?)
								AND
								(ccl.logged_table_num = ? OR ccls.subject_table_num = ?)
								AND
								(ccl.changetype = 'U')
								{$user_sql}

						")->setArguments([$subject_tablenum, (int)$range['start'], (int)$range['end'], $subject_tablenum,  $subject_tablenum]);
					break;
				default:
					throw new SearchException(_t('Invalid change log search mode: %1', $field));
					break;
			}
			return $query;
		}
		throw new SearchException(_t('You cannot perform a change log search without specifying a change log field. Specified field is %1', $field));
	}
	# -------------------------------------------------------
	/**
	 *
	 */
	private function _processQueryRange(int $subject_tablenum, Zend_Search_Lucene_Search_Query_Range $query):SearchQuery {
	 ...
	 * @throws ApplicationException
	 */
	private function _processMetadataDataType(int $subject_tablenum, array $ap, $query):?SearchQuery {
		$qclass = get_class($query);
		...
		switch($qclass) {
			...
			case 'Zend_Search_Lucene_Search_Query_Range':
				...
				break;
			default:
				throw new SearchException(_t('Invalid query passed to _processMetadataDataType: %1', $qclass));
				break;
		}
		...
		if($ap['type'] === 'INTRINSIC') {
			...
			if(!$t_instance->hasField($field_name)) { return null; }
		...
		if(is_array($qinfo) && sizeof($qinfo)) {
			...
			$search_query->setQuery($qinfo['sql'])->setArguments($params);
			...
			if(is_a($ap_instance, 'BaseLabel')) {
				...
				// TODO join from label table to base table
				$qr = $this->db->query("
			...
			if(!($qr = caMakeSearchResult($ap['table_num'], array_keys($row_ids)))) { return null; }
	 ...
	 * @throws ApplicationException
	 */
	private function _filterQueryResult(int $subject_tablenum, $hits, array $filters) {
		if (is_array($filters) && sizeof($filters) && ($hits) && sizeof($hits)) {
			...
			$sql_joins = join("\n", $joins);
			$sql_wheres = $wheres ? 'WHERE ' . join("\n AND ", $wheres) : '';

			if ($hits instanceof SearchQuery) {
				// TODO: work out which of these is quicker
				$subquery = $hits->getQuery();
				$query = $this->createSearchQuery();
				$query->setQuery("
				WITH subquery AS ($subquery)
				SELECT $pk
				FROM {$table}
				JOIN subquery ON subquery.row_id = $pk
				$sql_joins
				{$sql_wheres}
				ORDER BY subquery.boost DESC
				");

				$query->setArguments($hits->getArguments());
				$query->execute();
				return $query;
			}
			$log = caGetLogger();
			$log->logError('Search did not return a SearchQuery, falling back to array comparison');
	 ...
	 *
	 */
	private function _getFieldRestrictions(int $table_num): array {
	...
	public function quickSearch($pn_table_num, $ps_search, $pa_options=null) {
		// TODO implement quicksearch using SearchQuery
		if (!is_array($pa_options)) { $pa_options = []; }
		$vs_limit_sql = '';
		...
		if (sizeof($va_words)) {
			...
			$qr_res = $this->db->query("
				SELECT swi.row_id, SUM(boost) as boost
				FROM ca_sql_search_words sw
	 ...
	 *
	 */
	private function _queryForNumericAttribute($attrval, $ap, $text, $text_upper, $attr_field): ?array {
	 ...
	 *
	 */
	private function _queryForDateRangeAttribute($attrval, $ap, $text, $text_upper): ?array {
		list($text, $modifier) = $this->parseModifier($text);
	 ...
	 * @param SearchQuery $current_query
	 * @param int $subject_tablenum
	 *
	 * @return SearchQuery
	 * @throws ApplicationException
	 * @throws SearchException
	 */
	private function _processBooleanOperator(
		$signs, $i, SearchQuery $hits, SearchQuery $current_query, int $subject_tablenum
	): SearchQuery {
		...
		switch ( $op ) {
			...
			case 'OR':
				...
				$current_query->addOr( $hits );
				break;
			case 'NOT':
				if ( $i == 0 ) {
					$current_query->setQuery($this->getStarQuery($subject_tablenum));
				}
				$current_query->addNot($hits);
				break;
			default:
				throw new SearchException( _t( 'Invalid boolean operator: %1', $op ) );
		}
	...
	*
	* @return bool
	*/public function isChangeLog( string $field ): bool {
		$field_lc       = mb_strtolower( $field );
		$field_elements = explode( '.', $field_lc );

		return in_array( reset( $field_elements ), [ _t( 'created' ), _t( 'modified' ) ] );
	}

	private function getStarQuery( int $subject_tablenum ): string {
