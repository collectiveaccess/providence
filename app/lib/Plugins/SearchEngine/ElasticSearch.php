...
class WLPlugSearchEngineElasticSearch extends BaseSearchPlugin implements IWLPlugSearchEngine {
	...
	public function init() {
		if ( ( $vn_max_indexing_buffer_size
				= (int) $this->opo_search_config->get( 'elasticsearch_indexing_buffer_size' ) ) < 1
		) {
	...
	public function search( $pn_subject_tablenum, $ps_search_expression, $pa_filters = [], $po_rewritten_query = null
		...
		$o_query  = new ElasticSearch\Query( $pn_subject_tablenum, $ps_search_expression, $po_rewritten_query,
			$pa_filters );
	...
	public function removeRowIndexing(
		$pn_subject_tablenum, $pn_subject_row_id, $pn_field_tablenum = null, $pm_field_nums = null,
			...
			if ( ! is_array( $resp ) ) {
				...
				foreach ( $resp['items'] as $r ) {
					if ( is_array( $r['index']['error'] ) ) {
						throw new ApplicationException( _t( 'Indexing error [%1]: %2', $r['index']['error']['type'],
							$r['index']['error']['reason'] ) );
					}
				}
			}
