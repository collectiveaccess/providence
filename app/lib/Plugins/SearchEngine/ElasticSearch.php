...
class WLPlugSearchEngineElasticSearch extends BaseSearchPlugin implements IWLPlugSearchEngine
{
    ...
    public function __construct($po_db = null)
    {
        ...
        $this->ops_elasticsearch_base_url = trim(
    ...
    public function refreshMapping($pb_force = false)
    {
        ...
        if ($o_mapping->needsRefresh() || $pb_force) {
            ...
            try {
                ...
                throw new \Exception(
                    _t(
                        "Updating the ElasticSearch mapping failed. This is probably because of a type conflict. Try recreating the entire search index. The original error was %1",
                        $e->getMessage()
                    )
                );
            }
    ...
    public function updateIndexingInPlace(
        ...
        foreach ($pa_subject_row_ids as $pn_subject_row_id) {
            ...
            $this->addFragmentToUpdateContentBuffer(
                $va_fragment,
                $va_record,
                $vs_table_name,
                $pn_subject_row_id,
    ...
    {
        if (($vn_max_indexing_buffer_size = (int)$this->opo_search_config->get(
                'elasticsearch_indexing_buffer_size'
            )) < 1) {
    ...
    public function truncateIndex($pn_table_num = null, $pb_dont_refresh = false)
    {
        if (!$pn_table_num) {
            ...
            if (!$vs_table_name = Datamodel::getTableName($pn_table_num)) {
                return false;
            }
    ...
    public function search(
        ...
        $o_query = new ElasticSearch\Query(
            $pn_subject_tablenum, $ps_search_expression, $po_rewritten_query, $pa_filters
        );
        ...
        if (($va_additional_filters = $o_query->getAdditionalFilters()) && is_array($va_additional_filters) && (sizeof(
                    $va_additional_filters
                ) > 0)) {
    ...
    public function commitRowIndexing()
    {
        if (sizeof($this->opa_index_content_buffer) > 0) {
            $this->opa_doc_content_buffer[$this->ops_indexing_subject_tablename . '/' .
            $this->opn_indexing_subject_row_id] = $this->opa_index_content_buffer;
        }
                ...
                foreach ($pm_field_nums as $ps_content_fieldnum) {
                    ...
                    foreach ($va_fragment as $vs_key => $vm_val) {
                        if (isset($va_record[$vs_key])) {
                            ...
                            $this->opa_update_content_buffer[$vs_table_name][$pn_subject_row_id][$vs_key] = array_values(
                                $va_values
                            );
                            $this->opa_update_content_buffer[$vs_table_name][$pn_subject_row_id][$vs_key . '_content_ids'] = array_values(
                                $va_indexes
                            );
                        }
                    }
    ...
    public function flushContentBuffer()
    {
        ...
        if (sizeof($va_bulk_params['body'])) {
            ...
            if (caIsRunFromCLI() && $this->getIndexName() && (!defined(
    ...
    {
        $this->getClient()->indices()->forceMerge(
            array(
                'index' => $this->getIndexName()
            )
        );
