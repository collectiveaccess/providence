...
class WLPlugSearchEngineSqlSearch extends BaseSearchPlugin implements IWLPlugSearchEngine {
		...
		if (trim($ps_search_expression) === '((*))') {	
			...
			if (is_array($va_filters) && sizeof($va_filters)) {
				foreach($va_filters as $va_filter) {
					$va_tmp = explode('.', $va_filter['field']);
					$va_path = array();
					// TODO: Undefined variable $vs_table_name
					if ($va_tmp[0] != $vs_table_name) {
						$va_path = Datamodel::getPath($vs_table_name, $va_tmp[0]);
					} 
					if (sizeof($va_path)) {
	...
	private function _getElementIDForAccessPoint($pn_subject_tablenum, $ps_access_point) {
		...
		if (strtolower($vs_field) == 'count') {
			...
			return array(
                ...
                // TODO: Variable '$vn_rel_type' is probably undefined
		    ...
		    return array(
				...
				// TODO: Undefined variable '$vn_rel_type'
				'relationship_type' => (int)$vn_rel_type,
				'table_num' => $vs_table_num,
				'element_id' => null,
					...
					switch(get_class($o_lucene_query_element)) {
						case 'Zend_Search_Lucene_Search_Query_Range':
							...
							if (!$vs_direct_sql_query) {
                                // TODO: Variable '$vn_lower_val' is probably undefined
							    // TODO: Variable '$vn_upper_val' is probably undefined
								$vs_direct_sql_query = "
									SELECT ca.row_id, 1, ca.attribute_id
									FROM ca_attribute_values cav
									INNER JOIN ca_attributes AS ca ON ca.attribute_id = cav.attribute_id
	...
	if ($this->getOption('strictPhraseSearching') && !in_array($vs_first_term_table, array(_t('modified'), _t('created')))) {
							...
							foreach($va_words as $vs_word) {
								...
								// TODO: Variable '{$vs_fld_limit_sql}' is probably undefined
								$vs_sql = "
					...
					if ($vs_access_point) {
						...
						if (in_array($vs_table, array(_t('created'), _t('modified')))) {
							...
							if ((!$vb_is_blank_search && !$vb_is_not_blank_search) && $vs_table && $vs_field && ($t_table = Datamodel::getInstanceByTableName($vs_table, true)) ) {
								...
								// TODO: Variable '$vn_intrinsic_type' is probably undefined
					...
					if ($vb_is_blank_search) {
						...
						if (sizeof($va_ft_terms)) {
							if (($t_table) && (strlen($vs_fld_num) > 1)) {
								...
								// TODO: Variable '$vs_field' is probably undefined
					...
					// TODO: Variable '$va_access_point_info' is probably undefined
					...
					if (!$vs_fld_num && is_array($va_restrict_to_fields = caGetOption('restrictSearchToFields', $pa_options, null)) && sizeof($va_restrict_to_fields)) {
						...
						// TODO: Variable '$vs_sql_where' is probably undefined
	...
	public function suggest($ps_text, $pa_options=null) {
		...
		foreach($va_tokens as $vn_i => $vs_token) {
			...
			while($qr_res->nextRow()) {
				...
				if (strpos($vs_word, $vs_token) === false) { 
				    ...
				    // TODO: Variable '$vn_score' is probably undefined
					$vn_score -= 150;
				}
		...
		if (sizeof($va_word_ids) > 1) {
			foreach($va_word_ids as $vn_i => $va_word_list) {
				...
				// TODO: Variable '{$vs_table_sql}' is probably undefined
				$vs_sql = "
					INSERT INTO {$vs_temp_table}
