...
class WLPlugSearchEngineSqlSearch extends BaseSearchPlugin implements IWLPlugSearchEngine
{
    ...
    static public $s_word_cache = array();                        // cached word-to-word_id values used when indexing
    static public $s_metadata_elements;                        // cached metadata element info
    static public $s_fieldnum_cache = array();                // cached field name-to-number values used when indexing
    ...
    public function __construct($po_db = null)
    {
        ...
        $this->opb_do_stemming = (int)trim(
            $this->opo_search_config->get('search_sql_search_do_stemming')
        ) ? true : false;
    ...
    private function initDbStatements()
    {
        ...
        $this->opqr_delete_with_field_num_without_rel_type_id = $this->opo_db->prepare(
        ...
        $this->opqr_delete_with_field_row_id_and_num = $this->opo_db->prepare(
    ...
    public function search(
        $pn_subject_tablenum,
        $ps_search_expression,
        $pa_filters = array(),
        $po_rewritten_query = null
    ) {
        $t = new Timer();
        ...
        if (trim($ps_search_expression) === '((*))') {
            ...
            if (is_array($va_filters) && sizeof($va_filters)) {
                foreach ($va_filters as $va_filter) {
                    ...
                    if (sizeof($va_path)) {
                        ...
                        $vs_where = "(" . $va_filter['field'] . " " . $va_filter['operator'] . " " . $this->_filterValueToQueryValue(
                                $va_filter
                            ) . ")";
                    } else {
            ...
            $this->_doQueriesForSqlSearch(
                $po_rewritten_query,
                $pn_subject_tablenum,
                'ca_sql_search_search_final',
                0,
                array(
                ...
                foreach ($va_filters as $va_filter) {
                    ...
                    if (sizeof($va_path)) {
                        ...
                        foreach ($va_path as $vs_table => $va_info) {
                            $t_table = Datamodel::getInstanceByTableName($vs_table, true);
                            if (!$vs_last_table) {
                                $va_joins[$vs_table] = "INNER JOIN " . $vs_table . " ON " . $vs_table . "." . $t_table->primaryKey(
                        ...
                        if (!isset($va_joins[$va_tmp[0]])) {
                            $va_joins[$va_tmp[0]] = "INNER JOIN " . $va_tmp[0] . " ON " . $va_tmp[0] . "." . $t_table->primaryKey(
                                ) . " = ca_sql_search_search_final.row_id";
                        }
                        $vs_where = "(" . $va_filter['field'] . " " . $va_filter['operator'] . " " . $this->_filterValueToQueryValue(
                                $va_filter
    ...
    private function _getElementIDForAccessPoint($pn_subject_tablenum, $ps_access_point)
    {
        ...
        $va_rel_type_ids = ($va_tmp[1] && $vs_rel_table) ? caMakeRelationshipTypeIDList(
            $vs_rel_table,
            preg_split(
                "![,;]+!",
                $va_tmp[1]
        ...
        if (strtolower($vs_field) == 'count') {
            ...
            $vs_fld_num = null;
            if ($vn_fld_num = $this->getFieldNum(
                $vs_table,
                $vs_subsubsubfield ? $vs_subsubsubfield : $vs_subsubfield
            )) {
                $vs_fld_num = "I{$vn_fld_num}";
            } elseif ($t_element = ca_metadata_elements::getInstance(
                $vs_subsubsubfield ? $vs_subsubsubfield : $vs_subsubfield
            )) {
                        ...
                        case 'AND':
                            if ($vn_i > 0) {
                                ...
                                $qr_res = $this->opo_db->query(
                                    "INSERT INTO {$ps_dest_table} SELECT row_id, boost, field_container_id FROM {$ps_dest_table}_acc"
                                );
                ...
                case 'Zend_Search_Lucene_Search_Query_Range':
                    ...
                    switch (get_class($o_lucene_query_element)) {
                        case 'Zend_Search_Lucene_Search_Query_Range':
                            ...
                            if (is_array($va_tmp) && (sizeof($va_tmp) > 1) && is_array(
                                    $va_indexed_fields
                            ...
                            $va_element = $this->_getElementIDForAccessPoint(
                                $pn_subject_tablenum,
                                $va_lower_term->field
                            );

                            $vn_direct_sql_target_table_num = $va_element['table_num'];
                            $va_indexed_fields = $o_base->getFieldsToIndex(
                                $pn_subject_tablenum,
                            ...
                            switch ($va_element['datatype']) {
                                ...
                                case __CA_ATTRIBUTE_VALUE_GEOCODE__:
                                    ...
                                    $va_parsed_value = $t_geocode->parseValue(
                                        '[' . $va_lower_term->text . ']',
                                    ...
                                    $vs_direct_sql_query = "
										...
										WHERE
											...
											AND
											(cav.value_decimal1 BETWEEN " . floatval(
                                            $vs_lower_lat
                                        ) . " AND " . floatval($vs_upper_lat) . ")
											AND
											(cav.value_decimal2 BETWEEN " . floatval(
                                            $vs_lower_long
                                        ) . " AND " . floatval($vs_upper_long) . ")	
									";
                                    break;
                                case __CA_ATTRIBUTE_VALUE_CURRENCY__:
                                    $t_cur = new CurrencyAttributeValue();
                                    $va_parsed_value = $t_cur->parseValue(
                                        $va_lower_term->text,
                                        $va_element['element_info']
                                    );
                                    $vs_currency = preg_replace(
                                        '![^A-Z0-9]+!',
                                        '',
                                ...
                                case __CA_ATTRIBUTE_VALUE_TIMECODE__:
                                    ...
                                    $va_parsed_value = $t_timecode->parseValue(
                                        $va_lower_term->text,
                                        $va_element['element_info']
                                    );
                                    $vn_lower_val = $va_parsed_value['value_decimal1'];
                                ...
                                case __CA_ATTRIBUTE_VALUE_LENGTH__:
                                    ...
                                    $va_parsed_value = $t_len->parseValue(
                                        $va_lower_term->text,
                                        $va_element['element_info']
                                    );
                                    $vn_lower_val = $va_parsed_value['value_decimal1'];
                                ...
                                case __CA_ATTRIBUTE_VALUE_WEIGHT__:
                                    ...
                                    $va_parsed_value = $t_weight->parseValue(
                                        $va_lower_term->text,
                                        $va_element['element_info']
                                    );
                                    $vn_lower_val = $va_parsed_value['value_decimal1'];
                                ...
                                case __CA_ATTRIBUTE_VALUE_INTEGER__:
                                    ...
                                    $vs_direct_sql_query = "
										...
										WHERE
											(cav.element_id = " . intval($va_element['element_id']) . ") AND (ca.table_num = ?)
											AND
											(cav.value_integer1 BETWEEN " . floatval(
                                            $vn_lower_val
                                        ) . " AND " . floatval($vn_upper_val) . ")
                            ...
                            if (!$vs_direct_sql_query) {
                                $vs_direct_sql_query = "
									...
									WHERE
										(cav.element_id = " . intval($va_element['element_id']) . ") AND (ca.table_num = ?)
										AND
										(cav.value_decimal1 BETWEEN " . floatval($vn_lower_val) . " AND " . floatval(
                                        $vn_upper_val
                                    ) . ")
                        ...
                        case 'Zend_Search_Lucene_Search_Query_Phrase':
                            ...
                            if ($this->getOption('strictPhraseSearching') && !in_array(
                                    $vs_first_term_table,
                                    array(
                                        _t('modified'),
                                        _t('created')
                                ...
                                foreach ($o_lucene_query_element->getQueryTerms() as $o_term) {
                                    ...
                                    $va_terms = $this->_tokenize(
                                        (string)$o_term->text,
                                        true
                                    ); //preg_split("![".$this->ops_search_tokenizer_regex."]+!u", (string)$o_term->text);
                                ...
                                if (sizeof($va_ap_tmp) >= 2) {
                                    ...
                                    if (isset($va_element['field_num'], $va_element['table_num'])) {
                                        ...
                                        if (is_array($va_element['relationship_type_ids']) && sizeof(
                                                $va_element['relationship_type_ids']
                                            )) {
                                ...
                                foreach ($va_words as $vs_word) {
                                    $vn_w++;
                                    $vs_temp_table = 'ca_sql_search_phrase_' . md5(
                                            $pn_subject_tablenum . "/" . $vs_word . "/" . $vn_w
                                        );
									...
									" . (sizeof($va_temp_tables) ? " INNER JOIN " . $va_temp_tables[sizeof(
                                                $va_temp_tables
                                            ) - 1] . " AS tt ON swi.index_id = tt.row_id" : "") . "
									WHERE 
                        ...
                        default:
                            ...
                            foreach ($va_term_objs as $o_term) {
                                $va_access_point_info = $this->_getElementIDForAccessPoint(
                                    $pn_subject_tablenum,
                                    $o_term->field
                                );
                                ...
                                foreach ($va_terms as $vs_term) {
                                    ...
                                    if ($vb_has_wildcard) {
                                        ...
                                        if ($vb_do_stemming) {
                                            ...
                                            if (!preg_match('!y$!u', $vs_to_stem) && (preg_match(
                                                        '!^[\d]+$!',
                                                        $vs_to_stem
                                                    ) || preg_match(
                                                        '!^[\w\-]+$!',
                    ...
                    if ($vs_access_point) {
                        ...
                        if (in_array($vs_table, array(_t('created'), _t('modified')))) {
                            ...
                            if ($vs_field = trim($vs_field)) {
                                if (!is_int($vs_field)) {
                                    ...
                                    if (
                                        ...
                                        ||
                                        ((strpos($vs_field, "_") !== false) && $t_user->load(
                                                array("user_name" => str_replace("_", " ", $vs_field))
                                            ))
                                    ) {
                            ...
                            if ((!$vb_is_blank_search && !$vb_is_not_blank_search) && $vs_table && $vs_field && ($t_table = Datamodel::getInstanceByTableName(
                                    $vs_table,
                                    true
                                ))) {
                                $vs_table_num = $t_table->tableNum();
                                ...
                                if (is_numeric($vs_field)) {
                                    ...
                                    if (!strlen($vn_fld_num)) {
                                        ...
                                        if ($t_element->load(
                                            array('element_code' => ($vs_sub_field ? $vs_sub_field : $vs_field))
                                        )) {
                                                ...
                                                switch ($t_element->get('datatype')) {
                                                    case __CA_ATTRIBUTE_VALUE_DATERANGE__:
                                                        ...
                                                        switch (substr($vs_raw_term, 0, 4)) {
                                                                ...
                                                                if ($this->opo_tep->parse($vs_raw_term)) {
                                                                    ...
                                                                    $vs_direct_sql_query = "
																		...
																		WHERE
																			...
																			AND
																			(
																				(cav.value_decimal2 <{$vs_eq} " . (($vs_eq === '=') ? floatval(
                                                                            $va_dates['end']
                                                                ...
                                                                if ($this->opo_tep->parse($vs_raw_term)) {
                                                                    ...
                                                                    $vs_direct_sql_query = "
																		...
																		WHERE
																			...
																			AND
																			(
																				(cav.value_decimal1 >{$vs_eq} " . (($vs_eq === '=') ? floatval(
                                                                            $va_dates['start']
                                                            ...
                                                            default:
                                                                if ($this->opo_tep->parse($vs_raw_term)) {
                                                                    $va_dates = $this->opo_tep->getHistoricTimestamps();
                                                                    if (((int)$va_dates[0] === -2000000000) && $this->opo_search_config->get(
                                                                            'treat_before_dates_as_circa'
                                                                        )) {
                                                                        $va_dates[0] = $va_dates['start'] = (int)$va_dates['end'] + 0.1231235959;
                                                                    }
                                                                    if (((int)$va_dates[1] === 2000000000) && $this->opo_search_config->get(
                                                                            'treat_after_dates_as_circa'
                                                                        )) {
                                                                    ...
                                                                    $vs_direct_sql_query = "
																		...
																		WHERE
                                                                            ...
                                                                            $va_dates['start']
																				...
																				OR
																				(cav.value_decimal1 <= " . floatval(
                                                                            $va_dates['start']
                                                                        ) . " AND cav.value_decimal2 >= " . floatval(
                                                                            $va_dates['end']
                                                                        ) . ")	
																			)
                                                    ...
                                                    case __CA_ATTRIBUTE_VALUE_GEOCODE__:
                                                        ...
                                                        switch ($vs_class) {
                                                            ...
                                                            default:
                                                                $va_term_objs = array(
                                                                    $o_lucene_query_element->getTerm()
                                                                );
                                                                break;
                                                        }
                                                        foreach ($va_term_objs as $o_term) {
                                                    ...
                                                    case __CA_ATTRIBUTE_VALUE_CURRENCY__:
                                                        ...
                                                        $vs_direct_sql_query = "
															...
															WHERE
																...
																AND
																(cav.value_longtext1 = '" . $this->opo_db->escape(
                                                                $vs_currency
                                                            ) . "')
                                                    ...
                                                    case __CA_ATTRIBUTE_VALUE_LENGTH__:
                                                        ...
                                                        try {
                                                            switch (sizeof($va_raw_terms)) {
                                                                case 2:
                                                                    $vs_dimension = $va_raw_terms[0] . caGetDecimalSeparator(
                                                                        ) . $va_raw_terms[1];
                                                                    break;
                                                                case 3:
                                                                    $vs_dimension = $va_raw_terms[0] . caGetDecimalSeparator(
                                                                        ) . $va_raw_terms[1] . " " . $va_raw_terms[2];
                                                                    break;
                                                                default:
                                                                    $vs_dimension = join(' ', $va_raw_terms);
                                                            }
                                                            $vo_parsed_measurement = caParseLengthDimension(
                                                                $vs_dimension
                                                            );
                                                            $vn_len = $vo_parsed_measurement->convertTo(
                                                                'METER',
                                                                6,
                                                                'en_US'
                                                            );
                                                    ...
                                                    case __CA_ATTRIBUTE_VALUE_WEIGHT__:
                                                        ...
                                                        try {
                                                            ...
                                                            $vo_parsed_measurement = caParseWeightDimension(
                                                                $vs_dimension
                                                            );
                                                            $vn_weight = $vo_parsed_measurement->convertTo(
                                                                'KILOGRAM',
                                                                6,
                                                                'en_US'
                                                            );
                                                    ...
                                                    case __CA_ATTRIBUTE_VALUE_INTEGER__:
                                                        $vs_direct_sql_query = "
															...
															WHERE
																(cav.element_id = {$vn_fld_num}) AND (ca.table_num = ?)
																AND
																(cav.value_integer1 = " . intval(
                                                                array_shift($va_raw_terms)
                                                            ) . ")
                                        ...
                                        case '#eq#':
                                            ...
                                            if ($this->opo_tep->parse($vs_raw_term)) {
                                                ...
                                                $vs_direct_sql_query = "
													...
													WHERE
														(
															({$vs_date_start_fld} BETWEEN " . floatval(
                                                        $va_dates['start']
                                                    ) . " AND " . floatval($va_dates['end']) . ")
															AND
															({$vs_date_end_fld} BETWEEN " . floatval(
                                                        $va_dates['start']
                                                    ) . " AND " . floatval($va_dates['end']) . ")
														)
                                        ...
                                        case '#lt#':
                                            ...
                                            if ($this->opo_tep->parse($vs_raw_term)) {
                                                ...
                                                $vs_direct_sql_query = "
													...
													WHERE
														(
															({$vs_date_end_fld} <{$vs_eq} " . (($vs_eq === '=') ? floatval(
                                                        $va_dates['end']
                                                    ) : floatval($va_dates['start'])) . ")
														)
                                        ...
                                        case '#gt#':
                                            ...
                                            if ($this->opo_tep->parse($vs_raw_term)) {
                                                ...
                                                $vs_direct_sql_query = "
													...
													WHERE
														(
															({$vs_date_start_fld} >{$vs_eq} " . (($vs_eq === '=') ? floatval(
                                                        $va_dates['start']
                                                    ) : floatval($va_dates['end'])) . ")
														)
                                                ...
                                                $vs_direct_sql_query = "
													...
													WHERE
                                                        ...
                                                        $va_dates['start']
															...
															OR
															({$vs_date_start_fld} <= " . floatval(
                                                        $va_dates['start']
                                                    ) . " AND {$vs_date_end_fld} >= " . floatval($va_dates['end']) . ")	
														)
                                        ...
                                        case '#eq#':
                                            ...
                                            if ($vs_raw_term == '0') {
                                                ...
                                                $vs_direct_sql_query = "
													...
													WHERE
														(
															({$vs_intrinsic_field_name} BETWEEN " . floatval(
                                                        $va_dates['start']
                                                    ) . " AND " . floatval($va_dates['end']) . ")
														)
                                        ...
                                        case '#lt#':
                                            ...
                                            if ($this->opo_tep->parse($vs_raw_term)) {
                                                ...
                                                $vs_direct_sql_query = "
													...
													WHERE
														(
															({$vs_intrinsic_field_name} <{$vs_eq} " . (($vs_eq === '=') ? floatval(
                                                        $va_dates['end']
                                                    ) : floatval($va_dates['start'])) . ")
														)
                    ...
                    if ($vb_is_blank_search) {
                        $va_sql_where[] = "((swi.field_table_num = " . intval(
                                $vs_table_num
                        ...
                        if (sizeof($va_ft_terms)) {
                            if (($t_table) && (strlen($vs_fld_num) > 1)) {
                                ...
                                if (!is_array(
                                    $va_field_info = $o_search->getFieldOptions(
                                        $pn_subject_tablenum,
                                        $vs_table_num,
                                        $vs_field
                                    )
                                ...
                                $va_sql_where[] = "((swi.field_table_num = " . intval(
                                        $vs_table_num
                                    ) . ") AND (swi.field_num = '{$vs_fld_num}') AND (sw.word IN (" . ((in_array(
                                            'DONT_TOKENIZE',
                                            $va_field_info,
                                            true
                                        ) || in_array('INDEX_AS_IDNO', $va_field_info, true)) ? join(
                                        ',',
                                        $va_raw_terms_escaped
                                    ) : join(',', $va_ft_terms)) . ")))";
                            } else {
                        ...
                        if (sizeof($va_ft_like_terms)) {
                            ...
                            if (($t_table) && (strlen($vs_fld_num) > 1)) {
                                $va_sql_where[] = "((swi.field_table_num = " . intval(
                                        $vs_table_num
                                    ) . ") AND (swi.field_num = '{$vs_fld_num}') AND (" . join(' AND ', $va_tmp) . "))";
                            } else {
                        ...
                        if (sizeof($va_ft_stem_terms)) {
                            if (($t_table) && (strlen($vs_fld_num) > 1)) {
                                $va_sql_where[] = "((swi.field_table_num = " . intval(
                                        $vs_table_num
                                    ) . ") AND (swi.field_num = '{$vs_fld_num}') AND (sw.stem IN (" . join(
                                        ',',
                                        $va_ft_stem_terms
                                    ) . ")))";
                            } else {
                    ...
                    if ((is_array($va_access_point_info['relationship_type_ids']) && sizeof(
                    ...
                    if (!$vs_fld_num && is_array(
                        ...
                        foreach ($va_restrict_to_fields as $va_restrict) {
                            $va_field_restrict_sql[] = "((swi.field_table_num = " . intval(
                                    $va_restrict['table_num']
                                ) . ") AND (swi.field_num = '" . $va_restrict['field_num'] . "'))";
                        }
                        $vs_sql_where .= " AND (" . join(" OR ", $va_field_restrict_sql) . ")";
                    }
                    if (!$vs_fld_num && is_array(
                            $va_exclude_fields_from_search = caGetOption('excludeFieldsFromSearch', $pa_options, null)
                    ...
                    if (($vn_direct_sql_target_table_num != $pn_subject_tablenum) && !$vb_dont_rewrite_direct_sql_query) {
                        ...
                        if ($t_target = Datamodel::getInstanceByTableNum($vn_direct_sql_target_table_num, true)) {
                            ...
                            $va_path = array_keys(
                                Datamodel::getPath($vn_direct_sql_target_table_num, $pn_subject_tablenum)
                            );

                            $vs_left_table = array_shift($va_path);
                            $va_join[] = "INNER JOIN {$vs_left_table} ON {$vs_left_table}." . Datamodel::primaryKey(
                                    $vs_left_table
                                ) . " = ca.row_id";
                            ...
                            foreach ($va_path as $vs_right_table) {
                                if (sizeof(
                                        $va_rels = Datamodel::getRelationships($vs_left_table, $vs_right_table)
                                    ) > 0) {
                            ...
                            $vs_direct_sql_query = str_replace(
                                "SELECT ca.row_id",
                                "SELECT " . Datamodel::primaryKey(
                                    $pn_subject_tablenum,
                                    true
                                ),
                                $vs_direct_sql_query
                    ...
                    if ($vn_i == 0) {
                        if ($vs_direct_sql_query) {
                            ...
                            if ((strpos($vs_sql, '?') !== false) && (!is_array($pa_direct_sql_query_params) || sizeof(
                        ...
                        if ((($vn_num_terms = (sizeof($va_ft_terms) + sizeof($va_ft_like_terms) + sizeof(
                                        $va_ft_stem_terms
                                    ))) > 1) && (!$vs_direct_sql_query)) {
                            $vs_sql .= " HAVING count(distinct sw.stem) >= {$vn_num_terms}";
                        }

                        $t = new Timer();
                        $pa_direct_sql_query_params = is_array(
                            $pa_direct_sql_query_params
                        ) ? $pa_direct_sql_query_params : array();
                            ...
                            case 'AND':
                                if ($vs_direct_sql_query) {
                                    if ($vn_direct_sql_target_table_num != $pn_subject_tablenum) {
                                        array_push(
                                            $va_join,
                                            "INNER JOIN {$ps_dest_table} AS ftmp1 ON ftmp1.row_id = " . Datamodel::primaryKey(
                                                $pn_subject_tablenum,
                                                true
                                            )
                                        );
                                ...
                                if (!$vs_direct_sql_query && (($vn_num_terms = (sizeof($va_ft_terms) + sizeof(
                                                $va_ft_like_terms
                                ...
                                if (is_array(
                                        $va_ids = $qr_res->getAllFieldValues(
                                            ($vs_direct_sql_query && ($vn_direct_sql_target_table_num != $pn_subject_tablenum)) ? Datamodel::primaryKey(
                                                $pn_subject_tablenum
                                            ) : 'row_id'
                                        )
                                    ) && sizeof($va_ids)) {
                                    $va_field_container_ids = array_filter(
                                        $qr_res->getAllFieldValues(
                                            ($vs_direct_sql_query) ? 'attribute_id' : 'field_container_id'
                                        ),
                                        "strlen"
                                    );
    ...
    public function indexField(
        ...
        if ($this->debug) {
            Debug::msg(
                "[SqlSearchDebug] indexField: $pn_content_tablenum/$ps_content_fieldname [$pn_content_row_id] =&gt; $pm_content"
            );
        }
        ...
        if ((!is_array($pm_content) && !strlen($pm_content)) || !sizeof($pm_content) || (((sizeof(
                        $pm_content
                    ) == 1) && strlen((string)$pm_content[0]) == 0))) {
            $va_words = null;
        } else {
            // Tokenize string
            $va_words = [];
        ...
        if (!defined("__CollectiveAccess_IS_REINDEXING__") && $vb_incremental_reindexing) {
            $this->removeRowIndexing(
                $this->opn_indexing_subject_tablenum,
                $this->opn_indexing_subject_row_id,
                $pn_content_tablenum,
                array($ps_content_fieldname),
                $pn_content_row_id,
                $vn_rel_type_id
            );
        }
    ...
    public function getWordID($ps_word)
    {
        ...
        if ($qr_res = $this->opqr_lookup_word->execute($ps_word)) {
            if ($qr_res->nextRow()) {
                return WLPlugSearchEngineSqlSearch::$s_word_cache[$ps_word] = (int)$qr_res->get(
                    'word_id',
                    array('binary' => true)
                );
            }
    ...
    {
        if ((sizeof(WLPlugSearchEngineSqlSearch::$s_word_cache)) > ($vn_max_size = $this->getOption(
                'maxWordCacheSize'
            ))) {
            WLPlugSearchEngineSqlSearch::$s_word_cache = array_slice(
                WLPlugSearchEngineSqlSearch::$s_word_cache,
                0,
                $vn_max_size * $this->getOption(
                    'cacheCleanFactor'
                ),
                true
            );
    ...
    public function removeRowIndexing(
        $pn_subject_tablenum,
        $pn_subject_row_id,
        $pn_field_tablenum = null,
        $pa_field_nums = null,
        $pn_field_row_id = null,
        $pn_rel_type_id = null
    ) {
        ...
        if ($pn_subject_tablenum && $pn_subject_row_id && !is_null($pn_field_tablenum) && !is_null(
                $pn_field_row_id
            ) && is_array($pa_field_nums) && sizeof($pa_field_nums)) {
            foreach ($pa_field_nums as $pn_field_num) {
                ...
                $this->opqr_delete_with_field_row_id_and_num->execute(
                    (int)$pn_subject_tablenum,
                    (int)$pn_subject_row_id,
                    (int)$pn_field_tablenum,
                    (string)$pn_field_num,
                    (int)$pn_field_row_id,
                    $vn_rel_type_id
                );
            ...
            if ($pn_subject_tablenum && $pn_subject_row_id && !is_null($pn_field_tablenum) && !is_null(
                ...
                return $this->opqr_delete_with_field_row_id->execute(
                    (int)$pn_subject_tablenum,
                    (int)$pn_subject_row_id,
                    (int)$pn_field_tablenum,
                    (int)$pn_field_row_id,
                ...
                if (!is_null($pn_field_tablenum) && is_array($pa_field_nums) && sizeof($pa_field_nums)) {
                    foreach ($pa_field_nums as $pn_field_num) {
                        ...
                        if (!is_null($pn_rel_type_id)) {
                            $this->opqr_delete_with_field_num->execute(
                                (int)$pn_subject_tablenum,
                                (int)$pn_subject_row_id,
                                (int)$pn_field_tablenum,
                                (string)$pn_field_num,
                            ...
                            $this->opqr_delete_with_field_num_without_rel_type_id->execute(
                                (int)$pn_subject_tablenum,
                                (int)$pn_subject_row_id,
                                (int)$pn_field_tablenum,
                                (string)$pn_field_num
                    ...
                    if (!$pn_subject_tablenum && !$pn_subject_row_id && !is_null($pn_field_tablenum) && !is_null(
                        ...
                        $this->opqr_delete_dependent_sql->execute(
                            (int)$pn_field_tablenum,
                            (int)$pn_field_row_id,
                            $vn_rel_type_id
                        );
                        ...
                        return $this->opqr_delete->execute(
                            (int)$pn_subject_tablenum,
                            (int)$pn_subject_row_id,
                            $vn_rel_type_id
                        );
    ...
    public function updateIndexingInPlace(
        ...
        if (caGetOption("INDEX_AS_IDNO", $pa_options, false) || in_array('INDEX_AS_IDNO', $pa_options, true)) {
            ...
            if (!sizeof($va_values) && method_exists(
                    $t_content,
    ...
    private function _tokenize($ps_content, $pb_for_search = false, $pn_index = 0)
    {
        $ps_content = preg_replace(
            ...
            '',
            $ps_content
        );        // strip apostrophes for compatibility with SearchEngine class, which does the same to all search expressions
        ...
        if ($pb_for_search) {
            ...
            return preg_split(
                ...
                trim(
                    preg_replace(
                        "%((?<!\d)[" . $this->ops_search_tokenizer_regex . "]+(?!\d))%u",
                        ' ',
            ...
            return preg_split(
                ...
                trim(
                    preg_replace(
                        "%((?<!\d)[" . $this->ops_search_tokenizer_regex . "]+|[" . $this->ops_search_tokenizer_regex . "]+(?!\d))%u",
                        ' ',
                        strip_tags($ps_content)
                    )
                )
    ...
    private function getFieldNum($pn_table_name_or_num, $ps_fieldname)
    {
        ...
        $vs_table_name = is_numeric($pn_table_name_or_num) ? Datamodel::getTableName(
            (int)$pn_table_name_or_num
        ) : (string)$pn_table_name_or_num;
        return WLPlugSearchEngineSqlSearch::$s_fieldnum_cache[$pn_table_name_or_num . '/' . $ps_fieldname] = Datamodel::getFieldNum(
            $vs_table_name,
            $ps_fieldname
        );
    }
