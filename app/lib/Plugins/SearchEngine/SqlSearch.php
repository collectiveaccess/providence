...
class WLPlugSearchEngineSqlSearch extends BaseSearchPlugin implements IWLPlugSearchEngine {
	...
	public function __construct( $po_db = null ) {
		...
		$this->opb_do_stemming = (int) trim( $this->opo_search_config->get( 'search_sql_search_do_stemming' ) ) ? true
			: false;

		$this->initDbStatements();

		if ( ! ( $this->ops_indexing_tokenizer_regex
			= trim( $this->opo_search_config->get( 'indexing_tokenizer_regex' ) ) )
		) {
			$this->ops_indexing_tokenizer_regex = "^\pL\pN\pNd/_#\@\&\.";
		}
		if ( ! ( $this->ops_search_tokenizer_regex
			= trim( $this->opo_search_config->get( 'search_tokenizer_regex' ) ) )
		) {
	...
	public function init() {
		if ( ( $vn_max_indexing_buffer_size = (int) $this->opo_search_config->get( 'max_indexing_buffer_size' ) )
		     < 1
		) {
		...
		$this->opa_options = array(
			'limit'                         => 2000,
			// maximum number of hits to return [default=2000]  ** NOT CURRENTLY ENFORCED -- MAY BE DROPPED **
			'maxIndexingBufferSize'         => $vn_max_indexing_buffer_size,
			// maximum number of indexed content items to accumulate before writing to the database
			'maxWordIndexInsertSegmentSize' => ceil( $vn_max_indexing_buffer_size / 2 ),
			// maximum number of word index rows to put into a single insert
			'maxWordCacheSize'              => 131072,
			// maximum number of words to cache while indexing before purging
			'cacheCleanFactor'              => 0.50,
			// percentage of words retained when cleaning the cache
			...
			// strict phrase searching finds only records with the precise phrase; non-strict will find fields with all of the words, in any order
		);
		...
		$this->opa_capabilities = array(
			...
			// can update indexing using only changed fields, rather than having to reindex the entire row (and related stuff) every time
	...
	private function initDbStatements() {
		...
		$this->ops_delete_sql
			               = "DELETE FROM ca_sql_search_word_index WHERE (table_num = ?) AND (row_id = ?) AND (rel_type_id = ?)";
		$this->opqr_delete = $this->opo_db->prepare( $this->ops_delete_sql );

		$this->ops_delete_with_field_num_sql
			                              = "DELETE FROM ca_sql_search_word_index WHERE (table_num = ?) AND (row_id = ?) AND (field_table_num = ?) AND (field_num = ?) AND (rel_type_id = ?)";
		$this->opqr_delete_with_field_num = $this->opo_db->prepare( $this->ops_delete_with_field_num_sql );

		$this->ops_delete_with_field_num_sql_without_rel_type_id
			= "DELETE FROM ca_sql_search_word_index WHERE (table_num = ?) AND (row_id = ?) AND (field_table_num = ?) AND (field_num = ?)";
		$this->opqr_delete_with_field_num_without_rel_type_id
			= $this->opo_db->prepare( $this->ops_delete_with_field_num_sql_without_rel_type_id );

		$this->ops_delete_with_field_row_id_sql
			                                 = "DELETE FROM ca_sql_search_word_index WHERE (table_num = ?) AND (row_id = ?) AND (field_table_num = ?) AND (field_row_id = ?) AND (rel_type_id = ?)";
		$this->opqr_delete_with_field_row_id = $this->opo_db->prepare( $this->ops_delete_with_field_row_id_sql );

		$this->ops_delete_with_field_row_id_and_num
			= "DELETE FROM ca_sql_search_word_index WHERE (table_num = ?) AND (row_id = ?) AND (field_table_num = ?) AND (field_num = ?) AND (field_row_id = ?) AND (rel_type_id = ?)";
		$this->opqr_delete_with_field_row_id_and_num
			= $this->opo_db->prepare( $this->ops_delete_with_field_row_id_and_num );

		$this->ops_delete_dependent_sql
			                             = "DELETE FROM ca_sql_search_word_index WHERE (field_table_num = ?) AND (field_row_id = ?) AND (rel_type_id = ?)";
	...
	public function truncateIndex( $pn_table_num = null ) {
		if ( $pn_table_num > 0 ) {
			$this->opo_db->query( "DELETE FROM ca_sql_search_word_index WHERE table_num = ?",
				array( (int) $pn_table_num ) );
		} else {
	...
	public function search(
		$pn_subject_tablenum, $ps_search_expression, $pa_filters = array(), $po_rewritten_query = null
	) {
		$t = new Timer();
		...
		if ( trim( $ps_search_expression ) === '((*))' ) {
						...
						foreach ( $va_path as $vs_table => $va_info ) {
							...
							if ( $vs_last_table ) {
								...
								if ( $vs_table == $va_rels['one_table'] ) {
									$va_joins[ $vs_table ] = "INNER JOIN " . $va_rels['one_table'] . " ON "
									                         . $va_rels['one_table'] . "." . $va_rels['one_table_field']
									                         . " = " . $va_rels['many_table'] . "."
									                         . $va_rels['many_table_field'];
								} else {
									$va_joins[ $vs_table ] = "INNER JOIN " . $va_rels['many_table'] . " ON "
									                         . $va_rels['many_table'] . "."
									                         . $va_rels['many_table_field'] . " = "
									                         . $va_rels['one_table'] . "."
									                         . $va_rels['one_table_field'];
								}
							}
							$t_last_table  = $t_table;
							$vs_last_table = $vs_table;
						}
						$vs_where = "(" . $va_filter['field'] . " " . $va_filter['operator'] . " "
						            . $this->_filterValueToQueryValue( $va_filter ) . ")";
					} else {
			...
			$this->_doQueriesForSqlSearch( $po_rewritten_query, $pn_subject_tablenum, 'ca_sql_search_search_final', 0,
				array(
					...
					'excludeFieldsFromSearch' => $va_exclude_fields_from_search
				) );
				...
				foreach ( $va_filters as $va_filter ) {
					...
					if ( sizeof( $va_path ) ) {
						...
						foreach ( $va_path as $vs_table => $va_info ) {
							$t_table = Datamodel::getInstanceByTableName( $vs_table, true );
							if ( ! $vs_last_table ) {
								$va_joins[ $vs_table ] = "INNER JOIN " . $vs_table . " ON " . $vs_table . "."
								                         . $t_table->primaryKey()
								                         . " = ca_sql_search_search_final.row_id";
							} else {
								$va_rels = Datamodel::getOneToManyRelations( $vs_last_table, $vs_table );
								if ( ! sizeof( $va_rels ) ) {
									$va_rels = Datamodel::getOneToManyRelations( $vs_table, $vs_last_table );
						...
						if ( ! isset( $va_joins[ $va_tmp[0] ] ) ) {
							$va_joins[ $va_tmp[0] ] = "INNER JOIN " . $va_tmp[0] . " ON " . $va_tmp[0] . "."
							                          . $t_table->primaryKey() . " = ca_sql_search_search_final.row_id";
						}
						$vs_where = "(" . $va_filter['field'] . " " . $va_filter['operator'] . " "
						            . $this->_filterValueToQueryValue( $va_filter ) . ")";
					}
	...
	private function _getElementIDForAccessPoint( $pn_subject_tablenum, $ps_access_point ) {
		...
		$va_rel_type_ids = ( $va_tmp[1] && $vs_rel_table ) ? caMakeRelationshipTypeIDList( $vs_rel_table,
			preg_split( "![,;]+!", $va_tmp[1] ) ) : [];
		...
		if ( strtolower( $vs_field ) == 'count' ) {
			...
			$vs_fld_num = null;
			if ( $vn_fld_num = $this->getFieldNum( $vs_table,
				$vs_subsubsubfield ? $vs_subsubsubfield : $vs_subsubfield )
			) {
				$vs_fld_num = "I{$vn_fld_num}";
			} elseif ( $t_element = ca_metadata_elements::getInstance( $vs_subsubsubfield ? $vs_subsubsubfield
				: $vs_subsubfield )
			) {
		...
		if ( ! strlen( $vs_fld_num ) ) {
			...
			return array(
				...
				'relationship_type'     => $va_tmp[1],
				'table_num'             => $vs_table_num,
				'field_num'             => 'I' . $vs_fld_num,
				'field_num_raw'         => $vs_fld_num,
				'datatype'              => null,
				'relationship_type_ids' => $va_rel_type_ids
			);
		}
	...
	# -------------------------------------------------------
	private function _doQueriesForSqlSearch(
		$po_rewritten_query, $pn_subject_tablenum, $ps_dest_table, $pn_level = 0, $pa_options = null
		...
		foreach ( $va_elements as $o_lucene_query_element ) {
			...
			if ( is_null( $va_old_signs ) ) {    // if array is null then according to Zend Lucene all subqueries should be "are required"... so we AND them
				...
				if ( is_null( $va_old_signs[ $vn_i ] ) ) {    // is the sign for a particular query is null then OR is (it is "neither required nor prohibited")
					...
					$vs_op = ( $va_old_signs[ $vn_i ] === false ) ? 'NOT'
						: 'AND';    // true sign indicated "required" (AND) operation, false indicated "prohibited" (NOT) operation
				}
			}
			if ( $vn_i == 0 ) {
				$vs_op = 'OR';
			...
			$va_direct_query_temp_tables
				                            = array();    // List of temporary tables created by direct search queries; tables listed here are dropped at the end of processing for the query element
			$pa_direct_sql_query_params
				                            = null; // set to array with values to use with direct SQL query placeholders or null to pass single standard table_num value as param (most queries just need this single value)
			...
			switch ( $vs_class = get_class( $o_lucene_query_element ) ) {
				...
				case 'Zend_Search_Lucene_Search_Query_MultiTerm':
					...
					if ( ( $vs_op == 'AND' ) && ( $vn_i == 0 ) ) {
						$this->_doQueriesForSqlSearch( $o_lucene_query_element, $pn_subject_tablenum, $ps_dest_table,
							( $pn_level + 1 ) );
					} else {
						$this->_doQueriesForSqlSearch( $o_lucene_query_element, $pn_subject_tablenum,
							'ca_sql_search_temp_' . $pn_level, ( $pn_level + 1 ) );
					}
					...
					switch ( $vs_op ) {
						case 'AND':
							if ( $vn_i > 0 ) {
								...
								$qr_res
									= $this->opo_db->query( "INSERT INTO {$ps_dest_table} SELECT row_id, boost, field_container_id FROM {$ps_dest_table}_acc" );
				...
				case 'Zend_Search_Lucene_Search_Query_Range':
					...
					switch ( get_class( $o_lucene_query_element ) ) {
						case 'Zend_Search_Lucene_Search_Query_Range':
							...
							if ( is_array( $va_tmp ) && ( sizeof( $va_tmp ) > 1 ) && is_array( $va_indexed_fields )
							     && isset( $va_indexed_fields[ $va_tmp[1] ] )
							) {
								// is intrinsic
								if ( $t_instance = Datamodel::getInstanceByTableNum( $pn_subject_tablenum, true ) ) {
							...
							$va_element = $this->_getElementIDForAccessPoint( $pn_subject_tablenum,
								$va_lower_term->field );

							$vn_direct_sql_target_table_num = $va_element['table_num'];
							$va_indexed_fields              = $o_base->getFieldsToIndex( $pn_subject_tablenum,
								$vn_direct_sql_target_table_num );

							$vn_root_element_id = $va_element['element_info']['hier_element_id'];
							if ( ( $va_element['datatype'] !== 'COUNT' )
							     && ! isset( $va_indexed_fields[ '_ca_attribute_' . $va_element['element_id'] ] )
							     && ( ! $vn_root_element_id
							          || ( $vn_root_element_id
							               && ! isset( $va_indexed_fields[ '_ca_attribute_'
							                                               . $vn_root_element_id ] ) ) )
							) {
								break( 2 );
							} // skip if not indexed
							...
							switch ( $va_element['datatype'] ) {
								case 'COUNT':
									...
									$vs_direct_sql_query              = "
										...
										WHERE
											...
											(ca.field_table_num = ?)
											AND
											(ca.rel_type_id IN (" . join( ',',
											( is_array( $va_element['relationship_type_ids'] )
											  && sizeof( $va_element['relationship_type_ids'] ) )
												? $va_element['relationship_type_ids'] : [ 0 ] ) . "))
											AND
											(ca.field_num = '" . $va_element['field_num'] . "')
											AND
											(sw.word BETWEEN " . (int) $va_lower_term->text . " and "
									                                    . (int) $va_upper_term->text . ")
											
									" . ( $this->getOption( 'omitPrivateIndexing' ) ? " AND ca.access = 0" : '' );
									break;
								case __CA_ATTRIBUTE_VALUE_GEOCODE__:
									$t_geocode       = new GeocodeAttributeValue();
									$va_parsed_value = $t_geocode->parseValue( '[' . $va_lower_term->text . ']',
										$va_element['element_info'] );
									$vs_lower_lat    = $va_parsed_value['value_decimal1'];
									$vs_lower_long   = $va_parsed_value['value_decimal2'];

									$va_parsed_value = $t_geocode->parseValue( '[' . $va_upper_term->text . ']',
										$va_element['element_info'] );
									...
									$vs_direct_sql_query = "
										...
										WHERE
											...
											AND
											(cav.value_decimal1 BETWEEN " . floatval( $vs_lower_lat ) . " AND "
									                       . floatval( $vs_upper_lat ) . ")
											AND
											(cav.value_decimal2 BETWEEN " . floatval( $vs_lower_long ) . " AND "
									                       . floatval( $vs_upper_long ) . ")	
									";
									break;
								case __CA_ATTRIBUTE_VALUE_CURRENCY__:
									$t_cur           = new CurrencyAttributeValue();
									$va_parsed_value = $t_cur->parseValue( $va_lower_term->text,
										$va_element['element_info'] );
									$vs_currency     = preg_replace( '![^A-Z0-9]+!', '',
										$va_parsed_value['value_longtext1'] );
									$vn_lower_val    = $va_parsed_value['value_decimal1'];

									$va_parsed_value = $t_cur->parseValue( $va_upper_term->text,
										$va_element['element_info'] );
									...
									$vs_direct_sql_query = "
										...
										WHERE
											(cav.element_id = " . intval( $va_element['element_id'] ) . ") AND (ca.table_num = ?)
											AND
											(cav.value_decimal1 BETWEEN " . floatval( $vn_lower_val ) . " AND "
									                       . floatval( $vn_upper_val ) . ")
											AND
								...
								case __CA_ATTRIBUTE_VALUE_TIMECODE__:
									...
									$va_parsed_value = $t_timecode->parseValue( $va_lower_term->text,
										$va_element['element_info'] );
									$vn_lower_val    = $va_parsed_value['value_decimal1'];

									$va_parsed_value = $t_timecode->parseValue( $va_upper_term->text,
										$va_element['element_info'] );
									$vn_upper_val    = $va_parsed_value['value_decimal1'];
									break;
								case __CA_ATTRIBUTE_VALUE_LENGTH__:
									$t_len           = new LengthAttributeValue();
									$va_parsed_value = $t_len->parseValue( $va_lower_term->text,
										$va_element['element_info'] );
									$vn_lower_val    = $va_parsed_value['value_decimal1'];

									$va_parsed_value = $t_len->parseValue( $va_upper_term->text,
										$va_element['element_info'] );
									$vn_upper_val    = $va_parsed_value['value_decimal1'];
									break;
								case __CA_ATTRIBUTE_VALUE_WEIGHT__:
									$t_weight        = new WeightAttributeValue();
									$va_parsed_value = $t_weight->parseValue( $va_lower_term->text,
										$va_element['element_info'] );
									$vn_lower_val    = $va_parsed_value['value_decimal1'];

									$va_parsed_value = $t_weight->parseValue( $va_upper_term->text,
										$va_element['element_info'] );
									$vn_upper_val    = $va_parsed_value['value_decimal1'];
									break;
								case __CA_ATTRIBUTE_VALUE_INTEGER__:
									...
									$vs_direct_sql_query = "
										...
										WHERE
											(cav.element_id = " . intval( $va_element['element_id'] ) . ") AND (ca.table_num = ?)
											AND
											(cav.value_integer1 BETWEEN " . floatval( $vn_lower_val ) . " AND "
									                       . floatval( $vn_upper_val ) . ")
						...
						case 'Zend_Search_Lucene_Search_Query_Phrase':
							...
							if ( $this->getOption( 'strictPhraseSearching' )
							     && ! in_array( $vs_first_term_table, array( _t( 'modified' ), _t( 'created' ) ) )
							) {
								$va_words = array();
								foreach ( $o_lucene_query_element->getQueryTerms() as $o_term ) {
									...
									$va_terms               = $this->_tokenize( (string) $o_term->text,
										true ); //preg_split("![".$this->ops_search_tokenizer_regex."]+!u", (string)$o_term->text);
								...
								if ( sizeof( $va_ap_tmp ) >= 2 ) {
									...
									if ( isset( $va_element['field_num'], $va_element['table_num'] ) ) {
										...
										$vs_fld_limit_sql
										                  = " AND (swi.field_table_num = {$vs_fld_table_num} AND swi.field_num = '{$vs_fld_num}')";

										if ( is_array( $va_element['relationship_type_ids'] )
										     && sizeof( $va_element['relationship_type_ids'] )
										) {
											$vs_fld_limit_sql .= " AND (swi.rel_type_id IN (" . join( ",",
													$va_element['relationship_type_ids'] ) . "))";
										}
									}
								}
								...
								foreach ( $va_words as $vs_word ) {
									$vn_w ++;
									$vs_temp_table = 'ca_sql_search_phrase_' . md5( $pn_subject_tablenum . "/"
									                                                . $vs_word . "/" . $vn_w );
									...
									" . ( sizeof( $va_temp_tables ) ? " INNER JOIN "
									                                  . $va_temp_tables[ sizeof( $va_temp_tables ) - 1 ]
									                                  . " AS tt ON swi.index_id = tt.row_id" : "" ) . "
									WHERE 
						...
						default:
							...
							foreach ( $va_term_objs as $o_term ) {
								$va_access_point_info           = $this->_getElementIDForAccessPoint( $pn_subject_tablenum,
									$o_term->field );
								...
								if ( $vs_access_point
								     && ( mb_strtoupper( $vs_raw_term ) == '['
								                                           . caGetBlankLabelText( $pn_subject_tablenum )
								                                           . ']' )
								) {
									$t_ap = Datamodel::getInstanceByTableNum( $va_access_point_info['table_num'],
										true );
									if ( is_a( $t_ap,
										'BaseLabel' )
										...
										break;
								           ...
								           && ( mb_strtoupper( $vs_raw_term ) == '[' . _t( 'SET' ) . ']' )
								) {
								...
								foreach ( $va_terms as $vs_term ) {
									...
									if ( $vb_has_wildcard ) {
										...
										if ( mb_substr( $vs_term, - 1 ) == '|' ) {
											...
											$vs_raw_term    = mb_substr( $vs_raw_term, 0,
												mb_strlen( $vs_raw_term ) - 1 );
										...
										if ( $vb_do_stemming ) {
											...
											if ( ! preg_match( '!y$!u', $vs_to_stem )
											     && ( preg_match( '!^[\d]+$!', $vs_to_stem )
											          || preg_match( '!^[\w\-]+$!', $vs_to_stem ) )
											) {    // don't stem things ending in 'y' as that can cause problems (eg "Bowery" becomes "Boweri")
												if ( ! ( $vs_stem
													= trim( $this->opo_stemmer->stem( $vs_to_stem ) ) )
												) {
								...
								if ( $vb_output_term ) {
									...
									$va_raw_terms_escaped[] = '"' . $this->opo_db->escape( $vs_raw_term ) . '"';
								}
							}
					...
					if ( $vs_access_point ) {
						...
						if ( in_array( $vs_table, array( _t( 'created' ), _t( 'modified' ) ) ) ) {
							...
							if ( $vs_field = trim( $vs_field ) ) {
								if ( ! is_int( $vs_field ) ) {
									...
									if (
										...
										|| ( ( strpos( $vs_field, "_" ) !== false )
										     && $t_user->load( array(
												"user_name" => str_replace( "_", " ", $vs_field )
											) ) )
									) {
							...
							switch ( $vs_table ) {
								case _t( 'created' ):
									$vs_direct_sql_query = "
											...
											WHERE
												(ccl.log_datetime BETWEEN " . (int) $va_range['start'] . " AND "
									                       . (int) $va_range['end'] . ")
												AND
							...
							if ( ( ! $vb_is_blank_search && ! $vb_is_not_blank_search ) && $vs_table && $vs_field
							     && ( $t_table = Datamodel::getInstanceByTableName( $vs_table, true ) )
							) {
								$vs_table_num = $t_table->tableNum();
								...
								if ( is_numeric( $vs_field ) ) {
									...
									if ( ! strlen( $vn_fld_num ) ) {
										...
										if ( $t_element->load( array(
											...
											$va_indexed_fields
											                                = $o_base->getFieldsToIndex( $pn_subject_tablenum,
												$vn_direct_sql_target_table_num );
											$vn_fld_num                     = $t_element->getPrimaryKey();
											$vn_root_element_id             = $t_element->get( 'hier_element_id' );

											if ( ! isset( $va_indexed_fields[ '_ca_attribute_' . $vn_fld_num ] )
											     && ( ! $vn_root_element_id
											          || ( $vn_root_element_id
											               && ! isset( $va_indexed_fields[ '_ca_attribute_'
											                                               . $vn_root_element_id ] ) ) )
												...
												switch ( $t_element->get( 'datatype' ) ) {
														...
														switch ( substr( $vs_raw_term, 0, 4 ) ) {
															case '#eq#':
																...
																if ( $this->opo_tep->parse( $vs_raw_term ) ) {
																	...
																	$vs_direct_sql_query            = "
																		SELECT ca.row_id, 1, " . ( $vs_sub_field
																			? "ca.attribute_id" : "null" ) . "
																		FROM ca_attribute_values cav
																		INNER JOIN ca_attributes AS ca ON ca.attribute_id = cav.attribute_id
																		^JOIN
																		WHERE
																			...
																			AND
																			(
																				(cav.value_decimal1 BETWEEN "
																	                                  . floatval( $va_dates['start'] )
																	                                  . " AND "
																	                                  . floatval( $va_dates['end'] )
																	                                  . ")
																				AND
																				(cav.value_decimal2 BETWEEN "
																	                                  . floatval( $va_dates['start'] )
																	                                  . " AND "
																	                                  . floatval( $va_dates['end'] )
																	                                  . ")
															...
															default:
																if ( $this->opo_tep->parse( $vs_raw_term ) ) {
																	$va_dates = $this->opo_tep->getHistoricTimestamps();
																	if ( ( (int) $va_dates[0] === - 2000000000 )
																	     && $this->opo_search_config->get( 'treat_before_dates_as_circa' )
																	) {
																		$va_dates[0]
																			=
																		$va_dates['start'] = (int) $va_dates['end']
																		                     + 0.1231235959;
																	}
																	if ( ( (int) $va_dates[1] === 2000000000 )
																	     && $this->opo_search_config->get( 'treat_after_dates_as_circa' )
																	) {
																		$va_dates[1]
																			=
																		$va_dates['end'] = (int) $va_dates['start'];
																	}
													...
													case __CA_ATTRIBUTE_VALUE_GEOCODE__:
														...
														switch ( $vs_class ) {
															case 'Zend_Search_Lucene_Search_Query_Phrase':
																$va_term_objs
																	= $o_lucene_query_element->getQueryTerms();
																break;
															case 'Zend_Search_Lucene_Index_Term':
																$va_term_objs = array( $o_lucene_query_element );
																break;
															default:
																$va_term_objs
																	= array( $o_lucene_query_element->getTerm() );
																break;
														}
														foreach ( $va_term_objs as $o_term ) {
															$va_gis_terms[] = trim( (string) $o_term->text );
														}
														if ( $va_coords = caParseGISSearch( join( ' ',
															$va_gis_terms ) )
														) {
															$vs_direct_sql_query            = "
																SELECT ca.row_id, 1, " . ( $vs_sub_field
													...
													case __CA_ATTRIBUTE_VALUE_CURRENCY__:
														$t_cur           = new CurrencyAttributeValue();
														$va_parsed_value = $t_cur->parseValue( join( ' ',
															$va_raw_terms ), $t_element->getFieldValuesArray() );
														$vn_amount       = $va_parsed_value['value_decimal1'];
														$vs_currency     = preg_replace( '![^A-Z0-9]+!', '',
															$va_parsed_value['value_longtext1'] );

														$vs_direct_sql_query            = "
															SELECT ca.row_id, 1, " . ( $vs_sub_field ? "ca.attribute_id"
																: "null" ) . "
															FROM ca_attribute_values cav
															INNER JOIN ca_attributes AS ca ON ca.attribute_id = cav.attribute_id
															^JOIN
															WHERE
																...
																AND
																(cav.value_longtext1 = '"
														                                  . $this->opo_db->escape( $vs_currency )
														                                  . "')
													...
													case __CA_ATTRIBUTE_VALUE_LENGTH__:
														...
														try {
															switch ( sizeof( $va_raw_terms ) ) {
																case 2:
																	$vs_dimension = $va_raw_terms[0]
																	                . caGetDecimalSeparator()
																	                . $va_raw_terms[1];
																	break;
																case 3:
																	$vs_dimension = $va_raw_terms[0]
															...
															$vo_parsed_measurement
																    = caParseLengthDimension( $vs_dimension );
															$vn_len = $vo_parsed_measurement->convertTo( 'METER', 6,
																'en_US' );
														} catch ( Exception $e ) {
															$vs_direct_sql_query = null;
															break;
													...
													case __CA_ATTRIBUTE_VALUE_WEIGHT__:
														...
														try {
															...
															$vo_parsed_measurement
																       = caParseWeightDimension( $vs_dimension );
															$vn_weight = $vo_parsed_measurement->convertTo( 'KILOGRAM',
																6, 'en_US' );
														} catch ( Exception $e ) {
															$vs_direct_sql_query = null;
															break;
													...
													case __CA_ATTRIBUTE_VALUE_TIMECODE__:
														$t_timecode      = new TimecodeAttributeValue();
														$va_parsed_value = $t_timecode->parseValue( join( ' ',
															$va_raw_terms ), $t_element->getFieldValuesArray() );
													...
													case __CA_ATTRIBUTE_VALUE_INTEGER__:
														$vs_direct_sql_query            = "
															...
															WHERE
																...
																AND
																(cav.value_integer1 = "
														                                  . intval( array_shift( $va_raw_terms ) )
														                                  . ")
													...
													case __CA_ATTRIBUTE_VALUE_NUMERIC__:
														$vs_direct_sql_query            = "
															...
															WHERE
																...
																AND
																(cav.value_decimal1 = "
														                                  . floatval( array_shift( $va_raw_terms ) )
														                                  . ")
										...
										case '#eq#':
											...
											if ( $this->opo_tep->parse( $vs_raw_term ) ) {
												...
												$vs_direct_sql_query = "
													...
													WHERE
														(
															({$vs_date_start_fld} BETWEEN "
												                       . floatval( $va_dates['start'] ) . " AND "
												                       . floatval( $va_dates['end'] ) . ")
															AND
															({$vs_date_end_fld} BETWEEN "
												                       . floatval( $va_dates['start'] ) . " AND "
												                       . floatval( $va_dates['end'] ) . ")
														)
													...
													WHERE
														(
															...
															({$vs_date_end_fld} BETWEEN "
												                       ...
												                       . floatval( $va_dates['end'] ) . ")
															OR
															({$vs_date_start_fld} <= " . floatval( $va_dates['start'] )
												                       . " AND {$vs_date_end_fld} >= "
												                       . floatval( $va_dates['end'] ) . ")	
														)
										...
										case '#eq#':
											...
											if ( $vs_raw_term == '0' ) {
												...
												$vs_direct_sql_query = "
													...
													WHERE
														(
															({$vs_intrinsic_field_name} BETWEEN "
												                       . floatval( $va_dates['start'] ) . " AND "
												                       . floatval( $va_dates['end'] ) . ")
														)
										...
										case '#lt#':
											...
											if ( $this->opo_tep->parse( $vs_raw_term ) ) {
												...
												$vs_direct_sql_query = "
													...
													WHERE
														(
															({$vs_intrinsic_field_name} <{$vs_eq} " . ( ( $vs_eq
												                                                          === '=' )
														? floatval( $va_dates['end'] )
														: floatval( $va_dates['start'] ) ) . ")
														)
					...
					if ( $vb_is_blank_search ) {
						$va_sql_where[] = "((swi.field_table_num = " . intval( $vs_table_num )
						                  . ") AND (swi.field_num = '{$vs_fld_num}') AND (swi.word_id = 0))";
						...
						if ( sizeof( $va_ft_terms ) ) {
							if ( ( $t_table ) && ( strlen( $vs_fld_num ) > 1 ) ) {
								...
								if ( ! is_array(
									$va_field_info = $o_search->getFieldOptions( $pn_subject_tablenum, $vs_table_num,
										$vs_field ) )
								) {
									$va_field_info = array();
								}
								$va_sql_where[] = "((swi.field_table_num = " . intval( $vs_table_num )
								                  . ") AND (swi.field_num = '{$vs_fld_num}') AND (sw.word IN ("
								                  . ( ( in_array( 'DONT_TOKENIZE', $va_field_info, true )
								                        || in_array( 'INDEX_AS_IDNO', $va_field_info, true ) )
										? join( ',', $va_raw_terms_escaped ) : join( ',', $va_ft_terms ) ) . ")))";
							} else {
						...
						if ( sizeof( $va_ft_like_terms ) ) {
							...
							if ( ( $t_table ) && ( strlen( $vs_fld_num ) > 1 ) ) {
								$va_sql_where[] = "((swi.field_table_num = " . intval( $vs_table_num )
								                  . ") AND (swi.field_num = '{$vs_fld_num}') AND (" . join( ' AND ',
										$va_tmp ) . "))";
							} else {
						...
						if ( sizeof( $va_ft_stem_terms ) ) {
							if ( ( $t_table ) && ( strlen( $vs_fld_num ) > 1 ) ) {
								$va_sql_where[] = "((swi.field_table_num = " . intval( $vs_table_num )
								                  . ") AND (swi.field_num = '{$vs_fld_num}') AND (sw.stem IN ("
								                  . join( ',', $va_ft_stem_terms ) . ")))";
							} else {
					...
					if ( ( is_array( $va_access_point_info['relationship_type_ids'] )
					       && sizeof( $va_access_point_info['relationship_type_ids'] ) )
					) {
						$vs_rel_type_id_sql = " AND (swi.rel_type_id IN (" . join( ",",
								$va_access_point_info['relationship_type_ids'] ) . "))";
					}
					if ( ! $vs_fld_num
					     && is_array(
						     $va_restrict_to_fields = caGetOption( 'restrictSearchToFields', $pa_options, null ) )
					     && sizeof( $va_restrict_to_fields )
					) {
						$va_field_restrict_sql = array();
						foreach ( $va_restrict_to_fields as $va_restrict ) {
							$va_field_restrict_sql[] = "((swi.field_table_num = " . intval( $va_restrict['table_num'] )
							                           . ") AND (swi.field_num = '" . $va_restrict['field_num'] . "'))";
						}
						$vs_sql_where .= " AND (" . join( " OR ", $va_field_restrict_sql ) . ")";
					}
					if ( ! $vs_fld_num
					     && is_array(
						     $va_exclude_fields_from_search = caGetOption( 'excludeFieldsFromSearch', $pa_options,
							     null ) )
					     && sizeof( $va_exclude_fields_from_search )
					) {
						$va_field_restrict_sql = [];
						foreach ( $va_exclude_fields_from_search as $va_restrict ) {
							$va_field_restrict_sql[] = "'" . (int) $va_restrict['table_num'] . "/"
							                           . $va_restrict['field_num'] . "'";
						}
						$vs_sql_where .= " AND (CONCAT(swi.field_table_num, '/', swi.field_num) NOT IN (" . join( ",",
								$va_field_restrict_sql ) . "))";
					}

					$va_join = array();
					if ( ( $vn_direct_sql_target_table_num != $pn_subject_tablenum )
					     && ! $vb_dont_rewrite_direct_sql_query
					) {
						...
						if ( $t_target = Datamodel::getInstanceByTableNum( $vn_direct_sql_target_table_num, true ) ) {
							...
							$va_path = array_keys( Datamodel::getPath( $vn_direct_sql_target_table_num,
								$pn_subject_tablenum ) );

							$vs_left_table = array_shift( $va_path );
							$va_join[]     = "INNER JOIN {$vs_left_table} ON {$vs_left_table}."
							                 . Datamodel::primaryKey( $vs_left_table ) . " = ca.row_id";

							$vn_cj = 0;
							foreach ( $va_path as $vs_right_table ) {
								if ( sizeof( $va_rels = Datamodel::getRelationships( $vs_left_table, $vs_right_table ) )
								     > 0
								) {
									$va_join[] = "INNER JOIN {$vs_right_table} ON {$vs_right_table}."
									             . $va_rels[ $vs_left_table ][ $vs_right_table ][0][1] . " = "
									             . ( "{$vs_left_table}."
									                 . $va_rels[ $vs_left_table ][ $vs_right_table ][0][0] );
								}
							...
							$vs_direct_sql_query = str_replace( "SELECT ca.row_id",
								"SELECT " . Datamodel::primaryKey( $pn_subject_tablenum, true ), $vs_direct_sql_query );
							// Finally we pray
						}
					}
					...
					if ( $vn_i == 0 ) {
						if ( $vs_direct_sql_query ) {
							...
							if ( ( strpos( $vs_sql, '?' ) !== false )
							     && ( ! is_array( $pa_direct_sql_query_params )
							          || sizeof( $pa_direct_sql_query_params ) == 0 )
							) {
								$pa_direct_sql_query_params = array(
									( $vn_direct_sql_target_table_num != $pn_subject_tablenum )
										? $vn_direct_sql_target_table_num : (int) $pn_subject_tablenum
								);
							}
						} else {
							$vs_sql                     = "
						...
						if ( ( ( $vn_num_terms = ( sizeof( $va_ft_terms ) + sizeof( $va_ft_like_terms )
						                           + sizeof( $va_ft_stem_terms ) ) ) > 1 )
						     && ( ! $vs_direct_sql_query )
						) {
						...
						$t                          = new Timer();
						$pa_direct_sql_query_params = is_array( $pa_direct_sql_query_params )
							? $pa_direct_sql_query_params : array();
						if ( strpos( $vs_sql, '?' ) === false ) {
							$pa_direct_sql_query_params = array();
						}
						...
						switch ( $vs_op ) {
							case 'AND':
								if ( $vs_direct_sql_query ) {
									if ( $vn_direct_sql_target_table_num != $pn_subject_tablenum ) {
										array_push( $va_join, "INNER JOIN {$ps_dest_table} AS ftmp1 ON ftmp1.row_id = "
										                      . Datamodel::primaryKey( $pn_subject_tablenum, true ) );
									} elseif ( $vn_intrinsic_type ) {
										array_unshift( $va_join,
											"INNER JOIN {$ps_dest_table} AS ftmp1 ON ftmp1.row_id = "
											. Datamodel::primaryKey( $pn_subject_tablenum, true ) );
									} else {
										array_unshift( $va_join,
											"INNER JOIN {$ps_dest_table} AS ftmp1 ON ftmp1.row_id = ca.row_id" );
									}
									$vs_direct_sql_query = str_replace( '^JOIN', join( "\n", $va_join ),
										$vs_direct_sql_query );
								...
								if ( ! $vs_direct_sql_query
								     && ( ( $vn_num_terms = ( sizeof( $va_ft_terms ) + sizeof( $va_ft_like_terms )
								                              + sizeof( $va_ft_stem_terms ) ) ) > 1 )
								) {
									$vs_sql .= " HAVING count(distinct sw.stem) >= {$vn_num_terms}";
								}
								...
								if ( is_array( $va_ids = $qr_res->getAllFieldValues( ( $vs_direct_sql_query
								                                                       && ( $vn_direct_sql_target_table_num
								                                                            != $pn_subject_tablenum ) )
										? Datamodel::primaryKey( $pn_subject_tablenum ) : 'row_id' ) )
								     && sizeof( $va_ids )
								) {
									$va_field_container_ids
										= array_filter( $qr_res->getAllFieldValues( ( $vs_direct_sql_query )
										? 'attribute_id' : 'field_container_id' ), "strlen" );
									...
									$vs_sql
										    = "DELETE FROM {$ps_dest_table} WHERE row_id NOT IN (?) {$vs_container_sql}";
							...
							case 'NOT':
								...
								$qr_res = $this->opo_db->query( $vs_sql, $pa_direct_sql_query_params );
								$va_ids = $qr_res->getAllFieldValues( ( $vs_direct_sql_query
								                                        && ( $vn_direct_sql_target_table_num
								                                             != $pn_subject_tablenum ) )
									? Datamodel::primaryKey( $pn_subject_tablenum ) : 'row_id' );
								...
								if ( sizeof( $va_ids ) > 0 ) {
									...
									if ( sizeof( $va_field_container_ids ) > 0 ) {
										$vs_container_sql
											         = " AND (field_container_id IN (?) OR field_container_id IS NULL) ";
										$va_params[] = $va_field_container_ids;
									}
							...
							case 'OR':
								...
								$vs_sql = ( $vs_direct_sql_query )
									? "INSERT IGNORE INTO {$ps_dest_table} {$vs_direct_sql_query}"
									: "
									INSERT IGNORE INTO {$ps_dest_table}
									SELECT swi.row_id, SUM(swi.boost),swi.field_container_id
									FROM ca_sql_search_word_index swi
	...
	public function indexField(
		$pn_content_tablenum, $ps_content_fieldname, $pn_content_row_id, $pm_content, $pa_options
	) {
		...
		$vb_force_tokenize = ( in_array( 'TOKENIZE', array_values( $pa_options ), true )
		                       || isset( $pa_options['TOKENIZE'] ) );
		$vb_tokenize       = $pa_options['DONT_TOKENIZE'] ? false : true;

		$vn_rel_type_id  = ( isset( $pa_options['relationship_type_id'] )
		                     && ( $pa_options['relationship_type_id'] > 0 ) )
			? (int) $pa_options['relationship_type_id'] : 0;
		$vn_container_id = ( isset( $pa_options['container_id'] ) && ( $pa_options['container_id'] > 0 ) )
			? (int) $pa_options['container_id'] : 'NULL';
		...
		if ( ( ! is_array( $pm_content ) && ! strlen( $pm_content ) ) || ! sizeof( $pm_content )
		...
		if ( ! defined( "__CollectiveAccess_IS_REINDEXING__" ) && $vb_incremental_reindexing ) {
			$this->removeRowIndexing( $this->opn_indexing_subject_tablenum, $this->opn_indexing_subject_row_id,
				$pn_content_tablenum, array( $ps_content_fieldname ), $pn_content_row_id, $vn_rel_type_id );
		}
		if ( ! $va_words ) {
			self::$s_doc_content_buffer[] = '(' . $this->opn_indexing_subject_tablenum . ','
			                                . $this->opn_indexing_subject_row_id . ',' . $pn_content_tablenum . ',\''
			                                . $ps_content_fieldname . '\',' . $vn_container_id . ','
			                                . $pn_content_row_id . ',0,0,' . $vn_private . ',' . $vn_rel_type_id . ')';
		} else {
			foreach ( $va_words as $vs_word ) {
				if ( ! strlen( $vs_word ) ) {
					continue;
				...
				self::$s_doc_content_buffer[] = '(' . $this->opn_indexing_subject_tablenum . ','
				                                . $this->opn_indexing_subject_row_id . ',' . $pn_content_tablenum
				                                . ',\'' . $ps_content_fieldname . '\',' . $vn_container_id . ','
				                                . $pn_content_row_id . ',' . $vn_word_id . ',' . $vn_boost . ','
				                                . $vn_private . ',' . $vn_rel_type_id . ')';
			}
		}
	}
		...
		if ( is_array( self::$s_doc_content_buffer ) && sizeof( self::$s_doc_content_buffer ) ) {
			while ( sizeof( self::$s_doc_content_buffer ) > 0 ) {
				...
				$this->opo_db->query( $this->ops_insert_word_index_sql . "\n" . join( ",",
						array_splice( self::$s_doc_content_buffer, 0, $vn_max_word_segment_size ) ) );
				if ( defined( "__CollectiveAccess_IS_REINDEXING__" ) ) {
	...
	public function getWordID( $ps_word ) {
		...
		if ( $qr_res = $this->opqr_lookup_word->execute( $ps_word ) ) {
			if ( $qr_res->nextRow() ) {
				return WLPlugSearchEngineSqlSearch::$s_word_cache[ $ps_word ] = (int) $qr_res->get( 'word_id',
					array( 'binary' => true ) );
			}
		}
	...
	private function _checkWordCacheSize() {
		if ( ( sizeof( WLPlugSearchEngineSqlSearch::$s_word_cache ) ) > ( $vn_max_size
				= $this->getOption( 'maxWordCacheSize' ) )
		) {
			WLPlugSearchEngineSqlSearch::$s_word_cache = array_slice( WLPlugSearchEngineSqlSearch::$s_word_cache, 0,
				$vn_max_size * $this->getOption( 'cacheCleanFactor' ), true );
		}
	}

	# ------------------------------------------------
	public function removeRowIndexing(
		$pn_subject_tablenum, $pn_subject_row_id, $pn_field_tablenum = null, $pa_field_nums = null,
		$pn_field_row_id = null, $pn_rel_type_id = null
	) {
		...
		if ( $pn_subject_tablenum && $pn_subject_row_id && ! is_null( $pn_field_tablenum )
		     && ! is_null( $pn_field_row_id )
		     && is_array( $pa_field_nums )
		     && sizeof( $pa_field_nums )
		) {
			foreach ( $pa_field_nums as $pn_field_num ) {
				...
				$this->opqr_delete_with_field_row_id_and_num->execute( (int) $pn_subject_tablenum,
					(int) $pn_subject_row_id, (int) $pn_field_tablenum, (string) $pn_field_num, (int) $pn_field_row_id,
					$vn_rel_type_id );
			}
			...
			if ( $pn_subject_tablenum && $pn_subject_row_id && ! is_null( $pn_field_tablenum )
				...
				return $this->opqr_delete_with_field_row_id->execute( (int) $pn_subject_tablenum,
					(int) $pn_subject_row_id, (int) $pn_field_tablenum, (int) $pn_field_row_id, $vn_rel_type_id );
			} else {
				if ( ! is_null( $pn_field_tablenum ) && is_array( $pa_field_nums ) && sizeof( $pa_field_nums ) ) {
					foreach ( $pa_field_nums as $pn_field_num ) {
						...
						if ( ! is_null( $pn_rel_type_id ) ) {
							$this->opqr_delete_with_field_num->execute( (int) $pn_subject_tablenum,
								(int) $pn_subject_row_id, (int) $pn_field_tablenum, (string) $pn_field_num,
								$vn_rel_type_id );
						} else {
							$this->opqr_delete_with_field_num_without_rel_type_id->execute( (int) $pn_subject_tablenum,
								(int) $pn_subject_row_id, (int) $pn_field_tablenum, (string) $pn_field_num );
						}
					}
	...
	public function updateIndexingInPlace(
		...
		foreach ( $pa_subject_row_ids as $vn_subject_row_id ) {
			$this->removeRowIndexing( $pn_subject_tablenum, $vn_subject_row_id, $pn_content_tablenum,
				array( $ps_content_fieldnum ), $pn_content_row_id,
				caGetOption( 'relationship_type_id', $pa_options, null ) );
		}
		...
		if ( caGetOption( "INDEX_AS_IDNO", $pa_options, false ) || in_array( 'INDEX_AS_IDNO', $pa_options, true ) ) {
			...
			if ( ! sizeof( $va_values ) && method_exists( $t_content, "getIDNoPlugInInstance" )
			     && ( $o_idno
					= $t_content->getIDNoPlugInInstance() )
			) {
		...
		foreach ( $pa_subject_row_ids as $vn_row_id ) {
			...
			foreach ( $va_words as $vs_word ) {
				...
				$va_row_insert_sql[]
					= "({$pn_subject_tablenum}, {$vn_row_id}, {$pn_content_tablenum}, '{$ps_content_fieldnum}', "
					  . ( $pn_content_container_id ? $pn_content_container_id : 'NULL' )
					  . ", {$pn_content_row_id}, {$vn_word_id}, {$vn_boost}, {$vn_private}, {$vn_rel_type_id})";
				$vn_seq ++;
			}
	...
	private function _tokenize( $ps_content, $pb_for_search = false, $pn_index = 0 ) {
		$ps_content = preg_replace( '![\']+!', '',
			$ps_content );        // strip apostrophes for compatibility with SearchEngine class, which does the same to all search expressions
		...
		if ( $pb_for_search ) {
			...
			return preg_split( '![ ]+!',
				trim( preg_replace( "%((?<!\d)[" . $this->ops_search_tokenizer_regex . "]+(?!\d))%u", ' ',
					strip_tags( $ps_content ) ) ) );
		} else {
			return preg_split( '![ ]+!', trim( preg_replace( "%((?<!\d)[" . $this->ops_search_tokenizer_regex . "]+|["
			                                                 . $this->ops_search_tokenizer_regex . "]+(?!\d))%u", ' ',
				strip_tags( $ps_content ) ) ) );
		}
	}
	# --------------------------------------------------
	...
	private function getFieldNum( $pn_table_name_or_num, $ps_fieldname ) {
		...
		$vs_table_name = is_numeric( $pn_table_name_or_num ) ? Datamodel::getTableName( (int) $pn_table_name_or_num )
			: (string) $pn_table_name_or_num;

		return WLPlugSearchEngineSqlSearch::$s_fieldnum_cache[ $pn_table_name_or_num . '/' . $ps_fieldname ]
			= Datamodel::getFieldNum( $vs_table_name, $ps_fieldname );
	}
