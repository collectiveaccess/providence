...
class WLPlugSearchEngineSqlSearch extends BaseSearchPlugin implements IWLPlugSearchEngine
{
                            ...
                            if (is_array($va_tmp) && (sizeof($va_tmp) > 1) && is_array(
                                ...
                                if ($t_instance = Datamodel::getInstanceByTableNum($pn_subject_tablenum, true)) {
                                    $vs_direct_sql_query = "
                                        ...
                                        WHERE
                                            (" . $va_lower_term->field . " BETWEEN " . floatval(
                                            $vn_lower_val
                    ...
                    if ($vs_access_point) {
                        ...
                        if (in_array($vs_table, array('created', 'modified'))) {
                            ...
                            if ((!$vb_is_blank_search && !$vb_is_not_blank_search) && $vs_table && $vs_field && ($t_table = Datamodel::getInstanceByTableName(
                                ...
                                if (($vs_intrinsic_field_name = $t_table->fieldName(
                                        $vn_fld_num
                                    )) && (($vn_intrinsic_type = $t_table->getFieldInfo(
                                            $vs_intrinsic_field_name,
                                            'FIELD_TYPE'
                                        )) == FT_BIT)) {
        ...
        if (is_array($this->opa_doc_content_buffer) && sizeof($this->opa_doc_content_buffer)) {
            while (sizeof($this->opa_doc_content_buffer) > 0) {
                ...
                $this->opo_db->query(
                    $this->ops_insert_word_index_sql . "\n" . join(
                        ",",
                        array_splice($this->opa_doc_content_buffer, 0, $vn_max_word_segment_size)
                    )
                );
                if (defined("__CollectiveAccess_IS_REINDEXING__")) {
    ...
    public function suggest($ps_text, $pa_options = null)
    {
        ...
        if (caGetOption('returnAsLink', $pa_options, false) && ($po_request = caGetOption(
                'request',
                $pa_options,
                null
            ))) {
            foreach ($va_phrases as $vn_i => $vs_phrase) {
                $va_phrases[$vn_i] = caNavLink(
                    $po_request,
                    $vs_phrase,
                    '',
                    '*',
