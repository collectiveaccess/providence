...
class Query {
    ...
    foreach ($this->getRewrittenQuery()->getSubqueries() as $o_subquery) {
      switch (get_class($o_subquery)) {
        ...
        case 'Zend_Search_Lucene_Search_Query_Boolean':
          ...
          break;
        default:
          throw new Exception('Encountered unknown Zend query type in Elastic8\Query: ' . get_class($o_subquery) . '. Query was: ' . $vs_search_expression);
          break;
      }
    }
  ...
  public function rewriteSubquery($o_subquery) {
    switch (get_class($o_subquery)) {
      ...
      default:
        throw new Exception('Encountered unknown Zend subquery type in Elastic8\Query: ' . get_class($o_subquery));
        break;
    }
  }
