...
class Mapping {
	...
	public function getFieldsToIndex( $ps_table ) {
		...
		$va_table_fields = $this->getSearchBase()->getFieldsToIndex( $ps_table, null,
	...
	public function getConfigForElement( $ps_table, $pn_element_id, $pa_element_info, $pa_indexing_config ) {
		...
		switch ( $pa_element_info['datatype'] ) {
			case 2:    // daterange
				...
				$va_element_config[ $ps_table . '/' . $vs_element_code ]['format']
				                                                                                        = 'date_time_no_millis';
				$va_element_config[ $ps_table . '/' . $vs_element_code ]['ignore_malformed']            = true;
				$va_element_config[ $ps_table . '/' . $vs_element_code . '_text' ]['type']
				                                                                                        = ( $this->version
				                                                                                            == 2 )
					? 'string' : 'text';
			...
			case 4:    // geocode
				...
				$va_element_config[ $ps_table . '/' . $vs_element_code . '_text' ] = array(
					'type' => ( $this->version == 2 ) ? 'string' : 'text'
				);
				break;
			case 6: // currency
				...
				$va_element_config[ $ps_table . '/' . $vs_element_code . '_currency' ]
				                                                                 = array(
					'type' => ( $this->version == 2 ) ? 'string' : 'text'
				);
				break;
			...
			case 30: // object lots
			default:
				$va_element_config[ $ps_table . '/' . $vs_element_code ]['type'] = ( $this->version == 2 ) ? 'string'
					: 'text';
				break;
		}
			...
			case ( FT_HISTORIC_DATERANGE ):
				switch ( $this->version ) {
					...
					case 7:
						...
						$va_field_options[ $ps_table . '/' . $vs_field_name ]['format']
						                                                                          = 'date_time_no_millis';
		...
		foreach ( $this->getTables() as $vs_table ) {
			...
			foreach ( $this->getFieldsToIndex( $vs_table ) as $vs_field => $va_indexing_info ) {
				if ( preg_match( "/^(ca[\_a-z]+)\.A([0-9]+)$/", $vs_field, $va_matches ) ) { // attribute
					$va_mapping_config[ $vs_table ]['properties'] = array_merge(
