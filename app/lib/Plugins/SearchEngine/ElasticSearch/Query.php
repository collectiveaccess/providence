...
class Query
{
    ...
    public function __construct(
        $opn_subject_table_num,
        $ops_search_expression,
        Zend_Search_Lucene_Search_Query_Boolean $opo_rewritten_query,
        array $opa_filters
    ) {
        ...
        foreach ($this->getRewrittenQuery()->getSubqueries() as $o_subquery) {
            switch (get_class($o_subquery)) {
                ...
                case 'Zend_Search_Lucene_Search_Query_Boolean':
                    ...
                    $o_new_subquery = new Zend_Search_Lucene_Search_Query_Boolean(
                        $va_new_subqueries,
                        $o_subquery->getSigns()
                    );
                    $va_new_search_expression_parts[] = preg_replace('/^\+/u', '', (string)$o_new_subquery);
                ...
                default:
                    throw new Exception(
                        'Encountered unknown Zend query type in ElasticSearch\Query: ' . get_class(
                            $o_subquery
                        ) . '. Query was: ' . $vs_search_expression
                    );
                    break;
            }
        }
    ...
    public function rewriteSubquery($o_subquery)
    {
        switch (get_class($o_subquery)) {
            case 'Zend_Search_Lucene_Search_Query_Range':
                ...
                if ($o_lower_fld instanceof FieldTypes\Geocode) {
                    $this->opa_additional_filters[] = [
                        'geo_shape' => $o_lower_fld->getFilterForRangeQuery(
                            $o_lower_term,
                            $o_upper_term
                        )
                    ];
            ...
            case 'Zend_Search_Lucene_Search_Query_Phrase':
                ...
                $vb_multiterm_all_terms_same_field = (sizeof(array_unique($va_fields_in_subquery)) < 2) && (sizeof(
                            $vo_terms
                        ) > 1);
            ...
            case 'Zend_Search_Lucene_Search_Query_MultiTerm':
                ...
                $o_new_subquery = new Zend_Search_Lucene_Search_Query_MultiTerm(
                    $va_new_terms, $o_subquery->getSigns()
                );
                return $this->getSubqueryWithAdditionalTerms($o_new_subquery, $o_fld, $o_term);
            case 'Zend_Search_Lucene_Search_Query_Boolean':
                /** @var $o_subquery Zend_Search_Lucene_Search_Query_Boolean */
            ...
            default:
                throw new Exception(
                    'Encountered unknown Zend subquery type in ElasticSearch\Query: ' . get_class($o_subquery)
                );
                break;
        }
    }
