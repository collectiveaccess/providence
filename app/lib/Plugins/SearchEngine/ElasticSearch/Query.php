...
class Query {
	...
	public function __construct(
		$opn_subject_table_num, $ops_search_expression, \Zend_Search_Lucene_Search_Query_Boolean $opo_rewritten_query,
		array $opa_filters
	) {
		...
		foreach ( $this->getRewrittenQuery()->getSubqueries() as $o_subquery ) {
			switch ( get_class( $o_subquery ) ) {
				...
				case 'Zend_Search_Lucene_Search_Query_Boolean':
					...
					$o_new_subquery                   = new \Zend_Search_Lucene_Search_Query_Boolean( $va_new_subqueries,
						$o_subquery->getSigns() );
					$va_new_search_expression_parts[] = preg_replace( '/^\+/u', '', (string) $o_new_subquery );
					break;
				default:
					throw new \Exception( 'Encountered unknown Zend query type in ElasticSearch\Query: '
					                      . get_class( $o_subquery ) . '. Query was: ' . $vs_search_expression );
					break;
			}
		}
		...
		switch ( get_class( $o_subquery ) ) {
			case 'Zend_Search_Lucene_Search_Query_Range':
				...
				if ( $o_lower_fld instanceof FieldTypes\Geocode ) {
					$this->opa_additional_filters[]
						= [ 'geo_shape' => $o_lower_fld->getFilterForRangeQuery( $o_lower_term, $o_upper_term ) ];
				} else {
			...
			case 'Zend_Search_Lucene_Search_Query_Term':
				...
				if ( ( $o_fld instanceof FieldTypes\DateRange ) || ( $o_fld instanceof FieldTypes\Timestamp ) ) {
					...
					if ( $o_rewritten_term = $o_fld->getRewrittenTerm( $o_term ) ) {
						$o_new_subquery
							= new \Zend_Search_Lucene_Search_Query_Term( $o_fld->getRewrittenTerm( $o_term ) );
					}
				}
			...
			case 'Zend_Search_Lucene_Search_Query_Phrase':
				...
				$vb_multiterm_all_terms_same_field = ( sizeof( array_unique( $va_fields_in_subquery ) ) < 2 )
				                                     && ( sizeof( $vo_terms ) > 1 );
				...
				if ( $vb_multiterm_all_terms_same_field && ( $o_first_term = array_shift( $vo_terms ) ) ) {
					...
					if ( ( $o_fld instanceof FieldTypes\Length ) || ( $o_fld instanceof FieldTypes\Weight )
					     || ( $o_fld instanceof FieldTypes\Currency )
					) {
						$vs_acc = '';
				...
				foreach ( $vo_terms as $o_term ) {
					...
					if ( $o_fld instanceof FieldTypes\Geocode ) {
						$o_new_subquery = null;
						$this->opa_additional_filters[]
						                = [ 'geo_shape' => $o_fld->getFilterForPhraseQuery( $o_subquery ) ];
						break;
					} elseif ( ( $o_fld instanceof FieldTypes\DateRange )
					           || ( $o_fld instanceof FieldTypes\Timestamp )
					) {
						$o_new_subquery = null;
			...
			case 'Zend_Search_Lucene_Search_Query_MultiTerm':
				...
				$o_new_subquery = new \Zend_Search_Lucene_Search_Query_MultiTerm( $va_new_terms,
					$o_subquery->getSigns() );
			...
			default:
				throw new \Exception( 'Encountered unknown Zend subquery type in ElasticSearch\Query: '
				                      . get_class( $o_subquery ) );
				break;
		}
	}
