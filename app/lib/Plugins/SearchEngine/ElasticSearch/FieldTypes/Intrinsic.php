...
class Intrinsic extends FieldType {
	...
	public function getIndexingFragment( $pm_content, $pa_options ) {
		...
		if ( $t_instance->getProperty( 'ID_NUMBERING_ID_FIELD' ) == $this->getFieldName()
		     || ( is_array( $pa_options )
		          && in_array( 'INDEX_AS_IDNO', $pa_options, true ) )
		) {
			if ( method_exists( $t_instance, "getIDNoPlugInInstance" )
			     && ( $o_idno
					= $t_instance->getIDNoPlugInInstance() )
			) {
		...
		if ( $vn_rel_type_id = caGetOption( 'relationship_type_id', $pa_options ) ) {
			...
			$va_return[ $this->getTableName() . '/' . $this->getFieldName() . '|'
			            . caGetRelationshipTypeCode( $vn_rel_type_id ) ]
				= $pm_content;
		}
	...
	public function getRewrittenTerm( $po_term ) {
		...
		if ( ( strtolower( $vs_raw_term ) === '[blank]' ) ) {
			...
			( $t_instance instanceof \BaseModel ) && isset( $va_field_components[1] )
