...
class SqlSearchConfigurationSettings extends ASearchConfigurationSettings
{
    ...
    public function setSettings()
    {
        $this->opa_possible_errors = array(
            __CA_SQLSEARCH_RUNNING_MYSQL__,
            __CA_SQLSEARCH_TABLES_EXIST__
        );
    }
    ...
    public function getSettingDescription($pn_setting_num)
    {
        switch ($pn_setting_num) {
            ...
            case __CA_SQLSEARCH_TABLES_EXIST__:
                return _t(
                    "The SqlSearch search engine requires that certain tables be present in your database. They are installed by default and should be present, but if they are not SqlSearch will not be able to operate."
                );
            default:
                return null;
        }
    ...
    public function getSettingHint($pn_setting_num)
    {
        switch ($pn_setting_num) {
            case __CA_SQLSEARCH_RUNNING_MYSQL__:
                return _t(
                    "Try reinstalling your system with MySQL as the back-end database or try a different search engine."
                );
            case __CA_SQLSEARCH_TABLES_EXIST__:
                return _t(
                    "Try reloading the definitions for these tables: ca_sql_search_words, ca_sql_search_word_index, ca_sql_search_ngrams"
                );
            default:
                return null;
        }
