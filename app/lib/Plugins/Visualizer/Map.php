...
class WLPlugVisualizerMap Extends BaseVisualizerPlugIn Implements IWLPlugVisualizer {
	 ...
	 *                                width =
	 *                                height =
	 *                                mapType - type of map to render; valid values are 'ROADMAP', 'SATELLITE',
	 *                                'HYBRID', 'TERRAIN'; if not specified 'google_maps_default_type' setting in
	 *                                app.conf is used; if that is not set default is 'SATELLITE'
	 *                                showNavigationControls - if true, navigation controls are displayed; default is
	 *                                to use 'google_maps_show_navigation_controls' setting in app.conf
	 *                                showScaleControls -  if true, scale controls are displayed; default is to use
	 *                                'google_maps_show_scale_controls' setting in app.conf showMapTypeControls -  if
	 *                                true, map type controls are displayed; default is to use
	 *                                'google_maps_show_map_type_controls' setting in app.conf minZoomLevel - Minimum
	 *                                zoom level to allow; leave null if you don't want to enforce a limit maxZoomLevel
	 *                                - Maximum zoom level to allow; leave null if you don't want to enforce a limit zoomLevel - Zoom map to specified level rather than fitting all markers into view; leave null if you don't want to specify a zoom level. IF this option is set minZoomLevel and maxZoomLevel will be ignored. pathColor - pathWeight - pathOpacity - request = current request; required for generation of editor links
	 */
	public function render( $pa_viz_settings, $ps_format = 'HTML', $pa_options = null ) {
		...
		list( $vs_width, $vs_height ) = $this->_parseDimensions( caGetOption( 'width', $pa_options, 500 ),
			caGetOption( 'height', $pa_options, 500 ) );
		...
		foreach ( $pa_viz_settings['sources'] as $vs_source_code => $va_source_info ) {
			...
			if ( isset( $va_source_info['display']['ajax_content_url'] )
			     && ( $va_source_info['display']['ajax_content_url'] )
			) {
