...
";
			...
			foreach ( $pa_viz_settings['sources'] as $vs_source_name => $va_source ) {
				...
				$va_data['events'][] = [
					...
					'media'      => [
						...
						'thumbnail' => $qr_res->getWithTemplate( $va_source['display']['icon'],
							array( 'returnURL' => true ) ),
	...
	private function getLocaleJSSrc( $po_request ) {
		...
		if ( isset( $va_matches[1] )
		     && file_exists( __CA_BASE_DIR__ . '/assets/timelinejs/js/locale/' . $va_matches[1] . '.js' )
		) {
			return "<script src='" . $po_request->getBaseUrlPath() . "/assets/timelinejs/js/locale/" . $va_matches[1]
