...
class WLPlugVisualizerCalendar Extends BaseVisualizerPlugIn Implements IWLPlugVisualizer {
		    ...
		    <script type='text/javascript'>
				...
				jQuery(document).ready(function() {
					jQuery('#calendar-embed').fullCalendar({
						...
						editable: true,
						lang: 'fr',
						events: '" . caNavUrl( $po_request, '*', '*', '*',
				array( 'renderData' => '1', 'viz' => $pa_viz_settings['code'] ) ) . "',
	...
	public function getDataForVisualization( $pa_viz_settings, $pa_options = null ) {
		...
		$va_default_colors_lighter = array(
			"#c7999e",
			"#fc6691",
			"#cb42d4",
			"#2670de",
		...
		$va_default_colors_darker  = array(
			"#87595e",
			"#bc2651",
			"#8b0294",
			"#06309e",
		...
		while ( $qr_res->nextHit() ) {
			foreach ( $pa_viz_settings['sources'] as $vs_source_name => $va_source ) {
				$vn_event_num    = 1;
				$vs_dates        = $qr_res->get( $va_source['data'],
					array( 'sortable' => true, 'returnAsArray' => false, 'delimiter' => ';' ) );
				...
				if ( $vs_dates !== "" ) {
					...
					if ( ( $va_calendar_dates["start"]["hours"] === "00" )
					     && ( $va_calendar_dates["end"]["hours"] === "23" )
					     && ( $va_calendar_dates["start"]["minutes"] === "00" )
					     && ( $va_calendar_dates["end"]["minutes"] === "59" )
					) {
						$data["allDay"] = true;
					};
					$va_data[] = $data;
