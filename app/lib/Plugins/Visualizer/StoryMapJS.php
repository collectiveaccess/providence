...
";
    ...
    private function getLocaleJSSrc($po_request)
    {
        ...
        if (isset($va_matches[1]) && file_exists(
                __CA_BASE_DIR__ . '/assets/storymapjs/js/locale/' . $va_matches[1] . '.js'
            )) {
            return "<script src='" . $po_request->getBaseUrlPath(
                ) . "/assets/storymapjs/js/locale/" . $va_matches[1] . ".js' type='text/javascript'></script>";
        }
        return '';
    }
    # ------------------------------------------------
