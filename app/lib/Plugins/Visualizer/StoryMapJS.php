...
class WLPlugVisualizerStoryMapJS Extends BaseVisualizerPlugIn Implements IWLPlugVisualizer {
    ...
    <script type='text/javascript'>
    	var storymap_data = '" . caNavUrl( $po_request, '*', '*', '*',
...
";
	...
	public function getDataForVisualization( $pa_viz_settings, $pa_options = null ) {
		...
		$va_data = [
			'storymap' => [
				'language'     => 'en',
				'map_type'     => isset( $pa_viz_settings['display']['map_type'] )
					? $pa_viz_settings['display']['map_type'] : 'stamen:toner-lite',
				'map_as_image' => false,
				'slides'       => [
					[
						'type'  => 'overview',
						'text'  => [
							'headline' => isset( $pa_viz_settings['display']['headline'] )
								? $pa_viz_settings['display']['headline'] : '',
							'text'     => isset( $pa_viz_settings['display']['introduction'] )
								? $pa_viz_settings['display']['introduction'] : '',
						],
						'media' => [
							'url'     => '',
							'credit'  => '',
		...
		while ( $qr_res->nextHit() ) {
			foreach ( $pa_viz_settings['sources'] as $vs_source_name => $va_source ) {
				$va_locations = $qr_res->get( $va_source['data'],
					array( 'coordinates' => true, 'returnAsArray' => true ) );
				...
				$va_data['storymap']['slides'][] = [
					'text'     => [
						'headline' => $po_request ? caEditorLink( $po_request, $vs_title, '', $vs_table_name,
							$vn_row_id ) : $vs_title,
						'text'     => $qr_res->getWithTemplate( $va_source['display']['description_template'] ),
					],
					'media'    => [
						'url'     => $qr_res->getWithTemplate( $va_source['display']['image'],
							array( 'returnURL' => true ) ),
	...
	private function getLocaleJSSrc( $po_request ) {
		...
		if ( isset( $va_matches[1] )
		     && file_exists( __CA_BASE_DIR__ . '/assets/storymapjs/js/locale/' . $va_matches[1] . '.js' )
		) {
			return "<script src='" . $po_request->getBaseUrlPath() . "/assets/storymapjs/js/locale/" . $va_matches[1]
