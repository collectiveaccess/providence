...
abstract class BaseExternalExportFormatPlugin Extends WLPlug
{
    ...
    public function _processFiles($t_instance, $content_spec, $options = null)
    {
        ...
        foreach ($instance_list as $t) {
            if (is_array($restrict_to_types) && sizeof($restrict_to_types) && !in_array(
                    $t->getTypeCode(),
                    $restrict_to_types
                )) {
                continue;
            ...
            foreach ($content_spec['files'] as $get_spec => $export_filename_spec) {
                ...
                $filenames = $t->get(
                    "{$pathless_spec}.filename",
                    ['returnAsArray' => true, 'filterNonPrimaryRepresentations' => false]
                );
                $mimetypes = $t->get(
                    "{$pathless_spec}.mimetype",
                    ['returnAsArray' => true, 'filterNonPrimaryRepresentations' => false]
                );
                $file_mod_times = $t->get(
                    "{$pathless_spec}.fileModificationTime",
                    ['returnAsArray' => true, 'filterNonPrimaryRepresentations' => false]
                );
                ...
                foreach ($files as $i => $f) {
                    ...
                    $m = $mimetypes[$i];
                    $t = $file_mod_times[$i];
                    if (is_array($restrict_to_mimetypes) && sizeof($restrict_to_mimetypes) && !sizeof(
                            array_filter(
                                $restrict_to_mimetypes,
                                function ($v) use ($m) {
                    ...
                    $e = $export_filename = self::processExportFilename(
                        $export_filename_spec,
                        [
                            'extension' => $extension,
                            'original_filename' => $original_basename ? "{$original_basename}.{$extension}" : "media_{$i}.{$extension}",
                    ...
                    if ($file_list_template && !isset($file_list[$export_filename])) {
                        $file_list_template_proc = caProcessTemplate(
                            $file_list_template,
                            [
                                'filename' => $original_basename ? "{$original_basename}.{$extension}" : "{$basename}.{$extension}",
                                'filesize_in_bytes' => $fs,
                        ...
                        $file_list[$export_filename] = $t_instance->getWithTemplate(
                            caProcessTemplate($file_list_template, $d, ['skipTagsWithoutValues' => true])
                        );
                    }
