...
class WLPlugSimpleZip Extends BaseExternalExportFormatPlugin Implements IWLPlugExternalExportFormat {
	 ...
	 *                           Constants are, in descending order of shrillness: ALERT = Alert messages (action must
	 *                           be taken immediately) CRIT = Critical conditions ERR = Error conditions WARN =
	 *                           Warnings NOTICE = Notices (normal but significant conditions) INFO = Informational
	 *                           messages DEBUG = Debugging messages
	 *
	...
	public function process( $t_instance, $target_info, $options = null ) {
		$log = caGetLogger( [ 'logLevel' => caGetOption( 'logLevel', $options, null ) ],
		...
		$name           = preg_replace( "![^A-Za-z0-9\-\.\_]+!", "_",
			$t_instance->getWithTemplate( caGetOption( 'name', $output_config, null ) ) );
			...
			switch ( $content_spec['type'] ) {
				case 'export':
					if ( ca_data_exporters::exporterExists( $content_spec['exporter'] ) ) {
						$data = ca_data_exporters::exportRecord( $content_spec['exporter'],
							$t_instance->getPrimaryKey(), [] );
						$zip->addFile( $data, $path );
						$log->logDebug( _t( '[ExternalExport::Output::SimpleZip] Added %1 bytes of data exporter %2 output to ZIP archive using path %3',
							strlen( $data ), $content_spec['exporter'], $path ) );
					} else {
						$log->logError( _t( '[ExternalExport::Output::SimpleZip] Could not generate data export using exporter %1 for external export %2: exporter does not exist',
							$content_spec['exporter'], $target_info['target'] ) );
					}
					break;
				case 'file':
					...
					foreach ( $file_list as $file_info ) {
						...
						$log->logDebug( _t( '[ExternalExport::Output::SimpleZip] Added added file %1 to ZIP archive using path %2',
							$file_info['path'], $p ) );
					}
					break;
				default:
		...
		if ( copy( $zip->output( ZIPFILE_FILEPATH ),
			$f = __CA_APP_DIR__ . "/tmp/{$name}" . ( ! is_null( $media_index ) ? '-' . ( $media_index + 1 ) : '' )
			     . ".zip" )
		) {
