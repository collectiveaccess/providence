...
class WLPlugSimpleZip Extends BaseExternalExportFormatPlugin Implements IWLPlugExternalExportFormat
{
    ...
    public function process($t_instance, $target_info, $options = null)
    {
            ...
            switch ($content_spec['type']) {
                case 'export':
                    if (ca_data_exporters::exporterExists($content_spec['exporter'])) {
                        $data = ca_data_exporters::exportRecord(
                            $content_spec['exporter'],
                        ...
                        $log->logDebug(
                            _t(
                                '[ExternalExport::Output::SimpleZip] Added %1 bytes of data exporter %2 output to ZIP archive using path %3',
                                strlen($data),
                                $content_spec['exporter'],
                                $path
                            )
                        ...
                        $log->logError(
                            _t(
                                '[ExternalExport::Output::SimpleZip] Could not generate data export using exporter %1 for external export %2: exporter does not exist',
                                $content_spec['exporter'],
                                $target_info['target']
                            )
                        );
                ...
                case 'file':
                    ...
                    foreach ($file_list as $file_info) {
                        ...
                        $log->logDebug(
                            _t(
                                '[ExternalExport::Output::SimpleZip] Added added file %1 to ZIP archive using path %2',
                                $file_info['path'],
                                $p
                            )
                        );
        ...
        if (copy(
            $zip->output(ZIPFILE_FILEPATH),
            $f = __CA_APP_DIR__ . "/tmp/{$name}" . (!is_null(
                    $media_index
                ) ? '-' . ($media_index + 1) : '') . ".zip"
        )) {
