...
class WLPlugBagIt Extends BaseExternalExportFormatPlugin Implements IWLPlugExternalExportFormat {
	...
	public function process( $t_instance, $target_info, $options = null ) {
			...
			switch ( $content_spec['type'] ) {
				case 'export':
					if ( ca_data_exporters::exporterExists( $content_spec['exporter'] ) ) {
						...
						$log->logError( _t( '[BagIt] Could not generate data export using exporter %1 for external export %2: exporter does not exist',
				...
				case 'file':
					...
					foreach ( $file_list as $file_info ) {
						...
						if ( $file_info['filemodtime'] > 0 ) {    // preserve file modification date/times
							touch( "{$staging_dir}/{$name}/data/{$path}/{$file_info['name']}",
								$file_info['filemodtime'] );
						}
					}
					break;
				default:
		...
		$special_bag_vals = [
			...
			'total_filesize_in_bytes'    => $total_filesize,
			'total_filesize_for_display' => caHumanFilesize( $total_filesize ),
			'file_count'                 => is_array( $file_list ) ? sizeof( $file_list ) : 0,
			'mimetypes'                  => join( ", ", array_keys( $file_mimetypes ) ),
