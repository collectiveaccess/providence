...
class WLPlugsFTP Extends BaseExternalExportTransportPlugin Implements IWLPlugExternalExportTransport {
	 ...
	 *                       Constants are, in descending order of shrillness: ALERT = Alert messages (action must be
	 *                       taken immediately) CRIT = Critical conditions ERR = Error conditions WARN = Warnings
	 *                       NOTICE = Notices (normal but significant conditions) INFO = Informational messages DEBUG =
	 *                       Debugging messages
	 *
	...
	public function process( $destination_info, $files, $options = null ) {
		...
		if ( $sftp->login( $destination_info['user'], $destination_info['password'] ) === false ) {
			$log->logError( _t( '[ExternalExport::Transport::sFTP] Login to %1 failed',
		...
		if ( $sftp->chdir( $destination_info['path'] ) === false ) {
			$err = error_get_last();
			$log->logError( _t( '[ExternalExport::Transport::sFTP] Could not change to directory %1 on %2: %3',
				$destination_info['path'], $destination_info['hostname'], $err['message'] ) );
			throw new WLPlugsFTPException( _t( 'Could not change to directory %1 on %2: %3', $destination_info['path'],
				$destination_info['hostname'], $err['message'] ) );
		}
		$log->logDebug( _t( '[ExternalExport::Transport::sFTP] Changed directory to %1 on %2',
			$destination_info['path'], $destination_info['hostname'] ) );

		$count = 0;
		foreach ( $files as $f ) {
			if ( $sftp->put( pathinfo( $f, PATHINFO_BASENAME ), $f, SFTP::SOURCE_LOCAL_FILE ) !== false ) {
				$log->logDebug( _t( '[ExternalExport::Transport::sFTP] Uploaded file %1 to %2', $f,
					$destination_info['hostname'] ) );
				$count ++;
			} else {
				$err = error_get_last();
				$log->logError( _t( '[ExternalExport::Transport::sFTP] Could not upload file %1 to %2: %3', $f,
					$destination_info['hostname'], $err['message'] ) );
				throw new WLPlugsFTPException( _t( 'Could not upload file %1 to %2: %3', $f,
					$destination_info['hostname'], $err['message'] ) );
			}
		}
