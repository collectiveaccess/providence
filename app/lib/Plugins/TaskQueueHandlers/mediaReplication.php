...
class WLPlugTaskQueueHandlermediaReplication Extends WLPlug Implements IWLPlugTaskQueueHandler
{
    ...
    public function process($pa_parameters)
    {
        ...
        if (!$t_instance->load($pa_parameters['PK_VAL'])) {
            $o_eventlog->log(
                array(
                    ...
                    "SOURCE" => "TaskQueue->mediaReplication->process()",
                    "MESSAGE" => _t(
                        "Row_id %1 is invalid for table %2",
                        $pa_parameters['PK_VAL'],
                        $pa_parameters['TABLE']
                    )
                )
            );
        ...
        $va_media_info['REPLICATION_LOG'][$ps_target][] = array(
            'STATUS' => __CA_MEDIA_REPLICATION_STATE_UPLOADING__,
            'DATETIME' => time()
        );
        ...
        if (!is_array($va_media_desc = $t_instance->_FIELD_VALUES[$ps_field])) {
            ...
            $va_report['errors'][] = _t(
                "Could not record replication status: %1 has no media description data",
                $ps_field
            );
            return false;
        }
        ...
        try {
            if ($vs_key = $o_replicator->replicateMedia(
                $t_instance->getMediaPath($ps_field, $ps_version),
                $pa_target_info,
                $pa_data
            )) {
                $va_media_info['REPLICATION_STATUS'][$ps_target] = __CA_MEDIA_REPLICATION_STATE_PROCESSING__;
                $va_media_info['REPLICATION_LOG'][$ps_target][] = array(
                    'STATUS' => __CA_MEDIA_REPLICATION_STATE_PROCESSING__,
                    'DATETIME' => time()
                );
                ...
                $va_media_info['REPLICATION_LOG'][$ps_target][] = array(
                    'STATUS' => __CA_MEDIA_REPLICATION_STATE_ERROR__,
                    'DATETIME' => time()
                );
                $va_media_info['REPLICATION_KEYS'][$ps_target] = null;
                $t_instance->setMediaInfo($ps_field, $va_media_info);
            ...
            $va_report['errors'][] = _t(
                'Media replication for %1 failed: %2',
                $pa_target_info['name'],
                $e->getMessage()
            );
        }
