...
class WLPlugTaskQueueHandlermediaproc Extends WLPlug Implements IWLPlugTaskQueueHandler {
	 ...
	 *
	 * @param array $pa_rec An raw database record array for the queued task (eg. each key a field in ca_task_queue and
	 *                      the values are raw database data that has not been manipulated or unserialized)
	 *
	 * @return array An array of printable parameters for the task; array keys are parameter codes, values are arrays
	 *               with two keys: 'label' = a printable parameter description'; 'value' is a printable parameter
	 *               setting
	 */
	 ...
	 *
	 * @param array $pa_parameters An unserialized parameters array for the current task (eg. unserialized data from
	 *                             ca_task_queue.parameters)
	 *
	...
	public function process( $pa_parameters ) {
		...
		$va_options
			= $pa_parameters["OPTIONS"];                // Array of processing options; names of options to employ are keys, settings are values
		...
		if ( ! file_exists( $vs_input_file ) ) {
			...
			$va_report['errors'][] = _t( "Record %1.field = file '%2' did not exist; queued file was discarded",
				$vs_table, $vs_input_file );
		...
		if ( $t_instance = Datamodel::getInstanceByTableName( $vs_table, true ) ) {
			if ( $t_instance->hasField( $vs_field ) ) {
				if ( ! $t_instance->load( $vn_id ) ) {
					...
					$o_media->cleanup();
					$va_report['errors'][] = _t( "Record %1.field = %2 did not exist; queued file was discarded",
						$vs_table, $vn_id );
				...
				# bad field name
				$this->error->setError( 551, _t( "Invalid media field '%1' in table '%2'", $vs_field, $vs_table ),
					"mediaproc->process()" );
		...
		if ( ! file_exists( $vs_input_file ) ) {
			$this->error->setError( 505, _t( "Input media file '%1' does not exist", $vs_input_file ),
				"mediaproc->process()" );
			$o_media->cleanup();
		...
		if ( ! is_readable( $vs_input_file ) ) {
			$this->error->setError( 506, _t( "Denied permission to read input media file '%1'", $vs_input_file ),
				"mediaproc->process()" );
			$o_media->cleanup();
		...
		foreach ( $va_versions as $v => $va_version_settings ) {
			...
			if ( ! $o_media->read( $vs_input_file ) ) {
				$this->error->setError( 1600, _t( "Could not process input media file '%1': %2", $vs_input_file,
					join( '; ', $o_media->getErrors() ) ), "mediaproc->process()" );
				$o_media->cleanup();
			...
			if ( sizeof( $va_rules ) == 0 ) {
				...
				if ( ! $vs_ext ) {
					$this->error->setError( 1600,
						_t( "File could not be copied for %1; can't convert mimetype %2 to extension", $vs_field,
							$vs_output_mimetype ), "mediaproc->process()" );
					$o_media->cleanup();
				...
				if ( ( $vs_dirhash = $this->_getDirectoryHash( $va_volume_info["absolutePath"], $vn_id ) ) === false ) {
					$this->error->setError( 1600, _t( "Couldn't create subdirectory for file for %1", $vs_field ),
						"mediaproc->process()" );
					$o_media->cleanup();
				...
				$vs_filepath = $va_volume_info["absolutePath"] . "/" . $vs_dirhash . "/" . $vs_magic . "_" . $vs_table
				               . "_" . $vs_field . "_" . $vn_id . "_" . $v . "." . $vs_ext;

				if ( ! copy( $vs_input_file, $vs_filepath ) ) {
					$this->error->setError( 504, _t( "File could not be copied for %1", $vs_field ),
						"mediaproc->process()" );
					$o_media->cleanup();
				...
				if ( is_array( $va_volume_info["mirrors"] ) && sizeof( $va_volume_info["mirrors"] ) > 0 ) {
					...
					foreach ( $va_volume_info["mirrors"] as $vs_mirror_code => $va_mirror_info ) {
						...
						if ( ! ( $tq->cancelPendingTasksForEntity( $entity_key ) ) ) {
							$this->error->setError( 560, _t( "Could not cancel pending tasks" ),
								"mediaproc->process()" );
							$o_media->cleanup();
						...
						if ( $tq->addTask(
							...
							array(
								...
								"FILES"   => array(
									array(
										...
										"HASH"      => $vs_dirhash,
										"FILENAME"  => $vs_magic . "_" . $vs_table . "_" . $vs_field . "_" . $vn_id
										               . "_" . $v . "." . $vs_ext
									)
								),
							...
							$this->error->setError( 100,
								_t( "Couldn't queue mirror using '%1' for version '%2' (handler '%3')",
									$vs_mirror_method, $v, $vs_queue ), "mediaproc->process()" );
						}
				...
				if ( ! ( $vs_ext = $o_media->mimetype2extension( $vs_output_mimetype ) ) ) {
					$this->error->setError( 1600,
						_t( "File could not be processed for %1; can't convert mimetype %2 to extension", $vs_field,
							$vs_output_mimetype ), "mediaproc->process()" );
					$o_media->cleanup();
			...
			if ( ! $vb_dont_delete_original_media ) {
				...
				if ( ( $vs_old_media_path ) && ( $vs_filepath . "." . $vs_ext != $vs_old_media_path )
				     && ( $vs_input_file != vs_old_media_path )
				) {
		...
		if ( $t_instance->load( $vn_id ) ) {
			...
			if ( $t_instance->numErrors() ) {
				...
				$this->error->setError( 560,
					_t( "Could not update %1.%2: %3", $vs_table, $vs_field, join( ", ", $t_instance->getErrors() ) ),
					"mediaproc->process()" );
				$o_media->cleanup();
			...
			if ( ( (bool) $o_config->get( 'video_preview_generate_frames' )
			       || (bool) $o_config->get( 'document_preview_generate_pages' ) )
			     && method_exists( $t_instance, 'addFile' )
			) {
				...
				if ( is_array( $va_preview_frame_list ) ) {
					foreach ( $va_preview_frame_list as $vn_time => $vs_frame ) {
						$t_instance->addFile( $vs_frame, $vn_time,
			...
			$va_report['errors'][]
				= _t( "Record {$vs_table}.field = {$vn_id} did not exist; queued file was discarded" );
