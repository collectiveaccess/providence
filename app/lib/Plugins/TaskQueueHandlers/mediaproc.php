...
class WLPlugTaskQueueHandlermediaproc Extends WLPlug Implements IWLPlugTaskQueueHandler
{
    ...
    public function process($pa_parameters)
    {
        ...
        if (!file_exists($vs_input_file)) {
            ...
            $va_report['errors'][] = _t(
                "Record %1.field = file '%2' did not exist; queued file was discarded",
                $vs_table,
                $vs_input_file
            );
            return $va_report;
        ...
        if ($t_instance = Datamodel::getInstanceByTableName($vs_table, true)) {
            if ($t_instance->hasField($vs_field)) {
                if (!$t_instance->load($vn_id)) {
                    ...
                    $o_media->cleanup();
                    $va_report['errors'][] = _t(
                        "Record %1.field = %2 did not exist; queued file was discarded",
                        $vs_table,
                        $vn_id
                    );
                    return $va_report;
                ...
                $this->error->setError(
                    551,
                    _t("Invalid media field '%1' in table '%2'", $vs_field, $vs_table),
                    "mediaproc->process()"
                );
                return false;
            }
        ...
        if (!file_exists($vs_input_file)) {
            $this->error->setError(
                505,
                _t("Input media file '%1' does not exist", $vs_input_file),
                "mediaproc->process()"
            );
            $o_media->cleanup();
            return false;
        ...
        if (!is_readable($vs_input_file)) {
            $this->error->setError(
                506,
                _t("Denied permission to read input media file '%1'", $vs_input_file),
                "mediaproc->process()"
            );
            $o_media->cleanup();
            return false;
        ...
        foreach ($va_versions as $v => $va_version_settings) {
            ...
            if (!$o_media->read($vs_input_file)) {
                $this->error->setError(
                    1600,
                    _t(
                        "Could not process input media file '%1': %2",
                        $vs_input_file,
                        join('; ', $o_media->getErrors())
                    ),
                    "mediaproc->process()"
                );
                $o_media->cleanup();
            ...
            if (sizeof($va_rules) == 0) {
                ...
                if (($vs_dirhash = $this->_getDirectoryHash($va_volume_info["absolutePath"], $vn_id)) === false) {
                    $this->error->setError(
                        1600,
                        _t("Couldn't create subdirectory for file for %1", $vs_field),
                        "mediaproc->process()"
                    );
                    $o_media->cleanup();
                    return false;
                ...
                if (!copy($vs_input_file, $vs_filepath)) {
                    $this->error->setError(
                        504,
                        _t("File could not be copied for %1", $vs_field),
                        "mediaproc->process()"
                    );
                    $o_media->cleanup();
                    return false;
        ...
        if ($t_instance->load($vn_id)) {
            ...
            if (((bool)$o_config->get('video_preview_generate_frames') || (bool)$o_config->get(
