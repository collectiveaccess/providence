...
class WLPlugTaskQueueHandlermediaImport Extends WLPlug Implements IWLPlugTaskQueueHandler {
	...
	public function getParametersForDisplay( $pa_rec ) {
		...
		$vs_relative_directory                = preg_replace( "!{$vs_batch_media_import_root_directory}[/]*!", "",
			$va_parameters["importFromDirectory"] );
		...
		if ( file_exists( $va_parameters["importFromDirectory"] ) ) {
			$va_counts                    = caGetDirectoryContentsCount( $va_parameters["importFromDirectory"],
				$va_parameters["includeSubDirectories"], false );
	...
	public function process( $pa_parameters ) {
		...
		$o_request  = new RequestHTTP( $o_response, array(
			'simulateWith' => array(
				...
				'REQUEST_METHOD'  => 'POST',
				'REQUEST_URI'     => join( '/',
					array( __CA_URL_ROOT__, 'index.php', 'batch', 'MediaImport', 'Save', $pa_parameters['screen'] ) ),
				'PATH_INFO'       => '/' . join( '/',
						array( 'batch', 'MediaImport', 'Save', $pa_parameters['screen'] ) ),
