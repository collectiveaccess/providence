...
class WLPlugTaskQueueHandlerftpmirror Extends WLPlug Implements IWLPlugTaskQueueHandler {
	...
	public function process( $pa_parameters ) {
		...
		if ( ! $r_ftp ) {
			$this->error->setError( 580,
				"Could not connect to FTP server at " . $pa_parameters["MIRROR_INFO"]["hostname"],
				"ftpmirror->process()" );

			return 0;
		}
		if ( ! ftp_login( $r_ftp, $pa_parameters["MIRROR_INFO"]["username"],
			$pa_parameters["MIRROR_INFO"]["password"] )
		) {
			$this->error->setError( 581,
				"Could not login to FTP server at " . $pa_parameters["MIRROR_INFO"]["hostname"],
				"ftpmirror->process()" );
		...
		foreach ( $va_files as $va_file_info ) {
			...
			if ( $pa_parameters["DELETE"] ) {
				...
				#
				$vs_remote_path = join( "/", array(
					$pa_parameters["MIRROR_INFO"]["directory"],
					$va_file_info["HASH"],
					$va_file_info["FILENAME"]
				) );
				...
				if ( file_exists( $vs_file_path ) ) {
					...
					# bad table name
					$this->error->setError( 570, "File to mirror '$vs_file_path' does not exist",
						"ftpmirror->process()" );
				}
			}
		}
				...
				if ( $table_obj->load( $id ) ) {
					...
					if ( $table_obj->numErrors() ) {
						$o_eventlog->log( array(
							...
							"MESSAGE" => "Could not update mirror status for mirror '" . $pa_parameters["MIRROR"]
							             . "' on '$table'; row_id=$id\n"
						) );
						$va_report['errors'][]
							= _t( "Could not update mirror status for mirror '%1' on '%2'; row_id=%3",
							$pa_parameters["MIRROR"], $table, $id );
					}
				}
			}
		}
