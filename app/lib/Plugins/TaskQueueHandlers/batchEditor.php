...
class WLPlugTaskQueueHandlerbatchEditor Extends WLPlug Implements IWLPlugTaskQueueHandler {
	...
	public function getParametersForDisplay( $pa_rec ) {
		...
		if ( $t_screen->load( array(
			'ui_id'     => $t_ui->getPrimaryKey(),
			'screen_id' => str_ireplace( "screen", "", $va_parameters['screen'] )
		) )
		) {
			$va_params['ui'] = array(
	...
	public function process( $pa_parameters ) {
		...
		$o_request  = new RequestHTTP( $o_response, array(
			'simulateWith' => $x = array(
				'POST'            => $pa_parameters['values'],
				'SCRIPT_NAME'     => join( '/', array( __CA_URL_ROOT__, 'index.php' ) ),
				'REQUEST_METHOD'  => 'POST',
				'REQUEST_URI'     => join( '/', array(
					__CA_URL_ROOT__,
					'index.php',
					'batch',
					'Editor',
					'Save',
					$pa_parameters['screen'],
					'set_id',
					$pa_parameters['set_id']
				) ),
				'PATH_INFO'       => '/' . join( '/', array(
						'batch',
						'Editor',
						'Save',
						$pa_parameters['screen'],
		...
		if ( isset( $pa_parameters['isBatchTypeChange'] ) && $pa_parameters['isBatchTypeChange'] ) {
			$va_report = BatchProcessor::changeTypeBatchForSet( $o_request, $pa_parameters['new_type_id'], $t_set,
				$t_subject, array(
					'sendMail' => (bool) $pa_parameters['sendMail'],
					'sendSMS'  => (bool) $pa_parameters['sendSMS']
				) );
		} else {
			$va_report = BatchProcessor::saveBatchEditorFormForSet( $o_request, $t_set, $t_subject, array(
				'sendMail' => (bool) $pa_parameters['sendMail'],
				'sendSMS'  => (bool) $pa_parameters['sendSMS']
			) );
		}
