...
class WLPlugMediaGmagick Extends BaseMediaPlugin Implements IWLPlugMedia {
	...
	public function __construct() {
		$this->description
			= _t( 'Provides image processing and conversion services using ImageMagick via the PECL Gmagick PHP extension' );
	}
	# ------------------------------------------------
	...
	public function divineFileFormat( $ps_filepath ) {
		...
		if ( $this->ops_dcraw_path ) {
			caExec( $this->ops_dcraw_path . " -i " . caEscapeShellArg( $ps_filepath ) . " 2> /dev/null", $va_output,
				$vn_return );
			if ( $vn_return == 0 ) {
				if ( ( ! preg_match( "/^Cannot decode/", $va_output[0] ) )
				     && ( ! preg_match( "/Master/i", $va_output[0] ) )
				) {
	...
	public function read( $ps_filepath, $mimetype = "", $options = null ) {
		if ( ! ( ( $this->handle ) && ( $ps_filepath === $this->filepath ) ) ) {
			...
			if ( $mimetype == 'image/tilepic' ) {
				...
				$this->properties["exif_orientation"] = ( in_array(
					$orientation = (int) $this->metadata['EXIF']['IFD0']['Orientation'], [ 6, 8 ], true ) )
					? $orientation : null;
			...
			switch ( $operation ) {
				...
				case 'ANNOTATE':
					...
					if ( $parameters['font'] ) {
						try {
							...
							$this->postError( 1655,
								_t( "Couldn't set font to %1. Gmagick error message: %2", $parameters['font'],
									$e->getMessage() ), "WLPlugGmagick->transform()" );
							break;
						}
					}
				...
				case 'WATERMARK':
					...
					try {
						...
						$this->postError( 1610, _t( "Couldn't load watermark image at %1", $parameters['image'] ),
							"WLPlugGmagick->transform:WATERMARK()" );
				...
				case 'SCALE':
					...
					if ( $w > 0 && $h > 0 ) {
						...
						if ( ! $this->handle->resizeimage( $w + ( $crop_w_edge * 2 ), $h + ( $crop_h_edge * 2 ),
							Gmagick::FILTER_CUBIC, $aa )
						) {
							$this->postError( 1610, _t( "Error during resize operation" ),
								"WLPlugGmagick->transform()" );
						...
						if ( $do_fill_box_crop ) {
								...
								default:
									...
									if ( $w > $parameters["width"] ) {
										...
										if ( $h > $parameters["height"] ) {
											$crop_from_offset_y = ceil( $h * $vn_center_y ) - ( $parameters["height"]
											                                                    / 2 );
							...
							if ( ! $this->handle->cropimage( $parameters["width"], $parameters["height"],
								$crop_w_edge + $crop_from_offset_x, $crop_h_edge + $crop_from_offset_y )
							) {
								$this->postError( 1610, _t( "Error during crop operation" ),
									"WLPlugGmagick->transform()" );
				...
				case "ROTATE":
					...
					if ( ( $angle > - 360 ) && ( $angle < 360 ) ) {
						if ( ! $this->handle->rotateimage( "#FFFFFF", $angle ) ) {
							$this->postError( 1610, _t( "Error during image rotate" ),
								"WLPlugGmagick->transform():ROTATE" );
				...
				case "DESPECKLE":
					if ( ! $this->handle->despeckleimage() ) {
						$this->postError( 1610, _t( "Error during image despeckle" ),
							"WLPlugGmagick->transform:DESPECKLE()" );
				...
				case "MEDIAN":
					...
					if ( ! $this->handle->medianfilterimage( $radius ) ) {
						$this->postError( 1610, _t( "Error during image median filter" ),
							"WLPlugGmagick->transform:MEDIAN()" );
				...
				case "SHARPEN":
					...
					if ( ! $this->handle->sharpenImage( $radius, $sigma ) ) {
						$this->postError( 1610, _t( "Error during image sharpen" ),
							"WLPlugGmagick->transform:SHARPEN()" );
				...
				case "FLIP":
					...
					if ( $dir == 'vertical' ) {
						if ( ! $this->handle->flipimage() ) {
							$this->postError( 1610, _t( "Error during vertical image flip" ),
						...
						if ( ! $this->handle->flopimage() ) {
							$this->postError( 1610, _t( "Error during horizontal image flip" ),
								"WLPlugGmagick->transform:FLIP()" );
	...
	public function write( $ps_filepath, $mimetype ) {
		...
		if ( $mimetype == "image/tilepic" ) {
			...
			$use_exif_tool_to_strip
				= (bool) $this->opo_config->get( 'dont_use_exiftool_to_strip_exif_orientation_tags' );
			...
			if ( $this->properties['gamma'] ) {
				...
				$this->handle->levelimage( $this->properties['reference-black'], $this->properties['gamma'],
					$this->properties['reference-white'] );
			}
	...
	private function _dcrawConvertToTiff( $ps_filepath ) {
		if ( ! $this->ops_dcraw_path ) {
			$this->postError( 1610,
				_t( "Could not convert Camera RAW format file because conversion tool (dcraw) is not installed" ),
				"WLPlugGmagick->read()" );
		...
		if ( ! copy( $ps_filepath, $vs_tmp_name ) ) {
			$this->postError( 1610, _t( "Could not copy Camera RAW file to temporary directory" ),
				"WLPlugGmagick->read()" );
		...
		if ( $vn_return != 0 ) {
			$this->postError( 1610, _t( "Camera RAW file conversion failed: %1", $vn_return ),
				"WLPlugGmagick->read()" );
		...
		try {
			...
			if ( WLPlugMediaGmagick::$s_metadata_read_cache[ $ps_filepath ] ) {
				...
				if ( function_exists( 'exif_read_data' )
					...
					if ( ( $vn_exif_size <= $this->opo_config->get( 'dont_use_exif_read_data_if_larger_than' ) )
					     && ( is_array( $va_exif = caSanitizeArray( $va_exif_data ) ) )
					) {
				...
				if ( isset( $va_metadata['EXIF'] ) && is_array( $va_metadata['EXIF'] )
				     && array_key_exists( "FILE", $va_metadata['EXIF'] )
				...
				if ( $this->ops_graphicsmagick_path ) {
					...
					@rename( $vs_iptc_file,
						$vs_iptc_file . '.iptc' );  // GM uses the file extension to figure out what we want
					$vs_iptc_file .= '.iptc';
					caExec( $this->ops_graphicsmagick_path . " convert " . caEscapeShellArg( $ps_filepath ) . " "
					        . caEscapeShellArg( $vs_iptc_file ) . ( caIsPOSIX() ? " 2> /dev/null" : "" ), $va_output,
						$vn_return );
					...
					/* DPX metadata */
					caExec( $this->ops_graphicsmagick_path . " identify -format '%[DPX:*]' "
				...
				if ( sizeof( WLPlugMediaGmagick::$s_metadata_read_cache ) > 100 ) {
					WLPlugMediaGmagick::$s_metadata_read_cache
						= array_slice( WLPlugMediaGmagick::$s_metadata_read_cache, 50 );
				}
