...
class WLPlugMediaGmagick Extends BaseMediaPlugin Implements IWLPlugMedia
{
     ...
     */
    static public $s_metadata_read_cache = [];

    # ------------------------------------------------
    public function __construct()
    {
        $this->description = _t(
            'Provides image processing and conversion services using ImageMagick via the PECL Gmagick PHP extension'
        );
    }
    # ------------------------------------------------
    ...
    public function divineFileFormat($ps_filepath)
    {
        ...
        if ($this->ops_dcraw_path) {
            caExec(
                $this->ops_dcraw_path . " -i " . caEscapeShellArg($ps_filepath) . " 2> /dev/null",
                $va_output,
                $vn_return
            );
            if ($vn_return == 0) {
        ...
        if (!(($this->handle) && ($ps_filepath === $this->filepath))) {
            if ($mimetype == 'image/tilepic') {
                ...
                $this->properties["exif_orientation"] = (in_array(
                    $orientation = (int)$this->metadata['EXIF']['IFD0']['Orientation'],
                    [6, 8],
                    true
                )) ? $orientation : null;
                ...
                if (!$this->handle->setimagecolorspace(Gmagick::COLORSPACE_RGB)) {
                    $this->postError(
                        1610,
                        _t("Error during RGB colorspace transformation operation"),
                        "WLPlugGmagick->read()"
                    );
                    return false;
                }
            ...
            switch ($operation) {
                    ...
                    if ($parameters['font']) {
                        try {
                            ...
                            $this->postError(
                                1655,
                                _t(
                                    "Couldn't set font to %1. Gmagick error message: %2",
                                    $parameters['font'],
                                    $e->getMessage()
                                ),
                ...
                case 'WATERMARK':
                    ...
                    try {
                        ...
                        $this->postError(
                            1610,
                            _t("Couldn't load watermark image at %1", $parameters['image']),
                ...
                case 'SCALE':
                    ...
                    if ($w > 0 && $h > 0) {
                        ...
                        if ($do_fill_box_crop) {
                            ...
                            if (!$this->handle->cropimage(
                                $parameters["width"],
                                $parameters["height"],
                                $crop_w_edge + $crop_from_offset_x,
                                $crop_h_edge + $crop_from_offset_y
                            )) {
                            ...
                            if ($crop_w_edge || $crop_h_edge) {
                                if (!$this->handle->cropimage($w, $h, $crop_w_edge, $crop_h_edge)) {
                                    $this->postError(
                                        1610,
                                        _t("Error during crop operation"),
                                        "WLPlugGmagick->transform()"
                                    );
                                    return false;
                                }
                ...
                case "DESPECKLE":
                    if (!$this->handle->despeckleimage()) {
                        $this->postError(
                            1610,
                            _t("Error during image despeckle"),
                ...
                case "MEDIAN":
                    ...
                    if (!$this->handle->medianfilterimage($radius)) {
                        $this->postError(
                            1610,
                            _t("Error during image median filter"),
                ...
                case "FLIP":
                    ...
                    if ($dir == 'vertical') {
                        if (!$this->handle->flipimage()) {
                            $this->postError(
                                1610,
                                _t("Error during vertical image flip"),
                        ...
                        if (!$this->handle->flopimage()) {
                            $this->postError(
                                1610,
                                _t("Error during horizontal image flip"),
                                "WLPlugGmagick->transform:FLIP()"
                            );
                            return false;
                        }
    ...
    public function write($ps_filepath, $mimetype)
    {
        ...
        if ($mimetype == "image/tilepic") {
            ...
            if ($this->properties['gamma']) {
                ...
                $this->handle->levelimage(
                    $this->properties['reference-black'],
                    $this->properties['gamma'],
    ...
    private function _dcrawConvertToTiff($ps_filepath)
    {
        if (!$this->ops_dcraw_path) {
            $this->postError(
                1610,
                _t(
                    "Could not convert Camera RAW format file because conversion tool (dcraw) is not installed"
                ),
                "WLPlugGmagick->read()"
            );
            return false;
        ...
        if (!copy($ps_filepath, $vs_tmp_name)) {
            $this->postError(
                1610,
                _t("Could not copy Camera RAW file to temporary directory"),
                "WLPlugGmagick->read()"
            );
            return false;
        }
        ...
        try {
            ...
            if (WLPlugMediaGmagick::$s_metadata_read_cache[$ps_filepath]) {
                ...
                if (function_exists('exif_read_data') && !($this->opo_config->get('dont_use_exif_read_data'))) {
                    ...
                    if (($vn_exif_size <= $this->opo_config->get(
                                'dont_use_exif_read_data_if_larger_than'
                ...
                if (isset($va_metadata['EXIF']) && is_array($va_metadata['EXIF']) && array_key_exists(
                        "FILE",
                        $va_metadata['EXIF']
                ...
                if (sizeof(WLPlugMediaGmagick::$s_metadata_read_cache) > 100) {
                    WLPlugMediaGmagick::$s_metadata_read_cache = array_slice(
