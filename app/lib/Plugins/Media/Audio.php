...
class WLPlugMediaAudio Extends BaseMediaPlugin Implements IWLPlugMedia
{
    ...
    public function read($filepath)
    {
        ...
        if (!((isset($this->handle["error"])) && (is_array($this->handle["error"])) && (sizeof(
                    $this->handle["error"]
                ) > 0))) {
            ...
            switch ($this->properties["mimetype"]) {
                case 'audio/mpeg':
                    ...
                    if (
                        ...
                        $this->properties["description"] = join(
                                "; ",
                                $this->handle["tags"]["id3v1"]["album"]
                            ) . " " . join("; ", $this->handle["tags"]["id3v1"]["year"]) . " " . join(
                                "; ",
                                $this->handle["tags"]["id3v1"]["genre"]
                            );
                    }
    ...
    public function write($filepath, $mimetype, $pa_options = null)
    {
        ...
        if (!($ext = $this->info["EXPORT"][$mimetype])) {
            ...
            $this->postError(
                1610,
                _t(
                    "Can't convert '%1' to '%2': unsupported format",
                    $this->handle["mime_type"],
                    $mimetype
                ),
                "WLPlugAudio->write()"
            );
        ...
        if (
            ...
            if (($mimetype != "image/png") && ($mimetype != "image/jpeg") && ($this->opb_ffmpeg_available)) {
                ...
                if ($mimetype == 'audio/ogg') {
                    exec(
                        $this->ops_path_to_ffmpeg . " -f " . $this->info["IMPORT"][$this->properties["mimetype"]] . " -i " . caEscapeShellArg(
                            $this->filepath
                        ) . " -acodec libvorbis -ab " . $vn_output_bitrate . " -ar " . $vn_sample_frequency . " -ac " . $vn_channels . "  -y " . caEscapeShellArg(
                            $filepath . "." . $ext
                        ) . (caIsPOSIX() ? " 2>&1" : ""),
                        $va_output,
                        $vn_return
                    ...
                    exec(
                        $this->ops_path_to_ffmpeg . " -f " . $this->info["IMPORT"][$this->properties["mimetype"]] . " -i " . caEscapeShellArg(
                            $this->filepath
                        ) . " -f " . $this->info["EXPORT"][$mimetype] . " -ab " . $vn_output_bitrate . " -ar " . $vn_sample_frequency . " -ac " . $vn_channels . "  -y " . caEscapeShellArg(
                            $filepath . "." . $ext
                        ) . (caIsPOSIX() ? " 2>&1" : ""),
                        $va_output,
                        $vn_return
                ...
                if ($vn_return != 0) {
                    ...
                    $this->postError(
                        1610,
                        _t(
                            "Error converting file to %1 [%2]: %3",
                            $this->typenames[$mimetype],
                            $mimetype,
                            join("; ", $va_output)
                        ),
                        "WLPlugAudio->write()"
                ...
                if ($mimetype == "audio/mpeg") {
                    if ($vs_intro_filepath || $vs_outro_filepath) {
                        ...
                        if ($vs_intro_filepath) {
                            exec(
                                $this->ops_path_to_ffmpeg . " -i " . caEscapeShellArg(
                                    $vs_intro_filepath
                                ) . " -f mp3 -ab " . $vn_output_bitrate . " -ar " . $vn_sample_frequency . " -ac " . $vn_channels . " -y " . caEscapeShellArg(
                                    $vs_tmp_filename
                                ) . (caIsPOSIX() ? " 2>&1" : ""),
                                $va_output,
                                $vn_return
                            ...
                            if ($vn_return != 0) {
                                ...
                                $this->postError(
                                    1610,
                                    _t(
                                        "Error converting intro to %1 [%2]: %3",
                                        $this->typenames[$mimetype],
                                        $mimetype,
                                        join("; ", $va_output)
                                    ),
                        ...
                        if ($vs_outro_filepath) {
                            ...
                            if ($vn_return != 0) {
                                ...
                                $this->postError(
                                    1610,
                                    _t(
                                        "Error converting outro to %1 [%2]: %3",
                                        $this->typenames[$mimetype],
                                        $mimetype,
                                        join("; ", $va_output)
                                    ),
                    ...
                    if (
                        ...
                        $this->properties["description"] = join(
                                "; ",
                                $va_mp3_output_info["tags"]["id3v1"]["album"]
                            ) . " " . join("; ", $va_mp3_output_info["tags"]["id3v1"]["year"]) . " " . join(
                                "; ",
        ...
        switch ($pa_properties["mimetype"]) {
            ...
            case 'audio/mpeg':
                ...
                <div class="<?php print (isset($pa_options["class"])) ? $pa_options["class"] : "caAudioPlayer"; ?>">
                    <audio id="<?php print $vs_id; ?>"
                           src="<?php print $ps_url; ?>" <?php print ($vs_poster_url = caGetOption(
                        'posterURL',
                        $pa_options,
                        null
                    )) ? "poster='{$vs_poster_url}'" : ''; ?> type="audio/mp3" controls="controls"></audio>
                </div>
