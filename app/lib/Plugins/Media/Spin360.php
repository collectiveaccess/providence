...
class WLPlugMediaSpin360 extends BaseMediaPlugin implements IWLPlugMedia
{
    ...
    {
        $this->description = _t(
            'Accepts ZIP archives containing 360 spinnable images in SpinCar format (http://SpinCar.com)'
        );
        ...
        if (is_array($this->handle) && ($this->filepath == $ps_filepath)) {
            ...
            if (!($vs_mimetype = $this->divineFileFormat($ps_filepath))) {
                $this->postError(
                    3005,
                    _t(
                        "File %1 is not a valid ZIP archive or does not contain valid images",
                        $ps_filepath
                    ),
                    "WLPlugMediaSpin360->read()"
                );
                $this->handle = $this->filepath = "";
                return false;
            }
