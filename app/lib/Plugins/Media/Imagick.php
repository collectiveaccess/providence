...
class WLPlugMediaImagick Extends BaseMediaPlugin Implements IWLPlugMedia
{
    ...
    {
        $this->description = _t(
            'Provides image processing and conversion services using ImageMagick via the PECL Imagick PHP extension'
        );
    }
    # ------------------------------------------------
    ...
    public function divineFileFormat($ps_filepath)
    {
        ...
        if (caMediaPluginDcrawInstalled($this->ops_dcraw_path)) {
            exec(
                $this->ops_dcraw_path . " -i " . caEscapeShellArg($ps_filepath) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                $va_output,
                $vn_return
            );
            if ($vn_return == 0) {
        ...
        if (!(($this->handle) && ($$ps_filepath === $this->filepath))) {
            if ($mimetype == 'image/tilepic') {
                ...
                if ($mimetype == 'image/x-dcraw') {
                    ...
                    exec(
                        $this->ops_dcraw_path . " -T " . caEscapeShellArg($vs_tmp_name) . (caIsPOSIX(
                        ) ? " 2> /dev/null" : ""),
                        $va_output,
                        $vn_return
                    );
                    if ($vn_return != 0) {
                        $this->postError(
                            1610,
                            _t("Camera RAW file conversion failed: %1", $vn_return),
                            "WLPlugImagick->read()"
                        );
                        return false;
                    }
                    if (!(file_exists($vs_tmp_name . '.tiff') && (filesize($vs_tmp_name . '.tiff') > 0))) {
                        $this->postError(
                            1610,
                            _t("Translation from Camera RAW to TIFF failed"),
                            "WLPlugImagick->read()"
                        );
                        return false;
                    }
                ...
                if ($handle->readImage($ps_filepath)) {
                    ...
                    $this->properties["colorspace"] = $this->_getColorspaceAsString(
                        $this->handle->getImageColorspace()
                    );
            ...
            switch ($operation) {
                ...
                case "UNSHARPEN_MASK":
                    ...
                    if (!$this->handle->unsharpMaskImage($radius, $sigma, $amount, $threshold)) {
                        $this->postError(
                            1610,
                            _t("Error during image unsharp mask"),
