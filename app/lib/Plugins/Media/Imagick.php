...
class WLPlugMediaImagick Extends BaseMediaPlugin Implements IWLPlugMedia {
	...
	public function __construct() {
		$this->description
			= _t( 'Provides image processing and conversion services using ImageMagick via the PECL Imagick PHP extension' );
	}
	# ------------------------------------------------
	...
	public function divineFileFormat( $ps_filepath ) {
		...
		if ( $this->ops_dcraw_path ) {
			caExec( $this->ops_dcraw_path . " -i " . caEscapeShellArg( $ps_filepath ) . ( caIsPOSIX() ? " 2> /dev/null"
					: "" ), $va_output, $vn_return );
			if ( $vn_return == 0 ) {
		...
		if ( ! ( ( $this->handle ) && ( $$ps_filepath === $this->filepath ) ) ) {
			...
			if ( $mimetype == 'image/tilepic' ) {
				...
				if ( $mimetype == 'image/x-dcraw' ) {
					...
					caExec( $this->ops_dcraw_path . " -T " . caEscapeShellArg( $vs_tmp_name ) . ( caIsPOSIX()
					...
					if ( ! ( file_exists( $vs_tmp_name . '.tiff' ) && ( filesize( $vs_tmp_name . '.tiff' ) > 0 ) ) ) {
						$this->postError( 1610, _t( "Translation from Camera RAW to TIFF failed" ),
							"WLPlugImagick->read()" );
				...
				if ( $handle->readImage( $ps_filepath ) ) {
					...
					if ( function_exists( 'exif_read_data' )
						...
						if ( is_array(
							$va_exif = caSanitizeArray( @exif_read_data( $ps_filepath, 'EXIF', true, false ) ) )
						) {
							//
					...
					$this->properties["colorspace"]
					                                = $this->_getColorspaceAsString( $this->handle->getImageColorspace() );
			...
			switch ( $operation ) {
				...
				case 'WATERMARK':
					...
					if ( ! $w->readImage( $parameters['image'] ) ) {
						$this->postError( 1610, _t( "Couldn't load watermark image at %1", $parameters['image'] ),
							"WLPlugImagick->transform:WATERMARK()" );
					...
					//}
					$d->composite( imagick::COMPOSITE_DISSOLVE, $vn_watermark_x, $vn_watermark_y, $vn_watermark_width,
						$vn_watermark_height, $w );
				...
				case "DESPECKLE":
					...
					if ( ! $this->handle->despeckleImage() ) {
						$this->postError( 1610, _t( "Error during image despeckle" ),
							"WLPlugImagick->transform:DESPECKLE()" );
				...
				case "MEDIAN":
					...
					if ( ! $this->handle->medianFilterImage( $radius ) ) {
						$this->postError( 1610, _t( "Error during image median filter" ),
							"WLPlugImagick->transform:MEDIAN()" );
				...
				case "SHARPEN":
					...
					if ( ! $this->handle->sharpenImage( $radius, $sigma ) ) {
						$this->postError( 1610, _t( "Error during image sharpen" ),
							"WLPlugImagick->transform:SHARPEN()" );
				...
				case "UNSHARPEN_MASK":
					...
					if ( ! $this->handle->unsharpMaskImage( $radius, $sigma, $amount, $threshold ) ) {
						$this->postError( 1610, _t( "Error during image unsharp mask" ),
							"WLPlugImagick->transform:UNSHARPEN_MASK()" );
