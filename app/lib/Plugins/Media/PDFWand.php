...
class WLPlugMediaPDFWand Extends BaseMediaPlugin implements IWLPlugMedia
{
    ...
    {
        $this->description = _t(
            'Provides PDF conversion services using ImageMagick or the Zend_PDF library. Will use Ghostscript to generate image-previews of PDF files.'
        );
    }
    # ------------------------------------------------
    ...
    public function checkStatus()
    {
        ...
        if (!caMediaPluginPdftotextInstalled($this->ops_pdftotext_path)) {
            $va_status['warnings'][] = _t(
                "PDFToText cannot be found: indexing of text in PDF files will not be performed; you can obtain PDFToText at http://www.foolabs.com/xpdf/download.html"
            );
        }
    ...
    public function divineFileFormat($ps_filepath)
    {
        ...
        if ((isset(WLPlugMediaPDFWand::$s_pdf_info_cache[$ps_filepath]) && ($va_info = WLPlugMediaPDFWand::$s_pdf_info_cache[$ps_filepath])) || (WLPlugMediaPDFWand::$s_pdf_info_cache[$ps_filepath] = $va_info = caGetPDFInfo(
                $ps_filepath
            ))) {
                ...
                switch ($property) {
                    case 'quality':
                        if (($value < 1) || ($value > 100)) {
                            $this->postError(
                                1650,
                                _t("Quality property must be between 1 and 100"),
                                "WLPlugPDFWand->set()"
                            );
                            return '';
                        }
                    ...
                    case 'antialiasing':
                        if (($value < 0) || ($value > 100)) {
                            $this->postError(
                                1650,
                                _t("Antialiasing property must be between 0 and 100"),
                                "WLPlugPDFWand->set()"
                            );
                            return '';
                        }
    ...
    public function read($ps_filepath)
    {
        ...
        if (caPDFMinerInstalled($this->ops_pdfminer_path)) {
            ...
            exec(
                $this->ops_pdfminer_path . ' -t text ' . caEscapeShellArg($ps_filepath) . ' > ' . caEscapeShellArg(
                    $vs_tmp_filename
            ...
            exec(
                $this->ops_pdfminer_path . ' -A -t xml ' . caEscapeShellArg($ps_filepath) . ' > ' . caEscapeShellArg(
                    $vs_tmp_filename
                ) . (caIsPOSIX() ? " 2> /dev/null" : "")
            );
            ...
            if (caMediaPluginPdftotextInstalled($this->ops_pdftotext_path)) {
                $vs_tmp_filename = tempnam('/tmp', 'CA_PDF_TEXT');
                exec(
                    $this->ops_pdftotext_path . ' -q -enc UTF-8 ' . caEscapeShellArg(
                        $ps_filepath
        ...
        if ($ps_mimetype == "application/pdf") {
            ...
            if (caMediaPluginGhostscriptInstalled($this->ops_ghostscript_path)) {
                ...
                switch ($ps_mimetype) {
                    ...
                    case 'image/png':
                        exec(
                            $this->ops_ghostscript_path . " -dNumRenderingThreads=6 -dNOPAUSE -dBATCH -sDEVICE=pngalpha {$vs_antialiasing} -dFirstPage=" . $vn_start_page . " -dLastPage=" . $vn_end_page . " -dMaxPatternBitmap=1000000 -dBandBufferSpace=500000000 -sBandListStorage=memory -dBufferSpace=1000000000 -dBandHeight=100 -sOutputFile=" . caEscapeShellArg(
                                $ps_filepath . "." . $vs_ext
                            ) . " -r" . $vs_res . " -c \"30000000 setvmthreshold\" -f " . caEscapeShellArg(
                                $this->handle["filepath"]
                            ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                    ...
                    case 'image/gif':
                        exec(
                            $this->ops_ghostscript_path . " -dNumRenderingThreads=6 -dNOPAUSE -dBATCH -sDEVICE=tiff24nc {$vs_antialiasing} -dFirstPage=" . $vn_start_page . " -dLastPage=" . $vn_end_page . " -dMaxPatternBitmap=1000000 -dBandBufferSpace=500000000 -sBandListStorage=memory -dBufferSpace=1000000000 -dBandHeight=100 -sOutputFile=" . caEscapeShellArg(
                                $ps_filepath . "." . $vs_ext
                            ) . " -r" . $vs_res . " -c \"30000000 setvmthreshold\" -f " . caEscapeShellArg(
                                $this->handle["filepath"]
                            ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                        ...
                        if ($vs_crop = $this->get("crop")) {
                            ...
                            if (($vn_w > 0) && ($vn_h > 0)) {
                                ...
                                if (!$o_media->numErrors()) {
                                    ...
                                    $o_media->write(
                                        preg_replace("!\.[A-Za-z0-9]+$!", '', $vs_file),
                                        $ps_mimetype,
                                        array()
                                    );
                                    if (!$o_media->numErrors()) {
    ...
    {
        if (!(bool)$this->opo_config->get(
                "document_preview_generate_pages"
            ) && (!isset($pa_options['force']) || !$pa_options['force'])) {
            return false;
        }
        if (!isset($pa_options['outputDirectory']) || !$pa_options['outputDirectory'] || !file_exists(
                $pa_options['outputDirectory']
            )) {
        ...
        $va_files = $this->write(
            ...
            [
                ...
                'antialiasing' => true,
                'start' => $vn_start_at,
                'numPages' => (($vn_tot_pages > $vn_max_number_of_pages) > $vn_max_number_of_pages) ? $vn_max_number_of_pages : $vn_tot_pages
            ]
        );
    ...
    public function htmlTag($ps_url, $pa_properties, $pa_options = null, $pa_volume_info = null)
    {
        ...
        if (preg_match("/\.pdf\$/", $ps_url)) {
            if ($vs_poster_frame_url = $pa_options["poster_frame_url"]) {
                $vs_poster_frame = "<img src='{$vs_poster_frame_url}'/ alt='" . _t(
