            ...
            if ($vb_require_schema = (bool)$o_xml_config->get('xml_do_validation')) {
                ...
                if (!$o_xml->schemaValidate($vs_xml_resource_path . '/schemas/' . $vs_schema_path)) {
                    ...
                    foreach ($va_xml_errors as $va_xml_error) {
                        $va_xml_error_messages[] = _t(
                                'At line %1',
                                $va_xml_error->line
                            ) . ': ' . $va_xml_error->message;
                    }
                    libxml_clear_errors();
                    $this->postError(
                        3020,
                        _t(
                            "Validation against XML schema failed with errors:<br/>" . join(
                                '; ',
                                $va_xml_error_messages
                            )
                        ),
                        "WLPlugMediaXMLDoc->read()"
                    );
                    $this->handle = $this->filepath = "";
                    return false;
                }
        ...
        if (preg_match("/\.xml\$/", $ps_url)) {
            ...
            if (preg_match("/\.pdf\$/", $ps_url)) {
                if ($pa_options['embed']) {
                    ...
                    return "<object data='{$ps_url}' type='application/pdf' width='{$vn_viewer_width}' height='{$vn_viewer_height}'><p><a href='$ps_url' target='_pdf'>" . _t(
                            "View PDF file"
                        ) . "</a></p></object>";
                } else {
