...
class WLPlugMediaImageMagick Extends BaseMediaPlugin Implements IWLPlugMedia
{
    ...
    {
        $this->description = _t(
            'Provides image processing and conversion services using ImageMagick via exec() calls to ImageMagick binaries'
        );
    }
    # ------------------------------------------------
    ...
    public function &writePreviews($ps_filepath, $pa_options)
    {
        ...
        $va_output = array();
        exec(
            $this->ops_imagemagick_path . '/identify -format "%m\n" ' . caEscapeShellArg($this->filepath) . (caIsPOSIX(
            ) ? " 2> /dev/null" : ""),
            $va_output,
            $vn_return
        );
        ...
        exec(
            $this->ops_imagemagick_path . '/convert ' . caEscapeShellArg(
                $this->filepath
    ...
    public function joinArchiveContents($pa_files, $pa_options = array())
    {
        ...
        if (sizeof($va_acceptable_files)) {
            exec(
                $this->ops_imagemagick_path . "/convert " . join(
                    " ",
                    $va_acceptable_files
    ...
    private function _imageMagickIdentify($ps_filepath)
    {
        if (caMediaPluginImageMagickInstalled($this->ops_imagemagick_path)) {
            exec(
                $this->ops_imagemagick_path . '/identify -format "%m" ' . caEscapeShellArg(
                    $ps_filepath . '[0]'
    ...
    private function _imageMagickGetMetadata($ps_filepath)
    {
        if (caMediaPluginImageMagickInstalled($this->ops_imagemagick_path)) {
            ...
            exec(
                $this->ops_imagemagick_path . "/identify -format '%[DPX:*]' " . caEscapeShellArg(
                    $ps_filepath
                ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                $va_output,
                $vn_return
            ...
            $vs_iptc_file .= '.iptc';
            exec(
                $this->ops_imagemagick_path . "/convert " . caEscapeShellArg($ps_filepath) . " " . caEscapeShellArg(
                    $vs_iptc_file
                ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                $va_output,
                $vn_return
    ...
    private function _imageMagickRead($ps_filepath)
    {
        if (caMediaPluginImageMagickInstalled($this->ops_imagemagick_path)) {
            exec(
                $this->ops_imagemagick_path . '/identify -format "%m;%w;%h;%[colorspace];%[depth];%[xresolution];%[yresolution]\n" ' . caEscapeShellArg(
                    $ps_filepath
                ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                $va_output,
                $vn_return
    ...
    private function _imageMagickWrite($pa_handle, $ps_filepath, $ps_mimetype, $pn_quality = null)
    {
        if (caMediaPluginImageMagickInstalled($this->ops_imagemagick_path)) {
            ...
            if (is_array($va_ops['convert']) && sizeof($va_ops['convert'])) {
                ...
                exec(
                    $this->ops_imagemagick_path . '/convert ' . caEscapeShellArg($vs_input_file . '[0]') . ' ' . join(
                        ' ',
                        $va_ops['convert']
            ...
            if (is_array($va_ops['composite']) && sizeof($va_ops['composite'])) {
                exec(
                    $this->ops_imagemagick_path . '/composite ' . join(
                        ' ',
                        $va_ops['composite']
