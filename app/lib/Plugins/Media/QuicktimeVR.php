...
class WLPlugMediaQuicktimeVR Extends BaseMediaPlugin Implements IWLPlugMedia
{
    ...
    public function read($filepath)
    {
        ...
        if (!((isset($this->handle["error"])) && (is_array($this->handle["error"])) && (sizeof(
            ...
            # -- get bandwidth
            $this->properties["has_video"] = (isset($this->handle["video"]["bitrate"]) && sizeof(
                $this->handle["video"]["bitrate"]
            ) ? 1 : 0);
            $this->properties["has_audio"] = (isset($this->handle["audio"]["bitrate"]) && sizeof(
                $this->handle["audio"]["bitrate"]
            ) ? 1 : 0);
        ...
        switch ($mimetype) {
            ...
            case 'image/jpeg':
                ...
                if (caMediaPluginFFmpegInstalled($this->ops_path_to_ffmpeg)) {
                    ...
                    exec(
                        $this->ops_path_to_ffmpeg . " -ss " . ($vn_start_secs) . " -i " . caEscapeShellArg(
                            $this->filepath
                        ) . " -f mjpeg -t 0.001 -y " . caEscapeShellArg($filepath . "." . $ext) . (caIsPOSIX(
                        ) ? " 2> /dev/null" : ""),
                        $va_output,
                        $vn_return
                    );
                    if (($vn_return < 0) || ($vn_return > 1) || (!@filesize($filepath . "." . $ext))) {
                        ...
                        $o_media->transform(
                            'SCALE',
                            array(
                                'width' => $vn_preview_width,
                                'height' => $vn_preview_height,
                                'mode' => 'bounding_box',
                                'antialiasing' => 0.5
                            )
                        );
            ...
            default:
                if (($mimetype != $this->handle["mime_type"])) {
                    ...
                    $this->postError(
                        1610,
                        _t("Can't convert '%1' to %2", $this->handle["mime_type"], $mimetype),
                        "WLPlugQuicktimeVR->write()"
                    );
                    return false;
                }
                # write the file
    ...
    public function &writePreviews($ps_filepath, $pa_options)
    {
        ...
        exec(
            $this->ops_path_to_ffmpeg . " -i " . caEscapeShellArg(
                $this->filepath
            ) . " -f image2 -r " . $vs_freq . " -ss {$vn_s} -t {$vn_previewed_duration} -s " . $vn_preview_width . "x" . $vn_preview_height . " -y " . caEscapeShellArg(
                $vs_output_file
