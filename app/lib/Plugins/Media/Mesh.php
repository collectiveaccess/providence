        ...
        switch ($ps_mimetype) {
            case 'application/ctm':
                if (file_exists($this->filepath) && caOpenCTMInstalled()) {
                    exec(
                        caGetExternalApplicationPath('openctm') . ' ' . caEscapeShellArg(
                            $this->filepath
                        ) . " " . caEscapeShellArg($ps_filepath) . ".ctm --method MG2 --level 9 2>&1",
                        $va_output
                    );
                ...
                if (($this->properties['mimetype'] == 'application/ply') && ($ps_mimetype == 'application/stl')) {
                    if (file_exists($this->filepath)) {
                        if (caMeshlabServerInstalled()) {
                            ...
                            chdir('/usr/local/bin');
                            exec(
                                caGetExternalApplicationPath('meshlabserver') . " -i " . caEscapeShellArg(
                                    $this->filepath
