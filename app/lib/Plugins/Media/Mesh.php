...
class WLPlugMediaMesh extends BaseMediaPlugin implements IWLPlugMedia {
	...
	public function checkStatus() {
		...
		if ( ! caMeshlabServerInstalled() ) {
			$va_status['warnings'][]
				= _t( "MeshLab cannot be found: you will not be able to process 3D files; you can obtain MeshLab at http://www.meshlab.net/" );
		} else {
		...
		switch ( $ps_mimetype ) {
			case 'application/ctm':
				if ( file_exists( $this->filepath ) && $this->ops_path_to_openctm ) {
					caExec( $this->ops_path_to_openctm . ' ' . caEscapeShellArg( $this->filepath ) . " "
					        . caEscapeShellArg( $ps_filepath ) . ".ctm --method MG2 --level 9 2>&1", $va_output );
				...
				if ( ( $this->properties['mimetype'] == 'application/ply' ) && ( $ps_mimetype == 'application/stl' ) ) {
					if ( file_exists( $this->filepath ) ) {
						if ( $this->ops_path_to_meshlab ) {
							putenv( "DISPLAY=:0" );
							chdir( '/usr/local/bin' );
							caExec( $this->ops_path_to_meshlab . " -i " . caEscapeShellArg( $this->filepath ) . " -o "
							        . caEscapeShellArg( $ps_filepath ) . ".stl 2>&1", $va_output );
							...
							$this->postError( 1610, _t( "Couldn't convert ply model to stl" ),
								"WLPlugMediaMesh->write()" );
	...
	public function htmlTag( $ps_url, $pa_properties, $pa_options = null, $pa_volume_info = null ) {
		...
		$vn_width  = ( isset( $pa_options["viewer_width"] ) && ( $pa_options["viewer_width"] > 0 ) )
			? $pa_options["viewer_width"] : 820;
		$vn_height = ( isset( $pa_options["viewer_height"] ) && ( $pa_options["viewer_height"] > 0 ) )
			? $pa_options["viewer_height"] : 520;

		$vs_id = ( isset( $pa_options["id"] ) && $pa_options["id"] ) ? $pa_options["id"] : "mesh_canvas";

		$vs_bgcolor = ( isset( $pa_options["background_color"] ) && $pa_options["background_color"] )
			? preg_replace( "![^A-Fa-f0-9]+!", "", $pa_options["background_color"] ) : "CCCCCC";

		$vs_progress_id = ( isset( $pa_options["progress_id"] ) && $pa_options["progress_id"] )
			? $pa_options["progress_id"] : "caMediaOverlayProgress";

		$vn_progress_total_filesize = ( isset( $pa_options["progress_total_filesize"] )
		                                && ( $pa_options["progress_total_filesize"] > 0 ) )
			? $pa_options["progress_total_filesize"] : 0;


		if ( in_array( $pa_properties['mimetype'],
			array( "application/ply", "application/stl", "application/ctm", "text/prs.wavefront-obj" ) )
		) {
			ob_start();
			?>
			<div id="viewer"></div>
	...
	public function _parseOBJ( $ps_filepath ) {
		...
		while ( ( ( $vs_line = trim( fgets( $r_rp ), "\n" ) ) !== false ) && ( $vn_c < 100 ) ) {
			...
			if ( in_array( $va_toks[0], [ 'v', 'vn' ] ) && ( sizeof( $va_toks ) >= 4 ) && is_numeric( $va_toks[1] )
			     && is_numeric( $va_toks[2] )
			     && is_numeric( $va_toks[3] )
			) {
				fclose( $r_rp );

				return true;
			}
			if ( ( $va_toks[0] === 'vt' ) && ( sizeof( $va_toks ) >= 3 ) && is_numeric( $va_toks[1] )
			     && is_numeric( $va_toks[2] )
			) {
				fclose( $r_rp );
