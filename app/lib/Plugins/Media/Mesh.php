...
class WLPlugMediaMesh extends BaseMediaPlugin implements IWLPlugMedia
{
    ...
    public function checkStatus()
    {
        ...
        if (!caMeshlabServerInstalled()) {
            $va_status['warnings'][] = _t(
                "MeshLab cannot be found: you will not be able to process 3D files; you can obtain MeshLab at http://www.meshlab.net/"
            );
        } else {
        ...
        switch ($ps_mimetype) {
            case 'application/ctm':
                if (file_exists($this->filepath) && $this->ops_path_to_openctm) {
                    caExec(
                        $this->ops_path_to_openctm . ' ' . caEscapeShellArg($this->filepath) . " " . caEscapeShellArg(
                            $ps_filepath
                        ) . ".ctm --method MG2 --level 9 2>&1",
                        $va_output
                    );
                ...
                if (($this->properties['mimetype'] == 'application/ply') && ($ps_mimetype == 'application/stl')) {
                    if (file_exists($this->filepath)) {
                        if ($this->ops_path_to_meshlab) {
                            ...
                            chdir('/usr/local/bin');
                            caExec(
                                $this->ops_path_to_meshlab . " -i " . caEscapeShellArg(
                                    $this->filepath
                                ) . " -o " . caEscapeShellArg($ps_filepath) . ".stl 2>&1",
                                $va_output
                            );
    ...
    public function htmlTag($ps_url, $pa_properties, $pa_options = null, $pa_volume_info = null)
    {
        ...
        $vs_bgcolor = (isset($pa_options["background_color"]) && $pa_options["background_color"]) ? preg_replace(
            "![^A-Fa-f0-9]+!",
            "",
            $pa_options["background_color"]
        ) : "CCCCCC";
    ...
    public function _parseOBJ($ps_filepath)
    {
        ...
        while ((($vs_line = trim(fgets($r_rp), "\n")) !== false) && ($vn_c < 100)) {
            ...
            if (in_array($va_toks[0], ['v', 'vn']) && (sizeof($va_toks) >= 4) && is_numeric($va_toks[1]) && is_numeric(
                    $va_toks[2]
                ) && is_numeric($va_toks[3])) {
                fclose($r_rp);
                return true;
            }
            if (($va_toks[0] === 'vt') && (sizeof($va_toks) >= 3) && is_numeric($va_toks[1]) && is_numeric(
                    $va_toks[2]
                )) {
                fclose($r_rp);
                return true;
