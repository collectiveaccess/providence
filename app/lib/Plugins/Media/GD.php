...
class WLPlugMediaGD Extends BaseMediaPlugin Implements IWLPlugMedia {
	...
	var $info
		= array(
			...
			"TRANSFORMATIONS" => array(
				...
				// trim_edges and crop_from are dummy and not supported by GD; they are supported by the ImageMagick-based plugins
	...
	public function checkStatus() {
		...
		if ( $this->register() ) {
			...
			if ( caMediaPluginImageMagickInstalled() ) {
				$va_status['unused'] = true;
				$va_status['warnings'][]
				                     = _t( "Didn't load because ImageMagick (command-line) is available and preferred" );
			}
			if ( caMediaPluginGmagickInstalled() ) {
				$va_status['unused']     = true;
			...
			switch ( $va_info[2] ) {
				...
				case IMAGETYPE_JPEG:
					if ( function_exists( 'exif_read_data' )
						...
						$this->metadata["EXIF"]
							= $va_exif = caSanitizeArray( @exif_read_data( $filepath, 'EXIF', true, false ) );

						// rewrite file name to use originally uploaded name
						if ( array_key_exists( "FILE", $this->metadata['EXIF'] )
						     && ( $f
								= caGetOption( 'original_filename', $options, null ) )
						) {
		...
		switch ( $operation ) {
			...
			case "FLIP":
				...
				if ( ! imageflip( $this->handle, ( $dir == 'vertical' ) ? IMG_FLIP_VERTICAL : IMG_FLIP_HORIZONTAL ) ) {
					$this->postError( 1610, _t( "Error during %1 image flip", $dir ),
						"WLPlugGmagick->transform:FLIP()" );
	...
	public function write( $filepath, $mimetype ) {
		...
		$vn_jpeg_quality = ( isset( $this->properties["quality"] ) && $this->properties["quality"] )
			? intval( $this->properties["quality"] ) : 75;
		...
		if ( $mimetype == "image/tilepic" ) {
			...
			if ( $this->properties["mimetype"] == "image/tilepic" ) {
				if ( ! ( $h = $this->handle->getLayer( $this->properties["output_layer"]
					? $this->properties["output_layer"] : intval( $this->properties["layers"] / 2.0 ), $mimetype ) )
				) {
