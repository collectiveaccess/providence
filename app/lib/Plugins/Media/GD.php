...
class WLPlugMediaGD Extends BaseMediaPlugin Implements IWLPlugMedia
{
            ...
            switch ($va_info[2]) {
                ...
                case IMAGETYPE_JPEG:
                    if (function_exists('exif_read_data') && !($this->opo_config->get('dont_use_exif_read_data'))) {
                        $this->metadata["EXIF"] = $va_exif = caSanitizeArray(
                            @exif_read_data($filepath, 'EXIF', true, false)
                        );
    ...
    public function write($filepath, $mimetype)
    {
        ...
        $vn_jpeg_quality = (isset($this->properties["quality"]) && $this->properties["quality"]) ? intval(
        ...
        if ($mimetype == "image/tilepic") {
            ...
            if ($this->properties["mimetype"] == "image/tilepic") {
                if (!($h = $this->handle->getLayer(
                    $this->properties["output_layer"] ? $this->properties["output_layer"] : intval(
                        $this->properties["layers"] / 2.0
                    ),
                    $mimetype
                ))) {
