...
class WLPlugMediaVideo Extends BaseMediaPlugin Implements IWLPlugMedia {
	...
	public function checkStatus() {
		...
		if ( ! $this->ops_path_to_ffmeg ) {
			$va_status['errors'][]
				= _t( "Incoming video files will not be transcoded because ffmpeg is not installed." );
		}
	...
	public function read( $filepath, $mimetype = "", $options = null ) {
		...
		if ( ! ( ( $this->opa_media_metadata ) && ( $this->opa_media_metadata["filepath"] == $filepath ) ) ) {
			...
			if ( $vs_mimetype = caMediaInfoGuessFileFormat( $filepath ) ) {
				...
				$o_tc->setTimebase(
					$vn_framerate = trim( str_replace( "fps", "", $va_media_metadata['VIDEO']['Frame rate'] ) ) );
				...
				$this->properties['framerate']       = ( (float) $this->opa_media_metadata['duration'] > 0 )
					? (float) $this->opa_media_metadata['framecount'] / (float) $this->opa_media_metadata['duration']
					: 0;
			}
		}
		...
		if ( ! ( ( isset( $this->opa_media_metadata["error"] ) ) && ( is_array( $this->opa_media_metadata["error"] ) )
		         && ( sizeof( $this->opa_media_metadata["error"] ) > 0 ) )
		) {
			...
			if ( ( $this->opa_media_metadata["mime_type"] == 'video/x-flv' )
			     && is_array( $this->opa_media_metadata['meta'] )
			     && is_array( $this->opa_media_metadata['meta']['onMetaData'] )
			) {
			...
			$this->properties["typename"] = $this->typenames[ $this->properties["mimetype"] ]
				? $this->typenames[ $this->properties["mimetype"] ] : "Unknown";
			...
			if ( $this->opb_mediainfo_available
			     && ( $vn_mediainfo_duration
					= caExtractVideoFileDurationWithMediaInfo( $filepath ) )
			) {
			...
			switch ( $this->properties["mimetype"] ) {
				...
				case 'video/x-dv':
					...
					$vn_bitrate                    = preg_replace( "/[\D]/", '',
						$this->opa_media_metadata['VIDEO']['Bit rate'] );
				...
				case 'video/mp4':
					$this->properties["has_video"] = ( isset( $this->opa_media_metadata["video"]["bitrate"] )
					                                   && ( $this->opa_media_metadata["video"]["bitrate"] ) ? 1 : 0 );
					$this->properties["has_audio"] = ( isset( $this->opa_media_metadata["audio"]["bitrate"] )
					                                   && ( $this->opa_media_metadata["audio"]["bitrate"] ) ? 1 : 0 );
					...
					$this->properties["bandwidth"]
						= array(
						"min" => (int) $this->opa_media_metadata["theora"]['nombitrate']
						         + (int) $this->opa_media_metadata["vorbis"]['bitrate'],
						"max" => (int) $this->opa_media_metadata["theora"]['nombitrate']
						         + (int) $this->opa_media_metadata["vorbis"]['bitrate']
					);
					break;
				case 'video/ogg':
					...
					$this->properties["bandwidth"] = array(
						"min" => $this->opa_media_metadata["bitrate"],
						"max" => $this->opa_media_metadata["bitrate"]
					);
					break;
				case 'application/x-shockwave-flash':
					$this->properties["has_video"] = ( ( $this->opa_media_metadata["header"]["frame_width"] > 0 ) ? 1
						: 0 );
					...
					$this->properties["bandwidth"] = array(
						"min" => $this->opa_media_metadata["filesize"] / $this->opa_media_metadata["playtime_seconds"],
						"max" => $this->opa_media_metadata["filesize"] / $this->opa_media_metadata["playtime_seconds"]
					);
					break;
				case 'video/mpeg':
					$this->properties["has_video"] = ( isset( $this->opa_media_metadata["video"]["bitrate"] )
	...
	public function write( $filepath, $mimetype, $pa_options = null ) {
		...
		} elseif ( ! ( $vn_start_secs = ( $o_tc->parse( $this->opo_app_config->get( 'video_poster_frame_grab_at' ) ) )
			? $o_tc->getSeconds() : 5 )
		) {
			$vn_start_secs = 5;
		}
		...
		switch ( $mimetype ) {
			...
			case 'image/jpeg':
				...
				if ( caMediaPluginFFmpegInstalled()
				     && ( $this->opa_media_metadata["mime_type"] != 'application/x-shockwave-flash' )
				) {
					caExec( caGetExternalApplicationPath( 'ffmpeg' ) . " -i " . caEscapeShellArg( $this->filepath )
					        . " -f image2 -ss " . ( $vn_start_secs )
					        . " -t 0.04 -vf \"scale=w={$vn_preview_width}:h={$vn_preview_height}:force_original_aspect_ratio=decrease,pad={$vn_preview_width}:{$vn_preview_height}:(ow-iw)/2:(oh-ih)/2\"  -y "
					        . caEscapeShellArg( $filepath . "." . $ext ) . ( caIsPOSIX() ? " 2> /dev/null" : "" ),
						$va_output, $vn_return );
					if ( ( $vn_return < 0 ) || ( $vn_return > 1 ) || ( ! @filesize( $filepath . "." . $ext ) ) ) {
						...
						caExec( caGetExternalApplicationPath( 'ffmpeg' ) . " -i " . caEscapeShellArg( $this->filepath )
						        . " -f image2 -ss " . ( $vn_start_secs )
						        . " -t 0.04 -vf \"scale=w={$vn_preview_width}:h={$vn_preview_height}\"  -y "
						        . caEscapeShellArg( $filepath . "." . $ext ) . ( caIsPOSIX() ? " 2> /dev/null" : "" ),
							$va_output, $vn_return );
					}
				...
				$this->properties["typename"] = isset( $this->typenames[ $mimetype ] ) ? $this->typenames[ $mimetype ]
					: $mimetype;
			...
			case 'image/png':
				...
				if ( caMediaPluginFFmpegInstalled()
					...
					caExec( caGetExternalApplicationPath( 'ffmpeg' ) . " -i " . caEscapeShellArg( $this->filepath )
					        . " -vcodec png -ss " . ( $vn_start_secs )
					        . " -t 0.04 -s {$vn_preview_width}x{$vn_preview_height} -y " . caEscapeShellArg( $filepath
					                                                                                         . "."
					                                                                                         . $ext )
			...
			case 'video/x-flv':
				if ( caMediaPluginFFmpegInstalled() ) {
					...
					if ( ( $vn_audio_sample_freq != 44100 ) && ( $vn_audio_sample_freq != 22050 )
					     && ( $vn_audio_sample_freq != 11025 )
					) {
						$vn_audio_sample_freq = 44100;
					}
					caExec( $vs_cmd = caGetExternalApplicationPath( 'ffmpeg' ) . " -i "
					                  . caEscapeShellArg( $this->filepath ) . " -f flv -b "
					                  . intval( $vn_video_bitrate ) . " -ab " . intval( $vn_audio_bitrate ) . " -ar "
					                  . intval( $vn_audio_sample_freq ) . " -y " . caEscapeShellArg( $filepath . "."
					                                                                                 . $ext )
					                  . ( caIsPOSIX() ? " 2> /dev/null" : "" ), $va_output, $vn_return );
			...
			case 'video/x-dv':
				if ( caMediaPluginFFmpegInstalled() ) {
					...
					if ( $vs_ffmpeg_command ) {
						caExec( $vs_cmd .= caGetExternalApplicationPath( 'ffmpeg' ) . " -i "
						                   . caEscapeShellArg( $this->filepath ) . " {$vs_ffmpeg_command} "
						                   . caEscapeShellArg( $filepath . "." . $ext ) . ( caIsPOSIX()
						...
						if ( $vs_vpreset ) {
							...
							caExec( $vs_cmd .= caGetExternalApplicationPath( 'ffmpeg' ) . " -i "
							                   ...
							                   . " -f mp4 -vcodec libx264 -acodec libfaac {$vs_other_params} -vpre {$vs_vpreset} -y "
							                   . caEscapeShellArg( $filepath . "." . $ext ) . ( caIsPOSIX()
									? " 2> /dev/null" : "" ), $va_output, $vn_return );
						} else {
							if ( ! $vb_twopass ) {
								caExec( $vs_cmd .= caGetExternalApplicationPath( 'ffmpeg' ) . " -i "
								                   . caEscapeShellArg( $this->filepath )
								                   . " -f mp4 -vcodec libx264 -acodec libfaac " . join( " ",
										$va_ffmpeg_params ) . " -y " . caEscapeShellArg( $filepath . "." . $ext )
								                   . ( ( caGetOSFamily() == OS_POSIX ) ? " 2> /dev/null" : "" ),
									$va_output, $vn_return );
							} else {
								caExec( $vs_cmd .= caGetExternalApplicationPath( 'ffmpeg' ) . " -i "
								                   . caEscapeShellArg( $this->filepath )
								                   . " -f mp4 -vcodec libx264 -pass 1 -acodec libfaac " . join( " ",
										$va_ffmpeg_params ) . " -y " . caEscapeShellArg( $filepath . "." . $ext )
								                   . ( caIsPOSIX() ? " 2> /dev/null" : "" ), $va_output, $vn_return );
								caExec( $vs_cmd .= caGetExternalApplicationPath( 'ffmpeg' ) . " -i "
								                   . caEscapeShellArg( $this->filepath )
								                   . " -f mp4 -vcodec libx264 -pass 2 -acodec libfaac " . join( " ",
										$va_ffmpeg_params ) . " -y " . caEscapeShellArg( $filepath . "." . $ext )
								                   . ( caIsPOSIX() ? " 2> /dev/null" : "" ), $va_output, $vn_return );
					...
					if ( @filesize( $filepath . "." . $ext ) == 0 ) {
						...
						if ( $vs_vpreset ) {
							$this->postError( 1610,
								_t( "Couldn't convert file to MPEG4 format [%1]; does the ffmpeg preset '%2' exist? (command was %3)",
									$vn_return, $vs_vpreset, $vs_cmd ), "WLPlugVideo->write()" );
						} else {
							$this->postError( 1610,
								_t( "Couldn't convert file to MPEG4 format [%1] (command was %2)", $vn_return,
									$vs_cmd ), "WLPlugVideo->write()" );
						}
			...
			default:
				if ( ( $mimetype != $this->opa_media_metadata["mime_type"] ) ) {
					...
					$this->postError( 1610,
						_t( "Can't convert '%1' to %2", $this->opa_media_metadata["mime_type"], $mimetype ),
						"WLPlugVideo->write()" );
	 ...
	 *        endAtTime
	 *        outputDirectory
	 *        force = ignore setting of "video_preview_generate_frames" app.conf directive and generate previews no
	 *        matter what
	 */
	# This method must be implemented for plug-ins that can output preview frames for videos or pages for documents
	public function &writePreviews( $ps_filepath, $pa_options ) {
		if ( ! (bool) $this->getAppConfig()->get( "video_preview_generate_frames" )
		...
		$vn_preview_width  = ( isset( $pa_options['width'] ) && ( (int) $pa_options['width'] > 0 ) )
			? (int) $pa_options['width'] : 320;
		$vn_preview_height = ( isset( $pa_options['height'] ) && ( (int) $pa_options['height'] > 0 ) )
			? (int) $pa_options['height'] : 320;
		...
		caExec( caGetExternalApplicationPath( 'ffmpeg' ) . " -i " . caEscapeShellArg( $this->filepath )
		        . " -f image2 -r " . $vs_freq
		        . " -ss {$vn_s} -t {$vn_previewed_duration} -vf \"scale=w={$vn_preview_width}:h={$vn_preview_height}:force_original_aspect_ratio=decrease,pad={$vn_preview_width}:{$vn_preview_height}:(ow-iw)/2:(oh-ih)/2\"  -y "
		        . caEscapeShellArg( $vs_output_file ) . ( caIsPOSIX() ? " 2> /dev/null" : "" ), $va_output,
			$vn_return );
		$vn_i     = 1;
		$va_files = array();
		while ( file_exists( $vs_output_file_prefix . sprintf( "%05d", $vn_i ) . '.jpg' ) ) {
			// add frame to list
			$va_files[ '' . sprintf( "%4.2f", ( ( ( $vn_i - 1 ) * $vn_frame_interval ) + $vn_s ) ) . 's' ]
				= $vs_output_file_prefix . sprintf( "%05d", $vn_i ) . '.jpg';

			$vn_i ++;
		}

		if ( ! sizeof( $va_files ) ) {
			$this->postError( 1610, _t( "Couldn't not write video preview frames to tmp directory (%1)", $vs_tmp_dir ),
				"WLPlugVideo->write()" );
		}
	...
	public function writeClip( $ps_filepath, $ps_start, $ps_end, $pa_options = null ) {
		...
		caExec( caGetExternalApplicationPath( 'ffmpeg' ) . " -i " . caEscapeShellArg( $this->filepath )
		        . " -f mp4 -vcodec libx264 -acodec mp3 -t {$vn_duration}  -y -ss {$vn_start} "
		        . caEscapeShellArg( $ps_filepath ) . ( caIsPOSIX() ? " 2> /dev/null" : "" ), $va_output, $vn_return );
		if ( $vn_return != 0 ) {
			@unlink( $ps_filepath );
			$this->postError( 1610,
				_t( "Error extracting clip from %1 to %2: %3", $ps_start, $ps_end, join( "; ", $va_output ) ),
				"WLPlugVideo->writeClip()" );
		...
		switch ( $pa_properties["mimetype"] ) {
			...
			case 'video/quicktime':
				...
				if ( $pa_options["text_only"] ) {
					return "<a href='$ps_url'>" . ( ( $pa_options["text_only"] ) ? $pa_options["text_only"]
							: "View QuickTime" ) . "</a>";
				} else {
					?>
					<table>
			...
			case 'video/mp4':
				...
				<video id="<?php print $vs_id; ?>" class="video-js vjs-default-skin"
					...
					if ( is_array( $va_captions ) ) {
						foreach ( $va_captions as $vn_locale_id => $va_caption_track ) {
							print "<track kind=\"captions\" src=\"" . $va_caption_track['url'] . "\" srclang=\""
							      . $va_caption_track["locale_code"] . "\" label=\"" . $va_caption_track['locale']
							      . "\">\n";
						}
					}
					?>
				...
				<script type="text/javascript">
					...
					if (caUI.mediaPlayerManager) {
						caUI.mediaPlayerManager.register("<?php print $vs_id; ?>", _V_.players["<?php print $vs_id; ?>"], 'VideoJS');
					}
				</script>
				<!-- End VideoJS -->
				<?php
