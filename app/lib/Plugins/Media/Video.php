...
class WLPlugMediaVideo Extends BaseMediaPlugin Implements IWLPlugMedia
{
    ...
    public function read($filepath, $mimetype = "", $options = null)
    {
        ...
        if (!((isset($this->opa_media_metadata["error"])) && (is_array($this->opa_media_metadata["error"])) && (sizeof(
                    $this->opa_media_metadata["error"]
                ) > 0))) {
            ...
            if (($this->opa_media_metadata["mime_type"] == 'video/x-flv') && is_array(
                    $this->opa_media_metadata['meta']
            ...
            if ($this->opb_mediainfo_available && ($vn_mediainfo_duration = caExtractVideoFileDurationWithMediaInfo(
                    $filepath
                ))) {
            ...
            switch ($this->properties["mimetype"]) {
                ...
                case 'video/mp4':
                    ...
                    $this->properties["bandwidth"] = array(
                        "min" => (int)$this->opa_media_metadata["theora"]['nombitrate'] + (int)$this->opa_media_metadata["vorbis"]['bitrate'],
                        "max" => (int)$this->opa_media_metadata["theora"]['nombitrate'] + (int)$this->opa_media_metadata["vorbis"]['bitrate']
                    );
                    break;
                case 'video/ogg':
    ...
    public function write($filepath, $mimetype, $pa_options = null)
    {
        ...
        } elseif (!($vn_start_secs = ($o_tc->parse(
            $this->opo_app_config->get('video_poster_frame_grab_at')
        )) ? $o_tc->getSeconds() : 5)) {
            $vn_start_secs = 5;
        }
        ...
        switch ($mimetype) {
            ...
            case 'image/jpeg':
                ...
                if (caMediaPluginFFmpegInstalled(
                    ...
                    caExec(
                        caGetExternalApplicationPath('ffmpeg') . " -i " . caEscapeShellArg(
                            $this->filepath
                        ) . " -f image2 -ss " . ($vn_start_secs) . " -t 0.04 -vf \"scale=w={$vn_preview_width}:h={$vn_preview_height}:force_original_aspect_ratio=decrease,pad={$vn_preview_width}:{$vn_preview_height}:(ow-iw)/2:(oh-ih)/2\"  -y " . caEscapeShellArg(
                            $filepath . "." . $ext
                    ...
                    if (($vn_return < 0) || ($vn_return > 1) || (!@filesize($filepath . "." . $ext))) {
                        ...
                        caExec(
                            caGetExternalApplicationPath('ffmpeg') . " -i " . caEscapeShellArg(
                                $this->filepath
                            ) . " -f image2 -ss " . ($vn_start_secs) . " -t 0.04 -vf \"scale=w={$vn_preview_width}:h={$vn_preview_height}\"  -y " . caEscapeShellArg(
                                $filepath . "." . $ext
                            ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                            $va_output,
                            $vn_return
            ...
            case 'image/png':
                ...
                if (caMediaPluginFFmpegInstalled(
                    ...
                    caExec(
                        caGetExternalApplicationPath('ffmpeg') . " -i " . caEscapeShellArg(
                            $this->filepath
                        ) . " -vcodec png -ss " . ($vn_start_secs) . " -t 0.04 -s {$vn_preview_width}x{$vn_preview_height} -y " . caEscapeShellArg(
                            $filepath . "." . $ext
                        ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                        $va_output,
                        $vn_return
                    ...
                    if (($vn_return < 0) || ($vn_return > 1) || (!@filesize($filepath . "." . $ext))) {
                        ...
                        caExec(
                            caGetExternalApplicationPath('ffmpeg') . " -i " . caEscapeShellArg(
                                $this->filepath
                            ) . " -vcodec png -ss " . ($vn_start_secs) . " -t 0.04  -vf \"scale=w={$vn_preview_width}:h={$vn_preview_height}:force_original_aspect_ratio=decrease,pad={$vn_preview_width}:{$vn_preview_height}:(ow-iw)/2:(oh-ih)/2\"   -y " . caEscapeShellArg(
                                $filepath . "." . $ext
                            ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                            $va_output,
                            $vn_return
            ...
            case 'video/x-flv':
                if (caMediaPluginFFmpegInstalled()) {
                    ...
                    caExec(
                        $vs_cmd = caGetExternalApplicationPath('ffmpeg') . " -i " . caEscapeShellArg(
                                $this->filepath
                            ) . " -f flv -b " . intval($vn_video_bitrate) . " -ab " . intval(
                                $vn_audio_bitrate
                            ) . " -ar " . intval($vn_audio_sample_freq) . " -y " . caEscapeShellArg(
                                $filepath . "." . $ext
                            ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                        $va_output,
                        $vn_return
                ...
                if (caMediaPluginFFmpegInstalled()) {
                    ...
                    if ($vs_ffmpeg_command) {
                        caExec(
                            $vs_cmd .= caGetExternalApplicationPath('ffmpeg') . " -i " . caEscapeShellArg(
                                    $this->filepath
                                ) . " {$vs_ffmpeg_command} " . caEscapeShellArg($filepath . "." . $ext) . (caIsPOSIX(
                                ) ? " 2> /dev/null" : ""),
                            $va_output,
                            $vn_return
                        );
                        ...
                        if ($vs_vpreset) {
                            ...
                            caExec(
                                $vs_cmd .= caGetExternalApplicationPath('ffmpeg') . " -i " . caEscapeShellArg(
                                        $this->filepath
                                    ) . " -f mp4 -vcodec libx264 -acodec libfaac {$vs_other_params} -vpre {$vs_vpreset} -y " . caEscapeShellArg(
                                        $filepath . "." . $ext
                                    ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                                $va_output,
                                $vn_return
                            ...
                            if (!$vb_twopass) {
                                caExec(
                                    $vs_cmd .= caGetExternalApplicationPath('ffmpeg') . " -i " . caEscapeShellArg(
                                            ...
                                            " ",
                                            $va_ffmpeg_params
                                        ) . " -y " . caEscapeShellArg($filepath . "." . $ext) . ((caGetOSFamily(
                                            ) == OS_POSIX) ? " 2> /dev/null" : ""),
                                    $va_output,
                                    $vn_return
                                );
                                ...
                                caExec(
                                    $vs_cmd .= caGetExternalApplicationPath('ffmpeg') . " -i " . caEscapeShellArg(
                                            ...
                                            " ",
                                            $va_ffmpeg_params
                                        ) . " -y " . caEscapeShellArg($filepath . "." . $ext) . (caIsPOSIX(
                                        ) ? " 2> /dev/null" : ""),
                                    $va_output,
                                    $vn_return
                                );
                    ...
                    if (@filesize($filepath . "." . $ext) == 0) {
                        ...
                        if ($vs_vpreset) {
                            $this->postError(
                                1610,
                                _t(
                                    "Couldn't convert file to MPEG4 format [%1]; does the ffmpeg preset '%2' exist? (command was %3)",
                                    $vn_return,
                                    $vs_vpreset,
                                    $vs_cmd
                                ),
                            ...
                            $this->postError(
                                1610,
                                _t(
                                    "Couldn't convert file to MPEG4 format [%1] (command was %2)",
                                    $vn_return,
                                    $vs_cmd
                                ),
                                "WLPlugVideo->write()"
    ...
    public function &writePreviews($ps_filepath, $pa_options)
    {
        ...
        caExec(
            caGetExternalApplicationPath('ffmpeg') . " -i " . caEscapeShellArg(
                $this->filepath
            ) . " -f image2 -r " . $vs_freq . " -ss {$vn_s} -t {$vn_previewed_duration} -vf \"scale=w={$vn_preview_width}:h={$vn_preview_height}:force_original_aspect_ratio=decrease,pad={$vn_preview_width}:{$vn_preview_height}:(ow-iw)/2:(oh-ih)/2\"  -y " . caEscapeShellArg(
                $vs_output_file
            ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
            $va_output,
            $vn_return
        ...
        while (file_exists($vs_output_file_prefix . sprintf("%05d", $vn_i) . '.jpg')) {
            ...
            $va_files['' . sprintf(
                "%4.2f",
                ((($vn_i - 1) * $vn_frame_interval) + $vn_s)
            ) . 's'] = $vs_output_file_prefix . sprintf("%05d", $vn_i) . '.jpg';
        ...
        if (!sizeof($va_files)) {
            $this->postError(
                1610,
                _t("Couldn't not write video preview frames to tmp directory (%1)", $vs_tmp_dir),
                "WLPlugVideo->write()"
            );
        }
    ...
    public function writeClip($ps_filepath, $ps_start, $ps_end, $pa_options = null)
    {
        ...
        caExec(
            caGetExternalApplicationPath('ffmpeg') . " -i " . caEscapeShellArg(
                $this->filepath
            ) . " -f mp4 -vcodec libx264 -acodec mp3 -t {$vn_duration}  -y -ss {$vn_start} " . caEscapeShellArg(
                $ps_filepath
            ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
            $va_output,
            $vn_return
