...
class WLPlugMediaGraphicsMagick Extends BaseMediaPlugin Implements IWLPlugMedia
{
    ...
    {
        $this->description = _t(
            'Provides image processing and conversion services using GraphicsMagick via exec() calls'
        );
    }
    # ------------------------------------------------
    ...
    public function divineFileFormat($filepath)
    {
        if (!strpos($filepath, ':') || (caGetOSFamily() == OS_WIN32)) {
            ...
            $this->postError(
                1610,
                _t("Filenames with colons (:) are not allowed"),
        ...
        if ($this->handle) {
            if ($property == "tile_size") {
                if (($value < 10) || ($value > 10000)) {
                    $this->postError(
                        1650,
                        _t("Tile size property must be between 10 and 10000"),
                    ...
                    switch ($property) {
                        ...
                        case 'tile_width':
                            if (($value < 10) || ($value > 10000)) {
                                $this->postError(
                                    1650,
                                    _t("Tile width property must be between 10 and 10000"),
                                    "WLPlugImageMagick->set()"
                                );
                                return '';
                            }
                        ...
                        case 'tile_height':
                            if (($value < 10) || ($value > 10000)) {
                                $this->postError(
                                    1650,
                                    _t("Tile height property must be between 10 and 10000"),
                                    "WLPlugImageMagick->set()"
                                );
                                return '';
                            }
                        ...
                        case 'layer_ratio':
                            if (($value < 0.1) || ($value > 10)) {
                                $this->postError(
                                    1650,
                                    _t("Layer ratio property must be between 0.1 and 10"),
                                    "WLPlugImageMagick->set()"
                                );
                                return '';
                            }
                        ...
                        case 'layers':
                            if (($value < 1) || ($value > 25)) {
                                $this->postError(
                                    1650,
                                    _t("Layer property must be between 1 and 25"),
                                    "WLPlugImageMagick->set()"
                                );
                                return '';
                            }
                        ...
                        case 'tile_mimetype':
                            if ((!($this->info["EXPORT"][$value])) && ($value != "image/tilepic")) {
                                $this->postError(
                                    1650,
                                    _t("Tile output type '%1' is invalid", $value),
                                    "WLPlugImageMagick->set()"
                                );
                                return '';
                            }
    ...
    public function write($filepath, $mimetype)
    {
        ...
        if ($mimetype == "image/tilepic") {
            ...
            if (!$this->_graphicsMagickWrite(
                ...
                $this->postError(
                    1610,
                    _t("Could not write file %1", $filepath . "." . $ext),
    ...
    public function &writePreviews($ps_filepath, $pa_options)
    {
        ...
        $va_output = array();
        exec(
            $this->ops_graphicsmagick_path . ' identify -format "%m\n" ' . caEscapeShellArg(
                $this->filepath
        ...
        exec(
            $this->ops_graphicsmagick_path . ' convert ' . caEscapeShellArg(
                $this->filepath
            ) . " +adjoin " . $vs_output_file . (caIsPOSIX() ? " 2> /dev/null" : ""),
            $va_output,
            $vn_return
        );
    ...
    public function joinArchiveContents($pa_files, $pa_options = array())
    {
        ...
        if (sizeof($va_acceptable_files)) {
            exec(
                $this->ops_graphicsmagick_path . " convert " . join(
                    " ",
                    $va_acceptable_files
    ...
    private function _graphicsMagickIdentify($ps_filepath)
    {
        if (caMediaPluginGraphicsMagickInstalled($this->ops_graphicsmagick_path)) {
            exec(
                $this->ops_graphicsmagick_path . ' identify -format "%m;" ' . caEscapeShellArg(
                    $ps_filepath . '[0]'
                ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                $va_output,
                $vn_return
    ...
    private function _graphicsMagickGetMetadata($ps_filepath)
    {
        ...
        if (!isset($va_metadata['EXIF']) || !is_array($va_metadata['EXIF'])) {
            exec(
                $this->ops_graphicsmagick_path . ' identify -format "%[EXIF:*]" ' . caEscapeShellArg(
                    $ps_filepath
                ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                $va_output,
                $vn_return
        ...
        $vs_iptc_file .= '.iptc';
        exec(
            $this->ops_graphicsmagick_path . " convert " . caEscapeShellArg($ps_filepath) . " " . caEscapeShellArg(
                $vs_iptc_file
            ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
            $va_output,
            $vn_return
        ...
        /* DPX metadata */
        exec(
            $this->ops_graphicsmagick_path . " identify -format '%[DPX:*]' " . caEscapeShellArg(
                $ps_filepath
            ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
            $va_output,
            $vn_return
    ...
    private function _graphicsMagickRead($ps_filepath)
    {
        if (caMediaPluginGraphicsMagickInstalled($this->ops_graphicsmagick_path)) {
            exec(
                $this->ops_graphicsmagick_path . ' identify -format "%m;%w;%h;%q;%x;%y\n" ' . caEscapeShellArg(
                    $ps_filepath
                ) . (caIsPOSIX() ? " 2> /dev/null" : ""),
                $va_output,
                $vn_return
    ...
    private function _graphicsMagickWrite($pa_handle, $ps_filepath, $ps_mimetype, $pn_quality = null)
    {
        if (caMediaPluginGraphicsMagickInstalled($this->ops_graphicsmagick_path)) {
                ...
                switch ($va_op['op']) {
                    ...
                    case 'annotation':
                        $vs_op = '-gravity ' . $va_op['position'] . ' -fill ' . str_replace(
                                ...
                                '\\#',
                                $va_op['color']
                            ) . ' -pointsize ' . $va_op['size'] . ' -draw "text ' . $va_op['inset'] . ',' . $va_op['inset'] . ' \'' . $va_op['text'] . '\'"';
            ...
            if (is_array($va_ops['composite']) && sizeof($va_ops['composite'])) {
                exec(
                    $this->ops_graphicsmagick_path . ' composite ' . join(
                        ' ',
                        $va_ops['composite']
                    ) . ' ' . caEscapeShellArg($vs_input_file . '[0]') . ' ' . caEscapeShellArg(
                        $ps_filepath
                    ) . (caIsPOSIX() ? " 2> /dev/null" : "")
                );
            }
