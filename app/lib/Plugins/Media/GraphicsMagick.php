...
class WLPlugMediaGraphicsMagick Extends BaseMediaPlugin Implements IWLPlugMedia {
	...
	public function divineFileFormat($filepath) {
		if(!strpos($filepath, ':') || (caGetOSFamily() == OS_WIN32)) {
			...
			if ($vs_mimetype = $this->_graphicsMagickIdentify($filepath)) { return $vs_mimetype; }
		} else {
	...
	public function compose(array $images, string $filepath, int $width, int $height) :  bool {
		...
		foreach($images as $image) {
			$image_cmds[] = "-page +{$image['x']}+{$image['y']} {$image['path']}";
		}
		
		caExec( join( ' ', [
			$this->ops_graphicsmagick_path, 'convert',
			caEscapeShellArg("-size {$width}x{$height}"),
			'xc:white',
			caEscapeShellArg(join(' ', $image_cmds)),
			'-mosaic',
			caEscapeShellArg($filepath) . ( caIsPOSIX() ? ' 2> /dev/null' : '' )
		]), $output, $return);
