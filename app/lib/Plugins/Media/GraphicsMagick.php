...
class WLPlugMediaGraphicsMagick Extends BaseMediaPlugin Implements IWLPlugMedia {
	...
	public function __construct() {
		$this->description
			= _t( 'Provides image processing and conversion services using GraphicsMagick via exec() calls' );
	}
	# ------------------------------------------------
	...
	public function divineFileFormat( $filepath ) {
		if ( ! strpos( $filepath, ':' ) || ( caGetOSFamily() == OS_WIN32 ) ) {
			...
			$this->postError( 1610, _t( "Filenames with colons (:) are not allowed" ),
				"WLPlugImageMagick->divineFileFormat()" );
		...
		if ( $this->handle ) {
			if ( $property == "tile_size" ) {
				if ( ( $value < 10 ) || ( $value > 10000 ) ) {
					$this->postError( 1650, _t( "Tile size property must be between 10 and 10000" ),
						"WLPlugImageMagick->set()" );
				...
				if ( $this->info["PROPERTIES"][ $property ] ) {
					switch ( $property ) {
						...
						case 'tile_width':
							if ( ( $value < 10 ) || ( $value > 10000 ) ) {
								$this->postError( 1650, _t( "Tile width property must be between 10 and 10000" ),
									"WLPlugImageMagick->set()" );
						...
						case 'tile_height':
							if ( ( $value < 10 ) || ( $value > 10000 ) ) {
								$this->postError( 1650, _t( "Tile height property must be between 10 and 10000" ),
									"WLPlugImageMagick->set()" );
						...
						case 'layer_ratio':
							if ( ( $value < 0.1 ) || ( $value > 10 ) ) {
								$this->postError( 1650, _t( "Layer ratio property must be between 0.1 and 10" ),
									"WLPlugImageMagick->set()" );
						...
						case 'layers':
							if ( ( $value < 1 ) || ( $value > 25 ) ) {
								$this->postError( 1650, _t( "Layer property must be between 1 and 25" ),
									"WLPlugImageMagick->set()" );
						...
						case 'tile_mimetype':
							if ( ( ! ( $this->info["EXPORT"][ $value ] ) ) && ( $value != "image/tilepic" ) ) {
								$this->postError( 1650, _t( "Tile output type '%1' is invalid", $value ),
									"WLPlugImageMagick->set()" );
			...
			case 'SCALE':
				...
				switch ( $parameters["mode"] ) {
					...
					case "bounding_box":
						$scale_factor_w = $w / $cw;
						$scale_factor_h = $h / $ch;
						$w              = $cw * ( ( $scale_factor_w < $scale_factor_h ) ? $scale_factor_w
								: $scale_factor_h );
						$h              = $ch * ( ( $scale_factor_w < $scale_factor_h ) ? $scale_factor_w
								: $scale_factor_h );
						break;
					# ----------------
					case "fill_box":
	...
	public function write( $filepath, $mimetype ) {
		...
		if ( $mimetype == "image/tilepic" ) {
			...
			if ( ! $this->_graphicsMagickWrite( $this->handle, $filepath . "." . $ext, $mimetype,
				$this->properties["quality"] )
			) {
				$this->postError( 1610, _t( "Could not write file %1", $filepath . "." . $ext ),
					"WLPlugImageMagick->write()" );
	...
	public function &writePreviews( $ps_filepath, $pa_options ) {
		...
		caExec( $this->ops_graphicsmagick_path . ' convert ' . caEscapeShellArg( $this->filepath ) . " +adjoin "
		        . $vs_output_file . ( caIsPOSIX() ? " 2> /dev/null" : "" ), $va_output, $vn_return );
	...
	public function joinArchiveContents( $pa_files, $pa_options = array() ) {
		...
		if ( sizeof( $va_acceptable_files ) ) {
			caExec( $this->ops_graphicsmagick_path . " convert " . join( " ", $va_acceptable_files ) . " "
			        . $vs_archive_original . ( caIsPOSIX() ? " 2> /dev/null" : "" ), $va_output, $vn_return );
			if ( $vn_return === 0 ) {
	...
	private function _graphicsMagickIdentify( $ps_filepath ) {
		if ( $this->ops_graphicsmagick_path ) {
			caExec( $this->ops_graphicsmagick_path . ' identify -format "%m;" ' . caEscapeShellArg( $ps_filepath
			                                                                                        . '[0]' )
			        . ( caIsPOSIX() ? " 2> /dev/null" : "" ), $va_output, $vn_return );
	...
	private function _graphicsMagickGetMetadata( $ps_filepath, $options = null ) {
		...
		if ( ! isset( $va_metadata['EXIF'] ) || ! is_array( $va_metadata['EXIF'] ) ) {
			caExec( $this->ops_graphicsmagick_path . ' identify -format "%[EXIF:*]" ' . caEscapeShellArg( $ps_filepath )
			        . ( caIsPOSIX() ? " 2> /dev/null" : "" ), $va_output, $vn_return );
		...
		if ( array_key_exists( "FILE", $va_metadata['EXIF'] )
		     && ( $f = caGetOption( 'original_filename', $options, null ) )
		) {
		...
		$vs_iptc_file .= '.iptc';
		caExec( $this->ops_graphicsmagick_path . " convert " . caEscapeShellArg( $ps_filepath ) . " "
		        . caEscapeShellArg( $vs_iptc_file ) . ( caIsPOSIX() ? " 2> /dev/null" : "" ), $va_output, $vn_return );
		...
		/* DPX metadata */
		caExec( $this->ops_graphicsmagick_path . " identify -format '%[DPX:*]' " . caEscapeShellArg( $ps_filepath )
		        . ( caIsPOSIX() ? " 2> /dev/null" : "" ), $va_output, $vn_return );
		if ( $va_output[0] ) {
	...
	private function _graphicsMagickRead( $ps_filepath ) {
		if ( $this->ops_graphicsmagick_path ) {
			...
			return array(
				...
				'magick'     => $va_tmp[0],
				'width'      => in_array( $this->properties["orientation_rotate"], array( 90, - 90 ) ) ? $va_tmp[2]
					: $va_tmp[1],
				'height'     => in_array( $this->properties["orientation_rotate"], array( 90, - 90 ) ) ? $va_tmp[1]
					: $va_tmp[2],
				'depth'      => $va_tmp[3],
				'resolution' => array(
					'x' => $va_tmp[4],
				...
				'ops'        => $this->properties["orientation_rotate"] ? array( 0 => array( 'op' => 'strip' ) )
					: array(),
	...
	private function _graphicsMagickWrite( $pa_handle, $ps_filepath, $ps_mimetype, $pn_quality = null ) {
		if ( $this->ops_graphicsmagick_path ) {
			...
			foreach ( $pa_handle['ops'] as $va_op ) {
				switch ( $va_op['op'] ) {
					case 'strip':
						...
						break;
					case 'annotation':
						$vs_op = '-gravity ' . $va_op['position'] . ' -fill ' . str_replace( '#', '\\#',
								$va_op['color'] ) . ' -pointsize ' . $va_op['size'] . ' -draw "text ' . $va_op['inset']
						         . ',' . $va_op['inset'] . ' \'' . $va_op['text'] . '\'"';
						...
						break;
					case 'watermark':
						$vs_op                 = "-dissolve " . ( $va_op['opacity'] * 100 ) . " -gravity "
						                         . $va_op['position'] . " "
						                         . $va_op['watermark_image']; //"  -geometry ".$va_op['watermark_width']."x".$va_op['watermark_height']; [Seems to be interpreted as scaling the image being composited on as of at least v6.5.9; so we don't scale watermarks in ImageMagick... we just use the native size]
					...
					case 'crop':
						...
						$va_ops['convert'][] = '-crop ' . $va_op['width'] . 'x' . $va_op['height'] . '+' . $va_op['x']
						                       . '+' . $va_op['y'];
						break;
					case 'rotate':
			...
			if ( isset( $this->properties["orientation_rotate"] )
			     && ( $this->properties["orientation_rotate"] != 0 )
			) {
			...
			if ( is_array( $va_ops['convert'] ) && sizeof( $va_ops['convert'] ) ) {
				...
				caExec( $this->ops_graphicsmagick_path . ' convert ' . caEscapeShellArg( $vs_input_file . '[0]' ) . ' '
				        . join( ' ', $va_ops['convert'] ) . ' ' . caEscapeShellArg( $ps_filepath ) . ( caIsPOSIX()
						? " 2> /dev/null" : "" ) );
				$vs_input_file = $ps_filepath;
			}
			if ( is_array( $va_ops['composite'] ) && sizeof( $va_ops['composite'] ) ) {
				caExec( $this->ops_graphicsmagick_path . ' composite ' . join( ' ', $va_ops['composite'] ) . ' '
				        . caEscapeShellArg( $vs_input_file . '[0]' ) . ' ' . caEscapeShellArg( $ps_filepath )
