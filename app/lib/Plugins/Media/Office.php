...
class WLPlugMediaOffice Extends BaseMediaPlugin Implements IWLPlugMedia {
	...
	public function checkStatus() {
		...
		if ( ! ( $this->ops_libreoffice_path ) ) {
			$va_status['warnings'][]
				= _t( "LibreOffice cannot be found: conversion to PDF and generation of page previews will not be performed; you can obtain LibreOffice at http://www.libreoffice.org/" );
		}
	...
	private function isWord972000doc( $ps_filepath ) {
		if ( $r_fp = @fopen( $ps_filepath, "r" ) ) {
			...
			if (
				( ord( $ps_sig{0} ) == 0xD0 ) && ( ord( $ps_sig{1} ) == 0xCF ) && ( ord( $ps_sig{2} ) == 0x11 )
				&& ( ord( $ps_sig{3} ) == 0xE0 )
				&& ( ord( $ps_sig{4} ) == 0xA1 )
				&& ( ord( $ps_sig{5} ) == 0xB1 )
				&& ( ord( $ps_sig{6} ) == 0x1A )
				...
				while ( ! feof( $r_fp ) ) {
					...
					if ( preg_match( "!W.{1}o.{1}r.{1}d.{1}D.{1}o.{1}c.{1}u.{1}m.{1}e.{1}n.{1}t!", $buffer )
					     !== false
					) {
						return true;
					}
	 ...
	 *
	 * @return string WORD if the document is a Word doc, EXCEL if the document is an Excel doc, PPT if it is a
	 *                PowerPoint doc or boolean false if it's not a valid Word or Excel XML (OpenOffice) file
	 */
	private function isWordExcelorPPTdoc( $ps_filepath ) {
		...
		if ( in_array( pathinfo( strtolower( $ps_filepath ), PATHINFO_EXTENSION ), [ 'ppt', 'pptx' ] ) ) {
			$va_ppt_types
				= [
				'PowerPoint2007' => 'application/vnd.openxmlformats-officedocument.presentationml.presentation',
				'PowerPoint97'   => 'application/vnd.ms-powerpoint'
			];
		...
		// 2007+ .docx files
		if ( in_array( pathinfo( strtolower( $ps_filepath ), PATHINFO_EXTENSION ),
			[ 'doc', 'docx' ] )
		) {    // PhpWord often will identify Excel docs as Word (and \PhpOffice\PhpSpreadsheet\Spreadsheet will identify Word docs as Excel...) so we test file extensions here
			// Check Word
		...
		if ( in_array( pathinfo( strtolower( $ps_filepath ), PATHINFO_EXTENSION ), [ 'xls', 'xlsx' ] ) ) {
			$va_excel_types = [
				...
				'Excel2003XML' => 'application/vnd.ms-excel'
			];
	...
	public function read( $ps_filepath, $mimetype = "", $options = null ) {
		if ( is_array( $this->handle ) && ( $this->handle["filepath"] == $ps_filepath ) ) {
			...
			if ( ! ( $this->divineFileFormat( $ps_filepath ) ) ) {
				$this->postError( 1650,
					_t( "File %1 is not a Microsoft Word, Excel or PowerPoint document", $ps_filepath ),
					"WLPlugMediaOffice->read()" );
				$this->handle   = "";
				$this->filepath = "";
	 ...
	 *                          when preview can't be generated. Default is false – to use default icons.
	 *
	 */
	public function write( $ps_filepath, $ps_mimetype, $pa_options = null ) {
		...
		$vb_dont_allow_default_icons = ( isset( $pa_options['dontUseDefaultIcons'] )
		                                 && $pa_options['dontUseDefaultIcons'] ) ? true : false;
		...
		if ( $ps_mimetype == "application/msword" ) {
			if ( ! copy( $this->filepath, $ps_filepath . ".doc" ) ) {
				$this->postError( 1610, _t( "Couldn't write file to '%1'", $ps_filepath ),
					"WLPlugMediaOffice->write()" );
			...
			if ( ! isset( WLPlugMediaOffice::$s_pdf_conv_cache[ $this->filepath ] ) && $this->ops_libreoffice_path ) {
				...
				caExec( $this->ops_libreoffice_path
				        . " --headless --convert-to pdf:writer_pdf_Export \"-env:UserInstallation=file:///tmp/LibreOffice_Conversion_${USER}\" "
				        . caEscapeShellArg( $this->filepath ) . "  --outdir " . caEscapeShellArg( $vs_tmp_dir_path )
				        . ( caIsPOSIX() ? " 2>&1" : "" ), $va_output, $vn_return );
				caExec( $this->ops_libreoffice_path
				        . " --headless --convert-to html:HTML \"-env:UserInstallation=file:///tmp/LibreOffice_Conversion_${USER}\" "
				        . caEscapeShellArg( $this->filepath ) . "  --outdir " . caEscapeShellArg( $vs_tmp_dir_path )
				        . ( caIsPOSIX() ? " 2>&1" : "" ), $va_output, $vn_return );
				...
				$this->handle['content']
					           =
				$this->ohandle['content'] = file_exists( "{$vs_tmp_dir_path}/" . join( ".", $va_out_file ) . ".html" )
					? strip_tags( file_get_contents( "{$vs_tmp_dir_path}/" . join( ".", $va_out_file ) . ".html" ) )
					: '';
				$va_out_file[] = 'pdf';
				...
				WLPlugMediaOffice::$s_pdf_conv_cache[ $this->filepath ] = "{$vs_tmp_dir_path}/" . join( ".",
						$va_out_file );
				$o_media                                                = new Media();
