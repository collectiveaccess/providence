...
class WLPlugMediaOffice Extends BaseMediaPlugin Implements IWLPlugMedia
{
     ...
     */
    static public $s_pdf_conv_cache = array();
    ...
    public function checkStatus()
    {
        ...
        if (!($this->ops_libreoffice_path)) {
            $va_status['warnings'][] = _t(
                "LibreOffice cannot be found: conversion to PDF and generation of page previews will not be performed; you can obtain LibreOffice at http://www.libreoffice.org/"
            );
        }
    ...
    private function isWordExcelorPPTdoc($ps_filepath)
    {
        ...
        // 2007+ .docx files
        if (in_array(
            pathinfo(strtolower($ps_filepath), PATHINFO_EXTENSION),
            ['doc', 'docx']
    ...
    public function write($ps_filepath, $ps_mimetype, $pa_options = null)
    {
        ...
        if ($ps_mimetype == "application/msword") {
            ...
            if (!isset(WLPlugMediaOffice::$s_pdf_conv_cache[$this->filepath]) && $this->ops_libreoffice_path) {
                ...
                caExec(
                    $this->ops_libreoffice_path . " --headless --convert-to pdf:writer_pdf_Export \"-env:UserInstallation=file:///tmp/LibreOffice_Conversion_${USER}\" " . caEscapeShellArg(
                        $this->filepath
                    ) . "  --outdir " . caEscapeShellArg($vs_tmp_dir_path) . (caIsPOSIX() ? " 2>&1" : ""),
                    $va_output,
                    $vn_return
                );
                caExec(
                    $this->ops_libreoffice_path . " --headless --convert-to html:HTML \"-env:UserInstallation=file:///tmp/LibreOffice_Conversion_${USER}\" " . caEscapeShellArg(
                        $this->filepath
                    ) . "  --outdir " . caEscapeShellArg($vs_tmp_dir_path) . (caIsPOSIX() ? " 2>&1" : ""),
                    $va_output,
                    $vn_return
                ...
                $this->handle['content'] = $this->ohandle['content'] = file_exists(
                    "{$vs_tmp_dir_path}/" . join(".", $va_out_file) . ".html"
