				...
				switch ( $this->backend ) {
					case LIBRARY_GD:
						if ( $vs_mimetype = $this->test( $ps_orig_filepath ) ) {
							...
							if ( ! imagecopyresampled( $rdest, $rsource, 0, 0, 0, 0, $vn_target_width,
								$vn_target_height, $vn_width, $vn_height )
							) {
								return false;
							}
					...
					default:
						...
						if ( MagickReadImage( $handle, $ps_orig_filepath ) ) {
							...
							if ( ! MagickResizeImage( $handle, $vn_target_width, $vn_target_height, MW_CubicFilter,
								1 )
							) {
								return false;
							}
