...
class WLPlugPDFRendererPhantomJS Extends BasePDFRendererPlugIn Implements IWLPlugPDFRenderer
{
    ...
    public function render($ps_content, $pa_options = null)
    {
        ...
        exec(
            $x = $this->ops_phantom_path . " " . __CA_LIB_DIR__ . "/Print/phantomjs/rasterise.js " . caEscapeShellArg(
                    $vs_content_path
                ) . " " . caEscapeShellArg(
                    $vs_output_path
                ) . " {$this->ops_page_size} {$this->ops_page_orientation}  {$this->ops_margin_right} {$this->ops_margin_bottom} {$this->ops_margin_left}",
            $va_output,
            $vn_return
        );
