...
class GoogleDrive Extends BaseMediaUrlPlugin {
	 ...
	 *                        format = Preferred format to grab GoogleDrive resource in, if possible. May be ignored is
	 *                        format is not possible for resource. Valid values are pdf, xlsx, docx. [Default is pdf]
	 *
	...
	public function parse( string $url, array $options = null ) {
		...
		$filename = preg_match( '!<meta property="og:title" content="([^"]+)">!', $content, $m )
			? preg_replace( '![^A-Za-z0-9\.\-_]+!', '_', $m[1] ) : 'document';
	...
	/**
	 * Attempt to fetch content from a GoogleDrive URL, transforming GoogleDoc content to a PDF or other user-specified
	 * format (xlsx or docx).
	 *
	 * @param string $url
	 * @param array  $options Options include:
	 ...
	 *                        used as the extension. [Default is null] format = Preferred format to grab GoogleDrive
	 *                        resource in, if possible. May be ignored is format is not possible for resource. Valid
	 *                        values are pdf, xlsx, docx. [Default is pdf] returnAsString = Return fetched content as
	...
	public function fetch( string $url, array $options = null ) {
		if ( $p = $this->parse( $url, $options ) ) {
			$format
				= $p['format']; //caGetOption('format', $options, null, ['validValues' => [null, 'pdf', 'xlsx', 'docx']]);
