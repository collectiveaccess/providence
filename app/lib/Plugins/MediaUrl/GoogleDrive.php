...
class GoogleDrive Extends BaseMediaUrlPlugin
{
    ...
    public function parse(string $url, array $options = null)
    {
        ...
        if (!isUrl($url_stub) || !preg_match(
                '!^https://(docs|drive).google.com/(spreadsheets|file|document)/d/!',
                $url_stub,
                $m
            )) {
            return false;
        ...
        $filename = preg_match('!<meta property="og:title" content="([^"]+)">!', $content, $m) ? preg_replace(
            '![^A-Za-z0-9\.\-_]+!',
            '_',
            $m[1]
        ) : 'document';
