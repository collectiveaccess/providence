...
class InternetArchive Extends BaseMediaUrlPlugin {
	...
	public function parse( string $url, array $options = null ) {
		...
		if ( ! preg_match( '!\.archive\.org$!', $parsed_url['host'] )
		     && ! preg_match( '!^archive\.org$!', $parsed_url['host'] )
		) {
			return false;
		}
		...
		if ( preg_match( '!^/details/[A-Za-z0-9_\-\.]+!', $parsed_url['path'], $m ) ) {
			...
			if ( preg_match( '!<meta property="og:video" content="([^"]+)">!', $content, $m ) ) {
				return [
					...
					'originalFilename' => pathInfo( $m[1], PATHINFO_BASENAME )
				];
			}
		}
	...
	/**
	 * Attempt to fetch content from a InternetArchive URL, transforming content to a PDF or other user-specified
	 * format (xlsx or docx).
	 *
	 * @param string $url
	 * @param array  $options Options include:
	 *                        filename = File name to use for fetched file. If omitted a random name is generated.
	 *                        [Default is null] extension = Extension to use for fetched file. If omitted ".bin" is
	 *                        used as the extension. [Default is null] returnAsString = Return fetched content as
	 *                        string rather than in a file. [Default is false]
	 *
	 * @return bool|array|string False if url is not valid, array with path to file with content and format if
	 *                           successful, string with content if returnAsString option is set.
