...
class WLPlugGeographicMapGoogleMaps Extends BaseGeographicMapPlugIn Implements IWLPlugGeographicMap
{
    ...
    public function render($ps_format, $pa_options = null)
    {
        ...
        $vs_path_color = (isset($pa_options['pathColor'])) ? $pa_options['pathColor'] : $this->opo_config->get(
            'google_maps_path_color'
        );
        ...
        $vs_type = (isset($pa_options['mapType'])) ? strtoupper($pa_options['mapType']) : strtoupper(
            $this->opo_config->get('google_maps_default_type')
        );
        ...
        switch (strtoupper($ps_format)) {
            ...
            case 'GIF':
                ...
                foreach ($va_map_items as $o_map_item) {
                    ...
                    if (sizeof($va_coords) > 1) {
                        ...
                        $va_markers[] = "markers=" . urlencode(
                                "color:red|label:" . $o_map_item->getLabel(
                                ) . "|" . $va_coord['latitude'] . ',' . $va_coord['longitude']
                            );
                ...
                return "<img src='http://maps.googleapis.com/maps/api/staticmap?format={$vs_format}&maptype=" . strtolower(
                        $vs_type
                    ) . "&zoom={$vn_zoom_level}&sensor=false&size={$vn_width}x{$vn_height}&" . join(
                        "&",
                        array_merge(
                            $va_markers,
                            $va_paths
            ...
            default:
                ...
                if (isset($pa_options['showNavigationControls'])) {
                    ...
                    $vb_show_navigation_control = $this->opo_config->get(
                        'google_maps_show_navigation_controls'
                    ) ? 'true' : 'false';
                }
                if (isset($pa_options['showScaleControls'])) {
                    $vb_show_scale_control = $pa_options['showScaleControls'] ? 'true' : 'false';
                } else {
                    $vb_show_scale_control = $this->opo_config->get(
                        'google_maps_show_scale_controls'
                    ) ? 'true' : 'false';
                }
                if (isset($pa_options['showMapTypeControls'])) {
                    $vb_show_map_type_control = $pa_options['showMapTypeControls'] ? 'true' : 'false';
                } else {
                    $vb_show_map_type_control = $this->opo_config->get(
                        'google_maps_show_map_type_controls'
                    ) ? 'true' : 'false';
                }
...
";
                ...
                foreach ($va_map_items as $o_map_item) {
                    ...
                    if (sizeof($va_coords) > 1) {
                        ...
                        if ($vb_obscure) {
                            ...
                            $va_locs[$va_coord['latitude']][$va_coord['longitude']][] = array(
                                'label' => $o_map_item->getLabel(),
                                'content' => $o_map_item->getContent(),
                                'ajaxContentUrl' => $o_map_item->getAjaxContentUrl(),
                                'ajaxContentID' => $o_map_item->getAjaxContentID()
                ...
                foreach ($va_locs as $vn_latitude => $va_locs_by_longitude) {
                    foreach ($va_locs_by_longitude as $vn_longitude => $va_marker_content_items) {
                        ...
                        foreach ($va_marker_content_items as $va_marker_content_item) {
                            ...
                            $va_buf[md5(
                                $va_marker_content_item['content']
                        ...
                        if ($vs_balloon_view && isset($pa_options['request']) && $pa_options['request']) {
                            $o_view = new View(
                                $pa_options['request'],
                                (isset($pa_options['viewPath']) && $pa_options['viewPath']) ? $pa_options['viewPath'] : $pa_options['request']->getViewsDirectoryPath(
                                )
                            );
                        ...
                        if ($vb_circle) {
                            $vs_buf .= "	caMap_{$vs_id}_markers.push(caMap_{$vs_id}.makeCircle(" . $vn_latitude . ", " . $vn_longitude . ", '" . preg_replace(
                                    ...
                                    "![\n\r]+!",
                                    " ",
                                    addslashes(join($vs_delimiter, $va_buf))
                                ) . "', '" . preg_replace(
                                    "![\n\r]+!",
                                    " ",
                                    ($vs_ajax_content_url ? addslashes(
                                        $vs_ajax_content_url . "/id/" . join(';', $va_ajax_ids)
                                    ) : '')
                            ...
                            $va_extents = array(
                                "north" => number_format(
                                    ($va_extents['north'] + $vn_lat_offset),
                                    7,
                                    ".",
                                    ""
                                ),
                            ...
                            if (file_exists(__CA_THEME_DIR__ . "/assets/pawtucket/graphics/google_map_marker.png")) {
                                $vs_buf .= "	caMap_{$vs_id}_markers.push(caMap_{$vs_id}.makeMarker(" . $vn_latitude . ", " . $vn_longitude . ", '" . preg_replace(
                                        ...
                                        "![\n\r]+!",
                                        " ",
                                        addslashes($vs_balloon_content)
                                    ) . "', '" . preg_replace(
                                        "![\n\r]+!",
                                        " ",
                                        ($vs_ajax_content_url ? addslashes(
                                            $vs_ajax_content_url . "/_ajax/1/id/" . join(';', $va_ajax_ids)
                                        ) : '')
                                    ) . "', {icon: '" . __CA_THEME_URL__ . "/assets/pawtucket/graphics/google_map_marker.png'}));\n";
                            } else {
                                $vs_buf .= "	caMap_{$vs_id}_markers.push(caMap_{$vs_id}.makeMarker(" . $vn_latitude . ", " . $vn_longitude . ", '" . preg_replace(
                                        "![\n\r]+!",
                                        " ",
                ...
                foreach ($va_paths as $vn_i => $va_path) {
                    $vs_buf .= "caMap_{$vs_id}.makePath([" . join(',', $va_path['pathJS']) . "], '" . preg_replace(
                            "![\n\r]+!",
                            " ",
                            addslashes($va_path['label'])
                        ) . "','" . preg_replace(
                            "![\n\r]+!",
                            " ",
                            addslashes($va_path['content'])
    ...
    public function getAttributeBundleHTML($pa_element_info, $pa_options = null)
    {
        ...
        if (!in_array(
            $vs_map_type = $o_config->get('google_maps_default_type'),
        ...
        $vs_element .= '<input type="text" class="mapSearchText" name="searchtext"  id="{fieldNamePrefix}' . $pa_element_info['element_id'] . '_{n}" size="60" value="' . _t(
                'Search for geographic location'
            ) . '..." autocomplete="off"/>';
        ...
        $vs_element .= _t(
                "Select KML or KMZ file"
            ) . ': <input type="file" name="{fieldNamePrefix}' . $pa_element_info['element_id'] . '_{n}"/><a href="#" class="button">' . _t(
                'Use map'
            ) . ' &rsaquo;</a>';
        $vs_element .= '</div>';
        ...
        $vs_element .= "jQuery(document).ready(function() {
				var mID_{n} = " . $pa_element_info['element_id'] . ";
										...
										var mapOptions = {
											zoom: 12,
											mapTypeControl: " . ((bool)$o_config->get(
                'google_maps_show_map_type_controls'
            ) ? 'true' : 'false') . ",
											mapTypeControlOptions: {
												style: google.maps.MapTypeControlStyle.DEFAULT
											},
											navigationControl: " . ((bool)$o_config->get(
                'google_maps_show_navigation_controls'
            ) ? 'true' : 'false') . ",
											navigationControlOptions: {
												style: google.maps.NavigationControlStyle.DEFAULT
											},
											scaleControl: " . ((bool)$o_config->get(
                'google_maps_show_scale_controls'
            ) ? 'true' : 'false') . ",
											scaleControlOptions: {
