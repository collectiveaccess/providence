...
class WLPlugGeographicMapLeaflet Extends BaseGeographicMapPlugIn Implements IWLPlugGeographicMap
{
    ...
    public function render($ps_format, $pa_options = null)
    {
        ...
        $vb_show_scale_controls = (bool)caGetOption(
            'showScaleControls',
            $pa_options,
        ...
        if (($vn_path_weight = caGetOption(
                'pathWeight',
                $pa_options,
                $this->opo_config->get('leaflet_maps_path_weight')
            )) < 1) {
            $vn_path_weight = 1;
        }
        if (($vn_path_opacity = caGetOption(
                'pathOpacity',
                $pa_options,
                $this->opo_config->get('leaflet_maps_path_opacity')
            )) < 0) {
            $vn_path_opacity = 1;
        }

        if (!($vs_fill_color = caGetOption(
            'fillColor',
            $pa_options,
        ...
        if (($vn_fill_opacity = caGetOption(
                'fillOpacity',
                $pa_options,
                $this->opo_config->get('leaflet_maps_fill_opacity')
            )) < 0) {
            $vn_fill_opacity = 1;
        }
...
<script type='text/javascript'>
		...
		if (bounds.isValid()) { map.fitBounds(bounds)" . ((strlen(
                            $vn_zoom_level
                        ) && !$we_set_zoom) ? ".setZoom({$vn_zoom_level})" : "") . "; }
</script>\n";
                break;
            # ---------------------------------
        }
