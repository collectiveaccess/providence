...
abstract class BaseGeographicMapPlugIn Extends WLPlug {
	...
	public function getExtents() {
		if (
			...
			||
			! isset( $this->opn_map_extent_south )
			|| is_null( $this->opn_map_extent_south )
			||
			! isset( $this->opn_map_extent_east )
			|| is_null( $this->opn_map_extent_east )
			||
			! isset( $this->opn_map_extent_west )
			|| is_null( $this->opn_map_extent_west )
		) {
		...
		return array(
			'north' => $this->opn_map_extent_north,
			'south' => $this->opn_map_extent_south,
			'east'  => $this->opn_map_extent_east,
			'west'  => $this->opn_map_extent_west
		);
	}
	# ------------------------------------------------
	...
	/**
	 * Change map extents to fit all mapped items. This is essentially the same as "zooming out" the map to include all
	 * items.
	 *
