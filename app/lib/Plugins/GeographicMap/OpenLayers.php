...
class WLPlugGeographicMapOpenLayers Extends BaseGeographicMapPlugIn Implements IWLPlugGeographicMap {
	 ...
	 *                    baseLayer = Tiles to user for base layer. Should be full class name with optional constructor
	 *                    string (Eg. OpenLayers.Layer.Stamen('toner')); default is OpenLayers.Layer.OSM() pointRadius
	 *                    = Radius, in pixels, of plotted points. Default is 5 pixels fillColor = Color (in hex format
	 *                    with leading "#") to fill regions and points with strokeWidth = Width of plotted paths, in
	 *                    pixels. Default is 2 strokeColor = Color of plotted paths, in hex format with leading "#"
	 ...
	 *                    layerSwitcherControl = Show layer switcher controls? [Default is false] delimiter = HTML to
	 *                    place between items displayed in info overlays for plotted items. Default is an HTML break
	 *                    ("<br/>")
	 *
		...
		switch ( strtoupper( $ps_format ) ) {
			...
			default:
				...
				if ( $vs_tileserver_url = caGetOption( 'tileServerURL', $pa_options, null ) ) {
					...
					$va_layers[]
						= "new OpenLayers.Layer.OSM('{$vs_tile_layer_name}', '{$vs_tileserver_url}',{ isBaseLayer: false, tileOptions : {crossOriginKeyword: null}})";
				}

				$vs_layer_switcher_control = caGetOption( 'layerSwitcherControl', $pa_options, null )
					? "map_{$vs_id}.addControl(new OpenLayers.Control.LayerSwitcher());" : "";


				$va_layers[] = "new {$vs_base_layer}";
				$vs_buf      = "<div style='width:{$vs_width}; height:{$vs_height}' id='{$vs_id}' "
				               . ( ( isset( $pa_options['classname'] ) && $pa_options['classname'] ) ? "class='"
				                                                                                       . $pa_options['classname']
				                                                                                       . "'" : "" )
				               . "> </div>\n";
				$vs_buf      .= "
...
";
			...
			var points_{$vs_id} = new OpenLayers.Layer.Vector('Points', {
				...
				foreach ( $va_paths as $vn_i => $va_path ) {
					...
					$vs_ajax_url = preg_replace( "![\n\r]+!", " ",
						( $va_path['ajaxContentUrl'] ? addslashes( $va_path['ajaxContentUrl'] . "/id/"
						                                           . $va_path['ajaxContentID'] ) : '' ) );


					$va_path_coords = array();
					foreach ( $va_path['path'] as $va_path_point ) {
						$va_path_coords[] = "new OpenLayers.Geometry.Point(" . $va_path_point['longitude'] . ","
						                    . $va_path_point['latitude']
						                    . ").transform(new OpenLayers.Projection('EPSG:4326'), map_{$vs_id}.getProjectionObject())";
					}

					$vs_buf .= "var lineFeature = new OpenLayers.Feature.Vector(new OpenLayers.Geometry.LineString(["
					           . join( ",", $va_path_coords ) . "]), {
					label: '{$vs_label}',
					content: '{$vs_content}',
					ajaxUrl: '{$vs_ajax_url}'
	 ...
	 *                               width = Width of map + controls in pixels; default is 690
	 *                               height = Height of map + controls in pixels; default is 300
	 *                               baseLayer = Tiles to user for base layer. Should be full class name with optional
	 *                               constructor string (Eg. OpenLayers.Layer.Stamen('toner')); default is
	 *                               OpenLayers.Layer.OSM() pointRadius = Radius, in pixels, of plotted points. Default
	 *                               is 5 pixels fillColor = Color (in hex format with leading "#") to fill regions and
	 *                               points with strokeWidth = Width of plotted paths, in pixels. Default is 2
	 *                               strokeColor = Color of plotted paths, in hex format with leading "#"
	 *                               fillColorSelected = Color to fill regions with when selected, in hex format with
	 *                               leading "#" strokeColorSelected = Color of plotted paths when selected, in hex
	 *                               format with leading "#"
	 *
	 * @return string HTML output
	 */
	public function getAttributeBundleHTML( $pa_element_info, $pa_options = null ) {
		...
		$va_options        = caGetOptions( $pa_options, array(
			'width'  => $vn_element_width,
			'height' => $vn_element_height
		) );
		...
		if ( $vs_tileserver_url = caGetOption( 'tileServerURL', $pa_element_info['settings'], null ) ) {
			if ( ! ( $vs_tile_layer_name = trim( caGetOption( 'tileLayerName', $pa_element_info['settings'],
				null ) ) )
			) {
				$vs_tile_layer_name = 'Custom layer';
			}
		...
		$vs_layer_switcher_control = caGetOption( 'layerSwitcherControl', $pa_element_info['settings'], null )
			? "map_{$vs_id}.addControl(new OpenLayers.Control.LayerSwitcher());" : "";


		$vs_element = '<div id="{fieldNamePrefix}mapholder_' . $vs_id . '_{n}" class="mapholder" style="width:'
		              . $vn_width . 'px; height:' . ( $vn_height + 40 ) . 'px; float: left; margin:-18px 0 0 0;">';

		$vs_element .= '<div class="olMapSearchControls" id="{fieldNamePrefix}Controls_{n}">';
		if ( $po_request ) {
			$vs_element .= '<div class="olMapSearchBox">';
			$vs_element .= '<input type="text" class="olMapSearchText" name="{fieldNamePrefix}'
			               . $pa_element_info['element_id'] . '_{n}_search"  id="{fieldNamePrefix}'
			               . $pa_element_info['element_id']
			               . '_{n}_search" size="30" value="" autocomplete="off" onfocus="this.value = \'\';" onkeypress="return map_geocode_'
			               . $vs_id . '(event);"/>';
			$vs_element .= "<a href='#' onclick='map_geocode_{$vs_id}();'>" . caNavIcon( __CA_NAV_ICON_SEARCH__, 1,
					array(
						'class' => 'olMapSearchBoxIcon',
						'id'    => "{fieldNamePrefix}" . $pa_element_info['element_id'] . "_{n}_search_button"
					) ) . "</a>";
			$vs_element .= '</div>';
		}
		$vs_element .= '<div class="olMapKmlControl" id="{fieldNamePrefix}showKmlControl_{n}">';
		$vs_element .= '<div style="position: absolute; bottom: 0px; left: 0px;"><a href="#" class="button" id="{fieldNamePrefix}showKmlControl_{n}_button">'
		               . _t( 'Upload KML file' ) . ' &rsaquo;</a></div>';
		$vs_element .= '</div>';
		$vs_element .= '</div>';
		...
		$vs_element .= _t( "Select KML or KMZ file" ) . ': <input type="file" name="{fieldNamePrefix}'
		               . $pa_element_info['element_id']
		               . '_{n}"/><a href="#" class="button"  id="{fieldNamePrefix}hideKmlControl_{n}_button">'
		               . _t( 'Use map' ) . ' &rsaquo;</a>';
		$vs_element .= '</div>';
		$vs_element .= '<div class="olMap" id="{fieldNamePrefix}map_' . $vs_id . '_{n}" style="width:' . $vn_width
		               . 'px; height:' . $vn_height . 'px;"> </div>';
		$vs_element .= '</div>';
	...
	jQuery(document).ready(function() {
		...
		$vs_element .= '<input class="coordinates mapCoordinateDisplay" type="text" name="{fieldNamePrefix}'
		               . $pa_element_info['element_id'] . '_{n}" id="{fieldNamePrefix}' . $pa_element_info['element_id']
		               . '_{n}"/>';
