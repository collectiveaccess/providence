    ...
    public function render($ps_format, $pa_options = null)
    {
        ...
        switch (strtoupper($ps_format)) {
            ...
            default:
                ...
                $vs_layer_switcher_control = caGetOption(
...
";
			...
			var points_{$vs_id} = new OpenLayers.Layer.Vector('Points', {
                ...
                foreach ($va_paths as $vn_i => $va_path) {
                    ...
                    $vs_ajax_url = preg_replace(
                        "![\n\r]+!",
                        " ",
                        ($va_path['ajaxContentUrl'] ? addslashes(
                            $va_path['ajaxContentUrl'] . "/id/" . $va_path['ajaxContentID']
                        ) : '')
                    );
                    ...
                    $vs_buf .= "var lineFeature = new OpenLayers.Feature.Vector(new OpenLayers.Geometry.LineString([" . join(
                            ",",
                            $va_path_coords
                        ) . "]), {
					label: '{$vs_label}',
    ...
    public function getAttributeBundleHTML($pa_element_info, $pa_options = null)
    {
        ...
        if ($po_request) {
            ...
            $vs_element .= "<a href='#' onclick='map_geocode_{$vs_id}();'>" . caNavIcon(
        ...
        $vs_element .= '<div style="position: absolute; bottom: 0px; left: 0px;"><a href="#" class="button" id="{fieldNamePrefix}showKmlControl_{n}_button">' . _t(
                'Upload KML file'
            ) . ' &rsaquo;</a></div>';
        $vs_element .= '</div>';
        $vs_element .= '</div>';
        ...
        $vs_element .= _t(
            ...
            ) . ': <input type="file" name="{fieldNamePrefix}' . $pa_element_info['element_id'] . '_{n}"/><a href="#" class="button"  id="{fieldNamePrefix}hideKmlControl_{n}_button">' . _t(
                'Use map'
            ) . ' &rsaquo;</a>';
        $vs_element .= '</div>';
