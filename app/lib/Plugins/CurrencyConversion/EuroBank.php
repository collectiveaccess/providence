...
class WLPlugCurrencyConversionEuroBank Extends BaseCurrencyConversionPlugIn implements IWLPlugCurrencyConversion
{
    ...
    public function __construct()
    {
        ...
        $this->description = _t(
            'Converts currency using European bank rates available at %1',
        ...
        if (preg_match("!^([^\d]+)([\d\.\,]+)$!", trim($ps_value), $va_matches)) {
            ...
            if (preg_match("!^([\d\.\,]+)([^\d]+)$!", trim($ps_value), $va_matches)) {
                ...
                if (preg_match("!(^[\d\,\.]+$)!", trim($ps_value), $va_matches)) {
                    ...
                    throw(new Exception(
                        _t('%1 is not a valid currency value; be sure to include a currency symbol', $ps_value)
                    ));
                    return false;
                }
            }
    ...
    private static function _loadData()
    {
        ...
        if ($vs_data = caQueryExternalWebservice(WLPlugCurrencyConversionEuroBank::CONVERSION_SERVICE_URL)) {
            if (!($o_data = new SimpleXMLElement($vs_data))) {
                throw(new Exception(
                    _t("Cannot parse data from %1", WLPlugCurrencyConversionEuroBank::CONVERSION_SERVICE_URL)
                ));
                return null;
            }
