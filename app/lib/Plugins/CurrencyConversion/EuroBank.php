...
class WLPlugCurrencyConversionEuroBank Extends BaseCurrencyConversionPlugIn implements IWLPlugCurrencyConversion {
	...
	public function __construct() {
		...
		$this->description = _t( 'Converts currency using European bank rates available at %1',
			WLPlugCurrencyConversionEuroBank::CONVERSION_SERVICE_URL );
	}
	# ------------------------------------------------
	 ...
	 *                    numericValue = return floating point numeric value only, without currency specifier. Default
	 *                    is false.
	 *
	...
	static public function convert( $ps_value, $ps_to, $pa_options = null ) {
		...
		if ( preg_match( "!^([^\d]+)([\d\.\,]+)$!", trim( $ps_value ), $va_matches ) ) {
			...
			throw( new Exception( _t( '%1 is not a valid currency value; be sure to include a currency symbol',
				$ps_value ) ) );
		...
		$vs_decimal_with_placeholder = Zend_Locale_Format::toNumber( $vn_converted_value,
	...
	static private function _loadData() {
		...
		if ( $vs_data = caQueryExternalWebservice( WLPlugCurrencyConversionEuroBank::CONVERSION_SERVICE_URL ) ) {
			if ( ! ( $o_data = new SimpleXMLElement( $vs_data ) ) ) {
				throw( new Exception( _t( "Cannot parse data from %1",
					WLPlugCurrencyConversionEuroBank::CONVERSION_SERVICE_URL ) ) );
		...
		throw( new Exception( _t( "Cannot fetch data from %1",
			WLPlugCurrencyConversionEuroBank::CONVERSION_SERVICE_URL ) ) );
