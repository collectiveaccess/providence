...
abstract class BaseApplicationTool implements IApplicationTool {
	 ...
	 * @param string $ps_mode             Tool run mode. Either CLI (command line) or WebUI (via web-based user
	 *                                    interface).
	 * @param        $ps_tool_config_path Path to tool configuration file.
	 * @param        $ps_log_path         Path to tool log directory. If omitted log are placed in default app/log
	 *                                    directory.
	 */
	public function __construct( $pa_settings = null, $ps_mode = 'CLI', $ps_tool_config_path = null, $ps_log_path = null
		...
		if ( is_array( $pa_settings ) ) {
			$this->setSettings( $pa_settings );
		}
		if ( $ps_tool_config_path ) {
			$this->ops_tool_config_path = $ps_tool_config_path;
		}

		if ( ! $ps_log_path ) {
			$ps_log_path = __CA_APP_DIR__ . '/log';
		}
		if ( $ps_log_path ) {
			$this->setLogPath( $ps_log_path );
		}
		if ( $ps_mode ) {
	...
	public function setJobID( $ps_job_id = null, $pa_options = null ) {
		$this->ops_job_id = ( $ps_job_id )
			? $ps_job_id
			: md5( caGetOption( 'data', $pa_options, '' ) . '_' . $this->getToolIdentifier() . '_' . uniqid( rand(),
					true ) . '_' . microtime( true ) );
	 ...
	 *
	 * @param int $pn_log_level KLogger constant for minimum log level to record. Default is KLogger::INFO. Constants
	 *                          are, in descending order of shrillness: KLogger::EMERG = Emergency messages (system is
	 *                          unusable) KLogger::ALERT = Alert messages (action must be taken immediately)
	 *                          KLogger::CRIT = Critical conditions KLogger::ERR = Error conditions KLogger::WARN =
	 *                          Warnings KLogger::NOTICE = Notices (normal but significant conditions) KLogger::INFO =
