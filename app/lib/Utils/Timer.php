...
class Timer {
	...
	/**
	 * @float microtime the timer has run before the current start. This allows additive timer runs. Each time the
	 *        timer is started and stoped the time run is added to $elapsed and $start is reset to 0.
	 */
	private $elapsed = 0;
	 ...
	 *
	 * @param string $ps_name    Name of timer
	 * @param string $ps_message Formatting string for display. String may contain placeholders for time (%time) and
	 *                           timer name (%name). [Default is "[%name] %time seconds<br/>\n"]
	...
	static function get( $ps_name, $ps_message = "[%name] %time seconds<br/>\n", $pa_options = null ) {
		if ( Timer::$s_timers[ $ps_name ] instanceof Timer ) {
			...
			return $ps_message ? str_replace( "%time", $vn_time, str_replace( "%name", $ps_name, $ps_message ) )
				: $vn_time;
		}
