...
class DataMigrationUtils
{
    ...
    static function getListItemID(
        ...
        if ($pa_options['cache'] && isset(DataMigrationUtils::$s_cached_list_item_ids[$vs_cache_key])) {
            ...
            if ($o_log) {
                $o_log->logDebug(
                    _t(
                        "Found existing list item %1 (member of list %2) in DataMigrationUtils::getListItemID() using idno",
                        $ps_item_idno,
                        $pm_list_code_or_id
                    )
                );
                    ...
                    if (trim($vs_singular_label) || trim($vs_plural_label)) {
                        ...
                        if ($vn_item_id = (ca_list_items::find(
                            ...
                            if ($o_log) {
                                $o_log->logDebug(
                                    _t(
                                        "Found existing list item %1 (member of list %2) in DataMigrationUtils::getListItemID() using singular label %3",
                                        $ps_item_idno,
                                        $pm_list_code_or_id,
                                        $vs_singular_label
                                    )
                            ...
                            if ($vn_item_id = (ca_list_items::find(
                                ...
                                if ($o_log) {
                                    $o_log->logDebug(
                                        _t(
                                            "Found existing list item %1 (member of list %2) in DataMigrationUtils::getListItemID() using plural label %3",
                                            $ps_item_idno,
                                            $pm_list_code_or_id,
                                            $vs_plural_label
                                        )
                ...
                case 'idno':
                    ...
                    if ($vn_item_id = (ca_list_items::find(
                        ...
                        if ($o_log) {
                            $o_log->logDebug(
                                _t(
                                    "Found existing list item %1 (member of list %2) in DataMigrationUtils::getListItemID() using idno with %3",
                                    $ps_item_idno,
                                    $pm_list_code_or_id,
                                    $ps_item_idno
                                )
        ...
        if (isset($pa_options['dontCreate']) && $pa_options['dontCreate']) {
            if ($o_log) {
                $o_log->logNotice(
                    _t("Not adding \"%1\" to list %2 as dontCreate option is set", $ps_item_idno, $pm_list_code_or_id)
                );
            }
            return false;
        }
        ...
        if ($t_item = $t_list->addItem(
            ...
            if ($o_log) {
                $o_log->logInfo(
                    _t(
                        "Created new list item %1 in list %2",
                        "{$vs_singular_label}/{$vs_plural_label}/{$ps_item_idno}",
                        $pm_list_code_or_id
                    )
                );
                    ...
                    if (is_array($va_value)) {
                        ...
                        foreach ($va_expanded_values as $va_v) {
                            $pt_instance->addAttribute(
                                ...
                                null,
                                [
                                    'skipExistingValues' => (caGetOption(
                                            'skipExistingValues',
                                            $pa_options,
                                            false
                                        ) || caGetOption('skipExistingValues', $va_values, false)),
    ...
    private static function _getID(
        ...
        if (!$vn_id) {
            ...
            if ($t_instance->hasField('media') && ($t_instance->getFieldInfo(
                ...
                if (($pb_match_media_without_ext) && !isURL($pa_values['media']) && !file_exists($pa_values['media'])) {
                    ...
                    foreach ($va_files_in_dir as $vs_filepath) {
                        if ($o_log) {
                            $o_log->logDebug(
                                _t("Trying media %1 in place of %2/%3", $vs_filepath, $vs_original_path, $vs_filename)
                            );
                        }
                        if (pathinfo($vs_filepath, PATHINFO_FILENAME) == $vs_filename) {
                            if ($o_log) {
                                $o_log->logNotice(
                                    _t("Found media %1 for %2/%3", $vs_filepath, $vs_original_path, $vs_filename)
                                );
                            }
            ...
            $t_instance->insert();
            if ($o_log) {
                $o_log->logDebug(_t("Could not create %1 record: %2", $ps_table, join("; ", $t_instance->getErrors())));
            }
            ...
            if ($o_log) {
                $o_log->logDebug(
                    _t(
                        "Found existing %1 %2 in DataMigrationUtils::_getID()",
                        $vs_table_display_name,
                        $pa_label[$vs_label_display_fld]
                    )
                );
            ...
            if (($vb_force_update = caGetOption(
                ...
                if (!$vn_rc) {
                    if ($o_log) {
                        $o_log->logError(
                            _t(
                                "Could not load existing %1 with id %2 (%3) in DataMigrationUtils::_getID() [THIS SHOULD NOT HAPPEN]",
                                $vs_table_display_name,
                                $vn_id,
                                $pa_label[$vs_label_display_fld]
                            )
                    ...
                    if ($vb_force_update && $vb_has_attr) {
                        if ($vb_attr_errors = !DataMigrationUtils::_setAttributes(
                            ...
                            if ($o_log) {
                                $o_log->logError(
                                    _t(
                                        "Could not set attributes for %1 with id %2 (%3) in DataMigrationUtils::_getID(): %4",
                                        $vs_table_display_name,
                                        $vn_id,
                                        $pa_label[$vs_label_display_fld],
                                        join("; ", $t_instance->getErrors())
