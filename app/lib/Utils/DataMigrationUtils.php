 ...
 */
  ...
  */
	...
	class DataMigrationUtils {
		 ...
		 *		replacePunctuation = convert curly apostrophes and quotes, em dashes and É to ascii equivalents in the process to avoid encoding issues with iconv. [Default=true]
		 * @return string
		 */
		static function transformTextEncoding($ps_text, $pa_options=null) {
			if (caGetOption('replacePunctuation', $pa_options, true)) {
				$ps_text = str_replace("Ô", "'", $ps_text);
				$ps_text = str_replace("Õ", "'", $ps_text);
				$ps_text = str_replace("Ò", '"', $ps_text);
				$ps_text = str_replace("Ó", '"', $ps_text);
				$ps_text = str_replace("Ð", "-", $ps_text);
				$ps_text = str_replace("É", "...", $ps_text);
			}
