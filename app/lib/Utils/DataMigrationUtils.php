...
class DataMigrationUtils
{
     ...
     */
    static public $s_source_encoding = 'ISO-8859-1';

    /**
     * @var encoding of target data (should almost always be UTF-8)
     */
    static public $s_target_encoding = 'UTF-8';

    /**
     * @var array cache of created list item_ids
     */
    static public $s_cached_list_item_ids = array();
    ...
    public static function getEntityID(
        ...
        return DataMigrationUtils::_getID(
            'ca_entities',
            $pa_entity_name,
            null,
            $pn_type_id,
    ...
    public static function getPlaceID(
        ...
        if ($pn_hierarchy_id) {
            ...
            $t_list = new ca_lists();
            if (sizeof(
                $va_hierarchy_ids = $t_list->getItemsForList(
                    'place_hierarchies',
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getOccurrenceID(
        $ps_occ_name,
        $pn_parent_id,
        $pn_type_id,
        $pn_locale_id,
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getListItemID(
        $pm_list_code_or_id,
        $ps_item_idno,
        $pn_type_id,
        $pn_locale_id,
        ...
        if (!$vs_singular_label) {
            $vs_singular_label = (isset($pa_values['name_singular']) && $pa_values['name_singular']) ? $pa_values['name_singular'] : str_replace(
                "_",
                " ",
                $ps_item_idno
            );
        ...
        if (!$vs_plural_label) {
            $vs_plural_label = (isset($pa_values['name_plural']) && $pa_values['name_plural']) ? $pa_values['name_plural'] : str_replace(
                "_",
                " ",
                $ps_item_idno
            );
        ...
        // Create cache key
        $vs_cache_key = md5(
            $pm_list_code_or_id . '/' . $ps_item_idno . '/' . $vn_parent_id . '/' . $vs_singular_label . '/' . $vs_plural_label . '/' . json_encode(
                $pa_match_on
            )
        );
        ...
        if ($pa_options['cache'] && isset(DataMigrationUtils::$s_cached_list_item_ids[$vs_cache_key])) {
            ...
            if ($o_event) {
                ...
                $o_event->endItem(
                    DataMigrationUtils::$s_cached_list_item_ids[$vs_cache_key],
            ...
            if ($o_log) {
                $o_log->logDebug(
                    _t(
                        "%3Found existing list item %1 (member of list %2) in DataMigrationUtils::getListItemID() using idno",
                        $ps_item_idno,
                        $pm_list_code_or_id,
                        $log_reference_str
                    )
        ...
        if (!($vn_list_id = ca_lists::getListID($pm_list_code_or_id))) {
            ...
            if ($o_log) {
                $o_log->logError(
                    _t("%2Could not find list with list code %1", $pm_list_code_or_id, $log_reference_str)
                );
            }
            ...
            switch (strtolower($vs_match_on)) {
                ...
                case 'nonpreferred_labels':
                    ...
                    if (trim($vs_singular_label) || trim($vs_plural_label)) {
                        ...
                        if ($vn_item_id = (ca_list_items::find(
                            $va_criteria,
                            array(
                                'returnAs' => 'firstId',
                            ...
                            if ($o_log) {
                                $o_log->logDebug(
                                    _t(
                                        "%4Found existing list item %1 (member of list %2) in DataMigrationUtils::getListItemID() using singular label %3",
                                        $ps_item_idno,
                                        $pm_list_code_or_id,
                                        $vs_singular_label,
                                        $log_reference_str
                            ...
                            if ($vn_item_id = (ca_list_items::find(
                                ...
                                if ($o_log) {
                                    $o_log->logDebug(
                                        _t(
                                            "%4Found existing list item %1 (member of list %2) in DataMigrationUtils::getListItemID() using plural label %3",
                                            $ps_item_idno,
                                            $pm_list_code_or_id,
                                            $vs_plural_label,
                                            $log_reference_str
                ...
                case 'idno':
                    ...
                    if ($vn_item_id = (ca_list_items::find(
                        ...
                        if ($o_log) {
                            $o_log->logDebug(
                                _t(
                                    "%4Found existing list item %1 (member of list %2) in DataMigrationUtils::getListItemID() using idno with %3",
                                    $ps_item_idno,
                                    $pm_list_code_or_id,
                                    $ps_item_idno,
                                    $log_reference_str
        ...
        if ($vn_item_id) {
            ...
            if (($vb_force_update = caGetOption(
                    'forceUpdate',
                    $pa_options,
                    false
                )) || ($vb_return_instance = caGetOption('returnInstance', $pa_options, false))) {
                $vb_has_attr = false;
                if ($vb_force_update) {
                ...
                if ($vb_force_update && $vb_has_attr) {
                    $vb_attr_errors = !DataMigrationUtils::_setAttributes(
                        $t_item,
                        $pn_locale_id,
                        $pa_values,
                        $pa_options
        ...
        if (isset($pa_options['dontCreate']) && $pa_options['dontCreate']) {
            if ($o_log) {
                ...
                if ((bool)$o_config->get('log_import_dont_create_events_as_errors')) {
                    $o_log->logError(
                        _t(
                            "%3Not adding \"%1\" to list %2 because dontCreate option is set",
                            $ps_item_idno,
                            $pm_list_code_or_id,
                            $log_reference_str
                        )
        ...
        if ($t_item = $t_list->addItem(
            $ps_item_idno,
            $pa_values['is_enabled'],
            $pa_values['is_default'],
            $vn_parent_id,
            $pn_type_id,
            $ps_item_idno,
            '',
            (int)$pa_values['status'],
            (int)$pa_values['access'],
            $pa_values['rank']
        )) {
            ...
            if ($t_item->numErrors()) {
                if ($pb_output_errors) {
                    print "[Error] " . _t(
                            "Could not set preferred label for list item %1: %2",
                ...
                if ($o_log) {
                    $o_log->logError(
                        _t(
                            "%3Could not set preferred label for list item %1: %2",
                            "{$vs_singular_label}/{$vs_plural_label}/{$ps_item_idno}",
                            join('; ', $t_item->getErrors()),
                            $log_reference_str
                        )
            ...
            if ($o_log) {
                $o_log->logInfo(
                    _t(
                        "%3Created new list item %1 in list %2",
                        "{$vs_singular_label}/{$vs_plural_label}/{$ps_item_idno}",
                        $pm_list_code_or_id,
                        $log_reference_str
                    )
            ...
            if ($o_log) {
                $o_log->logError(
                    _t("%2Could not find add item to list: %1", join("; ", $t_list->getErrors()), $log_reference_str)
                );
            }
        }
        return null;
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getObjectLotID(
        $ps_idno_stub,
        $ps_lot_name,
        $pn_type_id,
        $pn_locale_id,
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getObjectRepresentationID(
        $ps_representation_name,
        $pn_type_id,
        $pn_locale_id,
        $pa_values = null,
        $pa_options = null
        ...
        return DataMigrationUtils::_getID(
            ...
            array('name' => $ps_representation_name),
            null,
            $pn_type_id,
            $pn_locale_id,
            $pa_values,
     ...
     * @see DataMigrationUtils::_getID()
     */
    public static function getLoanID($ps_loan_name, $pn_type_id, $pn_locale_id, $pa_values = null, $pa_options = null)
    {
        return DataMigrationUtils::_getID(
            'ca_loans',
            array('name' => $ps_loan_name),
            null,
            $pn_type_id,
            $pn_locale_id,
            $pa_values,
     ...
     * @return string
     */
    public static function transformTextEncoding($ps_text, $pa_options = null)
    {
     ...
     */
    public static function splitEntityName($ps_text, $pa_options = null)
    {
        global $g_ui_locale;
        ...
        if ($vs_suffix_for_name) {
            ...
            $ps_original_text = trim(
                "{$vs_forename} {$vs_surname}" . ((sizeof($va_tmp) > 0) ? ' ' . join(' ', $va_tmp) : '')
            );
        } else {
            $va_name = array(
                'surname' => '',
        ...
        switch ($vs_format = caGetOption(
            ...
            case 'surnamecommaforename':
                $va_name['displayname'] = ((strlen(
                        trim($va_name['surname'])
                    )) ? $va_name['surname'] . ", " : '') . $va_name['forename'];
                break;
        ...
        if (is_array($pa_values)) {
            foreach ($pa_values as $vs_element => $va_values) {
                ...
                foreach ($va_values as $va_value) {
                    if (is_array($va_value)) {
                        if (($vs_delimiter = caGetOption('delimiter', $va_value, null)) && !sizeof(
                                array_filter(
                                    $va_value,
                                    function ($v) {
                                        return is_array($v);
                        ...
                        if ($va_value) {
                            $pt_instance->addAttribute(
                                ...
                                $vs_element,
                                null,
                                ['skipExistingValues' => true, 'matchOn' => caGetOption('matchOn', $va_values, null)]
                            );
                        }
                    }
            ...
            if ($pt_instance->numErrors()) {
                if (isset($pa_options['outputErrors']) && $pa_options['outputErrors']) {
                    print "[Error] " . _t(
                            "Could not set values for %1 %2: %3",
                            $pt_instance->getProperty('NAME_SINGULAR'),
                            $pt_instance->getLabelForDisplay(),
        ...
        if (is_array($va_nonpreferred_labels = caGetOption("nonPreferredLabels", $pa_options, null))) {
            ...
            foreach ($va_labels as $va_label) {
                ...
                if ($pt_instance->numErrors()) {
                    if (isset($pa_options['outputErrors']) && $pa_options['outputErrors']) {
                        print "[Error] " . _t(
                                "Could not set non-preferred label for %1 %2: %3",
                                $pt_instance->getProperty('NAME_SINGULAR'),
                                $pt_instance->getLabelForDisplay(),
                                join('; ', $pt_instance->getErrors())
                            ) . "\n";
        ...
        if ($o_idno = $pt_instance->getIDNoPlugInInstance()) {
            ...
            if (($vs_proc_idno = join($vs_sep, $va_values)) && ($vs_proc_idno != $ps_idno)) {
                ...
                if ($pt_instance->numErrors()) {
                    if (isset($pa_options['outputErrors']) && $pa_options['outputErrors']) {
                        print "[Error] " . _t(
                                "Could not update idno for %1 %2: %3",
                                $pt_instance->getProperty('NAME_SINGULAR'),
                                $pt_instance->getLabelForDisplay(),
                                join('; ', $pt_instance->getErrors())
                            ) . "\n";
     ...
     */
    public static function getIDFor(
        $ps_table,
        $pa_label,
        $pn_parent_id,
        $pn_type_id,
    ...
    private static function _getID(
        ...
        $pa_match_on = caGetOption(
            'matchOn',
            $pa_options,
            array('label', 'idno', 'displayname'),
            array('castTo' => "array")
        );
        ...
        if (!$vs_idno) {
            if (isset($pa_options['generateIdnoWithTemplate']) && $pa_options['generateIdnoWithTemplate']) {
                $pa_values[$vs_idno_fld] = $vs_idno = $t_instance->setIdnoWithTemplate(
                                    ...
                                    foreach ($va_regex_info['regexes'] as $vs_regex) {
                                        foreach ($va_idnos_to_match as $vs_idno_match) {
                                            ...
                                            if (preg_match('!' . $vs_regex . '!', $vs_idno_match, $va_matches)) {
                                                if ($vn_id = (ca_object_representations::find(
                                                    array('idno' => $va_matches[1]),
                                                    array(
                                                        'returnAs' => 'firstId',
                                                        'purifyWithFallback' => true,
                                                        'transaction' => $pa_options['transaction']
        ...
        if (!$vn_id) {
            ...
            if (caGetOption('dontCreate', $pa_options, false)) {
                if ($o_log) {
                    ...
                    if ((bool)$o_config->get('log_import_dont_create_events_as_errors')) {
                        $o_log->logError(
                            _t(
                                "%4Not adding \"%1\" (%2) as %3 because dontCreate option is set",
                                $vs_label,
                                $vs_idno,
                                $ps_table,
                                $log_reference_str
            ...
            foreach ($va_intrinsics as $vs_fld => $vm_fld_default) {
                if ($t_instance->hasField($vs_fld)) {
                    ...
                    $vs_v = (isset($pa_values[$vs_fld]) && is_array($pa_values[$vs_fld])) ? caGetOption(
                        $vs_fld,
                        $pa_values[$vs_fld],
                        $vm_fld_default
            ...
            if ($t_instance->hasField('media') && ($t_instance->getFieldInfo(
                        'media',
                        'FIELD_TYPE'
                    ) == FT_MEDIA) && isset($pa_values['media']) && $pa_values['media']) {
                ...
                if (($pb_match_media_without_ext) && !isURL($pa_values['media']) && !file_exists($pa_values['media'])) {
                    ...
                    $va_files_in_dir = caGetDirectoryContentsAsList(
                        ($vb_allow_any_directory && $vs_dirname) ? $vs_dirname : $vs_import_dir,
                        true,
                        false,
                        false,
                        false
                    ...
                    foreach ($va_files_in_dir as $vs_filepath) {
                        if ($o_log) {
                            $o_log->logDebug(
                                _t(
                                    "%4Trying media %1 in place of %2/%3",
                                    $vs_filepath,
                                    $vs_original_path,
                                    $vs_filename,
                                    $log_reference_str
            ...
            if ($t_instance->numErrors()) {
                if ($pb_output_errors) {
                    print "[Error] " . _t(
                            "Could not insert %1 %2: %3",
                            $vs_table_display_name,
                            $pa_label[$vs_label_display_fld],
            ...
            if ($t_instance->numErrors()) {
                if ($pb_output_errors) {
                    print "[Error] " . _t(
                            "Could not set preferred label for %1 %2: %3",
                            $vs_table_display_name,
                            $pa_label[$vs_label_display_fld],
                            join('; ', $t_instance->getErrors())
                        ) . "\n";
                ...
                if ($o_log) {
                    $o_log->logError(
                        _t(
                            "%4Could not set preferred label for %1 %2: %3",
                            $vs_table_display_name,
                            $pa_label[$vs_label_display_fld],
                            join('; ', $t_instance->getErrors()),
                            $log_reference_str
            ...
            if ($o_log) {
                $o_log->logDebug(
                    _t(
                        "%3Found existing %1 %2 in DataMigrationUtils::_getID()",
                        $vs_table_display_name,
                        $pa_label[$vs_label_display_fld],
                        $log_reference_str
                    )
            ...
            if (($vb_force_update = caGetOption(
                ...
                if (!$vn_rc) {
                    if ($o_log) {
                        $o_log->logError(
                            _t(
                                "%4Could not load existing %1 with id %2 (%3) in DataMigrationUtils::_getID() [THIS SHOULD NOT HAPPEN]",
                                $vs_table_display_name,
                                $vn_id,
                                $pa_label[$vs_label_display_fld],
                                $log_reference_str
                    ...
                    if ($vb_force_update && $vb_has_attr) {
                        if ($vb_attr_errors = !DataMigrationUtils::_setAttributes(
                            $t_instance,
                            $pn_locale_id,
                            $pa_values,
                            $pa_options
                            ...
                            if ($o_log) {
                                $o_log->logError(
                                    _t(
                                        "%5Could not set attributes for %1 with id %2 (%3) in DataMigrationUtils::_getID(): %4",
                                        $vs_table_display_name,
                                        $vn_id,
                                        $pa_label[$vs_label_display_fld],
                                        join("; ", $t_instance->getErrors()),
     ...
     * @return string
     */
    public static function postError($po_object, $ps_message, $pn_level = __CA_DATA_IMPORT_ERROR__, $pa_options = null)
    {
