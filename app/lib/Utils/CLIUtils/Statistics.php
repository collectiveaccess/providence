...
trait CLIUtilsStatistics
{
    ...
    public static function fetch_statistics($po_opts = null)
    {
        ...
        CLIUtils::addMessage(
            ($num_sites === 1) ? _t("Cached statistics for %1 site: %2", $num_sites, $site_list) : _t(
                "Cached statistics for %1 sites: %2",
                $num_sites,
                $site_list
            )
        );
    ...
    {
        return _t(
            'Fetches data and usage statistics from local and remote CollectiveAccess instances and makes them available in the Statistics Dashboard.'
        );
    }
