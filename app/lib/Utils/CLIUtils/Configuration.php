...
trait CLIUtilsConfiguration {
	...
	public static function install( $po_opts = null, $pb_installing = true ) {
		...
		$vs_profile_directory = $vs_profile_directory ? $vs_profile_directory
	...
	public static function installParamList() {
		return array(
			...
			"profile-directory|p=s" => _t( 'Directory to get profile. Default is: "%1". This directory must contain the profile.xsd schema so that the installer can validate the installation profile.',
				__CA_BASE_DIR__ . '/install/profiles/xml' ),
...
\tIf the database schema already exists, this operation will fail, unless the --overwrite flag is set, in which case all existing data will be deleted (use with caution!)." );
	...
	public static function export_profile( $po_opts = null ) {
		...
		$vs_profile = ConfigurationExporter::exportConfigurationAsXML( $po_opts->getOption( "name" ),
			$po_opts->getOption( "description" ), $po_opts->getOption( "base" ), $po_opts->getOption( "infoURL" ),
			$vn_timestamp );
	...
	public static function push_config_changes( $po_opts = null ) {
		...
		foreach ( $va_targets as $vs_target ) {
			...
			if ( ! isURL( $vs_target ) ) {
				...
				if ( $o_log ) {
					$o_log->logError( _t( "[push-config-changes] The target '%1' doesn't seem to be in URL format",
						$vs_target ) );
				}
			...
			if ( $o_log ) {
				$o_log->logDebug( _t( "[push-config-changes] Service endpoint is '%1'. Timestamp for diff config is %2",
					$vs_target, $vn_target_timestamp ) );
			}

			$vs_config                   = ConfigurationExporter::exportConfigurationAsXML( '', '', '', '',
				$vn_target_timestamp, true );
			...
			if ( $vn_code != 200 ) {
				CLIUtils::addError( _t( "Pushing to target '%1' seems to have failed. HTTP response code was %2.",
					$vs_target, $vn_code ) );
				if ( $o_log ) {
					$o_log->logError( _t( "[push-config-changes] Pushing to target '%1' seems to have failed. HTTP response code was %2. Enable debug logging mode to get more info below.",
						$vs_target, $vn_code ) );
				}
			}
			...
			if ( $o_log ) {
				$o_log->logDebug( _t( "[push-config-changes] Decoded response from target '%1' is '%2'", $vs_target,
					print_r( $va_response, true ) ) );
			}

			if ( ! isset( $va_response['ok'] ) || ! $va_response['ok'] ) {
				if ( is_array( $va_errors = $va_response['errors'] ) ) {
					CLIUtils::addError( _t( "Pushing to target '%1' seems to have failed. Response was not marked as okay. Errors were: %2",
						$vs_target, join( ',', $va_errors ) ) );
				} else {
					CLIUtils::addError( _t( "Pushing to target '%1' seems to have failed. Response was not marked as okay. Raw response was: %2",
						$vs_target, $vs_exec ) );
				}
			}
		...
		$o_vars->save();
		if ( $o_log ) {
			$o_log->logDebug( _t( "[push-config-changes] Saved sync timestamps are: %1",
				print_r( $va_timestamps, true ) ) );
		}
