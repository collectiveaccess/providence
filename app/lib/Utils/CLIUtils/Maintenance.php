	...
	public static function rebuild_sort_values() {
		...
		foreach (
			...
			if ( $vs_label_table_name = $t_table->getLabelTableName() ) {
				...
				print CLIProgressBar::start( $qr_labels->numRows(),
			...
			print CLIProgressBar::start( $qr_res->numRows(),
...
\tNote that depending upon the size of your database reloading sort values can take from a few minutes to an hour or more. During the reloading process the system will remain usable but search and browse functions may return incorrectly sorted results." );
	...
	public static function remove_unused_media( $po_opts = null ) {
		...
		if ( $vn_delete_count == 1 ) {
			CLIUtils::addMessage( $vb_delete_opt ? _t( "%1 file (%2) was deleted", $vn_delete_count, $vs_percent )
				: _t( "%1 file (%2) is unused", $vn_delete_count, $vs_percent ) );
		} else {
			CLIUtils::addMessage( $vb_delete_opt ? _t( "%1 files (%2) were deleted", $vn_delete_count, $vs_percent )
				: _t( "%1 files (%2) are unused", $vn_delete_count, $vs_percent ) );
		}
	...
	/**
	 * Permanently remove object representations marked for deletion, deleting referenced files on disk and reclaiming
	 * disk space
	 */
	public static function remove_deleted_representations( $po_opts = null ) {
		...
		if ( $vb_delete_opt && ( $qr_reps->numRows() > 0 ) ) {
			...
			CLIUtils::addMessage( _t( 'There are no deleted representations to process!' ),
				array( 'color' => 'green' ) );
		} else {
			CLIUtils::addMessage( _t( "%1 files are referenced by %2 deleted records. Both the records and the files will be deleted if you re-run the script with the -d (--delete) option and the correct permissions.",
				sizeof( $va_paths ), $qr_reps->numRows() ) );
			CLIUtils::addMessage( _t( "It is highly recommended to create a full backup before you do this!" ),
	...
	public static function purge_deleted( $po_opts = null ) {
		...
		CLIUtils::addMessage( _t( "Are you sure you want to PERMANENTLY remove all deleted records? This cannot be undone.\n\nType 'y' to proceed or 'N' to cancel, then hit return ",
			$vn_current_revision, __CollectiveAccess_Schema_Rev__ ) );
		flush();
		ob_flush();
		...
		$o_db                 = new Db();
		$va_tables_to_process = array_filter( array_map( "trim",
			preg_split( '![ ,;]!', (string) $po_opts->getOption( 'tables' ) ) ), "strlen" );

		$vn_t = 0;
		foreach ( $va_tables as $vs_table ) {
			if ( is_array( $va_tables_to_process ) && sizeof( $va_tables_to_process )
			     && ! in_array( $vs_table, $va_tables_to_process )
			) {
				continue;
			}
			...
			if ( $qr_del->numRows() > 0 ) {
				print CLIProgressBar::start( $qr_del->numRows(),
					_t( 'Removing deleted %1 from database', $t_instance->getProperty( 'NAME_PLURAL' ) ) );
				...
				CLIUtils::addMessage( _t( 'Removed %1 %2', $vn_c,
	...
	public static function update_database_schema( $po_opts = null ) {
		...
		if ( ( $vn_current_revision = ConfigurationCheck::getSchemaVersion() ) < __CollectiveAccess_Schema_Rev__ ) {
			...
			print CLIProgressBar::finish();
			CLIUtils::addMessage( _t( "Database already at revision %1. No update is required.",
	...
	public static function fix_permissions( $po_opts = null ) {
		...
		if ( ! $po_opts->getOption( "quiet" ) ) {
			CLIUtils::addMessage( _t( "Fixing permissions for the temporary directory (app/tmp) for ownership by \"%1\"...",
				$vs_user ) );
		}
		...
		if ( ! $po_opts->getOption( "quiet" ) ) {
			CLIUtils::addMessage( _t( "Fixing permissions for the media directory (media) for ownership by \"%1\"...",
				$vs_user ) );
		}
		...
		if ( ! $po_opts->getOption( "quiet" ) ) {
			CLIUtils::addMessage( _t( "Fixing permissions for the HTMLPurifier definition cache directory (vendor/ezyang/htmlpurifier/library/HTMLPurifier/DefinitionCache/Serializer) for ownership by \"%1\"...",
				$vs_user ) );
		}
	...
	public static function fix_permissionsShortHelp() {
		return _t( "Fix folder permissions. MUST BE RUN WHILE LOGGED IN WITH ADMINSTRATIVE/ROOT PERMISSIONS. You are currently logged in as %1 (uid %2)",
			caGetProcessUserName(), caGetProcessUserID() );
	}
	# -------------------------------------------------------
	...
	public static function reset_password( $po_opts = null ) {
		if ( ! ( $vs_user_name = (string) $po_opts->getOption( 'user' ) )
		     && ! ( $vs_user_name
				= (string) $po_opts->getOption( 'username' ) )
		) {
		...
		$t_user->update();
		if ( $t_user->numErrors() ) {
			CLIUtils::addError( _t( "Password change for user %1 failed: %2", $vs_user_name,
				join( "; ", $t_user->getErrors() ) ) );
	...
	public static function check_media_fixity( $po_opts = null ) {
		...
		switch ( $ps_format = caGetOption( 'format', $po_opts, 'text',
			[ 'forceLowercase' => true, 'validValues' => [ 'text', 'tab', 'csv' ], 'castTo' => 'string' ] ) ) {
			case 'tab':
		...
		$vs_report_output = join( ( $ps_format == 'tab' ) ? "\t" : ",", array(
				_t( 'Type' ),
				_t( 'Error' ),
				_t( 'Name' ),
				_t( 'ID' ),
		...
		if ( in_array( 'all', $pa_kinds ) || in_array( 'ca_object_representations', $pa_kinds ) ) {
			...
			if ( $vs_object_ids = (string) $po_opts->getOption( 'object_ids' ) ) {
				...
				if ( sizeof( $va_object_ids ) ) {
					...
					$vs_sql_joins
					                 = "INNER JOIN ca_objects_x_object_representations AS oxor ON oxor.representation_id = o_r.representation_id";
			...
			if ( ! $quiet ) {
				print CLIProgressBar::start( $vn_rep_count = $qr_reps->numRows(),
				...
				while ( $qr_reps->nextRow() ) {
					...
					if ( ! $quiet ) {
						print CLIProgressBar::next( 1,
							_t( "Checking representation media %1", $vn_representation_id ) );
					}

					$va_media_versions = ( is_array( $pa_versions ) && sizeof( $pa_versions ) > 0 ) ? $pa_versions
						: $qr_reps->getMediaVersions( 'media' );
					foreach ( $va_media_versions as $vs_version ) {
						...
						if ( $vs_database_md5 !== $vs_file_md5 ) {
							...
							$vs_message = _t( "[Object representation][MD5 mismatch] %1; version %2 [%3]",
								$t_rep->get( "ca_objects.preferred_labels.name" ) . " ("
								. $t_rep->get( "ca_objects.idno" ) . "); representation_id={$vn_representation_id}",
								$vs_version, $vs_path );
							switch ( $ps_format ) {
								case 'text':
								default:
								...
								case 'csv':
									$va_log           = array(
										...
										( "MD5 mismatch" ),
										'"'
										. caEscapeForDelimitedOutput( $t_rep->get( "ca_objects.preferred_labels.name" )
										                              . " (" . $t_rep->get( "ca_objects.idno" ) . ")" )
										. '"',
					...
					foreach ( $va_elements as $vs_element_code => $va_element_info ) {
						...
						foreach ( $va_vals as $vn_value_id ) {
							...
							if ( $t_attr_val->getPrimaryKey() ) {
								...
								$va_media_versions = ( is_array( $pa_versions ) && sizeof( $pa_versions ) > 0 )
									? $pa_versions : $t_attr_val->getMediaVersions( 'value_blob' );
								foreach ( $va_media_versions as $vs_version ) {
									if ( ! ( $vs_path = $t_attr_val->getMediaPath( 'value_blob', $vs_version ) ) ) {
										continue;
									}

									if ( ! ( $vs_database_md5 = $t_attr_val->getMediaInfo( 'value_blob', $vs_version,
										'MD5' ) )
									) {
										continue;
									}    // skip missing MD5s - indicates empty file
									...
									if ( $vs_database_md5 !== $vs_file_md5 ) {
										...
										if ( $t_instance
											= Datamodel::getInstanceByTableNum( $t_attr->get( 'table_num' ), true )
										) {
											if ( $t_instance->load( $t_attr->get( 'row_id' ) ) ) {
												$vs_label = $t_instance->get( $t_instance->tableName()
												                              . '.preferred_labels' );
												if ( $vs_idno
										...
										$vs_message
											= _t( "[Media attribute][MD5 mismatch] %1; value_id=%2; version %3 [%4]",
											$vs_label, $vn_value_id, $vs_version, $vs_path );
										...
										switch ( $ps_format ) {
											...
											case 'csv':
												...
												$vs_report_output .= join( ( $ps_format == 'tab' ) ? "\t" : ",",
													$va_log );
												break;
										}
						...
						foreach ( $va_vals as $vn_value_id ) {
							...
							if ( $t_attr_val->getPrimaryKey() ) {
								...
								if ( $vs_database_md5 !== $vs_file_md5 ) {
									...
									$vs_label = "attribute_id={$vn_attribute_id}; value_id={$vn_value_id}";
									if ( $t_instance = Datamodel::getInstanceByTableNum( $t_attr->get( 'table_num' ),
										true )
									) {
										if ( $t_instance->load( $t_attr->get( 'row_id' ) ) ) {
											$vs_label = $t_instance->get( $t_instance->tableName()
									...
									$vs_message = _t( "[File attribute][MD5 mismatch] %1; value_id=%2; version %3 [%4]",
										$vs_label, $vn_value_id, $vs_version, $vs_path );
		...
		if ( in_array( $ps_cache, array( 'all', 'usermedia' ) ) ) {
			if ( ( $vs_tmp_directory = $o_config->get( 'ajax_media_upload_tmp_directory' ) )
				...
				if ( ! $vs_tmp_directory ) {
					...
					CLIUtils::addError( _t( 'Skipping clearing of user media cache because the configured directory at %1 does not exist',
						$vs_tmp_directory ) );
				}
			}
		}
	...
	public static function do_configuration_check( $po_opts = null ) {
		...
		CLIUtils::addMessage( _t( "Checking system configuration... this may take a while." ),
	...
	public static function reload_service_values( $po_opts = null ) {
		...
		foreach ( $va_infoservice_elements as $va_element ) {
			...
			print CLIProgressBar::start( $qr_values->numRows(),
	...
	public static function reload_ulan_records( $po_opts = null ) {
		...
		$t_importer->importDataFromSource( join( ',', $va_sources ), $vs_mapping, array(
			'format'             => 'ULAN',
	...
	public static function precache_search_index( $po_opts = null ) {
		...
		$o_db->query( "SELECT * FROM ca_sql_search_word_index FORCE INDEX(i_index_table_num)", array(),
			array( 'resultMode' => MYSQLI_USE_RESULT ) );
		CLIUtils::addMessage( _t( "Preloading index i_index_field_table_num..." ), array( 'color' => 'bold_blue' ) );
		$o_db->query( "SELECT * FROM ca_sql_search_word_index  FORCE INDEX(i_index_field_table_num)", array(),
			array( 'resultMode' => MYSQLI_USE_RESULT ) );
		CLIUtils::addMessage( _t( "Preloading index i_index_field_num..." ), array( 'color' => 'bold_blue' ) );
		$o_db->query( "SELECT * FROM ca_sql_search_word_index  FORCE INDEX(i_index_field_num)", array(),
			array( 'resultMode' => MYSQLI_USE_RESULT ) );
		...
		foreach ( $va_cached_actions as $vs_controller => $va_actions ) {
			...
			switch ( $vs_controller ) {
				...
				case 'Search':
					...
					CLIUtils::addMessage( _t( "Preloading from %1 is not supported", $vs_controller ),
						array( 'color' => 'yellow' ) );
					break;
				case 'Detail':
					...
					foreach ( $va_actions as $vs_action => $vn_ttl ) {
						if ( is_array( $va_detail_types[ $vs_action ] ) ) {
							...
							if ( $vs_url = caNavUrl( $o_request, $vs_module_path, $vs_controller, $vs_action ) ) {
								// get ids
								$va_ids = call_user_func_array( array( $vs_table, 'find' ), array(
									[ 'deleted' => 0 ],
								...
								if ( is_array( $va_ids ) && sizeof( $va_ids ) ) {
									foreach ( $va_ids as $vn_id ) {
										CLIUtils::addMessage( _t( "Preloading from %1::%2 (%3)", $vs_controller,
											$vs_action, $vn_id ), array( 'color' => 'bold_blue' ) );
										file_get_contents( $vs_site_protocol . "://" . $vs_site_hostname . $vs_url
										                   . "/$vn_id?noCache=1" );
									}
								}
							}
						} else {
							CLIUtils::addMessage( _t( "Preloading from %1::%2 failed because there is no detail configured for %2",
								$vs_controller, $vs_action ), array( 'color' => 'yellow' ) );
						}
				...
				default:
					...
					foreach ( $va_actions as $vs_action => $vn_ttl ) {
						if ( $vs_url = caNavUrl( $o_request, $vs_module_path, $vs_controller, $vs_action,
							...
							CLIUtils::addMessage( _t( "Preloading from %1::%2", $vs_controller, $vs_action ),
								array( 'color' => 'bold_blue' ) );
	...
	public static function generate_missing_guids( $po_opts = null ) {
		...
		foreach ( Datamodel::getTableNames() as $vs_table ) {
			...
			if (
				...
				|| ( $t_instance->getProperty( 'LOG_CHANGES_TO_SELF' )
				     && method_exists( $t_instance, 'getGUIDByPrimaryKey' ) )
			) {
				$qr_results = $o_db->query( "SELECT " . $t_instance->primaryKey() . " FROM "
				                            . $t_instance->tableName() );
				if ( $qr_results && ( $qr_results->numRows() > 0 ) ) {
					print CLIProgressBar::start( $qr_results->numRows(),
	...
	public static function remove_duplicate_records( $po_opts = null ) {
		...
		foreach ( $va_tables as $vs_t ) {
			if ( class_exists( $vs_t ) && method_exists( $vs_t, 'listPotentialDupes' ) ) {
				...
				if ( sizeof( $va_dupes ) ) {
					CLIUtils::addMessage( _t( 'Table %1 has %2 records that have potential duplicates.', $vs_t,
						sizeof( $va_dupes ) ), array( 'color' => 'red' ) );


					$t_instance = Datamodel::getInstance( $vs_t );

					foreach ( $va_dupes as $vs_sha2 => $va_keys ) {
						CLIUtils::addMessage( "\t" . _t( '%1 records have the checksum %2', sizeof( $va_keys ),
								$vs_sha2 ) );
						foreach ( $va_keys as $vn_key ) {
							$t_instance->load( $vn_key );
							CLIUtils::addMessage( "\t\t" . $t_instance->primaryKey() . ': '
						...
						if ( $vb_delete_opt ) {
							...
							if ( $vn_entity_id ) {
								CLIUtils::addMessage( "\t" . _t( "Successfully consolidated them under id %1",
										$vn_entity_id ), array( 'color' => 'green' ) );
							} else {
								CLIUtils::addMessage( "\t"
					...
					CLIUtils::addMessage( _t( 'Table %1 does not seem to have any duplicates!', $vs_t ),
		...
		foreach ( $va_elements as $va_element ) {
			...
			foreach ( $va_type_res_list as $va_type_res ) {
				...
				if ( $va_type_res['type_id'] > 0 ) {
					$qr_res = call_user_func( "{$vs_table_name}::find", [ "type_id" => (int) $va_type_res['type_id'] ],
				...
				while ( $qr_res->nextHit() ) {
					$va_value_list = $qr_res->get( "{$vs_table_name}.{$vs_root_code}",
						[ 'returnWithStructure' => true, 'convertCodesToDisplayText' => true ] );
					foreach ( $va_value_list as $vn_row_id => $va_values_by_attribute_id ) {
						CLIUtils::addMessage( _t( 'Processing row %1 for %2.%3', $vn_row_id, $vs_root_code,
							$va_element['element_code'] ) );
						foreach ( $va_values_by_attribute_id as $vn_attr_id => $va_values ) {
							$vs_processed_value = caProcessTemplate( $va_element['settings']['dependentValueTemplate'],
								$va_values );
							...
							if ( $t_instance->numErrors() > 0 ) {
								CLIUtils::addError( _t( "Could not update dependent value: %1",
	...
	public static function check_relationship_type_roots( $po_opts = null ) {
		...
		foreach ( $va_tables as $vs_table ) {
			...
			if ( ! $t_root = ca_relationship_types::find( [ 'parent_id' => null, 'table_num' => $vn_table_num ],
				...
				if ( $t_root->numErrors() ) {
					CLIUtils::addError( _t( "Could not create root for relationship %1: %2", $vs_table,
				...
				if ( $t_root->numErrors() ) {
					CLIUtils::addError( _t( "Could not add label to root for relationship %1: %2", $vs_table,
						join( '; ', $t_root->getErrors() ) ) );
					continue;
				}
		...
		CLIUtils::addMessage( ( $c == 1 ) ? _t( "Recreated %1 root record", $c )
			: _t( "Recreated %1 root records", $c ) );
	}
		...
		foreach ( $tables as $table ) {
			...
			while ( $qr->nextHit() ) {
				if ( $t->load( $qr->getPrimaryKey() ) ) {
					print CLIProgressBar::next( 1,
						_t( 'Processing %1', $t->getWithTemplate( "^{$table}.preferred_labels (^{$table}.idno)" ) ) );
	...
	public static function reload_object_current_location_dates( $po_opts = null ) {
		...
		if ( $vs_movement_storage_element = $o_config->get( 'movement_storage_location_date_element' ) ) {
			...
			while ( $qr_movements->nextHit() ) {
				if ( $va_dates = $qr_movements->get( "ca_movements.{$vs_movement_storage_element}",
					[ 'returnAsArray' => true, 'rawDate' => true ] )
				) {
					$va_date = array_shift( $va_dates );

					// get movement-object relationships
					if ( is_array( $va_rel_ids = $qr_movements->get( 'ca_movements_x_objects.relation_id',
							[ 'returnAsArray' => true ] ) )
					     && sizeof( $va_rel_ids )
					) {
						$qr_res = $o_db->query(
					...
					if ( is_array( $va_rel_ids = $qr_movements->get( 'ca_movements_x_storage_locations.relation_id',
							[ 'returnAsArray' => true ] ) )
					     && sizeof( $va_rel_ids )
					) {
						$qr_res = $o_db->query(
						...
						while ( $qr_rels->nextHit() ) {
							if ( ! is_array( $va_source_info = $qr_rels->get( 'source_info' ) )
								...
								$qr_res    = $o_db->query(
									...
									array(
										caSerializeForDatabase( array(
											'path' => $qr_rels->get( 'ca_storage_locations.hierarchy.preferred_labels.name',
												array( 'returnAsArray' => true ) ),
											'ids'  => $qr_rels->get( 'ca_storage_locations.hierarchy.location_id',
												array( 'returnAsArray' => true ) )
										) ),
										$vn_rel_id
									)
		...
		while ( $qr_loc_rels->nextHit() ) {
			if ( ! $qr_loc_rels->get( 'ca_objects_x_storage_locations.effective_date' ) ) {
				if ( ( $ts = $qr_loc_rels->get( 'ca_objects_x_storage_locations.lastModified' ) )
				     && ( $start_end
						= caDateToHistoricTimestamps( $ts ) )
				) {
					try {
						$qr_res = $o_db->query(
				...
				while ( $qr_res->nextHit() ) {
					...
					foreach ( $va_value_list as $vn_row_id => $va_values_by_attribute_id ) {
						foreach ( $va_values_by_attribute_id as $vn_attr_id => $va_values ) {
							...
							if ( ! isset( $va_values[ $va_element['element_code'] ] )
							     && ( $va_element['element_code'] === $vs_root_code )
							) {
							...
							$c ++;
							if ( $t_instance->numErrors() > 0 ) {
								CLIUtils::addError( _t( "Could not set default value: %1",
									join( "; ", $t_instance->getErrors() ) ) );
							}
						}
					}
