	...
	public static function precache_simple_services( $po_opts = null ) {
		...
		foreach ( $va_endpoints as $vs_endpoint => $va_endpoint_info ) {
			if ( $va_precache_config = caGetOption( 'precache', $va_endpoint_info, null ) ) {
				if ( ! ( $t_instance = Datamodel::getInstanceByTableName( $vs_table = $va_endpoint_info['table'],
					true ) )
				) {
					continue;
				}
					...
					case 'refineablesearch':
						if ( isset( $va_precache_config['searches'] ) && is_array( $va_precache_config['searches'] ) ) {
							foreach ( $va_precache_config['searches'] as $vs_search ) {
								if ( sizeof( $va_tags = caGetTemplateTags( $vs_search, [ 'stripOptions' => true ] ) )
								     > 0
								) {
									$va_vals = [];
									foreach ( $va_tags as $vs_tag ) {
										...
										foreach ( $va_tag_vals as $vs_val ) {
											...
											file_get_contents(
												$vs_url = $o_app_conf->get( 'site_protocol' ) . "://{$vs_auth}"
												          . $o_app_conf->get( 'site_hostname' ) . '/'
												          . $o_app_conf->get( 'ca_url_root' )
												          . "/service.php/simple/{$vs_endpoint}?noCache=1&q="
												          . urlencode( $vs_search_proc ) );
											CLIUtils::addMessage( _t( "[" . $t_instance->getProperty( 'NAME_PLURAL' )
											                          . "] Cached endpoint %1 for search %2",
												$vs_endpoint, $vs_search_proc ) );
										}
									}
								} else {
									file_get_contents( $vs_url = $o_app_conf->get( 'site_protocol' ) . "://{$vs_auth}"
									                             . $o_app_conf->get( 'site_hostname' ) . '/'
									                             . $o_app_conf->get( 'ca_url_root' )
									                             . "/service.php/simple/{$vs_endpoint}?noCache=1&q="
									                             . urlencode( $vs_search ) );
									CLIUtils::addMessage( _t( "[" . $t_instance->getProperty( 'NAME_PLURAL' )
									                          . "] Cached endpoint %1 for search %2", $vs_endpoint,
										$vs_search ) );
								}
							}
						}
						...
						if ( $qr_res = $vs_table::find( '*', [ 'returnAs' => 'searchResult' ] ) ) {
							while ( $qr_res->nextHit() ) {
								file_get_contents( $vs_url = $o_app_conf->get( 'site_protocol' ) . "://{$vs_auth}"
								                             . $o_app_conf->get( 'site_hostname' ) . '/'
								                             . $o_app_conf->get( 'ca_url_root' )
								                             . "/service.php/simple/{$vs_endpoint}/id/"
								                             . $qr_res->get( $vs_pk ) );
								CLIUtils::addMessage( _t( "[" . $t_instance->getProperty( 'NAME_PLURAL' )
								                          . "] Cached endpoint %1: %2", $vs_endpoint,
