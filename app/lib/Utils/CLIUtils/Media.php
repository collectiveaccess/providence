...
trait CLIUtilsMedia {
	...
	public static function reprocess_media( $po_opts = null ) {
		...
		$pa_kinds     = caGetOption( 'kinds', $po_opts, 'all', [
			'forceLowercase' => true,
		...
		if ( in_array( 'all', $pa_kinds ) || in_array( 'ca_object_representations', $pa_kinds ) ) {
			...
			if ( sizeof( $va_ids ) ) {
				...
				if (
					( ( $vn_start > 0 ) && ( $vn_end > 0 ) && ( $vn_start <= $vn_end ) )
					|| ( ( $vn_start > 0 )
					     && ( $vn_end == null ) )
				) {
			...
			if ( $vs_object_ids = (string) $po_opts->getOption( 'object_ids' ) ) {
				...
				$vs_sql_where = ( $vs_sql_where ? "WHERE " : " AND " )
				                . "(ca_objects_x_object_representations.object_id IN (?))";
				$vs_sql_joins
				              = "INNER JOIN ca_objects_x_object_representations ON ca_objects_x_object_representations.representation_id = ca_object_representations.representation_id";
			...
			while ( $qr_reps->nextRow() ) {
				...
				if ( ! $quiet ) {
					$vs_message = _t( "Re-processing %1",
						( $vs_original_filename ? $vs_original_filename . " (" . $qr_reps->get( 'representation_id' )
		...
		if ( ( in_array( 'all', $pa_kinds ) || in_array( 'ca_attributes', $pa_kinds ) )
			...
			$va_elements = ca_metadata_elements::getElementsAsList( false, null, null, true, false, true,
				array( 16 ) ); // 16=media

			if ( is_array( $va_elements ) && sizeof( $va_elements ) ) {
				if ( is_array( $va_element_ids = caExtractValuesFromArrayList( $va_elements, 'element_id',
					...
					foreach ( $va_elements as $vs_element_code => $va_element_info ) {
						$qr_vals = $o_db->query( "SELECT value_id FROM ca_attribute_values WHERE element_id = ?",
						...
						foreach ( $va_vals as $vn_value_id ) {
							...
							if ( $t_attr_val->getPrimaryKey() ) {
								...
								$t_attr_val->set( 'value_blob', $t_attr_val->getMediaPath( 'value_blob', 'original' ),
									array( 'original_filename' => $vs_original_filename ) );
		...
		if ( ( in_array( 'all', $pa_kinds ) || in_array( 'icons', $pa_kinds ) ) && ( ! $vn_start && ! $vn_end ) ) {
			...
			foreach ( $icon_tables as $icon_table ) {
				...
				if ( ! $quiet ) {
					print CLIProgressBar::start( $icon_table::find( '*', [ 'returnAs' => 'count' ] ),
				...
				foreach ( $ids as $id ) {
					if ( $t_instance->load( $id ) ) {
						...
						$t_instance->set( 'icon', ( $p = $t_instance->getMediaPath( 'icon', 'original' ) ) ? $p
							: $t_instance->getMediaPath( 'icon', 'iconlarge' ) );

						$t_instance->update();
						if ( $t_instance->numErrors() ) {
							$vs_message = _t( "Error processing icon media: %1",
	...
	public static function reindex_pdfs( $po_opts = null ) {
		...
		if ( ( in_array( 'all', $va_kinds ) || in_array( 'ca_object_representations', $va_kinds ) )
			...
			while ( $qr_reps->nextRow() ) {
				...
				print CLIProgressBar::next( 1, _t( "Reindexing PDF %1",
					( $vs_original_filename ? $vs_original_filename . " (" . $qr_reps->get( 'representation_id' ) . ")"
						: $qr_reps->get( 'representation_id' ) ) ) );
				...
				$m = new Media();
				if ( ( $m->read( $vs_path = $t_rep->getMediaPath( 'media', 'original' ) ) )
				     && is_array( $va_locs = $m->getExtractedTextLocations() )
				) {
					...
					foreach ( $va_locs as $vs_content => $va_loc_list ) {
						foreach ( $va_loc_list as $va_loc ) {
							MediaContentLocationIndexer::index( $vn_rep_table_num, $vn_rep_id, $vs_content,
								$va_loc['p'], $va_loc['x1'], $va_loc['y1'], $va_loc['x2'], $va_loc['y2'] );
						}
					}
			...
			foreach ( $va_elements as $vs_element_code => $va_element_info ) {
				...
				foreach ( $va_vals as $vn_value_id ) {
					...
					if ( $t_attr_val->getPrimaryKey() ) {
						...
						print CLIProgressBar::next( 1, _t( "Reindexing %1",
							( $vs_original_filename ? $vs_original_filename . " ({$vn_value_id})" : $vn_value_id ) ) );

						$m = new Media();
						if ( ( $m->read( $vs_path = $t_attr_val->getMediaPath( 'value_blob', 'original' ) ) )
						     && is_array( $va_locs = $m->getExtractedTextLocations() )
						) {
							MediaContentLocationIndexer::clear( $vn_attr_table_num, $vn_attr_table_num );
							foreach ( $va_locs as $vs_content => $va_loc_list ) {
								foreach ( $va_loc_list as $va_loc ) {
									MediaContentLocationIndexer::index( $vn_attr_table_num, $vn_value_id, $vs_content,
										$va_loc['p'], $va_loc['x1'], $va_loc['y1'], $va_loc['x2'], $va_loc['y2'] );
								}
							}
							MediaContentLocationIndexer::write();
