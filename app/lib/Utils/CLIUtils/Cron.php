...
trait CLIUtilsCron { 
	...
	public static function process_task_queue($po_opts=null) {
		return TaskQueue::run(['quiet' => (bool)$po_opts->getOption("quiet"), 'restart' => $po_opts->getOption("restart")]);
	}
	# -------------------------------------------------------
	/**
	 *
	...
	public static function list_task_queue($po_opts=null) {
		...
		while($qr_all->nextRow()){
			...
			$va_all_tasks[$va_row["task_id"]]["handler_name"] = $tq->getHandlerName($va_row['handler']);
			...
			$va_all_tasks[$va_row["task_id"]]["status"] = $tq->getParametersForDisplay($va_row);
		}
