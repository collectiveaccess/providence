...
trait CLIUtilsImportExport {
	...
	public static function import_media( $po_opts = null ) {
		...
		if ( ( $vs_add_to_set = $po_opts->getOption( 'add-to-set' ) )
		     && ( ! ( $t_set
				= ca_sets::find( [ 'set_code' => $vs_add_to_set ], [ 'returnAs' => 'firstModelInstance' ] ) ) )
		) {
		...
		if ( $vs_user_name = $po_opts->getOption( 'username' ) ) {
			if ( $t_user = ca_users::find( [ 'user_name' => $vs_user_name ],
		...
		if ( ! in_array( $vs_import_target_idno_mode,
		...
		if ( ! in_array( $vs_representation_idno_mode,
			[ 'AUTO', 'FILENAME', 'FILENAME_NO_EXT', 'DIRECTORY_AND_FILENAME' ] )
		) {
		...
		if ( ! $vn_type_id ) {
			...
			CLIUtils::addMessage( _t( 'Setting target type to default %1',
				$t_instance->getTypeCodeForID( $vn_type_id ) ) );
		}
		...
		if ( ! $vn_rep_type_id ) {
			...
			CLIUtils::addMessage( _t( 'Setting representation type to default %1',
				$t_rep->getTypeCodeForID( $vn_rep_type_id ) ) );
		}
		...
		if ( ! $vn_rep_access ) {
			...
			CLIUtils::addMessage( _t( 'Setting representation access to default %1',
				caGetListItemByIDForDisplay( $vn_id ) ) );
		}
		...
		if ( ! $vn_rep_status ) {
			...
			CLIUtils::addMessage( _t( 'Setting representation status to default %1',
				caGetListItemByIDForDisplay( $vn_id ) ) );
		}
		...
		$va_counts = caGetDirectoryContentsCount( $vs_data_source,
			(bool) $po_opts->getOption( 'include-subdirectories' ) );

		if ( (bool) $po_opts->getOption( 'include-subdirectories' ) ) {
			CLIUtils::addMessage( _t( 'Found %1 files in %2 directories', $va_counts['files'],
		...
		if ( ! BatchProcessor::importMediaFromDirectory( null, $va_opts ) ) {
			CLIUtils::addError( _t( "Could not import media from %1: %2", $vs_data_source,
				join( "; ", BatchProcessor::getErrorList() ) ) );
	...
	public static function import_mediaParamList() {
		...
		$access_status_values  = caGetListItems( 'access_statuses',
			[ 'index' => 'item_value', 'value' => 'name_plural' ] );
		$access_status_default = caGetListItems( 'access_statuses',
		...
		$workflow_status_values  = caGetListItems( 'workflow_statuses',
			[ 'index' => 'item_value', 'value' => 'name_plural' ] );
		$workflow_status_default = caGetListItems( 'workflow_statuses',
			[ 'index' => 'item_value', 'value' => 'name_plural', 'defaultOnly' => true ] );
		...
		$object_type_default = caGetListItems( 'object_types',
			[ 'index' => 'item_value', 'value' => 'name_plural', 'defaultOnly' => true ] );
		...
		$representation_type_values  = caGetListItems( 'object_representation_types',
			[ 'index' => 'item_value', 'value' => 'name_plural' ] );
		$representation_type_default = caGetListItems( 'object_representation_types',
			[ 'index' => 'item_value', 'value' => 'name_plural', 'defaultOnly' => true ] );
		...
		return array(
			...
			'import-target-access|a-s'         => _t( 'Set access for newly created target records. Possible values are %1. Default is %2.',
				$access_status_list_str, $access_status_default_str ),
			'import-target-status|w-s'         => _t( 'Set status for newly created target records. Possible values are %1. Default is %2.',
				$workflow_status_list_str, $workflow_status_default_str ),
			'representation-type|T-s'          => _t( 'Type to use for all newly created representations. Possible values are %1. Default is %2.',
				$representation_type_list_str, $representation_type_default_str ),
			...
			'representation-access|A-s'        => _t( 'Set access for newly created representations. Possible values are %1. Default is %2.',
				$access_status_list_str, $access_status_default_str ),
			'representation-status|W-s'        => _t( 'Set status for newly created representations. Possible values are %1. Default is %2.',
				$workflow_status_list_str, $workflow_status_default_str ),
	...
	public static function load_import_mapping( $po_opts = null ) {
		...
		if ( ! ( $t_importer = ca_data_importers::loadImporterFromFile( $vs_file_path, $va_errors,
			array( 'logDirectory' => $vs_log_dir, 'logLevel' => $vn_log_level ) ) )
		) {
			...
			if ( is_array( $va_errors ) && ( sizeof( $va_errors ) > 0 ) ) {
				CLIUtils::addMessage( CLIUtils::textWithColor( _t( "There were warnings when adding mapping from file '%1': %2",
					$vs_file_path, join( "; ", $va_errors ) ), 'yellow' ) );
			}

			CLIUtils::addMessage( _t( "Created mapping %1 from %2",
				CLIUtils::textWithColor( $t_importer->get( 'importer_code' ), 'yellow' ), $vs_file_path ),
				array( 'color' => 'none' ) );
	...
	public static function import_data( $po_opts = null ) {
		...
		if ( ! $t_importer->importDataFromSource( $vs_data_source, $vs_mapping,
			...
			CLIUtils::addError( _t( "Could not import source %1: %2", $vs_data_source,
				join( "; ", $t_importer->getErrorList() ) ) );
	...
	public static function load_export_mapping( $po_opts = null ) {
		...
		if ( ! ( $t_exporter = ca_data_exporters::loadExporterFromFile( $vs_file_path, $va_errors ) ) ) {
			...
			print _t( "Created mapping %1 from %2",
					CLIUtils::textWithColor( $t_exporter->get( 'exporter_code' ), 'yellow' ), $vs_file_path ) . "\n";
	...
	public static function export_data( $po_opts = null ) {
		...
		if ( $vb_rdf ) {
			...
			if ( ca_data_exporters::exportRDFMode( $vs_config, $vs_filename,
				array( 'showCLIProgressBar' => true, 'logDirectory' => $vs_log_dir, 'logLevel' => $vn_log_level ) )
			) {
		...
		if ( $vs_search ) {
			if ( ! ca_data_exporters::exportRecordsFromSearchExpression( $vs_mapping, $vs_search, $vs_filename,
				array( 'showCLIProgressBar' => true, 'logDirectory' => $vs_log_dir, 'logLevel' => $vn_log_level ) )
			) {
			...
			if ( $vs_export = ca_data_exporters::exportRecord( $vs_mapping, $vs_id,
				array( 'singleRecord' => true, 'logDirectory' => $vs_log_dir, 'logLevel' => $vn_log_level ) )
			) {
	...
	public static function load_chenhall_nomenclature( $po_opts = null ) {
		...
		$is_update
			= (bool) $po_opts->getOption( 'update' );    // "update" parameter; we only allow updating of a list if this is explicitly set
		...
		if ( ! ( $t_list = ca_lists::find( [ 'list_code' => $list_code ], [ 'returnAs' => 'firstModelInstance' ] ) ) ) {
			...
			if ( $t_list->numErrors() ) {
				CLIUtils::addError( _t( "Could not create list %1: %2", $list_code,
			...
			if ( $t_list->numErrors() ) {
				CLIUtils::addError( _t( "Could not label list %1: %2", $list_code,
					join( "; ", $t_list->getErrors() ) ) );
			...
			if ( ! $is_update ) {
				CLIUtils::addError( _t( "List %1 is not empty. The Chenhall Nomenclature may only be imported into an empty list.",
					$list_code ) );
		...
		while ( $o_rows->valid() && ( $o_row = $o_rows->current() ) ) {
			...
			$o_rows->next();
			print CLIProgressBar::next( 1,
				_t( $is_existing_item ? 'Updated preferred term %1' : 'Added preferred term %1', $data[ $level ] ) );
			if ( ! sizeof( $data ) ) {
				continue;
			}
			...
			if ( $is_update ) {
				...
				if ( $t_item = ca_list_items::find( [ 'list_id' => $list_id, 'idno' => mb_substr( $id, 0, 255 ) ],
					[ 'returnAs' => 'firstModelInstance' ] )
				) {
					if ( ( $t_item->get( 'ca_list_items.preferred_labels.name_plural' ) !== $data[ $level ] )
					     || ( $t_item->get( 'ca_list_items.preferred_labels.description' ) !== $description )
					) {
						if ( ! $t_item->replaceLabel( [
							...
							CLIUtils::addError( _t( "Could not update term %1: %2", $data[ $level ],
					...
					if ( ! $t_item->removeAllLabels( __CA_LABEL_TYPE_NONPREFERRED__ ) ) {
						CLIUtils::addError( _t( "Could not remove nonpreferred labels for update for term %1: %2",
							$data[ $level ], join( "; ", $t_item->getErrors() ) ) );
					}
					...
					if ( $parent_id != $t_item->get( 'ca_list_items.parent_id' ) ) {
						...
						if ( ! $t_item->update() ) {
							CLIUtils::addError( _t( "Could not update parent for term %1: %2", $data[ $level ],
								join( "; ", $t_item->getErrors() ) ) );
						}
					}
				}
			...
			if ( ! $t_item ) {
				if ( ! ( $t_item = $t_list->addItem( $data[ $level ], true, false, $parent_id, null, $id, '', 0,
					1 ) )
				) {
					CLIUtils::addError( _t( "Could not add term %1: %2", $data[ $level ],
						join( "; ", $t_list->getErrors() ) ) );
					print_R( $data );
					continue;
				}
			...
			if ( ! $is_existing_item ) {
				if ( ! $t_item->addLabel( [
					...
					CLIUtils::addError( _t( "Could not add term label %1: %2", $data[ $level ],
						join( "; ", $t_list->getErrors() ) ) );
					print_R( $data );
					continue;
				}
			...
			if ( is_array( $non_preferred_terms ) ) {
				foreach ( $non_preferred_terms as $npt ) {
					if ( ! ( $t_item->addLabel( [ 'name_singular' => $npt, 'name_plural' => $npt, 'description' => '' ],
						$locale_id, null, false ) )
					) {
						CLIUtils::addError( _t( "Could not add non-preferred term %1 to %2: %3", $npt, $data[ $level ],
							join( "; ", $t_list->getErrors() ) ) );
						continue;
					}
				}
	...
	public static function write_exporter_to_file( $po_opts = null ) {
		...
		if ( $file
		     && ( ( file_exists( $file ) && ! is_writeable( $file ) )
		          || ( ! file_exists( $file )
