...
trait CLIUtilsImportExport
{
    ...
    public static function import_mediaParamList()
    {
        ...
        return array(
            ...
            'import-target-idno-mode|m-s' => _t(
    ...
    public static function import_data($po_opts = null)
    {
        ...
        if (!$t_importer->importDataFromSource(
            ...
            [
                ...
                'logDirectory' => $vs_log_dir,
                'logLevel' => $po_opts->getOption('log-level'),
            ...
            CLIUtils::addError(
                _t("Could not import source %1: %2", $vs_data_source, join("; ", $t_importer->getErrorList()))
            );
            return false;
        } else {
    ...
    public static function import_dataParamList()
    {
        return array(
            ...
            "detailed-log-name" => _t(
                'Name to use for detailed field-level error log. By default these log files are named with the date and code for the import mapping.'
            )
        );
    }
    # -------------------------------------------------------
    ...
    public static function load_chenhall_nomenclature($po_opts = null)
    {
        ...
        while ($o_rows->valid() && ($o_row = $o_rows->current())) {
            ...
            print CLIProgressBar::next(
                1,
                _t(
                    $is_existing_item ? 'Updated preferred term %1' : 'Added preferred term %1',
                    $data[$level]
                )
            );
            if (!sizeof($data)) {
            ...
            if ($is_update) {
                ...
                if ($t_item = ca_list_items::find(
                    ['list_id' => $list_id, 'idno' => mb_substr($id, 0, 255)],
                    ['returnAs' => 'firstModelInstance']
                )) {
                    if (($t_item->get('ca_list_items.preferred_labels.name_plural') !== $data[$level]) || ($t_item->get(
                                'ca_list_items.preferred_labels.description'
                            ) !== $description)) {
                        if (!$t_item->replaceLabel(
                            [
                                'name_singular' => $data[$level],
                                'name_plural' => $data[$level],
                            ...
                            CLIUtils::addError(
                                _t("Could not update term %1: %2", $data[$level], join("; ", $t_item->getErrors()))
                            );
                        }
                    }
            ...
            $non_preferred_terms = array_filter(
                $non_preferred_terms,
                function ($v) {
                    return strlen($v);
                }
            ...
            if (!$t_item) {
                if (!($t_item = $t_list->addItem($data[$level], true, false, $parent_id, null, $id, '', 0, 1))) {
                    CLIUtils::addError(
                        _t("Could not add term %1: %2", $data[$level], join("; ", $t_list->getErrors()))
                    );
                    print_R($data);
                    continue;
                }
            ...
            if (!$is_existing_item) {
                if (!$t_item->addLabel(
                    ['name_singular' => $data[$level], 'name_plural' => $data[$level], 'description' => $definition],
                    $locale_id,
                    null,
                    true
                )) {
                    CLIUtils::addError(
                        _t("Could not add term label %1: %2", $data[$level], join("; ", $t_list->getErrors()))
                    );
                    print_R($data);
                    continue;
                }
            ...
            if (is_array($non_preferred_terms)) {
                foreach ($non_preferred_terms as $npt) {
                    if (!($t_item->addLabel(
                        ['name_singular' => $npt, 'name_plural' => $npt, 'description' => ''],
                        $locale_id,
                        null,
                        false
                    ))) {
    ...
    {
        return _t(
            'Loads Chenhall Nomenclature from Excel XLSX format file into the specified list. You can obtain a copy of the Nomenclature from https://nomenclature.info.'
        );
    }
    # -------------------------------------------------------
    ...
    public static function write_exporter_to_file($po_opts = null)
    {
        ...
        if ($file && ((file_exists($file) && !is_writeable($file)) || (!file_exists($file) && !is_writeable(
