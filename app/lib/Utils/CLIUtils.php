...
class CLIUtils extends CLIBaseUtils
{
    ...
    public static function installParamList()
    {
        return array(
            ...
            "profile-directory|p=s" => _t(
                'Directory to get profile. Default is: "%1". This directory must contain the profile.xsd schema so that the installer can validate the installation profile.',
                __CA_BASE_DIR__ . '/install/profiles/xml'
            ),
            "admin-email|e=s" => _t('Email address of the system administrator (user@domain.tld).'),
            "overwrite" => _t(
                'Flag must be set in order to overwrite an existing installation.  Also, the __CA_ALLOW_INSTALLER_TO_OVERWRITE_EXISTING_INSTALLS__ global must be set to a true value.'
            ),
...
\tIf the database schema already exists, this operation will fail, unless the --overwrite flag is set, in which case all existing data will be deleted (use with caution!)."
    ...
    public static function update_installation_profileHelp()
    {
        return _t("Updates the configuration to match a supplied profile name.") . "\n" .
            "\t" . _t(
                "This function only creates new values and is useful if you want to append changes from one profile onto another."
            ) . "\n" .
            "\t" . _t(
                "Your new profile must exist in a directory that contains the profile.xsd schema and must validate against that schema in order for the update to apply successfully."
            );
    }
    # -------------------------------------------------------
    ...
    {
        return _t(
            "Updates the installation profile to match a supplied profile name. Backup your database before you use this!"
        );
    }
    ...
    public static function rebuild_search_indexParamList()
    {
        return array(
            "tables|t-s" => _t(
                'Specific tables to reindex, separated by commas or semicolons. If omitted all tables will be reindexed.'
            )
        );
    }
    # -------------------------------------------------------
    ...
    public static function rebuild_sort_values()
    {
        ...
        foreach (
            ...
            if ($vs_label_table_name = $t_table->getLabelTableName()) {
                ...
                print CLIProgressBar::start(
                    $qr_labels->numRows(),
                    _t('Processing %1', $t_label->getProperty('NAME_PLURAL'))
                );
            ...
            print CLIProgressBar::start(
                $qr_res->numRows(),
                _t('Processing %1 identifiers', $t_table->getProperty('NAME_SINGULAR'))
            );
...
\tNote that depending upon the size of your database reloading sort values can take from a few minutes to an hour or more. During the reloading process the system will remain usable but search and browse functions may return incorrectly sorted results."
    ...
    public static function remove_unused_media($po_opts = null)
    {
        ...
        if ($vn_delete_count == 1) {
            CLIUtils::addMessage(
                $vb_delete_opt ? _t("%1 file (%2) was deleted", $vn_delete_count, $vs_percent) : _t(
                    "%1 file (%2) is unused",
                    $vn_delete_count,
                    $vs_percent
                )
            ...
            CLIUtils::addMessage(
                $vb_delete_opt ? _t("%1 files (%2) were deleted", $vn_delete_count, $vs_percent) : _t(
                    "%1 files (%2) are unused",
                    $vn_delete_count,
                    $vs_percent
                )
            );
    ...
    {
        return _t(
            "Detects and, optionally, removes media present in the media directories but not referenced in the database."
        );
    }
    # -------------------------------------------------------
    ...
    public static function remove_deleted_representations($po_opts = null)
    {
        ...
        if ($vb_delete_opt && ($qr_reps->numRows() > 0)) {
            ...
            CLIUtils::addMessage(
                _t(
                    "%1 files are referenced by %2 deleted records. Both the records and the files will be deleted if you re-run the script with the -d (--delete) option and the correct permissions.",
                    sizeof($va_paths),
                    $qr_reps->numRows()
                )
            );
            CLIUtils::addMessage(
                _t("It is highly recommended to create a full backup before you do this!"),
                array('color' => 'bold_red')
            );
        }
    }
    # -------------------------------------------------------
    ...
    public static function remove_deleted_representationsParamList()
    {
        return array(
            "delete|d" => _t(
                'Removes representations marked as deleted. Default is false. Note that the system user that runs this script has to be able to write/delete the referenced media files if you want them to be removed.'
            )
        );
    }
    # -------------------------------------------------------
    ...
    {
        return _t(
            "Detects and, optionally, completely removes object representations marked as deleted in the database. Files referenced by these records are also removed."
        );
    }
    # -------------------------------------------------------
    ...
    {
        return _t(
            "Detects and, optionally, completely removes object representations marked as deleted in the database. Files referenced by these records are also removed. This can be useful if there has been a lot of fluctuation in your representation stock and you want to free up disk space."
        );
    }
    # -------------------------------------------------------
    ...
    public static function purge_deleted($po_opts = null)
    {
        CLIUtils::addMessage(
            _t(
                ...
                $vn_current_revision,
                __CollectiveAccess_Schema_Rev__
            )
        );
        flush();
        ob_flush();
        ...
        $o_db = new Db();
        $va_tables_to_process = array_filter(
            array_map("trim", preg_split('![ ,;]!', (string)$po_opts->getOption('tables'))),
            "strlen"
        );

        $vn_t = 0;
        foreach ($va_tables as $vs_table) {
            if (is_array($va_tables_to_process) && sizeof($va_tables_to_process) && !in_array(
                    $vs_table,
                    $va_tables_to_process
                )) {
                continue;
            }
            ...
            if ($qr_del->numRows() > 0) {
                print CLIProgressBar::start(
                    $qr_del->numRows(),
                    _t(
                        'Removing deleted %1 from database',
                ...
                CLIUtils::addMessage(
                    _t(
                        'Removed %1 %2',
                        $vn_c,
                        $t_instance->getProperty(($vn_c == 1) ? 'NAME_SINGULAR' : 'NAME_PLURAL')
                    ),
                    array('color' => 'green')
                );
                print CLIProgressBar::finish();
    ...
    public static function purge_deletedParamList()
    {
        return array(
            "tables|t=s" => _t(
                'List of tables for which to purge deleted records. List multiple tables names separated by commas. If no table list is provided all tables are purged.'
            )
        );
    }
    # -------------------------------------------------------
    ...
    {
        return _t(
            "Completely and permanently removes records marked as deleted in the database. Files referenced by these records are also removed. Note that this cannot be undone."
        );
    }
    # -------------------------------------------------------
    ...
    public static function export_profile($po_opts = null)
    {
        ...
        $vs_profile = ConfigurationExporter::exportConfigurationAsXML(
            $po_opts->getOption("name"),
            $po_opts->getOption("description"),
            $po_opts->getOption("base"),
            $po_opts->getOption("infoURL"),
            $vn_timestamp
        );
    ...
    public static function export_profileParamList()
    {
        return array(
            "base|b-s" => _t(
                'File name of profile to use as base profile. Omit if you do not want to use a base profile. (Optional)'
            ),
            ...
            "output|o-s" => _t(
                'File to output profile to. If omitted profile is printed to standard output. (Optional)'
            ),
    ...
    public static function reprocess_media($po_opts = null)
    {
        ...
        $pa_kinds = caGetOption(
            'kinds',
            $po_opts,
            'all',
            [
                'forceLowercase' => true,
                'validValues' => ['all', 'ca_object_representations', 'ca_attributes', 'icons'],
                'delimiter' => [',', ';']
            ]
        );
            ...
            while ($qr_reps->nextRow()) {
                ...
                if (!$quiet) {
                    print CLIProgressBar::next(
                        ...
                        _t(
                            "Re-processing %1",
                            ($vs_original_filename ? $vs_original_filename . " (" . $qr_reps->get(
                                    'representation_id'
                                ) . ")" : $qr_reps->get('representation_id'))
                        )
                    );
                }
                ...
                $t_rep->set(
                    'media',
                    $qr_reps->getMediaPath('media', 'original'),
                ...
                if ($t_rep->numErrors()) {
                    CLIUtils::addError(
                        _t("Error processing representation media: %1", join('; ', $t_rep->getErrors()))
                    );
                }
            }
            if (!$quiet) {
        ...
        if ((in_array('all', $pa_kinds) || in_array('ca_attributes', $pa_kinds)) && (!$vn_start && !$vn_end)) {
            ...
            $va_elements = ca_metadata_elements::getElementsAsList(
                false,
                null,
                null,
                true,
            ...
            if (is_array($va_elements) && sizeof($va_elements)) {
                if (is_array(
                        $va_element_ids = caExtractValuesFromArrayList(
                            $va_elements,
                            'element_id',
                            array('preserveKeys' => false)
                        )
                    ) && sizeof($va_element_ids)) {
                    $qr_c = $o_db->query(
                        "
							SELECT count(*) c
                    ...
                    foreach ($va_elements as $vs_element_code => $va_element_info) {
                        $qr_vals = $o_db->query(
                            ...
                            (int)$va_element_info['element_id']
                        );
                        ...
                        foreach ($va_vals as $vn_value_id) {
                            ...
                            if ($t_attr_val->getPrimaryKey()) {
                                ...
                                $vs_original_filename = is_array(
                                    $va_media_info
                                ) ? $va_media_info['ORIGINAL_FILENAME'] : '';
                                ...
                                if (!$quiet) {
                                    print CLIProgressBar::next(
                                        ...
                                        _t(
                                            "Re-processing %1",
                                            ($vs_original_filename ? $vs_original_filename . " ({$vn_value_id})" : $vn_value_id)
                                        )
                                    );
                                }
                                ...
                                $t_attr_val->set(
                                    'value_blob',
                                    $t_attr_val->getMediaPath('value_blob', 'original'),
                                    array('original_filename' => $vs_original_filename)
                                );
                                ...
                                if ($t_attr_val->numErrors()) {
                                    CLIUtils::addError(
                                        _t("Error processing attribute media: %1", join('; ', $t_attr_val->getErrors()))
                                    );
                                }
                            }
                        }
        ...
        if ((in_array('all', $pa_kinds) || in_array('icons', $pa_kinds)) && (!$vn_start && !$vn_end)) {
            ...
            foreach ($icon_tables as $icon_table) {
                ...
                if (!$quiet) {
                    print CLIProgressBar::start(
                        $icon_table::find('*', ['returnAs' => 'count']),
                        _t('Re-processing icons')
                    );
                }
                ...
                foreach ($ids as $id) {
                    if ($t_instance->load($id)) {
                        ...
                        if (!$quiet) {
                            print CLIProgressBar::next(1, _t("Re-processing %1 from %2", $id, $icon_table));
                        }


                        $t_instance->set(
                            'icon',
                            ($p = $t_instance->getMediaPath(
                                'icon',
                                'original'
                            )) ? $p : $t_instance->getMediaPath('icon', 'iconlarge')
                        );

                        $t_instance->update();
                        if ($t_instance->numErrors()) {
                            CLIUtils::addError(
                                _t("Error processing icon media: %1", join('; ', $t_instance->getErrors()))
                            );
                        }
                    }
                }
    ...
    public static function reprocess_mediaParamList()
    {
        return array(
            "mimetypes|m-s" => _t(
                "Limit re-processing to specified mimetype(s) or mimetype stubs. Separate multiple mimetypes with commas."
            ),
    ...
    public static function reindex_pdfs($po_opts = null)
    {
        ...
        if ((in_array('all', $va_kinds) || in_array(
                    'ca_object_representations',
                    $va_kinds
                )) && (!$vn_start && !$vn_end)) {
            if (!($vn_start = (int)$po_opts->getOption('start_id'))) {
            ...
            while ($qr_reps->nextRow()) {
                ...
                $m = new Media();
                if (($m->read($vs_path = $t_rep->getMediaPath('media', 'original'))) && is_array(
                        $va_locs = $m->getExtractedTextLocations()
                    )) {
            ...
            foreach ($va_elements as $vs_element_code => $va_element_info) {
                ...
                foreach ($va_vals as $vn_value_id) {
                    ...
                    if ($t_attr_val->getPrimaryKey()) {
                        ...
                        $m = new Media();
                        if (($m->read($vs_path = $t_attr_val->getMediaPath('value_blob', 'original'))) && is_array(
                                $va_locs = $m->getExtractedTextLocations()
                            )) {
    ...
    public static function reindex_pdfsParamList()
    {
        return array(
            ...
            "kinds|k-s" => _t(
                'Comma separated list of kind of media to reindex. Valid kinds are ca_object_representations (object representations), and ca_attributes (metadata elements). You may also specify "all" to reindex both kinds of media. Default is "all"'
            )
        );
    }
    # -------------------------------------------------------
    ...
    public static function update_database_schema($po_opts = null)
    {
        ...
        if (($vn_current_revision = ConfigurationCheck::getSchemaVersion()) < __CollectiveAccess_Schema_Rev__) {
            ...
            print CLIProgressBar::finish();
            CLIUtils::addMessage(
                _t("Database already at revision %1. No update is required.", __CollectiveAccess_Schema_Rev__)
            );
        }
    ...
    public static function load_import_mapping($po_opts = null)
    {
        ...
        if (!($t_importer = ca_data_importers::loadImporterFromFile(
            $vs_file_path,
            $va_errors,
            array(
            ...
            if (is_array($va_errors) && (sizeof($va_errors) > 0)) {
                CLIUtils::addMessage(
                    CLIUtils::textWithColor(
                        _t(
                            "There were warnings when adding mapping from file '%1': %2",
                            $vs_file_path,
                            join("; ", $va_errors)
                        ),
                        'yellow'
            ...
            CLIUtils::addMessage(
                _t(
                    "Created mapping %1 from %2",
                    CLIUtils::textWithColor($t_importer->get('importer_code'), 'yellow'),
                    $vs_file_path
                ),
                array('color' => 'none')
            );
    ...
    public static function import_data($po_opts = null)
    {
        ...
        if (($vs_add_to_set = $po_opts->getOption('add-to-set')) && (!($t_set = ca_sets::find(
                ['set_code' => $vs_add_to_set],
        ...
        if (!ca_data_importers::importDataFromSource(
            $vs_data_source,
            $vs_mapping,
            array(
                'noTransaction' => $vb_direct,
                'format' => $vs_format,
                'showCLIProgressBar' => true,
                'logDirectory' => $vs_log_dir,
                'logLevel' => $vn_log_level,
            ...
            CLIUtils::addError(
                _t("Could not import source %1: %2", $vs_data_source, join("; ", ca_data_importers::getErrorList()))
            );
            return false;
        } else {
    ...
    public static function import_dataParamList()
    {
        return array(
            "source|s=s" => _t(
                'Data to import. For files provide the path; for database, OAI and other non-file sources provide a URL.'
            ),
            "mapping|m=s" => _t('Mapping to import data with.'),
            "format|f-s" => _t(
                'The format of the data to import. (Ex. XLSX, tab, CSV, mysql, OAI, Filemaker XML, ExcelXML, MARC). If omitted an attempt will be made to automatically identify the data format.'
            ),
            "log|l-s" => _t('Path to directory in which to log import details. If not set no logs will be recorded.'),
            "log-level|d-s" => _t(
                'Logging threshold. Possible values are, in ascending order of important: DEBUG, INFO, NOTICE, WARN, ERR, CRIT, ALERT. Default is INFO.'
            ...
            "log-to-tmp-directory-as-fallback" => _t(
                'Use the system temporary directory for the import log if the application logging directory is not writable. Default report an error if the application log directory is not writeable.'
            )
        );
    }
    # -------------------------------------------------------
    ...
    {
        return _t(
            "Import data from many types of data sources including other CollectiveAccess systems, MySQL databases and Excel, delimited text, XML and MARC files."
        );
    }
    # -------------------------------------------------------
    ...
    public static function load_export_mapping($po_opts = null)
    {
        ...
        if (!($t_exporter = ca_data_exporters::loadExporterFromFile($vs_file_path, $va_errors))) {
            ...
            print _t(
                    "Created mapping %1 from %2",
                    CLIUtils::textWithColor($t_exporter->get('exporter_code'), 'yellow'),
                    $vs_file_path
                ) . "\n";
            return true;
        }
    ...
    public static function export_data($po_opts = null)
    {
        ...
        if (!$vb_rdf && !$vs_search && !$vs_id) {
            print _t(
                    'You must specify either an idno or a search expression to select a record or record set for export or activate RDF mode.'
                ) . "\n";
            return false;
        }
        ...
        if ($vb_rdf) {
            if (!($vs_config = $po_opts->getOption('config'))) {
                print _t(
                        'You must specify a configuration file that contains the export definition for the RDF mode.'
                    ) . "\n";
                return false;
            }
            ...
            if (ca_data_exporters::exportRDFMode(
                $vs_config,
                $vs_filename,
                array(
                    'showCLIProgressBar' => true,
        ...
        if ($vs_search) {
            if (!ca_data_exporters::exportRecordsFromSearchExpression(
                $vs_mapping,
                $vs_search,
                $vs_filename,
                array(
            ...
            if ($vs_id) {
                if ($vs_export = ca_data_exporters::exportRecord(
                    $vs_mapping,
                    $vs_id,
                    array(
                        'singleRecord' => true,
    ...
    public static function export_dataParamList()
    {
        return array(
            ...
            "log-level|d-s" => _t(
                'Optional logging threshold. Possible values are, in ascending order of important: DEBUG, INFO, NOTICE, WARN, ERR, CRIT, ALERT. Default is INFO.'
            ),
            "rdf" => _t(
                'Switches to RDF export mode. You can use this to assemble record-level exports across authorities with multiple mappings in a single export (usually an RDF graph). -s, -i and -m are ignored and -c is required.'
            ),
    ...
    {
        return _t(
            "Replicates data in one CollectiveAccess instance based upon data in another instance, subject to configuration in replication.conf."
        );
    }
    # -------------------------------------------------------
    ...
    public static function fix_permissions($po_opts = null)
    {
        ...
        if (!$po_opts->getOption("quiet")) {
            CLIUtils::addMessage(
                _t("Fixing permissions for the temporary directory (app/tmp) for ownership by \"%1\"...", $vs_user)
            );
        }
        ...
        if (!$po_opts->getOption("quiet")) {
            CLIUtils::addMessage(
                _t("Fixing permissions for the media directory (media) for ownership by \"%1\"...", $vs_user)
            );
        }
        ...
        if (!$po_opts->getOption("quiet")) {
            CLIUtils::addMessage(
                _t(
                    "Fixing permissions for the HTMLPurifier definition cache directory (vendor/ezyang/htmlpurifier/library/HTMLPurifier/DefinitionCache/Serializer) for ownership by \"%1\"...",
                    $vs_user
                )
            );
        }
        $va_files = caGetDirectoryContentsAsList(
            $vs_path = __CA_BASE_DIR__ . '/vendor/ezyang/htmlpurifier/library/HTMLPurifier/DefinitionCache/Serializer',
            true,
            false,
            false,
            true
    ...
    public static function fix_permissionsParamList()
    {
        return array(
            "user|u=s" => _t(
                "Set ownership of directories to specifed user. If not set, an attempt will be made to determine the name of the web server user automatically. If the web server user cannot be determined the current user will be used."
            ),
            "group|g=s" => _t(
                "Set ownership of directories to specifed group. If not set, the current group will be used."
            ),
            "quiet|q" => _t("Run without outputting progress information.")
        );
    }
    ...
    {
        return _t(
            "Fix folder permissions. MUST BE RUN WHILE LOGGED IN WITH ADMINSTRATIVE/ROOT PERMISSIONS. You are currently logged in as %1 (uid %2)",
            caGetProcessUserName(),
            caGetProcessUserID()
        );
    }
    # -------------------------------------------------------
    ...
    {
        if (!($vs_user_name = (string)$po_opts->getOption('user')) && !($vs_user_name = (string)$po_opts->getOption(
                'username'
            ))) {
        ...
        if ($t_user->numErrors()) {
            CLIUtils::addError(
                _t("Password change for user %1 failed: %2", $vs_user_name, join("; ", $t_user->getErrors()))
            );
            return false;
        }
    ...
    public static function load_metadata_dictionary_from_excel_file($po_opts = null)
    {
        ...
        while ($o_rows->valid() && ($o_row = $o_rows->current())) {
            ...
            if ($t_entry->numErrors()) {
                CLIUtils::addError(
                    _t("Error while adding definition for %1: %2", $va_data[0], join("; ", $t_entry->getErrors()))
                );
            }
            ...
            if ($va_data[6]) {
                ...
                foreach ($va_rules as $va_rule) {
                    ...
                    if ($t_rule->numErrors()) {
                        CLIUtils::addError(
                            _t("Error while adding rule for %1: %2", $va_data[0], join("; ", $t_rule->getErrors()))
                        );
                    } else {
                        $vn_rule_count++;
                    }
        ...
        CLIUtils::addMessage(
            _t('Added %1 entries and %2 rules', $vn_add_count, $vn_rule_count),
            array('color' => 'bold_green')
        );
        return true;
    }
    # -------------------------------------------------------
    ...
    {
        return _t(
            'Load metadata dictionary entries from an Excel file using the format described at http://docs.collectiveaccess.org/metadata_dictionary'
        );
    }
    # -------------------------------------------------------
    ...
    public static function validate_using_metadata_dictionary_rules($po_opts = null)
    {
        ...
        foreach ($va_rules as $va_rule) {
            ...
            $qr_records = call_user_func_array(
                ($vs_table_name = $t_instance->tableName()) . "::find",
                array(
                    array('deleted' => 0),
            ...
            while ($qr_records->nextHit()) {
                ...
                if ($t_found = ca_metadata_dictionary_rule_violations::find(
                    array('rule_id' => $va_rule['rule_id'], 'row_id' => $vn_id, 'table_num' => $vn_table_num),
    ...
    public static function check_media_fixity($po_opts = null)
    {
        ...
        switch ($ps_format = caGetOption(
            'format',
            $po_opts,
            'text',
            [
                'forceLowercase' => true,
                'validValues' => ['text', 'tab', 'csv'],
                'castTo' => 'string'
            ]
        )) {
            case 'tab':
        ...
        if (in_array('all', $pa_kinds) || in_array('ca_object_representations', $pa_kinds)) {
            ...
            if (!$quiet) {
                print CLIProgressBar::start(
                        $vn_rep_count = $qr_reps->numRows(),
                        _t('Checking object representations')
                    ) . "\n";
            }
            $vn_errors = 0;
            ...
            if ($qr_reps->numRows() > 0) {
                ...
                while ($qr_reps->nextRow()) {
                    ...
                    $va_media_versions = (is_array($pa_versions) && sizeof(
                            $pa_versions
                        ) > 0) ? $pa_versions : $qr_reps->getMediaVersions('media');
                    foreach ($va_media_versions as $vs_version) {
                        ...
                        if ($vs_database_md5 !== $vs_file_md5) {
                            ...
                            $vs_message = _t(
                                "[Object representation][MD5 mismatch] %1; version %2 [%3]",
                                $t_rep->get("ca_objects.preferred_labels.name") . " (" . $t_rep->get(
                                    "ca_objects.idno"
                            ...
                            switch ($ps_format) {
                                ...
                                case 'csv':
                                    $va_log = array(
                                        _t('Object representation'),
                                        ("MD5 mismatch"),
                                        '"' . caEscapeForDelimitedOutput(
                                            $t_rep->get("ca_objects.preferred_labels.name") . " (" . $t_rep->get(
                                                "ca_objects.idno"
                                            ) . ")"
                                        ) . '"',
                                        $vn_representation_id,
                                        $vs_version,
                                        $vs_path,
                                        $vs_database_md5,
                                        $vs_file_md5
                    ...
                    foreach ($va_elements as $vs_element_code => $va_element_info) {
                        ...
                        foreach ($va_vals as $vn_value_id) {
                            ...
                            if ($t_attr_val->getPrimaryKey()) {
                                ...
                                $va_media_versions = (is_array($pa_versions) && sizeof(
                                        $pa_versions
                                    ) > 0) ? $pa_versions : $t_attr_val->getMediaVersions('value_blob');
                                foreach ($va_media_versions as $vs_version) {
                                    if (!($vs_path = $t_attr_val->getMediaPath('value_blob', $vs_version))) {
                                        continue;
                                    }

                                    if (!($vs_database_md5 = $t_attr_val->getMediaInfo(
                                        'value_blob',
                                        $vs_version,
                                        'MD5'
                                    ))) {
                                    ...
                                    if ($vs_database_md5 !== $vs_file_md5) {
                                        $t_attr = new ca_attributes(
                                            $vn_attribute_id = $t_attr_val->get('attribute_id')
                                        );

                                        $vs_label = "attribute_id={$vn_attribute_id}; value_id={$vn_value_id}";
                                        if ($t_instance = Datamodel::getInstanceByTableNum(
                                            $t_attr->get('table_num'),
                                            true
                                        )) {
                                            if ($t_instance->load($t_attr->get('row_id'))) {
                                                $vs_label = $t_instance->get(
                                                    $t_instance->tableName() . '.preferred_labels'
                                                );
                                                if ($vs_idno = $t_instance->get(
                                        ...
                                        $vs_message = _t(
                                            "[Media attribute][MD5 mismatch] %1; value_id=%2; version %3 [%4]",
                                            $vs_label,
                                            $vn_value_id,
                                            $vs_version,
                                            $vs_path
                                        ...
                                        switch ($ps_format) {
                                            ...
                                            case 'csv':
                                                $va_log = array(
                                                    _t('Media attribute'),
                                                    _t("MD5 mismatch"),
                                                    '"' . caEscapeForDelimitedOutput($vs_label) . '"',
                                                    $vn_value_id,
                                                    $vs_version,
                                                    $vs_path,
                                                    $vs_database_md5,
                        ...
                        foreach ($va_vals as $vn_value_id) {
                            ...
                            if ($t_attr_val->getPrimaryKey()) {
                                ...
                                if ($vs_database_md5 !== $vs_file_md5) {
                                    ...
                                    $vs_message = _t(
                                        "[File attribute][MD5 mismatch] %1; value_id=%2; version %3 [%4]",
                                        $vs_label,
                                        $vn_value_id,
                                        $vs_version,
                                        $vs_path
            ...
            if (sizeof($counts) > 0) {
                if (!caSendMessageUsingView(
                    ...
                    [
                        'date' => $d,
                        'app_name' => $a,
                        'num_errors' => $vn_errors,
                        'counts' => caMakeCommaListWithConjunction($counts)
                    ],
                    null,
                    null,
    ...
    public static function check_media_fixityParamList()
    {
        return array(
            ...
            "file|o=s" => _t(
                'Location to write report to. The placeholder %date may be included to date/time stamp the report.'
            ),
            ...
            "kinds|k-s" => _t(
                'Comma separated list of kind of media to check. Valid kinds are ca_object_representations (object representations), and ca_attributes (metadata elements). You may also specify "all" to check both kinds of media. Default is "all"'
            )
        );
    }
    # -------------------------------------------------------
    ...
    {
        return _t(
            'Verifies that media files on disk are consistent with file signatures recorded in the database at time of upload.'
        );
    }
    # -------------------------------------------------------
    ...
    {
        return _t(
            'CollectiveAccess supports browse on current locations of collection objects using values cached in the object records. From time to time these values may become out of date. Use this command to regenerate the cached values based upon the current state of the database.'
        );
    }
    # -------------------------------------------------------
    ...
    public static function clear_caches($po_opts = null)
    {
        ...
        if (in_array($ps_cache, array('all', 'usermedia'))) {
            if (($vs_tmp_directory = $o_config->get('ajax_media_upload_tmp_directory')) && (file_exists(
                    $vs_tmp_directory
                ))) {
                ...
                if (!$vs_tmp_directory) {
                    CLIUtils::addError(
                        _t('Skipping clearing of user media cache because no cache directory is configured')
                    );
                } else {
                    CLIUtils::addError(
                        _t(
                            'Skipping clearing of user media cache because the configured directory at %1 does not exist',
                            $vs_tmp_directory
                        )
                    );
                }
    ...
    public static function clear_cachesParamList()
    {
        return array(
            "cache|c=s" => _t(
                'Which cache to clear. Use "app" for the application cache (include all user sessions); use "userMedia" for user-uploaded media; use "all" for all cached. Default is all.'
            ),
        );
    }
    # -------------------------------------------------------
    ...
    {
        return _t(
            'CollectiveAccess stores often used values, processed configuration files, user-uploaded media and other types of data in application caches. You can clear these caches using this command.'
        );
    }
    # -------------------------------------------------------
    ...
    public static function do_configuration_check($po_opts = null)
    {
        ...
        CLIUtils::addMessage(
            "\t(" . _t('GD is a graphics processing library required for all barcode generation.') . ")"
        );
        ...
        CLIUtils::addMessage(
            _t("Checking system configuration... this may take a while."),
            array('color' => 'bold_blue')
        );
    ...
    {
        return _t(
            'CollectiveAccess requires certain PHP configuration options to be set and for file permissions in several directories to be web-server writable. This command will check these settings and file permissions and return warnings if configuration appears to be incorrect.'
        );
    }
    ...
    public static function reload_service_values($po_opts = null)
    {
        ...
        foreach ($va_infoservice_elements as $va_element) {
            ...
            print CLIProgressBar::start(
                $qr_values->numRows(),
                "Reloading values for element code " . $va_element['element_code']
            );
    ...
    public static function reload_ulan_records($po_opts = null)
    {
        ...
        ca_data_importers::importDataFromSource(
            join(',', $va_sources),
            $vs_mapping,
            array(
                'format' => 'ULAN',
    ...
    {
        return _t(
            'Reload records imported from ULAN with the specified mapping. This utility assumes that the mapping is set up with an existingRecordPolicy that ensures that existing records are matched properly. It will create duplicates if it does not match existing records so be sure to test your mapping first!'
        );
    }
    ...
    public static function reload_object_current_location_dates($po_opts = null)
    {
        ...
        if ($vs_movement_storage_element = $o_config->get('movement_storage_location_date_element')) {
            ...
            while ($qr_movements->nextHit()) {
                if ($va_dates = $qr_movements->get(
                    "ca_movements.{$vs_movement_storage_element}",
                    ['returnAsArray' => true, 'rawDate' => true]
                )) {
                    $va_date = array_shift($va_dates);

                    // get movement-object relationships
                    if (is_array(
                            $va_rel_ids = $qr_movements->get(
                                'ca_movements_x_objects.relation_id',
                                ['returnAsArray' => true]
                            )
                        ) && sizeof($va_rel_ids)) {
                        $qr_res = $o_db->query(
                    ...
                    if (is_array(
                            $va_rel_ids = $qr_movements->get(
                                'ca_movements_x_storage_locations.relation_id',
                                ['returnAsArray' => true]
                            )
                        ) && sizeof($va_rel_ids)) {
                        $qr_res = $o_db->query(
                        ...
                        while ($qr_rels->nextHit()) {
                            if (!is_array(
                                    $va_source_info = $qr_rels->get('source_info')
                                ) || !isset($va_source_info['path'])) {
                                    ...
                                    array(
                                        caSerializeForDatabase(
                                            array(
                                                ...
                                                'ids' => $qr_rels->get(
                                                    'ca_storage_locations.hierarchy.location_id',
                                                    array('returnAsArray' => true)
                                                )
                                            )
                                        ),
        ...
        while ($qr_loc_rels->nextHit()) {
            if (!$qr_loc_rels->get('ca_objects_x_storage_locations.effective_date')) {
                if (($ts = $qr_loc_rels->get(
                        'ca_objects_x_storage_locations.lastModified'
                    )) && ($start_end = caDateToHistoricTimestamps($ts))) {
                    try {
                        $qr_res = $o_db->query(
                            ...
                            [
                                $start_end[0],
                                $start_end[1],
                                $qr_loc_rels->get('ca_objects_x_storage_locations.relation_id')
                            ]
                        );
                    } catch (Exception $e) {
                        // noop
    ...
    public static function precache_search_index($po_opts = null)
    {
        ...
        $o_db->query(
            "SELECT * FROM ca_sql_search_word_index FORCE INDEX(i_index_table_num)",
            array(),
            array('resultMode' => MYSQLI_USE_RESULT)
        );
        CLIUtils::addMessage(_t("Preloading index i_index_field_table_num..."), array('color' => 'bold_blue'));
        $o_db->query(
            "SELECT * FROM ca_sql_search_word_index  FORCE INDEX(i_index_field_table_num)",
            array(),
            array('resultMode' => MYSQLI_USE_RESULT)
        );
        CLIUtils::addMessage(_t("Preloading index i_index_field_num..."), array('color' => 'bold_blue'));
        $o_db->query(
            "SELECT * FROM ca_sql_search_word_index  FORCE INDEX(i_index_field_num)",
            array(),
            array('resultMode' => MYSQLI_USE_RESULT)
        );
        return true;
    ...
    {
        return _t(
            'Preload SQLSearch index into MySQL in-memory cache. This is only relevant if you are using the MySQL-based SQLSearch engine. Preloading can significantly improve performance on system with large search indices. Note that your MySQL installation must have a large enough buffer pool configured to hold the index. Loading may take several minutes.'
        );
    }
    # -------------------------------------------------------
        ...
        foreach ($va_cached_actions as $vs_controller => $va_actions) {
            ...
            switch ($vs_controller) {
                ...
                case 'Search':
                    ...
                    CLIUtils::addMessage(
                        _t("Preloading from %1 is not supported", $vs_controller),
                        array('color' => 'yellow')
                    );
                    break;
                case 'Detail':
                        ...
                        if (is_array($va_detail_types[$vs_action])) {
                            ...
                            if ($vs_url = caNavUrl($o_request, $vs_module_path, $vs_controller, $vs_action)) {
                                ...
                                $va_ids = call_user_func_array(
                                    ...
                                    array(
                                        ['deleted' => 0],
                                        ['restrictToTypes' => $va_types, 'returnAs' => 'ids']
                                    )
                                );
                                ...
                                if (is_array($va_ids) && sizeof($va_ids)) {
                                    foreach ($va_ids as $vn_id) {
                                        CLIUtils::addMessage(
                                            _t("Preloading from %1::%2 (%3)", $vs_controller, $vs_action, $vn_id),
                                            array('color' => 'bold_blue')
                                        );
                                        file_get_contents(
                                            $vs_site_protocol . "://" . $vs_site_hostname . $vs_url . "/$vn_id?noCache=1"
                                        );
                                    }
                                }
                            }
                            ...
                            CLIUtils::addMessage(
                                _t(
                                    "Preloading from %1::%2 failed because there is no detail configured for %2",
                                    $vs_controller,
                                    $vs_action
                                ),
                                array('color' => 'yellow')
                ...
                default:
                    ...
                    foreach ($va_actions as $vs_action => $vn_ttl) {
                        if ($vs_url = caNavUrl(
                            ...
                            CLIUtils::addMessage(
                                _t("Preloading from %1::%2", $vs_controller, $vs_action),
                                array('color' => 'bold_blue')
                            );
    ...
    {
        return _t(
            'Pre-loads content cache by loading each cached page url. Pre-caching may take a while depending upon the quantity of content configured for caching.'
        );
    }
    # -------------------------------------------------------
    ...
    public static function load_chenhall_nomenclature($po_opts = null)
    {
        ...
        $pb_update = (bool)$po_opts->getOption(
            'update'
        );    // "update" parameter; we only allow updating of a list if this is explicitly set
        ...
        if (!($t_list = ca_lists::find(['list_code' => $ps_list_code], ['returnAs' => 'firstModelInstance']))) {
            ...
            if ($pb_update) {
                ...
                CLIUtils::addError(
                    _t(
                        "List %1 is not empty. The Chenhall Nomenclature may only be imported into an empty list.",
                        $ps_list_code
                    )
                );
                return false;
        ...
        while ($o_rows->valid() && ($o_row = $o_rows->current())) {
            ...
            if ($vb_is_update) {
                ...
                if ($t_item = ca_list_items::find(
                    ['list_id' => $vn_list_id, 'idno' => mb_substr($vs_term, 0, 255)],
                    ['returnAs' => 'firstModelInstance']
                )) {
                    if (($t_item->get('ca_list_items.preferred_labels.name_plural') !== $vs_term) || ($t_item->get(
                                'ca_list_items.preferred_labels.description'
                            ) !== $va_data[6])) {
                        if (!$t_item->replaceLabel(
                            ['name_singular' => $vs_term, 'name_plural' => $vs_term, 'description' => $va_data[6]],
                            $vn_locale_id,
                            null,
                            true
                        )) {
                            CLIUtils::addError(
                                _t("Could not update term %1: %2", $vs_term, join("; ", $t_item->getErrors()))
                            );
                        }
                    }
                    ...
                    if (!$t_item->removeAllLabels(__CA_LABEL_TYPE_NONPREFERRED__)) {
                        CLIUtils::addError(
                            _t(
                                "Could not remove nonpreferred labels for update for term %1: %2",
                                $vs_term,
                    ...
                    if ($vn_parent_id != $t_item->get('ca_list_items.parent_id')) {
                        ...
                        if (!$t_item->update()) {
                            CLIUtils::addError(
                                _t(
                                    "Could not update parent for term %1: %2",
                                    $vs_term,
                                    join("; ", $t_item->getErrors())
                                )
                            );
            ...
            if (!$vb_is_existing_item) {
                if (!$t_item->addLabel(
                    ...
                    CLIUtils::addError(
                        _t("Could not add term label %1: %2", $vs_term, join("; ", $t_list->getErrors()))
                    );
                    continue;
                }
            }
            print CLIProgressBar::next(
                1,
                _t(
                    $vb_is_existing_item ? 'Updated preferred term %1' : 'Added preferred term %1',
                    $vs_term
                )
            );
            ...
            if (is_array($va_non_preferred_terms[$vs_term])) {
                foreach ($va_non_preferred_terms[$vs_term] as $vs_non_preferred_term) {
                    if (!($t_item->addLabel(
                        [
                            'name_singular' => $vs_non_preferred_term,
                            'name_plural' => $vs_non_preferred_term,
                            'description' => ''
                        ],
                        $vn_locale_id,
                        null,
                        ...
                        CLIUtils::addError(
                            _t(
                                "Could not add non-preferred term %1 to %2: %3",
                                $vs_non_preferred_term,
                                $vs_term,
                                join("; ", $t_list->getErrors())
                            )
                        );
                        continue;
                    }
    ...
    public static function load_chenhall_nomenclatureParamList()
    {
        return array(
            ...
            "list|l=s" => _t(
                'Code for list to load Chenhall Nomenclature into. If list with code does not exist it will be created.'
            ),
    ...
    {
        return _t(
            'Loads Chenhall Nomenclature from Excel XLSX format file into the specified list. You can obtain a copy of the Nomenclature from the American Association of State and Local History (AASLH).'
        );
    }
    # -------------------------------------------------------
        ...
        foreach (Datamodel::getTableNames() as $vs_table) {
            ...
            if (
                ...
                if ($qr_results && ($qr_results->numRows() > 0)) {
                    print CLIProgressBar::start(
                        $qr_results->numRows(),
                        _t('Generating/verifying GUIDs for table %1', $t_instance->tableName())
                    );
    ...
    {
        return _t(
            'Generates guids for all records that don\'t have one yet. This can be useful if you plan on using the data synchronization/replication feature in the future. For more info see here: http://docs.collectiveaccess.org/wiki/Replication'
        );
    }
    # -------------------------------------------------------
    ...
    public static function remove_duplicate_records($po_opts = null)
    {
        ...
        foreach ($va_tables as $vs_t) {
            if (class_exists($vs_t) && method_exists($vs_t, 'listPotentialDupes')) {
                ...
                if (sizeof($va_dupes)) {
                    CLIUtils::addMessage(
                        _t('Table %1 has %2 records that have potential duplicates.', $vs_t, sizeof($va_dupes)),
                        array('color' => 'red')
                    );
                    ...
                    foreach ($va_dupes as $vs_sha2 => $va_keys) {
                        ...
                        foreach ($va_keys as $vn_key) {
                            ...
                            CLIUtils::addMessage(
                                "\t\t" . $t_instance->primaryKey() . ': ' . $t_instance->getPrimaryKey(
                        ...
                        if ($vb_delete_opt) {
                            ...
                            if ($vn_entity_id) {
                                CLIUtils::addMessage(
                                    "\t" . _t("Successfully consolidated them under id %1", $vn_entity_id),
                                    array('color' => 'green')
                                );
                            } else {
                                CLIUtils::addMessage(
                                    "\t" . _t("It seems like there was an error while deduplicating those records"),
                                    array('color' => 'bold_red')
                                );
                            }
                        }
                    ...
                    CLIUtils::addMessage(
                        _t('Table %1 does not seem to have any duplicates!', $vs_t),
                        array('color' => 'bold_green')
                    );
                }
            }
    ...
    {
        return _t(
            'Lists and optionally removed duplicate records. For more info on how the algorithm works see here: http://docs.collectiveaccess.org/wiki/Deduplication'
        );
    }
    # -------------------------------------------------------
    ...
    public static function push_config_changes($po_opts = null)
    {
        ...
        foreach ($va_targets as $vs_target) {
            ...
            if (!isURL($vs_target)) {
                ...
                if ($o_log) {
                    $o_log->logError(
                        _t("[push-config-changes] The target '%1' doesn't seem to be in URL format", $vs_target)
                    );
                }
                return false;
            }
            ...
            if ($o_log) {
                $o_log->logDebug(
                    _t(
                        "[push-config-changes] Service endpoint is '%1'. Timestamp for diff config is %2",
                        $vs_target,
                        $vn_target_timestamp
                    )
                );
            ...
            if ($o_log) {
                $o_log->logDebug(
                    _t("[push-config-changes] Configuration fragment for target '%1' is \n %2", $vs_target, $vs_config)
                );
            }
            ...
            if ($vn_code != 200) {
                CLIUtils::addError(
                    _t("Pushing to target '%1' seems to have failed. HTTP response code was %2.", $vs_target, $vn_code)
                );
                if ($o_log) {
                    $o_log->logError(
                        _t(
                            "[push-config-changes] Pushing to target '%1' seems to have failed. HTTP response code was %2. Enable debug logging mode to get more info below.",
                            $vs_target,
                            $vn_code
                        )
                    );
            ...
            if ($o_log) {
                $o_log->logDebug(
                    _t(
                        "[push-config-changes] Decoded response from target '%1' is '%2'",
                        $vs_target,
                        print_r($va_response, true)
                    )
                );
            }
            ...
            if (!isset($va_response['ok']) || !$va_response['ok']) {
                if (is_array($va_errors = $va_response['errors'])) {
                    CLIUtils::addError(
                        _t(
                            "Pushing to target '%1' seems to have failed. Response was not marked as okay. Errors were: %2",
                            $vs_target,
                            join(',', $va_errors)
                        )
                    );
                    ...
                    CLIUtils::addError(
                        _t(
                            "Pushing to target '%1' seems to have failed. Response was not marked as okay. Raw response was: %2",
                            $vs_target,
                            $vs_exec
                        )
                    );
    ...
    public static function push_config_changesParamList()
    {
        return [
            "targets|t=s" => _t(
                'Comma- or semicolon separated list of target systems to push changes to. We assume the same service account exists on all of these systems'
            ),
            "username|u=s" => _t(
                'User name to use to log into the targets. We assume the same credentials can be used to log into all target systems.'
            ),
            "password|p=s" => _t(
                'Password to use to log into the targets. We assume the same credentials can be used to log into all target systems.'
            ),
            "timestamp|s=s" => _t(
                'Timestamp to use to filter the configuration changes that should be exported/pushed. Optional. The timestamp is only used for the very first push to that system. After that the master system will store the last push timestamp and use that instead. This parameter is a fixed offset/"starting point" of sorts.'
            ),
            "log|l-s" => _t('Path to directory in which to log import details. If not set no logs will be recorded.'),
            "log-level|d-s" => _t(
                'Logging threshold. Possible values are, in ascending order of important: DEBUG, INFO, NOTICE, WARN, ERR, CRIT, ALERT. Default is INFO.'
    ...
    {
        return _t(
            'This utility generates a new system GUID for the current system. This can be useful is you used a copy of another system to set it up and are now trying to sync/replicate data between the two. You may have to reset the system GUID for one of them in that case.'
        );
    }
    # -------------------------------------------------------
    ...
    public static function check_url_reference_integrityParamList()
    {
        return [
            "users|u=s" => _t(
                'User names to notify if there are errors. Multiple entries are delimited by comma or semicolon. Invalid or non-existing user named will be ignored. [Optional]'
            ),
            "groups|g=s" => _t(
                'Groups to notify if there are errors. They\'re identified by group code. Multiple entries are delimited by comma or semicolon. Invalid or non-existing groups will be ignored. [Optional]'
            ),
        ];
    }
    # -------------------------------------------------------
    ...
    {
        return _t(
            'This utility checks the integrity for all URL attribute references in the database. It does so by trying to hit each URL and reading a few bytes. It does not download the whole file.'
        );
    }
    # -------------------------------------------------------
    ...
    {
        return _t(
            'This utility checks all periodic metadatadata alert triggers users have set up and, if they triggered, sends notifications to the recipients of these rules.'
        );
    }
    # -------------------------------------------------------
    ...
    public static function import_media($po_opts = null)
    {
        ...
        if (!caCheckMediaDirectoryPermissions()) {
            CLIUtils::addError(
                _t(
                    'The media directory is not writeable by the current user. Try again, running the import as the web server user.'
                )
            );
            return false;
        }
        ...
        if (!in_array(
            ...
            ['AUTO', 'FILENAME', 'FILENAME_NO_EXT', 'DIRECTORY_AND_FILENAME']
        )) {
        ...
        if (!$vn_rep_type_id) {
            ...
            CLIUtils::addMessage(
                _t('Setting representation type to default %1', $t_rep->getTypeCodeForID($vn_rep_type_id))
            );
        }
        ...
        if (!$vn_rep_access) {
            ...
            CLIUtils::addMessage(
                _t('Setting representation access to default %1', caGetListItemByIDForDisplay($vn_id))
            );
        }
        ...
        if (!$vn_rep_status) {
            ...
            CLIUtils::addMessage(
                _t('Setting representation status to default %1', caGetListItemByIDForDisplay($vn_id))
            );
        }
        ...
        if ((bool)$po_opts->getOption('include-subdirectories')) {
            CLIUtils::addMessage(
                _t('Found %1 files in %2 directories', $va_counts['files'], $va_counts['directories'])
            );
        } else {
        ...
        if (!BatchProcessor::importMediaFromDirectory(null, $va_opts)) {
            CLIUtils::addError(
                _t("Could not import media from %1: %2", $vs_data_source, join("; ", BatchProcessor::getErrorList()))
            );
            return false;
        } else {
    ...
    public static function import_mediaParamList()
    {
        ...
        $access_status_default = caGetListItems(
            'access_statuses',
            [
                'index' => 'item_value',
                'value' => 'name_plural',
        ...
        $access_status_list_str = join(
            ...
            array_map(
                ...
                array_keys($access_status_values)
            )
        );
        $access_status_default_str = join(
            ", ",
            array_map(
                function ($v, $k) {
                    return "{$k} ({$v})";
                },
                $access_status_default,
                array_keys($access_status_default)
            )
        );

        $workflow_status_values = caGetListItems(
            'workflow_statuses',
            ['index' => 'item_value', 'value' => 'name_plural']
        );
        $workflow_status_default = caGetListItems(
            'workflow_statuses',
            [
                'index' => 'item_value',
                'value' => 'name_plural',
        ...
        $workflow_status_list_str = join(
            ...
            array_map(
                ...
                array_keys($workflow_status_values)
            )
        );
        $workflow_status_default_str = join(
            ", ",
            array_map(
                function ($v, $k) {
                    return "{$k} ({$v})";
                ...
                array_keys($workflow_status_default)
            )
        );

        $object_type_values = caGetListItems('object_types', ['index' => 'item_value', 'value' => 'name_plural']);
        $object_type_default = caGetListItems(
            'object_types',
            ['index' => 'item_value', 'value' => 'name_plural', 'defaultOnly' => true]
        );

        $object_type_list_str = join(
            ", ",
            array_map(
                function ($v, $k) {
                    return "{$k} ({$v})";
                ...
                $object_type_values,
                array_keys($object_type_values)
            )
        );
        $object_type_default_str = join(
            ", ",
            array_map(
                function ($v, $k) {
                    return "{$k} ({$v})";
                ...
                $object_type_default,
                array_keys($object_type_default)
            )
        );

        $representation_type_values = caGetListItems(
            'object_representation_types',
            ['index' => 'item_value', 'value' => 'name_plural']
        );
        $representation_type_default = caGetListItems(
            'object_representation_types',
            [
                'index' => 'item_value',
                'value' => 'name_plural',
        ...
        $representation_type_list_str = join(
            ", ",
            array_map(
                function ($v, $k) {
                    return "{$k} ({$v})";
                },
                $representation_type_values,
                array_keys($representation_type_values)
            )
        );
        $representation_type_default_str = join(
            ", ",
            array_map(
                function ($v, $k) {
                    return "{$k} ({$v})";
                },
                $representation_type_default,
                array_keys($representation_type_default)
            )
        );
        ...
        return array(
            ...
            "add-to-set|t-s" => _t('Optional identifier of set to add all imported items to.'),
            "log-to-tmp-directory-as-fallback|f-s" => _t(
                'Use the system temporary directory for the import log if the application logging directory is not writable. Default report an error if the application log directory is not writeable.'
            ),
            "include-subdirectories|i-s" => _t('Process media in sub-directories. Default is false.'),
            "match-type|mt-s" => _t(
                'Sets how match between media and target record identifier is made. Valid values are: STARTS, ENDS, CONTAINS, EXACT. Default is EXACT.'
            ),
            "match-mode|m-s" => _t(
            ...
            "import-mode" => _t(
                'Determines if target records are created for media that do not match existing target records. Set to TRY_TO_MATCH to create new target records when no match is found. Set to ALWAYS_MATCH to only import media for existing records. Default is TRY_TO_MATCH.'
            ),
            'allow-duplicate-media|du-s' => _t(
                'Import media even if it already exists in CollectiveAccess. Default is false – skip import of duplicate media.'
            ),
            'import-target|it-s' => _t(
                'Table name of record to import media into. Should be a valid representation-taking table such as ca_objects, ca_entities, ca_occurrences, ca_places, etc. Default is ca_objects.'
            ),
            'import-target-type|itt-s' => _t(
                'Type to use for all newly created target records. Default is the first type in the target\'s type list.'
            ),
            'import-target-idno|iti-s' => _t('Identifier to use for all newly created target records.'),
            'import-target-idno-mode|itim-s' => _t(
            ...
            'import-target-access|ita-s' => _t(
                'Set access for newly created target records. Possible values are %1. Default is %2.',
                $access_status_list_str,
                $access_status_default_str
            ),
            'import-target-status|its-s' => _t(
                'Set status for newly created target records. Possible values are %1. Default is %2.',
                $workflow_status_list_str,
                $workflow_status_default_str
            ),
            'representation-type|rt-s' => _t(
                'Type to use for all newly created representations. Possible values are %1. Default is %2.',
                $representation_type_list_str,
                $representation_type_default_str
            ),
            ...
            'representation-access|ra-s' => _t(
                'Set access for newly created representations. Possible values are %1. Default is %2.',
                $access_status_list_str,
                $access_status_default_str
            ),
            'representation-status|rs-s' => _t(
                'Set status for newly created representations. Possible values are %1. Default is %2.',
                $workflow_status_list_str,
                $workflow_status_default_str
            ),
            'representation-mapping|rm-s' => _t('Code for mapping to apply when importing media.'),
            'delete-media-on-import|dmoi-s' => _t(
                'Remove media from directory after it has been successfully imported. Default is false.'
            )
        );
    }
    # -------------------------------------------------------
        ...
        foreach ($va_elements as $va_element) {
            ...
            foreach ($va_type_res_list as $va_type_res) {
                ...
                if ($va_type_res['type_id'] > 0) {
                    $qr_res = call_user_func(
                        "{$vs_table_name}::find",
                        ["type_id" => (int)$va_type_res['type_id']],
                ...
                while ($qr_res->nextHit()) {
                    ...
                    foreach ($va_value_list as $vn_row_id => $va_values_by_attribute_id) {
                        CLIUtils::addMessage(
                            _t('Processing row %1 for %2.%3', $vn_row_id, $vs_root_code, $va_element['element_code'])
                        );
                        foreach ($va_values_by_attribute_id as $vn_attr_id => $va_values) {
                            $vs_processed_value = caProcessTemplate(
                                $va_element['settings']['dependentValueTemplate'],
                                $va_values
                            );
                            ...
                            if ($t_instance->numErrors() > 0) {
                                CLIUtils::addError(
                                    _t("Could not update dependent value: %1", join("; ", $t_instance->getErrors()))
                                );
                            }
                        }
                    }
    ...
    {
        return _t(
            'Print the system GUID value used for replication operations to the console. The system GUID uniquely identifies a CollectiveAccess installation.'
        );
    }
    # -------------------------------------------------------
    ...
    public static function check_relationship_type_roots($po_opts = null)
    {
        ...
        foreach ($va_tables as $vs_table) {
            ...
            if (!$t_root = ca_relationship_types::find(
                ['parent_id' => null, 'table_num' => $vn_table_num],
                ['returnAs' => 'firstModelInstance']
            )) {
                ...
                if ($t_root->numErrors()) {
                    CLIUtils::addError(
                        _t("Could not create root for relationship %1: %2", $vs_table, join('; ', $t_root->getErrors()))
                    );
                    continue;
                }
                $t_root->addLabel(
                ...
                if ($t_root->numErrors()) {
                    CLIUtils::addError(
                        _t(
                            "Could not add label to root for relationship %1: %2",
                            $vs_table,
                            join('; ', $t_root->getErrors())
                        )
                    );
                    continue;
                }
    ...
    public static function clear_search_indexing_queue_lock_file($po_opts = null)
    {
        ...
        if (ca_search_indexing_queue::lockExists()) {
            if (ca_search_indexing_queue::lockCanBeRemoved()) {
                ...
                CLIUtils::addMessage(
                    _t(
                        "Insufficient privileges to remove search indexing queue. Try running caUtils under a user with privileges"
                    )
                );
            }
        } else {
