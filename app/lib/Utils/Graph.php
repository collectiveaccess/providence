...
class Graph
{
    ...
    {
        if (isset($this->opa_graph["EDGES"][$ps_node1][$ps_node2]) && is_array(
                $this->opa_graph["EDGES"][$ps_node1][$ps_node2]
            )) {
            return true;
        } else {
    ...
    public function getPath($ps_start_node, $ps_end_node)
    {
        ...
        if (isset($va_paths[$ps_end_node]) && is_array(
                $va_paths[$ps_end_node]
    ...
    private function _doTopoSort()
    {
        ...
        do {
            ...
            $va_leaves = [];
            foreach (array_keys($this->getNodes()) as $vs_key) {
                if (!$this->getAttribute('topo_visited', $vs_key) && (($vn_in_degree = $this->_getTopoInDegree(
                            $vs_key
                        )) == 0)) {
                    $va_leaves[] = $vs_key;
                }
