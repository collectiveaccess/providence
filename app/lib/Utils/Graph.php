...
class Graph {
	...
	public function hasRelationship( $ps_node1, $ps_node2 ) {
		if ( isset( $this->opa_graph["EDGES"][ $ps_node1 ][ $ps_node2 ] )
		     && is_array( $this->opa_graph["EDGES"][ $ps_node1 ][ $ps_node2 ] )
		) {
			return true;
		} else {
			return false;
	...
	public function getAttribute( $ps_attribute, $ps_node1, $ps_node2 = null ) {
		if ( $ps_node2 ) {                # get attribute from relationship
			...
			return isset( $this->opa_graph["NODES"][ $ps_node1 ][ $ps_attribute ] )
				? $this->opa_graph["NODES"][ $ps_node1 ][ $ps_attribute ] : "";
		}
	}
	# --------------------------------------------------------------------------------------------
	# --- Paths
	...
	public function getPath( $ps_start_node, $ps_end_node ) {
			...
			if ( is_array( $va_edges[ $vs_closest_node ] ) ) {
				foreach ( $va_edges[ $vs_closest_node ] as $vs_vertex => $va_attr ) {
					...
					if ( ! isset( $va_paths[ $vs_vertex ] ) || ( $vn_distance < $va_paths[ $vs_vertex ][0] ) ) {
						$va_paths[ $vs_vertex ]    = isset( $va_paths[ $vs_closest_node ] )
							? $va_paths[ $vs_closest_node ] : [];
		...
		if ( isset( $va_paths[ $ps_end_node ] )
		     && is_array( $va_paths[ $ps_end_node ] )
		) {    # no path exists is $va_paths[$ps_end_node] is not set
		...
		do {
			...
			foreach ( array_keys( $this->getNodes() ) as $vs_key ) {
				if ( ! $this->getAttribute( 'topo_visited', $vs_key )
				     && ( ( $vn_in_degree
							= $this->_getTopoInDegree( $vs_key ) ) == 0 )
				) {
					$va_leaves[] = $vs_key;
				}
			}
