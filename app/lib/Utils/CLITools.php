		...
		foreach ( $o_sheet->getRowIterator() as $o_row ) {
			...
			foreach ( $o_cells as $c => $o_cell ) {
				if ( $label = trim( (string) $o_cell->getValue() ) ) {
					$item_value       = ( strlen( $item_value_col ) )
						? $o_sheet->getCellByColumnAndRow( $item_value_col, $vn_c )->getValue() : null;
					$item_description = ( strlen( $item_desc_col ) ) ? $o_sheet->getCellByColumnAndRow( $item_desc_col,
						$vn_c )->getValue() : null;
					$np_labels        = ( strlen( $np_label_col ) ) ? $o_sheet->getCellByColumnAndRow( $np_label_col,
						$vn_c )->getValue() : null;

					if ( $vn_col > $vn_last_level ) {
						$va_stack[] = &$va_stack[ sizeof( $va_stack ) - 1 ][ sizeof( $va_stack[ sizeof( $va_stack )
						                                                                        - 1 ] )
						                                                     - 1 ]['subitems'];
					} elseif ( $vn_col < $vn_last_level ) {
						while ( sizeof( $va_stack )
						        && ( $va_stack[ sizeof( $va_stack ) - 1 ][0]['level'] > $vn_col ) ) {
							array_pop( $va_stack );
						}
					}
...
{$vn_ident}\t</labels>" .
			           ( ( is_array( $va_item['subitems'] ) && sizeof( $va_item['subitems'] ) )
				           ? "{$vn_ident}\t<items>\n{$vn_indent}" . CLITools::_makeList( $va_item['subitems'],
					           $pn_indent + 2, array_merge( $pa_stack, array( substr( $vs_label_proc, 0, 10 ) ) ),
					           $locale ) . "{$vn_ident}\t</items>" : '' ) . "
{$vn_ident}</item>\n";
...
{$vn_ident}\t</labels>\n" .
			           ( ( is_array( $va_item['subitems'] ) && sizeof( $va_item['subitems'] ) )
				           ? "{$vn_ident}\t<types>\n{$vn_indent}" . CLITools::_makeTypeList( $va_item['subitems'],
					           $pn_indent + 2, array_merge( $pa_stack, array( substr( $vs_label_proc, 0, 10 ) ) ) )
				             . "{$vn_ident}\t</types>" : '' ) . "
{$vn_ident}</type>\n";
	...
	public static function convert_xml_to_delimited( $po_opts = null ) {
		...
		switch ( $input_format ) {
			...
			default:
				if ( $input_format ) {
					CLITools::addError( _t( "%1 is not a valid input format. Defaulting to PastPerfect.",
						$input_format ) );
				} else {
	...
	public static function filter_invalid_xml_characters( $po_opts = null ) {
		...
		while ( ( $line = fgets( $fp_in ) ) !== false ) {
			if ( preg_match( "![^\x{0009}\x{000A}\x0D\x{0020}-\x{D7FF}\x{E000}-\x{FFFD}\x{10000}-\x{10FFFF}]+!u", $line,
				$m )
			) {
				CLITools::addError( _t( "Found invalid character at line %1: %2", $l, $line ) );
				$line
					= preg_replace( "![^\x{0009}\x{000A}\x0D\x{0020}-\x{D7FF}\x{E000}-\x{FFFD}\x{10000}-\x{10FFFF}]+!u",
					"", $line );
			}
			fputs( $fp_out, $line );
			$l ++;
