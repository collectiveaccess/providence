...
class CLITools extends CLIBaseUtils {
	...
	private static function _makeList($pa_list, $pn_indent=0, $pa_stack=null, $locale=null) {
		...
		$vs_indent = $pn_indent ? str_repeat("\t", $pn_indent) : '';
		$vs_buf = '';
		foreach($pa_list as $vn_i => $va_item) {
			...
			$vs_buf .= "{$vs_indent}<item idno=\"{$vs_label_prefix}{$vs_label_proc}\" enabled=\"1\" default=\"0\" value=\"{$vs_item_value}\">
{$vs_indent}\t<labels>
{$vs_indent}\t\t<label locale=\"{$locale}\" preferred=\"1\">
{$vs_indent}\t\t\t<name_singular>{$vs_label}</name_singular>
{$vs_indent}\t\t\t<name_plural>{$vs_label}</name_plural>
{$vs_indent}\t\t\t<description>{$vs_item_desc}</description>
{$vs_indent}\t\t</label>
{$vs_indent}{$non_preferred_labels}
{$vs_indent}\t</labels>".
((is_array($va_item['subitems']) && sizeof($va_item['subitems'])) ? "{$vs_indent}\t<items>\n{$vs_indent}".CLITools::_makeList($va_item['subitems'], $pn_indent + 2, array_merge($pa_stack, array(substr($vs_label_proc, 0, 10))), $locale)."{$vs_indent}\t</items>" : '')."
{$vs_indent}</item>\n";
	...
	private static function _makeTypeList($pa_list, $pn_indent=0, $pa_stack=null) {
		...
		foreach($pa_list as $vn_i => $va_item) {
			...
			$vs_buf .= "{$vs_indent}<type code=\"{$label_prefix}{$label_proc}\" default=\"0\">
{$vs_indent}\t<labels>
{$vs_indent}\t\t<label locale=\"{$locale}\">
{$vs_indent}\t\t\t<typename>{$label_forward}</typename>
{$vs_indent}\t\t\t<typename_reverse>{$label_reverse}</typename_reverse>
{$vs_indent}\t\t</label>
{$vs_indent}\t</labels>\n".
((is_array($va_item['subitems']) && sizeof($va_item['subitems'])) ? "{$vs_indent}\t<types>\n{$vs_indent}".CLITools::_makeTypeList($va_item['subitems'], $pn_indent + 2, array_merge($pa_stack, array(substr($label_proc, 0, 10))))."{$vs_indent}\t</types>" : '')."
{$vs_indent}</type>\n";
		}
