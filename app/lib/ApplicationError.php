...
class ApplicationError
{
     ...
     *
     * @param int $pn_error_number The numeric error code. Code should be defined in the error definition file
    ...
    public function __construct(
        $pn_error_number = 0,
        $ps_error_description = '',
        $ps_error_context = '',
        $ps_error_source = '',
        $pb_halt_on_error = true,
        $pb_report_on_error = true,
        $ps_error_definition_file = ''
    ) {
        ...
        if (($vs_locale = $this->opo_config->get("locale")) && ((file_exists(
                    "Error/errors." . $vs_locale
        ...
        if (!$vs_error_definitions_loaded) {
            $vs_error_definitions_loaded = $this->setErrorDefinitionFile(
                __CA_LIB_DIR__ . "/Error/errors." . $this->ops_locale
            );
        }
        if (!$vs_error_definitions_loaded) {
     ...
     *
     * @param int $pb_halt_on_error True to halt on error, false otherwise.
     * @return int Always returns 1
     */
     ...
     *
     * @param int $pb_report_on_error True to print an error message on error, false otherwise.
     * @return int Always returns 1
     */
     ...
     *
     * @param int $pb_redirect_on_error True to redirect to another URL on error, false otherwise.
     * @return int Always returns 1
     */
     ...
     *
     * @param int $ps_redirect_on_error_url The URL to redirect to. The URL should be absolute or root-relative
     ...
     * @return int Returns 1 on success, zero on failure
     */
    ...
    public function halt()
    {
        ...
        if ($this->opb_redirect_on_error) {
            if ($vs_error_page = $this->ops_redirect_on_error_page) {
                header(
                    "Location: " . $vs_error_page . "?n=" . $this->getErrorNumber() . "&desc=" . urlencode(
        ...
        if ($this->opb_report_on_error) {
            $this->haltmsg(
                $this->getErrorNumber() . ": " . $this->getErrorMessage() . "<br/>" . $this->getErrorDescription(
                ) . " (in " . $this->getErrorContext() . ")"
            );
        }
