...
class ApplicationError {
	...
	/**
	 * Context of current error. This is typically the Class name and method name where the error occurred. Ex.
	 * "Configuration->new()"
	 *
	 * @access private
	 */
	...
	/**
	 * Source code for error. While it can be used as a general-purpose identifier for the cause of an error, it
	 * typically used to record which field in an input form the error is associated with.
	 *
	 * @access private
	 */
	...
	/**
	 * Constructor takes optional parameters to create a new error. If parameters are omitted, an empty (non-error
	 * state) error object is created. An error state can subsequently be set using the setError() method
	 *
	 * @param integer $pn_error_number          The numeric error code. Code should be defined in the error definition
	 *                                          file
	 * @param string  $ps_error_description     Description of error condition
	 * @param string  $ps_error_context         Context where error occurred. This is typically the Class name and
	 *                                          method name where the error occurred. Ex. "Configuration->new()"
	 * @param string  $ps_error_source          Source of error - typically a string identifying the field in a form
	 *                                          where the error occurred.
	 * @param bool    $pb_halt_on_error         Whether or not to halt on error state (ie. whether do die()) [default
	 *                                          is true]
	 * @param bool    $pb_report_on_error       Whether or not to emit a visible report of error state [default is
	 *                                          true]
	 * @param string  $ps_error_definition_file Path to error definition file; if omitted default file, based upon
	 *                                          locale, is used
	 */
	public function __construct(
		$pn_error_number = 0, $ps_error_description = '', $ps_error_context = '', $ps_error_source = '',
		$pb_halt_on_error = true, $pb_report_on_error = true, $ps_error_definition_file = ''
	) {
		...
		if ( ( $vs_locale = $this->opo_config->get( "locale" ) )
		     && ( ( file_exists( "Error/errors." . $vs_locale ) )
		          || ( $this->file_exists_incpath( "Error/errors." . $vs_locale ) ) )
		) {
		...
		if ( ! $vs_error_definitions_loaded ) {
			$vs_error_definitions_loaded = $this->setErrorDefinitionFile( __CA_LIB_DIR__ . "/Error/errors."
			                                                              . $this->ops_locale );
		}
		if ( ! $vs_error_definitions_loaded ) {
	 ...
	 * Sets an error state.
	 *
	 * Note: if the configuration directive 'error_email_notification_to' is set to an e-mail address in the
	 * application configuration file a notification of error will be sent.
	 *
	 * @param integer $pn_error_number      The numeric error code. Code should be defined in the error definition file
	 * @param string  $ps_error_description Description of error condition
	 * @param string  $ps_error_context     Context where error occurred. This is typically the Class name and method
	 *                                      name where the error occurred. Ex. "Configuration->new()"
	 * @param string  $ps_error_source      Source of error - typically a string identifying the field in a form where
	 *                                      the error occurred.
	 *
	 * @return integer Always returns 1
	 */
	public function setError(
		$pn_error_number, $ps_error_description = '', $ps_error_context = '', $ps_error_source = ''
	) {
	...
	/**
	 * Set error output options: halt on error, report on error and redirect on error. Halt on error, well..., halts
	 * the request and prints a message. Report on error prints an error message but does not halt. This can be useful
	 * for debugging. Redirect on error redirects the request to a specified URL on error, passing basic information
	 * about the error in the URL query parameters. This can be useful for cleanly logging and handling errors in a
	 * production application.
	 *
	 * @param bool $pb_halt_and_report_on_error True if halt, report and redirect should be active, false if not.
	 *                                          Generally this method is used to turn all options off in one shot.
	 *
	 ...
	 *                                          without query parameters. Query parameters containing information about
	 *                                          the error states will be appended to this URL.
	 */
	...
	/**
	 * Loads an error definition file. By default, the error definition file is a standard system error file for the
	 * current locale (if locale is set by the 'locale' directive in the application configuration file). If the locale
	 * is not set or there is no standard error definition file for the locale, the default en_us (US English) locale
	 * is used. You can override the use of standard locale-based error messages with your own error definitions by
	 * calling this method with the path to your error definition file.
	 *
	 * An error definition file is simply a standard configuration file containing one error message per line. The
	 * configuration directive (or "key") for each error is the error number. The value is the error message itself.
	 *
	...
	public function getError() {
		return array(
			$this->opn_error_number,
			$this->getErrorMessage( $this->opn_error_number ),
			$this->ops_error_description,
			$this->ops_error_context,
			$this->ops_error_source
		);
	}
	...
	public function halt() {
		...
		if ( $this->opb_redirect_on_error ) {
			if ( $vs_error_page = $this->ops_redirect_on_error_page ) {
				header( "Location: " . $vs_error_page . "?n=" . $this->getErrorNumber() . "&desc="
				        . urlencode( $this->getErrorDescription() ) );
				exit;
			}
		}

		if ( $this->opb_report_on_error ) {
			$this->haltmsg( $this->getErrorNumber() . ": " . $this->getErrorMessage() . "<br/>"
			                . $this->getErrorDescription() . " (in " . $this->getErrorContext() . ")" );
		}
