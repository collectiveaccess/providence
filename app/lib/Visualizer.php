...
class Visualizer {
	 ...
	 *
	 * @param mixed $po_data  An model instance subclassed from BundlableLabelableBaseModelWithAttributes, a subclass
	 *                        of SearchResult or an array of numeric ids
	 * @param bool  $pb_reset If true, any previously set data set is removed prior to addition of $po_data. Default is
	 *                        false.
	 *
	...
	public function addData( $po_data, $pb_reset = false ) {
		...
		if ( ! is_array( $po_data ) && ! is_subclass_of( $po_data, 'SearchResult' )
		     && ! is_subclass_of( $po_data, 'BundlableLabelableBaseModelWithAttributes' )
		) {
			return false;
		}
	 ...
	 *
	 * @param string $ps_visualization Identifier of visualization to use. The identifier is the language-independent
	 *                                 name of the visualization as specified in visualization.conf
	 * @param string $ps_format        Format of rendered visualization. The default, and only supported format at this
	 *                                 time, is HTML.
	...
	public function render( $ps_visualization, $ps_format = 'HTML', $pa_options = null ) {
		...
		if ( ! isset( $va_viz_list[ $ps_visualization ] )
		     || ! is_array( $va_viz_settings = $va_viz_list[ $ps_visualization ] )
		) {
			return null;
		}
		...
		if ( $o_viz = $this->getVisualizationPlugin( $vs_viz_plugin ) ) {
			...
			if ( sizeof( $va_ids ) ) {
				...
				$vs_html                      = $o_viz->render( array_merge( $va_viz_settings,
					array( 'code' => $ps_visualization ) ), $ps_format,
					array_merge( $va_viz_settings['options'], $pa_options ) );
	...
	public function getDataForVisualization( $ps_visualization, $pa_options = null ) {
		...
		if ( $o_viz = $this->getVisualizationPlugin( $vs_viz_plugin ) ) {
			...
			if ( sizeof( $va_ids ) ) {
				...
				$va_data                      = $o_viz->getDataForVisualization( $va_viz_settings,
					array_merge( $pa_options, $va_viz_settings['options'] ) );
				$this->opn_num_items_rendered = $this->opn_num_items_rendered + $o_viz->numItemsRendered();
	 ...
	 *                           restrictToTypes = optional list of types relevant to the specified table. Only
	 *                           unrestricted visualizations and visualizations restricted to the specified types will
	 *                           be included.
	 *
	 * @return array
	 */
	public static function getAvailableVisualizations( $ps_table, $pa_options = null ) {
		...
		if ( is_array( $va_viz_list = $o_viz_config->getAssoc( $ps_table ) ) ) {
			...
			if ( isset( $pa_options['restrictToTypes'] ) && is_array( $pa_options['restrictToTypes'] )
			     && sizeof( $pa_options['restrictToTypes'] )
			) {
				$va_filter_on_types = caMakeTypeIDList( $ps_table, $pa_options['restrictToTypes'] );
				foreach ( $va_viz_list as $vs_code => $va_viz ) {
					if ( isset( $va_viz['restrictToTypes'] ) && is_array( $va_viz['restrictToTypes'] )
					     && ( sizeof( $va_viz['restrictToTypes'] ) )
					) {
	 ...
	 *                              restrictToTypes = optional list of types relevant to the specified table. Only
	 *                              unrestricted visualizations and visualizations restricted to the specified types
	 *                              will be included. resultContext =
	 *
	 * @return string HTML for a <select> element with available visualizations; will return null if no visualizations
	 *                are available.
	 */
	public static function getAvailableVisualizationsAsHTMLFormElement(
		$ps_table, $ps_name, $pa_attributes = null, $pa_options = null
	) {
		$va_viz            = Visualizer::getAvailableVisualizations( $ps_table, $pa_options );
		$vo_result_context = ( isset( $pa_options['resultContext'] )
		                       && ( $pa_options['resultContext'] instanceOf ResultContext ) )
			? $pa_options['resultContext'] : null;
		if ( $vo_result_context && ( $vo_result_context->getParameter( 'availableVisualizationChecked' ) > 0 ) ) {
			if ( $vo_result_context->getParameter( 'availableVisualizationCount' ) > 0 ) {
				if ( is_array(
					$va_dependencies = $vo_result_context->getParameter( 'availableVisualizationDependencies' ) )
				) {
		...
		if ( isset( $pa_options['data'] ) && is_subclass_of( $pa_options['data'], 'SearchResult' ) ) {
			$va_valid_viz_codes = Visualizer::getVisualizationsForData( $pa_options['data'],
				array( 'resultContext' => $vo_result_context ) );
		}
	 ...
	 *
	 * @param string $ps_plugin_name Name of plugin. The name is the same as the plugin's filename minus the .php
	 *                               extension.
	 *
