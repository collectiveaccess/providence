...
class ExternalCache {
	...
	private static function getRedisObject() {
		...
		$driver = new Stash\Driver\Redis( [
			'servers'  => [ [ __CA_REDIS_HOST__, __CA_REDIS_PORT__ ] ],
			'database' => __CA_REDIS_DB__
		] );
	...
	private static function makeKey( $ps_key, $ps_namespace ) {
		...
		return substr( __CA_APP_TYPE__, 0, 4 ) . '/' . $ps_namespace . '/'
		       . $ps_key; // only use the first four chars of app type for compactness
	}
	...
	private static function makeCacheName() {
		...
		return substr( __CA_APP_TYPE__, 0, 4 ) . '_'
		       . __CA_APP_NAME__;    // only use the first four chars of app type for compactness
	}
	# ------------------------------------------------
}
