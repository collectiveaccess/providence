...
class ExternalCache
{
    ...
    public static function setInvalidationMode($pn_method)
    {
        if (!in_array(
            ...
            [
                ...
                Stash\Invalidation::PRECOMPUTE,
                Stash\Invalidation::OLD,
                Stash\Invalidation::VALUE,
                Stash\Invalidation::SLEEP
            ]
        )) {
            return false;
        }
    ...
    private static function checkParameters($ps_namespace, $ps_key)
    {
        ...
        if (!preg_match("/^[A-Za-z0-9_]+$/", $ps_namespace)) {
            throw new ExternalCacheInvalidParameterException(
                'Caching namespace must only contain alphanumeric characters, dashes and underscores'
            );
        }
    ...
    private static function getMemcachedObject()
    {
        ...
        $driver = new Stash\Driver\Memcache(
            [
                'servers' => [
                    __CA_MEMCACHED_HOST__,
                    __CA_MEMCACHED_PORT__,
                    'prefix_key' => ExternalCache::makeCacheName(),
                    'serializer' => 'json'
                ]
            ]
        );
    ...
    private static function getRedisObject()
    {
        ...
        $driver = new Stash\Driver\Redis(
            ['servers' => [[__CA_REDIS_HOST__, __CA_REDIS_PORT__]], 'database' => __CA_REDIS_DB__]
        );
        return new Stash\Pool($driver);
    }
    ...
    private static function makeKey($ps_key, $ps_namespace)
    {
        ...
        return substr(
                ...
                0,
                4
            ) . '/' . $ps_namespace . '/' . $ps_key; // only use the first four chars of app type for compactness
    }
    ...
    private static function makeCacheName()
    {
        ...
        return substr(
                ...
                0,
                4
            ) . '_' . __CA_APP_NAME__;    // only use the first four chars of app type for compactness
    }
    # ------------------------------------------------
}
