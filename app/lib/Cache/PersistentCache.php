...
class PersistentCache {
	...
	public static function save( $key, $data, $namespace = 'default', $ttl = null ) {
		...
		if ( self::contains( $key, $namespace ) ) {
			$qr
				= self::$db->query( "UPDATE ca_persistent_cache SET cache_value = ?, updated_on = ? WHERE cache_key = ?",
				[ caSerializeForDatabase( $data ), time(), $cache_key ] );
		} else {
			$qr
				= self::$db->query( "INSERT INTO ca_persistent_cache (cache_key, cache_value, created_on, updated_on, namespace) VALUES (?, ?, ?, ?, ?)",
				[ $cache_key, caSerializeForDatabase( $data ), time(), time(), $namespace ] );
		}
	 ...
	 *
	 * @param int    $since     Unix timeststamp. All entries that have not been updated since this time will be
	 *                          removed.
	...
	public static function clean( $since, $namespace = null ) {
		...
		if ( $namespace ) {
			$qr = self::$db->query( "DELETE FROM ca_persistent_cache WHERE namespace = ? AND updated_on < ?",
				[ $namespace, (int) $since ] );
		} else {
