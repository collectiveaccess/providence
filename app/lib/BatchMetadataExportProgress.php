...
class BatchMetadataExportProgress extends AppControllerPlugin
{
    ...
    public function dispatchLoopShutdown()
    {
        ...
        if ($req->getParameter(
            'caIsExportFromSearchOrBrowseResult',
            pInteger
            ...
            ca_data_exporters::exportRecordsFromSearchResult(
                $t_exporter->get('exporter_code'),
                $o_result,
                $vs_file,
                array(
                    'request' => $req,
                    'progressCallback' => 'caIncrementBatchMetadataExportProgress'
                )
            );
        } else {
            if ($vn_set_id = $req->getParameter('set_id', pInteger)) { // batch export from set

                ca_data_exporters::exportRecordsFromSet(
                    $t_exporter->get('exporter_code'),
                    $vn_set_id,
                    $vs_file,
                    array(
                ...
                ca_data_exporters::exportRecordsFromSearchExpression(
                    $t_exporter->get('exporter_code'),
                    $vs_search,
                    $vs_file,
                    array(
