...
class AccessRestrictions
{
    ...
    public function userCanAccess(
        ...
        $ps_controller,
        $ps_action,
        $pa_fake_parameters = array()
    ) {
        if (!$this->opo_acr_config->get(
            "enforce_access_restrictions"
        ...
        // check controller
        $vs_controller_path = join("/", (is_array($pa_module_path) ? $pa_module_path : array())) . "/" . ucfirst(
                $ps_controller
            ) . 'Controller';
        ...
        // check action
        $vs_action_path = join("/", (is_array($pa_module_path) ? $pa_module_path : array())) . "/" . ucfirst(
                $ps_controller
            ) . "Controller/" . $ps_action;
        ...
        foreach ($va_groups_to_check as $va_group) {
            ...
            if (is_array($va_group["parameters"])) {
                if (!$this->_parameterRestrictionsApply(
                    $va_group["parameters"],
                    $ps_controller,
                    $ps_action,
                    $pa_fake_parameters
            ...
            if (($vs_key == "type") && !is_array($va_value_data) && strlen($va_value_data) > 0) {
                ...
                if (substr($va_value_data["value"], 0, 1) == "!") {
                    switch ($va_value_data["type"]) {
                        case "int":
                            if (intval(substr($va_value_data["value"], 1)) == intval(
                                    $vs_fake_val ? $vs_fake_val : $this->opo_request->getParameter($vs_key, pInteger)
                                )) {
                                return false;
                            }
                            break;
                        case "float":
                            if (floatval(substr($va_value_data["value"], 1)) == floatval(
                                    $vs_fake_val ? $vs_fake_val : $this->opo_request->getParameter($vs_key, pFloat)
                                )) {
                                return false;
                            }
                            break;
                        default:
                            if (strval(substr($va_value_data["value"], 1)) == strval(
                                    $vs_fake_val ? $vs_fake_val : $this->opo_request->getParameter($vs_key, pString)
                                )) {
                                return false;
                            }
                            break;
                    ...
                    if ($va_value_data["value"] == "not_set") {
                        ...
                        switch ($va_value_data["type"]) {
                            case "int":
                                if (intval($va_value_data["value"]) != intval(
                                        $vs_fake_val ? $vs_fake_val : $this->opo_request->getParameter(
                                            $vs_key,
                                            pInteger
                                        )
                                    )) {
                                ...
                                break;
                            case "float":
                                if (floatval($va_value_data["value"]) != floatval(
                                        $vs_fake_val ? $vs_fake_val : $this->opo_request->getParameter($vs_key, pFloat)
                                    )) {
                                    return false;
                                }
                                break;
                            default:
                                if (strval($va_value_data["value"]) != strval(
                                        $vs_fake_val ? $vs_fake_val : $this->opo_request->getParameter($vs_key, pString)
                                    )) {
                                    return false;
                                }
