...
class BaseSearchController extends BaseRefineableSearchController
{
    ...
    public function Index($pa_options = null)
    {
        ...
        if ((bool)$this->request->getParameter('reset', pString) && ($this->request->getParameter(
                    'reset',
                    pString
                ) != 'save')) {
            $vs_search = '';
        ...
        switch ($pa_options['output_format']) {
            ...
            default:
                ...
                if ($this->opb_uses_hierarchy_browser) {
                    ...
                    $vn_id = ($this->request->config->get(
                        $this->ops_tablename . '_dont_remember_last_browse_item'
    ...
    public function _genTypeNav($pa_params)
    {
        ...
        $limit_to_types = $this->getRequest()->config->getList(
            $this->ops_tablename . '_navigation_find_menu_limit_types_to'
        );
        ...
        if (is_array($va_hier)) {
            ...
            foreach ($va_hier as $vn_item_id => $va_item) {
                ...
                if (
                    !$this->getRequest()->config->get(
                        $this->ops_tablename . '_navigation_find_menu_shows_top_level_types_only'
                    )
                    &&
                    (isset($va_item['item_id']) && isset($va_types_by_parent_id[$va_item['item_id']]) && is_array(
                    ...
                    $va_subtypes = method_exists($this, "_getSubTypeActionNav") ? $this->_getSubTypeActionNav(
                        $va_item
                    ) : [];
                }
                $va_types[] = array(
    ...
    public function lookup()
    {
        ...
        if ($this->request->config->get('quicksearch_return_ca_objects')) {
            $va_results = caExtractValuesByUserLocale(
                SearchEngine::quickSearch(
                    $vs_search,
                    'ca_objects',
                    57,
            ...
            foreach ($va_results as $vn_id => $va_match_info) {
                $vs_type = caUcFirstUTF8Safe(
                    $t_list->getItemFromListForDisplayByItemID('object_types', $va_match_info['type_id'], true)
                );
        ...
        if ($this->request->config->get('quicksearch_return_ca_entities')) {
            $va_data['ca_entities'][_t('Entities')] = caExtractValuesByUserLocale(
                SearchEngine::quickSearch(
                    $vs_search,
                    'ca_entities',
                    20,
        ...
        if ($this->request->config->get('quicksearch_return_ca_places')) {
            $va_data['ca_places'][_t('Places')] = caExtractValuesByUserLocale(
                SearchEngine::quickSearch(
                    $vs_search,
                    'ca_places',
                    72,
        ...
        if ($this->request->config->get('quicksearch_return_ca_occurrences')) {
            ...
            foreach ($va_results as $vn_id => $va_match_info) {
                $vs_type = caUcFirstUTF8Safe(
                    $t_list->getItemFromListForDisplayByItemID('occurrence_types', $va_match_info['type_id'], true)
                );
        ...
        if ($this->request->config->get('quicksearch_return_ca_collections')) {
            $va_data['ca_collections'][_t('Collections')] = caExtractValuesByUserLocale(
                SearchEngine::quickSearch(
                    $vs_search,
                    'ca_collections',
                    13,
