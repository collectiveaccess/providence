...
class BaseSearchController extends BaseRefineableSearchController {
	 ...
	 *        output_format = determines format out search result output. "PDF" and "HTML" are currently supported;
	 *        "HTML" is the default view = view with path relative to controller to use overriding default
	 *        ("search/<table_name>_search_basic_html.php") vars = associative array with key value pairs to assign to
	 *        the view
	 *
	 * Callbacks:
	 *        hookBeforeNewSearch() is called just before executing a new search. The first parameter is the
	 *        BrowseEngine object containing the search.
	 */
	public function Index( $pa_options = null ) {
		...
		$vs_search
		                  = html_entity_decode( $this->opo_result_context->getSearchExpression() );    // decode entities encoded to avoid Apache request parsing issues (Eg. forward slashes [/] in searches)
		$vb_is_new_search = $this->opo_result_context->isNewSearch();

		if ( (bool) $this->request->getParameter( 'reset', pString )
		     && ( $this->request->getParameter( 'reset', pString ) != 'save' )
		) {
			$vs_search        = '';
		...
		if ( $vs_search ) { /* any request? */
			...
			if ( $this->opn_type_restriction_id > 0 ) {
				$exclude_type_ids = caMakeTypeIDList( $this->ops_tablename,
					$this->request->config->getList( $this->ops_tablename . '_find_dont_expand_hierarchically' ),
					[ 'dontIncludeSubtypesInTypeRestriction' => true ] );
				$po_search->setTypeRestrictions( [ $this->opn_type_restriction_id ],
					[ 'dontExpandHierarchically' => in_array( $this->opn_type_restriction_id, $exclude_type_ids ) ] );
			}
			...
			if ( is_subclass_of( $po_search, "BrowseEngine" ) ) {
				...
				//
				if ( $vs_facet_group = $this->request->config->get( $this->ops_tablename
				                                                    . '_search_refine_facet_group' )
				) {
				...
				if ( $vs_group_name = $this->request->config->get( 'browse_facet_group_for_'
				                                                   . $this->ops_tablename )
				) {
				...
				if ( ! is_array( $va_facets_with_info = $po_search->getInfoForAvailableFacets() )
			...
			default:
				...
				if ( $this->opb_uses_hierarchy_browser ) {
					...
					if ( $vs_type_list_code = $t_model->getTypeListCode() ) {
						...
						$this->view->setVar( 'type_menu',
							$t_list->getListAsHTMLFormElement( $vs_type_list_code, 'type_id',
								array( 'id' => 'hierTypeList' ) ) );
					}

					// set last browse id for hierarchy browser
					$vn_id = ( $this->request->config->get( $this->ops_tablename . '_dont_remember_last_browse_item' ) )
						? null : intval( Session::getVar( $this->ops_tablename . '_browse_last_id' ) );
	...
	public function _genTypeNav( $pa_params ) {
		...
		if ( $t_subject->getAppConfig()->get( 'perform_type_access_checking' ) ) {
			$va_restrict_to_types = caGetTypeRestrictionsForUser( $this->ops_tablename,
				array( 'access' => __CA_BUNDLE_ACCESS_READONLY__ ) );
		}

		$limit_to_types = $this->getRequest()->config->getList( $this->ops_tablename
		                                                        . '_navigation_find_menu_limit_types_to' );
		$exclude_types  = $this->getRequest()->config->getList(
			$z = $this->ops_tablename . '_navigation_find_menu_exclude_types' );
		...
		if ( is_array( $va_hier ) ) {
			...
			foreach ( $va_hier as $vn_item_id => $va_item ) {
				...
				if (
					! $this->getRequest()->config->get( $this->ops_tablename
					                                    . '_navigation_find_menu_shows_top_level_types_only' )
					&& ( isset( $va_item['item_id'] ) && isset( $va_types_by_parent_id[ $va_item['item_id'] ] )
					     && is_array( $va_types_by_parent_id[ $va_item['item_id'] ] ) )
				) {
					$va_subtypes = $this->_getSubTypes( $va_types_by_parent_id[ $va_item['item_id'] ],
						$va_types_by_parent_id, $va_restrict_to_types );
				} else {
					$va_subtypes = method_exists( $this, "_getSubTypeActionNav" )
						? $this->_getSubTypeActionNav( $va_item ) : [];
				}
				$va_types[] = array(
	...
	private function _getSubTypes( $pa_subtypes, $pa_types_by_parent_id, $pa_restrict_to_types = null ) {
		...
		foreach ( $pa_subtypes as $vn_i => $va_type ) {
			...
			if ( isset( $pa_types_by_parent_id[ $va_type['item_id'] ] )
			     && is_array( $pa_types_by_parent_id[ $va_type['item_id'] ] )
			) {
				$va_subsubtypes = $this->_getSubTypes( $pa_types_by_parent_id[ $va_type['item_id'] ],
					$pa_types_by_parent_id, $pa_restrict_to_types );
			} else {
				$va_subsubtypes = array();
			}

			if ( is_array( $limit_to_types ) && sizeof( $limit_to_types )
			     && ! in_array( $va_type['idno'], $limit_to_types )
			) {
				continue;
			}
			if ( is_array( $exclude_types ) && sizeof( $exclude_types )
			     && in_array( $va_type['idno'], $exclude_types )
			) {
				continue;
			}
	...
	public function lookup() {
		...
		if ( $this->request->config->get( 'quicksearch_return_ca_objects' ) ) {
			$va_results = caExtractValuesByUserLocale( SearchEngine::quickSearch( $vs_search, 'ca_objects', 57,
				array( 'limit' => 3, 'checkAccess' => $va_access_values ) ) );
			// break found objects out by type
			foreach ( $va_results as $vn_id => $va_match_info ) {
				$vs_type                                     = caUcFirstUTF8Safe( $t_list->getItemFromListForDisplayByItemID( 'object_types',
					$va_match_info['type_id'], true ) );
				$va_data['ca_objects'][ $vs_type ][ $vn_id ] = $va_match_info;
			}
		}

		if ( $this->request->config->get( 'quicksearch_return_ca_entities' ) ) {
			$va_data['ca_entities'][ _t( 'Entities' ) ]
				= caExtractValuesByUserLocale( SearchEngine::quickSearch( $vs_search, 'ca_entities', 20,
				array( 'limit' => 10, 'checkAccess' => $va_access_values ) ) );
		}

		if ( $this->request->config->get( 'quicksearch_return_ca_places' ) ) {
			$va_data['ca_places'][ _t( 'Places' ) ]
				= caExtractValuesByUserLocale( SearchEngine::quickSearch( $vs_search, 'ca_places', 72,
				array( 'limit' => 10, 'checkAccess' => $va_access_values ) ) );
		}

		if ( $this->request->config->get( 'quicksearch_return_ca_occurrences' ) ) {
			$va_results = caExtractValuesByUserLocale( SearchEngine::quickSearch( $vs_search, 'ca_occurrences', 67,
				array( 'limit' => 10, 'checkAccess' => $va_access_values ) ) );
			// break found occurrences out by type
			foreach ( $va_results as $vn_id => $va_match_info ) {
				$vs_type                                         = caUcFirstUTF8Safe( $t_list->getItemFromListForDisplayByItemID( 'occurrence_types',
					$va_match_info['type_id'], true ) );
		...
		if ( $this->request->config->get( 'quicksearch_return_ca_collections' ) ) {
			$va_data['ca_collections'][ _t( 'Collections' ) ]
				= caExtractValuesByUserLocale( SearchEngine::quickSearch( $vs_search, 'ca_collections', 13,
				array( 'limit' => 10, 'checkAccess' => $va_access_values ) ) );
		}
