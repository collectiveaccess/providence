...
class BaseQuickAddController extends ActionController {
	...
	public function Form( $pa_values = null, $pa_options = null ) {
		...
		if ( ! ( is_array( $pa_options ) && isset( $pa_options['dontCheckQuickAddAction'] )
		         && (bool) $pa_options['dontCheckQuickAddAction'] )
		     && ( ! $this->request->user->canDoAction( 'can_quickadd_' . $t_subject->tableName() ) )
		) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/2585?r='
		...
		if ( ( $vn_type_id = $t_subject->get( 'type_id' ) ) && is_array( $va_restrict_to_types )
		     && ! in_array( $vn_type_id, $va_restrict_to_types )
		) {
		...
		if ( $t_subject->getAppConfig()->get( 'perform_source_access_checking' ) ) {
			$va_restrict_to_sources = caGetSourceRestrictionsForUser( $this->ops_table_name,
				array( 'access' => $vn_subject_id ? __CA_BUNDLE_ACCESS_READONLY__ : __CA_BUNDLE_ACCESS_EDIT__ ) );

			if ( ! $t_subject->get( 'source_id' ) ) {
				$t_subject->set( 'source_id', $t_subject->getDefaultSourceID( array( 'request' => $this->request ) ) );
			}

			if ( is_array( $va_restrict_to_sources )
			     && ! in_array( $t_subject->get( 'source_id' ), $va_restrict_to_sources )
			) {
		...
		if ( is_array( $va_prepopulate_quickadd_fields = explode( ";",
			$this->request->getParameter( 'prepopulate_fields', pString ) ) )
		) {
		...
		if ( ! is_array( $va_prepopulate_quickadd_fields ) || ! sizeof( $va_prepopulate_quickadd_fields )
		     || in_array( 'preferred_labels', $va_prepopulate_quickadd_fields )
		) {
			global $g_ui_locale_id;
			$va_force_new_label = [
				'locale_id'                        => $g_ui_locale_id,
				// use default locale
				$t_subject->getLabelDisplayField() => caUcFirstUTF8Safe( $this->view->getVar( 'q' ) )
				// query text is used for display field
			];
		...
		if ( ! ( $vn_type_id = $t_subject->getTypeID() ) ) {
			if ( ! ( $vn_type_id = $this->request->getParameter( $t_subject->getTypeFieldName(), pString ) ) ) {
				if ( $vs_type = $this->request->config->get( 'quickadd_' . $t_subject->tableName()
				                                             . '_default_type' )
				) {
		...
		if ( $vs_restrict_to_types = trim( $this->request->getParameter( 'types', pString ) ) ) {
			$this->view->setVar( 'restrict_to_types',
				$va_restrict_to_type_ids = caMakeTypeIDList( $t_subject->tableName(),
					explode( ',', $vs_restrict_to_types ), array(
						'dont_include_subtypes_in_type_restriction' => (bool) $this->request->getParameter( 'dont_include_subtypes_in_type_restriction',
							pString )
					) ) );

			if ( ! in_array( $vn_type_id, $va_restrict_to_type_ids ) ) {
				$vn_type_id
					= $va_restrict_to_type_ids[0];        // get first type on list since default isn't part of restriction
			}
		}
		...
		$t_ui = ca_editor_uis::loadDefaultUI( $this->ops_table_name, $this->request, $vn_type_id,
			array( 'editorPref' => 'quickadd' ) );
		if ( ! $t_ui ) {
			$this->postError( 1100, _t( 'No interface defined for %1', $this->ops_table_name ),
				'BaseQuickAddController->Form' );
		...
		$va_field_values = $t_subject->extractValuesFromRequest( $va_nav['defaultScreen'], $this->request,
			array( 'ui_instance' => $t_ui, 'dontReturnSerialIdno' => true ) );
		...
		if ( is_array( $va_field_values['annotation_properties'] )
		     && method_exists( $t_subject, 'setPropertyValue' )
		) {
		...
		$this->view->setVar( 'default_parent_id',
			$this->opo_result_context->getParameter( $t_subject->tableName() . '_last_parent_id' ) );
	...
	public function Save( $pa_options = null ) {
		...
		if ( ! ( is_array( $pa_options ) && isset( $pa_options['dontCheckQuickAddAction'] )
			...
			$va_response = array(
				...
				'display' => null,
				'errors'  => array(
					_t( "You are not allowed to quickadd %1",
						$t_subject->getProperty( 'NAME_PLURAL' ) ) => _t( "You are not allowed to quickadd %1",
		...
		if ( $t_subject->getAppConfig()->get( 'perform_source_access_checking' ) ) {
			if ( is_array( $va_restrict_to_sources = caGetSourceRestrictionsForUser( $this->ops_table_name,
				...
				if (
					...
					$t_subject->set( 'source_id',
						$t_subject->getDefaultSourceID( array( 'request' => $this->request ) ) );
				}
		...
		if ( ( $t_subject->tableName() == 'ca_objects' )
		     && ( $vn_lot_id = $this->request->getParameter( 'lot_id', pInteger ) )
		) {
		...
		$vb_save_rc = $t_subject->saveBundlesForScreen( $this->request->getParameter( 'screen', pString ),
			$this->request, $va_opts );
		$this->view->setVar( 't_ui', $t_ui );
		...
		if ( $vn_id && ! $vb_no_save_error ) {
			$va_tmp  = caProcessRelationshipLookupLabel( $t_subject->makeSearchResult( $t_subject->tableName(),
				array( $vn_id ) ), $t_subject );
			...
			if ( $pn_related_id && $ps_related_table && $ps_relationship_type ) {
				if ( $t_rel = $t_subject->addRelationship( $ps_related_table, $pn_related_id,
					$ps_relationship_type )
				) {
	...
	protected function _initView( $pa_options = null ) {
		...
		if ( is_array( $pa_options ) && isset( $pa_options['loadSubject'] ) && (bool) $pa_options['loadSubject']
		     && ( $vn_subject_id = (int) $this->request->getParameter( $t_subject->primaryKey(), pInteger ) )
		) {
			$t_subject->load( $vn_subject_id );
		}
		if ( is_array( $pa_options ) && isset( $pa_options['forceSubjectValues'] )
		     && is_array( $pa_options['forceSubjectValues'] )
		) {
