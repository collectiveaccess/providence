...
class BaseQuickAddController extends ActionController
{
    ...
    public function Form($pa_values = null, $pa_options = null)
    {
        ...
        if (!(is_array(
                    $pa_options
                ) && isset($pa_options['dontCheckQuickAddAction']) && (bool)$pa_options['dontCheckQuickAddAction']) && (!$this->request->user->canDoAction(
                'can_quickadd_' . $t_subject->tableName()
            ))) {
            $this->response->setRedirect(
                $this->request->config->get('error_display_url') . '/n/2585?r=' . urlencode(
        ...
        if (($vn_type_id = $t_subject->get('type_id')) && is_array($va_restrict_to_types) && !in_array(
                $vn_type_id,
        ...
        if (is_array(
            $va_prepopulate_quickadd_fields = explode(";", $this->request->getParameter('prepopulate_fields', pString))
        )) {
            $va_prepopulate_quickadd_fields = array_filter(
                $va_prepopulate_quickadd_fields,
                function ($v) {
                    return strlen($v);
                }
            );
        ...
        if (!is_array($va_prepopulate_quickadd_fields) || !sizeof($va_prepopulate_quickadd_fields) || in_array(
                'preferred_labels',
                $va_prepopulate_quickadd_fields
            )) {
        ...
        if ($vs_restrict_to_types = trim($this->request->getParameter('types', pString))) {
            $this->view->setVar(
                ...
                $va_restrict_to_type_ids = caMakeTypeIDList(
                    $t_subject->tableName(),
                    explode(',', $vs_restrict_to_types),
                    array(
                        'dont_include_subtypes_in_type_restriction' => (bool)$this->request->getParameter(
        ...
        $t_ui = ca_editor_uis::loadDefaultUI(
            $this->ops_table_name,
            $this->request,
            $vn_type_id,
        ...
        if (!$t_ui) {
            $this->postError(
                1100,
                _t('No interface defined for %1', $this->ops_table_name),
        ...
        $va_field_values = $t_subject->extractValuesFromRequest(
            $va_nav['defaultScreen'],
            $this->request,
            array(
                'ui_instance' => $t_ui,
                'dontReturnSerialIdno' => true
            )
        );
    ...
    public function Save($pa_options = null)
    {
        ...
        if (!(is_array(
            ...
            $va_response = array(
                ...
                'errors' => array(
                    _t(
                        "You are not allowed to quickadd %1",
        ...
        if (!sizeof($_POST)) {
            $va_response = array(
                ...
                'display' => null,
                'errors' => array(
                    _t("Cannot save using empty request. Are you using a bookmark?") => _t(
        ...
        if (($t_subject->tableName() == 'ca_objects') && ($vn_lot_id = $this->request->getParameter(
                'lot_id',
                pInteger
            ))) {
        ...
        $vb_save_rc = $t_subject->saveBundlesForScreen(
            $this->request->getParameter('screen', pString),
            $this->request,
            $va_opts
        );
        $this->view->setVar('t_ui', $t_ui);
        ...
        if ($vn_id && !$vb_no_save_error) {
            $va_tmp = caProcessRelationshipLookupLabel(
                $t_subject->makeSearchResult($t_subject->tableName(), array($vn_id)),
                $t_subject
            );
    ...
    protected function _initView($pa_options = null)
    {
        ...
        if (is_array(
                $pa_options
            ) && isset($pa_options['loadSubject']) && (bool)$pa_options['loadSubject'] && ($vn_subject_id = (int)$this->request->getParameter(
                $t_subject->primaryKey(),
                pInteger
            ))) {
            $t_subject->load($vn_subject_id);
        }
        if (is_array($pa_options) && isset($pa_options['forceSubjectValues']) && is_array(
                $pa_options['forceSubjectValues']
            )) {
