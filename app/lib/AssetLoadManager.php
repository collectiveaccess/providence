...
class AssetLoadManager {
	...
	static function init() {
		...
		$vb_used_minified = ! $o_config->get( 'debug' ) && $o_config->get( 'minification' )
		                    && $g_asset_config->get( 'minification' );
	 ...
	 *        (1) If you pass via the $ps_package parameter a loadSet name defined in assets.conf then all of the
	 *        libraries in that loadSet will be loaded
	 *        (2) If you pass a package and library name (via $ps_package and $ps_library) then the specified library
	 *        will be loaded
	 *
	 * Whether you use a loadSet or a package/library combination, if it doesn't have a definition in assets.conf
	 * nothing will be loaded.
	 *
	 * @param $ps_package  (string) The package name containing the library to be loaded *or* a loadSet name. LoadSets
	 *                     describe a set of libraries to be loaded as a unit.
	 * @param $ps_library  (string) The name of the library contained in $ps_package to be loaded.
	 * @param $pn_priority (integer) Control order in which libraries are loaded. Higher numbers indicate earlier
	 *                     loading. Default is 10.
	 *
	...
	static function register( $ps_package, $ps_library = null, $pn_priority = 10 ) {
		...
		if ( $ps_library ) {
			...
			if ( isset( $va_list[ $ps_library ] ) && $va_list[ $ps_library ] ) {
				...
				$g_asset_load_list[ $pn_priority ][ $ps_package . '/' . $va_list[ $ps_library ] ][ $vb_is_theme_specific
					? "THEME" : "APP" ]
					= true;
				...
				if ( $o_config->get( 'allowThemeInheritance' ) ) {
					...
					while ( $vs_inherit_from_theme = trim( trim( $o_config->get( [ 'inheritFrom', 'inherit_from' ] ) ),
						...
						$g_asset_load_list[ $pn_priority ][ $ps_package . '/'
						                                    . $va_list[ $ps_library ] ]["INHERITED_THEME_{$vs_inherit_from_theme}"]
							= true;
	 ...
	 *                          dontLoadAppAssets = don't load assets specified in the app asset.conf file [default is
	 *                          false]
	 *
