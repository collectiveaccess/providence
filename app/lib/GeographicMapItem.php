...
class GeographicMapItem {
	 ...
	 *                      coordinates = an array or coordinate pairs as would be passed to setCoordinates(); if this
	 *                      setting is passed latitude and longitude settings will be ignored. label = the text label
	 *                      to use for the item; this is typically used for the item tooltip content = text content to
	 *                      use in the on-click info window; if you leave this blank no info window will appear color =
	 *                      hex color to use for marker
	 */
	public function __construct( $pa_item_info = null ) {
		if ( is_array( $pa_item_info ) ) {
			if ( isset( $pa_item_info['coordinates'] ) && is_array( $pa_item_info['coordinates'] )
			     && sizeof( $pa_item_info['coordinates'] )
			) {
				...
				if ( isset( $pa_item_info['latitude'] ) && isset( $pa_item_info['longitude'] ) ) {
					$this->addCoordinate( $pa_item_info['latitude'], $pa_item_info['longitude'],
						isset( $pa_item_info['radius'] ) ? $pa_item_info['radius'] : null,
						isset( $pa_item_info['angle'] ) ? (float) $pa_item_info['angle'] : null );
				}
			}
	 ...
	 *
	 * @param $pa_coordinate_pairs - array of coordinate pairs; each pair is an array with 'latitude' and 'longitude'
	 *                             keys
	 *
	...
	/**
	 * Returns coordinate in list at specified index; coordinates are indexed from zero (eg. index 0 is the first
	 * coordinate pair)
	 *
	...
	/**
	 * Sets content for this item. Content is typically used to fill an "info window" attached to the item when it is
	 * displayed on a map.
	 *
	 ...
	 *                    padding - the amount of space to add along each edge of the map after fitting it to the
	 *                    mapped items. This is in decimal degrees! (eg. a valid of 1 is quite large!) minWidth - the
	 *                    minimum width of the horizontal extent in decimal degrees minHeight - the minimum width of
	 *                    the vertical extent in decimal degrees
	 *
