...
class Db extends DbBase {
	 ...
	 * @param array  $pa_options          Database options like username, pw, host, etc - if ommitted, it is fetched
	...
	/**
	 * Executes a SQL statement. The SQL statement may contain question-mark ("?") placeholders whose values can be
	 * specified by parameters passed in addition to the SQL query itself. You can pass placeholder values in one of
	 * two ways:
	 *    1. As the second parameter to query() you pass an array; the values will be substituted in order for the
	 *    placeholders
	 *  2. Starting with the second parameter you can pass as many parameters as you like which will be substituted in
	 *  order for the placeholders.
	 *
	 * In either case, if the number of values passed does not equal the number of placeholders, an error will be
	 * returned.
	 *
	 ...
	 *
	 * Note that the PHP type of the values your pass matter. Non-numeric values will be quoted, while numeric values
	 * will not. You should case values to their intended type before passing them to a query. Eg.
	 * (int)$cost_of_painting
	 *
	 * @param string $ps_sql SQL statement, can contain placeholders with attached values for SQL injection avoidance
	 * @param - first place holder value, or an array of placeholder values; if it is an array then the array is used
	 *                for ALL placeholder values in order. If it is a scalar value then it will be used for the first
	 *                placeholder, and subsequent parameters used for other placeholders in order.
	 * @param array  $pa_options
	 *
	...
	public function beginTransaction() {
		...
		if ( $this->opn_transaction_count >= $this->supports( "max_nested_transactions" ) ) {
			$this->postError( 211,
				_t( "Transaction count cannot exceed maximum of %1", $this->supports( "max_nested_transactions" ) ),
				"Db->beginTransaction()" );
	 ...
	 * Returns false if you're not connected to a database.
	 *
	 * @param $ps_key possible values: 'limit', 'numrows', 'pconnect', 'prepare', 'ssl', 'transactions',
	...
	public function getFieldsFromTable( $ps_table, $ps_fieldname = null ) {
		...
		while ( $qr_cols->nextRow() ) {
			...
			$va_fields[]    = array(
				...
				"index"       => $vs_index,
				"default"     => ( $va_row['Default'] == "NULL" ) ? null
					: ( $va_row['Default'] !== "" ? $va_row['Default'] : null ),
				"options"     => $va_options
			);
		}
