  		...
  		foreach($template_dirs as $template_dir) {
			...
			foreach($va_template_names[$template_dir] as $vs_template_name) {
				...
				foreach($va_tags as $vs_tag) {
					...
					if (in_array($vs_tag_name, $va_restricted_tag_names)) { continue; }
					if (preg_match("!^media:!", $vs_tag_name)) { continue; }
					
					$va_tags_with_info[$vs_tag_name] = [];
					if (is_array($tag_parsed['options'])) {
						$tag_parsed['options'] = array_map(function($v) { return urldecode($v); }, $tag_parsed['options']);
						$va_tags_with_info[$vs_tag_name] = $tag_parsed['options'];
					}
					if (is_array($va_config[$vs_tag_name])) {
						foreach($va_config[$vs_tag_name] as $k => $v) {
							if (!isset($va_tags_with_info[$vs_tag_name][$k])) { $va_tags_with_info[$vs_tag_name][$k] = $v; }
						}
					}
