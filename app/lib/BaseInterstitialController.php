...
class BaseInterstitialController extends BaseEditorController
{
    ...
    public function __construct(&$po_request, &$po_response, $pa_view_paths = null)
    {
        ...
        $this->result_context = new ResultContext(
            $po_request,
            $this->ops_table_name,
    ...
    public function Form($pa_values = null, $options = null)
    {
        ...
        list($t_subject, $t_ui, $vn_parent_id, $vn_above_id) = $this->_initView(
            array_merge($options, array('loadSubject' => true))
        );
        ...
        $pn_placement_id = $this->request->getParameter(
            'placement_id',
            pInteger
        );        // placement_id of bundle that launched interstitial editor
        $pn_n = $this->request->getParameter(
            'n',
            pInteger
        );                // index of bundle that launched interstitial editor

        $ps_primary_table = $this->request->getParameter(
            'primary',
            pString
        );            // table name for item from which the interstitial editor was launched
        $pn_primary_id = $this->request->getParameter(
            'primary_id',
            pInteger
        );        // row_id of item from which the interstitial editor was launched
        ...
        $this->app_plugin_manager->hookEditItem(
            array(
                'id' => null,
    ...
    public function Save($options = null)
    {
        ...
        $this->app_plugin_manager->hookBeforeSaveItem(
            array(
                'id' => null,
                'table_num' => $t_subject->tableNum(),
                'table_name' => $t_subject->tableName(),
        ...
        //
        $va_bundle_values = array_shift(
            caProcessRelationshipLookupLabel(
                $qr_rel_items,
                $t_subject,
                array('template' => $vs_template, 'primaryIDs' => array($ps_primary_table => array($pn_primary_id)))
            )
        );
        ...
        if ($t_subject->hasField('type_id')) {
            if (method_exists($t_subject, "isSelfRelationship") && $t_subject->isSelfRelationship()) {
                ...
                $va_bundle_values['relationship_typename'] = $t_subject->getRelationshipTypename(
                    ($vn_left_id == $pn_primary_id) ? 'ltol' : 'rtol'
                );
            } else {
                $va_bundle_values['relationship_typename'] = $t_subject->getRelationshipTypename(
                    ($t_subject->getLeftTableFieldName() == $vs_editor_table) ? 'rtol' : 'ltor'
                );
            }
        ...
        $this->app_plugin_manager->hookSaveItem(
            array(
    ...
    protected function _initView($options = null)
    {
        ...
        if (is_array(
                $options
            ) && isset($options['loadSubject']) && (bool)$options['loadSubject'] && ($vn_subject_id = (int)$this->request->getParameter(
