...
class Zend_Amf_Util_BinaryStream
{
    ...
    public function readBytes($length)
    {
        if (($length + $this->_needle) > $this->_streamLength) {
            ...
            throw new Zend_Amf_Exception(
                'Buffer underrun at needle position: ' . $this->_needle . ' while requesting length: ' . $length
            );
        }
        $bytes = $this->_mbStringFunctionsOverloaded ? mb_substr(
            $this->_stream,
            $this->_needle,
            $length,
            '8bit'
        ) : substr($this->_stream, $this->_needle, $length);
        $this->_needle += $length;
        return $bytes;
    }
    ...
    {
        $bytes = $this->_mbStringFunctionsOverloaded ? mb_substr($this->_stream, $this->_needle, 8, '8bit') : substr(
            $this->_stream,
            $this->_needle,
            8
        );
