    ...
    public static function toNumber($value, array $options = array())
    {
        ...
        if ($group == 0) {
            ...
            if ($group == $group2) {
                ...
                $format = iconv_substr($format, 0, iconv_strpos($format, '#')) . $number . iconv_substr(
                        $format,
                        $point
                    );
            } else {
                    ...
                    switch ($part) {
                        ...
                        case '-#':
                            ...
                            if (($parts[$key + 1]) == '##0') {
                                ...
                                if (($parts[$key + 1]) == '##') {
                                    ...
                                    throw new Zend_Locale_Exception(
                                        'Unsupported token for numberformat (Pos 1):"' . $pattern . '"'
                                    );
                                }
                            }
                            break;
                        case '##':
                            if ($parts[$key + 1] == '##0') {
                                ...
                                throw new Zend_Locale_Exception(
                                    'Unsupported token for numberformat (Pos 2):"' . $pattern . '"'
                                );
                            }
                            break;
                        case '##0':
                            if ($parts[$key - 1] == '##') {
                                ...
                                if (($parts[$key - 1] == '#') || ($parts[$key - 1] == '-#')) {
                                    ...
                                    throw new Zend_Locale_Exception(
                                        'Unsupported token for numberformat (Pos 3):"' . $pattern . '"'
                                    );
                                }
                            }
                            break;
    ...
    public static function checkDateFormat($date, array $options = array())
    {
        ...
        if (((iconv_strpos($options['date_format'], 'Y', 0, 'UTF-8') !== false) or
                (iconv_strpos(
                        ...
                        0,
                        'UTF-8'
                    ) !== false)) and (!isset($date['year']) or ($date['year'] === ""))) {
            return false;
        }
        ...
        if (((iconv_strpos($options['date_format'], 'H', 0, 'UTF-8') !== false) or
                (iconv_strpos(
                        ...
                        0,
                        'UTF-8'
                    ) !== false)) and (!isset($date['hour']) or ($date['hour'] === ""))) {
            return false;
        }
