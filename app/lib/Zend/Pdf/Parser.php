    ...
    private function _loadXRefTable($offset)
    {
        ...
        if ($nextLexeme == 'xref') {
            ...
            while (($nextLexeme = $this->_stringParser->readLexeme()) != 'trailer') {
                ...
                if (!ctype_digit($refCount)) {
                    ...
                    throw new Zend_Pdf_Exception(
                        sprintf(
                            'PDF file syntax error. Offset - 0x%X. Cross-reference table subheader values must contain only digits.',
                            $this->_stringParser->offset - strlen($refCount)
                        )
                    );
                }
            ...
            if (!$xrefStream instanceof Zend_Pdf_Element_Object_Stream) {
                require_once 'Zend/Pdf/Exception.php';
                throw new Zend_Pdf_Exception(
                    sprintf('PDF file syntax error. Offset - 0x%X.  Cross-reference stream expected.', $offset)
                );
            }
            ...
            for ($count = 0; $count < $sections; $count++) {
                ...
                for ($count2 = 0; $count2 < $entries; $count2++) {
                    if ($entryField1Size == 0) {
                        ...
                        if ($entryField1Size == 1) { // Optimyze one-byte field case
