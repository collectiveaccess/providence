    ...
    implements Zend_Mail_Storage_Writable_Interface
{
    ...
    public function __construct($params)
    {
        ...
        if (!empty($params->create) && isset($params->dirname) && !file_exists(
                $params->dirname . DIRECTORY_SEPARATOR . 'cur'
            )) {
    ...
    public function setFlags($id, $flags)
    {
        ...
        $new_filename = dirname(
                dirname($filedata['filename'])
            ) . DIRECTORY_SEPARATOR . 'cur' . DIRECTORY_SEPARATOR . "$filedata[uniq]$info";
    ...
    protected function _calculateQuota($forceRecalc = false)
    {
        ...
        $messages = 0;
        $maildirsize = '';
        if (!$forceRecalc && file_exists($this->_rootdir . 'maildirsize') && filesize(
