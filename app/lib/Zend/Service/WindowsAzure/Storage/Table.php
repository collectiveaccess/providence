    ...
    extends Zend_Service_WindowsAzure_Storage_BatchStorageAbstract
{
    ...
    public function createTable($tableName = '')
    {
        ...
        $requestBody = $this->_fillTemplate(
            $requestBody,
            array(
    ...
    public function retrieveEntities(
        ...
        if (!is_null($nextPartitionKey) && !is_null($nextRowKey)) {
            ...
            $queryString .= 'NextPartitionKey=' . rawurlencode($nextPartitionKey) . '&NextRowKey=' . rawurlencode(
                    $nextRowKey
                );
        }
        ...
        if ($response->isSuccessful()) {
            ...
            // More entities?
            if (!is_null($response->getHeader('x-ms-continuation-NextPartitionKey')) && !is_null(
     ...
     * @throws Zend_Service_WindowsAzure_Exception
     */
    protected function _changeEntity(
        $httpVerb = Zend_Http_Client::PUT,
        $tableName = '',
        Zend_Service_WindowsAzure_Storage_TableEntity $entity = null,
        $verifyEtag = false
    ) {
