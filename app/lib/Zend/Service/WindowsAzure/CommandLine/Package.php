    ...
    extends Zend_Service_Console_Command
{
    ...
    public function scaffoldCommand($path, $scaffolder, $argv)
    {
        ...
        if (!class_exists('Scaffolder')) {
            ...
            throw new Zend_Service_Console_Exception(
                'Could not locate a class named Scaffolder in the given scaffolder: ' . $scaffolder . '. Make sure the scaffolder package contains a file named index.php and contains a class named Scaffolder.'
            );
        }
    ...
    public function createPackageCommand($path, $runDevFabric, $outputPath)
    {
        ...
        $windowsAzureSdkFolderCandidates = array_merge(
            isset($_SERVER['ProgramFiles']) ? glob(
                $_SERVER['ProgramFiles'] . '\Windows Azure SDK\*\bin',
                GLOB_NOSORT
            ) : array(),
            isset($_SERVER['ProgramFiles']) ? glob(
                $_SERVER['ProgramFiles(x86)'] . '\Windows Azure SDK\*\bin',
                GLOB_NOSORT
            ) : array(),
            isset($_SERVER['ProgramFiles']) ? glob(
                $_SERVER['ProgramW6432'] . '\Windows Azure SDK\*\bin',
                GLOB_NOSORT
            ) : array()
        );
