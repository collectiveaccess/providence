...
class Zend_Service_WindowsAzure_SessionHandler
{
    ...
    public function __construct(
        Zend_Service_WindowsAzure_Storage $storage,
        ...
        if (!($storage instanceof Zend_Service_WindowsAzure_Storage_Table || $storage instanceof Zend_Service_WindowsAzure_Storage_Blob)) {
            ...
            throw new Zend_Service_WindowsAzure_Exception(
                'Invalid storage back-end given. Storage back-end should be of type Zend_Service_WindowsAzure_Storage_Table or Zend_Service_WindowsAzure_Storage_Blob.'
            );
        }
        ...
        if ($sessionContainer == '' || $sessionContainerPartition == '') {
            require_once 'Zend/Service/WindowsAzure/Exception.php';
            throw new Zend_Service_WindowsAzure_Exception(
                'Session container and session partition should be specified.'
            );
        }
    ...
    public function write($id, $serializedData)
    {
        ...
        if (strlen($serializedData) >= self::MAX_TS_PROPERTY_SIZE && $this->_storageType == self::STORAGE_TYPE_TABLE) {
            throw new Zend_Service_WindowsAzure_Exception(
                'Session data exceeds the maximum allowed size of ' . self::MAX_TS_PROPERTY_SIZE . ' bytes that can be stored using table storage. Consider switching to a blob storage back-end or try reducing session data size.'
            );
        }
    ...
    public function gc($lifeTime)
    {
        if ($this->_storageType == self::STORAGE_TYPE_TABLE) {
            // In table storage
            try {
                $result = $this->_storage->retrieveEntities(
                    $this->_sessionContainer,
                    'PartitionKey eq \'' . $this->_sessionContainerPartition . '\' and sessionExpires lt ' . (time(
                        ) - $lifeTime)
                );
            ...
            if ($this->_storageType == self::STORAGE_TYPE_BLOB) {
                // In blob storage
                try {
                    $result = $this->_storage->listBlobs(
                        $this->_sessionContainer,
                        $this->_sessionContainerPartition,
                        '',
                        null,
