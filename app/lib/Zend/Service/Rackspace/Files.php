...
class Zend_Service_Rackspace_Files extends Zend_Service_Rackspace_Abstract
{
    ...
    public function getObjects($container, $options = array())
    {
        ...
        if ($result->isSuccessful()) {
            return new Zend_Service_Rackspace_Files_ObjectList(
                $this, json_decode($result->getBody(), true), $container
            );
        }
        return false;
    }
    ...
    public function getObject($container, $object, $headers = array())
    {
        ...
        $result = $this->httpCall(
            $this->getStorageUrl() . '/' . rawurlencode($container) . '/' . rawurlencode($object),
            'GET',
            $headers
        );
        $status = $result->getStatus();
    ...
    public function copyObject(
        ...
        $result = $this->httpCall(
            $this->getStorageUrl() . '/' . rawurlencode($container_dest) . '/' . rawurlencode($obj_dest),
            'PUT',
            $headers
        );
        $status = $result->getStatus();
