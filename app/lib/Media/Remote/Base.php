...
abstract class Base
{
    ...
    public static function getPluginInstance($ps_url)
    {
        ...
        if (!((bool)\Configuration::load()->get('allow_fetching_of_media_from_remote_urls'))) {
            return false;
        }
     ...
     * and transform the incoming url (from @param string $ps_url
     * @param string $ps_target
     * @throws BaseRemoteMediaException
     * @see downloadMediaForProcessing) if needed
     *
     */
    protected function downloadFromVerifiedURL($ps_url, $ps_target)
    {
        if (!((bool)ini_get('allow_url_fopen'))) {
            throw new BaseRemoteMediaException(
                "It looks like allow_url_fopen is set to false. This means CollectiveAccess is not able to download the given file. Please contact your system administrator."
            );
        }
        ...
        if (!$r_outgoing_fp) {
            throw new BaseRemoteMediaException(
                _t('Cannot open temporary file for media fetched from URL [%1]', $ps_url)
            );
        }
