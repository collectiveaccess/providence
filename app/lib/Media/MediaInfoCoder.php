...
class MediaInfoCoder {
	...
	public function getMediaInfo( $ps_version = null, $ps_key = null, $pa_options = null ) {
		if ( ! ( $va_media_info = $this->opa_media_info )
		     && ! ( $va_media_info = caGetOption( 'data', $pa_options, null ) )
		) {
			return false;
		}
		...
		if ( $va_media_info[ $ps_version ]["FILENAME"] ) {
			if ( isset( $va_media_info[ $ps_version ]["PAGES"] ) && ( $va_media_info[ $ps_version ]["PAGES"] > 1 ) ) {
				...
				return join( "/", array(
					...
					$va_media_info[ $ps_version ]["MAGIC"] . "_" . $va_media_info[ $ps_version ]["FILESTEM"] . "_"
					. $vn_page . "." . $va_media_info[ $ps_version ]["EXTENSION"]
				) );
			} else {
				return join( "/", array(
					$va_volume_info["absolutePath"],
	 ...
	 *                          localOnly = if true url to locally hosted media is always returned, even if an external
	 *                          url is available externalOnly = if true url to externally hosted media is always
	 *                          returned, even if an no external url is available
	 */
	public function getMediaUrl( $ps_version, $pa_options = null ) {
		...
		# is this mirrored?
		if ( isset( $va_volume_info["accessUsingMirror"] ) && ( $va_volume_info["accessUsingMirror"] )
	...
	public function getMediaTag( $ps_version, $pa_options = null ) {
		...
		# Use icon
		#
		if ( isset( $va_media_info[ $ps_version ] ) && isset( $va_media_info[ $ps_version ]['USE_ICON'] )
		     && ( $vs_icon_code = $va_media_info[ $ps_version ]['USE_ICON'] )
		) {
		...
		$vs_url  = caGetOption( 'usePath', $pa_options, false ) ? $this->getMediaPath( $ps_version, $pa_options )
			: $this->getMediaUrl( $ps_version, $pa_options );
		$o_media = new Media();
		...
		return $o_media->htmlTag( $va_media_info[ $ps_version ]["MIMETYPE"], $vs_url, $va_properties, $pa_options,
			$va_volume );
	}
	# ---------------------------------------------------------------------------
