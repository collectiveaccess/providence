...
class MediaProcessingSettings {
	...
	public function loadSettings( $m_table, $ps_field_name ) {
		...
		if ( ! isset( $va_field_info['MEDIA_ACCEPT'] ) || ! is_array( $va_field_info['MEDIA_ACCEPT'] ) ) {
			...
			if ( ! ( $this->opa_config_settings_as_array
				= $this->opo_config_settings->getAssoc( $vs_media_processing_setting ) )
			) {
				return false;
			}
		} else {
			...
			foreach ( $va_media_types as $vs_media_type => $va_type_info ) {
				if ( $this->opa_table_settings ) {
					if ( is_array( $this->opa_table_settings['MEDIA_TYPES'][ $vs_media_type ] ) ) {
						$va_version_list = array_merge( $va_version_list,
							$this->opa_table_settings['MEDIA_TYPES'][ $vs_media_type ]['VERSIONS'] );
					}
				} else {
					if ( $this->opo_config_settings ) {
						if ( is_array( $this->opa_config_settings_as_array['MEDIA_TYPES'][ $vs_media_type ] ) ) {
							$va_version_list = array_merge( $va_version_list,
								$this->opa_config_settings_as_array['MEDIA_TYPES'][ $vs_media_type ]['VERSIONS'] );
						}
					}
				}
			}
	...
	/**
	 * Returns default media queue settings for specified media type in an array with key'ed parameters. Returns null
	 * if the media type is not defined.
	 */
	public function getMediaTypeQueueSettings( $ps_media_type ) {
		...
		if ( isset( $va_type_settings ) ) {
			return array(
				'QUEUE'                       => isset( $va_type_settings['QUEUE'] ) ? $va_type_settings['QUEUE']
					: null,
				'QUEUED_MESSAGE'              => isset( $va_type_settings['QUEUED_MESSAGE'] )
					? $va_type_settings['QUEUED_MESSAGE'] : null,
				'QUEUE_WHEN_FILE_LARGER_THAN' => ( isset( $va_type_settings['QUEUE_WHEN_FILE_LARGER_THAN'] )
				                                   && ( (int) $va_type_settings['QUEUE_WHEN_FILE_LARGER_THAN'] > 0 ) )
					? (int) $va_type_settings['QUEUE_WHEN_FILE_LARGER_THAN'] : null,
				'QUEUE_USING_VERSION'         => isset( $va_type_settings['QUEUE_USING_VERSION'] )
					? $va_type_settings['QUEUE_USING_VERSION'] : null,
			);
		}
	 ...
	 * This is only a suggestion - it's the version to display in the absence of any overriding value provided by the
	 * user
	 */
