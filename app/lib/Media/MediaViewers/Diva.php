...
class Diva extends BaseMediaViewer implements IMediaViewer
{
        ...
        if ($o_view = BaseMediaViewer::getView($po_request)) {
            ...
            if ($pa_data['t_subject']) {
                ...
                if ($pa_data['t_instance']) {
                    ...
                    if (is_array(
                        $va_rep_ids = Diva::getViewerRepresentationIDList(
                            $po_request,
                            $ps_identifier,
                            $pa_data,
                            $pa_options
    ...
    public static function getViewerRepresentationIDList(
        ...
        if ($t_instance = caGetOption('t_instance', $pa_data, null)) {
            ...
            $vn_use_diva_for_image_list_length = caGetOption(
                'use_diva_for_image_list_length_at_least',
                $pa_data['display'],
                null
            );
            if (is_a($t_instance, "ca_object_representations") && caGetOption(
                ...
                ) && $pa_data['t_subject'] && $vn_use_diva_for_image_list_length && ($va_reps = $pa_data['t_subject']->getRepresentations(
                    ['small'],
                    null,
                    []
    ...
    public static function getViewerData($po_request, $ps_identifier, $pa_data = null, $pa_options = null)
    {
        if ($o_view = BaseMediaViewer::getView($po_request)) {
            if ($t_instance = caGetOption('t_instance', $pa_data, null)) {
                ...
                $o_view->setVar(
                    'id',
                    'caMediaOverlayDiva_' . $t_instance->getPrimaryKey() . '_' . ($vs_display_type = caGetOption(
                        'display_type',
                        $pa_data,
                        caGetOption(
                            'display_version',
                ...
                if (((($vs_display_version = caGetOption(
                            ...
                            $pa_data['display'],
                            'tilepic'
                        )) == 'tilepic')) && !$vn_use_diva_for_image_list_length) {
