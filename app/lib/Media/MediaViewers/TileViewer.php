...
class TileViewer extends BaseMediaViewer implements IMediaViewer
{
    ...
    public static function getViewerHTML($po_request, $ps_identifier, $pa_data = null, $pa_options = null)
    {
        if ($o_view = BaseMediaViewer::getView($po_request)) {
            ...
            $o_view->setVar(
                'id',
                $vs_id = 'caMediaOverlayTileViewer_' . $t_instance->getPrimaryKey(
            ...
            if (is_a($t_instance, "ca_object_representations")) {
                ...
                $vb_no_overlay = (caGetOption('no_overlay', $pa_data['display'], null) || caGetOption(
                        'noOverlay',
                        $pa_options,
                        null
                    ));
                if ($vb_no_overlay) {
                    ...
                    $o_view->setVar(
                        'viewerHTML',
                        "<a href='#' class='zoomButton' onclick='caMediaPanel.showPanel(\"" . caNavUrl(
                            $po_request,
                            '',
                            'Detail',
                            'GetMediaOverlay',
                        ...
                        ) . "\"); return false;'>" . $t_instance->getMediaTag(
                            'media',
                            $vs_version,
                            $va_viewer_opts
                        ) . "</a>"
