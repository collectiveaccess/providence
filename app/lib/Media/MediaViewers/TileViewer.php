...
class TileViewer extends BaseMediaViewer implements IMediaViewer {
	...
	public static function getViewerHTML( $po_request, $ps_identifier, $pa_data = null, $pa_options = null ) {
		if ( $o_view = BaseMediaViewer::getView( $po_request ) ) {
			...
			if ( ! $t_instance->hasMediaVersion( 'media',
				$vs_version = caGetOption( 'display_version', $pa_data['display'], 'tilepic' ) )
			) {
				if ( ! $t_instance->hasMediaVersion( 'media',
					$vs_version = caGetOption( 'alt_display_version', $pa_data['display'], 'tilepic' ) )
				) {
					$vs_version = 'original';
				}
			}
			...
			$o_view->setVar( 'id',
				$vs_id = 'caMediaOverlayTileViewer_' . $t_instance->getPrimaryKey() . '_' . ( $vs_display_type
			...
			if ( is_a( $t_instance, "ca_object_representations" ) ) {
				$va_viewer_opts = [
					...
					'annotation_load_url'   => caNavUrl( $po_request, '*', '*', 'GetAnnotations', array(
						'representation_id'      => (int) $t_instance->getPrimaryKey(),
						$t_subject->primaryKey() => (int) $t_subject->getPrimaryKey()
					) ),
					'annotation_save_url'   => caNavUrl( $po_request, '*', '*', 'SaveAnnotations', array(
						'representation_id'      => (int) $t_instance->getPrimaryKey(),
						$t_subject->primaryKey() => (int) $t_subject->getPrimaryKey()
					) ),
					'download_url'          => caNavUrl( $po_request, '*', '*', 'DownloadMedia', array(
						'representation_id'      => (int) $t_instance->getPrimaryKey(),
						$t_subject->primaryKey() => (int) $t_subject->getPrimaryKey(),
						'version'                => 'original'
					) ),
					...
					'annotationEditorUrl'   => caNavUrl( $po_request, 'editor/representation_annotations',
						'RepresentationAnnotationQuickAdd', 'Form',
						array( 'representation_id' => (int) $t_instance->getPrimaryKey() ) ),
				...
				$vb_no_overlay = ( caGetOption( 'no_overlay', $pa_data['display'], null )
				                   || caGetOption( 'noOverlay', $pa_options, null ) );
				if ( $vb_no_overlay ) {
					// HTML for tileviewer
					$o_view->setVar( 'viewerHTML', $t_instance->getMediaTag( 'media', $vs_version, $va_viewer_opts ) );
				} else {
					// HTML for tileviewer
					$o_view->setVar( 'viewerHTML',
						"<a href='#' class='zoomButton' onclick='caMediaPanel.showPanel(\"" . caNavUrl( $po_request, '',
							'Detail', 'GetMediaOverlay', array(
								'context'           => caGetOption( 'context', $pa_options, null ),
								'id'                => (int) $t_subject->getPrimaryKey(),
								'representation_id' => (int) $t_instance->getPrimaryKey(),
								'overlay'           => 1
							) ) . "\"); return false;'>" . $t_instance->getMediaTag( 'media', $vs_version,
							$va_viewer_opts ) . "</a>" );
				}
				...
				$va_viewer_opts = [
					...
					'viewer_base_url' => $po_request->getBaseUrlPath(),
					'download_url'    => caNavUrl( $po_request, '*', '*', 'DownloadMedia', array(
						'media_id'               => (int) $t_instance->getPrimaryKey(),
						$t_subject->primaryKey() => (int) $t_subject->getPrimaryKey(),
						'version'                => 'original'
					) ),
					'help_load_url'   => caNavUrl( $po_request, '*', '*', 'ViewerHelp', array() )
				...
				$va_viewer_opts = [
					...
					'viewer_base_url' => $po_request->getBaseUrlPath(),
					'download_url'    => caNavUrl( $po_request, '*', '*', 'DownloadMedia', array(
						'value_id'               => (int) $t_instance->getPrimaryKey(),
						$t_subject->primaryKey() => (int) $t_subject->getPrimaryKey(),
						'version'                => 'original'
					) ),
					'help_load_url'   => caNavUrl( $po_request, '*', '*', 'ViewerHelp', array() ),
				...
				if ( ! $t_instance->hasMediaVersion( 'value_blob',
					$vs_version = caGetOption( 'display_version', $pa_data['display'], 'original' ) )
				) {
					if ( ! $t_instance->hasMediaVersion( 'value_blob',
						$vs_version = caGetOption( 'alt_display_version', $pa_data['display'], 'original' ) )
					) {
						$vs_version = 'original';
					}
				}
