...
class UniversalViewer extends BaseMediaViewer implements IMediaViewer {
	...
	public static function getViewerHTML( $po_request, $ps_identifier, $pa_data = null, $pa_options = null ) {
		if ( $o_view = BaseMediaViewer::getView( $po_request ) ) {
			...
			$o_view->setVar( 'data_url',
				caNavUrl( $po_request, '*', '*', 'GetMediaData', $va_params, [ 'absolute' => true ] ) );
	...
	public static function getViewerData( $po_request, $ps_identifier, $pa_data = null, $pa_options = null ) {
		...
		if ( $o_view = BaseMediaViewer::getView( $po_request ) ) {
			if ( $t_instance = caGetOption( 't_instance', $pa_data, null ) ) {
				...
				$o_view->setVar( 'id',
					'caMediaOverlayUniversalViewer_' . $t_instance->getPrimaryKey() . '_' . ( $vs_display_type
						= caGetOption( 'display_type', $pa_data,
						caGetOption( 'display_version', $pa_data['display'], '' ) ) ) );

				$vn_use_universal_viewer_for_image_list_length
					= caGetOption( 'use_universal_viewer_for_image_list_length_at_least', $pa_data['display'], null );
				if ( ( ( ( $vs_display_version = caGetOption( 'display_version', $pa_data['display'], 'tilepic' ) )
				         == 'tilepic' ) )
				     && ! $vn_use_universal_viewer_for_image_list_length
				) {
					$pa_data['resources'] = $t_instance->getFileList( null, null, null,
						[ $vs_display_version, 'preview' ] );
				} elseif ( is_a( $t_instance, "ca_object_representations" ) && $pa_data['t_subject']
				           && $vn_use_universal_viewer_for_image_list_length
				) {
					$va_reps = $pa_data['t_subject']->getRepresentations( [ 'small', $vs_display_version, 'original' ],
						null, [] );

					$t_rep = new ca_object_representations();
					$va_labels
					       = $t_rep->getPreferredDisplayLabelsForIDs( caExtractArrayValuesFromArrayOfArrays( $va_reps,
						'representation_id' ) );

					foreach ( $va_reps as $va_rep ) {
						if ( is_array( $access_values ) && sizeof( $access_values )
						     && ! in_array( $va_rep['access'], $access_values )
						) {
							continue;
						}
						$pa_data['resources'][] = [
							'title'             => str_replace( "[" . caGetBlankLabelText( 'ca_object_representations' )
							                                    . "]", "", $va_labels[ $va_rep['representation_id'] ] ),
	...
	public static function searchViewerData( $po_request, $ps_identifier, $pa_data = null, $pa_options = null ) {
		if ( $o_view = BaseMediaViewer::getView( $po_request ) ) {
			...
			if (
				( $va_identifier['type'] == 'representation' )
				&& ( ! is_array( $va_pages = $t_rep->getFileList( $va_identifier['id'], 0, 1, [ 'original' ] ) )
				     || ! sizeof( $va_pages ) )
				|| ( $va_identifier['type'] == 'attribute' )
				   && ( ! is_array( $va_pages = $t_attr->getFileList( $va_identifier['id'], 0, 1, [ 'original' ] ) )
				        || ! sizeof( $va_pages ) )
			) {
				...
				switch ( $va_identifier['type'] ) {
					case 'representation':
					default:
						$va_locs = MediaContentLocationIndexer::SearchWithinMedia( $ps_q, 'ca_object_representations',
							$va_identifier['id'], 'media' );
						break;
					case 'attribute':
						$va_locs = MediaContentLocationIndexer::SearchWithinMedia( $ps_q, 'ca_attribute_values',
							$va_identifier['id'], 'value_blob' );
						break;
				}
			}
	...
	public static function autocomplete( $po_request, $ps_identifier, $pa_data = null, $pa_options = null ) {
		if ( $o_view = BaseMediaViewer::getView( $po_request ) ) {
			...
			switch ( $va_identifier['type'] ) {
				case 'representation':
				default:
					$va_matches = MediaContentLocationIndexer::autocomplete( $ps_q, 'ca_object_representations',
						$va_identifier['id'] );
					break;
				case 'attribute':
					$va_matches = MediaContentLocationIndexer::autocomplete( $ps_q, 'ca_attribute_values',
						$va_identifier['id'] );
					break;
			}
