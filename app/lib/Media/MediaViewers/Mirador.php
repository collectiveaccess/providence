...
class Mirador extends BaseMediaViewer implements IMediaViewer
{
        ...
        if ($o_view = BaseMediaViewer::getView($po_request)) {
            ...
            if ($pa_data['t_subject']) {
                ...
                if ($pa_data['t_instance']) {
                    ...
                    if (is_array(
                        $va_rep_ids = Mirador::getViewerRepresentationIDList(
                            $po_request,
                            $ps_identifier,
                            $pa_data,
                            $pa_options
    ...
    public static function getViewerRepresentationIDList(
        ...
        if ($t_instance = caGetOption('t_instance', $pa_data, null)) {
            ...
            if (is_a($t_instance, "ca_object_representations") && caGetOption(
                    'expand_hierarchically',
                    $pa_data['display'],
                    null
                ) && $pa_data['t_subject'] && $pa_data['t_subject']->isHierarchical() && (is_array(
                    $va_ids = $pa_data['t_subject']->getHierarchy(null, ['idsOnly' => true, 'sort' => 'idno_sort'])
                )) && sizeof($va_ids)) {
                $vn_root_id = $pa_data['t_subject']->getHierarchyRootID();
                $va_ids = array_filter(
                    $va_ids,
                    function ($v) use ($vn_root_id) {
                        return $v != $vn_root_id;
                    }
                );
                return array_values(
                ...
                ) && $pa_data['t_subject'] && $vn_use_mirador_for_image_list_length && ($va_reps = $pa_data['t_subject']->getRepresentations(
                    ['small'],
                    null,
                    []
    ...
    public static function getViewerData($po_request, $ps_identifier, $pa_data = null, $pa_options = null)
    {
        ...
        if ($o_view = BaseMediaViewer::getView($po_request)) {
            if ($t_instance = caGetOption('t_instance', $pa_data, null)) {
                ...
                $o_view->setVar(
                    'id',
                    'caMediaOverlayMirador_' . $t_instance->getPrimaryKey() . '_' . ($vs_display_type = caGetOption(
                        'display_type',
                        $pa_data,
                        caGetOption(
                            'display_version',
                ...
                if (((($vs_display_version = caGetOption(
                            'display_version',
                            $pa_data['display'],
                            'tilepic'
                        )) == 'tilepic')) && !$vn_use_mirador_for_image_list_length) {
                    ...
                    if (is_a($t_instance, "ca_object_representations") && caGetOption(
                            ...
                            $va_ids = $pa_data['t_subject']->getHierarchy(
                                null,
                        ...
                        $va_reps = $pa_data['t_subject']->getPrimaryMediaForIDs(
                            $va_ids,
                            [
                                'small',
                                $vs_display_version,
                    ...
                    if (sizeof($va_reps) < $vn_use_mirador_for_image_list_length) {
                        ...
                        $va_labels = $t_rep->getPreferredDisplayLabelsForIDs(
                            caExtractArrayValuesFromArrayOfArrays($va_reps, 'representation_id')
                        );

                        foreach ($va_reps as $va_rep) {
                            if (is_array($access_values) && sizeof($access_values) && !in_array(
                                    $va_rep['access'],
                                    $access_values
                                )) {
                                continue;
