<?php
/** ---------------------------------------------------------------------
 * app/lib/Media/MediaViewers/EmbeddedMedia.php :
 * ----------------------------------------------------------------------
 * CollectiveAccess
...
class EmbeddedMedia extends BaseMediaViewer implements IMediaViewer {
	# -------------------------------------------------------
	/**
	 *
	 */
	...
	public static function getViewerHTML($po_request, $ps_identifier, $pa_data=null, $pa_options=null) {
		if ($o_view = BaseMediaViewer::getView($po_request)) {
			...
			$o_view->setVar('viewer', 'EmbeddedMedia');
			...
			$media_info = $t_instance->getMediaInfo('media');
			$va_viewer_opts = [
			...
			// HTML for EmbeddedMedia
			$mv = new \CA\MediaUrl();
			$tag = $mv->embedTag($media_info['INPUT']['FETCHED_FROM']);
			$o_view->setVar('viewerHTML', $tag);
