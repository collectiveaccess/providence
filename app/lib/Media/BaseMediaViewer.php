...
class BaseMediaViewer {
	...
	static public function prepareViewerHTML( $po_request, $o_view, $pa_data = null, $pa_options = null ) {
		...
		if ( $t_subject ) {
			...
			if ( ( $vs_media_overlay_titlebar_template = $po_request->config->get( 'media_overlay_titlebar_template' ) )
			     && ( is_a( $t_instance, 'BundlableLabelableBaseModelWithAttributes' ) )
			) {
				// for everything except ca_site_page_media when a template is defined
				$vs_media_overlay_titlebar_text = caProcessTemplateForIDs( $vs_media_overlay_titlebar_template,
					$t_subject->tableName(), [ $t_subject->getPrimaryKey() ], $pa_options );
			} elseif ( is_a( $t_instance, 'BundlableLabelableBaseModelWithAttributes' ) ) {
				// for everything except ca_site_page_media
				$vs_media_overlay_titlebar_text = caTruncateStringWithEllipsis( $t_subject->get( $t_subject->tableName()
				                                                                                 . '.preferred_labels' ),
						80 ) . " (" . $t_subject->get( $t_subject->tableName() . '.'
				                                       . $t_subject->getProperty( 'ID_NUMBERING_ID_FIELD' ) ) . ")";
			} else {
				// for ca_site_page_media
				$vs_media_overlay_titlebar_text
					= caTruncateStringWithEllipsis( $t_instance->get( $t_instance->tableName() . '.'
					                                                  . array_shift( $t_instance->getProperty( 'LIST_FIELDS' ) ) ),
						80 ) . " (" . $t_instance->get( $t_instance->tableName() . '.'
		...
		if ( $t_subject && $t_instance && is_a( $t_instance, 'ca_object_representations' ) ) {
			if ( ( $vn_num_media = $t_media->getRepresentationCount( [ 'checkAccess' => $pa_check_access ] ) ) > 1 ) {
				...
				if ( $vn_rep_index > 0 ) {
					$vs_controls .= "<a href='#' onClick='jQuery(\"#caMediaPanelContentArea\").load(\""
					                . caNavUrl( $po_request, '*', '*', $po_request->getAction(), array(
							'representation_id'      => (int) $va_ids[ $vn_rep_index - 1 ],
							$t_subject->primaryKey() => (int) $t_subject->getPrimaryKey(),
							'context'                => $vs_context
						) ) . "\");'>←</a>";
				}
		...
		if ( $t_subject && $t_instance
		     && ( $po_request->user->canDoAction( 'can_download_media' )
		          || $po_request->user->canDoAction( 'can_download_ca_object_representations' ) )
		) {
			if ( is_array(
				$va_versions = $po_request->config->getList( 'ca_object_representation_download_versions' ) )
			) {
				...
				$vs_controls .= caFormTag( $po_request, 'DownloadMedia', 'caMediaDownloadForm', $vs_download_path,
					'post', 'multipart/form-data', '_top',
					array( 'noCSRFToken' => true, 'disableUnsavedChangesWarning' => true, 'noTimestamp' => true ) );
				$vs_controls .= _t( 'Download as %1',
					caHTMLSelect( 'version', array_combine( array_map( "_t", $va_versions ), $va_versions ),
						array( 'style' => 'font-size: 8px; height: 16px;' ) ) );
				$vs_controls .= caFormSubmitLink( $po_request,
					caNavIcon( __CA_NAV_ICON_DOWNLOAD__, 1, [], [ 'color' => 'white' ] ), '', 'caMediaDownloadForm',
					'caMediaDownloadFormButton' );
				$vs_controls .= caHTMLHiddenInput( $t_media->primaryKey(),
					array( 'value' => $t_media->getPrimaryKey() ) );
				if ( is_a( $t_instance, 'ca_object_representations' ) ) {
					$vs_controls .= caHTMLHiddenInput( "representation_id",
						array( 'value' => $t_instance->getPrimaryKey() ) );
				}
				if ( is_a( $t_instance, 'ca_site_page_media' ) ) {
					$vs_controls .= caHTMLHiddenInput( "media_id", array( 'value' => $t_instance->getPrimaryKey() ) );
				}
				if ( is_a( $t_instance, 'ca_attribute_values' ) ) {
					$vs_controls .= caHTMLHiddenInput( "value_id", array( 'value' => $t_instance->getPrimaryKey() ) );
				}
				$vs_controls .= caHTMLHiddenInput( "download", array( 'value' => 1 ) );
				$vs_controls .= "</form>\n";

				if ( is_array( $va_ids ) && ( sizeof( $va_ids ) > 1 ) ) {
					$vs_controls .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . caNavLink( $po_request,
							_t( 'Download all' ) . " " . caNavIcon( __CA_NAV_ICON_DOWNLOAD__, 1, [],
								[ 'color' => 'white' ] ), 'xxx', '*', '*', 'DownloadMedia',
