...
class BaseMediaViewer
{
     ...
     *
     */
    public static function getView($po_request)
    {
     ...
     *
     */
    public static function prepareViewerHTML($po_request, $o_view, $pa_data = null, $pa_options = null)
    {
        ...
        if ($t_subject) {
            ...
            if (($vs_media_overlay_titlebar_template = $po_request->config->get(
                ...
                $vs_media_overlay_titlebar_text = caProcessTemplateForIDs(
                    $vs_media_overlay_titlebar_template,
                    $t_subject->tableName(),
                ...
                $vs_media_overlay_titlebar_text = caTruncateStringWithEllipsis(
                        $t_subject->get($t_subject->tableName() . '.preferred_labels'),
                        80
                    ) . " (" . $t_subject->get(
                        $t_subject->tableName() . '.' . $t_subject->getProperty('ID_NUMBERING_ID_FIELD')
                    ) . ")";
            } else {
                // for ca_site_page_media
                $vs_media_overlay_titlebar_text = caTruncateStringWithEllipsis(
                        $t_instance->get(
                            $t_instance->tableName() . '.' . array_shift($t_instance->getProperty('LIST_FIELDS'))
                        ),
                        80
                    ) . " (" . $t_instance->get(
                        $t_instance->tableName() . '.' . $t_instance->getProperty('ID_NUMBERING_ID_FIELD')
                    ) . ")";
            }
        ...
        if ($t_subject && $t_instance && is_a($t_instance, 'ca_object_representations')) {
            if (($vn_num_media = $t_media->getRepresentationCount(['checkAccess' => $pa_check_access])) > 1) {
                ...
                if ($vn_rep_index > 0) {
                    $vs_controls .= "<a href='#' onClick='jQuery(\"#caMediaPanelContentArea\").load(\"" . caNavUrl(
                            $po_request,
                            '*',
                            '*',
                            $po_request->getAction(),
                            array(
        ...
        if ($t_subject && $t_instance && ($po_request->user->canDoAction(
                    'can_download_media'
            ...
            if (is_array($va_versions = $po_request->config->getList('ca_object_representation_download_versions'))) {
                ...
                $vs_controls .= _t(
                    ...
                    caHTMLSelect(
                        'version',
                        array_combine(array_map("_t", $va_versions), $va_versions),
                        array('style' => 'font-size: 8px; height: 16px;')
                    )
                );
                $vs_controls .= caFormSubmitLink(
                    $po_request,
                    caNavIcon(__CA_NAV_ICON_DOWNLOAD__, 1, [], ['color' => 'white']),
                    '',
                    'caMediaDownloadForm',
                ...
                if (is_array($va_ids) && (sizeof($va_ids) > 1)) {
                    $vs_controls .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . caNavLink(
                            $po_request,
                            _t('Download all') . " " . caNavIcon(
