...
class SearchEngine extends SearchBase
{
    ...
    public function doSearch($ps_search, $po_result = null, $pa_options = null)
    {
        ...
        $ps_search = preg_replace(
            '![\|]([A-Za-z0-9_,;]+[:]{1})!',
            "/$1",
            $ps_search
        );    // allow | to be used in lieu of / as the relationship type separator, as "/" is problematic to encode in GET requests
        $ps_search = preg_replace(
            '/(?!")\[' . caGetBlankLabelText() . '\](?!")/i',
            '"[' . caGetBlankLabelText() . ']"',
            $ps_search
        ...
        $ps_search = preg_replace(
            ...
            '"[' . _t('SET') . ']"',
            $ps_search
        ); // the special [SET] search term, which returns records that have *any* content in a specific fields, has to be quoted in order to protect the square brackets from the parser.
        ...
        if (is_array($va_suffixes) && sizeof($va_suffixes) && (!preg_match(
                '!"!',
                $ps_search
        ...
        if (is_array($va_rewrite_regexs = $this->opo_search_config->get('rewrite_regexes'))) {
            if (isset($va_rewrite_regexs[$this->ops_tablename]) && is_array($va_rewrite_regexs[$this->ops_tablename])) {
                $va_rewrite_regexs = $va_rewrite_regexs[$this->ops_tablename];
            }
            foreach ($va_rewrite_regexs as $vs_regex_name => $va_rewrite_regex) {
                $ps_search = preg_replace(
                    "!" . trim($va_rewrite_regex[0]) . "!",
                    trim($va_rewrite_regex[1]),
                    $ps_search
                );
            }
        }

        if ((is_array($va_idno_regexs = $this->opo_search_config->get('idno_regexes'))) && (!preg_match(
                "!" . $this->ops_tablename . ".{$vs_idno_fld}!",
                $ps_search
            ))) {
            if (isset($va_idno_regexs[$this->ops_tablename]) && is_array($va_idno_regexs[$this->ops_tablename])) {
                $va_idno_regexs = $va_idno_regexs[$this->ops_tablename];
            }
            foreach ($va_idno_regexs as $vs_idno_regex) {
                if ((preg_match(
                    ...
                    $ps_search = str_replace(
                        $va_matches[0],
                        $this->ops_tablename . ".{$vs_idno_fld}:\"" . $va_matches[0] . "\"",
                        $ps_search
                    );
                }
            }
        ...
        $vs_cache_key = md5(
            $ps_search . "/" . serialize($this->getTypeRestrictionList($pa_options)) . "/" . serialize(
        ...
        if (!$vb_from_cache) {
            ...
            $ps_search = preg_replace(
                "/\[((?!SET|BLANK)[A-Za-z0-9\-]+[ ]+(?!to)[^\]]*)\]/",
                "$1",
                $ps_search
            );        // remove search strings (but not range expressions) from square brackets so they may be searched
            ...
            $o_rewritten_query = new Zend_Search_Lucene_Search_Query_Boolean(
                $va_rewrite_results['terms'],
            ...
            if ((isset($pa_options['deletedOnly']) && $pa_options['deletedOnly']) && $t_table->hasField('deleted')) {
                ...
                if ((!isset($pa_options['showDeleted']) || !$pa_options['showDeleted']) && $t_table->hasField(
                        'deleted'
                    )) {
            ...
            $vb_no_types = false;
            if (!$pa_options['expandToIncludeParents'] && is_array(
                    $va_type_ids = $this->getTypeRestrictionList()
                ) && (sizeof($va_type_ids) > 0) && $t_table->hasField('type_id')) {
            ...
            if (!$vb_no_types) {
                ...
                $vb_do_acl = $this->opo_app_config->get('perform_item_level_access_checking') && method_exists(
                        $t_table,
                        "supportsACL"
                    ) && $t_table->supportsACL();

                $o_res = $this->opo_engine->search(
                    $this->opn_tablenum,
                    $vs_search,
                    $this->opa_result_filters,
                    $o_rewritten_query
                ...
                if ($pa_options['expandToIncludeParents'] && sizeof($va_hits)) {
                    ...
                    while ($qr_exp->nextHit()) {
                        if ($vn_parent_id = $qr_exp->get('parent_id')) {
                            if (
                            ((!$va_type_ids) || (in_array(
                                    $qr_exp->get($this->opn_tablenum . '.parent.type_id'),
                                    $va_type_ids
                                )))
                            ) {
    ...
    public function filterHitsBySets($pa_hits, $pa_set_ids, $pa_options = null)
    {
        ...
        $vs_search_tmp_table = $this->loadListIntoTemporaryResultTable(
            $pa_hits,
            md5(
                isset($pa_options['search']) ? $pa_options['search'] : rand(
                    0,
                    1000000
                )
            )
        ...
        foreach ($va_items as $o_term) {
            switch (get_class($o_term)) {
                case 'Zend_Search_Lucene_Search_Query_Preprocessing_Term':
                    $va_terms[] = new Zend_Search_Lucene_Search_Query_Term(
                        new Zend_Search_Lucene_Index_Term($o_term->__toString())
                    );
                ...
                case 'Zend_Search_Lucene_Search_Query_Term':
                    ...
                    if (sizeof($va_rewritten_terms['terms']) == 1) {
                        $va_terms[] = new Zend_Search_Lucene_Search_Query_Term(
                            array_shift($va_rewritten_terms['terms'])
                        );
                        $va_signs[] = array_shift($va_rewritten_terms['signs']);
                    } else {
                        for ($vn_j = 0; $vn_j < sizeof($va_rewritten_terms['terms']); $vn_j++) {
                            $va_terms[] = new Zend_Search_Lucene_Search_Query_MultiTerm(
                                array($va_rewritten_terms['terms'][$vn_j]), array($va_rewritten_terms['signs'][$vn_j])
                            );
                            $va_signs[] = $va_rewritten_terms['signs'][$vn_j] ? true : is_null(
                                $va_rewritten_terms['signs'][$vn_j]
                            ) ? null : false;
                        }
                    }
                    break;
                case 'Zend_Search_Lucene_Index_Term':
                    $va_rewritten_terms = $this->_rewriteTerm(
                        new Zend_Search_Lucene_Search_Query_Term($o_term),
                        $va_old_signs[$vn_i]
                    );
                    if (sizeof($va_rewritten_terms['terms']) == 1) {
                        $o_mt = new Zend_Search_Lucene_Search_Query_Term($va_rewritten_terms['terms'][0]);
                    } else {
                        $o_mt = new Zend_Search_Lucene_Search_Query_MultiTerm(
                            $va_rewritten_terms['terms'],
                            $va_rewritten_terms['signs']
                        );
                    }
                    $va_terms[] = $o_mt;
                    $va_signs[] = sizeof($va_rewritten_terms['signs']) ? array_shift(
                        $va_rewritten_terms['signs']
                    ) : true;
                    break;
                case 'Zend_Search_Lucene_Search_Query_Wildcard':
                    $va_rewritten_terms = $this->_rewriteTerm(
                        new Zend_Search_Lucene_Search_Query_Term($o_term->getPattern()),
                        $va_old_signs[$vn_i]
                    );
                    $o_mt = new Zend_Search_Lucene_Search_Query_MultiTerm(
                        $va_rewritten_terms['terms'],
        ...
        if (sizeof($va_access_points = $this->getAccessPoints($this->opn_tablenum))) {
            ...
            if (
                ...
                foreach ($va_fields as $vs_field) {
                    ...
                    if (isset($va_ap_info['options']) && ($va_ap_info['options']['DONT_STEM'] || in_array(
                                'DONT_STEM',
                                $va_ap_info['options']
                            ))) {
                        $vs_term .= '|';
                    }
                    $va_terms['terms'][] = new Zend_Search_Lucene_Index_Term(
                        $vs_term,
                    ...
                    return array(
                        'terms' => array(
                            new Zend_Search_Lucene_Index_Term(
                                (string)((sizeof($va_matches) > 1) ? $va_matches[1] : $va_matches[0]),
                                "{$vs_table_name}.{$vs_idno_fld}"
                            )
                        ),
                        'signs' => array($pb_sign),
                        'options' => array()
            ...
            if ($t_instance = Datamodel::getInstanceByTableName($va_tmp2[0], true)) {
                if (method_exists($t_instance, "getLabelTableName")) {
                    return array(
                        'terms' => array(
                            new Zend_Search_Lucene_Index_Term(
                                $po_term->getTerm()->text,
                                $t_instance->getLabelTableName(
                                ) . '.' . ((isset($va_tmp2[2]) && $va_tmp2[2]) ? $va_tmp2[2] : $t_instance->getLabelDisplayField(
                                )) . ($va_tmp[1] ? '/' . $va_tmp[1] : '')
                            )
                        ),
                        'signs' => array($pb_sign),
    ...
    private function _rewritePhrase($po_term, $pb_sign)
    {
        ...
        if (sizeof($va_access_points = $this->getAccessPoints($this->opn_tablenum))) {
            ...
            if (
                ...
                foreach ($va_fields as $vs_field) {
                    $va_terms['terms'][] = new Zend_Search_Lucene_Search_Query_Phrase(
        ...
        if (sizeof($va_access_points = $this->getAccessPoints($this->opn_tablenum))) {
            ...
            if ($va_ap_info = $va_access_points[$po_range->getField()]) {
                ...
                foreach ($va_fields as $vs_field) {
                    ...
                    $o_range = new Zend_Search_Lucene_Search_Query_Range(
                        $po_range->getLowerTerm(),
                        $po_range->getUpperTerm(),
                        (($vs_bool == 'OR') ? null : true)
                    );
                ...
                if (is_array($va_additional_criteria = $va_ap_info['additional_criteria'])) {
                    foreach ($va_additional_criteria as $vs_criterion) {
                        $va_terms[] = new Zend_Search_Lucene_Search_Query_Term(
                            new Zend_Search_Lucene_Index_Term($vs_criterion)
                        );
                    }
                }
                return $va_terms;
    ...
    static public function getSearchExpressionForDisplay($ps_search, $ps_table)
    {
        ...
        if ($t_instance = Datamodel::getInstanceByTableName($ps_table, true)) {
            ...
            try {
                ...
                // Retry search with all non-alphanumeric characters removed
                try {
                    $o_parsed_query = $o_query_parser->parse(
                        preg_replace("![^A-Za-z0-9 ]+!", " ", $ps_search),
                        $vs_char_set
                    );
                } catch (Exception $e) {
                    $o_parsed_query = $o_query_parser->parse("", $vs_char_set);
            ...
            foreach ($va_field_list as $vs_field) {
                ...
                if ($vs_label = $t_instance->getDisplayLabel($vs_field_proc)) {
                    $ps_search = str_replace(
                        $vs_field,
                        $vs_rel_type ? _t("%1 [as %2]", $vs_label, $vs_rel_type) : $vs_label,
                        $ps_search
                    );
                }
            }
