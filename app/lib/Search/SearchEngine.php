...
class SearchEngine extends SearchBase {
	 ...
	 * @param SearchResult $po_result  A newly instantiated sub-class of SearchResult to place search results into and
	 *                                 return. If this is not set, then a generic SearchResults object will be
	 *                                 returned.
	 * @param array        $pa_options Optional array of options for the search. Options include:
	 *
	 *        sort = field or attribute to sort on in <table name>.<field or attribute name> format (eg.
	 *        ca_objects.idno); default is to sort on relevance (aka. sort='_natural') sortDirection = direction to
	 *        sort results by, either 'asc' for ascending order or 'desc' for descending order; default is 'asc'
	 *        no_cache = if true, search is performed regardless of whether results for the search are already cached;
	 *        default is false limit = if set then search results will be limited to the quantity specified. If not set
	 *        then all results are returned. form_id = optional form identifier string to record in log for search
	 *        log_details = optional form description to record in log for search search_source = optional source
	 *        indicator text to record in log for search checkAccess = optional array of access values to filter
	 *        results on showDeleted = if set to true, related items that have been deleted are returned. Default is
	 *        false. deletedOnly = if set to true, only deleted items are returned. Default is false. limitToModifiedOn
	 *        = if set returned results will be limited to rows modified within the specified date range. The value should be a date/time expression parse-able by TimeExpressionParser sets = if value is a list of set_ids, only rows that are members of those sets will be returned user_id = If set item level access control is performed relative to specified user_id, otherwise defaults to logged in user dontFilterByACL = if true ACL checking is not performed on results appendToSearch = restrictSearchToFields = rootRecordsOnly = Only return records that are the root of whatever hierarchy they are in. [Default is false]
	 *
	 * @return SearchResult Results packages in a SearchResult object, or sub-class of SearchResult if an instance was
	 *                      passed in $po_result
	...
	public function doSearch( $ps_search, $po_result = null, $pa_options = null ) {
		...
		$ps_search = preg_replace( '![\|]([A-Za-z0-9_,;]+[:]{1})!', "/$1",
			$ps_search );    // allow | to be used in lieu of / as the relationship type separator, as "/" is problematic to encode in GET requests
		$ps_search = preg_replace( '/(?!")\[' . caGetBlankLabelText( $this->ops_tablename ) . '\](?!")/i',
			'"[' . caGetBlankLabelText( $this->ops_tablename ) . ']"',
			$ps_search ); // the special [BLANK] search term, which returns records that have *no* content in a specific fields, has to be quoted in order to protect the square brackets from the parser.
		$ps_search = preg_replace( '/(?!")\[' . _t( 'SET' ) . '\](?!")/i', '"[' . _t( 'SET' ) . ']"',
			$ps_search ); // the special [SET] search term, which returns records that have *any* content in a specific fields, has to be quoted in order to protect the square brackets from the parser.
		...
		if ( is_array( $va_rewrite_regexs = $this->opo_search_config->get( 'rewrite_regexes' ) ) ) {
			if ( isset( $va_rewrite_regexs[ $this->ops_tablename ] )
			     && is_array( $va_rewrite_regexs[ $this->ops_tablename ] )
			) {
				foreach ( $va_rewrite_regexs[ $this->ops_tablename ] as $vs_regex_name => $va_rewrite_regex ) {
					$ps_search = preg_replace( "!" . trim( $va_rewrite_regex[0] ) . "!", trim( $va_rewrite_regex[1] ),
						$ps_search );
				}
			}
		}
		...
		if ( ( is_array( $va_idno_regexs = $this->opo_search_config->get( 'idno_regexes' ) ) )
		     && ( ! preg_match( "!" . $this->ops_tablename . ".{$vs_idno_fld}!", $ps_search ) )
		) {
			if ( isset( $va_idno_regexs[ $this->ops_tablename ] )
			     && is_array( $va_idno_regexs[ $this->ops_tablename ] )
			) {
				foreach ( $va_idno_regexs[ $this->ops_tablename ] as $vs_idno_regex ) {
					if ( ( preg_match( "!{$vs_idno_regex}!", $ps_search, $va_matches ) )
					     && ( $t_instance
							= Datamodel::getInstanceByTableName( $this->ops_tablename, true ) )
					     && ( $vs_idno_fld = $t_instance->getProperty( 'ID_NUMBERING_ID_FIELD' ) )
					) {
						$ps_search = str_replace( $va_matches[0],
							$this->ops_tablename . ".{$vs_idno_fld}:\"" . $va_matches[0] . "\"", $ps_search );
					}
				}
			}
		}
		...
		$vs_cache_key = md5( $ps_search . "/" . serialize( $this->getTypeRestrictionList( $pa_options ) ) . "/"
		                     . serialize( $this->opa_result_filters ) );
		...
		if ( ! $vb_from_cache ) {
			...
			$ps_search = preg_replace( "/\[((?!SET|BLANK)[A-Za-z0-9\-]+[ ]+(?!to)[^\]]*)\]/", "$1",
				$ps_search );        // remove search strings (but not range expressions) from square brackets so they may be searched
			...
			$o_rewritten_query  = new Zend_Search_Lucene_Search_Query_Boolean( $va_rewrite_results['terms'],
				$va_rewrite_results['signs'] );
			...
			if ( ( isset( $pa_options['deletedOnly'] ) && $pa_options['deletedOnly'] )
			     && $t_table->hasField( 'deleted' )
			) {
			...
			$vb_no_types = false;
			if ( ! $pa_options['expandToIncludeParents'] && is_array( $va_type_ids = $this->getTypeRestrictionList() )
			     && ( sizeof( $va_type_ids ) > 0 )
			...
			if ( ! $vb_no_types ) {
				...
				if ( in_array( $t_table->getHierarchyType(),
				...
				if ( is_array( $va_restrict_to_fields = caGetOption( 'restrictSearchToFields', $pa_options, null ) )
				     && $this->opo_engine->can( 'restrict_to_fields' )
				) {
				...
				$vb_do_acl = $this->opo_app_config->get( 'perform_item_level_access_checking' )
				             && method_exists( $t_table, "supportsACL" )
				             && $t_table->supportsACL();

				$o_res = $this->opo_engine->search( $this->opn_tablenum, $vs_search, $this->opa_result_filters,
					$o_rewritten_query );
				...
				if ( $pa_options['expandToIncludeParents'] && sizeof( $va_hits ) ) {
					...
					while ( $qr_exp->nextHit() ) {
						if ( $vn_parent_id = $qr_exp->get( 'parent_id' ) ) {
							if (
							( ( ! $va_type_ids )
							  || ( in_array( $qr_exp->get( $this->opn_tablenum . '.parent.type_id' ), $va_type_ids ) ) )
							) {
			...
			if ( ( ! isset( $pa_options['dontFilterByACL'] ) || ! $pa_options['dontFilterByACL'] ) && $vb_do_acl ) {
				$va_hits = $this->filterHitsByACL( $va_hits, $this->opn_tablenum, $vn_user_id,
			...
			// cache for later use
			if ( ! $vb_no_cache ) {
				$o_cache->save( $vs_cache_key, $this->opn_tablenum, $va_hits, array( 'created_on' => time() ), null,
					$pa_options );
			}
	...
	public function filterHitsBySets( $pa_hits, $pa_set_ids, $pa_options = null ) {
		...
		$vs_search_tmp_table = $this->loadListIntoTemporaryResultTable( $pa_hits,
			md5( isset( $pa_options['search'] ) ? $pa_options['search'] : rand( 0, 1000000 ) ) );
		...
		foreach ( $va_items as $o_term ) {
			switch ( get_class( $o_term ) ) {
				case 'Zend_Search_Lucene_Search_Query_Preprocessing_Term':
					$va_terms[]
						        = new Zend_Search_Lucene_Search_Query_Term( new Zend_Search_Lucene_Index_Term( $o_term->__toString() ) );
				...
				case 'Zend_Search_Lucene_Search_Query_Term':
					...
					if ( sizeof( $va_rewritten_terms['terms'] ) == 1 ) {
						$va_terms[]
							        = new Zend_Search_Lucene_Search_Query_Term( array_shift( $va_rewritten_terms['terms'] ) );
						$va_signs[] = array_shift( $va_rewritten_terms['signs'] );
					} else {
						for ( $vn_j = 0; $vn_j < sizeof( $va_rewritten_terms['terms'] ); $vn_j ++ ) {
							$va_terms[]
								        = new Zend_Search_Lucene_Search_Query_MultiTerm( array( $va_rewritten_terms['terms'][ $vn_j ] ),
								array( $va_rewritten_terms['signs'][ $vn_j ] ) );
							$va_signs[] = $va_rewritten_terms['signs'][ $vn_j ] ? true
								: is_null( $va_rewritten_terms['signs'][ $vn_j ] ) ? null : false;
						}
					}
					break;
				case 'Zend_Search_Lucene_Index_Term':
					$va_rewritten_terms = $this->_rewriteTerm( new Zend_Search_Lucene_Search_Query_Term( $o_term ),
						$va_old_signs[ $vn_i ] );
					if ( sizeof( $va_rewritten_terms['terms'] ) == 1 ) {
						$o_mt = new Zend_Search_Lucene_Search_Query_Term( $va_rewritten_terms['terms'][0] );
					} else {
						$o_mt = new Zend_Search_Lucene_Search_Query_MultiTerm( $va_rewritten_terms['terms'],
							$va_rewritten_terms['signs'] );
					}
					$va_terms[] = $o_mt;
					$va_signs[] = sizeof( $va_rewritten_terms['signs'] ) ? array_shift( $va_rewritten_terms['signs'] )
						: true;
					break;
				case 'Zend_Search_Lucene_Search_Query_Wildcard':
					$va_rewritten_terms
						        = $this->_rewriteTerm( new Zend_Search_Lucene_Search_Query_Term( $o_term->getPattern() ),
						$va_old_signs[ $vn_i ] );
					$o_mt       = new Zend_Search_Lucene_Search_Query_MultiTerm( $va_rewritten_terms['terms'],
						$va_rewritten_terms['signs'] );
					$va_terms[] = $o_mt;
	...
	private function _rewriteTerm( $po_term, $pb_sign ) {
		...
		if ( is_array( $va_access_points = $this->getAccessPoints( $this->opn_tablenum ) )
		     && sizeof( $va_access_points )
		) {
			...
			if (
				...
				foreach ( $va_fields as $vs_field ) {
					...
					if ( isset( $va_ap_info['options'] )
					     && ( $va_ap_info['options']['DONT_STEM']
					          || in_array( 'DONT_STEM', $va_ap_info['options'] ) )
					) {
						$vs_term .= '|';
					}
					$va_terms['terms'][]   = new Zend_Search_Lucene_Index_Term( $vs_term,
						$vs_field . ( $vs_rel_types ? "/{$vs_rel_types}" : '' ) );
		...
		if ( is_array( $va_idno_regexs = $this->opo_search_config->get( 'idno_regexes' ) ) ) {
			if ( isset( $va_idno_regexs[ $this->ops_tablename ] )
				...
				foreach ( $va_idno_regexs[ $this->ops_tablename ] as $vs_idno_regex ) {
					if ( ( preg_match( "!{$vs_idno_regex}!", (string) $po_term->getTerm()->text, $va_matches ) )
						...
						return array(
							'terms'   => array(
								new Zend_Search_Lucene_Index_Term( (string) ( ( sizeof( $va_matches ) > 1 )
									? $va_matches[1] : $va_matches[0] ), "{$vs_table_name}.{$vs_idno_fld}" )
							),
							'signs'   => array( $pb_sign ),
							'options' => array()
						);
		...
		if ( in_array( $va_tmp2[1], array( 'preferred_labels', 'nonpreferred_labels' ) ) ) {
			if ( $t_instance = Datamodel::getInstanceByTableName( $va_tmp2[0], true ) ) {
				if ( method_exists( $t_instance, "getLabelTableName" ) ) {
					return array(
						'terms'   => array(
							new Zend_Search_Lucene_Index_Term( $po_term->getTerm()->text,
								$t_instance->getLabelTableName() . '.' . ( ( isset( $va_tmp2[2] ) && $va_tmp2[2] )
									? $va_tmp2[2] : $t_instance->getLabelDisplayField() ) . ( $va_tmp[1] ? '/'
								                                                                           . $va_tmp[1]
									: '' ) )
						),
						'signs'   => array( $pb_sign ),
	...
	private function _rewritePhrase( $po_term, $pb_sign ) {
		...
		if ( sizeof( $va_access_points = $this->getAccessPoints( $this->opn_tablenum ) ) ) {
			...
			if (
				...
				foreach ( $va_fields as $vs_field ) {
					$va_terms['terms'][]   = new Zend_Search_Lucene_Search_Query_Phrase( $va_index_term_strings, null,
						$vs_field );
					$va_terms['signs'][]   = ( $vs_bool == 'AND' ) ? true : null;
					$va_terms['options'][] = is_array( $va_ap_info['options'] ) ? $va_ap_info['options'] : array();
				}
		...
		if ( in_array( $va_tmp2[1], array( 'preferred_labels', 'nonpreferred_labels' ) ) ) {
			if ( $t_instance = Datamodel::getInstanceByTableName( $va_tmp2[0], true ) ) {
				if ( method_exists( $t_instance, "getLabelTableName" ) ) {
					return array(
						'terms'   => array(
							new Zend_Search_Lucene_Search_Query_Phrase( $va_index_term_strings, null,
								$t_instance->getLabelTableName() . '.' . $t_instance->getLabelDisplayField()
								. ( $va_tmp[1] ? '/' . $va_tmp[1] : '' ) )
						),
						'signs'   => array( $pb_sign ),
						'options' => array()
		...
		if ( sizeof( $va_access_points = $this->getAccessPoints( $this->opn_tablenum ) ) ) {
			...
			if ( $va_ap_info = $va_access_points[ $po_range->getField() ] ) {
				...
				foreach ( $va_fields as $vs_field ) {
					...
					$o_range
					                                 = new Zend_Search_Lucene_Search_Query_Range( $po_range->getLowerTerm(),
						$po_range->getUpperTerm(), ( ( $vs_bool == 'OR' ) ? null : true ) );
				...
				if ( is_array( $va_additional_criteria = $va_ap_info['additional_criteria'] ) ) {
					foreach ( $va_additional_criteria as $vs_criterion ) {
						$va_terms[]
							= new Zend_Search_Lucene_Search_Query_Term( new Zend_Search_Lucene_Index_Term( $vs_criterion ) );
					}
				}
	 ...
	 * $pm_value is the value to apply; this is usually text or a number; for the "in" operator this is a
	...
	public function addResultFilter( $ps_field, $ps_operator, $pm_value ) {
		...
		if ( ! in_array( $ps_operator,
			array( '=', '<', '>', '<=', '>=', '-', 'in', 'not in', '<>', 'is', 'is not' ) )
		) {
			return false;
		}
	 ...
	 *
	 * @param array $pa_type_codes_or_ids List of type_id or code values to filter search by. When set, the search will
	 ...
	 *                                    includeSubtypes = include any child types in the restriction. Default is
	 *                                    true.
	 *
	...
	public function getTypeRestrictionList( $pa_options = null ) {
		if ( function_exists( "caGetTypeRestrictionsForUser" ) ) {
			$va_pervasive_types = caGetTypeRestrictionsForUser( $this->ops_tablename,
				$pa_options );    // restrictions set in app.conf or by associated user role
	 ...
	 *
	 * @param array $pa_source_codes_or_ids List of source_id or code values to filter search by. When set, the search
	...
	public static function checkPluginConfiguration() {
		...
		if ( ! file_exists( __CA_LIB_DIR__ . '/Plugins/SearchEngine/' . $ps_plugin_name
	 ...
	 *                      (the maximum number of results to return), 'checkAccess' (only return results that have an
	 *                      access value = to the specified value)
	 *
	 * @return Array - an array of results is returned keys first by primary key id, then by locale_id. The array
	 *               values are associative arrays with two keys: type_id (the type_id of the result; this points to a
	 *               ca_list_items row defining the type of the result item) and label (the row item's label display
	 *               field). You can push the returned results array from caExtractValuesByUserLocale() to get an array
	 *               keyed by primary key id and returning for each id a displayable text label + the type of the found
	 *               result item.
	 *
	 */
	...
	static public function getSearchExpressionForDisplay( $ps_search, $ps_table ) {
		...
		if ( $t_instance = Datamodel::getInstanceByTableName( $ps_table, true ) ) {
			...
			try {
				...
				// Retry search with all non-alphanumeric characters removed
				try {
					$o_parsed_query = $o_query_parser->parse( preg_replace( "![^A-Za-z0-9 ]+!", " ", $ps_search ),
						$vs_char_set );
				} catch ( Exception $e ) {
					$o_parsed_query = $o_query_parser->parse( "", $vs_char_set );
				}
			...
			foreach ( $va_field_list as $vs_field ) {
				...
				if ( $vs_label = $t_instance->getDisplayLabel( $vs_field_proc ) ) {
					$ps_search = str_replace( $vs_field,
						$vs_rel_type ? _t( "%1 [as %2]", $vs_label, $vs_rel_type ) : $vs_label, $ps_search );
				}
			}
		}
