...
//
				if (in_array($va_path_components['field_name'], array('preferred_labels', 'nonpreferred_labels')) && ($t_instance instanceof LabelableBaseModelWithAttributes)) {
					...
					if($va_path_components['previousvalue'] ?? false) {
						$vm_val = $this->_getPreviousValues($va_path_components['field_name'], $t_instance, array_merge($va_val_opts, ['row_id' => $vn_row_id]));
					} else {
...
//				
					...
					if($va_path_components['previousvalue'] ?? false) {
						$vm_val = $this->_getPreviousValues($va_path_components['field_name'], $t_instance, array_merge($va_val_opts, ['returnWithStructure' => $vb_return_with_structure, 'row_id' => $vn_row_id]));
					} else {
	...
	static public function parseFieldPathComponents(string $table, string $ps_path) : array {
		...
		if($modifier == 'previousvalue') {
			array_splice($va_tmp, 1, 1);
			$vb_is_change = true;
			$change_index = 0;
		} elseif(preg_match("!^previousvalue_([\d]+)$!", $modifier, $m)) {
			array_splice($va_tmp, 1, 1);
			$vb_is_change = true;
	...
	protected function _getPreviousValues($bundle, $pt_instance, $options) {
		...
		if(is_null($ci)) {
			if($return_with_structure) {
				foreach($h as $i => $d) {
					$h[$i] = array_reverse($h[$i], true);
				}
			} else {
				$h = array_reverse($h, true);
			}
			$v = $h;
		} else {
			if($return_with_structure) {
			...
			$h = array_reverse($h, true);
			$v = array_slice(array_values($h), $ci + 1, 1);
		}
		return $v;
	}
	# ------------------------------------------------------------------
