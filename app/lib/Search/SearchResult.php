...
class SearchResult extends BaseObject
{
    ...
    static public $s_prefetch_cache = array();
    static public $s_instance_cache = array();
    static public $s_timestamp_cache = array();
    static public $s_rel_prefetch_cache = array();
    static public $s_parsed_field_component_cache = array();
    static public $opa_hierarchy_parent_prefetch_cache = array();
    static public $opa_hierarchy_children_prefetch_cache = array();
    static public $opa_hierarchy_parent_prefetch_cache_index = array();
    static public $opa_hierarchy_children_prefetch_cache_index = array();
    static public $opa_hierarchy_siblings_prefetch_cache = array();
    static public $opa_hierarchy_siblings_prefetch_cache_index = array();
     ...
     */
    static public $s_cache_size_limit = 256;
    ...
    public static function getCacheSizes()
    {
        return [
            ...
            'hierarchy_parent_prefetch_cache_index' => strlen(
                serialize(self::$opa_hierarchy_parent_prefetch_cache_index)
            ),
            'hierarchy_children_prefetch_cache_index' => strlen(
                serialize(self::$opa_hierarchy_children_prefetch_cache_index)
            ),
            'hierarchy_siblings_prefetch_cache' => strlen(serialize(self::$opa_hierarchy_siblings_prefetch_cache)),
            'hierarchy_siblings_prefetch_cache_index' => strlen(
                serialize(self::$opa_hierarchy_siblings_prefetch_cache_index)
            )
        ];
    }
        ...
        foreach (
            ...
            switch ($vs_cache) {
                ...
                case 'hierarchy_children_prefetch_cache_index':
                    if (is_array($va_cache) && is_array($va_cache[$ps_tablename]) && (sizeof(
                                $va_cache[$ps_tablename]
                            ) > SearchResult::$s_cache_size_limit)) {
                        $va_cache[$ps_tablename] = [];
                    }
                    break;
            }
    ...
    public function __construct($po_engine_result = null, $pa_tables = null)
    {
        ...
        $this->opb_use_identifiers_in_urls = (bool)$this->opo_subject_instance->getAppConfig()->get(
            'use_identifiers_in_urls'
        );
        $this->ops_use_alt_identifiers_in_urls = $this->opo_subject_instance->getAppConfig()->get(
            'use_alternate_identifiers_in_urls_for_' . $this->ops_table_name
        );
     ...
     *
     * @return bool True if current hit is the last in the results set, false otherwise
     */
    ...
    public function prefetchLabels($ps_tablename, $pn_start, $pn_num_rows, $pa_options = null)
    {
        ...
        if (!($t_rel_instance = SearchResult::$s_instance_cache[$ps_tablename])) {
            $t_rel_instance = SearchResult::$s_instance_cache[$ps_tablename] = Datamodel::getInstanceByTableName(
                $ps_tablename,
                true
            );
        }
    ...
    public function prefetchHierarchyParents($ps_tablename, $pn_start, $pn_num_rows, $pa_options = null)
    {
        ...
        if ($ps_tablename !== $this->ops_table_name) {
            $va_row_ids = $this->_getRelatedIDsForPrefetch(
                $ps_tablename,
                $pn_start,
                $pn_num_rows,
                SearchResult::$opa_hierarchy_parent_prefetch_cache_index,
                $t_rel_instance,
                $va_row_ids,
                $pa_options
            );
        ...
        if (is_array(
            ...
            $vs_type_sql = " AND (p.type_id IN (?)" . ($t_rel_instance->getFieldInfo(
                    'type_id',
                    'IS_NULL'
        ...
        while (true) {
            if (!sizeof($va_row_ids_in_current_level) || !is_array($va_params) || !is_array($va_params[0]) || !sizeof(
                    $va_params[0]
                )) {
                break;
            }
    ...
    public function prefetchHierarchyChildren($ps_tablename, $pn_start, $pn_num_rows, $pa_options = null)
    {
        ...
        if ($ps_tablename != $this->ops_table_name) {
            $va_row_ids = $this->_getRelatedIDsForPrefetch(
                ...
                $pn_start,
                $pn_num_rows,
                SearchResult::$opa_hierarchy_children_prefetch_cache_index,
                $t_rel_instance,
                $va_row_ids,
                $pa_options
            );
        ...
        if (is_array(
            ...
            $vs_type_sql = " AND (type_id IN (?)" . ($t_rel_instance->getFieldInfo(
                    'type_id',
                    'IS_NULL'
    ...
    public function prefetchHierarchySiblings($ps_tablename, $pn_start, $pn_num_rows, $pa_options = null)
    {
        ...
        if ($ps_tablename != $this->ops_table_name) {
            $va_row_ids = $this->_getRelatedIDsForPrefetch(
                ...
                $pn_start,
                $pn_num_rows,
                SearchResult::$opa_hierarchy_siblings_prefetch_cache_index,
                $t_rel_instance,
                $va_row_ids,
                $pa_options
            );
        ...
        $vs_sql = "
			...
			WHERE
				 t.{$vs_pk} IN (?)" . ($t_rel_instance->hasField(
                'deleted'
    ...
    private function _getRelatedIDsForPrefetch(
        ...
        foreach ($va_row_ids as $vn_row_id) {
            if (is_array(
                $va_related_items = self::$s_rel_prefetch_cache[$this->ops_table_name][$vn_row_id][$ps_tablename][$vs_opt_md5]
            )) {
                $va_base_row_ids[$vn_row_id] = caExtractValuesFromArrayList(
                    $va_related_items,
    ...
    public function prefetch($ps_tablename, $pn_start, $pn_num_rows, $pa_options = null)
    {
        ...
        if (!($t_instance = SearchResult::$s_instance_cache[$this->ops_table_name])) {
            $t_instance = SearchResult::$s_instance_cache[$this->ops_table_name] = Datamodel::getInstanceByTableName(
                $this->ops_table_name,
                true
            );
        }
        ...
        if ($ps_tablename != $this->ops_table_name) {
            ...
            if (method_exists($t_rel_instance, "getTypeFieldName") && ($t_rel_instance->getTypeFieldName(
                ...
                if (method_exists($t_rel_instance, "getSubjectTableInstance")) {
                    ...
                    if (method_exists(
                            $t_label_subj_instance,
                            "getTypeFieldName"
                        ) && ($vs_type_fld_name = $t_label_subj_instance->getTypeFieldName())) {
            ...
            foreach ($va_linking_tables as $vs_right_table) {
                $vs_join_eq = '';
                if (($va_rels = Datamodel::getOneToManyRelations($vs_left_table)) && is_array(
                        $va_rels[$vs_right_table]
                    )) {
                    $va_acc = array();
                    ...
                    if (!($t_link = SearchResult::$s_instance_cache[$va_rel['many_table']])) {
                        $t_link = SearchResult::$s_instance_cache[$va_rel['many_table']] = Datamodel::getInstanceByTableName(
                            $va_rel['many_table'],
                            true
                        );
                    }
                    ...
                    if (($va_rels = Datamodel::getOneToManyRelations($vs_right_table)) && is_array(
                            $va_rels[$vs_left_table]
                        )) {
                        $va_acc = array();
        ...
        $vs_criteria_sql = '';
        if (is_array($this->opa_tables[$ps_tablename]['criteria']) && (sizeof(
                    $this->opa_tables[$ps_tablename]['criteria']
                ) > 0)) {
        ...
        if (isset($pa_options['checkAccess']) && is_array($pa_options['checkAccess']) && sizeof(
            ...
            $vs_criteria_sql .= " AND ({$this->ops_table_name}.access IN (" . join(
                    ",",
                    $pa_options['checkAccess']
                ) . "))";
        }
    ...
    public function prefetchRelated($ps_tablename, $pn_start, $pn_num_rows, $pa_options)
    {
        ...
        $va_rel_items = $this->opo_subject_instance->getRelatedItems(
            $ps_tablename,
            $va_opts
        );        // if there are more than 100,000 then we have a problem
    ...
    public function prefetchChangeLogData($ps_tablename, $pn_start, $pn_num_rows)
    {
        ...
        self::$s_timestamp_cache['created_on'][$ps_tablename] += $o_log->getCreatedOnTimestampsForIDs(
            $ps_tablename,
            $va_row_ids
        );
        ...
        self::$s_timestamp_cache['last_changed'][$ps_tablename] += $o_log->getLastChangeTimestampsForIDs(
            $ps_tablename,
            $va_row_ids
        );
    ...
    {
        if (($id = $this->opo_engine_result->get(
                $this->opo_subject_instance->primaryKey()
    ...
    public function get($ps_field, $pa_options = null)
    {
        ...
        $va_exclude_values = (isset($pa_options['excludeValues']) && $pa_options['excludeValues']) ? is_array(
            $pa_options['excludeValues']
        ) ? $pa_options['excludeValues'] : [$pa_options['excludeValues']] : [];
        $va_exclude_idnos = (isset($pa_options['excludeIdnos']) && $pa_options['excludeIdnos']) ? is_array(
            $pa_options['excludeIdnos']
        ) ? $pa_options['excludeIdnos'] : [$pa_options['excludeIdnos']] : [];
        ...
        $vn_max_levels_from_bottom = caGetOption(
            array('maxLevelsFromBottom', 'maxLevels', 'level_limit', 'hierarchy_limit'),
            $pa_options,
            null
        );
        $vn_remove_first_items = isset($pa_options['removeFirstItems']) ? (int)$pa_options['removeFirstItems'] : 0;

        $va_check_access = isset($pa_options['checkAccess']) ? (is_array(
            $pa_options['checkAccess']
        ...
        $va_path_components = isset(SearchResult::$s_parsed_field_component_cache[$this->ops_table_name . '/' . $ps_field]) ? SearchResult::$s_parsed_field_component_cache[$this->ops_table_name . '/' . $ps_field] : $this->parseFieldPathComponents(
            $ps_field
        );
        ...
        if (($va_path_components['field_name'] !== 'access') && (caGetBundleAccessLevel(
                    $va_path_components['table_name'],
                    $vs_access_chk_key
        ...
        if (!($t_instance = SearchResult::$s_instance_cache[$va_path_components['table_name']])) {
            $t_instance = SearchResult::$s_instance_cache[$va_path_components['table_name']] = Datamodel::getInstanceByTableName(
                $va_path_components['table_name'],
                true
            );
        }
        ...
        if ($va_path_components['hierarchical_modifier']) {
            if ($vb_assume_display_field && in_array(
            ...
            switch ($va_path_components['hierarchical_modifier']) {
                case 'parent':
                    ...
                    foreach ($va_ids as $vn_id) {
                        ...
                        if (
                            ...
                            if (!is_array(
                                $va_parent_ids = SearchResult::$opa_hierarchy_parent_prefetch_cache[$va_path_components['table_name']][$vn_id][$vs_opt_md5]
                            )) {
                                return $pa_options['returnAsArray'] ? array() : null;
                            }
                        }
                        ...
                        if (!($qr_hier = $t_instance->makeSearchResult(
                            $va_path_components['table_name'],
                            $va_parent_ids,
                            $pa_options
                        ))) {
                ...
                case 'hierarchy':
                    ...
                    $vs_hierarchy_direction = isset($pa_options['hierarchyDirection']) ? strtolower(
                        $pa_options['hierarchyDirection']
                    ) : 'asc';
                    ...
                    if ($t_instance->isHierarchical()) {
                        if ($va_path_components['field_name'] === $vs_hier_pk_fld) {
                            if ($va_path_components['related']) {
                                foreach ($va_ids as $vn_id) {
                                    if (is_array(
                                        ...
                                        $va_hier_id_list = array_merge(
                                            array($vn_id),
                                            SearchResult::$opa_hierarchy_parent_prefetch_cache[$va_path_components['table_name']][$vn_id][$vs_opt_md5]
                                        );
                                        $va_hier_id_list = array_filter(
                                            $va_hier_id_list,
                                            function ($v) {
                                                return $v > 0;
                                            }
                                        ...
                                        if (!is_null($vn_max_levels_from_top) && ($vn_max_levels_from_top > 0)) {
                                            $va_hier_id_list = array_slice(
                                                $va_hier_id_list,
                                                0,
                                                $vn_max_levels_from_top,
                                                true
                                            );
                                        } elseif (!is_null(
                                ...
                                if (($vn_type_id = $this->get(
                                        $va_path_components['table_name'] . ".type_id"
                            ...
                            $vm_val = $vb_return_as_array ? $va_hier_ids : join(
                                $vs_hierarchical_delimiter,
                                $va_hier_ids
                            );
                            goto filter;
                        } else {
                            ...
                            $va_ancestor_id_list = $this->get(
                                $va_path_components['table_name'] . '.hierarchy.' . $vs_hier_pk_fld,
                                array_merge(
                                    $pa_options,
                                    array(
                                        'returnAsArray' => true,
                            ...
                            if (
                                ...
                                $filter_by_types = caMakeTypeIDList(
                                    $va_path_components['table_name'],
                                    $pa_options['filterTypes'],
                                    ['dontIncludeSubtypesInTypeRestriction' => true]
                                );
                            ...
                            foreach ($va_ancestor_id_list as $va_ancestor_ids) {
                                ...
                                if ($qr_hier = caMakeSearchResult(
                                    ...
                                    if (($va_path_components['table_name'] == 'ca_objects') && caGetOption(
                                            'showCollectionObjectHierarchy',
                                            $pa_options,
                                            false
                                        ) && ($config->get('ca_objects_x_collections_hierarchy_enabled'))) {
                                        if (($qr_bridge = caMakeSearchResult(
                                                $va_path_components['table_name'],
                                                [$va_ancestor_ids[0]],
                                                $pa_options
                                            )) && $qr_bridge->nextHit()) {
                ...
                case 'children':
                    ...
                    if ($va_path_components['related']) {
                        ...
                        if (!isset(SearchResult::$opa_hierarchy_children_prefetch_cache_index[$this->ops_table_name][$vn_row_id][$va_path_components['table_name']][$vs_opt_md5])) {
                            $this->prefetchHierarchyChildren(
                                $va_path_components['table_name'],
                                $this->opo_engine_result->currentRow(),
                                $this->getOption('prefetch'),
                                $pa_options
                    ...
                    foreach ($va_ids as $vn_id) {
                        ...
                        $qr_hier = $t_instance->makeSearchResult(
                            $va_path_components['table_name'],
                            SearchResult::$opa_hierarchy_children_prefetch_cache[$va_path_components['table_name']][$vn_id][$vs_opt_md5],
                            $pa_options
                        );
                        ...
                        while ($qr_hier->nextHit()) {
                            $vm_val = $qr_hier->get($vs_hier_fld_name, $pa_options);
                            $va_hier_list[$qr_hier->get(
                                $va_path_components['table_name'] . '.' . $vs_pk
                            )] = $vb_return_as_array ? array_shift($vm_val) : $vm_val;
                        }
                    }

                    if (!$vb_return_as_array) {
                        return $vb_return_as_count ? sizeof($va_hier_list) : join(
                            $vs_hierarchical_delimiter,
                            $va_hier_list
                        );
                    }
                ...
                case 'siblings':
                    ...
                    if ($va_path_components['related']) {
                        ...
                        if (!isset(SearchResult::$opa_hierarchy_siblings_prefetch_cache_index[$this->ops_table_name][$vn_row_id][$va_path_components['table_name']][$vs_opt_md5])) {
                            $this->prefetchHierarchySiblings(
                                $va_path_components['table_name'],
                                $this->opo_engine_result->currentRow(),
                                $this->getOption('prefetch'),
                                $pa_options
...
//
                if ($va_path_components['field_name'] == '_guid') {
                    $vm_val = ca_guids::getForRow(
                        Datamodel::getTableNum($va_path_components['table_name']),
                        $vn_row_id
                    );
...
//
                if (in_array(
                    ...
                    $vm_val = $this->_getLabelValue(
                        self::$s_prefetch_cache[$vs_label_table_name][$vn_row_id][$vs_opt_md5],
                        $t_instance,
                        array_merge(
                            $va_val_opts,
                            [
...
//
                    ...
                    $vm_val = $this->_getIntrinsicValue(
                        self::$s_prefetch_cache[$va_path_components['table_name']][$vn_row_id][$vs_opt_md5],
                        $t_instance,
                        $va_val_opts
                    );
                    goto filter;
                } elseif (method_exists($t_instance, 'isValidBundle') && !$t_instance->hasElement(
                        $va_path_components['field_name'],
                        null,
                        false,
...
//
                    ...
                    $vm_val = $t_instance->renderBundleForDisplay(
                        $va_path_components['field_name'],
                        $vn_row_id,
                        self::$s_prefetch_cache[$va_path_components['table_name']][$vn_row_id][$vs_opt_md5],
                        $va_val_opts
                        ...
                        if ($pa_options['returnAllLocales']) {
                            $vm_val = [
                                $vn_row_id => [
                                    $t_instance->get(
                                        'locale_id'
                                    ) => [$va_path_components['field_name'] => $vm_val]
                                ]
                            ];
                        } else {
...
//
                    ...
                    if (($t_instance instanceof BaseModelWithAttributes) && isset($va_path_components['field_name']) && $va_path_components['field_name'] && $t_element = ca_metadata_elements::getInstance(
                            $va_path_components['field_name']
                        )) {
                    ...
                    if (!isset(ca_attributes::$s_get_attributes_cache[(int)$this->opn_table_num . '/' . (int)$vn_row_id][(int)$vn_element_id])) {
                        ...
                        if (is_array($va_prefetch_attributes = $this->getOption('prefetchAttributes')) && sizeof(
                                $va_prefetch_attributes
                            )) {
                            $va_element_ids = array_unique($va_element_ids + $va_prefetch_attributes);
                        }
                        ca_attributes::prefetchAttributes(
                            $this->opo_subject_instance->getDb(),
                            $this->opn_table_num,
                            $this->getRowIDsToPrefetch(
                                $this->opo_engine_result->currentRow(),
                                $this->getOption('prefetch')
                            ),
                            $va_element_ids,
                    ...
                    $va_attributes = ca_attributes::getAttributes(
                        $this->opo_subject_instance->getDb(),
                        $this->opn_table_num,
                        $vn_row_id,
                        array($vn_element_id),
        ...
        if ($vb_return_as_array && sizeof($va_filters)) {
            ...
            if (!($t_instance = SearchResult::$s_instance_cache[$va_tmp[0]])) {
                $t_instance = SearchResult::$s_instance_cache[$va_tmp[0]] = Datamodel::getInstanceByTableName(
                    $va_tmp[0],
                    true
                );
            }
            ...
            if ($t_instance) {
                ...
                foreach ($va_filters as $vs_filter => $va_filter_vals) {
                    ...
                    $va_filter_values = $this->get(
                        join(".", $va_tmp) . ".{$vs_filter}",
                        array(
                            'returnAsArray' => true,
                            'alwaysReturnItemID' => true,
                            'sort' => caGetOption('sort', $pa_options, null),
                            'sortDirection' => caGetOption(
                                'sortDirection',
                                $pa_options,
                                null
                            )
                    ...
                    if (is_array($va_filter_values)) {
                        foreach ($va_filter_values as $vn_id => $vm_filtered_val) {
                            if ((!isset($va_keepers[$vn_id]) || $va_keepers[$vn_id]) && in_array(
                                    $vm_filtered_val,
                                    $va_filter_vals
    ...
    private function _getRelatedValue($pa_value_list, $pa_options = null)
    {
        ...
        if (!($t_rel_instance = SearchResult::$s_instance_cache[$va_path_components['table_name']])) {
            $t_rel_instance = SearchResult::$s_instance_cache[$va_path_components['table_name']] = Datamodel::getInstanceByTableName(
                $va_path_components['table_name'],
                true
            );
        }
        ...
        while ($qr_rel->nextHit()) {
            ...
            if (is_array($pa_check_access) && sizeof($pa_check_access) && $t_rel_instance->hasField(
                    'access'
            ...
            if (($vs_rel_table_name == 'ca_list_items') && is_array($pa_restrict_to_lists) && sizeof(
                    $pa_restrict_to_lists
        ...
        if ($vb_return_as_link) {
            $va_return_values = caCreateLinksFromText(
                $va_return_values,
    ...
    private function _getLabelValue($pa_value_list, $pt_instance, $pa_options)
    {
        ...
        if (
            ...
            &&
            ($vs_label_type_list_code = $pt_instance->getAppConfig()->get(
                ($pt_instance->tableName(
                    ) . '_' . (($va_path_components['field_name'] == 'nonpreferred_labels') ? 'nonpreferred_label_type_list' : 'preferred_label_type_list'))
            ))
        ) {
        ...
        if (is_array($pa_value_list)) {
            foreach ($pa_value_list as $vn_locale_id => $va_labels_by_locale) {
                if ($pa_options['useLocaleCodes']) {
                    if (!$vn_locale_id || !($vm_locale_id = SearchResult::$opo_locales->localeIDToCode(
                            $vn_locale_id
                        ))) {
                ...
                foreach ($va_labels_by_locale as $vn_id => $va_label) {
                    if (is_array($va_restrict_to_type_ids) && sizeof($va_restrict_to_type_ids) && !in_array(
                            $va_label['type_id'],
                            $va_restrict_to_type_ids
                        )) {
                        continue;
                    }
                    if (is_array($va_exclude_type_ids) && sizeof($va_exclude_type_ids) && in_array(
                            $va_label['type_id'],
                            $va_exclude_type_ids
                        )) {
                        continue;
                    ...
                    switch ($pa_options['output']) {
                        case 'text':
                            $vs_val_proc = $this->_convertCodeToDisplayText(
                                $vs_val_proc,
                                $va_path_components,
                                $pt_instance->getLabelTableInstance(),
                                $pa_options
                            );
                            break;
                        case 'idno':
                            $vs_val_proc = $this->_convertCodeToIdno(
                                $vs_val_proc,
                                $va_path_components,
                                $pt_instance->getLabelTableInstance(),
                                $pa_options
                    ...
                    if ($pa_options['makeLink']) {
                        $vs_val_proc = array_shift(
                            caCreateLinksFromText(array($vs_val_proc), $vs_table_name, array($vn_id))
                        );
                    }

                    if ($pa_options['returnWithStructure']) {
                        if (!$va_path_components['subfield_name'] && isset($va_label['type_id'])) {
                            $va_mod_path_components = array_merge(
                                $va_path_components,
                            ...
                            switch ($pa_options['output']) {
                                case 'text':
                                    $va_label['type_id'] = $this->_convertCodeToDisplayText(
                                        $va_label['type_id'],
                                        $va_mod_path_components,
                                        $pt_instance->getLabelTableInstance(),
                                        $pa_options
                                    );
                                    break;
                                case 'idno':
                                    $va_label['type_id'] = $this->_convertCodeToIdno(
                                        $va_label['type_id'],
                                        $va_mod_path_components,
                                        $pt_instance->getLabelTableInstance(),
                                        $pa_options
    ...
    private function _getAttributeValue($pa_value_list, $pt_instance, $pa_options)
    {
        ...
        if (is_array($pa_value_list) && sizeof($pa_value_list)) {
            ...
            foreach ($pa_value_list as $o_attribute) {
                ...
                if ($pa_options['useLocaleCodes']) {
                    if (!$o_attribute->getLocaleID() || !($vm_locale_id = SearchResult::$opo_locales->localeIDToCode(
                            $o_attribute->getLocaleID()
                        ))) {
                        $vm_locale_id = __CA_DEFAULT_LOCALE__;
                    }
                } else {
                ...
                $vb_return_value_id = ($va_path_components['components'][sizeof(
                        $va_path_components['components']
                    ) - 1] === 'value_id');
                ...
                foreach ($va_values as $o_value) {
                    ...
                    if (is_a($o_value, "AuthorityAttributeValue")) {
                        ...
                        $vb_element_is_present = $pt_instance->hasElement(
                            $va_path_components['field_name'],
                            null,
                            true,
                            array('dontCache' => false)
                        );
                        $vb_sub_element_is_present = $pt_instance->hasElement(
                            $va_path_components['subfield_name'],
                            null,
                            true,
                            array('dontCache' => false)
                        ...
                        if (SearchResult::_isHierarchyModifier(
                                $va_path_components['field_name']
                    ...
                    if ($va_path_components['subfield_name'] && ($va_path_components['subfield_name'] !== $vs_element_code) && !SearchResult::_isHierarchyModifier(
                            $va_path_components['subfield_name']
                        ) && !($o_value instanceof InformationServiceAttributeValue) && !($o_value instanceof LCSHAttributeValue) && !($o_value instanceof MediaAttributeValue) && !($o_value instanceof FileAttributeValue) && !is_a(
                            $o_value,
                            "AuthorityAttributeValue"
                        )) {
                    ...
                    if (is_a($o_value, "AuthorityAttributeValue")) {
                        ...
                        if ($qr_res = caMakeSearchResult($vs_auth_table_name, array($o_value->getID()))) {
                            ...
                            if ($qr_res->nextHit()) {
                                if (($t_instance = $o_value->elementTypeToInstance(
                                        $o_value->getType()
                                    )) && ($vs_idno_fld = $t_instance->getProperty('ID_NUMBERING_ID_FIELD'))) {
                                    if (in_array(
                                        $qr_res->get($vs_auth_table_name . '.' . $vs_idno_fld),
                                        $pa_exclude_idnos
                                    )) {
                                        continue;
                                    }
                                ...
                                if (is_array($va_val_proc)) {
                                    foreach ($va_val_proc as $vn_i => $vs_v) {
                                        ...
                                        if ($pa_options['returnWithStructure']) {
                                            ...
                                            ) . (($vn_i > 0) ? "_{$vn_i}" : '')][$vs_element_code] = ($vb_has_field_spec || $vb_has_hierarchy_modifier) ? $vs_v : $o_value->getDisplayValue(
                                                array_merge(
                                                    $pa_options,
                                            ...
                                            if ($include_value_ids) {
                                                $va_return_values[(int)$vn_id][$vm_locale_id][(int)$o_attribute->getAttributeID(
                                                ) . (($vn_i > 0) ? "_{$vn_i}" : '')]["{$vs_element_code}_value_id"] = $o_value->getValueID(
                                                );
                                            }
                                        } else {
                                            $va_return_values[(int)$vn_id][$vm_locale_id][(int)$o_attribute->getAttributeID(
                                            )][] = ($vb_has_field_spec || $vb_has_hierarchy_modifier) ? $vs_v : $o_value->getDisplayValue(
                                                array_merge(
                                                    $pa_options,
                                                    array('output' => $pa_options['output'], 'list_id' => $vn_list_id)
                                                )
                    ...
                    if (is_null($vs_val_proc)) {
                        switch ($o_value->getType()) {
                            ...
                            case __CA_ATTRIBUTE_VALUE_FILE__:
                                $vs_return_type = 'tag';
                                $va_versions = ($o_value->getType(
                                    ) == __CA_ATTRIBUTE_VALUE_MEDIA__) ? $o_value->getVersions() : [];
                                ...
                                if ($alt_text = caGetOption('alt', $pa_options, null)) {
                                    // noop
                                } elseif ($alt_text_template = Configuration::load()->get(
                                    $this->tableName() . "_alt_text_template"
                                )) {
                                ...
                                $vs_val_proc = $o_value->getDisplayValue(
                                    ['alt' => $alt_text, 'return' => $vs_return_type, 'version' => $vs_version]
                                );
                                break;
                            case __CA_ATTRIBUTE_VALUE_LIST__:
                                ...
                                $vn_list_id = $t_element->get('list_id');
                                if (in_array(
                                    $o_value->getDisplayValue(array('output' => 'idno', 'list_id' => $vn_list_id)),
                                    $pa_exclude_idnos
                                )) {
                                    continue(2);
                                }
                            ...
                            case __CA_ATTRIBUTE_VALUE_LCSH__:
                                switch ($va_path_components['subfield_name']) {
                                    ...
                                    default:
                                        $vs_val_proc = $o_value->getDisplayValue(
                                            array_merge($pa_options, array('output' => $pa_options['output']))
                                        );
                                        break;
                                }
                                break;
                                ...
                                if ($va_path_components['subfield_name'] && ($vs_element_code != $va_path_components['subfield_name']) && ($vs_element_code == $va_path_components['field_name'])) {
                                    ...
                                    if (isset($pa_options['coordinates']) && $pa_options['coordinates']) {
                                        if (preg_match("!\[([^\]]+)!", $vs_val_proc, $va_matches)) {
                                            ...
                                            if ((sizeof($va_tmp) == 2) && (is_numeric($va_tmp[0])) && (is_numeric(
                                                    $va_tmp[1]
                                                ))) {
                                                $vs_val_proc = array(
                    ...
                    array_pop($va_spec);
                    $va_acc[join('.', $va_spec) . '.' . $vs_element_code] = $o_value->getDisplayValue(
                        array_merge($pa_options, array('output' => 'idno'))
                    );
                    ...
                    if (!$vb_dont_return_value) {
                        ...
                        if ($pa_options['returnWithStructure']) {
                            $va_return_values[(int)$vn_id][$vm_locale_id][(int)$o_attribute->getAttributeID(
                            )][$vs_element_code] = $vs_val_proc;
                            if ($o_value->getType() == __CA_ATTRIBUTE_VALUE_DATERANGE__ && !caGetOption(
                                    'omitDateSortKey',
                                    $pa_options,
                                    false
                                )) {  // add sortable alternate value for dates
                                $va_return_values[(int)$vn_id][$vm_locale_id][(int)$o_attribute->getAttributeID(
                                )][$vs_element_code . '_sort_'] = $o_value->getDisplayValue(
                                    ['sortable' => true]
                                ); // add sortable alternate representation for dates; this will be used by the SearchResult::get() "sort" option to properly sort date values
                            }
                            if ($include_value_ids) {
                                $va_return_values[(int)$vn_id][$vm_locale_id][(int)$o_attribute->getAttributeID(
                                )]["{$vs_element_code}_value_id"] = $o_value->getValueID();
                            }
                        } else {
                            $va_return_values[(int)$vn_id][$vm_locale_id][(int)$o_attribute->getAttributeID(
                            )][] = $vs_val_proc;
                ...
                if ($va_path_components['subfield_name'] && $pa_options['returnBlankValues'] && !$vb_did_return_value) {
                    ...
                    if ($pa_options['returnWithStructure']) {
                        ...
                        if ($include_value_ids) {
                            $va_return_values[(int)$vn_id][$vm_locale_id][(int)$o_attribute->getAttributeID(
                            )]["{$vs_element_code}_value_id"] = null;
                        }
                    } else {
                        $va_return_values[(int)$vn_id][$vm_locale_id][(int)$o_attribute->getAttributeID()][] = '';
                    }
            ...
            // is blank
            $default_value = ca_metadata_elements::getElementDefaultValue(
                $va_path_components['subfield_name'] ? $va_path_components['subfield_name'] : $va_path_components['field_name']
            );
        ...
        // Flatten array for return as string or simple array value
        //
        $va_flattened_values = array_map(
            function ($v) use ($pa_options) {
                    ...
                    foreach ($va_values as $vn_i => $va_value) {
                        ...
                        if (in_array(
                            ...
                            if ($va_field_info['FIELD_TYPE'] == FT_DATERANGE) {
                                $this->opo_tep->setUnixTimestamps(
                                    $va_value[$va_field_info['START']],
                                    $va_value[$va_field_info['END']]
                                );
                            } else {
                                $this->opo_tep->setHistoricTimestamps(
                                    $va_value[$va_field_info['START']],
                                    $va_value[$va_field_info['END']]
                                );
                            }
                        ...
                        if ($vb_return_as_link) {
                            $vs_prop = array_shift(
                                caCreateLinksFromText(array($vs_prop), $vs_table_name, array($vn_id))
                            );
                        }
            ...
            case FT_MEDIA:
                ...
                foreach ($pa_value_list as $vn_locale_id => $va_values) {
                    ...
                    foreach ($va_values as $vn_i => $va_value) {
                        ...
                        $o_media_settings = new MediaProcessingSettings(
                            $va_path_components['table_name'],
                            $va_path_components['field_name']
                        );
                        ...
                        if ($pa_options['unserialize']) {
                            $va_return_values[$vn_id][$vm_locale_id] = caUnserializeForDatabase(
                                $va_value[$va_path_components['field_name']]
                            );
                        } elseif ($vs_info_element && (!in_array($vs_info_element, ['url', 'path', 'tag']))) {
                            if (in_array(
                                strtolower($vs_info_element),
                                ['original_filename', 'originalfilename', 'filename']
                            )) {
                                $media_info = $this->getMediaInfo(
                                    $va_path_components['table_name'] . '.' . $va_path_components['field_name'],
                                    null,
                                    null,
                                    $pa_options
                                );
                                $va_return_values[$vn_id][$vm_locale_id] = caGetOption(
                                    'ORIGINAL_FILENAME',
                                    $media_info,
                                    pathinfo(
                                        $this->getMediaPath(
                                ...
                                $e = strtolower($vs_info_element),
                                ['mimetype', 'filemodificationtime']
                            )) {
                                $media_info = $this->getMediaInfo(
                                    $va_path_components['table_name'] . '.' . $va_path_components['field_name'],
                                    $va_path_components['subfield_name'] ? $va_path_components['subfield_name'] : 'original',
                                    null,
                                    $pa_options
                                );
                                switch ($e) {
                                ...
                                $va_return_values[$vn_id][$vm_locale_id] = $this->getMediaInfo(
                                    $va_path_components['table_name'] . '.' . $va_path_components['field_name'],
                                    $vs_version,
                                    $vs_info_element,
                                    $pa_options
                            ...
                            $va_return_values[$vn_id][$vm_locale_id] = $this->getMediaUrl(
                                $va_path_components['table_name'] . '.' . $va_path_components['field_name'],
                                $vs_version,
                                $pa_options
                            );
                        } elseif ((isset($pa_options['returnPath']) && ($pa_options['returnPath'])) || ($vs_info_element == 'path')) {
                            $va_return_values[$vn_id][$vm_locale_id] = $this->getMediaPath(
                                $va_path_components['table_name'] . '.' . $va_path_components['field_name'],
                                $vs_version,
                                $pa_options
                            );
                        } else {
                            $va_return_values[$vn_id][$vm_locale_id] = $this->getMediaTag(
                                $va_path_components['table_name'] . '.' . $va_path_components['field_name'],
                                $vs_version,
                                $pa_options
                            );
            ...
            case FT_VARS:
                foreach ($pa_value_list as $vn_locale_id => $va_values) {
                    foreach ($va_values as $vn_i => $va_value) {
                        ...
                        if ((isset($d['EXIF']) || isset($d['XMP']) || isset($d['IFD0'])) && (in_array(
                                strtolower($va_path_components['subfield_name']),
                                [
                                    'title',
                                    'description',
                                    'creator',
                ...
                foreach ($pa_value_list as $vn_locale_id => $va_values) {
                    ...
                    foreach ($va_values as $vn_i => $va_value) {
                        ...
                        if ($pa_options['convertCodesToDisplayText']) {
                            $vs_prop = $this->_convertCodeToDisplayText(
                                $vs_prop,
                                $va_path_components,
                                $pt_instance,
                                $pa_options
                            ...
                            $vs_prop = $this->_convertCodeToIdno(
                                $vs_prop,
                                $va_path_components,
                                $pt_instance,
                                $pa_options
                            ...
                            $vs_prop = $this->_convertCodeToValue(
                                $vs_prop,
                                $va_path_components,
                                $pt_instance,
                                $pa_options
        ...
        if ($pa_options['returnAllLocales']) {
            ...
            foreach ($pa_array as $va_by_attr) {
                ...
                foreach ($va_by_attr as $vs_val) {
                    ...
                    $vn_start = (strlen($pa_options['start']) && is_numeric(
                            $pa_options['start']
                        )) ? (int)$pa_options['start'] : 0;
                    $vn_length = (strlen(
                            $pa_options['length']
                        ) && ($pa_options['length'] > 0)) ? (int)$pa_options['length'] : null;
    ...
    public function getWithTemplate($ps_template, $pa_options = null)
    {
        ...
        return caProcessTemplateForIDs(
            $ps_template,
            $this->ops_table_name,
        ...
        if (!isset(
            $this->opa_template_prefetch_cache[$vs_cache_base_key][$vn_cur_row = $this->opo_engine_result->currentRow()]
        )) {
    ...
    private function _convertCodeToIdno($ps_prop, $pa_path_components, $pt_instance, $pa_options = null)
    {
        ...
        if (method_exists($pt_instance, 'setLabelTypeList')) {
            $pt_instance->setLabelTypeList(
                $this->opo_subject_instance->getAppConfig()->get(
                    ($pa_path_components['field_name'] == 'nonpreferred_labels') ? "{$vs_table_name}_nonpreferred_label_type_list" : "{$vs_table_name}_preferred_label_type_list"
                )
            );
        }
        if (isset($pa_options['convertCodesToIdno']) && $pa_options['convertCodesToIdno'] && ($vs_list_code = $pt_instance->getFieldInfo(
                $vs_field_name,
                "LIST_CODE"
            ))) {
            $vs_prop = caGetListItemIdno($vs_prop);
        } elseif (isset($pa_options['convertCodesToIdno']) && $pa_options['convertCodesToIdno'] && ($vs_list_code = $pt_instance->getFieldInfo(
                $vs_field_name,
                "LIST"
            ))) {
            $vs_prop = caGetListItemIdno(caGetListItemIDForValue($vs_list_code, $vs_prop));
        } elseif ($vb_convert_codes_to_display_text && method_exists(
                $pt_instance,
                "isRelationship"
            ) && $pt_instance->isRelationship() && ($vs_field_name == 'type_id')) {
    ...
    private function _convertCodeToValue($ps_prop, $pa_path_components, $pt_instance, $pa_options = null)
    {
        ...
        if (isset($pa_options['convertCodesToValue']) && $pa_options['convertCodesToValue'] && ($vs_list_code = $pt_instance->getFieldInfo(
                $vs_field_name,
                "LIST_CODE"
            ))) {
            $vs_prop = caGetListItemValueForID($vs_prop);
        } elseif (isset($pa_options['convertCodesToValue']) && $pa_options['convertCodesToValue'] && ($vs_list_code = $pt_instance->getFieldInfo(
                $vs_field_name,
                "LIST"
            ))) {
            return $vs_prop;
    ...
    private function _convertCodeToDisplayText($ps_prop, $pa_path_components, $pt_instance, $pa_options = null)
    {
        ...
        if (method_exists(
                $pt_instance,
                'setLabelTypeList'
            ) && ($vs_label_list_name = $this->opo_subject_instance->getAppConfig()->get(
                ($pa_path_components['field_name'] == 'nonpreferred_labels') ? "{$vs_table_name}_nonpreferred_label_type_list" : "{$vs_table_name}_preferred_label_type_list"
            ))) {
            $pt_instance->setLabelTypeList($vs_label_list_name);
        }

        if ($vb_convert_codes_to_display_text && ($vs_list_code = $pt_instance->getFieldInfo(
                $vs_field_name,
                "LIST_CODE"
            ))) {
            $vs_prop = SearchResult::$opt_list->getItemFromListForDisplayByItemID($vs_list_code, $vs_prop);
        } elseif ($vb_convert_codes_to_display_text && ($vs_list_code = $pt_instance->getFieldInfo(
                $vs_field_name,
                "LIST"
            ))) {
            $vs_prop = SearchResult::$opt_list->getItemFromListForDisplayByItemValue($vs_list_code, $vs_prop);
        } elseif ($vb_convert_codes_to_display_text && is_array(
                $options = $pt_instance->getFieldInfo($vs_field_name, "OPTIONS")
            ...
            $vs_prop = $t_locale->getName();
                ...
                $va_list = $pt_instance->getFieldInfo($vs_field_name, "BOUNDS_CHOICE_LIST")
            ))) {
    ...
    public function hasData($pa_fields, $pa_options = null)
    {
        ...
        foreach ($pa_fields as $vn_i => $vs_field) {
            ...
            if (!$t_instance->hasField($va_tmp[1]) && (!$t_instance->hasElement(
                    $va_tmp[1],
                    null,
                    false,
                    array('dontCache' => false)
    ...
    public function getMediaInfo($ps_field, $ps_version = null, $ps_key = null, $pa_options = null)
    {
        ...
        if (!$this->opa_field_media_info[$ps_field]) {
            $this->opa_field_media_info[$ps_field] = $this->get(
                $ps_field,
                array(
                    "unserialize" => true,
                    'returnWithStructure' => true
        ...
        return $GLOBALS["_DbResult_mediainfocoder"]->getMediaInfo(
            $ps_version,
            $ps_key,
            array_merge(
                $pa_options,
                [
                    'data' => array_shift(
                        $this->opa_field_media_info[$ps_field]
                    )
                ]
            )
        );
    ...
    public function getMediaPath($ps_field, $ps_version, $pa_options = null)
    {
        ...
        return $GLOBALS["_DbResult_mediainfocoder"]->getMediaPath(
            $ps_version,
            array_merge(
                $pa_options,
                [
    ...
    public function getMediaPaths($ps_field, $ps_version, $pa_options = null)
    {
        ...
        if (is_array($this->opa_field_media_info[$ps_field]) && sizeof($this->opa_field_media_info[$ps_field])) {
            foreach ($this->opa_field_media_info[$ps_field] as $vm_media) {
                $va_media_paths[] = $GLOBALS["_DbResult_mediainfocoder"]->getMediaPath(
                    $ps_version,
                    array_merge(
                        $pa_options,
                        ['data' => $vm_media]
    ...
    public function getMediaUrl($ps_field, $ps_version, $pa_options = null)
    {
        ...
        return $GLOBALS["_DbResult_mediainfocoder"]->getMediaUrl(
            $ps_version,
            array_merge(
                $pa_options,
                [
    ...
    public function getMediaUrls($ps_field, $ps_version, $pa_options = null)
    {
        ...
        if (is_array($this->opa_field_media_info[$ps_field]) && sizeof($this->opa_field_media_info[$ps_field])) {
            foreach ($this->opa_field_media_info[$ps_field] as $vm_media) {
                $va_media_urls[] = $GLOBALS["_DbResult_mediainfocoder"]->getMediaUrl(
                    $ps_version,
                    array_merge(
                        $pa_options,
                        ['data' => $vm_media]
    ...
    public function getMediaTag($ps_field, $ps_version, $pa_options = null)
    {
        ...
        return $GLOBALS["_DbResult_mediainfocoder"]->getMediaTag(
            $ps_version,
            array_merge(
                $pa_options,
                [
    ...
    public function getMediaTags($ps_field, $ps_version, $pa_options = null)
    {
        ...
        if (is_array($this->opa_field_media_info[$ps_field]) && sizeof($this->opa_field_media_info[$ps_field])) {
            foreach ($this->opa_field_media_info[$ps_field] as $vm_media) {
                $va_media_tags[] = $GLOBALS["_DbResult_mediainfocoder"]->getMediaTag(
                    $ps_version,
                    array_merge(
                        $pa_options,
                        ['data' => $vm_media]
    ...
    public function getMediaInfos($ps_field)
    {
        ...
        if (is_array($this->opa_field_media_info[$ps_field]) && sizeof($this->opa_field_media_info[$ps_field])) {
            foreach ($this->opa_field_media_info[$ps_field] as $vm_media) {
                $va_media_infos[] = $GLOBALS["_DbResult_mediainfocoder"]->getMediaInfo(
                    null,
                    null,
                    ['data' => $vm_media]
                );
    ...
    public function getMediaVersions($ps_field)
    {
        ...
        return $GLOBALS["_DbResult_mediainfocoder"]->getMediaVersions(
    ...
    public function hasMedia($ps_field)
    {
        ...
        return $GLOBALS["_DbResult_mediainfocoder"]->hasMedia(
            ['data' => reset($this->opa_field_media_info[$ps_field])]
        );
    }
    # ------------------------------------------------------------------
    ...
    public function getMediaScale($ps_field)
    {
        ...
        return $GLOBALS["_DbResult_mediainfocoder"]->getMediaScale(
            ['data' => reset($this->opa_field_media_info[$ps_field])]
        );
    }
    # ------------------------------------------------------------------
    ...
    public function mediaIsMirrored($ps_field, $ps_version)
    {
        ...
        return $GLOBALS["_DbResult_mediainfocoder"]->mediaIsMirrored(
            $ps_version,
            [
                'data' => reset(
                    $this->opa_field_media_info[$ps_field]
    ...
    public function getMediaMirrorStatus($ps_field, $ps_version, $ps_mirror = null)
    {
        ...
        return $GLOBALS["_DbResult_mediainfocoder"]->getMediaMirrorStatus(
            $ps_version,
            $ps_mirror,
            [
                'data' => reset(
    ...
    public function getFileInfo($ps_field)
    {
        ...
        return $GLOBALS["_DbResult_fileinfocoder"]->getFileInfo(
            array_shift($this->get($va_field["field"], array("unserialize" => true, 'returnWithStructure' => true)))
        );
    }
    # ------------------------------------------------------------------
    ...
    {
        $va_field = $this->getFieldInfo($ps_field);
        return $GLOBALS["_DbResult_fileinfocoder"]->getFilePath(
            array_shift($this->get($va_field["field"], array("unserialize" => true, 'returnWithStructure' => true)))
        );
    }
    # ------------------------------------------------------------------
    ...
    {
        $va_field = $this->getFieldInfo($ps_field);
        return $GLOBALS["_DbResult_fileinfocoder"]->getFileUrl(
            array_shift($this->get($va_field["field"], array("unserialize" => true, 'returnWithStructure' => true)))
        );
    }
    # ------------------------------------------------------------------
    ...
    {
        $va_field = $this->getFieldInfo($ps_field);
        return $GLOBALS["_DbResult_fileinfocoder"]->hasFile(
            array_shift($this->get($va_field["field"], array("unserialize" => true, 'returnWithStructure' => true)))
        );
    }
    # ------------------------------------------------------------------
    ...
    {
        $va_field = $this->getFieldInfo($ps_field);
        return $GLOBALS["_DbResult_fileinfocoder"]->getFileConversions(
            array($this->get($va_field["field"], array("unserialize" => true, 'returnWithStructure' => true)))
        );
    }
    # ------------------------------------------------------------------
    ...
    {
        $va_field = $this->getFieldInfo($ps_field);
        return $GLOBALS["_DbResult_fileinfocoder"]->getFileConversionPath(
            array($this->get($va_field["field"], array("unserialize" => true, 'returnWithStructure' => true))),
            $ps_mimetype
        );
    }
    ...
    {
        $va_field = $this->getFieldInfo($ps_field);
        return $GLOBALS["_DbResult_fileinfocoder"]->getFileConversionUrl(
            array_shift($this->get($va_field["field"], array("unserialize" => true, 'returnWithStructure' => true))),
            $ps_mimetype
        );
    }
    ...
    public function getDate($ps_field, $pa_options = null)
    {
        ...
        if (is_object($va_field["instance"])) {
            if (!in_array(
                $vn_field_type = $va_field["instance"]->getFieldInfo($va_field["field"], "FIELD_TYPE"),
                array(
                    FT_DATE,
                    FT_TIME,
                    FT_DATETIME,
                ...
                case (FT_HISTORIC_DATERANGE):
                    ...
                    if (!$pa_options["getRawDate"]) {
                        ...
                        if ($vn_field_type == FT_HISTORIC_DATERANGE) {
                            $GLOBALS["_DbResult_time_expression_parser"]->setHistoricTimestamps(
                                $this->get($vs_start_field_name, array("binary" => true)),
                                $this->get($vs_end_field_name, array("binary" => true))
                            );
                        } else {
                            $GLOBALS["_DbResult_time_expression_parser"]->setUnixTimestamps(
                                $this->get($vs_start_field_name, array("binary" => true)),
                                $this->get($vs_end_field_name, array("binary" => true))
                            );
                        }
    ...
    private function parseFieldPathComponents($ps_path)
    {
        ...
        if (($t_instance = Datamodel::getInstanceByTableName($vs_table_name, true)) && (is_a(
                $t_instance,
                "BaseLabel"
            ))) {
