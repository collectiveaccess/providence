...
class SearchResult extends BaseObject {
		...
		foreach (
			...
			switch ( $vs_cache ) {
				...
				case 'hierarchy_children_prefetch_cache_index':
					if ( is_array( $va_cache ) && is_array( $va_cache[ $ps_tablename ] )
					     && ( sizeof( $va_cache[ $ps_tablename ] ) > SearchResult::$s_cache_size_limit )
					) {
	...
	public function __construct( $po_engine_result = null, $pa_tables = null ) {
		...
		$this->opb_use_identifiers_in_urls     = (bool) $this->opo_subject_instance->getAppConfig()
		                                                                           ->get( 'use_identifiers_in_urls' );
		$this->ops_use_alt_identifiers_in_urls = $this->opo_subject_instance->getAppConfig()
		                                                                    ->get( 'use_alternate_identifiers_in_urls_for_'
		                                                                           . $this->ops_table_name );
	 ...
	 *                                              db = optional Db instance to use for database connectivity. If
	 *                                              omitted a new database connection is used. If you need to have your
	 *                                              result set access the database within a specific transaction you
	 *                                              should pass the Db object used by the transaction here.
	 */
	...
	/**
	 * Controls prefetching for @param bool $pb_disable do prefetching or not?
	 *
	 * @see SearchResult::getWithTemplate()
	 */
	...
	public function prefetchLabels( $ps_tablename, $pn_start, $pn_num_rows, $pa_options = null ) {
		...
		if ( ! ( $t_rel_instance = SearchResult::$s_instance_cache[ $ps_tablename ] ) ) {
			$t_rel_instance
				=
			SearchResult::$s_instance_cache[ $ps_tablename ] = Datamodel::getInstanceByTableName( $ps_tablename, true );
		}
	...
	public function prefetchHierarchyParents( $ps_tablename, $pn_start, $pn_num_rows, $pa_options = null ) {
		...
		if ( $ps_tablename !== $this->ops_table_name ) {
			$va_row_ids = $this->_getRelatedIDsForPrefetch( $ps_tablename, $pn_start, $pn_num_rows,
				SearchResult::$opa_hierarchy_parent_prefetch_cache_index, $t_rel_instance, $va_row_ids, $pa_options );
		}
		...
		$vs_type_sql = '';
		if ( is_array(
			     $va_type_ids = caMakeTypeIDList( $ps_tablename, caGetOption( 'restrictToTypes', $pa_options, null ) ) )
		     && sizeof( $va_type_ids )
		) {
			$vs_type_sql = " AND (p.type_id IN (?)" . ( $t_rel_instance->getFieldInfo( 'type_id', 'IS_NULL' )
					? " OR (p.type_id IS NULL)" : '' ) . ')';
		...
		while ( true ) {
			if ( ! sizeof( $va_row_ids_in_current_level ) || ! is_array( $va_params ) || ! is_array( $va_params[0] )
			     || ! sizeof( $va_params[0] )
			) {
				break;
			}
			...
			while ( $qr_rel->nextRow() ) {
				...
				$va_row_id_map[ $va_row[ $vs_pk ] ]
					= $va_row[ $vs_parent_id_fld ];        // list of ids indexed by parent_id
			}
	...
	public function prefetchHierarchyChildren( $ps_tablename, $pn_start, $pn_num_rows, $pa_options = null ) {
		...
		if ( $ps_tablename != $this->ops_table_name ) {
			$va_row_ids = $this->_getRelatedIDsForPrefetch( $ps_tablename, $pn_start, $pn_num_rows,
				SearchResult::$opa_hierarchy_children_prefetch_cache_index, $t_rel_instance, $va_row_ids, $pa_options );
		}
		...
		if ( is_array(
			...
			$vs_type_sql      = " AND (type_id IN (?)" . ( $t_rel_instance->getFieldInfo( 'type_id', 'IS_NULL' )
		...
		if ( isset( $pa_options['checkAccess'] ) && is_array( $pa_options['checkAccess'] )
		     ...
		     && $t_rel_instance->hasField( 'access' )
		) {
		...
		while ( true ) {
			if ( ! is_array( $va_params ) || ! sizeof( $va_params ) || ! is_array( $va_params[0] )
			     || ! sizeof( $va_params[0] )
			) {
				break;
			}
			...
			if ( ( ! isset( $pa_options['allDescendants'] ) || ! $pa_options['allDescendants'] )
			     && ( $vn_level > 0 )
			) {
				break;
			}
		...
		foreach ( $va_row_ids as $vn_row_id ) {
			if ( ! isset( SearchResult::$opa_hierarchy_children_prefetch_cache[ $ps_tablename ][ $vn_row_id ][ $vs_opt_md5 ] ) ) {
				SearchResult::$opa_hierarchy_children_prefetch_cache[ $ps_tablename ][ $vn_row_id ][ $vs_opt_md5 ]
					= array();
			}
		}
	...
	public function prefetchHierarchySiblings( $ps_tablename, $pn_start, $pn_num_rows, $pa_options = null ) {
		...
		if ( $ps_tablename != $this->ops_table_name ) {
			$va_row_ids = $this->_getRelatedIDsForPrefetch( $ps_tablename, $pn_start, $pn_num_rows,
				SearchResult::$opa_hierarchy_siblings_prefetch_cache_index, $t_rel_instance, $va_row_ids, $pa_options );
		}
		...
		$vs_sql           = "
			...
			WHERE
				 t.{$vs_pk} IN (?)" . ( $t_rel_instance->hasField( 'deleted' )
		...
		while ( $qr_rel->nextRow() ) {
			...
			SearchResult::$opa_hierarchy_siblings_prefetch_cache[ $ps_tablename ][ $va_row[ $vs_pk ] ][ $vs_opt_md5 ][]
				= $va_row['sibling_id'];
		}

		foreach ( $va_row_ids as $vn_row_id ) {
			if ( ! isset( SearchResult::$opa_hierarchy_siblings_prefetch_cache[ $ps_tablename ][ $vn_row_id ][ $vs_opt_md5 ] ) ) {
				SearchResult::$opa_hierarchy_siblings_prefetch_cache[ $ps_tablename ][ $vn_row_id ][ $vs_opt_md5 ]
					= array();
			}
		}
	 ...
	 *
	 */
	private function _getRelatedIDsForPrefetch(
		$ps_tablename, $pn_start, $pn_num_rows, &$pa_cache, $t_rel_instance, $va_row_ids, $pa_options
	) {
		...
		foreach ( $va_row_ids as $vn_row_id ) {
			if ( is_array( $va_related_items
				= self::$s_rel_prefetch_cache[ $this->ops_table_name ][ $vn_row_id ][ $ps_tablename ][ $vs_opt_md5 ] )
			) {
				$va_base_row_ids[ $vn_row_id ] = caExtractValuesFromArrayList( $va_related_items,
					$t_rel_instance->primaryKey() );
				$va_related_ids                += $va_base_row_ids[ $vn_row_id ];
				$pa_cache[ $this->ops_table_name ][ $vn_row_id ][ $ps_tablename ][ $vs_opt_md5 ]
				                               = $va_base_row_ids[ $vn_row_id ];
			} else {
	...
	/**
	 * TODO: implement prefetch of related and non-indexed-stored fields. Basically, instead of doing a query for every
	 * row via get() [which will still be an option if you're lazy] prefetch() will allow you to tell SearchResult to
	 * preload values for a set of hits starting at $pn_start Because this can be done in a single query it'll
	 * presumably be faster than lazy loading lots of rows
	 */
	public function prefetch( $ps_tablename, $pn_start, $pn_num_rows, $pa_options = null ) {
		...
		if ( ! ( $t_instance = SearchResult::$s_instance_cache[ $this->ops_table_name ] ) ) {
			$t_instance
				= SearchResult::$s_instance_cache[ $this->ops_table_name ]
				= Datamodel::getInstanceByTableName( $this->ops_table_name, true );
		}
		if ( ! ( $t_rel_instance = SearchResult::$s_instance_cache[ $ps_tablename ] ) ) {
			$t_rel_instance
				=
		...
		if ( $ps_tablename != $this->ops_table_name ) {
			...
			if ( method_exists( $t_rel_instance, "getTypeFieldName" ) && ( $t_rel_instance->getTypeFieldName() )
			     && ( $vs_type_fld_name = $t_rel_instance->getTypeFieldName() )
			) {
				...
				if ( method_exists( $t_rel_instance, "getSubjectTableInstance" ) ) {
					...
					if ( method_exists( $t_label_subj_instance, "getTypeFieldName" )
					     && ( $vs_type_fld_name
							= $t_label_subj_instance->getTypeFieldName() )
					) {
			...
			foreach ( $va_linking_tables as $vs_right_table ) {
				$vs_join_eq = '';
				if ( ( $va_rels = Datamodel::getOneToManyRelations( $vs_left_table ) )
				     && is_array( $va_rels[ $vs_right_table ] )
				) {
					$va_acc = array();
					foreach ( $va_rels[ $vs_right_table ] as $va_rel ) {
						$va_acc[] = $va_rel['one_table'] . '.' . $va_rel['one_table_field'] . ' = '
						            . $va_rel['many_table'] . '.' . $va_rel['many_table_field'];
					}
					...
					if ( ! ( $t_link = SearchResult::$s_instance_cache[ $va_rel['many_table'] ] ) ) {
						$t_link
							= SearchResult::$s_instance_cache[ $va_rel['many_table'] ]
							= Datamodel::getInstanceByTableName( $va_rel['many_table'], true );
					}
					...
					if ( ( $va_rels = Datamodel::getOneToManyRelations( $vs_right_table ) )
					     && is_array( $va_rels[ $vs_left_table ] )
					) {
						$va_acc = array();
		...
		$vs_criteria_sql = '';
		if ( is_array( $this->opa_tables[ $ps_tablename ]['criteria'] )
		     && ( sizeof( $this->opa_tables[ $ps_tablename ]['criteria'] ) > 0 )
		) {
		...
		while ( $qr_rel->nextRow() ) {
			...
			self::$s_prefetch_cache[ $ps_tablename ][ $vn_row_id ][ $vs_md5 ][ $vn_locale_id ][ $vn_rel_row_id ]
			              = $va_row;
		}
	...
	public function prefetchRelated( $ps_tablename, $pn_start, $pn_num_rows, $pa_options ) {
		...
		$va_criteria = is_array( $this->opa_tables[ $ps_tablename ] ) ? $this->opa_tables[ $ps_tablename ]['criteria']
			: null;
		...
		$va_rel_items = $this->opo_subject_instance->getRelatedItems( $ps_tablename,
			$va_opts );        // if there are more than 100,000 then we have a problem
		...
		foreach ( $va_rel_items as $vs_key => $va_rel_item ) {
			self::$s_rel_prefetch_cache[ $this->ops_table_name ][ (int) $va_rel_item['row_id'] ][ $ps_tablename ][ $vs_md5 ][ $va_rel_item[ $va_rel_item['_key'] ] ]
				= $va_rel_item;
		}
		...
		foreach ( $va_row_ids as $vn_row_id ) {
			if ( ! isset( self::$s_rel_prefetch_cache[ $this->ops_table_name ][ (int) $vn_row_id ][ $ps_tablename ][ $vs_md5 ] ) ) {
				self::$s_rel_prefetch_cache[ $this->ops_table_name ][ (int) $vn_row_id ][ $ps_tablename ][ $vs_md5 ]
					= array();
			}
		}
	...
	public function prefetchChangeLogData( $ps_tablename, $pn_start, $pn_num_rows ) {
		...
		self::$s_timestamp_cache['created_on'][ $ps_tablename ] += $o_log->getCreatedOnTimestampsForIDs( $ps_tablename,
			$va_row_ids );
		if ( ! is_array( self::$s_timestamp_cache['last_changed'][ $ps_tablename ] ) ) {
			self::$s_timestamp_cache['last_changed'][ $ps_tablename ] = array();
		}
		self::$s_timestamp_cache['last_changed'][ $ps_tablename ] += $o_log->getLastChangeTimestampsForIDs( $ps_tablename,
			$va_row_ids );
	...
	public function getInstance( $always_return = false ) {
		if ( ( $id = $this->opo_engine_result->get( $this->opo_subject_instance->primaryKey() ) )
		     && $this->opo_subject_instance->load( $id )
		) {
	 ...
	 * If you need to extract all values from single field in a result set this method provides a convenient means to
	 * do so.
	 *
	...
	/**
	 * Returns a value from the query result. This can be a single value if it is a field in the subject table (eg.
	 * objects table in an objects search), or perhaps multiple related values (eg. related entities in an objects
	 * search).
	 *
	 * You can fetch the values attached to a subject using the bundle specification, generally in the format
	 * <subject_table_name>.<element_code> (ex. ca_objects.date_created) If the bundle is a container then you can
	 * fetch a specific value using the format <subject_table_name>.<attribute_element_code>.<value_element_code> For
	 * example, to get the "date_value" value out of a "date" container attached to a ca_objects row, get() would be
	 * called with the field parameter set to ca_objects.date.date_value
	 *
	 * By default get() returns a string for display in the current locale. You can control the formatting of the
	 * output using various options described below including "template" (format output using a displayt template),
	 * "makeLink" (convert references to records into clickable links) and "delimiter" (specify text to place between
	 * multiple values)
	 *
	 * When the "returnAsArray" option is set get() will return a numerically indexed array list of values. This array
	 * will always be one-dimensional with a sequence of display values.
	 *
	 * You can force values for all available locales to be included in the returned string or array list using the
	 * "returnAllLocales" option.
	 *
	 * CollectiveAccess stores related, repeating and multilingual data in a fairly complex series of nested
	 * structures. get() is intended to faciliate output of data so most of its options are geared towards flattening
	 * of data for easy of formatting and display, with commensurate loss of internal structre. Set the
	 * "returnWithStructure" option to obtain the "raw" data with all of its internal structure intact. The returned
	 * value will be a multidimensional array tailored to the type of data being returned. Typically this array will be
	 * indexed first by the id of the record to which the returned data is attached, then by locale_id or code (if
	 * "returnAllLocales" is set), then the id specific to the data item (Eg. internal attribute_id for metadata,
	 * label_id for labels, Etc.), and finally an array with keys set to data element names and associated values.
	 *
	 * Return values can be modified using the following options:
	 *
	 *        [Options controlling type of return value]
	 *            returnAsArray = return values in a one-dimensional, numerically indexed array. If not not a string is
	 *            always returned. [Default is false] returnWithStructure = return values in a multi-dimensional array
	 *            mirroring the internal storage structure of CollectiveAccess. [Default is false] returnAsSearchResult
	 *            = return values as search result instance. [Default is false] returnAsCount = return the number of
	 *            values that would be returned. If returnAsArray or returnWithStructure is set then the count will be
	 *            returned as a one-element array, otherwise an integer value will be returned. [Default is false]
	 *
	 *        [Options controlling scope of data in return value]
	 *            returnAllLocales = Return values from all available locales, rather than just the most appropriate
	 *            locale for the current user. For string and array return values, returnAllLocales will result in
	 *            inclusion of additional values. For returnWithStructure, additional entries keys on locale_id or code
	 *            will be added.  [Default is false] useLocaleCodes =  For returnWithStructure locale codes (ex. en_US)
	 *            will be used rather than numeric locale_ids. [Default is false] restrictToTypes = For bundles
	 *            referencing data in related tables (ex. calling ca_entities.idno from a ca_objects result) will
	 *            restrict returned items to those of the specified types. An array of list item idnos and/or item_ids
	 *            may be specified. [Default is null] restrictToRelationshipTypes =  For bundles referencing data in
	 *            related tables (ex. calling ca_entities.idno from a ca_objects result) will restrict returned items
	 *            to those related using the specified relationship types. An array of relationship type idnos and/or
	 *            type_ids may be specified. [Default is null] excludeTypes = For bundles referencing data in related tables (ex. calling ca_entities.idno from a ca_objects result) will restrict returned items to those *not* of the specified types. An array of list item idnos and/or item_ids may be specified. [Default is null] excludeRelationshipTypes = For bundles referencing data in related tables (ex. calling ca_entities.idno from a ca_objects result) will restrict returned items to those *not* related using the specified relationship types. An array of relationship type idnos and/or type_ids may be specified. [Default is null] restrictToType = Synonym for restrictToTypes. [Default is null] restrictToRelationshipType = Synonym for restrictToRelationshipTypes. [Default is null] excludeType = Synonym for excludeTypes. [Default is null] excludeRelationshipType = Synonym for excludeRelationshipTypes. [Default is null] excludeValues = An array of values to exclude if found. [Default is null; return all values] excludeIdnos = An array of idnos to exclude if found. Only relevant when pulling related records, either via a related get spec (Eg. ca_entities.preferred_labels from a ca_object instance) or an authority metadata element. [Default is null; return all values] filters = Array list of elements to filter returned values on. The element must be part of the container being fetched from. For example, when fetching a value from a container element (ex. ca_objects.dates.date_value) you can filter on any other subelement in that container by passing the name of the subelement and a value (ex. "date_type" => "copyright"). Pass only the name of the subelement, not the full path that includes the table and container element. You may filter on multiple subelements by passing each subelement as a key in the array. Only values that match all filters are returned. You can filter on multiple values for a subelement by passing an array of values rather than a scalar (Eg. "date_type" => array("copyright", "patent")). Values that match *any* of the values will be returned. Only simple equivalance is supported. NOTE: Filters are only available when returnAsArray or returnWithStructure are set. [Default is null] assumeDisplayField = For returnWithStructure, return display field for ambiguous preferred label specifiers (ex. ca_entities.preferred_labels => ca_entities.preferred_labels.displayname). If set to false an array with all label fields is returned. [Default is true] returnURL = When fetching intrinsic value of type FT_MEDIA return URL to media rather than HTML tag. [Default is false] returnPath = When fetching intrinsic value of type FT_MEDIA return path to media rather than HTML tag. [Default is false] unserialize = When fetching intrinsic value of type FT_VARS (serialized variables) return unserialized value. [Default is false] list = A list code or array or list codes to restrict returned values to when referencing ca_list_items values. [Default is null] primaryOnly = Return only related representations marked "primary", otherwise return all representations. Has no effect when not pulling related representations. [Default is false]
	 *
	 *        [Formatting options for strings and arrays]
	 *            template = Display template use when formatting return values. @see
	 *            http://docs.collectiveaccess.org/wiki/Display_Templates. [Default is null] delimiter = Characters to
	 *            place in between repeating values when returning a string makeLink = Return value as a link to the
	 *            relevant editor (Providence) or detail (Pawtucket) when bundle references data in a related table;
	 *            return value as HTML link when value is URL type. [Default is false] returnAsLink = Synonym for
	 *            makeLink. [Default is false] convertCodesToDisplayText = Convert list item_ids text in the user's
	 *            preferred locale for display. [Default is false] convertCodesToIdno = Convert list item_ids to idno's
	 *            (ca_list_items.idno). If convertCodesToDisplayText is also set then it will take precedence. [Default
	 *            is false] convertCodesToValue = Convert list item_ids to item value's (ca_list_items.item_value). If
	 *            convertCodesToDisplayText is also set then it will take precedence. [Default is false] output =
	 *            Convert list item_ids to display text in user's preferred locale ("text") or idno ("idno"). This is an easier to type alternative to the convertCodesToDisplayText and convertCodesToIdno options. [Default is null] sort = Array list of bundles to sort returned values on. Currently sort is only supported when getting related values via simple related <table_name> and <table_name>.related bundle specifiers. Eg. from a ca_objects results you can sort when fetching 'ca_entities', 'ca_entities.related', 'ca_objects.related', etc.. The sortable bundle specifiers are fields with or without tablename. Only those fields returned for the related tables (intrinsics and label fields) are sortable. You can also sort on attributes if returnWithStructure is set. [Default is null] stripTags = Remove HTML/XML tags from returned values. [Default is false]
	 *
	 ...
	 *            truncate = Return all values from the beginning truncated to a maximum length; equivalent of passing
	 *            start=0 and length. [Default is null] ellipsis = Add ellipsis ("...") to truncated values. Values
	 *            will be set to the truncated length including the ellipsis. Eg. a value truncated to 12 characters
	 *            will include 9 characters of text and 3 characters of ellipsis. [Default is false] convertLineBreaks
	 *            = Convert newlines to <br/> tags. [Default is false]
	 *
	 *        [Formatting options for hierarchies]
	 *            maxLevelsFromTop = Restrict the number of levels returned to the top-most beginning with the root.
	 *            [Default is null] maxLevelsFromBottom = Restrict the number of levels returned to the bottom-most
	 *            starting with the lowest leaf node. [Default is null] maxLevels = synonym for maxLevelsFromBottom.
	 *            [Default is null] removeFirstItems = Number of levels from top of hierarchy before returning.
	 *            [Default is null] hierarchyDirection = Order in which to return hierarchical levels. Set to either
	 *            "asc" or "desc". "Asc"ending returns hierarchy beginning with the root; "desc"ending begins with the
	 *            child furthest from the root. [Default is asc] allDescendants = Return all items from the full depth
	 *            of the hierarchy when fetching children. By default only immediate children are returned. [Default is
	 *            false] hierarchyDelimiter = Characters to place in between separate hiearchy levels. Defaults to the
	 *            'delimiter' option. filterTypes = A list of types. Only hierarchy items with specified types will be
	 *            returned. [Default is null]
	 *
	 *        [Front-end access control]
	 *            checkAccess = Array of access values to filter returned values on. Available for any table with an
	 *            "access" field (ca_objects, ca_entities, etc.). If omitted no filtering is performed. [Default is
	 *            null]
	 *
	...
	public function get( $ps_field, $pa_options = null ) {
		...
		$vb_return_with_structure = isset( $pa_options['returnWithStructure'] )
			? (bool) $pa_options['returnWithStructure'] : false;
		if ( $vb_return_as_search_result = isset( $pa_options['returnAsSearchResult'] )
			? (bool) $pa_options['returnAsSearchResult'] : false
		) {
		...
		$vb_convert_line_breaks = isset( $pa_options['convertLineBreaks'] ) ? (bool) $pa_options['convertLineBreaks']
			: false;
		...
		if ( ( $ps_field == $this->ops_table_pk )
		     || ( $ps_field == $this->ops_table_name . '.' . $this->ops_table_pk )
		) {
		...
		$vb_return_all_locales = isset( $pa_options['returnAllLocales'] ) ? (bool) $pa_options['returnAllLocales']
			: false;

		$vs_delimiter              = isset( $pa_options['delimiter'] ) ? $pa_options['delimiter'] : ';';
		$vs_hierarchical_delimiter = isset( $pa_options['hierarchyDelimiter'] ) ? $pa_options['hierarchyDelimiter']
			: $vs_delimiter;
		...
		$vb_return_path                   = isset( $pa_options['returnPath'] ) ? (bool) $pa_options['returnPath']
			: false;
		...
		$vb_convert_codes_to_idno         = isset( $pa_options['convertCodesToIdno'] )
			? (bool) $pa_options['convertCodesToIdno'] : false;
		$vb_convert_codes_to_value        = isset( $pa_options['convertCodesToValue'] )
			? (bool) $pa_options['convertCodesToValue'] : false;

		$vb_strip_tags = isset( $pa_options['stripTags'] ) ? (bool) $pa_options['stripTags'] : false;

		$va_exclude_values = ( isset( $pa_options['excludeValues'] ) && $pa_options['excludeValues'] )
			? is_array( $pa_options['excludeValues'] ) ? $pa_options['excludeValues'] : [ $pa_options['excludeValues'] ]
			: [];
		$va_exclude_idnos  = ( isset( $pa_options['excludeIdnos'] ) && $pa_options['excludeIdnos'] )
			? is_array( $pa_options['excludeIdnos'] ) ? $pa_options['excludeIdnos'] : [ $pa_options['excludeIdnos'] ]
			: [];


		$vb_use_locale_codes     = isset( $pa_options['useLocaleCodes'] ) ? (bool) $pa_options['useLocaleCodes']
			: false;
		$vb_assume_display_field = isset( $pa_options['assumeDisplayField'] ) ? (bool) $pa_options['assumeDisplayField']
			: true;
		...
		if ( ! ( $vb_return_as_link = ( isset( $pa_options['makeLink'] ) ? (bool) $pa_options['makeLink']
			: false ) )
		) {
		...
		$vn_max_levels_from_top    = isset( $pa_options['maxLevelsFromTop'] ) ? (int) $pa_options['maxLevelsFromTop']
			: null;
		$vn_max_levels_from_bottom = caGetOption( array(
			'maxLevelsFromBottom',
			'maxLevels',
			'level_limit',
			'hierarchy_limit'
		), $pa_options, null );
		$vn_remove_first_items     = isset( $pa_options['removeFirstItems'] ) ? (int) $pa_options['removeFirstItems']
			: 0;

		$va_check_access = isset( $pa_options['checkAccess'] ) ? ( is_array( $pa_options['checkAccess'] )
			? $pa_options['checkAccess'] : array( $pa_options['checkAccess'] ) ) : null;
		$vs_template     = isset( $pa_options['template'] ) ? (string) $pa_options['template'] : null;


		$va_path_components = isset( SearchResult::$s_parsed_field_component_cache[ $this->ops_table_name . '/'
		                                                                            . $ps_field ] )
			? SearchResult::$s_parsed_field_component_cache[ $this->ops_table_name . '/' . $ps_field ]
			: $this->parseFieldPathComponents( $ps_field );
		...
		if ( $va_path_components['table_name'] != $this->ops_table_name ) {
			$vs_access_chk_key = $va_path_components['table_name'] . ( $va_path_components['field_name'] ? '.'
			                                                                                               . $va_path_components['field_name']
					: '' );
		} else {
			$vs_access_chk_key = $va_path_components['field_name'];
		}

		if ( ( $va_path_components['field_name'] !== 'access' )
		     && ( caGetBundleAccessLevel( $va_path_components['table_name'], $vs_access_chk_key )
		...
		if ( ! ( $t_instance = SearchResult::$s_instance_cache[ $va_path_components['table_name'] ] ) ) {
			$t_instance
				= SearchResult::$s_instance_cache[ $va_path_components['table_name'] ]
				= Datamodel::getInstanceByTableName( $va_path_components['table_name'], true );
		}
		if ( ! $t_instance ) {
			return null;
		}    // Bad table
		...
		if ( $va_path_components['hierarchical_modifier'] ) {
			...
			if ( $vb_assume_display_field
			     && in_array( $va_path_components['field_name'], array( 'preferred_labels', 'nonpreferred_labels' ) )
			     && ! $va_path_components['subfield_name']
			) {
				$va_path_components['subfield_name']
					                                  =
				$va_path_components['components'][2] = $t_instance->getLabelDisplayField();
			...
			switch ( $va_path_components['hierarchical_modifier'] ) {
				case 'parent':
					...
					if ( $va_path_components['related'] ) {
						...
						if ( ! isset( SearchResult::$opa_hierarchy_parent_prefetch_cache_index[ $this->ops_table_name ][ $vn_row_id ][ $va_path_components['table_name'] ][ $vs_opt_md5 ] ) ) {
							$this->prefetchHierarchyParents( $va_path_components['table_name'],
								$this->opo_engine_result->currentRow(), $this->getOption( 'prefetch' ), $pa_options );
						}

						$va_ids
							= SearchResult::$opa_hierarchy_parent_prefetch_cache_index[ $this->ops_table_name ][ $vn_row_id ][ $va_path_components['table_name'] ][ $vs_opt_md5 ];
					} else {
					...
					foreach ( $va_ids as $vn_id ) {
						...
						if (
							...
							if ( ! is_array( $va_parent_ids
								= SearchResult::$opa_hierarchy_parent_prefetch_cache[ $va_path_components['table_name'] ][ $vn_id ][ $vs_opt_md5 ] )
							) {
								return $pa_options['returnAsArray'] ? array() : null;
							}
						}
						...
						if ( ! ( $qr_hier = $t_instance->makeSearchResult( $va_path_components['table_name'],
							$va_parent_ids, $pa_options ) )
						) {
							return $pa_options['returnAsArray'] ? array() : null;
						}
				...
				case 'hierarchy':
					...
					if ( $va_path_components['related'] ) {
						...
						$va_ids
							= array_values( SearchResult::$opa_hierarchy_parent_prefetch_cache_index[ $this->ops_table_name ][ $vn_row_id ][ $va_path_components['table_name'] ][ $vs_opt_md5 ] );
					...
					$vs_hierarchy_direction = isset( $pa_options['hierarchyDirection'] )
						? strtolower( $pa_options['hierarchyDirection'] ) : 'asc';
					...
					if ( $t_instance->isHierarchical() ) {
						if ( $va_path_components['field_name'] === $vs_hier_pk_fld ) {
							if ( $va_path_components['related'] ) {
								foreach ( $va_ids as $vn_id ) {
									if ( is_array( SearchResult::$opa_hierarchy_parent_prefetch_cache[ $va_path_components['table_name'] ][ $vn_id ][ $vs_opt_md5 ] ) ) {
										$va_hier_id_list = array_merge( array( $vn_id ),
											SearchResult::$opa_hierarchy_parent_prefetch_cache[ $va_path_components['table_name'] ][ $vn_id ][ $vs_opt_md5 ] );
										...
										if ( ! is_null( $vn_max_levels_from_top ) && ( $vn_max_levels_from_top > 0 ) ) {
											$va_hier_id_list = array_slice( $va_hier_id_list, 0,
												$vn_max_levels_from_top, true );
										} elseif ( ! is_null( $vn_max_levels_from_bottom )
										           && ( $vn_max_levels_from_bottom > 0 )
										) {
											if ( ( $vn_start = sizeof( $va_hier_id_list ) - $vn_max_levels_from_bottom )
											     < 0
											) {
												$vn_start = 0;
											}
											$va_hier_id_list = array_slice( $va_hier_id_list, $vn_start,
												$vn_max_levels_from_bottom, true );
										}
								...
								if ( is_array( SearchResult::$opa_hierarchy_parent_prefetch_cache[ $va_path_components['table_name'] ][ $vn_row_id ][ $vs_opt_md5 ] ) ) {
									$va_hier_ids = array_merge( array( $vn_row_id ),
										SearchResult::$opa_hierarchy_parent_prefetch_cache[ $va_path_components['table_name'] ][ $vn_row_id ][ $vs_opt_md5 ] );
								} else {
									$va_hier_ids = array( $vn_row_id );
								}

								if ( ( $vn_type_id = $this->get( $va_path_components['table_name'] . ".type_id" ) )
								     && ( $va_restrict_to_types = caGetOption( 'restrictToTypes', $pa_options, null ) )
								) {
									// NOTE: this restriction is always "straight" – it doesn't automatically include sub-types
									$va_types = caMakeTypeIDList( $va_path_components['table_name'],
										$va_restrict_to_types, [ 'dontIncludeSubtypesInTypeRestriction' => true ] );
									if ( ! in_array( $vn_type_id, $va_types ) ) {
										array_shift( $va_hier_ids );
									}
								}

								if ( in_array( $t_instance->getHierarchyType(),
									[ __CA_HIER_TYPE_SIMPLE_MONO__, __CA_HIER_TYPE_MULTI_MONO__ ] )
								) {
								...
								if ( ! is_null( $vn_max_levels_from_top ) && ( $vn_max_levels_from_top > 0 ) ) {
									...
									$va_hier_ids = array_slice( $va_hier_ids, $vn_start, $vn_max_levels_from_bottom,
										true );
								}

							}

							$vm_val = $vb_return_as_array ? $va_hier_ids
								: join( $vs_hierarchical_delimiter, $va_hier_ids );
							goto filter;
						} else {
							$vs_field_spec = join( '.', array_values( $va_path_components['components'] ) );

							$va_ancestor_id_list = $this->get( $va_path_components['table_name'] . '.hierarchy.'
							                                   . $vs_hier_pk_fld, array_merge( $pa_options, array(
								'returnAsArray'    => true,
								'returnAsLink'     => false,
							...
							if (
								...
								$filter_by_types = caMakeTypeIDList( $va_path_components['table_name'],
									$pa_options['filterTypes'], [ 'dontIncludeSubtypesInTypeRestriction' => true ] );
								...
								if ( $qr_hier = caMakeSearchResult( $va_path_components['table_name'], $va_ancestor_ids,
									...
									while ( $qr_hier->nextHit() ) {
										if ( is_array( $filter_by_types ) && sizeof( $filter_by_types ) ) {
											if ( ! is_array( $type_struct = $qr_hier->get( $type_spec, array(
												'returnWithStructure' => true,
												'returnAllLocales'    => true,
												'useLocaleCodes'      => $pa_options['useLocaleCodes']
											) ) )
											) {
												continue;
											}
										...
										if ( is_array( $qh = $qr_hier->get( $vs_field_spec, array(
											'returnWithStructure'         => true,
											'returnAllLocales'            => true,
											'useLocaleCodes'              => $pa_options['useLocaleCodes'],
											'convertCodesToDisplayText'   => $pa_options['convertCodesToDisplayText'],
											'convertCodesToIdno'          => $pa_options['convertCodesToIdno'],
											'convertCodesToValue'         => $pa_options['convertCodesToValue'],
											'omitDateSortKey'             => true,
											'restrictToTypes'             => caGetOption( 'restrictToTypes',
												$pa_options, null ),
									...
									if ( ( $va_path_components['table_name'] == 'ca_objects' )
									     && caGetOption( 'showCollectionObjectHierarchy', $pa_options, false )
									     && ( $config->get( 'ca_objects_x_collections_hierarchy_enabled' ) )
									) {
										if ( ( $qr_bridge = caMakeSearchResult( $va_path_components['table_name'],
												[ $va_ancestor_ids[0] ], $pa_options ) )
										     && $qr_bridge->nextHit()
										) {
											$t           = explode( '.', $vs_field_spec );
											$t[0]        = 'ca_collections';
											$collections = $qr_bridge->get( join( '.', $t ), [
												'returnWithStructure'         => true,
												'returnAllLocales'            => true,
												'useLocaleCodes'              => $pa_options['useLocaleCodes'],
												'restrictToRelationshipTypes' => [ $config->get( 'ca_objects_x_collections_hierarchy_relationship_type' ) ]
											] );
									...
									if ( ! is_null( $vn_max_levels_from_top ) && ( $vn_max_levels_from_top > 0 ) ) {
										...
										if ( ( $vn_start = sizeof( $va_hier_item ) - $vn_max_levels_from_bottom )
										     < 0
										) {
											$vn_start = 0;
										}
										$va_hier_item = array_slice( $va_hier_item, $vn_start,
											$vn_max_levels_from_bottom, true );
									}
					...
					foreach ( $va_hier_list as $vn_h => $va_hier_item ) {
						...
						if ( $vb_return_with_structure ) {
							...
							$va_acc[] = join( $vs_hierarchical_delimiter,
								$this->_flattenArray( $va_hier_item, $pa_options ) );
						}
					}
					if ( ! $vb_return_as_array ) {
				...
				case 'children':
					...
					if ( $va_path_components['related'] ) {
						...
						if ( ! isset( SearchResult::$opa_hierarchy_children_prefetch_cache_index[ $this->ops_table_name ][ $vn_row_id ][ $va_path_components['table_name'] ][ $vs_opt_md5 ] ) ) {
							$this->prefetchHierarchyChildren( $va_path_components['table_name'],
								$this->opo_engine_result->currentRow(), $this->getOption( 'prefetch' ), $pa_options );
						}

						$va_ids
							= SearchResult::$opa_hierarchy_children_prefetch_cache_index[ $this->ops_table_name ][ $vn_row_id ][ $va_path_components['table_name'] ][ $vs_opt_md5 ];
					} else {
					...
					foreach ( $va_ids as $vn_id ) {
						...
						$qr_hier = $t_instance->makeSearchResult( $va_path_components['table_name'],
						...
						while ( $qr_hier->nextHit() ) {
							$vm_val = $qr_hier->get( $vs_hier_fld_name, $pa_options );
							$va_hier_list[ $qr_hier->get( $va_path_components['table_name'] . '.' . $vs_pk ) ]
							        = $vb_return_as_array ? array_shift( $vm_val ) : $vm_val;;
						}
					}

					if ( ! $vb_return_as_array ) {
						return $vb_return_as_count ? sizeof( $va_hier_list )
							: join( $vs_hierarchical_delimiter, $va_hier_list );
					}
					$vm_val = $va_hier_list;
					goto filter;
					break;
				case 'siblings':
					...
					if ( $va_path_components['related'] ) {
						...
						if ( ! isset( SearchResult::$opa_hierarchy_siblings_prefetch_cache_index[ $this->ops_table_name ][ $vn_row_id ][ $va_path_components['table_name'] ][ $vs_opt_md5 ] ) ) {
							$this->prefetchHierarchySiblings( $va_path_components['table_name'],
								$this->opo_engine_result->currentRow(), $this->getOption( 'prefetch' ), $pa_options );
						}

						$va_ids
							= SearchResult::$opa_hierarchy_siblings_prefetch_cache_index[ $this->ops_table_name ][ $vn_row_id ][ $va_path_components['table_name'] ][ $vs_opt_md5 ];
					...
					foreach ( $va_ids as $vn_id ) {
						...
						$qr_hier = $t_instance->makeSearchResult( $va_path_components['table_name'],
							SearchResult::$opa_hierarchy_siblings_prefetch_cache[ $va_path_components['table_name'] ][ $vn_id ][ $vs_opt_md5 ],
							$pa_options );
...
//
			...
			$vs_opt_md5 = caMakeCacheKeyFromOptions(
				$va_get_opts = array_merge( $pa_options, array( 'dontReturnLabels' => false ) ) );
			...
			if ( ! isset( self::$s_rel_prefetch_cache[ $this->ops_table_name ][ $vn_row_id ][ $va_path_components['table_name'] ][ $vs_opt_md5 ] ) ) {
				$this->prefetchRelated( $va_path_components['table_name'], $this->opo_engine_result->currentRow(),
					$this->getOption( 'prefetch' ), $va_get_opts );
			}

			$va_related_items
				= self::$s_rel_prefetch_cache[ $this->ops_table_name ][ $vn_row_id ][ $va_path_components['table_name'] ][ $vs_opt_md5 ];
...
//
				if ( $va_path_components['field_name'] == '_guid' ) {
					$vm_val = ca_guids::getForRow( Datamodel::getTableNum( $va_path_components['table_name'] ),
						$vn_row_id );
...
//
				if ( $va_path_components['field_name'] == 'created' ) {
					if ( ! isset( self::$s_timestamp_cache['created_on'][ $this->ops_table_name ][ $vn_row_id ] ) ) {
						$this->prefetchChangeLogData( $this->ops_table_name, $this->opo_engine_result->currentRow(),
							$this->getOption( 'prefetch' ) );
					}

					if ( $vb_return_as_array ) {
						if ( $va_path_components['subfield_name'] ) {
							$vm_val
								= [ self::$s_timestamp_cache['created_on'][ $this->ops_table_name ][ $vn_row_id ][ $va_path_components['subfield_name'] ] ];
						} elseif ( $vb_return_with_structure ) {
							$vm_val = self::$s_timestamp_cache['created_on'][ $this->ops_table_name ][ $vn_row_id ];
						} else {
							$vm_val
								= self::$s_timestamp_cache['created_on'][ $this->ops_table_name ][ $vn_row_id ]['timestamp'];
							$this->opo_tep->init();
						...
						switch ( $vs_subfield ) {
							...
							case 'email':
								$vm_val
									= self::$s_timestamp_cache['created_on'][ $this->ops_table_name ][ $vn_row_id ][ $vs_subfield ];
								break;
							case 'timestamp':
								// noop
								break;
...
//
				if ( $va_path_components['field_name'] == 'lastModified' ) {
					...
					if ( $vb_return_as_array ) {
						if ( $va_path_components['subfield_name'] ) {
							$vm_val
								= [ self::$s_timestamp_cache['last_changed'][ $this->ops_table_name ][ $vn_row_id ][ $va_path_components['subfield_name'] ] ];
						} elseif ( $vb_return_with_structure ) {
							$vm_val = self::$s_timestamp_cache['last_changed'][ $this->ops_table_name ][ $vn_row_id ];
						} else {
							$vm_val
								= self::$s_timestamp_cache['last_changed'][ $this->ops_table_name ][ $vn_row_id ]['timestamp'];
							$this->opo_tep->init();
							$this->opo_tep->setUnixTimestamps( $vm_val, $vm_val );
							$vm_val = [ $this->opo_tep->getText( $pa_options ) ];
						}
						...
						switch ( $vs_subfield ) {
							...
							case 'email':
								$vm_val
									= self::$s_timestamp_cache['last_changed'][ $this->ops_table_name ][ $vn_row_id ][ $vs_subfield ];
								break;
							case 'timestamp':
								// noop
								break;
...
//
				if ( in_array( $va_path_components['field_name'], array( 'preferred_labels', 'nonpreferred_labels' ) )
				     && ( $t_instance instanceof LabelableBaseModelWithAttributes )
				) {
					$vs_label_table_name = $t_instance->getLabelTableName();
					if ( ! isset( self::$s_prefetch_cache[ $vs_label_table_name ][ $vn_row_id ][ $vs_opt_md5 ] ) ) {
						$this->prefetchLabels( $va_path_components['table_name'],
							$this->opo_engine_result->currentRow(), $this->getOption( 'prefetch' ), $pa_options );
					}

					$vm_val
						= $this->_getLabelValue( self::$s_prefetch_cache[ $vs_label_table_name ][ $vn_row_id ][ $vs_opt_md5 ],
						$t_instance, array_merge( $va_val_opts, [
							'restrictToTypes' => caGetOption( 'restrictToTypes', $pa_options, null ),
							'excludeTypes'    => caGetOption( 'excludeTypes', $pa_options, null )
						] ) );
					if ( $vb_return_as_count ) {
						return $vm_val;
					}
...
//
					if ( ! isset( self::$s_prefetch_cache[ $va_path_components['table_name'] ][ $vn_row_id ][ $vs_opt_md5 ] ) ) {
						$this->prefetch( $va_path_components['table_name'], $this->opo_engine_result->currentRow(),
							$this->getOption( 'prefetch' ), $pa_options );
					}
					$vm_val
						= $this->_getIntrinsicValue( self::$s_prefetch_cache[ $va_path_components['table_name'] ][ $vn_row_id ][ $vs_opt_md5 ],
						$t_instance, $va_val_opts );
					goto filter;
				} elseif ( method_exists( $t_instance, 'isValidBundle' )
				           && ! $t_instance->hasElement( $va_path_components['field_name'], null, false,
						array( 'dontCache' => false ) )
				           && $t_instance->isValidBundle( $va_path_components['field_name'] )
				) {
//
...
//
					...
					$vm_val = $t_instance->renderBundleForDisplay( $va_path_components['field_name'], $vn_row_id,
						self::$s_prefetch_cache[ $va_path_components['table_name'] ][ $vn_row_id ][ $vs_opt_md5 ],
						$va_val_opts );

					if ( $pa_options['returnWithStructure'] ) {
						if ( $pa_options['returnAllLocales'] ) {
							$vm_val
								= [ $vn_row_id => [ $t_instance->get( 'locale_id' ) => [ $va_path_components['field_name'] => $vm_val ] ] ];
						} else {
...
//
					...
					if ( ( $t_instance instanceof BaseModelWithAttributes )
					     && isset( $va_path_components['field_name'] )
					     && $va_path_components['field_name']
					     && $t_element = ca_metadata_elements::getInstance( $va_path_components['field_name'] )
					) {
						$vn_element_id = $t_element->getPrimaryKey();
					} else {
						return $pa_options['returnAsArray'] ? array() : null;
					}
					if ( ! isset( ca_attributes::$s_get_attributes_cache[ (int) $this->opn_table_num . '/'
					                                                      . (int) $vn_row_id ][ (int) $vn_element_id ] )
					) {
						$va_element_ids = ( $vn_element_id ? array( $vn_element_id ) : null );
						if ( is_array( $va_prefetch_attributes = $this->getOption( 'prefetchAttributes' ) )
						     && sizeof( $va_prefetch_attributes )
						) {
							$va_element_ids = array_unique( $va_element_ids + $va_prefetch_attributes );
						}
						ca_attributes::prefetchAttributes( $this->opo_subject_instance->getDb(), $this->opn_table_num,
							$this->getRowIDsToPrefetch( $this->opo_engine_result->currentRow(),
								$this->getOption( 'prefetch' ) ), $va_element_ids,
							array( 'dontFetchAlreadyCachedValues' => true ) );
					}

					$va_attributes = ca_attributes::getAttributes( $this->opo_subject_instance->getDb(),
						$this->opn_table_num, $vn_row_id, array( $vn_element_id ), array() );
		...
		if ( $vb_return_as_array && sizeof( $va_filters ) ) {
			...
			if ( ! ( $t_instance = SearchResult::$s_instance_cache[ $va_tmp[0] ] ) ) {
				$t_instance
					=
				SearchResult::$s_instance_cache[ $va_tmp[0] ] = Datamodel::getInstanceByTableName( $va_tmp[0], true );
			}
				...
				foreach ( $va_filters as $vs_filter => $va_filter_vals ) {
					...
					foreach ( $va_filter_vals as $vn_index => $vs_filter_val ) {
						...
						if ( ! is_numeric( $vs_filter_val )
						     && ( ( $t_element
									= ca_metadata_elements::getInstance( $vs_filter ) )
					...
					$va_filter_values = $this->get( join( ".", $va_tmp ) . ".{$vs_filter}", array(
						'returnAsArray'      => true,
						'alwaysReturnItemID' => true,
					...
					if ( is_array( $va_filter_values ) ) {
						foreach ( $va_filter_values as $vn_id => $vm_filtered_val ) {
							if ( ( ! isset( $va_keepers[ $vn_id ] ) || $va_keepers[ $vn_id ] )
							     && in_array( $vm_filtered_val, $va_filter_vals )
	 ...
	 * Results are speculatively pre-fetched in blocks to improve performance. This can cause issues
	 * if you're inserting rows and expect a previously create SearchResult to "see" those new rows.
	 * SearchResult::clearResultCacheForRow() will clear the result cache for a single row, specified by its primary
	 * key, causing get() to re-fetch fresh data for the row. If applicable, the table storing related labels will also
	 * be purged.
	 *
	 * @param string $ps_table Name of table to purge cache for
	 *
	...
	private function _getRelatedValue( $pa_value_list, $pa_options = null ) {
		...
		if ( ! $va_path_components['field_name'] ) {
			if ( $pa_options['returnWithStructure'] ) {
				...
				$va_path_components['subfield_name']
				                                      =
				$va_path_components['components'][2] = $t_rel_instance->getLabelDisplayField();
				$va_path_components['num_components'] = sizeof( $va_path_components['components'] );
			}
		}
		...
		while ( $qr_rel->nextHit() ) {
			...
			if ( is_array( $pa_check_access ) && sizeof( $pa_check_access ) && $t_rel_instance->hasField( 'access' )
			     && ! in_array( $qr_rel->get( $va_path_components['table_name'] . ".access" ), $pa_check_access )
			) {
				continue;
			}
			...
			if ( ( $vs_rel_table_name == 'ca_list_items' ) && is_array( $pa_restrict_to_lists )
			     && sizeof( $pa_restrict_to_lists )
			     && ! in_array( $qr_rel->get( "ca_list_items.list_id" ), $pa_restrict_to_lists )
			) {
				continue;
			}
		...
		if ( $vb_return_as_link ) {
			$va_return_values = caCreateLinksFromText( $va_return_values, $t_rel_instance->tableName(),
				array( $va_relation_info[ $vs_pk ] ) );
		}
	...
	private function _getLabelValue( $pa_value_list, $pt_instance, $pa_options ) {
		$vb_assume_display_field          = isset( $pa_options['assumeDisplayField'] )
			? (bool) $pa_options['assumeDisplayField'] : true;
		$vb_convert_codes_to_display_text = isset( $pa_options['convertCodesToDisplayText'] )
			? (bool) $pa_options['convertCodesToDisplayText'] : false;
		$vb_convert_codes_to_idno         = isset( $pa_options['convertCodesToIdno'] )
			? (bool) $pa_options['convertCodesToIdno'] : false;
		...
		if (
			...
			&& ( $vs_label_type_list_code = $pt_instance->getAppConfig()->get( ( $pt_instance->tableName() . '_'
			                                                                     . ( ( $va_path_components['field_name']
			                                                                           == 'nonpreferred_labels' )
					? 'nonpreferred_label_type_list' : 'preferred_label_type_list' ) ) ) )
		) {
		...
		if ( $vb_assume_display_field && ! $va_path_components['subfield_name'] ) {
			$va_path_components['components'][2]
				                                  =
			$va_path_components['subfield_name'] = $pt_instance->getLabelDisplayField();
			$va_path_components['num_components'] = sizeof( $va_path_components['components'] );
		}
		...
		if ( is_array( $pa_value_list ) ) {
			foreach ( $pa_value_list as $vn_locale_id => $va_labels_by_locale ) {
				...
				if ( $pa_options['useLocaleCodes'] ) {
					if ( ! $vn_locale_id
					     || ! ( $vm_locale_id
							= SearchResult::$opo_locales->localeIDToCode( $vn_locale_id ) )
					) {
				...
				foreach ( $va_labels_by_locale as $vn_id => $va_label ) {
					if ( is_array( $va_restrict_to_type_ids ) && sizeof( $va_restrict_to_type_ids )
					     && ! in_array( $va_label['type_id'], $va_restrict_to_type_ids )
					) {
						continue;
					}
					if ( is_array( $va_exclude_type_ids ) && sizeof( $va_exclude_type_ids )
					     && in_array( $va_label['type_id'], $va_exclude_type_ids )
					) {
						continue;
					}
					...
					if ( isset( $va_label['is_preferred'] ) ) {
						if ( ( ( (bool) $va_label['is_preferred'] )
							...
							// noop
						} elseif ( ( ( ! (bool) $va_label['is_preferred'] )
						             && ( $va_path_components['field_name'] == 'nonpreferred_labels' ) )
						) {
							// noop
						} else {
							continue;
					...
					switch ( $pa_options['output'] ) {
						case 'text':
							$vs_val_proc = $this->_convertCodeToDisplayText( $vs_val_proc, $va_path_components,
								$pt_instance->getLabelTableInstance(), $pa_options );
							break;
						case 'idno':
							$vs_val_proc = $this->_convertCodeToIdno( $vs_val_proc, $va_path_components,
								$pt_instance->getLabelTableInstance(), $pa_options );
							break;
					}

					if ( $pa_options['makeLink'] ) {
						$vs_val_proc = array_shift( caCreateLinksFromText( array( $vs_val_proc ), $vs_table_name,
							array( $vn_id ) ) );
					}

					if ( $pa_options['returnWithStructure'] ) {
						if ( ! $va_path_components['subfield_name'] && isset( $va_label['type_id'] ) ) {
							$va_mod_path_components                  = array_merge( $va_path_components,
								[ 'subfield_name' => 'type_id', 'num_components' => 3 ] );
							$va_mod_path_components['components'][2] = 'type_id';
							switch ( $pa_options['output'] ) {
								case 'text':
									$va_label['type_id'] = $this->_convertCodeToDisplayText( $va_label['type_id'],
										$va_mod_path_components, $pt_instance->getLabelTableInstance(), $pa_options );
									break;
								case 'idno':
									$va_label['type_id'] = $this->_convertCodeToIdno( $va_label['type_id'],
										$va_mod_path_components, $pt_instance->getLabelTableInstance(), $pa_options );
									break;
							}
						}
						$va_return_values[ $vn_id ][ $vm_locale_id ][ $va_label['label_id'] ]
							= $va_path_components['subfield_name']
							? array( $va_path_components['subfield_name'] => $vs_val_proc ) : $va_label;
					} else {
						$va_return_values[ $vn_id ][ $vm_locale_id ][ $va_label['label_id'] ]
							= $va_path_components['subfield_name'] ? $vs_val_proc : $va_label;
					}
				}
			}
		}
		...
		if ( $pa_options['returnAsArray'] ) {
			...
			return ( sizeof( $va_flattened_values ) > 0 ) ? join( $pa_options['delimiter'], $va_flattened_values )
				: null;
		}
	...
	private function _getAttributeValue( $pa_value_list, $pt_instance, $pa_options ) {
		...
		if ( is_array( $pa_value_list ) && sizeof( $pa_value_list ) ) {
			...
			foreach ( $pa_value_list as $o_attribute ) {
				...
				if ( $pa_options['useLocaleCodes'] ) {
					if ( ! $o_attribute->getLocaleID()
					     || ! ( $vm_locale_id
							= SearchResult::$opo_locales->localeIDToCode( $o_attribute->getLocaleID() ) )
					) {
						$vm_locale_id = __CA_DEFAULT_LOCALE__;
					};
				} else {
				...
				$vb_return_value_id  = ( $va_path_components['components'][ sizeof( $va_path_components['components'] )
				                                                            - 1 ] === 'value_id' );
				...
				foreach ( $va_values as $o_value ) {
					...
					if ( ( $vn_attr_type == 0 ) && $va_path_components['subfield_name']
					     && ( $vs_element_code != $va_path_components['subfield_name'] )
					) {
						continue;
					}
					...
					if ( is_a( $o_value, "AuthorityAttributeValue" ) ) {
						...
						$vb_element_is_present     = $pt_instance->hasElement( $va_path_components['field_name'], null,
							true, array( 'dontCache' => false ) );
						$vb_sub_element_is_present = $pt_instance->hasElement( $va_path_components['subfield_name'],
							null, true, array( 'dontCache' => false ) );

						if ( SearchResult::_isHierarchyModifier( $va_path_components['field_name'] )
						     && $vb_sub_element_is_present
						) {
							...
							while ( sizeof( $va_auth_spec )
							        && ! SearchResult::_isHierarchyModifier( $va_auth_spec[0] ) ) {
								array_shift( $va_auth_spec ); // remove auth_attr_code
							}
					...
					if ( $va_path_components['subfield_name']
					     && ( $va_path_components['subfield_name'] !== $vs_element_code )
					     && ! SearchResult::_isHierarchyModifier( $va_path_components['subfield_name'] )
					     && ! ( $o_value instanceof InformationServiceAttributeValue )
					     && ! ( $o_value instanceof LCSHAttributeValue )
					     && ! ( $o_value instanceof MediaAttributeValue )
					     && ! ( $o_value instanceof FileAttributeValue )
					     && ! is_a( $o_value, "AuthorityAttributeValue" )
					) {
					...
					if ( is_a( $o_value, "AuthorityAttributeValue" ) ) {
						...
						if ( $qr_res = caMakeSearchResult( $vs_auth_table_name, array( $o_value->getID() ) ) ) {
							...
							if ( $qr_res->nextHit() ) {
								if ( ( $t_instance = $o_value->elementTypeToInstance( $o_value->getType() ) )
								     && ( $vs_idno_fld = $t_instance->getProperty( 'ID_NUMBERING_ID_FIELD' ) )
								) {
									if ( in_array( $qr_res->get( $vs_auth_table_name . '.' . $vs_idno_fld ),
										$pa_exclude_idnos )
									) {
										continue;
									}
									...
									foreach ( $va_val_proc as $vn_i => $vs_v ) {
										...
										if ( $pa_options['returnWithStructure'] ) {
											$va_return_values[ (int) $vn_id ][ $vm_locale_id ][ (int) $o_attribute->getAttributeID()
											                                                    . ( ( $vn_i > 0 )
												? "_{$vn_i}" : '' ) ][ $vs_element_code ]
												= ( $vb_has_field_spec || $vb_has_hierarchy_modifier )
												? $vs_v
												: $o_value->getDisplayValue( array_merge( $pa_options, array(
													'output'  => $pa_options['output'],
													'list_id' => $vn_list_id
												) ) );
											if ( $include_value_ids ) {
												$va_return_values[ (int) $vn_id ][ $vm_locale_id ][ (int) $o_attribute->getAttributeID()
												                                                    . ( ( $vn_i > 0 )
													? "_{$vn_i}" : '' ) ]["{$vs_element_code}_value_id"]
													= $o_value->getValueID();
											}
										} else {
											$va_return_values[ (int) $vn_id ][ $vm_locale_id ][ (int) $o_attribute->getAttributeID() ][]
					...
					if ( is_null( $vs_val_proc ) ) {
						switch ( $o_value->getType() ) {
							...
							case __CA_ATTRIBUTE_VALUE_FILE__:
								$vs_return_type = 'tag';
								$va_versions    = ( $o_value->getType() == __CA_ATTRIBUTE_VALUE_MEDIA__ )
									? $o_value->getVersions() : [];
								...
								if ( $alt_text = caGetOption( 'alt', $pa_options, null ) ) {
									// noop
								} elseif ( $alt_text_template = Configuration::load()->get( $this->tableName()
								                                                            . "_alt_text_template" )
								) {
								...
								$vs_val_proc = $o_value->getDisplayValue( [
									'alt'     => $alt_text,
									'return'  => $vs_return_type,
									'version' => $vs_version
								] );
								break;
							case __CA_ATTRIBUTE_VALUE_LIST__:
								$t_element  = ca_metadata_elements::getInstance( $o_value->getElementID() );
								$vn_list_id = $t_element->get( 'list_id' );
								if ( in_array( $o_value->getDisplayValue( array(
									'output'  => 'idno',
									'list_id' => $vn_list_id
								) ), $pa_exclude_idnos )
								) {
								...
								$vs_val_proc = $o_value->getDisplayValue( array_merge( $pa_options,
									array( 'output' => $pa_options['output'], 'list_id' => $vn_list_id ) ) );
								break;
							case __CA_ATTRIBUTE_VALUE_LCSH__:
								switch ( $va_path_components['subfield_name'] ) {
									...
									case 'url':
										$vs_val_proc = $o_value->getDisplayValue( array_merge( $pa_options,
											array( $va_path_components['subfield_name'] => true ) ) );
										break;
									default:
										$vs_val_proc = $o_value->getDisplayValue( array_merge( $pa_options,
											array( 'output' => $pa_options['output'] ) ) );
										break;
								}
								break;
							case __CA_ATTRIBUTE_VALUE_INFORMATIONSERVICE__:
								...
								if ( $va_path_components['subfield_name']
								     && ( $vs_element_code != $va_path_components['subfield_name'] )
								     && ( $vs_element_code == $va_path_components['field_name'] )
								) {
									switch ( $va_path_components['subfield_name'] ) {
										case 'uri':
										case 'url':
										...
										default:
											$vs_val_proc
												= $o_value->getExtraInfo( $va_path_components['subfield_name'] );
											break;
									}
									...
									if ( isset( $pa_options['coordinates'] ) && $pa_options['coordinates'] ) {
										if ( preg_match( "!\[([^\]]+)!", $vs_val_proc, $va_matches ) ) {
											...
											if ( ( sizeof( $va_tmp ) == 2 ) && ( is_numeric( $va_tmp[0] ) )
											     && ( is_numeric( $va_tmp[1] ) )
											) {
												$vs_val_proc = array(
								...
								if ( ( $vs_element_code == $va_path_components['subfield_name'] )
								     && ( $va_path_components['num_components'] == 4 )
								) {
								...
								if ( ( $vs_element_code == $va_path_components['field_name'] )
								     || ( $vs_element_code == $va_path_components['subfield_name'] )
								) {
									$vs_val_proc          = $o_value->getDisplayValue( array_merge( $pa_options,
										array( 'output' => $pa_options['output'] ) ) );
								...
								continue( 2 );
							default:
								if ( in_array( $o_value->getDisplayValue( array( 'output' => 'idno' ) ),
									$pa_exclude_idnos )
								) {
									continue( 2 );
								}
					...
					if ( ( $vn_attr_type == __CA_ATTRIBUTE_VALUE_CONTAINER__ ) && ! $va_path_components['subfield_name']
					     && ! $pa_options['returnWithStructure']
					     && ! $vs_val_proc
					) {
					...
					array_pop( $va_spec );
					$va_acc[ join( '.', $va_spec ) . '.' . $vs_element_code ]
						= $o_value->getDisplayValue( array_merge( $pa_options, array( 'output' => 'idno' ) ) );
					...
					if ( ! $vb_dont_return_value ) {
						...
						if ( $pa_options['returnWithStructure'] ) {
							$va_return_values[ (int) $vn_id ][ $vm_locale_id ][ (int) $o_attribute->getAttributeID() ][ $vs_element_code ]
								= $vs_val_proc;
							if ( $o_value->getType() == __CA_ATTRIBUTE_VALUE_DATERANGE__
							     && ! caGetOption( 'omitDateSortKey', $pa_options, false )
							) {  // add sortable alternate value for dates
								$va_return_values[ (int) $vn_id ][ $vm_locale_id ][ (int) $o_attribute->getAttributeID() ][ $vs_element_code
								                                                                                            . '_sort_' ]
									= $o_value->getDisplayValue( [ 'sortable' => true ] ); // add sortable alternate representation for dates; this will be used by the SearchResult::get() "sort" option to properly sort date values
							}
							if ( $include_value_ids ) {
								$va_return_values[ (int) $vn_id ][ $vm_locale_id ][ (int) $o_attribute->getAttributeID() ]["{$vs_element_code}_value_id"]
									= $o_value->getValueID();
							}
						} else {
							$va_return_values[ (int) $vn_id ][ $vm_locale_id ][ (int) $o_attribute->getAttributeID() ][]
				...
				if ( $va_path_components['subfield_name'] && $pa_options['returnBlankValues']
				     && ! $vb_did_return_value
				) {
			...
			// is blank
			$default_value = ca_metadata_elements::getElementDefaultValue( $va_path_components['subfield_name']
				? $va_path_components['subfield_name'] : $va_path_components['field_name'] );

			if ( $pa_options['returnWithStructure'] && $pa_options['returnBlankValues'] ) {
				$va_return_values[ (int) $vn_id ][ null ][ null ][ $e = $va_path_components['subfield_name']
					? $va_path_components['subfield_name'] : $va_path_components['field_name'] ]
					= $default_value;
				if ( $include_value_ids ) {
			...
			case FT_HISTORIC_DATETIME:
				foreach ( $pa_value_list as $vn_locale_id => $va_values ) {
					...
					foreach ( $va_values as $vn_i => $va_value ) {
						...
						if ( in_array( $va_field_info['FIELD_TYPE'],
							array( FT_TIMESTAMP, FT_DATETIME, FT_HISTORIC_DATETIME ) )
						) {
							...
							if ( $va_field_info['FIELD_TYPE'] == FT_DATERANGE ) {
								$this->opo_tep->setUnixTimestamps( $va_value[ $va_field_info['START'] ],
									$va_value[ $va_field_info['END'] ] );
							} else {
								$this->opo_tep->setHistoricTimestamps( $va_value[ $va_field_info['START'] ],
									$va_value[ $va_field_info['END'] ] );
							}
							$vs_prop = $this->opo_tep->getText( $pa_options );
						}
						...
						if ( $vb_return_as_link ) {
							$vs_prop = array_shift( caCreateLinksFromText( array( $vs_prop ), $vs_table_name,
								array( $vn_id ) ) );
						}
			...
			case FT_MEDIA:
				...
				foreach ( $pa_value_list as $vn_locale_id => $va_values ) {
					...
					foreach ( $va_values as $vn_i => $va_value ) {
						...
						$o_media_settings = new MediaProcessingSettings( $va_path_components['table_name'],
							$va_path_components['field_name'] );
						...
						$vs_info_element = ( $va_path_components['num_components'] == 4 )
							? $va_path_components['components'][3] : null;

						if ( $pa_options['unserialize'] ) {
							$va_return_values[ $vn_id ][ $vm_locale_id ]
								= caUnserializeForDatabase( $va_value[ $va_path_components['field_name'] ] );
						} elseif ( $vs_info_element && ( ! in_array( $vs_info_element, [ 'url', 'path', 'tag' ] ) ) ) {
							if ( in_array( strtolower( $vs_info_element ),
								[ 'original_filename', 'originalfilename', 'filename' ] )
							) {
								$media_info                                  = $this->getMediaInfo( $va_path_components['table_name']
								                                                                    . '.'
								                                                                    . $va_path_components['field_name'],
									null, null, $pa_options );
								$va_return_values[ $vn_id ][ $vm_locale_id ] = caGetOption( 'ORIGINAL_FILENAME',
									$media_info, pathinfo( $this->getMediaPath( $va_path_components['table_name'] . '.'
									                                            . $va_path_components['field_name'],
										'original', $pa_options ), PATHINFO_BASENAME ) );
							} elseif ( in_array( $e = strtolower( $vs_info_element ),
								[ 'mimetype', 'filemodificationtime' ] )
							) {
								$media_info = $this->getMediaInfo( $va_path_components['table_name'] . '.'
								                                   . $va_path_components['field_name'],
									$va_path_components['subfield_name'] ? $va_path_components['subfield_name']
										: 'original', null, $pa_options );
								switch ( $e ) {
									default:
									case 'mimetype':
										$k = "MIMETYPE";
								...
								$va_return_values[ $vn_id ][ $vm_locale_id ]
									= $this->getMediaInfo( $va_path_components['table_name'] . '.'
									                       . $va_path_components['field_name'], $vs_version,
									$vs_info_element, $pa_options );
							}
						} elseif ( ( isset( $pa_options['returnURL'] ) && ( $pa_options['returnURL'] ) )
						           || ( $vs_info_element == 'url' )
						) {
							$va_return_values[ $vn_id ][ $vm_locale_id ]
								= $this->getMediaUrl( $va_path_components['table_name'] . '.'
								                      . $va_path_components['field_name'], $vs_version, $pa_options );
						} elseif ( ( isset( $pa_options['returnPath'] ) && ( $pa_options['returnPath'] ) )
						           || ( $vs_info_element == 'path' )
						) {
							$va_return_values[ $vn_id ][ $vm_locale_id ]
								= $this->getMediaPath( $va_path_components['table_name'] . '.'
								                       . $va_path_components['field_name'], $vs_version, $pa_options );
						} else {
							$va_return_values[ $vn_id ][ $vm_locale_id ]
								= $this->getMediaTag( $va_path_components['table_name'] . '.'
								                      . $va_path_components['field_name'], $vs_version, $pa_options );
						}
					}
				}
			...
			case FT_VARS:
				foreach ( $pa_value_list as $vn_locale_id => $va_values ) {
					foreach ( $va_values as $vn_i => $va_value ) {
						...
						if ( ( isset( $d['EXIF'] ) || isset( $d['XMP'] ) || isset( $d['IFD0'] ) )
						     && ( in_array( strtolower( $va_path_components['subfield_name'] ), [
								'title',
								'description',
								'creator',
								'subjects',
					...
					foreach ( $va_values as $vn_i => $va_value ) {
						...
						if ( $pa_options['unserialize'] ) {
							...
							if ( is_array( $vs_prop ) && $va_path_components['subfield_name'] ) {
								$vs_prop = isset( $vs_prop[ $va_path_components['subfield_name'] ] )
									? $vs_prop[ $va_path_components['subfield_name'] ] : null;
							}
						}

						if ( $pa_options['convertCodesToDisplayText'] ) {
							$vs_prop = $this->_convertCodeToDisplayText( $vs_prop, $va_path_components, $pt_instance,
								$pa_options );
						} elseif ( $pa_options['convertCodesToIdno'] ) {
							$vs_prop = $this->_convertCodeToIdno( $vs_prop, $va_path_components, $pt_instance,
								$pa_options );
						} elseif ( $pa_options['convertCodesToValue'] ) {
							$vs_prop = $this->_convertCodeToValue( $vs_prop, $va_path_components, $pt_instance,
								$pa_options );
						}
	 ...
	 *                          makeFirstUpper = Force first character of all values to upper case. [Default is false]
	 *                          trim = Trim white space from beginning and end of string. [Default is false]
	 *                          start = Return all values trimmed to start at the specified character. [Default is
	 *                          null]
	 *                          length = Return all values truncated to a maximum length. [Default is null]
	 *                          truncate = Return all values from the beginning truncated to a maximum length;
	 *                          equivalent of passing start=0 and length. [Default is null] ellipsis = Add ellipsis
	 *                          ("...") to truncated values. Values will be set to the truncated length including the
	 *                          ellipsis. Eg. a value truncated to 12 characters will include 9 characters of text and
	 *                          3 characters of ellipsis. [Default is false] sort = Sort returned values. [Default is
	 *                          false] sortDirection = Direction of sort. Values are ASC (ascending) or DESC
		...
		if ( $pa_options['returnAllLocales'] ) {
			...
			foreach ( $pa_array as $va_by_attr ) {
				...
				foreach ( $va_by_attr as $vs_val ) {
					...
					$vn_start  = ( strlen( $pa_options['start'] ) && is_numeric( $pa_options['start'] ) )
						? (int) $pa_options['start'] : 0;
					$vn_length = ( strlen( $pa_options['length'] ) && ( $pa_options['length'] > 0 ) )
						? (int) $pa_options['length'] : null;
	...
	public function getWithTemplate( $ps_template, $pa_options = null ) {
		...
		return caProcessTemplateForIDs( $ps_template, $this->ops_table_name,
			array( $this->get( $this->ops_table_name . "." . $this->ops_subject_pk ) ),
			array_merge( $pa_options, [ 'dontPrefetchRelated' => true ] ) );
		//}
		...
		if ( ! isset( $this->opa_template_prefetch_cache[ $vs_cache_base_key ][ $vn_cur_row
				= $this->opo_engine_result->currentRow() ] )
		) {
	...
	private function prefetchForGetWithTemplate( $ps_template, $pa_options ) {
		...
		$pa_options['returnAsArray']
		                   = true; // careful, this would change the cache key ... which is why we generate it before
		$pa_options['includeBlankValuesInTopLevelForPrefetch']
		                   = true; // if we don't set this blank values are omitted and array offsets following a blank value will be incorrect. A recipe for a bad day.
	...
	public function getWithTemplateForResults( $ps_template, $pa_options = null ) {
		...
		return caProcessTemplateForIDs( $ps_template, $this->ops_table_name,
			$this->getRowIDsToPrefetch( $pn_start, $this->numHits() ),
	...
	private function _convertCodeToIdno( $ps_prop, $pa_path_components, $pt_instance, $pa_options = null ) {
		...
		$vs_field_name = $pa_path_components['subfield_name'] ? $pa_path_components['subfield_name']
			: $pa_path_components['field_name'];

		$vs_table_name = $pa_path_components['table_name'];
		if ( method_exists( $pt_instance, 'setLabelTypeList' ) ) {
			$pt_instance->setLabelTypeList( $this->opo_subject_instance->getAppConfig()
			                                                           ->get( ( $pa_path_components['field_name']
			                                                                    == 'nonpreferred_labels' )
				                                                           ? "{$vs_table_name}_nonpreferred_label_type_list"
				                                                           : "{$vs_table_name}_preferred_label_type_list" ) );
		}
		if ( isset( $pa_options['convertCodesToIdno'] ) && $pa_options['convertCodesToIdno']
		     && ( $vs_list_code
				= $pt_instance->getFieldInfo( $vs_field_name, "LIST_CODE" ) )
		) {
			$vs_prop = caGetListItemIdno( $vs_prop );
		} elseif ( isset( $pa_options['convertCodesToIdno'] ) && $pa_options['convertCodesToIdno']
		           && ( $vs_list_code
				= $pt_instance->getFieldInfo( $vs_field_name, "LIST" ) )
		) {
			$vs_prop = caGetListItemIdno( caGetListItemIDForValue( $vs_list_code, $vs_prop ) );
		} elseif ( $vb_convert_codes_to_display_text && method_exists( $pt_instance, "isRelationship" )
		           && $pt_instance->isRelationship()
		           && ( $vs_field_name == 'type_id' )
		) {
	...
	private function _convertCodeToValue( $ps_prop, $pa_path_components, $pt_instance, $pa_options = null ) {
		...
		if ( isset( $pa_options['convertCodesToValue'] ) && $pa_options['convertCodesToValue']
		     && ( $vs_list_code
				= $pt_instance->getFieldInfo( $vs_field_name, "LIST_CODE" ) )
		) {
			$vs_prop = caGetListItemValueForID( $vs_prop );
		} elseif ( isset( $pa_options['convertCodesToValue'] ) && $pa_options['convertCodesToValue']
		           && ( $vs_list_code
				= $pt_instance->getFieldInfo( $vs_field_name, "LIST" ) )
		) {
			return $vs_prop;
	...
	private function _convertCodeToDisplayText( $ps_prop, $pa_path_components, $pt_instance, $pa_options = null ) {
		...
		if ( method_exists( $pt_instance, 'setLabelTypeList' )
		     && ( $vs_label_list_name
				= $this->opo_subject_instance->getAppConfig()->get( ( $pa_path_components['field_name']
				                                                      == 'nonpreferred_labels' )
				? "{$vs_table_name}_nonpreferred_label_type_list" : "{$vs_table_name}_preferred_label_type_list" ) )
		) {
			$pt_instance->setLabelTypeList( $vs_label_list_name );
		}

		if ( $vb_convert_codes_to_display_text
		     && ( $vs_list_code = $pt_instance->getFieldInfo( $vs_field_name, "LIST_CODE" ) )
		) {
			$vs_prop = SearchResult::$opt_list->getItemFromListForDisplayByItemID( $vs_list_code, $vs_prop );
		} elseif ( $vb_convert_codes_to_display_text
		           && ( $vs_list_code = $pt_instance->getFieldInfo( $vs_field_name, "LIST" ) )
		) {
			$vs_prop = SearchResult::$opt_list->getItemFromListForDisplayByItemValue( $vs_list_code, $vs_prop );
		} elseif ( $vb_convert_codes_to_display_text
		           && is_array( $options = $pt_instance->getFieldInfo( $vs_field_name, "OPTIONS" ) )
		           && ( ( $k = array_search( $vs_prop, $options ) ) !== false )
		) {
			$vs_prop = $k;
			...
			$vs_prop  = $t_locale->getName();
		           ...
		           && ( is_array( $va_list = $pt_instance->getFieldInfo( $vs_field_name, "BOUNDS_CHOICE_LIST" ) ) )
		) {
	 ...
	 * Typically used to determine if a result set can be used for visualization (Eg. does a result set have mappable
	 * data?)
	 *
	 * @param mixed $pa_fields  Field or list of fields to check
	 * @param array $pa_options Options include:
	 *                          limit = number of rows to check before giving up; should be capped at a reasonable
	 *                          value for large empty result sets to avoid timeouts [Default=10000]
	 *
	...
	public function hasData( $pa_fields, $pa_options = null ) {
		...
		foreach ( $pa_fields as $vn_i => $vs_field ) {
			...
			if ( ! $t_instance->hasField( $va_tmp[1] )
			     && ( ! $t_instance->hasElement( $va_tmp[1], null, false, array( 'dontCache' => false ) ) )
			) {
				unset( $pa_fields[ $vn_i ] );
			}
		}
	...
	# ------------------------------------------------------------------
	/**
	 * Fetches an array of information about the specified bundle. Information includes the table name, fields name
	 * and, for intrinsics a model instance.
	 *
	...
	function getMediaInfo( $ps_field, $ps_version = null, $ps_key = null, $pa_options = null ) {
		...
		$vn_index = ( isset( $pa_options['index'] ) && ( (int) $pa_options['index'] > 0 ) ) ? (int) $pa_options['index']
			: 0;

		if ( ! $this->opa_field_media_info[ $ps_field ] ) {
			$this->opa_field_media_info[ $ps_field ] = $this->get( $ps_field,
				array( "unserialize" => true, 'returnWithStructure' => true ) );
		}

		return $GLOBALS["_DbResult_mediainfocoder"]->getMediaInfo( $ps_version, $ps_key,
			array_merge( $pa_options, [ 'data' => array_shift( $this->opa_field_media_info[ $ps_field ] ) ] ) );
	}
	# ------------------------------------------------------------------
	...
	function getMediaPath( $ps_field, $ps_version, $pa_options = null ) {
		...
		return $GLOBALS["_DbResult_mediainfocoder"]->getMediaPath( $ps_version,
			array_merge( $pa_options, [ 'data' => array_shift( $this->opa_field_media_info[ $ps_field ] ) ] ) );
	}
	# ------------------------------------------------------------------
	...
	function getMediaPaths( $ps_field, $ps_version, $pa_options = null ) {
		...
		if ( is_array( $this->opa_field_media_info[ $ps_field ] )
		     && sizeof( $this->opa_field_media_info[ $ps_field ] )
		) {
			foreach ( $this->opa_field_media_info[ $ps_field ] as $vm_media ) {
				$va_media_paths[] = $GLOBALS["_DbResult_mediainfocoder"]->getMediaPath( $ps_version,
					array_merge( $pa_options, [ 'data' => $vm_media ] ) );
			}
		}
	...
	function getMediaUrl( $ps_field, $ps_version, $pa_options = null ) {
		...
		return $GLOBALS["_DbResult_mediainfocoder"]->getMediaUrl( $ps_version,
			array_merge( $pa_options, [ 'data' => array_shift( $this->opa_field_media_info[ $ps_field ] ) ] ) );
	}
	# ------------------------------------------------------------------
	...
	function getMediaUrls( $ps_field, $ps_version, $pa_options = null ) {
		...
		if ( is_array( $this->opa_field_media_info[ $ps_field ] )
			...
			foreach ( $this->opa_field_media_info[ $ps_field ] as $vm_media ) {
				$va_media_urls[] = $GLOBALS["_DbResult_mediainfocoder"]->getMediaUrl( $ps_version,
					array_merge( $pa_options, [ 'data' => $vm_media ] ) );
			}
		}
	...
	function getMediaTag( $ps_field, $ps_version, $pa_options = null ) {
		...
		return $GLOBALS["_DbResult_mediainfocoder"]->getMediaTag( $ps_version, array_merge( $pa_options,
			[ 'alt' => $alt_text, 'data' => reset( $this->opa_field_media_info[ $ps_field ] ) ] ) );
	}
	# ------------------------------------------------------------------
	...
	function getMediaTags( $ps_field, $ps_version, $pa_options = null ) {
		...
		if ( is_array( $this->opa_field_media_info[ $ps_field ] )
			...
			foreach ( $this->opa_field_media_info[ $ps_field ] as $vm_media ) {
				$va_media_tags[] = $GLOBALS["_DbResult_mediainfocoder"]->getMediaTag( $ps_version,
					array_merge( $pa_options, [ 'data' => $vm_media ] ) );
			}
		}
	...
	/**
	 * Return array of media info arrays attached to this search result. An object can have more than more
	 * representation.
	 *
	 */
	function getMediaInfos( $ps_field ) {
		...
		if ( is_array( $this->opa_field_media_info[ $ps_field ] )
			...
			foreach ( $this->opa_field_media_info[ $ps_field ] as $vm_media ) {
				$va_media_infos[] = $GLOBALS["_DbResult_mediainfocoder"]->getMediaInfo( null, null,
					[ 'data' => $vm_media ] );
			}
		}
	...
	function mediaIsMirrored( $ps_field, $ps_version ) {
		...
		return $GLOBALS["_DbResult_mediainfocoder"]->mediaIsMirrored( $ps_version,
			[ 'data' => reset( $this->opa_field_media_info[ $ps_field ] ) ] );
	}
	# ------------------------------------------------------------------
	...
	function getMediaMirrorStatus( $ps_field, $ps_version, $ps_mirror = null ) {
		...
		return $GLOBALS["_DbResult_mediainfocoder"]->getMediaMirrorStatus( $ps_version, $ps_mirror,
			[ 'data' => reset( $this->opa_field_media_info[ $ps_field ] ) ] );
	}
	# ------------------------------------------------------------------
	...
	function getFileInfo( $ps_field ) {
		...
		return $GLOBALS["_DbResult_fileinfocoder"]->getFileInfo( array_shift( $this->get( $va_field["field"],
			array( "unserialize" => true, 'returnWithStructure' => true ) ) ) );
	}
	# ------------------------------------------------------------------
	...
	function getFilePath( $ps_field ) {
		...
		return $GLOBALS["_DbResult_fileinfocoder"]->getFilePath( array_shift( $this->get( $va_field["field"],
			array( "unserialize" => true, 'returnWithStructure' => true ) ) ) );
	}
	# ------------------------------------------------------------------
	...
	function getFileUrl( $ps_field ) {
		...
		return $GLOBALS["_DbResult_fileinfocoder"]->getFileUrl( array_shift( $this->get( $va_field["field"],
			array( "unserialize" => true, 'returnWithStructure' => true ) ) ) );
	}
	# ------------------------------------------------------------------
	...
	function hasFile( $ps_field ) {
		...
		return $GLOBALS["_DbResult_fileinfocoder"]->hasFile( array_shift( $this->get( $va_field["field"],
			array( "unserialize" => true, 'returnWithStructure' => true ) ) ) );
	}
	# ------------------------------------------------------------------
	...
	function getFileConversions( $ps_field ) {
		...
		return $GLOBALS["_DbResult_fileinfocoder"]->getFileConversions( array(
			$this->get( $va_field["field"], array( "unserialize" => true, 'returnWithStructure' => true ) )
		) );
	}
	# ------------------------------------------------------------------
	...
	function getFileConversionPath( $ps_field, $ps_mimetype ) {
		...
		return $GLOBALS["_DbResult_fileinfocoder"]->getFileConversionPath( array(
			$this->get( $va_field["field"], array( "unserialize" => true, 'returnWithStructure' => true ) )
		), $ps_mimetype );
	}
	# ------------------------------------------------------------------
	...
	function getFileConversionUrl( $ps_field, $ps_mimetype ) {
		...
		return $GLOBALS["_DbResult_fileinfocoder"]->getFileConversionUrl( array_shift( $this->get( $va_field["field"],
			array( "unserialize" => true, 'returnWithStructure' => true ) ) ), $ps_mimetype );
	}
	# ------------------------------------------------------------------
	...
	function getDate( $ps_field, $pa_options = null ) {
		...
		if ( is_object( $va_field["instance"] ) ) {
			if ( ! in_array( $vn_field_type = $va_field["instance"]->getFieldInfo( $va_field["field"], "FIELD_TYPE" ),
				array(
					FT_DATE,
					FT_TIME,
					FT_DATETIME,
					FT_TIMESTAMP,
					FT_HISTORIC_DATETIME,
				...
				case ( FT_HISTORIC_DATERANGE ):
					...
					if ( ! $pa_options["getRawDate"] ) {
						...
						if ( $vn_field_type == FT_HISTORIC_DATERANGE ) {
							$GLOBALS["_DbResult_time_expression_parser"]->setHistoricTimestamps( $this->get( $vs_start_field_name,
								array( "binary" => true ) ),
								$this->get( $vs_end_field_name, array( "binary" => true ) ) );
						} else {
							$GLOBALS["_DbResult_time_expression_parser"]->setUnixTimestamps( $this->get( $vs_start_field_name,
								array( "binary" => true ) ),
								$this->get( $vs_end_field_name, array( "binary" => true ) ) );
						}
						...
						return array(
							$this->get( $vs_start_field_name, array( "binary" => true ) ),
							$this->get( $vs_end_field_name, array( "binary" => true ) )
						);
					}
					break;
	...
	function getChoiceListValue( $ps_field ) {
		...
		if ( is_object( $va_field["instance"] ) ) {
			if ( is_array( $va_field["instance"]->getFieldInfo( $va_field["field"], "BOUNDS_CHOICE_LIST" ) ) ) {
				return $va_field["instance"]->getChoiceListValue( $va_field["field"],
					$this->get( $va_field["field"] ) );
			} else {
		...
		if ( $this->isValidOption( $ps_option ) ) {
			switch ( $ps_option ) {
				case 'prefetchAttributes':
					$this->opa_options[ $ps_option ]
						= array_values( ca_metadata_elements::elementCodesToIDs( $pm_value ) );
					break;
				default:
	 ...
	 *
	 * @param array $pa_field_list List of fields to fetch counts for. Fields should be fully qualified <table>.<field>
	 *                             specifications (eg. ca_objects.type_id)
