...
abstract class ASearchConfigurationSettings
{
    ...
    protected function __construct()
    {
        ...
        if (is_array($this->opa_possible_errors)) {
            ...
            foreach ($this->opa_possible_errors as $vn_setting_num) {
                ...
                $this->opa_settings[$this->opn_current_setting]['description'] = $this->getSettingDescription(
                    $vn_setting_num
                );
    ...
    # ------------------------------------------------
    public abstract function setSettings(); // is supposed to have side-effect on opa_possible_errors property!

    # ------------------------------------------------
    public abstract function checkSetting($pn_setting_num);

    # ------------------------------------------------
    public abstract function getSettingDescription($pn_setting_num);

    public abstract function getSettingName($pn_setting_num);

    public abstract function getSettingHint($pn_setting_num);
    # ------------------------------------------------
}
# ------------------------------------------------
