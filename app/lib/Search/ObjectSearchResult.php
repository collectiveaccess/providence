...
class ObjectSearchResult extends BaseSearchResult {
	 ...
	 *                                              filterNonPrimaryRepresentations = If set only primary
	 *                                              representations are returned. This can improve performance somewhat
	 *                                              in most cases. Default is true.
	 */
	public function init( $po_engine_result, $pa_tables, $pa_options = null ) {
		...
		if ( $pa_options['filterNonPrimaryRepresentations'] ) {
			$va_criteria = array(
				...
				'ca_object_representations.deleted = 0'
			);
		} else {
	...
	public function representationsOfClass( $ps_class, $pa_options = null ) {
		...
		return isset( $this->opa_class_cache[ $this->get( 'object_id' ) ][ $ps_class ] )
			? $this->opa_class_cache[ $this->get( 'object_id' ) ][ $ps_class ] : array();
	}
	# -------------------------------------
	 ...
	 *                            checkAccess = array of access values to restrict returned values to.
	 *
	 * @return array An array with information about matching representations, in the same format as that returned by
	 *               ca_objects::getRepresentations()
	 */
	public function representationsWithMimeType( $ps_mimetype, $pa_options = null ) {
		...
		return isset( $this->opa_mimetype_cache[ $this->get( 'object_id' ) ][ $ps_mimetype ] )
			? $this->opa_mimetype_cache[ $this->get( 'object_id' ) ][ $ps_mimetype ] : array();
	}
	# -------------------------------------
	...
	private function _prefetchMimeTypes( $pa_options ) {
		...
		while ( $qr_res->nextRow() ) {
			$this->opa_mimetype_cache[ (int) $qr_res->get( 'object_id' ) ][ $qr_res->get( 'mimetype' ) ][]
				= (int) $qr_res->get( 'representation_id' );
			$this->opa_class_cache[ (int) $qr_res->get( 'object_id' ) ][ caGetMediaClass( $qr_res->get( 'mimetype' ) ) ][]
				= (int) $qr_res->get( 'representation_id' );
		}
	...
	private function _haveAccessToRepresentation( $pa_access_values ) {
		...
		if ( ! sizeof( array_intersect( $this->get( 'ca_object_representations.access',
			array( 'returnAsArray' => true ) ), $pa_access_values ) )
		) {
			return false;
		}
