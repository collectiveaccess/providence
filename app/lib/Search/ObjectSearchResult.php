...
class ObjectSearchResult extends BaseSearchResult
{
    ...
    public function representationsOfClass($ps_class, $pa_options = null)
    {
        ...
        return isset($this->opa_class_cache[$this->get('object_id')][$ps_class]) ? $this->opa_class_cache[$this->get(
            'object_id'
        )][$ps_class] : array();
    }
    # -------------------------------------
    ...
    public function representationsWithMimeType($ps_mimetype, $pa_options = null)
    {
        ...
        return isset(
            $this->opa_mimetype_cache[$this->get(
                'object_id'
            )][$ps_mimetype]
    ...
    private function _prefetchMimeTypes($pa_options)
    {
        ...
        while ($qr_res->nextRow()) {
            $this->opa_mimetype_cache[(int)$qr_res->get('object_id')][$qr_res->get('mimetype')][] = (int)$qr_res->get(
                'representation_id'
            );
            $this->opa_class_cache[(int)$qr_res->get('object_id')][caGetMediaClass(
                $qr_res->get('mimetype')
    ...
    private function _haveAccessToRepresentation($pa_access_values)
    {
        ...
        if (!sizeof(
            array_intersect(
                $this->get('ca_object_representations.access', array('returnAsArray' => true)),
                $pa_access_values
            )
        )) {
            return false;
