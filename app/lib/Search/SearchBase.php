...
class SearchBase extends BaseFindEngine {
	...
	/**
	 * @param Db     $po_db          A database client object to use rather than creating a new connection. [Default is
	 ...
	 * @param bool   $pb_load_engine if set to true (default is false) we don't attempt to load an engine instance.
	 *                               this is useful if you just want to use SearchBase for the utility methods
	 */
	 ...
	 *
	 * @param string $ps_plugin_name A valid plugin file name (eg. 'ElasticSearch'), not the actual class name (eg.
	...
	static public function newSearchEngine( $ps_plugin_name = null, $po_db = null ) {
		...
		if ( ! file_exists( __CA_LIB_DIR__ . '/Plugins/SearchEngine/' . $ps_plugin_name . '.php' ) ) {
			return null;
		}
	 ...
	 * @param mixed $pm_content_table Name or number of table containing content being indexed. [Default is
	 *                                $pm_subject_table]
	 * @param array $pa_options       Options include:
	 *                                currentValueFields = Return fields for current value indexing instead of standard
	 *                                indexing field set. [Default is false] clearCache = Clear field cache. [Default
	 *                                is false] intrinsicOnly = Return only intrinsic fields. [Default is false]
	 *
	 * @return array
	 */
	public function getFieldsToIndex( $pm_subject_table, $pm_content_table = null, $pa_options = null ) {
		...
		if ( ! ( $va_info = $this->opo_search_indexing_config->getAssoc( $vs_subject_table ) ) ) {
			return SearchBase::$s_fields_to_index_cache[ $pm_subject_table . '/' . $pm_content_table . '/' . $vs_key ]
				=
			SearchBase::$s_fields_to_index_cache[ $vs_subject_table . '/' . $vs_content_table . '/' . $vs_key ] = null;
		}

		if ( $current_value_fields_only = caGetOption( 'currentValueFields', $pa_options, false ) ) {
			$va_fields_to_index = is_array( $va_info[ $vs_content_table ]['current_values'] )
				? $va_info[ $vs_content_table ]['current_values'] : [];
		} else {
		...
		foreach ( $va_fields_to_index as $p => $field_list ) {
			if ( caGetOption( 'intrinsicOnly', $pa_options, false ) ) {
				...
				foreach ( $field_list as $vs_f => $va_data ) {
					...
					if ( ( $vs_start = $t_subject->getFieldInfo( $vs_f, 'START' )
					                   && ( $vs_end
							= $t_subject->getFieldInfo( $vs_f, 'END' ) ) )
					) {
			...
			if ( isset( $field_list['_metadata'] ) ) {
				...
				$va_field_data                = $t_subject->getApplicableElementCodes( null,
					$vb_include_non_root_elements, false );
			...
			if ( is_array( $field_list ) ) {
				foreach ( $field_list as $vs_f => $va_info ) {
					if (
						( ( substr( $vs_f, 0, 14 ) === '_ca_attribute_' )
						  && preg_match( '!^_ca_attribute_([A-Za-z]+[A-Za-z0-9_]*)$!', $vs_f, $va_matches )
						  && ( $element_id = ca_metadata_elements::getElementID( $va_matches[1] ) ) )
			...
			if ( ! $current_value_fields_only && method_exists( $t_subject, 'getTypeFieldName' )
			     && ( $vs_type_field
					= $t_subject->getTypeFieldName() )
			     && ! isset( $field_list[ $vs_type_field ] )
			) {
		...
		return SearchBase::$s_fields_to_index_cache[ $pm_subject_table . '/' . $pm_content_table . '/' . $vs_key ]
			= SearchBase::$s_fields_to_index_cache[ $vs_subject_table . '/' . $vs_content_table . '/' . $vs_key ]
			= $va_fields_to_index;
	}
	# ------------------------------------------------
	...
	public function getHistoryTrackingPoliciesToIndex( $pm_subject_table, $pa_options = null ) {
		...
		return is_array( $va_info[ $vs_subject_table ]['current_values'] )
			? $va_info[ $vs_subject_table ]['current_values'] : [];
	}
	...
	public function getRelatedIndexingTables( $pm_subject_table ) {
		...
		$vs_label_table = null;
		if ( ( $t_instance = Datamodel::getInstanceByTableName( $pm_subject_table, true ) )
		     && ( method_exists( $t_instance, 'getLabelTableName' ) )
		) {
			$vs_label_table = $t_instance->getLabelTableName();
		}

		if ( ! isset( $va_info[ $pm_subject_table ]['related'] )
		     && ( ! $vs_label_table
		          || ! isset( $va_info[ $vs_label_table ]['related'] ) )
		) {
