...
class SearchBase extends BaseFindEngine
{
    ...
    static public $s_fields_to_index_cache = array();
    # ------------------------------------------------
     ...
     */
    public static function newSearchEngine($ps_plugin_name = null, $po_db = null)
    {
        if (!$ps_plugin_name) {
        ...
        foreach ($va_fields_to_index as $p => $field_list) {
            if (caGetOption('intrinsicOnly', $pa_options, false)) {
                ...
                foreach ($field_list as $vs_f => $va_data) {
                    ...
                    if (($vs_start = $t_subject->getFieldInfo($vs_f, 'START') && ($vs_end = $t_subject->getFieldInfo(
                            $vs_f,
                            'END'
                        )))) {
            ...
            if (is_array($field_list)) {
                foreach ($field_list as $vs_f => $va_info) {
                    if (
                        ((substr($vs_f, 0, 14) === '_ca_attribute_') && preg_match(
                                '!^_ca_attribute_([A-Za-z]+[A-Za-z0-9_]*)$!',
                                $vs_f,
                                $va_matches
            ...
            if (!$current_value_fields_only && method_exists(
                    $t_subject,
                    'getTypeFieldName'
                ) && ($vs_type_field = $t_subject->getTypeFieldName()) && !isset($field_list[$vs_type_field])) {
    ...
    public function getRelatedIndexingTables($pm_subject_table)
    {
        ...
        $vs_label_table = null;
        if (($t_instance = Datamodel::getInstanceByTableName($pm_subject_table, true)) && (method_exists(
                $t_instance,
                'getLabelTableName'
            ))) {
