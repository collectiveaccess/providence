...
class BaseSearchResult extends SearchResult {
	...
	/**
	 * Name of field in labels table to use for display for this type of search subject (eg. for ca_objects, the label
	 * display field is 'name')
	 */
	...
	public function __construct( $po_engine_result = null, $pa_tables = null ) {
		...
		$this->ops_label_table_name    = method_exists( $this->opo_subject_instance, "getLabelTableName" )
			? $this->opo_subject_instance->getLabelTableName() : null;
		$this->ops_label_display_field = method_exists( $this->opo_subject_instance, "getLabelDisplayField" )
			? $this->opo_subject_instance->getLabelDisplayField() : null;
	}
	# -------------------------------------------------------
	...
	public function getDisplayLabels( $pb_has_preferred_flag = true ) {
		return $this->get( $this->ops_table_name . '.preferred_labels.' . $this->ops_label_display_field,
	 ...
	 * @param array  $pa_values List of values to filter rows by; only rows where the specified field contains at least
	 *                          one of the values in $pa_values will be returned
	 *
	 * @return void
	 */
