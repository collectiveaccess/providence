...
class LuceneSyntaxParser extends Zend_Search_Lucene_FSM
{
    ...
    public function subqueryStart()
    {
        ...
        $this->opo_context = new Zend_Search_Lucene_Search_QueryParserContext(
            $this->_encoding,
    ...
    public function subqueryEnd()
    {
        if (count($this->opa_context_stack) == 0) {
            require_once __CA_LIB_DIR__ . '/Zend/Search/Lucene/Search/QueryParserException.php';
            throw new Zend_Search_Lucene_Search_QueryParserException(
                'Syntax Error: mismatched parentheses, every opening must have closing. Char position ' . $this->opo_current_token->position . '.'
            );
        }
