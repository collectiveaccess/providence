...
class LuceneSyntaxLexer extends Zend_Search_Lucene_FSM {
	...
	public function addQuerySyntaxLexeme() {
		...
		if ( strpos( self::QUERY_DOUBLECHARLEXEME_CHARS, $lexeme ) !== false ) {
			...
			if ( $this->_queryStringPosition == count( $this->_queryString )
				...
				throw new Zend_Search_Lucene_Search_QueryParserException( 'Two chars lexeme expected. '
				                                                          . $this->_positionMsg() );
			}
		...
		if ( $token->type == Zend_Search_Lucene_Search_QueryToken::TT_FIELD_INDICATOR ) {
			...
			if ( $token === null || $token->type != Zend_Search_Lucene_Search_QueryToken::TT_WORD ) {
				throw new Zend_Search_Lucene_Search_QueryParserException( 'Field mark \':\' must follow field name. '
				                                                          . $this->_positionMsg() );
			}
	...
	public function lexModifierErrException() {
		throw new Zend_Search_Lucene_Search_QueryParserException( 'Lexeme modifier character can be followed only by number, white space or query syntax element. '
		                                                          . $this->_positionMsg() );
	}

	public function quoteWithinLexemeErrException() {
		throw new Zend_Search_Lucene_Search_QueryParserException( 'Quote within lexeme must be escaped by \'\\\' char. '
		                                                          . $this->_positionMsg() );
	}
