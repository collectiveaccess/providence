...
class LanguageDetection {
	...
	function loadFingerprints() {
		...
		if ( $r_dir = opendir( __CA_LIB_DIR__ . '/Search/Common/Language/fingerprints' ) ) {
			...
			while ( false !== ( $vs_filename = readdir( $r_dir ) ) ) {
				if ( is_file( __CA_LIB_DIR__ . '/Search/Common/Language/fingerprints/' . $vs_filename )
				     && ( $vs_filename[0] != '.' )
				) {
					$vs_language = basename( $vs_filename, ".lm" );
					$r_file      = fopen( __CA_LIB_DIR__ . '/Search/Common/Language/fingerprints/' . $vs_filename,
						'r' );
	...
	function createNGrams( $ps_text ) {
		...
		foreach ( $va_words as $vs_word ) {
			...
			for ( $vn_i = 0; $vn_i < $vs_word_size; $vn_i ++ )
			{                            // start position within word
