...
class RepresentationAnnotationSearchResult extends BaseSearchResult
{
    ...
    public function loadProperties($ps_type, $pa_parameters = null)
    {
        ...
        $this->opo_annotations_properties->setPropertyValues(
            is_array($pa_parameters) ? $pa_parameters : array_shift(
                $this->get(
    ...
    public function get($ps_field, $pa_options = null)
    {
        ...
        if ((sizeof($va_tmp) == 3) && ($va_tmp[1] == 'props')) {
            ...
            return $vm_val;
        } elseif ((sizeof($va_tmp) == 2) && in_array(
                $va_tmp[1],
                array('start', 'end', 'duration')
            ) && ($vs_annotation_type = $this->getAnnotationType(
                parent::get('ca_representation_annotations.representation_id')
            ...
            switch ($va_tmp[1]) {
                ...
                case 'end':
                    $vm_val = parent::get(
                        "ca_representation_annotations.props.{$va_tmp[1]}Timecode",
                    ...
                    break;
                case 'duration':
                    $vn_duration_in_seconds = (float)$this->getPropertyValue(
                            'endTimecode',
                            true
