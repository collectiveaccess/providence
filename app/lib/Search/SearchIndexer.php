...
class SearchIndexer extends SearchBase {
	 ...
	 * the case of BaseModel::insert(), BaseModel::update() and BaseModel::delete()they're always in a transactional
	 * context so this is critical. If you don't pass an Db() instance then the constructor creates a new one, which is
	 * useful for cases where you're reindexing and not in a transaction.
	 */
	...
	public function getIndexedTables() {
		...
		foreach ( $va_table_names as $vs_table ) {
			...
			$va_tables_to_index[ $vs_table ] = array(
				'name'        => $vs_table,
				'num'         => $vn_table_num,
				'count'       => $vn_num_rows,
				'displayName' => $t_instance->getProperty( 'NAME_PLURAL' )
			);
	...
	public function reindex( $pa_table_names = null, $pa_options = null ) {
		...
		$pb_interactive_display = isset( $pa_options['interactiveProgressDisplay'] )
			? (bool) $pa_options['interactiveProgressDisplay'] : false;
		...
		foreach ( $va_table_names as $vn_table_num => $va_table_info ) {
			...
			if ( $pb_display_progress ) {
				print CLIProgressBar::start( $vn_num_rows,
					_t( 'Indexing %1', $t_instance->getProperty( 'NAME_PLURAL' ) ) );
			}
			...
			$va_intrinsic_list                 = $this->getFieldsToIndex( $vs_table, $vs_table,
				array( 'intrinsicOnly' => true ) );
		...
		if ( $pb_display_progress ) {
			print _t( "\n\n\nDone! [Indexing for %1 took %2]\n", join( ", ", $va_names ),
				caFormatInterval( (float) $t_timer->getTime( 4 ) ) );
	 ...
	 * @param array $pa_options           An array of options to pass through to SearchIndexer::indexRow(). Normally
	 *                                    omitted.
	 *
	...
	public function reindexRows( $pm_table_name_or_num, $pa_ids, $pa_options = null ) {
		...
		foreach ( $pa_ids as $vn_i => $vn_id ) {
			if ( $va_element_ids
				...
				ca_attributes::prefetchAttributes( $this->getDb(), $vn_table_num,
					$va_id_slice = array_slice( $pa_ids, $vn_i, 200 ), $va_element_ids );
			...
			$this->indexRow( $vn_table_num, $vn_id, $va_field_data[ $vn_id ], false, null,
				array( $vs_table_pk => true ), $pa_options );
	...
	public function getDependencies( $ps_subject_table ) {
		...
		if ( isset( $va_cache_data[ $ps_subject_table ] )
		     && is_array( $va_cache_data[ $ps_subject_table ] )
		) { /* cache hit */
	...
	private function _genHierarchicalPath( $pn_subject_row_id, $ps_field, $t_subject, $pa_options = null ) {
		...
		$va_ids               = $t_subject->getHierarchyAncestors( $pn_subject_row_id,
	 ...
	 *
	 */
	private function _genIndexInheritance(
		$pt_subject, $pt_rel, $ps_field_num, $pn_subject_row_id, $pn_content_row_id, $pa_values_to_index, $pa_data,
		$pa_options = null
	) {
		$is_generic        = caGetOption( 'isGeneric', $pa_options, false );
		$subject_table_num = $pt_subject->tableNum();

		if ( caGetOption( 'CHILDREN_INHERIT', $pa_data, false )
		     || ( array_search( 'CHILDREN_INHERIT', $pa_data, true ) !== false )
		) {
			...
			if ( is_array( $va_ids = $pt_subject->getHierarchy( $pn_subject_row_id, [ 'idsOnly' => true ] ) ) ) {
				foreach ( $va_ids as $vn_id ) {
					...
					$o_indexer->opo_engine->indexField( $is_generic ? $subject_table_num : $vn_rel_table_num,
						$ps_field_num, $pn_content_row_id, $pa_values_to_index, array_merge( $pa_data ) );
					$o_indexer->opo_engine->commitRowIndexing();
				}
			}
		}
		if ( caGetOption( 'ANCESTORS_INHERIT', $pa_data, false )
		     || ( array_search( 'ANCESTORS_INHERIT', $pa_data, true ) !== false )
		) {
			if ( ! $o_indexer ) {
				$o_indexer = new SearchIndexer( $this->opo_db );
			}
			...
			if ( is_array(
				$va_ids = $pt_subject->getHierarchyAncestors( $pn_subject_row_id, [ 'idsOnly' => true ] ) )
			) {
				foreach ( $va_ids as $vn_id ) {
					if ( $vn_id == $pn_subject_row_id ) {
						continue;
	 ...
	 *
	 * "subject" tablenum/row_id refer to the row **to which the indexing is being applied**. This may be the row being
	 * indexed or it may be a dependent row. The "content" tablenum/fieldnum/row_id parameters define the specific row
	 * and field being indexed. This is always the actual row being indexed. $pm_content is the content to be indexed
	 * and $pa_options is an optional associative array of indexing options passed through from the
	 * search_indexing.conf (no options are defined yet - but will be soon)
	 *
	 ...
	 *                                         (to prevent endless recursive reindexing). Should always be null when
	 *                                         called externally.
	 ...
	 * @throws Exception
	 */
	public function indexRow(
		$pn_subject_table_num, $pn_subject_row_id, $pa_field_data, $pb_reindex_mode = false, $pa_exclusion_list = null,
		$pa_changed_fields = null, $pa_options = null
	) {
		$vb_initial_reindex_mode   = $pb_reindex_mode;
		$for_current_value_reindex = caGetOption( 'forCurrentValueReindex', $pa_options, false );
		if ( ! $pb_reindex_mode && ! $for_current_value_reindex && is_array( $pa_changed_fields )
		     && ! sizeof( $pa_changed_fields )
		) {
			return;
		...
		if ( is_array( $pa_exclusion_list[ $pn_subject_table_num ] )
		     && ( isset( $pa_exclusion_list[ $pn_subject_table_num ][ $pn_subject_row_id ] ) )
		) {
			return;
		}

		if ( caGetOption( 'queueIndexing', $pa_options, false )
		     && ! $t_subject->getAppConfig()->get( 'disable_out_of_process_search_indexing' )
		) {
		...
		$vb_can_do_incremental_indexing = $this->opo_engine->can( 'incremental_reindexing' ) ? true
			: false;        // can the engine do incremental indexing? Or do we need to reindex the entire row every time?
		...
		if ( ! $for_current_value_reindex ) {
			...
			if ( is_array( $va_fields_to_index ) ) {
				...
				if ( method_exists( $t_subject, 'getTypeFieldName' )
				     && ( $vs_type_field
			...
			//
			if ( $t_subject->isHierarchical() && isset( $pa_changed_fields['parent_id'] )
			     && $pa_changed_fields['parent_id']
			     && method_exists( $t_subject, "makeSearchResult" )
			) {
			...
			if ( is_array( $va_fields_to_index ) ) {
				...
				foreach ( $va_fields_to_index as $vs_field => $va_data ) {
					...
					if ( substr( $vs_field, 0, 14 ) === '_ca_attribute_' ) {
						...
						if ( $vb_can_do_incremental_indexing && ( ! $pb_is_new_row ) && ( ! $pb_reindex_mode )
						     && ( ! isset( $pa_changed_fields[ $vs_field ] ) || ! $pa_changed_fields[ $vs_field ] )
						) {
						...
						if ( $va_data['DONT_INDEX'] && is_array( $va_data['DONT_INDEX'] ) ) {
							...
							foreach ( $va_data["DONT_INDEX"] as $vs_exclude_type ) {
								if ( ca_metadata_elements::getElementID( $vs_exclude_type )
								     == intval( $va_matches[1] )
								) {
									$vb_cont = true;
									break;
						...
						if ( $va_data['datatype'] === 0 ) {
							...
							if ( is_array( $child_ids = ca_metadata_elements::getElementsForSet( $va_matches[1],
								[ 'idsOnly' => true ] ) )
							) {
								foreach ( $child_ids as $child_id ) {
									...
									$va_data[ '_ca_attribute_' . $child_id ] = $va_fields_to_index[ '_ca_attribute_'
									                                                                . $child_id ];
								}
							}
						}
						$this->_indexAttribute( $t_subject, $pn_subject_row_id, $va_matches[1], $va_data,
							[ 'reindex' => $pb_reindex_mode ] );
						...
						//
						if ( $vb_can_do_incremental_indexing && ( ! $pb_is_new_row ) && ( ! $pb_reindex_mode )
						     && ( ! isset( $pa_changed_fields[ $vs_field ] ) )
						     && ( $vs_field != $vs_subject_pk )
						) {    // skip unchanged
							continue;
						}
						...
						//
						if ( ( ( isset( $va_data['INDEX_ANCESTORS'] ) && $va_data['INDEX_ANCESTORS'] )
						       || in_array( 'INDEX_ANCESTORS', $va_data, true ) )
						) {
							if ( $t_subject && $t_subject->isHierarchical() ) {
								$vn_fld_num = $t_subject->fieldNum( $vs_field );
								if ( $va_hier_values = $this->_genHierarchicalPath( $pn_subject_row_id, $vs_field,
									$t_subject, $va_data )
								) {
									$this->opo_engine->indexField( $pn_subject_table_num, "I{$vn_fld_num}",
										$pn_subject_row_id, $va_hier_values['values'], $va_data );
									$this->_genIndexInheritance( $t_subject, null, "I{$vn_fld_num}", $pn_subject_row_id,
										$pn_subject_row_id, $va_hier_values['values'], $va_data );

									if ( caGetOption( 'INDEX_ANCESTORS_AS_PATH_WITH_DELIMITER', $va_data, false )
									     !== false
									) {
										$this->opo_engine->indexField( $pn_subject_table_num, "I{$vn_fld_num}",
											$pn_subject_row_id, [ $va_hier_values['path'] ],
											array_merge( $va_data, array( 'DONT_TOKENIZE' => 1 ) ) );
										$this->opo_engine->indexField( $pn_subject_table_num, "I{$vn_fld_num}",
											$pn_subject_row_id, [ $va_hier_values['path'] ],
											array_merge( $va_data, array( 'TOKENIZE' => 1 ) ) );
										$this->_genIndexInheritance( $t_subject, null, "I{$vn_fld_num}",
											$pn_subject_row_id, $pn_subject_row_id, [ $va_hier_values['path'] ],
											array_merge( $va_data, array( 'DONT_TOKENIZE' => 1 ) ) );
										$this->_genIndexInheritance( $t_subject, null, "I{$vn_fld_num}",
											$pn_subject_row_id, $pn_subject_row_id, [ $va_hier_values['path'] ],
											array_merge( $va_data, array( 'TOKENIZE' => 1 ) ) );
								...
								$va_children_ids = $t_subject->getHierarchyAsList( $pn_subject_row_id,
									array( 'idsOnly' => true ) );

								if ( ! $pb_reindex_mode && is_array( $va_children_ids )
								     && sizeof( $va_children_ids ) > 0
								) {
									// trigger reindexing of children
									$o_indexer       = new SearchIndexer( $this->opo_db );
									$qr_children_res = $t_subject->makeSearchResult( $vs_subject_tablename,
										$va_children_ids, array( 'db' => $this->getDb() ) );
									while ( $qr_children_res->nextHit() ) {
										$o_indexer->indexRow( $pn_subject_table_num,
											$qr_children_res->get( $vs_subject_pk ), array(
												'parent_id' => $qr_children_res->get( 'parent_id' ),
												$vs_field   => $qr_children_res->get( $vs_field )
											), false, $pa_exclusion_list, array( $vs_field => true ) );
									}
								}
								continue;
							}
						...
						if ( ( ( isset( $va_data['INDEX_AS_MIMETYPE'] ) && $va_data['INDEX_AS_MIMETYPE'] )
							...
							$this->opo_engine->indexField( $pn_subject_table_num, "I{$vn_fld_num}", $pn_subject_row_id,
								[ $pa_field_data[ $vs_field ] ],
								array_merge( $va_data, array( 'DONT_TOKENIZE' => true ) ) );
						...
						if ( in_array( $va_field_list[ $vs_field ]['FIELD_TYPE'],
							array( FT_DATERANGE, FT_HISTORIC_DATERANGE ) )
						) {
							...
							if ( isset( $va_field_list[ $vs_field ]['LIST_CODE'] )
							     && $va_field_list[ $vs_field ]['LIST_CODE']
							) {
								// Is reference to list item so index preferred label values
								$t_item = new ca_list_items( (int) $pa_field_data[ $vs_field ] );
								$va_labels
								        = $t_item->getPreferredDisplayLabelsForIDs( array( (int) $pa_field_data[ $vs_field ] ),
									array( 'returnAllLocales' => true ) );
								...
								if ( is_array( $va_rels = Datamodel::getManyToOneRelations( $vs_subject_tablename ) )
								     && ( $va_rels[ $vs_field ] )
								) {
									if ( isset( $va_rels[ $vs_field ] ) ) {
										if ( $pa_changed_fields[ $vs_field ] ) {
											$pb_reindex_mode
												= true;    // trigger full reindex of record so it reflects text of related item (if so indexed)
										}
									}
								}
							}
							...
							$this->opo_engine->indexField( $pn_subject_table_num, "I{$vn_fld_num}", $pn_subject_row_id,
								array_keys( $va_content ), $va_data );
							$this->_genIndexInheritance( $t_subject, null, "I{$vn_fld_num}", $pn_subject_row_id,
								$pn_subject_row_id, array_keys( $va_content ), $va_data );
							continue;
						}
						...
						$this->opo_engine->indexField( $pn_subject_table_num, "I{$vn_fld_num}", $pn_subject_row_id,
							[ $pn_content ], $va_data );
						$this->_genIndexInheritance( $t_subject, null, "I{$vn_fld_num}", $pn_subject_row_id,
							$pn_subject_row_id, [ $pn_content ], $va_data );
					}
				}
			}
		}
		...
		if ( ! $vb_can_do_incremental_indexing || $pb_reindex_mode || $for_current_value_reindex ) {
			if ( is_array( $va_related_tables = $this->getRelatedIndexingTables( $pn_subject_table_num ) ) ) {
				...
				if ( is_array( $va_self_info['related']['fields'] ) && sizeof( $va_self_info['related']['fields'] )
				     && ! in_array( $vs_subject_tablename, $va_related_tables )
				) {
				...
				if ( is_array( $va_self_info['related']['types'] ) && sizeof( $va_self_info['related']['types'] ) ) {
					$va_restrict_self_indexing_to_types = caMakeTypeIDList( $vs_subject_tablename,
						$va_self_info['related']['types'] );
				}
				...
				foreach ( $va_related_tables as $vs_related_table ) {
					...
					$va_info                       = $this->getTableIndexingInfo( $vs_subject_tablename,
						$vb_force_related ? "{$vs_related_table}.related" : $vs_related_table );
					...
					// Get current values
					$current_value_ids    = [];
					$current_history_type = ( is_a( $t_rel, "BaseLabel" ) ) ? $t_rel->getSubjectTableName()
						: $vs_related_table;    // for labels use the subject table as type
					if ( is_array( $policies = $va_info['current_values'] ) ) {
						foreach ( $policies as $p => $pinfo ) {
							$history = $t_subject->getHistory( [
								'policy' => $p,
								'limit'  => 1,
								'row_id' => $pn_subject_row_id
							] ); //'currentOnly' => true,
					...
					$va_query_info         = $this->_getQueriesForRelatedRows( $t_subject, $pn_subject_row_id, $t_rel,
						$pb_reindex_mode, [
							'forceRelated'           => $vb_force_related,
							'restrictToTypes'        => $va_restrict_indexing_to_types,
							'forCurrentValueReindex' => $for_current_value_reindex
						] );
					...
					if ( $vb_index_count = ( isset( $va_fields_to_index['_count'] )
					                         && is_array( $va_fields_to_index['_count'] ) )
					) {
						$va_counts = $this->_getInitedCountList( $t_rel );
					}

					foreach ( $va_queries as $vn_i => $va_query ) {
						$va_linking_table_config = is_array( $va_query_info['linking_table_config_per_query'][ $vn_i ] )
							? $va_query_info['linking_table_config_per_query'][ $vn_i ] : [];
						...
						foreach ( $va_linking_table_config as $vs_linking_table => $va_linking_config ) {
							if ( is_array( $va_linking_config ) && sizeof( $va_linking_config )
							     && isset( $va_linking_config['PRIVATE'] )
							     && Datamodel::isRelationship( $vs_linking_table )
							) {
								$va_private_rel_types = caMakeRelationshipTypeIDList( $vs_linking_table,
									$va_linking_config['PRIVATE'], [] );
								break;
							}
						}
						...
						if ( $this->opo_db->numErrors() ) {
							...
							throw new Exception( _t( "SQL error while getting content for index of related fields: %1; SQL was %2",
								$this->opo_db->getErrors(), $vs_sql ) );
						}

						if ( method_exists( $t_rel, "getApplicableElementCodes" ) ) {
							if ( is_array( $va_element_ids = array_keys( $t_rel->getApplicableElementCodes( null, false,
									false ) ) )
							     && sizeof( $va_element_ids )
							) {
								$va_rel_row_ids = $qr_res->getAllFieldValues( $vs_related_pk,
									[ 'limit' => ca_attributes::attributeCacheSize() ] ); // only pull as many rows as the cache can hold, anything more than that you're just wasting time

								if ( sizeof( $va_rel_row_ids ) > 0 ) {
									ca_attributes::prefetchAttributes( $this->opo_db, $vn_related_table_num,
										$va_rel_row_ids, $va_element_ids );
								}
							}
						}
						...
						while ( $qr_res->nextRow() ) {
							...
							if ( is_a( $t_rel, "BaseLabel" ) ) {
								$vn_rel_subject_row_id
									= $qr_res->get( $t_rel->getSubjectTableInstance( [ 'dontLoadInstance' => true ] )
									                      ->primaryKey() );
							}
							...
							if ( ! $vb_force_related && ( $vs_related_table == $vs_subject_tablename )
							     && is_array( $va_restrict_self_indexing_to_types )
							     && ! in_array( $vn_row_type_id, $va_restrict_self_indexing_to_types )
							) {
								continue;
							}
							if ( is_array( $va_restrict_indexing_to_types )
							     && ! in_array( $vn_row_type_id, $va_restrict_indexing_to_types )
							) {
								continue;
							}

							$vn_private = ( ( ! is_array( $va_private_rel_types ) || ! sizeof( $va_private_rel_types )
							                  || ! in_array( $vn_rel_type_id, $va_private_rel_types ) ) ) ? 0 : 1;

							$fields_to_index_list = [
								'fields'         => $va_fields_to_index,
								'current_values' => $va_cv_fields_to_index
							];
							if ( $for_current_value_reindex ) {
							...
							foreach ( $fields_to_index_list as $m => $field_lists ) {
								...
								foreach ( $field_lists as $p => $field_list ) {
									if ( $m == 'current_values' ) {
										if ( ! isset( $current_value_ids[ $vn_rel_subject_row_id ] )
										     || ! in_array( $p, $current_value_ids[ $vn_rel_subject_row_id ] )
										) {
											continue;
										}
									}
									foreach ( $field_list as $vs_rel_field => $va_rel_field_info ) {
										if ( is_array( $va_rel_field_info['BOOST'] ) ) {
											if ( isset( $va_rel_field_info['BOOST'][ $vs_subject_type_code ] ) ) {
												$va_rel_field_info['BOOST']
													= $va_rel_field_info['BOOST'][ $vs_subject_type_code ];
										...
										if ( substr( $vs_rel_field, 0, 14 ) === '_ca_attribute_' ) {
											if ( ! preg_match( '!^_ca_attribute_(.*)$!', $vs_rel_field,
												$va_matches )
											) {
												continue;
											}

											if ( $va_rel_field_info['DONT_INDEX']
											     && is_array( $va_rel_field_info['DONT_INDEX'] )
											) {
												$vb_cont = false;
											...
											$va_rel_field_info['datatype']
												= (int) ca_metadata_elements::getElementDatatype( $va_matches[1] );

											$va_data = array_merge( $va_rel_field_info, [
												'PRIVATE'              => $vn_private,
											...
											if ( $va_rel_field_info['datatype'] === 0 ) {
												...
												if ( is_array( $child_ids
													= ca_metadata_elements::getElementsForSet( $va_matches[1],
													[ 'idsOnly' => true ] ) )
												) {
													foreach ( $child_ids as $child_id ) {
														if ( ! is_array( $field_list[ '_ca_attribute_'
														                              . $child_id ] )
														) {
															continue;
														}
														$va_data[ '_ca_attribute_' . $child_id ]
															= $field_list[ '_ca_attribute_' . $child_id ];
													}
												}
											}
											$this->_indexAttribute( $t_rel, $vn_row_id, $va_matches[1], $va_data, [
												'reindex'                => $pb_reindex_mode,
												't_inheritance_subject'  => $t_subject,
												'inheritance_subject_id' => $pn_subject_row_id,
												'currentValues'          => ( $m == 'current_values' ),
												'policy'                 => $p
											] );
										}
										...
										//
										if ( ( ( isset( $va_rel_field_info['INDEX_ANCESTORS'] )
										         && $va_rel_field_info['INDEX_ANCESTORS'] )
										       || in_array( 'INDEX_ANCESTORS', $va_rel_field_info, true ) )
										) {
											...
											if ( $t_hier_rel
											     && ( $t_hier_rel->isHierarchical()
											          || is_subclass_of( $t_hier_rel, "BaseLabel" ) )
											) {
												// get hierarchy
												if ( $va_hier_values = $this->_genHierarchicalPath( $vn_id,
													$vs_rel_field, $t_hier_rel, $va_rel_field_info )
												) {
													foreach ( $field_nums as $field_num => $is_generic ) {
														$this->opo_engine->indexField( $is_generic
															? $pn_subject_table_num : $vn_related_table_num, $field_num,
															$is_generic ? $pn_subject_row_id : $vn_id,
															array_unique( array_merge( [ $vs_fld_data ],
																$va_hier_values['values'] ) ),
															array_merge( $va_rel_field_info, array(
																'relationship_type_id' => $vn_rel_type_id,
																'PRIVATE'              => $vn_private
															) ) );
														$this->_genIndexInheritance( $t_subject, $t_hier_rel,
															$field_num, $pn_subject_row_id,
															$is_generic ? $pn_subject_row_id : $vn_id,
															array_merge( [ $vs_fld_data ], $va_hier_values['values'] ),
															array_merge( $va_rel_field_info, array(
																'relationship_type_id' => $vn_rel_type_id,
																'PRIVATE'              => $vn_private,
														...
														if ( caGetOption( 'INDEX_ANCESTORS_AS_PATH_WITH_DELIMITER',
																$va_rel_field_info, false ) !== false
														) {
															$this->opo_engine->indexField( $is_generic
																? $pn_subject_table_num : $vn_related_table_num,
																$field_num, $is_generic ? $pn_subject_row_id : $vn_id,
																[ $va_hier_values['path'] ],
																array_merge( $va_rel_field_info, array(
																	'DONT_TOKENIZE'        => 1,
																	'relationship_type_id' => $vn_rel_type_id,
												...
												if ( strlen( $vn_fn = Datamodel::getFieldNum( $vs_related_table,
													...
													foreach ( $field_nums as $field_num => $is_generic ) {
														if ( ( ( isset( $va_rel_field_info['INDEX_AS_IDNO'] )
														       ...
														       || in_array( 'INDEX_AS_IDNO', $va_rel_field_info,
																	true ) )
															...
															$this->opo_engine->indexField( $is_generic
																? $pn_subject_table_num : $vn_related_table_num,
																$field_num, $vn_id = $is_generic ? $pn_subject_row_id
														...
														if ( ( ( isset( $va_rel_field_info['INDEX_AS_MIMETYPE'] )
															...
															$va_values = [];
															if ( $vs_typename
																= Media::getTypenameForMimetype( $vs_fld_data )
															) {
																$va_values[] = $vs_typename;
															}
															// Index mimetype as-is
															$this->opo_engine->indexField( $is_generic
																? $pn_subject_table_num : $vn_related_table_num,
																$field_num, $vn_id = $is_generic ? $pn_subject_row_id
																	: $qr_res->get( $vs_related_pk ), [ $vs_fld_data ],
																array_merge( $va_rel_field_info, array(
																	'relationship_type_id' => $vn_rel_type_id,
																	'PRIVATE'              => $vn_private,
																	'DONT_TOKENIZE'        => true
															...
															// regular intrinsic
															$this->opo_engine->indexField(
																$z = ( $is_generic ? $pn_subject_table_num
																	: $vn_related_table_num ), $field_num,
																$vn_rid = $is_generic ? $pn_subject_row_id
																	: $qr_res->get( $vs_related_pk ), [ $vs_fld_data ],
																array_merge( $va_rel_field_info, array(
																	'relationship_type_id' => $vn_rel_type_id,
																	'PRIVATE'              => $vn_private
															...
															$this->_genIndexInheritance( $t_subject, $t_rel, $field_num,
																$pn_subject_row_id, $vn_rid, [ $vs_fld_data ],
																array_merge( $va_rel_field_info, array(
																	'relationship_type_id' => $vn_rel_type_id,
																	'PRIVATE'              => $vn_private,
																	'isGeneric'            => $is_generic
							...
							if ( ! $for_current_value_reindex ) {
								...
								if ( ! $vb_force_related && ( $vs_subject_tablename == $vs_related_table ) ) {
									if ( $t_label = $t_rel->getLabelTableInstance() ) {
										...
										$va_label_info = $this->getTableIndexingInfo( $vs_subject_tablename,
											$t_label->tableName() );
										if ( is_array( $va_label_info['related']['fields'] )
										     && sizeof( $va_label_info['related']['fields'] )
										) {
											$vn_label_table_num = $t_label->tableNum();

											$vb_skip = false;
											if ( is_array( $va_restrict_self_indexing_to_types )
											     && $t_rel->load( $vn_row_id )
											     && ! in_array( $t_rel->getTypeID(),
													$va_restrict_self_indexing_to_types )
											) {
												$vb_skip = true;
											}

											if ( ! $vb_skip
											     && is_array( $va_labels = $t_rel->getPreferredLabels( null, false,
													array( 'row_id' => $vn_row_id ) ) )
											) {
												foreach ( $va_labels as $vn_x => $va_labels_by_locale ) {
													foreach ( $va_labels_by_locale as $vn_locale_id => $va_label_list )
													{
														foreach ( $va_label_list as $va_label ) {
															...
															foreach (
																$va_label_info['related']['fields'] as $vs_label_field
															=> $va_config
															) {
																$this->opo_engine->indexField( $vn_label_table_num,
																	'I' . ( $vn_fn
																		= Datamodel::getFieldNum( $vn_label_table_num,
																		$vs_label_field ) ), $vn_label_id,
																	[ $va_label[ $vs_label_field ] ],
																	array_merge( $va_config, array(
																		'relationship_type_id' => $vn_rel_type_id,
																		'PRIVATE'              => $vn_private
																	) ) );
																$this->_genIndexInheritance( $t_subject, $t_label,
																	"I{$vn_fn}", $pn_subject_row_id, $vn_label_id,
																	[ $va_label[ $vs_label_field ] ],
																	array_merge( $va_config, array(
																		'relationship_type_id' => $vn_rel_type_id,
																		'PRIVATE'              => $vn_private
					...
					if ( $vb_index_count ) {
						foreach ( $va_counts as $vs_key => $vn_count ) {
							$this->opo_engine->indexField( $vn_related_table_num, 'COUNT', 0, [ (int) $vn_count ],
								[ 'relationship_type_id' => $vs_key, 'PRIVATE' => $vn_private ] );
							$this->_genIndexInheritance( $t_subject, $t_rel, 'COUNT', $pn_subject_row_id, 0,
								[ (int) $vn_count ], [ 'relationship_type_id' => $vs_key, 'PRIVATE' => $vn_private ] );
						}
					}
				}
			}
		...
		if ( ( ! $vb_initial_reindex_mode ) && ( sizeof( $pa_changed_fields ) > 0 ) ) {
			...
			//
			$va_rows_to_reindex = $this->_getDependentRowsForSubject( $pn_subject_table_num, $pn_subject_row_id,
				$va_deps, $va_changed_field_nums );

			if ( $vb_can_do_incremental_indexing ) {
				if ( method_exists( $vs_subject_tablename, "getDependentCurrentValues" ) ) {
					$current_values = $vs_subject_tablename::getDependentCurrentValues( $pn_subject_table_num,
						$pn_subject_row_id, [ 'db' => $this->getDb() ] );
				} else {
					$current_values = [];
				}
				...
				foreach ( $va_rows_to_reindex as $vs_key => $va_row_to_reindex ) {
					foreach ( $va_row_to_reindex['field_nums'] as $vs_fld_name => $vn_fld_num ) {
						$vs_new_key = $va_row_to_reindex['table_num'] . '/' . $va_row_to_reindex['field_table_num']
						              . '/' . $vn_fld_num . '/' . $va_row_to_reindex['field_row_id'];
						...
						if ( ! isset( $va_rows_to_reindex_by_row_id[ $vs_new_key ] ) ) {
							if ( is_array( $va_row_to_reindex['indexing_info'][ $vs_fld_name ]['BOOST'] ) ) {
								if ( isset( $va_row_to_reindex['indexing_info'][ $vs_fld_name ]['BOOST'][ $vs_subject_type_code ] ) ) {
									$va_row_to_reindex['indexing_info'][ $vs_fld_name ]['BOOST']
										= $va_row_to_reindex['indexing_info'][ $vs_fld_name ]['BOOST'][ $vs_subject_type_code ];
						...
						$va_row_ids_to_reindex_by_table[ $va_row_to_reindex['field_table_num'] ][]
					...
					if ( $t_rel = Datamodel::getInstanceByTableNum( $vn_rel_table_num, true ) ) {
						...
						if ( method_exists( $t_rel, "getApplicableElementCodes" ) ) {
							if ( is_array( $va_element_ids = array_keys( $t_rel->getApplicableElementCodes( null, false,
								...
								ca_attributes::prefetchAttributes( $this->opo_db, $vn_rel_table_num, $va_rel_row_ids,
									$va_element_ids );
							}
						}
					}
				...
				foreach ( $va_rows_to_reindex_by_row_id as $va_row_to_reindex ) {
					...
					$t_rel->setDb( $this->getDb() );
					if ( substr( $va_row_to_reindex['field_name'], 0, 14 )
					     == '_ca_attribute_'
					) {        // is attribute
						$va_row_to_reindex['indexing_info']['datatype']
							= ca_metadata_elements::getElementDatatype( substr( $va_row_to_reindex['field_name'],
							14 ) );
					}

					if ( $va_row_to_reindex['field_name'] == '_count' ) {
						foreach ( $va_row_to_reindex['row_ids'] as $vn_subject_row_id ) {
							$this->_doCountIndexing( Datamodel::getInstanceByTableNum( $va_row_to_reindex['table_num'],
								true ), $vn_subject_row_id, $t_rel, false );
						}
					}

					if ( ( ( isset( $va_row_to_reindex['indexing_info']['INDEX_ANCESTORS'] )
					         && $va_row_to_reindex['indexing_info']['INDEX_ANCESTORS'] )
					       || in_array( 'INDEX_ANCESTORS', $va_row_to_reindex['indexing_info'], true ) )
					) {
						...
						foreach ( $va_row_to_reindex['row_ids'] as $vn_row_id ) {
							$va_content = $this->_genHierarchicalPath( $va_row_to_reindex['field_row_id'],
								$va_row_to_reindex['field_name'], $t_label, $va_row_to_reindex['indexing_info'] );
							$vs_content = is_array( $va_content['values'] ) ? join( " ",
								array_unique( $va_content['values'] ) ) : "";

							if ( is_array( $va_row_to_reindex['indexing_info'] ) ) {
								$this->opo_engine->updateIndexingInPlace( $va_row_to_reindex['table_num'],
									array( $vn_row_id ), $va_row_to_reindex['field_table_num'],
									$va_row_to_reindex['field_num'], null, $va_row_to_reindex['field_row_id'],
									$vs_content, array_merge( $va_row_to_reindex['indexing_info'], array(
										'PRIVATE'              => $vn_private,
										'relationship_type_id' => $vn_rel_type_id,
										'literalContent'       => $va_content['path']
									) ) );
							}
							if ( is_array( $va_row_to_reindex['cv_indexing_info'] ) ) {
								foreach ( $va_row_to_reindex['cv_indexing_info'] as $p => $pinfo ) {
									...
									$this->opo_engine->updateIndexingInPlace( $va_row_to_reindex['table_num'],
										array( $vn_row_id ), $va_row_to_reindex['field_table_num'],
										"CV{$p}_" . $va_row_to_reindex['field_num'], null,
										$va_row_to_reindex['field_row_id'], $vs_content,
										array_merge( $va_row_to_reindex['indexing_info'], array(
											'PRIVATE'              => $vn_private,
											'relationship_type_id' => $vn_rel_type_id,
											'literalContent'       => $va_content['path']
										) ) );
					...
					} elseif ( ( ( isset( $va_row_to_reindex['indexing_info']['INDEX_AS_IDNO'] )
					               && $va_row_to_reindex['indexing_info']['INDEX_AS_IDNO'] )
					             || in_array( 'INDEX_AS_IDNO', $va_row_to_reindex['indexing_info'], true ) )
					           && method_exists( $t_rel, "getIDNoPlugInInstance" )
					           && ( $o_idno = $t_rel->getIDNoPlugInInstance() )
					) {
						foreach ( $va_row_to_reindex['row_ids'] as $vn_row_id ) {
							if ( is_array( $va_row_to_reindex['indexing_info'] ) ) {
								$this->opo_engine->updateIndexingInPlace( $va_row_to_reindex['table_num'],
									$va_row_to_reindex['row_ids'], $va_row_to_reindex['field_table_num'],
									$va_row_to_reindex['field_num'], null, $va_row_to_reindex['field_row_id'],
									$va_row_to_reindex['field_values'][ $va_row_to_reindex['field_name'] ],
									array_merge( $va_row_to_reindex['indexing_info'], array(
										'PRIVATE'              => $vn_private,
										'relationship_type_id' => $vn_rel_type_id,
										'INDEX_AS_IDNO'        => $va_row_to_reindex['indexing_info']['INDEX_AS_IDNO'],
										'IDNO_DELIMITERS'      => $va_row_to_reindex['indexing_info']['IDNO_DELIMITERS']
									) ) );
							}
							if ( is_array( $va_row_to_reindex['cv_indexing_info'] ) ) {
								foreach ( $va_row_to_reindex['cv_indexing_info'] as $p => $pinfo ) {
					...
					if ( isset( $va_row_to_reindex['indexing_info']['datatype'] ) ) {
						...
						switch ( $va_row_to_reindex['indexing_info']['datatype'] ) {
							case __CA_ATTRIBUTE_VALUE_CONTAINER__:        // container
								...
								foreach ( $va_row_to_reindex['row_ids'] as $vn_rel_row_id ) {
									$this->opo_engine->startRowIndexing( $va_row_to_reindex['table_num'],
										$vn_rel_row_id );

									$va_attributes = $t_rel->getAttributesByElement( $vs_element_code,
										array( 'row_id' => $va_row_to_reindex['field_row_id'] ) );
									if ( sizeof( $va_attributes ) ) {
										foreach ( $va_attributes as $vo_attribute ) {
											foreach ( $vo_attribute->getValues() as $vo_value ) {
												...
												$vn_list_id
												                   = ca_metadata_elements::getElementListID( $vn_sub_element_id );
												...
												if ( is_array( $va_additional_indexing )
												     && ( sizeof( $va_additional_indexing ) > 0 )
												) {
												...
												if ( $t_to_reindex
													= Datamodel::getInstanceByTableNum( $va_row_to_reindex['table_num'],
													true )
												) {
													if ( is_array( $va_row_to_reindex['indexing_info'] ) ) {
														$this->opo_engine->updateIndexingInPlace( $va_row_to_reindex['table_num'],
															[ $vn_rel_row_id ], $va_row_to_reindex['field_table_num'],
															"A{$vn_sub_element_id}", $attribute_id,
															$va_row_to_reindex['field_row_id'], $vs_value_to_index,
															array_merge( $va_row_to_reindex['indexing_info'], array(
																'PRIVATE'              => $vn_private,
																'relationship_type_id' => $vn_rel_type_id
															) ) );
													...
													if ( is_array( $va_row_to_reindex['cv_indexing_info'] ) ) {
														foreach (
															$va_row_to_reindex['cv_indexing_info'] as $p => $pinfo
														) {
															...
															$this->opo_engine->updateIndexingInPlace( $va_row_to_reindex['table_num'],
																...
																"CV{$p}_A{$vn_sub_element_id}", $attribute_id,
																$va_row_to_reindex['field_row_id'], $vs_value_to_index,
																array_merge( $va_row_to_reindex['indexing_info'], array(
																	'PRIVATE'              => $vn_private,
																	'relationship_type_id' => $vn_rel_type_id
										...
										$va_sub_elements
											= $this->opo_metadata_element->getElementsInSet( $vs_element_code );
										...
										foreach ( $va_sub_elements as $vn_i => $va_element_info ) {
											if ( $t_to_reindex
												...
												if ( is_array( $va_row_to_reindex['indexing_info'] ) ) {
													$this->opo_engine->removeRowIndexing( $va_row_to_reindex['table_num'],
														$vn_rel_row_id, $va_row_to_reindex['field_table_num'],
														[ 'A' . $va_element_info['element_id'] ],
														$va_row_to_reindex['field_row_id'], $vn_rel_type_id );
												}
												if ( is_array( $va_row_to_reindex['cv_indexing_info'] ) ) {
													foreach ( $va_row_to_reindex['cv_indexing_info'] as $p => $pinfo ) {
														if ( ! isset( $current_values[ $p ][ $va_row_to_reindex['table_num'] ][ $vn_rel_row_id ] ) ) {
														...
														$this->opo_engine->removeRowIndexing( $va_row_to_reindex['table_num'],
															$vn_rel_row_id, $va_row_to_reindex['field_table_num'],
															[ "CV{$p}_A" . $va_element_info['element_id'] ],
															$va_row_to_reindex['field_row_id'], $vn_rel_type_id );
													}
												}
											}
							...
							case __CA_ATTRIBUTE_VALUE_LIST__:            // list
								$va_tmp = array();
								if ( is_array( $va_attributes = $t_rel->getAttributesByElement( $vs_element_code,
									array( 'row_id' => $va_row_to_reindex['field_row_id'] ) ) )
								) {
									foreach ( $va_attributes as $vo_attribute ) {
										foreach ( $vo_attribute->getValues() as $vo_value ) {
								...
								$va_labels     = $t_item->getPreferredDisplayLabelsForIDs( $va_tmp,
									array( 'returnAllLocales' => true ) );
								...
								foreach ( $va_tmp as $vn_attribute_id => $vn_item_id ) {
									...
									if ( ! isset( $va_new_values[ $vn_item_id ] )
									     || ! is_array( $va_new_values[ $vn_item_id ] )
									) {
										continue;
									}
									$vs_v = join( ' ;  ', array_merge( array( $vn_item_id ),
										array_keys( $va_new_values[ $vn_item_id ] ) ) );
								}
								...
								if ( is_array( $va_row_to_reindex['indexing_info'] ) ) {
									$this->opo_engine->updateIndexingInPlace( $va_row_to_reindex['table_num'],
										$va_row_to_reindex['row_ids'], $va_row_to_reindex['field_table_num'],
										$va_row_to_reindex['field_num'], null, $va_row_to_reindex['field_row_id'],
										$vs_v, array_merge( $va_row_to_reindex['indexing_info'], array(
											'PRIVATE'              => $vn_private,
											'relationship_type_id' => $vn_rel_type_id
										) ) );
								}
								if ( is_array( $va_row_to_reindex['cv_indexing_info'] ) ) {
									foreach ( $va_row_to_reindex['cv_indexing_info'] as $p => $pinfo ) {
										...
										$row_ids
											= array_intersect( array_keys( $current_values[ $p ][ $va_row_to_reindex['table_num'] ] ),
										...
										$this->opo_engine->updateIndexingInPlace( $va_row_to_reindex['table_num'],
											$row_ids, "CV{$p}_" . $va_row_to_reindex['field_table_num'],
											$va_row_to_reindex['field_num'], null, $va_row_to_reindex['field_row_id'],
											$vs_v, array_merge( $va_row_to_reindex['indexing_info'], array(
												'PRIVATE'              => $vn_private,
												'relationship_type_id' => $vn_rel_type_id
								...
								break;
							default:
								if ( method_exists( $t_rel, 'getAttributesByElement' )
								     && is_array( $va_attributes = $t_rel->getAttributesByElement( $vs_element_code,
										array( 'row_id' => $va_row_to_reindex['field_row_id'] ) ) )
								) {
									foreach ( $va_attributes as $vo_attribute ) {
										foreach ( $vo_attribute->getValues() as $vo_value ) {
											...
											if ( is_array( $va_row_to_reindex['indexing_info'] ) ) {
												$this->opo_engine->updateIndexingInPlace( $va_row_to_reindex['table_num'],
													$va_row_to_reindex['row_ids'],
													$va_row_to_reindex['field_table_num'],
													$va_row_to_reindex['field_num'], $vo_attribute->getAttributeID(),
													$va_row_to_reindex['field_row_id'], $vs_value_to_index,
													array_merge( $va_row_to_reindex['indexing_info'], array(
														'PRIVATE'              => $vn_private,
														'relationship_type_id' => $vn_rel_type_id
											...
											if ( is_array( $va_row_to_reindex['cv_indexing_info'] ) ) {
												foreach ( $va_row_to_reindex['cv_indexing_info'] as $p => $pinfo ) {
													...
													$this->opo_engine->updateIndexingInPlace( $va_row_to_reindex['table_num'],
														$row_ids, $va_row_to_reindex['field_table_num'],
						...
						if ( is_array( $va_row_to_reindex['indexing_info'] ) ) {
							$this->opo_engine->updateIndexingInPlace( $va_row_to_reindex['table_num'],
								...
								$va_row_to_reindex['field_num'], null, $va_row_to_reindex['field_row_id'],
								$va_row_to_reindex['field_values'][ $va_row_to_reindex['field_name'] ],
								array_merge( $va_row_to_reindex['indexing_info'],
									array( 'PRIVATE' => $vn_private, 'relationship_type_id' => $vn_rel_type_id ) ) );
						}
						if ( is_array( $va_row_to_reindex['cv_indexing_info'] ) ) {
							foreach ( $va_row_to_reindex['cv_indexing_info'] as $p => $pinfo ) {
								if ( ! isset( $current_values[ $p ][ $va_row_to_reindex['table_num'] ] ) ) {
								...
								$this->opo_engine->updateIndexingInPlace( $va_row_to_reindex['table_num'], $row_ids,
									$va_row_to_reindex['field_table_num'], "CV{$p}_" . $va_row_to_reindex['field_num'],
									null, $va_row_to_reindex['field_row_id'],
									$va_row_to_reindex['field_values'][ $va_row_to_reindex['field_name'] ],
									array_merge( $va_row_to_reindex['indexing_info'], array(
										'PRIVATE'              => $vn_private,
										'relationship_type_id' => $vn_rel_type_id
				...
				foreach ( $va_rows_to_reindex as $va_row_to_reindex ) {
					...
					if ( is_array( $pa_exclusion_list[ $va_row_to_reindex['table_num'] ] )
					     && ( isset( $pa_exclusion_list[ $va_row_to_reindex['table_num'] ][ $va_row_to_reindex['row_id'] ] ) )
					) {
						continue;
					}
					...
					$this->opo_engine->removeRowIndexing( $va_row_to_reindex['table_num'], $va_row_to_reindex['row_id'],
						null, null, null, $va_row_to_reindex['relationship_type_id'] );
					if ( $vb_support_attributes ) {
						if ( $t_dep->load( $va_row_to_reindex['row_id'] ) ) {
							$o_indexer->indexRow( $va_row_to_reindex['table_num'], $va_row_to_reindex['row_id'],
								$t_dep->getFieldValuesArray(), true, $pa_exclusion_list );
						}
					} else {
						$o_indexer->indexRow( $va_row_to_reindex['table_num'], $va_row_to_reindex['row_id'],
							$va_row_to_reindex['field_values'], true, $pa_exclusion_list );
					}
		...
		if ( $vb_reindex_children && method_exists( $t_subject, "makeSearchResult" ) ) {
			...
			if ( is_array( $va_children_ids ) && sizeof( $va_children_ids ) > 0 ) {
				...
				$qr_children_res = $t_subject->makeSearchResult( $vs_subject_tablename, $va_children_ids,
					array( 'db' => $this->getDb() ) );
				while ( $qr_children_res->nextHit() ) {
					$o_indexer->indexRow( $pn_subject_table_num, $vn_id = $qr_children_res->get( $vs_subject_pk ),
						array( $vs_subject_pk => $vn_id, 'parent_id' => $qr_children_res->get( 'parent_id' ) ), true,
						$pa_exclusion_list, array() );
				}
			}
		}
	...
	private function _indexAttribute( $pt_subject, $pn_row_id, $pm_element_code_or_id, $pa_data, $pa_options = null ) {
		...
		$va_attributes        = $pt_subject->getAttributesByElement( $pm_element_code_or_id,
			array( 'row_id' => $pn_row_id ) );
		...
		$vn_datatype = isset( $pa_data['datatype'] ) ? $pa_data['datatype']
			: ca_metadata_elements::getElementDatatype( $pm_element_code_or_id );

		switch ( $vn_datatype ) {
			case __CA_ATTRIBUTE_VALUE_CONTAINER__:        // container
				$vn_element_id = is_numeric( $pm_element_code_or_id ) ? $pm_element_code_or_id
					: ca_metadata_elements::getElementID( $pm_element_code_or_id );
				...
				if ( $vn_count = sizeof( $va_attributes ) ) {
					...
					if ( $private_when = caGetOption( 'PRIVATE_WHEN', $pa_data, null ) ) {
						foreach ( $va_attributes as $i => $vo_attribute ) {
							foreach ( $vo_attribute->getValues() as $vo_value ) {
								$map[ $i ][ ca_metadata_elements::getElementCodeForId( $vo_value->getElementID() ) ]
									= $vo_value->getDisplayValue( [ 'returnIdno' => true ] );
							}
						}
					}
					foreach ( $va_attributes as $i => $vo_attribute ) {
						/* index each element of the container */
						$va_sub_element_ids = $this->opo_metadata_element->getElementsInSet( $vn_element_id, true,
							array( 'idsOnly' => true ) );
						if ( is_array( $va_sub_element_ids ) && sizeof( $va_sub_element_ids ) ) {
							...
							foreach ( $vo_attribute->getValues() as $vo_value ) {
								...
								if ( ! in_array( $vs_raw_display_value = $vo_value->getDisplayValue(),
									$va_values_to_index )
								) {
								...
								if ( isset( $pa_data[ '_ca_attribute_' . $vn_sub_element_id ] )
								     && is_array( $pa_data[ '_ca_attribute_' . $vn_sub_element_id ] )
								) {
									$va_sub_data = array_merge( $pa_data,
										$pa_data[ '_ca_attribute_' . $vn_sub_element_id ] );
								}
								$va_sub_data = array_filter( $va_sub_data, function ( $v ) {
									return ! is_array( $v ) && ( strlen( $v ) > 0 );
								} );

								$this->opo_engine->indexField( $pn_subject_table_num,
									"{$field_num_prefix}{$vn_sub_element_id}", $pn_row_id, $va_values_to_index,
									$va_sub_data );
								$this->_genIndexInheritance( $t_inheritance_subject ? $t_inheritance_subject
									: $pt_subject, $t_inheritance_subject ? $pt_subject : null,
									"{$field_num_prefix}{$vn_sub_element_id}",
									$pn_inheritance_subject_id ? $pn_inheritance_subject_id : $pn_row_id, $pn_row_id,
									$va_values_to_index, $va_sub_data, $pa_options );
							...
							foreach ( array_keys( $va_sub_element_ids ) as $vn_e ) {
								$va_sub_data = $pa_data;
								if ( isset( $pa_data[ '_ca_attribute_' . $vn_e ] )
								     && is_array( $pa_data[ '_ca_attribute_' . $vn_e ] )
								) {
									$va_sub_data = array_merge( $pa_data, $pa_data[ '_ca_attribute_' . $vn_e ] );
								}
								$this->opo_engine->indexField( $pn_subject_table_num, $field_num_prefix . $vn_e,
									$pn_row_id, [ '' ], array_merge( $va_sub_data,
										[ 'container_id' => $vo_attribute->getAttributeID() ] ) );
								$this->_genIndexInheritance( $t_inheritance_subject ? $t_inheritance_subject
									: $pt_subject, $t_inheritance_subject ? $pt_subject : null,
									$field_num_prefix . $vn_e,
									$pn_inheritance_subject_id ? $pn_inheritance_subject_id : $pn_row_id, $pn_row_id,
									[ '' ], $pa_data );
							}
						}
					...
					if ( is_array(
						...
						foreach ( $va_sub_elements as $vn_i => $va_element_info ) {
							...
							if ( isset( $pa_data[ '_ca_attribute_' . $sub_element_id ] )
							     && is_array( $pa_data[ '_ca_attribute_' . $sub_element_id ] )
							) {
								$va_sub_data = array_merge( $pa_data, $pa_data[ '_ca_attribute_' . $sub_element_id ] );
							}
							$this->opo_engine->indexField( $pn_subject_table_num, $field_num_prefix . $sub_element_id,
								$pn_row_id, [ '' ], $va_sub_data );
							$this->_genIndexInheritance( $t_inheritance_subject ? $t_inheritance_subject : $pt_subject,
								$t_inheritance_subject ? $pt_subject : null,
								$field_num_prefix . $va_element_info['element_id'],
								$pn_inheritance_subject_id ? $pn_inheritance_subject_id : $pn_row_id, $pn_row_id,
								[ '' ], $pa_data );
						}
					}
			...
			case __CA_ATTRIBUTE_VALUE_OBJECTLOTS__:
				...
				if ( is_array( $va_attributes ) && ( $vn_count = sizeof( $va_attributes ) ) ) {
					foreach ( $va_attributes as $vo_attribute ) {
						foreach ( $vo_attribute->getValues() as $vo_value ) {
							...
							if ( $vn_datatype == __CA_ATTRIBUTE_VALUE_LIST__ ) {
								$this->opo_engine->indexField( $pn_subject_table_num,
									$field_num_prefix . $vn_element_id, $pn_row_id,
									[ $vs_v = $vo_value->getDisplayValue( [ 'output' => 'idno' ] ) ],
									array_merge( $pa_data, [ 'DONT_TOKENIZE' => 1 ] ) );
								$this->_genIndexInheritance( $t_inheritance_subject ? $t_inheritance_subject
									: $pt_subject, $t_inheritance_subject ? $pt_subject : null,
									$field_num_prefix . $vn_element_id, $pn_inheritance_subject_id
										? $pn_inheritance_subject_id : $pn_inheritance_subject_id
											? $pn_inheritance_subject_id : $pn_row_id, $pn_row_id, [ $vs_v ],
									array_merge( $pa_data, [ 'DONT_TOKENIZE' => 1 ] ) );
							}
					...
					// Delete indexing
					$this->opo_engine->indexField( $pn_subject_table_num, $field_num_prefix . $vn_element_id,
						$pn_row_id, [ '' ], $pa_data );
					$this->_genIndexInheritance( $t_inheritance_subject ? $t_inheritance_subject : $pt_subject,
						$t_inheritance_subject ? $pt_subject : null, $field_num_prefix . $vn_element_id,
						$pn_inheritance_subject_id ? $pn_inheritance_subject_id : $pn_row_id, $pn_row_id, [ '' ],
						$pa_data );
				}
				...
				if ( is_array( $va_tmp ) && sizeof( $va_tmp ) ) {
					...
					if ( $t_item = AuthorityAttributeValue::elementTypeToInstance( $vn_datatype ) ) {
						...
						foreach ( $va_tmp as $vn_attribute_id => $vn_item_id ) {
							...
							$vs_v = join( ' ;  ',
								array_merge( array( $vn_item_id ), array_keys( $va_new_values[ $vn_item_id ] ) ) );
							$this->opo_engine->indexField( $pn_subject_table_num, $field_num_prefix . $vn_element_id,
								$pn_row_id, [ $vs_v ], $pa_data );
							$this->_genIndexInheritance( $t_inheritance_subject ? $t_inheritance_subject : $pt_subject,
								$t_inheritance_subject ? $pt_subject : null, $field_num_prefix . $vn_element_id,
								$pn_inheritance_subject_id ? $pn_inheritance_subject_id : $pn_row_id, $pn_row_id,
								[ $vs_v ], $pa_data, $pa_options );

							foreach (
								[ "preferred_labels." . $t_item->getLabelDisplayField(), $t_item->primaryKey() ] as
								$vs_f
							) {
								if ( $va_hier_values = $this->_genHierarchicalPath( $vn_item_id, $vs_f, $t_item,
									$pa_data )
								) {

									$this->opo_engine->indexField( $pn_subject_table_num,
										$field_num_prefix . $vn_element_id, $pn_row_id,
										array_merge( [ $vs_v ], $va_hier_values['values'] ), $pa_data );
									$this->_genIndexInheritance( $t_inheritance_subject ? $t_inheritance_subject
										: $pt_subject, $t_inheritance_subject ? $pt_subject : null,
										$field_num_prefix . $vn_element_id,
										$pn_inheritance_subject_id ? $pn_inheritance_subject_id : $pn_row_id,
										$pn_row_id, array_merge( [ $vs_v ], $va_hier_values['values'] ), $pa_data,
										$pa_options );

									if ( caGetOption( 'INDEX_ANCESTORS_AS_PATH_WITH_DELIMITER', $pa_data, false )
									     !== false
									) {
										$this->opo_engine->indexField( $pn_subject_table_num,
											$field_num_prefix . $vn_element_id, $pn_row_id, [ $va_hier_values['path'] ],
											[ $pa_data, [ 'DONT_TOKENIZE' => 1 ] ] );
										$this->_genIndexInheritance( $t_inheritance_subject ? $t_inheritance_subject
											: $pt_subject, $t_inheritance_subject ? $pt_subject : null,
											$field_num_prefix . $vn_element_id,
											$pn_inheritance_subject_id ? $pn_inheritance_subject_id : $pn_row_id,
											$pn_row_id, [ $va_hier_values['path'] ],
											[ $pa_data, [ 'DONT_TOKENIZE' => 1 ] ], $pa_options );
									}
								}
							}
			...
			default:
				...
				if ( ( $vn_count = sizeof( $va_attributes ) ) > 0 ) {
					foreach ( $va_attributes as $vo_attribute ) {
						foreach ( $vo_attribute->getValues() as $vo_value ) {
							...
							if ( ( ( isset( $pa_data['INDEX_AS_IDNO'] ) && $pa_data['INDEX_AS_IDNO'] )
							       || in_array( 'INDEX_AS_IDNO', $pa_data, true ) )
							     && method_exists( $pt_subject, "getIDNoPlugInInstance" )
							     && ( $o_idno = $pt_subject->getIDNoPlugInInstance() )
							) {
								if ( strlen( $pa_data['IDNO_DELIMITERS'] )
								     || ( is_array( $pa_data['IDNO_DELIMITERS'] )
								          && count( $pa_data['IDNO_DELIMITERS'] ) )
								) {
									...
									$va_values = array_map( function ( $v ) {
										return trim( $v );
									}, preg_split( '!(' . join( '|', $pa_data['IDNO_DELIMITERS'] ) . ')!',
										$vs_value_to_index ) );
								} else {
									$va_values = $o_idno->getIndexValues( $vs_value_to_index, $pa_data );
								}
								$this->opo_engine->indexField( $pn_subject_table_num,
									"{$field_num_prefix}{$vn_element_id}", $pn_row_id, $va_values, $pa_data );
								$this->_genIndexInheritance( $pt_subject, null, "{$field_num_prefix}{$vn_element_id}",
									$pn_row_id, $pn_row_id, $va_values, $pa_data );
							}
							$this->opo_engine->indexField( $pn_subject_table_num, $field_num_prefix . $vn_element_id,
								$pn_row_id, [ $vs_value_to_index ], $pa_data );
							$this->_genIndexInheritance( $t_inheritance_subject ? $t_inheritance_subject : $pt_subject,
								$t_inheritance_subject ? $pt_subject : null, $field_num_prefix . $vn_element_id,
								$pn_inheritance_subject_id ? $pn_inheritance_subject_id : $pn_row_id, $pn_row_id,
								[ $vs_value_to_index ], $pa_data );
						}
					}
				} else {
					// Delete indexing
				...
				// reindex children?
				if ( ( caGetOption( 'INDEX_ANCESTORS', $pa_data, false ) !== false )
				     || ( in_array( 'INDEX_ANCESTORS', $pa_data, true ) )
				) {
					if ( $pt_subject && $pt_subject->isHierarchical() ) {
						if ( $va_hier_values = $this->_genHierarchicalPath( $pn_row_id,
							$vs_element_code = ca_metadata_elements::getElementCodeForId( $vn_element_id ), $pt_subject,
							$pa_data )
						) {
							$this->opo_engine->indexField( $pn_subject_table_num, $field_num_prefix . $vn_element_id,
								$pn_row_id, $va_hier_values['values'], $pa_data );
							$this->_genIndexInheritance( $t_inheritance_subject ? $t_inheritance_subject : $pt_subject,
								$t_inheritance_subject ? $pt_subject : null, $field_num_prefix . $vn_element_id,
								$pn_inheritance_subject_id ? $pn_inheritance_subject_id : $pn_row_id, $pn_row_id,
								$va_hier_values['values'], $pa_data );
							...
							if ( caGetOption( 'INDEX_ANCESTORS_AS_PATH_WITH_DELIMITER', $pa_data, false ) !== false ) {
								...
								$this->_genIndexInheritance( $t_inheritance_subject ? $t_inheritance_subject
									...
									$field_num_prefix . $vn_element_id,
									$pn_inheritance_subject_id ? $pn_inheritance_subject_id : $pn_row_id, $pn_row_id,
									[ $va_hier_values['path'] ], array_merge( $pa_data, [ 'DONT_TOKENIZE' => 1 ] ) );
							}
						}

						$va_children_ids = $pt_subject->getHierarchyAsList( $pn_row_id,
							array( 'idsOnly' => true, 'includeSelf' => false ) );

						if ( ! caGetOption( 'reindex', $pa_options, false ) && is_array( $va_children_ids )
						     && sizeof( $va_children_ids ) > 0
						) {
							// trigger reindexing of children
							$o_indexer = new SearchIndexer( $this->opo_db );
							$pt_subject->load( $pn_row_id );
							$va_content = $pt_subject->get( $pt_subject->tableName() . "." . $vs_element_code, array(
								...
								foreach ( $va_content as $vn_i => $va_by_locale ) {
									foreach ( $va_by_locale as $vn_locale_id => $va_content_list ) {
										foreach ( $va_content_list as $va_content_container ) {
											$o_indexer->opo_engine->indexField( $pn_subject_table_num,
												$field_num_prefix . $vn_element_id, $vn_id,
												[ $va_content_container[ $vs_element_code ] ],
												array_merge( $pa_data, [ 'DONT_TOKENIZE' => 1, 'TOKENIZE' => 1 ] ) );
											$this->_genIndexInheritance( $t_inheritance_subject ? $t_inheritance_subject
												: $pt_subject, $t_inheritance_subject ? $pt_subject : null,
												$field_num_prefix . $vn_element_id, $pn_row_id, $vn_id,
												[ $va_content_container[ $vs_element_code ] ],
												array_merge( $pa_data, [ 'DONT_TOKENIZE' => 1, 'TOKENIZE' => 1 ] ) );
										}
									}
		...
		if ( ( isset( $pa_data['COUNT'] ) && (bool) $pa_data['COUNT'] ) || in_array( 'COUNT', $pa_data, true ) ) {
			$this->opo_engine->indexField( $pn_subject_table_num, 'COUNT' . $vn_element_id, $pn_row_id, [ $vn_count ],
				array_merge( $pa_data, [ 'relationship_type_id' => 0 ] ) );
			$this->_genIndexInheritance( $t_inheritance_subject ? $t_inheritance_subject : $pt_subject,
				$t_inheritance_subject ? $pt_subject : null, 'COUNT' . $vn_element_id,
				$pn_inheritance_subject_id ? $pn_inheritance_subject_id : $pn_row_id, $pn_row_id, [ $vn_count ],
				array_merge( $pa_data, [ 'relationship_type_id' => 0 ] ) );
		}
	 ...
	 * (Note that while this is called this a "public" call in fact you shouldn't need to call this directly.
	 * BaseModel.php does this for you during delete().)
	 */
	public function startRowUnIndexing( $pn_subject_table_num, $pn_subject_row_id ) {
		...
		if ( in_array( $pn_subject_table_num, array( 3, 4 ) ) || isset( $va_indexed_tables[ $pn_subject_table_num ] )
		     || sizeof( $va_deps )
		) {
			$this->opa_dependencies_to_update = $this->_getDependentRowsForSubject( $pn_subject_table_num,
				$pn_subject_row_id, $va_deps );
		}
	...
	public function commitRowUnIndexing( $pn_subject_table_num, $pn_subject_row_id, $pa_options = null ) {
		...
		if ( caGetOption( 'queueIndexing', $pa_options, false )
		     && ! $this->opo_app_config->get( 'disable_out_of_process_search_indexing' )
		) {
			...
			if ( ! $vb_can_do_incremental_indexing ) {
				...
				foreach ( $va_id_list as $vn_table_num => $va_row_ids ) {
					if ( $t_instance = Datamodel::getInstanceByTableNum( $vn_table_num, true ) ) {
						$va_field_values[ $vn_table_num ]
							= $t_instance->getFieldValuesForIDs( array_keys( $va_row_ids ) );
					}
				}
				...
				foreach ( $this->opa_dependencies_to_update as $va_item ) {
					...
					$this->opo_engine->removeRowIndexing( $va_item['table_num'], $va_item['row_id'], null, null, null,
						$va_item['relationship_type_id'] );

					$this->indexRow( $va_item['table_num'], $va_item['row_id'],
						$va_field_values[ $va_item['table_num'] ][ $va_item['row_id'] ] );
					$this->_doCountIndexing( Datamodel::getInstanceByTableNum( $va_item['table_num'], true ),
						$va_item['row_id'], $t_subject, false );
				...
				foreach ( $this->opa_dependencies_to_update as $va_item ) {
					$this->opo_engine->removeRowIndexing( $va_item['table_num'], $va_item['row_id'],
						$va_item['field_table_num'], $va_item['field_nums'], $va_item['field_row_id'],
						$va_item['relationship_type_id'] );
					...
					if ( is_array( $va_item['cv_indexing_info'] ) ) {
						...
						foreach ( $va_item['cv_indexing_info'] as $f => $finfo ) {
							...
							foreach ( $finfo as $p => $pinfo ) {
								if ( is_array(
									$fn = self::_searchConfigFieldEntryToIndexFieldNums( $va_item['field_table_num'],
										$f, [ 'policy' => $p ] ) )
								) {
									$cv_field_nums += $fn;
								}
						...
						if ( sizeof( $cv_field_nums ) ) {
							$this->opo_engine->removeRowIndexing( $va_item['table_num'], $va_item['row_id'],
								$va_item['field_table_num'], $cv_field_nums, $va_item['field_row_id'],
								$va_item['relationship_type_id'] );
						}
					}
					// Remove existing count index and recreate
					$this->opo_engine->removeRowIndexing( $va_item['table_num'], $va_item['row_id'],
						$va_item['field_table_num'], null, 0, $va_item['relationship_type_id'] );
					$this->_doCountIndexing( $t_rel = Datamodel::getInstanceByTableNum( $va_item['table_num'], true ),
						$va_item['row_id'], $t_subject, false );

					// Force recounts on related items when relationship record is modified
					if ( $t_subject->isRelationship() ) {
						$this->_doCountIndexing( $t_rel, $va_item['row_id'], Datamodel::getInstanceByTableName(
							$x = $t_subject->getOppositeTableName( $t_rel->tableName() ), true ), false );
					}
	 ...
	 * @param array $pa_changed_field_nums Array of fields that have changed, where array keys are field names and
	 *                                     array values are field number codes (Eg. I15 or A4)
	 *
	 * @return bool
	 */
	 ...
	 */
	private function _getDependentRowsForSubject(
		$pn_subject_table_num, $pn_subject_row_id, $va_deps, $pa_changed_field_nums = null
	) {
...
// Loop through dependent tables
		...
		foreach ( $va_deps as $vs_dep_table ) {
			...
			if (
				...
				$va_params = null;
				if ( ( $vs_subject_tablename === $vs_dep_table )
				     && ( $vs_self_rel_table_name
						= $t_dep->getSelfRelationTableName() )
				) {
					//
					...
					if ( $t_subject->hasField( 'deleted' ) ) {
						$vs_sql_joins  = "
							INNER JOIN {$vs_subject_tablename} AS t1 ON t1.{$vs_subject_pk} = t0."
						                 . $t_self_rel->getLeftTableFieldName() . "
							INNER JOIN {$vs_subject_tablename} AS t2 ON t2.{$vs_subject_pk} = t0."
						                 . $t_self_rel->getRightTableFieldName();
					...
					$vs_sql = "
						...
						{$vs_sql_joins}
						WHERE
							(t0." . $t_self_rel->getLeftTableFieldName() . " = ? OR t0."
					          . $t_self_rel->getRightTableFieldName() . " = ?)
							{$vs_delete_sql}
					";
				} else {
				...
				while ( $qr_res->nextRow() ) {
					...
					if ( isset( $va_info['related'] ) && is_array( $va_info['related']['fields'] ) ) {
						foreach ( $va_info['related']['fields'] as $vs_field => $va_config ) {
							$vn_field_num                    = $this->_getFieldNumForIndexing( $t_dep,
								$vn_dep_table_num, $vs_field );
					...
					if ( $t_label = $t_dep->getLabelTableInstance() ) {
						...
						if ( is_array( $va_label_info['related']['fields'] )
							...
							foreach ( $va_label_info['related']['fields'] as $vs_label_field => $va_config ) {
								$vn_label_field_num                          = $this->_getFieldNumForIndexing( $t_label,
									$vn_label_table_num, $vs_label_field );
					...
					foreach (
						array( $vn_left_id => $vn_right_id, $vn_right_id => $vn_left_id ) as $vn_id_1 => $vn_id_2
					) {
							...
							foreach ( $va_labels as $vn_x => $va_labels_by_locale ) {
								foreach ( $va_labels_by_locale as $vn_locale_id => $va_label_list ) {
									foreach ( $va_label_list as $va_label ) {
										...
										$vs_key = $vn_dep_table_num . '/' . $vn_id_1 . '/' . $vn_label_table_num . '/'
										          . $vn_label_id;
...
// loop through the tables for each relationship between the subject and the dep
				...
				foreach ( $va_table_list_list as $vs_list_name => $va_linking_tables_config ) {
					...
					$va_linking_tables = is_array( $va_linking_tables ) ? array_reverse( $va_linking_tables )
						: array();        // they come out of the conf file reversed from how we want them
					array_unshift( $va_linking_tables, $vs_dep_rel_table );
					array_push( $va_linking_tables,
						$vs_dep_table );                                                            // the dep table name is not listed in the config file (it's redundant)
...
// update indexing for each relationship
				foreach ( $va_rel_tables_to_index_list as $vs_rel_table ) {
					...
					foreach ( $va_table_path as $vs_n => $va_linking_tables_config ) {
						$va_table_list = ( caIsIndexedArray( $va_linking_tables_config ) ) ? $va_linking_tables_config
							: array_keys( $va_linking_tables_config );
						if ( caIsIndexedArray( $va_linking_tables_config ) ) {
							$va_tmp = array();
							foreach ( $va_linking_tables_config as $vs_t ) {
								$va_tmp[ $vs_t ] = [];
						...
						$va_cv_fields_to_index = $this->getFieldsToIndex( $vn_dep_table_num, $vs_rel_table,
							[ 'currentValueFields' => true ] );
						...
						if ( $vs_rel_table == $vs_subject_tablename ) {
							if ( is_array( $pa_changed_field_nums )
							     && ! $this->_indexedFieldsHaveChanged( $all_fields_to_index, $pa_changed_field_nums )
							) {
								continue;
						...
						$va_rows = $this->_getRelatedRows( array_reverse( $va_full_path ), $va_linking_tables_config,
							isset( $va_table_key_list[ $vs_list_name ] ) ? $va_table_key_list[ $vs_list_name ] : null,
							$vs_subject_tablename, $pn_subject_row_id, $vs_rel_table ? $vs_rel_table : $vs_dep_table,
							$all_fields_to_index );
								...
								foreach ( $all_fields_to_index as $vs_field => $va_indexing_info ) {
									switch ( $vs_field ) {
										case '_count':
											...
											break;
										default:
											$vn_fld_num = $this->_getFieldNumForIndexing( $t_rel, $vn_rel_table_num,
												$vs_field );
											break;
									}
									...
									$vn_private = ( is_array( $va_private_rel_types ) && sizeof( $va_private_rel_types )
									                && in_array( $vn_rel_type_id, $va_private_rel_types ) ) ? 1 : 0;

									$vs_key = $vn_dep_table_num . '/' . $vn_row_id . '/' . $vn_rel_table_num . '/'
									          . $vn_fld_row_id;
									...
									$va_dependent_rows[ $vs_key ]['indexing_info'][ $vs_field ]
									                                                            = isset( $va_fields_to_index[ $vs_field ] )
										? $va_fields_to_index[ $vs_field ] : null;

									$va_dependent_rows[ $vs_key ]['cv_indexing_info'][ $vs_field ] = null;
									foreach ( $va_cv_fields_to_index as $p => $indexing_config ) {
										if ( isset( $indexing_config[ $vs_field ] )
										     && is_array( $indexing_config[ $vs_field ] )
										) {
											$va_dependent_rows[ $vs_key ]['cv_indexing_info'][ $vs_field ]
												= [ $p => $indexing_config[ $vs_field ] ];
										}
									}

									// reindex any rows that have authority metadata elements that reference this
									if ( method_exists( $t_dep, "getAuthorityElementReferences" )
									     && is_array( $va_element_references
											= $t_dep->getAuthorityElementReferences( array( 'row_id' => $vn_row_id ) ) )
									) {
										foreach ( $va_element_references as $vn_element_table_num => $va_references ) {
											...
											$va_element_fields_to_index
												                   = $this->getFieldsToIndex( $vn_element_table_num,
												$vn_element_table_num );
											...
											$va_field_data = [];
											while ( $qr_field_data->nextRow() ) {
												$va_field_data[ (int) $qr_field_data->get( $vs_element_table_pk ) ]
													= $qr_field_data->getRow();
											}

											foreach ( $va_references as $vn_element_row_id => $va_element_ids ) {
												$vs_key = $vn_element_table_num . '/' . $vn_element_row_id . '/'
												          . $vn_element_table_num . '/' . $vn_element_row_id;
												if ( ! isset( $va_dependent_rows[ $vs_key ] ) ) {
												...
												foreach ( $va_element_ids as $vn_element_id ) {
													$va_dependent_rows[ $vs_key ]['field_nums'][ '_ca_attribute_'
													                                             . $vn_element_id ]
														= 'A' . $vn_element_id;
													// TODO: Undefined variable '$field_num_prefix'
													$va_dependent_rows[ $vs_key ]['field_names'][ $field_num_prefix
													                                              . $vn_element_id ]
														= '_ca_attribute_' . $vn_element_id;
													$va_dependent_rows[ $vs_key ]['indexing_info'][ '_ca_attribute_'
													                                                . $vn_element_id ]
														= $va_element_fields_to_index[ '_ca_attribute_'
														                               . $vn_element_id ];
												}
											}
										}
	 ...
	 */
	private function _getRelatedRows(
		$pa_tables, $pa_linking_tables_config, $pa_table_keys, $ps_subject_tablename, $pn_row_id, $ps_table_to_index,
		$pa_fields_to_index
	) {
		$vs_key = md5( print_r( $pa_tables, true ) . "/" . print_r( $pa_table_keys, true ) . "/"
		               . $ps_subject_tablename );
		...
		if ( true ) { //!MemoryCache::contains($vs_key, 'SearchIndexerRelatedRowsJoins')) {
			...
			foreach ( $pa_tables as $vs_right_table ) {
				...
				$vs_rel_type_res_sql = '';
				if ( ( $va_type_res = $pa_linking_tables_config[ $vs_right_table ]['types'] )
				     && is_array( $va_type_res )
				     && sizeof( $va_type_res )
				) {
				...
				if ( is_array( $pa_table_keys )
				     && ( isset( $pa_table_keys[ $vs_right_table ][ $vs_left_table ] )
				          || isset( $pa_table_keys[ $vs_left_table ][ $vs_right_table ] ) )
					...
					if ( isset( $pa_table_keys[ $vs_left_table ][ $vs_right_table ] ) ) {
						...
						$vs_join
							= "INNER JOIN {$vs_right_table} AS {$vs_alias} ON ({$vs_alias}.{$va_key_spec['right_key']} = {$vs_prev_alias}.{$va_key_spec['left_key']}"
							  . $vs_rel_type_res_sql;

						if ( $va_key_spec['left_table_num'] || $va_key_spec['right_table_num'] ) {
							if ( $va_key_spec['right_table_num'] ) {
								$vs_join .= " AND {$vs_alias}.{$va_key_spec['right_table_num']} = "
								            . Datamodel::getTableNum( $vs_left_table );
								$vs_t    = $vs_right_table;
							} else {
								$vs_join .= " AND {$vs_prev_alias}.{$va_key_spec['left_table_num']} = "
								            . Datamodel::getTableNum( $vs_right_table );
								$vs_t    = $vs_left_table;
							}
						}
						$vs_join .= ')';
						...
						$vs_join
							= "INNER JOIN {$vs_right_table} AS {$vs_alias} ON ({$vs_alias}.{$va_key_spec['left_key']} = {$vs_prev_alias}.{$va_key_spec['right_key']}"
							  . $vs_rel_type_res_sql;

						if ( $va_key_spec['left_table_num'] || $va_key_spec['right_table_num'] ) {
							if ( $va_key_spec['right_table_num'] ) {
								$vs_join .= " AND {$vs_prev_alias}.{$va_key_spec['right_table_num']} = "
								            . Datamodel::getTableNum( $vs_right_table );
								$vs_t    = $vs_left_table;
							} else {
								$vs_join .= " AND {$vs_alias}.{$va_key_spec['left_table_num']} = "
								            . Datamodel::getTableNum( $vs_left_table );
								$vs_t    = $vs_right_table;
							}
						}
					...
					if ( method_exists( $t_right_table, "isSelfRelationship" )
					     && ( $t_right_table->isSelfRelationship() )
					) {
						$va_joins[] = array(
							"INNER JOIN {$va_rel['many_table']} AS {$vs_alias} ON {$vs_prev_alias}.{$va_rel['one_table_field']} = {$vs_alias}."
							. $t_right_table->getLeftTableFieldName() . $vs_rel_type_res_sql,
							"INNER JOIN {$va_rel['many_table']} AS {$vs_alias} ON {$vs_prev_alias}.{$va_rel['one_table_field']} = {$vs_alias}."
							. $t_right_table->getRightTableFieldName() . $vs_rel_type_res_sql
						);
					} else {
						$va_joins[]
							= "INNER JOIN {$va_rel['many_table']} AS {$vs_alias} ON {$vs_prev_alias}.{$va_rel['one_table_field']} = {$vs_alias}.{$va_rel['many_table_field']}"
							  . $vs_rel_type_res_sql;
					}
					...
					if ( method_exists( $t_left_table, "isSelfRelationship" )
					     && ( $t_left_table->isSelfRelationship() )
					) {
						$va_joins[] = array(
							"INNER JOIN {$va_rel['one_table']} AS {$vs_alias} ON {$vs_alias}.{$va_rel['one_table_field']} = {$vs_prev_alias}."
							. $t_left_table->getRightTableFieldName() . $vs_rel_type_res_sql,
							"INNER JOIN {$va_rel['one_table']} AS {$vs_alias} ON {$vs_alias}.{$va_rel['one_table_field']} = {$vs_prev_alias}."
							. $t_left_table->getLeftTableFieldName() . $vs_rel_type_res_sql
						);
					} else {
						$va_joins[]
							= "INNER JOIN {$va_rel['one_table']} AS {$vs_alias} ON {$vs_alias}.{$va_rel['one_table_field']} = {$vs_prev_alias}.{$va_rel['many_table_field']}"
							  . $vs_rel_type_res_sql;
					}
				}

				// Add relationship type with "rel_type_id" alias to distinguish it from record type_id fields (eg. ca_objects.type_id);
				if ( ( $t_table = Datamodel::getInstanceByTableName( $vs_right_table, true ) )
				     && ( $t_table->isRelationship() && $t_table->hasField( 'type_id' ) )
				) {
			...
			if ( $t_indexed_table = Datamodel::getInstanceByTableName( $ps_table_to_index, true ) ) {
				foreach ( $pa_fields_to_index as $vs_f => $va_field_info ) {
					...
					if ( in_array( $t_indexed_table->getFieldInfo( $vs_f, 'FIELD_TYPE' ),
						array( FT_MEDIA, FT_FILE, FT_VARS, FT_DATERANGE, FT_HISTORIC_DATERANGE, FT_TIMERANGE ) )
					) {
						continue;
					}
			...
			foreach ( $va_tmp as $vn_i => $vs_v ) {
				if ( in_array( $t_select->getFieldInfo( $vs_v, 'FIELD_TYPE' ),
					array( FT_MEDIA, FT_FILE, FT_VARS, FT_DATERANGE, FT_HISTORIC_DATERANGE, FT_TIMERANGE ) )
				) {
					continue;
				}
		...
		for ( $i = 0; $i < $vn_num_queries_required; $i ++ ) {
			...
			if ( ! $qr_res ) {
				throw new Exception( _t( "Invalid _getRelatedRows query: %1",
					join( "; ", $this->opo_db->getErrors() ) ) );
			}
				...
				foreach ( $va_table_list_list as $vs_list_name => $va_linking_tables_config ) {
					...
					if ( in_array( $ps_subject_table,
						...
						foreach ( $va_table_list as $vs_tablename ) {
							...
							if ( $vs_last_table ) {
								if ( $va_rel = Datamodel::getOneToManyRelations( $vs_tablename,
									$vs_last_table )
	 ...
	 *
	 */
	private function _getQueriesForRelatedRows(
		$pt_subject, $pn_subject_row_id, $pt_rel, $pb_reindex_mode, $pa_options = null
	) {
		$for_current_value_reindex = caGetOption( 'forCurrentValueReindex', $pa_options, false );
		...
		$va_table_info = $this->getTableIndexingInfo( $vs_subject_tablename,
			$pb_force_related ? "{$vs_related_table}.related" : $vs_related_table );
		...
		if ( ! $pb_force_related && ( $vs_subject_tablename == $vs_related_table ) ) {
			...
			$vs_sql        = "
				...
				INNER JOIN {$vs_self_rel_table_name} ON {$vs_self_rel_table_name}."
			                 . $t_self_rel->getLeftTableFieldName() . " = {$vs_related_table}.{$vs_related_pk}
				WHERE
				...
				FROM {$vs_related_table}
				INNER JOIN {$vs_self_rel_table_name} ON {$vs_self_rel_table_name}."
			                 . $t_self_rel->getRightTableFieldName() . " = {$vs_related_table}.{$vs_related_pk}
				WHERE
			...
			if ( ! is_array( $va_fields_to_index = $this->getFieldsToIndex( $vs_subject_tablename,
				$pb_force_related ? "{$vs_related_table}.related" : $vs_related_table ) )
			) {
				$va_fields_to_index = [];
			}
			if ( ! is_array( $va_cv_fields_to_index = $this->getFieldsToIndex( $vs_subject_tablename,
				$pb_force_related ? "{$vs_related_table}.related" : $vs_related_table,
				[ 'currentValueFields' => true ] ) )
			) {
				$va_cv_fields_to_index = [];
			...
			foreach ( $va_table_list_list as $vs_list_name => $va_linking_tables_config ) {
				...
				foreach ( $va_linking_tables as $vs_right_table ) {
					$va_rel_type_ids     = array();
					$vs_rel_type_res_sql = '';
					if ( ( $va_type_res = $va_linking_tables_config[ $vs_right_table ]['types'] )
					     && is_array( $va_type_res )
					     && sizeof( $va_type_res )
					) {
						$va_rel_type_ids = caMakeRelationshipTypeIDList( $vs_right_table, $va_type_res );
					...
					if ( is_array( $va_table_key_list )
					     && ( isset( $va_table_key_list[ $vs_list_name ][ $vs_right_table ][ $vs_left_table ] )
					          || isset( $va_table_key_list[ $vs_list_name ][ $vs_left_table ][ $vs_right_table ] ) )
					) {        // are the keys for this join specified in the indexing config?
						...
						if ( isset( $va_table_key_list[ $vs_list_name ][ $vs_left_table ][ $vs_right_table ] ) ) {
							...
							$vs_join
							             = "INNER JOIN {$vs_right_table} AS {$vs_alias} ON ({$vs_alias}.{$va_key_spec['right_key']} = {$vs_prev_alias}.{$va_key_spec['left_key']}"
							               . $vs_rel_type_res_sql . self::_genQueryFilters( $vs_right_table,
									$va_table_info, $va_linking_tables_config[ $vs_right_table ], $vs_alias );
							if ( $va_key_spec['left_table_num'] || $va_key_spec['right_table_num'] ) {
								if ( $va_key_spec['right_table_num'] ) {
									$vs_join .= " AND {$vs_alias}.{$va_key_spec['right_table_num']} = "
									            . Datamodel::getTableNum( $vs_left_table );
						...
						if ( ( $pt_rel_instance = Datamodel::getInstanceByTableName( $vs_right_table, true ) )
						     && method_exists( $pt_rel_instance, "isRelationship" )
						     && $pt_rel_instance->isRelationship()
						     && $pt_rel_instance->hasField( 'type_id' )
						) {
						...
						if ( $va_rel = Datamodel::getOneToManyRelations( $vs_left_table, $vs_right_table ) ) {
							...
							if ( Datamodel::isSelfRelationship( $va_rel['many_table'] ) ) {
								...
								$va_joins[] = [
									"INNER JOIN {$va_rel['many_table']} AS {$vs_alias} ON {$vs_prev_alias}.{$va_rel['one_table_field']} = {$vs_alias}."
									. $t_self_rel->getLeftTableFieldName() . $vs_rel_type_res_sql
									. self::_genQueryFilters( $vs_right_table, $va_table_info,
										$va_linking_tables_config[ $vs_right_table ], $vs_alias ),
									"INNER JOIN {$va_rel['many_table']} AS {$vs_alias} ON {$vs_prev_alias}.{$va_rel['one_table_field']} = {$vs_alias}."
									. $t_self_rel->getRightTableFieldName() . $vs_rel_type_res_sql
									. self::_genQueryFilters( $vs_right_table, $va_table_info,
										$va_linking_tables_config[ $vs_right_table ], $vs_alias )
								];
								...
								if ( ( $pt_rel_instance = Datamodel::getInstanceByTableName( $va_rel['many_table'],
										true ) )
								     && method_exists( $pt_rel_instance, "isRelationship" )
								     && $pt_rel_instance->isRelationship()
								     && $pt_rel_instance->hasField( 'type_id' )
									...
									$vs_rel_type_id_fld = "{$vs_alias}.type_id";
								} elseif ( ( $vn_t > 0 ) && ( $vs_related_table == $va_rel['many_table'] )
								           && is_array( $pa_restrict_to_types )
								           && sizeof( $pa_restrict_to_types )
								           && $pt_rel_instance->hasField( 'type_id' )
								) {
									$vs_type_id_fld      = "{$vs_alias}.type_id";
									$vs_rel_type_res_sql .= " AND {$vs_type_id_fld} IN (" . join( ',',
											$pa_restrict_to_types ) . ")";
								}
								$va_joins[]
									= [
									"INNER JOIN {$va_rel['many_table']} AS {$vs_alias} ON {$vs_prev_alias}.{$va_rel['one_table_field']} = {$vs_alias}.{$va_rel['many_table_field']}"
									. $vs_rel_type_res_sql . self::_genQueryFilters( $vs_right_table, $va_table_info,
										$va_linking_tables_config[ $vs_right_table ], $vs_alias )
								];
							}
						} elseif ( $va_rel = Datamodel::getOneToManyRelations( $vs_right_table, $vs_left_table ) ) {
							...
							if ( Datamodel::isSelfRelationship( $va_rel['many_table'] ) ) {
								...
								if ( ( $pt_rel_instance = Datamodel::getInstanceByTableName( $va_rel['one_table'],
										true ) )
								     && method_exists( $pt_rel_instance, "isRelationship" )
								     && $pt_rel_instance->isRelationship()
								     && $pt_rel_instance->hasField( 'type_id' )
								...
								} elseif ( ( $vn_t > 0 ) && ( $vs_related_table == $va_rel['one_table'] )
								           && is_array( $pa_restrict_to_types )
								           && sizeof( $pa_restrict_to_types )
								           && $pt_rel_instance->hasField( 'type_id' )
								) {
				...
				for ( $vn_i = 0; $vn_i < $vn_field_list_count; $vn_i ++ ) {
					...
					$va_proc_field_list[ $vn_i ]
						= $va_aliases[ $vs_related_table ][ sizeof( $va_aliases[ $vs_related_table ] ) - 1 ] . '.'
						  . $va_field_list[ $vn_i ];
				}
				$va_proc_field_list[] = $va_aliases[ $vs_related_table ][ sizeof( $va_aliases[ $vs_related_table ] )
				                                                          - 1 ] . '.' . $vs_related_pk;
				...
				if ( isset( $va_rel['many_table'] ) && $va_rel['many_table'] ) {
					$va_proc_field_list[]
						= $va_aliases[ $va_rel['many_table'] ][ sizeof( $va_aliases[ $va_rel['many_table'] ] ) - 1 ]
				...
				if ( $vn_num_queries_required > 1 ) {
					foreach ( $va_joins as $vn_i => $va_join_list ) {
						...
						$va_joins[ $vn_i ] = array_pad( $va_joins[ $vn_i ], $vn_num_queries_required,
							$va_joins[ $vn_i ][0] );
					}
				}
		...
		return [
			'queries'                        => $va_queries,
			'fields_to_index'                => $va_fields_to_index,
			'current_value_fields_to_index'  => $va_cv_fields_to_index,
			'field_list'                     => $va_field_list,
			'table_info'                     => $va_table_info,
			'linking_table_config_per_query' => $va_linking_table_config_per_query
		];
	}
	# ------------------------------------------------
	 ...
	 * @return string
	 */
	private static function _genQueryFilters(
		string $table, array $table_info, ?array $linking_tables_config, string $alias
	) {
		$filter_sql = null;
				...
				switch ( $k ) {
					...
					case 'type_id':
						...
						foreach ( $types as $t ) {
							if ( is_numeric( $t ) ) {
								$type_criteria[] = (int) $t;
							} elseif ( ! empty( $field_info['LIST_CODE'] )
							           && ( $type_id
									= caGetListItemID( $field_info['LIST_CODE'], $t ) )
							) {
								$type_criteria[] = $type_id;
							}
						}
						if ( sizeof( $type_criteria ) > 0 ) {
							$filter_criteria[] = "({$alias}.{$k} IN (" . join( ', ', $type_criteria )
							                     . ") OR {$alias}.{$k} IS NULL)";
						}
						break;
				}
	...
	private function _doCountIndexing( $pt_subject, $pn_subject_row_id, $pt_rel, $pb_reindex_mode, $pa_options = null
		...
		if ( isset( $va_fields_to_index['_count'] ) && is_array( $va_fields_to_index['_count'] ) ) {
			...
			foreach ( $va_queries as $va_query ) {
				...
				if ( $this->opo_db->numErrors() ) {
					// Shouldn't ever happen
					throw new ApplicationException( _t( "SQL error while getting content for index of related fields: %1; SQL was %2",
						$this->opo_db->getErrors(), $vs_sql ) );
				}
					...
					if ( $pb_reindex_mode ) {
						$this->opo_engine->indexField( $vn_related_table_num, 'COUNT', 0, [ (int) $vn_count ],
							array_merge( $va_rel_field_info,
								[ 'relationship_type_id' => ( $vs_key != '_total' ) ? $vs_key : 0 ] ) );
						$this->_genIndexInheritance( $pt_subject, $pt_rel, 'COUNT', $pn_subject_row_id, 0,
							[ (int) $vn_count ], array_merge( $va_rel_field_info,
								[ 'relationship_type_id' => ( $vs_key != '_total' ) ? $vs_key : 0 ] ) );
					} else {
						if ( ( caGetOption( 'CHILDREN_INHERIT', $pa_options, false )
						       || ( array_search( 'CHILDREN_INHERIT', $pa_options, true ) !== false ) )
						     && ( is_array(
								$va_ids = $pt_subject->getHierarchy( $pn_subject_row_id, [ 'idsOnly' => true ] ) ) )
						) {
							$this->opo_engine->updateIndexingInPlace( $vn_subject_table_num, $va_ids,
								$vn_related_table_num, 'COUNT', null, 0, $vn_count, array_merge( $va_rel_field_info,
									[ 'relationship_type_id' => ( $vs_key != '_total' ) ? $vs_key : 0 ] ) );
						} else {
							$this->opo_engine->updateIndexingInPlace( $vn_subject_table_num, [ $pn_subject_row_id ],
								$vn_related_table_num, 'COUNT', null, 0, $vn_count, array_merge( $va_rel_field_info,
									[ 'relationship_type_id' => ( $vs_key != '_total' ) ? $vs_key : 0 ] ) );
						}
					}
	...
	public function updateCurrentValueIndexing( $policy, $subject_table_num, $subject_row_id, $values, $options = null
		...
		foreach ( $indexing_tables as $rel_table ) {
			if ( is_array( $fields_to_index = $this->getFieldsToIndex( $subject_table_num, $rel_table,
				[ 'currentValueFields' => true ] ) )
			) {
				$field_nums = [];
				foreach ( $fields_to_index as $p => $pinfo ) {
					foreach ( $pinfo as $f => $finfo ) {
						if ( is_array(
							$fn = self::_searchConfigFieldEntryToIndexFieldNums( $rel_table, $f, [ 'policy' => $p ] ) )
						) {
				...
				$this->opo_engine->removeRowIndexing( $subject_table_num, $subject_row_id,
					Datamodel::getTableNum( $rel_table ), $field_nums );
			}
		}
		$this->indexRow( $subject_table_num, $subject_row_id, $values, false, null, null,
			[ 'forCurrentValueReindex' => $policy ] );
	}
	# ------------------------------------------------
}
