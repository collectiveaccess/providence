...
class SearchIndexer extends SearchBase
{
    ...
    static public $s_search_indexing_queue_inserts = [];
    static public $s_search_unindexing_queue_inserts = [];

    /**
     *
     */
    static public $s_search_config_field_entry_to_index_field_nums_cache = [];
    ...
    public function __destruct()
    {
        ...
        if (sizeof(self::$s_search_indexing_queue_inserts) > 0) {
            ...
            foreach ($va_insert_segments as $x) {
                $o_db->query(
                    "INSERT INTO ca_search_indexing_queue (table_num, row_id, field_data, reindex, changed_fields, options) VALUES {$x}"
                );
            }
        }
        ...
        if (sizeof(self::$s_search_unindexing_queue_inserts) > 0) {
            ...
            foreach ($va_insert_segments as $x) {
                $o_db->query(
                    "INSERT INTO ca_search_indexing_queue (table_num, row_id, is_unindex, dependencies) VALUES {$x}"
                );
            }
        }
    }
    ...
    public function reindex($pa_table_names = null, $pa_options = null)
    {
        ...
        if ($pb_display_progress) {
            ...
            print _t(
                "Note that if you're using an external search service like ElasticSearch, the data may only now be sent to the actual service because it was buffered until now. So you still might have to wait a while for the script to finish.\n"
            );
        }
        if ($ps_callback) {
            $ps_callback(
        ...
        foreach ($pa_ids as $vn_i => $vn_id) {
            if ($va_element_ids && (!($vn_i % 200))) {    // Pre-load attribute values for next 200 items to index; improves index performance
                ca_attributes::prefetchAttributes(
                    $this->getDb(),
                    $vn_table_num,
                    $va_id_slice = array_slice($pa_ids, $vn_i, 200),
                    $va_element_ids
                );
    ...
    private function _genHierarchicalPath($pn_subject_row_id, $ps_field, $t_subject, $pa_options = null)
    {
        ...
        $va_ids = $t_subject->getHierarchyAncestors(
            $pn_subject_row_id,
    ...
    private function _genIndexInheritance(
        ...
        if (caGetOption('CHILDREN_INHERIT', $pa_data, false) || (array_search(
                    'CHILDREN_INHERIT',
                    $pa_data,
                    true
                ) !== false)) {
            ...
            if (is_array($va_ids = $pt_subject->getHierarchy($pn_subject_row_id, ['idsOnly' => true]))) {
                foreach ($va_ids as $vn_id) {
                    ...
                    $o_indexer->opo_engine->indexField(
                        $is_generic ? $subject_table_num : $vn_rel_table_num,
                        $ps_field_num,
                        $pn_content_row_id,
                        $pa_values_to_index,
                        array_merge($pa_data)
        ...
        if (caGetOption('ANCESTORS_INHERIT', $pa_data, false) || (array_search(
                    'ANCESTORS_INHERIT',
                    $pa_data,
                    true
                ) !== false)) {
    ...
    public function indexRow(
        ...
        $pa_exclusion_list = null,
        $pa_changed_fields = null,
        $pa_options = null
    ) {
        $vb_initial_reindex_mode = $pb_reindex_mode;
        $for_current_value_reindex = caGetOption('forCurrentValueReindex', $pa_options, false);
        if (!$pb_reindex_mode && !$for_current_value_reindex && is_array($pa_changed_fields) && !sizeof(
                $pa_changed_fields
            )) {
            return;
        ...
        if (is_array(
                $pa_exclusion_list[$pn_subject_table_num]
            ) && (isset($pa_exclusion_list[$pn_subject_table_num][$pn_subject_row_id]))) {
            return;
        }

        if (caGetOption('queueIndexing', $pa_options, false) && !$t_subject->getAppConfig()->get(
        ...
        $vb_can_do_incremental_indexing = $this->opo_engine->can(
            'incremental_reindexing'
        ) ? true : false;        // can the engine do incremental indexing? Or do we need to reindex the entire row every time?
        ...
        if (!$for_current_value_reindex) {
            ...
            if ($t_subject->isHierarchical(
                ) && isset($pa_changed_fields['parent_id']) && $pa_changed_fields['parent_id'] && method_exists(
                    $t_subject,
                    "makeSearchResult"
                )) {
                ...
                foreach ($va_fields_to_index as $vs_field => $va_data) {
                    ...
                    if (substr($vs_field, 0, 14) === '_ca_attribute_') {
                        ...
                        if ($va_data['datatype'] === 0) {
                            ...
                            if (is_array(
                                $child_ids = ca_metadata_elements::getElementsForSet(
                                    $va_matches[1],
                                    ['idsOnly' => true]
                                )
                            )) {
                        ...
                        //
                        if (((isset($va_data['INDEX_ANCESTORS']) && $va_data['INDEX_ANCESTORS']) || in_array(
                                'INDEX_ANCESTORS',
                                $va_data,
                                true
                            ))) {
                            if ($t_subject && $t_subject->isHierarchical()) {
                                ...
                                if ($va_hier_values = $this->_genHierarchicalPath(
                                    $pn_subject_row_id,
                                    $vs_field,
                                    $t_subject,
                                    $va_data
                                ...
                                $va_children_ids = $t_subject->getHierarchyAsList(
                                    $pn_subject_row_id,
                                    array('idsOnly' => true)
                                );
                                ...
                                if (!$pb_reindex_mode && is_array($va_children_ids) && sizeof($va_children_ids) > 0) {
                                    ...
                                    $o_indexer = new SearchIndexer($this->opo_db);
                                    $qr_children_res = $t_subject->makeSearchResult(
                                        $vs_subject_tablename,
                                        $va_children_ids,
                                        array('db' => $this->getDb())
                                    );
                        ...
                        if (((isset($va_data['INDEX_AS_IDNO']) && $va_data['INDEX_AS_IDNO']) || in_array(
                                    'INDEX_AS_IDNO',
                                    $va_data,
                                    true
                                )) && method_exists(
                            ...
                            if (strlen($va_data['IDNO_DELIMITERS']) || (is_array($va_data['IDNO_DELIMITERS']) && count(
                        ...
                        if (((isset($va_data['INDEX_AS_MIMETYPE']) && $va_data['INDEX_AS_MIMETYPE']) || in_array(
                                'INDEX_AS_MIMETYPE',
                                $va_data,
                                true
                            ))) {
                        ...
                        if (in_array(
                            $va_field_list[$vs_field]['FIELD_TYPE'],
                            ...
                            if (isset($va_field_list[$vs_field]['LIST_CODE']) && $va_field_list[$vs_field]['LIST_CODE']) {
                                ...
                                $va_labels = $t_item->getPreferredDisplayLabelsForIDs(
                                    array((int)$pa_field_data[$vs_field]),
                                ...
                                if (is_array(
                                        $va_rels = Datamodel::getManyToOneRelations($vs_subject_tablename)
                                    ) && ($va_rels[$vs_field])) {
        ...
        if (!$vb_can_do_incremental_indexing || $pb_reindex_mode || $for_current_value_reindex) {
            if (is_array($va_related_tables = $this->getRelatedIndexingTables($pn_subject_table_num))) {
                ...
                if (is_array($va_self_info['related']['fields']) && sizeof(
                        $va_self_info['related']['fields']
                ...
                if (is_array($va_self_info['related']['types']) && sizeof($va_self_info['related']['types'])) {
                    $va_restrict_self_indexing_to_types = caMakeTypeIDList(
                        $vs_subject_tablename,
                ...
                foreach ($va_related_tables as $vs_related_table) {
                    ...
                    $va_info = $this->getTableIndexingInfo(
                        $vs_subject_tablename,
                    ...
                    if (is_array($policies = $va_info['current_values'])) {
                        foreach ($policies as $p => $pinfo) {
                            $history = $t_subject->getHistory(
                                ['policy' => $p, 'limit' => 1, 'row_id' => $pn_subject_row_id]
                            ); //'currentOnly' => true,
                    ...
                    $va_query_info = $this->_getQueriesForRelatedRows(
                        $t_subject,
                        $pn_subject_row_id,
                        $t_rel,
                        $pb_reindex_mode,
                    ...
                    if ($vb_index_count = (isset($va_fields_to_index['_count']) && is_array(
                            $va_fields_to_index['_count']
                        ))) {
                        $va_counts = $this->_getInitedCountList($t_rel);
                    }

                    foreach ($va_queries as $vn_i => $va_query) {
                        $va_linking_table_config = is_array(
                        ...
                        foreach ($va_linking_table_config as $vs_linking_table => $va_linking_config) {
                            if (is_array($va_linking_config) && sizeof(
                                    $va_linking_config
                                ) && isset($va_linking_config['PRIVATE']) && Datamodel::isRelationship(
                                    $vs_linking_table
                                )) {
                                $va_private_rel_types = caMakeRelationshipTypeIDList(
                                    $vs_linking_table,
                        ...
                        if ($this->opo_db->numErrors()) {
                            ...
                            throw new Exception(
                                _t(
                                    "SQL error while getting content for index of related fields: %1; SQL was %2",
                        ...
                        if (method_exists($t_rel, "getApplicableElementCodes")) {
                            if (is_array(
                                    $va_element_ids = array_keys($t_rel->getApplicableElementCodes(null, false, false))
                                ) && sizeof($va_element_ids)) {
                                $va_rel_row_ids = $qr_res->getAllFieldValues(
                                    $vs_related_pk,
                                    [
                                        'limit' => ca_attributes::attributeCacheSize()
                                    ]
                                ); // only pull as many rows as the cache can hold, anything more than that you're just wasting time
                        ...
                        while ($qr_res->nextRow()) {
                            ...
                            if (is_a($t_rel, "BaseLabel")) {
                                $vn_rel_subject_row_id = $qr_res->get(
                                    $t_rel->getSubjectTableInstance(['dontLoadInstance' => true])->primaryKey()
                                );
                            }
                            ...
                            if (!$vb_force_related && ($vs_related_table == $vs_subject_tablename) && is_array(
                                    $va_restrict_self_indexing_to_types
                                ) && !in_array($vn_row_type_id, $va_restrict_self_indexing_to_types)) {
                                continue;
                            }
                            if (is_array($va_restrict_indexing_to_types) && !in_array(
                                    $vn_row_type_id,
                                    $va_restrict_indexing_to_types
                                )) {
                                continue;
                            }

                            $vn_private = ((!is_array($va_private_rel_types) || !sizeof(
                                    $va_private_rel_types
                            ...
                            foreach ($fields_to_index_list as $m => $field_lists) {
                                ...
                                foreach ($field_lists as $p => $field_list) {
                                    if ($m == 'current_values') {
                                        if (!isset($current_value_ids[$vn_rel_subject_row_id]) || !in_array(
                                                $p,
                                                $current_value_ids[$vn_rel_subject_row_id]
                                            )) {
                                            continue;
                                        }
                                    }
                                    foreach ($field_list as $vs_rel_field => $va_rel_field_info) {
                                        ...
                                        if (substr($vs_rel_field, 0, 14) === '_ca_attribute_') {
                                            ...
                                            if ($va_rel_field_info['DONT_INDEX'] && is_array(
                                                    $va_rel_field_info['DONT_INDEX']
                                                )) {
                                                $vb_cont = false;
                                                foreach ($va_rel_field_info["DONT_INDEX"] as $vs_exclude_type) {
                                                    if (ca_metadata_elements::getElementID($vs_exclude_type) == intval(
                                                            $va_matches[1]
                                                        )) {
                                                        $vb_cont = true;
                                                        break;
                                            ...
                                            $va_rel_field_info['datatype'] = (int)ca_metadata_elements::getElementDatatype(
                                                $va_matches[1]
                                            );
                                        ...
                                        //
                                        if (((isset($va_rel_field_info['INDEX_ANCESTORS']) && $va_rel_field_info['INDEX_ANCESTORS']) || in_array(
                                                'INDEX_ANCESTORS',
                                                $va_rel_field_info,
                                                true
                                            ))) {
                                            ...
                                            if ($t_hier_rel && ($t_hier_rel->isHierarchical() || is_subclass_of(
                                                        $t_hier_rel,
                                                        "BaseLabel"
                                                    ))) {
                                                // get hierarchy
                                                if ($va_hier_values = $this->_genHierarchicalPath(
                                                    ...
                                                    foreach ($field_nums as $field_num => $is_generic) {
                                                        $this->opo_engine->indexField(
                                                            $is_generic ? $pn_subject_table_num : $vn_related_table_num,
                                                            $field_num,
                                        ...
                                        switch ($vs_rel_field) {
                                            case '_count':
                                                if ($vb_index_count) {
                                                    ...
                                                    if ($vn_rel_type_id || $vn_row_type_id || !$t_rel->hasField(
                                                            'type_id'
                                                        )) {
                                                        $va_counts[$t_rel->isRelationship(
                                                        ) ? $vn_rel_type_id : $vn_row_type_id]++;
                                                    }
                                                }
                                                break;
                                            default:
                                                if (strlen(
                                                        $vn_fn = Datamodel::getFieldNum(
                                                            $vs_related_table,
                                                            $vs_rel_field
                                                        )
                                                    ) > 0) {
                                                    ...
                                                    foreach ($field_nums as $field_num => $is_generic) {
                                                        if (((isset($va_rel_field_info['INDEX_AS_IDNO']) && $va_rel_field_info['INDEX_AS_IDNO']) || in_array(
                                                                    'INDEX_AS_IDNO',
                                                                    $va_rel_field_info,
                                                                    true
                                                                )) && method_exists(
                                                            ...
                                                            if (strlen(
                                                                ...
                                                                $va_values = $o_idno->getIndexValues(
                                                                    $vs_fld_data,
                                                                    $va_rel_field_info
                                                                );
                                                            }
                                                            $this->opo_engine->indexField(
                                                                $is_generic ? $pn_subject_table_num : $vn_related_table_num,
                                                                $field_num,
                                                                $vn_id = $is_generic ? $pn_subject_row_id : $qr_res->get(
                                                                    $vs_related_pk
                                                                ),
                                                                $va_values,
                                                                array_merge(
                                                        ...
                                                        if (((isset($va_rel_field_info['INDEX_AS_MIMETYPE']) && $va_rel_field_info['INDEX_AS_MIMETYPE']) || in_array(
                                                                'INDEX_AS_MIMETYPE',
                                                                $va_rel_field_info,
                                                                true
                                                            ))) {
                                                            // specialized mimetype processing
                                                            $va_values = [];
                                                            if ($vs_typename = Media::getTypenameForMimetype(
                                                                $vs_fld_data
                                                            )) {
                                                                $va_values[] = $vs_typename;
                                                            }
                                                            ...
                                                            // regular intrinsic
                                                            $this->opo_engine->indexField(
                                                                $z = ($is_generic ? $pn_subject_table_num : $vn_related_table_num),
                                                                $field_num,
                                                                $vn_rid = $is_generic ? $pn_subject_row_id : $qr_res->get(
                                                                    $vs_related_pk
                                                                ),
                                                                [$vs_fld_data],
                                                                array_merge(
                            ...
                            if (!$for_current_value_reindex) {
                                ...
                                if (!$vb_force_related && ($vs_subject_tablename == $vs_related_table)) {
                                    if ($t_label = $t_rel->getLabelTableInstance()) {
                                        ...
                                        $va_label_info = $this->getTableIndexingInfo(
                                            $vs_subject_tablename,
                                            $t_label->tableName()
                                        );
                                        if (is_array($va_label_info['related']['fields']) && sizeof(
                                                $va_label_info['related']['fields']
                                            )) {
                                            $vn_label_table_num = $t_label->tableNum();

                                            $vb_skip = false;
                                            if (is_array($va_restrict_self_indexing_to_types) && $t_rel->load(
                                                    $vn_row_id
                                                ) && !in_array(
                                                    $t_rel->getTypeID(),
                                                    $va_restrict_self_indexing_to_types
                                            ...
                                            if (!$vb_skip && is_array(
                                                    $va_labels = $t_rel->getPreferredLabels(
                                                        null,
                                                        false,
        ...
        if ((!$vb_initial_reindex_mode) && (sizeof($pa_changed_fields) > 0)) {
            ...
            //
            $va_rows_to_reindex = $this->_getDependentRowsForSubject(
                $pn_subject_table_num,
                $pn_subject_row_id,
                $va_deps,
                $va_changed_field_nums
            );

            if ($vb_can_do_incremental_indexing) {
                if (method_exists($vs_subject_tablename, "getDependentCurrentValues")) {
                    $current_values = $vs_subject_tablename::getDependentCurrentValues(
                        $pn_subject_table_num,
                        $pn_subject_row_id,
                        ['db' => $this->getDb()]
                    );
                ...
                foreach ($va_rows_to_reindex_by_row_id as $va_row_to_reindex) {
                    ...
                    if (substr($va_row_to_reindex['field_name'], 0, 14) == '_ca_attribute_') {        // is attribute
                        $va_row_to_reindex['indexing_info']['datatype'] = ca_metadata_elements::getElementDatatype(
                            substr($va_row_to_reindex['field_name'], 14)
                        );
                    }

                    if ($va_row_to_reindex['field_name'] == '_count') {
                        foreach ($va_row_to_reindex['row_ids'] as $vn_subject_row_id) {
                            $this->_doCountIndexing(
                                Datamodel::getInstanceByTableNum($va_row_to_reindex['table_num'], true),
                                $vn_subject_row_id,
                                $t_rel,
                                false
                            );
                    ...
                    if (((isset($va_row_to_reindex['indexing_info']['INDEX_ANCESTORS']) && $va_row_to_reindex['indexing_info']['INDEX_ANCESTORS']) || in_array(
                            'INDEX_ANCESTORS',
                            $va_row_to_reindex['indexing_info'],
                            true
                        ))) {
                        ...
                        foreach ($va_row_to_reindex['row_ids'] as $vn_row_id) {
                            $va_content = $this->_genHierarchicalPath(
                                $va_row_to_reindex['field_row_id'],
                                $va_row_to_reindex['field_name'],
                                $t_label,
                                $va_row_to_reindex['indexing_info']
                            );
                    ...
                    } elseif (((isset($va_row_to_reindex['indexing_info']['INDEX_AS_IDNO']) && $va_row_to_reindex['indexing_info']['INDEX_AS_IDNO']) || in_array(
                                'INDEX_AS_IDNO',
                                $va_row_to_reindex['indexing_info'],
                                true
                            )) && method_exists(
                        ...
                        switch ($va_row_to_reindex['indexing_info']['datatype']) {
                            case __CA_ATTRIBUTE_VALUE_CONTAINER__:        // container
                                ...
                                foreach ($va_row_to_reindex['row_ids'] as $vn_rel_row_id) {
                                    $this->opo_engine->startRowIndexing(
                                        $va_row_to_reindex['table_num'],
                                        $vn_rel_row_id
                                    );

                                    $va_attributes = $t_rel->getAttributesByElement(
                                        $vs_element_code,
                                    ...
                                    if (sizeof($va_attributes)) {
                                        foreach ($va_attributes as $vo_attribute) {
                                            foreach ($vo_attribute->getValues() as $vo_value) {
                                                ...
                                                $vn_list_id = ca_metadata_elements::getElementListID(
                                                    $vn_sub_element_id
                                                );
                                                ...
                                                if (is_array($va_additional_indexing) && (sizeof(
                                                            $va_additional_indexing
                                                        ) > 0)) {
                                                    foreach ($va_additional_indexing as $vs_additional_value) {
                                                        $vs_value_to_index .= " ; " . $vs_additional_value;
                                                    }
                                                }
                                                if ($t_to_reindex = Datamodel::getInstanceByTableNum(
                                                    $va_row_to_reindex['table_num'],
                                                    true
                                                )) {
                                                    if (is_array($va_row_to_reindex['indexing_info'])) {
                                        ...
                                        $va_sub_elements = $this->opo_metadata_element->getElementsInSet(
                                            $vs_element_code
                                        );
                                        ...
                                        foreach ($va_sub_elements as $vn_i => $va_element_info) {
                                            if ($t_to_reindex = Datamodel::getInstanceByTableNum(
                                                ...
                                                if (is_array($va_row_to_reindex['indexing_info'])) {
                                                    $this->opo_engine->removeRowIndexing(
                                                        $va_row_to_reindex['table_num'],
                                                        $vn_rel_row_id,
                                                        $va_row_to_reindex['field_table_num'],
                            ...
                            case __CA_ATTRIBUTE_VALUE_LIST__:            // list
                                ...
                                if (is_array($va_row_to_reindex['cv_indexing_info'])) {
                                    foreach ($va_row_to_reindex['cv_indexing_info'] as $p => $pinfo) {
                                        ...
                                        $row_ids = array_intersect(
                                            array_keys($current_values[$p][$va_row_to_reindex['table_num']]),
                                            $va_row_to_reindex['row_ids']
                                        );
                                ...
                                break;
                            default:
                                if (method_exists($t_rel, 'getAttributesByElement') && is_array(
                                        $va_attributes = $t_rel->getAttributesByElement(
                                            $vs_element_code,
                                            array('row_id' => $va_row_to_reindex['field_row_id'])
                                        )
                                        ...
                                        foreach ($vo_attribute->getValues() as $vo_value) {
                                            ...
                                            if (is_array($va_row_to_reindex['cv_indexing_info'])) {
                                                foreach ($va_row_to_reindex['cv_indexing_info'] as $p => $pinfo) {
                                                    ...
                                                    $row_ids = array_intersect(
                                                        array_keys(
                                                            $current_values[$p][$va_row_to_reindex['table_num']]
                                                        ),
                                                        $va_row_to_reindex['row_ids']
                                                    );
                                                    if (!$row_ids || !is_array($row_ids) || !sizeof($row_ids)) {
                ...
                foreach ($va_rows_to_reindex as $va_row_to_reindex) {
                    ...
                    if (is_array(
                            $pa_exclusion_list[$va_row_to_reindex['table_num']]
                    ...
                    $this->opo_engine->removeRowIndexing(
                        ...
                        null,
                        null,
                        $va_row_to_reindex['relationship_type_id']
                    );
    ...
    private function _indexAttribute($pt_subject, $pn_row_id, $pm_element_code_or_id, $pa_data, $pa_options = null)
    {
        ...
        $vn_datatype = isset($pa_data['datatype']) ? $pa_data['datatype'] : ca_metadata_elements::getElementDatatype(
            $pm_element_code_or_id
        );

        switch ($vn_datatype) {
            case __CA_ATTRIBUTE_VALUE_CONTAINER__:        // container
                $vn_element_id = is_numeric(
                    $pm_element_code_or_id
                ) ? $pm_element_code_or_id : ca_metadata_elements::getElementID($pm_element_code_or_id);
                ...
                if ($vn_count = sizeof($va_attributes)) {
                    ...
                    if ($private_when = caGetOption('PRIVATE_WHEN', $pa_data, null)) {
                        foreach ($va_attributes as $i => $vo_attribute) {
                            foreach ($vo_attribute->getValues() as $vo_value) {
                                $map[$i][ca_metadata_elements::getElementCodeForId(
                                    $vo_value->getElementID()
                    ...
                    foreach ($va_attributes as $i => $vo_attribute) {
                        ...
                        $va_sub_element_ids = $this->opo_metadata_element->getElementsInSet(
                            $vn_element_id,
                            true,
                            array('idsOnly' => true)
                        );
                        if (is_array($va_sub_element_ids) && sizeof($va_sub_element_ids)) {
                            ...
                            foreach ($vo_attribute->getValues() as $vo_value) {
                                ...
                                if (!in_array(
                                    $vs_raw_display_value = $vo_value->getDisplayValue(),
                                    $va_values_to_index
                                )) {
                                ...
                                if (isset($pa_data['_ca_attribute_' . $vn_sub_element_id]) && is_array(
                                        $pa_data['_ca_attribute_' . $vn_sub_element_id]
                                    )) {
                                    $va_sub_data = array_merge(
                                        $pa_data,
                                        $pa_data['_ca_attribute_' . $vn_sub_element_id]
                                    );
                                }
                                $va_sub_data = array_filter(
                                    $va_sub_data,
                                    function ($v) {
                            ...
                            foreach (array_keys($va_sub_element_ids) as $vn_e) {
                                $va_sub_data = $pa_data;
                                if (isset($pa_data['_ca_attribute_' . $vn_e]) && is_array(
                                        $pa_data['_ca_attribute_' . $vn_e]
                                    )) {
                    ...
                    if (is_array(
                        ...
                        foreach ($va_sub_elements as $vn_i => $va_element_info) {
                            ...
                            if (isset($pa_data['_ca_attribute_' . $sub_element_id]) && is_array(
                                    $pa_data['_ca_attribute_' . $sub_element_id]
                                )) {
                                $va_sub_data = array_merge($pa_data, $pa_data['_ca_attribute_' . $sub_element_id]);
                            }
                            $this->opo_engine->indexField(
                                $pn_subject_table_num,
                                $field_num_prefix . $sub_element_id,
                                $pn_row_id,
                                [''],
                                $va_sub_data
                            );
                        ...
                        foreach ($vo_attribute->getValues() as $vo_value) {
                            ...
                            if ($vn_datatype == __CA_ATTRIBUTE_VALUE_LIST__) {
                                ...
                                $this->_genIndexInheritance(
                                    ...
                                    $t_inheritance_subject ? $pt_subject : null,
                                    $field_num_prefix . $vn_element_id,
                                    $pn_inheritance_subject_id ? $pn_inheritance_subject_id : $pn_inheritance_subject_id ? $pn_inheritance_subject_id : $pn_row_id,
                                    $pn_row_id,
                                    [$vs_v],
                                    array_merge($pa_data, ['DONT_TOKENIZE' => 1])
                                );
                ...
                if (is_array($va_tmp) && sizeof($va_tmp)) {
                    ...
                    if ($t_item = AuthorityAttributeValue::elementTypeToInstance($vn_datatype)) {
                        ...
                        foreach ($va_tmp as $vn_attribute_id => $vn_item_id) {
                            ...
                            foreach (
                                [
                                    "preferred_labels." . $t_item->getLabelDisplayField(),
                                    $t_item->primaryKey()
                                ] as $vs_f
                            ) {
                                if ($va_hier_values = $this->_genHierarchicalPath(
                                    ...
                                    $this->opo_engine->indexField(
                                        ...
                                        $field_num_prefix . $vn_element_id,
                                        $pn_row_id,
                                        array_merge([$vs_v], $va_hier_values['values']),
                                        $pa_data
                                    );
                                    $this->_genIndexInheritance(
                                        $t_inheritance_subject ? $t_inheritance_subject : $pt_subject,
            ...
            default:
                ...
                $va_attributes = $pt_subject->getAttributesByElement(
                    $pm_element_code_or_id,
                ...
                if (($vn_count = sizeof($va_attributes)) > 0) {
                    foreach ($va_attributes as $vo_attribute) {
                        foreach ($vo_attribute->getValues() as $vo_value) {
                            ...
                            if (((isset($pa_data['INDEX_AS_IDNO']) && $pa_data['INDEX_AS_IDNO']) || in_array(
                                        'INDEX_AS_IDNO',
                                        $pa_data,
                                        true
                                    )) && method_exists(
                                ...
                                if (strlen($pa_data['IDNO_DELIMITERS']) || (is_array(
                                            $pa_data['IDNO_DELIMITERS']
                ...
                // reindex children?
                if ((caGetOption('INDEX_ANCESTORS', $pa_data, false) !== false) || (in_array(
                        'INDEX_ANCESTORS',
                        $pa_data,
                        true
                    ))) {
                    if ($pt_subject && $pt_subject->isHierarchical()) {
                        if ($va_hier_values = $this->_genHierarchicalPath(
                            $pn_row_id,
                            $vs_element_code = ca_metadata_elements::getElementCodeForId(
                                $vn_element_id
                            ),
                            $pt_subject,
                            $pa_data
                        ...
                        $va_children_ids = $pt_subject->getHierarchyAsList(
                            $pn_row_id,
                            array(
                                'idsOnly' => true,
                                'includeSelf' => false
                        ...
                        if (!caGetOption('reindex', $pa_options, false) && is_array($va_children_ids) && sizeof(
                                $va_children_ids
                            ) > 0) {
                            // trigger reindexing of children
                            $o_indexer = new SearchIndexer($this->opo_db);
                            $pt_subject->load($pn_row_id);
                            $va_content = $pt_subject->get(
                                $pt_subject->tableName() . "." . $vs_element_code,
                                array(
    ...
    public function startRowUnIndexing($pn_subject_table_num, $pn_subject_row_id)
    {
        ...
        if (in_array($pn_subject_table_num, array(3, 4)) || isset($va_indexed_tables[$pn_subject_table_num]) || sizeof(
                $va_deps
            )) {
            $this->opa_dependencies_to_update = $this->_getDependentRowsForSubject(
                $pn_subject_table_num,
                $pn_subject_row_id,
                $va_deps
            );
    ...
    public function commitRowUnIndexing($pn_subject_table_num, $pn_subject_row_id, $pa_options = null)
    {
        ...
        if (caGetOption('queueIndexing', $pa_options, false) && !$this->opo_app_config->get(
                'disable_out_of_process_search_indexing'
            )) {
        ...
        if (is_array($this->opa_dependencies_to_update)) {
            ...
            if (!$vb_can_do_incremental_indexing) {
                ...
                foreach ($this->opa_dependencies_to_update as $va_item) {
                    ...
                    $this->_doCountIndexing(
                        Datamodel::getInstanceByTableNum($va_item['table_num'], true),
                        $va_item['row_id'],
                        $t_subject,
                        false
                    );
                ...
                foreach ($this->opa_dependencies_to_update as $va_item) {
                    $this->opo_engine->removeRowIndexing(
                        $va_item['table_num'],
                        $va_item['row_id'],
                        $va_item['field_table_num'],
                        $va_item['field_nums'],
                        $va_item['field_row_id'],
                        $va_item['relationship_type_id']
                    );
                    ...
                    if (is_array($va_item['cv_indexing_info'])) {
                        ...
                        foreach ($va_item['cv_indexing_info'] as $f => $finfo) {
                            ...
                            foreach ($finfo as $p => $pinfo) {
                                if (is_array(
                                    $fn = self::_searchConfigFieldEntryToIndexFieldNums(
                                        $va_item['field_table_num'],
                                        $f,
                                        ['policy' => $p]
                                    )
                    ...
                    $this->_doCountIndexing(
                        $t_rel = Datamodel::getInstanceByTableNum($va_item['table_num'], true),
                        $va_item['row_id'],
                        $t_subject,
                        false
                    );
                    ...
                    if ($t_subject->isRelationship()) {
                        $this->_doCountIndexing(
                            $t_rel,
                            $va_item['row_id'],
                            Datamodel::getInstanceByTableName(
                                $x = $t_subject->getOppositeTableName($t_rel->tableName()),
                                true
                            ),
                            false
                        );
            ...
            if (
                ...
                if (($vs_subject_tablename === $vs_dep_table) && ($vs_self_rel_table_name = $t_dep->getSelfRelationTableName(
                    ...
                    $vs_sql = "
						...
						{$vs_sql_joins}
						WHERE
							(t0." . $t_self_rel->getLeftTableFieldName(
                        ) . " = ? OR t0." . $t_self_rel->getRightTableFieldName() . " = ?)
							{$vs_delete_sql}
					";
                } else {
                    //
                ...
                while ($qr_res->nextRow()) {
                    ...
                    if ($t_label = $t_dep->getLabelTableInstance()) {
                        ...
                        if (is_array($va_label_info['related']['fields']) && sizeof(
                            ...
                            foreach ($va_label_info['related']['fields'] as $vs_label_field => $va_config) {
                                $vn_label_field_num = $this->_getFieldNumForIndexing(
                                    $t_label,
                                    $vn_label_table_num,
                                    $vs_label_field
                                );
...
// loop through the tables for each relationship between the subject and the dep
                ...
                foreach ($va_table_list_list as $vs_list_name => $va_linking_tables_config) {
                    ...
                    $va_linking_tables = is_array($va_linking_tables) ? array_reverse(
                        $va_linking_tables
                    ...
                    array_push(
                        $va_linking_tables,
                        $vs_dep_table
                    );                                                            // the dep table name is not listed in the config file (it's redundant)
...
// update indexing for each relationship
                foreach ($va_rel_tables_to_index_list as $vs_rel_table) {
                    ...
                    foreach ($va_table_path as $vs_n => $va_linking_tables_config) {
                        $va_table_list = (caIsIndexedArray(
                            $va_linking_tables_config
                        )) ? $va_linking_tables_config : array_keys($va_linking_tables_config);
                        if (caIsIndexedArray($va_linking_tables_config)) {
                            $va_tmp = array();
                            foreach ($va_linking_tables_config as $vs_t) {
                                $va_tmp[$vs_t] = [];
                        ...
                        $va_cv_fields_to_index = $this->getFieldsToIndex(
                            $vn_dep_table_num,
                            $vs_rel_table,
                        ...
                        if ($vs_rel_table == $vs_subject_tablename) {
                            if (is_array($pa_changed_field_nums) && !$this->_indexedFieldsHaveChanged(
                                    $all_fields_to_index,
                                    $pa_changed_field_nums
                                )) {
                                continue;
                        ...
                        $va_rows = $this->_getRelatedRows(
                            array_reverse($va_full_path),
                            $va_linking_tables_config,
                            isset($va_table_key_list[$vs_list_name]) ? $va_table_key_list[$vs_list_name] : null,
                            $vs_subject_tablename,
                            $pn_subject_row_id,
                            $vs_rel_table ? $vs_rel_table : $vs_dep_table,
                            $all_fields_to_index
                        );
                                ...
                                foreach ($all_fields_to_index as $vs_field => $va_indexing_info) {
                                    switch ($vs_field) {
                                        case '_count':
                                            ...
                                            break;
                                        default:
                                            $vn_fld_num = $this->_getFieldNumForIndexing(
                                                $t_rel,
                                                $vn_rel_table_num,
                                                $vs_field
                                            );
                                    ...
                                    foreach ($va_cv_fields_to_index as $p => $indexing_config) {
                                        if (isset($indexing_config[$vs_field]) && is_array(
                                                $indexing_config[$vs_field]
                                            )) {
                                    ...
                                    if (method_exists($t_dep, "getAuthorityElementReferences") && is_array(
                                            $va_element_references = $t_dep->getAuthorityElementReferences(
                                                array('row_id' => $vn_row_id)
                                            )
                                        )) {
                                        foreach ($va_element_references as $vn_element_table_num => $va_references) {
                                            ...
                                            $va_element_fields_to_index = $this->getFieldsToIndex(
                                            ...
                                            $va_field_data = [];
                                            while ($qr_field_data->nextRow()) {
                                                $va_field_data[(int)$qr_field_data->get(
                                                    $vs_element_table_pk
    ...
    private function _getRelatedRows(
        ...
        if (true) { //!MemoryCache::contains($vs_key, 'SearchIndexerRelatedRowsJoins')) {
            ...
            foreach ($pa_tables as $vs_right_table) {
                ...
                $vs_rel_type_res_sql = '';
                if (($va_type_res = $pa_linking_tables_config[$vs_right_table]['types']) && is_array(
                        $va_type_res
                    ) && sizeof($va_type_res)) {
                ...
                if (is_array(
                        $pa_table_keys
                    ) && (isset($pa_table_keys[$vs_right_table][$vs_left_table]) || isset($pa_table_keys[$vs_left_table][$vs_right_table]))) {        // are the keys for this join specified in the indexing config?
                    if (isset($pa_table_keys[$vs_left_table][$vs_right_table])) {
                        ...
                        if ($va_key_spec['left_table_num'] || $va_key_spec['right_table_num']) {
                            if ($va_key_spec['right_table_num']) {
                                $vs_join .= " AND {$vs_alias}.{$va_key_spec['right_table_num']} = " . Datamodel::getTableNum(
                                        $vs_left_table
                                    );
                                $vs_t = $vs_right_table;
                            } else {
                                $vs_join .= " AND {$vs_prev_alias}.{$va_key_spec['left_table_num']} = " . Datamodel::getTableNum(
                                        $vs_right_table
                                    );
                                $vs_t = $vs_left_table;
                            }
                        ...
                        if ($va_key_spec['left_table_num'] || $va_key_spec['right_table_num']) {
                            if ($va_key_spec['right_table_num']) {
                                $vs_join .= " AND {$vs_prev_alias}.{$va_key_spec['right_table_num']} = " . Datamodel::getTableNum(
                                        $vs_right_table
                                    );
                                $vs_t = $vs_left_table;
                            } else {
                                $vs_join .= " AND {$vs_alias}.{$va_key_spec['left_table_num']} = " . Datamodel::getTableNum(
                                        $vs_left_table
                                    );
                                $vs_t = $vs_right_table;
                            }
                ...
                if (($t_table = Datamodel::getInstanceByTableName($vs_right_table, true)) && ($t_table->isRelationship(
                        ) && $t_table->hasField('type_id'))) {
            ...
            if ($t_indexed_table = Datamodel::getInstanceByTableName($ps_table_to_index, true)) {
                foreach ($pa_fields_to_index as $vs_f => $va_field_info) {
                    ...
                    if (in_array(
                        $t_indexed_table->getFieldInfo($vs_f, 'FIELD_TYPE'),
            ...
            foreach ($va_tmp as $vn_i => $vs_v) {
                if (in_array(
                    $t_select->getFieldInfo($vs_v, 'FIELD_TYPE'),
                    array(FT_MEDIA, FT_FILE, FT_VARS, FT_DATERANGE, FT_HISTORIC_DATERANGE, FT_TIMERANGE)
                )) {
                    continue;
                }
                ...
                foreach ($va_table_list_list as $vs_list_name => $va_linking_tables_config) {
                    ...
                    if (in_array(
                        ...
                        foreach ($va_table_list as $vs_tablename) {
                            ...
                            if ($vs_last_table) {
                                if ($va_rel = Datamodel::getOneToManyRelations(
                                    $vs_tablename,
                                    $vs_last_table
    ...
    private function _getQueriesForRelatedRows(
        ...
        $va_table_info = $this->getTableIndexingInfo(
            $vs_subject_tablename,
        ...
        if (!$pb_force_related && ($vs_subject_tablename == $vs_related_table)) {
            ...
            $vs_sql = "
				...
				INNER JOIN {$vs_self_rel_table_name} ON {$vs_self_rel_table_name}." . $t_self_rel->getLeftTableFieldName(
                ) . " = {$vs_related_table}.{$vs_related_pk}
				WHERE
				...
				SELECT ' . join(",", $va_proc_field_list) . "
				FROM {$vs_related_table}
				INNER JOIN {$vs_self_rel_table_name} ON {$vs_self_rel_table_name}." . $t_self_rel->getRightTableFieldName(
                ) . " = {$vs_related_table}.{$vs_related_pk}
				WHERE
            ...
            if (!is_array(
                $va_fields_to_index = $this->getFieldsToIndex(
                    $vs_subject_tablename,
                    $pb_force_related ? "{$vs_related_table}.related" : $vs_related_table
                )
            )) {
            ...
            foreach ($va_table_list_list as $vs_list_name => $va_linking_tables_config) {
                ...
                foreach ($va_linking_tables as $vs_right_table) {
                    $va_rel_type_ids = array();
                    $vs_rel_type_res_sql = '';
                    if (($va_type_res = $va_linking_tables_config[$vs_right_table]['types']) && is_array(
                            $va_type_res
                        ) && sizeof($va_type_res)) {
                        $va_rel_type_ids = caMakeRelationshipTypeIDList($vs_right_table, $va_type_res);
                    }
                    ...
                    if (is_array(
                            $va_table_key_list
                        ) && (isset($va_table_key_list[$vs_list_name][$vs_right_table][$vs_left_table]) || isset($va_table_key_list[$vs_list_name][$vs_left_table][$vs_right_table]))) {        // are the keys for this join specified in the indexing config?
                        ...
                        if (isset($va_table_key_list[$vs_list_name][$vs_left_table][$vs_right_table])) {
                            ...
                            $vs_join = "INNER JOIN {$vs_right_table} AS {$vs_alias} ON ({$vs_alias}.{$va_key_spec['right_key']} = {$vs_prev_alias}.{$va_key_spec['left_key']}" . $vs_rel_type_res_sql . self::_genQueryFilters(
                                    $vs_right_table,
                                    $va_table_info,
                                    $va_linking_tables_config[$vs_right_table],
                                    $vs_alias
                                );
                            if ($va_key_spec['left_table_num'] || $va_key_spec['right_table_num']) {
                                if ($va_key_spec['right_table_num']) {
                            ...
                            $vs_join = "INNER JOIN {$vs_right_table} AS {$vs_alias} ON ({$vs_alias}.{$va_key_spec['left_key']} = {$vs_prev_alias}.{$va_key_spec['right_key']}" . $vs_rel_type_res_sql . self::_genQueryFilters(
                                    $vs_right_table,
                                    $va_table_info,
                                    $va_linking_tables_config[$vs_right_table],
                                    $vs_alias
                        ...
                        if (($pt_rel_instance = Datamodel::getInstanceByTableName(
                                $vs_right_table,
                                true
                            )) && method_exists($pt_rel_instance, "isRelationship") && $pt_rel_instance->isRelationship(
                        ...
                        if ($va_rel = Datamodel::getOneToManyRelations($vs_left_table, $vs_right_table)) {
                            ...
                            if (sizeof($va_rel_type_ids) > 0) {
                                $vs_rel_type_res_sql = " AND {$vs_alias}.type_id IN (" . join(
                                        ",",
                                        $va_rel_type_ids
                                    ) . ")";
                            }
                            ...
                            if (Datamodel::isSelfRelationship($va_rel['many_table'])) {
                                ...
                                $va_joins[] = [
                                    ...
                                    ) . $vs_rel_type_res_sql . self::_genQueryFilters(
                                        $vs_right_table,
                                        $va_table_info,
                                        $va_linking_tables_config[$vs_right_table],
                                        $vs_alias
                                ...
                                if (($pt_rel_instance = Datamodel::getInstanceByTableName(
                                        ...
                                        $pt_rel_instance,
                                        "isRelationship"
                                    ) && $pt_rel_instance->isRelationship() && $pt_rel_instance->hasField('type_id')) {
                                    $vs_rel_type_id_fld = "{$vs_alias}.type_id";
                                } elseif (($vn_t > 0) && ($vs_related_table == $va_rel['many_table']) && is_array(
                                        $pa_restrict_to_types
                                    ) && sizeof($pa_restrict_to_types) && $pt_rel_instance->hasField('type_id')) {
                                ...
                                $va_joins[] = [
                                    "INNER JOIN {$va_rel['many_table']} AS {$vs_alias} ON {$vs_prev_alias}.{$va_rel['one_table_field']} = {$vs_alias}.{$va_rel['many_table_field']}" . $vs_rel_type_res_sql . self::_genQueryFilters(
                                        $vs_right_table,
                                        $va_table_info,
                                        $va_linking_tables_config[$vs_right_table],
                                        $vs_alias
                            ...
                            if (Datamodel::isSelfRelationship($va_rel['many_table'])) {
                                ...
                                } elseif (($vn_t > 0) && ($vs_related_table == $va_rel['one_table']) && is_array(
                                        $pa_restrict_to_types
                                    ) && sizeof($pa_restrict_to_types) && $pt_rel_instance->hasField('type_id')) {
                                    $vs_type_id_fld = "{$vs_alias}.type_id";
                                    $vs_rel_type_res_sql .= " AND {$vs_type_id_fld} IN (" . join(
                                ...
                                $va_joins[] = [
                                    "INNER JOIN {$va_rel['one_table']} AS {$vs_alias} ON {$vs_alias}.{$va_rel['one_table_field']} = {$vs_prev_alias}.{$va_rel['many_table_field']}" . $vs_rel_type_res_sql . self::_genQueryFilters(
                                        $vs_right_table,
                                        $va_table_info,
                                        $va_linking_tables_config[$vs_right_table],
                                        $vs_alias
                ...
                for ($vn_i = 0; $vn_i < $vn_field_list_count; $vn_i++) {
                    ...
                    $va_proc_field_list[$vn_i] = $va_aliases[$vs_related_table][sizeof(
                            $va_aliases[$vs_related_table]
                        ) - 1] . '.' . $va_field_list[$vn_i];
                }
                $va_proc_field_list[] = $va_aliases[$vs_related_table][sizeof(
                        $va_aliases[$vs_related_table]
                    ) - 1] . '.' . $vs_related_pk;
                ...
                if (isset($va_rel['many_table']) && $va_rel['many_table']) {
                    $va_proc_field_list[] = $va_aliases[$va_rel['many_table']][sizeof(
                            $va_aliases[$va_rel['many_table']]
                        ) - 1] . '.' . $va_rel['many_table_field'];
                }
                ...
                switch ($k) {
                    ...
                    case 'type_id':
                        ...
                        foreach ($types as $t) {
                            if (is_numeric($t)) {
                                $type_criteria[] = (int)$t;
                            } elseif (!empty($field_info['LIST_CODE']) && ($type_id = caGetListItemID(
                                    $field_info['LIST_CODE'],
                                    $t
                                ))) {
                        ...
                        if (sizeof($type_criteria) > 0) {
                            $filter_criteria[] = "({$alias}.{$k} IN (" . join(
                                    ', ',
                                    $type_criteria
                                ) . ") OR {$alias}.{$k} IS NULL)";
                        }
            ...
            foreach ($va_queries as $va_query) {
                ...
                foreach ($va_counts as $vs_key => $vn_count) {
                    if ($pb_reindex_mode) {
                        ...
                        if ((caGetOption('CHILDREN_INHERIT', $pa_options, false) || (array_search(
                                        'CHILDREN_INHERIT',
                                        $pa_options,
                                        true
                                    ) !== false)) && (is_array(
    ...
    public function updateCurrentValueIndexing($policy, $subject_table_num, $subject_row_id, $values, $options = null)
    {
        ...
        foreach ($indexing_tables as $rel_table) {
            if (is_array(
                $fields_to_index = $this->getFieldsToIndex(
                    $subject_table_num,
                    $rel_table,
                    ['currentValueFields' => true]
                )
                ...
                $this->opo_engine->removeRowIndexing(
                    $subject_table_num,
                    $subject_row_id,
                    Datamodel::getTableNum($rel_table),
                    $field_nums
                );
            }
        }
