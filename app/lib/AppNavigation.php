...
class AppNavigation extends BaseObject {
	...
	private function _genReverseNavTable() {
		...
		while ( sizeof( $va_stack ) > 0 ) {
			...
			if ( isset( $va_node['navnode'] ) && isset( $va_node['navnode']['useActionInPath'] )
			     && intval( $va_node['navnode']['useActionInPath'] )
			) {
				$vs_controller_path .= '/' . $vs_action;
			}
			if ( isset( $va_node['navnode'] ) && isset( $va_node['navnode']['useActionExtraInPath'] )
			     && intval( $va_node['navnode']['useActionExtraInPath'] )
			     && ( sizeof( $va_tmp ) > 0 )
			) {
			...
			if ( isset( $va_node['navnode'] ) && isset( $va_node['navnode']['navigation'] )
			     && sizeof( $va_node['navnode']['navigation'] ) > 0
			) {
				// yes... push children onto stack
				$vn_level ++;
				foreach ( $va_node['navnode']['navigation'] as $vs_key => $va_info ) {
					array_push( $va_stack, array(
						'key'     => $vs_key,
						'level'   => $vn_level,
						'navnode' => $va_node['navnode']['navigation'][ $vs_key ]
					) );
				}
				$va_path[] = $va_node['key'];
			} else {
				// no
				$this->opa_reverse_nav_table[ $vs_controller_path ] = join( '/',
					array_merge( is_array( $va_path ) ? $va_path : array(), array( $va_node['key'] ) ) );
			}
		...
		foreach ( $va_aliases_to_resolve as $vs_alias_controller_path => $vs_alias_nav_path ) {
			$this->opa_reverse_nav_table[ $vs_alias_controller_path ]
				= $this->opa_reverse_nav_table[ $vs_alias_nav_path ];
		}
	}
	...
	public function getDestination( $pb_include_action = false, $pb_include_action_extra = false ) {
		...
		return '/' . $this->opo_request->getModulePath() . '/' . $this->opo_request->getController()
		       . ( $pb_include_action ? '/' . $vs_action : '' ) . ( $pb_include_action_extra ? '/' . $vs_action_extra
				: '' );
	}
		...
		foreach ( $va_tmp as $vs_part ) {
			if ( $va_node[ $vs_part ] ) {
				...
				if ( $va_node['type'] == 'dynamic' ) {
					...
					if ( $vb_submenu_set ) {
						if ( isset( $va_node['submenu']['type'] ) && ( $va_node['submenu']['type'] == 'dynamic' )
						     && is_array( $va_sub_menu = $this->getDynamicSubmenu( $va_node['submenu'] ) )
						) {
							if ( isset( $va_node['submenu']['breadcrumbHints'] )
							     && is_array( $va_node['submenu']['breadcrumbHints'] )
							) {
								if ( $vs_trail_item
									= $this->_getBreadcrumbHint( $va_node['submenu']['breadcrumbHints'] )
								) {
				...
				if ( is_array( $va_node ) ) {
					foreach ( $va_node as $vs_key => $va_menu ) {
						if ( isset( $va_menu['handler'] ) && isset( $va_menu['type'] ) && $va_menu['handler']
						     && ( $va_menu['type'] == 'dynamic' )
						) {
	...
	/**
	 * Navigation.conf supports the ability to key navigation "breadcrumb" text for specific menu items to the presence
	 * of specific request parameters. This is needed to handle sections of navigation.conf that are reused across
	 * several different user actions (both creating new records and editing existing ones, for example).
	 * _getBreadcrumbHint() extracts relevant text based upon configuration and request parameters and returns it. Will
	 * return null if there are no relevant breadcrumb hints.
	 */
	 ...
	 * file. This data can be used to generate navigation controls using various markup schemes (eg. HTML as done by
	 * getHTMLMenuBar())
	 */
	public function &getNavInfo( $pn_level = 0 ) {
		...
		while ( sizeof( $va_path ) && ( $vn_i < $pn_level ) ) {
			...
			$va_nav_info = isset( $va_nav_info[ $vs_path_element ]['navigation'] )
				? $va_nav_info[ $vs_path_element ]['navigation'] : null;
	...
	public function getHTMLMenuBar( $ps_css_id ) {
		...
		if ( ( ( time() - Session::getVar( 'ca_nav_menubar_cache_lasttime' ) ) < 600 )
		     && ( intval( $this->opo_config->get( 'do_menu_bar_caching' ) ) > 0 )
		     && ( $vs_menu_cache = Session::getVar( 'ca_nav_menubar_cache' ) )
		) {
	...
	public function getHTMLMenuBarAsLinkArray() {
		...
		if ( ( intval( $this->opo_config->get( 'do_menu_bar_caching' ) ) > 0 )
		     && ( $va_menu_cache
				= Session::getVar( 'ca_nav_menubar_link_cache' ) )
		) {
		...
		foreach ( $va_nav_info as $vs_key => $va_menu ) {
			...
			$va_links[] = caNavLink( $this->opo_request, $va_menu['displayName'], '',
				trim( $va_menu['default']['module'] ), trim( $va_menu['default']['controller'] ),
				trim( $va_menu['default']['action'] ) );
		}
	...
	public function getHTMLSideNav( $ps_css_id ) {
		...
		$va_nav_info
			    = $this->getNavInfo( 2 ); // get third-level navigation (zero-indexed); first two levels are in top-level nav bar
		$vs_buf = '';
		if ( is_array( $va_nav_info ) ) {
			...
			foreach ( $va_nav_info as $vs_key => $va_menu ) {
				if ( isset( $va_menu['navigation'] ) && is_array( $va_menu['navigation'] ) ) {
					if ( $vs_menu_item = $this->_genMenuItem( $va_menu, $vs_key, $vs_base_path, $vs_cur_selection,
						"nav_{$vs_key}", array( 'has_children' => true ),
						array( 'onclick' => "$(\"#subNav_{$vs_key}\").slideToggle(350); return false;" ) )
					) {
	...
	public function getHTMLWidgets() {
		...
		foreach ( $va_widgets_config as $vs_key => $va_info ) {
			if ( preg_match( '!^/' . $va_info['domain']['module'] . '/' . $va_info['domain']['controller'] . '$!i',
				$vs_cur_selection )
			) {
				...
				if ( ! $va_info['handler']['isplugin'] ) {
					if ( ! include_once( $this->ops_controller_path . '/' . $va_info['handler']['module'] . '/'
					                     . $vs_classname . '.php' )
					) {
					...
					if ( ! include_once( $this->opo_config->get( 'application_plugins' ) . '/'
					                     . $va_info['handler']['module'] . '/controllers/' . $vs_classname . '.php' )
					) {
						$this->postError( 2300, _t( "Invalid controller path" ), "AppNavigation->getHTMLWidgets()" );
				...
				$o_action_controller = new $vs_classname( $this->opo_request, $this->opo_response,
					$this->opo_request->config->get( 'views_directory' ) . '/' . $va_info['handler']['module'] );
	...
	public function getDynamicNavigation( $va_info ) {
		...
		if ( ! include_once( $this->ops_controller_path . '/' . $va_info['handler']['module'] . '/' . $vs_classname
		                     . '.php' )
		) {
			// Invalid controller path
		...
		if ( $o_action_controller->numErrors() ) {
			$this->postError( 2300, _t( "Controller error: %1", join( '; ', $o_action_controller->getErrors() ) ),
				"AppNavigation->getDynamicNavigation()" );
	...
	public function getDynamicSubmenu( $va_info ) {
		...
		if ( $o_action_controller->numErrors() ) {
			$this->postError( 2300, _t( "Controller error: %1", join( '; ', $o_action_controller->getErrors() ) ),
				"AppNavigation->getDynamicSubmenu()" );
	...
	public function addNavItem(
		$ps_display_name, $ps_menu_name, $pa_defaults, $pa_requirements, $pn_insert_index = null,
		$pa_sub_navigation = null
	) {
		$this->opa_nav_config = $this->_addNavItem( $this->opa_nav_config, $ps_display_name, $ps_menu_name,
			$pa_defaults, $pa_requirements, $pn_insert_index, $pa_sub_navigation );
	}

	# -------------------------------------------------------
	private function _addNavItem(
		&$pa_menu_info, $ps_display_name, $ps_menu_name, $pa_defaults, $pa_requirements, $pn_insert_index = null,
		$pa_sub_navigation = null
	) {
		if ( isset( $pn_insert_index ) && ( $pn_insert_index >= 0 )
		     && ( $pn_insert_index < sizeof( $pa_menu_info ) )
		) {
		...
		if ( isset( $pn_insert_index ) && ( $pn_insert_index >= 0 )
			...
			if ( sizeof( $va_tmp ) < ( sizeof( $pa_menu_info ) + 1 ) ) {
				$va_tmp = array_merge( $va_tmp,
					array_slice( $pa_menu_info, $pn_insert_index, ( sizeof( $pa_menu_info ) - $pn_insert_index ),
						true ) );
			}
		}
		$pa_menu_info =& $va_tmp;
	...
	private function &_genMenu( &$pa_navinfo, $ps_base_path, $ps_cur_selection ) {
		...
		foreach ( $pa_navinfo as $vs_nav => $va_nav_info ) {
			...
			if ( ! isset( $pa_navinfo[ $vs_nav ]['displayName'] )
			     || ( ! $vs_display_name
					= $pa_navinfo[ $vs_nav ]['displayName'] )
			) {
				$vs_display_name = $vs_nav;
			}
			$va_additional_params = $this->_parseAdditionalParameters( ( isset( $pa_navinfo[ $vs_nav ] )
			                                                             && ( isset( $pa_navinfo[ $vs_nav ]['parameters'] ) ) )
				? $pa_navinfo[ $vs_nav ]['parameters'] : null );
			...
			//
			if ( isset( $pa_navinfo[ $vs_nav ]['remember_last_used_navigation'] )
			     && $pa_navinfo[ $vs_nav ]['remember_last_used_navigation']
			) {
				$va_nav_defaults
					= Session::getVar( 'ca_app_nav_defaults' );    // get stored defaults - contains the last used navigation items keyed by base path
				if ( isset( $va_nav_defaults[ $ps_base_path . '/' . $vs_nav ] ) ) {
					...
					$va_defaults = $va_top_level_nav_info[ $va_nav_defaults[ $ps_base_path . '/'
					                                                         . $vs_nav ] ]['default'];
				}
			}
			...
			if ( isset( $pa_navinfo[ $vs_nav ]['type'] ) && ( $pa_navinfo[ $vs_nav ]['type'] == 'dynamic' ) ) {
				...
				if ( sizeof( $va_submenu_nav ) ) {
					$vs_buf .= $this->_genDynamicTopLevelMenuItems( $va_submenu_nav, $vs_cur_selection,
						$va_additional_params, $ps_base_path, $va_defaults );
				}
			} else {
				$va_req         = $pa_navinfo[ $vs_nav ]['submenu']['requires'];
				$vb_submenu_set = $this->_evaluateRequirements( $va_req );
				if ( $vb_submenu_set && isset( $pa_navinfo[ $vs_nav ] ) && isset( $pa_navinfo[ $vs_nav ]['submenu'] )
				     && $pa_navinfo[ $vs_nav ]['submenu']
				) {
					if ( $pa_navinfo[ $vs_nav ]['submenu']['type'] == 'dynamic' ) {
						...
						if ( sizeof( $va_submenu_nav ) ) {
							...
							$is_link = ( is_array( $va_defaults ) && $va_defaults['module']
							             && $va_defaults['module'] == "find" );
							...
							$vs_buf .= "<li>" . ( $is_link
									? caNavLink( $this->opo_request, $vs_display_name,
										( ( $vs_cur_selection == $ps_base_path . '/' . $vs_nav ) ? 'sf-menu-selected'
											: '' ), $va_defaults['module'], $va_defaults['controller'],
										$va_defaults['action'], $va_additional_params )
									: caHTMLLink( $vs_display_name, array(
										'class' => ( ( $vs_cur_selection == $ps_base_path . '/' . $vs_nav )
											? 'sf-menu-selected' : '' ),
										'href'  => '#'
									) ) );
							$vs_buf .= $this->_genSubMenu( $va_submenu_nav, $vs_cur_selection, $va_additional_params,
								$ps_base_path, $va_defaults );
							$vs_buf .= "</li>\n";
						}
					} else {
						$vs_link = ( is_array( $va_defaults ) && $va_defaults['module'] )
							? caNavLink( $this->opo_request, $vs_display_name,
								( ( $vs_cur_selection == $ps_base_path . '/' . $vs_nav ) ? 'sf-menu-selected' : '' ),
								$va_defaults['module'], $va_defaults['controller'], $va_defaults['action'],
								$va_additional_params ) : "<a href='#'>{$vs_display_name}</a>";
						$vs_buf  .= "<li>{$vs_link}\n";
						$vs_buf  .= $this->_genSubMenu( $pa_navinfo[ $vs_nav ]['submenu']['navigation'],
							$vs_cur_selection, $va_additional_params, $ps_base_path, $va_defaults );
						$vs_buf  .= "</li>\n";
					}
				} else {
					if ( is_array( $va_defaults ) && ( sizeof( $va_defaults ) == 0 ) ) {
						...
						$vs_buf .= "<li " . ( ( $vs_last_selected_path_item == $vs_nav ) ? 'class="sf-menu-selected"'
								: '' ) . ">" . caNavLink( $this->opo_request, $vs_display_name,
								( ( $vs_last_selected_path_item == $vs_nav ) ? 'sf-menu-selected' : '' ),
								$va_defaults['module'], $va_defaults['controller'], $va_defaults['action'],
								$va_additional_params ) . "<li>\n";
					}
				}
			}
	...
	/**
	 * Rewrite "find" menu item defaults to honor settings in app.conf <table>_no_search_for_types,
	 * <table>_no_advanced_search_for_types and <table>_no_browse_for_types
	 */
	private function _rewriteDefaultsForFindMenuItems( $pa_submenu_item, $pa_defaults ) {
		if ( ( $type_id = (int) caGetOption( 'type_id', $pa_submenu_item['parameters'], null ) )
		     && preg_match( "!^([A-Z]{1}[a-z]+)(.*)$!", $pa_defaults['controller'], $m )
		     && ( in_array( $m[1], [ 'Search', 'Browse' ] )
		          && is_array( $info = caFindControllerNameInfo( $pa_defaults['controller'] ) ) )
		) {
	...
	private function _genSubMenu( $pa_submenu_nav, $ps_cur_selection, $pa_additional_params, $ps_base_path, $pa_defaults
		...
		foreach ( $pa_submenu_nav as $va_submenu_item ) {
			...
			if ( isset( $va_submenu_item ) && isset( $va_submenu_item['default'] )
			     && is_array( $va_submenu_item['default'] )
			) {
			...
			if ( isset( $va_submenu_item )
			     && ( ( isset( $va_submenu_item['is_enabled'] )
			            && intval( $va_submenu_item['is_enabled'] ) )
			          || ( is_array( $pa_defaults ) && $pa_defaults['module']
			               && $pa_defaults['module'] == "find" ) )
			) {
				$defaults_proc = $this->_rewriteDefaultsForFindMenuItems( $va_submenu_item, $pa_defaults );
				$vs_buf        .= caNavLink( $this->opo_request, $va_submenu_item['displayName'],
					( ( $ps_cur_selection == $ps_base_path ) ? 'sf-menu-selected' : '' ), $defaults_proc['module'],
					$defaults_proc['controller'], $defaults_proc['action'], array_merge( $pa_additional_params,
						$this->_parseAdditionalParameters( $va_submenu_item['parameters'] ) ) );
			} else {
				$vs_buf .= "<a href='#'>" . $va_submenu_item['displayName'] . "</a>";
			}
			if ( isset( $va_submenu_item['navigation'] ) && $va_submenu_item['navigation'] ) {
				$vs_buf .= $this->_genSubMenu( $va_submenu_item['navigation'], $ps_cur_selection, $pa_additional_params,
					$ps_base_path, $pa_defaults );
			}
			$vs_buf .= "</li>\n";
		}
		$vs_buf .= '</ul>';
	...
	# -------------------------------------------------------
	private function _genDynamicTopLevelMenuItems(
		$pa_menu_nav, $ps_cur_selection, $pa_additional_params, $ps_base_path, $pa_defaults
	) {
		...
		foreach ( $pa_menu_nav as $va_submenu_item ) {
			...
			if ( $type_id = isset( $va_submenu_item['parameters']['type_id'] )
				? $va_submenu_item['parameters']['type_id'] : null
			) {
				$dont_expand_types = caMakeTypeIDList( $info['table'],
					$this->opo_config->get( $info['table'] . '_find_dont_expand_hierarchically' ) );
				if ( is_array( $dont_expand_types ) && in_array( $type_id, $dont_expand_types ) ) {
					$result_expansion = false;
				}
			}
			$vb_disabled = ( ( isset( $va_submenu_item['is_enabled'] ) && $va_submenu_item['is_enabled'] )
			                 || ( is_array( $pa_defaults ) && $pa_defaults['module']
			                      && $pa_defaults['module'] == "find" )
			                    && $result_expansion ) ? false : true;

			if ( $vb_disabled ) {
				$vs_buf .= caHTMLLink( caUcFirstUTF8Safe( isset( $va_submenu_item['displayName'] )
					? $va_submenu_item['displayName'] : '' ), array(
					'href'  => '#',
					'class' => ( ( $ps_cur_selection == $ps_base_path ) ? 'sf-menu-disabled-selected' : '' )
				) );
			} else {
				$defaults_proc = $this->_rewriteDefaultsForFindMenuItems( $va_submenu_item, $pa_defaults );
				$vs_buf        .= caNavLink( $this->opo_request,
					caUcFirstUTF8Safe( isset( $va_submenu_item['displayName'] ) ? $va_submenu_item['displayName']
						: '' ), ( ( $ps_cur_selection == $ps_base_path ) ? 'sf-menu-selected' : '' ),
					$defaults_proc['module'], $defaults_proc['controller'], $defaults_proc['action'],
					array_merge( $pa_additional_params, $va_submenu_item['parameters'] ) );
			}
			if ( isset( $va_submenu_item['navigation'] ) && $va_submenu_item['navigation'] ) {
				$vs_buf .= $this->_genSubMenu( $va_submenu_item['navigation'], $ps_cur_selection, $pa_additional_params,
					$ps_base_path, $pa_defaults );
	...
	# -------------------------------------------------------
	private function _genMenuItem(
		&$pa_iteminfo, $ps_key, $ps_base_path, $ps_cur_selection, $ps_css_id = '', $pa_options = null,
		$pa_attributes = null
	) {
		$vs_buf = '';
		...
		$va_additional_params = $this->_parseAdditionalParameters( isset( $pa_iteminfo['parameters'] )
			? $pa_iteminfo['parameters'] : null );

		if ( $vb_disabled ) {
			if ( ! ( $vb_no_access
			         && ( isset( $pa_iteminfo['hideIfNoAccess'] )
			              && $pa_iteminfo['hideIfNoAccess'] ) )
			) {
				$vs_buf .= caHTMLLink( $vs_display_name, array(
					'href'  => '#',
					'class' => ( ( $ps_cur_selection == $ps_base_path . '/' . $ps_key ) ? 'sf-menu-disabled-selected'
						: 'sf-menu-disabled' ),
					'title' => _t( 'Disabled' )
				) );
			}
			...
			$vs_buf .= caNavLink( $this->opo_request, $vs_display_name,
					( ( $ps_cur_selection == $ps_base_path . '/' . $ps_key ) ? 'sf-menu-selected' : '' ),
					$va_defaults['module'], $va_defaults['controller'], $va_defaults['action'], $va_additional_params,
					$pa_attributes ) . "\n";
			if ( is_array( $pa_iteminfo['typeRestrictions'] ) && $pa_iteminfo['typeRestrictions'] ) {
				TooltipManager::add( "#" . $pa_attributes['id'],
					( sizeof( $pa_iteminfo['typeRestrictions'] ) == 1 ) ? _t( "For type <em>%1</em>",
						join( ", ", $pa_iteminfo['typeRestrictions'] ) )
						: _t( "For types <em>%1</em>", join( ", ", $pa_iteminfo['typeRestrictions'] ) ) );
			}
	...
	private function _parseAdditionalParameters( $pa_defaults ) {
		...
		foreach ( $pa_defaults as $vs_param => $vs_value ) {
			...
			if ( count( $va_tmp ) == 2 ) {
				...
				if ( $va_tmp[0] ) {
					$va_additional_params[ $va_tmp[0] ] = ( $v = $this->_parseParameterValue( $vs_value ) ) ? $v
						: $vs_value;
				}
			}
		}
	...
	private function _evaluateRequirements( &$pa_requirements, $options = null ) {
		if ( ! is_array( $pa_requirements )
		     || ( is_array( $pa_requirements )
		          && ( sizeof( $pa_requirements ) == 0 ) )
		) {
			return true;
		...
		foreach ( $pa_requirements as $vs_requirement => $vs_boolean ) {
			...
			switch ( strtolower( $va_tmp[0] ) ) {
				...
				case 'parameter':
					if ( isset( $va_tmp[2] ) ) {
						$vs_value = ( $this->opo_request->getParameter( $va_tmp[1], pString ) == $va_tmp[2] ) ? true
							: false;
					} else {
				...
				case 'checktypelimitinconfig':
					...
					$l        = caMakeTypeIDList( $vs_table, $this->opo_request->config->get( $vs_pref ),
