...
class AppNavigation extends BaseObject
{
    ...
    private function _genReverseNavTable()
    {
        ...
        while (sizeof($va_stack) > 0) {
            ...
            if (isset($va_node['navnode']) && isset($va_node['navnode']['useActionInPath']) && intval(
                    $va_node['navnode']['useActionInPath']
                )) {
                $vs_controller_path .= '/' . $vs_action;
            }
            if (isset($va_node['navnode']) && isset($va_node['navnode']['useActionExtraInPath']) && intval(
                    $va_node['navnode']['useActionExtraInPath']
                ) && (sizeof($va_tmp) > 0)) {
                $vs_controller_path .= '/' . join('/', $va_tmp);
            }

            // does this node have children?
            if (isset($va_node['navnode']) && isset($va_node['navnode']['navigation']) && sizeof(
                    $va_node['navnode']['navigation']
                ) > 0) {
                ...
                $this->opa_reverse_nav_table[$vs_controller_path] = join(
                    '/',
                    array_merge(
                        is_array($va_path) ? $va_path : array(),
                        array($va_node['key'])
                    )
                );
            }
        ...
        foreach ($va_tmp as $vs_part) {
            if ($va_node[$vs_part]) {
                ...
                if ($va_node['type'] == 'dynamic') {
                    ...
                    if ($vb_submenu_set) {
                        if (isset($va_node['submenu']['type']) && ($va_node['submenu']['type'] == 'dynamic') && is_array(
                                $va_sub_menu = $this->getDynamicSubmenu($va_node['submenu'])
                            )) {
                            if (isset($va_node['submenu']['breadcrumbHints']) && is_array(
                                    $va_node['submenu']['breadcrumbHints']
                                )) {
                                if ($vs_trail_item = $this->_getBreadcrumbHint(
                                    $va_node['submenu']['breadcrumbHints']
                                )) {
    ...
    public function getHTMLMenuBar($ps_css_id)
    {
        ...
        if (((time() - Session::getVar('ca_nav_menubar_cache_lasttime')) < 600) && (intval(
                    $this->opo_config->get('do_menu_bar_caching')
                ) > 0) && ($vs_menu_cache = Session::getVar('ca_nav_menubar_cache'))) {
    ...
    public function getHTMLMenuBarAsLinkArray()
    {
        ...
        if ((intval($this->opo_config->get('do_menu_bar_caching')) > 0) && ($va_menu_cache = Session::getVar(
                'ca_nav_menubar_link_cache'
            ))) {
        ...
        foreach ($va_nav_info as $vs_key => $va_menu) {
            ...
            $va_links[] = caNavLink(
                ...
                $va_menu['displayName'],
                '',
                trim($va_menu['default']['module']),
                trim($va_menu['default']['controller']),
    ...
    public function getHTMLSideNav($ps_css_id)
    {
        ...
        $va_nav_info = $this->getNavInfo(
            2
        ); // get third-level navigation (zero-indexed); first two levels are in top-level nav bar
        $vs_buf = '';
        if (is_array($va_nav_info)) {
            ...
            foreach ($va_nav_info as $vs_key => $va_menu) {
                if (isset($va_menu['navigation']) && is_array($va_menu['navigation'])) {
                    if ($vs_menu_item = $this->_genMenuItem(
                        $va_menu,
                        $vs_key,
                        $vs_base_path,
                        $vs_cur_selection,
    ...
    public function getHTMLWidgets()
    {
        ...
        foreach ($va_widgets_config as $vs_key => $va_info) {
            if (preg_match(
                '!^/' . $va_info['domain']['module'] . '/' . $va_info['domain']['controller'] . '$!i',
                $vs_cur_selection
            )) {
                ...
                if (!$va_info['handler']['isplugin']) {
                    ...
                    if (!include_once($this->opo_config->get(
                            'application_plugins'
                ...
                $o_action_controller = new $vs_classname(
                    $this->opo_request,
                    $this->opo_response,
                    $this->opo_request->config->get(
                        'views_directory'
                    ) . '/' . $va_info['handler']['module']
                );
    ...
    public function getDynamicNavigation($va_info)
    {
        ...
        if ($o_action_controller->numErrors()) {
            $this->postError(
                2300,
                _t("Controller error: %1", join('; ', $o_action_controller->getErrors())),
    ...
    public function addNavItem(
        ...
        $pa_defaults,
        $pa_requirements,
        $pn_insert_index = null,
        $pa_sub_navigation = null
    ) {
        $this->opa_nav_config = $this->_addNavItem(
            $this->opa_nav_config,
            $ps_display_name,
            $ps_menu_name,
            $pa_defaults,
            ...
            if (sizeof($va_tmp) < (sizeof($pa_menu_info) + 1)) {
                $va_tmp = array_merge(
                    ...
                    array_slice(
                        $pa_menu_info,
                        $pn_insert_index,
                        (sizeof($pa_menu_info) - $pn_insert_index),
                        true
                    )
                );
            }
    ...
    private function &_genMenu(&$pa_navinfo, $ps_base_path, $ps_cur_selection)
    {
        ...
        foreach ($pa_navinfo as $vs_nav => $va_nav_info) {
            ...
            $va_additional_params = $this->_parseAdditionalParameters(
                (isset($pa_navinfo[$vs_nav]) && (isset($pa_navinfo[$vs_nav]['parameters']))) ? $pa_navinfo[$vs_nav]['parameters'] : null
            );
            ...
            if (isset($pa_navinfo[$vs_nav]['remember_last_used_navigation']) && $pa_navinfo[$vs_nav]['remember_last_used_navigation']) {
                $va_nav_defaults = Session::getVar(
                    'ca_app_nav_defaults'
                );    // get stored defaults - contains the last used navigation items keyed by base path
            ...
            if (isset($pa_navinfo[$vs_nav]['type']) && ($pa_navinfo[$vs_nav]['type'] == 'dynamic')) {
                ...
                if (sizeof($va_submenu_nav)) {
                    $vs_buf .= $this->_genDynamicTopLevelMenuItems(
                        $va_submenu_nav,
                        $vs_cur_selection,
                        $va_additional_params,
                        $ps_base_path,
                        $va_defaults
                    );
                }
                ...
                if ($vb_submenu_set && isset($pa_navinfo[$vs_nav]) && isset($pa_navinfo[$vs_nav]['submenu']) && $pa_navinfo[$vs_nav]['submenu']) {
                    if ($pa_navinfo[$vs_nav]['submenu']['type'] == 'dynamic') {
                        ...
                        if (sizeof($va_submenu_nav)) {
                            ...
                            $is_link = (is_array(
                                    $va_defaults
                                ) && $va_defaults['module'] && $va_defaults['module'] == "find");
                                    ...
                                    array_filter(
                                        array_values(
                                            array_map(
                                                function ($v) {
                                                    return preg_match(
                                                        "!^action:can_search_([a-z_]+)$!",
                                                        $v,
                                                        $m
                                                    ) ? $m[1] : null;
                                                },
                                                array_keys($pa_navinfo[$vs_nav]['requires'])
                                            )
                                        ),
                                        function ($v) {
                                            return $v;
                            ...
                            $vs_buf .= "<li>" . ($is_link ? caNavLink(
                                    $this->opo_request,
                                    $vs_display_name,
                                    (($vs_cur_selection == $ps_base_path . '/' . $vs_nav) ? 'sf-menu-selected' : ''),
                                    $va_defaults['module'],
                                    $va_defaults['controller'],
                                    $va_defaults['action'],
                                    $va_additional_params
                                ) : caHTMLLink(
                                    $vs_display_name,
                                    array(
                                        'class' => (($vs_cur_selection == $ps_base_path . '/' . $vs_nav) ? 'sf-menu-selected' : ''),
                                        'href' => '#'
                                    )
                                ));
                            $vs_buf .= $this->_genSubMenu(
                                $va_submenu_nav,
                                $vs_cur_selection,
                                $va_additional_params,
                                $ps_base_path,
                        ...
                        $vs_link = (is_array($va_defaults) && $va_defaults['module']) ? caNavLink(
                            $this->opo_request,
                            $vs_display_name,
                            (($vs_cur_selection == $ps_base_path . '/' . $vs_nav) ? 'sf-menu-selected' : ''),
                            $va_defaults['module'],
                            ...
                            $va_defaults['action'],
                            $va_additional_params
                        ) : "<a href='#'>{$vs_display_name}</a>";
                        $vs_buf .= "<li>{$vs_link}\n";
                        $vs_buf .= $this->_genSubMenu(
                            $pa_navinfo[$vs_nav]['submenu']['navigation'],
                            $vs_cur_selection,
                            $va_additional_params,
                            $ps_base_path,
                            $va_defaults
                    ...
                    if (is_array($va_defaults) && (sizeof($va_defaults) == 0)) {
                        ...
                        $vs_buf .= "<li " . (($vs_last_selected_path_item == $vs_nav) ? 'class="sf-menu-selected"' : '') . ">" . caNavLink(
                                $this->opo_request,
                                $vs_display_name,
    ...
    {
        if (($type_id = (int)caGetOption('type_id', $pa_submenu_item['parameters'], null)) && preg_match(
                "!^([A-Z]{1}[a-z]+)(.*)$!",
                $pa_defaults['controller'],
                $m
            ) && (in_array($m[1], ['Search', 'Browse']) && is_array(
    ...
    private function _genSubMenu($pa_submenu_nav, $ps_cur_selection, $pa_additional_params, $ps_base_path, $pa_defaults)
    {
        ...
        foreach ($pa_submenu_nav as $va_submenu_item) {
            ...
            if (isset($va_submenu_item) && isset($va_submenu_item['default']) && is_array(
                    $va_submenu_item['default']
                )) {
            ...
            if (isset($va_submenu_item) && ((isset($va_submenu_item['is_enabled']) && intval(
                            $va_submenu_item['is_enabled']
                        )) || (is_array($pa_defaults) && $pa_defaults['module'] && $pa_defaults['module'] == "find"))) {
                $defaults_proc = $this->_rewriteDefaultsForFindMenuItems($va_submenu_item, $pa_defaults);
                $vs_buf .= caNavLink(
                    $this->opo_request,
                    $va_submenu_item['displayName'],
                    (($ps_cur_selection == $ps_base_path) ? 'sf-menu-selected' : ''),
                    $defaults_proc['module'],
                    $defaults_proc['controller'],
                    $defaults_proc['action'],
                    array_merge(
    ...
    private function _genDynamicTopLevelMenuItems(
        ...
        foreach ($pa_menu_nav as $va_submenu_item) {
            ...
            if ($type_id = isset($va_submenu_item['parameters']['type_id']) ? $va_submenu_item['parameters']['type_id'] : null) {
                $dont_expand_types = caMakeTypeIDList(
                    $info['table'],
                    $this->opo_config->get(
                        $info['table'] . '_find_dont_expand_hierarchically'
                    )
                );
            ...
            $vb_disabled = ((isset($va_submenu_item['is_enabled']) && $va_submenu_item['is_enabled']) || (is_array(
                        $pa_defaults
                    ) && $pa_defaults['module'] && $pa_defaults['module'] == "find") && $result_expansion) ? false : true;
            ...
            if ($vb_disabled) {
                ...
                $vs_buf .= caNavLink(
                    $this->opo_request,
                    caUcFirstUTF8Safe(
                        isset($va_submenu_item['displayName']) ? $va_submenu_item['displayName'] : ''
                    ),
                    (($ps_cur_selection == $ps_base_path) ? 'sf-menu-selected' : ''),
                    $defaults_proc['module'],
                    $defaults_proc['controller'],
    ...
    private function _genMenuItem(
        ...
        $va_additional_params = $this->_parseAdditionalParameters(
            isset($pa_iteminfo['parameters']) ? $pa_iteminfo['parameters'] : null
        );
        ...
        if ($vb_disabled) {
            if (!($vb_no_access && (isset($pa_iteminfo['hideIfNoAccess']) && $pa_iteminfo['hideIfNoAccess']))) {
                $vs_buf .= caHTMLLink(
                    ...
                    array(
                        'href' => '#',
                        'class' => (($ps_cur_selection == $ps_base_path . '/' . $ps_key) ? 'sf-menu-disabled-selected' : 'sf-menu-disabled'),
                        'title' => _t('Disabled')
                    )
                );
            }
            ...
            if (is_array($pa_iteminfo['typeRestrictions']) && $pa_iteminfo['typeRestrictions']) {
                TooltipManager::add(
                    "#" . $pa_attributes['id'],
                    (sizeof($pa_iteminfo['typeRestrictions']) == 1) ? _t(
                        "For type <em>%1</em>",
    ...
    private function _evaluateRequirements(&$pa_requirements, $options = null)
    {
        ...
        foreach ($pa_requirements as $vs_requirement => $vs_boolean) {
            ...
            switch (strtolower($va_tmp[0])) {
                ...
                case 'parameter':
                    if (isset($va_tmp[2])) {
                        $vs_value = ($this->opo_request->getParameter(
                                $va_tmp[1],
                                pString
                            ) == $va_tmp[2]) ? true : false;
                    } else {
                ...
                case 'checktypelimitinconfig':
                    ...
                    $l = caMakeTypeIDList(
                        $vs_table,
                        $this->opo_request->config->get($vs_pref),
