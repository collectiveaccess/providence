...
class ResultContext
{
    ...
    {
        if (!$pb_from_context_only && ($ps_search = urldecode(
                (strip_tags(
                    html_entity_decode($this->opo_request->getParameter('search', pString, ['forcePurify' => true]))
                ))
            )) != '') {
    ...
    public function searchExpressionHasChanged($pb_from_context_only = false)
    {
        ...
        if (!$pb_from_context_only && ($ps_search = urldecode(
                (strip_tags($this->opo_request->getParameter('search', pString, ['forcePurify' => true])))
            )) != '') {
            // search specified by request parameter
            if ($ps_search != $this->getContextValue('expression')) {
    ...
    public function setSearchExpressionForDisplay($ps_display_expression)
    {
        ...
        if (!$va_expressions_for_display[$vs_current_expression = $this->getSearchExpression(
                true
    ...
    public function getSearchExpressionForDisplay($ps_search_expression = null)
    {
        ...
        if ($vs_display_expression = $va_expressions_for_display[$vs_original_expression = $this->getSearchExpression(
            true
        )]) {
    ...
    {
        if (!($ps_letter_bar_page = htmlspecialchars(
            strip_tags($this->opo_request->getParameter('l', pString, ['forcePurify' => true]))
        ))) {
            if ($va_context = $this->getContext()) {
    ...
    {
        if (!($ps_view = htmlspecialchars(
            $this->opo_request->getParameter('view', pString, ['forcePurify' => true])
        ))) {
            if ($va_context = $this->getContext()) {
    ...
    {
        if (!($ps_sort = htmlspecialchars(
            $this->opo_request->getParameter('sort', pString, ['forcePurify' => true])
        ))) {
            if ($va_context = $this->getContext()) {
    ...
    {
        if (!($ps_secondary_sort = htmlspecialchars(
            $this->opo_request->getParameter('secondarySort', pString, ['forcePurify' => true])
        ))) {
            if ($va_context = $this->getContext()) {
    ...
    {
        if (!($ps_sort_direction = htmlspecialchars(
            $this->opo_request->getParameter('direction', pString, ['forcePurify' => true])
        ))) {
            if ($va_context = $this->getContext()) {
                return in_array(
                    $va_context['sort_direction'],
                    array('asc', 'desc')
                ) ? $va_context['sort_direction'] : 'asc';
            }
        } else {
    ...
    public function getTypeRestriction(&$pb_type_restriction_has_changed)
    {
        ...
        if (!($pn_type_id = htmlspecialchars(
            html_entity_decode($this->opo_request->getParameter('type_id', pString, ['forcePurify' => true]))
        ))) {
            if ($va_context = $this->getContext()) {
    ...
    {
        if (!strlen(
            $pn_display_id = htmlspecialchars(
                $this->opo_request->getParameter('display_id', pString, ['forcePurify' => true])
            )
        )) {
            if ($va_context = $this->getContext()) {
            ...
            if (!$pn_display_id) {
                ...
                if (is_array(
                    $displays = $t_display->getBundleDisplays(
                        ['table' => $this->tableName(), 'restrictToTypes' => $pn_type_id ? [$pn_type_id] : null]
                    )
                )) {
                    if ($ps_show_in) {
                        foreach ($displays as $id => $d) {
                            $d = array_shift($d);
                            if (($show_in_setting = caGetOption(
                                    'show_only_in',
                                    $d['settings'],
                                    null,
                                    ['castTo' => 'array']
    ...
    {
        if (!($ps_children_display_mode = $this->opo_request->getParameter(
            'children',
            pString,
            ['forcePurify' => true]
        ))) {
            if ($va_context = $this->getContext()) {
                ...
                return (in_array(
                    strtolower($va_context['children_display_mode']),
                    ['show', 'hide', 'alwaysshow', 'alwayshide']
                ) ? $va_context['children_display_mode'] : (($vs_children_display_mode_default = $o_config->get(
                    $this->ops_table_name . "_children_display_mode_in_results"
    ...
    public function getParameter($ps_param, $pb_dont_fetch_from_request = false)
    {
        if ($pb_dont_fetch_from_request) {
            ...
            if (!isset($_REQUEST[$ps_param]) && (!$this->opo_request || !$this->opo_request->getParameter(
                        $ps_param,
                        pString,
                        ['forcePurify' => true]
                    ))) {
    ...
    public function setAsLastFind($pb_set_action = true)
    {
        ...
        ResultContextStorage::setVar(
            'result_last_context_' . $this->ops_table_name,
            $this->ops_find_type . ($this->ops_find_subtype ? '/' . $this->ops_find_subtype : ''),
            array('volatile' => true)
        );
        ResultContextStorage::setVar(
            'result_last_context_' . $this->ops_table_name . '_action',
    ...
    public static function getResultsUrl($request, $table_name_or_num, $find_type, $options = null)
    {
        ...
        $o_find_navigation = Configuration::load(
            ((defined(
                    '__CA_THEME_DIR__'
                ) && (__CA_APP_TYPE__ == 'PAWTUCKET')) ? __CA_THEME_DIR__ : __CA_APP_DIR__) . '/conf/find_navigation.conf'
        );
    ...
    public static function getResultsUrlForLastFind($po_request, $pm_table_name_or_num, $pa_params = null)
    {
        ...
        if (__CA_APP_TYPE__ == 'PAWTUCKET') {
            ...
            if (
                !file_exists(
                    $vs_path = __CA_THEME_DIR__ . "/controllers/" . (trim($va_nav['module_path']) ? trim(
                                $va_nav['module_path']
                            ) . "/" : "") . $va_nav['controller'] . "Controller.php"
                )
                &&
                !file_exists(
                    $vs_path = __CA_APP_DIR__ . "/controllers/" . (trim($va_nav['module_path']) ? trim(
                                $va_nav['module_path']
                            ) . "/" : "") . $va_nav['controller'] . "Controller.php"
                )
            ) {
            ...
            $va_nav = call_user_func_array(
                "{$vs_controller_class}::" . $va_nav['action'],
        ...
        if (is_array($va_nav['params'])) {
            $o_context = new ResultContext(
                $po_request,
                $pm_table_name_or_num,
                $va_tmp[0],
        ...
        return caNavUrl(
            $po_request,
            trim($va_nav['module_path']),
            trim($va_nav['controller']),
            trim($vs_action),
            $pa_params
        );
    }
     ...
     */
    public static function getResultsLinkForLastFind(
        $po_request,
        $pm_table_name_or_num,
        $ps_content,
        $ps_class = null,
        ...
        $o_find_navigation = Configuration::load(
            ((defined('__CA_THEME_DIR__') && file_exists(
                    __CA_THEME_DIR__ . '/conf/find_navigation.conf'
        ...
        if (is_array($va_nav['params'])) {
            ...
            foreach ($va_nav['params'] as $vs_param) {
                ...
                if (!trim(
                    $va_params[$vs_param] = $po_request->getParameter($vs_param, pString, ['forcePurify' => true])
                )) {
                    $va_params[$vs_param] = trim($o_context->getParameter($vs_param));
                }
            }
    ...
    protected function getContext($ps_find_type = null, $ps_find_subtype = null)
    {
        ...
        if ($ps_find_type) {
            ...
            if (is_array(
                $d = ResultContextStorage::getVar(
                    'result_context_' . $this->ops_table_name . '_' . $ps_find_type . '_' . $ps_find_subtype
                )
            )) {
                $va_context = array_merge($va_context, $d);
            }
        ...
        if (!$this->opa_context) {
            ...
            if (is_array(
                $d = ResultContextStorage::getVar(
                    'result_context_' . $this->ops_table_name . '_' . $vs_find_type . '_' . $vs_find_subtype
                )
            )) {
                $va_context = array_merge($va_context, $d);
            }
    ...
    public function saveContext($ps_find_type = null, $pa_context = null, $ps_find_subtype = null)
    {
        ...
        ResultContextStorage::setVar(
            'result_context_' . $this->ops_table_name . '_' . $vs_find_type . ($vs_find_subtype ? "_{$vs_find_subtype}" : ""),
            $va_context
        );
        ...
        if (!is_array(
            $va_existing_semi_context = ResultContextStorage::getVar(
                'result_context_' . $this->ops_table_name . '_' . $vs_find_type . ($vs_find_subtype ? "_{$vs_find_subtype}" : "")
            )
        )) {
    ...
    public function removeIDFromResults($pn_row_id)
    {
        ...
        foreach ($va_findtypes as $vs_findtype) {
            ...
            if (is_array($va_context) && isset($va_context['result_list']) && is_array(
                    $va_results = $va_context['result_list']
                )) {
                if (($vn_index = array_search($pn_row_id, $va_results)) !== false) {
     ...
     *
     * @return bool True if cache if valid, false if not
     */
...
class ResultContextStorage
{
    ...
    public static function init($po_request)
    {
        ...
        if ($po_request->isLoggedIn() && (!(bool)$po_request->config->get(
                'always_use_session_based_storage_for_find_result_contexts'
            ))) {
