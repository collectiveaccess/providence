...
class ResultContext {
	 ...
	 * @param $pm_table_name_or_num - table name or num of result context (eg. what kind of item is the find result
	 *                              composed of?)
	 * @param $ps_find_type         - a __CA_FIND_CONTEXT_*__ constant indicating the source of the find; separate
	 *                              contexts are maintained for each find type
	 *
	 */
	 ...
	 *
	 * @param $pb_from_context_only boolean Optional; if true then search expression is returned from context only and
	 *                              any search expression request parameter is ignored. Default is false.
	 *
	...
	public function getSearchExpression( $pb_from_context_only = false ) {
		if ( ! $pb_from_context_only
		     && ( $ps_search
				= urldecode( ( strip_tags( html_entity_decode( $this->opo_request->getParameter( 'search', pString,
				[ 'forcePurify' => true ] ) ) ) ) ) ) != ''
		) {
	...
	public function searchExpressionHasChanged( $pb_from_context_only = false ) {
		...
		if ( ! $pb_from_context_only
		     && ( $ps_search
				= urldecode( ( strip_tags( $this->opo_request->getParameter( 'search', pString,
				[ 'forcePurify' => true ] ) ) ) ) ) != ''
		) {
			// search specified by request parameter
			if ( $ps_search != $this->getContextValue( 'expression' ) ) {
	 ...
	 * @param string $ps_expression Alternate text to use when displaying search to user. Typically used in "advanced"
	 *                              searches where access points and booleans may prove unattractive or hard to read
	 *
	 * @return bool
	 */
	...
	public function setSearchExpressionForDisplay( $ps_display_expression ) {
		...
		if ( ! $va_expressions_for_display[ $vs_current_expression = $this->getSearchExpression( true ) ]
		     || ( $vs_current_expression != $ps_display_expression )
		) {
	...
	public function getSearchExpressionForDisplay( $ps_search_expression = null ) {
		...
		if ( $vs_display_expression = $va_expressions_for_display[ $vs_original_expression
			= $this->getSearchExpression( true ) ]
		) {
	 ...
	 *
	 * @return integer - number of page; the first page is 1 (*not* zero). Returns 1 as default if no page has been set
	 *                 for the current context.
	 */
	public function getCurrentResultsPageNumber() {
		if ( ( $pn_page = intval( $this->opo_request->getParameter( 'page', pString, [ 'forcePurify' => true ] ) ) )
		     < 1
		) {
	...
	public function getCurrentSortDirection() {
		if ( ! ( $ps_sort_direction = htmlspecialchars( $this->opo_request->getParameter( 'direction', pString,
			...
			if ( $va_context = $this->getContext() ) {
				return in_array( $va_context['sort_direction'], array( 'asc', 'desc' ) ) ? $va_context['sort_direction']
					: 'asc';
			}
		} else {
	 ...
	 * as the current context value when present.
	 *
	 * @param boolean $pb_type_restriction_has_changed Optional variable that will be set to true if the type
	 *                                                 restriction has changed due to a new value being present as a
	 *                                                 request parameter. This allows your code to detect changes in
	 *                                                 type restriction.
	 *
	 * @return string - the view to use
	 */
	public function getTypeRestriction( &$pb_type_restriction_has_changed ) {
		...
		if ( ! ( $pn_type_id = htmlspecialchars( html_entity_decode( $this->opo_request->getParameter( 'type_id',
			pString, [ 'forcePurify' => true ] ) ) ) )
		) {
			if ( $va_context = $this->getContext() ) {
			...
			$_GET['type_id']
				= $pn_type_id;                                // push type_id into globals so breadcrumb trail can pick it up
	...
	public function getCurrentBundleDisplay( $pn_type_id = null, $ps_show_in = null ) {
		if ( ! strlen( $pn_display_id = htmlspecialchars( $this->opo_request->getParameter( 'display_id', pString,
			...
			if ( ! $pn_display_id ) {
				...
				if ( is_array( $displays = $t_display->getBundleDisplays( [
					'table'           => $this->tableName(),
					'restrictToTypes' => $pn_type_id ? [ $pn_type_id ] : null
				] ) )
				) {
					if ( $ps_show_in ) {
	...
	public function getCurrentChildrenDisplayMode() {
		if ( ! ( $ps_children_display_mode = $this->opo_request->getParameter( 'children', pString,
			...
			if ( $va_context = $this->getContext() ) {
				...
				return ( in_array( strtolower( $va_context['children_display_mode'] ),
					[ 'show', 'hide', 'alwaysshow', 'alwayshide' ] )
					? $va_context['children_display_mode']
					: ( ( $vs_children_display_mode_default = $o_config->get( $this->ops_table_name
	 ...
	 * @param bool   $pb_dont_fetch_from_request - optional flag; if set then value is returned from the context only;
	 *                                           if not set (the default) then any value present in the request will be
	 *                                           used (and set into the context) - the context value will only be
	 *                                           returned if there is no value in the request with the given name.
	 *
	...
	public function getParameter( $ps_param, $pb_dont_fetch_from_request = false ) {
		if ( $pb_dont_fetch_from_request ) {
			...
			if ( ! isset( $_REQUEST[ $ps_param ] )
			     && ( ! $this->opo_request
			          || ! $this->opo_request->getParameter( $ps_param, pString, [ 'forcePurify' => true ] ) )
			) {
				if ( $va_context = $this->getContext() ) {
					if ( is_array( $va_context[ 'param_' . $ps_param ] ) ) {
						return $va_context[ 'param_' . $ps_param ];
					} else {
						return strlen( $va_context[ 'param_' . $ps_param ] ) ? $va_context[ 'param_' . $ps_param ]
							: null;
					}
				}
			} else {
	...
	public function setAsLastFind( $pb_set_action = true ) {
		...
		ResultContextStorage::setVar( 'result_last_context_' . $this->ops_table_name,
			$this->ops_find_type . ( $this->ops_find_subtype ? '/' . $this->ops_find_subtype : '' ),
			array( 'volatile' => true ) );
		ResultContextStorage::setVar( 'result_last_context_' . $this->ops_table_name . '_action',
			$pb_set_action ? $vs_action : null );
	...
	/**
	 * Return the result content for the last performed find operation for the specified table, as set with
	 * setAsLastFind()
	 *
	 * @param $po_request           - the current request
	 * @param $pm_table_name_or_num - the name or number of the table to get the last find operation for
	...
	static public function getResultsUrl( $request, $table_name_or_num, $find_type, $options = null ) {
		...
		$o_find_navigation = Configuration::load( ( ( defined( '__CA_THEME_DIR__' )
		                                              && ( __CA_APP_TYPE__ == 'PAWTUCKET' ) ) ? __CA_THEME_DIR__
				: __CA_APP_DIR__ ) . '/conf/find_navigation.conf' );
	...
	static public function getResultsUrlForLastFind( $po_request, $pm_table_name_or_num, $pa_params = null ) {
		...
		if ( __CA_APP_TYPE__ == 'PAWTUCKET' ) {
			...
			if (
				! file_exists( $vs_path = __CA_THEME_DIR__ . "/controllers/" . ( trim( $va_nav['module_path'] )
						? trim( $va_nav['module_path'] ) . "/" : "" ) . $va_nav['controller'] . "Controller.php" )
				&&
				! file_exists( $vs_path = __CA_APP_DIR__ . "/controllers/" . ( trim( $va_nav['module_path'] )
						? trim( $va_nav['module_path'] ) . "/" : "" ) . $va_nav['controller'] . "Controller.php" )
			) {
				return false;
			}
			...
			$va_nav              = call_user_func_array( "{$vs_controller_class}::" . $va_nav['action'],
				array( $po_request, $vs_table_name ) );

			if ( ! ( $vs_action = ResultContextStorage::getVar( 'result_last_context_' . $vs_table_name
			                                                    . '_action' ) )
			) {
		...
		if ( is_array( $va_nav['params'] ) ) {
			$o_context = new ResultContext( $po_request, $pm_table_name_or_num, $va_tmp[0],
				isset( $va_tmp[1] ) ? $va_tmp[1] : null );
		...
		return caNavUrl( $po_request, trim( $va_nav['module_path'] ), trim( $va_nav['controller'] ), trim( $vs_action ),
			$pa_params );
	}
	# ------------------------------------------------------------------
	 ...
	 * @param $pa_attributes        - (optional) associative array of values to use as attributes in the key (keys are
	 ...
	 */
	static public function getResultsLinkForLastFind(
		$po_request, $pm_table_name_or_num, $ps_content, $ps_class = null, $pa_params = null, $pa_attributes = null
	) {
		if ( ! ( $vs_table_name = Datamodel::getTableName( $pm_table_name_or_num ) ) ) {
			return null;
		}
		...
		$o_find_navigation = Configuration::load( ( ( defined( '__CA_THEME_DIR__' )
		                                              && file_exists( __CA_THEME_DIR__
		                                                              . '/conf/find_navigation.conf' ) )
				? __CA_THEME_DIR__ : __CA_APP_DIR__ ) . '/conf/find_navigation.conf' );
		$va_find_nav       = $o_find_navigation->getAssoc( $vs_table_name );
		$va_nav            = $va_find_nav[ $va_tmp[0] ];
		if ( ! $va_nav ) {
			return false;
		...
		return caNavLink( $po_request, $ps_content, $ps_class, trim( $va_nav['module_path'] ),
			trim( $va_nav['controller'] ), trim( $vs_action ), $pa_params, $pa_attributes );
	}
	# ------------------------------------------------------------------
	# Find history
	# ------------------------------------------------------------------
	 ...
	 *
	 * @param string Optional find type string; allows you to load any context regardless of what the current find type
	 *                        is. Don't use this unless you know what you're doing.
	 * @param string Optional find subtype string; allows you to load any context regardless of what the current find
	 *                        subtype is. Don't use this unless you know what you're doing.
	 *
	...
	protected function getContext( $ps_find_type = null, $ps_find_subtype = null ) {
		...
		if ( $ps_find_type ) {
			...
			if ( is_array(
				$d = ResultContextStorage::getVar( 'result_context_' . $this->ops_table_name . '_' . $ps_find_type ) )
			) {
				$va_context = array_merge( $va_context, $d );
			}
			if ( is_array(
				$d = ResultContextStorage::getVar( 'result_context_' . $this->ops_table_name . '_' . $ps_find_type . '_'
				                                   . $ps_find_subtype ) )
			) {
				$va_context = array_merge( $va_context, $d );
			}
		...
		if ( ! $this->opa_context ) {
			...
			if ( is_array(
				$d = ResultContextStorage::getVar( 'result_context_' . $this->ops_table_name . '_' . $vs_find_type ) )
			) {
				$va_context = array_merge( $va_context, $d );
			}
			if ( is_array(
				$d = ResultContextStorage::getVar( 'result_context_' . $this->ops_table_name . '_' . $vs_find_type . '_'
				                                   . $vs_find_subtype ) )
			) {
				$va_context = array_merge( $va_context, $d );
			}
	 ...
	 *
	 * @param string Optional find type string to save context under; allows you to save to any context regardless of
	 *                        what is currently loaded. Don't use this unless you know what you're doing.
	 *
	 * @return boolean - always returns true
	 */
	public function saveContext( $ps_find_type = null, $pa_context = null, $ps_find_subtype = null ) {
		...
		ResultContextStorage::setVar( 'result_context_' . $this->ops_table_name . '_' . $vs_find_type
		                              . ( $vs_find_subtype ? "_{$vs_find_subtype}" : "" ), $va_context );
		...
		if ( ! is_array(
			$va_existing_semi_context = ResultContextStorage::getVar( 'result_context_' . $this->ops_table_name . '_'
			                                                          . $vs_find_type . ( $vs_find_subtype
					? "_{$vs_find_subtype}" : "" ) ) )
		) {
		...
		ResultContextStorage::setVar( 'result_context_' . $this->ops_table_name . '_' . $vs_find_type
		                              . ( $vs_find_subtype ? "_{$vs_find_subtype}" : "" ), $va_semi_context );
	...
	public function removeIDFromResults( $pn_row_id ) {
		...
		foreach ( $va_findtypes as $vs_findtype ) {
			...
			if ( is_array( $va_context ) && isset( $va_context['result_list'] )
			     && is_array( $va_results = $va_context['result_list'] )
			) {
				if ( ( $vn_index = array_search( $pn_row_id, $va_results ) ) !== false ) {
...
class ResultContextStorage {
	...
	static public function init( $po_request ) {
		...
		if ( $po_request->isLoggedIn()
		     && ( ! (bool) $po_request->config->get( 'always_use_session_based_storage_for_find_result_contexts' ) )
		) {
