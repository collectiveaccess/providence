...
class Representation extends Bundlable
{
        ...
        if (isset($va_snapshot['media_media_desc']) && is_array($va_snapshot['media_media_desc'])) {
            if (is_array($va_snapshot['media_media_desc']['_CENTER'])) {
                ReplicationService::$s_logger->log(
                    "Set media center to " . print_R($va_snapshot['media_media_desc']['_CENTER'], true)
                );
                $this->getModelInstance()->setMediaCenter(
                    'media',
                    $va_snapshot['media_media_desc']['_CENTER']['x'],
    ...
    public function sanityCheck()
    {
        ...
        if (isset($va_snapshot['media']) && (strlen($va_snapshot['media']) == 32) && preg_match(
                "/^[a-f0-9]+$/",
                $va_snapshot['media']
            )) {
