...
class Relationship extends Base {
	...
	public function sanityCheck() {
		...
		if ( $vs_type_field = $this->getModelInstance()->getProperty( 'RELATIONSHIP_TYPE_FIELDNAME' ) ) {
			...
			if ( isset( $va_snapshot[ $vs_potential_code_field ] )
			     && ( $vs_rel_type_code
					= $va_snapshot[ $vs_potential_code_field ] )
			) {
				if ( ! ( $vn_rel_type_id = caGetRelationshipTypeID( $this->getModelInstance()->tableNum(),
					$vs_rel_type_code ) )
				) {
					throw new InvalidLogEntryException( _t( "Couldn't find relationship type with type code '%1'.",
						$vs_rel_type_code ) );
				}
			} else {
				if ( $this->isInsert() ) {
		...
		// left
		if ( $this->isInsert()
		     && ( $vs_field = $this->getModelInstance()->getProperty( 'RELATIONSHIP_LEFT_FIELDNAME' ) )
		) {
	...
	private function setLeftOrRightFieldNameFromSnapshot( $ps_field, $pb_left = true ) {
		$va_snapshot = $this->getSnapshot();
		if ( isset( $va_snapshot[ $ps_field . '_guid' ] )
		     && ( $vs_reference_guid = $va_snapshot[ $ps_field . '_guid' ] )
		) {
			/** @var \BundlableLabelableBaseModelWithAttributes $t_instance */
			$t_instance = $pb_left ? $this->getModelInstance()->getLeftTableInstance()
				: $this->getModelInstance()->getRightTableInstance();
			if ( ! $t_instance ) {
	...
	private function verifyLeftOrRightFieldNameFromSnapshot( $ps_field, $pb_left = true ) {
		...
		if ( isset( $va_snapshot[ $ps_field . '_guid' ] )
			...
			if ( $pb_left ) {
				...
				$t_instance = \Datamodel::getInstanceByTableName( $this->getModelInstance()->getRightTableName(),
					true );
			}
			...
			if ( ! method_exists( $t_instance, "loadByGUID" ) || ! $t_instance->loadByGUID( $vs_reference_guid ) ) {
				...
				throw new IrrelevantLogEntry( "Could not load {$t_instance->tableName()} record by GUID {$vs_reference_guid} (referenced in {$vs_property} in a relationship record ["
				                              . $this->getModelInstance()->tableName() . "])" );
			}
		} else {
			if ( $this->isInsert() ) {
				// TODO: confirm irrelevant is the way to go here
