...
use BaseLabel;
use BaseModel;
use BaseRelationshipModel;
use BundlableLabelableBaseModelWithAttributes;
use ca_attribute_values;
use ca_attributes;
use ca_object_representations;
use ca_representation_annotations;
use Datamodel;
use Exception;
use Transaction;
...
abstract class Base
{
    ...
    public function __construct($ops_source_system_id, $opn_log_id, array $opa_log, Transaction $po_tx)
    {
        ...
        if (strlen($ops_source_system_id) != 36) {
            throw new InvalidLogEntryException(
                'source system GUID is not a valid guid. String length was: ' . strlen($ops_source_system_id)
            );
        }
        ...
        if ($this->isUpdate() || $this->isDelete()) {
            ...
            if ((!$this->getModelInstance()->loadByGUID($this->getGUID())) && $this->isUpdate()) {
                if ($this->getModelInstance()->tableName() !== 'ca_users') {
                    throw new IrrelevantLogEntry(
                        'Mode was update but the given GUID "' . $this->getGUID(
            ...
            if ((!$this->getModelInstance()->loadByGUID($this->getGUID())) && $this->isDelete()) {
                throw new IrrelevantLogEntry(
                    'Mode was delete but the given GUID "' . $this->getGUID(
                    ) . '" could not be found for table num ' . $this->getTableNum()
                );
            }
        }
            ...
            if ($va_fld_info = $this->getModelInstance()->getFieldInfo($vs_field)) {
                ...
                if (isset($va_fld_info['LIST']) || isset($va_fld_info['LIST_CODE'])) {
                    if (array_key_exists($vs_potential_code_field, $va_snapshot)) {
                        ...
                        if (strlen($vs_code) && ($vs_code !== 'null') && !($vn_item_id = caGetListItemID(
                                $vs_list,
                                $vs_code,
                                ['includeDeleted' => true]
                            ))) {
                ...
                if (($vs_field == $this->getModelInstance()->getProperty('HIERARCHY_PARENT_ID_FLD')) && (intval(
                            $va_snapshot[$vs_field]
                        ) != 1)) {
                    if (array_key_exists(
                            $vs_field . '_guid',
                            $va_snapshot
                        ) && ($vs_parent_guid = $va_snapshot[$vs_field . '_guid'])) {
                        if (($vs_idno = $va_snapshot[$this->getModelInstance()->getProperty(
                            ...
                            if (!$t_instance->loadByGUID($vs_parent_guid) && !(intval($va_snapshot[$vs_field]) == 1)) {
                                throw new InvalidLogEntryException(
                                    _t(
                                        "Could not load GUID %1 (referenced in HIERARCHY_PARENT_ID_FLD)",
                                        $vs_parent_guid
                                    )
                                );
                            }
    ...
    public function setIntrinsicsFromSnapshotInModelInstance()
    {
        ...
        foreach ($va_snapshot as $vs_field => $vm_val) {
            ...
            if ($va_fld_info = $this->getModelInstance()->getFieldInfo($vs_field)) {
                ...
                if (($va_fld_info['FIELD_TYPE'] == FT_MEDIA) && (strlen($va_snapshot[$vs_field]) == 32) && preg_match(
                        "/^[a-f0-9]+$/",
                        $va_snapshot[$vs_field]
                    )) {
                        ...
                        if ($vn_item_id = caGetListItemID($vs_list, $vs_code)) {
                            if (isset($va_fld_info['LIST'])) { // access, status -> set item value (0,1 etc.)
                                $this->getModelInstance()->set(
                                    $vs_field,
                                    caGetListItemValueForID($vn_item_id),
                    ...
                    if (intval($va_snapshot[$vs_field]) == 1) {
                        ...
                        if (array_key_exists(
                            ...
                            if ($t_instance->loadByGUID($vs_parent_guid)) {
                                ...
                                throw new InvalidLogEntryException(
                                    "Could not load GUID {$vs_parent_guid} (referenced in HIERARCHY_PARENT_ID_FLD)"
                                );
                            }
                        } elseif ($va_snapshot[$vs_field]) {
                ...
                if (($vs_field == 'row_id') && isset($va_snapshot['row_guid']) && ($t_rel_item = Datamodel::getInstanceByTableNum(
                        $va_snapshot['table_num'],
                        true
                    ))) {
                ...
                if (isset($va_many_to_one_rels[$vs_field]) && ($t_rel_item = Datamodel::getInstanceByTableName(
                        $va_many_to_one_rels[$vs_field]['one_table'],
                        true
                    ...
                    if ($t_rel_item->loadByGUID($va_snapshot[$vs_field . '_guid'])) {
                        ...
                        if (($vs_field === 'user_id') && ($AUTH_CURRENT_USER_ID > 0)) {
                            ...
                            throw new IrrelevantLogEntry(
                                _t(
                                    "%1 guid value '%2' is not defined on this system for %3: %4",
                                    $vs_field,
                ...
                if (($this->getModelInstance(
                        ) instanceof ca_representation_annotations) && ($vs_field == 'representation_id')) {
                    if (isset($va_snapshot[$vs_field . '_guid']) && ($vs_rep_guid = $va_snapshot[$vs_field . '_guid'])) {
                        ...
                        if ($t_rep->loadByGUID($vs_rep_guid)) {
                            ...
                            throw new InvalidLogEntryException(
                                "Could not load GUID {$vs_rep_guid} (referenced in representation_id)"
                            );
                        }
                    }
                }
    ...
    public function checkModelInstanceForErrors()
    {
        ...
        if ($this->getModelInstance()->numErrors() > 0) { // is this critical or not? hmm
            if (($this->getModelInstance()->numErrors() == 1) && ($o_error = $this->getModelInstance(
                )->errors[0]) && ($o_error->getErrorNumber() == 251)) {
