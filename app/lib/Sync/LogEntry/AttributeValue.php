        ...
        if (
            ...
            if (
                ...
                if ($vn_list_id = $t_element->get('list_id')) {
                    if (isset($va_snapshot['item_id_guid']) && ($vs_item_id_guid = $va_snapshot['item_id_guid'])) {
                        ...
                        if ($this->isUpdate() && !$t_item->loadByGUID($vs_item_id_guid)) {
                            throw new InvalidLogEntryException(
                                _t("Could not find list item with guid %1", $vs_item_id_guid)
                            );
                        }
                        ...
                        if (!($vn_item_id = caGetListItemID($vn_list_id, $vs_item_code))) {
                            throw new InvalidLogEntryException(
                                _t("Invalid list item code %1 for attribute value log entry.", $vs_item_code)
                            );
                        }
                        ...
                        if (!($vn_item_id = caGetListItemIDForLabel($vn_list_id, $vs_item_label))) {
                            throw new InvalidLogEntryException(
                                _t("Invalid list item label %1 for attribute value log entry.", $vs_item_label)
                            );
                        }
                    } else {
                    ...
                    throw new InvalidLogEntryException(
                        _t("Referenced element is not of type List, but item_id is set in the snapshot")
                    );
                }
            }
        }
