		...
		if (
			...
			if (
				...
				if ( $vn_list_id = $t_element->get( 'list_id' ) ) {
					if ( isset( $va_snapshot['item_id_guid'] )
					     && ( $vs_item_id_guid
							= $va_snapshot['item_id_guid'] )
					) {
						$t_item = new \ca_list_items();
						if ( $this->isUpdate() && ! $t_item->loadByGUID( $vs_item_id_guid ) ) {
							throw new InvalidLogEntryException( _t( "Could not find list item with guid %1",
								$vs_item_id_guid ) );
						}
						...
						if ( ! ( $vn_item_id = caGetListItemID( $vn_list_id, $vs_item_code ) ) ) {
							throw new InvalidLogEntryException( _t( "Invalid list item code %1 for attribute value log entry.",
								$vs_item_code ) );
						}
					} elseif ( isset( $va_snapshot['item_label'] )
					           && ( $vs_item_label
							= $va_snapshot['item_label'] )
					) {
						if ( ! ( $vn_item_id = caGetListItemIDForLabel( $vn_list_id, $vs_item_label ) ) ) {
							throw new InvalidLogEntryException( _t( "Invalid list item label %1 for attribute value log entry.",
								$vs_item_label ) );
						}
					} else {
	...
	public function setIntrinsicsFromSnapshotInModelInstance() {
		...
		foreach ( $va_snapshot as $vs_field => $vm_val ) {
			...
			if ( $vs_field == 'element_id' ) {
				...
				if ( isset( $va_snapshot['attribute_guid'] )
				     && ( $vs_attribute_guid
						= $va_snapshot['attribute_guid'] )
				) {
					$t_attr = new \ca_attributes();
				...
				if (
					...
					if ( $vn_list_id = $t_element->get( 'list_id' ) ) {
						if ( isset( $va_snapshot['item_id_guid'] )
							...
							$t_item = new \ca_list_items();
							if ( $t_item->loadByGUID( $vs_item_id_guid )
							     && ( $vn_item_id
						...
						} elseif ( isset( $va_snapshot['item_code'] )
						           && ( $vs_item_code
								= $va_snapshot['item_code'] )
						) {
							if ( $vn_item_id = caGetListItemID( $vn_list_id, $vs_item_code ) ) {
								$this->getModelInstance()->set( 'item_id', $vn_item_id );
								$this->getModelInstance()->set( 'value_longtext1', $vn_item_id );
