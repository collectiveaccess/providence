                            ...
                            if (is_array($va_source_log_entry['subjects'])) {
                                ...
                                foreach ($va_source_log_entry['subjects'] as $va_source_log_subject) {
                                    ...
                                    if ($pa_filter_on_access_settings) {
                                        if ($va_access_by_guid[$va_source_log_subject['guid']] !== '?') {
                                            $vb_have_access_to_subject = in_array(
                                                $va_access_by_guid[$va_source_log_subject['guid']],
                                                $pa_filter_on_access_settings
                                            );
                                        }
                                    }
                                    ...
                                    if (!$vb_have_access_to_subject && $vb_subject_exists_on_target) {
                                        ...
                                        $this->log(
                                            _t(
                                                "Passing on " . $va_source_log_subject['guid'] . " because it has already been seen: %1/%2",
                                                ($vb_have_access_to_subject ? "HAVE ACCESS" : "NO ACCESS"),
                                                ($vb_subject_exists_on_target ? "EXISTS ON TARGET" : "DOES NOT EXIST ON TARGET")
                                            ),
                                            Zend_Log::DEBUG
                                        );
                                        $va_filtered_log_entries[$vn_log_id] = $va_source_log_entry;
...
//                                                 $va_source_log_entries_for_missing_guids[$vs_missing_guid] = $va_entry_tmp;
                                                $this->log(
                                                    _t("Skipped %1 because we've seen it already ", $vs_missing_guid),
                                                    Zend_Log::DEBUG
                                                );
                                                continue;
                                            }
                                            ...
                                            $this->log(
                                                _t("Getting log for missing guid %1", $vs_missing_guid),
                                                Zend_Log::DEBUG
                                            );
                                            ...
                                            if (is_array($va_log_for_missing_guid)) {
                                                ...
                                                $o_guids_exist_for_missing = $o_target->setRequestMethod(
                                                    'POST'
                                                )->setEndpoint('hasGUID')
                                                    ->setRequestBody(
                                                        array_unique(
                                                ...
                                                $va_guids_that_exist_for_missing = array_keys(
                                                    array_filter(
                                                        $va_guids_exist_for_missing,
                                                        function ($v) {
                                                            return !is_array($v);
                                                        }
                                                    )
                                                ...
                                                foreach ($va_log_for_missing_guid as $va_missing_entry) {
                                                    ...
                                                    if ($pa_filter_on_access_settings && ($va_access_for_dependent[$va_missing_entry['guid']] !== '?') && !in_array(
                                                        ...
                                                        $this->log(
                                                            _t(
                                                                "SKIP %1 because we have no access: %2",
                                                                $va_missing_entry['guid'],
                                                                print_R($va_missing_entry, true)
                                                            ),
                                                            Zend_Log::DEBUG
                                                        );
                                                        continue;
                                                    ...
                                                    if ($va_guids_that_exist_for_missing && ($va_guids_that_exist_for_missing[$va_missing_entry['guid']])) {
                                                        ...
                                                        $this->log(
                                                            _t(
                                                                "SKIP %1 because it already exists on target",
                                                                $va_missing_entry['guid']
                                                            ),
                                                            Zend_Log::DEBUG
                                                        );
                                                                    ...
                                                                    array_filter(
                                                                        ...
                                                                        function ($v, $k) use (
                                                                            ...
                                                                            if ($v == $vs_missing_guid) {
                                                                                $x->log(
                                                                                    _t(
                                                                                        "SKIP dependent %1 because log entry matches guid: %3",
                                                                                        $v,
                                                                                        $matches[1],
                                                                                        print_R($va_missing_entry, true)
                                                                                    ),
                                                ...
                                                if (sizeof($va_dependent_guids) > 0) {
                                                    ...
                                                    $va_missing_guids = array_unique(
                                                        array_merge($va_missing_guids, $va_dependent_guids)
                                                ...
                                                $this->log(
                                                    _t("%1", print_R($va_filtered_log_for_missing_guid, true)),
                                                    Zend_Log::DEBUG
                                                );
                                                ...
                                                if (sizeof($va_dependent_guids) == 0) {
                                                    ...
                                                    $this->log(
                                                        _t(
                                                            "Immediately pushing %1 missing entries for %2",
                                                            sizeof($va_filtered_log_for_missing_guid),
                                                            $vs_missing_guid
                                                        ),
                                                        Zend_Log::DEBUG
                                                    ...
                                                    while (sizeof($va_filtered_log_for_missing_guid) > 0) {
                                                        ...
                                                        while (sizeof($va_filtered_log_for_missing_guid) > 0) {
                                                            ...
                                                            if (!$vn_mlog_id) {
                                                                $this->log(
                                                                    _t(
                                                                        "Skipped entry because it lacks a log_id %1",
                                                            ...
                                                            if ($vn_mlog_id >= $pn_start_replicated_id) {
                                                                $this->log(
                                                                    _t(
                                                                        "Skipped %1 because it's in the future",
                                                                        $vn_mlog_id
                                                                    ),
                                                                    Zend_Log::DEBUG
                                                                );
                                                            ...
                                                            if (($va_log_entry['logged_table_num'] == 3) && ($vs_attr_guid = caGetOption(
                                                                ...
                                                                if ($va_chk_attr_existence[$vs_attr_guid] == '???') {
                                                                    ...
                                                                    if (is_array($va_attr_log)) {
                                                                        ...
                                                                        $this->log(
                                                                            _t(
                                                                                "Adding %3 unpushed attribute log entries starting with %1 for %2 for immediate push.",
                                                            ...
                                                            if ((sizeof($va_entries) >= 10) || (sizeof(
                                                        ...
                                                        $this->log(
                                                            _t(
                                                                "Immediately pushing missing log entries starting with %1 for %2. %3",
                                                                $vn_first_missing_log_id,
                                                                $vs_missing_guid,
                                                                print_R($va_entries, true)
                                                            ),
                                                            Zend_Log::DEBUG
                                                        );
                        ...
                        if (sizeof($va_source_log_entries_for_missing_guids)) {
                            $va_source_log_entries_for_missing_guids = array_reverse(
                                $va_source_log_entries_for_missing_guids
                            );
                            ...
                            foreach ($va_source_log_entries_for_missing_guids as $vs_missing_guid => $va_source_log_entries_for_missing_guid) {
                                $this->log(
                                    _t(
                                        "Pushing %1 missing entries for %2",
                                ...
                                while (sizeof($va_source_log_entries_for_missing_guid) > 0) {
                                    ...
                                    while (sizeof($va_source_log_entries_for_missing_guid) > 0) {
                                        ...
                                        if ($vn_mlog_id >= $pn_start_replicated_id) {
                                            $this->log(
                                                _t("Skipped %1 because it's in the future", $vn_mlog_id),
                                                Zend_Log::DEBUG
                                            );
                                            continue;
                                        }
                                        ...
                                        foreach ($va_log_entry['snapshot'] as $k => $v) {
                                            if (caIsGUID($v) && in_array($v, $va_guids_to_skip, true)) {
                                                if (preg_match("!parent!", $k)) {
                                                    ...
                                                    $this->log(
                                                        _t(
                                                            "Skipped %1 because dependency %2 is not available",
                                                            $vn_mlog_id,
                                                            $v
                                                        ),
                                                        Zend_Log::DEBUG
                                        ...
                                        if (sizeof($va_dependency_list[$va_log_entry['guid']]) > 0) {
                                            ...
                                            $this->log(
                                                _t(
                                                    "Skipped %1 because it still has dependencies: %2",
                                                    $vn_mlog_id,
                                                    print_R($va_dependency_list[$va_log_entry['guid']], true)
                                                ),
                                                Zend_Log::DEBUG
                                            );
                                            continue;
                                        ...
                                        if ($va_seen[$vn_mlog_id]) {
                                            ...
                                            $this->log(
                                                _t("Skipped %1 because it has already been pushed", $vn_mlog_id),
                                                Zend_Log::DEBUG
                                            );
                                            continue;
                                        }
                                        ...
                                        if (($va_log_entry['logged_table_num'] == 3) && ($vs_attr_guid = caGetOption(
                                            ...
                                            $o_chk_attr_existence = $o_target->setRequestMethod('POST')->setEndpoint(
                                                'hasGUID'
                                            )
                                                ->setRequestBody([$vs_attr_guid])
                                                ->request();
                                            ...
                                            if ($va_chk_attr_existence[$vs_attr_guid] == '???') {
                                                ...
                                                if (is_array($va_attr_log) && sizeof($va_attr_log)) {
                                                    ...
                                                    $this->log(
                                                        _t(
                                                            "Adding %3 unpushed attribute log entries starting with %1 for %2 [%4].",
                                                            $vn_first_missing_log_id,
                                                            $vs_missing_guid,
                                                            sizeof($acc),
                                                            $vs_attr_guid
                                    ...
                                    $this->log(
                                        _t(
                                            "Pushing %3 missing log entries starting with %1 for %2. [%4]",
                                            $vn_first_missing_log_id,
                                            $vs_missing_guid,
                                            sizeof($va_entries),
                                            print_R($va_entries, true)
                    ...
                    if (is_array($va_filtered_log_entries)) {
                        if (sizeof($va_filtered_log_entries) == 0) {
                            ...
                            $this->log(
                                _t(
                                    "Nothing to push. Incrementing log index to %1 (%2)",
                                    $pn_replicated_log_id,
                    ...
                    $va_set_intrinsics_source = is_array(
                        $va_set_intrinsics_config[$vs_source_system_guid]
                    ) ? $va_set_intrinsics_config[$vs_source_system_guid] : array();
                    ...
                    if (!$o_resp->isOk() || !isset($va_response_data['replicated_log_id'])) {
                        ...
                        $this->log(
                            _t(
                                "Pushed %1 log entries. Incrementing log index to %2 (%3)",
                                $vn_num_log_entries,
                                $pn_replicated_log_id,
                                date(DATE_RFC2822, $va_last_log_entry['log_datetime'])
                            ),
