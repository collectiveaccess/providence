...
class Replicator {
	...
	public function replicate() {
		foreach ( $this->getSourcesAsServiceClients() as $vs_source_key => $o_source ) {
			...
			$o_result = $o_source->setEndpoint( 'getsysguid' )->setRetries( $this->max_retries )
			                     ->setRetryDelay( $this->retry_delay )->request();
			if ( ! $o_result || ! ( $res = $o_result->getRawData() )
			     || ! ( strlen( $source_system_guid = $res['system_guid'] ) )
			) {
				$this->log(
			...
			foreach ( $this->getTargetsAsServiceClients() as $vs_target_key => $o_target ) {
				...
				if ( ! $o_result || ! is_array( $res = $o_result->getRawData() ) || isset( $res['errors'] ) ) {
					$this->log( _t( "There were errors getting last replicated log id for source %1 and target %2: %3.",
						$vs_source_key, $vs_target_key, join( '; ', $res['errors'] ) ), Zend_Log::ERR );
					continue;
				}
				...
				if ( $ps_min_log_timestamp
					= $this->opo_replication_conf->get( 'sources' )[ $vs_source_key ]['from_log_timestamp']
				) {
					...
					$pn_min_log_id
						= (int) $this->opo_replication_conf->get( 'sources' )[ $vs_source_key ]['from_log_id'];
				}
				if ( $pn_min_log_id > $replicated_log_id ) {
					$replicated_log_id = $pn_min_log_id;
					$this->log( _t( "[%1] Set log id to minimum (%2).", $vs_source_key, $replicated_log_id ),
						Zend_Log::INFO );
				}

				// get skip if expression
				$pa_skip_if_expression
					                   = $this->opo_replication_conf->get( 'sources' )[ $vs_source_key ]['skipIfExpression'];
				...
				if ( is_array(
					$pa_ignore_tables_global = $this->opo_replication_conf->get( 'sources' )['ignoreTables'] )
				) {
				...
				if ( is_array(
					$pa_only_tables_global = $this->opo_replication_conf->get( 'sources' )['onlyTables'] )
				) {
				...
				$pa_include_metadata
					                 = $this->opo_replication_conf->get( 'sources' )[ $vs_source_key ]['includeMetadata'];
				...
				$pa_exclude_metadata
					                 = $this->opo_replication_conf->get( 'sources' )[ $vs_source_key ]['excludeMetadata'];
				...
				if ( ! is_array( $pa_filter_on_access_settings
						= $this->opo_replication_conf->get( 'sources' )[ $vs_source_key ]['filter_on_access_settings'] )
				     || ! sizeof( $pa_filter_on_access_settings )
				) {
				...
				$missing_guids
					= []; // List of guids missing on target that we'll need to replicate to synthesize subject
				...
				while ( true ) { // use chunks of 10 entries until something happens (success/err)
					...
					if ( sizeof( $this->sent_log_ids ) > 100000 ) {
						$this->log( _t( "Reset sent log list because it was over 100000 entries. Memory usage was %1",
							caGetMemoryUsage() ), Zend_Log::DEBUG );
						$this->sent_log_ids = [];
					}
					...
					$va_source_log_entries = $o_source->setEndpoint( 'getlog' )->clearGetParameters()
					                                  ...
					                                  ->setRetries( $this->max_retries )
					                                  ->setRetryDelay( $this->retry_delay )
					                                  ->request()->getRawData();
					...
					$this->log( _t( "[%1] Found %2 source log entries starting at %3 [%4 - %5].", $vs_source_key,
						sizeof( $va_source_log_entries ), $replicated_log_id, $start_log_id, $end_log_id ),
						Zend_Log::DEBUG );
					...
					if (
						...
						$o_guid_already_exists  = $o_target->setRequestMethod( 'POST' )->setEndpoint( 'hasGUID' )
						                                   ->setRequestBody( array_merge( caExtractArrayValuesFromArrayOfArrays( $va_source_log_entries,
							                                   'guid' ), array_keys( $va_subject_guids ) ) )
						                                   ->setRetries( $this->max_retries )
						                                   ->setRetryDelay( $this->retry_delay )
						                                   ->request();
						...
						$vs_access         = is_array( $pa_filter_on_access_settings ) ? join( ";",
							$pa_filter_on_access_settings ) : "";
						$o_access_by_guid  = $o_source->setRequestMethod( 'POST' )->setEndpoint( 'hasAccess' )
						                              ->addGetParameter( 'access', $vs_access )
						                              ->setRequestBody( array_unique( array_merge( caExtractArrayValuesFromArrayOfArrays( $va_source_log_entries,
							                              'guid' ), array_keys( $va_subject_guids ) ) ) )
						                              ->setRetries( $this->max_retries )
						                              ->setRetryDelay( $this->retry_delay )
						                              ->request();
						...
						foreach ( $va_source_log_entries as $vn_log_id => $va_source_log_entry ) {
							...
							$vb_logged_exists_on_target
								= is_array( $va_guid_already_exists[ $va_source_log_entry['guid'] ] );
							if ( $pa_filter_on_access_settings
							     && ( $va_access_by_guid[ $va_source_log_entry['guid'] ] !== '?' )
							     && ! in_array( (int) $va_access_by_guid[ $va_source_log_entry['guid'] ],
									$pa_filter_on_access_settings, true )
							     && ! $vb_logged_exists_on_target
							) {
							...
							if ( ! $vb_logged_exists_on_target ) {
								$missing_guids[]
									= $va_source_log_entry['guid']; // add changed "source" row to replication list
							}
							...
							if ( is_array( $va_source_log_entry['subjects'] ) ) {
								...
								foreach ( $va_source_log_entry['subjects'] as $va_source_log_subject ) {
									...
									$vb_subject_exists_on_target
										= is_array( $va_guid_already_exists[ $va_source_log_subject['guid'] ] );
									$vb_subject_is_relationship
										= Datamodel::isRelationship( $va_source_log_subject['subject_table_num'] );

									$vb_have_access_to_subject = true;
									if ( $pa_filter_on_access_settings ) {
										if ( $va_access_by_guid[ $va_source_log_subject['guid'] ] !== '?' ) {
											$vb_have_access_to_subject
												= in_array( (int) $va_access_by_guid[ $va_source_log_subject['guid'] ],
												$pa_filter_on_access_settings, true );
										}
									}
									...
									if ( ! $vb_have_access_to_subject && $vb_subject_exists_on_target ) {
										...
										$missing_guids[]
											= $va_source_log_subject['guid'];   // add subject to list of guids to replicate
										...
										while ( ( sizeof( $missing_guids ) > 0 ) ) {
											...
											if ( $source_log_entries_for_missing_guids_seen_guids[ $vs_missing_guid ] ) {
												$this->log( _t( "[%1] Skipped %2 because we've seen it already.",
													$vs_source_key, $vs_missing_guid ), Zend_Log::DEBUG );
												continue;
											}
...
// 											}
											...
											$this->log( _t( "[%1] Fetching log for missing guid %2.", $vs_source_key,
												$vs_missing_guid ), Zend_Log::DEBUG );
											$va_log_for_missing_guid = $o_source->setEndpoint( 'getlog' )
											                                    ->clearGetParameters()
											                                    ->addGetParameter( 'forGUID',
												                                    $vs_missing_guid )
											                                    ->addGetParameter( 'skipIfExpression',
												                                    $vs_skip_if_expression )
											                                    ->addGetParameter( 'ignoreTables',
												                                    $vs_ignore_tables )
											                                    ->addGetParameter( 'onlyTables',
												                                    $vs_only_tables )
											                                    ->addGetParameter( 'includeMetadata',
												                                    $vs_include_metadata )
											                                    ->addGetParameter( 'excludeMetadata',
												                                    $vs_exclude_metadata )
											                                    ->addGetParameter( 'pushMediaTo',
												                                    $vs_push_media_to )
											                                    ->setRetries( $this->max_retries )
											                                    ->setRetryDelay( $this->retry_delay )
											                                    ->request()->getRawData();
											...
											if ( is_array( $va_log_for_missing_guid ) ) {
												...
												$o_access_for_dependent  = $o_source->setRequestMethod( 'POST' )
												                                    ->setEndpoint( 'hasAccess' )
												                                    ->addGetParameter( 'access',
													                                    $vs_access )
												                                    ->setRequestBody( array_unique( caExtractArrayValuesFromArrayOfArrays( $va_log_for_missing_guid,
													                                    'guid' ) ) )
												                                    ->setRetries( $this->max_retries )
												                                    ->setRetryDelay( $this->retry_delay )
												                                    ->request();
												...
												$this->log( _t( "[%1] Missing log for %2 is %3", $vs_source_key,
													$vs_missing_guid, print_R( $va_log_for_missing_guid, true ) ),
													Zend_Log::DEBUG );
												foreach ( $va_log_for_missing_guid as $va_missing_entry ) {
													...
													if ( $pa_filter_on_access_settings
													     && ( $va_access_for_dependent[ $va_missing_entry['guid'] ]
													          !== '?' )
													     && ! in_array( (int) $va_access_for_dependent[ $va_missing_entry['guid'] ],
															$pa_filter_on_access_settings, true )
													) {
														// skip rows for which we have no access
														$this->log( _t( "[%1] Skip %2 because we have no access.",
															$vs_source_key, $va_missing_entry['guid'] ),
															Zend_Log::DEBUG );
														continue;
													}

													$va_filtered_log_for_missing_guid[ $va_missing_entry['log_id'] ]
														= $va_missing_entry;

													// Add guids for dependencies referenced by this log entry
													if ( is_array( $va_missing_entry['snapshot'] ) ) {
														$va_dependent_guids
															= array_unique( array_merge( $va_dependent_guids,
															array_values( array_filter( $va_missing_entry['snapshot'],
																function ( $v, $k ) use (
																	$va_missing_entry, $vs_missing_guid
																) {
																	if ( $v == $vs_missing_guid ) {
																		return false;
																	}
																	if ( preg_match( "!([A-Za-z0-9_]+)_guid$!", $k,
																		$matches )
																	) {
																		if (
																			is_array( Datamodel::getFieldInfo( $va_missing_entry['logged_table_num'],
																				$matches[1] ) )
																			|| is_array( Datamodel::getFieldInfo( $va_missing_entry['logged_table_num'],
																				$matches[1] . '_id' ) )
																		) {
												...
												if ( sizeof( $va_dependent_guids ) > 0 ) {
													...
													while ( ( $tries < 5 )
													        && ( ! is_array( $va_guids_exist_for_dependencies ) ) ) {
														$o_guids_exist_for_dependencies
															= $o_target->setRequestMethod( 'POST' )
															           ->setEndpoint( 'hasGUID' )
															           ->setRequestBody( $va_dependent_guids )
															           ->setRetries( $this->max_retries )
															           ->setRetryDelay( $this->retry_delay )
															           ->request();
														$va_guids_exist_for_dependencies
															= $o_guids_exist_for_dependencies->getRawData();
														$tries ++;
													}

													$va_dependent_guids
														= array_keys( array_filter( $va_guids_exist_for_dependencies,
														function ( $v ) {
															return ! is_array( $v );
														} ) );

													$missing_guids = array_unique( array_merge( $missing_guids,
														$va_dependent_guids ) );  // add dependent guid lists to "missing" list; this will forcing dependencies to be processed through this loop

													if ( is_array( $pa_filter_on_access_settings ) ) {
														// Filter missing guid list using access criteria
														$o_access_for_missing  = $o_source->setRequestMethod( 'POST' )
														                                  ->setEndpoint( 'hasAccess' )
														                                  ->addGetParameter( 'access',
															                                  $vs_access )
														...
														if ( is_array( $va_access_for_missing ) ) {
															$missing_guids
																= array_keys( array_filter( $va_access_for_missing,
																function ( $v ) use ( $pa_filter_on_access_settings ) {
																	return ( ( $v == '?' )
																	         || ( in_array( (int) $v,
																			$pa_filter_on_access_settings, true ) ) );
																} ) );
															$this->guids_to_skip
																= array_unique( array_merge( $this->guids_to_skip,
																array_keys( array_filter( $va_access_for_missing,
																	function ( $v ) use ( $pa_filter_on_access_settings
																	) {
																		return ! in_array( (int) $v,
																			$pa_filter_on_access_settings, true );
																	} ) ) ) );
														} else {
															$this->log( _t( "[%1] Failed to retrieve access values for missing GUID.",
																$vs_source_key ), Zend_Log::DEBUG );
														}
													}
												}

												ksort( $va_filtered_log_for_missing_guid );
												$this->log( _t( "[%1] Found %2 entries for %3.", $vs_source_key,
													sizeof( $va_filtered_log_for_missing_guid ), $vs_missing_guid ),
													Zend_Log::DEBUG );

												if ( sizeof( $va_dependent_guids ) == 0 ) {
													// Missing guid has no outstanding dependencies so push it immediately
													$this->log( _t( "[%1] Immediately pushing %2 missing entries for %3.",
														$vs_source_key, sizeof( $va_filtered_log_for_missing_guid ),
														$vs_missing_guid ), Zend_Log::DEBUG );
													...
													while ( sizeof( $va_filtered_log_for_missing_guid ) > 0 ) {
														...
														while ( sizeof( $va_filtered_log_for_missing_guid ) > 0 ) {
															$va_log_entry
																= array_shift( $va_filtered_log_for_missing_guid );
															...
															if ( ! $vn_mlog_id ) {
																$this->log( _t( "[%1] Skipped entry because it lacks a log_id.",
																	$vs_source_key ), Zend_Log::WARN );
																continue;
															}
															if ( $vn_log_id == $vn_mlog_id ) {
																$this->log( _t( "[%1] Skipped missing entry because it is the current id.",
																	$vs_source_key ), Zend_Log::DEBUG );
																continue;
															} // don't pull in current log entry

															if ( $vn_mlog_id > $this->last_log_id ) {
																$this->log( _t( "[%1] Skipped entry (%2) because it's in the future.",
																	$vs_source_key, $vn_mlog_id ), Zend_Log::DEBUG );
																continue;
															}
															if ( $vn_mlog_id >= $pn_start_replicated_id ) {
																$this->log( _t( "[%1] Skipped entry (%2) because it's within the current sync frame (log_id >= %3).",
																	$vs_source_key, $vn_mlog_id,
																	$pn_start_replicated_id ), Zend_Log::DEBUG );
																continue;
															}
															...
															if ( ( $va_log_entry['logged_table_num'] == 3 )
															     && ( $vs_attr_guid = caGetOption( [
																	'attribute_guid',
																	'attribute_id_guid'
																], $va_log_entry['snapshot'], null ) )
															) {
																$o_chk_attr_existence
																	= $o_target->setRequestMethod( 'POST' )
																	           ->setEndpoint( 'hasGUID' )
																	           ...
																	           ->setRetryDelay( $this->retry_delay )
																	           ->request();
																$va_chk_attr_existence
																	= $o_chk_attr_existence->getRawData();
																	...
																	if ( is_array( $va_attr_log ) ) {
																		...
																		foreach ( $va_attr_log as $l => $x ) {
																			if ( ! $va_seen[ $x['log_id'] ] ) {
																				...
																				$source_log_entries_for_missing_guids_seen_guids[ $x['guid'] ]
																					= true;
																			}
																		}
																		$size = sizeof( $acc );
																		if ( $size > 0 ) {
																			$this->log( _t( "[%1] Adding %4 unpushed attribute log entries starting with %2 for %3 for immediate push.",
																				$vs_source_key,
																				$vn_first_missing_log_id,
																				$vs_missing_guid, $size ),
																				Zend_Log::DEBUG );
																		}
																	}
																}
															...
															// mark as seen so we don't process the same thing twice
															$source_log_entries_for_missing_guids_seen_guids[ $va_log_entry['guid'] ]
																= true;
															if ( ( is_array( $va_entries )
															       && ( sizeof( $va_entries ) >= 10 ) )
															     || ( is_array( $va_source_log_entries_for_missing_guid )
															          && ( sizeof( $va_source_log_entries_for_missing_guid )
															               == 0 ) )
															) {
																break;
															}
														...
														if ( $size > 0 ) {
															$this->log( _t( "[%1] Immediately pushing missing log entries starting with %2 for %3.",
																$vs_source_key, $vn_first_missing_log_id,
																$vs_missing_guid ), Zend_Log::DEBUG );
														}

														$o_backlog_resp = $o_target->setRequestMethod( 'POST' )
														                           ->setEndpoint( 'applylog' )
														                           ->addGetParameter( 'system_guid',
															                           $source_system_guid )
													...
													$source_log_entries_for_missing_guids[ $vs_missing_guid ]
														= $va_filtered_log_for_missing_guid;

													// mark as seen so we don't process the same thing twice
													$source_log_entries_for_missing_guids_seen_guids[ $vs_missing_guid ]
														= true;
												}
											} else {
												$this->log( _t( "[%1] No log for %2.", $vs_source_key, $vs_guid ),
													Zend_Log::WARN );
											}
						...
						if ( sizeof( $source_log_entries_for_missing_guids ) ) {
							$source_log_entries_for_missing_guids
								= array_reverse( $source_log_entries_for_missing_guids );

							list( $source_log_entries_for_missing_guids,
								$source_log_entries_for_missing_guids_seen_guids )
								= $this->_pushMissingGUIDs( $source_log_entries_for_missing_guids,
								$source_log_entries_for_missing_guids_seen_guids );
						}
					}
					...
					if ( is_array( $va_filtered_log_entries ) ) {
						if ( sizeof( $va_filtered_log_entries ) == 0 ) {
							...
							$this->log( _t( "[%1] Nothing to push. Incrementing log index to %2 (%3)", $vs_source_key,
								$replicated_log_id, date( DATE_RFC2822, $va_last_log_entry['log_datetime'] ) ),
								Zend_Log::DEBUG );
					...
					$va_set_intrinsics_config
						                       = $this->opo_replication_conf->get( 'targets' )[ $vs_target_key ]['setIntrinsics'];
					$va_set_intrinsics_default = is_array( $va_set_intrinsics_config['__default__'] )
						? $va_set_intrinsics_config['__default__'] : array();
					$va_set_intrinsics_source  = is_array( $va_set_intrinsics_config[ $source_system_guid ] )
						? $va_set_intrinsics_config[ $source_system_guid ] : array();
					...
					$this->log( _t( "[%1] Pushed %2 primary entries.", $vs_source_key,
						sizeof( $va_source_log_entries ) ), Zend_Log::DEBUG );
					$va_response_data = $o_resp->getRawData();

					if ( ! $o_resp->isOk() || ! isset( $va_response_data['replicated_log_id'] ) ) {
						$this->log( _t( "There were errors while processing sync for source %1 and target %2: %3",
							$vs_source_key, $vs_target_key, join( ' ', $o_resp->getErrors() ) ), Zend_Log::ERR );

						break;
					} else {
						$replicated_log_id = ( $this->last_log_id > 0 ) ? ( $this->last_log_id + 1 )
							: ( (int) $va_response_data['replicated_log_id'] ) + 1;
						$this->log( _t( "Chunk sync for source %1 and target %2 successful.", $vs_source_key,
							$vs_target_key ), Zend_Log::DEBUG );
						$vn_num_log_entries = sizeof( $va_source_log_entries );
						$va_last_log_entry  = array_pop( $va_source_log_entries );

						$this->log( _t( "[%1] Pushed %2 log entries. Incrementing log index to %3 (%4).",
							$vs_source_key, $vn_num_log_entries, $replicated_log_id,
							date( DATE_RFC2822, $va_last_log_entry['log_datetime'] ) ), Zend_Log::DEBUG );
						$this->log( _t( "[%1] Last replicated log ID is: %2 (%3).", $vs_source_key,
							$va_response_data['replicated_log_id'],
							date( DATE_RFC2822, $va_last_log_entry['log_datetime'] ) ), Zend_Log::DEBUG );
					}

					if ( isset( $va_response_data['warnings'] ) && is_array( $va_response_data['warnings'] )
					     && sizeof( $va_response_data['warnings'] )
					) {
					...
					if ( sizeof( $source_log_entries_for_missing_guids ) ) {    // try to run missing queue
						$this->log( _t( "[%1] Running missing guid queue with %2 guids.", $vs_source_key,
							sizeof( $source_log_entries_for_missing_guids ) ), Zend_Log::DEBUG );
						$source_log_entries_for_missing_guids = array_reverse( $source_log_entries_for_missing_guids );

						list( $source_log_entries_for_missing_guids, $source_log_entries_for_missing_guids_seen_guids )
							= $this->_pushMissingGUIDs( $source_log_entries_for_missing_guids,
							$source_log_entries_for_missing_guids_seen_guids );
					}
				}
				...
				if ( $pb_ok ) {
					$this->log( _t( "Sync for source %1 and target %2 successful.", $vs_source_key, $vs_target_key ),
						Zend_Log::INFO );

					// run dedup if configured
					$va_dedup_after_replication
						                        = $this->opo_replication_conf->get( 'targets' )[ $vs_target_key ]['deduplicateAfterReplication'];
					...
					if ( is_array( $va_dedup_after_replication ) && sizeof( $va_dedup_after_replication ) ) {
						...
						if ( ! $o_dedup_response->isOk() ) {
							$this->log( _t( "There were errors while processing deduplication for at target %1: %2.",
								$vs_target_key, join( ' ', $o_dedup_response->getErrors() ) ), Zend_Log::ERR );
						} else {
					...
					$this->log( _t( "Sync for source %1 and target %2 finished, but there were errors.", $vs_source_key,
						$vs_target_key ), Zend_Log::ERR );
				}
			}
		}
	}
					...
					foreach ( $entry['snapshot'] as $snk => $snv ) {
						if ( preg_match( "!([A-Za-z0-9_]+)_guid$!", $snk, $matches ) ) {
							if (
								...
								|| is_array( Datamodel::getFieldInfo( $entry['logged_table_num'],
									$matches[1] . '_id' ) )
							) {
								...
								$this->log( _t( "[%1] Added %2 [%3] as dep from snapshot for %4.", $this->source_key,
									$snk, $snv, $entry['guid'] ), Zend_Log::DEBUG );
	 ...
	 *
	 */
	private function _pushMissingGUIDs(
		$source_log_entries_for_missing_guids, $source_log_entries_for_missing_guids_seen_guids
	) {
		...
		foreach ( $source_log_entries_for_missing_guids as $vs_missing_guid => $va_source_log_entries_for_missing_guid )
		{
			if ( is_array( $va_dependency_list[ $vs_missing_guid ] )
			     && ( ( $s
						= sizeof( $va_dependency_list[ $vs_missing_guid ] ) ) > 0 )
			) {
				$this->log( _t( "[%1] Holding push of %2 missing entries for %3 because it still has %4 dependencies: %5.",
					$this->source_key, sizeof( $va_source_log_entries_for_missing_guid ), $vs_missing_guid, $s,
					print_R( array_keys( $va_dependency_list[ $vs_missing_guid ] ), true ) ), Zend_Log::DEBUG );
				continue;
			}
			...
			$this->log( _t( "[%1] Pushing %2 missing entries for %3.", $this->source_key,
				sizeof( $va_source_log_entries_for_missing_guid ), $vs_missing_guid ), Zend_Log::DEBUG );
			...
			while ( sizeof( $va_source_log_entries_for_missing_guid ) > 0 ) {
				...
				while ( sizeof( $va_source_log_entries_for_missing_guid ) > 0 ) {
					...
					if ( ! $vn_mlog_id ) {
						$this->log( _t( "[%1] Skipped entry because it lacks a log_id %2.", $this->source_key,
							print_R( $va_log_entry, true ) ), Zend_Log::DEBUG );
						continue;
					}
					if ( $vn_mlog_id > $this->last_log_id ) {
						$this->log( _t( "[%1] Skipped %2 (during push of missing?) because it's in the future.",
							$this->source_key, $vn_mlog_id ), Zend_Log::DEBUG );
						continue;
					}
					...
					foreach ( $va_log_entry['snapshot'] as $k => $v ) {
						if ( in_array( $v, $this->guids_to_skip, true ) ) {
							...
							if ( preg_match( "!parent!", $k ) ) {
								...
								$this->log( _t( "[%1] Skipped %2 because dependency %3 is not available.",
									$this->source_key, $vn_mlog_id, $v ), Zend_Log::DEBUG );
								continue( 2 );
							}
						}
					}

					if ( is_array( $va_dependency_list[ $va_log_entry['guid'] ] )
					     && ( ( $s
								= sizeof( $va_dependency_list[ $va_log_entry['guid'] ] ) ) > 0 )
					) {
						// Skip log entry because it still has dependencies
						$this->log( _t( "[%1] Skipped log_id %2 [%3] because it still has %4 dependencies: %5.",
							$this->source_key, $vn_mlog_id, $va_log_entry['guid'], $s,
							print_R( array_keys( $va_dependency_list[ $va_log_entry['guid'] ] ), true ) ),
							Zend_Log::DEBUG );
						continue;
					}
					...
					if ( $va_seen[ $vn_mlog_id ] ) {
						...
						$this->log( _t( "[%1] Skipped %2 because it has already been pushed.", $this->source_key,
							$vn_mlog_id ), Zend_Log::DEBUG );
						continue;
					}
					...
					if ( ( $va_log_entry['logged_table_num'] == 3 )
					     && ( $vs_attr_guid
							= caGetOption( [ 'attribute_guid', 'attribute_id_guid' ], $va_log_entry['snapshot'],
							null ) )
					) {
						...
						if ( $va_chk_attr_existence[ $vs_attr_guid ] == '???' ) {
							...
							$va_attr_log = $this->source->setEndpoint( 'getlog' )
							                            ...
							                            ->addGetParameter( 'skipIfExpression',
								                            $this->get_log_service_params['skipIfExpression'] )
							                            ->addGetParameter( 'ignoreTables',
								                            $this->get_log_service_params['ignoreTables'] )
							                            ->addGetParameter( 'onlyTables',
								                            $this->get_log_service_params['onlyTables'] )
							                            ->addGetParameter( 'includeMetadata',
								                            $this->get_log_service_params['includeMetadata'] )
							                            ->addGetParameter( 'excludeMetadata',
								                            $this->get_log_service_params['excludeMetadata'] )
							                            ->addGetParameter( 'pushMediaTo',
								                            $this->get_log_service_params['pushMediaTo'] )
							                            ->setRetries( $this->max_retries )
							                            ->setRetryDelay( $this->retry_delay )
							                            ->request()->getRawData();
							...
							if ( is_array( $va_attr_log ) && sizeof( $va_attr_log ) ) {
								...
								foreach ( $va_attr_log as $x ) {
									if ( $x['log_id'] == 1 ) {
										...
										$base_log_id        = $vn_mlog_id
										                      - 1; // If all else fails we'll map sync attr before current, as that has the best odds of working
										$sorted_missing_log = $va_attr_log;
										ksort( $sorted_missing_log );
										foreach ( $sorted_missing_log as $l ) {
											if ( ( $l['logged_table_num'] == 3 )
											     && ( $l['snapshot']['attribute_guid'] === $x['guid'] )
											) {
												$base_log_id = (int) $l['log_id']
												               - 1;    // insert before attribute value
												break;
											}
										}
										...
										if ( $synth_log_id ) {
											...
											$this->log( "Unable to map synth attr $vs_attr_guid log_id.",
												Zend_Log::DEBUG );
										}
									}
								...
								$size = sizeof( $acc );
								if ( $size > 0 ) {
									$this->log( _t( "[%1] Adding %4 unpushed attribute log entries starting with %2 for %3 [%5].",
										$this->source_key, $vn_first_missing_log_id, $vs_missing_guid, $size,
										$vs_attr_guid ), Zend_Log::DEBUG );
								}
							}
						}
					}
					...
					if ( ( sizeof( $va_entries ) >= 10 )
					     || ( sizeof( $va_source_log_entries_for_missing_guid ) == 0 )
					) {
						break;
					}
				}
				...
				if ( $size > 0 ) {
					$this->log( _t( "[%1] Pushing %4 missing log entries starting with %2 for %3.", $this->source_key,
						$vn_first_missing_log_id, $vs_missing_guid, $size ), Zend_Log::DEBUG );
				}
