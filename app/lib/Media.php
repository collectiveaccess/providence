...
class Media extends BaseObject
{
    ...
    public function __construct($pb_no_cache = false)
    {
        ...
        if (!(Media::$s_file_extension_to_plugin_map = CompositeCache::contains(
            'media_file_extension_to_plugin_map'
        ))) {
    ...
    {
        if (!($p = $this->getUnregisteredPlugin($ps_plugin_name))) {
            return null;
        }
        ...
        if ($vo_instance = $p->register()) {
            if ($this->DEBUG) {
                print "[DEBUG] LOADED $ps_plugin_name<br>\n";
            }
            return Media::$WLMedia_plugin_cache[$ps_plugin_name] = $vo_instance;
        } else {
            if ($this->DEBUG) {
                print "[DEBUG] DID NOT LOAD $ps_plugin_name<br>\n";
            }
    ...
    {
        if (!in_array($ps_plugin_name, $this->getPluginNames())) {
            return null;
        }
    ...
    public function checkPluginStatus($ps_plugin_name)
    {
        ...
        if (isset(Media::$WLMedia_plugin_cache[$ps_plugin_name])) {
            return Media::$WLMedia_plugin_cache[$ps_plugin_name];
        }
