...
class Media extends BaseObject {
	...
	public function __construct( $pb_no_cache = false ) {
		...
		if ( ! ( Media::$s_file_extension_to_plugin_map
			= CompositeCache::contains( 'media_file_extension_to_plugin_map' ) )
		) {
	 ...
	 *                        returnPluginInstance = Return instance of media plugin rather than mimetype. [Default is
	 *                        false]
	 *
	 * @return mixed String Mimetype of file, or null if file is in an unknown format. If returnPluginInstance option
	 *               is set then a plugin capable of handling the file is returned.
	 */
	function divineFileFormat( $ps_filepath, $options = null ) {
		...
		if ( ! $pb_no_cache && $pb_return_plugin_instance
		     && isset( Media::$s_divine_cache[ $ps_filepath . '_plugin' ] )
		) {
		...
		if ( $vs_plugin_name = Media::$s_file_extension_to_plugin_map[ pathinfo( $ps_filepath,
			PATHINFO_EXTENSION ) ]
		) {
	 ...
	 *
	 * @param array $options Options include:
	 *                       cleanupFiles = Delete generated files when Media instance is garbage collected. [Default
	 *                       is true]
	 */
	...
	/**
	 * Return mimetype for given file extension. Only formats supported by an installed plugin for import or export are
	 * recognized.
	 *
	...
	public static function getMimetypeForExtension( $ps_extension ) {
		...
		CompositeCache::save( $ps_extension, $ret = $va_formats[ strtolower( $ps_extension ) ],
			'Media_getMimetypeForExtension' );
	...
	/**
	 * Return file extension for given mimetype. Only formats supported by an installed plugin for import or export are
	 * recognized.
	 *
	...
	public static function getExtensionForMimetype( $mimetype ) {
		...
		CompositeCache::save( $mimetype, $ret = $va_formats[ strtolower( $mimetype ) ],
			'Media_getExtensionForMimetype' );
	...
	/**
	 * Return file type name for given mimetype. Only formats supported by an installed plugin for import or export are
	 * recognized.
	 *
