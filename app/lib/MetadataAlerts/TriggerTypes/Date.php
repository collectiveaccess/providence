...
class Date extends Base
{
    ...
    public function getTypeSpecificSettings()
    {
        return [
            'offset' => array(
                ...
                'suffix' => _t('date'),
                'description' => _t(
                    'Set an interval before or after an event in which to trigger a notification. Note that if the event is a date range, the trigger will fire before the beginning of the range or after the end of the range.'
                )
            ),
        ];
    }
    ...
    public function check(&$t_instance)
    {
        ...
        foreach (
            $t_instance->get(
                $vs_get_spec,
                ['returnAsArray' => true, 'dateFormat' => 'iso8601', 'filters' => $va_filters]
            ) as $vs_val
        ) {
            $o_tep->parse($vs_val);
    ...
    public function getElementFilters($pn_element_id, $ps_prefix_id, array $pa_options = [])
    {
        if ($t_root = ca_metadata_elements::getInstance(
            ca_metadata_elements::getElementHierarchyID($pn_element_id)
        )) {
    ...
    public function getEventKey($t_instance)
    {
        $va_spec = $this->_getSpec($t_instance);
        return md5(
            $t_instance->tableName() . '/' . $t_instance->getPrimaryKey() . '/' . $t_instance->get($va_spec['spec'])
        );
    }
    ...
    public function getTriggerQueryCriteria($pa_trigger_values)
    {
        ...
        $vs_parent_code = ca_metadata_elements::getElementCodeForId(
            ca_metadata_elements::getElementHierarchyID($vs_element_code)
        );
        ...
        if ($vn_offset <= 0) {
            ...
            $vn_start = time() - abs(
                    $vn_offset
                ) - (12 * 60 * 60);                    // start: offset seconds in the past
            $vn_end = time();                                        // end:  now
        }
