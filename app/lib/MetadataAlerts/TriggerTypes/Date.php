...
class Date extends Base {
	...
	public function check( &$t_instance ) {
		...
		if ( \ca_metadata_elements::getDataTypeForElementCode( $vs_element_code )
		...
		foreach (
			$t_instance->get( $vs_get_spec,
				[ 'returnAsArray' => true, 'dateFormat' => 'iso8601', 'filters' => $va_filters ] ) as $vs_val
		) {
			$o_tep->parse( $vs_val );
	...
	public function getElementFilters( $pn_element_id, $ps_prefix_id, array $pa_options = [] ) {
		if ( $t_root
			= \ca_metadata_elements::getInstance( \ca_metadata_elements::getElementHierarchyID( $pn_element_id ) )
		) {
					...
					if ( $va_element['datatype'] == __CA_ATTRIBUTE_VALUE_LIST__ ) {
						if ( $vs_list = \ca_lists::getListAsHTMLFormElement(
							...
							[
								'maxItemCount' => 100,
								'render'       => 'multiple',
								'values'       => caGetOption( $va_element['element_code'], $va_values, null )
							]
						)
						) {
							$va_html[] = "<span class='formLabelPlain'>" . $va_element['display_label']
							             . ':</span><br/>' . $vs_list;
						}
					}
				}
	...
	public function getEventKey( $t_instance ) {
		...
		return md5( $t_instance->tableName() . '/' . $t_instance->getPrimaryKey() . '/'
		            . $t_instance->get( $va_spec['spec'] ) );
	}
	...
	public function getData( $t_instance ) {
		return [
			...
			'row_id'     => $t_instance->getPrimaryKey()
		];
	}
	...
	public function _getSpec( $t_instance ) {
		...
		return [
			...
			'parent_code'  => $vs_parent_code,
			'element_code' => $vs_element_code,
			'spec'         => $vs_get_spec
		];
	}
	 ...
	 * may require notifications. For dates, we take the offset and generate a date range around the current date/time
	 * to search on.
	 *
	 * @param array $pa_trigger_values
	 *
	 * @return array Query parameters for use with BundlableLabelableBaseModelWithAttributes::find() or false if
	 *               trigger is invalid.
	 */
	public function getTriggerQueryCriteria( $pa_trigger_values ) {
		...
		$vs_parent_code
			= \ca_metadata_elements::getElementCodeForId( \ca_metadata_elements::getElementHierarchyID( $vs_element_code ) );
		...
		if ( $vn_offset <= 0 ) {
			...
			$vn_end   = time() + abs( $vn_offset ) + ( 12 * 60
			                                           * 60 );    // end: offset seconds + 12 hours in the future
		} else {
			// Notifications where user wants to know *after* the event happens
			$vn_start = time() - abs( $vn_offset ) - ( 12 * 60
			                                           * 60 );                    // start: offset seconds in the past
			$vn_end   = time();                                        // end:  now
		}
