...
abstract class Base {
	...
	/**
	 * Return query criteria for BundlableLabelableBaseModelWithAttributes::find() to generate a set of records that
	 * may require notifications. This base implementation just returns null.
	 *
	 * @param array $pa_trigger_values
	 *
	 * @return array Query parameters for use with BundlableLabelableBaseModelWithAttributes::find(), false if trigger
	 *               is invalid or null if criteria not support for trigger type.
	 */
	...
	public function getNotificationMessage( &$t_instance ) {
		...
		if ( ! $g_request ) {
			$g_request = new \RequestHTTP( null, [
				'no_headers'   => true,
				'simulateWith' => [ 'REQUEST_METHOD' => 'GET', 'SCRIPT_NAME' => 'index.php' ]
			] );
		}
		...
		if ( ! ( $vs_template = $this->getTriggerValues()['settings']['notificationTemplate'] ) ) {
			...
			return _t(
				...
				caEditorLink( $g_request, $t_instance->get( $t_instance->tableName() . ".preferred_labels" ), '',
					$t_instance->tableName(), $t_instance->getPrimaryKey(), [], [ 'absolute' => true ] )
			);
		} else {
