    ...
    public function getAvailableSettings()
    {
        $va_generic_settings = [
            'notificationTemplate' => array(
                ...
                'validForRootOnly' => 1,
                'description' => _t(
                    'Message for the alert notification sent to users. This is a display template with access to all metadata in the affected record.'
                )
            ),
    ...
    public function getNotificationMessage(&$t_instance)
    {
        ...
        if (!($vs_template = $this->getTriggerValues()['settings']['notificationTemplate'])) {
            ...
            return _t(
                ...
                caEditorLink(
                    $g_request,
                    $t_instance->get($t_instance->tableName() . ".preferred_labels"),
                    '',
