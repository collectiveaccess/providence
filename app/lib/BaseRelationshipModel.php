...
class BaseRelationshipModel extends BundlableLabelableBaseModelWithAttributes implements IRelationshipModel {
	...
	public function insert( $pa_options = null ) {
		if ( $vn_rc = parent::insert( $pa_options ) ) {
			...
			foreach (
				array( $this->getRightTableName() => $t_left, $this->getLeftTableName() => $t_right ) as
				$vs_other_table_name => $t_instance
			) {
				if ( (bool) $t_instance->getProperty( 'SUPPORTS_ACL_INHERITANCE' ) ) {
					if ( is_array( $va_inheritors = $t_instance->getProperty( 'ACL_INHERITANCE_LIST' ) )
					     && in_array( $vs_other_table_name, $va_inheritors )
					) {
						ca_acl::applyACLInheritanceToRelatedRowFromRow( $t_instance,
							( $vs_other_table_name == $this->getLeftTableName() )
								? $this->get( $this->getRightTableFieldName() )
								: $this->get( $this->getLeftTableFieldName() ), $vs_other_table_name,
							( $vs_other_table_name == $this->getLeftTableName() )
								? $this->get( $this->getLeftTableFieldName() )
								: $this->get( $this->getRightTableFieldName() ) );
					}
				}
			}
			...
			foreach (
				...
				if ( (bool) $t_instance->getProperty( 'SUPPORTS_ACL_INHERITANCE' ) ) {
					if ( is_array( $va_inheritors = $t_instance->getProperty( 'ACL_INHERITANCE_LIST' ) )
						...
						ca_acl::applyACLInheritanceToRelatedRowFromRow( $t_instance,
							( $vs_other_table_name == $this->getLeftTableName() ) ? $vn_right_id : $vn_left_id,
							$vs_other_table_name,
							( $vs_other_table_name == $this->getLeftTableName() ) ? $vn_left_id : $vn_right_id,
							array( 'deleteACLOnly' => true ) );
					}
				}
			}
		}
	 ...
	 * (eg. if the table name passed is ca_objects and the relationship is ca_objects_x_entities then the "other" name
	 * is ca_entities)
	 *
	 * @param string $ps_tablename A table name that is part of the relationship the model represents
	 *
	 * @return string The name of the table in the relationship that is not the specified table. If the specified table
	 *                is not part of the relationship null is returned.
	 */
	public function getOtherTableName( $ps_tablename ) {
		if ( ! in_array( $ps_tablename,
			array( $this->RELATIONSHIP_LEFT_TABLENAME, $this->RELATIONSHIP_RIGHT_TABLENAME ) )
		) {
			return null;
		}

		return ( $ps_tablename == $this->RELATIONSHIP_LEFT_TABLENAME ) ? $this->RELATIONSHIP_RIGHT_TABLENAME
			: $this->RELATIONSHIP_LEFT_TABLENAME;
	}
	# ------------------------------------------------------
	...
	/**
	 * Returns table number of the "right" table (by convention the table mentioned second in the relationship table
	 * name)
	...
	/**
	 * Returns name of the "left" table (by convention the table mentioned first in the relationship table name) field
	 * name
	 * (eg. if the table name is ca_objects_x_entities then the "left" name is ca_objects)
	 *
	 * @return string
	...
	/**
	 * Returns name of the "right" table (by convention the table mentioned first in the relationship table name) field
	 * name
	 * (eg. if the table name is ca_objects_x_entities then the "left" name is ca_objects)
	 *
	 * @return string
	...
	public function getRelationshipTypes( $pn_sub_type_left_id = null, $pn_sub_type_right_id = null, $pa_options = null
		...
		if ( $pn_sub_type_left_id ) {
			...
			$va_ancestor_ids = $t_list_item->getHierarchyAncestors( null,
				array( 'idsOnly' => true, 'includeSelf' => true ) );

			$va_params[] = $pn_sub_type_left_id;
			$va_params[] = $va_ancestor_ids;
			$vs_sub_type_left_sql
			             = '(crt.sub_type_left_id IS NULL OR crt.sub_type_left_id = ? OR (crt.include_subtypes_left = 1 AND crt.sub_type_left_id IN (?)))';
		}
		...
		if ( $pn_sub_type_right_id ) {
			...
			$va_params[] = $va_ancestor_ids;
			$vs_sub_type_right_sql
			             = '(crt.sub_type_right_id IS NULL OR crt.sub_type_right_id = ? OR (crt.include_subtypes_right = 1 AND crt.sub_type_right_id IN (?)))';
		}
			...
			if ( sizeof( $pa_options['restrict_to_relationship_types'] ) ) {
				...
				foreach ( $pa_options['restrict_to_relationship_types'] as $vs_type_code ) {
					...
					if ( $t_rel_type->load( $va_criteria ) ) {
						$va_restrict_to_type_list[] = "(crt.hier_left >= " . $t_rel_type->get( 'hier_left' )
						                              . " AND crt.hier_right <= " . $t_rel_type->get( 'hier_right' )
						                              . ")";
					}
				}
		...
		while ( $qr_res->nextRow() ) {
			if ( $pb_ids_only ) {
				...
				$va_types[ $vs_key ][ (int) $qr_res->get( 'type_id' ) ][ (int) $qr_res->get( 'locale_id' ) ]
				        = $qr_res->getRow();
			}
		}
		if ( $pb_ids_only ) {
	 ...
	 */
	public function getRelationshipTypesAsHTMLSelect(
		$ps_orientation, $pn_sub_type_left_id = null, $pn_sub_type_right_id = null, $pa_attributes = null,
		$pa_options = null
	) {
		...
		foreach ( $va_types as $va_type ) {
			...
			$va_options[ str_repeat( "&#160;&#160;&#160;", $vn_l - 1 ) . ( ( $ps_orientation == $vs_left_table_name )
				? $va_type['typename'] : $va_type['typename_reverse'] ) ]
				= $va_type['type_id'];
		}
	...
	public function getRelationshipTypesBySubtype( $ps_orientation, $pn_type_id, $pa_options = null ) {
		...
		$vb_dont_include_subtypes_in_type_restriction = caGetOptions( 'dont_include_subtypes_in_type_restriction',
			$pa_options, false );
		...
		if ( ! $vb_dont_include_subtypes_in_type_restriction ) {
			if ( ! is_array( $va_ancestor_ids = $t_list_item->getHierarchyAncestors( null,
				array( 'idsOnly' => true, 'includeSelf' => true ) ) )
			) {
				$va_ancestor_ids = [];
			}
		...
		$vn_root_id = ( $t_rel_type->load( array( 'parent_id' => null, 'table_num' => $this->tableNum() ) ) )
			? $t_rel_type->getPrimaryKey() : null;
		$va_hier    = [];
		...
		if ( $vs_left_table_name === $vs_right_table_name ) {
			...
			while ( $qr_res->nextRow() ) {
				...
				if ( $pn_type_id ) {
					if ( ! $va_row['sub_type_left_id'] ) {
						// no left restriction
						$vs_subtype_orientation = 'none';
					} elseif ( $va_row['sub_type_left_id']
					           && in_array( $va_row['sub_type_left_id'], $va_left_subtype_candidates )
					) {
						// is left
						$vs_subtype_orientation = "ltor";
						$vs_subtype             = $va_row['sub_type_right_id'];
						$va_right_x             = $va_row['sub_type_right_id'] ? [ $va_row['sub_type_right_id'] ]
							: null;
					} else {
						// is right
					...
					} elseif ( $va_row['sub_type_right_id']
					           && in_array( $va_row['sub_type_right_id'], $va_right_subtype_candidates )
					) {
						// is right
					...
					if ( ( $va_row['sub_type_left_id'] || $va_row['sub_type_right_id'] )
				...
				switch ( $vs_subtype_orientation ) {
					case 'rtol':
						$va_tmp             = $va_row;
						$vs_key             = ( ( strlen( $va_tmp['rank'] ) > 0 ) ? sprintf( "%08d",
								(int) $va_tmp['rank'] ) : "" ) . preg_replace( '![^A-Za-z0-9_]+!', '_',
								$va_tmp['typename_reverse'] );
						...
						if ( ! $va_left_x ) {
							$va_types[ $vn_parent_id ]['NULL'][ $vs_key ][ $va_row['type_id'] ][ $va_row['locale_id'] ]
								= $va_tmp;
						} else {
							foreach ( $va_left_x as $vs_left_subtype ) {
								$va_types[ $vn_parent_id ][ $vs_left_subtype ][ $vs_key ][ $va_row['type_id'] ][ $va_row['locale_id'] ]
									= $va_tmp;
							}
						}
						break;
					case 'ltor':
						...
						$vs_key = ( ( strlen( $va_tmp['rank'] ) > 0 ) ? sprintf( "%08d", (int) $va_tmp['rank'] ) : "" )
						          . preg_replace( '![^A-Za-z0-9_]+!', '_', $va_tmp['typename'] );
						...
						if ( ! $va_right_x ) {
							...
							foreach ( $va_right_x as $vs_right_subtype ) {
								$va_types[ $vn_parent_id ][ $vs_right_subtype ][ $vs_key ][ $va_row['type_id'] ][ $va_row['locale_id'] ]
									= $va_tmp;
							}
						}
						break;
					default:
						...
						if ( trim( $va_tmp['typename'] ) == trim( $va_tmp['typename_reverse'] ) ) {
							...
							if ( ! isset( $va_types[ $vn_parent_id ][ $vs_subtype ][ $vs_key ][ $va_row['type_id'] ][ $va_row['locale_id'] ] ) ) {
								$va_types[ $vn_parent_id ][ $vs_subtype ][ $vs_key ][ $va_row['type_id'] ][ $va_row['locale_id'] ]
									= $va_tmp;
							}
						} else {
							//
							...
							if ( is_array( $va_left_x ) ) {
								foreach ( $va_left_x as $vs_left_subtype ) {
									$va_types[ $vn_parent_id ][ $vs_left_subtype ][ $vs_key ][ 'ltor_'
									                                                           . $va_row['type_id'] ][ $va_row['locale_id'] ]
										= $va_tmp;
								}
							}

							if ( ! isset( $va_types[ $vn_parent_id ][ $vs_subtype ][ $vs_key ][ 'ltor_'
							                                                                    . $va_row['type_id'] ][ $va_row['locale_id'] ] )
							) {
								$va_types[ $vn_parent_id ][ $vs_subtype ][ $vs_key ][ 'ltor_'
								                                                      . $va_row['type_id'] ][ $va_row['locale_id'] ]
									= $va_tmp;
							}
							...
							if ( is_array( $va_right_x ) ) {
								foreach ( $va_right_x as $vs_right_subtype ) {
									$va_types[ $vn_parent_id ][ $vs_right_subtype ][ $vs_key ][ 'rtol_'
									                                                            . $va_row['type_id'] ][ $va_row['locale_id'] ]
										= $va_tmp;
								}
							}

							if ( ! isset( $va_types[ $vn_parent_id ][ $vs_subtype ][ $vs_key ][ 'rtol_'
							                                                                    . $va_row['type_id'] ][ $va_row['locale_id'] ] )
							) {
								$va_types[ $vn_parent_id ][ $vs_subtype ][ $vs_key ][ 'rtol_'
								                                                      . $va_row['type_id'] ][ $va_row['locale_id'] ]
									= $va_tmp;
							}
						}
			...
			foreach ( $va_types as $vs_subtype => $va_types_by_subtype ) {
				...
				if ( ! $vb_dont_include_subtypes_in_type_restriction ) {
					...
					if ( ( $vs_subtype != 'NULL' )
					     && ( ! isset( $va_subtype_lookups[ $vs_subtype ] )
				...
				$va_processed_types[ $vs_subtype ] = caExtractValuesByUserLocale( $va_types_by_locale, null, null,
					array( 'returnList' => true ) );
			}
			...
			while ( $qr_res->nextRow() ) {
				...
				if ( $ps_orientation == $vs_left_table_name ) {
					...
					// expand subtype
					$va_subtypes_to_check = ( $va_row['sub_type_left_id'] > 0 )
						? ( (bool) $va_row['include_subtypes_left'] ? caMakeTypeIDList( $vs_left_table_name,
							[ $va_row['sub_type_left_id'] ] ) : [ $va_row['sub_type_left_id'] ] ) : null;

					// skip type if record type is set, it has a subtype set and it's not in our list
					if ( $pn_type_id
					     && ( $va_subtypes_to_check
					          && ! sizeof( array_intersect( $va_subtypes_to_check, $va_ancestor_ids ) ) )
					) {
						continue;
					}
					$vs_subtype = $va_row['sub_type_right_id'];

					$vs_key = ( ( strlen( $va_row['rank'] ) > 0 ) ? sprintf( "%08d", (int) $va_row['rank'] ) : "" )
					          . preg_replace( '![^A-Za-z0-9_]+!', '_', $va_row['typename'] );

				} else {
					// left-to-right

					// expand subtype
					$va_subtypes_to_check = ( $va_row['sub_type_right_id'] > 0 )
						? ( (bool) $va_row['include_subtypes_right'] ? caMakeTypeIDList( $vs_right_table_name,
							[ $va_row['sub_type_right_id'] ] ) : [ $va_row['sub_type_right_id'] ] ) : null;
					...
					$vs_key = ( ( strlen( $va_row['rank'] ) > 0 ) ? sprintf( "%08d", (int) $va_row['rank'] ) : "" )
					          . preg_replace( '![^A-Za-z0-9_]+!', '_', $va_row['typename_reverse'] );
			...
			foreach ( $va_list as $vs_key => $va_list_by_type_id ) {
				foreach ( $va_list_by_type_id as $vn_type_id => $va_item ) {
					...
					if ( is_array( $pa_hier[ $vn_type_id ] ) ) {
						if ( is_array( $va_tmp = $this->_processRelationshipHierarchy( $vn_type_id, $pa_hier, $pa_types,
							$pn_level + 1 ) )
						) {
								...
								foreach ( $x_va_list as $x_vs_key => $x_va_list_by_type_id ) {
									foreach ( $x_va_list_by_type_id as $x_vn_type_id => $x_va_item ) {
										foreach ( $x_va_item as $vn_i => $x_va_item_entry ) {
											$x_va_item[ $vn_i ]['typename'] = '&#160;&#160;&#160;&#160;'
											                                  . $x_va_item_entry['typename'];
										}
										$va_types_to_return[ $x_vs_sub_types ][ $x_vs_key ][ $x_vn_type_id ]
											= $x_va_item;
									}
								}
							}
	...
	public function getTypeID( $pn_id = null ) {
		if ( $pn_id ) {
			$qr_res = $this->getDb()->query( "SELECT type_id FROM " . $this->tableName() . " WHERE "
			                                 . $this->primaryKey() . " = ?", array( (int) $pn_id ) );
			if ( $qr_res->nextRow() ) {
	...
	/**
	 * Returns relationship type name for the currently loaded row. Directionality of the type name can be controlled
	 * using the $ps_direction parameter.
	 *
	 * @param string $ps_direction Determines the reading direction of the relationship. Possible values are 'ltor'
	 *                             (left-to-right) and 'rtol' (right-to-left). Default value is ltor.
	 *
	...
	public function getRelationshipTypename( $ps_direction = 'ltor', $pn_type_id = null ) {
		if ( ( $vn_type_id = $pn_type_id ) || ( $vn_type_id = $this->getTypeID() ) ) {
			...
			return ( $ps_direction == 'ltor' ) ? $t_rel_type->get( 'ca_relationship_types.preferred_labels.typename' )
				: $t_rel_type->get( 'ca_relationship_types.preferred_labels.typename_reverse' );
		}
	 ...
	 * You can omit specific row_ids from the list of returned ids by including them in the $pa_primary_ids parameters
	 * array. You can obtain linked row_ids for multiple relationships, not necessarily including the currently loaded
	 * relationship, by passing a list of relation_ids in the optional $pa_relationship_ids parameter.
	 *
	 * This method only returns values for self-relationships (Ex. ca_objects_x_objects, ca_entities_x_entities). It
	 * will return null when called for non-self-relationships. The returned values are primary keys (row_ids) for the
	 * related record - Eg. ca_objects object_id's for ca_objects_x_objects, ca_entities entity_id's for
	 * ca_entities_x_entities.
	 *
	 * @param array $pa_primary_ids      List of row_ids to not include in the returned list. [Default is to include
	 *                                   all row_ids]
	 * @param array $pa_relationship_ids List of relationship relation_ids to return related row_ids for. [Default is
	 *                                   to return row_ids for the currently loaded relationship only.]
	 *
	 * @return array An array of row_ids. If called on a non-self-relationship or with no loaded relationship and and
	 *               empty $pa_relationship_ids parameter, will return null.
	 */
	...
	public function updateRanksForList( $pa_ids ) {
		...
		foreach ( $pa_ids as $i => $vn_id ) {
			$this->getDb()->query( "UPDATE " . $this->tableName() . " SET `rank` = ? WHERE relation_id = ?", $i,
				$vn_id );
		}
	}
