...
class BaseRelationshipModel extends BundlableLabelableBaseModelWithAttributes implements IRelationshipModel
{
    ...
    public function insert($pa_options = null)
    {
        if ($vn_rc = parent::insert($pa_options)) {
            ...
            foreach (
                array(
                    $this->getRightTableName() => $t_left,
                    $this->getLeftTableName() => $t_right
                ) as $vs_other_table_name => $t_instance
            ) {
                if ((bool)$t_instance->getProperty('SUPPORTS_ACL_INHERITANCE')) {
                    if (is_array($va_inheritors = $t_instance->getProperty('ACL_INHERITANCE_LIST')) && in_array(
                            $vs_other_table_name,
                            $va_inheritors
                        )) {
                        ca_acl::applyACLInheritanceToRelatedRowFromRow(
                            $t_instance,
                            ($vs_other_table_name == $this->getLeftTableName()) ? $this->get(
                                $this->getRightTableFieldName()
                            ) : $this->get($this->getLeftTableFieldName()),
                            $vs_other_table_name,
                            ($vs_other_table_name == $this->getLeftTableName()) ? $this->get(
                                $this->getLeftTableFieldName()
    ...
    public function getRelationshipTypes($pn_sub_type_left_id = null, $pn_sub_type_right_id = null, $pa_options = null)
    {
        ...
        if ($pn_sub_type_left_id) {
            ...
            $va_ancestor_ids = $t_list_item->getHierarchyAncestors(
                null,
            ...
            if (sizeof($pa_options['restrict_to_relationship_types'])) {
                ...
                foreach ($pa_options['restrict_to_relationship_types'] as $vs_type_code) {
                    ...
                    if ($t_rel_type->load($va_criteria)) {
                        $va_restrict_to_type_list[] = "(crt.hier_left >= " . $t_rel_type->get(
                                'hier_left'
    ...
    public function getRelationshipTypesAsHTMLSelect(
        $ps_orientation,
        $pn_sub_type_left_id = null,
        $pn_sub_type_right_id = null,
        $pa_attributes = null,
        $pa_options = null
    ) {
        ...
        foreach ($va_types as $va_type) {
            ...
            $va_options[str_repeat(
                "&#160;&#160;&#160;",
                $vn_l - 1
            ) . (($ps_orientation == $vs_left_table_name) ? $va_type['typename'] : $va_type['typename_reverse'])] = $va_type['type_id'];
        }
    ...
    public function getRelationshipTypesBySubtype($ps_orientation, $pn_type_id, $pa_options = null)
    {
        ...
        $vb_dont_include_subtypes_in_type_restriction = caGetOptions(
        ...
        $vn_root_id = ($t_rel_type->load(
            array('parent_id' => null, 'table_num' => $this->tableNum())
        ...
        if ($vs_left_table_name === $vs_right_table_name) {
            ...
            while ($qr_res->nextRow()) {
                ...
                if ($pn_type_id) {
                    if (!$va_row['sub_type_left_id']) {
                        // no left restriction
                        $vs_subtype_orientation = 'none';
                    } elseif ($va_row['sub_type_left_id'] && in_array(
                            $va_row['sub_type_left_id'],
                            $va_left_subtype_candidates
                        )) {
                        // is left
                    ...
                    } elseif ($va_row['sub_type_right_id'] && in_array(
                            $va_row['sub_type_right_id'],
                            $va_right_subtype_candidates
                        )) {
                        // is right
                ...
                switch ($vs_subtype_orientation) {
                    case 'rtol':
                        $va_tmp = $va_row;
                        $vs_key = ((strlen($va_tmp['rank']) > 0) ? sprintf(
                                "%08d",
                                (int)$va_tmp['rank']
                            ) : "") . preg_replace('![^A-Za-z0-9_]+!', '_', $va_tmp['typename_reverse']);
            ...
            foreach ($va_types as $vs_subtype => $va_types_by_subtype) {
                ...
                $va_processed_types[$vs_subtype] = caExtractValuesByUserLocale(
                    $va_types_by_locale,
                    null,
                    null,
            ...
            while ($qr_res->nextRow()) {
                ...
                if ($ps_orientation == $vs_left_table_name) {
                    ...
                    // expand subtype
                    $va_subtypes_to_check = ($va_row['sub_type_left_id'] > 0) ? ((bool)$va_row['include_subtypes_left'] ? caMakeTypeIDList(
                        $vs_left_table_name,
                        [$va_row['sub_type_left_id']]
                    ) : [$va_row['sub_type_left_id']]) : null;

                    // skip type if record type is set, it has a subtype set and it's not in our list
                    if ($pn_type_id && ($va_subtypes_to_check && !sizeof(
                                array_intersect($va_subtypes_to_check, $va_ancestor_ids)
                            ))) {
                        continue;
                    }
                    $vs_subtype = $va_row['sub_type_right_id'];

                    $vs_key = ((strlen($va_row['rank']) > 0) ? sprintf(
                            "%08d",
                            (int)$va_row['rank']
                    ...
                    // expand subtype
                    $va_subtypes_to_check = ($va_row['sub_type_right_id'] > 0) ? ((bool)$va_row['include_subtypes_right'] ? caMakeTypeIDList(
                        $vs_right_table_name,
                        [$va_row['sub_type_right_id']]
                    ) : [$va_row['sub_type_right_id']]) : null;
            ...
            foreach ($va_list as $vs_key => $va_list_by_type_id) {
                foreach ($va_list_by_type_id as $vn_type_id => $va_item) {
                    ...
                    if (is_array($pa_hier[$vn_type_id])) {
                        if (is_array(
                            $va_tmp = $this->_processRelationshipHierarchy(
                                $vn_type_id,
                                $pa_hier,
                                $pa_types,
                                $pn_level + 1
    ...
    public function getRelationshipTypename($ps_direction = 'ltor', $pn_type_id = null)
    {
        if (($vn_type_id = $pn_type_id) || ($vn_type_id = $this->getTypeID())) {
            ...
            return ($ps_direction == 'ltor') ? $t_rel_type->get(
