...
class ApplicationChangeLog
{
    ...
    public function getRecentChangesForDisplay(
        $pn_table_num,
        $pn_num_seconds = 604800,
        $pn_limit = 0,
        $po_request = null,
        $ps_css_id = null
    ) {    // 604800 = number of seconds in one week
        return $this->_getLogDisplayOutput(
            $this->_getChangeLogFromRawData(
                $this->getRecentChanges($pn_table_num, $pn_num_seconds, $pn_limit),
                $pn_table_num,
                array('return_item_names' => true)
            ),
    ...
    {
        return $this->_getLogDisplayOutputForRow(
            $this->getChangeLogForRow($t_item, array('user_id' => $pn_user_id)),
            array('id' => $ps_css_id)
        );
    }
    # ----------------------------------------
    ...
    {
        return $this->_getChangeLogFromRawData(
            $t_item->getChangeLog($t_item->getPrimaryKey(), $options),
            $t_item->tableNum(),
            $options
        );
    }
    ...
    {
        return $this->_getChangeLogFromRawData(
            $this->getRecentChangesAsRawData($pn_table_num, $pn_num_seconds, $pn_limit, true),
            $pn_table_num,
            array('return_item_names' => true)
        );
    }
...
/* ]]> */
        ...
        if (!sizeof($pa_log)) {
            $vs_output .= "<tr><td colspan='3'><div class='contentError' align='center'>" . _t(
...
/* ]]> */
        ...
        if (!sizeof($pa_log)) {
            $vs_output .= "<tr><td colspan='4'><div class='contentError' align='center'>" . _t(
                    'No change log available'
                ) . "</div></td></tr>\n";
        } else {
            foreach (array_reverse($pa_log) as $vn_unit_id => $va_log_entries) {
                if (is_array($va_log_entries) && sizeof($va_log_entries)) {
                    ...
                    if (isset($options['request']) && $options['request']) {
                        $vs_output .= "<td><a href='" . caEditorUrl(
                                $options['request'],
                                $va_log_entries[0]['subject_table_num'],
                                $va_log_entries[0]['subject_id']
            ...
            foreach ($va_grouped_data as $vn_unit_id => $va_log_entries_by_table) {
                foreach ($va_log_entries_by_table as $vs_table_key => $va_log_entries) {
                    foreach ($va_log_entries as $va_log_entry) {
                        ...
                        //
                        $t_obj = Datamodel::getInstanceByTableNum(
                            $va_log_entry['logged_table_num'],
                            true
                                ...
                                if ($t_subject->load($vn_subject_row_id)) {
                                    if (method_exists($t_subject, 'getLabelForDisplay')) {
                                        ...
                                        if ($vs_idno_field = $t_subject->getProperty('ID_NUMBERING_ID_FIELD')) {
                                            $vs_subject_display_name = $t_subject->getProperty(
                                                    'NAME_SINGULAR'
                        ...
                        if (($pn_table_num == $va_log_entry['logged_table_num'])) {
                            foreach ($va_log_entry['snapshot'] as $vs_field => $vs_value) {
                                ...
                                if ((isset($va_field_info['DISPLAY_FIELD'])) && (is_array(
                                        $va_field_info['DISPLAY_FIELD']
                                    ...
                                    if (sizeof($va_keys)) {
                                        ...
                                        if ($t_user && !$t_user->getBundleAccessLevel(
                                                $t_item->tableName(),
                                                $va_keys['one_table']
                                            )) {
                                            continue;
                                        ...
                                        if ($va_field_info['LIST']) {
                                            ...
                                            if ($t_list->load(array('list_code' => $va_field_info['LIST']))) {
                                                ...
                                                if ($t_list_item->load(
                                                    array('list_id' => $vn_list_id, 'item_value' => $vs_value)
                                                )) {
                                ...
                                $va_changes[] = array(
                                    ...
                                    'description' => (strlen(
                                        (string)$vs_proc_val
                                    ) ? $vs_proc_val : $vs_blank_placeholder),
                                    'value' => $vs_value
                                );
                            }
                        }
                        ...
                        if ($va_log_entry['logged_table_num'] == 3) {    // attribute_values
                            if ($t_element = ca_attributes::getElementInstance(
                                $va_log_entry['snapshot']['element_id']
                            )) {
                                if ($t_element->get('parent_id') && ($t_container = ca_attributes::getElementInstance(
                                        $t_element->get('hier_element_id')
                                    ))) {
                                ...
                                if ($vn_list_id = $t_element->get('list_id')) {
                                    $t_list = new ca_lists();
                                    $vs_attr_val = $t_list->getItemFromListForDisplayByItemID(
                                        $vn_list_id,
                                        $vs_attr_val,
                                        true
                                    );
                        ...
                        if (sizeof($va_keys) > 0) {
                            if (method_exists($t_obj, 'getLeftTableNum')) {
                                if ($t_obj->getLeftTableNum() == $t_item->tableNum()) {
                                    ...
                                    $t_related_table = Datamodel::getInstanceByTableNum(
                                        $t_obj->getRightTableNum(),
                                        true
                                    );
                                ...
                                $va_changes[] = array(
                                    ...
                                    'idno' => ($vs_idno_field = $t_related_table->getProperty(
                                        'ID_NUMBERING_ID_FIELD'
                                    )) ? $t_related_table->get($vs_idno_field) : null,
                                    'description' => method_exists(
                                        $t_related_table,
                                        'getLabelForDisplay'
                                    ...
                                    'rel_typename' => $t_rel->getRelationshipTypename(
                                        'ltor',
    ...
    public static function getChangeLogUsers($options = null)
    {
        ...
        $params = [];
        $daterange_values = ($daterange = caGetOption('daterange', $options, null)) ? caDateToUnixTimestamps(
            $daterange
        ) : null;
    ...
    public static function getChangeData($options = null)
    {
        ...
        $sql_daterange = null;
        if (($daterange = caGetOption('daterange', $options, null)) && is_array(
                $d = caDateToUnixTimestamps($daterange)
            )) {
            ...
            foreach ($va_grouped_data as $vn_unit_id => $va_log_entries_by_table) {
                foreach ($va_log_entries_by_table as $vs_table_key => $va_log_entries) {
                    foreach ($va_log_entries as $va_log_entry) {
                        ...
                        // object (and probably the ca_entities row as well) that you can about the change.
                        //
                        $t_subject = $va_log_entry['subject_table_num'] ? Datamodel::getInstanceByTableNum(
                            $va_log_entry['subject_table_num'],
                            true
                        ) : null;
                        ...
                        if ($return_item_names) {
                            ...
                            if (isset($label_cache[$vn_subject_table_num][$vn_subject_row_id])) {
                                ...
                                $vs_subject_display_name = Datamodel::getTableProperty(
                            ...
                            foreach ($va_log_entry['snapshot'] as $vs_field => $vs_value) {
                                ...
                                if ((isset($va_field_info['DISPLAY_FIELD'])) && (is_array(
                                    ...
                                    if (sizeof($va_keys)) {
                                        ...
                                        // Adjust display of value for lists
                                        if ($va_field_info['LIST']) {
                                            $vs_proc_val = caGetListItemByIDForDisplay(
                                                caGetListItemIDForValue($va_field_info['LIST'], $vs_value)
                                            );
                                        } else {
                                            if ($va_field_info['BOUNDS_CHOICE_LIST']) {
                                                // TODO
                        ...
                        if ($t_subject) {
                            if ($t_logged->isRelationship()) {
                                if (method_exists($t_logged, 'getLeftTableNum')) {
                                    ...
                                    $ltor_rel_type = $t_logged->getRelationshipTypename(
                                        'ltor',
                                        $va_log_entry['snapshot']['type_id']
                                    );
                                    $rtol_rel_type = $t_logged->getRelationshipTypename(
                                        'ltor',
                                        $va_log_entry['snapshot']['type_id']
                                    );
                                    ...
                                    $va_changes[] = array(
                                        ...
                                        'idno' => ($vs_idno_field = $t_left->getProperty(
                                            'ID_NUMBERING_ID_FIELD'
                                        ...
                                        'rel_type_id' => $va_log_entry['snapshot']['type_id'],
                                        'rel_typename' => $t_logged->getRelationshipTypename(
                                            'ltor',
                                            $va_log_entry['snapshot']['type_id']
                                        ),
                                        'message' => _t(
