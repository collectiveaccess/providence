...
class ApplicationChangeLog {
	 ...
	 *
	 */
	public function getRecentChangesForDisplay(
		$pn_table_num, $pn_num_seconds = 604800, $pn_limit = 0, $po_request = null, $ps_css_id = null
	) {    // 604800 = number of seconds in one week
		return $this->_getLogDisplayOutput( $this->_getChangeLogFromRawData( $this->getRecentChanges( $pn_table_num,
			$pn_num_seconds, $pn_limit ), $pn_table_num, array( 'return_item_names' => true ) ),
			array( 'id' => $ps_css_id, 'request' => $po_request ) );
	}
	# ----------------------------------------
	...
	public function getChangeLogForRowForDisplay( $t_item, $ps_css_id = null, $pn_user_id = null ) {
		return $this->_getLogDisplayOutputForRow( $this->getChangeLogForRow( $t_item,
			array( 'user_id' => $pn_user_id ) ), array( 'id' => $ps_css_id ) );
	}
	# ----------------------------------------
	...
	public function getChangeLogForRow( $t_item, $options = null ) {
		return $this->_getChangeLogFromRawData( $t_item->getChangeLog( $t_item->getPrimaryKey(), $options ),
			$t_item->tableNum(), $options );
	}
	# ----------------------------------------------------------------------
	...
	public function getRecentChanges( $pn_table_num, $pn_num_seconds = 604800, $pn_limit = 0 ) {
		return $this->_getChangeLogFromRawData( $this->getRecentChangesAsRawData( $pn_table_num, $pn_num_seconds,
			$pn_limit, true ), $pn_table_num, array( 'return_item_names' => true ) );
	}
	# ----------------------------------------------------------------------
	 ...
	 *
	 */
	public function getRecentChangesAsRawData(
		$pn_table_num, $pn_num_seconds = 604800, $pn_limit = 0, $return_snapshot = false
	) {    // 604800 = number of seconds in one week
		$o_db   = new Db();
		$qs_log = $o_db->prepare( "
			SELECT DISTINCT
				wcl.log_id, wcl.log_datetime log_datetime, wcl.user_id, wcl.changetype, wcl.logged_table_num, wcl.logged_row_id,
				 wcl.unit_id, wu.email, wu.fname, wu.lname, wcls.subject_table_num, wcls.subject_row_id "
		                          . ( $return_snapshot ? " , wclsnap.snapshot" : "" ) . "
...
/* ]]> */
		...
		if ( ! sizeof( $pa_log ) ) {
			$vs_output .= "<tr><td colspan='3'><div class='contentError' align='center'>"
			              . _t( 'No change log available' ) . "</div></td></tr>\n";
		} else {
				...
				if ( is_array( $va_log_entries ) && sizeof( $va_log_entries ) ) {
					...
					if ( trim( $va_log_entries[0]['user_fullname'] ) ) {
						...
						if ( trim( $va_log_entries[0]['user_email'] ) ) {
							$vs_output .= " <a href='mailto:" . $va_log_entries[0]['user_email'] . "'>"
							              . $va_log_entries[0]['user_fullname'] . "</a>";
						} else {
					...
					foreach ( $va_log_entries as $va_log_entry ) {
						foreach ( $va_log_entry['changes'] as $va_change ) {
							$vs_output .= '<span class="logChangeLabel">' . $va_log_entry['changetype_display'] . ' '
							              . $va_change['label'] . '</span>: ' . $va_change['description']
							              . ( isset( $va_change['rel_typename'] ) ? ' (' . $va_change['rel_typename']
							                                                        . ')' : '' ) . "<br/>\n";
						}
					}
					$vs_output .= "</div></td>";
...
/* ]]> */
		...
		if ( ! sizeof( $pa_log ) ) {
			$vs_output .= "<tr><td colspan='4'><div class='contentError' align='center'>"
			              . _t( 'No change log available' ) . "</div></td></tr>\n";
		} else {
			foreach ( array_reverse( $pa_log ) as $vn_unit_id => $va_log_entries ) {
				if ( is_array( $va_log_entries ) && sizeof( $va_log_entries ) ) {
					...
					if ( isset( $options['request'] ) && $options['request'] ) {
						$vs_output .= "<td><a href='" . caEditorUrl( $options['request'],
								$va_log_entries[0]['subject_table_num'], $va_log_entries[0]['subject_id'] ) . "'>"
						              . $va_log_entries[0]['subject'] . "</a></td>";
					} else {
					...
					foreach ( $va_log_entries as $va_log_entry ) {
						foreach ( $va_log_entry['changes'] as $va_change ) {
							$vs_output .= '<span class="logChangeLabel">' . $va_log_entry['changetype_display'] . ' '
							              . $va_change['label'] . '</span>: ' . $va_change['description'] . "<br/>\n";
						}
					}
					$vs_output .= "</div></td>";
					$vs_output .= "</tr>\n";
	...
	private function _getChangeLogFromRawData( $pa_data, $pn_table_num, $options = null ) {
		...
		if ( sizeof( $pa_data ) ) {
			...
			foreach ( $pa_data as $va_log_entry ) {
				$va_grouped_data[ $va_log_entry['unit_id'] ][ 'ca_table_num_' . $va_log_entry['logged_table_num'] ][]
					= $va_log_entry;
			}
			...
			foreach ( $va_grouped_data as $vn_unit_id => $va_log_entries_by_table ) {
				foreach ( $va_log_entries_by_table as $vs_table_key => $va_log_entries ) {
					foreach ( $va_log_entries as $va_log_entry ) {
						...
						//		
						$t_obj = Datamodel::getInstanceByTableNum( $va_log_entry['logged_table_num'],
							true );    // get instance for logged table
						if ( ! $t_obj ) {
							continue;
						}
								...
								if ( $t_subject->load( $vn_subject_row_id ) ) {
									if ( method_exists( $t_subject, 'getLabelForDisplay' ) ) {
										...
										if ( $vs_idno_field = $t_subject->getProperty( 'ID_NUMBERING_ID_FIELD' ) ) {
											$vs_subject_display_name = $t_subject->getProperty( 'NAME_SINGULAR' ) . ' ['
											                           . $t_subject->get( $vs_idno_field ) . ']';
										} else {
											$vs_subject_display_name = $t_subject->getProperty( 'NAME_SINGULAR' ) . ' ['
											                           . $vn_subject_row_id . ']';
										}
						...
						if ( ( $pn_table_num == $va_log_entry['logged_table_num'] ) ) {
							foreach ( $va_log_entry['snapshot'] as $vs_field => $vs_value ) {
								...
								if ( isset( $va_field_info['DISPLAY_TYPE'] )
								     && $va_field_info['DISPLAY_TYPE'] == DT_OMIT
								) {
									continue;
								}
								...
								if ( ( isset( $va_field_info['DISPLAY_FIELD'] ) )
								     && ( is_array( $va_field_info['DISPLAY_FIELD'] ) )
								     && ( $va_disp_fields = $va_field_info['DISPLAY_FIELD'] )
								) {
									//
									...
									if ( sizeof( $va_disp_fields ) ) {
										$va_rel        = Datamodel::getManyToOneRelations( $t_obj->tableName(),
											$vs_field );
										$va_rel_values = [];
									...
									if ( sizeof( $va_keys ) ) {
										...
										if ( $t_user
										     && ! $t_user->getBundleAccessLevel( $t_item->tableName(),
												$va_keys['one_table'] )
										) {
											continue;
										}    // does user have access to this bundle?
										...
										if ( $t_user
										     && ! $t_user->getBundleAccessLevel( $t_item->tableName(), $vs_field )
										) {
											continue;
										}    // does user have access to this bundle?
								...
								$va_changes[] = array(
									...
									'description' => ( strlen( (string) $vs_proc_val ) ? $vs_proc_val
										: $vs_blank_placeholder ),
									'value'       => $vs_value
								);
							}
						}
						...
						if ( $va_log_entry['logged_table_num'] == 3 ) {    // attribute_values
							if ( $t_element
								= ca_attributes::getElementInstance( $va_log_entry['snapshot']['element_id'] )
							) {

								if ( $t_element->get( 'parent_id' )
								     && ( $t_container
										= ca_attributes::getElementInstance( $t_element->get( 'hier_element_id' ) ) )
								) {
								...
								if ( $t_user
								     && ! $t_user->getBundleAccessLevel( $t_item->tableName(), $vs_element_code )
								) {
									continue;
								}    // does user have access to this bundle?
								...
								if ( $vn_list_id = $t_element->get( 'list_id' ) ) {
									$t_list      = new ca_lists();
									$vs_attr_val = $t_list->getItemFromListForDisplayByItemID( $vn_list_id,
										$vs_attr_val, true );
								}
						...
						if ( sizeof( $va_keys ) > 0 ) {
							if ( method_exists( $t_obj, 'getLeftTableNum' ) ) {
								if ( $t_obj->getLeftTableNum() == $t_item->tableNum() ) {
									...
									$t_related_table = Datamodel::getInstanceByTableNum( $t_obj->getRightTableNum(),
										true );
									$t_related_table->load( $va_log_entry['snapshot'][ $t_obj->getRightTableFieldName() ] );
								} else {
									// other side of rel is on left
									$t_related_table = Datamodel::getInstanceByTableNum( $t_obj->getLeftTableNum(),
										true );
								...
								$va_changes[] = array(
									...
									'idno'         => ( $vs_idno_field
										= $t_related_table->getProperty( 'ID_NUMBERING_ID_FIELD' ) )
										? $t_related_table->get( $vs_idno_field ) : null,
									'description'  => method_exists( $t_related_table, 'getLabelForDisplay' )
										? $t_related_table->getLabelForDisplay() : '',
									...
									'rel_typename' => $t_rel->getRelationshipTypename( 'ltor',
										$va_log_entry['snapshot']['type_id'] )
								);
							}
						}
						// ---------------------------------------------------------------	
	 ...
	 *                       range = optional range to restrict returned entries to. Should be array with 0th key set
	 *                       to start and 1st key set to end of range. Both values should be Unix timestamps. You can
	 *                       also use 'start' and 'end' as keys if desired. limit = maximum number of entries returned.
	 *                       Omit or set to zero for no limit. [default=all]
	 *
	...
	public function getDeletions( $pm_table_name_or_num, $options = null ) {
		...
		$pa_datetime_range           = ( isset( $options['range'] ) && is_array( $options['range'] ) )
			? $options['range'] : null;
		$pn_max_num_entries_returned = ( isset( $options['limit'] ) && (int) $options['limit'] )
			? (int) $options['limit'] : 0;
	 ...
	 *                       limit = The maximum number of entries returned. Omit or set to zero for no limit. [Default
	 *                       is null – no limit] transaction = A database transaction to perform log queries within.
	 *                       [Default is null]
	 *
	 * @return array An array of users. Each user is represented by an array with all ca_user record keys plus an
	 *               additional "user" key with the user's full name and email address.
	 */
	static public function getChangeLogUsers( $options = null ) {
		$o_db = ( $trans = caGetOption( 'transaction', $options, null ) ) ? $trans->getDb() : new Db();

		$params           = [];
		$daterange_values = ( $daterange = caGetOption( 'daterange', $options, null ) )
			? caDateToUnixTimestamps( $daterange ) : null;
	 ...
	 * @param array $options An array of options:
	 *                       daterange = A valid date/time expression. [Default is null]
	 *                       format = A display template with which to format user names. Tags are ca_users field names
	 *                       and the additional "user" value set by ApplicationChangeLog::getChangeLogUsers, upon which
	 *                       this method relies. If omitted the "user" value is returned. [Default is "^user"] limit =
	 *                       The maximum number of entries returned. Omit or set to zero for no limit. [Default is null
	 *                       – no limit] transaction = A database transaction to perform log queries within. [Default
	 *                       is null]
	 *
	 ...
	 *                       table = Limit returned data to that related to a specific table. Value may be a table name
	 *                       or number. [Default is null] tables = Limit returned data to that related to a list of
	 *                       tables. Values may be table names or numbers. [Default is null] daterange = A valid
	 *                       date/time expression to limit returned log entries to. [Default is null] user_id = ID of
	 *                       user to perform access control checks as. If omitted no access control checks are
	 *                       performed. [Default is null] start = Index of log entry (or unit if "limitByUnit" option
	 *                       is set) to start result set with. [Default is 0] limit = Maximum number of entries
	 *                       returned. Omit or set to zero for no limit. [Default is null – no limit] transaction = A
	 *                       database transaction to perform log queries within. [Default is null] noSnapshot = Don't
	 *                       return full record snapshot in log data result set. [Default is false] groupBySubject =
	 *                       Group returned results by subject table number and row_id for each returned unit. [Default is false] limitByUnit = Apply start and limit values to units, not log entries. [Default is false]
	 *
	 * @return array An array of change log data key'ed on unit_id. Each unit may be optionally key'ed on subject table
	 *               number and row_id
	 */
	 ...
	 *                       is set) to start result set with. [Default is 0] limit = Maximum number of entries
	 *                       returned. Omit or set to zero for no limit. [Default is null – no limit] transaction = A
	 *                       database transaction to perform log queries within. [Default is null] groupBySubject =
	 *                       Group returned results by subject table number and row_id for each returned unit. [Default
	 *                       is false] limitByUnit = Apply start and limit values to units, not log entries. [Default is false]
	 *
	...
	static public function getChangeData( $options = null ) {
		...
		$sql_daterange = null;
		if ( ( $daterange = caGetOption( 'daterange', $options, null ) )
		     && is_array( $d = caDateToUnixTimestamps( $daterange ) )
		) {
	 ...
	 *                       groupBySubject = Group log entries within a logging unit by subject table number and
	 *                       row_id, rather than returning all log entries in a unit a single list. [Default is false]
	 *                       user_id = ID of user to perform access control checks as. If omitted no access control
	 *                       checks are performed. [Default is null]
	 *
	 * @return array An array of change log entries with, key'ed on unit_id. If the groupBySubject option is set, each
	 *               unit array is key'ed on subject table number and row_id in the form <table_num>:<row_id>.
	 */
		...
		if ( sizeof( $data ) ) {
			...
			foreach ( $va_grouped_data as $vn_unit_id => $va_log_entries_by_table ) {
				foreach ( $va_log_entries_by_table as $vs_table_key => $va_log_entries ) {
					foreach ( $va_log_entries as $va_log_entry ) {
						$vs_label_table_name = $vs_label_display_name = '';
						$t_logged            = Datamodel::getInstanceByTableNum( $va_log_entry['logged_table_num'],
							true );
						if ( ! $t_logged ) {
							continue;
						}
						...
						// object (and probably the ca_entities row as well) that you can about the change.
						//		
						$t_subject = $va_log_entry['subject_table_num']
							? Datamodel::getInstanceByTableNum( $va_log_entry['subject_table_num'], true ) : null;
						...
						if ( $return_item_names ) {
							...
							if ( isset( $label_cache[ $vn_subject_table_num ][ $vn_subject_row_id ] ) ) {
								...
								$vs_subject_display_name = Datamodel::getTableProperty( $vn_subject_table_num,
										'NAME_SINGULAR' ) . " [{$vn_subject_row_id}]";
							}
						}
						...
						if ( ( $vn_subject_table_num == $va_log_entry['logged_table_num'] ) ) {
							foreach ( $va_log_entry['snapshot'] as $vs_field => $vs_value ) {
								...
								if ( ( isset( $va_field_info['DISPLAY_FIELD'] ) )
									...
									if ( sizeof( $va_disp_fields ) ) {
										$va_rel        = Datamodel::getManyToOneRelations( $t_logged->tableName(),
											$vs_field );
										$va_rel_values = [];
									...
									if ( sizeof( $va_keys ) ) {
										...
										if ( $t_user
										     && ! $t_user->getBundleAccessLevel( $t_logged->tableName(),
												$va_keys['one_table'] )
										) {
											continue;
										}    // does user have access to this bundle?
										...
										if ( $t_user
										     && ! $t_user->getBundleAccessLevel( $t_logged->tableName(), $vs_field )
										) {
											continue;
										}    // does user have access to this bundle?
										...
										if ( $va_field_info['LIST'] ) {
											$vs_proc_val
												= caGetListItemByIDForDisplay( caGetListItemIDForValue( $va_field_info['LIST'],
												$vs_value ) );
										} else {
											if ( $va_field_info['BOUNDS_CHOICE_LIST'] ) {
												// TODO
								...
								$va_changes[] = array(
									...
									'description' => $desc = ( strlen( (string) $vs_proc_val ) ? $vs_proc_val
										: $vs_blank_placeholder ),
									'value'       => $vs_value,
						...
						if ( $va_log_entry['logged_table_num'] == 3 ) {    // attribute_values
							if ( $t_element
								...
								if ( $t_user
								     && ! $t_user->getBundleAccessLevel( $t_logged->tableName(), $vs_element_code )
								) {
									continue;
								}    // does user have access to this bundle?
						...
						if ( $t_subject ) {
							if ( $t_logged->isRelationship() ) {
								if ( method_exists( $t_logged, 'getLeftTableNum' ) ) {
									...
									$t_related_table->load(
										$id = $va_log_entry['snapshot'][ $t_related_table->primaryKey() ] );

									$ltor_rel_type = $t_logged->getRelationshipTypename( 'ltor',
										$va_log_entry['snapshot']['type_id'] );
									$rtol_rel_type = $t_logged->getRelationshipTypename( 'ltor',
										$va_log_entry['snapshot']['type_id'] );
									...
									$va_changes[] = array(
										...
										'idno'             => ( $vs_idno_field
											= $t_left->getProperty( 'ID_NUMBERING_ID_FIELD' ) )
											? $t_left->get( $vs_idno_field ) : null,
										'description'      => method_exists( $t_left, 'getLabelForDisplay' )
											? $t_left->getLabelForDisplay() : '',
										...
										'rel_type_id'      => $va_log_entry['snapshot']['type_id'],
										'rel_typename'     => $t_logged->getRelationshipTypename( 'ltor',
											$va_log_entry['snapshot']['type_id'] ),
										'message'          => _t( 'Related to %1 (%2)',
											$t_related_table->getLabelForDisplay(), $ltor_rel_type )
									);
								}
							}
						}
						...
						if ( sizeof( $va_changes ) ) {
							...
							if ( $group_by_subject ) {
								$va_log_output[ $vs_unit_identifier ]["{$vn_subject_table_num}:{$vn_subject_row_id}"][]
									= $entry;
							} else {
