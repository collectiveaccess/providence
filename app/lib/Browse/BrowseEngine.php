...
class BrowseEngine extends BaseFindEngine
{
     ...
     * @var Type_id cache
     */
    static public $s_type_id_cache = array();
    static public $s_source_id_cache = array();
    # ------------------------------------------------------
    ...
    private function _processBrowseSettings()
    {
        ...
        foreach ($this->opa_browse_settings['facets'] as $vs_facet_name => $va_facet_info) {
            ...
            if ((in_array($va_facet_info['type'], ['location', 'current_value'])) && (caGetOption(
                        'group_mode',
                        $va_facet_info,
                        null
                    ) == 'hierarchical')) {
                ...
                $policy = caGetOption(
                    'policy',
                    $va_facet_info,
                    $subject_table::getDefaultHistoryTrackingCurrentValuePolicyForTable(
                        $subject_table
                    )
                );
                ...
                if (is_array($policy_info['elements']) && (sizeof(
                            $policy_info['elements']
                        ) == 1) && isset($policy_info['elements']['ca_storage_locations'])) {
            ...
            if (($va_facet_info['type'] === 'authority') && isset($va_facet_info['generate_facets_for_types']) && $va_facet_info['generate_facets_for_types']) {
                ...
                foreach ($va_type_list as $vn_type_id => $va_type_info) {
                    if ($va_type_info['is_enabled']) {
                        $va_facet_info = array_merge(
                            $va_facet_info,
                            array(
                                'label_singular' => $va_type_info['name_singular'],
                                'label_singular_with_indefinite_article' => _t(
                                        'a'
        ...
        foreach ($va_revised_facets as $vs_facet => $va_facet_info) {
            ...
            switch ($va_facet_info['type']) {
                case 'attribute':
                    ...
                    if ($t_element->load(
                        array('element_code' => array_pop(explode(".", $va_facet_info['element_code'])))
                    )) {
                        if (($t_element->get(
                                    'datatype'
                                ) == __CA_ATTRIBUTE_VALUE_LIST__) && ($vn_list_id = $t_element->get('list_id'))) {
                ...
                case 'fieldList':
                    ...
                    if ($vn_item_id = caGetListItemID(
                        $t_instance->getFieldInfo($va_facet_info['field'], 'LIST_CODE'),
                        $va_facet_info['single_value']
                    )) {
                        $va_revised_facets[$vs_facet]['single_value'] = $vn_item_id;
                    }
                    break;
    ...
    public function addCriteria($ps_facet_name, $pa_row_ids, $pa_display_strings = null)
    {
        ...
        foreach ($pa_row_ids as $vn_i => $vn_row_id) {
            $vn_row_id = str_replace(
                "&amp;",
                "&",
                $purifier->purify(rawurldecode($vn_row_id))
            ); // sanitize facet values
        ...
        if ($ps_facet_name) {
            if (is_array($va_criteria_display_strings[$ps_facet_name])) {
                foreach ($va_criteria_display_strings[$ps_facet_name] as $vm_criterion => $vs_display_criterion) {
                    $va_criteria_with_labels[$vm_criterion] = $this->getCriterionLabel(
                        $ps_facet_name,
                ...
                foreach ($va_criteria as $vs_facet_name => $va_criteria_by_facet) {
                    if (is_array($va_criteria_display_strings[$vs_facet_name])) {
                        foreach ($va_criteria_display_strings[$vs_facet_name] as $vm_criterion => $vs_display_criterion) {
                            $va_criteria_with_labels[$vs_facet_name][$vm_criterion] = $this->getCriterionLabel(
                                $vs_facet_name,
                                $vs_display_criterion
                            );
                        }
    ...
    public function getCriterionLabel($ps_facet_name, $pn_row_id)
    {
        ...
        switch ($va_facet_info['type']) {
            # -----------------------------------------------------
            case 'has':
                $vs_yes_text = (isset($va_facet_info['label_yes']) && $va_facet_info['label_yes']) ? $va_facet_info['label_yes'] : _t(
                    'Yes'
                );
                $vs_no_text = (isset($va_facet_info['label_no']) && $va_facet_info['label_no']) ? $va_facet_info['label_no'] : _t(
                    'No'
                );
            ...
            # -----------------------------------------------------
            case 'label':
                if (!($t_table = Datamodel::getInstanceByTableName(
                    (isset($va_facet_info['relative_to']) && $va_facet_info['relative_to']) ? $va_facet_info['relative_to'] : $this->ops_browse_table_name,
                    true
                ))) {
                    break;
                }
            ...
            case 'attribute':
                ...
                switch ($vn_element_type = $t_element->get('datatype')) {
                    case __CA_ATTRIBUTE_VALUE_LIST__:
                        $vs_label = caProcessTemplateForIDs(
                            "^ca_list_items.hierarchy.preferred_labels.name_plural",
                            "ca_list_items",
                            array($pn_row_id),
                            array("delimiter" => " ➜ ")
                        );
            ...
            # -----------------------------------------------------
            case 'field':
                if (!($t_item = Datamodel::getInstanceByTableName(
                    $va_facet_info['relative_to'] ? $va_facet_info['relative_to'] : $this->ops_browse_table_name,
                    true
                ))) {
                    break;
                }
                if ($vb_is_bit = ($t_item->getFieldInfo($va_facet_info['field'], 'FIELD_TYPE') == FT_BIT)) {
                    return ((bool)$pn_row_id) ? caGetOption('label_yes', $va_facet_info, _t('Yes')) : caGetOption(
                        'label_no',
                        $va_facet_info,
                        _t('No')
                    );
                ...
                switch ($va_facet_info['mode']) {
                    case 'user':
                        ...
                        if ($t_user->getPrimaryKey()) {
                            $vs_name = $t_user->get('fname') . ' ' . $t_user->get('lname') . (($vs_email = $t_user->get(
                                    'email'
                                )) ? " ({$vs_email})" : "");
            ...
            case 'current_value':
                ...
                if (isset($va_collapse_map[$table][$va_row_tmp[1]])) {
                ...
                } elseif ($va_row_tmp[2] && ($qr_res = caMakeSearchResult(
                        $table,
                        [$va_row_tmp[2]]
                    )) && $qr_res->nextHit()) {
                    // Return label for id
                    $cv_config = ca_objects::getConfigurationForHistoryTrackingCurrentValue(
                        $policy,
                        $table,
                        $va_row_tmp[1]
                    );
                    ...
                    $template = str_replace(
                        "<l>",
                        "",
                        str_replace("</l>", "", $template)
                    );    // don't allow links in criterion labels
            ...
            # -----------------------------------------------------
            case 'inHomeLocation':
                return $pn_row_id ? caGetOption('label_yes', $va_facet_info, _t('In home location')) : caGetOption(
                    'label_no',
                    $va_facet_info,
                    _t('Not in home location')
                );
                break;
            # -----------------------------------------------------
            ...
            case 'fieldList':
                ...
                if ($va_facet_info['table'] && ($t_browse_table = Datamodel::getInstanceByTableName(
                        $vs_facet_table = $va_facet_info['table'],
                        true
                    ))) {
                    if (!($app = AppController::getInstance())) {
                        return '???';
                    }
                    if ($t_browse_table->load($pn_row_id) && $t_browse_table->isReadable(
                            $app->getRequest(),
                            'preferred_labels'
                        )) {
                        return $t_browse_table->getWithTemplate(
                            isset($va_facet_info['display']) ? $va_facet_info['display'] : "^" . $t_browse_table->tableName(
                                ) . ".preferred_labels"
                        );
                    }
                }
    ...
    public function getFacetList()
    {
        ...
        if (is_array($va_type_restrictions) && sizeof($va_type_restrictions)) {
            $va_facets = array();
            foreach ($this->opa_browse_settings['facets'] as $vs_facet_name => $va_facet_info) {
                if (in_array($vs_facet_name, $va_criteria_facets) && (caGetOption(
                            'type',
                            $va_facet_info,
                            null
                        ) == 'field')) {
                ...
                if (isset($va_facet_info['requires']) && !is_array(
                ...
                if ($vb_facet_is_meets_requirements) {
                    if (isset($va_facet_info['type_restrictions']) && is_array(
                            $va_facet_restrictions = $va_facet_info['type_restrictions']
    ...
    public function getInfoForAvailableFacets()
    {
        ...
        foreach ($va_facets as $vs_facet_name => $va_facet_info) {
            if ($vs_facet_group) {
                if (!(isset($va_facet_info['facet_groups']) && is_array($va_facet_info['facet_groups']) && in_array(
                        $vs_facet_group,
                        $va_facet_info['facet_groups']
                    ))) {
    ...
    public function getInfoForFacetsWithContent()
    {
        ...
        if (is_array($va_facets_with_content)) {
            ...
            foreach ($va_facets_with_content as $vs_facet) {
                if (($vs_facet_group && $va_facets[$vs_facet]['facet_groups'] && is_array(
                            $va_facets[$vs_facet]['facet_groups']
        ...
        if ($this->opo_ca_browse_cache->load($vs_cache_key)) {
            $vn_created_on = $this->opo_ca_browse_cache->getParameter(
                'created_on'
        ...
        if (is_array($va_criteria) && (sizeof($va_criteria) > 0)) {
            if (!$vb_results_cached) {
                ...
                foreach ($va_criteria as $vs_facet_name => $va_row_ids) {
                    ...
                    $vb_is_relative_to_parent = ($va_facet_info['relative_to'] && $this->_isParentRelative(
                    ...
                    switch ($va_facet_info['type']) {
                        ...
                        case 'has':
                            ...
                            if ($va_facet_info['relative_to']) {
                                if ($va_relative_execute_sql_data = $this->_getRelativeExecuteSQLData(
                                    $va_facet_info['relative_to'],
                                    $pa_options
                                )) {
                            ...
                            if (isset($va_facet_info['element_code']) && $t_user && $t_user->isLoaded(
                                ) && ($t_user->getBundleAccessLevel(
                                        $this->ops_browse_table_name,
                                        array_shift(
                                            explode('.', $va_facet_info['element_code'])
                                        )
                            ...
                            if ($va_facet_info['element_code']) {
                                ...
                                if ($vn_state == 0) {
                                    $va_wheres[] = $this->ops_browse_table_name . '.' . $t_item->primaryKey(
                                        ) . " NOT IN (SELECT row_id FROM ca_attributes WHERE table_num = " . $t_item->tableNum(
                                        ) . " AND element_id = " . $t_element->getPrimaryKey() . ")";
                                } else {
                                    $va_joins[] = "INNER JOIN ca_attributes AS caa ON caa.row_id = " . $this->ops_browse_table_name . '.' . $t_item->primaryKey(
                                ...
                                $va_restrict_to_relationship_types = $this->_getRelationshipTypeIDs(
                                ...
                                $va_exclude_relationship_types = $this->_getRelationshipTypeIDs(
                                ...
                                switch (sizeof(
                                    $va_path = array_keys(
                                        Datamodel::getPath($vs_target_browse_table_name, $vs_rel_table_name)
                                    )
                                )) {
                                    case 3:
                                ...
                                if (!is_array(
                                    $va_restrict_to_types = $this->_convertTypeCodesToIDs(
                                ...
                                if (!is_array(
                                    $va_exclude_types = $this->_convertTypeCodesToIDs(
                                        $va_exclude_types,
                                        array('instance' => $t_rel_item)
                                    )
                                )) {
                                ...
                                $va_wheres = array();
                                if (is_array($va_restrict_to_relationship_types) && (sizeof(
                                            $va_restrict_to_relationship_types
                                        ) > 0) && is_object($t_item_rel) && (bool)$vn_state) {
                                    $va_wheres[] = "(" . $t_item_rel->tableName() . ".type_id IN (" . join(
                                            ',',
                                            $va_restrict_to_relationship_types
                                        ) . "))";
                                }
                                if (is_array($va_exclude_relationship_types) && (sizeof(
                                            $va_exclude_relationship_types
                                        ) > 0) && is_object($t_item_rel) && (bool)$vn_state) {
                                ...
                                if (is_array($va_restrict_to_types) && (sizeof($va_restrict_to_types) > 0) && is_object(
                                        $t_rel_item
                                    )) {
                                ...
                                if (is_array($va_exclude_types) && (sizeof($va_exclude_types) > 0) && is_object(
                                        $t_rel_item
                                    )) {
                                    $va_wheres[] = "((" . $t_rel_item->tableName() . ".type_id NOT IN (" . join(
                                            ',',
                                            $va_exclude_types
                                        ) . ")) OR (" . $t_rel_item->tableName() . ".type_id IS NULL))";
                                }
                            ...
                            if ($t_item_rel && isset($va_facet_info['filter_on_interstitial']) && is_array(
                                    $va_facet_info['filter_on_interstitial']
                                ...
                                foreach ($va_facet_info['filter_on_interstitial'] as $vs_field_name => $va_values) {
                                    ...
                                    if (!($o_value = Attribute::getValueInstance(
                                        ca_metadata_elements::getElementDatatype($vs_field_name)
                                    ))) {
                                        continue;
                                    }

                                    $va_element_value = $o_value->parseValue(
                                        $va_values[0],
                                        array_merge(
                                            ca_metadata_elements::getElementSettingsForId(
                                                $vs_field_name
                                            ),
                                            [
                                                'list_id' => ca_metadata_elements::getElementListID(
                                                    $vs_field_name
                                                )
                                            ],
                                            ['matchOn' => ['idno']]
                                    ...
                                    $va_joins[] = "INNER JOIN ca_attributes c_a ON c_a.row_id = " . $t_item_rel->primaryKey(
                                            true
                            ...
                            $va_acc[$vn_i] = $qr_res->getAllFieldValues(
                        ...
                        case 'label':
                            ...
                            if ($va_facet_info['relationship']) {
                                ...
                                if (is_array(
                                    ...
                                    ) && ($rel_type_ids = caMakeRelationshipTypeIDList(
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                $va_acc[$vn_i] = array_merge(
                                    $va_acc[$vn_i],
                                    $qr_res->getAllFieldValues(
                                        $this->ops_browse_table_name . '.' . $t_item->primaryKey()
                                    )
                                );
                        ...
                        case 'field':
                            ...
                            if ($t_user && $t_user->isLoaded() && ($t_user->getBundleAccessLevel(
                                        $vs_table_name,
                                        $bundle_name
                                    ) < __CA_BUNDLE_ACCESS_READONLY__)) {
                                break;
                        ...
                        case 'attribute':
                            ...
                            if ($t_user->getBundleAccessLevel(
                                    $this->ops_browse_table_name,
                                    $va_facet_info['element_code']
                                ) < __CA_BUNDLE_ACCESS_READONLY__) {
                                break;
                            }
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                if (is_array($va_restrict_to_types) && (sizeof(
                                            $va_restrict_to_types
                                        ) > 0) && method_exists($t_item, "getTypeList")) {
                                    ...
                                    foreach ($va_value as $vs_f => $vs_v) {
                                        switch ($vn_datatype) {
                                            case __CA_ATTRIBUTE_VALUE_LIST__:
                                                ...
                                                $va_item_ids = $t_list_item->getHierarchy(
                                                    (int)$vs_v,
                                                    array(
                                                        'idsOnly' => true,
                                                        'includeSelf' => true
                                            ...
                                            case __CA_ATTRIBUTE_VALUE_LCSH__:
                                                ...
                                                break;
                                            default:
                                                $va_attr_sql[] = "(ca_attribute_values.{$vs_f} " . (is_null(
                                                        $vs_v
                                                    ) ? " IS " : " = ") . " ?)";
                                                $va_attr_values[] = $vs_v;
                                                break;
                                ...
                                $vs_container_sql = '';
                                if (is_array($va_element_code) && (sizeof($va_element_code) == 1) && is_array(
                                ...
                                if (is_array($va_element_code) && sizeof($va_element_code) == 1) {
                                    ...
                                    $va_container_ids[$va_element_code[0]] = array_unique(
                                        $qr_res->getAllFieldValues('attribute_id')
                                    );
                                    $this->opo_ca_browse_cache->setParameter(
                                        'container_ids',
                                        $va_container_ids[$va_element_code[0]]
                                    );
                                }
                        ...
                        case 'normalizedDates':
                            ...
                            if (!($vb_is_element = $t_element->load(
                                    array('element_code' => array_pop($va_element_code))
                                )) && !($vb_is_field = ($t_item->hasField(
                                        $va_facet_info['element_code']
                                    ) && ($t_item->getFieldInfo(
                                            $va_facet_info['element_code'],
                                            'FIELD_TYPE'
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                if ($vn_row_id !== 'null') {
                                    ...
                                    if (substr(
                                            ...
                                            0,
                                            10
                                        ) == '0101000000') { // rewrite start date to encompass circa dates
                        ...
                        case 'normalizedLength':
                            ...
                            $vs_increment = caGetOption('increment', $va_facet_info, '1 m');
                            $vo_increment = caParseLengthDimension($vs_increment);
                            $vn_increment_in_current_units = (float)$vo_increment->convertTo(
                                $vs_output_units,
                                6,
                                'en_US'
                            );
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                $vn_start_in_meters = (float)$vo_start->convertTo(
                        ...
                        case 'normalizedWeight':
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                $vn_end_in_meters = (float)$vo_end->convertTo(
                                    Zend_Measure_Weight::KILOGRAM,
                                    6,
                                    'en_US'
                                );
                        ...
                        case 'relationship_types':
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                if ($browse_on_self_relation) {
                                    $va_joins[] = 'INNER JOIN ' . $va_path[1] . ' ON (' . $va_path[0] . '.' . $t_item->primaryKey(
                                        ) . ' = ' . $va_path[1] . '.' . $t_item_rel->getRightTableFieldName(
                                        ) . ' OR ' . $va_path[0] . '.' . $t_item->primaryKey(
                                        ) . ' = ' . $va_path[1] . '.' . $t_item_rel->getLeftTableFieldName() . ")\n";
                                    $va_joins[] = 'INNER JOIN ' . $va_path[2] . ' AS ' . $va_path[2] . '_rel ON (' . $va_path[2] . '_rel.' . $t_item->primaryKey(
                                        ) . ' = ' . $va_path[1] . '.' . $t_item_rel->getLeftTableFieldName(
                                        ) . ') OR (' . $va_path[2] . '_rel.' . $t_item->primaryKey(
                                        ) . ' = ' . $va_path[1] . '.' . $t_item_rel->getRightTableFieldName() . ")\n";
                                } else {
                                    $vs_cur_table = array_shift($va_path);
                                    $va_joins = array();
                                ...
                                if ($t_item_rel && isset($va_facet_info['filter_on_interstitial']) && is_array(
                                    ...
                                    foreach ($va_facet_info['filter_on_interstitial'] as $vs_field_name => $va_values) {
                                        ...
                                        if (!($vn_element_id = (int)ca_metadata_elements::getElementID(
                                            $vs_field_name
                                        ))) {
                                            continue;
                                        }
                                ...
                                if ($va_facet_info['type'] == 'relationship_types') {
                                    $vs_get_item_sql = "(" . $t_item_rel->tableName(
                                        ) . ".type_id = " . (int)$vn_row_id . ")";
                                } elseif ((!isset($va_facet_info['dont_expand_hierarchically']) || !$va_facet_info['dont_expand_hierarchically']) && $t_rel_item->isHierarchical(
                                    ...
                                    $vs_get_item_sql = "{$vs_rel_item_table_name}.{$vs_hier_left_fld} >= " . $t_rel_item->get(
                                            $vs_hier_left_fld
                                        ) . " AND {$vs_rel_item_table_name}.{$vs_hier_right_fld} <= " . $t_rel_item->get(
                                            $vs_hier_right_fld
                                        );
                                    if ($vn_hier_id_fld = $t_rel_item->getProperty('HIERARCHY_ID_FLD')) {
                                        $vs_get_item_sql .= " AND {$vs_rel_item_table_name}.{$vn_hier_id_fld} = " . (int)$t_rel_item->get(
                                                $vn_hier_id_fld
                                            );
                                    }
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                if ($va_row_tmp[0] == 89) { // ca_storage_locations
                                    ...
                                    $vs_sql = "
                                        ...
                                        . ((sizeof(
                                                $va_row_tmp
                                            ) > 2) ? " AND (((cv.current_type_id = ?) AND (cv.current_row_id = ?)) OR (cv.current_row_id IN (?)))" : "");
                        ...
                        case 'fieldList':
                            ...
                            $dont_expand_hierarchically = caGetOption(
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                if (!$dont_expand_hierarchically) {
                                    if (!($t_list_item = ca_list_items::find(
                                        $vn_row_id,
                                        [
                                    ...
                                    $ids = $t_list_item->get(
                                        'ca_list_items.children.item_id',
                                        ['returnAsArray' => true]
                                    );
                                }
                                $ids[] = $vn_row_id;
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                if ($vn_i == 0) {
                                    $vs_sql = "
											SELECT " . $this->ops_browse_table_name . '.' . $t_item->primaryKey() . "
											FROM " . $this->ops_browse_table_name . "
											INNER JOIN ca_metadata_dictionary_rule_violations ON ca_metadata_dictionary_rule_violations.row_id = " . $this->ops_browse_table_name . '.' . $t_item->primaryKey(
                                        ) . " AND ca_metadata_dictionary_rule_violations.table_num = " . $t_item->tableNum(
                                        ) . "
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                if ($vn_i == 0) {
                                    $vs_sql = "
											SELECT " . $this->ops_browse_table_name . '.' . $t_item->primaryKey() . "
											FROM " . $this->ops_browse_table_name . "
											INNER JOIN ca_items_x_tags ON ca_items_x_tags.row_id = " . $this->ops_browse_table_name . '.' . $t_item->primaryKey(
                        ...
                        case 'dupeidno':
                            ...
                            if (is_array($idno_list = $qr_res->getAllFieldValues($idno_fld)) && (sizeof(
                                        $idno_list
                                    ) > 0)) {
                                ...
                                $va_acc[$vn_i] = array_merge(
                                    $va_acc[$vn_i],
                                    $qr_res->getAllFieldValues(
                                        $vs_browse_table_name . '.' . $t_item->primaryKey()
                                    )
                                );
                        ...
                        default:
                            switch ($vs_facet_name) {
                                case '_search':
                                    ...
                                    if (is_array($va_type_ids = $this->getTypeRestrictionList()) && sizeof(
                                            $va_type_ids
                                        )) {
                                        $o_search->setTypeRestrictions($va_type_ids);
                                    }
                                    if (is_array($va_source_ids = $this->getSourceRestrictionList()) && sizeof(
                                            $va_source_ids
                                        )) {
                                ...
                                case '_reltypes':
                                    ...
                                    foreach ($va_row_ids as $vs_tmp) {
                                        ...
                                        $va_path_to_target = array_keys(
                                            Datamodel::getPath($vs_target_browse_table_name, $t_target->tableName())
                                        );

                                        if (is_array($va_path_to_target) && (sizeof($va_path_to_target) == 3)) {
                                            if (!($t_target_rel = Datamodel::getInstanceByTableName(
                                                    $va_path_to_target[1],
                                                    true
                                                )) || !$t_target_rel->isRelationship() || !$t_target_rel->hasField(
                                                    'type_id'
                                                )) {
                                                break;
                                            }
                                            $va_ids_from_rel = array_unique(
                                            ...
                                            if (is_array($va_ids_from_rel) && (sizeof($va_ids_from_rel) > 0)) {
                                                $qr_res = $this->opo_db->query(
                                                    "SELECT DISTINCT {$vs_target_browse_table_pk} FROM " . $va_path_to_target[1] . " WHERE type_id IN (?)",
                                                    [$va_ids_from_rel]
                                                );
                                                $va_acc[$vn_i] = array_merge(
                                                    $va_acc[$vn_i],
                ...
                if ((!isset($pa_options['omitChildRecords']) || !$pa_options['omitChildRecords']) && caGetOption(
                        'expandResultsHierarchically',
                        $pa_options,
                        false
                    ) && ($vs_hier_id_fld = Datamodel::getTableProperty(
                        $this->ops_browse_table_name,
                        'HIERARCHY_ID_FLD'
                    ))) {
                    ...
                    foreach ($va_acc as $vn_i => $va_acc_content) {
                        ...
                        if (is_array($va_expanded_res = $qr_expand->getAllFieldValues($t_item->primaryKey())) && sizeof(
                                $va_expanded_res
                            )) {
                ...
                if (sizeof($va_res)) {
                    ...
                    if ((isset($pa_options['limitToModifiedOn']) && $pa_options['limitToModifiedOn'])) {
                        ...
                        if ($o_tep->parse($pa_options['limitToModifiedOn'])) {
                            ...
                            $va_joins['ca_change_log_subjects'] = "INNER JOIN ca_change_log_subjects ON ca_change_log_subjects.subject_row_id = " . $this->ops_browse_table_name . "." . $t_item->primaryKey(
                    ...
                    if (($va_browse_type_ids = $this->getTypeRestrictionList()) && is_array(
                            $va_browse_type_ids
                        ...
                        $va_wheres[] = '(' . $this->ops_browse_table_name . '.' . $t_subject->getTypeFieldName(
                            ) . ' IN (' . join(', ', $va_browse_type_ids) . ')' . ($t_subject->getFieldInfo(
                                'type_id',
                                'IS_NULL'
                            ) ? " OR (" . $this->ops_browse_table_name . '.' . $t_subject->getTypeFieldName(
                                ) . " IS NULL)" : '') . ')';
                    }

                    if (($va_browse_source_ids = $this->getSourceRestrictionList()) && is_array(
                            $va_browse_source_ids
                        ...
                        $va_wheres[] = '(' . $this->ops_browse_table_name . '.' . $t_subject->getSourceFieldName(
                                ...
                                ', ',
                                $va_browse_source_ids
                            ) . ') OR (' . $this->ops_browse_table_name . '.' . $t_subject->getSourceFieldName(
                            ) . ' IS NULL))';
                    }
                    ...
                    $va_results = $vb_preserve_order ? array_intersect(
                        $va_possible_values,
                        $qr_res->getAllFieldValues($t_item->primaryKey())
                    ) : $qr_res->getAllFieldValues($t_item->primaryKey());

                    if ((!isset($pa_options['dontFilterByACL']) || !$pa_options['dontFilterByACL']) && $this->opo_config->get(
                            'perform_item_level_access_checking'
                        ) && method_exists($t_item, "supportsACL") && $t_item->supportsACL()) {
                        $va_results = $this->filterHitsByACL(
                            $va_results,
            ...
            if (
                ...
                if ((!isset($pa_options['dontFilterByACL']) || !$pa_options['dontFilterByACL']) && $this->opo_config->get(
                    ...
                    $va_results = array_keys(
                        $this->filterHitsByACL(
                            $va_results,
                            $this->opn_browse_table_num,
                            $vn_user_id,
                            __CA_ACL_READONLY_ACCESS__
    ...
    public function getFacetWithGroups($ps_facet_name, $ps_group_mode, $ps_grouping_field = null, $pa_options = null)
    {
        ...
        if ($vs_grouping_attribute_element_code = (preg_match(
            '!^ca_attribute_([\w]+)!',
            $ps_grouping_field,
            $va_matches
        )) ? $va_matches[1] : null) {
        ...
        if ((!isset($va_facet_info['groupings'][$ps_grouping_field]) || !($va_facet_info['groupings'][$ps_grouping_field])) && is_array(
                $va_facet_info['groupings']
            )) {
                            ...
                            if ($vn_grouping_attribute_id) {
                                switch ($vn_element_datatype) {
                                    case 2: //date
                                        ...
                                        if (isset($va_item['ca_attribute_' . $vn_grouping_attribute_id]) && is_array(
                                                $va_item['ca_attribute_' . $vn_grouping_attribute_id]
                                            )) {
                                            foreach ($va_item['ca_attribute_' . $vn_grouping_attribute_id] as $vn_i => $va_v) {
                                                $va_v = $o_tep->normalizeDateRange(
                                                    $va_v['value_decimal1'],
                                                    $va_v['value_decimal2'],
                                                    (isset($va_tmp[1]) && in_array(
                                                            $va_tmp[1],
                                                            array(
                                                                'years',
                                                                'decades',
    ...
    public function getFacetContent($ps_facet_name, $pa_options = null)
    {
        ...
        if (($va_browse_type_ids = $this->getTypeRestrictionList()) && is_array($va_browse_type_ids) && sizeof(
                $va_browse_type_ids
            )) {        // type restrictions
            $vs_browse_type_limit_sql = '(' . $t_subject->tableName() . '.' . $t_subject->getTypeFieldName(
                ) . ' IN (' . join(', ', $va_browse_type_ids) . ')' . ($t_subject->getFieldInfo(
                    'type_id',
                    'IS_NULL'
                ) ? " OR (" . $this->ops_browse_table_name . '.' . $t_subject->getTypeFieldName(
            ...
            if (is_array(
                $va_facet_info['type_restrictions']
        ...
        if (($va_browse_source_ids = $this->getSourceRestrictionList()) && is_array($va_browse_source_ids) && sizeof(
                $va_browse_source_ids
            )) {        // source restrictions
            $vs_browse_source_limit_sql = '(' . $t_subject->tableName() . '.' . $t_subject->getSourceFieldName(
                ) . ' IN (' . join(', ', $va_browse_source_ids) . ')' . ($t_subject->getFieldInfo(
                    'source_id',
                    'IS_NULL'
                ) ? " OR (" . $this->ops_browse_table_name . '.' . $t_subject->getSourceFieldName(
                    ) . " IS NULL)" : '') . ')';
            ...
            if (is_array(
                $va_facet_info['source_restrictions']
            )) {        // facet source restrictions bind a facet to specific sources; we check them here
        ...
        switch ($va_facet_info['type']) {
                ...
                if ($va_facet_info['element_code']) {
                    ...
                    foreach ($va_facet_values as $vs_state_name => $va_state_info) {
                        ...
                        if (!(bool)$va_state_info['id']) {    // no option
                            $va_wheres[] = $this->ops_browse_table_name . '.' . $vs_browse_table_pk . " NOT IN (select row_id from ca_attributes where table_num = " . $t_item->tableNum(
                                ) . " AND element_id = " . $t_element->getPrimaryKey() . ")";
                        } else {                            // yes option
                            $va_joins[] = "LEFT JOIN ca_attributes AS caa ON  " . $this->ops_browse_table_name . '.' . $vs_browse_table_pk . " = caa.row_id AND " . $t_item->tableNum(
                                ) . " = caa.table_num";
                        ...
                        if ($va_facet_info['relative_to']) {
                            ...
                            if ($va_relative_sql_data = $this->_getRelativeFacetSQLData(
                                $va_facet_info['relative_to'],
                                $pa_options
                            )) {
                        ...
                        if ($vb_check_availability_only) {
                            ...
                            $qr_res = $this->opo_db->query($vs_sql);
                            if ($qr_res->numRows() > 0) {
                                $va_facet[$vs_state_name] = array_merge(
                                    $va_state_info,
                    ...
                    foreach ($va_facet_values as $vs_state_name => $va_state_info) {
                        ...
                        $id_set = (is_array($va_results) && sizeof($va_results)) ? "{$vs_browse_table_pk} IN (" . join(
                                ",",
                                $va_results
                            ) . ")" : null;
            ...
            case 'label':
                ...
                if (sizeof(
                    $va_label_order_by_fields = isset($va_facet_info['order_by_label_fields']) ? $va_facet_info['order_by_label_fields'] : []
                )) {
                    $va_label_order_by_fields = array_map(
                        function ($v) {
                            return "l.{$v}";
                        },
                ...
                if (isset($va_facet_info['preferred_labels_only']) && $va_facet_info['preferred_labels_only'] && $t_label->hasField(
                        'is_preferred'
                    )) {
                ...
                if (is_array($va_restrict_to_types) && sizeof($va_restrict_to_types)) {
                    $va_restrict_to_type_ids = caMakeTypeIDList(
                        $vs_browse_table_name,
                        $va_restrict_to_types,
                        array('dont_include_subtypes_in_type_restriction' => true)
                    );
                    if (is_array($va_restrict_to_type_ids) && sizeof($va_restrict_to_type_ids)) {
                        $va_where_sql[] = "(" . $vs_browse_table_name . "." . $t_item->getTypeFieldName(
                            ) . " IN (" . join(", ", $va_restrict_to_type_ids) . ")" . ($t_item->getFieldInfo(
                                'type_id',
                                'IS_NULL'
                            ) ? " OR (" . $vs_browse_table_name . '.' . $t_item->getTypeFieldName(
                                ) . " IS NULL)" : '') . ")";
                        $vb_needs_join = true;
                    }
                }
                if (is_array($va_exclude_types) && sizeof($va_exclude_types)) {
                    $va_exclude_type_ids = caMakeTypeIDList(
                        $vs_browse_table_name,
                        $va_exclude_types,
                        array('dont_include_subtypes_in_type_restriction' => true)
                    );
                    if (is_array($va_exclude_type_ids) && sizeof($va_exclude_type_ids)) {
                        $va_where_sql[] = "(" . $vs_browse_table_name . "." . $t_item->getTypeFieldName(
                            ) . " NOT IN (" . join(", ", $va_exclude_type_ids) . ")" . ($t_item->getFieldInfo(
                                'type_id',
                                'IS_NULL'
                            ) ? " OR (" . $vs_browse_table_name . '.' . $t_item->getTypeFieldName(
                                ) . " IS NULL)" : '') . ")";
                ...
                if ($vb_needs_join) {
                    $va_joins[] = "INNER JOIN " . $vs_browse_table_name . " ON " . $vs_browse_table_name . "." . $t_item->primaryKey(
                        ) . " = l." . $t_item->primaryKey();
                }
                ...
                if (is_array($va_results) && sizeof($va_results)) {
                    if ($va_facet_info['relative_to']) {
                        $va_where_sql[] = $this->ops_browse_table_name . "." . $t_subject->primaryKey(
                            ) . " IN (" . join(",", $va_results) . ")";
                    } else {
                ...
                if ($va_facet_info['relationship']) {
                    $va_joins[] = "INNER JOIN " . $va_facet_info['relationship'] . " ON {$vs_browse_table_name}." . $t_item->primaryKey(
                        ) . " = " . $va_facet_info['relationship'] . "." . $t_item->primaryKey();
                    if (is_array(
                            $va_facet_info['restrict_to_relationship_types']
                        ) && ($rel_type_ids = caMakeRelationshipTypeIDList(
                            $va_facet_info['relationship'],
                ...
                if ($vb_check_availability_only) {
                    ...
                    $vs_parent_fld_select = (($vs_parent_fld = $t_item->getProperty(
                        'HIERARCHY_PARENT_ID_FLD'
                    )) ? $vs_browse_table_name . "." . $vs_parent_fld : '');

                    $group_by_fields = array_unique(
                        array_merge(
                            $va_label_order_by_fields,
                            $va_label_ui_fields,
                    ...
                    $vs_sql = "
							SELECT COUNT(*) as _count, l.locale_id, l.{$vs_label_display_field} " . ($vs_parent_fld_select ? ", {$vs_parent_fld_select}" : "") . ", l.{$vs_item_pk}" . ((sizeof(
                                $va_label_ui_fields
                            ) > 0) ? ", " . join(", ", $va_label_ui_fields) : "") . "
							FROM {$vs_label_table_name} l
								{$vs_join_sql}
								{$vs_where_sql}
                    ...
                    while ($qr_res->nextRow()) {
                        ...
                        if (!isset($va_values[$vs_label_key][$locale_id])) {
                            $va_values[$vs_label_key][$locale_id] = array_merge(
                                $qr_res->getRow(),
                                array(
                            ...
                            $va_values[$vs_label_key][$qr_res->get('locale_id')]['content_count'] += (int)$qr_res->get(
                                '_count'
                            );
                        }
            ...
            case 'attribute':
                ...
                $vs_container_code = (is_array($va_element_code) && (sizeof($va_element_code) > 0)) ? array_pop(
                    $va_element_code
                ) : null;
                ...
                $va_joins = array(
                    ...
                    'INNER JOIN ' . (!$vb_is_relative_to_parent ? "{$vs_browse_table_name} ON {$vs_browse_table_name}." : "{$vs_browse_table_name} AS parent ON parent.") . $t_item->primaryKey(
                ...
                if (is_array($va_results) && sizeof($va_results) && ($this->numCriteria() > 0)) {
                    ...
                    if (($ps_facet_name === $last_facet) && $va_facet_info['multiple'] && (is_array(
                                $c[$ps_facet_name]
                            ) && sizeof($c[$ps_facet_name]))) {
                ...
                if ($vb_check_availability_only) {
                    ...
                    if (is_array(
                        ...
                        $va_suppress_values = array_filter(
                            $va_suppress_values,
                            function ($v) {
                                return trim(strtolower($v)) !== 'null';
                            }
                    ...
                    if (is_array($va_container_ids[$vs_container_code]) && sizeof(
                            $va_container_ids[$vs_container_code]
                        )) {
                        $vs_container_sql = " AND ca_attributes.attribute_id IN (?)";
                    ...
                    switch ($vn_element_type) {
                        case __CA_ATTRIBUTE_VALUE_LIST__:
                            ...
                            $t_list_item = new ca_list_items();
                            $va_list_item_cache = $t_list_item->getFieldValuesForIDs(
                                $va_values,
                                array(
                                    'type_id',
                                    'idno',
                            ...
                            if (is_array($va_list_item_cache)) {
                                foreach ($va_list_item_cache as $vn_id => $va_item) {
                                    ...
                                    if (is_array($pa_options['checkAccess']) && sizeof(
                                            $pa_options['checkAccess']
                            ...
                            if (is_array($va_suppress_values)) {
                                $va_suppress_values = ca_lists::getItemIDsFromList(
                            ...
                            foreach ($va_values as $i => $vn_val) {
                                ...
                                if (is_array($pa_options['checkAccess']) && sizeof(
                                        ...
                                        $va_list_item_cache[$vn_val]['access'],
                                        $pa_options['checkAccess']
                                    )) {
                                    continue;
                                }

                                if (is_array($va_restrict_to_types) && sizeof($va_restrict_to_types) && !in_array(
                                        $va_list_item_cache[$vn_val]['type_id'],
                                        $va_restrict_to_types
                                    )) {
                                    continue;
                                }
                            ...
                            if (!isset($va_facet_info['dont_expand_hierarchically']) || !$va_facet_info['dont_expand_hierarchically']) {
                                ...
                                $qr_ancestors = call_user_func(
                                    $t_rel_item->tableName() . '::getHierarchyAncestorsForIDs',
                                    $va_values,
                                ...
                                $vb_check_ancestor_access = (bool)(isset($pa_options['checkAccess']) && is_array(
                                        $pa_options['checkAccess']
                                    ) && sizeof($pa_options['checkAccess']) && $t_rel_item->hasField('access'));
                                ...
                                if ($qr_ancestors) {
                                    ...
                                    while ($qr_ancestors->nextHit()) {
                                        ...
                                        if (is_array($va_suppress_values) && (in_array(
                                                $vn_ancestor_id,
                                                $va_suppress_values
                                            ))) {
                                            continue;
                                        }
                                        if (is_array($va_exclude_types) && (sizeof($va_exclude_types) > 0) && in_array(
                                                $vn_parent_type_id,
                                                $va_exclude_types
                                            )) {
                                            continue;
                                        ...
                                        if (is_array($va_restrict_to_types) && (sizeof(
                                                    $va_restrict_to_types
                                                ) > 0) && !in_array($vn_parent_type_id, $va_restrict_to_types)) {
                                            continue;
                                        }
                                        if ($vb_check_ancestor_access && !in_array(
                                                $qr_ancestors->get('access'),
                                                $pa_options['checkAccess']
                                            )) {
                                            continue;
                                        }
                                        ...
                                        if ((!isset($va_facet_info['dont_expand_hierarchically']) || !$va_facet_info['dont_expand_hierarchically']) && $t_rel_item->isHierarchical(
                                            ...
                                            $vs_get_item_sql = "{$vs_rel_table}.{$vs_hier_left_fld} >= " . $qr_ancestors->get(
                                                    $vs_hier_left_fld
                                                ) . " AND {$vs_rel_table}.{$vs_hier_right_fld} <= " . $qr_ancestors->get(
                                                    $vs_hier_right_fld
                                                );
                                            if ($vn_hier_id_fld = $t_rel_item->getProperty('HIERARCHY_ID_FLD')) {
                                                $vs_get_item_sql .= " AND {$vs_rel_table}.{$vn_hier_id_fld} = " . (int)$qr_ancestors->get(
                                                        $vn_hier_id_fld
                                                    );
                                            }
                                            $vs_get_item_sql = "({$vs_get_item_sql})";
                                        ...
                                        $va_facet_list[$vn_ancestor_id] = array(
                                            'id' => $vn_ancestor_id,
                                            'label' => ($vs_label = $qr_ancestors->get(
                            ...
                            if ($vn_list_id = $t_element->get('list_id')) {
                                ...
                                switch ($t_list->get('default_sort')) {
                                    ...
                                    case __CA_LISTS_SORT_BY_VALUE__:
                                        ...
                                        break;
                                    default:
                                        return caSortArrayByKeyInValue(
                                            $va_facet_list,
                                            ['label'],
                                            'ASC',
                                            ['naturalSort' => true]
                        ...
                        switch ($vn_element_type) {
                            case __CA_ATTRIBUTE_VALUE_LIST__:
                                ...
                                $va_values[strToLower($vs_val)] = array(
                                    ...
                                    'label' => html_entity_decode(
                                        $va_list_items[$vs_val]['name_plural'] ? $va_list_items[$vs_val]['name_plural'] : $va_list_items[$vs_val]['item_value']
                                    ),
                            ...
                            case __CA_ATTRIBUTE_VALUE_OBJECTLOTS__:
                                ...
                                $va_values[strToLower($vs_val)] = array(
                                    'id' => $vn_id,
                                    'label' => html_entity_decode(
                                        $va_auth_items[$vn_id] ? $va_auth_items[$vn_id] : $vs_val
                                    ),
                                    'content_count' => $qr_res->get('_count')
                                );
                                break;
                            case __CA_ATTRIBUTE_VALUE_LCSH__:
                                ...
                                break;
                            case __CA_ATTRIBUTE_VALUE_GEONAMES__:
                                $value_id = ca_attribute_values::getValueIDFor(
                                    $o_attr->getElementID(),
                                    preg_replace(
                                        '![ ]*\[[^\]]*\]$!',
                                        '',
            ...
            case 'current_value':
                ...
                $va_joins = [
                    "INNER JOIN ca_history_tracking_current_values AS cv ON cv.row_id = {$vs_browse_table_name}." . $t_item->primaryKey(
                    ) . " AND cv.table_num = {$vs_browse_table_num}"
                ];
                ...
                if ($vb_check_availability_only) {
                    ...
                    foreach ($values_by_table as $current_table_num => $current_row_ids_by_type) {
                        foreach ($current_row_ids_by_type as $current_type_id => $current_row_info) {
                            ...
                            $va_config = ca_objects::getConfigurationForHistoryTrackingCurrentValue(
                                $policy,
                                $current_table_name,
                                $current_type_id,
                                array('facet' => isset($va_facet_info['display']) ? $va_facet_info['display'] : null)
                            );
                            ...
                            if ($current_table_name == 'ca_storage_locations') {
                                ...
                                while ($qr_res->nextHit()) {
                                    if (is_array(
                                        $va_ids = $qr_res->get(
                                            "{$current_table_name}.hierarchy.{$vs_hier_pk}",
                            ...
                            $display_config = $this->getTableEntryFromMap(
                                $va_facet_info,
                                'display',
                                $current_table_name
                            );
            ...
            case 'fieldList':
                ...
                if (isset($va_field_info['LIST_CODE']) && ($vs_list_name = $va_field_info['LIST_CODE'])) {
                    ...
                    if ($vb_check_availability_only) {
                        ...
                        $va_ordering_fields_to_fetch = (isset($va_facet_info['order_by_label_fields']) && is_array(
                        ...
                        // Get label ordering fields
                        if (isset($va_facet_info['order_by_label_fields']) && is_array(
                                $va_facet_info['order_by_label_fields']
                    ...
                    if ($vs_list_name = $va_field_info['LIST']) {
                        ...
                        if (is_array($va_results) && sizeof($va_results) && ($this->numCriteria() > 0)) {
                            $va_wheres[] = "(" . $t_subject->tableName() . '.' . $t_subject->primaryKey(
                                ) . " IN (" . join(',', $va_results) . "))";
                        }
                        ...
                        if ($t_browse_table = Datamodel::getInstanceByTableName(
                            ...
                            if (method_exists($t_browse_table, 'getLabelTableInstance')) {
                                ...
                                $va_joins[] = 'INNER JOIN ' . $t_label_instance->tableName(
                                    ) . " AS lab ON lab." . $t_browse_table->primaryKey(
                                    ) . ' = ' . $t_browse_table->tableName() . '.' . $t_browse_table->primaryKey();
                            }
                            ...
                            if (is_array($va_wheres) && sizeof($va_wheres) && ($vs_where_sql = join(
                                    ' AND ',
                                    $va_wheres
                                ))) {
                            ...
                            if ($vb_check_availability_only) {
                                ...
                                $values = caProcessTemplateForIDs(
                                    $vs_display,
            ...
            case 'field':
                ...
                $va_restrict_to_types_expanded = $this->_convertTypeCodesToIDs(
                    $va_restrict_to_types,
                    array('instance' => $t_item)
                );
                ...
                if (is_array($va_restrict_to_types) && (sizeof($va_restrict_to_types) > 0) && method_exists(
                        $t_rel_item,
                        "getTypeList"
                    )) {
                    $va_wheres[] = "{$va_restrict_to_types_expanded}.type_id IN (" . join(
                            ...
                            caGetOption(
                                ...
                                $va_facet_info,
                                false
                            ) ? $va_restrict_to_types : $va_restrict_to_types_expanded
                        ) . ")";
            ...
            case 'violations':
                ...
                if ($vb_check_availability_only) {
                    $vs_sql = "
							SELECT 1
							FROM {$vs_browse_table_name}
							INNER JOIN ca_metadata_dictionary_rule_violations ON ca_metadata_dictionary_rule_violations.row_id = {$vs_browse_table_name}." . $t_item->primaryKey(
                        ) . " AND ca_metadata_dictionary_rule_violations.table_num = {$vs_browse_table_num}
							{$vs_join_sql}
							WHERE
								{$vs_where_sql}
							LIMIT 2";
                    ...
                    while ($qr_res->nextRow()) {
                        if ($t_rule->load($qr_res->get('rule_id'))) {
                            if (!($vs_val = trim(
                                caExtractSettingsValueByUserLocale('label', $t_rule->getSettings())
                            ))) {
                                continue;
                            }
            ...
            case 'tags':
                ...
                if ($vb_check_availability_only) {
                    $vs_sql = "
							SELECT 1
							FROM {$vs_browse_table_name}
							INNER JOIN ca_items_x_tags ON ca_items_x_tags.row_id = {$vs_browse_table_name}." . $t_item->primaryKey(
                        ) . " AND ca_items_x_tags.table_num = {$vs_browse_table_num}
							{$vs_join_sql}
							WHERE
								{$vs_where_sql}
							LIMIT 2";
                    ...
                    switch ($va_facet_info['mode']) {
                        ...
                        case 'all':
                            $vs_sql = "
									...
									{$vs_join_sql}
									WHERE
										ca_objects.deleted = 0 " . (sizeof(
                                    $va_results
                                ) ? "AND (" . $t_subject->tableName() . '.' . $t_subject->primaryKey() . " IN (" . join(
                                        ',',
                                        $va_results
                                    ) . "))" : "") . "
									LIMIT 2";
                    ...
                    switch ($va_facet_info['mode']) {
                        case 'user':
                            $vs_sql = "
									...
									{$vs_join_sql}
									WHERE
										{$vs_where_sql} " . ((sizeof($va_results) ? " AND (" . $t_subject->tableName(
                                    ) . '.' . $t_subject->primaryKey() . " IN (" . join(
                                        ',',
                                        $va_results
                                    ) . "))" : "")) .
                            ...
                            while ($qr_res->nextRow()) {
                                ...
                                $vs_val = $qr_res->get('fname') . ' ' . $qr_res->get(
                                        'lname'
                                    ) . (($vs_email = $qr_res->get('email')) ? "({$vs_email})" : '');
                                if ($va_criteria[$vs_val]) {
                                    continue;
                                }        // skip items that are used as browse critera - don't want to browse on something you're already browsing on
            ...
            case 'normalizedDates':
                ...
                if (!($vb_is_element = $t_element->load(
                        array('element_code' => array_pop($va_element_code))
                    )) && !($vb_is_field = ($t_item->hasField($va_facet_info['element_code']) && ($t_item->getFieldInfo(
                                $va_facet_info['element_code'],
                                'FIELD_TYPE'
                            ) === FT_HISTORIC_DATERANGE)))) {
                    return array();
                ...
                if ($vb_is_element) {
                    $va_joins = array(
                        'INNER JOIN ca_attribute_values ON ca_attributes.attribute_id = ca_attribute_values.attribute_id',
                        'INNER JOIN ' . $vs_browse_table_name . ' ON ' . $vs_browse_table_name . '.' . $t_item->primaryKey(
                        ) . ' = ca_attributes.row_id AND ca_attributes.table_num = ' . intval($vs_browse_table_num)
                    );
                } else {
                    $va_joins = array();
                                    ...
                                    foreach (array_keys($va_criteria_values) as $vs_date) {
                                        if ($o_tep->parse($vs_date)) {
                                            ...
                                            if (is_null($vn_min) || ($va_ts['start'] < $vn_min)) {
                                                $va_facet_info['minimum_date'] = $o_tep->getText(
                                                    ['start_as_iso8601' => true]
                                                );
                                                $vn_min = $va_ts['start'];
                                            }
                                            if (is_null($vn_max) || ($va_ts['end'] > $vn_max)) {
                                                $va_facet_info['maximum_date'] = $o_tep->getText(
                                                    ['end_as_iso8601' => true]
                                                );
                                                $vn_max = $va_ts['end'];
                                            }
                        ...
                        while ($qr_res->nextRow()) {
                            ...
                            foreach ($va_normalized_values as $vn_sort_value => $vs_normalized_value) {
                                ...
                                if (!isset($va_values[$vn_sort_value][$vs_normalized_value])) {
                                    ...
                                    $va_values[$vn_sort_value][$vs_normalized_value]['content_count'] += $qr_res->get(
                                        '_count'
                                    );
                                }
                        ...
                        while ($qr_res->nextRow()) {
                            ...
                            foreach ($va_normalized_values as $vn_sort_value => $vs_normalized_value) {
                                ...
                                if (!isset($va_values[$vn_sort_value][$vs_normalized_value])) {
                                    ...
                                    $va_values[$vn_sort_value][$vs_normalized_value]['content_count'] += (int)$qr_res->get(
                                        '_count'
                                    );
                                }
                                if (!is_null($vs_single_value) && ($vs_normalized_value == $vs_single_value)) {
            ...
            case 'normalizedLength':
                ...
                $vs_min_sql = $vs_max_sql = '';
                $vo_minimum_dimension = caParseLengthDimension(
                    caGetOption('minimum_dimension', $va_facet_info, "0 in")
                );
                $vo_maximum_dimension = caParseLengthDimension(
                    caGetOption('maximum_dimension', $va_facet_info, "0 in")
                );
                ...
                if ($vb_check_availability_only) {
                    ...
                    while ($qr_res->nextRow()) {
                        ...
                        $vn_normalized = (floor(
                                $vn_dim / $vn_increment_in_current_units
            ...
            case 'normalizedWeight':
                ...
                $vs_min_sql = $vs_max_sql = '';
                $vo_minimum_dimension = caParseWeightDimension(
                    caGetOption('minimum_dimension', $va_facet_info, "0 kg")
                );
                $vo_maximum_dimension = caParseWeightDimension(
                    caGetOption('maximum_dimension', $va_facet_info, "0 kg")
                );
                if ($vo_minimum_dimension) {
            ...
            case 'relationship_types':
                ...
                $va_exclude_types_expanded = $this->_convertTypeCodesToIDs(
                    $va_exclude_types,
                    array('instance' => $t_rel_item)
                );
                ...
                if (!$va_facet_info['show_all_when_first_facet'] || ($this->numCriteria() > 0)) {
                    if ($browse_on_self_relation) {
                        $va_joins[] = 'INNER JOIN ' . $va_path[1] . ' ON (' . $va_path[0] . '.' . $t_subject->primaryKey(
                            ) . ' = ' . $va_path[1] . '.' . $t_item_rel->getRightTableFieldName(
                            ) . ' OR ' . $va_path[0] . '.' . $t_subject->primaryKey(
                            ) . ' = ' . $va_path[1] . '.' . $t_item_rel->getLeftTableFieldName() . ")\n";
                        $va_joins[] = 'INNER JOIN ' . $va_path[2] . ' AS ' . $va_path[2] . '_rel ON (' . $va_path[2] . '_rel.' . $t_subject->primaryKey(
                            ) . ' = ' . $va_path[1] . '.' . $t_item_rel->getLeftTableFieldName(
                            ) . ') OR (' . $va_path[2] . '_rel.' . $t_subject->primaryKey(
                            ) . ' = ' . $va_path[1] . '.' . $t_item_rel->getRightTableFieldName() . ")\n";
                    } else {
                        $vs_cur_table = array_shift($va_path);
                ...
                if (is_array($va_restrict_to_lists) && (sizeof(
                            $va_restrict_to_lists
                        ) > 0) && ($vs_rel_item_table_name == 'ca_list_items')) {
                    $va_list_ids = array();
                ...
                if (is_array($va_exclude_types) && (sizeof($va_exclude_types) > 0) && method_exists(
                        $t_rel_item,
                        "getTypeList"
                    )) {
                    $va_wheres[] = "{$vs_rel_item_table_name}.type_id NOT IN (" . join(
                            ...
                            caGetOption(
                                ...
                                $va_facet_info,
                                false
                            ) ? $va_exclude_types : $va_exclude_types_expanded
                        ) . ")";
                }
                ...
                if (isset($va_facet_info['groupings']) && is_array($va_facet_info['groupings']) && sizeof(
                        $va_facet_info['groupings']
                    )) {
                ...
                if ($this->opo_config->get('perform_item_level_access_checking')) {
                    if ($t_item = Datamodel::getInstanceByTableName($vs_browse_table_name, true)) {
                        ...
                        $va_joins[] = 'LEFT JOIN ca_acl AS rel_acl ON ' . $vs_rel_item_table_name . '.' . $t_rel_item->primaryKey(
                            ) . ' = rel_acl.row_id AND rel_acl.table_num = ' . $t_rel_item->tableNum() . "\n";
                        $va_wheres[] = "(
								((
                ...
                if (is_array($va_criteria) && sizeof($va_criteria) > 0) {
                    $va_wheres[] = "(" . $vs_rel_item_table_name . "." . $t_rel_item->primaryKey() . " NOT IN (" . join(
                            ",",
                            caQuoteList(array_map("intval", array_keys($va_criteria)))
                        ) . "))";
                }
                ...
                if ($vb_check_availability_only) {
                    ...
                    while ($qr_res->nextRow()) {
                        ...
                        if ($va_facet_info['type'] === 'relationship_types') {
                            if (sizeof($va_restrict_to_relationship_types) && !in_array(
                                    $va_fetched_row['rel_type_id'],
                                    $va_restrict_to_relationship_types
                                )) {
                                continue;
                            }
                            if (sizeof($va_exclude_relationship_types) && in_array(
                                    $va_fetched_row['rel_type_id'],
                                    $va_exclude_relationship_types
                                )) {
                                continue;
...
// 								}
                            ...
                            if ($checkAccess && isset($va_fetched_row['access']) && !in_array(
                                    (int)$va_fetched_row['access'],
                                    $checkAccess,
                                    true
                                )) {
                    ...
                    if (!isset($va_facet_info['dont_expand_hierarchically']) || !$va_facet_info['dont_expand_hierarchically']) {
                        ...
                        if ($qr_ancestors) {
                            ...
                            while ($qr_ancestors->nextHit()) {
                                ...
                                if (is_array($va_restrict_to_types) && (sizeof($va_restrict_to_types) > 0) && !in_array(
                                        $vn_parent_type_id,
                                        $va_restrict_to_types
                                    )) {
                                    continue;
                                ...
                                $ancestor_count = array_reduce(
                                    $children,
                                    function ($c, $i) use ($va_facet_items) {
                                ...
                                $va_facet_items_ancestors[$vn_ancestor_id] = array(
                                    ...
                                    'parent_id' => $vb_rel_is_hierarchical ? $qr_ancestors->get(
                                        "{$vs_hier_parent_id_fld}"
                                    ) : null,
                                    'hierarchy_id' => ($vb_rel_is_hierarchical && $vs_hier_id_fld) ? $qr_ancestors->get(
                                        $vs_hier_id_fld
                                    ) : null,
                                    'rel_type_id' => array(),
                    ...
                    if (sizeof($va_row_ids = array_keys($va_facet_items))) {
                        if ($vs_label_table_name = $t_rel_item->getLabelTableName()) {
                            ...
                            $va_label_wheres[] = "({$vs_label_table_name}.{$vs_rel_pk} IN (" . join(
                                    ",",
                                    $va_row_ids
                                ) . "))";
                            ...
                            $vs_sql = "
									...
									INNER JOIN " . $vs_rel_table_name . " AS {$vs_rel_item_table_name} ON " . $vs_rel_item_table_name . ".{$vs_rel_pk} = {$vs_label_table_name}.{$vs_rel_pk}
										" . (sizeof($va_label_wheres) ? ' WHERE ' : '') . join(
                                    " AND ",
                                    $va_label_wheres
                                ) . "
										" . (sizeof($va_orderbys) ? "ORDER BY " . join(', ', $va_orderbys) : '') . "";
                            ...
                            while ($qr_labels->nextRow()) {
                                ...
                                if ($natural_sort) {
                                    $label_values['label_sort_'] = caSortableValue(
                                        $va_fetched_row[$vs_label_display_field]
                                    );
                                }

                                $va_facet_item = array_merge(
                                    $va_facet_items[$va_fetched_row[$vs_rel_pk]],
                                    $label_values
                                );
    ...
    protected function doGetResults($po_result = null, $pa_options = null)
    {
        ...
        $vs_sort_direction = strtolower(
            caGetOption('sortDirection', $pa_options, caGetOption('sort_direction', $pa_options, null))
        );

        $t_item = Datamodel::getInstanceByTableName($this->ops_browse_table_name, true);
        $vb_will_sort = ($vs_sort && (($this->getCachedSortSetting(
                    ) != $vs_sort) || ($this->getCachedSortDirectionSetting() != $vs_sort_direction)));
        ...
        if ($po_result) {
            $po_result->init(
                new WLPlugSearchEngineBrowseEngine($va_results, $this->opn_browse_table_num),
                array(),
                $pa_options
            );
    ...
    public function getBrowseSubjectName($ps_subject_table_name = null)
    {
        ...
        if (is_array(
                $va_tmp = $this->opo_ca_browse_config->getAssoc($ps_subject_table_name)
    ...
    {
        $this->opa_browse_type_ids = $this->_convertTypeCodesToIDs(
            $pa_type_codes_or_ids,
            array(
                'dontExpandHierarchically' => caGetOption(
                    'dontExpandHierarchically',
                    $pa_options,
                    false
                )
    ...
    public function getTypeRestrictionList()
    {
        if (function_exists("caGetTypeRestrictionsForUser")) {
            $va_pervasive_types = caGetTypeRestrictionsForUser(
                $this->ops_tablename
    ...
    public function getSourceRestrictionList()
    {
        if (function_exists("caGetSourceRestrictionsForUser")) {
            $va_pervasive_sources = caGetSourceRestrictionsForUser(
                $this->ops_tablename
            );    // restrictions set in app.conf or by associated user role
    ...
    private function _getRelativeExecuteSQLData($ps_relative_to_table, $pa_options = null)
    {
        ...
        if ($t_item_rel) { // foo_x_bar Table exists ==> join foo_x_bar and relative_to table
            $va_relative_to_join[] = "INNER JOIN " . $t_item_rel->tableName() . " ON " . $t_item_rel->tableName(
                ) . "." . $t_item->primaryKey() . " = " . $this->ops_browse_table_name . '.' . $t_item->primaryKey();
            $va_relative_to_join[] = "INNER JOIN {$ps_relative_to_table} ON {$ps_relative_to_table}.{$vs_target_browse_table_pk} = " . $t_item_rel->tableName(
                ) . "." . $t_target->primaryKey();
