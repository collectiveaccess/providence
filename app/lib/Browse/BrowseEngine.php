...
class BrowseEngine extends BaseFindEngine {
	...
	private function _processBrowseSettings() {
		...
		foreach ( $this->opa_browse_settings['facets'] as $vs_facet_name => $va_facet_info ) {
			...
			if ( ( in_array( $va_facet_info['type'], [ 'location', 'current_value' ] ) )
			     && ( caGetOption( 'group_mode', $va_facet_info, null ) == 'hierarchical' )
			) {
				$subject_table = $this->ops_browse_table_name;
				$policy        = caGetOption( 'policy', $va_facet_info,
					$subject_table::getDefaultHistoryTrackingCurrentValuePolicyForTable( $subject_table ) );

				$policy_info = $subject_table::getHistoryTrackingCurrentValuePolicy( $policy );

				if ( is_array( $policy_info['elements'] ) && ( sizeof( $policy_info['elements'] ) == 1 )
				     && isset( $policy_info['elements']['ca_storage_locations'] )
				) {
			...
			if ( ( $va_facet_info['type'] === 'authority' ) && isset( $va_facet_info['generate_facets_for_types'] )
			     && $va_facet_info['generate_facets_for_types']
			) {
				...
				foreach ( $va_type_list as $vn_type_id => $va_type_info ) {
					if ( $va_type_info['is_enabled'] ) {
						$va_facet_info                                           = array_merge( $va_facet_info, array(
							...
							'label_singular_with_indefinite_article' => _t( 'a' ) . ' '
							                                            . $va_type_info['name_singular'],
		...
		foreach ( $va_revised_facets as $vs_facet => $va_facet_info ) {
			...
			switch ( $va_facet_info['type'] ) {
				case 'attribute':
					...
					if ( $t_element->load( array(
						'element_code' => array_pop( explode( ".", $va_facet_info['element_code'] ) )
					) )
					) {
				...
				case 'fieldList':
					...
					if ( $vn_item_id = caGetListItemID( $t_instance->getFieldInfo( $va_facet_info['field'],
						'LIST_CODE' ), $va_facet_info['single_value'] )
					) {
						$va_revised_facets[ $vs_facet ]['single_value'] = $vn_item_id;
					}
					break;
	...
	public function addCriteria( $ps_facet_name, $pa_row_ids, $pa_display_strings = null ) {
		...
		foreach ( $pa_row_ids as $vn_i => $vn_row_id ) {
			$vn_row_id                                   = str_replace( "&amp;", "&",
				$purifier->purify( rawurldecode( $vn_row_id ) ) ); // sanitize facet values
			$va_criteria[ $ps_facet_name ][ $vn_row_id ] = true;

			if ( isset( $pa_display_strings[ $vn_i ] ) ) {
				$va_criteria_display_strings[ $ps_facet_name ][ urldecode( $vn_row_id ) ]
					= $pa_display_strings[ $vn_i ];
			}
		}
	 ...
	 * @param array  $pa_row_ids    One or more facet values applied as criteria. Values that are not current criteria
	 *                              are ignored.
	 *
	 * @return boolean - true on success, false on error
	 */
		...
		if ( $ps_facet_name ) {
			if ( is_array( $va_criteria_display_strings[ $ps_facet_name ] ) ) {
				foreach ( $va_criteria_display_strings[ $ps_facet_name ] as $vm_criterion => $vs_display_criterion ) {
					$va_criteria_with_labels[ $vm_criterion ] = $this->getCriterionLabel( $ps_facet_name,
						$vs_display_criterion );
				}
			...
			if ( is_array( $va_criteria ) ) {
				foreach ( $va_criteria as $vs_facet_name => $va_criteria_by_facet ) {
					if ( is_array( $va_criteria_display_strings[ $vs_facet_name ] ) ) {
						foreach (
							$va_criteria_display_strings[ $vs_facet_name ] as $vm_criterion => $vs_display_criterion
						) {
							$va_criteria_with_labels[ $vs_facet_name ][ $vm_criterion ]
								= $this->getCriterionLabel( $vs_facet_name, $vs_display_criterion );
						}
					} else {
						foreach ( $va_criteria_by_facet as $vm_criterion => $vn_tmp ) {
							$va_criteria_with_labels[ $vs_facet_name ][ $vm_criterion ]
								= $this->getCriterionLabel( $vs_facet_name, $vm_criterion );
						}
					}
				}
			}
	...
	public function getCriterionLabel( $ps_facet_name, $pn_row_id ) {
		...
		switch ( $va_facet_info['type'] ) {
			# -----------------------------------------------------
			case 'has':
				$vs_yes_text = ( isset( $va_facet_info['label_yes'] ) && $va_facet_info['label_yes'] )
					? $va_facet_info['label_yes'] : _t( 'Yes' );
				$vs_no_text  = ( isset( $va_facet_info['label_no'] ) && $va_facet_info['label_no'] )
					? $va_facet_info['label_no'] : _t( 'No' );

				return ( (bool) $pn_row_id ) ? $vs_yes_text : $vs_no_text;
				break;
			# -----------------------------------------------------
			case 'label':
				if ( ! ( $t_table = Datamodel::getInstanceByTableName( ( isset( $va_facet_info['relative_to'] )
				                                                         && $va_facet_info['relative_to'] )
					? $va_facet_info['relative_to'] : $this->ops_browse_table_name, true ) )
				) {
					break;
				}
			...
			case 'attribute':
				...
				switch ( $vn_element_type = $t_element->get( 'datatype' ) ) {
					case __CA_ATTRIBUTE_VALUE_LIST__:
						$vs_label = caProcessTemplateForIDs( "^ca_list_items.hierarchy.preferred_labels.name_plural",
							"ca_list_items", array( $pn_row_id ), array( "delimiter" => " ➜ " ) );

						if ( is_array( $va_facet_info['relabel'] )
						     && isset( $va_facet_info['relabel'][ $vs_label ] )
						) {
			...
			# -----------------------------------------------------
			case 'field':
				if ( ! ( $t_item = Datamodel::getInstanceByTableName( $va_facet_info['relative_to']
					? $va_facet_info['relative_to'] : $this->ops_browse_table_name, true ) )
				) {
					break;
				}
				if ( $vb_is_bit = ( $t_item->getFieldInfo( $va_facet_info['field'], 'FIELD_TYPE' ) == FT_BIT ) ) {
					return ( (bool) $pn_row_id ) ? caGetOption( 'label_yes', $va_facet_info, _t( 'Yes' ) )
						: caGetOption( 'label_no', $va_facet_info, _t( 'No' ) );
				}
			...
			case 'checkouts':
				$vs_status_text = null;
				$vs_status_code = ( isset( $va_facet_info['status'] ) && $va_facet_info['status'] )
					? $va_facet_info['status'] : $pn_row_id;
				...
				switch ( $va_facet_info['mode'] ) {
					case 'user':
						...
						if ( $t_user->getPrimaryKey() ) {
							$vs_name = $t_user->get( 'fname' ) . ' ' . $t_user->get( 'lname' ) . ( ( $vs_email
									= $t_user->get( 'email' ) ) ? " ({$vs_email})" : "" );
			...
			case 'current_value':
				...
				} elseif ( $va_row_tmp[2] && ( $qr_res = caMakeSearchResult( $table, [ $va_row_tmp[2] ] ) )
				           && $qr_res->nextHit()
				) {
					// Return label for id
					$cv_config = ca_objects::getConfigurationForHistoryTrackingCurrentValue( $policy, $table,
						$va_row_tmp[1] );

					$display_config = $this->getTableEntryFromMap( $va_facet_info, 'display', $table );
					$template       = isset( $display_config['template'] ) ? $display_config['template']
						: ( isset( $cv_config['template'] ) ? $cv_config['template'] : "^{$table}.preferred_labels" );
					$template       = str_replace( "<l>", "",
						str_replace( "</l>", "", $template ) );    // don't allow links in criterion labels
			...
			# -----------------------------------------------------
			case 'inHomeLocation':
				return $pn_row_id ? caGetOption( 'label_yes', $va_facet_info, _t( 'In home location' ) )
					: caGetOption( 'label_no', $va_facet_info, _t( 'Not in home location' ) );
				break;
			# -----------------------------------------------------
			case 'normalizedLength':
				$vn_start = urldecode( $pn_row_id );
				if ( ! ( $vs_output_units = caGetLengthUnitType(
					$vs_units = caGetOption( 'units', $va_facet_info, 'm' ) ) )
				) {
			...
			case 'normalizedWeight':
				...
				if ( ! ( $vs_output_units = caGetWeightUnitType(
					$vs_units = caGetOption( 'units', $va_facet_info, 'g' ) ) )
				) {
			...
			case 'fieldList':
				...
				if ( $va_facet_info['table']
				     && ( $t_browse_table = Datamodel::getInstanceByTableName(
						$vs_facet_table = $va_facet_info['table'], true ) )
				) {
					if ( ! ( $app = AppController::getInstance() ) ) {
						return '???';
					}
					if ( $t_browse_table->load( $pn_row_id )
					     && $t_browse_table->isReadable( $app->getRequest(), 'preferred_labels' )
					) {

						return $t_browse_table->getWithTemplate( isset( $va_facet_info['display'] )
							? $va_facet_info['display'] : "^" . $t_browse_table->tableName() . ".preferred_labels" );
					}
				}
	...
	public function getFacetList() {
		...
		if ( is_array( $va_type_restrictions ) && sizeof( $va_type_restrictions ) ) {
			$va_facets = array();
			foreach ( $this->opa_browse_settings['facets'] as $vs_facet_name => $va_facet_info ) {
				if ( in_array( $vs_facet_name, $va_criteria_facets )
				     && ( caGetOption( 'type', $va_facet_info, null ) == 'field' )
				) {
					continue;
				}    // fields can only appear once
				if ( isset( $va_facet_info['requires'] ) && ! is_array( $va_facet_info['requires'] )
				     && $va_facet_info['requires']
				) {
				...
				if ( $vb_facet_is_meets_requirements ) {
					if ( isset( $va_facet_info['type_restrictions'] )
					     && is_array( $va_facet_restrictions = $va_facet_info['type_restrictions'] )
					     && sizeof( $va_facet_restrictions )
					) {
	...
	public function getInfoForAvailableFacets() {
		...
		foreach ( $va_facets as $vs_facet_name => $va_facet_info ) {
			if ( $vs_facet_group ) {
				if ( ! ( isset( $va_facet_info['facet_groups'] ) && is_array( $va_facet_info['facet_groups'] )
				         && in_array( $vs_facet_group, $va_facet_info['facet_groups'] ) )
				) {
	...
	public function getInfoForFacetsWithContent() {
		...
		if ( is_array( $va_facets_with_content ) ) {
			...
			foreach ( $va_facets_with_content as $vs_facet ) {
				if ( ( $vs_facet_group && $va_facets[ $vs_facet ]['facet_groups']
				       && is_array( $va_facets[ $vs_facet ]['facet_groups'] ) )
				     && ( ! in_array( $vs_facet_group, $va_facets[ $vs_facet ]['facet_groups'] ) )
				) {
					continue;
				}
	 ...
	 *                          showDeleted = if set to true, related items that have been deleted are returned.
	 *                          Default is false. limitToModifiedOn = if set returned results will be limited to rows
	 *                          modified within the specified date range. The value should be a date/time expression
	 *                          parse-able by TimeExpressionParser user_id = If set item level access control is
	 *                          performed relative to specified user_id, otherwise defaults to logged in user
	 *                          expandResultsHierarchically = expand result set items that are hierarchy roots to
	 *                          include their entire hierarchy. [Default is false] omitChildRecords = only return
	 *                          records that are the root of whatever hierarchy they are in. [Default is false]
	 *                          rootRecordsOnly = Synonym for omitChildRecords. omitChildRecordsForTypes = List of
	 *                          types for which to return only records that are not children [Default is null]
	 *
	...
	public function execute( $pa_options = null ) {
		...
		if ( is_array( $omit_child_records_for_types = caGetOption( 'omitChildRecordsForTypes', $pa_options, null ) )
		     && sizeof( $omit_child_records_for_types )
		) {
			$omit_child_records_for_types   = caMakeTypeIDList( $this->opn_browse_table_num,
		...
		$vb_no_cache = caGetOption( 'noCache', $pa_options,
			caGetOption( 'no_cache', $pa_options, false, array( 'castTo' => 'bool' ) ), array( 'castTo' => 'bool' ) );
		...
		if ( $this->opo_ca_browse_cache->load( $vs_cache_key ) ) {
			$vn_created_on
				              = $this->opo_ca_browse_cache->getParameter( 'created_on' ); //$t_new_browse->get('created_on', array('getDirectDate' => true));
		...
		if ( is_array( $va_criteria ) && ( sizeof( $va_criteria ) > 0 ) ) {
			if ( ! $vb_results_cached ) {
				...
				foreach ( $va_criteria as $vs_facet_name => $va_row_ids ) {
					...
					$vb_is_relative_to_parent = ( $va_facet_info['relative_to']
					                              && $this->_isParentRelative( $va_facet_info['relative_to'] ) );
					...
					switch ( $va_facet_info['type'] ) {
						...
						case 'has':
							...
							if ( $va_facet_info['relative_to'] ) {
								if ( $va_relative_execute_sql_data
									= $this->_getRelativeExecuteSQLData( $va_facet_info['relative_to'], $pa_options )
								) {
									$va_joins = array_merge( $va_joins,
										$va_relative_execute_sql_data['relative_joins'] );
							...
							if ( isset( $va_facet_info['element_code'] ) && $t_user && $t_user->isLoaded()
							     && ( $t_user->getBundleAccessLevel( $this->ops_browse_table_name,
										array_shift( explode( '.', $va_facet_info['element_code'] ) ) )
							          < __CA_BUNDLE_ACCESS_READONLY__ )
							) {
								break;
							} elseif ( ( isset( $va_facet_info['table'] ) && $t_user && $t_user->isLoaded()
							             && ( $t_user->getBundleAccessLevel( $this->ops_browse_table_name,
										$va_facet_info['table'] ) < __CA_BUNDLE_ACCESS_READONLY__ ) )
							) {
								break;
							}
							...
							if ( $va_facet_info['element_code'] ) {
								...
								if ( $vn_state == 0 ) {
									$va_wheres[] = $this->ops_browse_table_name . '.' . $t_item->primaryKey()
									               . " NOT IN (SELECT row_id FROM ca_attributes WHERE table_num = "
									               . $t_item->tableNum() . " AND element_id = "
									               . $t_element->getPrimaryKey() . ")";
								} else {
									$va_joins[]  = "INNER JOIN ca_attributes AS caa ON caa.row_id = "
									               . $this->ops_browse_table_name . '.' . $t_item->primaryKey()
									               . " AND caa.table_num = " . $t_item->tableNum();
									$va_wheres[] = "caa.element_id = " . $t_element->getPrimaryKey();
								}
							} else {
								if ( ! is_array( $va_restrict_to_relationship_types
									= $va_facet_info['restrict_to_relationship_types'] )
								) {
									$va_restrict_to_relationship_types = array();
								}
								$va_restrict_to_relationship_types
									= $this->_getRelationshipTypeIDs( $va_restrict_to_relationship_types,
									$va_facet_info['relationship_table'] );

								if ( ! is_array(
									$va_exclude_relationship_types = $va_facet_info['exclude_relationship_types'] )
								) {
									$va_exclude_relationship_types = array();
								}
								$va_exclude_relationship_types
									= $this->_getRelationshipTypeIDs( $va_exclude_relationship_types,
									$va_facet_info['relationship_table'] );
								...
								switch ( sizeof(
									$va_path = array_keys( Datamodel::getPath( $vs_target_browse_table_name,
										$vs_rel_table_name ) ) ) ) {
									case 3:
								...
								if ( ! is_array(
									$va_restrict_to_types = $this->_convertTypeCodesToIDs( $va_restrict_to_types,
										array( 'instance' => $t_rel_item ) ) )
								) {
								...
								if ( ! is_array( $va_exclude_types = $this->_convertTypeCodesToIDs( $va_exclude_types,
									array( 'instance' => $t_rel_item ) ) )
								) {
									$va_exclude_types = [];
								}
								...
								foreach ( $va_path as $vs_join_table ) {
									...
									$va_joins[]   = ( $vn_state ? 'INNER' : 'LEFT' ) . ' JOIN ' . $vs_join_table
									                . ' ON ' . $vs_cur_table . '.'
									                . $va_rel_info[ $vs_cur_table ][ $vs_join_table ][0][0] . ' = '
									                . $vs_join_table . '.'
								...
								$va_wheres = array();
								if ( is_array( $va_restrict_to_relationship_types )
								     && ( sizeof( $va_restrict_to_relationship_types ) > 0 )
								     && is_object( $t_item_rel )
								     && (bool) $vn_state
								) {
									$va_wheres[] = "(" . $t_item_rel->tableName() . ".type_id IN (" . join( ',',
											$va_restrict_to_relationship_types ) . "))";
								}
								if ( is_array( $va_exclude_relationship_types )
								     && ( sizeof( $va_exclude_relationship_types ) > 0 )
								     && is_object( $t_item_rel )
								     && (bool) $vn_state
								) {
									$va_wheres[] = "(" . $t_item_rel->tableName() . ".type_id NOT IN (" . join( ',',
											$va_exclude_relationship_types ) . "))";
								}

								// yes option
								$va_wheres[] = "(" . $t_rel_item->tableName() . "." . $t_rel_item->primaryKey()
								               . " IS NOT NULL)";
								...
								if ( isset( $pa_options['checkAccess'] ) && is_array( $pa_options['checkAccess'] )
									...
									$va_wheres[] = "(" . $t_rel_item->tableName() . ".access IN (" . join( ',',
											$pa_options['checkAccess'] ) . "))";
								}

								if ( is_array( $va_restrict_to_types ) && ( sizeof( $va_restrict_to_types ) > 0 )
								     && is_object( $t_rel_item )
								) {
									$va_wheres[] = "(" . $t_rel_item->tableName() . ".type_id IN (" . join( ',',
											$va_restrict_to_types ) . "))";
								}
								if ( is_array( $va_exclude_types ) && ( sizeof( $va_exclude_types ) > 0 )
								     && is_object( $t_rel_item )
								) {
									$va_wheres[] = "((" . $t_rel_item->tableName() . ".type_id NOT IN (" . join( ',',
											$va_exclude_types ) . ")) OR (" . $t_rel_item->tableName()
									               . ".type_id IS NULL))";
								}

								if ( is_array( $va_restrict_to_lists ) && sizeof( $va_restrict_to_lists ) ) {
									$va_wheres[] = "(" . $t_rel_item->tableName() . ".list_id IN (" . join( ',',
											$va_restrict_to_lists ) . "))";
								}
							}
							...
							if ( isset( $pa_options['checkAccess'] ) && is_array( $pa_options['checkAccess'] )
							     && sizeof( $pa_options['checkAccess'] )
							     && $t_item->hasField( 'access' )
							) {
								$va_wheres[] = "(" . $t_item->tableName() . ".access IN (" . join( ',',
										$pa_options['checkAccess'] ) . "))";
							}


							if ( $t_item_rel && isset( $va_facet_info['filter_on_interstitial'] )
							     && is_array( $va_facet_info['filter_on_interstitial'] )
							     && sizeof( $va_facet_info['filter_on_interstitial'] )
							) {
								foreach ( $va_facet_info['filter_on_interstitial'] as $vs_field_name => $va_values ) {
									...
									if ( ! ( $vn_element_id
										= (int) ca_metadata_elements::getElementID( $vs_field_name ) )
									) {
										continue;
									}
									if ( ! ( $o_value
										= Attribute::getValueInstance( ca_metadata_elements::getElementDatatype( $vs_field_name ) ) )
									) {
										continue;
									}

									$va_element_value = $o_value->parseValue( $va_values[0],
										array_merge( ca_metadata_elements::getElementSettingsForId( $vs_field_name ),
											[ 'list_id' => ca_metadata_elements::getElementListID( $vs_field_name ) ],
											[ 'matchOn' => [ 'idno' ] ] ) );

									$va_joins[]  = "INNER JOIN ca_attributes c_a ON c_a.row_id = "
									               . $t_item_rel->primaryKey( true ) . " AND c_a.table_num = "
									               . $t_item_rel->tableNum();
									$va_joins[]
									             = "INNER JOIN ca_attribute_values c_av ON c_a.attribute_id = c_av.attribute_id";
									$va_wheres[] = "c_av.element_id = {$vn_element_id} AND "
									               . ( isset( $va_element_value['item_id'] ) ? "c_av.item_id = ?"
											: "c_av.value_longtext1 = ?" );

									$va_sql_params[] = ( isset( $va_element_value['item_id'] ) )
										? (int) $va_element_value['item_id'] : $va_element_value['value_longtext1'];
								}
							}
							...
							if ( $vn_state ) {
								...
								if ( $va_facet_info['element_code'] ) {
									$vs_sql = "
                                                    SELECT " . $this->ops_browse_table_name . '.'
									          . $t_item->primaryKey() . "
                                                    FROM " . $this->ops_browse_table_name . "
                                                    {$vs_relative_to_join}
                                                    {$vs_join_sql}
							...
							$va_acc[ $vn_i ] = $qr_res->getAllFieldValues( $this->ops_browse_table_name . '.'
							                                               . $t_item->primaryKey() );
							$vn_i ++;
							break;
						# -----------------------------------------------------
						case 'label':
							...
							$wheres = $params = [];
							if ( is_array(
								$sort_fields = caGetOption( 'order_by_label_fields', $va_facet_info, null ) )
							) {
							...
							if ( $va_facet_info['relative_to'] ) {
								if ( $va_relative_execute_sql_data
									...
									$t_target       = Datamodel::getInstanceByTableName( $va_facet_info['relative_to'],
										true );
									...
									$va_relative_to_join[]
									                     = "INNER JOIN {$vs_label_table_name} ON {$vs_label_table_name}.{$vs_label_item_pk} = {$vs_target_browse_table_name}.{$vs_target_browse_table_pk}";
								}
							} else {
								$va_relative_to_join
									= array( "INNER JOIN {$vs_label_table_name} ON {$vs_label_table_name}.{$vs_label_item_pk} = {$vs_target_browse_table_name}.{$vs_target_browse_table_pk}" );
							}

							if ( $va_facet_info['relationship'] ) {
								$va_relative_to_join[] = "INNER JOIN " . $va_facet_info['relationship']
								                         . " ON {$vs_target_browse_table_name}.{$vs_target_browse_table_pk} = "
								                         . $va_facet_info['relationship']
								                         . ".{$vs_target_browse_table_pk}";
								if ( is_array( $va_facet_info['restrict_to_relationship_types'] )
								     && ( $rel_type_ids
										= caMakeRelationshipTypeIDList( $va_facet_info['relationship'],
										$va_facet_info['restrict_to_relationship_types'] ) )
								     && sizeof( $rel_type_ids )
								) {
							...
							foreach ( $va_row_ids as $vn_row_id ) {
								...
								$va_acc[ $vn_i ] = array_merge( $va_acc[ $vn_i ],
									$qr_res->getAllFieldValues( $this->ops_browse_table_name . '.'
									                            . $t_item->primaryKey() ) );
						...
						case 'field':
							...
							$bundle_name = ( in_array( $vs_field_name, [
								'is_deaccessioned',
								'deaccession_date',
								'deaccession_notes',
								'deaccession_type_id',
								'deaccession_disposal_date'
							] ) ) ? 'ca_objects_deaccession' : $vs_field_name;
							if ( $t_user && $t_user->isLoaded()
							     && ( $t_user->getBundleAccessLevel( $vs_table_name, $bundle_name )
							          < __CA_BUNDLE_ACCESS_READONLY__ )
							) {
								break;
							}
						...
						case 'attribute':
							...
							if ( $t_user->getBundleAccessLevel( $this->ops_browse_table_name,
									$va_facet_info['element_code'] ) < __CA_BUNDLE_ACCESS_READONLY__
							) {
								break;
							}
							...
							foreach ( $va_row_ids as $vn_row_id ) {
								...
								if ( is_array( $va_restrict_to_types ) && ( sizeof( $va_restrict_to_types ) > 0 )
								     && method_exists( $t_item, "getTypeList" )
								) {
									$va_attr_sql[] = "(" . $this->ops_browse_table_name . ".type_id IN (" . join( ", ",
											$va_restrict_to_types ) . "))";
								}
									...
									foreach ( $va_value as $vs_f => $vs_v ) {
										switch ( $vn_datatype ) {
											case __CA_ATTRIBUTE_VALUE_LIST__:
												...
												$va_item_ids = $t_list_item->getHierarchy( (int) $vs_v,
											...
											case __CA_ATTRIBUTE_VALUE_LCSH__:
												...
												break;
											default:
												$va_attr_sql[]    = "(ca_attribute_values.{$vs_f} " . ( is_null( $vs_v )
														? " IS " : " = " ) . " ?)";
												$va_attr_values[] = $vs_v;
												break;
								...
								if ( is_array( $va_element_code ) && ( sizeof( $va_element_code ) == 1 )
								     && is_array( $va_container_ids[ $va_element_code[0] ] )
								     && sizeof( $va_container_ids[ $va_element_code[0] ] )
								) {
								...
								$vs_sql = "
											...
											FROM " . $this->ops_browse_table_name . "
											{$vs_relative_to_join}
											INNER JOIN ca_attributes ON ca_attributes.row_id = "
								          . ( ( ! $vb_is_relative_to_parent
										? "{$vs_target_browse_table_name}.{$vs_target_browse_table_pk}"
										: "parent.{$vs_target_browse_table_pk}" ) ) . " AND ca_attributes.table_num = ?
								...
								if ( is_array( $va_element_code ) && sizeof( $va_element_code ) == 1 ) {
									...
									$va_container_ids[ $va_element_code[0] ]
										= array_unique( $qr_res->getAllFieldValues( 'attribute_id' ) );
									$this->opo_ca_browse_cache->setParameter( 'container_ids',
										$va_container_ids[ $va_element_code[0] ] );
								}
								if ( ! caGetOption( 'multiple', $va_facet_info, false ) ) {
									$vn_i ++;
						...
						case 'normalizedDates':
							...
							if ( ! ( $vb_is_element
									= $t_element->load( array( 'element_code' => array_pop( $va_element_code ) ) ) )
							     && ! ( $vb_is_field = ( $t_item->hasField( $va_facet_info['element_code'] )
							                             && ( $t_item->getFieldInfo( $va_facet_info['element_code'],
							...
							foreach ( $va_row_ids as $vn_row_id ) {
								...
								if ( $vn_row_id !== 'null' ) {
									...
									if ( substr( $tmp[1], 0, 10 )
									     == '0101000000'
									) { // rewrite start date to encompass circa dates
								...
								if (
									...
									&& ( $va_facet_info['treat_before_dates_as_circa']
									     || $va_facet_info['treat_after_dates_as_circa'] )
								) {
								...
								if ( $vb_is_element ) {
									...
									if ( is_null( $va_dates ) ) {
										...
										$vs_sql = "
										          ...
										          . $t_item->primaryKey() . "
													...
													{$vs_relative_to_join}
													INNER JOIN ca_attributes ON ca_attributes.row_id = "
										          . $vs_target_browse_table_name . '.' . $vs_target_browse_table_pk . " AND ca_attributes.table_num = ?
													INNER JOIN ca_attribute_values ON ca_attribute_values.attribute_id = ca_attributes.attribute_id
													WHERE
										...
										$va_params = [
											intval( $vs_target_browse_table_num ),
											$vn_element_id,
											$va_dates['start'],
											$va_dates['end'],
											$va_dates['start'],
										...
										$vs_sql = "
													SELECT " . $this->ops_browse_table_name . '.'
										          . $t_item->primaryKey() . ", ca_attributes.attribute_id
													FROM " . $this->ops_browse_table_name . "
													{$vs_relative_to_join}
													INNER JOIN ca_attributes ON ca_attributes.row_id = "
										          . $vs_target_browse_table_name . '.' . $vs_target_browse_table_pk . " AND ca_attributes.table_num = ?
													...
													WHERE
														...
														(
															(
																...
																(ca_attribute_values.value_decimal1 <> "
										          . TEP_START_OF_UNIVERSE . ".0000000000) AND
																(ca_attribute_values.value_decimal2 <> "
										          . TEP_END_OF_UNIVERSE . ".1231235959) 
															)
															OR
									...
									if ( is_null( $va_dates ) ) {
										...
										$vs_sql = "
													...
													WHERE
														(
															(
																...
																({$this->ops_browse_table_name}.{$vs_browse_start_fld} <> "
										          . TEP_START_OF_UNIVERSE . ") AND
												...
												";
										$qr_res = $this->opo_db->query( $vs_sql, $va_dates['start'], $va_dates['end'],
											$va_dates['start'], $va_dates['end'], $va_dates['start'],
											$va_dates['end'] );
									}
								}
								...
								if ( $vb_is_element && is_array( $va_element_code )
								     && ( sizeof( $va_element_code ) == 1 )
								) {
									// is sub-element in container
									$va_container_ids[ $va_element_code[0] ]
										= array_unique( $qr_res->getAllFieldValues( 'attribute_id' ) );
						...
						case 'normalizedLength':
							...
							$vs_increment                  = caGetOption( 'increment', $va_facet_info, '1 m' );
							$vo_increment                  = caParseLengthDimension( $vs_increment );
							$vn_increment_in_current_units = (float) $vo_increment->convertTo( $vs_output_units, 6,
								'en_US' );

							foreach ( $va_row_ids as $vn_row_id ) {
								$vn_start = urldecode( trim( $vn_row_id ) ); // is start dimension

								if ( preg_match( "!^([\d][\.]*[\d]*)[ ]*\-[ ]*([\d][\.]*[\d]*)$!", $vn_start,
									$m )
								) { // range?
									$vn_start = (float) $m[1];
									$vn_end   = (float) $m[2];
								...
								$vn_start_in_meters = (float) $vo_start->convertTo( Zend_Measure_Length::METER, 6,
									'en_US' );
								$vn_end_in_meters   = (float) $vo_end->convertTo( Zend_Measure_Length::METER, 6,
									'en_US' );
						...
						case 'normalizedWeight':
							...
							foreach ( $va_row_ids as $vn_row_id ) {
								...
								$vn_start_in_meters = (float) $vo_start->convertTo( Zend_Measure_Weight::KILOGRAM, 6,
									'en_US' );
								$vn_end_in_meters   = (float) $vo_end->convertTo( Zend_Measure_Weight::KILOGRAM, 6,
									'en_US' );
						...
						case 'relationship_types':
							...
							if ( $t_user && $t_user->isLoaded()
							     && ( $t_user->getBundleAccessLevel( $this->ops_browse_table_name, $vs_rel_table_name )
							          < __CA_BUNDLE_ACCESS_READONLY__ )
							) {
								break;
							}
							...
							if ( ! is_array(
								$va_restrict_to_relationship_types = $va_facet_info['restrict_to_relationship_types'] )
							) {
								$va_restrict_to_relationship_types = array();
							}
							$va_restrict_to_relationship_types
							...
							foreach ( $va_row_ids as $vn_row_id ) {
								...
								if ( $this->ops_browse_table_name == $vs_rel_table_name ) {
									...
									if ( ! ( $self_rel_table
										= $t_rel_item->getProperty( 'SELF_RELATION_TABLE_NAME' ) )
									) {
										break;
									}
									$t_item_rel              = Datamodel::getInstanceByTableName( $self_rel_table,
										true );
									$vs_key                  = 'relation_id';
									$browse_on_self_relation = true;
									$va_path                 = [
										$this->ops_browse_table_name,
										$self_rel_table,
										$this->ops_browse_table_name
									];
								...
								if ( $browse_on_self_relation ) {
									$va_joins[] = 'INNER JOIN ' . $va_path[1] . ' ON (' . $va_path[0] . '.'
									              . $t_item->primaryKey() . ' = ' . $va_path[1] . '.'
									              . $t_item_rel->getRightTableFieldName() . ' OR ' . $va_path[0] . '.'
									              . $t_item->primaryKey() . ' = ' . $va_path[1] . '.'
									              . $t_item_rel->getLeftTableFieldName() . ")\n";
									$va_joins[] = 'INNER JOIN ' . $va_path[2] . ' AS ' . $va_path[2] . '_rel ON ('
									              . $va_path[2] . '_rel.' . $t_item->primaryKey() . ' = ' . $va_path[1]
									              . '.' . $t_item_rel->getLeftTableFieldName() . ') OR (' . $va_path[2]
									              . '_rel.' . $t_item->primaryKey() . ' = ' . $va_path[1] . '.'
								...
								if ( $va_facet_info['type'] == 'relationship_types' ) {
									$vs_get_item_sql = "(" . $t_item_rel->tableName() . ".type_id = " . (int) $vn_row_id
									                   . ")";
								} elseif ( ( ! isset( $va_facet_info['dont_expand_hierarchically'] )
								             || ! $va_facet_info['dont_expand_hierarchically'] )
								           && $t_rel_item->isHierarchical()
								           && $t_rel_item->load( (int) $vn_row_id )
								) {
									...
									$vs_get_item_sql = "{$vs_rel_item_table_name}.{$vs_hier_left_fld} >= "
									                   . $t_rel_item->get( $vs_hier_left_fld )
									                   . " AND {$vs_rel_item_table_name}.{$vs_hier_right_fld} <= "
									                   . $t_rel_item->get( $vs_hier_right_fld );
									if ( $vn_hier_id_fld = $t_rel_item->getProperty( 'HIERARCHY_ID_FLD' ) ) {
										$vs_get_item_sql .= " AND {$vs_rel_item_table_name}.{$vn_hier_id_fld} = "
										                    . (int) $t_rel_item->get( $vn_hier_id_fld );
									}
									...
									$vs_get_item_sql = "({$vs_rel_item_table_name}.{$vs_rel_table_pk} = "
									                   . (int) $vn_row_id . ")";
								}
						...
						case 'current_value':
							if ( $t_user && $t_user->isLoaded()
							     && ( $t_user->getBundleAccessLevel( $this->ops_browse_table_name,
										'ca_objects_location' ) < __CA_BUNDLE_ACCESS_READONLY__ )
							) {
								return [];
							}
							...
							foreach ( $va_row_ids as $vn_row_id ) {
								...
								if ( sizeof( $va_row_tmp ) === 1 ) {
									...
									$va_row_tmp = [
										$t_loc->tableNum(),
										$t_loc->get( 'ca_storage_locations.type_id' ),
										$vn_row_id
									];
								}
								$va_row_tmp = array_filter( $va_row_tmp, function ( $v ) {
									return (bool) $v;
								} );
								if ( $va_row_tmp[0] == 89 ) { // ca_storage_locations
									$t_loc = new ca_storage_locations();
									if ( ! is_array( $va_loc_ids = $t_loc->getHierarchy( $va_row_tmp[2],
											[ 'returnAsArray' => true, 'includeSelf' => true, 'idsOnly' => true ] ) )
									     || ! sizeof( $va_loc_ids )
									) {
										continue;
									}
									...
									$vs_sql = "
											...
											: "" )
									          . ( ( sizeof( $va_row_tmp ) > 2 )
											? " AND (((cv.current_type_id = ?) AND (cv.current_row_id = ?)) OR (cv.current_row_id IN (?)))"
											: "" );
								...
								$va_acc[ $vn_i ] = array_merge( $va_acc[ $vn_i ],
									$qr_res->getAllFieldValues( 'row_id' ) );
						...
						case 'inHomeLocation':
							...
							if ( is_array( $va_restrict_to_types ) && sizeof( $va_restrict_to_types ) ) {
								$va_wheres[] = "{$this->ops_browse_table_name}.type_id IN (" . join( ",",
										$va_restrict_to_types ) . ")";
							}
							if ( is_array( $va_exclude_types ) && sizeof( $va_exclude_types ) ) {
								$va_wheres[] = "{$this->ops_browse_table_name}.type_id NOT IN (" . join( ",",
										$va_exclude_types ) . ")";
							}
							if ( isset( $pa_options['checkAccess'] ) && is_array( $pa_options['checkAccess'] )
							     && sizeof( $pa_options['checkAccess'] )
								...
								$va_wheres[] = "({$this->ops_browse_table_name}.access IN (" . join( ',',
										$pa_options['checkAccess'] ) . "))";
							}
						...
						case 'fieldList':
							...
							// Map deaccession fields to deaccession bundle, which is used for access control on all
							$bundle_name = ( in_array( $vs_field_name, [ 'deaccession_type_id' ] ) )
								? 'ca_objects_deaccession' : $vs_field_name;
							if ( $t_user && $t_user->isLoaded()
							     && ( $t_user->getBundleAccessLevel( $vs_table_name, $bundle_name )
							          < __CA_BUNDLE_ACCESS_READONLY__ )
							...
							$dont_expand_hierarchically = caGetOption( 'dontExpandHierarchically', $va_facet_info,
								false );
							foreach ( $va_row_ids as $vn_row_id ) {
								$vn_row_id = urldecode( $vn_row_id );
								...
								if ( ! $dont_expand_hierarchically ) {
									if ( ! ( $t_list_item = ca_list_items::find( $vn_row_id, [
									...
									$ids = $t_list_item->get( 'ca_list_items.children.item_id',
										[ 'returnAsArray' => true ] );
								}
								$ids[] = $vn_row_id;
								if ( $vn_i == 0 ) {
							...
							foreach ( $va_row_ids as $vn_row_id ) {
								...
								if ( $vn_i == 0 ) {
									$vs_sql = "
											SELECT " . $this->ops_browse_table_name . '.' . $t_item->primaryKey() . "
											FROM " . $this->ops_browse_table_name . "
											INNER JOIN ca_metadata_dictionary_rule_violations ON ca_metadata_dictionary_rule_violations.row_id = "
									          . $this->ops_browse_table_name . '.' . $t_item->primaryKey()
									          . " AND ca_metadata_dictionary_rule_violations.table_num = "
									          . $t_item->tableNum() . "
							...
							foreach ( $va_row_ids as $vn_row_id ) {
								...
								if ( $vn_i == 0 ) {
									$vs_sql = "
											SELECT " . $this->ops_browse_table_name . '.' . $t_item->primaryKey() . "
											FROM " . $this->ops_browse_table_name . "
											INNER JOIN ca_items_x_tags ON ca_items_x_tags.row_id = "
									          . $this->ops_browse_table_name . '.' . $t_item->primaryKey()
									          . " AND ca_items_x_tags.table_num = " . $t_item->tableNum() . "
						...
						case 'checkouts':
							...
							foreach ( $va_row_ids as $vn_row_id ) {
								$vs_checkout_join_sql
									= "INNER JOIN ca_object_checkouts ON ca_object_checkouts.object_id = ca_objects.object_id";

								$vs_status_code = ( isset( $va_facet_info['status'] ) && $va_facet_info['status'] )
									? $va_facet_info['status'] : $vn_row_id;
								switch ( $vs_status_code ) {
									case 'overdue':
										$vs_where
											= "((ca_object_checkouts.checkout_date <= {$vn_current_time}) AND (ca_object_checkouts.return_date IS NULL) AND (ca_object_checkouts.due_date <= {$vn_current_time}))";
										break;
									case 'reserved':
										$vs_where
											= "((ca_object_checkouts.checkout_date IS NULL) AND (ca_object_checkouts.return_date IS NULL))";
										break;
									case 'available':
										$vs_checkout_join_sql = '';
										$vs_where
										                      = "(ca_objects.object_id NOT IN (SELECT object_id FROM ca_object_checkouts WHERE (ca_object_checkouts.checkout_date <= {$vn_current_time}) AND (ca_object_checkouts.return_date IS NULL)))";
										break;
									case 'all':
									...
									case 'out':
										$vs_where
											= "((ca_object_checkouts.checkout_date <= {$vn_current_time}) AND (ca_object_checkouts.return_date IS NULL))";
										break;
								}
								...
								$va_acc[ $vn_i ] = array_merge( $va_acc[ $vn_i ],
									$qr_res->getAllFieldValues( 'ca_objects.object_id' ) );
						...
						case 'dupeidno':
							...
							if ( is_array( $va_restrict_to_types ) && sizeof( $va_restrict_to_types ) ) {
								$va_wheres[] = "{$vs_browse_table_name}.type_id IN (" . join( ",",
										$va_restrict_to_types ) . ")";
							}
							if ( is_array( $va_exclude_types ) && sizeof( $va_exclude_types ) ) {
								$va_wheres[] = "{$vs_browse_table_name}.type_id NOT IN (" . join( ",",
										$va_exclude_types ) . ")";
							}
							...
							if ( is_array( $idno_list = $qr_res->getAllFieldValues( $idno_fld ) )
							     && ( sizeof( $idno_list ) > 0 )
							) {
								...
								$va_acc[ $vn_i ] = array_merge( $va_acc[ $vn_i ],
									$qr_res->getAllFieldValues( $vs_browse_table_name . '.' . $t_item->primaryKey() ) );
							...
							switch ( $vs_facet_name ) {
								case '_search':
									...
									if ( ! ( $o_search = caGetSearchInstance( $this->ops_browse_table_name ) ) ) {
										$this->postError( 2900, _t( "Invalid search type" ),
											"BrowseEngine->execute()" );
										break;
									}
									...
									$va_options['filterNonPrimaryRepresentations']
										= true;    // filter out non-primary representations in ca_objects results to save (a bit) of time

									$o_search->setOption( 'strictPhraseSearching',
										caGetOption( 'strictPhraseSearching', $va_options, true ) );
								...
								case '_reltypes':
									...
									foreach ( $va_row_ids as $vs_tmp ) {
										...
										$va_path_to_target
											= array_keys( Datamodel::getPath( $vs_target_browse_table_name,
											$t_target->tableName() ) );

										if ( is_array( $va_path_to_target ) && ( sizeof( $va_path_to_target ) == 3 ) ) {
											if ( ! ( $t_target_rel
													= Datamodel::getInstanceByTableName( $va_path_to_target[1], true ) )
											     || ! $t_target_rel->isRelationship()
											     || ! $t_target_rel->hasField( 'type_id' )
											) {
												break;
											}
											$va_ids_from_rel
												= array_unique( $this->_getRelationshipTypeIDs( explode( ",",
												$va_tmp[1] ), $va_path_to_target[1] ) );
											if ( is_array( $va_ids_from_rel ) && ( sizeof( $va_ids_from_rel ) > 0 ) ) {
												$qr_res
													             = $this->opo_db->query( "SELECT DISTINCT {$vs_target_browse_table_pk} FROM "
													                                     . $va_path_to_target[1]
													                                     . " WHERE type_id IN (?)",
													[ $va_ids_from_rel ] );
												$va_acc[ $vn_i ] = array_merge( $va_acc[ $vn_i ],
													$qr_res->getAllFieldValues( $vs_target_browse_table_pk ) );
											}
										}
									}
									$vn_i ++;
				...
				if ( ( ! isset( $pa_options['omitChildRecords'] ) || ! $pa_options['omitChildRecords'] )
				     && caGetOption( 'expandResultsHierarchically', $pa_options, false )
				     && ( $vs_hier_id_fld = Datamodel::getTableProperty( $this->ops_browse_table_name,
						'HIERARCHY_ID_FLD' ) )
				) {
					$vs_parent_id_fld                = Datamodel::getTableProperty( $this->ops_browse_table_name,
						'PARENT_ID_FLD' );
					...
					foreach ( $va_acc as $vn_i => $va_acc_content ) {
						...
						if ( is_array( $omit_child_records_for_types ) && sizeof( $omit_child_records_for_types ) ) {
							$omit_child_records_for_type_sql
								      = "AND (({$vs_parent_id_fld} IS NULL) OR ({$vs_parent_id_fld} IS NOT NULL AND type_id NOT IN (?)))";
						...
						if ( is_array( $va_expanded_res = $qr_expand->getAllFieldValues( $t_item->primaryKey() ) )
						     && sizeof( $va_expanded_res )
						) {
				...
				if ( sizeof( $va_res ) ) {
					...
					if ( sizeof( $this->opa_result_filters ) ) {
						...
						foreach ( $this->opa_result_filters as $va_filter ) {
							...
							$va_wheres[] = $this->ops_browse_table_name . '.' . $va_filter['field'] . " "
							               . $va_filter['operator'] . " " . $vm_val;
						}
					...
					if ( is_array( $omit_child_records_for_types ) && sizeof( $omit_child_records_for_types )
						...
						$va_wheres[]
							= "(({$this->ops_browse_table_name}.parent_id IS NULL) OR ({$this->ops_browse_table_name}.parent_id IS NOT NULL AND {$this->ops_browse_table_name}.type_id NOT IN ("
							  . join( ",", $omit_child_records_for_types ) . ")))";
					}
					...
					if ( ( isset( $pa_options['limitToModifiedOn'] ) && $pa_options['limitToModifiedOn'] ) ) {
						...
						if ( $o_tep->parse( $pa_options['limitToModifiedOn'] ) ) {
							...
							$va_joins['ca_change_log_subjects']
								= "INNER JOIN ca_change_log_subjects ON ca_change_log_subjects.subject_row_id = "
								  . $this->ops_browse_table_name . "." . $t_item->primaryKey()
								  . " AND ca_change_log_subjects.subject_table_num = " . $t_item->tableNum();
							$va_joins['ca_change_log']
								= "INNER JOIN ca_change_log ON ca_change_log.log_id = ca_change_log_subjects.log_id";

							$va_wheres[] = "(((ca_change_log.log_datetime BETWEEN " . (int) $va_range['start'] . " AND "
							               . (int) $va_range['end']
							               . ") AND (ca_change_log.changetype IN ('I', 'U', 'D'))))";

							$vs_sql_distinct
								= 'DISTINCT';    // need to pull distinct rows since joining the change log can cause dupes
						}
					}

					if ( ( $va_browse_type_ids = $this->getTypeRestrictionList() ) && is_array( $va_browse_type_ids )
					     && sizeof( $va_browse_type_ids )
					) {
						$t_subject   = $this->getSubjectInstance();
						$va_wheres[] = '(' . $this->ops_browse_table_name . '.' . $t_subject->getTypeFieldName()
						               . ' IN (' . join( ', ', $va_browse_type_ids ) . ')'
						               . ( $t_subject->getFieldInfo( 'type_id', 'IS_NULL' ) ? " OR ("
						                                                                      . $this->ops_browse_table_name
						                                                                      . '.'
						                                                                      . $t_subject->getTypeFieldName()
						                                                                      . " IS NULL)" : '' )
						               . ')';
					}

					if ( ( $va_browse_source_ids = $this->getSourceRestrictionList() )
					     && is_array( $va_browse_source_ids )
					     && sizeof( $va_browse_source_ids )
					) {
						$t_subject   = $this->getSubjectInstance();
						$va_wheres[] = '(' . $this->ops_browse_table_name . '.' . $t_subject->getSourceFieldName()
						               . ' IN (' . join( ', ', $va_browse_source_ids ) . ') OR ('
						               . $this->ops_browse_table_name . '.' . $t_subject->getSourceFieldName()
						               . ' IS NULL))';
					}

					$vs_filter_where_sql = "WHERE (" . $this->ops_browse_table_name . "." . $t_item->primaryKey()
					                       . " IN (?)) ";
					...
					$va_results = $vb_preserve_order ? array_intersect( $va_possible_values,
						$qr_res->getAllFieldValues( $t_item->primaryKey() ) )
						: $qr_res->getAllFieldValues( $t_item->primaryKey() );

					if ( ( ! isset( $pa_options['dontFilterByACL'] ) || ! $pa_options['dontFilterByACL'] )
					     && $this->opo_config->get( 'perform_item_level_access_checking' )
					     && method_exists( $t_item, "supportsACL" )
					     && $t_item->supportsACL()
					) {
						$va_results = $this->filterHitsByACL( $va_results, $this->opn_browse_table_num, $vn_user_id,
			...
			if (
				...
				if ( ( isset( $pa_options['limitToModifiedOn'] ) && $pa_options['limitToModifiedOn'] ) ) {
					...
					if ( $o_tep->parse( $pa_options['limitToModifiedOn'] ) ) {
						...
						$va_joins['ca_change_log_subjects']
							= "INNER JOIN ca_change_log_subjects ON ca_change_log_subjects.subject_row_id = "
							  . $this->ops_browse_table_name
							  . ".{$vs_pk} AND ca_change_log_subjects.subject_table_num = " . $t_item->tableNum();
						$va_joins['ca_change_log']
							= "INNER JOIN ca_change_log ON ca_change_log.log_id = ca_change_log_subjects.log_id";
				...
				if ( ( ! isset( $pa_options['dontFilterByACL'] ) || ! $pa_options['dontFilterByACL'] )
					...
					$va_results = array_keys( $this->filterHitsByACL( $va_results, $this->opn_browse_table_num,
						$vn_user_id, __CA_ACL_READONLY_ACCESS__ ) );
				}
	...
	public function loadFacetContent( $pa_options = null ) {
		...
		if ( ( ! $va_criteria ) || ( is_array( $va_criteria ) && ( sizeof( $va_criteria ) == 0 ) ) ) {
			...
			$this->opo_ca_browse_cache->setGlobalParameter( 'facets_with_content',
				array_keys( $va_facets_with_content ) );
		}
	...
	public function getFacetWithGroups( $ps_facet_name, $ps_group_mode, $ps_grouping_field = null, $pa_options = null
		...
		if ( $vs_grouping_attribute_element_code = ( preg_match( '!^ca_attribute_([\w]+)!', $ps_grouping_field,
			$va_matches ) ) ? $va_matches[1] : null
		) {
			$t_element = new ca_metadata_elements();
		...
		if ( ( ! isset( $va_facet_info['groupings'][ $ps_grouping_field ] )
		       || ! ( $va_facet_info['groupings'][ $ps_grouping_field ] ) )
		     && is_array( $va_facet_info['groupings'] )
		) {
		...
		switch ( $ps_group_mode ) {
			...
			default:
				...
				$va_label_order_by_fields = isset( $va_facet_info['order_by_label_fields'] )
					? $va_facet_info['order_by_label_fields'] : [ 'label' ];
							...
							if ( $vn_grouping_attribute_id ) {
								switch ( $vn_element_datatype ) {
									case 2: //date
										...
										if ( isset( $va_item[ 'ca_attribute_' . $vn_grouping_attribute_id ] )
										     && is_array( $va_item[ 'ca_attribute_' . $vn_grouping_attribute_id ] )
										) {
											foreach (
												$va_item[ 'ca_attribute_' . $vn_grouping_attribute_id ] as $vn_i =>
												$va_v
											) {
												$va_v = $o_tep->normalizeDateRange( $va_v['value_decimal1'],
													$va_v['value_decimal2'], ( isset( $va_tmp[1] )
													                           && in_array( $va_tmp[1],
															array( 'years', 'decades', 'centuries' ) ) ) ? $va_tmp[1]
														: 'decades' );
	...
	/**
	 * Returns list of hierarchy_ids used by entries in the specified authority facet. For example, if the current
	 * browse has an authority facet for vocabulary terms (aka. ca_list_items), then this methods will return list_ids
	 * for all of the lists in which the facet entries reside. For a ca_places authority facet, this will return all of
	 * the place hierarchy_ids for which places in the facet reside.
	 *
	 ...
	 *                              checkAccess = array of access values to filter facets that have an 'access' field
	 *                              by
	 *
	...
	/**
	 * Return list of items from the specified table that are related to the current browse set. This is the method
	 * that actually pulls the facet content, regardless of whether the facet is cached yet or not. If you want to use
	 * the facet cache, call BrowseEngine::getFacet()
	 *
	 * @see BrowseEngine::getFacet()
	 * Options:
	 *        checkAccess = array of access values to filter facets that have an 'access' field by
	 *        checkAvailabilityOnly = if true then content is not actually fetch - only the availablility of content is
	 *        verified user_id = If set item level access control is performed relative to specified user_id, otherwise
	...
	public function getFacetContent( $ps_facet_name, $pa_options = null ) {
		...
		$vn_user_id        = ( isset( $pa_options['user_id'] ) && (int) $pa_options['user_id'] )
			? (int) $pa_options['user_id'] : (int) $AUTH_CURRENT_USER_ID;
		...
		$vb_check_availability_only = ( isset( $pa_options['checkAvailabilityOnly'] ) )
			? (bool) $pa_options['checkAvailabilityOnly'] : false;
		...
		if ( ( $va_browse_type_ids = $this->getTypeRestrictionList() ) && is_array( $va_browse_type_ids )
			...
			$vs_browse_type_limit_sql = '(' . $t_subject->tableName() . '.' . $t_subject->getTypeFieldName() . ' IN ('
			                            . join( ', ', $va_browse_type_ids ) . ')'
			                            . ( $t_subject->getFieldInfo( 'type_id', 'IS_NULL' ) ? " OR ("
			                                                                                   . $this->ops_browse_table_name
			                                                                                   . '.'
			                                                                                   . $t_subject->getTypeFieldName()
		...
		if ( ( $va_browse_source_ids = $this->getSourceRestrictionList() ) && is_array( $va_browse_source_ids )
		     && sizeof( $va_browse_source_ids )
		) {        // source restrictions
			$vs_browse_source_limit_sql = '(' . $t_subject->tableName() . '.' . $t_subject->getSourceFieldName()
			                              . ' IN (' . join( ', ', $va_browse_source_ids ) . ')'
			                              . ( $t_subject->getFieldInfo( 'source_id', 'IS_NULL' ) ? " OR ("
			                                                                                       . $this->ops_browse_table_name
			                                                                                       . '.'
			                                                                                       . $t_subject->getSourceFieldName()
			                                                                                       . " IS NULL)" : '' )
			                              . ')';
		...
		switch ( $va_facet_info['type'] ) {
			...
			case 'has':
				...
				if ( isset( $va_facet_info['element_code'] ) && $t_user && $t_user->isLoaded()
				     && ( $t_user->getBundleAccessLevel( $vs_browse_table_name,
							array_shift( explode( '.', $va_facet_info['element_code'] ) ) )
				          < __CA_BUNDLE_ACCESS_READONLY__ )
				) {
					return [];
				} elseif ( ( isset( $va_facet_info['table'] ) && $t_user && $t_user->isLoaded()
				             && ( $t_user->getBundleAccessLevel( $vs_browse_table_name, $va_facet_info['table'] )
				                  < __CA_BUNDLE_ACCESS_READONLY__ ) )
				) {
					return [];
				}
				...
				if ( $va_facet_info['element_code'] ) {
					...
					foreach ( $va_facet_values as $vs_state_name => $va_state_info ) {
						...
						if ( ! (bool) $va_state_info['id'] ) {    // no option
							$va_wheres[] = $this->ops_browse_table_name . '.' . $vs_browse_table_pk
							               . " NOT IN (select row_id from ca_attributes where table_num = "
							               . $t_item->tableNum() . " AND element_id = " . $t_element->getPrimaryKey()
							               . ")";

						} else {                            // yes option
							$va_joins[] = "LEFT JOIN ca_attributes AS caa ON  " . $this->ops_browse_table_name . '.'
							              . $vs_browse_table_pk . " = caa.row_id AND " . $t_item->tableNum()
							              . " = caa.table_num";
						...
						if ( isset( $pa_options['checkAccess'] ) && is_array( $pa_options['checkAccess'] )
							...
							$va_wheres[] = "(" . $vs_browse_table_name . ".access IN (" . join( ',',
									$pa_options['checkAccess'] ) . "))";
						}

						if ( is_array( $va_results ) && sizeof( $va_results ) ) {
							$va_wheres[] = $vs_browse_table_name . "." . $vs_browse_table_pk . " IN (" . join( ",",
									$va_results ) . ")";
						}
						...
						if ( $va_facet_info['relative_to'] ) {
							...
							if ( $va_relative_sql_data = $this->_getRelativeFacetSQLData( $va_facet_info['relative_to'],
								$pa_options )
							) {
						...
						if ( $this->opo_config->get( 'perform_item_level_access_checking' ) ) {
							if ( $t_item = Datamodel::getInstanceByTableName( $vs_browse_table_name, true ) ) {
								...
								$va_joins[]  = 'LEFT JOIN ca_acl ON ' . $vs_browse_table_name . '.'
								               . $vs_browse_table_pk . ' = ca_acl.row_id AND ca_acl.table_num = '
								               . $t_item->tableNum() . "\n";
								$va_wheres[] = "(
										((
					...
					$va_exclude_relationship_types = $this->_getRelationshipTypeIDs( $va_exclude_relationship_types,
						$va_facet_info['relationship_table'] );
					...
					switch ( sizeof(
						$va_path = array_keys( Datamodel::getPath( $vs_browse_table_name, $vs_rel_table_name ) ) ) ) {
						case 3:
							$t_item_rel = Datamodel::getInstanceByTableName( $va_path[1], true );
							$t_rel_item = Datamodel::getInstanceByTableName( $va_path[2], true );
							$vs_key     = 'relation_id';
					...
					if ( ! is_array( $va_restrict_to_types = $this->_convertTypeCodesToIDs( $va_restrict_to_types,
						array( 'instance' => $t_rel_item ) ) )
					) {
						$va_restrict_to_types = [];
					}
					...
					foreach ( $va_path as $vs_join_table ) {
						$va_rel_info     = Datamodel::getRelationships( $vs_cur_table, $vs_join_table );
						$va_joins_init[] = ( $vn_state ? 'INNER' : 'LEFT' ) . ' JOIN ' . $vs_join_table . ' ON '
						                   . $vs_cur_table . '.' . $va_rel_info[ $vs_cur_table ][ $vs_join_table ][0][0]
						                   . ' = ' . $vs_join_table . '.'
						                   . $va_rel_info[ $vs_cur_table ][ $vs_join_table ][0][1] . "\n";
						$vs_cur_table    = $vs_join_table;
					}
					...
					foreach ( $va_facet_values as $vs_state_name => $va_state_info ) {
						...
						if ( is_array( $va_exclude_types ) && sizeof( $va_exclude_types ) ) {
							$va_wheres[] = "((" . $t_rel_item->tableName() . ".type_id NOT IN (" . join( ',',
									$va_exclude_types ) . ")) OR (" . $t_rel_item->tableName() . ".type_id IS NULL))";
						}
						...
						$id_set = ( is_array( $va_results ) && sizeof( $va_results ) ) ? "{$vs_browse_table_pk} IN ("
						                                                                 . join( ",", $va_results )
						                                                                 . ")" : null;
			...
			case 'label':
				...
				if ( $t_user && $t_user->isLoaded()
				     && ( $t_user->getBundleAccessLevel( $vs_browse_table_name, 'preferred_labels' )
				          < __CA_BUNDLE_ACCESS_READONLY__ )
				) {
					return [];
				}
				...
				if ( sizeof( $va_label_order_by_fields = isset( $va_facet_info['order_by_label_fields'] )
					? $va_facet_info['order_by_label_fields'] : [] )
				) {
				...
				if ( isset( $va_facet_info['preferred_labels_only'] ) && $va_facet_info['preferred_labels_only']
				     && $t_label->hasField( 'is_preferred' )
				) {
				...
				if ( isset( $pa_options['checkAccess'] ) && is_array( $pa_options['checkAccess'] )
					...
					$va_where_sql[] = "(" . $vs_browse_table_name . ".access IN (" . join( ',',
							$pa_options['checkAccess'] ) . "))";
				}
				...
				if ( is_array( $va_restrict_to_types ) && sizeof( $va_restrict_to_types ) ) {
					$va_restrict_to_type_ids = caMakeTypeIDList( $vs_browse_table_name, $va_restrict_to_types,
						array( 'dont_include_subtypes_in_type_restriction' => true ) );
					if ( is_array( $va_restrict_to_type_ids ) && sizeof( $va_restrict_to_type_ids ) ) {
						$va_where_sql[] = "(" . $vs_browse_table_name . "." . $t_item->getTypeFieldName() . " IN ("
						                  . join( ", ", $va_restrict_to_type_ids ) . ")"
						                  . ( $t_item->getFieldInfo( 'type_id', 'IS_NULL' ) ? " OR ("
						                                                                      . $vs_browse_table_name
						                                                                      . '.'
						                                                                      . $t_item->getTypeFieldName()
						                                                                      . " IS NULL)" : '' )
						                  . ")";
						$vb_needs_join  = true;
					}
				...
				if ( is_array( $va_exclude_types ) && sizeof( $va_exclude_types ) ) {
					$va_exclude_type_ids = caMakeTypeIDList( $vs_browse_table_name, $va_exclude_types,
						array( 'dont_include_subtypes_in_type_restriction' => true ) );
					if ( is_array( $va_exclude_type_ids ) && sizeof( $va_exclude_type_ids ) ) {
						$va_where_sql[] = "(" . $vs_browse_table_name . "." . $t_item->getTypeFieldName() . " NOT IN ("
						                  . join( ", ", $va_exclude_type_ids ) . ")"
						                  . ( $t_item->getFieldInfo( 'type_id', 'IS_NULL' ) ? " OR ("
						                                                                      . $vs_browse_table_name
						                                                                      . '.'
						                                                                      . $t_item->getTypeFieldName()
				...
				if ( $vb_needs_join ) {
					$va_joins[] = "INNER JOIN " . $vs_browse_table_name . " ON " . $vs_browse_table_name . "."
					              . $t_item->primaryKey() . " = l." . $t_item->primaryKey();
				}
				...
				if ( is_array( $va_results ) && sizeof( $va_results ) ) {
					if ( $va_facet_info['relative_to'] ) {
						$va_where_sql[] = $this->ops_browse_table_name . "." . $t_subject->primaryKey() . " IN ("
						                  . join( ",", $va_results ) . ")";
					} else {
				...
				if ( $va_facet_info['relationship'] ) {
					$va_joins[] = "INNER JOIN " . $va_facet_info['relationship'] . " ON {$vs_browse_table_name}."
					              . $t_item->primaryKey() . " = " . $va_facet_info['relationship'] . "."
					              . $t_item->primaryKey();
					if ( is_array( $va_facet_info['restrict_to_relationship_types'] )
					     && ( $rel_type_ids
							= caMakeRelationshipTypeIDList( $va_facet_info['relationship'],
				...
				if ( $this->opo_config->get( 'perform_item_level_access_checking' ) ) {
					if ( $t_item = Datamodel::getInstanceByTableName( $vs_browse_table_name, true ) ) {
						// Join to limit what browse table items are used to generate facet
						$va_joins[]     = 'LEFT JOIN ca_acl ON ' . $vs_browse_table_name . '.' . $t_item->primaryKey()
						                  . ' = ca_acl.row_id AND ca_acl.table_num = ' . $t_item->tableNum() . "\n";
						$va_where_sql[] = "(
								((
									(ca_acl.user_id = " . (int) $vn_user_id . ")
									" . ( ( sizeof( $va_group_ids ) > 0 ) ? "OR
				...
				if ( $vb_check_availability_only ) {
					...
					$vs_parent_fld_select = ( ( $vs_parent_fld = $t_item->getProperty( 'HIERARCHY_PARENT_ID_FLD' ) )
						? $vs_browse_table_name . "." . $vs_parent_fld : '' );

					$group_by_fields = array_unique( array_merge( $va_label_order_by_fields, $va_label_ui_fields,
						[ "l.{$vs_label_display_field}", $vs_parent_fld_select, "l.locale_id", "l.{$vs_item_pk}" ] ) );
					$vs_sql          = "
							SELECT COUNT(*) as _count, l.locale_id, l.{$vs_label_display_field} "
					                   . ( $vs_parent_fld_select ? ", {$vs_parent_fld_select}" : "" )
					                   . ", l.{$vs_item_pk}" . ( ( sizeof( $va_label_ui_fields ) > 0 ) ? ", "
					                                                                                     . join( ", ",
								$va_label_ui_fields ) : "" ) . "
							FROM {$vs_label_table_name} l
								{$vs_join_sql}
							...
							ORDER BY " . ( ( sizeof( $va_label_order_by_fields ) > 0 ) ? join( ", ",
							$va_label_order_by_fields ) : "l.{$vs_label_display_field}" ) . "
						";
			...
			case 'attribute':
				...
				$vs_container_code = ( is_array( $va_element_code ) && ( sizeof( $va_element_code ) > 0 ) )
					? array_pop( $va_element_code ) : null;
				...
				$va_joins = array(
					...
					'INNER JOIN ' . ( ! $vb_is_relative_to_parent
						? "{$vs_browse_table_name} ON {$vs_browse_table_name}."
						: "{$vs_browse_table_name} AS parent ON parent." ) . $t_item->primaryKey()
					. ' = ca_attributes.row_id AND ca_attributes.table_num = ' . intval( $vs_browse_table_num )
				);
				...
				if ( is_array( $va_results ) && sizeof( $va_results ) && ( $this->numCriteria() > 0 ) ) {
					...
					if ( ( $ps_facet_name === $last_facet ) && $va_facet_info['multiple']
					     && ( is_array( $c[ $ps_facet_name ] ) && sizeof( $c[ $ps_facet_name ] ) )
					) {
					...
					$va_wheres[] = "(" . $t_subject->tableName() . '.' . $t_subject->primaryKey() . " IN (" . join( ',',
							$va_results ) . "))";
				}
				...
				if ( $vb_check_availability_only ) {
					...
					if ( is_array(
						$va_suppress_values = caGetOption( [ 'suppress', 'exclude_values' ], $va_facet_info, null ) )
					) {
					...
					if ( is_array( $va_container_ids[ $vs_container_code ] )
					     && sizeof( $va_container_ids[ $vs_container_code ] )
					) {
						$vs_container_sql = " AND ca_attributes.attribute_id IN (?)";
					...
					switch ( $vn_element_type ) {
						case __CA_ATTRIBUTE_VALUE_LIST__:
							if ( ! is_array( $va_restrict_to_types
								= $this->_convertTypeCodesToIDs( $va_facet_info['restrict_to_types'],
								array( 'instance' => new ca_list_items(), 'dontExpandHierarchically' => true ) ) )
							) {
								$va_restrict_to_types = array();
							}
							...
							$t_list_item               = new ca_list_items();
							$va_list_item_cache        = $t_list_item->getFieldValuesForIDs( $va_values,
								array( 'type_id', 'idno', 'item_value', 'parent_id', 'access', 'deleted', 'rank' ) );
							...
							if ( is_array( $va_list_item_cache ) ) {
								foreach ( $va_list_item_cache as $vn_id => $va_item ) {
									...
									if ( is_array( $pa_options['checkAccess'] ) && sizeof( $pa_options['checkAccess'] )
									     && ! in_array( $va_item['access'], $pa_options['checkAccess'] )
									) {
										continue;
									}
							...
							if ( is_array( $va_suppress_values ) ) {
								$va_suppress_values = ca_lists::getItemIDsFromList( $t_element->get( 'list_id' ),
									$va_suppress_values );
							}
							...
							foreach ( $va_values as $i => $vn_val ) {
								...
								if ( is_array( $pa_options['checkAccess'] ) && sizeof( $pa_options['checkAccess'] )
								     && ! in_array( $va_list_item_cache[ $vn_val ]['access'],
										$pa_options['checkAccess'] )
								) {
									continue;
								}
								...
								if ( is_array( $va_restrict_to_types ) && sizeof( $va_restrict_to_types )
								     && ! in_array( $va_list_item_cache[ $vn_val ]['type_id'], $va_restrict_to_types )
								) {
									continue;
								}
								...
								$vn_child_count = isset( $va_list_child_count_cache[ $vn_val ] )
									? $va_list_child_count_cache[ $vn_val ] : 0;
								...
								$va_facet_list[ $vn_val ]                    = array(
									...
									'label'         => $vs_label,
									'parent_id'     => $vn_parent_id
										= isset( $va_list_item_cache[ $vn_val ]['parent_id'] )
										? $va_list_item_cache[ $vn_val ]['parent_id'] : null,
							...
							if ( ! isset( $va_facet_info['dont_expand_hierarchically'] )
							     || ! $va_facet_info['dont_expand_hierarchically']
							) {
								$t_rel_item = new ca_list_items();
								$qr_res->seek( 0 );
								$va_ids       = $qr_res->getAllFieldValues( $vs_rel_pk );
								$qr_ancestors = call_user_func( $t_rel_item->tableName()
								                                . '::getHierarchyAncestorsForIDs', $va_values,
									array( 'returnAs' => 'SearchResult' ) );

								$vs_rel_table = $t_rel_item->tableName();
								$vs_rel_pk    = $t_rel_item->primaryKey();

								$vb_check_ancestor_access = (bool) ( isset( $pa_options['checkAccess'] )
								                                     && is_array( $pa_options['checkAccess'] )
								                                     && sizeof( $pa_options['checkAccess'] )
								                                     && $t_rel_item->hasField( 'access' ) );
								...
								if ( $qr_ancestors ) {
									...
									while ( $qr_ancestors->nextHit() ) {
										...
										if ( is_array( $va_suppress_values )
										     && ( in_array( $vn_ancestor_id, $va_suppress_values ) )
										) {
											continue;
										}
										if ( is_array( $va_exclude_types ) && ( sizeof( $va_exclude_types ) > 0 )
										     && in_array( $vn_parent_type_id, $va_exclude_types )
										) {
											continue;
										}
										if ( is_array( $va_restrict_to_types )
										     && ( sizeof( $va_restrict_to_types ) > 0 )
										     && ! in_array( $vn_parent_type_id, $va_restrict_to_types )
										) {
											continue;
										}
										if ( $vb_check_ancestor_access
										     && ! in_array( $qr_ancestors->get( 'access' ), $pa_options['checkAccess'] )
										) {
											continue;
										}
										...
										if ( ( ! isset( $va_facet_info['dont_expand_hierarchically'] )
											...
											$vs_hier_left_fld = $t_rel_item->getProperty( 'HIERARCHY_LEFT_INDEX_FLD' );
											$vs_hier_right_fld
											                  = $t_rel_item->getProperty( 'HIERARCHY_RIGHT_INDEX_FLD' );

											$vs_get_item_sql = "{$vs_rel_table}.{$vs_hier_left_fld} >= "
											                   . $qr_ancestors->get( $vs_hier_left_fld )
											                   . " AND {$vs_rel_table}.{$vs_hier_right_fld} <= "
											                   . $qr_ancestors->get( $vs_hier_right_fld );
											if ( $vn_hier_id_fld = $t_rel_item->getProperty( 'HIERARCHY_ID_FLD' ) ) {
												$vs_get_item_sql .= " AND {$vs_rel_table}.{$vn_hier_id_fld} = "
												                    . (int) $qr_ancestors->get( $vn_hier_id_fld );
											}
											$vs_get_item_sql = "({$vs_get_item_sql})";
										} else {
										...
										$va_facet_list[ $vn_ancestor_id ] = array(
											'id'            => $vn_ancestor_id,
											'label'         => ( $vs_label
												= $qr_ancestors->get( 'ca_list_items.preferred_labels.name_plural' ) )
												? $vs_label : '[' . caGetBlankLabelText( 'ca_list_items' ) . ']',
											'parent_id'     => $vn_parent_id,
							...
							if ( $vn_list_id = $t_element->get( 'list_id' ) ) {
								...
								switch ( $t_list->get( 'default_sort' ) ) {
									...
									case __CA_LISTS_SORT_BY_VALUE__:
										...
										break;
									default:
										return caSortArrayByKeyInValue( $va_facet_list, [ 'label' ], 'ASC',
											[ 'naturalSort' => true ] );
										break;
								}
							} else {
						...
						case __CA_ATTRIBUTE_VALUE_OBJECTLOTS__:
							if ( $t_rel_item = AuthorityAttributeValue::elementTypeToInstance( $vn_element_type ) ) {
								...
								if ( is_array(
									$access = $t_rel_item->getFieldValuesForIDs( $va_ids, [ 'access' ] ) )
								) {
									$va_ids = [];
					...
					while ( $qr_res->nextRow() ) {
						...
						if ( is_array( $access ) && isset( $access[ $qr_res->get( 'value_integer1' ) ] )
						     && ( $access[ $qr_res->get( 'value_integer1' ) ] != 1 )
						) {
							continue;
						}
						switch ( $vn_element_type ) {
							case __CA_ATTRIBUTE_VALUE_LIST__:
								...
								$va_values[ strToLower( $vs_val ) ] = array(
									'id'            => $vn_id,
									'label'         => html_entity_decode( $va_list_items[ $vs_val ]['name_plural']
										? $va_list_items[ $vs_val ]['name_plural']
										: $va_list_items[ $vs_val ]['item_value'] ),
							...
							case __CA_ATTRIBUTE_VALUE_OBJECTLOTS__:
								...
								$va_values[ strToLower( $vs_val ) ] = array(
									'id'            => $vn_id,
									'label'         => html_entity_decode( $va_auth_items[ $vn_id ]
										? $va_auth_items[ $vn_id ] : $vs_val ),
									'content_count' => $qr_res->get( '_count' )
								);
								break;
							case __CA_ATTRIBUTE_VALUE_LCSH__:
								$value_id                           = ca_attribute_values::getValueIDFor( $o_attr->getElementID(),
									$vs_val );
								$va_values[ strToLower( $vs_val ) ] = array(
									'id'            => $value_id,
			...
			case 'current_value':
				$t_item = Datamodel::getInstanceByTableName( $vs_browse_table_name, true );
				if ( $t_user && $t_user->isLoaded()
				     && ( $t_user->getBundleAccessLevel( $vs_browse_table_name, 'ca_objects_location' )
				          < __CA_BUNDLE_ACCESS_READONLY__ )
				) {
					return [];
				}
				...
				$va_joins
					       = [
					"INNER JOIN ca_history_tracking_current_values AS cv ON cv.row_id = {$vs_browse_table_name}."
					. $t_item->primaryKey() . " AND cv.table_num = {$vs_browse_table_num}"
				];
				...
				if ( $vb_check_availability_only ) {
					...
					while ( $qr_res->nextRow() ) {
						...
						if ( ! ( $current_type_id = trim( $qr_res->get( 'current_type_id' ) ) ) ) {
							continue;
						}
					...
					foreach ( $values_by_table as $current_table_num => $current_row_ids_by_type ) {
						foreach ( $current_row_ids_by_type as $current_type_id => $current_row_info ) {
							...
							if ( isset( $va_collapse_map[ $current_table_name ] )
							     && isset( $va_collapse_map[ $current_table_name ]['*'] )
							     && $va_collapse_map[ $current_table_name ]['*']
							) {
							...
							$va_config = ca_objects::getConfigurationForHistoryTrackingCurrentValue( $policy,
								$current_table_name, $current_type_id, array(
									'facet' => isset( $va_facet_info['display'] ) ? $va_facet_info['display'] : null
								) );

							if ( $current_table_name == 'ca_storage_locations' ) {

								if ( ! ( $vn_max_browse_depth = caGetOption( 'maximumBrowseDepth', $va_facet_info,
									null ) )
								) {
								...
								while ( $qr_res->nextHit() ) {
									if ( is_array(
										$va_ids = $qr_res->get( "{$current_table_name}.hierarchy.{$vs_hier_pk}",
											[ 'returnAsArray' => true, 'maxLevelsFromTop' => $vn_max_browse_depth ] ) )
									) {
							...
							$display_config = $this->getTableEntryFromMap( $va_facet_info, 'display',
								$current_table_name );
							$vs_template    = isset( $display_config['template'] ) ? $display_config['template']
								: "^{$current_table_name}.preferred_labels";
							...
							while ( $qr_res->nextHit() ) {
								...
								if ( isset( $va_collapse_map[ $current_table_name ] )
								     && isset( $va_collapse_map[ $current_table_name ][ $type_id ] )
								     && $va_collapse_map[ $current_table_name ][ $type_id ]
								) {
								...
								if ( ! $row_id
								     || ! ( $vs_label = $qr_res->getWithTemplate( $vs_template, $va_config ) )
								) {
									continue;
								}
			...
			case 'inHomeLocation':
				...
				if ( is_array( $va_results ) && sizeof( $va_results ) && ( $this->numCriteria() > 0 ) ) {
					$va_wheres[] = "({$vs_browse_table_name}." . $t_subject->primaryKey() . " IN (" . join( ',',
							$va_results ) . "))";
				}
				...
				$labels = [
					...
					0 => [
						'id'    => 0,
						'label' => caGetOption( 'label_no', $va_facet_info, _t( 'Not in home location' ) )
					]
				];
			...
			case 'fieldList':
				...
				// Map deaccession fields to deaccession bundle, which is used for access control on all
				$bundle_name = ( in_array( $vs_field_name, [ 'deaccession_type_id' ] ) ) ? 'ca_objects_deaccession'
					: $vs_field_name;
				if ( $t_user && $t_user->isLoaded()
				     && ( $t_user->getBundleAccessLevel( $vs_browse_table_name, $bundle_name )
				          < __CA_BUNDLE_ACCESS_READONLY__ )
				) {
					return [];
				}
				...
				if ( isset( $va_field_info['LIST_CODE'] ) && ( $vs_list_name = $va_field_info['LIST_CODE'] ) ) {
					...
					$va_joins = array(
						'INNER JOIN ' . $vs_browse_table_name . ' ON ' . $vs_browse_table_name . '.' . $vs_field_name
						. ' = li.item_id',
						'INNER JOIN ca_lists ON ca_lists.list_id = li.list_id'
					);
					if ( is_array( $va_results ) && sizeof( $va_results ) && ( $this->numCriteria() > 0 ) ) {
						$va_wheres[] = "(" . $t_subject->tableName() . '.' . $t_subject->primaryKey() . " IN ("
						               . join( ',', $va_results ) . "))";
					}
					...
					if ( $vb_check_availability_only ) {
						...
						$va_ordering_fields_to_fetch = ( isset( $va_facet_info['order_by_label_fields'] )
						                                 && is_array( $va_facet_info['order_by_label_fields'] ) )
							? $va_facet_info['order_by_label_fields'] : array();

						$va_orderbys = array();

						// Get label ordering fields
						if ( isset( $va_facet_info['order_by_label_fields'] )
						     && is_array( $va_facet_info['order_by_label_fields'] )
						     && sizeof( $va_facet_info['order_by_label_fields'] )
						) {
						...
						if ( $va_facet_info['group_mode'] == 'hierarchical' ) {
							$t_rel_item   = new ca_list_items();
							$qr_ancestors = call_user_func( $t_rel_item->tableName() . '::getHierarchyAncestorsForIDs',
								array_keys( $va_values ), array( 'returnAs' => 'SearchResult' ) );
					...
					if ( $vs_list_name = $va_field_info['LIST'] ) {
						...
						if ( $va_list_items
							= caExtractValuesByUserLocale( $t_list->getItemsForList( $vs_list_name ) )
						) {
							foreach ( $va_list_items as $vn_id => $va_list_item ) {
						...
						if ( $this->opo_config->get( 'perform_item_level_access_checking' ) ) {
							if ( $t_item = Datamodel::getInstanceByTableName( $vs_browse_table_name, true ) ) {
								// Join to limit what browse table items are used to generate facet
								$va_joins[]  = 'LEFT JOIN ca_acl ON ' . $vs_browse_table_name . '.'
								               . $t_item->primaryKey() . ' = ca_acl.row_id AND ca_acl.table_num = '
								               . $t_item->tableNum() . "\n";
								$va_wheres[] = "(
										((
											(ca_acl.user_id = " . (int) $vn_user_id . ")
						...
						if ( $t_browse_table = Datamodel::getInstanceByTableName(
							$vs_facet_table = $va_facet_info['table'], true )
						) {
							// Handle fields containing ca_list_item.item_id's
							$va_joins = array(
								'INNER JOIN ' . $vs_browse_table_name . ' ON ' . $vs_browse_table_name . '.'
								. $vs_field_name . ' = ' . $vs_facet_table . '.' . $t_browse_table->primaryKey()
							);


							$vs_display = caGetOption( 'display', $va_facet_info,
								"^" . $t_browse_table->tableName() . ".preferred_labels" );
							if ( method_exists( $t_browse_table, 'getLabelTableInstance' ) ) {
								$t_label_instance = $t_browse_table->getLabelTableInstance();
								$va_joins[]       = 'INNER JOIN ' . $t_label_instance->tableName() . " AS lab ON lab."
								                    . $t_browse_table->primaryKey() . ' = '
								                    . $t_browse_table->tableName() . '.'
								                    . $t_browse_table->primaryKey();
							}
							...
							if ( $va_facet_info['relative_to'] ) {
								...
								if ( $va_relative_sql_data
									= $this->_getRelativeFacetSQLData( $va_facet_info['relative_to'], $pa_options )
								) {
									$va_joins  = array_merge( $va_joins, $va_relative_sql_data['joins'] );
									$va_wheres = array_merge( $va_wheres, $va_relative_sql_data['wheres'] );
								}
							...
							if ( $vb_check_availability_only ) {
								...
								$values = caProcessTemplateForIDs( $vs_display, $t_browse_table->tableName(),
									$qr_res->getAllFieldValues( $vs_pk ),
									[ 'returnAsArray' => true, 'indexWithIDs' => true ] );
								...
								while ( $qr_res->nextRow() ) {
									...
									$va_values[ $vn_id ][ $qr_res->get( 'locale_id' ) ] = array(
										'id'            => $vn_id,
										'label'         => isset( $values[ $vn_id ] ) ? $values[ $vn_id ] : "???",
										//$qr_res->get($vs_display_field_name),
										'content_count' => $qr_res->get( '_count' )
									);
									if ( ! is_null( $vs_single_value ) && ( $vn_id == $vs_single_value ) ) {
										$vb_single_value_is_present = true;
			...
			case 'field':
				...
				if ( ! is_array( $va_restrict_to_types = $this->_convertTypeCodesToIDs( $va_restrict_to_types,
					array( 'instance' => $t_item, 'dontExpandHierarchically' => true ) ) )
				) {
					$va_restrict_to_types = array();
				}
				$va_restrict_to_types_expanded = $this->_convertTypeCodesToIDs( $va_restrict_to_types,
					array( 'instance' => $t_item ) );
				...
				if ( is_array( $va_restrict_to_types ) && ( sizeof( $va_restrict_to_types ) > 0 )
				     && method_exists( $t_rel_item, "getTypeList" )
				) {
					$va_wheres[]  = "{$va_restrict_to_types_expanded}.type_id IN (" . join( ',',
							caGetOption( 'dont_include_subtypes', $va_facet_info, false ) ? $va_restrict_to_types
								: $va_restrict_to_types_expanded ) . ")";
			...
			case 'violations':
				...
				if ( $vb_check_availability_only ) {
					$vs_sql = "
							SELECT 1
							FROM {$vs_browse_table_name}
							INNER JOIN ca_metadata_dictionary_rule_violations ON ca_metadata_dictionary_rule_violations.row_id = {$vs_browse_table_name}."
					          . $t_item->primaryKey() . " AND ca_metadata_dictionary_rule_violations.table_num = {$vs_browse_table_num}
							{$vs_join_sql}
							WHERE
								{$vs_where_sql}
							LIMIT 2";
					...
					while ( $qr_res->nextRow() ) {
						if ( $t_rule->load( $qr_res->get( 'rule_id' ) ) ) {
							if ( ! ( $vs_val = trim( caExtractSettingsValueByUserLocale( 'label',
								$t_rule->getSettings() ) ) )
							) {
								continue;
							}
			...
			case 'tags':
				...
				if ( isset( $va_facet_info['moderated'] ) ) {
					$va_wheres[] = (bool) $va_facet_info['moderated'] ? "(ca_items_x_tags.moderated_on > 0)"
						: "(ca_items_x_tags.moderated_on IS NULL)";
				}
				...
				if ( $vb_check_availability_only ) {
					$vs_sql = "
							SELECT 1
							FROM {$vs_browse_table_name}
							INNER JOIN ca_items_x_tags ON ca_items_x_tags.row_id = {$vs_browse_table_name}."
					          . $t_item->primaryKey() . " AND ca_items_x_tags.table_num = {$vs_browse_table_num}
							{$vs_join_sql}
							WHERE
								{$vs_where_sql}
							LIMIT 2";
					...
					switch ( $va_facet_info['mode'] ) {
						...
						case 'all':
							$vs_sql = "
									...
									{$vs_join_sql}
									WHERE
										ca_objects.deleted = 0 " . ( sizeof( $va_results ) ? "AND ("
							                                                                 . $t_subject->tableName()
							                                                                 . '.'
							                                                                 . $t_subject->primaryKey()
							                                                                 . " IN (" . join( ',',
										$va_results ) . "))" : "" ) . "
									LIMIT 2";
							break;
					...
					switch ( $va_facet_info['mode'] ) {
						case 'user':
							$vs_sql = "
									...
									{$vs_join_sql}
									WHERE
										{$vs_where_sql} " . ( ( sizeof( $va_results ) ? " AND ("
							                                                            . $t_subject->tableName() . '.'
							                                                            . $t_subject->primaryKey()
							                                                            . " IN (" . join( ',',
										$va_results ) . "))" : "" ) ) .
							...
							while ( $qr_res->nextRow() ) {
								...
								$vs_val     = $qr_res->get( 'fname' ) . ' ' . $qr_res->get( 'lname' ) . ( ( $vs_email
										= $qr_res->get( 'email' ) ) ? "({$vs_email})" : '' );
								if ( $va_criteria[ $vs_val ] ) {
									continue;
								}        // skip items that are used as browse critera - don't want to browse on something you're already browsing on
						...
						default:
							foreach (
								...
								if ( is_array( $va_results ) && sizeof( $va_results )
									...
									$vs_where .= " AND (" . $t_subject->tableName() . '.' . $t_subject->primaryKey()
									             . " IN (" . join( ',', $va_results ) . "))";
								}
			...
			case 'normalizedDates':
				...
				$vb_is_element = $vb_is_field = false;
				if ( ! ( $vb_is_element = $t_element->load( array( 'element_code' => array_pop( $va_element_code ) ) ) )
				     && ! ( $vb_is_field = ( $t_item->hasField( $va_facet_info['element_code'] )
				                             && ( $t_item->getFieldInfo( $va_facet_info['element_code'], 'FIELD_TYPE' )
				                                  === FT_HISTORIC_DATERANGE ) ) )
				) {
					return array();
				}
				...
				$vs_container_code = ( is_array( $va_element_code ) && ( sizeof( $va_element_code ) > 0 ) )
					? array_pop( $va_element_code ) : null;
				if ( $t_user && $t_user->isLoaded()
				     && ( $t_user->getBundleAccessLevel( $vs_browse_table_name, $vs_container_code )
				          < __CA_BUNDLE_ACCESS_READONLY__ )
				) {
					return [];
				}
				...
				if ( $vb_is_element ) {
					$va_joins = array(
						'INNER JOIN ca_attribute_values ON ca_attributes.attribute_id = ca_attribute_values.attribute_id',
						'INNER JOIN ' . $vs_browse_table_name . ' ON ' . $vs_browse_table_name . '.'
						. $t_item->primaryKey() . ' = ca_attributes.row_id AND ca_attributes.table_num = '
						. intval( $vs_browse_table_num )
					);
				} else {
					$va_joins = array();
				}
				...
				$va_wheres = array();
				$vs_normalization
				           = $va_facet_info['normalization'];    // how do we construct the date ranges presented to uses. In other words - how do we want to allow users to browse dates? By year, decade, century?
									...
									foreach ( array_keys( $va_criteria_values ) as $vs_date ) {
										if ( $o_tep->parse( $vs_date ) ) {
											...
											if ( is_null( $vn_min ) || ( $va_ts['start'] < $vn_min ) ) {
												$va_facet_info['minimum_date']
													    = $o_tep->getText( [ 'start_as_iso8601' => true ] );
												$vn_min = $va_ts['start'];
											}
											if ( is_null( $vn_max ) || ( $va_ts['end'] > $vn_max ) ) {
												$va_facet_info['maximum_date']
													    = $o_tep->getText( [ 'end_as_iso8601' => true ] );
												$vn_max = $va_ts['end'];
											}
										}
									}
			...
			case 'normalizedLength':
				...
				$va_wheres = array();
				$vs_normalization
				           = $va_facet_info['normalization'];    // how do we construct the dimensions ranges presented to users. In other words - what increments do we can to use to  browse measurments?
				...
				$vs_min_sql           = $vs_max_sql = '';
				$vo_minimum_dimension = caParseLengthDimension( caGetOption( 'minimum_dimension', $va_facet_info,
					"0 in" ) );
				$vo_maximum_dimension = caParseLengthDimension( caGetOption( 'maximum_dimension', $va_facet_info,
					"0 in" ) );
				...
				if ( $vb_check_availability_only ) {
					...
					while ( $qr_res->nextRow() ) {
						...
						$vn_normalized = ( floor( $vn_dim / $vn_increment_in_current_units )
						                   * $vn_increment_in_current_units );
						if ( isset( $va_criteria[ $vn_normalized ] ) ) {
							continue;
						}
						$vs_normalized_range_with_units = "{$vn_normalized} {$vs_units} - " . ( $vn_normalized
						                                                                        + $vn_increment_in_current_units )
						                                  . " {$vs_units}";
			...
			case 'normalizedWeight':
				...
				$vs_min_sql           = $vs_max_sql = '';
				$vo_minimum_dimension = caParseWeightDimension( caGetOption( 'minimum_dimension', $va_facet_info,
					"0 kg" ) );
				$vo_maximum_dimension = caParseWeightDimension( caGetOption( 'maximum_dimension', $va_facet_info,
					"0 kg" ) );
				if ( $vo_minimum_dimension ) {
				...
				if ( $vb_check_availability_only ) {
					...
					while ( $qr_res->nextRow() ) {
						...
						$vs_normalized_range_with_units                = "{$vn_normalized} {$vs_units} - "
						                                                 . ( $vn_normalized
						                                                     + $vn_increment_in_current_units )
						                                                 . " {$vs_units}";
						$va_values[ $vn_normalized ][ $vn_normalized ] = array(
			...
			case 'relationship_types':
				...
				if ( $t_user && $t_user->isLoaded()
				     && ( $t_user->getBundleAccessLevel( $vs_browse_table_name, $vs_rel_table_name )
				          < __CA_BUNDLE_ACCESS_READONLY__ )
				) {
					return [];
				}
				...
				//
				if ( ! is_array( $va_restrict_to_types = $this->_convertTypeCodesToIDs( $va_restrict_to_types,
					array( 'instance' => $t_rel_item, 'dontExpandHierarchically' => true ) ) )
				) {
					$va_restrict_to_types = array();
				}
				...
				$va_restrict_to_types_expanded = $this->_convertTypeCodesToIDs( $va_restrict_to_types,
					array( 'instance' => $t_rel_item ) );
				$va_exclude_types_expanded     = $this->_convertTypeCodesToIDs( $va_exclude_types,
					array( 'instance' => $t_rel_item ) );

				// look up relationship type restrictions
				$va_restrict_to_relationship_types = $this->_getRelationshipTypeIDs( $va_restrict_to_relationship_types,
					$va_facet_info['relationship_table'] );
				$va_exclude_relationship_types     = $this->_getRelationshipTypeIDs( $va_exclude_relationship_types,
					$va_facet_info['relationship_table'] );
				...
				if ( ! $va_facet_info['show_all_when_first_facet'] || ( $this->numCriteria() > 0 ) ) {
					if ( $browse_on_self_relation ) {
						$va_joins[] = 'INNER JOIN ' . $va_path[1] . ' ON (' . $va_path[0] . '.'
						              . $t_subject->primaryKey() . ' = ' . $va_path[1] . '.'
						              . $t_item_rel->getRightTableFieldName() . ' OR ' . $va_path[0] . '.'
						              . $t_subject->primaryKey() . ' = ' . $va_path[1] . '.'
						              . $t_item_rel->getLeftTableFieldName() . ")\n";
						$va_joins[] = 'INNER JOIN ' . $va_path[2] . ' AS ' . $va_path[2] . '_rel ON (' . $va_path[2]
						              . '_rel.' . $t_subject->primaryKey() . ' = ' . $va_path[1] . '.'
						              . $t_item_rel->getLeftTableFieldName() . ') OR (' . $va_path[2] . '_rel.'
						              . $t_subject->primaryKey() . ' = ' . $va_path[1] . '.'
						              . $t_item_rel->getRightTableFieldName() . ")\n";
					} else {
						$vs_cur_table = array_shift( $va_path );
					...
					if ( $browse_on_self_relation ) {
						...
						if ( $va_facet_info['show_all_when_first_facet'] ) {
							$va_path
								           = array_reverse( $va_path );        // in "show_all" mode we turn the browse on it's head and grab records by the "subject" table, rather than the browse table
							$vs_cur_table  = array_shift( $va_path );
							$vs_join_table = $va_path[0];
							$va_rel_info   = Datamodel::getRelationships( $vs_cur_table, $vs_join_table );
							$va_joins[]    = 'LEFT JOIN ' . $vs_join_table . ' ON ' . $vs_cur_table . '.'
							                 . $va_rel_info[ $vs_cur_table ][ $vs_join_table ][0][0] . ' = '
							                 . $vs_join_table . '.'
							                 . $va_rel_info[ $vs_cur_table ][ $vs_join_table ][0][1] . "\n";
						}
				...
				if ( is_array( $va_restrict_to_lists ) && ( sizeof( $va_restrict_to_lists ) > 0 )
				     && ( $vs_rel_item_table_name == 'ca_list_items' )
				) {
					$va_list_ids = array();
				...
				if ( $va_facet_info['type'] == 'relationship_types' ) {
					...
					$va_joins[]    = "INNER JOIN ca_relationship_types ON " . $t_item_rel->tableName()
					                 . ".type_id = ca_relationship_types.type_id";
					$va_joins[]
					               = "INNER JOIN ca_relationship_type_labels ON ca_relationship_type_labels.type_id = ca_relationship_types.type_id";
				...
				if ( is_array( $va_exclude_types ) && ( sizeof( $va_exclude_types ) > 0 )
					...
					$va_wheres[] = "{$vs_rel_item_table_name}.type_id NOT IN (" . join( ',',
							caGetOption( 'dont_include_subtypes', $va_facet_info, false ) ? $va_exclude_types
								: $va_exclude_types_expanded ) . ")";
				}
				...
				if ( isset( $pa_options['checkAccess'] ) && is_array( $pa_options['checkAccess'] )
					...
					$va_wheres[] = "(" . $vs_rel_item_table_name . ".access IN (" . join( ',',
							$pa_options['checkAccess'] )
					               . "))";                // exclude non-accessible authority items
					if ( ! $va_facet_info['show_all_when_first_facet'] || ( $this->numCriteria() > 0 ) ) {
						$va_wheres[] = "(" . $vs_browse_table_name . ".access IN (" . join( ',',
								$pa_options['checkAccess'] ) . "))";        // exclude non-accessible browse items
					}
				}
				...
				if ( $t_item_rel && isset( $va_facet_info['filter_on_interstitial'] )
					...
					foreach ( $va_facet_info['filter_on_interstitial'] as $vs_field_name => $va_values ) {
						...
						$va_joins[]  = "INNER JOIN ca_attributes c_a ON c_a.row_id = " . $t_item_rel->primaryKey( true )
						               . " AND c_a.table_num = " . $t_item_rel->tableNum();
				...
				if ( isset( $va_facet_info['groupings'] ) && is_array( $va_facet_info['groupings'] )
				     && sizeof( $va_facet_info['groupings'] )
				) {
				...
				if ( is_array( $va_exclude_relationship_types ) && ( sizeof( $va_exclude_relationship_types ) > 0 )
				     && is_object( $t_item_rel )
				) {
					$va_wheres[] = $t_item_rel->tableName() . ".type_id NOT IN (" . join( ',',
							$va_exclude_relationship_types ) . ")";
				...
				if ( $this->opo_config->get( 'perform_item_level_access_checking' ) ) {
					if ( $t_item = Datamodel::getInstanceByTableName( $vs_browse_table_name, true ) ) {
						...
						$va_joins[]  = 'LEFT JOIN ca_acl AS rel_acl ON ' . $vs_rel_item_table_name . '.'
						               . $t_rel_item->primaryKey() . ' = rel_acl.row_id AND rel_acl.table_num = '
						               . $t_rel_item->tableNum() . "\n";
						$va_wheres[] = "(
								((
				...
				if ( is_array( $va_criteria ) && sizeof( $va_criteria ) > 0 ) {
					$va_wheres[] = "(" . $vs_rel_item_table_name . "." . $t_rel_item->primaryKey() . " NOT IN ("
					               . join( ",", caQuoteList( array_map( "intval", array_keys( $va_criteria ) ) ) )
					               . "))";
				}
				...
				if ( $vb_check_availability_only ) {
					...
					if ( ! $va_facet_info['show_all_when_first_facet'] || ( $this->numCriteria() > 0 ) ) {
						$vs_sql = "
							SELECT COUNT(DISTINCT " . $t_item->primaryKey( true ) . ") _count, " . join( ', ',
								$va_selects ) . "
							FROM {$vs_browse_table_name}
							{$vs_join_sql}
								" . ( sizeof( $va_wheres ) ? ' WHERE ' : '' ) . join( " AND ", $va_wheres ) . "
					...
					while ( $qr_res->nextRow() ) {
						...
						if ( $va_facet_info['type'] === 'relationship_types' ) {
							if ( sizeof( $va_restrict_to_relationship_types )
							     && ! in_array( $va_fetched_row['rel_type_id'], $va_restrict_to_relationship_types )
							) {
								continue;
							}
							if ( sizeof( $va_exclude_relationship_types )
							     && in_array( $va_fetched_row['rel_type_id'], $va_exclude_relationship_types )
							) {
								continue;
							}
...
// 								}
							...
							$va_facet_items[ $va_fetched_row[ $vs_rel_pk ] ] = array(
								...
								'type_id'       => array(),
								'parent_id'     => $vb_rel_is_hierarchical ? $va_fetched_row[ $vs_hier_parent_id_fld ]
									: null,
							...
							if ( $checkAccess && isset( $va_fetched_row['access'] )
							     && ! in_array( (int) $va_fetched_row['access'], $checkAccess, true )
							) {
								$va_facet_items[ $va_fetched_row[ $vs_rel_pk ] ]['no_access'] = 1;
							}

							if ( ! is_null( $vs_single_value )
							     && ( $va_fetched_row[ $vs_rel_pk ] == $vs_single_value )
							) {
								$vb_single_value_is_present = true;
							}
						} else {
						...
						if ( $va_fetched_row['rel_type_id'] ) {
							$va_facet_items[ $va_fetched_row[ $vs_rel_pk ] ]['rel_type_id'][]
								                                                                                                     = $va_fetched_row['rel_type_id'];
					...
					if ( ! isset( $va_facet_info['dont_expand_hierarchically'] )
						...
						$va_ids       = $qr_res->getAllFieldValues( $vs_rel_pk );
						$qr_ancestors = call_user_func( $t_rel_item->tableName() . '::getHierarchyAncestorsForIDs',
							$va_ids, array( 'returnAs' => 'SearchResult' ) );
						...
						if ( $qr_ancestors ) {
							...
							while ( $qr_ancestors->nextHit() ) {
								...
								$ancestor_count = array_reduce( $children, function ( $c, $i ) use ( $va_facet_items ) {
									return isset( $va_facet_items[ $i ] ) ? $c + $va_facet_items[ $i ]['content_count']
										: $c;
								}, 0 );
								...
								$va_facet_items_ancestors[ $vn_ancestor_id ] = array(
									...
									'parent_id'     => $vb_rel_is_hierarchical
										? $qr_ancestors->get( "{$vs_hier_parent_id_fld}" ) : null,
									'hierarchy_id'  => ( $vb_rel_is_hierarchical && $vs_hier_id_fld )
										? $qr_ancestors->get( $vs_hier_id_fld ) : null,
									'rel_type_id'   => array(),
					...
					if ( sizeof( $va_row_ids = array_keys( $va_facet_items ) ) ) {
						if ( $vs_label_table_name = $t_rel_item->getLabelTableName() ) {
							...
							foreach ( $va_ordering_fields_to_fetch as $vs_sort_by_field ) {
								if ( $t_rel_item_label->hasField( $vs_sort_by_field ) ) {
									$va_orderbys[]
										= $va_label_selects[] = $vs_label_table_name . '.' . $vs_sort_by_field;
								} elseif ( $t_rel_item->hasField( $vs_sort_by_field ) ) {
									$va_orderbys[]
										= $va_label_selects[] = $vs_rel_item_table_name . '.' . $vs_sort_by_field;
								}
							}
							...
							$vs_sql = "
									...
									INNER JOIN " . $vs_rel_table_name . " AS {$vs_rel_item_table_name} ON "
							          . $vs_rel_item_table_name . ".{$vs_rel_pk} = {$vs_label_table_name}.{$vs_rel_pk}
										" . ( sizeof( $va_label_wheres ) ? ' WHERE ' : '' ) . join( " AND ",
									$va_label_wheres ) . "
										" . ( sizeof( $va_orderbys ) ? "ORDER BY " . join( ', ', $va_orderbys ) : '' )
							          . "";
							//print $vs_sql;
							...
							while ( $qr_labels->nextRow() ) {
								...
								if ( $natural_sort ) {
									$label_values['label_sort_']
										= caSortableValue( $va_fetched_row[ $vs_label_display_field ] );
								}

								$va_facet_item = array_merge( $va_facet_items[ $va_fetched_row[ $vs_rel_pk ] ],
									$label_values );
								...
								$va_facet[ $va_fetched_row[ $vs_rel_pk ] ][ $va_fetched_row['locale_id'] ]
									= $va_facet_item;
							}
						}
						...
						if ( sizeof( $va_attrs_to_fetch ) ) {
							...
							while ( $qr_attrs->nextRow() ) {
								...
								$va_facet[ $vn_id ][ $vn_locale_id ][ 'ca_attribute_'
								                                      . $va_fetched_row['element_id'] ][]
									= $va_fetched_row;
							}
						}
					}
			...
			case 'dupeidno':
				...
				if ( is_array( $va_results ) && sizeof( $va_results ) ) {
					$va_wheres[] = $vs_browse_table_name . "." . $t_item->primaryKey() . " IN (" . join( ",",
							$va_results ) . ")";
				}
	...
	protected function doGetResults( $po_result = null, $pa_options = null ) {
		...
		$vs_sort_direction = strtolower( caGetOption( 'sortDirection', $pa_options,
			caGetOption( 'sort_direction', $pa_options, null ) ) );

		$t_item       = Datamodel::getInstanceByTableName( $this->ops_browse_table_name, true );
		$vb_will_sort = ( $vs_sort
		                  && ( ( $this->getCachedSortSetting() != $vs_sort )
		                       || ( $this->getCachedSortDirectionSetting() != $vs_sort_direction ) ) );
		...
		if ( is_array( $va_results = $this->opo_ca_browse_cache->getResults() ) && sizeof( $va_results ) ) {
			if ( $vb_will_sort ) {
				$va_results = $this->sortHits( $va_results, $this->ops_browse_table_name, $vs_sort,
					$vs_sort_direction );
	 ...
	 * $pm_value is the value to apply; this is usually text or a number; for the "in" and "not in" operators this is a
	 * comma-separated list of string or numeric values
	 *
	 *
	 */
	 ...
	 *
	 * @param string $ps_subject_table_name Optional table name to get display name for (eg. ca_objects might return
	 *                                      "objects"); if not specified the table the instance is configured to browse
	 *                                      is used
	 *
	...
	public function getBrowseSubjectName( $ps_subject_table_name = null ) {
		...
		if ( is_array( $va_tmp = $this->opo_ca_browse_config->getAssoc( $ps_subject_table_name ) )
		     && ( isset( $va_tmp['name'] ) && $va_tmp['name'] )
		) {
			return $va_tmp['name'];
		}
	 ...
	 *
	 * @param array $pa_type_codes_or_ids List of type_id or code values to filter browse by. When set, the browse will
	 *                                    only consider items of the specified types. Using a hierarchical parent type
	 *                                    will automatically include its children in the restriction.
	 * @param array $pa_options           Options include
	...
	public function setTypeRestrictions( $pa_type_codes_or_ids, $pa_options = null ) {
		$this->opa_browse_type_ids = $this->_convertTypeCodesToIDs( $pa_type_codes_or_ids,
			array( 'dontExpandHierarchically' => caGetOption( 'dontExpandHierarchically', $pa_options, false ) ) );
	...
	private function _convertTypeCodesToIDs( $pa_type_codes_or_ids, $pa_options = null ) {
		...
		foreach ( $pa_type_codes_or_ids as $vs_code_or_id ) {
			...
			if ( isset( $va_type_list[ $vn_type_id ] )
			     && $va_type_list[ $vn_type_id ]
			) {    // is valid type for this subject
				// See if there are any child types
				if ( ( ! caGetOption( 'dontExpandHierarchically', $pa_options, false ) )
				     && ! $this->opb_dont_expand_type_restrictions
				) {
	...
	public function getTypeRestrictionList() {
		if ( function_exists( "caGetTypeRestrictionsForUser" ) ) {
			$va_pervasive_types
				= caGetTypeRestrictionsForUser( $this->ops_tablename );    // restrictions set in app.conf or by associated user role
	 ...
	 *
	 * @param array $pa_source_codes_or_ids List of source_id or code values to filter browse by. When set, the browse
	 *                                      will only consider items of the specified sources. Using a hierarchical
	 *                                      parent source will automatically include its children in the restriction.
	 * @param array $pa_options             Options include
	 *                                      includeSubsources = include any child sources in the restriction. Default
	 *                                      is true.
	 *
	 * @return boolean True on success, false on failure
	 */
	...
	public function getSourceRestrictionList() {
		if ( function_exists( "caGetSourceRestrictionsForUser" ) ) {
			$va_pervasive_sources
				= caGetSourceRestrictionsForUser( $this->ops_tablename );    // restrictions set in app.conf or by associated user role
	...
	private function _getRelationshipTypeIDs( $pa_relationship_types, $pm_relationship_table_or_id ) {
		...
		foreach ( $va_type_list as $vn_i => $vm_type ) {
			...
			if ( ! is_numeric( $vm_type ) ) {
				...
				if ( ! ( $vn_type_id = $t_rel_type->getRelationshipTypeID( $pm_relationship_table_or_id,
					$vm_type ) )
				) {
					unset( $pa_relationship_types[ $vn_i ] );
					continue;
	...
	private function _getRelativeFacetSQLData( $ps_relative_to_table, $pa_options ) {
		...
		if ( $vs_relative_to_table = $this->_isParentRelative( $ps_relative_to_table ) ) {
			...
			$va_joins[] = "INNER JOIN {$ps_relative_to_table} ON " . $this->ops_browse_table_name
		...
		switch ( sizeof(
			$va_path = array_keys( Datamodel::getPath( $ps_relative_to_table, $this->ops_browse_table_name ) ) ) ) {
			case __CA_ATTRIBUTE_VALUE_LIST__:
				$t_item_rel = Datamodel::getInstanceByTableName( $va_path[1], true );
				$t_rel_item = Datamodel::getInstanceByTableName( $va_path[2], true );
				$vs_key     = 'relation_id';
	...
	private function _getRelativeExecuteSQLData( $ps_relative_to_table, $pa_options = null ) {
		if ( $vs_relative_to_table = $this->_isParentRelative( $ps_relative_to_table ) ) {
			...
			$va_relative_joins[] = "INNER JOIN {$ps_relative_to_table} AS parent ON " . $this->ops_browse_table_name
			                       . ".parent_id = parent.object_id";
			$va_wheres           = [];
			...
			return array(
				'joins'             => [],
				'wheres'            => $va_wheres,
				'relative_joins'    => $va_relative_joins,
		...
		if ( $t_item_rel ) { // foo_x_bar Table exists ==> join foo_x_bar and relative_to table
			$va_relative_to_join[] = "INNER JOIN " . $t_item_rel->tableName() . " ON " . $t_item_rel->tableName() . "."
			                         . $t_item->primaryKey() . " = " . $this->ops_browse_table_name . '.'
			                         . $t_item->primaryKey();
			$va_relative_to_join[]
			                       = "INNER JOIN {$ps_relative_to_table} ON {$ps_relative_to_table}.{$vs_target_browse_table_pk} = "
			                         . $t_item_rel->tableName() . "." . $t_target->primaryKey();
			...
			$va_relative_to_join[]
			             = "INNER JOIN {$ps_relative_to_table} ON {$ps_relative_to_table}.{$va_rel_info[$t_rel_item->tableName()][$ps_relative_to_table][0][0]} = {$t_rel_item->tableName()}.{$va_rel_info[$ps_relative_to_table][$t_rel_item->tableName()][0][0]}";
		}

		return array(
			'joins'             => $va_joins,
			'wheres'            => $va_wheres,
			'relative_joins'    => $va_relative_to_join,
...
// 								break;
	...
	public function getAvailableRelationshipTypesForCurrentResult( $ps_rel_table, $pa_rel_ids, $pa_options = null ) {
		...
		if ( is_array( $va_results = $this->opo_ca_browse_cache->getResults() ) && sizeof( $va_results ) ) {
			switch ( sizeof(
				$va_path = array_keys( Datamodel::getPath( $this->ops_browse_table_name, $ps_rel_table ) ) ) ) {
				case 3:
