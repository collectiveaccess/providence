...
class BrowseEngine extends BaseFindEngine
{
    ...
    private function _processBrowseSettings()
    {
        ...
        foreach ($this->opa_browse_settings['facets'] as $vs_facet_name => $va_facet_info) {
            ...
            if (($va_facet_info['type'] === 'location') && (caGetOption(
                        'group_mode',
                        $va_facet_info,
                        null
                    ) == 'hierarchical')) {
                if ((is_array(
                        $current_location_criteria = $this->opo_config->get('current_location_criteria')
                    )) && (sizeof(
                            $current_location_criteria
                        ) == 1) && isset($current_location_criteria['ca_storage_locations'])) {
        ...
        switch ($va_facet_info['type']) {
            ...
            case 'location':
                ...
                if (
                    (sizeof($va_row_tmp) < 3)
                    &&
                    ($object_location_tracking_relationship_type = $this->opo_config->get(
                        'object_storage_location_tracking_relationship_type'
                    ))
                    &&
                    ($object_location_tracking_relationship_type_id = array_shift(
                        caMakeRelationshipTypeIDList(
                            'ca_objects_x_storage_locations',
                            [$object_location_tracking_relationship_type]
                        )
                    ))
                ) {
                    //
                    ...
                    if (sizeof($va_row_tmp) < 3) {
                        array_unshift($va_row_tmp, 119);
                    }    // assume ca_objects_x_storage_locations
                }
                    ...
                    switch ($va_facet_info['type']) {
                        ...
                        case 'has':
                            ...
                            if ($vn_state) {
                                ...
                                $qr_res = $this->opo_db->query(
                                    "SELECT t." . $t_item->primaryKey(
                                    ) . " FROM " . $this->ops_browse_table_name . " t LEFT JOIN _browseTmp AS b ON " . $t_item->primaryKey(
                                    ) . " = b.row_id WHERE b.row_id IS NULL"
                                );
                        ...
                        case 'relationship_types':
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                if ($va_facet_info['type'] == 'relationship_types') {
                                    ...
                                    $vs_get_item_sql = "{$vs_rel_table_name}.{$vs_hier_left_fld} >= " . $t_rel_item->get(
                                            $vs_hier_left_fld
                                        ) . " AND {$vs_rel_table_name}.{$vs_hier_right_fld} <= " . $t_rel_item->get(
                                            $vs_hier_right_fld
                                        );
                                    if ($vn_hier_id_fld = $t_rel_item->getProperty('HIERARCHY_ID_FLD')) {
                                        $vs_get_item_sql .= " AND {$vs_rel_table_name}.{$vn_hier_id_fld} = " . (int)$t_rel_item->get(
                                                $vn_hier_id_fld
                                            );
                                    }
                            ...
                            foreach ($va_row_ids as $vn_row_id) {
                                ...
                                if ($va_row_tmp[0] == 119) { // ca_objects_x_storage_locations
                                    ...
                                    $vs_sql = "
                                        ...
                                        . ((sizeof(
                                                $va_row_tmp
                                            ) == 2) ? " AND ({$this->ops_browse_table_name}.current_loc_id IN (?))" : "")
                                        . ((sizeof(
                                                $va_row_tmp
                                            ) > 2) ? " AND ({$this->ops_browse_table_name}.current_loc_subclass = ?) AND ({$this->ops_browse_table_name}.current_loc_id IN (?))" : "");
                                    ...
                                    $vs_sql = "
                                        ...
                                        . ((sizeof(
                                                $va_row_tmp
                                            ) > 1) ? " AND ({$this->ops_browse_table_name}.current_loc_subclass = ?)" : "")
                                        . ((sizeof(
                                                $va_row_tmp
                                            ) > 2) ? " AND ({$this->ops_browse_table_name}.current_loc_id = ?)" : "");
                                    $qr_res = $this->opo_db->query($vs_sql, $va_row_tmp);
                                }
                        ...
                        case 'dupeidno':
                            ...
                            if (is_array($va_restrict_to_types) && sizeof($va_restrict_to_types)) {
                                $va_wheres[] = "{$va_restrict_to_types}.type_id IN (" . join(
                                        ",",
                                        $va_restrict_to_types
                                    ) . ")";
                            }
    ...
    public function getFacetContent($ps_facet_name, $pa_options = null)
    {
        ...
        if (isset($va_facet_info['multiple']) && $va_facet_info['multiple'] && isset($va_full_criteria[$ps_facet_name])) {
            $pa_options['returnFullFacet'] = true;
        }
        ...
        switch ($va_facet_info['type']) {
                ...
                if ($va_facet_info['element_code']) {
                    ...
                    foreach ($va_facet_values as $vs_state_name => $va_state_info) {
                        ...
                        if (!(bool)$va_state_info['id']) {    // no option
                            ...
                            $va_joins[] = "LEFT JOIN ca_attributes AS caa ON  " . $this->ops_browse_table_name . '.' . $t_item->primaryKey(
                                ) . " = caa.row_id AND " . $t_item->tableNum() . " = caa.table_num";
                    ...
                    foreach ($va_facet_values as $vs_state_name => $va_state_info) {
                        ...
                        if (!(bool)$va_state_info['id']) {    // no option
                            ...
                            if ($t_rel_item->hasField('deleted')) {
                                $va_wheres[] = "((" . $t_rel_item->tableName(
                                    ) . ".deleted = 0) OR (" . $t_rel_item->tableName() . ".deleted IS NULL))";
                            }
                            if (isset($pa_options['checkAccess']) && is_array($pa_options['checkAccess']) && sizeof(
                                    $pa_options['checkAccess']
                                ) && $t_rel_item->hasField('access')) {
                                $va_wheres[] = "((" . $t_rel_item->tableName() . ".access IN (" . join(
                                        ',',
                                        $pa_options['checkAccess']
                                    ) . ")) OR (" . $t_rel_item->tableName() . ".access IS NULL))";
                            }
                            ...
                            if (is_array($va_restrict_to_relationship_types) && (sizeof(
                                ...
                                $va_wheres[] = "((" . $t_item_rel->tableName() . ".type_id NOT IN (" . join(
                                        ',',
                                        $va_restrict_to_relationship_types
                                    ) . ")) OR (" . $t_item_rel->tableName() . ".type_id IS NULL))";
                            }
                            if (is_array($va_exclude_relationship_types) && (sizeof(
                                        $va_exclude_relationship_types
                                    ) > 0) && is_object($t_item_rel)) {
                ...
                if ($vb_check_availability_only) {
                    ...
                    while ($qr_res->nextRow()) {
                        ...
                        if (!isset($va_values[$vs_label_key][$qr_res->get('locale_id')])) {
                            $va_values[$vs_label_key][$qr_res->get('locale_id')] = array_merge(
                                $qr_res->getRow(),
                                array(
            ...
            case 'attribute':
                ...
                if ($vb_check_availability_only) {
                    ...
                    if (is_array($va_criteria) && sizeof($va_criteria)) {
                        $vs_criteria_exclude_sql = ' AND (ca_attribute_values.value_longtext1 NOT IN (' . join(
                                ", ",
                                caQuoteList(
                                    array_keys(
                                        $va_criteria
                    ...
                    foreach ($va_values_by_table as $vs_loc_class => $va_loc_id_by_subclass) {
                        foreach ($va_loc_id_by_subclass as $vs_loc_subclass => $va_loc_ids) {
                            if (sizeof($va_tmp = array_keys($va_loc_ids))) {
                                ...
                                $va_config = ca_objects::getConfigurationForCurrentLocationType(
                                    $vs_table_name,
                                    $vs_loc_subclass,
                                ...
                                if ($vs_hier_table_name == 'ca_storage_locations') {
                                    if (!($vn_max_browse_depth = caGetOption(
                                        'maximumBrowseDepth',
                                        $va_facet_info,
                                        null
                                    ))) {
                                    ...
                                    if (!$vn_max_browse_depth) {
                                        ...
                                        $vn_max_browse_depth++;
                                    }    // add one to account for invisible root
                                    ...
                                    while ($qr_res->nextHit()) {
                                        if (is_array(
                                            $va_ids = $qr_res->get(
                                                "{$vs_hier_table_name}.hierarchy.{$vs_hier_pk}",
                                                ['returnAsArray' => true, 'maxLevelsFromBottom' => $vn_max_browse_depth]
                                            )
                                        )) {
                                ...
                                $vs_template = strip_tags(
                                    isset($va_config['template']) ? $va_config['template'] : "^{$vs_table_name}.preferred_labels"
                                );
                                ...
                                while ($qr_res->nextHit()) {
                                    ...
                                    if (isset($va_collapse_map[$vs_table_name]) && isset($va_collapse_map[$vs_table_name][$vs_loc_subclass]) && $va_collapse_map[$vs_table_name][$vs_loc_subclass]) {
                                        if (!($vs_label = $va_collapse_map[$vs_table_name][$vs_loc_subclass])) {
                                            continue;
                                        }
                                    ...
                                    if (!$vn_id || !($vs_label = $qr_res->getWithTemplate($vs_template, $va_config))) {
                                        continue;
                                    }
            ...
            case 'relationship_types':
                ...
                if (is_array($va_restrict_to_lists) && (sizeof($va_restrict_to_lists) > 0) && ($t_rel_item->tableName(
                        ) == 'ca_list_items')) {
                    $va_list_ids = array();
                ...
                if (is_array($va_restrict_to_types) && (sizeof($va_restrict_to_types) > 0) && method_exists(
                    ...
                    $va_wheres[] = "{$vs_rel_table_name}.type_id IN (" . join(
                            ',',
                            caGetOption(
                                'dont_include_subtypes',
                                $va_facet_info,
                            ...
                            'type_id',
                            'IS_NULL'
                        ) ? " OR ({$vs_rel_table_name}.type_id IS NULL)" : '');
                ...
                if (is_array($va_exclude_types) && (sizeof($va_exclude_types) > 0) && method_exists(
                    ...
                    $va_wheres[] = "{$vs_rel_table_name}.type_id NOT IN (" . join(
                            ',',
                            caGetOption(
                                'dont_include_subtypes',
                                $va_facet_info,
...
//}
                $va_selects[] = $t_rel_item->tableName(
                    ) . '.' . $vs_rel_pk;                // get primary key of related
                ...
                if ($this->opo_config->get('perform_item_level_access_checking')) {
                    if ($t_item = Datamodel::getInstanceByTableName($vs_browse_table_name, true)) {
                        ...
                        $va_joins[] = 'LEFT JOIN ca_acl AS rel_acl ON ' . $t_rel_item->tableName(
                ...
                if (is_array($va_criteria) && sizeof($va_criteria) > 0) {
                    $va_wheres[] = "(" . $t_rel_item->tableName() . "." . $t_rel_item->primaryKey(
                        ) . " NOT IN (" . join(",", caQuoteList(array_map("intval", array_keys($va_criteria)))) . "))";
                }
                ...
                if ($vb_check_availability_only) {
                    ...
                    while ($qr_res->nextRow()) {
                        ...
                        if (!$va_facet_items[$va_fetched_row[$vs_rel_pk]]) {
                            if (is_array($va_restrict_to_types) && sizeof(
                                    $va_restrict_to_types
                                ) && $va_fetched_row['type_id'] && !in_array(
                                    $va_fetched_row['type_id'],
                                    $va_restrict_to_types
                                )) {
                                continue;
                            }
                        ...
                        if ($vs_label_table_name = $t_rel_item->getLabelTableName()) {
                            ...
                            foreach ($va_ordering_fields_to_fetch as $vs_sort_by_field) {
                                if ($t_rel_item_label->hasField($vs_sort_by_field)) {
                                    ...
                                    $va_orderbys[] = $va_label_selects[] = $t_rel_item->tableName(
                                        ) . '.' . $vs_sort_by_field;
                                }
                            }
                ...
                if (sizeof($va_selector) == 1) {
                    ...
                    switch ($va_selector[0]) {
                        case 'ca_objects_x_storage_locations':
                            $t_rel_type = new ca_relationship_types();
                            $vn_type_id = $t_rel_type->getRelationshipTypeID(
                                'ca_objects_x_storage_locations',
                                $va_selector[1]
                            );
                            break;
