...
class BrowseCache
{
        ...
        if (ExternalCache::contains($ps_cache_key, 'Browse')) {
            ...
            if (caGetOption('removeDeletedItems', $pa_options, true)) {
                if (($t_instance = Datamodel::getInstanceByTableNum(
                        $this->opa_browse['params']['table_num'],
                        true
                    ...
                    if (is_array($va_ids = $this->opa_browse['results']) && sizeof($va_ids)) {
                        ...
                        $qr_deleted = $t_instance->getDb()->query(
                            $x = "
    ...
    {
        $this->opa_browse['source_restrictions'] = is_array(
            $pa_source_restrictions
        ) ? $pa_source_restrictions : array();
        return true;
    }
    # ------------------------------------------------------
