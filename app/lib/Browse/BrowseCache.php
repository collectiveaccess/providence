...
class BrowseCache {
	 ...
	 *                           removeDeletedItems = remove any items in the cache that are currently marked as
	...
	public function load( $ps_cache_key, $pa_options = null ) {
		if ( ExternalCache::contains( $ps_cache_key, 'Browse' ) ) {
			...
			if ( caGetOption( 'removeDeletedItems', $pa_options, true ) ) {
				if ( ( $t_instance = Datamodel::getInstanceByTableNum( $this->opa_browse['params']['table_num'],
						true ) )
				     && ( $t_instance->hasField( 'deleted' ) )
				) {
	...
	public function getFacet( $ps_facet_name ) {
		return ( isset( $this->opa_browse['facets'][ $ps_facet_name ] ) ? $this->opa_browse['facets'][ $ps_facet_name ]
			: null );
	}
	# ------------------------------------------------------
	...
	public function setSourceRestrictions( $pa_source_restrictions ) {
		$this->opa_browse['source_restrictions'] = is_array( $pa_source_restrictions ) ? $pa_source_restrictions
			: array();
	...
	public function getSourceRestrictions() {
		return is_array( $this->opa_browse['source_restrictions'] ) ? $this->opa_browse['source_restrictions']
			: array();
	}
	# ------------------------------------------------------
	...
	public static function makeCacheKey( $pa_params, $pa_type_restrictions, $pa_source_restrictions ) {
		...
		return md5( $pa_params['context'] . '/' . $pa_params['table_num'] . '/' . print_r( $pa_params['criteria'],
				true ) . '/' . print_r( $pa_type_restrictions, true ) . '/' . print_r( $pa_source_restrictions,
				true ) );
	}
	# ------------------------------------------------------
