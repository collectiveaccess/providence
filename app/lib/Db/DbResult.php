...
class DbResult extends DbBase {
	...
	function __construct( &$po_db, $pr_res ) {
		...
		if ( ! isset( $GLOBALS["_DbResult_time_expression_parser"] )
		     || ! $GLOBALS["_DbResult_time_expression_parser"]
		) {
	...
	function get( $ps_field, $pa_options = null ) {
		...
		$va_field = isset( DbResult::$s_field_info_cache[ $ps_field ] ) ? DbResult::$s_field_info_cache[ $ps_field ]
			: $this->getFieldInfo( $ps_field );

		if ( ! isset( $this->opa_current_row[ $va_field["field"] ] ) ) {
			return null;
		}

		$vs_val = isset( $this->opa_current_row[ $va_field["field"] ] ) ? $this->opa_current_row[ $va_field["field"] ]
			: null;
		...
		if ( isset( $pa_options["unserialize"] ) && $pa_options["unserialize"] ) {
			if ( ! isset( $this->opa_unserialized_cache[ $va_field["field"] ] )
			     || ! ( $vm_data
					= $this->opa_unserialized_cache[ $va_field["field"] ] )
			) {
	...
	/**
	 * Return all rows from the result set as a list. Each item in the list is an array with keys set to field names
	 * and values set to field values
	 *
	...
	/**
	 * Returns a list of values for the specified field from all rows in the result set. If you need to extract all
	 * values from single field in a result set this method provides a convenient means to do so.
	 *
	...
	public function getAllFieldValues( $pm_field, $pa_options = null ) {
		...
		if ( is_array( $pm_field ) ) {
			...
			foreach ( $pm_field as $vs_field ) {
				$va_field   = isset( DbResult::$s_field_info_cache[ $vs_field ] )
					? DbResult::$s_field_info_cache[ $vs_field ] : $this->getFieldInfo( $vs_field );
				$vm_field[] = $va_field['field'];
			}
		} else {
			$va_field = isset( DbResult::$s_field_info_cache[ $pm_field ] ) ? DbResult::$s_field_info_cache[ $pm_field ]
				: $this->getFieldInfo( $pm_field );
			$vm_field = $va_field['field'];
		}
	...
	function getFieldInfo( $ps_field ) {
		...
		switch ( sizeof( $va_tmp ) ) {
			case 1:        // query field name (no table specified, in other words)
				return DbResult::$s_field_info_cache[ $ps_field ] = array(
					"table"    => null,
					"field"    => $ps_field,
					"instance" => null
				);
	 ...
	 *  VOLUME => the logical volume the derived file is stored on; corresponds to a name defined in the media_volumes
	 *  configuration file MIMETYPE => the mimetype of the derived file WIDTH => the width, in pixels, of the derived
	 *  file HEIGHT => the height, in pixels, of the derived file PROPERTIES => an associative array of format-specific
	 *  information about the media; the format of this array varies by media processing plug-in FILENAME => the file
	 *  name of the derived file, including the file extension HASH => a series of nested numeric directories, based
	 *  upon the primary key of the row the media file is part of, that the derived file is contained within. The
	 *  directory nesting ensures that no single directory in the file system ever has too many files MAGIC => a random
	 *  number prepended to the file name to prevent browser caching of outdated media and to prevent easy "sucking" of
	 *  media off of a logical volume by making it hard to guess file names EXTENSION => the file extension used for
	 *  the derived file MD5 => md5 hash for derived file
	 *
	 * Note that while you can derive the path to a derived file by putting path information from the volume together
	 * with the hash, magic and file name, you are much better off using the getMediaPath() method
	 *
	...
	function getMediaInfo( $ps_field, $ps_version = null, $ps_key = null ) {
		...
		return $GLOBALS["_DbResult_mediainfocoder"]->getMediaInfo( $ps_version, $ps_key,
			[ 'data' => $this->opa_media_info_cache[ $ps_field ] ] );
	}
	...
	function getMediaPath( $ps_field, $ps_version, $pa_options = null ) {
		...
		return $GLOBALS["_DbResult_mediainfocoder"]->getMediaPath( $ps_version,
			array_merge( $pa_options, [ 'data' => $this->opa_media_info_cache[ $ps_field ] ] ) );
	}
	...
	/**
	 * Returns an HTML tag for the specified version in a media field. For images this will be an <img> tag, for
	 * andio/video this will usually be some kind of HTML embedding tag, but can vary from media type to media type.
	 *
	 * An optional options array may be passed specifying opp. The options vary by media type but usually include the
	 * following basic HTML attributes:
	 *
	 ...
	 *
	 * If the field is not declared as FT_MEDIA in the table's class definition or the version is invalid then the
	...
	function getMediaTag( $ps_field, $ps_version, $pa_options = null ) {
		...
		return $GLOBALS["_DbResult_mediainfocoder"]->getMediaTag( $ps_version,
			array_merge( $pa_options, [ 'data' => $this->opa_media_info_cache[ $ps_field ] ] ) );
	}
	...
	/**
	 * The returns the number of external servers the specified media field version is configured to be mirrored to. If
	 * the version is not configured for mirroring, then returns zero.
	 *
	 * @access public
	 *
	 * @param string tablename.fieldname, e.g. object_representations.media
	 ...
	 * ORIGINAL_FILENAME => original name of uploaded file
	 * VOLUME => the logical volume the uploaded file is stored on; corresponds to a name defined in the file_volumes
	 * configuration file MIMETYPE => he mimetype of the uploaded file FILENAME => the file name of the uploaded file,
	 * including the file extension HASH => a series of nested numeric directories, based upon the primary key of the
	 * row the file is part of, that the uploaded file is contained within. The directory nesting ensures that no
	 * single directory in the file system ever has too many files MAGIC => a random number prepended to the file name
	 * to prevent browser caching of outdated media and to prevent easy "sucking" of media off of a logical volume by
	 * making it hard to guess file names PROPERTIES => an associative array of format-specific information about the
	 * file; for unrecognized formats this will be minimal; the format of this array varies by file processing plug-in
	 * DANGEROUS => will evaluate to true if the file is considered "dangerous" (can be executed on the server); an
	 * additional non-executable file extension is automatically added to such files CONVERSIONS => an array list of
	 * format conversions performed upon upload; not all files can be converted other formats, and conversions are never guaranteed to succeed in any event (this is different from FT_MEDIA fields where conversions are guaranteed to succeed otherwise the uploaded file is rejected) Each item in the CONVERSIONS list is an associative array with the following keys: MIMETYPE => the mimetype of the converted file FILENAME => the filename of the converted file (the volume and hash are the same as the original file) PROPERTIES => an associative array of information about the converted file, including these keys: filesize => size of file in bytes extension => file extension of file format_name => short display-able name of format file conversion is in long_format_name => more descriptive name of format MD5 => md5 hash for the uploaded file
	 *
	 *
	...
	function getFileInfo( $ps_field ) {
		...
		return $GLOBALS["_DbResult_fileinfocoder"]->getFileInfo( $this->get( $va_field["field"],
			array( "unserialize" => true ) ) );
	}
	...
	function getFilePath( $ps_field ) {
		...
		return $GLOBALS["_DbResult_fileinfocoder"]->getFilePath( $this->get( $va_field["field"],
			array( "unserialize" => true ) ) );
	}
	...
	function getFileUrl( $ps_field ) {
		...
		return $GLOBALS["_DbResult_fileinfocoder"]->getFileUrl( $this->get( $va_field["field"],
			array( "unserialize" => true ) ) );
	}
	...
	function hasFile( $ps_field ) {
		...
		return $GLOBALS["_DbResult_fileinfocoder"]->hasFile( $this->get( $va_field["field"],
			array( "unserialize" => true ) ) );
	}

	/**
	 * Returns an array list of available conversions based upon the uploaded file. Each item in the list contains the
	 * following keys:
	 *
	...
	function getFileConversions( $ps_field ) {
		...
		return $GLOBALS["_DbResult_fileinfocoder"]->getFileConversions( $this->get( $va_field["field"],
			array( "unserialize" => true ) ) );
	}
	...
	function getFileConversionPath( $ps_field, $ps_mimetype ) {
		...
		return $GLOBALS["_DbResult_fileinfocoder"]->getFileConversionPath( $this->get( $va_field["field"],
			array( "unserialize" => true ) ), $ps_mimetype );
	}
	...
	function getFileConversionUrl( $ps_field, $ps_mimetype ) {
		...
		return $GLOBALS["_DbResult_fileinfocoder"]->getFileConversionUrl( $this->get( $va_field["field"],
			array( "unserialize" => true ) ), $ps_mimetype );
	}

	/**
	 * Converts a date field into text for display. Note that dates are stored as either traditional Unix timestamps
	 * or as "historic" dates in a floating point format. Date ranges are stored as a pair of fields, single dates in a
	 * single field. This method automatically deals with Unix timestamps vs. historic dates, but care must be taken
	 * when dealing with date ranges. The field name for a single date is simply the field name, of course. But for
	 * ranges, the field name you pass in the first parameter is the *virtual* field for the range name defined in the
	 * table's class definition. This virtual field is composed of the two fields used to store the range data. So, in
	 * other words, for date ranges the field name you pass to this method will *not* exist in the database, but rather
	 * will be defined in the table class definition and point to two fields that do in fact exist in the database.
	 *
	 * @access public
	 *
	 * @param string tablename.fieldname, e.g. object_documents.file_info
	 * @param array options; valid options include
	 *        getRawDate => returns raw numeric date data as set in the database; for single dates this is a number;
	 *        for date ranges this will be an array including the start and end dates
	 *
	 * @return string
	 */
	...
	/**
	 * For text and numeric fields with a static choice list defined with a BOUNDS_CHOICE_LIST setting in the table
	 * class definition this method will convert the value of the field into the display value of the choice list. For
	 * example, if you have a choice list with values 0, 1 and 2 and corresponding display options ('New', 'First
	 * edit', 'Complete'), then if the value in the database is 2, this function will return 'Complete'
	 *
	 * @access public
	 *
