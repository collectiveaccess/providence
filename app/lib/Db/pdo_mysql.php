...
class Db_pdo_mysql extends DbDriverBase {
	...
	public function connect( $po_caller, $pa_options ) {
		...
		if ( ! class_exists( "PDO" ) ) {
			throw new DatabaseException( _t( "Your PHP installation lacks PDO MySQL support. Please add it and retry..." ),
				200, "Db->pdo_mysql->connect()" );
		}

		try {
			$this->opr_db = new PDO( 'mysql:host=' . $pa_options["host"] . ';dbname=' . $pa_options["database"],
				$pa_options["username"], $pa_options["password"], array(
	...
	public function execute( $po_caller, $opo_statement, $ps_sql, $pa_values, $pa_options = null ) {
		...
		try {
			...
			$opo_statement->execute( ( is_array( $va_tmp['values'] ) && sizeof( $va_tmp['values'] ) )
				? array_values( $va_tmp['values'] ) : null );
		} catch ( PDOException $e ) {
			$po_caller->postError( $this->nativeToDbError( $this->opr_db->errorCode() ), $e->getMessage(),
				"Db->pdo_mysql->execute()" );
			throw new DatabaseException( $e->getMessage(), $this->nativeToDbError( $this->opr_db->errorCode() ),
				"Db->pdo_mysql->execute()" );
	...
	public function beginTransaction( $po_caller ) {
		try {
			...
			$po_caller->postError( 250, _t( "Could not start transaction: %1", $e->getMessage() ),
				"Db->pdo_mysql->beginTransaction()" );
			throw new DatabaseException( _t( "Could not start transaction: %1", $e->getMessage() ), 250,
				"Db->pdo_mysql->beginTransaction()" );
	...
	public function commitTransaction( $po_caller ) {
		try {
			...
			$po_caller->postError( 250, "Could not commit transaction: " . $e->getMessage(),
				"Db->pdo_mysql->commitTransaction()" );
			throw new DatabaseException( _t( "Could not commit transaction: %1", $e->getMessage() ), 250,
				"Db->pdo_mysql->commitTransaction()" );
	...
	public function rollbackTransaction( $po_caller ) {
		if ( ! $this->opr_db->rollBack() ) {
			...
			throw new DatabaseException( _t( "Could not rollback transaction: %1", $e->getMessage() ), 250,
				"Db->pdo_mysql->rollbackTransaction()" );
	...
	public function nextRow( $po_caller, $po_stmt ) {
		if ( ! ( $po_stmt instanceof PDOStatement ) ) {
			return null;
		}
	...
	private static function rewriteQueryAndParams( $ps_sql, $pa_values ) {
		...
		foreach ( $pa_values as $vn_i => $vm_val ) {
			...
			if ( is_array( $vm_val ) ) {
				if ( count( $vm_val ) == 0 ) {
					return false;
				} // don't allow empty value arrays
