...
class Db_pdo_mysql extends DbDriverBase
{
    ...
    public function connect($po_caller, $pa_options)
    {
        ...
        if (!class_exists("PDO")) {
            throw new DatabaseException(
                _t("Your PHP installation lacks PDO MySQL support. Please add it and retry..."),
                200,
        ...
        try {
            $this->opr_db = new PDO(
                'mysql:host=' . $pa_options["host"] . ';dbname=' . $pa_options["database"],
                $pa_options["username"],
                $pa_options["password"],
                array(
    ...
    public function execute($po_caller, $opo_statement, $ps_sql, $pa_values, $pa_options = null)
    {
        ...
        try {
            ...
            $opo_statement->execute(
                (is_array($va_tmp['values']) && sizeof($va_tmp['values'])) ? array_values($va_tmp['values']) : null
            );
        } catch (PDOException $e) {
            $po_caller->postError(
                $this->nativeToDbError($this->opr_db->errorCode()),
                $e->getMessage(),
    ...
    public function beginTransaction($po_caller)
    {
        try {
            ...
            $po_caller->postError(
                250,
                _t("Could not start transaction: %1", $e->getMessage()),
    ...
    public function commitTransaction($po_caller)
    {
        try {
            ...
            throw new DatabaseException(
                _t("Could not commit transaction: %1", $e->getMessage()),
                250,
                "Db->pdo_mysql->commitTransaction()"
            );
            return false;
    ...
    public function rollbackTransaction($po_caller)
    {
        if (!$this->opr_db->rollBack()) {
            ...
            throw new DatabaseException(
                _t("Could not rollback transaction: %1", $e->getMessage()),
                250,
     ...
     */
    public function getAllFieldValues($po_caller, $pr_res, $pm_field, $pa_options = null)
    {
        $va_vals = array();
