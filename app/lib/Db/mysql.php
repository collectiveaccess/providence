...
class Db_mysql extends DbDriverBase
{
    ...
    public function connect($po_caller, $pa_options)
    {
        ...
        if (!$vb_unique_connection && ($vb_persistent_connections = caGetOption(
        ...
        if (!($r_res = mysql_query($vs_sql, $this->opr_db))) {
            ...
            switch ($vn_mysql_err) {
                ...
                case 1216:        // deadlock
                    ...
                    if (!$r_res) {
                        $opo_statement->postError(
                            $this->nativeToDbError($vn_mysql_err),
                            mysql_error($this->opr_db),
                            "Db->mysql->execute()"
                        );
                        throw new DatabaseException(
                            mysql_error($this->opr_db),
