...
class Db_mysqli extends DbDriverBase {
	...
	public function execute( $po_caller, $po_statement, $ps_sql, $pa_values, $pa_options = null ) {
		...
		if ( ! ( $r_res = @mysqli_query( $this->opr_db, $vs_sql,
			caGetOption( 'resultMode', $pa_options, MYSQLI_STORE_RESULT ) ) )
		) {
			//print "<pre>".caPrintStacktrace()."</pre>\n";
			$po_statement->postError( $this->nativeToDbError( mysqli_errno( $this->opr_db ) ),
				mysqli_error( $this->opr_db ), "Db->mysqli->execute()" );
			throw new DatabaseException( mysqli_error( $this->opr_db ),
				$this->nativeToDbError( mysqli_errno( $this->opr_db ) ), "Db->mysqli->execute()" );
		}

		if ( Db::$monitor ) {
			Db::$monitor->logQuery( $ps_sql, $pa_values, $t->getTime( 4 ),
				is_bool( $r_res ) ? null : mysqli_num_rows( $r_res ) );
		}
	...
	public function seek( $po_caller, $pr_res, $pn_offset ) {
		...
		if ( ! @mysqli_data_seek( $pr_res, $pn_offset ) ) {
			...
			throw new DatabaseException( _t( "seek(%1) failed: result has %2 rows", $pn_offset,
				$this->numRows( $pr_res ) ), 260, "Db->mysqli->seek()" );
