...
class Db_mysqli extends DbDriverBase
{
    ...
    public function connect($po_caller, $pa_options)
    {
        ...
        if (!function_exists("mysqli_connect")) {
            throw new DatabaseException(
                _t("Your PHP installation lacks MySQL support. Please add it and retry..."),
                200,
                "Db->mysqli->connect()"
            );
        }
    ...
    public function execute($po_caller, $po_statement, $ps_sql, $pa_values, $pa_options = null)
    {
        ...
        if (!($r_res = @mysqli_query(
            ...
            $po_statement->postError(
                $this->nativeToDbError(mysqli_errno($this->opr_db)),
                mysqli_error($this->opr_db),
                "Db->mysqli->execute()"
            );
            throw new DatabaseException(
                mysqli_error($this->opr_db),
