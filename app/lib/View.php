...
class View extends BaseObject {
	...
	public function setViewPath( $pm_path, $pa_options = null ) {
		...
		if ( caGetOption( 'allowThemeInheritance', $pa_options, true ) ) {
			if ( file_exists( __CA_THEME_DIR__ . '/conf/app.conf' ) ) {
				...
				while ( $vs_inherit_from_theme = trim( trim( $o_config->get( [ 'inheritFrom', 'inherit_from' ] ) ),
					...
					if ( ! in_array( $vs_inherited_theme_path, $pm_path )
					     && ! in_array( $vs_inherited_theme_path . '/', $pm_path )
					) {
					...
					$o_config = Configuration::load( __CA_THEMES_DIR__ . "/{$vs_inherit_from_theme}/conf/app.conf",
						false, false, true );
					if ( $i > 10 ) {
						break;
					} // max 10 levels
		...
		if ( caGetOption( 'includeDefaultThemePath', $pa_options, true ) ) {
			$vs_default_theme_path = $this->opo_request ? $this->opo_request->getDefaultThemeDirectoryPath() . '/views'
			                                              . $vs_suffix
				: __CA_THEME_DIR__ . "/default/views{$vs_suffix}";
			if ( ! in_array( $vs_default_theme_path, $pm_path )
			     && ! in_array( $vs_default_theme_path . '/', $pm_path )
			) {
	...
	public function render( $ps_filename, $pb_dont_do_var_replacement = false, $pa_options = null ) {
		...
		if ( caGetOption( 'string', $pa_options, false ) ) {
			$vs_buf    = $ps_filename;
			$vb_output = true;
		} elseif ( ( $ps_filename[0] == '/' )
		           || ( preg_match( "!^[A-Za-z]{1}:!", $ps_filename ) )
		) {    // absolute path
