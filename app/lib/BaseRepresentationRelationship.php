...
class BaseRepresentationRelationship extends BaseRelationshipModel
{
    ...
    private function _getTarget()
    {
        ...
        $vs_target_key = ($this->getLeftTableName() == 'ca_object_representations') ? $this->getRightTableFieldName(
    ...
    public function insert($pa_options = null)
    {
        ...
        if (!$t_target->load($vn_target_id)) {
            ...
            $this->postError(
                720,
                _t("Related %1 does not exist", $t_target->getProperty('NAME_SINGULAR')),
                ...
                if ($qr_res->nextRow()) {
                    ...
                    if ($t_rep_link->load($qr_res->get('relation_id'))) {
                        ...
                        $this->postError(
                            2700,
                            _t(
                                'Could not load %1-representation link',
                                $t_target->getProperty('NAME_SINGULAR')
                            ),
                            'BaseRepresentationRelationship->delete()'
                        );
                        if ($vb_we_set_transaction) {
