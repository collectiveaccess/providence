...
class BaseRepresentationRelationship extends BaseRelationshipModel {
	...
	private function _getTarget() {
		$vs_target_table = ( $this->getLeftTableName() == 'ca_object_representations' ) ? $this->getRightTableName()
			: $this->getLeftTableName();
		$vs_target_key   = ( $this->getLeftTableName() == 'ca_object_representations' )
	...
	public function insert( $pa_options = null ) {
		...
		if ( ! $t_target->load( $vn_target_id ) ) {
			// invalid object
			$this->postError( 720, _t( "Related %1 does not exist", $t_target->getProperty( 'NAME_SINGULAR' ) ),
				"BaseRepresentationRelationship->insert()" );
	...
	public function update( $pa_options = null ) {
		...
		if ( ! $t_target->load( $vn_target_id ) ) {
			// invalid target
			$this->postError( 720, _t( "Related %1 does not exist", $t_target->getProperty( 'NAME_SINGULAR' ) ),
				"BaseRepresentationRelationship->update()" );
	...
	public function delete( $pb_delete_related = false, $pa_options = null, $pa_fields = null, $pa_table_list = null ) {
		...
		if ( ! $t_target->load( $vn_target_id ) ) {
			// invalid object
			$this->postError( 720, _t( "Related %1 does not exist", $t_target->getProperty( 'NAME_SINGULAR' ) ),
				"BaseRepresentationRelationship->delete()" );
					...
					if ( $t_rep_link->load( $qr_res->get( 'relation_id' ) ) ) {
						...
						if ( $t_rep_link->numErrors() ) {
							$this->postError( 2700, _t( 'Could not update primary flag for representation: %1',
								join( '; ', $t_rep_link->getErrors() ) ), 'BaseRepresentationRelationship->delete()' );
							if ( $vb_we_set_transaction ) {
								$o_trans->rollbackTransaction();
							}
						...
						$this->postError( 2700,
							_t( 'Could not load %1-representation link', $t_target->getProperty( 'NAME_SINGULAR' ) ),
							'BaseRepresentationRelationship->delete()' );
						if ( $vb_we_set_transaction ) {
							$o_trans->rollbackTransaction();
						}
