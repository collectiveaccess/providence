...
class ExportRefineryManager {
	...
	public static function getRefineryInstance( $ps_refinery_name ) {
		ExportRefineryManager::initRefineries();
		if ( isset( ExportRefineryManager::$s_refinery_instances[ $ps_refinery_name ] )
		     && ExportRefineryManager::$s_refinery_instances[ $ps_refinery_name ]
		) {
	...
	public static function checkRefineryStatus( $ps_refinery_name ) {
		...
		if ( isset( ExportRefineryManager::$s_refinery_instances[ $ps_refinery_name ] )
		     && is_object( ExportRefineryManager::$s_refinery_instances[ $ps_refinery_name ] )
		) {
	...
	public function getRefinerySettingsForm( $ps_refinery_name, $ps_refinery_id, $pa_settings ) {
		...
		$o_appvar = null;
		ExportRefineryManager::initRefineries();
		if ( ExportRefineryManager::$s_refinery_instances[ $ps_refinery_name ]
		     && is_object( ExportRefineryManager::$s_refinery_instances[ $ps_refinery_name ] )
		) {
			$va_available_settings
				= ExportRefineryManager::$s_refinery_instances[ $ps_refinery_name ]->getAvailableSettings();
			...
			foreach ( $va_available_settings as $vs_setting_name => $va_setting_info ) {
				...
				if ( isset( $va_setting_info['scope'] ) && ( $va_setting_info['scope'] == 'application' ) ) {
					...
					$pa_settings[ $vs_setting_name ] = $o_appvar->getVar( 'export_refinery_settings_'
					                                                      . $ps_refinery_name . '_'
					                                                      . $vs_setting_name );
				}
				$vs_buf .= ExportRefineryManager::$s_refinery_instances[ $ps_refinery_name ]->settingHTMLFormElement( $ps_refinery_id,
