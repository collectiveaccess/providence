...
class ExportRefineryManager
{
     ...
     */
    static public $s_did_do_refinery_init = false;

    static public $s_refinery_instances = array();
    ...
    public static function initRefineries()
    {
        ...
        foreach ($va_refinery_dirs as $vs_refinery_dir) {
            if (!file_exists(
                $vs_base_refinery_dir . '/' . $vs_refinery_dir . '/' . $vs_refinery_dir . 'Refinery.php'
            )) {
                continue;
            }
    ...
    public static function getRefineryNames()
    {
        ...
        if (is_resource($r_dir = opendir($vs_base_refinery_dir))) {
            while (($vs_refinery = readdir($r_dir)) !== false) {
                if (is_dir($vs_base_refinery_dir . '/' . $vs_refinery) && preg_match(
                        "/^[A-Za-z_]+[A-Za-z0-9_]*$/",
                        $vs_refinery
                    )) {
    ...
    public static function checkRefineryStatus($ps_refinery_name)
    {
        ...
        if (isset(ExportRefineryManager::$s_refinery_instances[$ps_refinery_name]) && is_object(
    ...
    public function getRefinerySettingsForm($ps_refinery_name, $ps_refinery_id, $pa_settings)
    {
        ...
        $o_appvar = null;
        ExportRefineryManager::initRefineries();
        if (ExportRefineryManager::$s_refinery_instances[$ps_refinery_name] && is_object(
                ExportRefineryManager::$s_refinery_instances[$ps_refinery_name]
            )) {
            ...
            foreach ($va_available_settings as $vs_setting_name => $va_setting_info) {
                ...
                if (isset($va_setting_info['scope']) && ($va_setting_info['scope'] == 'application')) {
                    ...
                    $pa_settings[$vs_setting_name] = $o_appvar->getVar(
                        'export_refinery_settings_' . $ps_refinery_name . '_' . $vs_setting_name
                    );
                }
                $vs_buf .= ExportRefineryManager::$s_refinery_instances[$ps_refinery_name]->settingHTMLFormElement(
                    $ps_refinery_id,
                    $vs_setting_name,
                    array('value' => $pa_settings[$vs_setting_name])
                );
            }
        }
        return $vs_buf;
