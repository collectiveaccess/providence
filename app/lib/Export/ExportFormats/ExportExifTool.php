...
class ExportExifTool extends BaseExportFormat {
	...
	public function __construct() {
		...
		$this->ops_element_description
		                = _t( 'Values are ExifTool XML element names. See http://www.sno.phy.queensu.ca/~phil/exiftool/metafiles.html#xml' );
	...
	public function processExport( $pa_data, $pa_options = array() ) {
		...
		foreach ( $pa_data as $va_element ) {
			if ( ! isset( $va_element['element'] ) || ! $va_element['element'] || ! isset( $va_element['text'] )
			     || ! $va_element['text']
			) {
	...
	public function getMappingErrors( $t_mapping ) {
		...
		foreach ( $va_top as $va_item ) {
			...
			if ( ! in_array( $vs_element_ns, $va_namespace_keys ) ) {
				$va_errors[]
					= _t( '%1 is not a valid element for ExifTool exports. It must be in of one of the ExifTool namespaces.',
					$va_item['element'] );
			}
			...
			if ( is_array( $va_children ) && ( sizeof( $va_children ) > 0 ) ) {
				$va_errors[] = _t( "ExifTool mappings can't be hierarchical. It looks like element %1 has children.",
					$va_item['element'] );
			}
		}
