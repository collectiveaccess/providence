...
class ExportMARC extends BaseExportFormat
{
    ...
    public function __construct()
    {
        ...
        $this->ops_element_description = _t(
            'Values reference a combination of MARC 21 field tags and associated indicators separated by a forward slash ("/"), e.g. "300/##". For further information on how to create a MARC mapping, please refer to the CollectiveAccess online documentation.'
        );
        parent::__construct();
    }
            ...
            if (stripos($vs_element, "/") !== false) { // data field
                ...
                if (is_array($va_item['children'])) {
                    foreach ($va_item['children'] as $va_child) {
                        $va_subfields[] = new File_MARC_Subfield(
                            $va_child['element'],
                            caDecodeAllEntities(strip_tags($va_child['text']))
                        );
                    }
                }
    ...
    public function getMappingErrors($t_mapping)
    {
        ...
        foreach ($va_top as $va_item) {
            ...
            if (stripos($vs_element, "/") == 3) { // data field in format 300/##
                ...
                if (is_numeric($vs_element)) { // control field, e.g. 001
...
BaseExportFormat::$s_format_settings['MARC'] = array(
    'MARC_outputFormat' => array(
        ...
        'description' => _t(
            'Set output format. Currently supported: human-readable MARC21, raw MARC for saving into MARC files and MARCXML.'
        )
    ),
);
