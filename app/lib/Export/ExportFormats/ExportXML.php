...
class ExportXML extends BaseExportFormat {
	...
	public function __construct() {
		...
		$this->ops_element_description
		                = _t( 'Values prefixed with @ reference XML attributes. All other values define XML elements. The usual restrictions and naming conventions for XML elements and attributes apply.' );
	...
	public function processExport( $pa_data, $pa_options = array() ) {
		...
		$vs_return = ( $pb_single_record ? $this->opo_dom->saveXML()
			: $this->opo_dom->saveXML( $this->opo_dom->firstChild ) );
	...
	private function processItem( $pa_item, $po_parent, $pa_options = null ) {
		...
		$this->log( _t( "XML export formatter: Processing element or attribute '%1' with text '%2' and parent element '%3' ...",
			$vs_element, $vs_text, $po_parent->nodeName ) );
