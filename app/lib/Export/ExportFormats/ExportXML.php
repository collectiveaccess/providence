...
class ExportXML extends BaseExportFormat
{
    ...
    {
        $this->ops_name = 'XML';
        $this->ops_element_description = _t(
            'Values prefixed with @ reference XML attributes. All other values define XML elements. The usual restrictions and naming conventions for XML elements and attributes apply.'
        );
    ...
    public function processExport($pa_data, $pa_options = array())
    {
        ...
        $vs_return = ($pb_single_record ? $this->opo_dom->saveXML() : $this->opo_dom->saveXML(
            $this->opo_dom->firstChild
        ));
    ...
    private function processItem($pa_item, $po_parent)
    {
        ...
        $this->log(
            _t(
                "XML export formatter: Processing element or attribute '%1' with text '%2' and parent element '%3' ...",
                $vs_element,
                $vs_text,
                $po_parent->nodeName
            )
...
BaseExportFormat::$s_format_settings['XML'] = array(
    'stripCDATA' => array(
        ...
        'description' => _t(
            "By default the exporter wraps field content that contains invalid XML in CDATA sections to make sure the XML is valid regardless of the field content. If this option is set, the exporter explicitly strips the CDATA tags before returning the XML text. Use only if you know what you're doing!"
        )
    ),
);
