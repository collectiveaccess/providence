...
$_ca_attribute_settings['InformationServiceAttributeValue'] = array(        // global
    ...
    'doesNotTakeLocale' => array(
        ...
        'description' => _t(
            'Check this option if you don\'t want your values to be locale-specific. (The default is to not be.)'
        )
    ),
    ...
    'sortUsingList' => array(
        ...
        'description' => _t(
            'List code for list to sort information service values on. Each item in the referenced list should have an identifier that matches the information service item uri or id and a rank that reflects the desired sort order. Leave empty to sort information service items by their value.'
        )
    )
);
...
class InformationServiceAttributeValue extends AttributeValue implements IAttributeValue
{
    ...
    public function loadTypeSpecificValueFromRow($pa_value_array)
    {
        ...
        $this->opa_indexing_info =
            (is_array($va_info) && isset($va_info['indexing_info']) && is_array(
                    $va_info['indexing_info']
                )) ? $va_info['indexing_info'] : array();

        $this->opa_extra_info =
            (is_array($va_info) && isset($va_info['extra_info']) && is_array(
                    $va_info['extra_info']
                )) ? $va_info['extra_info'] : array();
    }
    # ------------------------------------------------------------------
    ...
    public function parseValue($ps_value, $pa_element_info, $pa_options = null)
    {
        ...
        $vs_service = caGetOption(
            'service',
            $this->getSettingValuesFromElementArray(
                $pa_element_info,
                array('service')
            )
        );
        ...
        if (trim($ps_value)) {
            ...
            if (sizeof($va_tmp) == 3) { /// value is already in desired format (from autocomplete lookup)
                if ($va_tmp[2]) {    // Skip if no url set (is "no match" message)
                    ...
                    $va_info['indexing_info'] = $this->opo_plugin->getDataForSearchIndexing(
            ...
            } elseif ((sizeof($va_tmp) == 1) && (isURL($va_tmp[0], array('strict' => true)) || is_numeric(
                        $va_tmp[0]
                ...
                if (is_array($va_ret['results']) && (sizeof($va_ret['results']) == 1)) {
                    ...
                    $va_info['extra_info'] = $this->opo_plugin->getExtraInfo(
                        $pa_element_info['settings'],
                        $va_hit['url']
                    );
                    ...
                    $this->postError(
                        1970,
                        _t(
                            'Value for InformationService lookup has to be an ID or URL that returns exactly 1 hit. We got more or no hits. Value was %1',
                            $ps_value
                        ),
                ...
                return $va_return;
            } elseif ((sizeof($va_tmp) == 1) && (preg_match(
                    "!^\[([0-9]+)\][ ]*(.*)!",
                    $va_tmp[0],
                    $m
    ...
    public function htmlFormElement($pa_element_info, $pa_options = null)
    {
        ...
        if (!$pb_for_search) {
            ...
            $vs_element .= " <a href='#' class='caInformationServiceMoreLink' id='{fieldNamePrefix}" . $pa_element_info['element_id'] . "_link{n}'>" . _t(
                    "More &rsaquo;"
                ) . "</a>";
            $vs_element .= "<div id='{fieldNamePrefix}" . $pa_element_info['element_id'] . "_detail{n}' class='caInformationServiceDetail'>" . ($pa_options['request'] ? caBusyIndicatorIcon(
                    $pa_options['request']
                ) : '') . "</div></div>";
            ...
            $vs_element .= "					if ('{{" . $pa_element_info['element_id'] . "}}') {
                            jQuery('#{fieldNamePrefix}" . $pa_element_info['element_id'] . "_link{n}').css('display', 'inline').on('click', function(e) {
                                if (jQuery('#{fieldNamePrefix}" . $pa_element_info['element_id'] . "_detail{n}').css('display') == 'none') {
                                    ...
                                    jQuery('#{fieldNamePrefix}" . $pa_element_info['element_id'] . "_link{n}').html('" . addslashes(
                    _t("Less &rsaquo;")
                ) . "');
                                } else {
