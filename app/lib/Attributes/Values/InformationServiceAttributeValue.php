...
class InformationServiceAttributeValue extends AttributeValue implements IAttributeValue {
	...
	public function loadTypeSpecificValueFromRow( $pa_value_array ) {
		...
		$this->opa_indexing_info = ( is_array( $va_info ) && isset( $va_info['indexing_info'] )
		                             && is_array( $va_info['indexing_info'] ) ) ? $va_info['indexing_info'] : array();

		$this->opa_extra_info = ( is_array( $va_info ) && isset( $va_info['extra_info'] )
		                          && is_array( $va_info['extra_info'] ) ) ? $va_info['extra_info'] : array();
	}
	# ------------------------------------------------------------------
		...
		if ( trim( $ps_value ) ) {
			...
			if ( sizeof( $va_tmp ) == 3 ) { /// value is already in desired format (from autocomplete lookup)
				if ( $va_tmp[2] ) {    // Skip if no url set (is "no match" message)
					...
					$va_info['indexing_info']
					                       = $this->opo_plugin->getDataForSearchIndexing( $pa_element_info['settings'],
						$va_tmp[2] );
					$va_info['extra_info'] = $this->opo_plugin->getExtraInfo( $pa_element_info['settings'],
						$va_tmp[2] );
			...
			} elseif ( ( sizeof( $va_tmp ) == 1 )
			           && ( isURL( $va_tmp[0], array( 'strict' => true ) )
			                || is_numeric( $va_tmp[0] ) )
				...
				if ( is_array( $va_ret['results'] ) && ( sizeof( $va_ret['results'] ) == 1 ) ) {
					...
					$this->postError( 1970,
						_t( 'Value for InformationService lookup has to be an ID or URL that returns exactly 1 hit. We got more or no hits. Value was %1',
							$ps_value ), 'ListAttributeValue->parseValue()' );
				...
				return $va_return;
			           ...
			           && ( preg_match( "!^\[([0-9]+)\][ ]*(.*)!", $va_tmp[0], $m ) )
			) {   // [ID] TEXT format string where ID is numeric
				return [
					'value_longtext1' => $m[2],
					'value_longtext2' => '',
	...
	public function htmlFormElement( $pa_element_info, $pa_options = null ) {
		...
		if ( ! $pb_for_search ) {
			...
			if ( $pa_options['request'] ) {
				$vs_url        = caNavUrl( $pa_options['request'], 'lookup', 'InformationService', 'Get',
					array( 'max' => 100, 'element_id' => $pa_element_info['element_id'] ) );
				$vs_detail_url = caNavUrl( $pa_options['request'], 'lookup', 'InformationService', 'GetDetail',
					array( 'element_id' => $pa_element_info['element_id'] ) );
			} else {
			...
			$vs_element .= " <a href='#' class='caInformationServiceMoreLink' id='{fieldNamePrefix}"
			               . $pa_element_info['element_id'] . "_link{n}'>" . _t( "More &rsaquo;" ) . "</a>";
			$vs_element .= "<div id='{fieldNamePrefix}" . $pa_element_info['element_id']
			               . "_detail{n}' class='caInformationServiceDetail'>" . ( $pa_options['request']
					? caBusyIndicatorIcon( $pa_options['request'] ) : '' ) . "</div></div>";
			...
			$vs_element .= "
					...
					? "
					  ...
					  . "_{n}').val(ui.item.label + '|' + ui.item.idno + '|' + ui.item.url);"
					:
			...
			$vs_element .= "					if ('{{" . $pa_element_info['element_id'] . "}}') {
                            jQuery('#{fieldNamePrefix}" . $pa_element_info['element_id'] . "_link{n}').css('display', 'inline').on('click', function(e) {
                                if (jQuery('#{fieldNamePrefix}" . $pa_element_info['element_id'] . "_detail{n}').css('display') == 'none') {
                                    ...
                                    jQuery('#{fieldNamePrefix}" . $pa_element_info['element_id'] . "_link{n}').html('"
			               . addslashes( _t( "Less &rsaquo;" ) ) . "');
                                } else {
                                    jQuery('#{fieldNamePrefix}" . $pa_element_info['element_id'] . "_detail{n}').slideToggle(250);
                                    jQuery('#{fieldNamePrefix}" . $pa_element_info['element_id'] . "_link{n}').html('"
			               . addslashes( _t( "More &rsaquo;" ) ) . "');
                                }
                                return false;
                            });
                        }
	...
	public function getAvailableSettings( $pa_element_info = null ) {
		global $_ca_attribute_settings;
		if ( ! ( $vs_service = isset( $pa_element_info['settings']['service'] )
			? $pa_element_info['settings']['service'] : null )
		) {
		...
		if ( $this->opo_plugin = InformationServiceManager::getInformationServiceInstance( $vs_service ) ) {
			$va_settings                       = $this->opo_plugin->getAvailableSettings()
			                                     + $_ca_attribute_settings['InformationServiceAttributeValue'];
