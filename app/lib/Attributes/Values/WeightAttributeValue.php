...
$_ca_attribute_settings['WeightAttributeValue'] = array(        // global
    ...
    'doesNotTakeLocale' => array(
        ...
        'description' => _t(
            'Check this option if you don\'t want your measurements to be locale-specific. (The default is not to be.)'
        )
    ),
    'requireValue' => array(
        'formatType' => FT_NUMBER,
...
class WeightAttributeValue extends AttributeValue implements IAttributeValue
{
    ...
    public function loadTypeSpecificValueFromRow($pa_value_array)
    {
        ...
        switch ($g_ui_units_pref) {
            case 'metric':
                $vo_measurement = new Zend_Measure_Weight(
                    (float)$pa_value_array['value_decimal1'],
                    'KILOGRAM',
                    $g_ui_locale
                );
    ...
    public function parseValue($ps_value, $pa_element_info, $pa_options = null)
    {
        ...
        try {
            ...
            $this->postError(
                1970,
                _t('%1 is not a valid measurement', $pa_element_info['displayLabel']),
                'WeightAttributeValue->parseValue()'
            );
            return false;
        }
        ...
        if ($vo_parsed_measurement->getValue() < 0) {
            ...
            $this->postError(
                1970,
                _t('%1 must not be less than zero', $pa_element_info['displayLabel']),
                'WeightAttributeValue->parseValue()'
            );
            return false;
        }
        ...
        return array(
            'value_longtext1' => $vo_parsed_measurement->toString(),
            // parsed measurement
            'value_longtext2' => $vo_parsed_measurement->getType(),
            // units constant
            'value_decimal1' => $vo_parsed_measurement->convertTo('KILOGRAM', 6, 'en_US')
            // measurement in metric (for searching)
        );
    }
    # ------------------------------------------------------------------
