...
class WeightAttributeValue extends AttributeValue implements IAttributeValue {
	...
	public function loadTypeSpecificValueFromRow( $pa_value_array ) {
		...
		switch ( $g_ui_units_pref ) {
			case 'metric':
				$vo_measurement       = new Zend_Measure_Weight( (float) $pa_value_array['value_decimal1'], 'KILOGRAM',
					$g_ui_locale );
			...
			default: // show value in unit entered, but adjusted for the UI locale
				try {
					$vo_measurement       = new Zend_Measure_Weight( (float) $pa_value_array['value_decimal1'],
						'KILOGRAM', $g_ui_locale );
	 ...
	 *                    units = force units used for display. Values are: metric, english, as_entered. [Default is to
	 *                    use units system of as entered value]
	 *
	...
	public function parseValue( $ps_value, $pa_element_info, $pa_options = null ) {
		...
		try {
			...
			$this->postError( 1970, _t( '%1 is not a valid measurement', $pa_element_info['displayLabel'] ),
		...
		if ( $vo_parsed_measurement->getValue() < 0 ) {
			...
			$this->postError( 1970, _t( '%1 must not be less than zero', $pa_element_info['displayLabel'] ),
				'WeightAttributeValue->parseValue()' );
