...
$_ca_attribute_settings['CurrencyAttributeValue'] = array(        // global
    ...
    'doesNotTakeLocale' => array(
        ...
        'description' => _t(
            'Check this option if you don\'t want your currency values to be locale-specific. (The default is to not be.)'
        )
    ),
    ...
    'mustNotBeBlank' => array(
        ...
        'description' => _t(
            'Check this option if this attribute value must be set to some value - it must not be blank in other words. (The default is not to be.)'
        )
    ),
...
class CurrencyAttributeValue extends AttributeValue implements IAttributeValue
{
    ...
    public function getDisplayValue($pa_options = null)
    {
        ...
        $vs_decimal_with_placeholder = Zend_Locale_Format::toNumber(
            $this->opn_value,
            array(
                'locale' => $o_locale,
                'number_format' => $vs_format,
                'precision' => 2
            )
        );
        ...
        $vs_val = str_replace(
            '%',
            caGetCurrencySymbol($this->ops_currency_specifier, $this->getElementID()),
            $vs_decimal_with_placeholder
        );
        if (($vs_to_currency = caGetOption(
                'displayCurrencyConversion',
                $pa_options,
                false
            )) && ($this->ops_currency_specifier != $vs_to_currency)) {
            $vs_val .= " (" . _t(
                    "~%1",
                    caConvertCurrencyValue(
                        caGetCurrencySymbol(
                            $this->ops_currency_specifier,
                            $this->getElementID()
                        ) . ' ' . $this->opn_value,
                        $vs_to_currency
                    )
    ...
    public function parseValue($ps_value, $pa_element_info, $pa_options = null)
    {
        ...
        if (strlen($ps_value) == 0) {
            if ((bool)$va_settings['mustNotBeBlank']) {
                $this->postError(
                    1970,
                    _t('%1 must not be empty', $pa_element_info['displayLabel']),
                    'CurrencyAttributeValue->parseValue()'
                );
                return false;
            }
            return [
        ...
        if (is_array($va_parsed_value = caParseCurrencyValue($ps_value))) {
            ...
            $this->postError(
                1970,
                _t(
                    '%1 is not a valid currency value; be sure to include a currency symbol',
        ...
        try {
            ...
            $this->postError(
                1970,
                _t(
                    '%1 does not use a valid decimal notation for your locale',
                    $pa_element_info['displayLabel']
                ),
                'CurrencyAttributeValue->parseValue()'
            );
        ...
        switch ($vs_currency_specifier) {
            case '$':
                ...
                $vs_currency_specifier = ($vs_dollars_are_this = caGetOption(
                    'dollarCurrency',
                    $va_settings,
                    $o_config->get('default_dollar_currency')
                )) ? $vs_dollars_are_this : 'USD';
                break;
            case '¥':
        ...
        if (strlen($vs_currency_specifier) != 3) {
            $this->postError(
                1970,
                _t(
                    'Currency specified for %1 does not appear to be valid',
                    $pa_element_info['displayLabel']
                ),
                'CurrencyAttributeValue->parseValue()'
            );
        ...
        if ($vn_value < 0) {
            $this->postError(
                1970,
                _t('%1 must not be negative', $pa_element_info['displayLabel']),
                'CurrencyAttributeValue->parseValue()'
            );
            return false;
        }
        if ($vn_value < floatval($va_settings['minValue'])) {
            ...
            $this->postError(
                1970,
                _t('%1 must be at least %2', $pa_element_info['displayLabel'], $va_settings['minValue']),
                'CurrencyAttributeValue->parseValue()'
            );
            return false;
        }
        if ((floatval($va_settings['maxValue']) > 0) && ($vn_value > floatval($va_settings['maxValue']))) {
            ...
            $this->postError(
                1970,
                _t('%1 must be less than %2', $pa_element_info['displayLabel'], $va_settings['maxValue']),
                'CurrencyAttributeValue->parseValue()'
            );
            return false;
        }
