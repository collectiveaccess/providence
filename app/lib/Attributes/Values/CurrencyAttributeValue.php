...
class CurrencyAttributeValue extends AttributeValue implements IAttributeValue {
	...
	public function htmlFormElement($element_info, $options=null) {
		$va_settings = $this->getSettingValuesFromElementArray($element_info, array('fieldWidth', 'fieldHeight', 'minValue', 'maxValue'));
		$vs_class = trim((isset($options['class']) && $options['class']) ? $options['class'] : 'currencyBg');
		
		return caHTMLTextInput(
			'{fieldNamePrefix}'.$element_info['element_id'].'_{n}', 
			array(
				'size' => (isset($options['width']) && $options['width'] > 0) ? $options['width'] : $va_settings['fieldWidth'] ?? null,
				'height' => (isset($options['height']) && $options['height'] > 0) ? $options['height'] : $va_settings['fieldHeight'] ?? null, 
