...
class UrlAttributeValue extends AttributeValue implements IAttributeValue {
	 ...
	 *                          asHTML = if set, URL is returned as a simple HTML link with target set to _url_details
	 *                          (deprecated) returnAsLink = if set, URL is returned as a link formatted according to
	 *                          settings in the other returnAsLink* options described below returnAsLinkText = text to
	 *                          use a content of HTML link. If omitted the url itself is used as the link content.
	 *                          returnAsLinkAttributes = array of attributes to include in link <a> tag. Use this to
	 *                          set class, alt and any other link attributes.
	 *
	 * @return string The url or link HTML
	 */
	public function getDisplayValue( $pa_options = null ) {
		if ( isset( $pa_options['asHTML'] ) && $pa_options['asHTML'] ) {
			return caHTMLLink( $this->ops_text_value,
				array( 'href' => $this->ops_text_value, 'target' => '_url_details' ) );
		}
		...
		if ( $vs_return_as_link ) {
			...
			$va_return_as_link_attributes = ( isset( $pa_options['returnAsLinkAttributes'] )
			                                  && is_array( $pa_options['returnAsLinkAttributes'] ) )
				? $pa_options['returnAsLinkAttributes'] : array();
	...
	public function parseValue( $ps_value, $pa_element_info, $pa_options = null ) {
		...
		if ( $vn_strlen > $va_settings['maxChars'] ) {
			...
			$vs_err_msg = ( $va_settings['maxChars'] == 1 ) ? _t( '%1 must be no more than 1 character long',
				$pa_element_info['displayLabel'] )
				: _t( '%1 be no more than %2 characters long', $pa_element_info['displayLabel'],
		...
		if ( ! $va_settings['regex'] ) {
			$va_settings['regex']
				= "(http|ftp|https|rtmp|rtsp):\/\/[\w\-_]+(\.[\w\-_]+)+([\w\-\.,@?^=%&;:/~\+#]*[\w\-\@?^=%&/~\+#])?";
		}
		if ( $va_settings['regex'] && ! preg_match( "!" . $va_settings['regex'] . "!", $ps_value ) ) {
			...
			if ( ! preg_match( "!^[A-Za-z]+:\/\/!", $ps_value ) ) {
				...
				// regex failed
				$this->postError( 1970, _t( '%1 is not a valid url', $pa_element_info['displayLabel'] ),
					'UrlAttributeValue->parseValue()' );
	 ...
	 *                               defined in metadata element definition] t_subject = an instance of the model to
	 *                               which the attribute belongs; required if suggestExistingValues lookups are enabled
	 *                               [Default is null] request = the RequestHTTP object for the current request;
	 *                               required if suggestExistingValues lookups are enabled [Default is null]
	 *                               suggestExistingValues = suggest values based on existing input for this element as
	 *                               user types [Default is false]
	 *
	 * @return string
	 */
	public function htmlFormElement( $pa_element_info, $pa_options = null ) {
		$va_settings = $this->getSettingValuesFromElementArray( $pa_element_info,
			array( 'fieldWidth', 'fieldHeight', 'minChars', 'maxChars', 'suggestExistingValues' ) );
		...
		$vs_element .= " <a href='#' style='display: none; vertical-align: top;' id='{fieldNamePrefix}"
		               . $pa_element_info['element_id'] . "_link{n}' target='_url_details'>" . _t( "Open &rsaquo;" )
		               . "</a>";
		...
		if ( isset( $pa_options['t_subject'] ) && is_object( $pa_options['t_subject'] ) ) {
			...
			if ( $pa_options['request'] ) {
				$vs_lookup_url = caNavUrl( $pa_options['request'], 'lookup', 'AttributeValue', 'Get',
					array( 'bundle' => $vs_bundle_name, 'max' => 500 ) );
			}
		}
		...
		$vs_element .= "
				<script type='text/javascript'>
					if ('{{" . $pa_element_info['element_id'] . "}}') {
		               ...
		               . "_link{n}').css('display', 'inline').attr('href', '{{" . $pa_element_info['element_id'] . "}}');
						}
				</script>
			";
	...
	public static function checkIntegrityForAllElements( array $pa_options = [] ) {
		...
		$qr_elements = $o_db->query( 'SELECT element_id FROM ca_metadata_elements WHERE datatype=? ORDER BY element_id',
		...
		while ( $qr_elements->nextRow() ) {
			...
			$qr_vals = $o_db->query( 'SELECT * FROM ca_attribute_values WHERE element_id=? ORDER BY value_id',
			...
			while ( $qr_vals->nextRow() ) {
				...
				try {
					...
					if ( $po_request instanceof RequestHTTP ) {
						$vs_msg .= "\n<br/><br/>" . caEditorLink( $po_request, _t( "Open record" ), '',
								$qr_attr->get( 'table_num' ), $qr_attr->get( 'row_id' ), null, null,
								[ 'action' => 'Edit' ] );
					}
