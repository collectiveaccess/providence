...
$_ca_attribute_settings['UrlAttributeValue'] = array(        // global
    ...
    'doesNotTakeLocale' => array(
        ...
        'description' => _t(
            'Check this option if you don\'t want your urls to be locale-specific. (The default is to not be.)'
        )
    ),
    'requireValue' => array(
        'formatType' => FT_NUMBER,
    ...
    'suggestExistingValues' => array(
        ...
        'description' => _t(
            'Check this option if you want this attribute to suggest previously saved values as text is entered. This option is only effective if the display height of the element is equal to 1.'
        )
    ),
...
class UrlAttributeValue extends AttributeValue implements IAttributeValue
{
    ...
    public function getDisplayValue($pa_options = null)
    {
        ...
        if ($vs_return_as_link) {
            ...
            $va_return_as_link_attributes = (isset($pa_options['returnAsLinkAttributes']) && is_array(
                    $pa_options['returnAsLinkAttributes']
    ...
    public function parseValue($ps_value, $pa_element_info, $pa_options = null)
    {
        ...
        if ($vn_strlen > $va_settings['maxChars']) {
            ...
            $vs_err_msg = ($va_settings['maxChars'] == 1) ? _t(
                ...
                $pa_element_info['displayLabel']
            ) : _t('%1 be no more than %2 characters long', $pa_element_info['displayLabel'], $va_settings['maxChars']);
            $this->postError(1970, $vs_err_msg, 'UrlAttributeValue->parseValue()');
            return false;
        }
        ...
        if ($va_settings['regex'] && !preg_match("!" . $va_settings['regex'] . "!", $ps_value)) {
            ...
            if (!preg_match("!^[A-Za-z]+:\/\/!", $ps_value)) {
                ...
                $this->postError(
                    1970,
                    _t('%1 is not a valid url', $pa_element_info['displayLabel']),
    ...
    public function htmlFormElement($pa_element_info, $pa_options = null)
    {
        ...
        $vs_element .= " <a href='#' style='display: none; vertical-align: top;' id='{fieldNamePrefix}" . $pa_element_info['element_id'] . "_link{n}' target='_url_details'>" . _t(
                "Open &rsaquo;"
            ) . "</a>";
    ...
    public function checkIntegrity()
    {
        ...
        $o_context = stream_context_create(
            array(
                'http' => array(
                    'timeout' => 3
                )
    ...
    public static function checkIntegrityForAllElements(array $pa_options = [])
    {
        ...
        $qr_elements = $o_db->query(
            'SELECT element_id FROM ca_metadata_elements WHERE datatype=? ORDER BY element_id',
        ...
        while ($qr_elements->nextRow()) {
            ...
            $qr_vals = $o_db->query(
                'SELECT * FROM ca_attribute_values WHERE element_id=? ORDER BY value_id',
