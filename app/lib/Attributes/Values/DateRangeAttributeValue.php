...
class DateRangeAttributeValue extends AttributeValue implements IAttributeValue {
	 ...
	 *      getUnixTimestamp = Return date value as single Unix timestamp value. Only valid for dates in the Unix
	 *      timestamp domain (post 1970).
	 */
	public function getDisplayValue( $pa_options = null ) {
		...
		if ( caGetOption( 'GET_DIRECT_DATE', $pa_options, false )
		...
		$vs_cache_key = caMakeCacheKeyFromOptions( $pa_options,
			$vs_date_format . $this->opn_start_date . $this->opn_end_date );
		...
		if ( $vs_date_format == 'original' ) {
			...
			return DateRangeAttributeValue::$s_date_cache[ $vs_cache_key ]
				= DateRangeAttributeValue::$o_tep->getText( array_merge( array(
				'dateFormat' => $vs_date_format,
				'isLifespan' => $va_settings['isLifespan']
	...
	public function parseValue( $ps_value, $pa_element_info, $pa_options = null ) {
		...
		if ( $ps_value ) {
			...
			if ( ! DateRangeAttributeValue::$o_tep->parse( $ps_value ) ) {
				// invalid date
				$this->postError( 1970, _t( '%1 is invalid', $pa_element_info['displayLabel'] ),
					'DateRangeAttributeValue->parseValue()' );
			...
			if ( $va_settings['dateRangeBoundaries'] ) {
				if ( DateRangeAttributeValue::$o_tep->parse( $va_settings['dateRangeBoundaries'] ) ) {
					...
					if (
						( $va_dates[0] < $va_boundary_dates[0] ) || ( $va_dates[0] > $va_boundary_dates[1] )
						|| ( $va_dates[1] < $va_boundary_dates[0] )
						|| ( $va_dates[1] > $va_boundary_dates[1] )
					) {
						// date is out of bounds
						$this->postError( 1970, _t( '%1 must be within %2', $pa_element_info['displayLabel'],
							$va_settings['dateRangeBoundaries'] ), 'DateRangeAttributeValue->parseValue()' );
	 ...
	 *                               user types [Default is false] useDatePicker = use calendar-style date picker
	 *                               [Default is false],
	 *                               8            datePickerDateFormat = Format to use for dates selected from the date
	 *                               picker [Default is 'yy-mm-dd']
	 *
	 * @return string
	 */
	public function htmlFormElement( $pa_element_info, $pa_options = null ) {
		$va_settings = $this->getSettingValuesFromElementArray( $pa_element_info,
			array( 'fieldWidth', 'suggestExistingValues', 'useDatePicker', 'datePickerDateFormat' ) );
		...
		if ( (bool) $va_settings['useDatePicker'] ) {
			...
			$vs_element .= "<script type='text/javascript'>
 					jQuery(document).ready(function() {
 						jQuery('#{fieldNamePrefix}" . $pa_element_info['element_id']
			               . "_{n}').datepicker({dateFormat: '" . ( isset( $va_settings['datePickerDateFormat'] )
					? $va_settings['datePickerDateFormat'] : 'yy-mm-dd' ) . "', constrainInput: false});
 					});
 				</script>\n";
			...
			if ( file_exists( __CA_BASE_DIR__ . $vs_i18n_relative_path ) ) {
				$vs_element .= "<script src='" . __CA_URL_ROOT__ . $vs_i18n_relative_path
	...
	public function getDataForSearchIndexing() {
		...
		} elseif ( self::$o_search_config->get( 'treat_before_dates_as_circa' )
		           && ( (int) $this->opn_start_date === - 2000000000 )
		) {
			$d = (int) $this->opn_end_date;
		} elseif ( self::$o_search_config->get( 'treat_after_dates_as_circa' )
		           && ( (int) $this->opn_end_date === 2000000000 )
		) {
