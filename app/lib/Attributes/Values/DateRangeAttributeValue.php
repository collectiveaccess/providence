...
$_ca_attribute_settings['DateRangeAttributeValue'] = array(        // global
    'dateRangeBoundaries' => array(
        ...
        'description' => _t(
            'The range of dates that are accepted. Input dates outside the range will be rejected. Leave blank if you do not require restrictions.'
        )
    ),
    'fieldWidth' => array(
    ...
    'doesNotTakeLocale' => array(
        ...
        'description' => _t(
            'Check this option if you don\'t want your date ranges to be locale-specific. (The default is to not be.)'
        )
    ),
    ...
    'useDatePicker' => array(
        ...
        'description' => _t(
            'Check this option if you want a calendar-based date picker to be available for date entry. (The default is to not include a picker.)'
        )
    ),
    ...
    'isLifespan' => array(
        ...
        'description' => _t(
            'Check this option if this attribute value represents a persons lifespan. Lifespans are displayed in a slightly different format in many languages than standard dates. (The default is not to be.)'
        )
    ),
    'default_text' => array(
        'formatType' => FT_TEXT,
    ...
    'suggestExistingValueSort' => array(
        ...
        'description' => _t(
            'If suggestion of existing values is enabled this option determines how returned values are sorted. Choose <i>value</i> to sort alphabetically. Choose <i>most recently added </i> to sort with most recently entered values first.'
        ),
        'options' => array(
            _t('Value') => 'value',
...
class DateRangeAttributeValue extends AttributeValue implements IAttributeValue
{
    ...
    public function getDisplayValue($pa_options = null)
    {
        ...
        if ($vs_date_format == 'original') {
            ...
            return DateRangeAttributeValue::$s_date_cache[$vs_cache_key] = DateRangeAttributeValue::$o_tep->getText(
                array_merge(array('isLifespan' => $va_settings['isLifespan']), $pa_options)
            ); //$this->ops_text_value;
        }
    }
    ...
    public function parseValue($ps_value, $pa_element_info, $pa_options = null)
    {
        ...
        if ($ps_value) {
            if (!DateRangeAttributeValue::$o_tep->parse($ps_value)) {
                ...
                $this->postError(
                    1970,
                    _t('%1 is invalid', $pa_element_info['displayLabel']),
                    ...
                    if (
                        ...
                        $this->postError(
                            ...
                            _t(
                                ...
                                $va_settings['dateRangeBoundaries']
                            ),
                            'DateRangeAttributeValue->parseValue()'
                        );
                        return false;
    ...
    public function getDataForSearchIndexing()
    {
        ...
        if (!self::$o_lang) {
            self::$o_lang = TimeExpressionParser::getSettingsForLanguage(__CA_DEFAULT_LOCALE__);
        }
        ...
        } elseif (self::$o_search_config->get(
