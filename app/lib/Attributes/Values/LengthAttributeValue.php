...
class LengthAttributeValue extends AttributeValue implements IAttributeValue {
	...
	public function _getValueAsText( $pa_value_array, $pa_options = null ) {
		...
		try {
			$vo_measurement = new Zend_Measure_Length( (float) $pa_value_array['value_decimal1'], 'METER',
				$g_ui_locale );
					...
					if ( $vn_decimal > 0 ) {
						switch ( $vs_convert_to_units ) {
							case Zend_Measure_Length::FEET:
								...
								$vo_inches = new Zend_Measure_Length( $vn_inches, Zend_Measure_Length::INCH,
									$g_ui_locale );

								$vs_value = $vo_feet->convertTo( $vs_convert_to_units, $vn_precision );
								if ( in_array( $vs_convert_to_units,
									$this->config->get( 'add_period_after_units' ) )
								) {
									$vs_value .= '.';
								}

								if ( $vn_inches > 0 ) {
									$vs_value .= " " . caLengthToFractions( $vn_inches, $vn_maximum_denominator, true, [
											'precision'                   => $this->config->get( 'inch_decimal_precision' ),
											'allowFractionsFor'           => $fracs,
											'useUnicodeFractionGlyphsFor' => $unicode_fracs
										] );
									if ( in_array( Zend_Measure_Length::INCH,
										$this->config->get( 'add_period_after_units' ) )
									) {
										$vs_value .= '.';
									}
							...
							case Zend_Measure_Length::MILE:
								...
								$vo_feet   = new Zend_Measure_Length( $vn_whole_feet, Zend_Measure_Length::FEET,
									$g_ui_locale );
								$vo_inches = new Zend_Measure_Length( $vn_inches, Zend_Measure_Length::INCH,
									$g_ui_locale );
								...
								if ( is_null( $vn_precision ) ) {
									$vn_precision = $this->config->get( strtolower( $vs_convert_to_units )
									                                    . '_decimal_precision' );
								}
								...
								if ( $vn_whole_feet > 0 ) {
									$vs_value .= " " . $vo_feet->convertTo( Zend_Measure_Length::FEET,
											$this->config->get( 'feet_decimal_precision' ) );
									if ( in_array( Zend_Measure_Length::INCH,
										$this->config->get( 'add_period_after_units' ) )
									) {
										$vs_value .= '.';
								...
								if ( $vn_inches > 0 ) {
									$vs_value .= " " . caLengthToFractions( $vn_inches, $vn_maximum_denominator, true, [
											'units'                       => Zend_Measure_Length::INCH,
											'allowFractionsFor'           => $fracs,
											'useUnicodeFractionGlyphsFor' => $unicode_fracs
										] );
									if ( in_array( Zend_Measure_Length::INCH,
							...
							default:
								$vs_value = caLengthToFractions( $vs_value_in_inches, $vn_maximum_denominator, true, [
									'units'                       => $vs_convert_to_units,
									'allowFractionsFor'           => $fracs,
									'useUnicodeFractionGlyphsFor' => $unicode_fracs
								] );
								break;
	...
	public function parseValue( $ps_value, $pa_element_info, $pa_options = null ) {
		...
		return array(
			...
			// parsed measurement with units
			'value_longtext2' => $vo_parsed_measurement->getType(),
			// units constant
			'value_decimal1'  => $vo_parsed_measurement->convertTo( 'METER', 6, 'en_US' )
