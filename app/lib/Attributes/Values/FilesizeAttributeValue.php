...
class FilesizeAttributeValue extends AttributeValue implements IAttributeValue {
	 ...
	 *                 normalize = Return filesize with normalized units - the largest unit that will include a whole
	 *                 number component. [Default is null]
	 *
	...
	public function parseValue( $value, $element_info, $options = null ) {
		...
		if ( is_null( $size_in_bytes ) ) {
			$this->postError( 1970, _t( '%1 is not a valid file size', $element_info['displayLabel'] ),
				'FilesizeAttributeValue->parseValue()' );
		...
		return [
			...
			// originally entered filesize expression
			'value_decimal1'  => $size_in_bytes
			// size in bytes
		];
	}
	# ------------------------------------------------------------------
	...
	public function htmlFormElement( $element_info, $options = null ) {
		...
		return caHTMLTextInput(
			...
			[
				'size'   => ( isset( $options['width'] ) && $options['width'] > 0 ) ? $options['width']
					: $settings['fieldWidth'],
				'height' => ( isset( $options['height'] ) && $options['height'] > 0 ) ? $options['height']
