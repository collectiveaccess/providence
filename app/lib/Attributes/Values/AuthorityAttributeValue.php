...
abstract class AuthorityAttributeValue extends AttributeValue
{
    ...
    public function getDisplayValue($pa_options = null)
    {
        ...
        if (is_array($va_lookup_template = $o_config->getList($this->ops_table_name . '_lookup_settings'))) {
            $vs_default_template = join(
                $o_config->get($this->ops_table_name . '_lookup_delimiter'),
                $va_lookup_template
            );
        } else {
        ...
        return $this->opn_id ? caProcessTemplateForIDs(
                $ps_template,
                $this->ops_table_name,
                array($this->opn_id),
                array('returnAsArray' => false, 'returnAllLocales' => false)
            ) . ($vb_include_id ? " [" . $this->opn_id . "]" : '') : "";
    }
    # ------------------------------------------------------------------
    ...
    public function parseValue($ps_value, $pa_element_info, $pa_options = null)
    {
        ...
        )) ? true : (bool)$pa_element_info['settings']['requireValue'];
        ...
        foreach ($va_match_on as $vs_match_on) {
            switch ($vs_match_on) {
                case 'idno':
                    ...
                    if ($vn_id = call_user_func(
                        $this->ops_table_name . '::find',
                    ...
                    if ($vn_id = call_user_func(
                        ...
                        array(
                            'preferred_labels' => array(
                                $t_item->getLabelDisplayField() => $ps_value
                            )
                        ),
                        array('transaction' => $o_trans, 'returnAs' => 'firstId')
                    )) {
        ...
        if ((!$vn_id) && ($o_log = caGetOption('log', $pa_options, null))) {
            $o_log->logError(
                _t(
                    'Value %1 was not set for %2 because it does not refer to an existing %3',
                    $ps_value,
        ...
        if (!$vn_id) {
            $this->postError(
                1970,
                _t(
                    '%1 id %2 is not valid for element %3',
                    $this->ops_name_singular,
                    $ps_value,
                    $pa_element_info["element_code"]
                ),
                $this->ops_name_plural . 'AttributeValue->parseValue()'
            );
            return false;
        }
    ...
    public function htmlFormElement($pa_element_info, $pa_options = null)
    {
        ...
        if ($pa_options['request']) {
            if ($va_restrict_to_types = array_filter(
                caGetOption('restrictToTypes', $va_settings, [], ['castTo' => 'array']),
                function ($v) {
                    return strlen($v);
                }
            )) {
                $va_params = array('max' => 50, 'types' => join(";", $va_restrict_to_types));
            } elseif ($vs_restrict_to_type = caGetOption(
                'restrictTo' . $this->ops_name_singular . 'TypeIdno',
