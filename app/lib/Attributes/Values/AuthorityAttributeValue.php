...
abstract class AuthorityAttributeValue extends AttributeValue {
	 ...
	 *            output = Authority value to return. Valid values are text [display text], idno [identifier; same as
	 *            returnIdno option], value [numeric id; same as idsOnly option]. [Default is value] forDuplication =
	 *            Forces value suitable duplication of the record. This is almost always the numeric primary key ID for
	 *            the related authority record. [Default is false] includeID = Include numeric primary key ID at end of
	 *            display text, surrounded by brackets (Eg. [353]) [Default is false] template =  Display template for
	 *            format returned value with. Template is evaluated related to the related authority record. [Default
	 *            is null] checkAccess = Only return list items with a specified access value. [Default is null; no
	 *            filtering performed]
	 *
	...
	public function getDisplayValue( $pa_options = null ) {
		...
		if ( is_array( $va_lookup_template = $o_config->getList( $this->ops_table_name . '_lookup_settings' ) ) ) {
			$vs_default_template = join( $o_config->get( $this->ops_table_name . '_lookup_delimiter' ),
				$va_lookup_template );
		} else {
		...
		return $this->opn_id ? caProcessTemplateForIDs( $ps_template, $this->ops_table_name, array( $this->opn_id ),
				array( 'returnAsArray' => false, 'returnAllLocales' => false ) ) . ( $vb_include_id ? " ["
		                                                                                              . $this->opn_id
		                                                                                              . "]" : '' ) : "";
	}
	# ------------------------------------------------------------------
	...
	public function parseValue( $ps_value, $pa_element_info, $pa_options = null ) {
		...
		$vb_require_value       = ( is_null( $pa_element_info['settings']['requireValue'] ) ) ? true
		...
		foreach ( $va_match_on as $vs_match_on ) {
			switch ( $vs_match_on ) {
				case 'idno':
					...
					if ( $vn_id = call_user_func( $this->ops_table_name . '::find',
						array( $t_item->getProperty( 'ID_NUMBERING_ID_FIELD' ) => $ps_value ),
						array( 'transaction' => $o_trans, 'returnAs' => 'firstId' ) )
					) {
						break( 2 );
					}
					break;
				...
				case 'labels':
					...
					if ( $vn_id = call_user_func( $this->ops_table_name . '::find',
						array( 'preferred_labels' => array( $t_item->getLabelDisplayField() => $ps_value ) ),
						array( 'transaction' => $o_trans, 'returnAs' => 'firstId' ) )
					) {
						break( 2 );
					}
				...
				default:
					if ( $vn_id = call_user_func( $this->ops_table_name . '::find',
						array( $t_item->primaryKey() => $ps_value ),
						array( 'transaction' => $o_trans, 'returnAs' => 'firstId' ) )
					) {
						break( 2 );
					}
		...
		if ( ( ! $vn_id ) && ( $o_log = caGetOption( 'log', $pa_options, null ) ) ) {
			$o_log->logError( _t( 'Value %1 was not set for %2 because it does not refer to an existing %3', $ps_value,
				caGetOption( 'logReference', $pa_options, '???' ), $t_item->getProperty( 'name_singular' ) ) );
		}
		...
		if ( ! $vn_id ) {
			$this->postError( 1970, _t( '%1 id %2 is not valid for element %3', $this->ops_name_singular, $ps_value,
				$pa_element_info["element_code"] ), $this->ops_name_plural . 'AttributeValue->parseValue()' );
	 ...
	 *                               width = the width of the form element [Default=field width defined in metadata
	 *                               element definition] height = the height of the form element [Default=field height
	 *                               defined in metadata element definition] request = the RequestHTTP object for the
	 *                               current request; required for lookups to work [Default is null]
	 *
	 * @return string
	 */
	public function htmlFormElement( $pa_element_info, $pa_options = null ) {
		...
		$va_settings = $this->getSettingValuesFromElementArray( $pa_element_info,
			array( 'fieldWidth', 'restrictToTypes' ) );
		$vs_class    = trim( ( isset( $pa_options['class'] ) && $pa_options['class'] ) ? $pa_options['class']
			: 'lookupBg' );

		if ( $pa_options['request'] ) {
			if ( $va_restrict_to_types = array_filter( caGetOption( 'restrictToTypes', $va_settings, [],
				[ 'castTo' => 'array' ] ), function ( $v ) {
				return strlen( $v );
			} )
			) {
				$va_params = array( 'max' => 50, 'types' => join( ";", $va_restrict_to_types ) );
			} elseif ( $vs_restrict_to_type = caGetOption( 'restrictTo' . $this->ops_name_singular . 'TypeIdno',
				$pa_element_info['settings'], null )
			) {
		...
		$vs_quickadd_url = caNavUrl(
			$pa_options['request'], $va_pieces['module'], $va_pieces['controller'], $va_pieces['action'],
		...
		$o_view->setVar( 'allowQuickadd',
			( strpos( $pa_options['request']->getController(), 'Interstitial' ) === false ) );
