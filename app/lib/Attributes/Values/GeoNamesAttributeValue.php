...
class GeoNamesAttributeValue extends AttributeValue implements IAttributeValue {
	 ...
	 *                          forDuplication = returns full text + Geonames URL suitable for setting a duplicate
	 *                          attribute. Used in BaseModelWithAttributes::copyAttributesTo()
	 *
		...
		if ( isset( $pa_options['coordinates'] ) && $pa_options['coordinates'] ) {
			if ( preg_match( "!\[([^\]]+)!", $this->ops_text_value, $va_matches ) ) {
				...
				if ( ( sizeof( $va_tmp ) == 2 ) && ( is_numeric( $va_tmp[0] ) ) && ( is_numeric( $va_tmp[1] ) ) ) {
					return array(
						'latitude'  => trim( $va_tmp[0] ),
						'longitude' => trim( $va_tmp[1] ),
						'path'      => trim( $va_matches[1] ),
						'label'     => $this->ops_text_value
					);
				} else {
					return array(
						'latitude'  => null,
	...
	public function parseValue( $ps_value, $pa_element_info, $pa_options = null ) {
		...
		if ( ! $ps_value ) {
			if ( ! $va_settings["canBeEmpty"] ) {
				$this->postError( 1970, _t( 'Entry for <em>%1</em> was blank.', $pa_element_info['displayLabel'] ),
					'GeoNamesAttributeValue->parseValue()' );
			...
			if ( ! $vs_id ) {
				...
				try {
					...
					if ( $va_attr && isset( $va_attr['value'] ) && ( (int) $va_attr['value'] > 0 ) ) {
						$this->postError( 1970,
							_t( 'Connection to GeoNames with username "%1" was rejected with the message "%2". Check your configuration and make sure your GeoNames.org account is enabled for web services.',
								$vs_user, $va_attr['message'] ), 'GeoNamesAttributeValue->parseValue()' );
					...
					$this->postError( 1970, _t( 'Could not connect to GeoNames' ),
						'GeoNamesAttributeValue->parseValue()' );
	 ...
	 *                               forSearch = simple text entry is returned for use with search forms
	 *                               [Default=false]
	 ...
	 *                               current request; required for lookups to work [Default is null] disableMap = don't
	 *                               show map with Geonames data [Default=false]
	 *
	 * @return string
	 */
	public function htmlFormElement( $pa_element_info, $pa_options = null ) {
		...
		if ( isset( $pa_options['forSearch'] ) && $pa_options['forSearch'] ) {
			return caHTMLTextInput( "{fieldNamePrefix}" . $pa_element_info['element_id'] . "_{n}", array(
				'id'    => "{fieldNamePrefix}" . $pa_element_info['element_id'] . "_{n}",
				'value' => $pa_options['value']
			), $pa_options );
		}
		$o_config = Configuration::load();

		$va_settings = $this->getSettingValuesFromElementArray( $pa_element_info,
			array( 'fieldWidth', 'fieldHeight', 'disableMap', 'maxResults', 'gnElements', 'gnDelimiter' ) );
		...
		if ( $pa_options['request'] ) {
			$vs_url = caNavUrl( $pa_options['request'], 'lookup', 'GeoNames', 'Get', array(
				'maxRows'     => $vn_max_results,
				'gnElements'  => urlencode( $vs_gn_elements ),
				'gnDelimiter' => urlencode( $vs_gn_delimiter )
			) );
		}
		...
		$vs_element = '<div id="geonames_' . $pa_element_info['element_id'] . '_input{n}">' .
		              caHTMLTextInput(
			              ...
			              array(
				              'size'      => ( isset( $pa_options['width'] ) && $pa_options['width'] > 0 )
					              ? $pa_options['width'] : $va_settings['fieldWidth'],
				              'height'    => ( isset( $pa_options['height'] ) && $pa_options['height'] > 0 )
					              ? $pa_options['height'] : $va_settings['fieldHeight'],
		...
		$vs_element .= " <a href='#' class='caGeoNamesServiceMoreLink' id='{fieldNamePrefix}"
		               . $pa_element_info['element_id'] . "_link{n}' target='_geonames_details'>"
		               . _t( "More &rsaquo;" ) . "</a>";
		...
		if ( ! caGetOption( "disableMap", $va_settings, false ) && ! caGetOption( "disableMap", $pa_options, false ) ) {
			if ( strlen( $o_config->get( 'google_maps_key' ) ) ) {
				...
				$vs_element .= "
						...
						if (latlong) {
							// map vars are global
							map_" . $pa_element_info['element_id']
				               . "{n} = new google.maps.Map(document.getElementById('map_"
				               . $pa_element_info['element_id'] . "{n}'), {
