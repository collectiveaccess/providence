...
class ListAttributeValue extends AuthorityAttributeValue implements IAttributeValue {
	...
	/**
	 * Get string value of list item attribute value for display. When returning text will return plural value of list
	 * item unless useSingular option is set to true, in which case singular version of list item label will be used.
	 *
	 * @param array Optional array of options. Supported options include:
	 *            list_id = if set then the numeric item_id value is translated into label text in the current locale.
	 *            If not set then the numeric item_id is returned. useSingular = If list_id is set then by default the
	 *            returned text is the plural label. Setting this option to true will force use of the singular label.
	 *            [Default is false] returnIdno = If true list item idno is returned rather than preferred label
	 *            [Default is false] idsOnly = Return numeric item_id only [Default is false] alwaysReturnItemID =
	 *            Synonym for idsOnly [Default is false] output = List item value return. Valid values are text
	 *            [display text], idno [identifier; same as returnIdno option], value [numeric item_id; same as idsOnly
	 *            option]. [Default is "value"] transaction = transaction to get list item information in the context
	 *            of [Default is false] checkAccess = Only return list items with a specified access value. [Default is
	 *            null; no filtering performed]
	 *
	 *            HIERARCHICAL OPTIONS:
	 *                showHierarchy = Return hierarchical parents of list item. Hierarchical options described below
	 *                are used to control the output. [Default is false] direction = For hierarchy specifications (eg.
	 *                ca_objects.hierarchy) this determines the order in which the hierarchy is returned. ASC will
	 *                return the hierarchy root first while DESC will return it with the lowest node first. Default is
	 *                ASC. top = For hierarchy specifications (eg. ca_objects.hierarchy) this option, if set, will
	 *                limit the returned hierarchy to the first X nodes from the root down. Default is to not limit.
	 *                bottom = For hierarchy specifications (eg. ca_objects.hierarchy) this option, if set, will limit
	 *                the returned hierarchy to the first X nodes from the lowest node up. Default is to not limit.
	 *                hierarchicalDelimiter = Text to place between items in a hierarchy for a hierarchical
	 *                specification (eg. ca_objects.hierarchy) when returning as a string removeFirstItems = If set to
	 *                a non-zero value, the specified number of items at the top of the hierarchy will be omitted. For example, if set to 2, the root and first child of the hierarchy will be omitted. Default is zero (don't delete anything). transaction = The transaction to execute database actions within. [Default is null] filterTypes = Filter hierarchy showing only item with specified types. Implies showHierarchy option. [Default is null]
	 *
	...
	public function getDisplayValue( $pa_options = null ) {
		...
		if ( ! $pa_options['showHierarchy'] ) {
			...
			if ( $vb_return_idno = ( ( isset( $pa_options['returnDisplayText'] )
			                           && (bool) $pa_options['returnDisplayText'] ) )
			) {
				return caGetListItemByIDForDisplay( $this->opn_item_id, array_merge( $pa_options,
					[ 'return' => caGetOption( 'useSingular', $pa_options, false ) ? 'singular' : 'plural' ] ) );
			}

			if ( is_null( $vb_ids_only = isset( $pa_options['idsOnly'] ) ? (bool) $pa_options['idsOnly'] : null ) ) {
				$vb_ids_only = isset( $pa_options['alwaysReturnItemID'] ) ? (bool) $pa_options['alwaysReturnItemID']
					: false;
			}
		...
		$vn_list_id = ( is_array( $pa_options ) && isset( $pa_options['list_id'] ) ) ? (int) $pa_options['list_id']
			: null;
		...
		if ( $vn_list_id > 0 ) {
			...
			$vs_get_spec = ( ( isset( $pa_options['useSingular'] ) && $pa_options['useSingular'] )
				? 'preferred_labels.name_singular' : 'preferred_labels.name_plural' );
			...
			if ( $pa_options['showHierarchy'] ) {
				...
				if ( is_array( $pa_options['filterTypes'] ) ) {
					return $t_item->get( 'ca_list_items.hierarchy.' . $vs_get_spec, array_merge( array(
						'filterTypes' => $pa_options['filterTypes'],
						'delimiter'   => ' ➔ ',
						$pa_options
					) ) );
				}

				return $t_item->get( 'ca_list_items.hierarchy.' . $vs_get_spec,
					array_merge( array( 'delimiter' => ' ➔ ', $pa_options ) ) );
			}

			return $t_list->getItemFromListForDisplayByItemID( $vn_list_id, $this->opn_item_id,
	 ...
	 *                          alwaysTreatValueAsIdno = Always try to convert $ps_value to a list idno value, even if
	 *                          it is numeric matchOn =
	 *
	 * @return array
	 */
	public function parseValue( $ps_value, $pa_element_info, $pa_options = null ) {
		...
		$vb_require_value = ( is_null( $pa_element_info['settings']['requireValue'] ) ) ? false
			: (bool) $pa_element_info['settings']['requireValue'];
		...
		foreach ( $va_match_on as $vs_match_on ) {
			switch ( $vs_match_on ) {
				...
				case 'item_id':
				default:
					if ( $vn_id = ca_list_items::find( array(
						'item_id' => (int) $ps_value,
						'list_id' => $pa_element_info['list_id']
					), array( 'returnAs' => 'firstId', 'transaction' => $o_trans ) )
					) {
						break( 2 );
					}
					break;
		...
		if ( ( ! $vn_id ) && ( $o_log = caGetOption( 'log', $pa_options, null ) ) && ( strlen( $ps_value ) > 0 ) ) {
			$o_log->logError( _t( 'Value %1 was not set for %2 because it does not exist in list %3', $ps_value,
				caGetOption( 'logReference', $pa_options, '???' ), caGetListCode( $pa_element_info['list_id'] ) ),
				[
		...
		if ( ! $vb_require_value && ! $vn_id ) {
			...
			$this->postError( 1970, _t( 'Value for %1 [%2] cannot be blank', $pa_element_info['displayLabel'],
				$pa_element_info['element_code'] ), 'ListAttributeValue->parseValue()' );

			return false;
		} elseif ( $vb_require_value && ! $vn_id ) {
			$this->postError( 1970, _t( 'Value %3 for %1 [%2] is invalid', $pa_element_info['displayLabel'],
				$pa_element_info['element_code'], $ps_value ), 'ListAttributeValue->parseValue()' );
	 ...
	 *
	 * @param $pa_element_info array Array with information about the metadata element with which this value is
	 *                         associated. Keys taken to be ca_metadata_elements field names and the 'settings' field
	 *                         must be deserialized into an array.
	 * @param $pa_options      array Array of options. Supported options are:
	 *                         width - The width of the list drop-down in characters unless suffixed with 'px' in which
	 *                         case width will be set in pixels. any option supported by
	 *                         ca_lists::getListAsHTMLFormElement with the exception of 'maxColumns', which is set out
	 *                         of information in $pa_element_info
	 *
	...
	public function htmlFormElement( $pa_element_info, $pa_options = null ) {
		...
		if ( ( ! isset( $pa_options['width'] ) || ! strlen( $pa_options['width'] ) )
		     && isset( $pa_element_info['settings']['listWidth'] )
		     && strlen( $pa_element_info['settings']['listWidth'] ) > 0
		) {
			$pa_options['width'] = $pa_element_info['settings']['listWidth'];
		}
		if ( ( ! isset( $pa_options['height'] ) || ! strlen( $pa_options['height'] ) )
		     && isset( $pa_element_info['settings']['listHeight'] )
		     && strlen( $pa_element_info['settings']['listHeight'] ) > 0
		) {
		...
		$vs_render      = $for_search ? ""
			: caGetOption( 'render', $pa_options, caGetOption( 'render', $pa_element_info['settings'], '' ) );
		$vb_auto_shrink = (bool) caGetOption( 'auto_shrink', $pa_options,
			caGetOption( 'auto_shrink', $pa_element_info['settings'], false ) );

		$current_selection_display_format = caGetOption( 'currentSelectionDisplayFormat', $pa_options,
			caGetOption( 'currentSelectionDisplayFormat', $pa_element_info['settings'], null ) );
		...
		if ( ! isset( $pa_options['useDefaultWhenNull'] ) ) {
			$pa_options['useDefaultWhenNull'] = isset( $pa_element_info['settings']['useDefaultWhenNull'] )
				? (bool) $pa_element_info['settings']['useDefaultWhenNull'] : false;
		}
		...
		$vs_element = ca_lists::getListAsHTMLFormElement(
			...
			array_merge(
				...
				[
					'render'                        => $vs_render,
					'maxColumns'                    => $vn_max_columns,
					'element_id'                    => $pa_element_info['element_id'],
					'nullOption'                    => $vb_null_option,
					'implicitNullOption'            => $vb_implicit_nulls,
					'auto_shrink'                   => $vb_auto_shrink,
		...
		if ( Configuration::load()->get( 'enable_dependent_field_visibility' ) ) {
			...
			if ( $vb_print_js ) {
				...
				if ( ! $pa_element_info['settings']['requireValue']
				     && is_array( $pa_element_info['settings']['hideIfSelected___null__'] )
				     && sizeof( $pa_element_info['settings']['hideIfSelected___null__'] )
				) {
					...
					foreach ( $va_list_items as $va_items_by_locale ) {
						foreach ( $va_items_by_locale as $vn_locale_id => $va_item ) {
							...
							if ( isset( $pa_element_info['settings'][ 'hideIfSelected_' . $va_item['idno'] ] ) ) {
								$va_hideif_for_idno = $pa_element_info['settings'][ 'hideIfSelected_'
								                                                    . $va_item['idno'] ];
							...
							switch ( $pa_element_info['settings']['render'] ) {
								case 'radio_buttons':
									$vs_condition = "jQuery('input[name={fieldNamePrefix}"
									                . $pa_element_info['element_id'] . "_{n}]:checked').val() === '"
									                . $va_item['item_id'] . "'";
									break;
								case 'yes_no_checkboxes':
									if ( $vb_yes_was_set ) {
										$vs_condition = "!(jQuery('input[name={fieldNamePrefix}"
										                . $pa_element_info['element_id'] . "_{n}]').is(':checked'))";
									} else {
										$vb_yes_was_set = true;
										$vs_condition   = "jQuery('input[name={fieldNamePrefix}"
										                  . $pa_element_info['element_id'] . "_{n}]').is(':checked')";
...
<script type='text/javascript'>jQuery(document).ready(function() { 
	...
	public function getAvailableSettings( $pa_element_info = null ) {
		...
		if (
			Configuration::load()->get( 'enable_dependent_field_visibility' ) && is_array( $pa_element_info )
			&& isset( $pa_element_info['list_id'] )
			&& // select is the default, so empty does count
			( ! ( $pa_element_info['settings']['render'] )
			  || in_array( $pa_element_info['settings']['render'],
					array( 'select', 'radio_buttons', 'yes_no_checkboxes' ) ) )
			&& $g_request
			&& ( $g_request instanceof RequestHTTP )
		) {
			/*
				...
				foreach ( $va_ui_list as $vn_ui_id => $vs_ui_name ) {
					...
					foreach ( $t_ui->getScreens() as $va_screen ) {
						...
						foreach ( $t_ui->getScreenBundlePlacements( $va_screen['screen_id'] ) as $va_placement ) {
							$m = [];
							if ( preg_match( "!^ca_attribute_([A-Za-z0-9_]+)$!", $va_placement['bundle_name'], $m )
							     && ( $m[1] === $pa_element_info['element_code'] )
							) {
								continue;
							}

							$va_options_for_settings[ $t_ui->get( 'editor_code' ) . '/' . $va_screen['idno'] . '/'
							                          . ( isset( $m[1] ) ? $m[1] : $va_placement['placement_code'] ) ]
								= $t_ui->get( 'editor_code' ) . '/' . $va_screen['idno'] . '/'
								  . $va_placement['placement_code'];
						}
					}
				}
			}
		...
		} elseif ( defined( '__CollectiveAccess_Installer__' )
			...
			if ( is_array( $va_list_items ) && ( sizeof( $va_list_items ) > 0 ) ) {
				foreach ( $va_list_items as $va_items_by_locale ) {
					foreach ( $va_items_by_locale as $vn_locale_id => $va_item ) {
						$va_element_settings[ 'hideIfSelected_' . $va_item['idno'] ] = [
							'deferred' => true,
							'multiple' => true
						];
					}
	 ...
	 *
	 * @return boolean True if value is valid for setting, false if not. If validation fails an error message is
	 *                 returned in $ps_error
	 */
	public function validateSetting( $pa_element_info, $ps_setting_key, $ps_value, &$ps_error ) {
		...
		if ( preg_match( "/^hideIfSelected/", $ps_setting_key ) ) {
			if ( isset( $pa_element_info['settings']['render'] )
			     && ! is_null( $pa_element_info['settings']['render'] )
			) {
				if ( ! in_array( $pa_element_info['settings']['render'],
					array( 'radio_buttons', 'select', 'yes_no_checkboxes' ) )
				) {
					$ps_error
						= _t( "dependent field visibility is only supported for radio buttons and drop-down (select) menus" );
	...
	/**
	 * Returns a list of ui id, screen id and placement id for a given setting key
	 * (editor_code/screen_idno/placement_code)
	 *
	 * @param string $ps_key
	 *
	 * @return array|bool
	...
	public static function resolveHideIfSelectedKey( $ps_key ) {
		...
		if ( ! $t_placement->load( array(
			'screen_id'      => $t_screen->getPrimaryKey(),
			'placement_code' => $va_tmp[2]
		) )
		) {
			return false;
		}
