...
$_ca_attribute_settings['ListAttributeValue'] = array(        // global
    'listWidth' => array(
        ...
        'description' => _t(
            'Width, in characters or pixels, of the list when displayed in a user interface. When list is rendered as a hierarchy browser width must be in pixels.'
        )
    ),
    'listHeight' => array(
        'formatType' => FT_NUMBER,
    ...
    'doesNotTakeLocale' => array(
        ...
        'description' => _t(
            'Check this option if you don\'t want your list values to be locale-specific. (The default is to not be.)'
        )
    ),
    'requireValue' => array(
        'formatType' => FT_NUMBER,
    ...
    'allowDuplicateValues' => array(
        ...
        'description' => _t(
            'Check this option if you want to allow duplicate values to be set when element is not in a container and is repeating.'
        )
    ),
    ...
    'useDefaultWhenNull' => array(
        ...
        'description' => _t(
            'Check this option if the list default value should be used when the item value is null. (The default is to disregard the default value and show the null value.)'
        )
    ),
    'canBeUsedInSort' => array(
        ...
        'description' => _t(
            'Check this option if this attribute value can be used for sorting of search results. (The default is to be.)'
        )
    ),
    'render' => array(
        'formatType' => FT_TEXT,
    ...
    'auto_shrink' => array(
        ...
        'description' => _t(
            'Check this option if you want the hierarchy browser to automatically shrink or expand based on the height of the column with the most data. Only affects the horizontal browser!'
        )
    ),
    'maxColumns' => array(
        'formatType' => FT_NUMBER,
    ...
    'canBeUsedInSearchForm' => array(
        ...
        'description' => _t(
            'Check this option if this attribute value can be used in search forms. (The default is to be.)'
        )
    ),
    'canBeUsedInDisplay' => array(
        ...
        'description' => _t(
            'Check this option if this attribute value can be used for display in search results. (The default is to be.)'
        )
    ),
    'canMakePDF' => array(
        'formatType' => FT_NUMBER,
        ...
        'description' => _t(
            'Check this option if this metadata element can be output as a printable PDF. (The default is not to be.)'
        )
    ),
    'canMakePDFForValue' => array(
        ...
        'description' => _t(
            'Check this option if individual values for this metadata element can be output as a printable PDF. (The default is not to be.)'
        )
    ),
    'displayTemplate' => array(
        ...
        'validForRootOnly' => 1,
        'description' => _t(
            'Layout for value when used in a display (can include HTML). Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^my_element_code</i>.'
        )
    ),
    ...
    'currentSelectionDisplayFormat' => array(
        ...
        'validForRootOnly' => 1,
        'description' => _t(
            'Template formatting current selection text. You may use the following tags: ^current (the currently selected list item), ^parent (the parent of the currently selected list item), ^hierarchy (the full hierarchal path to the list item).'
        )
    ),
    'minimizeExistingValues' => array(
        ...
        'description' => _t(
            'Check this option if existing values should displayed in a minimized, non-editable format.'
        )
    )
);
...
class ListAttributeValue extends AuthorityAttributeValue implements IAttributeValue
{
    ...
    public function getDisplayValue($pa_options = null)
    {
        ...
        if (!$pa_options['showHierarchy']) {
            ...
            if ($vb_return_idno = ((isset($pa_options['returnDisplayText']) && (bool)$pa_options['returnDisplayText']))) {
                return caGetListItemByIDForDisplay(
                    $this->opn_item_id,
                    array_merge(
                        $pa_options,
                        [
        ...
        if ($vn_list_id > 0) {
            ...
            if ($pa_options['showHierarchy']) {
                ...
                if (is_array($pa_options['filterTypes'])) {
                    return $t_item->get(
                        'ca_list_items.hierarchy.' . $vs_get_spec,
                        array_merge(
                            array(
            ...
            return $t_list->getItemFromListForDisplayByItemID(
                $vn_list_id,
                $this->opn_item_id,
                array_merge(
                    $pa_options,
    ...
    public function parseValue($ps_value, $pa_element_info, $pa_options = null)
    {
        ...
        $vb_require_value = (is_null(
            $pa_element_info['settings']['requireValue']
        )) ? false : (bool)$pa_element_info['settings']['requireValue'];
        ...
        foreach ($va_match_on as $vs_match_on) {
            switch ($vs_match_on) {
                ...
                case 'item_id':
                default:
                    if ($vn_id = ca_list_items::find(
                        array('item_id' => (int)$ps_value, 'list_id' => $pa_element_info['list_id']),
        ...
        if ((!$vn_id) && ($o_log = caGetOption('log', $pa_options, null)) && (strlen($ps_value) > 0)) {
            $o_log->logError(
                _t(
                    'Value %1 was not set for %2 because it does not exist in list %3',
                    $ps_value,
                    caGetOption('logReference', $pa_options, '???'),
                    caGetListCode($pa_element_info['list_id'])
                ),
                [
        ...
        if (!$vb_require_value && !$vn_id) {
            ...
            $this->postError(
                1970,
                _t(
                    'Value for %1 [%2] cannot be blank',
                    $pa_element_info['displayLabel'],
                    $pa_element_info['element_code']
                ),
                'ListAttributeValue->parseValue()'
            );
            return false;
    ...
    public function htmlFormElement($pa_element_info, $pa_options = null)
    {
        ...
        if ((!isset($pa_options['width']) || !strlen(
                    $pa_options['width']
                )) && isset($pa_element_info['settings']['listWidth']) && strlen(
                $pa_element_info['settings']['listWidth']
            ) > 0) {
            $pa_options['width'] = $pa_element_info['settings']['listWidth'];
        }
        if ((!isset($pa_options['height']) || !strlen(
                    $pa_options['height']
                )) && isset($pa_element_info['settings']['listHeight']) && strlen(
                $pa_element_info['settings']['listHeight']
            ) > 0) {
        ...
        if (isset($pa_options['nullOption']) && strlen($pa_options['nullOption'])) {
            ...
            $vb_null_option = !$vb_require_value ? ($pa_element_info['settings']['nullOptionText'] ? $pa_element_info['settings']['nullOptionText'] : _t(
                'Not set'
            )) : null;
        }
        ...
        $vs_render = $for_search ? "" : caGetOption(
            'render',
            $pa_options,
            caGetOption('render', $pa_element_info['settings'], '')
        );
        $vb_auto_shrink = (bool)caGetOption(
            'auto_shrink',
            $pa_options,
            caGetOption('auto_shrink', $pa_element_info['settings'], false)
        );

        $current_selection_display_format = caGetOption(
            'currentSelectionDisplayFormat',
            $pa_options,
            caGetOption(
                'currentSelectionDisplayFormat',
                $pa_element_info['settings'],
                null
            )
        );
        ...
        if (Configuration::load()->get('enable_dependent_field_visibility')) {
            ...
            if ($vb_print_js) {
                ...
                $cases = $all_ids = [];
                if (!$pa_element_info['settings']['requireValue'] && is_array(
                        $pa_element_info['settings']['hideIfSelected___null__']
            ...
            if (is_array($cases) && sizeof($cases)) {
                ...
                $all_ids_sel = array_map(
                    function ($v) {
                        return "#{$v}";
                    },
                    $all_ids
                );
                $case_switch = array_map(
                    function ($v) {
                        $id_list = array_map(
                            function ($x) {
                                return "#{$x}";
...
<script type='text/javascript'>jQuery(document).ready(function() { 
    ...
    public function getAvailableSettings($pa_element_info = null)
    {
        ...
        if (
            ...
            (!($pa_element_info['settings']['render']) || in_array(
                    $pa_element_info['settings']['render'],
                ...
                foreach ($va_ui_list as $vn_ui_id => $vs_ui_name) {
                    ...
                    foreach ($t_ui->getScreens() as $va_screen) {
                        ...
                        foreach ($t_ui->getScreenBundlePlacements($va_screen['screen_id']) as $va_placement) {
                            ...
                            $va_options_for_settings[$t_ui->get(
                                'editor_code'
                            ) . '/' . $va_screen['idno'] . '/' . (isset($m[1]) ? $m[1] : $va_placement['placement_code'])] = $t_ui->get(
                                    'editor_code'
                                ) . '/' . $va_screen['idno'] . '/' . $va_placement['placement_code'];
                        }
                    }
                }
            }
                ...
                foreach ($t_list->getItemsForList($pa_element_info['list_id']) as $va_items_by_locale) {
                    if (!$pa_element_info['settings']['requireValue']) {
                        $va_element_settings['hideIfSelected___null__'] = array(
                            ...
                            'description' => _t(
                                'Select bundles from the list below to be hidden when no value is set for this element.'
                            )
                        );
                    }
                    foreach ($va_items_by_locale as $vn_locale_id => $va_item) {
        ...
        } elseif (defined('__CollectiveAccess_Installer__') && Configuration::load()->get(
                'enable_dependent_field_visibility'
            )) {
     ...
     * @return bool True if value is valid for setting, false if not. If validation fails an error message is returned in $ps_error
     */
        ...
        if (preg_match("/^hideIfSelected/", $ps_setting_key)) {
            if (isset($pa_element_info['settings']['render']) && !is_null($pa_element_info['settings']['render'])) {
                if (!in_array(
                    ...
                    $ps_error = _t(
                        "dependent field visibility is only supported for radio buttons and drop-down (select) menus"
                    );
                    return false;
                }
            }
