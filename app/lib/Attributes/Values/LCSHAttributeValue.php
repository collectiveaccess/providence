...
class LCSHAttributeValue extends AttributeValue implements IAttributeValue {
	...
	public function getDisplayValue( $pa_options = null ) {
		if ( isset( $pa_options['asHTML'] ) && $pa_options['asHTML'] ) {
			if ( preg_match( '!sh([\d]+)!', $this->ops_text_value, $va_matches ) ) {
				...
				return "<a href='http://id.loc.gov/authorities/sh" . $va_matches[1] . "' target='_lcsh_details'>"
				       . $vs_value . '</a>';
			}
		}
		...
		if ( caGetOption( [ 'id', 'idno' ], $pa_options, false )
		     && preg_match( '!\[([^\]]*)!', $this->ops_text_value, $va_matches )
		) {
			return $va_matches[1];
		}
		if ( caGetOption( 'n', $pa_options, false )
		     && preg_match( '!\[([^\]]*)!', $this->ops_text_value, $va_matches )
		) {
	 ...
	 *                           matchUsingLOCLabel = Match term using LOC label data rather than LOC subject heading
	 *                           search. The former is much more restrictive. [Default is false]
	 *
	 */
	public function parseValue( $ps_value, $pa_element_info, $pa_options = null ) {
			...
			if ( is_array( $va_tmp ) && ( sizeof( $va_tmp ) > 1 ) ) {
				...
				if ( $vs_service_url ) {
					...
					try {
						...
						$this->postError( 1970,
							_t( 'Could not connect to LCSH service for %1: %2', $ps_value, $e->getMessage() ),
							'LCSHAttributeValue->parseValue()' );
					...
					if ( ( isset( $va_headers['X-preflabel'] ) ) && $va_headers['X-preflabel'] ) {
						...
						if ( $vs_url ) {
							...
							$this->postError( 1970,
								_t( 'Could not get results from LCSH service for %1 [%2]', $ps_value, $vs_service_url ),
								'LCSHAttributeValue->parseValue()' );
						...
						$this->postError( 1970,
							_t( 'Could not get results from LCSH service for %1 [%2]; status was %3', $ps_value,
								$vs_service_url, $vn_status ), 'LCSHAttributeValue->parseValue()' );
		...
		if ( is_array( LCSHAttributeValue::$s_term_cache )
		     && ( sizeof( LCSHAttributeValue::$s_term_cache ) > LCSHAttributeValue::$s_term_cache_max_size )
		) {
	 ...
	 *                               forSearch = settings and options regarding visual text editor are ignored
	 *                               [Default=false] class = the CSS class to apply to all visible form elements
	 *                               [Default=lookupBg] width = the width of the form element [Default=field width
	 *                               defined in metadata element definition] height = the height of the form element
	 *                               [Default=field height defined in metadata element definition] request = the
	 *                               RequestHTTP object for the current request; required for lookups to work [Default
	 *                               is null]
	 *
	 * @return string
	 */
	public function htmlFormElement( $pa_element_info, $pa_options = null ) {
		...
		if ( $pa_options['request'] ) {
			$vs_url = caNavUrl( $pa_options['request'], 'lookup', 'LCSH', 'Get',
				array( 'max' => 100, 'element_id' => (int) $pa_element_info['element_id'] ) );
		} else {
			// hardcoded default for testing.
			$vs_url = '/index.php/lookup/LCSH/Get';
		}

		$vs_element .= " <a href='#' class='caLCSHServiceMoreLink' id='{fieldNamePrefix}"
		               . $pa_element_info['element_id'] . "_link{n}' target='_lcsh_details'>" . _t( "More &rsaquo;" )
		               . "</a>";
