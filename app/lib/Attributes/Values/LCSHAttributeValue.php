...
$_ca_attribute_settings['LCSHAttributeValue'] = array(        // global
    ...
    'doesNotTakeLocale' => array(
        ...
        'description' => _t(
            'Check this option if you don\'t want your LCSH values to be locale-specific. (The default is to not be.)'
        )
    ),
...
class LCSHAttributeValue extends AttributeValue implements IAttributeValue
{
    ...
    static public $s_term_cache = array();
    static public $s_term_cache_max_size = 2048;
    ...
    public function getDisplayValue($pa_options = null)
    {
        ...
        if (caGetOption(['id', 'idno'], $pa_options, false) && preg_match(
                '!\[([^\]]*)!',
                $this->ops_text_value,
                $va_matches
            )) {
    ...
    public function parseValue($ps_value, $pa_element_info, $pa_options = null)
    {
                ...
                if ($vs_service_url) {
                    ...
                    try {
                        ...
                        $this->postError(
                            1970,
                            _t(
                                'Could not connect to LCSH service for %1: %2',
                                $ps_value,
                                $e->getMessage()
                            ),
                    ...
                    if ((isset($va_headers['X-preflabel'])) && $va_headers['X-preflabel']) {
                        ...
                        if ($vs_url) {
                            ...
                            $this->postError(
                                1970,
                                _t(
                                    'Could not get results from LCSH service for %1 [%2]',
                                    $ps_value,
                                    $vs_service_url
                                ),
                                'LCSHAttributeValue->parseValue()'
                        ...
                        $this->postError(
                            1970,
                            _t(
                                'Could not get results from LCSH service for %1 [%2]; status was %3',
                                $ps_value,
                                $vs_service_url,
                                $vn_status
                            ),
        ...
        if (is_array(LCSHAttributeValue::$s_term_cache) && (sizeof(
                    LCSHAttributeValue::$s_term_cache
                ) > LCSHAttributeValue::$s_term_cache_max_size)) {
    ...
    public function htmlFormElement($pa_element_info, $pa_options = null)
    {
        ...
        $vs_element .= " <a href='#' class='caLCSHServiceMoreLink' id='{fieldNamePrefix}" . $pa_element_info['element_id'] . "_link{n}' target='_lcsh_details'>" . _t(
                "More &rsaquo;"
            ) . "</a>";
