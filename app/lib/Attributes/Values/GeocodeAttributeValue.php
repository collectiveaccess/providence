...
class GeocodeAttributeValue extends AttributeValue implements IAttributeValue {
	...
	/**
	 * Returns value of geocode suitable for display output. This consists of the user's entry + the geocoded
	 * coordinates surrounded by brackets ("[" and "]"). If you need to get the coordinates parsed out and suitable for
	 * mapping then pass the 'coordinates' option set to true; this will cause an array to be returned with keys for
	 * latitude, longitude (for the first point, if a path), path (a string with all coordinates in the path) and label
	 * (the display string)
	 *
	 * @param $pa_options         - options for generating display value. Supported options are:
	 *                            coordinates - if passed a representation of the geocode value with coordinates parse
	 *                            is returned as an array. This array has the following keys: latitude - the latitude
	 *                            of the first point in the geocode longitude - the longitude of the first point in the
	 *                            geocode path - a full path of coordinates (useful if the geocode is a path rather
	 *                            than a point) as a string with each coordinate pair separated with semicolons label -
	 *                            the display text for the geocode path - only return path as plain text (the path is a
	 *                            colon-delimited list of coordinates)
	 *
	 * @return mixed - will return string with display value by default; array with parsed coordinate values if the
	 *               "coordinates" option is passed
	 */
	public function getDisplayValue( $pa_options = null ) {
		if ( isset( $pa_options['coordinates'] ) && $pa_options['coordinates'] ) {
			return array(
				'latitude'  => $this->opn_latitude,
				'longitude' => $this->opn_longitude,
				'path'      => $this->ops_path_value,
	...
	public function parseValue( $ps_value, $pa_element_info, $pa_options = null ) {
		...
		if ( ! trim( $ps_value ) ) {
			if ( $va_settings['mustNotBeBlank'] ) {
				$this->postError( 1970, _t( 'Address or georeference was blank.' ),
		...
		if (
		...
		} elseif ( preg_match( "!^([\-]{0,1}[\d]{1,3})[\D]+([\d]{1,2})[\D]+([\d\.]+)[^NSEW]+([NSEW]{1})[\D]+([\-]{0,1}[\d]{1,3})[\D]+([\d]{1,2})[\D]+([\d\.]+)[^NSEW]+([NSEW]{1})!",
			$ps_value, $va_matches )
		) {
			...
			$vs_google_response = @file_get_contents( 'https://maps.googleapis.com/maps/api/geocode/json?address='
			                                          . urlencode( $ps_value ) . '&sensor=false'
			                                          . ( ( defined( "__CA_GOOGLE_MAPS_KEY__" )
			                                                && __CA_GOOGLE_MAPS_KEY__ ) ? "&key="
			                                                                              . __CA_GOOGLE_MAPS_KEY__
					: "" ) );
			if ( ! ( $va_google_response = json_decode( $vs_google_response, true ) )
			     || ! isset( $va_google_response['status'] )
			) {
				$this->postError( 1970, _t( 'Could not connect to Google for geocoding' ),
					'GeocodeAttributeValue->parseValue()' );

				return false;
			}

			if ( ( $va_google_response['status'] != 'OK' ) || ! isset( $va_google_response['results'] )
			     || sizeof( $va_google_response['results'] ) == 0
			) {
				$this->postError( 1970,
					_t( 'Could not geocode address "%1": [%2]', $ps_value, $va_google_response['status'] ),
					'GeocodeAttributeValue->parseValue()' );
			...
			if ( isset( $va_first_result['geometry']['location'] )
			     && is_array( $va_first_result['geometry']['location'] )
			) {
				return array(
					'value_longtext1' => $ps_value,
					'value_longtext2' => $va_first_result['geometry']['location']['lat'] . ','
					                     . $va_first_result['geometry']['location']['lng'],
					'value_decimal1'  => $va_first_result['geometry']['location']['lat'],
					'value_decimal2'  => $va_first_result['geometry']['location']['lng']
				);
			} else {
				$this->postError( 1970, _t( 'Could not geocode address "%1"', $ps_value ),
					'GeocodeAttributeValue->parseValue()' );
	...
	public function htmlFormElement( $pa_element_info, $pa_options = null ) {
		...
		if ( ( ! isset( $pa_options['baseLayer'] ) || ! $pa_options['baseLayer'] )
		     || ( isset( $pa_options['request'] )
		          && ( $pa_options['request'] ) )
		) {
