...
$_ca_attribute_settings['GeocodeAttributeValue'] = array(        // global
    ...
    'pointsAreDirectional' => array(
        ...
        'description' => _t(
            'Check this option to enable setting of directions for point locations. (The default is not to be.)'
        )
    ),
    ...
    'tileServerURL' => array(
        ...
        'validForRootOnly' => 0,
        'description' => _t(
            'URL for tileserver to load custom tiles from, with placeholders for X, Y and Z values in the format <em>${x}</em>. Ex. http://tileserver.net/maps/${z}/${x}/${y}.png. Leave blank if you do not wish to use custom map tiles.'
        )
    ),
    'tileLayerName' => array(
        'formatType' => FT_TEXT,
        ...
        'validForRootOnly' => 0,
        'description' => _t(
            'Display name for layer containing tiles loaded from tile server specified in the <em>tile server URL</em> setting.'
        )
    ),
...
class GeocodeAttributeValue extends AttributeValue implements IAttributeValue
{
    ...
    public function parseValue($ps_value, $pa_element_info, $pa_options = null)
    {
        ...
        if (
            ...
            "!^([\-]{0,1}[\d]{1,3})[\D]+([\d]{1,2})[\D]+([\d\.]+)[^NSEW]+([NSEW]{1})[\D]+([\-]{0,1}[\d]{1,3})[\D]+([\d]{1,2})[\D]+([\d\.]+)[^NSEW]+([NSEW]{1})!",
            $ps_value,
            $va_matches
        )) {
            ...
            $vs_google_response = @file_get_contents(
                'https://maps.googleapis.com/maps/api/geocode/json?address=' . urlencode($ps_value) . '&sensor=false'
            );
            if (!($va_google_response = json_decode(
                    $vs_google_response,
                    true
                )) || !isset($va_google_response['status'])) {
                $this->postError(
                    1970,
                    _t('Could not connect to Google for geocoding'),
                    'GeocodeAttributeValue->parseValue()'
                );
                return false;
            }

            if (($va_google_response['status'] != 'OK') || !isset($va_google_response['results']) || sizeof(
                    $va_google_response['results']
                ) == 0) {
                $this->postError(
                    1970,
            ...
            if (isset($va_first_result['geometry']['location']) && is_array($va_first_result['geometry']['location'])) {
                ...
                $this->postError(
                    1970,
                    _t('Could not geocode address "%1"', $ps_value),
                    'GeocodeAttributeValue->parseValue()'
                );
                return false;
            }
