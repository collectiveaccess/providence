...
$_ca_attribute_settings['TextAttributeValue'] = array(        // global
    ...
    'regex' => array(
        ...
        'description' => _t(
            'A Perl-format regular expression with which to validate the input. Input not matching the expression will be rejected. Do not include the leading and trailling delimiter characters (typically "/") in your expression. Leave blank if you don\'t want to use regular expression-based validation.'
        )
    ),
    'fieldWidth' => array(
        'formatType' => FT_NUMBER,
    ...
    'doesNotTakeLocale' => array(
        ...
        'description' => _t(
            'Check this option if you don\'t want your text to be locale-specific. (The default is to be.)'
        )
    ),
    ...
    'suggestExistingValues' => array(
        ...
        'description' => _t(
            'Check this option if you want this attribute to suggest previously saved values as text is entered. This option is only effective if the display height of the text entry is equal to 1.'
        )
    ),
    'suggestExistingValueSort' => array(
        ...
        'description' => _t(
            'If suggestion of existing values is enabled this option determines how returned values are sorted. Choose <em>value</em> to sort alphabetically. Choose <em>most recently added </em> to sort with most recently entered values first.'
        ),
        'options' => array(
            _t('Value') => 'value',
    ...
    'displayTemplate' => array(
        ...
        'validForRootOnly' => 1,
        'description' => _t(
            'Layout for value when used in a display (can include HTML). Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <em>^my_element_code</em>.'
        )
    ),
    ...
    'isDependentValue' => array(
        ...
        'description' => _t(
            'If set then this element is set using values in other fields in the same container. This is only relevant when the element is in a container and is ignored otherwise.'
        )
    ),
    'dependentValueTemplate' => array(
        ...
        'validForNonRootOnly' => 1,
        'description' => _t(
            'Template to be used to format content for dependent values. Template should reference container values using their bare element code prefixed with a caret (^). Do not include the table or container codes.'
        )
    ),
    'mustBeUnique' => array(
        'formatType' => FT_NUMBER,
...
class TextAttributeValue extends AttributeValue implements IAttributeValue
{
    ...
    public function parseValue($ps_value, $pa_element_info, $pa_options = null)
    {
        ...
        if ($vn_strlen > $va_settings['maxChars']) {
            // text is too short
            $vs_err_msg = ($va_settings['maxChars'] == 1) ? _t(
                '%1 must be no more than 1 character long',
                $pa_element_info['displayLabel']
            ) : _t(
                '%1 must be no more than %2 characters long',
                $pa_element_info['displayLabel'],
                $va_settings['maxChars']
            );
            $this->postError(1970, $vs_err_msg, 'TextAttributeValue->parseValue()');
            return false;
        }
        ...
        if ($va_settings['regex'] && !preg_match("!" . $va_settings['regex'] . "!", $ps_value)) {
            ...
            $this->postError(
                1970,
                _t('%1 does not conform to required format', $pa_element_info['displayLabel']),
                'TextAttributeValue->parseValue()'
            );
            return false;
        }

        if (isset($va_settings['mustBeUnique']) && (bool)$va_settings['mustBeUnique'] && ($vn_strlen > 0)) {
            if (BaseModelWithAttributes::valueExistsForElement(
                ...
                $this->postError(
                    1970,
                    _t(
                        '%1 must be unique across all values. The value you entered already exists.',
                        $pa_element_info['displayLabel']
                    ),
                    'TextAttributeValue->parseValue()'
                );
    ...
    public function htmlFormElement($pa_element_info, $pa_options = null)
    {
        ...
        $va_settings = $this->getSettingValuesFromElementArray(
            ...
            array(
                ...
                'minChars',
                'maxChars',
                'suggestExistingValues',
                'usewysiwygeditor',
                'isDependentValue',
                'dependentValueTemplate',
                'mustBeUnique',
                'referenceMediaIn'
            )
        );
        ...
        $vs_width = trim(
            (isset($pa_options['width']) && $pa_options['width'] > 0) ? $pa_options['width'] : $va_settings['fieldWidth']
        );
        $vs_height = trim(
            (isset($pa_options['height']) && $pa_options['height'] > 0) ? $pa_options['height'] : $va_settings['fieldHeight']
        );
        ...
        if ($va_settings['usewysiwygeditor']) {
            ...
            $vs_element = "<script type='text/javascript'>jQuery(document).ready(function() {
						var ckEditor = CKEDITOR.replace( '{fieldNamePrefix}" . $pa_element_info['element_id'] . "_{n}',
						{
				            ...
				            contentUrl;: " . ($vb_show_media_content_option ? "'" . caNavUrl(
                        $g_request,
                        '*',
                        '*',
                        'getMediaAttributeList',
                        [
                            'bundle' => $va_settings['referenceMediaIn'],
                            $pa_options['t_subject']->primaryKey() => $pa_options['t_subject']->getPrimaryKey()
                        ]
                    ) . "'" : "null") . ",
        ...
        $va_opts = array(
            ...
            'class' => "{$vs_class}" . ($va_settings['usewysiwygeditor'] ? " ckeditor" : '')
        );
        ...
        if (!caGetOption(
                ...
                $pa_options,
                false
            ) && ($va_settings['isDependentValue'] || $pa_options['isDependentValue'])) {
            ...
            foreach (
                [
                    'inch_decimal_precision',
                    'feet_decimal_precision',
                    'mile_decimal_precision',
                    'millimeter_decimal_precision',
                    'centimeter_decimal_precision',
                    'meter_decimal_precision',
                    'kilometer_decimal_precision',
                    'use_unicode_fraction_glyphs_for',
                    'display_fractions_for',
                    'add_period_after_units',
                    'use_inches_for_display_up_to',
                    'use_feet_for_display_up_to',
                    'use_millimeters_for_display_up_to',
                    'use_centimeters_for_display_up_to',
                    'use_meters_for_display_up_to',
            ...
            $vs_omit_units = ((bool)$o_dimensions_config->get(
                'omit_repeating_units_for_measurements_in_templates'
            )) ? "true" : "false";
            $vs_element .= "<script type='text/javascript'>jQuery(document).ready(function() {
 				    caDisplayTemplateParser.setOptions(" . json_encode($va_parser_opts) . ";)
 					jQuery('#{fieldNamePrefix}" . $pa_element_info['element_id'] . "_{n}').val(caDisplayTemplateParser.processDependentTemplate('" . addslashes(
                    preg_replace("![\r\n]+!", " ", $va_settings['dependentValueTemplate'])
                ) . "', " . json_encode($va_element_dom_ids, JSON_FORCE_OBJECT) . ", true, {$vs_omit_units};))
 				";
        ...
        if (isset($pa_options['t_subject']) && is_object($pa_options['t_subject'])) {
            ...
            if ($pa_options['request']) {
                if (isset($pa_options['lookupUrl']) && $pa_options['lookupUrl']) {
                    ...
                    $vs_lookup_url = caNavUrl(
                        $pa_options['request'],
                        'lookup',
                        'AttributeValue',
                        'Get',
                        array('max' => 500, 'bundle' => $vs_bundle_name)
                    );
                }
            }
        }
        ...
        if (isset($va_settings['mustBeUnique']) && $va_settings['mustBeUnique']) {
            $vs_unique_lookup_url = caNavUrl(
                $pa_options['request'],
                'lookup',
                'AttributeValue',
                'ValueExists',
                array('bundle' => $vs_bundle_name)
            );
            $vs_element .= "<script type='text/javascript'>
