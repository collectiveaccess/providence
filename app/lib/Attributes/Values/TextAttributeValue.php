...
require_once( __CA_LIB_DIR__
              . '/BaseModel.php' );    // we use the BaseModel field type (FT_*) and display type (DT_*) constants
...
class TextAttributeValue extends AttributeValue implements IAttributeValue {
	 ...
	 *                          doRefSubstitution = Parse and replace reference tags (in the form [table
	 *                          idno="X"]...[/table]). [Default is false in Providence; true in Pawtucket].
	 *
	 * @return string
	 */
	...
	public function parseValue( $ps_value, $pa_element_info, $pa_options = null ) {
		...
		if ( $vn_strlen < $va_settings['minChars'] ) {
			// text is too short
			$vs_err_msg = ( $va_settings['minChars'] == 1 ) ? _t( '%1 must be at least 1 character long',
				$pa_element_info['displayLabel'] )
				: _t( '%1 must be at least %2 characters long', $pa_element_info['displayLabel'],
		...
		if ( $vn_strlen > $va_settings['maxChars'] ) {
			// text is too short
			$vs_err_msg = ( $va_settings['maxChars'] == 1 ) ? _t( '%1 must be no more than 1 character long',
				$pa_element_info['displayLabel'] )
				: _t( '%1 must be no more than %2 characters long', $pa_element_info['displayLabel'],
		...
		if ( $va_settings['regex'] && ! preg_match( "!" . $va_settings['regex'] . "!", $ps_value ) ) {
			// regex failed
			$this->postError( 1970, _t( '%1 does not conform to required format', $pa_element_info['displayLabel'] ),
		...
		if ( isset( $va_settings['mustBeUnique'] ) && (bool) $va_settings['mustBeUnique'] && ( $vn_strlen > 0 ) ) {
			...
			if ( BaseModelWithAttributes::valueExistsForElement( $pa_element_info['element_id'], $ps_value,
				[ 'transaction' => $pa_options['transaction'], 'value_id' => $this->getValueID() ] )
			) {
				$this->postError( 1970,
					_t( '%1 must be unique across all values. The value you entered already exists.',
						$pa_element_info['displayLabel'] ), 'TextAttributeValue->parseValue()' );
	 ...
	 *                               usewysiwygeditor = overrides element level setting for visual text editor
	 *                               [Default=false] forSearch = settings and options regarding visual text editor are
	 *                               ignored [Default=false] class = the CSS class to apply to all visible form
	 *                               elements [Default=null] width = the width of the form element [Default=field width
	 *                               defined in metadata element definition] height = the height of the form element
	 *                               [Default=field height defined in metadata element definition] t_subject = an
	 *                               instance of the model to which the attribute belongs; required if
	 *                               suggestExistingValues lookups are enabled [Default is null] request = the
	 *                               RequestHTTP object for the current request; required if suggestExistingValues
	 *                               lookups are enabled [Default is null] suggestExistingValues = suggest values based
	 *                               on existing input for this element as user types [Default is false]
	 *
	 * @return string
	 */
	public function htmlFormElement( $pa_element_info, $pa_options = null ) {
		global $g_request;
		$va_settings = $this->getSettingValuesFromElementArray( $pa_element_info, array(
			'fieldWidth',
			'fieldHeight',
			'minChars',
			'maxChars',
		...
		$vs_width   = trim( ( isset( $pa_options['width'] ) && $pa_options['width'] > 0 ) ? $pa_options['width']
			: $va_settings['fieldWidth'] );
		$vs_height  = trim( ( isset( $pa_options['height'] ) && $pa_options['height'] > 0 ) ? $pa_options['height']
			: $va_settings['fieldHeight'] );
		...
		if ( $va_settings['usewysiwygeditor'] ) {
			...
			if (
				...
				&& ( $vb_show_media_content_option = ( isset( $va_settings['referenceMediaIn'] )
				                                       && (bool) $va_settings['referenceMediaIn'] ) )
			) {
			...
			$vs_element = "<script type='text/javascript'>jQuery(document).ready(function() {
						var ckEditor = CKEDITOR.replace( '{fieldNamePrefix}" . $pa_element_info['element_id'] . "_{n}',
						{
				            ...
				            contentUrl: " . ( $vb_show_media_content_option ? "'" . caNavUrl( $g_request, '*', '*',
		...
		if ( ! caGetOption( 'forSearch', $pa_options, false )
		     && ( $va_settings['isDependentValue']
		          || $pa_options['isDependentValue'] )
		) {
			...
			foreach ( $va_elements as $vn_i => $va_element ) {
				...
				$va_element_dom_ids[ $va_element['element_code'] ] = "#{fieldNamePrefix}" . $va_element['element_id']
				                                                     . "_{n}";
			}
			...
			$vs_omit_units
				        = ( (bool) $o_dimensions_config->get( 'omit_repeating_units_for_measurements_in_templates' ) )
				? "true" : "false";
			$vs_element .= "<script type='text/javascript'>jQuery(document).ready(function() {
 				    caDisplayTemplateParser.setOptions(" . json_encode( $va_parser_opts ) . ");
 					jQuery('#{fieldNamePrefix}" . $pa_element_info['element_id']
			               . "_{n}').val(caDisplayTemplateParser.processDependentTemplate('"
		...
		if ( isset( $pa_options['t_subject'] ) && is_object( $pa_options['t_subject'] ) ) {
			...
			if ( $pa_options['request'] ) {
				if ( isset( $pa_options['lookupUrl'] ) && $pa_options['lookupUrl'] ) {
					...
					$vs_lookup_url = caNavUrl( $pa_options['request'], 'lookup', 'AttributeValue', 'Get',
		...
		if ( isset( $va_settings['mustBeUnique'] ) && $va_settings['mustBeUnique'] ) {
			$vs_unique_lookup_url = caNavUrl( $pa_options['request'], 'lookup', 'AttributeValue', 'ValueExists',
