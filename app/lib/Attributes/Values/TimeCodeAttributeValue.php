...
class TimeCodeAttributeValue extends AttributeValue implements IAttributeValue {
	 ...
	 *                    format = timecode format, either colonDelimited, hoursMinutesSeconds, or raw. [Default is
	 *                    hoursMinutesSeconds] returnAsDecimal = return duration in seconds as decimal number. [Default
	 *                    is false] omitSeconds = omit seconds from returned value. [Default is false]
	 *
		...
		if ( strlen( $ps_value ) ) {
			if ( $o_tcp->parse( $ps_value ) === false ) {
				...
				$this->postError( 1970, _t( '%1 is invalid', $pa_element_info['displayLabel'] ),
					'TimecodeAttributeValue->parseValue()' );
			...
			if ( isset( $va_settings['requireValue'] ) && $va_settings['requireValue'] ) {
				$this->postError( 1970, _t( '%1 must not be empty', $pa_element_info['displayLabel'] ),
					'DataRangeAttributeValue->parseValue()' );
