...
class MediaAttributeValue extends AttributeValue implements IAttributeValue {
	 ...
	 *
	 * Options:
	 *    showMediaInfo - if true media info (dimensions, filesize, bit depth) is returns as part of display; default
	 *    is false version - name of media version to return; default is 'thumbnail' return - valid settings are url,
	 *    tag, path; if set to a valid value then the url, tag or path for the media is returned rather than display
	 *    HTML alt = ALT text to use when generating a media tag
	 *
	 * You can also pass other options to be passed-through to the underlying media plugin. Useful ones for video
	 * include: viewer_width        (also used for audio and tilepic image versions) viewer_height        (also used
	 * for audio and tilepic image versions) poster_frame_version (which will be transformed into the correct
	 * poster_frame_url)
	 */
	public function getDisplayValue( $pa_options = null ) {
			...
			switch ( $pa_options['return'] ) {
				...
				case 'tag':
					return $this->opo_media_info_coder->getMediaTag( $vs_version,
						[ 'alt' => caGetOption( 'alt', $pa_options, null ) ] );
					break;
				case 'path':
				...
				case 'filename':
					...
					return caGetOption( 'ORIGINAL_FILENAME', $media_info,
						pathinfo( $this->opo_media_info_coder->getMediaPath( $vs_version ? $vs_version : 'original' ),
							PATHINFO_BASENAME ) );
					break;
				case 'url':
				default:
		...
		if ( $vs_url = $this->opo_media_info_coder->getMediaUrl( 'original' ) ) {
			...
			if ( $pa_options['showMediaInfo'] ) {
				...
				if ( isset( $va_info['original']['WIDTH'] ) && isset( $va_info['original']['HEIGHT'] ) ) {
					if ( ( $vn_w = $va_info['original']['WIDTH'] ) && ( $vn_h = $va_info['original']['WIDTH'] ) ) {
						$va_dimensions[] = $va_info['original']['WIDTH'] . 'p x ' . $va_info['original']['HEIGHT']
						                   . 'p';
					}
				}
				if ( isset( $va_info['original']['PROPERTIES']['bitdepth'] )
				     && ( $vn_depth
						= $va_info['original']['PROPERTIES']['bitdepth'] )
				) {
					$va_dimensions[] = intval( $vn_depth ) . ' bpp';
				}
				if ( isset( $va_info['original']['PROPERTIES']['colorspace'] )
				     && ( $vs_colorspace
						= $va_info['original']['PROPERTIES']['colorspace'] )
				) {
					$va_dimensions[] = $vs_colorspace;
				}
				if ( isset( $va_info['original']['PROPERTIES']['resolution'] )
				     && is_array( $va_resolution = $va_info['original']['PROPERTIES']['resolution'] )
				) {
				...
				if ( isset( $va_info['original']['PROPERTIES']['duration'] )
				     && ( $vn_duration
						= $va_info['original']['PROPERTIES']['duration'] )
				) {
					$va_dimensions[] = sprintf( "%4.1f", $vn_duration ) . 's';
				}
				if ( isset( $va_info['original']['PROPERTIES']['pages'] )
				     && ( $vn_pages
						= $va_info['original']['PROPERTIES']['pages'] )
				) {
					$va_dimensions[] = $vn_pages . ' ' . ( ( $vn_pages == 1 ) ? _t( 'page' ) : _t( 'pages' ) );
				}

				if ( ! isset( $va_info['PROPERTIES']['filesize'] )
				     || ! ( $vn_filesize
						= $va_info['PROPERTIES']['filesize'] )
				) {
			...
			if ( isset( $pa_options['poster_frame_version'] ) && $pa_options['poster_frame_version'] ) {
				$pa_options['poster_frame_url']
					= $this->opo_media_info_coder->getMediaUrl( $pa_options['poster_frame_version'] );
			}

			$vs_tag = $this->opo_media_info_coder->getMediaTag( $vs_version, $pa_options );

			if ( is_object( $pa_options['request'] ) ) {
				$vs_view_url = urldecode( caNavUrl( $pa_options['request'], $pa_options['request']->getModulePath(),
					$pa_options['request']->getController(), 'GetMediaOverlay',
					array( 'value_id' => $this->opn_value_id ) ) );
				$vs_val      = "<div id='caMediaAttribute" . $this->opn_value_id
				               . "' class='attributeMediaInfoContainer'>";
				...
				$vs_val .= "<div style='float: left;'>" . urlDecode( caNavLink( $pa_options['request'],
						caNavIcon( __CA_NAV_ICON_DOWNLOAD__, 1, array( 'align' => 'middle' ) ), '',
						$pa_options['request']->getModulePath(), $pa_options['request']->getController(),
						'DownloadAttributeFile', array( 'download' => 1, 'value_id' => $this->opn_value_id ),
				...
				$vs_val = "<div id='caMediaAttribute" . $this->opn_value_id
				          . "' class='attributeMediaInfoContainer'><div class='attributeMediaThumbnail'>{$vs_tag}</div></div>";
			}

			if ( $pa_options['showMediaInfo'] ) {
				TooltipManager::add( '#caMediaAttribute' . $this->opn_value_id,
					"<h2>" . _t( 'Media details' ) . "</h2> <p>{$vs_filename}</p><p>{$vs_dimensions}</p>" );
			}
		}
	...
	public function parseValue( $ps_value, $pa_element_info, $pa_options = null ) {
		...
		if ( $media_prefix = caGetOption( 'mediaPrefix', $pa_options, null ) ) {
			...
			if ( $batch_media_directory = $config->get( 'batch_media_import_root_directory' ) ) {
				$va_files = caBatchFindMatchingMedia( $batch_media_directory . $media_prefix, $ps_value, [
					'matchMode' => caGetOption( 'matchMode', $pa_options, 'FILE_NAME' ),
					'matchType' => caGetOption( 'matchType', $pa_options, null ),
		...
		if (
			( is_array( $ps_value ) && $ps_value['_uploaded_file'] && file_exists( $ps_value['tmp_name'] )
			  && ( filesize( $ps_value['tmp_name'] ) > 0 ) )
			...
			if ( $vb_is_user_media ) {
				...
				if ( file_exists( "{$ps_value}_metadata" ) ) {
					if ( is_array(
						$va_tmp_metadata = json_decode( file_get_contents( "{$ps_value}_metadata" ), true ) )
					) {
			...
			if ( $vb_is_file_path ) {
				return array(
					...
					'value_decimal2'  => null,
					'_media'          => true
					// this tells the ca_attribute_values (which is the caller) to treat value_blob as a path to a file to be ingested
				);
			} else {
				return array(
	...
	public function htmlFormElement( $pa_element_info, $pa_options = null ) {
		...
		$vs_element .= '<div id="{fieldNamePrefix}upload_control_{n}" class="attributeMediaDownloadControl">'
		               . _t( "Upload" ) . ': <input type="file" name="{fieldNamePrefix}'
		               . $pa_element_info['element_id'] . '_{n}"></div>';
		$vs_element .= '</div>';
