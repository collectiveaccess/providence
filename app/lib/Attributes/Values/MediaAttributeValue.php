...
$_ca_attribute_settings['MediaAttributeValue'] = array(        // global
    'doesNotTakeLocale' => array(
        ...
        'description' => _t(
            'Check this option if you don\'t want your georeferences to be locale-specific. (The default is to not be.)'
        )
    ),
...
class MediaAttributeValue extends AttributeValue implements IAttributeValue
{
    ...
    public function getDisplayValue($pa_options = null)
    {
        ...
        if (isset($pa_options['return'])) {
            switch ($pa_options['return']) {
                ...
                case 'mimetype':
                    ...
                    break;
                case 'tag':
                    return $this->opo_media_info_coder->getMediaTag(
                        $vs_version,
                ...
                case 'filename':
                    ...
                    return caGetOption(
                        ...
                        $media_info,
                        pathinfo(
                            $this->opo_media_info_coder->getMediaPath(
                                $vs_version ? $vs_version : 'original'
                            ),
                            PATHINFO_BASENAME
                        )
                    );
        ...
        if ($vs_url = $this->opo_media_info_coder->getMediaUrl('original')) {
            ...
            if ($pa_options['showMediaInfo']) {
                ...
                if (isset($va_info['original']['PROPERTIES']['resolution']) && is_array(
                        $va_resolution = $va_info['original']['PROPERTIES']['resolution']
                    )) {
            ...
            if (isset($pa_options['poster_frame_version']) && $pa_options['poster_frame_version']) {
                $pa_options['poster_frame_url'] = $this->opo_media_info_coder->getMediaUrl(
                    $pa_options['poster_frame_version']
                );
            }
            ...
            if (is_object($pa_options['request'])) {
                $vs_view_url = urldecode(
                    caNavUrl(
                        $pa_options['request'],
                        $pa_options['request']->getModulePath(),
                        $pa_options['request']->getController(),
                        'GetMediaOverlay',
                ...
                $vs_val .= "<div style='float: left;'>" . urlDecode(
                        caNavLink(
                            ...
                            array('class' => 'attributeDownloadButton')
                        )
                    ) . "</div>";
            ...
            if ($pa_options['showMediaInfo']) {
                TooltipManager::add(
                    '#caMediaAttribute' . $this->opn_value_id,
    ...
    public function parseValue($ps_value, $pa_element_info, $pa_options = null)
    {
        ...
        if ($media_prefix = caGetOption('mediaPrefix', $pa_options, null)) {
            ...
            if ($batch_media_directory = $config->get('batch_media_import_root_directory')) {
                $va_files = caBatchFindMatchingMedia(
                    $batch_media_directory . $media_prefix,
                    $ps_value,
                    [
        ...
        if (
            (is_array($ps_value) && $ps_value['_uploaded_file'] && file_exists($ps_value['tmp_name']) && (filesize(
                        $ps_value['tmp_name']
                    ) > 0))
            ||
    ...
    public function htmlFormElement($pa_element_info, $pa_options = null)
    {
        ...
        $vs_element .= '<div id="{fieldNamePrefix}upload_control_{n}" class="attributeMediaDownloadControl">' . _t(
                "Upload"
            ) . ': <input type="file" name="{fieldNamePrefix}' . $pa_element_info['element_id'] . '_{n}"></div>';
        $vs_element .= '</div>';
        return $vs_element;
    }
    # ------------------------------------------------------------------
