...
abstract class AttributeValue extends BaseObject {
	...
	/**
	 * Parse attribute value and return parsed component values as an array of keys corresponding to
	 * ca_attribute_values field. If parse is successful an array is returned. If parse is unsuccessful and an error
	 * message should be displayed, boolean false is returned and the error(s) posted via BaseObject::postError()
	 * [inherited by this class]. If the parse is silently unsuccessful - that is nothing can be parsed but no error
	 * message should be displayed to the user - then null is returned and no errors posted.
	 *
	 ...
	 * @param mixed An array of parsed component values on success, false on failure and null on "silent" failure (eg.
	 *                 failed but don't show an error message to the user)
	 */
	 ...
	 *
	 * @todo There is a bug in PHP 5.3.x that prevents us from using this definition. The resulting error is: Can't
	 *       inherit abstract function IAttributeValue::getType() (previously declared abstract in AttributeValue).
	 *       There is an abstract function getType() here and the same thing is defined in the Interface. The
	 *       AttributeValue implementations inherit from this class and implement the Interface, which makes PHP
	 *       versions prior to 5.3.9 barf (even though it shouldn't). It's fixed in 5.3.9+. The widely used RHEL/CentOS
	 *       6.5 (latest version as of July 2014) still uses PHP 5.3.3 though.
	 *
	 ...
	 * Allows implementations to add extra data to the search index, not just whatever happens to be in their display
