...
class FloorPlanAttributeValue extends AttributeValue implements IAttributeValue {
	 ...
	 *                          forDuplication = returns full text + FloorPlan URL suitable for setting a duplicate
		...
		if ( ! $ps_value ) {
			if ( ! $va_settings["canBeEmpty"] ) {
				$this->postError( 1970, _t( 'Entry for <em>%1</em> was blank.', $pa_element_info['displayLabel'] ),
					'FloorPlanAttributeValue->parseValue()' );

				return false;
			}

			return array();
		} elseif ( is_array( $va_data = json_decode( preg_replace( '/[\x00-\x1F\x80-\xFF]/', '',
			preg_replace( "/[\\\\]{2}/", "\\", $ps_value ) ) ) )
		) {
			return array(
				'value_blob' => $ps_value
			);
	...
	public function htmlFormElement( $pa_element_info, $pa_options = null ) {
		...
		if ( ! (
		( ( ( $t_instance = $t_subject->getLeftTableInstance() ) && $t_instance->hasField( 'floorplan' )
		    && ( $t_target
					= $t_subject->getRightTableInstance() ) )
		  || ( ( $t_instance = $t_subject->getRightTableInstance() ) && $t_instance->hasField( 'floorplan' )
		       && ( $t_target = $t_subject->getLeftTableInstance() ) ) )
		)
		) {
			return _t( 'Floor plan is not supported' );
		}
		...
		$vs_element = "<div style='width: 850px;'><a href='#' class=\"{fieldNamePrefix}"
		              . $pa_element_info['element_id'] . "_{n}_trigger\">" . $t_instance->getMediaTag( 'floorplan',
				'preview' ) . "</a>";
		...
		if ( ! is_array(
			$va_floor_plan_annotations = json_decode( $t_subject->get( $pa_element_info['element_code'] ) ) )
		) {
			$va_floor_plan_annotations = [];
		}
		$vn_num_annotations = sizeof( $va_floor_plan_annotations );
		$vs_element         .= "<div style=\"float:right; width: 50%;\" id=\"{fieldNamePrefix}"
		                       . $pa_element_info['element_id'] . "_{n}_info\">
			<div>
				<h2>" . _t( "Floor plan for <em>%1</em>", $vs_target_name ) . "</h2>
			</div>
			<div id=\"{fieldNamePrefix}" . $pa_element_info['element_id'] . "_{n}_stats\">
			" . ( ( $vn_num_annotations == 1 ) ? _t( '%1 annotation on this floor plan', $vn_num_annotations )
				: _t( '%1 annotations on this floor plan', $vn_num_annotations ) ) . "
			</div>
			<div style='margin-top: 10px'>
				<a href='#' class=\"{fieldNamePrefix}" . $pa_element_info['element_id']
		                       . "_{n}_trigger form-button\"><span class=\"form-button\">"
		                       . caNavIcon( __CA_NAV_ICON_EDIT__, 2, [ 'style' => 'margin-right: 5px;' ] ) . " "
		                       . _t( 'Edit floor plan' ) . "</span></a>
			</div>
		</div></div>\n";
