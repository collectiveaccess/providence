...
$_ca_attribute_settings['FloorPlanAttributeValue'] = array(        // global
    ...
    'doesNotTakeLocale' => array(
        ...
        'description' => _t(
            'Check this option if you don\'t want your FloorPlan values to be locale-specific. (The default is to not be.)'
        )
    ),
...
class FloorPlanAttributeValue extends AttributeValue implements IAttributeValue
{
    ...
    public function parseValue($ps_value, $pa_element_info, $pa_options = null)
    {
        ...
        if (!$ps_value) {
            ...
            $va_data = json_decode(
                preg_replace('/[\x00-\x1F\x80-\xFF]/', '', preg_replace("/[\\\\]{2}/", "\\", $ps_value))
            )
        )) {
            return array(
                'value_blob' => $ps_value
    ...
    public function htmlFormElement($pa_element_info, $pa_options = null)
    {
        ...
        if (!(
        ((($t_instance = $t_subject->getLeftTableInstance()) && $t_instance->hasField(
                    'floorplan'
                ) && ($t_target = $t_subject->getRightTableInstance()))
            ||
            (($t_instance = $t_subject->getRightTableInstance()) && $t_instance->hasField(
                    'floorplan'
        ...
        $vs_element = "<div style='width: 850px;'><a href='#' class=\"{fieldNamePrefix}" . $pa_element_info['element_id'] . "_{n}_trigger\">" . $t_instance->getMediaTag(
                'floorplan',
                'preview'
            ) . "</a>";
        ...
        $vs_element .= "<div style=\"float:right; width: 50%;\" id=\"{fieldNamePrefix}" . $pa_element_info['element_id'] . "_{n}_info\">
			...
			" . (($vn_num_annotations == 1) ? _t('%1 annotation on this floor plan', $vn_num_annotations) : _t(
                '%1 annotations on this floor plan',
                $vn_num_annotations
            )) . "
			</div>
			<div style='margin-top: 10px'>
				<a href='#' class=\"{fieldNamePrefix}" . $pa_element_info['element_id'] . "_{n}_trigger form-button\"><span class=\"form-button\">" . caNavIcon(
