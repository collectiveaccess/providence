...
$_ca_attribute_settings['NumericAttributeValue'] = array(        // global
    ...
    'minValue' => array(
        ...
        'description' => _t(
            'The minimum numeric value to allow. Values smaller than required will be rejected. Leave blank to indicate no lower limit.'
        )
    ),
    'maxValue' => array(
        'formatType' => FT_NUMBER,
        ...
        'description' => _t(
            'The maximum numeric value to allow. Values larger than required will be rejected. Leave blank to indicate no upper limit.'
        )
    ),
    'regex' => array(
        'formatType' => FT_TEXT,
    ...
    'doesNotTakeLocale' => array(
        ...
        'description' => _t(
            'Check this option if you don\'t want your numeric values to be locale-specific. (The default is to be.)'
        )
    ),
...
class NumericAttributeValue extends AttributeValue implements IAttributeValue
{
    ...
    public function parseValue($ps_value, $pa_element_info, $pa_options = null)
    {
        ...
        if ($va_settings['minChars'] && ($vn_strlen < $va_settings['minChars'])) {
            // length is too short
            $vs_err_str = ($va_settings['minChars'] == 1) ? _t(
        ...
        if (strlen($ps_value) == 0) {
            if ((bool)$va_settings['mustNotBeBlank']) {
                $this->postError(
                    ...
                    'NumericAttributeValue->parseValue()'
                );
                return false;
            } else {
                return array(
        ...
        if (!(is_numeric($ps_value))) {
            ...
            $this->postError(
                1970,
                _t('%1 is not a numeric value', $pa_element_info['displayLabel']),
                'NumericAttributeValue->parseValue()'
            );
            return false;
        }
