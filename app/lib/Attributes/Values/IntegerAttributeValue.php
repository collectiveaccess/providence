...
$_ca_attribute_settings['IntegerAttributeValue'] = array(        // global
    ...
    'doesNotTakeLocale' => array(
        ...
        'description' => _t(
            'Check this option if you don\'t want your integer values to be locale-specific. (The default is to be.)'
        )
    ),
...
class IntegerAttributeValue extends AttributeValue implements IAttributeValue
{
    ...
    public function parseValue($ps_value, $pa_element_info, $pa_options = null)
    {
        ...
        if (strlen($ps_value) == 0) {
            if ((bool)$va_settings['mustNotBeBlank']) {
                $this->postError(
                    ...
                    'IntegerAttributeValue->parseValue()'
                );
                return false;
            } else {
                return array(
        ...
        if (!((preg_match('/[^\d\-]*$/', $ps_value)))) {
            ...
            $this->postError(
                1970,
                _t('%1 is not an integer value', $pa_element_info['displayLabel']),
                'IntegerAttributeValue->parseValue()'
            );
            return false;
        }
        ...
        if (isset($va_settings['mustBeUnique']) && (bool)$va_settings['mustBeUnique']) {
            ...
            $qr_values = $o_db->query(
                'SELECT value_id FROM ca_attribute_values WHERE element_id=? AND value_integer1=?',
