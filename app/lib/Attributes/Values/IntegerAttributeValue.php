...
class IntegerAttributeValue extends AttributeValue implements IAttributeValue {
	...
	public function parseValue( $ps_value, $pa_element_info, $pa_options = null ) {
		...
		if ( strlen( $va_settings['maxValue'] ) && ( $pn_value > $va_settings['maxValue'] ) ) {
			// value is too large
			$vs_err_str = _t( '%1 must be no more than %2', $pa_element_info['displayLabel'],
				$va_settings['maxValue'] );
			$this->postError( 1970, $vs_err_str, 'NumericAttributeValue->parseValue()' );
		...
		if ( ! ( ( preg_match( '/[^\d\-]*$/', $ps_value ) ) ) ) {
			...
			$this->postError( 1970, _t( '%1 is not an integer value', $pa_element_info['displayLabel'] ),
				'IntegerAttributeValue->parseValue()' );
		...
		if ( isset( $va_settings['mustBeUnique'] ) && (bool) $va_settings['mustBeUnique'] ) {
			...
			$qr_values
				= $o_db->query( 'SELECT value_id FROM ca_attribute_values WHERE element_id=? AND value_integer1=?',
				$pa_element_info['element_id'], $pn_value );

			if ( $qr_values->numRows() > 0 ) {
				$this->postError( 1970,
					_t( '%1 must be unique across all values. The value you entered already exists.',
						$pa_element_info['displayLabel'] ), 'IntegerAttributeValue->parseValue()' );
	...
	public function htmlFormElement( $pa_element_info, $pa_options = null ) {
		$va_settings = $this->getSettingValuesFromElementArray( $pa_element_info,
			array( 'fieldWidth', 'fieldHeight', 'minChars', 'maxChars' ) );
