...
class ApplicationPluginManager {
	...
	public function __call( $ps_hook_name, $pa_params ) {
		...
		foreach ( ApplicationPluginManager::$s_application_plugin_hooks as $vs_plugin_name => $va_hooks ) {
			if ( isset( ApplicationPluginManager::$s_application_plugin_hooks[ $vs_plugin_name ][ $ps_hook_name ] )
			     && ApplicationPluginManager::$s_application_plugin_hooks[ $vs_plugin_name ][ $ps_hook_name ]
			) {
				$va_params
					= ApplicationPluginManager::$s_application_plugin_instances[ $vs_plugin_name ]->$ps_hook_name( $va_params );
	...
	public static function initPlugins() {
		...
		foreach ( $va_plugin_locations as $vs_app_plugin_dir ) {
			foreach ( $va_app_plugin_dirs as $vs_plugin_dir ) {
				...
				if ( ! file_exists( $vs_app_plugin_dir . '/' . $vs_plugin_dir . '/' . $vs_plugin_dir
				                    . 'Plugin.php' )
				) {
					continue;
				}
				...
				if ( is_array( $va_method_list = $o_class_info->getMethods() ) ) {
					foreach ( $va_method_list as $o_method ) {
						...
						ApplicationPluginManager::$s_application_plugin_hooks[ $vs_plugin_dir ][ $vs_method_name ]
							= true;
					}
				}
		...
		foreach ( $pa_plugin_locations as $vs_app_plugin_dir ) {
			if ( is_resource( $r_dir = opendir( $vs_app_plugin_dir ) ) ) {
				while ( ( $vs_plugin_dir = readdir( $r_dir ) ) !== false ) {
					if ( is_dir( $vs_app_plugin_dir . '/' . $vs_plugin_dir )
					     && preg_match( "/^[A-Za-z_]+[A-Za-z0-9_]*$/", $vs_plugin_dir )
					) {
	...
	public static function checkPluginStatus( $ps_plugin_name ) {
		...
		if ( isset( ApplicationPluginManager::$s_application_plugin_instances[ $ps_plugin_name ] )
		     && is_object( ApplicationPluginManager::$s_application_plugin_instances[ $ps_plugin_name ] )
		) {
	...
	static public function getPluginRoleActions() {
		...
		foreach ( $va_app_plugin_dirs as $vs_plugin_dir ) {
			...
			$va_actions = array_merge( $va_actions,
				call_user_func( array( $vs_plugin_classname, 'getRoleActionList' ) ) );
		}
