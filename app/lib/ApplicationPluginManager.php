...
class ApplicationPluginManager
{
     ...
     */
    static public $s_application_plugin_manager_did_do_plugin_init = false;

    /**
     *
     */
    static public $s_application_plugin_instances = array();

    /**
     *
     */
    static public $s_application_plugin_hooks = array();
    ...
    public function __call($ps_hook_name, $pa_params)
    {
        ...
        foreach (ApplicationPluginManager::$s_application_plugin_hooks as $vs_plugin_name => $va_hooks) {
            if (isset(ApplicationPluginManager::$s_application_plugin_hooks[$vs_plugin_name][$ps_hook_name]) && ApplicationPluginManager::$s_application_plugin_hooks[$vs_plugin_name][$ps_hook_name]) {
                $va_params = ApplicationPluginManager::$s_application_plugin_instances[$vs_plugin_name]->$ps_hook_name(
                    $va_params
                );
        ...
        foreach ($pa_plugin_locations as $vs_app_plugin_dir) {
            if (is_resource($r_dir = opendir($vs_app_plugin_dir))) {
                while (($vs_plugin_dir = readdir($r_dir)) !== false) {
                    if (is_dir($vs_app_plugin_dir . '/' . $vs_plugin_dir) && preg_match(
                            "/^[A-Za-z_]+[A-Za-z0-9_]*$/",
                            $vs_plugin_dir
                        )) {
    ...
    public static function checkPluginStatus($ps_plugin_name)
    {
        ...
        if (isset(ApplicationPluginManager::$s_application_plugin_instances[$ps_plugin_name]) && is_object(
                ApplicationPluginManager::$s_application_plugin_instances[$ps_plugin_name]
            )) {
