...
class DashboardManager {
	 ...
	 * @param string $ps_widget_id   Unique identifer for placement of widget in dashboard used internally (is 32
	 *                               character MD5 hash in case you care)
	 * @param array  $pa_settings    Array of settings for widget instance, as defined by widget's entry in
	 *                               BaseWidget::$s_widget_settings
	 *
	...
	public function getWidgetsForColumn( $pn_column ) {
		if ( ! isset( $this->opa_dashboard_config['columns'][ $pn_column ] )
		     || ! is_array( $this->opa_dashboard_config['columns'][ $pn_column ] )
		) {
			return array();
		}
	...
	public function getWidgetByLocation( $pn_column, $pn_pos ) {
		...
		if ( ! isset( $this->opa_dashboard_config['columns'][ $pn_column ][ $pn_pos ] )
		     || ! is_array( $this->opa_dashboard_config['columns'][ $pn_column ][ $pn_pos ] )
		) {
			return null;
		}
	...
	public function addWidget( $ps_widget_name, $pn_column, $pa_settings = null ) {
		$col_count                                             = is_array( $this->opa_dashboard_config['columns'][ $pn_column ] )
			? sizeof( $this->opa_dashboard_config['columns'][ $pn_column ] ) : 1;
		...
		foreach ( $pa_order_info as $vn_cur_column => $va_widgets_by_column ) {
			foreach ( $va_widgets_by_column as $vn_i => $vs_widget ) {
				...
				if ( $this->opa_dashboard_config['columns'][ $vn_column ][ $vn_pos ] ) {
					$va_reordered_columns[ $vn_cur_column ][ (int) $va_counters[ $vn_cur_column ] ]
						= $this->opa_dashboard_config['columns'][ $vn_column ][ $vn_pos ];
				}
	...
	public function getWidgetSettingsFormHTML( $ps_widget_id ) {
		if ( $va_widget_info = $this->getWidgetByID( $ps_widget_id ) ) {
			return $this->opo_widget_manager->getWidgetSettingsForm( $va_widget_info['widget'], $ps_widget_id,
	...
	public function widgetHasSettings( $ps_widget_name ) {
		if ( is_array( BaseWidget::$s_widget_settings[ $ps_widget_name . 'Widget' ] )
		     && ( sizeof( BaseWidget::$s_widget_settings[ $ps_widget_name . 'Widget' ] ) > 0 )
		) {
	...
	public function saveWidgetSettings( $ps_widget_id ) {
		if ( $va_widget_info = $this->getWidgetByID( $ps_widget_id ) ) {
			...
			$va_setting_values
				= $this->opa_dashboard_config['columns'][ $va_widget_info['col'] ][ $va_widget_info['pos'] ]['settings'];

			$o_appvar = null;
			foreach ( $va_available_settings as $vs_setting_name => $va_setting_info ) {
				$va_setting_values[ $vs_setting_name ] = $this->opo_request->getParameter( 'setting_'
				                                                                           . $vs_setting_name,
					pString );
				...
				if ( isset( $va_setting_info['scope'] ) && ( $va_setting_info['scope'] == 'application' ) ) {
					...
					$o_appvar->setVar( 'widget_settings_' . $va_widget_info['widget'] . '_' . $vs_setting_name,
						$va_setting_values[ $vs_setting_name ] );    // put setting value into application var
				}
			}
