...
class DashboardManager
{
    ...
    static public $opo_instance = null;
    ...
    {
        if (!isset($this->opa_dashboard_config['columns'][$pn_column]) || !is_array(
                $this->opa_dashboard_config['columns'][$pn_column]
            )) {
            return array();
        }
    ...
    public function getWidgetByLocation($pn_column, $pn_pos)
    {
        ...
        if (!isset($this->opa_dashboard_config['columns'][$pn_column][$pn_pos]) || !is_array(
                $this->opa_dashboard_config['columns'][$pn_column][$pn_pos]
            )) {
            return null;
        }
    ...
    {
        $col_count = is_array($this->opa_dashboard_config['columns'][$pn_column]) ? sizeof(
            $this->opa_dashboard_config['columns'][$pn_column]
        ) : 1;
    ...
    {
        if ($va_widget_info = $this->getWidgetByID($ps_widget_id)) {
            return $this->opo_widget_manager->getWidgetSettingsForm(
     ...
     * @return bool True if widget has at least one settings, false if it doesn't
     */
    public function widgetHasSettings($ps_widget_name)
    {
        if (is_array(BaseWidget::$s_widget_settings[$ps_widget_name . 'Widget']) && (sizeof(
                    BaseWidget::$s_widget_settings[$ps_widget_name . 'Widget']
                ) > 0)) {
    ...
    public function saveWidgetSettings($ps_widget_id)
    {
        if ($va_widget_info = $this->getWidgetByID($ps_widget_id)) {
            ...
            foreach ($va_available_settings as $vs_setting_name => $va_setting_info) {
                $va_setting_values[$vs_setting_name] = $this->opo_request->getParameter(
                    'setting_' . $vs_setting_name,
                    pString
                );
                ...
                if (isset($va_setting_info['scope']) && ($va_setting_info['scope'] == 'application')) {
                    ...
                    $o_appvar->setVar(
                        'widget_settings_' . $va_widget_info['widget'] . '_' . $vs_setting_name,
                        $va_setting_values[$vs_setting_name]
