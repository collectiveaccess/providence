...
class InformationServiceManager {
	...
	public static function getInformationServiceInstance( $ps_service ) {
		InformationServiceManager::initInformationServices();
		if ( isset( InformationServiceManager::$s_information_service_instances[ $ps_service ] )
		     && InformationServiceManager::$s_information_service_instances[ $ps_service ]
		) {
	...
	public static function getInformationServiceNames() {
		...
		if ( is_resource( $r_dir = opendir( $vs_base_service_dir ) ) ) {
			while ( ( $vs_service = readdir( $r_dir ) ) !== false ) {
				...
				if ( file_exists( $vs_base_service_dir . '/' . $vs_service )
				     && preg_match( "/^([A-Za-z_]+[A-Za-z0-9_]*)\.php$/", $vs_service, $va_matches )
				) {
	...
	public static function checkInformationServiceStatus( $ps_service_name ) {
		...
		if ( isset( InformationServiceManager::$s_information_service_instances[ $ps_service_name ] )
		     && is_object( InformationServiceManager::$s_information_service_instances[ $ps_service_name ] )
		) {
