...
class BaseFindController extends ActionController
{
    ...
    public function Index($pa_options = null)
    {
        ...
        if ($vn_type_id = $this->opo_result_context->getTypeRestriction(
            $vb_type
        ...
        foreach (caExtractValuesByUserLocale($t_display->getBundleDisplays($va_display_options)) as $va_display) {
            ...
            foreach (
                is_array(
                    $va_display['settings']['show_only_in']
                ) ? $va_display['settings']['show_only_in'] : [] as $k => $v
            ) {
                $v = str_replace('search_browse_', '', $v);
                if (!in_array($v, ['list', 'full', 'thumbnail'])) {
                    continue;
                }
                $va_show_only_in[] = $v;
            }
        ...
        if (!isset($va_displays[$vn_display_id])) {
            $vn_display_id = array_shift(array_keys($va_displays));
        }
        ...
        if (method_exists($t_instance, 'getApplicableElementCodes')) {
            ...
            foreach ($va_display_list as $vn_i => $va_display_item) {
                ...
                if ($va_tmp[0] != $this->ops_tablename) {
                    ...
                    if (($t_rel = Datamodel::getInstance($va_tmp[0], true)) && method_exists(
                            $t_rel,
                        ...
                        $va_display_list[$vn_i]['bundle_sort'] = "{$va_tmp[0]}.preferred_labels." . $t_rel->getLabelSortField(
        ...
        $this->view->setVar(
            ...
            caGetAvailablePrintTemplates(
                'labels',
                array('table' => $this->ops_tablename, 'type' => 'label')
            )
        );
    ...
    protected function _setBottomLineValues($po_result, $pa_display_list, $pt_display)
    {
        ...
        foreach ($pa_display_list as $vn_placement_id => $va_placement) {
            if (isset($va_placement['settings']['bottom_line']) && $va_placement['settings']['bottom_line']) {
                $va_bottom_line[$vn_placement_id] = caProcessBottomLineTemplateForPlacement(
                    $this->request,
                    $va_placement,
                    $po_result,
                    array(
                        'pageStart' => ($vn_page_num - 1) * $vn_items_per_page,
                        'pageEnd' => (($vn_page_num - 1) * $vn_items_per_page) + $vn_items_per_page
                    )
                );
            ...
            $o_pdf->render(
                ...
                array(
                    ...
                    'filename' => ($vs_filename = $this->view->getVar(
                        'filename'
                    )) ? $vs_filename : caGetOption('filename', $va_template_info, 'labels.pdf')
                )
            );
            ...
            try {
                ...
                $o_pdf->render(
                    ...
                    array(
                        ...
                        'filename' => ($vs_filename = $this->view->getVar(
                            'filename'
                        )) ? $vs_filename : caGetOption('filename', $va_template_info, 'export_results.pdf')
                    )
                );
                exit;
            } catch (Exception $e) {
    ...
    public function DownloadMedia()
    {
        if ($t_subject = Datamodel::getInstanceByTableName($this->ops_tablename, true)) {
            ...
            $pa_ids = null;
            if (($vs_ids = trim($this->request->getParameter($t_subject->tableName(), pString))) || ($vs_ids = trim(
            ...
            if (is_array($pa_ids) && sizeof($pa_ids)) {
                ...
                if ($qr_res = $t_subject->makeSearchResult(
                    ...
                    while ($qr_res->nextHit()) {
                        if (!is_array(
                                $va_version_list = $qr_res->getMediaVersions('ca_object_representations.media')
                            ) || !in_array($ps_version, $va_version_list)) {
                            $vs_version = 'original';
                        } else {
                        ...
                        foreach ($va_paths as $vn_i => $vs_path) {
                            ...
                            $vs_idno_proc = preg_replace(
                                '![^A-Za-z0-9_\-]+!',
                                '_',
                                $qr_res->get($t_subject->tableName() . '.idno')
                            );
    ...
    public function resultsEditor()
    {
        ...
        $this->view->setVar(
            'columns',
            ca_bundle_displays::getColumnsForResultsEditor(
                $va_display_list,
    ...
    {
        if (($pn_s = (int)$this->request->getParameter('s', pInteger)) < 0) {
            $pn_s = 0;
        }
        if (($pn_c = (int)$this->request->getParameter('c', pInteger)) < 1) {
            $pn_c = 10;
        }
        ...
        while ($qr_res->nextHit()) {
            ...
            foreach ($va_display_list as $va_display_item) {
                $va_display_value = $t_display->getDisplayValue(
                    $qr_res,
                    $va_display_item['placement_id'],
                    ['returnInfo' => true]
                );
                ...
                $va_row[$vs_bundle = str_replace(
                    ".",
                    ",",
                    $va_display_item['bundle_name']
                )] = $va_display_value['value'];
    ...
    public function saveResultsEditorData()
    {
        ...
        $va_response = $t_display->saveResultsEditorData(
            $this->ops_tablename,
            [
                'request' => $this->request,
                'user_id' => $this->request->getUserID(),
                'type_id' => $this->opo_result_context->getTypeRestriction(
                    $vb_dummy
                )
            ]
        );
    ...
    public function resultsComplexDataEditor()
    {
        ...
        $this->view->setVar(
            'bundles',
            $va_bundles = ca_bundle_displays::makeBundlesForResultsEditor(
                [$ps_bundle],
                [$t_display->get('settings')]
            )
        );
        $this->view->setVar('t_subject', $t_instance);
    ...
    private function _getDisplayList($pn_display_id)
    {
        ...
        $va_ret = $t_display->getDisplayListForResultsEditor(
            $this->ops_tablename,
            [
                'user_id' => $this->request->getUserID(),
                'request' => $this->request,
    ...
    public function getResultsDisplayName($ps_mode = 'singular')
    {
        ...
        if ($this->request->config->get($this->ops_tablename . '_breakout_find_by_type_in_menu')) {
            ...
            if (!($vs_name = ($ps_mode == 'singular') ? $va_hier[$vn_type_id]['name_singular'] : $va_hier[$vn_type_id]['name_plural'])) {
                $vs_name = mb_strtolower(
                    ($ps_mode == 'singular') ? $t_instance->getProperty('NAME_SINGULAR') : $t_instance->getProperty(
                        'NAME_PLURAL'
                    )
                );
            }
