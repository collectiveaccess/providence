...
class BaseFindController extends ActionController {
	 ...
	 * Is associative array: values are display names for fields, keys are full fields names (table.field) to be used
	 * as sort
	 */
	protected $opa_sorts;
	# ------------------------------------------------------------------
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		if ( $this->ops_tablename ) {
			...
			if ( $this->request->config->get( $this->ops_tablename . '_breakout_find_by_type_in_submenu' )
			     || $this->request->config->get( $this->ops_tablename . '_breakout_find_by_type_in_menu' )
			) {
				if ( $this->opn_type_restriction_id
					= $this->opo_result_context->getTypeRestriction( $pb_type_restriction_has_changed )
				) {
			...
			if ( $display_id = $this->opo_result_context->getCurrentBundleDisplay( $this->opn_type_restriction_id,
				$this->_getShowInStr() )
			) {
				$this->opa_sorts = caGetAvailableSortFields( $this->ops_tablename, $this->opn_type_restriction_id,
					array(
						'request'           => $po_request,
						'restrictToDisplay' => $this->request->config->get( 'restrict_find_result_sort_options_to_current_display' )
							? $display_id : null
					) );
			} else {
				$this->opa_sorts = caGetAvailableSortFields( $this->ops_tablename, $this->opn_type_restriction_id,
					array( 'request' => $po_request ) );
			}
		} else {
			$this->opa_sorts = [];
		}
	...
	public function Index( $pa_options = null ) {
		...
		if (
			...
			&& (
				...
				&& ( ! is_null( $va_types = caGetTypeListForUser( $this->ops_tablename,
		...
		$display_id = $this->opo_result_context->getCurrentBundleDisplay( $this->opn_type_restriction_id,
			$this->_getShowInStr() );
		...
		if ( ( $vn_type_id = $this->opo_result_context->getTypeRestriction( $vb_type ) )
		     && ( $t_instance::typeCodeForID( $vn_type_id ) )
		) { // occurrence searches are inherently type-restricted
		...
		if ( $t_display = $this->view->getVar( 't_display' ) ) {
			...
			if ( is_array( $va_show_in ) && sizeof( $va_show_in )
			     && ! in_array( 'search_browse_' . $this->opo_result_context->getCurrentView(),
					$t_display->getSetting( 'show_only_in' ) )
			) {
				$display_id   = 0;
				$display_list = $this->_getDisplayList( $display_id );
			}
		...
		if ( method_exists( $t_instance, 'getApplicableElementCodes' ) ) {
			...
			$va_attribute_list
			                        = array_flip( $t_instance->getApplicableElementCodes( $this->opo_result_context->getTypeRestriction( $vb_dummy ),
				false, false ) );
			...
			foreach ( $display_list as $i => $va_display_item ) {
				...
				if ( isset( $va_display_item['settings']['sort'] )
				     && ( strlen( $va_display_item['settings']['sort'] ) > 0 )
				) {
					$display_list[ $i ]['is_sortable'] = true;
				...
				if ( $tmp[0] != $this->ops_tablename ) {
					...
					if ( ( $t_rel = Datamodel::getInstance( $tmp[0], true ) )
					     && method_exists( $t_rel, "getLabelTableInstance" )
					     && ( $t_rel_label = $t_rel->getLabelTableInstance() )
					) {
						$display_list[ $i ]['is_sortable'] = true;
						$types                             = array_merge( caGetOption( 'restrict_to_relationship_types',
							$va_display_item['settings'], [], [ 'castTo' => 'array' ] ),
							caGetOption( 'restrict_to_types', $va_display_item['settings'], [],
								[ 'castTo' => 'array' ] ) );

						$display_list[ $i ]['bundle_sort'] = "{$tmp[0]}.preferred_labels." . $t_rel->getLabelSortField()
						                                     . ( ( is_array( $types ) && sizeof( $types ) ) ? "|"
						                                                                                      . join( ",",
									$types ) : "" );
					}
					continue;
				...
				if ( $t_instance->hasField( $tmp[1] ) ) {
					if ( $t_instance->getFieldInfo( $tmp[1], 'FIELD_TYPE' )
					     == FT_MEDIA
						...
						foreach ( $elements as $e ) {
							switch ( $e['datatype'] ) {
								...
								case __CA_ATTRIBUTE_VALUE_LENGTH__:
									$display_list[ $i ]['bundle_sort']
										= "{$va_display_item['bundle_name']}.{$e['element_code']}";
									break( 2 );
							}
						}
					}
		...
		$this->view->setVar( 'label_formats', caGetAvailablePrintTemplates( 'labels', array(
			'table'           => $this->ops_tablename,
			'type'            => 'label',
			'restrictToTypes' => $this->opn_type_restriction_id
		) ) );
		...
		$include_export_options = $this->request->config->getList( $this->ops_tablename
		                                                           . '_standard_results_export_formats' );
		foreach (
			[
				'tab'  => _t( 'Tab delimited' ),
				'csv'  => _t( 'Comma delimited (CSV)' ),
				'xlsx' => _t( 'Spreadsheet (XLSX)' ),
				'docx' => _t( 'Word processing (DOCX)' )
			] as $ext => $name
		) {
		...
		$va_export_options = array_merge( $va_export_options, caGetAvailablePrintTemplates( 'results', array(
			'showOnlyIn'      => [ 'search_browse_' . $this->opo_result_context->getCurrentView() ],
			'table'           => $this->ops_tablename,
			'restrictToTypes' => $this->opn_type_restriction_id
		) ) );
		...
		if ( $this->request->user->canDoAction( 'can_batch_export_metadata' )
		     && $this->request->user->canDoAction( 'can_export_' . $this->ops_tablename )
		) {
		...
		//
		$t_set = new ca_sets();
		$this->view->setVar( 'available_sets', caExtractValuesByUserLocale( $t_set->getSets( array(
			'table'      => $this->ops_tablename,
			'user_id'    => ! (bool) $this->request->config->get( 'ca_sets_all_users_see_all_sets' )
				? $this->request->getUserID() : null,
			'access'     => __CA_SET_READ_ACCESS__,
			'omitCounts' => true
		) ) ) );
		$this->view->setVar( 'available_editable_sets', caExtractValuesByUserLocale( $t_set->getSets( array(
			'table'      => $this->ops_tablename,
			'user_id'    => ! (bool) $this->request->config->get( 'ca_sets_all_users_see_all_sets' )
				? $this->request->getUserID() : null,
		...
		#
		#
		$this->view->setVar( 'children_display_mode_default',
			( $vs_children_display_mode_default = $this->request->config->get( $this->ops_tablename
			                                                                   . "_children_display_mode_in_results" ) )
		...
		$this->view->setVar( 'hide_children',
			$pb_hide_children = in_array( strtolower( $ps_children_display_mode ), [ 'hide', 'alwayshide' ] ) );
		$this->view->setVar( 'show_children_display_mode_control',
			! in_array( strtolower( $vs_children_display_mode_default ), [ 'alwaysshow', 'alwayshide' ] ) );


		$this->view->setVar( 'ca_object_representation_download_versions',
			$this->request->config->getList( 'ca_object_representation_download_versions' ) );

		$media_elements = ca_metadata_elements::getElementsAsList( false, $this->ops_tablename,
			$this->opn_type_restriction_id, false, false, true, [ __CA_ATTRIBUTE_VALUE_MEDIA__ ] );
		$this->view->setVar( 'media_metadata_elements',
	...
	protected function _setBottomLineValues( $po_result, $pa_display_list, $pt_display ) {
		...
		foreach ( $pa_display_list as $placement_id => $va_placement ) {
			if ( isset( $va_placement['settings']['bottom_line'] ) && $va_placement['settings']['bottom_line'] ) {
				$va_bottom_line[ $placement_id ] = caProcessBottomLineTemplateForPlacement( $this->request,
					$va_placement, $po_result, array(
		...
		//
		$this->view->setVar( 'bottom_line_totals',
			caProcessBottomLineTemplateForDisplay( $this->request, $pt_display, $po_result, array(
				'pageStart' => ( $vn_page_num - 1 ) * $items_per_page,
				'pageEnd'   => ( ( $vn_page_num - 1 ) * $items_per_page ) + $items_per_page
			) ) );
	}
	...
	protected function _genLabels( $po_result, $ps_label_code, $ps_output_filename, $ps_title = null ) {
		$vs_border = ( (bool) $this->request->config->get( 'add_print_label_borders' ) )
		...
		try {
			...
			$vn_horizontal_gutter = caConvertMeasurement( caGetOption( 'horizontalGutter', $va_template_info, null ),
				'mm' );
			$vn_vertical_gutter   = caConvertMeasurement( caGetOption( 'verticalGutter', $va_template_info, null ),
				'mm' );
			...
			$va_defined_vars
				= array_keys( $this->view->getAllVars() );        // get list defined vars (we don't want to copy over them)
			$va_tag_list
				= $this->getTagListForView( $va_template_info['path'] );                // get list of tags in view

			$va_barcode_files_to_delete = array();

			$vn_page_count = 0;
			while ( $po_result->nextHit() ) {
				$va_barcode_files_to_delete = array_merge( $va_barcode_files_to_delete,
					caDoPrintViewTagSubstitution( $this->view, $po_result, $va_template_info['path'],
			...
			$o_pdf->render( $vs_content, array(
				'stream'   => true,
				'filename' => ( $filename = $this->view->getVar( 'filename' ) ) ? $filename
					: caGetOption( 'filename', $va_template_info, 'labels.pdf' )
			) );
	...
	protected function _genExport( $po_result, $ps_output_type, $ps_output_filename, $ps_title = null ) {
		$this->view->setVar( 'criteria_summary', $vs_criteria_summary
			= $this->getCriteriaForDisplay() );    // add displayable description of current search/browse parameters
		$this->view->setVar( 'criteria_summary_truncated',
			mb_substr( $vs_criteria_summary, 0, 60 ) . ( ( mb_strlen( $vs_criteria_summary ) > 60 ) ? '...' : '' ) );
		...
		if ( substr( $ps_output_type, 0, 4 ) !== '_pdf' ) {
			switch ( $ps_output_type ) {
				...
				case '_csv':
					$vs_delimiter        = ",";
					$vs_output_file_name = mb_substr( preg_replace( "/[^A-Za-z0-9\-]+/", '_',
						$ps_output_filename . '_csv' ), 0, 30 );
					...
					if ( substr( $ps_output_type, 0, 5 ) === '_docx' ) {
						...
						if ( ! is_array( $va_template_info ) ) {
							$this->postError( 3110, _t( "Could not find view for PDF" ),
								"BaseFindController->PrintSummary()" );
			...
			header( "Content-Disposition: attachment; filename=export_" . $vs_output_file_name . "."
			        . $vs_file_extension );
			...
			while ( $po_result->nextHit() ) {
				...
				foreach ( $display_list as $placement_id => $va_display_item ) {
					$vs_value = html_entity_decode( $t_display->getDisplayValue( $po_result, $placement_id,
						array( 'convert_codes_to_display_text' => true, 'convertLineBreaks' => false ) ), ENT_QUOTES,
						'UTF-8' );
			...
			try {
				...
				$o_pdf->render( $vs_content, array(
					'stream'   => true,
					'filename' => ( $filename = $this->view->getVar( 'filename' ) ) ? $filename
						: caGetOption( 'filename', $va_template_info, 'export_results.pdf' )
				) );
				exit;
			} catch ( Exception $e ) {
				die( $e->getMessage() );
			...
			if ( ! $ps_rows || ! sizeof( $pa_row_ids ) ) {
				...
				if ( $t_set->haveAccessToSet( $this->request->getUserID(), __CA_SET_EDIT_ACCESS__ ) ) {
					...
					if ( $t_set->getPrimaryKey() && ( $t_set->get( 'table_num' ) == $t_instance->tableNum() ) ) {
						...
						if ( ( $vn_added_items_count = $t_set->addItems( array_keys( $va_row_ids_to_add ),
								[ 'user_id' => $this->request->getUserID() ] ) ) === false
						) {
	...
	public function createSetFromResult() {
		...
		if ( $this->request->user->canDoAction( 'can_create_sets' ) ) {
			...
			if ( is_array( $va_row_ids ) && sizeof( $va_row_ids ) ) {
				...
				$t_set->set( 'set_code',
					$vs_set_code = mb_substr( preg_replace( "![^A-Za-z0-9_\-]+!", "_", $vs_set_name ), 0, 100 ) );
	...
	public function addSavedSearch() {
		...
		if ( $vs_md5 = $this->request->user->addSavedSearch( $this->ops_tablename, $this->ops_find_type,
			$va_values )
		) {
	...
	public function doSavedSearch() {
		if ( $va_saved_search = $this->request->user->getSavedSearchByKey( $this->ops_tablename, $this->ops_find_type,
			$this->request->getParameter( 'saved_search_key', pString ) )
		) {
	...
	public function DownloadMedia() {
		if ( $t_subject = Datamodel::getInstanceByTableName( $this->ops_tablename, true ) ) {
			...
			if ( ( $ids = trim( $this->request->getParameter( $t_subject->tableName(), pString ) ) )
			     || ( $ids
					= trim( $this->request->getParameter( $t_subject->primaryKey(), pString ) ) )
			) {
				if ( $ids !== 'all' ) {
					$id_list = explode( ';', $ids );
			...
			if ( ( $limit = (int) $t_subject->getAppConfig()->get( 'maximum_download_file_count' ) )
			     > 0
			) {    // truncate to maximum
			...
			if ( is_array( $id_list ) && sizeof( $id_list ) ) {
				...
				if ( preg_match( '!^attribute([\d]+)$!', $preferred_version, $m ) ) {
					...
					if ( ! ( $t_root = ca_metadata_elements::getInstance(
						$root_id = $t_element->get( 'ca_metadata_elements.hier_element_id' ) ) )
					) {
						throw new ApplicationException( _t( 'Invalid parent element_id' ) );
					}
					$element_code = $t_subject->tableName() . '.' . ( ( $root_id != $element_id )
							? $t_root->get( 'element_code' ) . '.' . $t_element->get( 'element_code' )
							: $t_element->get( 'element_code' ) );
				}
				if ( $qr_res = $t_subject->makeSearchResult( $t_subject->tableName(), $id_list,
					array( 'filterNonPrimaryRepresentations' => false ) )
				) {
					...
					while ( $qr_res->nextHit() ) {
						if ( ! $element_code ) {
							// representation
							$version = ( is_array( $version_list
									= $qr_res->getMediaVersions( 'ca_object_representations.media' ) )
							             || ! in_array( $preferred_version, $version_list ) ) ? $preferred_version
								: 'original';

							$paths                = $qr_res->getMediaPaths( 'ca_object_representations.media',
								$version );
							$infos                = $qr_res->getMediaInfos( 'ca_object_representations.media' );
							$representation_ids   = $qr_res->get( 'ca_object_representations.representation_id',
								array( 'returnAsArray' => true ) );
							$representation_types = $qr_res->get( 'ca_object_representations.type_id',
								array( 'returnAsArray' => true ) );
							...
							foreach ( $paths as $i => $path ) {
								...
								if ( ! caCanRead( $this->request->user->getPrimaryKey(), 'ca_object_representations',
									$representation_id )
								) {
									continue;
								}

								$filename = caGetRepresentationDownloadFileName( $this->ops_tablename, [
									'idno'              => $idno,
									'index'             => $index,
									'version'           => $version,
									'extension'         => $ext,
								...
								if ( $o_md_conf->get( 'do_metadata_embedding_for_search_result_media_download' ) ) {
									if ( $path_with_embedding = caEmbedMediaMetadataIntoFile( $path,
										'ca_objects', $qr_res->get( 'ca_objects.object_id' ),
							...
							// metadata element
							$paths             = $qr_res->get( "{$element_code}.{$version}.path",
								[ 'returnAsArray' => true ] );
							$idno              = $qr_res->get( $t_subject->tableName() . '.idno' );
							$original_filename = pathinfo( $qr_res->get( $element_code . '.original_filename' ),
								PATHINFO_BASENAME );
							...
							foreach ( $paths as $i => $path ) {
								...
								], [ 'mode' => $original_filename ? 'original_filename' : 'idno' ] );
			...
			if ( $file_count > 1 ) {
				...
				$o_view->setVar( 'archive_name',
					'media_for_' . mb_substr( preg_replace( '![^A-Za-z0-9]+!u', '_', $this->getCriteriaForDisplay() ),
						0, 20 ) . '.zip' );
		...
		// post error
		$this->postError( 3100, _t( "Could not generate ZIP file for download" ),
			"BaseFindController->DownloadMedia()" );
	}
	# ------------------------------------------------------------------
	...
	public function Tools( $pa_parameters ) {
		...
		$this->opa_sorts = array_replace( $this->opa_sorts,
			caGetAvailableSortFields( $this->ops_tablename, $this->opn_type_restriction_id, array(
		...
		$set_list = caExtractValuesByUserLocale( $t_set->getSets( array(
			'table'   => $this->ops_tablename,
			'user_id' => ! (bool) $this->request->config->get( 'ca_sets_all_users_see_all_sets' )
				? $this->request->getUserID() : null,
			'access'  => __CA_SET_READ_ACCESS__
		...
		if ( $this->request->user->getPreference( 'list_public_sets' ) === 'show' ) {
			$all_users_public_sets
				= caExtractValuesByUserLocale( $t_set->getSets( array(
				'table'       => $this->ops_tablename,
				'user_id'     => ! (bool) $this->request->config->get( 'ca_sets_all_users_see_all_sets' )
					? $this->request->getUserID() : null,
				'allUsers'    => true,
		...
		$this->view->setVar( 'result', ( is_array( $va_results_id_list ) && sizeof( $va_results_id_list ) > 0 )
			? caMakeSearchResult( $this->ops_tablename, $va_results_id_list ) : null );
	...
	public function Viz() {
		...
		$this->view->setVar( "viz_html",
			$o_viz->render( $ps_viz, "HTML", array( 'classname' => 'vizFullScreen', 'request' => $this->request ) ) );
		...
		if ( $pb_render_data ) {
			$this->response->addContent( $o_viz->getDataForVisualization( $ps_viz,
	...
	public function resultsEditor() {
		...
		$this->view->setVar( 'columns',
			ca_bundle_displays::getColumnsForResultsEditor( $display_list, [ 'request' => $this->request ] ) );
	...
	public function getResultsEditorData() {
		...
		while ( $qr_res->nextHit() ) {
			...
			foreach ( $display_list as $display_item ) {
				...
				$row[ $display_item['placement_id'] . "_edit_mode" ] = $display_value['inlineEditable'] ? "inline"
					: "overlay";
			}
			$data[] = $row;
			$count ++;
	...
	public function saveResultsEditorData() {
		$t_display
			      = new ca_bundle_displays( $this->opo_result_context->getCurrentBundleDisplay( $this->opn_type_restriction_id,
			$this->_getShowInStr() ) );
	...
	public function resultsComplexDataEditor() {
		...
		$this->view->setVar( 'bundles',
			ca_bundle_displays::makeBundlesForResultsEditor( [ $bundle ], [ $t_placement->get( 'settings' ) ] ) );
		$this->view->setVar( 't_subject', $t_instance );
	...
	private function _getDisplayList( $display_id ) {
		...
		$ret     = $t_display->getDisplayListForResultsEditor( $this->ops_tablename,
			[
				'user_id' => $this->request->getUserID(),
				'request' => $this->request,
				'type_id' => $this->opo_result_context->getTypeRestriction( $dummy )
			] );
		if ( ! is_array( $ret ) ) {
			return null;
		}
	...
	public function getResultsDisplayName( $mode = 'singular' ) {
		...
		if ( $this->request->config->get( $this->ops_tablename . '_breakout_find_by_type_in_menu' ) ) {
			...
			if ( ! ( $name = ( $mode == 'singular' ) ? $hier[ $type_id ]['name_singular']
				: $hier[ $type_id ]['name_plural'] )
			) {
				$name = mb_strtolower( ( $mode == 'singular' ) ? $t_instance->getProperty( 'NAME_SINGULAR' )
					: $t_instance->getProperty( 'NAME_PLURAL' ) );
			}

			return mb_strtolower( $name );
		} else {
			return mb_strtolower( ( $mode == 'singular' ) ? $t_instance->getProperty( 'NAME_SINGULAR' )
				: $t_instance->getProperty( 'NAME_PLURAL' ) );
		}
	}
	# -------------------------------------------------------
