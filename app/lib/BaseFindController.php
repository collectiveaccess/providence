...
class BaseFindController extends ActionController
{
    ...
    public function __construct(&$po_request, &$po_response, $pa_view_paths = null)
    {
        ...
        if ($this->ops_tablename) {
            ...
            if ($this->request->config->get(
                    $this->ops_tablename . '_breakout_find_by_type_in_submenu'
                ) || $this->request->config->get($this->ops_tablename . '_breakout_find_by_type_in_menu')) {
                if ($this->opn_type_restriction_id = $this->opo_result_context->getTypeRestriction(
            ...
            if ($display_id = $this->opo_result_context->getCurrentBundleDisplay(
                ...
                $this->opa_sorts = caGetAvailableSortFields(
                    ...
                    array(
                        'request' => $po_request,
                        'restrictToDisplay' => $this->request->config->get(
                            'restrict_find_result_sort_options_to_current_display'
                        ) ? $display_id : null
                    )
                );
            } else {
    ...
    public function Index($pa_options = null)
    {
        ...
        $display_id = $this->opo_result_context->getCurrentBundleDisplay(
            $this->opn_type_restriction_id,
            $this->_getShowInStr()
        );
        ...
        if (($vn_type_id = $this->opo_result_context->getTypeRestriction($vb_type)) && ($t_instance::typeCodeForID(
                $vn_type_id
        ...
        if ($t_display = $this->view->getVar('t_display')) {
            ...
            if (is_array($va_show_in) && sizeof($va_show_in) && !in_array(
                    'search_browse_' . $this->opo_result_context->getCurrentView(),
        ...
        if (method_exists($t_instance, 'getApplicableElementCodes')) {
            ...
            $va_attribute_list = array_flip(
                $t_instance->getApplicableElementCodes(
                    $this->opo_result_context->getTypeRestriction($vb_dummy),
                    false,
                    false
                )
            );
            ...
            foreach ($display_list as $i => $va_display_item) {
                ...
                if ($tmp[0] != $this->ops_tablename) {
                    ...
                    if (($t_rel = Datamodel::getInstance($tmp[0], true)) && method_exists(
                            $t_rel,
                        ...
                        $types = array_merge(
                            caGetOption(
                                ...
                                $va_display_item['settings'],
                                [],
                                ['castTo' => 'array']
                            ),
                            caGetOption('restrict_to_types', $va_display_item['settings'], [], ['castTo' => 'array'])
                        );

                        $display_list[$i]['bundle_sort'] = "{$tmp[0]}.preferred_labels." . $t_rel->getLabelSortField(
                            ) . ((is_array($types) && sizeof($types)) ? "|" . join(",", $types) : "");
                    }
                    continue;
                }

                if ($t_instance->hasField($tmp[1])) {
                    if ($t_instance->getFieldInfo(
                            $tmp[1],
                            'FIELD_TYPE'
        ...
        $include_export_options = $this->request->config->getList(
            $this->ops_tablename . '_standard_results_export_formats'
        );
        foreach (
            [
        ...
        // export mapping list
        if ($this->request->user->canDoAction('can_batch_export_metadata') && $this->request->user->canDoAction(
                'can_export_' . $this->ops_tablename
            )) {
        ...
        $this->view->setVar(
            ...
            caExtractValuesByUserLocale(
                $t_set->getSets(
                    array(
                        'table' => $this->ops_tablename,
                        'user_id' => !(bool)$this->request->config->get(
                            'ca_sets_all_users_see_all_sets'
    ...
    protected function _genExport($po_result, $ps_output_type, $ps_output_filename, $ps_title = null)
    {
        $this->view->setVar(
            'criteria_summary',
            $vs_criteria_summary = $this->getCriteriaForDisplay()
        );    // add displayable description of current search/browse parameters
        $this->view->setVar(
            'criteria_summary_truncated',
            mb_substr($vs_criteria_summary, 0, 60) . ((mb_strlen(
                    $vs_criteria_summary
                ) > 60) ? '...' : '')
        );
        ...
        if (substr($ps_output_type, 0, 4) !== '_pdf') {
            switch ($ps_output_type) {
                ...
                case '_csv':
                    $vs_delimiter = ",";
                    $vs_output_file_name = mb_substr(
                        preg_replace("/[^A-Za-z0-9\-]+/", '_', $ps_output_filename . '_csv'),
                        0,
                        30
                    );
                ...
                case '_tab':
                    $vs_delimiter = "\t";
                    $vs_output_file_name = mb_substr(
                        preg_replace("/[^A-Za-z0-9\-]+/", '_', $ps_output_filename . '_tab'),
                        0,
                        30
                    );
                    $vs_file_extension = 'txt';
            ...
            header(
                "Content-Disposition: attachment; filename=export_" . $vs_output_file_name . "." . $vs_file_extension
            );
            ...
            while ($po_result->nextHit()) {
                ...
                foreach ($display_list as $placement_id => $va_display_item) {
                    $vs_value = html_entity_decode(
                        $t_display->getDisplayValue(
                            $po_result,
                            $placement_id,
                            array('convert_codes_to_display_text' => true, 'convertLineBreaks' => false)
                        ),
                        ENT_QUOTES,
                        'UTF-8'
                    );
            ...
            try {
                ...
                $o_pdf->render(
                    ...
                    array(
                        'stream' => true,
                        'filename' => ($filename = $this->view->getVar(
                            'filename'
                        )) ? $filename : caGetOption('filename', $va_template_info, 'export_results.pdf')
                    )
                );
                exit;
            } catch (Exception $e) {
            ...
            if (!$ps_rows || !sizeof($pa_row_ids)) {
                ...
                if ($t_set->haveAccessToSet($this->request->getUserID(), __CA_SET_EDIT_ACCESS__)) {
                    ...
                    if ($t_set->getPrimaryKey() && ($t_set->get('table_num') == $t_instance->tableNum())) {
                        ...
                        if (($vn_added_items_count = $t_set->addItems(
                                array_keys($va_row_ids_to_add),
                                ['user_id' => $this->request->getUserID()]
                            )) === false) {
    ...
    {
        if ($va_saved_search = $this->request->user->getSavedSearchByKey(
            $this->ops_tablename,
            $this->ops_find_type,
    ...
    {
        return $this->opo_result_context->getSearchExpression(
        );        // just give back the search expression verbatim; works ok for simple searches
    }
    # ------------------------------------------------------------------
    ...
    public function DownloadMedia()
    {
        if ($t_subject = Datamodel::getInstanceByTableName($this->ops_tablename, true)) {
            ...
            if (($ids = trim($this->request->getParameter($t_subject->tableName(), pString))) || ($ids = trim(
                    $this->request->getParameter($t_subject->primaryKey(), pString)
                ))) {
                if ($ids !== 'all') {
                    $id_list = explode(';', $ids);
            ...
            if (($limit = (int)$t_subject->getAppConfig()->get(
                    'maximum_download_file_count'
                )) > 0) {    // truncate to maximum
            ...
            if (is_array($id_list) && sizeof($id_list)) {
                ...
                if (preg_match('!^attribute([\d]+)$!', $preferred_version, $m)) {
                    ...
                    $element_code = $t_subject->tableName() . '.' . (($root_id != $element_id) ? $t_root->get(
                                'element_code'
                            ) . '.' . $t_element->get('element_code') : $t_element->get('element_code'));
                }
                if ($qr_res = $t_subject->makeSearchResult(
                    $t_subject->tableName(),
                    $id_list,
                    ...
                    while ($qr_res->nextHit()) {
                        if (!$element_code) {
                            // representation
                            $version = (is_array(
                                    $version_list = $qr_res->getMediaVersions('ca_object_representations.media')
                                ) || !in_array($preferred_version, $version_list)) ? $preferred_version : 'original';
                            ...
                            foreach ($paths as $i => $path) {
                                ...
                                if (!caCanRead(
                                    $this->request->user->getPrimaryKey(),
                                ...
                                $filename = caGetRepresentationDownloadFileName(
                                    $this->ops_tablename,
                                    [
                                        'idno' => $idno,
                                        'index' => $index,
                            ...
                            $idno = $qr_res->get($t_subject->tableName() . '.idno');
                            $original_filename = pathinfo(
                                $qr_res->get($element_code . '.original_filename'),
                                PATHINFO_BASENAME
                            );
            ...
            if ($file_count > 1) {
                ...
                $o_view->setVar(
                    'archive_name',
                    'media_for_' . mb_substr(
                        preg_replace('![^A-Za-z0-9]+!u', '_', $this->getCriteriaForDisplay()),
                        0,
                        20
                    ) . '.zip'
                );
    ...
    public function Tools($pa_parameters)
    {
        ...
        $set_list = caExtractValuesByUserLocale(
            $t_set->getSets(
                array(
                    'table' => $this->ops_tablename,
                    'user_id' => !(bool)$this->request->config->get(
        ...
        if ($this->request->user->getPreference('list_public_sets') === 'show') {
            $all_users_public_sets = caExtractValuesByUserLocale(
                $t_set->getSets(
                    array(
                        'table' => $this->ops_tablename,
                        'user_id' => !(bool)$this->request->config->get(
        ...
        $this->view->setVar(
            'result',
            (is_array($va_results_id_list) && sizeof($va_results_id_list) > 0) ? caMakeSearchResult(
                $this->ops_tablename,
                $va_results_id_list
            ) : null
        );
    ...
    public function Viz()
    {
        ...
        $this->view->setVar(
            ...
            $o_viz->render(
                $ps_viz,
                "HTML",
                array('classname' => 'vizFullScreen', 'request' => $this->request)
            )
        );
    ...
    public function saveResultsEditorData()
    {
        $t_display = new ca_bundle_displays(
            $this->opo_result_context->getCurrentBundleDisplay($this->opn_type_restriction_id, $this->_getShowInStr())
        );
    ...
    public function getResultsDisplayName($mode = 'singular')
    {
        ...
        if ($this->request->config->get($this->ops_tablename . '_breakout_find_by_type_in_menu')) {
            ...
            if (!($name = ($mode == 'singular') ? $hier[$type_id]['name_singular'] : $hier[$type_id]['name_plural'])) {
                $name = mb_strtolower(
                    ($mode == 'singular') ? $t_instance->getProperty('NAME_SINGULAR') : $t_instance->getProperty(
                        'NAME_PLURAL'
                    )
                );
            }
