...
class BaseVersionUpdater {
	 ...
	 *                          cleanCache = Remove all application caches after database update if true. Default is
	 *                          false.
	 *
	 * @return bool
	 */
		...
		while ( ! feof( $o_handle ) ) {
			...
			if ( $vs_before_eol == ';' ) {
				...
				if ( $o_db->numErrors() > 0 ) {
					...
					$va_messages[ 'error_' . $pn_version ]
						= _t( "Error applying database migration %1: %2; error was at query %3; query was %4",
						$pn_version, join( '; ', $va_error_array_for_printing ), $vn_query_total_nb, $vs_query );
