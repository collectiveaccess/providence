...
class BaseLookupController extends ActionController {
	 ...
	 * bundle specs; array values are *array* lists of values. If an item is not equal to a value in the array it will
	 * not be returned. Leave set to null or empty array if you don't want to filter.
	 */
	...
	public function Get( $pa_additional_query_params = null, $pa_options = null ) {
		...
		if ( ! ca_user_roles::isValidAction( 'can_search_' . $this->ops_table_name )
		     || ( $this->request->user->canDoAction( 'can_search_' . $this->ops_table_name ) )
		) {
			...
			$self                        = explode( ':',
				$this->request->getParameter( 'self', pString ) );        // table:id of calling record

			if ( ( ! (bool) $o_config->get( 'allow_duplicate_items_in_sets' ) )
			     && ( $set_id
					= $this->request->getParameter( 'set_id', pInteger ) )
			) {
			...
			if ( ( $vn_str_len = mb_strlen( $ps_query ) ) > 0 ) {
					...
					if ( sizeof( $va_ids ) > 0 ) {
						...
						if ( ! $pb_no_subtypes ) {
							foreach ( $va_ids as $vn_id ) {
								if ( is_array( $va_children
									= $t_list->getItemsForList( $this->opo_item_instance->getTypeListCode(),
						...
						$o_search->addResultFilter( $this->opo_item_instance->tableName() . '.'
				...
				// get sort field
				$vs_sort = $this->request->getAppConfig()->get( $this->opo_item_instance->tableName()
				                                                . '_lookup_sort' );
				if ( ! $vs_sort ) {
					$vs_sort = '_natural';
				}
				...
				if ( $vs_hier_fld
				     && ( $vn_restrict_to_hier_id = $this->request->getParameter( 'currentHierarchyOnly', pInteger ) )
				) {
					$o_search->addResultFilter( $this->opo_item_instance->tableName() . '.' . $vs_hier_fld, '=',
						(int) $vn_restrict_to_hier_id );
				}
				...
				if ( $vs_additional_query_params || $vs_restrict_to_search ) {
					$vs_search = '(' . trim( $ps_query_proc ) . ( intval( $pb_exact ) ? '' : '*' ) . ')'
					             . $vs_additional_query_params . $vs_restrict_to_search;
				} else {
					$vs_search = trim( $ps_query_proc ) . ( intval( $pb_exact ) ? '' : '*' );
				}

				$qr_res = $o_search->search( $vs_search,
					array( 'search_source' => 'Lookup', 'no_cache' => false, 'sort' => $vs_sort ) );
				...
				if ( ! $pb_no_inline
				     && ( $pb_quickadd
				          || ( ! strlen( $pb_quickadd ) && $this->request->user
				               && $this->request->user->canDoAction( 'can_quickadd_'
				                                                     . $this->opo_item_instance->tableName() )
				               && ! ( (bool) $o_config->get( $this->opo_item_instance->tableName()
				                                             . '_disable_quickadd' ) ) ) )
				) {
					...
					if ( ( strlen( $ps_restrict_to_search ) > 0 ) ) {
						$o_no_filter_result = $o_search->search( trim( $ps_query_proc ) . ( intval( $pb_exact ) ? ''
								: '*' ) . $vs_type_query . $vs_additional_query_params,
							array( 'search_source' => 'Lookup', 'no_cache' => false, 'sort' => $vs_sort ) );
						if ( $o_no_filter_result->numHits() != $qr_res->numHits() ) {
							$va_opts['inlineCreateMessageDoesNotExist']
								= _t( "<em>%1</em> doesn't exist with this filter but %2 record(s) match overall. Create <em>%1</em>?",
								$ps_query, $o_no_filter_result->numHits() );
							$va_opts['inlineCreateMessage']
								= _t( '<em>%1</em> matches %2 more record(s) without the current filter. Create <em>%1</em>?',
								$ps_query, ( $o_no_filter_result->numHits() - $qr_res->numHits() ) );
						}
					}
					if ( ! isset( $va_opts['inlineCreateMessageDoesNotExist'] ) ) {
						$va_opts['inlineCreateMessageDoesNotExist'] = _t( '<em>%1</em> does not exist. Create?',
							$ps_query );
					}
				...
				if ( isset( $self[0] ) && ( $self[0] === $this->ops_table_name ) && isset( $self[1] )
				     && ( $self[1] > 0 )
				) {
			...
			$this->view->setVar( str_replace( ' ', '_', $this->ops_name_singular ) . '_list',
				array_values( $va_items ) );
		} else {
			$this->view->setVar( str_replace( ' ', '_', $this->ops_name_singular ) . '_list', [
				_t( 'You do not have access to search %1', $this->opo_item_instance->getProperty( 'NAME_PLURAL' ) )
			] );
		}
	...
	public function GetHierarchyLevel() {
		...
		foreach ( $pa_ids as $pn_id ) {
			...
			if ( ( ! ( $vn_id ) ) && method_exists( $t_item, "getHierarchyList" ) ) {
				...
				if ( ( sizeof( $va_items_for_locale ) == 1 )
				     && $this->request->getAppConfig()->get( $t_item->tableName() . '_hierarchy_browser_hide_root' )
				) {
			...
			if ( $vb_gen
				...
				$o_config = Configuration::load();
				if ( ! ( is_array(
						$va_sorts = $o_config->getList( $this->ops_table_name . '_hierarchy_browser_sort_values' ) ) )
				     || ! sizeof( $va_sorts )
				) {
					$va_sorts = array();
				}
				...
				if ( ! in_array( $vs_sort_dir = strtolower( $o_config->get( $this->ops_table_name
				                                                            . '_hierarchy_browser_sort_direction' ) ),
					array( 'asc', 'desc' ) )
				) {
					$vs_sort_dir = 'asc';
				}
				...
				if ( is_array( $va_item_ids = $t_item->getHierarchyChildren( $t_item->getPrimaryKey(),
						array( 'idsOnly' => true ) ) )
				     && sizeof( $va_item_ids )
				) {
					$qr_children     = $t_item->makeSearchResult( $t_item->tableName(), $va_item_ids,
						[ 'sort' => $va_sorts, 'sortDirection' => $vs_sort_dir ] );
					$va_child_counts = $t_item->getHierarchyChildCountsForIDs( $va_item_ids );

					if ( ! ( $vs_item_template
						= trim( $o_config->get( "{$vs_table_name}_hierarchy_browser_display_settings" ) ) )
					) {
					...
					while ( $qr_children->nextHit() ) {
						$va_tmp = array(
							$vs_pk                       => $vn_id = $qr_children->get( $this->ops_table_name . '.'
							                                                            . $vs_pk ),
							'item_id'                    => $vn_id,
							'parent_id'                  => $qr_children->get( $this->ops_table_name . '.parent_id' ),
							'idno'                       => $qr_children->get( $this->ops_table_name . '.idno' ),
							$vs_label_display_field_name => $qr_children->get( $this->ops_table_name
							                                                   . '.preferred_labels.'
							                                                   . $vs_label_display_field_name ),
							'locale_id'                  => $qr_children->get( $this->ops_table_name . '.'
							                                                   . 'locale_id' )
						);
						...
						$va_tmp['name'] = caProcessTemplateForIDs( $vs_item_template, $vs_table_name,
							array( $va_tmp[ $vs_pk ] ), array( 'requireLinkTags' => true ) );
						if ( ! $va_tmp['name'] ) {
							$va_tmp['name'] = '??? ' . $va_tmp[ $vs_pk ];
						}

						// Child count is only valid if has_children is not null
						$va_tmp['children'] = isset( $va_child_counts[ $vn_id ] ) ? (int) $va_child_counts[ $vn_id ]
							: 0;
			...
			$va_items_for_locale['_primaryKey']
			                                   = $t_item->primaryKey();    // pass the name of the primary key so the hierbrowser knows where to look for item_id's
	...
	public function GetHierarchyAncestorList() {
		...
		if ( $t_item->getPrimaryKey() ) {
			if ( ! is_array( $va_ancestors = $t_item->getHierarchyAncestors( null,
	...
	public function IDNo() {
		...
		if ( $vs_idno_field = $this->opo_item_instance->getProperty( 'ID_NUMBERING_ID_FIELD' ) ) {
			...
			if ( $vs_idno_context_field
				= $this->opo_item_instance->getProperty( 'ID_NUMBERING_CONTEXT_FIELD' )
			) {        // want to set context before doing identifier lookup, if the table supports contexts (ca_list_items and ca_place do, others don't)
	...
	public function Attribute() {
		...
		if ( mb_strlen( $ps_val ) > 0 ) {
			$qr_values
				           = $o_db->query( 'SELECT value_id FROM ca_attribute_values WHERE element_id=? AND value_longtext1=?',
				$pn_element_id, $ps_val );
	...
	public function SetSortOrder() {
		...
		try {
			...
			switch ( $t_item->getProperty( 'HIERARCHY_TYPE' ) ) {
				...
				case __CA_HIER_TYPE_MULTI_MONO__:
					...
					if ( ! $t_def->isLoaded() ) {
						throw new ApplicationException( _t( 'Could not load %1',
							$t_def->getProperty( 'NAME_PLURAL' ) ) );
					}
					$vs_def_pk = $t_def->primaryKey();

					if ( ! $this->request->user->canDoAction( "can_edit_{$this->ops_table_name}" )
					     && ! $this->request->user->canDoAction( "can_edit_{$vs_def_table_name}" )
					) {
						throw new ApplicationException( _t( 'Access denied' ) );
					}
					break;
			...
			if ( ! $vs_rank_fld ) {
				throw new ApplicationException( _t( 'Sorting is not supported for %1',
			...
			if ( $vs_def_table_name && $t_def->hasField( 'default_sort' ) ) {
				if ( (int) $t_def->get( 'default_sort' ) !== __CA_LISTS_SORT_BY_RANK__ ) {
					throw new ApplicationException( _t( '%1 must have default sort set to rank',
						$t_def->getProperty( 'NAME_SINGULAR' ) ) );
				}
			...
			if ( ! $vb_has_sort_by_rank ) {
				$va_sort_values
					= $this->getRequest()->config->getList( "{$this->ops_table_name}_hierarchy_browser_sort_values" );
				if ( ( sizeof( $va_sort_values ) < 1 )
				     || ( $va_sort_values[0] != "{$this->ops_table_name}.{$vs_rank_fld}" )
				) {
					throw new ApplicationException( _t( '%1 must have sort configured to use rank',
		...
		$this->view->setVar( "result", [
			'ok'        => $vn_return,
			'errors'    => $va_errors,
			'timestamp' => ( sizeof( $va_errors ) == 0 ) ? time() : null
		] );
