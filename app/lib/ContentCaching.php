...
class ContentCaching extends AppControllerPlugin
{
    ...
    private function getCachingTTLForRequest()
    {
        ...
        if (isset($va_cached_actions[$vs_path]) && is_array($va_cached_actions[$vs_path])) {
            ...
            if (isset($va_cached_actions[$vs_path][$vs_action]) && is_numeric(
                    $va_cached_actions[$vs_path][$vs_action]
                )) {
    ...
    public function preDispatch()
    {
        ...
        if ($vs_key = $this->getKeyForRequest()) {
            // is this cached?
            if (!(bool)$this->getRequest()->getParameter('noCache', pInteger) && ExternalCache::contains(
                    $vs_key,
                    'PawtucketPageCache'
                )) {
