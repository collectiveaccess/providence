...
trait DeduplicateBaseModel {
	...
	public function getChecksum() {
		...
		if ( ( $vs_idno_fld = $this->getProperty( 'ID_NUMBERING_ID_FIELD' ) )
		     && ! ( $this->getAppConfig()->get( $this->tableName() . '_dont_use_idno_in_checksums' ) )
		) {
		...
		if ( method_exists( $this, 'getPreferredLabelCount' ) && ( $this->getPreferredLabelCount() > 0 ) ) {
			$va_hash_components[] = $this->get( $this->tableName() . '.preferred_labels',
				array( 'returnAllLocales' => true ) );
		}

		if ( method_exists( $this, 'getNonPreferredLabelCount' ) && ( $this->getNonPreferredLabelCount() > 0 ) ) {
			$va_hash_components[] = $this->get( $this->tableName() . '.nonpreferred_labels',
				array( 'returnAllLocales' => true ) );
		}
	...
	/**
	 * Merge an arbitrary number of records into one. Note that this will delete all records but the first one in the
	 * array.
	 *
	...
	public static function mergeRecords( $pa_records ) {
		...
		foreach ( $pa_records as $vn_record_id ) {
			if ( ! is_numeric( $vn_record_id ) ) {
				throw new DeduplicateException( 'One of the identifiers in the deduplication list is not numeric: '
				                                . $vn_record_id );
			}
	...
	protected static function mergeAttributes( $t_main, $t_dupe ) {
		...
		foreach ( $t_dupe->getApplicableElementCodes() as $vs_element_code )
		{ // dupe and main applicable codes are the same
			...
			foreach ( $t_dupe->getAttributesByElement( $vs_element_code ) as $o_dupe_attribute ) {
				...
				$va_dupe_attributes[ $o_dupe_attribute->getAttributeID() ]
					= md5( serialize( $o_dupe_attribute->getDisplayValues() ) );
			}

			foreach ( $t_main->getAttributesByElement( $vs_element_code ) as $o_main_attribute ) {
				/** @var Attribute $o_main_attribute */
				$va_main_attributes[ $o_main_attribute->getAttributeID() ]
					= md5( serialize( $o_main_attribute->getDisplayValues() ) );
			}
