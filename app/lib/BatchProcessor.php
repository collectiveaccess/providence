...
class BatchProcessor {
	...
	public static function saveBatchEditorFormForSet( $po_request, $t_set, $t_subject, $pa_options = null ) {
		...
		if ( $vb_perform_type_access_checking = (bool) $t_subject->getAppConfig()
		                                                         ->get( 'perform_type_access_checking' )
		) {
			$va_restrict_to_types = caGetTypeRestrictionsForUser( $t_subject->tableName(),
				array( 'access' => __CA_BUNDLE_ACCESS_EDIT__ ) );
		}
		$vb_perform_item_level_access_checking = (bool) $t_subject->getAppConfig()
		                                                          ->get( 'perform_item_level_access_checking' );
		...
		foreach ( array_keys( $va_row_ids ) as $vn_row_id ) {
			...
			if ( $t_subject->load( $vn_row_id ) ) {
				...
				//
				if ( ( $vb_perform_type_access_checking )
				     && ( is_array( $va_restrict_to_types )
				          && ! in_array( $t_subject->get( 'type_id' ), $va_restrict_to_types ) )
				) {
					continue;        // skip
				}
				...
				//
				if ( ( $vb_perform_item_level_access_checking )
				     && ( $t_subject->checkACLAccessForUser( $po_request->user ) == __CA_ACL_EDIT_ACCESS__ )
				) {
					continue;        // skip
				}
				...
				if ( isset( $pa_options['progressCallback'] ) && ( $ps_callback = $pa_options['progressCallback'] ) ) {
					$ps_callback( $po_request, $vn_c, $vn_num_items, _t( "[%3/%4] Processing %1 (%2)",
						caTruncateStringWithEllipsis( $t_subject->getLabelForDisplay(), 50 ),
						$t_subject->get( $t_subject->getProperty( 'ID_NUMBERING_ID_FIELD' ) ), $vn_c, $vn_num_items ),
						time() - $vn_start_time, memory_get_usage( true ), sizeof( $va_notices ),
						sizeof( $va_errors ) );
				}
		...
		if ( isset( $pa_options['progressCallback'] ) && ( $ps_callback = $pa_options['progressCallback'] ) ) {
			$ps_callback( $po_request, $vn_num_items, $vn_num_items, _t( "Processing completed" ),
				time() - $vn_start_time, memory_get_usage( true ), sizeof( $va_notices ), sizeof( $va_errors ) );
		}
...
// 			}
		...
		if ( isset( $pa_options['sendMail'] ) && $pa_options['sendMail'] ) {
			if ( $vs_email = trim( $po_request->user->get( 'email' ) ) ) {
				caSendMessageUsingView( $po_request,
		...
		if ( isset( $pa_options['sendSMS'] ) && $pa_options['sendSMS'] ) {
			SMS::send( $po_request->getUserID(),
				_t( "[%1] Batch processing for set %2 with %3 %4 begun at %5 is complete",
					$po_request->config->get( 'app_display_name' ), caTruncateStringWithEllipsis( $vs_set_name, 20 ),
					$vn_num_items, $t_subject->getProperty( ( $vn_num_items == 1 ) ? 'NAME_SINGULAR' : 'NAME_PLURAL' ),
					$vs_started_on ) );
		}
		...
		return array(
			'errors'          => $va_errors,
			'notices'         => $va_notices,
			'processing_time' => caFormatInterval( $vn_elapsed_time )
		);
	}
	# ----------------------------------------
	...
	public static function deleteBatchForSet( $po_request, $t_set, $t_subject, $pa_options = null ) {
		...
		foreach ( array_keys( $va_row_ids ) as $vn_row_id ) {
			if ( $t_subject->load( $vn_row_id ) ) {
				...
				if ( isset( $pa_options['progressCallback'] ) && ( $ps_callback = $pa_options['progressCallback'] ) ) {
					$ps_callback( $po_request, $vn_c, $vn_num_items,
						_t( "[%3/%4] Processing %1 (%2)", caTruncateStringWithEllipsis( $vs_label, 50 ), $vs_idno,
							$vn_c, $vn_num_items ), time() - $vn_start_time, memory_get_usage( true ),
						sizeof( $va_notices ), sizeof( $va_errors ) );
				}
	...
	public static function changeTypeBatchForSet( $po_request, $pn_type_id, $t_set, $t_subject, $pa_options = null ) {
		...
		if ( ! method_exists( $t_subject, 'getTypeList' ) ) {
			return array(
				'errors'          => array( _t( 'Invalid subject' ) ),
				'notices'         => array(),
				'processing_time' => caFormatInterval( 0 )
			);
		}
		$va_type_list = $t_subject->getTypeList();
		if ( ! isset( $va_type_list[ $pn_type_id ] ) ) {
			return array(
				'errors'          => array( _t( 'Invalid type_id' ) ),
				'notices'         => array(),
				'processing_time' => caFormatInterval( 0 )
			);
		}
	 ...
	 *            in descending order of shrillness: KLogger::EMERG = Emergency messages (system is unusable)
	 *            KLogger::ALERT = Alert messages (action must be taken immediately) KLogger::CRIT = Critical
	 *            conditions KLogger::ERR = Error conditions KLogger::WARN = Warnings KLogger::NOTICE = Notices (normal
	 *            but significant conditions) KLogger::INFO = Informational messages KLogger::DEBUG = Debugging
	 *            messages
	 *
	 * @return array
	 */
	public static function importMediaFromDirectory( $po_request, $pa_options = null ) {
		...
		if ( ! is_dir( $pa_options['importFromDirectory'] ) ) {
			$o_eventlog->log( array(
				"CODE"    => 'ERR',
				"SOURCE"  => "mediaImport",
				"MESSAGE" => $vs_msg = _t( "Specified import directory '%1' is not a directory",
		...
		if ( ! preg_match( "!^{$vs_batch_media_import_root_directory}!", $pa_options['importFromDirectory'] ) ) {
			$o_eventlog->log( array(
				"CODE"    => 'ERR',
				"SOURCE"  => "mediaImport",
				"MESSAGE" => $vs_msg = _t( "Specified import directory '%1' is not within the configured root %2",
					$pa_options['importFromDirectory'], $vs_batch_media_import_root_directory )
			) );
			$o_log->logError( $vs_msg );
			BatchProcessor::$s_import_error_list[] = $vs_msg;
		...
		if ( preg_match( "!\.\./!", $pa_options['importFromDirectory'] ) ) {
			$o_eventlog->log( array(
				"CODE"    => 'ERR',
				"SOURCE"  => "mediaImport",
				"MESSAGE" => $vs_msg = _t( "Specified import directory '%1' is invalid",
					$pa_options['importFromDirectory'] )
			) );
			$o_log->logError( $vs_msg );
			BatchProcessor::$s_import_error_list[] = $vs_msg;
		...
		$va_files_to_process = caGetDirectoryContentsAsList( $pa_options['importFromDirectory'],
		...
		foreach ( $va_files_to_process as $vs_file ) {
			...
			if ( ! $vb_allow_duplicate_media && ( $t_dupe = ca_object_representations::mediaExists( $vs_file ) ) ) {
				if ( ! is_array(
						$dupes_rel_ids = $t_dupe->get( $t_instance->primaryKey(), [ 'returnAsArray' => true ] ) )
					...
					$va_notices[ $vs_relative_directory . '/' . $f ] = array(
						...
						'message' => $vs_msg = _t( 'Skipped %1 from %2 because it already exists %3', $f,
							$vs_relative_directory, $po_request ? caEditorLink( $po_request, _t( '(view)' ), 'button',
					...
					foreach ( $va_regex_info['regexes'] as $vs_regex ) {
						switch ( $vs_match_mode ) {
							...
							case 'FILE_AND_DIRECTORY_NAMES':
								...
								$o_log->logDebug( _t( "Trying to match on directory '%1' and file name '%2'", $d,
									$f ) );
								break;
							default:
							case 'FILE_NAME':
								...
								if ( in_array( $vs_import_mode, array( 'TRY_TO_MATCH', 'ALWAYS_MATCH' ) ) ) {
									...
									if ( class_exists( $vs_import_target )
										...
										if ( $t_instance->load( $vn_id ) ) {
											$va_notices[ $vs_relative_directory . '/' . $vs_match_name . '_match' ]
												= array(
												'idno'    => $t_instance->get( $t_instance->getProperty( 'ID_NUMBERING_ID_FIELD' ) ),
												'label'   => $t_instance->getLabelForDisplay(),
												'message' => $vs_msg
													= _t( 'Matched media %1 from %2 to %3 using expression "%4"', $f,
			...
			if ( $t_instance->getPrimaryKey() && ( $t_instance instanceof RepresentableBaseModel ) ) {
				...
				if ( $use_existing_representation_id ) {
					...
					$t_new_rep = $t_instance->addRepresentation(
						...
						$vn_locale_id, $vn_object_representation_status, $vn_object_representation_access, false,
				...
				if ( in_array( $vs_import_mode, array( 'TRY_TO_MATCH', 'DONT_MATCH' ) ) ) {
					...
					switch ( strtolower( $vs_idno_mode ) ) {
						...
						default:
							...
							if ( ! is_array(
								$va_idno_values = $o_numbering_plugin->htmlFormValuesAsArray( 'idno', null, false,
									false, true ) )
							) {
					...
					if ( $t_instance->numErrors() ) {
						...
						$va_errors[ $vs_relative_directory . '/' . $f ] = array(
							...
							'label'   => $t_instance->getLabelForDisplay(),
							'errors'  => $t_instance->errors(),
							'message' => $vs_msg = _t( "Error creating new record while importing %1 from %2: %3", $f,
								$vs_relative_directory, join( '; ', $t_instance->getErrors() ) ),
							'file'    => $f,
							'status'  => 'ERROR',
						);
					...
					if ( $t_instance->numErrors() ) {
						...
						$va_errors[ $vs_relative_directory . '/' . $f ] = array(
							...
							'label'   => $t_instance->getLabelForDisplay(),
							'errors'  => $t_instance->errors(),
							'message' => $vs_msg = _t( "Error creating record label while importing %1 from %2: %3", $f,
								$vs_relative_directory, join( '; ', $t_instance->getErrors() ) ),
							'file'    => $f,
							'status'  => 'ERROR',
						);
			...
			if ( $t_instance->getPrimaryKey() ) {
				...
				if ( ! $vn_mapping_id
					...
					foreach ( $media_metadata_extraction_defaults as $m => $importer_code ) {
						if ( caCompareMimetypes( $media_mimetype, $m ) ) {
							if ( ! ( $vn_mapping_id = ca_data_importers::find( [ 'importer_code' => $importer_code ],
								[ 'returnAs' => 'firstId' ] ) )
							) {
								if ( $o_log ) {
				...
				if ( $vn_mapping_id
				     && ( $t_mapping = ca_data_importers::find( [ 'importer_id' => $vn_mapping_id ],
				...
				if ( $vn_object_representation_mapping_id
					...
					$t_importer = new ca_data_importers();
					$t_importer->importDataFromSource( $vs_directory . '/' . $f, $vn_object_representation_mapping_id, [
						'logLevel'                  => $o_config->get( 'embedded_metadata_extraction_mapping_log_level' ),
						'format'                    => $format,
								...
								if ( ! $t_instance->numErrors() ) {
									...
									$va_notices[ $t_instance->getPrimaryKey() ] = array(
										...
										'message' => $vs_msg
											...
											$vs_label, $t_rel->getProperty( 'NAME_SINGULAR' ),
											$t_rel->getLabelForDisplay(), join( "; ", $t_instance->getErrors() ) ),
										'file'    => $f,
										'status'  => 'ERROR'
									);
									$o_log->logError( $vs_msg );
		...
		$r_err = fopen( $error_log = caGetTempFileName( "mediaImporterErrorLog", "csv", [ 'useAppTmpDir' => true ] ),
			"w" );
		fputcsv( $r_err, [ 'idno', 'file', 'message', 'status' ] );
		$r_skip = fopen( $skip_log = caGetTempFileName( "mediaImporterSkipLog", "csv", [ 'useAppTmpDir' => true ] ),
			"w" );
			...
			if ( $vs_email = trim( $po_request->user->get( 'email' ) ) ) {
				...
				caSendMessageUsingView( $po_request,
					...
					array(
						'notices'             => $va_notices,
						'errors'              => $va_errors,
						'directory'           => $vs_relative_directory,
						'numErrors'           => sizeof( $va_errors ),
						'numProcessed'        => $vn_c,
		...
		if ( $po_request && isset( $pa_options['sendSMS'] ) && $pa_options['sendSMS'] ) {
			SMS::send( $po_request->getUserID(),
				_t( "[%1] Media import processing for directory %2 with %3 %4 begun at %5 is complete",
					$po_request->config->get( 'app_display_name' ), $vs_relative_directory, $vn_num_items,
					( ( $vn_num_items == 1 ) ? _t( 'file' ) : _t( 'files' ) ), $vs_started_on ) );
		}
		$o_log->logInfo( _t( "Media import processing for directory %1 with %2 %3 begun at %4 is complete",
			$vs_relative_directory, $vn_num_items, ( ( $vn_num_items == 1 ) ? _t( 'file' ) : _t( 'files' ) ),
			$vs_started_on ) );
	 ...
	 *                                     KLogger::NOTICE = Notices (normal but significant conditions) KLogger::INFO
	 *                                     = Informational messages KLogger::DEBUG = Debugging messages
	 */
	public static function importMetadata( $po_request, $ps_source, $ps_importer, $ps_input_format, $pa_options = null
		...
		foreach ( $va_sources as $vs_source ) {
			$vn_file_num ++;
			$t_importer = new ca_data_importers();
			if ( ! $t_importer->importDataFromSource( $vs_source, $ps_importer, array(
				...
				'progressCallback'   => isset( $pa_options['progressCallback'] ) ? $pa_options['progressCallback']
					: null,
		...
		if ( isset( $pa_options['sendSMS'] ) && $pa_options['sendSMS'] ) {
			SMS::send( $po_request->getUserID(), _t( "[%1] Metadata import processing for begun at %2 is complete",
				$po_request->config->get( 'app_display_name' ), caGetLocalizedDate( $vn_start_time ) ) );
		}
