...
class BatchProcessor
{
    ...
    public static function saveBatchEditorFormForSet($po_request, $t_set, $t_subject, $pa_options = null)
    {
        ...
        $vb_perform_item_level_access_checking = (bool)$t_subject->getAppConfig()->get(
        ...
        foreach (array_keys($va_row_ids) as $vn_row_id) {
            ...
            if ($t_subject->load($vn_row_id)) {
                ...
                //
                if (($vb_perform_type_access_checking) && (is_array($va_restrict_to_types) && !in_array(
                            $t_subject->get('type_id'),
                ...
                //
                if (($vb_perform_item_level_access_checking) && ($t_subject->checkACLAccessForUser(
                            $po_request->user
...
// 			}
        ...
        if (isset($pa_options['sendSMS']) && $pa_options['sendSMS']) {
            SMS::send(
                ...
                _t(
                    "[%1] Batch processing for set %2 with %3 %4 begun at %5 is complete",
                    $po_request->config->get('app_display_name'),
                    caTruncateStringWithEllipsis($vs_set_name, 20),
                    $vn_num_items,
                    $t_subject->getProperty(($vn_num_items == 1) ? 'NAME_SINGULAR' : 'NAME_PLURAL'),
                    $vs_started_on
                )
            );
        }
            ...
            if ($t_subject->load($vn_row_id)) {
                ...
                if (isset($pa_options['progressCallback']) && ($ps_callback = $pa_options['progressCallback'])) {
                    $ps_callback(
                        ...
                        _t(
                            "[%3/%4] Processing %1 (%2)",
                            caTruncateStringWithEllipsis($vs_label, 50),
                            $vs_idno,
                            $vn_c,
                            $vn_num_items
                        ),
    ...
    public static function importMediaFromDirectory($po_request, $pa_options = null)
    {
        ...
        if (!is_dir($pa_options['importFromDirectory'])) {
            $o_eventlog->log(
                array(
                    ...
                    "SOURCE" => "mediaImport",
                    "MESSAGE" => $vs_msg = _t(
                        "Specified import directory '%1' is not a directory",
        ...
        if (!preg_match("!^{$vs_batch_media_import_root_directory}!", $pa_options['importFromDirectory'])) {
            $o_eventlog->log(
                array(
                    ...
                    "SOURCE" => "mediaImport",
                    "MESSAGE" => $vs_msg = _t(
                        "Specified import directory '%1' is not within the configured root %2",
                        $pa_options['importFromDirectory'],
                        $vs_batch_media_import_root_directory
                    )
                )
            );
            $o_log->logError($vs_msg);
        ...
        if (preg_match("!\.\./!", $pa_options['importFromDirectory'])) {
            $o_eventlog->log(
                array(
                    ...
                    "SOURCE" => "mediaImport",
                    "MESSAGE" => $vs_msg = _t(
                        "Specified import directory '%1' is invalid",
                        $pa_options['importFromDirectory']
                    )
                )
            );
        ...
        $va_files_to_process = caGetDirectoryContentsAsList(
            $pa_options['importFromDirectory'],
            $vb_include_subdirectories
        );
        $o_log->logInfo(
            _t('Found %1 files in directory \'%2\'', sizeof($va_files_to_process), $pa_options['importFromDirectory'])
        );
        ...
        if ($t_set->getPrimaryKey() && !$t_set->haveAccessToSet($vn_user_id, __CA_SET_EDIT_ACCESS__)) {
            $va_notices['set_access'] = array(
                ...
                'message' => $vs_msg = _t(
                    'Cannot add to set %1 because you do not have edit access',
                    $vs_set_create_name
                ),
                'file' => '',
                'status' => 'SET ERROR'
        ...
        foreach ($va_files_to_process as $vs_file) {
            ...
            if (isset($pa_options['progressCallback']) && ($ps_callback = $pa_options['progressCallback'])) {
                $ps_callback(
                    ...
                    _t(
                        "[%3/%4] Processing %1 (%3)",
                        caTruncateStringWithEllipsis($vs_relative_directory, 20) . '/' . caTruncateStringWithEllipsis(
                            $f,
                            30
                        ),
            ...
            if (!$vb_allow_duplicate_media && ($t_dupe = ca_object_representations::mediaExists($vs_file))) {
                if (!is_array(
                    ...
                    $va_notices[$vs_relative_directory . '/' . $f] = array(
                        ...
                        'label' => $f,
                        'message' => $vs_msg = _t(
                            'Skipped %1 from %2 because it already exists %3',
                            $f,
                            $vs_relative_directory,
                            $po_request ? caEditorLink(
                                $po_request,
                                _t('(view)'),
                                'button',
            ...
            if (in_array($vs_import_mode, array('TRY_TO_MATCH', 'ALWAYS_MATCH')) || (is_array(
                        $va_create_relationship_for
                    ) && sizeof($va_create_relationship_for))) {
                    ...
                    foreach ($va_regex_info['regexes'] as $vs_regex) {
                                ...
                                foreach ($va_replacements_list as $vs_replacement_code => $va_replacement) {
                                    if (isset($va_replacement['regexes']) && is_array($va_replacement['regexes'])) {
                                        ...
                                        if (is_null($vs_replacement_result)) {
                                            $o_log->logError(
                                                _t(
                                                    "There was an error in preg_replace while processing replacement %1.",
                                                    $vs_replacement_code
                                                )
                                            );
                                        }
                                        ...
                                        if ($vs_replacement_result && strlen($vs_replacement_result) > 0) {
                                            $o_log->logDebug(
                                                _t(
                                                    "The result for replacement with code %1 applied to value '%2' is '%3' and was added to the list of file names used for matching.",
                                                    $vs_replacement_code,
                                                    $vs_name,
                                                    $vs_replacement_result
                                                )
                                            );
                                        ...
                                        $o_log->logDebug(
                                            _t(
                                                "Skipped replacement %1 because no search expression was defined.",
                                                $vs_replacement_code
                                            )
                                        );
                                    }
                        ...
                        foreach ($va_names_to_match as $vs_match_name) {
                            if (preg_match('!' . $vs_regex . '!', $vs_match_name, $va_matches)) {
                                ...
                                if (!$vs_modified_filename || (strlen($vs_modified_filename) > strlen(
                                            $va_matches[1]
                                        ))) {
                                ...
                                if (in_array($vs_import_mode, array('TRY_TO_MATCH', 'ALWAYS_MATCH'))) {
                                    if (!is_array(
                                            $va_fields_to_match_on = $o_config->getList('batch_media_import_match_on')
                                    ...
                                    $o_log->logDebug(
                                        "Trying to find records using boolean {$vs_bool} and values " . print_r(
                                            $va_values,
                                            true
                                        )
                                    );

                                    if (class_exists($vs_import_target) && ($vn_id = $vs_import_target::find(
                                            $va_values,
                                            array(
                                                'returnAs' => 'firstId',
                                                'allowWildcards' => true,
                                        ...
                                        if ($t_instance->load($vn_id)) {
                                            $va_notices[$vs_relative_directory . '/' . $vs_match_name . '_match'] = array(
                                                ...
                                                'message' => $vs_msg = _t(
                                                    'Matched media %1 from %2 to %3 using expression "%4"',
                                                    $f,
                                                    $vs_relative_directory,
            ...
            if (!$t_instance->getPrimaryKey() && ($vs_import_mode !== 'DONT_MATCH')) {
                ...
                if ($t_instance->load(array('idno' => $f, 'deleted' => 0))) {
                    $va_notices[$vs_relative_directory . '/' . $f . '_match'] = array(
                        ...
                        'label' => $t_instance->getLabelForDisplay(),
                        'message' => $vs_msg = _t(
                            'Matched media %1 from %2 to %3 using filename',
                            $f,
                            $vs_relative_directory,
                            caGetTableDisplayName($vs_import_target, false)
                        ),
            ...
            if ($t_instance->getPrimaryKey() && ($t_instance instanceof RepresentableBaseModel)) {
                ...
                if ($use_existing_representation_id) {
                    if (!($t_new_rep = $t_instance->addRelationship(
                        'ca_object_representations',
                        $use_existing_representation_id,
                        $vn_rel_type_id
                    ))) {
                        continue;
                    }
                ...
                if ($t_instance->numErrors()) {
                    $o_eventlog->log(
                        array(
                            ...
                            "SOURCE" => "mediaImport",
                            "MESSAGE" => _t(
                                "Error importing {$f} from {$vs_directory}: %1",
                ...
                if (in_array($vs_import_mode, array('TRY_TO_MATCH', 'DONT_MATCH'))) {
                    ...
                    if ($t_instance->numErrors()) {
                        $o_eventlog->log(
                            array(
                                ...
                                "SOURCE" => "mediaImport",
                                "MESSAGE" => _t(
                                    "Error creating new record while importing %1 from %2: %3",
                                    $f,
                                    $vs_relative_directory,
                                    join('; ', $t_instance->getErrors())
                                )
                    ...
                    if ($t_instance->tableName() == 'ca_entities') { // entity labels deserve special treatment
                        ...
                        $t_instance->addLabel(
                            array($t_instance->getLabelDisplayField() => $f),
                            $vn_locale_id,
                            null,
                            true
                        );
                    ...
                    if ($t_instance->numErrors()) {
                        $o_eventlog->log(
                            array(
                                ...
                                "SOURCE" => "mediaImport",
                                "MESSAGE" => _t(
                                    "Error creating record label while importing %1 from %2: %3",
                                    $f,
                                    $vs_relative_directory,
                                    join('; ', $t_instance->getErrors())
                                )
            ...
            if ($t_instance->getPrimaryKey()) {
                if (!$vn_mapping_id && is_array(
                        $media_metadata_extraction_defaults = $o_config->getAssoc(
                    ...
                    foreach ($media_metadata_extraction_defaults as $m => $importer_code) {
                        if (caCompareMimetypes($media_mimetype, $m)) {
                            if (!($vn_mapping_id = ca_data_importers::find(
                                ['importer_code' => $importer_code],
                                ['returnAs' => 'firstId']
                            ))) {
                                if ($o_log) {
                                    $o_log->logInfo(
                                        _t('Could not find embedded metadata importer with code %1', $importer_code)
                                    );
                                }
                            }
                            break;
                ...
                if ($vn_mapping_id && ($t_mapping = ca_data_importers::find(
                        ['importer_id' => $vn_mapping_id],
                    ...
                    if ($o_log) {
                        $o_log->logDebug(
                            _t('Using embedded media mapping %1 (format %2)', $t_mapping->get('importer_code'), $format)
                        );
                    }
                ...
                if (is_array($va_create_relationship_for) && sizeof($va_create_relationship_for) && is_array(
                        $va_extracted_idnos_from_filename
                            ...
                            if ($t_rel->load(array($t_rel->getProperty('ID_NUMBERING_ID_FIELD') => $vs_idno))) {
                                ...
                                if (!$t_instance->numErrors()) {
                                    $va_notices[$t_instance->getPrimaryKey() . '_rel'] = array(
                                        ...
                                        'message' => $vs_msg = _t(
                                            'Added relationship between <em>%1</em> and %2 <em>%3</em>',
                                            $vs_label,
                                            $t_rel->getProperty('NAME_SINGULAR'),
                                    ...
                                    $va_notices[$t_instance->getPrimaryKey()] = array(
                                        ...
                                        'label' => $vs_label = $t_instance->getLabelForDisplay(),
                                        'message' => $vs_msg = _t(
                                            'Could not add relationship between <em>%1</em> and %2 <em>%3</em>: %4',
                                            $vs_label,
                                            $t_rel->getProperty('NAME_SINGULAR'),
                                            $t_rel->getLabelForDisplay(),
                                            join("; ", $t_instance->getErrors())
                ...
                $va_notices[$vs_relative_directory . '/' . $f] = array(
                    'idno' => '',
                    'label' => $f,
                    'message' => $vs_msg = (($vs_import_mode == 'ALWAYS_MATCH') ? _t(
                        'Skipped %1 from %2 because it could not be matched',
                        $f,
                        $vs_relative_directory
        ...
        if ($po_request && isset($pa_options['sendSMS']) && $pa_options['sendSMS']) {
            SMS::send(
                $po_request->getUserID(),
                _t(
                    "[%1] Media import processing for directory %2 with %3 %4 begun at %5 is complete",
                    $po_request->config->get('app_display_name'),
                    $vs_relative_directory,
                    $vn_num_items,
                    (($vn_num_items == 1) ? _t('file') : _t('files')),
                    $vs_started_on
                )
            );
        }
        $o_log->logInfo(
            _t(
                "Media import processing for directory %1 with %2 %3 begun at %4 is complete",
                $vs_relative_directory,
                $vn_num_items,
                (($vn_num_items == 1) ? _t('file') : _t('files')),
                $vs_started_on
    ...
    public static function importMetadata($po_request, $ps_source, $ps_importer, $ps_input_format, $pa_options = null)
    {
        ...
        foreach ($va_sources as $vs_source) {
            ...
            if (!$t_importer->importDataFromSource(
                ...
                array(
                    ...
                    'logDirectory' => $o_config->get(
                        'batch_metadata_import_log_directory'
                    ),
                    'request' => $po_request,
        ...
        if (isset($pa_options['sendSMS']) && $pa_options['sendSMS']) {
            SMS::send(
                $po_request->getUserID(),
                _t(
                    "[%1] Metadata import processing for begun at %2 is complete",
                    $po_request->config->get('app_display_name'),
                    caGetLocalizedDate($vn_start_time)
                )
            );
        }
        return array(
