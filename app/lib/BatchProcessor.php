...
class BatchProcessor
{
    ...
    {
        if (preg_match("!SynoResource!", $ps_file)) {
            return true;
            ...
            if ($t_instance->getPrimaryKey()) {
                ...
                if ($vn_mapping_id) {
                    ca_data_importers::importDataFromSource(
                        ...
                        $vn_mapping_id,
                        [
                            'logLevel' => $vs_log_level,
                            'format' => 'exif',
                            'forceImportForPrimaryKeys' => [
                                $t_instance->getPrimaryKey()
                            ]
                        ]
                    );//, 'transaction' => $o_trans]);
                }
        ...
        if ($po_request && isset($pa_options['sendMail']) && $pa_options['sendMail']) {
            if ($vs_email = trim($po_request->user->get('email'))) {
                caSendMessageUsingView(
                    ...
                    null,
                    null,
                    ['source' => 'Batch media import complete']
                );
            }
        }
