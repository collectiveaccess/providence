...
class BaseModel extends BaseObject
{
     ...
     */
    static public $s_ca_models_definitions;

    /**
     * Array of cached relationship info arrays
     */
    static public $s_relationship_info_cache = array();

    /**
     * Cached field values for load()'ed rows
     */
    static public $s_instance_cache = array();

    /**
     *
     */
    static public $s_field_value_arrays_for_IDs_cache = array();
    ...
    public function __construct($pn_id = null, $pb_use_cache = true)
    {
        ...
        $this->opb_purify_input = strlen($this->_CONFIG->get("purify_all_text_input")) ? (bool)$this->_CONFIG->get(
            "purify_all_text_input"
        ) : true;
            ...
            if ($va_tmp[0] === $this->tableName()) {
                ...
                switch (sizeof($va_tmp)) {
                    ...
                    default: // > 2 elements
                        ...
                        if (($va_tmp[1] == 'parent') && ($this->isHierarchical()) && ($vn_parent_id = $this->get(
                            ...
                            if (($va_tmp[1] == 'children') && ($this->isHierarchical())) {
                                ...
                                $vs_sort_direction = (isset($pa_options['sort_direction']) && in_array(
                                        strtolower($pa_options['sort_direction']),
                                        array('asc', 'desc')
                                    )) ? strtolower($pa_options['sort_direction']) : 'asc';
                                ...
                                if (is_array($va_children_ids) && sizeof($va_children_ids)) {
                                    ...
                                    if (($va_tmp[1] == $this->primaryKey()) && !$vs_sort) {
                                        ...
                                        if (method_exists($this, "makeSearchResult")) {
                                            ...
                                            $qr_children = $this->makeSearchResult(
                                                $this->tableName(),
                                                $va_children_ids
                                            );
                                            while ($qr_children->nextHit()) {
                                                if ($vb_check_access && !in_array(
                                                        $qr_children->get("{$vs_table}.access"),
                                                ...
                                                if ($vb_return_as_array) {
                                                    $va_data[$vs_sort_key][$vn_child_id] = array_shift(
                                                        $qr_children->get(
                                                            $vs_childless_path,
                                                            array_merge(
                                                                $pa_options,
                                                    ...
                                                    $va_data[$vs_sort_key][$vn_child_id] = $qr_children->get(
                                                        $vs_childless_path,
                                                        array_merge(
                                                            $pa_options,
                                                            array(
                                            ...
                                            foreach ($va_children_ids as $vn_child_id) {
                                                if ($t_instance->load($vn_child_id)) {
                                                    ...
                                                    if ($vb_return_as_array) {
                                                        ...
                                                        $va_data[$vs_sort_key][$vn_child_id] = $t_instance->get(
                                                            $vs_childless_path,
                                                            array_merge(
                                                                $pa_options,
                                                                array(
        ...
        if (array_key_exists($ps_field, $this->FIELDS)) {
            ...
            switch ($ps_field_type) {
                ...
                case (FT_DATE):
                    ...
                    } elseif (caGetOption('GET_DIRECT_DATE', $pa_options, false) || caGetOption(
                            'getDirectDate',
                            $pa_options,
                            false
                ...
                case (FT_TIME):
                    ...
                    } elseif (caGetOption('GET_DIRECT_TIME', $pa_options, false) || caGetOption(
                            'getDirectTime',
                            $pa_options,
                            false
                ...
                case (FT_TIMECODE):
                    ...
                    $vs_prop = $o_tp->getText(
                        isset($pa_options["TIMECODE_FORMAT"]) ? $pa_options["TIMECODE_FORMAT"] : null
                    );
                    break;
                case (FT_MEDIA):
                case (FT_FILE):
    ...
    public function getFieldValuesForIDs($pa_ids, $pa_fields = null, $pa_options = null)
    {
        ...
        if (!$vb_dont_use_cache && isset(
                BaseModel::$s_field_value_arrays_for_IDs_cache[$vn_table_num = $this->tableNum()][$vs_cache_key]
            )) {
        ...
        $vs_fld_list = (sizeof($va_fld_list)) ? join(
            ", ",
            array_map(
                function ($v) {
     ...
     */
    public static function getIDsForIdnos($pa_idnos, $pa_options = null)
    {
        ...
        $va_ret = [];
        while ($qr_res->nextRow()) {
            $va_ret[$pb_force_to_lowercase ? strtolower($qr_res->get($vs_idno_fld)) : $qr_res->get(
                $vs_idno_fld
            )] = $qr_res->get($vs_pk);
        }
        return $va_ret;
            ...
            if (array_key_exists($vs_field, $this->FIELDS)) {
                ...
                switch ($pa_fields_type) {
                    case (FT_NUMBER):
                        ...
                        if (($vs_field == $this->HIERARCHY_PARENT_ID_FLD) && ((strlen($vm_value) > 0) && (!is_numeric(
                                        $vm_value
                        ...
                        if (($vm_value !== "") || ($this->getFieldInfo($vs_field, "IS_NULL") && ($vm_value == ""))) {
                            if ($vm_value) {
                                if (($vs_list_code = $this->getFieldInfo($vs_field, "LIST_CODE")) && (!is_numeric(
                                        $vm_value
                    ...
                    case (FT_TEXT):
                        ...
                        if ((isset($pa_options['purify']) && ($pa_options['purify'])) || ((bool)$this->opb_purify_input) || ($this->getFieldInfo(
                                $vs_field,
                                "PURIFY"
                            ))) {
                        ...
                        if ($this->getFieldInfo($vs_field, "DISPLAY_TYPE") == DT_LIST_MULTIPLE) {
                            if (is_array($vm_value)) {
                                if (!($vs_list_multiple_delimiter = $this->getFieldInfo(
                                    $vs_field,
                    ...
                    case (FT_FILE):
                        ...
                        if (caGetOSFamily(
                            ) == OS_WIN32) {    // fix for paths using backslashes on Windows failing in processing
                        ...
                        if ((isset($pa_options['purify']) && ($pa_options['purify'])) || ((bool)$this->opb_purify_input) || ($this->getFieldInfo(
                            ...
                            $pa_options["original_filename"] = BaseModel::getPurifier()->purify(
                                (string)$pa_options["original_filename"]
                            );
                            $vm_value = BaseModel::getPurifier()->purify((string)$vm_value);
                        }
                ...
                $this->postError(
                    710,
                    _t("'%1' does not exist in this object", $vs_field),
                    "BaseModel->set()",
                    $this->tableName() . '.' . $vs_field
                );
                return false;
    ...
    public function load($pm_id = null, $pb_use_cache = true)
    {
        $this->clear();
        $vs_table_name = $this->tableName();
        if ($pb_use_cache && is_numeric(
                $pm_id
            ) && isset(BaseModel::$s_instance_cache[$vs_table_name][(int)$pm_id]) && is_array(
                BaseModel::$s_instance_cache[$vs_table_name][(int)$pm_id]
            )) {
                ...
                if (preg_match("/([\w_]+)\.([\w_]+)/", $vs_field, $va_matches)) {
                    if ($va_matches[1] != $this->tableName()) {
                        if (Datamodel::tableExists($va_matches[1])) {
                            ...
                            $this->postError(
                                715,
                                _t("BaseModel '%1' does not exist", $va_matches[1]),
                                "BaseModel->load()"
                            );
                            return false;
                        }
        ...
        if ($qr_res->nextRow()) {
            ...
            if ($vn_id = $this->_FIELD_VALUES[$this->primaryKey()]) {
                if (is_array(BaseModel::$s_instance_cache[$vs_table_name]) && sizeof(
                        BaseModel::$s_instance_cache[$vs_table_name]
                    ) > 100) {
                    BaseModel::$s_instance_cache[$vs_table_name] = array_slice(
                        BaseModel::$s_instance_cache[$vs_table_name],
                        0,
                        50,
                        true
    ...
    private function _calcHierarchicalIndexing($pa_parent_info)
    {
		...
		",
            $pa_parent_info[$vs_hier_left_fld],
            $pa_parent_info[$vs_hier_right_fld],
    ...
    public function insert($pa_options = null)
    {
        ...
        if (($vb_is_hierarchical = $this->isHierarchical(
            )) && (!isset($pa_options['dontSetHierarchicalIndexing']) || !$pa_options['dontSetHierarchicalIndexing'])) {
        ...
        foreach ($this->FIELDS as $vs_field => $va_attr) {
            ...
            if ($pb_need_reload) {
                $vs_field_value = $this->get(
                    $vs_field,
                    array("TIMECODE_FORMAT" => "RAW")
                );    // reload value since verifyFieldValue() may force the value to a default
            }
                ...
                if (!(
                    ...
                    if ($t_many_table->numErrors()) {
                        $this->postError(
                            750,
                            _t(
                                "%1 record with $vs_field = %2 does not exist",
                                $t_many_table->tableName(),
                                $this->get($vs_field)
                            ),
                            "BaseModel->insert()",
                            $this->tableName() . '.' . $vs_field
                ...
                switch ($vs_field_type) {
                    ...
                    case (FT_HISTORIC_DATE):
                        ...
                        if ((($v == '') || is_null($v)) && !$va_attr["IS_NULL"]) {
                            $this->postError(
                                1805,
                                _t(
                                    "Date is undefined but field %1 does not support NULL values",
                                    $vs_field
                                ),
                                "BaseModel->insert()",
                                $this->tableName() . '.' . $vs_field
                        ...
                        if (!is_numeric($v) && !(is_null($v) && $va_attr["IS_NULL"])) {
                            $this->postError(
                                1100,
                                _t("Date is invalid for %1", $vs_field),
                                "BaseModel->insert()",
                                $this->tableName() . '.' . $vs_field
                            );
                            if ($vb_we_set_transaction) {
                    ...
                    case (FT_TIME):
                        ...
                        if ((($v == '') || is_null($v)) && !$va_attr["IS_NULL"]) {
                            $this->postError(
                                1805,
                                _t(
                                    "Time is undefined but field %1 does not support NULL values",
                                    $vs_field
                                ),
                                "BaseModel->insert()",
                                $this->tableName() . '.' . $vs_field
                        ...
                        if (!is_numeric($v) && !(is_null($v) && $va_attr["IS_NULL"])) {
                            $this->postError(
                                1100,
                                _t("Time is invalid for ", $vs_field),
                                "BaseModel->insert()",
                                $this->tableName() . '.' . $vs_field
                            );
                            if ($vb_we_set_transaction) {
                    ...
                    case (FT_HISTORIC_DATERANGE):
                        ...
                        if (
                            !$va_attr["IS_NULL"]
                            &&
                            ((($this->_FIELD_VALUES[$start_field_name] == '') || is_null(
                                        $this->_FIELD_VALUES[$start_field_name]
                                    ))
                                ||
                                (($this->_FIELD_VALUES[$end_field_name] == '') || is_null(
                        ...
                        if (!is_numeric($this->_FIELD_VALUES[$start_field_name]) && !($va_attr["IS_NULL"] && is_null(
                                    $this->_FIELD_VALUES[$start_field_name]
                                ))) {
                            $this->postError(
                                1100,
                                _t("Starting date is invalid"),
                                "BaseModel->insert()",
                                $this->tableName() . '.' . $vs_field
                            );
                            if ($vb_we_set_transaction) {
                        ...
                        if (!is_numeric($this->_FIELD_VALUES[$end_field_name]) && !($va_attr["IS_NULL"] && is_null(
                                    $this->_FIELD_VALUES[$end_field_name]
                                ))) {
                            $this->postError(
                                1100,
                                _t("Ending date is invalid"),
                                "BaseModel->insert()",
                                $this->tableName() . '.' . $vs_field
                            );
                            if ($vb_we_set_transaction) {
                    ...
                    case (FT_BIT):
                        ...
                        if (!is_numeric($v)) {
                            $this->postError(
                                1100,
                                _t("Number is invalid for %1 [%2]", $vs_field, $v),
                                "BaseModel->insert()",
                                $this->tableName() . '.' . $vs_field
                            );
                            if ($vb_we_set_transaction) {
                    ...
                    case (FT_VARS):
                        $vs_fields .= $vs_field . ",";
                        $vs_values .= $this->quote(
                                caSerializeForDatabase(
                                    $this->get($vs_field),
        ...
        if ($this->numErrors() == 0) {
            ...
            if ($this->numErrors() == 0) {
                ...
                if ((sizeof($va_media_fields) > 0) || (sizeof($va_file_fields) > 0) || ($this->getHierarchyType(
                        ) == __CA_HIER_TYPE_ADHOC_MONO__)) {
                    $vs_sql = "";
                    if (sizeof($va_media_fields) > 0) {
                        foreach ($va_media_fields as $f) {
                            if ($vs_msql = $this->_processMedia(
                                $f,
                                array(
                    ...
                    if ($this->numErrors() == 0) {
                        ...
                        if ($vs_sql) {
                            $o_db->query(
                                "UPDATE " . $this->tableName() . " SET " . $vs_sql . " WHERE " . $this->primaryKey(
                                ) . " = ?",
                ...
                if ((!isset($pa_options['dont_do_search_indexing']) || (!$pa_options['dont_do_search_indexing'])) && !defined(
                ...
                if (is_array(BaseModel::$s_instance_cache[$vs_table_name = $this->tableName()]) && (sizeof(
                            BaseModel::$s_instance_cache[$vs_table_name = $this->tableName()]
                        ) > 100)) {    // Limit cache to 100 instances per table
                    BaseModel::$s_instance_cache[$vs_table_name] = array_slice(
                        BaseModel::$s_instance_cache[$vs_table_name],
                        0,
                        50,
            ...
            foreach ($o_db->errors() as $o_e) {
                switch ($vn_err_num = $o_e->getErrorNumber()) {
                    case 251:    // violation of unique key (duplicate record)
                        ...
                        if (preg_match(
                            ...
                            if (sizeof($va_field_labels) > 0) {
                                $vs_err_desc = _t(
                                    "The combination of %1 and %2 must be unique",
                                    join(', ', $va_field_labels),
                                    $vs_last_name
                                );
                            } else {
                    ...
                    default:
                        $this->postError(
                            $vn_err_num,
                            $o_e->getErrorDescription() . ' [' . $vn_err_num . ']',
                            "BaseModel->insert()",
                            $this->tableName() . '.' . $vs_field
                        );
                        break;
    ...
    public function update($pa_options = null)
    {
        ...
        if (!caGetOption(
                ...
                $pa_options,
                false
            ) && isset($_REQUEST['form_timestamp']) && ($vn_form_timestamp = $_REQUEST['form_timestamp'])) {
            $va_possible_conflicts = $this->getChangeLog(
                null,
                array(
                    'forTable' => true,
                    'range' => array(
                        'start' => $vn_form_timestamp,
                        'end' => time()
                    ),
                ...
                switch (sizeof($va_conflict_users)) {
                    case 0:
                        $this->postError(
                            795,
                            _t(
                                'Changes have been made since you loaded this data. Save again to overwrite the changes or cancel to keep the changes.'
                            ),
                            "BaseModel->update()"
                        );
                        break;
                    case 1:
                        $this->postError(
                            795,
                            _t(
                                'Changes have been made since you loaded this data by %1. Save again to overwrite the changes or cancel to keep the changes.',
                                join(', ', array_keys($va_conflict_users))
                            ),
                            "BaseModel->update()"
                        );
                        break;
                    default:
                        $this->postError(
                            795,
                            _t(
                                'Changes have been made since you loaded this data by these users: %1. Save again to overwrite the changes or cancel to keep the changes.',
                                join(', ', array_keys($va_conflict_users))
                            ),
                            "BaseModel->update()"
                        );
        ...
        if ($vb_is_hierarchical = $this->isHierarchical()) {
            ...
            if ($vb_parent_id_changed = $this->changed($vs_parent_id_fld)) {
                ...
                if (!$pa_options['dontCheckCircularReferences']) {
                    ...
                    if (in_array($this->get($vs_parent_id_fld), $va_ids)) {
                        $this->postError(
                            ...
                            "BaseModel->update()",
                            $this->tableName() . '.' . $vs_parent_id_fld
                        );
                        if ($vb_we_set_transaction) {
                            $this->removeTransaction(false);
                        }
                ...
                switch ($this->getHierarchyType()) {
                    ...
                    case __CA_HIER_TYPE_MULTI_MONO__:    // you need to set parent_id; you only need to set hierarchy_id if you're creating a root
                        ...
                        if (!($vn_hierarchy_id = $this->get($vs_hier_id_fld))) {
                            $this->postError(
                                2030,
                                _t("Hierarchy ID must be specified for this update"),
                                "BaseModel->update()",
                                $this->tableName() . '.' . $vs_hier_id_fld
                            );
                            if ($vb_we_set_transaction) {
                                $this->removeTransaction(false);
                            }
                        ...
                        if (is_array($va_parent_info) && ($this->get(
                                    $vs_hier_id_fld
                                ) != $va_parent_info[$vs_hier_id_fld])) {
                            $vn_hierarchy_id = $va_parent_info[$vs_hier_id_fld];
                            $this->set($this->getProperty('HIERARCHY_ID_FLD'), $vn_hierarchy_id);

                            $va_rebuild_hierarchical_index = $this->getHierarchyChildrenForIDs(
                                [$this->getPrimaryKey()],
                                [
                                    'idsOnly' => true,
                                    'includeSelf' => false
                ...
                if (!isset($pa_options['dontSetHierarchicalIndexing']) || !$pa_options['dontSetHierarchicalIndexing']) {
                    ...
                    $this->set(
                        $this->getProperty('HIERARCHY_LEFT_INDEX_FLD'),
                        $vn_orig_hier_left = $va_hier_indexing['left']
                    );
                    $this->set(
                        $this->getProperty('HIERARCHY_RIGHT_INDEX_FLD'),
        ...
        foreach ($this->FIELDS as $vs_field => $va_attr) {
            ...
            if ($va_attr["IS_NULL"] && (!in_array($vs_field_type, array(FT_MEDIA, FT_FILE))) && (strlen(
                        $vs_field_value
                    ) == 0)) {
                ...
                switch ($vs_field_type) {
                    ...
                    case (FT_BIT):
                        ...
                        if (!is_numeric($vm_val)) {
                            $this->postError(
                                1100,
                                _t("Number is invalid for %1", $vs_field),
                                "BaseModel->update()",
                                $this->tableName() . '.' . $vs_field
                            );
                            if ($vb_we_set_transaction) {
                    ...
                    case (FT_HISTORIC_DATE):
                        ...
                        if ((($vm_val == '') || is_null($vm_val)) && !$va_attr["IS_NULL"]) {
                            $this->postError(
                                1805,
                                _t("Date is undefined but field does not support NULL values"),
                                "BaseModel->update()",
                                $this->tableName() . '.' . $vs_field
                            );
                            if ($vb_we_set_transaction) {
                    ...
                    case (FT_TIME):
                        ...
                        if ((($vm_val == '') || is_null($vm_val)) && !$va_attr["IS_NULL"]) {
                            $this->postError(
                                1805,
                                _t("Time is undefined but field does not support NULL values"),
                                "BaseModel->update()",
                                $this->tableName() . '.' . $vs_field
                            );
                            if ($vb_we_set_transaction) {
                        ...
                        if (!is_numeric($vm_val) && !(is_null($vm_val) && $va_attr["IS_NULL"])) {
                            $this->postError(
                                1100,
                                _t("Time is invalid for %1", $vs_field),
                                "BaseModel->update()",
                                $this->tableName() . '.' . $vs_field
                            );
                            if ($vb_we_set_transaction) {
                    ...
                    case (FT_MEDIA):
                        ...
                        if ($vs_media_sql = $this->_processMedia(
                            $vs_field,
                            array(
                                'processingMediaForReplication' => caGetOption(
                                    'processingMediaForReplication',
                                    $pa_options,
                                    false
                                ),
        ...
        if ($this->numErrors() == 0) {
            if ($vn_fields_that_have_been_set > 0) {
                ...
                if (($fields_changed > 0) && method_exists($this, "deriveHistoryTrackingCurrentValue") && !caGetOption(
                        'dontUpdateHistoryCurrentValueTracking',
                        $pa_options,
                        false
                ...
                if (is_array(
                    ...
                    foreach ($va_rebuild_hierarchical_index as $vn_child_id) {
                        ...
                        if ($t_instance->load($vn_child_id)) {
                            ...
                            $va_hier_info = $this->_calcHierarchicalIndexing(
                                $this->_getHierarchyParent($va_child_info[$vs_parent_id_fld])
                            );
    ...
    public function doSearchIndexing(
        $pa_changed_field_values_array = null,
        $pb_reindex_mode = false,
        $pa_options = null
    ) {
                        ...
                        if ($pb_delete_related) {
                            while ($qr_record_check->nextRow()) {
                                if ($t_related->load($qr_record_check->get($t_related->primaryKey()))) {
                                    $t_related->delete(
                                        $pb_delete_related,
                                        array_merge($pa_options, array('hard' => true)),
                                        null,
                                        $pa_table_list
                                    );
                                    ...
                                    if ($t_related->numErrors()) {
                                        $this->postError(
                                            790,
                                            _t(
                                                "Can't delete item because items related to it have sub-records (%1)",
                                                $vs_many_table
                                            ),
                                            "BaseModel->delete()"
                                        );
                            ...
                            $this->postError(
                                780,
                                _t("Can't delete item because it is in use (%1)", $vs_many_table),
                                "BaseModel->delete()"
                            );
                            if ($vb_we_set_transaction) {
                                $this->removeTransaction(false);
                ...
                case 'PARTIAL':
                    ...
                    if ($o_tq->addTask(
                        ...
                        $this->postError(
                            100,
                            _t(
                                "Couldn't queue mirror using '%1' for version '%2' (handler '%3')",
                                $vs_mirror_method,
                                $ps_version,
                                $vs_queue
                            ),
                            "BaseModel->retryMediaMirror()"
                        );
                    }
                    break;
            }
    ...
    public function getMediaUrl($ps_field, $ps_version, $pn_page = 1, $pa_options = null)
    {
        ...
        if (isset($va_media_info[$ps_version]['USE_ICON']) && ($vs_icon_code = $va_media_info[$ps_version]['USE_ICON'])) {
            return caGetDefaultMediaIconUrl(
                $vs_icon_code,
                $va_media_info[$ps_version]['WIDTH'],
    ...
    public function getMediaPath($ps_field, $ps_version, $pn_page = 1)
    {
        ...
        #
        if (isset($va_media_info[$ps_version]['USE_ICON']) && ($vs_icon_code = $va_media_info[$ps_version]['USE_ICON'])) {
            return caGetDefaultMediaIconPath(
                $vs_icon_code,
                $va_media_info[$ps_version]['WIDTH'],
                $va_media_info[$ps_version]['HEIGHT']
            );
    ...
    public function getMediaTag($ps_field, $ps_version, $pa_options = null)
    {
        ...
        #
        if (isset($va_media_info[$ps_version]['USE_ICON']) && ($vs_icon_code = $va_media_info[$ps_version]['USE_ICON'])) {
            return caGetDefaultMediaIconTag(
                $vs_icon_code,
                $va_media_info[$ps_version]['WIDTH'],
                $va_media_info[$ps_version]['HEIGHT']
            );
        ...
        $url = caGetOption('usePath', $pa_options, false) ? $this->getMediaPath(
            $ps_field,
            $ps_version,
        ...
        return $m->htmlTag(
            $va_media_info[$ps_version]["MIMETYPE"],
            $url,
            $va_media_info[$ps_version]["PROPERTIES"],
            array_merge($pa_options, ['alt' => $alt_text]),
            $va_volume
        );
    }
    ...
    public function &getMediaInfo($ps_field, $ps_version = null, $ps_property = null)
    {
        ...
        if ($ps_version && (!$ps_property || (in_array($ps_property, array('WIDTH', 'HEIGHT'))))) {
            if (isset($va_media_info[$ps_version]['USE_ICON']) && ($vs_icon_code = $va_media_info[$ps_version]['USE_ICON'])) {
                if ($va_icon_size = caGetMediaIconForSize(
                    $vs_icon_code,
                    $va_media_info[$ps_version]['WIDTH'],
                    $va_media_info[$ps_version]['HEIGHT']
                )) {
            ...
            if (!$ps_property || (in_array($ps_property, array('WIDTH', 'HEIGHT')))) {
                foreach (array_keys($va_media_info) as $vs_version) {
                    if (isset($va_media_info[$vs_version]['USE_ICON']) && ($vs_icon_code = $va_media_info[$vs_version]['USE_ICON'])) {
                        if ($va_icon_size = caGetMediaIconForSize(
                            $vs_icon_code,
                            $va_media_info[$vs_version]['WIDTH'],
                            $va_media_info[$vs_version]['HEIGHT']
                        )) {
        ...
        if ($va_media_info = $this->getMediaInfo($ps_field)) {
            ...
            if ($vs_media_type = $o_media_proc_settings->canAccept($va_media_info["INPUT"]["MIMETYPE"])) {
                ...
                if (isset($va_media_type_info['VERSIONS'][$ps_version])) {
                    if ($va_rule = $o_media_proc_settings->getMediaTransformationRule(
                        $va_media_type_info['VERSIONS'][$ps_version]['RULE']
                    )) {
    ...
    public function getDefaultMediaVersion($ps_field)
    {
        if ($va_media_info = $this->getMediaInfo($ps_field)) {
            $o_media_proc_settings = new MediaProcessingSettings($this, $ps_field);
            $va_type_info = $o_media_proc_settings->getMediaTypeInfo(
                $o_media_proc_settings->canAccept($va_media_info["INPUT"]["MIMETYPE"])
            );

            return ($va_type_info['MEDIA_VIEW_DEFAULT_VERSION']) ? $va_type_info['MEDIA_VIEW_DEFAULT_VERSION'] : array_shift(
                $this->getMediaVersions($ps_field)
            );
        } else {
            return null;
        }
    ...
    public function getDefaultMediaPreviewVersion($ps_field)
    {
        if ($va_media_info = $this->getMediaInfo($ps_field)) {
            ...
            return ($va_type_info['MEDIA_PREVIEW_DEFAULT_VERSION']) ? $va_type_info['MEDIA_PREVIEW_DEFAULT_VERSION'] : array_shift(
                $this->getMediaVersions($ps_field)
            );
        } else {
            return null;
    ...
    public function _removeMedia($ps_field, $ps_version, $ps_dont_delete_path = "", $ps_dont_delete_extension = "")
    {
        ...
        if (is_array($va_volume_info["mirrors"]) && sizeof($va_volume_info["mirrors"]) > 0) {
            ...
            if (!($o_tq->cancelPendingTasksForEntity($vs_entity_key))) {
                $this->postError(
                    560,
                    _t("Could not cancel pending tasks: %1", $this->error),
            ...
            if (isset($this->_SET_FILES[$ps_field]['tmp_name'])) {
                ...
                if (
                    ...
                    try {
                        ...
                        if (is_array($r)) {
                            $this->_SET_FILES[$ps_field]['original_filename'] = !empty($r['originalFilename']) ? $r['originalFilename'] : pathinfo(
                                $r['file'],
                                PATHINFO_BASENAME
                            );
                            $vs_tmp_file = $r['file'];
                ...
                if (preg_match("!^userMedia[\d]+/!", $this->_SET_FILES[$ps_field]['tmp_name'])) {
                    ...
                    if (file_exists("{$vs_tmp_directory}/" . $this->_SET_FILES[$ps_field]['tmp_name'] . "_metadata")) {
                        if (is_array(
                            $va_tmp_metadata = json_decode(
                                file_get_contents(
                                    "{$vs_tmp_directory}/" . $this->_SET_FILES[$ps_field]['tmp_name'] . "_metadata"
                                )
                            )
                        )) {
                ...
                if (isset($this->_SET_FILES[$ps_field]['tmp_name']) && (file_exists(
                        $this->_SET_FILES[$ps_field]['tmp_name']
                    ))) {
                    if (!isset($pa_options['dont_allow_duplicate_media'])) {
                        $pa_options['dont_allow_duplicate_media'] = (bool)$this->getAppConfig()->get(
                            'dont_allow_duplicate_media'
                        );
                    }
                    if (isset($pa_options['dont_allow_duplicate_media']) && $pa_options['dont_allow_duplicate_media']) {
                        if ($this->hasField('md5')) {
                            $qr_dupe_chk = $this->getDb()->query(
                                ...
                                ) . " WHERE md5 = ? " . ($this->hasField('deleted') ? ' AND deleted = 0' : '') . "
							",
                            ...
                            if ($qr_dupe_chk->nextRow()) {
                                $this->postError(
                                    1600,
                                    _t("Media already exists in database"),
                                    "BaseModel->_processMedia()",
                                    $this->tableName() . '.' . $ps_field
                                );
                                return false;
                    ...
                    if (!isset($va_matches[1])) { // file has no extension, i.e. is probably PHP upload tmp file
                        ...
                        preg_match(
                            "/[.]*\.([a-zA-Z0-9]+)$/",
                            $this->_SET_FILES[$ps_field]['original_filename'],
                            $va_matches
                        );
                        if (strlen($va_matches[1]) > 0) {
                            $va_parts = explode("/", $this->_SET_FILES[$ps_field]['tmp_name']);
                            $vs_new_filename = sys_get_temp_dir() . "/" . $va_parts[sizeof(
                                    $va_parts
                                ) - 1] . "." . $va_matches[1];
                    ...
                    if (!$input_type = $o_media_proc_settings->canAccept($input_mimetype)) {
                        ...
                        $this->postError(
                            1600,
                            ($input_mimetype) ? _t(
                                "File type %1 not accepted by %2",
                                $input_mimetype,
                                $ps_field
                            ) : _t("Unknown file type not accepted by %1", $ps_field),
                            "BaseModel->_processMedia()",
                            $this->tableName() . '.' . $ps_field
                        );
                        set_time_limit($vn_max_execution_time);
                    ...
                    if (isset($this->_SET_FILES[$ps_field]['options']['TRANSFORMATION_HISTORY']) && is_array(
                    ...
                    if (isset($this->_SET_FILES[$ps_field]['options']['undo']) && file_exists(
                            $this->_SET_FILES[$ps_field]['options']['undo']
                        )) {
                        if ($volume = $version_info['original']['VOLUME']) {
                            $vi = $this->_MEDIA_VOLUMES->getVolumeInformation($volume);
                            if ($vi["absolutePath"] && (strlen(
                                    $dirhash = $this->_getDirectoryHash($vi["absolutePath"], $this->getPrimaryKey())
                                ))) {
                                $magic = rand(0, 99999);
                    ...
                    if (isset($pa_options['these_versions_only']) && is_array(
                            $pa_options['these_versions_only']
                        ...
                        if (sizeof($va_process_these_versions_only)) {
                            foreach (array_keys($va_tmp) as $v) {
                                if (!in_array($v, $va_process_these_versions_only) && !in_array(
                                        $v,
                                        [
                                            '_START_AT_PAGE',
                                            '_START_AT_TIME'
                    ...
                    $queue_enabled = (!sizeof($va_process_these_versions_only) && $this->getAppConfig()->get(
                            'queue_enabled'
                        )) ? true : false;
                    ...
                    foreach ($va_versions as $v) {
                        ...
                        if (sizeof($va_process_these_versions_only) && (!in_array(
                                $v,
                                $va_process_these_versions_only
                            ))) {
                        ...
                        if (isset($media_desc[$basis]) && isset($media_desc[$basis]['FILENAME'])) {
                            if (!isset($va_media_objects[$basis])) {
                                ...
                                if ($o_media->read(
                                    $p = $basis_vi['absolutePath'] . "/" . $media_desc[$basis]['HASH'] . "/" . $media_desc[$basis]['MAGIC'] . "_" . $media_desc[$basis]['FILENAME'],
                                    null,
                                    ['original_filename' => $vs_original_filename]
                                )) {
                        ...
                        if (($queue_enabled) && ($queue) && ($queue_threshold > 0) && ($queue_threshold < (int)$media_desc["INPUT"]["FILESIZE"]) && ($va_default_queue_settings['QUEUE_USING_VERSION'] != $v)) {
                            ...
                            if ($version_info[$v]["QUEUED_MESSAGE"]) {
                                ...
                                $media_desc[$v]["QUEUED_MESSAGE"] = ($va_default_queue_settings['QUEUED_MESSAGE']) ? $va_default_queue_settings['QUEUED_MESSAGE'] : _t(
                                    "Media is being processed and will be available shortly."
                                );
                            }
                        ...
                        if (sizeof($rules) == 0) {
                            ...
                            #
                            $ext = ($output_mimetype == 'application/octet-stream') ? pathinfo(
                                $this->_SET_FILES[$ps_field]['original_filename'],
                                PATHINFO_EXTENSION
                            ...
                            if (($dirhash = $this->_getDirectoryHash(
                                    $vi["absolutePath"],
                                    $this->getPrimaryKey()
                                )) === false) {
                                $this->postError(
                                    1600,
                                    _t(
                                        "Could not create subdirectory for uploaded file in %1. Please ask your administrator to check the permissions of your media directory.",
                                        $vi["absolutePath"]
                                    ),
                                    "BaseModel->_processMedia()",
                                    $this->tableName() . '.' . $ps_field
                            ...
                            if (
                                ...
                                (is_null(
                                        $skip_threshold
                                    ) && isset($version_info[$v]['REPLACE_WITH_VERSION']) && in_array(
                                        $version_info[$v]['REPLACE_WITH_VERSION'],
                                        $va_versions,
                                        true
                                    ))
                            ) {
                                ...
                                $alt_version_list = array_filter(
                                    array_keys($media_desc),
                                    function ($x) use ($v) {
                                        return ($x !== $v);
                                    }
                                );
                                $replace_with_version = caGetOption(
                                    'REPLACE_WITH_VERSION',
                                    $version_info[$v],
                                    array_pop($alt_version_list),
                                ...
                                if ((bool)$version_info[$v]["USE_EXTERNAL_URL_WHEN_AVAILABLE"]) {
                                    ...
                                    $magic = rand(0, 99999);
                                    $filepath = $vi["absolutePath"] . "/" . $dirhash . "/" . $magic . "_" . $this->_genMediaName(
                                            $ps_field
                                        ) . "_" . $v . "." . $ext;
                                    ...
                                    if (!copy($this->_SET_FILES[$ps_field]['tmp_name'], $filepath)) {
                                        $this->postError(
                                            1600,
                                            _t(
                                                "File could not be copied. Ask your administrator to check permissions and file space for %1",
                                                $vi["absolutePath"]
                                            ),
                                            "BaseModel->_processMedia()",
                                            $this->tableName() . '.' . $ps_field
                                    ...
                                    if ($pa_options['delete_old_media']) {
                                        $va_files_to_delete[] = array(
                                            'field' => $ps_field,
                                            'version' => $v,
                                            'dont_delete_path' => $vi["absolutePath"] . "/" . $dirhash . "/" . $magic . "_" . $this->_genMediaName(
                                                    $ps_field
                                                ) . "_" . $v,
                                                ...
                                                array(
                                                    ...
                                                    "FILES" => array(
                                                        array(
                                                            ...
                                                            "HASH" => $dirhash,
                                                            "FILENAME" => $magic . "_" . $this->_genMediaName(
                                                                    $ps_field
                                                                ) . "_" . $v . "." . $ext
                                                        )
                                                    ),
                            ...
                            while (list($operation, $parameters) = each($rules)) {
                                if ($operation === 'SET') {
                                    ...
                                    if (is_array($this->_FIELD_VALUES[$ps_field]) && (is_array(
                                            $va_media_center = $this->getMediaCenter($ps_field)
                                        ))) {
                            ...
                            if (!($vs_output_file = $m->write($filepath, $output_mimetype, $va_media_write_options))) {
                                $this->postError(
                                    1600,
                                    _t("Couldn't write file: %1", join("; ", $m->getErrors())),
                                    "BaseModel->_processMedia()",
                                    $this->tableName() . '.' . $ps_field
                                );
                                $m->cleanup();
                            ...
                            if ($vs_use_icon) {
                                ...
                                $media_desc[$v] = array(
                                    ...
                                    "EXTENSION" => $ext,
                                    "MD5" => md5_file(
                                        $fp = $vi["absolutePath"] . "/" . $dirhash . "/" . $magic . "_" . $this->_genMediaName(
                                                $ps_field
                                            ) . "_" . $v . "." . $ext
                                    ),
                    ...
                    if (sizeof($va_queued_versions)) {
                        $vs_entity_key = md5(
                            join(
                                "/",
                                array_merge(
                                    array($this->tableName(), $ps_field, $this->getPrimaryKey()),
                        ...
                        if (!($filename = $vs_path_to_queue_media)) {
                            ...
                            $filename = $o_tq->copyFileToQueueTmp(
                        ...
                        if ($filename) {
                            if ($o_tq->addTask(
                                ...
                                $this->postError(
                                    100,
                                    _t(
                                        "Couldn't queue processing for version '%1' using handler '%2'",
                                        !$v,
                                        $queue
                                    ),
                                    "BaseModel->_processMedia()"
                        ...
                        if (!sizeof($va_process_these_versions_only) && ((bool)$this->_CONFIG->get(
                                    'video_preview_generate_frames'
                                ) || (bool)$this->_CONFIG->get('document_preview_generate_pages')) && method_exists(
                                $this,
                                'addFile'
                            )) {
                    ...
                    if (!$error) {
                        ...
                        foreach ($va_files_to_delete as $va_file_to_delete) {
                            $this->_removeMedia(
                                $va_file_to_delete['field'],
                                $va_file_to_delete['version'],
                                $va_file_to_delete['dont_delete_path'],
                                $va_file_to_delete['dont_delete_extension']
                            );
                        }
                        ...
                        if (($vs_metadata_field_name = $o_media_proc_settings->getMetadataFieldName(
                            ...
                            $vn_embedded_media_metadata_limit = (int)$this->_CONFIG->get(
                                'dont_extract_embedded_media_metdata_when_length_exceeds'
                            );
                            if (($vn_embedded_media_metadata_limit > 0) && (strlen(
                                        $vs_serialized_metadata = caSerializeForDatabase($media_metadata, true)
                                    ) > $vn_embedded_media_metadata_limit)) {
                                $media_metadata = null;
                                $vs_serialized_metadata = '';
                            }
                            $this->set($vs_metadata_field_name, $media_metadata);
                            $vs_sql .= " " . $vs_metadata_field_name . " = " . $this->quote(
                                    $vs_serialized_metadata
                                ) . ",";
                        }
                        ...
                        if (is_array($va_locs = $m->getExtractedTextLocations())) {
                            ...
                            foreach ($va_locs as $vs_content => $va_loc_list) {
                                foreach ($va_loc_list as $va_loc) {
                                    MediaContentLocationIndexer::index(
                                        $this->tableNum(),
                                        $this->getPrimaryKey(),
                                        $vs_content,
                                        $va_loc['p'],
                        ...
                        $this->postError(
                            1600,
                            _t("File could not be processed for %1: %2", $ps_field, $error_msg),
                            "BaseModel->_processMedia()"
                        );
                        #	    return false;
                    }
                    ...
                    $vs_sql = "{$ps_field} = " . $this->quote(
                            caSerializeForDatabase($this->_FILES[$ps_field], true)
                        ) . ",";
                }
    ...
    public function applyMediaTransformation($ps_field, $ps_op, $pa_params, $pa_options = null)
    {
        ...
        $this->set(
            $ps_field,
            $vs_tmp_basename . "." . $va_media_info['original']['EXTENSION'],
            $vs_undo_path ? array(
                'undo' => $vs_undo_path,
                'TRANSFORMATION_HISTORY' => $va_transformation_history
    ...
    public function getMediaTransformationHistory($ps_field, $ps_op = null)
    {
        ...
        if ($ps_op && isset($va_media_info['TRANSFORMATION_HISTORY'][$ps_op]) && is_array(
                $va_media_info['TRANSFORMATION_HISTORY'][$ps_op]
            )) {
            return $va_media_info['TRANSFORMATION_HISTORY'][$ps_op];
        }
        return (isset($va_media_info['TRANSFORMATION_HISTORY']) && is_array(
                $va_media_info['TRANSFORMATION_HISTORY']
            )) ? $va_media_info['TRANSFORMATION_HISTORY'] : array();
    }
    # --------------------------------------------------------------------------------
    ...
    public function setMediaCenter($ps_field, $pn_center_x, $pn_center_y, $pa_options = null)
    {
        ...
        $this->set(
            'media',
            $this->getMediaPath('media', 'original'),
    ...
    public function setMediaScale($ps_field, $ps_dimension, $pn_percent_of_image_width = 1, $pa_options = null)
    {
        ...
        if ($vo_parsed_measurement && (($vn_measurement = (float)$vo_parsed_measurement->toString(4)) > 0)) {
            ...
            $va_media_info['_SCALE_UNITS'] = caGetLengthUnitType(
                $vo_parsed_measurement->getType(),
                array('short' => true)
            );
    ...
    public function _processFiles($field)
    {
        ...
        if ((isset($this->_FILES_CLEAR[$field])) && ($this->_FILES_CLEAR[$field])) {
            ...
            if ((file_exists($this->_SET_FILES[$field]['tmp_name']))) {
                $ff = new File();
                $mimetype = $ff->divineFileFormat(
                    $this->_SET_FILES[$field]['tmp_name'],
                ...
                if (is_array($va_field_info["FILE_FORMATS"]) && sizeof($va_field_info["FILE_FORMATS"]) > 0) {
                    if (!in_array($mimetype, $va_field_info["FILE_FORMATS"])) {
                        $this->postError(
                            1605,
                            _t("File is not a valid format"),
                            "BaseModel->_processFiles()",
                ...
                #
                if (isset($va_field_info["FILE_CONVERSIONS"]) && is_array(
                        $va_field_info["FILE_CONVERSIONS"]
                    ) && (sizeof($va_field_info["FILE_CONVERSIONS"]) > 0)) {
                    foreach ($va_field_info["FILE_CONVERSIONS"] as $vs_output_format) {
                        if ($va_tmp = $ff->convert(
                            $vs_output_format,
                            $filepath,
                            $filestem
    ...
    public function hasBundle($ps_bundle, $pn_type_id = null)
    {
        ...
        if ($vn_num_bits == 1) {
            ...
            } elseif (($va_bundle_bits[0] != $this->tableName()) && ($t_rel = Datamodel::getInstanceByTableName(
                    $va_bundle_bits[0],
                    true
                ))) {
    ...
    public function verifyFieldValue($field, $value, &$pb_need_reload)
    {
        ...
        if (!$va_attr) {
            $this->postError(
                716,
                _t("%1 does not exist", $field),
                "BaseModel->verifyFieldValue()",
                $this->tableName() . '.' . $field
            );
            return false;
        }
        ...
        if ($data_type == 0) {    # number; check value
            ...
            if (!($va_attr["IS_NULL"] && (!$value))) {
                if ((isset($min_value)) && ($value < $min_value)) {
                    $this->postError(
                        1101,
                        _t("%1 must not be less than %2", $va_attr["LABEL"], $min_value),
                        "BaseModel->verifyFieldValue()",
                        $this->tableName() . '.' . $field
                    );
                    return false;
                ...
                if ((isset($max_value)) && ($value > $max_value)) {
                    $this->postError(
                        1101,
                        _t("%1 must not be greater than %2", $va_attr["LABEL"], $max_value),
                        "BaseModel->verifyFieldValue()",
                        $this->tableName() . '.' . $field
                    );
                    return false;
        ...
        if (!($va_attr["IS_NULL"] && (!$value))) {
            ...
            if ((isset($min_length)) && (strlen($value) < $min_length)) {
                $this->postError(
                    1102,
                    _t("%1 must be at least %2 characters", $va_attr["LABEL"], $min_length),
                    "BaseModel->verifyFieldValue()",
                    $this->tableName() . '.' . $field
                );
                return false;
            ...
            if (isset($va_attr['LIST']) && $va_attr['LIST']) {
                ...
                if ($t_list->load(array('list_code' => $va_attr['LIST']))) {
                    ...
                    foreach ($va_items as $vn_item_id => $va_item_info) {
                        if (is_null(
                                $vs_list_default
                            ) || (isset($va_item_info['is_default']) && $va_item_info['is_default'])) {
            ...
            if ((in_array($data_type, array(FT_NUMBER, FT_TEXT))) && (isset($va_list)) && (is_array(
                    $va_list
                )) && (count($va_list) > 0)) { # string; check choice list
                    ...
                    if ($va_attr['DISPLAY_TYPE'] == DT_LIST_MULTIPLE) {
                        ...
                        foreach ($va_tmp as $vs_mult_item) {
                            if (!in_array($vs_mult_item, $va_list)) {
                                $this->postError(
                                    1103,
                                    _t("'%1' is not valid choice for %2", $v, $va_attr["LABEL"]),
                                    "BaseModel->verifyFieldValue()",
                                    $this->tableName() . '.' . $field
                                );
                                return false;
    ...
    public function getFieldInfo($field, $attribute = "")
    {
        if (isset($this->FIELDS[$field])) {
            ...
            if (isset($translations[$this->tableName()][$field]) && is_array(
                    $translations[$this->tableName()][$field]
                )) {
    ...
    public function getDisplayLabel($ps_field)
    {
        ...
        $translations = $this->_TRANSLATIONS->getAssoc('fields');
        if (isset($translations[$this->tableName()][$va_tmp[0]]) && is_array(
                $translations[$this->tableName()][$va_tmp[0]]
        ...
        if (isset($translations[$this->tableName()][$va_tmp[1]]) && is_array(
                $translations[$this->tableName()][$va_tmp[1]]
            ) && isset($translations[$this->tableName()][$va_tmp[1]]['LABEL'])) {
    ...
    public function logChange($ps_change_type, $pn_user_id = null, $pa_options = null)
    {
        ...
        if ($vb_is_metadata) {
            ...
            if (($ps_change_type !== 'D') && !caGetOption('forceLogChange', $pa_options, false) && !sizeof(
                    $this->getChangedFieldValuesArray()
                )) {
                return null;
    ...
    {
        $pa_datetime_range = (isset($pa_options['range']) && is_array(
                $pa_options['range']
        ...
        $ps_change_type = (isset($pa_options['changeType']) && in_array(
                $pa_options['changeType'],
                array('I', 'U', 'D')
        ...
        if (!$this->opqs_get_change_log) {
            ...
            if ($pb_for_table) {
                if (!($this->opqs_get_change_log = $o_db->prepare(
					...
					WHERE
						(
							(wcl.logged_table_num = " . ((int)$this->tableNum(
                    )) . ") AND " . (($ps_change_type) ? "(wcl.changetype = '" . $ps_change_type . "') AND " : "") . "
							(wcl.logged_row_id = ?)
						)
						{$vs_daterange_sql}
                ...
                if (!($this->opqs_get_change_log = $o_db->prepare(
					...
					WHERE
						 (
							(wcls.subject_table_num = " . ((int)$this->tableNum(
                    )) . ") AND " . (($ps_change_type) ? "(wcl.changetype = '" . $ps_change_type . "') AND " : "") . "
							(wcls.subject_row_id  = ?)
						)
						{$vs_daterange_sql}
        ...
        if ($pb_for_table) {
            $qr_log = $this->opqs_get_change_log->execute(
                $vs_daterange_sql ? array((int)$pn_row_id, (int)$vn_start, (int)$vn_end) : array((int)$pn_row_id)
            );
        } else {
            $qr_log = $this->opqs_get_change_log->execute(
                $vs_daterange_sql ? array(
                    (int)$pn_row_id,
                    (int)$vn_start,
                    (int)$vn_end,
                    (int)$pn_row_id,
        ...
        while ($qr_log->nextRow()) {
            ...
            $va_log[sizeof($va_log) - 1]['snapshot'] = caUnserializeForDatabase(
                $va_log[sizeof($va_log) - 1]['snapshot']
            );
        }
    ...
    public function &getHierarchy($pn_id = null, $pa_options = null)
    {
        ...
        if ($this->isHierarchical()) {
            ...
            if (!($vs_rank_fld = caGetOption('sort', $pa_options, $this->getProperty('RANK'))) || !$this->hasField(
                    $vs_rank_fld
                )) {
            ...
            $qr_root = $o_db->query(
                "
				SELECT $vs_hier_left_fld, $vs_hier_right_fld " . (($this->hasField(
                    $vs_hier_id_fld
                )) ? ", $vs_hier_id_fld" : "") . "
				FROM " . $this->tableName() . "
				WHERE
            ...
            if ($o_db->numErrors()) {
                ...
                if ($qr_root->nextRow()) {
                    $va_count = array();
                    if (($this->hasField($vs_hier_id_fld)) && (!($vn_hierarchy_id = $t_instance->get(
                            $vs_hier_id_fld
                    ...
                    if (isset($pa_options['additionalTableToJoin']) && ($pa_options['additionalTableToJoin'])) {
                        ...
                        if (isset($pa_options['additionalTableWheres']) && is_array(
                                $pa_options['additionalTableWheres']
                            )) {
                    ...
                    if ($this->hasField(
                            'deleted'
                        ) && (!isset($pa_options['returnDeleted']) || (!$pa_options['returnDeleted']))) {
                    ...
                    $vs_sql = "
						...
						{$vs_sql_joins}
						WHERE
							({$vs_table_name}.{$vs_hier_left_fld} BETWEEN " . $qr_root->get(
                            $vs_hier_left_fld
    ...
    public function getHierarchyChildCountsForIDs($pa_ids, $pa_options = null)
    {
        ...
        $qr_res = $o_db->query(
			...
			FROM {$vs_table_name}
			WHERE
				parent_id IN (?) " . ((sizeof($va_additional_table_wheres)) ? " AND " . join(
                    " AND ",
    ...
    public function &getHierarchyChildrenAsQuery($pn_id = null, $pa_options = null)
    {
        ...
        if ($ps_additional_table_to_join) {
            ...
            if (isset($pa_options['additionalTableSelectFields']) && is_array(
                    $pa_options['additionalTableSelectFields']
                )) {
        ...
        if ($this->isHierarchical()) {
            ...
            if ($ps_additional_table_to_join) {
                if (is_array(
                    ...
                    // one-many rel
                    $va_sql_joins[] = $ps_additional_table_join_type . " JOIN {$ps_additional_table_to_join} ON " . $this->tableName(
                        ) . '.' . $va_rel['one_table_field'] . " = {$ps_additional_table_to_join}." . $va_rel['many_table_field'];
                } else {
                    // TODO: handle many-many cases
                }
            ...
            if ($pb_return_child_counts) {
                $vs_additional_child_join_conditions = sizeof($va_additional_child_join_conditions) ? " AND " . join(
                        " AND ",
                        $va_additional_child_join_conditions
                    ) : "";
                $qr_hier = $o_db->query(
                    "
					SELECT " . $this->tableName() . ".* " . (sizeof($va_additional_table_select_fields) ? ', ' . join(
                            ', ',
                            $va_additional_table_select_fields
                        ) : '') . ", count(*) child_count, sum(p2." . $this->primaryKey() . ") has_children
					FROM " . $this->tableName() . "
					{$vs_sql_joins}
					LEFT JOIN " . $this->tableName(
                    ) . " AS p2 ON p2." . $vs_hier_parent_id_fld . " = " . $this->tableName() . "." . $this->primaryKey(
                    ) . " {$vs_additional_child_join_conditions}
					WHERE
						(" . $this->tableName() . ".{$vs_hier_parent_id_fld} = ?) " . ((sizeof(
                            $va_additional_table_wheres
                        ) > 0) ? ' AND ' . join(' AND ', $va_additional_table_wheres) : '') . "
					GROUP BY
    ...
    public function getHierarchySiblings($pn_id = null, $pa_options = null)
    {
        ...
        if ($qr_parent = $this->getDb()->query(
			...
			FROM " . $this->tableName() . " 
			WHERE " . join(' AND ', $va_additional_table_wheres),
            (int)$pn_id
        )) {
            if ($qr_parent->nextRow()) {
                ...
                $this->postError(
                    250,
                    _t(
                        'Could not get parent_id to load siblings by: %1',
                        join(';', $this->getDb()->getErrors())
                    ),
    ...
    public function &getHierarchyAncestors($pn_id = null, $pa_options = null)
    {
        ...
        if ($this->isHierarchical()) {
            ...
            $qr_root = $o_db->query(
				...
				{$vs_sql_joins}
				WHERE
					({$vs_table_name}." . $this->primaryKey() . " = ?) " . ((sizeof(
                        $va_additional_table_wheres
                    ) > 0) ? ' AND ' . join(' AND ', $va_additional_table_wheres) : '') . "
			",
                intval($pn_id)
                    ...
                    if ($vn_parent_id) {
                        do {
                            $vs_sql = "
								...
								FROM {$vs_table_name} 
								{$vs_sql_joins}
								WHERE ({$vs_table_name}." . $this->primaryKey() . " = ?) " . ((sizeof(
                                        $va_additional_table_wheres
                                    ) > 0) ? ' AND ' . join(' AND ', $va_additional_table_wheres) : '') . "
							";
    ...
    public function hierarchyWithTemplate($ps_template, $pa_options = null)
    {
        ...
        $va_hier = $this->getHierarchyAsList(
            $pn_id,
        ...
        $va_vals = caProcessTemplateForIDs(
            $ps_template,
            $this->tableName(),
            $va_ids,
            array_merge(
                ...
                array(
                    'indexWithIDs' => true,
                    'includeBlankValuesInArray' => true,
                    'returnAsArray' => true
                )
            )
        );
        ...
        if (is_array($pa_sort) && sizeof($pa_sort) && sizeof($va_ids)) {
            ...
            return $this->_getFlattenedHierarchyArray(
                $va_hierarchy_data,
                $va_parent_ids[$pn_id] ? $va_parent_ids[$pn_id] : null,
                $ps_sort_direction
            );
        } else {
            foreach ($va_vals as $vn_i => $vs_val) {
    ...
    private function _rebuildHierarchicalIndex($pn_parent_id, $pn_hier_left)
    {
        ...
        if ($o_db->numErrors()) {
            ...
            $qr_up = $o_db->query(
				...
				SET " . $vs_hier_left_fld . " = " . intval($pn_hier_left) . ", " . $vs_hier_right_fld . " = " . intval(
                    $vn_hier_right
                ) . "
				WHERE 
					(" . $vs_pk . " = ?)
    ...
    public function htmlFormElement($ps_field, $ps_format = null, $pa_options = null)
    {
        ...
        $vs_field_label = (isset($pa_options['label']) && (strlen(
                    $pa_options['label']
                ) > 0)) ? $pa_options['label'] : $va_attr["LABEL"];
...
// TODO: PULL THIS FROM A CONFIG FILE
        ...
        if ($va_attr) {
            ...
            if (($va_attr["DISPLAY_TYPE"] == DT_HIDDEN) || (caGetOption('hidden', $pa_options, false))) {
                return '<input type="hidden" name="' . $pa_options["name"] . '" value="' . $this->escapeHTML(
                        $vm_field_value
                    ) . '"/>';
            }
                ...
                case(FT_VARS):
                    ...
                    if ($va_attr['DISPLAY_TYPE'] == DT_COUNTRY_LIST) {
                        ...
                        if ($va_attr['STATEPROV_FIELD']) {
                            ...
                            $vs_element .= "
								jQuery('#{$ps_field}').click({countryID: '{$ps_field}', stateProvID: '" . $va_attr['STATEPROV_FIELD'] . "', value: '" . addslashes(
                                    $this->get($va_attr['STATEPROV_FIELD'])
                                ) . "', statesByCountryList: caStatesByCountryList}, caUI.utils.updateStateProvinceForCountry);
								jQuery(document).ready(function() {
									caUI.utils.updateStateProvinceForCountry({data: {countryID: '{$ps_field}', stateProvID: '" . $va_attr['STATEPROV_FIELD'] . "', value: '" . addslashes(
                                    $this->get($va_attr['STATEPROV_FIELD'])
                                ) . "', statesByCountryList: caStatesByCountryList}});
								});
							";
                    ...
                    if (($vn_display_width > 0) && (in_array(
                            $va_attr["DISPLAY_TYPE"],
                            array(DT_SELECT, DT_LIST, DT_LIST_MULTIPLE)
                        ))) {
                        #
                        ...
                        if ($vs_list_code) {
                            ...
                            if (method_exists($this, 'getTypeFieldName') && ($ps_field == $this->getTypeFieldName())) {
                                $va_limit_list = caGetTypeListForUser(
                                    $this->tableName(),
                            ...
                            if (isset($va_many_to_one_relations[$ps_field]) && $va_many_to_one_relations[$ps_field]) {
                                ...
                                #
                                $o_one_table = Datamodel::getInstance(
                                    $va_many_to_one_relations[$ps_field]["one_table"]
                                );
                                ...
                                if ($o_one_table->isHierarchical()) {
                                    ...
                                    # Hierarchical <select>
                                    #
                                    $va_hier = $o_one_table->getHierarchyAsList(
                                        0,
                                        $vs_display_use_count,
                                    ...
                                    if (sizeof($va_options) == 0) {
                                        ...
                                        foreach ($va_options as $vn_val => $va_option_info) {
                                            $vs_selected = (($vn_val == $vm_field_value) || in_array(
                                                    $vn_val,
                                                    $va_selection
                                                )) ? "selected='selected'" : "";
                                            ...
                                            if (
                                                ...
                                                (strlen($vs_option_text) + strlen(
                                                        $vs_use_count
                                                    ) + $vn_indent > $pa_options["maxOptionLength"])
                                            ) {
                                                if (($vn_strlen = $pa_options["maxOptionLength"] - strlen(
                                                            $vs_indent
                                                        ) - strlen(
                                                            $vs_use_count
                                    ...
                                    if (isset($pa_options["WHERE"]) && (is_array(
                                                $pa_options["WHERE"]
                                            ) && ($vs_where = join(" AND ", $pa_options["WHERE"]))) || ((is_array(
                                                $va_attr["DISPLAY_WHERE"]
                                            )) && ($vs_where = join(" AND ", $va_attr["DISPLAY_WHERE"])))) {
                                        $vs_sql .= " WHERE $vs_where ";
                                    }

                                    if ((isset($va_attr["DISPLAY_ORDERBY"])) && ($va_attr["DISPLAY_ORDERBY"]) && ($vs_orderby = join(
                                            ",",
                                            $va_attr["DISPLAY_ORDERBY"]
                                        ))) {
                                    ...
                                    if ($pa_options["select_item_text"]) {
                                        $va_opts[$pa_options["select_item_text"]] = array(
                                            $pa_options["select_item_text"],
                                            null
                                        );
                                    }
                                    ...
                                    if (sizeof($va_opts) == 0) {
                                        ...
                                        if (isset($pa_options['hide_select_if_only_one_option']) && $pa_options['hide_select_if_only_one_option'] && (sizeof(
                                                    $va_opts
                                                ) == 1)) {
                                            ...
                                            foreach ($va_opts as $va_opt) {
                                                ...
                                                $vs_value = $va_opt[1];
                                                $vs_selected = (($vs_value == $vm_field_value) || in_array(
                                                        $vs_value,
                                                        $va_selection
                                                    )) ? "selected='selected'" : "";
                                                ...
                                                if (
                                                    ...
                                                    $vs_option_text = mb_substr(
                                                            $vs_option_text,
                                                            0,
                                                            $pa_options["maxOptionLength"] - 3 - strlen(
                                                                $vs_use_count
                                                            )
                                                        ) . "...";
                                                }
                                ...
                                if (!$vs_element && (isset($va_attr["BOUNDS_CHOICE_LIST"]) && is_array(
                                            $va_attr["BOUNDS_CHOICE_LIST"]
                                        ))) {
                                    if (sizeof($va_attr["BOUNDS_CHOICE_LIST"]) == 0) {
                                        ...
                                        if ($pa_options["select_item_text"]) {
                                            $vs_element .= "<option value=''>" . $this->escapeHTML(
                                                    $pa_options["select_item_text"]
                                                ) . "</option>\n";
                                        }
                                        if (!$pa_options["nullOption"] && $vb_is_null) {
                                        ...
                                        foreach ($va_attr["BOUNDS_CHOICE_LIST"] as $vs_option => $vs_value) {
                                            $vs_selected = ((strval($vs_value) === strval($vm_field_value)) || in_array(
                                                    $vs_value,
                                                    $va_selection
                                                )) ? "selected='selected'" : "";

                                            if (($pa_options["maxOptionLength"]) && (strlen(
                                                        $vs_option
                                                    ) > $pa_options["maxOptionLength"])) {
                                                $vs_option = mb_substr(
                                                        $vs_option,
                                            ...
                                            $vs_element .= "<option value='$vs_value' $vs_selected>" . $this->escapeHTML(
                                                    $vs_option
                                                ) . "</option>\n";
                                        }
                                        $vs_element .= "</select>\n";
                        ...
                        if ($va_attr["DISPLAY_TYPE"] === DT_COLORPICKER) {        // COLORPICKER
                            $vs_element = '<input name="' . $pa_options["name"] . '" type="hidden" size="' . ($pa_options['size'] ? $pa_options['size'] : $vn_display_width) . '" value="' . $this->escapeHTML(
                                    $vm_field_value
                                ) . '" ' . $vs_js . ' id=\'' . $pa_options["id"] . "' style='{$vs_dim_style}'/>\n";
                            ...
                            if ($vn_display_height > 1) {
                                $vs_element = '<' . $vs_text_area_tag_name . ' name="' . $pa_options["name"] . '" rows="' . $vn_display_height . '" cols="' . $vn_display_width . '"' . ($pa_options['readonly'] ? ' readonly="readonly" disabled="disabled"' : '') . ' wrap="soft" ' . $vs_js . ' id=\'' . $pa_options["id"] . "' style='{$vs_dim_style}' " . $vs_css_class_attr . ">" . $this->escapeHTML(
                                        $vm_field_value
                                ...
                                $vs_element = '<input name="' . $pa_options["name"] . '" type="text" size="' . ($pa_options['size'] ? $pa_options['size'] : $vn_display_width) . '"' . ($pa_options['readonly'] ? ' readonly="readonly" ' : '') . ' value="' . $this->escapeHTML(
                                        $vm_field_value
                                    ) . '" ' . $vs_js . ' id=\'' . $pa_options["id"] . "' {$vs_css_class_attr} style='{$vs_dim_style}'/>\n";
                            }
		...
		alreadyInUseMessage;: '" . addslashes(_t('Value must be unique. Please try another.')) . "';
                                ...
                                if ((isset($va_attr['LOOKUP']) && ($va_attr['LOOKUP'])) || $pa_options['lookup_url']) {
                                    if ((class_exists("AppController")) && ($app = AppController::getInstance(
                                        ...
                                        $vs_element .= "<script type='text/javascript'>
	jQuery('#" . $pa_options["id"] . "').autocomplete({ source: '" . ($pa_options['lookup_url'] ? $pa_options['lookup_url'] : caNavUrl(
                                                $req,
                                                'lookup',
                                                'Intrinsic',
                                                'Get',
                                                array('bundle' => $this->tableName() . ".{$ps_field}", "max" => 500)
                ...
                case (FT_HISTORIC_DATE):
                    ...
                    switch ($va_attr["DISPLAY_TYPE"]) {
                        ...
                        default:
                            ...
                            if ($vn_display_height > 1) {
                                $vs_element = '<' . $vs_text_area_tag_name . ' name="' . $pa_options["name"] . '" rows="' . $vn_display_height . '" cols="' . $vn_display_width . '" wrap="soft" ' . $vs_js . ' ' . $vs_css_class_attr . " style='{$vs_dim_style}'" . ($pa_options['readonly'] ? ' readonly="readonly" ' : '') . ">" . $this->escapeHTML(
                                        $vm_field_value
                                    ) . '</' . $vs_text_area_tag_name . '>';
                            } else {
                                $vs_element = '<input type="text" name="' . $pa_options["name"] . '" value="' . $this->escapeHTML(
                                        $vm_field_value
                                    ) . "\" size='{$vn_display_width}' {$vs_max_length} {$vs_js} {$vs_css_class_attr} style='{$vs_dim_style}'" . ($pa_options['readonly'] ? ' readonly="readonly" ' : '') . "/>";
                            }
                            break;
                    }
                    break;
                ...
                case(FT_TIME):
                    ...
                    switch ($va_attr["DISPLAY_TYPE"]) {
                        ...
                        default:
                            ...
                            if ($vn_display_height > 1) {
                                $vs_element = '<' . $vs_text_area_tag_name . ' name="' . $pa_options["name"] . '" id="' . $pa_options["id"] . '" rows="' . $vn_display_height . '" cols="' . $vn_display_width . '" wrap="soft" ' . $vs_js . ' ' . $vs_css_class_attr . " style='{$vs_dim_style}'" . ($pa_options['readonly'] ? ' readonly="readonly" ' : '') . ">" . $this->escapeHTML(
                                        $vm_field_value
                                    ) . '</' . $vs_text_area_tag_name . '>';
                            } else {
                                $vs_element = '<input type="text" name="' . $pa_options["name"] . '" id="' . $pa_options["id"] . '" value="' . $this->escapeHTML(
                                        $vm_field_value
                                    ) . "\" size='{$vn_display_width}' {$vs_max_length} {$vs_js} {$vs_css_class_attr}' style='{$vs_dim_style}'" . ($pa_options['readonly'] ? ' readonly="readonly" ' : '') . "/>";
                            }
                            break;
                    ...
                    switch ($va_attr["DISPLAY_TYPE"]) {
                        ...
                        default:
                            ...
                            if ($vn_display_height > 1) {
                                ...
                                $vs_element = '<input type="text" name="' . $pa_options["name"] . '" id="' . $pa_options["id"] . '" value="' . $this->escapeHTML(
                                        $vm_field_value
                                    ) . "\" size='{$vn_display_width}' {$vn_max_length} {$vs_js} {$vs_css_class_attr} style='{$vs_dim_style}'" . ($pa_options['readonly'] ? ' readonly="readonly" ' : '') . "/>";
                            }
                            break;
                    }
                    break;
                ...
                case(FT_FILE):
                    ...
                    $vs_element = '<div class="formLabelUploadSizeNote"><input type="file" name="' . $pa_options["name"] . '" id="' . $pa_options["id"] . '" ' . $vs_js . '/>' . _t(
                            "Maximum upload size is ${post_max_size}"
                        ) . '</div>';

                    // show current media icon
                    if ($vs_version = (array_key_exists(
                        'displayMediaVersion',
                        $pa_options
                ...
                case(FT_PASSWORD):
                    ...
                    $vs_element = '<input type="password" name="' . $pa_options["name"] . '" id="' . $pa_options["id"] . '" value="' . $this->escapeHTML(
                            $vm_field_value
                        ) . '" size="' . $vn_display_width . '" ' . $vs_max_length . ' ' . $vs_js . ' autocomplete="off" ' . $vs_css_class_attr . " style='{$vs_dim_style}'" . ($pa_options['readonly'] ? ' readonly="readonly" ' : '') . " " . ($pa_options['placeholder'] ? "placeholder='" . htmlentities(
                                $pa_options['placeholder']
                            ) . "'" : "") . "/>";
                    break;
                # ----------------------------
                case(FT_BIT):
                    switch ($va_attr["DISPLAY_TYPE"]) {
                        ...
                        case (DT_SELECT):
                            ...
                            foreach (array("Yes" => 1, "No" => 0) as $vs_option => $vs_value) {
                                ...
                                $vs_element .= "<option value='$vs_value' {$vs_selected}" . ($pa_options['readonly'] ? ' disabled="disabled" ' : '') . ">" . _t(
                                        $vs_option
                                    ) . "</option>\n";
                            }
                            $vs_element .= "</select>\n";
                ...
                if (
                    ...
                    if (preg_match("/\^DESCRIPTION/", $ps_formatted_element)) {
                        ...
                        $ps_formatted_element = str_replace(
                            "^DESCRIPTION",
                            ((isset($pa_options["description"]) && $pa_options["description"]) ? $pa_options["description"] : $va_attr["DESCRIPTION"]),
                        ...
                        $ps_formatted_element = str_replace(
                            "^LABEL",
                            '<span id="' . $vs_field_id . '">' . $vs_field_label . '</span>',
                            $ps_formatted_element
                        );
                        ...
                        if (!isset($pa_options['no_tooltips']) || !$pa_options['no_tooltips']) {
                            TooltipManager::add(
                                '#' . $vs_field_id,
                                "<h3>{$vs_field_label}</h3>" . ((isset($pa_options["description"]) && $pa_options["description"]) ? $pa_options["description"] : $va_attr["DESCRIPTION"]),
    ...
    {
        $t_list = new ca_lists();
        return caExtractValuesByUserLocale(
            $t_list->getItemsForList($ps_list_code ? $ps_list_code : $this->getFieldListCode($ps_field))
        );
    }
    # --------------------------------------------------------------------------------------------
    ...
    public function addRelationship(
        ...
        if (!($va_rel_info = $this->_getRelationshipInfo($pm_rel_table_name_or_num))) {
            $this->postError(
                1240,
                _t('Related table specification "%1" is not valid', $pm_rel_table_name_or_num),
        ...
        if ($pm_type_id && !is_numeric($pm_type_id)) {
            ...
            if ($vs_linking_table = $t_rel_type->getRelationshipTypeTable(
                $this->tableName(),
        ...
        if (!is_numeric($pn_rel_id)) {
            if ($t_rel_item = Datamodel::getInstanceByTableName($va_rel_info['related_table_name'], true)) {
                ...
                if (($vs_idno_fld = $t_rel_item->getProperty('ID_NUMBERING_ID_FIELD')) && $t_rel_item->load(
                        array($vs_idno_fld => $pn_rel_id)
                    )) {
        ...
        if ((!isset($pa_options['allowDuplicates']) || !$pa_options['allowDuplicates']) && !$this->getAppConfig()->get(
                'allow_duplicate_relationships'
            ...
            if (isset($pa_options['setErrorOnDuplicate']) && $pa_options['setErrorOnDuplicate']) {
                $this->postError(
                    1100,
                    _t('Relationship already exists'),
                    'BaseModel->addRelationship',
        ...
        if ($va_rel_info['related_table_name'] == $this->tableName()) {
            ...
            if ($pn_rel_id === $this->getPrimaryKey()) {
                $this->postError(
                    1100,
                    _t('Cannot relate a record to itself'),
                    'BaseModel->addRelationship',
                    $va_rel_info['related_table_name']
                );
                return false;
            ...
            switch (sizeof($va_rel_info['path'])) {
                ...
                default:
                    $this->postError(
                        280,
                        _t('Could not find a path to the specified related table'),
                        'BaseModel->addRelationship',
    ...
    public function editRelationship(
        ...
        if ((!isset($pa_options['allowDuplicates']) || !$pa_options['allowDuplicates']) && !$this->getAppConfig()->get(
                ...
                $ps_effective_date,
                $ps_direction,
                array('relation_id' => $pn_relation_id)
            )) {
            if (isset($pa_options['setErrorOnDuplicate']) && $pa_options['setErrorOnDuplicate']) {
                $this->postError(
                    1100,
        ...
        if ($pa_type_ids = caGetOption('restrictToTypes', $pa_options, null)) {
            ...
            if (is_array($pa_type_ids) && (sizeof($pa_type_ids) > 0)) {
                if ($t_item_rel->tableName() == $this->getSelfRelationTableName()) {
                    $vs_join_sql = "INNER JOIN " . $this->tableName() . " AS t1 ON t1." . $this->primaryKey(
                        ) . " = r." . $t_item_rel->getLeftTableFieldName() . "\n" .
                        "INNER JOIN " . $this->tableName() . " AS t2 ON t2." . $this->primaryKey(
                        ) . " = r." . $t_item_rel->getRightTableFieldName() . "\n";
    ...
    public function copyRelationships($pm_rel_table_name_or_num, $pn_to_id, $pa_options = null)
    {
        ...
        $vb_copy_attributes = caGetOption(
                'copyAttributes',
                $pa_options,
                false,
                array('castTo' => 'boolean')
    ...
    public function relationshipExists(
        ...
        if (($t_item_rel = $va_rel_info['t_item_rel']) && method_exists($t_item_rel, 'getLeftTableName')) {
            ...
            if ($ps_effective_date && $t_item_rel->hasField(
                    'effective_date'
                ) && ($va_timestamps = caDateToHistoricTimestamps($ps_effective_date))) {
        ...
        if (is_array($many_to_many_relations)) {
            ...
            foreach ($many_to_many_relations as $rel_info) {
                ...
                if (is_array($restrict_to_relationship_types)) {
                    $restrict_to_relationship_type_ids = caMakeRelationshipTypeIDList(
                        $table,
    ...
    public function changeTagAccess($pn_relation_id, $pn_access = 0, $pn_moderator = null, $pn_user_id = null)
    {
        ...
        if (
            ...
            $this->postError(
                2810,
                _t('Tag is not part of the current row'),
                'BaseModel->changeTagAccess()',
                'ca_item_tags'
            );
            return false;
        }
        ...
        if ($pn_user_id) {
            if ($t_ixt->get('user_id') != $pn_user_id) {
                $this->postError(
                    2820,
                    _t('Tag was not created by specified user'),
                    'BaseModel->changeTagAccess()',
                    'ca_item_tags'
                );
                return false;
    ...
    public function editComment(
        ...
        if (
            ...
            $this->postError(
                2810,
                _t('Comment is not part of the current row'),
                'BaseModel->editComment()',
                'ca_item_comments'
            );
            return false;
        }
        ...
        if (!$force) {
            ...
            if ($pn_user_id) {
                if ($t_comment->get('user_id') != $pn_user_id) {
                    $this->postError(
                        2820,
                        _t('Comment was not created by specified user'),
                        'BaseModel->removeComment()',
                        'ca_item_comments'
                    );
                    return false;
    ...
    public function getComments($pn_user_id = null, $pb_moderation_status = null, $pa_options = null)
    {
        ...
        switch ($vs_return_as) {
            ...
            case 'modelInstances':
            case 'firstModelInstance':
                $va_ids = ($qr_comments->getAllFieldValues('comment_id') + $qr_set_comments->getAllFieldValues(
                        'comment_id'
                    ));
                if ($vs_return_as === 'ids') {
                    return $va_ids;
            ...
            default:
                ...
                while ($qr_comments->nextRow()) {
                    ...
                    $va_comments[$comment_id]['created_on_display'] = caGetLocalizedDate(
                        $va_comments[$comment_id]['created_on']
                    );
                    if ($va_comments[$comment_id]['user_id']) {
                        $va_comments[$comment_id]['name'] = trim(
                            $va_comments[$comment_id]['fname'] . ' ' . $va_comments[$comment_id]['lname']
                        );
                ...
                while ($qr_set_comments->nextRow()) {
                    ...
                    if ($o_request) {
                        $va_comments[$comment_id]['set_message'] = _t(
                            "Comment made in set %1",
                            caEditorLink(
                                $o_request,
                                $qr_set_comments->get('set_name'),
                                '',
                                'ca_sets',
        ...
        if (caGetOption(
                ...
                $pa_options,
                'ASC',
                ['validValues' => ['ASC', 'DESC'], 'forceToUppercase' => true]
            ) === 'DESC') {
    ...
    public function getHighestRated($pb_moderation_status = true, $pn_num_to_return = 1, $va_access_values = array())
    {
        ...
        if (isset($va_access_values) && is_array($va_access_values) && sizeof($va_access_values) && $this->hasField(
                'access'
            )) {
    ...
    public function registerItemView()
    {
        ...
        $this->getDb()->query(
            "UPDATE " . $this->tableName() . " SET view_count = view_count + 1 WHERE " . $this->primaryKey() . " = ?",
            array($vn_row_id)
        );
    ...
    public function clearItemViewCount()
    {
        ...
        $this->getDb()->query(
            "UPDATE " . $this->tableName() . " SET view_count = 0 WHERE " . $this->primaryKey() . " = ?",
            array($vn_row_id)
        );
    ...
    public function getMostViewedItems($pn_limit = 10, $pa_options = null)
    {
        ...
        if (method_exists($this, 'getTypeFieldName') && ($vs_type_field_name = $this->getTypeFieldName())) {
            ...
            if (is_array($va_type_ids) && sizeof($va_type_ids)) {
                $va_wheres[] = "(t.{$vs_type_field_name} IN (" . join(',', $va_type_ids) . ')' . ($this->getFieldInfo(
                        $vs_type_field_name,
                        'IS_NULL'
        ...
        $vs_join_sql = '';
        if (isset($pa_options['hasRepresentations']) && $pa_options['hasRepresentations'] && ($this->tableName(
                ) == 'ca_objects')) {
            ...
            if ($qr_res = caMakeSearchResult($vs_table_name, $va_recently_viewed_items[$vn_table_num])) {
                ...
                while ($qr_res->nextHit()) {
                    ...
                    if (isset($pa_options['hasRepresentations']) && $pa_options['hasRepresentations']) {
                        if (!($va_reps = $qr_res->get(
                                'ca_object_representations.representation_id',
                                array('returnAsArray' => true)
                            )) || !is_array($va_reps) || !sizeof($va_reps)) {
                            continue;
                        }
                    }
                    if (is_array($pa_options['checkAccess']) && sizeof($pa_options['checkAccess']) && ($this->hasField(
                            'access'
                    ...
                    if (($vs_type_field_name) && (is_array($va_type_ids) && sizeof($va_type_ids))) {
                        ...
                        if (!in_array($vn_type_id, $va_type_ids) || (is_null($vn_type_id) && !$this->getFieldInfo(
                                    $vs_type_field_name,
                                    'IS_NULL'
                                ))) {
                            continue;
                    ...
                    if (($vs_source_field_name) && (is_array($va_source_ids) && sizeof($va_source_ids))) {
                        ...
                        if (!in_array($vn_source_id, $va_source_ids) || (is_null($vn_source_id) && !$this->getFieldInfo(
                                    $vs_source_field_name,
                                    'IS_NULL'
                                ))) {
                            continue;
                        }
    ...
    public function getRandomItems($pn_limit = 10, $pa_options = null)
    {
        ...
        if (method_exists($this, 'getTypeFieldName') && ($vs_type_field_name = $this->getTypeFieldName())) {
            $va_type_ids = caMergeTypeRestrictionLists($this, $pa_options);
            if (is_array($va_type_ids) && sizeof($va_type_ids)) {
                $va_wheres[] = "({$vs_table_name}.{$vs_type_field_name} IN (" . join(
                        ',',
                        $va_type_ids
                    ) . ')' . ($this->getFieldInfo(
                        $vs_type_field_name,
        ...
        if (method_exists($this, 'getSourceFieldName') && ($vs_source_id_field_name = $this->getSourceFieldName())) {
            $va_source_ids = caMergeSourceRestrictionLists($this, $pa_options);
            if (is_array($va_source_ids) && sizeof($va_source_ids)) {
                $va_wheres[] = $vs_table_name . '.' . $vs_source_id_field_name . ' IN (' . join(
                        ',',
                        $va_source_ids
                    ) . ')';
            }
    ...
    {
        $o_trans = caGetOption('transaction', $pa_options, null);
        if (($t_instance = Datamodel::getInstance(static::class, true)) && ($vs_idno_fld = $t_instance->getProperty(
                'ID_NUMBERING_ID_FIELD'
            ))) {
            $o_db = $o_trans ? $o_trans->getDb() : new Db();
            $qr_res = $o_db->query(
                "SELECT {$vs_idno_fld} FROM " . $t_instance->tableName() . " WHERE " . $t_instance->primaryKey(
                ) . " = ?",
                [(int)$pn_id]
            );
            ...
            if ($qr_res->nextRow()) {
                if ((is_array($pa_check_access) && (sizeof($pa_check_access) > 0)) && $t_instance->hasField(
                        'access'
    ...
    public static function getIDForIdno($ps_idno, $pa_options = null)
    {
        ...
        if (($t_instance = Datamodel::getInstance(static::class, true)) && ($vs_idno_fld = $t_instance->getProperty(
            ...
            $vs_pk = $t_instance->primaryKey();
            $qr_res = $o_db->query(
                "SELECT {$vs_pk} FROM " . $t_instance->tableName() . " WHERE {$vs_idno_fld} = ?",
                [$ps_idno]
            );
    ...
    public static function find($pa_values, $pa_options = null)
    {
        ...
        if ($va_restrict_to_types = caGetOption('restrictToTypes', $pa_options, null)) {
            ...
            if (is_array(
                    $va_restrict_to_types = caMakeTypeIDList(
                        $vs_table,
                            ...
                            if (is_array($va_field_values = $pa_values['type_id'])) {
                                foreach ($va_field_values as $vn_i => $va_field_value) {
                                    ...
                                    if (!is_numeric($vm_value)) {
                                        ...
                                        if ($va_types = caMakeRelationshipTypeIDList(
        ...
        $vs_sql = "SELECT * FROM {$vs_table} " . ((sizeof($va_sql) > 0) ? " WHERE (" . join(
                    " AND ",
                    $va_sql
                ) . ")" : "");
        ...
        $qr_res = $o_db->query(
            $vs_sql . ($limit_sql ? " LIMIT {$limit_sql}" : ''),
    ...
    public static function exists($pm_id, $pa_options = null)
    {
        ...
        if (is_numeric($pm_id) && $pm_id > 0) {
            $vn_c = self::find(
                [Datamodel::primaryKey(get_called_class()) => $pm_id],
        ...
        if (!caGetOption('idOnly', $pa_options, false) && ($vs_idno_fld = Datamodel::getTableProperty(
                get_called_class(),
                'ID_NUMBERING_ID_FIELD'
            ))) {
    ...
    public function addNotification($pn_type, $ps_message, $pb_system = false, array $pa_options = [])
    {
        ...
        if ($this->getPrimaryKey() > 0) {
            ...
            if ((!defined("__CA_QUEUE_ENABLED__") || !__CA_QUEUE_ENABLED__) && (bool)$vb_send_email && $this->hasField(
                    'email'
                ...
                if (caSendMessageUsingView(
                    ...
                    $vs_to_email,
                    $vs_sender_email,
                    $this->getAppConfig()->get('notification_email_subject'),
                    "notification.tpl",
                    ['notification' => $ps_message, 'sent_on' => time()],
                    null,
                    null,
                    ['source' => 'Notification']
                )) {
        ...
        if (is_array($va_additional_subjects)) {
            foreach ($va_additional_subjects as $va_subject) {
                ...
                // Send email immediately when queue is not enabled
                if ((!defined(
                            "__CA_QUEUE_ENABLED__"
                        ) || !__CA_QUEUE_ENABLED__) && (bool)$vb_send_email && $t_instance->hasField(
                        'email'
                    ) && ($t_instance->load($va_subject['row_id'])) && ($vs_to_email = $t_instance->get('email'))) {
                    if (caSendMessageUsingView(
                        null,
                        $vs_to_email,
                        $vs_sender_email,
    ...
    public function getNotifications(array $pa_options = [])
    {
        ...
        while ($qr_notifications->nextRow()) {
            ...
            $va_row['delivery_email_sent_on_display'] = $va_row['delivery_email_sent_on'] ? caGetLocalizedDate(
                $va_row['delivery_email_sent_on']
            ) : null;
    ...
    public function getRelatedItems($pm_rel_table_name_or_num, $pa_options = null)
    {
        ...
        $va_primary_ids = (isset($pa_options['primaryIDs']) && is_array(
                $pa_options['primaryIDs']
            )) ? $pa_options['primaryIDs'] : null;


        $va_get_where = (isset($pa_options['where']) && is_array($pa_options['where']) && sizeof(
                $pa_options['where']
            )) ? $pa_options['where'] : null;

        $va_row_ids = (isset($pa_options['row_ids']) && is_array(
                $pa_options['row_ids']
        ...
        if (is_array($va_sort_fields) && sizeof($va_rels)) {
            ...
            foreach ($va_sort_fields as $vn_x => $vs_sort_field) {
                if ($vs_sort_field == 'relation_id') { // sort by relationship primary key
                    if ($t_item_rel) {
                        $va_sort_fields[$vn_x] = $vs_sort_field = $t_item_rel->tableName(
                            ) . '.' . $t_item_rel->primaryKey();
                    }
                    continue;
                }
                    ...
                    while ($qr_rel->nextHit()) {
                        ...
                        foreach ($va_rels as $vn_rel_id => $va_rel) {
                            if ($va_rel[$vs_rel_pk] == $vn_pk_val) {
                                $va_rels[$vn_rel_id][$vs_key] = $qr_rel->get(
                                    $vs_sort_field,
