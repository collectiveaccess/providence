...
class BaseModel extends BaseObject {
	...
	/**
	 * The current locale. Used to determine which set of localized error messages to use. Default is US English
	 * ("en_us")
	 *
	 * @access private
	 */
	...
	public function __construct( $pn_id = null, $pb_use_cache = true ) {
		...
		$this->opb_purify_input = strlen( $this->_CONFIG->get( "purify_all_text_input" ) )
			? (bool) $this->_CONFIG->get( "purify_all_text_input" ) : true;
	 ...
	 *
	 * @param bool $pb_purify If true, all input passed via BaseModel::set() is run through HTML Purifier prior to
	 *                        being stored. If false, input is stored as-is. If null or omitted, no change is made to
	 *                        current setting
	 *
	 * @return bool Returns current purification settings. If you simply want to get the current setting call
	 *              BaseModel::purify without any parameters.
	 */
	 ...
	 *
	 * @param bool $pb_log_changes If true, changes will be recorded in the change log. By default changes are logged
	 *                             unless explicitly set not to. If omitted then the current state of logging is
	 *                             returned.
	 *
	...
	public function didChange( $ps_field ) {
		return isset( $this->_FIELD_VALUE_DID_CHANGE[ $ps_field ] ) ? $this->_FIELD_VALUE_DID_CHANGE[ $ps_field ]
			: null;
	}
	# --------------------------------------------------------------------------------
	 ...
	 *                           It should be an associative array of boolean (except one of the options) flags. In
	 *                           case that some of the options are not set, they are treated as 'false'. Possible
	 *                           options (keys) are: BINARY: return field value as is FILTER_HTML_SPECIAL_CHARS:
	 *                           convert all applicable chars to their html entities DONT_PROCESS_GLOSSARY_TAGS: ?
	 *                           CONVERT_HTML_BREAKS: similar to nl2br() convertLineBreaks: same as CONVERT_HTML_BREAKS
	 *                           getDirectDate: return raw date value from database if $ps_field adresses a date field,
	 *                           otherwise the value will be parsed using the TimeExpressionParser::getText() method
	 *                           getDirectTime: return raw time value from database if $ps_field adresses a time field,
	 *                           otherwise the value will be parsed using the TimeExpressionParser::getText() method
	 *                           TIMECODE_FORMAT: set return format for fields representing time ranges possible
	 *                           (string) values: COLON_DELIMITED, HOURS_MINUTES_SECONDS, RAW; data will be passed through floatval() by default QUOTE: set return value into quotes URL_ENCODE: value will be passed through urlencode() ESCAPE_FOR_XML: convert <, >, &, ' and " characters for XML use DONT_STRIP_SLASHES: if set to true, return value will not be passed through stripslashes() template: formatting string to use for returned value; ^<fieldname> placeholder is used to represent field value in template returnAsArray: if true, fields that can return multiple values [currently only <table_name>.children.<field>] will return values in an indexed array; default is false returnAllLocales: delimiter: if set, value is used as delimiter when fields that can return multiple fields are returned as strings; default is a single space convertCodesToDisplayText: if set, id values refering to foreign keys are returned as preferred label text in the current locale returnURL: if set then url is returned for media, otherwise an HTML tag for display is returned dateFormat: format to return created or lastModified dates in. Valid values are iso8601 or text checkAccess = array of access values to filter results by; if defined only items with the specified access code(s) are returned. Only supported for table that have an "access" field. sort = optional field to sort returned values on. The field specifiers are fields with or without table name specified. sort_direction = direction to sort results by, either 'asc' for ascending order or 'desc' for descending order; default is 'asc'
	 */
	public function get( $ps_field, $pa_options = null ) {
		...
		if (
			...
			$va_data = ( in_array( 'lastModified', $va_tmp, true ) ) ? $this->getLastChangeTimestamp()
				...
				switch ( sizeof( $va_tmp ) ) {
					...
					default: // > 2 elements
						...
						if ( ( $va_field_info['FIELD_TYPE'] === FT_MEDIA )
						     && ( ! isset( $pa_options['returnAsArray'] ) )
						     && ! $pa_options['returnAsArray']
						) {
						...
						if ( ( $va_tmp[1] == 'parent' ) && ( $this->isHierarchical() )
						     && ( $vn_parent_id
								= $this->get( $this->getProperty( 'HIERARCHY_PARENT_ID_FLD' ) ) )
						) {
							...
							if ( ( $va_tmp[1] == 'children' ) && ( $this->isHierarchical() ) ) {
								$vb_check_access = is_array( $pa_options['checkAccess'] )
								                   && $this->hasField( 'access' );
								...
								$vs_sort_direction = ( isset( $pa_options['sort_direction'] )
								...
								if ( is_array( $va_children_ids ) && sizeof( $va_children_ids ) ) {
									...
									if ( ( $va_tmp[1] == $this->primaryKey() ) && ! $vs_sort ) {
										...
										if ( method_exists( $this, "makeSearchResult" ) ) {
											...
											$qr_children = $this->makeSearchResult( $this->tableName(),
												$va_children_ids );
											while ( $qr_children->nextHit() ) {
												if ( $vb_check_access
												     && ! in_array( $qr_children->get( "{$vs_table}.access" ),
												...
												if ( $vb_return_as_array ) {
													$va_data[ $vs_sort_key ][ $vn_child_id ]
														= array_shift( $qr_children->get( $vs_childless_path,
														array_merge( $pa_options, array(
															'returnAsArray'    => $vb_return_as_array,
															'returnAllLocales' => $vb_return_all_locales
														) ) ) );
												} else {
													$va_data[ $vs_sort_key ][ $vn_child_id ]
														= $qr_children->get( $vs_childless_path,
														array_merge( $pa_options, array(
															'returnAsArray'    => false,
											...
											foreach ( $va_children_ids as $vn_child_id ) {
												if ( $t_instance->load( $vn_child_id ) ) {
													if ( $vb_check_access
													     && ! in_array( $t_instance->get( "access" ),
															$pa_options['checkAccess'] )
													) {
														continue;
													}
													...
													if ( $vb_return_as_array ) {
														$va_data[ $vs_sort_key ][ $vn_child_id ]
															= array_shift( $t_instance->get( $vs_childless_path,
															array_merge( $pa_options, array(
																'returnAsArray'    => $vb_return_as_array,
																'returnAllLocales' => $vb_return_all_locales
															) ) ) );
														...
														$va_data[ $vs_sort_key ][ $vn_child_id ]
															= $t_instance->get( $vs_childless_path,
															array_merge( $pa_options, array(
																'returnAsArray'    => false,
																'returnAllLocales' => false
															) ) );
			...
			switch ( $ps_field_type ) {
				case ( FT_BIT ):
					...
					if ( isset( $pa_options['convertCodesToDisplayText'] )
					     && $pa_options['convertCodesToDisplayText']
					) {
				...
				case ( FT_PASSWORD ):
					...
					if ( isset( $pa_options["FILTER_HTML_SPECIAL_CHARS"] )
					...
					if ( $pb_convert_to_display_text ) {
						if ( $vs_list_code = $this->getFieldInfo( $ps_field, "LIST_CODE" ) ) {
							...
							$t_list = new ca_lists();
							if ( ! ( $vs_tmp = $t_list->getItemFromListForDisplayByItemValue( $vs_list_code,
								$vs_prop ) )
							) {
				...
				case ( FT_DATE ):
					...
					if ( $vb_return_with_structure ) {
						$vs_prop = array(
							'start' => $this->_FIELD_VALUES[ $ps_field ],
							'end'   => $this->_FIELD_VALUES[ $ps_field ]
						);
					} elseif ( caGetOption( 'GET_DIRECT_DATE', $pa_options, false )
					           || caGetOption( 'getDirectDate', $pa_options, false )
					           || caGetOption( 'rawDate', $pa_options, false )
					) {
				...
				case ( FT_TIME ):
					...
					} elseif ( caGetOption( 'GET_DIRECT_TIME', $pa_options, false )
					           || caGetOption( 'getDirectTime', $pa_options, false )
					           || caGetOption( 'rawTime', $pa_options, false )
					) {
						$vs_prop = $this->_FIELD_VALUES[ $ps_field ];
					} else {
						$o_tep        = new TimeExpressionParser();
						$vn_timestamp = isset( $this->_FIELD_VALUES[ $ps_field ] ) ? $this->_FIELD_VALUES[ $ps_field ]
							: 0;
				...
				case ( FT_HISTORIC_DATERANGE ):
					...
					$vn_start_date = isset( $this->_FIELD_VALUES[ $vs_start_field_name ] )
						? $this->_FIELD_VALUES[ $vs_start_field_name ] : null;
					$vn_end_date   = isset( $this->_FIELD_VALUES[ $vs_end_field_name ] )
				...
				case ( FT_TIMECODE ):
					...
					$vs_prop = $o_tp->getText( isset( $pa_options["TIMECODE_FORMAT"] ) ? $pa_options["TIMECODE_FORMAT"]
						: null );
					break;
				case ( FT_MEDIA ):
				case ( FT_FILE ):
					if ( $vb_return_with_structure || $pa_options["USE_MEDIA_FIELD_VALUES"] ) {
						if ( isset( $pa_options["USE_MEDIA_FIELD_VALUES"] ) && $pa_options["USE_MEDIA_FIELD_VALUES"] ) {
							...
							$vs_prop = ( isset( $this->_SET_FILES[ $ps_field ]['tmp_name'] )
							             && $this->_SET_FILES[ $ps_field ]['tmp_name'] )
								? $this->_SET_FILES[ $ps_field ]['tmp_name'] : $this->_FIELD_VALUES[ $ps_field ];
						}
					} else {
		...
		if ( ( isset( $pa_options["template"] ) && $pa_options["template"] ) ) {
			...
			$vs_prop                       = str_replace( "^" . $this->tableName() . "." . $ps_field, $vs_prop,
				$vs_template_with_substitution );
		}
	 ...
	 * is empty or omitted all fields in the table will be returned. For best performance use as few fields as
	 * possible.
	 *
	 * Note that if you request only a single field value the returned array will have values set to the request field.
	 * If more than one field is requested the values in the returned array will be arrays key'ed on field name.
	 *
	 * The primary key for each row is always returned as a key in the returned array of values. The key will not be
	 * included with field values unless you explicitly request it in the $pa_fields array.
	 *
	 ...
	 *
	 * @return array An array with keys set to primary keys of fetched rows and values set to either (a) a field value
	 *               when only a single field is requested or (b) an array key'ed on field name when more than one
	 *               field is requested
	 *
	...
	public function getFieldValuesForIDs( $pa_ids, $pa_fields = null, $pa_options = null ) {
		...
		if ( ! $vb_dont_use_cache
		     && isset( BaseModel::$s_field_value_arrays_for_IDs_cache[ $vn_table_num
					= $this->tableNum() ][ $vs_cache_key ] )
		) {
	 ...
	 *                          checkAccess = array of access values to filter results by; if defined only items with
	 *                          the specified access code(s) are returned. Only supported for table that have an
	 *                          "access" field.
	 *
	...
	static public function getIDsForIdnos( $pa_idnos, $pa_options = null ) {
		if ( ! is_array( $pa_idnos ) && strlen( $pa_idnos ) ) {
			$pa_idnos = [ $pa_idnos ];
		}
		...
		$va_ret = [];
		while ( $qr_res->nextRow() ) {
			$va_ret[ $pb_force_to_lowercase ? strtolower( $qr_res->get( $vs_idno_fld ) )
				: $qr_res->get( $vs_idno_fld ) ]
				= $qr_res->get( $vs_pk );
		}
	 ...
	 * - original_filename : (note that it is lower case) optional parameter which enables you to pass the original
	 * filename of a file, in addition to the representation in the temporary, global _FILES array;
	 *
	 * for text fields:
			...
			if ( array_key_exists( $vs_field, $this->FIELDS ) ) {
				...
				switch ( $pa_fields_type ) {
					case ( FT_NUMBER ):
						...
						if ( ( $vs_field == $this->HIERARCHY_PARENT_ID_FLD )
						     && ( ( strlen( $vm_value ) > 0 )
						          && ( ! is_numeric( $vm_value )
						               || caGetOption( 'treatParentIDAsIdno', $pa_options, false ) ) )
						) {
							if ( is_array( $va_ids = call_user_func_array( $this->tableName() . "::find", array(
									array( 'idno' => $vm_value, 'deleted' => 0 ),
									array( 'returnAs' => 'ids', 'transaction' => $this->getTransaction() )
								) ) )
							     && sizeof( $va_ids )
							) {
						...
						if ( ( $vm_value !== "" )
						     || ( $this->getFieldInfo( $vs_field, "IS_NULL" )
						          && ( $vm_value == "" ) )
						) {
							if ( $vm_value ) {
								if ( ( $vs_list_code = $this->getFieldInfo( $vs_field, "LIST_CODE" ) )
									...
									if ( $vn_id = ca_lists::getItemID( $vs_list_code, $vm_value ) ) {
										...
										$this->postError( 1103,
											_t( 'Value %1 is not in list %2', $vm_value, $vs_list_code ),
											'BaseModel->set()', $this->tableName() . '.' . $vs_field );

										return false;
									}
								} else {
									$vm_orig_value = $vm_value;
									$vm_value      = preg_replace( "/[^\d\-\.]+/", "",
										$vm_value ); # strip non-numeric characters
									if ( ! preg_match( "/^[\-]{0,1}[\d\.]+$/", $vm_value ) ) {
										$this->postError( 1100,
											_t( "'%1' for %2 is not numeric", $vm_orig_value, $vs_field ),
											"BaseModel->set()", $this->tableName() . '.' . $vs_field );
							...
							if ( ! $vm_value && $this->FIELDS[ $vs_field ]["IS_NULL"] ) {
								...
								if ( ! $va_timestamps ) {
									$this->postError( 1805, $o_tep->getParseErrorMessage(), 'BaseModel->set()',
										$this->tableName() . '.' . $vs_field );

									return false;
								}

								if ( ( $pa_fields_type == FT_HISTORIC_DATETIME )
								     || ( $pa_fields_type == FT_HISTORIC_DATE )
								) {
					...
					case ( FT_HISTORIC_DATERANGE ):
						...
						$vn_end_date   = isset( $this->_FIELD_VALUES[ $vs_end_field_name ] )
							? $this->_FIELD_VALUES[ $vs_end_field_name ] : null;
						if ( ( $this->DIRECT_DATETIMES ) || ( $pa_options["SET_DIRECT_DATE"] ) ) {
							if ( is_array( $vm_value ) && ( sizeof( $vm_value ) == 2 )
							     && ( $vm_value[0] <= $vm_value[1] )
							) {
					...
					case ( FT_TIMERANGE ):
						...
						if ( ( $this->DIRECT_TIMES ) || ( $pa_options["SET_DIRECT_TIMES"] ) ) {
							...
							if ( ! $vm_value && $this->FIELDS[ $vs_field ]["IS_NULL"] ) {
								if ( $this->_FIELD_VALUES[ $vs_start_field_name ]
								     || $this->_FIELD_VALUES[ $vs_end_field_name ]
								) {
									$this->_FIELD_VALUE_CHANGED[ $vs_field ] = true;
								}
								$this->_FIELD_VALUES[ $vs_start_field_name ] = null;
					...
					case ( FT_TEXT ):
						...
						if ( ( isset( $pa_options['purify'] ) && ( $pa_options['purify'] ) )
						     || ( (bool) $this->opb_purify_input )
						     || ( $this->getFieldInfo( $vs_field, "PURIFY" ) )
						) {
							$vm_value = str_replace( "&amp;", "&",
								BaseModel::getPurifier()->purify( (string) $vm_value ) );
						}

						if ( $this->getFieldInfo( $vs_field, "DISPLAY_TYPE" ) == DT_LIST_MULTIPLE ) {
							if ( is_array( $vm_value ) ) {
								if ( ! ( $vs_list_multiple_delimiter = $this->getFieldInfo( $vs_field,
									'LIST_MULTIPLE_DELIMITER' ) )
								) {
					...
					case ( FT_FILE ):
						...
						if ( ( isset( $pa_options['purify'] ) && ( $pa_options['purify'] ) )
							...
							$pa_options["original_filename"] = BaseModel::getPurifier()
							                                            ->purify( (string) $pa_options["original_filename"] );
							$vm_value                        = BaseModel::getPurifier()->purify( (string) $vm_value );
						}
	 ...
	 *
	 * @param bool $pb_include_tablename Return primary key field name prepended with table name (Eg.
	 *                                   ca_objects.object_id) [Default is false]
	 *
	 ...
	 *
	 * As of version 1.7.7 access modes are deprecated and this method always returns ACCESS_WRITE (or "write" if the
	 * "return name" parameter is set.
	 *
	 * @param bool $pb_return_name If set to true, return string representations of the modes
	 *                             (i.e. 'read' or 'write'). If set to false (it is by default), returns ACCESS_READ or
	 *                             ACCESS_WRITE.
	 *
	 * @return int|string As of version 1.7.7 access modes are deprecated and this method always returns ACCESS_WRITE
	 *                    (or "write" if the "return name" parameter is set.
	 */
	 ...
	 *
	 * As of version 1.7.7 access modes are deprecated and access mode setting are ingored. This method always returns
	 * return.
	 *
	 * @param int $pn_mode access mode representation, either ACCESS_READ or ACCESS_WRITE
	 *
	 * @return bool As of version 1.7.7 access modes are deprecated and access mode setting are ingored. This method
	 *              always returns return.
	 */
	 ...
	 * @param       $ps_table_name string The table to fetch values from. If omitted the name of the subclass through
	 *                             which the method was invoked is uses. This means that, for example,
	 *                             ca_entities::getFieldValueArraysForIDs(array(1,2,3)) and
	 *                             BaseModel::getFieldValueArraysForIDs(array(1,2,3), "ca_entities") are the same
	 *                             thing.
	 *
	...
	public function load( $pm_id = null, $pb_use_cache = true ) {
		$this->clear();
		$vs_table_name = $this->tableName();
		if ( $pb_use_cache && is_numeric( $pm_id )
		     && isset( BaseModel::$s_instance_cache[ $vs_table_name ][ (int) $pm_id ] )
		     && is_array( BaseModel::$s_instance_cache[ $vs_table_name ][ (int) $pm_id ] )
		) {
				...
				if ( preg_match( "/([\w_]+)\.([\w_]+)/", $vs_field, $va_matches ) ) {
					if ( $va_matches[1] != $this->tableName() ) {
						if ( Datamodel::tableExists( $va_matches[1] ) ) {
							$this->postError( 715,
								_t( "BaseModel '%1' cannot be accessed with this class", $va_matches[1] ),
								"BaseModel->load()" );

							return false;
						} else {
							$this->postError( 715, _t( "BaseModel '%1' does not exist", $va_matches[1] ),
								"BaseModel->load()" );
		...
		if ( $qr_res->nextRow() ) {
			...
			if ( $vn_id = $this->_FIELD_VALUES[ $this->primaryKey() ] ) {
				if ( is_array( BaseModel::$s_instance_cache[ $vs_table_name ] )
				     && sizeof( BaseModel::$s_instance_cache[ $vs_table_name ] ) > 100
				) {
					BaseModel::$s_instance_cache[ $vs_table_name ]
						= array_slice( BaseModel::$s_instance_cache[ $vs_table_name ], 0, 50, true );
				}
		...
		$qr_up = $o_db->query( "
			...
			WHERE
				...
				({$vs_hier_right_fld} < ?) AND (" . $this->primaryKey() . " <> ?)" .
		                       ( $vs_hier_id_fld ? " AND ({$vs_hier_id_fld} = "
		                                           . intval( $pa_parent_info[ $vs_hier_id_fld ] ) . ")" : '' ) . "
		", $pa_parent_info[ $vs_hier_left_fld ], $pa_parent_info[ $vs_hier_right_fld ],
			$pa_parent_info[ $this->primaryKey() ] );
	...
	/**
	 * Internal-use-only function for getting child record ids in a hierarchy. Unlike the public getHierarchyChildren()
	 * method which uses nested set hierarchical indexing to fetch all children in a single pass (and also can return
	 * more than just the id's of the children), _getHierarchyChildren() recursively traverses the hierarchy using
	 * parent_id field values. This makes it useful for getting children in situations when the hierarchical indexing
	 * may not be valid, such as when moving items between hierarchies.
	 *
	 * @access private
	 *
	...
	/**
	 * Clear cached instance values for this model/table. When optional $pb_all_tables parameter is set all cache
	 * entries from all models are cleared.
	 *
			...
			case 251: // Duplicate key error
				...
				if ( preg_match( "/for key [']{0,1}([\w]+)[']{0,1}$/", $e->getMessage(), $matches ) ) {
					...
					if ( sizeof( $field_labels ) > 0 ) {
						$msg = _t( "The combination of %1 and %2 must be unique", join( ', ', $field_labels ),
							$last_name );
					} else {
	 ...
	 *                    dont_do_search_indexing = if set to true then no search indexing on the inserted record is
	 *                    performed dontLogChange = don't log change in change log. [Default is false]
	 *
	...
	public function insert( $pa_options = null ) {
		...
		if ( ( $vb_is_hierarchical = $this->isHierarchical() )
		     && ( ! isset( $pa_options['dontSetHierarchicalIndexing'] )
		          || ! $pa_options['dontSetHierarchicalIndexing'] )
		) {
			...
			switch ( $this->getHierarchyType() ) {
				...
				case __CA_HIER_TYPE_MULTI_MONO__:    // you need to set parent_id (otherwise it defaults to the hierarchy root); you only need to set hierarchy_id if you're creating a root
					if ( ! ( $vn_hierarchy_id = $this->get( $this->getProperty( 'HIERARCHY_ID_FLD' ) ) ) ) {
						$this->set( $this->getProperty( 'HIERARCHY_ID_FLD' ),
							$va_parent_info[ $this->getProperty( 'HIERARCHY_ID_FLD' ) ] );
					}
					if ( ! $vn_parent_id ) {
		...
		foreach ( $this->FIELDS as $vs_field => $va_attr ) {
			...
			if ( isset( $va_attr['DONT_PROCESS_DURING_INSERT_UPDATE'] )
			     && (bool) $va_attr['DONT_PROCESS_DURING_INSERT_UPDATE']
			) {
				continue;
			}
			...
			if ( $pb_need_reload ) {
				$vs_field_value = $this->get( $vs_field,
					array( "TIMECODE_FORMAT" => "RAW" ) );    // reload value since verifyFieldValue() may force the value to a default
			}
			...
			if ( isset( $va_many_to_one_relations[ $vs_field ] ) && $va_many_to_one_relations[ $vs_field ] ) {
				...
				if ( ! (
					...
					if ( $t_many_table
						= Datamodel::getInstance( $va_many_to_one_relations[ $vs_field ]["one_table"] )
					) {
						if ( $this->inTransaction() ) {
					...
					if ( $t_many_table->numErrors() ) {
						$this->postError( 750,
							_t( "%1 record with $vs_field = %2 does not exist", $t_many_table->tableName(),
								$this->get( $vs_field ) ), "BaseModel->insert()",
							$this->tableName() . '.' . $vs_field );
					}
				}
			}
			...
			if ( $vs_field_value_is_null && ! in_array( $vs_field_type, array( FT_MEDIA, FT_FILE ) ) ) {
				if ( ( $vs_field_type == FT_DATERANGE ) || ( $vs_field_type == FT_HISTORIC_DATERANGE )
				     || ( $vs_field_type == FT_TIMERANGE )
				) {
				...
				switch ( $vs_field_type ) {
					...
					case ( FT_HISTORIC_DATE ):
						$vs_fields .= "$vs_field,";
						$v         = isset( $this->_FIELD_VALUES[ $vs_field ] ) ? $this->_FIELD_VALUES[ $vs_field ]
							: null;
						if ( ( ( $v == '' ) || is_null( $v ) ) && ! $va_attr["IS_NULL"] ) {
							$this->postError( 1805,
								_t( "Date is undefined but field %1 does not support NULL values", $vs_field ),
								"BaseModel->insert()", $this->tableName() . '.' . $vs_field );
					...
					case ( FT_TIME ):
						...
						if ( ( ( $v == '' ) || is_null( $v ) ) && ! $va_attr["IS_NULL"] ) {
							$this->postError( 1805,
								_t( "Time is undefined but field %1 does not support NULL values", $vs_field ),
								"BaseModel->insert()", $this->tableName() . '.' . $vs_field );
							if ( $vb_we_set_transaction ) {
								$this->removeTransaction( false );
							}
					...
					case ( FT_HISTORIC_DATERANGE ):
						...
						if (
							! $va_attr["IS_NULL"]
							&& ( ( ( $this->_FIELD_VALUES[ $start_field_name ] == '' )
							       || is_null( $this->_FIELD_VALUES[ $start_field_name ] ) )
							     || ( ( $this->_FIELD_VALUES[ $end_field_name ] == '' )
							          || is_null( $this->_FIELD_VALUES[ $end_field_name ] ) ) )
						) {
							$this->postError( 1805,
								_t( "Daterange is undefined but field does not support NULL values" ),
								"BaseModel->insert()", $this->tableName() . '.' . $vs_field );
							if ( $vb_we_set_transaction ) {
								$this->removeTransaction( false );
							}
					...
					case ( FT_TIMERANGE ):
						...
						if (
							...
							$this->postError( 1805,
								_t( "Time range is undefined but field does not support NULL values" ),
								"BaseModel->insert()", $this->tableName() . '.' . $vs_field );
							if ( $vb_we_set_transaction ) {
								$this->removeTransaction( false );
							}
					...
					case ( FT_BIT ):
						...
						if ( ( isset( $this->RANK ) ) && ( $vs_field == $this->RANK )
						     && ( ! $this->get( $this->RANK ) )
						) {
						...
						if ( ! is_numeric( $v ) ) {
							$this->postError( 1100, _t( "Number is invalid for %1 [%2]", $vs_field, $v ),
								"BaseModel->insert()", $this->tableName() . '.' . $vs_field );
							if ( $vb_we_set_transaction ) {
								$this->removeTransaction( false );
							}
					...
					case ( FT_VARS ):
						$vs_fields .= $vs_field . ",";
						$vs_values .= $this->quote( caSerializeForDatabase( $this->get( $vs_field ),
								( isset( $va_attr['COMPRESS'] ) && $va_attr['COMPRESS'] ) ? true : false ) ) . ",";
						$vn_fields_that_have_been_set ++;
						break;
					# -----------------------------
					default:
		...
		if ( $this->numErrors() == 0 ) {
			...
			if ( $this->numErrors() == 0 ) {
				...
				if ( ( sizeof( $va_media_fields ) > 0 ) || ( sizeof( $va_file_fields ) > 0 )
				     || ( $this->getHierarchyType() == __CA_HIER_TYPE_ADHOC_MONO__ )
				) {
					$vs_sql = "";
					if ( sizeof( $va_media_fields ) > 0 ) {
						foreach ( $va_media_fields as $f ) {
							if ( $vs_msql = $this->_processMedia( $f, array(
					...
					if ( $this->numErrors() == 0 ) {
						...
						if ( $vs_sql ) {
							$o_db->query( "UPDATE " . $this->tableName() . " SET " . $vs_sql . " WHERE "
							              . $this->primaryKey() . " = ?", $this->getPrimaryKey( 1 ) );
						}
					} else {
						# media and/or file error
						$o_db->query( "DELETE FROM " . $this->tableName() . " WHERE " . $this->primaryKey() . " = ?",
							$this->getPrimaryKey( 1 ) );
				...
				if ( ( ! isset( $pa_options['dont_do_search_indexing'] )
				       || ( ! $pa_options['dont_do_search_indexing'] ) )
				     && ! defined( '__CA_DONT_DO_SEARCH_INDEXING__' )
				) {
				...
				if ( ( $vn_fields_that_have_been_set > 0 ) && ! caGetOption( 'dontLogChange', $pa_options, false ) ) {
					$this->logChange( "I", null,
						[ 'log_id' => $vn_log_id = caGetOption( 'log_id', $pa_options, null ) ] );
				}
				...
				if ( is_array( BaseModel::$s_instance_cache[ $vs_table_name = $this->tableName() ] )
				     && ( sizeof( BaseModel::$s_instance_cache[ $vs_table_name = $this->tableName() ] ) > 100 )
				) {    // Limit cache to 100 instances per table
					BaseModel::$s_instance_cache[ $vs_table_name ]
						= array_slice( BaseModel::$s_instance_cache[ $vs_table_name ], 0, 50, true );
				}
			...
			foreach ( $o_db->errors() as $o_e ) {
				switch ( $vn_err_num = $o_e->getErrorNumber() ) {
					case 251:    // violation of unique key (duplicate record)
						...
						if ( preg_match( "/for key [']{0,1}([\w]+)[']{0,1}$/", $o_e->getErrorDescription(),
							$va_matches )
						) {
							...
							if ( sizeof( $va_field_labels ) > 0 ) {
								$vs_err_desc = _t( "The combination of %1 and %2 must be unique",
									join( ', ', $va_field_labels ), $vs_last_name );
							} else {
						...
						break;
					default:
						$this->postError( $vn_err_num, $o_e->getErrorDescription() . ' [' . $vn_err_num . ']',
							"BaseModel->insert()", $this->tableName() . '.' . $vs_field );
						break;
				}
			}
	 ...
	 *                          possible options (keys):
	 *                          dontCheckCircularReferences = when dealing with strict monohierarchical lists (also
	 *                          known as trees), you can use this option to disable checks for circuits in the graph
	 *                          updateOnlyMediaVersions = when set to an array of valid media version names, media is
	 *                          only processed for the specified versions force = if set field values are not verified
	 *                          prior to performing the update dontLogChange = don't log change in change log. [Default
	 *                          is false] dontUpdateHistoryCurrentValueTracking = Skip updating current value tracking
	 *                          caches. Used internally when deleting rows. [Default is false]
	 *
	 * @return bool success state
	 */
	public function update( $pa_options = null ) {
		...
		if ( ! $this->getPrimaryKey() ) {
			$this->postError( 765, _t( "Can't do update() on new record; use insert() instead" ),
				"BaseModel->update()" );

			return false;
		}
		// do form timestamp check
		if ( ! caGetOption( 'force', $pa_options, false ) && isset( $_REQUEST['form_timestamp'] )
		     && ( $vn_form_timestamp = $_REQUEST['form_timestamp'] )
		) {
			$va_possible_conflicts = $this->getChangeLog( null, array(
				'forTable'      => true,
				'range'         => array( 'start' => $vn_form_timestamp, 'end' => time() ),
				'excludeUnitID' => $this->getCurrentLoggingUnitID()
			) );
			...
			if ( sizeof( $va_possible_conflicts ) ) {
				...
				switch ( sizeof( $va_conflict_users ) ) {
					case 0:
						$this->postError( 795,
							_t( 'Changes have been made since you loaded this data. Save again to overwrite the changes or cancel to keep the changes.' ),
							"BaseModel->update()" );
						break;
					case 1:
						$this->postError( 795,
							_t( 'Changes have been made since you loaded this data by %1. Save again to overwrite the changes or cancel to keep the changes.',
								join( ', ', array_keys( $va_conflict_users ) ) ), "BaseModel->update()" );
						break;
					default:
						$this->postError( 795,
							_t( 'Changes have been made since you loaded this data by these users: %1. Save again to overwrite the changes or cancel to keep the changes.',
								join( ', ', array_keys( $va_conflict_users ) ) ), "BaseModel->update()" );
						break;
				}
			}
			...
			if ( $vb_parent_id_changed = $this->changed( $vs_parent_id_fld ) ) {
				...
				if ( ! $pa_options['dontCheckCircularReferences'] ) {
					...
					if ( in_array( $this->get( $vs_parent_id_fld ), $va_ids ) ) {
						$this->postError( 2010,
							_t( "Cannot move %1 under its sub-record", $this->getProperty( 'NAME_SINGULAR' ) ),
							"BaseModel->update()", $this->tableName() . '.' . $vs_parent_id_fld );
						if ( $vb_we_set_transaction ) {
							$this->removeTransaction( false );
						}
						if ( $we_set_change_log_unit_id ) {
				...
				switch ( $this->getHierarchyType() ) {
					...
					case __CA_HIER_TYPE_MULTI_MONO__:    // you need to set parent_id; you only need to set hierarchy_id if you're creating a root
						...
						if ( ! ( $vn_hierarchy_id = $this->get( $vs_hier_id_fld ) ) ) {
							$this->postError( 2030, _t( "Hierarchy ID must be specified for this update" ),
								"BaseModel->update()", $this->tableName() . '.' . $vs_hier_id_fld );
							if ( $vb_we_set_transaction ) {
								$this->removeTransaction( false );
							}
							if ( $we_set_change_log_unit_id ) {
						...
						if ( is_array( $va_parent_info )
						     && ( $this->get( $vs_hier_id_fld ) != $va_parent_info[ $vs_hier_id_fld ] )
						) {
							$vn_hierarchy_id = $va_parent_info[ $vs_hier_id_fld ];
							$this->set( $this->getProperty( 'HIERARCHY_ID_FLD' ), $vn_hierarchy_id );

							$va_rebuild_hierarchical_index
								= $this->getHierarchyChildrenForIDs( [ $this->getPrimaryKey() ],
								[ 'idsOnly' => true, 'includeSelf' => false ] );
					...
					case __CA_HIER_TYPE_ADHOC_MONO__:    // you need to set parent_id for this hierarchy; you never need to set hierarchy_id
						if ( $va_parent_info ) {
							...
							if ( sizeof( $va_ancestors = $this->getHierarchyAncestors( $vn_parent_id,
								array( 'idsOnly' => true, 'includeSelf' => true ) ) )
							) {
				...
				if ( ! isset( $pa_options['dontSetHierarchicalIndexing'] )
				     || ! $pa_options['dontSetHierarchicalIndexing']
				) {
					if ( $va_parent_info ) {
						$va_hier_indexing = $this->_calcHierarchicalIndexing( $va_parent_info );
					...
					$this->set( $this->getProperty( 'HIERARCHY_LEFT_INDEX_FLD' ),
						$vn_orig_hier_left = $va_hier_indexing['left'] );
					$this->set( $this->getProperty( 'HIERARCHY_RIGHT_INDEX_FLD' ),
						$vn_orig_hier_right = $va_hier_indexing['right'] );
				}
			}
		}
		...
		foreach ( $this->FIELDS as $vs_field => $va_attr ) {
			...
			$vs_field_type  = isset( $va_attr["FIELD_TYPE"] ) ? $va_attr["FIELD_TYPE"]
				: null;                # field type
			...
			if ( $va_attr["IS_NULL"] && ( ! in_array( $vs_field_type, array( FT_MEDIA, FT_FILE ) ) )
			     && ( strlen( $vs_field_value ) == 0 )
			) {
			...
			if ( isset( $va_many_to_one_relations[ $vs_field ] ) && $va_many_to_one_relations[ $vs_field ] ) {
				...
				if ( ! ( ( $va_attr["IS_NULL"] ) && ( $vs_field_value == "" ) ) ) {
					...
					if ( $t_many_table->numErrors() ) {
						$this->postError( 750,
							_t( "%1 record with $vs_field = %2  does not exist", $t_many_table->tableName(),
								$this->get( $vs_field ) ), "BaseModel->update()",
							$this->tableName() . '.' . $vs_field );
					}
				}
			}
			...
			if ( ( $vs_field_value_is_null )
			     && ( ! ( isset( $va_attr["UPDATE_ON_UPDATE"] )
			              && $va_attr["UPDATE_ON_UPDATE"] ) )
			) {
				if ( ( $vs_field_type == FT_DATERANGE ) || ( $vs_field_type == FT_HISTORIC_DATERANGE )
				     || ( $vs_field_type == FT_TIMERANGE )
				) {
				...
				switch ( $vs_field_type ) {
					...
					case ( FT_VARS ):
						$vm_val = isset( $this->_FIELD_VALUES[ $vs_field ] ) ? $this->_FIELD_VALUES[ $vs_field ] : null;
						$vs_sql .= "{$vs_field} = " . $this->quote( caSerializeForDatabase( $vm_val,
								( ( isset( $va_attr['COMPRESS'] ) && $va_attr['COMPRESS'] ) ? true : false ) ) ) . ",";
						$vn_fields_that_have_been_set ++;
						$fields_changed ++;
						break;
					...
					case ( FT_HISTORIC_DATE ):
						...
						if ( ( ( $vm_val == '' ) || is_null( $vm_val ) ) && ! $va_attr["IS_NULL"] ) {
							$this->postError( 1805, _t( "Date is undefined but field does not support NULL values" ),
								"BaseModel->update()", $this->tableName() . '.' . $vs_field );
							if ( $vb_we_set_transaction ) {
								$this->removeTransaction( false );
							}
							if ( $we_set_change_log_unit_id ) {
					...
					case ( FT_TIME ):
						$vm_val = isset( $this->_FIELD_VALUES[ $vs_field ] ) ? $this->_FIELD_VALUES[ $vs_field ] : null;
						if ( ( ( $vm_val == '' ) || is_null( $vm_val ) ) && ! $va_attr["IS_NULL"] ) {
							$this->postError( 1805, _t( "Time is undefined but field does not support NULL values" ),
								"BaseModel->update()", $this->tableName() . '.' . $vs_field );
							if ( $vb_we_set_transaction ) {
								$this->removeTransaction( false );
							}
					...
					case ( FT_MEDIA ):
						...
						if ( $vs_media_sql = $this->_processMedia( $vs_field, array(
							'processingMediaForReplication' => caGetOption( 'processingMediaForReplication',
								$pa_options, false ),
							'these_versions_only'           => $va_limit_to_versions,
							'batch'                         => caGetOption( 'batch', $pa_options, false )
						) )
						) {
							$vs_sql .= $vs_media_sql;
		...
		if ( $this->numErrors() == 0 ) {
			if ( $vn_fields_that_have_been_set > 0 ) {
				...
				if ( ( $fields_changed > 0 ) && method_exists( $this, "deriveHistoryTrackingCurrentValue" )
				     && ! caGetOption( 'dontUpdateHistoryCurrentValueTracking', $pa_options, false )
				     && ! defined( '__CA_DONT_UPDATE_HISTORY_TRACKING_CACHE__' )
				) {
					$table = $this->tableName();
					$this->deriveHistoryTrackingCurrentValue();
					if ( $table::isHistoryTrackingCriterion( $table ) ) {
				...
				if ( is_array( $va_rebuild_hierarchical_index )
					...
					foreach ( $va_rebuild_hierarchical_index as $vn_child_id ) {
						...
						if ( $t_instance->load( $vn_child_id ) ) {
							...
							$va_hier_info
							               = $this->_calcHierarchicalIndexing( $this->_getHierarchyParent( $va_child_info[ $vs_parent_id_fld ] ) );
			...
			// Update instance cache
			if ( sizeof( BaseModel::$s_instance_cache[ $vs_table_name = $this->tableName() ] )
			     > 100
			) {    // Limit cache to 100 instances per table
				BaseModel::$s_instance_cache[ $vs_table_name ]
					= array_slice( BaseModel::$s_instance_cache[ $vs_table_name ], 0, 50, true );
	 ...
	 * @param bool  $pb_reindex_mode               If set indexing is done in "reindex mode"; that is the row is
	 *                                             reindexed from scratch as if the entire database is being reindexed.
	 *                                             [Default is false]
	 * @param array $pa_options                    Options include
	 *                                             engine = Name of the search engine to use. [Default is the engine
	 *                                             configured using "search_engine_plugin" in app.conf] isNewRow = Set
	 *                                             to true if row is being indexed for the first time.
	 *                                             BaseModel::insert() should set this. [Default is false]
	 *
	 * @return bool true on success, false on failure of indexing
	 */
	public function doSearchIndexing(
		$pa_changed_field_values_array = null, $pb_reindex_mode = false, $pa_options = null
	) {
	 ...
	 *
	 * @param string $ps_engine Name of the search engine to use. [Default is the engine configured using
	 *                          "search_engine_plugin" in app.conf]
	 *
	 ...
	 *                                 hard = if true records which can support "soft" delete are really deleted rather
	 *                                 than simply being marked as deleted queueIndexing = dontLogChange = don't log
	 *                                 change in change log. [Default is false]
	 * @param array $pa_fields         instead of deleting the record represented by this object instance you can
	 *                                 pass an array of field => value assignments which is used in a SQL-DELETE-WHERE
	 *                                 clause.
	...
	public function delete( $pb_delete_related = false, $pa_options = null, $pa_fields = null, $pa_table_list = null ) {
			...
			if ( $vn_rc = self::update( array( 'force' => true, 'dontUpdateHistoryCurrentValueTracking' => true ) ) ) {
				if ( ! defined( '__CA_DONT_DO_SEARCH_INDEXING__' ) || ! __CA_DONT_DO_SEARCH_INDEXING__ ) {
					...
					$o_indexer->commitRowUnIndexing( $this->tableNum(), $vn_id,
						array( 'queueIndexing' => $pb_queue_indexing ) );
				}
			}
		...
		if ( is_array( $pa_fields ) ) {
			...
			$vs_sql = "DELETE FROM " . $this->tableName() . " WHERE " . $this->primaryKey() . " = "
			          . $this->getPrimaryKey( 1 );
		}
		...
		if ( ! defined( '__CA_DONT_DO_SEARCH_INDEXING__' ) ) {
			$o_indexer = $this->getSearchIndexer();
			$o_indexer->startRowUnIndexing( $this->tableNum(),
				$vn_id ); // records dependencies but does not actually delete indexing
		}
		...
		if ( is_array( $va_one_to_many_relations ) ) {
			foreach ( $va_one_to_many_relations as $vs_many_table => $va_info ) {
				foreach ( $va_info as $va_relationship ) {
					if ( isset( $pa_table_list[ $vs_many_table . '/' . $va_relationship["many_table_field"] ] )
					     && $pa_table_list[ $vs_many_table . '/' . $va_relationship["many_table_field"] ]
					) {
						continue;
					}
						...
						if ( $pb_delete_related ) {
							while ( $qr_record_check->nextRow() ) {
								if ( $t_related->load( $qr_record_check->get( $t_related->primaryKey() ) ) ) {
									$t_related->delete( $pb_delete_related,
										array_merge( $pa_options, array( 'hard' => true ) ), null, $pa_table_list );

									if ( $t_related->numErrors() ) {
										$this->postError( 790,
											_t( "Can't delete item because items related to it have sub-records (%1)",
												$vs_many_table ), "BaseModel->delete()" );
										if ( $vb_we_set_transaction ) {
											$this->removeTransaction( false );
										}
										if ( $we_set_change_log_unit_id ) {
							...
							$this->postError( 780, _t( "Can't delete item because it is in use (%1)", $vs_many_table ),
								"BaseModel->delete()" );
							if ( $vb_we_set_transaction ) {
								$this->removeTransaction( false );
							}
			...
			switch ( $vs_status ) {
				...
				case 'PARTIAL':
					...
					if ( $o_tq->addTask(
						...
						$this->postError( 100,
							_t( "Couldn't queue mirror using '%1' for version '%2' (handler '%3')", $vs_mirror_method,
								$ps_version, $vs_queue ), "BaseModel->retryMediaMirror()" );
					}
					break;
			}
		}
	 ...
	 *                           localOnly = if true url to locally hosted media is always returned, even if an
	 *                           external url is available externalOnly = if true url to externally hosted media is
	 *                           always returned, even if an no external url is available
	 *
	...
	public function getMediaUrl( $ps_field, $ps_version, $pn_page = 1, $pa_options = null ) {
		...
		# Use icon
		#
		if ( isset( $va_media_info[ $ps_version ]['USE_ICON'] )
		     && ( $vs_icon_code
				= $va_media_info[ $ps_version ]['USE_ICON'] )
		) {
			return caGetDefaultMediaIconUrl( $vs_icon_code, $va_media_info[ $ps_version ]['WIDTH'],
				$va_media_info[ $ps_version ]['HEIGHT'] );
		}
		...
		if ( ! isset( $pa_options['localOnly'] ) || ! $pa_options['localOnly'] ) {
			if ( isset( $va_media_info[ $ps_version ]["EXTERNAL_URL"] )
			     && ( $va_media_info[ $ps_version ]["EXTERNAL_URL"] )
			) {
		...
		if ( $va_media_info[ $ps_version ]["FILENAME"] ) {
			$vs_fpath = join( "/", array(
				$vs_url_path,
				$va_media_info[ $ps_version ]["HASH"],
				$va_media_info[ $ps_version ]["MAGIC"] . "_" . $va_media_info[ $ps_version ]["FILENAME"]
			) );
	...
	public function getMediaPath( $ps_field, $ps_version, $pn_page = 1 ) {
		...
		if ( isset( $va_media_info[ $ps_version ]['USE_ICON'] )
			...
			return caGetDefaultMediaIconPath( $vs_icon_code, $va_media_info[ $ps_version ]['WIDTH'],
				$va_media_info[ $ps_version ]['HEIGHT'] );
		}
	 ...
	 * @param string $vspace  vspace attribute of the img tag - note: deprecated in HTML 4.01, not supported in XHTML
	 *                        1.0 Strict
	 * @param string $hspace  hspace attribute of the img tag - note: deprecated in HTML 4.01, not supported in XHTML
	 *                        1.0 Strict
	 * @param string $alt     alt attribute of the img tag
	 * @param int    $border  border attribute of the img tag - note: deprecated in HTML 4.01, not supported in XHTML
	 *                        1.0 Strict
	 * @param string $usemap  usemap attribute of the img tag
	 * @param int    $align   align attribute of the img tag - note: deprecated in HTML 4.01, not supported in XHTML
	 *                        1.0 Strict
	 *
	...
	public function getMediaTag( $ps_field, $ps_version, $pa_options = null ) {
		...
		if ( isset( $va_media_info[ $ps_version ]['USE_ICON'] )
			...
			return caGetDefaultMediaIconTag( $vs_icon_code, $va_media_info[ $ps_version ]['WIDTH'],
				$va_media_info[ $ps_version ]['HEIGHT'] );
		}
		...
		$url = caGetOption( 'usePath', $pa_options, false ) ? $this->getMediaPath( $ps_field, $ps_version,
			isset( $pa_options["page"] ) ? $pa_options["page"] : null )
			: $this->getMediaUrl( $ps_field, $ps_version, isset( $pa_options["page"] ) ? $pa_options["page"] : null );
		$m   = new Media();

		$o_vol     = new MediaVolumes();
		$va_volume = $o_vol->getVolumeInformation( $va_media_info[ $ps_version ]['VOLUME'] );

		return $m->htmlTag( $va_media_info[ $ps_version ]["MIMETYPE"], $url,
			$va_media_info[ $ps_version ]["PROPERTIES"], array_merge( $pa_options, [ 'alt' => $alt_text ] ),
			$va_volume );
	}
	 ...
	 *
	 * @param string $field    field name
	 * @param string $version  version of the media file, as defined in media_processing.conf, can be omitted to
	 *                         retrieve information about all versions
	 * @param string $property this is your opportunity to restrict the result to a certain property for the given
	 *                         (field,version) pair. possible values are:
	 *                         -VOLUME
	 *                         -MIMETYPE
	 *                         -WIDTH
	 *                         -HEIGHT
	 *                         -PROPERTIES: returns an array with some media metadata like width, height, mimetype,
	 *                         etc.
	 *                         -FILENAME
	 *                         -HASH
	 *                         -MAGIC
	...
	public function &getMediaInfo( $ps_field, $ps_version = null, $ps_property = null ) {
		$va_media_info = self::get( $ps_field,
			array( 'returnWithStructure' => true, 'USE_MEDIA_FIELD_VALUES' => true ) );
		if ( ! is_array( $va_media_info ) || ! is_array( $va_media_info = array_shift( $va_media_info ) ) ) {
			return null;
		}
		...
		if ( $ps_version && ( ! $ps_property || ( in_array( $ps_property, array( 'WIDTH', 'HEIGHT' ) ) ) ) ) {
			if ( isset( $va_media_info[ $ps_version ]['USE_ICON'] )
				...
				if ( $va_icon_size = caGetMediaIconForSize( $vs_icon_code, $va_media_info[ $ps_version ]['WIDTH'],
					$va_media_info[ $ps_version ]['HEIGHT'] )
				) {
			...
			if ( ! $ps_property || ( in_array( $ps_property, array( 'WIDTH', 'HEIGHT' ) ) ) ) {
				foreach ( array_keys( $va_media_info ) as $vs_version ) {
					if ( isset( $va_media_info[ $vs_version ]['USE_ICON'] )
					     && ( $vs_icon_code
							= $va_media_info[ $vs_version ]['USE_ICON'] )
					) {
						if ( $va_icon_size = caGetMediaIconForSize( $vs_icon_code,
							$va_media_info[ $vs_version ]['WIDTH'], $va_media_info[ $vs_version ]['HEIGHT'] )
						) {
		...
		if ( $va_media_info = $this->getMediaInfo( $ps_field ) ) {
			...
			if ( $vs_media_type = $o_media_proc_settings->canAccept( $va_media_info["INPUT"]["MIMETYPE"] ) ) {
				...
				if ( isset( $va_media_type_info['VERSIONS'][ $ps_version ] ) ) {
					if ( $va_rule
						= $o_media_proc_settings->getMediaTransformationRule( $va_media_type_info['VERSIONS'][ $ps_version ]['RULE'] )
					) {
	...
	public function getDefaultMediaVersion( $ps_field ) {
		if ( $va_media_info = $this->getMediaInfo( $ps_field ) ) {
			$o_media_proc_settings = new MediaProcessingSettings( $this, $ps_field );
			$va_type_info
			                       = $o_media_proc_settings->getMediaTypeInfo( $o_media_proc_settings->canAccept( $va_media_info["INPUT"]["MIMETYPE"] ) );

			return ( $va_type_info['MEDIA_VIEW_DEFAULT_VERSION'] ) ? $va_type_info['MEDIA_VIEW_DEFAULT_VERSION']
				: array_shift( $this->getMediaVersions( $ps_field ) );
		} else {
			return null;
		}
	}
	...
	public function getDefaultMediaPreviewVersion( $ps_field ) {
		if ( $va_media_info = $this->getMediaInfo( $ps_field ) ) {
			...
			return ( $va_type_info['MEDIA_PREVIEW_DEFAULT_VERSION'] ) ? $va_type_info['MEDIA_PREVIEW_DEFAULT_VERSION']
				: array_shift( $this->getMediaVersions( $ps_field ) );
		} else {
			return null;
		}
			...
			if ( $vs_media_type = $o_media_proc_settings->canAccept( $ps_mimetype ) ) {
				...
				if ( is_array( $va_version_list ) ) {
					...
					foreach ( $va_version_list as $vs_version => $va_version_info ) {
						if ( isset( $va_version_info['BASIS'] )
						     && isset( $va_version_list[ $va_version_info['BASIS'] ] )
						) {
	...
	public function _removeMedia( $ps_field, $ps_version, $ps_dont_delete_path = "", $ps_dont_delete_extension = "" ) {
		...
		$vs_delete_path = $va_volume_info["absolutePath"] . "/" . $va_media_info["HASH"] . "/" . $va_media_info["MAGIC"]
		                  . "_" . $va_media_info["FILENAME"];
		if ( ( $va_media_info["FILENAME"] )
		     && ( $vs_delete_path != $ps_dont_delete_path . "." . $ps_dont_delete_extension )
		) {
		...
		if ( is_array( $va_volume_info["mirrors"] ) && sizeof( $va_volume_info["mirrors"] ) > 0 ) {
			...
			if ( ! ( $o_tq->cancelPendingTasksForEntity( $vs_entity_key ) ) ) {
				$this->postError( 560, _t( "Could not cancel pending tasks: %1", $this->error ),
					"BaseModel->_removeMedia()" );
			...
			foreach ( $va_volume_info["mirrors"] as $vs_mirror_code => $va_mirror_info ) {
				...
				if ( $o_tq->addTask(
					...
					$this->postError( 100,
						_t( "Couldn't queue mirror using '%1' for version '%2' (handler '%3')", $vs_mirror_method,
							$ps_version, $vs_queue ), "BaseModel->_removeMedia()" );
				}
			}
		}
	}
	 ...
	 *        these_versions_only = if set to an array of valid version names, then only the specified versions are
	 *        updated with the currently updated file; ignored if no media already exists dont_allow_duplicate_media =
	 *        if set to true, and the model has a field named "md5" then media will be rejected if a row already exists
	...
	public function _processMedia( $ps_field, $pa_options = null ) {
		...
		if ( ( isset( $this->_FILES_CLEAR[ $ps_field ] ) ) && ( $this->_FILES_CLEAR[ $ps_field ] ) ) {
			...
			$this->_FIELD_VALUES[ $ps_field ] = null;
			$vs_sql                           = "{$ps_field} = "
			                                    . $this->quote( caSerializeForDatabase( $this->_FILES[ $ps_field ],
					true ) ) . ",";
		} else {
			...
			if ( isset( $this->_SET_FILES[ $ps_field ]['tmp_name'] ) ) {
				...
				if (
					$vb_allow_fetching_of_urls && isUrl( $vs_url ) && ( $media_url = new CA\MediaUrl() )
				) {
					$vs_tmp_file = null;
					try {
						$r = $media_url->fetch( $vs_url );
						if ( is_array( $r ) ) {
							$this->_SET_FILES[ $ps_field ]['original_filename'] = ! empty( $r['originalFilename'] )
								? $r['originalFilename'] : pathinfo( $r['file'], PATHINFO_BASENAME );
							$vs_tmp_file                                        = $r['file'];
						} else {
							$this->postError( 1600, _t( 'Could not download media' ), "BaseModel->_processMedia()",
								$this->tableName() . '.' . $ps_field );
						...
						$this->postError( 1600, $e->getMessage(), "BaseModel->_processMedia()",
							$this->tableName() . '.' . $ps_field );
				...
				if ( preg_match( "!^userMedia[\d]+/!", $this->_SET_FILES[ $ps_field ]['tmp_name'] ) ) {
					...
					if ( ! is_writeable(
						$vs_tmp_directory = $this->getAppConfig()->get( 'ajax_media_upload_tmp_directory' ) )
					) {
						$vs_tmp_directory = caGetTempDirPath();
					}
					$this->_SET_FILES[ $ps_field ]['tmp_name'] = "{$vs_tmp_directory}/"
					                                             . $this->_SET_FILES[ $ps_field ]['tmp_name'];

					// read metadata
					if ( file_exists( "{$vs_tmp_directory}/" . $this->_SET_FILES[ $ps_field ]['tmp_name']
					                  . "_metadata" )
					) {
						if ( is_array( $va_tmp_metadata = json_decode( file_get_contents( "{$vs_tmp_directory}/"
						                                                                  . $this->_SET_FILES[ $ps_field ]['tmp_name']
						                                                                  . "_metadata" ) ) )
						) {
				...
				if ( isset( $this->_SET_FILES[ $ps_field ]['tmp_name'] )
				     && ( file_exists( $this->_SET_FILES[ $ps_field ]['tmp_name'] ) )
				) {
					if ( ! isset( $pa_options['dont_allow_duplicate_media'] ) ) {
						$pa_options['dont_allow_duplicate_media'] = (bool) $this->getAppConfig()
						                                                        ->get( 'dont_allow_duplicate_media' );
					}
					if ( isset( $pa_options['dont_allow_duplicate_media'] )
					     && $pa_options['dont_allow_duplicate_media']
					) {
						if ( $this->hasField( 'md5' ) ) {
							$qr_dupe_chk = $this->getDb()->query( "
								SELECT " . $this->primaryKey() . " FROM " . $this->tableName() . " WHERE md5 = ? "
							                                      . ( $this->hasField( 'deleted' ) ? ' AND deleted = 0'
									: '' ) . "
							", (string) md5_file( $this->_SET_FILES[ $ps_field ]['tmp_name'] ) );

							if ( $qr_dupe_chk->nextRow() ) {
								$this->postError( 1600, _t( "Media already exists in database" ),
									"BaseModel->_processMedia()", $this->tableName() . '.' . $ps_field );
					...
					if ( ! isset( $va_matches[1] ) ) { // file has no extension, i.e. is probably PHP upload tmp file
						$va_matches = array();
						preg_match( "/[.]*\.([a-zA-Z0-9]+)$/", $this->_SET_FILES[ $ps_field ]['original_filename'],
							$va_matches );
						if ( strlen( $va_matches[1] ) > 0 ) {
							$va_parts        = explode( "/", $this->_SET_FILES[ $ps_field ]['tmp_name'] );
							$vs_new_filename = sys_get_temp_dir() . "/" . $va_parts[ sizeof( $va_parts ) - 1 ] . "."
							                   . $va_matches[1];
							if ( caGetOption( 'batch', $pa_options, false ) ) {
								copy( $this->_SET_FILES[ $ps_field ]['tmp_name'], $vs_new_filename );
							} else {
								if ( ! move_uploaded_file( $this->_SET_FILES[ $ps_field ]['tmp_name'],
									$vs_new_filename )
								) {
					...
					if ( ! $input_type = $o_media_proc_settings->canAccept( $input_mimetype ) ) {
						...
						$this->postError( 1600,
							( $input_mimetype ) ? _t( "File type %1 not accepted by %2", $input_mimetype, $ps_field )
								: _t( "Unknown file type not accepted by %1", $ps_field ), "BaseModel->_processMedia()",
							$this->tableName() . '.' . $ps_field );
						set_time_limit( $vn_max_execution_time );
					...
					# ok process file...
					if ( ! ( $m->read( $this->_SET_FILES[ $ps_field ]['tmp_name'], null,
						[ 'original_filename' => $vs_original_filename ] ) )
					) {
					...
					if ( isset( $this->_SET_FILES[ $ps_field ]['options']['TRANSFORMATION_HISTORY'] )
					     && is_array( $this->_SET_FILES[ $ps_field ]['options']['TRANSFORMATION_HISTORY'] )
					) {
						$media_desc['TRANSFORMATION_HISTORY']
							= $this->_SET_FILES[ $ps_field ]['options']['TRANSFORMATION_HISTORY'];
					}
					...
					if ( isset( $this->_SET_FILES[ $ps_field ]['options']['undo'] )
					     && file_exists( $this->_SET_FILES[ $ps_field ]['options']['undo'] )
					) {
						if ( $volume = $version_info['original']['VOLUME'] ) {
							$vi = $this->_MEDIA_VOLUMES->getVolumeInformation( $volume );
							if ( $vi["absolutePath"]
							     && ( strlen( $dirhash = $this->_getDirectoryHash( $vi["absolutePath"],
									$this->getPrimaryKey() ) ) )
							) {
								$magic       = rand( 0, 99999 );
					...
					if ( isset( $pa_options['these_versions_only'] ) && is_array( $pa_options['these_versions_only'] )
					     && sizeof( $pa_options['these_versions_only'] )
					) {
						...
						if ( sizeof( $va_process_these_versions_only ) ) {
							foreach ( array_keys( $va_tmp ) as $v ) {
								if ( ! in_array( $v, $va_process_these_versions_only )
								     && ! in_array( $v, [ '_START_AT_PAGE', '_START_AT_TIME' ] )
								) {
					...
					$queue_enabled      = ( ! sizeof( $va_process_these_versions_only )
					                        && $this->getAppConfig()->get( 'queue_enabled' ) ) ? true : false;
					...
					foreach ( $va_versions as $v ) {
						...
						if ( sizeof( $va_process_these_versions_only )
						     && ( ! in_array( $v, $va_process_these_versions_only ) )
						) {
						...
						$queue_threshold = caGetOption( 'QUEUE_WHEN_FILE_LARGER_THAN', $version_info[ $v ],
							(int) $va_default_queue_settings['QUEUE_WHEN_FILE_LARGER_THAN'] );
						...
						if ( isset( $media_desc[ $basis ] ) && isset( $media_desc[ $basis ]['FILENAME'] ) ) {
							if ( ! isset( $va_media_objects[ $basis ] ) ) {
								$o_media = new Media();
								$basis_vi
								         = $this->_MEDIA_VOLUMES->getVolumeInformation( $media_desc[ $basis ]['VOLUME'] );
								if ( $o_media->read(
									$p = $basis_vi['absolutePath'] . "/" . $media_desc[ $basis ]['HASH'] . "/"
									     . $media_desc[ $basis ]['MAGIC'] . "_" . $media_desc[ $basis ]['FILENAME'],
									null, [ 'original_filename' => $vs_original_filename ] )
								) {
						...
						if ( ( $queue_enabled ) && ( $queue ) && ( $queue_threshold > 0 )
						     && ( $queue_threshold < (int) $media_desc["INPUT"]["FILESIZE"] )
						     && ( $va_default_queue_settings['QUEUE_USING_VERSION'] != $v )
						) {
							...
							if ( $version_info[ $v ]["QUEUED_MESSAGE"] ) {
								...
								$media_desc[ $v ]["QUEUED_MESSAGE"] = ( $va_default_queue_settings['QUEUED_MESSAGE'] )
									? $va_default_queue_settings['QUEUED_MESSAGE']
									: _t( "Media is being processed and will be available shortly." );
							}
						...
						if ( sizeof( $rules ) == 0 ) {
							...
							#
							$ext = ( $output_mimetype == 'application/octet-stream' )
								? pathinfo( $this->_SET_FILES[ $ps_field ]['original_filename'], PATHINFO_EXTENSION )
								: $m->mimetype2extension( $output_mimetype );

							if ( ! $ext ) {
								$this->postError( 1600,
									_t( "File could not be copied for %1; can't convert mimetype '%2' to extension",
										$ps_field, $output_mimetype ), "BaseModel->_processMedia()",
									$this->tableName() . '.' . $ps_field );
								$m->cleanup();
								set_time_limit( $vn_max_execution_time );
								if ( $vb_is_fetched_file ) {
							...
							if ( ( $dirhash = $this->_getDirectoryHash( $vi["absolutePath"], $this->getPrimaryKey() ) )
							     === false
							) {
								$this->postError( 1600,
									_t( "Could not create subdirectory for uploaded file in %1. Please ask your administrator to check the permissions of your media directory.",
										$vi["absolutePath"] ), "BaseModel->_processMedia()",
									$this->tableName() . '.' . $ps_field );
								set_time_limit( $vn_max_execution_time );
								if ( $vb_is_fetched_file ) {
									@unlink( $vs_tmp_file );
							...
							if (
								( ! is_null( $skip_threshold )
								  && ( (int) $skip_threshold < (int) $media_desc["INPUT"]["FILESIZE"] ) )
								|| ( is_null( $skip_threshold ) && isset( $version_info[ $v ]['REPLACE_WITH_VERSION'] )
								     && in_array( $version_info[ $v ]['REPLACE_WITH_VERSION'], $va_versions, true ) )
							) {
								$media_desc[ $v ]['SKIP'] = 1;

								$alt_version_list                         = array_filter( array_keys( $media_desc ),
									function ( $x ) use ( $v ) {
										return ( $x !== $v );
									} );
								$replace_with_version                     = caGetOption( 'REPLACE_WITH_VERSION',
									$version_info[ $v ], array_pop( $alt_version_list ),
									[ 'validValues' => $va_versions ] );
								$media_desc[ $v ]['REPLACE_WITH_VERSION'] = $replace_with_version
									? $replace_with_version : $va_versions[0];
								...
								$magic    = rand( 0, 99999 );
								$filepath = $vi["absolutePath"] . "/" . $dirhash . "/" . $magic . "_"
								            . $this->_genMediaName( $ps_field ) . "_" . $v . "." . $ext;

								if ( ! copy( $this->_SET_FILES[ $ps_field ]['tmp_name'], $filepath ) ) {
									$this->postError( 1600,
										_t( "File could not be copied. Ask your administrator to check permissions and file space for %1",
											$vi["absolutePath"] ), "BaseModel->_processMedia()",
										$this->tableName() . '.' . $ps_field );
									$m->cleanup();
									set_time_limit( $vn_max_execution_time );
								...
								if ( $pa_options['delete_old_media'] ) {
									$va_files_to_delete[] = array(
										'field'                 => $ps_field,
										'version'               => $v,
										'dont_delete_path'      => $vi["absolutePath"] . "/" . $dirhash . "/" . $magic
										                           . "_" . $this->_genMediaName( $ps_field ) . "_" . $v,
								...
								if ( is_array( $vi["mirrors"] ) && sizeof( $vi["mirrors"] ) > 0 ) {
									$vs_entity_key = join( "/",
										array( $this->tableName(), $ps_field, $this->getPrimaryKey(), $v ) );
									$vs_row_key    = join( "/", array( $this->tableName(), $this->getPrimaryKey() ) );
									...
									foreach ( $vi["mirrors"] as $vs_mirror_code => $va_mirror_info ) {
										...
										if ( $o_tq->addTask(
											...
											array(
												...
												"FILES"   => array(
													array(
														...
														"HASH"      => $dirhash,
														"FILENAME"  => $magic . "_" . $this->_genMediaName( $ps_field )
														               . "_" . $v . "." . $ext
													)
												),
											...
											$this->postError( 100,
												_t( "Couldn't queue mirror using '%1' for version '%2' (handler '%3')",
													$vs_mirror_method, $v, $queue ), "BaseModel->_processMedia()" );
										}
							...
							while ( list( $operation, $parameters ) = each( $rules ) ) {
								if ( $operation === 'SET' ) {
									...
									if ( is_array( $this->_FIELD_VALUES[ $ps_field ] )
									     && ( is_array( $va_media_center = $this->getMediaCenter( $ps_field ) ) )
									) {
							...
							if ( ! ( $ext = $m->mimetype2extension( $output_mimetype ) ) ) {
								$this->postError( 1600,
									_t( "File could not be processed for %1; can't convert mimetype '%2' to extension",
										$ps_field, $output_mimetype ), "BaseModel->_processMedia()",
									$this->tableName() . '.' . $ps_field );
								$m->cleanup();
								set_time_limit( $vn_max_execution_time );
							...
							$magic    = rand( 0, 99999 );
							$filepath = $vi["absolutePath"] . "/" . $dirhash . "/" . $magic . "_"
							            . $this->_genMediaName( $ps_field ) . "_" . $v;

							$m->set( 'colorspace', 'RGB' );
							if ( ! ( $vs_output_file = $m->write( $filepath, $output_mimetype,
								$va_media_write_options ) )
							) {
								$this->postError( 1600, _t( "Couldn't write file: %1", join( "; ", $m->getErrors() ) ),
									"BaseModel->_processMedia()", $this->tableName() . '.' . $ps_field );
								$m->cleanup();
								set_time_limit( $vn_max_execution_time );
								if ( $vb_is_fetched_file ) {
							...
							if ( $vs_use_icon ) {
								...
								$media_desc[ $v ] = array(
									...
									"EXTENSION"          => $ext,
									"MD5"                => md5_file(
										$fp = $vi["absolutePath"] . "/" . $dirhash . "/" . $magic . "_"
										      . $this->_genMediaName( $ps_field ) . "_" . $v . "." . $ext ),
					...
					if ( sizeof( $va_queued_versions ) ) {
						$vs_entity_key = md5( join( "/",
							array_merge( array( $this->tableName(), $ps_field, $this->getPrimaryKey() ),
								array_keys( $va_queued_versions ) ) ) );
						$vs_row_key    = join( "/", array( $this->tableName(), $this->getPrimaryKey() ) );
						...
						if ( ! ( $filename = $vs_path_to_queue_media ) ) {
							...
							$filename = $o_tq->copyFileToQueueTmp( $va_default_queue_settings['QUEUE'],
								$this->_SET_FILES[ $ps_field ]['tmp_name'] );
						}
						...
						if ( $filename ) {
							if ( $o_tq->addTask(
								...
								$this->postError( 100,
									_t( "Couldn't queue processing for version '%1' using handler '%2'", ! $v, $queue ),
									"BaseModel->_processMedia()" );
							}
						} else {
						...
						if ( ! sizeof( $va_process_these_versions_only )
						     && ( (bool) $this->_CONFIG->get( 'video_preview_generate_frames' )
						          || (bool) $this->_CONFIG->get( 'document_preview_generate_pages' ) )
						     && method_exists( $this, 'addFile' )
						) {
							...
							if ( is_array( $va_preview_frame_list ) ) {
								foreach ( $va_preview_frame_list as $vn_time => $vs_frame ) {
									$this->addFile( $vs_frame, $vn_time,
										true );    // the resource path for each frame is it's time, in seconds (may be fractional) for video, or page number for documents
					...
					if ( ! $error ) {
						...
						foreach ( $va_files_to_delete as $va_file_to_delete ) {
							$this->_removeMedia( $va_file_to_delete['field'], $va_file_to_delete['version'],
								$va_file_to_delete['dont_delete_path'], $va_file_to_delete['dont_delete_extension'] );
						}
						...
						if ( ( $vs_metadata_field_name = $o_media_proc_settings->getMetadataFieldName() )
						     && $this->hasField( $vs_metadata_field_name )
						) {
							$vn_embedded_media_metadata_limit
								= (int) $this->_CONFIG->get( 'dont_extract_embedded_media_metdata_when_length_exceeds' );
							if ( ( $vn_embedded_media_metadata_limit > 0 )
							     && ( strlen( $vs_serialized_metadata
										= caSerializeForDatabase( $media_metadata, true ) )
							          > $vn_embedded_media_metadata_limit )
							) {
								$media_metadata         = null;
						...
						if ( ( $vs_content_field_name = $o_media_proc_settings->getMetadataContentName() )
						     && $this->hasField( $vs_content_field_name )
						) {
							$this->_FIELD_VALUES[ $vs_content_field_name ] = $this->quote( $m->getExtractedText() );
							$vs_sql                                        .= " " . $vs_content_field_name . " = "
							                                                  . $this->_FIELD_VALUES[ $vs_content_field_name ]
							                                                  . ",";
						}
						...
						if ( is_array( $va_locs = $m->getExtractedTextLocations() ) ) {
							...
							foreach ( $va_locs as $vs_content => $va_loc_list ) {
								foreach ( $va_loc_list as $va_loc ) {
									MediaContentLocationIndexer::index( $this->tableNum(), $this->getPrimaryKey(),
										$vs_content, $va_loc['p'], $va_loc['x1'], $va_loc['y1'], $va_loc['x2'],
										$va_loc['y2'] );
								}
							}
						...
						# error - invalid media
						$this->postError( 1600, _t( "File could not be processed for %1: %2", $ps_field, $error_msg ),
							"BaseModel->_processMedia()" );
						#	    return false;
					}
	 ...
	 * it is applied to all versions, including the "original." A copy of the original is stashed in a "virtual"
	 * version named "_undo_" to make it possible to recover the original media, if desired, by calling
	 * removeMediaTransformations().
	 *
	 * @param string $ps_field   The name of the media field
	 * @param string $ps_op      A valid media transformation op code, as defined by the media plugin handling the
	 *                           media being transformed.
	 * @param array  $pa_params  The parameters for the op code, as defined by the media plugin handling the media
	 *                           being transformed.
	...
	public function applyMediaTransformation( $ps_field, $ps_op, $pa_params, $pa_options = null ) {
		...
		if ( isset( $pa_options['revert'] ) && $pa_options['revert'] && isset( $va_media_info['_undo_'] ) ) {
			...
			$vs_undo_path = ( ! isset( $va_media_info['_undo_'] ) ) ? $vs_path
				: $this->getMediaPath( $ps_field, '_undo_' );
		...
		$this->set( $ps_field, $vs_tmp_basename . "." . $va_media_info['original']['EXTENSION'],
			$vs_undo_path ? array( 'undo' => $vs_undo_path, 'TRANSFORMATION_HISTORY' => $va_transformation_history )
				: array( 'TRANSFORMATION_HISTORY' => $va_transformation_history ) );
	...
	/**
	 * Remove all media transformation to media in specified field of current loaded row by reverting to the unmodified
	 * media.
	 *
	 * @param string $ps_field   The name of the media field
	 * @param array  $pa_options An array of options. No options are currently implemented.
	...
	public function getMediaTransformationHistory( $ps_field, $ps_op = null ) {
		...
		if ( $ps_op && isset( $va_media_info['TRANSFORMATION_HISTORY'][ $ps_op ] )
		     && is_array( $va_media_info['TRANSFORMATION_HISTORY'][ $ps_op ] )
		) {
			return $va_media_info['TRANSFORMATION_HISTORY'][ $ps_op ];
		}

		return ( isset( $va_media_info['TRANSFORMATION_HISTORY'] )
		         && is_array( $va_media_info['TRANSFORMATION_HISTORY'] ) ) ? $va_media_info['TRANSFORMATION_HISTORY']
			: array();
	}
	# --------------------------------------------------------------------------------
	...
	/**
	 * Return coordinates of center of image media as decimals between 0 and 1. By default this is dead-center (x=0.5,
	 * y=0.5) but the user may override this to optimize cropping of images. Currently the center point is only used
	 * when cropping the image from the "center" but it may be used for other transformation (Eg. rotation) in the
	 * future.
	 *
	 * @param string $ps_field   The name of the media field
	 * @param array  $pa_options An array of options. No options are currently implemented.
	...
	/**
	 * Sets the center of the currently loaded media. X and Y coordinates are fractions of the width and height
	 * respectively expressed as decimals between 0 and 1. Currently the center point is only used when cropping the
	 * image from the "center" but it may be used for other transformation (Eg. rotation) in the future.
	 *
	 * @param string $ps_field    The name of the media field
	 * @param float  $pn_center_x X-coordinate for the new center, as a fraction of the width of the image. Value must
	 *                            be between 0 and 1.
	 * @param float  $pn_center_y Y-coordinate for the new center, as a fraction of the height of the image. Value must
	 *                            be between 0 and 1.
	 * @param array  $pa_options  An array of options. No options are currently implemented.
	 *
	 * @return bool True on success, false if an error occurred.
	...
	public function setMediaCenter( $ps_field, $pn_center_x, $pn_center_y, $pa_options = null ) {
		...
		$this->setMediaInfo( $ps_field, $va_media_info );
		$this->update();
		$this->set( 'media', $this->getMediaPath( 'media', 'original' ),
			array( 'original_filename' => $vs_original_filename ) );
		$this->update();
	...
	/**
	 * Set scaling conversion factor for media. Allows physical measurements to be derived from image pixel
	 * measurements. A measurement with physical units of the kind passable to caConvertMeasurementToPoints() (Eg.
	 * "55mm", "5 ft", "10km") and the percentage of the image *width* the measurement covers are passed, from which
	 * the scale factor is calculated and stored.
	 *
	 * @param string $ps_field                  The name of the media field
	 * @param string $ps_dimension              A measurement with dimensional units (ex. "55mm", "5 ft", "10km")
	 * @param float  $pn_percent_of_image_width Percentage of image *width* the measurement covers from 0 to 1. If you
	 *                                          pass a value > 1 it will be divided by 100 for calculations. [Default
	 *                                          is 1]
	 * @param array  $pa_options                An array of options. No options are currently implemented.
	 *
	 * @return bool True on success, false if an error occurred.
	...
	public function setMediaScale( $ps_field, $ps_dimension, $pn_percent_of_image_width = 1, $pa_options = null ) {
		...
		if ( $vo_parsed_measurement && ( ( $vn_measurement = (float) $vo_parsed_measurement->toString( 4 ) ) > 0 ) ) {
			...
			$va_media_info['_SCALE_UNITS'] = caGetLengthUnitType( $vo_parsed_measurement->getType(),
				array( 'short' => true ) );
	...
	/**
	 * Returns scaling conversion factor for media. Allows physical measurements to be derived from image pixel
	 * measurements.
	 *
	 * @param string $ps_field   The name of the media field
	 * @param array  $pa_options An array of options. No options are currently implemented.
	...
	public function getFileUrl( $ps_field ) {
		...
		$vs_host     = $va_volume_info["hostname"];
		$vs_path     = join( "/", array(
			$va_volume_info["urlPath"],
			$va_file_info["HASH"],
			$va_file_info["MAGIC"] . "_" . $va_file_info["FILENAME"]
		) );
	...
	public function getFileConversionPath( $ps_field, $ps_format ) {
		...
		if ( $va_conversions[ $ps_format ] ) {
			return join( "/", array(
				$vi["absolutePath"],
				$va_file_info["HASH"],
				$va_file_info["MAGIC"] . "_" . $va_conversions[ $ps_format ]["FILENAME"]
			) );
		} else {
			return "";
		}
	...
	public function _processFiles( $field ) {
		...
		if ( ( isset( $this->_FILES_CLEAR[ $field ] ) ) && ( $this->_FILES_CLEAR[ $field ] ) ) {
			...
			if ( ( file_exists( $this->_SET_FILES[ $field ]['tmp_name'] ) ) ) {
				$ff       = new File();
				$mimetype = $ff->divineFileFormat( $this->_SET_FILES[ $field ]['tmp_name'],
					$this->_SET_FILES[ $field ]['original_filename'] );
				...
				if ( ( $dirhash = $this->_getDirectoryHash( $vi["absolutePath"], $this->getPrimaryKey() ) )
					...
					$this->postError( 1600,
						_t( "Could not create subdirectory for uploaded file in %1. Please ask your administrator to check the permissions of your media directory.",
							$vi["absolutePath"] ), "BaseModel->_processFiles()", $this->tableName() . '.' . $field );
				...
				#
				if ( isset( $va_field_info["FILE_CONVERSIONS"] ) && is_array( $va_field_info["FILE_CONVERSIONS"] )
				     && ( sizeof( $va_field_info["FILE_CONVERSIONS"] ) > 0 )
				) {
					foreach ( $va_field_info["FILE_CONVERSIONS"] as $vs_output_format ) {
						if ( $va_tmp = $ff->convert( $vs_output_format, $filepath,
							$filestem )
				...
				$vs_sql                        = "$field = "
				                                 . $this->quote( caSerializeForDatabase( $this->_FILES[ $field ],
						true ) ) . ",";
	...
	public function dump() {
		...
		while ( list( $field, $attr ) = each( $this->FIELDS ) ) {
			switch ( $attr['FIELD_TYPE'] ) {
				case FT_HISTORIC_DATERANGE:
					echo "{$field} = " . $this->_FIELD_VALUES[ $attr['START'] ] . "/"
					     . $this->_FIELD_VALUES[ $attr['END'] ] . "<BR>\n";
					break;
				default:
	...
	public function hasBundle( $ps_bundle, $pn_type_id = null ) {
		...
		if ( $vn_num_bits == 1 ) {
			...
			} elseif ( ( $va_bundle_bits[0] != $this->tableName() )
			           && ( $t_rel
					= Datamodel::getInstanceByTableName( $va_bundle_bits[0], true ) )
			) {
	...
	public function verifyFieldValue( $field, $value, &$pb_need_reload ) {
		...
		if ( $data_type == 0 ) {    # number; check value
			...
			if ( ! ( $va_attr["IS_NULL"] && ( ! $value ) ) ) {
				if ( ( isset( $min_value ) ) && ( $value < $min_value ) ) {
					$this->postError( 1101, _t( "%1 must not be less than %2", $va_attr["LABEL"], $min_value ),
						"BaseModel->verifyFieldValue()", $this->tableName() . '.' . $field );

					return false;
				}
				if ( ( isset( $max_value ) ) && ( $value > $max_value ) ) {
					$this->postError( 1101, _t( "%1 must not be greater than %2", $va_attr["LABEL"], $max_value ),
						"BaseModel->verifyFieldValue()", $this->tableName() . '.' . $field );
		...
		if ( ! ( $va_attr["IS_NULL"] && ( ! $value ) ) ) {
			...
			if ( ( isset( $min_length ) ) && ( strlen( $value ) < $min_length ) ) {
				$this->postError( 1102, _t( "%1 must be at least %2 characters", $va_attr["LABEL"], $min_length ),
					"BaseModel->verifyFieldValue()", $this->tableName() . '.' . $field );
			...
			if ( ( isset( $max_length ) ) && ( strlen( $value ) > $max_length ) ) {
				$this->postError( 1102,
					_t( "%1 must not be more than %2 characters long", $va_attr["LABEL"], $max_length ),
					"BaseModel->verifyFieldValue()", $this->tableName() . '.' . $field );
			...
			if ( isset( $va_attr['LIST'] ) && $va_attr['LIST'] ) {
				...
				if ( $t_list->load( array( 'list_code' => $va_attr['LIST'] ) ) ) {
					...
					foreach ( $va_items as $vn_item_id => $va_item_info ) {
						if ( is_null( $vs_list_default )
						     || ( isset( $va_item_info['is_default'] )
						          && $va_item_info['is_default'] )
						) {
			...
			if ( ( in_array( $data_type, array( FT_NUMBER, FT_TEXT ) ) ) && ( isset( $va_list ) )
			     && ( is_array( $va_list ) )
			     && ( count( $va_list ) > 0 )
				...
				if ( ! isset( $va_attr['LIST_MULTIPLE_DELIMITER'] )
				     || ! ( $vs_list_multiple_delimiter
						= $va_attr['LIST_MULTIPLE_DELIMITER'] )
				) {
					$vs_list_multiple_delimiter = ';';
				}
				foreach ( $value as $v ) {
					...
					if ( $va_attr['DISPLAY_TYPE'] == DT_LIST_MULTIPLE ) {
						...
						foreach ( $va_tmp as $vs_mult_item ) {
							if ( ! in_array( $vs_mult_item, $va_list ) ) {
								$this->postError( 1103, _t( "'%1' is not valid choice for %2", $v, $va_attr["LABEL"] ),
									"BaseModel->verifyFieldValue()", $this->tableName() . '.' . $field );
	 ...
	 *
	 * If $sql_fields all virtual fields (Eg. date ranges) are returned as their underlying SQL fields. A date range
	 * will thus be two fields for the start and end of the range.
	 *
	...
	public function getFieldInfo( $field, $attribute = "" ) {
		if ( isset( $this->FIELDS[ $field ] ) ) {
			...
			if ( isset( $translations[ $this->tableName() ][ $field ] )
			     && is_array( $translations[ $this->tableName() ][ $field ] )
			) {
	...
	public function getDisplayLabel( $ps_field ) {
		...
		$translations = $this->_TRANSLATIONS->getAssoc( 'fields' );
		if ( isset( $translations[ $this->tableName() ][ $va_tmp[0] ] )
		     && is_array( $translations[ $this->tableName() ][ $va_tmp[0] ] )
		     && isset( $translations[ $this->tableName() ][ $va_tmp[1] ]['LABEL'] )
		) {
		...
		if ( isset( $translations[ $this->tableName() ][ $va_tmp[1] ] )
		     && is_array( $translations[ $this->tableName() ][ $va_tmp[1] ] )
		     && isset( $translations[ $this->tableName() ][ $va_tmp[1] ]['LABEL'] )
		) {
	...
	/**
	 * Returns display description for element specified by standard "get" bundle code (eg. <table_name>.<bundle_name>
	 * format)
	 */
	...
	/**
	 * Returns HTML search form input widget for bundle specified by standard "get" bundle code (eg.
	 * <table_name>.<bundle_name> format) This method handles generation of search form widgets for intrinsic fields in
	 * the primary table. If this method can't handle the bundle (because it is not an intrinsic field in the primary
	 * table...) it will return null.
	 *
	 ...
	 *
	 * @return string HTML text of form element. Will return null if it is not possible to generate an HTML form widget
	 *                for the bundle.
	 */
	public function htmlFormElementForSearch( $po_request, $ps_field, $pa_options = null ) {
		...
		if ( in_array( $va_tmp[0], array( 'created', 'modified' ) ) ) {
			...
			return caHTMLTextInput( $ps_field, array(
				...
				'width'  => ( isset( $pa_options['width'] ) && ( $pa_options['width'] > 0 ) ) ? $pa_options['width']
					: 30,
				'height' => ( isset( $pa_options['height'] ) && ( $pa_options['height'] > 0 ) ) ? $pa_options['height']
					: 1,
				'value'  => $value
			) );
		}
		...
		if ( $this->hasField( $va_tmp[1] ) ) {
			...
			$value = caGetOption( [ $ps_field, $n ], $pa_options['values'],
				'' );    // use field as well as name when looking for default value
			if ( is_null( $value ) && $use_current_row_value ) {
				$value = $this->get( $ps_field );
			}
			...
			return $this->htmlFormElement( $va_tmp[1], '^ELEMENT', array_merge( $pa_options, array(
				...
				'id'          => caGetOption( 'id', $pa_options,
						str_replace( ".", "_", caGetOption( 'name', $pa_options, $ps_field ) ) )
				                 . ( caGetOption( 'autocomplete', $pa_options, false ) ? "_autocomplete" : "" ),
				'nullOption'  => '-',
				'classname'   => ( isset( $pa_options['class'] ) ? $pa_options['class'] : '' ),
				'value'       => $value,
				'width'       => ( isset( $pa_options['width'] ) && ( $pa_options['width'] > 0 ) )
					? $pa_options['width'] : 30,
				'height'      => ( isset( $pa_options['height'] ) && ( $pa_options['height'] > 0 ) )
					? $pa_options['height'] : 1,
				'no_tooltips' => true
			) ) );
		}
	...
	/**
	 * Returns HTML form input widget for bundle specified by standard "get" bundle code (eg.
	 * <table_name>.<bundle_name> format) suitable for use in a simple data entry form, such as the front-end
	 * "contribute" user-provided content submission form
	 *
	 * This method handles generation of search form widgets for intrinsic fields in the primary table. If this method
	 * can't handle the bundle (because it is not an intrinsic field in the primary table...) it will return null.
	 *
	 * @param $po_request HTTPRequest
	 * @param $ps_field   string
	 * @param $pa_options array Options include:
	...
	public function htmlFormElementForSimpleForm( $po_request, $ps_field, $pa_options = null ) {
		...
		if ( $this->hasField( $va_tmp[1] ) ) {
			...
			if ( $this->getProperty( 'ID_NUMBERING_ID_FIELD' ) == $va_tmp[1] ) {
				...
				return $this->htmlFormElement( $va_tmp[1],
					$this->getAppConfig()->get( 'idno_element_display_format_without_label' ),
					array_merge( $pa_options, array(
						'name'               => $ps_field,
	 ...
	 * the row_id and snapshot options to manually specify what gets logged. Manual logging is only supported for
	 * intrinsic fields in tables. You cannot log metadata attribute changes manually.
	 *
	 * @access private
	 *
	 ...
	 * @param array  $pa_options     Options include:
	 *                               row_id = Force logging for specified row_id. [Default is to use id from currently
	 *                               loaded row] snapshot = Row snapshot array to use for logging. [Default is to use
	 *                               snapshot from currently loaded row] log_id = Force logging using a specific
	 *                               log_id. [Default is to use next available log_id]
	 */
	public function logChange( $ps_change_type, $pn_user_id = null, $pa_options = null ) {
		...
		if ( ! ( $vn_row_id = caGetOption( 'row_id', $pa_options, null ) )
		     && ! ( $vn_row_id
				= $this->getPrimaryKey() )
		) {
			return false;
		...
		if ( $vb_is_metadata ) {
			...
			if ( ( $ps_change_type !== 'D' ) && ! caGetOption( 'forceLogChange', $pa_options, false )
			     && ! sizeof( $this->getChangedFieldValuesArray() )
			) {
				return null;
			...
			if ( is_array( $va_subject_config['RELATED_TABLES'] ) && ! $vb_is_metadata && ! $vb_is_metadata_value ) {
				...
				foreach ( $va_subject_config['RELATED_TABLES'] as $vs_dest_table => $va_path_to_dest ) {
					...
					foreach ( $va_path_to_dest as $vs_ltable ) {
						...
						$vs_sql       .= "INNER JOIN $vs_ltable ON $vs_cur_table."
						                 . $va_relations[ $vs_cur_table ][ $vs_ltable ][0][0] . " = $vs_ltable."
						                 . $va_relations[ $vs_cur_table ][ $vs_ltable ][0][1] . "\n";
						$vs_cur_table = $vs_ltable;
					}
					$vs_sql .= "WHERE " . $this->tableName() . "." . $this->primaryKey() . " = " . $vn_row_id;

					if ( $qr_subjects = $o_db->query( $vs_sql ) ) {
						if ( ! isset( $va_subjects[ $vn_dest_table_num ] )
						     || ! is_array( $va_subjects[ $vn_dest_table_num ] )
						) {
		...
		if ( ! ( ( $ps_change_type == 'U' ) && ( ! sizeof( $va_snapshot ) ) ) ) {
			...
			if ( $g_change_log_delegate && method_exists( $g_change_log_delegate, "onLogChange" ) ) {
				call_user_func( array( $g_change_log_delegate, 'onLogChange' ), $this->tableNum(), $vn_row_id,
					$vn_log_id );
			}
	 ...
	 * @access public
	 *
	 * @param int   $pn_row_id  Return change log for row with specified primary key id. If omitted currently loaded
	 *                          record is used.
	 * @param array $pa_options Array of options. Valid options are:
	 *                          range = optional range to restrict returned entries to. Should be array with 0th key
	 *                          set to start and 1st key set to end of range. Both values should be Unix timestamps.
	 *                          You can also use 'start' and 'end' as keys if desired. limit = maximum number of
	 *                          entries returned. Omit or set to zero for no limit. [default=all] forTable = if true
	 *                          only return changes made directly to the current table (ie. omit changes to related
	 *                          records that impact this record [default=false] excludeUnitID = if set, log records
	 *                          with the specific unit_id are not returned [default=not set] changeType = if set to I,
	 *                          U or D, will limit change log to inserts, updates or deletes respectively. If not set
	 *                          all types are returned.
	 *
	 * @return array Change log data
	 */
	public function getChangeLog( $pn_row_id = null, $pa_options = null ) {
		$pa_datetime_range           = ( isset( $pa_options['range'] ) && is_array( $pa_options['range'] ) )
			? $pa_options['range'] : null;
		$pn_max_num_entries_returned = ( isset( $pa_options['limit'] ) && (int) $pa_options['limit'] )
			? (int) $pa_options['limit'] : 0;
		$pb_for_table                = ( isset( $pa_options['forTable'] ) ) ? (bool) $pa_options['forTable'] : false;
		$ps_exclude_unit_id          = ( isset( $pa_options['excludeUnitID'] ) && $pa_options['excludeUnitID'] )
			? $pa_options['excludeUnitID'] : null;
		$ps_change_type              = ( isset( $pa_options['changeType'] )
		                                 && in_array( $pa_options['changeType'], array( 'I', 'U', 'D' ) ) )
			? $pa_options['changeType'] : null;
		...
		if ( ! $this->opqs_get_change_log ) {
			...
			if ( $pb_for_table ) {
				if ( ! ( $this->opqs_get_change_log = $o_db->prepare( "
					...
					WHERE
						(
							(wcl.logged_table_num = " . ( (int) $this->tableNum() ) . ") AND " . ( ( $ps_change_type )
						? "(wcl.changetype = '" . $ps_change_type . "') AND " : "" ) . "
							(wcl.logged_row_id = ?)
						)
						{$vs_daterange_sql}
				...
				if ( ! ( $this->opqs_get_change_log = $o_db->prepare( "
					...
					WHERE
						 (
							(wcls.subject_table_num = " . ( (int) $this->tableNum() ) . ") AND " . ( ( $ps_change_type )
						? "(wcl.changetype = '" . $ps_change_type . "') AND " : "" ) . "
							(wcls.subject_row_id  = ?)
						)
						{$vs_daterange_sql}
		...
		if ( $pb_for_table ) {
			$qr_log = $this->opqs_get_change_log->execute( $vs_daterange_sql ? array(
				(int) $pn_row_id,
				(int) $vn_start,
				(int) $vn_end
			) : array( (int) $pn_row_id ) );
			...
			$qr_log = $this->opqs_get_change_log->execute( $vs_daterange_sql ? array(
				...
				(int) $vn_start,
				(int) $vn_end
			) : array( (int) $pn_row_id, (int) $pn_row_id ) );
		}
		...
		while ( $qr_log->nextRow() ) {
			...
			$va_log[ sizeof( $va_log ) - 1 ]['snapshot'] = caUnserializeForDatabase( $va_log[ sizeof( $va_log )
			                                                                                  - 1 ]['snapshot'] );
		}
	 ...
	 *
	 * @param int   $pn_row_id  If set information is returned about the specified row instead of the currently loaded
	 *                          one.
	 * @param array $pa_options Array of options. Supported options are:
	 *                          timestampOnly = if set to true an integer Unix timestamp for the date/time of creation
	 *                          is returned. If false (default) then an array is returned with timestamp and user
	 *                          information about the creation.
	 *
	 * @return mixed An array detailing the date/time and creator of the row. Array includes the user_id, fname, lname
	 *               and email of the user who executed the change as well as an integer Unix-style timestamp. If the
	 *               timestampOnly option is set then only the timestamp is returned.
	 */
	 ...
	 *                          one.
	 * @param array $pa_options Array of options. Supported options are:
	 *                          timestampOnly = if set to true an integer Unix timestamp for the last change is
	 *                          returned. If false (default) then an array is returned with timestamp and user
	 *                          information about the change.
	 *
	 * @return mixed An array detailing the date/time and initiator of the last change. Array includes the user_id,
	 *               fname, lname and email of the user who executed the change as well as an integer Unix-style
	 *               timestamp. If the timestampOnly option is set then only the timestamp is returned.
	 */
	 ...
	 *                     additionalTableToJoin = name of table to join to hierarchical table (and return fields
	 *                     from); only fields related many-to-one are currently supported idsOnly = return simple array
	 *                     of primary key values for child records rather than full result sort =
	 *
	...
	public function &getHierarchy( $pn_id = null, $pa_options = null ) {
		...
		if ( $this->isHierarchical() ) {
			...
			if ( ! ( $vs_rank_fld = caGetOption( 'sort', $pa_options, $this->getProperty( 'RANK' ) ) )
			     || ! $this->hasField( $vs_rank_fld )
			) {
			...
			$o_db    = $this->getDb();
			$qr_root = $o_db->query( "
				SELECT $vs_hier_left_fld, $vs_hier_right_fld " . ( ( $this->hasField( $vs_hier_id_fld ) )
					? ", $vs_hier_id_fld" : "" ) . "
				FROM " . $this->tableName() . "
				WHERE
					(" . $this->primaryKey() . " = ?)		
			...
			if ( $o_db->numErrors() ) {
				...
				if ( $qr_root->nextRow() ) {
					...
					$va_count = array();
					if ( ( $this->hasField( $vs_hier_id_fld ) )
					     && ( ! ( $vn_hierarchy_id
							= $t_instance->get( $vs_hier_id_fld ) ) )
					     && ( ! ( $vn_hierarchy_id = $qr_root->get( $vs_hier_id_fld ) ) )
					) {
					...
					if ( isset( $pa_options['additionalTableToJoin'] ) && ( $pa_options['additionalTableToJoin'] ) ) {
						...
						if ( isset( $pa_options['additionalTableJoinType'] )
						     && ( $pa_options['additionalTableJoinType'] === 'LEFT' )
						) {
							$ps_additional_table_join_type = 'LEFT';
						}
						if ( is_array(
							$va_rel = Datamodel::getOneToManyRelations( $vs_table_name, $ps_additional_table_to_join ) )
						) {
							// one-many rel
							$va_sql_joins[]
								= "{$ps_additional_table_join_type} JOIN {$ps_additional_table_to_join} ON {$vs_table_name}"
								  . '.' . $va_rel['one_table_field'] . " = {$ps_additional_table_to_join}."
								  . $va_rel['many_table_field'];
						} else {
						...
						if ( isset( $pa_options['additionalTableWheres'] )
						     && is_array( $pa_options['additionalTableWheres'] )
						) {
							$va_additional_table_wheres = $pa_options['additionalTableWheres'];
						}

						$vs_additional_wheres = '';
						if ( is_array( $va_additional_table_wheres )
						     && ( sizeof( $va_additional_table_wheres ) > 0 )
						) {
					...
					$vs_deleted_sql = '';
					if ( $this->hasField( 'deleted' )
					     && ( ! isset( $pa_options['returnDeleted'] )
					          || ( ! $pa_options['returnDeleted'] ) )
					) {
					...
					$vs_sql = "
						...
						{$vs_sql_joins}
						WHERE
							({$vs_table_name}.{$vs_hier_left_fld} BETWEEN " . $qr_root->get( $vs_hier_left_fld )
					          . " AND " . $qr_root->get( $vs_hier_right_fld ) . ")
							{$vs_hier_id_sql}
							{$vs_deleted_sql}
							{$vs_additional_wheres}
						ORDER BY
	 ...
	 * @param array $pa_options
	 *
	 *        additionalTableToJoin: name of table to join to hierarchical table (and return fields from); only fields
	 *        related many-to-one are currently supported idsOnly = return simple array of primary key values for child
	 *        records rather than full data array returnDeleted = return deleted records in list (def. false) maxLevels
	 *        = dontIncludeRoot = includeSelf =
	 *
	 * @return array
	 */
			...
			WHERE
				" . ( ( $pn_id > 0 ) ? "({$vs_hier_left_fld} >= ?) AND ({$vs_hier_right_fld} <= ?) " : '' ) .
		                        ( $vs_hier_id_fld ? ' ' . ( ( $pn_id > 0 ) ? ' AND ' : '' ) . "({$vs_hier_id_fld} = ?)"
			                        : '' ) .
		                        ( $this->hasField( 'deleted' ) ? ' ' . ( ( $vs_hier_id_fld ) ? ' AND ' : '' )
		                                                         . "(deleted = 0)" : '' )
		                        . "
		", $va_params );
	...
	public function getHierarchyChildCountsForIDs( $pa_ids, $pa_options = null ) {
		...
		$qr_res = $o_db->query( "
			...
			FROM {$vs_table_name}
			WHERE
				parent_id IN (?) " . ( ( sizeof( $va_additional_table_wheres ) ) ? " AND " . join( " AND ",
					$va_additional_table_wheres ) : "" ) . "
			GROUP BY parent_id
		", array( $pa_ids ) );
	 ...
	 * @param array, optional associative array of options. Valid keys for the array are:
	 *        additionalTableToJoin: name of table to join to hierarchical table (and return fields from); only fields
	 *        related many-to-one are currently supported returnChildCounts: if true, the number of children under each
	 *        returned child is calculated and returned in the result set under the column name 'child_count'. Note
	 *        that this count is always at least 1, even if there are no children. The 'has_children' column will be
	 *        null if the row has, in fact no children, or non-null if it does have children. You should check
	 *        'has_children' before using 'child_count' and disregard 'child_count' if 'has_children' is null.
	 *        returnDeleted = return deleted records in list (def. false) sort =
	 *
	 * @return DbResult
	 */
	public function &getHierarchyChildrenAsQuery( $pn_id = null, $pa_options = null ) {
		...
		$ps_additional_table_to_join = isset( $pa_options['additionalTableToJoin'] )
			? $pa_options['additionalTableToJoin'] : null;
		if ( $ps_additional_table_to_join ) {
			...
			if ( isset( $pa_options['additionalTableSelectFields'] )
			     && is_array( $pa_options['additionalTableSelectFields'] )
			) {
		...
		if ( $this->isHierarchical() ) {
			...
			if ( $ps_additional_table_to_join ) {
				if ( is_array(
					$va_rel = Datamodel::getOneToManyRelations( $this->tableName(), $ps_additional_table_to_join ) )
				) {
					// one-many rel
					$va_sql_joins[] = $ps_additional_table_join_type . " JOIN {$ps_additional_table_to_join} ON "
					                  . $this->tableName() . '.' . $va_rel['one_table_field']
					                  . " = {$ps_additional_table_to_join}." . $va_rel['many_table_field'];
				} else {
					// TODO: handle many-many cases
				}

				$t_additional_table_to_join           = Datamodel::getInstance( $ps_additional_table_to_join );
				$vs_additional_table_to_join_group_by = ', ' . $ps_additional_table_to_join . '.'
				                                        . $t_additional_table_to_join->primaryKey();
			}
			$vs_sql_joins = join( "\n", $va_sql_joins );
			...
			if ( $pb_return_child_counts ) {
				$vs_additional_child_join_conditions = sizeof( $va_additional_child_join_conditions ) ? " AND "
				                                                                                        . join( " AND ",
						$va_additional_child_join_conditions ) : "";
				$qr_hier                             = $o_db->query( "
					SELECT " . $this->tableName() . ".* " . ( sizeof( $va_additional_table_select_fields ) ? ', '
				                                                                                             . join( ', ',
							$va_additional_table_select_fields ) : '' ) . ", count(*) child_count, sum(p2."
				                                                     . $this->primaryKey() . ") has_children
					FROM " . $this->tableName() . "
					{$vs_sql_joins}
					LEFT JOIN " . $this->tableName() . " AS p2 ON p2." . $vs_hier_parent_id_fld . " = "
				                                                     . $this->tableName() . "." . $this->primaryKey() . " {$vs_additional_child_join_conditions}
					WHERE
						(" . $this->tableName() . ".{$vs_hier_parent_id_fld} = ?) "
				                                                     . ( ( sizeof( $va_additional_table_wheres ) > 0 )
						? ' AND ' . join( ' AND ', $va_additional_table_wheres ) : '' ) . "
					GROUP BY
				...
				$qr_hier = $o_db->query( "
					...
					WHERE
						(" . $this->tableName() . ".{$vs_hier_parent_id_fld} = ?) "
				                         . ( ( sizeof( $va_additional_table_wheres ) > 0 ) ? ' AND ' . join( ' AND ',
							$va_additional_table_wheres ) : '' ) . "
					ORDER BY
						" . $vs_order_by . "
				", (int) $pn_id );
	 ...
	 *        that this count is always at least 1, even if there are no children. The 'has_children' column will be
	 *        null if the row has, in fact no children, or non-null if it does have children. You should check
	 *        'has_children' before using 'child_count' and disregard 'child_count' if 'has_children' is null. idsOnly:
	 *        if true, only the primary key id values of the children records are returned returnDeleted = return
	 *        deleted records in list [default=false] start = Offset to start returning records from [default=0; no
	 *        offset] limit = Maximum number of records to return [default=null; no limit]
	 *
	 * @return array
	 */
	 ...
	 *        additionalTableToJoin: name of table to join to hierarchical table (and return fields from); only fields
	 *        related many-to-one are currently supported returnChildCounts: if true, the number of children under each
	 *        returned sibling is calculated and returned in the result set under the column name 'sibling_count'.d
	 *        idsOnly: if true, only the primary key id values of the chidlren records are returned returnDeleted =
	 *        return deleted records in list (def. false)
	 *
	 * @return array
	 */
	public function getHierarchySiblings( $pn_id = null, $pa_options = null ) {
		...
		if ( $qr_parent = $this->getDb()->query( "
			...
			if ( $qr_parent->nextRow() ) {
				...
				$this->postError( 250,
					_t( 'Could not get parent_id to load siblings by: %1', join( ';', $this->getDb()->getErrors() ) ),
					'BaseModel->getHierarchySiblings' );

				return false;
			}
		} else {
			$this->postError( 250,
				_t( 'Could not get hierarchy siblings: %1', join( ';', $this->getDb()->getErrors() ) ),
				'BaseModel->getHierarchySiblings' );
	...
	public function &getHierarchyAncestors( $pn_id = null, $pa_options = null ) {
		...
		if ( $this->isHierarchical() ) {
			...
			if ( $ps_additional_table_to_join ) {
				...
				switch ( sizeof( $va_path ) ) {
					case 2:
						...
						$va_sql_joins[] = $ps_additional_table_join_type . " JOIN {$ps_additional_table_to_join} ON "
						                  . $vs_table_name . '.'
						                  . $va_rels[ $ps_additional_table_to_join ][ $vs_table_name ][0][1]
						                  . " = {$ps_additional_table_to_join}."
						                  . $va_rels[ $ps_additional_table_to_join ][ $vs_table_name ][0][0];
						break;
					case 3:
						// TODO: handle many-many cases
						break;
			...
			$o_db    = $this->getDb();
			$qr_root = $o_db->query( "
				SELECT {$vs_table_name}.* " . ( sizeof( $va_additional_table_select_fields ) ? ', ' . join( ', ',
						$va_additional_table_select_fields ) : '' ) . "
				FROM {$vs_table_name}
				{$vs_sql_joins}
				WHERE
					({$vs_table_name}." . $this->primaryKey() . " = ?) " . ( ( sizeof( $va_additional_table_wheres )
			                                                                   > 0 ) ? ' AND ' . join( ' AND ',
						$va_additional_table_wheres ) : '' ) . "
			", intval( $pn_id ) );
			...
			if ( $o_db->numErrors() ) {
				...
				if ( $qr_root->numRows() ) {
					...
					if ( $vn_parent_id ) {
						do {
							$vs_sql = "
								SELECT {$vs_table_name}.* " . ( sizeof( $va_additional_table_select_fields ) ? ', '
							                                                                                   . join( ', ',
										$va_additional_table_select_fields ) : '' ) . "
								FROM {$vs_table_name} 
								{$vs_sql_joins}
								WHERE ({$vs_table_name}." . $this->primaryKey() . " = ?) "
							          . ( ( sizeof( $va_additional_table_wheres ) > 0 ) ? ' AND ' . join( ' AND ',
										$va_additional_table_wheres ) : '' ) . "
							";
	 ...
	 * @param array  $pa_options Any options supported by BaseModel::getHierarchyAsList() and caProcessTemplateForIDs()
	 *                           as well as: sort = An array or semicolon delimited list of elements to sort on.
	 *                           [Default is null] sortDirection = Direction to sorting, either 'asc' (ascending) or
	...
	public function hierarchyWithTemplate( $ps_template, $pa_options = null ) {
		...
		$va_hier = $this->getHierarchyAsList( $pn_id,
			array_merge( $pa_options, array( 'idsOnly' => false, 'sort' => null ) ) );
		...
		$va_vals = caProcessTemplateForIDs( $ps_template, $this->tableName(), $va_ids, array_merge( $pa_options,
			array( 'indexWithIDs' => true, 'includeBlankValuesInArray' => true, 'returnAsArray' => true ) ) );
		...
		if ( is_array( $pa_sort ) && sizeof( $pa_sort ) && sizeof( $va_ids ) ) {
			...
			return $this->_getFlattenedHierarchyArray( $va_hierarchy_data,
				$va_parent_ids[ $pn_id ] ? $va_parent_ids[ $pn_id ] : null, $ps_sort_direction );
		} else {
			foreach ( $va_vals as $vn_i => $vs_val ) {
	...
	private function _getFlattenedHierarchyArray( $pa_hierarchy_data, $pn_id, $ps_sort_direction = 'asc' ) {
		...
		foreach ( $pa_hierarchy_data[ $pn_id ] as $vs_sort_key => $va_item ) {
			$va_data[] = $va_item;
			$va_data   = array_merge( $va_data,
				$this->_getFlattenedHierarchyArray( $pa_hierarchy_data, $va_item['id'] ) );
		}
	...
	/**
	 * Return all ancestors for a list of row_ids. The list is an aggregation of ancestors for all of the row_ids. It
	 * will not possible to determine which row_ids have which ancestors from the returned value.
	 *
	 * @param array $pa_row_ids
	 * @param array $pa_options Options include:
	 *                          transaction = optional Transaction instance. If set then all database access is done
	 *                          within the context of the transaction returnAs = what to return; possible values are:
	 *                          searchResult            = a search result instance (aka. a subclass of
	 *                          BaseSearchResult), when the calling subclass is searchable (ie. <classname>Search and
	 *                          <classname>SearchResult classes are defined) ids                        = an array of
	 *                          ids (aka. primary keys) modelInstances            = an array of instances, one for each
	 *                          ancestor. Each instance is the same class as the caller, a subclass of BaseModel
	 *                          firstId                    = the id (primary key) of the first ancestor. This is the
	 *                          same as the first item in the array returned by 'ids' firstModelInstance        = the
	 *                          instance of the first ancestor. This is the same as the first instance in the array
	 *                          returned by 'modelInstances' count                    = the number of ancestors
	 *                          [Default is ids]
	 *
	 *        limit = if searchResult, ids or modelInstances is set, limits number of returned ancestoes. [Default is
	 *        no limit] includeSelf = Include initial row_id values in returned set [Default is false]
	 *
	 * @return mixed
	 */
	static public function getHierarchyAncestorsForIDs( $pa_row_ids, $pa_options = null ) {
		...
		$ps_return_as = caGetOption( 'returnAs', $pa_options, 'ids', array(
			'forceLowercase' => true,
			'validValues'    => array(
				'searchResult',
				'ids',
		...
		$vn_limit = ( isset( $pa_options['limit'] ) && ( (int) $pa_options['limit'] > 0 ) ) ? (int) $pa_options['limit']
			: null;
	...
	/**
	 * Return all children for a list of row_ids. The list is an aggregation of children for all of the row_ids. It
	 * will not possible to determine which row_ids have which children from the returned value.
	 *
	 * @param array $pa_row_ids
	 * @param array $pa_options Options include:
	 *                          transaction = optional Transaction instance. If set then all database access is done
	 ...
	 *                          <classname>SearchResult classes are defined) ids                        = an array of
	 *                          ids (aka. primary keys) modelInstances            = an array of instances, one for each
	 *                          children. Each instance is the same class as the caller, a subclass of BaseModel
	 *                          firstId                    = the id (primary key) of the first children. This is the
	 *                          same as the first item in the array returned by 'ids' firstModelInstance        = the
	 *                          instance of the first children. This is the same as the first instance in the array
	 *                          returned by 'modelInstances' count                    = the number of children
	 *                          [Default is ids]
	 *
	 *        limit = if searchResult, ids or modelInstances is set, limits number of returned children. [Default is no
	 *        limit]
	 *
	 * @return mixed
	 */
	...
	private function _rebuildHierarchicalIndex( $pn_parent_id, $pn_hier_left ) {
		...
		if ( $o_db->numErrors() ) {
			...
			$qr_up = $o_db->query( "
				...
				SET " . $vs_hier_left_fld . " = " . intval( $pn_hier_left ) . ", " . $vs_hier_right_fld . " = "
			                       . intval( $vn_hier_right ) . "
				WHERE 
					(" . $vs_pk . " = ?)
			", intval( $pn_parent_id ) );
	...
	public function htmlFormElement( $ps_field, $ps_format = null, $pa_options = null ) {
		...
		foreach (
			array(
				'display_form_field_tips',
				'classname',
				'maxOptionLength',
				'textAreaTagName',
				'display_use_count',
				'display_omit_items__with_zero_count',
				'display_use_count_filters',
				'display_use_count_filters',
				'selection',
				'name',
				'value',
				...
				'size',
				'multiple',
				'show_text_field_for_vars',
				'nullOption',
				'empty_message',
				'displayMessageForFieldValues',
				'DISPLAY_FIELD',
				'WHERE',
				'select_item_text',
				'hide_select_if_only_one_option',
				'field_errors',
				'display_form_field_tips',
				'form_name',
				'no_tooltips',
		...
		foreach (
			array(
				...
				'DISPLAY_USE_COUNT',
				'DISPLAY_SHOW_COUNT',
				'DISPLAY_OMIT_ITEMS_WITH_ZERO_COUNT',
				'DISPLAY_TYPE',
				'IS_NULL',
				'DEFAULT_ON_NULL',
				'DEFAULT',
		...
		$vn_display_width  = ( isset( $pa_options['width'] ) && ( $pa_options['width'] > 0 ) ) ? $pa_options['width']
			: $va_attr["DISPLAY_WIDTH"];
		$vn_display_height = ( isset( $pa_options['height'] ) && ( $pa_options['height'] > 0 ) ) ? $pa_options['height']
			: $va_attr["DISPLAY_HEIGHT"];
		...
		$vs_field_label = ( isset( $pa_options['label'] ) && ( strlen( $pa_options['label'] ) > 0 ) )
			? $pa_options['label'] : $va_attr["LABEL"];
...
// TODO: PULL THIS FROM A CONFIG FILE
		...
		if ( ! isset( $va_attr["DISPLAY_SHOW_COUNT"] )
		     || ! ( $vb_display_show_count
				= (boolean) $va_attr["DISPLAY_SHOW_COUNT"] )
		) {
			$vb_display_show_count = isset( $pa_options["display_show_count"] )
				? (boolean) $pa_options["display_show_count"] : null;
		}
		if ( ! isset( $va_attr["DISPLAY_OMIT_ITEMS_WITH_ZERO_COUNT"] )
		     || ! ( $vb_display_omit_items__with_zero_count
				= (boolean) $va_attr["DISPLAY_OMIT_ITEMS_WITH_ZERO_COUNT"] )
		) {
			$vb_display_omit_items__with_zero_count = isset( $pa_options["display_omit_items__with_zero_count"] )
				? (boolean) $pa_options["display_omit_items__with_zero_count"] : null;
		}
		if ( ! isset( $va_attr["DISPLAY_OMIT_ITEMS_WITH_ZERO_COUNT"] )
		     || ! ( $va_display_use_count_filters
				= $va_attr["DISPLAY_USE_COUNT_FILTERS"] )
		) {
			$va_display_use_count_filters = isset( $pa_options["display_use_count_filters"] )
				? $pa_options["display_use_count_filters"] : null;
		}
		...
		if ( $va_attr ) {
			...
			if ( ( $va_attr["DISPLAY_TYPE"] == DT_HIDDEN ) || ( caGetOption( 'hidden', $pa_options, false ) ) ) {
				return '<input type="hidden" name="' . $pa_options["name"] . '" value="'
				       . $this->escapeHTML( $vm_field_value ) . '"/>';
			}
			...
			if ( isset( $pa_options["size"] ) && ( $pa_options["size"] > 0 ) ) {
				...
				if ( ( ( $va_attr["DISPLAY_TYPE"] == DT_LIST_MULTIPLE ) || ( $va_attr["DISPLAY_TYPE"] == DT_LIST ) )
				     && ( $vn_display_height > 1 )
				) {
			...
			if ( $vs_is_multiple = ( ( isset( $pa_options["multiple"] ) && $pa_options["multiple"] )
			                         || ( $va_attr["DISPLAY_TYPE"] == DT_LIST_MULTIPLE ) ? "multiple='1'" : "" )
			) {
				$vs_multiple_name_extension = '[]';
			...
			switch ( $va_attr["FIELD_TYPE"] ) {
				...
				case( FT_VARS ):
					...
					if ( $va_attr['DISPLAY_TYPE'] == DT_COUNTRY_LIST ) {
						$vs_element = caHTMLSelect( $ps_field, caGetCountryList(), array( 'id' => $ps_field ),
							array( 'value' => $vm_field_value ) );
						...
						if ( $va_attr['STATEPROV_FIELD'] ) {
							...
							$vs_element .= "
								jQuery('#{$ps_field}').click({countryID: '{$ps_field}', stateProvID: '"
							               . $va_attr['STATEPROV_FIELD'] . "', value: '"
							               . addslashes( $this->get( $va_attr['STATEPROV_FIELD'] ) ) . "', statesByCountryList: caStatesByCountryList}, caUI.utils.updateStateProvinceForCountry);
								jQuery(document).ready(function() {
									caUI.utils.updateStateProvinceForCountry({data: {countryID: '{$ps_field}', stateProvID: '"
							               . $va_attr['STATEPROV_FIELD'] . "', value: '"
							               . addslashes( $this->get( $va_attr['STATEPROV_FIELD'] ) ) . "', statesByCountryList: caStatesByCountryList}});
								});
							";
					...
					if ( $va_attr['DISPLAY_TYPE'] == DT_STATEPROV_LIST ) {
						$vs_element = caHTMLSelect( $ps_field . '_select', array(),
							array( 'id' => $ps_field . '_select' ), array( 'value' => $vm_field_value ) );
						$vs_element .= caHTMLTextInput( $ps_field . '_name',
							array( 'id' => $ps_field . '_text', 'value' => $vm_field_value ) );
						break;
					}


					if ( ( $vn_display_width > 0 )
					     && ( in_array( $va_attr["DISPLAY_TYPE"], array( DT_SELECT, DT_LIST, DT_LIST_MULTIPLE ) ) )
					) {
						#
						...
						if ( $vs_list_code ) {
							...
							$va_list_attrs = array(
								'id'    => $pa_options['id'],
								'class' => caGetOption( 'classname', $pa_options, null )
							);
							//if ($vn_max_pixel_width) { $va_list_attrs['style'] = $vs_width_style; }

							if ( method_exists( $this, 'getTypeFieldName' )
							     && ( $ps_field == $this->getTypeFieldName() )
							) {
								$va_limit_list = caGetTypeListForUser( $this->tableName(),
									array( 'access' => __CA_BUNDLE_ACCESS_EDIT__ ) );
							}
							...
							if ( isset( $pa_options['hide_select_if_no_options'] )
							     && $pa_options['hide_select_if_no_options']
							     && ( ! $vs_element )
							) {
								$vs_element = "";
							...
							// -----
							$va_many_to_one_relations = Datamodel::getManyToOneRelations( $this->tableName() );
							if ( isset( $va_many_to_one_relations[ $ps_field ] )
							     && $va_many_to_one_relations[ $ps_field ]
							) {
								#
								# Use foreign  key to populate <select>
								#
								$o_one_table
									                      = Datamodel::getInstance( $va_many_to_one_relations[ $ps_field ]["one_table"] );
								...
								if ( $o_one_table->isHierarchical() ) {
									...
									# Hierarchical <select>
									#
									$va_hier = $o_one_table->getHierarchyAsList( 0, $vs_display_use_count,
										$va_display_use_count_filters, $vb_display_omit_items__with_zero_count );
									if ( ! is_array( $va_hier ) ) {
										return '';
									}
									$va_display_fields = $va_attr["DISPLAY_FIELD"];
									if ( ! in_array( $vs_one_table_primary_key, $va_display_fields ) ) {
										$va_display_fields[] = $o_one_table->tableName() . "."
										                       . $vs_one_table_primary_key;
									}
									...
									if ( sizeof( $va_options ) == 0 ) {
										$vs_element = isset( $pa_options['empty_message'] )
											? $pa_options['empty_message'] : 'No options available';
									} else {
										$vs_element = "<select name='" . $pa_options["name"]
										              . $vs_multiple_name_extension . "' " . $vs_js . " "
										              . $vs_is_multiple . " " . $ps_size . " id='" . $pa_options["id"]
										              . $vs_multiple_name_extension
										              . "' {$vs_css_class_attr}  style='{$vs_dim_style}'"
										              . ( $pa_options['readonly'] ? ' disabled="disabled" ' : '' )
										              . ">\n";
										...
										if ( ! $pa_options["nullOption"] && $vb_is_null ) {
											...
											if ( $pa_options["nullOption"] ) {
												$vs_element .= "<option value=''>" . $pa_options["nullOption"]
												               . "</option>\n";
											}
										}

										foreach ( $va_options as $vn_val => $va_option_info ) {
											$vs_selected = ( ( $vn_val == $vm_field_value )
											                 || in_array( $vn_val, $va_selection ) )
												? "selected='selected'" : "";
											...
											$vs_use_count = "";
											if ( $vs_display_use_count && $vb_display_show_count
											     && ( $vn_val != "" )
											) {
											...
											if ( is_array( $pa_options['displayMessageForFieldValues'] ) ) {
												foreach (
													$pa_options['displayMessageForFieldValues'] as $vs_df => $va_df_vals
												) {
													if ( ( isset( $va_option_info[3][ $vs_df ] ) )
													     && is_array( $va_df_vals )
													) {
											...
											if (
												...
												&& ( strlen( $vs_option_text ) + strlen( $vs_use_count ) + $vn_indent
												     > $pa_options["maxOptionLength"] )
											) {
												if ( ( $vn_strlen = $pa_options["maxOptionLength"]
												                    - strlen( $vs_indent ) - strlen( $vs_use_count )
												                    - 3 ) < $pa_options["maxOptionLength"]
												) {
											...
											$vs_element .= $vs_indent . $vs_option_text . $vs_use_count
											               . $vs_display_message . "</option>\n";
										}
										$vs_element .= "</select>\n";
									}
								} else {
									...
									if ( isset( $pa_options["WHERE"] )
									     && ( is_array( $pa_options["WHERE"] )
									          && ( $vs_where = join( " AND ", $pa_options["WHERE"] ) ) )
									     || ( ( is_array( $va_attr["DISPLAY_WHERE"] ) )
									          && ( $vs_where = join( " AND ", $va_attr["DISPLAY_WHERE"] ) ) )
									) {
										$vs_sql .= " WHERE $vs_where ";
									}

									if ( ( isset( $va_attr["DISPLAY_ORDERBY"] ) ) && ( $va_attr["DISPLAY_ORDERBY"] )
									     && ( $vs_orderby = join( ",", $va_attr["DISPLAY_ORDERBY"] ) )
									) {
									...
									if ( isset( $pa_options["nullOption"] ) && $pa_options["nullOption"] ) {
										$va_opts[ $pa_options["nullOption"] ] = array(
											$pa_options["nullOption"],
											null
										);
									} else {
										if ( $vb_is_null ) {
									...
									if ( $pa_options["select_item_text"] ) {
										$va_opts[ $pa_options["select_item_text"] ]
											= array( $pa_options["select_item_text"], null );
									}
									...
									while ( $qr_res->nextRow() ) {
										...
										$va_opts[] = array(
											$vs_display,
											$qr_res->get( $vs_one_table_primary_key ),
											$qr_res->getRow()
										);
									}
									...
									if ( sizeof( $va_opts ) == 0 ) {
										...
										if ( isset( $pa_options['hide_select_if_only_one_option'] )
										     && $pa_options['hide_select_if_only_one_option']
										     && ( sizeof( $va_opts ) == 1 )
										) {

											$vs_element = "<input type='hidden' name='" . $pa_options["name"] . "' "
											              . $vs_js . " " . $ps_size . " id='" . $pa_options["id"]
											              . "' value='" . ( $vm_field_value ? $vm_field_value
													: $va_opts[0][1] ) . "' {$vs_css_class_attr}/>";
											$ps_format  = '^ERRORS^ELEMENT';
										} else {
											$vs_element = "<select name='" . $pa_options["name"]
											              . $vs_multiple_name_extension . "' " . $vs_js . " "
											              . $vs_is_multiple . " " . $ps_size . " id='"
											              . $pa_options["id"] . $vs_multiple_name_extension
											              . "' {$vs_css_class_attr} style='{$vs_dim_style}'"
											              . ( $pa_options['readonly'] ? ' disabled="disabled" ' : '' )
											              . ">\n";
											foreach ( $va_opts as $va_opt ) {
												...
												$vs_value       = $va_opt[1];
												$vs_selected    = ( ( $vs_value == $vm_field_value )
												                    || in_array( $vs_value, $va_selection ) )
													? "selected='selected'" : "";
												...
												if (
													( $pa_options["maxOptionLength"] )
													&& ( strlen( $vs_option_text ) + strlen( $vs_use_count )
													     > $pa_options["maxOptionLength"] )
												) {
													$vs_option_text = mb_substr( $vs_option_text, 0,
															$pa_options["maxOptionLength"] - 3
															- strlen( $vs_use_count ) ) . "...";
												}
												...
												if ( is_array( $pa_options['displayMessageForFieldValues'] ) ) {
													foreach (
														$pa_options['displayMessageForFieldValues'] as $vs_df =>
														$va_df_vals
													) {
														if ( ( isset( $va_opt[2][ $vs_df ] ) )
														     && is_array( $va_df_vals )
														) {
								...
								if ( isset( $va_attr['LIST'] ) && $va_attr['LIST'] ) {
									// NOTE: "raw" field value (value passed into method, before the model default value is applied) is used so as to allow the list default to be used if needed
									$vs_element = ca_lists::getListAsHTMLFormElement( $va_attr['LIST'],
										$pa_options["name"] . $vs_multiple_name_extension,
										array( 'class' => $pa_options['classname'], 'id' => $pa_options['id'] ), array(
											'key'         => 'item_value',
											'value'       => $vm_raw_field_value,
											'nullOption'  => $pa_options['nullOption'],
											'readonly'    => $pa_options['readonly'],
											'checkAccess' => $pa_options['checkAccess'],
											'table'       => $this->tableName(),
											'type'        => method_exists( $this, 'getTypeCode' )
												? $this->getTypeCode() : null
										) );
								}
								if ( ! $vs_element
								     && ( isset( $va_attr["BOUNDS_CHOICE_LIST"] )
								          && is_array( $va_attr["BOUNDS_CHOICE_LIST"] ) )
								) {
									...
									if ( sizeof( $va_attr["BOUNDS_CHOICE_LIST"] ) == 0 ) {
										...
										if ( $pa_options["select_item_text"] ) {
											$vs_element .= "<option value=''>"
											               . $this->escapeHTML( $pa_options["select_item_text"] )
											               . "</option>\n";
										}
										if ( ! $pa_options["nullOption"] && $vb_is_null ) {
											$vs_element .= "<option value=''>" . _t( '- NONE -' ) . "</option>\n";
										...
										foreach ( $va_attr["BOUNDS_CHOICE_LIST"] as $vs_option => $vs_value ) {
											...
											$vs_selected = ( ( strval( $vs_value ) === strval( $vm_field_value ) )
											                 || in_array( $vs_value, $va_selection ) )
												? "selected='selected'" : "";

											if ( ( $pa_options["maxOptionLength"] )
											     && ( strlen( $vs_option ) > $pa_options["maxOptionLength"] )
											) {
												$vs_option = mb_substr( $vs_option, 0,
														$pa_options["maxOptionLength"] - 3 ) . "...";
											}

											$vs_element .= "<option value='$vs_value' $vs_selected>"
											               . $this->escapeHTML( $vs_option ) . "</option>\n";
										}
										$vs_element .= "</select>\n";
									}
								}
						...
						if ( $va_attr["DISPLAY_TYPE"] === DT_COLORPICKER ) {        // COLORPICKER
							$vs_element = '<input name="' . $pa_options["name"] . '" type="hidden" size="'
							              . ( $pa_options['size'] ? $pa_options['size'] : $vn_display_width )
							              . '" value="' . $this->escapeHTML( $vm_field_value ) . '" ' . $vs_js
							              . ' id=\'' . $pa_options["id"] . "' style='{$vs_dim_style}'/>\n";
							$vs_element .= '<div id="' . $pa_options["id"]
							               . '_colorchip" class="colorpicker_chip" style="background-color: #'
							               . $vm_field_value . '"><!-- empty --></div>';
							$vs_element .= "<script type='text/javascript'>jQuery(document).ready(function() { jQuery('#"
							               . $pa_options["name"] . "_colorchip').ColorPicker({
							...
							if ( $vn_display_height > 1 ) {
								$vs_element = '<' . $vs_text_area_tag_name . ' name="' . $pa_options["name"]
								              . '" rows="' . $vn_display_height . '" cols="' . $vn_display_width . '"'
								              . ( $pa_options['readonly'] ? ' readonly="readonly" disabled="disabled"'
										: '' ) . ' wrap="soft" ' . $vs_js . ' id=\'' . $pa_options["id"]
								              . "' style='{$vs_dim_style}' " . $vs_css_class_attr . ">"
								              . $this->escapeHTML( $vm_field_value ) . '</' . $vs_text_area_tag_name
								              . '>' . "\n";
							} else {
								$vs_element = '<input name="' . $pa_options["name"] . '" type="text" size="'
								              . ( $pa_options['size'] ? $pa_options['size'] : $vn_display_width ) . '"'
								              . ( $pa_options['readonly'] ? ' readonly="readonly" ' : '' ) . ' value="'
								              . $this->escapeHTML( $vm_field_value ) . '" ' . $vs_js . ' id=\''
								              . $pa_options["id"]
								              . "' {$vs_css_class_attr} style='{$vs_dim_style}'/>\n";
							}
	...
	caUI.initUniquenessChecker({
		...
		alreadyInUseMessage: '" . addslashes( _t( 'Value must be unique. Please try another.' ) ) . "'
								...
								if ( ( isset( $va_attr['LOOKUP'] ) && ( $va_attr['LOOKUP'] ) )
								     || $pa_options['lookup_url']
								) {
									if ( ( class_exists( "AppController" ) ) && ( $app = AppController::getInstance() )
									     && ( $req = $app->getRequest() )
									) {
										AssetLoadManager::register( 'jquery', 'autocomplete' );
										$vs_element .= "<script type='text/javascript'>
	jQuery('#" . $pa_options["id"] . "').autocomplete({ source: '" . ( $pa_options['lookup_url']
												? $pa_options['lookup_url']
												: caNavUrl( $req, 'lookup', 'Intrinsic', 'Get', array(
													'bundle' => $this->tableName() . ".{$ps_field}",
													"max"    => 500
							...
							if ( isset( $pa_options['usewysiwygeditor'] ) && $pa_options['usewysiwygeditor'] ) {
								...
								if ( ! is_array(
									$va_toolbar_config = $this->getAppConfig()->getAssoc( 'wysiwyg_editor_toolbar' ) )
								) {
					...
					switch ( $va_attr["DISPLAY_TYPE"] ) {
						...
						default:
							...
							if ( $vn_display_height > 1 ) {
								$vs_element = '<' . $vs_text_area_tag_name . ' name="' . $pa_options["name"]
								              . '" rows="' . $vn_display_height . '" cols="' . $vn_display_width
								              . '" wrap="soft" ' . $vs_js . ' ' . $vs_css_class_attr
								              . " style='{$vs_dim_style}'" . ( $pa_options['readonly']
										? ' readonly="readonly" ' : '' ) . ">" . $this->escapeHTML( $vm_field_value )
								              . '</' . $vs_text_area_tag_name . '>';
							} else {
								$vs_element = '<input type="text" name="' . $pa_options["name"] . '" value="'
								              . $this->escapeHTML( $vm_field_value )
								              . "\" size='{$vn_display_width}' {$vs_max_length} {$vs_js} {$vs_css_class_attr} style='{$vs_dim_style}'"
								              . ( $pa_options['readonly'] ? ' readonly="readonly" ' : '' ) . "/>";
							}
							break;
					}
					break;
				...
				case( FT_TIME ):
					...
					switch ( $va_attr["DISPLAY_TYPE"] ) {
						...
						default:
							...
							if ( $vn_display_height > 1 ) {
								$vs_element = '<' . $vs_text_area_tag_name . ' name="' . $pa_options["name"] . '" id="'
								              . $pa_options["id"] . '" rows="' . $vn_display_height . '" cols="'
								              . $vn_display_width . '" wrap="soft" ' . $vs_js . ' ' . $vs_css_class_attr
								              . " style='{$vs_dim_style}'" . ( $pa_options['readonly']
										? ' readonly="readonly" ' : '' ) . ">" . $this->escapeHTML( $vm_field_value )
								              . '</' . $vs_text_area_tag_name . '>';
							} else {
								$vs_element = '<input type="text" name="' . $pa_options["name"] . '" id="'
								              . $pa_options["id"] . '" value="' . $this->escapeHTML( $vm_field_value )
								              . "\" size='{$vn_display_width}' {$vs_max_length} {$vs_js} {$vs_css_class_attr}' style='{$vs_dim_style}'"
								              . ( $pa_options['readonly'] ? ' readonly="readonly" ' : '' ) . "/>";
							}
							break;
					...
					switch ( $va_attr["DISPLAY_TYPE"] ) {
						...
						default:
							...
							if ( $vn_display_height > 1 ) {
								...
								$vs_element = '<input type="text" name="' . $pa_options["name"] . '" id="'
								              . $pa_options["id"] . '" value="' . $this->escapeHTML( $vm_field_value )
								              . "\" size='{$vn_display_width}' {$vn_max_length} {$vs_js} {$vs_css_class_attr} style='{$vs_dim_style}'"
								              . ( $pa_options['readonly'] ? ' readonly="readonly" ' : '' ) . "/>";
							}
							break;
					}
				...
				case( FT_TIMECODE ):
					...
					if ( $vn_display_height > 1 ) {
						$vs_element = '<' . $vs_text_area_tag_name . ' name="' . $pa_options["name"] . '" id="'
						              . $pa_options["id"] . '" rows="' . $vn_display_height . '" cols="'
						              . $vn_display_width . '" wrap="soft" ' . $vs_js . ' ' . $vs_css_class_attr
						              . " style='{$vs_dim_style}'" . ( $pa_options['readonly'] ? ' readonly="readonly" '
								: '' ) . ">" . $this->escapeHTML( $vs_timecode ) . '</' . $vs_text_area_tag_name . '>';
					} else {
						$vs_element = '<input type="text" NAME="' . $pa_options["name"] . '" id="' . $pa_options["id"]
						              . '" value="' . $this->escapeHTML( $vs_timecode )
						              . "\" size='{$vn_display_width}' {$vs_max_length} {$vs_js} {$vs_css_class_attr} style='{$vs_dim_style}'"
						              . ( $pa_options['readonly'] ? ' readonly="readonly" ' : '' ) . "/>";
					}
					break;
				...
				case( FT_FILE ):
					...
					$vs_element = '<div class="formLabelUploadSizeNote"><input type="file" name="' . $pa_options["name"]
					              . '" id="' . $pa_options["id"] . '" ' . $vs_js . '/>'
					              . _t( "Maximum upload size is ${post_max_size}" ) . '</div>';

					// show current media icon
					if ( $vs_version = ( array_key_exists( 'displayMediaVersion', $pa_options ) )
						? $pa_options['displayMediaVersion'] : 'icon'
					) {
				...
				case( FT_PASSWORD ):
					...
					$vs_element = '<input type="password" name="' . $pa_options["name"] . '" id="' . $pa_options["id"]
					              . '" value="' . $this->escapeHTML( $vm_field_value ) . '" size="' . $vn_display_width
					              . '" ' . $vs_max_length . ' ' . $vs_js . ' autocomplete="off" ' . $vs_css_class_attr
					              . " style='{$vs_dim_style}'" . ( $pa_options['readonly'] ? ' readonly="readonly" '
							: '' ) . " " . ( $pa_options['placeholder'] ? "placeholder='"
					                                                      . htmlentities( $pa_options['placeholder'] )
					                                                      . "'" : "" ) . "/>";
					break;
				# ----------------------------
				case( FT_BIT ):
					switch ( $va_attr["DISPLAY_TYPE"] ) {
						case ( DT_FIELD ):
							$vs_element = '<input type="text" name="' . $pa_options["name"]
							              . "\" value='{$vm_field_value}' maxlength='1' size='2' {$vs_js} style='{$vs_dim_style}'"
							              . ( $pa_options['readonly'] ? ' readonly="readonly" ' : '' ) . "/>";
							break;
						case ( DT_SELECT ):
							$vs_element = "<select name='" . $pa_options["name"] . "' " . $vs_js . " id='"
							              . $pa_options["id"] . "' {$vs_css_class_attr} style='{$vs_dim_style}'"
							              . ( $pa_options['readonly'] ? ' disabled="disabled" ' : '' ) . ">\n";
							foreach ( array( "Yes" => 1, "No" => 0 ) as $vs_option => $vs_value ) {
								$vs_selected = ( $vs_value == $vm_field_value ) ? "selected='selected'" : "";
								$vs_element  .= "<option value='$vs_value' {$vs_selected}" . ( $pa_options['readonly']
										? ' disabled="disabled" ' : '' ) . ">" . _t( $vs_option ) . "</option>\n";
							}
							$vs_element .= "</select>\n";
							break;
						case ( DT_CHECKBOXES ):
							$vs_element = '<input type="checkbox" name="' . $pa_options["name"] . '" value="1" '
							              . ( $vm_field_value ? 'checked="1"' : '' ) . ' ' . $vs_js
							              . ( $pa_options['readonly'] ? ' disabled="disabled" ' : '' ) . ' id="'
							              . $pa_options["id"] . '"/>';
							break;
			...
			if ( isset( $pa_options['field_errors'] ) && is_array( $pa_options['field_errors'] )
			     && sizeof( $pa_options['field_errors'] )
			) {
			...
			if ( $ps_format != '' ) {
				...
				if (
					...
					|| ( ! isset( $pa_options["display_form_field_tips"] )
					     && $va_attr["DISPLAY_DESCRIPTION"] )
					|| ( ! isset( $pa_options["display_form_field_tips"] ) && ! isset( $va_attr["DISPLAY_DESCRIPTION"] )
					     && $vb_fl_display_form_field_tips )
				) {
					if ( preg_match( "/\^DESCRIPTION/", $ps_formatted_element ) ) {
						$ps_formatted_element = str_replace( "^LABEL", $vs_field_label, $ps_formatted_element );
						$ps_formatted_element = str_replace( "^DESCRIPTION",
							( ( isset( $pa_options["description"] ) && $pa_options["description"] )
								? $pa_options["description"] : $va_attr["DESCRIPTION"] ), $ps_formatted_element );
					} else {
						// no explicit placement of description text, so...
						$vs_field_id          = '_' . $this->tableName() . '_' . $this->getPrimaryKey() . '_'
						                        . $pa_options["name"] . '_' . $pa_options['form_name'];
						$ps_formatted_element = str_replace( "^LABEL",
							'<span id="' . $vs_field_id . '">' . $vs_field_label . '</span>', $ps_formatted_element );


						if ( ! isset( $pa_options['no_tooltips'] ) || ! $pa_options['no_tooltips'] ) {
							TooltipManager::add( '#' . $vs_field_id,
								"<h3>{$vs_field_label}</h3>" . ( ( isset( $pa_options["description"] )
								                                   && $pa_options["description"] )
									? $pa_options["description"] : $va_attr["DESCRIPTION"] ),
								$pa_options['tooltip_namespace'] );
						}
					}
					...
					if ( preg_match( "/\^SUB_DESCRIPTION/", $ps_formatted_element ) ) {
						$ps_formatted_element = str_replace( "^SUB_LABEL", $va_attr["SUB_LABEL"],
							$ps_formatted_element );
						$ps_formatted_element = str_replace( "^SUB_DESCRIPTION", $va_attr["SUB_DESCRIPTION"],
							$ps_formatted_element );
					} else {
						// no explicit placement of description text, so...
				...
				$ps_formatted_element = str_replace( "^EXTRA",
					isset( $pa_options['extraLabelText'] ) ? $pa_options['extraLabelText'] : '',
					$ps_formatted_element );
	 ...
	 * @param string $ps_list_code Optional list code or list_id to force return of, overriding the list configured in
	 *                             the model
	 *
	 * @return array A list of items, filtered on the current user locale; the format is the same as that returned by
	 *               ca_lists::getItemsForList()
	 */
	public function getFieldList( $ps_field, $ps_list_code = null ) {
		$t_list = new ca_lists();

		return caExtractValuesByUserLocale( $t_list->getItemsForList( $ps_list_code ? $ps_list_code
			: $this->getFieldListCode( $ps_field ) ) );
	}
	# --------------------------------------------------------------------------------------------
	 ...
	 *
	 * @param mixed  $pm_rel_table_name_or_num Table name (eg. "ca_entities") or number as defined in datamodel.conf of
	 *                                         table containing row to creation relationship to.
	 * @param int    $pn_rel_id                primary key value of row to creation relationship to.
	 * @param mixed  $pm_type_id               Relationship type type_code or type_id, as defined in the
	 *                                         ca_relationship_types table. This is required for all relationships that
	 *                                         use relationship types. This includes all of the most common types of
	 *                                         relationships.
	 * @param string $ps_effective_date        Optional date expression to qualify relation with. Any expression that
	 *                                         the TimeExpressionParser can handle is supported here.
	 * @param string $ps_source_info           Text field for storing information about source of relationship. Not
	 *                                         currently used.
	 * @param string $ps_direction             Optional direction specification for self-relationships (relationships
	 *                                         linking two rows in the same table). Valid values are 'ltor'
	 *                                         (left-to-right) and  'rtol' (right-to-left); the direction determines
	 *                                         which "side" of the relationship the currently loaded row is on: 'ltor'
	 *                                         puts the current row on the left side. For many self-relations the
	 *                                         direction determines the nature and display text for the relationship.
	 * @param array  $pa_options               Array of additional options:
	 *                                         allowDuplicates = if set to true, attempts to add a relationship that
	 *                                         already exists will succeed. Default is false - duplicate relationships
	 *                                         will not be created. setErrorOnDuplicate = if set to true, an error will
	 *                                         be set if an attempt is made to add a duplicate relationship. Default is
	 *                                         false - don't set error. addRelationship() will always return false when
	 *                                         creation of a duplicate relationship fails, no matter how the
	 *                                         setErrorOnDuplicate option is set.
	 *
	 * @return BaseRelationshipModel Loaded relationship model instance on success, false on error.
	 */
	public function addRelationship(
		$pm_rel_table_name_or_num, $pn_rel_id, $pm_type_id = null, $ps_effective_date = null, $ps_source_info = null,
		$ps_direction = null, $pn_rank = null, $pa_options = null
	) {
		if ( ! ( $va_rel_info = $this->_getRelationshipInfo( $pm_rel_table_name_or_num ) ) ) {
			$this->postError( 1240, _t( 'Related table specification "%1" is not valid', $pm_rel_table_name_or_num ),
				'BaseModel->addRelationship()' );
		...
		if ( $pm_type_id && ! is_numeric( $pm_type_id ) ) {
			...
			if ( $vs_linking_table = $t_rel_type->getRelationshipTypeTable( $this->tableName(),
				$t_item_rel->tableName() )
			) {
				$pn_type_id = $t_rel_type->getRelationshipTypeID( $vs_linking_table, $pm_type_id );
			} else {
				$this->postError( 2510, _t( 'Type id "%1" is not valid', $pm_type_id ),
					'BaseModel->addRelationship()' );
		...
		if ( ! is_numeric( $pn_rel_id ) ) {
			if ( $t_rel_item = Datamodel::getInstanceByTableName( $va_rel_info['related_table_name'], true ) ) {
				...
				if ( ( $vs_idno_fld = $t_rel_item->getProperty( 'ID_NUMBERING_ID_FIELD' ) )
				     && $t_rel_item->load( array( $vs_idno_fld => $pn_rel_id ) )
				) {
		...
		if ( ( ! isset( $pa_options['allowDuplicates'] ) || ! $pa_options['allowDuplicates'] )
		     && ! $this->getAppConfig()->get( 'allow_duplicate_relationships' )
		     && $this->relationshipExists( $pm_rel_table_name_or_num, $pn_rel_id, $pm_type_id, $ps_effective_date,
				$ps_direction )
		) {
			if ( isset( $pa_options['setErrorOnDuplicate'] ) && $pa_options['setErrorOnDuplicate'] ) {
				$this->postError( 1100, _t( 'Relationship already exists' ), 'BaseModel->addRelationship',
					$va_rel_info['related_table_name'] );
			}
		...
		if ( $va_rel_info['related_table_name'] == $this->tableName() ) {
			...
			if ( $pn_rel_id === $this->getPrimaryKey() ) {
				$this->postError( 1100, _t( 'Cannot relate a record to itself' ), 'BaseModel->addRelationship',
					$va_rel_info['related_table_name'] );
			...
			$t_item_rel->set( $t_item_rel->getTypeFieldName(),
				$pn_type_id );        // TODO: verify type_id based upon type_id's of each end of the relationship
			...
			switch ( sizeof( $va_rel_info['path'] ) ) {
				...
				case 2:        // many-to-one relationship
					...
					break;
				default:
					$this->postError( 280, _t( 'Could not find a path to the specified related table' ),
						'BaseModel->addRelationship', $t_rel_item->tableName() );
	 ...
	 *
	 * @param mixed  $pm_rel_table_name_or_num Table name (eg. "ca_entities") or number as defined in datamodel.conf of
	 *                                         table containing row to create relationships to.
	 ...
	 *                                         direction determines the nature and display text for the relationship.
	 * @param array  $pa_options               Array of additional options:
	 *                                         allowDuplicates = if set to true, attempts to edit a relationship to
	 *                                         match one that already exists will succeed. Default is false - duplicate
	 *                                         relationships will not be created. setErrorOnDuplicate = if set to true,
	 *                                         an error will be set if an attempt is made to create a duplicate
	 *                                         relationship. Default is false - don't set error. editRelationship()
	 *                                         will always return false when editing of a relationship fails, no matter
	 *                                         how the setErrorOnDuplicate option is set.
	 *
	 * @return BaseRelationshipModel Loaded relationship model instance on success, false on error.
	 */
	public function editRelationship(
		$pm_rel_table_name_or_num, $pn_relation_id, $pn_rel_id, $pm_type_id = null, $ps_effective_date = null,
		$pa_source_info = null, $ps_direction = null, $pn_rank = null, $pa_options = null
	) {
		if ( ! ( $va_rel_info = $this->_getRelationshipInfo( $pm_rel_table_name_or_num ) ) ) {
			$this->postError( 1240, _t( 'Related table specification "%1" is not valid', $pm_rel_table_name_or_num ),
				'BaseModel->editRelationship()' );
		...
		if ( ( ! isset( $pa_options['allowDuplicates'] ) || ! $pa_options['allowDuplicates'] )
		     && ! $this->getAppConfig()->get( 'allow_duplicate_relationships' )
		     && $this->relationshipExists( $pm_rel_table_name_or_num, $pn_rel_id, $pm_type_id, $ps_effective_date,
				$ps_direction, array( 'relation_id' => $pn_relation_id ) )
		) {
			if ( isset( $pa_options['setErrorOnDuplicate'] ) && $pa_options['setErrorOnDuplicate'] ) {
				$this->postError( 1100, _t( 'Relationship already exists' ), 'BaseModel->addRelationship',
					$t_rel_item->tableName() );
			}
		...
		if ( $va_rel_info['related_table_name'] == $this->tableName() ) {
			// is self relation
			if ( $pn_rel_id === $this->getPrimaryKey() ) {
				$this->postError( 1100, _t( 'Cannot relate a record to itself' ), 'BaseModel->editRelationship',
					$va_rel_info['related_table_name'] );
				...
				case 2:        // many-to-one relations
					if ( $this->tableName() == $va_rel_info['rel_keys']['one_table'] ) {
						if ( $t_item_rel->load( $pn_relation_id ) ) {
							...
							if ( is_array( $cf = $t_item_rel->getChangedFieldValuesArray() )
							     && ( sizeof( $cf ) > 0 )
							) {
								$t_item_rel->update();
	 ...
	 *
	 * @param mixed $pm_rel_table_name_or_num Table name (eg. "ca_entities") or number as defined in datamodel.conf of
	 *                                        table containing row to edit relationships to.
	 ...
	 *
	 * @param mixed $pm_rel_table_name_or_num Table name (eg. "ca_entities") or number as defined in datamodel.conf of
	 *                                        table containing row to removes relationships to.
	 * @param mixed $pm_type_id               If set to a relationship type code or numeric type_id, only relationships
	 *                                        with the specified type are removed.
	 * @param array $pa_options               Options include:
	 *                                        restrictToTypes =
	 *
	 * @return boolean True on success, false on error
		...
		if ( $pa_type_ids = caGetOption( 'restrictToTypes', $pa_options, null ) ) {
			...
			if ( is_array( $pa_type_ids ) && ( sizeof( $pa_type_ids ) > 0 ) ) {
				...
				if ( $t_item_rel->tableName() == $this->getSelfRelationTableName() ) {
					$vs_join_sql = "INNER JOIN " . $this->tableName() . " AS t1 ON t1." . $this->primaryKey() . " = r."
					               . $t_item_rel->getLeftTableFieldName() . "\n" .
					               "INNER JOIN " . $this->tableName() . " AS t2 ON t2." . $this->primaryKey() . " = r."
					               . $t_item_rel->getRightTableFieldName() . "\n";

					$vs_type_limit_sql = " AND (t1.type_id IN (?) OR t2.type_id IN (?))";
					$va_sql_params[]   = $pa_type_ids;
					$va_sql_params[]   = $pa_type_ids;
				} else {
					$vs_target_table_name = ( $t_item_rel->getLeftTableName() == $this->tableName() )
						? $t_item_rel->getRightTableName() : $t_item_rel->getLeftTableName();
					$vs_target_table_pk   = Datamodel::primaryKey( $vs_target_table_name );

					$vs_join_sql
						= "INNER JOIN {$vs_target_table_name} AS t ON t.{$vs_target_table_pk} = r.{$vs_target_table_pk}\n";
	 ...
	 * rows are simply re-pointed to the new row, so this is a relatively fast operation. Note that this method does
	 * not copy relationships, it moves them. After the operation completes no relationships to the specified related
	 * table will exist for the current row.
	 *
	 * @param mixed $pm_rel_table_name_or_num The table name or number of the related table. Only relationships
	 *                                        pointing to this table will be moved.
	 ...
	 *
	 * @return int Number of relationships moved, or null on error. Note that you should carefully test the return
	 *             value for null-ness rather than false-ness, since zero is a valid return value in cases where no
	 *             relationships need to be moved.
	 */
	public function moveRelationships( $pm_rel_table_name_or_num, $pn_to_id, $pa_options = null ) {
		...
		if ( $t_item_rel->tableName() == $this->getSelfRelationTableName() ) {
			...
			$o_db->query( "
				UPDATE IGNORE " . $t_item_rel->tableName() . " SET " . $t_item_rel->getLeftTableFieldName()
			              . " = ? WHERE " . $t_item_rel->getLeftTableFieldName() . " = ?
			...
			$o_db->query( "
				UPDATE IGNORE " . $t_item_rel->tableName() . " SET " . $t_item_rel->getRightTableFieldName()
			              . " = ? WHERE " . $t_item_rel->getRightTableFieldName() . " = ?
			", (int) $pn_to_id, (int) $vn_row_id );
			if ( $o_db->numErrors() ) {
				$this->errors = $o_db->errors;
		...
		foreach ( $va_to_reindex_relations as $vn_relation_id => $va_row ) {
			$o_indexer->indexRow( $vn_rel_table_num, $vn_relation_id, $va_row, false, null,
				array( $vs_item_pk => true ) );
		}
	...
	/**
	 * Copies relationships from currently loaded row to another row specified by $pn_row_id. If you want to transfer
	 * relationships from one row to another use moveRelationships() which is much faster than copying and then
	 ...
	 *                                        copyAttributes = Copy metadata attributes associated with each
	 *                                        relationship, if the calling model supports attributes. [Default is
	 *                                        false] relationIds = List of relationship relation_id's to limit copy to.
	 *                                        [Default is null]
	 *
	 * @return int Number of relationships copied, or null on error. Note that you should carefully test the return
	 *             value for null-ness rather than false-ness, since zero is a valid return value in cases where no
	 *             relationships were available to be copied.
	...
	public function copyRelationships( $pm_rel_table_name_or_num, $pn_to_id, $pa_options = null ) {
		...
		$vb_copy_attributes = caGetOption( 'copyAttributes', $pa_options, false, array( 'castTo' => 'boolean' ) )
		                      && method_exists( $this, 'copyAttributesFrom' );
		...
		$relation_id_sql = ( is_array( $relation_ids ) && sizeof( $relation_ids ) ) ? " AND (relation_id IN (?))"
			: null;
	...
	private function _getRelationshipInfo( $pm_rel_table_name_or_num, $pb_use_cache = true ) {
		$vs_table = $this->tableName();
		if ( $pb_use_cache
		     && isset( BaseModel::$s_relationship_info_cache[ $vs_table ][ $pm_rel_table_name_or_num ] )
		) {
		...
		return BaseModel::$s_relationship_info_cache[ $vs_table ][ $vs_related_table_name ]
			= BaseModel::$s_relationship_info_cache[ $vs_table ][ $pm_rel_table_name_or_num ] = array(
	 ...
	 *                                         relation_id = an optional relation_id to ignore when checking for
	 *                                         existence. If you are checking for relations other than one you know
	 *                                         exists you can set this to ensure that relationship is not considered.
	 *
	 * @return mixed Array of matched relation_ids on success, false on error.
	 */
	public function relationshipExists(
		$pm_rel_table_name_or_num, $pn_rel_id, $pm_type_id = null, $ps_effective_date = null, $ps_direction = null,
		$pa_options = null
	) {
		...
		$vn_relation_id     = ( ( isset( $pa_options['relation_id'] ) && (int) $pa_options['relation_id'] ) )
			? (int) $pa_options['relation_id'] : null;
		...
		if ( ( $t_item_rel = $va_rel_info['t_item_rel'] ) && method_exists( $t_item_rel, 'getLeftTableName' ) ) {
			...
			if ( $ps_effective_date && $t_item_rel->hasField( 'effective_date' )
			     && ( $va_timestamps
					= caDateToHistoricTimestamps( $ps_effective_date ) )
			) {
			...
			if ( sizeof( $va_rel_info['path'] ) == 2 ) {        // many-one rel
				...
				$vs_where_sql = "(ot." . $va_rel_keys['one_table_field'] . " = ?) AND (mt."
				                . $va_rel_keys['many_table_field'] . " = ?)";
				...
				$vs_relation_id_fld = ( $vb_is_one_table ? "mt." . $va_rel_keys['many_table_field']
					: "ot." . $va_rel_keys['one_table_field'] );
				$qr_res             = $o_db->query( $x = "
	 ...
	 *                          None yet
	 *
	 * @return mixed Array of table names for which this row has at least one relationship, with keys set to table
	 *               names and values set to the number of relationships per table.
	 */
	 ...
	 *                        restrictToTypes = Only consider relationships that link to related rows with the
	 *                        specified types. [Default is null; consider all rows] restrictToRelationshipTypes = Only
	 *                        consider relationships with the specified types. [Default is null; consider all
	 *                        relationships]
	 *
	 * @return mixed The number of relationships, false is there are no relationships, null if $table is not relatable
		...
		if ( is_array( $many_to_many_relations ) ) {
			...
			foreach ( $many_to_many_relations as $rel_info ) {
				...
				if ( is_array( $restrict_to_relationship_types ) ) {
					$restrict_to_relationship_type_ids = caMakeRelationshipTypeIDList( $table,
	 ...
	 * @param       $pn_user_id   [integer] A valid ca_users.user_id indicating the user who added the tag; is null for
	 *                            tags from non-logged-in users (optional - default is null)
	 * @param       $pn_locale_id [integer] A valid ca_locales.locale_id indicating the language of the comment. If
	 *                            omitted or left null then the value in the global $g_ui_locale_id variable is used.
	 *                            If $g_ui_locale_id is not set and $pn_locale_id is not set then an error will occur
	 *                            (optional - default is to use $g_ui_locale_id)
	 * @param       $pn_access    [integer] Determines public visibility of tag; if set to 0 then tag is not visible to
	 *                            public; if set to 1 tag is visible (optional - default is 0)
	 * @param       $pn_moderator [integer] A valid ca_users.user_id value indicating who moderated the tag; if omitted
	 *                            or set to null then moderation status will not be set unless app.conf setting
	 *                            dont_moderate_comments = 1 (optional - default is null)
	 * @param array $pa_options   Array of options. Supported options are:
	 *                            purify = if true, comment, name and email are run through HTMLPurifier before being
	 *                            stored in the database. Default is true. rank = option rank used for sorting. If
	 *                            omitted the tag is added to the end of the display list. [Default is null]
	 *                            forceModeration = force status of newly created tag to moderated. [Default is false]
	 */
	public function addTag(
		$ps_tag, $pn_user_id = null, $pn_locale_id = null, $pn_access = 0, $pn_moderator = null, $pa_options = null
	) {
		global $g_ui_locale_id;
		if ( ! ( $vn_row_id = $this->getPrimaryKey() ) ) {
			return null;
		...
		} elseif ( caGetOption( 'forceModerated', $pa_options, false )
		           || $this->_CONFIG->get( "dont_moderate_comments" )
		) {
			$t_ixt->set( 'moderated_on', _t( 'now' ) );
		}
	 ...
	 * if tag access setting was successfully changed, false if an error occurred in which case the errors will be
	 * available via the model's standard error methods (getErrors() and friends.
	 *
	 ...
	 * The parameters are:
	 *
	 * @param $pn_relation_id [integer] A valid ca_items_x_tags.relation_id value specifying the tag relation to modify
	 *                        (mandatory)
	 * @param $pn_access      [integer] Determines public visibility of tag; if set to 0 then tag is not visible to
	 *                        public; if set to 1 tag is visible (optional - default is 0)
	 * @param $pn_moderator   [integer] A valid ca_users.user_id value indicating who moderated the tag; if omitted or
	 *                        set to null then moderation status will not be set (optional - default is null)
	 * @param $pn_user_id     [integer] A valid ca_users.user_id valid; if set only tag relations created by the
	 *                        specified user will be modifed  (optional - default is null)
	 */
	public function changeTagAccess( $pn_relation_id, $pn_access = 0, $pn_moderator = null, $pn_user_id = null ) {
		...
		if (
			( $t_ixt->get( 'table_num' ) != $this->tableNum() ) || ( $t_ixt->get( 'row_id' ) != $vn_row_id )
		) {
	 ...
	 * via the model's standard error methods (getErrors() and friends.
	 *
	 * @param $pn_relation_id int A valid ca_items_x_tags.relation_id value specifying the tag relation to modify
	 *                        (mandatory)
	...
	/**
	 * Deletes the tag relation specified by $pn_relation_id (a ca_items_x_tags.relation_id value) from the currently
	 * loaded row. Will only delete tags attached to the currently loaded row. If you attempt to delete a
	 * ca_items_x_tags.relation_id not attached to the current row removeTag() will return false and post an error. If
	 * you attempt to call removeTag() with no row loaded null will be returned. If $pn_user_id is specified then only
	 * tags created by the specified user will be deleted; if the tag being deleted is not created by the user then
	 * false is returned and an error posted.
	 *
	 * @param $pn_relation_id [integer] a valid ca_items_x_tags.relation_id to be removed; must be related to the
	 *                        currently loaded row (mandatory)
	 * @param $pn_user_id     [integer] a valid ca_users.user_id value; if specified then only tag relations added by
	 *                        the specified user will be deleted (optional - default is null)
	 */
	 ...
	 *
	 * @param $pn_user_id [integer] A valid ca_users.user_id value. If specified, only tags added by the specified user
	 *                    will be removed. (optional - default is null)
	 */
	 ...
	 *        If you want both moderated and unmoderated tags to be returned then omit the parameter or pass a null
	 *        value
	 *
	 * @param $pn_user_id           [integer] A valid ca_users.user_id value. If specified, only tags added by the
	 *                              specified user will be returned. (optional - default is null)
	 * @param $pb_moderation_status [boolean] To return only unmoderated tags set to FALSE; to return only moderated
	 *                              tags set to TRUE; to return all tags set to null or omit
	 */
	public function getTags( $pn_user_id = null, $pb_moderation_status = null, $pn_row_id = null ) {
		...
		if ( ! is_null( $pb_moderation_status ) ) {
			$vs_moderation_sql = ( $pb_moderation_status ) ? ' AND (cixt.moderated_on IS NOT NULL)'
				: ' AND (cixt.moderated_on IS NULL)';
		}
	 ...
	 * comment text is monolingual; if you want to do multilingual comments (which aren't really comments then, are
	 * they?) then you should add multiple comments.
	 *
	 * @param       $ps_comment   [string] Text of the comment (mandatory)
	 * @param       $pn_rating    [integer] A number between 1 and 5 indicating the user's rating of the row; larger is
	 *                            better (optional - default is null)
	 * @param       $pn_user_id   [integer] A valid ca_users.user_id indicating the user who posted the comment; is
	 *                            null for comments from non-logged-in users (optional - default is null)
	 * @param       $pn_locale_id [integer] A valid ca_locales.locale_id indicating the language of the comment. If
	 *                            omitted or left null then the value in the global $g_ui_locale_id variable is used.
	 *                            If $g_ui_locale_id is not set and $pn_locale_id is not set then an error will occur
	 *                            (optional - default is to use $g_ui_locale_id)
	 * @param       $ps_name      [string] Name of user posting comment. Only needs to be set if $pn_user_id is *not*
	 *                            set; used to identify comments posted by non-logged-in users (optional - default is
	 *                            null)
	 * @param       $ps_email     [string] E-mail address of user posting comment. Only needs to be set if $pn_user_id
	 *                            is *not* set; used to identify comments posted by non-logged-in users (optional -
	 *                            default is null)
	 * @param       $pn_access    [integer] Determines public visibility of comments; if set to 0 then comment is not
	 *                            visible to public; if set to 1 comment is visible (optional - default is 0)
	 * @param       $pn_moderator [integer] A valid ca_users.user_id value indicating who moderated the comment; if
	 *                            omitted or set to null then moderation status will not be set unless app.conf setting
	 *                            dont_moderate_comments = 1 (optional - default is null)
	 * @param array $pa_options   Array of options. Supported options are:
	 *                            purify = if true, comment, name and email are run through HTMLPurifier before being
	 *                            stored in the database. Default is true. media1_original_filename = original file
	 *                            name to set for comment "media1" media2_original_filename = original file name to set
	 *                            for comment "media2" media3_original_filename = original file name to set for comment
	 *                            "media3" media4_original_filename = original file name to set for comment "media4"
	 * @param       $ps_location  [string] = location of user
	 *
	 * @return ca_item_comments BaseModel representation of newly created comment, false on error or null if parameters
	 *                          are invalid
	 */
	public function addComment(
		$ps_comment, $pn_rating = null, $pn_user_id = null, $pn_locale_id = null, $ps_name = null, $ps_email = null,
		$pn_access = 0, $pn_moderator = null, $pa_options = null, $ps_media1 = null, $ps_media2 = null,
		$ps_media3 = null, $ps_media4 = null, $ps_location = null
	) {
		global $g_ui_locale_id;
		if ( ! ( $vn_row_id = $this->getPrimaryKey() ) ) {
			return null;
		...
		$t_comment->set( 'media1', $ps_media1,
			array( 'original_filename' => $pa_options['media1_original_filename'] ) );
		$t_comment->set( 'media2', $ps_media2,
			array( 'original_filename' => $pa_options['media2_original_filename'] ) );
		$t_comment->set( 'media3', $ps_media3,
			array( 'original_filename' => $pa_options['media3_original_filename'] ) );
		$t_comment->set( 'media4', $ps_media4,
			array( 'original_filename' => $pa_options['media4_original_filename'] ) );
	 ...
	 * will be written into the comment. For example, if you don't bother passing $ps_name then it will be set to null,
	 * even if there's an existing name value in the field. The only exception is $pn_locale_id; if set to null or
	 * omitted then editComment() will attempt to use the locale value in the global $g_ui_locale_id variable. If this
	 * is not set then an error will be posted and editComment() will return false.
	 *
	 * @param       $pn_comment_id [integer] a valid comment_id to be edited; must be related to the currently loaded
	 *                             row (mandatory)
	 * @param       $ps_comment    [string] the text of the comment (mandatory)
	 * @param       $pn_rating     [integer] a number between 1 and 5 indicating the user's rating of the row; higher
	 *                             is better (optional - default is null)
	 * @param       $pn_user_id    [integer] A valid ca_users.user_id indicating the user who posted the comment; is
	 *                             null for comments from non-logged-in users (optional - default is null)
	 * @param       $pn_locale_id  [integer] A valid ca_locales.locale_id indicating the language of the comment. If
	 *                             omitted or left null then the value in the global $g_ui_locale_id variable is used.
	 ...
	 *                             visible to public; if set to 1 comment is visible (optional - default is 0)
	 * @param       $pn_moderator  [integer] A valid ca_users.user_id value indicating who moderated the comment; if
	 *                             omitted or set to null then moderation status will not be set (optional - default is
	 *                             null)
	 * @param array $pa_options    Array of options. Supported options are:
	 *                             purify = if true, comment, name and email are run through HTMLPurifier before being
	 *                             stored in the database. Default is true. media1_original_filename = original file
	 *                             name to set for comment "media1" media2_original_filename = original file name to
	 *                             set for comment "media2" media3_original_filename = original file name to set for
	 *                             comment "media3" media4_original_filename = original file name to set for comment
	 *                             "media4"
	 */
	public function editComment(
		$pn_comment_id, $ps_comment, $pn_rating = null, $pn_user_id = null, $pn_locale_id = null, $ps_name = null,
		$ps_email = null, $pn_access = null, $pn_moderator = null, $pa_options = null, $ps_media1 = null,
		$ps_media2 = null, $ps_media3 = null, $ps_media4 = null
	) {
		global $g_ui_locale_id;
		if ( ! ( $vn_row_id = $this->getPrimaryKey() ) ) {
			return null;
		...
		if (
			( $t_comment->get( 'table_num' ) != $this->tableNum() ) || ( $t_comment->get( 'row_id' ) != $vn_row_id )
		) {
	 ...
	 * will return false and post an error. If you attempt to call removeComment() with no row loaded null will be
	 * returned. If $pn_user_id is specified then only comments created by the specified user will be deleted; if the
	 * comment being deleted is not created by the user then false is returned and an error posted.
	 *
	 * @param $pn_comment_id [integer] a valid comment_id to be removed; must be related to the currently loaded row
	 *                       (mandatory)
	 * @param $pn_user_id    [integer] a valid ca_users.user_id value; if specified then only comments by the specified
	 *                       user will be deleted (optional - default is null)
	 * @param $options       Options include:
	 *                       force = Remove comment even if it's not parent of the current or created by the current
		...
		if ( ! $force ) {
			...
			if ( $pn_user_id ) {
				if ( $t_comment->get( 'user_id' ) != $pn_user_id ) {
					$this->postError( 2820, _t( 'Comment was not created by specified user' ),
						'BaseModel->removeComment()', 'ca_item_comments' );
	 ...
	 * If the optional $ps_user_id parameter is passed then only comments created by the specified user will be
	 * removed.
	 *
	 * @param $pn_user_id [integer] A valid ca_users.user_id value. If specified, only comments by the specified user
	 *                    will be removed. (optional - default is null)
	 */
	 ...
	 * If the optional $ps_user_id parameter is passed then only comments created by the specified user will be
	 * returned. If the optional $pb_moderation_status parameter is passed then only comments matching the criteria
	 * will be returned: Passing $pb_moderation_status = TRUE will cause only moderated comments to be returned Passing
	 * $pb_moderation_status = FALSE will cause only unmoderated comments to be returned If you want both moderated and
	 * unmoderated comments to be returned then omit the parameter or pass a null value
	 *
	 * @param int   $pn_user_id           A valid ca_users.user_id value. If specified, only comments by the specified
	 *                                    user will be returned. (optional - default is null)
	 * @param bool  $pb_moderation_status To return only unmoderated comments set to FALSE; to return only moderated
	 *                                    comments set to TRUE; to return all comments set to null or omit
	 * @param array $pa_options           Options include:
	 *                                    transaction = optional Transaction instance. If set then all database access
	 *                                    is done within the context of the transaction returnAs = what to return;
	 *                                    possible values are: array                   = an array of comments
	 *                                    searchResult            = a search result instance (aka. a subclass of
	 *                                    BaseSearchResult), when the calling subclass is searchable (ie.
	 *                                    <classname>Search and <classname>SearchResult classes are defined) ids
	 *                                                    = an array of ids (aka. primary keys) modelInstances
	 *                                                      = an array of instances, one for each match. Each instance
	 *                                                      is the same class as the caller, a subclass of BaseModel
	 *                                                      firstId                    = the id (primary key) of the
	 *                                                      first match. This is the same as the first item in the array returned by 'ids' firstModelInstance        = the instance of the first match. This is the same as the first instance in the array returned by 'modelInstances' count                    = the number of matches
	 *
	...
	public function getComments( $pn_user_id = null, $pb_moderation_status = null, $pa_options = null ) {
		...
		$vs_moderation_sql = '';
		if ( ! is_null( $pb_moderation_status ) ) {
			$vs_moderation_sql = ( $pb_moderation_status ) ? ' AND (c.moderated_on IS NOT NULL)'
				: ' AND (c.moderated_on IS NULL)';
		}
		...
		switch ( $vs_return_as ) {
			...
			case 'firstModelInstance':
				$va_ids = ( $qr_comments->getAllFieldValues( 'comment_id' )
				            + $qr_set_comments->getAllFieldValues( 'comment_id' ) );
				if ( $vs_return_as === 'ids' ) {
					return $va_ids;
				}
			...
			default:
				...
				while ( $qr_comments->nextRow() ) {
					...
					$va_comments[ $comment_id ]['created_on_display']
						= caGetLocalizedDate( $va_comments[ $comment_id ]['created_on'] );
					if ( $va_comments[ $comment_id ]['user_id'] ) {
						$va_comments[ $comment_id ]['name']  = trim( $va_comments[ $comment_id ]['fname'] . ' '
						                                             . $va_comments[ $comment_id ]['lname'] );
				...
				while ( $qr_set_comments->nextRow() ) {
					...
					foreach ( array( "media1", "media2", "media3", "media4" ) as $vs_media_field ) {
						...
						if ( is_array( $va_media_versions ) && ( sizeof( $va_media_versions ) > 0 ) ) {
							foreach ( $va_media_versions as $vs_version ) {
								$va_image_info           = array();
								$va_image_info           = $qr_set_comments->getMediaInfo( $vs_media_field,
									$vs_version );
								$va_image_info["TAG"]    = $qr_set_comments->getMediaTag( $vs_media_field,
									$vs_version );
								$va_image_info["URL"]    = $qr_set_comments->getMediaUrl( $vs_media_field,
									$vs_version );
								$va_media[ $vs_version ] = $va_image_info;
							}
					...
					if ( $o_request ) {
						$va_comments[ $comment_id ]['set_message'] = _t( "Comment made in set %1",
							caEditorLink( $o_request, $qr_set_comments->get( 'set_name' ), '', 'ca_sets',
								$va_comments[ $comment_id ]['set_id'] ) );
					}
				}
				break;
		}
		...
		if ( caGetOption( 'sortDirection', $pa_options, 'ASC',
				[ 'validValues' => [ 'ASC', 'DESC' ], 'forceToUppercase' => true ] ) === 'DESC'
		) {
	...
	public function getAverageRating( $pb_moderation_status = true ) {
		...
		$vs_moderation_sql = '';
		if ( ! is_null( $pb_moderation_status ) ) {
			$vs_moderation_sql = ( $pb_moderation_status ) ? ' AND (ca_item_comments.moderated_on IS NOT NULL)'
				: ' AND (ca_item_comments.moderated_on IS NULL)';
		}
	...
	public function getHighestRated( $pb_moderation_status = true, $pn_num_to_return = 1, $va_access_values = array()
		...
		if ( isset( $va_access_values ) && is_array( $va_access_values ) && sizeof( $va_access_values )
			...
			if ( $vs_table_name && $vs_primary_key ) {
				$vs_access_join  = 'INNER JOIN ' . $vs_table_name . ' as rel ON rel.' . $vs_primary_key
	...
	public function registerItemView() {
		...
		$this->getDb()->query( "UPDATE " . $this->tableName() . " SET view_count = view_count + 1 WHERE "
		                       . $this->primaryKey() . " = ?", array( $vn_row_id ) );
	...
	public function clearItemViewCount() {
		...
		$this->getDb()->query( "UPDATE " . $this->tableName() . " SET view_count = 0 WHERE " . $this->primaryKey()
		                       . " = ?", array( $vn_row_id ) );
	 ...
	 *                          restrictToTypes = array of type names or type_ids to restrict to. Only items with a
	 *                          type_id in the list will be returned. hasRepresentations = if set when model is for
	 *                          ca_objects views are only returned when the object has at least one representation.
	 *                          checkAccess = an array of access values to filter only. Items will only be returned if
	 *                          the item's access setting is in the array. idsOnly = return primary keys only. [Default
	 *                          is false]
	 *
	 * @return array
	 */
	public function getMostViewedItems( $pn_limit = 10, $pa_options = null ) {
		...
		if ( method_exists( $this, 'getTypeFieldName' ) && ( $vs_type_field_name = $this->getTypeFieldName() ) ) {
			...
			if ( is_array( $va_type_ids ) && sizeof( $va_type_ids ) ) {
				$va_wheres[] = "(t.{$vs_type_field_name} IN (" . join( ',', $va_type_ids ) . ')'
				               . ( $this->getFieldInfo( $vs_type_field_name, 'IS_NULL' )
						? " OR t.{$vs_type_field_name} IS NULL" : '' ) . ')';
			}
		}

		if ( method_exists( $this, 'getSourceFieldName' )
		     && ( $vs_source_id_field_name
		...
		if ( isset( $pa_options['hasRepresentations'] ) && $pa_options['hasRepresentations']
		     && ( $this->tableName() == 'ca_objects' )
		) {
			$vs_join_sql
				         = ' INNER JOIN ca_objects_x_object_representations ON ca_objects_x_object_representations.object_id = t.object_id';
	 ...
	 * recent views. If there is no cache available it will query the database to look at the most recent (4 x your
	 * limit) viewings. If there are many views of the same item in that group then it is possible that fewer items
	 * than your limit will be returned)
	 *
	 * @param int   $pn_limit   The maximum number of items to return. Default is 10.
	 * @param array $pa_options Supported options are:
	 *                          restrictToTypes = array of type names or type_ids to restrict to. Only items with a
	 *                          type_id in the list will be returned. checkAccess = array of access values to filter
	 *                          results by; if defined only items with the specified access code(s) are returned
	 *                          hasRepresentations = if set to a non-zero (boolean true) value only items with
	...
	public function getRecentlyViewedItems( $pn_limit = 10, $pa_options = null ) {
		...
		if ( is_array( $va_recently_viewed_items ) && is_array( $va_recently_viewed_items[ $vn_table_num ] ) ) {
			if ( $qr_res = caMakeSearchResult( $vs_table_name, $va_recently_viewed_items[ $vn_table_num ] ) ) {
				...
				$va_source_ids        = caMergeSourceRestrictionLists( $this, $pa_options );
				$vs_source_field_name = method_exists( $this, 'getSourceFieldName' ) ? $this->getSourceFieldName()
					: null;
				...
				while ( $qr_res->nextHit() ) {
					...
					if ( isset( $pa_options['hasRepresentations'] ) && $pa_options['hasRepresentations'] ) {
						if ( ! ( $va_reps = $qr_res->get( 'ca_object_representations.representation_id',
					...
					if ( ( $vs_source_field_name ) && ( is_array( $va_source_ids ) && sizeof( $va_source_ids ) ) ) {
						...
						if ( ! in_array( $vn_source_id, $va_source_ids )
						     || ( is_null( $vn_source_id )
						          && ! $this->getFieldInfo( $vs_source_field_name, 'IS_NULL' ) )
						) {
							continue;
						}
					}
	 ...
	 *                          ca_objects views are only returned when the object has at least one representation.
	 *                          checkAccess = an array of access values to filter only. Items will only be returned if
	 *                          the item's access setting is in the array. restrictByIntrinsic = an associative array
	 *                          of intrinsic fields and values to sort returned records on
	 *
	 * @return bool True on success, false on error
	 */
	public function getRandomItems( $pn_limit = 10, $pa_options = null ) {
		...
		if ( method_exists( $this, 'getTypeFieldName' ) && ( $vs_type_field_name = $this->getTypeFieldName() ) ) {
			...
			if ( is_array( $va_type_ids ) && sizeof( $va_type_ids ) ) {
				$va_wheres[] = "({$vs_table_name}.{$vs_type_field_name} IN (" . join( ',', $va_type_ids ) . ')'
				               . ( $this->getFieldInfo( $vs_type_field_name, 'IS_NULL' )
						? " OR {$vs_table_name}.{$vs_type_field_name} IS NULL" : '' ) . ')';
			}
		}
		...
		if ( isset( $pa_options['hasRepresentations'] ) && $pa_options['hasRepresentations']
			...
			$vs_join_sql
				= ' INNER JOIN ca_objects_x_object_representations ON ca_objects_x_object_representations.object_id = '
				  . $vs_table_name . '.object_id';
		}
	...
	/**
	 * Returns number of records in table, filtering out those marked as deleted or those not meeting the specific
	 * access critera
	 *
	 * @param array $pa_access An option array of record-level access values to filter on. If omitted no filtering on
	 *                         access values is done.
	 *
	...
	/**
	 * Sets selected attribute of field in model to new value, overriding the value coded into the model for ** the
	 * duration of the current request** This is useful in some unusual situations where you need to have a field
	 * behave differently than normal without making permanent changes to the model. Don't use this unless you know
	 * what you're doing.
	 *
	 ...
	 * @param array $pa_options Options include:
	 *                          checkAccess = Array of access values to filter returned values on. If omitted no
	 *                          filtering is performed. [Default is null] transaction = optional Transaction instance.
	 *                          If set then all database access is done within the context of the transaction
	 *
	...
	public static function getIdnoForID( $pn_id, $pa_options = null ) {
		$o_trans = caGetOption( 'transaction', $pa_options, null );
		if ( ( $t_instance = Datamodel::getInstance( static::class, true ) )
		     && ( $vs_idno_fld
				= $t_instance->getProperty( 'ID_NUMBERING_ID_FIELD' ) )
		) {
			$o_db   = $o_trans ? $o_trans->getDb() : new Db();
			$qr_res = $o_db->query( "SELECT {$vs_idno_fld} FROM " . $t_instance->tableName() . " WHERE "
			                        . $t_instance->primaryKey() . " = ?", [ (int) $pn_id ] );

			$pa_check_access = caGetOption( 'checkAccess', $pa_options, null );
			if ( $qr_res->nextRow() ) {
				if ( ( is_array( $pa_check_access ) && ( sizeof( $pa_check_access ) > 0 ) )
				     && $t_instance->hasField( 'access' )
				     && ! in_array( $qr_res->get( 'access' ), $pa_check_access )
				) {
					return false;
				}
	...
	/**
	 * Return primary key id value for idno value. If more than one primary key exists for the idno, the first id found
	 * is returned.
	 *
	...
	public static function getIDForIdno( $ps_idno, $pa_options = null ) {
		...
		if ( ( $t_instance = Datamodel::getInstance( static::class, true ) )
			...
			$o_db   = $o_trans ? $o_trans->getDb() : new Db();
			$vs_pk  = $t_instance->primaryKey();
			$qr_res = $o_db->query( "SELECT {$vs_pk} FROM " . $t_instance->tableName() . " WHERE {$vs_idno_fld} = ?",
				[ $ps_idno ] );
	 ...
	 *
	 * Matching is performed using values in $pa_values. PPartial and pattern matching are supported as are
	 * inequalities. Searches may include multiple fields with boolean AND and OR. For example, you can find ca_objects
	 * rows with idno = 2012.001 and access = 1 by passing the
	 ...
	 *
	 * BaseModel::find() is not a replacement for the SearchEngine. It is intended as a quick and convenient way to
	 * programatically fetch rows using simple, clear cut criteria. If you need to fetch rows based upon an identifer
	 * or status value BaseModel::find() will be quicker and less code than using the SearchEngine. For full-text
	 * searches or searches that require transformations or complex boolean operations use the SearchEngine.
	 *
	 * @param array $pa_values  An array of values to match. Keys in the $pa_values parameters must be valid fields in
	 *                          the table which the model sub-class represents. You may also search on preferred and
	 *                          non-preferred labels by specified keys and values for label table fields in
	 *                          "preferred_labels" and "nonpreferred_labels" sub-arrays. For example:
	 *
	 *        ["idno" => 2012.001", "access" => 1]
	 *
	 *        will find rows with the idno, and access  values.
	 *
	 *        You can specify operators with an expanded array format where each value is an array containing both an
	 *        operator and a value. Ex.:
	 *
	 *        ["idno" => ['=', '2012.001'], "access" => ['>', 1]]
	 *
	 ...
	 *
	 *         If you pass an integer instead of an array it will be used as the primary key value for the table;
	 *         result will be returned as "firstModelInstance" unless the returnAs option is explicitly set.
	 *
	 * @param array $pa_options Options are:
	 *                          transaction = optional Transaction instance. If set then all database access is done
	 *                          within the context of the transaction returnAs = what to return; possible values are:
	 ...
	 *                          <classname>SearchResult classes are defined) ids                        = an array of
	 *                          ids (aka. primary keys) modelInstances            = an array of instances, one for each
	 *                          match. Each instance is the same class as the caller, a subclass of BaseModel firstId
	 *                                           = the id (primary key) of the first match. This is the same as the
	 *                                           first item in the array returned by 'ids' firstModelInstance        =
	 *                                           the instance of the first match. This is the same as the first
	 *                                           instance in the array returned by 'modelInstances' count                    = the number of matches arrays                    = an array of arrays, each of which contains values for each intrinsic field in the model queryresult                = a query result
	 *
	 *            The default is ids
	 *
	 *        start = if searchResult, ids or modelInstances is set, starts returned list of matches at specified
	 *        index. Default is 0. limit = if searchResult, ids or modelInstances is set, limits number of returned
	 *        matches. Default is no limit. boolean = determines how multiple field values in $pa_values are combined
	 *        to produce the final result. Possible values are: AND                        = find rows that match all
	 *        criteria in $pa_values OR                        = find rows that match any criteria in $pa_values
	 *
	 *            The default is AND
	 *
	 *        sort = field to sort on. Must be in <table>.<field> or <field> format and be an intrinsic field in the
	 *        primary table. Sort order can be set using the sortDirection option. sortDirection = the direction of the
	 *        sort. Values are ASC (ascending) and DESC (descending). Default is ASC. allowWildcards = consider "%" as
	 *        a wildcard when searching. Any term including a "%" character will be queried using the SQL LIKE
	 *        operator. [Default is false] purify = process text with HTMLPurifier before search. Purifier encodes &, <
	 *        and > characters, and performs other transformations that can cause searches on literal text to fail. If
	 *        you are purifying all input (the default) then leave this set true. [Default is true] purifyWithFallback
	 *        = executes the search with "purify" set and falls back to search with unpurified text if nothing is
	 *        found. [Default is false] checkAccess = array of access values to filter results by; if defined only
	 *        items with the specified access code(s) are returned. Only supported for
	 *        <table_name>.hierarchy.preferred_labels and <table_name>.children.preferred_labels because these returns sets of items. For <table_name>.parent.preferred_labels, which returns a single row at most, you should do access checking yourself. (Everything here applies equally to nonpreferred_labels) restrictToTypes = Restrict returned items to those of the specified types. An array of list item idnos and/or item_ids may be specified. [Default is null] dontIncludeSubtypesInTypeRestriction = If restrictToTypes is set, by default the type list is expanded to include subtypes (aka child types). If set, no expansion will be performed. [Default is false]
	 *
	 * @return mixed Depending upon the returnAs option setting, an array, subclass of BaseModel or integer may be
	 *               returned.
	 */
	public static function find( $pa_values, $pa_options = null ) {
		...
		$ps_boolean      = caGetOption( 'boolean', $pa_options, 'and',
			array( 'forceLowercase' => true, 'validValues' => array( 'and', 'or' ) ) );
		$o_trans         = caGetOption( 'transaction', $pa_options, null );
		$pa_check_access = caGetOption( 'checkAccess', $pa_options, null );
		...
		if ( $va_restrict_to_types = caGetOption( 'restrictToTypes', $pa_options, null ) ) {
			...
			if ( is_array( $va_restrict_to_types = caMakeTypeIDList( $vs_table, $va_restrict_to_types,
					[ 'dontIncludeSubtypesInTypeRestriction' => $include_subtypes ] ) )
		...
		if ( method_exists( $t_instance, "getTypeFieldName" ) ) {
			if ( is_array( $va_field_values = $pa_values[ $vs_type_field_name = $t_instance->getTypeFieldName() ] ) ) {
				...
				foreach ( $va_field_values as $vn_i => $va_field_value ) {
					...
					if ( ! caIsValidSqlOperator( $vs_op,
						[ 'type' => 'numeric', 'nullable' => false, 'isList' => is_array( $vm_value ) ] )
					) {
						...
						if ( method_exists( $t_instance, "isRelationship" ) && $t_instance->isRelationship() ) {
							if ( is_array( $va_field_values = $pa_values['type_id'] ) ) {
								...
								foreach ( $va_field_values as $vn_i => $va_field_value ) {
									...
									if ( ! is_numeric( $vm_value ) ) {
										...
										if ( $va_types = caMakeRelationshipTypeIDList( $t_instance->tableName(),
											$vm_value )
										) {
							...
							if ( is_array( $vm_value ) ) {
								...
								foreach ( $vm_value as $vn_j => $vs_value ) {
									...
									} elseif ( $vn_id = ca_lists::getItemID( $t_instance->getTypeListCode(),
										$vs_value )
									) {
										$va_trans_vals[] = $vn_id;
									}
		...
		foreach ( $pa_values as $vs_field => $va_field_values ) {
			foreach ( $va_field_values as $vn_i => $va_field_value ) {
				...
				if ( $vs_list_code = $t_instance->getFieldInfo( $vs_field, 'LIST_CODE' ) ) {
					if ( ! caIsValidSqlOperator( $vs_op, [
						'type'     => 'numeric',
						'nullable' => $t_instance->getFieldInfo( $vs_field, 'IS_NULL' ),
						'isList'   => is_array( $vm_value )
					] )
					) {
						throw new ApplicationException( _t( 'Invalid numeric operator: %1', $vs_op ) );
					...
					if ( is_array( $vm_value ) ) {
						$pa_values[ $vs_field ][ $vn_i ] = [
							$vs_op,
							$z = array_map( function ( $v ) use ( $vs_list_code ) {
			...
			foreach ( $pa_values as $vs_field => $va_field_values ) {
				foreach ( $va_field_values as $va_field_value ) {
					...
					if ( $t_instance->_getFieldTypeType( $vs_field ) == 0 ) {
						if ( ! caIsValidSqlOperator( $vs_op,
							[ 'type' => 'numeric', 'nullable' => true, 'isList' => is_array( $vm_value ) ] )
						) {
							throw new ApplicationException( _t( 'Invalid numeric operator: %1', $vs_op ) );
						}
						...
						if ( ! caIsValidSqlOperator( $vs_op,
							[ 'type' => 'string', 'nullable' => true, 'isList' => is_array( $vm_value ) ] )
						) {
						...
						if ( is_array( $vm_value ) ) {
							foreach ( $vm_value as $vn_i => $vs_value ) {
								$vm_value[ $vn_i ] = $t_instance->quote( $vs_field,
									is_null( $vs_value ) ? '' : $vs_value );
							}
						} else {
					...
					} elseif ( caGetOption( 'allowWildcards', $pa_options, false )
					           && ( strpos( $vm_value, '%' ) !== false )
					) {
		...
		if ( $vs_sort = caGetOption( 'sort', $pa_options, null ) ) {
			$vs_sort_direction = caGetOption( 'sortDirection', $pa_options, 'ASC',
				array( 'validValues' => array( 'ASC', 'DESC' ) ) );
		...
		$start = ( isset( $pa_options['start'] ) && ( (int) $pa_options['start'] > 0 ) ) ? (int) $pa_options['start']
			: 0;
		$limit = ( isset( $pa_options['limit'] ) && ( (int) $pa_options['limit'] > 0 ) ) ? (int) $pa_options['limit']
			: null;
		...
		$qr_res = $o_db->query( $vs_sql . ( $limit_sql ? " LIMIT {$limit_sql}" : '' ),
			array_merge( $va_sql_params, $va_type_restriction_params ) );

		if ( $vb_purify_with_fallback && ( $qr_res->numRows() == 0 ) ) {
			return self::find( $pa_values,
				array_merge( $pa_options, [ 'purifyWithFallback' => false, 'purify' => false ] ) );
		}
	...
	/**
	 * Check if record with primary key id or idno exists. Will check box, giving preference to primary key id unless
	 * the 'idOnly' option is set in which case it will only consider primary key ids. If the 'idnoOnly' option is set
	 * and the model supports idno's then only idno's will be considered
	 *
	...
	public static function exists( $pm_id, $pa_options = null ) {
		...
		if ( is_numeric( $pm_id ) && $pm_id > 0 ) {
			$vn_c = self::find( [ Datamodel::primaryKey( get_called_class() ) => $pm_id ],
				[ 'returnAs' => 'count', 'transaction' => $o_trans ] );
			if ( $vn_c > 0 ) {
				return true;
			}
		}

		if ( ! caGetOption( 'idOnly', $pa_options, false )
		     && ( $vs_idno_fld
				= Datamodel::getTableProperty( get_called_class(), 'ID_NUMBERING_ID_FIELD' ) )
		) {
	 ...
	 * @param bool   $pb_system Indicates if this notification is global and can be seen and interacted with by
	 *                          everyone, system-wide
	 * @param array  $pa_options
	 *                          datetime = Date/time for notification. [Default is now]
	 *                          additionalSubjects = list of ['table_num' => X, 'row_id' => Y, deliverByEmail =>
	 *                          true/false, deliverToInbox => true/false] pairs to add as additional subjects. [Default
	 *                          is null] key = Optional unique md5 signature for notification. This should be unique to
	 *                          the situation in which the notification was generated. [Default is null] data =
	 *                          Additional data to attach to the notification. Data can be in the form of a scalar
	 *                          value or array and will be serialized. [Default is null] deliverByEmail = Deliver
	 *                          notification by email if possible. [Default is false] deliverToInbox = Deliver
	 *                          notification to user's dashboard inbox. [Default is true]
	 *
	 * @return bool True on success
	 */
	public function addNotification( $pn_type, $ps_message, $pb_system = false, array $pa_options = [] ) {
		...
		if ( $this->getPrimaryKey() > 0 ) {
			...
			if ( ( ! defined( "__CA_QUEUE_ENABLED__" ) || ! __CA_QUEUE_ENABLED__ ) && (bool) $vb_send_email
			     && $this->hasField( 'email' )
			     && ( $vs_to_email = $this->get( 'email' ) )
			) {
				if ( caSendMessageUsingView( null, $vs_to_email, $vs_sender_email,
					$this->getAppConfig()->get( 'notification_email_subject' ), "notification.tpl",
					[ 'notification' => $ps_message, 'sent_on' => time() ], null, null, [ 'source' => 'Notification' ] )
				) {
		...
		if ( is_array( $va_additional_subjects ) ) {
			foreach ( $va_additional_subjects as $va_subject ) {
				if ( ! is_array( $va_subject ) || ! isset( $va_subject['table_num'] )
				     || ! isset( $va_subject['row_id'] )
				) {
					continue;
				}
				...
				// Send email immediately when queue is not enabled
				if ( ( ! defined( "__CA_QUEUE_ENABLED__" ) || ! __CA_QUEUE_ENABLED__ ) && (bool) $vb_send_email
				     && $t_instance->hasField( 'email' )
				     && ( $t_instance->load( $va_subject['row_id'] ) )
				     && ( $vs_to_email = $t_instance->get( 'email' ) )
				) {
					if ( caSendMessageUsingView( null, $vs_to_email, $vs_sender_email,
						$this->getAppConfig()->get( 'notification_email_subject' ), "notification.tpl", [
							'notification'     => $ps_message,
							'datetime'         => time(),
							'datetime_display' => caGetLocalizedDate()
						], null, null, [ 'source' => 'Notification' ] )
					) {
						$t_subject->set( 'delivery_email_sent_on', _t( 'now' ) );
						$t_subject->update();
					} // caSendMessageUsingView logs failures
	 ...
	 *
	 * @return mixed Integer, non-zero subject_id if exists; false if notification does not exist, null if error (Eg.
	 *               no row is loaded)
	 */
	 ...
	 *        deliverToInbox = Return notifications marked for deliery to user's dashboard inbox. [Default is null -
	 *        don't filter]
	 *
	 * @return array Array of notifications indexed by notification subject_id. Each value is an array of notification
	 *               content.
	 */
	public function getNotifications( array $pa_options = [] ) {
		...
		if ( ! caGetOption( 'includeRead', $pa_options, false ) ) {
			if ( $vb_inbox ) {
				...
				$va_additional_wheres[]
					= '((ns.was_read = 0 AND ns.delivery_inbox = 1) or (ns.delivery_email_sent_on IS NULL AND ns.delivery_email = 1))';
			}
		}
		...
		while ( $qr_notifications->nextRow() ) {
			...
			$va_row['datetime_display']               = $va_row['datetime'] ? caGetLocalizedDate( $va_row['datetime'] )
				: null;
			$va_row['read_on_display']                = $va_row['read_on'] ? caGetLocalizedDate( $va_row['read_on'] )
				: null;
			$va_row['delivery_email_sent_on_display'] = $va_row['delivery_email_sent_on']
				? caGetLocalizedDate( $va_row['delivery_email_sent_on'] ) : null;
	 ...
	 * This is a simplified version of BundlableLabelableBaseModelWithAttributes::getRelatedItems() for models derived
	 * directly from BaseModel.
	 *
	 * @param $pm_rel_table_name_or_num - the table name or table number of the item type you want to get a list of
	 *                                  (eg. if you are calling this on an ca_item_comments instance passing 'ca_users'
	 *                                  here will get you a list of users related to the comment)
	 * @param $pa_options               - array of options. Supported options are:
	 *
	 *        [Options controlling rows for which data is returned]
	 *            row_ids = Array of primary key values to use when fetching related items. If omitted or set to a null
	 *            value the 'row_id' option will be used. [Default is null] row_id = Primary key value to use when
	 *            fetching related items. If omitted or set to a false value (null, false, 0) then the primary key
	 *            value of the currently loaded row is used. [Default is currently loaded row] start = Zero-based index
	 *            to begin return set at. [Default is 0] limit = Maximum number of related items to return. [Default is
	 *            1000] showDeleted = Return related items that have been deleted. [Default is false] primaryIDs =
	 *            array of primary keys in related table to exclude from returned list of items. Array is keyed on
	 *            table name for compatibility with the parameter as used in the caProcessTemplateForIDs() helper
	 *            [Default is null - nothing is excluded]. where = Restrict returned items to specified field values.
	 *            The fields must be intrinsic and in the related table. This option can be useful when you want to
	 *            efficiently fetch specific rows from a related table. Note that multiple fields/values are logically AND'ed together – all must match for a row to be returned - and that only equivalence is supported. [Default is null] criteria = Restrict returned items using SQL criteria appended directly onto the query. Criteria is used as-is and must be compatible with the generated SQL query. [Default is null]
	 *
	 *        [Options controlling scope of data in return value]
	 *            restrictToLists = Restrict returned items to those that are in one or more specified lists. This
	 *            option is only relevant when fetching related ca_list_items. An array of list list_codes or list_ids
	 *            may be specified. [Default is null] fields = array of fields (in table.fieldname format) to include
	 *            in returned data. [Default is null] idsOnly = Return one-dimensional array of related primary key
	 *            values only. [Default is false]
	 *
	 *        [Options controlling format of data in return value]
	 *            sort = Array list of bundles to sort returned values on. The sortable bundle specifiers are fields
	 *            with or without tablename. Only those fields returned for the related tables (intrinsics) are
	 *            sortable. [Default is null] sortDirection = Direction of sort. Use "asc" (ascending) or "desc"
	 *            (descending). [Default is asc]
	 *
	 *        [Front-end access control]
	 *            checkAccess = Array of access values to filter returned values on. Available for any related table
	 *            with an "access" field (ca_objects, ca_entities, etc.). If omitted no filtering is performed.
	 *            [Default is null] user_id = Perform item level access control relative to specified user_id rather
	 *            than currently logged in user. [Default is user_id for currently logged in user]
	 *
	 * @return array List of related items
	 */
	public function getRelatedItems( $pm_rel_table_name_or_num, $pa_options = null ) {
		global $AUTH_CURRENT_USER_ID;
		$vn_user_id        = ( isset( $pa_options['user_id'] ) && $pa_options['user_id'] ) ? $pa_options['user_id']
			: (int) $AUTH_CURRENT_USER_ID;
		$vb_show_if_no_acl = (bool) ( $this->getAppConfig()->get( 'default_item_access_level' )
		                              > __CA_ACL_NO_ACCESS__ );

		$va_primary_ids = ( isset( $pa_options['primaryIDs'] ) && is_array( $pa_options['primaryIDs'] ) )
			? $pa_options['primaryIDs'] : null;


		$va_get_where = ( isset( $pa_options['where'] ) && is_array( $pa_options['where'] )
		                  && sizeof( $pa_options['where'] ) ) ? $pa_options['where'] : null;

		$va_row_ids = ( isset( $pa_options['row_ids'] ) && is_array( $pa_options['row_ids'] ) ) ? $pa_options['row_ids']
			: null;
		$vn_row_id  = ( isset( $pa_options['row_id'] ) && $pa_options['row_id'] ) ? $pa_options['row_id']
			: $this->getPrimaryKey();
		...
		$va_sort_fields    = ( isset( $pa_options['sort'] ) && is_array( $pa_options['sort'] ) ) ? $pa_options['sort']
			: null;
		$vs_sort_direction = ( isset( $pa_options['sortDirection'] ) && $pa_options['sortDirection'] )
			? $pa_options['sortDirection'] : null;
		...
		$vn_limit = ( isset( $pa_options['limit'] ) && ( (int) $pa_options['limit'] > 0 ) ) ? (int) $pa_options['limit']
			: 1000;
		$vn_start = ( isset( $pa_options['start'] ) && ( (int) $pa_options['start'] > 0 ) ) ? (int) $pa_options['start']
			: 0;
		...
		if ( is_array( $va_get_where ) ) {
			foreach ( $va_get_where as $vs_fld => $vm_val ) {
				if ( $t_rel_item->hasField( $vs_fld ) ) {
					$va_wheres[] = "({$vs_related_table_name}.{$vs_fld} = " . ( ! is_numeric( $vm_val ) ? "'"
					                                                                                      . $this->getDb()
					                                                                                             ->escape( $vm_val )
					                                                                                      . "'"
							: $vm_val ) . ")";
		...
		if ( isset( $pa_options['checkAccess'] ) && is_array( $pa_options['checkAccess'] )
		     && sizeof( $pa_options['checkAccess'] )
		     && $t_rel_item->hasField( 'access' )
		) {
			$va_wheres[] = "({$vs_related_table}.access IN (" . join( ',', $pa_options['checkAccess'] ) . "))";
		}

		if ( ( ! isset( $pa_options['showDeleted'] ) || ! $pa_options['showDeleted'] )
		     && $t_rel_item->hasField( 'deleted' )
		) {
			$va_wheres[] = "({$vs_related_table}.deleted = 0)";
		}

		if ( ( $va_criteria = ( isset( $pa_options['criteria'] ) ? $pa_options['criteria'] : null ) )
		     && ( is_array( $va_criteria ) )
		     && ( sizeof( $va_criteria ) )
		) {
		...
		foreach ( $va_path as $vs_join_table ) {
			...
			$va_joins[]   = 'INNER JOIN ' . $vs_join_table . ' ON ' . $vs_cur_table . '.'
			                . $va_rel_info[ $vs_cur_table ][ $vs_join_table ][0][0] . ' = ' . $vs_join_table . '.'
			                . $va_rel_info[ $vs_cur_table ][ $vs_join_table ][0][1] . "\n";
		...
		if ( is_array( $va_sort_fields ) && sizeof( $va_rels ) ) {
			...
			foreach ( $va_sort_fields as $vn_x => $vs_sort_field ) {
				if ( $vs_sort_field == 'relation_id' ) { // sort by relationship primary key
					if ( $t_item_rel ) {
						$va_sort_fields[ $vn_x ]
							= $vs_sort_field = $t_item_rel->tableName() . '.' . $t_item_rel->primaryKey();
					}
					continue;
				}
					...
					while ( $qr_rel->nextHit() ) {
						...
						foreach ( $va_rels as $vn_rel_id => $va_rel ) {
							if ( $va_rel[ $vs_rel_pk ] == $vn_pk_val ) {
								$va_rels[ $vn_rel_id ][ $vs_key ] = $qr_rel->get( $vs_sort_field,
									array( "delimiter" => ";", 'sortable' => 1 ) );
								break;
							}
						}
					}
	...
	public function setRankAfter( $pn_after_id, $pa_options = null ) {
		...
		if ( ! $pn_after_id ) {
			$qr_res = $o_db->query( "
				SELECT {$vs_item_pk}, `{$vs_rank_fld}` FROM {$vs_item_table} " . ( $vs_parent_sql
					? "WHERE {$vs_parent_sql}" : "" ) . " ORDER BY `{$vs_rank_fld}` LIMIT 1
			", $va_params );
		} else {
			$qr_res = $o_db->query( "
				SELECT {$vs_item_pk}, `{$vs_rank_fld}` FROM {$vs_item_table} WHERE {$vs_item_pk} = ? "
			                        . ( $vs_parent_sql ? "AND {$vs_parent_sql}" : "" ) . " ORDER BY `{$vs_rank_fld}` LIMIT 1
			", $va_params );
		}
		...
		$qr_res = $o_db->query( "
			UPDATE {$vs_item_table} SET `{$vs_rank_fld}` = `{$vs_rank_fld}` + 1 WHERE " . ( $vs_parent_sql
				? "{$vs_parent_sql} AND" : "" ) . " `{$vs_rank_fld}` > ?
		", $va_params );
