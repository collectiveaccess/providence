...
{
    # ------------------------------------------------------------------
    static public $s_label_cache = array();
     ...
     */
    static public $s_label_cache_size = 1024;


    static public $s_labels_by_id_cache = array();
     ...
     */
    static public $s_labels_by_id_cache_size = 1024;
    ...
    public function preferredLabelExistsForLocale($pn_locale_id)
    {
        ...
        if (is_array(
            $ld = $l::find(
                [$this->primaryKey() => $vn_id, 'locale_id' => $pn_locale_id, 'is_preferred' => 1],
    ...
    public function addLabel(
        ...
        $t_label->setLabelTypeList(
            $this->getAppConfig()->get(
                $pb_is_preferred ? "{$vs_table_name}_preferred_label_type_list" : "{$vs_table_name}_nonpreferred_label_type_list"
            )
        );
        ...
        foreach ($pa_label_values as $vs_field => $vs_value) {
            if ($t_label->hasField($vs_field)) {
                if ($vb_truncate_long_labels) {
                    ...
                    if (isset($va_field_len[1]) && ($va_field_len[1] > 0) && (mb_strlen(
                                $vs_value
                            ) > $va_field_len[1])) {
        ...
        $this->opo_app_plugin_manager->hookBeforeLabelInsert(
            array(
        ...
        $this->opo_app_plugin_manager->hookAfterLabelInsert(
            array(
                'id' => $this->getPrimaryKey(),
                'table_num' => $this->tableNum(),
                'table_name' => $this->tableName(),
    ...
    public function editLabel(
        ...
        $this->opo_app_plugin_manager->hookBeforeLabelUpdate(
            array(
                'id' => $this->getPrimaryKey(),
                'table_num' => $this->tableNum(),
                'table_name' => $this->tableName(),
        ...
        try {
            ...
            $this->opo_app_plugin_manager->hookAfterLabelUpdate(
                array(
                    'id' => $this->getPrimaryKey(),
                    'table_num' => $this->tableNum(),
                    'table_name' => $this->tableName(),
    ...
    public function removeLabel($pn_label_id, $pa_options = null)
    {
        ...
        $this->opo_app_plugin_manager->hookBeforeLabelDelete(
            array(
                'id' => $this->getPrimaryKey(),
                'table_num' => $this->tableNum(),
                'table_name' => $this->tableName(),
        ...
        $this->opo_app_plugin_manager->hookAfterLabelDelete(
            array(
                'id' => $this->getPrimaryKey(),
                'table_num' => $this->tableNum(),
                'table_name' => $this->tableName(),
    ...
    public static function find($pa_values, $pa_options = null)
    {
        ...
        if (!is_array($pa_values)) {
            if ((int)$pa_values > 0) {
                ...
                $pa_values = (caGetOption('includeDeleted', $pa_options, false) || !$t_instance->hasField(
                        'deleted'
                    )) ? [] : ['deleted' => 0];
            }
        }
        ...
        foreach ($pa_values as $vs_field => $va_field_values) {
            foreach ($va_field_values as $va_field_value) {
                $vs_op = $va_field_value[0];
                $vm_value = $va_field_value[1];
                if (in_array($vs_field, array('preferred_labels', 'nonpreferred_labels')) && is_array(
                        $va_field_value
        ...
        // ... and in sort list
        $vs_sort_proc = null;
        if ((preg_match("!^{$vs_table}.preferred_labels[\.]{0,1}(.*)!", $ps_sort, $va_matches)) || (preg_match(
                "!^{$vs_table}.nonpreferred_labels[\.]{0,1}(.*)!",
                $ps_sort,
                $va_matches
            ))) {
            $vs_sort_proc = ($va_matches[1] && ($t_label->hasField(
                    $va_matches[1]
                ))) ? "{$vs_label_table}." . $va_matches[1] : "{$vs_label_table}." . $t_label->getDisplayField();
            $vb_has_label_fields = true;
        } elseif (preg_match("!^{$vs_table}\.([A-Za-z0-9_]+)!", $ps_sort, $va_matches) && $t_instance->hasField(
                $va_matches[1]
            )) {
        ...
        if ($vb_has_simple_fields) {
            ...
            if ($t_instance->ATTRIBUTE_TYPE_LIST_CODE) {
                if (isset($pa_values[$vs_type_field_name = $t_instance->getTypeFieldName()]) && !is_numeric(
                        $pa_values[$vs_type_field_name]
                    )) {
                ...
                if (isset($pa_values[$vs_label_type]) && is_array($pa_values[$vs_label_type])) {
                    ...
                    foreach ($pa_values[$vs_label_type] as $vs_field => $va_field_values) {
                        foreach ($va_field_values as $vn_i => $va_field_value) {
                            ...
                            } elseif (caGetOption('allowWildcards', $pa_options, false) && (strpos(
                                        $vm_value,
                                        '%'
                                    ) !== false)) {
        ...
        if ($vb_has_simple_fields) {
            foreach ($pa_values as $vs_field => $va_field_values) {
                foreach ($va_field_values as $va_field_value) {
                    ...
                    } elseif (caGetOption('allowWildcards', $pa_options, false) && !is_array($vm_value) && (strpos(
                                $vm_value,
                                '%'
                            ) !== false)) {
        ...
        if ($vb_has_attributes) {
            ...
            foreach ($pa_values as $vs_field => $va_field_values) {
                foreach ($va_field_values as $vs_key => $va_field_values_by_key) {
                    if (is_array($va_field_values_by_key) && isset($va_field_values_by_key[0]) && !is_array(
                            $va_field_values_by_key[0]
                        ...
                        if (($vn_element_id = array_search($vs_field, $va_element_codes)) !== false) {
                            ...
                            switch ($vn_datatype = ca_metadata_elements::getElementDatatype($vs_field)) {
                                case __CA_ATTRIBUTE_VALUE_CONTAINER__:
                                    $va_subelement_codes = $t_instance->getApplicableElementCodes(
                                        $vn_element_id,
                                        true,
                                        true
                                    );
                                    ...
                                    foreach ($va_field_value as $vs_subfld => $va_subfield_value) {
                                        if (($vn_subelement_id = array_search(
                                                $vs_subfld,
                                                $va_subelement_codes
                                            )) === false) {
                                            continue;
                                        ...
                                        if ($t_element = ca_metadata_elements::getInstance($vs_field)) {
                                            if (is_array($vm_value)) {
                                                foreach ($vm_value as $vn_i => $vm_list_value) {
                                                    $vm_value[$vn_i] = is_numeric(
                                                        $vm_list_value
                                                    ) ? (int)$vm_list_value : (int)caGetListItemID(
                                                ...
                                                $vm_value = is_numeric(
                                                    $vm_value
                                                ) ? (int)$vm_value : (int)caGetListItemID(
                                                    $t_element->get('list_id'),
                                                    $vm_value
                                                );
                                            }
        ...
        if (($vn_attr_count > 1) && (strtolower($ps_boolean) == 'and')) {
            ...
            foreach ($va_attr_sql as $i => $vs_attr_sql) {
                $qr_p = $o_db->query(
                    "SELECT t.{$vs_table_pk} FROM {$vs_table} t INNER JOIN ca_attributes ON ca_attributes.table_num = {$vn_table_num} AND ca_attributes.row_id = t.{$vs_table_pk} INNER JOIN ca_attribute_values ON ca_attribute_values.attribute_id = ca_attributes.attribute_id WHERE {$vs_attr_sql}",
                    $va_attr_params[$i]
                );
        ...
        switch ($ps_return_as) {
            ...
            case 'searchresult':
                ...
                if ($ps_return_as == 'searchresult') {
                    return $t_instance->makeSearchResult(
    ...
    public function get($ps_field, $pa_options = null)
    {
        ...
        $t_instance = $this;
        if ((sizeof($va_tmp) >= 3 && ($va_tmp[2] == 'preferred_labels' && (!$va_tmp[3] || $t_label->hasField(
                            $va_tmp[3]
                        )))) || ($va_tmp[1] == 'hierarchy')) {
            switch ($va_tmp[1]) {
                case 'parent':
                    if (($this->isHierarchical()) && ($vn_parent_id = $this->get(
                    ...
                    if ($this->isHierarchical()) {
                        ...
                        if (is_array($va_children_ids) && sizeof($va_children_ids)) {
                            ...
                            while ($qr_children->nextHit()) {
                                ...
                                $va_data[$vs_sort_key] = array_merge(
                                    $va_data[$vs_sort_key],
                                    $qr_children->get(
                                        $vs_childless_path,
                                        array_merge(
                ...
                case 'hierarchy':
                    ...
                    if (!$vs_template && ($vs_display_field = ($t_label_instance->hasField(
                            $va_tmp[2]
                        )) ? $t_label_instance->tableName() . "." . $va_tmp[2] : ($this->hasField(
                            $va_tmp[2]
                        ) ? $this->tableName() . "." . $va_tmp[2] : null))) {
                    ...
                    if (($this->tableName() == 'ca_objects') && $this->getAppConfig()->get(
                        ...
                        ) && ($vs_coll_rel_type = $this->getAppConfig()->get(
                        ...
                        if (is_array(
                            ...
                            foreach ($va_collections as $vn_i => $va_collection) {
                                if (($va_collections_ancestor_list = $t_collection->getHierarchyAncestors(
                    ...
                    if ($vs_template) {
                        $va_tmp = caProcessTemplateForIDs(
                            $vs_template,
                            $this->tableName(),
                            $va_ancestor_list,
                            array('returnAsArray' => true)
                        ...
                        $va_tmp = $this->getPreferredDisplayLabelsForIDs(
                            $va_ancestor_list,
                            array(
                                'returnAsArray' => true,
            ...
            case 1:
                switch ($va_tmp[0]) {
                    ...
                    case 'preferred_labels':
                        if (!$vb_return_as_array) {
                            $va_labels = caExtractValuesByUserLocale(
                                $t_instance->getPreferredLabels(),
                                null,
                    ...
                    case 'nonpreferred_labels':
                        if (!$vb_return_as_array) {
                            $vs_disp_field = $this->getLabelDisplayField();
                            $va_labels = caExtractValuesByUserLocale(
                                $t_instance->getNonPreferredLabels(),
                                null,
                                $va_preferred_locales
                            );
                            ...
                            if ($vb_convert_codes_to_display_text) {
                                $va_types = $t_list->getItemsForList(
                                    $this->getLabelTableInstance()->getFieldInfo('type_id', 'LIST_CODE'),
    ...
    public function hasBundle($ps_bundle, $pn_type_id = null)
    {
        ...
        if (($vn_num_bits == 1) && (in_array($ps_bundle, array('preferred_labels', 'nonpreferred_labels')))) {
            ...
            if (($va_bundle_bits[0] == $this->tableName()) && (in_array(
                    $va_bundle_bits[1],
    ...
    public function getValuesForExport($pa_options = null)
    {
        ...
        $va_nonpreferred_labels = $this->get(
            $this->tableName() . ".nonpreferred_labels",
            array(
                'returnWithStructure' => true,
                'returnAsArray' => true,
                'returnAllLocales' => true,
     ...
     *
     * @param bool $pb_dont_cache If true then fetched label is not cached and reused for future invokations. Default is true (don't cache) because in some cases [like when editing labels] caching can cause undesirable side-effects. However in read-only situations it measurably increase performance.
    ...
    public function getLabelForDisplay($pb_dont_cache = true, $pm_locale = null, $pa_options = null)
    {
        ...
        $va_tmp = caExtractValuesByUserLocale(
            $this->getLabels(null, __CA_LABEL_TYPE_PREFERRED__, $pb_dont_cache, $pa_options),
            null,
            $va_preferred_locales,
            array()
        );
    ...
    public function getLabelUIValuesFromRequest($po_request, $ps_form_prefix, $ps_label_id, $pb_is_preferred = false)
    {
        ...
        foreach ($va_fields as $vs_field) {
            if ($vs_val = $po_request->getParameter(
                $ps_form_prefix . $vs_pref_key . $vs_field . '_' . $ps_label_id,
                pString
            )) {
    ...
    public function getLabels(
        ...
        if (($pn_mode == __CA_LABEL_TYPE_ANY__) && (caGetBundleAccessLevel(
                    $this->tableName(),
                    'preferred_labels'
        ...
        if (($pn_mode == __CA_LABEL_TYPE_PREFERRED__) && (caGetBundleAccessLevel(
                    $this->tableName(),
                    'preferred_labels'
                ) == __CA_BUNDLE_ACCESS_NONE__)) {
            return null;
        }
        if (($pn_mode == __CA_LABEL_TYPE_NONPREFERRED__) && (caGetBundleAccessLevel(
                    $this->tableName(),
                    'nonpreferred_labels'
                ) == __CA_BUNDLE_ACCESS_NONE__)) {
            return null;
        ...
        $vs_cache_key = caMakeCacheKeyFromOptions(
            array_merge($pa_options, array('table_name' => $this->tableName(), 'id' => $vn_id, 'mode' => (int)$pn_mode))
        );
        if (!$pb_dont_cache && is_array(
                $va_tmp = LabelableBaseModelWithAttributes::$s_label_cache[$this->tableName()][$vn_id][$vs_cache_key]
            )) {
            return $va_tmp;
        }
        if (!($t_label = Datamodel::getInstanceByTableName($this->getLabelTableName(), true))) {
        ...
        if (isset($pa_options['restrict_to_types']) && $pa_options['restrict_to_types'] && is_array(
            ...
            foreach ($pa_options['restrict_to_types'] as $vs_type) {
                ...
                if ($vn_restrict_to_type_id) {
                    $va_children = $t_list_item->getHierarchyChildren(
                        $vn_restrict_to_type_id,
                        array('idsOnly' => true)
                    );
        ...
        $t_label->clear();
        while ($qr_res->nextRow()) {
            $va_labels[$vn_id][$qr_res->get('locale_id')][] = array_merge(
                $qr_res->getRow(),
                array(
                    'form_element' => $t_label->htmlFormElement(
                        $this->getLabelDisplayField(),
                        null
                    )
                )
            );
        ...
        if (is_array(LabelableBaseModelWithAttributes::$s_label_cache[$this->tableName()]) && (sizeof(
                    LabelableBaseModelWithAttributes::$s_label_cache[$this->tableName()]
     ...
     * @return bool True on success, false on error. Success occurs when a default label is successfully added or when a default label is not required. false is only returned when an actionable error state occurs (eg. a blank label is not allowed, or the addition of the default label fails for some reason)
     */
    public function addDefaultLabel($pn_locale_id = null)
    {
        ...
        if (!$this->getPreferredLabelCount()) {
            ...
            if (!(bool)$this->getAppConfig()->get(
                ...
                $this->postError(
                    1130,
                    _t('Label must not be blank'),
                    'LabelableBaseModelWithAttributes->addDefaultLabel()',
                    $this->tableName() . '.preferred_labels'
                );
                return false;
            }
    ...
    public function getPreferredLabelHTMLFormBundle(
        ...
        $o_view->setVar(
            'add_label',
            isset($pa_bundle_settings['add_label'][$g_ui_locale]) ? $pa_bundle_settings['add_label'][$g_ui_locale] : null
        );
        ...
        if ($this->getPrimaryKey()) {
            ...
            if ($this->numErrors()) {
                foreach ($_REQUEST as $vs_key => $vs_value) {
                    if (!preg_match(
                        '/' . $ps_placement_code . $ps_form_name . '_Pref' . 'locale_id_new_([\d]+)/',
                        $vs_key,
                        $va_matches
                    )) {
                        continue;
                    ...
                    if ($vn_new_label_locale_id = $po_request->getParameter(
                        $ps_placement_code . $ps_form_name . '_Pref' . 'locale_id_new_' . $vn_c,
                        pString
                    )) {
                        if (is_array(
                            $va_label_values = $this->getLabelUIValuesFromRequest(
                                $po_request,
                                $ps_placement_code . $ps_form_name,
                                'new_' . $vn_c,
                                true
                            )
                        )) {
    ...
    public function getNonPreferredLabelHTMLFormBundle(
        ...
        if ($this->getPrimaryKey()) {
            ...
            if ($this->numErrors()) {
                foreach ($_REQUEST as $vs_key => $vs_value) {
                    if (!preg_match(
                        '/' . $ps_placement_code . $ps_form_name . '_NPref' . 'locale_id_new_([\d]+)/',
                        $vs_key,
                        $va_matches
                    )) {
                        continue;
                    ...
                    $vn_c = intval($va_matches[1]);
                    if ($vn_new_label_locale_id = $po_request->getParameter(
                        $ps_placement_code . $ps_form_name . '_NPref' . 'locale_id_new_' . $vn_c,
                        pString
                    )) {
                        if (is_array(
                            $va_label_values = $this->getLabelUIValuesFromRequest(
        ...
        if (is_array($this->opa_failed_nonpreferred_label_inserts) && sizeof(
                $this->opa_failed_nonpreferred_label_inserts
            )) {
            $va_new_labels_to_force_due_to_error = $this->opa_failed_preferred_label_inserts;
        }
     ...
     *
     * @param bool $pb_dont_cache If true label cache is bypassed; default is false
     * @param array $pa_options Array of options. Supported options are those of getLabels()
     *
     * @return array List of labels
     */
    public function getDisplayLabels($pb_dont_cache = false, $pa_options = null)
    {
        return caExtractValuesByUserLocale(
            $this->getPreferredLabels(null, $pb_dont_cache, $pa_options),
            null,
            null,
            array()
        );
    ...
    public function getPreferredDisplayLabelsForIDs($pa_ids, $pa_options = null)
    {
        ...
        while ($qr_res->nextRow()) {
            if ($vb_return_all_locales) {
                $va_labels[(int)$qr_res->get($vs_pk)][(int)$qr_res->get('locale_id')][] = $qr_res->get(
                    $vs_display_field
                );
            } else {
        ...
        if (sizeof(
                LabelableBaseModelWithAttributes::$s_labels_by_id_cache
        ...
        return LabelableBaseModelWithAttributes::$s_labels_by_id_cache[$vs_cache_key] = caExtractValuesByUserLocale(
            $va_sorted_labels
        );
    }
    # ------------------------------------------------------------------
    ...
    public function getNonPreferredDisplayLabelsForIDs($pa_ids, $pa_options = null)
    {
        ...
        $vs_cache_key = md5(
            $this->tableName() . "/" . print_r($pa_ids, true) . '/' . print_R($pa_options, true) . '_non_preferred'
        );
        if (!isset($pa_options['noCache']) && !$pa_options['noCache'] && LabelableBaseModelWithAttributes::$s_labels_by_id_cache[$vs_cache_key]) {
            return LabelableBaseModelWithAttributes::$s_labels_by_id_cache[$vs_cache_key];
        }
    ...
    public function getHierarchyWithLabels($pn_id = null, $pa_options = null)
    {
        ...
        $va_hier = array();
        while ($qr_res->nextRow()) {
            if (isset($pa_options['preferred_only']) && (bool)$pa_options['preferred_only'] && (!$qr_res->get(
                    'is_preferred'
                ))) {
                continue;
            }
