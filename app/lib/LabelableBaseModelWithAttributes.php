...
class LabelableBaseModelWithAttributes extends BaseModelWithAttributes implements ILabelable {
	...
	public function preferredLabelExistsForLocale( $pn_locale_id ) {
		...
		if ( is_array(
			$ld = $l::find( [ $this->primaryKey() => $vn_id, 'locale_id' => $pn_locale_id, 'is_preferred' => 1 ],
				[ 'returnAs' => 'array', 'transaction' => $this->getTransaction() ] ) )
		) {
			return ( sizeof( $ld ) > 0 );
		}
	...
	/**
	 *    Adds a label to the currently loaded row; the $pa_label_values array an associative array where keys are
	 *    field names and values are the field values; some label are defined by more than a single field (people's
	 *    names for instance) which is why the label value is an array rather than a simple scalar value
	 *
	 ...
	 *                          truncateLongLabels = truncate label values that exceed the maximum storable length.
	 *                          [Default=false] queueIndexing =
	 *
	 * @return int id for newly created label, false on error or null if no row is loaded
	 */
	public function addLabel(
		$pa_label_values, $pn_locale_id, $pn_type_id = null, $pb_is_preferred = false, $pa_options = null
	) {
		if ( ! ( $vn_id = $this->getPrimaryKey() ) ) {
			return null;
		}
		...
		$t_label->setLabelTypeList( $this->getAppConfig()->get( $pb_is_preferred
			? "{$vs_table_name}_preferred_label_type_list" : "{$vs_table_name}_nonpreferred_label_type_list" ) );
		...
		foreach ( $pa_label_values as $vs_field => $vs_value ) {
			if ( $t_label->hasField( $vs_field ) ) {
				if ( $vb_truncate_long_labels ) {
					...
					if ( isset( $va_field_len[1] ) && ( $va_field_len[1] > 0 )
					     && ( mb_strlen( $vs_value ) > $va_field_len[1] )
					) {
		...
		$this->opo_app_plugin_manager->hookBeforeLabelInsert( array(
			'id'             => $this->getPrimaryKey(),
			'table_num'      => $this->tableNum(),
			'table_name'     => $this->tableName(),
			'instance'       => $this,
			'label_instance' => $t_label
		) );

		$vn_label_id = $t_label->insert( array( 'queueIndexing' => $pb_queue_indexing, 'subject' => $this ) );

		$this->opo_app_plugin_manager->hookAfterLabelInsert( array(
			'id'             => $this->getPrimaryKey(),
			'table_num'      => $this->tableNum(),
			'table_name'     => $this->tableName(),
			'instance'       => $this,
	...
	public function editLabel(
		$pn_label_id, $pa_label_values, $pn_locale_id, $pn_type_id = null, $pb_is_preferred = false, $pa_options = null
	) {
		if ( ! ( $vn_id = $this->getPrimaryKey() ) ) {
			return null;
		}
		...
		$this->opo_app_plugin_manager->hookBeforeLabelUpdate( array(
			'id'             => $this->getPrimaryKey(),
			'table_num'      => $this->tableNum(),
			'table_name'     => $this->tableName(),
			'instance'       => $this,
		...
		try {
			...
			$this->opo_app_plugin_manager->hookAfterLabelUpdate( array(
				'id'             => $this->getPrimaryKey(),
				'table_num'      => $this->tableNum(),
				'table_name'     => $this->tableName(),
				'instance'       => $this,
	...
	public function removeLabel( $pn_label_id, $pa_options = null ) {
		...
		$this->opo_app_plugin_manager->hookBeforeLabelDelete( array(
			'id'             => $this->getPrimaryKey(),
			'table_num'      => $this->tableNum(),
			'table_name'     => $this->tableName(),
			'instance'       => $this,
		...
		$this->opo_app_plugin_manager->hookAfterLabelDelete( array(
			'id'             => $this->getPrimaryKey(),
			'table_num'      => $this->tableNum(),
			'table_name'     => $this->tableName(),
			'instance'       => $this,
	 ...
	 *
	 * @param int   $pn_mode    Set to __CA_LABEL_TYPE_PREFERRED__ or __CA_LABEL_TYPE_NONPREFERRED__ to restrict
	 *                          removal. Default is __CA_LABEL_TYPE_ANY__ (no restriction)
	 * @param array $pa_options Options include:
	 *                          locales = Limit removal to specific locales. Values must be valid locale ids or codes.
	 *                          A single locale or array of locales may be passed. [Default is null, delete all labels]
	...
	public function removeAllLabels( $pn_mode = __CA_LABEL_TYPE_ANY__, $pa_options = null ) {
		...
		foreach ( $va_labels as $vn_id => $va_labels_by_locale ) {
			foreach ( $va_labels_by_locale as $vn_locale_id => $va_labels ) {
				if ( is_array( $va_locale_ids ) && sizeof( $va_locale_ids )
				     && ! in_array( $vn_locale_id, $va_locale_ids )
				) {
					continue;
				}
				foreach ( $va_labels as $vn_i => $va_label ) {
					...
					$this->setAsChanged( (bool) $va_label['is_preferred'] ? 'preferred_labels'
						: 'nonpreferred_labels' );
				}
			}
		}
	 ...
	 *
	 */
	public function replaceLabel(
		$pa_label_values, $pn_locale_id, $pn_type_id = null, $pb_is_preferred = true, $pa_options = null
	) {
		if ( ! ( $vn_id = $this->getPrimaryKey() ) ) {
			return null;
		}
		$va_labels = $this->getLabels( array( $pn_locale_id ),
			$pb_is_preferred ? __CA_LABEL_TYPE_PREFERRED__ : __CA_LABEL_TYPE_NONPREFERRED__ );
	...
	public function loadByLabel( $pa_label_values, $pa_table_values = null ) {
		...
		$vs_sql = "
 				...
 				INNER JOIN  " . $this->tableName() . " ON " . $this->tableName() . "." . $this->primaryKey() . " = "
		          . $this->getLabelTableName() . "." . $this->primaryKey() . " 
 				WHERE	
	 ...
	 *
	 * Matching is performed using values in $pa_values. Partial and pattern matching are supported as are
	 ...
	 *
	 * Keys in the $pa_values parameters must be valid fields in the table which the model sub-class represents. You
	 * may also search on preferred and non-preferred labels by specified keys and values for label table fields in
	 ...
	 *
	 * You can specify operators with an expanded array format where each value is an array containing both an operator
	 * and a value. Ex.:
	 *
	 * ["idno" => ['=', '2012.001'], "access" => ['>', 1], 'preferred_labels' => ['name' => ['LIKE', '%Luna Park at
	 * Night%']]]
	 *
	 * You may also specify lists of values for use with the IN and NOT IN operator:
	 *
	 * ["idno" => ['=', '2012.001'], "access" => ['IN', [1,2,3]], 'preferred_labels' => ['name' => ['LIKE', '%Luna Park
	 * at Night%']]]
	 *
	 * Range searches may be performed using the BETWEEN operator:
	 *
	 * ["idno" => ['=', '2012.001'], "access" => ['BETWEEN', [1,3]], 'preferred_labels' => ['name' => ['LIKE', '%Luna
	 * Park at Night%']]]
	 *
	 ...
	 * ["idno" => [['=', '2012.001'], ['=', '2012.001']]]
	 *
	 * Limited search capabilities on attribute values are provided. Search is on the raw attribute values stored in
	 * the database, with provision for translation of user values provided for DateRange and Currency values. When
	 * searching on a date range or currency you can pass the parseable date or currency value, which will be
	 * translated and searched apppropriately. Container values may be searched on constituent subfields. An example
	 * search on a text, a currency and a date range attribute:
	 *
	 * ["date" => ["=", "6/1954"], "valuation" => [">", "€200"], "description" => ["LIKE", "%Suez Canal%"]]
	 *
	 * Note that the date and currency values will be converted to internal representation formats for the query, while
	 * the text will be searched as-is.
	 *
	 * Containers may be searches by specifying the container field name and an array of subfields. Ex.
	 *
	 * ["address" => ["address_line1" => "100 Main Street", "city" => "Cole Harbour", "province" => "Nova Scotia",
	 * "country" => "Canada"]]
	 *
	 * LabelableBaseModelWithAttributes::find() is not a replacement for the SearchEngine. It is intended as a quick
	 * and convenient way to programatically fetch rows using simple, clear cut criteria. If you need to fetch rows
	 * based upon an identifer or status value LabelableBaseModelWithAttributes::find() will be quicker and less code
	 * than using the SearchEngine. For full-text searches, complex searches on attributes, or searches that require
	 * transformations or complex boolean operations use the SearchEngine.
	 *
	 * @param array $pa_values  An array of values to match. Keys are field names, metadata element codes or
	 *                          preferred_labels and /or nonpreferred_labels. This must be an array with at least one
	 *                          key-value pair where the key is a valid field name for the model. If you pass an
	 *                          integer instead of an array it will be used as the primary key value for the table;
	 *                          result will be returned as "firstModelInstance" unless the returnAs option is
	 *                          explicitly set.
	 ...
	 *                                           instance in the array returned by 'modelInstances' count                    = the number of matches arrays                    = an array of arrays, each of which contains values for each intrinsic field in the model
	 *
	 *            The default is ids
	 *
	 ...
	 *            The default is AND
	 *
	 *        labelBoolean = determines how multiple field values in $pa_values['preferred_labels'] and
	 *        $pa_values['nonpreferred_labels'] are combined to produce the final result. Possible values are: AND
	 *                          = find rows that match all criteria in
	 *                          $pa_values['preferred_labels']/$pa_values['nonpreferred_labels'] OR
	 *                             = find rows that match any criteria in
	 *                             $pa_values['preferred_labels']/$pa_values['nonpreferred_labels']
	 *
	 *            The default is AND
	 *
	 *        sort = field to sort on. Must be in <table>.<field> format and be an intrinsic field in either the
	 *        primary table or the label table. Sort order can be set using the sortDirection option. sortDirection =
	 *        the direction of the sort. Values are ASC (ascending) and DESC (descending). [Default is ASC]
	 *        allowWildcards = consider "%" as a wildcard when searching. Any term including a "%" character will be
	 *        queried using the SQL LIKE operator. [Default is false] purify = process text with HTMLPurifier before
	 *        search. Purifier encodes &, < and > characters, and performs other transformations that can cause
	 *        searches on literal text to fail. If you are purifying all input (the default) then leave this set true.
	 *        [Default is true] purifyWithFallback = executes the search with "purify" set and falls back to search
	 *        with unpurified text if nothing is found. [Default is false] checkAccess = array of access values to
	 *        filter results by; if defined only items with the specified access code(s) are returned. Only supported
	 *        for <table_name>.hierarchy.preferred_labels and <table_name>.children.preferred_labels because these returns sets of items. For <table_name>.parent.preferred_labels, which returns a single row at most, you should do access checking yourself. (Everything here applies equally to nonpreferred_labels) restrictToTypes = Restrict returned items to those of the specified types. An array of list item idnos and/or item_ids may be specified. [Default is null] dontIncludeSubtypesInTypeRestriction = If restrictToTypes is set, by default the type list is expanded to include subtypes (aka child types). If set, no expansion will be performed. [Default is false] includeDeleted = If set deleted rows are returned in result set. [Default is false] dontFilterByACL = If set don't enforce item-level ACL rules. [Default is false]
	 *
	 * @return mixed Depending upon the returnAs option setting, an array, subclass of LabelableBaseModelWithAttributes
	 *               or integer may be returned.
	 */
	public static function find( $pa_values, $pa_options = null ) {
		...
		if ( ! is_array( $pa_values ) ) {
			if ( (int) $pa_values > 0 ) {
				...
				$pa_values = ( caGetOption( 'includeDeleted', $pa_options, false )
				               || ! $t_instance->hasField( 'deleted' ) ) ? [] : [ 'deleted' => 0 ];
			}
		}
		...
		$ps_label_boolean  = caGetOption( 'labelBoolean', $pa_options, 'and',
			array( 'forceLowercase' => true, 'validValues' => array( 'and', 'or' ) ) );
		$ps_sort           = caGetOption( 'sort', $pa_options, null );
		$ps_sort_direction = caGetOption( 'sortDirection', $pa_options, 'ASC',
		...
		foreach ( $pa_values as $vs_field => $va_field_values ) {
			foreach ( $va_field_values as $va_field_value ) {
				$vs_op    = $va_field_value[0];
				$vm_value = $va_field_value[1];
				if ( in_array( $vs_field, array( 'preferred_labels', 'nonpreferred_labels' ) )
				     && is_array( $va_field_value )
				     && sizeof( $va_field_value )
				) {
		...
		// ... and in sort list
		$vs_sort_proc = null;
		if ( ( preg_match( "!^{$vs_table}.preferred_labels[\.]{0,1}(.*)!", $ps_sort, $va_matches ) )
		     || ( preg_match( "!^{$vs_table}.nonpreferred_labels[\.]{0,1}(.*)!", $ps_sort, $va_matches ) )
		) {
			$vs_sort_proc        = ( $va_matches[1] && ( $t_label->hasField( $va_matches[1] ) ) ) ? "{$vs_label_table}."
			                                                                                        . $va_matches[1]
				: "{$vs_label_table}." . $t_label->getDisplayField();
			$vb_has_label_fields = true;
		} elseif ( preg_match( "!^{$vs_table}\.([A-Za-z0-9_]+)!", $ps_sort, $va_matches )
		           && $t_instance->hasField( $va_matches[1] )
		) {
		...
		if ( $vb_has_simple_fields ) {
			...
			if ( $t_instance->ATTRIBUTE_TYPE_LIST_CODE ) {
				if ( isset( $pa_values[ $vs_type_field_name = $t_instance->getTypeFieldName() ] )
				     && ! is_numeric( $pa_values[ $vs_type_field_name ] )
				) {
		...
		if ( $vb_has_label_fields && $t_label ) {
			$va_joins[]
				= " INNER JOIN {$vs_label_table} ON {$vs_label_table}.{$vs_table_pk} = {$vs_table}.{$vs_table_pk} ";
			...
			foreach ( [ 'preferred_labels', 'nonpreferred_labels' ] as $vs_label_type ) {
				...
				if ( isset( $pa_values[ $vs_label_type ] ) && is_array( $pa_values[ $vs_label_type ] ) ) {
					...
					if ( $t_label->hasField( 'is_preferred' ) ) {
						$va_label_sql_wheres[] = "({$vs_label_table}.is_preferred = " . ( ( $vs_label_type
						                                                                    == 'preferred_labels' )
								? "1" : "0" ) . ")";
					}
							...
							if ( $t_label->_getFieldTypeType( $vs_field ) == 0 ) {
								...
								if ( is_array( $vm_value ) ) {
									foreach ( $vm_value as $vn_i => $vs_value ) {
										$vm_value[ $vn_i ] = $t_label->quote( $vs_field,
			...
			foreach ( $pa_values as $vs_field => $va_field_values ) {
				foreach ( $va_field_values as $va_field_value ) {
					...
					if ( $t_instance->_getFieldTypeType( $vs_field ) == 0 ) {
						if ( ! caIsValidSqlOperator( $vs_op, [ 'type' => 'numeric', 'nullable' => true ] ) ) {
							throw new ApplicationException( _t( 'Invalid numeric operator: %1', $vs_op ) );
						}
					} else {
						if ( ! caIsValidSqlOperator( $vs_op, [ 'type' => 'string', 'nullable' => true ] ) ) {
							throw new ApplicationException( _t( 'Invalid string operator: %1', $vs_op ) );
						}
					}
					...
					} elseif ( caGetOption( 'allowWildcards', $pa_options, false ) && ! is_array( $vm_value )
					           && ( strpos( $vm_value, '%' ) !== false )
					) {
		...
		if ( $vb_has_attributes ) {
			$va_joins[]
				= " INNER JOIN ca_attributes ON ca_attributes.row_id = {$vs_table}.{$vs_table_pk} AND ca_attributes.table_num = {$vn_table_num} ";
			$va_joins[]
			...
			foreach ( $pa_values as $vs_field => $va_field_values ) {
				foreach ( $va_field_values as $vs_key => $va_field_values_by_key ) {
					if ( is_array( $va_field_values_by_key ) && isset( $va_field_values_by_key[0] )
					     && ! is_array( $va_field_values_by_key[0] )
					     && caIsValidSqlOperator( $va_field_values_by_key[0], [ 'nullable' => true, 'isList' => true ] )
					) {
						...
						if ( ( $vn_element_id = array_search( $vs_field, $va_element_codes ) ) !== false ) {
							...
							switch ( $vn_datatype = ca_metadata_elements::getElementDatatype( $vs_field ) ) {
								case __CA_ATTRIBUTE_VALUE_CONTAINER__:
									$va_subelement_codes = $t_instance->getApplicableElementCodes( $vn_element_id, true,
										true );

									foreach ( $va_field_value as $vs_subfld => $va_subfield_value ) {

										if ( ( $vn_subelement_id = array_search( $vs_subfld, $va_subelement_codes ) )
										     === false
										) {
											continue;
										}
										...
										if ( ! ( $vs_subfld = Attribute::getSortFieldForDatatype( $vn_datatype ) ) ) {
											$vs_subfld = 'value_longtext1';
										}
								...
								case __CA_ATTRIBUTE_VALUE_DATERANGE__:
									if ( is_array( $va_date = caDateToHistoricTimestamps( $vm_value ) ) ) {
										$va_q[]
											              = "((ca_attribute_values.element_id = {$vn_element_id}) AND ((ca_attribute_values.value_decimal1 BETWEEN ? AND ?) OR (ca_attribute_values.value_decimal2 BETWEEN ? AND ?)))";
										$va_attr_params[] = [
											$va_date['start'],
											$va_date['end'],
											$va_date['start'],
								...
								case __CA_ATTRIBUTE_VALUE_CURRENCY__:
									if ( is_array( $va_parsed_value = caParseCurrencyValue( $vm_value ) ) ) {
										$va_q[]
											              = "((ca_attribute_values.element_id = {$vn_element_id}) AND ((ca_attribute_values.value_longtext1 = ?) OR (ca_attribute_values.value_decimal1 {$vs_op} ?)))";
										...
										if ( $t_element = ca_metadata_elements::getInstance( $vs_field ) ) {
											if ( is_array( $vm_value ) ) {
												foreach ( $vm_value as $vn_i => $vm_list_value ) {
													$vm_value[ $vn_i ] = is_numeric( $vm_list_value )
														? (int) $vm_list_value
														: (int) caGetListItemID( $t_element->get( 'list_id' ),
															$vm_list_value );
												}
											} else {
												$vm_value = is_numeric( $vm_value ) ? (int) $vm_value
													: (int) caGetListItemID( $t_element->get( 'list_id' ), $vm_value );
											}
										}
									}
		...
		if ( ( $vn_attr_count > 1 ) && ( strtolower( $ps_boolean ) == 'and' ) ) {
			...
			foreach ( $va_attr_sql as $i => $vs_attr_sql ) {
				$qr_p
					= $o_db->query( "SELECT t.{$vs_table_pk} FROM {$vs_table} t INNER JOIN ca_attributes ON ca_attributes.table_num = {$vn_table_num} AND ca_attributes.row_id = t.{$vs_table_pk} INNER JOIN ca_attribute_values ON ca_attribute_values.attribute_id = ca_attributes.attribute_id WHERE {$vs_attr_sql}",
					$va_attr_params[ $i ] );
			...
			case 'searchresult':
				...
				if ( $ps_return_as == 'searchresult' ) {
					return $t_instance->makeSearchResult( $t_instance->tableName(), $va_ids,
						[ 'sort' => $ps_sort, 'sortDirection' => $ps_sort_direction ] );
				} else {
	...
	/**
	 * Find row(s) with fields having values matching specific values. Returns a model instance for the first record
	 * found. This is a convenience wrapper around LabelableBaseModelWithAttributes::find() and support all options
	 * offered by that method.
	 *
	 * @see LabelableBaseModelWithAttributes::find()
	 */
	...
	/**
	 * Find row(s) with fields having values matching specific values. Returns a the primary key (id) of the first
	 * record found. This is a convenience wrapper around LabelableBaseModelWithAttributes::find() and support all
	 * options offered by that method.
	 *
	 * @see LabelableBaseModelWithAttributes::find()
	 */
	 ...
	 *                    template -
	 *                    locale -
	 *                    returnAllLocales - Returns requested value in all locales for which it is defined. Default is
	 *                    false. Note that this is not supported for hierarchy specifications (eg.
	 *                    ca_objects.hierarchy). direction - For hierarchy specifications (eg. ca_objects.hierarchy)
	 *                    this determines the order in which the hierarchy is returned. ASC will return the hierarchy
	 *                    root first while DESC will return it with the lowest node first. Default is ASC. top - For
	 *                    hierarchy specifications (eg. ca_objects.hierarchy) this option, if set, will limit the
	 *                    returned hierarchy to the first X nodes from the root down. Default is to not limit. bottom -
	 *                    For hierarchy specifications (eg. ca_objects.hierarchy) this option, if set, will limit the
	 *                    returned hierarchy to the first X nodes from the lowest node up. Default is to not limit.
	 *                    hierarchicalDelimiter - Text to place between items in a hierarchy for a hierarchical
	 *                    specification (eg. ca_objects.hierarchy) when returning as a string removeFirstItems - If set to a non-zero value, the specified number of items at the top of the hierarchy will be omitted. For example, if set to 2, the root and first child of the hierarchy will be omitted. Default is zero (don't delete anything). checkAccess = array of access values to filter results by; if defined only items with the specified access code(s) are returned. Only supported for <table_name>.hierarchy.preferred_labels and <table_name>.children.preferred_labels because these returns sets of items. For <table_name>.parent.preferred_labels, which returns a single row at most, you should do access checking yourself. (Everything here applies equally to nonpreferred_labels) sort = optional bundles to sort returned values on. Only supported for <table_name>.children.preferred_labels. The bundle specifiers are fields with or without tablename. sort_direction = direction to sort results by, either 'asc' for ascending order or 'desc' for descending order; default is 'asc' convertCodesToDisplayText = if true then non-preferred label type_ids are automatically converted to display text in the current locale; default is false (return non-preferred label type_id raw)
	 */
	public function get( $ps_field, $pa_options = null ) {
		$vs_template                      = ( isset( $pa_options['template'] ) ) ? $pa_options['template'] : null;
		$vb_return_as_array               = ( isset( $pa_options['returnAsArray'] ) )
			? (bool) $pa_options['returnAsArray'] : false;
		$vb_return_all_locales            = ( isset( $pa_options['returnAllLocales'] ) )
			? (bool) $pa_options['returnAllLocales'] : false;
		$vs_delimiter                     = ( isset( $pa_options['delimiter'] ) ) ? $pa_options['delimiter'] : ' ';
		$vb_convert_codes_to_display_text = ( isset( $pa_options['convertCodesToDisplayText'] ) )
			? (bool) $pa_options['convertCodesToDisplayText'] : false;
		...
		if ( ( sizeof( $va_tmp ) >= 3
			...
			switch ( $va_tmp[1] ) {
				...
				case 'children':
					if ( $this->isHierarchical() ) {
						...
						if ( is_array( $va_children_ids ) && sizeof( $va_children_ids ) ) {
							...
							$vs_sort_direction = ( isset( $pa_options['sort_direction'] )
									...
									array( 'asc', 'desc' ) ) ) ? strtolower( $pa_options['sort_direction'] ) : 'asc';
							...
							while ( $qr_children->nextHit() ) {
								...
								$va_data[ $vs_sort_key ] = array_merge( $va_data[ $vs_sort_key ],
									$qr_children->get( $vs_childless_path,
										array_merge( $pa_options, array( 'returnAsArray' => true ) ) ) );
							}
							ksort( $va_data );
					...
					break;
				case 'hierarchy':
					$vs_direction = ( isset( $pa_options['direction'] ) ) ? strtoupper( $pa_options['direction'] )
						: null;
					...
					if ( ! $vs_template
					     && ( $vs_display_field = ( $t_label_instance->hasField( $va_tmp[2] ) )
							? $t_label_instance->tableName() . "." . $va_tmp[2]
							: ( $this->hasField( $va_tmp[2] ) ? $this->tableName() . "." . $va_tmp[2] : null ) )
					) {
						$vs_template = "^{$vs_display_field}";
					}

					$vn_top_id = null;
					if ( ! ( $va_ancestor_list = $this->getHierarchyAncestors( null,
					...
					if ( ( $this->tableName() == 'ca_objects' )
						...
						if ( is_array( $va_collections = $t_object->getRelatedItems( 'ca_collections',
							array( 'restrictToRelationshipTypes' => $vs_coll_rel_type ) ) )
						) {
							require_once( __CA_MODELS_DIR__ . '/ca_collections.php' );
							$t_collection = new ca_collections();
							foreach ( $va_collections as $vn_i => $va_collection ) {
								if ( ( $va_collections_ancestor_list
					...
					if ( $vs_template ) {
						$va_tmp = caProcessTemplateForIDs( $vs_template, $this->tableName(), $va_ancestor_list,
							array( 'returnAsArray' => true ) );
					} else {
						$va_tmp = $this->getPreferredDisplayLabelsForIDs( $va_ancestor_list,
							array( 'returnAsArray' => true, 'returnAllLocales' => $vb_return_all_locales ) );
					}
					...
					if ( caGetOption( 'returnAsLink', $pa_options, false ) ) {
						$va_tmp = caCreateLinksFromText( array_values( $va_tmp ), $this->tableName(),
							array_keys( $va_tmp ) );
					}
					...
					if ( $vb_return_as_array ) {
						...
						$vs_hier_delimiter = ( isset( $pa_options['hierarchicalDelimiter'] ) )
							? $pa_options['hierarchicalDelimiter'] : $pa_options['delimiter'];
			...
			case 1:
				switch ( $va_tmp[0] ) {
					...
					case 'preferred_labels':
						if ( ! $vb_return_as_array ) {
							$va_labels     = caExtractValuesByUserLocale( $t_instance->getPreferredLabels(), null,
							...
							foreach ( $va_labels as $vn_row_id => $va_label_list ) {
								foreach ( $va_label_list as $vn_i => $va_label ) {
									if ( $vs_template ) {
										$va_values[] = caProcessTemplate( $vs_template, $va_label,
											array( 'removePrefix' => 'preferred_labels.' ) );
									} else {
							...
							if ( $vb_return_all_locales ) {
								...
								$va_labels           = caExtractValuesByUserLocale( $va_labels, null,
									$va_preferred_locales );
					...
					case 'nonpreferred_labels':
						if ( ! $vb_return_as_array ) {
							$vs_disp_field = $this->getLabelDisplayField();
							$va_labels     = caExtractValuesByUserLocale( $t_instance->getNonPreferredLabels(), null,
								$va_preferred_locales );

							$t_list = new ca_lists();
							if ( $vb_convert_codes_to_display_text ) {
								$va_types = $t_list->getItemsForList( $this->getLabelTableInstance()
								                                           ->getFieldInfo( 'type_id', 'LIST_CODE' ),
									array( 'extractValuesByUserLocale' => true ) );
							}
							...
							foreach ( $va_labels as $vn_row_id => $va_label_list ) {
								foreach ( $va_label_list as $vn_i => $va_label ) {
									if ( $vs_template ) {
										...
										$va_label_values['typename_singular']
										                 = $va_types[ $va_label['type_id'] ]['name_singular'];
										$va_label_values['typename_plural']
										                 = $va_types[ $va_label['type_id'] ]['name_plural'];

										if ( $vb_convert_codes_to_display_text ) {
											$va_label_values['type_id']
												= $va_types[ $va_label['type_id'] ]['name_singular'];
										}
										$va_values[] = caProcessTemplate( $vs_template, $va_label_values,
											array( 'removePrefix' => 'nonpreferred_labels.' ) );
									} else {
									...
									if ( $vb_return_all_locales ) {
										foreach ( $va_labels as $vn_label_id => $va_labels_by_locale ) {
											foreach ( $va_labels_by_locale as $vn_locale_id => $va_label_list ) {
												foreach ( $va_label_list as $vn_i => $va_label ) {
													$va_labels[ $vn_label_id ][ $vn_locale_id ][ $vn_i ]
														= $va_label[ $va_tmp[2] ];
												}
											}
										}
								...
								foreach ( $va_labels as $vn_row_id => $va_label_list ) {
									foreach ( $va_label_list as $vn_i => $va_label ) {
										if ( $vs_template ) {
											...
											if ( $vb_convert_codes_to_display_text
											     && ( $vs_disp_field == 'type_id' )
											) {
												$va_values[]
													= $va_types[ $va_label[ $vs_disp_field ] ]['name_singular'];
											} else {
												$va_values[] = $va_label[ $vs_disp_field ];
											}
										}
	...
	public function hasBundle( $ps_bundle, $pn_type_id = null ) {
		...
		if ( ( $vn_num_bits == 1 ) && ( in_array( $ps_bundle, array( 'preferred_labels', 'nonpreferred_labels' ) ) ) ) {
			...
			if ( ( $va_bundle_bits[0] == $this->tableName() )
			     && ( in_array( $va_bundle_bits[1], array( 'preferred_labels', 'nonpreferred_labels' ) ) )
			) {
				return true;
	...
	public function getValuesForExport( $pa_options = null ) {
		...
		// get labels
		$va_preferred_labels = $this->get( $this->tableName() . ".preferred_labels", array(
			'returnWithStructure' => true,
			'returnAsArray'       => true,
			'returnAllLocales'    => true,
			'assumeDisplayField'  => false
		) );
		...
		$va_nonpreferred_labels = $this->get( $this->tableName() . ".nonpreferred_labels", array(
			'returnWithStructure' => true,
			'returnAsArray'       => true,
			'returnAllLocales'    => true,
			'assumeDisplayField'  => false
	 ...
	 *
	 * @param boolean $pb_dont_cache If true then fetched label is not cached and reused for future invokations.
	 *                               Default is true (don't cache) because in some cases [like when editing labels]
	 *                               caching can cause undesirable side-effects. However in read-only situations it
	 *                               measurably increase performance.
	 * @param mixed   $pm_locale     If set to a valid locale_id or locale code value will be returned in specified
	 *                               language instead of user's default language, assuming the label is available in
	 *                               the specified language. If it is not the value will be returned in a language that
	 *                               is available using the standard fall-back procedure.
	...
	public function getLabelForDisplay( $pb_dont_cache = true, $pm_locale = null, $pa_options = null ) {
		...
		$va_tmp   = caExtractValuesByUserLocale( $this->getLabels( null, __CA_LABEL_TYPE_PREFERRED__, $pb_dont_cache,
			$pa_options ), null, $va_preferred_locales, array() );
	...
	/**
	 * Returns list of names of fields that may be used to generate the display field. For most labels this will be
	 * empty. For ca_entities this will be a list on constituent name fields.
	 *
	 * @return array
	 */
	...
	public function getLabelUIValuesFromRequest( $po_request, $ps_form_prefix, $ps_label_id, $pb_is_preferred = false
		...
		foreach ( $va_fields as $vs_field ) {
			if ( $vs_val = $po_request->getParameter( $ps_form_prefix . $vs_pref_key . $vs_field . '_' . $ps_label_id,
				pString )
			) {
	...
	/**
	 * Returns labels associated with this row. By default all labels - preferred and non-preferred, and from all
	 * locales - are returned. You can limit the returned labels to specified locales by passing a list of locale_ids
	 * (numeric ids, *not* locale codes) in $pn_locale_ids. Similarly you can limit return labels to preferred on
	 * non-preferred by setting $pn_mode to __CA_LABEL_TYPE_PREFERRED__ or __CA_LABEL_TYPE_NONPREFERRED__
	 *
	 * getLabels() returns an associated array keyed by the primary key of the item the label is attached to; each
	 * value is an array keyed by locale_id, the values of which is a list of associative arrays with the label table
	 * data. This return format is designed to be digested by the displayHelper function caExtractValuesByUserLocale()
	 *
	 * @param array   $pa_locale_ids
	 * @param int     $pn_mode
	 * @param boolean $pb_dont_cache
	 * @param array   $pa_options Array of options. Supported options are:
	 *                            row_id = The row_id to return labels for. If omitted the id of the currently loaded
	 *                            row is used. If row_id is not set and now row is loaded then getLabels() will return
	 *                            null. restrict_to_types = an optional array of numeric type ids or alphanumeric type
	 *                            identifiers to restrict the returned labels to. The types are list items in a list
	 *                            specified in app.conf (or, if not defined there, by hardcoded constants in the model)
	 *                            restrictToTypes = synonym for restrict_to_types extractValuesByUserLocale = if set
	 *                            returned array of values is filtered to include only values appropriate for the
	 *                            current user's locale forDisplay = if true, a simple list of labels ready for display
	 *                            is returned; implies the extractValuesByUserLocale option
	 *
	 * @return array List of labels
	 */
	public function getLabels(
		$pa_locale_ids = null, $pn_mode = __CA_LABEL_TYPE_ANY__, $pb_dont_cache = true, $pa_options = null
	) {
		if ( isset( $pa_options['restrictToTypes'] )
		     && ( ! isset( $pa_options['restrict_to_types'] )
		          || ! $pa_options['restrict_to_types'] )
		) {
		...
		if ( ( $pn_mode == __CA_LABEL_TYPE_ANY__ )
		     && ( caGetBundleAccessLevel( $this->tableName(), 'preferred_labels' ) == __CA_BUNDLE_ACCESS_NONE__ )
		) {
			$pn_mode = __CA_LABEL_TYPE_NONPREFERRED__;
		}
		if ( ( $pn_mode == __CA_LABEL_TYPE_ANY__ )
		     && ( caGetBundleAccessLevel( $this->tableName(), 'nonpreferred_labels' ) == __CA_BUNDLE_ACCESS_NONE__ )
		) {
		...
		if ( ( $pn_mode == __CA_LABEL_TYPE_NONPREFERRED__ )
		     && ( caGetBundleAccessLevel( $this->tableName(), 'nonpreferred_labels' ) == __CA_BUNDLE_ACCESS_NONE__ )
		) {
			return null;
		}
		...
		$vs_cache_key = caMakeCacheKeyFromOptions( array_merge( $pa_options,
			array( 'table_name' => $this->tableName(), 'id' => $vn_id, 'mode' => (int) $pn_mode ) ) );
		if ( ! $pb_dont_cache
		     && is_array( $va_tmp
				= LabelableBaseModelWithAttributes::$s_label_cache[ $this->tableName() ][ $vn_id ][ $vs_cache_key ] )
		) {
			return $va_tmp;
		}
		if ( ! ( $t_label = Datamodel::getInstanceByTableName( $this->getLabelTableName(), true ) ) ) {
		...
		if ( isset( $pa_options['restrict_to_types'] ) && $pa_options['restrict_to_types']
		     && is_array( $pa_options['restrict_to_types'] )
		     && $vs_list_code
		) {
			$t_list      = new ca_lists();
			...
			foreach ( $pa_options['restrict_to_types'] as $vs_type ) {
				...
				if ( $vn_restrict_to_type_id ) {
					$va_children = $t_list_item->getHierarchyChildren( $vn_restrict_to_type_id,
						array( 'idsOnly' => true ) );
		...
		$t_label->clear();
		while ( $qr_res->nextRow() ) {
			$va_labels[ $vn_id ][ $qr_res->get( 'locale_id' ) ][] = array_merge( $qr_res->getRow(),
				array( 'form_element' => $t_label->htmlFormElement( $this->getLabelDisplayField(), null ) ) );
		}
		...
		if ( is_array( LabelableBaseModelWithAttributes::$s_label_cache[ $this->tableName() ] )
		     && ( sizeof( LabelableBaseModelWithAttributes::$s_label_cache[ $this->tableName() ] )
		          > LabelableBaseModelWithAttributes::$s_label_cache_size )
		) {
			array_splice( LabelableBaseModelWithAttributes::$s_label_cache[ $this->tableName() ], 0,
				ceil( LabelableBaseModelWithAttributes::$s_label_cache_size / 2 ) );
		}
	 ...
	 *
	 * @return boolean True on success, false on error. Success occurs when a default label is successfully added or
	 *                 when a default label is not required. false is only returned when an actionable error state
	 *                 occurs (eg. a blank label is not allowed, or the addition of the default label fails for some
	 *                 reason)
	 */
	public function addDefaultLabel( $pn_locale_id = null ) {
		...
		if ( ! $this->getPreferredLabelCount() ) {
			...
			if ( ! (bool) $this->getAppConfig()->get( 'require_preferred_label_for_'
			                                          . $this->tableName() )
				...
				$this->postError( 1130, _t( 'Label must not be blank' ),
					'LabelableBaseModelWithAttributes->addDefaultLabel()', $this->tableName() . '.preferred_labels' );
	...
	/**
	 * Static equivalent to @param string $ps_table_name the base table name
	 *
	 * @return string|bool
	 * @see getLabelTableName()
	 */
	 ...
	 */
	public function getPreferredLabelHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pa_bundle_settings = null, $pa_options = null
	) {
		global $g_ui_locale;
		...
		$o_view->setVar( 'add_label',
			isset( $pa_bundle_settings['add_label'][ $g_ui_locale ] ) ? $pa_bundle_settings['add_label'][ $g_ui_locale ]
				: null );
		...
		if ( $this->getPrimaryKey() ) {
			...
			if ( $this->numErrors() ) {
				foreach ( $_REQUEST as $vs_key => $vs_value ) {
					if ( ! preg_match( '/' . $ps_placement_code . $ps_form_name . '_Pref' . 'locale_id_new_([\d]+)/',
						$vs_key, $va_matches )
					) {
						continue;
					}
					$vn_c = intval( $va_matches[1] );
					if ( $vn_new_label_locale_id = $po_request->getParameter( $ps_placement_code . $ps_form_name
					                                                          . '_Pref' . 'locale_id_new_' . $vn_c,
						pString )
					) {
						if ( is_array( $va_label_values = $this->getLabelUIValuesFromRequest( $po_request,
							$ps_placement_code . $ps_form_name, 'new_' . $vn_c, true ) )
						) {
		...
		if ( is_array( $this->opa_failed_preferred_label_inserts )
		     && sizeof( $this->opa_failed_preferred_label_inserts )
		) {
	 ...
	 * @return string Rendered HTML bundle
	 */
	public function getNonPreferredLabelHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pa_bundle_settings = null, $pa_options = null
	) {
		global $g_ui_locale;
		...
		if ( $this->getPrimaryKey() ) {
			...
			if ( $this->numErrors() ) {
				foreach ( $_REQUEST as $vs_key => $vs_value ) {
					if ( ! preg_match( '/' . $ps_placement_code . $ps_form_name . '_NPref' . 'locale_id_new_([\d]+)/',
						$vs_key, $va_matches )
					) {
						continue;
					}
		...
		if ( is_array( $this->opa_failed_nonpreferred_label_inserts )
		     && sizeof( $this->opa_failed_nonpreferred_label_inserts )
		) {
			$va_new_labels_to_force_due_to_error = $this->opa_failed_preferred_label_inserts;
		}
	...
	public function getDisplayLabels( $pb_dont_cache = false, $pa_options = null ) {
		return caExtractValuesByUserLocale( $this->getPreferredLabels( null, $pb_dont_cache, $pa_options ), null, null,
			array() );
	}
	# ------------------------------------------------------------------
	 ...
	 *                                returnAllLocales = if set to true, an array indexed by row_id and then locale_id
	 *                                will be returned
	 *
	 * @return array An array of preferred labels in the current locale indexed by row_id, unless returnAllLocales is
	 *               set, in which case the array includes preferred labels in all available locales and is indexed by
	 *               row_id and locale_id
	 */
	public function getPreferredDisplayLabelsForIDs( $pa_ids, $pa_options = null ) {
		...
		if ( ! isset( $pa_options['noCache'] ) && ! $pa_options['noCache']
		...
		if ( ( $t_label_instance = $this->getLabelTableInstance() )
		     && ( $t_label_instance->hasField( 'is_preferred' ) )
		) {
		...
		while ( $qr_res->nextRow() ) {
			if ( $vb_return_all_locales ) {
				$va_labels[ (int) $qr_res->get( $vs_pk ) ][ (int) $qr_res->get( 'locale_id' ) ][]
					= $qr_res->get( $vs_display_field );
			} else {
				$va_labels[ (int) $qr_res->get( $vs_pk ) ][ (int) $qr_res->get( 'locale_id' ) ]
					= $qr_res->get( $vs_display_field );
			}
		...
		if ( sizeof( LabelableBaseModelWithAttributes::$s_labels_by_id_cache )
		     > LabelableBaseModelWithAttributes::$s_labels_by_id_cache_size
		) {
			array_splice( LabelableBaseModelWithAttributes::$s_labels_by_id_cache, 0,
				ceil( LabelableBaseModelWithAttributes::$s_labels_by_id_cache_size / 2 ) );
		}
		...
		return LabelableBaseModelWithAttributes::$s_labels_by_id_cache[ $vs_cache_key ]
			= caExtractValuesByUserLocale( $va_sorted_labels );
	}
	# ------------------------------------------------------------------
	 ...
	 *                                will be returned
	 *
	 * @return array An array of nonpreferred labels in the current locale indexed by row_id, unless returnAllLocales
	 *               is set, in which case the array includes preferred labels in all available locales and is indexed
	 *               by row_id and locale_id
	 */
	 ...
	 *
	 * @param int   $pn_id      Optional row_id to use as root of returned hierarchy. If omitted hierarchy root is
	 *                          used.
	 * @param array $pa_options Array of options; support options are:
	 *                          preferred_only = if set to false then all label, preferred and nonpreferred are
	 *                          returned; if set to true only preferred labels are returned; default is true - only
	 *                          return preferred labels
	 *
	 * @return array Array of row data, key'ed on row primary key and locale_id. Values are arrays of field values from
	 *               rows in the hierarchy.
	 */
	public function getHierarchyWithLabels( $pn_id = null, $pa_options = null ) {
		...
		if ( ! ( $qr_res = $this->getHierarchy( $pn_id,
			array( 'additionalTableToJoin' => $this->getLabelTableName() ) ) )
		) {
			return null;
		}
		...
		$va_hier = array();
		while ( $qr_res->nextRow() ) {
			if ( isset( $pa_options['preferred_only'] ) && (bool) $pa_options['preferred_only']
			     && ( ! $qr_res->get( 'is_preferred' ) )
			) {
				continue;
			}
	 ...
	 *
	 * @param array $pa_options Options include:
	 *                          row_id = Get group list for a specific row rather than the currently loaded one.
	 *                          [Default is null]
	 *
	 ...
	 *
	 * @return bool True if at least one group can access the currently loaded row, false if no groups have access;
	 *              returns null if no row is currently loaded.
	 */
		...
		if ( is_array( $va_groups = $this->getUserGroups() ) ) {
			foreach ( $pm_group_id as $pn_group_id ) {
				if ( isset( $va_groups[ $pn_group_id ] ) && ( is_array( $va_groups[ $pn_group_id ] ) ) ) {
					...
					if ( ( $va_groups[ $pn_group_id ]['sdatetime'] > 0 )
					     && ( $va_groups[ $pn_group_id ]['edatetime'] > 0 )
					) {
						if ( ( $va_groups[ $pn_group_id ]['sdatetime'] > time() )
						     || ( $va_groups[ $pn_group_id ]['edatetime'] <= time() )
						) {
							return false;
						}
					}
	 ...
	 *
	 */
	public function getUserGroupHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pn_table_num, $pn_item_id, $pn_user_id = null,
		$pa_options = null
	) {
		$vs_view_path = ( isset( $pa_options['viewPath'] ) && $pa_options['viewPath'] ) ? $pa_options['viewPath']
			: $po_request->getViewsDirectoryPath();
	 ...
	 *
	 * @param array $pa_options Options include:
	 *                          row_id = Get user list for a specific row rather than the currently loaded one.
	 *                          [Default is null]
	 *
	 * @return array List of groups associated with the currently loaded row
	 */
	public function getUsers( $pa_options = null ) {
		...
		while ( $qr_res->nextRow() ) {
			$va_row = array();
			foreach (
				array( 'user_id', 'user_name', 'fname', 'lname', 'email', 'sdatetime', 'edatetime', 'access' ) as $vs_f
			) {
				$va_row[ $vs_f ] = $qr_res->get( $vs_f );
			}
			if ( $vb_supports_date_restrictions ) {
		...
		if ( is_array( $va_users = $this->getUsers() ) ) {
			foreach ( $pm_user_id as $pn_user_id ) {
				if ( isset( $va_users[ $pn_user_id ] ) && ( is_array( $va_users[ $pn_user_id ] ) ) ) {
					// is effective date set?
					if ( ( $va_users[ $pn_user_id ]['sdatetime'] > 0 )
					     && ( $va_users[ $pn_user_id ]['edatetime'] > 0 )
					) {
						if ( ( $va_users[ $pn_user_id ]['sdatetime'] > time() )
						     || ( $va_users[ $pn_user_id ]['edatetime'] <= time() )
						) {
							return false;
						}
					}
	 ...
	 *
	 */
	public function getUserHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pn_table_num, $pn_item_id, $pn_user_id = null,
		$pa_options = null
	) {
		$vs_view_path = ( isset( $pa_options['viewPath'] ) && $pa_options['viewPath'] ) ? $pa_options['viewPath']
	 ...
	 *
	 * @return bool True if at least one role can access the currently loaded row, false if no roles have access;
	 *              returns null if no row is currently loaded.
	 */
		...
		if ( is_array( $va_roles = $this->getUserRoles() ) ) {
			foreach ( $pm_role_id as $pn_role_id ) {
				if ( isset( $va_roles[ $pn_role_id ] ) && ( is_array( $va_roles[ $pn_role_id ] ) ) ) {
					// is effective date set?
					if ( ( $va_roles[ $pn_role_id ]['sdatetime'] > 0 )
					     && ( $va_roles[ $pn_role_id ]['edatetime'] > 0 )
					) {
						if ( ( $va_roles[ $pn_role_id ]['sdatetime'] > time() )
						     || ( $va_roles[ $pn_role_id ]['edatetime'] <= time() )
						) {
							return false;
						}
					}
	 ...
	 *
	 */
	public function getUserRoleHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pn_table_num, $pn_item_id, $pn_user_id = null,
		$pa_options = null
	) {
		$vs_view_path = ( isset( $pa_options['viewPath'] ) && $pa_options['viewPath'] ) ? $pa_options['viewPath']
