...
class Datamodel
{
    ...
    public static function load($pb_dont_cache = false)
    {
        ...
        if (!$pb_dont_cache && ExternalCache::contains('ca_datamodel_graph')) {
            if (is_array($va_graph = ExternalCache::fetch('ca_datamodel_graph')) && isset($va_graph['NODES']) && sizeof(
                    $va_graph['NODES']
                )) {
     ...
     */
    public static function getInstance($pm_table_name_or_num, $pb_use_cache = false, $pn_id = null)
    {
     ...
     * @return null|BaseModel
     */
    public static function getInstanceByTableName($ps_table, $pb_use_cache = false)
    {
        if (!$ps_table) {
            return null;
        }
     ...
     * @return null|BaseModel
     */
    public static function getInstanceByTableNum($pn_tablenum, $pb_use_cache = false)
    {
        ...
        if ($vs_class_name = Datamodel::getTableName($pn_tablenum)) {
            ...
            if ($pb_use_cache) {
                ...
                Datamodel::$s_instance_cache[$pn_tablenum] = Datamodel::$s_instance_cache[$vs_class_name] = $t_instance;
            }
            return $t_instance;
        } else {
            return null;
    ...
    {
        if ($t_instance = is_numeric($pn_tablenum) ? Datamodel::getInstanceByTableNum(
            $pn_tablenum,
            true
    ...
    {
        if (is_array($path = Datamodel::getPath($ps_left_table, $ps_right_table)) && (sizeof(
                    $path
                ) == 3) && ($path = array_keys($path))) {
            return $path[1];
        }
        return null;
    }
