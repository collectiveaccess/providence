...
class Datamodel {
	...
	static public function load( $pb_dont_cache = false ) {
		...
		if ( ! $pb_dont_cache && ExternalCache::contains( 'ca_datamodel_graph' ) ) {
			if ( is_array( $va_graph = ExternalCache::fetch( 'ca_datamodel_graph' ) ) && isset( $va_graph['NODES'] )
			     && sizeof( $va_graph['NODES'] )
			) {
			...
			foreach ( $va_relationships as $vs_relationship ) {
				...
				switch ( $vn_num_keys ) {
					case 2:
						...
						$vn_weight = ( isset( $va_tmp[1] ) && ( intval( $va_tmp[1] ) > 0 ) ) ? intval( $va_tmp[1] )
							: 10;
						break;
					default:
	 ...
	 *
	 * @return mixed If $ps_key is set the specified value will be returned, which may be a string, number or array. If
	 *               $ps_key is omitted the entire information array is returned.
	 */
	 ...
	 *
	 * By default a cached instance is returned. The initial state (Eg. is a row loaded, field values) for the returned
	 * cached instance is undefined and may reflect previous use and may be referenced by previous callers. You should
	 * be sure to do any initialization required before use, or don't use the cache. When caching is bypassed you are
	 * guaranteed a newly created, freshly initialized instance.
	 *
	...
	/**
	 * Returns an object representing table; object can be used to manipulate records or get information on various
	 * table attributes.
	 *
	 * @param string $ps_table     Table name
	 * @param bool   $pb_use_cache Use a cached instance. Default is false.
	...
	static public function getInstanceByTableName( $ps_table, $pb_use_cache = false ) {
		...
		if ( $pb_use_cache && isset( Datamodel::$s_instance_cache[ $ps_table ] ) ) {
			return Datamodel::$s_instance_cache[ $ps_table ];
		...
		if ( Datamodel::$opo_graph->hasNode( $ps_table ) ) {
			...
			if ( $pb_use_cache ) {
				Datamodel::$s_instance_cache[ $t_instance->tableNum() ]
					= Datamodel::$s_instance_cache[ $ps_table ] = $t_instance;
			}
	...
	static public function getInstanceByTableNum( $pn_tablenum, $pb_use_cache = false ) {
		if ( $pb_use_cache && isset( Datamodel::$s_instance_cache[ $pn_tablenum ] ) ) {
			return Datamodel::$s_instance_cache[ $pn_tablenum ];
		}        // keep instances in statics for speed
		if ( $vs_class_name = Datamodel::getTableName( $pn_tablenum ) ) {
			...
			if ( $pb_use_cache ) {
				...
				Datamodel::$s_instance_cache[ $pn_tablenum ]
					= Datamodel::$s_instance_cache[ $vs_class_name ] = $t_instance;
			}
	...
	static public function primaryKey( $pn_tablenum, $pb_include_tablename = false ) {
		if ( $t_instance = is_numeric( $pn_tablenum ) ? Datamodel::getInstanceByTableNum( $pn_tablenum, true )
			: Datamodel::getInstanceByTableName( $pn_tablenum, true )
		) {
			return $pb_include_tablename ? $t_instance->tableName() . '.' . $t_instance->primaryKey()
	...
	static public function getManyToOneRelations( $ps_table, $ps_field = null ) {
		...
		if ( $o_table = Datamodel::getInstanceByTableName( $ps_table, true ) ) {
			...
			foreach ( $va_related_tables as $vs_related_table ) {
				$va_relationships = Datamodel::$opo_graph->getAttribute( "relationships", $ps_table,
					$vs_related_table );
						...
						if ( $va_fields[0] != $vs_table_pk ) {
							if ( $ps_field ) {
								if ( $va_fields[0] == $ps_field ) {
									...
									MemoryCache::save( "{$ps_table}/{$ps_field}", $va_many_to_one_relations,
										'DatamodelManyToOneRelations' );
						...
						if ( $va_fields[0] == $vs_table_pk ) {
							if ( $ps_many_table ) {
								if ( $ps_many_table == $vs_related_table ) {
									...
									MemoryCache::save( "{$ps_table}/{$ps_many_table}", $va_one_to_many_relations,
										'DatamodelOneToManyRelations' );
						...
						foreach ( $va_many_to_one_relations as $va_right_relation ) {
							if ( $ps_table != $va_right_relation["one_table"] ) {
								if ( $ps_table2 == $va_right_relation["one_table"] ) {
									MemoryCache::save( "{$ps_table}/{$ps_table2}", $va_left_relation["many_table"],
										'DatamodelManyToManyRelations' );
	...
	static public function getPath( $ps_left_table, $ps_right_table ) {
		...
		if ( $ps_left_table == $ps_right_table ) {
			...
			$va_path = array(
				$ps_left_table => Datamodel::getTableNum( $ps_left_table ),
				$rel_table     => Datamodel::getTableNum( $rel_table )
			);
		} else {
	...
	static public function getLinkingTableName( $ps_left_table, $ps_right_table ) {
		if ( is_array( $path = Datamodel::getPath( $ps_left_table, $ps_right_table ) ) && ( sizeof( $path ) == 3 )
