...
abstract class BaseWidget implements IWidget {
	...
	public function __construct( $ps_widget_path, $pa_settings ) {
		...
		if ( ( AppController::instanceExists() ) && ( $o_app = AppController::getInstance() )
		     && ( $o_req
				= $o_app->getRequest() )
		) {
			$this->request = $o_req;
	 ...
	 * implementation returns a status without errors but with the 'available' flag set to false (ie. widget isn't
	 * functional)
	 *
	 * @return array associative array indicating availability of widget, and any initialization errors and warnings.
	 *               Also includes text description of widget for display.
	 */
	...
	/**
	 * Base class for widget renderer. Will set basic widget view variables such as DOM 'id', 'widget_id' and the
	 * 'settings' array as well as filling in default values for any unspecified widget settings
	 *
	 * @param $ps_widget_id string the unique id (actually an md5 hash) for the widget
	 * @param $pa_settings  array An array of widget settings in a key-value format; this is passed by reference to
	 *                      this base class so it can modify the array for the caller with appropriate default values
	 *
	 * @return void
	 */
		...
		if ( is_array( $va_settings = $this->getAvailableSettings( true ) ) ) {
			foreach ( $va_settings as $vs_setting_name => $va_setting_info ) {
				...
				if ( isset( $va_setting_info['scope'] ) && ( $va_setting_info['scope'] == 'application' ) ) {
					$vs_widget_name = preg_replace( '!Widget$!', '',
						get_class( $this ) );    // name is class name minus trailing "Widget"
					if ( ! $o_appvar ) {
						$o_appvar = new ApplicationVars();
					}
					$pa_settings[ $vs_setting_name ] = $o_appvar->getVar( 'widget_settings_' . $vs_widget_name . '_'
					                                                      . $vs_setting_name );
				}
			}
		}
	 ...
	 * info about the setting itself (label, description type of value, how to display an entry element for the setting
	 * in a form)
	 */
	...
	public function getSetting( $ps_setting ) {
		...
		$vs_default        = isset( $va_setting_config[ $ps_setting ]['default'] )
			? $va_setting_config[ $ps_setting ]['default'] : null;
	 ...
	 *  'value' => sets the value of the HTML form element explicitly, otherwise the current value for the setting in
	...
	public function settingHTMLFormElement( $ps_widget_id, $ps_setting, $pa_options = null ) {
		...
		switch ( $va_properties['displayType'] ) {
			...
			case DT_FIELD:
				...
				if ( isset( $va_properties['takesLocale'] ) && $va_properties['takesLocale'] ) {
					...
					$va_locales      = ca_locales::getLocaleList( array(
						'sort_field'    => '',
						'sort_order'    => 'asc',
						'index_by_code' => true
					) );
				...
				foreach ( $va_locales as $vs_locale => $va_locale_info ) {
					...
					$vs_element .= caHTMLTextInput( $vs_input_name . $vs_input_name_suffix, array(
							'size'   => $va_properties["width"],
							'height' => $va_properties["height"],
							'value'  => $vs_value,
							'id'     => $vs_input_name . $vs_input_name_suffix
						) ) . "{$vs_locale_label}";

					// focus code is needed by Firefox for some reason
					$vs_element .= "<script type='text/javascript'>jQuery('#" . $vs_input_name . $vs_input_name_suffix
					               . "').click(function() { this.focus(); });</script>";
				}
				break;
			# --------------------------------------------
			case DT_CHECKBOXES:
			...
			case DT_SELECT:
				...
				$vs_element .= caHTMLSelect( $vs_input_name, $va_properties['options'], array(),
					array( 'value' => $vs_value ) );
				break;
			# --------------------------------------------
			default:
				break;
		...
		$vs_return = "\n" . '<div class="formLabel" id="_widget_setting_' . $ps_setting . '_' . $ps_widget_id
		             . '"><span>' . $vs_label . '</span>';
		if ( ! $vb_element_is_part_of_label ) {
			$vs_return .= '<br />' . $vs_element;
		}
		$vs_return .= '</div>' . "\n";
		TooltipManager::add( '#_widget_setting_' . $ps_setting . '_' . $ps_widget_id,
			"<h3>" . str_replace( '^ELEMENT', 'X', $va_properties["label"] ) . "</h3>"
			. $va_properties["description"] );
