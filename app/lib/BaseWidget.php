...
abstract class BaseWidget implements IWidget
{
    ...
    static public $s_widget_settings;
            ...
            foreach ($va_settings as $vs_setting_name => $va_setting_info) {
                ...
                if (isset($va_setting_info['scope']) && ($va_setting_info['scope'] == 'application')) {
                    ...
                    $pa_settings[$vs_setting_name] = $o_appvar->getVar(
                        'widget_settings_' . $vs_widget_name . '_' . $vs_setting_name
                    );
                }
            }
        }
    ...
    public function settingHTMLFormElement($ps_widget_id, $ps_setting, $pa_options = null)
    {
            ...
            case DT_FIELD:
                ...
                if (isset($va_properties['takesLocale']) && $va_properties['takesLocale']) {
                    ...
                    $va_locales = ca_locales::getLocaleList(
                        array('sort_field' => '', 'sort_order' => 'asc', 'index_by_code' => true)
                    );
                } else {
                ...
                foreach ($va_locales as $vs_locale => $va_locale_info) {
                    ...
                    $vs_element .= caHTMLTextInput(
                            $vs_input_name . $vs_input_name_suffix,
                            array(
        ...
        $vs_return .= '</div>' . "\n";
        TooltipManager::add(
            '#_widget_setting_' . $ps_setting . '_' . $ps_widget_id,
            "<h3>" . str_replace(
                '^ELEMENT',
                'X',
                $va_properties["label"]
            ) . "</h3>" . $va_properties["description"]
        );
