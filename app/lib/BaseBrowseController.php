...
class BaseBrowseController extends BaseFindController
{
    ...
    public function __construct(&$po_request, &$po_response, $pa_view_paths = null)
    {
        ...
        if ($this->ops_tablename) {
            if ($va_items_per_page_config = $po_request->config->get(
                'items_per_page_options_for_' . $this->ops_tablename . '_browse'
            )) {
                $this->opa_items_per_page = $va_items_per_page_config;
            }
            if (($vn_items_per_page_default = (int)$po_request->config->get(
                    'items_per_page_default_for_' . $this->ops_tablename . '_browse'
                )) > 0) {
    ...
    public function getFacet($pa_options = null)
    {
        ...
        if (!($va_facet = $this->opo_browse->getFacet(
            $ps_facet_name,
            array('sort' => 'name', 'checkAccess' => $va_access_values)
        ))) {
            $this->opo_browse->removeAllCriteria();
            $this->opo_browse->execute();
            $va_facet = $this->opo_browse->getFacet(
                $ps_facet_name,
                array('sort' => 'name', 'checkAccess' => $va_access_values)
            );
            $va_facet_info = $this->opo_browse->getInfoForFacet($ps_facet_name);
        ...
        $this->view->setVar(
            'browse_last_id',
            (int)$vm_id ? (int)$vm_id : (int)$this->opo_result_context->getParameter(
                $ps_facet_name . '_browse_last_id'
            )
        );
        ...
        $this->view->setVar(
            'individual_group_display',
            isset($va_facet_info['individual_group_display']) ? (bool)$va_facet_info['individual_group_display'] : false
        );

        // this should be 'facet' but we don't want to render all old 'ajax_browse_facet_html' views (pawtucket themes) unusable
        $this->view->setVar(
            'grouped_facet',
            $this->opo_browse->getFacetWithGroups(
                $ps_facet_name,
                $va_facet_info["group_mode"],
                $vs_grouping,
                array('sort' => 'name', 'checkAccess' => $va_access_values)
            )
        );
    ...
    public function getFacetHierarchyLevel()
    {
        ...
        if (!(is_array(
                $va_sorts = $o_config->getList($this->ops_tablename . '_hierarchy_browser_sort_values')
            )) || !sizeof($va_sorts)) {
            $va_sorts = array();
        }
        ...
        foreach ($pa_ids as $pn_id) {
            ...
            switch ($va_facet_info['type']) {
                    ...
                    if ($t_element->load(array('element_code' => $va_facet_info['element_code']))) {
                        switch ($t_element->get('datatype')) {
                            case __CA_ATTRIBUTE_VALUE_LIST__:
                                ...
                                if ($qr_res) {
                                    // expand facet
                                    $va_ancestors = $t_item->getHierarchyAncestorsForIDs(
                                        array_keys($va_facet),
                                        array(
                                            'returnAs' => 'ids',
                                            'includeSelf' => true
                ...
                default:
                    ...
                    if (!$vn_id) {
                        $va_hier_ids = $this->opo_browse->getHierarchyIDsForFacet(
                            $ps_facet_name,
                        ...
                        if (is_array($va_hier_ids) && (sizeof($va_hier_ids) > 0) && $t_item) {
                            ...
                            foreach ($va_hierarchy_list as $vn_i => $va_item) {
                                ...
                                if (is_array($va_access_values) && isset($va_item['access']) && (!in_array(
                                        $va_item['access'],
                                        $va_access_values
                                    ))) {
                                    continue;
                            ...
                            if (sizeof(
                                    $va_json_data
                                ) == 2) {    // if only one hierarchy root (root +  _primaryKey in array) then don't bother showing it
                                $vn_id = $vn_last_id;
    ...
    public function getFacetHierarchyAncestorList()
    {
        ...
        switch ($va_facet_info['type']) {
            ...
            default:
                ...
                if (($vn_hierarchies_in_use <= 1) && ($t_item->getProperty(
                            'HIERARCHY_TYPE'
