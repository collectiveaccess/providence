...
class BaseBrowseController extends BaseFindController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		if ( $this->ops_tablename ) {
			if ( $va_items_per_page_config = $po_request->config->get( 'items_per_page_options_for_'
			                                                           . $this->ops_tablename . '_browse' )
			) {
				$this->opa_items_per_page = $va_items_per_page_config;
			}
			if ( ( $vn_items_per_page_default = (int) $po_request->config->get( 'items_per_page_default_for_'
			                                                                    . $this->ops_tablename . '_browse' ) )
			     > 0
			) {
			...
			if ( $vs_view_default = $po_request->config->get( 'view_default_for_' . $this->ops_tablename
			                                                  . '_browse' )
			) {
	...
	public function Index( $pa_options = null ) {
		...
		$pb_dont_render_view = ( isset( $pa_options['dontRenderView'] ) && (bool) $pa_options['dontRenderView'] ) ? true
			: false;
		...
		if ( $this->opb_type_restriction_has_changed
		     || ( $this->request->getParameter( 'reset', pString ) == 'clear' )
		) {
		...
		if (
			...
			if ( $this->request->config->get( 'redirect_to_' . $va_facet_info['table']
			                                  . '_detail_if_is_first_facet' )
			) {
				$t_table = Datamodel::getInstanceByTableName( $va_facet_info['table'], true );

				$va_newmuseum_hack_occurrence_type_ids
					= $this->request->config->getList( 'newmuseum_hack_browse_should_redirect_occurrence_types_to_object_details' );
				if ( is_array( $va_newmuseum_hack_occurrence_type_ids )
				     && sizeof( $va_newmuseum_hack_occurrence_type_ids )
					...
					if ( $t_table->load( $va_tmp1[0] ) ) {
						if ( in_array( $t_table->getTypeID(), $va_newmuseum_hack_occurrence_type_ids ) ) {
							if ( sizeof( $va_objects = $t_table->getRelatedItems( 'ca_objects' ) ) ) {
								...
								$this->response->setRedirect( caNavUrl( $this->request, 'Detail', 'Object', 'Show',
				...
				$this->response->setRedirect( caNavUrl( $this->request, 'Detail',
					ucfirst( $t_table->getProperty( 'NAME_SINGULAR' ) ), 'Show',
		...
		//
		if ( $vs_facet_group = $this->request->config->get( $this->ops_tablename . ( ( $this->opo_browse->numCriteria()
		                                                                               < 1 ) ? '_browse_facet_group'
				: '_browse_refine_facet_group' ) )
		) {
			$this->opo_browse->setFacetGroup( $vs_facet_group );
		}
		$this->opo_browse->execute( array(
		...
		if ( ! is_array( $va_facets_with_info = $this->opo_browse->getInfoForAvailableFacets() )
		     || ! sizeof( $va_facets_with_info )
		) {
		...
		foreach ( $va_facet_info as $vs_facet => $va_facet_settings ) {
			$va_single_facet_values[ $vs_facet ] = isset( $va_facet_settings['single_value'] )
				? $va_facet_settings['single_value'] : null;
		}
		...
		if ( $pa_options['output_format'] ) {
			$vo_result = $this->opo_browse->getResults( array(
				'sort'           => $vs_sort,
		...
		if ( $vo_result ) {
			if ( $vb_criteria_have_changed || $vb_sort_has_changed ) {
				...
				$vo_full_result = $this->opo_browse->getResults( array(
					'sort'           => $vs_sort,
					'sort_direction' => $vs_sort_direction
				) );
			...
			if ( $vo_result ) {
				...
				while ( ( $vn_item_count < 100 ) && $vo_result->nextHit() ) {
					...
					foreach ( $va_display_list as $vn_placement_id => $va_bundle_info ) {
						$va_result[ str_replace( ".", "-", $va_bundle_info['bundle_name'] ) ]
							= $t_display->getDisplayValue( $vo_result, $vn_placement_id,
					...
					$va_row_headers[] = ( $vn_item_count ) . " " . caEditorLink( $this->request,
							caNavIcon( __CA_NAV_ICON_EDIT__, 2 ), 'caResultsEditorEditLink', $this->ops_tablename,
							$vn_id );
		...
		switch ( $pa_options['output_format'] ) {
			# ------------------------------------
			case 'LABELS':
				$this->_genLabels( $vo_result, $this->request->getParameter( "label_form", pString ), _t( 'Browse' ),
					_t( 'Browse' ) );
				break;
			# ------------------------------------
			case 'EXPORT':
				$this->_genExport( $vo_result, $this->request->getParameter( "export_format", pString ), _t( 'Browse' ),
					_t( 'Browse' ) );
				break;
			# ------------------------------------
	...
	public function getFacet( $pa_options = null ) {
		...
		$this->view->setVar( 'only_show_group',
			$vs_show_group = $this->request->getParameter( 'show_group', pString ) );
...
//  			}
		...
		$this->view->setVar( 'browse_last_id', (int) $vm_id
			? (int) $vm_id : (int) $this->opo_result_context->getParameter( $ps_facet_name . '_browse_last_id' ) );
		...
		$this->view->setVar( 'individual_group_display',
			isset( $va_facet_info['individual_group_display'] ) ? (bool) $va_facet_info['individual_group_display']
				: false );

		// this should be 'facet' but we don't want to render all old 'ajax_browse_facet_html' views (pawtucket themes) unusable
		$this->view->setVar( 'grouped_facet',
			$this->opo_browse->getFacetWithGroups( $ps_facet_name, $va_facet_info["group_mode"], $vs_grouping,
				array( 'sort' => 'name', 'checkAccess' => $va_access_values ) ) );
		...
		if ( $va_facet_info['type'] === 'authority' ) {
			...
			$this->view->setVar( 'relationship_type_list',
				$t_rel_types->getRelationshipInfo( $va_facet_info['relationship_table'] ) );
	...
	public function getFacetHierarchyLevel() {
		...
		$va_facet = $this->opo_browse->getFacet( $ps_facet_name,
			array( 'sort' => 'name', 'checkAccess' => $va_access_values ) );
		...
		if ( ! ( is_array( $va_sorts = $o_config->getList( $this->ops_tablename . '_hierarchy_browser_sort_values' ) ) )
		     || ! sizeof( $va_sorts )
		) {
			$va_sorts = array();
		}
		...
		if ( ! in_array(
			$vs_sort_dir = strtolower( $o_config->get( $this->ops_tablename . '_hierarchy_browser_sort_direction' ) ),
			array( 'asc', 'desc' ) )
		) {
			$vs_sort_dir = 'asc';
		}
		...
		foreach ( $pa_ids as $pn_id ) {
			...
			switch ( $va_facet_info['type'] ) {
					...
					if ( $t_element->load( array( 'element_code' => $va_facet_info['element_code'] ) ) ) {
						switch ( $t_element->get( 'datatype' ) ) {
							case __CA_ATTRIBUTE_VALUE_LIST__:
								...
								if ( $qr_res ) {
									// expand facet
									$va_ancestors = $t_item->getHierarchyAncestorsForIDs( array_keys( $va_facet ),
										array( 'returnAs' => 'ids', 'includeSelf' => true ) );
									while ( $qr_res->nextHit() ) {
										...
										if ( is_array( $va_access_values )
										     && ! in_array( $vn_access, $va_access_values )
										) {
											continue;
										}
										...
										$va_item['children']         = ( isset( $va_child_counts[ $vn_item_id ] )
										                                 && $va_child_counts[ $vn_item_id ] )
											? $va_child_counts[ $vn_item_id ] : 0;
				...
				default:
					...
					if ( ! $vn_id ) {
						$va_hier_ids = $this->opo_browse->getHierarchyIDsForFacet( $ps_facet_name,
						...
						if ( is_array( $va_hier_ids ) && ( sizeof( $va_hier_ids ) > 0 ) && $t_item ) {
							...
							foreach ( $va_hierarchy_list as $vn_i => $va_item ) {
								...
								if ( ! isset( $va_facet[ $va_item['item_id'] ] )
								     && ( $vn_root == $va_item['item_id'] )
								) {
									continue;
								}
								if ( is_array( $va_access_values ) && isset( $va_item['access'] )
								     && ( ! in_array( $va_item['access'], $va_access_values ) )
								) {
									continue;
								}
								...
								if ( ! is_null( $vn_max_items_per_page )
								     && ( $vn_c >= ( $vn_max_items_per_page + $vn_start ) )
								) {
									break;
								}
							}
							if ( sizeof( $va_json_data )
							     == 2
	...
	public function getFacetHierarchyAncestorList() {
		...
		switch ( $va_facet_info['type'] ) {
			...
			default:
				...
				if ( $t_item->getPrimaryKey() ) {
					$va_ancestors = $t_item->getHierarchyAncestors( null,
						array( 'includeSelf' => true, 'idsOnly' => true ) );
				...
				if ( ( $vn_hierarchies_in_use <= 1 )
				     && ( $t_item->getProperty( 'HIERARCHY_TYPE' ) !== __CA_HIER_TYPE_ADHOC_MONO__ )
				) {
					array_shift( $va_ancestors );
				}
				break;
	...
	public function addCriteria() {
		$ps_facet_name = $this->request->getParameter( 'facet', pString );
		$this->opo_browse->addCriteria( $ps_facet_name,
	...
	public function getCriteriaForDisplay() {
		...
		foreach ( $va_criteria as $vs_facet => $va_vals ) {
			$va_buf[] = caUcFirstUTF8Safe( $va_criteria_info[ $vs_facet ]['label_singular'] ) . ': ' . join( ", ",
					$va_vals );
		}
