...
class MediaContentLocationIndexer {
	...
	static public function index( $pm_table, $pn_row_id, $ps_content, $pn_page, $pn_x1, $pn_y1, $pn_x2, $pn_y2 ) {
		...
		MediaContentLocationIndexer::$s_data[ (int) $pn_table ][ (int) $pn_row_id ][ (string) trim( $ps_content ) ][]
			= array(
			'p'  => $pn_page,
			'x1' => $pn_x1,
			'y1' => $pn_y1,
		...
		foreach ( MediaContentLocationIndexer::$s_data as $vn_table_num => $va_rows ) {
			foreach ( $va_rows as $vn_row_id => $va_content ) {
				foreach ( $va_content as $vs_content => $va_locs ) {
					MediaContentLocationIndexer::$s_index_insert->execute( (int) $vn_table_num, (int) $vn_row_id,
						(string) caEncodeUTF8Deep( trim( $vs_content ) ), serialize( $va_locs ) );
				}
			}
		}
	...
	static public function _init() {
		if ( ! MediaContentLocationIndexer::$s_db ) {
			...
			MediaContentLocationIndexer::$s_index_insert
			                                   = MediaContentLocationIndexer::$s_db->prepare( "INSERT INTO ca_media_content_locations (table_num, row_id, content, loc) VALUES (?, ?, ?, ?)" );
			MediaContentLocationIndexer::$s_index_delete
			                                   = MediaContentLocationIndexer::$s_db->prepare( "DELETE FROM ca_media_content_locations WHERE table_num = ? AND row_id = ?" );

			MediaContentLocationIndexer::$s_index_find
				= MediaContentLocationIndexer::$s_db->prepare( "SELECT * FROM ca_media_content_locations WHERE table_num = ? AND row_id = ? AND content LIKE ?" );
			MediaContentLocationIndexer::$s_autocomplete
				= MediaContentLocationIndexer::$s_db->prepare( "SELECT content FROM ca_media_content_locations WHERE table_num = ? AND row_id = ? AND content LIKE ?" );
			MediaContentLocationIndexer::$s_index_check
				= MediaContentLocationIndexer::$s_db->prepare( "SELECT row_id FROM ca_media_content_locations WHERE table_num = ? AND row_id = ? LIMIT 1" );
		}
	}
	# ------------------------------------------------
	...
	static function searchWithinMedia( $ps_query, $pm_table, $pn_row_id, $ps_field ) {
		...
		if ( ! ( $t_instance = is_numeric( $pm_table ) ? Datamodel::getInstanceByTableNum( $pm_table )
			: Datamodel::getInstanceByTableName( $pm_table ) )
		) {
					...
					if ( $vn_word_num > 0 ) {
						...
						foreach ( $va_hit_acc[ $va_hit['p'] ] as $vn_i => $va_existing_loc ) {
							...
							if (
								...
								if (
									( abs( ( $y2_percent - ( $va_existing_loc['y2'] + ( $va_existing_loc['y2']
									                                                    - $va_existing_loc['y1'] ) ) ) )
									  < 0.003 )
									&& ( $va_existing_loc['x1'] > $x1_percent )
								) {
									if ( isset( $va_hit_acc[ $va_hit['p'] ][ $vn_i ]['isNewlined'] )
									     && $va_hit_acc[ $va_hit['p'] ][ $vn_i ]['isNewlined']
									) {
										continue;
									}
									...
									array_splice( $va_hit_acc[ $va_hit['p'] ], $vn_i, 0, array(
										array(
											'word'         => "newline $vs_word",
											'x1'           => $x1_percent,
											'y1'           => $y1_percent,
											'x2'           => $x2_percent,
											'y2'           => $y2_percent,
											'c'            => $vn_word_num + 1,
											'previousLine' => &$va_hit_acc[ $va_hit['p'] ][ $vn_i ]
										)
									) );
									continue;
								} else {
		...
		foreach ( $va_hit_acc as $vn_p => $va_hits_per_page ) {
			foreach ( $va_hits_per_page as $vn_i => $va_hit ) {
				...
				$va_results['locations'][ $vn_p ][] = array(
					'c'    => $va_hit['c'],
					'word' => $va_hit['word'],
					'x1'   => $x1r,
					'y1'   => $y1r,
