...
{
    # ------------------------------------------------
    static public $s_table_num_cache = array();
    static public $s_db;
    static public $s_index_insert;
    static public $s_index_delete;
    static public $s_index_find;
    static public $s_index_check;
    static public $s_autocomplete;
    static public $s_data = array();
    # ------------------------------------------------
        ...
        foreach (MediaContentLocationIndexer::$s_data as $vn_table_num => $va_rows) {
            foreach ($va_rows as $vn_row_id => $va_content) {
                foreach ($va_content as $vs_content => $va_locs) {
                    MediaContentLocationIndexer::$s_index_insert->execute(
                        (int)$vn_table_num,
                        (int)$vn_row_id,
                        (string)caEncodeUTF8Deep(trim($vs_content)),
                        serialize($va_locs)
                    );
                }
            }
    ...
    public static function _init()
    {
        if (!MediaContentLocationIndexer::$s_db) {
            ...
            MediaContentLocationIndexer::$s_index_insert = MediaContentLocationIndexer::$s_db->prepare(
                "INSERT INTO ca_media_content_locations (table_num, row_id, content, loc) VALUES (?, ?, ?, ?)"
            );
            MediaContentLocationIndexer::$s_index_delete = MediaContentLocationIndexer::$s_db->prepare(
                "DELETE FROM ca_media_content_locations WHERE table_num = ? AND row_id = ?"
            );

            MediaContentLocationIndexer::$s_index_find = MediaContentLocationIndexer::$s_db->prepare(
                "SELECT * FROM ca_media_content_locations WHERE table_num = ? AND row_id = ? AND content LIKE ?"
            );
            MediaContentLocationIndexer::$s_autocomplete = MediaContentLocationIndexer::$s_db->prepare(
                "SELECT content FROM ca_media_content_locations WHERE table_num = ? AND row_id = ? AND content LIKE ?"
            );
            MediaContentLocationIndexer::$s_index_check = MediaContentLocationIndexer::$s_db->prepare(
    ...
    public static function searchWithinMedia($ps_query, $pm_table, $pn_row_id, $ps_field)
    {
        ...
        if (!($t_instance = is_numeric($pm_table) ? Datamodel::getInstanceByTableNum(
            $pm_table
                    ...
                    if ($vn_word_num > 0) {
                        ...
                        foreach ($va_hit_acc[$va_hit['p']] as $vn_i => $va_existing_loc) {
                            ...
                            if (
                                ...
                                if (
                                    (abs(
                                            ($y2_percent - ($va_existing_loc['y2'] + ($va_existing_loc['y2'] - $va_existing_loc['y1'])))
                                        ) < 0.003)
                                    &&
