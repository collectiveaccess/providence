...
class BaseRefineableSearchController extends BaseFindController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		if ( $this->ops_tablename ) {
			if ( $va_items_per_page_config = $po_request->config->getList( 'items_per_page_options_for_'
			                                                               . $this->ops_tablename . '_search' )
			) {
	...
	public function getFacetHierarchyLevel() {
		...
		foreach ( $pa_ids as $pn_id ) {
			...
			$va_json_data['_primaryKey']
			                            = $t_model->primaryKey();    // pass the name of the primary key so the hierbrowser knows where to look for item_id's
	...
	/**
	 * Callbacks:
	 *        hookAfterClearCriteria() is called after clearing criteria. The first parameter is the BrowseEngine
