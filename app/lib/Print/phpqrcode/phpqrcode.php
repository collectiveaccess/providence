...
class qrstr {
	public static function set( &$srctab, $x, $y, $repl, $replLen = false ) {
		$srctab[ $y ] = substr_replace( $srctab[ $y ], ( $replLen !== false ) ? substr( $repl, 0, $replLen ) : $repl,
			$x, ( $replLen !== false ) ? $replLen : strlen( $repl ) );
	}
}
...
class QRtools {
	...
	public static function timeBenchmark() {
		...
		foreach ( $GLOBALS['qr_time_bench'] as $markerId => $thisTime ) {
			if ( $p > 0 ) {
				echo '<tr><th style="text-align:right">till ' . $markerId . ': </th><td>' . number_format( $thisTime
				                                                                                           - $lastTime,
						6 ) . 's</td></tr>';
			} else {
				$startTime = $thisTime;
			...
			for ( $j = $i; $j > 0; $j -- ) {
				if ( $rs->genpoly[ $j ] != 0 ) {
					$rs->genpoly[ $j ] = $rs->genpoly[ $j - 1 ]
					                     ^ $rs->alpha_to[ $rs->modnn( $rs->index_of[ $rs->genpoly[ $j ] ] + $root ) ];
				} else {
					$rs->genpoly[ $j ] = $rs->genpoly[ $j - 1 ];
				}
			}
...
class QRmask {
		...
		for ( $i = 0; $i < $length; $i ++ ) {
			...
			if ( $i & 1 ) {
				if ( ( $i >= 3 ) && ( $i < ( $length - 2 ) ) && ( $this->runLength[ $i ] % 3 == 0 ) ) {
					...
					if ( ( $this->runLength[ $i - 2 ] == $fact ) && ( $this->runLength[ $i - 1 ] == $fact )
	...
	public function evaluateSymbol( $width, $frame ) {
		...
		for ( $y = 0; $y < $width; $y ++ ) {
			...
			for ( $x = 0; $x < $width; $x ++ ) {
				if ( ( $x > 0 ) && ( $y > 0 ) ) {
					$b22 = ord( $frameY[ $x ] ) & ord( $frameY[ $x - 1 ] ) & ord( $frameYM[ $x ] ) & ord( $frameYM[ $x
					                                                                                                - 1 ] );
					$w22 = ord( $frameY[ $x ] ) | ord( $frameY[ $x - 1 ] ) | ord( $frameYM[ $x ] ) | ord( $frameYM[ $x
					                                                                                                - 1 ] );
