	...
	class QRcode {
		...
		protected function encodeMask( $mask ) {
			...
			if ( $mask < 0 ) {
				if ( QR_FIND_BEST_MASK ) {
					...
					$masked = $this->makeMask( $this->width, $this->frame, ( intval( QR_DEFAULT_MASK ) % 8 ),
						$this->level );
				}
			} else {
		...
		protected function init( $spec ) {
			...
			for ( $i = 0; $i < $endfor; ++ $i ) {
				...
				$this->ecccode                            = array_merge( array_slice( $this->ecccode, 0, $eccPos ),
					$ecc );
				$dataPos                                  += $dl;
				$eccPos                                   += $el;
				$blockNo ++;
		...
		protected function toUpper() {
			...
			while ( $p < $stringLen ) {
				...
				if ( $mode == QR_MODE_KJ ) {
					...
					if ( ( ord( $this->dataStr[ $p ] ) >= ord( 'a' ) ) AND ( ord( $this->dataStr[ $p ] )
					                                                         <= ord( 'z' ) )
					) {
		...
		protected function qrstrset( $srctab, $x, $y, $repl, $replLen = false ) {
			$srctab[ $y ] = substr_replace( $srctab[ $y ],
				( $replLen !== false ) ? substr( $repl, 0, $replLen ) : $repl, $x,
		...
		protected function init_rs_char( $symsize, $gfpoly, $fcr, $prim, $nroots, $pad ) {
			...
			for ( $i = 0, $root = $fcr * $prim; $i < $nroots; $i ++, $root += $prim ) {
				...
				$rs['genpoly'][0] = $rs['alpha_to'][ $this->modnn( $rs,
		...
		protected function encode_rs_char( $rs, $data, $parity ) {
			...
			$ALPHA_TO
			        =& $rs['alpha_to']; // the address of an array of NN elements to convert Galois field elements in index (log) form to polynomial form
			$INDEX_OF
			        =& $rs['index_of']; // the address of an array of NN elements to convert Galois field elements in polynomial form to index (log) form
			$GENPOLY
			        =& $rs['genpoly']; // an array of NROOTS+1 elements containing the generator polynomial in index form
			$NROOTS
			        =& $rs['nroots']; // the number of roots in the RS code generator polynomial, which is the same as the number of parity symbols in a block
