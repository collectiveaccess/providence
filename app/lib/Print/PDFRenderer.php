...
class PDFRenderer
{
     ...
     *
     */
    static public $s_plugin_codes;

    /**
     *
     */
    static public $WLPDFRenderer_plugin_cache;
    ...
    public function setPage(
        ...
        $ps_margin_top = "0mm",
        $ps_margin_right = "0mm",
        $ps_margin_bottom = "0mm",
        $ps_margin_left = "0mm"
    ) {
        if (!$this->renderer) {
            return null;
        }
        return $this->renderer->setPage(
            $ps_size,
            $ps_orientation,
            $ps_margin_top,
            $ps_margin_right,
    ...
    public function render($ps_content, $pa_options = null)
    {
        ...
        if (is_array($append = caGetOption('append', $pa_options, null)) && (sizeof(
                    $append
                ) > 0) && caMediaPluginGhostscriptInstalled()) {
            if ($content = $this->renderer->render($ps_content, array_merge($pa_options, ['stream' => false]))) {
                ...
                caExec(
                    "{$ghostscript_path} -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -dPDFSETTINGS=/prepress -sOutputFile={$outfile} {$basefile} " . join(
                        " ",
                        array_map(
                            function ($v) {
            ...
            if (caGetOption('stream', $pa_options, false)) {
                ...
                header(
                    "Content-Disposition: attachment; filename=" . caGetOption('filename', $pa_options, 'output.pdf')
                );
                print $content;
            }
            return $content;
    ...
    public static function getPageSize($ps_size, $ps_units = 'mm', $ps_orientation = 'portrait')
    {
        ...
        return ($ps_orientation == 'portrait') ? array(
            'width' => $vn_page_width,
            'height' => $vn_page_height
