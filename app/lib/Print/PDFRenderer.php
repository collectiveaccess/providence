...
class PDFRenderer {
	...
	public function setPage(
		$ps_size, $ps_orientation, $ps_margin_top = "0mm", $ps_margin_right = "0mm", $ps_margin_bottom = "0mm",
		$ps_margin_left = "0mm"
	) {
		if ( ! $this->renderer ) {
			return null;
		...
		return $this->renderer->setPage( $ps_size, $ps_orientation, $ps_margin_top, $ps_margin_right, $ps_margin_bottom,
			$ps_margin_left );
	}
	# --------------------------------------------------------------------------------
	...
	public function render( $ps_content, $pa_options = null ) {
		...
		if ( is_array( $append = caGetOption( 'append', $pa_options, null ) ) && ( sizeof( $append ) > 0 )
		     && caMediaPluginGhostscriptInstalled()
		) {

			if ( $content = $this->renderer->render( $ps_content,
				array_merge( $pa_options, [ 'stream' => false ] ) )
			) {
				...
				caExec( "{$ghostscript_path} -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -dPDFSETTINGS=/prepress -sOutputFile={$outfile} {$basefile} "
			...
			if ( caGetOption( 'stream', $pa_options, false ) ) {
				...
				header( "Content-Disposition: attachment; filename=" . caGetOption( 'filename', $pa_options,
						'output.pdf' ) );
				print $content;
			}
	 ...
	 *                             stream = Send PDF output directly to browser [default=false]
	 *                             filename = If streaming, set filename of PDF [default=output.pdf]
	 *                             append = List of absolute PDF file paths to append to end of output. [Default is
	 *                             null]
	 *
	 * @return string PDF content
	 */
	 ...
	 *
	 * @param string $ps_plugin_code Unique code for plugin. The code is the same as the plugin's filename minus the
	 *                               .php extension.
	 *
	...
	static public function getPageSize( $ps_size, $ps_units = 'mm', $ps_orientation = 'portrait' ) {
		...
		return ( $ps_orientation == 'portrait' ) ? array( 'width' => $vn_page_width, 'height' => $vn_page_height )
			: array( 'width' => $vn_page_height, 'height' => $vn_page_width );
	}
	# --------------------------------------------------------------------------------
