...
trait SetUniqueIdnoTrait {
	...
	public function addLabel(
		...
		if ( $vn_rc = parent::addLabel( $pa_label_values, $pn_locale_id, $pn_type_id, $pb_is_preferred,
			$pa_options )
		) {
			$this->_setUniqueIdno();
		}
	...
	private function _setUniqueIdno( $pa_options = null ) {
		...
		if ( ! strlen( trim( $this->get( $vs_idno_fld ) ) ) ) {
			if ( ! ( $vn_len = ( is_array( $va_len = $this->getFieldInfo( $vs_idno_fld, 'BOUNDS_LENGTH' ) )
			                     && sizeof( $va_len ) > 1 ) ? (int) $va_len[1] : 0 )
			) {
				$vn_len = 20;
			}
			...
			if ( call_user_func_array( $this->tableName() . '::find',
					[ $x = array( $vs_idno_fld => $vs_new_code ), array( 'returnAs' => 'firstId' ) ] ) > 0
			) {
