...
BaseModel::$s_ca_models_definitions['ca_entity_labels'] = array(
	...
	'FIELDS'        => array(
		...
		'displayname'     => array(
			...
			'DESCRIPTION'    => _t( 'Name as it should be formatted for display (eg. in catalogues and exhibition label text). If you leave this blank the display name will be automatically derived from the input of other, more specific, fields.' ),
		...
		'other_forenames' => array(
			...
			'DESCRIPTION'    => _t( 'Enter forenames other than the primary forename here.' ),
			'BOUNDS_LENGTH'  => array( 0, 100 )
		),
		'middlename'      => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DESCRIPTION'    => _t( 'Many names include one or more middle names, placed between the forename and the surname. In the Western world, a middle name is effectively a second given name. You should enter all middle names here. If there is more than one separate the names with spaces.' ),
			'BOUNDS_LENGTH'  => array( 0, 100 )
		),
		'surname'         => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Surname/organization' ),
			'DESCRIPTION'    => _t( 'A surname is a name added to a given name and is part of a personal name. In many cases a surname is a family name. For organizations this should be set to the full name.' ),
			'BOUNDS_LENGTH'  => array( 0, 512 )
		),
		'prefix'          => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Prefix' ),
			'DESCRIPTION'    => _t( 'A prefix may be added to a name to signify veneration, a social position, an official position or a professional or academic qualification.' ),
			'BOUNDS_LENGTH'  => array( 0, 100 )
		),
		'suffix'          => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Suffix' ),
			'DESCRIPTION'    => _t( 'A suffix may be added to a name to signify veneration, a social position, an official position or a professional or academic qualification.' ),
			'BOUNDS_LENGTH'  => array( 0, 100 )
		),
		'name_sort'       => array(
			'FIELD_TYPE'     => FT_TEXT,
	...
	public function insert( $pa_options = null ) {
		if ( ! trim( $this->get( 'surname' ) ) && ! trim( $this->get( 'forename' ) ) ) {
			...
			if ( $vs_display_name = trim( $this->get( 'displayname' ) ) ) {
				...
				if ( ( $t_entity = caGetOption( 'subject', $pa_options, null ) )
				     && ( $t_entity->getTypeSetting( 'entity_class' ) == 'ORG' )
				) {
					$va_label = [
				...
				if ( is_array( $va_label ) ) {
					...
					$this->postError( 1100, _t( 'Something went wrong when splitting displayname' ),
						'ca_entity_labels->insert()' );

					return false;
				}
			} else {
				$this->postError( 1100, _t( 'Surname, forename or displayname must be set' ),
					'ca_entity_labels->insert()' );
		...
		if ( ( $t_entity = caGetOption( 'subject', $pa_options, null ) )
			...
			$this->set( 'displayname', trim( preg_replace( '![ ]+!', ' ',
				$this->get( 'forename' ) . ' ' . $this->get( 'middlename' ) . ' ' . $this->get( 'surname' ) ) ) );
		}
