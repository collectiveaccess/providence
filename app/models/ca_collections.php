...
BaseModel::$s_ca_models_definitions['ca_collections'] = array(
	...
	'FIELDS'        => array(
		'collection_id'           => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this collection' )
		),
		'parent_id'               => array(
			'FIELD_TYPE'     => FT_NUMBER,
		...
		'type_id'                 => array(
			...
			'LABEL'          => _t( 'Type' ),
			'DESCRIPTION'    => _t( 'The type of the collection. In CollectiveAccess every collection has a single "instrinsic" type that determines the set of descriptive and administrative metadata that can be applied to it.' )
		),
		'idno'                    => array(
			'FIELD_TYPE'                => FT_TEXT,
		...
		'source_id'               => array(
			...
			'LABEL'                     => _t( 'Source' ),
			'DESCRIPTION'               => _t( 'Administrative source of the collection. This value is often used to indicate the administrative sub-division or legacy database from which the collection originates, but can also be re-tasked for use as a simple classification tool if needed.' )
		),
		'source_info'             => array(
			'FIELD_TYPE'     => FT_VARS,
			'DISPLAY_TYPE'   => DT_OMIT,
		...
		'acl_inherit_from_parent' => array(
			...
			'DESCRIPTION'               => _t( 'Determines whether access settings set for parent collections are applied to this collection.' )
		),
		'view_count'              => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_OMIT,
...
class ca_collections extends RepresentableBaseModel implements IBundleProvider {
	...
	public function __construct( $pn_id = null ) {
		...
		if ( ! is_null( BaseModel::$s_ca_models_definitions['ca_collections']['FIELDS']['acl_inherit_from_parent']['DEFAULT'] ) ) {
			...
			BaseModel::$s_ca_models_definitions['ca_collections']['FIELDS']['acl_inherit_from_parent']['DEFAULT']
			          = (int) $o_config->get( 'ca_collections_acl_inherit_from_parent_default' );
		}
	...
	protected function initLabelDefinitions( $pa_options = null ) {
		...
		$this->BUNDLES['ca_tour_stops'] = array(
			'type'      => 'related_table',
			'repeating' => true,
	...
	/**
	 * Override BaseModel::hierarchyWithTemplate() to optionally include top level of objects in collection hierarchy
	 * when object-collection hierarchies are enabled.
	 *
	 ...
	 *                           'desc' (descending). [Default is asc] includeObjects = Include top level of objects in
	 *                           collection hierarchy when object-collection hierarchies are enabled. id values for
	 *                           included objects will be prefixed with "ca_objects:" [Default is true] objectTemplate
	 *                           = Display template to use for included objects. [Default is to use the value set in
	 *                           app.conf in the "ca_objects_hierarchy_browser_display_settings" directive]
	 *
	 * @return array
	 */
	public function hierarchyWithTemplate( $ps_template, $pa_options = null ) {
		...
		if ( $this->getAppConfig()->get( 'ca_objects_x_collections_hierarchy_enabled' )
		     ...
		     && ( $ps_object_template = caGetOption( 'objectTemplate', $pa_options,
				$this->getAppConfig()->get( 'ca_objects_hierarchy_browser_display_settings' ) ) )
		) {
			...
			while ( $qr->nextHit() ) {
				$va_objects_by_collection[ $qr->get( 'ca_objects_x_collections.collection_id' ) ][]
					= $qr->get( 'ca_objects_x_collections.object_id' );
			}
			...
			if ( $this->getAppConfig()->get( 'ca_collections_hierarchy_summary_show_full_object_hierarachy' ) ) {
				foreach ( $va_objects_by_collection as $vn_collection_id => $va_object_ids ) {
					foreach ( $va_object_ids as $vn_object_id ) {
						$va_objects[ $vn_collection_id ][ $vn_object_id ]
							= $t_obj->hierarchyWithTemplate( $ps_object_template, [
							'object_id'     => $vn_object_id,
							'returnAsArray' => true,
							'sort'          => $this->getAppConfig()->get( 'ca_objects_hierarchy_browser_sort_values' ),
							'sortDirection' => $this->getAppConfig()
							                        ->get( 'ca_objects_hierarchy_browser_sort_direction' )
						] );
					}
				}
				...
				foreach ( $va_objects_by_collection as $vn_collection_id => $va_object_ids ) {
					$va_objects[ $vn_collection_id ] = caProcessTemplateForIDs( $ps_object_template, 'ca_objects',
						$va_object_ids, [
							'returnAsArray' => true,
							'sort'          => $this->getAppConfig()->get( 'ca_objects_hierarchy_browser_sort_values' ),
							'sortDirection' => $this->getAppConfig()
