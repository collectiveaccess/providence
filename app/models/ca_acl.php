...
class ca_acl extends BaseModel
{
    ...
    static public $s_acl_access_value_cache = array();
    ...
    public static function accessForRow($t_user, $pn_table_num, $pn_row_id)
    {
        ...
        return ca_acl::$s_acl_access_value_cache[$vn_user_id][$pn_table_num][$pn_row_id] = (int)$o_config->get(
    ...
    public static function applyACLInheritanceToRelatedRowFromRow(
        ...
        if ($t_link = $t_subject->getRelationshipInstance($ps_target)) {
            if ($t_rel_item = Datamodel::getInstanceByTableName($ps_target, false)) {
                ...
                // Delete existing inherited rows
                $o_db->query(
                    "DELETE FROM ca_acl WHERE inherited_from_table_num = ? AND inherited_from_row_id = ? AND table_num = ? AND row_id = ?",
                    array(
                        (int)$vn_subject_table_num,
                        (int)$pn_subject_id,
                ...
                if (!isset($pa_options['deleteACLOnly']) || !$pa_options['deleteACLOnly']) {
                    ...
                    $qr_inherit = $o_db->query(
                        "SELECT acl_inherit_from_{$vs_subject} FROM {$ps_target} WHERE {$vs_target_pk} = ?",
                        $pn_target_id
                    );
