...
BaseModel::$s_ca_models_definitions['ca_acl'] = array(
	...
	'FIELDS'        => array(
		'acl_id'    => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => 'ACL id',
			'DESCRIPTION'    => 'Identifier for ACL'
		),
		'group_id'  => array(
			'FIELD_TYPE'     => FT_NUMBER,
		...
		'row_id'    => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Row id',
			'DESCRIPTION'    => 'Identifier for Row'
		),
		'access'    => array(
			'FIELD_TYPE'         => FT_NUMBER,
...
class ca_acl extends BaseModel {
	...
	public static function accessForRow( $t_user, $pn_table_num, $pn_row_id ) {
		...
		return ca_acl::$s_acl_access_value_cache[ $vn_user_id ][ $pn_table_num ][ $pn_row_id ]
			= (int) $o_config->get( 'default_item_access_level' );
	}
	# ------------------------------------------------------
	...
	public static function applyACLInheritanceToChildrenFromRow( $t_subject ) {
		...
		$qr_del
			= $o_db->query( "DELETE FROM ca_acl WHERE inherited_from_table_num = ? AND inherited_from_row_id = ? AND table_num = ?",
			array( (int) $vn_subject_table_num, (int) $vn_subject_id, (int) $vn_subject_table_num ) );
	...
	public static function applyACLInheritanceToRelatedFromRow( $t_subject, $ps_target ) {
		...
		if ( $t_link = $t_subject->getRelationshipInstance( $ps_target ) ) {
			if ( $t_rel_item = Datamodel::getInstanceByTableName( $ps_target, false ) ) {
				...
				$qr_del
					= $o_db->query( "DELETE FROM ca_acl WHERE inherited_from_table_num = ? AND inherited_from_row_id = ? AND table_num = ?",
					array( (int) $vn_subject_table_num, (int) $vn_subject_id, (int) $vn_target_table_num ) );

				$qr_res = $o_db->query( "
					SELECT {$ps_target}.{$vs_target_pk}
					FROM {$vs_subject}
					" . join( "\n", $va_joins ) . "
					WHERE ({$vs_subject}.{$vs_subject_pk} = ?) AND {$ps_target}.acl_inherit_from_{$vs_subject} = 1",
					(int) $t_subject->getPrimaryKey() );
	 ...
	 *
	 */
	public static function applyACLInheritanceToRelatedRowFromRow(
		$t_subject, $pn_subject_id, $ps_target, $pn_target_id, $pa_options = null
	) {
		$o_db = new Db();
		...
		if ( $t_link = $t_subject->getRelationshipInstance( $ps_target ) ) {
			if ( $t_rel_item = Datamodel::getInstanceByTableName( $ps_target, false ) ) {
				...
				// Delete existing inherited rows
				$o_db->query( "DELETE FROM ca_acl WHERE inherited_from_table_num = ? AND inherited_from_row_id = ? AND table_num = ? AND row_id = ?",
					array(
						(int) $vn_subject_table_num,
						(int) $pn_subject_id,
						(int) $vn_target_table_num,
						(int) $pn_target_id
					) );
				...
				if ( ! isset( $pa_options['deleteACLOnly'] ) || ! $pa_options['deleteACLOnly'] ) {
					...
					$qr_inherit
						= $o_db->query( "SELECT acl_inherit_from_{$vs_subject} FROM {$ps_target} WHERE {$vs_target_pk} = ?",
						$pn_target_id );
