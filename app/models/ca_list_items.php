...
BaseModel::$s_ca_models_definitions['ca_list_items'] = array(
	...
	'FIELDS'        => array(
		'item_id'           => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this item' )
		),
		'parent_id'         => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Parent' ),
			'DESCRIPTION'    => _t( 'Parent list item for this item' )
		),
		'list_id'           => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'List' ),
			'DESCRIPTION'    => _t( 'List item belongs to' )
		),
		'type_id'           => array(
			'FIELD_TYPE'      => FT_NUMBER,
			...
			'DEFAULT'         => '',
			'LABEL'           => _t( 'Type' ),
			'DESCRIPTION'     => _t( 'Indicates the type of the list item.' )
		),
		'idno'              => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Identifier' ),
			'DESCRIPTION'    => _t( 'Unique identifier for this list item' ),
		...
		'idno_sort'         => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Identifier sort' ),
			'DESCRIPTION'    => _t( 'Sortable value for identifier' ),
			'BOUNDS_LENGTH'  => array( 0, 255 )
		),
		'item_value'        => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Item value' ),
			'DESCRIPTION'    => _t( 'Value of this list item; is stored in database when this item is selected' ),
			'BOUNDS_LENGTH'  => array( 0, 255 )
		),
		'color'             => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Item color' ),
			'DESCRIPTION'    => _t( 'Color to display item in' )
		),
		'icon'              => array(
			'FIELD_TYPE'               => FT_MEDIA,
			'DISPLAY_TYPE'             => DT_FIELD,
			...
			'LABEL'                    => _t( 'Item icon' ),
			'DESCRIPTION'              => _t( 'Optional icon to use with item' )
		),
		'rank'              => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_FIELD,
		...
		'is_enabled'        => array(
			...
			'DEFAULT'        => '1',
			'LABEL'          => _t( 'Is enabled?' ),
			'DESCRIPTION'    => _t( 'If checked this item is selectable and can be used in cataloguing.' )
		),
		'is_default'        => array(
			'FIELD_TYPE'     => FT_BIT,
			'DISPLAY_TYPE'   => DT_SELECT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Is default?' ),
			'DESCRIPTION'    => _t( 'If checked this item will be the default selection for the list.' )
		),
		'validation_format' => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Validation format' ),
			'DESCRIPTION'    => _t( 'NOT CURRENTLY SUPPORTED; WILL BE A PERL-COMPATIBLE REGEX APPLIED TO VALIDATE INPUT' ),
			'BOUNDS_LENGTH'  => array( 0, 255 )
		),
		'access'            => array(
			'FIELD_TYPE'         => FT_NUMBER,
			...
			'LABEL'              => _t( 'Access' ),
			'DESCRIPTION'        => _t( 'Indicates if the list item is accessible to the public or not. ' )
		),
		'settings'          => array(
			'FIELD_TYPE'     => FT_VARS,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Settings' ),
			'DESCRIPTION'    => _t( 'List item settings' )
		),
		'status'            => array(
			'FIELD_TYPE'         => FT_NUMBER,
			'DISPLAY_TYPE'       => DT_SELECT,
			...
			'LABEL'              => _t( 'Status' ),
			'DESCRIPTION'        => _t( 'Indicates the current state of the list item.' )
		),
		'deleted'           => array(
			'FIELD_TYPE'     => FT_BIT,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => 0,
			'LABEL'          => _t( 'Is deleted?' ),
			'DESCRIPTION'    => _t( 'Indicates if list item is deleted or not.' )
		),
		'source_id'         => array(
			'FIELD_TYPE'                => FT_NUMBER,
			'DISPLAY_TYPE'              => DT_SELECT,
			...
			'LABEL'                     => _t( 'Source' ),
			'DESCRIPTION'               => _t( 'Administrative source of list item. This value is often used to indicate the administrative sub-division or legacy database from which the object originates, but can also be re-tasked for use as a simple classification tool if needed.' )
		),
		'source_info'       => array(
			'FIELD_TYPE'     => FT_VARS,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DESCRIPTION'    => 'Serialized array used to store source information for list item information retrieved via web services [NOT IMPLEMENTED YET].'
		)
	)
);
...
class ca_list_items extends RepresentableBaseModel implements IHierarchy {
	...
	public function insert( $pa_options = null ) {
		...
		if ( $this->getPrimaryKey() ) {
			...
			if ( ( $t_list->load( $this->get( 'list_id' ) ) ) && ( $t_list->get( 'list_code' ) == 'place_hierarchies' )
			     && ( $this->get( 'parent_id' ) )
			) {
				...
				if ( ! $vn_locale_id ) {
					$this->postError( 750, _t( 'Locale %1 does not exist', $va_locales[0] ),
						'ca_list_items->insert()' );
	...
	public function getHierarchyList( $pb_vocabularies = false ) {
		...
		$qr_res = $o_db->query( "
			...
			WHERE 
				cli.parent_id IS NULL and cli.list_id IN (" . join( ',', $va_hierarchy_ids ) . ") " . ( $pb_vocabularies
				? " AND (l.use_as_vocabulary = 1)" : "" ) . " AND
				l.deleted = 0
			GROUP BY
		...
		while ( $qr_res->nextRow() ) {
			...
			$va_hierarchies[ $vn_hierarchy_id ]['list_id']
			                                               = $qr_res->get( 'list_id' );        // when we need to edit the list
			...
			$va_hierarchies[ $vn_hierarchy_id ]['name']         = caProcessTemplateForIDs( $vs_template, 'ca_lists',
				array( $vn_hierarchy_id ), array( 'requireLinkTags' => true ) );
	...
	public function set( $pa_fields, $pm_value = "", $pa_options = null ) {
		...
		foreach ( $pa_fields as $vs_field => $vm_value ) {
			if ( ( $vs_field == 'list_id' ) && ( ! is_numeric( $vm_value ) )
			     && ( $vn_list_id
					= caGetListID( $vm_value ) )
			) {
	...
	/**
	 * Override standard implementation to insert list_code for current list_id into returned data. The list_code is
	 * required for consumers of export data when dealing with lists.
	 *
	 * @param array $pa_options Array of options for BaseModel::getValuesForExport(). No additional options are defined
	 *                          by this subclass.
	 *
