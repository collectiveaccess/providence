...
BaseModel::$s_ca_models_definitions['ca_list_items'] = array(
    ...
    'FIELDS' => array(
        ...
        'source_id' => array(
            ...
            'LABEL' => _t('Source'),
            'DESCRIPTION' => _t(
                'Administrative source of list item. This value is often used to indicate the administrative sub-division or legacy database from which the object originates, but can also be re-tasked for use as a simple classification tool if needed.'
            )
        ),
        'source_info' => array(
            'FIELD_TYPE' => FT_VARS,
...
$_ca_list_items_settings = array(
    'entity_types' => array(        // global
        'entity_class' => array(
            ...
            'description' => _t(
                'The class of entity the type represents. Use <em>Individual person</em> for entities that require a fully articulated personal name. Use <em>organization</em> for group entities such as corporations, clubs and families.'
            )
        ),
...
class ca_list_items extends RepresentableBaseModel implements IHierarchy
{
    ...
    public function insert($pa_options = null)
    {
        ...
        if ($this->getPrimaryKey()) {
            ...
            if (($t_list->load($this->get('list_id'))) && ($t_list->get(
                        'list_code'
                    ) == 'place_hierarchies') && ($this->get('parent_id'))) {
    ...
    public function getHierarchyList($pb_vocabularies = false)
    {
        ...
        $qr_res = $o_db->query(
			...
			WHERE 
				cli.parent_id IS NULL and cli.list_id IN (" . join(
                ',',
                $va_hierarchy_ids
            ) . ") " . ($pb_vocabularies ? " AND (l.use_as_vocabulary = 1)" : "") . " AND
				l.deleted = 0
			GROUP BY
        ...
        while ($qr_res->nextRow()) {
            ...
            $va_hierarchies[$vn_hierarchy_id]['list_id'] = $qr_res->get(
                'list_id'
            ...
            $va_hierarchies[$vn_hierarchy_id]['name'] = caProcessTemplateForIDs(
                $vs_template,
                'ca_lists',
                array($vn_hierarchy_id),
                array('requireLinkTags' => true)
            );
    ...
    {
        // Is row loaded?
        if (!($vn_list_id = $this->get(
                'list_id'
            )) && $po_request) { // this happens when a new list item is about to be created. in those cases we extract the list from the request.
