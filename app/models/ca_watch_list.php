...
BaseModel::$s_ca_models_definitions['ca_watch_list'] = array(
	...
	'FIELDS'        => array(
		'watch_id'  => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this watch list entry' )
		),
		'table_num' => array(
			'FIELD_TYPE'     => FT_NUMBER,
		...
		'row_id'    => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Row ID',
			'DESCRIPTION'    => 'Primary key value of the row in the table specified by table_num that this watch applies to.'
		),
		'user_id'   => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'User' ),
			'DESCRIPTION'    => _t( 'The user who is watching the specified record.' )
		)
	)
);
...
class ca_watch_list extends BaseModel {
	...
	public function isItemWatched( $pn_row_id, $pn_table_num, $pn_user_id ) {
		...
		$o_db = $this->getDb();
		$q_is_watched
		      = $o_db->query( "SELECT watch_id from ca_watch_list WHERE row_id = ? AND table_num = ? AND user_id = ?",
			$pn_row_id, $pn_table_num, $pn_user_id );
	...
	public function getWatchedItems( $pn_user_id, $pn_table_num = null, $pa_options = null ) {
		...
		$vn_request_user_id = ( $po_request = caGetOption( 'request', $pa_options, null ) ) ? $po_request->getUserID()
			: null;
		...
		if ( $q_watched_items->numRows() > 0 ) {
			while ( $q_watched_items->nextRow() ) {
				...
				$va_items[] = array(
					...
					"change_log"  => $t_changelog->getChangeLogForRowForDisplay( $t_item_table, null,
						$vn_request_user_id )
				);
			}
		}
