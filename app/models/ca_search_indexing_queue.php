...
BaseModel::$s_ca_models_definitions['ca_search_indexing_queue'] = array(
    ...
    'FIELDS' => array(
        'entry_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique numeric identifier used by CollectiveAccess internally to identify this queue entry'
            )
        ),
        'table_num' => array(
            'FIELD_TYPE' => FT_NUMBER,
...
class ca_search_indexing_queue extends BaseModel
{
     ...
     * Path to lock file
     */
    static public $s_lock_filepath = __CA_APP_DIR__ . DIRECTORY_SEPARATOR . 'tmp' . DIRECTORY_SEPARATOR . __CA_APP_NAME__ . '_search_indexing_queue.lock';

    /**
     * Lock time out. Locks older than this will be removed.
     */
    static public $s_lock_timeout = 3 * 60 * 60;   // in seconds
    ...
    public static function process()
    {
        if (self::lockAcquire()) {
            ...
            do {
                $num_entries = 0;
                if ($o_result = $o_db->query(
                    "SELECT * FROM ca_search_indexing_queue WHERE started_on IS NULL ORDER BY entry_id LIMIT 100"
                )) {
                    ...
                    if ($num_entries > 0) {
                        ...
                        while ($o_result->nextRow()) {
                            ...
                            $o_db->query(
                                'DELETE FROM ca_search_indexing_queue WHERE entry_id = ?',
