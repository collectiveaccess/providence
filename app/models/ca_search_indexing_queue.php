...
BaseModel::$s_ca_models_definitions['ca_search_indexing_queue'] = array(
	...
	'FIELDS'        => array(
		'entry_id'       => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this queue entry' )
		),
		'table_num'      => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Table',
			'DESCRIPTION'    => 'Table to index for search',
		...
		'row_id'         => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Row id',
			'DESCRIPTION'    => 'Identifier of row to index for search'
		),
		'field_data'     => array(
			'FIELD_TYPE'     => FT_VARS,
		...
		'reindex'        => array(
			...
			'LABEL'                     => 'Reindex?',
			'DESCRIPTION'               => 'Indicates if this is a full reindex for this row'
		),
		...
		'is_unindex'     => array(
			...
			'DONT_ALLOW_IN_UI'          => true,
			'LABEL'                     => 'Is unindex?',
			'DESCRIPTION'               => 'Indicates if this is a unindexing instruction'
		),
		'dependencies'   => array(
			'FIELD_TYPE'     => FT_VARS,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Dependencies',
			'DESCRIPTION'    => 'Dependencies to unindex (only set if is_unindex = 1)'
		),
		'started_on'     => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_FIELD,
...
class ca_search_indexing_queue extends BaseModel {
	 ...
	 * Path to lock file
	 */
	static $s_lock_filepath
		= __CA_APP_DIR__ . DIRECTORY_SEPARATOR . 'tmp' . DIRECTORY_SEPARATOR . __CA_APP_NAME__
		  . '_search_indexing_queue.lock';
	...
	static public function process() {
		if ( self::lockAcquire() ) {
			...
			do {
				$num_entries = 0;
				if ( $o_result
					= $o_db->query( "SELECT * FROM ca_search_indexing_queue WHERE started_on IS NULL ORDER BY entry_id LIMIT 100" )
				) {
					...
					if ( $num_entries > 0 ) {
						...
						while ( $o_result->nextRow() ) {
							$o_db->query( 'UPDATE ca_search_indexing_queue SET started_on = ? WHERE entry_id = ?',
								[ time(), (int) $o_result->get( 'entry_id' ) ] );
							if ( ! $o_result->get( 'is_unindex' ) ) { // normal indexRow() call
								$o_si->indexRow(
									...
									array_merge( caUnserializeForDatabase( $o_result->get( 'options' ) ),
										array( 'queueIndexing' => false ) )
								);
								...
								$o_si->commitRowUnIndexing(
									...
									[
										'queueIndexing' => false,
										'dependencies'  => caUnserializeForDatabase( $o_result->get( 'dependencies' ) )
									]
								);
							}

							$o_db->query( 'DELETE FROM ca_search_indexing_queue WHERE entry_id = ?',
								[ $o_result->get( 'entry_id' ) ] );
						}
					}
				}
			} while ( $num_entries > 0 );
