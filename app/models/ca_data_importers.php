...
BaseModel::$s_ca_models_definitions['ca_data_importers'] = array(
    ...
    'FIELDS' => array(
        'importer_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique numeric identifier used by CollectiveAccess internally to identify this importer'
            )
        ),
        'importer_code' => array(
            'FIELD_TYPE' => FT_TEXT,
...
class ca_data_importers extends BundlableLabelableBaseModelWithAttributes
{
    ...
    protected function initSettings()
    {
        ...
        $va_settings['type'] = array(
            ...
            'description' => _t(
                'Type to set all imported records to. If import includes a mapping to type_id, that will be privileged and the type setting will be ignored.'
            )
        );
        ...
        $va_settings['code'] = array(
            ...
            'description' => _t(
                'Arbitrary alphanumeric code for the import mapping (no special characters or spaces).  Pending implementation.'
            )
        );
        ...
        $va_settings['existingRecordPolicy'] = array(
            ...
            'description' => _t(
                'Determines how existing records are checked for and handled by the import mapping.  Pending implementation.'
            )
        );
        $va_settings['ignoreTypeForExistingRecordPolicy'] = array(
            ...
            'default' => 0,
            'label' => _t(
                'Ignore record type when looking for existing records as specified by the existing records policy.'
            ),
            'description' => _t('.')
        );
        $va_settings['mergeOnly'] = array(
            ...
            'default' => 0,
            'label' => _t(
                'If set data will only be merged with existing records using the existing records policy. No new records will be created.'
            ),
            'description' => _t('.')
        );
        ...
        $va_settings['archiveDataSets'] = array(
            ...
            'description' => _t(
                'Set to yes to save the data spreadsheet or no to delete it from the server after import.  Pending implementation.'
            )
        );
        $va_settings['errorPolicy'] = array(
            ...
            'description' => _t(
                'Determines how errors are handled for the import.  Options are to ignore the error, stop the import when an error is encountered and to receive a prompt when the error is encountered.'
            )
        );
        ...
        $va_settings['locale'] = array(
            ...
            'label' => _t('Locale'),
            'description' => _t(
                'Set the locale used for all imported data. Leave on "DEFAULT" to use the system default locale. Otherwise set it to a valid locale code (Ex. en_US, es_MX, fr_CA).'
            )
        );
        ...
        $va_settings['sourceUrl'] = array(
            ...
            'label' => _t('Source URL'),
            'description' => _t(
                'URL importer worksheet was fetched from. Will be null if importer was directly uploaded.'
            )
        );
    ...
    public static function getAvailableInputFormats()
    {
        ...
        foreach ($va_readers as $vs_reader) {
            if ((DataReaderManager::checkDataReaderStatus(
                    $vs_reader
                )) && ($o_reader = DataReaderManager::getDataReaderInstance($vs_reader))) {
    ...
    public static function getImporters($pn_table_num = null, $pa_options = null)
    {
        ...
        while ($qr_res->nextRow()) {
            ...
            if (isset($va_settings['inputFormats']) && is_array($va_settings['inputFormats']) && is_array(
                    $va_formats
                )) {
            ...
            $va_importers[$vn_id]['last_modified_on'] = $t_importer->getLastChangeTimestamp(
                $vn_id,
     ...
     * @return int
     */
    public static function getImporterListAsHTMLFormElement(
        $ps_name,
        $pn_table_num = null,
        $pa_attributes = null,
        $pa_options = null
    ...
    public function getItems()
    {
        ...
        while ($qr_items->nextRow()) {
            ...
            $va_return[$qr_items->get("item_id")]['settings'] = caUnserializeForDatabase(
    ...
    public static function loadImporterFromFile($ps_source, &$pa_errors, $pa_options = null, &$is_new = null)
    {
        ...
        $o_excel = IOFactory::load($ps_source);
        ...
        foreach ($o_sheet->getRowIterator() as $o_row) {
            ...
            switch (strtolower($vs_mode)) {
                ...
                case 'constant':
                    ...
                    if (!$vs_source) {
                        $pa_errors[] = _t(
                            "Warning: skipped mapping at row %1 because source was not defined",
                            $vn_row_num
                        );
                        if ($o_log) {
                            $o_log->logWarn(
                                _t(
                                    "[loadImporterFromFile:%1] Skipped mapping at row %2 because source was not defined",
                                    $ps_source,
                                    $vn_row_num
                                )
                            );
                    ...
                    if (!$vs_destination) {
                        $pa_errors[] = _t(
                            "Warning: skipped mapping at row %1 because destination was not defined",
                            $vn_row_num
                        );
                        if ($o_log) {
                            $o_log->logWarn(
                                _t(
                                    "[loadImporterFromFile:%1] Skipped mapping at row %2 because destination was not defined",
                                    $ps_source,
                                    $vn_row_num
                                )
                            );
                    ...
                    if ($vs_options_json = (string)$o_options->getValue()) {
                        if (is_null($va_options = @json_decode(caRepairJson($vs_options_json), true))) {
                            ...
                            $pa_errors[] = _t(
                                "Warning: invalid json in \"options\" column at line %4  for group %1/source %2. Json was: %3",
                                $vs_group,
                                $vs_source,
                                $vs_options_json,
                                $vn_row_num
                            ...
                            if ($o_log) {
                                $o_log->logWarn(
                                    _t(
                                        "[loadImporterFromFile:%1] Invalid json in \"options\" column at line %5  for group %2/source %3. Json was: %4.",
                                        $ps_source,
                                        $vs_group,
                                        $vs_source,
                                        $vs_options_json,
                    ...
                    if ($vs_refinery && ($vs_refinery_options_json = (string)$o_refinery_options->getValue())) {
                        if (!in_array($vs_refinery, $va_refineries)) {
                            ...
                            if ($o_log) {
                                $o_log->logWarn(
                                    _t(
                                        "[loadImporterFromFile:%1] Invalid options for group %2/source %3",
                                        $ps_source,
                                        $vs_group,
                                        $vs_source
                                    )
                            ...
                            if (is_null(
                                ...
                                // Error while json decode
                                $pa_errors[] = _t(
                                    "invalid json for refinery options at line %4 for group %1/source %2 = %3",
                                    $vs_group,
                                    $vs_source,
                                    $vs_refinery_options_json,
                                    $vn_row_num
                                );
                                if ($o_log) {
                                    $o_log->logError(
                                        _t(
                                            "[loadImporterFromFile:%1] invalid json for refinery options at line %5 for group %2/source %3 = %4",
                                            $ps_source,
                                            $vs_group,
                                            $vs_source,
                                            $vs_refinery_options_json,
                    ...
                    if ($va_options && is_array(
                            $va_options
                        ) && isset($va_options['transformValuesUsingWorksheet']) && $va_options['transformValuesUsingWorksheet']) {
                        if ($o_opt_sheet = $o_excel->getSheetByName($va_options['transformValuesUsingWorksheet'])) {
                            foreach ($o_opt_sheet->getRowIterator() as $o_sheet_row) {
                                if (!$vs_original_value = trim(
                                    mb_strtolower(
                                        (string)$o_opt_sheet->getCellByColumnAndRow(1, $o_sheet_row->getRowIndex())
                                    )
                                )) {
                                    continue;
                                }
                                $vs_replacement_value = trim(
                                    (string)$o_opt_sheet->getCellByColumnAndRow(2, $o_sheet_row->getRowIndex())
                                );
                ...
                case 'environment':
                    ...
                    if ($vs_options_json = (string)$o_options->getValue()) {
                        ...
                        if (is_null($va_options = @json_decode($vs_options_json, true))) {
                            $pa_errors[] = _t(
                                "Warning: invalid options for environment %1.",
                                (string)$o_source->getValue()
                            );
                            if ($o_log) {
                                $o_log->logWarn(
                                    _t(
                                        "[loadImporterFromFile:environment %1] Invalid options for environment value %1. Options were: %2.",
                                        (string)$o_source->getValue(),
                                        $vs_options_json
                                    )
                                );
        ...
        if (isset($va_settings['exporter_format'])) {
            $pa_errors[] = _t(
                "It looks like this is a mapping for the data export framework and you're trying to add it as import mapping!"
            );
            if ($o_log) {
                $o_log->logError(
                    _t(
                        "[loadImporterFromFile:%1] It looks like this is a mapping for the data export framework and you're trying to add it as import mapping!",
                        $ps_source
                    )
                );
            }
        ...
        if (!isset($va_settings['inputFormats']) || !is_array($va_settings['inputFormats']) || !sizeof(
        ...
        if (!($t_instance = Datamodel::getInstance($va_settings['table']))) {
            ...
            if ($o_log) {
                $o_log->logError(
                    _t(
                        "[loadImporterFromFile:%1] Mapping target table %2 is invalid\n",
                        $ps_source,
                        $va_settings['table']
                    )
                );
        ...
        if ($t_importer->load(array('importer_code' => $va_settings['code']))) {
            ...
            if ($t_importer->numErrors()) {
                $pa_errors[] = _t(
                        "Could not delete existing mapping for %1: %2",
                        $va_settings['code'],
                ...
                if ($o_log) {
                    $o_log->logError(
                        _t(
                            "[loadImporterFromFile:%1] Could not delete existing mapping for %2: %3",
                            $ps_source,
                            $va_settings['code'],
                            join("; ", $t_importer->getErrors())
                        )
        ...
        if ($t_importer->numErrors()) {
            ...
            if ($o_log) {
                $o_log->logError(
                    _t(
                        "[loadImporterFromFile:%1] Error creating mapping: %2",
                        $ps_source,
                        join("; ", $t_importer->getErrors())
                    )
                );
        ...
        if ($t_importer->numErrors()) {
            $pa_errors[] = _t(
                    "Could not save worksheet for future download: %1",
                    join("; ", $t_importer->getErrors())
                ) . "\n";
            if ($o_log) {
                $o_log->logError(
                    _t(
                        "[loadImporterFromFile:%1] Error saving worksheet for future download: %2",
                        $ps_source,
                        join("; ", $t_importer->getErrors())
                    )
                );
        ...
        foreach ($va_mapping as $vs_group => $va_mappings_for_group) {
            ...
            if (!$vs_group_dest) {
                ...
                $pa_errors[] = _t(
                        "Skipped items for %1 because no common grouping could be found",
                        $va_item['destination']
                    ) . "\n";
                if ($o_log) {
                    $o_log->logWarn(
                        _t(
                            "[loadImporterFromFile:%1] Skipped items for %2 because no common grouping could be found",
                            $ps_source,
                            $va_item['destination']
                        )
                    );
            ...
            if (!$t_group) {
                ...
                if ($o_log) {
                    $o_log->logError(
                        _t("[loadImporterFromFile:%1] There was an error when adding group %2", $ps_source, $vs_group)
                    );
                }
                return null;
            }
    ...
    public function importDataFromSource($ps_source, $ps_mapping, $pa_options = null)
    {
        ...
        $this->log_path = $log_path = caGetOption(
            'logDirectory',
            $pa_options,
        ...
        $this->detailed_log_name = $detailed_log_name = pathinfo(
            caGetOption(
                'detailedLogName',
                $pa_options,
                "import_errors_" . $t_mapping->get('importer_code') . "_" . date("Y-m-d")
            ),
            PATHINFO_FILENAME
        );

        $o_event = ca_data_import_events::newEvent(
            isset($pa_options['user_id']) ? $pa_options['user_id'] : null,
            $pa_options['format'],
            $ps_source,
        ...
        $va_log_import_error_opts = array(
            ...
            'logReference' => null,
            'request' => $po_request,
            'progressCallback' => (isset($pa_options['progressCallback']) && ($ps_callback = $pa_options['progressCallback'])) ? $ps_callback : null,
            'reportCallback' => (isset($pa_options['reportCallback']) && ($ps_callback = $pa_options['reportCallback'])) ? $ps_callback : null
        );


        $t_add_to_set = null;
        if ($ps_add_to_set = caGetOption('addToSet', $pa_options, null)) {
            if (!($t_add_to_set = ca_sets::find(
                ['set_code' => $ps_add_to_set],
                ['returnAs' => 'firstModelInstance']
            ))) {
                // Set doesn't exist
                $this->logImportError(
                    _t("Set %1 does not exist. Create it and try the import again.", $ps_add_to_set),
            ...
            if ((int)$t_mapping->get('table_num') !== (int)$t_add_to_set->get('table_num')) {
                ...
                $this->logImportError(
                    _t("Set %1 is not the correct type for this import.", $ps_add_to_set),
                    $va_log_import_error_opts
                );
                return false;
            }
        ...
        if (!is_array(
                $pa_options
            ) || !isset($pa_options['logDirectory']) || !$pa_options['logDirectory'] || !file_exists(
        ...
        $merge_only = (bool)$t_mapping->getSetting(
            'mergeOnly'
        );    // If set we only merge; no new records will be created.
        ...
        if (!($o_reader = $t_mapping->getDataReader($ps_source, $ps_format))) {
            $this->logImportError(
                _t("Could not open source %1 (format=%2)", $ps_source, $ps_format),
                $va_log_import_error_opts
            );
            if ($o_trans) {
                $o_trans->rollback();
        ...
        if (!$o_reader->read($ps_source, $va_reader_opts)) {
            $this->logImportError(
                _t("Could not read source %1 (format=%2)", $ps_source, $ps_format),
                $va_log_import_error_opts
            );
            if ($o_trans) {
                $o_trans->rollback();
        ...
        for ($vn_dataset = 0; $vn_dataset < $vn_dataset_count; $vn_dataset++) {
            ...
            $vn_primary_key_mapping_item_id = $vn_type_id_mapping_item_id = $vn_idno_mapping_item_id = $vn_list_mapping_list_id = null;
            ...
            if (is_array($va_environment_config = $t_mapping->getEnvironment())) {
                foreach ($va_environment_config as $vn_i => $va_environment_item) {
                    ...
                    if (!$o_env_reader->read(
                        $ps_source,
                        array(
                            'basePath' => '',
                            'originalFilename' => caGetOption(
                                'originalFilename',
                                $pa_options,
                                null
                            )
                    ...
                    switch (sizeof($va_env_tmp)) {
                        case 1:
                            $vs_env_value = $o_env_reader->get(
                                $va_environment_item['value'],
            ...
            while ($o_reader->nextRow()) {
                ...
                foreach ($va_mapping_items as $vn_item_id => $va_item) {
                    ...
                    if (is_array($va_row[$vs_key])) {
                        $va_row_with_replacements[$vs_key] = array_map(
                            function ($v) use ($va_item) {
                                return ca_data_importers::replaceValue($v, $va_item, []);
                            },
                            $va_row[$vs_key]
                        );
                    } else {
                        $va_row_with_replacements[$vs_key] = ca_data_importers::replaceValue(
                            $va_row[$vs_key],
                            $va_item,
                            []
                        );
                ...
                foreach ($va_mapping_rules as $va_rule) {
                    ...
                    try {
                                    ...
                                    default:
                                        if ($vs_action_code != 'skip') {
                                            if ($log_skip) {
                                                $o_log->logInfo(
                                                    _t(
                                                        'Row was skipped using rule "%1" with default action because an invalid action ("%2") was specified',
                                                        $va_rule['trigger'],
                                                        $vs_action_code
                                                    )
                                                );
                                            ...
                                            $o_log->logDebug(
                                                _t(
                                                    'Row was skipped using rule "%1" with action "%2"',
                                                    $va_rule['trigger'],
                                                    $vs_action_code
                                                )
                                            );
                        ...
                        $o_log->logError(
                            _t(
                                "Expression trigger could not be parsed. Please double-check the syntax! Expression was: %1 [%2]",
                                $va_rule['trigger'],
                                $e->getMessage()
                            )
                        );
                ...
                if ($vn_type_id_mapping_item_id) {
                    ...
                    $vs_type = ca_data_importers::getValueFromSource(
                        $va_mapping_items[$vn_type_id_mapping_item_id],
                        $o_reader,
                        array(
                    ...
                    if (isset($va_mapping_items[$vn_type_id_mapping_item_id]['settings']['default']) && strlen(
                            $va_mapping_items[$vn_type_id_mapping_item_id]['settings']['default']
                        ) && !strlen($vs_type)) {
                ...
                if (isset($va_mapping_items[$vn_type_id_mapping_item_id]['settings']['default']) && strlen(
                        $va_mapping_items[$vn_type_id_mapping_item_id]['settings']['default']
                            ...
                            array_filter(
                                $va_subject_type_list,
                                function ($v) use ($vs_type) {
                ...
                if ($vn_idno_mapping_item_id) {
                    ...
                    $vs_idno = ca_data_importers::getValueFromSource(
                        $va_mapping_items[$vn_idno_mapping_item_id],
                        $o_reader,
                        array(
                            'otherValues' => $va_rule_set_values,
                            'environment' => $va_environment
                    ...
                    if (isset($va_mapping_items[$vn_idno_mapping_item_id]['settings']['default']) && strlen(
                            $va_mapping_items[$vn_idno_mapping_item_id]['settings']['default']
                        ) && !strlen($vs_idno)) {
                    ...
                    if ($vs_idno && isset($va_mapping_items[$vn_idno_mapping_item_id]['settings']['prefix']) && strlen(
                            $va_mapping_items[$vn_idno_mapping_item_id]['settings']['prefix']
                        )) {
                        $vs_idno = DisplayTemplateParser::processTemplate(
                                $va_mapping_items[$vn_idno_mapping_item_id]['settings']['prefix'],
                                $va_row_with_replacements,
                                ['getFrom' => $o_reader]
                            ) . $vs_idno;
                    }
                    if ($vs_idno && isset($va_mapping_items[$vn_idno_mapping_item_id]['settings']['suffix']) && strlen(
                            $va_mapping_items[$vn_idno_mapping_item_id]['settings']['suffix']
                        )) {
                        $vs_idno .= DisplayTemplateParser::processTemplate(
                            $va_mapping_items[$vn_idno_mapping_item_id]['settings']['suffix'],
                            $va_row_with_replacements,
                    ...
                    if (isset($va_mapping_items[$vn_idno_mapping_item_id]['settings']['applyRegularExpressions']) && is_array(
                            $va_mapping_items[$vn_idno_mapping_item_id]['settings']['applyRegularExpressions']
                        )) {
                        $vs_idno = self::_processAppliedRegexes(
                            $o_reader,
                            $va_mapping_items[$vn_idno_mapping_item_id],
                            0,
                            $va_mapping_items[$vn_idno_mapping_item_id]['settings']['applyRegularExpressions'],
                    ...
                    if (isset($va_mapping_items[$vn_idno_mapping_item_id]['settings']['formatWithTemplate']) && strlen(
                            $va_mapping_items[$vn_idno_mapping_item_id]['settings']['formatWithTemplate']
                        )) {
                        $vs_idno = DisplayTemplateParser::processTemplate(
                            $va_mapping_items[$vn_idno_mapping_item_id]['settings']['formatWithTemplate'],
                            $va_row_with_replacements,
                    ...
                    if ($va_mapping_items[$vn_idno_mapping_item_id]['settings']['delimiter'] && $va_mapping_items[$vn_idno_mapping_item_id]['settings']['treatAsIdentifiersForMultipleRows']) {
                        $va_idnos_for_row = explode(
                            $va_mapping_items[$vn_idno_mapping_item_id]['settings']['delimiter'],
                            $vs_idno
                        );
                    }
                } else {
                ...
                if ($vn_primary_key_mapping_item_id) {
                    $vn_mapped_primary_key_value = ca_data_importers::getValueFromSource(
                        $va_mapping_items[$vn_primary_key_mapping_item_id],
                        $o_reader,
                        array('otherValues' => $va_rule_set_values, 'environment' => $va_environment)
                    );
                }
                ...
                foreach ($va_idnos_for_row as $vs_idno) {
                    ...
                    foreach ($va_preferred_label_mapping_ids as $vn_preferred_label_mapping_id => $vs_preferred_label_mapping_fld) {
                        $vs_label_val = ca_data_importers::getValueFromSource(
                        ...
                        if (isset($va_mapping_items[$vn_preferred_label_mapping_id]['settings']['formatWithTemplate']) && strlen(
                                $va_mapping_items[$vn_preferred_label_mapping_id]['settings']['formatWithTemplate']
                            )) {
                            $vs_label_val = DisplayTemplateParser::processTemplate(
                                $va_mapping_items[$vn_preferred_label_mapping_id]['settings']['formatWithTemplate'],
                                $va_row_with_replacements
                            );
                        }
                        if ($vs_opt = $va_mapping_items[$vn_preferred_label_mapping_id]['settings']['displaynameFormat']) {
                            $va_label_val = DataMigrationUtils::splitEntityName(
                                $vs_label_val,
                                array(
                    ...
                    $va_base_criteria = ($vs_type && !(bool)$t_mapping->getSetting(
                    ...
                    if (is_array($pa_force_import_for_primary_keys) && sizeof($pa_force_import_for_primary_keys) > 0) {
                        ...
                        if (!$t_subject->load($vn_id)) {
                            if ($log_erp) {
                                $o_log->logInfo(
                                    _t('[%1] Skipped import because forced primary key \'%1\' does not exist', $vn_id)
                                );
                            }
                            $this->num_records_skipped++;
                            ...
                            case 'merge_on_id_with_skip':
                                ...
                                if (is_array($va_ids) && (sizeof($va_ids) > 0)) {
                                    ...
                                    if ($log_erp) {
                                        $o_log->logInfo(
                                            _t(
                                                '[%1] Merged with existing record matched on primary key %2 for %3 by policy %4',
                                                $vs_idno,
                                    ...
                                    if ($vs_existing_record_policy === 'merge_on_id_with_skip') {
                                        if ($log_erp) {
                                            $o_log->logInfo(
                                                _t(
                                                    '[%1] Skipped import because existing record could not be matched on primary key %2 for %3 by policy %4',
                                                    $vs_idno,
                                                    $vn_mapped_primary_key_value,
                                                    $t_subject->tableName(),
                                                    $vs_existing_record_policy
                                        ...
                                        if ($log_erp) {
                                            $o_log->logInfo(
                                                _t(
                                                    '[%1] Could not match existing record on primary key %2 for %3 by policy %4',
                                                    $vs_idno,
                                                    $vn_mapped_primary_key_value,
                                                    $t_subject->tableName(),
                                                    $vs_existing_record_policy
                            ...
                            case 'skip_on_id':
                                ...
                                if (is_array($va_ids) && (sizeof($va_ids) > 0)) {
                                    if ($log_erp) {
                                        $o_log->logInfo(
                                            _t(
                                                '[%1] Skipped import because of existing record matched on primary key %2 for %3 by policy %4',
                                                $vs_idno,
                                                $vn_mapped_primary_key_value,
                                                $t_subject->tableName(),
                                                $vs_existing_record_policy
                            ...
                            case 'overwrite_on_id':
                                ...
                                if (is_array($va_ids) && (sizeof($va_ids) > 0)) {
                                    ...
                                    if ($t_subject->numErrors()) {
                                        $this->logImportError(
                                            _t(
                                                '[%1] Could not delete existing record matched on primary key %2 for %3 by policy %4',
                                                $vs_idno,
                                                $vn_mapped_primary_key_value,
                                                $t_subject->tableName(),
                                                $vs_existing_record_policy
                                        ...
                                        if ($log_erp) {
                                            $o_log->logInfo(
                                                _t(
                                                    '[%1] Overwrote existing record matched on primary key %2 for %3 by policy %4',
                                                    $vs_idno,
                                                    $vn_mapped_primary_key_value,
                                                    $t_subject->tableName(),
                                                    $vs_existing_record_policy
                                ...
                                if (!$vb_idno_is_template) {
                                    ...
                                    if (is_array($va_ids) && (sizeof($va_ids) > 0)) {
                                        if ($log_erp) {
                                            $o_log->logInfo(
                                                _t(
                                                    '[%1] Skipped import because of existing record matched on identifier by policy %2',
                                                    $vs_idno,
                                                    $vs_existing_record_policy
                                                )
                                            );
                            ...
                            case 'skip_on_preferred_labels':
                                ...
                                if (is_array($va_ids) && (sizeof($va_ids) > 0)) {
                                    if ($log_erp) {
                                        $o_log->logInfo(
                                            _t(
                                                '[%1] Skipped import because of existing record matched on label by policy %2',
                                                $vs_idno,
                                                $vs_existing_record_policy
                                            )
                                        );
                                ...
                                if (!$vb_idno_is_template) {
                                    ...
                                    if (is_array($va_ids) && (sizeof($va_ids) > 0)) {
                                        ...
                                        if ($log_erp) {
                                            $o_log->logInfo(
                                                _t(
                                                    '[%1] Merged with existing record matched on identifer by policy %2',
                                                    $vs_idno,
                                                    $vs_existing_record_policy
                                                )
                                            );
                                        ...
                                        if ($vs_existing_record_policy === 'merge_on_idno_with_skip') {
                                            if ($log_erp) {
                                                $o_log->logInfo(
                                                    _t(
                                                        '[%1] Skipped import because existing record could not be matched on identifier by policy %2 using base criteria %3',
                                                        $vs_idno,
                                                        $vs_existing_record_policy,
                                            ...
                                            if ($log_erp) {
                                                $o_log->logInfo(
                                                    _t(
                                                        '[%1] Could not match existing record on identifer by policy %2 using base criteria %3',
                                                        $vs_idno,
                                                        $vs_existing_record_policy,
                                                        print_r($va_base_criteria, true)
                                                    )
                            ...
                            case 'merge_on_preferred_labels_with_replace':
                                ...
                                if (is_array($va_ids) && (sizeof($va_ids) > 0)) {
                                    ...
                                    if ($log_erp) {
                                        $o_log->logInfo(
                                            _t(
                                                '[%1] Merged with existing record matched on label by policy %2',
                                                $vs_idno,
                                                $vs_existing_record_policy
                                            )
                                        );
                                    ...
                                    if ($log_erp) {
                                        $o_log->logInfo(
                                            _t(
                                                '[%1] Could not match existing record on label values %3 by policy %2 using base criteria %4',
                                                $vs_idno,
                                                $vs_existing_record_policy,
                                ...
                                if (!$vb_idno_is_template && $vs_idno) {
                                    ...
                                    if (is_array($va_ids) && (sizeof($va_ids) > 0)) {
                                        ...
                                        if ($t_subject->numErrors()) {
                                            $this->logImportError(
                                                _t(
                                                    '[%1] Could not delete existing record matched on identifier by policy %2',
                                                    $vs_idno,
                                                    $vs_existing_record_policy
                                                ),
                                                $va_log_import_error_opts
                                            ...
                                            if ($log_erp) {
                                                $o_log->logInfo(
                                                    _t(
                                                        '[%1] Overwrote existing record matched on identifier by policy %2',
                                                        $vs_idno,
                                                        $vs_existing_record_policy
                                                    )
                                                );
                            ...
                            case 'overwrite_on_preferred_labels':
                                ...
                                if (is_array($va_ids) && (sizeof($va_ids) > 0)) {
                                    ...
                                    if ($t_subject->numErrors()) {
                                        $this->logImportError(
                                            _t(
                                                '[%1] Could not delete existing record matched on label by policy %2',
                                                $vs_idno,
                                                $vs_existing_record_policy
                                            ),
                                            $va_log_import_error_opts
                                        ...
                                        if ($log_erp) {
                                            $o_log->logInfo(
                                                _t(
                                                    '[%1] Overwrote existing record matched on label by policy %2',
                                                    $vs_idno,
                                                    $vs_existing_record_policy
                                                )
                                            );
                    ...
                    if ($merge_only && !$t_subject->getPrimaryKey()) {
                        if ($log_erp) {
                            $o_log->logInfo(
                                _t(
                                    '[%1] Skipping because mergeOnly option is set and could not match existing record by policy %2.',
                                    $vs_idno,
                                    $vs_existing_record_policy
                                )
                            );
                    ...
                    if ($po_request && isset($pa_options['progressCallback']) && ($ps_callback = $pa_options['progressCallback'])) {
                        $ps_callback(
                            ...
                            _t(
                                "[%3/%4] Processing %1 (%2)",
                                caTruncateStringWithEllipsis($vs_display_label, 50),
                                $vs_idno,
                                $this->num_records_processed,
                                $vn_num_items
                            ),
                    ...
                    foreach ($va_items_by_group as $vn_group_id => $va_items) {
                        ...
                        foreach ($va_items as $vn_item_id => $va_item) {
                            ...
                            if ($vb_use_as_single_value = caGetOption(
                                ...
                                $va_vals = [
                                    ca_data_importers::getValueFromSource(
                                        $va_item,
                                        $o_reader,
                                        [
                                            'otherValues' => $va_rule_set_values,
                                ...
                                $va_vals = ca_data_importers::getValueFromSource(
                                    $va_item,
                                    $o_reader,
                                    array(
                                        'otherValues' => $va_rule_set_values,
                            ...
                            foreach ($va_vals as $vn_i => $vm_val) {
                                    ...
                                    foreach ($va_item['settings']['skipWhenEmpty'] as $v) {
                                        if (!strlen(
                                            ...
                                            if ($log_skip) {
                                                $o_log->logInfo(
                                                    _t(
                                                        '[%1] Skipped mapping for %2 because at least one value in list (%3) is empty',
                                                        $vs_idno,
                                                        $va_item['destination'],
                                ...
                                if (isset($va_item['settings']['skipWhenAllEmpty']) && is_array(
                                    ...
                                    if ($all_empty) {
                                        if ($log_skip) {
                                            $o_log->logInfo(
                                                _t(
                                                    '[%1] Skipped mapping for %2 because all values in list (%3) are empty',
                                                    $vs_idno,
                                                    $vn_row,
                                                    join('; ', $va_item['settings']['skipWhenEmpty'])
                                                )
                                    ...
                                    foreach ($va_item['settings']['skipGroupWhenEmpty'] as $v) {
                                        if (!strlen(
                                            ...
                                            if ($log_skip) {
                                                $o_log->logInfo(
                                                    _t(
                                                        '[%1] Skipped group %2 because at least one value in list (%3) is empty',
                                                        $vs_idno,
                                                        $vn_group_id,
                                ...
                                if ($va_item['settings']['skipGroupWhenAllEmpty'] && !is_array(
                                ...
                                if (isset($va_item['settings']['skipGroupWhenAllEmpty']) && is_array(
                                        $va_item['settings']['skipGroupWhenAllEmpty']
                                    )) {
                                    $all_empty = true;
                                    ...
                                    if ($all_empty) {
                                        if ($log_skip) {
                                            $o_log->logInfo(
                                                _t(
                                                    '[%1] Skipped group %2 because all values in list (%3) are empty',
                                                    $vs_idno,
                                                    $vn_group_id,
                                    ...
                                    foreach ($va_item['settings']['skipRowWhenEmpty'] as $v) {
                                        if (!strlen(
                                            ...
                                            if ($log_skip) {
                                                $o_log->logInfo(
                                                    _t(
                                                        '[%1] Skipped row %2 because at least one value in list (%3) is empty',
                                                        $vs_idno,
                                                        $vn_row,
                                ...
                                if (isset($va_item['settings']['skipRowWhenAllEmpty']) && is_array(
                                    ...
                                    if ($all_empty) {
                                        if ($log_skip) {
                                            $o_log->logInfo(
                                                _t(
                                                    '[%1] Skipped row %2 because all values in list (%3) are empty',
                                                    $vs_idno,
                                                    $vn_row,
                            ...
                            foreach ($va_vals as $vn_i => $vm_val) {
                                ...
                                if (isset($va_item['settings']['delimiter']) && $va_item['settings']['delimiter']
                                    ...
                                    if (sizeof($va_item['settings']['delimiter'])) {
                                        ...
                                        $va_val_list = preg_split(
                                            "!(" . join("|", $va_item['settings']['delimiter'])
                                            . ")!",
                                            $vm_val
                                        );
                                        ...
                                        foreach ($va_val_list as $vs_list_val) {
                                            $vs_list_val = trim(
                                                ca_data_importers::replaceValue(
                                                    $vs_list_val,
                                                    $va_item,
                                                    ['log' => $o_log, 'logReference' => $vs_idno]
                                                )
                    ...
                    $o_log->logDebug(
                        _t(
                            'Finished building content tree for %1 at %2 seconds [%3]',
                            $vs_idno,
                            $t->getTime(4),
                            $vn_row
                        )
                    ...
                    //die("END\n\n");
                    //continue;
                    if (!($opa_app_plugin_manager->hookDataImportContentTree(
                        array(
                            'mapping' => $t_mapping,
                                ...
                                if ($vs_element == 'parent_id') {
                                    if (($vn_parent_id = (int)$va_element_value['parent_id']) > 0) {
                                        if ($t_subject->load($vn_parent_id)) {
                                            $va_content_tree[$vs_subject_table][$vn_i]['parent_id']['parent_id'] = $t_subject->get(
                                                'parent_id'
                                            );
                                            $va_content_tree[$vs_subject_table][$vn_i]['idno']['idno'] = $t_subject->get(
                                                'idno'
                                            );
                                        }
                                    }
                    ...
                    if (!$t_subject->getPrimaryKey()) {
                        ...
                        foreach ($va_mandatory_field_mapping_ids as $vs_mandatory_field => $vn_mandatory_mapping_item_id) {
                            ...
                            if ($va_field_info['FIELD_TYPE'] == FT_MEDIA) {
                                $va_opts['original_filename'] = basename(
                                    $va_mandatory_field_values[$vs_mandatory_field]
                                );
                            }
                        ...
                        try {
                            ...
                            $this->logImportError(
                                ...
                                array_merge(
                                    ...
                                    [
                                        'bundle' => null,
                                        'notes' => _t('Exception while performing initial insert')
                                    ]
                                )
                            );
                        ...
                        if ($vs_error = DataMigrationUtils::postError(
                            $t_subject,
                            _t(
                                "[%1] Could not insert new record for %2 (type was %3): ",
                                $vs_idno,
                                $t_subject->getProperty('NAME_SINGULAR'),
                                $vs_type
                            ),
                        ...
                        $o_log->logDebug(
                            _t(
                                'Created idno %1 at %2 seconds [id=%3]',
                                $vs_idno,
                                $t->getTime(4),
                                $t_subject->getPrimaryKey()
                            )
                        ...
                        if ($vs_error = DataMigrationUtils::postError(
                            $t_subject,
                            _t(
                                "[%1] Could not update matched record",
                                $vs_idno
                            ),
                            __CA_DATA_IMPORT_ERROR__,
                            array(
                        ...
                        if (sizeof($va_preferred_label_mapping_ids) && ($t_subject->getPreferredLabelCount(
                                ) > 0) && (!$vb_was_preferred_label_match)) {
                            $vb_remove_labels = true;

                            $pref_label_data = array_filter(
                                $va_content_tree[$vs_subject_table],
                                function ($v) {
                            ...
                            if ($vb_remove_labels) {
                                ...
                                if ($vs_error = DataMigrationUtils::postError(
                                    $t_subject,
                                    _t(
                                        "[%1] Could not remove preferred labels from matched record",
                                        $vs_idno
                                    ),
                                    __CA_DATA_IMPORT_ERROR__,
                                    array(
                        ...
                        if (sizeof($va_nonpreferred_label_mapping_ids) && ($t_subject->getNonPreferredLabelCount(
                                ) > 0)) {
                            if (!preg_match("!^merge!", $vs_existing_record_policy)) {
                                ...
                                $t_subject->removeAllLabels(
                                    __CA_LABEL_TYPE_NONPREFERRED__,
                                    ['locales' => [$vn_locale_id]]
                                );
                                if ($vs_error = DataMigrationUtils::postError(
                                    $t_subject,
                                    _t(
                                        "[%1] Could not remove nonpreferred labels from matched record",
                                        $vs_idno
                                    ),
                                    __CA_DATA_IMPORT_ERROR__,
                                    array(
                    ...
                    $o_log->logDebug(
                        _t(
                            'Started insert of content tree for idno %1 at %2 seconds [id=%3]',
                            $vs_idno,
                            $t->getTime(4),
                            $t_subject->getPrimaryKey()
                        )
                        ...
                        if ($vs_table_name == $vs_subject_table) {
                            foreach ($va_content as $vn_i => $va_element_data) {
                                foreach ($va_element_data as $vs_element => $va_element_content) {
                                    $o_log->logDebug(
                                        _t(
                                            'Started insert of %1.%2 for idno %3 at %4 seconds [id=%3]',
                                            $vs_table_name,
                                            $vs_element,
                                            $vs_idno,
                                            $t->getTime(4),
                                    ...
                                    try {
                                        if (is_array($va_element_content)) {
                                            $vb_truncate_long_labels = caGetOption(
                                                '_truncateLongLabels',
                                                $va_element_content,
                                                false
                                            );
                                            ...
                                            $vb_treat_numeric_value_as_id = caGetOption(
                                                '_treatNumericValueAsID',
                                                $va_element_content,
                                                false
                                            );
                                        ...
                                        switch ($vs_element) {
                                            case 'preferred_labels':
                                                if (!$vb_was_preferred_label_match) {
                                                    if (
                                                        (!isset(
                                                                $va_element_content[$vs_disp_field = $t_subject->getLabelDisplayField(
                                                                )]
                                                            ) || !strlen($va_element_content[$vs_disp_field]))
                                                        &&
                                                        (sizeof(
                                                                array_filter(
                                                                    $t_subject->getSecondaryLabelDisplayFields(),
                                                                    function ($v) use ($va_element_content) {
                                                                        return isset($va_element_content[$v]) && strlen(
                                                                                $va_element_content[$v]
                                                                            );
                                                                    }
                                                                )
                                                            ) == 0)
                                                    ...
                                                    if ($vb_skip_if_data_present && ($t_subject->getLabelCount(
                                                                true,
                                                                $vn_locale_id
                                                            ) > 0)) {
                                                        continue(2);
                                                    ...
                                                    if ($vs_error = DataMigrationUtils::postError(
                                                        $t_subject,
                                                        _t(
                                                            "[%1] Could not add preferred label to %2. Record was deleted because no preferred label could be applied: ",
                                                            $vs_idno,
                                                            $t_subject->tableName()
                                                        ),
                                                        __CA_DATA_IMPORT_ERROR__,
                                                        ...
                                                        if ($vs_import_error_policy == 'stop') {
                                                            $o_log->logAlert(
                                                                _t(
                                                                    'Import stopped due to import error policy %1',
                                                        ...
                                                        if ($vs_item_error_policy == 'stop') {
                                                            $o_log->logAlert(
                                                                _t('Import stopped due to mapping error policy')
                                                            );
                                            ...
                                            case 'nonpreferred_labels':
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    $t_subject,
                                                    _t(
                                                        "[%1] Could not add non-preferred label to %2:",
                                                        $vs_idno,
                                                        $t_subject->tableName()
                                                    ),
                                                    __CA_DATA_IMPORT_ERROR__,
                                            ...
                                            case '_children':    // child records (usually generated by a refinery)
                                                ...
                                                break;
                                            default:
                                                if ($t_subject->hasField(
                                                        $vs_element
                                                    ) && (($vs_element != $t_subject->primaryKey(
                                                                true
                                                            )) && ($vs_element != $t_subject->primaryKey()))) {
                                                    if ($vb_skip_if_data_present && strlen(
                                                            $t_subject->get($vs_element)
                                                        )) {
                                                        continue(2);
                                                    }
                                                    ...
                                                    $va_opts = array(
                                                        ...
                                                        'assumeIdnoStubForLotID' => !isset($va_element_content['_matchOn']) && !$vb_treat_numeric_value_as_id,
                                                    ...
                                                    $vs_content = (is_array($va_element_content) && array_key_exists(
                                                            $vs_element,
                                                            $va_element_content
                                                        )) ? $va_element_content[$vs_element] :
                                                    ...
                                                    if ($vs_error = DataMigrationUtils::postError(
                                                        $t_subject,
                                                        _t(
                                                            "[%1] Could not set intrinsic %2 to %3:",
                                                            $vs_idno,
                                                            $vs_element,
                                                            $t_subject->tableName()
                                                        ),
                                                    ...
                                                    if ($vs_error = DataMigrationUtils::postError(
                                                        $t_subject,
                                                        _t(
                                                            "[%1] Could not add intrinsic %2 to %3:",
                                                            $vs_idno,
                                                            $vs_element,
                                                            $t_subject->tableName()
                                                        ),
                                                    ...
                                                    try {
                                                        if ((is_array(
                                                                    $va_rel_info = Datamodel::getManyToOneRelations(
                                                                        $vs_table_name,
                                                                        $vs_element
                                                                    )
                                                                ) && (sizeof($va_rel_info) > 0)) && is_array(
                                                                $va_element_data[$vs_element]
                                                            ) && is_array(
                                                                $va_element_data[$vs_element]['_related_related']
                                                            ) && sizeof(
                                                                $va_element_data[$vs_element]['_related_related']
                                                            )) {
                                                            foreach ($va_element_data[$vs_element]['_related_related'] as $vs_rel_rel_table => $va_rel_rels) {
                                                                foreach ($va_rel_rels as $vn_i => $va_rel_rel) {
                                                                    if (!($t_rel_instance = Datamodel::getInstance(
                                                                        $va_rel_info['one_table']
                                                                    ))) {
                                                                        $o_log->logWarn(
                                                                            _t(
                                                                                "[%1] Could not instantiate related table %2",
                                                                                $vs_idno,
                                                                                $vs_table_name
                                                                            )
                                                                        );
                                                                    ...
                                                                    if ($t_rel_instance->load(
                                                                        $va_element_content[$vs_element]
                                                                    )) {
                                                                        if ($t_rel_rel = $t_rel_instance->addRelationship(
                                                                            $vs_rel_rel_table,
                                                                            $va_rel_rel['id'],
                                                                            ...
                                                                            if ($log_relations) {
                                                                                $o_log->logInfo(
                                                                                    _t(
                                                                                        '[%1] Related %2 (%3) to related %4 with relationship %5',
                                                                                        $vs_idno,
                                                                                        Datamodel::getTableProperty(
                                                                                            $vs_rel_rel_table,
                                                                                            'NAME_SINGULAR'
                                                                                        ),
                                                                                        $va_rel_rel['id'],
                                                                                        $t_rel_instance->getProperty(
                                                                                            'NAME_SINGULAR'
                                                                                        ),
                                                                                        trim(
                                                                                            $va_rel_rel['_relationship_type']
                                                                            ...
                                                                            if ($vs_error = DataMigrationUtils::postError(
                                                                                $t_subject,
                                                                                _t(
                                                                                    "[%1] Could not add related %2 (%3) to related %4 with relationship %5:",
                                                                                    $vs_idno,
                                                                                    Datamodel::getTableProperty(
                                                                                        $vs_rel_rel_table,
                                                                                        'NAME_SINGULAR'
                                                ...
                                                if (($vs_subject_table == 'ca_representation_annotations') && ($vs_element == 'properties')) {
                                                    foreach ($va_element_content as $vs_prop => $vs_prop_val) {
                                                        if ($vb_skip_if_data_present && strlen(
                                                                $t_subject->getPropertyValue($vs_prop)
                                                            )) {
                                                            continue;
                                                        }
                                                ...
                                                if ($vb_skip_if_data_present && ($t_subject->getAttributeCountByElement(
                                                            $vs_element
                                                        ) > 0)) {
                                                    continue(2);
                                                }
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    $t_subject,
                                                    _t(
                                                        "[%1] Failed to add value for %2; values were %3: ",
                                                        $vs_idno,
                                                        $vs_element,
                                                        $errored_values = ca_data_importers::formatValuesForLog(
                                                            $va_element_content,
                                                            $vs_element
                                                        )
                                                    ),
                            ...
                            $o_log->logDebug(
                                _t(
                                    'Started insert of %1 for idno %2 at %3 seconds [id=%4]',
                                    $vs_table_name,
                                    $vs_idno,
                                    $t->getTime(4),
                                    $t_subject->getPrimaryKey()
                            ...
                            foreach ($va_content as $vn_i => $va_element_data) {
                                ...
                                if ($vs_table_name === 'ca_item_tags') {
                                    if (isset($va_element_data['ca_item_tags']['tag'])) {
                                        $t_subject->addTag(
                                            $va_element_data['ca_item_tags']['tag'],
                                            null,
                                            null,
                                            $va_element_data['ca_item_tags']['access'],
                                            null,
                                            ['forceModerated' => true]
                                        );
                                    } elseif (isset($va_element_data['ca_item_tags']) && is_string(
                                            $va_element_data['ca_item_tags']
                                        )) {
                                        $t_subject->addTag(
                                            $va_element_data['ca_item_tags'],
                                            null,
                                ...
                                $va_data_for_rel_table = array_merge(
                                    $va_data_for_rel_table,
                                    ca_data_importers::_extractIntrinsicValues(
                                        $va_data_for_rel_table,
                                        $vs_table_name
                                    )
                                );
                                ...
                                try {
                                    ...
                                    } elseif (isset($va_element_data) && !is_array($va_element_data) && strlen(
                                            $va_element_data
                                        )) {
                                    ...
                                    switch ($vs_table_name) {
                                        case 'ca_objects':
                                            if ($vn_rel_id = DataMigrationUtils::getObjectID(
                                                $vs_name,
                                                $va_element_data['_parent_id'],
                                                $va_element_data['_type'],
                                                $vn_locale_id,
                                                $va_data_for_rel_table,
                                                array(
                                                    'logReference' => $vs_idno,
                                                ...
                                                if (
                                                    ...
                                                    $o_log->logError(
                                                        _t('Relationship type is missing for ca_objects relationship')
                                                    );
                                                    break;
                                                }

                                                $t_subject->addRelationship(
                                                    $vs_table_name,
                                                    $vn_rel_id,
                                                    trim(
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    $t_subject,
                                                    _t(
                                                        "[%1] Could not add related object with relationship %2:",
                                                        $vs_idno,
                                                        trim(
                                                            $va_element_data['_relationship_type']
                                                        )
                                        ...
                                        case 'ca_object_lots':
                                            ...
                                            if ($vn_rel_id = DataMigrationUtils::getObjectLotID(
                                                ...
                                                if (!($vs_rel_type = $va_element_data['_relationship_type']) && !(is_array(
                                                            $va_element_data['idno_stub']
                                                        ) && ($vs_rel_type = $va_element_data['idno_stub']['_relationship_type']))) {
                                                    break;
                                                }
                                                $t_subject->addRelationship(
                                                    $vs_table_name,
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    $t_subject,
                                                    _t(
                                                        "[%1] Could not add related object lot %2 with relationship %3:",
                                                        $vs_idno,
                                                        $vs_idno_stub,
                                                        trim(
                                                            $va_element_data['_relationship_type']
                                        ...
                                        case 'ca_entities':
                                            if ($vn_rel_id = DataMigrationUtils::getEntityID(
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    $t_subject,
                                                    _t(
                                                        "[%1] Could not add related entity with relationship %2:",
                                                        $vs_idno,
                                                        trim(
                                                            $va_element_data['_relationship_type']
                                                        )
                                            ...
                                            break;
                                        case 'ca_places':
                                            if ($vn_rel_id = DataMigrationUtils::getPlaceID(
                                                $vs_name,
                                                $va_element_data['_parent_id'],
                                                $va_element_data['_type'],
                                                $vn_locale_id,
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    $t_subject,
                                                    _t(
                                                        "[%1] Could not add related place with relationship %2:",
                                                        $vs_idno,
                                                        trim(
                                                            $va_element_data['_relationship_type']
                                                        )
                                            ...
                                            break;
                                        case 'ca_collections':
                                            if ($vn_rel_id = DataMigrationUtils::getCollectionID(
                                                $vs_name,
                                                $va_element_data['_type'],
                                                $vn_locale_id,
                                                $va_data_for_rel_table,
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    $t_subject,
                                                    _t(
                                                        "[%1] Could not add related collection with relationship %2:",
                                                        $vs_idno,
                                                        $vs_rel_type
                                                    ),
                                                    __CA_DATA_IMPORT_ERROR__,
                                            ...
                                            break;
                                        case 'ca_occurrences':
                                            if ($vn_rel_id = DataMigrationUtils::getOccurrenceID(
                                                $vs_name,
                                                $va_element_data['_parent_id'],
                                                $va_element_data['_type'],
                                                $vn_locale_id,
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    $t_subject,
                                                    _t(
                                                        "[%1] Could not add related occurrence with relationship %2:",
                                                        $vs_idno,
                                                        $vs_rel_type
                                                    ),
                                                    __CA_DATA_IMPORT_ERROR__,
                                        ...
                                        case 'ca_storage_locations':
                                            if ($vn_rel_id = DataMigrationUtils::getStorageLocationID(
                                                $vs_name,
                                                $va_element_data['_parent_id'],
                                                $va_element_data['_type'],
                                                $vn_locale_id,
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    $t_subject,
                                                    _t(
                                                        "[%1] Could not add related storage location with relationship %2:",
                                                        $vs_idno,
                                                        trim(
                                                            $va_element_data['_relationship_type']
                                                        )
                                        ...
                                        case 'ca_list_items':
                                            ...
                                            if ($vn_rel_id = DataMigrationUtils::getListItemID(
                                                $va_element_data['_list'],
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    $t_subject,
                                                    _t(
                                                        "[%1] Could not add related list item with relationship %2:",
                                                        $vs_idno,
                                                        trim(
                                                            $va_element_data['_relationship_type']
                                                        )
                                        ...
                                        case 'ca_object_representations':
                                            if ($vn_rel_id = DataMigrationUtils::getObjectRepresentationID(
                                                $vs_name,
                                                $va_element_data['_type'],
                                                $vn_locale_id,
                                                $va_data_for_rel_table,
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    $t_subject,
                                                    _t(
                                                        "[%1] Could not add related object representation with:",
                                                        $vs_idno
                                                    ),
                                                    __CA_DATA_IMPORT_ERROR__,
                                                    array(
                                            ...
                                            break;
                                        case 'ca_loans':
                                            if ($vn_rel_id = DataMigrationUtils::getLoanID(
                                                $vs_name,
                                                $va_element_data['_type'],
                                                $vn_locale_id,
                                                $va_data_for_rel_table,
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    $t_subject,
                                                    _t(
                                                        "[%1] Could not add related loan with relationship %2:",
                                                        $vs_idno,
                                                        trim(
                                                            $va_element_data['_relationship_type']
                                                        )
                                            ...
                                            break;
                                        case 'ca_movements':
                                            if ($vn_rel_id = DataMigrationUtils::getMovementID(
                                                $vs_name,
                                                $va_element_data['_type'],
                                                $vn_locale_id,
                                                $va_data_for_rel_table,
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    $t_subject,
                                                    _t(
                                                        "[%1] Could not add related movement with relationship %2:",
                                                        $vs_idno,
                                                        trim(
                                                            $va_element_data['_relationship_type']
                                                        )
                                    ...
                                    // noop
                                    $o_log->logError(
                                        _t('Error while processing related content: %1', $e->getMessage())
                                    );
                                }
                                try {
                                    if (is_array($va_element_data['_related_related']) && sizeof(
                                            $va_element_data['_related_related']
                                        )) {
                                    ...
                                    // noop
                                    $o_log->logError(
                                        _t('Error while processing related-to-related content: %1', $e->getMessage())
                                    );
                                }
                            }
                        }
                    ...
                    $o_log->logDebug(
                        _t(
                            'Finished inserting content tree for %1 at %2 seconds into database',
                            $vs_idno,
                            $t->getTime(4)
                        )
                    );
                    ...
                    if (!$vb_output_subject_preferred_label && ($t_subject->getPreferredLabelCount() == 0)) {
                        ...
                        if ($vs_error = DataMigrationUtils::postError(
                            $t_subject,
                            _t("[%1] Could not add default label", $vs_idno),
                            __CA_DATA_IMPORT_ERROR__,
                            array(
                                'dontOutputLevel' => true,
                                'dontPrint' => true
                    ...
                    $opa_app_plugin_manager->hookDataPostImport(
                        array(
                            'subject' => $t_subject,
                            'mapping' => $t_mapping,
                            'content_tree' => &$va_content_tree,
                    ...
                    if ($log_general) {
                        $o_log->logInfo(
                            _t('[%1] Imported %2 as %3 ', $vs_idno, $vs_preferred_label_for_log, $vs_subject_table_name)
                        );
                    }
                    $o_event->endItem(
                        $t_subject->getPrimaryKey(),
                    ...
                    if ($t_add_to_set) {
                        if (!$t_add_to_set->addItem($t_subject->getPrimaryKey(), null, $pa_options['user_id'])) {
                            $o_log->logError(
                                _t(
                                    'Could not add imported item %1 to set %2: %3',
                                    $vs_idno,
                                    $ps_add_to_set,
                                    join('; ', $t_add_to_set->getErrors())
                                )
                            );
                        }
                    }
        ...
        if ($log_general) {
            $o_log->logInfo(
                _t(
                    'Import of %1 completed using mapping %2: %3 imported/%4 skipped/%5 errors',
                    $ps_source,
    ...
    private function _processChildren($parent_table, $parent_id, $children, $options = null)
    {
        ...
        foreach ($children as $child) {
            ...
            if ($parent_table == $child['_table']) {    // direct child to parent (both are same table)
                $id = DataMigrationUtils::getIDFor(
                    $child['_table'],
                    $child['preferred_labels'],
                    $parent_id,
                    $child['_type'],
                    1,
                    $attrs,
                ...
                ) && ($parent_table == 'ca_collections') && ($child['_table'] == 'ca_objects')) {    // collection-object hierarchy
                if (($id = DataMigrationUtils::getIDFor(
                        $child['_table'],
                        $child['preferred_labels'],
                        null,
                        $child['_type'],
                        ...
                        $attrs,
                        $options
                    )) && ($parent = Datamodel::getInstance($parent_table, true)) && $parent->load($parent_id)) {
                    if ($trans) {
                    ...
                    if ($parent->numErrors() > 0) {
                        if ($log) {
                            $log->logInfo(
                                _t(
                                    'Could not create object-collection hierarchy child record relationship: %1',
    ...
    public static function getValueFromSource($pa_item, $po_reader, $pa_options = null)
    {
        ...
        $va_filter_to_types = caGetOption(
            'filterToTypes',
            $pa_options,
            false
        );    // supported by CollectiveAccessDataReader
        $va_filter_to_relationship_types = caGetOption(
            'filterToRelationshipTypes',
            $pa_options,
            false
        );    // supported by CollectiveAccessDataReader
    ...
    public static function replaceValue($pm_value, $pa_item, $pa_options = null)
    {
        if (strlen($pm_value) && is_array($pa_item['settings']['original_values'])) {
            if (($vn_index = array_search(
                    trim(mb_strtolower($pm_value)),
                    $pa_item['settings']['original_values']
                )) !== false) {
                ...
                if ($o_log = caGetOption('log', $pa_options, null)) {
                    ...
                    if ($pm_value !== $vs_replaced_value) {
                        $o_log->logInfo(
                            _t(
                                'Replaced value "%1" (%2) with "%3" (%4) for %5 using replacement values',
                                $vs_original_display_value,
                                $pm_value,
        ...
        if (is_array($regexes)) {
            foreach ($regexes as $regex_index => $regex_info) {
                ...
                $regex = "!" . str_replace(
                        ...
                        "\\!",
                        $regex_info['match']
                    ) . "!u" . ((isset($regex_info['caseSensitive']) && (bool)$regex_info['caseSensitive']) ? '' : 'i');
                ...
                foreach ($matches as $mi => $m) {
                    ...
                    if ($o_reader->valuesCanRepeat()) {
                        $row_with_replacements[$item['source'] . ":{$mi}"][$index] = $row[$item['source' . ":{$mi}"]][$index] = $row[mb_strtolower(
                            $item['source'] . ":{$mi}"
                        )][$index] = $m;
                    } else {
                        $row_with_replacements[$item['source'] . ":{$mi}"] = $row[$item['source' . ":{$mi}"]] = $row[mb_strtolower(
                            $item['source'] . ":{$mi}"
                        )] = $m;
                    }
                }
