...
class ca_data_importers extends BundlableLabelableBaseModelWithAttributes
{
    ...
    protected function initSettings()
    {
        ...
        $va_settings['archiveMapping'] = array(
            ...
            'description' => _t(
                'Set to yes to save the mapping spreadsheet; no to delete it from the server after import.  Pending implementation.'
            )
        );
    ...
    public static function loadImporterFromFile($ps_source, &$pa_errors, $pa_options = null)
    {
        ...
        if (!($o_log = (is_writable($pa_options['logDirectory'])) ? new KLogger(
            $pa_options['logDirectory'],
            $pa_options['logLevel']
        ) : null)) {
            throw new ApplicationException(
                _t(
                    "Cannot write log to %1. Please check the directory's permissions and retry loading.",
                    $pa_options['logDirectory']
                )
            );
        }
                ...
                case 'constant':
                    ...
                    if ($vs_options_json = (string)$o_options->getValue()) {
                        ...
                        if (is_null($va_options = @json_decode($vs_options_json, true))) {
                            ...
                            $pa_errors[] = _t(
                                "Warning: invalid json in \"options\" column for group %1/source %2. Json was: %3",
                                $vs_group,
                                $vs_source,
                                $vs_options_json
                            );
                            if ($o_log) {
                                $o_log->logWarn(
                                    _t(
                                        "[loadImporterFromFile:%1] Invalid json in \"options\" column for group %2/source %3. Json was: %4.",
                                        $ps_source,
                                        $vs_group,
                                        $vs_source,
                                        $vs_options_json
                    ...
                    if ($vs_refinery && ($vs_refinery_options_json = (string)$o_refinery_options->getValue())) {
                        if (!in_array($vs_refinery, $va_refineries)) {
                            ...
                            if (is_null($va_refinery_options = json_decode($vs_refinery_options_json, true))) {
                                // Error while json decode
                                $pa_errors[] = _t(
                                    "invalid json for refinery options for group %1/source %2 = %3",
                                    $vs_group,
                                    $vs_source,
                                ...
                                if ($o_log) {
                                    $o_log->logError(
                                        _t(
                                            "[loadImporterFromFile:%1] invalid json for refinery options for group %2/source %3 = %4",
                                            $ps_source,
                                            $vs_group,
                                            $vs_source,
                                            $vs_refinery_options_json
                        ...
                        if ($o_opt_sheet = $o_excel->getSheetByName($va_options['transformValuesUsingWorksheet'])) {
                            foreach ($o_opt_sheet->getRowIterator() as $o_sheet_row) {
                                if (!$vs_original_value = trim(
                                    mb_strtolower(
                                        (string)$o_opt_sheet->getCellByColumnAndRow(0, $o_sheet_row->getRowIndex())
                                    )
                                )) {
                                    continue;
                                }
    ...
    static public function importDataFromSource($ps_source, $ps_mapping, $pa_options = null)
    {
        ...
        if ($ps_add_to_set = caGetOption('addToSet', $pa_options, null)) {
            if (!($t_add_to_set = ca_sets::find(
                ...
                // Set doesn't exist
                ca_data_importers::logImportError(
        ...
        if (!($o_log = (is_writable($pa_options['logDirectory'])) ? new KLogger(
            ...
            if (!caGetOption('logToTempDirectoryIfLogDirectoryIsNotWritable', $pa_options, false)) {
                ca_data_importers::logImportError(
                    _t(
                        "Cannot write log to %1. Please check the directory permissions and retry the import.",
                        $pa_options['logDirectory']
                    )
                );
                return false;
                ...
                ca_data_importers::logImportError(
                    _t(
                        "Cannot write log to %1. Writing log to %2 instead.",
                        $vs_original_log_directory,
                ...
                if (!($o_log = (is_writable($pa_options['logDirectory'])) ? new KLogger(
                    ...
                    ca_data_importers::logImportError(
                        _t(
                            "Cannot write log to either %1 or %2. Please check the directory permissions and retry the import.",
                            $vs_original_log_directory,
                            $pa_options['logDirectory']
                        )
                    );
                    ...
                    if (isset($va_mapping_items[$vn_idno_mapping_item_id]['settings']['applyRegularExpressions']) && is_array(
                        ...
                        if (is_array(
                            ...
                            foreach ($va_mapping_items[$vn_idno_mapping_item_id]['settings']['applyRegularExpressions'] as $vn_regex_index => $va_regex) {
                                ...
                                $vs_idno = preg_replace(
                                    "!" . str_replace(
                                        "!",
                                        "\\!",
                                        $va_regex['match']
                                    ) . "!" . ((isset($va_regex['caseSensitive']) && (bool)$va_regex['caseSensitive']) ? '' : 'i'),
                                    $va_regex['replaceWith'],
                                    $vs_idno
                                );
                            }
                        }
                ...
                foreach ($va_idnos_for_row as $vs_idno) {
                            ...
                            case 'skip_on_preferred_labels':
                                $va_ids = call_user_func_array(
                                    ...
                                    array(
                                        array_merge(
                                            $va_base_criteria,
                                            array('preferred_labels' => $va_pref_label_values, 'deleted' => 0)
                                        ),
                                        array(
                                            'returnAs' => 'ids',
                                            'purifyWithFallback' => true,
                    ...
                    foreach ($va_items_by_group as $vn_group_id => $va_items) {
                        ...
                        foreach ($va_items as $vn_item_id => $va_item) {
                            ...
                            if (isset($va_item['settings']['filterEmptyValues']) && (bool)$va_item['settings']['filterEmptyValues'] && is_array(
                                    $va_vals
                                )) {
                            ...
                            foreach ($va_vals as $vn_i => $vm_val) {
                                ...
                                if (isset($va_item['settings']['skipRowIfEmpty']) && (bool)$va_item['settings']['skipRowIfEmpty'] && !strlen(
                                        $vm_val
                                    )) {
                                    $o_log->logInfo(
                                        _t(
                                ...
                                if (isset($va_item['settings']['skipGroupIfEmpty']) && (bool)$va_item['settings']['skipGroupIfEmpty'] && !strlen(
                                        $vm_val
                                    )) {
                                    $o_log->logInfo(
                                        _t(
                                ...
                                if (isset($va_item['settings']['skipIfEmpty']) && (bool)$va_item['settings']['skipIfEmpty'] && !strlen(
                                        $vm_val
                                    )) {
                                    $o_log->logInfo(
                                        _t(
                                ...
                                if ($va_item['settings']['skipIfValue'] && !is_array(
                                        $va_item['settings']['skipIfValue']
                                    )) {
                                    $va_item['settings']['skipIfValue'] = array($va_item['settings']['skipIfValue']);
                                }
                                if (isset($va_item['settings']['skipIfValue']) && is_array(
                                        $va_item['settings']['skipIfValue']
                                    ) && strlen($vm_val) && in_array($vm_val, $va_item['settings']['skipIfValue'])) {
                                    $o_log->logInfo(
                                        _t(
                                ...
                                if ($va_item['settings']['skipIfNotValue'] && !is_array(
                                        $va_item['settings']['skipIfNotValue']
                                    )) {
                                    $va_item['settings']['skipIfNotValue'] = array($va_item['settings']['skipIfNotValue']);
                                }
                                if (isset($va_item['settings']['skipIfNotValue']) && is_array(
                                        $va_item['settings']['skipIfNotValue']
                                    ) && strlen($vm_val) && !in_array(
                                        $vm_val,
                                        $va_item['settings']['skipIfNotValue']
                                    )) {
                                    $o_log->logInfo(
                                ...
                                if ($va_item['settings']['skipRowIfValue'] && !is_array(
                                        $va_item['settings']['skipRowIfValue']
                                    )) {
                                    $va_item['settings']['skipRowIfValue'] = array($va_item['settings']['skipRowIfValue']);
                                }
                                if (isset($va_item['settings']['skipRowIfValue']) && is_array(
                                        $va_item['settings']['skipRowIfValue']
                                    ) && strlen($vm_val) && in_array($vm_val, $va_item['settings']['skipRowIfValue'])) {
                                    $o_log->logInfo(
                                        _t(
                                ...
                                if ($va_item['settings']['skipRowIfNotValue'] && !is_array(
                                        $va_item['settings']['skipRowIfNotValue']
                                    )) {
                                    $va_item['settings']['skipRowIfNotValue'] = array($va_item['settings']['skipRowIfNotValue']);
                                }
                                if (isset($va_item['settings']['skipRowIfNotValue']) && is_array(
                                        $va_item['settings']['skipRowIfNotValue']
                                    ) && strlen($vm_val) && !in_array(
                                        $vm_val,
                                        $va_item['settings']['skipRowIfNotValue']
                                ...
                                if ($va_item['settings']['skipGroupIfValue'] && !is_array(
                                        $va_item['settings']['skipGroupIfValue']
                                    )) {
                                    $va_item['settings']['skipGroupIfValue'] = array($va_item['settings']['skipGroupIfValue']);
                                }
                                if (isset($va_item['settings']['skipGroupIfValue']) && is_array(
                                        $va_item['settings']['skipGroupIfValue']
                                    ) && strlen($vm_val) && in_array(
                                        $vm_val,
                                        $va_item['settings']['skipGroupIfValue']
                                ...
                                if ($va_item['settings']['skipGroupIfNotValue'] && !is_array(
                                        $va_item['settings']['skipGroupIfNotValue']
                                    )) {
                                    $va_item['settings']['skipGroupIfNotValue'] = array($va_item['settings']['skipGroupIfNotValue']);
                                }
                                if (isset($va_item['settings']['skipGroupIfNotValue']) && is_array(
                                        $va_item['settings']['skipGroupIfNotValue']
                                    ) && strlen($vm_val) && !in_array(
                                        $vm_val,
                                        $va_item['settings']['skipGroupIfNotValue']
                                ...
                                if (isset($va_item['settings']['default']) && strlen(
                                        $va_item['settings']['default']
                                    ) && !strlen($vm_val)) {
                                    $vm_val = $va_item['settings']['default'];
                                }

                                // Apply prefix/suffix *AFTER* setting default
                                if ($vm_val && isset($va_item['settings']['prefix']) && strlen(
                                        $va_item['settings']['prefix']
                                    )) {
                                    $vm_val = $va_item['settings']['prefix'] . $vm_val;
                                }
                                if ($vm_val && isset($va_item['settings']['suffix']) && strlen(
                                ...
                                if (isset($va_item['settings']['formatWithTemplate']) && strlen(
                                ...
                                if (isset($va_item['settings']['applyRegularExpressions']) && is_array(
                                ...
                                if ($o_reader->valuesCanRepeat()) {
                                    $va_row_with_replacements[$va_item['source']][$vn_i] = $va_row[$va_item['source']][$vn_i] = $va_row[mb_strtolower(
                                        $va_item['source']
                                    )][$vn_i] = $vm_val;
                                } else {
                                    $va_row_with_replacements[$va_item['source']] = $va_row[$va_item['source']] = $va_row[mb_strtolower(
                                        $va_item['source']
                                    )] = $vm_val;
                                }
                            }
                            ...
                            foreach ($va_vals as $vn_i => $vm_val) {
                                ...
                                if (isset($va_item['settings']['convertNewlinesToHTML']) && (bool)$va_item['settings']['convertNewlinesToHTML'] && is_string(
                                        $vm_val
                                    )) {
                                    $vm_val = nl2br($vm_val);
                                }
                                if (isset($va_item['settings']['collapseSpaces']) && (bool)$va_item['settings']['collapseSpaces'] && is_string(
                                        $vm_val
                                    )) {
                                    $vm_val = preg_replace("![ ]+!", " ", $vm_val);
                                }

                                if (isset($va_item['settings']['restrictToTypes']) && is_array(
                                        $va_item['settings']['restrictToTypes']
                                ...
                                if (isset($va_item['settings']['skipRowIfExpression']) && strlen(
                                        trim($va_item['settings']['skipRowIfExpression'])
                                    )) {
                                    try {
                                        if ($vm_ret = ExpressionParser::evaluate(
                                            ...
                                            $o_log->logInfo(
                                                _t(
                                                    '[%1] Skipped row %2 because expression %3 is true',
                                                    $vs_idno,
                                                    $vn_row,
                                                    $va_item['settings']['skipRowIfExpression']
                                                )
                                ...
                                if (isset($va_item['settings']['skipGroupIfExpression']) && strlen(
                                        trim($va_item['settings']['skipGroupIfExpression'])
                                    )) {
                                    try {
                                        if ($vm_ret = ExpressionParser::evaluate(
                                            ...
                                            $o_log->logInfo(
                                                _t(
                                                    '[%1] Skipped group %2 because skipRowIfExpression %3 is true',
                                                    $vs_idno,
                                                    $vn_group_id,
                                                    $va_item['settings']['skipGroupIfExpression']
                                                )
                                ...
                                if (isset($va_item['settings']['skipIfExpression']) && strlen(
                                        trim($va_item['settings']['skipIfExpression'])
                                    )) {
                                    try {
                                        if ($vm_ret = ExpressionParser::evaluate(
                                ...
                                if (!isset($va_item['settings']['errorPolicy']) || !in_array(
                                ...
                                //
                                if (isset($va_item['settings']['relationshipType']) && strlen(
                                        $vs_rel_type = $va_item['settings']['relationshipType']
                                ...
                                if (preg_match("!^_CONSTANT_:[\d]+:(.*)!", $va_item['source'], $va_matches)) {
                                    ...
                                    if (($vs_target_table == $vs_subject_table_name) && (($vs_k = array_search(
                                                $vn_item_id,
                                ...
                                if (isset($va_item['settings']['refineries']) && is_array(
                                        $va_item['settings']['refineries']
                                    )) {
                                    foreach ($va_item['settings']['refineries'] as $vs_refinery) {
                                        ...
                                        if ($o_refinery = RefineryManager::getRefineryInstance($vs_refinery)) {
                                            ...
                                            if (!$va_refined_values || (is_array($va_refined_values) && !sizeof(
                                                        $va_refined_values
                                                    ))) {
                                                continue(2);
                                            }
                                            ...
                                            ca_data_importers::logImportError(
                                                _t('[%1] Invalid refinery %2 specified', $vs_idno, $vs_refinery)
                                            );
                                        }
                                    }
                                }
                                ...
                                $vn_max_length = (!is_array(
                                        $vm_val
                                    ) && isset($va_item['settings']['maxLength']) && (int)$va_item['settings']['maxLength']) ? (int)$va_item['settings']['maxLength'] : null;
                                ...
                                if (in_array('preferred_labels', $va_item_dest) || in_array(
                                        'nonpreferred_labels',
                                        $va_item_dest
                                    )) {
                                ...
                                switch ($vs_item_terminal) {
                                    case 'type_id':
                                        if (($vs_target_table == $vs_subject_table_name) && (sizeof(
                                                    $va_item_dest
                                                ) == 2)) {
                                            $vs_type = $vm_val;
                                        }
                    ...
                    if (!$t_subject->getPrimaryKey()) {
                        ...
                        if ($vs_error = DataMigrationUtils::postError(
                            $t_subject,
                            _t(
                                "Could not insert new record for %1: ",
                        ...
                        if ($vs_error = DataMigrationUtils::postError(
                            $t_subject,
                            _t("Could not update matched record"),
                            __CA_DATA_IMPORT_ERROR__,
                            array(
                                'dontOutputLevel' => true,
                                'dontPrint' => true
                        ...
                        if (sizeof($va_preferred_label_mapping_ids) && ($t_subject->getPreferredLabelCount(
                            ...
                            if ($vb_remove_labels) {
                                ...
                                if ($vs_error = DataMigrationUtils::postError(
                                    $t_subject,
                                    _t(
                                        "Could not remove preferred labels from matched record"
                                    ),
                                    __CA_DATA_IMPORT_ERROR__,
                                    array(
                                        'dontOutputLevel' => true,
                            ...
                            if (preg_match("!^merge!", $vs_existing_record_policy)) {
                                ...
                                if ($vb_remove_labels) {
                                    ...
                                    if ($vs_error = DataMigrationUtils::postError(
                                        $t_subject,
                                        _t(
                                            "Could not remove nonpreferred labels from matched record"
                                        ),
                                        __CA_DATA_IMPORT_ERROR__,
                                        array(
                                            'dontOutputLevel' => true,
                                    ...
                                    try {
                                        ...
                                        switch ($vs_element) {
                                            ...
                                            default:
                                                ...
                                                if (!isset($va_elements_set_for_this_record[$vs_element]) && !$va_elements_set_for_this_record[$vs_element] && in_array(
                                                        $vs_existing_record_policy,
                                                        array(
                                                            'merge_on_idno_with_replace',
                                                            'merge_on_preferred_labels_with_replace',
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    ...
                                                    _t(
                                                        ...
                                                        $vs_idno,
                                                        $vs_element,
                                                        ca_data_importers::formatValuesForLog(
                                                            $va_element_content
                                                        )
                                                    ),
                                                    __CA_DATA_IMPORT_ERROR__,
                                        ...
                                        case 'ca_object_lots':
                                            ...
                                            if ($vn_rel_id = DataMigrationUtils::getObjectLotID(
                                                ...
                                                if ($vs_error = DataMigrationUtils::postError(
                                                    $t_subject,
                                                    _t(
                                                        "[%1] Could not add related object lot with relationship %2:",
                                                        $vs_idno,
                                                        trim(
                                                            $va_element_data['_relationship_type']
                                                        )
        ...
        $o_progress->setDataForJobID(
            ...
            _t('Import complete'),
            [
                'numRecordsProcessed' => ca_data_importers::$s_num_records_processed,
                    ...
                    foreach ($vals['_related_related'] as $vs_rel_rel_table => $va_rel_rels) {
                        foreach ($va_rel_rels as $vn_i => $va_rel_rel) {
                            if (!($t_rel_instance = Datamodel::getInstance($child['_table']))) {
                                if ($log) {
                                    $log->logWarn(
                                        _t("[%1] Could not instantiate related table %2", $vs_idno, $child['_table'])
                                    );
                                }
                                continue;
                            }
                    ...
                    if ($pm_value !== $vs_replaced_value) {
                        $o_log->logInfo(
                            _t(
                                ...
                                $vs_replaced_value,
                                caGetOption('logIdno', $pa_options, '???')
                            )
                        );
                    }
                }
