...
BaseModel::$s_ca_models_definitions['ca_data_importers'] = array(
	...
	'FIELDS'        => array(
		'importer_id'   => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this importer' )
		),
		'importer_code' => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Importer code' ),
			'DESCRIPTION'    => _t( 'Unique alphanumeric identifier for this importer.' ),
			'UNIQUE_WITHIN'  => array()
		...
		'table_num'     => array(
			...
			'DESCRIPTION'        => _t( 'Indicates type of item importer is used for.' ),
		...
		'worksheet'     => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Importer worksheet' ),
			'DESCRIPTION'    => _t( 'Archived copy of worksheet used to create the importer.' )
		),
		'deleted'       => array(
			'FIELD_TYPE'     => FT_BIT,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => 0,
			'LABEL'          => _t( 'Is deleted?' ),
			'DESCRIPTION'    => _t( 'Indicates if the importer is deleted or not.' ),
...
class ca_data_importers extends BundlableLabelableBaseModelWithAttributes {
	...
	static public function getAvailableInputFormats() {
		...
		foreach ( $va_readers as $vs_reader ) {
			if ( ( DataReaderManager::checkDataReaderStatus( $vs_reader ) )
			     && ( $o_reader
					= DataReaderManager::getDataReaderInstance( $vs_reader ) )
			) {
	 ...
	 *        formats = array of input formats to limit returned importers to. Only importers that accept at least one
	 *        of the specified formats will be returned. dontIncludeWorksheet = don't include compressed metadata
	 *        describing import worksheet in return value.  If you are serializing the array to JSON this data can
	 *        cause problems. [Default is false]
	 *
	...
	static public function getImporters( $pn_table_num = null, $pa_options = null ) {
		...
		while ( $qr_res->nextRow() ) {
			...
			if ( isset( $va_settings['inputFormats'] ) && is_array( $va_settings['inputFormats'] )
			     && is_array( $va_formats )
			) {
			...
			$t_instance                                 = Datamodel::getInstanceByTableNum( $va_row['table_num'],
				true );
			...
			$va_importers[ $vn_id ]['last_modified_on'] = $t_importer->getLastChangeTimestamp( $vn_id,
				array( 'timestampOnly' => true ) );
	 ...
	 * @return int
	 */
	static public function getImporterListAsHTMLFormElement(
		$ps_name, $pn_table_num = null, $pa_attributes = null, $pa_options = null
	) {
		...
		foreach ( $va_importers as $vn_importer_id => $va_importer_info ) {
			$va_opts[ $va_importer_info['label'] . " (" . $va_importer_info['importer_code'] . ")" ]
				= $va_importer_info['importer_id'];
		}
		ksort( $va_opts );
	...
	public function getItems() {
		...
		while ( $qr_items->nextRow() ) {
			...
			$va_return[ $qr_items->get( "item_id" ) ]['settings']
			                                          = caUnserializeForDatabase( $va_return[ $qr_items->get( "item_id" ) ]['settings'] );
		}
	...
	public static function loadImporterFromFile( $ps_source, &$pa_errors, $pa_options = null, &$is_new = null ) {
		...
		$vn_locale_id = ( isset( $pa_options['locale_id'] ) && (int) $pa_options['locale_id'] )
			? (int) $pa_options['locale_id'] : $g_ui_locale_id;
		$pa_errors    = array();
		...
		foreach ( $o_sheet->getRowIterator() as $o_row ) {
			...
			switch ( strtolower( $vs_mode ) ) {
				...
				case 'constant':
					...
					if ( ! $vs_source ) {
						$pa_errors[] = _t( "Warning: skipped mapping at row %1 because source was not defined",
							$vn_row_num );
						if ( $o_log ) {
							$o_log->logWarn( _t( "[loadImporterFromFile:%1] Skipped mapping at row %2 because source was not defined",
								$ps_source, $vn_row_num ) );
						}
						continue( 2 );
					}
					if ( ! $vs_destination ) {
						$pa_errors[] = _t( "Warning: skipped mapping at row %1 because destination was not defined",
							$vn_row_num );
						if ( $o_log ) {
							$o_log->logWarn( _t( "[loadImporterFromFile:%1] Skipped mapping at row %2 because destination was not defined",
								$ps_source, $vn_row_num ) );
						}
						continue( 2 );
					}
					...
					if ( $vs_options_json = (string) $o_options->getValue() ) {
						if ( is_null( $va_options = @json_decode( caRepairJson( $vs_options_json ), true ) ) ) {
							...
							$pa_errors[]
								= _t( "Warning: invalid json in \"options\" column at line %4  for group %1/source %2. Json was: %3",
								$vs_group, $vs_source, $vs_options_json, $vn_row_num );
							if ( $o_log ) {
								$o_log->logWarn( _t( "[loadImporterFromFile:%1] Invalid json in \"options\" column at line %5  for group %2/source %3. Json was: %4.",
									$ps_source, $vs_group, $vs_source, $vs_options_json, $vn_row_num ) );
							}
					...
					if ( $vs_refinery && ( $vs_refinery_options_json = (string) $o_refinery_options->getValue() ) ) {
						if ( ! in_array( $vs_refinery, $va_refineries ) ) {
							...
							if ( $o_log ) {
								$o_log->logWarn( _t( "[loadImporterFromFile:%1] Invalid options for group %2/source %3",
									$ps_source, $vs_group, $vs_source ) );
							}
						} else {
							// Test whether the JSON is valid
							if ( is_null(
								$va_refinery_options = json_decode( caRepairJson( $vs_refinery_options_json ), true ) )
							) {
								// Error while json decode
								$pa_errors[]
									= _t( "invalid json for refinery options at line %4 for group %1/source %2 = %3",
									$vs_group, $vs_source, $vs_refinery_options_json, $vn_row_num );
								if ( $o_log ) {
									$o_log->logError( _t( "[loadImporterFromFile:%1] invalid json for refinery options at line %5 for group %2/source %3 = %4",
										$ps_source, $vs_group, $vs_source, $vs_refinery_options_json, $vn_row_num ) );
								}
					...
					if ( $va_options && is_array( $va_options ) && isset( $va_options['transformValuesUsingWorksheet'] )
					     && $va_options['transformValuesUsingWorksheet']
					) {
						if ( $o_opt_sheet = $o_excel->getSheetByName( $va_options['transformValuesUsingWorksheet'] ) ) {
							foreach ( $o_opt_sheet->getRowIterator() as $o_sheet_row ) {
								if ( ! $vs_original_value
									= trim( mb_strtolower( (string) $o_opt_sheet->getCellByColumnAndRow( 1,
									$o_sheet_row->getRowIndex() ) ) )
								) {
									continue;
								}
								$vs_replacement_value    = trim( (string) $o_opt_sheet->getCellByColumnAndRow( 2,
									$o_sheet_row->getRowIndex() ) );
						...
						$va_original_values = preg_split( "![\n\r]{1}!",
							mb_strtolower( (string) $o_orig_values->getValue() ) );
						array_walk( $va_original_values, function ( &$v ) {
							$v = trim( $v );
						} );
						$va_replacement_values = preg_split( "![\n\r]{1}!",
							(string) $o_replacement_values->getValue() );
				...
				case 'setting':
					...
					switch ( $vs_setting_name = (string) $o_setting_name->getValue() ) {
						...
						case 'inputFormats':
							...
							$va_settings[ $vs_setting_name ] = preg_split( "![ ]*;[ ]*!",
								(string) $o_setting_value->getValue() );
							break;
						default:
				...
				case 'environment':
					...
					if ( $vs_options_json = (string) $o_options->getValue() ) {
						...
						if ( is_null( $va_options = @json_decode( $vs_options_json, true ) ) ) {
							$pa_errors[] = _t( "Warning: invalid options for environment %1.",
								(string) $o_source->getValue() );
							if ( $o_log ) {
								$o_log->logWarn( _t( "[loadImporterFromFile:environment %1] Invalid options for environment value %1. Options were: %2.",
									(string) $o_source->getValue(), $vs_options_json ) );
							}
						}
					}
		...
		if ( isset( $va_settings['exporter_format'] ) ) {
			$pa_errors[]
				= _t( "It looks like this is a mapping for the data export framework and you're trying to add it as import mapping!" );
			if ( $o_log ) {
				$o_log->logError( _t( "[loadImporterFromFile:%1] It looks like this is a mapping for the data export framework and you're trying to add it as import mapping!",
					$ps_source ) );
			}
		...
		if ( $t_importer->load( array( 'importer_code' => $va_settings['code'] ) ) ) {
			...
			if ( $t_importer->numErrors() ) {
				$pa_errors[] = _t( "Could not delete existing mapping for %1: %2", $va_settings['code'],
						join( "; ", $t_importer->getErrors() ) ) . "\n";
				if ( $o_log ) {
					$o_log->logError( _t( "[loadImporterFromFile:%1] Could not delete existing mapping for %2: %3",
						$ps_source, $va_settings['code'], join( "; ", $t_importer->getErrors() ) ) );
				}
		...
		if ( $t_importer->numErrors() ) {
			...
			if ( $o_log ) {
				$o_log->logError( _t( "[loadImporterFromFile:%1] Error creating mapping: %2", $ps_source,
					join( "; ", $t_importer->getErrors() ) ) );
			}
		...
		if ( $t_importer->numErrors() ) {
			$pa_errors[] = _t( "Could not save worksheet for future download: %1",
					join( "; ", $t_importer->getErrors() ) ) . "\n";
			if ( $o_log ) {
				$o_log->logError( _t( "[loadImporterFromFile:%1] Error saving worksheet for future download: %2",
					$ps_source, join( "; ", $t_importer->getErrors() ) ) );
			}
		}
		...
		foreach ( $va_mapping as $vs_group => $va_mappings_for_group ) {
			...
			if ( ! $vs_group_dest ) {
				...
				$pa_errors[] = _t( "Skipped items for %1 because no common grouping could be found",
						$va_item['destination'] ) . "\n";
				if ( $o_log ) {
					$o_log->logWarn( _t( "[loadImporterFromFile:%1] Skipped items for %2 because no common grouping could be found",
						$ps_source, $va_item['destination'] ) );
				}
				continue;
			}
			...
			if ( ! $t_group ) {
				...
				if ( $o_log ) {
					$o_log->logError( _t( "[loadImporterFromFile:%1] There was an error when adding group %2",
						$ps_source, $vs_group ) );
				}
			...
			foreach ( $va_mappings_for_group as $vs_source => $va_mappings_for_source ) {
				foreach ( $va_mappings_for_source as $va_row ) {
					...
					$t_group->addItem( $vs_source, $va_row['destination'], $va_item_settings,
						array( 'returnInstance' => true ) );
				}
			}
		}
	...
	public function logImportError( $ps_message, $pa_options = null ) {
		...
		if ( $po_request && isset( $pa_options['reportCallback'] )
		     && ( $ps_callback
				= $pa_options['reportCallback'] )
		) {
			$va_general = array(
		...
		if ( $this->log_path && $this->detailed_log_name ) {
			...
			$line = [
				date( 'c' ),
				caGetOption( 'idno', $pa_options, null ),
				caGetOption( 'row', $pa_options, null ),
				$ps_message,
				caGetOption( 'values', $pa_options, null ),
				caGetOption( 'notes', $pa_options, null ),
				caGetOption( 'dataset', $pa_options, null )
			];
			fputcsv( $r, $line );
		}
	}
	 ...
	 *            logLevel = KLogger constant for minimum log level to record. Default is KLogger::INFO. Constants are,
	 *            in descending order of shrillness: ALERT = Alert messages (action must be taken immediately) CRIT =
	 *            Critical conditions ERR = Error conditions WARN = Warnings NOTICE = Notices (normal but significant
	 *            conditions) INFO = Informational messages DEBUG = Debugging messages limitLogTo = Limit logging to
	 *            specific event types for log level INFO. Valid values are: GENERAL (general status messages)
	 *            EXISTING_RECORD_POLCY (messages relating to merging of existing records SKIP (messages relating to
	 *            conditional skipping of mappings, groups or records) RELATIONSHIPS (messages relating to creating of
	 *            relationships)
	 *
	 *            Seprate multiple types with commas or semicolors. [Default is null]
	 *        logToTempDirectoryIfLogDirectoryIsNotWritable = use system temporary directory for log location if
	 *        specified logDirectory is not writable. [Default is false] dryRun = do import but don't actually save
	 *        data environment = an array of environment values to provide to the import process. The keys manifest
	 *        themselves as mappable tags. forceImportForPrimaryKeys = list of primary key ids to force mapped source
	 *        data into. The number of keys passed should equal or exceed the number of rows in the source data.
	 *        [Default is empty] transaction = transaction to perform import within. Will not be used if noTransaction
	 *        option is set. [Default is to create a new transaction] noTransaction = don't wrap the import in a
	 *        transaction. [Default is false] importAllDatasets = for data formats (such as Excel/XLSX) that support
	 *        multiple data sets in a single file (worksheets in Excel), indicated that all data sets should be
	 *        imported; otherwise only the default data set is imported [Default=false] addToSet = identifier for set
	 *        to add all imported items to. [Default is null] detailedLogName = [Default is null]
	 */
	public function importDataFromSource( $ps_source, $ps_mapping, $pa_options = null ) {
		...
		if ( is_array( $limit_logging_to ) ) {
			$log_erp       = ( in_array( 'EXISTING_RECORD_POLICY', $limit_logging_to, true )
			                   || is_null( $limit_logging_to ) );
		...
		$pb_no_transaction                = caGetOption( 'noTransaction', $pa_options, false,
			array( 'castTo' => 'bool' ) );
		...
		$this->log_path
			= $log_path = caGetOption( 'logDirectory', $pa_options,
			caGetLogPath( null, 'batch_metadata_import_log_directory' ) );

		// Name to use for detailed (bundle-level) logging
		$this->detailed_log_name
			= $detailed_log_name = pathinfo( caGetOption( 'detailedLogName', $pa_options,
			"import_errors_" . $t_mapping->get( 'importer_code' ) . "_" . date( "Y-m-d" ) ), PATHINFO_FILENAME );

		$o_event = ca_data_import_events::newEvent( isset( $pa_options['user_id'] ) ? $pa_options['user_id'] : null,
			$pa_options['format'], $ps_source, isset( $pa_options['description'] ) ? $pa_options['description'] : '' );
		...
		$va_log_import_error_opts = array(
			'startTime'        => $vn_start_time,
			'log'              => $o_log,
			'logReference'     => null,
			'request'          => $po_request,
			'progressCallback' => ( isset( $pa_options['progressCallback'] )
			                        && ( $ps_callback = $pa_options['progressCallback'] ) ) ? $ps_callback : null,
			'reportCallback'   => ( isset( $pa_options['reportCallback'] )
			                        && ( $ps_callback = $pa_options['reportCallback'] ) ) ? $ps_callback : null
		);


		$t_add_to_set = null;
		if ( $ps_add_to_set = caGetOption( 'addToSet', $pa_options, null ) ) {
			if ( ! ( $t_add_to_set = ca_sets::find( [ 'set_code' => $ps_add_to_set ],
				[ 'returnAs' => 'firstModelInstance' ] ) )
			) {
				// Set doesn't exist
				$this->logImportError( _t( "Set %1 does not exist. Create it and try the import again.",
					$ps_add_to_set ), $va_log_import_error_opts );

				return false;
			}
			if ( (int) $t_mapping->get( 'table_num' ) !== (int) $t_add_to_set->get( 'table_num' ) ) {
				// Set doesn't take type of item mapping imports
				$this->logImportError( _t( "Set %1 is not the correct type for this import.", $ps_add_to_set ),
		...
		if ( ! is_array( $pa_options ) || ! isset( $pa_options['logDirectory'] ) || ! $pa_options['logDirectory']
		     || ! file_exists( $pa_options['logDirectory'] )
		) {
		...
		$vb_show_cli_progress_bar = (bool) ( isset( $pa_options['showCLIProgressBar'] )
		                                     && ( $pa_options['showCLIProgressBar'] ) );
		...
		if ( $log_general ) {
			$o_log->logInfo( _t( 'Started import of %1 using mapping %2', $ps_source,
		...
		if ( ! ( $vn_locale_id = caGetOption( 'locale_id', $pa_options, null ) ) ) {    // set as option?
			if ( ! ( $vn_locale_id
				= ca_locales::codeToID( $t_mapping->getSetting( 'locale' ) ) )
			) {    // set in mapping?
		...
		$merge_only
			= (bool) $t_mapping->getSetting( 'mergeOnly' );    // If set we only merge; no new records will be created.

		$vb_import_all_datasets = caGetOption( 'importAllDatasets', $pa_options, false );

		$o_progress->start( _t( 'Reading %1', $ps_source ) );

		if ( $po_request && isset( $pa_options['progressCallback'] )
		     && ( $ps_callback
				= $pa_options['progressCallback'] )
		) {
			$ps_callback( $po_request, $pn_file_number, $pn_number_of_files, $ps_source, 0, 100,
				_t( 'Reading %1', $ps_source ), ( time() - $vn_start_time ), memory_get_usage( true ), 0,
				$this->num_import_errors );
		}
		...
		if ( ! ( $o_reader = $t_mapping->getDataReader( $ps_source, $ps_format ) ) ) {
			$this->logImportError( _t( "Could not open source %1 (format=%2)", $ps_source, $ps_format ),
				$va_log_import_error_opts );
			if ( $o_trans ) {
				$o_trans->rollback();
			}
		...
		if ( ! $o_reader->read( $ps_source, $va_reader_opts ) ) {
			$this->logImportError( _t( "Could not read source %1 (format=%2)", $ps_source, $ps_format ),
				$va_log_import_error_opts );
			if ( $o_trans ) {
				$o_trans->rollback();
			}
		...
		for ( $vn_dataset = 0; $vn_dataset < $vn_dataset_count; $vn_dataset ++ ) {
			...
			if ( $po_request && isset( $pa_options['progressCallback'] )
				...
				$ps_callback( $po_request, $pn_file_number, $pn_number_of_files, $ps_source, 0, $vn_num_items,
					_t( 'Importing from %1', $ps_source ), ( time() - $vn_start_time ), memory_get_usage( true ), 0,
					$this->num_import_errors );
			}
			...
			if ( ! in_array( $vs_import_error_policy = $t_mapping->getSetting( 'errorPolicy' ),
				array( 'ignore', 'stop' ) )
			) {
			...
			$vn_primary_key_mapping_item_id
				                            =
			...
			if ( is_array( $va_environment_config = $t_mapping->getEnvironment() ) ) {
				foreach ( $va_environment_config as $vn_i => $va_environment_item ) {
					...
					if ( ! $o_env_reader->read( $ps_source, array(
						'basePath'         => '',
						'originalFilename' => caGetOption( 'originalFilename', $pa_options, null )
					) )
					) {
					...
					switch ( sizeof( $va_env_tmp ) ) {
						case 1:
							$vs_env_value = $o_env_reader->get( $va_environment_item['value'],
								array( 'returnAsArray' => false ) );
							break;
						case 2:
			...
			while ( $o_reader->nextRow() ) {
				...
				foreach ( $va_mapping_items as $vn_item_id => $va_item ) {
					...
					if ( is_array( $va_row[ $vs_key ] ) ) {
						...
						$va_row_with_replacements[ $vs_key ] = ca_data_importers::replaceValue( $va_row[ $vs_key ],
							$va_item, [] );
					}
				}
				...
				foreach ( $va_mapping_rules as $va_rule ) {
					...
					if ( ! isset( $va_rule['actions'] ) || ! is_array( $va_rule['actions'] )
					     || ! sizeof( $va_rule['actions'] )
					) {
						continue;
					}
					...
					try {
						if ( ExpressionParser::evaluate( $va_rule['trigger'], $va_row ) ) {
							foreach ( $va_rule['actions'] as $va_action ) {
								...
								switch ( $vs_action_code = strtolower( $va_action['action'] ) ) {
									case 'set':
										$va_rule_set_values[ $va_action['target'] ]
											= $va_action['value']; // TODO: transform value using mapping rules?
									...
									default:
										if ( $vs_action_code != 'skip' ) {
											if ( $log_skip ) {
												$o_log->logInfo( _t( 'Row was skipped using rule "%1" with default action because an invalid action ("%2") was specified',
													$va_rule['trigger'], $vs_action_code ) );
											}
										} else {
											$o_log->logDebug( _t( 'Row was skipped using rule "%1" with action "%2"',
												$va_rule['trigger'], $vs_action_code ) );
										}
										continue( 4 );
										break;
								...
								switch ( $vs_action_code = strtolower( $va_action['action'] ) ) {
									case 'set':
										if ( isset( $va_action['else'] ) ) {
											$va_rule_set_values[ $va_action['target'] ]
												= $va_action['else']; // TODO: transform value using mapping rules?
										}
										break;
								}
							}
						...
						$o_log->logError( _t( "Expression trigger could not be parsed. Please double-check the syntax! Expression was: %1 [%2]",
							$va_rule['trigger'], $e->getMessage() ) );
					}
				}
				...
				if ( $vn_type_id_mapping_item_id ) {
					...
					$vs_type = ca_data_importers::getValueFromSource( $va_mapping_items[ $vn_type_id_mapping_item_id ],
						$o_reader, array( 'otherValues' => $va_rule_set_values, 'environment' => $va_environment ) );
					if ( isset( $va_mapping_items[ $vn_type_id_mapping_item_id ]['settings']['default'] )
					     && strlen( $va_mapping_items[ $vn_type_id_mapping_item_id ]['settings']['default'] )
					     && ! strlen( $vs_type )
					) {
				...
				if ( isset( $va_mapping_items[ $vn_type_id_mapping_item_id ]['settings']['default'] )
				     && strlen( $va_mapping_items[ $vn_type_id_mapping_item_id ]['settings']['default'] )
				     && ( sizeof( array_filter( $va_subject_type_list, function ( $v ) use ( $vs_type ) {
				...
				if ( $vn_idno_mapping_item_id ) {
					...
					$vs_idno = ca_data_importers::getValueFromSource( $va_mapping_items[ $vn_idno_mapping_item_id ],
						$o_reader, array( 'otherValues' => $va_rule_set_values, 'environment' => $va_environment ) );

					if ( isset( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['default'] )
					     && strlen( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['default'] )
					     && ! strlen( $vs_idno )
					) {
					...
					if ( ! is_array( $vs_idno ) && ( $vs_idno[0] == '^' ) && preg_match( "!^\^[^ ]+$!", $vs_idno ) ) {
						...
						if ( ! is_null(
							$vm_parsed_val = BaseRefinery::parsePlaceholder( $vs_idno, $va_row_with_replacements,
								$va_mapping_items[ $vn_idno_mapping_item_id ], null,
					...
					if ( $vs_idno && isset( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['prefix'] )
					     && strlen( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['prefix'] )
					) {
						$vs_idno
							= DisplayTemplateParser::processTemplate( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['prefix'],
								$va_row_with_replacements, [ 'getFrom' => $o_reader ] ) . $vs_idno;
					}
					if ( $vs_idno && isset( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['suffix'] )
					     && strlen( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['suffix'] )
					) {
						$vs_idno .= DisplayTemplateParser::processTemplate( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['suffix'],
							$va_row_with_replacements, [ 'getFrom' => $o_reader ] );
					}

					if ( isset( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['applyRegularExpressions'] )
					     && is_array( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['applyRegularExpressions'] )
					) {
						$vs_idno = self::_processAppliedRegexes( $o_reader,
							$va_mapping_items[ $vn_idno_mapping_item_id ], 0,
							$va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['applyRegularExpressions'],
							$vs_idno, $va_row, $va_row_with_replacements );
					}

					if ( isset( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['formatWithTemplate'] )
					     && strlen( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['formatWithTemplate'] )
					) {
						$vs_idno
							= DisplayTemplateParser::processTemplate( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['formatWithTemplate'],
							$va_row_with_replacements, [ 'getFrom' => $o_reader ] );
					}

					if ( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['delimiter']
					     && $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['treatAsIdentifiersForMultipleRows']
					) {
						$va_idnos_for_row
							= explode( $va_mapping_items[ $vn_idno_mapping_item_id ]['settings']['delimiter'],
							$vs_idno );
					}
				...
				if ( $vn_primary_key_mapping_item_id ) {
					$vn_mapped_primary_key_value
						= ca_data_importers::getValueFromSource( $va_mapping_items[ $vn_primary_key_mapping_item_id ],
						$o_reader, array( 'otherValues' => $va_rule_set_values, 'environment' => $va_environment ) );
				}
				...
				foreach ( $va_idnos_for_row as $vs_idno ) {
					...
					foreach (
						$va_preferred_label_mapping_ids as $vn_preferred_label_mapping_id =>
						$vs_preferred_label_mapping_fld
					) {
						$vs_label_val
							= ca_data_importers::getValueFromSource( $va_mapping_items[ $vn_preferred_label_mapping_id ],
							$o_reader, array(
						...
						if ( isset( $va_mapping_items[ $vn_preferred_label_mapping_id ]['settings']['formatWithTemplate'] )
						     && strlen( $va_mapping_items[ $vn_preferred_label_mapping_id ]['settings']['formatWithTemplate'] )
						) {
							$vs_label_val
								= DisplayTemplateParser::processTemplate( $va_mapping_items[ $vn_preferred_label_mapping_id ]['settings']['formatWithTemplate'],
								$va_row_with_replacements );
						}
						if ( $vs_opt
							= $va_mapping_items[ $vn_preferred_label_mapping_id ]['settings']['displaynameFormat']
						) {
							$va_label_val = DataMigrationUtils::splitEntityName( $vs_label_val, array(
								'displaynameFormat' => $vs_opt,
								'doNotParse'        => $va_mapping_items[ $vn_preferred_label_mapping_id ]['settings']['doNotParse']
							) );
							$vs_label_val = $va_label_val['displayname'];
						}
						$va_pref_label_values[ $vs_preferred_label_mapping_fld ] = $vs_label_val;
					}
					$vs_display_label = isset( $va_pref_label_values[ $vs_label_display_fld ] )
						? $va_pref_label_values[ $vs_label_display_fld ] : $vs_idno;
					...
					$va_base_criteria = ( $vs_type
					                      && ! (bool) $t_mapping->getSetting( 'ignoreTypeForExistingRecordPolicy' ) )
						? [ 'type_id' => $vs_type ] : [];
					if ( $vn_list_mapping_list_id !== null ) {
						$va_base_criteria['list_id'] = $vn_list_mapping_list_id;
					}

					if ( is_array( $pa_force_import_for_primary_keys )
					     && sizeof( $pa_force_import_for_primary_keys ) > 0
					) {
						$vn_id = array_shift( $pa_force_import_for_primary_keys );
						if ( ! $t_subject->load( $vn_id ) ) {
							if ( $log_erp ) {
								$o_log->logInfo( _t( '[%1] Skipped import because forced primary key \'%1\' does not exist',
									$vn_id ) );
							}
							$this->num_records_skipped ++;
						...
						switch ( $vs_existing_record_policy ) {
							...
							case 'merge_on_id_with_skip':
								...
								if ( is_array( $va_ids ) && ( sizeof( $va_ids ) > 0 ) ) {
									...
									if ( $log_erp ) {
										$o_log->logInfo( _t( '[%1] Merged with existing record matched on primary key %2 for %3 by policy %4',
											$vs_idno, $vn_mapped_primary_key_value, $t_subject->tableName(),
									...
									if ( $vs_existing_record_policy === 'merge_on_id_with_skip' ) {
										if ( $log_erp ) {
											$o_log->logInfo( _t( '[%1] Skipped import because existing record could not be matched on primary key %2 for %3 by policy %4',
												$vs_idno, $vn_mapped_primary_key_value, $t_subject->tableName(),
												$vs_existing_record_policy ) );
										}
										$this->num_records_skipped ++;
										...
										if ( $log_erp ) {
											$o_log->logInfo( _t( '[%1] Could not match existing record on primary key %2 for %3 by policy %4',
												$vs_idno, $vn_mapped_primary_key_value, $t_subject->tableName(),
												$vs_existing_record_policy ) );
										}
									}
							...
							case 'skip_on_id':
								...
								if ( is_array( $va_ids ) && ( sizeof( $va_ids ) > 0 ) ) {
									if ( $log_erp ) {
										$o_log->logInfo( _t( '[%1] Skipped import because of existing record matched on primary key %2 for %3 by policy %4',
											$vs_idno, $vn_mapped_primary_key_value, $t_subject->tableName(),
											$vs_existing_record_policy ) );
									}
									$this->num_records_skipped ++;
							...
							case 'overwrite_on_id':
								...
								if ( is_array( $va_ids ) && ( sizeof( $va_ids ) > 0 ) ) {
									...
									if ( $t_subject->numErrors() ) {
										$this->logImportError( _t( '[%1] Could not delete existing record matched on primary key %2 for %3 by policy %4',
											$vs_idno, $vn_mapped_primary_key_value, $t_subject->tableName(),
											$vs_existing_record_policy ), $va_log_import_error_opts );
										// Don't stop?
									} else {
										if ( $log_erp ) {
											$o_log->logInfo( _t( '[%1] Overwrote existing record matched on primary key %2 for %3 by policy %4',
												$vs_idno, $vn_mapped_primary_key_value, $t_subject->tableName(),
												$vs_existing_record_policy ) );
										}
										$t_subject->clear();
							...
							case 'skip_on_idno':
								if ( ! $vb_idno_is_template ) {
									$va_ids = call_user_func_array( $t_subject->tableName() . "::find", array(
										array_merge( $va_base_criteria,
											array( $t_subject->getProperty( 'ID_NUMBERING_ID_FIELD' ) => $vs_idno ) ),
										array(
											'returnAs'           => 'ids',
											'purifyWithFallback' => true,
											'transaction'        => $o_trans
										)
									) );
									if ( is_array( $va_ids ) && ( sizeof( $va_ids ) > 0 ) ) {
										if ( $log_erp ) {
											$o_log->logInfo( _t( '[%1] Skipped import because of existing record matched on identifier by policy %2',
												$vs_idno, $vs_existing_record_policy ) );
										}
										$this->num_records_skipped ++;
										continue( 2 );    // skip because idno matched
									}
							...
							case 'skip_on_preferred_labels':
								$va_ids = call_user_func_array( $t_subject->tableName() . "::find", array(
									array_merge( $va_base_criteria,
										array( 'preferred_labels' => $va_pref_label_values ) ),
									array(
										'returnAs'           => 'ids',
										'purifyWithFallback' => true,
										'transaction'        => $o_trans
								...
								if ( is_array( $va_ids ) && ( sizeof( $va_ids ) > 0 ) ) {
									if ( $log_erp ) {
										$o_log->logInfo( _t( '[%1] Skipped import because of existing record matched on label by policy %2',
											$vs_idno, $vs_existing_record_policy ) );
									}
									$this->num_records_skipped ++;
							...
							case 'merge_on_idno_with_skip':
								if ( ! $vb_idno_is_template ) {
									...
									if ( is_array( $va_ids ) && ( sizeof( $va_ids ) > 0 ) ) {
										$t_subject->load( $va_ids[0] );
										if ( $log_erp ) {
											$o_log->logInfo( _t( '[%1] Merged with existing record matched on identifer by policy %2',
												$vs_idno, $vs_existing_record_policy ) );
										}
										break;
									} else {
										if ( $vs_existing_record_policy === 'merge_on_idno_with_skip' ) {
											if ( $log_erp ) {
												$o_log->logInfo( _t( '[%1] Skipped import because existing record could not be matched on identifier by policy %2 using base criteria %3',
													$vs_idno, $vs_existing_record_policy,
													print_r( $va_base_criteria, true ) ) );
											}
											$this->num_records_skipped ++;
											continue( 2 );    // skip
										} else {
											if ( $log_erp ) {
												$o_log->logInfo( _t( '[%1] Could not match existing record on identifer by policy %2 using base criteria %3',
													$vs_idno, $vs_existing_record_policy,
													print_r( $va_base_criteria, true ) ) );
											}
										}
								...
								if ( in_array( $vs_existing_record_policy,
									array( 'merge_on_idno', 'merge_on_idno_with_replace' ) )
								) {
									break;
							...
							case 'merge_on_preferred_labels_with_replace':
								...
								if ( is_array( $va_ids ) && ( sizeof( $va_ids ) > 0 ) ) {
									$t_subject->load( $va_ids[0] );
									if ( $log_erp ) {
										$o_log->logInfo( _t( '[%1] Merged with existing record matched on label by policy %2',
											$vs_idno, $vs_existing_record_policy ) );
									}
									$vb_was_preferred_label_match = true;
								} else {
									if ( $log_erp ) {
										$o_log->logInfo( _t( '[%1] Could not match existing record on label values %3 by policy %2 using base criteria %4',
											$vs_idno, $vs_existing_record_policy,
											print_r( $va_pref_label_values, true ),
											print_r( $va_base_criteria, true ) ) );
									}
								}
								break;
							...
							case 'overwrite_on_idno':
								if ( ! $vb_idno_is_template && $vs_idno ) {
									...
									if ( is_array( $va_ids ) && ( sizeof( $va_ids ) > 0 ) ) {
										...
										$t_subject->delete( true, array( 'hard' => true ) );
										if ( $t_subject->numErrors() ) {
											$this->logImportError( _t( '[%1] Could not delete existing record matched on identifier by policy %2',
												$vs_idno, $vs_existing_record_policy ), $va_log_import_error_opts );
											// Don't stop?
										} else {
											if ( $log_erp ) {
												$o_log->logInfo( _t( '[%1] Overwrote existing record matched on identifier by policy %2',
													$vs_idno, $vs_existing_record_policy ) );
											}
											$t_subject->clear();
											break;
							...
							case 'overwrite_on_preferred_labels':
								...
								if ( is_array( $va_ids ) && ( sizeof( $va_ids ) > 0 ) ) {
									...
									if ( $t_subject->numErrors() ) {
										$this->logImportError( _t( '[%1] Could not delete existing record matched on label by policy %2',
											$vs_idno, $vs_existing_record_policy ), $va_log_import_error_opts );
										// Don't stop?
									} else {
										if ( $log_erp ) {
											$o_log->logInfo( _t( '[%1] Overwrote existing record matched on label by policy %2',
												$vs_idno, $vs_existing_record_policy ) );
										}
										break;
									}
					...
					if ( $merge_only && ! $t_subject->getPrimaryKey() ) {
						if ( $log_erp ) {
							$o_log->logInfo( _t( '[%1] Skipping because mergeOnly option is set and could not match existing record by policy %2.',
								$vs_idno, $vs_existing_record_policy ) );
						}
						continue;
					}
					...
					if ( $po_request && isset( $pa_options['progressCallback'] )
						...
						$ps_callback( $po_request, $pn_file_number, $pn_number_of_files, $ps_source,
							$this->num_records_processed, $vn_num_items,
							_t( "[%3/%4] Processing %1 (%2)", caTruncateStringWithEllipsis( $vs_display_label, 50 ),
								$vs_idno, $this->num_records_processed, $vn_num_items ), ( time() - $vn_start_time ),
							memory_get_usage( true ), $this->num_records_processed, $this->num_import_errors );
					}
					...
					foreach ( $va_items_by_group as $vn_group_id => $va_items ) {
						...
						foreach ( $va_items as $vn_item_id => $va_item ) {
								...
								$va_vals = [
									ca_data_importers::getValueFromSource( $va_item, $o_reader, [
										...
										'lookahead'                   => caGetOption( 'lookahead', $va_item['settings'],
											0 ),
										'filterToTypes'               => caGetOption( 'filterToTypes',
											$va_item['settings'],
											null ),
							...
							foreach ( $va_vals as $vn_i => $vm_val ) {
								if ( $va_item['settings']['skipWhenEmpty']
								     && ! is_array( $va_item['settings']['skipWhenEmpty'] )
								) {
									$va_item['settings']['skipWhenEmpty']
										= array( $va_item['settings']['skipWhenEmpty'] );
								}
								if ( isset( $va_item['settings']['skipWhenEmpty'] )
								     && is_array( $va_item['settings']['skipWhenEmpty'] )
								) {
									foreach ( $va_item['settings']['skipWhenEmpty'] as $v ) {
										if ( ! strlen( $vp = BaseRefinery::parsePlaceholder( $v,
											$use_raw ? $va_raw_row : $va_row_with_replacements, $va_item, $vn_i,
											[ 'reader' => $o_reader, 'returnAsString' => true ] ) )
										) {
											if ( $log_skip ) {
												$o_log->logInfo( _t( '[%1] Skipped mapping for %2 because at least one value in list (%3) is empty',
													$vs_idno, $va_item['destination'],
													join( '; ', $va_item['settings']['skipWhenEmpty'] ) ) );
											}
											continue( 2 );
										}
									}
								}
								if ( $va_item['settings']['skipWhenAllEmpty']
								     && ! is_array( $va_item['settings']['skipWhenAllEmpty'] )
								) {
									$va_item['settings']['skipWhenAllEmpty']
										= array( $va_item['settings']['skipWhenAllEmpty'] );
								}
								if ( isset( $va_item['settings']['skipWhenAllEmpty'] )
								     && is_array( $va_item['settings']['skipWhenAllEmpty'] )
								) {
									$all_empty = true;
									...
									if ( $all_empty ) {
										if ( $log_skip ) {
											$o_log->logInfo( _t( '[%1] Skipped mapping for %2 because all values in list (%3) are empty',
												$vs_idno, $vn_row,
												join( '; ', $va_item['settings']['skipWhenEmpty'] ) ) );
										}
										continue( 2 );
								...
								if ( $va_item['settings']['skipGroupWhenEmpty']
								     && ! is_array( $va_item['settings']['skipGroupWhenEmpty'] )
								) {
									$va_item['settings']['skipGroupWhenEmpty']
										= array( $va_item['settings']['skipGroupWhenEmpty'] );
								}
								if ( isset( $va_item['settings']['skipGroupWhenEmpty'] )
								     && is_array( $va_item['settings']['skipGroupWhenEmpty'] )
								) {
									foreach ( $va_item['settings']['skipGroupWhenEmpty'] as $v ) {
										if ( ! strlen( $vp = BaseRefinery::parsePlaceholder( $v,
											...
											if ( $log_skip ) {
												$o_log->logInfo( _t( '[%1] Skipped group %2 because at least one value in list (%3) is empty',
													$vs_idno, $vn_group_id,
													join( '; ', $va_item['settings']['skipGroupWhenEmpty'] ) ) );
											}
											continue( 3 );
										}
									}
								}
								if ( $va_item['settings']['skipGroupWhenAllEmpty']
								     && ! is_array( $va_item['settings']['skipGroupWhenAllEmpty'] )
								) {
									$va_item['settings']['skipGroupWhenAllEmpty']
										= array( $va_item['settings']['skipGroupWhenAllEmpty'] );
								}
								if ( isset( $va_item['settings']['skipGroupWhenAllEmpty'] )
								     && is_array( $va_item['settings']['skipGroupWhenAllEmpty'] )
								) {
									$all_empty = true;
									...
									if ( $all_empty ) {
										if ( $log_skip ) {
											$o_log->logInfo( _t( '[%1] Skipped group %2 because all values in list (%3) are empty',
												$vs_idno, $vn_group_id,
												join( '; ', $va_item['settings']['skipGroupWhenAllEmpty'] ) ) );
										}
										continue( 3 );
									}
								}

								if ( $va_item['settings']['skipRowWhenEmpty']
								     && ! is_array( $va_item['settings']['skipRowWhenEmpty'] )
								) {
									$va_item['settings']['skipRowWhenEmpty']
										= array( $va_item['settings']['skipRowWhenEmpty'] );
								}
								if ( isset( $va_item['settings']['skipRowWhenEmpty'] )
								     && is_array( $va_item['settings']['skipRowWhenEmpty'] )
								) {
									foreach ( $va_item['settings']['skipRowWhenEmpty'] as $v ) {
										if ( ! strlen( $vp = BaseRefinery::parsePlaceholder( $v,
											...
											if ( $log_skip ) {
												$o_log->logInfo( _t( '[%1] Skipped row %2 because at least one value in list (%3) is empty',
													$vs_idno, $vn_row,
													join( '; ', $va_item['settings']['skipRowWhenEmpty'] ) ) );
											}
											continue( 4 );
										}
									}
								}
								if ( $va_item['settings']['skipRowWhenAllEmpty']
								     && ! is_array( $va_item['settings']['skipRowWhenAllEmpty'] )
								) {
									$va_item['settings']['skipRowWhenAllEmpty']
										= array( $va_item['settings']['skipRowWhenAllEmpty'] );
								}
								if ( isset( $va_item['settings']['skipRowWhenAllEmpty'] )
								     && is_array( $va_item['settings']['skipRowWhenAllEmpty'] )
								) {
									$all_empty = true;
									...
									if ( $all_empty ) {
										if ( $log_skip ) {
											$o_log->logInfo( _t( '[%1] Skipped row %2 because all values in list (%3) are empty',
												$vs_idno, $vn_row,
												join( '; ', $va_item['settings']['skipRowWhenAllEmpty'] ) ) );
										}
										continue( 4 );
									}
								}
								...
								if ( isset( $va_item['settings']['skipIfEmpty'] )
									...
									if ( $log_skip ) {
										$o_log->logInfo( _t( '[%1] Skipped mapping because value for %2 is empty',
											$vs_idno,
											$va_item['destination'] ) );
									}
									continue( 2 );
								...
								if ( isset( $va_item['settings']['default'] )
								     && strlen( $va_item['settings']['default'] )
								     && ! strlen( $vm_val )
								) {
								...
								if ( ! is_array( $vm_val ) && ( $vm_val[0] == '^' )
									...
									// Parse placeholder
									if ( ! is_null(
										$vm_parsed_val = BaseRefinery::parsePlaceholder( $vm_val,
											$va_row_with_replacements,
											$va_item, $vn_i,
											array( 'reader' => $o_reader, 'returnAsString' => true ) ) )
									) {
								...
								if ( isset( $va_item['settings']['skipIfExpression'] )
									...
									try {
										if ( $vm_ret
											= ExpressionParser::evaluate( $va_item['settings']['skipIfExpression'],
							...
							foreach ( $va_vals as $vn_i => $vm_val ) {
								...
								if ( ( $va_item['destination'] == "{$vs_subject_table}.{$vs_type_id_fld}" )
									...
									$vs_old_type = $vs_type;
									$vs_type     = $vm_val;
									$o_log->logDebug( _t( "Reset type to %1 using value in content tree; was %2",
										$vs_type,
										$vs_old_type ) );
								}
								...
								if ( isset( $va_item['settings']['refineries'] )
									...
									foreach ( $va_item['settings']['refineries'] as $vs_refinery ) {
										...
										if ( $o_refinery = RefineryManager::getRefineryInstance( $vs_refinery ) ) {
											$va_refined_values = $o_refinery->refine( $va_content_tree, $va_group,
												$va_item,
								...
								if ( isset( $va_item['settings']['delimiter'] ) && $va_item['settings']['delimiter']
									...
									if ( sizeof( $va_item['settings']['delimiter'] ) ) {
										...
										foreach ( $va_val_list as $vs_list_val ) {
											$vs_list_val = trim( ca_data_importers::replaceValue( $vs_list_val,
												$va_item,
											...
											switch ( $vs_item_terminal ) {
												...
												case 'nonpreferred_labels':
													if ( $t_instance = Datamodel::getInstance( $vs_target_table,
														true )
													) {
													...
													if ( ! $vb_item_error_policy_is_default
														...
														if ( is_array( $va_group_buf[ $vn_c ] ) ) {
															$va_group_buf[ $vn_c ]['_errorPolicy']
																= $vs_item_error_policy;
														}
													}
								...
								if ( isset( $va_item['settings']['mediaPrefix'] )
								     && $va_item['settings']['mediaPrefix']
								) {
								...
								switch ( $vs_item_terminal ) {
									...
									case 'status':
										...
										if ( $t_subject->hasField( $vs_item_terminal ) ) {
											...
											if ( strlen( $item_val = caGetListItemValueForIdno( $list, $vm_val ) )
											     > 0
											) {
												$vm_val = $item_val;
											}
					...
					$o_log->logDebug( _t( 'Finished building content tree for %1 at %2 seconds [%3]', $vs_idno,
						$t->getTime( 4 ), $vn_row ) );
					...
					//die("END\n\n");
					//continue;
					if ( ! ( $opa_app_plugin_manager->hookDataImportContentTree( array(
						'mapping'           => $t_mapping,
								...
								if ( $vs_element == 'parent_id' ) {
									if ( ( $vn_parent_id = (int) $va_element_value['parent_id'] ) > 0 ) {
										if ( $t_subject->load( $vn_parent_id ) ) {
											$va_content_tree[ $vs_subject_table ][ $vn_i ]['parent_id']['parent_id']
												= $t_subject->get( 'parent_id' );
											$va_content_tree[ $vs_subject_table ][ $vn_i ]['idno']['idno']
												= $t_subject->get( 'idno' );
										}
									}
								}
					...
					if ( ! $t_subject->getPrimaryKey() ) {
						...
						if ( $vb_idno_is_template ) {
							...
							$t_subject->set( $vs_idno_fld, $vs_idno, array(
								'assumeIdnoForRepresentationID'    => true,
								'assumeIdnoStubForLotID'           => true,
								'tryObjectIdnoForRepresentationID' => true
										...
										switch ( $vs_element ) {
											case $vs_parent_id_fld:
												if ( $va_element_content[ $vs_parent_id_fld ] ) {
													$t_subject->set( $vs_parent_id_fld,
														$va_element_content[ $vs_parent_id_fld ],
														array( 'treatParentIDAsIdno' => true ) );
												}
												break;
										}
									}
						...
						foreach (
							$va_mandatory_field_mapping_ids as $vs_mandatory_field => $vn_mandatory_mapping_item_id
						) {
							...
							$va_opts       = array(
								...
								'assumeIdnoStubForLotID'           => true,
								'tryObjectIdnoForRepresentationID' => true,
								'treatParentIDAsIdno'              => true
							);
							if ( $va_field_info['FIELD_TYPE'] == FT_MEDIA ) {
								$va_opts['original_filename']
									= basename( $va_mandatory_field_values[ $vs_mandatory_field ] );
							}

							if ( ! $t_subject->set( $vs_mandatory_field,
								$va_mandatory_field_values[ $vs_mandatory_field ], $va_opts )
							) {
								$this->logImportError( join( "; ", $t_subject->getErrors() ),
									array_merge( $va_log_import_error_opts, [
										'bundle' => $vs_mandatory_field,
										'notes'  => _t( 'While setting mandatory field' )
									] ) );
							}
						}
						try {
							...
							$this->logImportError( $e->getMessage(), array_merge( $va_log_import_error_opts,
								[ 'bundle' => null, 'notes' => _t( 'Exception while performing initial insert' ) ] ) );
						...
						if ( $vs_error = DataMigrationUtils::postError( $t_subject,
							_t( "[%1] Could not insert new record for %2 (type was %3): ", $vs_idno,
								$t_subject->getProperty( 'NAME_SINGULAR' ), $vs_type ), __CA_DATA_IMPORT_ERROR__,
							array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
						) {

							$this->logImportError( $vs_error,
								array_merge( $va_log_import_error_opts, [ 'bundle' => null, 'notes' => null ] ) );
							if ( $vs_import_error_policy == 'stop' ) {
								$o_log->logAlert( _t( 'Import stopped due to import error policy' ) );
						...
						$o_log->logDebug( _t( 'Created idno %1 at %2 seconds [id=%3]', $vs_idno, $t->getTime( 4 ),
						...
						if ( $vn_idno_mapping_item_id || ! $t_subject->get( $vs_idno_fld ) ) {
							if ( $vb_idno_is_template ) {
								...
								$ret = $t_subject->set( $vs_idno_fld, $vs_idno, array(
									'assumeIdnoForRepresentationID'    => true,
									'assumeIdnoStubForLotID'           => true,
									'tryObjectIdnoForRepresentationID' => true,
									'treatParentIDAsIdno'              => true
							...
							if ( ! $ret && ( $vs_error = join( "; ", $t_subject->getErrors() ) ) ) {
								$this->logImportError( $vs_error, array_merge( $va_log_import_error_opts,
									[ 'bundle' => $vs_idno_fld, 'notes' => _t( 'While updating matched record' ) ] ) );
							}
						}

						$t_subject->update( [ 'queueIndexing' => true ] );
						if ( $vs_error = DataMigrationUtils::postError( $t_subject,
							_t( "[%1] Could not update matched record", $vs_idno ), __CA_DATA_IMPORT_ERROR__,
							array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
						) {
							$this->logImportError( $vs_error, array_merge( $va_log_import_error_opts,
								[ 'bundle' => null, 'notes' => _t( 'While updating matched record' ) ] ) );
							if ( $vs_import_error_policy == 'stop' ) {
								$o_log->logAlert( _t( 'Import stopped due to import error policy' ) );
						...
						if ( sizeof( $va_preferred_label_mapping_ids ) && ( $t_subject->getPreferredLabelCount() > 0 )
							...
							if ( $vb_remove_labels ) {
								$t_subject->removeAllLabels( __CA_LABEL_TYPE_PREFERRED__,
									[ 'locales' => [ $vn_locale_id ] ] );
								if ( $vs_error = DataMigrationUtils::postError( $t_subject,
									_t( "[%1] Could not remove preferred labels from matched record", $vs_idno ),
									__CA_DATA_IMPORT_ERROR__, array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
								) {
									$this->logImportError( $vs_error, array_merge( $va_log_import_error_opts, [
						...
						if ( sizeof( $va_nonpreferred_label_mapping_ids )
						     && ( $t_subject->getNonPreferredLabelCount() > 0 )
						) {
							if ( ! preg_match( "!^merge!", $vs_existing_record_policy ) ) {
								foreach (
									$va_nonpreferred_label_mapping_ids as $vn_nonpreferred_label_mapping_id => $vs_fld
								) {
								...
								$t_subject->removeAllLabels( __CA_LABEL_TYPE_NONPREFERRED__,
									[ 'locales' => [ $vn_locale_id ] ] );
								if ( $vs_error = DataMigrationUtils::postError( $t_subject,
									_t( "[%1] Could not remove nonpreferred labels from matched record", $vs_idno ),
									__CA_DATA_IMPORT_ERROR__, array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
								) {
									$this->logImportError( $vs_error, array_merge( $va_log_import_error_opts, [
										'bundle' => 'nonpreferred_labels',
										'notes'  => _t( 'While updating matched record' )
									] ) );
									if ( $vs_import_error_policy == 'stop' ) {
										$o_log->logAlert( _t( 'Import stopped due to import error policy' ) );
					...
					if ( $vs_idno_fld && ( $o_idno = $t_subject->getIDNoPlugInInstance() ) ) {
						...
						if ( ( $vs_proc_idno = join( $o_idno->getSeparator(), $va_values ) )
						     && ( $vs_proc_idno != $vs_idno )
						) {
							$t_subject->set( $vs_idno_fld, $vs_proc_idno );
							$t_subject->update( [ 'queueIndexing' => true ] );

							if ( $vs_error = DataMigrationUtils::postError( $t_subject,
								_t( "[%1] Could update idno to %2", $vs_idno, $vs_proc_idno ), __CA_DATA_IMPORT_ERROR__,
								array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
							) {
								$this->logImportError( $vs_error, array_merge( $va_log_import_error_opts,
									[ 'bundle' => $vs_idno_fld, 'notes' => _t( 'While updating matched record' ) ] ) );
					...
					$o_log->logDebug( _t( 'Started insert of content tree for idno %1 at %2 seconds [id=%3]', $vs_idno,
						$t->getTime( 4 ), $t_subject->getPrimaryKey() ) );
					$va_elements_set_for_this_record = array();
					foreach ( $va_content_tree as $vs_table_name => $va_content ) {
						if ( $vs_table_name == $vs_subject_table ) {
							foreach ( $va_content as $vn_i => $va_element_data ) {
								foreach ( $va_element_data as $vs_element => $va_element_content ) {
									$o_log->logDebug( _t( 'Started insert of %1.%2 for idno %3 at %4 seconds [id=%3]',
										$vs_table_name, $vs_element, $vs_idno, $t->getTime( 4 ),
										$t_subject->getPrimaryKey() ) );

									try {
										if ( is_array( $va_element_content ) ) {
											$vb_truncate_long_labels = caGetOption( '_truncateLongLabels',
												$va_element_content, false );
											...
											$vb_treat_numeric_value_as_id = caGetOption( '_treatNumericValueAsID',
												$va_element_content, false );
										...
										switch ( $vs_element ) {
											case 'preferred_labels':
												if ( ! $vb_was_preferred_label_match ) {
													if (
														( ! isset( $va_element_content[ $vs_disp_field
																	= $t_subject->getLabelDisplayField() ] )
														  || ! strlen( $va_element_content[ $vs_disp_field ] ) )
														&& ( sizeof( array_filter( $t_subject->getSecondaryLabelDisplayFields(),
																function ( $v ) use ( $va_element_content ) {
																	return isset( $va_element_content[ $v ] )
																	       && strlen( $va_element_content[ $v ] );
																} ) ) == 0 )
													) {
														$va_element_content[ $vs_disp_field ] = '['
														                                        . caGetBlankLabelText( $vn_table_num )
														                                        . ']';
													}

													if ( $vb_skip_if_data_present
													     && ( $t_subject->getLabelCount( true, $vn_locale_id ) > 0 )
													) {
														continue( 2 );
													}

													$t_subject->replaceLabel(
														$va_element_content, $vn_locale_id,
														isset( $va_element_content['type_id'] )
															? $va_element_content['type_id'] : null, true,
														array( 'truncateLongLabels' => $vb_truncate_long_labels )
													);
													...
													if ( $vs_error = DataMigrationUtils::postError( $t_subject,
														_t( "[%1] Could not add preferred label to %2. Record was deleted because no preferred label could be applied: ",
															$vs_idno, $t_subject->tableName() ),
														__CA_DATA_IMPORT_ERROR__,
														array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
													) {
														$this->logImportError( $vs_error,
															array_merge( $va_log_import_error_opts,
																[ 'bundle' => 'preferred_labels', 'notes' => null ] ) );
														$t_subject->delete( true, array( 'hard' => false ) );

														if ( $vs_import_error_policy == 'stop' ) {
															$o_log->logAlert( _t( 'Import stopped due to import error policy %1',
																$vs_import_error_policy ) );

															$o_event->endItem( $t_subject->getPrimaryKey(),
																__CA_DATA_IMPORT_ITEM_FAILURE__,
																_t( 'Failed to import %1', $vs_idno ) );
											...
											case 'nonpreferred_labels':
												if ( $vb_skip_if_data_present
												     && ( $t_subject->getLabelCount( false, $vn_locale_id ) > 0 )
												) {
													continue( 2 );
												}
												$t_subject->addLabel(
													$va_element_content, $vn_locale_id,
													isset( $va_element_content['type_id'] )
														? $va_element_content['type_id'] : null, false,
													array( 'truncateLongLabels' => $vb_truncate_long_labels )
												);

												if ( $vs_error = DataMigrationUtils::postError( $t_subject,
													_t( "[%1] Could not add non-preferred label to %2:", $vs_idno,
														$t_subject->tableName() ), __CA_DATA_IMPORT_ERROR__,
													array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
												) {
													$this->logImportError( $vs_error,
														array_merge( $va_log_import_error_opts,
															[ 'bundle' => 'nonpreferred_labels', 'notes' => null ] ) );
													if ( $vs_item_error_policy == 'stop' ) {
														$o_log->logAlert( _t( 'Import stopped due to mapping error policy' ) );
											...
											case '_children':    // child records (usually generated by a refinery)
												$this->_processChildren( $t_subject->tableName(),
													$t_subject->getPrimaryKey(), $va_element_content, [
														'log'          => $o_log,
														'logReference' => $vs_idno,
														'transaction'  => $o_trans,
														'matchOn'      => [ 'idno' ]
											...
											default:
												if ( $t_subject->hasField( $vs_element )
												     && ( ( $vs_element != $t_subject->primaryKey( true ) )
												          && ( $vs_element != $t_subject->primaryKey() ) )
												) {
													if ( $vb_skip_if_data_present
													     && strlen( $t_subject->get( $vs_element ) )
													) {
														continue( 2 );
													}
													...
													$va_opts
													               = array(
														'assumeIdnoForRepresentationID'    => ! $vb_treat_numeric_value_as_id,
														'assumeIdnoStubForLotID'           => ! isset( $va_element_content['_matchOn'] )
														                                      && ! $vb_treat_numeric_value_as_id,
														'tryObjectIdnoForRepresentationID' => ! $vb_treat_numeric_value_as_id,
														'treatParentIDAsIdno'              => ! $vb_treat_numeric_value_as_id
													);
													if ( $va_field_info['FIELD_TYPE'] == FT_MEDIA ) {
														$va_opts['original_filename']
															= basename( $va_element_content[ $vs_element ] );
													}

													$vs_content = ( is_array( $va_element_content )
													                && array_key_exists( $vs_element,
															$va_element_content ) )
														? $va_element_content[ $vs_element ]
														:
														( ! is_array( $va_element_content ) ? $va_element_content
															: null );

													$t_subject->set( $vs_element, $vs_content, $va_opts );
													if ( $vs_error = DataMigrationUtils::postError( $t_subject,
														_t( "[%1] Could not set intrinsic %2 to %3:", $vs_idno,
															$vs_element, $t_subject->tableName() ),
														__CA_DATA_IMPORT_ERROR__,
														array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
													) {
														$this->logImportError( $vs_error,
															array_merge( $va_log_import_error_opts,
																[ 'bundle' => $vs_element, 'notes' => null ] ) );
													}

													$t_subject->update( [ 'queueIndexing' => true ] );
													if ( $vs_error = DataMigrationUtils::postError( $t_subject,
														_t( "[%1] Could not add intrinsic %2 to %3:", $vs_idno,
															$vs_element, $t_subject->tableName() ),
														__CA_DATA_IMPORT_ERROR__,
														array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
													) {
													...
													try {
														if ( ( is_array( $va_rel_info
																	= Datamodel::getManyToOneRelations( $vs_table_name,
																	$vs_element ) )
														       && ( sizeof( $va_rel_info ) > 0 ) )
														     && is_array( $va_element_data[ $vs_element ] )
														     && is_array( $va_element_data[ $vs_element ]['_related_related'] )
														     && sizeof( $va_element_data[ $vs_element ]['_related_related'] )
														) {
															foreach (
																$va_element_data[ $vs_element ]['_related_related'] as
																$vs_rel_rel_table => $va_rel_rels
															) {
																foreach ( $va_rel_rels as $vn_i => $va_rel_rel ) {
																	if ( ! ( $t_rel_instance
																		= Datamodel::getInstance( $va_rel_info['one_table'] ) )
																	) {
																		$o_log->logWarn( _t( "[%1] Could not instantiate related table %2",
																			$vs_idno, $vs_table_name ) );
																		continue;
																	}
																	if ( $o_trans ) {
																	...
																	if ( $t_rel_instance->load( $va_element_content[ $vs_element ] ) ) {
																		if ( $t_rel_rel
																			= $t_rel_instance->addRelationship( $vs_rel_rel_table,
																			$va_rel_rel['id'],
																			$va_rel_rel['_relationship_type'] )
																		) {
																			if ( $log_relations ) {
																				$o_log->logInfo( _t( '[%1] Related %2 (%3) to related %4 with relationship %5',
																					$vs_idno,
																					Datamodel::getTableProperty( $vs_rel_rel_table,
																						'NAME_SINGULAR' ),
																					$va_rel_rel['id'],
																					$t_rel_instance->getProperty( 'NAME_SINGULAR' ),
																					trim( $va_rel_rel['_relationship_type'] ) ) );
																			}
																		} else {
																			if ( $vs_error
																				= DataMigrationUtils::postError( $t_subject,
																				_t( "[%1] Could not add related %2 (%3) to related %4 with relationship %5:",
																					$vs_idno,
																					Datamodel::getTableProperty( $vs_rel_rel_table,
																						'NAME_SINGULAR' ),
																					$va_rel_rel['id'],
																					$t_rel_instance->getProperty( 'NAME_SINGULAR' ),
																					trim( $va_rel_rel['_relationship_type'] ) ),
																				__CA_DATA_IMPORT_ERROR__, array(
																					'dontOutputLevel' => true,
																					'dontPrint'       => true
																				) )
																			) {
																				$this->logImportError( $vs_error,
																					array_merge( $va_log_import_error_opts,
																						[
																							'bundle' => $vs_rel_rel_table,
																							'notes'  => _t( 'While adding rel-rel data' )
																						] ) );
																			}
																		}
																	}
												...
												if ( ( $vs_subject_table == 'ca_representation_annotations' )
												     && ( $vs_element == 'properties' )
												) {
													foreach ( $va_element_content as $vs_prop => $vs_prop_val ) {
														if ( $vb_skip_if_data_present
														     && strlen( $t_subject->getPropertyValue( $vs_prop ) )
														) {
															continue;
														}
												...
												if ( $vb_skip_if_data_present
												     && ( $t_subject->getAttributeCountByElement( $vs_element ) > 0 )
												) {
													continue( 2 );
												}
												if (
													...
													$t_subject->removeAttributes( $vs_element,
														array( 'force' => true ) );
												}
												$va_elements_set_for_this_record[ $vs_element ] = true;

												$va_opts = [
													'showRepeatCountErrors'  => true,
												...
												if ( $va_match_on = caGetOption( '_matchOn', $va_element_content,
													null )
												) {
												...
												if ( isset( $va_element_data[ $vs_element ]['_mediaPrefix'] ) ) {
													$va_opts['mediaPrefix']
														= $va_element_data[ $vs_element ]['_mediaPrefix'];
													$va_opts['matchMode']
														= $va_element_data[ $vs_element ]['_matchMode'];
													$va_opts['matchType']
														= $va_element_data[ $vs_element ]['_matchType'];
												}
												$t_subject->addAttribute( $va_element_content, $vs_element, null,
													$va_opts );
												$t_subject->update( [ 'queueIndexing' => true ] );

												if ( $vs_error = DataMigrationUtils::postError( $t_subject,
													_t( "[%1] Failed to add value for %2; values were %3: ", $vs_idno,
														$vs_element, $errored_values
															= ca_data_importers::formatValuesForLog( $va_element_content,
															$vs_element ) ), __CA_DATA_IMPORT_ERROR__,
													array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
												) {
													$this->logImportError( $vs_error,
														array_merge( $va_log_import_error_opts, [
							...
							if ( $vs_error = DataMigrationUtils::postError( $t_subject,
								_t( "[%1] Update failed: %2", $vs_idno, join( '; ', $t_subject->getErrors() ) ),
								__CA_DATA_IMPORT_ERROR__, array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
							) {
								$this->logImportError( $vs_error, array_merge( $va_log_import_error_opts,
									[ 'bundle' => null, 'notes' => null, 'values' => null ] ) );
								if ( $vs_item_error_policy == 'stop' ) {
									$o_log->logAlert( _t( 'Import stopped due to mapping error policy' ) );
							...
							$o_log->logDebug( _t( 'Started insert of %1 for idno %2 at %3 seconds [id=%4]',
								$vs_table_name, $vs_idno, $t->getTime( 4 ), $t_subject->getPrimaryKey() ) );

							foreach ( $va_content as $vn_i => $va_element_data ) {
								// Importing tags?
								if ( $vs_table_name === 'ca_item_tags' ) {
									if ( isset( $va_element_data['ca_item_tags']['tag'] ) ) {
										$t_subject->addTag( $va_element_data['ca_item_tags']['tag'], null, null,
											$va_element_data['ca_item_tags']['access'], null,
											[ 'forceModerated' => true ] );
									} elseif ( isset( $va_element_data['ca_item_tags'] )
									           && is_string( $va_element_data['ca_item_tags'] )
									) {
										$t_subject->addTag( $va_element_data['ca_item_tags'], null, null, 1, null,
											[ 'forceModerated' => true ] );
									}
									continue;
								}
								...
								$va_nonpreferred_labels = isset( $va_data_for_rel_table['nonpreferred_labels'] )
									? $va_data_for_rel_table['nonpreferred_labels'] : null;
								...
								$va_data_for_rel_table = array_merge( $va_data_for_rel_table,
									ca_data_importers::_extractIntrinsicValues( $va_data_for_rel_table,
										$vs_table_name ) );
								...
								try {
									...
									if ( isset( $va_element_data['preferred_labels'][ $vs_rel_label_display_fld ] ) ) {
										...
										$vs_name
											= $va_element_data[ $vs_rel_label_display_fld ][ $vs_rel_label_display_fld ];
									} elseif ( isset( $va_element_data[ $vs_rel_label_display_fld ] ) ) {
										$vs_name = $va_element_data[ $vs_rel_label_display_fld ];
									} elseif ( isset( $va_element_data ) && ! is_array( $va_element_data )
									           && strlen( $va_element_data )
									) {
									...
									switch ( $vs_table_name ) {
										case 'ca_objects':
											if ( $vn_rel_id = DataMigrationUtils::getObjectID( $vs_name,
												$va_element_data['_parent_id'], $va_element_data['_type'],
												$vn_locale_id, $va_data_for_rel_table, array(
													'logReference'       => $vs_idno,
													'forceUpdate'        => true,
													'dontCreate'         => $vb_dont_create,
													'ignoreParent'       => $vb_ignore_parent,
													'matchOn'            => $va_match_on,
													'log'                => $o_log,
													'transaction'        => $o_trans,
													'importEvent'        => $o_event,
													'importEventSource'  => $vn_row,
													'nonPreferredLabels' => $va_nonpreferred_labels
												) )
											) {
												...
												$t_subject->addRelationship( $vs_table_name, $vn_rel_id,
													trim( $va_element_data['_relationship_type'] ), null, null, null,
													null,
													array( 'interstitialValues' => $va_element_data['_interstitial'] ) );

												if ( $vs_error = DataMigrationUtils::postError( $t_subject,
													_t( "[%1] Could not add related object with relationship %2:",
														$vs_idno, trim( $va_element_data['_relationship_type'] ) ),
													__CA_DATA_IMPORT_ERROR__,
													array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
												) {
													$this->logImportError( $vs_error,
														array_merge( $va_log_import_error_opts,
															[ 'bundle' => $vs_table_name, 'notes' => null ] ) );
													if ( $vs_item_error_policy == 'stop' ) {
														$o_log->logAlert( _t( 'Import stopped due to mapping error policy' ) );
										...
										case 'ca_object_lots':
											...
											if ( is_array( $va_element_data['idno_stub'] ) ) {
												$vs_idno_stub = isset( $va_element_data['idno_stub']['idno_stub'] )
													? $va_element_data['idno_stub']['idno_stub'] : '';
											} else {
												$vs_idno_stub = isset( $va_element_data['idno_stub'] )
													? $va_element_data['idno_stub'] : '';
											}
											if ( $vn_rel_id = DataMigrationUtils::getObjectLotID( $vs_idno_stub,
												$vs_name, $va_element_data['_type'], $vn_locale_id,
												$va_data_for_rel_table, array(
													'logReference'       => $vs_idno,
													'forceUpdate'        => true,
													'dontCreate'         => $vb_dont_create,
													'ignoreParent'       => $vb_ignore_parent,
												...
												if ( ! ( $vs_rel_type = $va_element_data['_relationship_type'] )
												     && ! ( is_array( $va_element_data['idno_stub'] )
												            && ( $vs_rel_type
															= $va_element_data['idno_stub']['_relationship_type'] ) )
												) {
													break;
												}
												$t_subject->addRelationship( $vs_table_name, $vn_rel_id,
												...
												if ( $vs_error = DataMigrationUtils::postError( $t_subject,
													_t( "[%1] Could not add related object lot %2 with relationship %3:",
														$vs_idno, $vs_idno_stub,
														trim( $va_element_data['_relationship_type'] ) ),
													__CA_DATA_IMPORT_ERROR__,
													array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
												) {
													$this->logImportError( $vs_error,
										...
										case 'ca_entities':
											if ( $vn_rel_id
												= DataMigrationUtils::getEntityID( $va_element_data['preferred_labels'],
												$va_element_data['_type'], $vn_locale_id, $va_data_for_rel_table, array(
													'logReference'       => $vs_idno,
													'forceUpdate'        => true,
													'dontCreate'         => $vb_dont_create,
													'ignoreParent'       => $vb_ignore_parent,
												...
												if ( ! ( $vs_rel_type = $va_element_data['_relationship_type'] )
												     && ! ( $vs_rel_type
														= $va_element_data['idno']['_relationship_type'] )
												) {
													break;
												}
												...
												if ( $vs_error = DataMigrationUtils::postError( $t_subject,
													_t( "[%1] Could not add related entity with relationship %2:",
														$vs_idno, trim( $va_element_data['_relationship_type'] ) ),
													__CA_DATA_IMPORT_ERROR__,
													array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
												) {
											...
											break;
										case 'ca_places':
											if ( $vn_rel_id = DataMigrationUtils::getPlaceID( $vs_name,
												$va_element_data['_parent_id'], $va_element_data['_type'],
												$vn_locale_id, null, $va_data_for_rel_table, array(
													'logReference'       => $vs_idno,
													'forceUpdate'        => true,
													'dontCreate'         => $vb_dont_create,
													'ignoreParent'       => $vb_ignore_parent,
												...
												if ( $vs_error = DataMigrationUtils::postError( $t_subject,
													_t( "[%1] Could not add related place with relationship %2:",
														$vs_idno, trim( $va_element_data['_relationship_type'] ) ),
													__CA_DATA_IMPORT_ERROR__,
													array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
												) {
											...
											break;
										case 'ca_collections':
											if ( $vn_rel_id = DataMigrationUtils::getCollectionID( $vs_name,
												$va_element_data['_type'], $vn_locale_id, $va_data_for_rel_table, array(
													'logReference'       => $vs_idno,
													'forceUpdate'        => true,
													'dontCreate'         => $vb_dont_create,
												...
												$t_subject->addRelationship( $vs_table_name, $vn_rel_id, $vs_rel_type,
													null, null, null, null,
													array( 'interstitialValues' => $va_element_data['_interstitial'] ) );

												if ( $vs_error = DataMigrationUtils::postError( $t_subject,
													_t( "[%1] Could not add related collection with relationship %2:",
														$vs_idno, $vs_rel_type ), __CA_DATA_IMPORT_ERROR__,
													array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
												) {
													$this->logImportError( $vs_error,
														array_merge( $va_log_import_error_opts,
											...
											break;
										case 'ca_occurrences':
											if ( $vn_rel_id = DataMigrationUtils::getOccurrenceID( $vs_name,
												$va_element_data['_parent_id'], $va_element_data['_type'],
												$vn_locale_id, $va_data_for_rel_table, array(
													'logReference'       => $vs_idno,
													'forceUpdate'        => true,
												...
												if ( $vs_error = DataMigrationUtils::postError( $t_subject,
													_t( "[%1] Could not add related occurrence with relationship %2:",
														$vs_idno, $vs_rel_type ), __CA_DATA_IMPORT_ERROR__,
													array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
												) {
													$this->logImportError( $vs_error,
										...
										case 'ca_storage_locations':
											if ( $vn_rel_id = DataMigrationUtils::getStorageLocationID( $vs_name,
												$va_element_data['_parent_id'], $va_element_data['_type'],
												$vn_locale_id, $va_data_for_rel_table, array(
													'logReference'       => $vs_idno,
													'forceUpdate'        => true,
												...
												if ( $vs_error = DataMigrationUtils::postError( $t_subject,
													_t( "[%1] Could not add related storage location with relationship %2:",
														$vs_idno, trim( $va_element_data['_relationship_type'] ) ),
													__CA_DATA_IMPORT_ERROR__,
													array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
												) {
										...
										case 'ca_list_items':
											...
											$va_data_for_rel_table['preferred_labels']
											                                     = $va_element_data['preferred_labels'];

											if ( $vn_rel_id
												= DataMigrationUtils::getListItemID( $va_element_data['_list'],
												$va_element_data['idno'] ? $va_element_data['idno'] : null,
												$va_element_data['_type'], $vn_locale_id, $va_data_for_rel_table, array(
													'logReference'       => $vs_idno,
													'forceUpdate'        => true,
													'dontCreate'         => $vb_dont_create,
												...
												if ( $vs_error = DataMigrationUtils::postError( $t_subject,
													_t( "[%1] Could not add related list item with relationship %2:",
														$vs_idno, trim( $va_element_data['_relationship_type'] ) ),
													__CA_DATA_IMPORT_ERROR__,
													array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
												) {
										...
										case 'ca_object_representations':
											if ( $vn_rel_id = DataMigrationUtils::getObjectRepresentationID( $vs_name,
												...
												$t_subject->linkRepresentation( $vn_rel_id, true,
													[ 'type_id' => trim( $va_element_data['_relationship_type'] ) ] );

												if ( $vs_error = DataMigrationUtils::postError( $t_subject,
													_t( "[%1] Could not add related object representation with:",
														$vs_idno ), __CA_DATA_IMPORT_ERROR__,
													array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
												) {
													$this->logImportError( $vs_error,
														array_merge( $va_log_import_error_opts,
											...
											break;
										case 'ca_loans':
											if ( $vn_rel_id = DataMigrationUtils::getLoanID( $vs_name,
												$va_element_data['_type'], $vn_locale_id, $va_data_for_rel_table, array(
													'logReference'       => $vs_idno,
													'forceUpdate'        => true,
													'dontCreate'         => $vb_dont_create,
												...
												if ( $vs_error = DataMigrationUtils::postError( $t_subject,
													_t( "[%1] Could not add related loan with relationship %2:",
														$vs_idno, trim( $va_element_data['_relationship_type'] ) ),
													__CA_DATA_IMPORT_ERROR__,
													array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
												) {
											...
											break;
										case 'ca_movements':
											if ( $vn_rel_id = DataMigrationUtils::getMovementID( $vs_name,
												$va_element_data['_type'], $vn_locale_id, $va_data_for_rel_table, array(
													'logReference'       => $vs_idno,
													'forceUpdate'        => true,
													'dontCreate'         => $vb_dont_create,
												...
												if ( $vs_error = DataMigrationUtils::postError( $t_subject,
													_t( "[%1] Could not add related movement with relationship %2:",
														$vs_idno, trim( $va_element_data['_relationship_type'] ) ),
													__CA_DATA_IMPORT_ERROR__,
													array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
												) {
									...
									// noop
									$o_log->logError( _t( 'Error while processing related content: %1',
										$e->getMessage() ) );
								}
								try {
									if ( is_array( $va_element_data['_related_related'] )
									     && sizeof( $va_element_data['_related_related'] )
									) {
										foreach (
											$va_element_data['_related_related'] as $vs_rel_rel_table => $va_rel_rels
										) {
											foreach ( $va_rel_rels as $vn_i => $va_rel_rel ) {
												if ( ! ( $t_rel_instance
													= Datamodel::getInstance( $vs_table_name ) )
												) {
													$o_log->logWarn( _t( "[%1] Could not instantiate related table %2",
														$vs_idno, $vs_table_name ) );
													continue;
												...
												if ( $vn_rel_id && $t_rel_instance->load( $vn_rel_id ) ) {
													if ( $t_rel_rel
														= $t_rel_instance->addRelationship( $vs_rel_rel_table,
														$va_rel_rel['id'], $va_rel_rel['_relationship_type'] )
													) {
														if ( $log_relations ) {
															$o_log->logInfo( _t( '[%1] Related %2 (%3) to related %4 with relationship %5',
																$vs_idno,
																Datamodel::getTableProperty( $vs_rel_rel_table,
																	'NAME_SINGULAR' ), $va_rel_rel['id'],
																$t_rel_instance->getProperty( 'NAME_SINGULAR' ),
																trim( $va_rel_rel['_relationship_type'] ) ) );
														}
													} else {
									...
									// noop
									$o_log->logError( _t( 'Error while processing related-to-related content: %1',
										$e->getMessage() ) );
								}
					...
					$o_log->logDebug( _t( 'Finished inserting content tree for %1 at %2 seconds into database',
						$vs_idno, $t->getTime( 4 ) ) );
					...
					if ( ! $vb_output_subject_preferred_label && ( $t_subject->getPreferredLabelCount() == 0 ) ) {
						...
						if ( $vs_error = DataMigrationUtils::postError( $t_subject,
							_t( "[%1] Could not add default label", $vs_idno ), __CA_DATA_IMPORT_ERROR__,
							array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
						) {
							$this->logImportError( $vs_error, array_merge( $va_log_import_error_opts,
								[ 'bundle' => 'preferred_labels', 'notes' => null ] ) );
					...
					$opa_app_plugin_manager->hookDataPostImport( array(
						'subject'           => $t_subject,
						'mapping'           => $t_mapping,
						'content_tree'      => &$va_content_tree,
						'idno'              => &$vs_idno,
					...
					if ( $log_general ) {
						$o_log->logInfo( _t( '[%1] Imported %2 as %3 ', $vs_idno, $vs_preferred_label_for_log,
							$vs_subject_table_name ) );
					}
					$o_event->endItem( $t_subject->getPrimaryKey(), __CA_DATA_IMPORT_ITEM_SUCCESS__,
						_t( 'Imported %1', $vs_idno ) );

					if ( $t_add_to_set ) {
						if ( ! $t_add_to_set->addItem( $t_subject->getPrimaryKey(), null, $pa_options['user_id'] ) ) {
							$o_log->logError( _t( 'Could not add imported item %1 to set %2: %3', $vs_idno,
								$ps_add_to_set, join( '; ', $t_add_to_set->getErrors() ) ) );
						}
					}
		...
		if ( $log_general ) {
			$o_log->logInfo( _t( 'Import of %1 completed using mapping %2: %3 imported/%4 skipped/%5 errors',
				$ps_source, $t_mapping->get( 'importer_code' ), $this->num_records_processed,
				$this->num_records_skipped, $this->num_import_errors ) );
		}

		$o_progress->setDataForJobID( null, _t( 'Import complete' ), [
			'numRecordsProcessed' => $this->num_records_processed,
			'table'               => $t_subject->tableName(),
			'created'             => $va_ids_created,
			'updated'             => $va_ids_updated
		] );
		$o_progress->finish();
		...
		if ( $po_request && isset( $pa_options['progressCallback'] )
			...
			$ps_callback( $po_request, $pn_file_number, $pn_number_of_files, $ps_source, $vn_num_items, $vn_num_items,
				_t( 'Import completed' ), ( time() - $vn_start_time ), memory_get_usage( true ),
				$this->num_records_processed, $this->num_import_errors );
		}
	...
	private function _processChildren( $parent_table, $parent_id, $children, $options = null ) {
		...
		foreach ( $children as $child ) {
			...
			if ( $parent_table == $child['_table'] ) {    // direct child to parent (both are same table)
				$id = DataMigrationUtils::getIDFor( $child['_table'], $child['preferred_labels'], $parent_id,
					$child['_type'], 1, $attrs, $options );
			} elseif ( $config->get( 'ca_objects_x_collections_hierarchy_enabled' )
			           && ( $parent_table == 'ca_collections' )
			           && ( $child['_table'] == 'ca_objects' )
			) {    // collection-object hierarchy
				if ( ( $id = DataMigrationUtils::getIDFor( $child['_table'], $child['preferred_labels'], null,
						$child['_type'], 1, $attrs, $options ) )
				     && ( $parent = Datamodel::getInstance( $parent_table, true ) )
				     && $parent->load( $parent_id )
				) {
					if ( $trans ) {
						$parent->setTransaction( $trans );
					}
					$parent->addRelationship( 'ca_objects', $id,
						$config->get( 'ca_objects_x_collections_hierarchy_relationship_type' ) );

					if ( $parent->numErrors() > 0 ) {
						if ( $log ) {
							$log->logInfo( _t( 'Could not create object-collection hierarchy child record relationship: %1',
								join( "; ", $parent->getErrors() ) ) );
						}
						continue;
					}
				}
					...
					foreach ( $vals['_related_related'] as $vs_rel_rel_table => $va_rel_rels ) {
						foreach ( $va_rel_rels as $vn_i => $va_rel_rel ) {
							if ( ! ( $t_rel_instance = Datamodel::getInstance( $child['_table'] ) ) ) {
								if ( $log ) {
									$log->logWarn( _t( "[%1] Could not instantiate related table %2", $log_reference,
										$child['_table'] ) );
								}
								continue;
							}
							...
							if ( $t_rel_instance->load( $id ) ) {
								if ( $t_rel_rel = $t_rel_instance->addRelationship( $vs_rel_rel_table,
									$va_rel_rel['id'], $va_rel_rel['_relationship_type'] )
								) {
									if ( $log ) {
										$log->logInfo( _t( '[%1] Related %2 (%3) to related %4 with relationship %5',
											$log_reference,
											Datamodel::getTableProperty( $vs_rel_rel_table, 'NAME_SINGULAR' ),
											$va_rel_rel['id'], $t_rel_instance->getProperty( 'NAME_SINGULAR' ),
											trim( $va_rel_rel['_relationship_type'] ) ) );
									}
								} else {
									if ( $vs_error = DataMigrationUtils::postError( $t_rel_instance,
										_t( "[%1] Could not add related %2 (%3) to related %4 with relationship %5:",
											$log_reference,
											Datamodel::getTableProperty( $vs_rel_rel_table, 'NAME_SINGULAR' ),
											$va_rel_rel['id'], $t_rel_instance->getProperty( 'NAME_SINGULAR' ),
											trim( $va_rel_rel['_relationship_type'] ) ), __CA_DATA_IMPORT_ERROR__,
										array( 'dontOutputLevel' => true, 'dontPrint' => true ) )
									) {
										$this->logImportError( $vs_error, array_merge( $options, [
											'bundle' => $vs_rel_rel_table,
											'notes'  => _t( 'While adding rel-rel data' )
										] ) );
									}
	...
	static public function getValueFromSource( $pa_item, $po_reader, $pa_options = null ) {
		...
		$va_filter_to_types              = caGetOption( 'filterToTypes', $pa_options,
			false );    // supported by CollectiveAccessDataReader
		$va_filter_to_relationship_types = caGetOption( 'filterToRelationshipTypes', $pa_options,
			false );    // supported by CollectiveAccessDataReader
		$pa_environment                  = caGetOption( 'environment', $pa_options, array(),
			array( 'castTo' => 'array' ) );
		$pa_other_values                 = caGetOption( 'otherValues', $pa_options, array(),
			array( 'castTo' => 'array' ) );
		...
		if ( preg_match( '!^_CONSTANT_:[^:]+:(.*)$!', $pa_item['source'], $va_matches ) ) {
			...
			if ( $po_reader->valuesCanRepeat() ) {
				$vm_value = $po_reader->get( $pa_item['source'], array(
					'returnAsArray'             => true,
					'filterToTypes'             => $va_filter_to_types,
					'filterToRelationshipTypes' => $va_filter_to_relationship_types,
					'hierarchicalDelimiter'     => $hierarchical_delimiter
				) );
				...
				$vm_value = trim( $po_reader->get( $pa_item['source'], array(
					'filterToTypes'             => $va_filter_to_types,
					'filterToRelationshipTypes' => $va_filter_to_relationship_types
				) ) );
			}
	...
	static public function replaceValue( $pm_value, $pa_item, $pa_options = null ) {
		if ( strlen( $pm_value ) && is_array( $pa_item['settings']['original_values'] ) ) {
			if ( ( $vn_index = array_search( trim( mb_strtolower( $pm_value ) ),
					$pa_item['settings']['original_values'] ) ) !== false
			) {
				$vs_replaced_display_value
					= $vs_replaced_value = $pa_item['settings']['replacement_values'][ $vn_index ];
				if ( $o_log = caGetOption( 'log', $pa_options, null ) ) {
					...
					if ( $pm_value !== $vs_replaced_value ) {
						$o_log->logInfo(
							_t( 'Replaced value "%1" (%2) with "%3" (%4) for %5 using replacement values',
								$vs_original_display_value, $pm_value, $vs_replaced_display_value, $vs_replaced_value,
								caGetOption( 'logReference', $pa_options, '???' ) )
						);
					}
				}
	 ...
	 *                       tables = Tables to return importers for (Ex. ['ca_objects', 'ca_object_representations']).
	 *                       If not specified all importers are returned. [Default is null] formats = Limit returned
	 *                       importers to specified formats (Ex. ['exif', 'mediainfo']). If not specified all formats
	 *                       are returned. [Default is null] nullOption = Include a no-value option using the specified
	 *                       text as label. [Default is null]
	 */
	public static function getImportersAsHTMLOptions( $options = null ) {
		...
		foreach ( $importer_list as $importer_id => $importer_info ) {
			if ( ! is_array( $tables ) || ! sizeof( $tables )
			     || in_array( (int) $importer_info['table_num'], $tables, true )
			) {
	 ...
	 *
	 */
	private static function _processAppliedRegexes(
		$o_reader, $item, $index, $regexes, $val, &$row, &$row_with_replacements
	) {
		if ( is_array( $regexes ) ) {
			foreach ( $regexes as $regex_index => $regex_info ) {
				...
				$regex = "!" . str_replace( "!", "\\!", $regex_info['match'] ) . "!u"
				         . ( ( isset( $regex_info['caseSensitive'] ) && (bool) $regex_info['caseSensitive'] ) ? ''
						: 'i' );
				...
				foreach ( $matches as $mi => $m ) {
					...
					if ( $o_reader->valuesCanRepeat() ) {
						$row_with_replacements[ $item['source'] . ":{$mi}" ][ $index ]
							= $row[ $item[ 'source' . ":{$mi}" ] ][ $index ]
							= $row[ mb_strtolower( $item['source'] . ":{$mi}" ) ][ $index ] = $m;
					} else {
						$row_with_replacements[ $item['source'] . ":{$mi}" ]
							=
						$row[ $item[ 'source' . ":{$mi}" ] ] = $row[ mb_strtolower( $item['source'] . ":{$mi}" ) ] = $m;
					}
				}
