								...
								try {
									...
									switch($vs_element) {
										...
										case '_children':	// child records (usually generated by a refinery) 
											self::_processChildren($t_subject->tableName(), $t_subject->getPrimaryKey(), $va_element_content, ['log' => $o_log, 'transaction' => $o_trans]);
											break;
										default:
			...
			if ($parent_table == $child['_table']) {	// direct child to parent (both are same table)
				...
				if (($id = DataMigrationUtils::getIDFor($child['_table'], $child['preferred_labels'], null, $child['_type'], 1, $child, $options)) && ($parent = Datamodel::getInstance($parent_table, true)) && $parent->load($parent_id)) {
					...
					if($parent->numErrors() > 0) {
						$o_log->logInfo(_t('Could not create object-collection hierarchy child record relationship: %1', join("; ", $parent->getErrors())));
						continue;
					}
				}
			}
			
			if (!$id) { $o_log->logInfo(_t('Could not create hierarchy child record for %1', $child['_table'])); continue; }
