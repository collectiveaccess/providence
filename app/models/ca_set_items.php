...
BaseModel::$s_ca_models_definitions['ca_set_items'] = array(
	...
	'FIELDS'        => array(
		'item_id'   => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this set item' )
		),
		'set_id'    => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Set' ),
			'DESCRIPTION'    => _t( 'Set item belongs to' )
		),
		'table_num' => array(
			'FIELD_TYPE'         => FT_NUMBER,
		...
		'row_id'    => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Row_id',
			'DESCRIPTION'    => 'Primary key value of item in set. Table primary key is of is determined by the table_num field in ca_sets.'
		),
		'type_id'   => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_SELECT,
		...
		'rank'      => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Sort order' ),
			'DESCRIPTION'    => _t( 'The relative priority of the set when displayed in a list with other sets. Lower numbers indicate higher priority.' ),
		),
		'vars'      => array(
			'FIELD_TYPE'     => FT_VARS,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => 'Set item variable storage',
			'DESCRIPTION'    => 'Storage area for set item variables'
		)
	)
);
...
class ca_set_items extends BundlableLabelableBaseModelWithAttributes {
	...
	protected function initLabelDefinitions( $pa_options = null ) {
		...
		$this->BUNDLES['preferred_labels'] = array(
			'type'      => 'preferred_label',
			'repeating' => true,
			'label'     => _t( "Item captions" )
		);
	}
	# ------------------------------------------------------
	...
	# ----------------------------------------
	/**
	 * Sets set item variable. Set item variables are names ("keys") with associated values (strings, numbers or
	 * arrays). Once a set item variable is set its value persists across instantiations until deleted or changed.
	 *
	 * Changes to set item variables are saved when the insert() (for new item records) or update() (for existing item
	...
	/**
	 * Deletes set item variable. Once deleted, you must call insert() (for new item records) or update() (for existing
	 * item records) to make the deletion permanent.
	 *
	 * @access public
	 *
	 * @param string $ps_key Name of set item variable
	...
	public function getVar( $ps_key ) {
		$this->clearErrors();
		if ( isset( $this->opa_set_item_vars[ $ps_key ] ) ) {
			return ( is_array( $this->opa_set_item_vars[ $ps_key ] ) ) ? $this->opa_set_item_vars[ $ps_key ]
				: stripSlashes( $this->opa_set_item_vars[ $ps_key ] );
		}
