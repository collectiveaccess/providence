...
BaseModel::$s_ca_models_definitions['ca_set_items'] = array(
    ...
    'FIELDS' => array(
        'item_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique numeric identifier used by CollectiveAccess internally to identify this set item'
            )
        ),
        'set_id' => array(
            'FIELD_TYPE' => FT_NUMBER,
        ...
        'rank' => array(
            ...
            'LABEL' => _t('Sort order'),
            'DESCRIPTION' => _t(
                'The relative priority of the set when displayed in a list with other sets. Lower numbers indicate higher priority.'
            ),
        ),
        'vars' => array(
            'FIELD_TYPE' => FT_VARS,
...
class ca_set_items extends BundlableLabelableBaseModelWithAttributes
{
    ...
    public function getVar($ps_key)
    {
        $this->clearErrors();
        if (isset($this->opa_set_item_vars[$ps_key])) {
            return (is_array($this->opa_set_item_vars[$ps_key])) ? $this->opa_set_item_vars[$ps_key] : stripSlashes(
                $this->opa_set_item_vars[$ps_key]
            );
        }
        return null;
    }
