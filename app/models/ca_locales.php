...
BaseModel::$s_ca_models_definitions['ca_locales'] = array(
	...
	'FIELDS'        => array(
		'locale_id'                => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this locale' )
		),
		'name'                     => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Name' ),
			'DESCRIPTION'    => _t( 'Name of locale. The chosen name should be descriptive and must be unique.' ),
		...
		'language'                 => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'ISO 639 language code' ),
			'DESCRIPTION'    => _t( '2 or 3 character language code for locale; use the ISO 639-1 standard for two letter codes or ISO 639-2 for three letter codes.' ),
		...
		'country'                  => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'ISO 3166-1 alpha-2 country code' ),
			'DESCRIPTION'    => _t( '2 characer country code for locale; use the ISO 3166-1 alpha-2 standard.' ),
		...
		'dialect'                  => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Dialect' ),
			'DESCRIPTION'    => _t( 'Optional dialect specifier for locale. Up to 8 characters, this code is usually one defined by the language lists at http://www.ethnologue.com.' ),
		...
		'dont_use_for_cataloguing' => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Do not use for cataloguing' ),
			'DESCRIPTION'    => _t( 'If checked then locale cannot be used to tag content.' )
		),
	)
);
...
class ca_locales extends BaseModel {
	...
	static public function getLocaleList( $pa_options = null ) {
		...
		$vs_sort_direction                 = isset( $pa_options['sort_direction'] ) ? $pa_options['sort_direction']
			: 'asc';
		$vb_index_by_code                  = ( isset( $pa_options['index_by_code'] ) && $pa_options['index_by_code'] )
			? true : false;
		$vb_return_display_values          = ( isset( $pa_options['return_display_values'] )
		                                       && $pa_options['return_display_values'] ) ? true : false;
		$vb_available_for_cataloguing_only = ( isset( $pa_options['available_for_cataloguing_only'] )
		                                       && $pa_options['available_for_cataloguing_only'] ) ? true : false;
		...
		$vs_cache_key = $vs_sort_field . '/' . $vs_sort_direction . '/' . ( $vb_index_by_code ? 1 : 0 ) . '/'
		                . ( $vb_return_display_values ? 1 : 0 ) . '/' . ( $vb_available_for_cataloguing_only ? 1 : 0 );
		if ( CompositeCache::contains( $vs_cache_key, 'LocaleList' ) ) {
			...
			if ( MemoryCache::itemCountForNamespace( 'LocaleCodeToId' ) == 0 ) {
				foreach ( $va_locales as $va_locale ) {
					...
					MemoryCache::save( $va_locale['language'] . '_' . $va_locale['country'], $va_locale['locale_id'],
						'LocaleCodeToId' );
					MemoryCache::save( $va_locale['locale_id'], $va_locale['language'] . '_' . $va_locale['country'],
						'LocaleIdToCode' );
	...
	static public function getCataloguingLocaleCodes() {
		return array_keys( self::getLocaleList( [
	...
	public function getCode() {
		return ( $this->get( 'country' ) ) ? ( $this->get( 'language' ) . "_" . $this->get( 'country' ) )
			: $this->get( 'language' );
	}
	# ------------------------------------------------------
	 ...
	 *                          forCataloguing - if set then only locales that are marked as available for cataloguing
	 *                          are counted
	 *
	 ...
	 *                            codesOnly = Return a list of locale codes for the given language. If not set then a
	 *                            list of arrays with details about each relevant locale is returned. [Default is
	 *                            false]
	 *
	 * @return array An array of arrays, each containing information about a locale. If the "codesOnly" option is set
	 *               then a simple list of locale codes is returned.
	 */
