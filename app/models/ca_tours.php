...
BaseModel::$s_ca_models_definitions['ca_tours'] = array(
    ...
    'FIELDS' => array(
        ...
        'source_id' => array(
            ...
            'LABEL' => _t('Source'),
            'DESCRIPTION' => _t(
                'Administrative source of tour. This value is often used to indicate the administrative sub-division or legacy database from which the object originates, but can also be re-tasked for use as a simple classification tool if needed.'
            )
        ),
        'source_info' => array(
            'FIELD_TYPE' => FT_VARS,
...
class ca_tours extends BundlableLabelableBaseModelWithAttributes
{
    ...
    static public $s_stop_info_cache;
    ...
    public function getStops($po_request = null, $pa_options = null)
    {
        ...
        $va_bundles_to_return = (isset($pa_options['bundles']) && is_array(
                $pa_options['bundles']
        ...
        if (is_array($va_stop_ids) && sizeof($va_stop_ids) > 0) {
            ...
            while ($qr_stops->nextHit()) {
                if (!$va_stops[$vn_stop_id = $qr_stops->get('stop_id')][$vn_screen_locale_id = $qr_stops->get(
                    'locale_id'
                )]) {
                    ...
                    if ($va_bundles_to_return) {
                        foreach ($va_bundles_to_return as $vs_fld_name => $vs_bundle) {
                            $va_tmp[$vs_fld_name] = $qr_stops->get(
                                $vs_bundle,
    ...
    public function reorderStops($pa_stop_ids, $pa_options = null)
    {
        ...
        foreach ($pa_stop_ids as $vn_rank => $vn_stop_id) {
            if (isset($va_stop_ranks[$vn_stop_id]) && $t_stop->load(
                    array('tour_id' => $vn_tour_id, 'stop_id' => $vn_stop_id)
                )) {
                if ($va_stop_ranks[$vn_stop_id] != $vn_rank) {
                    ...
                    if ($t_stop->numErrors()) {
                        $va_errors[$vn_stop_id] = _t(
                            'Could not reorder stop %1: %2',
                            $vn_stop_id,
