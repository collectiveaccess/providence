...
BaseModel::$s_ca_models_definitions['ca_tours'] = array(
	...
	'FIELDS'        => array(
		...
		'tour_code'   => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Tour code' ),
			'DESCRIPTION'    => _t( 'Unique code for tour; used to identify the tour for configuration purposes.' ),
		...
		'type_id'     => array(
			...
			'DEFAULT'         => '',
			'LABEL'           => _t( 'Type' ),
			'DESCRIPTION'     => _t( 'Indicates the type of the tour.' )
		),
		'user_id'     => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'DEFAULT'        => '',
			'LABEL'          => 'User id',
			'DESCRIPTION'    => 'Identifier for owner of tour'
		),
		'rank'        => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_FIELD,
		...
		'color'       => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Color' ),
			'DESCRIPTION'    => _t( 'Color to identify the tour with' )
		),
		'icon'        => array(
			'FIELD_TYPE'               => FT_MEDIA,
			'DISPLAY_TYPE'             => DT_FIELD,
			...
			'LABEL'                    => _t( 'Icon' ),
			'DESCRIPTION'              => _t( 'Optional icon to identify the tour with' )
		),
		'access'      => array(
			'FIELD_TYPE'         => FT_NUMBER,
			'DISPLAY_TYPE'       => DT_SELECT,
		...
		'status'      => array(
			...
			'LABEL'              => _t( 'Status' ),
			'DESCRIPTION'        => _t( 'Indicates the current state of the tour.' )
		),
		'source_id'   => array(
			'FIELD_TYPE'                => FT_NUMBER,
			'DISPLAY_TYPE'              => DT_SELECT,
			...
			'LABEL'                     => _t( 'Source' ),
			'DESCRIPTION'               => _t( 'Administrative source of tour. This value is often used to indicate the administrative sub-division or legacy database from which the object originates, but can also be re-tasked for use as a simple classification tool if needed.' )
		),
		'source_info' => array(
			'FIELD_TYPE'     => FT_VARS,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DESCRIPTION'    => 'Serialized array used to store source information for tour information retrieved via web services [NOT IMPLEMENTED YET].'
		),
		'view_count'  => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_OMIT,
...
class ca_tours extends BundlableLabelableBaseModelWithAttributes {
	...
	protected function initLabelDefinitions( $pa_options = null ) {
		...
		$this->BUNDLES['ca_tour_stops_list'] = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Tour stops' )
		);
	 ...
	 *
	 * @return array - List of available tours, indexed by tour_id and locale_id. Array values are arrays with list
	 *               information including name, locale and tour_id
	 */
	...
	public function getStops( $po_request = null, $pa_options = null ) {
		...
		$va_bundles_to_return = ( isset( $pa_options['bundles'] ) && is_array( $pa_options['bundles'] ) )
			? $pa_options['bundles'] : array();
		...
		$va_bundles_to_return += array(
			"stop_id"   => "ca_tour_stops.stop_id",
			"tour_id"   => "ca_tour_stops.tour_id",
			"idno"      => "ca_tour_stops.idno",
			"name"      => "ca_tour_stops.preferred_labels.name",
			"locale_id" => "ca_tour_stops.preferred_labels.locale_id",
			"parent_id" => "ca_tour_stops.parent_id"
		);
		...
		if ( is_array( $va_stop_ids ) && sizeof( $va_stop_ids ) > 0 ) {
			...
			while ( $qr_stops->nextHit() ) {
				if ( ! $va_stops[ $vn_stop_id = $qr_stops->get( 'stop_id' ) ][ $vn_screen_locale_id
					= $qr_stops->get( 'locale_id' ) ]
				) {
					...
					if ( $va_bundles_to_return ) {
						foreach ( $va_bundles_to_return as $vs_fld_name => $vs_bundle ) {
							$va_tmp[ $vs_fld_name ] = $qr_stops->get( $vs_bundle,
								array( 'convertValueToDisplayText' => true ) );
						}
					}
	 ...
	 *
	 * @param array $pa_stop_ids A list of stop_ids in the tour, in the order in which they should be displayed in the
	 *                           ui
	...
	public function reorderStops( $pa_stop_ids, $pa_options = null ) {
		...
		foreach ( $pa_stop_ids as $vn_rank => $vn_stop_id ) {
			if ( isset( $va_stop_ranks[ $vn_stop_id ] )
			     && $t_stop->load( array(
					'tour_id' => $vn_tour_id,
					'stop_id' => $vn_stop_id
				) )
				...
				if ( $va_stop_ranks[ $vn_stop_id ] != $vn_rank ) {
					...
					if ( $t_stop->numErrors() ) {
						$va_errors[ $vn_stop_id ] = _t( 'Could not reorder stop %1: %2', $vn_stop_id,
							join( '; ', $t_stop->getErrors() ) );
					}
				}
			}
		}
