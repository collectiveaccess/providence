...
$_ca_metadata_dictionary_entry_settings = array(        // global
    'mandatory' => array(
        ...
        'description' => _t(
            'If checked data entry bundle is mandatory and a valid value must be set before it can be saved.'
        )
    ),
    'definition' => array(
        'formatType' => FT_TEXT,
    ...
    'restrict_to_types' => array(
        ...
        'description' => _t(
            'Restricts entry to items of the specified type(s). Leave all unchecked for no restriction.'
        )
    ),
    'restrict_to_relationship_types' => array(
        ...
        'description' => _t(
            'Restricts entry to items related using the specified relationship type(s). Leave all unchecked for no restriction.'
        )
    )
);
...
class ca_metadata_dictionary_entries extends BundlableLabelableBaseModelWithAttributes
{
     ...
     */
    static public $s_definition_cache;

    /**
     * Index array converting bundle names to entry_id's
     */
    static public $s_definition_cache_index;

    /**
     * Index array converting bundle names to entry_id's
     */
    static public $s_definition_cache_relationship_type_ids;
    ...
    public static function getEntries()
    {
        ...
        while ($qr->nextRow()) {
            ...
            $row['bundle_label'] = caGetOption(
                'label',
                $row['settings'],
                caGetDisplayLabelForBundle($row['bundle_name']),
    ...
    public function setSettingDefinitionsForEntry($pa_additional_settings)
    {
        ...
        $this->SETTINGS = new ModelSettings(
            $this, 'settings', array_merge($standard_settings, $pa_additional_settings)
        );
    ...
    public function getRules($options = null)
    {
        ...
        if (!caGetOption('noCache', $options, false) && ($cache_key = caMakeCacheKeyFromOptions(
                $options,
                $id
    ...
    public static function getEntry($ps_bundle_name, $pt_subject, $pa_settings = null, $pa_options = null)
    {
        ...
        if (!is_array(
                $va_types = caGetOption(['restrict_to_types', 'restrictToTypes'], $pa_settings, null)
            ) && $va_types) {
            $va_types = [$va_types];
        }
        ...
        if (sizeof($va_relationship_types = array_filter($va_relationship_types, 'strlen'))) {
            $va_relationship_types = array_merge(
                $va_relationship_types,
                ca_relationship_types::relationshipTypeIDsToTypeCodes(
                    $va_relationship_types
                )
            );
        }
