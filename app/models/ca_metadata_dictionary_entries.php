...
BaseModel::$s_ca_models_definitions['ca_metadata_dictionary_entries'] = array(
	...
	'FIELDS'        => array(
		'entry_id'    => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Entry id',
			'DESCRIPTION'    => 'Identifier for metadata dictionary entry'
		),
		'table_num'   => array(
			'FIELD_TYPE'         => FT_NUMBER,
			...
			'DEFAULT'            => '',
			'LABEL'              => _t( 'Entry type' ),
			'DESCRIPTION'        => _t( 'Type of item this entry displays for.' ),
...
class ca_metadata_dictionary_entries extends BundlableLabelableBaseModelWithAttributes {
	...
	protected function initLabelDefinitions( $pa_options = null ) {
		...
		$this->BUNDLES['settings'] = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Data dictionary entry settings' )
		);

		$this->BUNDLES['ca_metadata_dictionary_rules'] = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Rules' )
		);
	...
	static public function preloadDefinitions( $pa_bundles ) {
		...
		while ( $qr_res->nextRow() ) {
			...
			$va_row['settings']
			        = caUnserializeForDatabase( $va_row['settings'] );
			ca_metadata_dictionary_entries::$s_definition_cache[ $va_row['entry_id'] ]
			        = $va_row;
			ca_metadata_dictionary_entries::$s_definition_cache_index[ $va_row['bundle_name'] ][ $va_row['entry_id'] ]
			        = 1;
	...
	static public function getEntries() {
		...
		while ( $qr->nextRow() ) {
			...
			$row['bundle_label']  = caGetOption( 'label', $row['settings'],
				caGetDisplayLabelForBundle( $row['bundle_name'] ), [ 'defaultOnEmptyString' => true ] );
	...
	/**
	 * Sets setting definitions for to use for the current entry. Note that these definitions persist no matter what
	 * row is loaded
	 * (or even if no row is loaded). You can set the definitions once and reuse the instance for many entries. All
	...
	public function setSettingDefinitionsForEntry( $pa_additional_settings ) {
		...
		$this->SETTINGS = new ModelSettings( $this, 'settings',
			array_merge( $standard_settings, $pa_additional_settings ) );
	...
	public function getRules( $options = null ) {
		...
		if ( ! caGetOption( 'noCache', $options, false ) && ( $cache_key = caMakeCacheKeyFromOptions( $options, $id ) )
		     && MemoryCache::contains( $cache_key, 'MDDictRuleList' )
		) {
	 ...
	 *                               noCache = Bypass cache (typically loaded using
	 *                               ca_metadata_dictionary_entries::preloadDefinitions()) and check entry directly.
	 *                               [Default=false]
	 *
	 * @return array List of entry_ids for the specified bundle if it exists. These can be plugged into the
	 *               ca_metadata_dictionary_entries::$s_definition_cache cache array to get entry data. Returns false
	...
	/**
	 * Get a dictionary entry for a bundle. Entries are matched first on bundle name, and then filtered on any
	 * restrict_to_types and restrict_to_relationship_types settings in the $pa_settings parameter. This allows you to
	 * have different dictionary entries for the same bundle name subject to type restrictions set in the user
	 * interface. For example, if you have a ca_entities bundle (related entities) you can have different dictionary
	 * entries return when ca_entities is restricted to authors vs. publishers.
	 *
	 * @param string    $ps_bundle_name The bundle name to find a dictionary entry for.
	 * @param BaseModel $pt_subject
	 * @param array     $pa_settings    Bundle settings to use when matching definitions. The bundle settings
	 *                                  restrict_to_types and restrict_to_relationship_types will be used, when
	 *                                  present, to find type-restricted dictionary entries.
	 * @param array     $pa_options     Options include:
	 *                                  noCache = Bypass cache (typically loaded using
	 *                                  ca_metadata_dictionary_entries::preloadDefinitions()) and check entry directly.
	 *                                  [Default=false]
	 *
	 * @return array An array with entry data. Keys are entry field names. The 'settings' key contains the label,
	 *               definition text and any type restrictions. Returns null if no entry is defined.
	 */
	public static function getEntry( $ps_bundle_name, $pt_subject, $pa_settings = null, $pa_options = null ) {
		...
		if ( ! is_array( $va_types = caGetOption( [ 'restrict_to_types', 'restrictToTypes' ], $pa_settings, null ) )
		     && $va_types
		) {
			$va_types = [ $va_types ];
		}
		...
		if ( ! is_array( $va_relationship_types = caGetOption( [
		...
		if ( sizeof( $va_relationship_types = array_filter( $va_relationship_types, 'strlen' ) ) ) {
			$va_relationship_types = array_merge( $va_relationship_types,
				ca_relationship_types::relationshipTypeIDsToTypeCodes( $va_relationship_types ) );
		}
	 ...
	 * @return string
	 */
	public function getRulesHTMLFormBundle(
	...
	public function saveRuleHTMLFormBundle( $po_request, $ps_form_prefix, $ps_placement_code ) {
		...
		foreach ( $_REQUEST as $vs_k => $vm_v ) {
			if (
				preg_match( "/^{$vs_id_prefix}_(.+?)_(new_[\d]+|[\d]+)_([A-Za-z]{2}_[A-Za-z]{2})$/u", $vs_k,
					$va_matches )
