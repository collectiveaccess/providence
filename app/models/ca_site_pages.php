...
BaseModel::$s_ca_models_definitions['ca_site_pages'] = array(
    ...
    'FIELDS' => array(
        ...
        'description' => array(
            ...
            'DESCRIPTION' => _t(
                'Text describing the intended content and purpose of the page, to aid in distinguishing it from other pages.'
            )
        ),
        'template_id' => array(
            'FIELD_TYPE' => FT_NUMBER,
        ...
        'path' => array(
            ...
            'DESCRIPTION' => _t(
                'The unique root-relative URL path used by the public to access this page. For example, if set to <em>/pages/staff</em> this page would be accessible to the public using a URL similiar to this: <em>http://your.domain.com/pages/staff</em>.'
            ),
        ...
        'access' => array(
            ...
            'LABEL' => _t('Access'),
            'DESCRIPTION' => _t(
                'Controls whether the page is available publicly. Set to <em>accessible to public</em> to make it available to the public, or <em>not accessible to public</em> to prevent access to all but content editors.'
            )
        ),
        'content' => array(
            'FIELD_TYPE' => FT_VARS,
...
class ca_site_pages extends BundlableLabelableBaseModelWithAttributes
{
    ...
    public function getHTMLFormElements($options = null)
    {
        ...
        $va_element_defs = $t_template->getHTMLFormElements(
            $va_page_content,
            array_merge(
                $options,
                [
                    'addTooltips' => true,
                    'contentUrl' => caGetOption(
                        'contentUrl',
                        $options,
                        null
                    )
    ...
    public static function renderPageForPath($po_controller, $ps_path, $options = null)
    {
        if (
            (
                ...
                &&
                ($t_template = ca_site_templates::find(
                    ['template_id' => $t_page->get('template_id')],
                    ['returnAs' => 'firstModelInstance']
                ))
            )
            ||
            ...
            if (is_array($va_content = caUnserializeForDatabase($t_page->get('content')))) {
                foreach ($va_content as $vs_tag => $vs_content) {
                    $o_content_view->setVar(
                        $vs_tag,
                        caProcessReferenceTags(
                            $po_controller->request,
                            $vs_content,
            ...
            if (sizeof($va_media_to_render) > 0) {
                ...
                if (!is_array($va_access_values = caGetUserAccessValues($po_controller->request)) || !sizeof(
                        $va_access_values
                    )) {
                ...
                foreach ($va_media_to_render as $va_media) {
                    ...
                    if (is_array($va_access_values) && !in_array(
                            $va_media_list[$vn_index]['access'],
                            $va_access_values
                        )) {
                        continue;
                    }
            ...
            if ((bool)$t_page->getAppConfig()->get('allow_php_in_site_page_templates')) {
                ...
                $vs_template = preg_replace(
                    '!\$this([^A-Za-z0-9]+)!',
                    '$that\1',
                    $t_template->get('template')
                );    // ... and then rewrite all instances of "$this" to "$that"
                eval("?>{$vs_template}");
    ...
    public function getPageMedia($pa_versions = null, $options = null)
    {
        ...
        foreach ($va_media as $i => $va_media_info) {
            ...
            $va_media_list[$va_media_info['media_id']] = array_merge(
                $va_media_info,
                [
            ...
            foreach ($pa_versions as $vs_version) {
                ...
                $va_media_list[$va_media_info['media_id']]['tags'][$vs_version] = $o_coder->getMediaTag(
                    $vs_version,
                    ['alt' => $alt_text]
                );
    ...
    public function getBundleFormValues($ps_bundle_name, $ps_placement_code, $pa_bundle_settings, $options = null)
    {
        ...
        foreach ($va_media as $vn_media_id => $va_m) {
            $va_initial_values[$vn_media_id] = array(
                ...
                'fetched' => $va_m['fetched_from'] ? _t(
                    "<h3>Fetched from:</h3> URL %1 on %2 uing %3 URL handler",
                    '<a href="' . $va_m['fetched_from'] . '" target="_ext" title="' . $va_m['fetched_from'] . '">' . $va_m['fetched_from'] . '</a>',
                    date('c', $va_m['fetched_on']),
