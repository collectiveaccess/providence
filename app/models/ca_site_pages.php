...
BaseModel::$s_ca_models_definitions['ca_site_pages'] = array(
	...
	'FIELDS'        => array(
		...
		'title'       => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Page metadata: title' ),
			'DESCRIPTION'    => _t( 'A short descriptive title for page, used to distinguish the page from others.' ),
		...
		'description' => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Page metadata: description' ),
			'DESCRIPTION'    => _t( 'Text describing the intended content and purpose of the page, to aid in distinguishing it from other pages.' )
		),
		'template_id' => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Page metadata: template' ),
			'DESCRIPTION'    => _t( 'The template selected to format this page for presentation.' )
		),
		'path'        => array(
			'FIELD_TYPE'     => FT_TEXT,
			'DISPLAY_TYPE'   => DT_FIELD,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Page metadata: URL path' ),
			'DESCRIPTION'    => _t( 'The unique root-relative URL path used by the public to access this page. For example, if set to <em>/pages/staff</em> this page would be accessible to the public using a URL similiar to this: <em>http://your.domain.com/pages/staff</em>.' ),
		...
		'access'      => array(
			...
			'LABEL'              => _t( 'Access' ),
			'DESCRIPTION'        => _t( 'Controls whether the page is available publicly. Set to <em>accessible to public</em> to make it available to the public, or <em>not accessible to public</em> to prevent access to all but content editors.' )
		),
		'content'     => array(
			'FIELD_TYPE'     => FT_VARS,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Page content' ),
			'DESCRIPTION'    => _t( 'JSON-encoded page content.' )
		),
		'keywords'    => array(
			'FIELD_TYPE'     => FT_TEXT,
			'DISPLAY_TYPE'   => DT_FIELD,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Page metdata: keywords' ),
			'DESCRIPTION'    => _t( 'Optional keywords for this page.' )
		),
		'deleted'     => array(
			'FIELD_TYPE'     => FT_BIT,
			'DISPLAY_TYPE'   => DT_OMIT,
		...
		'view_count'  => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'View count' ),
			'DESCRIPTION'    => _t( 'Number of views for this page.' )
		)
	)
);
...
class ca_site_pages extends BundlableLabelableBaseModelWithAttributes {
	...
	protected function initLabelDefinitions( $options = null ) {
		...
		$this->BUNDLES['ca_site_pages_content'] = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Page content' )
		);
		$this->BUNDLES['ca_site_page_media']    = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Page media' )
		);
	 ...
	 *                       type = Limit returned pages. Valid values are "all" (return all pages),
	 *                       "PROVIDENCE_HELP_MENU" (return only pages marked for back-end help menu), "PAWTUCKET"
	 *                       (return all pages with a URL set). [Default is "all"]
	 *
	...
	public static function getPageList( $options = null ) {
		$type = caGetOption( 'type', $options, 'all',
			[ 'forceLowercase' => true, 'validValues' => [ 'all', 'providence_help_text', 'pawtucket' ] ] );
	 ...
	 *
	 * @return array An array of arrays, each of which contains fields values for a content tag present in the page
	 *               template.
	 */
	public function getHTMLFormElements( $options = null ) {
		...
		$va_element_defs = $t_template->getHTMLFormElements( $va_page_content, array_merge( $options,
			[ 'addTooltips' => true, 'contentUrl' => caGetOption( 'contentUrl', $options, null ) ] ) );
	 ...
	 * @param array            $options       Options include:
	 *                                        incrementViewCount = increment view count value for page. [Default is
	 *                                        false] checkAccess = Array of access values for which rendering should
	 *                                        occur. If the page to render does not have one of the listed access
	 *                                        values rendering will fail. [Default is null]
	 *
	...
	public static function renderPageForPath( $po_controller, $ps_path, $options = null ) {
		if (
			(
				is_numeric( $ps_path )
				&& ( $t_page = ca_site_pages::find( [ 'page_id' => (int) $ps_path ], [
					'returnAs'    => 'firstModelInstance',
					'checkAccess' => caGetOption( 'checkAccess', $options, null )
				] ) )
				&& ( $t_template = ca_site_templates::find( [ 'template_id' => $t_page->get( 'template_id' ) ],
					[ 'returnAs' => 'firstModelInstance' ] ) )
			)
			|| ( $t_page = ca_site_pages::find( [ 'path' => $ps_path ], [
				'returnAs'    => 'firstModelInstance',
				'checkAccess' => caGetOption( 'checkAccess', $options, null )
			] ) )
			   && ( $t_template = ca_site_templates::find( [ 'template_id' => $t_page->get( 'template_id' ) ],
			...
			if ( is_array( $va_content = caUnserializeForDatabase( $t_page->get( 'content' ) ) ) ) {
				foreach ( $va_content as $vs_tag => $vs_content ) {
					$o_content_view->setVar( $vs_tag, caProcessReferenceTags( $po_controller->request, $vs_content,
						[ 'page' => $t_page->getPrimaryKey() ] ) );
				}
			}
			...
			if ( sizeof( $va_media_to_render ) > 0 ) {
				...
				if ( ! is_array( $va_access_values = caGetUserAccessValues( $po_controller->request ) )
				     || ! sizeof( $va_access_values )
				) {
				...
				foreach ( $va_media_to_render as $va_media ) {
					...
					if ( is_array( $va_access_values )
					     && ! in_array( $va_media_list[ $vn_index ]['access'], $va_access_values )
					) {
						continue;
					}
			...
			foreach ( [ 'title', 'description', 'path', 'access', 'keywords', 'view_count' ] as $vs_field ) {
				$o_content_view->setVar( "page_{$vs_field}",
					caProcessReferenceTags( $po_controller->request, $t_page->get( $vs_field ),
						[ 'page' => $t_page->getPrimaryKey() ] ) );
			}
			...
			if ( (bool) $t_page->getAppConfig()->get( 'allow_php_in_site_page_templates' ) ) {
				ob_start();
				$that
					         = $o_content_view;    // Simulate "$this" in a view while eval'ing the raw template code by copying view into "$that"...
				$vs_template = preg_replace( '!\$this([^A-Za-z0-9]+)!', '$that\1',
					$t_template->get( 'template' ) );    // ... and then rewrite all instances of "$this" to "$that"
				eval( "?>{$vs_template}" );
	 ...
	 */
	public function getPageContentHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pa_bundle_settings = null, $options = null
	) {
		global $g_ui_locale;
	 ...
	 * @return string Rendered HTML bundle
	 */
	public function getPageMediaHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pa_bundle_settings = null, $options = null
	) {
		global $g_ui_locale;
	...
	public function getPageMedia( $pa_versions = null, $options = null ) {
		...
		$va_media = ca_site_page_media::find( [ 'page_id' => $vn_page_id ],
			[ 'returnAs' => 'arrays', 'sort' => 'rank' ] );
		...
		foreach ( $va_media as $i => $va_media_info ) {
			...
			$va_media_list[ $va_media_info['media_id'] ]['info']['original_filename']
			                                             = $va_media_list[ $va_media_info['media_id'] ]['info']['ORIGINAL_FILENAME'];
			...
			foreach ( $pa_versions as $vs_version ) {
				...
				$va_media_list[ $va_media_info['media_id'] ]['tags'][ $vs_version ]
					                                                                      = $o_coder->getMediaTag( $vs_version,
					[ 'alt' => $alt_text ] );
				$va_media_list[ $va_media_info['media_id'] ]['urls'][ $vs_version ]
					                                                                      = $o_coder->getMediaUrl( $vs_version );
				$va_media_list[ $va_media_info['media_id'] ]['paths'][ $vs_version ]
					                                                                      = $o_coder->getMediaPath( $vs_version );
	...
	public function getBundleFormValues( $ps_bundle_name, $ps_placement_code, $pa_bundle_settings, $options = null ) {
		...
		foreach ( $va_media as $vn_media_id => $va_m ) {
			$va_initial_values[ $vn_media_id ] = array(
				...
				'filename'             => $va_m['info']['ORIGINAL_FILENAME'] ? $va_m['info']['ORIGINAL_FILENAME']
					: _t( 'Unknown' ),
				...
				'fetched'              => $va_m['fetched_from']
					? _t( "<h3>Fetched from:</h3> URL %1 on %2 uing %3 URL handler",
						'<a href="' . $va_m['fetched_from'] . '" target="_ext" title="' . $va_m['fetched_from'] . '">'
						. $va_m['fetched_from'] . '</a>', date( 'c', $va_m['fetched_on'] ),
						caGetOption( 'fetched_by', $va_rep, 'default' ) ) : ""
			);
		}
