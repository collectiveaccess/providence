...
BaseModel::$s_ca_models_definitions['ca_data_exporters'] = array(
	...
	'FIELDS'        => array(
		'exporter_id'   => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this exporter' )
		),
		'exporter_code' => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'exporter code' ),
			'DESCRIPTION'    => _t( 'Unique alphanumeric identifier for this exporter.' ),
			'UNIQUE_WITHIN'  => array()
		...
		'table_num'     => array(
			...
			'DESCRIPTION'        => _t( 'Indicates type of item exporter is used for.' ),
		...
		'settings'      => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Settings' ),
			'DESCRIPTION'    => _t( 'exporter settings' )
		),
	)
);
...
class ca_data_exporters extends BundlableLabelableBaseModelWithAttributes {
	...
	static public function getFormatSettings( $ps_format ) {
		return ( isset( BaseExportFormat::$s_format_settings[ $ps_format ] )
			? BaseExportFormat::$s_format_settings[ $ps_format ] : array() );
	}
	# ------------------------------------------------------
	...
	public function getItems( $pa_options = array() ) {
		...
		$vb_include_settings_form    = isset( $pa_options['includeSettingsForm'] )
			? (bool) $pa_options['includeSettingsForm'] : false;
		$vb_only_top_level           = isset( $pa_options['onlyTopLevel'] ) ? (bool) $pa_options['onlyTopLevel']
			: false;
		$vb_order_for_delete_cascade = isset( $pa_options['orderForDeleteCascade'] )
			? (bool) $pa_options['orderForDeleteCascade'] : false;
		...
		while ( $qr_items->nextRow() ) {
			...
			if ( $vb_include_settings_form ) {
				...
				$va_items[ $vn_item_id ]['settings']
				        = $t_item->getHTMLSettingForm( array(
					'settings' => $t_item->getSettings(),
					'id'       => "{$vs_id_prefix}_setting_{$vn_item_id}"
				) );
			}
		}
	...
	static public function getExporters( $pn_table_num = null, $pa_options = null ) {
		...
		while ( $qr_res->nextRow() ) {
			...
			$va_exporters[ $vn_id ]['last_modified_on'] = $t_exporter->getLastChangeTimestamp( $vn_id,
	 ...
	 */
	static public function getExporterListAsHTMLFormElement(
		...
		foreach ( $va_exporters as $va_importer_info ) {
			$va_opts[ $va_importer_info['label'] . " (" . $va_importer_info['exporter_code'] . ")" ]
				= $va_importer_info['exporter_id'];
		}
		ksort( $va_opts );
	...
	public static function loadExporterFromFile( $ps_source, &$pa_errors, $pa_options = null ) {
				...
				case 'template':
					...
					if ( ( $vs_mode !== 'template' ) && $parent_id ) {
						if ( ! in_array( $vs_parent_id, $va_ids ) && ( $vs_parent_id != $vs_id ) ) {
							$pa_errors[]
								=
							$m = _t( "Warning: skipped mapping at row %1 because parent id was invalid", $vn_row );
							$o_log->logWarn( $m );
							continue( 2 );
						}
					}
					...
					if ( ! ( $vs_element = trim( (string) $o_element->getValue() ) ) ) {
						$pa_errors[]
							= $m = _t( "Warning: skipped mapping at row %1 because element was not defined", $vn_row );
						$o_log->logWarn( $m );
						continue( 2 );
					}
					...
					if ( $vs_mode == 'Variable' ) {
						if ( preg_match( "/^[A-Za-z0-9\_\-]+$/", $vs_element ) ) {
							...
							$pa_errors[]
								= $m
								= _t( "Variable name %1 is invalid. It should only contain ASCII letters, numbers, hyphens and underscores. The variable was not created.",
								$vs_element );
							$o_log->logError( $m );
							continue( 2 );
						}
					...
					if ( $vs_options_json = (string) $o_options->getValue() ) {
						if ( is_null( $va_options = @json_decode( $vs_options_json, true ) ) ) {
							$pa_errors[]
								= $m = _t( "Warning: options for element %1 are not in proper JSON", $vs_element );
							$o_log->logWarn( $m );
						}
					}
						...
						foreach ( $va_mapping_items_to_repeat as $vs_mapping_item_to_repeat ) {
							...
							foreach ( $va_mapping as $vs_item_key => $va_item ) {
								if ( in_array( $va_item['parent_id'], $va_keys_to_lookup, true ) ) {
									...
									$va_new_items[ $vs_key . "_:_" . $vs_item_key ]['parent_id'] = $vs_key
									                                                               . ( $va_item['parent_id']
											? "_:_" . $va_item['parent_id'] : "" );
				...
				case 'setting':
					...
					switch ( $vs_setting_name = (string) $o_setting_name->getValue() ) {
						case 'typeRestrictions':        // older mapping worksheets use "inputTypes" instead of the preferred "inputFormats"
							$va_settings[ $vs_setting_name ] = preg_split( "/[;,]/u",
		...
		try {
			...
			foreach ( $o_sheet->getRowIterator() as $o_row ) {
				...
				if ( ! isset( $va_mapping[ $vs_mapping_num ] ) ) {
					$pa_errors[]
						= $m = _t( "Warning: Replacement sheet references invalid mapping number '%1'. Ignoring row.",
						$vs_mapping_num );
					$o_log->logWarn( $m );
					continue;
				}
				...
				if ( ! $vs_search ) {
					$pa_errors[]
						= $m
						= _t( "Warning: Search must be set for each row in the replacement sheet. Ignoring row for mapping '%1'",
						$vs_mapping_num );
					$o_log->logWarn( $m );
					continue;
				}
		...
		if ( $t_exporter->load( array( 'exporter_code' => $va_settings['code'] ) ) ) {
			...
			if ( $t_exporter->numErrors() ) {
				$pa_errors[]
					= $m = _t( "Could not delete existing mapping for %1: %2", $va_settings['code'],
					join( "; ", $t_exporter->getErrors() ) );
				$o_log->logError( $m );
		...
		foreach ( $va_mapping as $vs_mapping_id => $va_info ) {
			...
			if ( ! $va_info['skip'] ) {
				$t_item = $t_exporter->addItem( $vn_parent_id, $va_info['element'], $va_info['source'],
					$va_item_settings );

				if ( $t_exporter->numErrors() ) {
					$pa_errors[]
						= $m = _t( "Error adding item to exporter: %1", join( "; ", $t_exporter->getErrors() ) );
					$o_log->logError( $m );
	 ...
	 *                            logLevel = KLogger constant for minimum log level to record. Default is
	 *                            KLogger::INFO. Constants are, in descending order of shrillness: KLogger::EMERG =
	 *                            Emergency messages (system is unusable) KLogger::ALERT = Alert messages (action must
	 *                            be taken immediately) KLogger::CRIT = Critical conditions KLogger::ERR = Error
	 *                            conditions KLogger::WARN = Warnings KLogger::NOTICE = Notices (normal but significant
	 *                            conditions) KLogger::INFO = Informational messages KLogger::DEBUG = Debugging
	 *                            messages
	 *
	 * @return boolean success state
	 */
	static public function exportRDFMode( $ps_config, $ps_filename, $pa_options = array() ) {
		...
		$vb_show_cli_progress_bar = ( isset( $pa_options['showCLIProgressBar'] )
		...
		if ( is_array( $va_nodes ) ) {
			foreach ( $va_nodes as $va_mapping ) {
				...
				if ( $vb_show_cli_progress_bar ) {
					print CLIProgressBar::start( $o_result->numHits(),
						$vs_msg = _t( 'Adding %1 mapping records to set', $va_mapping['mapping'] ) );
				}
				...
				while ( $o_result->nextHit() ) {
					...
					if ( is_array( $va_mapping['related'] ) ) {
						foreach ( $va_mapping['related'] as $va_related_nodes ) {
							if ( ! $t_related_mapping
								= ca_data_exporters::loadExporterByCode( $va_related_nodes['mapping'] )
							) {
								continue;
							}
							...
							$va_restrict_to_types     = is_array( $va_related_nodes['restrictToTypes'] )
								? $va_related_nodes['restrictToTypes'] : null;
							$va_restrict_to_rel_types = is_array( $va_related_nodes['restrictToRelationshipTypes'] )
								? $va_related_nodes['restrictToRelationshipTypes'] : null;


							$va_related = $o_result->get( $vs_rel_table, array(
								'returnAsArray'               => true,
								'restrictToTypes'             => $va_restrict_to_types,
								'restrictToRelationshipTypes' => $va_restrict_to_rel_types
							) );
							foreach ( $va_related as $va_rel ) {
								if ( ! isset( $va_records_to_export[ $vn_rel_table . "/"
								                                     . $va_rel[ $vs_rel_key ] ] )
								) {
									$va_records_to_export[ $vn_rel_table . "/" . $va_rel[ $vs_rel_key ] ]
										= $t_related_mapping->get( 'exporter_code' );
								}
							}
						}
					}
		...
		foreach ( $va_records_to_export as $vs_key => $vs_mapping ) {
			...
			foreach ( $va_mappings as $vs_mapping ) {
				$vs_item_export = ca_data_exporters::exportRecord( $vs_mapping, $va_split[1],
					array( 'rdfMode' => true, 'logger' => $o_log ) );
	 ...
	 *                                 logLevel = KLogger constant for minimum log level to record. Default is
	 *                                 KLogger::INFO. Constants are, in descending order of shrillness: KLogger::EMERG
	 *                                 = Emergency messages (system is unusable) KLogger::ALERT = Alert messages
	 *                                 (action must be taken immediately) KLogger::CRIT = Critical conditions
	 *                                 KLogger::ERR = Error conditions KLogger::WARN = Warnings KLogger::NOTICE =
	 *                                 Notices (normal but significant conditions) KLogger::INFO = Informational
	 *                                 messages KLogger::DEBUG = Debugging messages
	 *
	 * @return boolean success state
	 */
	static public function exportRecordsFromSearchExpression(
		$ps_exporter_code, $ps_expression, $ps_filename, $pa_options = array()
	) {
	 ...
	 *                                       logDirectory = path to directory where logs should be written
	 *                                       logLevel = KLogger constant for minimum log level to record. Default is
	 *                                       KLogger::INFO. Constants are, in descending order of shrillness:
	 *                                       KLogger::EMERG = Emergency messages (system is unusable) KLogger::ALERT =
	 *                                       Alert messages (action must be taken immediately) KLogger::CRIT = Critical
	 *                                       conditions KLogger::ERR = Error conditions KLogger::WARN = Warnings
	 *                                       KLogger::NOTICE = Notices (normal but significant conditions)
	 *                                       KLogger::INFO = Informational messages KLogger::DEBUG = Debugging messages
	 *
	 * @return boolean success state
	 */
	static public function exportRecordsFromSearchResult(
		$ps_exporter_code, $po_result, $ps_filename = null, $pa_options = array()
	) {
		...
		if ( sizeof( $va_errors ) > 0 ) {
			if ( $po_request && isset( $pa_options['progressCallback'] )
				...
				$ps_callback( $po_request, 0, - 1, _t( 'Export failed: %1', join( "; ", $va_errors ) ), 0,
					memory_get_usage( true ), 0 );
			}
		...
		$o_log->logInfo( _t( "SearchResult contains %1 results. Now calling single-item export for each record.",
			$vn_num_items ) );
		...
		if ( $po_request && isset( $pa_options['progressCallback'] )
			...
			if ( $vn_num_items > 0 ) {
				$ps_callback( $po_request, 0, $vn_num_items, _t( "Exporting result" ), ( time() - $vn_start_time ),
					memory_get_usage( true ), 0 );
			} else {
				$ps_callback( $po_request, 0, - 1, _t( 'Found no records to export' ), ( time() - $vn_start_time ),
					memory_get_usage( true ), 0 );
			}
		}
		$vn_num_processed = 0;
		...
		if ( $t_mapping->getSetting( 'CSV_print_field_names' ) ) {
			...
			foreach ( $va_mapping_items as $vn_i => $va_mapping_item ) {
				...
				$va_header_sources[ (int) $va_mapping_item['element'] ] = $va_settings['_id'] ? $va_settings['_id']
					: $va_mapping_item['source'];
			}
		...
		while ( $po_result->nextHit() ) {
			...
			if ( $vb_have_request ) {
				if ( ! caCanRead( $po_request->getUserID(), $t_instance->tableNum(),
					$po_result->get( $t_instance->primaryKey() ) )
				) {
					continue;
				}
			}

			$vs_item_export = ca_data_exporters::exportRecord( $ps_exporter_code,
				$po_result->get( $t_instance->primaryKey() ), array( 'logger' => $o_log ) );
			if ( $vs_export_format == 'JSON' ) {
			...
			if ( $vb_have_request && isset( $pa_options['progressCallback'] )
			     && ( $ps_callback
					= $pa_options['progressCallback'] )
			) {
				$ps_callback( $po_request, $vn_num_processed, $vn_num_items,
					_t( "Exporting ... [%1/%2]", $vn_num_processed, $vn_num_items ), ( time() - $vn_start_time ),
					memory_get_usage( true ), $vn_num_processed );
			}
		}
		...
		if ( $po_request && isset( $pa_options['progressCallback'] )
			...
			$ps_callback( $po_request, $vn_num_items, $vn_num_items, _t( 'Export completed' ),
				( time() - $vn_start_time ), memory_get_usage( true ), $vn_num_processed );
		}
	...
	/**
	 * Export set of records from a given SearchResult object to an array of strings with the individual exports, keyed
	 * by primary key. The behavior is tailored towards the needs of the OAIPMHService.
	 *
	 * @param string       $ps_exporter_code defines the exporter to use
	...
	static public function checkMapping( $ps_exporter_code ) {
		if ( isset( ca_data_exporters::$s_mapping_check_cache[ $ps_exporter_code ] )
		     && is_array( ca_data_exporters::$s_mapping_check_cache[ $ps_exporter_code ] )
		) {
			...
			return ca_data_exporters::$s_mapping_check_cache[ $ps_exporter_code ]
				= $o_export->getMappingErrors( $t_mapping );
		}
	}
	# ------------------------------------------------------
	 ...
	 *
	 * @param string $ps_exporter_code defines the exporter to use
	 * @param int    $pn_record_id     Primary key of the record to export. Record type is determined by the table_num
	 *                                 field for this exporter.
	 * @param array  $pa_options
	 *                                 singleRecord = Gives a signal to the export format implementation that this is a
	 *                                 single record export. For certain formats this might trigger different behavior,
	 *                                 for instance the XML export format prepends the item-level output with <?xml ...
	 *                                 ?> in those cases. rdfMode = Signals the implementation that this is an RDF mode
	 *                                 export logDirectory = path to directory where logs should be written logLevel =
	 *                                 KLogger constant for minimum log level to record. Default is KLogger::INFO.
	 *                                 Constants are, in descending order of shrillness: KLogger::EMERG = Emergency
	 *                                 messages (system is unusable) KLogger::ALERT = Alert messages (action must be
	 *                                 taken immediately) KLogger::CRIT = Critical conditions KLogger::ERR = Error
	 *                                 conditions KLogger::WARN = Warnings KLogger::NOTICE = Notices (normal but
	 *                                 significant conditions) KLogger::INFO = Informational messages KLogger::DEBUG = Debugging messages logger = Optional ready-to-use instance of KLogger to use for logging/debugging
	 *
	...
	static public function exportRecord( $ps_exporter_code, $pn_record_id, $pa_options = array() ) {
		...
		if ( ! $t_exporter ) {
			$o_log->logError( _t( "Failed to load exporter with code '%1' for item with ID %2", $ps_exporter_code,
				$pn_record_id ) );
		...
		if ( is_array( $va_type_restrictions ) && sizeof( $va_type_restrictions ) ) {
			...
			if ( ! in_array( $t_instance->getTypeCode(), $va_type_restrictions ) ) {
				$o_log->logError( _t( "Could not run exporter with code '%1' for item with ID %2 because a type restriction is in place",
					$ps_exporter_code, $pn_record_id ) );

				return false;
			}
		}

		$o_log->logInfo( _t( "Successfully loaded exporter with code '%1' for item with ID %2", $ps_exporter_code,
			$pn_record_id ) );
		...
		foreach ( $t_exporter->getTopLevelItems() as $va_item ) {
			$va_export = array_merge( $va_export,
				$t_exporter->processExporterItem( $va_item['item_id'], $t_exporter->get( 'table_num' ), $pn_record_id,
					$pa_options ) );
		}

		$o_log->logInfo( _t( "The export tree for exporter code '%1' and item with ID %2 is now ready to be processed by the export format (i.e. transformed to XML, for example).",
			$ps_exporter_code, $pn_record_id ) );
		...
		$o_manager->hookExportRecord( array(
			'exporter_instance' => $t_exporter,
			'record_id'         => $pn_record_id,
			'export'            => &$va_export
		) );
	 ...
	 *                            if this export is a sub-export (context-switch), we have no way of knowing the
	 *                            relationship to the 'parent' element in the export, so there has to be a means to
	 *                            pass it down to make it accessible attribute_id = signals that this is an export
	 *                            relative to a specific attribute instance this triggers special behavior that allows
	 *                            getting container values in a kind of sub-export it's really only useful for
	 *                            Containers but in theory can be any attribute logger = KLogger instance to use for
	 *                            logging. This option is mandatory! offset =
	 *
	...
	public function processExporterItem( $pn_item_id, $pn_table_num, $pn_record_id, $pa_options = array() ) {
		...
		$o_log->logInfo( _t( "Export mapping processor called with parameters [exporter_item_id:%1 table_num:%2 record_id:%3]",
			$pn_item_id, $pn_table_num, $pn_record_id ) );
		...
		if ( ! ( $t_instance = ca_data_exporters::loadInstanceByID( $pn_record_id, $pn_table_num, $pa_options ) ) ) {
			throw new ApplicationException( _t( "Record with ID %1 does not exist in table %2", $pn_record_id,
				$pn_table_num ) );
		}

		$vb_strip_newlines = caGetOption( 'stripNewlines', $pa_options,
			$t_exporter_item->getSetting( 'stripNewlines' ) );
		...
		if ( ! $vb_ignore_context && ( $vs_context = $t_exporter_item->getSetting( 'context' ) ) ) {
			...
			$va_restrict_to_rel_types = array_merge( $va_restrict_to_rel_types,
				caGetOption( 'restrictToRelationshipTypes', $pa_options, [] ) );
			...
			$o_log->logInfo( _t( "Initiating context switch to '%1' for mapping ID %2 and record ID %3. The processor now tries to find matching records for the switch and calls itself for each of those items.",
				$vs_context, $pn_item_id, $pn_record_id ) );
			...
			switch ( $vs_context ) {
				...
				case 'ancestors':
				case 'hierarchy':
					$va_parents = $t_instance->get( "{$vs_new_table_name}.hierarchy.{$vs_key}",
						[ 'returnAsArray' => true, 'restrictToTypes' => $va_filter_types ] );
				...
				case 'ca_sets':
					...
					$va_set_ids                    = $t_set->getSetsForItem( $pn_table_num,
						$t_instance->getPrimaryKey(), $va_set_options );
					$va_related                    = array();
				...
				case 'ca_list_items.firstLevel':
					if ( $t_instance->tableName() == 'ca_lists' ) {
						$va_related             = [];
						$va_items_legacy_format = $t_instance->getListItemsAsHierarchy( null,
							array( 'maxLevels' => 1, 'dontIncludeRoot' => true ) );
				...
				default:
					if ( $vn_new_table_num ) {
						...
						$o_log->logDebug( _t( "Calling getRelatedItems with options: %1.",
							print_r( $va_options, true ) ) );
						...
						if ( is_array( $va_attrs ) && sizeof( $va_attrs ) > 0 ) {
							...
							$o_log->logDebug( _t( "Raw attribute value array is as follows. The mapping will now be repeated for each (outer) attribute. %1",
								print_r( $va_attrs, true ) ) );

							$vn_i = 0;
							foreach ( $va_attrs as $vo_attr ) {
								$va_attribute_export = $this->processExporterItem( $pn_item_id, $pn_table_num,
									$pn_record_id,
									array_merge( array(
										'ignoreContext' => true,
										'attribute_id'  => $vo_attr->getAttributeID(),
										'offset'        => $vn_i
									), $pa_options )
								);
							...
							$o_log->logInfo( _t( "Switching context for element code %1 failed. Either there is no attribute with that code attached to the current row or the code is invalid. Mapping is ignored for current row.",
								$va_context_tmp[1] ) );
						}
			...
			if ( is_array( $va_related ) && sizeof( $va_related ) ) {
				$o_log->logDebug( _t( "The current mapping will now be repeated for these items: %1",
					print_r( $va_related, true ) ) );
				if ( ! $vn_new_table_num ) {
				...
				foreach ( $va_related as $va_rel ) {
					...
					$va_rel_export = $this->processExporterItem( $pn_item_id, $vn_new_table_num, $va_rel[ $vs_key ],
						array_merge( array( 'ignoreContext' => true ), $pa_options ) );
		...
		if ( $t_exporter_item->getSetting( 'convertCodesToDisplayText' ) ) {
			$va_get_options['convertCodesToDisplayText']
				= true;        // try to return text suitable for display for system lists stored in intrinsics (ex. ca_objects.access, ca_objects.status, ca_objects.source_id)
			// this does not affect list attributes
		} else {
			$va_get_options['convertCodesToIdno']
				= true;                // if display text is not requested try to return list item idno's... since underlying integer ca_list_items.item_id values are unlikely to be useful in an export context
		}
		...
		if ( $va_restrict_to_types = $t_exporter_item->getSetting( 'restrictToTypes' ) ) {
			$va_get_options['restrictToTypes'] = is_array( $va_restrict_to_types ) ? $va_restrict_to_types
				: [ $va_restrict_to_types ];
		}
		if ( $va_restrict_to_relationship_types = $t_exporter_item->getSetting( 'restrictToRelationshipTypes' ) ) {
			$va_get_options['restrictToRelationshipTypes'] = is_array( $va_restrict_to_relationship_types )
				? $va_restrict_to_relationship_types : [ $va_restrict_to_relationship_types ];
		}
		...
		if ( $vn_attribute_id ) {
			...
			if ( $vs_template ) { // if template is set, run through template engine as <unit>
				...
				if ( preg_match( "/^_CONSTANT_:(.*)$/", $vs_source, $va_matches ) ) {
					...
					$o_log->logDebug( _t( "This is a constant in attribute mode. Value for this mapping is '%1'",
						trim( $va_matches[1] ) ) );
					...
					$o_log->logDebug( _t( "Trying to find code %1 in value array for the current attribute.",
						$vs_source ) );
					...
					foreach ( $va_values as $vo_val ) {
						...
						switch ( $vo_val->getDatatype() ) {
							case __CA_ATTRIBUTE_VALUE_LIST__: //if ($vo_val instanceof ListAttributeValue) {
								...
								if ( $t_exporter_item->getSetting( 'returnIdno' )
								     || $t_exporter_item->getSetting( 'convertCodesToIdno' )
								) {
							...
							case __CA_ATTRIBUTE_VALUE_INFORMATIONSERVICE__:
								...
								break;
							default:
								$o_log->logDebug( _t( "Trying to match code from array %1 and the code we're looking for %2.",
									$vo_val->getElementCode(), $vs_source ) );
								if ( $vo_val->getElementCode() == $vs_source ) {
									$vs_display_value     = $vo_val->getDisplayValue( $va_display_val_options );
			...
			if ( preg_match( "/^_CONSTANT_:(.*)$/", $vs_source, $va_matches ) ) {
			...
			} else if ( in_array( $vs_source,
				array( "relationship_type_id", "relationship_type_code", "relationship_typename" ) )
			) {
				if ( isset( $pa_options[ $vs_source ] ) && strlen( $pa_options[ $vs_source ] ) > 0 ) {

					$o_log->logDebug( _t( "Source refers to releationship type information. Value for this mapping is '%1'",
						$pa_options[ $vs_source ] ) );
				...
				if ( ! $vb_repeat ) {
					...
					$o_log->logDebug( _t( "Source is a simple get() for some bundle. Value for this mapping is '%1'",
						$vs_get ) );
					...
					$va_values     = $t_instance->get( $vs_source,
						array_merge( $va_get_options, [ 'returnAsArray' => true ] ) );
					$va_values_raw = $t_instance->get( $vs_source, [ 'returnAsArray' => true ] );
					$o_log->logDebug( _t( "Source is a get() that should be repeated for multiple values. Value for this mapping is '%1'. It includes the custom delimiter ';#;' that is later used to split the value into multiple values.",
						$vs_values ) );
					$o_log->logDebug( _t( "get() options are: %1", print_r( $va_get_options, true ) ) );
					...
					foreach ( $va_values as $vn_i => $vs_text ) {
						...
						if ( $vs_skip_if_expr ) {
							...
							$va_vars = array_merge( array(
								'value'     => $vs_text,
								'value_raw' => $va_values_raw[ $vn_i ]
							), ca_data_exporters::$s_variables );

							if ( is_array( $va_expr_tags ) ) {
								foreach ( $va_expr_tags as $vs_expr_tag ) {
									$va_v                    = $t_instance->get( $vs_expr_tag,
										[ 'convertCodesToIdno' => true, 'returnAsArray' => true ] );
			...
			$o_log->logDebug( _t( "Current mapping has no source but a template '%1'. Value from extracted via template processor is '%2'",
				$vs_template, $vs_proc_template ) );
		...
		foreach ( $va_item_info as $vn_key => &$va_item ) {
			$this->opo_app_plugin_manager->hookExportItemBeforeSettings( array(
				'instance'               => $t_instance,
				'exporter_item_instance' => $t_exporter_item,
				'export_item'            => &$va_item
			) );
			...
			// do templates
			if ( isset( $va_item['template'] )
			     || ( isset( $va_get_options['template'] )
			          && $va_get_options['template'] )
			) {
			...
			$this->opo_app_plugin_manager->hookExportItem( array(
				'instance'               => $t_instance,
				'exporter_item_instance' => $t_exporter_item,
				'export_item'            => &$va_item
			) );
		...
		if ( is_array( $va_children ) && sizeof( $va_children ) > 0 ) {
			...
			$o_log->logInfo( _t( "Now proceeding to process %1 direct children in the mapping hierarchy",
				sizeof( $va_children ) ) );

			foreach ( $va_children as $va_child ) {
				foreach ( $va_item_info as &$va_info ) {
					$va_child_export     = $this->processExporterItem( $va_child['item_id'], $pn_table_num,
						$pn_record_id, $pa_options );
	...
	static public function loadInstanceByID( $pn_record_id, $pn_table_num, $pa_options = null ) {
		...
		if ( isset( ca_data_exporters::$s_instance_cache[ $pn_table_num . "/" . $pn_record_id ] ) ) {
			...
			if ( is_numeric( $pn_record_id ) ) {
				// Try numeric id
				$t_instance = $table::find( $pn_record_id,
					array_merge( $pa_options, [ 'returnAs' => 'firstModelInstance' ] ) );
			}
			if ( ! $t_instance ) {
				$t_instance = $table::find( [
					Datamodel::getTableProperty( $table, 'ID_NUMBERING_ID_FIELD' ) => $pn_record_id
				], array_merge( $pa_options, [ 'returnAs' => 'firstModelInstance' ] ) );
			}
	...
	static public function writeExporterToFile( $exporter_code, $file ) {
		...
		if ( is_array( $items = $exporter->getItems() ) ) {
			...
			foreach ( $items as $item ) {
				...
				$o_sheet->setCellValue( $a_to_z[5] . $line,
					( is_array( $item_settings ) && sizeof( $item_settings ) ) ? json_encode( $item_settings ) : '' );
			}
		}
