...
BaseModel::$s_ca_models_definitions['ca_data_exporters'] = array(
    ...
    'FIELDS' => array(
        'exporter_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique numeric identifier used by CollectiveAccess internally to identify this exporter'
            )
        ),
        'exporter_code' => array(
            'FIELD_TYPE' => FT_TEXT,
...
class ca_data_exporters extends BundlableLabelableBaseModelWithAttributes
{
    ...
    protected function initSettings()
    {
        ...
        $va_settings['exporter_format'] = array(
            ...
            'description' => _t(
                'Set exporter type, i.e. the format of the exported data. Currently supported: XML, MARC21 and CSV.'
            )
        );
        ...
        $va_settings['wrap_before'] = array(
            ...
            'description' => _t(
                'If this exporter is used for an item set export (as opposed to a single item), the text set here will be inserted before the first item. This can for instance be used to wrap a repeating set of XML elements in a single global element. The text has to be valid for the current exporter format.'
            )
        );
        ...
        $va_settings['wrap_after'] = array(
            ...
            'description' => _t(
                'If this exporter is used for an item set export (as opposed to a single item), the text set here will be inserted after the last item. This can for instance be used to wrap a repeating set of XML elements in a single global element. The text has to be valid for the current exporter format.'
            )
        );
        ...
        $va_settings['typeRestrictions'] = array(
            ...
            'description' => _t(
                'If set, this mapping will only be available for these types. Multiple types are separated by commas or semicolons.'
            )
        );
    ...
    public function getItems($pa_options = array())
    {
        ...
        while ($qr_items->nextRow()) {
            ...
            if ($vb_include_settings_form) {
                ...
                $va_items[$vn_item_id]['settings'] = $t_item->getHTMLSettingForm(
                    array('settings' => $t_item->getSettings(), 'id' => "{$vs_id_prefix}_setting_{$vn_item_id}")
                );
            }
        }
    ...
    public static function getExporters($pn_table_num = null, $pa_options = null)
    {
        ...
        while ($qr_res->nextRow()) {
            ...
            $va_exporters[$vn_id]['last_modified_on'] = $t_exporter->getLastChangeTimestamp(
                $vn_id,
     ...
     */
    public static function getExporterListAsHTMLFormElement(
        $ps_name,
        $pn_table_num = null,
        $pa_attributes = null,
        $pa_options = null
                ...
                case 'template':
                    ...
                    if (($vs_mode !== 'template') && $parent_id) {
                        if (!in_array($vs_parent_id, $va_ids) && ($vs_parent_id != $vs_id)) {
                            $pa_errors[] = $m = _t(
                                "Warning: skipped mapping at row %1 because parent id was invalid",
                                $vn_row
                            );
                            $o_log->logWarn($m);
                            continue(2);
                    ...
                    if (!($vs_element = trim((string)$o_element->getValue()))) {
                        $pa_errors[] = $m = _t(
                            "Warning: skipped mapping at row %1 because element was not defined",
                            $vn_row
                        );
                        $o_log->logWarn($m);
                        continue(2);
                    ...
                    if ($vs_mode == 'Variable') {
                        if (preg_match("/^[A-Za-z0-9\_\-]+$/", $vs_element)) {
                            ...
                            $pa_errors[] = $m = _t(
                                "Variable name %1 is invalid. It should only contain ASCII letters, numbers, hyphens and underscores. The variable was not created.",
                                $vs_element
                            );
                            $o_log->logError($m);
                            continue(2);
                    ...
                    if ($vs_options_json = (string)$o_options->getValue()) {
                        if (is_null($va_options = @json_decode($vs_options_json, true))) {
                            $pa_errors[] = $m = _t(
                                "Warning: options for element %1 are not in proper JSON",
                                $vs_element
                            );
                            $o_log->logWarn($m);
                        }
        ...
        try {
            ...
            foreach ($o_sheet->getRowIterator() as $o_row) {
                ...
                if (!isset($va_mapping[$vs_mapping_num])) {
                    $pa_errors[] = $m = _t(
                        "Warning: Replacement sheet references invalid mapping number '%1'. Ignoring row.",
                        $vs_mapping_num
                    );
                    $o_log->logWarn($m);
                    continue;
                ...
                if (!$vs_search) {
                    $pa_errors[] = $m = _t(
                        "Warning: Search must be set for each row in the replacement sheet. Ignoring row for mapping '%1'",
                        $vs_mapping_num
                    );
                    $o_log->logWarn($m);
                    continue;
    ...
    public static function exportRDFMode($ps_config, $ps_filename, $pa_options = array())
    {
                ...
                while ($o_result->nextHit()) {
                    ...
                    if (is_array($va_mapping['related'])) {
                        foreach ($va_mapping['related'] as $va_related_nodes) {
                            if (!$t_related_mapping = ca_data_exporters::loadExporterByCode(
                                $va_related_nodes['mapping']
                            )) {
                                continue;
                            }
                            ...
                            $va_restrict_to_types = is_array(
                                $va_related_nodes['restrictToTypes']
                            ) ? $va_related_nodes['restrictToTypes'] : null;
                            $va_restrict_to_rel_types = is_array(
                                $va_related_nodes['restrictToRelationshipTypes']
                            ) ? $va_related_nodes['restrictToRelationshipTypes'] : null;


                            $va_related = $o_result->get(
                                $vs_rel_table,
                                array(
                                    'returnAsArray' => true,
                            ...
                            foreach ($va_related as $va_rel) {
                                if (!isset($va_records_to_export[$vn_rel_table . "/" . $va_rel[$vs_rel_key]])) {
                                    $va_records_to_export[$vn_rel_table . "/" . $va_rel[$vs_rel_key]] = $t_related_mapping->get(
                                        'exporter_code'
                                    );
                                }
                            }
        ...
        foreach ($va_records_to_export as $vs_key => $vs_mapping) {
            ...
            foreach ($va_mappings as $vs_mapping) {
                $vs_item_export = ca_data_exporters::exportRecord(
                    $vs_mapping,
                    $va_split[1],
     ...
     * @return bool success state
     */
    public static function exportRecordsFromSearchExpression(
        $ps_exporter_code,
        $ps_expression,
        $ps_filename,
        $pa_options = array()
     ...
     * @return bool success state
     */
    public static function exportRecordsFromSearchResult(
        $ps_exporter_code,
        $po_result,
        $ps_filename = null,
        $pa_options = array()
        ...
        if (sizeof($va_errors) > 0) {
            if ($po_request && isset($pa_options['progressCallback']) && ($ps_callback = $pa_options['progressCallback'])) {
                $ps_callback(
                    ...
                    0,
                    -1,
                    _t('Export failed: %1', join("; ", $va_errors)),
                    0,
        ...
        $o_log->logInfo(
            _t("SearchResult contains %1 results. Now calling single-item export for each record.", $vn_num_items)
        );
        ...
        while ($po_result->nextHit()) {
            ...
            if ($vb_have_request && isset($pa_options['progressCallback']) && ($ps_callback = $pa_options['progressCallback'])) {
                $ps_callback(
                    ...
                    $vn_num_processed,
                    $vn_num_items,
                    _t("Exporting ... [%1/%2]", $vn_num_processed, $vn_num_items),
                    (time() - $vn_start_time),
                    memory_get_usage(true),
                    $vn_num_processed
                );
    ...
    {
        if (isset(ca_data_exporters::$s_mapping_check_cache[$ps_exporter_code]) && is_array(
                ca_data_exporters::$s_mapping_check_cache[$ps_exporter_code]
            )) {
            ...
            return ca_data_exporters::$s_mapping_check_cache[$ps_exporter_code] = $o_export->getMappingErrors(
                $t_mapping
            );
        }
    }
    ...
    public static function exportRecord($ps_exporter_code, $pn_record_id, $pa_options = array())
    {
        ...
        if (!$t_exporter) {
            $o_log->logError(
                _t("Failed to load exporter with code '%1' for item with ID %2", $ps_exporter_code, $pn_record_id)
            );
            return false;
        }
        ...
        if (is_array($va_type_restrictions) && sizeof($va_type_restrictions)) {
            ...
            if (!in_array($t_instance->getTypeCode(), $va_type_restrictions)) {
                $o_log->logError(
                    _t(
                        "Could not run exporter with code '%1' for item with ID %2 because a type restriction is in place",
                        $ps_exporter_code,
                        $pn_record_id
                    )
                );
        ...
        $o_log->logInfo(
            _t("Successfully loaded exporter with code '%1' for item with ID %2", $ps_exporter_code, $pn_record_id)
        );
        ...
        foreach ($t_exporter->getTopLevelItems() as $va_item) {
            $va_export = array_merge(
                $va_export,
                $t_exporter->processExporterItem(
                    $va_item['item_id'],
                    $t_exporter->get('table_num'),
                    $pn_record_id,
                    $pa_options
                )
            );
        ...
        $o_log->logInfo(
            _t(
                "The export tree for exporter code '%1' and item with ID %2 is now ready to be processed by the export format (i.e. transformed to XML, for example).",
                $ps_exporter_code,
                $pn_record_id
            )
        );
        ...
        $o_manager->hookExportRecord(
            array('exporter_instance' => $t_exporter, 'record_id' => $pn_record_id, 'export' => &$va_export)
        );
    ...
    public function processExporterItem($pn_item_id, $pn_table_num, $pn_record_id, $pa_options = array())
    {
        ...
        $o_log->logInfo(
            _t(
                "Export mapping processor called with parameters [exporter_item_id:%1 table_num:%2 record_id:%3]",
                $pn_item_id,
                $pn_table_num,
                $pn_record_id
            )
        ...
        if (!($t_instance = ca_data_exporters::loadInstanceByID($pn_record_id, $pn_table_num, $pa_options))) {
            throw new ApplicationException(
                _t("Record with ID %1 does not exist in table %2", $pn_record_id, $pn_table_num)
            );
        }
        ...
        if (!$vb_ignore_context && ($vs_context = $t_exporter_item->getSetting('context'))) {
            ...
            if (!is_array($va_restrict_to_rel_types = $t_exporter_item->getSetting('restrictToRelationshipTypes'))) {
                $va_restrict_to_rel_types = [];
            }
            $va_restrict_to_rel_types = array_merge(
                $va_restrict_to_rel_types,
            ...
            $o_log->logInfo(
                _t(
                    "Initiating context switch to '%1' for mapping ID %2 and record ID %3. The processor now tries to find matching records for the switch and calls itself for each of those items.",
                    $vs_context,
                    $pn_item_id,
                    $pn_record_id
                )
            ...
            switch ($vs_context) {
                ...
                case 'ancestors':
                case 'hierarchy':
                    $va_parents = $t_instance->get(
                        "{$vs_new_table_name}.hierarchy.{$vs_key}",
                ...
                case 'ca_list_items.firstLevel':
                    if ($t_instance->tableName() == 'ca_lists') {
                        $va_related = [];
                        $va_items_legacy_format = $t_instance->getListItemsAsHierarchy(
                            null,
                            array(
                                'maxLevels' => 1,
                ...
                default:
                    if ($vn_new_table_num) {
                        ...
                        if (is_array($va_attrs) && sizeof($va_attrs) > 0) {
                            ...
                            $o_log->logDebug(
                                _t(
                                    "Raw attribute value array is as follows. The mapping will now be repeated for each (outer) attribute. %1",
                            ...
                            $o_log->logInfo(
                                _t(
                                    "Switching context for element code %1 failed. Either there is no attribute with that code attached to the current row or the code is invalid. Mapping is ignored for current row.",
                                    $va_context_tmp[1]
                                )
                            );
                        }
            ...
            if (is_array($va_related) && sizeof($va_related)) {
                $o_log->logDebug(
                    _t("The current mapping will now be repeated for these items: %1", print_r($va_related, true))
                );
                if (!$vn_new_table_num) {
                ...
                foreach ($va_related as $va_rel) {
                    ...
                    $va_rel_export = $this->processExporterItem(
                        $pn_item_id,
                        $vn_new_table_num,
                        $va_rel[$vs_key],
        ...
        if ($va_restrict_to_types = $t_exporter_item->getSetting('restrictToTypes')) {
            $va_get_options['restrictToTypes'] = is_array(
        ...
        if ($va_restrict_to_relationship_types = $t_exporter_item->getSetting('restrictToRelationshipTypes')) {
            $va_get_options['restrictToRelationshipTypes'] = is_array(
        ...
        if ($vn_attribute_id) {
            ...
            if ($vs_template) { // if template is set, run through template engine as <unit>
                ...
                $vs_get_with_template = trim(
                    $t_instance->getWithTemplate(
                        "
                ...
                if (preg_match("/^_CONSTANT_:(.*)$/", $vs_source, $va_matches)) {
                    $o_log->logDebug(
                        _t("This is a constant in attribute mode. Value for this mapping is '%1'", trim($va_matches[1]))
                    );
                    ...
                    $o_log->logDebug(
                        _t("Trying to find code %1 in value array for the current attribute.", $vs_source)
                    );
                    ...
                    foreach ($va_values as $vo_val) {
                        ...
                        switch ($vo_val->getDatatype()) {
                            case __CA_ATTRIBUTE_VALUE_LIST__: //if ($vo_val instanceof ListAttributeValue) {
                                ...
                                if ($t_exporter_item->getSetting('returnIdno') || $t_exporter_item->getSetting(
                                        'convertCodesToIdno'
                                    )) {
                            ...
                            default:
                                $o_log->logDebug(
                                    _t(
                                        "Trying to match code from array %1 and the code we're looking for %2.",
                                        $vo_val->getElementCode(),
                                        $vs_source
                                    )
                                );
                                if ($vo_val->getElementCode() == $vs_source) {
            ...
            if ($vs_source) {
                ...
                if (preg_match("/^_CONSTANT_:(.*)$/", $vs_source, $va_matches)) {
                    ...
                    if (in_array(
                        ...
                        if (isset($pa_options[$vs_source]) && strlen($pa_options[$vs_source]) > 0) {
                            $o_log->logDebug(
                                _t(
                                    "Source refers to releationship type information. Value for this mapping is '%1'",
                        ...
                        if (!$vb_repeat) {
                            ...
                            $o_log->logDebug(
                                _t("Source is a simple get() for some bundle. Value for this mapping is '%1'", $vs_get)
                            );
                            ...
                            $o_log->logDebug(
                                _t(
                                    "Source is a get() that should be repeated for multiple values. Value for this mapping is '%1'. It includes the custom delimiter ';#;' that is later used to split the value into multiple values.",
                                    $vs_values
                                )
                            );
                            $o_log->logDebug(_t("get() options are: %1", print_r($va_get_options, true)));
                            ...
                            foreach ($va_values as $vn_i => $vs_text) {
                                ...
                                if ($vs_skip_if_expr) {
                                    ...
                                    $va_vars = array_merge(
                                        array('value' => $vs_text, 'value_raw' => $va_values_raw[$vn_i]),
                ...
                if ($vs_template) {
                    ...
                    $vs_proc_template = caProcessTemplateForIDs(
                        $vs_template,
                        $pn_table_num,
                        array($pn_record_id),
                        array()
                    ...
                    $o_log->logDebug(
                        _t(
                            "Current mapping has no source but a template '%1'. Value from extracted via template processor is '%2'",
                            $vs_template,
                            $vs_proc_template
                        )
                    );
                    ...
                    $o_log->logDebug(
                        _t("Current mapping has no source and no template and is probably an XML/MARC wrapper element")
                    );
        ...
        $o_log->logDebug(
            _t(
                "We're now processing other settings like default, prefix, suffix, skipIfExpression, filterByRegExp, maxLength, plugins and replacements for this mapping"
            )
        );
        ...
        foreach ($va_item_info as $vn_key => &$va_item) {
            $this->opo_app_plugin_manager->hookExportItemBeforeSettings(
                array(
                    'instance' => $t_instance,
            ...
            $this->opo_app_plugin_manager->hookExportItem(
                array(
                    'instance' => $t_instance,
                    'exporter_item_instance' => $t_exporter_item,
                    'export_item' => &$va_item
        ...
        if (is_array($va_children) && sizeof($va_children) > 0) {
            $o_log->logInfo(
                _t("Now proceeding to process %1 direct children in the mapping hierarchy", sizeof($va_children))
            );

            foreach ($va_children as $va_child) {
                foreach ($va_item_info as &$va_info) {
                    $va_child_export = $this->processExporterItem(
                        $va_child['item_id'],
                        $pn_table_num,
                        $pn_record_id,
                        $pa_options
    ...
    public static function writeExporterToFile($exporter_code, $file)
    {
        ...
        if (is_array($items = $exporter->getItems())) {
            ...
            foreach ($items as $item) {
                ...
                $o_sheet->setCellValue(
                    $a_to_z[5] . $line,
                    (is_array($item_settings) && sizeof($item_settings)) ? json_encode(
                        $item_settings
                    ) : ''
                );
            }
