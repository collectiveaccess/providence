...
Timer::start("PROCESS");
		...
		if (MemoryCache::contains("exporter_item_{$pn_item_id}")) {
			...
			////Timer::p("OAI", "[BEFORE LOAD FOR $pn_table_num/$pn_record_id] %time<br>");
		if (!($t_instance = ca_data_exporters::loadInstanceByID($pn_record_id, $pn_table_num, $pa_options))) {
			...
			////Timer::p("OAI", "[AFTER LOAD FOR $pn_table_num/$pn_record_id] %time<br>");
...
//Timer::p("PROCESS", "[$pn_item_id/context switch] %time<br>\n");
			switch($vs_context) {
				case 'children':
...
Timer::p("PROCESS", "[$pn_item_id/end context switch] %time<br>\n");
			return $va_info;
		}
		// end switch context
...
//Timer::p("PROCESS", "[$pn_item_id/params] %time<br>\n");
...
//Timer::p("PROCESS", "[$pn_item_id/attr template] %time<br>\n");
...
//Timer::p("PROCESS", "[$pn_item_id/attr source] %time<br>\n");
				}
...
//Timer::p("PROCESS", "[$pn_item_id/source_1] %time<br>\n");
...
//Timer::p("PROCESS", "[$pn_item_id/source_2] %time<br>\n");
				}
			} else {
				if(!$vb_repeat) {
//Timer::p("PROCESS", "[$pn_item_id/source_3.1] %time<br>\n");
...
//Timer::p("PROCESS", "[$pn_item_id/source_3.2] %time<br>\n");
					$va_item_info[] = [
						'text' => $vs_get,
						'text_raw' => $return_raw_data ? $t_instance->get($vs_source) : null,
						'element' => $vs_element,
					];
//Timer::p("PROCESS", "[$pn_item_id/source_3.3] %time<br>\n");
...
//Timer::p("PROCESS", "[$pn_item_id/source_4] %time<br>\n");
				}
				
//Timer::p("PROCESS", "[$pn_item_id/source] %time<br>\n");
			}
		} else if($vs_template) {
...
//Timer::p("PROCESS", "[$pn_item_id/template withput source] %time<br>\n");	
...
//Timer::p("PROCESS", "[$pn_item_id/$pn_record_id] PROCESS %time<Br>\n");
		return $va_item_info;
	}
	# ------------------------------------------------------
