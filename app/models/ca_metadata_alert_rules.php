...
BaseModel::$s_ca_models_definitions['ca_metadata_alert_rules'] = array(
    ...
    'FIELDS' => array(
        ...
        'table_num' => array(
            ...
            'DESCRIPTION' => _t(
                'Determines what kind of item (objects, entities, places, etc.) is covered by the alert .'
            ),
...
class ca_metadata_alert_rules extends BundlableLabelableBaseModelWithAttributes
{
    ...
    static public $s_have_access_to_rule_cache = [];
    ...
    public function __construct($pn_id = null)
    {
        ...
        BaseModel::$s_ca_models_definitions['ca_metadata_alert_rules']['FIELDS']['table_num']['BOUNDS_CHOICE_LIST'] = array_flip(
            caGetPrimaryTables(true)
        );
    }
    ...
    public function getRules($pa_options = null)
    {
        ...
        $pb_dont_include_subtypes_in_type_restriction = caGetOption(
        ...
        while ($qr_res->nextRow()) {
            ...
            if (!isset($va_type_name_cache[$vn_table_num]) || !($vs_display_type = $va_type_name_cache[$vn_table_num])) {
                $vs_display_type = $va_type_name_cache[$vn_table_num] = $this->getMetadataAlertRuleTypeName(
                    $vn_table_num,
                    array('number' => 'plural')
                );
            }
            $va_rules[$vn_rule_id][$qr_res->get('locale_id')] = array_merge(
                $qr_res->getRow(),
                array(
                    'metadata_alert_rule_content_type' => $vs_display_type,
                    'trigger_types' => is_array(
    ...
    public function haveAccessToForm($pn_user_id, $pn_access, $pn_rule_id = null)
    {
        ...
        if ((bool)$t_rule->get(
                'is_system'
            ) && ($pn_access == __CA_ALERT_RULE_ACCESS_NOTIFICATION__)) {    // system forms are readable by all
            return ca_metadata_alert_rules::$s_have_access_to_rule_cache[$vn_rule_id . '/' . $pn_user_id . '/' . $pn_access] = true;
        }
    ...
    public function getTriggers(array $pa_options = [])
    {
        ...
        $qr_triggers = $this->getDb()->query(
            'SELECT * FROM ca_metadata_alert_triggers WHERE rule_id = ?',
            $this->getPrimaryKey()
        );
        ...
        while ($qr_triggers->nextRow()) {
            ...
            $va_return[$qr_triggers->get('trigger_id')]['settings'] = caUnserializeForDatabase(
                $qr_triggers->get('settings')
            );
            $va_return[$qr_triggers->get('trigger_id')]['element_filters'] = caUnserializeForDatabase(
                $qr_triggers->get('element_filters')
            );
        }
    ...
    public function saveTriggerHTMLFormBundle($po_request, $ps_form_prefix, $ps_placement_code)
    {
        ...
        if (in_array(
            $_REQUEST["{$vs_id_prefix}_element_id"],
