...
BaseModel::$s_ca_models_definitions['ca_metadata_alert_rules'] = array(
	...
	'FIELDS'        => array(
		'rule_id'   => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this rule' )
		),
		'user_id'   => array(
			'FIELD_TYPE'     => FT_NUMBER,
		...
		'table_num' => array(
			...
			'LABEL'              => _t( 'Alert target' ),
			'DESCRIPTION'        => _t( 'Determines what kind of item (objects, entities, places, etc.) is covered by the alert .' ),
		...
		'code'      => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Code' ),
			'DESCRIPTION'    => _t( 'Short code for rule (must be unique)' ),
...
class ca_metadata_alert_rules extends BundlableLabelableBaseModelWithAttributes {
	...
	public function __construct( $pn_id = null ) {
		...
		BaseModel::$s_ca_models_definitions['ca_metadata_alert_rules']['FIELDS']['table_num']['BOUNDS_CHOICE_LIST']
			= array_flip( caGetPrimaryTables( true ) );
	}
	...
	protected function initLabelDefinitions( $pa_options = null ) {
		...
		$this->BUNDLES['ca_users']       = array(
			'type'      => 'special',
			'repeating' => true,
			'label'     => _t( 'Recipient users' )
		);
		$this->BUNDLES['ca_user_groups'] = array(
			'type'      => 'special',
			'repeating' => true,
			'label'     => _t( 'Recipient user groups' )
		);

		$this->BUNDLES['ca_metadata_alert_rule_type_restrictions'] = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Type restrictions' )
		);
		$this->BUNDLES['ca_metadata_alert_triggers']               = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Trigger' )
		);
	 ...
	 *                          table = If set, list is restricted to rules that pertain to the specified table. You
	 *                          can pass a table name or number. If omitted rules for all tables will be returned.
	 *                          user_id = Restricts returned rules to those accessible by the current user. If omitted
	 *                          then all rules, regardless of access are returned. restrictToTypes = Restricts returned
	 *                          rules to those bound to the specified type. Default is to not restrict by type.
	 *                          dontIncludeSubtypesInTypeRestriction = If restrictToTypes is set, controls whether or
	 *                          not subtypes are automatically included in the restriction. Default is false – subtypes
	 *                          are included. access = Restricts returned rules to those with at least the specified
	 *                          access level for the specified user. If user_id is omitted then this option has no
	 *                          effect. If user_id is set and this option is omitted, then rules where the user has at
	 *                          least read access will be returned.
	 *
	 * @return array
	 */
	public function getRules( $pa_options = null ) {
		...
		$pb_dont_include_subtypes_in_type_restriction = caGetOption( 'dontIncludeSubtypesInTypeRestriction',
			$pa_options, false );
		...
		if ( is_array( $pa_restrict_to_types ) && sizeof( $pa_restrict_to_types ) ) {
			$va_type_list = caMakeTypeIDList( $pm_table_name_or_num, $pa_restrict_to_types,
				array( 'dontIncludeSubtypesInTypeRestriction' => $pb_dont_include_subtypes_in_type_restriction ) );
		...
		while ( $qr_res->nextRow() ) {
			...
			if ( ! isset( $va_type_name_cache[ $vn_table_num ] )
				...
				$vs_display_type
					= $va_type_name_cache[ $vn_table_num ] = $this->getMetadataAlertRuleTypeName( $vn_table_num,
					array( 'number' => 'plural' ) );
			}
			$va_rules[ $vn_rule_id ][ $qr_res->get( 'locale_id' ) ] = array_merge( $qr_res->getRow(), array(
				'metadata_alert_rule_content_type' => $vs_display_type,
				'trigger_types'                    => is_array( $va_triggers_by_rule_id[ $vn_rule_id ] ) ? join( ', ',
					$va_triggers_by_rule_id[ $vn_rule_id ] ) : ""
			) );
		}
	 ...
	 * @param int $pn_access  type of access required. Use __CA_ALERT_RULE_ACCESS_NOTIFICATION__ for read-only access
	 *                        or __CA_ALERT_RULE_ACCESS_ACCESS_EDIT__ for editing (full) access
	...
	public function haveAccessToForm( $pn_user_id, $pn_access, $pn_rule_id = null ) {
		...
		// return from cache
		if ( isset( ca_metadata_alert_rules::$s_have_access_to_rule_cache[ $vn_rule_id . '/' . $pn_user_id . '/'
		                                                                   . $pn_access ] )
		) {
			return ca_metadata_alert_rules::$s_have_access_to_rule_cache[ $vn_rule_id . '/' . $pn_user_id . '/'
			                                                              . $pn_access ];
		}
		...
		if ( (bool) $t_rule->get( 'is_system' )
		     && ( $pn_access == __CA_ALERT_RULE_ACCESS_NOTIFICATION__ )
		...
		return
			ca_metadata_alert_rules::$s_have_access_to_rule_cache[ $vn_rule_id . '/' . $pn_user_id . '/' . $pn_access ]
				= false;
	}
	# ------------------------------------------------------
	 ...
	 *
	 * @param int   $pm_table_name_or_num Table number to return name for. If omitted then the name for the content
	 *                                    type contained by the current bundle display will be returned. Use this
	 *                                    parameter if you want to force a content type without having to load a bundle
	 *                                    display.
	 * @param array $pa_options           Optional array of options. Supported options are:
	 ...
	 * @return string
	 */
	public function getTriggerHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, array $pa_bundle_settings = [], array $pa_options = []
	) {
		$o_view = new View( $po_request, $po_request->getViewsDirectoryPath() . '/bundles/' );
	...
	public function getTriggers( array $pa_options = [] ) {
		...
		$qr_triggers = $this->getDb()->query( 'SELECT * FROM ca_metadata_alert_triggers WHERE rule_id = ?',
			$this->getPrimaryKey() );
		...
		while ( $qr_triggers->nextRow() ) {
			...
			$va_return[ $qr_triggers->get( 'trigger_id' ) ]['settings']
			                                                = caUnserializeForDatabase( $qr_triggers->get( 'settings' ) );
			$va_return[ $qr_triggers->get( 'trigger_id' ) ]['element_filters']
			                                                = caUnserializeForDatabase( $qr_triggers->get( 'element_filters' ) );
		}
	...
	public function saveTriggerHTMLFormBundle( $po_request, $ps_form_prefix, $ps_placement_code ) {
		...
		if ( in_array( $_REQUEST["{$vs_id_prefix}_element_id"],
			[ '_intrinsic_idno', '_preferred_labels', '_nonpreferred_labels' ] )
		) {
