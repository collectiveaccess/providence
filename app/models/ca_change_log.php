...
BaseModel::$s_ca_models_definitions['ca_change_log'] = array(
	...
	'FIELDS'        => array(
		'log_id'           => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Log id',
			'DESCRIPTION'    => 'Identifier for Log'
		),
		'log_datetime'     => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Logged date and time' ),
			'DESCRIPTION'    => _t( 'Date and time logged event occurred' )
		),
		'user_id'          => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_FIELD,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'User' ),
			'DESCRIPTION'    => _t( 'User who performed event' )
		),
		'changetype'       => array(
			'FIELD_TYPE'     => FT_TEXT,
			'DISPLAY_TYPE'   => DT_FIELD,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Action' ),
			'DESCRIPTION'    => _t( 'Type of action performed by user' ),
		...
		'logged_table_num' => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Table',
			'DESCRIPTION'    => 'Table to which action was applied',
		...
		'logged_row_id'    => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Row id',
			'DESCRIPTION'    => 'Identifier of row to which action was applied'
		),
		'unit_id'          => array(
			'FIELD_TYPE'     => FT_TEXT,
			'DISPLAY_TYPE'   => DT_FIELD,
...
class ca_change_log extends BaseModel {
	...
	public static function getLog( $pn_from, $pn_limit = null, $pa_options = null, &$pa_media = null ) {
		...
		if ( $ps_for_logged_guid ) {            // pull based upon logged GUID only
			...
			if ( sizeof( $va_only_tables ) ) {
				$vs_table_filter_sql         = 'AND (cl.logged_table_num IN (' . join( ',', $va_only_tables )
				                               . ')) AND (csub.subject_table_num IN (' . join( ',', $va_only_tables )
				                               . ') OR csub.subject_table_num IS NULL) ';
				$vs_table_filter_subject_sql = 'AND csub.subject_table_num IN (' . join( ',', $va_only_tables )
				                               . ') AND (cl.logged_table_num IN (' . join( ',', $va_only_tables )
				                               . ')) ';
			} elseif ( sizeof( $va_ignore_tables ) ) {
				$vs_table_filter_sql         = 'AND cl.logged_table_num NOT IN (' . join( ',', $va_ignore_tables )
				                               . ') AND (csub.subject_table_num NOT IN (' . join( ',',
						$va_ignore_tables ) . ') OR csub.subject_table_num IS NULL) ';
				$vs_table_filter_subject_sql = 'AND (csub.subject_table_num NOT IN (' . join( ',', $va_ignore_tables )
				                               . ')) AND (cl.logged_table_num NOT IN (' . join( ',', $va_ignore_tables )
				                               . ')) ';
			}
			$qr_results = $o_db->query( "
				(SELECT cl.log_id i, cl.*, cls.* 
			...
			$qr_results = $o_db->query( "
				...
				{$vs_limit_sql}
			", [ $pn_from_datetime ] );
		} elseif ( caGetOption( 'includeSubjectEntries', $pa_options,
			false )
		) {        // pull based upon log_id with subject log entries included
			if ( sizeof( $va_only_tables ) ) {
				$vs_table_filter_sql = 'AND (cl.logged_table_num IN (' . join( ',', $va_only_tables )
				                       . ') OR clsub.subject_table_num IN (' . join( ',', $va_only_tables ) . '))';
			} elseif ( sizeof( $va_ignore_tables ) ) {
				$vs_table_filter_sql = 'AND (cl.logged_table_num NOT IN (' . join( ',', $va_ignore_tables )
				                       . ') OR clsub.subject_table_num NOT IN (' . join( ',', $va_ignore_tables )
				                       . '))';
			}
			$qr_results = $o_db->query( "
		...
		if ( $qr_results ) {
			if ( ( $qr_results->numRows() == 0 ) && $ps_for_guid ) {
				...
				if ( is_array( $va_guid_info = ca_guids::getInfoForGUID( $ps_for_guid ) )
				     && ( $va_guid_info['table_num'] == 4 )
				) {
					$qr_attr = $o_db->query( "SELECT * FROM ca_attributes WHERE attribute_id = ?",
						[ $va_guid_info['row_id'] ] );
					if ( $qr_attr->nextRow() ) {
			...
			while ( $qr_results->nextRow() ) {
				...
				if ( ! ( $vs_guid = ca_guids::getForRow( $qr_results->get( 'logged_table_num' ),
					$qr_results->get( 'logged_row_id' ) ) )
				) {
					continue;
				}
				$va_row['guid'] = $vs_guid;
				...
				foreach ( $va_snapshot as $vs_fld => $vm_val ) {
					switch ( $vs_fld ) {
						case 'source_info':
							if ( is_array( $va_snapshot['source_info'] )
							     && ( ( $vn_s
										= sizeof( $va_snapshot['source_info'] ) ) > 1000 )
							) {
								ReplicationService::$s_logger->log( "[" . $qr_results->get( 'log_id' )
								                                    . "] LARGE SOURCE INFO ($vn_s) FOUND IN $vs_table_name" );
							}
							$va_snapshot['source_info']
								= '';       // this field should be blank but in older systems may have a ton of junk data
							break;
						case 'element_id':
							if ( preg_match( "!^ca_metadata_element!", $t_instance->tableName() ) ) {
								...
								$vs_table_name
									= Datamodel::getTableName( ca_attributes::getTableNumForAttribute( $va_snapshot['attribute_id'] ) );

								// Skip elements not in include list, when a list is provided for the current table
								if ( ! $vs_table_name
								     || ( is_array( $pa_include_metadata[ $vs_table_name ] )
								          && ! isset( $pa_include_metadata[ $vs_table_name ][ $vs_code ] ) )
								) {
								...
								if ( is_array( $pa_exclude_metadata[ $vs_table_name ] )
								     && isset( $pa_exclude_metadata[ $vs_table_name ][ $vs_code ] )
								) {
									$va_snapshot = [ 'SKIP' => true ];
									continue( 2 );
								}
						...
						case 'type_id':
							if ( preg_match( "!^ca_relationship_type!", $t_instance->tableName() ) ) {
								...
								if ( $t_instance instanceof BaseRelationshipModel ) {
									...
									if ( ! ( $va_snapshot['type_code'] = caGetListItemIdno( $vm_val ) )
									     && ( ! $t_instance->getFieldInfo( 'type_id', 'IS_NULL' ) )
									) {
										continue( 2 );
									}
						...
						default:
							...
							if (
								...
								( Datamodel::getTableName( (int) $qr_results->get( 'logged_table_num' ) )
								  == 'ca_attribute_values' )
								&& ( $vs_fld == 'item_id' )
							) {
							...
							if ( ! is_null( $vm_val ) && ( $va_fld_info = $t_instance->getFieldInfo( $vs_fld ) ) ) {
								...
								if ( isset( $va_fld_info['LIST'] ) ) {
									$va_snapshot[ $vs_new_fld ]
										= caGetListItemIdno( caGetListItemIDForValue( $va_fld_info['LIST'], $vm_val ) );
								...
								if ( $vs_fld == $t_instance->getProperty( 'HIERARCHY_PARENT_ID_FLD' ) ) {
									...
									$va_snapshot[ $vs_fld . '_guid' ] = ca_guids::getForRow( $t_instance->tableNum(),
										$vm_val );
								} elseif ( isset( $va_many_to_one_rels[ $vs_fld ] )
								           && ( $t_rel_item
										= Datamodel::getInstanceByTableName( $va_many_to_one_rels[ $vs_fld ]['one_table'],
										true ) )
								) {
									// handle many-one keys
									$va_snapshot[ $vs_fld . '_guid' ] = ca_guids::getForRow( $t_rel_item->tableNum(),
										$vm_val );
								} else {
								...
								if ( ( $va_fld_info['FIELD_TYPE'] === FT_MEDIA )
									...
									while ( $qr_future_entries->nextRow() ) {
										$va_future_snap
											= caUnserializeForDatabase( $qr_future_entries->get( 'snapshot' ) );
									...
									if ( $pb_is_latest ) {
										...
										if ( is_string( $va_snapshot[ $vs_fld ] ) ) {
											if ( $va_row['logged_table_num'] == 3 ) {
												...
												$va_snapshot[ $vs_fld ] = $t_instance->getMediaUrl( $vs_fld,
													'original' );
												$va_snapshot[ $vs_fld . "_media_desc" ]
												                        = array_shift( $t_instance->get( $vs_fld,
													array( 'returnWithStructure' => true ) ) );
											}
								...
								if ( $t_instance instanceof BaseRelationshipModel ) {
									if ( $vs_fld == $t_instance->getProperty( 'RELATIONSHIP_LEFT_FIELDNAME' ) ) {
										$vs_left_guid                     = ca_guids::getForRow( $t_instance->getLeftTableNum(),
											$vm_val );
									...
									if ( $vs_fld == $t_instance->getProperty( 'RELATIONSHIP_RIGHT_FIELDNAME' ) ) {
										$vs_right_guid                    = ca_guids::getForRow( $t_instance->getRightTableNum(),
											$vm_val );
								...
								if ( $t_instance instanceof BaseLabel ) {
									...
									if ( $vs_fld == $t_instance->getSubjectKey() ) {
										...
										$va_snapshot[ $vs_label_subject_guid_field ]
										                             = ca_guids::getForRow( $t_instance->getSubjectTableInstance()
										                                                               ->tableNum(),
											$vm_val );
									}
								}
								...
								if ( ( $t_instance instanceof \ca_representation_annotations )
								     && ( $vs_fld == 'representation_id' )
								) {
									$va_snapshot['representation_guid']
										= ca_guids::getForRow( Datamodel::getTableNum( 'ca_object_representations' ),
										$vm_val );
								}
							}
							break;
				...
				if ( ( $t_instance->tableNum() == 3 )
				     && caGetOption( 'forceValuesForAllAttributeSLots', $pa_options, false )
				) {
					foreach (
						[
							'value_longtext1',
				...
				if ( $vb_synth_attr_log_entry ) {
					$vs_guid     = ca_guids::getForRow( $qr_attr->get( 'table_num' ), $qr_attr->get( 'row_id' ),
						[ 'dontAdd' => true ] );
					$qr_subjects = $o_db->query( "SELECT 1 as log_id, " . $qr_attr->get( 'table_num' )
					                             . " as subject_table_num, " . $qr_attr->get( 'row_id' )
					                             . " as subject_row_id, '{$vs_guid}' as guid" );
				} else {
					$qr_subjects = $o_db->query( "SELECT * FROM ca_change_log_subjects WHERE log_id=?",
						$qr_results->get( 'log_id' ) );
				}

				while ( $qr_subjects->nextRow() ) {
					// skip subjects without GUID -- we don't care about those
					if ( ! ( $vs_subject_guid = ca_guids::getForRow( $qr_subjects->get( 'subject_table_num' ),
						$qr_subjects->get( 'subject_row_id' ) ) )
					) {
						continue;
					}
					...
					if ( isset( $pa_skip_if_expression[ $vs_subject_table_name ] ) ) {
						...
						foreach ( ExpressionParser::getVariableList( $vs_exp ) as $vs_var_name ) {
							$va_exp_vars[ $vs_var_name ] = $t_subject_instance->get( $vs_var_name,
					...
					$va_row['subjects'][] = array_replace( $qr_subjects->getRow(),
						array( 'guid' => $vs_subject_guid ) );
				}
	...
	static public function logEntryHasAccess( $pa_log_entry, $pa_access ) {
		...
		if ( $t_instance->hasField( 'access' ) ) {
			$qr_res = $o_db->query( "SELECT access FROM " . $t_instance->tableName() . " WHERE "
			                        . $t_instance->primaryKey() . " = ?", [ (int) $pa_log_entry['logged_row_id'] ] );
			if ( $qr_res->nextRow() ) {
	...
	static public function rowHasAccess( $pn_table_num, $pn_row_id, $pa_access ) {
		...
		if ( $t_instance->hasField( 'access' ) ) {
			$qr_res = $o_db->query( "SELECT access FROM " . $t_instance->tableName() . " WHERE "
			                        . $t_instance->primaryKey() . " = ?", [ (int) $pn_row_id ] );
			if ( $qr_res->nextRow() ) {
				if ( in_array( $vn_access = $qr_res->get( 'access' ), $pa_access ) ) {
					return true;
				}
			...
			if ( $t_left->hasField( 'access' )
			     && ( ! $t_left->load( $t_instance->get( $t_instance->getLeftTableFieldName() ) )
			          || ! in_array( $t_left->get( 'access' ), $pa_access ) )
			) {
				return false;
			}
			if ( $t_right->hasField( 'access' )
			     && ( ! $t_right->load( $t_instance->get( $t_instance->getRightTableFieldName() ) )
			          || ! in_array( $t_right->get( 'access' ), $pa_access ) )
			) {
				return false;
			}
