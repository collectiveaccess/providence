...
class ca_change_log extends BaseModel
{
    ...
    public static function getLog($pn_from, $pn_limit = null, $pa_options = null, &$pa_media = null)
    {
        ...
        if ($ps_for_logged_guid) {            // pull based upon logged GUID only
            ...
            if (sizeof($va_only_tables)) {
                $vs_table_filter_sql = 'AND (cl.logged_table_num IN (' . join(
                        ',',
                        $va_only_tables
                    ) . ')) AND (csub.subject_table_num IN (' . join(
                        ',',
                        $va_only_tables
                    ) . ') OR csub.subject_table_num IS NULL) ';
                $vs_table_filter_subject_sql = 'AND csub.subject_table_num IN (' . join(
                        ',',
                        $va_only_tables
                    ) . ') AND (cl.logged_table_num IN (' . join(',', $va_only_tables) . ')) ';
            } elseif (sizeof($va_ignore_tables)) {
                $vs_table_filter_sql = 'AND cl.logged_table_num NOT IN (' . join(
                        ',',
                        $va_ignore_tables
                ...
                $vs_table_filter_subject_sql = 'AND (csub.subject_table_num NOT IN (' . join(
                        ',',
                        $va_ignore_tables
        ...
        if ($qr_results) {
            if (($qr_results->numRows() == 0) && $ps_for_guid) {
                ...
                if (is_array(
                        $va_guid_info = ca_guids::getInfoForGUID($ps_for_guid)
                    ) && ($va_guid_info['table_num'] == 4)) {
                    $qr_attr = $o_db->query(
                        "SELECT * FROM ca_attributes WHERE attribute_id = ?",
                        [$va_guid_info['row_id']]
                    );
                    if ($qr_attr->nextRow()) {
            ...
            while ($qr_results->nextRow()) {
                ...
                if (!($vs_guid = ca_guids::getForRow(
                    $qr_results->get('logged_table_num'),
                ...
                foreach ($va_snapshot as $vs_fld => $vm_val) {
                    switch ($vs_fld) {
                        case 'source_info':
                            if (is_array($va_snapshot['source_info']) && (($vn_s = sizeof(
                                        $va_snapshot['source_info']
                                    )) > 1000)) {
                                ReplicationService::$s_logger->log(
                                    "[" . $qr_results->get(
                                        'log_id'
                        ...
                        case 'element_id':
                            if (preg_match("!^ca_metadata_element!", $t_instance->tableName())) {
                                ...
                                $vs_table_name = Datamodel::getTableName(
                                    ca_attributes::getTableNumForAttribute($va_snapshot['attribute_id'])
                                );

                                // Skip elements not in include list, when a list is provided for the current table
                                if (!$vs_table_name || (is_array(
                                            $pa_include_metadata[$vs_table_name]
                                ...
                                if (is_array(
                                        $pa_exclude_metadata[$vs_table_name]
                        ...
                        case 'type_id':
                            if (preg_match("!^ca_relationship_type!", $t_instance->tableName())) {
                                ...
                                if ($t_instance instanceof BaseRelationshipModel) {
                                    ...
                                    if (!($va_snapshot['type_code'] = caGetListItemIdno(
                                            $vm_val
                        ...
                        default:
                            ...
                            if (
                                ...
                                (Datamodel::getTableName(
                                        (int)$qr_results->get('logged_table_num')
                                    ) == 'ca_attribute_values')
                                &&
                                ($vs_fld == 'item_id')
                            ) {
                            ...
                            if (!is_null($vm_val) && ($va_fld_info = $t_instance->getFieldInfo($vs_fld))) {
                                ...
                                if (isset($va_fld_info['LIST'])) {
                                    $va_snapshot[$vs_new_fld] = caGetListItemIdno(
                                        caGetListItemIDForValue($va_fld_info['LIST'], $vm_val)
                                    );
                                ...
                                if ($vs_fld == $t_instance->getProperty('HIERARCHY_PARENT_ID_FLD')) {
                                    ...
                                    $va_snapshot[$vs_fld . '_guid'] = ca_guids::getForRow(
                                        $t_instance->tableNum(),
                                        $vm_val
                                    );
                                } elseif (isset($va_many_to_one_rels[$vs_fld]) && ($t_rel_item = Datamodel::getInstanceByTableName(
                                        $va_many_to_one_rels[$vs_fld]['one_table'],
                                        true
                                    ))) {
                                ...
                                if (($va_fld_info['FIELD_TYPE'] === FT_MEDIA)
                                    ...
                                    $qr_future_entries = $o_db->query(
										...
										",
                                        $va_row['log_id'],
                                        $va_row['logged_row_id'],
                                        $va_row['logged_table_num']
                                    );
                                    ...
                                    if ($pb_is_latest) {
                                        ...
                                        if (is_string($va_snapshot[$vs_fld])) {
                                            if ($va_row['logged_table_num'] == 3) {
                                                ...
                                                $va_snapshot[$vs_fld . "_media_desc"] = array_shift(
                                                    $t_instance->get($vs_fld, array('returnWithStructure' => true))
                                                );
                                            }
                                        } elseif (is_array(
                                            $va_snapshot[$vs_fld]
                                        )) { // back in the day it would store the full media array here
                                ...
                                if ($t_instance instanceof BaseLabel) {
                                    if ($vs_fld == $t_instance->getSubjectKey()) {
                                        ...
                                        $va_snapshot[$vs_label_subject_guid_field] = ca_guids::getForRow(
                                            $t_instance->getSubjectTableInstance()->tableNum(),
                                            $vm_val
                                        );
                                    }
                                }
                                ...
                                if (($t_instance instanceof ca_representation_annotations) && ($vs_fld == 'representation_id')) {
                                    $va_snapshot['representation_guid'] = ca_guids::getForRow(
                                        Datamodel::getTableNum('ca_object_representations'),
                                        $vm_val
                                    );
                                }
                            }
                ...
                if (($t_instance->tableNum() == 3) && caGetOption(
                        'forceValuesForAllAttributeSLots',
                        $pa_options,
                        false
                    )) {
                    foreach (
                ...
                if ($vb_synth_attr_log_entry) {
                    $vs_guid = ca_guids::getForRow(
                        $qr_attr->get('table_num'),
                        $qr_attr->get('row_id'),
                        ['dontAdd' => true]
                    );
                    $qr_subjects = $o_db->query(
                        "SELECT 1 as log_id, " . $qr_attr->get('table_num') . " as subject_table_num, " . $qr_attr->get(
                            'row_id'
                        ) . " as subject_row_id, '{$vs_guid}' as guid"
                    );
                } else {
                    $qr_subjects = $o_db->query(
                        "SELECT * FROM ca_change_log_subjects WHERE log_id=?",
                ...
                while ($qr_subjects->nextRow()) {
                    ...
                    if (!($vs_subject_guid = ca_guids::getForRow(
                        $qr_subjects->get('subject_table_num'),
                    ...
                    if (isset($pa_skip_if_expression[$vs_subject_table_name])) {
                        ...
                        foreach (ExpressionParser::getVariableList($vs_exp) as $vs_var_name) {
                            $va_exp_vars[$vs_var_name] = $t_subject_instance->get(
                                $vs_var_name,
    ...
    public static function logEntryHasAccess($pa_log_entry, $pa_access)
    {
        ...
        if ($t_instance->hasField('access')) {
            $qr_res = $o_db->query(
                "SELECT access FROM " . $t_instance->tableName() . " WHERE " . $t_instance->primaryKey() . " = ?",
                [(int)$pa_log_entry['logged_row_id']]
            );
            if ($qr_res->nextRow()) {
    ...
    public static function rowHasAccess($pn_table_num, $pn_row_id, $pa_access)
    {
        ...
        if ($t_instance->hasField('access')) {
            ...
            if ($t_left->hasField('access') && (!$t_left->load(
                        $t_instance->get($t_instance->getLeftTableFieldName())
                    ) || !in_array($t_left->get('access'), $pa_access))) {
                return false;
            }
            if ($t_right->hasField('access') && (!$t_right->load(
                        $t_instance->get($t_instance->getRightTableFieldName())
