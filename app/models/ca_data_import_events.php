...
BaseModel::$s_ca_models_definitions['ca_data_import_events'] = array(
	...
	'FIELDS'        => array(
		'event_id'    => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Event id',
			'DESCRIPTION'    => 'Identifier for import event'
		),
		'user_id'     => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'User' ),
			'DESCRIPTION'    => _t( 'User who performed data import' )
		),
		'occurred_on' => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Began on' ),
			'DESCRIPTION'    => _t( 'Date and time import began' )
		),
		'type_code'   => array(
			'FIELD_TYPE'     => FT_TEXT,
			'DISPLAY_TYPE'   => DT_FIELD,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Type' ),
			'DESCRIPTION'    => _t( 'Code indicating type of import (eg. OAI)' ),
		...
		'description' => array(
			...
			'DESCRIPTION'    => _t( 'Description of data import event' ),
		...
		'source'      => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Source' ),
			'DESCRIPTION'    => _t( 'Text indicating source of data imported. For OAI and other web service imports, this will be the URL used to access the service.' ),
			'BOUNDS_LENGTH'  => array( 0, 65535 )
		)
	)
);
...
class ca_data_import_events extends BaseModel {
	...
	/**
	 * @property $opn_start_time Microtime recorded on call to ca_data_import_events::beginItem(); used to calculate
	 *           elapsed time on call to ca_data_import_events::endItem()
	 */
	 ...
	 * @param string $ps_typecode          Code indicating import action. Valid values are 'I' for insert (creation of
	 *                                     new record in database) or 'U' for update (updated existing record).
	 *
	 * @return int Returns id  for newly created item on success, false on error or null if no event is currently
	 *             loaded.
	 */
	 ...
	 *                                     new record in database) or 'U' for update (updated existing record).
	 *
	 * @return bool Returns id for newly created item on success, false on error or null if no event is currently
	 *              loaded.
	 */
	 ...
	 *
	 * @return array List of imported items. Each element of the list is an array with keys corresponding to fields in
	 *               ca_data_import_items and values for each field.
	 */
	 ...
	 * @param int   $pn_row_id            Primary key value of the imported item
	 *
	 * @return int Unix timestamp indicating the last date/time the specified row was modified by the current import
	 *             event. Will return null if the row has not been modified by the event, or if no event is currently
	 *             loaded.
	 */
	 ...
	 *
	 * @param string $ps_type_code A text code indicating the type of log message this is. Supported values are 'ERR'
	 *                             (error), 'WARN' (warning), 'NOTE' (notice), 'INFO' (informational), 'DEBG' (debug)
	 * @param string $ps_message   The log message
	 * @param string $ps_source    Text indicating the source of the log message
	 * @param int    $pn_item_id   Optional item_id of ca_data_import_items row that corresponds to the imported item
	 *                             this log entry is concerned with. Leave null for general event-scope log messages.
	 */
	...
	/**
	 * Returns a list of long entries associated with the currently loaded import event and, optionally, the specified
	 * imported item
	 *
	 * @param int   $pn_item_id Optional item_id of ca_data_import_items row that corresponds to the imported item to
	 *                          fetch log entries for. Leave null to obtain all log entries for the event.
	 * @param array $pa_options Optional array of options. [NOTE: no options are currently implemented]s
	 *
	 * @return array List of log entries. Each element of the list is an array with keys corresponding to fields in
	 *               ca_data_import_event_log and values for each field.
	 */
