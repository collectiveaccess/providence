...
BaseModel::$s_ca_models_definitions['ca_object_lots'] = array(
	...
	'FIELDS'        => array(
		...
		'type_id'              => array(
			...
			'LABEL'          => _t( 'Type' ),
			'DESCRIPTION'    => _t( 'The type of the object lot. In CollectiveAccess every lot has a single "instrinsic" type that determines the set of descriptive and administrative metadata that can be applied to it.' )
		),
		'lot_status_id'        => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_SELECT,
			...
			'DESCRIPTION'    => _t( 'Indicates accession/collection status of lot. (eg. accessioned, pending accession, loan, non-accessioned item, etc.)' )
		),
		'idno_stub'            => array(
			'FIELD_TYPE'     => FT_TEXT,
		...
		'extent'               => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Extent' ),
			'DESCRIPTION'    => _t( 'The extent of the object lot. This is typically the number of discrete items that compose the lot by this record. It is stored as a whole number (eg. 1, 2, 3...).' )
		),
		'extent_units'         => array(
			'FIELD_TYPE'     => FT_TEXT,
			'DISPLAY_TYPE'   => DT_FIELD,
		...
		'source_id'            => array(
			...
			'LABEL'                     => _t( 'Source' ),
			'DESCRIPTION'               => _t( 'Administrative source of lot. This value is often used to indicate the administrative sub-division or legacy database from which the object originates, but can also be re-tasked for use as a simple classification tool if needed.' )
		),
		'source_info'          => array(
			'FIELD_TYPE'     => FT_VARS,
			'DISPLAY_TYPE'   => DT_OMIT,
...
class ca_object_lots extends RepresentableBaseModel {
	 ...
	 * delete the related objects. It only removes their link to this lot.  Note that on error, the database maybe left
	 * in an inconsistent state where some objects are still linked to the lot. If you want to prevent this then wrap
	 * your call to removeAllObjects in a transaction and rollback the transaction on error.
	 *
	 ...
	 *
	 * @param int   $pn_lot_id  Optional lot_id to get object count for; if null then the id of the currently loaded
	 *                          lot will be used
	 * @param array $pa_options Options include:
	 *                          return = Set to "components" to return the count of component objects only; "objects"
	 *                          to return the count of objects (but not components) or "all" to return a count of any
	 *                          kind of object. [Default = "all"] noCache = If set cached object counts are generated
	 *                          from the database and any cached counts are ignored. [Default = false]
	 *
	 * @return int Number of objects related to the object lot or null if $pn_lot_id is not set and there is no
	 *             currently loaded lot
	 */
	 ...
	 *
	 * @param int   $pn_lot_id  Optional lot_id to get object list for; if null then the id of the currently loaded lot
	 *                          will be used
	 * @param array $pa_options Options include:
	 *                          return = Set to "components" to return the count of component objects only; "objects"
	 *                          to return the count of objects (but not components) or "all" to return a count of any
	 *                          kind of object. [Default = "all"] excludeChildObjects = Only return top-level objects,
	 *                          excluding sub-objects. [Default is false]
	 *
	 * @return array List of objects related to the object lot or null if $pn_lot_id is not set and there is no
	 *               currently loaded lot
	 */
	public function getObjects( $pn_lot_id = null, $pa_options = null ) {
		...
		if ( is_array( $va_component_types = $this->getAppConfig()->getList( 'ca_objects_component_types' ) )
		...
		$qr_res = $o_db->query( "
				...
				FROM ca_objects
				WHERE
					lot_id = ? AND deleted = 0 " . ( caGetOption( 'excludeChildObjects', $pa_options, false )
		...
		$qr_res = $o_db->query( "
			...
			FROM ca_objects
			WHERE
				hier_object_id IN (?) AND deleted = 0 " . ( caGetOption( 'excludeChildObjects', $pa_options, false )
				? " AND parent_id IS NULL" : "" ) . "
			ORDER BY
				idno_sort
	 ...
	 *
	 * @param array $pa_options Options include:
	 *                          reindex = force any idno that is out of the current 1-based index to be renumbered. Eg.
	 *                          if there are 12 objects in the lot, any one numbers > 12 will be renumbered to be
	 *                          between 1 and 12. [Default is false]
	 *
	...
	public function getObjectsWithNonConformingIdnos( $pa_options = null ) {
		...
		foreach ( $va_objects as $va_object ) {
			if ( ! preg_match( "!^{$vs_lot_num}{$vs_separator}!", $va_object['idno'] ) ) {
				$va_non_conforming_objects[ $va_object['object_id'] ] = array_merge( $va_object,
					[ '_reason' => 'out-of-lot' ] );
		...
		return ( is_array( $va_non_conforming_objects ) && sizeof( $va_non_conforming_objects ) )
			? $va_non_conforming_objects : false;
	}
	# ------------------------------------------------------
			...
			foreach ( $va_non_conforming_objects as $vn_object_id => $va_object_info ) {
				if ( $t_object->load( $vn_object_id ) ) {
					if ( $po_application_plugin_manager ) {
						$po_application_plugin_manager->hookBeforeSaveItem( array(
							'id'         => $vn_object_id,
							'table_num'  => $t_object->tableNum(),
							'table_name' => $t_object->tableName(),
							'instance'   => $t_object
						) );
					}
					$t_object->setMode( ACCESS_WRITE );
					...
					if ( $po_application_plugin_manager ) {
						$po_application_plugin_manager->hookSaveItem( array(
							'id'         => $vn_object_id,
							'table_num'  => $t_object->tableNum(),
							'table_name' => $t_object->tableName(),
							'instance'   => $t_object
