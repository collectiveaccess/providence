...
class ca_object_lots extends RepresentableBaseModel
{
    ...
    public function getObjects($pn_lot_id = null, $pa_options = null)
    {
        ...
        if (is_array($va_component_types = $this->getAppConfig()->getList('ca_objects_component_types')) && sizeof(
                $va_component_types
            )) {
        ...
        $qr_res = $o_db->query(
            "
				...
				FROM ca_objects
				WHERE
					lot_id = ? AND deleted = 0 " . (caGetOption(
                'excludeChildObjects',
                $pa_options,
                false
        ...
        $qr_res = $o_db->query(
			...
			FROM ca_objects
			WHERE
				hier_object_id IN (?) AND deleted = 0 " . (caGetOption(
                'excludeChildObjects',
                $pa_options,
                false
            ) ? " AND parent_id IS NULL" : "") . "
    ...
    public function getObjectsWithNonConformingIdnos($pa_options = null)
    {
        ...
        foreach ($va_objects as $va_object) {
            if (!preg_match("!^{$vs_lot_num}{$vs_separator}!", $va_object['idno'])) {
                $va_non_conforming_objects[$va_object['object_id']] = array_merge(
                    $va_object,
                    ['_reason' => 'out-of-lot']
                );
        ...
        return (is_array($va_non_conforming_objects) && sizeof(
                $va_non_conforming_objects
            ...
            foreach ($va_non_conforming_objects as $vn_object_id => $va_object_info) {
                if ($t_object->load($vn_object_id)) {
                    if ($po_application_plugin_manager) {
                        $po_application_plugin_manager->hookBeforeSaveItem(
                            array(
                                'id' => $vn_object_id,
                    ...
                    if ($po_application_plugin_manager) {
                        $po_application_plugin_manager->hookSaveItem(
                            array(
                                'id' => $vn_object_id,
                                'table_num' => $t_object->tableNum(),
                                'table_name' => $t_object->tableName(),
