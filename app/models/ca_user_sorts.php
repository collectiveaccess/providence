...
BaseModel::$s_ca_models_definitions['ca_user_sorts'] = array(
	...
	'FIELDS'        => array(
		'sort_id'   => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this sort' )
		),
		'table_num' => array(
			'FIELD_TYPE'         => FT_NUMBER,
			...
			'LABEL'              => _t( 'Sort subject' ),
			'DESCRIPTION'        => _t( 'Determines what kind of items (objects, entities, places, etc.) are being sorted.' ),
		...
		'user_id'   => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'User' ),
			'DESCRIPTION'    => _t( 'The user who created the sort.' )
		),
		'name'      => array(
			'FIELD_TYPE'     => FT_TEXT,
		...
		'rank'      => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Sort order' ),
			'DESCRIPTION'    => _t( 'The relative priority of sort when displayed in a list. Lower numbers indicate higher priority.' )
		),
		'deleted'   => array(
			'FIELD_TYPE'     => FT_BIT,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => 0,
			'LABEL'          => _t( 'Is deleted?' ),
			'DESCRIPTION'    => _t( 'Indicates if the sort is deleted or not.' ),
...
class ca_user_sorts extends BaseModel {
	...
	public function addSortBundle( $ps_bundle_name, $pn_rank = null ) {
		...
		if ( is_null( $pn_rank ) ) {
			$qr_max_rank = $this->getDb()->query( 'SELECT MAX(`rank`) AS rnk FROM ca_user_sort_items WHERE sort_id=?',
				$this->getPrimaryKey() );
	...
	public function getSortBundles() {
		...
		$qr_sort_bundles = $this->getDb()
		                        ->query( 'SELECT * FROM ca_user_sort_items WHERE sort_id=? ORDER BY `rank`, item_id',
			                        $this->getPrimaryKey() );
	...
	public function getSortBundleNames() {
		...
		$qr_sort_bundles = $this->getDb()
		                        ->query( 'SELECT bundle_name, `rank` FROM ca_user_sort_items WHERE sort_id=? ORDER BY `rank`, item_id',
			                        $this->getPrimaryKey() );
	...
	public function updateBundleNameAtRank( $pn_rank, $ps_bundle_name ) {
		$qr_sort_bundles = $this->getDb()
		                        ->query( 'SELECT bundle_name FROM ca_user_sort_items WHERE sort_id=? AND `rank`=?',
			                        $this->getPrimaryKey(), $pn_rank );
		if ( $qr_sort_bundles->numRows() > 1 ) {
			return false;
		} elseif ( $qr_sort_bundles->numRows() == 1 ) {
			$this->getDb()
			     ->query( 'UPDATE ca_user_sort_items SET bundle_name=? WHERE sort_id=? AND `rank`=?', $ps_bundle_name,
				     $this->getPrimaryKey(), $pn_rank );
		} else {
	...
	public function removeBundleByItemID( $pn_item_id ) {
		...
		$this->getDb()->query( 'DELETE FROM ca_user_sort_items WHERE sort_id=? AND item_id=?', $this->getPrimaryKey(),
			$pn_item_id );
	...
	public static function getAvailableSortsForTable( $pn_table_num ) {
		...
		$qr_sorts = $o_db->query( 'SELECT * FROM ca_user_sorts WHERE table_num=? AND deleted=0 ORDER BY `rank`',
			$pn_table_num );
