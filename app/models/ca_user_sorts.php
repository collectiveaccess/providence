...
BaseModel::$s_ca_models_definitions['ca_user_sorts'] = array(
    ...
    'FIELDS' => array(
        ...
        'rank' => array(
            ...
            'LABEL' => _t('Sort order'),
            'DESCRIPTION' => _t(
                'The relative priority of sort when displayed in a list. Lower numbers indicate higher priority.'
            )
        ),
        'deleted' => array(
            'FIELD_TYPE' => FT_BIT,
...
class ca_user_sorts extends BaseModel
{
     ...
     * @var resource|null
     */
    static public $s_lock_resource = null;
    ...
    public function addSortBundle($ps_bundle_name, $pn_rank = null)
    {
        ...
        if (is_null($pn_rank)) {
            $qr_max_rank = $this->getDb()->query(
                'SELECT MAX(`rank`) AS rnk FROM ca_user_sort_items WHERE sort_id=?',
                $this->getPrimaryKey()
            );
    ...
    public function getSortBundles()
    {
        ...
        $qr_sort_bundles = $this->getDb()->query(
            'SELECT * FROM ca_user_sort_items WHERE sort_id=? ORDER BY `rank`, item_id',
            $this->getPrimaryKey()
        );
    ...
    public function getSortBundleNames()
    {
        ...
        $qr_sort_bundles = $this->getDb()->query(
            'SELECT bundle_name, `rank` FROM ca_user_sort_items WHERE sort_id=? ORDER BY `rank`, item_id',
            $this->getPrimaryKey()
        );
    ...
    {
        $qr_sort_bundles = $this->getDb()->query(
            'SELECT bundle_name FROM ca_user_sort_items WHERE sort_id=? AND `rank`=?',
            $this->getPrimaryKey(),
            $pn_rank
        );
        if ($qr_sort_bundles->numRows() > 1) {
            ...
            $this->getDb()->query(
                'UPDATE ca_user_sort_items SET bundle_name=? WHERE sort_id=? AND `rank`=?',
                $ps_bundle_name,
                $this->getPrimaryKey(),
                $pn_rank
            );
    ...
    public function removeBundleByItemID($pn_item_id)
    {
        ...
        $this->getDb()->query(
            'DELETE FROM ca_user_sort_items WHERE sort_id=? AND item_id=?',
            $this->getPrimaryKey(),
            $pn_item_id
        );
        return true;
    ...
    public static function getAvailableSortsForTable($pn_table_num)
    {
        ...
        $qr_sorts = $o_db->query(
            'SELECT * FROM ca_user_sorts WHERE table_num=? AND deleted=0 ORDER BY `rank`',
            $pn_table_num
        );
