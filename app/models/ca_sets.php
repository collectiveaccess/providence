...
BaseModel::$s_ca_models_definitions['ca_sets'] = array(
    ...
    'FIELDS' => array(
        ...
        'table_num' => array(
            ...
            'DESCRIPTION' => _t(
                'Determines what kind of items (objects, entities, places, etc.) are stored by the set.'
            ),
        ...
        'type_id' => array(
            ...
            'LABEL' => _t('Type'),
            'DESCRIPTION' => _t(
                'The type of the set determines what sorts of information the set and each item in the set can have associated with them.'
            )
        ),
        'set_code' => array(
            'FIELD_TYPE' => FT_TEXT,
            ...
            'LABEL' => _t('Set code'),
            'DESCRIPTION' => _t(
                'A unique alphanumeric code for this set. You will need to specify this if you are using this set in a special context (on a web front-end, for example) in which the set must be unambiguously identified.'
            ),
...
class ca_sets extends BundlableLabelableBaseModelWithAttributes implements IBundleProvider
{
    ...
    static public $s_have_access_to_set_cache = array();
    ...
    public function __construct($pn_id = null)
    {
        ...
        BaseModel::$s_ca_models_definitions['ca_sets']['FIELDS']['table_num']['BOUNDS_CHOICE_LIST'] = caFilterTableList(
    ...
    public function delete($pb_delete_related = false, $pa_options = null, $pa_fields = null, $pa_table_list = null)
    {
        ...
        if ($pb_delete_related) {
            ...
            $this->getDb()->query(
                'DELETE FROM ca_set_item_labels WHERE item_id IN (SELECT item_id FROM ca_set_items WHERE set_id=?)',
                $this->getPrimaryKey()
            );
            ...
            $qr_items = $this->getDb()->query(
                'SELECT item_id FROM ca_set_items WHERE set_id = ?',
            ...
            foreach ($va_item_ids as $vn_item_id) {
                $this->getSearchIndexer()->commitRowUnIndexing(
                    $this->tableNum(),
                    $vn_item_id,
        ...
        if ($vn_rc = parent::delete(
            $pb_delete_related,
            array_merge(array('queueIndexing' => true), $pa_options),
            $pa_fields,
            $pa_table_list
        )) {
        ...
        if (!$pb_set_ids_only && !$pb_omit_counts) {
            ...
            while ($qr_table_nums->nextRow()) {
                $t_instance = Datamodel::getInstanceByTableNum(
                    $vn_table_num = (int)$qr_table_nums->get('table_num'),
                    true
                );
                if (!$t_instance) {
                    continue;
            ...
            while ($qr_res->nextRow()) {
                ...
                if (!isset($va_type_name_cache[$vn_table_num]) || !($vs_set_type = $va_type_name_cache[$vn_table_num])) {
                    $vs_set_type = $va_type_name_cache[$vn_table_num] = $this->getSetContentTypeName(
                        $vn_table_num,
                        array('number' => 'plural')
                    );
                }
            ...
            if ($pb_by_user) {
                ...
                foreach ($va_sets as $va_set) {
                    ...
                    $vs_user_key = strtolower(
                        str_pad(substr($va_set['lname'], 0, 20), 20, ' ', STR_PAD_RIGHT) . str_pad(
                            substr($va_set['fname'], 0, 20),
                            20,
                            ' ',
                            STR_PAD_RIGHT
                    ...
                    foreach ($va_users as $va_user) {
                        $vs_user_key = strtolower(
                            str_pad(substr($va_user['lname'], 0, 20), 20, ' ', STR_PAD_RIGHT) . str_pad(
                                substr($va_user['fname'], 0, 20),
                                20,
                                ' ',
                                STR_PAD_RIGHT
                            ) . str_pad($va_user['user_id'], 10, '0', STR_PAD_LEFT)
                        );
            ...
            while ($qr_res->nextRow()) {
                ...
                $va_sets[$qr_res->get('set_id')][$qr_res->get('locale_id')] = array_merge(
                    $qr_res->getRow(),
                    array(
                        'item_count' => intval(
                            $va_item_counts[$qr_res->get(
                                'set_id'
                            )]
                        ),
                        'set_content_type' => $vs_set_type,
    ...
    public function getSetsForItem($pm_table_name_or_num, $pn_row_id, $pa_options = null)
    {
        ...
        return $this->getSets(
            array_merge($pa_options, array('table' => $pm_table_name_or_num, 'row_id' => $pn_row_id))
        );
    }
    # ------------------------------------------------------
    ...
    public function haveAccessToSet($pn_user_id, $pn_access, $pn_set_id = null, $pa_options = null)
    {
        ...
        if (($t_set->get(
                    'access'
                ) > 0) && ($pn_access == __CA_SET_READ_ACCESS__)) {     // public sets are readable by all
            return ca_sets::$s_have_access_to_set_cache[$vn_set_id . '/' . $pn_user_id . '/' . $pn_access] = true;
        }
        ...
        //
        $t_user = new ca_users();
        if ($t_user->load($pn_user_id) && ($t_user->canDoAction('is_administrator') || $t_user->canDoAction(
    ...
    public function addItems($pa_row_ids, $pa_options = [])
    {
        ...
        if (sizeof($va_item_values)) {
            ...
            $this->getDb()->query(
                "INSERT INTO ca_set_items (set_id, table_num, row_id, type_id, vars) VALUES " . join(
                    ",",
                    $va_item_values
                )
            );
            ...
            $this->getDb()->query(
                "INSERT INTO ca_set_item_labels (item_id, locale_id, caption) VALUES " . join(",", $va_label_values)
            );
            if ($this->getDb()->numErrors()) {
                $this->errors = $this->getDb()->errors;
                return false;
            ...
            // Index the links
            $this->getSearchIndexer()->reindexRows(
                'ca_set_items',
                $va_item_ids,
                array(
                    'queueIndexing' => (bool)caGetOption(
                        'queueIndexing',
                        $pa_options,
                        true
                    )
            ...
            if (sizeof($va_item_ids)) {
                ...
                while ($qr_res->nextRow()) {
                    $va_snapshot = $qr_res->getRow();
                    $t_set_item_label->logChange(
                        "I",
                        $pn_user_id,
                        [
                            'row_id' => $qr_res->get('ca_set_item_labels.label_id'),
                            'snapshot' => $va_snapshot
                        ]
                    );
                }
    ...
    public function reorderItems($pa_row_ids, $pa_options = null)
    {
        ...
        foreach ($va_row_ranks as $vn_row_id => $va_rank) {
            if (!in_array($vn_row_id, $pa_row_ids)) {
                if ($vb_treat_row_ids_as_rids) {
                    ...
                    if ($t_set_item->load(
                        array('set_id' => $vn_set_id, 'row_id' => $va_tmp[0], 'item_id' => $va_tmp[1])
                    )) {
        ...
        foreach ($pa_row_ids as $vn_rank => $vn_row_id) {
            ...
            if (isset($va_row_ranks[$vn_row_id]) && $t_set_item->load(
                    $vb_treat_row_ids_as_rids ? array(
                        'set_id' => $vn_set_id,
                        'row_id' => $va_tmp[0],
                        'item_id' => $va_tmp[1]
                ...
                if ($va_row_ranks[$vn_row_id] != $vn_rank_inc) {
                    ...
                    if ($t_set_item->numErrors()) {
                        $va_errors[$vn_row_id] = _t(
                            'Could not reorder item %1: %2',
                            $vn_row_id,
    ...
    public function getItems($pa_options = null)
    {
        ...
        if ($t_rel_label_table) {
            ...
            $vs_label_join_sql = "LEFT JOIN " . $t_rel_label_table->tableName(
                ) . " AS rel_label ON rel." . $t_rel_table->primaryKey() . " = rel_label." . $t_rel_table->primaryKey(
                ) . " {$vs_preferred_sql}\n";
        }
        ...
        $vs_item_ids_sql = '';
        if (isset($pa_options['item_ids']) && (is_array($pa_options['item_ids'])) && (sizeof(
                    $pa_options['item_ids']
                ) > 0)) {
        ...
        if (($t_rel_table->tableName(
                ) === 'ca_objects') && (isset($pa_options['thumbnailVersion']) || isset($pa_options['thumbnailVersions']))) {
...
LEFT JOIN ca_object_representations AS cor ON coxor.representation_id = cor.representation_id\n";
        ...
        if ($ps_template = caGetOption('template', $pa_options, null)) {
            $va_processed_templates = caProcessTemplateForIDs(
                $ps_template,
                $t_rel_table->tableName(),
        ...
        if ($vs_rep_join_sql) {
            ...
            $va_alt_tags = caProcessTemplateForIDs(
                ($alt_text_template) ? $alt_text_template : "^" . $t_rel_table->tableName() . ".preferred_labels",
                $t_rel_table->tableName(),
                $qr_res->getAllFieldValues('row_id'),
                array('returnAsArray' => true)
            );
        ...
        while ($qr_res->nextRow()) {
            ...
            if ($vs_rep_join_sql && isset($pa_options['checkAccess']) && is_array($pa_options['checkAccess']) && sizeof(
                    $pa_options['checkAccess']
                )) {
            ...
            if ($vs_rep_join_sql && $vb_has_access_to_media) {
                ...
                if (isset($pa_options['thumbnailVersion'])) {
                    $va_row['representation_tag'] = $qr_res->getMediaTag(
                        'media',
                    ...
                    $va_row['representation_width'] = $qr_res->getMediaInfo(
                        'media',
                        $pa_options['thumbnailVersion'],
                        'WIDTH'
                    );
                    $va_row['representation_height'] = $qr_res->getMediaInfo(
                        'media',
                        $pa_options['thumbnailVersion'],
                        'HEIGHT'
                    );
                ...
                if (isset($pa_options['thumbnailVersions']) && is_array($pa_options['thumbnailVersions'])) {
                    foreach ($pa_options['thumbnailVersions'] as $vs_version) {
                        $va_row['representation_tag_' . $vs_version] = $qr_res->getMediaTag(
                            'media',
                            $vs_version,
                            array("alt" => $vs_alt_text)
                        );
                        ...
                        $va_row['representation_width_' . $vs_version] = $qr_res->getMediaInfo(
                            'media',
                            $vs_version,
                            'WIDTH'
                        );
                        $va_row['representation_height_' . $vs_version] = $qr_res->getMediaInfo(
                            'media',
                            $vs_version,
                            'HEIGHT'
                        );
            ...
            if (isset($pa_options['returnItemAttributes']) && is_array($pa_options['returnItemAttributes']) && sizeof(
                    $pa_options['returnItemAttributes']
                )) {
            ...
            $va_items[$qr_res->get('item_id')][($qr_res->get('rel_locale_id') ? $qr_res->get(
                'rel_locale_id'
            ) : 0)] = $va_row;
        }
    ...
    public function getItemCount($pa_options = null)
    {
        ...
        if (isset($pa_options['checkAccess']) && is_array($pa_options['checkAccess']) && sizeof(
            ...
            $vs_access_sql = ' AND ' . $vs_rel_table_name . '.access IN (' . join(
                    ',',
                    $pa_options['checkAccess']
                ) . ')';
        }
    ...
    public function getSetItemHTMLFormBundle(
        ...
        if ($this->getPrimaryKey()) {
            ...
            if (!($vs_template = caGetOption(
                "{$vs_set_table_name}_display_template",
                $pa_bundle_settings,
                null
                ...
                if (!($vs_template = caGetOption(
                    ...
                    $vs_template = $this->getAppConfig()->get(
                        "{$vs_set_table_name}_set_item_display_template"
                    );            // use default in app.conf
                }
            }
    ...
    public function getItemSetMembershipHTMLFormBundle(
        ...
        $va_cur_sets = caExtractValuesByUserLocale(
            $this->getSetsForItem($pn_table_num, $pn_row_id, array('user_id' => $pn_user_id))
        );
        ...
        $o_view->setVar(
            'available_set_ids',
            $va_available_sets = $this->getAvailableSetsForItem(
                $pn_table_num,
                $pn_row_id,
    ...
    public function getRepresentationTags($ps_version, $pa_options = null)
    {
        ...
        if (isset($pa_options['checkAccess']) && is_array($pa_options['checkAccess']) && sizeof(
            ...
            $vs_access_sql = ' AND o.access IN (' . join(
                    ',',
                    $pa_options['checkAccess']
                ) . ') AND caor.access IN (' . join(',', $pa_options['checkAccess']) . ')';
        }
        $o_db = $this->getDb();
        $qr_res = $o_db->query(
            "
        ...
        while ($qr_res->nextRow()) {
            $va_reps[$qr_res->get(
                'object_id'
            )] = (isset($pa_options['quote']) && $pa_options['quote']) ? '"' . $qr_res->getMediaTag(
                    'media',
                    $ps_version
                ) . '"' : $qr_res->getMediaTag('media', $ps_version);
        }
	...
	*/
    public function getSetsForUser($pa_options)
    {
        ...
        if ($pn_user_id) {
            ...
            $qr_res = $o_db->query(
                "SELECT COUNT(*) c, cs.set_id, cs.user_id, cs.type_id, cs.table_num, cu.fname, cu.lname
									FROM ca_sets cs
            ...
            while ($qr_res->nextRow()) {
                ...
                $va_sets[$set_id = $qr_res->get('set_id')] = array_merge(
                    $qr_res->getRow(),
                    [
                        'set_content_type' => $vs_set_type,
                        'set_type' => $vs_type,
...
// 				}
            ...
            $q_change_log = $o_db->query(
                ...
                )) . ") AND (wcls.subject_row_id IN (" . implode(
                    ", ",
                    $va_set_ids
                ) . ")) AND wcl.logged_table_num IN (" . Datamodel::getTableNum(
                    "ca_set_items"
                ) . ", " . Datamodel::getTableNum("ca_sets_x_users") . ", " . Datamodel::getTableNum(
            ...
            $q_set_comments = $o_db->query(
                "
								...
								WHERE c.table_num = " . ((int)$this->tableNum()) . " AND c.row_id IN (" . implode(
                    ", ",
                    $va_set_ids
                ) . ")
            ...
            $q_set_item_comments = $o_db->query(
                "
								...
								WHERE c.table_num = " . (Datamodel::getTableNum(
                    "ca_set_items"
    ...
    public function duplicateItemsInSet($pn_user_id, $pa_options = array())
    {
        ...
        if (caGetOption('addToCurrentSet', $pa_options, false)) {
            ...
            $t_set_to_add_dupes_to->addLabel(
                array('name' => $this->getLabelForDisplay() . ' ' . _t('[Duplicates]')),
                $g_ui_locale_id,
                null,
                true
            );
        ...
        foreach ($va_items as $vn_row_id) {
            ...
            if (!$t_user->canDoAction('can_duplicate_' . $t_instance->tableName())) {
                $this->postError(
                    2580,
                    _t('You do not have permission to duplicate these items'),
            ...
            // let's dupe
            $t_dupe = $t_instance->duplicate(
                array(
                    'user_id' => $pn_user_id,
                    'duplicate_nonpreferred_labels' => $t_user->getPreference(
                        $t_instance->tableName() . '_duplicate_nonpreferred_labels'
                    ),
                    'duplicate_attributes' => $t_user->getPreference(
                        $t_instance->tableName() . '_duplicate_attributes'
                    ),
                    'duplicate_relationships' => $t_user->getPreference(
                        $t_instance->tableName() . '_duplicate_relationships'
                    ),
    ...
    public static function setExists($set_code, $options = null)
    {
        if ($ids = ca_sets::find(['set_code' => $set_code], ['returnAs' => 'ids'])) {
            if ($user_id = caGetOption('user_id', $options, null)) {
                $id = array_shift($ids);
                $t_set = new ca_sets();
                return $t_set->haveAccessToSet(
                    $user_id,
                    caGetOption('access', $options, null),
                    $id,
    ...
    public static function setContents($set_code, $options = null)
    {
        ...
        if (($user_id = caGetOption('user_id', $options, null)) && !$set->haveAccessToSet(
                $user_id,
                caGetOption(
                    'access',
                    $options,
