...
BaseModel::$s_ca_models_definitions['ca_sets'] = array(
	...
	'FIELDS'        => array(
		'set_id'      => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this set' )
		),
		'parent_id'   => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Parent id',
			'DESCRIPTION'    => 'Identifier of parent set; is null if set is root of hierarchy.'
		),
		'hier_set_id' => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Set hierarchy',
			'DESCRIPTION'    => 'Identifier of set that is root of the set hierarchy.'
		),
		'user_id'     => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'User' ),
			'DESCRIPTION'    => _t( 'The user who created the set.' )
		),
		'table_num'   => array(
			'FIELD_TYPE'         => FT_NUMBER,
			...
			'LABEL'              => _t( 'Set content' ),
			'DESCRIPTION'        => _t( 'Determines what kind of items (objects, entities, places, etc.) are stored by the set.' ),
		...
		'type_id'     => array(
			...
			'LIST_CODE'      => 'set_types',
			'LABEL'          => _t( 'Type' ),
			'DESCRIPTION'    => _t( 'The type of the set determines what sorts of information the set and each item in the set can have associated with them.' )
		),
		'set_code'    => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Set code' ),
			'DESCRIPTION'    => _t( 'A unique alphanumeric code for this set. You will need to specify this if you are using this set in a special context (on a web front-end, for example) in which the set must be unambiguously identified.' ),
		...
		'status'      => array(
			...
			'LABEL'              => _t( 'Status' ),
			'DESCRIPTION'        => _t( 'Indicates the current state of the object record.' )
		),
		'hier_left'   => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_OMIT,
		...
		'deleted'     => array(
			...
			'DEFAULT'        => 0,
			'LABEL'          => _t( 'Is deleted?' ),
			'DESCRIPTION'    => _t( 'Indicates if the set is deleted or not.' ),
...
class ca_sets extends BundlableLabelableBaseModelWithAttributes implements IBundleProvider {
	...
	public function __construct( $pn_id = null ) {
		...
		BaseModel::$s_ca_models_definitions['ca_sets']['FIELDS']['table_num']['BOUNDS_CHOICE_LIST']
			= caFilterTableList( BaseModel::$s_ca_models_definitions['ca_sets']['FIELDS']['table_num']['BOUNDS_CHOICE_LIST'] );
	...
	protected function initLabelDefinitions( $pa_options = null ) {
		...
		$this->BUNDLES['ca_set_items']   = array(
			'type'      => 'special',
			'repeating' => true,
			'label'     => _t( 'Set items' )
		);
	...
	public function delete( $pb_delete_related = false, $pa_options = null, $pa_fields = null, $pa_table_list = null ) {
		...
		if ( $pb_delete_related ) {
			...
			$this->getDb()
			     ->query( 'DELETE FROM ca_set_item_labels WHERE item_id IN (SELECT item_id FROM ca_set_items WHERE set_id=?)',
				     $this->getPrimaryKey() );
			...
			$qr_items    = $this->getDb()->query( 'SELECT item_id FROM ca_set_items WHERE set_id = ?',
				array( $this->getPrimaryKey() ) );
			...
			foreach ( $va_item_ids as $vn_item_id ) {
				$this->getSearchIndexer()
				     ->commitRowUnIndexing( $this->tableNum(), $vn_item_id, array( 'queueIndexing' => true ) );
			}
		}

		if ( $vn_rc = parent::delete( $pb_delete_related, array_merge( array( 'queueIndexing' => true ), $pa_options ),
			$pa_fields, $pa_table_list )
		) {
	 ...
	 *                          returnAsInfoArray = Return an array for each item_id containing keys for set_id and
	 *                          item_id [Default is false]
	 *
	 ...
	 *                          table - if set, list is restricted to sets that can contain the specified item. You can
	 *                          pass a table name or number. If omitted sets containing any content will be returned.
	 *                          setType - Restricts returned sets to those of the specified type. You can pass a
	 *                          type_id or list item code for the set type. If omitted sets are returned regardless of
	 *                          type. user_id - Restricts returned sets to those accessible by the current user. If
	 *                          omitted then all sets, regardless of access are returned. access - Restricts returned
	 *                          sets to those with at least the specified access level for the specified user. If
	 *                          user_id is omitted then this option has no effect. If user_id is set and this option is
	 *                          omitted, then sets where the user has at least read access will be returned.
	 *                          checkAccess - Restricts returned sets to those with an public access level with the
	 *                          specified values. If omitted sets are returned regardless of public access (ca_sets.access) value. Can be a single value or array if you wish to filter on multiple public access values. row_id = if set to an integer only sets containing the specified row are returned setIDsOnly = if set to true only set_id values are returned, in a simple array omitCounts = all = allUsers = publicUsers = name = byUser = return sets grouped by user with access. The array will be key'ed by sortable user name. Each entry includes a 'user' array with information about the user and a 'sets' array with the list of sets that user has access to. [Default is false] codes = includeItems = item_id = Get set that contains specified item_id
	 *
	 * @return array A list of sets keyed by set_id and then locale_id. Keys for the per-locale value array include:
	 *               set_id, set_code, status, public access, owner user_id, content table_num, set type_id, set name,
	 *               number of items in the set (item_count), set type name for display and set content type name for
	 *               display. If setIDsOnly option is set then a simple array of set_id values is returned instead.
	 */
	public function getSets( $pa_options = null ) {
		...
		$pn_row_id = ( isset( $pa_options['row_id'] ) && ( (int) $pa_options['row_id'] ) ) ? (int) $pa_options['row_id']
			: null;
		...
		if ( ! $pb_set_ids_only && ! $pb_omit_counts ) {
			...
			while ( $qr_table_nums->nextRow() ) {
				$t_instance = Datamodel::getInstanceByTableNum(
					$vn_table_num = (int) $qr_table_nums->get( 'table_num' ), true );
				if ( ! $t_instance ) {
					continue;
				}
			...
			while ( $qr_res->nextRow() ) {
				...
				if ( ! isset( $va_type_name_cache[ $vn_table_num ] )
					...
					$vs_set_type
						= $va_type_name_cache[ $vn_table_num ] = $this->getSetContentTypeName( $vn_table_num,
						array( 'number' => 'plural' ) );
				}
				...
				$extras = [
					'item_count'       => intval( $va_item_counts[ $qr_res->get( 'set_id' ) ] ),
				...
				$va_sets[ $qr_res->get( 'set_id' ) ][ $qr_res->get( 'locale_id' ) ] = array_merge( $qr_res->getRow(),
					$extras );
			}
			...
			if ( $pb_by_user ) {
				...
				foreach ( $va_sets as $va_set ) {
					...
					$vs_user_key = strtolower( str_pad( substr( $va_set['lname'], 0, 20 ), 20, ' ', STR_PAD_RIGHT )
					                           . str_pad( substr( $va_set['fname'], 0, 20 ), 20, ' ', STR_PAD_RIGHT )
					                           . str_pad( $va_set['user_id'], 10, '0', STR_PAD_LEFT ) );
					if ( ! isset( $va_sets_by_user[ $vs_user_key ]['user'] ) ) {
						$va_sets_by_user[ $vs_user_key ]['user'] = [
							'user_id'   => $va_set['user_id'],
							'user_name' => $va_set['user_name'],
							'fname'     => $va_set['fname'],
							'lname'     => $va_set['lname'],
							'email'     => $va_set['email']
						];
					...
					foreach ( $va_users as $va_user ) {
						$vs_user_key                               = strtolower( str_pad( substr( $va_user['lname'], 0,
								20 ), 20, ' ', STR_PAD_RIGHT ) . str_pad( substr( $va_user['fname'], 0, 20 ), 20, ' ',
								STR_PAD_RIGHT ) . str_pad( $va_user['user_id'], 10, '0', STR_PAD_LEFT ) );
	...
	/**
	 * Returns list of sets to which the item (as specified by $pm_table_name_or_num and $pn_row_id) can be associated
	 * and is not already part of.
	 *
	 ...
	 * @param array $pa_options           Optional list of options. Supported options are the same as those for
	 *                                    ca_sets::getSets() and ca_sets::getSetsForItem()
	 *
	 * @return array A list of sets keyed by set_id and then locale_id. Keys for the per-locale value array include:
	 *               set_id, set_code, status, public access, owner user_id, content table_num, set type_id, set name,
	 *               number of items in the set (item_count), set type name for display and set content type name for
	 *               display. This is the same format as returned by ca_sets::getSets().
	 */
	public function getAvailableSetsForItem( $pm_table_name_or_num, $pn_row_id, $pa_options = null ) {
		if ( ! is_array( $pa_options ) ) {
			$pa_options = array();
		}
		if ( ! ( $va_full_set_list = $this->getSets( array_merge( array( 'table' => $pm_table_name_or_num ),
			$pa_options ) ) )
		) {
			return null;
		}
	...
	public function getSetsForItem( $pm_table_name_or_num, $pn_row_id, $pa_options = null ) {
		...
		return $this->getSets( array_merge( $pa_options,
			array( 'table' => $pm_table_name_or_num, 'row_id' => $pn_row_id ) ) );
	}
	# ------------------------------------------------------
	...
	/**
	 * Returns a random set_id from a group defined by specified options. These options are the same as those for
	 * ca_sets::getSets()
	 *
	 * @param array $pa_options Array of options. Supported options are:
	 *                          table - if set, list is restricted to sets that can contain the specified item. You can
	 ...
	 *                          omitted, then sets where the user has at least read access will be returned.
	 *                          checkAccess - Restricts returned sets to those with an public access level with the
	 *                          specified values. If omitted sets are returned regardless of public access (ca_sets.access) value. Can be a single value or array if you wish to filter on multiple public access values. row_id = if set to an integer only sets containing the specified row are returned
	 *
	 ...
	 * @param int   $pn_access  type of access required. Use __CA_SET_READ_ACCESS__ for read-only access or
	 *                          __CA_SET_EDIT_ACCESS__ for editing (full) access
	...
	public function haveAccessToSet( $pn_user_id, $pn_access, $pn_set_id = null, $pa_options = null ) {
		...
		if ( ( $t_set->get( 'access' ) > 0 )
		     && ( $pn_access == __CA_SET_READ_ACCESS__ )
		...
		if ( $t_user->load( $pn_user_id )
		     && ( $t_user->canDoAction( 'is_administrator' )
		          || $t_user->canDoAction( 'can_administrate_sets' ) )
		) {
			return ca_sets::$s_have_access_to_set_cache[ $vn_set_id . '/' . $pn_user_id . '/' . $pn_access ] = true;
		}
	 ...
	 * @param int $pn_access  type of access required. Use __CA_SET_READ_ACCESS__ for read-only access or
	 *                        __CA_SET_EDIT_ACCESS__ for editing (full) access
	 * @param int $pa_set_ids The ids of the sets to check. If omitted then currently loaded set will be checked. Can
	 *                        also be set to a single integer.
	 *
	 * @return int
	 */
	 ...
	 *
	 * @param int   $pn_row_id  The row_id to add to the set. Assumed to be a key to a record in the sets's content
	 *                          type's table.
	 * @param array $pa_labels  An array of ca_set_item label values to create. The array is keyed on locale_id; each
	 *                          value is an array with keys set to ca_set_item_labels fields with associated values.
	 * @param int   $pn_user_id user_id of user adding the item. Used to check if the user has editing access to the
	 *                          set. If omitted no checking of access is done.
	 * @param int   $pn_rank    position in the set of the newly added item
	 *
	 * @return int Returns item_id of newly created set item entry. The item_id is a unique identifier for the row_id
	 *             in the city at the specified position (rank). It is *not* the same as the row_id.
	 */
	...
	public function addItems( $pa_row_ids, $pa_options = [] ) {
		...
		foreach ( $va_row_ids as $vn_row_id ) {
			$va_item_values[] = "(" . (int) $vn_set_id . "," . (int) $vn_table_num . "," . (int) $vn_row_id . ","
			                    . (int) $vn_type_id . ", '')";
		}
		...
		if ( sizeof( $va_item_values ) ) {
			...
			$this->getDb()->query( "INSERT INTO ca_set_items (set_id, table_num, row_id, type_id, vars) VALUES "
			                       . join( ",", $va_item_values ) );
			...
			$qr_res      = $this->getDb()
			                    ->query( "SELECT item_id FROM ca_set_items WHERE set_id = ? AND table_num = ? AND type_id = ? AND row_id IN (?)",
				                    array(
					                    (int) $vn_set_id,
					                    (int) $vn_table_num,
					                    (int) $vn_type_id,
			...
			$this->getDb()
			     ->query( "UPDATE ca_set_items SET `rank` = item_id WHERE set_id = ? AND table_num = ? AND type_id = ? AND row_id IN (?)",
				     array(
					     $vn_set_id,
					     $vn_table_num,
					     $vn_type_id,
			...
			$this->getDb()->query( "INSERT INTO ca_set_item_labels (item_id, locale_id, caption) VALUES " . join( ",",
					$va_label_values ) );
			if ( $this->getDb()->numErrors() ) {
				$this->errors = $this->getDb()->errors;
			...
			$this->getSearchIndexer()->reindexRows( 'ca_set_items', $va_item_ids,
				array( 'queueIndexing' => (bool) caGetOption( 'queueIndexing', $pa_options, true ) ) );

			// Create change log entries
			if ( sizeof( $va_item_ids ) ) {
				$qr_res = $this->getDb()
				               ->query( "SELECT * FROM ca_set_items WHERE item_id IN (?)", array( $va_item_ids ) );
				...
				while ( $qr_res->nextRow() ) {
					...
					$t_set_item->logChange( "I", $pn_user_id,
						[ 'row_id' => $qr_res->get( 'ca_set_items.item_id' ), 'snapshot' => $va_snapshot ] );
				}

				$t_set_item_label = new ca_set_item_labels();
				$qr_res           = $this->getDb()->query( "SELECT * FROM ca_set_item_labels WHERE item_id IN (?)",
					array( $va_item_ids ) );
				while ( $qr_res->nextRow() ) {
					$va_snapshot = $qr_res->getRow();
					$t_set_item_label->logChange( "I", $pn_user_id,
						[ 'row_id' => $qr_res->get( 'ca_set_item_labels.label_id' ), 'snapshot' => $va_snapshot ] );
				}

				$qr_res = $this->getDb()->query( "SELECT * FROM ca_sets WHERE set_id IN (?)",
					array( array_keys( $va_set_ids ) ) );

				$t_set = new ca_sets();
				while ( $qr_res->nextRow() ) {
					$va_snapshot = $qr_res->getRow();
					$t_set->logChange( "U", $pn_user_id,
						[ 'row_id' => $qr_res->get( 'ca_sets.set_id' ), 'snapshot' => $va_snapshot ] );
				}
			}
	 ...
	 * @param int $pn_user_id Option user_id of user to check set access for; if no user_id is specified then no access
	 *                        checking is done
	 *
	 ...
	 * @param int   $pn_user_id           Option user_id of user to check set access for; if no user_id is specified
	 *                                    then no access checking is done
	 *
	 * @return bool True on success, false if item was not removed from one or more sets due to error or access
	 *              restrictions
	 */
	 ...
	 *                          user_id = the user_id of the current user; used to determine which sets the user has
	 *                          access to
	 *
	 ...
	 * @param array $pa_options An optional array of options. Supported options are:
	 *                          user_id = the user_id of the current user; used to determine which sets the user has
	 *                          access to treatRowIDsAsRIDs = use combination row_id/item_id indices in returned array
	 *                          instead of solely row_ids. Since a set can potentially contain multiple instances of
	 *                          the same row_id, only "rIDs" – a combination of the row_id and the set item_id (row_id
	 *                          + "_" + item_id) – are guaranteed to be unique. [Default=false]
	 *
	 * @return array Array keyed on row_id with values set to ranks for each item. If the set contains duplicate
	 *               row_ids then the list will only have the largest rank. If you have sets with duplicate rows use
	 *               getItemRanks() instead
	 */
	public function getRowIDRanks( $pa_options = null ) {
		...
		$va_ranks = array();
		foreach ( $va_items as $vn_item_id => $va_item ) {
			$va_ranks[ $vb_treat_row_ids_as_rids ? $va_item['row_id'] . "_{$vn_item_id}" : $va_item['row_id'] ]
				= $va_item['rank'];
		}
	...
	/**
	 * Returns a list of row_ids for a set with ranks for each, in rank order. This is a faster alternative to
	 * getRowIDRanks() that queries the database directly and does no access checking. It is intended for use with
	 * lower level functions that need to sort potentially large sets quickly.
	 *
	 * @param int   $pn_set_id
	 * @param array $pa_options An optional array of options. Supported options are:
	 *                          treatRowIDsAsRIDs = use combination row_id/item_id indices in returned array instead of
	 *                          solely row_ids. Since a set can potentially contain multiple instances of the same
	 *                          row_id, only "rIDs" – a combination of the row_id and the set item_id (row_id + "_" +
	 *                          item_id) – are guaranteed to be unique. [Default=false]
	 *
	 * @return array ray keyed on row_id with values set to ranks for each item. If the set contains duplicate row_ids
	 *               then the list will only have the largest rank.
	 */
	static public function getRowIDRanksForSet( $pn_set_id, $pa_options = null ) {
		...
		$o_db = new Db();
		$qr_res
		      = $o_db->query( "SELECT row_id, item_id, `rank` FROM ca_set_items WHERE set_id = ? AND deleted = 0 ORDER BY `rank`",
			[ $pn_set_id ] );
		...
		while ( $qr_res->nextRow() ) {
			...
			$va_ranks[ $vb_treat_row_ids_as_rids ? $va_row['row_id'] . "_" . $va_row['item_id'] : $va_row['row_id'] ]
			        = $va_row['rank'];
		}
	 ...
	 *                          user_id = the user_id of the current user; used to determine which sets the user has
	 *                          access to treatRowIDsAsRIDs = assume combination row_id/item_id indices in $pa_row_ids
	 *                          array instead of solely row_ids. Since a set can potentially contain multiple instances
	 *                          of the same row_id, only "rIDs" – a combination of the row_id and the set item_id
	 *                          (row_id + "_" + item_id) – are guaranteed to be unique. [Default=false]
	 *                          deleteExcludedItems = should the set items not passed in pa_row_ids be deleted?
	 *                          default is false
	 *
	...
	public function reorderItems( $pa_row_ids, $pa_options = null ) {
		...
		foreach ( $pa_row_ids as $vn_rank => $vn_row_id ) {
			...
			if ( isset( $va_row_ranks[ $vn_row_id ] )
			     && $t_set_item->load( $vb_treat_row_ids_as_rids ? array(
					'set_id'  => $vn_set_id,
					'row_id'  => $va_tmp[0],
					'item_id' => $va_tmp[1]
				) : array( 'set_id' => $vn_set_id, 'row_id' => $vn_row_id ) )
			) {
				if ( $va_row_ranks[ $vn_row_id ] != $vn_rank_inc ) {
					...
					if ( $t_set_item->numErrors() ) {
						$va_errors[ $vn_row_id ] = _t( 'Could not reorder item %1: %2', $vn_row_id,
							join( '; ', $t_set_item->getErrors() ) );
					}
				}
			} elseif ( $allow_dupes_in_set
			           || ( ! $rows_in_set[ (int) ( $vb_treat_row_ids_as_rids ? $va_tmp[0] : $vn_row_id ) ] )
			) {
		...
		foreach ( $va_rank_updates as $vn_row_id => $vn_new_rank ) {
			if ( $vb_treat_row_ids_as_rids ) {
				$va_tmp = explode( "_", $vn_row_id );
				$this->getDb()
				     ->query( "UPDATE ca_set_items SET `rank` = ? WHERE set_id = ? AND row_id = ? AND item_id = ?",
					     array( $vn_new_rank, $vn_set_id, $va_tmp[0], $va_tmp[1] ) );
				$va_updated_item_ids[ $va_tmp[1] ] = 1;
			} else {
				$this->getDb()->query( "UPDATE ca_set_items SET `rank` = ? WHERE set_id = ? AND row_id = ?",
					array( $vn_set_id, $vn_new_rank ) );
			}
		}

		if ( sizeof( $va_updated_item_ids ) > 0 ) {
			$qr_res = $this->getDb()->query( "SELECT * FROM ca_set_items WHERE item_id IN (?)",
				array( array_keys( $va_updated_item_ids ) ) );
	...
	public function getItemRowIDs( $pa_options = null ) {
		...
		if ( isset( $pa_options['shuffle'] ) && $pa_options['shuffle'] && isset( $va_row_ids )
		     && is_array( $va_row_ids )
		) {
	...
	public static function getItemsForSet( $set_code_or_id, $options = null ) {
		if ( ! ( $t_set = ca_sets::find( [ 'set_code' => $set_code_or_id ],
	 ...
	 *                          than full item-level info for each set member. IDs are keys in the returned array.
	 *                          returnItemIdsOnly = If true a simple array of item_ids (keys for the ca_set_items rows
	 *                          themselves) is returned rather than full item-level info for each set member. returnItemAttributes = A list of attribute element codes for the ca_set_item record to return values for. idsOnly = Return a simple numerically indexed array of row_ids template = item_ids = array of set item_ids to limit results to -> used by getPrimaryItemsFromSets so don't have to replicate all the functionality in this function
	 *
	 * @return array An array of items. The format varies depending upon the options set. If returnRowIdsOnly or
	 *               returnItemIdsOnly are set then the returned array is a simple list of ids. The full return array
	 *               is key'ed on ca_set_items.item_id and then on locale_id. The values are arrays with keys set to a
	 *               number of fields including: set_id, item_id, row_id, rank, label_id, locale_id, caption (from the
	 *               ca_set_items label), all instrinsic field content from the row_id, the display label of the row as
	 *               'set_item_label'. If 'thumbnailVersion' is set then additional keys will be available for the
	 *               selected media version: representation_tag, representation_url, representation_width and
	 *               representation_height (the HTML tag, URL, pixel width and pixel height of the representation
	 *               respectively) If 'thumbnailVersions' is set then additional keys will be available for the
	 *               selected media versions: representation_tag_<version_name>, representation_url_<version_name>,
	 *               representation_width_<version_name> and representation_height_<version_name> (the HTML tag, URL, pixel width and pixel height of the representation respectively) If 'returnItemAttributes' is set then there will be an additional key for each element_code prefixed with 'ca_attribute_' to ensure it doesn't conflict with any other key in the array.
	 *
	 */
	public function getItems( $pa_options = null ) {
		...
		if ( $t_rel_label_table ) {
			...
			$vs_label_join_sql = "LEFT JOIN " . $t_rel_label_table->tableName() . " AS rel_label ON rel."
			                     . $t_rel_table->primaryKey() . " = rel_label." . $t_rel_table->primaryKey()
			                     . " {$vs_preferred_sql}\n";
		}
		...
		$vs_item_ids_sql = '';
		if ( isset( $pa_options['item_ids'] ) && ( is_array( $pa_options['item_ids'] ) )
		     && ( sizeof( $pa_options['item_ids'] ) > 0 )
		) {
		...
		if ( ( $t_rel_table->tableName() === 'ca_objects' )
		     && ( isset( $pa_options['thumbnailVersion'] )
		          || isset( $pa_options['thumbnailVersions'] ) )
		) {
...
LEFT JOIN ca_object_representations AS cor ON coxor.representation_id = cor.representation_id\n";
			...
			while ( $qr_rep_counts->nextRow() ) {
				$va_representation_counts[ (int) $qr_rep_counts->get( 'object_id' ) ]
					= (int) $qr_rep_counts->get( 'c' );
			}
		}
		...
		if ( $ps_template = caGetOption( 'template', $pa_options, null ) ) {
			$va_processed_templates = caProcessTemplateForIDs( $ps_template, $t_rel_table->tableName(),
				$qr_res->getAllFieldValues( 'row_id' ), array( 'returnAsArray' => true ) );
			$qr_res->seek( 0 );
		}
		if ( $vs_rep_join_sql ) {
			$alt_text_template = Configuration::load()->get( $t_rel_table->tableName() . "_alt_text_template" );
			$va_alt_tags       = caProcessTemplateForIDs( ( $alt_text_template ) ? $alt_text_template
				: "^" . $t_rel_table->tableName() . ".preferred_labels", $t_rel_table->tableName(),
				$qr_res->getAllFieldValues( 'row_id' ), array( 'returnAsArray' => true ) );
			$qr_res->seek( 0 );
		}
		$va_items = array();
		...
		while ( $qr_res->nextRow() ) {
			...
			if ( $vs_rep_join_sql && isset( $pa_options['checkAccess'] ) && is_array( $pa_options['checkAccess'] )
			     && sizeof( $pa_options['checkAccess'] )
			) {
			...
			if ( $vs_rep_join_sql && $vb_has_access_to_media ) {
				...
				if ( isset( $pa_options['thumbnailVersion'] ) ) {
					$va_row['representation_tag']    = $qr_res->getMediaTag( 'media', $pa_options['thumbnailVersion'],
						array( "alt" => $vs_alt_text ) );
					$va_row['representation_url']    = $qr_res->getMediaUrl( 'media', $pa_options['thumbnailVersion'] );
					$va_row['representation_path']   = $qr_res->getMediaPath( 'media',
						$pa_options['thumbnailVersion'] );
					$va_row['representation_width']  = $qr_res->getMediaInfo( 'media', $pa_options['thumbnailVersion'],
						'WIDTH' );
					$va_row['representation_height'] = $qr_res->getMediaInfo( 'media', $pa_options['thumbnailVersion'],
						'HEIGHT' );
				}
				...
				if ( isset( $pa_options['thumbnailVersions'] ) && is_array( $pa_options['thumbnailVersions'] ) ) {
					foreach ( $pa_options['thumbnailVersions'] as $vs_version ) {
						$va_row[ 'representation_tag_' . $vs_version ]    = $qr_res->getMediaTag( 'media', $vs_version,
							array( "alt" => $vs_alt_text ) );
						$va_row[ 'representation_url_' . $vs_version ]    = $qr_res->getMediaUrl( 'media',
							$vs_version );
						$va_row[ 'representation_path_' . $vs_version ]   = $qr_res->getMediaPath( 'media',
							$vs_version );
						$va_row[ 'representation_width_' . $vs_version ]  = $qr_res->getMediaInfo( 'media', $vs_version,
							'WIDTH' );
						$va_row[ 'representation_height_' . $vs_version ] = $qr_res->getMediaInfo( 'media', $vs_version,
							'HEIGHT' );
					}
				}
			}
			...
			if ( isset( $pa_options['returnItemAttributes'] ) && is_array( $pa_options['returnItemAttributes'] )
			     && sizeof( $pa_options['returnItemAttributes'] )
			) {
				...
				foreach ( $pa_options['returnItemAttributes'] as $vs_element_code ) {
					$va_row[ 'ca_attribute_' . $vs_element_code ]
						= $t_item->getAttributesForDisplay( $vs_element_code );
				}
			...
			$va_items[ $qr_res->get( 'item_id' ) ][ ( $qr_res->get( 'rel_locale_id' ) ? $qr_res->get( 'rel_locale_id' )
				: 0 ) ]
				= $va_row;
		}
	 ...
	 *
	 * @param array $pa_options
	 *        user_id = Restricts access to sets accessible by the current user. If omitted then all sets, regardless
	 *        of access are returned. checkAccess = Restricts returned sets to those with an public access level with
	 *        the specified values. If omitted sets are returned regardless of public access (ca_sets.access) value.
	 *        Can be a single value or array if you wish to filter on multiple public access values. includeParents =
	 *        Include parent types in the returned type list. [Default is false]
	 *
	...
	public function getTypesForItems( $pa_options = null ) {
		...
		if ( caGetOption( 'includeParents', $pa_options, false ) ) {
			...
			foreach ( $va_type_ids as $vn_type_id => $vs_type ) {
				if ( is_array(
					$va_parents = $t_item->getHierarchyAncestors( $vn_type_id, array( 'idsOnly' => true ) ) )
				) {
	 ...
	 */
	public function getSetItemHTMLFormBundle(
		...
		if ( $this->getPrimaryKey() ) {
			...
			if ( ! ( $vs_template = caGetOption( "{$vs_set_table_name}_display_template", $pa_bundle_settings,
				null ) )
			) {        // display template by table
				if ( ! ( $vs_template = caGetOption( 'display_template', $pa_bundle_settings,
					null ) )
				) {                        // try the old non-table-specific template?
					$vs_template = $this->getAppConfig()
					                    ->get( "{$vs_set_table_name}_set_item_display_template" );            // use default in app.conf
				}
			}
	...
	/**
	 * Renders and returns HTML form bundle for set membership control. This a bundle that can appear in the editor of
	 * any item type (objects, entities, places, collections, etc.) that can be the content type of a set, and enables
	 * users to toggle membership in any set for the open record.
	 *
	 * @param object $po_request         The current request object
	 ...
	 * @param int    $pn_user_id         The user_id of the current user; used to determine which sets are displayed as
	 *                                   choices
	 ...
	 * @return string Rendered HTML bundle for display
	 */
	public function getItemSetMembershipHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pn_table_num, $pn_row_id, $pn_user_id = null,
		$pa_bundle_settings = null, $pa_options = null
	) {
		$o_view = new View( $po_request, $po_request->getViewsDirectoryPath() . '/bundles/' );

		$va_cur_sets = caExtractValuesByUserLocale( $this->getSetsForItem( $pn_table_num, $pn_row_id,
			array( 'user_id' => $pn_user_id ) ) );
		...
		$o_view->setVar( 'available_set_ids',
			$va_available_sets = $this->getAvailableSetsForItem( $pn_table_num, $pn_row_id,
				array( 'user_id' => $pn_user_id, 'access' => __CA_SET_EDIT_ACCESS__ ) ) );
		$o_view->setVar( 'sets', $va_sets = $this->getSets( array(
			'table'   => $pn_table_num,
			'user_id' => $pn_user_id,
		...
		foreach ( $va_cur_sets as $va_set_info ) {
			$va_initial_values[ $va_set_info['item_id'] . '' ] = array(
				'set_id'   => $va_set_info['set_id'],
				'set_name' => $va_set_info['name']
			);
			$vn_i ++;
		}
	...
	# ------------------------------------------------------
	/**
	 * Returns an empty model instance corresponding to the table that is the content type for the currently loaded
	 * set.
	 *
	 * @return object Instance of model corresponding to content type of the currently loaded set, or null if not set
	 *                is loaded.
	 */
	 ...
	 *
	 * @return array A list of items; the keys of the array are set_ids while the values are associative arrays
	...
	/**
	 * Returns the primary items from each set listed in $pa_set_ids.  If no primary item is selected, default to first
	 * item in set
	 *
	 * @param array $pa_set_ids The set_ids (*not* set codes) for which the first item should be fetched
	 * @param array $pa_options And optional array of options. Supported values include:
	 *                          version = the media version to include with returned set items, when media is available
	 *                          primary_attribute = md attribute used to indicate set item is primary; default =
	 *                          set_item_is_primary primary_list = list for primary attribute; default =
	 *                          set_item_is_primary primary_value = list item value for primary items; default =
	 *                          is_primary
	 *
	 * @return array A list of items; the keys of the array are set_ids while the values are associative arrays
	 *               containing the latest information.
	...
	public static function getPrimaryItemsFromSets( $pa_set_ids, $pa_options = null ) {
		...
		$vs_version                     = ( isset( $pa_options['version'] ) && $pa_options['version'] )
			? $pa_options['version'] : 'thumbnail';
		$pa_options["thumbnailVersion"] = $vs_version;
		$vs_primary_attribute           = ( isset( $pa_options['primary_attribute'] )
		                                    && $pa_options['primary_attribute'] ) ? $pa_options['primary_attribute']
			: 'set_item_is_primary';
		$vs_primary_list                = ( isset( $pa_options['primary_list'] ) && $pa_options['primary_list'] )
			? $pa_options['primary_list'] : 'set_item_is_primary';
		$vs_primary_value               = ( isset( $pa_options['primary_value'] ) && $pa_options['primary_value'] )
			? $pa_options['primary_value'] : 'is_primary';
	...
	/**
	 * Returns name of type of content (synonymous with the table name for the content) currently loaded set contains
	 * for display. Will return name in singular number unless the 'number' option is set to 'plural'
	 *
	 * @param int   $pn_table_num Table number to return name for. If omitted then the name for the content type
	 *                            contained by the current set will be returned. Use this parameter if you want to
	 *                            force a content type without having to load a set.
	 * @param array $pa_options   Optional array of options. Supported options are:
	 *                            number = Set to 'plural' to return plural version of name; set to 'singular'
	 *                            [default] to return the singular version
	 *
	 * @return string The name of the type of content or null if $pn_table_num is not set to a valid table and no set
	 *                is loaded.
	 */
	public function getSetContentTypeName( $pm_table_name_or_num = null, $pa_options = null ) {
		...
		return ( isset( $pa_options['number'] ) && ( $pa_options['number'] == 'plural' ) )
	 ...
	 * @param array $pa_options            Optional array of options. Supports all options that
	 *                                     BaseModeWithAttributes::getAttributeForIDs() does
	 */
	 ...
	 *                           checkAccess = an array of access values to filter returned representations on. If not
	 *                           set no access checking is performed. quote = if set to true the media tags are return
	 *                           in double quotes suitable for use in a Javascript array. This options makes it
	 *                           convenient to join the return array into a Javascript array for use with UI libraries.
	 *                           Default is false.
	 *
	 * @return array A list of HTML tags for primary representations of objects in the set, in set order. They array is
	 *               key'ed on object_id.
	 */
	public function getRepresentationTags( $ps_version, $pa_options = null ) {
		...
		if ( isset( $pa_options['checkAccess'] ) && is_array( $pa_options['checkAccess'] )
			...
			$vs_access_sql = ' AND o.access IN (' . join( ',', $pa_options['checkAccess'] ) . ') AND caor.access IN ('
			                 . join( ',', $pa_options['checkAccess'] ) . ')';
		}
		$o_db   = $this->getDb();
		$qr_res = $o_db->query( "
			SELECT 
		...
		$va_reps = array();
		while ( $qr_res->nextRow() ) {
			$va_reps[ $qr_res->get( 'object_id' ) ] = ( isset( $pa_options['quote'] ) && $pa_options['quote'] ) ? '"'
			                                                                                                      . $qr_res->getMediaTag( 'media',
					$ps_version ) . '"' : $qr_res->getMediaTag( 'media', $ps_version );
		}
	...
	# ------------------------------------------------------
	/**
	 * Returns set_id corresponding to set code, or passed back set_id if input in numeric (it doesn't actually verify
	 * that a numeric set_id is actually valid)
	 *
	...
	public function getSetsForUser( $pa_options ) {
		...
		if ( $pn_user_id ) {
			...
			$qr_res  = $o_db->query( "SELECT COUNT(*) c, cs.set_id, cs.user_id, cs.type_id, cs.table_num, cu.fname, cu.lname
									...
									" . join( "\n", $va_extra_joins ) . "
									" . ( sizeof( $va_sql_wheres ) ? "WHERE " : "" ) . " " . join( " AND ",
					$va_sql_wheres ) . "
			...
			while ( $qr_res->nextRow() ) {
				...
				$va_sets[ $set_id = $qr_res->get( 'set_id' ) ] = array_merge( $qr_res->getRow(), [
					...
					'set_type'           => $vs_type,
					'label'              => $labels[ $set_id ],
					'count'              => isset( $counts[ $set_id ] ) ? $counts[ $set_id ] : 0,
					'item_type_singular' => Datamodel::getTableProperty( $vn_table_num, 'NAME_SINGULAR' ),
					'item_type_plural'   => Datamodel::getTableProperty( $vn_table_num, 'NAME_PLURAL' ),
					'created'            => $created['timestamp'],
					'created_display'    => caGetLocalizedDate( $created['timestamp'] )
				] );
			}
...
// 				}
						...
						WHERE
							(
								(wcl.logged_table_num = " . ( (int) $this->tableNum() )
			                                   . ") AND (wcl.logged_row_id IN (" . implode( ", ", $va_set_ids ) . "))
								AND (csl.locale_id = ?)
							)
						UNION
						SELECT DISTINCT
						...
						WHERE
							 (
								(wcls.subject_table_num = " . ( (int) $this->tableNum() )
			                                   . ") AND (wcls.subject_row_id IN (" . implode( ", ", $va_set_ids )
			                                   . ")) AND wcl.logged_table_num IN ("
			                                   . Datamodel::getTableNum( "ca_set_items" ) . ", "
			                                   . Datamodel::getTableNum( "ca_sets_x_users" ) . ", "
			                                   . Datamodel::getTableNum( "ca_sets_x_user_groups" ) . ")
								AND (csl.locale_id = ?)
							)
			...
			if ( $q_change_log->numRows() ) {
				while ( $q_change_log->nextRow() ) {
					...
					$va_tmp['snapshot']
					                                                           = caUnserializeForDatabase( $q_change_log->get( 'snapshot' ) );
			...
			$q_set_comments = $o_db->query( "
								...
								WHERE c.table_num = " . ( (int) $this->tableNum() ) . " AND c.row_id IN ("
			                                . implode( ", ", $va_set_ids ) . ")
			...
			if ( $q_set_comments->numRows() ) {
				while ( $q_set_comments->nextRow() ) {
					...
					$va_tmp["logged_table_num"]
					                                                             = Datamodel::getTableNum( "ca_item_comments" );
			...
			$q_set_item_comments = $o_db->query( "
								...
								WHERE c.table_num = " . ( Datamodel::getTableNum( "ca_set_items" ) )
			                                     . " AND si.set_id IN (" . implode( ", ", $va_set_ids ) . ")
								ORDER BY c.created_on desc
							" );
	...
	public function getSetUsers() {
		...
		if ( $q_set_users->numRows() ) {
			while ( $q_set_users->nextRow() ) {
				$va_users[ $q_set_users->get( "user_id" ) ] = array(
					"user_id" => $q_set_users->get( "user_id" ),
					"name"    => trim( $q_set_users->get( "fname" ) . " " . $q_set_users->get( "lname" ) ),
					"email"   => $q_set_users->get( "email" ),
					"access"  => $q_set_users->get( "access" )
				);
			}
		}
		...
		if ( $q_set_owner->numRows() ) {
			$q_set_owner->nextRow();
			$va_users[ $this->get( "user_id" ) ] = array(
				"owner"   => true,
				"user_id" => $this->get( "user_id" ),
				"name"    => trim( $q_set_owner->get( "fname" ) . " " . $q_set_owner->get( "lname" ) ),
				"email"   => $q_set_owner->get( "email" ),
				"access"  => 2
			);
		}
	...
	public function getSetGroups() {
		...
		if ( $q_set_groups->numRows() ) {
			while ( $q_set_groups->nextRow() ) {
				$va_groups[ $q_set_groups->get( "group_id" ) ] = array(
					"group_id" => $q_set_groups->get( "group_id" ),
					"name"     => $q_set_groups->get( "name" ),
					"access"   => $q_set_groups->get( "access" )
				);
			}
		}
	 ...
	 * @param string      $ps_bundle_name Optional bundle name to test write-ability on. If omitted write-ability is
	 *                                    considered for the item as a whole.
	 *
	...
	public function isSaveable( $po_request, $ps_bundle_name = null ) {
		...
		if ( $ps_bundle_name ) {
			if ( $po_request->user->getBundleAccessLevel( $this->tableName(), $ps_bundle_name )
	...
	public function duplicateItemsInSet( $pn_user_id, $pa_options = array() ) {
		...
		if ( caGetOption( 'addToCurrentSet', $pa_options, false ) ) {
			...
			$t_set_to_add_dupes_to->addLabel( array(
				'name' => $this->getLabelForDisplay() . ' ' . _t( '[Duplicates]' )
		...
		foreach ( $va_items as $vn_row_id ) {
			...
			if ( ! $t_user->canDoAction( 'can_duplicate_' . $t_instance->tableName() ) ) {
				$this->postError( 2580, _t( 'You do not have permission to duplicate these items' ),
					'ca_sets->duplicateItemsInSet()' );
			...
			$t_dupe = $t_instance->duplicate( array(
				'user_id'                       => $pn_user_id,
				'duplicate_nonpreferred_labels' => $t_user->getPreference( $t_instance->tableName()
				                                                           . '_duplicate_nonpreferred_labels' ),
				'duplicate_attributes'          => $t_user->getPreference( $t_instance->tableName()
				                                                           . '_duplicate_attributes' ),
				'duplicate_relationships'       => $t_user->getPreference( $t_instance->tableName()
				                                                           . '_duplicate_relationships' ),
				'duplicate_media'               => $t_user->getPreference( $t_instance->tableName()
				                                                           . '_duplicate_media' ),
				'duplicate_subitems'            => $t_user->getPreference( $t_instance->tableName()
				                                                           . '_duplicate_subitems' )
			) );
	 ...
	 *                        access = Consider set to exist if user has at least the specified access level. If
	 *                        user_id is omitted then this option has no effect. If user_id is set and this option is
	 *                        omitted, then a set will be considered to exist if the user has at least read access.
	 *                        checkAccess = Consider set to exist if it has a public access level with the specified
	 *                        values. Can be a single value or array if you wish to filter on multiple public access
	 *                        values.
	 *
	 * @return bool
	 */
	static public function setExists( $set_code, $options = null ) {
		if ( $ids = ca_sets::find( [ 'set_code' => $set_code ], [ 'returnAs' => 'ids' ] ) ) {
			...
			if ( $user_id = caGetOption( 'user_id', $options, null ) ) {
				...
				return $t_set->haveAccessToSet( $user_id, caGetOption( 'access', $options, null ), $id,
					[ 'access' => caGetOption( 'access', $options, null ) ] );
			}
	...
	static public function setContents( $set_code, $options = null ) {
		...
		if ( ( $user_id = caGetOption( 'user_id', $options, null ) )
		     && ! $set->haveAccessToSet( $user_id, caGetOption( 'access', $options, null ), $set->getPrimaryKey(),
				[ 'access' => caGetOption( 'access', $options, null ) ] )
		) {
			return false;
		}
		...
		$reps
			= array_values( caExtractValuesByUserLocale( $set->getItems( [
			'thumbnailVersions' => caGetOption( 'versions', $options, [ 'large' ] ),
			'template'          => caGetOption( 'template', $options, null )
		] ) ) );

		return array_map( function ( $r ) {
			return [
				'key'            => md5( $r['representation_url_large'] ),
				'url'            => $r['representation_url_large'],
				'media_tag'      => $r['representation_tag_large'],
				'media_tag_link' => $r['representation_tag_large_as_link'],
				'caption'        => $r['displayTemplate']
			];
		}, $reps );
	}
	# ---------------------------------------------------------------
