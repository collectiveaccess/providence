...
BaseModel::$s_ca_models_definitions['ca_metadata_elements'] = array(
    ...
    'FIELDS' => array(
        'element_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique numeric identifier used by CollectiveAccess internally to identify this metadata element'
            )
        ),
        'parent_id' => array(
            'FIELD_TYPE' => FT_NUMBER,
        ...
        'documentation_url' => array(
            ...
            'DESCRIPTION' => _t(
                'URL pointing to documentation for this metadata element. Leave blank if no documentation URL exists.'
            ),
        ...
        'list_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Specifies the list to use as value for this element. Element must be a list type for this to apply.'
            ),
            'BOUNDS_VALUE' => array(0, 65535)
        ),
        'settings' => array(
        ...
        'rank' => array(
            ...
            'LABEL' => _t('Sort order'),
            'DESCRIPTION' => _t(
                'The relative priority of the element when displayed in a list with other element. Lower numbers indicate higher priority.'
            ),
            'BOUNDS_VALUE' => array(0, 65535)
        ),
        'hier_left' => array(
...
class ca_metadata_elements extends LabelableBaseModelWithAttributes implements ITakesSettings
{
    ...
    public function flushCacheForElement()
    {
        ...
        $vs_key = caMakeCacheKeyFromOptions(
            ['table_num' => null, 'type_id' => null, 'element_id' => $this->getPrimaryKey()]
        );
    ...
    public function getElementsInSet($pn_element_id = null, $pb_use_cache = true, $pa_options = null)
    {
        ...
        if ($pb_use_cache && CompositeCache::contains($pn_element_id, 'ElementSets')) {
            ...
            return (caGetOption('idsOnly', $pa_options, false) ? caExtractArrayValuesFromArrayOfArrays(
                $va_set,
                'element_id'
            ) : $va_set);
        }
        ...
        $va_root = null;
        foreach ($va_hier as $va_element) {
            $va_element['NODE']['settings'] = unserialize(
                base64_decode($va_element['NODE']['settings'])
            );    // decode settings vars into associative array
    ...
    public static function getElementSetsWithSetting($ps_setting, $pm_value = null, $pa_options = null)
    {
        return array_map(
            ...
            array_filter(
                ca_metadata_elements::find('*', ['returnAs' => 'arrays']),
                function ($v) use ($ps_setting, $pm_value) {
                    $va_settings = caUnserializeForDatabase($v['settings']);
                    if (isset($va_settings[$ps_setting]) && ((!is_null(
                                    $pm_value
                                ) && ($va_settings[$ps_setting] == $pm_value)) || (is_null(
                                    $pm_value
    ...
    public function settingHTMLFormElement($ps_setting, $pa_options = null)
    {
        ...
        if (((int)$this->get(
                    'parent_id'
                ) > 0) && isset($va_properties['validForRootOnly']) && $va_properties['validForRootOnly']) {
            return false;
        }

        if (((int)$this->get(
                    'parent_id'
                ) == 0) && isset($va_properties['validForNonRootOnly']) && $va_properties['validForNonRootOnly']) {
            return false;
        }
        ...
        switch ($va_properties['displayType']) {
            ...
            case DT_FIELD:
                if ($va_properties["height"] == 1) {
                    $vs_return .= '<input name="' . $vs_input_name . '" type="text" size="' . $va_properties["width"] . '" value="' . $this->getSetting(
                            $ps_setting
                        ) . '" />' . "\n";
                } else {
                    if ($va_properties["height"] > 1) {
                        $vs_return .= '<textarea name="' . $vs_input_name . '" cols="' . $va_properties["width"] . '" rows="' . $va_properties["height"] . '">' . $this->getSetting(
                                $ps_setting
                            ) . '</textarea>' . "\n";
                    }
                }
            ...
            # --------------------------------------------
            case DT_PASSWORD:
                $vs_return .= '<input name="' . $vs_input_name . '" type="password" size="' . $va_properties["width"] . '" value="' . $this->getSetting(
                        $ps_setting
                    ) . '" />' . "\n";
                break;
            # --------------------------------------------
            ...
            # --------------------------------------------
            case DT_SELECT:
                $vn_width = (isset($va_properties['width']) && (strlen(
                            $va_properties['width']
                        ) > 0)) ? $va_properties['width'] : "100px";
                $vn_height = (isset($va_properties['height']) && (strlen(
                            $va_properties['height']
                        ) > 0)) ? $va_properties['height'] : "50px";
                ...
                if ($va_properties['useList']) {
                    ...
                    $vs_return .= $t_list->getListAsHTMLFormElement(
    ...
    public static function getRootElementsAsList(
        $pm_table_name_or_num = null,
        $pm_type_name_or_id = null,
        $pb_use_cache = true,
        $pb_return_stats = false
    ) {
        return ca_metadata_elements::getElementsAsList(
            true,
    ...
    public static function getElementListAsHTMLSelect(
        ...
        foreach ($va_elements as $va_element) {
            ...
            $va_list[$va_element['element_id']] = (($va_element['parent_id'] > 0) ? self::getElementLabel(
                        $va_element['hier_element_id']
     ...
     *
     * @param $pb_root_elements_only bool If true, then only root elements are returned; default is false
     ...
     * @param $pb_use_cache bool Optional control for list cache; if true [default] then the will be cached for the request; if false the list will be generated from the database. The list is always generated at least once in the current request - there is no inter-request caching
     * @param $pa_data_types array Optional list of element data types to filter on.
     *
     * @return array A List of elements. Each list item is an array with keys set to field names; there is one additional value added with key "display_label" set to the display label of the element in the current locale
     */
    public static function getElementsAsList(
        $pb_root_elements_only = false,
        $pm_table_name_or_num = null,
        $pm_type_name_or_id = null,
        $pb_use_cache = true,
        $pb_return_stats = false,
        $pb_index_by_element_code = false,
        $pa_data_types = null
    ) {
        $vn_table_num = Datamodel::getTableNum($pm_table_name_or_num);
        $vs_cache_key = md5(
            $vn_table_num . '/' . $pm_type_name_or_id . '/' . ($pb_root_elements_only ? '1' : '0') . '/' . ($pb_index_by_element_code ? '1' : '0') . serialize(
                $pa_data_types
            )
        );
     ...
     *
     * @param $pb_root_elements_only bool If true, then only root elements are counted; default is false
    ...
    public static function getSortableElements($pm_table_name_or_num, $pm_type_name_or_id = null, $pa_options = null)
    {
        ...
        if (!caGetOption('noCache', $pa_options, false) && CompositeCache::contains('ElementsSortable') && is_array(
                $va_cached_data = CompositeCache::fetch('ElementsSortable')
        ...
        foreach ($va_elements as $vn_id => $va_element) {
            ...
            if ($va_element['settings']['canBeUsedInSort']) {
                $va_element['typeRestrictions'] = array_shift(
                    self::getTypeRestrictionsAsList($va_element['element_code'])
                );
    ...
    public static function elementIsInUse($pm_element_code_or_id, $pa_options = null)
    {
        ...
        $o_db = new Db();
        $qr_res = $o_db->query(
            "SELECT count(*) c FROM ca_attribute_values WHERE element_id = ? LIMIT 1",
            [$vn_element_id]
        );
    ...
    public static function getUIUsageCounts($pm_element_code_or_id = null)
    {
        ...
        while ($qr_use_counts->nextRow()) {
            ...
            if (preg_match(
                "!^($vs_table\.|ca_attribute_)([A-Za-z0-9_\\-]+)\$!",
                $qr_use_counts->get('bundle_name'),
                $va_matches
            )) {
                if (!($t_table = Datamodel::getInstanceByTableNum($vn_table_num, true))) {
                    continue;
                }
                $va_counts_by_attribute[$va_matches[2]][$t_table->getProperty('NAME_PLURAL')] = $qr_use_counts->get(
                    'c'
                );
            }
        }
    ...
    public static function getTypeRestrictionsAsList($pm_element_code_or_id = null)
    {
        ...
        while ($qr_restrictions->nextRow()) {
            ...
            $va_restrictions[$qr_restrictions->get('element_code')][$t_table->getProperty(
                'NAME_PLURAL'
            )][$vn_type_id] = $vs_type_name;
        }
    ...
    public static function getElementID($pm_element_code_or_id, $pa_options = null)
    {
        ...
        if (!caGetOption('noCache', $pa_options, false) && MemoryCache::contains(
    ...
    public static function getElementDefaultValue($pm_element_code_or_id)
    {
        ...
        if ($t_element = ca_metadata_elements::getInstance($pm_element_code_or_id)) {
            //$vm_return = (int) $t_element->get('datatype');
            $default_setting_name = Attribute::getValueDefaultSettingForDatatype(
                ca_metadata_elements::getDataTypeForElementCode($pm_element_code_or_id)
            );
    ...
    public function getTypeRestrictions($pn_table_num = null, $pn_type_id = null)
    {
        ...
        $vs_key = caMakeCacheKeyFromOptions(
            ['table_num' => $pn_table_num, 'type_id' => $pn_type_id, 'element_id' => $vn_element_id]
        );
    ...
    public function getTypeRestrictionInstanceForElement($pn_table_num, $pn_type_id)
    {
        ...
        if (($pn_type_id > 0) && $t_restriction->load(
                array(
            ...
            if ($t_restriction->load(
                array('table_num' => (int)$pn_table_num, 'type_id' => null, 'element_id' => (int)$vn_element_id)
            )) {
                return $t_restriction;
            }
            ...
            foreach ($va_presets as $vs_code => $va_preset) {
                foreach ($va_preset['values'] as $vs_k => $vs_v) {
                    ...
                    switch ((int)$va_element_info[$vs_k]['datatype']) {
                        case 3:
                            $va_presets[$vs_code]['values'][$va_element_code_to_ids[$vs_k]] = caGetListItemID(
                                $va_element_info[$vs_k]['list_id'],
                                $vs_v
                            );
                            break;
                        default:
