...
BaseModel::$s_ca_models_definitions['ca_metadata_elements'] = array(
	...
	'FIELDS'        => array(
		'element_id'        => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this metadata element' )
		),
		'parent_id'         => array(
			'FIELD_TYPE'     => FT_NUMBER,
		...
		'hier_element_id'   => array(
			...
			'DESCRIPTION'    => 'Identifier of element that is root of the element hierarchy.'
		),
		'element_code'      => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'FILTER'         => '!^[\p{L}0-9_]+$!u',
			'LABEL'          => _t( 'Element code' ),
			'DESCRIPTION'    => _t( 'Unique alphanumeric code for the metadata element.' ),
		...
		'documentation_url' => array(
			...
			'DESCRIPTION'    => _t( 'URL pointing to documentation for this metadata element. Leave blank if no documentation URL exists.' ),
		...
		'datatype'          => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Datatype' ),
			'DESCRIPTION'    => _t( 'Data type of metadata element.' ),
		...
		'list_id'           => array(
			...
			'DEFAULT'         => '',
			'LABEL'           => _t( 'Use list (for list elements only)' ),
			'DESCRIPTION'     => _t( 'Specifies the list to use as value for this element. Element must be a list type for this to apply.' ),
			'BOUNDS_VALUE'    => array( 0, 65535 )
		),
		'settings'          => array(
			'FIELD_TYPE'     => FT_VARS,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Settings' ),
			'DESCRIPTION'    => _t( 'Type-specific settings for metadata element' )
		),
		'rank'              => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Sort order' ),
			'DESCRIPTION'    => _t( 'The relative priority of the element when displayed in a list with other element. Lower numbers indicate higher priority.' ),
			'BOUNDS_VALUE'   => array( 0, 65535 )
		),
		'hier_left'         => array(
			'FIELD_TYPE'     => FT_NUMBER,
...
class ca_metadata_elements extends LabelableBaseModelWithAttributes implements ITakesSettings {
	...
	public function flushCacheForElement() {
		...
		$vs_key = caMakeCacheKeyFromOptions( [
			'table_num'  => null,
			'type_id'    => null,
	...
	static public function getElementsForSet( $pn_element_id, $pa_options = null ) {
		...
		return $t_element->getElementsInSet( $pn_element_id, ! caGetOption( 'noCache', $pa_options, false ),
			$pa_options );
	}
	# ------------------------------------------------------
	...
	public function getElementsInSet( $pn_element_id = null, $pb_use_cache = true, $pa_options = null ) {
		...
		if ( $pb_use_cache && CompositeCache::contains( $pn_element_id, 'ElementSets' ) ) {
			...
			return ( caGetOption( 'idsOnly', $pa_options, false ) ? caExtractArrayValuesFromArrayOfArrays( $va_set,
				'element_id' ) : $va_set );
		}
		...
		$va_root = null;
		foreach ( $va_hier as $va_element ) {
			$va_element['NODE']['settings']
				                                 = unserialize( base64_decode( $va_element['NODE']['settings'] ) );    // decode settings vars into associative array
			...
			if ( ! trim( $va_element['NODE']['parent_id'] ) ) {
				...
				$va_element_set[ $va_element['NODE']['parent_id'] ][ $va_element['NODE']['rank'] ][ $va_element['NODE']['element_id'] ]
					= $va_element['NODE'];
			}
		}
		...
		$va_tmp                           = array_merge( $va_tmp,
			$this->_getSortedElementsForParent( $va_element_set, $va_root['element_id'] ) );
	 ...
	 * @param mixed  $pm_value   Setting value. If set to null or omitted any element with a setting value that
	...
	public static function getElementSetsWithSetting( $ps_setting, $pm_value = null, $pa_options = null ) {
		return array_map( function ( $v ) {
			...
			return $v;
		}, array_filter( ca_metadata_elements::find( '*', [ 'returnAs' => 'arrays' ] ),
			function ( $v ) use ( $ps_setting, $pm_value ) {
				$va_settings = caUnserializeForDatabase( $v['settings'] );
				if ( isset( $va_settings[ $ps_setting ] )
				     && ( ( ! is_null( $pm_value )
				            && ( $va_settings[ $ps_setting ] == $pm_value ) )
				          || ( is_null( $pm_value )
				               && (bool) $va_settings[ $ps_setting ] ) )
				) {
					return true;
				}
	...
	public function setSetting( $ps_setting, $pm_value, &$ps_error = null ) {
		...
		if ( ( $va_properties['formatType'] == FT_NUMBER ) && ( $va_properties['displayType'] == DT_CHECKBOXES )
		     && ( ! $pm_value )
		) {
			$pm_value = '0';
		}
	...
	public function getSetting( $ps_setting, $pb_use_cache = false ) {
		...
		$vs_default = isset( $va_available_settings[ $ps_setting ]['default'] )
			? $va_available_settings[ $ps_setting ]['default'] : null;

		return ( isset( $this->opa_element_settings[ $ps_setting ] ) ? $this->opa_element_settings[ $ps_setting ]
			: $vs_default );
	}
	# ------------------------------------------------------
	...
	public function settingHTMLFormElement( $ps_setting, $pa_options = null ) {
		...
		if ( ( (int) $this->get( 'parent_id' ) > 0 ) && isset( $va_properties['validForRootOnly'] )
		     && $va_properties['validForRootOnly']
		) {
			return false;
		}

		if ( ( (int) $this->get( 'parent_id' ) == 0 ) && isset( $va_properties['validForNonRootOnly'] )
		     && $va_properties['validForNonRootOnly']
		) {
			return false;
		}
		...
		switch ( $va_properties['displayType'] ) {
			...
			case DT_FIELD:
				if ( $va_properties["height"] == 1 ) {
					$vs_return .= '<input name="' . $vs_input_name . '" type="text" size="' . $va_properties["width"]
					              . '" value="' . $this->getSetting( $ps_setting ) . '" />' . "\n";
				} else if ( $va_properties["height"] > 1 ) {
					$vs_return .= '<textarea name="' . $vs_input_name . '" cols="' . $va_properties["width"]
					              . '" rows="' . $va_properties["height"] . '">' . $this->getSetting( $ps_setting )
					              . '</textarea>' . "\n";
				}
				break;
			# --------------------------------------------
			case DT_PASSWORD:
				$vs_return .= '<input name="' . $vs_input_name . '" type="password" size="' . $va_properties["width"]
				              . '" value="' . $this->getSetting( $ps_setting ) . '" />' . "\n";
				break;
			# --------------------------------------------
			case DT_CHECKBOXES:
			...
			# --------------------------------------------
			case DT_SELECT:
				$vn_width  = ( isset( $va_properties['width'] ) && ( strlen( $va_properties['width'] ) > 0 ) )
					? $va_properties['width'] : "100px";
				$vn_height = ( isset( $va_properties['height'] ) && ( strlen( $va_properties['height'] ) > 0 ) )
					? $va_properties['height'] : "50px";
				...
				if ( $va_properties['showMediaElementBundles'] ) {
					...
					foreach ( $va_restrictions as $va_restriction ) {
						...
						if ( is_array(
							$va_elements = ca_metadata_elements::getElementsAsList( false, $va_restriction['table_num'],
							...
							foreach ( $va_elements as $vn_element_id => $va_element_info ) {
								if ( $va_element_info['datatype'] == __CA_ATTRIBUTE_VALUE_MEDIA__ ) {
									if ( $va_element_info['parent_id'] > 0 ) {
										$va_select_opts[ $va_elements[ $va_element_info['hier_element_id'] ]['display_label']
										                 . " &gt; " . $va_element_info['display_label'] ]
											= $va_elements[ $va_element_info['hier_element_id'] ]['element_code'] . "."
											  . $va_element_info['element_code'];
									} else {
										$va_select_opts[ $va_element_info['display_label'] ]
											= $va_element_info['element_code'];
									}
								}
							}
				...
				if ( isset( $va_properties['refreshOnChange'] ) && (bool) $va_properties['refreshOnChange'] ) {
					$va_attr['onchange']
						= "caSetElementsSettingsForm({ {$vs_input_name} : jQuery(this).val() }); return false;";
				}
				...
				if ( $va_properties['useList'] ) {
					...
					$vs_return .= $t_list->getListAsHTMLFormElement( $va_properties['useList'], $vs_input_name,
						$va_attr, $va_opts );
				} else {
		...
		TooltipManager::add( '.' . $vs_label_id,
			"<h3>" . $va_properties["label"] . "</h3>" . $va_properties["description"] );
	...
	/**
	 * Returns list of "root" metadata elements – elements that are either freestanding or at the top of an element
	 * hierarchy (Eg. have sub-elements but are not sub-elements themselves). Root elements are used to reference the
	 * element as a whole (including sub-elements), so it is useful to be able to obtain a list of these elements with
	 * sub-elements filtered out.
	 *
	 * @param $pm_table_name_or_num mixed Optional table name or number to filter list with. If specified then only
	 *                              elements that have a type restriction to the table are returned. If omitted
	 *                              (default) then all root elements, regardless of type restrictions, are returned.
	 * @param $pm_type_name_or_id   mixed
	 *
	 * @return array A List of elements. Each list item is an array with keys set to field names; there is one
	 *               additional value added with key "display_label" set to the display label of the element in the
	 *               current locale
	 */
	public static function getRootElementsAsList(
		$pm_table_name_or_num = null, $pm_type_name_or_id = null, $pb_use_cache = true, $pb_return_stats = false
	) {
		return ca_metadata_elements::getElementsAsList( true, $pm_table_name_or_num, $pm_type_name_or_id, $pb_use_cache,
			$pb_return_stats );
	}
	# ------------------------------------------------------
	 ...
	 * @return string
	 */
	public static function getElementListAsHTMLSelect(
		$ps_select_name, array $pa_attributes = [], array $pa_options = []
	) {
		...
		foreach ( $va_elements as $va_element ) {
			...
			if ( is_array( $pa_restrict_to_datatypes )
			     && ! in_array( $va_element['datatype'], $pa_restrict_to_datatypes )
			) {
				continue;
			}

			$va_list[ $va_element['element_id'] ] = ( ( $va_element['parent_id'] > 0 )
					? self::getElementLabel( $va_element['hier_element_id'] ) . " &gt; " : '' )
			                                        . $va_element['display_label'] . ' (' . $va_element['element_code']
			                                        . ')';
		}
		natsort( $va_list );
	 ...
	 * @param $pm_table_name_or_num  mixed Optional table name or number to filter list with. If specified then only
	 *                               elements that have a type restriction to the table are returned. If omitted
	 *                               (default) then all elements, regardless of type restrictions, are returned.
	 * @param $pm_type_name_or_id    mixed Optional type code or type_id to restrict elements to.  If specified then
	 *                               only elements that have a type restriction to the specified table and type are
	 *                               returned.
	 * @param $pb_use_cache          boolean Optional control for list cache; if true [default] then the will be cached
	 *                               for the request; if false the list will be generated from the database. The list
	 *                               is always generated at least once in the current request - there is no
	 ...
	 *               current locale
	 */
	public static function getElementsAsList(
		$pb_root_elements_only = false, $pm_table_name_or_num = null, $pm_type_name_or_id = null, $pb_use_cache = true,
		$pb_return_stats = false, $pb_index_by_element_code = false, $pa_data_types = null
	) {
		$vn_table_num = Datamodel::getTableNum( $pm_table_name_or_num );
		$vs_cache_key = md5( $vn_table_num . '/' . $pm_type_name_or_id . '/' . ( $pb_root_elements_only ? '1' : '0' )
		                     . '/' . ( $pb_index_by_element_code ? '1' : '0' ) . serialize( $pa_data_types ) );
		...
		if ( $vn_table_num ) {
			...
			if ( $pm_type_name_or_id ) {
				...
				if ( $vn_type_id = $t_list_item->getPrimaryKey() ) {
					...
					$va_wheres[]
						               = '((cmtr.type_id IS NULL) OR (cmtr.type_id = ?) OR (cmtr.include_subtypes = 1 AND cmtr.type_id IN (?)))';
	 ...
	 * @param $pm_table_name_or_num  mixed Optional table name or number to filter list with. If specified then only
	 *                               elements that have a type restriction to the table are counted. If omitted
	 *                               (default) then all elements, regardless of type restrictions, are returned.
	 * @param $pm_type_name_or_id    mixed Optional type code or type_id to restrict elements to.  If specified then
	 *                               only elements that have a type restriction to the specified table and type are
	 *                               counted.
	...
	public static function getElementCount(
		$pb_root_elements_only = false, $pm_table_name_or_num = null, $pm_type_name_or_id = null
	) {
		$vn_table_num = Datamodel::getTableNum( $pm_table_name_or_num );
	...
	public static function getSortableElements( $pm_table_name_or_num, $pm_type_name_or_id = null, $pa_options = null
		...
		if ( ! caGetOption( 'noCache', $pa_options, false ) && CompositeCache::contains( 'ElementsSortable' )
		     && is_array( $va_cached_data = CompositeCache::fetch( 'ElementsSortable' ) )
		     && isset( $va_cached_data[ $vs_cache_key ] )
		) {
		...
		foreach ( $va_elements as $vn_id => $va_element ) {
			...
			if ( $va_element['settings']['canBeUsedInSort'] ) {
				$va_element['typeRestrictions']
					= array_shift( self::getTypeRestrictionsAsList( $va_element['element_code'] ) );
	...
	public static function elementIsInUse( $pm_element_code_or_id, $pa_options = null ) {
		...
		$o_db   = new Db();
		$qr_res = $o_db->query( "SELECT count(*) c FROM ca_attribute_values WHERE element_id = ? LIMIT 1",
			[ $vn_element_id ] );
	 ...
	 *
	 * @return array List of user interfaces that include the currently loaded element. Array is keyed on ui_id. Values
	 *               are arrays with keys set to ca_ui_editor_labels field names and associated values.
	 */
	 ...
	 *
	 * @param $pm_element_code_or_id mixed Optional element_id or code. If set then counts are only returned for the
	 *                               specified element. If omitted then counts are returned for all elements.
	 *
	 * @return array Array of counts. Keys are element_codes, values are arrays keyed on table DISPLAY name (eg. "set
	 *               items", not "ca_set_items"). Values are the number of times the element is references in a user
	...
	static public function getUIUsageCounts( $pm_element_code_or_id = null ) {
		...
		while ( $qr_use_counts->nextRow() ) {
			...
			if ( preg_match( "!^($vs_table\.|ca_attribute_)([A-Za-z0-9_\\-]+)\$!", $qr_use_counts->get( 'bundle_name' ),
				$va_matches )
			) {
				if ( ! ( $t_table = Datamodel::getInstanceByTableNum( $vn_table_num, true ) ) ) {
					continue;
				}
				$va_counts_by_attribute[ $va_matches[2] ][ $t_table->getProperty( 'NAME_PLURAL' ) ]
					= $qr_use_counts->get( 'c' );
			}
		}
	...
	static public function getTypeRestrictionsAsList( $pm_element_code_or_id = null ) {
		...
		while ( $qr_restrictions->nextRow() ) {
			...
			$va_restrictions[ $qr_restrictions->get( 'element_code' ) ][ $t_table->getProperty( 'NAME_PLURAL' ) ][ $vn_type_id ]
				= $vs_type_name;
		}
	...
	static public function getElementID( $pm_element_code_or_id, $pa_options = null ) {
		...
		if ( ! caGetOption( 'noCache', $pa_options, false )
		     && MemoryCache::contains( $pm_element_code_or_id, 'ElementIDs' )
		) {
	...
	static public function getElementDefaultValue( $pm_element_code_or_id ) {
		...
		if ( $t_element = ca_metadata_elements::getInstance( $pm_element_code_or_id ) ) {
			...
			$default_setting_name
				       = Attribute::getValueDefaultSettingForDatatype( ca_metadata_elements::getDataTypeForElementCode( $pm_element_code_or_id ) );
	...
	public function getTypeRestrictionInstanceForElement( $pn_table_num, $pn_type_id ) {
		...
		if ( ( $pn_type_id > 0 )
		     && $t_restriction->load( array(
				'table_num'  => (int) $pn_table_num,
				'type_id'    => (int) $pn_type_id,
				'element_id' => (int) $vn_element_id
			) )
		) {
	...
	public function htmlFormElement( $ps_field, $ps_format = null, $pa_options = null ) {
		if ( $ps_field == 'list_id' ) {
			...
			ksort( $va_opts );
			$vs_format = str_replace( '^LABEL', $vs_field_label = $this->getFieldInfo( 'list_id', 'LABEL' ),
				$vs_format );
			$vs_format = str_replace( '^EXTRA', '', $vs_format );

			$vs_format = str_replace( '^ELEMENT', caHTMLSelect( $ps_field, $va_opts, array( 'id' => $ps_field ),
				array( 'value' => $this->get( 'list_id' ) ) ), $vs_format );

			if ( ! isset( $pa_options['no_tooltips'] ) || ! $pa_options['no_tooltips'] ) {
				TooltipManager::add( '#list_id',
					"<h3>{$vs_field_label}</h3>" . $this->getFieldInfo( 'list_id', 'DESCRIPTION' ),
			...
			foreach ( $va_presets as $vs_code => $va_preset ) {
				foreach ( $va_preset['values'] as $vs_k => $vs_v ) {
					...
					switch ( (int) $va_element_info[ $vs_k ]['datatype'] ) {
						case 3:
							$va_presets[ $vs_code ]['values'][ $va_element_code_to_ids[ $vs_k ] ]
								= caGetListItemID( $va_element_info[ $vs_k ]['list_id'], $vs_v );
							break;
						default:
	...
	/**
	 * Ensure that ElasticSearch mapping gets updated on next indexing run by setting the last time to be the start of
	 * the epoch.
	 */
