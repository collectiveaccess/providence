...
BaseModel::$s_ca_models_definitions['ca_site_templates'] = array(
	...
	'FIELDS'        => array(
		...
		'template_code' => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Template code' ),
			'DESCRIPTION'    => _t( 'Unique alphanumeric code for this template.' ),
		...
		'title'         => array(
			...
			'DESCRIPTION'    => _t( 'Short descriptive title for this template.' ),
		...
		'description'   => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Full description' ),
			'DESCRIPTION'    => _t( 'Full usage description for this template.' ),
			'BOUNDS_LENGTH'  => array( 0, 255 )
		),
		'template'      => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Template' ),
			'DESCRIPTION'    => _t( 'Full template text.' )
		),
		'tags'          => array(
			'FIELD_TYPE'     => FT_VARS,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Extracted template tags' ),
			'DESCRIPTION'    => _t( 'List of tags extracted from template.' )
		),
		'deleted'       => array(
			'FIELD_TYPE'     => FT_BIT,
			'DISPLAY_TYPE'   => DT_OMIT,
...
class ca_site_templates extends BundlableLabelableBaseModelWithAttributes {
	...
	public static function getTemplateListAsHTMLSelect( $pa_options = null ) {
		...
		return caHTMLSelect( caGetOption( 'name', $pa_options, 'template_id' ), $va_options,
			[ 'id' => caGetOption( 'id', $pa_options, null ) ] );
	}
	# ------------------------------------------------------
	...
	public function getHTMLFormElements( $pa_values = null, $pa_options = null ) {
		...
		foreach ( $va_tags as $vs_tag => $va_tag_info ) {
			...
			$va_form_elements[] = [
				...
				'element' => caHTMLTextInput( "{$vs_tagname_prefix}_{$vs_tag}",
					[ 'id' => "{$vs_tagname_prefix}_{$vs_tag}", 'value' => $pa_values[ $vs_tag ] ], [
						'width'            => caGetOption( 'width', $va_tag_info, '300px' ),
						'height'           => caGetOption( 'height', $va_tag_info, '35px' ),
						'usewysiwygeditor' => caGetOption( 'usewysiwygeditor', $va_tag_info, false ),
						'cktoolbar'        => $vs_cktoolbar,
						'contentUrl'       => $vs_content_url,
						'lookupUrls'       => $va_lookup_urls
					] ),
			...
			if ( $vs_form_element_format ) {
				...
				$va_form_elements[ $vn_partial_index ]['element_with_label']
				                  = caProcessTemplate( $vs_form_element_format, [
					"LABEL"   => "<span id='{$vs_tagname_prefix}_{$vs_tag}_label'>"
					             . $va_form_elements[ $vn_partial_index ]['label'] . "</span>",
					"ELEMENT" => $va_form_elements[ $vn_partial_index ]['element'],
					"EXTRA"   => ''
				] );

				if ( $pb_include_tooltips && $va_tag_info['label'] ) {
					TooltipManager::add( "#{$vs_tagname_prefix}_{$vs_tag}_label",
						"<strong>" . $va_tag_info['label'] . "</strong><br/>" . $va_tag_info['description'] );
				}
			} elseif ( $pb_include_tooltips && $va_tag_info['label'] ) {
				TooltipManager::add( "#{$vs_tagname_prefix}_{$vs_tag}",
					"<strong>" . $va_tag_info['label'] . "</strong><br/>" . $va_tag_info['description'] );
			}
