...
BaseModel::$s_ca_models_definitions['ca_storage_locations'] = array(
    ...
    'FIELDS' => array(
        'location_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique numeric identifier used by CollectiveAccess internally to identify this storage location'
            )
        ),
        'parent_id' => array(
            'FIELD_TYPE' => FT_NUMBER,
        ...
        'type_id' => array(
            ...
            'LABEL' => _t('Type'),
            'DESCRIPTION' => _t(
                'The type of the storage location. In CollectiveAccess every storage location has a single "instrinsic" type that determines the set of descriptive and administrative metadata that can be applied to it.'
            )
        ),
        'idno' => array(
            'FIELD_TYPE' => FT_TEXT,
            ...
            'LABEL' => _t('Location identifier'),
            'DESCRIPTION' => _t('A unique alphanumeric identifier for this location.'),
        ...
        'idno_sort' => array(
            ...
            'IS_NULL' => false,
            'DEFAULT' => '',
            'LABEL' => 'Sortable location identifier',
            'DESCRIPTION' => 'Value used for sorting locations on identifier value.',
            'BOUNDS_LENGTH' => array(0, 255)
        ),
        'source_id' => array(
            'FIELD_TYPE' => FT_NUMBER,
            ...
            'LABEL' => _t('Source'),
            'DESCRIPTION' => _t(
                'Administrative source of storage location. This value is often used to indicate the administrative sub-division or legacy database from which the object originates, but can also be re-tasked for use as a simple classification tool if needed.'
            )
        ),
        'source_info' => array(
            'FIELD_TYPE' => FT_VARS,
            ...
            'DESCRIPTION' => 'Serialized array used to store source information for storage location information retrieved via web services [NOT IMPLEMENTED YET].'
        ),
        'hier_left' => array(
            'FIELD_TYPE' => FT_NUMBER,
            'DISPLAY_TYPE' => DT_OMIT,
        ...
        'access' => array(
            ...
            'LABEL' => _t('Access'),
            'DESCRIPTION' => _t('Indicates if location information is accessible to the public or not. ')
        ),
        'status' => array(
            'FIELD_TYPE' => FT_NUMBER,
            'DISPLAY_TYPE' => DT_SELECT,
            ...
            'LABEL' => _t('Status'),
            'DESCRIPTION' => _t('Indicates the current state of the storage location record.')
        ),
        'deleted' => array(
            'FIELD_TYPE' => FT_BIT,
            'DISPLAY_TYPE' => DT_OMIT,
            ...
            'DEFAULT' => 0,
            'LABEL' => _t('Is deleted?'),
            'DESCRIPTION' => _t('Indicates if the storage location is deleted or not.'),
        ...
        'color' => array(
            'FIELD_TYPE' => FT_TEXT,
            'DISPLAY_TYPE' => DT_COLORPICKER,
            'DISPLAY_WIDTH' => 10,
            'DISPLAY_HEIGHT' => 1,
            'IS_NULL' => false,
            'DEFAULT' => '',
            'LABEL' => _t('Color'),
            'DESCRIPTION' => _t('Color to identify the editor UI with')
        ),
        'icon' => array(
            'FIELD_TYPE' => FT_MEDIA,
            'DISPLAY_TYPE' => DT_FIELD,
            ...
            'LABEL' => _t('Icon'),
            'DESCRIPTION' => _t('Optional icon to identify the editor UI with')
        ),
        'is_enabled' => array(
            'FIELD_TYPE' => FT_BIT,
            'DISPLAY_TYPE' => DT_SELECT,
            ...
            'DEFAULT' => '1',
            'LABEL' => _t('Is enabled?'),
            'DESCRIPTION' => _t("If unchecked this item is disabled and can't be edited or used in new relationships"),
            'BOUNDS_VALUE' => array(0, 1)
        ),
        'view_count' => array(
            'FIELD_TYPE' => FT_NUMBER,
...
class ca_storage_locations extends BaseObjectLocationModel implements IBundleProvider, IHierarchy
{
    ...
    protected function initLabelDefinitions($pa_options = null)
    {
        ...
        $this->BUNDLES['ca_storage_locations_contents'] = array(
            'type' => 'special',
            'repeating' => false,
            'label' => _t('Current contents of location')
        );
    }
    ...
    public function getCurrentObjectIDs()
    {
        ...
        if ($vs_movement_storage_element = $this->getAppConfig()->get('movement_storage_location_date_element')) {
            ...
            if (is_array($va_movement_ids) && sizeof($va_movement_ids)) {
                ...
                $va_object_ids = $t_movement->getRelatedItems(
                    'ca_objects',
                    array(
                        'idsOnly' => true,
                        'showCurrentOnly' => true,
                        'row_ids' => $va_movement_ids
                    )
                );

                // ... then get the list of objects for which the *current* movement is one of ours
                $t_object = new ca_objects();
                $va_current_movement_ids = $t_object->getRelatedItems(
                    'ca_movements',
                    array(
                        'idsOnly' => false,
                        'showCurrentOnly' => true,
                        'row_ids' => $va_object_ids
                    )
                );
        ...
        //
        $va_direct_object_ids = $this->getRelatedItems(
            'ca_objects',
            array('idsOnly' => true, 'showCurrentOnly' => true)
        );
    ...
    {
        $vb_parent_changed = (parent::saveBundlesForScreenWillChangeParent(
                $pm_screen,
                $po_request,
                $pa_options
            ) == __CA_PARENT_CHANGED__);
        if (($vn_rc = parent::saveBundlesForScreen($pm_screen, $po_request, $pa_options)) && $vb_parent_changed) {
            ...
            $vs_movement_storage_location_relationship_type = $this->getAppConfig()->get(
                'movement_storage_location_tracking_relationship_type'
            );
            $vs_movement_object_relationship_type = $this->getAppConfig()->get(
                ...
                if (preg_match('!^(.*)_movement_form_name$!', $vs_key, $va_matches)) {
                    ...
                    if (is_array($va_object_ids) && sizeof($va_object_ids)) {
                        ...
                        if ($vs_movement_storage_location_relationship_type) {
                            $t_movement->addRelationship(
                                'ca_storage_locations',
                                $this->getPrimaryKey(),
                                $vs_movement_storage_location_relationship_type
                            );
                        }
                        ...
                        if ($vs_movement_object_relationship_type) {
                            foreach ($va_object_ids as $vn_object_id) {
                                $t_movement->addRelationship(
                                    'ca_objects',
                                    $vn_object_id,
                                    $vs_movement_object_relationship_type
                                );
                            }
                        }
                    }
     ...
     */
    public function getLocationContentsHTMLFormBundle(
        $po_request,
        $ps_form_name,
        $ps_placement_code,
        ...
        switch ($ps_mode) {
            case 'ca_storage_locations':
                ...
                if (is_array($va_object_ids) && sizeof($va_object_ids)) {
                    ...
                    // ... then get the list of objects for which the *current* movement is one of ours
                    $t_object = new ca_objects();
                    $va_current_locations_ids = $t_object->getRelatedItems(
                        'ca_storage_locations',
                        array(
                            'idsOnly' => false,
                            'showCurrentOnly' => true,
                    ...
                    foreach ($va_current_locations_ids as $va_location_info) {
                        if ($va_location_info['location_id'] == $this->getPrimaryKey()) {
                            $va_object_rels[] = $va_location_info['relation_id'];
                        }
                    }

                    return sizeof($va_object_rels) ? caMakeSearchResult(
            ...
            default:
                // Get current movements for location
                $va_location_ids = array_merge(
                    $this->get($x = $this->tableName() . ".children." . $this->primaryKey(), ['returnAsArray' => true]),
                    [$this->getPrimaryKey()]
                );

                $va_movement_ids = $this->getRelatedItems(
                    'ca_movements',
                    array('idsOnly' => true, 'row_ids' => $va_location_ids)
                );
                if (is_array($va_movement_ids) && sizeof($va_movement_ids)) {
                    // get list of objects on these movements...
                    $t_movement = new ca_movements();
                    ...
                    foreach ($va_current_movement_ids as $vn_i => $va_movement_info) {
                        if (in_array($va_movement_info['movement_id'], $va_movement_ids)) {
                            $va_movement_rels[] = $va_movement_info['relation_id'];
                        }
                    }

                    return sizeof($va_movement_rels) ? caMakeSearchResult(
