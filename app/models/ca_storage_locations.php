...
BaseModel::$s_ca_models_definitions['ca_storage_locations'] = array(
	...
	'FIELDS'        => array(
		'location_id'          => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this storage location' )
		),
		'parent_id'            => array(
			'FIELD_TYPE'     => FT_NUMBER,
		...
		'type_id'              => array(
			...
			'LABEL'          => _t( 'Type' ),
			'DESCRIPTION'    => _t( 'The type of the storage location. In CollectiveAccess every storage location has a single "instrinsic" type that determines the set of descriptive and administrative metadata that can be applied to it.' )
		),
		'idno'                 => array(
			'FIELD_TYPE'                => FT_TEXT,
		...
		'source_id'            => array(
			...
			'LABEL'                     => _t( 'Source' ),
			'DESCRIPTION'               => _t( 'Administrative source of storage location. This value is often used to indicate the administrative sub-division or legacy database from which the object originates, but can also be re-tasked for use as a simple classification tool if needed.' )
		),
		'source_info'          => array(
			'FIELD_TYPE'     => FT_VARS,
			'DISPLAY_TYPE'   => DT_OMIT,
...
class ca_storage_locations extends RepresentableBaseModel implements IBundleProvider, IHierarchy {
	 ...
	 * $pn_id is always ignored and is optional. The parameter is included for consistency with getHierarchyName()
	 * implementations in other models (ca_objects, ca_places, ca_list_items, etc.)
	 */
	...
	public function saveBundlesForScreen( $pm_screen, $po_request, &$pa_options ) {
		$vb_parent_changed = ( parent::saveBundlesForScreenWillChangeParent( $pm_screen, $po_request, $pa_options )
		                       == __CA_PARENT_CHANGED__ );

		if ( ! $this->getPrimaryKey()
		     && ( $this->get( 'ca_storage_locations.parent_id' ) <= 0 )
		) {   // For new records zero or negative parent_id means root
