...
class ca_guids extends BaseModel
{
    ...
    private static function addForRow($pn_table_num, $pn_row_id, $pa_options = null)
    {
        ...
        $o_db->query(
            "INSERT INTO ca_guids(table_num, row_id, guid) VALUES (?,?,?)",
            $pn_table_num,
            $pn_row_id,
            $vs_guid
        );
    ...
    public static function getAccessForGUID($ps_guid, $pa_access, $pa_options = null)
    {
        ...
        if (in_array(
            Datamodel::getTableName($va_info['table_num']),
            [
                'ca_lists',
                'ca_list_items',
                'ca_list_labels',
            ...
            if (($t_label = Datamodel::getInstanceByTableNum($va_info['table_num'], true)) && $t_label->load(
                    $va_info['row_id']
                )) {
            ...
            $qr_guid = $o_db->query(
                '
                SELECT access FROM ' . Datamodel::getTableName($vn_table_num) . " WHERE " . Datamodel::primaryKey(
                    $vn_table_num
                ) . ' = ?
            ' . (Datamodel::getFieldInfo($vn_table_num, 'deleted') ? ' AND deleted = 0' : ''),
                [$vn_row_id]
            );
            ...
            $qr_guid = $o_db->query(
                '
                SELECT access FROM ' . Datamodel::getTableName(
                    $va_info['table_num']
                ) . " WHERE " . Datamodel::primaryKey($va_info['table_num']) . ' = ?
