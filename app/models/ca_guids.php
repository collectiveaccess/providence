...
class ca_guids extends BaseModel {
	...
	public static function getForRow( $pn_table_num, $pn_row_id, $pa_options = null ) {
		...
		if ( $qr_guid->nextRow() ) {
			...
			if ( ! caGetOption( 'dontAdd', $pa_options )
			     && ( $t_instance = Datamodel::getInstance( $pn_table_num, true ) )
			) {
	...
	private static function addForRow( $pn_table_num, $pn_row_id, $pa_options = null ) {
		...
		$o_db->query( "INSERT INTO ca_guids(table_num, row_id, guid) VALUES (?,?,?)", $pn_table_num, $pn_row_id,
			$vs_guid );
	...
	public static function getAccessForGUID( $ps_guid, $pa_access, $pa_options = null ) {
		...
		if ( in_array( Datamodel::getTableName( $va_info['table_num'] ), [
			'ca_lists',
			'ca_list_items',
			'ca_list_labels',
			'ca_list_item_labels',
		...
		) {   //TODO: make tables for which we should ignore access configurable
			return true;
		} elseif ( Datamodel::isLabel( $va_info['table_num'] ) ) {
			if ( ( $t_label = Datamodel::getInstanceByTableNum( $va_info['table_num'], true ) )
			     && $t_label->load( $va_info['row_id'] )
			) {
			...
			if ( in_array( Datamodel::getTableName( $vn_table_num ), [
				'ca_object_lots',
				'ca_object_lot_labels',
				'ca_lists',
				'ca_list_items',
			...
			$qr_guid = $o_db->query( '
                SELECT access FROM ' . Datamodel::getTableName( $vn_table_num ) . " WHERE "
			                         . Datamodel::primaryKey( $vn_table_num ) . ' = ?
			...
			$qr_guid = $o_db->query( '
                SELECT access FROM ' . Datamodel::getTableName( $va_info['table_num'] ) . " WHERE "
			                         . Datamodel::primaryKey( $va_info['table_num'] ) . ' = ?
            ' . ( Datamodel::getFieldInfo( $va_info['table_num'], 'deleted' ) ? ' AND deleted = 0' : '' ),
				[ $va_info['row_id'] ] );
