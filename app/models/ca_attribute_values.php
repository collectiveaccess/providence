...
BaseModel::$s_ca_models_definitions['ca_attribute_values'] = array(
	...
	'FIELDS'        => array(
		'value_id'        => array(
			...
			'DESCRIPTION'    => 'Unique identifier for this attribute value'
		),
		'element_id'      => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Element id',
			'DESCRIPTION'    => 'Identifier for Element'
		),
		'attribute_id'    => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_FIELD,
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Attribute',
			'DESCRIPTION'    => 'Attribute value is part of'
		),
		'item_id'         => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_FIELD,
			...
			'DEFAULT'        => '',
			'LABEL'          => 'List',
			'DESCRIPTION'    => 'List item this value uses (only set for list attributes)'
		),
		'value_longtext1' => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => 'Longtext value container 1',
			'DESCRIPTION'    => 'First longtext attribute value container'
		),
		'value_longtext2' => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => 'Longtext value container 2',
			'DESCRIPTION'    => 'Second longtext attribute value container'
		),
		'value_blob'      => array(
			'FIELD_TYPE'               => FT_MEDIA,
			'DISPLAY_TYPE'             => DT_FIELD,
			...
			'LABEL'                    => 'BLOB value container',
			'DESCRIPTION'              => 'BLOB attribute value container'
		),
		'value_decimal1'  => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => 'Decimal value container 1',
			'DESCRIPTION'    => 'First decimal attribute value container'
		),
		'value_decimal2'  => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => 'Decimal value container 2',
			'DESCRIPTION'    => 'Second decimal attribute value container'
		),
		'value_integer1'  => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => 'Integer value container',
			'DESCRIPTION'    => 'Integer attribute value container'
		),
		'source_info'     => array(
			'FIELD_TYPE'     => FT_TEXT,
			'DISPLAY_TYPE'   => DT_FIELD,
...
class ca_attribute_values extends BaseModel {
	 ...
	 *                                skipExistingValues = attempt to detect and skip values already attached to the
	 *                                specified row to which the attribute is bound. [Default is false]
	 *
	 * @return int Returns the value_id of the newly created value. If the value cannot be added due to an error, false
	 *             is returned. "Silent" failures, for which the user should not see an error message, are indicated by
	 *             a null return value.
	 */
	public function addValue( $ps_value, $pa_element_info, $pn_attribute_id, $pa_options = null ) {
		...
		if ( is_array( $va_values ) ) {
			if ( ( caGetOption( 'skipExistingValues', $pa_options, false ) )
			     && ( $t_attr = caGetOption( 't_attribute', $pa_options, null ) )
			     && ( $t_instance = $t_attr->getRowInstance() )
			) {
				if ( is_array( $va_attrs = $t_instance->getAttributesByElement(
					...
					foreach ( $va_attrs as $o_attr ) {
						foreach ( $o_attr->getValues() as $o_val ) {
							...
							if ( strlen( $vs_old_value ) && strlen( $vs_new_value )
							     && ( $vs_old_value === $vs_new_value )
							) {
								return null;
								break;
							}
	 ...
	 * @param string $ps_value The user-input value to parse
	 *
	 * @return int Returns the value_id of the value on success. If the value cannot be edited due to an error, false
	 *             is returned. "Silent" failures, for which the user should not see an error message, are indicated by
	 *             a null return value.
	 */
	 ...
	 * The return value is an array key'ed on the multifile_id (a unique identifier for each attached file); array
	 * values are arrays with keys set to values for each file version returned. They keys are:
	 ...
	 *
	 * @param int   $pn_value_id  The value_id of the attribute value to return files for. If omitted the currently
	 *                            loaded attribute value is used. If no value_id is specified and no row is loaded null
	 *                            will be returned.
	 * @param int   $pn_start     The index of the first file to return. Files are numbered from zero. If omitted the
	 ...
	 *
	 * @return array A list of files attached to the attribute value. If no files are associated an empty array is
	 *               returned.
	 */
	...
	static public function getValueIDFor( $element_id, $value, $options = null ) {
		...
		if ( $element_id > 0 ) {
			...
			if ( is_array( $value ) ) {
				...
				$qr = $db->query( "
                    SELECT value_id FROM ca_attribute_values WHERE element_id = ? AND " . ( join( " AND ",
						$sql_wheres ) ) . " LIMIT 1
                ", $params );
			} else {
				$qr = $db->query( "
	 ...
	 *                       transaction = A transaction within which to perform the value search. [Default is null]
	 *
	 * @return array Dictionary with raw attribute value fields (value_longtext1, value_longtext2, value_blob,
	 *               value_decimal1, value_decimal2, value_integer1, item_id) or null if value_id is invalid.
	 */
