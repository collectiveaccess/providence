...
class ca_attribute_values extends BaseModel
{
    ...
    public function addValue($ps_value, $pa_element_info, $pn_attribute_id, $pa_options = null)
    {
        ...
        if (is_array($va_values)) {
            if ((caGetOption('skipExistingValues', $pa_options, false)) && ($t_attr = caGetOption(
                    't_attribute',
                    $pa_options,
                    null
                )) && ($t_instance = $t_attr->getRowInstance())) {
                if (is_array(
                    $va_attrs = $t_instance->getAttributesByElement($vn_attr_element_id = $t_attr->get('element_id'))
                )) {
