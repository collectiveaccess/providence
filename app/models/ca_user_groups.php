...
BaseModel::$s_ca_models_definitions['ca_user_groups'] = array(
	...
	'FIELDS'        => array(
		'group_id'       => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Group id' ),
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this group' )
		),
		'parent_id'      => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Parent id',
			'DESCRIPTION'    => 'Identifier for parent record'
		),
		'name'           => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Name' ),
			'DESCRIPTION'    => _t( 'Name of group. Should be unique.' ),
		...
		'code'           => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Code' ),
			'DESCRIPTION'    => _t( 'Short code identifying group. Can use used by users in <em>Pawtucket</em> to join this group.' ),
		...
		'description'    => array(
			...
			'DESCRIPTION'    => _t( 'Description of group. This text will be displayed to system administrators only and should clearly document the purpose of the group.' ),
		...
		'for_public_use' => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'For public use?' ),
			'DESCRIPTION'    => _t( 'If set, public users will be able to join this group using the group code.' )
		),
		'user_id'        => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Group administrator' ),
			'DESCRIPTION'    => _t( 'The user who administers the group.' )
		),
		'rank'           => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Sort order' ),
			'DESCRIPTION'    => _t( 'The relative priority of the group when displayed in a list with other relationship types. Lower numbers indicate higher priority.' ),
		...
		'vars'           => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Vars',
			'DESCRIPTION'    => 'Storage for group-level variables',
			'BOUNDS_LENGTH'  => array( 0, 65535 )
		),
		'hier_left'      => array(
			'FIELD_TYPE'     => FT_NUMBER,
...
class ca_user_groups extends BaseModel {
	 ...
	 * By default it will return all groups. If a user_id is specified in $pn_user_id then only groups created (and
	 * administered) by the user are counted.
	 *
	 * @param int $pn_user_id Optional user_id to restrict group list with. If set only groups owned by the specified
	 *                        user are counted
	 *
	...
	/**
	 * Returns list of available user groups, sorted by $ps_sort_field in ascending or descending order by
	 * $ps_sort_direction. By default it will return all groups. If a user_id is specified in $pn_user_id then only
	 * groups created (and administered) by the user are returned.
	 *
	 * @param string $ps_sort_field     Optional field to sort the group list on (ex. 'name'; 'code'; 'rank')
	 * @param string $ps_sort_direction Optional direction of sort, either 'asc' for ascending or 'desc' for
	 *                                  descending; default is ascending
	 * @param int    $pn_user_id        Optional user_id to restrict group list with. If set only groups owned by the
	 *                                  specified user are returned. If not set then only groups with no user_id set
	 *                                  are returned.
	 *
	...
	public function getGroupList( $ps_sort_field = '', $ps_sort_direction = 'asc', $pn_user_id = null ) {
		...
		while ( $qr_groups->nextRow() ) {
			...
			while ( $qr_members->nextRow() ) {
				...
				$va_member_list[]                            = $qr_members->get( 'fname' ) . ' '
				                                               . $qr_members->get( 'lname' );
			}
		...
		if ( $pn_group_id = $this->getPrimaryKey() ) {
			...
			foreach ( $pm_roles as $vs_role ) {
				...
				if ( $t_gxr->numErrors() == 0 ) {
					...
					$this->postError( 930,
						_t( "Database error adding role '%1': %2", $vs_role, join( ';', $t_gxr->getErrors() ) ),
						"ca_user_groups->addRoles()" );
				}
			}
	...
	/**
	 * Get list of all roles supported by the application. If you want to get the current group's roles, use
	 * getGroupRoles()
	 *
	 * @access public
	 ...
	 *
	 * @access public
	 * @return array Returns associative array of users. Key is user id, value is array containing information about
	 *               the user.
	 *
	 * The role information array contains the following keys:
	 *        user_id
	...
	public function roleListAsHTMLFormElement( $pa_options = null ) {
		...
		if ( sizeof( $va_roles ) ) {
			...
			foreach ( $va_roles as $vn_role_id => $va_role_info ) {
				$SELECTED = ( isset( $va_group_roles[ $vn_role_id ] ) && $va_group_roles[ $vn_role_id ] )
					? "SELECTED='1'" : "";
	 ...
	 * @access public
	 *
	 * @param mixed $pm_user_ids Single group or list (array) of user_ids to add to the current group. Users must be
	 *                           specified with user_ids
	 *
	 * @return integer Returns number of users added to the group or false if there was an error. The number of users
	 *                 added will not necessarily match the number of users you passed in $pm_user_ids. If you try to
	 *                 add the user to the same group twice, addUsers() will silently ignore it.
	 */
	 ...
	 * @access public
	 *
	 * @param mixed $pm_groups Single group or list (array) of user_ids to remove from current group. Users must be
