...
BaseModel::$s_ca_models_definitions['ca_user_groups'] = array(
    ...
    'FIELDS' => array(
        ...
        'code' => array(
            ...
            'LABEL' => _t('Code'),
            'DESCRIPTION' => _t(
                'Short code identifying group. Can use used by users in <em>Pawtucket</em> to join this group.'
            ),
        ...
        'description' => array(
            ...
            'DESCRIPTION' => _t(
                'Description of group. This text will be displayed to system administrators only and should clearly document the purpose of the group.'
            ),
        ...
        'rank' => array(
            ...
            'LABEL' => _t('Sort order'),
            'DESCRIPTION' => _t(
                'The relative priority of the group when displayed in a list with other relationship types. Lower numbers indicate higher priority.'
            ),
...
class ca_user_groups extends BaseModel
{
            ...
            foreach ($pm_roles as $vs_role) {
                ...
                if ($t_gxr->numErrors() == 0) {
                    ...
                    $this->postError(
                        930,
                        _t(
                            "Database error adding role '%1': %2",
                            $vs_role,
                            join(';', $t_gxr->getErrors())
                        ),
     ...
     * @return int Returns number of users added to the group or false if there was an error. The number of users added will not necessarily match the number of users you passed in $pm_user_ids. If you try to add the user to the same group twice, addUsers() will silently ignore it.
     */
