...
BaseModel::$s_ca_models_definitions['ca_search_forms'] = array(
    ...
    'FIELDS' => array(
        ...
        'form_code' => array(
            ...
            'LABEL' => _t('Form code'),
            'DESCRIPTION' => _t(
                'Unique code for form, used to identify the form for configuration purposes. You will need to specify this if you are using this form in a special context (on a web front-end, for example) in which the form must be unambiguously identified.'
            ),
        ...
        'is_system' => array(
            ...
            'DESCRIPTION' => _t(
                'Set this if the form is a form used by the system (as opposed to a user defined form). In general, system forms are defined by the system installer - you should not have to create system forms on your own.'
            ),
        ...
        'table_num' => array(
            ...
            'DESCRIPTION' => _t(
                'Determines what type of search (objects, entities, places, etc.) the form will conduct.'
            ),
...
class ca_search_forms extends BundlableLabelableBaseModelWithAttributes
{
    ...
    static public $s_have_access_to_form_cache = [];
    ...
    public function __construct($pn_id = null)
    {
        ...
        BaseModel::$s_ca_models_definitions['ca_search_forms']['FIELDS']['table_num']['BOUNDS_CHOICE_LIST'] = caFilterTableList(
    ...
    public function getForms($pa_options = null)
    {
        ...
        while ($qr_res->nextRow()) {
            ...
            $va_forms[$qr_res->get('form_id')][$qr_res->get('locale_id')] = array_merge(
                $qr_res->getRow(),
    ...
    public function getFormsAsHTMLSelect($ps_select_name, $pa_attributes = null, $pa_options = null)
    {
        ...
        if (
            ...
            ||
            (isset($pa_options['addDefaultFormIfEmpty']) && ($pa_options['addDefaultFormIfEmpty']) && (!sizeof(
                    $va_available_forms
                )))
        ) {
    ...
    public static function getFormTypeNames($pa_form_ids, $pa_options = null)
    {
        ...
        while ($qr_res->nextRow()) {
            ...
            $va_restriction_names = array_map(
                function ($v) {
            ...
            switch ($t_instance->tableName()) {
                case 'ca_occurrences':
                    ...
                    break;
                default:
                    $va_names[$qr_res->get('form_id')] = caUcFirstUTF8Safe(
                            $t_instance->getProperty(($vb_use_singular ? 'NAME_SINGULAR' : 'NAME_PLURAL'))
                        ) . ((sizeof($va_restriction_names) > 0) ? " (" . join(", ", $va_restriction_names) . ")" : '');
                    break;
            }
        }
    ...
    public function haveAccessToForm($pn_user_id, $pn_access, $pn_form_id = null)
    {
        ...
        if ((bool)$t_form->get(
                'is_system'
            ) && ($pn_access == __CA_SEARCH_FORM_READ_ACCESS__)) {    // system forms are readable by all
            return ca_search_forms::$s_have_access_to_form_cache[$vn_form_id . '/' . $pn_user_id . '/' . $pn_access] = true;
        }
    ...
    public function getAvailableBundles($pm_table_name_or_num = null, $pa_options = null)
    {
        ...
        $va_search_settings = $this->opo_search_indexing_config->getAssoc(
            Datamodel::getTableName($pm_table_name_or_num)
        );
        ...
        // Full-text
        $vs_bundle = "_fulltext";
        $vs_display = "<div id='searchFormEditor__fulltext'><span class='bundleDisplayEditorPlacementListItemTitle'>" . _t(
                "General"
            ) . '</span> ' . ($vs_label = _t('Full text')) . "</div>";
        ...
        if (is_array($va_search_settings)) {
            foreach ($va_search_settings as $vs_table => $va_fields) {
                ...
                if ($vs_table == $vs_primary_table) {
                    $va_element_codes = (method_exists(
                        $t_instance,
                        'getApplicableElementCodes'
                    ...
                    foreach ($va_field_list as $vs_field => $va_field_indexing_info) {
                        ...
                        if (!($va_field_info = $t_instance->getFieldInfo($vs_field))) {
                            ...
                            if (in_array($vs_field, $va_element_codes)) {
                                ...
                                $vs_display = "<div id='searchFormEditor_{$vs_table}_{$vs_field}'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                                        $t_instance->getProperty('NAME_SINGULAR')
                                    ) . "</span> " . $policy_label . ($vs_label = $t_instance->getDisplayLabel(
                                        $vs_bundle
                                    )) . "</div>";
                                $va_available_bundles[strip_tags($vs_display)][$vs_bundle] = array(
                                    'bundle' => $vs_bundle,
                    ...
                    foreach ($va_field_list as $vs_field => $va_field_indexing_info) {
                        ...
                        if (($va_field_info = $t_table->getFieldInfo($vs_field)) || (method_exists(
                                    $t_table,
                                    "hasElement"
                                ) && $t_table->hasElement($vs_field))) {
                            if (isset($va_field_info['DONT_USE_AS_BUNDLE']) && $va_field_info['DONT_USE_AS_BUNDLE']) {
                                continue;
                            }
                            ...
                            if ($policy) {
                                $vs_label = _t(
                                    'Current value for <em>%1</em> from <em>%2</em>',
                                    $vs_label,
        ...
        // access points
        //
        $va_access_points = (isset($va_search_settings['_access_points']) && is_array(
        ...
        foreach ($va_access_points as $vs_access_point => $va_access_point_info) {
            ...
            $vs_display = "<div id='searchFormEditor_{$vs_access_point}'><span class='bundleDisplayEditorPlacementListItemTitle'>" . _t(
                    'Access point'
                ) . '</span> ' . ($vs_label = ((isset($va_access_point_info['name']) && $va_access_point_info['name']) ? $va_access_point_info['name'] : $vs_access_point)) . "</div>";
        ...
        foreach (array('created', 'modified') as $vs_bundle) {
            $vs_display = "<div id='searchFormEditor_{$vs_bundle}'><span class='bundleDisplayEditorPlacementListItemTitle'>" . _t(
                    'General'
    ...
    public function getPlacementsInForm($pa_options = null)
    {
        ...
        foreach ($va_placements as $vn_placement_id => $va_placement) {
            ...
            $va_placement['display'] = $vs_display = preg_replace(
                "!<div id='searchFormEditor_[^']+!",
                "<div id='searchFormEditorSelected_{$vn_placement_id}",
                $vs_display
            );
    ...
    public function getHTMLFormElements($po_request, $pa_form_data = null)
    {
        ...
        foreach ($va_form_contents as $vn_i => $va_element) {
            ...
            switch ($va_element['bundle_name']) {
                ...
                case 'modified':
                    $va_output[] = array(
                        'element' => $t_subject->htmlFormElementForSearch(
                            $po_request,
                            $va_element['bundle_name'],
                            array(
                                'values' => $pa_form_data,
                                'width' => (isset($va_element['settings']['width']) && ($va_element['settings']['width'] > 0)) ? $va_element['settings']['width'] : "100px",
                                'height' => (isset($va_element['settings']['height']) && ($va_element['settings']['height'] > 0)) ? $va_element['settings']['height'] : 1,
                        ...
                        'label' => ($vs_field_label) ? $vs_field_label : $t_subject->getDisplayLabel(
                            $va_element['bundle_name']
                        ),
                        'name' => $va_element['bundle_name']
                    );
            ...
            if (!($t_instance = Datamodel::getInstanceByTableName($va_tmp[0], true))) {
                ...
                $va_search_settings = $this->opo_search_indexing_config->getAssoc(
                    Datamodel::getTableName($vs_form_table_name)
                );
                $va_access_points = (isset($va_search_settings['_access_points']) && is_array(
                        $va_search_settings['_access_points']
                    )) ? $va_search_settings['_access_points'] : [];
            ...
            $va_output[] = array(
                'element' => $t_instance->htmlFormElementForSearch(
                    $po_request,
                    $vs_field,
                    array(
                        'values' => $pa_form_data,
        ...
        if (is_array($va_values) && sizeof($va_values)) {
            foreach ($va_values as $vs_element => $va_values) {
                ...
                foreach ($va_values as $vs_value) {
                    ...
                    if (preg_match('![^A-Za-z0-9]+!', $vs_value) && !preg_match(
                            '![\*]+!',
                            $vs_value
                        ) && ($vs_value{0} != '[')) {
                            ...
                            if ($va_tmp[1] == 'current_value') { // history tracking current value search (format is <table>.current_value.<policy name>
                                ...
                                switch (ca_metadata_elements::getDataTypeForElementCode($vs_element_code)) {
                                    case __CA_ATTRIBUTE_VALUE_CURRENCY__:
                                        ...
                                        if (preg_match(
                                            "!^([A-Z\$£¥€]+[ ]*[\d\.]+)[ ]*([-–]{1}|to)[ ]*([A-Z\$£¥€]+[ ]*[\d\.]+)$!",
                                            trim(str_replace('"', '', $vs_query_element)),
                                            $m
                                        )) {
