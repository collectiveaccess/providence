...
BaseModel::$s_ca_models_definitions['ca_search_forms'] = array(
	...
	'FIELDS'        => array(
		'form_id'   => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this form' )
		),
		'user_id'   => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'User' ),
			'DESCRIPTION'    => _t( 'The user who created the form.' )
		),
		'form_code' => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Form code' ),
			'DESCRIPTION'    => _t( 'Unique code for form, used to identify the form for configuration purposes. You will need to specify this if you are using this form in a special context (on a web front-end, for example) in which the form must be unambiguously identified.' ),
		...
		'is_system' => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Is system form' ),
			'DESCRIPTION'    => _t( 'Set this if the form is a form used by the system (as opposed to a user defined form). In general, system forms are defined by the system installer - you should not have to create system forms on your own.' ),
		...
		'table_num' => array(
			...
			'DESCRIPTION'        => _t( 'Determines what type of search (objects, entities, places, etc.) the form will conduct.' ),
		...
		'settings'  => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Settings' ),
			'DESCRIPTION'    => _t( 'Search form settings.' )
		)
	)
);
...
class ca_search_forms extends BundlableLabelableBaseModelWithAttributes {
	...
	public function __construct( $pn_id = null ) {
		...
		BaseModel::$s_ca_models_definitions['ca_search_forms']['FIELDS']['table_num']['BOUNDS_CHOICE_LIST']
			= caFilterTableList( BaseModel::$s_ca_models_definitions['ca_search_forms']['FIELDS']['table_num']['BOUNDS_CHOICE_LIST'] );
	...
	protected function initLabelDefinitions( $pa_options = null ) {
		...
		$this->BUNDLES['ca_search_form_placements'] = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Search form contents' )
		);
		$this->BUNDLES['settings']                  = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Search form settings' )
		);

		$this->BUNDLES['ca_search_form_type_restrictions'] = array(
			'type'      => 'special',
			'repeating' => false,
	 ...
	 * @param int    $pn_rank        Optional value that determines sort order of bundles in the form. If omitted,
	 *                               placement is added to the end of the form.
	...
	public function addPlacement( $ps_bundle_name, $pa_settings, $pn_rank = null, $pa_options = null ) {
		...
		$t_placement = new ca_search_form_placements( null,
			is_array( $pa_options['additional_settings'] ) ? $pa_options['additional_settings'] : null );
	 ...
	 *                          form has no placements, or if there is no form loaded table = if using the
	 *                          returnAllAvailableIfEmpty option and you expect a list of available bundles to be
	 *                          returned if no form is loaded, you must specify the table the bundles are intended for
	 *                          use with with this option. Either the table name or number may be used. user_id = if
	 ...
	 *               following keys: placement_id = primary key of ca_search_form_placements row - a unique id for the
	 *               placement bundle_name = bundle name (a code - not for form) settings = array of placement
	 *               settings. Keys are setting names. form = form string for bundle
	 */
	public function getPlacements( $pa_options = null ) {
		$pb_no_cache                      = ( isset( $pa_options['noCache'] ) ) ? (bool) $pa_options['noCache'] : false;
		$pb_settings_only                 = ( isset( $pa_options['settingsOnly'] ) )
			? (bool) $pa_options['settingsOnly'] : false;
		$pb_return_all_available_if_empty = ( isset( $pa_options['returnAllAvailableIfEmpty'] ) && ! $pb_settings_only )
			? (bool) $pa_options['returnAllAvailableIfEmpty'] : false;
		...
		if ( ! $pb_no_cache && isset( ca_search_forms::$s_placement_list_cache[ $vn_form_id ] )
		     && ca_search_forms::$s_placement_list_cache[ $vn_form_id ]
		) {
			return ca_search_forms::$s_placement_list_cache[ $vn_form_id ];
		}
		...
		if ( $qr_res->numRows() > 0 ) {
			...
			while ( $qr_res->nextRow() ) {
				...
				$va_placements[ $vn_placement_id ]['settings']
				                                                                          =
				...
				if ( ! $pb_settings_only ) {
					...
					$va_placements[ $vn_placement_id ]['settingsForm']
					                                           = $t_placement->getHTMLSettingForm( array(
						'id'       => $vs_bundle_name . '_' . $vn_placement_id,
	 ...
	 *                          table = if set, list is restricted to forms that pertain to the specified table. You
	 *                          can pass a table name or number. If omitted forms for all tables will be returned.
	 *                          [Default is null] user_id = Restricts returned forms to those accessible by the current
	 *                          user. If omitted then all forms, regardless of access are returned. [Default is null]
	 *                          restrictToTypes = Restricts returned forms to those bound to the specified type.
	 *                          [Default is null] access = Restricts returned forms to those with at least the
	 *                          specified access level for the specified user. If user_id is omitted then this option
	 *                          has no effect. If user_id is set and this option is omitted, then forms where the user
	 *                          has at least read access will be returned. [Default is null] restrictToTypes = restrict
	 *                          forms to specific types; only relevant if table option is set. [Default is null]
	 *
	 * @return array Array of forms keyed on form_id and then locale_id. Keys for the per-locale value array include:
	 *               form_id,  form_code, user_id, table_num,  label_id, name (display name of form), locale_id (locale
	 *               of form name), search_form_content_type (display name of content this form searches on)
	 */
	public function getForms( $pa_options = null ) {
		...
		if ( $pm_table_name_or_num && is_array( $pa_restrict_to_types ) && sizeof( $pa_restrict_to_types )
			...
			$va_wheres[]
				         = "(sftr.type_id IS NULL OR sftr.type_id IN (?) OR (sftr.include_subtypes = 1 AND sftr.type_id IN (?)))";
		...
		while ( $qr_res->nextRow() ) {
			...
			$va_forms[ $qr_res->get( 'form_id' ) ][ $qr_res->get( 'locale_id' ) ] = array_merge( $qr_res->getRow(),
				array( 'search_form_content_type' => $vs_display_type ) );
		}
	 ...
	 *                          table - if set, list is restricted to forms that pertain to the specified table. You
	 *                          can pass a table name or number. If omitted forms for all tables will be returned.
	 *                          user_id - Restricts returned forms to those accessible by the current user. If omitted
	 *                          then all forms, regardless of access are returned. access - Restricts returned forms to
	 *                          those with at least the specified access level for the specified user. If user_id is
	 *                          omitted then this option has no effect. If user_id is set and this option is omitted,
	 *                          then forms where the user has at least read access will be returned.
	 *
	 ...
	 *                               Supports all options supported by caHTMLSelect() and ca_search_forms::getForms() +
	 *                               the following: addDefaultForm = if true, the "default" form is included at the
	 *                               head of the list; this is simply a form called "default" that is assumed to be
	 *                               handled by your code; the default is not to add the default value (false)
	 *                               addDefaultFormIfEmpty = same as 'addDefaultForm' except that the default value is
	 *                               only added if the form list is empty restrictToTypes =
	 *
	...
	public function getFormsAsHTMLSelect( $ps_select_name, $pa_attributes = null, $pa_options = null ) {
		...
		if (
			...
			|| ( isset( $pa_options['addDefaultFormIfEmpty'] ) && ( $pa_options['addDefaultFormIfEmpty'] )
			     && ( ! sizeof( $va_available_forms ) ) )
		) {
	...
	/**
	 * Returns names of types of content (synonymous with the table name for the content) for a list of form_ids. Will
	 * return name in singular number unless the 'number' option is set to 'plural'
	 *
		...
		while ( $qr_res->nextRow() ) {
			...
			switch ( $t_instance->tableName() ) {
				case 'ca_occurrences':
					...
					break;
				default:
					$va_names[ $qr_res->get( 'form_id' ) ]
						= caUcFirstUTF8Safe( $t_instance->getProperty( ( $vb_use_singular ? 'NAME_SINGULAR'
							: 'NAME_PLURAL' ) ) ) . ( ( sizeof( $va_restriction_names ) > 0 ) ? " (" . join( ", ",
								$va_restriction_names ) . ")" : '' );
					break;
			}
		}
	 ...
	 * @param int $pn_access  type of access required. Use __CA_SEARCH_FORM_READ_ACCESS__ for read-only access or
	 *                        __CA_SEARCH_FORM_EDIT_ACCESS__ for editing (full) access
	...
	public function haveAccessToForm( $pn_user_id, $pn_access, $pn_form_id = null ) {
		...
		if ( isset( ca_search_forms::$s_have_access_to_form_cache[ $vn_form_id . '/' . $pn_user_id . '/'
		                                                           . $pn_access ] )
		) {
		...
		if ( (bool) $t_form->get( 'is_system' )
		     && ( $pn_access == __CA_SEARCH_FORM_READ_ACCESS__ )
		) {    // system forms are readable by all
			return ca_search_forms::$s_have_access_to_form_cache[ $vn_form_id . '/' . $pn_user_id . '/' . $pn_access ]
				= true;
		}
	...
	# ------------------------------------------------------
	/**
	 * Returns all available search form placements - those data bundles that can be searches for the given content
	 ...
	 *                                    omitted the content table of the currently loaded search form will be used.
	 *
	...
	public function getAvailableBundles( $pm_table_name_or_num = null, $pa_options = null ) {
		...
		$va_search_settings
		            = $this->opo_search_indexing_config->getAssoc( Datamodel::getTableName( $pm_table_name_or_num ) );
		...
		$vs_bundle                                                       = "_fulltext";
		$vs_display
		                                                                 = "<div id='searchFormEditor__fulltext'><span class='bundleDisplayEditorPlacementListItemTitle'>"
		...
		if ( is_array( $va_search_settings ) ) {
			foreach ( $va_search_settings as $vs_table => $va_fields ) {
				...
				if ( $vs_table == $vs_primary_table ) {
					$va_element_codes = ( method_exists( $t_instance, 'getApplicableElementCodes' )
						? $t_instance->getApplicableElementCodes( null, false, false ) : [] );
					...
					foreach ( $va_field_list as $vs_field => $va_field_indexing_info ) {
						...
						if ( ! ( $va_field_info = $t_instance->getFieldInfo( $vs_field ) ) ) {
							...
							if ( in_array( $vs_field, $va_element_codes ) ) {
								...
								if ( caGetBundleAccessLevel( $vs_primary_table, $vs_field )
								...
								$vs_display
									                                                             = "<div id='searchFormEditor_{$vs_table}_{$vs_field}'><span class='bundleDisplayEditorPlacementListItemTitle'>"
									                                                               . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) )
									                                                               . "</span> "
									                                                               . $policy_label . (
									                                                               $vs_label
										                                                               = $t_instance->getDisplayLabel( $vs_bundle ) )
									                                                               . "</div>";
								$va_available_bundles[ strip_tags( $vs_display ) ][ $vs_bundle ] = array(
									'bundle'       => $vs_bundle,
									'label'        => $vs_label,
									...
									'description'  => $vs_description
										= $t_instance->getDisplayDescription( $vs_bundle ),
							...
							if ( isset( $va_field_info['DONT_USE_AS_BUNDLE'] )
							     && $va_field_info['DONT_USE_AS_BUNDLE']
							) {
								continue;
							}
					...
					if ( ( method_exists( $t_table, "getSubjectTableName" )
					       && $vs_subject_table = $t_table->getSubjectTableName() )
					) {
					...
					$va_element_codes = ( method_exists( $t_table, 'getApplicableElementCodes' )
						? $t_table->getApplicableElementCodes( null, false, false ) : [] );
					...
					foreach ( $va_field_list as $vs_field => $va_field_indexing_info ) {
						...
						if ( ( $va_field_info = $t_table->getFieldInfo( $vs_field ) )
						     || ( method_exists( $t_table, "hasElement" ) && $t_table->hasElement( $vs_field ) )
						) {
							if ( isset( $va_field_info['DONT_USE_AS_BUNDLE'] )
							     && $va_field_info['DONT_USE_AS_BUNDLE']
							) {
							...
							$vs_bundle      = $policy ? "{$vs_table}.current_value.{$p}.{$vs_field}"
								: "{$vs_table}.{$vs_field}";
							...
							if ( $policy ) {
								$vs_label = _t( 'Current value for <em>%1</em> from <em>%2</em>', $vs_label,
									ca_objects::getHistoryTrackingCurrentValuePolicy( $policy, 'name' ) );
							}

							if ( method_exists( $t_table, "getSubjectTableName" )
							     && ( $vs_primary_table == $vs_subject_table )
							) {
								$vs_display
									= "<div id='searchFormEditor_{$vs_table}_{$vs_field}'><span class='bundleDisplayEditorPlacementListItemTitle'>"
									  . caUcFirstUTF8Safe( $t_subject->getProperty( 'NAME_SINGULAR' ) )
									  . "</span> {$vs_label}</div>";
							} else {
								$vs_display
									= "<div id='searchFormEditor_{$vs_table}_{$vs_field}'><span class='bundleDisplayEditorPlacementListItemTitle'>{$vs_related_table}</span> {$vs_label}</div>";
							}
		...
		// access points
		//
		$va_access_points = ( isset( $va_search_settings['_access_points'] )
		                      && is_array( $va_search_settings['_access_points'] ) )
			? $va_search_settings['_access_points'] : [];
		...
		foreach ( $va_access_points as $vs_access_point => $va_access_point_info ) {
			...
			$vs_display
				                                                                   = "<div id='searchFormEditor_{$vs_access_point}'><span class='bundleDisplayEditorPlacementListItemTitle'>"
				                                                                     . _t( 'Access point' ) . '</span> '
				                                                                     . ( $vs_label
					= ( ( isset( $va_access_point_info['name'] )
					      && $va_access_point_info['name'] ) ? $va_access_point_info['name'] : $vs_access_point ) )
				                                                                     . "</div>";
			$va_available_bundles[ strip_tags( $vs_display ) ][ $vs_access_point ] = array(
				'bundle'       => $vs_access_point,
				'label'        => $vs_label,
				'display'      => $vs_display,
				'description'  => $vs_description = ( ( isset( $va_access_point_info['description'] )
				                                        && $va_access_point_info['description'] )
					? $va_access_point_info['description'] : '' ),
		...
		foreach ( array( 'created', 'modified' ) as $vs_bundle ) {
			$vs_display
				                                                             = "<div id='searchFormEditor_{$vs_bundle}'><span class='bundleDisplayEditorPlacementListItemTitle'>"
				                                                               . _t( 'General' ) . "</span> " . (
				                                                               $vs_label
					                                                               = $t_instance->getDisplayLabel( $vs_bundle ) )
				                                                               . "</div>";
	...
	public function getPlacementsInForm( $pa_options = null ) {
		...
		foreach ( $va_placements as $vn_placement_id => $va_placement ) {
			...
			$va_placement['display']
				= $vs_display = preg_replace( "!<div id='searchFormEditor_[^']+!",
				"<div id='searchFormEditorSelected_{$vn_placement_id}", $vs_display );
	...
	public function getHTMLFormElements( $po_request, $pa_form_data = null ) {
		...
		foreach ( $va_form_contents as $vn_i => $va_element ) {
			...
			if ( is_array( $va_element['settings']['label'] ) && ( sizeof( $va_element['settings']['label'] ) > 0 ) ) {
				if ( ( is_array(
					       $va_field_labels = caExtractValuesByUserLocale( array( $va_element['settings']['label'] ) ) )
				       && sizeof( $va_field_labels ) > 0 )
				) {
			...
			switch ( $va_element['bundle_name'] ) {
				case '_fulltext':
					...
					$va_output[] = array(
						'element' => caHTMLTextInput( "_fulltext", array(
							...
							array(
								'width'  => ( isset( $va_element['settings']['width'] )
								              && ( $va_element['settings']['width'] > 0 ) )
									? $va_element['settings']['width'] : "100px",
								'height' => ( isset( $va_element['settings']['height'] )
								              && ( $va_element['settings']['height'] > 0 ) )
									? $va_element['settings']['height'] : 1
							) ),
				...
				case 'modified':
					$va_output[] = array(
						'element' => $t_subject->htmlFormElementForSearch( $po_request, $va_element['bundle_name'],
							array(
								'values' => $pa_form_data,
								'width'  => ( isset( $va_element['settings']['width'] )
								              && ( $va_element['settings']['width'] > 0 ) )
						...
						'label'   => ( $vs_field_label ) ? $vs_field_label
							: $t_subject->getDisplayLabel( $va_element['bundle_name'] ),
						'name'    => $va_element['bundle_name']
					);
					continue( 2 );
			}
			...
			if ( ! ( $t_instance = Datamodel::getInstanceByTableName( $va_tmp[0], true ) ) ) {
				...
				$va_search_settings
					              = $this->opo_search_indexing_config->getAssoc( Datamodel::getTableName( $vs_form_table_name ) );
				$va_access_points = ( isset( $va_search_settings['_access_points'] )
				                      && is_array( $va_search_settings['_access_points'] ) )
					? $va_search_settings['_access_points'] : [];
			...
			$va_output[] = array(
				'element' => $t_instance->htmlFormElementForSearch( $po_request, $vs_field, array(
					'values' => $pa_form_data,
					'width'  => ( isset( $va_element['settings']['width'] )
					              && ( $va_element['settings']['width'] > 0 ) ) ? $va_element['settings']['width']
						: "100px",
					'height' => ( isset( $va_element['settings']['height'] )
					              && ( $va_element['settings']['height'] > 0 ) ) ? $va_element['settings']['height']
						: 1,
		...
		if ( is_array( $va_values ) && sizeof( $va_values ) ) {
			foreach ( $va_values as $vs_element => $va_values ) {
				...
				foreach ( $va_values as $vs_value ) {
					...
					if ( preg_match( '![^A-Za-z0-9]+!', $vs_value ) && ! preg_match( '![\*]+!', $vs_value )
					     && ( $vs_value{0} != '[' )
					) {
					...
					switch ( $vs_element ) {
						...
						default:
							...
							if ( $va_tmp[1]
							     == 'current_value'
							) { // history tracking current value search (format is <table>.current_value.<policy name>
								$va_query_elements[] = "({$vs_element}:{$vs_query_element})";
							} else {
								$t_element = ca_metadata_elements::getInstance(
									$vs_element_code = array_pop( $va_tmp ) );
								switch ( ca_metadata_elements::getDataTypeForElementCode( $vs_element_code ) ) {
									case __CA_ATTRIBUTE_VALUE_CURRENCY__:
										// convert bare ranges to Lucene range syntax
										if ( preg_match( "!^([A-Z\$£¥€]+[ ]*[\d\.]+)[ ]*([-–]{1}|to)[ ]*([A-Z\$£¥€]+[ ]*[\d\.]+)$!",
											trim( str_replace( '"', '', $vs_query_element ) ), $m )
										) {
									...
									case __CA_ATTRIBUTE_VALUE_INFORMATIONSERVICE__:
										...
										$va_data = $o_value->parseValue( $vs_query_element,
	...
	public function extractFormValuesFromArray( $pa_form_content ) {
		...
		foreach ( $va_form_contents as $vn_i => $vs_element ) {
			...
			if ( isset( $pa_form_content[ $vs_dotless_element ] ) ) { // && strlen($pa_form_content[$vs_dotless_element])) {
				$va_values[ $vs_element ] = strlen( ( $pa_form_content[ $vs_dotless_element ] ) )
					? $pa_form_content[ $vs_dotless_element ] : ' ';
			} else {
	...
	public function savePlacementsFromHTMLForm( $po_request, $ps_form_name, $ps_placement_code, $pa_options = null ) {
		if ( $vs_bundles = $po_request->getParameter( "{$ps_placement_code}{$ps_form_name}displayBundleList",
			pString )
		) {
			...
			foreach ( $va_placements as $vn_placement_id => $va_bundle_info ) {
				if ( ! in_array( $va_bundle_info['bundle_name'] . '_' . $va_bundle_info['placement_id'],
					...
					$t_form->removePlacement( $va_bundle_info['placement_id'],
			...
			foreach ( $va_bundles as $vn_i => $vs_bundle ) {
				...
				if ( $vn_placement_id === 0 ) {
					$t_form->addPlacement( $vs_bundle, $va_settings[ $vn_placement_id ], $vn_i + 1, array(
					...
					$t_placement = new ca_search_form_placements( $vn_placement_id,
					...
					if ( is_array( $va_settings[ $vn_placement_id ] ) ) {
						...
						foreach ( $t_placement->getAvailableSettings() as $vs_setting => $va_setting_info ) {
							$vs_val = isset( $va_settings[ $vn_placement_id ][ $vs_setting ] )
								? $va_settings[ $vn_placement_id ][ $vs_setting ] : null;
	 ...
	 *                           includeSubtypes = Automatically include subtypes for all set type restrictions.
	 *                           [Default is false]
	 *
	 * @return bool True on success, false on error, null if no screen is loaded
	 *
	...
	public function removeTypeRestriction( $pn_type_id = null ) {
		...
		if ( is_array(
			$va_uis = ca_search_form_type_restrictions::find( $va_params, [ 'returnAs' => 'modelInstances' ] ) )
		) {
	...
	public function getTypeRestrictionsHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pa_options = null
	) {
		...
		if ( $t_instance instanceof BaseRelationshipModel ) { // interstitial
			$o_view->setVar( 'type_restrictions',
				$t_instance->getRelationshipTypesAsHTMLSelect( $t_instance->getLeftTableName(), null, null,
					array( 'name' => 'type_restrictions[]', 'multiple' => 1, 'size' => 5 ),
					array( 'values' => $va_restriction_type_ids ) ) . $vs_subtype_element );
		} elseif ( $t_instance instanceof ca_representation_annotations ) { // config based
			$o_annotation_type_conf         = Configuration::load( Configuration::load()
			...
			$o_view->setVar( 'type_restrictions', caHTMLSelect( 'type_restrictions[]', $va_annotation_type_select_list,
					array( 'multiple' => 1, 'size' => 5 ), array( 'value' => 0, 'values' => $va_restriction_type_ids ) )
			                                      . $vs_subtype_element );
		} else { // list-based
			$o_view->setVar( 'type_restrictions', $t_instance->getTypeListAsHTMLFormElement( 'type_restrictions[]',
					array( 'multiple' => 1, 'height' => 5 ),
					array( 'value' => 0, 'values' => $va_restriction_type_ids ) ) . $vs_subtype_element );
		}
	...
	public function saveTypeRestrictionsFromHTMLForm( $po_request, $ps_form_prefix, $ps_placement_code ) {
		...
		return $this->setTypeRestrictions( $po_request->getParameter( 'type_restrictions', pArray ),
			[ 'includeSubtypes' => $po_request->getParameter( 'type_restriction_include_subtypes', pInteger ) ] );
	}
	# ----------------------------------------
}
