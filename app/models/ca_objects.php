...
BaseModel::$s_ca_models_definitions['ca_objects'] = array(
	...
	'FIELDS'        => array(
		...
		'source_id'                       => array(
			...
			'LABEL'                     => _t( 'Source' ),
			'DESCRIPTION'               => _t( 'Administrative source of object. This value is often used to indicate the administrative sub-division or legacy database from which the object originates, but can also be re-tasked for use as a simple classification tool if needed.' )
		),
		'type_id'                         => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_SELECT,
		...
		'idno'                            => array(
			...
			'DESCRIPTION'               => _t( 'A unique alphanumeric identifier for this object. This is usually equivalent to the "accession number" in museum settings.' ),
		...
		'home_location_id'                => array(
			...
			'DEFAULT'                   => null,
			'ALLOW_BUNDLE_ACCESS_CHECK' => true,
			'LABEL'                     => _t( 'Home location' ),
			'DESCRIPTION'               => _t( 'The customary storage location for this object.' )
		),
		...
		'deaccession_authorized_by'       => array(
			...
			'ALLOW_BUNDLE_ACCESS_CHECK' => true,
			'DONT_ALLOW_IN_UI'          => true,
			'LABEL'                     => _t( 'Authorized by' ),
			'DESCRIPTION'               => _t( 'Deaccession authorized by' ),
			'BOUNDS_LENGTH'             => array( 0, 255 )
		),
		...
		'item_status_id'                  => array(
			...
			'DESCRIPTION'    => _t( 'Indicates accession/collection status of object. (eg. accessioned, pending accession, loan, non-accessioned item, etc.)' )
		),
		...
		'extent'                          => array(
			...
			'ALLOW_BUNDLE_ACCESS_CHECK' => true,
			'LABEL'                     => _t( 'Extent' ),
			'DESCRIPTION'               => _t( 'The extent of the object. This is typically the number of discrete items that compose the object represented by this record. It is stored as a whole number (eg. 1, 2, 3...).' )
		),
		'extent_units'                    => array(
			'FIELD_TYPE'                => FT_TEXT,
			'DISPLAY_TYPE'              => DT_FIELD,
		...
		'acl_inherit_from_ca_collections' => array(
			...
			'DESCRIPTION'               => _t( 'Determines whether access settings set for related collections are applied to this object.' )
		),
		'acl_inherit_from_parent'         => array(
			...
			'DESCRIPTION'               => _t( 'Determines whether item-level access control settings set for parent object is applied to this object.' )
		),
		'access_inherit_from_parent'      => array(
			...
			'DESCRIPTION'               => _t( 'Determines whether front-end access settings set for parent object is applied to this object.' )
		),
		'view_count'                      => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_OMIT,
...
class ca_objects extends RepresentableBaseModel implements IBundleProvider {
		...
		if (
			...
			if ( ! is_null( BaseModel::$s_ca_models_definitions['ca_objects']['FIELDS']['acl_inherit_from_parent']['DEFAULT'] ) ) {
				BaseModel::$s_ca_models_definitions['ca_objects']['FIELDS']['acl_inherit_from_parent']['DEFAULT']
					= (int) $o_config->get( 'ca_objects_acl_inherit_from_parent_default' );
			}
			if ( ! is_null( BaseModel::$s_ca_models_definitions['ca_objects']['FIELDS']['acl_inherit_from_ca_collections']['DEFAULT'] ) ) {
				BaseModel::$s_ca_models_definitions['ca_objects']['FIELDS']['acl_inherit_from_ca_collections']['DEFAULT']
					= (int) $o_config->get( 'ca_objects_acl_inherit_from_ca_collections_default' );
			}
		}
	...
	protected function initLabelDefinitions( $pa_options = null ) {
		...
		$this->BUNDLES['history_tracking_chronology']       = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'History tracking – chronology' )
		);
		$this->BUNDLES['history_tracking_current_contents'] = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'History tracking – current contents' )
		);
		...
		$this->BUNDLES['submitted_by_user'] = array(
			'type'        => 'special',
			'repeating'   => false,
			'label'       => _t( 'Submitted by' ),
			'displayOnly' => true
		);
		$this->BUNDLES['submission_group']  = array(
			'type'        => 'special',
			'repeating'   => false,
			'label'       => _t( 'Submission group' ),
			'displayOnly' => true
		);

		$this->BUNDLES['home_location_value'] = array(
			'type'        => 'special',
			'repeating'   => false,
			'label'       => _t( 'Home location' ),
			'displayOnly' => true
	...
	public function insert( $pa_options = null ) {
		if ( $vn_parent_id = $this->get( 'parent_id' ) ) {
			if ( ! $this->getAppConfig()->get( [
				'ca_objects_dont_inherit_lot_from_parent',
				'ca_objects_dont_inherit_lot_id_from_parent'
			] )
			) {
	 ...
	 *                          assumeIdnoStubForLotID = set to force lookup of lot_id values as
	 *                          ca_object_lots.idno_stub values first not matter what, before consideration as a
	 *                          numeric lot_id. The default is false, in which case integer values are considered
	 *                          lot_ids and non-numeric values possible idno_stubs.
	 *
	 * @return int
	 */
	...
	public function delete( $pb_delete_related = false, $pa_options = null, $pa_fields = null, $pa_table_list = null ) {
		...
		if ( is_array( $va_representations ) ) {
			foreach ( $va_representations as $va_rep ) {
				...
				$qr_res = $this->getDb()
				               ->query( "SELECT count(*) c FROM ca_objects_x_object_representations WHERE object_id <> ? AND representation_id = ?",
					               (int) $this->getPrimaryKey(), (int) $va_rep["representation_id"] );
				if ( $qr_res->nextRow() && ( $qr_res->get( 'c' ) == 0 ) ) {
					$this->removeRepresentation( $va_rep["representation_id"],
						array( 'dontCheckPrimaryValue' => true ) );
				}
			}
		}
	 ...
	 *                          object_id = object to return components for, rather than currently loaded object.
	 *                          [Default = null; use loaded object]
	 *
	 * @return int
	 */
	public function getComponentCount( $pa_options = null ) {
		...
		if ( is_array( $va_component_types )
		     && ( sizeof( $va_component_types )
		          && ! in_array( '*', $va_component_types ) )
		) {
			$va_ids = ca_objects::find( array(
				'parent_id' => $pn_object_id,
				'type_id'   => $va_component_types,
				array( 'returnAs' => 'ids' )
			) );
		} else {
	 ...
	 * @param array $pa_options Array of options:
	 *                          object_id = object to return components for, rather than currently loaded object.
	 *                          [Default = null; use loaded object] returnAs = what to return; possible values are:
	 ...
	 *                                           instance in the array returned by 'modelInstances' info                    = an array, keyed on object_id with label, type and idno of each component
	 *
	 * @return int
	 */
	public function getComponents( $pa_options = null ) {
		...
		if ( is_array( $va_component_types )
			...
			$vm_res = ca_objects::find( array( 'parent_id' => $pn_object_id, 'type_id' => $va_component_types ), array(
				'sort'     => 'ca_objects.idno',
				'returnAs' => ( $vs_return_as == 'info' ) ? 'searchResult' : $vs_return_as
			) );
		} else {
			$vm_res = ca_objects::find( array( 'parent_id' => $pn_object_id ), array(
				'sort'     => 'ca_objects.idno',
				'returnAs' => ( $vs_return_as == 'info' ) ? 'searchResult' : $vs_return_as
			) );
		}
	...
	public function addRelationship(
		...
		return parent::addRelationship( $pm_rel_table_name_or_num, $pn_rel_id, $pm_type_id, $ps_effective_date,
	...
	public function editRelationship(
		...
		$ps_source_info = null, $ps_direction = null, $pn_rank = null, $pa_options = null
	) {
		return parent::editRelationship( $pm_rel_table_name_or_num, $pn_relation_id, $pn_rel_id, $pm_type_id,
			$ps_effective_date, $ps_source_info, $ps_direction, $pn_rank, $pa_options );
	}
	# ------------------------------------------------------
	 ...
	 * Only object types configured for checkout in the object_checkout.conf configuration file may be used with the
	 * checkout system
	 *
	 * @return bool
	 */
	...
	/**
	 * Return checkout status for currently loaded object. By default a numeric constant is returned. Possible values
	 * are:
	 ...
	 *
	 * If the returnAsText option is set then a text status value suitable for display is returned. For detailed
	 * information about the current status use the returnAsArray option, which returns an array with the following
	 * elements:
	 *
	 *        status = numeric status code
	 *        status_display = status display text
	 *        user_id = The user_id of the user who checked out the object (if status is
	 *        __CA_OBJECTS_CHECKOUT_STATUS_OUT__ or __CA_OBJECTS_CHECKOUT_STATUS_OUT_WITH_RESERVATIONS__) user_name =
	 *        Displayable user name and email checkout_date = Date of checkout (if status is
	 *        __CA_OBJECTS_CHECKOUT_STATUS_OUT__ or __CA_OBJECTS_CHECKOUT_STATUS_OUT_WITH_RESERVATIONS__) as printable
	 *        text
	 *
	 ...
	 *
	 * @return mixed Will return a numeric status code by default; status text for display if returnAsText option is
	 *               set; or an array with details on the checkout if the returnAsArray option is set. Will return null
	 *               if not object is loaded or if the object may not be checked out.
	 */
	public function getCheckoutStatus( $pa_options = null ) {
		...
		if ( $va_restrict_to_circ_statuses = $o_lib_conf->get( 'restrict_to_circulation_statuses' ) ) {
			if ( sizeof( $va_restrict_to_circ_statuses ) ) {
				if ( ! in_array( $this->get( 'circulation_status_id', [ 'convertCodesToIdno' => true ] ),
		...
		if ( $va_is_out ) {
			...
			$va_info['status']         = $vb_is_reserved ? __CA_OBJECTS_CHECKOUT_STATUS_OUT_WITH_RESERVATIONS__
				: __CA_OBJECTS_CHECKOUT_STATUS_OUT__;
			$va_info['status_display'] = $vb_is_reserved ? ( ( ( $vn_num_reservations == 1 )
				? _t( 'Out; %1 reservation', $vn_num_reservations )
			...
			$va_info['user_name']      = $t_checkout->get( 'ca_users.fname' ) . ' '
			                             . $t_checkout->get( 'ca_users.lname' ) . ( ( $vs_email
					= $t_checkout->get( 'ca_users.email' ) ) ? " ({$vs_email})" : '' );
		} elseif ( $vb_is_reserved ) {
			$va_info['status']         = __CA_OBJECTS_CHECKOUT_STATUS_RESERVED__;
			$va_info['status_display'] = ( $vn_num_reservations == 1 ) ? _t( 'Reserved' )
				: _t( '%1 reservations', $vn_num_reservations );
	...
	public function renderBundleForDisplay( $ps_bundle_name, $pn_row_id, $pa_values, $pa_options = null ) {
		switch ( $ps_bundle_name ) {
			case 'home_location_value':
				...
				if ( $q && $q->nextHit() ) {
					if ( ( $home_location_id = $q->get( 'ca_objects.home_location_id' ) )
					     && ( $t_loc
							= ca_storage_locations::find( [ 'location_id' => $home_location_id ],
							[ 'returnAs' => 'firstModelInstance' ] ) )
					) {
						if ( ! ( $t = Configuration::load()->get( 'home_location_display_template' ) ) ) {
							$t = caGetOption( 'display_template', $pa_options,
								'^ca_storage_locations.hierarchy.preferred_labels' );
						}
				...
				break;
			default:
				return self::renderHistoryTrackingBundleForDisplay( $ps_bundle_name, $pn_row_id, $pa_values,
					$pa_options );
				break;
		}
	}
