...
BaseModel::$s_ca_models_definitions['ca_lists'] = array(
	...
	'FIELDS'        => array(
		'list_id'           => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this list' )
		),
		'list_code'         => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'List code' ),
			'DESCRIPTION'    => _t( 'Unique code for list; used to identify the list for configuration purposes.' ),
		...
		'default_sort'      => array(
			...
			'LABEL'              => _t( 'Default sort order' ),
			'DESCRIPTION'        => _t( 'Specifies the default method to employ to order items in this list.' ),
		...
		'is_system_list'    => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Is system list' ),
			'DESCRIPTION'    => _t( 'Set this if the list is a list used by the system to populate a specific field (as opposed to a user defined list or vocabulary). In general, system lists are defined by the system installer - you should not have to create system lists on your own.' ),
		...
		'is_hierarchical'   => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Is hierarchical' ),
			'DESCRIPTION'    => _t( 'Set this if the list is hierarchically structured; leave unset if you are creating a simple "flat" list.' ),
			'BOUNDS_VALUE'   => array( 0, 1 )
		),
		'use_as_vocabulary' => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Use as vocabulary' ),
			'DESCRIPTION'    => _t( 'Set this if the list is to be used as a controlled vocabulary for cataloguing.' ),
			'BOUNDS_VALUE'   => array( 0, 1 )
		),
		'deleted'           => array(
			'FIELD_TYPE'     => FT_BIT,
...
class ca_lists extends BundlableLabelableBaseModelWithAttributes {
	...
	public function addItem(
		$ps_value, $pb_is_enabled = true, $pb_is_default = false, $pn_parent_id = null, $pn_type_id = null,
		$ps_idno = null, $ps_validation_format = '', $pn_status = 0, $pn_access = 0, $pn_rank = null, $ps_color = null
	) {
	...
	public function editItem(
		$pn_item_id, $ps_value, $pb_is_enabled = true, $pb_is_default = false, $pn_parent_id = null, $ps_idno = null,
		$ps_validation_format = '', $pn_status = 0, $pn_access = 0, $pn_rank = null, $ps_color = null
	) {
		if ( ! ( $vn_list_id = $this->getPrimaryKey() ) ) {
			return false;
		}
	 ...
	 *
	 */
	public function addLabelToItem(
		$pn_item_id, $pa_label_values, $pn_locale_id, $pn_type_id = null, $pb_is_preferred = false, $pn_status = 0,
		$ps_description = ''
	) {
		die( "Not implemented" );
	}
	...
	/**
	 * Returns items contained in list, including their labels. This method will returned the list items sorted
	 * according to the default_sort field setting of the list they belong to. Note that correct order when sorting by
	 * label is only guaranteed if 'extractValuesByUserLocale' is set to true [default is false]. This is due to the
	 * list return format: since each item is indexed by item_id first, it can only have a single position in the
	 * return structure. If multiple labels are returned for an item then the item will only be in the correct sort
	 * order for one of the labels in most cases. To ensure proper sort order by label text, labels must be restricted
	 * to a single locale.
	 *
	 ...
	 *                            returnHierarchyLevels =        if true list is returned with 'LEVEL' field set to
	 *                            hierarchical level of item, and items are returned in order such that if you loop
	 *                            through the returned list and indent each item according to its level you get a
	 *                            nicely formatted hierarchical display. Default is false. extractValuesByUserLocale =
	 *                            if true then values are processed to be appropriate for current user locale; default
	 *                            is false:  return values for all locales directChildrenOnly =        if true, only
	 *                            children immediately below the specified item are returned; [default is false]
	 *                            includeSelf =    if true, the specified item is included in the returned set of
	 *                            items; [default is false] type_id =        optional list item type to limit returned
	 *                            items by; default is to not limit by type (eg. type_id = null) item_id =
	 *                            optional item_id to use as root of hierarchy for returned items; if this is not set (the default) then all items in the list are returned sort =            if set to a __CA_LISTS_SORT_BY_*__ constant, will force the list to be sorted by that criteria overriding the sort order set in the ca_lists.default_sort field idsOnly =        if true, only the primary key id values of the list items are returned enabledOnly =    return only enabled list items [default=false] omitRoot =        don't include root node [Default=false] labelsOnly =    if true only labels in the current locale are returns in an array key'ed on item_id start =        offset to start returning records from [default=0; no offset] limit =        maximum number of records to return [default=null; no limit] dontCache =        don't cache checkAccess =   Array of access values to filter returned values on. If omitted no filtering is performed. [Default is null]
	 *
	...
	public function getItemsForList( $pm_list_name_or_id, $pa_options = null ) {
		...
		if ( $t_list_item->getPrimaryKey() ) {
			$vs_hier_sql = " AND ((cli.hier_left >= " . ( floatval( $t_list_item->get( 'hier_left' ) ) )
			               . ") AND (cli.hier_right <= " . ( floatval( $t_list_item->get( 'hier_right' ) ) ) . "))";
		}
		...
		if ( ! isset( $pa_options['returnHierarchyLevels'] ) || ! $pa_options['returnHierarchyLevels'] ) {
			...
			while ( $qr_res->nextRow() ) {
				...
				if ( ( ! isset( $pa_options['includeSelf'] ) || ! $pa_options['includeSelf'] )
				     && ( $vn_item_id == $pn_item_id )
				) {
					continue;
				}
				if ( ( isset( $pa_options['directChildrenOnly'] ) && $pa_options['directChildrenOnly'] )
				     && ( $qr_res->get( 'parent_id' ) != $pn_item_id )
				) {
					continue;
				}
			...
			foreach ( $va_list_items as $vn_i => $va_item ) {
				...
				if ( is_array( $pa_check_access ) && ( sizeof( $pa_check_access ) > 0 )
				     && in_array( $va_item['access'], $pa_check_access )
				) {
					continue;
				}
				...
				if ( isset( $pa_options['extractValuesByUserLocale'] ) && $pa_options['extractValuesByUserLocale'] ) {
					$va_items[ $vn_parent_id ][ $vn_item_id ][ $va_item['NODE']['locale_id'] ][ $vs_key ][ $vn_item_id ]
						= array_merge( $va_item['NODE'], array( 'LEVEL' => $va_item['LEVEL'] ) );
				} else {
					$va_items[ $vn_parent_id ][ $va_item['NODE']['locale_id'] ][ $vs_key ][ $vn_item_id ]
						= array_merge( $va_item['NODE'], array( 'LEVEL' => $va_item['LEVEL'] ) );
				}
			}
			...
			if ( is_array( $va_items )
			     && ( isset( $pa_options['extractValuesByUserLocale'] )
			          && $pa_options['extractValuesByUserLocale'] )
			) {
				//$va_items = caExtractValuesByUserLocale($va_items);
				$va_proc_items = array();
			...
			foreach ( $va_items as $vn_locale_id => $va_items_by_locale_id ) {
				...
				foreach ( $va_items_by_locale_id as $vs_key => $va_items_by_item_id ) {
					foreach ( $va_items_by_item_id as $vn_item_id => $va_item_level ) {
						...
						if ( isset( $pa_items[ $vn_item_id ] ) ) {    // are there children?
							$this->_getItemsForListProcListLevel( $vn_item_id, $pa_items, $pa_sorted_items,
								$pa_options );
						}
					}
				}
	 ...
	 *        directChildrenOnly - if true, only children immediately below the specified item are returned; [default
	 *        is false] checkAccess =   Array of access values to filter returned values on. If omitted no filtering is
	 *        performed. [Default is null]
	 *
	 * @return array
	 */
	public function getChildItemsForList( $pm_list_name_or_id, $pn_item_id, $pa_options = null ) {
		...
		$va_params     = [
			(int) $vn_list_id,
			floatval( $t_item->get( 'hier_left' ) ),
			floatval( $t_item->get( 'hier_right' ) )
		];
		$vs_access_sql = '';
		if ( is_array( $pa_check_access ) && ( sizeof( $pa_check_access ) > 0 ) ) {
	...
	/**
	 * Returns number of items in specified list, not including the list root. If optional $pn_type_id parameter is
	 * passed then the count is for items with the specified type in the specified list.
	 *
	 ...
	 *                                   checkAccess =   Array of access values to filter returned values on. If
	 *                                   omitted no filtering is performed. [Default is null]
	 *
	...
	public function getItemFromList( $pm_list_name_or_id, $ps_idno, $pa_options = null ) {
		...
		if ( $qr_res->nextRow() ) {
			return ca_lists::$s_list_item_get_cache[ $vs_alt_key ]
				= ca_lists::$s_list_item_get_cache[ $vs_cache_key ] = $qr_res->getRow();
		}

		return ca_lists::$s_list_item_get_cache[ $vs_alt_key ]
			= ca_lists::$s_list_item_get_cache[ $vs_cache_key ] = null;
	}
	# ------------------------------------------------------
	 ...
	 * @params array $pa_options Options include:
	 *                                  checkAccess =   Array of access values to filter returned values on. If omitted
	 *                                  no filtering is performed. [Default is null]
	 *
	 * @return array
	 */
	...
	/**
	 * Returns data for list item(s) with a given item value. Item value is a text or numeric value associated with a
	 * list item. Unlike idno, it need not be unique.
	 *
	 * @param mixed $pm_list_name_or_id The list code or list_id for the list to search
	...
	public function getItemFromListByLabel( $pm_list_name_or_id, $ps_label_name, $pa_options = null ) {
		...
		if ( $qr_res->nextRow() ) {
			return ca_lists::$s_list_item_get_cache[ $vs_cache_key ]
				= ca_lists::$s_list_item_get_cache[ $vs_alt_key ] = $qr_res->getRow();
		}

		return ca_lists::$s_list_item_get_cache[ $vs_cache_key ]
			= ca_lists::$s_list_item_get_cache[ $vs_alt_key ] = null;
	 ...
	 * @param mixed  $pm_list_name_or_id The list code or list_id for the list to search
	 * @param string $ps_idno            The list item identifer
	 * @param mixed  $pa_options         If set to a boolean, controls whether the singular (false) or plural (true)
	 *                                   sense are returned. This allows an older calling signature, where the second
	 *                                   parameter was a boolean to be preserved. If set to an options array, available
	 *                                   options include: checkAccess = Array of access values to filter returned
	 *                                   values on. If omitted no filtering is performed. [Default is null] return =
	 *                                   Set to "singular" or "plural" to return singular or plural sense. [Default is
	 *                                   "singular"]
	 *
	 * @return string
	 */
	public function getItemFromListForDisplay( $pm_list_name_or_id, $ps_idno, $pa_options = null ) {
		$pb_return_plural = ! is_array( $pa_options ) ? (bool) $pa_options
			: ( caGetOption( 'return', $pa_options, 'singular' ) == 'plural' );
		$pa_check_access  = caGetOption( 'checkAccess', $pa_options, null );
		$vs_cache_key     = caMakeCacheKeyFromOptions( $pa_options, "{$pm_list_name_or_id}/{$ps_idno}" );
	 ...
	 * @param mixed $pa_options     If set to a boolean, controls whether the singular (false) or plural (true) sense
	 *                              are returned. This allows an older calling signature, where the second parameter
	 *                              was a boolean to be preserved. If set to an options array, available options
	 *                              include: checkAccess = Array of access values to filter returned values on. If
	 *                              omitted no filtering is performed. [Default is null] return = Set to "singular" or
	 *                              "plural" to return singular or plural sense. [Default is "singular"]
	 *
	 * @return string
	 */
	 ...
	 *                                  sense are returned. This allows an older calling signature, where the second
	 *                                  parameter was a boolean to be preserved. If set to an options array, available
	 *                                  options include: checkAccess = Array of access values to filter returned values
	 *                                  on. If omitted no filtering is performed. [Default is null] return = Set to
	 *                                  "singular" or "plural" to return singular or plural sense. [Default is
	 *                                  "singular"]
	 *
	 * @return string
	 */
	...
	/**
	 * Returns the item_id for first list item found in a list with a given item value. Item value is a text or numeric
	 * value associated with a list item. Unlike idno, it need not be unique.
	 *
	 * @param mixed $pm_list_name_or_id The list code or list_id for the list to search
	 * @param mixed $pm_value           The item value
	 * @param mixed $pa_options         Options include:
	...
	public function getItemIDFromListByLabel( $pm_list_name_or_id, $ps_label_name, $pa_options = null ) {
		if ( $vn_list_id = $this->_getListID( $pm_list_name_or_id ) ) {
			if ( $vn_id = ca_list_items::find( array(
				'list_id'          => $vn_list_id,
				'preferred_labels' => array( 'name_plural' => $ps_label_name )
			), array( 'returnAs' => 'firstId', 'checkAccess' => caGetOption( 'checkAccess', $pa_options, null ) ) )
			) {
				return $vn_id;
			} elseif ( $vn_id = ca_list_items::find( array(
				'list_id'          => $vn_list_id,
				'preferred_labels' => array( 'name_singular' => $ps_label_name )
			), array( 'returnAs' => 'firstId', 'checkAccess' => caGetOption( 'checkAccess', $pa_options, null ) ) )
			) {
				return $vn_id;
			}
	 ...
	 *
	 * @param mixed $pm_list_name_or_id The list code or list_id for the list. If omitted the list for the currently
	 *                                  loaded item is used.
	 *
	 ...
	 *
	 * @param mixed $pm_list_name_or_id List code or list_id of list to return default item_id for. If omitted the
	 *                                  currently loaded list will be used.
	 * @param array $pa_options         Options include options for @see ca_list_items::getItemsForList() as well as:
	 *                                  useFirstElementAsDefaultDefault = return first item in list if not explicit
	 *                                  default is set for the list. [Default is false]
	 *
	 * @return int The item_id of the default element or null if no list was specified or loaded. If no default is set
	 *             for the list in question null is returned
	 */
	public function getDefaultItemID( $pm_list_name_or_id = null, $pa_options = null ) {
		...
		return caGetOption( 'useFirstElementAsDefaultDefault', $pa_options, false )
			? array_shift( $this->getItemsForList( $vn_list_id,
				array_merge( $pa_options, array( 'idsOnly' => true ) ) ) ) : null;
	}
	# ------------------------------------------------------
	 ...
	 *                                  transaction = Transaction to perform database operations within. [Default is
	 *                                  null]
	 *
	 ...
	 *                           childrenOnlyForItemID = if set only items below item_id in the list item hierarchy are
	 *                           returned. Default (null) is to return all items in the list. directChildrenOnly = if
	 *                           set only items with item_id=childrenOnlyForItemID as parent in the list item hierarchy
	 *                           are returned. Default (null) is to return all items in the list. nullOption = if set
	 *                           then a "null" (no value) option is available labeled with the value passed in this
	 *                           option additionalOptions = an optional array of options that will be passed through to
	 *                           caHTMLSelect; keys are display labels and values are used as option values value = if
	 *                           set, the <select> will have default selection set to the item whose *value* matches
	 *                           the option value. If none is set then the first item in the list will be selected key
	 *                           = ca_list_item field to be used as value for the <select> element list; can be set to
	 *                           either item_id or item_value; default is item_id width = the display width of the list in characters or pixels limitToItemsWithID = An optional array of list item_ids. Item_ids not in the array will be omitted from the returned list. omitItemsWithID = An optional array of list item_ids. Item_ids in the array will be omitted from the returned list. disableItemsWithID = An optional array of list item_ids. Item_ids in the array will be disabled in the returned list. maxItemCount = Don't return an element if the list has more elements than limit. [Default is null – no limit]
	 *
	 *    limitToItemsRelatedToCollections = an array of collection_ids or collection idno's; returned items will be
	 *    restricted to those attached to the specified collections
	 *    limitToItemsRelatedToCollectionWithRelationshipTypes = array of collection type names or type_ids; returned
	 *    items will be restricted to those attached to the specified collectionss with the specified relationship type
	 *    limitToListIDs = array of list_ids to restrict returned items to when using
	 *    "limitToItemsRelatedToCollections"
	 *
	 ...
	 *
	 *  useDefaultWhenNull = if a list has a null value the default value is typically ignored and null used as the
	 *  initial value; set this option to use the default in all cases [Default=false] checkAccess = Array of access
	 *  values to filter returned values on. If omitted no filtering is performed. [Default is null] exclude = array of
	 *  item idnos to omit from the returned list. [Default is null]
	 *
	 ...
	 */
	static public function getListAsHTMLFormElement(
		$pm_list_name_or_id, $ps_name, $pa_attributes = null, $pa_options = null
	) {
		$t_list = new ca_lists();
		...
		if ( ( $vn_limit = caGetOption( 'maxItemCount', $pa_options, null ) )
		     && ( $t_list->numItemsInList( $pm_list_name_or_id, null, $pa_options ) > $vn_limit )
		) {
			return null;
		}

		if ( ! ( isset( $pa_options['limitToItemsRelatedToCollection'] )
		         && is_array( $pa_options['limitToItemsRelatedToCollections'] ) )
		) {
			$vn_list_id = $t_list->_getListID( $pm_list_name_or_id );
			$t_list->load( $vn_list_id );
		}
		$vn_root_id = ( isset( $pa_options['childrenOnlyForItemID'] ) && $pa_options['childrenOnlyForItemID'] )
			? $pa_options['childrenOnlyForItemID'] : null;
		...
		$vb_is_vertical_hier_browser = in_array( $vs_render_as,
			[ 'vert_hierbrowser', 'vert_hierbrowser_up', 'vert_hierbrowser_down' ] );

		$vn_sort_type = $t_list->get( 'default_sort' );
		if ( ( $vs_render_as == 'yes_no_checkboxes' ) && ( $vn_sort_type == __CA_LISTS_SORT_BY_LABEL__ ) ) {
			$vn_sort_type
				= __CA_LISTS_SORT_BY_IDENTIFIER__;    // never allow sort-by-label when rendering as yes/no checkbox
		}

		if ( ! in_array( $vs_render_as,
			array( 'lookup', 'horiz_hierbrowser', 'vert_hierbrowser', 'vert_hierbrowser_up', 'vert_hierbrowser_down' ) )
		) {
			if ( isset( $pa_options['limitToItemsRelatedToCollections'] )
			     && is_array( $pa_options['limitToItemsRelatedToCollections'] )
			) {
				...
				if ( sizeof( $va_collection_ids ) ) {
					$qr_collections = $t_list->makeSearchResult( 'ca_collections', $va_collection_ids, array(
						'restrictToRelationshipTypes' => isset( $pa_options['limitToItemsRelatedToCollectionWithRelationshipTypes'] )
							? $pa_options['limitToItemsRelatedToCollectionWithRelationshipTypes'] : null
					) );

					$va_item_ids = array();
					while ( $qr_collections->nextHit() ) {
						$va_list_items = $qr_collections->get( 'ca_list_items', array(
							'returnAsArray'       => true,
					...
					if ( $va_limit_to_listIDs = ( ( isset( $pa_options['limitToListIDs'] )
					                                && is_array( $pa_options['limitToListIDs'] ) )
						? $pa_options['limitToListIDs'] : null )
					) {
					...
					if ( is_array( $va_limit_to_listIDs ) && sizeof( $va_item_ids ) ) {
						...
						while ( $qr_list_items->nextHit() ) {
							if ( ! in_array( $qr_list_items->get( 'ca_list_items.list_id' ), $va_limit_to_listIDs ) ) {
								if ( is_array(
									$va_k = array_keys( $va_item_ids, $qr_list_items->get( 'ca_list_items.item_id' ) ) )
								) {
				...
				$va_list_items = $t_list->getItemsForList( $pm_list_name_or_id, array_merge( $pa_options, array(
					'returnHierarchyLevels'     => caGetOption( 'indentForHierarchy', $pa_options, true ),
					'item_id'                   => $vn_root_id,
					'extractValuesByUserLocale' => true,
					'sort'                      => $vn_sort_type
				) ) );
			}
		}
		...
		if ( ! isset( $pa_options['limitToItemsWithID'] ) || ! is_array( $pa_options['limitToItemsWithID'] )
		     || ! sizeof( $pa_options['limitToItemsWithID'] )
		) {
			$pa_options['limitToItemsWithID'] = null;
		}
		if ( ! isset( $pa_options['omitItemsWithID'] ) || ! is_array( $pa_options['omitItemsWithID'] )
		     || ! sizeof( $pa_options['omitItemsWithID'] )
		) {
		...
		if ( ( isset( $pa_options['nullOption'] ) && $pa_options['nullOption'] )
		     && ( ( $vs_render_as !== 'checklist' )
		          && ! $pa_options['requireValue'] )
		) {
		...
		if ( is_array( $pa_options['limitToItemsWithID'] ) && sizeof( $pa_options['limitToItemsWithID'] ) ) {
			...
			foreach ( $va_list_items as $vn_item_id => $va_item ) {
				if ( ( $vn_parent_id = $va_item['parent_id'] )
				     && in_array( $vn_item_id, $pa_options['limitToItemsWithID'] )
				) {
		...
		foreach ( $va_list_items as $vn_item_id => $va_item ) {
			...
			if ( is_array( $pa_options['omitItemsWithID'] )
			     && in_array( $vn_item_id, $pa_options['omitItemsWithID'] )
			) {
				continue;
			}
			...
			if ( is_array( $pa_exclude_items ) && ( sizeof( $pa_exclude_items ) > 0 )
			     && in_array( $va_item['idno'], $pa_exclude_items )
			) {
				continue;
			}

			$va_options[ $va_item[ $pa_options['key'] ] ] = str_repeat( '&nbsp;', intval( $va_item['LEVEL'] ) * 3 )
			                                                . ' ' . $va_item['name_singular'];
			if ( ! caGetOption( 'forceEnabled', $pa_options, false )
			     && ( ! $va_item['is_enabled']
			          || ( is_array( $va_disabled_item_ids )
			               && in_array( $vn_item_id, $va_disabled_item_ids ) ) )
			) {
			...
			if ( $va_item['is_default']
			     && ( ! isset( $pa_options['nullOption'] )
			          || ( isset( $pa_options['useDefaultWhenNull'] )
			               && (bool) $pa_options['useDefaultWhenNull'] ) )
			) {        // set default if needed, but only if there's not a null option set
				if (
				( ! is_array( $pa_options['value'] )
				  && ( ! isset( $pa_options['value'] )
				       || ! strlen( $pa_options['value'] ) ) )
				) {
		...
		if (
			...
			if ( isset( $omit_map[ $omit_table ] ) && ( $omit_map = $omit_map[ $omit_table ] ) ) {
				...
				if ( is_array( $omit_map ) ) {
					$v             = array_shift( caExtractValuesByUserLocale( caGetListItemForValue( 'access_statuses',
						$pa_options['value'] ) ) );
					$value_idno    = $v['idno'];
					...
					foreach ( $omit_map as $i => $value ) {
						...
						if ( ! is_numeric( $value ) ) {
							$value = caGetListItemValueForIdno( 'access_statuses',
								$value );    // convert status identifiers to integer code values
							if ( ! strlen( trim( $value ) ) ) {
								continue;
							}
						}
		...
		switch ( $vs_render_as ) {
			...
			case 'checklist':
				...
				$vs_buf = ( $max_columns > 1 ) ? "<div class='checklist' style='grid-template-columns: "
				                                 . str_repeat( " {$p}%", $max_columns ) . ";'>\n" : "<div>\n";
				...
				foreach ( $va_options as $vm_value => $vs_label ) {
					...
					$vs_buf .= "<div class='checklistItem'>" . caHTMLCheckboxInput( $ps_name . '_' . $vm_value,
							$va_attributes, $pa_options ) . " " . str_replace( '&nbsp;', '', $vs_label ) . "</div>\n";
				}
				$vs_buf .= "</div>";
			...
			case 'lookup':
				...
				if ( caGetOption( 'forSearch', $pa_options ) ) {
					if ( $vs_val_id = caGetOption( 'value', $pa_options ) ) {
						$vs_value        = $t_list->getItemFromListForDisplayByItemID( $pm_list_name_or_id,
							$vs_val_id );
				...
				if ( $pa_options['request'] ) {
					$vs_url = caNavUrl( $pa_options['request'], 'lookup', 'ListItem', 'Get', array(
						'list'      => ca_lists::getListCode( $vn_list_id ),
						'noInline'  => 1,
						'noSymbols' => 1,
						'max'       => 100
					) );
			...
			case 'vert_hierbrowser_down':
				$va_width = caParseFormElementDimension( $pa_options['width'] ? $pa_options['width']
					: $pa_options['width'] );
				...
				$vs_buf = "<div style='width: {$vn_width}; " . ( $vn_height ? "height: {$vn_height}" : "" )
				          . ";'><div id='{$ps_name}_hierarchyBrowser{n}' style='width: 100%; height: 100%;' class='"
				          . ( $vb_is_vertical_hier_browser ? 'hierarchyBrowserVertical' : 'hierarchyBrowser' ) . "'>
					<!-- Content for hierarchy browser is dynamically inserted here by ca.hierbrowser -->
				</div><!-- end hierarchyBrowser -->	</div>";

				if ( ! ( $current_selection_display_format = caGetOption( 'currentSelectionDisplayFormat', $pa_options,
					null ) )
				) {
					$current_selection_display_format
						= '<ifdef code="hierarchy">^hierarchy%delimiter=_➜_ ➜ </ifdef>^current';
				}
	...
	<script type='text/javascript'>
		jQuery(document).ready(function() { 
			var oHierBrowser = caUI.initHierBrowser('{$ps_name}_hierarchyBrowser{n}', {
				...
				levelDataUrl: '" . caNavUrl( $pa_options['request'], 'lookup', 'ListItem', 'GetHierarchyLevel',
						array( 'noSymbols' => 1 ) ) . "',
				...
				className: '" . ( $vb_is_vertical_hier_browser ? 'hierarchyBrowserLevelVertical'
						: 'hierarchyBrowserLevel' ) . "',
				classNameContainer: '" . ( $vb_is_vertical_hier_browser ? 'hierarchyBrowserContainerVertical'
...
";
				...
				if ( $vs_render_as == 'horiz_hierbrowser_with_search' ) {
					...
					{
						source: '" . caNavUrl( $pa_options['request'], 'lookup', 'ListItem', 'Get',
							array( 'list' => ca_lists::getListCode( $vn_list_id ), 'noSymbols' => 1 ) ) . "', 
				...
				if ( $vs_render_as == 'horiz_hierbrowser_with_search' ) {
					$vs_buf .= "<div class='hierarchyBrowserSearchBar'>" . _t( 'Search' )
					           . ": <input type='text' id='{$ps_name}_hierarchyBrowserSearch{n}' class='hierarchyBrowserSearchBar' name='search' value='' size='20'/></div>";
				}

				if ( ! $vb_is_vertical_hier_browser ) {
					$vs_buf .= "<div id='{$ps_name}_browseCurrentSelection{n}' class='hierarchyBrowserCurrentSelection'>"
					           . _t( "Current selection" )
					           . ": <span id='{$ps_name}_browseCurrentSelectionText{n}' class='hierarchyBrowserCurrentSelectionText'>-</span></div>";
				}
			...
			default:
				...
				return caHTMLSelect( $ps_name, $va_options, $pa_attributes, array_merge( $pa_options,
					array( 'contentArrayUsesKeysForValues' => true, 'colors' => $va_colors, 'height' => null ) ) );
				break;
		}
	}
	# ------------------------------------------------------
	 ...
	 *
	 * @return array - List of available lists, indexed by list_id and locale_id. Array values are arrays with list
	 *               information including name, locale and list_id
	 */
	...
	static public function getListCodes( $pa_options = null ) {
		...
		if ( ExternalCache::contains( $vs_cache_key, 'listCodes' )
		     && is_array( $v = ExternalCache::fetch( $vs_cache_key, 'listCodes' ) )
		) {
			return $v;
		}
	 ...
	 *                                    dont_include_sub_items = if set, returned list is not expanded to include
	 *                                    sub-items dontIncludeSubItems = synonym for dont_include_sub_items
	 *                                    transaction = transaction to perform database operations within. [Default is
	 *                                    null]
	 *
	...
	static public function getItemIDsFromList( $pm_list_name_or_id, $pa_idnos, $pa_options = null ) {
		if ( isset( $pa_options['dontIncludeSubItems'] )
		     && ( ! isset( $pa_options['dont_include_sub_items'] )
		          || ! $pa_options['dont_include_sub_items'] )
		) {
		...
		foreach ( $pa_idnos as $vs_idno ) {
			...
			if ( is_numeric( $vs_idno ) ) {
				...
				$va_tmp = ca_list_items::find( array( 'idno' => $vs_idno, 'deleted' => 0 ),
					array( 'returnAs' => 'ids', 'transaction' => $o_trans ) );
			}
	...
	static public function getItemID( $pm_list_name_or_id, $ps_idno, $pa_options = null ) {
		if ( ( ! isset( $pa_options['noCache'] ) || ! isset( $pa_options['noCache'] ) )
		     && isset( ca_lists::$s_item_id_cache[ $pm_list_name_or_id ][ $ps_idno ] )
		) {
		...
		if ( $vn_list_id = ca_lists::getListID( $pm_list_name_or_id ) ) {
			...
			$qr_res = $o_db->query( "SELECT item_id FROM ca_list_items WHERE deleted = 0 AND list_id = ? AND idno = ?",
	...
	/**
	 * Converts a list of item_id's to a list of value strings. The conversion is literal without hierarchical
	 * expansion.
	 *
	 * @param array $pa_list    A list of relationship numeric item_ids
	 * @param array $pa_options Options include:
	...
	/**
	 * Converts a list of item idno's to a list of numeric item_id's. The conversion is literal without hierarchical
	 * expansion.
	 *
	 * @param array $pa_list    A list of relationship numeric item_ids
	 * @param array $pa_options Options include:
	 *                          transaction = transaction to perform database operations within. [Default is null]
	 *                          list_id = restrict lookup to a specific list by list_id. [Default is null; no
	 *                          filtering]
	 *
	...
	static public function getAccessForItemID( $pn_item_id ) {
		$o_db = new Db();
		$q    = $o_db->query( "SELECT access FROM ca_list_items WHERE item_id = ? AND deleted = 0",
			[ (int) $pn_item_id ] );
		while ( $q->nextRow() ) {
			return $q->get( 'access' );
		}
