...
BaseModel::$s_ca_models_definitions['ca_lists'] = array(
    ...
    'FIELDS' => array(
        ...
        'is_system_list' => array(
            ...
            'DESCRIPTION' => _t(
                'Set this if the list is a list used by the system to populate a specific field (as opposed to a user defined list or vocabulary). In general, system lists are defined by the system installer - you should not have to create system lists on your own.'
            ),
        ...
        'is_hierarchical' => array(
            ...
            'DESCRIPTION' => _t(
                'Set this if the list is hierarchically structured; leave unset if you are creating a simple "flat" list.'
            ),
            'BOUNDS_VALUE' => array(0, 1)
        ),
...
class ca_lists extends BundlableLabelableBaseModelWithAttributes
{
    ...
    public function getItemsForList($pm_list_name_or_id, $pa_options = null)
    {
        ...
        if ($t_list_item->getPrimaryKey()) {
            $vs_hier_sql = " AND ((cli.hier_left >= " . (floatval(
                    $t_list_item->get('hier_left')
        ...
        if (!isset($pa_options['returnHierarchyLevels']) || !$pa_options['returnHierarchyLevels']) {
            ...
            while ($qr_res->nextRow()) {
                ...
                if ((isset($pa_options['directChildrenOnly']) && $pa_options['directChildrenOnly']) && ($qr_res->get(
                            'parent_id'
                        ) != $pn_item_id)) {
                    continue;
                }
            ...
            if (isset($pa_options['extractValuesByUserLocale']) && $pa_options['extractValuesByUserLocale']) {
                ...
                if (($pn_sort == 0) && (sizeof(
                            $va_seen_locales
                        ) > 1)) {    // do we need to resort list based upon labels? (will already be in correct order if there's only one locale)
                    $va_labels = array();
            ...
            // hierarchical output
            $va_list_items = $t_list_item->getHierarchyAsList(
                $pn_item_id,
                array(
            ...
            foreach ($va_list_items as $vn_i => $va_item) {
                ...
                if (is_array($pa_check_access) && (sizeof($pa_check_access) > 0) && in_array(
                        $va_item['access'],
                        $pa_check_access
                    )) {
                    continue;
                ...
                if (isset($pa_options['extractValuesByUserLocale']) && $pa_options['extractValuesByUserLocale']) {
                    $va_items[$vn_parent_id][$vn_item_id][$va_item['NODE']['locale_id']][$vs_key][$vn_item_id] = array_merge(
                        $va_item['NODE'],
                        array('LEVEL' => $va_item['LEVEL'])
                    );
                } else {
                    $va_items[$vn_parent_id][$va_item['NODE']['locale_id']][$vs_key][$vn_item_id] = array_merge(
                        $va_item['NODE'],
                        array('LEVEL' => $va_item['LEVEL'])
                    );
                }
            ...
            if (is_array(
                    $va_items
                ) && (isset($pa_options['extractValuesByUserLocale']) && $pa_options['extractValuesByUserLocale'])) {
                //$va_items = caExtractValuesByUserLocale($va_items);
                $va_proc_items = array();
    ...
    {
        $pb_return_plural = !is_array($pa_options) ? (bool)$pa_options : (caGetOption(
                'return',
                $pa_options,
                'singular'
            ) == 'plural');
    ...
    public function getItemIDFromListByLabel($pm_list_name_or_id, $ps_label_name, $pa_options = null)
    {
        if ($vn_list_id = $this->_getListID($pm_list_name_or_id)) {
            if ($vn_id = ca_list_items::find(
                array('list_id' => $vn_list_id, 'preferred_labels' => array('name_plural' => $ps_label_name)),
                array('returnAs' => 'firstId', 'checkAccess' => caGetOption('checkAccess', $pa_options, null))
            )) {
                return $vn_id;
            } elseif ($vn_id = ca_list_items::find(
                array('list_id' => $vn_list_id, 'preferred_labels' => array('name_singular' => $ps_label_name)),
                array('returnAs' => 'firstId', 'checkAccess' => caGetOption('checkAccess', $pa_options, null))
            )) {
                return $vn_id;
            }
    ...
    public function getDefaultItemID($pm_list_name_or_id = null, $pa_options = null)
    {
        ...
        return caGetOption('useFirstElementAsDefaultDefault', $pa_options, false) ? array_shift(
            $this->getItemsForList($vn_list_id, array_merge($pa_options, array('idsOnly' => true)))
        ) : null;
    }
    # ------------------------------------------------------
    ...
    static public function getListAsHTMLFormElement(
        ...
        if (($vn_limit = caGetOption('maxItemCount', $pa_options, null)) && ($t_list->numItemsInList(
                    $pm_list_name_or_id,
                    null,
                    $pa_options
                ) > $vn_limit)) {
        ...
        if (!(isset($pa_options['limitToItemsRelatedToCollection']) && is_array(
                $pa_options['limitToItemsRelatedToCollections']
            ))) {
        ...
        $vb_is_vertical_hier_browser = in_array(
            $vs_render_as,
        ...
        if (!in_array(
            ...
            array(
                ...
                'vert_hierbrowser',
                'vert_hierbrowser_up',
                'vert_hierbrowser_down'
            )
        )) {
            if (isset($pa_options['limitToItemsRelatedToCollections']) && is_array(
                    $pa_options['limitToItemsRelatedToCollections']
                )) {
                ...
                if (sizeof($va_collection_ids)) {
                    $qr_collections = $t_list->makeSearchResult(
                        'ca_collections',
                        $va_collection_ids,
                        array('restrictToRelationshipTypes' => isset($pa_options['limitToItemsRelatedToCollectionWithRelationshipTypes']) ? $pa_options['limitToItemsRelatedToCollectionWithRelationshipTypes'] : null)
                    );

                    $va_item_ids = array();
                    while ($qr_collections->nextHit()) {
                        $va_list_items = $qr_collections->get(
                            'ca_list_items',
                            array(
                                'returnAsArray' => true,
                    ...
                    if ($va_limit_to_listIDs = ((isset($pa_options['limitToListIDs']) && is_array(
                            $pa_options['limitToListIDs']
                ...
                $va_list_items = $t_list->getItemsForList(
                    $pm_list_name_or_id,
                    array_merge(
                        $pa_options,
                        array(
                            'returnHierarchyLevels' => caGetOption(
                                'indentForHierarchy',
                                $pa_options,
                                true
                            ),
        ...
        if (!isset($pa_options['limitToItemsWithID']) || !is_array($pa_options['limitToItemsWithID']) || !sizeof(
                $pa_options['limitToItemsWithID']
            )) {
            $pa_options['limitToItemsWithID'] = null;
        }
        if (!isset($pa_options['omitItemsWithID']) || !is_array($pa_options['omitItemsWithID']) || !sizeof(
                $pa_options['omitItemsWithID']
            )) {
        ...
        if (is_array($pa_options['limitToItemsWithID']) && sizeof($pa_options['limitToItemsWithID'])) {
            ...
            foreach ($va_list_items as $vn_item_id => $va_item) {
                if (($vn_parent_id = $va_item['parent_id']) && in_array(
                        $vn_item_id,
                        $pa_options['limitToItemsWithID']
                    )) {
        ...
        $vn_default_val = null;
        foreach ($va_list_items as $vn_item_id => $va_item) {
            if (is_array($pa_options['limitToItemsWithID']) && !in_array(
                    $vn_item_id,
                    $pa_options['limitToItemsWithID']
                )) {
                continue;
            ...
            if (is_array($pa_exclude_items) && (sizeof($pa_exclude_items) > 0) && in_array(
                    $va_item['idno'],
                    $pa_exclude_items
                )) {
                continue;
            ...
            $va_options[$va_item[$pa_options['key']]] = str_repeat(
                    '&nbsp;',
                    intval($va_item['LEVEL']) * 3
                ) . ' ' . $va_item['name_singular'];
            if (!caGetOption('forceEnabled', $pa_options, false) && (!$va_item['is_enabled'] || (is_array(
                            $va_disabled_item_ids
        ...
        if (
            ...
            if (isset($omit_map[$omit_table]) && ($omit_map = $omit_map[$omit_table])) {
                ...
                if (is_array($omit_map)) {
                    $v = array_shift(
                        caExtractValuesByUserLocale(caGetListItemForValue('access_statuses', $pa_options['value']))
                    );
                    $value_idno = $v['idno'];
                    ...
                    foreach ($omit_map as $i => $value) {
                        ...
                        if (!is_numeric($value)) {
                            $value = caGetListItemValueForIdno(
                                'access_statuses',
                                $value
                            );    // convert status identifiers to integer code values
                            if (!strlen(trim($value))) {
                                continue;
                            }
                        }
        ...
        switch ($vs_render_as) {
            case 'radio_buttons':
                ...
                foreach ($va_options as $vm_value => $vs_label) {
                    ...
                    if (is_array($pa_options['value']) ? in_array(
                        $vm_value,
                        $pa_options['value']
            ...
            case 'checklist':
                ...
                $vs_buf = ($max_columns > 1) ? "<div class='checklist' style='grid-template-columns: " . str_repeat(
                        " {$p}%",
                        $max_columns
                    ) . ";'>\n" : "<div>\n";
                ...
                foreach ($va_options as $vm_value => $vs_label) {
                    ...
                    $vs_buf .= "<div class='checklistItem'>" . caHTMLCheckboxInput(
                            $ps_name . '_' . $vm_value,
                            $va_attributes,
                            $pa_options
                        ) . " " . str_replace('&nbsp;', '', $vs_label) . "</div>\n";
                }
                $vs_buf .= "</div>";
                return $vs_buf;
                break;
            ...
            case 'vert_hierbrowser_down':
                $va_width = caParseFormElementDimension(
                    $pa_options['width'] ? $pa_options['width'] : $pa_options['width']
                );
                ...
                if (!($current_selection_display_format = caGetOption(
...
";
                ...
                if ($vs_render_as == 'horiz_hierbrowser_with_search') {
                    $vs_buf .= "<div class='hierarchyBrowserSearchBar'>" . _t(
                            'Search'
                ...
                if (!$vb_is_vertical_hier_browser) {
                    $vs_buf .= "<div id='{$ps_name}_browseCurrentSelection{n}' class='hierarchyBrowserCurrentSelection'>" . _t(
                            "Current selection"
            ...
            default:
                ...
                return caHTMLSelect(
                    ...
                    array_merge(
                        $pa_options,
                        array(
                            'contentArrayUsesKeysForValues' => true,
                            'colors' => $va_colors,
                            'height' => null
                        )
                    )
    ...
    public static function getListCodes($pa_options = null)
    {
        ...
        if (ExternalCache::contains($vs_cache_key, 'listCodes') && is_array(
                $v = ExternalCache::fetch($vs_cache_key, 'listCodes')
            )) {
            return $v;
        }
    ...
    public static function getItemIDsFromList($pm_list_name_or_id, $pa_idnos, $pa_options = null)
    {
        ...
        foreach ($pa_idnos as $vs_idno) {
            ...
            if (is_numeric($vs_idno)) {
                ...
                $va_tmp = ca_list_items::find(
                    array('idno' => $vs_idno, 'deleted' => 0),
