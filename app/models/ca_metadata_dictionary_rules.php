...
BaseModel::$s_ca_models_definitions['ca_metadata_dictionary_rules'] = array(
	...
	'FIELDS'        => array(
		...
		'rule_code'  => array(
			...
			//'FILTER' => '!^[\p{L}0-9_]+$!u',
			'LABEL'          => _t( 'Rule code' ),
			'DESCRIPTION'    => _t( 'Unique alphanumeric code for the rule.' ),
		...
		'expression' => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Expression' ),
			'DESCRIPTION'    => _t( 'Expression to evaluate' ),
		...
		'rule_level' => array(
			...
			'DEFAULT'            => '',
			'LABEL'              => _t( 'Rule level' ),
			'DESCRIPTION'        => _t( 'Level of importance of rule.' ),
...
class ca_metadata_dictionary_rules extends BaseModel {
	...
	/**
	 * Sets setting definitions for to use for the current rule. Note that these definitions persist no matter what row
	 * is loaded
	 * (or even if no row is loaded). You can set the definitions once and reuse the instance for many rules. All will
	 * have the set definitions.
	 *
	...
	public function setSettingDefinitionsForRule( $pa_additional_settings ) {
		...
		$this->SETTINGS = new ModelSettings( $this, 'settings',
			array_merge( $_ca_metadata_dictionary_rules_settings, $pa_additional_settings ) );
	 ...
	 *                          db = Database connection to use. If omitted a new connection is created. [Default is
	 *                          null] bundles = List of bundle name to return rules for. If omitted all rules for all
	 *                          bundles are returned. [Default is null] table = Table to restrict entries to. If
	 *                          omitted rules for all tables are returned. [Default is null]
	 *
