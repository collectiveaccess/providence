...
class ca_editor_ui_screens extends BundlableLabelableBaseModelWithAttributes
{
    ...
    static public $s_table_num_cache;            // cache for getTableNum()
    ...
    public static function loadScreen($pm_ui, $pm_screen, $pa_options = null)
    {
        ...
        if (ca_editor_ui_screens::find(
            ['ui_id' => $t_ui->getPrimaryKey(), 'preferred_labels' => ['name' => $pm_screen]]
        )) {
            return $t_screen;
        }
    ...
    public function addPlacement($ps_bundle_name, $ps_placement_code, $pa_settings, $pn_rank = null, $pa_options = null)
    {
        ...
        if (!($t_instance = Datamodel::getInstance($table_name, true))) {
            $this->postError(
                1100,
                _t(
                    "Could not created user interface placement: user interface table '%1' is not valid",
                    $table_name
                ),
                "ca_editor_ui_screens::addPlacement"
            );
            return false;
        }

        if ((substr($ps_bundle_name, 0, 13) === 'ca_attribute_') && ca_metadata_elements::getElementID(
                substr($ps_bundle_name, 13)
            )) {
        ...
        if (defined("__CollectiveAccess_Installer__") && __CollectiveAccess_Installer__) {
            ...
            );    // force refresh of cache when installing, otherwise we'll get false failures for getBundleInfo()
        }
        if (!$t_instance->getBundleInfo($ps_bundle_name)) {
            $this->postError(
                1100,
                $c = _t(
                    "Could not create user interface placement: bundle '%1' is not valid",
                    $ps_bundle_name
                ),
                "ca_editor_ui_screens::addPlacement"
            );
     ...
     * @return int Returns placement_id of newly created placement on success, false on error
     */
    public function addPlacementBefore(
        $ps_bundle_name,
        $ps_placement_code,
        $pa_settings,
        $ps_relative_to_bundle_name_or_placement_code,
        $pa_options = null
    ) {
     ...
     * @return int Returns placement_id of newly created placement on success, false on error
     */
    public function addPlacementAfter(
        $ps_bundle_name,
        $ps_placement_code,
        $pa_settings,
        $ps_relative_to_bundle_name_or_placement_code,
    ...
    public function getAvailableBundles($pm_table_name_or_num = null, $pa_options = null)
    {
        ...
        $va_defined_bundles = method_exists($t_instance, "getBundleList") ? $t_instance->getBundleList(
        ...
        foreach ($va_defined_bundles as $vs_bundle => $va_info) {
                ...
                case 'related_table':
                    if (preg_match("/^([a-z_]+)_(related_list|table)$/", $vs_bundle, $va_matches)) {
                        ...
                        $va_additional_settings = array(
                            ...
                            'showCount' => array(
                                ...
                                'description' => _t(
                                    'If checked the number of relationships will be displayed in the header for the field.'
                                )
                            ),
                            'effectiveDateDefault' => array(
                                ...
                                'description' => _t(
                                    'Default effective date value for newly added relationships. Leave blank if you do not wish to set an effective date.'
                                )
                            ),
                            'numPerPage' => array(
                                'formatType' => FT_NUMBER,
                    ...
                    if ($vs_bundle == 'ca_object_representations') {
                        ...
                        $va_additional_settings['showBundlesForEditingOrder'] = [
                            ...
                            'description' => _t(
                                'List metadata element codes separated by commas, semicolons or returns in the order in which they should appear. If omitted the default sort order is used.'
                            )
                        ];
                    }
                            ...
                            case 'history_tracking_chronology':
                                $va_additional_settings = array(
                                    ...
                                    'useAppConfDefaults' => array(
                                        ...
                                        'description' => _t(
                                            'If checked all settings are taken from history tracking policy. Uncheck to override values.'
                                        )
                                    ),
                                    'sortDirection' => array(
                                        'formatType' => FT_TEXT,
                                    ...
                                    'currentValueColor' => array(
                                        ...
                                        'description' => _t(
                                            'Color to use as highlight for the current value in the history.'
                                        )
                                    ),
                                    'futureValueColor' => array(
                                        ...
                                        'description' => _t(
                                            'Color to use as highlight for future values in the history.'
                                        )
                                    ),
                                    'pastValueColor' => array(
                                        ...
                                        'description' => _t(
                                            'Color to use as highlight for the previous values in the history.'
                                        )
                                    ),
                                    ...
                                    'hide_include_child_history_controls' => array(
                                        ...
                                        'description' => _t(
                                            'Check this option if you want to hide the "Include child history" controls in this bundle placement.'
                                        )
                                    ),
                                    'hide_add_to_loan_controls' => array(
                                        ...
                                        'description' => _t(
                                            'Check this option if you want to hide the "Add to loan" controls in this bundle placement.'
                                        )
                                    ),
                                    ...
                                    'hide_update_location_controls' => array(
                                        ...
                                        'description' => _t(
                                            'Check this option if you want to hide the "Update Location" controls in this bundle placement.'
                                        )
                                    ),
                                    ...
                                    'hide_add_to_occurrence_controls' => array(
                                        ...
                                        'description' => _t(
                                            'Check this option if you want to hide the "Add to" occurrence controls in this bundle placement.'
                                        ),
                                    ...
                                    'hide_add_to_collection_controls' => array(
                                        ...
                                        'description' => _t(
                                            'Check this option if you want to hide the "Add to collection" controls in this bundle placement.'
                                        )
                                    ),
                                    ...
                                    'hide_add_to_entity_controls' => array(
                                        ...
                                        'description' => _t(
                                            'Check this option if you want to hide the "Add to entity" controls in this bundle placement.'
                                        )
                                    ),
                                    ...
                                    'hide_add_to_object_controls' => array(
                                        ...
                                        'description' => _t(
                                            'Check this option if you want to hide the "Add to object" controls in this bundle placement.'
                                        )
                                    ),
                                    ...
                                    'hide_value_interstitial_edit' => array(
                                        ...
                                        'description' => _t(
                                            'Check this option if you want to hide "edit" controls for each history value in this bundle placement.'
                                        )
                                    ),
                                    'hide_value_delete' => array(
                                        ...
                                        'description' => _t(
                                            'Check this option if you want to hide "delete" controls for each history value in this bundle placement.'
                                        )
                                    )
                                );

                                $va_additional_settings = array_merge(
                                    $va_additional_settings,
                                    ca_objects::getHistoryTrackingEditorBundleSettingsData(
                                        $vs_table
                                    )
                                );

                                $va_to_hide_when_using_defaults = array_values(
                                    array_filter(
                                        array_keys($va_additional_settings),
                                        function ($v) {
                                            return preg_match(
                                                "!^(ca_|showDeaccessionInformation|deaccession_|dateMode)!",
                                                $v
                                            );
                                        }
                                    )
            ...
            $va_available_bundles[$vs_display][$vs_bundle] = array(
                ...
                'settingsForm' => $t_placement->getHTMLSettingForm(
                    array('id' => $vs_bundle . '_0_', 'table' => $vs_table)
                ),
