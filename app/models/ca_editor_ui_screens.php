...
BaseModel::$s_ca_models_definitions['ca_editor_ui_screens'] = array(
	...
	'FIELDS'        => array(
		'screen_id'  => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this user interface screen' )
		),
		'parent_id'  => array(
			'FIELD_TYPE'     => FT_NUMBER,
		...
		'is_default' => array(
			...
			'DESCRIPTION'    => _t( 'Indicates if this screen should be used as the default screen when creating a new item.' )
		),
		'color'      => array(
			'FIELD_TYPE'     => FT_TEXT,
...
class ca_editor_ui_screens extends BundlableLabelableBaseModelWithAttributes {
	...
	protected function initLabelDefinitions( $pa_options = null ) {
		...
		$this->BUNDLES['ca_user_roles']  = array(
			'type'      => 'special',
			'repeating' => true,
			'label'     => _t( 'Role access' )
		);
	...
	static public function loadScreen( $pm_ui, $pm_screen, $pa_options = null ) {
		...
		if ( ! $t_ui->isLoaded() ) {
			$t_ui = ca_editor_uis::find( [ 'preferred_labels' => [ 'name' => $pm_ui ] ],
		...
		if ( ca_editor_ui_screens::find( [
			...
			'preferred_labels' => [ 'name' => $pm_screen ]
		] )
		) {
			return $t_screen;
		}
	 ...
	 *                               Until version 1.7.9 metadata element codes had to be prefixed with
	 *                               "ca_attribute_"; as of version 1.7.9 the prefix is no longer required.
	 * @param array  $pa_settings    Placement settings array; keys should be valid setting names
	 * @param int    $pn_rank        Optional value that determines sort order of bundles in the screen. If omitted,
	 *                               placement is added to the end of the screen.
	...
	public function addPlacement( $ps_bundle_name, $ps_placement_code, $pa_settings, $pn_rank = null, $pa_options = null
		...
		if ( ! ( $t_instance = Datamodel::getInstance( $table_name, true ) ) ) {
			$this->postError( 1100,
				_t( "Could not created user interface placement: user interface table '%1' is not valid", $table_name ),
				"ca_editor_ui_screens::addPlacement" );

			return false;
		}

		if ( ( substr( $ps_bundle_name, 0, 13 ) === 'ca_attribute_' )
		     && ca_metadata_elements::getElementID( substr( $ps_bundle_name, 13 ) )
		) {
		...
		if ( ! $t_instance->getBundleInfo( $ps_bundle_name ) ) {
			$this->postError( 1100,
				$c = _t( "Could not create user interface placement: bundle '%1' is not valid", $ps_bundle_name ),
				"ca_editor_ui_screens::addPlacement" );

			return false;
		}

		$t_placement = new ca_editor_ui_bundle_placements( null,
	 ...
	 *
	 * @param string $ps_bundle_name                               Name of bundle to add (eg. ca_objects.idno,
	 *                                                             ca_objects.preferred_labels.name)
	 * @param string $ps_placement_code                            Placement code
	 * @param array  $pa_settings                                  Placement settings array; keys should be valid
	 *                                                             setting names
	 * @param string $ps_relative_to_bundle_name_or_placement_code Bundle name or placement code of placement to insert
	 *                                                             new placement before.
	 * @param array  $pa_options                                   Optional array of options. Supports the following
	 *                                                             options: user_id = if specified then add will fail
	 *                                                             if specified user does not have edit access for the
	 *                                                             display
	 *
	 * @return int Returns placement_id of newly created placement on success, false on error
	 */
	public function addPlacementBefore(
		$ps_bundle_name, $ps_placement_code, $pa_settings, $ps_relative_to_bundle_name_or_placement_code,
		$pa_options = null
	) {
		if ( $t_placement = $this->findPlacement( $ps_relative_to_bundle_name_or_placement_code ) ) {
			...
			foreach ( $placements as $placement_id => $placement ) {
				...
				if ( $placement['rank'] == $rank ) {
					if ( ! ( $rc = $this->addPlacement( $ps_bundle_name, $ps_placement_code, $pa_settings, $old_rank,
						$pa_options ) )
					) {
						return false;
					}
	...
	public function addPlacementAfter(
		...
		if ( $t_placement = $this->findPlacement( $ps_relative_to_bundle_name_or_placement_code ) ) {
			...
			foreach ( $placements as $placement_id => $placement ) {
				if ( $placement['rank'] == $rank ) {
					if ( ! ( $rc = $this->addPlacement( $ps_bundle_name, $ps_placement_code, $pa_settings, $rank + 1,
						$pa_options ) )
					) {
						return false;
					}
	...
	public function findPlacement( $ps_bundle_name_or_placement_code, $pa_options = null ) {
		...
		foreach ( $placements as $placement_id => $placement ) {
			if ( ( $placement['bundle_name'] === $ps_bundle_name_or_placement_code )
			     || ( $placement['placement_code'] === $ps_bundle_name_or_placement_code )
			) {
	 ...
	 *                          screen has no placements, or if there is no display loaded table = if using the
	 *                          returnAllAvailableIfEmpty option and you expect a list of available bundles to be
	 *                          returned if no display is loaded, you must specify the table the bundles are intended
	 *                          for use with with this option. Either the table name or number may be used. user_id =
	 *                          if specified then placements are only returned if the user has at least read access to
	 *                          the display, screen_id = get placements for specified screen id rather than currently
	 *                          loaded screen. [Default is null] placement_id = get specific placementrather than currently loaded screen. [Default is null]
	 *
	 * @return array List of placements in display order. Array is keyed on bundle name. Values are arrays with the
	 *               following keys: placement_id = primary key of ca_editor_ui_bundle_placements row - a unique id for
	...
	public function getPlacements( $pa_options = null ) {
		...
		$ps_table                         = ( isset( $pa_options['table'] ) ) ? $pa_options['table']
			: $this->getTableNum();
		$pn_user_id                       = isset( $pa_options['user_id'] ) ? $pa_options['user_id'] : null;
		...
		if ( ! $vn_screen_id && ! $vn_placement_id && ! ( $vn_screen_id = $this->getPrimaryKey() ) ) {
			if ( $pb_return_all_available_if_empty && $ps_table ) {
				return ca_editor_ui_screens::$s_placement_list_cache[ $vn_screen_id ]
					= $this->getAvailableBundles( $ps_table, [ 'table' => $ps_table ] );
			}
		...
		if ( ! $pb_no_cache && $vn_screen_id && isset( ca_editor_ui_screens::$s_placement_list_cache[ $vn_screen_id ] )
		     && ca_editor_ui_screens::$s_placement_list_cache[ $vn_screen_id ]
		) {
			return ca_editor_ui_screens::$s_placement_list_cache[ $vn_screen_id ];
		}
		...
		if ( $qr_res->numRows() > 0 ) {
			...
			while ( $qr_res->nextRow() ) {
				...
				if ( ! $pb_settings_only ) {
					...
					$t_instance                                   = Datamodel::getInstanceByTableName( $va_tmp[0],
						true );
					$va_placements[ $vn_placement_id ]['display'] = ( $t_instance
	...
	public function getAvailableBundles( $pm_table_name_or_num = null, $pa_options = null ) {
		...
		$va_defined_bundles = method_exists( $t_instance, "getBundleList" )
			? $t_instance->getBundleList( array( 'includeBundleInfo' => true ) )
			: [];        // these are the bundles defined for this type of editor

		$va_available_bundles = array();

		$va_elements = ca_metadata_elements::getElementsAsList( true, $pm_table_name_or_num, null, ! $pb_dont_cache,
			false, true );
		foreach ( $va_defined_bundles as $vs_bundle => $va_info ) {
			...
			switch ( $va_info['type'] ) {
				...
				case 'attribute':
					$va_additional_settings = array(
						'sort'              => array(
							...
							'multiple'                => true,
							'label'                   => _t( 'Sort using' ),
							'showSortableElementsFor' => $va_elements[ preg_replace( '!ca_attribute_!', '',
								$vs_bundle ) ]['element_id'],
					...
					if ( $va_elements[ preg_replace( '!ca_attribute_!', '', $vs_bundle ) ]['datatype']
					     == 1
					) {        // 1=text
						$va_additional_settings['usewysiwygeditor'] = array(
							'formatType'  => FT_TEXT,
				...
				case 'related_table':
					if ( preg_match( "/^([a-z_]+)_(related_list|table)$/", $vs_bundle, $va_matches ) ) {
						...
						$va_path                = array_keys( Datamodel::getPath( $t_instance->tableName(),
							$vs_bundle ) );
						$va_additional_settings = array(
							'restrict_to_relationship_types'            => array(
								'formatType'              => FT_TEXT,
							...
							'allowedSorts'                              => array(
								'formatType'  => FT_TEXT,
								'displayType' => DT_SELECT,
								'options'     => array_flip( caGetAvailableSortFields( $vs_bundle, null, [
									'includeInterstitialSortsFor' => $vs_table,
									'distinguishInterstitials'    => true
								] ) ),
								'default'     => null,
								'multiple'    => true,
								'width'       => "475px",
					...
					if ( ( $t_instance->tableName() == 'ca_objects' )
					     && in_array( $vs_bundle, array( 'ca_list_items' ) )
					) {
					...
					if ( in_array( $vs_bundle, array( 'hierarchy_location', 'hierarchy_navigation' ) ) ) {
						...
						switch ( $vs_bundle ) {
							...
							case 'history_tracking_chronology':
								...
								$va_additional_settings = array_merge( $va_additional_settings,
									ca_objects::getHistoryTrackingEditorBundleSettingsData( $vs_table ) );

								$va_to_hide_when_using_defaults
									= array_values( array_filter( array_keys( $va_additional_settings ),
									function ( $v ) {
										return preg_match( "!^(ca_|showDeaccessionInformation|deaccession_|dateMode)!",
											$v );
									} ) );
								$va_additional_settings['useAppConfDefaults']['hideOnSelect']
									= $va_to_hide_when_using_defaults;
								break;
							...
							case 'ca_set_items':
								...
								$va_additional_settings = array();
								foreach (
									$t_set->getFieldInfo( 'table_num', 'BOUNDS_CHOICE_LIST' ) as $vs_table_display_name
								=> $vn_table_num
								) {
									$va_additional_settings[ Datamodel::getTableName( $vn_table_num )
									                         . '_display_template' ]
										= array(
										'formatType'  => FT_TEXT,
										'displayType' => DT_FIELD,
										...
										'description' => _t( 'Layout for %1 set item information when used in a display list. For example: <i>^ca_objects.deaccession_notes</i>.',
			...
			$vs_label = $t_instance->getDisplayLabel( $t_instance->tableName() . '.' . $vs_bundle_proc )
				?: $va_info['label'];
			$vs_display
			          = "<div id='uiEditorBundle_{$vs_table}_{$vs_bundle_proc}'><span class='bundleDisplayEditorPlacementListItemTitle'>"
			            . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) ) . "</span> " . ( $vs_label )
			            . "</div>";
			...
			if ( $t_instance->getProperty( 'ATTRIBUTE_TYPE_ID_FLD' ) ) {
				$va_additional_settings['bundleTypeRestrictions']                = [
					...
					'width'             => "475px",
					'height'            => 5,
					'label'             => _t( 'Display bundle for types: %1',
	...
	public function removeTypeRestriction( $pn_type_id = null ) {
		...
		if ( is_array( $va_screens = ca_editor_ui_screen_type_restrictions::find( $va_params,
	...
	public function getPlacementsInScreen( $pa_options = null ) {
		...
		foreach ( $va_placements as $vn_placement_id => $va_placement ) {
			...
			$vs_label       = ( $vs_label = ( $t_instance->getDisplayLabel( $table . '.' . $vs_bundle_proc ) ) )
			...
			$vs_display
				= "<div id='uiEditor_{$vn_placement_id}'><span class='bundleDisplayEditorPlacementListItemTitle'>"
	...
	public function savePlacementsFromHTMLForm( $po_request, $ps_form_prefix, $ps_placement_code ) {
		if ( $vs_bundles = $po_request->getParameter( "{$ps_placement_code}{$ps_form_prefix}displayBundleList",
			pString )
		) {
			...
			foreach ( $va_placements as $vn_placement_id => $va_bundle_info ) {
				if ( ! in_array( $va_bundle_info['bundle_name'] . '_' . $va_bundle_info['placement_id'],
					...
					$t_screen->removePlacement( $va_bundle_info['placement_id'],
			...
			foreach ( $va_bundles as $vn_i => $vs_bundle ) {
				...
				foreach ( $_REQUEST as $vs_key => $vs_val ) {
					if ( preg_match( "!^{$vs_bundle_proc}_([\d]+)_(.*)$!", $vs_key, $va_matches ) ) {
						...
						if ( preg_match( '!(.*)_([a-z]{2}_[A-Z]{2})$!', $va_matches[2], $va_locale_matches ) ) {
							...
							//
							$va_settings[ (int) $va_matches[1] ][ $va_locale_matches[1] ][ $vn_locale_id ]
								= $va_settings[ (int) $va_matches[1] ][ $va_locale_matches[1] ][ $va_locale_matches[2] ]
								= $vs_val;
						} else {
				...
				if ( $vn_placement_id === 0 ) {
					$t_screen->addPlacement( $vs_bundle, $vs_bundle . ( $vn_i + 1 ), $va_settings[ $vn_placement_id ],
						$vn_i + 1, array(
