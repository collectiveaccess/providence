...
BaseModel::$s_ca_models_definitions['ca_editor_ui_screens'] = array(
    ...
    'FIELDS' => array(
        'screen_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique numeric identifier used by CollectiveAccess internally to identify this user interface screen'
            )
        ),
        'parent_id' => array(
            'FIELD_TYPE' => FT_NUMBER,
        ...
        'idno' => array(
            ...
            'IS_NULL' => false,
            'DEFAULT' => '',
            'LABEL' => _t('Screen identifier'),
            'DESCRIPTION' => _t('Unique alphanumeric identifier for this screen'),
        ...
        'is_default' => array(
            ...
            'IS_NULL' => false,
            'DEFAULT' => '',
            'LABEL' => _t('Is default screen?'),
            'DESCRIPTION' => _t(
                'Indicates if this screen should be used as the default screen when creating a new item.'
            )
        ),
        'color' => array(
            'FIELD_TYPE' => FT_TEXT,
            ...
            'DEFAULT' => '',
            'LABEL' => _t('Color'),
            'DESCRIPTION' => _t('Color to identify the screen with')
        ),
        'icon' => array(
            'FIELD_TYPE' => FT_MEDIA,
            'DISPLAY_TYPE' => DT_FIELD,
            ...
            'LABEL' => _t('Icon'),
            'DESCRIPTION' => _t('Optional icon identify the screen with')
        ),
        'hier_left' => array(
            'FIELD_TYPE' => FT_NUMBER,
            'DISPLAY_TYPE' => DT_OMIT,
...
class ca_editor_ui_screens extends BundlableLabelableBaseModelWithAttributes
{
    ...
    protected function initLabelDefinitions($pa_options = null)
    {
        ...
        $this->BUNDLES['ca_editor_ui_bundle_placements'] = array(
            'type' => 'special',
            'repeating' => false,
            'label' => _t('Screen content')
        );
        $this->BUNDLES['ca_editor_ui_screen_type_restrictions'] = array(
            'type' => 'special',
            'repeating' => false,
    ...
    public function getPlacements($pa_options = null)
    {
        ...
        if (!($vn_screen_id = $this->getPrimaryKey())) {
            if ($pb_return_all_available_if_empty && $ps_table) {
                return ca_editor_ui_screens::$s_placement_list_cache[$vn_screen_id] = $this->getAvailableBundles(
                    $ps_table
                );
            }
        }
    ...
    public function getAvailableBundles($pm_table_name_or_num = null, $pa_options = null)
    {
        ...
        $va_defined_bundles = $t_instance->getBundleList(
            array('includeBundleInfo' => true)
        ...
        foreach ($va_defined_bundles as $vs_bundle => $va_info) {
                ...
                case 'nonpreferred_label':
                    ...
                    if (($va_info['type'] == 'preferred_label') && ($vs_table == 'ca_objects')) {
                        $va_additional_settings['use_list'] = array(
                            ...
                            'description' => _t(
                                'Suggest values using a specific list. Select <em>All lists</em> to suggest any configured list value.'
                            )
                        );
                        $va_additional_settings['use_list_format'] = array(
                            ...
                            'description' => _t(
                                'Suggest values with the selected list as either a type-ahead lookup or a drop-down menu.'
                            )
                        );
                    }
                    break;
                case 'attribute':
                    $va_additional_settings = array(
                        'sort' => array(
                            ...
                            'multiple' => true,
                            'label' => _t('Sort using'),
                            'showSortableElementsFor' => $va_elements[preg_replace(
                                '!ca_attribute_!',
                                '',
                                $vs_bundle
                            )]['element_id'],
                        ...
                        'displayTemplate' => array(
                            ...
                            'label' => _t('Display template'),
                            'description' => _t(
                                'Layout for preview of this field. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_objects.my_element_code</i>.'
                            )
                        ),
                        'documentation_url' => array(
                            'formatType' => FT_TEXT,
                            ...
                            'label' => _t('Documentation URL'),
                            'description' => _t(
                                'URL pointing to documentation for this field. Leave blank if you wish to use the default URL for this metadata element.'
                            )
                        )
                    );
                    if ($va_elements[preg_replace(
                            '!ca_attribute_!',
                            '',
                            $vs_bundle
                        )]['datatype'] == 1) {        // 1=text
                ...
                case 'related_table':
                    if (preg_match("/^([a-z_]+)_(related_list|table)$/", $vs_bundle, $va_matches)) {
                        ...
                        $va_additional_settings = array(
                            ...
                            'restrict_to_search' => array(
                                ...
                                'description' => _t(
                                    'Restricts display to items matching the given search expression. Leave empty for no restriction.'
                                )
                            ),
                        ...
                        if (($t_instance->tableName() == 'ca_storage_locations') && ($t_rel->tableName(
                                ) == 'ca_objects')) {
                        ...
                        $va_additional_settings = array(
                            ...
                            'restrict_to_access_point' => array(
                                ...
                                'description' => _t(
                                    'Restricts display to items matching the given search expression in the specified access point. Leave empty to search in all fields.'
                                )
                            ),
                            'dont_include_subtypes_in_type_restriction' => array(
                                ...
                                'description' => _t(
                                    'Normally restricting to type(s) automatically includes all sub-(child) types. If this option is checked then the lookup results will include items with the selected type(s) <b>only</b>.'
                                )
                            ),
                            'list_format' => array(
                                'formatType' => FT_TEXT,
                            ...
                            'colorItem' => array(
                                ...
                                'label' => _t('Item color'),
                                'description' => _t(
                                    'If set item that are not first or last in list will use this color.'
                                )
                            ),
                            'colorLastItem' => array(
                                'formatType' => FT_TEXT,
                            ...
                            'documentation_url' => array(
                                ...
                                'label' => _t('Documentation URL'),
                                'description' => _t(
                                    'URL pointing to documentation for this relationship bundle. Leave blank if no documentation URL exists.'
                                )
                            ),
                            'minRelationshipsPerRow' => array(
                                ...
                                'default' => '',
                                'label' => _t(
                                    'Minimum number of relationships of this kind to be associated with an item. '
                                ),
                                'description' => _t(
                                    'If set to 0 a delete button will allow a cataloguer to clear all relationships.  If set to 1 or more, it will not be possible to delete all relationships once the minimum is established. Note that this is only a user interface limitations rather than constraints on the underlying data model.'
                                )
                            ),
                            'maxRelationshipsPerRow' => array(
                                ...
                                'default' => '',
                                'label' => _t(
                                    'Maximum number of relationships of this kind that can be associated with an item'
                                ),
                                'description' => _t(
                                    'The extent of repeatability for the relationship will match the number entered here. Note that this is only a user interface limitations rather than constraints on the underlying data model.'
                                )
                            ),
                            ...
                            'disableQuickadd' => array(
                                ...
                                'description' => _t(
                                    'If checked quickadd will be disabled regardless of user privileges.'
                                )
                            ),
                            'prepopulateQuickaddFields' => array(
                                ...
                                'default' => '0',
                                'options' => [
                                    _t('Preferred label') => 'preferred_labels',
                                    _t('Identifier') => $t_rel->getProperty('ID_NUMBERING_ID_FIELD')
                                ],
                                'label' => _t('Prepopulate quick add fields with search text'),
                                'description' => _t(
                                    'Select quickadd form fields to be pre-filled with the user-entered search value. If no fields are selected then the preferred label will be prepopulated by default.'
                                )
                            ),
                            'disableSorts' => array(
                                'formatType' => FT_TEXT,
                            ...
                            'allowedSorts' => array(
                                ...
                                'options' => array_flip(
                                    caGetAvailableSortFields(
                                        $vs_bundle,
                                        null,
                                        ['includeInterstitialSortsFor' => $vs_table, 'distinguishInterstitials' => true]
                                    )
                                ),
                                'default' => null,
                                'multiple' => true,
                    ...
                    if (in_array($vs_bundle, array('ca_places', 'ca_list_items', 'ca_storage_locations'))) {
                        $va_additional_settings['useHierarchicalBrowser'] = array(
                            ...
                            'description' => _t(
                                'If checked a hierarchical browser will be used to select related items instead of an auto-complete lookup.'
                            )
                        );
                    ...
                    if (($t_instance->tableName() == 'ca_objects') && in_array($vs_bundle, array('ca_list_items'))) {
                        ...
                        $va_additional_settings['restrictToTermsOnCollectionUseRelationshipType'] = array(
                            ...
                            'description' => _t(
                                'Specified the relationship used to relate collection-restricted terms to this object.'
                            )
                        );
                    }
                    ...
                    if (in_array($vs_bundle, array('hierarchy_location', 'hierarchy_navigation'))) {
                        $va_additional_settings = array(
                            ...
                            'auto_shrink' => array(
                                ...
                                'description' => _t(
                                    'Check this option if you want the hierarchy browser to automatically shrink or expand based on the height of the column with the most data.'
                                )
                            ),
                            'documentation_url' => array(
                                'formatType' => FT_TEXT,
                                ...
                                'label' => _t('Documentation URL'),
                                'description' => _t(
                                    'URL pointing to documentation for this hierarchy browser. Leave blank if no documentation URL exists.'
                                )
                            )
                        );
                    } else {
                        switch ($vs_bundle) {
                            case 'authority_references_list':
                                $va_additional_settings = array(
                                    ...
                                    'ca_objects_displayTemplate' => array(
                                        ...
                                        'description' => _t(
                                            'Layout for referencing objects. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_objects.idno</i>.'
                                        )
                                    ),
                                    'ca_object_lots_displayTemplate' => array(
                                        ...
                                        'description' => _t(
                                            'Layout for referencing lots. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_object_lots.idno_stub</i>.'
                                        )
                                    ),
                                    'ca_entities_displayTemplate' => array(
                                        ...
                                        'description' => _t(
                                            'Layout for referencing entities. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_entities.idno</i>.'
                                        )
                                    ),
                                    'ca_places_displayTemplate' => array(
                                        ...
                                        'description' => _t(
                                            'Layout for referencing places. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_places.idno</i>.'
                                        )
                                    ),
                                    'ca_occurrences_displayTemplate' => array(
                                        ...
                                        'description' => _t(
                                            'Layout for referencing occurrences. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_occurrences.idno</i>.'
                                        )
                                    ),
                                    'ca_collections_displayTemplate' => array(
                                        ...
                                        'description' => _t(
                                            'Layout for referencing collections. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_collections.idno</i>.'
                                        )
                                    ),
                                    'ca_loans_displayTemplate' => array(
                                        ...
                                        'description' => _t(
                                            'Layout for referencing loans. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_loans.idno</i>.'
                                        )
                                    ),
                                    'ca_movements_displayTemplate' => array(
                                        ...
                                        'description' => _t(
                                            'Layout for referencing movements. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_movements.idno</i>.'
                                        )
                                    ),
                                    'ca_object_representations_displayTemplate' => array(
                                        ...
                                        'description' => _t(
                                            'Layout for referencing object representations. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_object_representations.idno</i>.'
                                        )
                                    ),
                                    'ca_list_items_displayTemplate' => array(
                                        ...
                                        'description' => _t(
                                            'Layout for referencing list items. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_list_items.idno</i>.'
                                        )
                                    )
                                );
                                break;
                            case 'ca_object_representation_chooser':
                                $va_additional_settings = array(
                                    'elementCode' => array(
                                        ...
                                        'takesLocale' => false,
                                        'default' => '',
                                        'showMetadataElementsWithDataType' => 21,
                                        // 21="ObjectRepresentation" metadata elements
                                        'table' => $pm_table_name_or_num,
                                        'label' => _t('Metadata element'),
                                        'description' => _t(
                                            'Metadata element to store representation selection in. Must be of type ObjectRepresentation.'
                                        )
                                    )
                                );
                                break;
                            case 'ca_objects_components_list':
                                $va_additional_settings = array(
                                    'displayTemplate' => array(
                                        ...
                                        'description' => _t(
                                            'Layout for component when displayed in list (can include HTML). Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_objects.idno</i>.'
                                        )
                                    )
                                );
                                break;
                            case 'circulation_status':
                                $va_additional_settings = array(// @todo: maybe add settings!?
                                );
                                break;
                            case 'ca_objects_location':
                                $va_additional_settings = array(
                                    ...
                                    'currentLocationColor' => array(
                                        ...
                                        'description' => _t(
                                            'Color to use as highlight for the current location in the location history.'
                                        )
                                    ),
                                    'futureLocationColor' => array(
                                        ...
                                        'description' => _t(
                                            'Color to use as highlight for future locations in the location history.'
                                        )
                                    ),
                                    'pastLocationColor' => array(
                                        ...
                                        'description' => _t(
                                            'Color to use as highlight for the previous locations in the location history.'
                                        )
                                    ),
                                    'useHierarchicalBrowser' => array(
                                        ...
                                        'description' => _t(
                                            'If checked a hierarchical browser will be used to select storage location items rather than an auto-complete lookup.'
                                        )
                                    )
                                );
                                break;
                            case 'ca_objects_history':
                                $va_to_hide_when_using_defaults = array(
                                    'ca_object_lots_showTypes',
                                    'ca_occurrences_showTypes',
                                    'ca_loans_showTypes',
                                    'ca_movements_showTypes',
                                    'ca_storage_locations_showRelationshipTypes',
                                    'ca_storage_locations_color',
                                ...
                                $va_additional_settings = array(
                                    'useAppConfDefaults' => array(
                                        ...
                                        'description' => _t(
                                            'If checked all settings are taken from the main application configuration file (app.conf).'
                                        )
                                    ),
                                    'locationTrackingMode' => array(
                                        'formatType' => FT_TEXT,
                                    ...
                                    'hide_include_child_history_controls' => array(
                                        ...
                                        'description' => _t(
                                            'Check this option if you want to to hide the "Include child history" controls in this bundle placement.'
                                        )
                                    ),
                                    'hide_add_to_loan_controls' => array(
                                        ...
                                        'description' => _t(
                                            'Check this option if you want to to hide the "Add to loan" controls in this bundle placement.'
                                        )
                                    ),
                                    'hide_update_location_controls' => array(
                                        ...
                                        'description' => _t(
                                            'Check this option if you want to to hide the "Update Location" controls in this bundle placement.'
                                        )
                                    ),
                                    'hide_add_to_occurrence_controls' => array(
                                        ...
                                        'description' => _t(
                                            'Check this option if you want to to hide the "Add to" occurrence controls in this bundle placement.'
                                        ),
                                ...
                                foreach ($va_types as $vn_type_id => $va_type) {
                                    ...
                                    $va_additional_settings["ca_object_lots_{$va_type['idno']}_includeFromChildren"] = array(
                                        ...
                                        'height' => 4,
                                        'label' => _t(
                                            'Include history from lots (%1) related to child objects',
                                    ...
                                    $va_additional_settings["ca_object_lots_{$va_type['idno']}_childDisplayTemplate"] = array(
                                        ...
                                        'height' => 4,
                                        'label' => _t(
                                            'Lot (%1) display template when related to child objects',
                                            $va_type['name_singular']
                                        ),
                                        'description' => _t(
                                ...
                                foreach ($va_types as $vn_type_id => $va_type) {
                                    ...
                                    $va_additional_settings["ca_occurrences_{$va_type['idno']}_includeFromChildren"] = array(
                                        ...
                                        'height' => 4,
                                        'label' => _t(
                                            'Include history from %1 related to child objects',
                                            $va_type['name_plural']
                                        ),
                                        'description' => _t(
                                    ...
                                    $va_additional_settings["ca_occurrences_{$va_type['idno']}_childDisplayTemplate"] = array(
                                        ...
                                        'height' => 4,
                                        'label' => _t(
                                            'Display template for %1 when related to child objects',
                                            $va_type['name_plural']
                                        ),
                                        'description' => _t(
                                ...
                                $va_additional_settings += array(
                                    ...
                                    'ca_storage_locations_includeFromChildren' => array(
                                        ...
                                        'height' => 4,
                                        'label' => _t(
                                            'Include history from storage locations related to child objects'
                                        ),
                                        'description' => _t(
                                    ...
                                    'ca_storage_locations_childDisplayTemplate' => array(
                                        ...
                                        'height' => 4,
                                        'label' => _t(
                                            'Display template for storage locations when related to child objects'
                                        ),
                                        'description' => _t(
                                    ...
                                    'showDeaccessionInformation' => array(
                                        ...
                                        'description' => _t(
                                            'If clicked deaccession information will be shown in the history.'
                                        )
                                    ),
                                    ...
                                    'deaccession_includeFromChildren' => array(
                                        ...
                                        'height' => 4,
                                        'label' => _t(
                                            'Include history from deaccession information related to child objects'
                                        ),
                                        'description' => _t(
                            ...
                            case 'ca_storage_locations_contents':
                                $va_additional_settings = array(
                                    ...
                                    'displayTemplate' => array(
                                        ...
                                        'label' => _t('Display template'),
                                        'description' => _t(
                                            'Layout for each object in the storage location (can include HTML). The template is evaluated relative to each object-movement or object-location relationship. Element code tags prefixed with the ^ character can be used to represent the value in the template. For example: <i>^ca_objects.idno</i>.'
                                        )
                                    )
                                );
                                break;
                            case 'ca_set_items':
                                ...
                                foreach (
                                    $t_set->getFieldInfo(
                                        'table_num',
                                        'BOUNDS_CHOICE_LIST'
                                    ) as $vs_table_display_name => $vn_table_num
                                ) {
                                    $va_additional_settings[Datamodel::getTableName(
                                        $vn_table_num
                                    ) . '_display_template'] = array(
                                        'formatType' => FT_TEXT,
                                        'displayType' => DT_FIELD,
                                        'default' => '',
                                        'width' => "275px",
                                        ...
                                        'description' => _t(
                                            'Layout for %1 set item information when used in a display list. For example: <i>^ca_objects.deaccession_notes</i>.',
                        ...
                        $va_additional_settings['documentation_url'] = array(
                            ...
                            'label' => _t('Documentation URL'),
                            'description' => _t(
                                'URL pointing to documentation. Leave blank if no documentation URL exists.'
                            )
                        );
                    }
                    break;
            ...
            $vs_label = $t_instance->getDisplayLabel(
                $t_instance->tableName() . '.' . $vs_bundle_proc
            ) ?: $va_info['label'];
            $vs_display = "<div id='uiEditorBundle_{$vs_table}_{$vs_bundle_proc}'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
            ...
            if ($t_instance->getProperty('ATTRIBUTE_TYPE_ID_FLD')) {
                $va_additional_settings['bundleTypeRestrictions'] = [
                    ...
                    'height' => 4,
                    'label' => _t(
                        'Display bundle for types: %1',
                        Datamodel::getTableProperty($vs_table, 'NAME_PLURAL')
                    ),
                    'description' => _t(
                        'Restrict which types this bundle is displayed for. If no types are selected the bundle will be displayed for <strong>all</strong> types.'
                    )
                ];
    ...
    public function removeTypeRestriction($pn_type_id = null)
    {
        ...
        if (is_array(
            $va_screens = ca_editor_ui_screen_type_restrictions::find($va_params, ['returnAs' => 'modelInstances'])
        )) {
    ...
    public function getPlacementsInScreen($pa_options = null)
    {
        ...
        foreach ($va_placements as $vn_placement_id => $va_placement) {
            ...
            $vs_display = "<div id='uiEditor_{$vn_placement_id}'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                    $t_instance->getProperty('NAME_SINGULAR')
                ) . "</span> {$vs_label}</div>";
    ...
    public function getTypeRestrictionsHTMLFormBundle(
        ...
        if ($t_instance instanceof BaseRelationshipModel) { // interstitial
            $o_view->setVar(
                'type_restrictions',
                $t_instance->getRelationshipTypesAsHTMLSelect(
                    $t_instance->getLeftTableName(),
                    null,
                    null,
                    array('name' => 'type_restrictions[]', 'multiple' => 1, 'size' => 5),
                    array('values' => $va_restriction_type_ids)
                ) . $vs_subtype_element
            );
