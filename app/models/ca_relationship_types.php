...
BaseModel::$s_ca_models_definitions['ca_relationship_types'] = array(
	...
	'FIELDS'        => array(
		'type_id'                => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this relationship type' )
		),
		'sub_type_left_id'       => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Type restriction for "left" side of relationship' ),
			'DESCRIPTION'    => _t( 'Type restriction for "left" side of relationship (eg. if relationship is object-entity, then the restriction controls for which types of objects this relationship type is valid.' )
		),
		'include_subtypes_left'  => array(
			'FIELD_TYPE'     => FT_BIT,
			'DISPLAY_TYPE'   => DT_CHECKBOXES,
			'DISPLAY_WIDTH'  => 40,
			'DISPLAY_HEIGHT' => 1,
			'IS_NULL'        => false,
			'DEFAULT'        => '0',
			'LABEL'          => _t( 'Include subtypes in "left" side restriction?' ),
			'DESCRIPTION'    => _t( 'Automatically nclude subtypes of selected left-side restriction.' )
		),
		'sub_type_right_id'      => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Type restriction for "right" side of relationship' ),
			'DESCRIPTION'    => _t( 'Type restriction for "right" side of relationship (eg. if relationship is object-entity, then the restriction controls for which types of entities this relationship type is valid.' )
		),
		'include_subtypes_right' => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '0',
			'LABEL'          => _t( 'Include subtypes in "right" side restriction?' ),
			'DESCRIPTION'    => _t( 'Automatically nclude subtypes of selected left-side restriction.' )
		),
		'parent_id'              => array(
			'FIELD_TYPE'         => FT_NUMBER,
			...
			'DEFAULT'            => '',
			'LABEL'              => 'Parent',
			'DESCRIPTION'        => 'Identifier of parent relationship type; is null if relationship type is root of hierarchy.',
		...
		'hier_type_id'           => array(
			...
			'IS_NULL'            => true,
			'DEFAULT'            => '',
			'LABEL'              => _t( 'Relationship type hierarchy identifier' ),
			'DESCRIPTION'        => _t( 'Identifier of relationship_type that is root of the relationship_type hierarchy.' ),
			'DONT_USE_AS_BUNDLE' => true
		),
		'type_code'              => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Relationship type code' ),
			'DESCRIPTION'    => _t( 'Unique identifier for the relationship type; must be unique for relationship types for the current relationship.' ),
		...
		'table_num'              => array(
			...
			'DESCRIPTION'        => _t( 'Indicates the type of relationship for which this type is applicable.' ),
		...
		'rank'                   => array(
			...
			'DEFAULT'        => '10',
			'LABEL'          => _t( 'Sort order' ),
			'DESCRIPTION'    => _t( 'The relative priority of the relationship type when displayed in a list with other relationship types. Lower numbers indicate higher priority.' ),
		...
		'is_default'             => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Is default?' ),
			'DESCRIPTION'    => _t( 'If checked this relationship type will be the default type for the relationship.' )
		),
		'hier_left'              => array(
			'FIELD_TYPE'         => FT_NUMBER,
			'DISPLAY_TYPE'       => DT_OMIT,
...
class ca_relationship_types extends BundlableLabelableBaseModelWithAttributes {
	...
	public function load( $pm_id = null, $pb_use_cache = true ) {
		...
		$this->FIELDS['sub_type_left_id']['BOUNDS_CHOICE_LIST']
			= $this->FIELDS['sub_type_right_id']['BOUNDS_CHOICE_LIST'] = array();
	 ...
	 *
	 * Returns array keyed on relationship type_id; values are associative arrays keys on
	 * ca_relationship_types/ca_relationship_type_labels field names
	 */
	public function getRelationshipInfo( $pm_table_name_or_num, $ps_type_code = null, $options = null ) {
		...
		while ( $qr_res->nextRow() ) {
			...
			$va_row['type_code']
			                                                                                           = mb_strtolower( $va_row['type_code'] );
	 ...
	 *                          create = create relationship type using parameters if one with the specified type code
	 *                          or type_id doesn't exist already [default=false] cache = cache relationship types as
	 *                          they are referenced and return cached value if possible [default=true]
	 */
	public function getRelationshipTypeID(
		$pm_table_name_or_num, $pm_type_code_or_id, $pn_locale_id = null, $pa_values = null, $pa_options = null
	) {
		...
		if ( $pa_options['cache']
		     && isset( ca_relationship_types::$s_relationship_type_id_cache[ $vn_table_num . '/'
		                                                                     . $pm_type_code_or_id ] )
		) {
		...
		if ( is_numeric( $pm_type_code_or_id ) ) {
			if ( $va_relationships = $this->getRelationshipInfo( $pm_table_name_or_num ) ) {
				if ( isset( $va_relationships[ $pm_type_code_or_id ] ) ) {
					return ca_relationship_types::$s_relationship_type_id_cache[ $vn_table_num . '/'
					                                                             . $pm_type_code_or_id ]
						= $pm_type_code_or_id;
				}
			}
			...
			if ( $va_relationships = $this->getRelationshipInfo( $pm_table_name_or_num, $pm_type_code_or_id ) ) {
				foreach ( $va_relationships as $vn_type_id => $va_type_info ) {
					if ( ( mb_strtolower( $va_type_info['type_code'] ) === $pm_type_code_or_id )
					     || ( mb_strtolower( $va_type_info['typename'] ) === $pm_type_code_or_id )
					     || ( mb_strtolower( $va_type_info['typename_reverse'] ) === $pm_type_code_or_id )
					) {
						return ca_relationship_types::$s_relationship_type_id_cache[ $vn_table_num . '/'
						                                                             . $pm_type_code_or_id ]
							= $vn_type_id;
		...
		if ( isset( $pa_options['create'] ) && $pa_options['create'] && $pn_locale_id && is_array( $pa_values ) ) {
			...
			$t_rel->set( 'sub_type_left_id',
				isset( $pa_values['sub_type_left_id'] ) ? (int) $pa_values['sub_type_left_id'] : null );
			$t_rel->set( 'include_subtypes_left',
				( isset( $pa_values['include_subtypes_left'] ) && (bool) $pa_values['include_subtypes_left'] ) ? 1
					: 0 );
			$t_rel->set( 'sub_type_right_id',
				isset( $pa_values['sub_type_right_id'] ) ? (int) $pa_values['sub_type_right_id'] : null );
			$t_rel->set( 'include_subtypes_right',
				( isset( $pa_values['include_subtypes_right'] ) && (bool) $pa_values['include_subtypes_right'] ) ? 1
					: 0 );
			...
			$t_rel->addLabel(
				array(
					'typename'         => isset( $pa_values['typename'] ) ? $pa_values['typename']
						: $pm_type_code_or_id,
					'typename_reverse' => isset( $pa_values['typename_reverse'] ) ? $pa_values['typename_reverse']
						: $pm_type_code_or_id,

					'description'         => isset( $pa_values['description'] ) ? $pa_values['description'] : '',
					'description_reverse' => isset( $pa_values['description_reverse'] )
						? $pa_values['description_reverse'] : ''
				),
	 ...
	 *
	 * @param mixed $pm_table_name_or_num The name or number of the relationship table that the types are valid for
	 *                                    (Eg. ca_objects_x_entities)
	 * @param array $pa_list              A list of relationship type_code string and/or numeric type_ids
	 * @param array $pa_options           Options include:
	 *                                    includeChildren = If set to true, ids of children of relationship types are
	...
	public function relationshipTypeListToIDs( $pm_table_name_or_num, $pa_list, $pa_options = null ) {
		...
		if ( isset( $pa_options['includeChildren'] ) && $pa_options['includeChildren'] ) {
			...
			foreach ( $va_rel_ids as $vn_id ) {
				$va_children = array_merge( $va_children,
					$this->getHierarchyChildren( $vn_id, array( 'idsOnly' => true ) ) );
			}
	...
	/**
	 * Converts a list of relationship type_id's to a list of type_code strings. The conversion is literal without
	...
	static public function relationshipTypeIDsToTypeCodes( $pa_ids, $pa_options = null ) {
		...
		return ca_relationship_types::$s_relationship_type_id_to_code_cache[ $vs_key ] = $va_type_ids_to_codes
		                                                                                 + $va_non_numerics;
	}
	...
	/**
	 * Returns name of hierarchy for currently loaded item or, if specified, item with item_id = to optional $pn_id
	 * parameter
	 */
	...
	public function moveRelationshipsToType( $pm_type_code_or_id ) {
		...
		if ( ! ( $vn_type_id = $this->getRelationshipTypeID( $vn_table_num = $this->get( 'table_num' ),
			$pm_type_code_or_id ) )
		) {
			return null;
		}
		...
		foreach ( $va_to_reindex_relations as $vn_relation_id => $va_row ) {
			BaseModel::$search_indexer->indexRow( $vn_table_num, $vn_relation_id, $va_row, false, null,
				array( 'type_id' => true ) );
		}
	...
	public function isSaveable( $po_request, $ps_bundle_name = null ) {
		...
		if ( $po_request->user->canDoAction( 'is_administrator' )
		     || $po_request->user->canDoAction( 'can_configure_relationship_types' )
		) {
			return true;
		}
	...
	public function isDeletable( $po_request ) {
		...
		// Check actions
		if ( $this->getPrimaryKey()
		     && ( $po_request->user->canDoAction( 'is_administrator' )
		          || $po_request->user->canDoAction( 'can_configure_relationship_types' ) )
		) {
			return true;
		}
