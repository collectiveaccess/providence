...
BaseModel::$s_ca_models_definitions['ca_relationship_types'] = array(
    ...
    'FIELDS' => array(
        'type_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique numeric identifier used by CollectiveAccess internally to identify this relationship type'
            )
        ),
        'sub_type_left_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Type restriction for "left" side of relationship (eg. if relationship is object-entity, then the restriction controls for which types of objects this relationship type is valid.'
            )
        ),
        ...
        'sub_type_right_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Type restriction for "right" side of relationship (eg. if relationship is object-entity, then the restriction controls for which types of entities this relationship type is valid.'
            )
        ),
        ...
        'type_code' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique identifier for the relationship type; must be unique for relationship types for the current relationship.'
            ),
        ...
        'rank' => array(
            ...
            'LABEL' => _t('Sort order'),
            'DESCRIPTION' => _t(
                'The relative priority of the relationship type when displayed in a list with other relationship types. Lower numbers indicate higher priority.'
            ),
...
class ca_relationship_types extends BundlableLabelableBaseModelWithAttributes
{
     ...
     */
    static public $s_relationship_type_id_cache = array();
     ...
     */
    static public $s_relationship_type_table_cache = array();

    /**
     * @var cached type_id to type_code entries; used by relationshipTypeIDsToTypeCodes()
     */
    static public $s_relationship_type_id_to_code_cache = array();
    ...
    public function getRelationshipTypeID(
        ...
        if (is_numeric($pm_type_code_or_id)) {
            ...
            if ($va_relationships = $this->getRelationshipInfo($pm_table_name_or_num, $pm_type_code_or_id)) {
                foreach ($va_relationships as $vn_type_id => $va_type_info) {
                    if ((mb_strtolower($va_type_info['type_code']) === $pm_type_code_or_id) || (mb_strtolower(
                                $va_type_info['typename']
                            ) === $pm_type_code_or_id) || (mb_strtolower(
                                $va_type_info['typename_reverse']
                            ) === $pm_type_code_or_id)) {
        ...
        if (isset($pa_options['create']) && $pa_options['create'] && $pn_locale_id && is_array($pa_values)) {
            ...
            $t_rel->set(
                ...
                (isset($pa_values['include_subtypes_left']) && (bool)$pa_values['include_subtypes_left']) ? 1 : 0
            );
            $t_rel->set(
                'sub_type_right_id',
            ...
            $t_rel->set(
                ...
                (isset($pa_values['include_subtypes_right']) && (bool)$pa_values['include_subtypes_right']) ? 1 : 0
            );
    ...
    public function moveRelationshipsToType($pm_type_code_or_id)
    {
        ...
        if (!($vn_type_id = $this->getRelationshipTypeID(
        ...
        foreach ($va_to_reindex_relations as $vn_relation_id => $va_row) {
            BaseModel::$search_indexer->indexRow(
                $vn_table_num,
                $vn_relation_id,
                $va_row,
                false,
    ...
    {
        // Check actions
        if ($po_request->user->canDoAction('is_administrator') || $po_request->user->canDoAction(
    ...
    public function isDeletable($po_request)
    {
        ...
        // Check actions
        if ($this->getPrimaryKey() && ($po_request->user->canDoAction(
                    'is_administrator'
