...
BaseModel::$s_ca_models_definitions['ca_item_comments'] = array(
	...
	'FIELDS'        => array(
		'comment_id'           => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this comment' )
		),
		'table_num'            => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => 'Table comment applies to',
		...
		'user_id'              => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'User' ),
			'DESCRIPTION'    => _t( 'The user who created the comment.' )
		),
		'row_id'               => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_FIELD,
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Row ID',
			'DESCRIPTION'    => 'Primary key value of the row in the table specified by table_num that this comment applies to.'
		),
		'locale_id'            => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Locale' ),
			'DESCRIPTION'    => _t( 'The locale of the comment.' )
		),
		'comment'              => array(
			'FIELD_TYPE'     => FT_TEXT,
			'DISPLAY_TYPE'   => DT_FIELD,
		...
		'email'                => array(
			...
			'DESCRIPTION'    => _t( 'E-mail address of commentor.' ),
		...
		'name'                 => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Name' ),
			'DESCRIPTION'    => _t( 'Name of commenter.' ),
			'BOUNDS_LENGTH'  => array( 0, 255 )
		),
		'location'             => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Location' ),
			'DESCRIPTION'    => _t( 'Location of commenter.' ),
			'BOUNDS_LENGTH'  => array( 0, 255 )
		),
		'rating'               => array(
			'FIELD_TYPE'         => FT_NUMBER,
			...
			'LABEL'              => _t( 'Rating' ),
			'DESCRIPTION'        => _t( 'User-provided rating for item.' )
		),
		'media1'               => array(
			'FIELD_TYPE'     => FT_MEDIA,
			'DISPLAY_TYPE'   => DT_FIELD,
		...
		'created_on'           => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Comment creation date' ),
			'DESCRIPTION'    => _t( 'The date and time the comment was created.' )
		),
		'ip_addr'              => array(
			'FIELD_TYPE'     => FT_TEXT,
			'DISPLAY_TYPE'   => DT_FIELD,
		...
		'moderated_by_user_id' => array(
			...
			'DEFAULT'        => null,
			'LABEL'          => _t( 'Moderator' ),
			'DESCRIPTION'    => _t( 'The user who examined the comment for validity and applicability.' )
		),
		'moderated_on'         => array(
			...
			'DESCRIPTION'    => _t( 'The date and time the comment was examined for validity and applicability.' )
		)
	)
);
...
class ca_item_comments extends BaseModel {
	 ...
	 *
	 * @return BaseModel instance of model for item associated with comment; null if no comment is loaded; or false if
	 *                   the associated item cannot be fetched.
	 */
	...
	public static function getCommentUsersForSelect() {
		$o_db = new Db();
		$qr
		      = $o_db->query( "SELECT u.user_id, u.fname, u.lname, u.email FROM ca_item_comments c INNER JOIN ca_users AS u ON c.user_id = u.user_id GROUP BY u.user_id" );

		$opts = [];
		while ( $qr->nextRow() ) {
			$opts[ $qr->get( 'fname' ) . ' ' . $qr->get( 'lname' ) . ' (' . $qr->get( 'email' ) . ')' ]
				= $qr->get( 'user_id' );
		}
	 ...
	 *                                         itemsPerPage = Maximum number of comments to return at once. [Default is
	 *                                         36] request = The current request.
	 *
	 * @return array
	 */
	public static function getItemCommentDataForResult( $result, $options = null ) {
		...
		if ( sizeof( $item_ids ) > 0 ) {
			if ( is_array( $row_id_conv = ca_sets::getRowIDsForItemIDs( $item_ids ) ) && sizeof( $row_id_conv ) ) {
				foreach ( $row_id_conv as $tn => $l ) {
					...
					$idnos  = $t_table->getFieldValuesForIDs( array_values( $l ),
						[ $t_table->getProperty( 'ID_NUMBERING_ID_FIELD' ) ] );

					foreach ( $l as $item_id => $row_id ) {
						$set
							                     = array_shift( caExtractValuesByUserLocale( $t_set->getSets( [ 'item_id' => $item_id ] ) ) );
						$item_labels[ $item_id ] = [
							'table_num'   => $tn,
							'id'          => $row_id,
							'label'       => $labels[ $row_id ],
							'idno'        => $idnos[ $row_id ],
							'set_message' => _t( "Comment made in set %1",
								$request ? caEditorLink( $request, $set['name'], '', 'ca_sets', $set['set_id'] ) : '' )
						];
					}
				}
			}
		...
		if ( sizeof( $row_ids ) > 0 ) {
			foreach ( $row_ids as $tn => $l ) {
				...
				$labels = $t_table->getPreferredDisplayLabelsForIDs( array_values( $l ) );
				$flds   = $t_table->getFieldValuesForIDs( array_values( $l ),
					[ $t_table->getProperty( 'ID_NUMBERING_ID_FIELD' ), 'source_id' ] );

				foreach ( $l as $row_id ) {
					$row_labels[ $row_id ] = [
						'id'     => $row_id,
						'label'  => $labels[ $row_id ],
						'idno'   => $flds[ $row_id ]['idno'],
						'source' => $flds[ $row_id ]['source_id']
					];
				}
			}
		}
		...
		return [
			...
			'moderated_on' => $result->get( 'moderated_on' )
		];
	}
	# ------------------------------------------------------

	/**
	 * Fetch data for current item comment in a result set, using label data bulk-fetched via
	 * ca_item_comments::getItemCommentDataForResult()
	 *
	 * @param ItemCommentSearchResult $result  A ca_item_comments search result
	 * @param array                   $data    Item comment label data extracted from the result using
	 *                                         ca_item_comments::getItemCommentDataForResult()
	...
	public static function getItemCommentDataForDisplay( $result, $data, $options = null ) {
		...
		foreach (
			[
				'ca_item_comments.comment_id',
				'ca_item_comments.comment',
				'ca_item_comments.created_on',
				'ca_item_comments.moderated_on'
			] as $f
		) {
			$ft         = array_pop( explode( '.', $f ) );
			$res[ $ft ] = $result->get( $f );
