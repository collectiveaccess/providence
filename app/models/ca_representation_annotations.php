...
BaseModel::$s_ca_models_definitions['ca_representation_annotations'] = array(
	...
	'FIELDS'        => array(
		...
		'locale_id'         => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Locale' ),
			'DESCRIPTION'    => _t( 'The locale from which the annotation originates.' )
		),
		'user_id'           => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_SELECT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'User' ),
			'DESCRIPTION'    => _t( 'The user who created the annotation.' )
		),
		'type_code'         => array(
			'FIELD_TYPE'     => FT_TEXT,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Type code' ),
			'DESCRIPTION'    => _t( 'Code indicating type of annotation.' ),
		...
		'props'             => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Properties',
			'DESCRIPTION'    => 'Container for annotation properties.'
		),
		'preview'           => array(
			'FIELD_TYPE'     => FT_MEDIA,
			...
			'LABEL'       => _t( 'Preview media' ),
			'DESCRIPTION' => _t( 'Use this control to select media from your computer to upload for use as a preview.' )
		),
		'access'            => array(
			'FIELD_TYPE'         => FT_NUMBER,
			'DISPLAY_TYPE'       => DT_SELECT,
			...
			'LABEL'              => _t( 'Access' ),
			'DESCRIPTION'        => _t( 'Indicates if annotation is accessible to the public or not. ' )
		),
		'status'            => array(
			'FIELD_TYPE'         => FT_NUMBER,
			'DISPLAY_TYPE'       => DT_SELECT,
			...
			'LABEL'              => _t( 'Status' ),
			'DESCRIPTION'        => _t( 'Indicates the current state of the annotation .' )
		),
		'view_count'        => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_OMIT,
...
class ca_representation_annotations extends BundlableLabelableBaseModelWithAttributes implements IBundleProvider {
	...
	protected function initLabelDefinitions( $pa_options = null ) {
		...
		$this->BUNDLES['ca_representation_annotation_properties'] = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Annotation properties' )
		);
	 ...
	 *                          assumeIdnoForRepresentationID = set to force lookup of representation_id values as
	 *                          ca_representations.idno values first not matter what, before consideration as a numeric
	 *                          representation_id. The default is false, in which case integer values are considered
	 *                          representation_ids and non-numeric values possible idnos.
	 *                          tryObjectIdnoForRepresentationID = try to set representation_id using the primary
	 *                          representation of the object with idno as specified for representaton_id
	 *
	 * @return int
	 */
	public function set( $pm_fields, $pm_value = "", $pa_options = null ) {
		...
		foreach ( $pm_fields as $vs_fld => $vs_val ) {
			if ( ( $vs_fld == 'representation_id' )
			     && ( $pb_assume_idno_for_representation_id
			          || preg_match( "![^\d]+!", $vs_val ) )
			) {
			...
			if ( ( $vs_fld == 'representation_id' ) && ( $pb_try_object_idno )
			     && ( $t_object
					= ca_objects::find( [ 'idno' => "{$vs_val}" ],
					[ 'allowWildcards' => true, 'returnAs' => 'firstModelInstance' ] ) )
			) {
	 ...
	 *                          forcePreviewGeneration = if set preview media will be regenerated whether time code has
	...
	public function update( $pa_options = null ) {
		...
		if ( ! $this->getAppConfig()->get( 'dont_generate_annotation_previews' ) && $this->getPrimaryKey()
		     && ( $this->changed( 'props' )
		          || ( isset( $pa_options['forcePreviewGeneration'] )
		               && $pa_options['forcePreviewGeneration'] ) )
		) {
			...
			if ( ( $vs_file = $t_rep->getMediaPath( 'media', 'original' ) ) && file_exists( $vs_file ) ) {
				...
				if ( $o_media->read( $vs_file ) ) {
					if ( $o_media->writeClip( $vs_file = tempnam( caGetTempDirPath(), 'annotationPreview' ), $vs_start,
						$vs_end )
					) {
	...
	public function getPropertyList() {
		return is_object( $this->opo_annotations_properties ) ? $this->opo_annotations_properties->getPropertyList()
			: array();
	}
	...
	# ------------------------------------------------------
	public function getPropertyHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $ps_property, $pa_attributes = null, $pa_options = null
	) {
	...
	public function loadProperties( $ps_type, $pa_parameters = null ) {
		...
		$this->opo_annotations_properties->setPropertyValues( is_array( $pa_parameters ) ? $pa_parameters
			: $this->get( 'props' ) );
			...
			foreach ( $pa_versions as $vs_version ) {
				...
				if ( isset( $va_tmp['info'][ $vs_version ]['WIDTH'] )
					...
					if ( ( $vn_w = $va_tmp['info'][ $vs_version ]['WIDTH'] )
					     && ( $vn_h
							= $va_tmp['info'][ $vs_version ]['WIDTH'] )
					) {
						$va_dimensions[] = $va_tmp['info'][ $vs_version ]['WIDTH'] . 'p x '
						                   . $va_tmp['info'][ $vs_version ]['HEIGHT'] . 'p';
					}
				}
	...
	public function getTypeList( $pa_options = null ) {
		...
		foreach ( $o_annotation_type_conf->get( 'types' ) as $vs_type_code => $va_type_info ) {
			...
			$va_type_list[ $va_type_info['typeID'] ] = array_merge( array( 'idno' => $vs_type_code ), $va_type_info );
		}
