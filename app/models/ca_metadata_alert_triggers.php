...
BaseModel::$s_ca_models_definitions['ca_metadata_alert_triggers'] = array(
    ...
    'FIELDS' => array(
        'trigger_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique numeric identifier used by CollectiveAccess internally to identify this trigger'
            )
        ),
        'rule_id' => array(
            'FIELD_TYPE' => FT_NUMBER,
        ...
        'trigger_type' => array(
            ...
            'LABEL' => _t('Type'),
            'DESCRIPTION' => _t(
                'Alerts may be triggered by various types of events. Select the type of event to trigger this alert here.'
            ),
        )
    )
);
...
class ca_metadata_alert_triggers extends BaseModel
{
            ...
            if (is_array($va_users)) {
                foreach ($va_users as $va_user) {
                    if ($va_user['access'] >= __CA_ALERT_RULE_ACCESS_NOTIFICATION__) {
                        ...
                        if ($t_user->notificationExists(
                            __CA_NOTIFICATION_TYPE_METADATA_ALERT__,
                            $vs_notification_key
                        )) {
                            continue;
                        }
                        $t_user->addNotification(
                            __CA_NOTIFICATION_TYPE_METADATA_ALERT__,
                            $po_trigger->getNotificationMessage($t_subject),
                            false,
                            [
    ...
    public static function firePeriodicTriggers()
    {
        foreach (self::tablesWithRules() as $vn_table_num => $vs_table) {
            ...
            foreach ($va_triggers as $va_trigger) {
                $o_trigger = CA\MetadataAlerts\TriggerTypes\Base::getInstance(
                    $va_trigger['info']['trigger_type'],
                    $va_trigger['info']
                );
