...
BaseModel::$s_ca_models_definitions['ca_metadata_alert_triggers'] = array(
	...
	'FIELDS'        => array(
		'trigger_id'      => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this trigger' )
		),
		'rule_id'         => array(
			'FIELD_TYPE'       => FT_NUMBER,
			...
			'LABEL'            => 'User id',
			'DESCRIPTION'      => 'Identifier for rule this trigger belongs to'
		),
		'element_id'      => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Element id',
			'DESCRIPTION'    => 'Identifier for trigger element'
		),
		'element_filters' => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Element filters' ),
			'DESCRIPTION'    => _t( 'Serialized filter data' )
		),
		'settings'        => array(
			'FIELD_TYPE'     => FT_VARS,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Settings' ),
			'DESCRIPTION'    => _t( 'Trigger settings' )
		),
		'trigger_type'    => array(
			'FIELD_TYPE'         => FT_TEXT,
			...
			'LABEL'              => _t( 'Type' ),
			'DESCRIPTION'        => _t( 'Alerts may be triggered by various types of events. Select the type of event to trigger this alert here.' ),
		)
	)
);
...
class ca_metadata_alert_triggers extends BaseModel {
	...
	public static function fireTrigger( $po_trigger, &$t_subject, $pa_trigger ) {
		...
		if ( $po_trigger->check( $t_subject ) ) {
			...
			if ( ! is_array(
				$va_delivery_options = caGetOption( 'notificationDeliveryOptions', $pa_trigger['settings'], null ) )
			) {
				$va_delivery_options = [];
			}
			...
			if ( is_array( $va_users ) ) {
				foreach ( $va_users as $va_user ) {
					if ( $va_user['access'] >= __CA_ALERT_RULE_ACCESS_NOTIFICATION__ ) {
						...
						if ( $t_user->notificationExists( __CA_NOTIFICATION_TYPE_METADATA_ALERT__,
							$vs_notification_key )
						) {
							continue;
						}
						$t_user->addNotification( __CA_NOTIFICATION_TYPE_METADATA_ALERT__,
							$po_trigger->getNotificationMessage( $t_subject ), false, [
								'key'            => $vs_notification_key,
								'data'           => $po_trigger->getData( $t_subject ),
								'deliverByEmail' => $vb_email,
								'deliverToInbox' => $vb_inbox
							] );
					}
				}
			}
	...
	private static function getApplicableTriggersForInstance( $t_subject = null ) {
		...
		$va_rules = ca_metadata_alert_rules::find( [
			'table_num' => is_object( $t_subject ) ? $t_subject->tableNum() : $t_subject
	...
	private static function getApplicableTriggersForTable( $pn_table_num ) {
		...
		// find applicable rules
		$va_rules = ca_metadata_alert_rules::find( [ 'table_num' => $pn_table_num ],
			[ 'returnAs' => 'modelInstances' ] );
	...
	public static function firePeriodicTriggers() {
		foreach ( self::tablesWithRules() as $vn_table_num => $vs_table ) {
			...
			foreach ( $va_triggers as $va_trigger ) {
				$o_trigger = CA\MetadataAlerts\TriggerTypes\Base::getInstance( $va_trigger['info']['trigger_type'],
					$va_trigger['info'] );
	...
	public static function tablesWithRules() {
		...
		while ( $qr_table_nums->nextRow() ) {
			$va_table_list[ $vn_table_num = $qr_table_nums->get( 'table_num' ) ]
				= Datamodel::getTableName( $vn_table_num );
		}
