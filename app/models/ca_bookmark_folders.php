...
BaseModel::$s_ca_models_definitions['ca_bookmark_folders'] = array(
	...
	'FIELDS'        => array(
		'folder_id' => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this bookmark folder' )
		),
		'name'      => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Folder name' ),
			'DESCRIPTION'    => _t( 'A name to display for the folder.' ),
		...
		'user_id'   => array(
			...
			'DEFAULT'         => '',
			'LABEL'           => 'Owner',
			'DESCRIPTION'     => 'Identifier for owner of bookmark folder'
		),
		'rank'      => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_FIELD,
...
class ca_bookmark_folders extends BaseModel {
	...
	public function addBookmark(
		$pn_table_name_or_num, $pn_row_id, $ps_notes = null, $pn_rank = null, $pn_folder_id = null, $pn_user_id = null
	) {
		if ( ! ( $vn_folder_id = $this->_getFolderID( $pn_folder_id, $pn_user_id ) ) ) {
			return false;
		}
		...
		$t_bookmark->load( array(
			"folder_id" => $vn_folder_id,
	 ...
	 *
	 * @param array $pa_bookmark_ids A list of bookmark_ids in the folder, in the order in which they should be
	 *                               displayed in the ui
	...
	public function reorderBookmarks( $pa_bookmark_ids, $pn_folder_id = null, $pn_user_id = null, $pa_options = null ) {
		...
		$va_bookmark_ranks = $this->getBookmarkIDRanks( $pn_folder_id, $pn_user_id,
			$pa_options );    // get current ranks
		...
		foreach ( $pa_bookmark_ids as $vn_rank => $vn_bookmark_id ) {
			if ( isset( $va_bookmark_ranks[ $vn_bookmark_id ] )
			     && $t_bookmark->load( array( 'folder_id' => $vn_folder_id, 'bookmark_id' => $vn_bookmark_id ) )
			) {
				if ( $va_bookmark_ranks[ $vn_bookmark_id ] != $vn_rank ) {
					...
					if ( $t_bookmark->numErrors() ) {
						$va_errors[ $vn_bookmark_id ] = _t( 'Could not reorder bookmark %1: %2', $vn_bookmark_id,
							join( '; ', $t_bookmark->getErrors() ) );
					}
				}
			}
		}
	 ...
	 *
	 * @param int   $pn_folder_id Optional folder_id to set
	 * @param int   $pn_user_id   the user_id of the current user; used to determine which folders the user has access
	 *                            to
