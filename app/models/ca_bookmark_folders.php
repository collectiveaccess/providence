...
BaseModel::$s_ca_models_definitions['ca_bookmark_folders'] = array(
    ...
    'FIELDS' => array(
        'folder_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique numeric identifier used by CollectiveAccess internally to identify this bookmark folder'
            )
        ),
        'name' => array(
            'FIELD_TYPE' => FT_TEXT,
...
class ca_bookmark_folders extends BaseModel
{
    ...
    public function reorderBookmarks($pa_bookmark_ids, $pn_folder_id = null, $pn_user_id = null, $pa_options = null)
    {
        ...
        foreach ($pa_bookmark_ids as $vn_rank => $vn_bookmark_id) {
            if (isset($va_bookmark_ranks[$vn_bookmark_id]) && $t_bookmark->load(
                    array('folder_id' => $vn_folder_id, 'bookmark_id' => $vn_bookmark_id)
                )) {
                if ($va_bookmark_ranks[$vn_bookmark_id] != $vn_rank) {
                    ...
                    if ($t_bookmark->numErrors()) {
                        $va_errors[$vn_bookmark_id] = _t(
                            'Could not reorder bookmark %1: %2',
                            $vn_bookmark_id,
