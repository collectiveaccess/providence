...
BaseModel::$s_ca_models_definitions['ca_users'] = array(
	...
	'FIELDS'        => array(
		'user_id'          => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'User id' ),
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this user' )
		),
		'user_name'        => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'User name' ),
			'DESCRIPTION'    => _t( 'The login name for this user. This name is used in combination with the password set below to access the system.' ),
		...
		'userclass'        => array(
			...
			'DEFAULT'            => 0,
			'LABEL'              => _t( 'User class' ),
			'DESCRIPTION'        => _t( '"Full" user accounts may log into all CollectiveAccess interfaces. "Public" user accounts may only log into the publicly accessible front-end system (if one exists). "Deleted" users may not log into any interface – the account is considered removed.' ),
		...
		'password'         => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Password' ),
			'DESCRIPTION'    => _t( 'The login password for this user. Passwords must be at least 4 characters and should ideally contain a combination of letters and numbers. Passwords are case-sensitive.' ),
		...
		'fname'            => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'First name' ),
			'DESCRIPTION'    => _t( 'The forename of this user.' ),
		...
		'lname'            => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Last name' ),
			'DESCRIPTION'    => _t( 'The surname of this user.' ),
			'BOUNDS_LENGTH'  => array( 1, 255 )
		),
		'email'            => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'E-mail' ),
			'DESCRIPTION'    => _t( 'The e-mail address of this user. The address will be used for all mail-based system notifications and alerts to this user.' ),
			'BOUNDS_LENGTH'  => array( 0, 255 )
		),
		'sms_number'       => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'SMS number' ),
			'DESCRIPTION'    => _t( 'Phone number for contact by SMS (text message). The number will be used for all SMS-based system notifications and alerts to this user.' ),
		...
		'entity_id'        => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Related entity (optional)' ),
			'DESCRIPTION'    => _t( 'The entity this user login is associated with.' )
		),
		'vars'             => array(
			'FIELD_TYPE'     => FT_VARS,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => 'User variable storage',
			'DESCRIPTION'    => 'Storage area for user variables'
		),
		'volatile_vars'    => array(
			'FIELD_TYPE'     => FT_VARS,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => 'Volatile user variable storage',
			'DESCRIPTION'    => 'Storage area for user variables of limited size that change often'
		),
		'active'           => array(
			'FIELD_TYPE'     => FT_BIT,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => 1,
			'LABEL'          => _t( 'Account is activated?' ),
			"DESCRIPTION"    => "If checked, indicates user account is active. Only active users are allowed to log into the system.",
		...
		'registered_on'    => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Registered on' ),
			'DESCRIPTION'    => _t( 'Registered on' )
		),
		'confirmed_on'     => array(
			'FIELD_TYPE'     => FT_DATETIME,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Confirmed on' ),
			'DESCRIPTION'    => _t( 'Confirmed on' )
		),
		'confirmation_key' => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Confirmation key' ),
			'DESCRIPTION'    => _t( 'Confirmation key used for email verification.' ),
...
class ca_users extends BaseModel {
	 ...
	 * @access public
	 *
	 * @param integer $pm_user_id User id to load. If you pass a string instead of an integer, the record with a user
	 *                            name matching the string will be loaded.
	 *
	...
	/**
	 * Creates new user record. You must set all required user fields before calling this method. If errors occur you
	 * can use the standard Table class error handling methods to figure out what went wrong.
	 *
	...
	public function insert( $pa_options = null ) {
		...
		try {
			$vs_backend_password = AuthenticationManager::createUserAndGetPassword( $this->get( 'user_name' ),
				$this->get( 'password' ) );
			...
			$this->postError( 925, _t( "Current authentication adapter does not support creating new users." ),
				'ca_users->insert()' );
		...
		if ( is_array( $policies = $auth_config->get( 'password_policies' ) ) && sizeof( $policies ) ) {
			...
			foreach ( $policies as $p ) {
				if ( is_array( $rules = caGetOption( 'rules', $p, null ) ) ) {
					$builder->minPassingRules( caGetOption( 'min_passing_rules', $p, 1 ),
						function ( $b ) use ( $rules ) {
		...
		if ( is_array( $policies = $auth_config->get( 'password_policies' ) ) && sizeof( $policies ) ) {
			...
			foreach ( $policies as $p ) {
				if ( is_array( $rules = caGetOption( 'rules', $p, null ) ) ) {
					...
					$criteria[] = _t( 'conform to at least %1 of the following: %2',
						caGetOption( 'min_passing_rules', $p, 1 ), join( "; ", $group ) );
				} else {
					$group = [];
					foreach ( $p as $k => $v ) {
	...
	static private function _getPasswordPolicyRuleAsText( $rule, $value ) {
		...
		switch ( $rule ) {
			case 'min_length':
				return ( $v == 1 ) ? _t( 'be at least %1 character long', $v )
					: _t( 'be at least %1 characters long', $v );
				break;
			case 'max_length':
				return ( $v == 1 ) ? _t( 'be not longer than %1 character', $v )
					: _t( 'be not longer than %1 characters', $v );
				break;
			case 'upper_case':
				return ( $v == 1 ) ? _t( 'have at least %1 upper-case character', $v )
					: _t( 'have at least %1 upper-case character', $v );
				break;
			case 'lower_case':
				return ( $v == 1 ) ? _t( 'have at least %1 lower-case character', $v )
					: _t( 'have at least %1 lower-case characters', $v );
				break;
			case 'digits':
				return ( $v == 1 ) ? _t( 'have at least %1 digit', $v ) : _t( 'have at least %1 digits', $v );
				break;
			case 'special_characters':
				return ( $v == 1 ) ? _t( 'have at least %1 non-alphanumeric character', $v )
					: _t( 'have at least %1 non-alphanumeric characters', $v );
				break;
	...
	/**
	 * Saves changes to user record. You must make sure all required user fields are set before calling this method. If
	 * errors occur you can use the standard Table class error handling methods to figure out what went wrong.
	 *
	 * Required fields are user_name, password, fname and lname.
	 *
	 * If you do not call this method at the end of your request changed user vars will not be saved! If you are also
	 * using the Auth class, the Auth->close() method will call this for you.
	 *
	 * @access public
	 * @return bool Returns true if no error, false if error occurred
	 */
	public function update( $pa_options = null ) {
		...
		if ( $this->changed( 'password' ) ) {
			if ( ! self::applyPasswordPolicy( $this->get( 'password' ) ) ) {
				$this->postError( 922, _t( "Password must %1", self::getPasswordPolicyAsText() ),
					'ca_users->update()' );
			...
			try {
				$vs_backend_password = AuthenticationManager::updatePassword( $this->get( 'user_name' ),
					$this->get( 'password' ) );
				$this->set( 'password', $vs_backend_password );
	...
	/**
	 * Deletes user. Unlike standard model rows, ca_users rows should never actually be deleted because they need to
	 * exist for logging purposes. So this version of delete() marks the row as deleted by setting ca_users.userclass =
	 * 255 and *not* invoking to BaseModel::delete()
	 *
	 * @access public
	 * @return bool Returns true if no error, false if error occurred
	 */
	...
	public function set( $pa_fields, $pm_value = "", $pa_options = null ) {
		...
		if ( $this->getPrimaryKey() > 0 ) {
			if ( isset( $pa_fields['password'] )
			     && ! AuthenticationManager::supports( __CA_AUTH_ADAPTER_FEATURE_UPDATE_PASSWORDS__ )
			) {
				$this->postError( 922, _t( "Authentication back-end doesn't updating passwords of existing users." ),
					'ca_users->update()' );
	...
	public function getUserNameFormattedForLookup() {
		...
		return caProcessTemplate( join( $this->getAppConfig()->getList( 'ca_users_lookup_delimiter' ),
			$this->getAppConfig()->getList( 'ca_users_lookup_settings' ) ), $va_values, array() );
	}
	# ----------------------------------------
	# --- User variables
	# ----------------------------------------
	 ...
	 *
	 * Changes to user variables are saved when the insert() (for new user records) or update() (for existing user
	 * records) method is called. If you do not call either of these any changes will be lost when the request
	 * completes.
	 *
	 * @access public
	 *
	 ...
	 *                           - ENTITY_ENCODE_INPUT = Convert all "special" HTML characters in variable value to
	 *                           entities; default is true
	 *                           - URL_ENCODE_INPUT = Url encodes variable value; default is  false
	 *                           - volatile = Places value in "volatile" variable storage, which is usually faster.
	 *                           Only store small values, not large blocks of text or binary data, that are expected to
	 *                           frequently as volatile.
	 *
	...
	public function setVar( $ps_key, $pm_val, $pa_options = null ) {
		...
		if ( $ps_key ) {
			...
			(
				...
				|| ( ! is_array( $vs_proc_val ) && is_array( $va_vars[ $ps_key ] ) )
				|| ( is_array( $vs_proc_val ) && ( is_array( $va_vars[ $ps_key ] ) )
				     && ( sizeof( $vs_proc_val ) != sizeof( $va_vars[ $ps_key ] ) ) )
	...
	/**
	 * Deletes user variable. Once deleted, you must call insert() (for new user records) or update() (for existing
	 * user records) to make the deletion permanent.
	 *
	 * @access public
	 *
	 * @param string $ps_key Name of user variable
	...
	public function getVar( $ps_key ) {
		$this->clearErrors();
		if ( isset( $this->opa_user_vars[ $ps_key ] ) ) {
			return ( is_array( $this->opa_user_vars[ $ps_key ] ) ) ? $this->opa_user_vars[ $ps_key ]
				: stripSlashes( $this->opa_user_vars[ $ps_key ] );
		} else {
			if ( isset( $this->opa_volatile_user_vars[ $ps_key ] ) ) {
				return ( is_array( $this->opa_volatile_user_vars[ $ps_key ] ) )
					? $this->opa_volatile_user_vars[ $ps_key ]
					: stripSlashes( $this->opa_volatile_user_vars[ $ps_key ] );
			}
		}
	...
	public function getUserList( $pa_options = null ) {
		...
		while ( $qr_users->nextRow() ) {
			...
			$va_users[ $qr_users->get( 'user_id' ) ] = array_merge( $qr_users->getRow(),
				array( 'last_login' => $va_vars['last_login'] ) );
		}
	...
	public function userListAsHTMLFormElement( $pa_options = null ) {
		...
		$va_selected = ( isset( $pa_options['selected'] ) && is_array( $pa_options['selected'] ) )
			? $pa_options['selected'] : array();
		...
		if ( sizeof( $va_users ) ) {
			...
			foreach ( $va_users as $vn_user_id => $va_user_info ) {
				...
				$vs_buf   .= "<option value='{$vn_user_id}' {$SELECTED}>" . $va_user_info['fname'] . ' '
				             . $va_user_info['lname'] . ( $va_user_info['email'] ? " (" . $va_user_info['email'] . ")"
						: "" ) . "</option>\n";
			}
			$vs_buf .= "</select>\n";
		}
	 ...
	 *
	 * @return integer Returns number of roles added or false if there was an error. The number of roles added will not
	 *                 necessarily match the number of roles you tried to add. If you try to add the same role twice,
	 *                 or to add a role that already exists for this user, addRoles() will silently ignore it.
	 */
		...
		if ( $pn_user_id = $this->getPrimaryKey() ) {
			...
			foreach ( $pm_roles as $vs_role ) {
				...
				if ( $o_db->numErrors() == 0 ) {
					...
					$this->postError( 930,
						_t( "Database error adding role '%1': %2", $vs_role, join( ';', $o_db->getErrors() ) ),
						"User->addRoles()" );
				}
			}
	 ...
	 * @access public
	 *
	 * @param mixed $pm_roles Single role or list (array) of roles to remove. Roles may be specified by name, code or
	 *                        id.
	 *
	...
	public function removeRoles( $pm_roles ) {
		...
		if ( $pn_user_id = $this->getPrimaryKey() ) {
			...
			if ( sizeof( $va_role_ids ) > 0 ) {
				...
				if ( $o_db->numErrors() ) {
					$this->postError( 931, _t( "Database error: %1", join( ';', $o_db->getErrors() ) ),
						"User->removeRoles()" );
	 ...
	 *
	 * @access public
	 * @return integer Returns associative array of roles. Key is role id, value is array containing information about
	 *                 the role.
	 *
		...
		if ( sizeof( $va_roles ) ) {
			...
			foreach ( $va_roles as $vn_role_id => $va_role_info ) {
				$SELECTED = ( isset( $va_user_roles[ $vn_role_id ] ) && $va_user_roles[ $vn_role_id ] ) ? "SELECTED='1'"
					: "";
				$vs_buf   .= "<option value='{$vn_role_id}' {$SELECTED}>" . $va_role_info['name'] . " ["
				             . $va_role_info["code"] . "]</option>\n";
			}
			$vs_buf .= "</select>\n";
		}
		if ( $vs_buf && ( $vs_format = $this->_CONFIG->get( 'form_element_display_format' ) ) ) {
	 ...
	 * @access public
	 *
	 * @param mixed $pm_groups Single group or list (array) of group to add user to. Groups may be specified by name,
	 *                         short name or numeric id.
	 *
	 * @return integer Returns number of groups user was added to or false if there was an error. The number of groups
	 *                 user was added to will not necessarily match the number of groups you passed in $pm_groups. If
	 *                 you try to add the user to the same group twice, or to a group that the user is already a member
	 *                 of, addToGroups() will silently ignore it.
	 */
		...
		if ( $pn_user_id = $this->getPrimaryKey() ) {
			...
			foreach ( $pm_groups as $vs_group ) {
				...
				if ( $o_db->numErrors() == 0 ) {
					...
					$this->postError( 935, _t( "Database error: %1", join( ';', $o_db->getErrors() ) ),
						"User->addToGroups()" );
				}
			}
	 ...
	 * @access public
	 *
	 * @param mixed $pm_groups Single group or list (array) of groups to remove current user from. Groups may be
	 *                         specified by name, short name or id.
	 *
	 * @return bool Returns true on success, false on error.
	 */
	public function removeFromGroups( $pm_groups ) {
		...
		if ( $pn_user_id = $this->getPrimaryKey() ) {
			...
			if ( sizeof( $va_group_ids ) > 0 ) {
				...
				if ( $o_db->numErrors() ) {
					$this->postError( 936, _t( "Database error: %1", join( ';', $o_db->getErrors() ) ),
	 ...
	 * @access public
	 *
	 * @param mixed $ps_group The group to test for the current user for membership in. Group may be specified by name,
	 *                        short name or id.
	 *
	...
	public function groupListAsHTMLFormElement( $pa_options = null ) {
		...
		if ( sizeof( $va_groups ) ) {
			...
			foreach ( $va_groups as $vn_group_id => $va_group_info ) {
				$SELECTED = ( isset( $va_user_groups[ $vn_group_id ] ) && $va_user_groups[ $vn_group_id ] )
					? "SELECTED='1'" : "";
				$vs_buf   .= "<option value='{$vn_group_id}' {$SELECTED}>" . $va_group_info['name'] . " ["
				             . $va_group_info["code"] . "]</option>\n";
			}
			$vs_buf .= "</select>\n";
		}
		if ( $vs_buf && ( $vs_format = $this->_CONFIG->get( 'form_element_display_format' ) ) ) {
	...
	public function getPreference( $ps_pref ) {
		if ( $this->isValidPreference( $ps_pref ) ) {
			...
			if ( isset( $va_prefs[ $ps_pref ] ) ) {
				return ( ! is_null( $va_prefs[ $ps_pref ] ) ) ? $va_prefs[ $ps_pref ]
					: $this->getPreferenceDefault( $ps_pref );
			}
	...
	public function getPreferenceDefault( $ps_pref, $pa_options = null ) {
		...
		switch ( $va_pref_info["formatType"] ) {
			...
			case 'FT_IMPORT_EXPORT_MAPPING_GROUP_EDITOR_UI':
				$vn_type_id   = ( is_array( $pa_options ) && isset( $pa_options['type_id'] )
				                  && (int) $pa_options['type_id'] ) ? (int) $pa_options['type_id'] : null;
	 ...
	 * @param bool   $pb_post_errors If true, invalid parameter causes errors to be thrown; if false, error messages
	 *                               are supressed. Default is false.
	 *
	...
	public function isValidPreferenceValue( $ps_pref, $ps_value, $pb_post_errors = false ) {
		if ( $this->isValidPreference( $ps_pref ) ) {
			...
			if ( is_array( $ps_value ) && isset( $va_pref_info["picks"] ) ) {
				if ( ! ( ( sizeof( $ps_value ) >= $va_pref_info["picks"]["minimum"] )
				         && ( sizeof( $ps_value ) <= $va_pref_info["picks"]["maximum"] ) )
				) {
					if ( $pb_post_errors ) {
						if ( $va_pref_info["picks"]["minimum"] < $va_pref_info["picks"]["maximum"] ) {
							$this->postError( 921, _t( "You must select between %1 and %2 choices for %3",
								$va_pref_info["picks"]["minimum"], $va_pref_info["picks"]["maximum"],
								$va_pref_info["label"] ), "User->isValidPreferenceValue()" );
						} else {
							$this->postError( 921,
								_t( "You must select %1 choices for %2", $va_pref_info["picks"]["minimum"],
									$va_pref_info["label"] ), "User->isValidPreferenceValue()" );
						}
					}
				...
				if ( is_array( $ps_value ) ) {
					foreach ( $ps_value as $vs_value ) {
						if ( ! in_array( $vs_value, array_values( $va_pref_info["choiceList"] ) ) ) {
							if ( $pb_post_errors ) {
								$this->postError( 921,
									_t( "%1 is not a valid value for %2", $vs_value, $va_pref_info["label"] ),
									"User->isValidPreferenceValue()" );
							}
					...
					if ( ! in_array( $ps_value, array_values( $va_pref_info["choiceList"] ) ) ) {
						if ( $pb_post_errors ) {
							$this->postError( 921,
								_t( "%1 is not a valid value for %2", $ps_value, $va_pref_info["label"] ),
								"User->isValidPreferenceValue()" );
						}
					...
					if ( isset( $va_pref_info["value"] ) && is_array( $va_pref_info["value"] ) ) {
						...
						if ( strlen( $va_pref_info["value"]["minimum"] ) && ( $va_pref_info["value"]["maximum"] ) ) {
							if ( ! ( ( $ps_value >= $va_pref_info["value"]["minimum"] )
							         && ( $ps_value <= $va_pref_info["value"]["maximum"] ) )
							) {
								if ( $pb_post_errors ) {
									$this->postError( 921,
										_t( "Value for %1 must be between %2 and %3", $va_pref_info["label"],
											$va_pref_info["value"]["minimum"], $va_pref_info["value"]["maximum"] ),
										"User->isValidPreferenceValue()" );
								}
							...
							if ( strlen( $va_pref_info["value"]["minimum"] ) ) {
								if ( $ps_value < $va_pref_info["value"]["minimum"] ) {
									if ( $pb_post_errors ) {
										if ( $va_pref_info["value"]["minimum"] == 1 ) {
											$this->postError( 921, _t( "%1 must be set", $va_pref_info["label"],
												$va_pref_info["value"]["minimum"], $va_pref_info["value"]["maximum"] ),
												"User->isValidPreferenceValue()" );
										} else {
											$this->postError( 921,
												_t( "Value for %1 must be greater than %2", $va_pref_info["label"],
													$va_pref_info["value"]["minimum"] ),
												"User->isValidPreferenceValue()" );
										}
									}
								...
								if ( $ps_value > $va_pref_info["value"]["maximum"] ) {
									if ( $pb_post_errors ) {
										$this->postError( 921,
											_t( "Value for %1 must be less than %2", $va_pref_info["label"],
												$va_pref_info["value"]["maximum"] ), "User->isValidPreferenceValue()" );
									}
				...
				case 'FT_TEXT':
					...
					if ( isset( $va_pref_info["length"] ) && is_array( $va_pref_info["length"] ) ) {
						...
						if ( strlen( $va_pref_info["length"]["minimum"] ) && ( $va_pref_info["length"]["maximum"] ) ) {
							if ( ! ( ( strlen( $ps_value ) >= $va_pref_info["length"]["minimum"] )
							         && ( strlen( $ps_value ) <= $va_pref_info["length"]["maximum"] ) )
							) {
								if ( $pb_post_errors ) {
									$this->postError( 921,
										_t( "Value for %1 must be between %2 and %3 characters", $va_pref_info["label"],
											$va_pref_info["length"]["minimum"], $va_pref_info["length"]["maximum"] ),
										"User->isValidPreferenceValue()" );
								}
							...
							if ( strlen( $va_pref_info["length"]["minimum"] ) ) {
								if ( $ps_value < $va_pref_info["length"]["minimum"] ) {
									if ( $pb_post_errors ) {
										if ( $va_pref_info["length"]["minimum"] == 1 ) {
											$this->postError( 921, _t( "%1 must be set", $va_pref_info["label"],
												$va_pref_info["length"]["minimum"],
												$va_pref_info["length"]["maximum"] ),
												"User->isValidPreferenceValue()" );
										} else {
											$this->postError( 921,
												_t( "Value for %1 must be greater than %2 characters",
													$va_pref_info["label"], $va_pref_info["length"]["minimum"] ),
												"User->isValidPreferenceValue()" );
										}
									}
								...
								if ( $ps_value > $va_pref_info["length"]["maximum"] ) {
									if ( $pb_post_errors ) {
										$this->postError( 921,
											_t( "Value for %1 must be less than %2 characters", $va_pref_info["label"],
												$va_pref_info["length"]["maximum"] ),
											"User->isValidPreferenceValue()" );
									}
					...
					if ( is_array( $ps_value ) ) {
						foreach ( $ps_value as $vn_type_id => $vn_ui_id ) {
							if ( ! isset( $va_valid_uis[ $vn_type_id ][ $vn_ui_id ] ) ) {
								if ( $t_instance
								     && (bool) $t_instance->getFieldInfo( $t_instance->getTypeFieldName(), 'IS_NULL' )
								     && ( $vn_type_id === '_NONE_' )
								) {
									return true;
								}
	 ...
	 * By calling this method for a series of preference names, one can quickly create an HTML-based configuration
	 * form.
	 *
	 * @access public
	 *
	 * @param string $ps_pref    Name of user preference
	 * @param string $ps_format  Format string containing simple tags to be replaced with preference information. Tags
	 *                           supported are:
	 ...
	 *                           useTable = if true and displayType for element in DT_CHECKBOXES checkboxes will be
	 *                           formatted in a table with numTableColumns columns numTableColumns = Number of columns
	 *                           to use when formatting checkboxes as a table. Default, if omitted, is 3 genericUIList
	 *                           = forces FT_*_EDITOR_UI to return single UI list for table rather than by type
	...
	public function preferenceHtmlFormElement( $ps_pref, $ps_format = null, $pa_options = null ) {
		if ( $this->isValidPreference( $ps_pref ) ) {
			...
			switch ( $va_pref_info["displayType"] ) {
				...
				case 'DT_FIELD':
					...
					if ( $vn_display_height > 1 ) {
						$vs_output = "<textarea name='pref_$ps_pref' rows='" . $vn_display_height . "' cols='"
						             . $vn_display_width . "'>" . htmlspecialchars( $vs_current_value, ENT_QUOTES,
								'UTF-8' ) . "</textarea>\n";
					} else {
						$vs_output
							= "<input type='text' name='pref_$ps_pref' size='$vn_display_width' maxlength='$vn_max_input_length'"
					...
					switch ( $va_pref_info['formatType'] ) {
						case 'FT_UI_LOCALE':
							...
							foreach ( $va_locales as $vs_code => $va_parts ) {
								if ( is_array( $va_restrict_to_ui_locales ) && sizeof( $va_restrict_to_ui_locales )
								     && ! in_array( $vs_code, $va_restrict_to_ui_locales )
								) {
									continue;
								}
								try {
									$vs_lang_name    = Zend_Locale::getTranslation( strtolower( $va_parts[0] ),
										'language', strtolower( $va_parts[0] ) );
								...
								$va_opts[ ( $vs_lang_name ? $vs_lang_name : $vs_code ) . ( $vs_country_name ? ' ('
								                                                                              . $vs_country_name
								                                                                              . ')'
									: '' ) ]
									= $vs_code;
						...
						case 'FT_LOCALE':
							...
							while ( $qr_locales->nextRow() ) {
								$va_opts[ $qr_locales->get( 'name' ) ] = $qr_locales->get( 'language' ) . '_'
								                                         . $qr_locales->get( 'country' );
							}
							break;
						case 'FT_THEME':
							if ( $r_dir = opendir( $this->_CONFIG->get( 'themes_directory' ) ) ) {
								...
								while ( ( $vs_theme_dir = readdir( $r_dir ) ) !== false ) {
									...
									$o_theme_info                            = Configuration::load( $this->_CONFIG->get( 'themes_directory' )
									                                                                . '/'
									                                                                . $vs_theme_dir
									                                                                . '/themeInfo.conf' );
						...
						case 'FT_IMPORT_EXPORT_MAPPING_GROUP_EDITOR_UI':
							...
							if ( method_exists( $t_instance, 'getTypeFieldName' ) && ( $t_instance->getTypeFieldName() )
							     && ( ! isset( $pa_options['genericUIList'] ) || ! $pa_options['genericUIList'] )
							) {
								...
								if ( (bool) $t_instance->getFieldInfo( $t_instance->getTypeFieldName(), 'IS_NULL' ) ) {
									$va_types['_NONE_'] = array(
										'LEVEL'         => 0,
										'name_singular' => _t( 'NONE' ),
										'name_plural'   => _t( 'NONE' )
									);
								}
								...
								foreach ( $va_types as $vn_type_id => $va_type ) {
									...
									if ( is_array( $va_ui_list_by_type[ $vn_type_id ] ) ) {
										foreach (
											caExtractValuesByUserLocale( $va_ui_list_by_type[ $vn_type_id ] ) as
											$vn_ui_id => $vs_label
										) {
									...
									if ( is_array( $va_ui_list_by_type['__all__'] ) ) {
										foreach (
											caExtractValuesByUserLocale( $va_ui_list_by_type['__all__'] ) as $vn_ui_id
										=> $vs_label
										) {
											$va_opts[ $vn_ui_id ] = $vs_label;
										}
									...
									$vs_output .= "<tr><td>" . str_repeat( "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;",
											(int) $va_type['LEVEL'] ) . $va_type['name_singular']
									              . "</td><td><select name='pref_{$ps_pref}_{$vn_type_id}'>\n";
									foreach ( $va_opts as $vs_val => $vs_opt ) {
										$vs_selected = ( $vs_val == $va_values[ $vn_type_id ] ) ? "SELECTED" : "";
										$vs_output   .= "<option value='" . htmlspecialchars( $vs_val, ENT_QUOTES,
												'UTF-8' ) . "' {$vs_selected}>{$vs_opt}</option>\n";
									}
								...
								foreach ( $va_opts as $vs_val => $vs_opt ) {
									...
									$vs_output   .= "<option value='" . htmlspecialchars( $vs_val, ENT_QUOTES, 'UTF-8' )
									                . "' $vs_selected>" . $vs_opt . "</option>\n";
								}
								$vs_output .= "</select></td></tr>\n";
							}
				...
				case 'DT_CHECKBOXES':
					if ( $va_pref_info["formatType"] == 'FT_BIT' ) {
						...
						$vs_output   .= "<input type='checkbox' name='pref_$ps_pref' value='1'" . $vs_class
						                . " $vs_selected>\n";
					} else {
						if ( $vb_use_table = ( isset( $pa_options['useTable'] ) && (bool) $pa_options['useTable'] ) ) {
							$vs_output .= "<table width='100%'>";
						}
						$vn_num_table_columns = ( isset( $pa_options['numTableColumns'] )
						                          && ( (int) $pa_options['numTableColumns'] > 0 ) )
							? (int) $pa_options['numTableColumns'] : 3;
						...
						foreach ( $va_pref_info["choiceList"] as $vs_opt => $vs_val ) {
							...
							$vs_output .= "<input type='checkbox' name='pref_" . $ps_pref . "[]' value='"
							              . htmlspecialchars( $vs_val, ENT_QUOTES, 'UTF-8' ) . "'" . $vs_class
							              . " $vs_selected> " . $vs_opt . " \n";
				...
				case 'DT_STATEPROV_LIST':
					$vs_output .= caHTMLSelect( "pref_{$ps_pref}_select", array(),
						array( 'id' => "pref_{$ps_pref}_select", 'class' => $vs_classname ),
						array( 'value' => $vs_current_value ) );
					$vs_output .= caHTMLTextInput( "pref_{$ps_pref}_name", array(
						'id'    => "pref_{$ps_pref}_text",
						'value' => $vs_current_value,
						'class' => $vs_classname
					) );
				...
				case 'DT_COUNTRY_LIST':
					$vs_output .= caHTMLSelect( "pref_{$ps_pref}", caGetCountryList(),
						array( 'id' => "pref_{$ps_pref}", 'class' => $vs_classname ),
						array( 'value' => $vs_current_value ) );
					...
					if ( $va_pref_info['stateProvPref'] ) {
						...
						$vs_output .= "
							jQuery('#pref_{$ps_pref}').click({countryID: 'pref_{$ps_pref}', stateProvID: 'pref_"
						              . $va_pref_info['stateProvPref'] . "', value: '"
						              . addslashes( $this->getPreference( $va_pref_info['stateProvPref'] ) ) . "', statesByCountryList: caStatesByCountryList}, caUI.utils.updateStateProvinceForCountry);
							jQuery(document).ready(function() {
								caUI.utils.updateStateProvinceForCountry({data: {countryID: 'pref_{$ps_pref}', stateProvID: 'pref_"
						              . $va_pref_info['stateProvPref'] . "', value: '"
						              . addslashes( $this->getPreference( $va_pref_info['stateProvPref'] ) ) . "', statesByCountryList: caStatesByCountryList}});
							});
						";
				...
				# ---------------------------------
				case 'DT_CURRENCIES':
					$vs_output .= caHTMLSelect( "pref_{$ps_pref}", caAvailableCurrenciesForConversion(),
						array( 'id' => "pref_{$ps_pref}", 'class' => $vs_classname ),
						array( 'value' => $vs_current_value ) );
					break;
				# ---------------------------------
				case 'DT_RADIO_BUTTONS':
					foreach ( $va_pref_info["choiceList"] as $vs_opt => $vs_val ) {
						...
						$vs_output   .= "<input type='radio' name='pref_$ps_pref'" . $vs_class . " value='"
						                . htmlspecialchars( $vs_val, ENT_QUOTES, 'UTF-8' ) . "' $vs_selected> "
						                . $vs_opt . " \n";
					}
					break;
				# ---------------------------------
				case 'DT_PASSWORD':
					...
					$vs_output
						= "<input type='password' name='pref_$ps_pref' size='$vn_display_width' maxlength='$vn_max_input_length'"
						  . $vs_class . " value='" . htmlspecialchars( $vs_current_value, ENT_QUOTES, 'UTF-8' )
						  . "'/>\n";
			...
			if ( preg_match( "/\^DESCRIPTION/", $vs_format ) ) {
				...
				$vs_format   = str_replace( "^LABEL",
					'<span id="' . $vs_field_id . '">' . _t( $va_pref_info["label"] ) . '</span>', $vs_format );

				TooltipManager::add( '#' . $vs_field_id,
					"<h3>" . $va_pref_info["label"] . "</h3>" . $va_pref_info["description"] );
			}
	...
	public function _getUIListByType( $pn_table_num ) {
		...
		while ( $qr_uis->nextRow() ) {
			...
			$va_ui_list_by_type[ $vn_type_id ][ $qr_uis->get( 'ui_id' ) ][ $qr_uis->get( 'locale_id' ) ]
				= $qr_uis->get( 'name' );
		}
	 ...
	 *
	 * @param mixed  $pm_table_name_or_num Table name or number of search target (eg. ca_objects or 57 for an object
	 *                                     search)
	 * @param string $ps_type              A search type descriptor. This is just a string used to distinguish
	 *                                     different types of searches (eg. basic vs. advanced) and is typically set to
	 *                                     the "find_type" property value set in subclasses of BaseSearchController
	 * @param array  $pa_search            An array containing actual search parameters. For a basic search, this will
	 *                                     have one key: "search"; for advanced searches it will have keys for all form
	 *                                     values
	 *
	...
	/**
	 * Removes all searches for the specified table and, if specified, search type. If both parameters are omitted then
	 * all saved searches for all search targets are removed.
	 *
	 * @param mixed  $pm_table_name_or_num Optional table name or number of search target (eg. ca_objects or 57 for an
	 *                                     object search)
	 * @param string $ps_type              Optional search type descriptor. This is just a string used to distinguish
	 *                                     different types of searches (eg. basic vs. advanced) and is typically set to
	 *                                     the "find_type" property value set in subclasses of BaseSearchController
	 *
	 ...
	 * @param string $ps_key               The 32 character md5 hash key for the saved search
	 *
	 * @return array An array containing the search parameters + 2 special entries: (1) _label is a display label for
	 *               the search (2) _form_id is the ca_search_forms.form_id for the search, if the search was
	 *               form-based. _form_id will be undefined if the search was basic (eg. simple one-entry text search)
	 */
	public function getSavedSearchByKey( $pm_table_name_or_num, $ps_type, $ps_key ) {
		...
		return is_array( $va_searches[ $vn_table_num ][ strtolower( $ps_type ) ][ $ps_key ] )
			? $va_searches[ $vn_table_num ][ strtolower( $ps_type ) ][ $ps_key ] : array();
	}
	# ----------------------------------------
	 ...
	 *                                     the "find_type" property value set in subclasses of BaseSearchController
	 *
	 * @return array An array of saved searches, or an empty array if no searches have been saved. The array's keys are
	 *               32 character md5 saved search keys. The values are arrays with the search parameters + 2 special
	 *               entries: (1) _label is a display label for the search (2) _form_id is the ca_search_forms.form_id
	 *               for the search, if the search was form-based. _form_id will be undefined if the search was basic
	 *               (eg. simple one-entry text search)
	 */
	public function getSavedSearches( $pm_table_name_or_num, $ps_type ) {
		...
		return is_array( $va_searches[ $vn_table_num ][ strtolower( $ps_type ) ] )
	...
	public function setLastLogout( $pn_time ) {
		$this->setVar( $this->getAppConfig()->get( "app_name" ) . "_previous_to_last_logout", $this->getLastLogout(),
			array( 'volatile' => true ) );
		$this->setVar( $this->getAppConfig()->get( "app_name" ) . "_last_logout", $pn_time,
			array( 'volatile' => true ) );
	}
	# ----------------------------------------
	...
	private function passwordResetDeactivateAccount() {
		...
		$this->opo_log->log( array(
			'CODE'    => 'SYS',
			'SOURCE'  => 'ca_users/passwordResetDeactivateAccount',
			'MESSAGE' => _t( 'User %1 was permanently deactivated because the maximum number of consecutive unsuccessful password reset attemps was reached.',
				$this->get( 'user_name' ) )
		) );
	...
	public function authenticate( &$ps_username, $ps_password = "", $pa_options = null ) {
		...
		if ( $vs_rewrite_username_with_regex = $this->opo_auth_config->get( 'rewrite_username_with_regex' ) ) {
			...
			$vs_username                  = preg_replace( "!" . preg_quote( $vs_rewrite_username_with_regex, "!" )
			                                              . "!", $vs_rewrite_username_to_regex, $vs_username );
		}
		...
		if ( strlen( $vs_username ) > 0 && ! $this->load( $vs_username ) ) {
			if ( AuthenticationManager::supports( __CA_AUTH_ADAPTER_FEATURE_AUTOCREATE_USERS__ ) ) {
				try {
					...
					$this->opo_log->log( array(
						'CODE'    => 'SYS',
						'SOURCE'  => 'ca_users/authenticate',
						'MESSAGE' => _t( 'There was an error while trying to fetch information for a new user from the current authentication backend. The message was %1 : %2',
				...
				if ( ! $this->getPrimaryKey() ) {
					$this->opo_log->log( array(
						...
						'SOURCE'  => 'ca_users/authenticate',
						'MESSAGE' => $err
							= _t( 'User could not be created after getting info from authentication adapter: %1',
							join( " ", $this->getErrors() ) )
					) );
		...
		if ( $vs_username ) {
			try {
					...
					if ( $this->load( $vs_username ) ) {
						if (
							...
							$this->opo_log->log( array(
								'CODE'    => 'SYS',
								'SOURCE'  => 'ca_users/authenticate',
								'MESSAGE' => _t( 'There was an error while trying to authenticate user %1: User is not authorized to log into Pawtucket',
									$vs_username )
							) );
						...
						$this->opo_log->log( array(
							'CODE'    => 'SYS',
							'SOURCE'  => 'ca_users/authenticate',
							'MESSAGE' => _t( 'There was an error while trying to authenticate user %1: Load by user name failed',
								$vs_username )
						) );
				...
				$this->opo_log->log( array(
					'CODE'    => 'SYS',
					'SOURCE'  => 'ca_users/authenticate',
					'MESSAGE' => _t( 'There was an error while trying to authenticate user %1: The message was %2 : %3',
						$ps_username, get_class( $e ), $e->getMessage() )
				) );
	 ...
	 * @access public
	 *
	 * @param string IP address to authenticate. If it is omitted, the current client ip (taken from the REMOTE_ADDR
	 *                  environment variable) is used.
	 *
	 * @return bool True if ip is in the database (also loads the ip record into the instance); false if ip does not
	...
	public function ipAuthenticate( $ip = "" ) {
		...
		if ( sizeof( $ipp ) == 4 ) {
			...
			for ( $i = 0; $i < 4; $i ++ ) {
				if ( $i == 3 ) {
					$chk[] = "((ip4s <= " . $ipp[ $i ] . " AND ip4e >= " . $ipp[ $i ]
					         . ") OR (ip4s IS NULL AND ip4e IS NULL))";
				} else {
					$chk[] = "(ip" . ( $i + 1 ) . " = " . $ipp[ $i ] . ")";
				}
			}
	...
	# ----------------------------------------
	/**
	 * Checks if user is allowed to perform the specified action (possible actions are defined in
	 * app/conf/user_actions.conf) Returns true if user can do action, false otherwise.
	 */
	...
	public function getBundleAccessLevel( $ps_table_name, $ps_bundle_name ) {
		...
		if ( in_array( $ps_table_name,
			ca_users::$s_bundlable_tables )
		) { // bundle-level access control only applies to these tables
			...
			foreach ( $va_roles as $vn_role_id => $va_role_info ) {
				...
				if ( is_array( $va_vars['bundle_access_settings'] ) ) {
					if ( isset( $va_vars['bundle_access_settings'][ $ps_table_name . '.' . $ps_bundle_name ] )
					     && ( (int) $va_vars['bundle_access_settings'][ $ps_table_name . '.' . $ps_bundle_name ]
					          > $vn_access )
					) {
						...
						if ( isset( $va_vars['bundle_access_settings'][ $ps_table_name . '.ca_attribute_'
						                                                . $ps_bundle_name ] )
						     && ( (int) $va_vars['bundle_access_settings'][ $ps_table_name . '.ca_attribute_'
						                                                    . $ps_bundle_name ] > $vn_access )
						) {
							$vn_access = (int) $va_vars['bundle_access_settings'][ $ps_table_name . '.ca_attribute_'
							                                                       . $ps_bundle_name ];
	...
	public function getTypeAccessLevel( $ps_table_name, $pm_type_code_or_id ) {
		...
		if ( in_array( $ps_table_name,
			ca_users::$s_bundlable_tables )
		) { // type-level access control only applies to these tables
			$va_roles = array_merge( $this->getUserRoles(), $this->getGroupRoles() );
			...
			foreach ( $va_roles as $vn_role_id => $va_role_info ) {
				...
				if ( is_array( $va_vars['type_access_settings'] ) ) {
					if ( isset( $va_vars['type_access_settings'][ $ps_table_name . '.' . $vn_type_id ] )
					     && ( (int) $va_vars['type_access_settings'][ $ps_table_name . '.' . $vn_type_id ]
					          > $vn_access )
					) {
			...
			ca_users::$s_user_type_access_cache[ $ps_table_name . '/' . $vn_type_id . "/" . $this->getPrimaryKey() ]
				= ca_users::$s_user_type_access_cache[ $vs_cache_key ] = $vn_access;

			return $vn_access;
		} else {
			// no type level access control for tables not explicitly listed in $s_bundlable_tables
			ca_users::$s_user_type_access_cache[ $ps_table_name . '/' . $vn_type_id . "/" . $this->getPrimaryKey() ]
				= ca_users::$s_user_type_access_cache[ $vs_cache_key ] = __CA_BUNDLE_ACCESS_EDIT__;
		...
		foreach ( $va_roles as $vn_role_id => $va_role_info ) {
			...
			if ( is_array( $va_available_types ) ) {
				foreach ( $va_available_types as $vn_type_id ) {
					...
					if ( ( $vb_exact && ( $vn_access == $pn_access ) )
					     || ( ! $vb_exact
					          && ( $vn_access >= $pn_access ) )
					) {
	...
	public function getSourceAccessLevel( $ps_table_name, $pm_source_code_or_id ) {
		...
		if ( in_array( $ps_table_name,
			...
			if ( is_numeric( $pm_source_code_or_id ) ) {
				...
				$t_list       = new ca_lists();
				$t_instance   = Datamodel::getInstanceByTableName( $ps_table_name, true );
				$vn_source_id = (int) $t_list->getItemIDFromList( $t_instance->getSourceListCode(),
			...
			foreach ( $va_roles as $vn_role_id => $va_role_info ) {
				...
				if ( is_array( $va_vars['source_access_settings'] ) ) {
					if ( isset( $va_vars['source_access_settings'][ $ps_table_name . '.' . $vn_source_id ] )
					     && ( (int) $va_vars['source_access_settings'][ $ps_table_name . '.' . $vn_source_id ]
					          > $vn_access )
					) {
			...
			ca_users::$s_user_source_access_cache[ $ps_table_name . '/' . $vn_source_id . "/" . $this->getPrimaryKey() ]
				= ca_users::$s_user_source_access_cache[ $vs_cache_key ] = $vn_access;

			return $vn_access;
		} else {
			// no source level access control for tables not explicitly listed in $s_bundlable_tables
			ca_users::$s_user_source_access_cache[ $ps_table_name . '/' . $vn_source_id . "/" . $this->getPrimaryKey() ]
				= ca_users::$s_user_source_access_cache[ $vs_cache_key ] = __CA_BUNDLE_ACCESS_EDIT__;
	...
	public function getAccessStatuses( $pn_access_level = null ) {
		...
		$va_item_values = ca_lists::itemIDsToItemValues( array_keys( $va_access_by_item_id ),
			array( 'transaction' => $this->getTransaction() ) );
