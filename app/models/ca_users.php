...
BaseModel::$s_ca_models_definitions['ca_users'] = array(
    ...
    'FIELDS' => array(
        ...
        'user_name' => array(
            ...
            'LABEL' => _t('User name'),
            'DESCRIPTION' => _t(
                'The login name for this user. This name is used in combination with the password set below to access the system.'
            ),
        ...
        'userclass' => array(
            ...
            'LABEL' => _t('User class'),
            'DESCRIPTION' => _t(
                '"Full" user accounts may log into all CollectiveAccess interfaces. "Public" user accounts may only log into the publicly accessible front-end system (if one exists). "Deleted" users may not log into any interface – the account is considered removed.'
            ),
        ...
        'password' => array(
            ...
            'LABEL' => _t('Password'),
            'DESCRIPTION' => _t(
                'The login password for this user. Passwords must be at least 4 characters and should ideally contain a combination of letters and numbers. Passwords are case-sensitive.'
            ),
        ...
        'sms_number' => array(
            ...
            'LABEL' => _t('SMS number'),
            'DESCRIPTION' => _t(
                'Phone number for contact by SMS (text message). The number will be used for all SMS-based system notifications and alerts to this user.'
            ),
...
class ca_users extends BaseModel
{
     ...
     */
    static public $s_user_role_cache = [];
    static public $s_user_group_cache = [];
    static public $s_group_role_cache = [];
    static public $s_user_type_access_cache = [];
    static public $s_user_source_access_cache = [];
    static public $s_user_bundle_access_cache = [];
    static public $s_user_action_access_cache = [];
    static public $s_user_type_with_access_cache = [];
    static public $s_user_source_with_access_cache = [];

    /**
     * List of tables that can have bundle- or type-level access control
     */
    static public $s_bundlable_tables = array(
        'ca_objects',
        'ca_object_lots',
        'ca_entities',
        'ca_places',
     ...
     *
     * @access public
     * @param int $pm_user_id User id to load. If you pass a string instead of an integer, the record with a user name matching the string will be loaded.
    ...
    public function insert($pa_options = null)
    {
        ...
        try {
            $vs_backend_password = AuthenticationManager::createUserAndGetPassword(
                $this->get('user_name'),
                $this->get('password')
            );
            ...
            $this->postError(
                925,
                _t("Current authentication adapter does not support creating new users."),
                'ca_users->insert()'
            );
            return false;
        ...
        if (is_array($policies = $auth_config->get('password_policies')) && sizeof($policies)) {
            ...
            foreach ($policies as $p) {
                if (is_array($rules = caGetOption('rules', $p, null))) {
                    $builder->minPassingRules(
                        caGetOption('min_passing_rules', $p, 1),
        ...
        if (is_array($policies = $auth_config->get('password_policies')) && sizeof($policies)) {
            ...
            foreach ($policies as $p) {
                if (is_array($rules = caGetOption('rules', $p, null))) {
                    ...
                    $criteria[] = _t(
                        'conform to at least %1 of the following: %2',
                        caGetOption('min_passing_rules', $p, 1),
                        join("; ", $group)
                    );
                } else {
            ...
            return caMakeCommaListWithConjunction(
                array_filter(
                    $criteria,
                    function ($v) {
                        return strlen($v);
    ...
    private static function _getPasswordPolicyRuleAsText($rule, $value)
    {
        ...
        switch ($rule) {
            case 'min_length':
                ...
                break;
            case 'max_length':
                return ($v == 1) ? _t('be not longer than %1 character', $v) : _t(
                    'be not longer than %1 characters',
                    $v
                );
                break;
            case 'upper_case':
                return ($v == 1) ? _t('have at least %1 upper-case character', $v) : _t(
                    'have at least %1 upper-case character',
                    $v
                );
                break;
            case 'lower_case':
                return ($v == 1) ? _t('have at least %1 lower-case character', $v) : _t(
                    'have at least %1 lower-case characters',
                    $v
                );
                break;
            case 'digits':
            ...
            case 'special_characters':
                return ($v == 1) ? _t('have at least %1 non-alphanumeric character', $v) : _t(
    ...
    public function update($pa_options = null)
    {
        ...
        if ($this->changed('password')) {
            ...
            try {
                $vs_backend_password = AuthenticationManager::updatePassword(
                    $this->get('user_name'),
                    $this->get('password')
                );
                $this->set('password', $vs_backend_password);
            ...
            try {
                ...
                $this->opo_log->log(
                    array(
                        ...
                        'MESSAGE' => _t(
                            'Authentication adapter could not delete user. Message was: %1',
                            $e->getMessage()
                        )
                    )
                );
    ...
    public function set($pa_fields, $pm_value = "", $pa_options = null)
    {
        ...
        if ($this->getPrimaryKey() > 0) {
            if (isset($pa_fields['password']) && !AuthenticationManager::supports(
                    __CA_AUTH_ADAPTER_FEATURE_UPDATE_PASSWORDS__
                )) {
                $this->postError(
                    922,
                    _t("Authentication back-end doesn't updating passwords of existing users."),
                    'ca_users->update()'
                );
                return false;
            }
    ...
    public function getUserNameFormattedForLookup()
    {
        ...
        return caProcessTemplate(
            join(
                $this->getAppConfig()->getList('ca_users_lookup_delimiter'),
                $this->getAppConfig()->getList('ca_users_lookup_settings')
            ),
            $va_values,
            array()
        );
    ...
    public function setVar($ps_key, $pm_val, $pa_options = null)
    {
        ...
        if ($ps_key) {
            ...
            (
                ...
                (!is_array($vs_proc_val) && is_array($va_vars[$ps_key]))
                ||
                (is_array($vs_proc_val) && (is_array($va_vars[$ps_key])) && (sizeof($vs_proc_val) != sizeof(
                            $va_vars[$ps_key]
                        )))
                ||
    ...
    public function getVar($ps_key)
    {
        $this->clearErrors();
        if (isset($this->opa_user_vars[$ps_key])) {
            return (is_array($this->opa_user_vars[$ps_key])) ? $this->opa_user_vars[$ps_key] : stripSlashes(
                $this->opa_user_vars[$ps_key]
            );
        } else {
            if (isset($this->opa_volatile_user_vars[$ps_key])) {
                return (is_array(
                    $this->opa_volatile_user_vars[$ps_key]
                )) ? $this->opa_volatile_user_vars[$ps_key] : stripSlashes($this->opa_volatile_user_vars[$ps_key]);
            }
        }
        return null;
    }
    ...
    public function getUserList($pa_options = null)
    {
        ...
        while ($qr_users->nextRow()) {
            ...
            $va_users[$qr_users->get('user_id')] = array_merge(
                $qr_users->getRow(),
    ...
    public function userListAsHTMLFormElement($pa_options = null)
    {
        ...
        $va_selected = (isset($pa_options['selected']) && is_array(
                $pa_options['selected']
     ...
     * @return int Returns number of roles added or false if there was an error. The number of roles added will not necessarily match the number of roles you tried to add. If you try to add the same role twice, or to add a role that already exists for this user, addRoles() will silently ignore it.
     */
        ...
        if ($pn_user_id = $this->getPrimaryKey()) {
            ...
            if (sizeof($va_role_ids) > 0) {
                ...
                if ($o_db->numErrors()) {
                    $this->postError(
                        931,
                        _t("Database error: %1", join(';', $o_db->getErrors())),
                        "User->removeRoles()"
                    );
                    return false;
                } else {
     ...
     *
     * @access public
     * @return int Returns associative array of roles. Key is role id, value is array containing information about the role.
     *
     ...
     * @return int Returns number of groups user was added to or false if there was an error. The number of groups user was added to will not necessarily match the number of groups you passed in $pm_groups. If you try to add the user to the same group twice, or to a group that the user is already a member of, addToGroups() will silently ignore it.
     */
    ...
    public function getPreferenceDefault($ps_pref, $pa_options = null)
    {
        ...
        switch ($va_pref_info["formatType"]) {
            ...
            case 'FT_IMPORT_EXPORT_MAPPING_GROUP_EDITOR_UI':
                $vn_type_id = (is_array(
                        $pa_options
                    ) && isset($pa_options['type_id']) && (int)$pa_options['type_id']) ? (int)$pa_options['type_id'] : null;
    ...
    public function isValidPreferenceValue($ps_pref, $ps_value, $pb_post_errors = false)
    {
        if ($this->isValidPreference($ps_pref)) {
            ...
            if (is_array($ps_value) && isset($va_pref_info["picks"])) {
                if (!((sizeof($ps_value) >= $va_pref_info["picks"]["minimum"]) && (sizeof(
                            $ps_value
                        ) <= $va_pref_info["picks"]["maximum"]))) {
                    if ($pb_post_errors) {
                        if ($va_pref_info["picks"]["minimum"] < $va_pref_info["picks"]["maximum"]) {
                            $this->postError(
                                921,
                                _t(
                                    "You must select between %1 and %2 choices for %3",
                                    $va_pref_info["picks"]["minimum"],
                                    $va_pref_info["picks"]["maximum"],
                                    $va_pref_info["label"]
                                ),
                            ...
                            $this->postError(
                                921,
                                _t(
                                    "You must select %1 choices for %2",
                                    $va_pref_info["picks"]["minimum"],
                                    $va_pref_info["label"]
                                ),
                                "User->isValidPreferenceValue()"
                            ...
                            if (!(($ps_value >= $va_pref_info["value"]["minimum"]) && ($ps_value <= $va_pref_info["value"]["maximum"]))) {
                                if ($pb_post_errors) {
                                    $this->postError(
                                        921,
                                        _t(
                                            "Value for %1 must be between %2 and %3",
                                            $va_pref_info["label"],
                                            $va_pref_info["value"]["minimum"],
                                            $va_pref_info["value"]["maximum"]
                                        ),
                                        "User->isValidPreferenceValue()"
                                    );
                                }
                                    ...
                                    if ($pb_post_errors) {
                                        if ($va_pref_info["value"]["minimum"] == 1) {
                                            ...
                                            $this->postError(
                                                921,
                                                _t(
                                                    "Value for %1 must be greater than %2",
                                                    $va_pref_info["label"],
                                                    $va_pref_info["value"]["minimum"]
                                                ),
                                                "User->isValidPreferenceValue()"
                                ...
                                if ($ps_value > $va_pref_info["value"]["maximum"]) {
                                    if ($pb_post_errors) {
                                        $this->postError(
                                            921,
                                            _t(
                                                "Value for %1 must be less than %2",
                                                $va_pref_info["label"],
                                                $va_pref_info["value"]["maximum"]
                                            ),
                                            "User->isValidPreferenceValue()"
                ...
                case 'FT_TEXT':
                    ...
                    if (isset($va_pref_info["length"]) && is_array($va_pref_info["length"])) {
                        ...
                        if (strlen($va_pref_info["length"]["minimum"]) && ($va_pref_info["length"]["maximum"])) {
                            if (!((strlen($ps_value) >= $va_pref_info["length"]["minimum"]) && (strlen(
                                        $ps_value
                                ...
                                if ($pb_post_errors) {
                                    $this->postError(
                                        921,
                                        _t(
                                            "Value for %1 must be between %2 and %3 characters",
                                            $va_pref_info["label"],
                                    ...
                                    if ($pb_post_errors) {
                                        if ($va_pref_info["length"]["minimum"] == 1) {
                                            ...
                                            $this->postError(
                                                921,
                                                _t(
                                                    "Value for %1 must be greater than %2 characters",
                                                    $va_pref_info["label"],
                                                    $va_pref_info["length"]["minimum"]
                                                ),
                                                "User->isValidPreferenceValue()"
                                ...
                                if ($ps_value > $va_pref_info["length"]["maximum"]) {
                                    if ($pb_post_errors) {
                                        $this->postError(
                                            921,
                                            _t(
                                                "Value for %1 must be less than %2 characters",
                                                $va_pref_info["label"],
                                                $va_pref_info["length"]["maximum"]
                                            ),
                                            "User->isValidPreferenceValue()"
                    ...
                    if (is_array($ps_value)) {
                        foreach ($ps_value as $vn_type_id => $vn_ui_id) {
                            if (!isset($va_valid_uis[$vn_type_id][$vn_ui_id])) {
                                if ($t_instance && (bool)$t_instance->getFieldInfo(
    ...
    public function preferenceHtmlFormElement($ps_pref, $ps_format = null, $pa_options = null)
    {
        if ($this->isValidPreference($ps_pref)) {
            ...
            switch ($va_pref_info["displayType"]) {
                ...
                case 'DT_FIELD':
                    ...
                    if ($vn_display_height > 1) {
                        $vs_output = "<textarea name='pref_$ps_pref' rows='" . $vn_display_height . "' cols='" . $vn_display_width . "'>" . htmlspecialchars(
                                $vs_current_value,
                                ENT_QUOTES,
                                'UTF-8'
                            ) . "</textarea>\n";
                    } else {
                        $vs_output = "<input type='text' name='pref_$ps_pref' size='$vn_display_width' maxlength='$vn_max_input_length'" . $vs_class . " value='" . htmlspecialchars(
                                $vs_current_value,
                                ENT_QUOTES,
                                'UTF-8'
                            ) . "'/>\n";
                    ...
                    switch ($va_pref_info['formatType']) {
                        case 'FT_UI_LOCALE':
                            ...
                            foreach ($va_locales as $vs_code => $va_parts) {
                                if (is_array($va_restrict_to_ui_locales) && sizeof(
                                        $va_restrict_to_ui_locales
                                    ) && !in_array($vs_code, $va_restrict_to_ui_locales)) {
                                    continue;
                                }
                                try {
                                    $vs_lang_name = Zend_Locale::getTranslation(
                                        strtolower($va_parts[0]),
                                        'language',
                                        strtolower($va_parts[0])
                                    );
                        ...
                        case 'FT_LOCALE':
                            ...
                            while ($qr_locales->nextRow()) {
                                $va_opts[$qr_locales->get('name')] = $qr_locales->get(
                                        'language'
                                    ) . '_' . $qr_locales->get('country');
                            }
                            break;
                        case 'FT_THEME':
                            if ($r_dir = opendir($this->_CONFIG->get('themes_directory'))) {
                                ...
                                while (($vs_theme_dir = readdir($r_dir)) !== false) {
                                    ...
                                    $o_theme_info = Configuration::load(
                                        $this->_CONFIG->get(
                                            'themes_directory'
                                        ) . '/' . $vs_theme_dir . '/themeInfo.conf'
                                    );
                            ...
                            if (method_exists($t_instance, 'getTypeFieldName') && ($t_instance->getTypeFieldName(
                                ...
                                foreach ($va_types as $vn_type_id => $va_type) {
                                    ...
                                    if (is_array($va_ui_list_by_type[$vn_type_id])) {
                                        foreach (
                                            caExtractValuesByUserLocale(
                                                $va_ui_list_by_type[$vn_type_id]
                                            ) as $vn_ui_id => $vs_label
                                        ) {
                                    ...
                                    $vs_output .= "<tr><td>" . str_repeat(
                                            "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;",
                                            (int)$va_type['LEVEL']
                                        ) . $va_type['name_singular'] . "</td><td><select name='pref_{$ps_pref}_{$vn_type_id}'>\n";
                                    foreach ($va_opts as $vs_val => $vs_opt) {
                                        $vs_selected = ($vs_val == $va_values[$vn_type_id]) ? "SELECTED" : "";
                                        $vs_output .= "<option value='" . htmlspecialchars(
                                                $vs_val,
                                                ENT_QUOTES,
                                                'UTF-8'
                ...
                case 'DT_CHECKBOXES':
                    if ($va_pref_info["formatType"] == 'FT_BIT') {
                        ...
                        foreach ($va_pref_info["choiceList"] as $vs_opt => $vs_val) {
                            ...
                            $vs_output .= "<input type='checkbox' name='pref_" . $ps_pref . "[]' value='" . htmlspecialchars(
                                    $vs_val,
                                    ENT_QUOTES,
                                    'UTF-8'
                                ) . "'" . $vs_class . " $vs_selected> " . $vs_opt . " \n";
                ...
                case 'DT_COUNTRY_LIST':
                    ...
                    if ($va_pref_info['stateProvPref']) {
                        ...
                        $vs_output .= "
							jQuery('#pref_{$ps_pref}').click({countryID: 'pref_{$ps_pref}', stateProvID: 'pref_" . $va_pref_info['stateProvPref'] . "', value: '" . addslashes(
                                $this->getPreference($va_pref_info['stateProvPref'])
                            ) . "', statesByCountryList: caStatesByCountryList}, caUI.utils.updateStateProvinceForCountry);
							jQuery(document).ready(function() {
								caUI.utils.updateStateProvinceForCountry({data: {countryID: 'pref_{$ps_pref}', stateProvID: 'pref_" . $va_pref_info['stateProvPref'] . "', value: '" . addslashes(
                                $this->getPreference($va_pref_info['stateProvPref'])
                ...
                case 'DT_CURRENCIES':
                    $vs_output .= caHTMLSelect(
                        "pref_{$ps_pref}",
                        caAvailableCurrenciesForConversion(),
                        array('id' => "pref_{$ps_pref}", 'class' => $vs_classname),
                        array('value' => $vs_current_value)
                    );
                    break;
                ...
                case 'DT_RADIO_BUTTONS':
                    foreach ($va_pref_info["choiceList"] as $vs_opt => $vs_val) {
                        ...
                        $vs_output .= "<input type='radio' name='pref_$ps_pref'" . $vs_class . " value='" . htmlspecialchars(
                                $vs_val,
                                ENT_QUOTES,
                                'UTF-8'
                            ) . "' $vs_selected> " . $vs_opt . " \n";
                ...
                case 'DT_PASSWORD':
                    ...
                    $vs_output = "<input type='password' name='pref_$ps_pref' size='$vn_display_width' maxlength='$vn_max_input_length'" . $vs_class . " value='" . htmlspecialchars(
                            $vs_current_value,
                            ENT_QUOTES,
                            'UTF-8'
                        ) . "'/>\n";
            ...
            if (preg_match("/\^DESCRIPTION/", $vs_format)) {
                ...
                $vs_format = str_replace(
                    "^LABEL",
                    '<span id="' . $vs_field_id . '">' . _t($va_pref_info["label"]) . '</span>',
                    $vs_format
                );
     ...
     *
     * @access public
     * @param bool $pb_force_reload If true, load defs file even if it has already been loaded
     * @return void
     */
     ...
     * @return bool Returns true if specified search was cleared, false if not
     */
     ...
     * @return bool True if searches were cleared, false if the operation failed
     */
    ...
    public function getSavedSearchByKey($pm_table_name_or_num, $ps_type, $ps_key)
    {
        ...
        return is_array(
            $va_searches[$vn_table_num][strtolower($ps_type)][$ps_key]
    ...
    public function getSavedSearches($pm_table_name_or_num, $ps_type)
    {
        ...
        return is_array($va_searches[$vn_table_num][strtolower($ps_type)]) ? $va_searches[$vn_table_num][strtolower(
            $ps_type
        )] : array();
    }
    # ----------------------------------------
    ...
    {
        $this->setVar(
            $this->getAppConfig()->get("app_name") . "_previous_to_last_logout",
            $this->getLastLogout(),
            array('volatile' => true)
        );
    ...
    private function passwordResetDeactivateAccount()
    {
        ...
        $this->opo_log->log(
            array(
                ...
                'MESSAGE' => _t(
                    'User %1 was permanently deactivated because the maximum number of consecutive unsuccessful password reset attemps was reached.',
                    $this->get('user_name')
                )
            )
        );
    ...
    public function authenticate(&$ps_username, $ps_password = "", $pa_options = null)
    {
        ...
        if ($vs_rewrite_username_with_regex = $this->opo_auth_config->get('rewrite_username_with_regex')) {
            ...
            $vs_username = preg_replace(
                "!" . preg_quote($vs_rewrite_username_with_regex, "!") . "!",
                $vs_rewrite_username_to_regex,
                $vs_username
            );
        }
                ...
                try {
                    ...
                    $this->opo_log->log(
                        array(
                            ...
                            'MESSAGE' => _t(
                                'There was an error while trying to fetch information for a new user from the current authentication backend. The message was %1 : %2',
                                get_class($e),
                                $e->getMessage()
                            )
                        )
                ...
                if (!$this->getPrimaryKey()) {
                    $this->opo_log->log(
                        array(
                            ...
                            'SOURCE' => 'ca_users/authenticate',
                            'MESSAGE' => $err = _t(
                                'User could not be created after getting info from authentication adapter: %1',
                                join(" ", $this->getErrors())
                            )
                        )
                    );
            ...
            try {
                    ...
                    if ($this->load($vs_username)) {
                        if (
                            ...
                            $this->opo_log->log(
                                array(
                                    ...
                                    'SOURCE' => 'ca_users/authenticate',
                                    'MESSAGE' => _t(
                                        'There was an error while trying to authenticate user %1: User is not authorized to log into Pawtucket',
                                        $vs_username
                                    )
                                )
                            );
                        ...
                        $this->opo_log->log(
                            array(
                                ...
                                'SOURCE' => 'ca_users/authenticate',
                                'MESSAGE' => _t(
                                    'There was an error while trying to authenticate user %1: Load by user name failed',
                                    $vs_username
                                )
                            )
                        );
                ...
                $this->opo_log->log(
                    array(
                        ...
                        'SOURCE' => 'ca_users/authenticate',
                        'MESSAGE' => _t(
                            'There was an error while trying to authenticate user %1: The message was %2 : %3',
                            $ps_username,
                            get_class($e),
                            $e->getMessage()
                        )
    ...
    public function getSourceAccessLevel($ps_table_name, $pm_source_code_or_id)
    {
        ...
        if (in_array(
            ...
            if (is_numeric($pm_source_code_or_id)) {
                ...
                $t_list = new ca_lists();
                $t_instance = Datamodel::getInstanceByTableName($ps_table_name, true);
                $vn_source_id = (int)$t_list->getItemIDFromList(
                    $t_instance->getSourceListCode(),
    ...
    public function getAccessStatuses($pn_access_level = null)
    {
        ...
        $va_item_values = ca_lists::itemIDsToItemValues(
            array_keys($va_access_by_item_id),
