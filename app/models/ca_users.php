...
class ca_users extends BaseModel {
	...
	static public function applyPasswordPolicy($password) {
		...
		if(strtolower($auth_adapter->get('auth_adapter')) !== 'causers') { return true; }	// password policies only apply to integral auth system
	...
	static public function getPasswordPolicyAsText() {
		$auth_config = Configuration::load(__CA_APP_DIR__."/conf/authentication.conf");
		if(strtolower($auth_adapter->get('auth_adapter')) !== 'causers') { return ''; }	// password policies only apply to integral auth system
