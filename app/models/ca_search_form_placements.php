...
BaseModel::$s_ca_models_definitions['ca_search_form_placements'] = array(
	...
	'FIELDS'        => array(
		...
		'form_id'      => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Form id',
			'DESCRIPTION'    => 'Identifier for form'
		),
		'bundle_name'  => array(
			'FIELD_TYPE'     => FT_TEXT,
			'DISPLAY_TYPE'   => DT_FIELD,
		...
		'rank'         => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Sort order' ),
			'DESCRIPTION'    => _t( 'The relative priority of the search form element when displayed in a list with other elements in a form. Lower numbers indicate higher priority.' )
		)
	)
);
...
class ca_search_form_placements extends BaseModel {
	...
	/**
	 * Sets setting definitions for to use for the current search form placement. Note that these definitions persist
	 * no matter what row is loaded
	...
	public function setSettingDefinitionsForPlacement( $pa_additional_settings ) {
		...
		$this->SETTINGS = new ModelSettings( $this, 'settings',
			array_merge( $_ca_editor_search_form_placement_settings, $pa_additional_settings ) );
