...
BaseModel::$s_ca_models_definitions['ca_editor_uis'] = array(
	...
	'FIELDS'        => array(
		'ui_id'        => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this user interface' )
		),
		'editor_code'  => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Editor code' ),
			'DESCRIPTION'    => _t( 'Unique code for editor; used to identify the editor for configuration purposes.' ),
		...
		'is_system_ui' => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Is system UI?' ),
			'DESCRIPTION'    => _t( 'If set, user interface will be available to all users.' ),
		...
		'editor_type'  => array(
			...
			'IS_NULL'            => false,
			'DEFAULT'            => '',
			'LABEL'              => _t( 'Editor type' ),
			'DESCRIPTION'        => _t( 'Type of item this editor UI operates on.' ),
...
class ca_editor_uis extends BundlableLabelableBaseModelWithAttributes {
	...
	public function __construct( $pn_id = null ) {
		...
		if ( ! ca_editor_uis::$s_loaded_relationship_tables ) {
			...
			foreach ( $va_rels as $vn_table_num => $va_rel_table_info ) {
				BaseModel::$s_ca_models_definitions['ca_editor_uis']['FIELDS']['editor_type']['BOUNDS_CHOICE_LIST'][ $va_rel_table_info['name'] ]
					= $vn_table_num;
			}
	...
	protected function initLabelDefinitions( $pa_options = null ) {
		...
		$this->BUNDLES['ca_editor_ui_screens']           = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Screens' )
		);
		$this->BUNDLES['ca_editor_ui_type_restrictions'] = array(
			'type'      => 'special',
			'repeating' => false,
	 ...
	 *
	 * @param mixed       $pm_table_name_or_num Table name or number to load editor for. Values for
	 *                                          $pm_table_name_or_num may be, for example:
	 ...
	 *                                          editorPref = "cataloguing" to select UI using user's
	 *                                          "cataloguing_<table_name>_editor_ui" preference, "quickadd" to use
	 *                                          "quickadd_<table_name>_editor_ui" pref; default is "cataloguing"
	 *
	...
	static public function loadDefaultUI( $pm_table_name_or_num, $po_request, $pn_type_id = null, $pa_options = null ) {
		...
		if ( $vn_type_id && $va_uis_by_type ) {
			if ( ! is_array( $va_uis_by_type ) ) {
				if ( ! isset( $va_available_uis_by_type[ $vn_type_id ][ $va_uis_by_type ] )
				     && ! isset( $va_available_uis_by_type['__all__'][ $va_uis_by_type ] )
				) {
					$vn_type_id = null;
				}
				$va_uis_by_type = [];
			} else {
				if ( ! isset( $va_available_uis_by_type[ $vn_type_id ][ $va_uis_by_type[ $vn_type_id ] ] )
				     && ! isset( $va_available_uis_by_type['__all__'][ $va_uis_by_type[ $vn_type_id ] ] )
				) {
					$vn_type_id = null;
				}
			}
		...
		if ( ! $vn_type_id && method_exists( $t_instance, "getTypeFieldName" )
	...
	public function getScreens( $pn_type_id = null, $pa_options = null ) {
		...
		if ( $pn_type_id > 0 ) {
			$va_wheres[]
				         = "(ceustr.type_id IS NULL OR ceustr.type_id = ? OR (ceustr.include_subtypes = 1 AND ceustr.type_id IN (?)))";
		...
		while ( $qr_res->nextRow() ) {
			...
			if ( ! $va_screens[ $vn_screen_id = $qr_res->get( 'screen_id' ) ][ $vn_screen_locale_id
				= $qr_res->get( 'locale_id' ) ]
			) {
				...
				$va_screens[ $vn_screen_id ][ $vn_screen_locale_id ]['numPlacements']
					= sizeof( $this->getScreenBundlePlacements( $vn_screen_id ) );
			}

			if ( $qr_res->get( 'restriction_type_id' ) ) {
				$vs_key_to_add = ( $t_instance instanceof BaseRelationshipModel ) ? 'type_code' : 'name_plural';
				$va_screens[ $vn_screen_id ][ $vn_screen_locale_id ]['typeRestrictions'][ $qr_res->get( 'restriction_type_id' ) ]
				               = $va_types[ $qr_res->get( 'restriction_type_id' ) ][ $vs_key_to_add ];
			}
		}
		if ( $vb_ids_only ) {
		...
		foreach ( $va_screens as $vn_screen_id => $va_screen_labels_by_locale ) {
			...
			foreach ( $va_screen_labels_by_locale as $vn_locale_id => $va_restriction_info ) {
				...
				$va_screens[ $vn_screen_id ][ $vn_locale_id ]['typeRestrictionsForDisplay'] = join( ', ',
					$va_screens[ $vn_screen_id ][ $vn_locale_id ]['typeRestrictions'] );
			}
		}
	...
	public static function getAccessForUI( $po_request, $pm_ui_id ) {
		...
		if ( ! is_numeric( $pm_ui_id )
		     || ! ( $t_ui = ca_editor_uis::find( array( 'ui_id' => $pm_ui_id ),
				array( 'returnAs' => 'firstModelInstance' ) ) )
		) {
			$t_ui = ca_editor_uis::find( array( 'editor_code' => $pm_ui_id ),
				array( 'returnAs' => 'firstModelInstance' ) );
		}
		if ( ! $t_ui ) {
			return null;
	 ...
	 *
	 * @param int   $pn_type_id Optional type_id used when and per-screen type restrictions are enforced; if not set
	 *                          (the default) then all screens are returned - no type restrictions are enforced.
	 * @param array $pa_options Options include:
	 *                          user_id = User_id to apply access control for
	 *
	...
	public function getScreenBundlePlacements( $pm_screen, $pn_type_id = null, $pa_options = null ) {
		...
		while ( $qr_res->nextRow() ) {
			...
			if ( isset( $va_tmp['settings']['bundleTypeRestrictions'] ) ) {
				...
				$va_types = caMakeTypeIDList( $this->get( 'editor_type' ), $va_types, [
					'dontIncludeSubtypesInTypeRestriction' => ! ( isset( $va_tmp['settings']['bundleTypeRestrictionsIncludeSubtypes'] )
					                                              && (bool) $va_tmp['settings']['bundleTypeRestrictionsIncludeSubtypes'] )
				] );
			}
	...
	public function getPlacementsForBundle( $ps_bundle_name, $po_request = null, $pa_options = null ) {
		...
		foreach ( $this->getScreens( null, array_merge( $pa_options, [ 'idsOnly' => true ] ) ) as $vn_screen_id ) {
			$va_placements = $this->getScreenBundlePlacements( 'Screen' . $vn_screen_id, null,
				[ 'bundleList' => [ $ps_bundle_name ] ] );
	 ...
	 * @param array       $pa_options Values to include in returned array for each screen. Values are returned as-is.
	 *                                Specific options also have the following effects: returnTypeRestrictions = return
	 *                                list of type restrictions for screen. Default is false. restrictToTypes = user_id
	 *                                = User_id to apply access control for
	 *
	 * @return array
	 */
	public function getScreensAsNavConfigFragment(
		$po_request, $pn_type_id, $ps_module_path, $ps_controller, $ps_action, $pa_parameters, $pa_requirements,
		$pb_disable_options = false, $pa_options = null
	) {
		if ( ! caGetOption( 'user_id', $pa_options, null ) && $po_request ) {
			$pa_options['user_id'] = $po_request->getUserID();
		}
		...
		if ( is_array( $restrict_to_types = caGetOption( 'restrictToTypes', $pa_options, null ) )
		     && sizeof( $restrict_to_types )
		) {
		...
		foreach ( $va_screens as $va_screen ) {
			...
			if ( is_array( $va_screen_restrictions ) ) {
				$va_screen_restrictions = caMakeTypeIDList( $this->get( 'editor_type' ),
					array_keys( $va_screen_restrictions ) );
			}

			if ( is_array( $restrict_to_types ) && is_array( $va_screen_restrictions )
			     && ( sizeof( $va_screen_restrictions ) > 0 )
			) {
				$vb_skip = true;
			...
			if ( is_array( $pa_options ) ) {
				$va_nav[ 'screen_' . $va_screen['screen_id'] ] = array_merge( $va_nav[ 'screen_'
				                                                                       . $va_screen['screen_id'] ],
					$pa_options );
			}
	 ...
	 */
	public function addPlacementToScreen(
		$pm_screen, $ps_bundle_name, $ps_placement_code, $pa_settings, $pn_rank = null, $pa_options = null
	) {
		if ( ! $this->getPrimaryKey() ) {
			return null;
		}
	 ...
	 *
	 * @param mixed  $pm_screen                                    screen_id, idno or preferred label of screen to add
	 *                                                             bundle to.
	 ...
	 * @return int Returns placement_id of newly created placement on success, false on error
	 */
	public function addPlacementToScreenBefore(
		$pm_screen, $ps_bundle_name, $ps_placement_code, $pa_settings, $ps_relative_to_bundle_name_or_placement_code,
		$pa_options = null
	) {
		if ( ! $this->getPrimaryKey() ) {
			return null;
		...
		if ( $t_screen = ca_editor_ui_screens::loadScreen( $this->getPrimaryKey(), $pm_screen, $pa_options ) ) {
			return $t_screen->addPlacementBefore( $ps_bundle_name, $ps_placement_code, $pa_settings,
				$ps_relative_to_bundle_name_or_placement_code, $pa_options );
		}
	 ...
	 * @return int Returns placement_id of newly created placement on success, false on error
	 */
	public function addPlacementToScreenAfter(
		$pm_screen, $ps_bundle_name, $ps_placement_code, $pa_settings, $ps_relative_to_bundle_name_or_placement_code,
		$pa_options = null
	) {
		if ( ! $this->getPrimaryKey() ) {
		...
		if ( $t_screen = ca_editor_ui_screens::loadScreen( $this->getPrimaryKey(), $pm_screen, $pa_options ) ) {
			return $t_screen->addPlacementAfter( $ps_bundle_name, $ps_placement_code, $pa_settings,
				$ps_relative_to_bundle_name_or_placement_code, $pa_options );
		}
	...
	public static function getUIList( $pm_table = null, $pn_user_id = null, $pn_type_id = null ) {
		...
		if ( $pn_type_id ) {
			$va_wheres[]
				         = "(ceui.type_id IS NULL OR ceutr.type_id = ? OR (ceutr.include_subtypes = 1 AND ceutr.type_id IN (?)))";
		...
		return ca_editor_uis::$s_available_ui_cache[ $pm_table . '/' . $pn_user_id ]
			= ca_editor_uis::$s_available_ui_cache[ $pn_table_num . '/' . $pn_user_id ] = $va_uis;
	}
	# ----------------------------------------
	 ...
	 *
	 * @param array $pa_screen_ids A list of screen_ids in the ui, in the order in which they should be displayed in
	 *                             the ui
	...
	public function reorderScreens( $pa_screen_ids, $pa_options = null ) {
		...
		foreach ( $pa_screen_ids as $vn_rank => $vn_screen_id ) {
			if ( isset( $va_screen_ranks[ $vn_screen_id ] )
			     && $t_screen->load( array(
					'ui_id'     => $vn_ui_id,
					'screen_id' => $vn_screen_id
				) )
			) {
				if ( $va_screen_ranks[ $vn_screen_id ] != $vn_rank ) {
					...
					if ( $t_screen->numErrors() ) {
						$va_errors[ $vn_screen_id ] = _t( 'Could not reorder screen %1: %2', $vn_screen_id,
							join( '; ', $t_screen->getErrors() ) );
					}
				}
			} else {
				// add screen to UI
	...
	public function removeTypeRestriction( $pn_type_id = null ) {
		...
		if ( is_array(
			$va_uis = ca_editor_ui_type_restrictions::find( $va_params, [ 'returnAs' => 'modelInstances' ] ) )
		) {
	 ...
	 * @return bool
	 */
	public function createScreenFromPawtucketDetailTemplate(
		$ps_path_to_template, $ps_screen_name, array $pa_options = null
	) {
		global $g_ui_locale;
		if ( ! ( $vn_ui_id = $this->getPrimaryKey() ) ) {
			return null;
		...
		// load screen?
		if (
			! ( $t_screen = ca_editor_ui_screens::find( [ 'idno' => $ps_screen_name ],
				[ 'returnAs' => 'firstModelInstance' ] ) )
			&&
			! ( $t_screen = ca_editor_ui_screens::find( [ 'idno' => $vs_screen_code ],
				[ 'returnAs' => 'firstModelInstance' ] ) )
		) {
			// create screen
		...
		if ( is_array( $pa_type_restrictions = caGetOption( 'restrictions', $pa_options, null ) )
		     && sizeof( $pa_type_restrictions )
		) {
		...
		} elseif ( preg_match( "!^{$vs_table}_(.*)_html$!", pathinfo( $ps_path_to_template, PATHINFO_FILENAME ), $m )
		           && ( $vn_type_restriction_id = $t_instance->getTypeIDForCode( $vs_type_restriction = $m[1] ) )
		) {
			$t_screen->addTypeRestriction( $vn_type_restriction_id );
		...
		foreach ( $va_tags as $vs_tag ) {
			...
			if ( $va_tag[0] == $vs_table ) {
				if ( $t_instance->isValidBundle( $vs_tag ) ) {    //related
					...
					$va_bundles[ "ca_attribute_" . $va_tag[1] ] = preg_replace( "![^A-Za-z0-9_]+!", "_",
						$va_tag[0] . '_' . $va_tag[1] );
				}
		...
		foreach ( $va_bundles as $vs_bundle_name => $vs_placement_code ) {
			$t_screen->addPlacement( $vs_bundle_name, "screen_{$vn_screen_id}_{$vs_bundle_name}", [], $pn_rank = null,
				$pa_options = null );
		}
		...
		return [
			'screen'            => $ps_screen_name,
			'screen_idno'       => $vs_screen_code,
			'type_restrictions' => $pa_type_restrictions,
			'replace'           => $pb_replace,
			'bundles'           => $va_bundles_for_return
		];
	}
