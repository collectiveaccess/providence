...
BaseModel::$s_ca_models_definitions['ca_editor_uis'] = array(
    ...
    'FIELDS' => array(
        'ui_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique numeric identifier used by CollectiveAccess internally to identify this user interface'
            )
        ),
        'editor_code' => array(
            'FIELD_TYPE' => FT_TEXT,
...
class ca_editor_uis extends BundlableLabelableBaseModelWithAttributes
{
    ...
    static public $s_available_ui_cache = [];
    static public $s_default_ui_cache = [];
    ...
    static public $s_loaded_relationship_tables = false;
    static public $s_screen_cache = [];
    static public $s_screen_bundle_cache = [];
    static public $s_placements_for_bundle_cache = [];
    ...
    public static function loadDefaultUI($pm_table_name_or_num, $po_request, $pn_type_id = null, $pa_options = null)
    {
        ...
        if (!$vn_type_id && method_exists($t_instance, "getTypeFieldName") && (bool)$t_instance->getFieldInfo(
    ...
    public function getScreens($pn_type_id = null, $pa_options = null)
    {
        ...
        while ($qr_res->nextRow()) {
            ...
            if (!$va_screens[$vn_screen_id = $qr_res->get('screen_id')][$vn_screen_locale_id = $qr_res->get(
                'locale_id'
            )]) {
                ...
                $va_screens[$vn_screen_id][$vn_screen_locale_id]['numPlacements'] = sizeof(
                    $this->getScreenBundlePlacements($vn_screen_id)
                );
            }

            if ($qr_res->get('restriction_type_id')) {
                $vs_key_to_add = ($t_instance instanceof BaseRelationshipModel) ? 'type_code' : 'name_plural';
                $va_screens[$vn_screen_id][$vn_screen_locale_id]['typeRestrictions'][$qr_res->get(
                    'restriction_type_id'
    ...
    {
        if (!is_numeric($pm_ui_id) || !($t_ui = ca_editor_uis::find(
                array('ui_id' => $pm_ui_id),
        ...
        if ($vn_user_id = $po_request->getUserID()) {
            ...
            if ((is_array($va_groups = $t_user->getUserGroups())) && sizeof($va_groups)) {
                $qr_groups = $t_user->getDb()->query(
					...
					WHERE
						group_id IN (?) AND ui_id = ?",
                    array(array_keys($va_groups), $vn_ui_id)
                );
            ...
            if ((is_array($va_roles = $t_user->getUserRoles())) && sizeof($va_roles)) {
                $qr_roles = $t_user->getDb()->query(
					...
					WHERE
						role_id IN (?) AND ui_id = ?",
                    array(array_keys($va_roles), $vn_ui_id)
                );
    ...
    public static function getAccessForScreen($po_request, $pm_screen)
    {
        ...
        if ($vn_user_id = $po_request->getUserID()) {
            ...
            $qr_users = $t_user->getDb()->query(
				...
				FROM ca_editor_ui_screens_x_users
				WHERE
					user_id = ? AND screen_id = ?",
                array($vn_user_id, $vn_screen_id)
            );
            ...
            if ((is_array($va_groups = $t_user->getUserGroups())) && sizeof($va_groups)) {
                $qr_groups = $t_user->getDb()->query(
					...
					FROM ca_editor_ui_screens_x_user_groups
					WHERE
						group_id IN (?) AND screen_id = ?",
                    array(array_keys($va_groups), $vn_screen_id)
                );
            ...
            if ((is_array($va_roles = $t_user->getUserRoles())) && sizeof($va_roles)) {
                $qr_roles = $t_user->getDb()->query(
					...
					FROM ca_editor_ui_screens_x_roles
					WHERE
						role_id IN (?) AND screen_id = ?",
                    array(array_keys($va_roles), $vn_screen_id)
                );
        ...
        while ($qr_res->nextRow()) {
            ...
            if (isset($va_tmp['settings']['bundleTypeRestrictions'])) {
                ...
                $va_types = caMakeTypeIDList(
                    ...
                    $va_types,
                    ['dontIncludeSubtypesInTypeRestriction' => !(isset($va_tmp['settings']['bundleTypeRestrictionsIncludeSubtypes']) && (bool)$va_tmp['settings']['bundleTypeRestrictionsIncludeSubtypes'])]
                );
            }
    ...
    public function getPlacementsForBundle($ps_bundle_name, $po_request = null, $pa_options = null)
    {
        ...
        foreach ($this->getScreens(null, array_merge($pa_options, ['idsOnly' => true])) as $vn_screen_id) {
            $va_placements = $this->getScreenBundlePlacements(
                'Screen' . $vn_screen_id,
                null,
    ...
    public function getScreensAsNavConfigFragment(
        ...
        if (is_array($restrict_to_types = caGetOption('restrictToTypes', $pa_options, null)) && sizeof(
                $restrict_to_types
            )) {
        ...
        foreach ($va_screens as $va_screen) {
            ...
            if (is_array($va_screen_restrictions)) {
                $va_screen_restrictions = caMakeTypeIDList(
                    $this->get('editor_type'),
                    array_keys($va_screen_restrictions)
                );
            }

            if (is_array($restrict_to_types) && is_array($va_screen_restrictions) && (sizeof(
                        $va_screen_restrictions
                    ) > 0)) {
                $vb_skip = true;
            ...
            if (is_array($pa_options)) {
                $va_nav['screen_' . $va_screen['screen_id']] = array_merge(
                    $va_nav['screen_' . $va_screen['screen_id']],
                    $pa_options
                );
            }
    ...
    public function addPlacementToScreenBefore(
        ...
        if ($t_screen = ca_editor_ui_screens::loadScreen($this->getPrimaryKey(), $pm_screen, $pa_options)) {
            return $t_screen->addPlacementBefore(
                $ps_bundle_name,
                $ps_placement_code,
                $pa_settings,
                $ps_relative_to_bundle_name_or_placement_code,
    ...
    public function addPlacementToScreenAfter(
        ...
        if ($t_screen = ca_editor_ui_screens::loadScreen($this->getPrimaryKey(), $pm_screen, $pa_options)) {
            return $t_screen->addPlacementAfter(
                $ps_bundle_name,
                $ps_placement_code,
                $pa_settings,
                $ps_relative_to_bundle_name_or_placement_code,
    ...
    public function reorderScreens($pa_screen_ids, $pa_options = null)
    {
        ...
        foreach ($pa_screen_ids as $vn_rank => $vn_screen_id) {
            if (isset($va_screen_ranks[$vn_screen_id]) && $t_screen->load(
                    array('ui_id' => $vn_ui_id, 'screen_id' => $vn_screen_id)
                )) {
                if ($va_screen_ranks[$vn_screen_id] != $vn_rank) {
                    ...
                    if ($t_screen->numErrors()) {
                        $va_errors[$vn_screen_id] = _t(
                            'Could not reorder screen %1: %2',
                            $vn_screen_id,
    ...
    public function createScreenFromPawtucketDetailTemplate(
        ...
        if (is_array($pa_type_restrictions = caGetOption('restrictions', $pa_options, null)) && sizeof(
                $pa_type_restrictions
            )) {
        ...
        if (is_array($pa_type_restriction_ids) && sizeof($pa_type_restriction_ids)) {
            foreach ($pa_type_restriction_ids as $vn_type_restriction_id) {
                ...
                "!^{$vs_table}_(.*)_html$!",
                pathinfo($ps_path_to_template, PATHINFO_FILENAME),
                $m
        ...
        foreach ($va_tags as $vs_tag) {
            ...
            if ($va_tag[0] == $vs_table) {
                if ($t_instance->isValidBundle($vs_tag)) {    //related
                    ...
                    $va_bundles["ca_attribute_" . $va_tag[1]] = preg_replace(
                        "![^A-Za-z0-9_]+!",
                        "_",
                        $va_tag[0] . '_' . $va_tag[1]
                    );
        ...
        foreach ($va_bundles as $vs_bundle_name => $vs_placement_code) {
            $t_screen->addPlacement(
                $vs_bundle_name,
                "screen_{$vn_screen_id}_{$vs_bundle_name}",
                [],
                $pn_rank = null,
                $pa_options = null
            );
        ...
        $va_bundles_for_return = array_map(
            function ($v) {
