...
class ca_object_representations extends BundlableLabelableBaseModelWithAttributes implements IBundleProvider
{
        ...
        if ($vn_rc = parent::insert($pa_options)) {
            if (is_array($va_media_info = $this->getMediaInfo('media', 'original'))) {
                ...
                if (is_array(
                    ...
                    ) && isset($va_media_info['ORIGINAL_FILENAME']) && strlen($va_media_info['ORIGINAL_FILENAME'])) {
                    ...
                    if ($t_rep_link->load($qr_res->get('relation_id'))) {
                        ...
                        if ($t_rep_link->numErrors()) {
                            $this->postError(
                                2700,
                                _t(
                                    'Could not update primary flag for representation: %1',
                        ...
                        $this->postError(
                            2700,
                            _t('Could not load object-representation link'),
                            'ca_objects_x_object_representations->delete()'
                        );
                        return false;
                    }
    ...
    public function getAnnotations($pa_options = null)
    {
        ...
        while ($qr_annotations->nextHit()) {
            $va_labels[$vn_annotation_id = $qr_annotations->get(
                "{$vs_annotation_table}.annotation_id"
            )][$qr_annotations->get("{$vs_annotation_label_table}.locale_id")][] = $qr_annotations->get(
                "{$vs_annotation_table}.preferred_labels.name"
            );

            if ($vs_class_element) {
                $va_annotation_classes[$vn_annotation_id] = $qr_annotations->get(
                    "{$vs_annotation_table}.{$vs_class_element}",
        ...
        foreach ($va_annotations as $vs_key => $va_values) {
            foreach ($va_values as $va_val) {
                ...
                $vs_label = is_array($va_labels_for_locale[$va_val['annotation_id']]) ? array_shift(
                    $va_labels_for_locale[$va_val['annotation_id']]
                ) : '';
    ...
    protected function getRepresentationAnnotationHTMLFormBundle(
        ...
        if (sizeof($va_items = $this->getAnnotations())) {
            ...
            foreach ($va_items as $vn_id => $va_item) {
                ...
                $va_inital_values[$va_item[$t_item->primaryKey()]] = array_merge(
                    $va_item,
                    array(
    ...
    protected function _processRepresentationAnnotations($po_request, $ps_form_prefix, $ps_placement_code)
    {
        ...
        foreach ($va_rel_items as $vn_id => $va_rel_item) {
            ...
            if (strlen(
                $vn_status = $po_request->getParameter(
                    $ps_placement_code . $ps_form_prefix . '_status_' . $va_rel_item['annotation_id'],
                    pString
                )
            )) {
                $vn_access = $po_request->getParameter(
                    $ps_placement_code . $ps_form_prefix . '_access_' . $va_rel_item['annotation_id'],
                    pInteger
                );
                $vn_locale_id = $po_request->getParameter(
                    $ps_placement_code . $ps_form_prefix . '_locale_id_' . $va_rel_item['annotation_id'],
                    pInteger
                );
                ...
                foreach ($o_coder->getPropertyList() as $vs_property) {
                    $va_properties[$vs_property] = $po_request->getParameter(
                        $x = $ps_placement_code . $ps_form_prefix . '_' . $vs_property . '_' . $va_rel_item['annotation_id'],
                        pString
                    );
                }
                ...
                if ($this->numErrors()) {
                    ...
                    if ($vs_label = $po_request->getParameter(
                        $ps_placement_code . $ps_form_prefix . '_label_' . $va_rel_item['annotation_id'],
                        pString
                    )) {
                            ...
                            if (sizeof($va_pref_labels)) {
                                ...
                                foreach ($va_pref_labels as $vn_annotation_dummy_id => $va_labels_by_locale) {
                                    foreach ($va_labels_by_locale as $vn_locale_dummy_id => $va_labels) {
                                        $t_annotation->editLabel(
                                            $va_labels[0]['label_id'],
                                            array('name' => $vs_label),
                                            $vn_locale_id,
                                            null,
                                            true
                ...
                if (($po_request->getParameter(
                        $ps_placement_code . $ps_form_prefix . '_' . $va_rel_item['annotation_id'] . '_delete',
                        pInteger
                    )) > 0) {
                    // delete!
        ...
        foreach ($_REQUEST as $vs_key => $vs_value) {
            if (!preg_match(
                '/^' . $ps_placement_code . $ps_form_prefix . '_status_new_([\d]+)/',
                $vs_key,
                $va_matches
            )) {
                continue;
            ...
            if (strlen(
                    $vn_status = $po_request->getParameter(
                        $ps_placement_code . $ps_form_prefix . '_status_new_' . $vn_c,
                        pString
                    )
                ) > 0) {
                $vn_access = $po_request->getParameter(
                    $ps_placement_code . $ps_form_prefix . '_access_new_' . $vn_c,
                    pInteger
                );
                $vn_locale_id = $po_request->getParameter(
                    $ps_placement_code . $ps_form_prefix . '_locale_id_new_' . $vn_c,
                    pInteger
                );
                ...
                foreach ($o_coder->getPropertyList() as $vs_property) {
                    $va_properties[$vs_property] = $po_request->getParameter(
                        $ps_placement_code . $ps_form_prefix . '_' . $vs_property . '_new_' . $vn_c,
                        pString
                    );
                }

                // create annotation
                $vs_label = $po_request->getParameter(
                    $ps_placement_code . $ps_form_prefix . '_label_new_' . $vn_c,
                    pString
                );
                $vn_annotation_id = $this->addAnnotation(
                    $vs_label,
                    $vn_locale_id,
    ...
    public function addFile($ps_filepath, $ps_resource_path = '/', $pb_allow_duplicates = true)
    {
        ...
        if (!$pb_allow_duplicates) {
            if ($t_multifile->load(
                array('resource_path' => $ps_resource_path, 'representation_id' => $this->getPrimaryKey())
            )) {
                return null;
            }
        }
    ...
    public function removeFile($pn_multifile_id)
    {
        ...
        if ($t_multifile->get('representation_id') == $this->getPrimaryKey()) {
            ...
            $this->postError(
                2720,
                _t('File is not part of this representation'),
    ...
    public function removeCaptionFile($pn_caption_id)
    {
        ...
        if ($t_caption->get('representation_id') == $this->getPrimaryKey()) {
            ...
            $this->postError(
                2720,
                _t('Caption file is not part of this representation'),
        ...
        while ($qr_reps->nextRow()) {
            ...
            foreach ($pa_versions as $vs_version) {
                ...
                if ($vn_width && $vn_height) {
                    $va_tmp['tags'][$vs_version] = $qr_reps->getMediaTag(
                        'media',
                        $vs_version,
                        array_merge(
                            $pa_options,
                            array(
                                'viewer_width' => $vn_width,
                                'viewer_height' => $vn_height
                            )
                        )
                    );
                } else {
                ...
                if (isset($va_tmp['info'][$vs_version]['PROPERTIES']['resolution']) && is_array(
                        $va_resolution = $va_tmp['info'][$vs_version]['PROPERTIES']['resolution']
                    )) {
    ...
    public function getRepresentationViewerHTMLBundle(
        $po_request,
        $pa_options = null,
        $pa_additional_display_options = null
    ) {
     ...
     */
    public static function mediaExists($ps_filepath)
    {
     ...
     * @param array $pa_options No options are currently supported
     *
     * @return int The number of representations
     */
    ...
    public function getTranscription($options = null)
    {
        ...
        if (
            !($user_id && ($transcript = ca_representation_transcriptions::find(
                    ['representation_id' => $rep_id, 'user_id' => $user_id],
                    ['returnAs' => 'firstModelInstance']
                )))
            &&
            !($transcript = ca_representation_transcriptions::find(
                ['representation_id' => $rep_id, 'ip_addr' => $ip],
                ['returnAs' => 'firstModelInstance']
            ))
        ) {
            $transcript = null;
    ...
    public function removeTranscription($transcription_id, $options = null)
    {
        ...
        if (!$transcription->getPrimaryKey()) {
            $this->postError(
                2800,
                _t('Transcription id is invalid'),
                'ca_object_representations->removeTranscription()',
        ...
        if (!$force) {
            if ($transcription->get('representation_id') != $rep_id) {
                $this->postError(
                    2810,
                    _t('Transcription is not part of representation'),
                    'ca_object_representations->removeTranscription()',
                    'ca_representation_transcriptions'
                );
                return false;
            ...
            if ($user_id) {
                if ($transcription->get('user_id') != $user_id) {
                    $this->postError(
                        2820,
                        _t('Transcription was not created by specified user'),
                        'ca_object_representations->removeTranscription()',
                        'ca_representation_transcriptions'
                    );
                    return false;
        ...
        switch ($return_as) {
            ...
            default:
                ...
                while ($qr->nextRow()) {
                    ...
                    $transcriptions[$transcription_id]['created_on_display'] = caGetLocalizedDate(
                        $transcriptions[$transcription_id]['created_on']
                    );
                    $transcriptions[$transcription_id]['completed_on_display'] = caGetLocalizedDate(
                        $transcriptions[$transcription_id]['completed_on']
                    );
                    $transcriptions[$transcription_id]['transcription_duration'] = ($transcriptions[$transcription_id]['completed_on'] > 0) && (($transcriptions[$transcription_id]['completed_on'] - $transcriptions[$transcription_id]['created_on'] > 0)) ? caFormatInterval(
                        $transcriptions[$transcription_id]['completed_on'] - $transcriptions[$transcription_id]['created_on']
                    ) : '';
                    $transcriptions[$transcription_id]['validated_on_display'] = caGetLocalizedDate(
                        $transcriptions[$transcription_id]['validated_on']
                    );

                    if ($transcriptions[$transcription_id]['user_id']) {
                        $transcriptions[$transcription_id]['name'] = trim(
                            $transcriptions[$transcription_id]['fname'] . ' ' . $transcriptions[$transcription_id]['lname']
                        );
                        ...
                        $transcriptions[$transcription_id]['transcriber'] = _t(
                            'user at %1',
                            $transcriptions[$transcription_id]['ip_addr']
                        );
                    }
                    $transcriptions[$transcription_id]['status_message'] = _t(
                        'Created %1 by %2',
                    ...
                    if ($transcriptions[$transcription_id]['completed_on'] > 0) {
                        $transcriptions[$transcription_id]['status_message'] .= ($transcriptions[$transcription_id]['transcription_duration']) ? "<br/>" . _t(
                                'Completed on %1 (%2)',
    ...
    public function transcriptionIsComplete(array $options = null)
    {
        ...
        if ($current_user_only) {
            ...
            return ca_representation_transcriptions::find(
                ['representation_id' => $rep_id, 'completed_on' => ['>', 0]],
                ['returnAs' => 'firstModelInstance']
            ) ? true : false;
        }
        return false;
                ...
                if (($qr = caMakeSearchResult('ca_object_representations', [$row_id])) && $qr->nextHit()) {
                    ...
                    switch ($bundle_name) {
                        ...
                        case 'media_colorspace':
                            ...
                            break;
                        case 'media_resolution':
                            if (isset($info[$version]['PROPERTIES']['resolution']) && is_array(
                                    $resolution = $info[$version]['PROPERTIES']['resolution']
                                )) {
