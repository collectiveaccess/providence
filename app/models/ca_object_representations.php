...
BaseModel::$s_ca_models_definitions['ca_object_representations'] = array(
	...
	'FIELDS'        => array(
		'representation_id'    => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this representation' )
		),
		'locale_id'            => array(
			'FIELD_TYPE'     => FT_NUMBER,
		...
		'type_id'              => array(
			...
			'DEFAULT'         => '',
			'LABEL'           => _t( 'Type' ),
			'DESCRIPTION'     => _t( 'Indicates the type of the representation. The type can only be set when creating a new representation and cannot be changed once the representation is saved.' )
		),
		'idno'                 => array(
			'FIELD_TYPE'     => FT_TEXT,
		...
		'source_id'            => array(
			...
			'LABEL'                     => _t( 'Source' ),
			'DESCRIPTION'               => _t( 'Administrative source of object representation. This value is often used to indicate the administrative sub-division or legacy database from which the object originates, but can also be re-tasked for use as a simple classification tool if needed.' )
		),
		'source_info'          => array(
			'FIELD_TYPE'     => FT_VARS,
			'DISPLAY_TYPE'   => DT_OMIT,
...
class ca_object_representations extends BundlableLabelableBaseModelWithAttributes implements IBundleProvider {
	...
	protected function initLabelDefinitions( $pa_options = null ) {
		...
		$this->BUNDLES['ca_representation_annotations']          = array(
			'type'      => 'related_table',
			'repeating' => true,
			'label'     => _t( 'Related annotations' )
		);
		$this->BUNDLES['ca_user_representation_annotations']     = array(
			'type'      => 'related_table',
			'repeating' => true,
			'label'     => _t( 'Related user-generated annotations' )
		);
		...
		$this->BUNDLES['ca_sets_checklist']                      = array(
			'type'      => 'special',
			'repeating' => true,
			'label'     => _t( 'Sets' )
		);
		...
		$this->BUNDLES['ca_object_lots']                         = array(
			'type'      => 'related_table',
			'repeating' => true,
			'label'     => _t( 'Related lot' )
		);

		$this->BUNDLES['ca_item_tags']     = array( 'type' => 'special', 'repeating' => true, 'label' => _t( 'Tags' ) );
		$this->BUNDLES['ca_item_comments'] = array(
			'type'      => 'special',
			'repeating' => true,
			'label'     => _t( 'Comments' )
		);

		$this->BUNDLES['ca_representation_transcriptions'] = array(
			'type'      => 'special',
			'repeating' => true,
			'label'     => _t( 'Transcriptions' )
		);

		$this->BUNDLES['ca_object_representations_media_display'] = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Media and preview images' )
		);
		$this->BUNDLES['ca_object_representation_captions']       = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Captions/subtitles' )
		);
		...
		$this->BUNDLES['transcription_count'] = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Number of transcriptions' )
		);
		$this->BUNDLES['page_count']          = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Number of pages' )
		);
		$this->BUNDLES['preview_count']       = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Number of previews' )
		);
		$this->BUNDLES['media_dimensions']    = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Media dimensions' )
		);
		$this->BUNDLES['media_duration']      = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Media duration' )
		);
		$this->BUNDLES['media_class']         = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Media class' )
		);
		$this->BUNDLES['media_format']        = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Media format' )
		);
		$this->BUNDLES['media_colorspace']    = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Media colorspace' )
		);
		$this->BUNDLES['media_resolution']    = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Media resolution' )
		);
		$this->BUNDLES['media_bitdepth']      = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Media bit depth' )
		);
		$this->BUNDLES['media_filesize']      = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Media filesize' )
		);
		$this->BUNDLES['media_center_x']      = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Center of media x-coordinate' )
		);
		$this->BUNDLES['media_center_y']      = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Center of media y-coordinate' )
		);
		...
		if ( $vn_rc = parent::insert( $pa_options ) ) {
			if ( is_array( $va_media_info = $this->getMediaInfo( 'media', 'original' ) ) ) {
				...
				if ( is_array( $va_media_info = $this->getMediaInfo( 'media' ) )
				     && isset( $va_media_info['ORIGINAL_FILENAME'] )
				     && strlen( $va_media_info['ORIGINAL_FILENAME'] )
				) {
	 ...
	 *        dontCheckPrimaryValue = if set the is_primary state of other related representations is not considered
	 *        during the delete
				...
				while ( $qr_res->nextRow() ) {
					...
					if ( $t_rep_link->load( $qr_res->get( 'relation_id' ) ) ) {
						...
						if ( $t_rep_link->numErrors() ) {
							$this->postError( 2700, _t( 'Could not update primary flag for representation: %1',
								join( '; ', $t_rep_link->getErrors() ) ),
								'ca_objects_x_object_representations->delete()' );

							return false;
						}
					} else {
						$this->postError( 2700, _t( 'Could not load object-representation link' ),
							'ca_objects_x_object_representations->delete()' );
	...
	public function mediaIsEmpty() {
		...
		if ( preg_match( "!^userMedia[\d]+/!", $vs_media_path )
		     && file_exists( "{$vs_tmp_directory}/{$vs_media_path}" )
		) {
			return false;
		}
	 ...
	 * metadata is available for binary files limiting display choices. Use this method to determine if such metadata
	 ...
	 * Type codes are based upon the mimetype of the representation's media as defined in the annotation_types.conf
	 * file
	 *
	 * If you pass the options $pn_representation_id parameter then the returned type is for the specified
	 * representation rather than the currently loaded one.
	 */
	...
	protected function annotationTable() {
		return ( $this->ANNOTATION_MODE == 'user' ) ? 'ca_user_representation_annotations'
	...
	protected function annotationLabelTable() {
		return ( $this->ANNOTATION_MODE == 'user' ) ? 'ca_user_representation_annotation_labels'
	 ...
	 *                          checkAccess - array of access codes to filter count by. Only annotations with an access
	 *                          value set to one of the specified values will be counted.
	 *
	 ...
	 * @param array $pa_options Optional array of options. Supported options are:
	 *                          checkAccess = array of access codes to filter count by. Only annotations with an access
	 *                          value set to one of the specified values will be returned start = max = labelsOnly =
	 *                          idsOnly = user_id = item_id =
	 *
	 * @return array List of annotations attached to the current representation, key'ed on annotation_id. Value is an
	 *               array will all values; annotation labels are returned in the current locale.
	 */
	public function getAnnotations( $pa_options = null ) {
		...
		$vn_timecode_offset = isset( $va_rep_props['PROPERTIES']['timecode_offset'] )
			? (float) $va_rep_props['PROPERTIES']['timecode_offset'] : 0;
		...
		while ( $qr_annotations->nextHit() ) {
			$va_labels[ $vn_annotation_id
				= $qr_annotations->get( "{$vs_annotation_table}.annotation_id" ) ][ $qr_annotations->get( "{$vs_annotation_label_table}.locale_id" ) ][]
				= $qr_annotations->get( "{$vs_annotation_table}.preferred_labels.name" );

			if ( $vs_class_element ) {
				$va_annotation_classes[ $vn_annotation_id ]
					= $qr_annotations->get( "{$vs_annotation_table}.{$vs_class_element}",
		...
		$va_key = array();
		if ( $qr_list_items = caMakeSearchResult( 'ca_list_items',
			array_values( $va_annotation_classes_flattened ) )
		) {
		...
		foreach ( $va_annotations as $vs_key => $va_values ) {
			foreach ( $va_values as $va_val ) {
				...
				$vs_label         = is_array( $va_labels_for_locale[ $va_val['annotation_id'] ] )
					? array_shift( $va_labels_for_locale[ $va_val['annotation_id'] ] ) : '';
	...
	public function addAnnotation(
		$ps_title, $pn_locale_id, $pn_user_id, $pa_properties, $pn_status, $pn_access, $pa_values = null,
		$pa_options = null
	) {
		if ( ! ( $vn_representation_id = $this->getPrimaryKey() ) ) {
			return null;
	...
	public function editAnnotation(
		$pn_annotation_id, $pn_locale_id, $pa_properties, $pn_status, $pn_access, $pa_values = null, $pa_options = null
	) {
		if ( ! ( $vn_representation_id = $this->getPrimaryKey() ) ) {
			return null;
		}
		...
		if ( $t_annotation->getPrimaryKey()
		     && ( $t_annotation->get( 'representation_id' ) == $vn_representation_id )
		) {
			foreach ( $o_coder->getPropertyList() as $vs_property ) {
				$t_annotation->setPropertyValue( $vs_property, $o_coder->getProperty( $vs_property ) );
			}
	 ...
	 */
	protected function getRepresentationAnnotationHTMLFormBundle(
		...
		if ( sizeof( $va_items = $this->getAnnotations() ) ) {
			...
			foreach ( $va_items as $vn_id => $va_item ) {
				...
				$va_inital_values[ $va_item[ $t_item->primaryKey() ] ] = array_merge( $va_item, array(
					'id'                   => $va_item[ $vs_rel_pk ],
					'item_type_id'         => $va_item['item_type_id'],
	...
	protected function _processRepresentationAnnotations( $po_request, $ps_form_prefix, $ps_placement_code ) {
		...
		foreach ( $va_rel_items as $vn_id => $va_rel_item ) {
			$this->clearErrors();
			if ( strlen( $vn_status = $po_request->getParameter( $ps_placement_code . $ps_form_prefix . '_status_'
			                                                     . $va_rel_item['annotation_id'], pString ) )
			) {
				$vn_access    = $po_request->getParameter( $ps_placement_code . $ps_form_prefix . '_access_'
				                                           . $va_rel_item['annotation_id'], pInteger );
				$vn_locale_id = $po_request->getParameter( $ps_placement_code . $ps_form_prefix . '_locale_id_'
				                                           . $va_rel_item['annotation_id'], pInteger );

				$va_properties = array();
				foreach ( $o_coder->getPropertyList() as $vs_property ) {
					$va_properties[ $vs_property ] = $po_request->getParameter(
						$x = $ps_placement_code . $ps_form_prefix . '_' . $vs_property . '_'
						     . $va_rel_item['annotation_id'], pString );
				}

				// edit annotation
				$this->editAnnotation( $va_rel_item['annotation_id'], $vn_locale_id, $va_properties, $vn_status,
					$vn_access );

				if ( $this->numErrors() ) {
					$po_request->addActionErrors( $this->errors(), $vs_annotation_table,
						$va_rel_item['annotation_id'] );
				} else {
					// try to add/edit label
					if ( $vs_label = $po_request->getParameter( $ps_placement_code . $ps_form_prefix . '_label_'
					                                            . $va_rel_item['annotation_id'], pString )
					) {
						...
						if ( $t_annotation->getPrimaryKey() ) {
							...
							if ( sizeof( $va_pref_labels ) ) {
								...
								foreach ( $va_pref_labels as $vn_annotation_dummy_id => $va_labels_by_locale ) {
									foreach ( $va_labels_by_locale as $vn_locale_dummy_id => $va_labels ) {
										$t_annotation->editLabel( $va_labels[0]['label_id'],
											array( 'name' => $vs_label ), $vn_locale_id, null, true );
									}
								}
							} else {
								// create new label
							...
							if ( $t_annotation->numErrors() ) {
								$po_request->addActionErrors( $t_annotation->errors(), $vs_annotation_table,
									'new_' . $vn_c );
								$vn_c ++;
							}
						}
				...
				// is it a delete key?
				$this->clearErrors();
				if ( ( $po_request->getParameter( $ps_placement_code . $ps_form_prefix . '_'
				                                  . $va_rel_item['annotation_id'] . '_delete', pInteger ) ) > 0
				) {
					// delete!
		...
		foreach ( $_REQUEST as $vs_key => $vs_value ) {
			if ( ! preg_match( '/^' . $ps_placement_code . $ps_form_prefix . '_status_new_([\d]+)/', $vs_key,
				$va_matches )
			) {
				continue;
			}
			$vn_c = intval( $va_matches[1] );
			if ( strlen( $vn_status = $po_request->getParameter( $ps_placement_code . $ps_form_prefix . '_status_new_'
			                                                     . $vn_c, pString ) ) > 0
			) {
				$vn_access    = $po_request->getParameter( $ps_placement_code . $ps_form_prefix . '_access_new_'
				                                           . $vn_c, pInteger );
				$vn_locale_id = $po_request->getParameter( $ps_placement_code . $ps_form_prefix . '_locale_id_new_'
				                                           . $vn_c, pInteger );

				$va_properties = array();
				foreach ( $o_coder->getPropertyList() as $vs_property ) {
					$va_properties[ $vs_property ] = $po_request->getParameter( $ps_placement_code . $ps_form_prefix
					                                                            . '_' . $vs_property . '_new_' . $vn_c,
						pString );
				}

				// create annotation
				$vs_label         = $po_request->getParameter( $ps_placement_code . $ps_form_prefix . '_label_new_'
				                                               . $vn_c, pString );
				$vn_annotation_id = $this->addAnnotation( $vs_label, $vn_locale_id, $po_request->getUserID(),
					$va_properties, $vn_status, $vn_access );
	...
	public function addFile( $ps_filepath, $ps_resource_path = '/', $pb_allow_duplicates = true ) {
		...
		if ( ! $pb_allow_duplicates ) {
			if ( $t_multifile->load( array(
				'resource_path'     => $ps_resource_path,
	...
	public function removeFile( $pn_multifile_id ) {
		...
		if ( $t_multifile->get( 'representation_id' ) == $this->getPrimaryKey() ) {
			...
			$this->postError( 2720, _t( 'File is not part of this representation' ),
				'ca_object_representations->removeFile()' );
	...
	/**
	 * Returns list of additional files (page or frame previews for documents or videos, typically) attached to a
	 * representation The return value is an array key'ed on the multifile_id (a unique identifier for each attached
	 * file); array values are arrays with keys set to values for each file version returned. They keys are:
	 ...
	 *
	 * @param int   $pn_representation_id The representation_id of the representation to return files for. If omitted
	 *                                    the currently loaded representation is used. If no representation_id is
	 *                                    specified and no row is loaded null will be returned.
	 * @param int   $pn_start             The index of the first file to return. Files are numbered from zero. If
	 *                                    omitted the first file found is returned.
	 * @param int   $pn_num_files         The maximum number of files to return. If omitted all files are returned.
	 * @param array $pa_versions          A list of file versions to return. If omitted only the "preview" version is
	 *                                    returned.
	 *
	 * @return array A list of files attached to the representations. If no files are associated an empty array is
	 *               returned.
	 */
	public function getFileList(
		$pn_representation_id = null, $pn_start = null, $pn_num_files = null, $pa_versions = null
	) {
	...
	public function addCaptionFile( $ps_filepath, $pn_locale_id, $pa_options = null ) {
		...
		$t_caption->set( 'caption_file', $ps_filepath,
			array( 'original_filename' => caGetOption( 'originalFilename', $pa_options, array_pop( $va_tmp ) ) ) );
	...
	public function removeCaptionFile( $pn_caption_id ) {
		...
		if ( $t_caption->get( 'representation_id' ) == $this->getPrimaryKey() ) {
			...
			$this->postError( 2720, _t( 'Caption file is not part of this representation' ),
				'ca_object_representations->removeCaptionFile()' );
	 ...
	 * @param array $pa_options
	 *
	 * @return array A list of caption files attached to the representations. If no files are associated an empty array
	 *               is returned.
	 */
	public function getCaptionFileList( $pn_representation_id = null, $pa_locale_ids = null, $pa_options = null ) {
		...
		while ( $qr_res->nextRow() ) {
			...
			if ( file_exists( $va_files[ $vn_caption_id ]['path'] ) ) {
				$va_files[ $vn_caption_id ]['filesize']
					= caFormatFileSize( filesize( $va_files[ $vn_caption_id ]['path'] ) );
			}
	...
	# ------------------------------------------------------
	/**
	 * Matching method to ca_objects::getRepresentations(), except this one only returns a single representation - the
		...
		while ( $qr_reps->nextRow() ) {
			...
			foreach ( $pa_versions as $vs_version ) {
				...
				if ( $vn_width && $vn_height ) {
					$va_tmp['tags'][ $vs_version ] = $qr_reps->getMediaTag( 'media', $vs_version,
						array_merge( $pa_options,
							array( 'viewer_width' => $vn_width, 'viewer_height' => $vn_height ) ) );
				} else {
				...
				$va_dimensions = array();
				if ( isset( $va_tmp['info'][ $vs_version ]['WIDTH'] )
				     && isset( $va_tmp['info'][ $vs_version ]['HEIGHT'] )
				) {
					if ( ( $vn_w = $va_tmp['info'][ $vs_version ]['WIDTH'] )
					     && ( $vn_h
							= $va_tmp['info'][ $vs_version ]['HEIGHT'] )
					) {
						$va_dimensions[] = "{$vn_w}p x {$vn_h}p";
					}
				}
				if ( isset( $va_tmp['info'][ $vs_version ]['PROPERTIES']['bitdepth'] )
				     && ( $vn_depth
						= $va_tmp['info'][ $vs_version ]['PROPERTIES']['bitdepth'] )
				) {
					$va_dimensions[] = intval( $vn_depth ) . ' bpp';
				}
				if ( isset( $va_tmp['info'][ $vs_version ]['PROPERTIES']['colorspace'] )
				     && ( $vs_colorspace
						= $va_tmp['info'][ $vs_version ]['PROPERTIES']['colorspace'] )
				) {
					$va_dimensions[] = $vs_colorspace;
				}
				if ( isset( $va_tmp['info'][ $vs_version ]['PROPERTIES']['resolution'] )
				     && is_array( $va_resolution = $va_tmp['info'][ $vs_version ]['PROPERTIES']['resolution'] )
				) {
					if ( isset( $va_resolution['x'] ) && isset( $va_resolution['y'] ) && $va_resolution['x']
					     && $va_resolution['y']
					) {
				...
				if ( isset( $va_tmp['info'][ $vs_version ]['PROPERTIES']['duration'] )
				     && ( $vn_duration
						= $va_tmp['info'][ $vs_version ]['PROPERTIES']['duration'] )
				) {
					$va_dimensions[] = sprintf( "%4.1f", $vn_duration ) . 's';
				}
				if ( isset( $va_tmp['info'][ $vs_version ]['PROPERTIES']['pages'] )
				     && ( $vn_pages
						= $va_tmp['info'][ $vs_version ]['PROPERTIES']['pages'] )
				) {
					$va_dimensions[] = $vn_pages . ' ' . ( ( $vn_pages == 1 ) ? _t( 'page' ) : _t( 'pages' ) );
				}
				if ( ! isset( $va_tmp['info'][ $vs_version ]['PROPERTIES']['filesize'] )
				     || ! ( $vn_filesize
						= $va_tmp['info'][ $vs_version ]['PROPERTIES']['filesize'] )
				) {
			...
			if ( isset( $va_info['INPUT']['FETCHED_FROM'] )
			     && ( $vs_fetched_from_url
					= $va_info['INPUT']['FETCHED_FROM'] )
			) {
	...
	public function getRepresentationMediaForIDs( $pa_ids, $pa_versions, $pa_options = null ) {
		...
		while ( $qr_res->nextRow() ) {
			$va_media_tags = array();
			foreach ( $pa_versions as $vs_version ) {
				$va_media_tags['tags'][ $vs_version ] = $qr_res->getMediaTag( 'ca_object_representations.media',
					$vs_version );
				$va_media_tags['info'][ $vs_version ] = $qr_res->getMediaInfo( 'ca_object_representations.media',
					$vs_version );
				$va_media_tags['urls'][ $vs_version ] = $qr_res->getMediaUrl( 'ca_object_representations.media',
					$vs_version );
			}
	...
	/**
	 * Checks if currently loaded representation is of specified media class. Valid media classes are 'image', 'audio',
	 * 'video' and 'document'
	 *
	 ...
	 */
	public function getRepresentationViewerHTMLBundle(
		$po_request, $pa_options = null, $pa_additional_display_options = null
	) {
	 ...
	 */
	public function getMediaDisplayHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pa_bundle_settings = null, $pa_options = null
	) {
		global $g_ui_locale;
	 ...
	 * @return string Rendered HTML bundle
	 */
	public function getCaptionHTMLFormBundle(
		$po_request, $ps_form_name, $ps_placement_code, $pa_bundle_settings = null, $pa_options = null
	) {
		global $g_ui_locale;
	 ...
	 *
	 * @return mixed ca_object_representations instance representing the first representation that contains the file,
	 *               if representation exists with this file, false if the file does not yet exist
	 */
	 ...
	 *
	 * @param string $ps_class   The class of representation to return a count for. Valid classes are "image", "audio",
	 *                           "video" and "document"
	 ...
	 * Provided interface compatibility with RepresentableBaseModel classes.
	 *
	 * @param string $ps_class   The class of representation to return information for. Valid classes are "image",
	...
	/**
	 * Returns number of representations (always 1). Provided interface compatibility with RepresentableBaseModel
	 * classes.
	 *
	 * @param array $pa_options No options are currently supported
	 *
	 ...
	 * @param bool   $complete Indicates the transcription should be marked as complete. Once marked, it can only be
	 *                         unmarked by setting the uncomplete option.
	 * @param array  $options  Options include:
	 *                         user_id = Marks transcription with specified user_id. The user_id will also be used to
	 *                         determine if a transcription for the current user already exists. If null, only client
	 *                         IP address will be used to find existing transcriptions. [Default is null] uncomplete =
	 *                         Force the completed on flag for the transcription to be unset. [Default is false]
	 *
	 * @return ca_representation_transcriptions instance of transcript, null if no representation is loaded or false if
	 *                                          there was an error.
	 */
	 ...
	 *
	 * @param array $options Options include:
	 *                       user_id = Finds transcription with specified user_id. The user_id will also be used to
	 *                       determine if a transcription for the current user already exists. If null, only client IP
	 *                       address will be used to find existing transcriptions. [Default is null]
	 *
	 * @return ca_representation_transcriptions instance of transcript, null if no representation is loaded or a
	...
	public function getTranscription( $options = null ) {
		...
		if (
			! ( $user_id
			    && ( $transcript = ca_representation_transcriptions::find( [
	...
	/**
	 * Permanently deletes the transcription specified by $transcription_id. Will only delete transcriptions attached
	 * to the currently loaded row. If you attempt to delete a transcription_id not attached to the current row
	 * removeTranscription() will return false and post an error. If you attempt to call removeTranscription() with no
	 * row loaded null will be returned. If the user_id option is specified only transcriptions created by the
	 * specified user will be deleted; if the transcription being deleted is not created by the user then false is
	 * returned and an error posted.
	 *
	 * @param $transcription_id [integer] a valid transcription to be removed; must be related to the currently loaded
	 *                          row (mandatory)
	 * @param $options          Options include:
	 *                          force = Remove transcription even if it's not part of the current representation or
	 *                          created by the specified user. [Default is false] user_id = a valid ca_users.user_id
	 *                          value; if specified then only transcriptions by the specified user will be deleted
	...
	public function removeTranscription( $transcription_id, $options = null ) {
		...
		if ( ! $transcription->getPrimaryKey() ) {
			$this->postError( 2800, _t( 'Transcription id is invalid' ),
				'ca_object_representations->removeTranscription()', 'ca_representation_transcriptions' );
		...
		if ( ! $force ) {
			if ( $transcription->get( 'representation_id' ) != $rep_id ) {
				$this->postError( 2810, _t( 'Transcription is not part of representation' ),
					'ca_object_representations->removeTranscription()', 'ca_representation_transcriptions' );
			...
			if ( $user_id ) {
				if ( $transcription->get( 'user_id' ) != $user_id ) {
					$this->postError( 2820, _t( 'Transcription was not created by specified user' ),
						'ca_object_representations->removeTranscription()', 'ca_representation_transcriptions' );
	...
	/**
	 * Returns all transcriptions associated with the currently loaded row. Will return null if not row is currently
	 * loaded. If the optional $moderation_status parameter is passed then only transcriptions matching the criteria
	 * will be returned: Passing $moderation_status = TRUE will cause only validted transcriptions to be returned
	 * Passing $moderation_status = FALSE will cause only unvalidated transcriptions to be returned If you want both
	 * validated and unvalidated transcriptions to be returned then omit the parameter or pass a null value
	 *
	 ...
	 *                       array                   = an array of comments searchResult            = a search result
	 *                       instance (aka. a subclass of BaseSearchResult), when the calling subclass is searchable
	 *                       (ie. <classname>Search and <classname>SearchResult classes are defined) ids
	 *                               = an array of ids (aka. primary keys) modelInstances            = an array of
	 *                               instances, one for each match. Each instance is the same class as the caller, a
	 *                               subclass of BaseModel firstId                    = the id (primary key) of the
	 *                               first match. This is the same as the first item in the array returned by 'ids'
	 *                               firstModelInstance        = the instance of the first match. This is the same as
	 *                               the first instance in the array returned by 'modelInstances' count                    = the number of matches
	 *
	 *            The default is array
	 *      request = the current request (optional)
	 *        user_id  = A valid ca_users.user_id value. If specified, only transcriptions by the specified user will
	 *        be returned. (optional - default is null)
	 *
	 * @return array
	 */
	public function getTranscriptions( $moderation_status = null, $options = null ) {
		...
		if ( ! is_null( $moderation_status ) ) {
			$moderation_sql = ( $moderation_status ) ? ' AND (t.validated_on IS NOT NULL)'
				: ' AND (t.validated_on IS NULL)';
		}
		...
		switch ( $return_as ) {
			...
			default:
				...
				while ( $qr->nextRow() ) {
					...
					$transcriptions[ $transcription_id ]['created_on_display']
					                                     = caGetLocalizedDate( $transcriptions[ $transcription_id ]['created_on'] );
					$transcriptions[ $transcription_id ]['completed_on_display']
					                                     = caGetLocalizedDate( $transcriptions[ $transcription_id ]['completed_on'] );
					$transcriptions[ $transcription_id ]['transcription_duration']
					                                     = ( $transcriptions[ $transcription_id ]['completed_on'] > 0 )
					                                       && ( ( $transcriptions[ $transcription_id ]['completed_on']
					                                              - $transcriptions[ $transcription_id ]['created_on']
					                                              > 0 ) )
						? caFormatInterval( $transcriptions[ $transcription_id ]['completed_on']
						                    - $transcriptions[ $transcription_id ]['created_on'] ) : '';
					$transcriptions[ $transcription_id ]['validated_on_display']
					                                     = caGetLocalizedDate( $transcriptions[ $transcription_id ]['validated_on'] );

					if ( $transcriptions[ $transcription_id ]['user_id'] ) {
						$transcriptions[ $transcription_id ]['name']
							= trim( $transcriptions[ $transcription_id ]['fname'] . ' '
							        . $transcriptions[ $transcription_id ]['lname'] );
						$transcriptions[ $transcription_id ]['email']
							= $transcriptions[ $transcription_id ]['user_email'];
						$transcriptions[ $transcription_id ]['transcriber']
							= $transcriptions[ $transcription_id ]['fname'] . " "
							  . transcriptions[ $transcription_id ]['lname'] . " ("
							  . $transcriptions[ $transcription_id ]['email'] . ")";
					} else {
						$transcriptions[ $transcription_id ]['transcriber'] = _t( 'user at %1',
							$transcriptions[ $transcription_id ]['ip_addr'] );
					}
					$transcriptions[ $transcription_id ]['status_message'] = _t( 'Created %1 by %2',
						$transcriptions[ $transcription_id ]['created_on_display'],
						$transcriptions[ $transcription_id ]['transcriber'] );
					if ( $transcriptions[ $transcription_id ]['completed_on'] > 0 ) {
						$transcriptions[ $transcription_id ]['status_message'] .= ( $transcriptions[ $transcription_id ]['transcription_duration'] )
							? "<br/>" . _t( 'Completed on %1 (%2)',
								$transcriptions[ $transcription_id ]['completed_on_display'],
								$transcriptions[ $transcription_id ]['transcription_duration'] )
							: "<br>\n" . _t( 'Completed on %1',
								$transcriptions[ $transcription_id ]['completed_on_display'] );
					...
					if ( $transcriptions[ $transcription_id ]['validated_on'] > 0 ) {
						$transcriptions[ $transcription_id ]['status_message'] .= "; " . _t( 'validated on %1',
								$transcriptions[ $transcription_id ]['validated_on_display'] );
					}
				}
				break;
		...
		if ( caGetOption( 'sortDirection', $options, 'ASC',
	 ...
	 *
	 * @param array $options Options include:
	 *                       currentUser = Only consider transcriptions by the current user by user_id (if specified)
	 *                       and/or client IP address. [Default is false] user_id = Finds transcription with specified
	 *                       user_id. The user_id will also be used to determine if a transcription for the current
	 *                       user already exists. If null, only client IP address will be used to find existing
	 *                       transcriptions. [Default is null]
	 *
	 * @return bool
	 */
	public function transcriptionIsComplete( array $options = null ) {
		...
		if ( $current_user_only ) {
			...
			return ca_representation_transcriptions::find( [
				'representation_id' => $rep_id,
				'completed_on'      => [ '>', 0 ]
			], [ 'returnAs' => 'firstModelInstance' ] ) ? true : false;
		}
	 ...
	 */
	public function getTranscriptionHTMLFormBundle(
				...
				if ( ( $qr = caMakeSearchResult( 'ca_object_representations', [ $row_id ] ) ) && $qr->nextHit() ) {
					...
					switch ( $bundle_name ) {
						case 'media_dimensions':
							...
							break;
						case 'media_duration':
							if ( isset( $info[ $version ]['PROPERTIES']['duration'] )
							     && ( $duration
									= (float) $info[ $version ]['PROPERTIES']['duration'] )
							) {
						...
						case 'media_filesize':
							$filesize = null;
							if ( ! isset( $info[ $version ]['PROPERTIES']['filesize'] )
							     || ! ( $filesize
									= $info[ $version ]['PROPERTIES']['filesize'] )
							) {
							...
							break;
						case 'media_colorspace':
							if ( isset( $info[ $version ]['PROPERTIES']['colorspace'] )
							     && ( $colorspace
									= $info[ $version ]['PROPERTIES']['colorspace'] )
							) {
								return $colorspace;
							}
							break;
						case 'media_resolution':
							if ( isset( $info[ $version ]['PROPERTIES']['resolution'] )
							     && is_array( $resolution = $info[ $version ]['PROPERTIES']['resolution'] )
							) {
								if ( isset( $resolution['x'] ) && isset( $resolution['y'] ) && $resolution['x']
								     && $resolution['y']
								) {
							...
							break;
						case 'media_bitdepth':
							if ( isset( $info[ $version ]['PROPERTIES']['bitdepth'] )
							     && ( $depth
									= $info[ $version ]['PROPERTIES']['bitdepth'] )
							) {
