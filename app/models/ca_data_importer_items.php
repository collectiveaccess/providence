...
BaseModel::$s_ca_models_definitions['ca_data_importer_items'] = array(
    ...
    'FIELDS' => array(
        'item_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique numeric identifier used by CollectiveAccess internally to identify this importer item'
            )
        ),
        'importer_id' => array(
            'FIELD_TYPE' => FT_NUMBER,
        ...
        'group_id' => array(
            ...
            'DEFAULT' => '',
            'LABEL' => 'Group id',
            'DESCRIPTION' => 'Identifier for importer group'
        ),
        'source' => array(
            'FIELD_TYPE' => FT_TEXT,
            ...
            'DESCRIPTION' => _t(
                'Source in external format to map CollectiveAccess path to. The format of the external element is determined by the target. For XML-based formats this will typically be an XPath specification; for delimited targets this will be a column number.'
            ),
        ...
        'settings' => array(
            ...
            'DEFAULT' => '',
            'LABEL' => _t('Settings'),
            'DESCRIPTION' => _t('Importer item settings')
        )
    )
);
...
class ca_data_importer_items extends BaseModel
{
    ...
    public function initSettings($pa_settings = null)
    {
        ...
        $va_settings['refineries'] = array(
            ...
            'label' => _t('Refineries'),
            'description' => _t(
                'Select the refinery that preforms the correct function to alter your data source as it maps to CollectiveAccess.'
            )
        );
        $va_settings['original_values'] = array(
            ...
            'description' => _t(
                'Return-separated list of values from the data source to be replaced.  For example photo is used in the data source, but photograph is used in CollectiveAccess.'
            )
        );
        $va_settings['replacement_values'] = array(
            ...
            'description' => _t(
                'Return-separated list of CollectiveAccess list item idnos that correspond to the mapped values from the original data source.  For example sound recording (entered in the Original values column) maps to audio_digital, which is entered here in the Replacement values column.'
            )
        );
        ...
        $va_settings['skipGroupIfEmpty'] = array(
            ...
            'description' => _t(
                'Skip all of the elements in the group if value for this element is empty.  For example, a field called Description Type would be irrelevant if the Description field is empty.'
            )
        );
        $va_settings['skipGroupIfValue'] = array(
            ...
            'description' => _t(
                'Skip all of the elements in the group if value for this element is equal to the specified value(s).'
            )
        );
        $va_settings['skipGroupIfNotValue'] = array(
            ...
            'description' => _t(
                'Skip all of the elements in the group if value for this element is not equal to any of the specified values(s).'
            )
        );
        ...
        $va_settings['skipRowIfEmpty'] = array(
            ...
            'description' => _t(
                'Skip row if value for this element is empty.  For example, do not import the row if the Description field is empty.'
            )
        );
        ...
        $va_settings['filterToTypes'] = array(
            ...
            'description' => _t(
                'Restricts the mapping to pull only records related with the designated types from the source. This option is only supported by sources that have a notion of related data and types, most notably (and for now only) the CollectiveAccessDataReader.'
            )
        );
        $va_settings['filterToRelationshipTypes'] = array(
            ...
            'description' => _t(
                'Restricts the mapping to pull only records related with the designated relationship types from the source. This option is only supported by sources that have a notion of related data and relationship types, most notably (and for now only) the CollectiveAccessDataReader.'
            )
        );
        ...
        $va_settings['formatWithTemplate'] = array(
            ...
            'description' => _t(
                'Format imported value with provided template. Template may include caret (^) prefixed placeholders that refer to data source values.'
            )
        );
        $va_settings['applyRegularExpressions'] = array(
            ...
            'default' => '',
            'label' => _t(
                'Apply one or more regular expression-based substitutions to a soruce value prior to import.'
            ),
            'description' => _t(
                'A list of Perl-compatible regular expressions. Each expression has two parts, a matching expression and a substitution expression, and is expressed as a JSON object with <em>match</em> and <em>replaceWith</em> keys. Ex. [{"match": "([\\d]+)\\.([\\d]+)", "replaceWith": "\\1:\\2"}, {"match": "[^\\d:]+", "replaceWith": ""}] '
            )
        );
        ...
        $va_settings['useParentAsSubject'] = array(
            ...
            'description' => _t(
                'Import parent of subject instead of subject. This option is primarily useful when you are using a hierarchy builder refinery mapped to parent_id to create the entire hierarchy (including subject) and want the bottom-most level of the hierarchy to be the subject.'
            )
        );
        ...
        $va_settings['displaynameFormat'] = array(
            ...
            'description' => _t(
                'Transform label using options for formatting entity display names. Default is to use value as is. Other options are surnameCommaForename, forenameCommaSurname, forenameSurname. See DataMigrationUtils::splitEntityName().'
            )
        );
        $va_settings['useRawValuesWhenTestingExpression'] = array(
            ...
            'description' => _t(
                'When evaluating conditions to skip a mapping, row or group via an expression using a setting such as skipIfExpression, use raw un-transformed data rather than data that has been transformed with applyRegularExpressions and replacement values.'
            )
        );
        ...
        $va_settings['matchType'] = array(
            ...
            'description' => _t(
                'Determines how file names are compared to the match value. Valid values are STARTS, ENDS, CONTAINS and EXACT. (Default is EXACT)'
            )
        );
        $va_settings['matchMode'] = array(
            ...
            'description' => _t(
                'Determines whether to search on file names, enclosing directory names or both. Valid values are DIRECTORY_NAME, FILE_AND_DIRECTORY_NAMES and FILE_NAME. (Default is FILE_NAME).'
            )
        );
