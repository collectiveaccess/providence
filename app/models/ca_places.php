...
BaseModel::$s_ca_models_definitions['ca_places'] = array(
	...
	'FIELDS'        => array(
		'place_id'             => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this place' )
		),
		'parent_id'            => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_HIDDEN,
		...
		'locale_id'            => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Locale' ),
			'DESCRIPTION'    => _t( 'The primary locale associated with the place.' )
		),
		'type_id'              => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_SELECT,
			...
			'LABEL'          => _t( 'Type' ),
			'DESCRIPTION'    => _t( 'The type of the place. In CollectiveAccess every place has a single "instrinsic" type that determines the set of descriptive and administrative metadata that can be applied to it.' )
		),
		'source_id'            => array(
			'FIELD_TYPE'                => FT_NUMBER,
			'DISPLAY_TYPE'              => DT_SELECT,
			...
			'LABEL'                     => _t( 'Source' ),
			'DESCRIPTION'               => _t( 'Administrative source of the place. This value is often used to indicate the administrative sub-division or legacy database from which the place information originates, but can also be re-tasked for use as a simple classification tool if needed.' )
		),
		'hierarchy_id'         => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'LABEL'          => 'Place hierarchy',
			'DESCRIPTION'    => 'Hierarchy this place belongs to.'
		),
		'idno'                 => array(
			'FIELD_TYPE'                => FT_TEXT,
			...
			'DEFAULT'                   => '',
			'ALLOW_BUNDLE_ACCESS_CHECK' => true,
			'LABEL'                     => _t( 'Place identifier' ),
			'DESCRIPTION'               => _t( 'Unique identifier for place' ),
		...
		'source_info'          => array(
			...
			'DESCRIPTION'    => 'Serialized array used to store source information for place information retrieved via web services [NOT IMPLEMENTED YET].'
		),
		'lifespan'             => array(
			...
			'ALLOW_BUNDLE_ACCESS_CHECK' => true,
			'DEFAULT'                   => '',
			'START'                     => 'lifespan_sdate',
			'END'                       => 'lifespan_edate',
			'LABEL'                     => _t( 'Lifespan' ),
			'DESCRIPTION'               => _t( 'Lifespan of place (range of dates)' )
		),
		'access'               => array(
			'FIELD_TYPE'                => FT_NUMBER,
			'DISPLAY_TYPE'              => DT_SELECT,
			...
			'LABEL'                     => _t( 'Access' ),
			'DESCRIPTION'               => _t( 'Indicates if place information is accessible to the public or not. ' )
		),
		'status'               => array(
			'FIELD_TYPE'                => FT_NUMBER,
			'DISPLAY_TYPE'              => DT_SELECT,
			...
			'LABEL'                     => _t( 'Status' ),
			'DESCRIPTION'               => _t( 'Indicates the current state of the place record.' )
		),
		'floorplan'            => array(
			'FIELD_TYPE'               => FT_MEDIA,
			'DISPLAY_TYPE'             => DT_FIELD,
			...
			'LABEL'                    => _t( 'Floor plan' ),
			'DESCRIPTION'              => _t( 'Optional floor plan graphic to associate with place.' )
		),
		'deleted'              => array(
			'FIELD_TYPE'     => FT_BIT,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => 0,
			'LABEL'          => _t( 'Is deleted?' ),
			'DESCRIPTION'    => _t( 'Indicates if the place is deleted or not.' ),
...
class ca_places extends RepresentableBaseModel implements IBundleProvider, IHierarchy {
	...
	public function getHierarchyList( $pb_dummy = false ) {
		...
		while ( $qr_res->nextRow() ) {
			...
			$va_place_hierarchies[ $vn_hierarchy_id ]['place_id']
			                                                      =
			$va_place_hierarchies[ $vn_hierarchy_id ]['item_id'] = $qr_res->get( 'place_id' );
			$va_place_hierarchies[ $vn_hierarchy_id ]['name']
			                                                      = $va_place_hierarchies[ $vn_hierarchy_id ]['name_plural'];
	...
	/**
	 * Returns name of hierarchy for currently loaded place or, if specified, place with place_id = to optional $pn_id
	 * parameter
	 */
	...
	/**
	 * Returns the place_id in ca_places table for root of specified hierarchy. Place hierarchies are enumerated in the
	 * "place_hierarchies" list, so hierarchy specifications are really just plain old list items (ca_list_items
	 * records). You can specify a hierarchy using an hierarchy id
	 * (really just a ca_list_items item_id value for the item representing the hierarchy), or by passing the idno of
	 * the hierarchy (eg. ca_list_items.idno).
	 *
	 * @param mixed $pm_hierarchy_code_or_id The numeric id or alphanumeric code for the hierarchy. Since hierarchies
	 *                                       are represented with list items these are the item_id or idno values of
	 *                                       the hierarchy's list item.
	 *
