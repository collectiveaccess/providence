...
BaseModel::$s_ca_models_definitions['ca_places'] = array(
    ...
    'FIELDS' => array(
        ...
        'type_id' => array(
            ...
            'LABEL' => _t('Type'),
            'DESCRIPTION' => _t(
                'The type of the place. In CollectiveAccess every place has a single "instrinsic" type that determines the set of descriptive and administrative metadata that can be applied to it.'
            )
        ),
        'source_id' => array(
            'FIELD_TYPE' => FT_NUMBER,
            ...
            'LABEL' => _t('Source'),
            'DESCRIPTION' => _t(
                'Administrative source of the place. This value is often used to indicate the administrative sub-division or legacy database from which the place information originates, but can also be re-tasked for use as a simple classification tool if needed.'
            )
        ),
        'hierarchy_id' => array(
            'FIELD_TYPE' => FT_NUMBER,
...
class ca_places extends RepresentableBaseModel implements IBundleProvider, IHierarchy
{
    ...
    public function getHierarchyList($pb_dummy = false)
    {
        ...
        while ($qr_res->nextRow()) {
            ...
            $va_place_hierarchies[$vn_hierarchy_id]['place_id'] = $va_place_hierarchies[$vn_hierarchy_id]['item_id'] = $qr_res->get(
                'place_id'
            );
