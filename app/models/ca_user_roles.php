...
BaseModel::$s_ca_models_definitions['ca_user_roles'] = array(
	...
	'FIELDS'        => array(
		'role_id'      => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Role id' ),
			'DESCRIPTION'    => _t( 'Unique identifier for role' )
		),
		'name'         => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Name' ),
			'DESCRIPTION'    => _t( 'Name of role (must be unique)' ),
		...
		'code'         => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Code' ),
			'DESCRIPTION'    => _t( 'Short code (up to 20 characters) for role (must be unique)' ),
		...
		'description'  => array(
			...
			'DESCRIPTION'    => _t( 'Description of role. This text will be displayed to system administrators only and should clearly document the purpose of the role.' ),
		...
		'rank'         => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Sort order' ),
			'DESCRIPTION'    => _t( 'The relative priority of the role when displayed in a list with other roles. Lower numbers indicate higher priority.' ),
		...
		'vars'         => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Role settings' ),
			'DESCRIPTION'    => _t( 'Storage for role-level variables' )
		),
		'field_access' => array(
			'FIELD_TYPE'     => FT_VARS,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Field access' ),
			'DESCRIPTION'    => _t( 'Contains definitions of what table/field combinations users with this role can access.' )
		)
	)
);
...
class ca_user_roles extends BaseModel {
	...
	public function getRoleActions() {
		...
		$va_actions       = ( isset( $va_role_settings['actions'] ) && is_array( $va_role_settings['actions'] ) )
			? $va_role_settings['actions'] : array();
	 ...
	 * @param int    $pn_access access level, __CA_BUNDLE_ACCESS_NONE__, __CA_BUNDLE_ACCESS_READONLY__ or
	 *                          __CA_BUNDLE_ACCESS_EDIT__
	 *
	 * @return boolean success or not
	 */
	public function setAccessSettingForBundle( $ps_table, $ps_bundle, $pn_access ) {
		if ( ! in_array( $pn_access,
			array( __CA_BUNDLE_ACCESS_NONE__, __CA_BUNDLE_ACCESS_READONLY__, __CA_BUNDLE_ACCESS_EDIT__ ) )
		) {
			return false;
		}
		if ( ! $this->getPrimaryKey() ) {
		...
		if ( ! is_array( ca_user_roles::$s_bundle_list ) || ! is_array( ca_user_roles::$s_bundle_list[ $ps_table ] ) ) {
			...
			ca_user_roles::$s_bundle_list[ $ps_table ] = array_keys( $t_ui_screens->getAvailableBundles( $ps_table,
		...
		if ( ! ca_user_roles::$s_action_list ) {
			...
			foreach ( $va_raw_actions as $vs_group => $va_group_info ) {
				...
				foreach ( $va_group_info["actions"] as $vs_action_key => $va_action ) {
					if ( isset( $va_action['requires'] ) && is_array( $va_action['requires'] )
					     && ! ca_user_roles::_evaluateActionRequirements( $va_action['requires'] )
					) {
					...
					if ( is_array( $va_action["expand_types"] ) && strlen( $va_action["expand_types"]["table"] ) > 0 ) {
						...
						if ( method_exists( $t_instance, "getTypeList" ) ) {
							...
							foreach ( $va_type_list as $vn_type_id => $va_type ) {
								$vs_descr_app = str_replace( "%t", "&quot;" . $va_type["name_singular"] . "&quot;",
									$va_action["expand_types"]["description_appendix"] );
								$vs_label_app = str_replace( "%t", "&quot;" . $va_type["name_singular"] . "&quot;",
									$va_action["expand_types"]["label_appendix"] );
								$va_new_actions[ $vs_action_key
								                 . "_type:{$t_instance->tableName()}:{$va_type["idno"]}" ]
								              = array(
	...
	public static function getBundlesForRoleIDs( $pa_role_ids ) {
		...
		while ( $qr_res->nextRow() ) {
			$va_role_data = $qr_res->getVars( 'vars' );
			if ( isset( $va_role_data['bundle_access_settings'] )
			     && is_array( $va_role_data['bundle_access_settings'] )
			) {
