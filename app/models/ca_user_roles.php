...
BaseModel::$s_ca_models_definitions['ca_user_roles'] = array(
    ...
    'FIELDS' => array(
        ...
        'description' => array(
            ...
            'DESCRIPTION' => _t(
                'Description of role. This text will be displayed to system administrators only and should clearly document the purpose of the role.'
            ),
        ...
        'rank' => array(
            ...
            'LABEL' => _t('Sort order'),
            'DESCRIPTION' => _t(
                'The relative priority of the role when displayed in a list with other roles. Lower numbers indicate higher priority.'
            ),
        ...
        'field_access' => array(
            ...
            'DESCRIPTION' => _t(
                'Contains definitions of what table/field combinations users with this role can access.'
            )
        )
    )
);
...
class ca_user_roles extends BaseModel
{
    ...
    static public $s_action_list;
    static public $s_bundle_list;
    ...
    public function getRoleActions()
    {
        ...
        $va_actions = (isset($va_role_settings['actions']) && is_array(
                $va_role_settings['actions']
    ...
    public function setAccessSettingForBundle($ps_table, $ps_bundle, $pn_access)
    {
        ...
        if (!is_array(ca_user_roles::$s_bundle_list) || !is_array(ca_user_roles::$s_bundle_list[$ps_table])) {
            ...
            ca_user_roles::$s_bundle_list[$ps_table] = array_keys(
                $t_ui_screens->getAvailableBundles($ps_table, array('dontCache' => true))
            );
        }
        ...
        if (!ca_user_roles::$s_action_list) {
            ...
            foreach ($va_raw_actions as $vs_group => $va_group_info) {
                ...
                foreach ($va_group_info["actions"] as $vs_action_key => $va_action) {
                    if (isset($va_action['requires']) && is_array(
                            $va_action['requires']
                    ...
                    if (is_array($va_action["expand_types"]) && strlen($va_action["expand_types"]["table"]) > 0) {
                        ...
                        if (method_exists($t_instance, "getTypeList")) {
                            ...
                            foreach ($va_type_list as $vn_type_id => $va_type) {
                                $vs_descr_app = str_replace(
                                    "%t",
                                    "&quot;" . $va_type["name_singular"] . "&quot;",
                                    $va_action["expand_types"]["description_appendix"]
                                );
                                $vs_label_app = str_replace(
                                    "%t",
                                    "&quot;" . $va_type["name_singular"] . "&quot;",
