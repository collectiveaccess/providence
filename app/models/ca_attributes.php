...
class ca_attributes extends BaseModel
{
    ...
    static public $s_attribute_cache_size = 1024;
    static public $s_get_attributes_cache = array();
    static public $s_ca_attributes_element_instance_cache = array();
    ...
    public function addAttribute($pn_table_num, $pn_row_id, $pm_element_code_or_id, $pa_values, $pa_options = null)
    {
        ...
        foreach ($va_elements as $va_element) {
            ...
            if ((isset($va_element['settings']['isDependentValue']) && (bool)$va_element['settings']['isDependentValue']) && (is_null(
                    $vm_value
                ))) {
                $vm_value = caProcessTemplate($va_element['settings']['dependentValueTemplate'], $pa_values);
            }

            if (($vb_status = $t_attr_val->addValue(
                    $vm_value,
                    $va_element,
                    $vn_attribute_id,
                    array_merge($pa_options, ['t_attribute' => $this])
                )) === false) {
    ...
    public function editAttribute($pa_values, $pa_options = null)
    {
        ...
        foreach ($va_attr_vals as $o_attr_val) {
            ...
            if ($t_attr_val->load($o_attr_val->getValueID())) {
                $va_element = array_shift(
                    array_filter(
                        $va_elements,
                        function ($e) use ($vn_element_id) {
    ...
    public static function getElementInstance($pm_element_code_or_id)
    {
        ...
        if (!is_numeric($pm_element_code_or_id)) {
            if ($t_element->load(array('element_code' => $pm_element_code_or_id))) {
                return ca_attributes::$s_ca_attributes_element_instance_cache[$pm_element_code_or_id] = ca_attributes::$s_ca_attributes_element_instance_cache[$t_element->getPrimaryKey(
                )] = $t_element;
            }
        }
    ...
    public static function attributeHtmlFormElement($pa_element_info, $pa_options = null)
    {
        ...
        $ps_formatted_element = str_replace(
            "^LABEL",
            "<span class='_attribute_value_" . $pa_element_info['element_code'] . "'>{$vs_label}</span>",
            $ps_format
        );
        ...
        if ($vs_description) {
            ...
            $ps_formatted_element .= "\n" . caGetTooltipJS(
                    array('._attribute_value_' . $pa_element_info['element_code'] => $vs_description)
                );
        }
        ...
        foreach ($va_attrs as $vn_row_id => $va_attrs_by_element) {
            foreach ($va_attrs_by_element as $vn_element_id => $va_attrs_for_element) {
                ...
                if (sizeof(ca_attributes::$s_get_attributes_cache) > ca_attributes::$s_attribute_cache_size) {
                    ...
                    if (($vn_splice_length = ceil(
                            sizeof(
                                ca_attributes::$s_get_attributes_cache
                            ) - ca_attributes::$s_attribute_cache_size + (ca_attributes::$s_attribute_cache_size * 0.5)
    ...
    public static function getAttributes($po_db, $pn_table_num, $pn_row_id, $pa_element_ids, $pa_options = null)
    {
        ...
        if (sizeof($va_element_ids)) {
            if (!(ca_attributes::prefetchAttributes(
                $po_db,
                $pn_table_num,
                array($pn_row_id),
                $va_element_ids,
     ...
     */
    public static function getRawAttributeValuesForIDs(
        $po_db,
        $pn_table_num,
        $pa_row_ids,
        $pn_element_id,
     ...
     * @return array Array of values indexed on row_id, then locale_id and finally an index (to accommodate repeating values)
     */
    public static function getAttributeValueForIDs(
        $po_db,
        $pn_table_num,
        $pa_row_ids,
        $pn_element_id,
    ...
    public function getRowInstance()
    {
        ...
        if (($t_instance = Datamodel::getInstanceByTableNum($this->get('table_num'), true)) && ($t_instance->load(
                $this->get('row_id')
            ))) {
            return $t_instance;
        }
    ...
    public static function getRowInstanceForValueID($pn_value_id, $pa_options = null)
    {
        ...
        while ($qr_res->nextRow()) {
            if (($t_instance = Datamodel::getInstanceByTableNum($qr_res->get('table_num'), true)) && ($t_instance->load(
                    $qr_res->get('row_id')
                ))) {
                return $t_instance;
            }
