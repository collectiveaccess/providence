...
BaseModel::$s_ca_models_definitions['ca_attributes'] = array(
	...
	'FIELDS'        => array(
		...
		'locale_id'    => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Locale' ),
			'DESCRIPTION'    => _t( 'The locale best describing the origin of this information.' )
		),
		'table_num'    => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Table',
			'DESCRIPTION'    => 'Table to which this attribute is applied.',
		...
		'row_id'       => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Row id',
			'DESCRIPTION'    => 'Identifier of row to which this attibute is applied.'
		)
	)
);
...
class ca_attributes extends BaseModel {
	...
	public function addAttribute( $pn_table_num, $pn_row_id, $pm_element_code_or_id, $pa_values, $pa_options = null ) {
		...
		require_once( __CA_MODELS_DIR__
		              . '/ca_metadata_elements.php' );    // defer inclusion until runtime to ensure baseclasses are already loaded, otherwise you get circular dependencies
		...
		foreach ( $va_elements as $va_element ) {
			...
			if ( isset( $pa_values[ $va_element['element_id'] ] ) ) {
				...
				$vm_value = isset( $pa_values[ $va_element['element_code'] ] )
					? $pa_values[ $va_element['element_code'] ] : null;
			}

			if ( ( isset( $va_element['settings']['isDependentValue'] )
			       && (bool) $va_element['settings']['isDependentValue'] )
			     && ( is_null( $vm_value ) )
			) {
				$vm_value = caProcessTemplate( $va_element['settings']['dependentValueTemplate'], $pa_values );
			}

			if ( ( $vb_status = $t_attr_val->addValue( $vm_value, $va_element, $vn_attribute_id,
					array_merge( $pa_options, [ 't_attribute' => $this ] ) ) ) === false
			) {
				$this->postError( 1972, join( '; ', $t_attr_val->getErrors() ), 'ca_attributes->addAttribute()' );
				$vb_dont_create_attribute
					= false;    // this causes an error to be displayed to the user, which is what we want here
				break;
			}
	 ...
	 *                          attributeInstance        = an instance of the Attribute class loaded with the attribute
	 *                          value(s) count                    = the number of values in the attribute
	 *
	 * @return mixed An array, instance of class Attribute or an integer value count depending upon setting of returnAs
	 *               option. Returns null if no attribute is loaded.
	 */
	...
	static public function getElementInstance( $pm_element_code_or_id ) {
		...
		if ( ! is_numeric( $pm_element_code_or_id ) ) {
			if ( $t_element->load( array( 'element_code' => $pm_element_code_or_id ) ) ) {
				return ca_attributes::$s_ca_attributes_element_instance_cache[ $pm_element_code_or_id ]
					=
				ca_attributes::$s_ca_attributes_element_instance_cache[ $t_element->getPrimaryKey() ] = $t_element;
			}
		}
	...
	static public function attributeHtmlFormElement( $pa_element_info, $pa_options = null ) {
		...
		$ps_formatted_element = str_replace( "^LABEL",
			"<span class='_attribute_value_" . $pa_element_info['element_code'] . "'>{$vs_label}</span>", $ps_format );
		...
		if ( $vs_description ) {
			...
			$ps_formatted_element .= "\n" . caGetTooltipJS( array(
					'._attribute_value_' . $pa_element_info['element_code'] => $vs_description
				) );
		}
	...
	static public function prefetchAttributes( $po_db, $pn_table_num, $pa_row_ids, $pa_element_ids, $pa_options = null
		...
		while ( $qr_attrs->nextRow() ) {
			...
			if ( $vn_last_attribute_id != $va_raw_row['attribute_id'] ) {
				...
				$o_attr = new Attribute( array_merge( $va_raw_row,
					array( 'element_id' => $va_raw_row['element_set_id'] ) ) );
			}
		...
		foreach ( $va_attrs as $vn_row_id => $va_attrs_by_element ) {
			foreach ( $va_attrs_by_element as $vn_element_id => $va_attrs_for_element ) {
				...
				ca_attributes::$s_get_attributes_cache[ (int) $pn_table_num . '/'
				                                        . (int) $vn_row_id ][ (int) $vn_element_id ]
					= $va_attrs_for_element;
				// Limit cache size
				if ( sizeof( ca_attributes::$s_get_attributes_cache ) > ca_attributes::$s_attribute_cache_size ) {
					...
					if ( ( $vn_splice_length = ceil( sizeof( ca_attributes::$s_get_attributes_cache )
					                                 - ca_attributes::$s_attribute_cache_size
					                                 + ( ca_attributes::$s_attribute_cache_size * 0.5 ) ) )
					     > ca_attributes::$s_attribute_cache_size
					) {
			...
			foreach ( $pa_element_ids as $vn_id ) {
				if ( ! isset( ca_attributes::$s_get_attributes_cache[ (int) $pn_table_num . '/'
				                                                      . (int) $vn_row_id ][ $vn_id ] )
				) {
					ca_attributes::$s_get_attributes_cache[ (int) $pn_table_num . '/' . (int) $vn_row_id ][ $vn_id ]
						= false;
				}
			}
		}
	 ...
	 *
	 * @return array Array of values indexed on row_id, then locale_id and finally an index (to accommodate repeating
	 *               values)
	 */
	static public function getRawAttributeValuesForIDs(
		$po_db, $pn_table_num, $pa_row_ids, $pn_element_id, $pa_options = null
	) {
		$qr_attrs = $po_db->query( "
			SELECT 
				caa.attribute_id, caa.locale_id, caa.element_id element_set_id, caa.row_id,
	...
	static public function getAttributeCount( $po_db, $pn_table_num, $pn_row_id, $pn_element_id, $pa_options = null ) {
		...
		$qr_attrs          = $po_db->query( "
			...
			WHERE
				...
				" . ( ! $pb_include_blanks
				? ( "AND (cav.item_id IS NOT NULL OR cav.value_longtext1 IS NOT NULL OR cav.value_decimal1 IS NOT NULL OR cav.value_integer1 IS NOT NULL OR cav.value_blob IS NOT NULL)" )
				: "" ) . "
	 ...
	 *               values)
	 */
	static public function getAttributeValueForIDs(
		$po_db, $pn_table_num, $pa_row_ids, $pn_element_id, $pa_options = null
	) {
		$vb_is_cached = true;
		foreach ( $pa_row_ids as $vn_row_id ) {
			if ( ! is_array( ca_attributes::$s_get_attributes_cache[ $pn_table_num . '/'
			                                                         . $vn_row_id ][ $pn_element_id ] )
			) {
				$vb_is_cached = false;
				break;
			}
		...
		if ( ! $vb_is_cached ) {
			if ( ! ( ca_attributes::prefetchAttributes( $po_db, $pn_table_num, $pa_row_ids,
				array( $pn_element_id ) ) )
			) {
				return null;
			}
				...
				foreach ( $va_elements as $vn_j => $o_attr ) {
					if ( (int) $o_attr->getElementID() === (int) $pn_element_id ) {
						...
						foreach ( $va_attr_values as $va_attr_value ) {
							$va_values[ $vn_row_id ][ $vn_locale_id ][]
								= $va_attr_value->getDisplayValue( $pa_options );
						}
					}
				}
			}
	...
	public function getRowInstance() {
		...
		if ( ( $t_instance = Datamodel::getInstanceByTableNum( $this->get( 'table_num' ), true ) )
	 ...
	 *                          db = use provided database connection. [Default is null; use new connection which may
	 *                          or may not be the most recently created one.]
	 *
	...
	public static function getAttributeForValueID( $pn_value_id, $pa_options = null ) {
		...
		$qr_res = $o_db->query( "SELECT attribute_id FROM ca_attribute_values WHERE value_id = ?",
			[ (int) $pn_value_id ] );
	...
	public static function getRowInstanceForValueID( $pn_value_id, $pa_options = null ) {
		...
		while ( $qr_res->nextRow() ) {
			if ( ( $t_instance = Datamodel::getInstanceByTableNum( $qr_res->get( 'table_num' ), true ) )
	...
	public static function getTableNumForAttribute( $pn_attribute_id, $po_db = null ) {
		...
		if ( $qr_res = $o_db->query( "SELECT table_num FROM ca_attributes WHERE attribute_id = ?",
			[ $pn_attribute_id ] )
		) {
			if ( $qr_res->nextRow() ) {
