...
BaseModel::$s_ca_models_definitions['ca_object_checkouts'] = array(
	...
	'FIELDS'        => array(
		'checkout_id'                      => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Checkout id' ),
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this object checkout entry' )
		),
		'group_uuid'                       => array(
			'FIELD_TYPE'     => FT_TEXT,
			...
			'LABEL'          => 'Group UUID',
			'DESCRIPTION'    => 'UUID for group checkout is part of.'
		),
		'object_id'                        => array(
			'FIELD_TYPE'     => FT_NUMBER,
			'DISPLAY_TYPE'   => DT_FIELD,
			...
			'DEFAULT'        => '',
			'LABEL'          => 'Object ID',
			'DESCRIPTION'    => 'The id of the object that was checked out.'
		),
		'user_id'                          => array(
			'FIELD_TYPE'     => FT_NUMBER,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'User' ),
			'DESCRIPTION'    => _t( 'The user who checked out the object.' )
		),
		'created_on'                       => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Created on' ),
			'DESCRIPTION'    => _t( 'Date/time the checkout entry was created.' ),
		),
		'checkout_date'                    => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Checkout date' ),
			'DESCRIPTION'    => _t( 'Date/time the item was checked out.' ),
		),
		'due_date'                         => array(
			'FIELD_TYPE'     => FT_DATETIME,
			'DISPLAY_TYPE'   => DT_FIELD,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Date due' ),
			'DESCRIPTION'    => _t( 'Date/time the item is due to be returned.' ),
		),
		'return_date'                      => array(
			'FIELD_TYPE'     => FT_DATETIME,
			'DISPLAY_TYPE'   => DT_FIELD,
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Date returned' ),
			'DESCRIPTION'    => _t( 'Date/time the item was returned.' ),
		),
		'checkout_notes'                   => array(
			...
			'DESCRIPTION'    => 'Notes made at checkout time.'
		),
		'return_notes'                     => array(
			'FIELD_TYPE'     => FT_TEXT,
			'DISPLAY_TYPE'   => DT_FIELD,
			...
			'BOUNDS_LENGTH'  => array( 0, 65535 ),
			'LABEL'          => 'Return notes',
			'DESCRIPTION'    => 'Notes at return of object.'
		),
		'last_sent_coming_due_email'       => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Date of last coming due notice' ),
			'DESCRIPTION'    => _t( 'Date/time a coming due notice was last sent.' ),
		),
		'last_sent_overdue_email'          => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Date of last overdue notice' ),
			'DESCRIPTION'    => _t( 'Date/time an overdue notice was last sent.' ),
		),
		'last_reservation_available_email' => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Date of last reservation available notice' ),
			'DESCRIPTION'    => _t( 'Date/time a reservation available notice was last sent.' ),
		),
		'deleted'                          => array(
			'FIELD_TYPE'     => FT_BIT,
			'DISPLAY_TYPE'   => DT_OMIT,
			...
			'DEFAULT'        => 0,
			'LABEL'          => _t( 'Is deleted?' ),
			'DESCRIPTION'    => _t( 'Indicates if the order is deleted or not.' ),
...
class ca_object_checkouts extends BundlableLabelableBaseModelWithAttributes {
	...
	/**
	 * Checkout an object for a user. Will throw an exception if the item is currently out or object or user_id are
	 * invalid;
	 *
	 * @param int    $pn_object_id
	 * @param int    $pn_user_id
	 * @param string $ps_note     Optional checkin notes
	 * @param string $ps_due_date A valid date time expression for the date item is due to be returned. If omitted the
	 *                            default date as configured will be used.
	...
	public function checkout( $pn_object_id, $pn_user_id, $ps_note = null, $ps_due_date = null, $pa_options = null ) {
		...
		// is object available?
		if ( ! in_array( $t_object->getCheckoutStatus(),
			array( __CA_OBJECTS_CHECKOUT_STATUS_AVAILABLE__, __CA_OBJECTS_CHECKOUT_STATUS_RESERVED__ ) )
		) {
		...
		if ( is_null( $va_checkout_config = ca_object_checkouts::getObjectCheckoutConfigForType(
			$type_code = $t_object->getTypeCode() ) )
		) {
			throw new ApplicationException( _t( "Configuration for type %1 does not exist", $type_code ) );
		}

		if ( ! ( $va_checkout_config['allow_override_of_due_dates'] && $ps_due_date
		         && caDateToUnixTimestamp( $ps_due_date ) )
		) {
			// calculate default return date
			$ps_due_date = isset( $va_checkout_config['default_checkout_date'] )
				? $va_checkout_config['default_checkout_date'] : null;
		}
	...
	public function objectHasReservations( $pn_object_id, $pa_options = null ) {
		...
		while ( $qr_history->nextHit() ) {
			$va_reservations[] = array(
				...
				'user_name'      => $qr_history->get( 'ca_users.fname' ) . ' ' . $qr_history->get( 'ca_users.lname' )
				                    . ( ( $vs_email = $qr_history->get( 'ca_users.email' ) ) ? " ({$vs_email})" : '' ),
		...
		while ( $qr_history->nextHit() ) {
			$va_history[] = array(
				...
				'user_name'      => $qr_history->get( 'ca_users.fname' ) . ' ' . $qr_history->get( 'ca_users.lname' )
				                    . ( ( $vs_email = $qr_history->get( 'ca_users.email' ) ) ? " ({$vs_email})" : '' ),
				'created_on'     => $qr_history->get( 'ca_object_checkouts.created_on', array( 'timeOmit' => true ) ),
				'checkout_date'  => $qr_history->get( 'ca_object_checkouts.checkout_date',
					array( 'timeOmit' => true ) ),
	 ...
	 *                           db = A Db instance to use for database operations. If omitted a new Db instance will
	 ...
	 *
	 * @param int    $pn_user_id
	 * @param string $ps_display_template Display template evaluated relative to each ca_object_checkouts records;
	 *                                    return in array with key '_display'
	 * @param string $ps_datetime         Date/time range expression. [Default is null]
	 * @param array  $pa_options          Array of options. Options include
	 *                                    db = A Db instance to use for database operations. If omitted a new Db
	 *                                    instance will be used. [Default=null] omitOverdue = Omit overdue checkouts.
	 *                                    [Default is false]
	 *
	 * @return array
	 */
	static public function getOutstandingCheckoutsForUser(
		$pn_user_id, $ps_display_template = null, $ps_datetime = null, $pa_options = null
	) {
		if ( ! ( $o_db = caGetOption( 'db', $pa_options, null ) ) ) {
			$o_db = new Db();
		}
		...
		return ca_object_checkouts::_collectCheckoutData( caMakeSearchResult( 'ca_object_checkouts',
			$qr_res->getAllFieldValues( 'checkout_id' ) ), $ps_display_template );
	}
	# ------------------------------------------------------
	 ...
	 * @return array
	 */
	static public function getOverdueCheckoutsForUser(
		$pn_user_id, $ps_display_template = null, $ps_datetime = null, $pa_options = null
	) {
		if ( ! ( $o_db = caGetOption( 'db', $pa_options, null ) ) ) {
			$o_db = new Db();
	 ...
	 * @param array  $pa_options          Array of options. Options include
	 *                                    db = A Db instance to use for database operations. If omitted a new Db
	 *                                    instance will be used. [Default=null]
	 *
	 * @return array
	 */
	static public function getCheckinsForUser(
		$pn_user_id, $ps_display_template = null, $ps_datetime = null, $pa_options = null
	) {
		if ( ! ( $o_db = caGetOption( 'db', $pa_options, null ) ) ) {
			$o_db = new Db();
	 ...
	 * @return array
	 */
	static public function getOutstandingReservationsForUser(
		$pn_user_id, $ps_display_template = null, $pa_options = null
	) {
		if ( ! ( $o_db = caGetOption( 'db', $pa_options, null ) ) ) {
			$o_db = new Db();
		}
	 ...
	 * @param string       $ps_display_template Optional display template; will be returned in _display key in returned
	 *                                          array
	 *
		...
		if ( $qr_res ) {
			while ( $qr_res->nextHit() ) {
				$va_tmp = array(
					...
					'checkout_date'  => $qr_res->get( 'ca_object_checkouts.checkout_date',
						array( 'timeOmit' => true ) ),
	 ...
	 *
	 * @param string $ps_datetime Options date/time expression to return statistics for. If omitted current checkouts
	 *                            are considered. If provided, only checkouts out in the specified interval are
	 *                            counted.
	 * @param array  $pa_options  Array of options. Options include
	 *                            db = A Db instance to use for database operations. If omitted a new Db instance will
	 *                            be used. [Default=null]
	 ...
	 *
	 * @param array $pa_options Array of options. Options include
	 *                          db = A Db instance to use for database operations. If omitted a new Db instance will be
	 *                          used. [Default=null]
	 *
	 * @return int
	 */
	...
	static public function getDashboardStatistics( $ps_datetime = null, $pa_options = null ) {
		...
		$va_stats = array(
			'numOverdueCheckouts'     => ca_object_checkouts::numOverdueCheckouts( $ps_datetime ),
			//	Number of individual copies checked-out and overdue
			'overdueCheckoutUserList' => ca_object_checkouts::overdueCheckoutUserList( $ps_datetime ),
			'numCheckouts'            => ca_object_checkouts::numOutstandingCheckouts( $ps_datetime ),
			//	Number of individual copies checked-out
			'checkoutUserList'        => ca_object_checkouts::outstandingCheckoutUserList( $ps_datetime ),
			'numCheckins'             => ca_object_checkouts::numCheckins( $ps_datetime ),
			//	Number of individual copies checked-in
			'checkinUserList'         => ca_object_checkouts::checkinUserList( $ps_datetime ),
			'numReservations'         => ca_object_checkouts::numOutstandingReservations(),
			//	Number of individual copies currently reserved
	...
	static public function getItemsDueWithin( $ps_interval, $pa_options = null ) {
		...
		if ( $vs_template = caGetOption( 'template', $pa_options, null ) ) {
			$va_checkout_ids    = $qr_res->getAllFieldValues( 'checkout_id' );
			$va_template_values = caProcessTemplateForIDs( $vs_template, 'ca_object_checkouts', $va_checkout_ids,
