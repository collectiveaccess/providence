...
BaseModel::$s_ca_models_definitions['ca_bundle_displays'] = array(
    ...
    'FIELDS' => array(
        'display_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique numeric identifier used by CollectiveAccess internally to identify this display'
            )
        ),
        'user_id' => array(
            'FIELD_TYPE' => FT_NUMBER,
        ...
        'is_system' => array(
            ...
            'DESCRIPTION' => _t(
                'If set, display will be available to all users as part of the system-wide display list.'
            ),
...
$_ca_bundle_displays_settings = array(        // global
    ...
    'show_only_in' => array(
        ...
        'description' => _t(
            'Restrict display to use in specific contexts. If no contexts are selected the display will be shown in all contexts.'
        )
    )
);
...
class ca_bundle_displays extends BundlableLabelableBaseModelWithAttributes
{
    ...
    static public $s_have_access_to_display_cache = [];
    ...
    static public $s_placement_list_cache;        // cache for getPlacements()

    # ------------------------------------------------------
    public function __construct($pn_id = null)
    {
        // Filter list of tables display can be used for to those enabled in current config
        BaseModel::$s_ca_models_definitions['ca_bundle_displays']['FIELDS']['table_num']['BOUNDS_CHOICE_LIST'] = caFilterTableList(
    ...
    public function addPlacement($ps_bundle_name, $pa_settings, $pn_rank = null, $options = null)
    {
        ...
        $t_placement = new ca_bundle_display_placements(
            ...
            ) ? $options['additional_settings'] : null
        );
    ...
    public function getPlacements($options = null)
    {
        ...
        if (!($vn_display_id = $this->getPrimaryKey())) {
            if ($pb_return_all_available_if_empty && $ps_table) {
                return ca_bundle_displays::$s_placement_list_cache[$vn_display_id] = $this->getAvailableBundles(
                    $ps_table
                );
            }
            return [];
        ...
        if ($qr_res->numRows() > 0) {
            ...
            while ($qr_res->nextRow()) {
                ...
                $placements[$placement_id]['settings'] = $va_settings = caUnserializeForDatabase(
                    $qr_res->get('settings')
                );
                $placements[$placement_id]['allowEditing'] = $vb_user_can_edit;

                if (!$pb_settings_only) {
                    $t_placement->setSettingDefinitionsForPlacement(
                        $va_available_bundles[$vs_bundle_name_proc]['settings']
                    );
                    $placements[$placement_id]['display'] = $va_available_bundles[$vs_bundle_name]['display'];
                    $placements[$placement_id]['settingsForm'] = $t_placement->getHTMLSettingForm(
                        array(
                    ...
                    $placements[$placement_id]['display'] = ($t_instance ? $t_instance->getDisplayLabel(
                        $vs_bundle_name
                    ) : "???");
                }
                    ...
                    if ($va_bundle_name[0] == $vs_subject_table) {
                        ...
                        if ($va_bundle_name[1] === 'preferred_labels') {
                            ...
                            if (in_array(
                                ...
                                $placements[$placement_id]['allowEditing'] = false;
                                $placements[$placement_id]['inlineEditingType'] = null;
                            } elseif ($vs_edit_bundle = $t_subject->getFieldInfo(
                                $va_bundle_name[1],
                                ...
                                if (isset($va_bundle_name[1])) {
                                    ...
                                    if ($va_bundle_name[1] == $t_subject->getProperty('ID_NUMBERING_ID_FIELD')) {
                                        ...
                                        $vb_id_editable = $t_subject->opo_idno_plugin_instance->isFormatEditable(
                                            $vs_subject_table
                                        );
                                ...
                                switch ($t_subject->getFieldInfo($va_bundle_name[1], 'DISPLAY_TYPE')) {
                                    case 'DT_SELECT':
                                        ...
                                        if ($vs_list_code && ($t_list->numItemsInList($vs_list_code) <= 500)) {
                                            ...
                                            $placements[$placement_id]['inlineEditingListValues'] = array_values(
                                                $va_list_item_labels
                                            );
                                            $placements[$placement_id]['inlineEditingListValueMap'] = array_flip(
                                                $va_list_item_labels
                                            );
                                        } else {
                                            $placements[$placement_id]['inlineEditingType'] = DT_FIELD;
                                            ...
                                            switch ($t_element->getSetting('render')) {
                                                ...
                                                case 'vert_hierbrowser_down':
                                                    if ($t_list->numItemsInList($t_element->get("list_id")) > 500) {
                                                        ...
                                                        $va_list_values = $t_list->getItemsForList(
                                                            $t_element->get("list_id"),
                                                            array('labelsOnly' => true)
                                                        );

                                                        $qr_list_items = caMakeSearchResult(
                                                            'ca_list_items',
                                                        ...
                                                        while ($qr_list_items->nextHit()) {
                                                            ...
                                                            $va_list_item_labels[$vb_use_item_values ? $qr_list_items->get(
                                                                'ca_list_items.item_value'
                                                            ) : $qr_list_items->get('ca_list_items.item_id')] = $v;
                                                        }
    ...
    public function getBundleDisplays($options = null)
    {
        ...
        $pa_restrict_to_types = array_filter(
        ...
        if ($pm_table_name_or_num && is_array($pa_restrict_to_types) && sizeof($pa_restrict_to_types) && is_array(
                $va_ancestors = caGetAncestorsForItemID($pa_restrict_to_types, ['includeSelf' => true])
            ) && sizeof($va_ancestors)) {
        ...
        while ($qr_res->nextRow()) {
            ...
            if (!isset($va_type_name_cache[$vn_table_num]) || !($vs_display_type = $va_type_name_cache[$vn_table_num])) {
                $vs_display_type = $va_type_name_cache[$vn_table_num] = $this->getBundleDisplayTypeName(
                    $vn_table_num,
                    array('number' => 'plural')
                );
            }
            $va_displays[$qr_res->get('display_id')][$qr_res->get('locale_id')] = array_merge(
                $qr_res->getRow(),
                array('bundle_display_content_type' => $vs_display_type)
            );

            $va_displays[$qr_res->get('display_id')][$qr_res->get('locale_id')]['settings'] = caUnserializeForDatabase(
    ...
    public function getBundleDisplaysAsHTMLSelect($ps_select_name, $pa_attributes = null, $options = null)
    {
        ...
        if (
            ...
            ||
            (isset($options['addDefaultDisplayIfEmpty']) && ($options['addDefaultDisplayIfEmpty']) && (!sizeof(
                    $va_available_displays
                )))
        ) {
        ...
        foreach ($va_available_displays as $vn_display_id => $info) {
            if ($ps_context && is_array($info['settings']['show_only_in']) && sizeof(
                    $info['settings']['show_only_in']
    ...
    public function getBundleDisplayTypeName($pm_table_name_or_num = null, $options = null)
    {
        ...
        return (isset($options['number']) && ($options['number'] == 'plural')) ? $t_instance->getProperty(
            'NAME_PLURAL'
    ...
    public function haveAccessToDisplay($pn_user_id, $pn_user_access, $pn_display_id = null)
    {
        ...
        if ((bool)$t_disp->get(
                'is_system'
            ) && ($pn_user_access == __CA_BUNDLE_DISPLAY_READ_ACCESS__)) {    // system displays are readable by all
            return ca_bundle_displays::$s_have_access_to_display_cache[$vn_display_id . '/' . $pn_user_id . '/' . $pn_user_access] = true;
        }
    ...
    public function getAvailableBundles($pm_table_name_or_num = null, $options = null)
    {
        ...
        $vs_format = (isset($options['format']) && in_array(
                $options['format'],
                array('simple', 'full')
            )) ? $options['format'] : 'full';
        ...
        $vs_display = "<div id='bundleDisplayEditorBundle_{$vs_table}__generic_bundle_'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                $t_instance->getProperty('NAME_SINGULAR')
            ) . "</span> {$vs_label}</div>";
        ...
        if ($vb_show_tooltips) {
            TooltipManager::add(
                ...
                $this->_formatBundleTooltip(
                    $vs_label,
                    $vs_bundle,
                    _t(
                        'Use this generic %1 bundle to display %1 templates not specific to a single metadata element.',
                        $t_instance->getProperty('NAME_SINGULAR')
                    )
                )
            );
        ...
        foreach ($t_instance->getFormFields() as $vs_f => $info) {
            ...
            $vs_display = "<div id='bundleDisplayEditorBundle_{$vs_table}_{$vs_f}'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                    $t_instance->getProperty('NAME_SINGULAR')
                ) . "</span> " . ($vs_label = $t_instance->getDisplayLabel($vs_bundle)) . "</div>";
        ...
        foreach ($va_element_codes as $vn_element_id => $vs_element_code) {
                ...
                case __CA_ATTRIBUTE_VALUE_MEDIA__:
                    $va_even_more_settings = [
                        'appendMultiPagePDFToPDFOutput' => [
                            ...
                            'description' => _t(
                                'Check this option if you want PDF media in display appended to the end of PDF display output.'
                            )
                        ]
                    ];
                    break;
                ...
                case __CA_ATTRIBUTE_VALUE_NUMERIC__:
                    $va_even_more_settings = array(
                        'bottom_line' => array(
                            ...
                            'label' => _t('Bottom line format'),
                            'description' => _t(
                                'Template to format aggregate data for display under this column. The template can include these aggregate data tags: ^PAGEAVG, ^AVG, ^PAGESUM, ^SUM, ^PAGEMin, ^MIN, ^PAGEMAX, ^MAX. For containers follow the tag with the element code of the subelement to aggregate. Ex. ^SUM:dimensions_width'
                            )
                        )
                    );
                    ...
                    if ($va_all_elements[$vn_element_id]['datatype'] == 6) {
                        $va_even_more_settings['display_currency_conversion'] = array(
                            ...
                            'description' => _t(
                                'Check this option if you want your currency values to be displayed in both the specified and local currency.'
                            )
                        );
                    }
                    break;
            ...
            $vs_display = "<div id='bundleDisplayEditorBundle_{$vs_table}_{$vs_element_code}'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                    $t_instance->getProperty('NAME_SINGULAR')
                ) . "</span> " . ($vs_label = $t_instance->getDisplayLabel($vs_bundle)) . "</div>";
            $va_available_bundles[strip_tags($vs_display)][$vs_bundle] = array(
                'bundle' => $vs_bundle,
        ...
        if (caGetBundleAccessLevel($vs_table, "preferred_labels") != __CA_BUNDLE_ACCESS_NONE__) {
            ...
            $vs_display = "<div id='bundleDisplayEditorBundle_{$vs_table}_preferred_labels'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                    $t_instance->getProperty('NAME_SINGULAR')
                ) . "</span> " . ($vs_label = $t_instance->getDisplayLabel($vs_bundle)) . "</div>";
            $va_available_bundles[strip_tags($vs_display)][$vs_bundle] = array(
                'bundle' => $vs_bundle,
        ...
        if (caGetBundleAccessLevel($vs_table, "nonpreferred_labels") != __CA_BUNDLE_ACCESS_NONE__) {
            ...
            $vs_display = "<div id='bundleDisplayEditorBundle_{$vs_table}_nonpreferred_labels'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                    $t_instance->getProperty('NAME_SINGULAR')
                ) . "</span> " . ($vs_label = $t_instance->getDisplayLabel($vs_bundle)) . "</div>";
            $va_available_bundles[strip_tags($vs_display)][$vs_bundle] = array(
                'bundle' => $vs_bundle,
        ...
        if ($vs_table == 'ca_objects') {
            ...
            $vs_display = "<div id='bundleDisplayEditorBundle_{$vs_table}_ca_objects_location'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                    $t_instance->getProperty('NAME_SINGULAR')
                ) . "</span> " . _t('Current location') . "</div>";
        ...
        if (method_exists($t_instance, 'tablesTakeHistoryTracking') && in_array(
                $vs_table,
                $vs_table::tablesTakeHistoryTracking()
            )) {
            $va_additional_settings = array(
                'format' => array(
                    'formatType' => FT_TEXT,
            ...
            $vs_display = "<div id='bundleDisplayEditorBundle_{$vs_table}_history_tracking_current_value'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                    $t_instance->getProperty('NAME_SINGULAR')
                ) . "</span> " . _t('History tracking current value') . "</div>";
            ...
            $va_available_bundles[strip_tags($vs_display)][$vs_bundle] = array(
                ...
                'display' => ($vs_format == 'simple') ? $vs_label : $vs_display,
                'description' => $vs_description,
                'settingsForm' => $t_placement->getHTMLSettingForm(
                    array('id' => $vs_bundle . '_0', 'table' => $vs_table)
                ),
                'settings' => $va_additional_settings
            );
            ...
            $vs_display = "<div id='bundleDisplayEditorBundle_{$vs_table}_history_tracking_current_date'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                    $t_instance->getProperty('NAME_SINGULAR')
                ) . "</span> " . _t('History tracking current value date') . "</div>";
            ...
            $vs_display = "<div id='bundleDisplayEditorBundle_{$vs_table}_history_tracking_current_contents'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                    $t_instance->getProperty('NAME_SINGULAR')
                ) . "</span> " . _t('History tracking contents') . "</div>";
            $vs_description = _t('Current value date for history tracking policy');
            ...
            $vs_display = "<div id='bundleDisplayEditorBundle_{$vs_table}_submitted_by_user'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                    $t_instance->getProperty('NAME_SINGULAR')
                ) . "</span> " . _t('Submitted by user') . "</div>";
            ...
            $vs_display = "<div id='bundleDisplayEditorBundle_{$vs_table}_submission_group'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                    $t_instance->getProperty('NAME_SINGULAR')
                ) . "</span> " . _t('Submission group') . "</div>";
        ...
        if (caGetBundleAccessLevel($vs_table, "ca_object_representations") != __CA_BUNDLE_ACCESS_NONE__) {
            ...
            if ($vs_table == 'ca_objects') {
                ...
                foreach ($va_versions as $vs_version => $va_version_info) {
                    ...
                    $vs_display = "<div id='bundleDisplayEditorBundle_ca_object_representations_media_{$vs_version}'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                            $t_instance->getProperty('NAME_SINGULAR')
                        ) . "</span> " . ($vs_label = $t_instance->getDisplayLabel($vs_bundle)) . "</div>";
                    $va_available_bundles[strip_tags($vs_display)][$vs_bundle] = array(
                        'bundle' => $vs_bundle,
                ...
                foreach (array('mimetype', 'md5', 'original_filename') as $vs_rep_field) {
                    ...
                    $vs_display = "<div id='bundleDisplayEditorBundle_ca_object_representations_{$vs_rep_field}'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                            $t_rep->getProperty('NAME_SINGULAR')
                        ) . "</span> " . ($vs_label = $t_rep->getDisplayLabel($vs_bundle)) . "</div>";
        ...
        foreach (
            ...
            if ($this->getAppConfig()->get(
                    $vs_related_table . '_disable'
                ) && !(($vs_related_table == 'ca_object_representations') && (!$this->getAppConfig()->get(
                        'ca_objects_disable'
                    )))) {
                continue;
            }
            ...
            $va_additional_settings = array(
                ...
                'restrict_to_relationship_types' => array(
                    ...
                    'description' => _t(
                        'Restricts display to items related using the specified relationship type(s). Leave all unchecked for no restriction.'
                    )
                ),
                'restrict_to_types' => array(
                    ...
                    'description' => _t(
                        'Restricts display to items of the specified type(s). Leave all unchecked for no restriction.'
                    )
                ),
                'delimiter' => array(
                    'formatType' => FT_TEXT,
                ...
                'sort' => array(
                    ...
                    'label' => _t('Sort using'),
                    'description' => _t(
                        'Override sort option for this field. Use this if you want result lists to sort on a different field when clicking on this bundle.'
                    )
                ),
                'numPerPage' => array(
                    'formatType' => FT_NUMBER,
            ...
            if ($vs_related_table == 'ca_list_items') {
                $va_additional_settings['restrictToLists'] = array(
                    ...
                    'description' => _t(
                        'Display related items from selected lists only. If no lists are selected then all related items are displayed.'
                    )
                );
            }
            ...
            if ($t_rel_instance->isHierarchical()) {
                $va_additional_settings += array(
                    ...
                    'hierarchy_limit' => array(
                        ...
                        'description' => _t(
                            'Maximum number of items to show in the hierarchy. Leave blank to show the unabridged hierarchy.'
                        )
                    ),
            ...
            $vs_display = "<div id='bundleDisplayEditorBundle_{$vs_id_suffix}'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                    $t_rel_instance->getProperty('NAME_SINGULAR')
                ) . "</span> " . ($vs_label = $t_rel_instance->getDisplayLabel($vs_bundle)) . "</div>";
            $va_available_bundles[strip_tags($vs_display)][$vs_bundle] = array(
                'bundle' => $vs_bundle,
                'display' => ($vs_format == 'simple') ? $vs_label : $vs_display,
        ...
        $vs_display = "<div id='bundleDisplayEditorBundle_{$vs_table}_created'><span class='bundleDisplayEditorPlacementListItemTitle'>" . _t(
                'General'
            ) . "</span> " . ($vs_label = $t_instance->getDisplayLabel($vs_bundle)) . "</div>";
        $va_available_bundles[strip_tags($vs_display)][$vs_bundle] = array(
            'bundle' => $vs_bundle,
        ...
        $vs_display = "<div id='bundleDisplayEditorBundle_{$vs_table}_lastModified'><span class='bundleDisplayEditorPlacementListItemTitle'>" . _t(
                'General'
            ) . "</span> " . ($vs_label = $t_instance->getDisplayLabel($vs_bundle)) . "</div>";
        $va_available_bundles[strip_tags($vs_display)][$vs_bundle] = array(
            'bundle' => $vs_bundle,
    ...
    public function getPlacementsInDisplay($options = null)
    {
        ...
        foreach ($placements as $placement_id => $va_placement) {
            $vs_label = ($vs_label = $t_instance->getDisplayLabel(
                $va_placement['bundle_name']
            )) ? $vs_label : $va_placement['bundle_name'];
            ...
            $vs_display = "<div id='bundleDisplayEditor_{$placement_id}'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                    $t_instance->getProperty('NAME_SINGULAR')
                ) . "</span> {$vs_label}</div>";
    ...
    public function getTemplatePlaceholderListForBundle($ps_bundle_name)
    {
        ...
        $va_key = array(
            '^label' => array(
                ...
                'description' => _t(
                    'The label for this placement as defined in the placements settings. The value used will be adjusted to reflect the current user&apos;s locale.'
                )
            )
        );
        ...
        if ($vs_bundle == 'ca_object_representations') {
            if ($vs_table == 'ca_objects') {
                ...
                $va_key['^media:{version name}'] = array(
                    ...
                    'description' => _t(
                        'The version of the media representation specified by {version name} for display.'
                    )
                );
    ...
    public function getTemplatePlaceholderDisplayListForBundle($ps_bundle_name)
    {
        ...
        return '<div style="overflow: auto;"><table><tr><th>' . _t('Value') . '</th><th>' . _t(
                'Tag'
            ) . '</th></tr>' . join("", $va_buf) . '</table></div>';
    }
    # ------------------------------------------------------
    # Display of values
    # ------------------------------------------------------
    ...
    public function getDisplayValue($po_result, $pn_placement_id, $options = null)
    {
        ...
        if ((sizeof($va_bundle_bits) == 1) || ((sizeof($va_bundle_bits) == 2) && ($va_bundle_bits[1] == 'related'))) {
            $options['template'] = caGetOption(
                'format',
                $va_settings,
                $this->getAppConfig()->get(
                    $va_bundle_bits[0] . '_relationship_display_format'
                )
            );
        } else {
        ...
        if (!($vs_template = trim($options['template'])) && (sizeof(
                    $va_bundle_bits
                ) == 1) && ($t_instance = Datamodel::getInstanceByTableName($va_bundle_bits[0], true))) {
            $vs_template = $this->getAppConfig()->get($va_bundle_bits[0] . "_default_bundle_display_template");
        }

        if ((!$vs_template) && ($t_element = ca_metadata_elements::getInstance(
                $va_bundle_bits[sizeof($va_bundle_bits) - 1]
            ))) {
        ...
        if (!$pb_show_hierarchy && $vs_template) {
            ...
            if ($t_instance = Datamodel::getInstanceByTableName($va_bundle_bits[0], true)) {
                ...
                $vb_is_related = false;
                if ((sizeof($va_bundle_bits) == 1) || ((sizeof(
                                $va_bundle_bits
                            ) == 2) && $va_bundle_bits[1] == 'related')) {
                    // pulling related
                    $vb_is_related = true;
                } elseif ((sizeof($va_bundle_bits_proc) > 1) && (in_array(
                        $vs_tmp = array_pop($va_bundle_bits_proc),
                        array('related')
                    ))) {
                    // pulling related
                ...
                if ($vb_is_related) {
                    $vs_restrict_to_types = (is_array($options['restrictToTypes']) && sizeof(
                            $options['restrictToTypes']
                        )) ? "restrictToTypes=\"" . join("|", $options['restrictToTypes']) . "\"" : "";
                    $vs_restrict_to_relationship_types = (is_array($options['restrictToRelationshipTypes']) && sizeof(
                            $options['restrictToRelationshipTypes']
                        )) ? "restrictToRelationshipTypes=\"" . join(
                            "|",
                            $options['restrictToRelationshipTypes']
                        ) . "\"" : "";
                    ...
                    if (is_array(
                        $va_path = Datamodel::getPath($po_result->tableName(), $rel_table = $t_instance->tableName())
                    )) {
                            ...
                            case 3:
                                ...
                                $vs_val = $po_result->getWithTemplate(
                                    (caGetOption(
                                            ...
                                            $options,
                                            true
                                        ) && !$vs_restrict_to_types && !$vs_restrict_to_relationship_types) ? $vs_template : $vs_unit_tag . $vs_template . "</unit>",
                                    $options
                                );
                                break;
                            case 2:
                                ...
                                if (method_exists($t_rel, 'isSelfRelationship') && $t_rel->isSelfRelationship()) {
                                    ...
                                    $vs_val = $po_result->getWithTemplate(
                                        $vs_unit_tag . $vs_template . "</unit>",
                                        array_merge(
                                            $options,
                                            array(
                                                'primaryIDs' => array(
                                                    $po_result->tableName() => array($po_result->getPrimaryKey())
                                                )
                                            )
                                        )
                                    );
                    ...
                    $vs_val = $po_result->getWithTemplate(
                        $vs_template,
                        [
                            'relativeToContainer' => (ca_metadata_elements::getElementDatatype(
                                    $va_bundle_bits[sizeof($va_bundle_bits) - 1]
            ...
            $vs_val = $po_result->get(
                join(".", $va_bundle_bits),
                array_merge(
                    ['doRefSubstitution' => true],
                    $options,
        ...
        if ($vb_return_info) {
            ...
            if ($va_bundle_bits[0] !== $po_result->tableName()) {
                ...
                if ($t_element = ca_metadata_elements::getInstance($va_bundle_bits[1])) {
                    if ($t_restriction = $t_element->getTypeRestrictionInstanceForElement(
                        $t_instance->tableNum(),
                        $po_result->get(
                            $t_instance->tableName() . '.' . $t_instance->getTypeFieldName()
                        )
                    )) {
                ...
                return array(
                    ...
                    'ids' => array_keys($info_data),
                    'inlineEditable' => ca_bundle_displays::attributeTypeSupportsInlineEditing(
                            $vn_data_type
                        ) && ($vn_count >= 0) && ($vn_count <= 1)
                );
            } else {
                // special
                return array(
        ...
        if ($vs_bundles = $request->getParameter("{$ps_placement_code}{$ps_form_prefix}displayBundleList", pString)) {
            ...
            foreach ($placements as $placement_id => $va_bundle_info) {
                if (!in_array($va_bundle_info['bundle_name'] . '_' . $va_bundle_info['placement_id'], $va_bundles)) {
                    $t_display->removePlacement(
                        $va_bundle_info['placement_id'],
    ...
    public function getTypeRestrictionsHTMLFormBundle($request, $ps_form_name, $ps_placement_code, $options = null)
    {
        ...
        $vs_subtype_element = caProcessTemplate(
            $this->getAppConfig()->get('form_element_display_format_without_label'),
            [
                'ELEMENT' => _t('Include subtypes?') . ' ' . caHTMLCheckboxInput(
                        'type_restriction_include_subtypes',
                        [
                            'value' => '1',
                            'checked' => $vb_include_subtypes
                        ]
                    )
            ]
        );
        $o_view->setVar(
            'type_restrictions',
            $t_instance->getTypeListAsHTMLFormElement(
                'type_restrictions[]',
                array('multiple' => 1, 'height' => 5),
    ...
    public function saveTypeRestrictionsFromHTMLForm($request, $ps_form_prefix, $ps_placement_code)
    {
        ...
        return $this->setTypeRestrictions(
            $request->getParameter('type_restrictions', pArray),
            [
                'includeSubtypes' => $request->getParameter(
                    'type_restriction_include_subtypes',
                    pInteger
                )
            ]
        ...
        if (method_exists($t_model, 'getApplicableElementCodes')) {
            ...
            foreach ($display_list as $i => $display_item) {
                ...
                if ($t_model->hasField($tmp[1])) {
                    if ($t_model->getFieldInfo(
                            $tmp[1],
                            'FIELD_TYPE'
                        ) == FT_MEDIA) { // sorting media fields doesn't really make sense and can lead to sql errors
                        continue;
                    }
                    $display_list[$i]['is_sortable'] = true;
    ...
    public static function makeBundlesForResultsEditor($bundles, $settings = null)
    {
        ...
        foreach ($bundles as $i => $field) {
            ...
            $bundle = preg_replace(
                ...
                "",
                $bundle
            );  // Remove .related specifier as editor form generator doesn't need or recognize it
            $placements[] = [
                ...
                if ($t_subject->load($id)) {
                    ...
                    switch ($bundle_info['type']) {
                        ...
                        case 'nonpreferred_label':
                            ...
                            if (
                                is_array(
                                    $tmp = $t_subject->get($bundle['bundle_name'], ['returnWithStructure' => true])
                                )
                                &&
                            ...
                            $request->setParameter(
                                $vs_locale_key = $vs_key_stub . 'locale_id_' . $vs_label_id,
                    ...
                    if ($vb_set_value) {
                        $t_subject->saveBundlesForScreen(
                            null,
                            $request,
                            $save_options = [
                                'bundles' => [$bundle],
    ...
    public function saveResultsEditorComplexData($table, $options = null)
    {
        ...
        if ($t_subject->getAppConfig()->get('perform_type_access_checking')) {
            $restrict_to_types = caGetTypeRestrictionsForUser(
                $this->ops_tablename,
        ...
        if ($t_subject->getAppConfig()->get('perform_source_access_checking')) {
            if (is_array(
                $restrict_to_sources = caGetSourceRestrictionsForUser(
                    $this->ops_tablename,
                    array('access' => __CA_BUNDLE_ACCESS_EDIT__)
                )
            )) {
                if (
                    ...
                    ||
                    ((strlen($vn_source_id = $request->getParameter('source_id', pInteger))) && !in_array(
                            $vn_source_id,
                            $restrict_to_sources
                        ))
                ) {
        ...
        $va_general_errors = $request->getActionErrors(
            'general'
        );        // just "general" errors - ones that are not attached to a specific part of the form

        if (is_array($va_errors) && is_array($va_general_errors) && ((sizeof($va_errors) - sizeof(
                        $va_general_errors
                    )) > 0)) {
            $error_list = [];
            $no_save_error = false;
        ...
        $o_app_plugin_manager->hookSaveItem(
            array(
                'id' => $vn_subject_id,
                'table_num' => $t_subject->tableNum(),
                'table_name' => $t_subject->tableName(),
