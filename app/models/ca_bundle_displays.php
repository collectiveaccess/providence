...
BaseModel::$s_ca_models_definitions['ca_bundle_displays'] = array(
	...
	'FIELDS'        => array(
		'display_id'   => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this display' )
		),
		'user_id'      => array(
			'FIELD_TYPE'       => FT_NUMBER,
		...
		'is_system'    => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Is system display?' ),
			'DESCRIPTION'    => _t( 'If set, display will be available to all users as part of the system-wide display list.' ),
		...
		'table_num'    => array(
			...
			'IS_NULL'            => false,
			'DEFAULT'            => '',
			'LABEL'              => _t( 'Display type' ),
			'DESCRIPTION'        => _t( 'Indicates type of item display is used for.' ),
		...
		'display_code' => array(
			...
			'IS_NULL'        => true,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Display code' ),
			'DESCRIPTION'    => _t( 'Unique alphanumeric identifier for this display.' ),
			'UNIQUE_WITHIN'  => []
			//'REQUIRES' => array('is_administrator')
		),
		'settings'     => array(
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Settings' ),
			'DESCRIPTION'    => _t( 'Display settings' )
		),
		'access'       => array(
			'FIELD_TYPE'                => FT_NUMBER,
			'DISPLAY_TYPE'              => DT_SELECT,
			...
			'LABEL'                     => _t( 'Access' ),
			'DESCRIPTION'               => _t( 'Indicates if display is accessible to the public or not.' )
		)
	)
);
...
class ca_bundle_displays extends BundlableLabelableBaseModelWithAttributes {
	...
	public function __construct( $pn_id = null ) {
		...
		BaseModel::$s_ca_models_definitions['ca_bundle_displays']['FIELDS']['table_num']['BOUNDS_CHOICE_LIST']
			= caFilterTableList( BaseModel::$s_ca_models_definitions['ca_bundle_displays']['FIELDS']['table_num']['BOUNDS_CHOICE_LIST'] );
	...
	protected function initLabelDefinitions( $options = null ) {
		...
		$this->BUNDLES['ca_users']                            = array(
			'type'      => 'special',
			'repeating' => true,
			'label'     => _t( 'User access' )
		);
		$this->BUNDLES['ca_user_groups']                      = array(
			'type'      => 'special',
			'repeating' => true,
			'label'     => _t( 'Group access' )
		);
		$this->BUNDLES['ca_bundle_display_placements']        = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Display list contents' )
		);
		$this->BUNDLES['settings']                            = array(
			'type'      => 'special',
			'repeating' => false,
			'label'     => _t( 'Display settings' )
		);
		$this->BUNDLES['ca_bundle_display_type_restrictions'] = array(
			'type'      => 'special',
			'repeating' => false,
	 ...
	 * @param int    $pn_rank        Optional value that determines sort order of bundles in the display. If omitted,
	 *                               placement is added to the end of the display.
	 * @param array  $options        Optional array of options. Supports the following options:
	 *                               user_id = if specified then add will fail if specified user does not have edit
	 *                               access for the display
	 *
	...
	public function addPlacement( $ps_bundle_name, $pa_settings, $pn_rank = null, $options = null ) {
		...
		$t_placement = new ca_bundle_display_placements( null,
			is_array( $options['additional_settings'] ) ? $options['additional_settings'] : null );
	 ...
	 * @param array $options         Optional array of options. Supports the following options:
	 *                               user_id = if specified then remove will fail if specified user does not have edit
	 *                               access for the display
	 *
	 ...
	 *
	 * @param array $options Optional array of options. Supports the following options:
	 *                       noCache = if set to true then the returned list if always generated directly from the
	 *                       database, otherwise it is returned from the cache if possible. Set this to true if you
	 *                       expect the cache may be stale. Default is false. returnAllAvailableIfEmpty = if set to
	 *                       true then the list of all available bundles will be returned if the currently loaded
	 *                       display has no placements, or if there is no display loaded table = if using the
	 *                       returnAllAvailableIfEmpty option and you expect a list of available bundles to be returned
	 *                       if no display is loaded, you must specify the table the bundles are intended for use with
	 *                       with this option. Either the table name or number may be used. user_id = if specified then
	 *                       placements are only returned if the user has at least read access to the display
	 *                       settingsOnly = if true the settings forms are omitted and only setting values are
	 *                       returned. [Default is false] omitEditingInfo = don't include data required for inline in-spreadsheet editing. [Default is false]
	 *
	 * @return array List of placements in display order. Array is keyed on bundle name. Values are arrays with the
	 *               following keys: placement_id = primary key of ca_bundle_display_placements row - a unique id for
	 *               the placement bundle_name = bundle name (a code - not for display) settings = array of placement
	 *               settings. Keys are setting names. display = display string for bundle
	 */
	public function getPlacements( $options = null ) {
		$pb_no_cache                      = ( isset( $options['noCache'] ) ) ? (bool) $options['noCache'] : false;
		$pb_settings_only                 = ( isset( $options['settingsOnly'] ) ) ? (bool) $options['settingsOnly']
			: false;
		$pb_return_all_available_if_empty = ( isset( $options['returnAllAvailableIfEmpty'] ) && ! $pb_settings_only )
			? (bool) $options['returnAllAvailableIfEmpty'] : false;
		...
		if ( ! ( $vn_display_id = $this->getPrimaryKey() ) ) {
			if ( $pb_return_all_available_if_empty && $ps_table ) {
				return ca_bundle_displays::$s_placement_list_cache[ $vn_display_id ]
					= $this->getAvailableBundles( $ps_table );
			}
		...
		if ( ! $pb_no_cache && isset( ca_bundle_displays::$s_placement_list_cache[ $cache_key ] )
		     && ca_bundle_displays::$s_placement_list_cache[ $cache_key ]
		) {
		...
		if ( $qr_res->numRows() > 0 ) {
			...
			while ( $qr_res->nextRow() ) {
				...
				} elseif ( ! isset( $va_available_bundles[ $vs_bundle_name ] )
				           && ( sizeof( $va_bundle_name ) === 1 )
				) {
				...
				$placements[ $placement_id ]['settings']
				                                                                    =
				$va_settings = caUnserializeForDatabase( $qr_res->get( 'settings' ) );
				...
				if ( ! $pb_settings_only ) {
					...
					$placements[ $placement_id ]['settingsForm']
					                                        = $t_placement->getHTMLSettingForm( array(
						'id'       => $vs_bundle_name . '_' . $placement_id,
						'settings' => $va_settings,
						'table'    => $vs_subject_table
					) );
				} else {
					$t_instance                             = Datamodel::getInstanceByTableName( $va_bundle_name[0],
						true );
					$placements[ $placement_id ]['display'] = ( $t_instance
						? $t_instance->getDisplayLabel( $vs_bundle_name ) : "???" );
				}
				if ( ! $pb_omit_editing_info ) {
					if ( $va_bundle_name[0] == $vs_subject_table ) {
						...
						if ( $va_bundle_name[1] === 'preferred_labels' ) {
							...
							//
							if ( in_array( $va_bundle_name[1],
								[ $t_subject->getTypeFieldName(), $t_subject->primaryKey() ] )
							) {
								$placements[ $placement_id ]['allowInlineEditing'] = false;
								$placements[ $placement_id ]['allowEditing']       = false;
								$placements[ $placement_id ]['inlineEditingType']  = null;
							} elseif ( $vs_edit_bundle = $t_subject->getFieldInfo( $va_bundle_name[1],
								'RESULTS_EDITOR_BUNDLE' )
							) {
								$placements[ $placement_id ]['allowEditing']       = $vb_user_can_edit;
								$placements[ $placement_id ]['allowInlineEditing'] = false;
								$placements[ $placement_id ]['inlineEditingType']  = null;
								...
								if ( isset( $va_bundle_name[1] ) ) {
									...
									if ( $va_bundle_name[1] == $t_subject->getProperty( 'ID_NUMBERING_ID_FIELD' ) ) {
										...
										$vb_id_editable
											= $t_subject->opo_idno_plugin_instance->isFormatEditable( $vs_subject_table );

										$placements[ $placement_id ]['allowInlineEditing'] = false;
										$placements[ $placement_id ]['allowEditing']       = $vb_id_editable
										                                                     && $vb_user_can_edit;
									} else {
										$placements[ $placement_id ]['allowInlineEditing'] = $vb_user_can_edit;
									}
								...
								switch ( $t_subject->getFieldInfo( $va_bundle_name[1], 'DISPLAY_TYPE' ) ) {
									case 'DT_SELECT':
										...
										if ( $vs_list_code && ( $t_list->numItemsInList( $vs_list_code ) <= 500 ) ) {
											...
											if ( ! is_array(
												$va_list_items = $t_list->getItemsForList( $vs_list_code ) )
											) {
												break;
											}
											...
											foreach ( $va_list_items as $item_id => $va_list_item ) {
												$va_list_item_labels[ $vb_use_item_values ? $va_list_item['item_value']
													: $item_id ]
													= $va_list_item['name_plural'];
											}

											$placements[ $placement_id ]['inlineEditingListValues']
												= array_values( $va_list_item_labels );
											$placements[ $placement_id ]['inlineEditingListValueMap']
												= array_flip( $va_list_item_labels );
										} else {
											$placements[ $placement_id ]['inlineEditingType'] = DT_FIELD;
										}
										break;
										...
										if ( $t_element = ca_metadata_elements::getInstance( $va_bundle_name[1] ) ) {
											switch ( $t_element->getSetting( 'render' ) ) {
												...
												case 'vert_hierbrowser_down':
													if ( $t_list->numItemsInList( $t_element->get( "list_id" ) )
													     > 500
													) {
														...
														$placements[ $placement_id ]['allowInlineEditing']
															                                              = $vb_user_can_edit;
														$placements[ $placement_id ]['inlineEditingType'] = DT_SELECT;

														$va_list_values
															= $t_list->getItemsForList( $t_element->get( "list_id" ),
															array( 'labelsOnly' => true ) );

														$qr_list_items       = caMakeSearchResult( 'ca_list_items',
															array_keys( $va_list_values ) );
														$va_list_item_labels = [];

														while ( $qr_list_items->nextHit() ) {
															if ( ! ( $v
																= trim( $qr_list_items->get( 'ca_list_items.hierarchy.preferred_labels.name_plural',
																[ 'delimiter' => $ps_hierarchical_delimiter ] ) ) )
															) {
																continue;
															}
															$va_list_item_labels[ $vb_use_item_values
																? $qr_list_items->get( 'ca_list_items.item_value' )
																: $qr_list_items->get( 'ca_list_items.item_id' ) ]
																= $v;
														}
						...
						if ( ( $va_bundle_name[0] == 'ca_object_representations' )
						     && ( $va_bundle_name[1] == 'media' )
						) {
							$placements[ $placement_id ]['allowEditing'] = false;
						}
					}
	 ...
	 *                       table = If set, list is restricted to displays that pertain to the specified table. You
	 *                       can pass a table name or number. If omitted displays for all tables will be returned.
	 *                       user_id = Restricts returned displays to those accessible by the current user. If omitted
	 *                       then all displays, regardless of access are returned. restrictToTypes = Restricts returned
	 *                       displays to those bound to the specified type. Default is to not restrict by type. access
	 *                       = Restricts returned displays to those with at least the specified access level for the
	 *                       specified user. If user_id is omitted then this option has no effect. If user_id is set
	 *                       and this option is omitted, then displays where the user has at least read access will be
	 *                       returned.
	 *
	 * @return array Array of displays keyed on display_id and then locale_id. Keys for the per-locale value array
	 *               include: display_id,  display_code, user_id, table_num,  label_id, name (display name of display),
	 *               locale_id (locale of display name), bundle_display_content_type (display name of content this
	 *               display pertains to)
	 */
	public function getBundleDisplays( $options = null ) {
		...
		if ( $pm_table_name_or_num && is_array( $pa_restrict_to_types ) && sizeof( $pa_restrict_to_types )
		     && is_array( $va_ancestors = caGetAncestorsForItemID( $pa_restrict_to_types, [ 'includeSelf' => true ] ) )
		     && sizeof( $va_ancestors )
		) {
			$va_wheres[]
				         = "(cbdtr.type_id IS NULL OR cbdtr.type_id IN (?) OR (cbdtr.include_subtypes = 1 AND cbdtr.type_id IN (?)))";
		...
		while ( $qr_res->nextRow() ) {
			...
			if ( ! isset( $va_type_name_cache[ $vn_table_num ] )
			     || ! ( $vs_display_type
					= $va_type_name_cache[ $vn_table_num ] )
			) {
				$vs_display_type
					= $va_type_name_cache[ $vn_table_num ] = $this->getBundleDisplayTypeName( $vn_table_num,
					array( 'number' => 'plural' ) );
			}
			$va_displays[ $qr_res->get( 'display_id' ) ][ $qr_res->get( 'locale_id' ) ]
				= array_merge( $qr_res->getRow(), array( 'bundle_display_content_type' => $vs_display_type ) );

			$va_displays[ $qr_res->get( 'display_id' ) ][ $qr_res->get( 'locale_id' ) ]['settings']
				= caUnserializeForDatabase( $va_displays[ $qr_res->get( 'display_id' ) ][ $qr_res->get( 'locale_id' ) ]['settings'] );
		}
	 ...
	 * @param array  $pa_attributes  Optional array of attributes to embed in HTML <select> tag. Keys are attribute
	 *                               names and values are attribute values.
	 * @param array  $options        Optional array of options. Supported options include:
	 *                               Supports all options supported by caHTMLSelect() and
	 *                               ca_bundle_displays::getBundleDisplays() + the following: addDefaultDisplay = if
	 *                               true, the "default" display is included at the head of the list; this is simply a
	 *                               display called "default" that is assumed to be handled by your code; the default
	 *                               is not to add the default value (false) addDefaultDisplayIfEmpty = same as
	 *                               'addDefaultDisplay' except that the default value is only added if the display
	 *                               list is empty dontIncludeSubtypesInTypeRestriction = don't automatically include
	 *                               subtypes of a type when calculating type restrictions. [Default is true] context =
	 *                               context to filter display list for. [Default is null – no filtering performed]
	 *
	...
	public function getBundleDisplaysAsHTMLSelect( $ps_select_name, $pa_attributes = null, $options = null ) {
		...
		if (
			...
			|| ( isset( $options['addDefaultDisplayIfEmpty'] ) && ( $options['addDefaultDisplayIfEmpty'] )
			     && ( ! sizeof( $va_available_displays ) ) )
		) {
		...
		foreach ( $va_available_displays as $vn_display_id => $info ) {
			if ( $ps_context && is_array( $info['settings']['show_only_in'] )
			     && sizeof( $info['settings']['show_only_in'] )
			     && ! in_array( $ps_context, $info['settings']['show_only_in'] )
			) {
				continue;
			}
	...
	/**
	 * Returns name of type of content (synonymous with the table name for the content) currently loaded bundle display
	 * contains for display. Will return name in singular number unless the 'number' option is set to 'plural'
	 *
	 * @param int   $pn_table_num Table number to return name for. If omitted then the name for the content type
	 *                            contained by the current bundle display will be returned. Use this parameter if you
	 *                            want to force a content type without having to load a bundle display.
	 * @param array $options      Optional array of options. Supported options are:
	 *                            number = Set to 'plural' to return plural version of name; set to 'singular'
	 *                            [default] to return the singular version
	 *
	 * @return string The name of the type of content or null if $pn_table_num is not set to a valid table and no form
	 *                is loaded.
	 */
	public function getBundleDisplayTypeName( $pm_table_name_or_num = null, $options = null ) {
		...
		return ( isset( $options['number'] ) && ( $options['number'] == 'plural' ) )
			? $t_instance->getProperty( 'NAME_PLURAL' ) : $t_instance->getProperty( 'NAME_SINGULAR' );
	 ...
	 * @param int $pn_user_access type of access required. Use __CA_BUNDLE_DISPLAY_READ_ACCESS__ for read-only access
	 *                            or __CA_BUNDLE_DISPLAY_EDIT_ACCESS__ for editing (full) access
	 * @param int $pn_display_id  The id of the display to check. If omitted then currently loaded display will be
	 *                            checked.
	 *
	...
	public function haveAccessToDisplay( $pn_user_id, $pn_user_access, $pn_display_id = null ) {
		...
		if ( isset( ca_bundle_displays::$s_have_access_to_display_cache[ $vn_display_id . '/' . $pn_user_id . '/'
		                                                                 . $pn_user_access ] )
		) {
			return ca_bundle_displays::$s_have_access_to_display_cache[ $vn_display_id . '/' . $pn_user_id . '/'
			                                                            . $pn_user_access ];
		}
		...
		if ( (bool) $t_disp->get( 'is_system' )
		     && ( $pn_user_access == __CA_BUNDLE_DISPLAY_READ_ACCESS__ )
		) {    // system displays are readable by all
			return ca_bundle_displays::$s_have_access_to_display_cache[ $vn_display_id . '/' . $pn_user_id . '/'
			                                                            . $pn_user_access ]
				= true;
		}
	...
	public function getBundleDisplayHTMLFormBundle( $request, $ps_form_name, $ps_placement_code, $options = null ) {
		...
		$o_view->setVar( 'lookup_urls',
			caJSONLookupServiceUrl( $request, Datamodel::getTableName( $this->get( 'table_num' ) ) ) );
	...
	# ------------------------------------------------------
	/**
	 * Returns all available bundle display placements - those data bundles that can be displayed for the given content
	 * type, in other words. The returned value is a list of arrays; each array contains a 'bundle' specifier than can
	 * be passed got Model::get() or SearchResult::get() and a display name
	 *
	 * @param mixed $pm_table_name_or_num The table name or number specifying the content type to fetch bundles for. If
	 *                                    omitted the content table of the currently loaded display will be used.
	 * @param array $options              Support options are
	 *                                    no_cache = if set caching of underlying data required to generate list is
	 *                                    disabled. This is required in certain situations such as during installation.
	 *                                    Only set this if you suspect stale data is being used to generate the list.
	 *                                    Eg. if you've been changing metadata attributes in the same request in which
	 *                                    you call this method. Default is false. no_tooltips = if set no tooltips for
	 *                                    available bundles will be emitted. Default is false - tooltips will be
	 *                                    emitted. format = specifies label format for bundles. Valid values are
	 *                                    "simple" (just the name of the element) or "full" (name of element, name of
	 *                                    type of item element pertains to and alternate label, if defined). Default is
	 *                                    "full"
	 *
	...
	public function getAvailableBundles( $pm_table_name_or_num = null, $options = null ) {
		...
		$vs_format        = ( isset( $options['format'] ) && in_array( $options['format'], array( 'simple', 'full' ) ) )
			? $options['format'] : 'full';
		...
		$vs_display
		           = "<div id='bundleDisplayEditorBundle_{$vs_table}__generic_bundle_'><span class='bundleDisplayEditorPlacementListItemTitle'>"
		             . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) ) . "</span> {$vs_label}</div>";
		...
		$va_available_bundles[ $vs_display ][ $vs_bundle ] = [
			...
			'description'  => _t( 'Generic template bundle for %1',
				caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_PLURAL' ) ) ),
		...
		if ( $vb_show_tooltips ) {
			TooltipManager::add(
				...
				$this->_formatBundleTooltip( $vs_label, $vs_bundle,
					_t( 'Use this generic %1 bundle to display %1 templates not specific to a single metadata element.',
						$t_instance->getProperty( 'NAME_SINGULAR' ) ) )
			);
		}
		...
		foreach ( $t_instance->getFormFields() as $vs_f => $info ) {
			...
			$vs_display
			                                                                 = "<div id='bundleDisplayEditorBundle_{$vs_table}_{$vs_f}'><span class='bundleDisplayEditorPlacementListItemTitle'>"
			                                                                   . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) )
			                                                                   . "</span> " . ( $vs_label
					= $t_instance->getDisplayLabel( $vs_bundle ) ) . "</div>";
		...
		foreach ( $va_element_codes as $vn_element_id => $vs_element_code ) {
			if ( ! is_null( $va_all_elements[ $vn_element_id ]['settings']['canBeUsedInDisplay'] )
			     && ! $va_all_elements[ $vn_element_id ]['settings']['canBeUsedInDisplay']
			) {
				continue;
			}
			$t_placement = new ca_bundle_display_placements( null, $va_additional_settings );
			...
			$t_placement = new ca_bundle_display_placements( null,
				array_merge( $va_additional_settings, $va_even_more_settings ) );
			if ( $this->inTransaction() ) {
				$t_placement->setTransaction( $this->getTransaction() );
			}

			$vs_display
				                                                             = "<div id='bundleDisplayEditorBundle_{$vs_table}_{$vs_element_code}'><span class='bundleDisplayEditorPlacementListItemTitle'>"
				                                                               . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) )
				                                                               . "</span> " . ( $vs_label
					= $t_instance->getDisplayLabel( $vs_bundle ) ) . "</div>";
			$va_available_bundles[ strip_tags( $vs_display ) ][ $vs_bundle ] = array(
		...
		if ( caGetBundleAccessLevel( $vs_table, "preferred_labels" ) != __CA_BUNDLE_ACCESS_NONE__ ) {
			...
			$vs_display
				                                                             = "<div id='bundleDisplayEditorBundle_{$vs_table}_preferred_labels'><span class='bundleDisplayEditorPlacementListItemTitle'>"
				                                                               . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) )
				                                                               . "</span> " . ( $vs_label
					= $t_instance->getDisplayLabel( $vs_bundle ) ) . "</div>";
			$va_available_bundles[ strip_tags( $vs_display ) ][ $vs_bundle ] = array(
		...
		if ( caGetBundleAccessLevel( $vs_table, "nonpreferred_labels" ) != __CA_BUNDLE_ACCESS_NONE__ ) {
			...
			$vs_display
				                                                             = "<div id='bundleDisplayEditorBundle_{$vs_table}_nonpreferred_labels'><span class='bundleDisplayEditorPlacementListItemTitle'>"
				                                                               . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) )
				                                                               . "</span> " . ( $vs_label
					= $t_instance->getDisplayLabel( $vs_bundle ) ) . "</div>";
			$va_available_bundles[ strip_tags( $vs_display ) ][ $vs_bundle ] = array(
		...
		if ( $vs_table == 'ca_objects' ) {
			...
			$vs_display
			                = "<div id='bundleDisplayEditorBundle_{$vs_table}_preferred_labels'><span class='bundleDisplayEditorPlacementListItemTitle'>"
			                  . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) ) . "</span> "
			                  . _t( 'Library checkouts' ) . "</div>";
			...
			$vs_display
			                = "<div id='bundleDisplayEditorBundle_{$vs_table}_ca_objects_location'><span class='bundleDisplayEditorPlacementListItemTitle'>"
			                  . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) ) . "</span> "
			                  . _t( 'Current location' ) . "</div>";
		...
		if ( method_exists( $t_instance, 'tablesTakeHistoryTracking' )
		     && in_array( $vs_table, $vs_table::tablesTakeHistoryTracking() )
		) {
			$va_additional_settings = array(
				'format' => array(
					'formatType'  => FT_TEXT,
			...
			$vs_display
			                = "<div id='bundleDisplayEditorBundle_{$vs_table}_history_tracking_current_value'><span class='bundleDisplayEditorPlacementListItemTitle'>"
			                  . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) ) . "</span> "
			                  . _t( 'History tracking current value' ) . "</div>";
			...
			$va_available_bundles[ strip_tags( $vs_display ) ][ $vs_bundle ] = array(
				...
				'display'      => ( $vs_format == 'simple' ) ? $vs_label : $vs_display,
				'description'  => $vs_description,
				'settingsForm' => $t_placement->getHTMLSettingForm( array(
					'id'    => $vs_bundle . '_0',
					'table' => $vs_table
				) ),
				'settings'     => $va_additional_settings
			...
			$vs_display
			                = "<div id='bundleDisplayEditorBundle_{$vs_table}_history_tracking_current_date'><span class='bundleDisplayEditorPlacementListItemTitle'>"
			                  . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) ) . "</span> "
			                  . _t( 'History tracking current value date' ) . "</div>";
			...
			$vs_display
			                = "<div id='bundleDisplayEditorBundle_{$vs_table}_history_tracking_current_contents'><span class='bundleDisplayEditorPlacementListItemTitle'>"
			                  . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) ) . "</span> "
			                  . _t( 'History tracking contents' ) . "</div>";
			$vs_description = _t( 'Current value date for history tracking policy' );
			...
			$vs_display
			                = "<div id='bundleDisplayEditorBundle_{$vs_table}_submitted_by_user'><span class='bundleDisplayEditorPlacementListItemTitle'>"
			                  . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) ) . "</span> "
			                  . _t( 'Submitted by user' ) . "</div>";
			...
			$vs_display
			                = "<div id='bundleDisplayEditorBundle_{$vs_table}_submission_group'><span class='bundleDisplayEditorPlacementListItemTitle'>"
			                  . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) ) . "</span> "
			                  . _t( 'Submission group' ) . "</div>";
		...
		if ( caGetBundleAccessLevel( $vs_table, "ca_object_representations" ) != __CA_BUNDLE_ACCESS_NONE__ ) {
			...
			if ( $vs_table == 'ca_objects' ) {
				...
				foreach ( $va_versions as $vs_version => $va_version_info ) {
					...
					$vs_bundle                                                       = 'ca_object_representations.media.'
					                                                                   . $vs_version;
					$vs_display
					                                                                 = "<div id='bundleDisplayEditorBundle_ca_object_representations_media_{$vs_version}'><span class='bundleDisplayEditorPlacementListItemTitle'>"
					                                                                   . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) )
					                                                                   . "</span> " . ( $vs_label
							= $t_instance->getDisplayLabel( $vs_bundle ) ) . "</div>";
					$va_available_bundles[ strip_tags( $vs_display ) ][ $vs_bundle ] = array(
				...
				foreach ( array( 'mimetype', 'md5', 'original_filename' ) as $vs_rep_field ) {
					...
					$vs_display
					           = "<div id='bundleDisplayEditorBundle_ca_object_representations_{$vs_rep_field}'><span class='bundleDisplayEditorPlacementListItemTitle'>"
					             . caUcFirstUTF8Safe( $t_rep->getProperty( 'NAME_SINGULAR' ) ) . "</span> " . (
					             $vs_label = $t_rep->getDisplayLabel( $vs_bundle ) ) . "</div>";
		...
		foreach (
			...
			if ( $this->getAppConfig()->get( $vs_related_table . '_disable' )
			     && ! ( ( $vs_related_table == 'ca_object_representations' )
			            && ( ! $this->getAppConfig()->get( 'ca_objects_disable' ) ) )
			) {
				continue;
			}
			...
			$vs_id_suffix                                                    = "bundleDisplayEditorBundle_"
			                                                                   . str_replace( ".", "_", $vs_bundle );
			$vs_display
			                                                                 = "<div id='bundleDisplayEditorBundle_{$vs_id_suffix}'><span class='bundleDisplayEditorPlacementListItemTitle'>"
			                                                                   . caUcFirstUTF8Safe( $t_rel_instance->getProperty( 'NAME_SINGULAR' ) )
			                                                                   . "</span> " . ( $vs_label
					= $t_rel_instance->getDisplayLabel( $vs_bundle ) ) . "</div>";
			$va_available_bundles[ strip_tags( $vs_display ) ][ $vs_bundle ] = array(
				'bundle'       => $vs_bundle,
				'display'      => ( $vs_format == 'simple' ) ? $vs_label : $vs_display,
		...
		$vs_display
		                                                                 = "<div id='bundleDisplayEditorBundle_{$vs_table}_created'><span class='bundleDisplayEditorPlacementListItemTitle'>"
		                                                                   . _t( 'General' ) . "</span> " . ( $vs_label
				= $t_instance->getDisplayLabel( $vs_bundle ) ) . "</div>";
		$va_available_bundles[ strip_tags( $vs_display ) ][ $vs_bundle ] = array(
			'bundle'       => $vs_bundle,
			'display'      => ( $vs_format == 'simple' ) ? $vs_label : $vs_display,
		...
		$vs_display
		                                                                 = "<div id='bundleDisplayEditorBundle_{$vs_table}_lastModified'><span class='bundleDisplayEditorPlacementListItemTitle'>"
		                                                                   . _t( 'General' ) . "</span> " . ( $vs_label
				= $t_instance->getDisplayLabel( $vs_bundle ) ) . "</div>";
		$va_available_bundles[ strip_tags( $vs_display ) ][ $vs_bundle ] = array(
			'bundle'       => $vs_bundle,
	 ...
	 *                       no_tooltips = if set no tooltips for available bundles will be emitted. Default is false -
	 *                       tooltips will be emitted. format = specifies label format for bundles. Valid values are
	 *                       "simple" (just the name of the element) or "full" (name of element, name of type of item
	 *                       element pertains to and alternate label, if defined). Default is "full" user_id = if
	 *                       specified then placements are only returned if the user has at least read access to the
	 *                       display
	 *
	...
	public function getPlacementsInDisplay( $options = null ) {
		...
		$vs_format        = caGetOption( 'format', $options, 'full',
			array( 'validValues' => array( 'simple', 'full' ) ) );
		...
		foreach ( $placements as $placement_id => $va_placement ) {
			$vs_label = ( $vs_label = $t_instance->getDisplayLabel( $va_placement['bundle_name'] ) ) ? $vs_label
				: $va_placement['bundle_name'];
			...
			$vs_display
				                     = "<div id='bundleDisplayEditor_{$placement_id}'><span class='bundleDisplayEditorPlacementListItemTitle'>"
				                       . caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) )
				                       . "</span> {$vs_label}</div>";
			$va_placement['display'] = ( $vs_format == 'simple' ) ? $vs_label : $vs_display;
			$va_placement['bundle']
				                     = $va_placement['bundle_name']; // we used 'bundle' in the arrays, but the database field is called 'bundle_name' and getPlacements() returns data directly from the database
	 ...
	 * @param array $options Optional array of options. Supported options are:
	 *                       noCache = if set to true, no caching of placement values is performed.
	 *                       user_id = if specified then placements are only returned if the user has at least read
	 ...
	 *
	 * @return array - list of placeholders as keys; values are text description of value; will return null if bundle
	 *               name is invalid
	 */
	...
	public function getTemplatePlaceholderDisplayListForBundle( $ps_bundle_name ) {
		...
		if ( is_array( $va_list ) && sizeof( $va_list ) ) {
			foreach ( $va_list as $vs_tag => $info ) {
				$va_buf[]
					= "<tr><td class='settingsKeyRow'>{$info['label']}</td><td class='settingsKeyRow'>{$vs_tag}</td></tr>\n";
			}
		} else {
			return '';
		}

		return '<div style="overflow: auto;"><table><tr><th>' . _t( 'Value' ) . '</th><th>' . _t( 'Tag' ) . '</th></tr>'
		       . join( "", $va_buf ) . '</table></div>';
	}
	# ------------------------------------------------------
	# Display of values
	# ------------------------------------------------------
	/**
	 * Get display value(s) out of result $po_result for specified bundle and format it using the configured value
	 * template
	 *
	 ...
	 *                          convertCodesToDisplayText = If true numeric list id's and value lists are converted to
	 *                          display text. Default is true. If false then all such values are returned as the
	 *                          original integer codes. forReport = If true then certain values are transformed for
	 *                          display in a report. Namely, all media output is forced to use the version specified by
	 *                          the app.conf 'representation_version_for_report' directive, no matter the setting in
	 *                          the display. purify = if true then value is run through HTMLPurifier
	 *                          (http://htmlpurifier.org) before being returned; this is useful when you want to make
	 *                          sure any HTML in the value is valid, particularly when converting HTML to a PDF as
	 *                          invalid markup will cause an exception. Default is false as HTMLPurify can
	 *                          significantly slow down things if used everywhere. delimiter = character(s) to place
	 *                          between repeating values showHierarchy = returnInfo = Include information about the underlying display bundle in an array. Information includes the display value, the number of discrete values in the bundle for the current row, type of bundle, minumum and maximum number of repeating values allowed and where applicable a list of related ids (attribute ids for attributes, label_ids for labels, row_ids for related) [Default is false]
	 *
	 * @return mixed The processed value ready for display, unless returnInfo option is set in which case an array is
	 *               returned
	 */
	public function getDisplayValue( $po_result, $pn_placement_id, $options = null ) {
		...
		$options['useSingular'] = ( isset( $va_settings['sense'] ) && ( $va_settings['sense'] == 'singular' ) ) ? true
			: false;
		$options['returnURL']   = ( isset( $va_settings['display_mode'] ) && ( $va_settings['display_mode'] == 'url' ) )
			? true : false;

		if ( caGetOption( 'display_currency_conversion', $va_settings, false ) && $o_request
		     && $o_request->isLoggedIn()
		) {
		...
		if ( ( sizeof( $va_bundle_bits ) == 1 )
		     || ( ( sizeof( $va_bundle_bits ) == 2 )
		          && ( $va_bundle_bits[1] == 'related' ) )
		) {
			$options['template'] = caGetOption( 'format', $va_settings,
				$this->getAppConfig()->get( $va_bundle_bits[0] . '_relationship_display_format' ) );;
		} else {
		...
		if ( ! ( $vs_template = trim( $options['template'] ) ) && ( sizeof( $va_bundle_bits ) == 1 )
		     && ( $t_instance
		...
		if ( ( ! $vs_template )
		     && ( $t_element
				= ca_metadata_elements::getInstance( $va_bundle_bits[ sizeof( $va_bundle_bits ) - 1 ] ) )
		) {
		...
		if ( ! $pb_show_hierarchy && $vs_template ) {
			...
			if ( $t_instance = Datamodel::getInstanceByTableName( $va_bundle_bits[0], true ) ) {
				...
				if ( ( sizeof( $va_bundle_bits ) == 1 )
					...
					// pulling related
					$vb_is_related = true;
				} elseif ( ( sizeof( $va_bundle_bits_proc ) > 1 )
				           && ( in_array( $vs_tmp = array_pop( $va_bundle_bits_proc ), array( 'related' ) ) )
				) {
					// pulling related
				...
				if ( $vb_is_related ) {
					$vs_restrict_to_types              = ( is_array( $options['restrictToTypes'] )
					                                       && sizeof( $options['restrictToTypes'] ) )
						? "restrictToTypes=\"" . join( "|", $options['restrictToTypes'] ) . "\"" : "";
					$vs_restrict_to_relationship_types = ( is_array( $options['restrictToRelationshipTypes'] )
					                                       && sizeof( $options['restrictToRelationshipTypes'] ) )
						? "restrictToRelationshipTypes=\"" . join( "|", $options['restrictToRelationshipTypes'] ) . "\""
						: "";

					// resolve template relative to relationship
					if ( is_array( $va_path = Datamodel::getPath( $po_result->tableName(),
						$rel_table = $t_instance->tableName() ) )
					) {
						...
						$vs_unit_tag = "<unit " . ( ( $max_items > 0 ) ? "length=\"{$max_items}\"" : '' )
						               . " relativeTo=\"" . $va_path[1] . "\" delimiter=\"" . $options['delimiter']
						               . "\" {$vs_restrict_to_types} {$vs_restrict_to_relationship_types} {$vs_sort_attr} {$vs_sort_dir_attr}>";
						...
						switch ( sizeof( $va_path ) ) {
							case 3:
								...
								$vs_val = $po_result->getWithTemplate( ( caGetOption( 'showCurrentOnly', $options,
										true )
								                                         && ! $vs_restrict_to_types
								                                         && ! $vs_restrict_to_relationship_types )
									? $vs_template : $vs_unit_tag . $vs_template . "</unit>", $options );
								break;
							case 2:
								...
								if ( method_exists( $t_rel, 'isSelfRelationship' ) && $t_rel->isSelfRelationship() ) {
									...
									$vs_val = $po_result->getWithTemplate( $vs_unit_tag . $vs_template . "</unit>",
										array_merge( $options,
											array( 'primaryIDs' => array( $po_result->tableName() => array( $po_result->getPrimaryKey() ) ) ) ) );
								} else {
					...
					$vs_val = $po_result->getWithTemplate( $vs_template, [
						'relativeToContainer' => ( ca_metadata_elements::getElementDatatype( $va_bundle_bits[ sizeof( $va_bundle_bits )
						                                                                                      - 1 ] )
						                           === 0 ) ? $vs_bundle_name : null,
						'filters'             => $options['filters'],
						'delimiter'           => $options['delimiter'],
						'policy'              => $va_settings['policy']
			...
			if ( $vb_include_nonprimary_media ) {
				$po_result->filterNonPrimaryRepresentations( false );
			}
			...
			$vs_val = $po_result->get( join( ".", $va_bundle_bits ),
				array_merge( [ 'doRefSubstitution' => true ], $options,
					[ 'policy' => $va_settings['policy'], 'returnPath' => $options['forReport'] ] ) );
			if ( $vb_include_nonprimary_media ) {
				$po_result->filterNonPrimaryRepresentations( true );
			}
		}
		...
		if ( $vb_return_info ) {
			...
			if ( is_array( $tmp = $po_result->get( join( ".", $va_bundle_bits ),
				array_merge( $options, [ 'returnWithStructure' => true ] ) ) )
			) {
			...
			if ( $va_bundle_bits[0] !== $po_result->tableName() ) {
				...
				if ( $t_instance->getProperty( 'ID_NUMBERING_ID_FIELD' ) === $va_bundle_bits[1] ) {
					// ... except for idno's
					$vb_editable = false;
				} elseif ( in_array( $va_bundle_bits[1],
					[ $t_instance->getTypeFieldName(), $t_instance->primaryKey(), 'is_deaccessioned' ] )
				) {
				...
				if ( $t_element = ca_metadata_elements::getInstance( $va_bundle_bits[1] ) ) {
					if ( $t_restriction = $t_element->getTypeRestrictionInstanceForElement( $t_instance->tableNum(),
						$po_result->get( $t_instance->tableName() . '.' . $t_instance->getTypeFieldName() ) )
					) {
				...
				return array(
					...
					'ids'            => array_keys( $info_data ),
					'inlineEditable' => ca_bundle_displays::attributeTypeSupportsInlineEditing( $vn_data_type )
					                    && ( $vn_count >= 0 )
					                    && ( $vn_count <= 1 )
				);
			} else {
	...
	public function savePlacementsFromHTMLForm( $request, $ps_form_prefix, $ps_placement_code ) {
		;
		if ( $vs_bundles = $request->getParameter( "{$ps_placement_code}{$ps_form_prefix}displayBundleList",
			pString )
		) {
			...
			foreach ( $placements as $placement_id => $va_bundle_info ) {
				if ( ! in_array( $va_bundle_info['bundle_name'] . '_' . $va_bundle_info['placement_id'],
					$va_bundles )
				) {
					$t_display->removePlacement( $va_bundle_info['placement_id'],
						array( 'user_id' => $request->getUserID() ) );
			...
			foreach ( $va_bundles as $i => $vs_bundle ) {
				...
				foreach ( $_REQUEST as $vs_key => $vs_val ) {
					if ( preg_match( "!^{$vs_bundle_proc}_([\d]+)_([^\d]+.*)$!", $vs_key, $va_matches ) ) {
						...
						if ( preg_match( '!(.*)_([a-z]{2}_[A-Z]{2})$!', $va_matches[2], $va_locale_matches ) ) {
							$vn_locale_id                                                                  = isset( $va_locale_list[ $va_locale_matches[2] ] )
								? (int) $va_locale_list[ $va_locale_matches[2] ]['locale_id'] : 0;
				...
				if ( $placement_id === 0 ) {
					$t_display->addPlacement( $vs_bundle, $va_settings[ $placement_id ], $i + 1, array(
						'user_id'             => $request->getUserID(),
						'additional_settings' => $va_available_bundles[ $vs_bundle ]['settings']
					) );
					if ( $t_display->numErrors() ) {
						$this->errors = $t_display->errors;
					...
					$t_placement = new ca_bundle_display_placements( $placement_id,
						$va_available_bundles[ $vs_bundle ]['settings'] );
					if ( $this->inTransaction() ) {
						$t_placement->setTransaction( $this->getTransaction() );
					}
					$t_placement->setMode( ACCESS_WRITE );
					...
					if ( is_array( $va_settings[ $placement_id ] ) ) {
						...
						foreach ( $t_placement->getAvailableSettings() as $vs_setting => $va_setting_info ) {
							$vs_val = isset( $va_settings[ $placement_id ][ $vs_setting ] )
								? $va_settings[ $placement_id ][ $vs_setting ] : null;
	 ...
	 *                           includeSubtypes = automatically expand type restriction to include sub-types. [Default
	 *                           is false]
	 *
	...
	public function removeTypeRestriction( $pn_type_id = null ) {
		...
		if ( is_array(
			$va_displays = ca_bundle_display_type_restrictions::find( $va_params, [ 'returnAs' => 'modelInstances' ] ) )
		) {
	 ...
	 *
	 * @param int $pn_type_id Type to limit returned restrictions to; if omitted or null then all restrictions are
	 *                        returned
	 *
	...
	public function getTypeRestrictionsHTMLFormBundle( $request, $ps_form_name, $ps_placement_code, $options = null ) {
		...
		$vs_subtype_element = caProcessTemplate( $this->getAppConfig()
		                                              ->get( 'form_element_display_format_without_label' ), [
			'ELEMENT' => _t( 'Include subtypes?' ) . ' ' . caHTMLCheckboxInput( 'type_restriction_include_subtypes',
					[ 'value' => '1', 'checked' => $vb_include_subtypes ] )
		] );
		$o_view->setVar( 'type_restrictions',
			$t_instance->getTypeListAsHTMLFormElement( 'type_restrictions[]', array( 'multiple' => 1, 'height' => 5 ),
				array( 'forceEnabled' => true, 'value' => 0, 'values' => $va_restriction_type_ids ) )
			. $vs_subtype_element );
	...
	public function saveTypeRestrictionsFromHTMLForm( $request, $ps_form_prefix, $ps_placement_code ) {
		...
		return $this->setTypeRestrictions( $request->getParameter( 'type_restrictions', pArray ),
	 ...
	 *                             user_id = id of current user; used to calculate access restrictions. [Default is
	 *                             null] type_id = id of type to restrict display to. [Default is null]
	 *
	 * @return array Array with two keys: "displayList" contains the list of bundles; "headers" contains column headers
	 *               for the editor
	 */
	public function getDisplayListForResultsEditor( string $table, $options = null ) {
		...
		if ( ! sizeof( $display_list ) ) {
			...
			if ( method_exists( $t_model, 'getLabelTableInstance' ) && ( $t_label = $t_model->getLabelTableInstance() )
			     && ! ( ( $table === 'ca_objects' ) && ( $this->getAppConfig()->get( 'ca_objects_dont_use_labels' ) ) )
			) {
				$display_list['-2'] = array(
					'placement_id'              => '-2',
					'bundle_name'               => "{$table}.preferred_labels",
					'display'                   => $t_label->getDisplayLabel( $t_label->tableName() . '.'
		...
		if ( method_exists( $t_model, 'getApplicableElementCodes' ) ) {
			...
			foreach ( $display_list as $i => $display_item ) {
				...
				if ( $t_model->hasField( $tmp[1] ) ) {
					if ( $t_model->getFieldInfo( $tmp[1], 'FIELD_TYPE' )
					     == FT_MEDIA
					) { // sorting media fields doesn't really make sense and can lead to sql errors
						continue;
					}
					$display_list[ $i ]['is_sortable'] = true;
				...
				if ( isset( $va_attribute_list[ $tmp[1] ] )
				     && $va_sortable_elements[ $va_attribute_list[ $tmp[1] ] ]
				) {
					$display_list[ $i ]['is_sortable'] = true;
					$display_list[ $i ]['bundle_sort'] = $display_item['bundle_name'];
					continue;
	...
	static public function makeBundlesForResultsEditor( $bundles, $settings = null ) {
		...
		foreach ( $bundles as $i => $field ) {
			...
			$bundle       = preg_replace( "!\.related$!", "",
				$bundle );  // Remove .related specifier as editor form generator doesn't need or recognize it
			$placements[] = [
	 ...
	 *  (2) "complex" editing from a popup editing window. Data is submitted from a form as standard editor UI form
	 *  data from a psuedo editor UI screen.
	 *
	 * @param string $table   Name of edited tables
	 * @param array  $options Options include:
	 *                        request = The current request, used to fetch data to save. This option is mandatory. If
	 *                        not set no data will be saved. [Default is null]
	 *
				...
				if ( $t_subject->load( $id ) ) {
					...
					switch ( $bundle_info['type'] ) {
						case 'intrinsic':
							...
							$vs_key = 'P' . $bundle['placement_id'] . '_resultsEditor'
							          . $tmp[1]; // bare field name for intrinsics
						...
						case 'nonpreferred_label':
							$vs_label_id = null;
							if (
								is_array( $tmp = $t_subject->get( $bundle['bundle_name'],
							...
							$vs_key_stub = 'P' . $bundle['placement_id'] . ( ( $bundle_info['type']
							                                                   == 'nonpreferred_label' )
									? '_resultsEditor_NPref' : '_resultsEditor_Pref' );
							$vs_key      = $vs_key_stub . $t_subject->getLabelDisplayField() . '_' . $vs_label_id;
							$request->setParameter( $vs_locale_key = $vs_key_stub . 'locale_id_' . $vs_label_id,
								$_REQUEST[ $vs_locale_key ] = 1 );
						...
						case 'attribute':
							...
							$vs_key = 'P' . $bundle['placement_id'] . '_resultsEditor_attribute_' . $vn_element_id . '_'
							          . $vn_element_id . '_' . $vs_attribute_id;
	...
	public function saveResultsEditorComplexData( $table, $options = null ) {
		...
		if ( $t_subject->getAppConfig()->get( 'perform_type_access_checking' ) ) {
			$restrict_to_types = caGetTypeRestrictionsForUser( $this->ops_tablename,
		...
		if ( $t_subject->getAppConfig()->get( 'perform_source_access_checking' ) ) {
			if ( is_array( $restrict_to_sources = caGetSourceRestrictionsForUser( $this->ops_tablename,
				array( 'access' => __CA_BUNDLE_ACCESS_EDIT__ ) ) )
			) {
				if (
					( ! $t_subject->get( 'source_id' ) )
					|| ( $t_subject->get( 'source_id' )
					     && ! in_array( $t_subject->get( 'source_id' ), $restrict_to_sources ) )
					|| ( ( strlen( $vn_source_id = $request->getParameter( 'source_id', pInteger ) ) )
					     && ! in_array( $vn_source_id, $restrict_to_sources ) )
				) {
					$t_subject->set( 'source_id', $t_subject->getDefaultSourceID( array( 'request' => $request ) ) );
				}

				if ( is_array( $restrict_to_sources )
				     && ! in_array( $t_subject->get( 'source_id' ), $restrict_to_sources )
				) {
					return [
						'status'  => 30,
		...
		$vb_save_rc = $t_subject->saveBundlesForScreen( null, $request,
			$save_options = [
				'bundles'  => ca_bundle_displays::makeBundlesForResultsEditor( [ $bundle ] ),
				'formName' => 'complex'
			]
		);
		...
		$va_general_errors
		           = $request->getActionErrors( 'general' );        // just "general" errors - ones that are not attached to a specific part of the form

		if ( is_array( $va_errors ) && is_array( $va_general_errors )
		     && ( ( sizeof( $va_errors ) - sizeof( $va_general_errors ) ) > 0 )
		) {
			$error_list    = [];
			$no_save_error = false;
		...
		$o_app_plugin_manager->hookSaveItem( array(
			'id'         => $vn_subject_id,
			'table_num'  => $t_subject->tableNum(),
			'table_name' => $t_subject->tableName(),
			'instance'   => $t_subject,
