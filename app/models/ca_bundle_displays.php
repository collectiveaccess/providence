...
class ca_bundle_displays extends BundlableLabelableBaseModelWithAttributes
{
    ...
    public function addPlacement($ps_bundle_name, $pa_settings, $pn_rank = null, $pa_options = null)
    {
        ...
        $t_placement = new ca_bundle_display_placements(
            null,
            is_array(
                $pa_options['additional_settings']
            ) ? $pa_options['additional_settings'] : null
        );
    ...
    public function getPlacements($pa_options = null)
    {
        ...
        if ($qr_res->numRows() > 0) {
            ...
            while ($qr_res->nextRow()) {
                ...
                $va_placements[$vn_placement_id]['settings'] = $va_settings = caUnserializeForDatabase(
                    $qr_res->get('settings')
                );
                ...
                if (!$pb_settings_only) {
                    ...
                    $va_placements[$vn_placement_id]['settingsForm'] = $t_placement->getHTMLSettingForm(
                        array('id' => $vs_bundle_name . '_' . $vn_placement_id, 'settings' => $va_settings)
                    );
                } else {
                    $t_instance = Datamodel::getInstanceByTableName($va_bundle_name[0], true);
                    $va_placements[$vn_placement_id]['display'] = ($t_instance ? $t_instance->getDisplayLabel(
                        $vs_bundle_name
                    ) : "???");
                }
                                ...
                                switch ($t_subject->getFieldInfo($va_bundle_name[1], 'DISPLAY_TYPE')) {
                                    case 'DT_SELECT':
                                        ...
                                        if ($vs_list_code && ($t_list->numItemsInList($vs_list_code) <= 500)) {
                                            ...
                                            $va_placements[$vn_placement_id]['inlineEditingListValues'] = array_values(
                                                $va_list_item_labels
                                            );
                                            $va_placements[$vn_placement_id]['inlineEditingListValueMap'] = array_flip(
                                                $va_list_item_labels
                                            );
                                        } else {
                                            $va_placements[$vn_placement_id]['inlineEditingType'] = DT_FIELD;
    ...
    public function getBundleDisplaysAsHTMLSelect($ps_select_name, $pa_attributes = null, $pa_options = null)
    {
        ...
        if (!isset($pa_options['dontIncludeSubtypesInTypeRestriction'])) {
            $pa_options['dontIncludeSubtypesInTypeRestriction'] = true;
        }
        ...
        if (
            ...
            ||
            (isset($pa_options['addDefaultDisplayIfEmpty']) && ($pa_options['addDefaultDisplayIfEmpty']) && (!sizeof(
        ...
        foreach ($va_available_displays as $vn_display_id => $va_info) {
            if ($ps_context && is_array($va_info['settings']['show_only_in']) && sizeof(
                    $va_info['settings']['show_only_in']
                ) && !in_array($ps_context, $va_info['settings']['show_only_in'])) {
                continue;
            }
    ...
    public function getBundleDisplayTypeName($pm_table_name_or_num = null, $pa_options = null)
    {
        ...
        return (isset($pa_options['number']) && ($pa_options['number'] == 'plural')) ? $t_instance->getProperty(
            'NAME_PLURAL'
    ...
    public function getAvailableBundles($pm_table_name_or_num = null, $pa_options = null)
    {
        ...
        $vs_format = (isset($pa_options['format']) && in_array(
                $pa_options['format'],
                array('simple', 'full')
            )) ? $pa_options['format'] : 'full';
    ...
    public function getPlacementsInDisplay($pa_options = null)
    {
        ...
        foreach ($va_placements as $vn_placement_id => $va_placement) {
            ...
            $vs_display = "<div id='bundleDisplayEditor_{$vn_placement_id}'><span class='bundleDisplayEditorPlacementListItemTitle'>" . caUcFirstUTF8Safe(
                    $t_instance->getProperty('NAME_SINGULAR')
                ) . "</span> {$vs_label}</div>";
    ...
    public function getDisplayValue($po_result, $pn_placement_id, $pa_options = null)
    {
        ...
        if ((sizeof($va_bundle_bits) == 1) || ((sizeof($va_bundle_bits) == 2) && ($va_bundle_bits[1] == 'related'))) {
            $pa_options['template'] = caGetOption(
                'format',
                $va_settings,
        ...
        if (!($vs_template = trim($pa_options['template'])) && (sizeof(
                    $va_bundle_bits
        ...
        if (!$pb_show_hierarchy && $vs_template) {
            ...
            if ($t_instance = Datamodel::getInstanceByTableName($va_bundle_bits[0], true)) {
                ...
                if ($vb_is_related) {
                    $vs_restrict_to_types = (is_array($pa_options['restrictToTypes']) && sizeof(
                            $pa_options['restrictToTypes']
                        )) ? "restrictToTypes=\"" . join("|", $pa_options['restrictToTypes']) . "\"" : "";
                    $vs_restrict_to_relationship_types = (is_array(
            ...
            $vs_val = $po_result->get(
                join(".", $va_bundle_bits),
                array_merge(['doRefSubstitution' => true], $pa_options)
            );
        }
        ...
        if ($vb_return_info) {
            ...
            $va_info_data = array_shift(
                $va_tmp = $po_result->get(
                    join(".", $va_bundle_bits),
                    array_merge($pa_options, ['returnWithStructure' => true])
                )
            );
            if (!is_array($va_info_data)) {
                $va_info_data = [];
            }
    ...
    {
        ;
        if ($vs_bundles = $po_request->getParameter(
            "{$ps_placement_code}{$ps_form_prefix}displayBundleList",
            pString
        )) {
    ...
    public function removeTypeRestriction($pn_type_id = null)
    {
        ...
        if (is_array(
            $va_uis = ca_bundle_display_type_restrictions::find($va_params, ['returnAs' => 'modelInstances'])
        )) {
     ...
     */
    public function getTypeRestrictionsHTMLFormBundle(
        $po_request,
        $ps_form_name,
        $ps_placement_code,
        $pa_options = null
        ...
        $o_view->setVar(
            ...
            $t_instance->getTypeListAsHTMLFormElement(
                ...
                array('value' => 0, 'values' => $va_restriction_type_ids)
            ) . $vs_subtype_element
        );
    ...
    public function saveTypeRestrictionsFromHTMLForm($po_request, $ps_form_prefix, $ps_placement_code)
    {
        ...
        return $this->setTypeRestrictions(
            $po_request->getParameter('type_restrictions', pArray),
            [
                'includeSubtypes' => $po_request->getParameter(
                    'type_restriction_include_subtypes',
                    pInteger
                )
            ]
    ...
    public function getDisplayListForResultsEditor($ps_tablename, $pa_options = null)
    {
        ...
        if (!sizeof($va_display_list)) {
            ...
            if (method_exists($t_model, 'getLabelTableInstance') && ($t_label = $t_model->getLabelTableInstance(
                )) && !(($ps_tablename === 'ca_objects') && ($this->getAppConfig()->get(
    ...
    static public function makeBundlesForResultsEditor($pa_bundles, $pa_settings = null)
    {
        ...
        foreach ($pa_bundles as $vn_i => $vs_field) {
            ...
            if ($t_instance = Datamodel::getInstanceByTableName($va_tmp[0], true)) {
                if ($vs_edit_bundle = $t_instance->getFieldInfo(
                    $va_tmp[1],
                    'RESULTS_EDITOR_BUNDLE'
                )) {    // substitute bundle name for intrinsic (used to all "special" bundles to be used for editing of intrinsics)
    ...
    {
        if (!$t_subject = Datamodel::getInstanceByTableName($ps_tablename, true)) {
            return null;
        }
        $po_request = caGetOption('request', $pa_options, null);
    ...
    public function saveResultsEditorComplexData($ps_tablename, $pa_options = null)
    {
        ...
        if ($t_subject->getAppConfig()->get('perform_source_access_checking')) {
            if (is_array(
                ...
                if (
                    ...
                    ||
                    ((strlen($vn_source_id = $po_request->getParameter('source_id', pInteger))) && !in_array(
                            $vn_source_id,
                ...
                if (is_array($va_restrict_to_sources) && !in_array(
                        $t_subject->get('source_id'),
                        $va_restrict_to_sources
                    )) {
                    return [
        ...
        $vb_save_rc = $t_subject->saveBundlesForScreen(
            ...
            $po_request,
            $va_options = array(
                'bundles' => ca_bundle_displays::makeBundlesForResultsEditor(
                    array($ps_bundle)
                ),
                'formName' => 'complex'
            )
        ...
        $va_general_errors = $po_request->getActionErrors(
            'general'
