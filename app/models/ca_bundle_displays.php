	...
	public function getPlacements($options=null) {
		...
		if ($qr_res->numRows() > 0) {
			...
			while($qr_res->nextRow()) {
				...
				if(is_array($label = caExtractValuesByUserLocale((is_array($va_settings) && is_array($va_settings['label'] ?? null)) ? [$va_settings['label']] : [])) && sizeof($label) && ($label = array_shift($label))) {
