	...
	public function getPlacements($options=null) {
		...
		if ($qr_res->numRows() > 0) {
			...
			while($qr_res->nextRow()) {
				...
				$placements[$placement_id]['settings']     = $va_settings;
				...
				// Label aus settings holen – jetzt ist $va_settings garantiert ein Array
				if (
					is_array($va_settings)
					&& isset($va_settings['label'])
					&& is_array($va_settings['label'])
					&& is_array($label = caExtractValuesByUserLocale($va_settings['label']))
					&& sizeof($label)
	...
	public function getBundleDisplays($options=null) {
		...
		if($context = caGetOption('context', $options, null)) {
			foreach($va_displays as $display_id => $info) {
				...
				$show_only_in = $settings['show_only_in'] ?? null;
				if (is_array($show_only_in) && sizeof($show_only_in) && !in_array($context, $show_only_in)) {
