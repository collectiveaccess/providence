...
BaseModel::$s_ca_models_definitions['ca_notifications'] = array(
	...
	'FIELDS'        => array(
		'notification_id'   => array(
			...
			'DESCRIPTION'    => _t( 'Unique numeric identifier used by CollectiveAccess internally to identify this notification' )
		),
		'notification_type' => array(
			...
			'LABEL'                     => _t( 'Notification type' ),
			'DESCRIPTION'               => _t( 'Indicates the type of this notification.' )
		),
		'datetime'          => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Notification date and time' ),
			'DESCRIPTION'    => _t( 'Date and time for notification' )
		),
		'message'           => array(
			'FIELD_TYPE'     => FT_TEXT,
			'DISPLAY_TYPE'   => DT_FIELD,
			...
			'DEFAULT'        => '',
			'LABEL'          => _t( 'Message' ),
			'DESCRIPTION'    => _t( 'Notification message' )
		),
		'is_system'         => array(
			'FIELD_TYPE'     => FT_BIT,
			'DISPLAY_TYPE'   => DT_SELECT,
		...
		'notification_key'  => array(
			...
			'LABEL'         => _t( 'MD5 hash' ),
			'DESCRIPTION'   => _t( 'MD5-generated identifier for this notification.' ),
		...
		'extra_data'        => array(
			...
			'IS_NULL'        => false,
			'DEFAULT'        => '',
			'LABEL'          => 'Notification-specific data',
			'DESCRIPTION'    => 'Additional data attached to this notification'
		)
	)
);
...
class ca_notifications extends BaseModel {
	 ...
	 * @param int $pn_user_id An optional user_id. If provided, notifications attached to ca_users records will only be
	 *                        marked as read if the subject user_id matches. [Default is null]
	 *
	...
	public static function markAsRead( $pn_subject_id, $pn_user_id = null ) {
		...
		if ( $t_subject->isLoaded() ) {
			if ( ( $t_subject->get( 'table_num' ) == 94 ) && ( $pn_user_id )
			     && ( (int) $pn_user_id !== (int) $t_subject->get( 'row_id' ) )
			) { // 94 = ca_users
				return false;
			}
			$t_subject->setMode( ACCESS_WRITE );
