...
BaseModel::$s_ca_models_definitions['ca_notifications'] = array(
    ...
    'FIELDS' => array(
        'notification_id' => array(
            ...
            'DESCRIPTION' => _t(
                'Unique numeric identifier used by CollectiveAccess internally to identify this notification'
            )
        ),
...
class ca_notifications extends BaseModel
{
    ...
    public static function markAsRead($pn_subject_id, $pn_user_id = null)
    {
        ...
        if ($t_subject->isLoaded()) {
            if (($t_subject->get('table_num') == 94) && ($pn_user_id) && ((int)$pn_user_id !== (int)$t_subject->get(
                        'row_id'
                    ))) { // 94 = ca_users
                return false;
            }
