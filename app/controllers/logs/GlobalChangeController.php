...
class GlobalChangeController extends ActionController
{
    ...
    {
        if (!$this->request->getUser()->canDoAction('can_view_my_change_logs') && !$this->request->getUser(
            )->canDoAction('can_view_change_logs')) {
            $this->response->setRedirect(
                $this->request->getAppConfig()->get('error_display_url') . '/n/2320?r=' . urlencode(
        ...
        $this->view->setVar(
            'user_list',
            $user_list = ApplicationChangeLog::getChangeLogUsersForSelect(
        ...
        if ($can_filter_by_user = $this->request->user->canDoAction('can_view_change_logs')) {
            ...
            if (!$filter_user_id && !isset($_REQUEST['filter_user'])) {
                $filter_user_id = Session::varExists('global_change_log_filter_user_id') ? Session::getVar(
                    'global_change_log_filter_user_id'
                ) : null;
            }
        ...
        $this->view->setVar(
            'params_set',
            $params_set = ($params_set = $filter_user_id || $filter_change_type || $filter_table || ($filter_daterange && ($filter_daterange != _t(
                            'any time'
                        ))))
        );
