...
class ElementsController extends BaseEditorController {
				...
				foreach ( $va_type_restrictions as $va_restriction ) {
					if ( $t_restriction->load( $va_restriction['restriction_id'] ) ) {
						...
						foreach ( $va_restriction_settings as $vs_setting => $va_setting_info ) {
							...
							$va_initial_values[ $t_restriction->getPrimaryKey() ]
								= array_merge( $t_restriction->getFieldValuesArray(), $va_settings );
						}
					}
				}
			}
			...
			foreach ( $va_new_labels as $va_label ) {
				...
				if ( $t_element_label->numErrors() ) {
					foreach ( $t_element_label->errors() as $o_e ) {
						$this->request->addActionError( $o_e, 'general' );
						$this->notification->addNotification( $o_e->getErrorDescription(),
			...
			if ( is_array( $va_settings = $t_element->getAvailableSettings() ) ) {
				...
				foreach ( $va_settings as $vs_setting_key => $va_setting_info ) {
					...
					if ( ! ( $t_element->setSetting( $vs_setting_key, $vs_val, $vs_error ) ) ) {
						$this->notification->addNotification( _t( "Setting %2 is not valid: %1", $vs_error,
							$vs_setting_key ), __NOTIFICATION_TYPE_ERROR__ );
						continue;
					}
					$t_element->update();
				}
			...
			foreach ( $_REQUEST as $vs_key => $vs_value ) {
				if ( preg_match( '!^type_restrictions_table_num_([\d]+)$!', $vs_key, $va_matches ) ) {
					...
					if ( $t_restriction->load( $va_matches[1] ) ) {
						...
						$t_restriction->set( 'table_num',
							$this->request->getParameter( 'type_restrictions_table_num_' . $va_matches[1], pInteger ) );
						$t_restriction->set( 'type_id',
							( $vn_type_id = $this->request->getParameter( 'type_restrictions_type_id_' . $va_matches[1],
								pInteger ) ) ? $vn_type_id : null );
						$t_restriction->set( 'include_subtypes',
							( $vn_include_subtypes = $this->request->getParameter( 'type_restrictions_include_subtypes_'
							                                                       . $va_matches[1], pInteger ) )
								? $vn_include_subtypes : null );

						foreach ( $va_settings as $vs_setting ) {
							$t_restriction->setSetting( $vs_setting,
								$this->request->getParameter( 'type_restrictions_setting_' . $vs_setting . '_'
				...
				if ( preg_match( '!^type_restrictions_table_num_new_([\d]+)$!', $vs_key, $va_matches ) ) {
					...
					$t_restriction->set( 'table_num',
						$this->request->getParameter( 'type_restrictions_table_num_new_' . $va_matches[1], pInteger ) );
					$t_restriction->set( 'type_id',
						( $vn_type_id = $this->request->getParameter( 'type_restrictions_type_id_new_' . $va_matches[1],
							pInteger ) ) ? $vn_type_id : null );
					$t_restriction->set( 'include_subtypes',
						( $vn_include_subtypes = $this->request->getParameter( 'type_restrictions_include_subtypes_new_'
						                                                       . $va_matches[1], pInteger ) )
							? $vn_include_subtypes : null );

					foreach ( $va_settings as $vs_setting ) {
						$t_restriction->setSetting( $vs_setting,
							$this->request->getParameter( 'type_restrictions_setting_' . $vs_setting . '_new_'
							                              . $va_matches[1], pString ) );
					}
	...
	public function MoveElementUp() {
		$t_element = $this->getElementObject();
		if ( is_array(
			$va_ranks_to_stabilize = $this->elementRankStabilizationNeeded( $t_element->get( 'parent_id' ) ) )
		) {
		...
		if ( ! $qr_tmp->nextRow() ) {
			$this->notification->addNotification( _t( "This element is at the top of the list" ),
				__NOTIFICATION_TYPE_ERROR__ );
		} else { /* swap ranks */
	...
	public function MoveElementDown() {
		...
		", $t_element->get( 'rank' ), $t_element->get( 'parent_id' ) );
		if ( ! $qr_tmp->nextRow() ) {
			$this->notification->addNotification( _t( "This element is at the bottom of the list" ),
				__NOTIFICATION_TYPE_ERROR__ );
		} else { /* swap ranks */
			$t_element_rankswap = new ca_metadata_elements( $qr_tmp->get( 'element_id' ) );
			$this->swapRanks( $t_element, $t_element_rankswap );
