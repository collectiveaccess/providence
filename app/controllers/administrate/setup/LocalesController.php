...
class LocalesController extends ActionController {
	...
	public function ListLocales() {
		...
		$this->view->setVar( 'locale_list', ca_locales::getLocaleList( array(
			'sort_field'    => $vs_sort_field,
			'sort_order'    => 'asc',
			'index_by_code' => false
		) ) );
	...
	public function Delete() {
		...
		if ( $this->request->getParameter( 'confirm', pInteger ) ) {
			...
			if ( $t_locale->numErrors() ) {
				foreach ( $t_locale->errors() as $o_e ) {
					$this->notification->addNotification( _t( "Could not delete locale: %1", $o_e->getErrorMessage() ),
						__NOTIFICATION_TYPE_ERROR__ );
					$this->request->addActionError( $o_e, 'general' );
				}
			} else {
