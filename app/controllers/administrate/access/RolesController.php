...
class RolesController extends ActionController
{
    ...
    public function Edit()
    {
        ...
        foreach (ca_users::$s_bundlable_tables as $vs_table) {
            ...
            $va_table_names[$vs_table] = caUcFirstUTF8Safe($t_instance->getProperty('NAME_PLURAL'));
            $va_types = $t_list->getListItemsAsHierarchy(
                $vs_list_code,
        ...
        foreach (ca_users::$s_bundlable_tables as $vs_table) {
            ...
            $va_table_names[$vs_table] = caUcFirstUTF8Safe($t_instance->getProperty('NAME_PLURAL'));
            $va_sources = $t_list->getListItemsAsHierarchy(
                $vs_list_code,
                array('additionalTableToJoin' => 'ca_list_item_labels')
            );
        ...
        if (is_array(
            $va_access_statuses = $t_list->getListItemsAsHierarchy(
                'access_statuses',
                array('additionalTableToJoin' => 'ca_list_item_labels')
            )
        )) {
    ...
    public function Save()
    {
        ...
        if ($t_role->getAppConfig()->get('perform_source_access_checking')) {
            ...
            foreach (ca_users::$s_bundlable_tables as $vs_table) {
                ...
                $va_source_access_settings[$vs_table . '_default_id'] = $this->request->getParameter(
                    $vs_table . '_default_source',
                    pInteger
                );
            }
        ...
        if (is_array(
            $va_access_status_ids = $va_source_ids = $t_list->getItemsForList(
                'access_statuses',
                array('idsOnly' => true)
            )
        )) {
        ...
        AppNavigation::clearMenuBarCache(
            $this->request
