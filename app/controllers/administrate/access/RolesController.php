	...
	public function Edit() {
		...
		foreach ( ca_users::$s_bundlable_tables as $vs_table ) {
			...
			foreach ( $va_available_bundles as $vs_bundle_name => $va_bundle_info ) {
				...
				$vn_access                                      = isset( $va_bundle_access_settings[ $vs_table . '.'
				                                                                                     . $vs_bundle_name ] )
					? $va_bundle_access_settings[ $vs_table . '.' . $vs_bundle_name ] : $vn_default_bundle_access_level;
		...
		foreach ( ca_users::$s_bundlable_tables as $vs_table ) {
			...
			$va_table_names[ $vs_table ] = caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_PLURAL' ) );
			$va_types                    = $t_list->getListItemsAsHierarchy( $vs_list_code,
				array( 'additionalTableToJoin' => 'ca_list_item_labels' ) );

			if ( is_array( $va_types ) ) {
				foreach ( $va_types as $vn_i => $va_type_info ) {
					$vn_item_id                    = $va_type_info['NODE']['item_id'];
					$vn_access                     = isset( $va_type_access_settings[ $vs_table . '.' . $vn_item_id ] )
						? $va_type_access_settings[ $vs_table . '.' . $vn_item_id ] : $vn_default_type_access_level;
		...
		foreach ( ca_users::$s_bundlable_tables as $vs_table ) {
			...
			$va_table_names[ $vs_table ] = caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_PLURAL' ) );
			$va_sources                  = $t_list->getListItemsAsHierarchy( $vs_list_code,
				array( 'additionalTableToJoin' => 'ca_list_item_labels' ) );

			if ( is_array( $va_sources ) ) {
				foreach ( $va_sources as $vn_i => $va_source_info ) {
					$vn_item_id                      = $va_source_info['NODE']['item_id'];
					$vn_access                       = isset( $va_source_access_settings[ $vs_table . '.'
					                                                                      . $vn_item_id ] )
						? $va_source_access_settings[ $vs_table . '.' . $vn_item_id ] : $vn_default_source_access_level;
		...
		if ( is_array( $va_access_statuses = $t_list->getListItemsAsHierarchy( 'access_statuses',
			array( 'additionalTableToJoin' => 'ca_list_item_labels' ) ) )
		) {
			...
			foreach ( $va_access_statuses as $vn_i => $va_access_status_info ) {
				...
				$vn_access = isset( $va_access_status_settings[ $vn_item_id ] )
	...
	public function Save() {
		...
		if ( $t_role->getAppConfig()->get( 'perform_source_access_checking' ) ) {
			...
			foreach ( ca_users::$s_bundlable_tables as $vs_table ) {
				...
				$va_source_access_settings[ $vs_table . '_default_id' ] = $this->request->getParameter( $vs_table
				                                                                                        . '_default_source',
					pInteger );
		...
		if ( is_array( $va_access_status_ids
			= $va_source_ids = $t_list->getItemsForList( 'access_statuses', array( 'idsOnly' => true ) ) )
		) {
