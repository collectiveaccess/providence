...
class UsersController extends ActionController {
	...
	public function Edit() {
		...
		if ( is_array( $va_profile_prefs ) && sizeof( $va_profile_prefs ) ) {
			...
			foreach ( $va_profile_prefs as $vs_pref ) {
				...
				$va_elements[ $vs_pref ] = array(
					'element' => $t_user->preferenceHtmlFormElement( $vs_pref ),
					'info'    => $va_pref_info,
					'label'   => $va_pref_info['label']
				);
			}
	...
	public function Save() {
		...
		$this->opo_app_plugin_manager->hookBeforeUserSaveData( array(
			'user_id'  => $t_user->getPrimaryKey(),
			'instance' => $t_user
		) );

		$vb_send_activation_email = false;
		if ( $t_user->get( "user_id" ) && $this->request->config->get( "email_user_when_account_activated" )
		     && ( $_REQUEST["active"] != $t_user->get( "active" ) )
		) {
		...
		foreach ( $t_user->getFormFields() as $vs_f => $va_field_info ) {
			...
			if ( $vs_f == 'password' ) {
				if ( ! strlen( $_REQUEST[ $vs_f ] )
				     || ! AuthenticationManager::supports( __CA_AUTH_ADAPTER_FEATURE_UPDATE_PASSWORDS__ )
				) {
					continue;
				}
			}
		...
		if ( AuthenticationManager::supports( __CA_AUTH_ADAPTER_FEATURE_UPDATE_PASSWORDS__ ) ) {
			if ( $this->request->getParameter( 'password', pString )
			     != $this->request->getParameter( 'password_confirm', pString )
			) {
				$this->request->addActionError( new ApplicationError( 1050,
					_t( "Password does not match confirmation. Please try again." ),
					"administrate/UserController->Save()", '', false, false ), 'field_password' );
			}
		}
		...
		if ( $this->request->numActionErrors() == 0 ) {
			...
			$this->opo_app_plugin_manager->hookAfterUserSaveData( array(
				'user_id'  => $t_user->getPrimaryKey(),
				'instance' => $t_user
			) );
			...
			if ( $t_user->numErrors() ) {
				...
				if ( is_array( $va_profile_prefs ) && sizeof( $va_profile_prefs ) ) {
					...
					$this->opo_app_plugin_manager->hookBeforeUserSavePrefs( array(
						'user_id'  => $t_user->getPrimaryKey(),
						'instance' => $t_user
					) );
					...
					foreach ( $va_profile_prefs as $vs_pref ) {
						if ( $this->request->getParameter( 'pref_' . $vs_pref, pString )
						     != $t_user->getPreference( $vs_pref )
						) {
					...
					$this->opo_app_plugin_manager->hookAfterUserSavePrefs( array(
						'user_id'        => $t_user->getPrimaryKey(),
						'instance'       => $t_user,
						'modified_prefs' => $va_changed_prefs
					) );
				}
				...
				if ( $vb_send_activation_email ) {
					...
					caSendmail( $t_user->get( 'email' ), $this->request->config->get( "ca_admin_email" ),
						$vs_subject_line, $vs_mail_message_text, $vs_mail_message_html, null, null, null,
	...
	public function ListUsers() {
		...
		$this->view->setVar( 'user_list', $t_user->getUserList( array(
			'sort'           => $vs_sort_field,
			'sort_direction' => 'asc',
			'userclass'      => $vn_userclass
		) ) );
		...
		if ( $qr_users->numRows() ) {
			...
			while ( $qr_users->nextRow() ) {
				...
				foreach ( $va_fields as $vs_field ) {
					switch ( $vs_field ) {
						case "userclass":
							$va_row[] = $t_user->getChoiceListValue( $vs_field,
								$qr_users->get( "ca_users." . $vs_field ) );
							break;
						# -----------------------
						case "active":
							$va_row[] = ( $qr_users->get( "ca_users.{$vs_field}" ) == 1 ) ? _t( "active" )
								: _t( "not active" );
							break;
						# -----------------------
						case "roles":
							$qr_roles
								      = $o_db->query( "SELECT r.name, r.code FROM ca_user_roles r INNER JOIN ca_users_x_roles AS cuxr ON cuxr.role_id = r.role_id WHERE cuxr.user_id = ?",
								[ $qr_users->get( 'user_id' ) ] );
							$va_row[] = join( "; ", $qr_roles->getAllFieldValues( "name" ) );
							break;
						# -----------------------
						case "groups":
							$qr_groups
								      = $o_db->query( "SELECT g.name, g.code FROM ca_user_groups g INNER JOIN ca_users_x_groups AS cuxg ON cuxg.group_id = g.group_id WHERE cuxg.user_id = ?",
								[ $qr_users->get( 'user_id' ) ] );
	...
	public function Approve() {
		...
		if ( is_array( $pa_user_ids ) && ( sizeof( $pa_user_ids ) > 0 ) ) {
			...
			if ( sizeof( $va_errors ) > 0 ) {
				...
				$this->notification->addNotification( _t( "The registrations have been approved" ),
					__NOTIFICATION_TYPE_INFO__ );
			}
		} else {
			$this->notification->addNotification( _t( "Please use the checkboxes to select registrations for approval" ),
