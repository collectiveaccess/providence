...
class UsersController extends ActionController
{
    ...
    public function Save()
    {
        ...
        $this->opo_app_plugin_manager->hookBeforeUserSaveData(
            array('user_id' => $t_user->getPrimaryKey(), 'instance' => $t_user)
        );

        $vb_send_activation_email = false;
        if ($t_user->get("user_id") && $this->request->config->get(
                "email_user_when_account_activated"
        ...
        foreach ($t_user->getFormFields() as $vs_f => $va_field_info) {
            ...
            if ($vs_f == 'password') {
                if (!strlen($_REQUEST[$vs_f]) || !AuthenticationManager::supports(
                        __CA_AUTH_ADAPTER_FEATURE_UPDATE_PASSWORDS__
                    )) {
                    continue;
                }
            }
        ...
        if (AuthenticationManager::supports(__CA_AUTH_ADAPTER_FEATURE_UPDATE_PASSWORDS__)) {
            if ($this->request->getParameter('password', pString) != $this->request->getParameter(
                    'password_confirm',
                    pString
                )) {
                $this->request->addActionError(
                    new ApplicationError(
                        ...
                        "administrate/UserController->Save()",
                        '',
                        false,
                        false
                    ),
        ...
        if ($this->request->numActionErrors() == 0) {
            ...
            $this->opo_app_plugin_manager->hookAfterUserSaveData(
                array('user_id' => $t_user->getPrimaryKey(), 'instance' => $t_user)
            );
            ...
            if ($t_user->numErrors()) {
                ...
                $va_profile_prefs = $t_user->getValidPreferences('profile');
                if (is_array($va_profile_prefs) && sizeof($va_profile_prefs)) {
                    $this->opo_app_plugin_manager->hookBeforeUserSavePrefs(
                        array('user_id' => $t_user->getPrimaryKey(), 'instance' => $t_user)
                    );

                    $va_changed_prefs = array();
                    foreach ($va_profile_prefs as $vs_pref) {
                        if ($this->request->getParameter('pref_' . $vs_pref, pString) != $t_user->getPreference(
                                $vs_pref
                            )) {
                    ...
                    $this->opo_app_plugin_manager->hookAfterUserSavePrefs(
                        array(
                ...
                if ($vb_send_activation_email) {
                    ...
                    caSendmail(
                        ...
                        $vs_subject_line,
                        $vs_mail_message_text,
                        $vs_mail_message_html,
                        null,
                        null,
                        null,
            ...
            $this->notification->addNotification(
                _t("Your entry has errors. See below for details."),
    ...
    public function ListUsers()
    {
        ...
        $this->view->setVar(
            'user_list',
            $t_user->getUserList(
                array('sort' => $vs_sort_field, 'sort_direction' => 'asc', 'userclass' => $vn_userclass)
            )
        );
            ...
            while ($qr_users->nextRow()) {
                ...
                foreach ($va_fields as $vs_field) {
                    switch ($vs_field) {
                        ...
                        case "roles":
                            $qr_roles = $o_db->query(
                                "SELECT r.name, r.code FROM ca_user_roles r INNER JOIN ca_users_x_roles AS cuxr ON cuxr.role_id = r.role_id WHERE cuxr.user_id = ?",
                        ...
                        # -----------------------
                        case "groups":
                            $qr_groups = $o_db->query(
                                "SELECT g.name, g.code FROM ca_user_groups g INNER JOIN ca_users_x_groups AS cuxg ON cuxg.group_id = g.group_id WHERE cuxg.user_id = ?",
                                [$qr_users->get('user_id')]
                            );
    ...
    public function Approve()
    {
        ...
        if (is_array($pa_user_ids) && (sizeof($pa_user_ids) > 0)) {
            ...
            if (sizeof($va_errors) > 0) {
                ...
                $this->notification->addNotification(
                    _t("The registrations have been approved"),
                    __NOTIFICATION_TYPE_INFO__
                );
            }
        } else {
            $this->notification->addNotification(
                _t("Please use the checkboxes to select registrations for approval"),
