        ...
        foreach ($pa_ids as $pn_id) {
            ...
            if (!$vn_id && method_exists($t_item, "getHierarchyList")) {
                ...
                if ($t_item->load($vn_id)) {        // id is the id of the parent for the level we're going to return
                    ...
                    // output
                    $va_display_values = caProcessTemplateForIDs(
                        $vs_template,
                        'ca_list_items',
                        array_keys($va_list_items),
                        array(
            ...
            foreach ($va_list_items as $vn_item_id => $va_item) {
                $va_list_items_sortable[caSortableValue(
                    mb_strtolower(preg_replace('![^A-Za-z0-9]!', '_', caRemoveAccents($va_item['name_plural'])))
                ) . '_' . $vn_item_id] = $va_item;
            }
            ...
            $va_list_items['_itemCount'] = ca_list_items::find(
                ['list_id' => $vn_list_id, 'parent_id' => $vn_id],
                ['returnAs' => 'count']
            ); //sizeof($va_list_items); //$t_list ? $t_list->numItemsInList() : ($qr_res ? $qr_res->numRows() : 0);
