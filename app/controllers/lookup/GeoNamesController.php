...
class GeoNamesController extends ActionController {
	...
	public function Get( $pa_additional_query_params = null, $pa_options = null ) {
		...
		$vn_max   = ( $this->request->getParameter( 'maxRows', pInteger ) ? $this->request->getParameter( 'maxRows',
			pInteger ) : 20 );
		...
		if ( mb_strlen( $ps_query ) >= 3 ) {
			...
			try {
				...
				if ( $va_attr && isset( $va_attr['value'] ) && ( (int) $va_attr['value'] > 0 ) ) {
					$va_items[0]          = array(
						'displayname' => _t( 'Connection to GeoNames with username "%1" was rejected with the message "%2". Check your configuration and make sure your GeoNames.org account is enabled for web services.',
							$vs_user, $va_attr['message'] ),
						'lat'         => '',
						'lng'         => '',
					);
