...
class ObjectCollectionHierarchyController extends BaseLookupController
{
    ...
    public function Get($pa_additional_query_params = null, $pa_options = null)
    {
        ...
        if (($vn_str_len = mb_strlen($ps_query)) > 0) {
                ...
                if (sizeof($va_ids) > 0) {
                    ...
                    if (!$pb_no_subtypes) {
                        foreach ($va_ids as $vn_id) {
                            $va_children = $t_list->getItemsForList(
                                $this->opo_item_instance->getTypeListCode(),
                    ...
                    $o_object_search->addResultFilter(
                        $this->opo_item_instance->tableName() . '.' . $this->opo_item_instance->getTypeFieldName(),
                        'IN',
                        join(",", $va_ids)
                    );
                }
            ...
            if (!$pb_exact) {
                ...
                $ps_query = trim(
                    preg_replace(
                        "![" . str_replace("!", "\\!", $o_search_config->get('search_tokenizer_regex')) . "]+!u",
                        " ",
                        $ps_query
                    )
                );
            ...
            if (!(is_array($va_object_sort = $o_config->getList('ca_objects_lookup_sort'))) || !sizeof(
                    $va_object_sort
                )) {
    ...
    protected function GetHierarchyLevelData($pa_ids)
    {
        ...
        foreach ($pa_ids as $vn_i => $pn_id) {
            ...
            if ((($vn_max_items_per_page = $this->request->getParameter(
                        'max',
                        pInteger
                    )) < 1) || ($vn_max_items_per_page > 1000)) {
            ...
            if ((!($vn_id)) && method_exists($t_item, "getHierarchyList")) {
                ...
                $va_tmp = array(
                    ...
                    $vs_label_display_field_name => $t_item->get(
                        $vs_table . '.preferred_labels.' . $vs_label_display_field_name
                    ),
                ...
                $va_tmp['children'] = $t_item->get('has_children') ? (int)$t_item->get(
                    'child_count'
                ) : 1;    // TODO: fix
                ...
                $qr_children = caMakeSearchResult(
                    $vs_table,
                    array_slice(
                        $qr_children->getAllFieldValues($vs_pk),
                        $vn_start,
                        $vn_max_items_per_page
                    )
                );
                while ($qr_children->nextHit()) {
                    ...
                    // Child count is only valid if has_children is not null
                    $va_tmp['children'] = sizeof(
                        $qr_children->get("{$vs_table}.children.{$vs_pk}", ['returnAsArray' => true])
                    );
                ...
                if (sizeof($va_sorts) && sizeof($va_item_ids)) {
                    ...
                    while ($o_res->nextHit()) {
                        $va_sorted_items[$vs_table . '-' . $o_res->get(
                            $vs_table . '.' . $vs_pk
                ...
                if ($t_item->tableName() == 'ca_collections') {
                    ...
                    $vs_object_collection_rel_type = $o_config->get(
                    ...
                    if (sizeof($va_ids)) {
                        ...
                        $va_templates = caProcessTemplateForIDs(
                            $vs_item_template,
                            'ca_objects',
                            $va_ids,
    ...
    public function GetHierarchyAncestorList()
    {
        ...
        if ($t_item->getPrimaryKey()) {
            $va_ancestors = array_reverse(
                $t_item->getHierarchyAncestors(null, array('includeSelf' => true, 'idsOnly' => true))
            );
        }
