...
class ObjectCollectionHierarchyController extends BaseLookupController {
	...
	public function Get( $pa_additional_query_params = null, $pa_options = null ) {
		...
		if ( ( $vn_str_len = mb_strlen( $ps_query ) ) > 0 ) {
				...
				if ( sizeof( $va_ids ) > 0 ) {
					...
					if ( ! $pb_no_subtypes ) {
						foreach ( $va_ids as $vn_id ) {
							$va_children = $t_list->getItemsForList( $this->opo_item_instance->getTypeListCode(),
								array( 'item_id' => $vn_id, 'idsOnly' => true ) );
							$va_ids      = array_merge( $va_ids, $va_children );
						}
						$va_ids = array_flip( array_flip( $va_ids ) );
					}
					$o_object_search->addResultFilter( $this->opo_item_instance->tableName() . '.'
					                                   . $this->opo_item_instance->getTypeFieldName(), 'IN',
						join( ",", $va_ids ) );
				}
			} else {
				$va_ids = null;
			...
			// add filters
			if ( isset( $pa_options['filters'] ) && is_array( $pa_options['filters'] )
			     && sizeof( $pa_options['filters'] )
			) {
			...
			if ( ! $pb_exact ) {
				...
				$ps_query        = trim( preg_replace( "![" . str_replace( "!", "\\!",
						$o_search_config->get( 'search_tokenizer_regex' ) ) . "]+!u", " ", $ps_query ) );
			}

			if ( ! ( is_array( $va_object_sort = $o_config->getList( 'ca_objects_lookup_sort' ) ) )
			     || ! sizeof( $va_object_sort )
			) {
				$va_object_sort = array( 'ca_objects.idno_sort' );
			}

			$qr_res = $o_object_search->search(
				'(' . $ps_query . ( intval( $pb_exact ) ? '' : '*' ) . ')' . $vs_type_query
				. $vs_additional_query_params,
			...
			if ( ! ( is_array( $va_col_sort = $o_config->getList( 'ca_collections_lookup_sort' ) ) )
			     || ! sizeof( $va_col_sort )
			) {
			...
			if ( is_array(
				...
				foreach ( $va_collections as $vn_collection_id => $va_collection ) {
					$va_collections[ $vn_collection_id ]['id'] = 'ca_collections-'
					                                             . $va_collections[ $vn_collection_id ]['id'];
				}
			}
		}
	...
	protected function GetHierarchyLevelData( $pa_ids ) {
		...
		foreach ( $pa_ids as $vn_i => $pn_id ) {
			...
			if ( ( ( $vn_max_items_per_page = $this->request->getParameter( 'max', pInteger ) ) < 1 )
			     || ( $vn_max_items_per_page > 1000 )
			) {
			...
			if ( ( ! ( $vn_id ) ) && method_exists( $t_item, "getHierarchyList" ) ) {
				...
				$va_tmp = array(
					...
					$vs_label_display_field_name => $t_item->get( $vs_table . '.preferred_labels.'
					                                              . $vs_label_display_field_name ),
				...
				if ( ! ( $vs_item_template
					= trim( $o_config->get( "{$vs_table}_hierarchy_browser_display_settings" ) ) )
				) {
				...
				$va_tmp['children'] = $t_item->get( 'has_children' ) ? (int) $t_item->get( 'child_count' )
					: 1;    // TODO: fix
				...
				if ( $t_label_instance && $t_label_instance->hasField( 'is_preferred' ) ) {
					$va_additional_wheres[]
						= "(({$vs_label_table_name}.is_preferred = 1) OR ({$vs_label_table_name}.is_preferred IS NULL))";
				}
				...
				$qr_children = caMakeSearchResult( $vs_table,
					array_slice( $qr_children->getAllFieldValues( $vs_pk ), $vn_start, $vn_max_items_per_page ) );
				while ( $qr_children->nextHit() ) {
					...
					$va_tmp['name'] = caProcessTemplateForIDs( $vs_item_template, $vs_table,
						array( $va_tmp[ $vs_pk ] ) );

					// Child count is only valid if has_children is not null
					$va_tmp['children'] = sizeof( $qr_children->get( "{$vs_table}.children.{$vs_pk}",
						[ 'returnAsArray' => true ] ) );

					if ( $t_item->tableName() == 'ca_collections' ) {
						$va_tmp['children'] += sizeof( $qr_children->get( 'ca_objects.object_id', [
							'returnAsArray'               => true,
							'restrictToRelationshipTypes' => [ $vs_object_collection_rel_type ]
						] ) );
					}
				...
				if ( sizeof( $va_sorts ) && sizeof( $va_item_ids ) ) {
					...
					while ( $o_res->nextHit() ) {
						$va_sorted_items[ $vs_table . '-' . $o_res->get( $vs_table . '.' . $vs_pk ) ]
							= $va_items[ $vs_table . '-' . $o_res->get( $vs_table . '.' . $vs_pk ) ];
					}
				...
				if ( $t_item->tableName() == 'ca_collections' ) {
					...
					$vs_object_collection_rel_type
						= $o_config->get( 'ca_objects_x_collections_hierarchy_relationship_type' );
					...
					foreach ( $va_cross_table_items as $vn_x_item_id => $va_x_item ) {
						...
						$va_items[ 'ca_objects-' . $va_x_item['object_id'] ][ $va_x_item['locale_id'] ]['item_id']
							= 'ca_objects-' . $va_x_item['object_id'];
						$va_items[ 'ca_objects-' . $va_x_item['object_id'] ][ $va_x_item['locale_id'] ]['parent_id']
							= $vn_id;

						unset( $va_items[ 'ca_objects-'
						                  . $va_x_item['object_id'] ][ $va_x_item['locale_id'] ]['labels'] );
					...
					if ( ! ( $vs_item_template
						= trim( $o_config->get( "ca_objects_hierarchy_browser_display_settings" ) ) )
					) {
					...
					if ( sizeof( $va_ids ) ) {
						...
						$va_templates    = caProcessTemplateForIDs( $vs_item_template, 'ca_objects', $va_ids,
						...
						foreach ( $va_ids as $vn_i => $vn_id ) {
							$va_items[ 'ca_objects-' . $vn_id ][ $va_x_item['locale_id'] ]['name']
								= $va_templates[ $vn_i ];
						}
					}
				}
			...
			$va_items_for_locale['_primaryKey']
			                                   = $vs_level_pk;    // pass the name of the primary key so the hierbrowser knows where to look for item_id's
	...
	public function GetHierarchyAncestorList() {
		...
		if ( $t_item->getPrimaryKey() ) {
			$va_ancestors = array_reverse( $t_item->getHierarchyAncestors( null,
				array( 'includeSelf' => true, 'idsOnly' => true ) ) );
		}
			...
			if ( is_array( $va_cross_table_items ) ) {
				...
				foreach ( $va_cross_table_items as $vn_x_item_id => $va_x_item ) {
					...
					if ( ! ( $va_collection_ancestor_list
						= $t_collection->getHierarchyAncestors( $va_x_item['collection_id'], array(
					...
					foreach ( $va_collection_ancestor_list as $vn_id => $va_collection_ancestor ) {
						array_unshift( $va_ancestors,
							'ca_collections-' . $va_collection_ancestor['NODE']['collection_id'] );
					}
