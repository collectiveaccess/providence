...
class RelatedListController extends BaseSearchController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		$this->opo_result_context = new ResultContext( $this->getRequest(), $this->ops_tablename,
			'related_list_bundle' );
	}
	# -------------------------------------------------------
	...
	public function Index( $pa_options = null ) {
		...
		if ( $placement_id = $this->getRequest()->getParameter( 'placement_id', pInteger ) ) {
			...
			$va_relation_ids = $t_instance->getRelatedItems( $placement->get( 'bundle_name' ),
				[ 'returnAs' => 'ids' ] );
		} else {
		...
		while ( $o_interstitial_res->nextHit() ) {
			$va_relation_id_typenames[ $o_interstitial_res->getPrimaryKey() ]
				= $o_interstitial_res->getWithTemplate( '^relationship_typename' );
		}
		...
		$va_res                         = caMakeSearchResult( $t_related->tableName(), $va_relation_ids,
			$va_search_opts );
		$o_res                          = $va_res['result'];
