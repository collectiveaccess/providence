...
class MediaImportController extends ActionController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		if ( ! $po_request->user->canDoAction( 'can_batch_import_media' ) ) {
			$po_response->setRedirect( $po_request->config->get( 'error_display_url' ) . '/n/3410?r='
	...
	/**
	 * Generates a form for specification of media import settings. The form is rendered into the current view,
	...
	public function Index( $pa_values = null, $pa_options = null ) {
		...
		$this->view->setVar( 'batch_mediaimport_last_settings', $va_last_settings = is_array(
			$va_last_settings = $this->request->user->getVar( 'batch_mediaimport_last_settings' ) ) ? $va_last_settings
			: array() );
		...
		$this->view->setVar( 'import_target', caHTMLSelect( 'import_target', $this->opa_importable_tables, array(
			...
			'onchange' => 'window.location.replace("' . caNavUrl( $this->getRequest(),
					$this->getRequest()->getModulePath(), $this->getRequest()->getController(),
					$this->getRequest()->getAction() )
			              . '/target/" + jQuery("#caImportTargetSelect").val()); return false;'
		...
		$this->view->setVar( $vs_import_target . '_type_list',
			$t_instance->getTypeListAsHTMLFormElement( $vs_import_target . '_type_id', null,
				array( 'value' => $va_last_settings[ $vs_import_target . '_type_id' ] ) ) );
		$this->view->setVar( $vs_import_target . '_limit_to_types_list',
			$t_instance->getTypeListAsHTMLFormElement( $vs_import_target . '_limit_matching_to_type_ids[]',
				array( 'multiple' => 1 ), array(
					'height' => '100px',
					'values' => $va_last_settings[ $vs_import_target . '_limit_matching_to_type_ids' ]
				) ) );
		$this->view->setVar( 'ca_object_representations_type_list',
			$t_rep->getTypeListAsHTMLFormElement( 'ca_object_representations_type_id', null,
				array( 'value' => $va_last_settings['ca_object_representations_type_id'] ) ) );
		...
		if ( $vs_import_target != 'ca_objects' ) { // non-object representations have relationship types
			...
			$this->getView()->setVar( $vs_import_target . '_representation_relationship_type',
				$t_rel->getRelationshipTypesAsHTMLSelect( 'ltor', null, null,
					array( 'name' => $vs_import_target . '_representation_relationship_type' ), array(
						'value' => $va_last_settings[ $vs_import_target . '_representation_relationship_type' ]
					) ) );
		}
		if ( (bool) $this->request->config->get( 'allow_user_selection_of_embedded_metadata_extraction_mapping' ) ) {
			$add_null_opt
				                        = (bool) $this->request->config->get( 'allow_user_embedded_metadata_extraction_mapping_null_option' );
			$va_object_importer_options = ca_data_importers::getImportersAsHTMLOptions( [
				'formats'    => [
					'exif',
					'mediainfo'
				],
				'tables'     => [ $t_instance->tableName() ],
				'nullOption' => $add_null_opt ? '-' : null
			] );
			$va_object_representation_importer_options
				                        = ca_data_importers::getImportersAsHTMLOptions( [
				'formats'    => [
					'exif',
					'mediainfo'
				],
				'tables'     => [ 'ca_object_representations' ],
				'nullOption' => $add_null_opt ? '-' : null
			] );

			$this->view->setVar( $vs_import_target . '_mapping_list',
				caHTMLSelect( $vs_import_target . '_mapping_id', $va_object_importer_options, array(),
					array( 'value' => $va_last_settings[ $vs_import_target . '_mapping_id' ] ) ) );
			...
			$this->view->setVar( 'ca_object_representations_mapping_list',
				caHTMLSelect( 'ca_object_representations_mapping_id', $va_object_representation_importer_options,
					array(), array( 'value' => $va_last_settings['ca_object_representations_mapping_id'] ) ) );
		...
		//
		$t_set                 = new ca_sets();
		$va_available_set_list = caExtractValuesByUserLocale( $t_set->getSets( array(
			'table'      => $vs_import_target,
	...
	public function Save( $pa_options = null ) {
		...
		if ( preg_match( "!/\.\.!", $vs_directory ) || preg_match( "!\.\./!", $vs_directory ) ) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/3250?r='
		...
		$va_options = array(
			...
			'importFromDirectory'                             => $vs_batch_media_import_root_directory . '/'
			                                                     . $vs_directory,
			'includeSubDirectories'                           => (bool) $this->request->getParameter( 'include_subdirectories',
				pInteger ),
			'deleteMediaOnImport'                             => (bool) $this->request->getParameter( 'delete_media_on_import',
				pInteger ),
			...
			$vs_import_target . '_limit_matching_to_type_ids' => $this->request->getParameter( $vs_import_target
			                                                                                   . '_limit_matching_to_type_ids',
				pArray ),
			$vs_import_target . '_type_id'                    => $this->request->getParameter( $vs_import_target
			                                                                                   . '_type_id', pInteger ),
			'ca_object_representations_type_id'               => $this->request->getParameter( 'ca_object_representations_type_id',
				pInteger ),
			$vs_import_target . '_status'                     => $this->request->getParameter( $vs_import_target
			                                                                                   . '_status', pInteger ),
			'ca_object_representations_status'                => $this->request->getParameter( 'ca_object_representations_status',
				pInteger ),
			$vs_import_target . '_access'                     => $this->request->getParameter( $vs_import_target
			                                                                                   . '_access', pInteger ),
			'ca_object_representations_access'                => $this->request->getParameter( 'ca_object_representations_access',
				pInteger ),
			$vs_import_target . '_mapping_id'                 => $this->request->getParameter( $vs_import_target
			                                                                                   . '_mapping_id',
				pInteger ),
			'ca_object_representations_mapping_id'            => $this->request->getParameter( 'ca_object_representations_mapping_id',
				pInteger ),
			'setMode'                                         => $this->request->getParameter( 'set_mode', pString ),
			'setCreateName'                                   => $this->request->getParameter( 'set_create_name',
				pString ),
			...
			'representationIdnoMode'                          => $this->request->getParameter( 'representation_idno_mode',
				pString ),
			'representation_idno'                             => $this->request->getParameter( 'idno_representation_number',
				pString ),
			'logLevel'                                        => $this->request->getParameter( 'log_level', pString ),
			'allowDuplicateMedia'                             => $this->request->getParameter( 'allow_duplicate_media',
				pInteger ),
			'locale_id'                                       => $g_ui_locale_id,
			'user_id'                                         => $this->request->getUserID(),
			'skipFileList'                                    => $this->request->getParameter( 'skip_file_list',
				pString ),
			'importTarget'                                    => $vs_import_target
		);

		if ( $vn_rel_type = $this->request->getParameter( $vs_import_target . '_representation_relationship_type',
			pInteger )
		) {
		...
		if ( is_array(
			$va_create_relationships_for = $this->request->getParameter( 'create_relationship_for', pArray ) )
		) {
			$va_options['create_relationship_for'] = $va_create_relationships_for;
			foreach ( $va_create_relationships_for as $vs_rel_table ) {
				$va_options[ 'relationship_type_id_for_' . $vs_rel_table ]
					= $this->request->getParameter( 'relationship_type_id_for_' . $vs_rel_table, pString );
			}
		}

		$va_last_settings                        = $va_options;
		$va_last_settings['importFromDirectory'] = preg_replace( "!{$vs_batch_media_import_root_directory}[/]*!", "",
			$va_last_settings['importFromDirectory'] );
		...
		if ( (bool) $this->request->config->get( 'queue_enabled' )
			...
			$vs_row_key
				= $vs_entity_key = join( "/",
				array( $this->request->getUserID(), $va_options['importFromDirectory'], time(), rand( 1, 999999 ) ) );
			if ( ! $o_tq->addTask(
				'mediaImport',
				$va_options,
				array(
	 ...
	 * @param bool   $pb_include_hidden_files Optional. By default caGetDirectoryContentsAsList() does not consider
	 *                                        hidden files (files starting with a '.') when calculating file counts.
	 *                                        Set this to true to include hidden files in counts. Note that the special
	 *                                        UNIX '.' and '..' directory entries are *never* counted as files.
	 * @param int    $pn_max_length_of_name   Maximum length in characters of returned file names. Note that the full
	 *                                        name is always returned in the 'fullname' value. Only 'name' is
	 *                                        truncated.
	 *
	 * @return array An array of file names.
	 */
	private function _getDirectoryListing(
		$dir, $pb_include_hidden_files = false, $pn_max_length_of_name = 25, $pn_start_at = 0,
		$pn_max_items_to_return = 25
	) {
		if ( ! is_dir( $dir ) ) {
			return array();
		}
	...
	public function GetDirectoryLevel() {
		...
		if ( $this->request->getParameter( 'init', pInteger ) ) {
			...
			foreach ( $va_tmp as $vs_tmp ) {
				...
				$va_level_data["{$vs_k}|{$vn_i}"]
					                                             =
				$va_file_list = $this->_getDirectoryListing( $vs_root_directory . '/' . $vs_directory, false, 20,
					(int) $vn_start, (int) $pn_max );
				$va_level_data["{$vs_k}|{$vn_i}"]['_primaryKey'] = 'name';

				$va_counts                                      = caGetDirectoryContentsCount( $vs_root_directory . '/'
				                                                                               . $vs_directory, false,
					false );
			...
			if ( ! $ps_directory ) {
				...
				$va_level_data["{$vs_k}|{$vn_level}"]
					                                                 =
				$va_file_list = $this->_getDirectoryListing( $vs_root_directory . '/' . $ps_directory, false, 20,
					(int) $pn_start, (int) $pn_max );
				$va_level_data["{$vs_k}|{$vn_level}"]['_primaryKey'] = 'name';
	...
	public function UploadFiles() {
		...
		if ( ! is_writeable( $vs_batch_media_import_root_directory . $ps_directory ) ) {
			...
			foreach ( $_FILES as $vs_param => $va_file ) {
				foreach ( $va_file['name'] as $vn_i => $vs_name ) {
					...
					if ( copy( $va_file['tmp_name'][ $vn_i ],
						$vs_batch_media_import_root_directory . $ps_directory . "/" . $vs_name )
					) {
		...
		$va_response['uploadMessage'] = ( ( $vn_upload_count = sizeof( $va_response['copied'] ) ) == 1 )
			? _t( 'Uploaded %1 file', $vn_upload_count ) : _t( 'Uploaded %1 files', $vn_upload_count );
		if ( is_array( $va_response['skipped'] ) && ( $vn_skip_count = sizeof( $va_response['skipped'] ) )
		     && ! $va_response['error']
		) {
			$va_response['skippedMessage'] = ( $vn_skip_count == 1 ) ? _t( 'Skipped %1 file', $vn_skip_count )
				: _t( 'Skipped %1 files', $vn_skip_count );
		}
	...
	public function DownloadLog() {
		...
		if ( file_exists( $path = "{$tmp_dir}/{$file}.csv" ) ) {
			...
			$o_view->setVar( 'archive_name',
				( strpos( $path, "SkipLog" ) !== false ) ? "skipped_files_log.csv" : "error_log.csv" );
