...
class EditorController extends ActionController {
	...
	public function Edit( $pa_values = null, $pa_options = null ) {
		...
		if ( ! $vn_set_id ) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/3200?r='
		...
		if ( ! $this->request->user->canDoAction( 'can_batch_edit_'
		                                          . Datamodel::getTableName( $t_set->get( 'table_num' ) ) )
		) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/3210?r='
			                              . urlencode( $this->request->getFullUrlPath() ) );

			return;
		}
		if ( $t_set->getItemCount( array( 'user_id' => $this->request->getUserID() ) ) <= 0 ) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/3220?r='
			                              . urlencode( $this->request->getFullUrlPath() ) );

			return;
		}

		$this->view->setVar( 'batch_editor_last_settings', $va_last_settings = is_array(
			$va_last_settings = $this->request->user->getVar( 'batch_editor_last_settings' ) ) ? $va_last_settings
			: array() );

		$va_nav = $t_ui->getScreensAsNavConfigFragment( $this->request, null, $this->request->getModulePath(),
	...
	public function Save( $pa_options = null ) {
		...
		if ( (bool) $this->request->config->get( 'queue_enabled' )
		     && (bool) $this->request->getParameter( 'run_in_background', pInteger )
		) { // queue for background processing
			$o_tq = new TaskQueue();

			$vs_row_key
				= $vs_entity_key = join( "/",
				array( $this->request->getUserID(), $t_set->getPrimaryKey(), time(), rand( 1, 999999 ) ) );
			if ( ! $o_tq->addTask(
				'batchEditor',
				$va_last_settings,
				array(
			...
			$app->registerPlugin( new BatchEditorProgress( $this->request, $t_set, $t_subject, array(
				'sendMail'        => (bool) $this->request->getParameter( 'send_email_when_done', pInteger ),
				'sendSMS'         => (bool) $this->request->getParameter( 'send_sms_when_done', pInteger ),
	...
	public function Delete( $pa_options = null ) {
		...
		if ( ! $this->request->user->canDoAction( 'can_batch_delete_'
		                                          . Datamodel::getTableName( $t_set->get( 'table_num' ) ) )
		) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/3230?r='
			                              . urlencode( $this->request->getFullUrlPath() ) );
	...
	public function ChangeType( $pa_options = null ) {
		...
		if ( ! $this->request->user->canDoAction( "can_change_type_" . $t_subject->tableName() )
		     || ! method_exists( $t_subject, "getTypeList" )
		) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/3260?r='
			                              . urlencode( $this->request->getFullUrlPath() ) );
		...
		if ( (bool) $this->request->config->get( 'queue_enabled' )
			...
			if ( ! $o_tq->addTask(
				'batchEditor',
				array_merge( $va_last_settings,
					array( 'isBatchTypeChange' => true, 'new_type_id' => $vn_new_type_id ) ),
				array(
					"priority"   => 100,
					"entity_key" => $vs_entity_key,
					"row_key"    => $vs_row_key,
	...
	protected function _initView( $pa_options = null ) {
		...
		$vs_item_name  = ( $vn_item_count == 1 ) ? $t_subject->getProperty( "NAME_SINGULAR" )
			: $t_subject->getProperty( "NAME_PLURAL" );

		MetaTagManager::setWindowTitle( _t( "Batch editing %1 %2 with set %3", $vn_item_count, $vs_item_name,
			$t_set->getLabelForDisplay( true ) ) );
	...
	public function _genDynamicNav( $pa_params, $pa_options = null ) {
		...
		$va_nav = $t_ui->getScreensAsNavConfigFragment( $this->request, $vn_type_id, $pa_params['default']['module'],
			...
			false,
			[
				'hideIfNoAccess'         => isset( $pa_params['hideIfNoAccess'] ) ? $pa_params['hideIfNoAccess']
					: false,
				'returnTypeRestrictions' => true,
				'restrictToTypes'        => array_keys( $t_set->getTypesForItems() )
			]
		);
