...
class EditorController extends ActionController
{
    ...
    public function Edit($pa_values = null, $pa_options = null)
    {
        ...
        if (!$vn_set_id) {
            $this->response->setRedirect(
                $this->request->config->get('error_display_url') . '/n/3200?r=' . urlencode(
        ...
        if (!$this->request->user->canDoAction('can_batch_edit_' . Datamodel::getTableName($t_set->get('table_num')))) {
            $this->response->setRedirect(
                $this->request->config->get('error_display_url') . '/n/3210?r=' . urlencode(
                    $this->request->getFullUrlPath()
                )
            );
            return;
        ...
        if ($t_set->getItemCount(array('user_id' => $this->request->getUserID())) <= 0) {
            $this->response->setRedirect(
                $this->request->config->get('error_display_url') . '/n/3220?r=' . urlencode(
                    $this->request->getFullUrlPath()
                )
            );
            return;
        ...
        $this->view->setVar(
            'batch_editor_last_settings',
            $va_last_settings = is_array(
                $va_last_settings = $this->request->user->getVar('batch_editor_last_settings')
    ...
    public function Save($pa_options = null)
    {
        ...
        if ((bool)$this->request->config->get('queue_enabled') && (bool)$this->request->getParameter(
                'run_in_background',
                pInteger
            )) { // queue for background processing
            $o_tq = new TaskQueue();

            $vs_row_key = $vs_entity_key = join(
                "/",
                array(
    ...
    public function Delete($pa_options = null)
    {
        ...
        if (!$this->request->user->canDoAction(
            'can_batch_delete_' . Datamodel::getTableName($t_set->get('table_num'))
        )) {
            $this->response->setRedirect(
                $this->request->config->get('error_display_url') . '/n/3230?r=' . urlencode(
                    $this->request->getFullUrlPath()
                )
            );
            return;
        ...
        if ($vb_confirm = ($this->request->getParameter('confirm', pInteger) == 1) ? true : false) {
            ...
            $app = AppController::getInstance();
            $app->registerPlugin(
                new BatchEditorProgress($this->request, $t_set, $t_subject, array('isBatchDelete' => true))
            );
        }
    ...
    public function ChangeType($pa_options = null)
    {
        ...
        if (!$this->request->user->canDoAction("can_change_type_" . $t_subject->tableName()) || !method_exists(
                $t_subject,
                "getTypeList"
            )) {
            $this->response->setRedirect(
                $this->request->config->get('error_display_url') . '/n/3260?r=' . urlencode(
                    $this->request->getFullUrlPath()
                )
            );
            return;
    ...
    protected function _initView($pa_options = null)
    {
        ...
        $vs_item_name = ($vn_item_count == 1) ? $t_subject->getProperty("NAME_SINGULAR") : $t_subject->getProperty(
            "NAME_PLURAL"
        );

        MetaTagManager::setWindowTitle(
            _t("Batch editing %1 %2 with set %3", $vn_item_count, $vs_item_name, $t_set->getLabelForDisplay(true))
        );
