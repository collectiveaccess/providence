...
class MetadataImportController extends ActionController
{
    ...
    public function __construct(&$po_request, &$po_response, $pa_view_paths = null)
    {
        ...
        if (!$po_request->user->canDoAction('can_batch_import_metadata')) {
            $po_response->setRedirect(
                $po_request->config->get('error_display_url') . '/n/3400?r=' . urlencode($po_request->getFullUrlPath())
            );
            return;
        }
    ...
    public function UploadImporters()
    {
        ...
        foreach ($_FILES as $vs_param => $va_file) {
            foreach ($va_file['name'] as $vn_i => $vs_name) {
                if ($t_importer = ca_data_importers::loadImporterFromFile(
                    ...
                    $va_errors,
                    array(
                        'logDirectory' => $this->request->config->get(
        ...
        $va_response['uploadMessage'] = (($vn_upload_count = (is_array($va_response['copied']) && sizeof(
                    $va_response['copied']
                )) == 1)) ? _t('Uploaded %1 worksheet', $vn_upload_count) : _t(
            'Uploaded %1 worksheets',
            $vn_upload_count
        );
        if (is_array($va_response['skipped']) && ($vn_skip_count = sizeof(
                $va_response['skipped']
            )) && !$va_response['error']) {
            $va_response['skippedMessage'] = ($vn_skip_count == 1) ? _t('Skipped %1 worksheet', $vn_skip_count) : _t(
                'Skipped %1 worksheet',
                $vn_skip_count
            );
        }
    ...
    public function Load()
    {
        ...
        $is_new = true;
        try {
            $t_importer = ca_data_importers::loadImporterFromFile(
                $tmp_file,
                $errors,
                [
                    'logDirectory' => $this->request->config->get(
        ...
        if ($t_importer) {
            $this->notification->addNotification(
                $is_new ? _t("Added import worksheet %1", $t_importer->get('importer_code')) : _t(
                    "Updated import worksheet %1",
            ...
            $this->notification->addNotification(
                _t("Could not add import worksheet: %1", join("; ", $errors)),
    ...
    public function Download()
    {
        ...
        if (($vn_importer_id = $this->request->getParameter("importer_id", pInteger)) && $t_importer->load(
                $vn_importer_id
            ...
            $o_view->setVar(
                'archive_name',
                ($vs_importer_code = $t_importer->get(
                    'importer_code'
                )) ? "{$vs_importer_code}.xlsx" : "Importer_{$vn_importer_id}.xlsx"
            );
