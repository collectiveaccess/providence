...
class MetadataImportController extends ActionController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		if ( ! $po_request->user->canDoAction( 'can_batch_import_metadata' ) ) {
			$po_response->setRedirect( $po_request->config->get( 'error_display_url' ) . '/n/3400?r='
	...
	public function UploadImporters() {
		...
		foreach ( $_FILES as $vs_param => $va_file ) {
			foreach ( $va_file['name'] as $vn_i => $vs_name ) {
				if ( $t_importer = ca_data_importers::loadImporterFromFile( $va_file['tmp_name'][ $vn_i ], $va_errors,
					array(
						'logDirectory'     => $this->request->config->get( 'batch_metadata_import_log_directory' ),
		...
		$va_response['uploadMessage'] = ( ( $vn_upload_count = ( is_array( $va_response['copied'] )
		                                                         && sizeof( $va_response['copied'] ) ) == 1 ) )
	...
	public function Run() {
		...
		$this->view->setVar( 'last_settings',
			$va_last_settings = $this->request->user->getVar( 'batch_metadata_last_settings' ) );
	...
	public function ImportData() {
		...
		if ( $va_last_settings['fileInput'] === 'googledrive' ) {
			if ( $google_url = caValidateGoogleSheetsUrl(
				$google_url_orig = $this->request->getParameter( 'google_drive_url', pString ) )
			) {
				try {
	...
	public function Load() {
		...
		try {
			...
			if ( ! is_array( $parsed_data ) || ! isset( $parsed_data['file'] )
			     || ! ( $tmp_file
					= $parsed_data['file'] )
			) {
		...
		$is_new = true;
		try {
			$t_importer = ca_data_importers::loadImporterFromFile( $tmp_file, $errors, [
				'logDirectory' => $this->request->config->get( 'batch_metadata_import_log_directory' ),
				'logLevel'     => KLogger::INFO,
		...
		if ( $t_importer ) {
			$this->notification->addNotification( $is_new ? _t( "Added import worksheet %1",
				$t_importer->get( 'importer_code' ) )
				: _t( "Updated import worksheet %1", $t_importer->get( 'importer_code' ) ),
				__NOTIFICATION_TYPE_INFO__ );
		} else {
			$this->notification->addNotification( _t( "Could not add import worksheet: %1", join( "; ", $errors ) ),
	...
	public function Download() {
		...
		if ( ( $vn_importer_id = $this->request->getParameter( "importer_id", pInteger ) )
		     && $t_importer->load( $vn_importer_id )
		     && $t_importer->getFileInfo( 'worksheet' )
		) {
			$o_view = new View( $this->request, $this->request->getViewsDirectoryPath() . '/bundles/' );
			$o_view->setVar( 'archive_path', $t_importer->getFilePath( 'worksheet' ) );
			$o_view->setVar( 'archive_name',
				( $vs_importer_code = $t_importer->get( 'importer_code' ) ) ? "{$vs_importer_code}.xlsx"
					: "Importer_{$vn_importer_id}.xlsx" );
