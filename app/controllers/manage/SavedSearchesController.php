...
class SavedSearchesController extends ActionController {
	...
	public function ListSearches() {
		...
		foreach ( $va_tables as $vs_table ) {
			if ( sizeof( $va_saved_search_list = $this->request->user->getSavedSearches( $vs_table, "basic_search" ) )
			     > 0
			) {
			...
			if ( sizeof(
				     $va_saved_search_list = $this->request->user->getSavedSearches( $vs_table, "advanced_search" ) )
			     > 0
			) {
	...
	public function Delete() {
		...
		if ( is_array( $pa_saved_search_ids ) && ( sizeof( $pa_saved_search_ids ) > 0 ) ) {
			foreach ( $pa_saved_search_ids as $vn_search_id ) {
				...
				if ( ! $this->request->user->removeSavedSearch( $va_search_pieces[0], $va_search_pieces[1],
					$va_search_pieces[2] )
				) {
			...
			if ( sizeof( $va_errors ) > 0 ) {
				...
				$this->notification->addNotification( _t( "Your searches have been deleted" ),
					__NOTIFICATION_TYPE_INFO__ );
			}
		} else {
			$this->notification->addNotification( _t( "Please use the checkboxes to select searches for deletion" ),
