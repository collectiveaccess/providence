...
class TagsController extends BaseSearchController {
	...
	public function Approve() {
		...
		if ( is_array( $pa_tag_relation_ids ) && ( sizeof( $pa_tag_relation_ids ) > 0 ) ) {
			...
			if ( sizeof( $va_errors ) > 0 ) {
				...
				$this->notification->addNotification( _t( "Your tags have been approved" ),
					__NOTIFICATION_TYPE_INFO__ );
			}
		} else {
			$this->notification->addNotification( _t( "Please use the checkboxes to select tags for approval" ),
	...
	public function Delete() {
		...
		if ( is_array( $pa_tag_relation_ids ) && ( sizeof( $pa_tag_relation_ids ) > 0 ) ) {
			...
			$this->notification->addNotification( _t( "Please use the checkboxes to select tags for deletion" ),
				__NOTIFICATION_TYPE_WARNING__ );
		}
