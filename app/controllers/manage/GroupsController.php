	...
	public function Save() {
		...
		if ( $this->request->getParameter( 'password', pString ) != $this->request->getParameter( 'password_confirm',
			...
			$this->request->addActionError( new ApplicationError( 1050,
				_t( "Password does not match confirmation. Please try again." ), "manage/GroupsController->Save()", '',
				false, false ), 'field_password' );
		}
		...
		if ( $this->request->numActionErrors() == 0 ) {
			...
			if ( $t_group->numErrors() ) {
				...
				$va_user_list            = $this->request->user->getUserList( array( 'sort'           => 'lname',
				                                                                     'sort_direction' => 'asc',
				                                                                     'userclass'      => array( 0, 1 )
				) );    //userclass 0 ="back-end" users; 1=front-end users
		...
		if ( $this->request->numActionErrors() ) {
			...
			// success
			$this->view->setVar( 'group_list',
				$t_group->getGroupList( 'name', 'asc', $this->request->user->getUserID() ) );
