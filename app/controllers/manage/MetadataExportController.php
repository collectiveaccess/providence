...
class MetadataExportController extends ActionController {
	...
	# -------------------------------------------------------
	/**
	 * Download export generated by ExportData action
	 */
			...
			if(file_exists(__CA_APP_DIR__.'/tmp/'.$ps_file)){
				if($va_matches[1]){
					...
					if($t_exporter->getPrimaryKey()){
						...
						if($t_subject->getAppConfig()->get('exporter_show_destination_screen') && !$this->request->getParameter('exportDestinationsSet', pInteger)) {
							$this->render('export/export_destination_html.php');
							return;
						}
					}
