...
class MetadataExportController extends ActionController {
	...
	public function UploadExporters() {
		...
		foreach ( $_FILES as $va_file ) {
			foreach ( $va_file['name'] as $vn_i => $vs_name ) {
				if ( $t_importer = ca_data_exporters::loadExporterFromFile( $va_file['tmp_name'][ $vn_i ],
					$va_errors )
				) {
		...
		$va_response['uploadMessage'] = ( ( $vn_upload_count = sizeof( $va_response['copied'] ) ) == 1 )
			? _t( 'Uploaded %1 worksheet', $vn_upload_count ) : _t( 'Uploaded %1 worksheets', $vn_upload_count );
		if ( is_array( $va_response['skipped'] ) && ( $vn_skip_count = sizeof( $va_response['skipped'] ) )
		     && ! $va_response['error']
		) {
			$va_response['skippedMessage'] = ( $vn_skip_count == 1 ) ? _t( 'Skipped %1 worksheet', $vn_skip_count )
				: _t( 'Skipped %1 worksheet', $vn_skip_count );
		}
	...
	public function ExportData() {
		...
		if ( ! $this->getRequest()->user->canDoAction( 'can_batch_export_metadata' ) ) {
			$this->getResponse()->setRedirect( $this->getRequest()->config->get( 'error_display_url' ) . '/n/3440?r='
		...
		if ( ! $this->getRequest()->user->canDoAction( 'can_export_' . $t_subject->tableName() ) ) {
			$this->getResponse()->setRedirect( $this->getRequest()->config->get( 'error_display_url' ) . '/n/3430?r='
			                                   . urlencode( $this->getRequest()->getFullUrlPath() ) );
	...
	public function ExportSingleData() {
		...
		if ( ! $t_exporter->getPrimaryKey() ) {
			$this->getResponse()->setRedirect( $this->getRequest()->config->get( 'error_display_url' ) . '/n/3420?r='
			                                   . urlencode( $this->getRequest()->getFullUrlPath() ) );
		...
		if ( is_array( $va_errors ) && ( sizeof( $va_errors ) > 0 ) ) {
			...
			if ( ! $vs_filename
			     && ( $vs_export_filename_template = $o_config->get( $t_subject->tableName()
			                                                         . "_single_item_export_filename" ) )
			) {
				if ( $vs_filename = caProcessTemplateForIDs( $vs_export_filename_template, $t_subject->tableNum(),
					array( $vn_id ) )
				) {
			...
			if ( ! caCanRead( $this->getRequest()->getUserID(), $t_exporter->get( 'table_num' ), $vn_id ) ) {
				$this->getResponse()->setRedirect( $this->getRequest()->config->get( 'error_display_url' )
				                                   . '/n/2320?r=' . urlencode( $this->getRequest()
				                                                                    ->getFullUrlPath() ) );
			...
			$vs_export = ca_data_exporters::exportRecord( $t_exporter->get( 'exporter_code' ), $vn_id,
				array( 'singleRecord' => true ) );
	...
	public function ExternalExportSingle() {
		...
		$this->view->setVar( 'export_file',
			$f = array_shift( $t_exporter->process( $target_info['table'], $id, null, [ 'target' => $target ] ) ) );
	...
	public function ProcessDestination() {
		...
		if ( ! ( $vs_tmp_file = Session::getVar( 'export_file' ) )
		     && ! ( $vs_tmp_data
				= Session::getVar( 'export_data' ) )
		) {
		...
		if ( is_array( $va_alt_dest ) && sizeof( $va_alt_dest ) > 0 ) {
			if ( is_array( $va_alt_dest[ $vs_dest_code ] ) ) {
				...
				if ( isset( $va_dest['type'] ) && ( $va_dest['type'] == 'ResourceSpace' ) ) {
					...
					$vb_success = caExportDataToResourceSpace( $va_dest['user'], $va_dest['api_key'],
						$va_dest['base_api_url'], $vs_tmp_file );
				}
