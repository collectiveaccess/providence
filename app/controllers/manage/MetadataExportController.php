...
class MetadataExportController extends ActionController
{
    ...
    public function UploadExporters()
    {
        ...
        $va_response['uploadMessage'] = (($vn_upload_count = sizeof($va_response['copied'])) == 1) ? _t(
            'Uploaded %1 worksheet',
            $vn_upload_count
        ) : _t('Uploaded %1 worksheets', $vn_upload_count);
    ...
    public function ExportData()
    {
        ...
        if (!$this->getRequest()->user->canDoAction('can_batch_export_metadata')) {
            $this->getResponse()->setRedirect(
                $this->getRequest()->config->get('error_display_url') . '/n/3440?r=' . urlencode(
        ...
        if (!$this->getRequest()->user->canDoAction('can_export_' . $t_subject->tableName())) {
            $this->getResponse()->setRedirect(
                $this->getRequest()->config->get('error_display_url') . '/n/3430?r=' . urlencode(
                    $this->getRequest()->getFullUrlPath()
                )
            );
            return;
    ...
    public function ExportSingleData()
    {
        ...
        if (!$t_exporter->getPrimaryKey()) {
            $this->getResponse()->setRedirect(
                $this->getRequest()->config->get('error_display_url') . '/n/3420?r=' . urlencode(
                    $this->getRequest()->getFullUrlPath()
                )
            );
            return;
        ...
        if (is_array($va_errors) && (sizeof($va_errors) > 0)) {
            ...
            if (!$vs_filename && ($vs_export_filename_template = $o_config->get(
                    $t_subject->tableName() . "_single_item_export_filename"
                ))) {
                if ($vs_filename = caProcessTemplateForIDs(
                    $vs_export_filename_template,
                    $t_subject->tableNum(),
                    array($vn_id)
                )) {
            ...
            $vs_export = ca_data_exporters::exportRecord(
    ...
    public function ExternalExportSingle()
    {
        ...
        $this->view->setVar(
            'export_file',
            $f = array_shift(
                $t_exporter->process($target_info['table'], $id, null, ['target' => $target])
            )
        );
        ...
        if (is_array($va_alt_dest) && sizeof($va_alt_dest) > 0) {
            if (is_array($va_alt_dest[$vs_dest_code])) {
                ...
                if (isset($va_dest['type']) && ($va_dest['type'] == 'ResourceSpace')) {
                    ...
                    $vb_success = caExportDataToResourceSpace(
                        $va_dest['user'],
                        $va_dest['api_key'],
                        $va_dest['base_api_url'],
                        $vs_tmp_file
                    );
                }
            }
