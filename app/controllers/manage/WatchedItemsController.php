...
class WatchedItemsController extends ActionController
{
        ...
        if (is_array($pa_watch_ids) && (sizeof($pa_watch_ids) > 0)) {
            ...
            if (sizeof($va_errors) > 0) {
                ...
                $this->notification->addNotification(
                    _t("Your watched items have been deleted"),
                    __NOTIFICATION_TYPE_INFO__
                );
            }
        } else {
            $this->notification->addNotification(
                _t("Please use the checkboxes to select items to remove from your watch list"),
    ...
    public function CreateSet()
    {
        ...
        if ($t_set->numErrors()) {
            $this->opo_notification_manager->addNotification(
        ...
        $this->opo_notification_manager->addNotification(
            _t("Added set '%1' with %2 items", $ps_set_name, $vn_added_items_count),
            __NOTIFICATION_TYPE_INFO__
        );
        $this->ListItems();
    }
