...
class UserSortController extends ActionController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		if ( ! $po_request->isLoggedIn() || ! $po_request->user->canDoAction( 'can_manage_user_sorts' ) ) {
			$this->getResponse()->setRedirect( $po_request->config->get( 'error_display_url' ) . '/n/3500?r='
			                                   . urlencode( $po_request->getFullUrlPath() ) );
	...
	public function ListSorts() {
		...
		if ( sizeof( $va_list ) == 0 ) {
			$this->opo_notification_manager->addNotification( _t( "There are no user sorts" ),
	...
	public function Delete() {
		...
		if ( is_array( $pa_sort_ids ) && ( sizeof( $pa_sort_ids ) > 0 ) ) {
			...
			if ( sizeof( $va_errors ) > 0 ) {
				...
				$this->notification->addNotification( _t( "Your sorts have been deleted" ),
					__NOTIFICATION_TYPE_INFO__ );
			}
		} else {
			$this->notification->addNotification( _t( "Please use the checkboxes to select sorts to remove" ),
