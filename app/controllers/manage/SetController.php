...
class SetController extends ActionController
{
    ...
    public function ListSets()
    {
        ...
        $vn_user_id = !(bool)$this->request->config->get('ca_sets_all_users_see_all_sets') ? $this->request->getUserID(
        ) : null;
        ...
        if ($this->request->user->canDoAction('is_administrator') || $this->request->user->canDoAction(
            ...
            switch ($pn_mode) {
                case 0:
                default:
                    $va_set_list = caExtractValuesByUserLocale(
                        $t_set->getSets(
                            array(
                                'user_id' => $vn_user_id,
        ...
        if ($vb_confirm = ($this->request->getParameter('confirm', pInteger) == 1) ? true : false) {
            ...
            if (!is_array($pa_set_ids = $this->request->getParameter('row_id', pArray))) {
                $this->notification->addNotification(
                    _t("At least two sets must be selected"),
            ...
            $pa_set_ids = array_filter(
                $pa_set_ids,
                function ($vn_set_id) use ($t_set, $vn_user_id) {
            ...
            if (sizeof($pa_set_ids) > 0) {
                ...
                foreach ($pa_set_ids as $set_id) {
                    if ($t_set->load($set_id)) {
                        if (!$t_set->delete(true)) {
                            if ($delete_count > 0) {
                                $this->notification->addNotification(
                                    ($delete_count === 1) ? _t("Deleted %1 set", $delete_count) : _t(
                                        "Deleted %1 sets",
                                        $delete_count
                                    ),
                ...
                $this->notification->addNotification(
                    ($delete_count === 1) ? _t("Deleted %1 set", $delete_count) : _t("Deleted %1 sets", $delete_count),
                    __NOTIFICATION_TYPE_INFO__
                );
            } else {
                $this->notification->addNotification(
                    _t("You do not have access to the selected sets"),
                    __NOTIFICATION_TYPE_ERROR__
                );
            }
        }
                    ...
                    switch ($ps_op) {
                        ...
                        case 'INTERSECTION':
                            foreach ($pa_set_ids as $i => $vn_set_id) {
                                ...
                                $va_items = array_keys(
                                    $t_set->getItems(['returnRowIdsOnly' => true, 'user_id' => $vn_user_id])
                                );
                    ...
                    if (sizeof($va_set_list) > 0) {
                        ...
                        if ($t_set->numErrors() > 0) {
                            ...
                            if ($t_set->numErrors() > 0) {
                                $this->notification->addNotification(
                                    _t(
                                        "Could not add label to new set: %1",
                                        join("; ", $t_set->getErrors()),
                                        __NOTIFICATION_TYPE_ERROR__
                                    )
                                );
                                ...
                                $this->notification->addNotification(
                                    _t(
                                        "Could not add items to new set: %1",
                                        join("; ", $t_set->getErrors()),
                                        __NOTIFICATION_TYPE_ERROR__
                                    )
                                );
                                ...
                                $this->notification->addNotification(
                                    _t("Created new set <em>%1</em>", $set_name),
                                    __NOTIFICATION_TYPE_INFO__
                                );
                            }
                        }
                        ...
                        $this->notification->addNotification(
                            _t("Set was not created because it has no contents"),
    ...
    public function Info()
    {
        ...
        $va_set_stats = array(
            'mine' => caExtractValuesByUserLocale(
                $t_set->getSets(
                    array(
        ...
        if ($this->request->user->canDoAction('is_administrator') || $this->request->user->canDoAction(
            ...
            $va_set_stats['user'] = caExtractValuesByUserLocale(
                $t_set->getSets(
                    array('user_id' => $vn_user_id, 'allUsers' => true, 'setType' => $this->opn_list_set_type_id)
                ),
                null,
                null,
                array()
            );
            $va_set_stats['public'] = caExtractValuesByUserLocale(
                $t_set->getSets(
                    array('user_id' => $vn_user_id, 'publicUsers' => true, 'setType' => $this->opn_list_set_type_id)
                ),
                null,
                null,
                array()
