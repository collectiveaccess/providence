...
class SetController extends ActionController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		$this->opn_items_per_page = ( (int) $this->request->config->get( 'items_per_page_default_for_ca_sets_search' )
			?: 20 );
	...
	public function ListSets() {
		...
		$vn_user_id = ! (bool) $this->request->config->get( 'ca_sets_all_users_see_all_sets' )
		...
		if ( $this->request->user->canDoAction( 'is_administrator' )
			...
			switch ( $pn_mode ) {
				case 0:
				default:
					$va_set_list = caExtractValuesByUserLocale( $t_set->getSets( array(
						'user_id' => $vn_user_id,
						'access'  => __CA_SET_EDIT_ACCESS__,
						'setType' => $this->opn_list_set_type_id
					) ), null, null, array() );
					break;
				case 1:
					$va_set_list = caExtractValuesByUserLocale( $t_set->getSets( array(
		...
		if ( $va_set_list ) {
			foreach ( $va_set_list as $id => $va_set ) {
				...
				if ( ! in_array( $vs_sort, array( "status", "access" ) ) ) {
					...
					$va_set_list_sorted[ $t_set->getChoiceListValue( $vs_sort, $va_set[ $vs_sort ] ) . " " . $id ]
						= $va_set;
				}
				$va_set_ids[] = $id;
			}
	...
	public function Delete() {
		if ( $vb_confirm = ( $this->request->getParameter( 'confirm', pInteger ) == 1 ) ? true : false ) {
			...
			if ( ! is_array( $pa_set_ids = $this->request->getParameter( 'row_id', pArray ) ) ) {
				$this->notification->addNotification( _t( "At least two sets must be selected" ),
			...
			if ( sizeof( $pa_set_ids ) > 0 ) {
				...
				foreach ( $pa_set_ids as $set_id ) {
					if ( $t_set->load( $set_id ) ) {
						if ( ! $t_set->delete( true ) ) {
							if ( $delete_count > 0 ) {
								$this->notification->addNotification( ( $delete_count === 1 ) ? _t( "Deleted %1 set",
									$delete_count ) : _t( "Deleted %1 sets", $delete_count ),
									__NOTIFICATION_TYPE_INFO__ );
							}
							$this->notification->addNotification( _t( "Could not delete set %1: %2",
								$t_set->get( 'set_code' ), join( "; ", $t_set->getErrors() ) ),
								__NOTIFICATION_TYPE_ERROR__ );
							$this->ListSets();
				...
				$this->notification->addNotification( ( $delete_count === 1 ) ? _t( "Deleted %1 set", $delete_count )
					: _t( "Deleted %1 sets", $delete_count ), __NOTIFICATION_TYPE_INFO__ );
			} else {
				$this->notification->addNotification( _t( "You do not have access to the selected sets" ),
					__NOTIFICATION_TYPE_ERROR__ );
			}
		}
		$this->ListSets();
				...
				if ( sizeof( $pa_set_ids ) > 1 ) {
					...
					switch ( $ps_op ) {
						case 'UNION':
							foreach ( $pa_set_ids as $vn_set_id ) {
								...
								$va_set_list = array_merge( $va_set_list, array_keys( $t_set->getItems( [
									'returnRowIdsOnly' => true,
									'user_id'          => $vn_user_id
								] ) ) );
							}
						...
						case 'INTERSECTION':
							foreach ( $pa_set_ids as $i => $vn_set_id ) {
								...
								$va_items    = array_keys( $t_set->getItems( [
									'returnRowIdsOnly' => true,
									'user_id'          => $vn_user_id
								] ) );
						...
						case 'DIFFERENCE':
							...
							foreach ( $pa_set_ids as $i => $vn_set_id ) {
								...
								$va_set_list = array_merge( $va_set_list,
									$va_sets[ $i ] = array_keys( $t_set->getItems( [
										'returnRowIdsOnly' => true,
										'user_id'          => $vn_user_id
									] ) ) );
							}
							...
							break;
						default:
							$this->notification->addNotification( _t( "Invalid operation" ),
								__NOTIFICATION_TYPE_ERROR__ );
							break;
					}
					...
					if ( sizeof( $va_set_list ) > 0 ) {
						...
						if ( $t_set->numErrors() > 0 ) {
							$this->notification->addNotification( _t( "Could not create new set: %1",
								join( "; ", $t_set->getErrors() ), __NOTIFICATION_TYPE_ERROR__ ) );
						} else {
							$t_set->addLabel( [ 'name' => $set_name ], $g_ui_locale_id, null, true );
							if ( $t_set->numErrors() > 0 ) {
								$this->notification->addNotification( _t( "Could not add label to new set: %1",
									join( "; ", $t_set->getErrors() ), __NOTIFICATION_TYPE_ERROR__ ) );
							} elseif ( ! $t_set->addItems( $va_set_list ) ) {
								$this->notification->addNotification( _t( "Could not add items to new set: %1",
									join( "; ", $t_set->getErrors() ), __NOTIFICATION_TYPE_ERROR__ ) );
							} else {
								$this->notification->addNotification( _t( "Created new set <em>%1</em>", $set_name ),
									__NOTIFICATION_TYPE_INFO__ );
							}
						}
					} else {
						$this->notification->addNotification( _t( "Set was not created because it has no contents" ),
	...
	public function Info() {
		...
		$va_set_stats
			= array(
			'mine' => caExtractValuesByUserLocale( $t_set->getSets( array(
		...
		if ( $this->request->user->canDoAction( 'is_administrator' )
			...
			$va_set_stats['user']   = caExtractValuesByUserLocale( $t_set->getSets( array(
				'user_id'  => $vn_user_id,
				'allUsers' => true,
				'setType'  => $this->opn_list_set_type_id
			) ), null, null, array() );
			$va_set_stats['public'] = caExtractValuesByUserLocale( $t_set->getSets( array(
				'user_id'     => $vn_user_id,
				'publicUsers' => true,
				'setType'     => $this->opn_list_set_type_id
			) ), null, null, array() );
	...
	public function _genTypeNav( $pa_params ) {
		...
		if ( $t_subject->getAppConfig()->get( 'perform_type_access_checking' ) ) {
			$va_restrict_to_types = caGetTypeRestrictionsForUser( 'ca_sets',
		...
		foreach ( $pa_subtypes as $vn_i => $va_type ) {
			...
			if ( isset( $pa_types_by_parent_id[ $va_type['item_id'] ] )
				...
				$va_subsubtypes = $this->_getSubTypes( $pa_types_by_parent_id[ $va_type['item_id'] ],
					$pa_types_by_parent_id, $pn_sort_type, $pa_restrict_to_types );
			} else {
