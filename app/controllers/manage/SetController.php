                ...
                case 'UNION':
                    foreach ($pa_set_ids as $vn_set_id) {
                        ...
                        $va_set_list = array_merge(
                            ...
                            array_keys(
                                $t_set->getItems(
                                    ['returnRowIdsOnly' => true, 'user_id' => $vn_user_id]
                                )
                            )
                        );
                    }
            ...
            if (sizeof($va_set_list) > 0) {
                ...
                if ($t_set->numErrors() > 0) {
                    $this->notification->addNotification(
                        _t("Could not create new set: %1", join("; ", $t_set->getErrors()), __NOTIFICATION_TYPE_ERROR__)
                    );
                } else {
                    ...
                    if ($t_set->numErrors() > 0) {
                        ...
                        $this->notification->addNotification(
                            _t("Created new set <em>%1</em>", $ps_set_name),
