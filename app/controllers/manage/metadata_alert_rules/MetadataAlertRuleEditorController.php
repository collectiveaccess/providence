...
class MetadataAlertRuleEditorController extends BaseEditorController {
	...
	protected function _isRuleEditable() {
		...
		if ( ! $t_rule->haveAccessToForm( $this->getRequest()->getUserID(), __CA_BUNDLE_DISPLAY_EDIT_ACCESS__,
			$pn_rule_id )
		) {        // is user allowed to edit rule?
	...
	public function getTriggerTypeFilterForm() {
		...
		$this->view->setVar( 'filters', $t_trigger->getTriggerInstance()->getElementFilters( $pn_element_id, $ps_prefix,
			[ 'values' => $t_trigger->get( 'element_filters' ) ] ) );
