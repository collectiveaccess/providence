...
class PawtucketController extends ActionController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		if ( ! $this->request->isLoggedIn()
		     || ( ! $this->request->getUser()->canDoAction( 'can_edit_theme_global_values' )
		          && ! $this->request->getUser()->canDoAction( 'can_edit_theme_page_content' ) )
		) {
	...
	public function editGlobalValues() {
		...
		if ( is_array( $va_template_values = $this->request->config->getAssoc( 'global_template_values' ) ) ) {
			foreach ( $va_template_values as $vs_name => $va_info ) {
				...
				$vs_element = caHTMLTextInput( $vs_name, [
					'value'  => $o_appvars->getVar( "pawtucket_global_{$vs_name}" ),
					"width"  => $vn_width,
					"height" => $vn_height,
					'class'  => 'form-control',
					'id'     => "pawtucket_global_{$vs_name}"
				] );
