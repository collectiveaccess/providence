...
class ToolsController extends ActionController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		if ( ! $po_request->isLoggedIn() || ! $po_request->user->canDoAction( 'can_use_plugin_tools' ) ) {
			$this->response->setRedirect( $po_request->config->get( 'error_display_url' ) . '/n/3000?r='
	...
	public function Index( $pa_values = null, $pa_options = null ) {
		if ( ! $this->request->isLoggedIn() || ! $this->request->user->canDoAction( 'can_use_plugin_tools' ) ) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/3500?r='
	...
	public function Settings() {
		...
		if ( ! ( $o_tool = $this->opo_tools_manager->getTool( $ps_tool_name ) ) ) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/3510?r='
			                              . urlencode( $this->request->getFullUrlPath() ) );
	...
	public function Run() {
		...
		if ( ! in_array( $ps_command, $va_valid_commands ) ) { // Invalid command
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/3520?r='
			                              . urlencode( $this->request->getFullUrlPath() ) );
		...
		foreach ( $va_settings as $vs_setting => $va_setting_info ) {
			$va_setting_values[ $vs_setting ] =
			$va_last_setting_values[ $vs_setting ] = $this->request->getParameter( "{$vs_form_id}_{$vs_setting}",
				pString );
			if ( $va_setting_info['displayType'] == DT_FILE_BROWSER ) {
				$va_setting_values[ $vs_setting ] = $this->request->config->get( 'batch_media_import_root_directory' )
				                                    . '/' . $va_setting_values[ $vs_setting ];
			}
		}
