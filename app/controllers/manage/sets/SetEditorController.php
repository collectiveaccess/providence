...
class SetEditorController extends BaseEditorController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		if ( ! $t_set->haveAccessToSet( $po_request->getUserID(), __CA_SET_EDIT_ACCESS__, null,
	...
	public function Edit( $pa_values = null, $pa_options = null ) {
		...
		if ( ! $t_subject->haveAccessToSet( $this->request->getUserID(), __CA_SET_EDIT_ACCESS__, null,
	...
	public function GetItemInfo() {
		if ( $pn_set_id = $this->request->getParameter( 'set_id', pInteger ) ) {
			...
			// does user have edit access to set?
			if ( ! $t_set->haveAccessToSet( $this->request->getUserID(), __CA_SET_EDIT_ACCESS__, null,
				array( 'request' => $this->request ) )
			) {
				$this->notification->addNotification( _t( "You cannot edit this set" ), __NOTIFICATION_TYPE_ERROR__ );
				$this->Edit();
	...
	public function getSetMedia() {
		...
		if ( ! is_array( $va_record_ids ) || ! sizeof( $va_record_ids ) ) {
			$this->notification->addNotification( _t( 'No media is available for download' ),
		...
		if ( sizeof( $va_paths ) > 0 ) {
			...
			$o_view->setVar( 'archive_name', 'media_for_' . mb_substr( preg_replace( '![^A-Za-z0-9]+!u', '_',
					( $vs_set_code = $t_set->get( 'set_code' ) ) ? $vs_set_code : $t_set->getPrimaryKey() ), 0, 20 )
			                                 . '.zip' );
	...
	public function DuplicateItems() {
		...
		if ( ! (bool) $this->request->config->get( 'ca_sets_disable_duplication_of_items' )
		     && $this->request->user->canDoAction( 'can_duplicate_items_in_sets' )
		     && $this->request->user->canDoAction( 'can_duplicate_' . $t_set->getItemType() )
		) {
			...
			if ( ! $t_dupe_set ) {
				$this->notification->addNotification( _t( 'Could not duplicate items in set: %1',
					join( ';', $t_set->getErrors() ) ), __NOTIFICATION_TYPE_ERROR__ );
				$this->Edit();

				return;
			}

			$this->notification->addNotification( _t( 'Records have been successfully duplicated and added to set' ),
				__NOTIFICATION_TYPE_INFO__ );
			$this->opo_response->setRedirect( caEditorUrl( $this->getRequest(), 'ca_sets',
				$t_dupe_set->getPrimaryKey() ) );
		} else {
	...
	public function exportSetItems() {
		...
		$va_record_ids = array_keys( $t_set->getItemRowIDs( array( 'limit' => 100000 ) ) );
		if ( ! is_array( $va_record_ids ) || ! sizeof( $va_record_ids ) ) {
			$this->notification->addNotification( _t( 'No items are available for export' ),
				__NOTIFICATION_TYPE_ERROR__ );
			$this->opo_response->setRedirect( caEditorUrl( $this->opo_request, 'ca_sets', $t_set->getPrimaryKey() ) );
		...
		caExportResult( $this->request, $qr_res, $ps_export_format, '_output',
			[ 'printTemplateType' => 'sets', 'set' => $t_set, 'filename' => $filename_stub ] );
