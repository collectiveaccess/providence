...
class SetEditorController extends BaseEditorController
{
    ...
    public function __construct(&$po_request, &$po_response, $pa_view_paths = null)
    {
        ...
        if (!$t_set->haveAccessToSet(
            $po_request->getUserID(),
            __CA_SET_EDIT_ACCESS__,
            null,
            ...
            $o_view->setVar(
                ...
                'media_for_' . mb_substr(
                    preg_replace(
                        '![^A-Za-z0-9]+!u',
                        '_',
                        ($vs_set_code = $t_set->get(
                            'set_code'
                        )) ? $vs_set_code : $t_set->getPrimaryKey()
                    ),
                    0,
                    20
                ) . '.zip'
    ...
    public function DuplicateItems()
    {
        ...
        if (!(bool)$this->request->config->get(
                'ca_sets_disable_duplication_of_items'
            ) && $this->request->user->canDoAction('can_duplicate_items_in_sets') && $this->request->user->canDoAction(
                'can_duplicate_' . $t_set->getItemType()
            )) {
            ...
            if (!$t_dupe_set) {
                $this->notification->addNotification(
                    _t('Could not duplicate items in set: %1', join(';', $t_set->getErrors())),
            ...
            $this->notification->addNotification(
                _t('Records have been successfully duplicated and added to set'),
