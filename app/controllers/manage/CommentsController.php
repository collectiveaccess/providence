...
class CommentsController extends BaseSearchController
{
        ...
        if (is_array($pa_comment_ids) && (sizeof($pa_comment_ids) > 0)) {
            ...
            if (sizeof($va_errors) > 0) {
                ...
                $this->notification->addNotification(
                    _t("Your comments have been approved"),
                    __NOTIFICATION_TYPE_INFO__
                );
            }
        } else {
            $this->notification->addNotification(
                _t("Please use the checkboxes to select comments for approval"),
    ...
    public function Delete()
    {
        ...
        if (is_array($pa_comment_ids) && (sizeof($pa_comment_ids) > 0)) {
            ...
            $this->notification->addNotification(
                _t("Please use the checkboxes to select comments for deletion"),
                __NOTIFICATION_TYPE_WARNING__
            );
        }
        switch ($ps_mode) {
