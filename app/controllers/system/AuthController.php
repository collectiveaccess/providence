...
class AuthController extends ActionController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		if ( AuthenticationManager::supports( __CA_AUTH_ADAPTER_FEATURE_USE_ADAPTER_LOGIN_FORM__ )
		     && ! in_array( $this->request->getAction(), [ 'logout', 'callback' ] )
		) {
			$po_response->setRedirect( $po_request->getBaseUrlPath() . '/' . $po_request->getScriptName() . '/'
			                           . $po_request->config->get( "auth_login_welcome_path" ) );
	...
	public function DoLogin() {
		...
		$vs_redirect_url = $this->request->getParameter( 'redirect', pString, null, [ 'forcePurify' => true ] )
			?: caNavUrl( $this->request, null, null, null );
		if ( ! $this->request->doAuthentication( array(
			'dont_redirect_to_login' => true,
			'redirect'               => $vs_redirect_url,
			'noPublicUsers'          => true,
			'user_name'              => $this->request->getParameter( 'username', pString ),
			...
			$g_ui_units_pref
				= $this->request->user->getPreference( 'units' );            // user's selected display units for measurements 	(available as global)
		...
		if ( $t_user->load( $vs_username ) ) {
			if ( $t_user->isValidToken( $vs_token ) ) {
				...
				if ( $vs_pw !== $vs_pw_check ) {
					...
					$this->notification->addNotification( _t( "Passwords did not match. Please try again." ),
					...
					$this->notification->addNotification( _t( "Password was successfully changed. You can now log in with your new password." ),
