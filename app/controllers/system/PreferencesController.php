...
class PreferencesController extends ActionController
{
    ...
    public function EditDuplicationPrefs()
    {
        ...
        $vs_current_table = 'ca_' . $this->request->getActionExtra(
            );    // url action extra is table name without "ca_" (eg. places => ca_places)
        ...
        if (!is_array(
            $va_duplication_element_settings = $this->request->user->getPreference(
                $vs_current_table . '_duplicate_element_settings'
            )
        )) {
    ...
    public function Save()
    {
        ...
        switch ($vs_action) {
            case 'EditCataloguingPrefs':
                ...
                $this->request->user->setPreference(
                    'cataloguing_locale',
                    $this->request->getParameter('pref_cataloguing_locale', pString)
                );
                $this->request->user->setPreference(
                    'cataloguing_delete_reference_handling_default',
                    $this->request->getParameter(
                        'pref_cataloguing_delete_reference_handling_default',
                        pString
                    )
                );
            ...
            default:
                ...
                foreach ($this->request->user->getValidPreferences($vs_group) as $vs_pref) {
                    $this->request->user->setPreference(
                        $vs_pref,
                        $vs_locale = $this->request->getParameter(
                            'pref_' . $vs_pref,
                            pString
                        )
                    );
                    ...
                    if ($vs_pref == 'ui_theme') {
                        ...
                        $this->view->setViewPath(
                            $this->request->getViewsDirectoryPath() . '/' . $this->request->getModulePath()
                        );
                    }
                }
