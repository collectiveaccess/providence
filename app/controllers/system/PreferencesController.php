...
class PreferencesController extends ActionController {
	 ...
	 */
	public static $s_duplicable_tables
		= array(
			'ca_objects',
			'ca_object_lots',
			'ca_entities',
	...
	public function EditQuickSearchPrefs() {
		...
		foreach ( QuickSearch::availableSearches( [ 'expandByType' => true ] ) as $vs_bundle => $va_bundle_info ) {
			$va_available_display_items[ $vs_bundle ] = [
				'placement_id' => null,
				'display'      => $va_bundle_info['displayname'],
				'bundle'       => $vs_bundle
			];
		}
	...
	public function EditDuplicationPrefs() {
		...
		$vs_current_table = 'ca_'
		                    . $this->request->getActionExtra();    // url action extra is table name without "ca_" (eg. places => ca_places)
		if ( ! in_array( $vs_current_table, PreferencesController::$s_duplicable_tables ) ) {
			throw new ApplicationException( _t( 'No duplication preferences for %1',
		...
		if ( ! is_array( $va_duplication_element_settings = $this->request->user->getPreference( $vs_current_table
		...
		foreach ( $va_available_bundles as $vs_bundle_name => $va_bundle_info ) {
			...
			$vn_duplication_setting            = isset( $va_duplication_element_settings[ $vs_bundle_name ] )
				? $va_duplication_element_settings[ $vs_bundle_name ] : 1;
	...
	public function Save() {
		...
		switch ( $vs_action ) {
			case 'EditCataloguingPrefs':
				...
				$this->request->user->setPreference( 'cataloguing_locale',
					$this->request->getParameter( 'pref_cataloguing_locale', pString ) );
				$this->request->user->setPreference( 'cataloguing_delete_reference_handling_default',
					$this->request->getParameter( 'pref_cataloguing_delete_reference_handling_default', pString ) );
			...
			case 'EditUnitsPrefs':
				...
				foreach ( $this->request->user->getValidPreferences( $vs_group ) as $vs_pref ) {
					$this->request->user->setPreference( $vs_pref,
						$this->request->getParameter( 'pref_' . $vs_pref, pString ) );
				}
				break;
			...
			case 'EditDuplicationPrefs':
				...
				if ( in_array( $vs_current_table, PreferencesController::$s_duplicable_tables ) ) {
					...
					foreach ( $this->request->user->getValidPreferences( $vs_group ) as $vs_pref ) {
						...
						switch ( $vs_pref ) {
							case 'duplicate_relationships':
								...
								break;
							default:
								$vm_val = $this->request->getParameter( "pref_{$vs_current_table}_{$vs_pref}",
									pString );
								break;
						}
					...
					if ( (bool) $this->request->getParameter( "pref_{$vs_current_table}_duplicate_attributes",
						pString )
					) {
						$vm_val = $this->request->getParameter( "duplicate_element_settings", pArray );
						$this->request->user->setPreference( "{$vs_current_table}_duplicate_element_settings",
							$vm_val );
					}
				}
				$this->view->setVar( 'group', 'duplication' );
			...
			default:
				...
				foreach ( $this->request->user->getValidPreferences( $vs_group ) as $vs_pref ) {
					$this->request->user->setPreference( $vs_pref,
						$vs_locale = $this->request->getParameter( 'pref_' . $vs_pref, pString ) );
					...
					if ( ( $vs_pref == 'ui_locale' ) && $vs_locale ) {
						...
						$g_ui_locale_id
							= $this->request->user->getPreferredUILocaleID();            // get current UI locale as locale_id	 			(available as global)
						$g_ui_locale
							= $this->request->user->getPreferredUILocale();                // get current UI locale as locale string 			(available as global)
					...
					if ( $vs_pref == 'ui_theme' ) {
						...
						$this->view->setViewPath( $this->request->getViewsDirectoryPath() . '/'
