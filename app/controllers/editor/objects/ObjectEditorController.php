...
class ObjectEditorController extends BaseEditorController {
	...
	public function postSave( $t_object, $pb_is_insert ) {
		if (
			...
			&& ( $vs_coll_rel_type
				= $this->request->config->get( 'ca_objects_x_collections_hierarchy_relationship_type' ) )
			...
			if ( ! ( $t_object->addRelationship( 'ca_collections', $pn_collection_id, $vs_coll_rel_type ) ) ) {
				$this->notification->addNotification( _t( "Could not add parent collection to object: %1",
					join( "; ", $t_object->getErrors() ) ), __NOTIFICATION_TYPE_ERROR__ );
			}
		}
	}
	# -------------------------------------------------------
