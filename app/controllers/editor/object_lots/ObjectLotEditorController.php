		...
		if ( (bool) $this->request->config->get( 'allow_automated_renumbering_of_objects_in_a_lot' ) ) {
			...
			if ( $t_lot->getPrimaryKey() ) {
				...
				if ( $t_lot->numErrors() ) {
					...
					$this->notification->addNotification( _t( 'Renumbered contents of lot' ),
	...
	public function getLotMedia() {
		...
		if ( $t_lot->getPrimaryKey() ) {
			...
			if ( ! is_array( $va_object_ids ) || ! sizeof( $va_object_ids ) ) {
				...
				$this->opo_response->setRedirect( caEditorUrl( $this->opo_request, 'ca_object_lots',
					$t_lot->getPrimaryKey() ) );
			...
			if ( sizeof( $va_paths ) > 0 ) {
				...
				$o_view->setVar( 'archive_name', 'media_for_' . mb_substr( preg_replace( '![^A-Za-z0-9]+!u', '_',
						( $vs_idno = $t_lot->get( 'idno_stub' ) ) ? $vs_idno : $t_lot->getPrimaryKey() ), 0, 20 )
				                                 . '.zip' );
