...
class ObjectRepresentationEditorController extends BaseEditorController {
	...
	public function GetAnnotationEditor() {
		...
		// Get player
		$va_display_info = caGetMediaDisplayInfo( 'annotation_editor',
			$t_rep->getMediaInfo( "media", "original", "MIMETYPE" ) );
		$this->view->setVar( 'player', $t_rep->getMediaTag( 'media', $va_display_info['display_version'], array(
			'viewer_width'  => $vn_player_width = $va_display_info['viewer_width'],
			'viewer_height' => $vn_player_height = $va_display_info['viewer_height'],
			'id'            => 'caAnnoEditorMediaPlayer',
			'class'         => 'caAnnoEditorMediaPlayer' . ( ( true ) ? ' caAnnoEditorAudioMediaPlayer'
	...
	public function GetImageCenterEditor() {
		...
		$va_display_info = caGetMediaDisplayInfo( 'image_center_editor',
			$t_rep->getMediaInfo( "media", "original", "MIMETYPE" ) );
		$this->view->setVar( 'image', $t_rep->getMediaTag( 'media', $va_display_info['display_version'],
			array( 'id' => 'caImageCenterEditorImage', 'class' => 'caImageCenterEditor' ) ) );
	...
	public function GetAnnotationList() {
		...
		$this->view->setVar( 'annotation_list', array(
			'start' => $vn_start,
			'max'   => $vn_max,
			'total' => $vn_total,
			'list'  => array_values( $t_rep->getAnnotations( array( 'start' => $vn_start, 'max' => $vn_max ) ) )
		) );
	...
	public function DownloadCaptionFile() {
		...
		if ( ! $t_caption->getPrimaryKey()
		     || ( (int) $t_caption->get( 'representation_id' ) !== (int) $pn_representation_id )
		) {
		...
		$this->view->setVar( 'download_name', caGetRepresentationDownloadFileName( 'ca_object_representations', [
			'idno'              => $t_rep->get( 'idno' ),
			'index'             => null,
			'version'           => 'captions',
			'extension'         => 'vtt',
			'original_filename' => $va_info['ORIGINAL_FILENAME'],
	...
	public function info( $pa_parameters ) {
		...
		$vn_representation_id = ( isset( $pa_parameters['representation_id'] ) ) ? $pa_parameters['representation_id']
			: null;
		...
		if ( $vn_representation_id ) {
			...
			$this->view->setVar( 'object_editor_screen',
				$t_ui->getScreenWithBundle( 'ca_object_representations', $this->request ) );
		}
