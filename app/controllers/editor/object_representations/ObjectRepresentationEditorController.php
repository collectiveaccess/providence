...
class ObjectRepresentationEditorController extends BaseEditorController
{
    ...
    public function GetAnnotationEditor()
    {
        ...
        // Get player
        $va_display_info = caGetMediaDisplayInfo(
            'annotation_editor',
        ...
        $this->view->setVar(
            ...
            $t_rep->getMediaTag(
                'media',
                $va_display_info['display_version'],
                array(
                    ...
                    'class' => 'caAnnoEditorMediaPlayer' . ((true) ? ' caAnnoEditorAudioMediaPlayer' : ' caAnnoEditorVideoMediaPlayer')
                )
            )
        );
    ...
    public function GetAnnotationList()
    {
        ...
        $this->view->setVar(
            ...
            array(
                ...
                'total' => $vn_total,
                'list' => array_values(
                    $t_rep->getAnnotations(array('start' => $vn_start, 'max' => $vn_max))
                )
            )
        );
    ...
    public function DownloadCaptionFile()
    {
        ...
        if (!$t_caption->getPrimaryKey() || ((int)$t_caption->get(
                    'representation_id'
                ) !== (int)$pn_representation_id)) {
        ...
        switch ($this->request->user->getPreference('downloaded_file_naming')) {
            case 'idno':
                $this->view->setVar(
                    'download_name',
                    (str_replace(' ', '_', $t_rep->get('idno'))) . "_captions_{$vs_locale}.vtt"
                );
                break;
            ...
            case 'idno_and_rep_id_and_version':
                $this->view->setVar(
                    ...
                    (str_replace(
                        ...
                        '_',
                        $t_rep->get('idno')
                    )) . "_representation_{$pn_representation_id}_captions_{$vs_locale}.vtt"
                );
                break;
            case 'original_name':
            default:
