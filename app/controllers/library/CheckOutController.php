...
class CheckOutController extends ActionController {
	...
	public function GetObjectInfo() {
		...
		$vn_current_user_id
			= $vs_current_user
			= $vs_current_user_checkout_date = $vs_reservation_list = null;        // user_id of current holder of item
		...
		switch ( $vn_status = $t_object->getCheckoutStatus() ) {
			...
			case __CA_OBJECTS_CHECKOUT_STATUS_OUT__:
				...
				$vs_status_display             = ( $vn_current_user_id == $pn_user_id ) ? _t( 'Out with this user' )
					: _t( 'Out' );
			...
			case __CA_OBJECTS_CHECKOUT_STATUS_OUT_WITH_RESERVATIONS__:
				...
				$vs_status_display = ( $vn_num_reservations == 1 ) ? _t( 'Out with %1 reservation',
					$vn_num_reservations ) : _t( 'Out with %1 reservations', $vn_num_reservations );
				break;
			case __CA_OBJECTS_CHECKOUT_STATUS_RESERVED__:
				...
				$vs_status_display = ( $vn_num_reservations == 1 ) ? _t( 'Reserved' )
					: _t( 'Available with %1 reservations', $vn_num_reservations );
		...
		if ( is_array( $va_reservations ) ) {
			...
			foreach ( $va_reservations as $va_reservation ) {
				...
				$va_tmp[]                       = $t_user->get( 'fname' ) . ' ' . $t_user->get( 'lname' ) . ( (
					$vs_email = $t_user->get( 'email' ) ) ? " ({$vs_email})" : "" );
			}
		...
		if ( $vb_is_held_by_current_user ) {
			...
			$vs_reserve_display_label = ( $vn_status == 3 ) ? _t( 'Currently reserved by %1', $vs_reservation_list )
				: _t( 'Will reserve' );

			if ( in_array( $vn_status, array( 1, 2 ) ) ) {
				$vs_holder_display_label = _t( 'held by %1 since %2', $vs_current_user,
					$vs_current_user_checkout_date );
			}
		}
		$va_info          = array(
		...
		$va_info['storage_location']
			= $t_object->getWithTemplate( $va_checkout_config['show_storage_location_template'] );
				...
				if ( $va_checkout_info = $t_checkout->objectIsOut( $va_item['object_id'] ) ) {
					...
					try {
						...
						if ( $vb_res ) {
							...
							$va_ret['errors'][ $va_item['object_id'] ] = _t( 'Could not reserve <em>%1</em>: %2',
								$vs_name, join( '; ', $t_checkout->getErrors() ) );
						}
					} catch ( Exception $e ) {
						$va_ret['errors'][ $va_item['object_id'] ] = _t( 'Could not reserve <em>%1</em>: %2', $vs_name,
							$e->getMessage() );
					}
				} else {
					try {
						$vb_res = $t_checkout->checkout( $va_item['object_id'], $pn_user_id, $va_item['note'],
							$va_item['due_date'], array( 'request' => $this->request ) );

						if ( $vb_res ) {
							$va_ret['checkouts'][ $va_item['object_id'] ]
								= _t( 'Checked out <em>%1</em>; due date is %2', $vs_name, $va_item['due_date'] );
						} else {
							$va_ret['errors'][ $va_item['object_id'] ] = _t( 'Could not check out <em>%1</em>: %2',
								$vs_name, join( '; ', $t_checkout->getErrors() ) );
						}
					} catch ( Exception $e ) {
						$va_ret['errors'][ $va_item['object_id'] ] = _t( 'Could not check out <em>%1</em>: %2',
							$vs_name, $e->getMessage() );
					}
				}
			}
		}
