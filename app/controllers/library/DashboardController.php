...
class DashboardController extends ActionController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		if ( ! $this->request->isLoggedIn() || ! $this->request->user->canDoAction( 'can_do_library_checkout' )
		     || ! $this->request->config->get( 'enable_library_services' )
		     || ! $this->request->config->get( 'enable_object_checkout' )
		) {
	...
	public function Index() {
		...
		foreach (
			...
			foreach ( $va_stats[ $vs_stat_key ] as $va_user ) {
				$va_user_list[] = "<a href='#' class='caLibraryUserLink' data-user_id='" . $va_user['user_id'] . "'>"
				                  . trim( $va_user['fname'] . ' ' . $va_user['lname'] )
				                  . "</a>"; //.($va_user['email'] ? ' ('.$va_user['email'].')' : '');
				$vn_c ++;
				if ( $vn_c >= 100 ) {
		...
		$this->view->setVar( 'panels',
			$va_panels = is_array( $va_dashboard_config['panels'] ) ? $va_dashboard_config['panels'] : array() );
		...
		foreach ( $va_panels as $vs_panel => $va_panel_info ) {
			...
			while ( $qr_objects->nextHit() ) {
				foreach ( $va_group_bys as $vn_i => $vs_group_by ) {
					if ( is_array( $va_attrs = $qr_objects->get( $vs_group_by,
						array( 'returnWithStructure' => true, 'convertCodesToDisplayText' => true ) ) )
					) {
						if ( ! sizeof( $va_attrs ) ) {
							$va_count['?'] ++;
							break;
	...
	public function getUserDetail() {
		...
		if ( $t_user->getPrimaryKey() ) {
			...
			$vs_item_display_template
				= "<unit relativeTo=\"ca_objects\"><l>^ca_objects.preferred_labels.name</l> (^ca_objects.idno)</unit>";

			// Get checkouts
			$this->view->setVar( 'checkouts',
				ca_object_checkouts::getOutstandingCheckoutsForUser( $pn_user_id, $vs_item_display_template,
					$ps_daterange, [ 'omitOverdue' => true ] ) );

			// Get checkins
			$this->view->setVar( 'checkins',
				ca_object_checkouts::getCheckinsForUser( $pn_user_id, $vs_item_display_template, $ps_daterange ) );

			// Get overdue
			$this->view->setVar( 'overdue_checkouts',
				ca_object_checkouts::getOverdueCheckoutsForUser( $pn_user_id, $vs_item_display_template,
					$ps_daterange ) );

			// Get reservations
			$this->view->setVar( 'reservations',
				ca_object_checkouts::getOutstandingReservationsForUser( $pn_user_id, $vs_item_display_template ) );
