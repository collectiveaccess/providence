...
class DashboardController extends ActionController
{
    ...
    public function __construct(&$po_request, &$po_response, $pa_view_paths = null)
    {
        ...
        if (!$this->request->isLoggedIn() || !$this->request->user->canDoAction(
                'can_do_library_checkout'
            ) || !$this->request->config->get('enable_library_services') || !$this->request->config->get(
                'enable_object_checkout'
            )) {
    ...
    public function Index()
    {
        ...
        foreach (
            ...
            foreach ($va_stats[$vs_stat_key] as $va_user) {
                $va_user_list[] = "<a href='#' class='caLibraryUserLink' data-user_id='" . $va_user['user_id'] . "'>" . trim(
                        $va_user['fname'] . ' ' . $va_user['lname']
        ...
        $this->view->setVar(
            'panels',
            $va_panels = is_array(
                $va_dashboard_config['panels']
            ) ? $va_dashboard_config['panels'] : array()
        );
    ...
    public function getUserDetail()
    {
        ...
        if ($t_user->getPrimaryKey()) {
            ...
            $this->view->setVar(
                'checkouts',
                ca_object_checkouts::getOutstandingCheckoutsForUser(
                    $pn_user_id,
            ...
            $this->view->setVar(
                'checkins',
                ca_object_checkouts::getCheckinsForUser(
                    $pn_user_id,
                    $vs_item_display_template,
                    $ps_daterange
                )
            ...
            $this->view->setVar(
                'overdue_checkouts',
                ca_object_checkouts::getOverdueCheckoutsForUser(
                    $pn_user_id,
                    $vs_item_display_template,
                    $ps_daterange
                )
            ...
            $this->view->setVar(
                'reservations',
                ca_object_checkouts::getOutstandingReservationsForUser(
                    $pn_user_id,
                    $vs_item_display_template
                )
            );
