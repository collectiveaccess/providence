...
class DashboardController extends ActionController {
	...
	public function Index() {
		$can_access_non_local_system_statistics
			= (bool) $this->request->user->canDoAction( 'can_view_system_statistics' );
		...
		foreach ( $site_list as $site => $site_info ) {
			...
			$site_links[] = caNavLink( $this->request, $site_info['name'], '', '*', '*', '*',
				[ 'site' => $site, 'group' => $cur_group ] );
		}
		...
		$this->view->setVar( 'last_update',
			( $d = PersistentCache::fetch( 'site_statistics_last_fetch', 'statistics' ) ) ? caGetLocalizedDate( $d )
				: "" );
