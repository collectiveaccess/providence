...
class trackProcessingWidget extends BaseWidget implements IWidget
{
    ...
    public function renderWidget($ps_widget_id, &$pa_settings)
    {
        ...
        while ($qr_completed->nextRow()) {
            ...
            $va_completed[$va_row["task_id"]]["by"] = caFormatPersonName(
                $va_row["fname"],
                $va_row['lname'],
                _t('Command line or job')
            );
        ...
        while ($qr_qd->nextRow()) {
            ...
            if (!$vo_tq->rowKeyIsBeingProcessed($va_row["row_key"])) {
                ...
                $va_qd_jobs[$va_row["task_id"]]["by"] = caFormatPersonName(
                    $va_row["fname"],
                    $va_row['lname'],
                    _t('Command line or job')
                );
                ...
                $va_pr_jobs[$va_row["task_id"]]["by"] = caFormatPersonName(
                    $va_row["fname"],
                    $va_row['lname'],
                    _t('Command line or job')
                );
