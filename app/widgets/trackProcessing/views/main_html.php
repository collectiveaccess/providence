...
<div class="dashboardWidgetContentContainer" id="widget_<?php print $vs_widget_id; ?>">
    ...
    if ((sizeof($va_jobs_processing) > 0) || (sizeof($va_jobs_queued) > 0) || (sizeof($va_jobs_done) > 0)) {
        ...
        <div id="tabContainer_<?php print $vs_widget_id; ?>" class="tabContainer">
            <ul>
                ...
                if (sizeof($va_jobs_processing) > 0) {
                    ?>
                    <li><a href="#running_<?php print $vs_widget_id; ?>"><span><?php print _t(
                                    "%1 running",
                                    sizeof(
                                        $va_jobs_processing
                                    )
                ...
                if (sizeof($va_jobs_queued) > 0) {
                    ?>
                    <li><a href="#queued_<?php print $vs_widget_id; ?>"><span><?php print _t(
                                    "%1 queued",
                                    sizeof($va_jobs_queued)
                                ); ?></span></a></li>
                    <?php
                }
                if (sizeof($va_jobs_done) > 0) {
                    ?>
                    <li><a href="#completed_<?php print $vs_widget_id; ?>"><span><?php print _t(
                                    "%1 completed",
                                    $vn_jobs_done
                                ); ?></span></a></li>
                    <?php
                            ...
                            foreach ($va_jobs_processing as $va_job):
                                ...
                                <tr>
                                    <td>
                                        <?php print "<h2>" . _t(
                                                'By <em>%1</em>',
                                                mb_strtolower($va_job['handler_name'])
                                            ) . "</h2>"; ?>

                                        <?php print "<strong>" . _t("Created on") . "</strong>: " . date(
                                                "n/d/Y @ g:i:sa T",
                                                $va_job["created"]
                                            ) . "<br />"; ?>
                                        <?php print "<strong>" . _t(
            ...
            if (sizeof($va_jobs_done) > 0):
                ...
                <div id="completed_<?php print $vs_widget_id; ?>">
                    <div class="dashboardWidgetScrollMedium">
                        <table class='dashboardWidgetTable'>
                            <tr>
                                <th><strong><?php print _t(
                                                "Jobs completed in the last %1 hours",
                                                $this->getVar('hours')
                                            ) . ":"; ?></strong></th>
                            </tr>
                            <?php
                                ...
                                <tr>
                                    <td>
                                        ...
                                        if ((int)$va_job["completed_on"] > 0) {
                                            print "<strong>" . _t('Completed on') . "</strong>: " . date(
                                                    "n/d/Y @ g:i:sa T",
                                                    $va_job["completed_on"]
                                                ) . "<br/>\n";

                                            if ((int)$va_job["error_code"] > 0) {
                                                print "<span style='color: #cc0000;'><strong>" . _t(
                                                        'Error'
                                                    ) . "</strong>: " . $va_job["error_message"] . " [" . $va_job["error_code"] . "] <em>" . _t(
                                            ...
                                            switch ($vs_code) {
                                                case 'table':
                                                    ...
                                                    if ($vs_link = caEditorLink(
                                                        ...
                                                        array(),
                                                        array(),
                                                        array('verifyLink' => true)
                                                    )) {
                                                        print "<strong>" . $va_info['label'] . "</strong>: " . $vs_link . "<br/>\n";
                                                    } else {
                                                        print "<strong>" . $va_info['label'] . "</strong>: " . $va_info['value'] . " [<em>" . _t(
                                                                'DELETED'
                                                            ) . "</em>]<br/>\n";
                                                    }
                                                    break;
                                        ...
                                        <?php print "<strong>" . _t(
                                            ...
                                            ) . "</strong>: " . $va_job['processing_time'] . "s<br />"; ?>
                                    </td>
                                </tr>
                            <?php
                            endforeach;
        ...
        print _t(
            "There are no running jobs, queued jobs or jobs completed in the last %1 hours.",
            $this->getVar('hours')
        );
    }
    ?>
...
<script type="text/javascript">
    ...
    setInterval(function () {
        jQuery('#widget_<?php print $vs_widget_id; ?>').load('<?php print caNavUrl(
            $this->request,
            '',
            'Dashboard',
            'getWidget',
            array('widget_id' => $vs_widget_id)
        );?>');
