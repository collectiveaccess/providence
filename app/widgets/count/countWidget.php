...
class countWidget extends BaseWidget implements IWidget
{
    ...
    public function __construct($ps_widget_path, $pa_settings)
    {
        ...
        $this->opa_tables = array(
            ...
            'ca_occurrences' => 0,
            'ca_collections' => 0,
            'ca_storage_locations' => 0,
            'ca_object_representations' => 0,
            'ca_loans' => 0,
            'ca_movements' => 0
        );
        ...
        foreach (array_keys($va_tables) as $vs_table) {
            if (
                ...
                (
                    ...
                    &&
                    (!(($vs_table == 'ca_object_representations') && !(bool)$this->getRequest()->config->get(
                            'ca_objects_disable'
                        )))
                )
            ) {
    ...
    public static function getRoleActionList()
    {
        return array(
            'can_use_counts_widget' => array(
                ...
                'description' => _t(
                    'User can use dashboard widget that keeps track of totals for objects and authorities.'
                )
            )
        );
    }
