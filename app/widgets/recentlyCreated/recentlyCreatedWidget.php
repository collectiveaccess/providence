...
class recentlyCreatedWidget extends BaseWidget implements IWidget {
	...
	public function __construct( $ps_widget_path, $pa_settings ) {
		...
		foreach ( $this->opa_table_display_names as $vs_table => $vs_display ) {
			if ( ! $this->getRequest()
			     || ! $this->getRequest()->user->canDoAction( "can_use_recently_created_widget_{$vs_table}" )
			) {
				foreach (
					BaseWidget::$s_widget_settings['recentlyCreatedWidget']["display_type"]["options"] as
					$vs_setting_display => $vs_setting_table
				) {
	...
	public function renderWidget( $ps_widget_id, &$pa_settings ) {
		...
		if ( ! in_array( $pa_settings['display_type'],
		...
		if ( $t_table = Datamodel::getInstanceByTableName( $pa_settings['display_type'], true ) ) {
			...
			if ( $pa_settings["display_limit"] && intval( $pa_settings["display_limit"] ) > 0
			     && intval( $pa_settings["display_limit"] ) < 1000
			) {
			...
			while ( $qr_records->nextRow() && sizeof( $va_item_list ) < intval( $pa_settings["display_limit"] ) ) {
				if ( isset( $va_item_list[ $qr_records->get( $t_table->primaryKey() ) ] ) ) { // we only overwrite if we hit one with UI locale (i.e. the first hit wins if there is no label in UI locale)
					if ( ! ( intval( $qr_records->get( $t_table->getLabelTableName() . ".locale_id" ) )
					         == intval( $g_ui_locale_id ) )
					) {
						continue;
					}
				}
				...
				$va_item_list[ $qr_records->get( $t_table->primaryKey() ) ] = array(
					"display"   => $qr_records->get( $t_table->getLabelTableName() . "."
					                                 . $t_table->getLabelDisplayField() ),
			...
			if ( ! ( intval( $pa_settings["height_px"] ) > 30
			         && intval( $pa_settings["height_px"] ) < 1000 )
			...
			$this->opo_view->setVar( 'idno_display',
