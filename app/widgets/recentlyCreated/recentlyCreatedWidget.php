...
class recentlyCreatedWidget extends BaseWidget implements IWidget
{
    ...
    public function __construct($ps_widget_path, $pa_settings)
    {
        ...
        foreach ($this->opa_table_display_names as $vs_table => $vs_display) {
            if (!$this->getRequest() || !$this->getRequest()->user->canDoAction(
                    "can_use_recently_created_widget_{$vs_table}"
                )) {
    ...
    public function renderWidget($ps_widget_id, &$pa_settings)
    {
        ...
        if (!in_array(
            $pa_settings['display_type'],
            BaseWidget::$s_widget_settings['recentlyCreatedWidget']["display_type"]["options"]
        )) {
            return "";
        }
        if ($t_table = Datamodel::getInstanceByTableName($pa_settings['display_type'], true)) {
            ...
            if ($pa_settings["display_limit"] && intval($pa_settings["display_limit"]) > 0 && intval(
                    $pa_settings["display_limit"]
                ) < 1000) {
            ...
            while ($qr_records->nextRow() && sizeof($va_item_list) < intval($pa_settings["display_limit"])) {
                ...
                $va_item_list[$qr_records->get($t_table->primaryKey())] = array(
                    "display" => $qr_records->get(
                        $t_table->getLabelTableName() . "." . $t_table->getLabelDisplayField()
                    ),
            ...
            if (!(intval($pa_settings["height_px"]) > 30 && intval(
                    $pa_settings["height_px"]
                ) < 1000)) { // if value is not within reasonable boundaries, set it to default
            ...
            $this->opo_view->setVar(
                'idno_display',
                (isset($pa_settings["display_idno"]) ? $pa_settings["display_idno"] : false)
            );
    ...
    static public function getRoleActionList()
    {
        return array(
            'can_use_recently_created_widget_ca_objects' => array(
                ...
                'description' => _t(
                    'User can use widget that shows recently created items to list new objects in the dashboard.'
                )
            ),
            'can_use_recently_created_widget_ca_entities' => array(
                'label' => _t('Recently created: entities'),
                'description' => _t(
                    'User can use widget that shows recently created items to list new entities in the dashboard.'
                )
            ),
            'can_use_recently_created_widget_ca_places' => array(
                'label' => _t('Recently created: places'),
                'description' => _t(
                    'User can use widget that shows recently created items to list new places in the dashboard.'
                )
            ),
            'can_use_recently_created_widget_ca_occurrences' => array(
                'label' => _t('Recently created: occurrences'),
                'description' => _t(
                    'User can use widget that shows recently created items to list new occurrences in the dashboard.'
                )
            ),
            'can_use_recently_created_widget_ca_sets' => array(
                'label' => _t('Recently created: sets'),
                'description' => _t(
                    'User can use widget that shows recently created items to list new sets in the dashboard.'
                )
            ),
            'can_use_recently_created_widget_ca_collections' => array(
                'label' => _t('Recently created: collections'),
                'description' => _t(
                    'User can use widget that shows recently created items to list new collections in the dashboard.'
                )
            ),
            'can_use_recently_created_widget_ca_object_representations' => array(
                'label' => _t('Recently created: object representations'),
                'description' => _t(
                    'User can use widget that shows recently created items to list new object representations in the dashboard.'
                )
            ),
            'can_use_recently_created_widget_ca_object_lots' => array(
                'label' => _t('Recently created: object lots'),
                'description' => _t(
                    'User can use widget that shows recently created items to list new object lots in the dashboard.'
                )
            )
        );
    }
