...
class recordsByStatusWidget extends BaseWidget implements IWidget
{
    ...
    static public $s_widget_settings = array();
    ...
    public function __construct($ps_widget_path, $pa_settings)
    {
        ...
        foreach ($this->opa_table_display_names as $vs_table => $vs_display) {
            if (!$this->getRequest() || !$this->getRequest()->user->canDoAction(
                    "can_use_records_by_status_widget_{$vs_table}"
                )) {
    ...
    public function renderWidget($ps_widget_id, &$pa_settings)
    {
        ...
        if (!in_array(
            ...
            BaseWidget::$s_widget_settings['recordsByStatusWidget']["display_type"]["options"]
        )) {
            return "";
        }
        ...
        if ($t_table = Datamodel::getInstanceByTableName($pa_settings['display_type'], true)) {
            ...
            $this->opo_view->setVar(
                'status_display',
                $this->opa_status_display_names[intval($pa_settings["display_status"])]
            );
    ...
    public static function getRoleActionList()
    {
        return array(
            'can_use_records_by_status_widget_ca_objects' => array(
                ...
                'description' => _t(
                    'User can use widget that shows records by workflow status to list new objects in the dashboard.'
                )
            ),
            'can_use_records_by_status_widget_ca_entities' => array(
                'label' => _t('Workflow status: entities'),
                'description' => _t(
                    'User can use widget that shows records by workflow status to list new entities in the dashboard.'
                )
            ),
            'can_use_records_by_status_widget_ca_places' => array(
                'label' => _t('Workflow status: places'),
                'description' => _t(
                    'User can use widget that shows records by workflow status to list new places in the dashboard.'
                )
            ),
            'can_use_records_by_status_widget_ca_occurrences' => array(
                'label' => _t('Workflow status: occurrences'),
                'description' => _t(
                    'User can use widget that shows records by workflow status to list new occurrences in the dashboard.'
                )
            ),
            'can_use_records_by_status_widget_ca_sets' => array(
                'label' => _t('Workflow status: sets'),
                'description' => _t(
                    'User can use widget that shows records by workflow status to list new sets in the dashboard.'
                )
            ),
            'can_use_records_by_status_widget_ca_collections' => array(
                'label' => _t('Workflow status: collections'),
                'description' => _t(
                    'User can use widget that shows records by workflow status to list new collections in the dashboard.'
                )
            ),
            'can_use_records_by_status_widget_ca_object_representations' => array(
                'label' => _t('Workflow status: object representations'),
                'description' => _t(
                    'User can use widget that shows records by workflow status to list new object representations in the dashboard.'
                )
            ),
            'can_use_records_by_status_widget_ca_object_lots' => array(
                'label' => _t('Workflow status: object lots'),
                'description' => _t(
                    'User can use widget that shows records by workflow status to list new object lots in the dashboard.'
                )
            )
        );
    }
