...
class recordsByStatusWidget extends BaseWidget implements IWidget {
	...
	public function __construct( $ps_widget_path, $pa_settings ) {
		...
		foreach ( $this->opa_table_display_names as $vs_table => $vs_display ) {
			if ( ! $this->getRequest()
			     || ! $this->getRequest()->user->canDoAction( "can_use_records_by_status_widget_{$vs_table}" )
			) {
				if ( is_array( BaseWidget::$s_widget_settings['recordsByStatusWidget']["display_type"]["options"] ) ) {
					foreach (
						BaseWidget::$s_widget_settings['recordsByStatusWidget']["display_type"]["options"] as
	...
	public function renderWidget( $ps_widget_id, &$pa_settings ) {
		...
		if ( $t_table = Datamodel::getInstanceByTableName( $pa_settings['display_type'], true ) ) {
			...
			while ( $qr_records->nextRow() && sizeof( $va_item_list ) < intval( $pa_settings["display_limit"] ) ) {
				...
				if ( ! ( $vs_label = $qr_records->get( $t_table->getLabelTableName() . "."
			...
			$this->opo_view->setVar( 'status_display',
