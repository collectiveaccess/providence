...
$va_log_entries = array_reverse( $t_log->getRecentChanges( $vn_table_num, $vn_threshold_in_seconds,
	1000 ) ); // reverse to put most recent up top
?>
<div class="dashboardWidgetContentContainer">
	<?php
	if ( sizeof( $va_log_entries ) > 0 ) {
		?>
		<div class="dashboardWidgetHeading"><?php print _t( "Changes to <strong>%1</strong> from %2",
				$this->getVar( 'table_name_plural' ), $vn_displayed_date_range ); ?></div>
		<div class="dashboardWidgetScrollMedium" style="padding-left:10px;">
			<ul>
				...
				foreach ( $va_log_entries as $vs_log_key => $va_log_entry ) {
					...
					//
					print "<li><span style='font-size:12px; font-weight:bold;'>" . $va_log_entry[0]['user_fullname']
					      . " " . _t( 'edited' ) . " <a href='" . caEditorUrl( $po_request,
							$va_log_entry[0]['subject_table_num'], $va_log_entry[0]['subject_id'] ) . "'>"
					      . $va_log_entry[0]['subject'] . "</a></span><br/>\n";
					print $va_log_entry[0]['datetime'] . '<br/>';
					print "<a href='#' id='more" . $vs_log_key . "' onclick='jQuery(\"#more" . $vs_log_key
					      . "\").hide(); jQuery(\"#changes" . $vs_log_key
					      . "\").slideDown(250); return false;'>More Info &rsaquo;</a>";
					print "<div style='display:none;' id='changes" . $vs_log_key
					      . "'><ul>";                    // date/time of change, ready for display (don't use date() on it)
					...
					print "</ul>";
					print "<a href='#' id='hide" . $vs_log_key
					      . "' style='padding-left:10px;' onclick='jQuery(\"#changes" . $vs_log_key
					      . "\").slideUp(250); jQuery(\"#more" . $vs_log_key
					      . "\").show(); return false;'>Hide &rsaquo;</a>";
		...
		print "<div class='dashboardWidgetHeading'>" . _t( "There have been no changes to <strong>%1</strong> from %2.",
				$this->getVar( 'table_name_plural' ), $vn_displayed_date_range ) . "</div>";
	}
	?>
</div>
