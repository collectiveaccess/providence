...
$va_log_entries = array_reverse(
    $t_log->getRecentChanges($vn_table_num, $vn_threshold_in_seconds, 1000)
...
<div class="dashboardWidgetContentContainer">
    ...
    if (sizeof($va_log_entries) > 0) {
        ...
        <div class="dashboardWidgetHeading"><?php print _t(
                "Changes to <strong>%1</strong> from %2",
                $this->getVar('table_name_plural'),
                $vn_displayed_date_range
            ); ?></div>
        <div class="dashboardWidgetScrollMedium" style="padding-left:10px;">
            <ul>
                ...
                foreach ($va_log_entries as $vs_log_key => $va_log_entry) {
                    ...
                    //
                    print "<li><span style='font-size:12px; font-weight:bold;'>" . $va_log_entry[0]['user_fullname'] . " " . _t(
                            'edited'
                        ) . " <a href='" . caEditorUrl(
                            $po_request,
        ...
        print "<div class='dashboardWidgetHeading'>" . _t(
                "There have been no changes to <strong>%1</strong> from %2.",
                $this->getVar('table_name_plural'),
                $vn_displayed_date_range
            ) . "</div>";
    }
