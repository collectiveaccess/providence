...
if (!$vn_form_id) {
    ...
    ?>
    <div>
        <?php print _t("You must define a search form before you can use the advanced search.") . '<br/>' . caNavLink(
                $this->request,
                '<strong>' . _t('Click here to create a new form.') . '</strong>',
                '',
                'manage',
                'SearchForm',
                'ListForms'
    ...
    if (!$t_form->haveAccessToForm($this->request->getUserID(), __CA_SEARCH_FORM_READ_ACCESS__, $vn_form_id)) {
        ...
        print "<div class='dashboardWidgetHeading'>" . caUcFirstUTF8Safe(
                Datamodel::getTableProperty($t_form->get('table_num'), 'NAME_PLURAL')
        ...
        <?php print caFormTag(
            $this->request,
            'Index',
            "AdvancedSearchForm_{$vs_widget_id}",
            "find/{$vs_controller_name}",
            'post',
            'multipart/form-data',
            '_top',
        ...
        <?php
        print "<div style='float: right;'>" . caFormSearchButton(
                $this->request,
