...
if ( ! $vn_form_id ) {
	...
	?>
	<div>
		<?php print _t( "You must define a search form before you can use the advanced search." ) . '<br/>'
		            . caNavLink( $this->request, '<strong>' . _t( 'Click here to create a new form.' ) . '</strong>',
				'', 'manage', 'SearchForm', 'ListForms' ); ?>
	</div>
	<?php
} else {
	if ( ! $t_form->haveAccessToForm( $this->request->getUserID(), __CA_SEARCH_FORM_READ_ACCESS__, $vn_form_id ) ) {
		...
		print "<div class='dashboardWidgetHeading'>"
		      . caUcFirstUTF8Safe( Datamodel::getTableProperty( $t_form->get( 'table_num' ), 'NAME_PLURAL' ) ) . ": "
		      . $t_form->getLabelForDisplay() . "</div>\n";
		...
		?>
		<?php print caFormTag( $this->request, 'Index', "AdvancedSearchForm_{$vs_widget_id}",
			"find/{$vs_controller_name}", 'post', 'multipart/form-data', '_top',
			array( 'noCSRFToken' => true, 'disableUnsavedChangesWarning' => true ) ); ?>
		<?php
		print "<div style='float: right;'>" . caFormSearchButton( $this->request, __CA_NAV_ICON_SEARCH__,
				_t( "Search" ), "AdvancedSearchForm_{$vs_widget_id}" ) . '<br/>' .
		      caJSButton( $this->request, __CA_NAV_ICON_CANCEL__, _t( "Reset" ), "AdvancedSearchForm_{$vs_widget_id}",
			      array( 'onclick' => 'caAdvancedSearchFormReset()' ) ) . "</div>\n";
