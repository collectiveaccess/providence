...
class advancedSearchFormWidget extends BaseWidget implements IWidget {
	...
	public function __construct( $ps_widget_path, $pa_settings ) {
		...
		$va_forms = caExtractValuesByUserLocale( $t_form->getForms( array(
		...
		foreach ( $va_forms as $va_form ) {
			$va_form_list[ caUcFirstUTF8Safe( Datamodel::getTableProperty( $va_form['table_num'], 'NAME_PLURAL' ) )
			               . ": " . $va_form["name"] ]
				= $va_form["form_id"];
		}

		BaseWidget::$s_widget_settings['advancedSearchFormWidget']["form_code"] = array(
			'formatType'  => FT_TEXT,
	...
	public function renderWidget( $ps_widget_id, &$pa_settings ) {
		...
		if ( $t_form->haveAccessToForm( $this->getRequest()->user->getUserID(), __CA_SEARCH_FORM_READ_ACCESS__ ) ) {
			$vo_result_context = new ResultContext( $this->getRequest(),
				Datamodel::getTableName( $t_form->get( "table_num" ) ), "advanced_search" );
			$va_form_data      = $vo_result_context->getParameter( 'form_data' );

			$this->opo_view->setVar( "controller_name",
				$this->getAdvancedSearchControllerNameForTable( Datamodel::getTableName( $t_form->get( "table_num" ) ) ) );
			$this->opo_view->setVar( 'form_data', $va_form_data );
			$this->opo_view->setVar( 'form_elements',
				$t_form->getHTMLFormElements( $this->getRequest(), $va_form_data ) );
		} else {
			$t_form->clear();
		}
