...
class advancedSearchFormWidget extends BaseWidget implements IWidget
{
    ...
    public function __construct($ps_widget_path, $pa_settings)
    {
        ...
        $va_forms = caExtractValuesByUserLocale(
            $t_form->getForms(
                array('user_id' => $this->request->getUserID(), 'access' => __CA_SEARCH_FORM_READ_ACCESS__)
            )
        );

        $va_form_list = array();
        foreach ($va_forms as $va_form) {
            $va_form_list[caUcFirstUTF8Safe(
                Datamodel::getTableProperty($va_form['table_num'], 'NAME_PLURAL')
            ) . ": " . $va_form["name"]] = $va_form["form_id"];
        }
    ...
    public function renderWidget($ps_widget_id, &$pa_settings)
    {
        ...
        if ($t_form->haveAccessToForm($this->getRequest()->user->getUserID(), __CA_SEARCH_FORM_READ_ACCESS__)) {
            $vo_result_context = new ResultContext(
                $this->getRequest(),
                Datamodel::getTableName($t_form->get("table_num")),
                "advanced_search"
            );
            ...
            $this->opo_view->setVar(
                "controller_name",
                $this->getAdvancedSearchControllerNameForTable(
                    Datamodel::getTableName($t_form->get("table_num"))
                )
            );
    ...
    static public function getRoleActionList()
    {
        return array(
            'can_use_adv_search_form_widget' => array(
                ...
                'description' => _t(
                    'User can use widget that allows usage of an advanced search form in the dashboard.'
                )
            )
        );
    }
...
BaseWidget::$s_widget_settings['advancedSearchFormWidget'] = array(
    'form_width' => array(
        ...
        'description' => _t(
            'The number of form elements to place in each row of the search form. A value of one or two is recommended in most situations.'
        )
    )
);
