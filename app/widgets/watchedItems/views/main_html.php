        ...
        <form id="watchedItemsForm"><input type="hidden" name="mode" value="list">
            ...
            foreach ($va_watched_items as $va_item) {
                ...
                if ($va_item["primary_key"]) {
                    print "<span style='font-size:12px; font-weight:bold;'>" . caEditorLink(
                            $po_request,
                            $vs_idno . $va_item["displayName"],
                            '',
                            $va_item["table_name"],
                            $va_item["row_id"]
                        ) . "</span>";
                    print "<br/>&nbsp;&nbsp;&nbsp;<a href='#' id='moreWatchItem" . $va_item["watch_id"] . "' onclick='jQuery(\"#moreWatchItem" . $va_item["watch_id"] . "\").hide(); jQuery(\"#watchItem" . $va_item["watch_id"] . "\").slideDown(250); return false;'>" . _t(
                            "Recent Changes"
                        ) . " &rsaquo;</a>";
                    print "<div style='display:none; margin-right:5px;' id='watchItem" . $va_item["watch_id"] . "'>";
                    print $va_item["change_log"];
                    print "<a href='#' id='hideWatchItem" . $va_item["watch_id"] . "' style='padding-left:10px;' onclick='jQuery(\"#watchItem" . $va_item["watch_id"] . "\").slideUp(250); jQuery(\"#moreWatchItem" . $va_item["watch_id"] . "\").show(); return false;'>" . _t(
                            'Hide'
                        ) . " &rsaquo;</a>";
                    print "</div>";
                } else {
                    print _t(
                        "Item was deleted - row_id: %1; table number: %2",
                        $va_item["row_id"],
                        $va_item["table_num"]
                    );
                }
            ...
            <div style="padding-top:10px; text-align:center; padding-right:20px;">
                <a href='#' onclick='jQuery("#watchedItemsForm").attr("action", "<?php print caNavUrl(
                    $po_request,
                    'manage',
                    'WatchedItems',
                    'Delete'
