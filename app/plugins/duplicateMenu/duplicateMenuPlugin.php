...
class duplicateMenuPlugin extends BaseApplicationPlugin {
	...
	public function __construct( $ps_plugin_path ) {
		$this->description
			= _t( 'Adds a "duplicate" menu listing all recently duplicated items and providing an easy way to create additional duplicates.' );
		parent::__construct();
	}
	# -------------------------------------------------------
	...
	public function hookDuplicateItem( $pa_params ) {
		if ( ( $pa_params['id'] > 0 ) && ( $o_req = $this->getRequest() ) ) {
			if ( ! is_array(
				$va_activity_list = Session::getVar( $pa_params['table_name'] . '_duplicate_id_list' ) )
			) {
			...
			if ( ( ! isset( $va_activity_list[ $pa_params['id'] ] ) )
			     && ( sizeof( $va_activity_list ) >= $vn_max_num_items_in_activity_menu )
			) {
				$va_activity_list = array_slice( $va_activity_list,
					( sizeof( $va_activity_list ) - $vn_max_num_items_in_activity_menu - 1 ),
					$vn_max_num_items_in_activity_menu - 1, true );
			}
			...
			foreach (
				...
				if ( $vs_table_name === 'ca_occurrences' ) {
					...
					foreach ( $va_types as $vn_type_id => $va_type_info ) {
						$va_activity_menu_list = array();
						if ( isset( $va_sorted_by_type_id[ $vn_type_id ] )
							...
							foreach ( $va_sorted_by_type_id[ $vn_type_id ] as $vn_id => $va_info ) {
								$va_activity_menu_list[ $vs_table_name . '_' . $vn_type_id . '_' . $vn_id ] = array(
									...
									'displayName' => htmlspecialchars( $va_labels[ $vn_id ], ENT_QUOTES, 'UTF-8' )
									                 . ( ( trim( $va_info['idno'] ) ) ? ' [' . $va_info['idno'] . ']'
											: '' ),
									'is_enabled'  => 1,
									'requires'    => array(
						...
						if ( sizeof( $va_activity_menu_list ) > 0 ) {
							$va_activity_lists[ $vs_table_name . '_' . $vn_type_id ] = array(
								'displayName' => mb_strtolower( caUcFirstUTF8Safe( $va_type_info['name_singular'] ),
									'UTF-8' ),
								'submenu'     => array(
									"type"       => 'static',
					...
					foreach ( $va_keys as $vn_id ) {
						$va_info                                                = $va_activity_list[ $vn_id ];
						$va_editor_url_info                                     = caEditorUrl( $o_req, $vs_table_name,
							null, true );
					...
					$va_activity_lists[ $vs_table_name ] = array(
						'displayName' => mb_strtolower( caUcFirstUTF8Safe( $t_instance->getProperty( 'NAME_SINGULAR' ) ),
							'UTF-8' ),
