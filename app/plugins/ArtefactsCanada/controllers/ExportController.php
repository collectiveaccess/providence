...
class ExportController extends ActionController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		if ( ! ( $export_display_code = $this->config->get( 'export_display' ) )
		     || ( ca_bundle_displays::find( [ 'display_code' => $export_display_code ], [ 'returnAs' => 'count' ] )
		          == 0 )
		) {
			throw new ApplicationException( _t( "Artefacts Canada export display '%1' does not exist",
				$export_display_code ) );
		}
		...
		if ( ! $this->request->user->canDoAction( 'can_export_artefacts_canada' ) ) {
			$this->response->setRedirect( $this->request->config->get( 'error_display_url' ) . '/n/3000?r='
		...
		MetaTagManager::addLink( 'stylesheet',
			__CA_URL_ROOT__ . "/app/plugins/ArtefactsCanada/themes/" . __CA_THEME__ . "/css/ArtefactsCanada.css",
			'text/css' );
	}
	# -------------------------------------------------------
	...
	public function Index() {
		...
		$t_set = new ca_sets();
		$this->view->setVar( 'sets_list', $sets = caExtractValuesByUserLocale( $t_set->getSets( [
		...
		foreach ( $sets as $set ) {
			...
			$sets_list[ $set['set_code'] ] = $set['name'] . ' ' . ( ( $set['item_count'] == 1 ) ? _t( '(%1 item)',
					$set['item_count'] ) : _t( '(%1 items)', $set['item_count'] ) );
		}
	...
	public function Run() {
		...
		if ( ! ca_sets::setExists( $set_code,
			[ 'user_id' => $this->request->getUserID(), 'access' => __CA_SET_READ_ACCESS__ ] )
		) {
			throw new ApplicationException( _t( 'Set does not exist' ) );
		}
		$job_id = md5( 'U' . $this->request->getUserID() . "_{$set_code}_" . uniqid( rand(), true ) . '_'
		               . microtime( true ) );
	...
	public function RunExport() {
		...
		if ( ! ( $t_display = ca_bundle_displays::find( [
			'display_code' => $display_code
				= $this->config->get( 'export_display' )
		...
		$headers = array_merge( array_values( array_map( function ( $v ) {
			if ( is_array( $v['settings'] ) && is_array( $v['settings']['label'] )
			     && sizeof( $v['settings']['label'] )
			) {
		...
		while ( $qr->nextHit() ) {
			...
			foreach ( $placements as $placement_id => $placement_info ) {
				$v = preg_replace( "![\r\n\t]+!", " ", iconv( 'UTF-8', 'ISO-8859-1//IGNORE',
					html_entity_decode( strip_tags( $t_display->getDisplayValue( $qr, $placement_id, [
						'convert_codes_to_display_text' => true,
			...
			$media_refs = [];
			if ( is_array(
				$media_list = $qr->get( 'ca_object_representations.media.original.path', [ 'returnAsArray' => true ] ) )
			) {
				foreach ( $media_list as $media_path ) {
					...
					$zip->addFile( $media_path,
						$media_refs[] = "{$id}." . pathinfo( $media_path, PATHINFO_EXTENSION ) );
		...
		$links = [
			caNavLink( $this->request,
				_t( 'Download Artefacts Canada data as ZIP file (%1)', caHumanFilesize( filesize( $new_path ) ) ), '',
				'*', '*', 'Download', [ 'job_id' => $job_id, 'download' => 1 ] )
		];
