...
class ExportController extends ActionController
{
    ...
    public function __construct(&$po_request, &$po_response, $pa_view_paths = null)
    {
        ...
        if (!($export_display_code = $this->config->get('export_display')) || (ca_bundle_displays::find(
                    ['display_code' => $export_display_code],
                    ['returnAs' => 'count']
                ) == 0)) {
            throw new ApplicationException(
                _t("Artefacts Canada export display '%1' does not exist", $export_display_code)
            );
        }
        ...
        if (!$this->request->user->canDoAction('can_export_artefacts_canada')) {
            $this->response->setRedirect(
                $this->request->config->get('error_display_url') . '/n/3000?r=' . urlencode(
    ...
    public function Index()
    {
        ...
        $this->view->setVar(
            'sets_list',
            $sets = caExtractValuesByUserLocale(
                $t_set->getSets(['user_id' => $this->request->getUserID(), 'table' => 'ca_objects'])
            )
        );
        ...
        foreach ($sets as $set) {
            ...
            $sets_list[$set['set_code']] = $set['name'] . ' ' . (($set['item_count'] == 1) ? _t(
                    '(%1 item)',
                    $set['item_count']
                ) : _t('(%1 items)', $set['item_count']));
        }
    ...
    public function Run()
    {
        ...
        $job_id = md5(
            'U' . $this->request->getUserID() . "_{$set_code}_" . uniqid(rand(), true) . '_' . microtime(true)
        );
    ...
    public function RunExport()
    {
        ...
        if (!($t_display = ca_bundle_displays::find(
            ['display_code' => $display_code = $this->config->get('export_display')],
        ...
        $headers = array_merge(
            array_values(
                array_map(
                    function ($v) {
                        if (is_array($v['settings']) && is_array($v['settings']['label']) && sizeof(
                                $v['settings']['label']
                            )) {
