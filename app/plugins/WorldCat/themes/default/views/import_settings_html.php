...
<form action="#" id="caWorldCatSearchForm">
	<div class="formLabel">
		<?php print _t( 'Find in WorldCat' ) . ': ' . caHTMLTextInput( "term",
				array( 'value' => '', 'id' => 'caWorldCatTerm' ), array( 'width' => '250px' ) ); ?>
		<a href="#" id="caWorldCatTermLookup" class="button"><?php print caNavIcon( __CA_NAV_ICON_GO__, "18px" ); ?></a>
	</div>
</form>

<?php
print caFormTag( $this->request, 'Run', 'caWorldCatResultsForm', null, 'post', 'multipart/form-data', '_top',
...
<script type="text/javascript">
	...
	jQuery(document).ready(function () {
		...
		jQuery(document).on('click', '.caWorldCatSearchResultItem', {}, function (e) {
			jQuery(this).parent().find(".caWorldCatSearchResultDetails").slideToggle(250, function (e) {
				...
				var details = this;
				jQuery(details).html("<div class='caWorldCatDetailsMessage'><?php print caBusyIndicatorIcon( $this->request )
				                                                                        . ' '
	...
	function caSearchWorldCat(term, start, c, msg) {
		if (!msg) {
			msg = "<?php print addslashes( _t( 'Searching WorldCat...' ) ); ?>";
		}
		if (start <= 0) {
			start = 0;
		}
		...
		jQuery("#caWorldCatResults").html("<div class='caWorldCatResultsMessage'><?php print caBusyIndicatorIcon( $this->request )
		                                                                                     . ' '; ?>" + msg + "</div>");
