...
<script type="text/javascript">
	jQuery(document).ready(function () {
		...
		jQuery.post('<?php print caNavUrl( $this->request, '*', '*', 'RunImport',
				array() ); ?>', <?php print json_encode( array(
				'importer_id' => $pn_importer_id,
				'job_id' => $ps_job_id,
				'WorldCatID' => $pa_worldcat_ids,
		...
		updateProgressBarInterval = setInterval(function () {
			jQuery.getJSON('<?php print caNavUrl( $this->request, '*', '*', 'GetImportStatus',
				array( 'job_id' => $ps_job_id ) ); ?>', {}, function (data) {
