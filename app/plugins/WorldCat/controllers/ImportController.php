...
class ImportController extends ActionController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		MetaTagManager::addLink( 'stylesheet',
			__CA_URL_ROOT__ . "/app/plugins/WorldCat/themes/" . __CA_THEME__ . "/css/WorldCat.css", 'text/css' );
	}
	# -------------------------------------------------------
	...
	public function Index() {
		...
		$this->view->setVar( 'importer_list',
			$va_importer_list = ca_data_importers::getImporters( null, array( 'formats' => array( 'WorldCat' ) ) ) );
	...
	public function Run() {
		...
		$vs_job_id       = md5( 'U' . $this->request->getUserID() . '_' . $pn_importer_id . '_' . join( ';',
				$pa_worldcat_ids ) . '_' . uniqid( rand(), true ) . '_' . microtime( true ) );
		...
		$this->request->user->setVar( 'worldcat_log_level',
	...
	public function RunImport() {
		...
		$vn_status  = $t_importer->importDataFromSource( join( ",", $pa_worldcat_ids ), $pn_importer_id,
			array( 'progressBar' => $o_progress, 'format' => 'WorldCat', 'logLevel' => $pn_log_level ) );
