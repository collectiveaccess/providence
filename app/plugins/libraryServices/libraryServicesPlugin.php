...
class libraryServicesPlugin extends BaseApplicationPlugin
{
    ...
    public function hookPeriodicTask(&$pa_params)
    {
        ...
        if ((bool)$this->opo_config->get('enable_object_checkout')) {
            ...
            if (!is_array(
                $va_administrative_email_addresses = $this->opo_library_services_config->getList(
            ...
            if ($this->opo_library_services_config->get(
                    'send_coming_due_notices'
                ) && ($vs_interval = $this->opo_library_services_config->get('coming_due_interval'))) {
                try {
                    $va_items_by_user = ca_object_checkouts::getItemsDueWithin(
                        $vs_interval,
                        array(
                            'groupBy' => 'user_id',
                            'template' => $this->opo_library_services_config->get(
                                'coming_due_item_display_template'
                            ),
                            'notificationInterval' => $this->opo_library_services_config->get(
                                'coming_due_notification_interval'
                            )
                        )
                    );
                        ...
                        if ($t_user->load($vn_user_id)) {
                            if ($vs_user_email = $t_user->get('email')) {
                                ...
                                if (caSendMessageUsingView(
                                    ...
                                    $vs_user_email,
                                    $vs_sender_email,
                                    "[{$vs_app_name}] {$vs_subject}",
                                    "library_coming_due.tpl",
                                    [
                                        'subject' => $vs_subject,
                                    ...
                                    null,
                                    $va_administrative_email_addresses,
                                        ...
                                        if ($t_checkout->load($va_item['checkout_id'])) {
                                            ...
                                            if ($t_checkout->numErrors()) {
                                                $t_log->log(
                                                    array(
                                                        'CODE' => 'ERR',
                                                        'MESSAGE' => _t(
                                                            'Could not mark checkout coming due message sent time because update failed: %1',
                                            ...
                                            $t_log->log(
                                                array(
                                                    'CODE' => 'ERR',
                                                    'MESSAGE' => _t(
                                                        'Could not mark checkout coming due message sent time because checkout id %1 was not found',
                                                        $va_item['checkout_id']
                                                    ),
                                                    'SOURCE' => 'libraryServicesPlugin->hookPeriodicTask'
                                                )
                                ...
                                $t_log->log(
                                    array(
                                        'CODE' => 'ERR',
                                        'MESSAGE' => _t(
                                            'No email address set for user %1 (%2)',
                                            $t_user->get('user_name'),
                                            trim($t_user->get('fname') . ' ' . $t_user->get('lname'))
                                        ),
                                        'SOURCE' => 'libraryServicesPlugin->hookPeriodicTask'
                                    )
                                );
            ...
            if ($this->opo_library_services_config->get('send_overdue_notices')) {
                try {
                    $va_items_by_user = ca_object_checkouts::getOverdueItems(
                        array(
                            'groupBy' => 'user_id',
                                        ...
                                        if ($t_checkout->load($va_item['checkout_id'])) {
                                            ...
                                            if ($t_checkout->numErrors()) {
                                                $t_log->log(
                                                    array(
                                                        'CODE' => 'ERR',
                                                        'MESSAGE' => _t(
                                                            'Could not mark checkout overdue message sent time because update failed: %1',
                                                            join("; ", $t_checkout->getErrors())
                                                        ),
                                                        'SOURCE' => 'libraryServicesPlugin->hookPeriodicTask'
                                                    )
                                            ...
                                            $t_log->log(
                                                array(
                                                    'CODE' => 'ERR',
                                                    'MESSAGE' => _t(
                                                        'Could not mark checkout overdue message sent time because checkout id %1 was not found',
                                                        $va_item['checkout_id']
                                                    ),
                                                    'SOURCE' => 'libraryServicesPlugin->hookPeriodicTask'
                                                )
            ...
            if ($this->opo_library_services_config->get('send_reservation_available_notices')) {
                try {
                    $va_items_by_user = ca_object_checkouts::getReservedAvailableItems(
                        array(
                            'groupBy' => 'user_id',
                            'template' => $this->opo_library_services_config->get('overdue_item_display_template'),
                            'notificationInterval' => $this->opo_library_services_config->get(
                                'reservation_available_notification_interval'
                            )
                        )
                    );
                        ...
                        if ($t_user->load($vn_user_id)) {
                            if ($vs_user_email = $t_user->get('email')) {
                                ...
                                if (caSendMessageUsingView(
                                    ...
                                    $vs_sender_email,
                                    "[{$vs_app_name}] {$vs_subject}",
                                    "library_reservation_available.tpl",
                                    [
                                        'subject' => $vs_subject,
                                        'from_user_id' => $vn_user_id,
                                        'sender_name' => $vs_sender_name,
                                        ...
                                        if ($t_checkout->load($va_item['checkout_id'])) {
                                            ...
                                            if ($t_checkout->numErrors()) {
                                                $t_log->log(
                                                    array(
                                                        'CODE' => 'ERR',
                                                        'MESSAGE' => _t(
                                                            'Could not mark reserved available message sent time because update failed: %1',
                                                            join("; ", $t_checkout->getErrors())
                                                        ),
                                                        'SOURCE' => 'libraryServicesPlugin->hookPeriodicTask'
                                                    )
                                            ...
                                            $t_log->log(
                                                array(
                                                    'CODE' => 'ERR',
                                                    'MESSAGE' => _t(
                                                        'Could not mark reserved available message sent time because checkout id %1 was not found',
                                                        $va_item['checkout_id']
                                                    ),
                                                    'SOURCE' => 'libraryServicesPlugin->hookPeriodicTask'
                                                )
