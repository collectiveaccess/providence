...
class iDigBioExportPlugin extends BaseApplicationPlugin {
	...
	public function hookPeriodicTask() {
		...
		$ve_mainLink = $xml->createElement( "link",
			$this->opo_config->get( 'urlRoot' ) . $this->opo_config->get( 'rssPublicPath' ) . $rssFile . ".xml" );
		$channel->appendChild( $ve_mainLink );
		foreach ( $va_collections as $vs_collection_code => $va_collection ) {
			$vs_file_name = $this->getCSVExport( $va_collection, $vs_collection_code, $rssDirectory,
				$va_collection['recordtype'] );
	...
	private function getCSVExport( $va_collection, $vs_collection_code, $vs_rss_dir, $vs_type ) {
		...
		$va_errors        = [];
		$vo_exporter      = ca_data_exporters::loadExporterFromFile( $vs_exporter_dir . '/' . $vf_exporter,
			$va_errors );
		$vs_exporter_code = $vo_exporter->get( 'exporter_code' );
		ca_data_exporters::exportRecordsFromSearchResult( $vs_exporter_code, $qr_search_result,
			$vs_rss_dir . $vs_file_name, [
				'logLevel'           => KLogger::DEBUG,
				'logDirectory'       => __CA_BASE_DIR__ . '/app/plugins/iDigBioExport/logs',
