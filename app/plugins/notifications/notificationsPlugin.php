...
class notificationsPlugin extends BaseApplicationPlugin
{
    ...
    public function hookPeriodicTask(&$pa_params)
    {
        ...
        if (is_array($va_notifications = ca_users::getQueuedEmailNotifications())) {
            ...
            // digest by user
            $va_notifications_by_user = array_reduce(
                $va_notifications,
                function ($c, $i) {
                    $c[$i['user_id']][] = $i;
                    return $c;
            ...
            foreach ($va_notifications_by_user as $vn_user_id => $va_notifications_for_user) {
                ...
                if (caSendMessageUsingView(
                    ...
                    $vs_to_email,
                    $vs_sender_email,
                    $this->opo_config->get('notification_email_subject'),
                    "notification_digest.tpl",
                    ['notifications' => $va_notifications_for_user, 'sent_on' => time()],
                    null,
                    null,
                    ['source' => 'Notification']
                )) {
                    $va_notification_subject_ids = array_map(
                        function ($v) {
                            return $v['subject_id'];
                        },
                            ...
                            if (!$t_subject->update()) {
                                $t_log->log(
                                    array(
                                        'CODE' => 'ERR',
                                        'MESSAGE' => _t(
                                            'Could not set notification subject %1 as read: %2',
                                            $vn_subject_id,
