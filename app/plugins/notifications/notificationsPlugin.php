		...
		if ( is_array( $va_notifications = ca_users::getQueuedEmailNotifications() ) ) {
			...
			foreach ( $va_notifications_by_user as $vn_user_id => $va_notifications_for_user ) {
				...
				if ( caSendMessageUsingView( null, $vs_to_email, $vs_sender_email,
					$this->opo_config->get( 'notification_email_subject' ), "notification_digest.tpl",
					[ 'notifications' => $va_notifications_for_user, 'sent_on' => time() ], null, null,
					[ 'source' => 'Notification' ] )
				) {
					...
					foreach ( $va_notification_subject_ids as $vn_subject_id ) {
						if ( $t_subject->load( $vn_subject_id ) ) {
							...
							if ( ! $t_subject->update() ) {
								$t_log->log( array( 'CODE'    => 'ERR',
								                    'MESSAGE' => _t( 'Could not set notification subject %1 as read: %2',
									                    $vn_subject_id, join( '; ', $t_subject->getErrors() ) ),
								                    'SOURCE'  => 'notificationsPlugin->hookPeriodicTask'
								) );
								continue;
							}
						}
