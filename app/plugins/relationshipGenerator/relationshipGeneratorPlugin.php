...
class relationshipGeneratorPlugin extends BaseApplicationPlugin
{
    ...
    public function __construct($ps_plugin_path)
    {
        parent::__construct();
        $this->description = _t(
            'Automatically assigns an object to a collection, based upon rules you specify in the configuration file associated with the plugin'
        );
        $this->opo_config = Configuration::load(
            $ps_plugin_path . DIRECTORY_SEPARATOR . 'conf' . DIRECTORY_SEPARATOR . 'relationshipGenerator.conf'
        );
    }
    ...
    protected function _process(&$pa_params)
    {
        ...
        foreach ($this->opo_config->getAssoc('rules') as $va_rule) {
            ...
            $vo_related_model = new $vs_related_table(
                is_string($vm_related_record) && !is_numeric(
                    $vm_related_record
            ...
            if (sizeof($vo_related_model->getFieldValuesArray()) > 0) {
                ...
                $vn_relationship_id = self::_getRelationshipId(
                    $pa_params['instance'],
                    $vs_related_table,
                    $vm_related_record,
                ...
                if ($vb_add_matched && $vb_matches && is_null($vn_relationship_id)) {
                    ...
                    if ($this->opo_config->getBoolean('notify')) {
                        $this->_notifications()->addNotification(
                            _t(
                                isset($va_rule['add_relationship_notification']) ? $va_rule['add_relationship_notification'] : $this->opo_config->get(
                                    'default_add_relationship_notification'
                                ),
                ...
                if ($vb_remove_unmatched && !$vb_matches && !is_null($vn_relationship_id)) {
                    ...
                    if ($this->opo_config->getBoolean('notify')) {
                        $this->_notifications()->addNotification(
                            _t(
                                isset($va_rule['remove_relationship_notification']) ? $va_rule['remove_relationship_notification'] : $this->opo_config->get(
                                    'default_remove_relationship_notification'
                                ),
                                $vo_related_model->getTypeName(),
                                $vo_related_model->getListName()
                            ),
    ...
    protected function _hasMatch($pa_params, $pa_rule)
    {
        ...
        $vs_field_combination_operator = self::_getOperatorMethodName(
            isset($pa_rule['field_combination_operator']) ? $pa_rule['field_combination_operator'] : $this->opo_config->get(
                'default_field_combination_operator'
            )
        );
        $vs_default_value_combination_operator = self::_getOperatorMethodName(
            isset($pa_rule['value_combination_operator']) ? $pa_rule['value_combination_operator'] : $this->opo_config->get(
                'default_value_combination_operator'
            )
        );
        $vs_default_match_type = self::_getMatchTypeMethodName(
            isset($pa_rule['match_type']) ? $pa_rule['match_type'] : $this->opo_config->get('default_match_type')
        );
        $va_default_match_options = isset($pa_rule['match_options']) ? $pa_rule['match_options'] : $this->opo_config->get(
        ...
        foreach ($pa_rule['triggers'] as $vs_field => $va_trigger) {
            ...
            $vs_value_combination_operator = isset($va_trigger['value_combination_operator']) ? self::_getOperatorMethodName(
                $va_trigger['value_combination_operator']
            ) : $vs_default_value_combination_operator;
            $vs_match_type = isset($va_trigger['match_type']) ? self::_getMatchTypeMethodName(
                $va_trigger['match_type']
            ) : $vs_default_match_type;
            $vs_value_converter = isset($va_trigger['value_converter']) && function_exists(
                $va_trigger['value_converter']
    ...
    protected static function _getRelationshipId(
        ...
        $va_items = $po_instance->getRelatedItems(
            $ps_related_table,
            array(
    ...
    private function _testConfigurationSection($ps_section, $pa_requirements, $pf_get_value_callback, &$pa_errors)
    {
        foreach ($pa_requirements as $key => $requirement) {
            ...
            if (isset($requirement['type']) && is_string($requirement['type']) && !is_null($value) && gettype(
                    $value
                ) !== $requirement['type']) {
                $pa_errors[] = _t(
                    'Configuration item `%1` in %2 has incorrect type %3, expected %4',
                    $key,
                    $ps_section,
                    gettype($value),
                    $requirement['type']
            ...
            if (isset($requirement['call']) && is_string($requirement['call']) && !is_null($value)) {
                ...
                if (!method_exists($this, $this->$call($value))) {
                    $pa_errors[] = _t(
                        'Configuration item `%1` in %2 has value "%3", which is an invalid %4',
                        $key,
                        $ps_section,
                        $value,
                        $requirement['call']
