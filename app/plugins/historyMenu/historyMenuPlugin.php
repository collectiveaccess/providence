...
class historyMenuPlugin extends BaseApplicationPlugin
{
    ...
    public function __construct($ps_plugin_path)
    {
        ...
        $this->opo_config = Configuration::load(
            $ps_plugin_path . DIRECTORY_SEPARATOR . 'conf' . DIRECTORY_SEPARATOR . 'historyMenu.conf'
        );
    ...
    public function hookEditItem($pa_params)
    {
        if (($pa_params['id'] > 0) && ($o_req = $this->getRequest())) {
            ...
            if ((!isset($va_activity_list[$pa_params['id']])) && (sizeof(
                        $va_activity_list
                    ) >= $vn_max_num_items_in_activity_menu)) {
                $va_activity_list = array_slice(
                    $va_activity_list,
                    (sizeof($va_activity_list) - $vn_max_num_items_in_activity_menu - 1),
                    $vn_max_num_items_in_activity_menu - 1,
                    true
                );
            }
            ...
            if (!($display_template = $app_config->get("{$table_name}_history_menu_display_template"))) {
                $display_template = ($this->opo_config instanceof Configuration) ? $this->opo_config->get(
                    "{$table_name}_display_template"
                ) : null;
            }
            ...
            $va_activity_list[$pa_params['id']] = array(
                ...
                'label' => $display_template ? $pa_params['instance']->getWithTemplate(
                    $display_template
                ) : $pa_params['instance']->get("{$table_name}.preferred_labels") . ((trim($idno)) ? " [{$idno}]" : '')
            );
