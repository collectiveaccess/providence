	...
	public function hookEditItem( $pa_params ) {
		if ( ( $pa_params['id'] > 0 ) && ( $o_req = $this->getRequest() ) ) {
			...
			if ( ! ( $display_template = $app_config->get( "{$table_name}_history_menu_display_template" ) ) ) {
				$display_template = ( $this->opo_config instanceof Configuration )
					? $this->opo_config->get( "{$table_name}_display_template" ) : null;
			}
			...
			$va_activity_list[ $pa_params['id'] ] = array(
				...
				'label'   => $display_template
					? $pa_params['instance']->getWithTemplate( $display_template )
					: $pa_params['instance']->get( "{$table_name}.preferred_labels" ) . ( ( trim( $idno ) )
						? " [{$idno}]" : '' )
			);
