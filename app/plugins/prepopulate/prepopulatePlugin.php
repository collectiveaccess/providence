...
class prepopulatePlugin extends BaseApplicationPlugin
{
    ...
    public function prepopulateFields(&$t_instance, $pa_options = null)
    {
        ...
        if (!($this->opo_plugin_config->get('prepopulate_fields_on_save') || $this->opo_plugin_config->get(
                'prepopulate_fields_on_load'
            ))) {
            return false;
        }
                                ...
                                case 'addifempty':
                                    ...
                                    if (sizeof($va_labels)) {
                                        ...
                                        if (strtolower($vs_mode) == 'overwrite') {
                                            ...
                                            if (strlen(
                                                    trim($va_label[$va_parts[2]])
                                                ) == 0) { // in addifempty mode only edit label when field is not set
                                                $va_label[$va_parts[2]] = $vs_value;
                                                $vb_update = true;
                                            }
                                        }
