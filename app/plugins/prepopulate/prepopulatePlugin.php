...
class prepopulatePlugin extends BaseApplicationPlugin {
	...
	public function __construct( $ps_plugin_path ) {
		$this->description
			= _t( 'This plugin allows prepopulating field values based on display templates. See http://docs.collectiveaccess.org/wiki/Prepopulate for more info.' );
		parent::__construct();

		$this->opo_plugin_config = Configuration::load( $ps_plugin_path . DIRECTORY_SEPARATOR . 'conf'
		                                                . DIRECTORY_SEPARATOR . 'prepopulate.conf' );
	}
	# --------------------------------------------------------------------------------------------
	 ...
	 *                                                              prepopulateConfig = override path to
	 *                                                              prepopulate.conf, e.g. for testing purposes hook =
	 *                                                              indicates what triggered application: "save" or
	 *                                                              "edit"
	 *
		...
		foreach ( $va_rules as $vs_rule_key => $va_rule ) {
			...
			if ( ! $vb_is_relationship_rule ) {
				...
				if ( ( strlen( $vs_template ) < 1 )
				     && ( strlen( $vs_source = caGetOption( 'source', $va_rule, null ) ) < 1 )
				) {
			...
			if ( $va_rule['restrictToTypes'] && is_array( $va_rule['restrictToTypes'] )
			     && ( sizeof( $va_rule['restrictToTypes'] ) > 0 )
			) {
				if ( ! in_array( $t_instance->getTypeCode(), $va_rule['restrictToTypes'] ) ) {
					Debug::msg( "[prepopulateFields()] skipping rule $vs_rule_key because current record type "
					            . $t_instance->getTypeCode() . " is not in restrictToTypes" );
					continue;
				}
			}
			...
			if ( $va_rule['skipIfExpression'] && ( strlen( $va_rule['skipIfExpression'] ) > 0 ) ) {
				...
				foreach ( $va_tags as $vs_tag ) {
					if ( ! isset( $va_expression_vars[ $vs_tag ] ) ) {
						$va_expression_vars[ $vs_tag ] = $t_instance->get( $vs_tag,
							array( 'returnIdno' => true, 'delimiter' => ';' ) );
					}
				}
			...
			if ( ! $vb_is_relationship_rule ) {
				// evaluate template
				$vs_value = caProcessTemplateForIDs( $vs_template, $t_instance->tableNum(),
					array( $t_instance->getPrimaryKey() ), array( 'path' => true ) );
			...
			if ( ( sizeof( $va_parts ) == 1 ) && $vb_is_relationship_rule ) {    // clone relationships
				...
				switch ( $vs_context ) {
					case 'parent':
						...
						if ( ( $vn_parent_id
								= $t_instance->get( $t_instance->getProperty( 'HIERARCHY_PARENT_ID_FLD' ) ) )
						     && $t_parent->load( $vn_parent_id )
						) {

							$va_rels = $t_parent->getRelatedItems( $vs_target,
								[ 'showCurrentOnly' => caGetOption( 'currentOnly', $va_rule, false ) ] );
						}
						break;
					case 'children':
						if ( $t_instance->getPrimaryKey() ) {
							$va_child_ids = $t_instance->getHierarchy( $t_instance->getPrimaryKey(),
								[ 'idsOnly' => true ] );
							if ( is_array( $va_child_ids ) && sizeof( $va_child_ids ) ) {
								$va_rels = $t_parent->getRelatedItems( $vs_target, [
									'row_ids'         => $va_child_ids,
									'showCurrentOnly' => caGetOption( 'currentOnly', $va_rule, false )
								] );
							}
						}
					...
					case 'related':
						if ( $t_instance->getPrimaryKey() ) {
							$va_rel_ids = $t_instance->get( $t_instance->tableName() . '.related.'
							                                . $t_instance->primaryKey(),
								[ 'idsOnly' => true, 'returnAsArray' => true ] );
				...
				if ( is_array( $va_rels ) ) {
					foreach ( $va_rels as $va_rel ) {
						if ( is_array( $va_restrict_to_relationship_types )
						     && sizeof( $va_restrict_to_relationship_types )
						     && ! in_array( $va_rel['relationship_type_code'], $va_restrict_to_relationship_types )
						) {
							continue;
						}
						if ( is_array( $va_exclude_relationship_types ) && sizeof( $va_exclude_relationship_types )
						     && in_array( $va_rel['relationship_type_code'], $va_exclude_relationship_types )
						) {
							continue;
						}

						$va_related_types = caMakeTypeList( $vs_target, [ $va_rel['item_type_id'] ] );
						if ( is_array( $va_restrict_to_related_types ) && sizeof( $va_restrict_to_related_types )
						     && sizeof( array_intersect( $va_related_types, $va_restrict_to_related_types ) )
						) {
							continue;
						}
						if ( is_array( $va_exclude_related_types ) && sizeof( $va_exclude_related_types )
						     && ! sizeof( array_intersect( $va_related_types, $va_exclude_related_types ) )
						) {
							continue;
						}

						$vn_target_id = $va_rel[ Datamodel::primaryKey( $vs_target ) ];
						if ( ! ( $va_existing_rel_ids = $t_instance->relationshipExists( $vs_target, $vn_target_id,
							$va_rel['relationship_type_code'], $va_rel['effective_date'] ) )
						) {
							if ( $t = $t_instance->addRelationship( $vs_target, $vn_target_id,
								$va_rel['relationship_type_code'], $va_rel['effective_date'] )
							) {
						...
						if ( is_array( $va_instance_rels = $t_instance->getRelatedItems( $vs_target ) ) ) {
							foreach ( $va_instance_rels as $va_instance_rel ) {
								if ( ! in_array( $va_instance_rel['relation_id'], $va_instance_rel_ids ) ) {
									if ( ! $t_instance->removeRelationship( $vs_target,
										$va_instance_rel['relation_id'] )
									) {
...
// attribute/element
					...
					if ( ( $datatype == 0 )
					     && $vs_source
					) { // full container clone using "source" rather than template
						if ( is_array(
							$source_values = $t_instance->get( $vs_source, [ 'returnWithStructure' => true ] ) )
						) {
							...
							foreach ( $va_attributes as $attr ) {
								foreach ( $attr->getValues() as $v ) {
									if ( is_array( $va_source_map ) && sizeof( $va_source_map )
									     && ! array_key_exists( $v->getElementCode(), $va_source_map )
									) {
										continue;
									}
							...
							if ( $t_instance->numErrors() ) {
								Debug::msg( _t( "[prepopulateFields()] error while removing old values during copy of containers: %1",
							...
							foreach ( array_shift( $source_values ) as $attr_id => $attr ) {
								...
								if ( $t_instance->numErrors() ) {
									Debug::msg( _t( "[prepopulateFields()] error during copy of containers: %1",
										join( "; ", $t_instance->getErrors() ) ) );
								}
								$i ++;
							}
							...
							switch ( $vs_mode ) {
								case 'overwrite':
									...
									foreach ( $vo_attr->getValues() as $o_val ) {
										if ( $o_val->getElementCode() != $va_parts[2] ) {
											$va_value[ $o_val->getElementCode() ]
												       = $v = $o_val->getDisplayValue( [ 'idsOnly' => true ] );
											$vb_is_set = true;
										}
									}
									if ( ( $vs_mode === 'overwrite' ) || $vb_is_set ) {
										$t_instance->_editAttribute( $vo_attr->getAttributeID(), $va_value,
											$t_instance->getTransaction() );
									}
									break;
								case 'overwriteifset':
									$vo_attr  = array_pop( $va_attr );
								...
								case 'addifempty':
									...
									foreach ( $vo_attr->getValues() as $o_val ) {
										if ( $o_val->getElementCode() != $va_parts[2] ) {
											$va_value[ $o_val->getElementCode() ]
												= $o_val->getDisplayValue( [ 'idsOnly' => true ] );
									...
									if ( $vb_update ) {
										$t_instance->editAttribute( $vo_attr->getAttributeID(), $va_parts[1],
											$va_value );
									}
									break;
								default:
...
// labels
					...
					if ( ! ( $t_label = Datamodel::getInstanceByTableName( $t_instance->getLabelTableName(),
						true ) )
					) {
						continue;
					}
					...
					switch ( $t_instance->getLabelCount( $vb_preferred ) ) {
						...
						case 1:
							switch ( $vs_mode ) {
								...
								case 'overwriteifset':
								case 'addifempty':
									$va_labels = caExtractValuesByUserLocale( $t_instance->getLabels( null,
										$vb_preferred ? __CA_LABEL_TYPE_PREFERRED__
									...
									if ( sizeof( $va_labels ) ) {
										...
										$is_blank  = ( isset( $va_label[ $label_fld ] )
										               && ( $va_label[ $label_fld ] == '[' . _t( 'BLANK' ) . ']' ) );
