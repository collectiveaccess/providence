...
class prepopulatePlugin extends BaseApplicationPlugin
{
    ...
    {
        $this->description = _t(
            'This plugin allows prepopulating field values based on display templates. See http://docs.collectiveaccess.org/wiki/Prepopulate for more info.'
        );
        parent::__construct();

        $this->opo_plugin_config = Configuration::load(
            $ps_plugin_path . DIRECTORY_SEPARATOR . 'conf' . DIRECTORY_SEPARATOR . 'prepopulate.conf'
        );
    }
    # --------------------------------------------------------------------------------------------
    ...
    public function prepopulateFields(&$t_instance, $pa_options = null)
    {
        ...
        foreach ($va_rules as $vs_rule_key => $va_rule) {
            ...
            $useFor = is_array($useFor) ? array_map(
                function ($v) {
                    return strtolower($v);
                },
                $useFor
            ...
            if ($va_rule['restrictToTypes'] && is_array($va_rule['restrictToTypes']) && (sizeof(
                        $va_rule['restrictToTypes']
                    ) > 0)) {
                if (!in_array($t_instance->getTypeCode(), $va_rule['restrictToTypes'])) {
                    Debug::msg(
                        "[prepopulateFields()] skipping rule $vs_rule_key because current record type " . $t_instance->getTypeCode(
                        ) . " is not in restrictToTypes"
                    );
                    continue;
                }
            }
            ...
            if ($va_rule['skipIfExpression'] && (strlen($va_rule['skipIfExpression']) > 0)) {
                ...
                foreach ($va_tags as $vs_tag) {
                    if (!isset($va_expression_vars[$vs_tag])) {
                        $va_expression_vars[$vs_tag] = $t_instance->get(
                            $vs_tag,
                ...
                if (ExpressionParser::evaluate($va_rule['skipIfExpression'], $va_expression_vars)) {
                    Debug::msg(
                        "[prepopulateFields()] skipping rule $vs_rule_key because skipIfExpression evaluated true"
                    );
                    continue;
                }
            }
            ...
            if (!$vb_is_relationship_rule) {
                // evaluate template
                $vs_value = caProcessTemplateForIDs(
                    $vs_template,
                    $t_instance->tableNum(),
            ...
            if ((sizeof($va_parts) == 1) && $vb_is_relationship_rule) {    // clone relationships
                if (($vs_mode === 'addifempty') && $t_instance->hasRelationshipsWith($vs_target)) {
                    Debug::msg(
                        "[prepopulateFields()] skipped rule {$vs_rule_key} because mode is addIfEmpty and it already has {$vs_target} relationships."
                    );
                    continue;
                }
                ...
                switch ($vs_context) {
                    case 'parent':
                        ...
                        if (($vn_parent_id = $t_instance->get(
                                $t_instance->getProperty('HIERARCHY_PARENT_ID_FLD')
                            )) && $t_parent->load($vn_parent_id)) {
                            $va_rels = $t_parent->getRelatedItems(
                                $vs_target,
                                [
                                    'showCurrentOnly' => caGetOption(
                                        'currentOnly',
                                        $va_rule,
                                        false
                                    )
                    ...
                    case 'children':
                        if ($t_instance->getPrimaryKey()) {
                            $va_child_ids = $t_instance->getHierarchy(
                    ...
                    case 'related':
                        if ($t_instance->getPrimaryKey()) {
                            $va_rel_ids = $t_instance->get(
                                $t_instance->tableName() . '.related.' . $t_instance->primaryKey(),
                ...
                if (is_array($va_rels)) {
                    foreach ($va_rels as $va_rel) {
                        ...
                        if (is_array($va_restrict_to_related_types) && sizeof($va_restrict_to_related_types) && sizeof(
                                array_intersect($va_related_types, $va_restrict_to_related_types)
                            )) {
                            continue;
                        }
                        if (is_array($va_exclude_related_types) && sizeof($va_exclude_related_types) && !sizeof(
                                array_intersect($va_related_types, $va_exclude_related_types)
                            )) {
                            continue;
                        }

                        $vn_target_id = $va_rel[Datamodel::primaryKey($vs_target)];
                        if (!($va_existing_rel_ids = $t_instance->relationshipExists(
                            $vs_target,
                            $vn_target_id,
                            $va_rel['relationship_type_code'],
                            $va_rel['effective_date']
                        ))) {
                            if ($t = $t_instance->addRelationship(
                                $vs_target,
                                $vn_target_id,
                                $va_rel['relationship_type_code'],
                                $va_rel['effective_date']
                            ...
                            foreach ($va_instance_rels as $va_instance_rel) {
                                if (!in_array($va_instance_rel['relation_id'], $va_instance_rel_ids)) {
                                    if (!$t_instance->removeRelationship($vs_target, $va_instance_rel['relation_id'])) {
                                        Debug::msg(
                                            "[prepopulateFields()] could not delete {$vs_target} relationship in overwrite mode"
                                        );
                                    }
                                }
                            }
                    ...
                    switch ($vs_mode) {
                        ...
                        default:
                            if (!$t_instance->get($va_parts[1])) {
                                ...
                                Debug::msg(
                                    "[prepopulateFields()] rule {$vs_rule_key}: intrinsic skipped because it already has value and mode is addIfEmpty or merge"
                                );
                            }
                            break;
                    }
                        ...
                        if (is_array($source_values = $t_instance->get($vs_source, ['returnWithStructure' => true]))) {
                            ...
                            foreach ($va_attributes as $attr) {
                                foreach ($attr->getValues() as $v) {
                                    if (is_array($va_source_map) && sizeof($va_source_map) && !array_key_exists(
                                            $v->getElementCode(),
                                            $va_source_map
                                        )) {
                                        continue;
                            ...
                            if ($t_instance->numErrors()) {
                                Debug::msg(
                                    _t(
                                        "[prepopulateFields()] error while removing old values during copy of containers: %1",
                            ...
                            foreach (array_shift($source_values) as $attr_id => $attr) {
                                ...
                                if ($t_instance->numErrors()) {
                                    Debug::msg(
                                        _t(
                                            "[prepopulateFields()] error during copy of containers: %1",
                                            join("; ", $t_instance->getErrors())
                                        )
                                    );
                                }
                                ...
                                case 'overwrite':
                                    ...
                                    foreach ($vo_attr->getValues() as $o_val) {
                                        if ($o_val->getElementCode() != $va_parts[2]) {
                                            $va_value[$o_val->getElementCode()] = $v = $o_val->getDisplayValue(
                                                ['idsOnly' => true]
                                            );
                                            $vb_is_set = true;
                                        }
                                    ...
                                    if (($vs_mode === 'overwrite') || $vb_is_set) {
                                        $t_instance->_editAttribute(
                                            $vo_attr->getAttributeID(),
                                            $va_value,
                            ...
                            switch ($vs_mode) {
                                ...
                                case 'addifempty':
                                    $va_labels = caExtractValuesByUserLocale(
                                        $t_instance->getLabels(
                                            null,
                                            $vb_preferred ? __CA_LABEL_TYPE_PREFERRED__ : __CA_LABEL_TYPE_NONPREFERRED__
                                        )
                                    );
                                    ...
                                    if (sizeof($va_labels)) {
                                        ...
                                        $is_blank = (isset($va_label[$label_fld]) && ($va_label[$label_fld] == '[' . _t(
                                                    'BLANK'
                                                ) . ']'));
                                        ...
                                        if ($vs_mode == 'overwrite') {
                                            ...
                                            if ((strlen(
                                                        $l
                                                    ) == 0) || ($is_blank)) { // in addifempty mode only edit label when field is not set
                                                $va_label[$va_parts[2]] = $vs_value;
                                                $vb_update = true;
                                            }
                                        }
