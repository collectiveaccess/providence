...
class ImportController extends ActionController {
	...
	public function __construct( &$po_request, &$po_response, $pa_view_paths = null ) {
		...
		MetaTagManager::addLink( 'stylesheet',
			__CA_URL_ROOT__ . "/app/plugins/ResourceSpace/themes/" . __CA_THEME__ . "/css/ResourceSpace.css",
			'text/css' );
	}
	# -------------------------------------------------------
	...
	public function Index() {
		...
		$this->view->setVar( 'importer_list', $va_importer_list = ca_data_importers::getImporters( null,
			array( 'formats' => array( 'ResourceSpace' ) ) ) );

		$va_importer_options = array();
		foreach ( $va_importer_list as $vn_importer_id => $va_importer_info ) {
			$va_importer_options[ $va_importer_info['label'] . ' (creates ' . ( $va_importer_info['type_for_display']
				? $va_importer_info['type_for_display'] . ' ' : '' ) . $va_importer_info['importer_type'] . ')' ]
				= $vn_importer_id;
		}
	...
	public function Run() {
		...
		$vs_job_id      = md5( 'U' . $this->request->getUserID() . '_' . $pn_importer_id . '_' . join( ';',
				$pa_resourcespace_ids ) . '_' . uniqid( rand(), true ) . '_' . microtime( true ) );
		...
		$this->request->user->setVar( 'resourcespace_log_level',
			$vn_log_level = $this->request->getParameter( 'log_level', pInteger ) );
	...
	public function RunImport() {
		...
		$vn_status  = $t_importer->importDataFromSource( $pa_resourcespace_ids, $pn_importer_id,
			array( 'progressBar' => $o_progress, 'format' => 'resourcespace', 'logLevel' => $pn_log_level ) );
		...
		$va_links = caProcessTemplateForIDs( $vs_template, $va_job_info['data']['table'],
			array_merge( $va_job_info['data']['created'], $va_job_info['data']['updated'] ),
			[ 'delimiter' => '<br/>' ] );
	...
	public function Lookup() {
		...
		$this->view->setVar( 'results', $o_wc->lookup( array(), $this->request->getParameter( 'term', pString ), array(
			...
			'systems' => $this->request->getParameter( 'systems', pArray ),
	...
	public function Detail() {
		...
		$this->view->setVar( 'detail',
			$o_wc->getExtendedInformation( array(), $this->request->getParameter( 'url', pString ) ) );
