...
<form action="#" id="caResourceSpaceSearchForm">
	<div class="formLabel formLabelThird">
		<?php print '<div>' . _t( 'Search Resources' ) . '</div><div>' . caHTMLTextInput( "term",
				array( 'value' => '', 'id' => 'caResourceSpaceId' ), array( 'width' => '200px' ) ) . "</div>"; ?>
	</div>
	<div class="formLabel formLabelThird">
		<?php print '<div>' . _t( 'Search Collections' ) . '</div><div>' . caHTMLTextInput( "term",
				array( 'value' => '', 'id' => 'caResourceSpaceCollectionId' ), array( 'width' => '200px' ) )
		            . "</div>"; ?><br/>
	</div>
	<div class="formLabel formLabelThird">
		...
		<div id="caResourceSpaceSearchSystems">
			...
			foreach ( $this->getVar( 'rs_labels' ) as $vs_rs_code => $vs_rs_label ) {
				print "<input type='checkbox' id='" . $vs_rs_code . "_system' name='caResourceSpaceSystems' value='"
				      . $vs_rs_code . "' checked/>" . $vs_rs_label . "<br/>";
			}
			?>
		</div>
	</div>
	<div class="form-label">
		<a href="#" id="caResourceSpaceLookup" class="form-button"><span><?php print caNavIcon( __CA_NAV_ICON_GO__,
					"30px" ); ?> Search</span></a>
	</div>
</form>
<?php
print caFormTag( $this->request, 'Run', 'caResourceSpaceResultsForm', null, 'post', 'multipart/form-data', '_top',
...
<script type="text/javascript">
	...
	jQuery(document).ready(function () {
		...
		jQuery(document).on('click', '.caResourceSpaceSearchResultItem', {}, function (e) {
			jQuery(this).parent().find(".caResourceSpaceSearchResultDetails").slideToggle(250, function (e) {
				...
				var details = this;
				jQuery(details).html("<div class='caResourceSpaceDetailsMessage'><?php print caBusyIndicatorIcon( $this->request )
				                                                                             . ' '
	...
	function caSearchResourceSpace(term, collectionTerm, systems, start, c, msg) {
		if (!msg) {
			msg = "<?php print addslashes( _t( 'Searching ResourceSpace...' ) ); ?>";
		}
		if (start <= 0) {
			start = 0;
		}
		...
		jQuery("#caResourceSpaceResults").html("<div class='caResourceSpaceResultsMessage'><?php print caBusyIndicatorIcon( $this->request )
		                                                                                               . ' '; ?>" + msg + "</div>");
		jQuery.getJSON('<?php print caNavUrl( $this->request, '*', '*', 'Lookup' ); ?>', {
			term: term + '|' + collectionTerm,
			start: start,
			systems: systems,
			count: c
		}, function (data) {
