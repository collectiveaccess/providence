...
print $vs_control_box = caFormControlBox(
	( $vb_importers_available ? ( caFormSubmitButton( $this->request, __CA_NAV_ICON_SAVE__, _t( "Import" ),
		'caULANResultsForm' ) ) : '' ) . ' ' .
...
<form action="#" id="caULANSearchForm">
	<div class="formLabel">
		<?php print _t( 'Find in ULAN' ) . ': ' . caHTMLTextInput( "term", array( 'value' => '', 'id' => 'caULANTerm' ),
				array( 'width' => '250px' ) ); ?>
...
<?php
print caFormTag( $this->request, 'Run', 'caULANResultsForm', null, 'post', 'multipart/form-data', '_top',
...
<div class='formLabel'>
	<?php
	if ( $vb_importers_available ) {
		print _t( 'Log level' ) . ': ' . caHTMLSelect( 'log_level', caGetLogLevels(), array( 'id' => 'caLogLevel' ),
				array( 'value' => $this->getVar( 'log_level' ) ) );
...
<script type="text/javascript">
	...
	jQuery(document).ready(function () {
		...
		jQuery(document).on('click', '.caULANSearchResultItem', {}, function (e) {
			jQuery(this).parent().find(".caULANSearchResultDetails").slideToggle(250, function (e) {
				...
				var details = this;
				jQuery(details).html("<div class='caULANDetailsMessage'><?php print caBusyIndicatorIcon( $this->request )
				                                                                    . ' '
				                                                                    . _t( 'Loading details...' ); ?></div>");
				jQuery.getJSON('<?php print caNavUrl( $this->request, '*', '*',
					'Detail' ); ?>', {url: url}, function (data) {
	...
	function caSearchULAN(term, start, c, msg) {
		if (!msg) {
			msg = "<?php print addslashes( _t( 'Searching ULAN...' ) ); ?>";
		}
		if (start <= 0) {
			start = 0;
		}
		...
		jQuery("#caULANResults").html("<div class='caULANResultsMessage'><?php print caBusyIndicatorIcon( $this->request )
		                                                                             . ' '; ?>" + msg + "</div>");
		jQuery.getJSON('<?php print caNavUrl( $this->request, '*', '*', 'Lookup' ); ?>', {
			term: term,
			start: start,
			count: c
		}, function (data) {
