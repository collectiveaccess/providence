...
function processTarget(\BaseModel $rec, string $table, array $t, ?array $options = null): array
{
	...
	$rel_list = [];
	if (sizeof($rel_ids = array_map(function ($v) use ($resolve_to_related, $target_pk, $t) {
		return $v[$resolve_to_related ? $target_pk : 'relation_id'];
	}, $rels)) > 0) {
				...
				if (is_array($reps)) {
					foreach ($reps as $rep_id => $rep_info) {
						...
						foreach ($rep_info['urls'] as $version => $url) {
							if (!in_array($version, $media_versions)) {
								continue;
							}
							$versions[] = [
								'version' => $version,
