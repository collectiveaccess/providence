...
class ModelController extends BaseServiceController {
	...
	public function exportConfig() {
		...
		$this->getView()
		     ->setVar( 'content', ConfigurationExporter::exportConfigurationAsXML( '', '', '', '', $vn_timestamp ) );
		$this->render( 'xml.php' );
	}
	...
	public function updateConfig() {
		...
		try {
			...
			if ( $va_errors = $o_installer->getErrors() ) {
				$this->getView()->setVar( 'errors', $va_errors );
				$o_log->logInfo( _t( 'It seems there was at least one error while running Installer for updateConfig: %1',
					join( ',', $va_errors ) ) );
				$this->render( 'json_error.php' );
			...
			$o_log->logInfo( _t( 'It seems there was an error while running Installer for updateConfig: %1',
				$e->getMessage() ) );
