	...
	public function updateRelationshipTypes() {
		...
		foreach ( $va_tables as $vs_table ) {
			...
			if ( ! $t_root->load( array( 'type_code' => 'root_for_table_' . $vn_table_num ) ) ) {
				...
				if ( $t_root->numErrors() ) {
					$this->opa_error_messages[] = _t( "Could not create root for relationship %1: %2", $vs_table,
				...
				if ( $t_root->numErrors() ) {
					$this->opa_error_messages[] = _t( "Could not add label to root for relationship %1: %2", $vs_table,
						join( '; ', $t_root->getErrors() ) );
				}
			}
