                    ...
                    if ($o_xml->nodeType == XMLReader::END_ELEMENT) {
                        ...
                        if ($vs_description_element_code && $va_subject['description']) {
                            $t_place->addAttribute(
                                array(
                                    $vs_description_element_code => $va_subject['description'],
                        ...
                        if ($vs_georef_element_code && ($va_coords['latitude']['decimal'] && $va_coords['longitude']['decimal'])) {
                            $t_place->addAttribute(
                                array(
                                    $vs_georef_element_code => "[" . $va_coords['latitude']['decimal'] . $va_coords['latitude']['direction'] . "," . $va_coords['longitude']['decimal'] . $va_coords['longitude']['direction'] . "]",
                                    'locale_id' => $pn_en_locale_id
                                ),
                        ...
                        if ($vn_place_id = $t_place->insert(array('dontSetHierarchicalIndexing' => true))) {
                            if (!($t_place->addLabel(
                                ...
                                print "[Error] Could not add preferred label to TGN term [" . $va_subject['subject_id'] . "] " . $vs_preferred_term . ": " . join(
                                        "; ",
                                        $t_place->getErrors()
                                    ) . "\n";
                            }
                            ...
                            if (is_array($va_subject['non_preferred_terms'])) {
                                for ($vn_i = 0; $vn_i < sizeof($va_subject['non_preferred_terms']); $vn_i++) {
                                    ...
                                    if (!($t_place->addLabel(
                                        ...
                                        print "[Error] Could not add non-preferred label to TGN term [" . $va_subject['subject_id'] . "] " . $vs_np_label . ": " . join(
                                                "; ",
                                                $t_place->getErrors()
                                            ) . "\n";
                                    }
                            ...
                            if ($vs_place_type_relationship_code && $va_subject['place_type_id']) {
                                ...
                                if ($vn_item_id = DataMigrationUtils::getListItemID(
                                    'tgn_place_types',
                                    $va_tmp[0],
                                    null,
                                    $pn_en_locale_id,
                                    array(
                                        'name_singular' => $va_tmp[1],
                                        'name_plural' => $va_tmp[1]
                                    ),
                                    array()
                                )) {
                                    $t_place->addRelationship(
                                        'ca_list_items',
                                        $vn_item_id,
                                        $vs_place_type_relationship_code,
                                        null,
                                        null,
                                        null,
                                        null,
                            ...
                            print "[Error] Could not import TGN term [" . $va_subject['subject_id'] . "] " . $vs_preferred_term . ": " . join(
                                    "; ",
                                    $t_list->getErrors()
                                ) . "\n";
                        }
                                            ...
                                            switch ($o_xml->name) {
                                                ...
                                                case 'Direction':
                                                    switch ($o_xml->nodeType) {
                                                        case XMLReader::ELEMENT:
                                                            $o_xml->read();
                                                            $va_coords['latitude']['direction'] = substr(
                                                                $o_xml->value,
                                                                0,
                                                                1
                                                            );
                                            ...
                                            switch ($o_xml->name) {
                                                ...
                                                case 'Direction':
                                                    switch ($o_xml->nodeType) {
                                                        case XMLReader::ELEMENT:
                                                            $o_xml->read();
                                                            $va_coords['longitude']['direction'] = substr(
                                                                $o_xml->value,
                                                                0,
                                                                1
                                                            );
                ...
                case 'Subject':
                    if ($o_xml->nodeType == XMLReader::END_ELEMENT) {
                        ...
                        if ($vs_parent_id == '100000000') {
                            ...
                            $t_place->update(
                                array('dontSetHierarchicalIndexing' => true, 'dontCheckCircularReferences' => true)
                            );

                            if ($t_place->numErrors()) {
                                print "[Error] could not set parent_id for {$vs_child_id} (was translated to item_id=" . $t_place->getPrimaryKey(
                                    ) . "): " . join('; ', $t_place->getErrors()) . "\n";
                            }
                        }
                    } else {
                        $va_subject = array('subject_id' => $o_xml->getAttribute('Subject_ID'));
            ...
            switch ($o_xml->name) {
                ...
                case 'Subject':
                    if ($o_xml->nodeType == XMLReader::END_ELEMENT) {
                        ...
                        $vn_rel_type_id = $t_rel_type->getRelationshipTypeID(
                            $t_link->tableNum(),
                            $va_tmp[0],
                            $pn_en_locale_id,
                            array('typename' => $va_tmp[1]),
                            array('create' => true)
                        );
                        ...
                        if ($t_link->numErrors()) {
                            print "[Error] could not link " . $va_subject['subject_id'] . " to " . $va_subject['related_subject_id'] . ": " . join(
                                    '; ',
                                    $t_place->getErrors()
                                ) . "\n";
                        }
