            ...
            if ($o_xml->nodeType == XMLReader::END_ELEMENT) {
                ...
                if ($t_item = $t_list->addItem(
                    ...
                    if ($vs_preferred_term_dutch) {
                        if (!($t_item->addLabel(
                            array(
                                'name_singular' => $vs_preferred_term_dutch,
                                'name_plural' => $vs_preferred_term_dutch,
                                'description' => $va_subject['description_dutch']
                            ),
                            $pn_nl_locale_id,
                            null,
                            true
                        ))) {
                            print "ERROR: Could not add Dutch preferred label to AAT term [" . $va_subject['term_number'] . "] " . $vs_preferred_term_dutch . ": " . join(
                                    "; ",
                                    $t_item->getErrors()
                                ) . "\n";
                        }
                    ...
                    if ($vs_preferred_term_english) {
                        if (!($t_item->addLabel(
                            array(
                                'name_singular' => $vs_preferred_term_english,
                                'name_plural' => $vs_preferred_term_english,
                                'description' => $va_subject['description_english']
                            ),
                            $pn_nl_locale_id,
                            null,
                            true
                        ))) {
                            print "ERROR: Could not add English preferred label to AAT term [" . $va_subject['term_number'] . "] " . $vs_preferred_term_english . ": " . join(
                                    "; ",
                                    $t_item->getErrors()
                                ) . "\n";
                        }
                    ...
                    if (is_array($va_subject['use_for_english'])) {
                        for ($vn_i = 0; $vn_i < sizeof($va_subject['use_for_english']); $vn_i++) {
                            ...
                            if (!($t_item->addLabel(
                                array(
                                    'name_singular' => $vs_np_label,
                                    'name_plural' => $vs_np_label,
                                    'description' => ''
                                ),
                                $pn_en_locale_id,
                                $vn_list_item_label_type_uf,
                                false
                            ))) {
                                print "ERROR: Could not add English non-preferred label to AAT term [" . $va_subject['term_number'] . "] " . $vs_np_label . ": " . join(
                                        "; ",
                                        $t_item->getErrors()
                                    ) . "\n";
                            }
                    ...
                    if (is_array($va_subject['use_for_dutch'])) {
                        for ($vn_i = 0; $vn_i < sizeof($va_subject['use_for_dutch']); $vn_i++) {
                            ...
                            if (!($t_item->addLabel(
                                ...
                                print "ERROR: Could not add Dutch non-preferred label to AAT term [" . $va_subject['term_number'] . "] " . $vs_np_label . ": " . join(
                                        "; ",
                                        $t_item->getErrors()
                                    ) . "\n";
                            }
                    ...
                    print "ERROR: Could not import AAT term [" . $va_subject['term_number'] . "] " . $vs_preferred_term_dutch . '/' . $vs_preferred_term_english . ": " . join(
                            "; ",
                            $t_list->getErrors()
                        ) . "\n";
                }
