...
class ModelSchemaTest extends TestCase {
	...
	public function testDatamodelRelationshipsAreValid() {
		...
		foreach ( $va_relationships as $vn_i => $vs_rel ) {
			...
			$this->assertInstanceOf( 'BaseModel', $t_left,
				"Model {$vs_left_table} does not exist (relationship was {$vs_rel})" );

			$t_right = Datamodel::getInstanceByTableName( $vs_right_table );
			$this->assertInstanceOf( 'BaseModel', $t_left,
				"Model {$vs_right_table} does not exist (relationship was {$vs_rel})" );

			$this->assertTrue( $t_left->hasField( $vs_left_field ),
				"Field {$vs_left_field} does not exist in model {$vs_left_table} (relationship was {$vs_rel})" );
			$this->assertTrue( $t_right->hasField( $vs_right_field ),
				"Field {$vs_right_field} does not exist in model {$vs_right_table} (relationship was {$vs_rel})" );

			// Check that fields exists in database
			$qr_res = $o_db->query( "SHOW COLUMNS FROM {$vs_left_table} WHERE Field = ?", $vs_left_field );
			$this->assertEquals( 1, $qr_res->numRows(),
				"Field {$vs_left_field} does not exist in database table {$vs_left_table} (relationship was {$vs_rel})" );
			$qr_res = $o_db->query( "SHOW COLUMNS FROM {$vs_right_table} WHERE Field = ?", $vs_right_field );
			$this->assertEquals( 1, $qr_res->numRows(),
				"Field {$vs_right_field} does not exist in database table {$vs_right_table} (relationship was {$vs_rel})" );
		}
