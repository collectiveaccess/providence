...
class RelationshipGeneratorConfigurationPluginTest extends TestCase {
	...
	public function testDisabledConfigurationIsDisabledAndValid() {
		...
		$this->assertEmpty( $va_plugin_status['errors'],
			'Disabling the plugin via configuration does not produce any errors' );
	}
	...
	protected function assertCorrectErrorMessages(
		$pa_expectedErrorMessageContents, $pa_actualErrorMessages, $ps_sizeCheckDescription,
		$ps_valueCheckDescriptionTemplate
	) {
		$this->assertEquals( sizeof( $pa_expectedErrorMessageContents ), sizeof( $pa_actualErrorMessages ),
			_t( $ps_sizeCheckDescription ) );
		foreach ( $pa_expectedErrorMessageContents as $va_expectedContent ) {
			foreach ( $va_expectedContent as $vs_expectedContentSubstring ) {
				$this->assertRegExp( '/' . str_replace( '/', '\\/', $vs_expectedContentSubstring ) . '/',
					_t( $ps_valueCheckDescriptionTemplate, $vs_expectedContentSubstring ) );
			}
		}
	}
}
