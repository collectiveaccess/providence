 ...
 *         self::_init();
 *         self::_processConfiguration(__DIR__ . '/conf/integration', 'conf/fancyPants.conf.template',
 *         'conf/fancyPants.conf'); self::_switchInTestPlugin('fancyPants', new fancyPantsPlugin(__DIR__ .
 *         '/conf/integration'));
 ...
 *         $vo_test_collection->set(array( 'idno' => self::_getIdno('test_collection'), 'name' => 'test collection' ));
 *         // and perform any other object setup
...
abstract class AbstractPluginIntegrationTest extends TestCase {
			...
			while ( strpos( $vs_line, '%%' ) !== false ) {
				...
				$vs_line      = preg_replace_callback(
					...
					function ( $pa_match ) use ( $vs_className ) {
						return sizeof( $pa_match ) > 1 ? call_user_func( array( $vs_className, '_getIdno' ),
							$pa_match[1] ) : '::: ERROR PARSING CONFIGURATION TEMPLATE :::';
					},
					$vs_line
				);
			}
	...
	protected static function _retrieveCreatedInstance( $ps_class, $ps_key ) {
		return isset( self::$s_created_instances[ $ps_class ] )
		       && isset( self::$s_created_instances[ $ps_class ][ $ps_key ] )
			? self::$s_created_instances[ $ps_class ][ $ps_key ] : null;
	}
	...
	protected static function _createListItem( $ps_idno_base, $pn_list_id, $ps_type_code = null ) {
		...
		if ( ! is_null( $ps_type_code ) ) {
			...
			if ( is_null( $vo_test_list_item_type_list_item ) ) {
				$vo_test_list_item_type_list_item = self::_createListItem( $ps_type_code,
					BaseModel::$s_ca_models_definitions['ca_list_items']['FIELDS']['type_id']['LIST_CODE'] );
			}
		}
		$vo_list_item->set( array(
			'idno'       => self::_getIdno( $ps_idno_base ),
			'list_id'    => $pn_list_id,
			'type_id'    => ! is_null( $vo_test_list_item_type_list_item )
				? $vo_test_list_item_type_list_item->get( 'idno' ) : null,
			'is_enabled' => true
		) );
		$vo_list_item->insert();
	...
	protected static function _createCollection( $ps_idno_base ) {
		...
		if ( is_null( $vo_test_collection_type_list_item ) ) {
			$vo_test_collection_type_list_item = self::_createListItem( 'test_collection_type',
				BaseModel::$s_ca_models_definitions['ca_collections']['FIELDS']['type_id']['LIST_CODE'] );
		}
		...
		foreach ( self::_retrieveCreatedInstancesByClass( 'ca_metadata_elements' ) as $vo_metadata_element ) {
			$vo_collection->addMetadataElementToType( $vo_metadata_element->get( 'element_code' ),
				$vo_test_collection_type_list_item->getPrimaryKey() );
		}
	...
	protected static function _createEntity( $ps_idno_base ) {
		...
		if ( is_null( $vo_test_entity_type_list_item ) ) {
			$vo_test_entity_type_list_item = self::_createListItem( 'test_entity_type',
				BaseModel::$s_ca_models_definitions['ca_entities']['FIELDS']['type_id']['LIST_CODE'] );
		}
		$vo_entity->set( array(
			'idno'    => self::_getIdno( $ps_idno_base ),
