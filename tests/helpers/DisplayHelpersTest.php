...
class DisplayHelpersTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testTags()
    {
        ...
        $va_tags = caGetTemplateTags(
            'Here is a simple CA field spec with options: ^ca_objects.user_tags%delimiter=_➜_'
        );
        $this->assertEquals('ca_objects.user_tags%delimiter=_➜_', $va_tags[0]);
        $this->assertEquals(1, sizeof($va_tags));

        $va_tags = caGetTemplateTags(
            'Here is a simple CA field spec with options: ^ca_objects.user_tags%delimiter=_➜_&maxLength=100'
        );
        ...
        $va_tags = caGetTemplateTags(
            "Here are XPath tags: ^/datafield[@tag='040']/subfield[@code='a'] ^/datafield[@tag='040']/title,meow ^/datafield[@tag='040']/subfield[@code='c']"
        );
        ...
        $va_tags = caGetTemplateTags(
            "Here are namespaced XPath tags: ^/tei:title[@tag='040']/subfield[@code='a'],foo ^/tei:title[@tag='040']/subfield[@code='b'] ^/tei:title[@tag='040']/title"
        );
