...
class ConfigurationUpdateTest extends TestCase {
	...
	public function testAddNewLocale() {
		...
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/locales/add_new.xml' ) );
	...
	public function testUpdateLocale() {
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/locales/update_existing.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
	...
	public function testAddNewItemToExistingList() {
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/lists/add_new_item_to_existing_list.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
	...
	public function testEditItemInExistingList() {
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/lists/edit_item_in_existing_list.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
		$o_installer->processLists();
	...
	public function testAddNewList() {
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/lists/add_new_list_with_items.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
		$o_installer->processLists();
	...
	public function testDeleteListItem() {
		...
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/lists/delete_list_items.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
		$o_installer->processLists();
	...
	public function testDeleteList() {
		...
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/lists/delete_list.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
		$o_installer->processLists();
	...
	public function testAddNewElement() {
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/elements/add_new_element.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
	...
	public function testRemoveLabelsFromElement() {
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/elements/remove_labels_from_element.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
		$o_installer->processMetadataElements();
	...
	public function testAddElementExistingContainer() {
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/elements/add_new_element_to_existing_container.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
		$o_installer->processMetadataElements();
	...
	public function testDeleteElements() {
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/elements/delete_elements.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
		$o_installer->processMetadataElements();
	...
	public function testEditElementExistingContainer() {
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/elements/edit_element_in_existing_container.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
		$o_installer->processMetadataElements();
		...
		$va_elements_in_set
			= array_values( $t_instance->getElementsInSet() ); // we want to address things by position in the array below, not by element_id
	...
	public function testAddNewUI() {
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/uis/add_new_ui.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
		$o_installer->processUserInterfaces();

		$t_ui = ca_editor_uis::find( array( 'editor_code' => 'alternate_entity_ui', 'editor_type' => 20 ),
	...
	public function testAddScreenToExistingUI() {
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/uis/add_new_screen_to_existing_ui.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
		$o_installer->processUserInterfaces();

		$t_ui = ca_editor_uis::find( array( 'editor_code' => 'standard_entity_ui', 'editor_type' => 20 ),
			array( 'returnAs' => 'firstModelInstance' ) );
		$this->assertInstanceOf( 'ca_editor_uis', $t_ui );
	...
	public function testEditScreenInExistingUI() {
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/uis/edit_screen_in_existing_ui.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
		$o_installer->processUserInterfaces();
		...
		$va_screen
			= array_pop( $va_screens ); // should be the last screen (let's hope the prev test didn't have side effects ;-))
	...
	public function testDeleteRelType() {
		...
		$o_installer = Installer::getFromString( file_get_contents( dirname( __FILE__ )
		                                                            . '/profile_fragments/reltypes/delete_reltype.xml' ) );
		$this->assertTrue( $o_installer instanceof Installer );
		$o_installer->processLocales();
		...
		$t_rt->set( [
			'parent_id' => ca_relationship_types::find( [ 'parent_id' => null, 'table_num' => $tn ],
				[ 'returnAs' => 'firstId' ] ),
			'type_code' => 'describes',
			'table_num' => $tn
		] );
