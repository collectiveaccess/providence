...
class ExpressionParserTest extends TestCase {
	...
	public function testFunctions() {
		...
		$this->assertEquals( 15324,
			ExpressionParser::evaluate( 'agedays("7 June 1954", "9 May 1945", "23 June 1912")' ) );
		...
		$this->assertEquals( 13229,
			ExpressionParser::evaluate( 'avgdays("1912/06/23 - 1954/06/07", "1985/01/28 - 2015/07/24")' ) );
		$this->assertEquals( 0, ExpressionParser::evaluate( 'avgdays("1945/01/02", "1985/01/28")' ) );
		$this->assertEquals( 1,
			ExpressionParser::evaluate( 'avgdays("1945/01/02 - 1945/01/03", "1985/01/28 - 1985/01/29")' ) );
...
// 	}
	...
	public function testVars() {
		...
		$this->assertEquals( 'test123', ExpressionParser::evaluate( '^ca_objects.preferred_labels',
			array( 'ca_objects.preferred_labels' => 'test123' ) ) );
		$this->assertEquals( true,
			ExpressionParser::evaluate( '^ca_entities.type_id%convertCodesToDisplayText=0&convertCodesToIdno=1 =~ /ind/',
				array( 'ca_entities.type_id%convertCodesToDisplayText=0&convertCodesToIdno=1' => 'ind' ) ) );
		$this->assertEquals( 'Poa annua', ExpressionParser::evaluate( 'join(" ", ^Genus, ^Species)',
			array( 'Genus' => 'Poa', 'Species' => 'annua' ) ) );
		$this->assertEquals( 'Poa annua L.', ExpressionParser::evaluate( 'implode(" ", ^Genus, ^Species, ^Authority)',
			array( 'Genus' => 'Poa', 'Species' => 'annua', 'Authority' => 'L.' ) ) );
		$this->assertTrue( (bool) ExpressionParser::evaluate( 'trim(join(" ", ^Genus, ^Species)) = ^ScientificName',
			array( 'Genus' => 'Poa', 'Species' => '', 'ScientificName' => 'Poa' ) ) );
	}

	public function testObscureVars() {
		$this->assertTrue( ExpressionParser::evaluate( '^/gvp:Subject/gvp:parentString =~ /Corporate/',
			array( '/gvp:Subject/gvp:parentString' => 'Corporate' ) ) );
	}
	...
	public function testEvaluate() {
		...
		$this->assertEquals( true,
			ExpressionParser::evaluate( '"Seth" IN ["Julia", "Allison", "Sophie", "Maria", "Angie", "Seth"]' ) );
		$this->assertEquals( false,
			ExpressionParser::evaluate( '"Joe" IN ["Julia", "Allison", "Sophie", "Maria", "Angie", "Seth"]' ) );

		$this->assertEquals( false,
			ExpressionParser::evaluate( '"Seth" NOT IN ["Julia", "Allison", "Sophie", "Maria", "Angie", "Seth"]' ) );
		$this->assertEquals( true,
			ExpressionParser::evaluate( '"Joe" NOT IN ["Julia", "Allison", "Sophie", "Maria", "Angie", "Seth"]' ) );
