...
class ConfigurationTest extends TestCase {
	public function testScalars() {
		...
		$this->assertEquals( $o_config->get( 'a_scalar_starting_with_a_bracket' ),
			'[The bracket is part of the string]' );
		$this->assertEquals( $o_config->get( 'a_scalar_using_a_macro' ), '/usr/local/fish' );
		$this->assertEquals( $o_config->get( 'a_scalar_using_an_embedded_macro' ),
			'This scalar is embedded: "/usr/local/fish"' );
	...
	public function testMisc() {
		...
		$this->assertEquals( sizeof( $va_keys ),
			14 );        // 12 in config file + 1 "LOCALE" value that's automatically inserted
