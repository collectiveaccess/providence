...
class BundlableLabelableBaseModelWithAttributesTest extends TestCase {
	...
	public function testInsertLoadAndDeleteCycleWithCaObjectsModel() {
		...
		$this->assertTrue( $vb_res !== false,
			'Insert returned non-true value' ); // insert() returns false OR the primary key, therefore simply asserting $vb_res being true doesn't cut it
		...
		$this->assertGreaterThan( 0, $vb_res,
			'AddLabel returned zero value but should return non-zero label_id: ' . join( '; ',
				$t_object->getErrors() ) );

		$t_object2 = new ca_objects();
		$vb_res    = $t_object2->load( $t_object->getPrimaryKey() );
		$this->assertTrue( $vb_res,
			'Load of newly created record failed [record does not seem to exist or return row id was invalid?]' );

		$this->assertEquals( $t_object2->getLabelForDisplay(), 'Unit test object',
			'Retrieved row label does not match' );
		$this->assertEquals( $t_object2->getAttributesForDisplay( 'description' ), 'Test description',
			'Retrieved value for attribute "description" does not match expected value' );
...
// 	}
	...
	public function testPrepopulateFieldsSimple() {
		// load config
		$va_prepopulate_options = array(
			'prepopulateConfig' => dirname( __FILE__ ) . DIRECTORY_SEPARATOR . 'conf' . DIRECTORY_SEPARATOR
		...
		$this->assertTrue( $o_plugin->prepopulateFields( $t_object, $va_prepopulate_options ),
			'Prepopulate should return true' );
	...
	public function testPrepopulateFieldsIntrinsic() {
		...
		$this->assertEquals( 'a label', $t_object->get( 'ca_objects.preferred_labels' ),
			'label must match the one we just added' );
	...
	public function testPrepopulateFieldsLabels() {
		...
		$this->assertEquals( 'Description', $t_object->get( 'ca_objects.preferred_labels' ),
			'label must prepopulate with description' );
	}
	...
	public function testPrepopulateFieldsLabelsDontOverwrite() {
		...
		$this->assertTrue( $o_plugin->prepopulateFields( $t_object, $va_prepopulate_options ),
			'Prepopulate should return true' );
		$this->assertEquals( 'a label', $t_object->get( 'ca_objects.preferred_labels' ),
			'label must not prepopulate because mode is addifempty' );
	}
	...
	public function testPrepopulateFieldsContainers() {
		...
		$this->assertEquals( 'test123', $t_object->get( 'ca_objects.external_link.url_source' ),
			'url source must prepopulate with idno' );
	}
	...
	public function testPrepopulateFieldsDontOverwriteContainers() {
		...
		$this->assertEquals( 'Wikipedia', $t_object->get( 'ca_objects.external_link.url_source' ),
			'url source must not change' );
		$this->assertEquals( "http://en.wikipedia.org", $t_object->get( 'ca_objects.external_link.url_entry' ),
			'url entry must not change' );
	}
