...
abstract class BaseTestWithData extends TestCase {
	 ...
	 *                         Web Service API, only that it's a PHP array, obviously. In fact, we use the same code to
	 *                         insert the data.
	 *
	...
	protected function addTestRecord( $ps_table, $pa_data ) {
		...
		if ( ! $va_return ) {
			$this->assertTrue( false,
				'Inserting test data failed. API errors are: ' . join( ' ', $o_itemservice->getErrors() ) );
		}
