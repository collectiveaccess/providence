...
class HierarchyGetTest extends BaseTestWithData {
	...
	public function testGets() {
		...
		$vm_ret = $this->opt_parent_object->get( 'ca_objects.children.preferred_labels',
			array( 'returnWithStructure' => true, 'returnAllLocales' => true ) );
		...
		$vm_ret = $this->opt_child_object->get( 'ca_objects.hierarchy.preferred_labels',
			array( 'delimiter' => ' ➔ ', 'hierarchyDirection' => 'desc' ) );
		...
		$vm_ret = $this->opt_child_object->get( 'ca_objects.siblings.preferred_labels',
			array( 'returnAsArray' => true ) );
		$this->assertTrue( is_array( $vm_ret ) );
		$this->assertEquals( 3, sizeof( $vm_ret ) );
		$this->assertEquals( 'My test still', array_shift( $vm_ret ) );
	...
	public function testChildrenRestrictToTypesDisplayTemplates() {
		$vm_ret
			= DisplayTemplateParser::evaluate( "<unit delimiter='; ' restrictToTypes='image' relativeTo='ca_objects.children'>^ca_objects.preferred_labels.name</unit>",
			"ca_objects", array( $this->opt_parent_object->getPrimaryKey() ), array( 'returnAsArray' => true ) );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
		$this->assertEquals( 'My test still; Another test still', $vm_ret[0] );

		$vm_ret
			= DisplayTemplateParser::evaluate( "<unit delimiter='; ' restrictToTypes='moving_image' relativeTo='ca_objects.children'>^ca_objects.preferred_labels.name</unit>",
			"ca_objects", array( $this->opt_parent_object->getPrimaryKey() ), array( 'returnAsArray' => true ) );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
		...
		$vm_ret
			= DisplayTemplateParser::evaluate( "<unit delimiter='; ' restrictToTypes='image'>^ca_objects.children.preferred_labels.name</unit>",
			"ca_objects", array( $this->opt_parent_object->getPrimaryKey() ), array( 'returnAsArray' => true ) );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
		$this->assertEquals( 'My test still; Another test still', $vm_ret[0] );
		...
		$vm_ret
			= DisplayTemplateParser::evaluate( "<unit delimiter='; ' restrictToTypes='moving_image'>^ca_objects.children.preferred_labels.name</unit>",
			"ca_objects", array( $this->opt_parent_object->getPrimaryKey() ), array( 'returnAsArray' => true ) );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
		$this->assertEquals( 'A child movie', $vm_ret[0] );
	...
	public function testParentRestrictToTypesDisplayTemplates() {
		$vm_ret
			= DisplayTemplateParser::evaluate( "<unit delimiter='; ' restrictToTypes='moving_image' relativeTo='ca_objects.parent'>^ca_objects.preferred_labels.name</unit>",
			"ca_objects", array( $this->opt_another_child_object->getPrimaryKey() ), array( 'returnAsArray' => true ) );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
		$this->assertEquals( 'My test moving image', $vm_ret[0] );

		$vm_ret
			= DisplayTemplateParser::evaluate( "<unit delimiter='; ' restrictToTypes='image' relativeTo='ca_objects.parent'>^ca_objects.preferred_labels.name</unit>",
			"ca_objects", array( $this->opt_another_child_object->getPrimaryKey() ), array( 'returnAsArray' => true ) );
		$this->assertIsArray( $vm_ret );
	...
	public function testHierarchyDisplayTemplates() {
		$vm_ret
			= DisplayTemplateParser::evaluate( "<unit relativeTo='ca_objects.materials' delimiter='<br/>'>^ca_objects.materials.hierarchy.preferred_labels.name_plural%delimiter=_➔_</unit>",
			"ca_objects", array( $this->opt_parent_object->getPrimaryKey() ), array( 'returnAsArray' => true ) );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
	...
	public function testHierarchyRestrictToTypesDisplayTemplates() {
		$vm_ret
			= DisplayTemplateParser::evaluate( "<unit delimiter='; ' restrictToTypes='image' relativeTo='ca_objects.hierarchy'>^ca_objects.preferred_labels.name</unit>",
			"ca_objects", array( $this->opt_another_child_object->getPrimaryKey() ), array( 'returnAsArray' => true ) );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
		//print_R($vm_ret);
		...
		$vm_ret
			= DisplayTemplateParser::evaluate( "<unit delimiter='; ' restrictToTypes='moving_image' relativeTo='ca_objects.hierarchy'>^ca_objects.preferred_labels.name</unit>",
			"ca_objects", array( $this->opt_another_child_object->getPrimaryKey() ), array( 'returnAsArray' => true ) );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
		$this->assertEquals( 'My test moving image', $vm_ret[0] );
	...
	public function testSiblingRestrictToTypesDisplayTemplates() {
		$vm_ret
			= DisplayTemplateParser::evaluate( "<unit delimiter='; ' restrictToTypes='image' relativeTo='ca_objects.siblings'>^ca_objects.preferred_labels.name</unit>",
			"ca_objects", array( $this->opt_another_child_object->getPrimaryKey() ), array( 'returnAsArray' => true ) );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
		$this->assertEquals( 'My test still; Another test still', $vm_ret[0] );
		...
		$vm_ret
			= DisplayTemplateParser::evaluate( "<unit delimiter='; ' restrictToTypes='moving_image' relativeTo='ca_objects.siblings'>^ca_objects.preferred_labels.name</unit>",
			"ca_objects", array( $this->opt_another_child_object->getPrimaryKey() ), array( 'returnAsArray' => true ) );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
		$this->assertEquals( 'A child movie', $vm_ret[0] );
