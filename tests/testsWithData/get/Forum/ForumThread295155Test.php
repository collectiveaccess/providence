...
class ForumThread295155Test extends BaseTestWithData
{
    ...
    public function setUp()
    {
         ...
         */
        $vn_test_object = $this->addTestRecord(
            'ca_objects',
            array(
    ...
    public function testGets()
    {
        ...
        while ($vo_result->nextHit()) {
            $this->assertEquals(
                'Foo',
                $vo_result->getWithTemplate(
                    "<if rule='^ca_objects.date.dc_dates_types=~ /Date created/'>Foo</if>"
                )
            );
            $this->assertEquals(
                '',
                $vo_result->getWithTemplate(
                    "<if rule='^ca_objects.date.dc_dates_types=~ /Date copyrighted/'>Foo</if>"
                )
            );
        }
    }
