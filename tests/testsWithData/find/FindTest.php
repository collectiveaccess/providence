...
Said, hey honey, take a walk on the wild side.'
	...
	public function testFindByPreferredLabelNoPurify() {
		$vm_ret = ca_objects::find( [ 'preferred_labels' => [ 'name' => 'Sound & Motion' ] ],
			[ 'purify' => false, 'returnAs' => 'ids' ] );
	...
	public function testFindByPreferredLabelWithPurify() {
		$vm_ret = ca_objects::find( [ 'preferred_labels' => [ 'name' => 'Sound &amp; Motion' ] ],
			[ 'purifyWithFallback' => false, 'purify' => true, 'returnAs' => 'ids' ] );
	...
	public function testFindByPreferredLabelWithPurifyFallback() {
		$vm_ret = ca_objects::find( [ 'preferred_labels' => [ 'name' => 'Sound &amp; Motion' ] ],
			[ 'purifyWithFallback' => true, 'purify' => false, 'returnAs' => 'ids' ] );
	...
	public function testFindByPreferredLabelWithOperatorsAndWildcards() {
		$vm_ret = ca_objects::find( [ 'preferred_labels' => [ 'name' => [ 'LIKE', 'Sound%' ] ] ],
			[ 'purify' => false, 'returnAs' => 'ids' ] );
	...
	public function testFindByAttribute() {
		$vm_ret
			= ca_objects::find( [ 'internal_notes' => 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque ullamcorper sapien nec velit porta luctus.' ],
			[ 'purify' => false, 'returnAs' => 'ids' ] );
	...
	public function testFindByAttributeWithOperators() {
		$vm_ret = ca_objects::find( [ 'internal_notes' => [ [ 'LIKE', '%ipsum%' ] ] ],
			[ 'purify' => false, 'returnAs' => 'ids' ] );
	...
	public function testBaseModelFindByIntrinsicWithInOperator() {
		$vm_ret = ca_objects::find( [ 'idno' => [ 'IN', [ 'TEST &amp; STUFF', 'INVALID VALUE', 'Another TEST' ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
	...
	public function testBaseModelFindByIntrinsicWithNotInOperator() {
		$vm_ret = ca_objects::find( [ 'idno' => [ 'NOT IN', [ 'INVALID VALUE', 'Another TEST' ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
		$this->assertContains( $this->opn_object_id, $vm_ret );

		$vm_ret = ca_objects::find( [ 'idno' => [ 'NOT IN', [ 'TEST & STUFF' ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
		$this->assertContains( $this->opn_object_id2, $vm_ret );

		$vm_ret = ca_objects::find( [ 'idno' => [ 'NOT IN', [ 'INVALID VALUE' ] ] ],
			[ 'purify' => false, 'returnAs' => 'ids' ] );
	...
	public function testBaseModelFindByIntrinsicWithBetweenOperator() {
		$vm_ret = ca_objects::find( [ 'extent' => [ 'BETWEEN', [ 1, 5 ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
		$this->assertContains( $this->opn_object_id2, $vm_ret );

		$vm_ret = ca_objects::find( [ 'extent' => [ 'BETWEEN', [ 3, 100 ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 2, $vm_ret );
		$this->assertContains( $this->opn_object_id, $vm_ret );
		...
		$vm_ret = ca_objects::find( [ 'extent' => [ 'BETWEEN', [ 8, 100 ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
		$this->assertContains( $this->opn_object_id, $vm_ret );

		$vm_ret = ca_objects::find( [ 'extent' => [ 'BETWEEN', [ 0, 2 ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 0, $vm_ret );
	}
	...
	public function testBaseModelFindByAttributeWithBetweenOperator() {
		$vm_ret = ca_objects::find( [ 'integer_test' => [ 'BETWEEN', [ 400, 600 ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
		...
		$vm_ret = ca_objects::find( [ 'integer_test' => [ 'BETWEEN', [ 0, 1000 ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
		...
		$vm_ret = ca_objects::find( [ 'integer_test' => [ 'BETWEEN', [ 5000, 100000 ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
	...
	public function testFindByPreferredLabelWithInOperator() {
		$vm_ret = ca_objects::find( [ 'preferred_labels' => [ 'name' => [ 'IN', [ 'Sound & Motion' ] ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
	...
	public function testFindByPreferredLabelWithNotInOperator() {
		$vm_ret = ca_objects::find( [ 'preferred_labels' => [ 'name' => [ 'NOT IN', [ 'Sound & Motion' ] ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
		$this->assertIsArray( $vm_ret );
		$this->assertCount( 1, $vm_ret );
		$this->assertContains( $this->opn_object_id2, $vm_ret );
		...
		$vm_ret = ca_objects::find( [
			'preferred_labels' => [
				'name' => [
					'NOT IN',
					[ 'INVALID VALUE', 'A Walk on the Wild Side' ]
				]
			]
		], [ 'purify' => true, 'returnAs' => 'ids' ] );
		$this->assertIsArray( $vm_ret );
	...
	public function testFindByNonPreferredLabelWithInOperator() {
		$vm_ret = ca_objects::find( [ 'nonpreferred_labels' => [ 'name' => [ 'IN', [ 'My test image' ] ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
	...
	public function testFindByContainerSubAttributeWithOperators() {
		...
		$vm_ret = ca_objects::find( [ 'external_link' => [ 'url_source' => [ 'IN', [ 'wikipedia' ] ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
		...
		$vm_ret = ca_objects::find( [ 'external_link' => [ 'url_source' => [ 'LIKE', '%source' ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
	...
	public function testLabelableFindByType() {
		$vm_ret = ca_objects::find( [ 'type_id' => 'image', 'preferred_labels' => [ 'name' => [ 'LIKE', 'Sound%' ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
		...
		$vm_ret = ca_objects::find( [
			'type_id'          => 'dataset',
			'preferred_labels' => [ 'name' => [ 'LIKE', 'Sound%' ] ]
		], [ 'purify' => true, 'returnAs' => 'ids' ] );
	...
	public function testBaseModelFindWithTypeRestrictions() {
		$vm_ret = ca_objects::find( [ 'object_id' => [ '>', 0 ] ],
			[ 'purify' => true, 'returnAs' => 'ids', 'restrictToTypes' => [ 'image' ] ] );
		...
		$vm_ret = ca_objects::find( [ 'object_id' => [ '>', 0 ] ],
			[ 'purify' => true, 'returnAs' => 'ids', 'restrictToTypes' => [ 'dataset' ] ] );
	...
	public function testLabelableFindWithAllowWildcards() {
		$vm_ret = ca_objects::find( [ 'preferred_labels' => [ 'name' => 'Sound%' ] ],
			[ 'allowWildcards' => true, 'purify' => true, 'returnAs' => 'ids', 'restrictToTypes' => [ 'image' ] ] );
	...
	public function testLabelableFindByIntrinsicListBooleanOr() {
		$vm_ret = ca_objects::find( [
			'acquisition_type_id' => 'gift',
			'preferred_labels'    => [ 'name' => [ 'LIKE', 'A walk%' ] ]
		], [ 'boolean' => 'OR', 'purify' => true, 'returnAs' => 'ids' ] );
	...
	public function testFindWithArrayOfValues() {
		$vm_ret = ca_objects::find( [ 'idno' => [ [ 'LIKE', 'TEST%' ], 'Another TEST' ] ],
			[ 'boolean' => 'OR', 'purify' => true, 'returnAs' => 'ids' ] );
		...
		$vm_ret = ca_objects::find( [ 'idno' => [ 'TEST & STUFF', 'Another TEST' ] ],
			[ 'boolean' => 'AND', 'purify' => true, 'returnAs' => 'ids' ] );
	...
	public function testLabelableFindByDateRangeValue() {
		$vm_ret = ca_objects::find( [ 'date' => [ 'dates_value' => '1954' ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
		...
		$vm_ret = ca_objects::find( [ 'date' => [ 'dates_value' => [ '5/1975' ] ] ],
			[ 'purify' => true, 'returnAs' => 'ids' ] );
